...
extern int	tree(struct node *, unsigned char *, unsigned char *);
extern void	numtree(struct node *);
extern void	prinval(int, int);
extern void	buildtbl(char *);
extern char	*strsave(char *);
extern char *spack(char *);
extern struct node	*sortroot(struct node *);
extern int	yylex(void);
extern void	yyerror(char *);

static void	buildtree(struct sym *, char **);
static int	legallink(char *);
...
map		: MAP
		  ...
		  style '(' STRING
			{
if (inamap) {
	fprintf(stderr, gettxt("kbdcomp:1",
		"Error: 'map' used inside a map, line %d\n"), linnum);
	exit(1);
}
				++curmap;
				maplist[curmap].mapname = strsave(textline);
if (! legalname((char *)maplist[curmap].mapname)) {
	fprintf(stderr, gettxt("kbdcomp:2",
		"Error: \"%s\": illegal map name, line %d\n"),
		...
		| LINK '(' STRING 
			{
				...
				maplist[curmap].map_max =
				    strlen((char *)maplist[curmap].maptext) + 1;
if (! legallink((char *)maplist[curmap].maptext)) {
	fprintf(stderr, gettxt("kbdcomp:3",
		"Error: \"%s\": illegal linkage spec, line %d.\n"),
	...
	++nerrors;
		...
		| XTERN '(' STRING
			{
				fprintf(stderr, gettxt("kbdcomp:4",
					"Error: \"extern\" is a reserved word.\n"));
				++nerrors;
			}
		 ')'
		;
...
expr		: DEFINE '(' STRING
			{
				if (s_find(textline)) {
					fprintf(stderr, gettxt("kbdcomp:5",
						"Error: redefinition of %s, line %d\n"),
						textline, linnum);
					++nerrors;
				}
 ...
 */
			if (!(curparent = s_find(textline))) {
				fprintf(stderr, gettxt("kbdcomp:6",
					"Error: Undefined function %s, line %d\n"),
					textline, linnum);
				++nerrors;
		  ...
		  '(' expr_tail
			...
			if (childp) {
				...
				fprintf(stderr, gettxt("kbdcomp:7",
					"Error: arguments to %s, line %d\n"),
		...
		| STRLIST '('
			...
			if (!(curparent = s_find("string"))) {
				fprintf(stderr,	gettxt("kbdcomp:8",
					"Error: Undefined function %s, line %d\n"),
					textline, linnum);
				++nerrors;
				curparent = s_create("orphan");
		  ...
		  expr_tail
			...
			if (childp) {
				if (strlen(childlist[0]) != strlen(childlist[1])) {
					fprintf(stderr, gettxt("kbdcomp:9",
"Error: `keylist' string length unequal, line %d\n"),
					linnum);
					++nerrors;
				}
				...
				fprintf(stderr,	gettxt("kbdcomp:10",
					"Error: arguments to `keylist', line %d\n"),
					linnum);
				++nerrors;
			}
			}
...
expr_tail	: STRING
		  ...
		  ')'
		| error
			{ fprintf(stderr, gettxt("kbdcomp:11",
				"Error: null expression, line %d\n"),
				linnum);
			  ++nerrors;
			  yyclearin; }
		;
...
static void
buildtree(struct sym *par, char **ch)
{
	int rval;
	int i;
	char *t;
 ...
 */
	...
	if (strlen(bttmp) >= KBDIMAX) {
		fprintf(stderr, gettxt("kbdcomp:12",
			"Error: input string too long: %s\n"), bttmp);
		++nerrors;
	}
	++ch;	/* point at result */
	rval = tree(root, (unsigned char *)bttmp,
		(unsigned char *)*ch);
	if (rval == E_DUP) {
		...
		if (dup_error)
			fprintf(stderr, gettxt("kbdcomp:15",
				" (already defined on/near line %d)"),
				dup_error);
...
do_strlist(void)
{
	...
	if (childp) {
		if (strlen(childlist[0]) != strlen(childlist[1])) {
			fprintf(stderr,	gettxt("kbdcomp:17",
				"Error: `strlist' string length unequal, line %d\n"),
			linnum);
			++nerrors;
			...
			x[0] = *childlist[0];
			y[0] = *childlist[1];
			buildtree((struct sym *) 0, (char **)&s[0]);
			++(childlist[0]); ++(childlist[1]);
		    }
		}
	} else {
		fprintf(stderr,	gettxt("kbdcomp:18",
			"Error: arguments to `strlist', line %d\n"),
			linnum);
		++nerrors;
	}
}

...
{
	struct sym *start;
	int left, right;	/* tokens */
	char *lstr, *rstr;	/* strings */
	...
	if (lexfp = fopen(PREDEF, "r")) {	/* see if any pre-defs */
		while ((left = yylex()) != ACCEPT) {
			...
			if ((right = yylex()) == ACCEPT) {
				fprintf(stderr, gettxt("kbdcomp:47",
"Warning: pre-definition entry mismatch; ignoring \"%s\"\n"), lstr);
				return;
			}
...
s_create(char *name)
{
	...
	fprintf(stderr, gettxt("kbdcomp:48",
		"Error: symbol table overflow (calloc failed!)\n"));
	exit(1);
...
s_find(char *name)
{
	...
	for (ptr = s_head->s_next; ptr; ptr = ptr->s_next) {
		if (! ptr->s_name)
			fprintf(stderr, gettxt("kbdcomp:49",
				"Internal error in s_find.\n"));
		else {
			/* return if match */
...
strsave(char *s)
{
	...
	fprintf(stderr, gettxt("kbdcomp:50",
		"Error: string-save overflow (calloc failed).\n"));
	exit(1);
	return (NULL);	/* keep gcc happy */
}
...
spack(char *s)
{
	...
	fprintf(stderr, gettxt("kbdcomp:51",
		"Error: string overflow (calloc failed).\n"));
	exit(1);
	return (NULL);	/* keep gcc happy */
}
...
struct sym *
s_value(char *s)	/* find symbol whose VALUE is "s" */
{
	struct sym *entry;

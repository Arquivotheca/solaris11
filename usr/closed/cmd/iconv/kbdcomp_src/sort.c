...
struct node *findprev(struct node *, struct node *);
static void	exproot(struct node *);
...
struct node *
sortroot(struct node *p)	/* the root */
{
	int chg;
	struct node *q, *t;
...
struct node *
findprev(struct node *p, struct node *who)
{
	while (p) {
		if (p->n_next == who)
			return (p);
...
static void
exproot(struct node *p)
{
	struct node *q;	/* temp var */

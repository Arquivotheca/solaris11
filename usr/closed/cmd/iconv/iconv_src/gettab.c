	...
	char *f_data_base, int no_composites)
{
	...
	(void) snprintf(f_name, sizeof (f_name),
	    "%s%s", d_data_base, file);
	fd = open(f_name, 0);
	if (fd < 0)  {
		(void) fprintf(stderr, gettext("Cannot access table.\n"));
		exit(1);
	}

	if (read(fd, &hdr, sizeof (struct kbd_header)) !=
	    sizeof (struct kbd_header)) {
		(void) fprintf(stderr, gettext("Can't read header.\n"));
		exit(1);
	}
	...
	if (!tb) {
		(void) fprintf(stderr, gettext("Out of Space(1)\n"));
		exit(1);
	}
	...
	for (i = 0; i < hdr.h_ntabs; i++) {
		...
		if ((j = read(fd, tb, sizeof (struct kbd_tab))) !=
			...
			(void) fprintf(stderr, gettext("Can't access table "
				"header %d\n"), errno);
			exit(1);
		}
		...
		if (tb->t_flag & KBD_ONE) {
			...
			if (!found)
				(void) lseek(fd, (long)256, SEEK_CUR);
			else {
		...
		if (tb->t_text) {
			if (!found)
				(void) lseek(fd, (long)tb->t_text, SEEK_CUR);
			else {
				tb->t_textp =
...
checkmagic(struct kbd_header *h)
{
	if (strncmp((const char *)h->h_magic, KBD_MAGIC,
		strlen(KBD_MAGIC)) != 0) {
		(void) fprintf(stderr,
		    gettext("bad magic (not a kbd object)\n"));
		exit(1);
	}
	if (h->h_magic[KBD_HOFF] > KBD_VER) {
		(void) fprintf(stderr,
		    gettext("Version mismatch; this is %d, "
	...
	char *f_data_base, int *found, char *i_table)
{
	...
	s = malloc(t->t_max);
	if (s == (char *)NULL) {
		(void) fprintf(stderr, gettext("Cannot Allocate Link Table, No "
			"Space\n"));
		exit(1);
	}
	if (read(ifd, s, t->t_max) != t->t_max) {
		(void) fprintf(stderr, gettext("Invalid Composite Map\n"));
		exit(1);
	}
	/*
	 * If s we
	...
	*s++ = '\0';
	if (i == (int)t->t_max) {
		(void) fprintf(stderr, gettext("Invalid Table\n"));
		exit(1);
	}
	...
	for (i = 1; i < j; i++) {
		...
		if (search_dbase(file, n_table, d_data_base,
			...
			(void) fprintf(stderr, gettext("Can't Access Composite "
				"Table\n"));
			exit(1);

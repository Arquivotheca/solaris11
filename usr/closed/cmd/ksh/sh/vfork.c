...
int	vfork_save()
{
	...
	fp->f_staksave = staksave();
	(void) stakfreeze(0);
	fp->f_st = st;
	fp->f_jobstat = job;
	job.pwlist = 0;
	fp->f_savearg = arg_use();
	fp->f_iotable = io_ftbl_save(&fp->f_niotable);
...
void	vfork_restore()
{
	...
	io_ftbl_restore(fp->f_iotable, fp->f_niotable);
	st = fp->f_st;
	job = fp->f_jobstat;
	arg_free(fp->f_savearg,0);
	save_fork = fp->f_save_fork;
	stakrestore(fp->f_staksave, fp->f_stakoff);
	free(fp);
}

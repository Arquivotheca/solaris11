...
int  hist_open()
{
	...
	if(!histname)
	{
		...
		if ((cp != NULL ? strlen(cp) : 0) + strlen(hist_fname) + 1 >
		    sizeof (hname)) {
			/* HOME directory too long */
			(void) fprintf(stderr,
				"%s: HOME directory path %s\n",
				sh.shname,
				(const char *)gettext(e_longname));
			exit(1);
		}
...
static void hist_trim(n)
{
	...
	nfd = fcntl(hist_ptr->fixfd, F_DUPFD, USERIO);
	if (nfd > MAXFILES && nfd <= NFILE) { /* reserved area ? */
		(void) close(nfd);
		nfd = fcntl(hist_ptr->fixfd, F_DUPFD, NFILE + 1);
	}
	fdo= io_renumber(hist_ptr->fixfd, nfd);
	hist_ptr->fixfd = fdo;
	hist_ptr = 0;

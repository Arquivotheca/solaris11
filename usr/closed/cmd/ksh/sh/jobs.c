...
/*
 * The following structure is used to keep the exit status which
 * can be discarded but need to hold until the exit status is requested.
 * The knownpid will be set, if $! has been referred. jobsave will be
 * allocated for up to CHILD_MAX number of processes, which is required by
 * XCU.
 */
struct jobsave
{
...
static	struct jobsave	*bck_list, *bck_list_tail;
static	int		bck_count;

static struct jobsave	*job_new_save();
static struct jobsave	*job_lookup_save();
static void		job_delete_save();
static void		job_clear_save();
...
void job_init(lflag)
{
	...
	if (JOBTTY != -1) {
		JOBTTY = io_movefd(JOBTTY);
		(void) fcntl(JOBTTY, F_SETFD, FD_CLOEXEC);
	} else {
...
#ifdef SIGTSTP
	if((pw->p_flag&P_STOPPED) || tcgetpgrp(JOBTTY) == sh.pid)
...
register struct process *pw;
{
	struct termios ottymode = my_stty;
	...
	if(pw && (pw->p_flag&P_SIGNALLED) && pw->p_exit!=SIGHUP)
	{
		...
		(void) tty_get(JOBTTY, &my_stty);

		if ((pw->p_flag & P_ASYNCPROC) &&
		    memcmp(&my_stty, &ottymode, sizeof (my_stty)) != 0) {
			/*
			 * While the process was running in background,
			 * it might have taken the terminal mode when
			 * (foreground) ksh was in tty_raw mode. In such
			 * case, original terminal mode has not been restored,
			 * but tty_raw mode was restored. Check the current
			 * mode see if it is tty_raw mode. If so, restore
			 * it back to the original mode.
			 */
			if (tty_is_raw(&my_stty)) {
				my_stty = ottymode;
				tty_set(JOBTTY, TCSANOW, &my_stty);
			}
		}
...
char *jobs[];
{
	...
	else while(job = *jobs++)
	{
		...
		if (job_bypid(pid) == (struct process *) NULL) {
			...
			if ((jp = job_lookup_save(pid)) != NULL) {
				if (jp->knownpid || pid == sh.bckpid)
					sh.exitval = jp->exitval;
				job_delete_save(jp);
			}
			continue;
		}
...
int
job_post(pid_t pid, int background)
{
	register struct process *pw;
	...
	job.pwlist = pw;
	pw->p_pid = pid;
	pw->p_flag = P_RUNNING|P_EXITSAVE;
	if (background)
		pw->p_flag |= P_ASYNCPROC;
...
job_wait(pid_t pid)
{
	...
	int		w;
#ifdef WUNTRACED
	int		waitflag = WUNTRACED | WCONTINUED;
#endif /* WUNTRACED */
	char		intr = 0;
	char		bgjob = 0;
#ifndef SIG_NORESTART
...
#   endif /* apollo */
	...
	while(1)
	{
		...
		{
			pw->p_flag &= ~(P_NOTIFY|P_STOPPED|P_SIGNALLED);
			pw->p_flag |= P_RUNNING;
			goto tryagain;
		}
		else
		...
		jobdone:
			/*
			 * Since it's myjob, no need to save the exit status.
			 */
...
register struct process *pwtop;
{
	...
	for(pw=pwtop; pw ; pw=pw->p_nxtproc)
	{
		...
		if ((pw->p_flag & P_EXITSAVE) != 0 &&
			(jp = job_new_save()) != NULL) {
			jp->pid = pw->p_pid;
			jp->exitval = pw->p_exit;
			jp->knownpid = (pw->p_flag & P_KNOWNPID) ? 1 : 0;
			if (pw->p_flag & P_SIGNALLED)
				jp->exitval |= SIGFAIL;
		}
		pw->p_flag &= ~(P_DONE|P_EXITSAVE|P_KNOWNPID);
		job.numpost--;
...
/*
 * allocate a new jobsave which is linked to the list.
 */
...
job_new_save()
{
	...
	/*
	 * Remember exit status for sysconf(_SC_CHILD_MAX)
	 * number of processes. If it is exceeded we recycle the oldest one.
	 * In an unlikely event that sysconf(_SC_CHILD_MAX) is less than
	 * CHILD_MAX we use CHILD_MAX to oblige with the standard.
	 */
	if (child_max == 0) {
		if ((child_max = (int)sysconf(_SC_CHILD_MAX)) < CHILD_MAX)
			child_max = CHILD_MAX;
	}
	if (bck_count == child_max) {
		jp = bck_list_tail;
		bck_list_tail = jp->prev;
		bck_list_tail->next = NULL;
	} else {
		if ((jp = new_of(struct jobsave, 0)) == NULL)
			return (NULL);
		bck_count++;
	}
...
/*
 * return the jobsave associated with given pid.
 */
static struct jobsave *
job_lookup_save(pid_t pid)
{
	struct jobsave *jp;

	for (jp = bck_list; jp; jp = jp->next) {
		if (jp->pid == pid)
			break;
	}
	return (jp);
...
/*
 * unlink and free given jobsave.
 */
static void
job_delete_save(struct jobsave *jp)
{
	if (jp->prev)
...
/*
 * clear all saved exit status.
 */
static void
job_clear_save()
{
	struct jobsave *jp, *tjp;
...
/*
 * mark the job as it has reference from the script, so that the exit
 * status will be saved.
 */
void
job_mark_save(pid_t pid)
{
	struct process *pw;
	struct jobsave *jp;
	...
	if ((pw = job_bypid(pid)) != NULL) {
		/*
		 * job is still running. make sure the exit status
		 * will be saved.
		 */
		pw->p_flag |= (P_EXITSAVE|P_KNOWNPID);
	} else if ((jp = job_lookup_save(pid)) != NULL) {
		jp->knownpid = 1;
	}
}

/*
 * discard the saved exit status or turn off P_EXITSAVE if it's still
 * alive so that it won't save the exit status.
 */
void
job_discard_save(pid_t pid)
{
	struct process *pw;
	struct jobsave *jp;

	if ((pw = job_bypid(pid)) != NULL) {
		if ((pw->p_flag & P_KNOWNPID) == 0) {
			/*  
			 * pid has no reference. So no need to save the exit
			 * status.
			 */	
			pw->p_flag &= ~P_EXITSAVE;
		}

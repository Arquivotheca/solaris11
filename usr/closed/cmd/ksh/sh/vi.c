...
#   ifdef FIONREAD
#	include	<sys/ioccom.h>	/* XXX - need _IOR definition */
#   endif /* FIONREAD */
...
#   define digit(c)	sh_iswdigit(c)
#   define is_print(c)	sh_iswprint(c)
...
#define	o_v_char	editb.e_ovchar		/* prev virtual[ocur_virt] */
...
static int last_cmd = 0;		/* last command */
...
static wchar_t	getcount();
static void	getaline();
static genchar	getrchar();
static int	mvcursor();
static void	pr_prompt();
static void	pr_asciistring();
 ...
 * shbuf - shell line buffer
 * nchar - number of chars to read
 */

int
vi_read(int fd, char *shbuf, unsigned int nchar)
{
	genchar c;			/* general variable */
	int i;			/* general variable */
	genchar term_char;		/* read() termination character */
	genchar prompt[PRSIZE+2];		/* prompt */
...
#   endif /* FIORDCHK */
	...
	if (i > 0) {
		shbuf[i] = '\0';
		i = ed_internal((char *)shbuf, virtual);
	}
		...
		if( i == 0 )
		{
			...
			esc_or_hang = 1;
			term_char = ESC;
			virtual[i--] = '\0';	/* null terminate line */
		}
...
cntlmode(void)
{
	...
	for (;;) {
		...
		case cntl('V'):
		{
			/* Message translation will need fix here. */
			register const char *p = &e_version[5];
			save_v();
			del_line(BAD);
			while(c = *p++)
				append((wchar_t)c,APPEND);
			refresh(CONTROL);
			ed_getchar();
			restore_v();
			break;
	...
	newhist:
			...
			/* another history line loaded, so
			   refresh() has to refresh entire line. */
			ocur_virt = INVALID;
			if( (last_virt = genlen(virtual) - 1) >= 0 )
				cur_virt = 0;
			else
				cur_virt = INVALID;
			break;
...
find_b:
        ...
        case '%':
	{
		...
		while ((tcur_virt <= last_virt) &&
			(!iswascii(virtual[tcur_virt]) ||
			strchr(paren_chars, (int)(virtual[tcur_virt]))
				==(char*)0))
				tcur_virt++;
		if(tcur_virt > last_virt )
			return(BAD);
		nextc = (int)(virtual[tcur_virt]);
...
#	define	v	regb
	...
	if( v<ocur_virt || ocur_virt==INVALID
		|| ( v==ocur_virt
			&& (!sh_iswprint(virtual[v])||!sh_iswprint(o_v_char))))
	{
		opflag = FALSE;
		p = 0;
		v = 0;
	...
	else
	{
		...
		p = ocur_phys;
		v = ocur_virt;
		if (!sh_iswprint(virtual[v]))
		{
			/*** avoid double ^'s ***/
			p += (sh_wcwidth(virtual[v]) - 1);
			++v;
		}
	}
	virtual[last_virt+1] = 0;
...
register int increment;
{
	...
	cur_window = cur_phys - first_wind;
	if( ocur_virt == INVALID || !sh_iswprint(c)
		|| !sh_iswprint(virtual[cur_virt])
		|| !sh_iswprint(o_v_char)
		|| !sh_iswascii(c) || sh_wcwidth(o_v_char)>1
		|| (increment==TRUE && (cur_window==w_size-1)
			|| !sh_iswprint(virtual[cur_virt+1])) )
	{
...
sync_cursor()
{
	...
	for(; v <= last_virt; ++p, ++v)
	{
		...
		else if (!iswascii(c) && sh_iswprint(c))
				p += (sh_wcwidth(c) - 1);
	}
...
addin:
	...
	case '=':		/** list file name expansions **/
		...
		++last_virt;
		virtual[last_virt] = 0;
		if( ed_expand(virtual, &cur_virt, &last_virt, c) )
		{
			last_virt = i;
			bell;
		}
	...
	case '_':		/** append last argument of prev command **/
		...
		{
			char tmpbuf[MAXLINE];
			genchar gtmpbuf[MAXLINE];
			genchar	w;
			char *pm;
			if(repeat_set==0)
				repeat = -1;
			pm = hist_word(tmpbuf,repeat);
#ifndef KSHELL
			if(pm==0)
			{
				bell;
	...
	case 'c':		/** change **/
		if( mode )
			c = lastmotion;
		else if (!iswascii(w = getcount(getchar())))
			return (BAD);
		else
			c = (int)(w);
chgeol:
...
register int i;
{
	return (sh_iswblank(virtual[i]));
}

int ismetach(i)
register int i;
{
	return (iswmeta(virtual[i]));
}

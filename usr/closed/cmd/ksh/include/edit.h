...
struct edit
{
	...
	genchar	*e_prompt;	/* pointer to buffer containing the prompt */
	...
	genchar	e_lbuf[LOOKAHEAD];/* pointer to look-ahead buffer */
...
#endif /* KSHELL */
	genchar	e_ovchar;	/* prev virtual[ocur_virt] */
};
...
extern struct edit editb;
    ...
    extern void ed_putchar(wchar_t);
    extern void ed_putascii(int);
    extern int ed_internal(char *, wchar_t *);
    extern int ed_external(wchar_t *, char *);
    extern void ed_ringbell(void);
    extern void ed_setup(int);
    extern void ed_flush(void);
    extern wchar_t	ed_getchar(void);
    extern int	ed_virt_to_phys(genchar*,genchar*,int,int,int);
    extern int	ed_window(void);
    extern void ed_ungetchar(wchar_t);
#   ifdef KSHELL
	extern int ed_macro(int);
	extern int ed_expand(genchar[],int*,int*,int);
#   endif /* KSHELL */
#else
    extern void ed_crlf();
    extern void ed_putchar();
    extern void ed_putascii();
    extern int ed_internal();
    extern int ed_external();

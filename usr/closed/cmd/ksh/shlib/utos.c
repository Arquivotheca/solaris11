...
/*Similar to above ltos(), but working with longlongs*/

char *lltos(sint,base)
longlong_t sint;
int base;
{
	register char *sp;
	register longlong_t l = (sint>=0?sint:-sint);
#ifdef pdp11
	sp = utos(l,base);
#else
	sp = ulltos((u_longlong_t)l,base);
#endif /* pdp11 */
	if(sint<0)
		*--sp = '-';
	return(sp);
...
/*Convert unsigned longlong to string.*/
char *ulltos(usint,base)
register int base;
#ifdef pdp11
 /* unsigned longs are not supported on pdp11 */
long usint;

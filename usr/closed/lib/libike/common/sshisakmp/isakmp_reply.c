...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_reply.c,v $
 *        $Author: ltarkkal $
 *        Creation          : 19:56 Jun  1 1998 kivinen
 *        Last Modification : 17:09 Mar  5 2003 kivinen
 *        Last check in     : $Date: 2003/12/10 17:05:57 $
 *        Revision number   : $Revision: 1.3.2.4 $
 *        State             : $State: Exp $
 *        Version           : 1.384
 *        
 *
 *        Description       : Isakmp policy manager reply functions.
 *
 *
 *        $Log: isakmp_reply.c,v $ *        
 *        
 *        
 *        
 *        
                                     ...
                                     void *context)
{
  ...
  /* Otherwise restart packet. */
  negotiation->lock_flags &= ~(SSH_IKE_NEG_LOCK_FLAG_WAITING_PM_REPLY);
  ike_new_connection_cb_done(context);
}
      ...
      negotiation->sa->server_context->isakmp_context->accelerator_short_name)
    {
      ...
      handle =
        ssh_ek_generate_accelerated_public_key(ic->external_key,
                                         ssh_csstr(ic->accelerator_short_name),

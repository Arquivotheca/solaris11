...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 06:28 May 22 1997 kivinen
...
#ifdef SUNWIPSEC
  ssh_free(context->default_natt_port);
#endif
  ssh_free(context);
}
                           ...
                           SshAuditContext audit_context)
{
...
#ifdef SUNWIPSEC
  if (params->default_natt_port != NULL)
    {
      context->default_natt_port = ssh_strdup(params->default_natt_port);
      SSH_DEBUG(4, ("params->default_natt_port = %s",
		    params->default_natt_port));
    }
  else
    context->default_natt_port = ssh_strdup(SSH_IKE_DEFAULT_NATT_PORT);
#endif

  if (context->default_ip == NULL ||
      context->default_port == NULL
#ifdef SUNWIPSEC
      || context->default_natt_port == NULL
#endif
	  )
    {
      ssh_free(context->default_ip);
  ...
  if (params->base_retry_limit > 0)
    {
      ...
      SSH_DEBUG(4, ("params->base_retry_limit = %d",
                    params->base_retry_limit));
    }
  else
  ...
  if (params->base_retry_timer > 0 || params->base_retry_timer_usec > 0)
    {
      ...
      SSH_DEBUG(4, ("params->base_retry_timer = %d.%06d",
                    params->base_retry_timer, params->base_retry_timer_usec));
    }
  else
    {
  ...
  if (params->base_retry_timer_max > 0 ||
      params->base_retry_timer_max_usec > 0)
    {
      ...
      SSH_DEBUG(4, ("params->base_retry_timer_max = %d.%06d",
                    params->base_retry_timer_max,
                    params->base_retry_timer_max_usec));
    }
  else
    {
  ...
  if (params->base_expire_timer > 0 || params->base_expire_timer_usec > 0)
    {
      ...
      SSH_DEBUG(4, ("params->base_expire_timer = %d.%06d",
                    params->base_expire_timer,
                    params->base_expire_timer_usec));
    }
  else
    {
  ...
  if (params->extended_retry_limit > 0)
    {
      ...
      SSH_DEBUG(4, ("params->extended_retry_limit = %d",
                    params->extended_retry_limit));
    }
  else
  ...
  if (params->extended_retry_timer > 0 ||
      params->extended_retry_timer_usec > 0)
    {
      ...
      SSH_DEBUG(4, ("params->extended_retry_timer = %d.%06d",
                    params->extended_retry_timer,
                    params->extended_retry_timer_usec));
    }
  else
    {
  ...
  if (params->extended_retry_timer_max > 0 ||
      params->extended_retry_timer_max_usec > 0)
    {
      ...
      SSH_DEBUG(4, ("params->extended_retry_timer_max = %d.%06d",
                    params->extended_retry_timer_max,
                    params->extended_retry_timer_max_usec));
    }
  else
    {
  ...
  if (params->extended_expire_timer > 0 ||
      params->extended_expire_timer_usec > 0)
    {
      ...
      SSH_DEBUG(4, ("params->extended_expire_timer = %d.%06d",
                    params->extended_expire_timer,
                    params->extended_expire_timer_usec));
    }
  else
    {
  ...
  context->isakmp_sa_mapping =
    ssh_adt_create_generic(SSH_ADT_MAP,
                           ...
                           SSH_ADT_SIZE, SSH_IKE_COOKIE_LENGTH * 2,
                           SSH_ADT_ARGS_END);
  ...
  context->isakmp_cookie_mapping =
    ssh_adt_create_generic(SSH_ADT_MAP,
                           ...
                           SSH_ADT_SIZE, SSH_IKE_COOKIE_LENGTH,
                           SSH_ADT_ARGS_END);
  ...
  if (!ike_default_groups_init(context))
    {
      SSH_DEBUG(4, ("default_groups_init() failed."));
      ssh_adt_destroy(context->isakmp_sa_mapping);
      ssh_adt_destroy(context->isakmp_cookie_mapping);
      ssh_adt_destroy(context->prime_mapping);
      ssh_free(context->default_ip);
...
void ssh_ike_stop_server(SshIkeServerContext server_context)
{
  ...
  context = server_context->isakmp_context;
  ...
  ssh_udp_destroy_listener(server_context->listener);
  ssh_free(server_context->server_name);
  ssh_free(server_context->server_port);
#ifdef SUNWIPSEC
  if (server_context->listener_natt != NULL)
    ssh_udp_destroy_listener(server_context->listener_natt);
  ssh_free(server_context->server_natt_port);
#endif
                                         ...
                                         void *sa_callback_context)
{
...
#ifdef SUNWIPSEC
  if (server_natt_port == NULL)
    server_natt_port = context->default_natt_port;
#endif 
  ...
  server->isakmp_context = context;
  server->server_name = ssh_strdup(server_name);
  server->server_port = ssh_strdup(server_port);
#ifdef SUNWIPSEC
  server->server_natt_port = ssh_strdup(server_natt_port);
#endif
  if (server->server_name == NULL || server->server_port == NULL
#ifdef SUNWIPSEC
      || server->server_natt_port == NULL
#endif
	  )
    {
      ssh_free(server->server_name);
      ssh_free(server->server_port);
#ifdef SUNWIPSEC
      ssh_free(server->server_natt_port);
#endif
      ssh_free(server);
      return NULL;
    }
  ...
  server->listener = ssh_udp_make_listener(server_name, server_port,
                                           NULL, NULL, NULL,
                                           ike_udp_callback,
                                           server);
...
#ifdef SUNWIPSEC
  /* if it's v6, don't bother with natt */
  if (strchr((char *)server_name, ':') != NULL){
      server->listener_natt = NULL;
      return server;
  }

  server->listener_natt = ssh_udp_make_listener(server_name, server_natt_port,
						NULL, NULL, NULL,
                                                ike_udp_callback,
                                                server);

  if (server->listener_natt == NULL){
      SSH_DEBUG(3, ("ssh_iskamp_start_server: ssh_udp_make_listener failed for natt"));
  ...
  if (!ssh_push_natt_mod(server->listener_natt)){
      SSH_DEBUG(3, ("ssh_iskamp_start_server: ssh_push_natt_mod failed"));
      ssh_udp_destroy_listener(server->listener);
      ssh_udp_destroy_listener(server->listener_natt);
      ssh_free(server->server_name);
      ssh_free(server->server_port);
      ssh_free(server->server_natt_port);
      ssh_free(server);
...
#ifdef SUNWIPSEC
Boolean
ssh_ike_enable_mac_bypass(SshIkeServerContext server)
{
  if (!ssh_set_mac_bypass(server->listener, TRUE))
    return (FALSE);

  if (server->listener_natt == NULL)
    return (TRUE);
  
  return ssh_set_mac_bypass(server->listener_natt, TRUE);
}
#endif /* SUNWIPSEC */
                                ...
                                void *notify_callback_context)
{
  ...
  char id_txt[255];
  SshIkeAttributeAuthMethValues auth_method = -1;
  int j, k, l;
  ...
  *negotiation_out = NULL;
  ...
  SSH_DEBUG(5, ("Start, remote_name = %s:%s, xchg = %d, flags = %08x",
  ...
  if (!ike_init_isakmp_sa(sa,
			  ...
			  packet_context,
#endif /* SUNWIPSEC */
			  context->server_name, context->server_port,
...
#ifdef SUNWIPSEC
  negotiation->use_natt_port = FALSE;
#endif
  ...
  negotiation->ike_pm_info->auth_method = auth_method;
  ...
  if (connect_flags & SSH_IKE_IKE_FLAGS_TRUST_ICMP_MESSAGES ||
      context->isakmp_context->trust_icmp_messages)
    {
      negotiation->ike_ed->listener =
        ssh_udp_make_listener(context->server_name,
                              context->server_port,
                              remote_name,
                              remote_port,
                              NULL,
                              ike_udp_callback_first,
                              negotiation);
  ...
  *negotiation_out = negotiation;
  ...
  SSH_DEBUG(5, ("SA = { %08x %08x - %08x %08x}, nego = %d",
                                      ...
                                      void *notify_callback_context)
{
  ...
  *negotiation_out = NULL;
  ...
  empty = ssh_custr("");
  SSH_DEBUG(5, ("Start, remote_name = %s:%s, flags = %08x",
                (remote_name == NULL ? empty : remote_name),
                remote_port, connect_flags));
                      ...
                      if (attrs.group_desc != group_descriptor)
                        {
                          SSH_IKE_DEBUG(3, negotiation,
                                        ("Proposal contained several "
                                         ...
                                         attrs.group_desc, group_descriptor));
                          goto invalid_arg_error;
                        }
                    }
                  else
  ...
  if (sa == NULL)
    {
      ...
      if (!ike_init_isakmp_sa(sa,
#ifdef SUNWIPSEC
			      NULL, /* XXX TBD no cfg mode labels */
#endif /* SUNWIPSEC */
			      context->server_name, context->server_port,
                              remote_name, remote_port,
                              SSH_IKE_MAJOR_VERSION, SSH_IKE_MINOR_VERSION,
                                       ...
                                       size_t notification_data_size)
{
  ...
  const unsigned char *empty;
#ifdef SUNWIPSEC
  SshUdpPacketContext packet_context = isakmp_sa_negotiation->packet_context;
#endif /* SUNWIPSEC */
  ...
  SSH_DEBUG(6, ("Sending notification to %s:%s",
		(remote_name == NULL ? empty : remote_name), remote_port));
  ...
  if (sa == NULL)
    {
      ...
      if (!ike_init_isakmp_sa(sa,
#ifdef SUNWIPSEC
			      isakmp_sa_negotiation->packet_context,
#endif /* SUNWIPSEC */
			      context->server_name, context->server_port,
                              remote_name, remote_port,
                              SSH_IKE_MAJOR_VERSION, SSH_IKE_MINOR_VERSION,
                                       ...
                                       size_t spi_size)
{
  ...
  SSH_DEBUG(6, ("Sending delete to %s:%s",
		(remote_name == NULL ? empty : remote_name), remote_port));
...
#ifdef SUNWIPSEC
/* 
 * Functions to convert various other error codes to strings 
 * without exporting the various tables publicly
 */
const char *ike_xchg_type_to_string(SshIkeExchangeType code)
{
  const char *str;

  str = ssh_find_keyword_name(ssh_ike_xchg_type_keywords, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
...
const char *ike_connect_error_to_string(SshIkeErrorCode code)
{
  ...
  str = ssh_find_keyword_name(ssh_ike_connect_keywords, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
...
const char *ike_encryption_alg_to_string(SshIkeAttributeEncrAlgValues code)
{
  ...
  str = ssh_find_keyword_name(ssh_ike_encryption_algorithms, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
...
const char *ike_hash_alg_to_string(SshIkeAttributeHashAlgValues code)
{
  ...
  str = ssh_find_keyword_name(ssh_ike_hash_algorithms, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
...
const char *ike_auth_method_to_string(SshIkeAttributeAuthMethValues code)
{
  ...
  str = ssh_find_keyword_name(ssh_ike_auth_method_keywords, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
...
const char *ike_cert_encoding_to_string(SshIkeCertificateEncodingType code)
{
  ...
  str = ssh_find_keyword_name(ike_cert_encoding_type_keywords, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
...
const char *policy_key_type_to_string(SshPolicyKeyType code)
{
  ...
  str = ssh_find_keyword_name(policy_key_type_keywords, code);
  if (str == NULL)
    str = "unknown";
  return str;
}
                   ...
                   char *description)
{
  ...
  char negotiation_index[SSH_IKE_STR_INT32_LEN];
  const char *initiator;
  const char *exchange_type;
  char message_id[SSH_IKE_STR_INT32_LEN];
  char cookies[SSH_IKE_COOKIE_LENGTH * 2 * 2 + 8];
  char local_ip_port[SSH_IKE_STR_IP_LEN + SSH_IKE_STR_INT32_LEN + 1],
                          ...
                          const unsigned char *buffer)
{
  ...
  if (len == 0)
  ...
  else
    {
      ...
      desc_len = desc_len - strlen(description);
                   ...
                   const char *txt)
{
  ...
  if (negotiation->sa == NULL ||
      ...
      negotiation->sa->server_context->isakmp_context->audit_context == NULL)
    return;
  ...
  ssh_audit_event(negotiation->sa->server_context->isakmp_context->
                  audit_context, event,
  ...
  if (negotiation->sa->server_context != new_server)
    {
      ...
      /* Update the local IP and port. */

      local_ip = ssh_strdup(new_server->server_name);
      if (local_ip == NULL)
        goto error;

      local_port = ssh_strdup(new_server->server_port);
      if (local_port == NULL)
        goto error;

...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_crypt.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 17:29 Feb 28 2002 kivinen
 *        Last Modification : 17:06 Mar  5 2003 kivinen
 ...
 *        
 *
 *        Description       : Isakmp encryption / decryption routines.
 *
 *
 *        $Log: isakmp_crypt.c,v $ *        
 *        
 *        
 *        
 *        
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Tried to find public key with invalid "
                    "auth_method : %d",
                    negotiation->ike_pm_info->auth_method);
#endif
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("No public key found");
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
  if (hash_name != NULL)
  ...
  switch (negotiation->ike_pm_info->auth_method)
    ...
    case SSH_IKE_VALUES_AUTH_METH_XAUTH_R_PRE_SHARED:
      ...
      SSH_DEBUG(6, ("Isakmp_find_private_key called with invalid "
                    ...
                    negotiation->ike_pm_info->auth_method));
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Tried to find private key with invalid "
                    "auth_method : %d",
                    negotiation->ike_pm_info->auth_method);
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("No private key found");
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
  return 0;
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("No pre shared key found");
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
  return 0;
                                             ...
                                             size_t *return_len)
{
  ...
  SSH_DEBUG(5, ("RSA decrypt: data[0..%d] = %08x %08x ...",
                len, SSH_IKE_GET32(data), SSH_IKE_GET32(data + 4)));
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Error selecting private key scheme: %.200s",
                     ssh_crypto_status_message(cret));
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
  ...
  if (len > ssh_private_key_max_decrypt_input_len(negotiation->ike_ed->
                                                  private_key))
    {
      ...
      SSH_IKE_NOTIFY_TEXT(negotiation, "Data length too large for private "
                          "key to decrypt");
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Data length too large for private "
                          "key to decrypt");
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
                                             ...
                                             size_t *return_len)
{
  ...
  SSH_DEBUG(5, ("RSA encrypt: data[0..%d] = %08x %08x ...",
                len, SSH_IKE_GET32(data), SSH_IKE_GET32(data + 4)));
  ...
  if (cret != SSH_CRYPTO_OK)
    {
      SSH_IKE_DEBUG(3, negotiation,
                    ("Error in ssh_public_key_select_scheme: %.200s",
                     ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Error selecting public key scheme: %.200s",
                     ssh_crypto_status_message(cret));
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
  ...
  if (len > ssh_public_key_max_encrypt_input_len(negotiation->ike_ed->
                                                 public_key))
    {
      ...
      SSH_IKE_NOTIFY_TEXT(negotiation, "Length too large for public "
                          "key to encrypt");
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Length too large for public key to encrypt: "
          "len = %d bytes, max = %d bytes", len,
          ...
          if (cret != SSH_CRYPTO_OK)
            {
              SSH_IKE_DEBUG(3, negotiation,
                            ("ssh_hash_allocate failed: %.200s",
                             ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
              ssh_policy_sun_info("Hash allocation failed: %.200s",
                             ssh_crypto_status_message(cret));
#endif
              return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
            }
                                                ...
                                                SshIkeNegotiation negotiation)
{
  ...
  if (!negotiation->ike_ed->async_return_done &&
      negotiation->ike_ed->async_return_data_len == 0)
    {
      /* XXX Check diffie hellman input size */
      /* XXX Check for p-1 and 1 */
      handle =
  ...
  if (cret != SSH_CRYPTO_OK)
    {
      SSH_IKE_DEBUG(3, negotiation, ("ssh_mac_allocate(1) failed: %.200s",
                                     ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
      ssh_policy_sun_info("MAC allocation failed: %.200s",
                             ssh_crypto_status_message(cret));
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }

  /* XXX get mac key length, the required function is missing from
     crypto library, so use the mac_length == never expand key */
  key_len = ssh_mac_length(ssh_mac_name(prf_mac));
  bl = ssh_mac_length(ssh_mac_name(prf_mac));
  if (bl < key_len)
    isakmp_sa->skeyid.skeyid_size = ((key_len / bl) + 1) * bl;
  else
      ...
      if (cret != SSH_CRYPTO_OK)
        {
          ...
          SSH_IKE_DEBUG(3, negotiation,
                        ("ssh_cipher_allocate failed: %.200s",
                         ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
          ssh_policy_sun_info("cipher allocation failed: %.200s",
                             ssh_crypto_status_message(cret));
#endif
          return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
        }
...
#ifdef SUNWIPSEC
          ssh_policy_sun_info("Block size > hash length");
#endif
      ...
      if (cret != SSH_CRYPTO_OK)
        {
          SSH_IKE_DEBUG(3, negotiation,
                        ("ssh_cipher_set_iv(e) failed: %.200s",
                         ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
          ssh_policy_sun_info("Setting cipher IV failed: %.200s",
                             ssh_crypto_status_message(cret));
#endif
          return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
        }
...
#ifdef SUNWIPSEC
          ssh_policy_sun_info("Could not find encryption IV when calculating "
              "ISAKMP S/Key ID");
#endif
          return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
        }

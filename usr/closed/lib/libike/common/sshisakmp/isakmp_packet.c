...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_packet.c,v $
 *        $Author: tmo $
 *
 *        Creation          : 14:48 Jul 30 1997 kivinen
 *        Last Modification : 23:13 Nov 13 2002 kivinen
 *        Last check in     : $Date: 2003/12/08 08:09:37 $
 *        Revision number   : $Revision: 1.2.2.3 $
 *        State             : $State: Exp $
 *        Version           : 1.1343
 *        
 *
 *        Description       : Isakmp packet functions module
 *
 *
 *        $Log: isakmp_packet.c,v $ *        
 *        
 *        
 *        
 *        
...
#define APPEND_INT32(b,d,l) do { \
    ...
    if (((d) & 0xffffffff) != (d)) \
      { \
        SSH_IKE_DEBUG(3, negotiation, \
                      ("Value %d exceeds field width (32 bit)", (d))); \
        return SSH_IKE_NOTIFY_MESSAGE_PAYLOAD_MALFORMED; \
      } \
    SSH_IKE_PUT32(buf, (d)); \
                                          ...
                                          SshBuffer buffer)
{
  ...
  unsigned char *p;
  int i, j, k, l, m;
  unsigned int fake_spi_len = 0;
  
  SSH_DEBUG(5, ("Start, SA = { 0x%08x %08x - %08x %08x } / %08x, nego = %d",
  ...
  for (i = 0; i < isakmp_packet->number_of_payload_packets; i++)
    {
      ...
      payload->payload_offset = len - SSH_IKE_PACKET_GENERIC_HEADER_LEN - fake_spi_len;
      ...
      switch (payload->type)
        ...
        case SSH_IKE_PAYLOAD_TYPE_SA:
          ...
          if ((payload->pl.sa.situation.situation_flags &
            ...
            {
              SSH_IKE_DEBUG(9, negotiation, ("labeled_domain_identifier "
                                             "= %d (0x%x)",
              ...
              if (payload->pl.sa.situation.situation_flags &
                  SSH_IKE_SIT_SECRECY)
                {
                  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Secrecy level",
                                       payload->pl.sa.situation.
                                       secrecy_level_length,
                                       payload->pl.sa.situation.
                                       secrecy_level_data);
                  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Secrecy bitmap",
                                       payload->pl.sa.situation.
              ...
              if (payload->pl.sa.situation.situation_flags &
                  SSH_IKE_SIT_INTEGRITY)
                {
                  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Integrity level",
                                       payload->pl.sa.situation.
                                       integrity_level_length,
                                       payload->pl.sa.situation.
                                       integrity_level_data);
                  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Integrity bitmap",
                                       payload->pl.sa.situation.
        ...
        case SSH_IKE_PAYLOAD_TYPE_KE:
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode KE: ke",
        ...
        case SSH_IKE_PAYLOAD_TYPE_ID:
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode ID: packet",
        ...
        case SSH_IKE_PAYLOAD_TYPE_HASH:
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode HASH: hash",
                               payload->payload_length,
        ...
        case SSH_IKE_PAYLOAD_TYPE_SIG:
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode SIG: sig",
                               payload->payload_length,
        ...
        case SSH_IKE_PAYLOAD_TYPE_NONCE:
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode NONCE: nonce",
                               payload->payload_length,
        ...
        case SSH_IKE_PAYLOAD_TYPE_N:
          SSH_IKE_DEBUG_PRINTF_BUFFER(9, negotiation,
                                      ...
                                      payload->pl.n.spi);
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode N: data",
        ...
        case SSH_IKE_PAYLOAD_TYPE_VID:
          SSH_IKE_DEBUG_BUFFER(9, negotiation, "Encode VID: vendor id",
                               payload->payload_length,
...
#ifdef SUNWIPSEC
        case SSH_IKE_PAYLOAD_TYPE_DRAFT_NATD:
        case SSH_IKE_PAYLOAD_TYPE_NATD:
	  APPEND_PTR(buffer, payload->pl.natd.natd_data,
	    payload->pl.natd.natd_length, len);
	  break;
	case SSH_IKE_PAYLOAD_TYPE_DRAFT_NATOA:
	case SSH_IKE_PAYLOAD_TYPE_NATOA:
	  APPEND_CHAR(buffer, IPSEC_ID_IPV4_ADDR, len);
	  APPEND_CHAR(buffer, 0, len);
	  APPEND_INT16(buffer, 0, len);
	  APPEND_PTR(buffer, payload->pl.natoa.natoa_data, 4, len);
	  break;
#endif		  
	case SSH_IKE_PAYLOAD_TYPE_PRV:
      ...
      SSH_IKE_PUT16(p, len - (payload->payload_offset +
                              SSH_IKE_PACKET_GENERIC_HEADER_LEN + fake_spi_len));

      SSH_DEBUG(9, ("Payload length = %d",
                    len - (payload->payload_offset +
                           SSH_IKE_PACKET_GENERIC_HEADER_LEN + fake_spi_len)));
      payload->payload_length = len - SSH_IKE_PAYLOAD_GENERIC_HEADER_LEN - fake_spi_len -
  ...
  isakmp_packet->length = len - fake_spi_len;
  ...
  if (isakmp_packet->flags & SSH_IKE_FLAGS_ENCRYPTION)
    {
      ...
      /* Pad to block length */
      while (((len - fake_spi_len) - SSH_IKE_PACKET_GENERIC_HEADER_LEN) %
  ...
  /* Update length to reflect non padded length */
  SSH_IKE_PUT32(p + isakmp_packet_len_offset, len - fake_spi_len);
  /* Take a copy before running finalizers. Finalizers only change the packet
     going out, the packet in memory stays unmodified. */
  isakmp_packet->encoded_packet = ssh_memdup(ssh_buffer_ptr(buffer) +
    fake_spi_len, len - fake_spi_len);
  ...
  if (isakmp_packet->flags & SSH_IKE_FLAGS_ENCRYPTION)
    {
      ...
      cret = ssh_cipher_get_iv(negotiation->ed->decryption_cipher,
                               negotiation->ed->cipher_iv);

      if (cret != SSH_CRYPTO_OK)
        {
          SSH_IKE_DEBUG(3, negotiation, ("ssh_cipher_get_iv failed: %.200s",
      ...
      cret = ssh_cipher_get_iv(negotiation->ed->encryption_cipher,
                               negotiation->ed->cipher_iv);
  ...
  SSH_IKE_PUT32(p, len - fake_spi_len);

  SSH_DEBUG(7, ("Final length = %d", len - fake_spi_len));
  return 0;
 error:
  return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
}
...
{
  size_t ind, packet_len, payload_len, bytelen;
  unsigned char *p;
  ...
  SSH_IKE_DEBUG(9, negotiation,
                ("Decode SA: doi = %d, sit = 0x%x",
                 isakmp_payload->pl.sa.doi,
                 isakmp_payload->pl.sa.situation.situation_flags));
  ...
  if (isakmp_payload->pl.sa.situation.situation_flags & SSH_IKE_SIT_SECRECY ||
      isakmp_payload->pl.sa.situation.situation_flags & SSH_IKE_SIT_INTEGRITY)
    {
      ...
      ind += 4;
      SSH_IKE_DEBUG(9, negotiation, ("labeled domain identifier = %d (0x%x)",
      ...
      if (isakmp_payload->pl.sa.situation.situation_flags &
          SSH_IKE_SIT_SECRECY)
        {
          ...
          ind += 2;
          isakmp_payload->pl.sa.situation.secrecy_level_data =
	    ssh_memdup(buffer + ind, isakmp_payload->pl.sa.situation.secrecy_level_length);
          if (isakmp_payload->pl.sa.situation.secrecy_level_data == NULL)
	    return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
	  ...
	  bytelen = (isakmp_payload->pl.sa.situation.secrecy_category_bitmap_length + 7) / 8;
          isakmp_payload->pl.sa.situation.secrecy_category_bitmap_data =
	    ssh_memdup(buffer + ind, bytelen);
	  if (isakmp_payload->pl.sa.situation.secrecy_category_bitmap_data == NULL)
	    return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
	  ind += bytelen;
      ...
      if (isakmp_payload->pl.sa.situation.situation_flags &
	    ...
	    ssh_memdup(buffer + ind, isakmp_payload->pl.sa.situation.integrity_level_length);
	  if (isakmp_payload->pl.sa.situation.integrity_level_data == NULL)
	    return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
	  ...
	  bytelen = (isakmp_payload->pl.sa.situation.integrity_category_bitmap_length + 7) / 8;
          isakmp_payload->pl.sa.situation.integrity_category_bitmap_data =
	    ssh_memdup(buffer + ind, bytelen);
          if (isakmp_payload->pl.sa.situation.integrity_category_bitmap_data == NULL)
	    return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
          ind += bytelen;
                                              ...
                                              unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode KE: ke",
                                              ...
                                              unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode ID: packet",
                       isakmp_payload->payload_length,
                                              ...
                                              unsigned char *buffer)
{
  ...
  if (negotiation->ed->compat_flags & SSH_IKE_FLAGS_IGNORE_CR_PAYLOADS)
    return 0;
                                                ...
                                                unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode HASH: hash",
                       isakmp_payload->payload_length,
                                               ...
                                               unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode SIG: sig",
                       isakmp_payload->payload_length,
                                                 ...
                                                 unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode NONCE: nonce",
                       isakmp_payload->payload_length,
                                             ...
                                             unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode N: data",
                       isakmp_payload->pl.n.notification_data_size,
                                               ...
                                               unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode VID: data",
                       isakmp_payload->payload_length,
                                               ...
                                               unsigned char *buffer)
{
  ...
  SSH_IKE_DEBUG_PRINTF_BUFFER(9, negotiation, ("Decode PRV[%d]: data",
 ...
 * Decode natd
 */
SshIkeNotifyMessageType ike_decode_payload_natd(SshIkeContext isakmp_context,
                                               SshIkeNegotiation negotiation,
                                               SshIkePayload isakmp_payload,
                                               unsigned char *buffer)
{
  isakmp_payload->pl.natd.raw_natd = buffer;
  isakmp_payload->pl.natd.natd_length = isakmp_payload->payload_length;
  isakmp_payload->pl.natd.natd_data = buffer;
  SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode NATD: data",
                       isakmp_payload->payload_length,
                       isakmp_payload->pl.natd.raw_natd);
  return 0;
}
 ...
 * Decode natoa
 */
SshIkeNotifyMessageType ike_decode_payload_natoa(SshIkeContext isakmp_context,
                                               SshIkeNegotiation negotiation,
                                               SshIkePayload isakmp_payload,
                                               unsigned char *buffer)
{
	if (buffer[0] != IPSEC_ID_IPV4_ADDR)
		return SSH_IKE_NOTIFY_MESSAGE_ATTRIBUTES_NOT_SUPPORTED;

	isakmp_payload->pl.natoa.raw_natoa = buffer;
	isakmp_payload->pl.natoa.natoa_length = 
	   isakmp_payload->payload_length - 4;
	isakmp_payload->pl.natoa.natoa_data = buffer + 4;
	memcpy(&isakmp_payload->pl.natoa.natoa_addr, (buffer + 4), 4);
	SSH_IKE_DEBUG_BUFFER(9, negotiation, "Decode NATOA:",
	    isakmp_payload->payload_length,
	    isakmp_payload->pl.natd.raw_natd);
                                           ...
                                           unsigned char *buffer)
{
  ...
  switch (isakmp_payload->type)
    ...
    case SSH_IKE_PAYLOAD_TYPE_DRAFT_NATD:
    case SSH_IKE_PAYLOAD_TYPE_NATD:
      ret = ike_decode_payload_natd(isakmp_context, negotiation,
          isakmp_payload, buffer);
      break;
    case SSH_IKE_PAYLOAD_TYPE_DRAFT_NATOA:
    case SSH_IKE_PAYLOAD_TYPE_NATOA:
      ret = ike_decode_payload_natoa(isakmp_context, negotiation,
	  isakmp_payload, buffer);
      break;
#endif	  
#ifdef SSHDIST_ISAKMP_CFG_MODE
                                          ...
                                          SshBuffer buffer)
{
  ...
  *isakmp_packet_out = NULL;
  ...
  SSH_DEBUG(5, ("Start, SA = { %08x %08x - %08x %08x} / %08x, nego = %d",
  ...
  if (isakmp_packet->major_version != SSH_IKE_MAJOR_VERSION ||
      ...
      isakmp_sa->isakmp_negotiation->ike_pm_info->major_version)
    {
      SSH_IKE_DEBUG(3, negotiation, ("Invalid major version = %d",
                                    isakmp_packet->major_version));
      ssh_ike_audit(negotiation, SSH_AUDIT_IKE_INVALID_ISAKMP_VERSION,
      ...
      ssh_ike_remove_isakmp_sa(negotiation,
  ...
  if (first_payload_type >= SSH_IKE_PAYLOAD_TYPE_MAX)
    {
...
ok:
  ...
  if (isakmp_packet->exchange_type != negotiation->exchange_type)
    {
      SSH_IKE_DEBUG(3, negotiation,
  ...
  if (isakmp_packet->flags & SSH_IKE_FLAGS_ENCRYPTION)
    {
      ...
      if (negotiation->ed->decryption_cipher == NULL)
        {
          SSH_IKE_DEBUG(3, negotiation,
  ...
  while (next_payload_type != SSH_IKE_PAYLOAD_TYPE_NONE)
	...
	case SSH_IKE_PAYLOAD_TYPE_DRAFT_NATD:
	case SSH_IKE_PAYLOAD_TYPE_NATD:
	  ptr = &isakmp_packet->first_natd_payload;
	  break;
	case SSH_IKE_PAYLOAD_TYPE_DRAFT_NATOA:
	case SSH_IKE_PAYLOAD_TYPE_NATOA:
	  ptr = &isakmp_packet->first_natoa_payload;
	  break;
#endif		  
#ifdef SSHDIST_ISAKMP_CFG_MODE
        case SSH_IKE_PAYLOAD_TYPE_ATTR:
                                         ...
                                         size_t len)
{
...
#ifdef DEBUG_LIGHT
  {
    ...
    SSH_IKE_DEBUG(9, negotiation, ("Decoded ID = %s", buffer));
  }
#endif
  return 0;
}
                                         ...
                                         size_t *return_len)
{
...
#ifdef DEBUG_LIGHT
  {
    ...
    SSH_IKE_DEBUG(9, negotiation, ("Encoding ID = %s", buf));
  }
#endif
  ...
  for (i = 0; i < isakmp_packet->number_of_payload_packets; i++)
    {
      ...
      switch (isakmp_payload->type)
        {
        case SSH_IKE_PAYLOAD_TYPE_SA:
	  ssh_free(isakmp_payload->pl.sa.situation.secrecy_level_data);
	  ssh_free(isakmp_payload->pl.sa.situation.secrecy_category_bitmap_data);
	  ssh_free(isakmp_payload->pl.sa.situation.integrity_level_data);
	  ssh_free(isakmp_payload->pl.sa.situation.integrity_category_bitmap_data);

...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_finalize.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 17:29 Feb 28 2002 kivinen
 *        Last Modification : 23:09 Oct 21 2002 kivinen
 ...
 *        
 *
 *        Description       : Output finalize routines
 *
 *
 *        $Log: isakmp_finalize.c,v $ *        
 *        
 *        
 *        
 *        
                                               ...
                                               SshIkePayload payload)
{
  ...
  SSH_DEBUG(5, ("Hash[0..%d] = %08x %08x ...",
                hash_len, SSH_IKE_GET32(hash),
                SSH_IKE_GET32(hash + 4)));
  return 0;
}
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Failed to get private key info: %.200s",
                                     ssh_crypto_status_message(cret));
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
  ...
  if (handle != NULL)
    {
      ...
      ssh_fatal("XXX Started async signing operation. Not supported for "
                "revised hash yet");

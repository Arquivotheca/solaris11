...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_id.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 13:30 Jun 16 1998 tmo
 *        Last Modification : 23:12 Nov 13 2002 kivinen
 *        Last check in     : $Date: 2003/11/28 20:00:47 $
 *        Revision number   : $Revision: 1.7.2.1 $
 *        State             : $State: Exp $
 *        Version           : 1.309
 *        
 *        Description       : Isakmp PayloadID handling functions
 *
 *        $Log: isakmp_id.c,v $ *        
 *        
 *        
                                             ...
                                             SshIkePayloadID id)
{
  ...
  if (SSH_IP_IS4(&temp_addr) != is_ipv4)
    goto fail;
  SSH_IP_ENCODE(&temp_addr, destination, size);
  return id;
...
#define IKE_TRIM_WHITESPACE(s) \
  do { \
    unsigned char *__p; \
    for (__p = (s) + strlen(ssh_csstr(s)) - 1; \
         __p >= (s) && isspace(*__p); \
         __p--) ; \
    __p[1] = '\0'; \
  } while (0)
                                              ...
                                              size_t *bytes_used)
{
  ...
  *protoname++ = '\0';
  ...
  if (strcmp(ssh_csstr(protoname), ""))
    {
      /* XXX temporary casts until library API is changed XXX */
      newp->protocol_id = ssh_find_keyword_number(ssh_ike_protocol_id_keywords,
                                                  ssh_csstr(protoname));
      if (newp->protocol_id == -1)
        goto fail;
    }
  if (strcmp(ssh_csstr(portname), ""))
    {
      unsigned char *range_end = ssh_ike_split_string(portname, '-');

      IKE_SKIP_WHITESPACE(range_end);
      IKE_TRIM_WHITESPACE(portname);
      newp->port_number = (int)strtol(ssh_csstr(portname), (char **)NULL, 0);

      if (range_end != portname)
        newp->port_range_end = (int)strtol(ssh_csstr(range_end), (char**)NULL,
                                           0);
      else
  ...
  if (bytes_used)
  ...
  else
    {
      ...
      dh = data + strlen(ssh_csstr(data)) - 1;
      if (*dh != ')')
        goto fail;
      *dh = '\0';
    }
  ...
  switch (newp->id_type)
    {
    case IPSEC_ID_FQDN:
      newp->identification_len = strlen(ssh_csstr(data));
    ...
    case IPSEC_ID_IPV4_ADDR_SUBNET:
      {
        ...
        if (newp == NULL)
          goto id_fail;
        masklen = (int)strtol(ssh_csstr(p), (char **)NULL, 10);
        if (masklen > 32)
          goto fail;
                           ...
                           SshIkePayloadID id)
{
  ...
  const char *type;
  const char *proto;
  char tempbuf[20], portbuf[20], *start = buffer;
...
#define SSH_MIN(_a, _b) (((_a) < (_b)) ? (_a) : (_b))
  ...
  if (!id)
    {
      ssh_snprintf(buffer, buflen, "unknown(any:0,[0..0]=)");
      return start;
    }
  ...
  proto = ssh_find_keyword_name(ssh_ike_protocol_id_keywords,
                                id->protocol_id);
  if (proto == NULL)
    {
      proto = tempbuf;
  ...
  if (id->identification_len == 0)
    ssh_snprintf(buffer, buflen, "%s(%s:%s,", type, proto, portbuf);
  else
    ssh_snprintf(buffer, buflen, "%s(%s:%s,[0..%d]=", type, proto,
  ...
  switch (id->id_type)
    ...
    case IPSEC_ID_USER_FQDN:
      ...
      break;
    case IPSEC_ID_IPV4_ADDR:
      dlen = ssh_snprintf(buffer, buflen-1, "%u.%u.%u.%u",
                          data->ipv4_addr[0],
                          data->ipv4_addr[1],
                          data->ipv4_addr[2],
                          data->ipv4_addr[3]);
    ...
    case IPSEC_ID_IPV4_ADDR_SUBNET:
      ...
      if (i != 4 && j != 8)
        {
          dlen = ssh_snprintf(buffer, buflen-1,
        ...
        {
          dlen = ssh_snprintf(buffer, buflen-1, "%u.%u.%u.%u/%d",
                              data->ipv4_addr_subnet[0],
                              data->ipv4_addr_subnet[1],
                              data->ipv4_addr_subnet[2],
                              data->ipv4_addr_subnet[3],
      ...
      break;
    case IPSEC_ID_IPV4_ADDR_RANGE:
      dlen = ssh_snprintf(buffer, buflen-1, "%u.%u.%u.%u-%u.%u.%u.%u",
    ...
    case IPSEC_ID_IPV6_ADDR:
      ...
      dlen = ssh_snprintf(buffer, buflen, "%@",
    ...
    case IPSEC_ID_IPV6_ADDR_SUBNET:
      ...
      if (i != 16 && j != 8)
      ...
      else
        {
          ...
          dlen = ssh_snprintf(buffer, buflen, "%@/%d",
                              ssh_ipaddr_render, &ip1,
                              netmask_len);
        }
      break;
    ...
    case IPSEC_ID_DER_ASN1_GN:
        ...
        if (ssh_dn_decode_der(data->asn1_data, id->identification_len,
                              ...
                              NULL))
          {
            if (ssh_dn_encode_ldap(&dn, &ldap))
              {
                strncpy(buffer, ldap, buflen);
                dlen = ssh_strnlen((char *)ldap, buflen);
                ssh_free(ldap);
              }
          }
        ssh_dn_clear(&dn);
      ...
      for (i = 0; (unsigned)i < len; i++)
        dlen = ssh_snprintf(buffer + i * 3, 4, "%02x ", data->asn1_data[i]);
      break;
#endif /* SSHDIST_ISAKMP_AUTH_CERT */
    case IPSEC_ID_KEY_ID:
      if (len > buflen / 3 - 2)
        len = buflen / 3 - 2;
      for (i = 0; (unsigned)i < len; i++)
        dlen += ssh_snprintf(buffer + i * 3, 4, "%02x ", data->key_id[i]);
      break;
#ifdef SSHDIST_IKE_ID_LIST
    case IPSEC_ID_LIST:
      {
            ...
            for (cnt = 0; cnt < data->id_list_number_of_items; cnt++)
              {
                if (cnt != 0)
                  {
                    dlen += ssh_snprintf(buffer + dlen, buflen - dlen, ", ");
                  }
...
int ssh_ike_id_render(char *buf, int buf_size, int precision, void *datum)
{
  ...
  (void) ssh_ike_id_to_string(buf, buf_size, id);
  ...
  *port = id->port_number;
  ...
  switch (id->id_type)
    ...
    case IPSEC_ID_FQDN:
      if (!name1 || *name1_len == 0)
        return FALSE;
      strncpy(ssh_sstr(name1), ssh_csstr(id->identification.fqdn), *name1_len);
      n1 = name1;
      break;
    case IPSEC_ID_USER_FQDN:
      if (!name1 || !name2 || *name1_len == 0 || *name2_len == 0)
        return FALSE;
      p = at = strchr(ssh_csstr(id->identification.user_fqdn), '@');
      if (!at)
        return FALSE;
      *at++ = '\0';
      strncpy(ssh_sstr(name1), ssh_csstr(id->identification.user_fqdn),
              *name1_len);
      strncpy(ssh_sstr(name2), at, *name2_len);
    ...
    case IPSEC_ID_DER_ASN1_GN:
      {
        ...
        for (i = 0; i < len; i++)
          /* XXX temporary casts until library API is changed XXX */
          ssh_snprintf(ssh_sstr(name1) + i * 3, 4, "%02x ",
...
#undef SSH_MIN
  ...
  if (n1)
    *name1_len = strlen(ssh_csstr(n1));
  if (n2)
    *name2_len = strlen(ssh_csstr(n2));
                          ...
                          const unsigned char *name2)
{
  ...
  switch (id->id_type)
    ...
    case IPSEC_ID_USER_FQDN:
      ...
      len = strlen(ssh_csstr(name1)) + strlen(ssh_csstr(name2)) + 2;
      id->identification.user_fqdn = ssh_malloc(len);
      if (id->identification.user_fqdn == NULL)
        return FALSE;
      ssh_snprintf((char *)id->identification.user_fqdn, len,
                   "%s@%s", name1, name2);
    ...
    case IPSEC_ID_IPV4_ADDR_SUBNET:
      ...
      if (!ssh_inet_strtobin(name1, id->identification.ipv4_addr_subnet,
                             &datalen))
        return FALSE;
      if (strcmp(ssh_csstr(name2), "255.255.255.255"))
        {
    ...
    case IPSEC_ID_IPV6_ADDR_SUBNET:
      ...
      if (strchr(ssh_csstr(name2), ':'))
      ...
      else
        {
          ...
          masklen = (int) strtol(ssh_csstr(name2), NULL, 10);
          if (masklen > 128)
            return FALSE;

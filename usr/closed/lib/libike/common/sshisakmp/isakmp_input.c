...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_input.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 14:48 Jul 30 1997 kivinen
 *        Last Modification : 17:08 Mar  5 2003 kivinen
 ...
 *
 *
 *        Description       : Isakmp state machine input functions module
 *
 *
 *        $Log: isakmp_input.c,v $ *
 *
 *
 *
 *
...
/* XXX add check to verify that transform numbers grow linearly from 1..max */
                                          ...
                                          SshIkeStateMachine state)
{
  ...
  if (sa_r->situation.situation_flags & SSH_IKE_SIT_SECRECY ||
    ...
    {
      SSH_IKE_DEBUG(3, negotiation, ("Unsupported situation : %x",
                                     sa_r->situation.situation_flags));
      ssh_ike_audit(negotiation, SSH_AUDIT_IKE_INVALID_SITUATION,
                    "Situation field contains flags that are not supported");
      SSH_IKE_NOTIFY_DATA(negotiation, SSH_IKE_PAYLOAD_TYPE_SA,
                          isakmp_input_packet->first_sa_payload->payload_start,
                                    ...
                                    SshIkeStateMachine state)
{
  ...
  if ((&id->pl.id)->id_type == IPSEC_ID_LIST &&
    ...
    ike_register_item(isakmp_input_packet,
                                      ...
                                      SshIkeStateMachine state)
{
  ...
  SSH_DEBUG(5, ("Start, hash[0..%d] = %08x %08x ...",
                pl->payload_length,
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Hash payload data does not match");
#endif
                                          ...
                                          SshIkeStateMachine state)
{
  ...
  SSH_DEBUG(5, ("Start, key_hash[0..%d] = %08x %08x ...",
                pl->payload_length,
                SSH_IKE_GET32(pl->pl.hash.hash_data),
                SSH_IKE_GET32(pl->pl.hash.hash_data + 4)));
                                     ...
                                     SshIkeStateMachine state)
{
  ...
  SSH_DEBUG(5, ("Start, sig[0..%d] = %08x %08x ...",
                sig->payload_length,
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Failed to get public key info: %.200s",
                                     ssh_crypto_status_message(cret));
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
      ...
      if (cret != SSH_CRYPTO_OK)
        {
          SSH_IKE_DEBUG(3, negotiation,
                        ("ssh_public_key_select_scheme failed: %.200s",
                         ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
          ssh_policy_sun_info("Failed to select public key scheme: %.200s",
                                     ssh_crypto_status_message(cret));
#endif
          return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
        }
                                       ...
                                       SshIkeStateMachine state)
{
  ...
  SSH_DEBUG(5, ("Start, nonce[0..%d] = %08x %08x ...",
                pl->payload_length,
...
SshIkeNotifyMessageType ike_st_i_qm_sa_proposals(SshIkeContext isakmp_context,
                                                 ...
                                                 SshIkeStateMachine state)
{
  SshIkeNegotiation isakmp_neg;
  SshIkePayload pl;
  int i, sa_count;

  SSH_DEBUG(5, ("Start"));
  isakmp_neg = isakmp_sa->isakmp_negotiation;
  ...
  while (pl != NULL)
    {
      SSH_DEBUG(5, ("Start, doi = %d, protocol = %d, code = %s (%d), "
                    "spi[0..%d] = %08x %08x ..., data[0..%d] = %08x %08x ...",
      ...
      if (pl->pl.n.protocol_id == SSH_IKE_PROTOCOL_ISAKMP)
        {
          ...
          /* XXX, we should propably try to check if the spi matches
             any other IKE sa we have */
               ...
               pl->pl.n.protocol_id == SSH_IKE_PROTOCOL_IPCOMP)
        {
          ...
          /* XXX, we should propably also here try to verify that this
             notification is really for this SA */
        }
      else
        {
  ...
  for (pl = isakmp_input_packet->first_d_payload;
       ...
       pl = pl->next_same_payload)
    {
      SSH_DEBUG(5, ("Start, doi = %d, protocol = %d, "
                    "spis[0..%d][0..%d] = [%08x %08x ...]",
                    pl->pl.d.doi, pl->pl.d.protocol_id,
                    pl->pl.d.number_of_spis,
                    pl->pl.d.spi_size,
                    SSH_IKE_GET32(pl->pl.d.spis[0]),
                    SSH_IKE_GET32(pl->pl.d.spis[0] + 4)));
              ...
              if (sa == NULL)
                {
                  ...
                  ssh_audit_event(isakmp_context->audit_context,
...
#ifdef SUNWIPSEC
  /* Treat p1_use_natt like a boolean, even though it's 0, 1, or 2. */
  if (negotiation->ike_pm_info->p1_use_natt &&
      isakmp_sa->sa_natt_state == NATT_STATE_INIT){
      isakmp_sa->sa_natt_state = NATT_STATE_VID;
      negotiation->ike_pm_info->p1_natt_state = isakmp_sa->sa_natt_state;
  }
#endif
  return 0;
}
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Packet was sent unencrypted");
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
}
...
SshIkeNotifyMessageType ike_st_i_natd(SshIkeContext isakmp_context,
                                     SshIkePacket isakmp_input_packet,
                                     SshIkeSA isakmp_sa,
                                     SshIkeNegotiation negotiation,
                                     SshIkeStateMachine state)
{
  ...
  if (isakmp_sa->sa_natt_state < NATT_STATE_VID){
      isakmp_sa->sa_natt_state = NATT_STATE_NEVER;
      negotiation->ike_pm_info->p1_natt_state = isakmp_sa->sa_natt_state;
      return 0;
  }

  natd = isakmp_input_packet->first_natd_payload;
  ...
  /* I am... */
  if (!ssh_ipaddr_parse(&ipa, negotiation->ike_pm_info->local_ip))
  ...
  SSH_IP4_ENCODE(&ipa, &addr);
  pre_port = atoi((char *)negotiation->ike_pm_info->local_port);
  port = (unsigned short)(pre_port & 0x0000FFFF);
  port = htons(port);

  hash_buf = ssh_malloc(256);
  if (hash_buf == NULL) {
     return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
  }

  if (ike_calc_natd_hash(isakmp_sa, negotiation, addr, port, hash_buf,
			 &hash_len)){

      negotiation->ike_pm_info->p1_natt_state =
	  isakmp_sa->sa_natt_state = NATT_STATE_NEVER;
  ...
  /* is the hash what we think it should be */
  if (hash_len != natd->pl.natd.natd_length){
      SSH_DEBUG(1, ("unequal hash lengths %d != %d", hash_len,
		    natd->pl.natd.natd_length));
  ...
  SSH_IKE_DEBUG_BUFFER(3, negotiation, "NATD IN",hash_len,
		       natd->pl.natd.natd_data);

  if (memcmp(hash_buf, natd->pl.natd.natd_data, hash_len) != 0){
      /* address isn't an outside address */
      negotiation->ike_pm_info->p1_natt_state =
	  isakmp_sa->sa_natt_state = NATT_STATE_NATD;

      /* Because of this, indicate so on the Phase I state. */
      negotiation->ike_pm_info->behind_nat = _B_TRUE;

      ssh_free(hash_buf);
      return 0;
  }

  natd = natd->next_same_payload;
  ...
  /* you are... */
  if (!ssh_ipaddr_parse(&ipa, negotiation->ike_pm_info->remote_ip))
	  return SSH_IKE_NOTIFY_MESSAGE_NO_SA_ESTABLISHED;
  ...
  SSH_IP4_ENCODE(&ipa, &addr);
  pre_port = atoi((char *)negotiation->ike_pm_info->remote_port);
  port = (unsigned short)(pre_port & 0x0000FFFF);
  port = htons(port);

  SSH_DEBUG(5, ("natd_in ad:%s=%x p:%x", negotiation->ike_pm_info->remote_ip,
				addr, port));

  /* figure out natd hash as we see it */
  if (ike_calc_natd_hash(isakmp_sa, negotiation, addr, port,
			 hash_buf, &hash_len)){
  ...
  while (natd != NULL)
  {
	  ...
	  if (memcmp(hash_buf, natd->pl.natd.natd_data, hash_len) == 0){
		  /* address is an outside address */

		  ssh_policy_sun_info("NAT-Discovery - not a NAT-T connection");
  ...
  SSH_DEBUG(1, ("nat-t enabled"));
  ssh_policy_sun_info("NAT-Discovery - using NAT-T");
...
SshIkeNotifyMessageType ike_st_i_natoa(SshIkeContext isakmp_context,
                                     SshIkePacket isakmp_input_packet,
                                     SshIkeSA isakmp_sa,
                                     SshIkeNegotiation negotiation,
                                     SshIkeStateMachine state)
{
  SshIkePayload natoa;
  SshIpAddrStruct ipa;
  Boolean equal_extra_natoa;

  natoa = isakmp_input_packet->first_natoa_payload;

  if (natoa == NULL)
      return 0;

  if (isakmp_sa->sa_natt_state < NATT_STATE_PORT) {
      return 0;
  }
  /* only two */
  SSH_DEBUG(5, ("NAT0A[0..%d] = %08x %08x ...",
		natoa->payload_length,
		SSH_IKE_GET32(natoa->pl.vid.vid_data),
		SSH_IKE_GET32(natoa->pl.vid.vid_data + 4)));

  if (negotiation->qm_pm_info->natt_init_ip != 0) {
	  /*
	   * If we get here, we've already set natt_init_ip, and this is most
	   * likely a NAT-OA payload on the 3rd packet in the Quick Mode
	   * exchange.  Previous versions of Solaris mistakenly send this
	   * payload on both packet 1 and packet 3 of the 3-packet Quick Mode
	   * exchange.
	   */
	  equal_extra_natoa =
	      (natoa->pl.natoa.natoa_addr ==
		  negotiation->qm_pm_info->natt_init_ip);
	  ssh_policy_sun_info("Received NAT-OA (%s) on 3rd QM packet.\n",
	      equal_extra_natoa ? "equal" : "inequal");
	  /* NOTE:  We can return a payload error here in the future. */
	  if (!equal_extra_natoa)
		  return 0;
  }

  /* pull the address and stash it */
  negotiation->qm_pm_info->natt_init_ip = natoa->pl.natoa.natoa_addr;

  natoa = natoa->next_same_payload;

  if (natoa == NULL) {
      negotiation->qm_pm_info->natt_init_ip = 0;
      return SSH_IKE_NOTIFY_MESSAGE_NO_SA_ESTABLISHED;
  }

  negotiation->qm_pm_info->natt_resp_ip = natoa->pl.natoa.natoa_addr;
  if (negotiation->qm_pm_info->other_port)
      ssh_free(negotiation->qm_pm_info->other_port);

  negotiation->qm_pm_info->other_port = ssh_strdup(isakmp_sa->sa_other_port);

  if (negotiation->qm_pm_info->other_port == NULL)
    return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;

  return 0;
}
SshIkeNotifyMessageType
ike_st_i_natt_portjump(SshIkeContext isakmp_context,
					   SshIkePacket isakmp_input_packet,
					   SshIkeSA isakmp_sa,
					   SshIkeNegotiation negotiation,
					   SshIkeStateMachine state)
{
    ...
    SSH_DEBUG(1,("ike_st_i_natt_portjump: %d",
		 isakmp_sa->sa_natt_state));

    if (isakmp_sa->sa_natt_state >= NATT_STATE_NATD){

	negotiation->use_natt_port = TRUE;
	if (isakmp_sa->sa_natt_state == NATT_STATE_NATD)
	    isakmp_sa->sa_natt_state = NATT_STATE_PORT;
    }

    negotiation->ike_pm_info->p1_natt_state = isakmp_sa->sa_natt_state;
    ssh_policy_sun_info("Doing port jump in case we need NAT-T. "
	"Current NAT-T state %d", isakmp_sa->sa_natt_state);

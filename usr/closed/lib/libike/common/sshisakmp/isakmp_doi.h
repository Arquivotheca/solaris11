...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_doi.h,v $
 *        $Author: irwin $
 *
 *        Creation          : 23:14 May 25 1998 kivinen
 *        Last Modification : 17:10 Mar  5 2003 kivinen
 ...
 *        
 *
 *        Description       : Doi specific defines etc.
 *
 *
 *        $Log: isakmp_doi.h,v $ *        
 *        
...
typedef enum {
  ...
  SSH_IKE_IPSEC_AH_TRANSFORM_AH_SHA256	= 5,
  SSH_IKE_IPSEC_AH_TRANSFORM_AH_SHA384	= 6,
  SSH_IKE_IPSEC_AH_TRANSFORM_AH_SHA512	= 7
#else
  SSH_IKE_IPSEC_AH_TRANSFORM_AH_DES     = 4 /* (from doi 04) */
#endif
...
typedef enum {
  ...
  /* From draft-ietf-ipsec-ciph-aes-cbc */
...
#ifdef SSHDIST_CRYPT_RIJNDAEL
  ,SSH_IKE_IPSEC_ESP_TRANSFORM_ESP_AES_CTR      = 254
#endif /* SSHDIST_CRYPT_RIJNDAEL */
...
typedef enum {
  ...
  IPSEC_CLASSES_COMP_PRIV_ALG           = 9, /* Compression private
                                                algorithm  (B/V) */
  IPSEC_CLASSES_SA_LONGSEQ              = 32001 /* SA uses extended sequence 
                                                   numbers (B) */
...
typedef enum {
  ...
  IPSEC_VALUES_AUTH_ALGORITHM_HMAC_SHA_256        = 5,
  IPSEC_VALUES_AUTH_ALGORITHM_HMAC_SHA_384        = 6,
  IPSEC_VALUES_AUTH_ALGORITHM_HMAC_SHA_512        = 7,
  IPSEC_VALUES_AUTH_ALGORITHM_AES_GMAC_128	= 9,
  IPSEC_VALUES_AUTH_ALGORITHM_AES_GMAC_192	= 10,
  IPSEC_VALUES_AUTH_ALGORITHM_AES_GMAC_256	= 11
#else
  IPSEC_VALUES_AUTH_ALGORITHM_KPDK              = 4 /* (from doi 05) */
#endif
...
typedef enum {
  ...
  SSH_IKE_CLASSES_GRP_ORDER = 16, /* (V) (from ike 07),
                                    ...
                                    (from ike 05) */
  SSH_IKE_CLASSES_GRP_CARDINALITY = 16384 /* (V) Group cardinality, ssh
                                             private extension XXX */
...
#if 0 /* SUNWIPSEC - this is probably IKEv2-only. */
  /* More modern DOI values. */
  SSH_IKE_VALUES_HASH_ALG_AES_CBC = 4,
  SSH_IKE_VALUES_HASH_ALG_SHA2_256 = 5,
  SSH_IKE_VALUES_HASH_ALG_SHA2_384 = 6,
  SSH_IKE_VALUES_HASH_ALG_SHA2_512 = 7,
#else
  SSH_IKE_VALUES_HASH_ALG_SHA2_256 = 4,
  SSH_IKE_VALUES_HASH_ALG_SHA2_384 = 5,
  SSH_IKE_VALUES_HASH_ALG_SHA2_512 = 6,
#endif /* 0 - SUNWIPSEC */
  SSH_IKE_VALUES_HASH_ALG_RIPEMD160 = 0xff09 /* XXX, not official,
                                                timestep is using number 4 */
...
#endif /* SSHDIST_ISAKMP_CFG_MODE */
#ifdef SUNWIPSEC  
  SSH_IKE_PAYLOAD_TYPE_DRAFT_NATD = 15, /* I-D nat-d addr hashes */
  SSH_IKE_PAYLOAD_TYPE_DRAFT_NATOA = 16, /* I-D nat original addresses */
  SSH_IKE_PAYLOAD_TYPE_NATD = 20, /* nat-d addr hashes */
  SSH_IKE_PAYLOAD_TYPE_NATOA = 21, /* nat original addresses */
#endif  
...
#ifdef SUNWIPSEC
typedef struct SshIkePayloadNatdRec{
	unsigned char *raw_natd;
	unsigned short natd_length; /* length - hdr */
	unsigned char *natd_data; /* ptr to data after hdr */
} *SshIkePayloadNatd;

typedef struct SshIkePayloadNatoaRec{
	/* everything, including payload,including type hdr */
	unsigned char *raw_natoa;
	/* length - payload header  should always be 8*/
	unsigned short natoa_length; /* length - hdr */
	/* just the address should always be 4 bytes */
	unsigned char *natoa_data;/* ptr to data after both headers */
	unsigned int natoa_addr;
} *SshIkePayloadNatoa;
#endif
...
#define SSH_IKE_PRIVATE_PAYLOAD_TYPE_NAT_D      15
#define SSH_IKE_PRIVATE_PAYLOAD_TYPE_NAT_OA     16
...
#ifdef SUNWIPSEC
    struct SshIkePayloadNatdRec natd;
    struct SshIkePayloadNatoaRec natoa;
#endif	  
#ifdef SSHDIST_ISAKMP_CFG_MODE

...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_init.c,v $
 *        $Author: tmo $
 *
 *        Creation          : 02:47 Jun  2 1998 kivinen
 ...
 *        
 *
 *        Description       : Isakmp module initialization and delete functions
 *
 *
 *        $Log: isakmp_init.c,v $ *        
 *        
                                  ...
                                  Boolean this_end_is_initiator)
{
  ...
  *pm_info_ptr = NULL;
  ...
  /*
   * NOTE for SUNWIPSEC fans... p1_use_natt and remote_port_natt are already
   * initialized to 0 and NULL by the calloc() call above.
   */
...
void ike_free_phase_i_pm_info(SshIkePMPhaseI pm_info)
{
  ...
  ssh_free(pm_info->remote_port_natt);
#endif
                             ...
                             SshUInt32 message_id)
{
  ...
  *pm_info_ptr = NULL;
  ...
  /*
   * NOTE for SUNWIPSEC fans... other_port, natt_init_ip, and natt_resp_ip are
   * already initialized to 0 and NULL by the calloc() call above.
   */
...
#ifdef SUNWIPSEC
  negotiation->packet_context = packet_context;
#endif /* SUNWIPSEC */
...
SshIkeNegotiation ike_alloc_negotiation(SshIkeSA sa)
{
  ...
  SSH_DEBUG(5, ("Start, SA = { %08x %08x - %08x %08x}",
...
{
  SshIkeSA sa;
  SSH_DEBUG(5, ("Start, initiator = %d, message_id = %08x",
                               ...
                               SshIkePayload *pl_out)
{
  ...
  *isakmp_packet_out = ssh_calloc(1, sizeof(struct SshIkePacketRec));
  ...
  SSH_DEBUG(7, ("Created random message id = %08x",
                (*isakmp_packet_out)->message_id));
  if (!sa->phase_1_done ||
...
void ike_delete_negotiation(SshIkeNegotiation negotiation)
{
  ...
  if (sa)
    SSH_DEBUG(5, ("Start, SA = { %08x %08x - %08x %08x}, nego = %d",
                  SSH_IKE_GET32(sa->cookies.initiator_cookie),
                  SSH_IKE_GET32(sa->cookies.initiator_cookie + 4),
                  SSH_IKE_GET32(sa->cookies.responder_cookie),
                  SSH_IKE_GET32(sa->cookies.responder_cookie + 4),
      ...
      if (sa == NULL)
        {
          ...
          /* Remove timeouts */
	  if (negotiation->packet_context != NULL)
	    ssh_udp_free_context(negotiation->packet_context);
      ...
      for (i = 0; i < sa->number_of_negotiations; i++)
        {
	      ...
	      if (sa->negotiations[i]->packet_context != NULL)
		ssh_udp_free_context(sa->negotiations[i]->packet_context);
      ...
      if (sa->isakmp_negotiation)
        {
	      ...
	      if (sa->isakmp_negotiation->packet_context != NULL)
		ssh_udp_free_context(sa->isakmp_negotiation->packet_context);
              ssh_free(sa->isakmp_negotiation);
...
void ike_remove_callback(void *context)
{
  ...
  if (sa == NULL)
    ...
    SSH_DEBUG(5, ("Start, delete SA = { %08x %08x - %08x %08x}, nego = %d",
                  SSH_IKE_GET32(sa->cookies.initiator_cookie),
                  SSH_IKE_GET32(sa->cookies.initiator_cookie + 4),
                  SSH_IKE_GET32(sa->cookies.responder_cookie),
                  SSH_IKE_GET32(sa->cookies.responder_cookie + 4),
...
void ike_expire_callback(void *context)
{
  ...
  SSH_DEBUG(5, ("Start, expire SA = { %08x %08x - %08x %08x}, nego = %d",
                SSH_IKE_GET32(sa->cookies.initiator_cookie),
                SSH_IKE_GET32(sa->cookies.initiator_cookie + 4),
                SSH_IKE_GET32(sa->cookies.responder_cookie),
                SSH_IKE_GET32(sa->cookies.responder_cookie + 4),

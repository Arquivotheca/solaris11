...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_attrs.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 17:29 Feb 28 2002 kivinen
 *        Last Modification : 17:04 Mar  5 2003 kivinen
 ...
 *        
 *
 *        Description       : Isakmp attribute handling functions
 *
 *
 *        $Log: isakmp_attrs.c,v $ *        
 *        
 *        
 *        
 *        
                               ...
                               SshIkeGrpAttributes attrs)
{
  ...
  if (!value_given)
    return TRUE;
  if (attrs->group_descriptor < 16386 && attrs->group_descriptor != 0)
    {
      ...
      if (ssh_ike_get_data_attribute_int(attr, &value, flags))
        {
          ...
          switch (attr->attribute_type)
              ...
              if (!value_ok || !ssh_cipher_supported(name))
                {
                  SSH_IKE_DEBUG(8, negotiation,
                                ("Unsupported encryption algorithm : %d (%s)",
                                 attrs->encryption_algorithm, name));
                  return FALSE;
                }
              break;
            ...
            case SSH_IKE_CLASSES_AUTH_METH: /* Authentication method */
              ...
              SSH_IKE_DEBUG(10, negotiation, ("Auth method = %d", value));
              if (!value_ok)
                {
                  SSH_IKE_DEBUG(8, negotiation,
                                ("Unsupported auth method : %d", value));
                  return FALSE;
                }
              break;
            ...
            case SSH_IKE_CLASSES_LIFE_DURATION: /* Life type value */
              ...
              switch (life_type)
                ...
                case SSH_IKE_VALUES_LIFE_TYPE_SECONDS:
                  ...
                  SSH_IKE_DEBUG(10, negotiation,
                                ("Life duration %d secs", value));
                  break;
                case SSH_IKE_VALUES_LIFE_TYPE_KILOBYTES:
                  attrs->life_duration_kb = value;
                  SSH_IKE_DEBUG(10, negotiation,
                                ("Life duration %d kb", value));
                  break;
                }
              break;
            case SSH_IKE_CLASSES_PRF: /* PRF function */
              ...
              SSH_IKE_DEBUG(10, negotiation, ("PRF alg = %d (%s)",
                                              attrs->prf_algorithm, name));
              /* Check if algorithm is supported */
              if (!value_ok || !ssh_mac_supported(name))
                {
                  SSH_IKE_DEBUG(8, negotiation,
                                ("Unsupported PRF algorithm : %d", value));
                  return FALSE;
                }
              break;
      ...
      if (ssh_ike_get_data_attribute_int(attr, &value, flags))
        {
          ...
          switch (attr->attribute_type)
            ...
            case IPSEC_CLASSES_GRP_DESC: /* Group descriptor number */
              ...
              attrs->group_desc = value;
              SSH_IKE_DEBUG(10, negotiation, ("Group = %d", value));
              /* Find matching group */
            ...
            case IPSEC_CLASSES_KEY_ROUNDS: /* Key rounds */
              ...
              attrs->key_rounds = value;
              SSH_IKE_DEBUG(10, negotiation, ("Key rounds = %d", value));
              break;
      ...
      attr_i.group_parameters != attr_r.group_parameters)
    {
      SSH_IKE_DEBUG(7, negotiation,
                   ("Attributes didn't match "
                    ...
                    attr_i.life_duration_kb,
                    attr_r.life_duration_kb,
                    attr_i.life_duration_secs,
                    attr_r.life_duration_secs,
                    attr_i.key_length,
                    attr_r.key_length,

...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_sa.c,v $
 *        $Author: irwin $
 *
 *        Creation          : 14:48 Jul 30 1997 kivinen
 *        Last Modification : 20:48 Mar  5 2002 kivinen
 ...
 *        
 *
 *        Description       : Isakmp sa hash functions
 *
 *        $Log: isakmp_sa.c,v $ *        
 *        
 *        
      ...
      if (isakmp_sa_negotiation != NULL)
        {
          if (isakmp_sa_negotiation == sa->isakmp_negotiation)
            {
              SSH_DEBUG(5,
                        ("Remote = %s:%s, Found SA = { %08x %08x - %08x %08x}",
                         remote_ip ? remote_ip : all,
                         remote_port ? remote_port : all,
                     ...
                     const unsigned char *responder)
{
  ...
  SSH_DEBUG(12, ("Start, SA = { %08x %08x - %08x %08x }",
  ...
  if (h != SSH_ADT_INVALID)
    {
      sa = ssh_adt_map_lookup(context->isakmp_sa_mapping, h);
      SSH_DEBUG(5, ("Found SA = { %08x %08x - %08x %08x }",
                     SSH_IKE_GET32(initiator),
                     SSH_IKE_GET32(initiator + 4),
                     SSH_IKE_GET32(responder),
                     SSH_IKE_GET32(responder + 4)));
    ...
    {
      SSH_DEBUG(5, ("Not found SA = { %08x %08x - %08x %08x }",
                    SSH_IKE_GET32(initiator),
                    SSH_IKE_GET32(initiator + 4),
                    SSH_IKE_GET32(responder),
                    SSH_IKE_GET32(responder + 4)));
                          ...
                          const unsigned char *initiator)
{
  ...
  SSH_DEBUG(12, ("Start, SA = { %08x %08x - %08x %08x }",
                 SSH_IKE_GET32(initiator), SSH_IKE_GET32(initiator + 4),
                 0, 0));
  ...
  if (h != SSH_ADT_INVALID)
    {
      sa = ssh_adt_map_lookup(context->isakmp_sa_mapping, h);
      SSH_DEBUG(5, ("Found half SA = { %08x %08x - %08x %08x }",
                    SSH_IKE_GET32(initiator), SSH_IKE_GET32(initiator + 4),
                    0, 0));
      sa->last_use_time = ssh_time();
      return sa;
    ...
    {
      SSH_DEBUG(5, ("Not found half SA = { %08x %08x - %08x %08x }",
                    SSH_IKE_GET32(initiator), SSH_IKE_GET32(initiator + 4),
                    0, 0));
      return NULL;
    }
                        ...
                        const unsigned char *cookie)
{
  ...
  SSH_DEBUG(5, ("Start, SA = { %08x %08x - %08x %08x } -> { ... - %08x %08x }",
                SSH_IKE_GET32(sa->cookies.initiator_cookie),
                SSH_IKE_GET32(sa->cookies.initiator_cookie + 4),
                SSH_IKE_GET32(sa->cookies.responder_cookie),
                SSH_IKE_GET32(sa->cookies.responder_cookie + 4),
                SSH_IKE_GET32(cookie), SSH_IKE_GET32(cookie + 4)));
...
void ike_clean_mapping(SshIkeContext context)
{
  ...
  for (h = ssh_adt_enumerate_start(context->isakmp_sa_mapping);
       ...
       h = ssh_adt_enumerate_next(context->isakmp_sa_mapping, h))
    {
      ...
      if (t > life / 4 * 3)
        {
          SSH_DEBUG(9, ("Found old negotiation, age = %d, sa = %p", t, sa));
          time_table[cnt] = 0x7fffffff;
          sa_table[cnt++] = sa;
          sa->created_time = 0;
          if (cnt == max_cnt)
                         ...
                         const unsigned char *responder)
{
  ...
  SSH_DEBUG(5, ("Start, SA = { %08x %08x - %08x %08x }",
                SSH_IKE_GET32(initiator), SSH_IKE_GET32(initiator + 4),
                SSH_IKE_GET32(responder), SSH_IKE_GET32(responder + 4)));
                                   ...
                                   SshBuffer buffer)
{
  ...
  if (len < SSH_IKE_PACKET_GENERIC_HEADER_LEN)
    {
      SSH_DEBUG(3, ("ssh_isakmp_find_sa got short packet: %ld",
                    (unsigned long) len));
      ssh_audit_event(context->isakmp_context->audit_context,
                      SSH_AUDIT_IKE_UNEQUAL_PAYLOAD_LENGTHS,
                      SSH_AUDIT_SOURCE_ADDRESS_STR, context->server_name,
  ...
  *minor_version = SSH_IKE_GET4R(p + 17);
  ...
  SSH_DEBUG(5, ("Start, SA = { %08x %08x - %08x %08x } / %08x, remote = %s:%s",
  ...
  *isakmp_sa_return = ike_sa_find_half(context->isakmp_context, p);
  ...
  SSH_DEBUG(3, ("Invalid cookie, no sa found, "
                "SA = { %08x %08x - %08x %08x } / %08x, remote = %s:%s",
                SSH_IKE_GET32(p), SSH_IKE_GET32(p + 4),
                SSH_IKE_GET32(p + 8), SSH_IKE_GET32(p + 12),
                *message_id, remote_ip, remote_port));
  ssh_audit_event(context->isakmp_context->audit_context,
                  ...
                  SSH_AUDIT_SOURCE_ADDRESS_STR, context->server_name,
                  SSH_AUDIT_DESTINATION_ADDRESS_STR, remote_ip,
                  SSH_AUDIT_TXT, "Received invalid ISAKMP cookie",
                  SSH_AUDIT_ARGUMENT_END);

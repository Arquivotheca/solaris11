...
/*
 *        Program: sshisakmp
 *        $Source: /project/cvs/src/ipsec/lib/sshisakmp/isakmp_output.c,v $
 *        $Author: tmo $
 *
 *        Creation          : 14:48 Jul 30 1997 kivinen
 *        Last Modification : 13:46 Nov  7 2002 kivinen
 ...
 *        
 *
 *        Description       : Isakmp state machine output functions module
 *
 *        $Log: isakmp_output.c,v $ *        
 *        
 *        
 *        
 *        
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("No sa payload found, could not find group "
          "information");
#endif
...
#ifdef SUNWIPSEC
      ssh_policy_sun_info("Could not get private key info: %.200s",
          ssh_crypto_status_message(cret));
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
      ...
      if (cret != SSH_CRYPTO_OK)
        {
          SSH_IKE_DEBUG(3, negotiation,
                        ("ssh_private_key_select_scheme failed: %.200s",
                         ssh_crypto_status_message(cret)));
#ifdef SUNWIPSEC
          ssh_policy_sun_info("Could not select private key scheme: %.200s",
              ssh_crypto_status_message(cret));
#endif
          return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
        }
                                                  ...
                                                  private_key) < hash_len)
        {
          SSH_IKE_DEBUG(3, negotiation,
                        ("Hash too large, private key cannot sign it, "
                         ...
                         hash_len));
#ifdef SUNWIPSEC
          ssh_policy_sun_info("Hash too large, private key cannot sign it, "
                         "hash_size = %d",
                         hash_len);
#endif
          return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
  ...
  for (i = 0; i < negotiation->ike_ed->number_of_cas; i++)
    {
      if (negotiation->ike_ed->number_of_certificates[i] == 0)
        {
          /* XXX, later when notification about missing certificates for given
             certificate request is defined, we should add that notification
             here. */
        }
      else
        {
                  ...
                  max_packet_size / 3)
                {
                  SSH_IKE_DEBUG(3, negotiation,
                                ("Certificate or CRL not send, "
                                 "because its size exceeds %dkB.",
                                 max_packet_size / 3 / 1024));
                  continue;
                }
              ...
              else
                {
                  ...
                  if (total_crl_size + total_cert_size >
                      max_packet_size / 10 * 9)
                    {
                      SSH_IKE_DEBUG(3, negotiation,
                                    ("Certificate not send, because total "
                                     "size of certs and CRLs exceeds %dkB.",
                                     max_packet_size / 10 * 9 / 1024));
                      total_cert_size -=
                        negotiation->ike_ed->cert_lengths[i][j];
                      continue;
                    }
...
#ifdef SUNWIPSEC
          ssh_policy_sun_info("No sa payload found, could not find hash "
	      "information");
#endif
      return SSH_IKE_NOTIFY_MESSAGE_AUTHENTICATION_FAILED;
    }
                  ...
                  if (ssh_ike_read_isakmp_attrs(negotiation,
                                                ...
                                                &attrs))
                    {
                      ...
                      if (attrs.hash_algorithm != 0)
                        {
                          /* Yes use it */
                          /* XXX temporary until library API is changed XXX */
                          hash_name =
...
void ike_st_o_all_done(SshIkeContext isakmp_context,
                       ...
                       SshIkeNegotiation negotiation)
{
  SSH_DEBUG(4, ("MESSAGE: Phase 1 { 0x%08x %08x - 0x%08x %08x } / %08x, "
                ...
                negotiation->ike_ed->attributes.life_duration_kb,
                negotiation->ike_ed->attributes.life_duration_secs,
                negotiation->ike_ed->attributes.key_length,
                                         ...
                                         SshIkeStateMachine state)
{
  ...
  /* We received last packet from the other end, so we have
     to copy the decrypting cipher to cipher_iv, because
...
#ifdef SUNWIPSEC
SshIkeNotifyMessageType ike_st_o_natd(SshIkeContext isakmp_context,
                                        SshIkePacket isakmp_input_packet,
                                        SshIkePacket isakmp_output_packet,
                                        SshIkeSA isakmp_sa,
                                        SshIkeNegotiation negotiation,
                                        SshIkeStateMachine state)
{
	unsigned char *buf_loc = NULL;
	unsigned char *buf_rem = NULL;
	unsigned int addr;
	unsigned int len;
	int pre_port;
	unsigned short port;
	...
	SshIkePayload pl;
	SshIpAddrStruct ipa;
	SshIkeNotifyMessageType cret;


	/* Did the vid say to do natt? */

	if (isakmp_sa->sa_natt_state < NATT_STATE_VID){
		return 0;
	}

	if (negotiation->ike_pm_info->p1_use_natt == 1)
		natd_value = SSH_IKE_PAYLOAD_TYPE_DRAFT_NATD;
	else
		natd_value = SSH_IKE_PAYLOAD_TYPE_NATD;

	/*
	 * If we're the first packet sent by a responder, don't bother unless
	 * we're doing public-key encryption.
	 */
	if (state->state == SSH_IKE_ST_START_SA_NEGOTIATION_R &&
	    negotiation->ed->auth_method_type !=
	    SSH_IKE_AUTH_METHOD_PUBLIC_KEY_ENCRYPTION)
		return 0;
	...
	/* Tell the peer my view on his/her IP address. */
	...
	port = htons(port); 
	SSH_DEBUG(5, ("natd_out ad:%s=%x p:%x", 
		      negotiation->ike_pm_info->remote_ip, 
		      addr, port)); 

	buf_rem = ssh_malloc(256);
	if (buf_rem == NULL)
		goto no_memory;

        cret = ike_calc_natd_hash(isakmp_sa, negotiation, addr,  
				  port, buf_rem, &len); 
        ...
        /* fill up structure */ 
        pl = ike_append_payload(isakmp_context, isakmp_output_packet, 
	    isakmp_sa, negotiation, natd_value); 
 
        pl->pl.natd.natd_data = buf_rem; 
        pl->pl.natd.natd_length = len;

	/* Tell the peer my IP address. */
	if (!ssh_ipaddr_parse(&ipa, isakmp_sa->server_context->server_name)){
	    ssh_free(buf_rem);
	    return SSH_IKE_NOTIFY_MESSAGE_NO_SA_ESTABLISHED;
	}

	SSH_IP4_ENCODE(&ipa, &addr);
	pre_port = atoi((char *)isakmp_sa->server_context->server_port);
	port = (unsigned short)(pre_port & 0x0000FFFF);
	port = htons(port);
	
	buf_loc = ssh_malloc(256);
	if (buf_loc == NULL)
		goto no_memory;
	...
	pl->pl.natd.natd_data = buf_loc;
	pl->pl.natd.natd_length = len;

	if (!ike_register_item(isakmp_output_packet, buf_rem) ||
	    !ike_register_item(isakmp_output_packet, buf_loc)) {
no_memory:
		ssh_free(buf_loc);
		ssh_free(buf_rem);
		return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;
...
SshIkeNotifyMessageType ike_st_o_natoa(SshIkeContext isakmp_context,
                                        SshIkePacket isakmp_input_packet,
                                        SshIkePacket isakmp_output_packet,
                                        SshIkeSA isakmp_sa,
                                        SshIkeNegotiation negotiation,
                                        SshIkeStateMachine state)
{
	...
	SshIpAddrStruct ipa;	
	unsigned char *addr2do;
	int natoa_value;
	
	if (isakmp_sa->sa_natt_state < NATT_STATE_PORT)
		return 0;

	if (isakmp_output_packet->first_natoa_payload != NULL)
		return 0;
	
	if (negotiation->qm_pm_info->this_end_is_initiator){
		addr2do = negotiation->qm_pm_info->local_ip;
	} else {
		addr2do = negotiation->qm_pm_info->remote_ip;
	}

	if (negotiation->qm_pm_info->phase_i->p1_use_natt == 1)
		natoa_value = SSH_IKE_PAYLOAD_TYPE_DRAFT_NATOA;
	else
		natoa_value = SSH_IKE_PAYLOAD_TYPE_NATOA;

	SSH_DEBUG(1, ("NATOA:init addr: %s",addr2do));
	

	
	if (!ssh_ipaddr_parse(&ipa, addr2do)){
		return SSH_IKE_NOTIFY_MESSAGE_NO_SA_ESTABLISHED;
	}
	...
	SSH_IP4_ENCODE(&ipa,buf1);
			
	pl = ike_append_payload(isakmp_context, isakmp_output_packet,
	    isakmp_sa, negotiation, natoa_value);
	if (pl == NULL)
		goto no_memory;

	pl->pl.natoa.natoa_data = buf1;
	pl->pl.natoa.natoa_length = 4;
	...
	SSH_DEBUG(1, ("NATOA:resp addr: %s",addr2do));
	...
	pl->pl.natoa.natoa_data = buf2;
	pl->pl.natoa.natoa_length = 4;

	if (!ike_register_item(isakmp_output_packet, buf1) ||
	    !ike_register_item(isakmp_output_packet, buf2)) {
no_memory:
		ssh_free(buf1);
		ssh_free(buf2);
		return SSH_IKE_NOTIFY_MESSAGE_OUT_OF_MEMORY;

                             ...
                             SshAsn1Node *algorithm_node)
{
  ...
  if (oids == NULL)
    {
      ...
      strncpy(temp, ssh_csstr(cipher->name), sizeof(temp));
      if ((p = strstr(temp, "-cbc")) != NULL)
        *p = '\000';

      ssh_snprintf(ssh_sstr(oidname), sizeof(oidname),
                   "%s%d-cbc", temp, cipher->key_length * 8);
      oids = ssh_pkcs7_algorithm_oids(oidname);
      if (oids == NULL)
        return SSH_PKCS7_ALGORITHM_UNKNOWN;
    }

  if (!strncmp(ssh_csstr(cipher->name), "des", 3)
      || !strncmp(ssh_csstr(cipher->name), "3des", 4)
      || !strncmp(ssh_csstr(cipher->name), "aes", 3))
    {
      if (ssh_asn1_create_node(context, &params_node,
                               "(octet-string ())",
  ...
  else if (!strcmp(ssh_csstr(cipher->name), "rc2-cbc"))
    {
  ...
  else if (!strncmp(ssh_csstr(cipher->name), "rc5-cbc", 7))
    {
      if (ssh_asn1_create_node(context, &params_node,
                               "(sequence ()"
                               "  (integer-short ())"

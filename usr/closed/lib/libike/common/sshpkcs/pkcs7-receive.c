      ...
      if (expected_digest == NULL)
        {
          ...
          /* XXX: Allow data to be zero length for now. */
          ssh_hash_reset(hash);
                      ...
                      SshPkcs7ContentType subtype)
{
  ...
  if (ssh_cipher_allocate(ssh_csstr(data_encryption), key, key_len, FALSE,
                          &cipher) == SSH_CRYPTO_OK)
    {
      ...
      if (ssh_cipher_transform(cipher, data, data, data_len) == SSH_CRYPTO_OK)
        {
          size_t new_data_len;
...
{
  SshHash hash;
  unsigned char digest[SSH_MAX_HASH_DIGEST_LENGTH], *ber, *data;
  size_t digest_len, ber_len, data_len;
  ...
  if (envelope->type == SSH_PKCS7_DIGESTED_DATA)
    {
      ...
      if (ssh_hash_allocate(ssh_csstr(envelope->content_digest_algorithm),
                            &hash) == SSH_CRYPTO_OK)
        {
          ssh_pkcs7_encode_data(envelope->content, &ber, &ber_len);
          ...
          ssh_hash_free(hash);
          ssh_free(ber);
          if ((digest_len == envelope->content_digest_length) &&
              (memcmp(digest, envelope->content_digest, digest_len) == 0))
            return TRUE;
          else
            return FALSE;
        }

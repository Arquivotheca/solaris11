                                 ...
                                 SshPkcs12Bag bag)
{
  ...
  if (!strcmp(oid, PKCS9_OID".20"))
    type = SSH_PKCS12_ATTR_FRIENDLY_NAME;
  else if (!strcmp(oid, PKCS9_OID".21"))
    type = SSH_PKCS12_ATTR_LOCAL_KEY_ID;
  else
    type = SSH_PKCS12_ATTR_UNKNOWN;
                                 ...
                                 SshPkcs12Bag bag)
{
  ...
  while (node)
    {
      status = ssh_pkcs12_bag_decode_attribute(asn1, node, bag);
      node = ssh_asn1_node_next(node);
    }
  return SSH_PKCS12_OK;
}

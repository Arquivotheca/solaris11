  ...
  if (result == SSH_HTTP_RESULT_SUCCESS)
    {
      ...
      if (cp == NULL)
        ...
        {
          if (strcmp(cp, "application/x-pki-message") &&
              strcmp(cp, "x-pki-message"))
            {
              ssh_fsm_set_next(thread, scep_aborted);
              session->status = SSH_PKI_FAILED;
            }
                     ...
                     const unsigned char *data, size_t len)
{
  ...
  url = (char *)ssh_buffer_ptr(&buffer);
  tdata->transport_op =
...
static void scep_timeout_handler(void *context)
{
  ...
  SSH_DEBUG(SSH_D_HIGHSTART, ("Received timeout for thread %x", thread));
  ssh_fsm_continue(thread);
}
...
SSH_FSM_STEP(scep_connect)
{
  ...
  SSH_DEBUG(SSH_D_HIGHSTART, ("open to %s for thread %x",
...
SSH_FSM_STEP(scep_send_initial)
{
  ...
  SSH_DEBUG(SSH_D_HIGHSTART, ("Sending pkiReq for thread %x", thread));
  session = gdata->session;
...
SSH_FSM_STEP(scep_done)
{
  ...
  SSH_DEBUG(SSH_D_HIGHSTART, ("Enrollment complete (thread %x)", thread));
...
SSH_FSM_STEP(scep_process_input)
{
  ...
  SSH_DEBUG(SSH_D_HIGHSTART, ("Processing input (thread %x) signal", thread));

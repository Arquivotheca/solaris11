                      ...
                      void *context)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("thread %x %d bytes from CA", thread, len));
...
pkix_tcp_receive_eof(void *context)
{
  ...
  tdata->wrapper = NULL;
  SSH_DEBUG(SSH_D_HIGHOK, ("Received EOF from CA for thread %x", thread));
}
...
pkix_tcp_connect_callback(SshTcpError error, SshStream stream, void *context)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("thread %x connected error %d", thread, error));
  if (error == SSH_TCP_OK)
    {
      tdata->wrapper =
...
pkix_tcp_kill_input(SshFSMThread thread)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK,
            ("Deleting wrapper and timeouts for thread %x conn=%x",
             thread, tdata->http));
...
static void pkix_timeout_handler(void *context)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("Received timeout for thread %x", thread));
  tdata->timeout_set = FALSE;
...
SSH_FSM_STEP(pkix_connect)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("open to %s for thread %x",
  ...
  if (ssh_url_parse_and_decode(ssh_custr(gdata->session->access),
                               &scheme, &host, &port, NULL, NULL, NULL))
    {
      if (strncasecmp(ssh_csstr(scheme), "tcp", 3) == 0)
        {
          SshTcpConnectParamsStruct tcp_connect_params;
          memset(&tcp_connect_params, 0, sizeof(tcp_connect_params));
          tcp_connect_params.socks_server_url =
            ssh_ustr(gdata->session->socks);
      ...
      if (strncasecmp(ssh_csstr(scheme), "http", 4) == 0)
        {
          ssh_free(scheme);
          memset(&params, 0, sizeof(params));
          params.socks = gdata->session->socks;
          params.http_proxy_url = ssh_ustr(gdata->session->proxy);
          if (!tdata->http)
...
SSH_FSM_STEP(pkix_send_initial)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("Sending pkiReq for thread %x", thread));
  ...
  if (tdata->input_type != SSH_PKI_MSG_ERRORREP)
    {
      if (tdata->input_len == 0)
        {
          SSH_DEBUG(SSH_D_HIGHOK, ("Thread %x Waiting for input", thread));
          SSH_FSM_CONDITION_WAIT(gdata->input_avail);
        }
    }
  SSH_DEBUG(SSH_D_HIGHOK,
            ("Received %s for thread %x",
...
SSH_FSM_STEP(pkix_done)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("Enrollment complete (thread %x)", thread));
...
SSH_FSM_STEP(pkix_send_subsequent)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK,
            ("Sending pollReq (thread %x id %d)", thread, tdata->polling_id));
...
SSH_FSM_STEP(pkix_process_input)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("Processing input (thread %x) signal", thread));
...
SSH_FSM_STEP(pkix_aborted)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK,
            ("Aborting session %x thread %x", gdata->session, thread));
                           ...
                           const unsigned char *msg_data, size_t msg_len)
{
  ...
  ssh_encode_buffer(buffer, SSH_FORMAT_UINT32, n_msgs, SSH_FORMAT_END);
  if (n_msgs)
    ssh_encode_buffer(buffer,
                      SSH_FORMAT_UINT32_STR, msg_data, msg_len,
                      SSH_FORMAT_END);
}
                           ...
                           unsigned char **msg_data, size_t *msg_len)
{
  ...
  ssh_decode_buffer(buffer, SSH_FORMAT_UINT32, &n_msgs, SSH_FORMAT_END);
  if (n_msgs > 0)
    {
...
Boolean ssh_pki_pkix_session_linearize(SshPkiSession session)
{
  ...
  ssh_encode_buffer(&session->statebuffer,
                    ...
                    session->access, strlen(session->access),
                    SSH_FORMAT_UINT32_STR,
                    session->proxy, strlen(session->proxy),
                    SSH_FORMAT_UINT32_STR,
                    session->socks, strlen(session->socks),
                    SSH_FORMAT_UINT32, tdata->polling_id,
                    SSH_FORMAT_UINT64, tdata->polling_time,
                    SSH_FORMAT_END);
...
Boolean ssh_pki_pkix_session_delinarize(SshPkiSession session)
{
  ...
  ssh_decode_buffer(&session->statebuffer,
                    ...
                    SSH_FORMAT_UINT32_STR, &session->access, NULL,
                    SSH_FORMAT_UINT32_STR, &session->proxy, NULL,
                    SSH_FORMAT_UINT32_STR, &session->socks, NULL,
                    SSH_FORMAT_UINT32, &session->polling_id,
                    SSH_FORMAT_UINT64, &session->polling_time,
                    SSH_FORMAT_END);

...
/* Set one request response explicitly. The ber-blob contains the
   certificate per rfc2459, or per rfc2511 encrypted value.  XXX:
   todo; add possibility to pass private keys as well. */
void
                         ...
                         const SshCmpStatusInfo status)
{
  ...
  if (status)
    {
      memmove(&conf->pki_status, status, sizeof(status));
      if (status->freetext)

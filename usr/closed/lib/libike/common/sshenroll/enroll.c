...
void ssh_pki_session_abort(void *context)
{
  ...
  SSH_DEBUG(SSH_D_HIGHOK, ("Enrollment is aborted (sess=%x thread=%x)",
...
#define SSH_STRDUP(_str) ((_str) ? ssh_strdup((_str)) : ssh_strdup(""))
...
char *ssh_pki_session_linearize(SshPkiSession session)
{
  ...
  ssh_encode_buffer(&session->statebuffer,
                    SSH_FORMAT_UINT32_STR, session->extra, session->extra_len,
                    SSH_FORMAT_END);
...
SshPkiSession ssh_pki_session_delinearize(const char *linear)
{
  ...
  ssh_decode_buffer(&session->statebuffer,
                    ...
                    &session->extra, &session->extra_len,
                    SSH_FORMAT_END);

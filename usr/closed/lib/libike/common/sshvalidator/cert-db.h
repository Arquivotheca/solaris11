...
/* Add a key to the entry. 'Data' must point to memory allocated with
   ...
   not be changed after this call. */
...
/* XXX Match two key lists and return TRUE if the first one has
   atleast all the same keys as the second, but could have other too. */
Boolean ssh_certdb_key_match(SshCertDBKey *op1, SshCertDBKey *op2);

/* XXX Clean extra information in the key list. */

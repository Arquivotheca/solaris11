  ...
  if (ssh_ber_time_cmp(not_after, &context->max_cert_validity_time) < 0)
    {
      /* Now we have deduced that "not_after" is more restrictive than
         ...
         we would be interested in keeping the old value if possible.


         XXX This is a highly suspectible place. Please verify that
         this is something we can do (without causing some security
         implications which are not tolerable), but later... testing
         the code first.  */

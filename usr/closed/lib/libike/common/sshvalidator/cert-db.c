...
struct SshCertDBRec
{
  ...
  /* XXX: Callbacks to transform the information between memory and
     disk representation. */
  ...
  if (entry_class >= db->num_entry_classes)
    {
      ...
      old = db->num_entry_classes;
      db->num_entry_classes = entry_class + 1;
      tmp = ssh_realloc(db->entries,
                        (old * sizeof(*db->entries)),
                        (db->num_entry_classes - old) * sizeof(*db->entries));
      if (tmp)
        {
          db->entries = tmp;
          memset(&db->entries[old], 0,
                 (db->num_entry_classes - old) * sizeof(*db->entries));
        }
      else
        {
                    ...
                    void *context)
{
  ...
  if (k1->data_len == k2->data_len)
    ...
    return k1->data_len - k2->data_len;
}
  ...
  for (p = (SshCDBContainer) tmp->entries; p != NULL; p = p->next)
    {
      ...
      e->reference_count++;
      SSH_DEBUG(8, ("increasing reference count to %d", e->reference_count));
...
void ssh_certdb_free(SshCertDB db)
{
  ...
  /* Sanity check. */
  SSH_DEBUG(3, ("memory left %d", db->memory_usage));
...
void ssh_certdb_take_reference(SshCertDBEntry *entry)
{
  ...
  SSH_DEBUG(8, ("increasing reference count to %d", entry->reference_count));
}
...
void ssh_certdb_add_key(SshCertDB db,
                        SshCertDBEntry *entry,
                               ...
                               SshCertDBKey *name_list)
{
  ...
  /* XXX Perform comparison here between the entry keys and the input keys
  ...
  if (list && (node = ssh_malloc(sizeof(*node))) != NULL)
    {
      ...
      entry->reference_count++;
      SSH_DEBUG(8,
                ("increasing reference count to %d", entry->reference_count));
      return TRUE;
    }
  return FALSE;
}
...
void *ssh_certdb_get_context(SshCertDB db, SshCertDBEntry *entry)
{
  if (entry->context == NULL)
    {
      /* This is a dummy entry, meaning that the actual content is not
         read from disk yet. */
          ...
          if (entry->internal_flags & SSH_CDBF_LOCKED)
            {
              ...
              entry->reference_count--;
              SSH_DEBUG(8, ("decreasing reference count to %d",
                            entry->reference_count));
          ...
          if (!(entry->internal_flags & SSH_CDBF_LOCKED))
            {
              ...
              entry->reference_count++;
              db->num_locked++;
              SSH_DEBUG(8, ("increasing reference count to %d",
                            entry->reference_count));
                              ...
                              int flags)
{
  ...
  /* XXX Schedule this entry for writing. */
...
SshCDBError ssh_certdb_release_entry(SshCertDB db, SshCertDBEntry *entry)
{
  ...
  if ((--entry->reference_count) == 0)
    ...
    SSH_DEBUG(8, ("decreasing reference count to %d", entry->reference_count));

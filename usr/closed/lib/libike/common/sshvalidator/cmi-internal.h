...
struct SshCMCertificateRec
{
  ...
  unsigned int initialization_flags:8;
...
#define SSH_CM_CERT_IF_TRUSTED ((unsigned int)1 << 1)
  ...
  /* 2. If has ever acted as a CA certificate (with or without the
  ...
  /* 3. Defines whether this certificate is a CRL issuer or not. All
     certificates that are CA's must be certificate issuers for
     correct validation. */
  unsigned int crl_issuer:1;

  /* 4. This certificate is self-signed? */
  unsigned int self_signed:1;

  /* 5. Defines whether CRLs are used when validating this
     certificate. */
  unsigned int crl_user:1;

  /* 6. Was the revocator 'trusted root'. */
  unsigned int revocator_was_trusted:1;

  /* 7. Is this issued to self; does not mean self-signed but same

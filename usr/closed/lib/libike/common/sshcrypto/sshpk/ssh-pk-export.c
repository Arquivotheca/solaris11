...
ssh_pk_import_v1_randomizer_import(SshPkImportState state)
{
  ...
  if (!state->pk_group_randomizers)
    return SSH_CRYPTO_UNSUPPORTED; /* XXX parameter problem */
...
ssh_pk_import_v2_import(SshPkImportState state)
{
  ...
  if (strcmp(state->hash_name, "none") != 0)
    {
      SshHash hash;
      unsigned char result[SSH_MAX_HASH_DIGEST_LENGTH];
      ...
      status = ssh_hash_final(hash, result);
      ssh_hash_free(hash);

      if (status != SSH_CRYPTO_OK)
        goto common_error;

      if (memcmp(digest, result, digest_len) != 0)
        {
  ...
  if (state->type == SSH_PKF_PRIVATE_KEY ||
  ...
  else if (state->type == SSH_PKF_PK_GROUP_RANDOMIZERS)
    {
      ...
      SSH_ASSERT(pk_group->type->pk_group_import_randomizer != NULL);
...
ssh_pk_import_v2_init(SshPkImportState state)
{
  ...
  state->prv.v2.data_off = point - state->buf + 1;
  ...
  if (state->cipher_name && strcmp(state->cipher_name, "none") != 0)
    {
      ...
      cipher_block_len = ssh_cipher_get_block_length(state->cipher_name);
...
ssh_pk_export_v2_all(SshPkExportState state)
{
  ...
  if (cipher_name && strcmp(cipher_name, "none") != 0)
    {
      size_t block_len;
      ...
      if (block_len == 1)
      ...
      else
        {
          ...
          /* XXX change to calculate, not iterate */

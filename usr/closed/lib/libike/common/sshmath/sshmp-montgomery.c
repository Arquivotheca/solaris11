...
Boolean ssh_mpmzm_init_ideal(SshMPMontIntIdeal m, SshMPIntegerConst op)
{
  ...
  m->dynamic_karatsuba = FALSE;

  /* Do we need to allocate memory? */ 
  if (op->n > SSH_MP_MONT_IDEAL_STATIC_ARRAY_SIZE)
    m->dynamic = TRUE;
  else 
    m->dynamic = FALSE;
  ...
  if (m->dynamic)
    {
      ...
      SSH_DEBUG(12, ("Allocating %d bytes", 
                    (sizeof(SshWord) * (op->n + op->n))));
  ...
  if (m->karatsuba_work_space_n > 
      SSH_MP_MONT_IDEAL_KARATSUBA_STATIC_ARRAY_SIZE)
    {
      m->dynamic_karatsuba = TRUE;
      SSH_DEBUG(12, ("Allocating %d bytes", 
                    (sizeof(SshWord) * m->karatsuba_work_space_n)));
    ...
    {
      m->karatsuba_work_space = m->karatsuba_ws_static;
      m->karatsuba_work_space_n = 
        SSH_MP_MONT_IDEAL_KARATSUBA_STATIC_ARRAY_SIZE;
    }
...
void ssh_mpmzm_init(SshMPMontIntMod op, SshMPMontIntIdealConst m)
{
  ...
  if (m->m_n > SSH_MP_MONT_IDEAL_STATIC_ARRAY_SIZE)
    op->dynamic = TRUE;
  else 
    op->dynamic = FALSE;
  ...
  if (op->dynamic)
    {
      SSH_DEBUG(12, ("Allocating %d bytes", (sizeof(SshWord) * (m->m_n + 1))));
                            ...
                            SshMPMontIntModConst op2)
{
  ...
  if (op1->m->m_n > SSH_MP_MONT_IDEAL_STATIC_ARRAY_SIZE)
    op1->dynamic = TRUE;
  else 
    op1->dynamic = FALSE;
...
{
  if (!ssh_mpmzm_isnan(op) && op->dynamic)
    ssh_free(op->v); 
                   ...
                   SshMPIntegerConst e)
{
  SshMPMontIntModStruct temp, x;
  Boolean dynamic;
  unsigned int table_bits, table_size, static_table_size;
  ...
  /* XXX This is ugly. */
#if SSH_MP_INTEGER_BIT_SIZE_STATIC > 5678
  SshMPMontIntModStruct table_array[128];
#elif SSH_MP_INTEGER_BIT_SIZE_STATIC > 2173
  SshMPMontIntModStruct table_array[64];
#elif SSH_MP_INTEGER_BIT_SIZE_STATIC > 798
  SshMPMontIntModStruct table_array[32];
#elif SSH_MP_INTEGER_BIT_SIZE_STATIC > 277
  SshMPMontIntModStruct table_array[16];
#else 
  SshMPMontIntModStruct table_array[1];
#endif 

  static_table_size = sizeof(table_array) / sizeof(table_array[0]);
  ...
  if ((table_size > static_table_size) || 
      (ret->m->m_n  > SSH_MP_MONT_IDEAL_STATIC_ARRAY_SIZE))
    { 
      dynamic = TRUE;

      /* Allocate the table. */
      SSH_DEBUG(12, ("Allocating %d bytes for the Mont modexp table.", 
                    sizeof(SshMPMontIntModStruct) * table_size));

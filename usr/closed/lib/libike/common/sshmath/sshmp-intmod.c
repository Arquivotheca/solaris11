...
void ssh_mprzm_set_mprz(SshMPIntMod ret, SshMPIntegerConst op)
{
  ...
  if (ssh_mprz_isnan(op))
    {
      /* XXX: Is this the correct NaN type to return. */
...
void ssh_mprzm_pow(SshMPIntMod ret, SshMPIntModConst g, SshMPIntegerConst e)
{
  ...
  /* Allocate the table. */
  SSH_DEBUG(12, ("Allocating %d bytes for the IntMod modexp table.",
                sizeof(SshMPIntModStruct) * table_size));
...
void ssh_mprzm_pow_precomp_init(SshMPIntModPowPrecomp precomp,
                                SshMPIntModConst g,
                                SshMPIntegerConst order)
{
  ...
  precomp->table = ssh_malloc(sizeof(SshMPIntModStruct)*precomp->table_size);

  table = ssh_malloc(sizeof(SshMPIntModStruct)*SSH_MPRZM_POW_PRECOMP_K);

  if (table == NULL || precomp->table == NULL)
    {
      ssh_free(table);
      ssh_free(precomp->table); precomp->table = NULL;
      return;
    }
...
void ssh_mprzm_log(SshMPIntMod ret, SshMPIntModConst g, SshMPIntModConst x)
{
  /* XXX Implement Pollard rho, or something similar. */
  ssh_fatal("ssh_mprzm_log: not yet implemented.");
}

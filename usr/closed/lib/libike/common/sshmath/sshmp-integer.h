...
/* Multiple precision integer conversion to byte arrays and back
   ...
   XXX: check for conformance. */
size_t ssh_mp_to_buf(unsigned char *cp, size_t len, SshMPIntegerConst x);
void ssh_buf_to_mp(SshMPInteger x, const unsigned char *cp, size_t len);

/* Platform independent encodings for integers. These take signs into
   account, and add a length field to the outputs. This means that the
   decoding works even when the input integers length is not known
   a priori (except for some sensible bound). The encoding is quite
   efficient.

   As the encoding is proprietary, for convenience reasons, we explain it
   here exactly.

   The encodings consists of following parts

   first octet || length octets || integer octets

   The first octet contains

     bit 8         the sign bit
     bit 7         the extension bit
     bits 6 to 1   the most-significant 6 length bits

   the other length octets appear only if bit 7 of the first octet is
   set.

   The length octet has

     bit 8         the extension bit
     bits 7 to 1   the next most-significant bits of the length field

   the length octets in natural way encode the length field. We emphasize
   that the encoding is most significant bits first.

   The rest of the integer (the absolute value) is encoded most-signficant
   octet first.

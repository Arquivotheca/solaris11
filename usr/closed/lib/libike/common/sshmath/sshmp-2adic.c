...
void ssh_mp2az_set_prec(SshMP2AdicInteger op, unsigned int prec)
{
  ...
  if (op->v == NULL)
    {
      SSH_DEBUG(12, ("Allocating %d bytes", prec * sizeof(SshWord)));
...
int ssh_mp2az_dist(SshMP2AdicIntegerConst op1, SshMP2AdicIntegerConst op2)
{
  ...
  /* XXX: What value should be returned here. */
  ...
  while (1)
    {
      ...
      if (ssh_mp2az_dist_ui(&y, 1) == prec * SSH_WORD_BITS)
        {
          if (prec >= max_prec)
            break;
...
/* This table contains non-zero when that index has such square root (plus
   one). */
static const unsigned char ssh_mp2az_sqrt_tab[64] =
  ...
  while (1)
    {
      ...
      if (ssh_mp2az_dist(&y, &t) == prec * SSH_WORD_BITS)
        {
          if (prec >= max_prec)
            break;
          prec *= 2;
                   ...
                   SshMPIntegerConst e)
{
  ...
  /* Allocate the table. */
  SSH_DEBUG(12, ("Allocating %d bytes for the 2adic modexp table.",
                sizeof(SshMP2AdicIntegerStruct) * table_size));

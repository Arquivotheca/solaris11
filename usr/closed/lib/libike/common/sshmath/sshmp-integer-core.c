...
SshMPInteger ssh_mprz_malloc(void)
{
  ...
  SSH_DEBUG(12, ("Allocating %d bytes.", sizeof(*op)));
  ...
  if (new_size > op->m)
    {
      ...
      SSH_DEBUG(12,("Allocating %d bytes",(size_t)new_size * sizeof(SshWord)));
                           ...
                           SshMPIntegerConst op, SshWord u)
{
  ...
  if (op->n <= SSH_MP_INTEGER_STATIC_ARRAY_SIZE)
    ...
    {
      /* Normalize. */
      SSH_DEBUG(12, ("Allocating %d bytes.", sizeof(SshWord) * (op->n + 1)));
      dynamic = TRUE;
...
char *ssh_mprz_get_str(SshMPIntegerConst op, SshWord base)
{
  ...
  if (ssh_mprz_cmp_ui(op, 0) == 0)
    {
      SSH_DEBUG(12, ("Allocating %d bytes.", 2));
      if ((str = ssh_malloc(2)) == NULL)
        return NULL;
  ...
  SSH_DEBUG(12, ("Allocating %d bytes.", digits + 1));
  if ((str = ssh_malloc(digits + 1)) == NULL)
    return NULL;

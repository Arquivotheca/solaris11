                                       ...
                                       SshPrivateKey *key)
{
  ...
  /* XXX: Backwards compatilibity here. */
  ...
  if (pass_len != 0)
    {
      ...
      {
        int i;
        fprintf(stderr, "DEBUG: Passphrase=\"%s\" Len=%lu Expanded_Key=0x",
                passphrase, (unsigned long)pass_len);
        for (i = 0; i < cipher_keylen; i++)
          fprintf(stderr, "%02x", cipher_key[i]);
        fprintf(stderr, " Len=%lu\n", (unsigned long)cipher_keylen);
      }

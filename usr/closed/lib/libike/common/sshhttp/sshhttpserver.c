...
static SshStream ssh_http_server_content_read_stream(
                                                SshHttpServerConnection conn);

/* The write-only `Content Data Write' stream. */
static SshStream ssh_http_server_content_write_stream(
                                                SshHttpServerConnection conn);
...
ssh_http_server_start(SshHttpServerParams *params)
{
  ...
  ctx->address = ssh_xstrdup(SSH_IPADDR_ANY);
  ctx->port = ssh_xstrdup("80");

  ctx->server_name = ssh_xmalloc(1024);
  ...
  if (params)
    {
      ...
      if (params->address)
        {
          ssh_xfree(ctx->address);
          ctx->address = ssh_xstrdup(params->address);
        }
      if (params->port)
        {
          ssh_xfree(ctx->port);
          ctx->port = ssh_xstrdup(params->port);
        }
      ...
      if (params->server_name)
        {
          ssh_xfree(ctx->server_name);
          ctx->server_name = ssh_xstrdup(params->server_name);
        }
    }
...
void
ssh_http_server_set_handler(SshHttpServerContext ctx, const char *uri_pattern,
                            ...
                            void *handler_context)
{
  ...
  reg = ssh_xcalloc(1, sizeof(*reg));
  reg->pattern = ssh_xstrdup(uri_pattern);
...
ssh_http_server_set_void_handler(SshHttpServerContext ctx,
                                 const char *uri_pattern, int priority,
                                 SshHttpServerVoidUriHandler handler,
                                 void *context)
{
  WrappedHandler h = ssh_xmalloc(sizeof(*h));

  h->context = context;
  h->handler = handler;
  ssh_http_server_set_handler(ctx, uri_pattern, priority,
                              generic_uri_handler, h);
}
  ...
  for (prev = NULL, reg = ctx->handlers; reg; prev = reg, reg = reg->next)
    {
      if ((uri_pattern == SSH_HTTP_ALL_PATTERNS
           || strcmp(uri_pattern, reg->pattern) == 0)
...
ssh_http_server_get_header_field(SshHttpServerConnection conn,
                                 const char *field)
{
  unsigned char *nfield = ssh_xstrdup(field);
  int i;
  ...
  if (strcasecmp((char *) cp, "Basic") == 0)
    {
      ...
      cleaned = ssh_base64_remove_whitespace(value + i, 0);
      decoded = ssh_base64_to_buf(cleaned, &value_len);
      ssh_xfree(cleaned);
      ...
      *name_return = ssh_xmemdup(decoded, cp - decoded);
      cp++;
      *password_return = ssh_xmemdup(cp, value_len - (cp - decoded));
  ...
  while (1)
    {
      if (!ssh_http_get_av(value, &i, &attr, &attr_len, &attr_val,
        ...
        malformed_cookie:
          ...
          SSH_DEBUG(0, ("Malformed `Cookie' header field: "
      ...
      if (attr[0] == '$')
              ...
              if (target)
                {
                  ...
                  *target = (char *)ssh_http_unescape_attr_value(attr_val,
                                                                 attr_val_len);
                }
            }
        }
      else
        {
          ...
          if (conn->cookies == NULL)
            {
              conn->cookies = ssh_xmalloc(sizeof(SshHttpCookie));
              conn->num_cookies = 1;
            }
          else
            {
              conn->num_cookies++;
              conn->cookies = ssh_xrealloc(conn->cookies,
                                           conn->num_cookies
                                           * sizeof(SshHttpCookie));
            }

          cookie = &conn->cookies[conn->num_cookies - 1];
          memset(cookie, 0, sizeof(*cookie));

          cookie->name = ssh_xmemdup(attr, attr_len);
          cookie->value = (char *)ssh_http_unescape_attr_value(attr_val,
                                                               attr_val_len);
        }
                            ...
                            SshBuffer buffer)
{
  ...
  /* Allocate a context. */
  ctx = (SshServerSendBufferCtx *) ssh_xcalloc(1, sizeof(*ctx));
                               ...
                               unsigned char *remote_port)
{
  ...
  conn->stream = stream;
  conn->address = ssh_xstrdup(remote_address);
  conn->port = ssh_xstrdup(remote_port);
  conn->local_address = ssh_xstrdup(local_address);
  ...
  if (keep_open && conn->ctx->listener != NULL)
    {
      ...
      ssh_xfree(conn->method);
      conn->method = NULL;
      ssh_xfree(conn->uri);
      conn->uri = NULL;
      ssh_xfree(conn->uri_path);
      conn->uri_path = NULL;
...
ssh_http_server_remove_connection(SshHttpServerConnection conn)
{
  ...
  ssh_xfree(conn->local_address);
  ssh_xfree(conn->address);
  ssh_xfree(conn->port);
      ...
      if (ctx->server_stopped_callback != NULL_FNPTR)
        {
          ...
          (*ctx->server_stopped_callback)(ctx, ctx->server_stopped_context);
        }
  ...
  for (i = 0; i < conn->num_cookies; i++)
    {
      ...
      ssh_xfree(cookie->name);
      ssh_xfree(cookie->value);
      ssh_xfree(cookie->path);
      ssh_xfree(cookie->domain);
      ssh_xfree(cookie->port);
    }

  ssh_xfree(conn->cookies);
  ...
  for (i = 0; i < conn->num_set_cookies; i++)
    {
      ...
      ssh_xfree(cookie->name);
      ssh_xfree(cookie->value);
      ssh_xfree(cookie->comment);
      ssh_xfree(cookie->comment_url);
      ssh_xfree(cookie->domain);
      ssh_xfree(cookie->expires);
      ssh_xfree(cookie->path);
      ssh_xfree(cookie->port);
    }

  ssh_xfree(conn->set_cookies);
      ...
      while (1)
        {
          ...
          ssh_buffer_append_space(&conn->in_buffer, &p, to_read);
        ...
        process_more:
          ...
          switch (conn->state)
            ...
            case SSH_HTTP_CONN_READING_REQUEST:
              ...
              conn->uri = ssh_xmemdup(p + start, i - start);

              /* Extract and decode the path part of the uri. */
              {
                const unsigned char *sep;

                sep = ssh_custr(strchr(ssh_csstr(conn->uri), '?'));
                if (sep == NULL)
                  sep = conn->uri + strlen(ssh_csstr(conn->uri));

                conn->uri_path = (char *)ssh_url_data_decode(conn->uri,
                                                             sep - conn->uri,
                                                             NULL);
              }
              ...
              SSH_DEBUG(5, ("%s %s HTTP/%d.%d",
                            conn->method, conn->uri, conn->major,
                            conn->minor));
              ...
              while (ssh_http_server_has_one_line(p, len, &end))
                {
                  ...
                  if (i > 0)
                  ...
                  else
                    {
                      ...
                      if (i >= end)
                        {
                          SSH_DEBUG(0, ("Bad header line.  No ':' found"));
                          /* Bzzttt.  Bad request. */
                          goto bad_request;
                        }
                      key_end = i;
                      ...
                      SSH_DEBUG(6, ("%.*s: %.*s",
                                    key_end - start, p + start,
                                    i - value_start, p + value_start));
...
ssh_http_server_create_handler(SshHttpServerConnection conn)
{
  ...
  if (value)
    {
      conn->r.content_length = strtoul(ssh_csstr(value), NULL, 10);
      ...
      if (value)
        {
          ...
          /* XXX the encoding can have multiple values. */
          if (strcasecmp(ssh_csstr(value), "chunked") != 0)
            {
  ...
  if (value)
    {
      if (strcasecmp(ssh_csstr(value), "close") == 0)
        conn->close = TRUE;
      if (strcasecmp(ssh_csstr(value), "keep-alive") == 0)
      ...
      if (value)
        {
          ...
          if (strcmp(ssh_csstr(value), "100-continue") != 0)
            {
          ...
          ssh_buffer_append_cstrs(&conn->out_buffer,
                                  "HTTP/1.1 100 Continue\r\n\r\n", NULL);
        }
    }

  /* Was it a `HEAD' request? */
  conn->is_head = (strcmp(conn->method, "HEAD") == 0);
  ...
  if (client_expects_100_continue)
    {
      client_expects_100_continue = FALSE;
  ...
  if (conn->ctx->listener)
    {
      ...
      for (reg = conn->ctx->handlers; reg; reg = reg->next)
        if (ssh_match_pattern(conn->uri_path, reg->pattern))
          {
            /* Spawn the handler. */
                                 ...
                                 Boolean input_is_chunked)
{
  ...
  /* Second, we create a write-only stream that writes data to the
     HTTP stream.  */
  write_s = ssh_http_server_content_write_stream(conn);
  ...
  if (conn->major == 1 && conn->minor >= 1)
    {
      ...
      write_s = ssh_http_chunked_stream_create(write_s, FALSE, TRUE,
                                               NULL_FNPTR, NULL);
    }
                                      ...
                                      va_list ap, void *context)
{
  ...
  msg = ssh_custr("<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML 2.0//EN\">\n"
                  "<HTML>\n<HEAD>\n<TITLE>");
  ssh_buffer_append(buffer, msg, strlen(ssh_csstr(msg)));

  ssh_snprintf(ssh_sstr(buf), sizeof(buf), "%ld %s", code,
               ssh_http_status_to_string(code));
  ssh_buffer_append(buffer, buf, strlen(ssh_csstr(buf)));
  ...
  switch (code)
    ...
    case SSH_HTTP_STATUS_MOVED_PERMANENTLY:
      ...
      ssh_buffer_append(buffer, msg, strlen(ssh_csstr(msg)));
      ssh_buffer_append(buffer, conn->uri, strlen(ssh_csstr(conn->uri)));
      ...
      while ((type = va_arg(ap, int)) != SSH_HTTP_HDR_END)
        {
          switch (type)
            ...
            case SSH_HTTP_HDR_LOCATION:
              ...
              ssh_buffer_append_cstrs(buffer, "<A HREF=\"", msg, "\">",
                                      msg, "</A>", NULL);
              break;
            ...
            case SSH_HTTP_HDR_LOCATION_RELATIVE:
              msg = va_arg(ap, unsigned char *);
              {
                ...
                cp = ssh_http_kvhash_get(conn->req_header_fields,
                                         ssh_custr("HOST"));
                if (cp)
                  ssh_buffer_append_cstrs(buffer, "<A HREF=\"http://",
                                          cp, prefix, msg, "\">",
                                          "http://", cp, prefix, msg, "</A>",
                                          NULL);
                else
      ...
      ssh_buffer_append(buffer, ssh_custr(".\n"), 2);
      break;
                                               ...
                                               Boolean include_separator)
{
  ...
  if (include_separator)
    ssh_buffer_append_cstrs(out_buffer, "; ", NULL);
  ...
  if (value_len_growth) /* malloc only when we have some ESCs or quotes
                         * to escape */
    {
      new_value = ssh_xmalloc(strlen(ssh_csstr(value)) + value_len_growth + 2);
      for (p = value; *p;)
        {
          if (*p == '"' || *p == '\\')
            *new_value++ = '\\';
  ...
  if (need_quotes)
    ssh_buffer_append_cstrs(out_buffer, name, "=\"", new_value, "\"", NULL);
  else
    ssh_buffer_append_cstrs(out_buffer, name, "=", new_value, NULL);
...
ssh_http_server_format_reply(SshHttpServerConnection conn)
{
  ...
  ssh_snprintf(ssh_sstr(buf), sizeof(buf), "HTTP/1.1 %d %s\r\n",
               conn->status, status_desc);
  ssh_buffer_append_cstrs(&conn->out_buffer, buf, NULL);
      ...
      if (conn->w.content_length_known)
        {
          ...
          ssh_snprintf(ssh_sstr(buf), sizeof(buf), "Content-Length: %u\r\n",
  ...
  if (conn->close)
    ssh_buffer_append_cstrs(&conn->out_buffer, "Connection: close\r\n", NULL);
  ...
  for (ssh_http_kvhash_reset_index(conn->reply_header_fields);
    ...
    ssh_buffer_append_cstrs(&conn->out_buffer, key, ": ", value, "\r\n", NULL);
      ...
      for (i = 0; i < conn->num_set_cookies; i++)
        {
          ...
          ssh_http_server_format_append_cookie_attribute(&conn->out_buffer,
                                                         ssh_ustr(cookie->
                                                                  name),
                                                         ssh_ustr(cookie->
                                                                  value),
          ...
          if (cookie->comment_url && cookie->set_cookie2)
            ssh_http_server_format_append_cookie_attribute(
                                           &conn->out_buffer,
                                           ssh_ustr("CommentURL"),
                                           ssh_ustr(cookie->comment_url),
                                           TRUE);
          /* `Discard' Set-Cookie2 */
          if (cookie->discard && cookie->set_cookie2)
            ssh_buffer_append_cstrs(&conn->out_buffer, "; Discard", NULL);
          ...
          if (cookie->max_age_given)
            {
              ssh_snprintf(ssh_sstr(buf), sizeof(buf), "%lu",
          ...
          /* `Secure' Netscape */
          if (cookie->secure)
            ssh_buffer_append_cstrs(&conn->out_buffer, "; Secure", NULL);

          /* The `Version' Set-Cookie */
          ssh_buffer_append_cstrs(&conn->out_buffer, "; Version=1", NULL);

          /* Is this the last cookie? */
          if (i + 1 < conn->num_set_cookies)
            /* No it is not.  Separate cookies with a ',' string. */
            ssh_buffer_append_cstrs(&conn->out_buffer, ",", NULL);
...
ssh_http_server_internal_error(SshHttpServerConnection conn)
{
  ...
  /* Append the message from the <conn->error_message> to the output
     buffer.  But not for the HEAD requests. */
  if (conn->method == NULL || strcmp(conn->method, "HEAD") != 0)
    ssh_buffer_append(&conn->out_buffer, ssh_buffer_ptr(conn->error_message),
  ...
  while ((type = va_arg(ap, int)) != SSH_HTTP_HDR_END)
    {
      switch (type)
        ...
        case SSH_HTTP_HDR_COOKIE_SEND_EXPIRES:
          ...
          if (!cookie->max_age_given)
            ssh_fatal("ssh_http_server_set_values: "
                      ...
                      "SSH_HTTP_HDR_COOKIE_MAX_AGE");
          cookie->expires = ssh_sstr(ssh_http_make_rfc1123_date(ssh_time() +
                                                                cookie->
                                                                max_age));
        ...
        case SSH_HTTP_HDR_DATE:
          cp = ssh_http_make_rfc1123_date(va_arg(ap, SshTime));
        ...
        case SSH_HTTP_HDR_LOCATION_RELATIVE:
          {
            ...
            if (v)
              ssh_buffer_append_cstrs(&value, "http://", v, prefix, loc, NULL);
            else
              ssh_buffer_append_cstrs(&value, "http://",
                                      conn->ctx->server_name, ":",
                                      conn->ctx->port, prefix, loc, NULL);
        ...
        case SSH_HTTP_HDR_PROXY_AUTHENTICATE_BASIC:
          {
            ...
            value = ssh_xmalloc(strlen(prefix) + strlen(realm) + 2);
            strcpy(value, prefix);
            strcat(value, realm);
            strcat(value, "\"");
        ...
        case SSH_HTTP_HDR_FIELD:
          {
            ...
            for (i = 0; ssh_http_server_hdr_skip_list[i]; i++)
              if (strcasecmp(ssh_http_server_hdr_skip_list[i],
                             ssh_csstr(key)) == 0)
                break;
        ...
        case SSH_HTTP_HDR_COOKIE:
          {
            ...
            if (conn->set_cookies == NULL)
              {
                conn->set_cookies = ssh_xmalloc(sizeof(SshHttpSetCookie));
                conn->num_set_cookies = 1;
              }
            else
              {
                conn->num_set_cookies++;
                conn->set_cookies = ssh_xrealloc(conn->set_cookies,
                                                 conn->num_set_cookies
            ...
            cookie->name = ssh_xstrdup(name);
            cookie->value = ssh_xstrdup(value);
          }
          break;
        ...
        case SSH_HTTP_HDR_FIELD_LEN:
          {
            ...
            for (i = 0; ssh_http_server_hdr_skip_list[i]; i++)
              if (strlen(ssh_http_server_hdr_skip_list[i]) == key_len
                  && strncasecmp(ssh_http_server_hdr_skip_list[i],
                                 ssh_csstr(key), key_len) == 0)
                break;
...
{
  SshStream str;
  str = ssh_stream_create(&ssh_http_server_content_read_methods_table, conn);
  if (str == NULL)
                                   ...
                                   size_t size)
{
  ...
  if (!conn->is_head)
    ssh_buffer_append(&conn->out_buffer, buf, avail);
...
{
  SshStream str;
  str = ssh_stream_create(&ssh_http_server_content_write_methods_table,
                          conn);
  if (str == NULL)
    ssh_fatal("Insufficient memory to create HTTP read stream");
  return str;
...
ssh_http_server_byte_sink_write_response_timeout(void *context)
{
  ...
  SSH_DEBUG(0, ("Closing the connection"));
  byte_sink = (SshHttpServerSmartByteSink *) context;
...
ssh_http_server_ui_destroy(void *context)
{
  ...
  if ((!stream_ctx->read_s_at_eof && !stream_ctx->conn->close)
      || !stream_ctx->written)
    {
      ...
      byte_sink = ssh_xcalloc(1, sizeof(*byte_sink));
                          ...
                          SshStream write_s)
{
  ...
  str = ssh_stream_create(&ssh_http_server_ui_methods_table,
                          stream_ctx);
  if (str == NULL)
    ssh_fatal("Insufficient memory to create HTTP UI stream");
  return str;
}

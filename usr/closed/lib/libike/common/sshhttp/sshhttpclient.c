...
static SshStream ssh_http_content_write_stream_create(
                                        SshHttpClientContext ctx);

/* Read stream to pass the server content data to the user. */
static SshStream ssh_http_content_read_stream_create(SshHttpClientContext ctx);
      ...
      if (params->http_proxy_url)
        {
          if (ssh_url_parse(params->http_proxy_url, NULL,
                            (unsigned char **)&ctx->proxy_name,
                            (unsigned char **)&ctx->proxy_port,
                            (unsigned char **)&ctx->proxy_user_name,
                            (unsigned char **)&ctx->proxy_password,
                            NULL))
            {
...
ssh_http_client_uninit(SshHttpClientContext ctx)
{
  ...
  ssh_xfree(ctx->socks);
  ssh_xfree(ctx->proxy_name);
  ssh_xfree(ctx->proxy_port);
  ssh_xfree(ctx->proxy_user_name);
  ssh_xfree(ctx->proxy_password);
  ssh_xfree(ctx->user_name);
  ssh_xfree(ctx->password);
  ...
  ssh_xfree(ctx->status_reason_phrase);
...
const char *
ssh_http_get_header_field(SshHttpClientContext ctx, const char *field)
{
  ...
  while (1)
    {
      if (!ssh_http_get_av(value, &i, &attr, &attr_len, &attr_val,
        ...
        {
        malformed_cookie:
          SSH_DEBUG(0, ("Malformed `Set-Cookie' header field"));
          break;
        }
      ...
      if (first_av)
        {
          ...
          if ((attrp = ssh_memdup(attr, attr_len)) == NULL ||
              (valuep =
               (char *)ssh_http_unescape_attr_value(attr_val, attr_val_len))
              == NULL)
            {
              goto malformed_cookie;
            }
      ...
      if (first_av)
        {
          ...
          if (cookie->expires && !cookie->max_age_given)
            {
              /* XXX Count the Max-Age from the Expires.*/
              /* XXX */
            }
        }
...
SshOperationHandle
ssh_http_get(SshHttpClientContext ctx, const char *url,
             SshHttpClientResultCb callback, void *callback_context, ...)
{
  ...
  operation = ssh_http_operation(ctx, SSH_HTTP_GET, ssh_custr(url), NULL, 0,
                                 NULL_FNPTR, NULL,
...
SshOperationHandle
ssh_http_head(SshHttpClientContext ctx, const char *url,
              SshHttpClientResultCb callback, void *callback_context, ...)
{
  va_list ap;
  SshOperationHandle operation;
  ...
  operation = ssh_http_operation(ctx, SSH_HTTP_HEAD, ssh_custr(url), NULL, 0,
                                 NULL_FNPTR, NULL,
                                 callback, callback_context, ap);
...
SshOperationHandle
ssh_http_post(SshHttpClientContext ctx, const char *url,
              ...
              SshHttpClientResultCb callback, void *callback_context, ...)
{
  ...
  operation = ssh_http_operation(ctx, SSH_HTTP_POST, ssh_custr(url),
...
SshOperationHandle
ssh_http_post_stream(SshHttpClientContext ctx, const char *url,
                     SshHttpClientConnectCb connect_callback,
                     void *connect_context,
                     SshHttpClientResultCb result_callback,
                     void *result_context, ...)
{
  ...
  operation = ssh_http_operation(ctx, SSH_HTTP_POST, ssh_custr(url), NULL, 0,
...
SshOperationHandle
ssh_http_put(SshHttpClientContext ctx, const char *url,
             const unsigned char *content_data, size_t content_data_len,
             SshHttpClientResultCb callback, void *callback_context, ...)
{
  va_list(ap);
  ...
  operation = ssh_http_operation(ctx, SSH_HTTP_PUT, ssh_custr(url),
                                 content_data, content_data_len, NULL_FNPTR,
                                 NULL, callback, callback_context, ap);
...
SshOperationHandle
ssh_http_put_stream(SshHttpClientContext ctx, const char *url,
                    SshHttpClientConnectCb connect_callback,
                    void *connect_context,
                    SshHttpClientResultCb result_callback,
                    void *result_context, ...)
{
  ...
  operation = ssh_http_operation(ctx, SSH_HTTP_PUT, ssh_custr(url), NULL, 0,
                                 connect_callback, connect_context,
                                 result_callback, result_context, ap);
                       ...
                       const unsigned char *url)
{
  ...
  if (!ssh_url_parse(url, &url_scheme,
                     (unsigned char **)&req->host_name,
                     (unsigned char **)&req->port,
                     (unsigned char **)&req->user_name,
                     (unsigned char **)&req->password,
  ...
  if (strcasecmp(ssh_csstr(url_scheme), "http") != 0)
    {
      /* Not supported. */
      if (req->r.callback)
  ...
  if (request)
    {
      ...
      ssh_xfree(request->host_name);
      ssh_xfree(request->port);
      ssh_xfree(request->user_name);
      ssh_xfree(request->password);
      ssh_xfree(request->proxy_user_name);
      ssh_xfree(request->proxy_password);
      ssh_xfree(request->authorization);
      ssh_xfree(request->proxy_authorization);
      ssh_xfree(request->uri);
      ...
      ssh_xfree(request->cookies);
      ssh_xfree(request->w.content_data);
  ...
  while ((type = va_arg(ap, int)) != SSH_HTTP_HDR_END)
    {
      switch (type)
        ...
        case SSH_HTTP_HDR_COOKIE_PORT:
          ...
          /* XXX SSH_HTTP_AUTHORIZATION_DIGEST */
        ...
        case SSH_HTTP_HDR_FIELD:
          {
            ...
            for (i = 0; ssh_http_client_hdr_skip_list[i]; i++)
              if (strcasecmp(ssh_http_client_hdr_skip_list[i], ssh_csstr(key))
                  == 0)
                break;
        ...
        case SSH_HTTP_HDR_FIELD_LEN:
          {
            ...
            for (i = 0; ssh_http_client_hdr_skip_list[i]; i++)
              if (strlen(ssh_http_client_hdr_skip_list[i]) == key_len
                  && strncasecmp(ssh_http_client_hdr_skip_list[i],
...
ssh_http_format_http_request(SshHttpClientContext ctx)
{
  ...
  ssh_buffer_append_cstrs(&ctx->out_buffer, method, NULL);

  if (ctx->proxy_name)
    {
      if (strcmp(ssh_csstr(req->port), "80") == 0)
        ssh_buffer_append_cstrs(&ctx->out_buffer,
                                "http://", req->host_name, "/", NULL);
      else
        ssh_buffer_append_cstrs(&ctx->out_buffer,
                                "http://", req->host_name, ":", req->port, "/",
                                NULL);
    }
  else
    ssh_buffer_append_cstrs(&ctx->out_buffer, "/", NULL);

  ssh_buffer_append_cstrs(&ctx->out_buffer,
                          req->uri, " HTTP/1.", req->use_http_1_0 ? "0" : "1",
                          "\r\n", NULL);
  ...
  if (strcmp(ssh_csstr(host_port), "80") == 0)
    ssh_buffer_append_cstrs(&ctx->out_buffer,
                            "Host: ", host_name, "\r\n", NULL);
  else
    ssh_buffer_append_cstrs(&ctx->out_buffer,
  ...
  if (req->proxy_authorization)
    ssh_buffer_append_cstrs(&ctx->out_buffer,
                            "Proxy-Authorization:", req->proxy_authorization,
                            "\r\n", NULL);
 ...
 /* The content data length and its possible transfer encoding. */
          ...
          if (req->w.content_data)
            {
              ...
              ssh_snprintf(buf, sizeof(buf), "Content-Length: %d\r\n",
                           req->w.content_data_len);
              ssh_buffer_append_cstrs(&ctx->out_buffer, buf, NULL);
            }
  ...
  if (req->use_http_1_0)
  ...
  else                          /* HTTP/1.1 specific fields. */
    {
      if (req->has_content_data && !req->no_expect_100_continue)
        ssh_buffer_append_cstrs(&ctx->out_buffer, "Expect: 100-continue\r\n",
                                NULL);
    }
  ...
  if (req->w.close)
    {
      ...
      ssh_buffer_append_cstrs(&ctx->out_buffer, "Connection: close\r\n", NULL);
    }
  else if (req->use_http_1_0)
    {
      /* HTTP/1.0 `Connection: Keep-Alive'. */
      ssh_buffer_append_cstrs(&ctx->out_buffer, "Connection: Keep-Alive\r\n",
                              NULL);
    }
  ...
  for (ssh_http_kvhash_reset_index(req->header_fields);
    ...
    ssh_buffer_append_cstrs(&ctx->out_buffer, key, ": ", value, "\r\n", NULL);
      ...
      switch (ctx->req->w.state)
        ...
        case SSH_HTTP_REQUEST_WRITING_HEADER:
          ...
          while (ssh_buffer_len(&ctx->out_buffer) != 0)
            {
              SSH_DEBUG(8, ("Can output, sending %ld bytes",
              ...
              if (l == 0)
                {
                  ...
                  /* Report the failure. */
                  if (req->r.callback)
                    (*req->r.callback)(ctx, SSH_HTTP_RESULT_CONNECTION_CLOSED,
                                       SSH_TCP_OK, NULL,
          ...
          if (req->has_content_data)
            {
              ...
              ctx->w.stream = ssh_http_content_write_stream_create(ctx);

              /* Chunked transfer coding. */
              if (req->use_chunked_te)
                ctx->w.stream = ssh_http_chunked_stream_create(ctx->w.stream,
                                                               FALSE, TRUE,
                                                               NULL_FNPTR,
                                                               NULL);
      ...
      while (1)
        {
          ...
          ssh_buffer_append_space(&ctx->in_buffer, &p, to_read);
          ...
          SSH_DEBUG(9, ("read=%ld, buffered=%ld", l,
                        ssh_buffer_len(&ctx->in_buffer)));
...
ssh_http_finish_request(SshHttpClientContext ctx)
{
      ...
      switch (ctx->status_code)
	      ...
	      if (s)
		{
		  ...
		  ssh_free(s); ssh_free(u);
                  ...
                  ssh_xfree(req->authorization);
                  req->authorization = NULL;

                  ssh_xfree(req->proxy_authorization);
                  req->proxy_authorization = NULL;

                  ssh_xfree(req->user_name);
                  req->user_name = NULL;

                  ssh_xfree(req->password);
                  req->password = NULL;

                  ssh_xfree(req->proxy_user_name);
                  req->proxy_user_name = NULL;

                  ssh_xfree(req->proxy_password);
                  req->proxy_password = NULL;

                  old_uri = (uchar_t *)req->uri;
                  req->uri = NULL;
	      ...
	      if (old_host_name &&
		  strcmp(ssh_csstr(old_host_name),
                             ssh_csstr(req->host_name)) == 0 &&
		  old_port &&
                      strcmp(ssh_csstr(old_port),
                             ssh_csstr(req->port)) == 0)
                    recycle_same_server = TRUE;

              /* XXX Can we redirect all methods??? */
  ...
  if (req->w.state == SSH_HTTP_REQUEST_WRITTEN
      ...
      && (ctx->proxy_name != NULL
          ...
          || (!recycle_request
              && strcasecmp(ssh_csstr(req->host_name),
                            ssh_csstr(nreq->host_name)) == 0
              && strcasecmp(ssh_csstr(req->port),
                            ssh_csstr(nreq->port)) == 0)))
    {
      keep_alive = TRUE;
      ...
      if (ctx->version_minor == 0)
        {
          ...
          if (!value || strcasecmp(ssh_csstr(value), "KEEP-ALIVE") != 0)
            keep_alive = FALSE;
        }
    }
          ...
          if (j >= i)
            {
              ...
              if (value)
                {
                  ...
                  ctx->r.user_content_length = strtoul(ssh_csstr(value), NULL,
                                                       10);
                  ctx->r.user_content_data_read = 0;
              ...
              if (value && strcasecmp(ssh_csstr(value), "close") == 0)
                {
  ...
  if (value)
    {
      ...
      end = value + strlen(ssh_csstr(value));
      while (1)
        {
          ...
          if ((cmpvalue = ssh_xmemdup(start, end - start)) == NULL)
            {
              ctx->r.stream = NULL;
          ...
          if (strcasecmp(ssh_csstr(cmpvalue), "chunked") == 0)
            {
              /* Chunked Transfer Coding. */
              if (++chunked_count > 1)
                {
                  SSH_DEBUG(0, ("Chunked Transfer Coding applied more "
                                "than once"));
              ...
              user_stream = ssh_http_chunked_stream_create(
            ...
            {
              SSH_DEBUG(0, ("Unknown transfer encoding `%s'",
                            cmpvalue));
              ...
              cmpvalue = ssh_xmemdup(value, end - value);
...
ssh_http_authentication(SshHttpClientContext ctx)
{
  ...
  u = req->user_name ? req->user_name : "";
  p = req->password ? req->password : "";

  value = ssh_xmalloc(strlen(u) + 1 + strlen(p) + 1);
  strcpy(value, u);
  strcat(value, ":");
  strcat(value, p);

  base64 = (char *) ssh_buf_to_base64((unsigned char *) value, strlen(value));
  ssh_xfree(value);

  value = ssh_xmalloc(strlen("Basic ") + strlen(base64) + 1);
  strcpy(value, "Basic ");
  strcat(value, base64);
...
ssh_http_proxy_authentication(SshHttpClientContext ctx)
{
  ...
  u = req->proxy_user_name ? req->proxy_user_name : "";
  p = req->proxy_password ? req->proxy_password : "";
...
ssh_http_content_write_stream_create(SshHttpClientContext ctx)
{
  ...
  str = ssh_stream_create(&ssh_http_content_write_stream_methods_table, ctx);
  if (str == NULL)
    ssh_fatal("Insufficient memory to create HTTP write stream");
  return str;
}
...
ssh_http_content_read_stream_create(SshHttpClientContext ctx)
{
  ...
  str = ssh_stream_create(&ssh_http_content_read_stream_methods_table,
                          stream_ctx);

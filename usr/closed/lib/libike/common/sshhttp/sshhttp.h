...
/* Wildcard selectors for the ssh_http_server_remove_handlers()
   function. */
#define SSH_HTTP_ALL_PATTERNS       ((char *) 1)
...
typedef struct SshHttpServerParamsRec
{
  ...
  /* The port that the server will listen to.  The dafault port is
     80. */
  char *port;
...
SshOperationHandle ssh_http_get(SshHttpClientContext ctx, const char *url,
...
/* Perform an HTTP head request for URL <url>.  The function calls the
   ...
   request.  Additional options and header fields can be given as a
   SshHttpHeaderFieldSelector list after the <callback_context>. */
SshOperationHandle ssh_http_head(SshHttpClientContext ctx, const char *url,
                                 SshHttpClientResultCb callback,
                                 void *callback_context,
                                 ...);
...
/* Perform an HTTP post request for URL <url>.  The content data of
   ...
   request.  Additional options and header fields can be given as a
   SshHttpHeaderFieldSelector list after the <callback_context>. */
SshOperationHandle ssh_http_post(SshHttpClientContext ctx, const char *url,
...
/* Perform an HTTP put request for URL <url>.  The content data of the
   ...
   request.  Additional options and header fields can be given as a
   SshHttpHeaderFieldSelector list after the <callback_context>. */
SshOperationHandle ssh_http_put(SshHttpClientContext ctx, const char *url,
                                const unsigned char *content_data,
                                size_t content_data_len,
                                SshHttpClientResultCb callback,
                                void *callback_context,
...
const char *ssh_http_get_header_field(SshHttpClientContext ctx,
                                      const char *field);
...
/* Bind a handler function for the URI pattern <uri_pattern>.  The
   ...
   handlers with the priority abs(priority). */
void ssh_http_server_set_handler(SshHttpServerContext ctx,
...
/* Identical to ssh_http_server_set_handler() but for a different type of
   callback function. */
void ssh_http_server_set_void_handler(SshHttpServerContext ctx,
                                      const char *uri_pattern, int priority,
...
/* Get the IP address of the client.  The returned value is valid as
   long as the connection is alive. */
const char *ssh_http_server_get_address(SshHttpServerConnection conn);

/* Get the port of the client.  The returned value is valid as long as
   the connection is alive. */
const char *ssh_http_server_get_port(SshHttpServerConnection conn);
...
const char *ssh_http_server_get_method(SshHttpServerConnection conn);
...
/* Get the request URI from the connection <conn>.  The returned URI
   ...
   function. */
const char *ssh_http_server_get_uri(SshHttpServerConnection conn);
...
const char *ssh_http_server_get_header_field(SshHttpServerConnection conn,
                                             const char *field);
...
/* XXX Should add more shortcut error reporters */

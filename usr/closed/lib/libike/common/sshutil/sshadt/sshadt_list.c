...
#ifndef _KERNEL
static void reallocated(SshADTContainer c, SshADTHandle old, SshADTHandle newp)
{
  SshADTListNodeStruct *n = newp;
  if (n->prev == NULL) ROOT(c)->first_node = n;
  else n->prev->next = n;
  if (n->next == NULL) ROOT(c)->last_node = n;
  else n->next->prev = n;
}
#endif  /* _KERNEL */

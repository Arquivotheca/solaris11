  ...
  if (handle != NULL)
    {
      ...
      /* Clear the handle so that in the pre-allocated memory case,
         this is a valid handle for which the
         ssh_operation_unregister() can be called without any
         damage. */
      memset(handle, 0, sizeof(*handle));

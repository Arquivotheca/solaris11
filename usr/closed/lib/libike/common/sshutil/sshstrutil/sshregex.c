/*
  ...
  Created Tue Sep  7 12:07:23 1999.

  */

/* This file implements the interface for matching regular expressions in
   sshregex.h.

   The matching algorithm is worst-case linear in both the length of regular
   expression and the length of matched text, i.e.

     O(|e| |t|)

   if `e' is the regex and `t' is the text.

   The O(|t|) factor is there always.  The factor O(|e|) is typically much
   smaller.

   Alert: this file is very badly documented.  Do not modify the algorithms
...
static Boolean add_to_literal(SshRexLiteral *l, const unsigned char *data,
                              size_t len)
{
  l->data = ssh_xrealloc(l->data, l->data_len + len);
                                  ...
                                  size_t *match_len)
{
  ...
  *index = m->matches[subexpr_num].from - m->text;
  ...
  for (x = 0; x < dlex->num_matches; x++)
    {
      if (dlex->matches[x].from != NULL)
        {
          result = ssh_regex_access_submatch(dlex, x, &idx,
                                             (size_t *)match_len);

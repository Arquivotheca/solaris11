...
unsigned char *ssh_fingerprint_babble(const unsigned char *digest, 
                                      size_t digest_len);
unsigned char *ssh_fingerprint_babble_upper(const unsigned char *digest, 
                                            size_t digest_len);
unsigned char *ssh_fingerprint_pgp2(const unsigned char *digest,
                                    size_t digest_len);
unsigned char *ssh_fingerprint_pgp5(const unsigned char *digest,
                                    size_t digest_len);
unsigned char *ssh_fingerprint_hex(const unsigned char *digest,
                                   size_t digest_len);
unsigned char *ssh_fingerprint_hex_upper(const unsigned char *digest,
                                         size_t digest_len);
  ...
  for (x = 0; x < digest_len; x++)
    {
      ...
      ssh_dsprintf((char **) &b, "%s%s%02X", a, c, (unsigned int)digest[x]);
      ssh_xfree(a);
      a = b;
    }
  return a;
                                   ...
                                   size_t digest_len)
{
  ...
  for (x = 0; x < digest_len; x++)
    {
      /* XXX temporary casts until library API is changed XXX */
      ssh_dsprintf((char **) &b, "%s%02x", a, (unsigned int)digest[x]);
      ssh_xfree(a);
      a = b;
    }
  return a;

...
#define SSH_IS_IPADDR_ANY(addr) \
  ((addr)                       \
   && ((addr) == ssh_custr(SSH_IPADDR_ANY) \
       || strcmp(ssh_csstr(addr), SSH_IPADDR_ANY) == 0))
...
typedef enum {
  ...
  SSH_IPPROTO_RSVP_E2E_IGNORE = 134, /* RSVP-E2E-IGNORE [RFCXXXX] */
...
typedef struct SshIpAddrRec
{
  ...
  union {
    ...
    unsigned char _addr_data[4];
#else /* WITH_IPV6 */
    unsigned char _addr_data[16];
#endif /* !WITH_IPV6 */
...
#define SSH_IP_IS_NULLADDR(ip_addr)                     \
  (SSH_IP_DEFINED(ip_addr)                              \
   ? (SSH_IP_IS6(ip_addr)                               \
      ...
      : !memcmp((ip_addr)->addr_data, "\0\0\0\0", 4))   \
   : 0)
...
/* ICMP offsets to headers etc. (no HDRLEN since ICMP headers vary in
   size */
...
#define SSH_IPH6_CLASS(ucp) ((SSH_GET_32BIT(ucp) & SSH_IPH6_CLASS_MASK) \

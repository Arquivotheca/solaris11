...
void ssh_decay_counter_timer(void *context)
{
  ...
  if (counter->next_time < now)
    ssh_xregister_timeout(0, 0, ssh_decay_counter_timer, context);
  else
    ssh_xregister_timeout((long)(counter->next_time - now), 0,
                                           ...
                                           SshTime interval)
{
  ...
  counter = ssh_xcalloc(1, sizeof(*counter));
  if (interval <= 100)
    {
  ...
  ssh_xregister_timeout((long)counter->timer, 0, ssh_decay_counter_timer,
                        counter);
  return counter;
}

...
do {                                                                          \
  if (size_limit && size_limit < offset)                                      \
    {                                                                         \
      SSH_DEBUG(SSH_D_FAIL, ("File '%s': Size limit (%ld) exceeded (%ld)",    \
                             file_name, size_limit, offset));                 \
      goto failed;                                                            \
    }                                                                         \
} while (0)
                                 ...
                                 size_t *buf_len)
{
  ...
  if (fp == NULL)
    {
      SSH_DEBUG(SSH_D_FAIL, ("Can't open file %s for reading", file_name));
      return FALSE;
    }
  ...
  while ((ret = fread(iobuf + offset, 1, growth, fp)) == growth)
    {
      ...
      SSH_DEBUG(SSH_D_HIGHOK,
                ("Growing input buffer from %ld to %ld bytes",
                 plen, len));

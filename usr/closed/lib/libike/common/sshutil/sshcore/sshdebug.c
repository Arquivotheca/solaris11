...
#ifdef WANT_DEBUG_PARSER
SSH_GLOBAL_DECLARE(const char *, ssh_debug_default_format);
#define ssh_debug_default_format SSH_GLOBAL_USE(ssh_debug_default_format)
SSH_GLOBAL_DEFINE(const char *, ssh_debug_default_format);
...
void ssh_debug_set_module_level(const char *module, unsigned int level)
{
  ...
  /* XXX: How to fail on memory failure? Well, it's only debug
     level. Nothing fatal, I hope? */
...
void ssh_debug_set_level_string(const unsigned char *string)
{
  const unsigned char *name_start, *name_end, *level_start;
  char *name;
  size_t name_len;
  long level_value;
  Boolean error;
  ...
  while (*string)
    {
      ...
      if (*string == '=')
        {
          ...
          if (isdigit(*string))
            {
              level_value = atoi(ssh_csstr(string));
      ...
      if (name_len == 6 &&
          strncmp(ssh_csstr(name_start), "global", name_len) == 0)
        {
          if (level_value == -1)
            level_value = 0;
      ...
      else
        {
          ...
          if (name_len > 0 && isdigit(*name_start))
            {
              level_value = atoi(ssh_csstr(name_start));
              ssh_debug_set_global_level((unsigned int)level_value);
            }
          else
            {
  ...
  for (i = 0; i < buf_siz; i += 0x10)
    {
      ...
      for (j = 0; j < jmax; j++)
        {
          if ((j % 2) == 1)
            ssh_snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
      ...
      for (j = 0; j < jmax; j++)
        {
          c = data[i+j];
          c = c < 32 || c >= 127 ? '.' : c;
          ssh_snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%c", c);
        }
      ssh_debug("%s", buf);
    }
}
...
/* XXX Why does this unbuffer stdout and stderr unilaterally? At least
   the function is ill-named... //sjl 2002/10/07  */
static int
...
#ifndef KERNEL
#ifndef SUNWIPSEC
  abort();  /* Don't make apps dump core. */
#endif
  exit(1);
#endif /* KERNEL */
}
...
int ssh_generic_assert(int value, const char *expression,
                       const char *file,
              ...
              if (temp == NULL)
                {
                  if ((item->arg = ssh_malloc(strlen(arg) + 10)) != NULL)
                    ssh_snprintf(item->arg, strlen(arg) + 10,
                                 "<%s unset>", arg);
                }
              else
                {
...
failure: /* something went wrong... give a warning and fall
  ...
  /* XXX: How to recover from memory allocation failure here? */
          ...
          if (l > 0)
            {
              ...
              if (ssh_debug_format_wrapcol > 0 && inhibit_wrap == 0 &&
              ...
              else
                {
                  ...
                  strncpy(current_ptr, fragment, l);
                  current_ptr += l;
                  total_len += l;
                  column += l;
                }

/*
  ...
  Created Wed Oct 18 22:59:35 2000.
...
#if 0
/* XXX This would be the preferred implementation, but for obvious
   reasons it doesn't work correctly. */
static SshUInt32 icg_invert(SshUInt32 x, SshUInt32 p)
{
  SshUInt32 u0,u1,v0,v1;
  u0 = 1; u1 = x; v0 = 0; v1 = p;
  while (v1 != 0)
    {
      SshUInt32 t1, t2, t3;
                           ...
                           SshUInt32 lo, SshUInt32 hi)
{
  ...
  if (hi <= lo)
    ssh_fatal("ssh_rand: too narrow or invalid range ([%u,%u]).", lo,hi);
  ...
  if (n > 0x80000000)
    ssh_fatal("ssh_rand: too wide range ([%u,%u]).", lo,hi);
...
static SshRandStruct rand_default_state =
{
  {
    /* Default LFG. */

...
/* Uninitialize a memory allocator, with the provision that some of the blobs
   can be unfreed at this point.  No error will be triggered for that.  Should
   be used only if it is clearly not an error to have unfreed blobs. */
...
/* Reserve room for allocating at least `objects' new objects.  If this call
   returns TRUE, then the allocator has reserved room for `objects' new objects
   and it is guaranteed that they can be allocated without failures. If the
   call returns FALSE then there isn't currently enough memory available. */

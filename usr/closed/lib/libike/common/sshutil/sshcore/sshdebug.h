...
/* USAGE:
   ...
   The module numbers are assigned in sshdmod.h and the mapping from
   numbers to names is found in sshdmod.c.
   ...
   that the system in general is in a state where running the code of
   the function is allowable.
   ...
   SSH_INVARIANT(...) is used exclusively in for- and while-loops. It
   is used to check that a certain invariant holds for all iterations.
   ...
   appears is actually reached during execution. SSH_NOTREACHED is not
   compiled unless DEBUG_LIGHT is defined. Here are simple examples:
...
#define SSH_NOTREACHED          ((void) 0)
#define SSH_ELSE_NOTREACHED
#endif

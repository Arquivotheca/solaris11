...
SshAuditContext
ssh_audit_create(SshAuditCB callback, void *context)
{
  SshAuditContext ctx;
...
ssh_audit_event_va(SshAuditContext context, SshAuditEvent event, va_list ap)
{
  ...
  if (context == NULL || context->callback == NULL_FNPTR)
    return;
  ...
  (*context->callback)(event, argc, context->arguments,
                      ...
                      SshUInt32 argc, SshAuditArgument argv)
{
  ...
  /* Pass the event ot the user. */
  (*context->callback)(event, argc, argv, context->callback_context);
}

...
static void ldap_abort_operation(void *context)
{
  ...
  ssh_ldap_free_operation(op->client, op);
}
                       ...
                       SshLdapClientResultCB callback, void *callback_context)
{
  ...
  if ((h = ssh_adt_enumerate_start(client->freelist)) != SSH_ADT_INVALID)
    {
  ...
  if ((op->operation = ssh_operation_register(ldap_abort_operation, op))
      == NULL)
    {
      ssh_free(op);
      return NULL;
...
ssh_ldap_get_operation(SshLdapClient client, SshLdapMessageID id)
{
  ...
  optemp.id = id;
  if ((handle = ssh_adt_get_handle_to_equal(client->operations, &optemp))
      != SSH_ADT_INVALID)
...
ssh_ldap_free_operation(SshLdapClient client, SshLdapClientOperation op)
{
  ...
  if ((handle = ssh_adt_get_handle_to(client->operations, op))
      != SSH_ADT_INVALID)
    {
      if (op->operation) ssh_operation_unregister(op->operation);
      ...
      if ((asn1context = ssh_asn1_init()) != NULL)
        {
          if ((status =
              ...
              == SSH_ASN1_STATUS_OK)
            {
              ok = TRUE;
              if ((lresult =
                   ssh_ldap_send_operation(client, asn1context, message))
                  != SSH_ASN1_STATUS_OK)
                {
                  ok = FALSE;
                }
...
SshLdapClient ssh_ldap_client_create(const SshLdapClientParams params)
{
  ...
  if ((client = ssh_calloc(1, sizeof(*client))) != NULL)
    {
      client->current_id = 1;
...
{
  SSH_DEBUG(SSH_D_HIGHSTART, ("destroy(0x%x) v=%d", client, client->version));

  ssh_ldap_client_disconnect(client);

  if (client->socks) ssh_free(client->socks);

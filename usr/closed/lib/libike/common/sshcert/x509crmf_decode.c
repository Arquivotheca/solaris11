  ...
  if (key_input_found)
    {
      ...
      if (pubkey &&
          ssh_x509_decode_public_key(context, pubkey, &pop->pkey)
          != SSH_X509_OK)
      ...
      for (; pubinfo; pubinfo = ssh_asn1_node_next(pubinfo))
        {
          ...
          if (namenode_found)
            ssh_x509_decode_general_name(context,
      ...
      if (failcount == 0)
        {
          ...
          ssh_asn1_create_node(context, &opt_time,
                               "(sequence (4) (any ()))", tmp);
          failcount++;
          goto again;
        }
  ...
  if (public_key_found)
    {
      ...
      ssh_asn1_create_node(context, &tmp, "(sequence () (any ()))", tmp);

      if (ssh_x509_decode_public_key(context, tmp, &cert->subject_pkey)
          != SSH_X509_OK)
        {
                                        ...
                                        SshX509Certificate cert)
{
  ...
  if (pop_found)
    {
      /* A POP (proof-of-posessioin) message was found in the request. */
      switch (which_pop)
        {
        case 0:

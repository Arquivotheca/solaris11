...
Boolean ssh_x509_library_initialize_framework(SshX509Config config)
{
  ...
  return ssh_crypto_library_initialize() == SSH_CRYPTO_OK;
}
...
const SshX509PkAlgorithmDefStruct ssh_x509_pk_algorithm_def[] =
{
  ...
  { "dl-modp", NULL, "dh-none",
    ...
    SSH_X509_PKALG_DH },
  /* XXX: this is here only to satisfy needs of new crypto library where
     the keys have no default scheme. Keep it last resort. */
                                    ...
                                    SshX509PkAlgorithm *type)
{
  ...
  /* XXX: What should we be doing with the algorithm params? Maybe add
     some function pointers to the oid table (or somewhere else for
     this)? */

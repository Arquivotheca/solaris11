...
/* A list of all supported Oids. Another idea would be to allow
   inclusion of external oids by configuration file. We should
   implement such thing later. XXX */
...
const SshOidStruct ssh_oid_list_sig[] =
{
  /* Define the signature Oids. We shouldn't be using these 1.3.14.7.2.*
     numbers, because they are from the company that is out of bussiness */
  { "1.3.14.7.2.3.2",
...
const SshOidStruct ssh_oid_list_dn[] =
{
  ...
  { "2.5.4.5",              "SN", "serial number", NULL, 2 },
  ...
  { "2.5.4.4",              "S", "surname", NULL, 10 },
  { "2.5.4.12",             "T", "title", NULL, 11 },
  { "2.5.4.41",             "N", "name", NULL, 12 },
  { "2.5.4.42",             "G", "given name", NULL, 13 }, /* GNAME */
  { "2.5.4.43",             "I", "initials", NULL, 14 },
  { "2.5.4.44",             "GQ", "generation qualifier", NULL, 15 },
  /* XXX UI will not get encoded/decoded properly in RDNs */
  { "2.5.4.45",             "UI", "X.500 unique identifier", NULL, 16 },
  { "2.5.4.46",             "DNQ", "dn qualifier", NULL, 17 },
  ...
  /* XXX will not get encoded/decoded properly in RDNs or anywhere */
...
/* Note that this table must be in the order specified in the oid.h
   (same order thatn SSH_OID_* defines). */
                                                ...
                                                int type)
{
  ...
  for (i = 0; list[i].oid != NULL; i++)
    if (strcmp(list[i].oid, ssh_csstr(oid)) == 0)
      return &list[i];
  return NULL;
}

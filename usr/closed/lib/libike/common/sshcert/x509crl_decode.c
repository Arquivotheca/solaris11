                                       ...
                                       SshX509Crl     crl)
{
  ...
  /* Fail if version suggest no extensions. */
#if 0 /* XXX VRK case. */
  if (crl->version == SSH_X509_VERSION_1 && ext_found)
    {
      rv = SSH_X509_FAILED_VERSION_CHECK;
      goto failed;
    }
...
failed:
  if (rv != SSH_X509_OK)
    {
      for (prev_rc = crl->revoked; prev_rc; prev_rc = rc)
	{
	  rc = prev_rc->next;
	  ssh_x509_revoked_free(prev_rc);
	}
...
/* XXX Write a function that computes the length of an CRL in a
   buffer. */

...
#define SSH_VRK_UID "2.5.4.45"
              if (strcmp(ssh_csstr(temp_rdn->oid), SSH_VRK_UID) == 0)
                {
                  SshAsn1Tree tree;
      ...
      temp_rdn->c = ssh_str_make(charset, temp_str, temp_str_length);
...
SshCharset ssh_dn_charset_get_by_oid(const unsigned char *oid)
{
  int i;
    ...
    {
      if (strcmp(ssh_csstr(oid), ssh_rdn_preferred_charsets[i].oid) == 0)

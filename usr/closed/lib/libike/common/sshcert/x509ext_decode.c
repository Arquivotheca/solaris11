                                       ...
                                       SshBerTime     not_after)
{
  ...
  status =
    ssh_asn1_read_node(context, data,
                       ...
                       &which1, not_before, not_before,
                       &which2, not_after, not_after);
      ...
      if (dpn_found)
        {
          ...
          switch (which)
            ...
            case 1:
              ...
              ssh_dn_put_rdn(point->dn_relative_to_issuer, relative_dn);
                                              ...
                                              SshX509ExtUnknown *unknown_ret)
{
  ...
  if ((oiddup = ssh_strdup(oid)) == NULL)
    return SSH_X509_FAILURE;
  if (name)
    if ((namedup = ssh_strdup(name)) == NULL)
      return SSH_X509_FAILURE;
  if ((unknown = ssh_malloc(sizeof(*unknown))) == NULL)
    {
      *unknown_ret = NULL;
      return SSH_X509_FAILURE;
    }
        ...
        case SSH_X509_QCSTATEMENT_QCSYNTAXV1:
          {
            ...
            if (asn1status == SSH_ASN1_STATUS_OK)
            ...
            else
              {
                ...
                names = NULL;
                if (ssh_x509_decode_general_names(context, data,
                                                  &names,
                                                  config)
                    != SSH_X509_OK)
                  goto failed;
  ...
  for (; extensions; extensions = ssh_asn1_node_next(extensions))
    {
      ...
      switch (extension_type)
        ...
        case SSH_X509_EXT_CRL_DIST_POINTS:
          /* XXX Temporary solution!  This is due to recent change in
             the library to separate the extensions from the
             certificates. The current approach is more natural for
             the X.509v3 but not as natural for the user.
             The old approach combined transparently the issuer name
             with the end rdn... Currently this is not done.  */

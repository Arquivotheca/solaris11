...
#if defined(K_SOLARIS_PLATFORM) || defined(K_LINUX_PLATFORM)
static const char
*getnodeaddrstr( char *hostname, char *outbuf, size_t buflen )
{
	struct hostent *hptr;
	char **p;
	const char *res;
	...
	char sApplianceAddress[KMS_MAX_NETWORK_ADDRESS + 1];

	/*
	 * check if [] IPv6 url convention is being used and
	 * strip the brackets if so
	 */
	if (hostname[0] == '[') {
	...
	/* don't modify passed-in string */
	strncpy(sApplianceAddress, pStart, sizeof (sApplianceAddress));
	sApplianceAddress[sizeof (sApplianceAddress) - 1] = '\0';

       	pEnd = strchr(sApplianceAddress, ']');
	if ( pEnd != NULL ) {
		*pEnd = '\0';
	} else if (pStart != hostname) {
		/* invalid host specification */
		return (NULL);
	}

#ifdef K_SOLARIS_PLATFORM
	hptr = getipnodebyname(hostname, AF_INET6, 0, &err);
	if (hptr == NULL)
		hptr = getipnodebyname(hostname, AF_INET, 0, &err);
#endif
#ifdef K_LINUX_PLATFORM
	/* try IPv4 first */
	hptr = gethostbyname2(sApplianceAddress, AF_INET);
	if (hptr == NULL) {
		/* next, IPv6 */
		hptr = gethostbyname2(sApplianceAddress, AF_INET6);
	}
#endif
	if (hptr == NULL)
		return (NULL);
	...
	p = hptr->h_addr_list;
	if (hptr->h_addrtype == AF_INET6) {
#ifndef K_LINUX_PLATFORM
		if (IN6_IS_ADDR_V4MAPPED((struct in6_addr *)*p)) {
			IN6_V4MAPPED_TO_INADDR((struct in6_addr *)*p, &in4);
			af = AF_INET;
			addr = &in4;
		} else {
#endif
	...
	pStart = outbuf;
	sLen = buflen - 1; /* leave room for the trailing null */

	// surround IPv6 addresses with [] for use later on when crafting a URL
	if (af == AF_INET6) {
		*pStart = '[';
		pStart++;
		sLen--;
	...
	res = inet_ntop(af, addr, pStart, sLen);
#ifdef K_SOLARIS_PLATFORM
	freehostent(hptr);
#endif
                                       ...
                                       int i_eKMSmode)
{
    ...
    if ((i_pPassphrase == NULL) || ( i_pPassphrase[0] == '\0')) {
	nullPassphrase = true;
    } else {
	nullPassphrase = false;
    }

    if ((i_pAgentID && nullPassphrase) || (!nullPassphrase && !i_pAgentID))
    {
        Log(AUDIT_CLIENT_AGENT_LOAD_PROFILE_INVALID_PARAMETERS,
                NULL,
                NULL,
...
#if defined(K_SOLARIS_PLATFORM) || defined(K_LINUX_PLATFORM)
	/*
	 * Get IP address for KMA.  IPv6 addresses are returned in URL format
	 * per RFC 2732
	 */
	const char *hoststr = getnodeaddrstr(i_pInitialApplianceAddress,
	    sInitialApplianceAddress, sizeof(sInitialApplianceAddress));
	if (hoststr == NULL) {
		char errmsg[2048];
		(void) snprintf(errmsg, sizeof (errmsg),
		    "Cannot find address for %s",
		    i_pInitialApplianceAddress);

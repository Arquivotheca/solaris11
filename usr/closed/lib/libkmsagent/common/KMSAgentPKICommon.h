...
public:
...
#ifdef KMSUSERPKCS12
	bool LoadPKCS12CertAndKey(FILE *fp,
		int i_iFormat,
	...
	/**
	 * copies certificate to passed in pointer
	 * @param p_oCert
	 * @return bool
	 */
	bool CopyCert( void **p_oCert );

	bool CopyCertAndKey(
		CPrivateKey *i_pPrivateKey, void **o_pCert, void **o_pKey);

	/**
	 * destroys memory associated with passed in SSL type
	 */
	void
	DestroyCert(void *p_iCert);

	void
	DestroyCertAndKey(void *p_iCert, void *p_iKey);
#endif
...
class CPKI
{
  ...
  public:
       ...
       *   @param i_pcFile  open FILE* to file to store the cert and private key

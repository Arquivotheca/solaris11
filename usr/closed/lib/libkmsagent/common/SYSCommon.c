...
#elif defined K_OLD_LINUX_PLATFORM
#include <unistd.h>   /* it includes usleep(us) */
#include <sys/time.h>
#include <fts.h>
#else
/*
 * Directory traversal code is not yet available for Solaris or modern Linux.
...
int K_CreateMutex( K_MUTEX_HANDLE *o_phandle )
{
...
#else
    {
        ...
        if(bIsRecursive)
        {
            ...
            iType = PTHREAD_MUTEX_RECURSIVE_NP;
#else
	    iType = PTHREAD_MUTEX_RECURSIVE;
#endif
...
#if defined(WIN32) || defined(K_OLD_LINUX_PLATFORM)
    return 0;
#endif
}

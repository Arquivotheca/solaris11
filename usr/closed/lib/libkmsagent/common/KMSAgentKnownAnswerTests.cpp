...
#define AES_MAXKEYBYTES 32
#define	AES_MAXKEYBITS (AES_MAXKEYBYTES * 8)
                                  ...
                                  const unsigned char * const i_pKey)
{
    ...
    EVP_CIPHER_CTX_set_key_length(&ctx, AES_MAXKEYBITS);
    EVP_CipherInit_ex(&ctx, NULL, NULL, i_pKey, NULL, 1);
    out_len = sizeof (acCypherText);
    EVP_CipherUpdate(&ctx, acCypherText, &out_len, acPlainText, sizeof (acPlainText));
    total_len = out_len;
    out_len = sizeof (acCypherText) - total_len;
    EVP_CipherFinal_ex(&ctx, acCypherText+out_len, &out_len);
    ...
    EVP_CIPHER_CTX_set_key_length(&ctx, AES_MAXKEYBITS);
    EVP_CipherInit_ex(&ctx, NULL, NULL, i_pKey, NULL, 0);
    out_len = sizeof (acDecryptedCypherText);
    in_len = total_len;
    EVP_CipherUpdate(&ctx, acDecryptedCypherText, &out_len, acCypherText, in_len);
    total_len = out_len;
    out_len = in_len - total_len;
    EVP_CipherFinal_ex(&ctx, acDecryptedCypherText+out_len, &out_len);
    EVP_CIPHER_CTX_cleanup(&ctx);
    total_len += out_len;
#else

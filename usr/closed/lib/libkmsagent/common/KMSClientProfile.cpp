   ...
   char* const o_sHexHashedPassphrase )
{
   ...
   do
   {
	...
	if (bSuccess) {
      ...
      if ( bSuccess ) {
	      bSuccess = pRootCACertificate->CopyCert(&i_pProfile->ca_cert);
      }
      ...
      if ( bSuccess )
      {
         ...
         bSuccess =
            K_soap_ssl_client_context( 
               i_pProfile,			// in ->m_wsProfileName
	       NULL,				// password, not needed for server auth
...
#ifdef KMSUSERPKCS12
      if ( bSuccess ) {
	      bSuccess = pEntityCertificate->CopyCertAndKey(
		  pEntityPrivateKey, &i_pProfile->p12_cert, &i_pProfile->p12_key);
      }
#endif
                ...
                int       i_eKMSmode)
{
	...
	if (!bSuccess) {
		...
		bSuccess = pEntityCertificate->CopyCertAndKey(
		    pEntityPrivateKey, &io_pProfile->p12_cert, &io_pProfile->p12_key);
    ...
    if ( bSuccess && !bEnrolling && !io_pProfile->m_iEnrolled)
    {
        bSuccess = false;
        Log(AUDIT_CLIENT_LOAD_PROFILE,
          i_wsProfileName,
    ...
    if ( bSuccess && bClusterInformationFound )
    {
       ...
       // discovery KMA.
	    ...
	    strncpy(io_pProfile->m_wsEntitySiteID, pstTempProfile->m_wsEntitySiteID,
		   KMS_MAX_ENTITY_SITE_ID - 1);
    ...
    char sHexPassphrase[2*KMS_MAX_HASH_SIZE + 1];
    bzero(sHexPassphrase, sizeof (sHexPassphrase));
       ...
       // and keypair)
        ...
        if ( bSuccess )
        {
            bSuccess = 
                K_soap_ssl_client_context( 
                   io_pProfile,			// in/out
		   sHexPassphrase,		// passphrase needed for enrollment
                   pstSoap,			// out
                       ...
                       KMSClusterEntry                                   const *i_pRight)
{
    ...
    isTrue = i_pRight->m_iEnabled ? true : false;
    if ( i_pLeft->Enabled != isTrue )
    {
        return false;
    }
    ...
    isTrue = i_pRight->m_iResponding ? true : false;
    if ( i_pLeft->Responding != isTrue )
    {
	return false;
    }

    if ( strncmp(i_pLeft->KMAVersion, i_pRight->m_sKMAVersion,
	KMS_MAX_VERSION_LENGTH) != 0 )
    {
	return false;
    }
    ...
    isTrue = i_pRight->m_iKMALocked ? true : false;
    if ( i_pLeft->KMS_Discovery__Locked != isTrue )
    {
	return false;
    }
        ...
        int i_iClusterEntryArraySize)
{
   ...
   if (pstSoap)
   {
      ...
      {
          soap_free(pstSoap);
	  i_pProfile->m_pvDiscoverySoap = NULL;
      }
      else
      {
...
#ifdef KMSUSERPKCS12
	CCertificate* tempCert = new CCertificate;

	tempCert->DestroyCert(i_pProfile->ca_cert);
	tempCert->DestroyCertAndKey(i_pProfile->p12_cert,
		i_pProfile->p12_key);

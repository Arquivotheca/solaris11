...
#define	SCF_FUNC_NAME		"scf_meta_ioctl() "
	...
	case FIOMPDEVINFO:
		...
		if (u_mode == DDI_MODEL_ILP32) {
			...
			(void) strcpy(fiomp_devinfo32_p->real_name,
			    SCF_REAL_NAME);
			(void) strcpy(fiomp_devinfo32_p->user_path,
			    SCF_USER_PATH);
 ...
 */
	END_DEVINFO32:
			...
			mutex_exit(&scf_comtbl.attach_mutex);
			/* Set output information */
			(void) strcpy(fiomp_devinfo_p->real_name,
			    SCF_REAL_NAME);
			(void) strcpy(fiomp_devinfo_p->user_path,
			    SCF_USER_PATH);
 ...
 */
	...
	if (fiomp_all_devinfo32.num != 1) {
		...
		(void) strcpy(fiomp_devinfo32_p->real_name, SCF_REAL_NAME);
		(void) strcpy(fiomp_devinfo32_p->user_path, SCF_USER_PATH);
		fiomp_devinfo32_p->path_num = path_num;
		fiomp_devinfo32_p->mpmode = FIOMP_FALSE;
		fiomp_devinfo32_p->autopath = FIOMP_TRUE;
		fiomp_devinfo32_p->block = FIOMP_TRUE;
 ...
 */
	...
	if (fiomp_all_devinfo.num != 1) {
		...
		(void) strcpy(fiomp_devinfo_p->real_name, SCF_REAL_NAME);
		(void) strcpy(fiomp_devinfo_p->user_path, SCF_USER_PATH);
		fiomp_devinfo_p->path_num = path_num;
		fiomp_devinfo_p->mpmode = FIOMP_FALSE;
		fiomp_devinfo_p->autopath = FIOMP_TRUE;
		fiomp_devinfo_p->block = FIOMP_TRUE;
...
#define	SCF_FUNC_NAME		"scf_inst_ioctl() "
	...
	if (fiompprop32.num != scf_comtbl.path_num) {
		...
		/* Set output information */
		(void) strcpy(iomp_name_p, SCF_IOMP_NAME);
		(void) strcpy(iomp_real_name_p, SCF_REAL_NAME);
		(void) strcpy(iomp_user_path_p, SCF_USER_PATH);
		if ((scf_comtbl.scf_path_p) || (scf_comtbl.scf_exec_p)) {
			(void) strcpy(iomp_status_p, "online");
		} else if ((scf_comtbl.scf_stop_p) ||
			(scf_comtbl.scf_err_p)) {
			(void) strcpy(iomp_status_p, "offline");
		} else {
			(void) strcpy(iomp_status_p, "unconfigured");
		}
		for (ii = 0, jj = 0; ii < fiompprop32.num;
			...
			if ((statep = scf_comtbl.iomp_scf[ii]) != 0) {
				if (ddi_pathname(statep->dip,
					work_name_p)) {
					(void) sprintf(&iomp_path[jj],
					    "%s:scfc%d",
					    work_name_p, statep->instance);
				}
				(void) sprintf(&iomp_logical_path[jj],
				    "/dev/FJSVhwr/scfc%d",
				    statep->instance);
				switch (statep->path_status) {
				case FIOMP_STAT_ACTIVE:
					(void) strcpy(&iomp_path_status[jj],
					    "active");
					break;
				...
				(void) strcpy(&iomp_path_block[jj], "block");
			}
		}
	}
	mutex_exit(&scf_comtbl.all_mutex);
...
#define	SCF_FUNC_NAME		"scf_path_stmsg() "
	...
	if (statep->scf_herr & HERR_TESUM) {
		(void) strcpy(message_p, "Command error");
	} else if (statep->scf_herr & HERR_RESUM) {
		(void) strcpy(message_p, "Sumcheck error");
	} else if (statep->scf_herr & HERR_CMD_RTO) {
		(void) strcpy(message_p, "Command timeout");
	} else if (statep->scf_herr & HERR_BUSY_RTO) {
		(void) strcpy(message_p, "Command busy timeout");
	} else if (statep->scf_herr & HERR_DSCP_INTERFACE) {
		(void) strcpy(message_p, "SCF communication path error");
	} else if (statep->scf_herr & HERR_DSCP_ACKTO) {
		(void) strcpy(message_p, "DSCP ack response timeout");
	} else if (statep->scf_herr & HERR_DSCP_ENDTO) {
		(void) strcpy(message_p, "DSCP end response timeout");
	} else {
		(void) strcpy(message_p, "Good");
	}

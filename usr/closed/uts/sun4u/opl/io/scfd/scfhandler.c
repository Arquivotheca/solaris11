...
#define	SCF_FUNC_NAME		"scf_intr() "
	...
	/* Check if interrupt handler has been removed */
...
#define	SCF_FUNC_NAME		"scf_intr_cmdcmp() "
	...
	case NORMAL_END:
		...
		if (sum != *wk_in_p8) {
			...
			(void) strcpy(&sumerr_msg[0], "SCF device");
			statep->resum_rcnt++;
...
#define	SCF_FUNC_NAME		"scf_intr_cmdcmp_driver() "
	...
	case CMD_SCFI_PATH:		/* SCF Path change command */
		...
		if (scf_comtbl.scf_path_p != NULL) {
			...
			/* FMEMA interface */
			scf_avail_cmd_reg_paddr =
			    va_to_pa(&statep->scf_regs->COMMAND);
...
#define	SCF_FUNC_NAME		"scf_panic_callb() "
	...
	if (path_flag == 0) {
		for (ii = 0; ii < scf_cmdend_wait_rcnt_panic; ii++) {
			/* wait */
			drv_usecwait(SCF_MIL2MICRO(
			    scf_cmdend_wait_time_panic));
...
START_scf_panic_loop:
		...
		/* Check SCF offline status */

		status_exr = SCF_P_DDI_GET32(statep->scf_regs_handle,
		    &statep->scf_regs->STATUS_ExR);
		if ((status_exr & STATUS_SCF_STATUS) != STATUS_SCF_ONLINE) {
			for (ii = 0; ii < SCF_CALLB_ONLINE_RCNT; ii++) {
				drv_usecwait(SCF_MIL2MICRO(
				    SCF_CALLB_ONLINE_TIME));
				status_exr = SCF_P_DDI_GET32(
				    statep->scf_regs_handle,
				    &statep->scf_regs->STATUS_ExR);
				if ((status_exr & STATUS_SCF_STATUS) ==
				    STATUS_SCF_ONLINE) {
					break;
				}
			}

			if (ii == SCF_CALLB_ONLINE_RCNT) {
				SCF_PANIC_TRACE(__LINE__);
				goto END_scf_panic_callb;
			}
		}
...
#define	SCF_FUNC_NAME		"scf_shutdown_callb() "
	...
	struct scf_cmd		scf_cmd;
	scf_short_buffer_t	sbuf;
	int			buf_ful_cnt = SCF_CALLB_BUFF_FULL_RCNT;
	int			rci_busy_cnt = SCF_CALLB_RCI_BUSY_RCNT;
	int			ost;
	int			path_flag = 0;
	int			cmd_retry_flag = 0;
	int			ii;
	...
	if (statep == NULL) {
		...
		if (statep == NULL) {
			...
			if (statep == NULL) {
				...
				if (statep == NULL) {
					SC_DBG_DRV_TRACE(TC_SHUTDOWN|TC_ERR,
					    __LINE__, "shutdown", 8);
					goto END_scf_shutdown_callb;
				}
			}
	...
	if (scf_comtbl.scf_cmd_exec_flag) {
		/* wait */
		if (path_flag == 0) {
			(void) scf_shutdown_cmdend_check(statep);
		}
	}

	/* All device interrupt disable */
	if ((wkstatep = scf_comtbl.scf_exec_p) == NULL) {
		wkstatep = scf_comtbl.scf_path_p;
	}
	scf_comtbl.scf_exec_p = 0;
	scf_comtbl.scf_path_p = 0;
	if (wkstatep) {
		/* Exec device interrupt disable */
		scf_forbid_intr(wkstatep);
		scf_chg_scf(wkstatep, PATH_STAT_STOP);
	}
	...
	/*
	 * START_shutdown_path_send
	 */
START_shutdown_path_send:

	/* Check SCF path change send */
	if (path_flag == 0) {
		goto START_shutdown_report_send;
	}

	/* Check SCF offline status */
	if (scf_offline_check(statep, FLAG_OFF) != SCF_PATH_ONLINE) {
		for (ii = 0; ii < SCF_CALLB_ONLINE_RCNT; ii++) {
			drv_usecwait(SCF_MIL2MICRO(SCF_CALLB_ONLINE_TIME));
			if (scf_offline_check(statep, FLAG_OFF) ==
			    SCF_PATH_ONLINE) {
				break;
			}
		}
	...
	scf_cmd.subcmd = SUB_CMD_PATH;
	bzero((void *)&sbuf.b[0], SCF_S_CNT_16);
	sbuf.b[0] = CMD_PATH_TYPE_SCFD;
	scf_cmd.scount = SCF_S_CNT_15;
	scf_cmd.sbuf = &sbuf.b[0];
	scf_cmd.rcount = 0;
	scf_cmd.flag = SCF_USE_S_BUF;
	scf_i_send_cmd(&scf_cmd, statep);

	/* Check command complete */
	ret = scf_shutdown_cmdend_check(statep);
	...
	if (ret != 0) {
		...
		if (cmd_retry_flag == 0) {
			cmd_retry_flag = 1;
			scf_comtbl.scf_cmd_resend_flag = 1;
			goto START_shutdown_path_send;
		}
		goto END_scf_shutdown_callb;
	}
	switch ((statep->reg_status & STATUS_CMD_RTN_CODE) >> 4) {
	case NORMAL_END:
		/* command success */
		cmd_retry_flag = 0;
		break;
...
START_shutdown_report_send:
	...
	case BUF_FUL:
		...
		if (buf_ful_cnt > 0) {
			buf_ful_cnt--;
			drv_usecwait(SCF_MIL2MICRO(SCF_CALLB_BUFF_FULL_RTIME));
			goto START_shutdown_report_send;
		}
		break;
	...
	case RCI_BUSY:
		...
		if (rci_busy_cnt > 0) {
			rci_busy_cnt--;
			drv_usecwait(SCF_MIL2MICRO(SCF_CALLB_RCI_BUSY_RTIME));
			goto START_shutdown_report_send;
		}
		break;
...
/*
 * scf_shutdown_cmdend_check()
 *
 * Description: SCF command complete processing. (for scf_shutdown_callb)
 *
 */
int
scf_shutdown_cmdend_check(struct scf_state *statep)
{
#undef  SCF_FUNC_NAME
#define	SCF_FUNC_NAME	"scf_shutdown_cmdend_check() "
	int		ii;
	...
	/* Check command complete */
	for (ii = 0; ii < SCF_CALLB_CMDEND_RCNT; ii++) {
		/* wait */
		drv_usecwait(SCF_MIL2MICRO(SCF_CALLB_CMDEND_TIME));

		/* Get SCF status register */
		statep->reg_status = SCF_DDI_GET16(statep,
		    statep->scf_regs_handle, &statep->scf_regs->STATUS);
		SC_DBG_DRV_TRACE(TC_R_STATUS, __LINE__,
		    &statep->reg_status, sizeof (statep->reg_status));

		if (statep->reg_status & STATUS_CMD_COMPLETE) {
			/* Command complete */
			break;
		}
	}
	...
	SCFDBGMSG1(SCF_DBGFLAG_FOCK, SCF_FUNC_NAME ": end return = %d", ii);
	return ((ii != SCF_CALLB_CMDEND_RCNT) ? 0 : (-1));
}

...
static kmutex_t mpxu_pcf8584_mutex;
...
{
	mutex_init(&mpxu_pcf8584_mutex, NULL, NULL, NULL);
}
...
set_platform_defaults(void)
{
	...
	/*
	 * Enchilada server product must support CPR even though it has
	 * no energystar-v* property. When this variable is set to non-zero,
	 * CPR will proceed even in the absence of the energystar property.
	 */
	cpr_platform_enable = 1;
...
plat_add_mem_unum_label(char *unum, int mcid, int bank, int dimm)
{
	...
	printed = snprintf(unum, buflen,
	    (strcmp(platform, "SUNW,Sun-Fire-V440") ?
	    "MB/P%d/B%d" : "C%d/P0/B%d"), mcid, bank);
	buflen -= printed;
	unum += printed;
...
{
	/*
	 * This routine gets called early in the boot process
	 * before the platform variable has been initialized.
	 */
	if (platform[0] == '\0')
		(void) BOP_GETPROP(bootops, "impl-arch-name", platform);

	if (snprintf(buf, buflen,
	    (strcmp(platform, "SUNW,Sun-Fire-V440") ?  "MB" : "C%d"),
	    cpuid) >= buflen) {
		return (ENOSPC);
	} else {
		*lenp = strlen(buf);
...
/*
 * Enchilada's BBC pcf8584 controller is used by both OBP and the OS's i2c
...
{
	mutex_enter(&mpxu_pcf8584_mutex);
}

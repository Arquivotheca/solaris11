...
/*
 * N2CP Crypto DR Module Domain Service message handling
 */
 ...
 */
static ds_ver_t		dr_cwq_vers[] = { { 1, 0 } };
#define	DR_CWQ_NVERS	(sizeof (dr_cwq_vers) / sizeof (dr_cwq_vers[0]))

/*
 * DS Capability Description
 */
static ds_capability_t n2cp_dr_cap = {
	DR_CRYPTO_CWQ_ID,	/* svc_id */
	dr_cwq_vers,		/* vers */
	DR_CWQ_NVERS		/* nvers */
};
 ...
 * DS Callbacks
 */
static void n2cp_dr_reg_handler(ds_cb_arg_t, ds_ver_t *, ds_svc_hdl_t);
static void n2cp_dr_unreg_handler(ds_cb_arg_t arg);
static void n2cp_dr_data_handler(ds_cb_arg_t arg, void *buf, size_t buflen);

/*
 * DS Client Ops Vector
 */
static ds_clnt_ops_t n2cp_dr_ops = {
	n2cp_dr_reg_handler,	/* ds_reg_cb */
	n2cp_dr_unreg_handler,	/* ds_unreg_cb */
	n2cp_dr_data_handler,	/* ds_data_cb */
	NULL			/* cb_arg */
};
...
static void n2cp_dr_crypto_config(n2cp_t *, dr_crypto_hdr_t *,
	dr_crypto_hdr_t **, int *);
static void n2cp_dr_crypto_unconfig(n2cp_t *, dr_crypto_hdr_t *,
	dr_crypto_hdr_t **, int *);
static void n2cp_dr_crypto_status(n2cp_t *, dr_crypto_hdr_t *,
	dr_crypto_hdr_t **, int *);
static size_t n2cp_dr_pack_response(dr_crypto_hdr_t *, dr_crypto_res_t *,
	dr_crypto_hdr_t **);


/*
 * register the n2cp crypto domain services capabilities.
 */
int
n2cp_dr_init(n2cp_t *n2cp)
{
	int	rv;

	n2cp_dr_ops.cb_arg = n2cp;

	if ((rv = ds_cap_init(&n2cp_dr_cap, &n2cp_dr_ops)) != 0) {
...
/*
 * unregister the n2cp crypto domain services capabilities.
 */
int
n2cp_dr_fini(void)
{
	...
	if ((rv = ds_cap_fini(&n2cp_dr_cap)) != 0) {
...
static void
n2cp_dr_reg_handler(ds_cb_arg_t arg, ds_ver_t *ver, ds_svc_hdl_t hdl)
{
	n2cp_t	*n2cp = arg;
	...
	n2cp->n_ds_handle = hdl;
}

static void
n2cp_dr_unreg_handler(ds_cb_arg_t arg)
{
	n2cp_t	*n2cp = arg;

	DBG1(n2cp, DDR, "unreg_handler: arg=0x%p\n", arg);

	n2cp->n_ds_handle = DS_INVALID_HDL;
}
 ...
 */
static void
n2cp_dr_data_handler(ds_cb_arg_t arg, void *buf, size_t buflen)
{
	n2cp_t		*n2cp = arg;
	...
	DBG1(n2cp, DDR, "incoming request: %d\n", req->msg_type);

	if (req->num_records > N2_DR_MAX_CRYPTO_UNITS) {
		DBG2(n2cp, DWARN,
		    "Crypto list too long: %d when %d is the maximum\n",
		    req->num_records, N2_DR_MAX_CRYPTO_UNITS);
		goto done;
	}

	if (req->num_records == 0) {
		DBG0(n2cp, DWARN, "No crypto unit specified for operation\n");
		goto done;
	}
	...
	case DR_CRYPTO_CONFIG:
		n2cp_dr_crypto_config(n2cp, req, &resp, &resp_len);
		break;
	case DR_CRYPTO_UNCONFIG:
	case DR_CRYPTO_FORCE_UNCONFIG:
		n2cp_dr_crypto_unconfig(n2cp, req, &resp, &resp_len);
		break;
	case DR_CRYPTO_STATUS:
		n2cp_dr_crypto_status(n2cp, req, &resp, &resp_len);
		break;
	default:
	...
	DBG2(n2cp, DDR, "outgoing response: %d, len %d\n",
	    resp->msg_type, resp_len);

	/* send back the response */
	if (ds_cap_send(n2cp->n_ds_handle, resp, resp_len) != 0) {
		DBG0(n2cp, DWARN, "ds_send failed\n");
	}
 ...
 */
static void
n2cp_dr_crypto_config(n2cp_t *n2cp, dr_crypto_hdr_t *req,
	dr_crypto_hdr_t **resp, int *resp_len)
{
	...
	for (idx = 0; idx < count; idx++) {
		...
		n2cp_cwq_config(n2cp, &res[idx]);
	}

	*resp_len =  n2cp_dr_pack_response(req, res, resp);
}
 ...
 */
static void
n2cp_dr_crypto_unconfig(n2cp_t *n2cp, dr_crypto_hdr_t *req,
	dr_crypto_hdr_t **resp, int *resp_len)
{
	int		idx;
	int		count;
	...
	for (idx = 0; idx < count; idx++) {
		res[idx].cpuid = req_cpus[idx];
		n2cp_cwq_unconfig(n2cp, &res[idx]);
	}
 ...
 */
static size_t
n2cp_dr_pack_response(dr_crypto_hdr_t *req, dr_crypto_res_t *res,
...
/*
 * Crypto unit status handler.
 */
static void
n2cp_dr_crypto_status(n2cp_t *n2cp, dr_crypto_hdr_t *req,
	dr_crypto_hdr_t **resp,
	int *resp_len)
{
	int			idx;

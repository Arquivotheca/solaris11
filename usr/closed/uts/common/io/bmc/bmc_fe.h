...
#if !defined(__sparc)
#define	BMC_MAX_RESPONSE_PAYLOAD_SIZE   bmc_kcs_max_response_payload_size
#else
#define	BMC_MAX_RESPONSE_PAYLOAD_SIZE   bmc_vc_max_response_payload_size
#endif

#if !defined(__sparc)
void kcs_initregs(uint32_t base, int regspacing);
int do_kcs2bmc(ipmi_state_t *, bmc_req_t *, bmc_rsp_t *, boolean_t, uint8_t *);
int bmc_kcs_max_response_payload_size(void);
int bmc_kcs_max_request_payload_size(void);
void kcs_reset_bmc(ipmi_dev_t *, int *, boolean_t *);
#else
int do_vc2bmc(ipmi_state_t *, bmc_req_t *, bmc_rsp_t *, boolean_t, uint8_t *);

 ...
 * CPQARY3_MEM.C
 * Abstract:
 *   This module contains functions for the Driver Memory Management.
 */
...
cpqary3_meminit(cpqary3_t *cpqary3p)
{
	...
	cpqary3p->cmdmemlistp = memlistp = (cpqary3_cmdmemlist_t *)
...
#ifdef AMD64_DEBUG
	cmn_err(CE_WARN, "CPQary3: no_cmds = %d\n", no_cmds);
	cmn_err(CE_WARN, "CPQary3: mem_size = %d\n", mem_size);
	cmn_err(CE_WARN, "CPQary3: sizeof cmdpvt = %d\n",
	    sizeof (cpqary3_cmdpvt_t));
	cmn_err(CE_WARN, "CPQary3: head_pvtp = %lX\n", head_pvtp);
	cmn_err(CE_WARN, "CPQary3: tail_pvtp = %lX\n", tail_pvtp);
	...
	for (i = 0; i < no_cmds; i++) {
		...
		ptr->next		= (cpqary3_cmdpvt_t *)
		    ((i == (no_cmds - 1)) ?  NULL : &head_pvtp[i+1]);
		ptr->prev		= (cpqary3_cmdpvt_t *)
		    ((i == 0) ? NULL : &head_pvtp[i-1]);
		ptr->ctlr		= cpqary3p;
		ptr->pvt_pkt 		= (cpqary3_pkt_t *)NULL;
		ptr->sprev		= (cpqary3_cmdpvt_t *)NULL;
		ptr->snext		= (cpqary3_cmdpvt_t *)NULL;
		ptr->cpqary3_phyctgp	= (cpqary3_phyctg_t *)NULL;
	}
	...
	/* Maximum commands for controller */
	cpqary3p->cmdmemlistp->max_memcnt = 0;

	ptr = head_pvtp;

	DBG("CPQary3 : _meminit : virtual memory list init done \n");
...
#ifdef AMD64_DEBUG
	cmn_err(CE_WARN, "CPQary3: sizeof cmdlist = %ld\n", size_of_cmdlist);
	cmn_err(CE_WARN, "CPQary3: sizeof HRE = %ld\n", size_of_HRE);
#endif
	 ...
	 * 2 Command Blocks from every allocation even if we add any extra
	 * bytes after the initial allocation to make it 32 bit aligned.
	 */
	 ...
	 * The driver shall continue its initialization and working
	 * with whatever memory has been allocated.
	 *
	 * Free the latest virtual memory allocated.
	 * NULLify the last node created to maintain the memory block list.
	 ...
	 */
		 ...
		 */
		maxmemcnt = 0;
		maxmemcnt = ((uint32_t)(cpqary3_phyctgp->real_size -
		    (uint32_t)unused_mem)) / size_of_cmdlist;
	 ...
	 * Physical Address, Virtual address and the Physical Addres of the
	 * Error Info Descriptor
	 */
		...
		for (cntr = 0; cntr < maxmemcnt; cntr++) {
			...
			ptr->cpqary3_phyctgp = cpqary3_phyctgp;
...
cpqary3_cmdlist_occupy(cpqary3_t *ctlr)
{
	...
	DBG1("CPQary3 : _cmdlist_occupy : Tag value = %d \n",
	    memp->tag.tag_value);
	DBG1("CPQary3 : _cmdlist_occupy : Physical Addr = %x \n",
	    memp->cmdlist_phyaddr);
...
#ifdef AMD64_DEBUG
	cmn_err(CE_WARN, "CPQary3 : _cmdlist_occupy : Tag value = %ld \n",
	    memp->tag.tag_value);
	cmn_err(CE_WARN, "CPQary3 : _cmdlist_occupy : Physical Addr = %lX \n",
	    memp->cmdlist_phyaddr);
	cmn_err(CE_WARN, "CPQary3 : _cmdlist_occupy : cmdlist memAddr = %lX \n",
	    memp->cmdlist_memaddr);
	cmn_err(CE_WARN, "CPQary3 : _cmdlist_occupy : cmdlist size = %ld \n",
	    sizeof (CommandList_t));
	cmn_err(CE_WARN, "CPQary3 : _cmdlist_occupy : Err Addr  = %lX \n",
	    (memp->cmdlist_memaddr->ErrDesc.Addr));
	cmn_err(CE_WARN, "CPQary3 : _cmdlist_occupy : Err Len  = %ld \n",
	    (memp->cmdlist_memaddr->ErrDesc.Len));
...
cpqary3_cmdlist_release(cpqary3_cmdpvt_t *memp, uint8_t flag)
{
	...
	if (!memp) {
		DBG("CPQary3: _release: ERROR Command Memory is Null\n");
		return;
	}
...
}	/*** End of cpqary3_cmdlist_release() ***/
...
cpqary3_memfini(cpqary3_t *ctlr, uint8_t level)
{
	...
	blk_ptr = (cpqary3_phys_hdl_addr_t *)
	    ctlr->cmdmemlistp->cpqary3_phyctgp;

	/* CmdsOutMax = DDI_GET32(ctlr, &ctp->CmdsOutMax); */

	CmdsOutMax = ctlr->ctlr_maxcmds;

	DBG1("CPQary3: CmdsOutMax in fini = %d\n", CmdsOutMax);
	/*
	...
	if (level & CPQARY3_PHYCTGS_DONE) {
		if (blk_ptr) {
			while (blk_ptr->next) {
				tptr 	= blk_ptr;
				blk_ptr = blk_ptr->next;
				cpqary3_free_phyctgs_mem(tptr->blk_addr,
...
#ifdef AMD64_DEBUG
		cmn_err(CE_WARN, "CPQary3: mem_size in memfini = %d\n",
		    mem_size);
		cmn_err(CE_WARN, "CPQary3: memp in memfini = %X\n", memp);
		debug_enter(&debug_char);
#endif
 ...
 * If all succeed, update phyaddr to the physical address of the allocated
 * memory and return the pointer to the virtul allocated memory.
 */

	if (DDI_SUCCESS != (retvalue =
	    ddi_dma_alloc_handle((dev_info_t *)ctlr->dip,
		...
		cmn_err(CE_CONT, "CPQary3: Bad attributes in ddi_dma_attr"
		    " cannot allocate the DMA Handle \n");
		break;

		default:
		cmn_err(CE_CONT, "CPQary3: Unexpected Value %x from call to"
	...
	retvalue = ddi_dma_mem_alloc(phyctgp->cpqary3_dmahandle,
	    ...
	    0, &mempool, &real_len, &phyctgp->cpqary3_acchandle);
	...
	retvalue = ddi_dma_addr_bind_handle(phyctgp->cpqary3_dmahandle,
	    NULL, mempool, real_len, DDI_DMA_CONSISTENT | DDI_DMA_RDWR,
	    DDI_DMA_DONTWAIT, 0, &phyctgp->cpqary3_dmacookie, &cookiecnt);
	...
	case DDI_DMA_PARTIAL_MAP:
		cmn_err(CE_CONT, "CPQary3: Allocated the resources for "
		    " part of the object\n");
		break;
	...
	default:
		cmn_err(CE_WARN, "CPQary3 : Unexpected Return Value %x from"
		    " call to bind the DMA Handle\n", retvalue);
	}
...
}	/*** End of cpqary3_alloc_phyctgs_mem() ***/
...
cpqary3_free_phyctgs_mem(cpqary3_phyctg_t *cpqary3_phyctgp, uint8_t cleanstat)
{
	...
	if (cleanstat & CPQARY3_DMA_BIND_ADDR_DONE) {
		(void) ddi_dma_unbind_handle(cpqary3_phyctgp->
		    cpqary3_dmahandle);
	}
...
}	/*** End of cpqary3_free_phyctgs_mem() ***/

 ...
 *   CPQARY3_TRANSPORT.C
 * Abstract:
 *   This module contains all the mandatory and supported SCSA HBA
 *	driver entry points
 *
 */
...
static int	cpqary3_handle_flag_nointr(cpqary3_cmdpvt_t *,
    struct scsi_pkt *);
 ...
 *	Description      : 	This routine initializes the transport vector
 *				in the SCSA architecture for entry ponts in
 *				this driver.
    ...
    scsi_hba_tran_t *hba_tran, struct scsi_device *sd)
{
...
#ifdef CPQARY3_DEBUG
	cmn_err(CE_CONT, "CPQary3: _tgt_init : Entering\n");
#endif
	...
	/*
	 * PROBE PERFORMANCE - Call probe4targetes for
	 * target other than CTLR_SCSI_ID
	 */

	if (tid != CTLR_SCSI_ID) {
		(uint8_t)cpqary3_probe4targets(tid, ctlr);
	}
	/* PROBE PERFORMANCE */
	...
	if ((tid >= CPQARY3_MAX_TGT) || (lun != 0)) {
		DBG2("CPQary3 <%d>: Target Id <%d> Not Supported",
		    ctlr->instance, tid);
		return (DDI_FAILURE);
	}
...
#ifdef CPQARY3_DEBUG
	cmn_err(CE_CONT, "CPQary3 : _tgt_init : Target ID %d exists."
	    " Leaving tgt_init\n", tid);
#endif

	return (DDI_SUCCESS);

}	/* End of cpqary3_tgt_init() */
...
#ifdef CPQARY3_DEBUG
	int status;
	cmn_err(CE_CONT, "CPQary3 : _tgt_probe : Entering \n");
#endif

/*
 * Probe for the presence of the target, using the scsi_hba_probe().
 * It inturn issues the SCSI inquiry command that is serviced by our driver
 */

	/* HPQacucli Changes */
	struct scsi_hba_tran *hba_tran = sd->sd_address.a_hba_tran;
	cpqary3_t	*ctlr = (cpqary3_t *)hba_tran->tran_hba_private;
 ...
 *			   callback function, arguement for the callback
 *			   function
    ...
    int (*callback)(), caddr_t arg)
{
	...
	if (!scsi_pktp) {
		...
		if (!scsi_pktp) {
			DBG("CPQary3: InitPkt: pkt_alloc Failed\n");
			return (NULL);
		}
	...
	/* Store the CDB length and sense data length in the pkt private */
	 ...
	 * packet If first time allocation for this SCSI packet, allocate
	 * fresh DMA Else, move the already allocated DMA resources
	 */
	if (bp && bp->b_bcount != 0) {	/* I/O is desired */
		/* First time allocation */
		if (!cpqary3_pktp->cmd_dmahandle) {
			if (cpqary3_dma_alloc(cpqary3p, scsi_pktp, bp, flags,
			    callback) == CPQARY3_FAILURE) {
				if (new_scsi_pktp)
			...
			if (CPQARY3_FAILURE == cpqary3_dma_move(scsi_pktp, bp,
			    cpqary3p))
				return ((struct scsi_pkt *)NULL);
		}
	}
...
}	/* End of cpqary3_init_pkt() */


/*
 *	Function         :	cpqary3_dma_alloc()
 *	Description      : 	This routine services requests for memory
 *				(dynamic) as and when required by the OS.
 *	Called By        : 	cpqary3_init_pkt()
 *	Parameters       : 	per-controller, SCSI packet, buffer,
 *				flag modifier, callback function
 *	Calls            : 	None
    ...
    struct buf *bp, int flags, int (* callback)())
{
	...
	if (DDI_SUCCESS != (retvalue = ddi_dma_alloc_handle(cpqary3p->dip,
		...
		case DDI_DMA_BADATTR:
			/*
			 * The attributes stated in our DMA attribute structure
			 * is such that potential DMA resources can not
			 * be allocated.
			 */
		...
		default:
			 ...
			 */
			DBG1("CPQary3: dma_alloc: Unexpected Return Value %x"
	...
	case DDI_DMA_MAPPED:
		if (DDI_DMA_PARTIAL_MAP == retvalue) {
			...
			if (ddi_dma_getwin(cpqary3_pktp->cmd_dmahandle,
				...
				cmn_err(CE_PANIC, "CPQary3: Activation of"
				    " New DMA Window Failed \n");
		} else {
		...
		for (;;) {
			...
			if (i == cpqary3p->sg_cnt || i ==
			    cpqary3_pktp->cmd_ncookies)
				break;
			/* SG */
		...
		scsi_pktp->pkt_resid = bp->b_bcount -
		    cpqary3_pktp->cmd_dmacount;
	...
	default :
		cmn_err(CE_PANIC, "CPQary3: Unexpected ERROR "
		    "returned from Call to Bind Buffer to Handle : 0x%X \n", i);
	}
...
}	/* End of cpqary3_dma_move() */


/*
 *	Function         :	cpqary3_transport()
 *	Description      : 	This routine services requests from the OS that
 *				are directed towards the
 *				targets.(any SCSI command)
 *	Called By        : 	kernel
...
cpqary3_transport(struct scsi_address *sa, struct scsi_pkt *scsi_pktp)
{
	...
	if (tgtp->type == CPQARY3_TARGET_NONE) {
		DBG1("CPQary3: Transport: Target %d removed\n", sa->a_target);
		return (TRAN_FATAL_ERROR);
	}

	DBG1("CPQary3: _transport: SCSI Cmd %x\n", scsi_pktp->pkt_cdbp[0]);
	...
	if (NULL == (memp = cpqary3_cmdlist_occupy(ctlr))) {
		DBG("CPQary3: Transport: Memory for Command Unavailable\n");
		return (TRAN_BUSY);
	}
	...
	switch (cpqary3_build_cmdlist(memp, SA2TGT(sa))) {
		case CPQARY3_SUCCESS :
			if (scsi_pktp->pkt_flags & FLAG_NOINTR) {
				return (cpqary3_handle_flag_nointr(memp,
				    scsi_pktp));
			}
		...
		default: /* Never occurs */
			cmn_err(CE_NOTE, "CPQary3 : Transport :"
			    " Unexpected Error \n");
			return (TRAN_FATAL_ERROR);
	}
...
cpqary3_dmafree(struct scsi_address *sa, struct scsi_pkt *scsi_pktp)
{
	...
	if (cpqary3_pktp->cmd_flags & CFLAG_DMAVALID) {
		if (!cpqary3_pktp->cmd_dmahandle) {
			DBG("CPQary3: dmafree: CHECK Flag set"
			    " but dmahandle NULL\n");
			return;
		}
		cpqary3_pktp->cmd_flags &= ~CFLAG_DMAVALID;
...
}	/* End of cpqary3_dmafree() */


/*
 *	Function         : cpqary3_dma_sync
 *	Description      : This routine synchronizes the CPU's / HBA's view of
 *			   the data associated with the pkt,
 *			   typically by calling ddi_dma_sync().
 *	Called By        : kernel
 *	Parameters       : SCSI address, SCSI packet
 *	Calls            : None
 *	Return Values    : None
...
cpqary3_dma_sync(struct scsi_address *sa, struct scsi_pkt *scsi_pktp)
{
	...
	if (cpqary3_pktp->cmd_flags & CFLAG_DMAVALID) {
		if (cpqary3_pktp->cmd_flags & CFLAG_DMASEND) {
			(void) ddi_dma_sync(cpqary3_pktp->cmd_dmahandle,
			    cpqary3_pktp->cmd_dma_offset, cpqary3_pktp->
			    cmd_dma_len, DDI_DMA_SYNC_FORDEV);
		} else {
			(void) ddi_dma_sync(cpqary3_pktp->cmd_dmahandle,
			    cpqary3_pktp->cmd_dma_offset, cpqary3_pktp->
			    cmd_dma_len, DDI_DMA_SYNC_FORCPU);
		}

	}
	return;

}	/* End of cpqary3_dma_sync() */
...
cpqary3_destroy_pkt(struct scsi_address *sa, struct scsi_pkt *scsi_pktp)
{
	...
	if (cpqary3_pktp->cmd_flags & CFLAG_DMAVALID) {
		/* LINTED */
		if (!cpqary3_pktp->cmd_dmahandle) {
			DBG("CPQary3: _destroy_pkt : CHECK Flag set but"
			    " dmahandle NULL\n");
		} else {
			cpqary3_pktp->cmd_flags &= ~CFLAG_DMAVALID;
...
}	/* End of cpqary3_reset() */


/*
 *	Function         :	cpqary3_abort()
 *	Description      : 	This routine aborts a particular command or
 *				all commands directed towards a target.
 *	Called By        : 	kernel
 *	Parameters       : 	SCSI address, SCSI packet
 *	Calls            : 	None
 * 	Return Values    : 	SUCCESS / FAILURE
...
cpqary3_abort(struct scsi_address *sa, struct scsi_pkt *scsi_pktp)
{
	 ...
	 * In either of the cases, we shall have to wait after the
	 * abort functions are called to return the status.
	 */
...
cpqary3_getcap(struct scsi_address *sa, char *capstr, int tgtonly)
{
	 ...
	 */
	if (tgtonly == NULL) {	/* all targets */
		DBG1("CPQary3: cpqary3_getcap: Index 0x%x\n", index);
		DBG("CPQary3: cpqary3_getcap: Not For All Tgt. Returning \n");
		return (CAP_NOT_DEFINED);
	}

	switch (index) {
		case SCSI_CAP_DMA_MAX :
			DBG("CPQary3: cpqary3_getcap: DMA_MAX\n");
			/* LINTED */
			return (cpqary3_dma_attr.dma_attr_maxxfer);
		case SCSI_CAP_DISCONNECT:
			DBG("CPQary3: cpqary3_getcap: DISCONNECT\n");
			return (tgtp->ctlr_flags & CPQARY3_CAP_DISCON_ENABLED);
		case SCSI_CAP_SYNCHRONOUS:
			DBG("CPQary3: cpqary3_getcap: SYNCRONOUS\n");
			return (tgtp->ctlr_flags & CPQARY3_CAP_SYNC_ENABLED);
		case SCSI_CAP_WIDE_XFER:
			DBG("CPQary3: cpqary3_getcap: WIDE_XFER\n");
			return (tgtp->ctlr_flags &
			    CPQARY3_CAP_WIDE_XFER_ENABLED);
		case SCSI_CAP_ARQ:
			DBG("CPQary3: cpqary3_getcap: ARQ\n");
			return ((tgtp->ctlr_flags &
			    CPQARY3_CAP_ARQ_ENABLED) ? 1 : 0);
		case SCSI_CAP_INITIATOR_ID:
			DBG("CPQary3: cpqary3_getcap: INITIATOR_ID\n");
			return (CTLR_SCSI_ID);
		case SCSI_CAP_UNTAGGED_QING :
			DBG("CPQary3: cpqary3_getcap: UNTAGGED_QING\n");
			return (1);
		case SCSI_CAP_TAGGED_QING :
			DBG("CPQary3: cpqary3_getcap: TAGGED_QING\n");
			return (1);
		case SCSI_CAP_SECTOR_SIZE :
			DBG("CPQary3: cpqary3_getcap: SECTOR_SIZE\n");
			return (cpqary3_dma_attr.dma_attr_granular);
		case SCSI_CAP_TOTAL_SECTORS :
			DBG("CPQary3: cpqary3_getcap: TOTAL_SECTORS\n");
			return (CAP_NOT_DEFINED);
		case SCSI_CAP_GEOMETRY:
			DBG("CPQary3: cpqary3_getcap: GEOMETRY\n");
			return (cpqary3_target_geometry(sa));
		case SCSI_CAP_RESET_NOTIFICATION:
			DBG("CPQary3: cpqary3_getcap: RESET_NOTIFICATION\n");
			return (0);
		default:
			DBG("CPQary3: cpqary3_getcap: Default\n");
			return (CAP_NOT_DEFINED);
	}

}	/* End of cpqary3_getcap() */
...
cpqary3_setcap(struct scsi_address *sa, char *capstr, int value, int tgtonly)
{
	...
	DBG1("CPQary3: cpqary3_setcap: Target No. 0x%x\n", SA2TGT(sa));
	DBG1("CPQary3: cpqary3_setcap: Value 0x%x\n", value);

	/*
	 * If requested Capability is not supported, return -1.
	 */
	if ((index = scsi_hba_lookup_capstr(capstr)) == DDI_FAILURE) {
		DBG("CPQary3: cpqary3_setcap: lookup_capstr fail\n");
		return (retstatus);
	}
	 ...
	 */
	if (!tgtonly) {
		DBG1("CPQary3: cpqary3_setcap: Index 0x%x\n", index);
		DBG("CPQary3: cpqary3_setcap: Fail Not many Tgt at a time\n");
		return (retstatus);
	}

	switch (index) {
	case SCSI_CAP_DMA_MAX:
		DBG("CPQary3: cpqary3_setcap: DMA_MAX\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_DISCONNECT:
		DBG("CPQary3: cpqary3_setcap: DISCONNECT\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_SYNCHRONOUS:
		DBG("CPQary3: cpqary3_setcap: SYNCHRONOUS\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_WIDE_XFER:
		DBG("CPQary3: cpqary3_setcap: SCSI_CAP_WIDE_XFER\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_ARQ:
		return (1);
	case SCSI_CAP_INITIATOR_ID:
		DBG("CPQary3: cpqary3_setcap: INITIATOR_ID\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_UNTAGGED_QING:
		DBG("CPQary3: cpqary3_setcap: UNTAGGED_QING\n");
		return (1);
	case SCSI_CAP_TAGGED_QING:
		DBG("CPQary3: cpqary3_setcap: TAGGED_QING\n");
		return (1);
	case SCSI_CAP_SECTOR_SIZE:
		DBG("CPQary3: cpqary3_setcap: SECTOR_SIZE\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_TOTAL_SECTORS:
		DBG("CPQary3: cpqary3_setcap: TOTAL_SECTOR\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_GEOMETRY:
		DBG("CPQary3: cpqary3_setcap: GEOMATRY\n");
		return (CAP_CHG_NOT_ALLOWED);
	case SCSI_CAP_RESET_NOTIFICATION:
		DBG1("CPQary3: cpqary3_setcap: RESET_NOTIFICATION 0X%x\n",
		    value);
		return (CAP_CHG_NOT_ALLOWED);
	default:
		DBG1("CPQary3: cpqary3_setcap: Default Index 0x%x\n", index);
		return (CAP_NOT_DEFINED);
	}

}	/* End of cpqary3_setcap() */


/*
 *	Function         :	cpqary3_handle_flag_nointr
 *	Description      : 	This routine is called to handle submission
 *				and subsequently poll for the completion of
 *				a command, when its FLAG_NOINTR bit is set.
	...
	while (1) {
		...
		if (ctlr->poll_flag == CPQARY3_TRUE) {
			if (!(ctlr->bddef->bd_flags & SA_BD_SAS)) {
				while ((simple_tag = ddi_get32(ctlr->opq_handle,
				    (uint32_t *)ctlr->opq)) != 0xFFFFFFFF) {
					CmdsOutMax = ctlr->ctlr_maxcmds;
					if ((simple_tag >> CPQARY3_GET_MEM_TAG)
					    >= ((CmdsOutMax/3)*3)) {
...
cpqary3_poll(cpqary3_t *ctlr, uint32_t tag)
{
	...
	/* LINTED */
	while (1) {	/* this function is called with both the locks held */
		if (CPQARY3_SUCCESS != cpqary3_poll_retrieve(ctlr, tag, 1)) {
			ii++;
		if (ii > 120000)
			return (CPQARY3_FAILURE);
		drv_usecwait(500);
...
cpqary3_additional_cmd(struct scsi_pkt *scsi_pktp, cpqary3_t *ctlr) {
	...
	arqstat = (struct scsi_arq_status *)(void *)(scsi_pktp->pkt_scbp);
	...
	switch (scsi_pktp->pkt_cdbp[0]) {
		...
		case 0x04: /* Format Unit */
			cmn_err(CE_NOTE, "The FORMAT UNIT is not supported by"
			    " this device If this option is selected from the"
			    " format utility do not continue further. Please "
			    "refer to cpqary3 driver man pages for details.\n");
...
cpqary3_oscmd_complete(cpqary3_cmdpvt_t	*cpqary3_cmdpvtp)
{
	...
	if (cpqary3_check_dma_handle(dmahandle) != DDI_SUCCESS) {
		ddi_fm_dma_err_clear(dmahandle, DDI_FME_VERSION);
		ddi_fm_service_impact(cpqary3p->dip, DDI_SERVICE_UNAFFECTED);
	...
	case CISS_CMD_INVALID:
		DBG1("CPQary3 : _process_pkt : Invalid Command :"
		    " OpCode = %x \n", scsi_pktp->pkt_cdbp[0]);
	...
	if ((errorinfop->ScsiStatus == SCSI_CHECK_CONDITION) ||
		...
		if (errorinfop->SenseLen) {
			...
			arq_statusp = (struct scsi_arq_status *)
			    (void *)scsi_pktp->pkt_scbp;
...
{
	/*
	 * In the scsi packet structure, the first byte is the SCSI
	 * Command OpCode. We check to see if it is any one of the
	 * SCSI Read or Write opcodes.
	 */
...
}       /* End of cpqary3_is_scsi_read_write() */

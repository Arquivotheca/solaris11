    ...
    IN    lm_tcp_state_t        * tcp)
{
    ...
    DbgMessage1(pdev, FATAL, "##lm_tcp_erase_connection(0x%x)\n",tcp->cid);
    if (tcp->ulp_type == TOE_CONNECTION_TYPE) {
        ...
        if (tx_con->abortion_under_flr) {
            DbgMessage1(pdev, FATAL, "##lm_tcp_erase_connection(0x%x): Tx aborted\n",tcp->cid);           
        }
        ...
        if (mm_tcp_indicating_bufs(rx_con)) {
            DbgMessage1(pdev, FATAL, "##lm_tcp_erase_connection(0x%x): under indication\n",tcp->cid);
            DbgBreak();
        ...
        if (rx_con->abortion_under_flr) {
            DbgMessage1(pdev, FATAL, "##lm_tcp_erase_connection(0x%x): Rx aborted\n",tcp->cid);
        }
    ...
    lm_tcp_state_t *tcp)
{
    ...
    if (tcp->ulp_type != TOE_CONNECTION_TYPE) {
        DbgMessage2(pdev, WARN, "##lm_tcp_flush_db is not sent for connection(0x%x) of type %d\n",tcp->cid, tcp->ulp_type);
        return;
    }

    DbgMessage1(pdev, WARN, "##lm_tcp_flush_db (cid=0x%x)\n",tcp->cid);
    rx_con = tcp->rx_con;
    tx_con = tcp->tx_con;
...
static lm_status_t lm_tcp_alloc_resc(lm_device_t *pdev)
{
    ...
    LM_TOE_FOREACH_TSS_IDX(pdev, i)
    {
        ...
        mm_memset((void *) (bd_chain->bd_chain_virt), 0, mem_size);
    }
    ...
    LM_TOE_FOREACH_RSS_IDX(pdev, i)
    {
        ...
        mm_memset((void *) (toe_info->grqs[i].isles_pool), 0, mem_size);
    }
...
u8_t lm_tcp_rx_fill_grq(struct _lm_device_t * pdev, u8_t sb_idx, d_list_t * bypass_gen_pool_list, u8_t filling_mode)
{
    ...
    DbgMessage1(pdev, VERBOSEl4rx , "###lm_tcp_rx_fill_grq bd_left (to be filled)= %d\n", bd_chain->bd_left);
    ...
    while (num_bufs--) {
        ...
        DbgMessage2(pdev, VERBOSEl4rx, "curr_gen_buf->buf_virt=0x%p, END_SIG=0x%x\n", curr_gen_buf->buf_virt,
...
lm_status_t lm_tcp_init_resc(struct _lm_device_t *pdev, u8_t b_is_init )
{
    ...
    /* init l4 client params: 
     * _some_ of these values are used for all protocols. therefore, _all_ fields have default
     * values in pdev->params, and were _all_ already initiated in FW/HW in the per port init stage. 
     * they are copied here into toe_info->l4_params.
     * from here on, the values that rule are the values in toe_info->l4_params. 
     * the values can (and will) be updated by l4 client. */    
    _lm_get_default_l4cli_params(pdev, &toe_info->l4_params);    
    ...
    LM_TOE_FOREACH_TSS_IDX(pdev, i)
    {
        ...
        sb_indexes = lm_get_sb_indexes(pdev, (u8_t)i);
...
static void _lm_tcp_init_cstorm_intmem(lm_device_t *pdev)
{
    ...
    LM_TOE_FOREACH_TSS_IDX(pdev, drv_toe_rss_id)
    {
        ...
        LM_INTMEM_WRITE8(pdev, CSTORM_TOE_STATUS_BLOCK_ID_OFFSET(LM_TOE_FW_RSS_ID(pdev,drv_toe_rss_id), port), LM_FW_SB_ID(pdev,RSS_ID_TO_SB_ID(drv_toe_rss_id)), BAR_CSTRORM_INTMEM);
...
static void _lm_tcp_init_ustorm_intmem(lm_device_t *pdev)
{
    ...
    _lm_set_ofld_params_ustorm_toe(pdev, &toe_info->l4_params);
...
//            LM_INTMEM_WRITE32(pdev, CSTORM_BYTE_COUNTER_OFFSET(LM_TOE_FW_RSS_ID(pdev,drv_toe_rss_id), HC_INDEX_TOE_RX_CQ_CONS), l4_quasi_byte_counter, BAR_CSTRORM_INTMEM);
        ...
        if (CHIP_IS_E1x(pdev)) {
            ...
            if (pdev->params.enable_dynamic_hc[HC_INDEX_TOE_RX_CQ_CONS]) {
                pdev->vars.status_blocks_arr[RSS_ID_TO_SB_ID(drv_toe_rss_id)].hc_status_block_data.e2_sb_data.index_data[HC_INDEX_TOE_RX_CQ_CONS].flags |= HC_INDEX_DATA_DYNAMIC_HC_ENABLED;
            } else {
                pdev->vars.status_blocks_arr[RSS_ID_TO_SB_ID(drv_toe_rss_id)].hc_status_block_data.e2_sb_data.index_data[HC_INDEX_TOE_RX_CQ_CONS].flags &= ~HC_INDEX_DATA_DYNAMIC_HC_ENABLED;
            }
            LM_INTMEM_WRITE8(PFDEV(pdev), CSTORM_STATUS_BLOCK_DATA_OFFSET(fw_sb_id) 
                              ...
                              + sizeof(struct hc_index_data)*HC_INDEX_TOE_RX_CQ_CONS
                              + OFFSETOF(struct hc_index_data,flags),
                              pdev->vars.status_blocks_arr[RSS_ID_TO_SB_ID(drv_toe_rss_id)].hc_status_block_data.e2_sb_data.index_data[HC_INDEX_ETH_RX_CQ_CONS].flags, BAR_CSTRORM_INTMEM);
...
//        LM_INTMEM_WRITE8(pdev, USTORM_TOE_STATUS_BLOCK_ID_OFFSET(LM_TOE_FW_RSS_ID(pdev,drv_toe_rss_id), port),LM_TOE_FW_RSS_ID(pdev,drv_toe_rss_id), BAR_USTRORM_INTMEM); 
        LM_INTMEM_WRITE8(pdev, USTORM_TOE_STATUS_BLOCK_ID_OFFSET(LM_TOE_FW_RSS_ID(pdev,drv_toe_rss_id), port),LM_FW_SB_ID(pdev,RSS_ID_TO_SB_ID(drv_toe_rss_id)), BAR_USTRORM_INTMEM);
...
{           
    _lm_set_ofld_params_tstorm_toe(pdev, &pdev->toe_info.l4_params);        
...
{
    _lm_set_ofld_params_xstorm_toe(pdev, &pdev->toe_info.l4_params);
}
...
lm_status_t lm_tcp_init_chip_common(lm_device_t *pdev)
{
    ...
    /*
      !!DP Copy the values into pdev->toe_info.l4_params
    */
    pdev->toe_info.l4_params = l4_params;
...
lm_status_t lm_tcp_start_chip(lm_device_t *pdev)
{
    ...
    lm_command_post(pdev, LM_SW_LEADING_RSS_CID(pdev), RAMROD_CMD_ID_TOE_START, CMD_PRIORITY_NORMAL, TOE_CONNECTION_TYPE, data);  
    ...
    if(toe_info->state != LM_TOE_STATE_NORMAL) {
		...
		lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_START, TOE_CONNECTION_TYPE, LM_SW_LEADING_RSS_CID(pdev));
#endif
    ...
    u8_t update_suspend_rcq)
{
   ...
   /* Update update statistics - These statistics indicate which FW flow was taken and also count the overall number of updates */
    DbgMessage1(pdev, WARN, "lm_tcp_rss_update_ramrod_comp(): %d\n",update_stats_type);
    ...
    IN    lm_tcp_rcq_t        * rcq)
{
    ...
    if (rcq->suspend_processing == FALSE) 
    {
        ...
        if (mm_atomic_dec(&pdev->params.update_toe_comp_cnt) == 0)
        {
            ...
            lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_RSS_UPDATE, 
    ...
    if (rcq->suspend_processing == FALSE) 
    {
        ...
        DbgMessage(pdev, WARNl4sp, "lm_tcp_rss_update_suspend_rcq(): calling lm_eth_update_ramrod_comp\n");
...
lm_status_t lm_tcp_init_tcp_state(
    ...
    u32_t tcp_cid_addr)
{
    DbgMessage2(pdev, VERBOSEl4sp, "###lm_tcp_init_tcp_state, ptr=%p, src_port=%d\n", tcp, tcp_const->src_port);
    ...
    if (!tcp_cid_addr) {
        ...
        lm_set_cid_resc(pdev, TOE_CONNECTION_TYPE, (void *)tcp, tcp_cid_addr);
    }
...
u32_t lm_tcp_rx_con_get_virt_size(struct _lm_device_t * pdev, lm_tcp_state_t * tcp)
{
    ...
    mss = _lm_tcp_calc_mss(tcp->path->path_cached.path_mtu,
                           ...
                           (tcp->path->path_const.ip_version == IP_VERSION_IPV6),
                           tcp->tcp_const.tcp_flags & TCP_FLAG_ENABLE_TIME_STAMP,
                           pdev->toe_info.l4_params.flags & OFLD_PARAM_FLAG_SNAP_ENCAP,
                           tcp->path->neigh->neigh_const.vlan_tag  != 0);

    frag_cnt = _lm_tcp_calc_frag_cnt(pdev, tcp->tcp_cached.initial_rcv_wnd, mss);
    

    DbgMessage2(pdev, INFORMl4rx, "Calc #frags for rx-con initial_rcv_wnd: %d frag_cnt: %d\n", tcp->tcp_cached.initial_rcv_wnd, frag_cnt);
    ...
    lm_tcp_state_t *tcp)
{
    ...
    DbgMessage2(pdev, WARN, "_lm_tcp_init_ustorm_toe_context: IRO is 0x%x, IS is %d\n",
    ...
    /*DbgMessage1(pdev, FATAL, "_lm_tcp_init_ustorm_toe_context(): uctx_st->initial_rcv_wnd=%d\n", tcp->tcp_cached.initial_rcv_wnd);*/
    ...
    )
{
    ...
    l4_ofld_params_t *l4_params = &pdev->toe_info.l4_params;
    ...
    lm_tcp_state_t *tcp)
{
    ...
    u16_t pseudo_cs, i;
    u32_t sm_rtt, sm_delta;
    struct e2_integ_data * e2_integ_fw_data;
    u8_t * e2_integ_tcl_data;
    ...
    xtcp_st->ethernet.params |= (neigh->neigh_const.vlan_tag << XSTORM_ETH_CONTEXT_SECTION_VLAN_ID_SHIFT);
    if (pdev->params.e2_integ_testing_enabled)
    {
        if (tcp->cid & 1)
        {
            xtcp_st->ethernet.params |= (pdev->params.e2_integ_def_priority_setting_1 << XSTORM_ETH_CONTEXT_SECTION_PRIORITY_SHIFT);
        }
        else
        {
            xtcp_st->ethernet.params |= (pdev->params.e2_integ_def_priority_setting_0 << XSTORM_ETH_CONTEXT_SECTION_PRIORITY_SHIFT);
        }
    }
    ...
    xtcp_st->ethernet.params  |= (tcp->tcp_cached.user_priority << XSTORM_ETH_CONTEXT_SECTION_PRIORITY_SHIFT); 
    ...
    if (tcp->path->path_const.ip_version == IP_VERSION_IPV4) {
        ...
        e2_integ_fw_data = (struct e2_integ_data * )xtcp_st->ip_union.padded_ip_v4.reserved1;
        ASSERT_STATIC(ARRSIZE(tcp->tcp_const._pad) == 3); /* let's make sure no one took away our pad fields...*/
        if (pdev->params.e2_integ_testing_enabled) {
            DbgMessage(pdev, FATAL, "E2 integ testing enabled!!!\n");
            e2_integ_tcl_data  = tcp->tcp_const._pad;

            if (tcp->tcp_const._pad[0] == 0xff)
            {
                e2_integ_tcl_data[0] = 0;
                e2_integ_tcl_data[1] = 0;
                e2_integ_tcl_data[2] = 0;

                /* Take the default setting... */
                if (tcp->cid & 1)
                {
                    L4_DEBUG_SET_COS(e2_integ_tcl_data, pdev->params.e2_integ_def_cos_setting_1);
                    L4_DEBUG_SET_PBFQUEUE(e2_integ_tcl_data, pdev->params.e2_integ_def_pbfq_setting_1);
                    L4_DEBUG_SET_VOQ(e2_integ_tcl_data, pdev->params.e2_integ_def_voq_setting_1);
                    L4_DEBUG_SET_FLAGS(e2_integ_tcl_data, pdev->params.e2_integ_def_flags_1);
                }
                else 
                {
                    L4_DEBUG_SET_COS(e2_integ_tcl_data, pdev->params.e2_integ_def_cos_setting_0);
                    L4_DEBUG_SET_PBFQUEUE(e2_integ_tcl_data, pdev->params.e2_integ_def_pbfq_setting_0);
                    L4_DEBUG_SET_VOQ(e2_integ_tcl_data, pdev->params.e2_integ_def_voq_setting_0);                    
                    L4_DEBUG_SET_FLAGS(e2_integ_tcl_data, pdev->params.e2_integ_def_flags_0);
                }
            }

            
            SET_FLAGS(e2_integ_fw_data->flags, E2_INTEG_DATA_TESTING_EN);
            if (GET_FLAGS(L4_DEBUG_GET_FLAGS(e2_integ_tcl_data), L4_DEBUG_LB_EN_MASK)) {
                SET_FLAGS(e2_integ_fw_data->flags, E2_INTEG_DATA_LB_TX);
            }
            if (GET_FLAGS(L4_DEBUG_GET_FLAGS(e2_integ_tcl_data), L4_DEBUG_COS_EN_MASK)) {
                SET_FLAGS(e2_integ_fw_data->flags, E2_INTEG_DATA_COS_TX);
            }
            if (GET_FLAGS(L4_DEBUG_GET_FLAGS(e2_integ_tcl_data), L4_DEBUG_QM_OPPORT_EN_MASK)) {
                SET_FLAGS(e2_integ_fw_data->flags, E2_INTEG_DATA_OPPORTUNISTICQM);
            }
            e2_integ_fw_data->cos       = L4_DEBUG_GET_COS(e2_integ_tcl_data);
            e2_integ_fw_data->pbf_queue = L4_DEBUG_GET_PBFQUEUE(e2_integ_tcl_data);
            e2_integ_fw_data->voq       = L4_DEBUG_GET_VOQ(e2_integ_tcl_data);

            DbgMessage4(pdev, FATAL, "flags = %d, cos=%d, pbf_queue = %d, voq = %d\n",
                        e2_integ_fw_data->flags, e2_integ_fw_data->cos, e2_integ_fw_data->pbf_queue, e2_integ_fw_data->voq);
    } else {
            RESET_FLAGS(e2_integ_fw_data->flags, E2_INTEG_DATA_TESTING_EN);
        }
    } else {
        /* IPv6*/
    ...
    {    
        // set counter id
        SET_FLAGS( xtcp_st->flags, LM_STATS_CNT_ID(pdev)<<XSTORM_COMMON_CONTEXT_SECTION_STATISTICS_COUNTER_ID_SHIFT ) ;
    
        // set enable L2
        SET_FLAGS( xtcp_st->flags, 1<<XSTORM_COMMON_CONTEXT_SECTION_UPDATE_L2_STATSTICS_SHIFT );
    
        // set enable L4
        SET_FLAGS( xtcp_st->flags, 1<<XSTORM_COMMON_CONTEXT_SECTION_UPDATE_L4_STATSTICS_SHIFT );
    }
    if (tcp->ulp_type == ISCSI_CONNECTION_TYPE)
    {
        SET_FLAGS( xtcp_st->flags,(1 << XSTORM_COMMON_CONTEXT_SECTION_PHYSQ_INITIALIZED_SHIFT ));

        SET_FLAGS( xtcp_st->dcb_val,(PORT_ID(pdev) << XSTORM_COMMON_CONTEXT_SECTION_PBF_PORT_SHIFT));
    }
    return LM_STATUS_SUCCESS;
}
    ...
    lm_tcp_state_t *tcp)
{
    ...
    if (tcp->ulp_type == TOE_CONNECTION_TYPE) {
        ...
        if (tcp->rx_con->u.rx.gen_info.peninsula_nbytes > tcp->tcp_cached.initial_rcv_wnd) {
            ...
            DbgMessage4(pdev, WARN, "lm_tcp_init_tcp_state: pnb:%x, irw:%x, ext:%x, rnx:%x\n",tcp->rx_con->u.rx.gen_info.peninsula_nbytes,
         ...
         * In LH it's not the case. TBA: add assert that we are on LH operating system */
            DbgMessage4(pdev, WARN, "lm_tcp_init_tcp_state: pnb:%x, irw:%x, rws:%x, rnx:%x\n",tcp->rx_con->u.rx.gen_info.peninsula_nbytes,
                        tcp->tcp_cached.initial_rcv_wnd,
                        tcp->tcp_delegated.recv_win_seq,
                        tcp->tcp_delegated.recv_next);
...
void lm_tcp_comp_cb(struct _lm_device_t *pdev, struct sq_pending_command *pending)
{
    ...
    tcp = (lm_tcp_state_t *)lm_cid_cookie(pdev, TOE_CONNECTION_TYPE, pending->cid);
    ...
    switch (pending->cmd)
    {
    case RAMROD_CMD_ID_TOE_START:
        DbgBreakMsg("Not Supported\n");
        break;
    case RAMROD_CMD_ID_TOE_INITIATE_OFFLOAD:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_OFL_CMP);
        cmp_rx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_SEARCHER_DELETE:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_SRC_CMP);
        cmp_rx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_TERMINATE:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_TRM_CMP);
        cmp_rx = TRUE;
        LM_TCP_SET_CQE(tx_cqe.params, pending->cid, TOE_TX_CQES_OPCODE_TRM_CMP);
        cmp_tx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_QUERY:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_QRY_CMP);
        cmp_rx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_RESET:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_RST_CMP);
        cmp_rx = TRUE;
        LM_TCP_SET_CQE(tx_cqe.params, pending->cid, TOE_TX_CQES_OPCODE_RST_CMP);
        cmp_tx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_DRIVER_RESERVED:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_DRV_RSV);
        cmp_rx = TRUE;
        LM_TCP_SET_CQE(tx_cqe.params, pending->cid, TOE_TX_CQES_OPCODE_DRV_RSV);
        cmp_tx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_INVALIDATE:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_INV_CMP);
        cmp_rx = TRUE;
        LM_TCP_SET_CQE(tx_cqe.params, pending->cid, TOE_TX_CQES_OPCODE_INV_CMP);
        cmp_tx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_UPDATE:
        LM_TCP_SET_CQE(rx_cqe.params1, pending->cid, TOE_RX_CQES_OPCODE_UPD_CMP);
        cmp_rx = TRUE;
        break;
    case RAMROD_CMD_ID_TOE_RSS_UPDATE:
    ...
    lm_tcp_state_t *tcp)
{
    ...
    case L4_TCP_CON_STATE_FIN_WAIT1:
        DbgMessage1(pdev, WARNl4sp,"#tcp state offloaded in state FIN_WAIT1 (tcp=%p)\n", tcp);
        state_calc->fin_request_time = curr_time;
        break;
    case L4_TCP_CON_STATE_FIN_WAIT2:
        DbgMessage1(pdev, WARNl4sp,"#tcp state offloaded in state FIN_WAIT2 (tcp=%p)\n", tcp);
        state_calc->fin_request_time = curr_time - 1;
        state_calc->fin_completed_time = curr_time;
        break;
    case L4_TCP_CON_STATE_CLOSE_WAIT:
        DbgMessage1(pdev, WARNl4sp,"#tcp state offloaded in state CLOSE_WAIT (tcp=%p)\n", tcp);
        state_calc->fin_reception_time = curr_time;      
        break;
    case L4_TCP_CON_STATE_CLOSING:
        DbgMessage1(pdev, WARNl4sp,"#tcp state offloaded in state CLOSING (tcp=%p)\n", tcp);
        state_calc->fin_request_time = curr_time - 1;
        state_calc->fin_reception_time = curr_time;        
        break;
    case L4_TCP_CON_STATE_LAST_ACK:
        DbgMessage1(pdev, WARNl4sp,"#tcp state offloaded in state LAST_ACK (tcp=%p)\n", tcp);
    ...
    u64_t *data)
{
    ...
    *command = (tcp->ulp_type == TOE_CONNECTION_TYPE)? RAMROD_CMD_ID_TOE_INITIATE_OFFLOAD : L5CM_RAMROD_CMD_ID_ADD_NEW_CONNECTION;
    ...
    )
{
    ...
    *command = (tcp->ulp_type == TOE_CONNECTION_TYPE)? RAMROD_CMD_ID_TOE_TERMINATE : L5CM_RAMROD_CMD_ID_TERMINATE_OFFLOAD;
    *data = 0;
...
/*********************** Rx **********************/
    ...
    mm_release_tcp_lock(pdev, rx_con);
/**************Post the ramrod *******************/
    *command = RAMROD_CMD_ID_TOE_RESET;
    *data = 0;
    ...
    )
{
    ...
    *command = (tcp->ulp_type == TOE_CONNECTION_TYPE)? RAMROD_CMD_ID_TOE_SEARCHER_DELETE : L5CM_RAMROD_CMD_ID_SEARCHER_DELETE;
    ...
    )
{
    ...
    *command = (tcp->ulp_type == TOE_CONNECTION_TYPE)? RAMROD_CMD_ID_TOE_QUERY : L5CM_RAMROD_CMD_ID_QUERY;
    ...
    l4_path_delegated_state_t * ret_delegated)
{
    ...
    *ret_delegated = path_state->path_delegated;
    ...
    DbgMessage1(pdev, INFORMl4sp, "lm_tcp_post_upload_path_request: num_dependents=%d\n", path_state->num_dependents);
    ...
    )
{
    ...
    DbgMessage1(pdev, INFORMl4sp, "lm_tcp_post_upload_neigh_request: num_dependents=%d\n", neigh_state->num_dependents);
    ...
    )
{
    ...
    /* tcp-flags */
    DbgMessage1(pdev, INFORMl4sp, "## lm_tcp_set_tcp_cached cid=%d\n", tcp->cid);
    ...
    if ((tcp->tcp_cached.tcp_flags & TCP_FLAG_ENABLE_KEEP_ALIVE) !=
        ...
        DbgMessage2(pdev, WARNl4sp, "## tcp_cached: [cid=%d] update : flag TCP_FLAG_ENABLE_KEEP_ALIVE changed to %d\n", 
    ...
    if ((tcp->tcp_cached.tcp_flags & TCP_FLAG_ENABLE_NAGLING) !=
        ...
        DbgMessage2(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : flag TCP_FLAG_ENABLE_NAGLING changed to %d\n", 
                    tcp->cid, ctx->enable_nagle);
    }
    if (tcp_cached->tcp_flags & TCP_FLAG_RESTART_KEEP_ALIVE) {
        ctx->ka_restart = 1;
        DbgMessage1(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : flag TCP_FLAG_RESTART_KEEP_ALIVE set\n", 
                    tcp->cid);
    } else {
        ctx->ka_restart = 0;
    }
    if (tcp_cached->tcp_flags & TCP_FLAG_RESTART_MAX_RT) {
        ...
        DbgMessage1(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : flag TOE_CACHED_RESTART_MAX_RT set\n", 
                    tcp->cid);
    } else {
    ...
    if (tcp_cached->tcp_flags & TCP_FLAG_UPDATE_RCV_WINDOW) {
        ...
        DbgMessage1(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : flag TCP_FLAG_UPDATE_RCV_WINDOW set\n", 
                    tcp->cid);
    }
    ...
    if (tcp->path->path_const.ip_version == IP_VERSION_IPV6) {
        if (tcp->tcp_cached.flow_label != tcp_cached->flow_label) {
            DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : flow_label changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.initial_rcv_wnd != tcp_cached->initial_rcv_wnd) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : initial_rcv_wnd changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.ttl_or_hop_limit != tcp_cached->ttl_or_hop_limit) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : ttl_or_hop_limit changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.tos_or_traffic_class != tcp_cached->tos_or_traffic_class) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : tos_or_traffic_class changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.ka_probe_cnt != tcp_cached->ka_probe_cnt) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : ka_probe_cnt changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.user_priority != tcp_cached->user_priority) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : user_priority changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.rcv_indication_size != tcp_cached->rcv_indication_size) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : rcv_indication_size changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.ka_time_out != tcp_cached->ka_time_out) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : ka_time_out changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.ka_interval != tcp_cached->ka_interval) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : ka_interval changed from %d to %d\n", 
    ...
    if (tcp->tcp_cached.max_rt != tcp_cached->max_rt) {
        DbgMessage3(pdev, WARNl4sp, "## tcp_cached [cid=%d] update : max_rt changed from %d to %d\n", 
    ...
    if (!ctx->changed_fields && !ctx->ka_restart && !ctx->retransmit_restart) {
        DbgMessage1(pdev, INFORMl4sp, "## tcp_cached [cid=%d] update : nothing changed,  completing synchronously\n", tcp->cid);
        return LM_STATUS_SUCCESS; /* synchronous complete */
    }
    //DbgMessage1(pdev, WARNl4sp, "## lm_tcp_set_tcp_cached cid=%d DONE!\n", tcp->cid);
    return LM_STATUS_PENDING;
}
    ...
    )
{
    ...
    if (new_mss != tcp->rx_con->u.rx.sws_info.mss) {
         ...
         * required. */
        DbgMessage3(pdev, WARNl4sp, "## path_cached: tcp [cid=%d] update : mss (as a result of pathMtu) from %d to %d\n", 
    ...
    )
{
    ...
    DbgMessage1(pdev, WARNl4sp, "## neigh_cached: tcp [cid=%d] update : neighbor dst_addr\n", tcp->cid); 
    ...
    )
{
    ...
    *command = RAMROD_CMD_ID_TOE_UPDATE;
    spe.toe_data.phys_addr.hi = tcp->sp_req_data.phys_addr.as_u32.high;
    spe.toe_data.phys_addr.lo = tcp->sp_req_data.phys_addr.as_u32.low;
    *data = *((u64_t*)(&(spe.toe_data.phys_addr)));
    ...
    OUT u64_t                      * data)
{
    ...
    *command = RAMROD_CMD_ID_TOE_DRIVER_RESERVED;
    spe.toe_data.rx_completion.hash_value = (u16_t)(tcp->tcp_const.hash_value);
    *data = *((u64_t*)(&(spe.toe_data.rx_completion)));
    ...
    OUT u64_t                      * data)
{
    ...
    DbgMessage1(pdev, INFORMl4sp, "## lm_tcp_post_invalidate_request cid=%d\n", tcp->cid);
    ...
    *command = RAMROD_CMD_ID_TOE_INVALIDATE;
    *data = 0; 
    ...
    lm_tcp_slow_path_request_t *request)
{
    ...
    DbgMessage2(pdev, VERBOSEl4sp, "### lm_tcp_post_slow_path_request cid=%d, type=%d\n", tcp->cid, request->type);
...
void lm_tcp_service_deferred_cqes(lm_device_t * pdev, lm_tcp_state_t * tcp)
{
    ...
    DbgMessage1(pdev, INFORMl4sp, "### lm_tcp_service_deferred_cqes cid=%d\n", tcp->cid);
    ...
    for (idx = 0; idx < 2; idx++) {
        ...
        while(con->flags & TCP_DEFERRED_PROCESSING) {
            ...
            DbgMessage1(pdev, WARNl4sp, "### deferred cid=%d\n", tcp->cid);
    ...
    u32_t comp_status)
{
    ...
    if(!comp_status) { /* successful completion */
...
#ifndef _VBD_CMD_
        DbgMessage1(pdev, FATAL, "initiate offload failed. err=%x\n", comp_status);
#endif // _VBD_CMD_
...
//    DbgMessage2(pdev, FATAL, "#lm_tcp_comp_initiate_offload_request cid=%d, sp_request->status=%d\n", tcp->cid, tcp->sp_request->status);
    sp_request = tcp->sp_request;
    tcp->sp_request = NULL;
    ...
    lm_tcp_state_t *tcp)
{
    ...
    DbgMessage1(pdev, VERBOSEl4sp, "###lm_tcp_free_tcp_resc tcp=%p\n", tcp);
    DbgBreakIf(!(pdev && tcp));
...
{
    l4_ofld_params_t *curr_params = &pdev->toe_info.l4_params;
    ...
    )
{
    ...
    //DbgMessage1(pdev, WARNl4rx , "##lm_tcp_indicate_rst_received cid=%d\n", tcp->cid);
    ...
    )
{
    ...
    DbgMessage1(pdev, INFORMl4rx , "##lm_tcp_indicate_fin_received cid=%d\n", tcp->cid);
    DbgBreakIf( ! ( pdev && tcp ) );
    ...
    l4_upload_reason_t    upload_reason)
{
    u32_t rx_flags = 0;
    u32_t tx_flags = 0;
    DbgMessage2(pdev, WARNl4, "###lm_tcp_process_retrieve_indication_cqe cid=%d upload_reason=%d\n", tcp->cid, upload_reason);
    ...
    if (!GET_FLAGS(tcp->rx_con->flags, rx_flags) && !GET_FLAGS(tcp->tx_con->flags,tx_flags)) {
        ...
        DbgMessage2(pdev, WARNl4, "###Indicating UP: cid=%d upload_reason=%d\n", tcp->cid, upload_reason);
    ...
    )
{
    ...
    DbgMessage1(pdev, INFORMl4rx , "###lm_tcp_rx_fin_received_complete cid=%d\n", tcp->cid);
    DbgBreakIf( ! (pdev && tcp) );
    ...
    if (!is_empty_peninsula || mm_tcp_indicating_bufs(rx_con) ) {
        DbgMessage1(pdev, WARNl4 , "lm_tcp_process_fin_received_cqe - postponing fin indication cid=%d\n", tcp->cid);
        rx_con->u.rx.flags |= TCP_CON_FIN_IND_PENDING;
        indicate = 0;
    }
    ...
    IN    u32_t                 sp_type)
{
    ...
    DbgMessage2(pdev, INFORMl4rx | INFORMl4sp , 
    ...
    IN    u32_t                 sp_type)
{
    ...
    DbgMessage2(pdev, INFORMl4tx | INFORMl4sp , 
...
/***************** Rx ********************/
    ...
    if (mm_tcp_indicating_bufs(rx_con)) {
        if (pdev->params.l4_support_pending_sp_req_complete) {
            ...
            DbgMessage1(pdev, WARN, "Abortive disconnect completion during indication(%d)\n", tcp->cid);
        } else {
            DbgBreak();
        }
    }
    ...
    )
{
    ...
    DbgMessage1(pdev, INFORMl4rx , "###lm_tcp_process_rst_received_rcqe cid=%d\n", tcp->cid);
    DbgBreakIf( ! (pdev && tcp) );
    /* The state may only be NORMAL or UPLOAD_PENDING */
    DbgBreakIf( (tcp->hdr.status != STATE_STATUS_NORMAL) && 
                (tcp->hdr.status != STATE_STATUS_UPLOAD_PENDING) );
    ...
    if (rx_con->u.rx.gen_info.peninsula_nbytes || mm_tcp_indicating_bufs(rx_con) ) {
        DbgMessage1(pdev, INFORMl4rx , "lm_tcp_process_rst_received_cqe - postponing rst indication cid=%d\n", tcp->cid);
    ...
    )
{
    ...
    DbgMessage1(pdev, INFORMl4tx , "###lm_tcp_tx_rst_received_complete cid=%d\n", tcp->cid);
    DbgBreakIf( ! (pdev && tcp) );
    /* The state may only be NORMAL or UPLOAD_PENDING */
    DbgBreakIf( (tcp->hdr.status != STATE_STATUS_NORMAL) && 
                (tcp->hdr.status != STATE_STATUS_UPLOAD_PENDING) );
    ...
    if ( s_list_entry_cnt(&tx_con->active_tb_list) > 0 ) {
        DbgMessage1(pdev, INFORMl4rx , "TX lm_tcp_process_rst_received_cqe - postponing rst indication cid=%d sending empty ramrod\n", tcp->cid);
        tx_con->u.tx.flags |= TCP_CON_RST_IND_NOT_SAFE;
    ...
    if ( indicate ) {
        ...
        if ((lm_status != LM_STATUS_PENDING) && (lm_status != LM_STATUS_UPLOAD_IN_PROGRESS)) { /* we expect the posting of an empty ramrod to be pending... */
            ...
            if (upload_on_fail) {
                DbgMessage(pdev, WARN, "Couldn't send empty ramrod on TX when we needed\n");
    ...
    lm_tcp_state_t      * tcp)
{
    ...
    DbgMessage1(pdev, INFORMl4rx , "###lm_tcp_process_abortive_disconnect_request_rcqe cid=%d\n", tcp->cid);
    DbgBreakIf( ! (pdev && tcp) );
    ...
    lm_tcp_state_t      * tcp)
{
    ...
    DbgMessage1(pdev, INFORMl4tx , "###lm_tcp_tx_abortive_disconnect_request_complete cid=%d\n", tcp->cid);
    DbgBreakIf( ! (pdev && tcp) );
...
static void lm_tcp_comp_invalidate_request(
    ...
    lm_tcp_slow_path_request_t * request)
{
    DbgMessage1(pdev, INFORMl4sp, "### Completing invalidate request cid=%d\n", tcp->cid);
    ...
    lm_tcp_state_t      * tcp)
{
    ...
    DbgMessage1(pdev, INFORMl4tx , "###lm_tcp_tx_invalidate_request_complete cid=%d\n", tcp->cid);
    ...
    lm_tcp_state_t      * tcp)
{
    ...
    DbgMessage1(pdev, INFORMl4rx , "###lm_tcp_process_invalidate_request_rcqe cid=%d\n", tcp->cid);
    DbgBreakIf( ! (pdev && tcp) );
...
*/
    ...
    tcp->tcp_delegated.sm_rtt = 
        lm_time_resolution(pdev, tcp->tcp_delegated.sm_rtt, TIMERS_TICKS_PER_SEC, pdev->toe_info.l4_params.ticks_per_second)*8;
    tcp->tcp_delegated.sm_delta = 
        lm_time_resolution(pdev, tcp->tcp_delegated.sm_delta, TIMERS_TICKS_PER_SEC, pdev->toe_info.l4_params.ticks_per_second)*4;
    
    tcp->tcp_delegated.ts_recent     = tst_tcp->timestamp_recent;
    /* convert ms to ticks. */
    tcp->tcp_delegated.ts_recent_age = 
        lm_time_resolution(pdev, tst_tcp->timestamp_recent_time, TSEMI_CLK1_TICKS_PER_SEC, pdev->toe_info.l4_params.ticks_per_second);

    tcp->tcp_delegated.tstamp   = xst_tcp->ts_time_diff;
    /* convert ms to ticks. */
    tcp->tcp_delegated.total_rt = 
        lm_time_resolution(pdev, tst_tcp->retransmit_start_time, TIMERS_TICKS_PER_SEC, pdev->toe_info.l4_params.ticks_per_second);
    ...
    if(tcp->tcp_delegated.send_una == tcp->tcp_delegated.send_max && (send_wnd > 0)) { /* KA is running (?) */
        if ( (tcp->tcp_cached.tcp_flags & TCP_FLAG_ENABLE_KEEP_ALIVE)) {
           ...
           tcp->tcp_delegated.u.keep_alive.probe_cnt     = tst_tcp->ka_probe_count;
            ...
            lm_time_resolution(pdev, xag_tcp->ka_timer, TIMERS_TICKS_PER_SEC, pdev->toe_info.l4_params.ticks_per_second);
...
{
    lm_tcp_con_t *rx_con = tcp->rx_con, *tx_con = tcp->tx_con;
    u8_t has_fin = 0, has_rst = 0;
    lm_tcp_slow_path_request_t * sp_req = tcp->sp_request;
    ...
    if (tcp->ulp_type == TOE_CONNECTION_TYPE) {
        ...
        lm_tcp_rx_get_buffered_data_from_terminate(pdev, tcp, 
    ...
    if (path) {
        DbgMessage1(pdev, WARNl4sp, "_lm_tcp_comp_upload_request: last tcp dependent of pending path %p\n", path);
...
void lm_tcp_update_ramrod_complete(lm_device_t * pdev, lm_tcp_state_t * tcp)
{
    ...
    DbgMessage1(pdev, INFORMl4sp , "###lm_tcp_update_ramrod_complete cid=%d \n", tcp->cid);
...
{    
    ...
    DbgMessage3(pdev, FATAL, "## lm_tcp_debug_query START version %d.%d.%d\n", 
    ...
    while (tcp_state) {
        ...
        if(status != STATE_STATUS_NORMAL) {
            DbgMessage3(pdev, FATAL, "# tcp ptr 0x%p (cid %d), has status=%d (!= normal)\n",
                        tcp_state, tcp_state->cid, status);
        }

        /* verify the is no pending slow path request */
        if(tcp_state->sp_request) {
            DbgMessage4(pdev, FATAL, "# tcp ptr 0x%p (cid %d), has slow path request of type %d, not completed by FW (sp comp flags=0x%x\n",
        ...
        if(tcp_state->tx_con->bytes_post_cnt != tcp_state->tx_con->bytes_comp_cnt) {
            DbgMessage4(pdev, FATAL, "# tcp ptr 0x%p (cid %d), has TX pending bytes (%d). (con->flags=0x%x)\n",
        ...
        if(tcp_state->rx_con->bytes_post_cnt != tcp_state->rx_con->bytes_comp_cnt) {
            DbgMessage4(pdev, FATAL, "# tcp ptr 0x%p (cid %d), has RX pending bytes (%d). (con->flags=0x%x)\n",
                        tcp_state, tcp_state->cid, 
    ...
    /* print statistics */    
    DbgMessage1(pdev, FATAL, "# num offloaded connections=%d\n", num_tcps);
    for (i = 0; i < STATE_STATUS_ERR+1; i++) {
        if (status_arr[i]) {
            DbgMessage2(pdev, FATAL, "#    num connections in status %d=%d\n", i, status_arr[i]);
        }
    }
...
void lm_tcp_upld_close_received_complete(
    ...
    l4_upload_reason_t    upload_reason)
{
    DbgMessage1(pdev, INFORMl4sp , "###lm_tcp_drv_upl_received_complete cid=%d \n", tcp->cid);
    ...
    IN    lm_tcp_con_t        * con)
{
    ...
    if (con->dpc_info.snapshot_flags & LM_TCP_DPC_RAMROD_CMP) {
        ...
        case SP_REQUEST_PENDING_TX_RST:
            ...
            break;
        default:
            DbgMessage1(pdev, FATAL, "unexpected sp completion type=%d\n", tcp->sp_request->type);
            DbgBreak();
        }
        /* Get global TOE lock */
        MM_ACQUIRE_TOE_LOCK(pdev);
        ...
        if (complete_ramrod) {
            ...
            case SP_REQUEST_ABORTIVE_DISCONNECT:
                ...
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_RESET, tcp->ulp_type, tcp->cid);
            break;
            case SP_REQUEST_INVALIDATE:
                DbgBreakIf(request->type != SP_REQUEST_INVALIDATE);
                lm_tcp_comp_invalidate_request(pdev, tcp, request);
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_INVALIDATE, tcp->ulp_type, tcp->cid);
            break;
            case SP_REQUEST_TERMINATE1_OFFLOAD:        
                DbgBreakIf(request->type != SP_REQUEST_TERMINATE1_OFFLOAD);
                lm_tcp_terminate_ramrod_complete(pdev, tcp);
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_TERMINATE, tcp->ulp_type, tcp->cid);
            break;
            case SP_REQUEST_PENDING_LOCAL_FIN_DISCONNECT:
            case SP_REQUEST_PENDING_REMOTE_DISCONNECT:
            case SP_REQUEST_PENDING_TX_RST:
                lm_tcp_comp_empty_ramrod_request(pdev, tcp);
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_DRIVER_RESERVED, tcp->ulp_type, tcp->cid);
            break;
            default:
                DbgMessage1(pdev, FATAL, "unexpected sp completion type=%d\n", tcp->sp_request->type);
                DbgBreak();
    ...
    )
{
    ...
    DbgMessage2(pdev, INFORMl4rx, "lm_tcp_rx_complete_tcp_sp tcp=%p cid=%d \n", tcp, tcp->cid);
    ...
    if (con->dpc_info.snapshot_flags & LM_TCP_DPC_RAMROD_CMP) {
        ...
        switch (tcp->sp_request->type) {
            ...
            case SP_REQUEST_UPDATE_PATH_RELINK:
                ...
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_UPDATE, ulp_type, cid);
                return;
            case SP_REQUEST_QUERY:
                lm_tcp_query_ramrod_complete(pdev, tcp); /*  this may delete tcp !! */
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_QUERY, ulp_type, cid);
                return;
            case SP_REQUEST_TERMINATE_OFFLOAD:
                lm_tcp_searcher_ramrod_complete(pdev, tcp);
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_SEARCHER_DELETE, ulp_type, cid);
                return;
            case SP_REQUEST_INITIATE_OFFLOAD:
                ...
                lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_INITIATE_OFFLOAD, tcp->ulp_type, cid);
                return;
        }
    ...
    )
{
    ...
    if ((con_rst) || (con_upld_close)) {
        ...
        if (snd_una == snd_max) {
            if ( mm_get_current_time(pdev) - time_wait_state_entering_time > 2*pdev->toe_info.l4_params.ticks_per_second *MSL ) {
                tcp_state = L4_TCP_CON_STATE_CLOSED;
            } else  {

...
static void ecore_init_str_wr(struct _lm_device_t *pdev, u32 addr, const u32 *data,
			      u32 len)
{
	u32 i;
...
static void ecore_init_ind_wr(struct _lm_device_t *pdev, u32 addr, const u32 *data,
			      u32 len)
{
	u32 i;

	for (i = 0; i < len; i++)
		REG_WR_IND(pdev, addr + i*4, data[i]);
}

static void ecore_write_big_buf(struct _lm_device_t *pdev, u32 addr, u32 len, u8 wb)
{
	if (DMAE_READY(pdev))
		PHYS_WR_DMAE_LEN(pdev, GUNZIP_PHYS(pdev), addr, len);
	else if (wb) /* Wide bus registers with no dmae need to be written using indirect */
		ecore_init_ind_wr(pdev, addr, GUNZIP_BUF(pdev), len);
	else
		ecore_init_str_wr(pdev, addr, GUNZIP_BUF(pdev), len);
}

static void ecore_init_fill(struct _lm_device_t *pdev, u32 addr, int fill, u32 len, u8 wb)
{
...
static void ecore_init_wr_64(struct _lm_device_t *pdev, u32 addr, const u32 *data,
			     u32 len64)
{
...
static const u8 *ecore_sel_blob(struct _lm_device_t *pdev, u32 addr, const u8 *data)
{
...
static void ecore_init_wr_wb(struct _lm_device_t *pdev, u32 addr, const u32 *data,
			     u32 len)
{
	if (DMAE_READY(pdev))
...
static void ecore_init_fw(struct _lm_device_t *pdev, u32 addr, u32 len)
{
    ...
    data = (u32 *)ecore_sel_blob(pdev, addr, (const u8 *)data);
...
static void ecore_wr_64(struct _lm_device_t *pdev, u32 reg, u32 val_lo, u32 val_hi)
{
	u32 wb_write[2];
...
static void ecore_init_wr_zp(struct _lm_device_t *pdev, u32 addr, u32 len, u32 blob_off)
{
	const u8 *data = NULL;
	int rc;
	u32 i;
...
{
	u16 op_start =
		INIT_OPS_OFFSETS(pdev)[BLOCK_OPS_IDX(block,stage,STAGE_START)];
	u16 op_end =
		INIT_OPS_OFFSETS(pdev)[BLOCK_OPS_IDX(block,stage,STAGE_END)];
	union init_op *op;
	int hw_wr;
	...
	for (i = op_start; i < op_end; i++) {
		...
		op = (union init_op *)&(INIT_OPS(pdev)[i]);
		...
		/* HW/EMUL specific */
		if ((op_type > OP_LAST_GEN) && (op_type == hw_wr))
			op_type = OP_WR;
		...
		case OP_SI:
			...
			break;
		case OP_ZR:
			ecore_init_fill(pdev, addr, 0, op->zero.len, 0 /* wb */);
			break;
		case OP_WB_ZR:
			ecore_init_fill(pdev, addr, 0, op->zero.len, 1 /* wb */ );
			break;
		case OP_ZP:
...
static void ecore_init_pxp_arb(struct _lm_device_t *pdev, int r_order, int w_order)
{
	u32 val, i;

	if (r_order > MAX_RD_ORD) {
		DbgMessage2(pdev, WARNi, "read order of %d  order adjusted to %d\n",
		   r_order, MAX_RD_ORD);
		r_order = MAX_RD_ORD;
	}
	if (w_order > MAX_WR_ORD) {
		DbgMessage2(pdev, WARNi, "write order of %d  order adjusted to %d\n",
		   w_order, MAX_WR_ORD);
		w_order = MAX_WR_ORD;
	}
	if (CHIP_REV_IS_FPGA(pdev)) {
	...
	DbgMessage2(pdev, INFORMi, "read order %d  write order %d\n", r_order, w_order);
...
static int ecore_ilt_line_mem_op(struct _lm_device_t *pdev, struct ilt_line *line,
				 u32 size, u8 memop)
{
...
static int ecore_ilt_client_mem_op(struct _lm_device_t *pdev, int cli_num, u8 memop)
{
	int i, rc;
...
int ecore_ilt_mem_op(struct _lm_device_t *pdev, u8 memop)
{
...
static void ecore_ilt_line_init_op(struct _lm_device_t *pdev, struct ecore_ilt *ilt,
				   int idx, u8 initop)
{
...
void ecore_ilt_boundry_init_op(struct _lm_device_t *pdev,
...
void ecore_ilt_client_init_op_ilt(struct _lm_device_t *pdev, struct ecore_ilt *ilt, struct ilt_client_info *ilt_cli, u8 initop)
{
	int i;
...
void ecore_ilt_client_init_op(struct _lm_device_t *pdev, struct ilt_client_info *ilt_cli, u8 initop)
{
	struct ecore_ilt *ilt = PDEV_ILT(pdev);
...
static void ecore_ilt_client_id_init_op(struct _lm_device_t *pdev, int cli_num, u8 initop)
{
	struct ecore_ilt *ilt = PDEV_ILT(pdev);
	struct ilt_client_info *ilt_cli = &ilt->clients[cli_num];
...
void ecore_ilt_init_op(struct _lm_device_t *pdev, u8 initop)
{
...
void ecore_ilt_init_page_size(struct _lm_device_t *pdev, u8 initop)
{
...
void ecore_qm_init_cid_count(struct _lm_device_t *pdev, int qm_cid_count,
			     u8 initop)
{
...
static void ecore_qm_set_ptr_table(struct _lm_device_t *pdev, int qm_cid_count)
{
	...
	for (i = 0; i < 4 * QM_QUEUES_PER_FUNC; i++) {
		REG_WR(pdev, QM_REG_BASEADDR + i*4,
		       qm_cid_count * 4 * (i % QM_QUEUES_PER_FUNC));
		ecore_init_ind_wr(pdev, QM_REG_PTRTBL + i*8,
				  wb_data, 2);

		if (CHIP_IS_E1H(pdev)) {
			REG_WR(pdev, QM_REG_BASEADDR_EXT_A + i*4,
			       qm_cid_count * 4 * (i % QM_QUEUES_PER_FUNC));
			ecore_init_ind_wr(pdev, QM_REG_PTRTBL_EXT_A + i*8,
					  wb_data, 2);
		}
	}
}
...
void ecore_qm_init_ptr_table(struct _lm_device_t *pdev, int qm_cid_count,
			     u8 initop)
{
	if (!QM_INIT(qm_cid_count))
		return;
...
void ecore_src_init_t2(struct _lm_device_t *pdev, struct src_ent *t2,
		       lm_address_t t2_mapping, int src_cid_count)
{
	...
	for (i = 0; i < src_cid_count-1; i++)
		t2[i].next = (u64)(t2_mapping.as_u64 + (i+1)*sizeof(struct src_ent));

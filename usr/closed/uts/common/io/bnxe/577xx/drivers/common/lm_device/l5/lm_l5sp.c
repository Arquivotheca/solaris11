...
u32_t lm_sc_get_phys_size(
    struct _lm_device_t * pdev)
{
    u32_t mem_size = 0;
    mem_size = SC_SP_PHYS_DATA_SIZE;
    ...
    IN  lm_iscsi_slow_path_request_t *request)
{
    ...
    DbgMessage2(pdev, VERBOSEl5sp, "### lm_sc_post_slow_path_request cid=%d, type=%d\n", iscsi->cid, request->type);
...
{
    DbgMessage1(pdev, VERBOSEl5sp, "###lm_sc_init_iscsi_state, ptr=%p\n", iscsi);
...
lm_fc_init_fcoe_state(
    ...
    lm_fcoe_state_t                 *fcoe)
{
    DbgMessage1(pdev, VERBOSEl5sp, "###lm_fc_init_fcoe_state, ptr=%p\n", fcoe);

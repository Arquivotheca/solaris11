...
#define IS_OOO_CQE(__cmd)   ((__cmd == TOE_RX_CQES_OPCODE_GNI) \
                          || (__cmd == TOE_RX_CQES_OPCODE_GAIR) \
                          || (__cmd == TOE_RX_CQES_OPCODE_GAIL) \
                          || (__cmd == TOE_RX_CQES_OPCODE_GRI) \
                          || (__cmd == TOE_RX_CQES_OPCODE_GJ) \
                          || (__cmd == TOE_RX_CQES_OPCODE_DGI))
    ...
    )
{
    ...
    case TCP_RX_POST_SWS_INC:
        /*DbgMessage4(pdev, FATAL, "lm_tcp_rx_post_sws() INC: OLD drv_rcv_win_right_edge=%d, nbytes=%d, NEW drv_rcv_win_right_edge=%d FW right_edge=%d \n", rx_con->u.rx.sws_info.drv_rcv_win_right_edge, nbytes, rx_con->u.rx.sws_info.drv_rcv_win_right_edge + nbytes, db_data->rcv_win_right_edge);*/
    ...
    //DbgMessage3(pdev, WARNl4, "###lm_tcp_rx_post_sws cid=%d num_bytes=%d diff_to_fw=%d \n", tcp->cid, nbytes, diff_to_fw );
    ...
    )
{
    ...
    DbgMessage3(NULL, VERBOSEl4rx, "Setting Rx BD flags=0x%x, bd_addr=0x%p, size=%d\n", rx_bd->flags, rx_bd, frag->size);
    return rx_bd;
}
    ...
    )
{
    ...
    DbgMessage3(pdev, VERBOSEl4rx, "##lm_tcp_rx_cmp_process, completed_bytes=%d, push=%d cid=%d\n", completed_bytes, push, tcp->cid);
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "lm_tcp_rx_comp, after comp: pending=%d, active_bufs=%d\n", 
    ...
    )
{
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "##lm_tcp_rx_skp_process, bytes_skipped=%d, cid=%d\n", bytes_skipped, tcp->cid);
    ...
    u32_t                 num_of_isles)
{
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "##lm_tcp_rx_delete_isle cid=%d isle=%d\n", tcp->cid, num_isle);
...
u8_t lm_toe_is_rx_completion(lm_device_t *pdev, u8_t drv_toe_rss_id)
{
    ...
    DbgMessage1(pdev, INFORMl4int, "lm_toe_is_rx_completion(): result is:%s\n", result? "TRUE" : "FALSE");
...
u8_t lm_toe_is_rcq_suspended(lm_device_t *pdev, u8_t drv_toe_rss_id)
{
    ...
    DbgMessage2(pdev, INFORMl4int, "lm_toe_is_rcq_suspended(): sb_idx:%d, result is:%s\n", drv_toe_rss_id, result?"TRUE":"FALSE");
    return result;
}
    ...
    u8_t              dpc)
{
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "###lm_tcp_rx_peninsula_to_rq_copy tcp_buf = 0x%x cid=%d\n", *((u32_t *)&tcp_buf), tcp->cid);
...
u32_t lm_tcp_rx_peninsula_to_rq(lm_device_t * pdev, lm_tcp_state_t * tcp, u32_t max_num_bytes_to_copy, u8_t sb_idx)
{
    ...
    DbgMessage1(pdev, VERBOSEl4rx, "###lm_tcp_rx_peninsula_to_rq cid=%d\n", tcp->cid);
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "lm_tcp_rx_peninsula_to_rq copied %d bytes cid=%d\n", copied_bytes, tcp->cid);
    return copied_bytes;
}
...
void lm_tcp_rx_gen_peninsula_process(lm_device_t * pdev, lm_tcp_state_t * tcp, u32_t nbytes, lm_tcp_gen_buf_t * gen_buf)
{
    ...
    DbgMessage3(pdev, VERBOSEl4rx, "##lm_tcp_rx_gen_peninsula_process, nbytes=%d, cid=%d add=%s\n", nbytes, tcp->cid,
                (gen_buf)? "TRUE"  : "FALSE");    
...
#ifdef DEBUG_OOO_CQE
    SET_DEBUG_OOO_INFO(current_isle, TOE_RX_CQES_OPCODE_GNI, 0);
#endif
...
void lm_tcp_rx_gen_isle_right_process(lm_device_t * pdev, lm_tcp_state_t * tcp, u32_t nbytes, lm_tcp_gen_buf_t * gen_buf, u8_t sb_idx, u8_t isle_num)
{
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "##lm_tcp_rx_gen_isle_process nbytes = %d cid=%d\n", nbytes, tcp->cid);
...
#ifdef DEBUG_OOO_CQE
        SET_DEBUG_OOO_INFO(requested_isle, TOE_RX_CQES_OPCODE_GAIR, nbytes);
    } else {
        SET_DEBUG_OOO_INFO(requested_isle, TOE_RX_CQES_OPCODE_GRI, nbytes);
#endif
    }
}
...
#ifdef DEBUG_OOO_CQE
    SET_DEBUG_OOO_INFO(requested_isle, TOE_RX_CQES_OPCODE_GAIL, nbytes);
#endif
}
...
void lm_tcp_rx_gen_join_process(lm_device_t * pdev, lm_tcp_state_t * tcp, u8_t sb_idx, u8_t isle_num)
{
    ...
    d_list_t                   gen_buf_list;
    u32_t                      isle_nbytes;
    DbgMessage1(pdev, VERBOSEl4rx, "##lm_tcp_rx_gen_join_process cid=%d\n", tcp->cid);
...
#ifdef DEBUG_OOO_CQE
        SET_DEBUG_OOO_INFO(start_isle,TOE_RX_CQES_OPCODE_GJ,0);
#endif
    }
    rx_con->dpc_info.dpc_flags &= ~(LM_TCP_DPC_TOO_BIG_ISLE | LM_TCP_DPC_TOO_MANY_ISLES);    
    ...
    u8_t                sb_idx)
{
    ...
    if(IS_OOO_CQE(cmd)) {
        ...
        if (((isle_num == 0) && (cmd != TOE_RX_CQES_OPCODE_GJ)) || (isle_num > T_TCP_MAX_ISLES_PER_CONNECTION_TOE)) { 
            DbgMessage2(pdev, FATAL, "Isle number %d is not valid for OOO CQE %d\n", isle_num, cmd);
            DbgBreak();
        }
    } else if (cmd == TOE_RX_CQES_OPCODE_UPD_CMP) {
    ...
    switch(cmd) {
    case TOE_RX_CQES_OPCODE_SRC_ERR:
        DbgMessage(pdev, FATAL, "ERROR: NO SEARCHER ENTRY!\n");
        DbgBreakIfAll(TRUE);
        return;
    case TOE_RX_CQES_OPCODE_GA:
        //DbgMessage2(pdev, WARN, "GenericAdd cid=%d nbytes=%d!\n", tcp->cid, cqe->nbytes);
        lm_tcp_rx_gen_peninsula_process(pdev, tcp, nbytes, 
                                        lm_tcp_rx_next_grq_buf(pdev, sb_idx));
        return;
    case TOE_RX_CQES_OPCODE_GNI:
        //DbgMessage2(pdev, WARN, "GenericCreateIsle cid=%d isle_num=%d!\n", tcp->cid, isle_num);
        DbgBreakIf(nbytes);
        lm_tcp_rx_gen_isle_create(pdev, tcp, 
                                  lm_tcp_rx_next_grq_buf(pdev, sb_idx), sb_idx, isle_num);
        return;
    case TOE_RX_CQES_OPCODE_GAIR:
        //DbgMessage3(pdev, WARN, "GenericAddIsleR cid=%d isle_num=%d nbytes=%d!\n", tcp->cid, isle_num, nbytes);
        lm_tcp_rx_gen_isle_right_process(pdev, tcp, nbytes, 
                                   lm_tcp_rx_next_grq_buf(pdev, sb_idx), sb_idx, isle_num);
        return;
    case TOE_RX_CQES_OPCODE_GAIL:
        DbgMessage3(pdev, WARN, "GenericAddIsleL cid=%d isle_num=%d nbytes=%d!\n", tcp->cid, isle_num, nbytes);
        if (nbytes) {
        ...
        return;
    case TOE_RX_CQES_OPCODE_GRI:
//        DbgMessage3(pdev, WARN, "GenericReleaseIsle cid=%d isle_num=%d nbytes=%d!\n", tcp->cid, isle_num, nbytes);
        lm_tcp_rx_gen_isle_right_process(pdev, tcp, nbytes, NULL, sb_idx, isle_num);
        return;
    case TOE_RX_CQES_OPCODE_GR:
        //DbgMessage2(pdev, WARN, "GenericRelease cid=%d nbytes=%d!\n", tcp->cid, cqe->nbytes);
        lm_tcp_rx_gen_peninsula_process(pdev, tcp, nbytes, NULL); 
        return;
    case TOE_RX_CQES_OPCODE_GJ:
        //DbgMessage2(pdev, WARN, "GenericJoin cid=%d nbytes=%d!\n", tcp->cid, cqe->nbytes);
        lm_tcp_rx_gen_join_process(pdev, tcp, sb_idx, isle_num);
        return;
    case TOE_RX_CQES_OPCODE_CMP:
        //DbgMessage2(pdev, WARN, "Cmp(push) cid=%d nbytes=%d!\n", tcp->cid, cqe->nbytes);
        /* Add fast path handler here */
        lm_tcp_rx_cmp_process(pdev, tcp, nbytes, 1);
        return;
    case TOE_RX_CQES_OPCODE_REL:
        //DbgMessage2(pdev, WARN, "Rel(nopush) cid=%d nbytes=%d!\n", tcp->cid, cqe->nbytes);
        lm_tcp_rx_cmp_process(pdev, tcp, nbytes, 0);
        return;
    case TOE_RX_CQES_OPCODE_SKP:
        //DbgMessage2(pdev, WARN, "Skp cid=%d nbytes=%d!\n", tcp->cid, cqe->nbytes);
        lm_tcp_rx_skp_process(pdev, tcp, nbytes, sb_idx);
        return;
    case TOE_RX_CQES_OPCODE_DGI:
        DbgMessage1(pdev, WARN, "Delete Isle cid=%d!\n", tcp->cid);
    ...
    if ((cmd != TOE_RX_CQES_OPCODE_UPD_CMP) && nbytes) {
        lm_tcp_gen_buf_t * gen_buf;
    ...
    case TOE_RX_CQES_OPCODE_UPD_CMP:
        ...
        /*DbgMessage2(pdev, FATAL, "lm_tcp_rx_process_cqe() TOE_RX_CQES_OPCODE_UPD_CMP: IGNORE_WND_UPDATES=%d, cqe->nbytes=%d\n", GET_FLAGS(tcp->rx_con->db_data.rx->flags, TOE_RX_DB_DATA_IGNORE_WND_UPDATES), cqe->nbytes);*/
        ...
        return;
    case TOE_RX_CQES_OPCODE_URG:
        tcp->rx_con->dpc_info.dpc_flags |= LM_TCP_DPC_URG;
        return;
    case TOE_RX_CQES_OPCODE_MAX_RT:
        DbgMessage1(pdev, WARNl4, "lm_tcp_rx_process_cqe: TOE_RX_CQES_OPCODE_MAX_RT cid=%d\n", tcp->cid);
        tcp->rx_con->dpc_info.dpc_flags |= LM_TCP_DPC_RT_TO;
        return;
    case TOE_RX_CQES_OPCODE_RT_TO:
        DbgMessage1(pdev, WARNl4, "lm_tcp_rx_process_cqe: TOE_RX_CQES_OPCODE_RT_TO cid=%d\n", tcp->cid);
        tcp->rx_con->dpc_info.dpc_flags |= LM_TCP_DPC_RT_TO;
        return;
    case TOE_RX_CQES_OPCODE_KA_TO:
        tcp->rx_con->dpc_info.dpc_flags |= LM_TCP_DPC_KA_TO;
        return;
    case TOE_RX_CQES_OPCODE_DBT_RE:
        ...
        DbgMessage1(pdev, INFORMl4, "lm_tcp_rx_process_cqe: RCQE TOE_RX_CQES_OPCODE_DBT_RE, cid=%d\n", tcp->cid);
        DbgMessage1(pdev, WARNl4, "lm_tcp_rx_process_cqe: RCQE TOE_RX_CQES_OPCODE_DBT_RE, cid=%d IGNORING!!!\n", tcp->cid);
    ...
    case TOE_RX_CQES_OPCODE_TRM_CMP:        
        ...
        return;
    case TOE_RX_CQES_OPCODE_DRV_RSV:
        DbgBreakIf(nbytes);
        DbgBreakIf(! tcp->sp_request );
        ...
        tcp->rx_con->dpc_info.dpc_flags |= LM_TCP_DPC_RAMROD_CMP;
        return;
    case TOE_RX_CQES_OPCODE_OFL_CMP:
        DbgBreakIf(nbytes);
        DbgBreakIf(! tcp->sp_request );
        ...
        lm_sq_complete(pdev, CMD_PRIORITY_NORMAL, RAMROD_CMD_ID_TOE_INITIATE_OFFLOAD, tcp->ulp_type, tcp->cid);
        
        return;
    case TOE_RX_CQES_OPCODE_LCN_ERR:
        DbgBreakIf(! tcp->sp_request );
        DbgBreakIf(tcp->sp_request->type != SP_REQUEST_INITIATE_OFFLOAD);
        tcp->rx_con->dpc_info.dpc_flags |= LM_TCP_DPC_RAMROD_CMP;
        return;
    default:
        DbgMessage1(pdev, FATAL, "unexpected rx cqe opcode=%d\n", cmd);
        DbgBreakIfAll(TRUE);
    } 
}
...
u8_t lm_tcp_rx_process_cqes(lm_device_t *pdev, u8_t drv_toe_rss_id, s_list_t * connections)
{
    ...
    DbgMessage1(pdev, VERBOSEl4int , "###lm_tcp_rx_process_cqes num_cqes=%d\n", rcq->num_cqes_last_dpc);
    ...
    if (rcq->suspend_processing == TRUE) {
        ...
        if (rcq->suspend_processing == TRUE) {
            ...
            DbgMessage1(pdev, VERBOSEl4int, "lm_tcp_rx_process_cqes(): rcq suspended - idx:%d\n", drv_toe_rss_id);
        }
    }
    ...
    while(cq_old_idx != cq_new_idx) {
        ...
        if (opcode == TOE_RX_CQES_OPCODE_RSS_UPD) {

            /* update the saved consumer */
            cq_old_idx = lm_bd_chain_cons_idx(&rcq->bd_chain);

            /* rss update ramrod */
            DbgMessage1(pdev, WARNl4int, "lm_tcp_rx_process_cqes(): calling lm_tcp_rss_update_ramrod_comp - drv_toe_rss_id:%d\n", drv_toe_rss_id);
        ...
        if (cid < MAX_ETH_CONS) {
            ...
            DbgBreakIf(((cqe->params1 & TOE_RX_CQE_COMPLETION_OPCODE) >> TOE_RX_CQE_COMPLETION_OPCODE_SHIFT) 
                       != TOE_RX_CQES_OPCODE_TOE_START_CMP);
        ...
        *hist_cqe = *cqe;
        ...
        if (defer_cqe) {
            ...
            if (defer_cqe) {
                ...
                if (((cqe->params1 & TOE_RX_CQE_COMPLETION_OPCODE) >> TOE_RX_CQE_COMPLETION_OPCODE_SHIFT) == TOE_RX_CQES_OPCODE_OFL_CMP)
                {
    ...
    )
{
    ...
    DbgMessage1(pdev, VERBOSEl4rx, "###lm_tcp_rx_post_buf cid=%d\n", tcp->cid);
    DbgBreakIf(!(pdev && tcp)); 
...
//        DbgBreakIf(!tcp_buf); /* (lm_tcp_rx_post_buf design guides VBD doc) */
    ...
    if (tcp_buf) {
        ...
        if(lm_bd_chain_avail_bds(&rx_con->bd_chain) < frag_list->cnt) {
            ...
            DbgMessage3(pdev, INFORMl4rx, "post rx buf failed, rx chain is full (cid=%d, avail bds=%d, buf nfrags=%d)\n", 
    ...
    if ( gen_info->peninsula_nbytes ) {
        DbgMessage5(pdev, VERBOSEl4rx, "###lm_tcp_rx_post_buf WITH GENERIC, cid=%d, tcp_buf=%p, buf_size=%d, buf_flags=%d, peninsula_nbytes=%d\n",
        ...
        DbgMessage5(pdev, VERBOSEl4rx, "###lm_tcp_rx_post_buf NO COPY, cid=%d, tcp_buf=%p, buf_size=%d, buf_flags=%d, peninsula_nbytes=%d\n",
    ...
    )
{
    ...
    if(tcp_buf->flags & TCP_BUF_FLAG_L4_POST_END) {
        ...
        DbgMessage1(NULL, VERBOSEl4rx, "Setting Rx last BD flags=0x%x\n", rx_bd->flags);
    }
    ...
    )
{
    ...
    DbgMessage1(pdev, VERBOSEl4rx, "###_lm_tcp_rx_get_buffered_data cid=%d\n", tcp->cid);
    ...
    if (num_bufs_to_indicate > (*frag_list)->cnt) {
        DbgMessage3(pdev, WARNl4rx, "_lm_tcp_rx_get_buffered_data: number of buffers to indicate[%d] is larger than frag_cnt[%d] cid=%d\n",
    ...
    *gen_buf = head_of_indication;
    DbgMessage1(pdev, VERBOSEl4rx, "###_lm_tcp_rx_get_buffered_data ind_bytes = %d\n", (*frag_list)->size);
    ...
    )
{
    ...
    DbgMessage1(pdev, VERBOSEl4rx, "###lm_tcp_rx_get_buffered_data_from_terminate cid=%d\n", tcp->cid);
...
/*  DbgBreakIf(gen_info->peninsula_nbytes > tcp->tcp_cached.initial_rcv_wnd);*/
    ...
    if ((buff_cnt = (u16_t)d_list_entry_cnt(&gen_info->peninsula_list)) > gen_info->max_frag_count) {
        ...
        DbgMessage3(pdev, WARNl4rx | WARNl4sp,
    ...
    )
{
    ...
    DbgMessage1(pdev, VERBOSEl4rx, "###lm_tcp_rx_get_buffered_data cid=%d\n", tcp->cid);
    gen_info = &rx_con->u.rx.gen_info;
    ...
    )
{
    ...
    DbgMessage2(pdev, VERBOSEl4rx, "###lm_tcp_rx_buffered_data_indicated accepted_bytes = %d cid=%d\n", accepted_bytes, tcp->cid);
    ...
    if (gen_buf == NULL) { /* succesfull indication */
        ...
        DbgMessage3(pdev, INFORMl4rx, "GENERIC: %s Indication for cid=%d accepted_bytes=%d\n",
...
{
    DbgMessage1(pdev, VERBOSEl4rx, "###lm_tcp_rx_con_indication_returned cid=%d\n", tcp->cid);
    ...
    *unwanted_gen_buf = NULL;   
    ...
    if ((adjust_number * gen_buff_size) >= gen_info->peninsula_nbytes) {
        ...
        *unwanted_gen_buf = (lm_tcp_gen_buf_t*)d_list_peek_head(&unwanted_list);
        DbgMessage5(pdev, WARNl4rx | WARNl4sp,
        ...
        DbgMessage7(pdev, WARNl4rx | WARNl4sp,

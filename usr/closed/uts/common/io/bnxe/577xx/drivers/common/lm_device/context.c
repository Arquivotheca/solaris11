...
lm_status_t lm_setup_context_pool(struct _lm_device_t *pdev)
{
    ...
    context->proto_end  [ETH_CONNECTION_TYPE]   = MAX_VF_ETH_CONS + MAX_ETH_CONS - 1;
...
lm_status_t lm_allocate_cid(struct _lm_device_t *pdev, u32_t type, void * cookie, s32_t * out_cid){
    ...
    {
        DbgBreakIfAll(!out_cid) ;
        DbgBreakIfAll(!pdev);
        DbgBreakIfAll(!pdev->context_info);
        DbgBreakIfAll(!pdev->context_info->array);
        DbgBreakIfAll(!cookie);
        DbgBreakIfAll(type >= ARRSIZE(pdev->context_info->proto_pending)) ;
        return LM_STATUS_INVALID_PARAMETER ;
    }
    ...
    *out_cid = 0;
    ...
    if(LM_STATUS_SUCCESS == lm_status)
    {
        ...
        {
            DbgMessage2(pdev, FATAL, "lm_allocate_cid: mm_map_io_space failed. address low=%d address high=%d\n", phy_addr.as_u32.low,phy_addr.as_u32.high );
...
void lm_cfc_delete(struct _lm_device_t *pdev, void *param)
{
    ...
    {
        DbgBreakIfAll(!pdev);
        DbgBreakIfAll(cid > pdev->params.max_func_connections);
        if (!flr_in_progress) {
        DbgBreakIfAll(pdev->context_info->array[cid].invalid != LM_CONTEXT_INVALID_WAIT);
        }
        else
        {
            DbgMessage2(pdev, FATAL,"lm_cfc_delete: invalid %d for cid=%d\n", pdev->context_info->array[cid].invalid,cid);
    ...
    DbgMessage1(pdev, WARN, "lm_cfc_delete: cid=0x%x\n",cid);
...
void lm_free_cid(struct _lm_device_t *pdev, u32_t type, u32_t cid, u8_t notify_fw){
         ...
         (!lm_fl_reset_is_inprogress(pdev) && (pdev->context_info->array[cid].invalid != LM_CONTEXT_VALID)))
    {
        DbgBreakIfAll(!pdev);
        DbgBreakIfAll(!pdev->context_info);
        DbgBreakIfAll(type >= ARRSIZE(pdev->context_info->proto_end));
        DbgBreakIfAll(cid > (pdev->context_info->proto_end[type]));
        DbgBreakIfAll(cid < (pdev->context_info->proto_start[type]));
        DbgBreakIfAll(pdev->context_info->array[cid].invalid != LM_CONTEXT_VALID);
        return;
    }
    MM_ACQUIRE_CID_LOCK(pdev);
    ...
    {
        DbgBreakIfAll(pdev->context_info->array[cid].cid_resc.cookies[proto_idx]);
    }
    ...
    else
    {
        if (type == TOE_CONNECTION_TYPE) {
            DbgMessage1(pdev, WARN, "lm_free_cid: CFC delete: cid=0x%x\n",cid);
            lm_cfc_delete(pdev,*((void **)&cid));
        }
        else
        {
            DbgMessage1(pdev, WARN, "lm_free_cid: schedule CFC delete: cid=0x%x\n",cid);
...
void lm_recycle_cid(struct _lm_device_t *pdev, u32_t cid){
    ...
    {
        DbgBreakIfAll(!pdev);
        DbgBreakIfAll(pdev->context_info->array[cid].invalid != LM_CONTEXT_INVALID_DELETE);
        DbgBreakIfAll(cid > pdev->params.max_func_connections);
        return;
    }
    ...
    MM_ACQUIRE_CID_LOCK(pdev);
#ifdef _VBD_
    if ((type == TOE_CONNECTION_TYPE) && (pdev->toe_info.l4_params.ticks_per_second != 0)) {
        pdev->vars.last_recycling_timestamp = mm_get_current_time(pdev) * 1000 / pdev->toe_info.l4_params.ticks_per_second; /*time in ms*/
    }
#endif
...
void * lm_cid_cookie(struct _lm_device_t *pdev, u32_t type, u32_t cid){
         ...
         ERR_IF(pdev->context_info->array[cid].invalid != LM_CONTEXT_VALID) )
    {
        DbgBreakIfAll(!pdev);
        DbgBreakIfAll(!pdev->context_info);
        DbgBreakIfAll(type >= MAX_PROTO);
        DbgBreakIfAll(cid > (pdev->context_info->proto_end[MAX_PROTO - 1]));
        DbgBreakIfAll(pdev->context_info->array[cid].invalid != LM_CONTEXT_VALID);
    }
...
void lm_init_connection_context(struct _lm_device_t *pdev, u32_t const sw_cid, u8_t sb_id)
{
    ...
    {
        DbgBreakIfAll(!pdev);
        DbgBreakIfAll(sw_cid < PFDEV(pdev)->context_info->proto_start[ETH_CONNECTION_TYPE]); /* first legal NIC CID */
        DbgBreakIfAll(sw_cid > PFDEV(pdev)->context_info->proto_end[ETH_CONNECTION_TYPE]);   /* last legal NIC CID */
    }

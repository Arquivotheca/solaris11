...
#define USER_OPTION_NUM_RINGS_MIN      1
#define USER_OPTION_NUM_RINGS_MAX      MAX_RSS_CHAINS
#define USER_OPTION_NUM_RINGS_DEFAULT  4
...
#define BNXE_FCOE(dev)                                      \
    ((dev)->devParams.fcoeEnable                        &&  \
     ((CHIP_NUM((lm_device_t *)(dev)) == CHIP_NUM_5711)  || \
      (CHIP_NUM((lm_device_t *)(dev)) == CHIP_NUM_5711E) || \
      (CHIP_NUM((lm_device_t *)(dev)) == CHIP_NUM_5712)  || \
      (CHIP_NUM((lm_device_t *)(dev)) == CHIP_NUM_5712E)))
...
typedef struct _um_device
{
    ...
    kmutex_t              mcpMutex;
    kmutex_t              phyMutex;
    kmutex_t              dmaeStatsMutex;
    kmutex_t              dmaeMiscMutex;
    kmutex_t              indMutex;
    kmutex_t              cidMutex;
    ...
    mac_handle_t          pMac;
    mac_resource_handle_t macRxResourceHandles[MAX_RX_CHAIN];
    u8_t                  gldMac[ETHERNET_ADDRESS_SIZE];

    u8_t                  mcastTable[LM_MAX_MC_TABLE_SIZE * ETHERNET_ADDRESS_SIZE];
    u32_t                 mcastTableCnt[LM_CLI_IDX_MAX]; /* number of mcast addrs in the table */
    u8_t                  mcastTableBaseIdx[LM_CLI_IDX_MAX];
...
#define BNXE_LOCK_ENTER_DMAE_STATS(pUM)     mutex_enter(&(pUM)->dmaeStatsMutex)
#define BNXE_LOCK_EXIT_DMAE_STATS(pUM)      mutex_exit(&(pUM)->dmaeStatsMutex)
#define BNXE_LOCK_ENTER_DMAE_MISC(pUM)      mutex_enter(&(pUM)->dmaeMiscMutex)
#define BNXE_LOCK_EXIT_DMAE_MISC(pUM)       mutex_exit(&(pUM)->dmaeMiscMutex)
...
void BNXE_LOCK_ENTER_DMAE_STATS(um_device_t * pUM);
void BNXE_LOCK_EXIT_DMAE_STATS(um_device_t * pUM);
void BNXE_LOCK_ENTER_DMAE_MISC(um_device_t * pUM);
void BNXE_LOCK_EXIT_DMAE_MISC(um_device_t * pUM);
...
static inline boolean_t BnxeIsClientBound(um_device_t * pUM)
{
    return (CLIENT_HW(pUM, LM_CLI_IDX_NDIS) ||
            CLIENT_BOUND(pUM, LM_CLI_IDX_FCOE));
}

static inline char * BnxeClientsHw(um_device_t * pUM)
{
...
static inline char * BnxeClientsDevi(um_device_t * pUM)
{
    if (CLIENT_DEVI(pUM, LM_CLI_IDX_FCOE)) { return "FCoE"; }
    else                                   { return "None"; }
}

static inline char * BnxeClientsBound(um_device_t * pUM)
{
    if (CLIENT_HW(pUM, LM_CLI_IDX_NDIS) &&

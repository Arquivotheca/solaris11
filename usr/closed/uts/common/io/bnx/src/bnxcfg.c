...
#define USER_OPTION_TX_DESC_CNT_DEFAULT		(1024 - (1024 % MAX_BD_PER_PAGE))
...
bnx_cfg_init( um_device_t * const umdevice )
{
	...
	{
		lmdevice->params.tx_quick_cons_trip = USER_OPTION_TXFRAMES_DEFAULT;
	}
	...
	{
		lmdevice->params.tx_quick_cons_trip_int = USER_OPTION_TXFRAMES_INT_DEFAULT;
	}
	...
	{
		lmdevice->params.rx_quick_cons_trip = USER_OPTION_RXFRAMES_DEFAULT;
	}
	...
	{
		lmdevice->params.rx_quick_cons_trip_int = USER_OPTION_RXFRAMES_INT_DEFAULT;
	}
    ...
    lmdevice->params.mtu = umdevice->dev_var.mtu + sizeof(struct ether_header) + VLAN_TAGSZ;
	...
	bnx_cfg_readint( umdevice->os_param.dip,
	                 USER_OPTION_KEYWORD_RX_DCOPY, &option );
	if(option)
	{
...
bnx_cfg_reset( um_device_t * const umdevice )
{
	...
	bcopy( &(umdevice->hwinit),
	       &(umdevice->curcfg),
	       sizeof(bnx_phy_cfg_t) );
} /* bnx_cfg_reset */
...
bnx_cfg_map_serdes( um_device_t * const umdevice )
{
	...
	if( umdevice->curcfg.lnkcfg.link_autoneg )
	{
		...
		{
			lmdevice->params.selective_autoneg = SELECTIVE_AUTONEG_SINGLE_SPEED;
		...
		/* Enable serdes fallback for all but one particular HP platform. */
		    ...
		    lmdevice->hw_info.ssid == 0x310c) )
		{
			if( umdevice->curcfg.lnkcfg.param_2500fdx )
			{
				lmmedium |= LM_MEDIUM_SPEED_AUTONEG_2_5G_FALLBACK;
			}
			else
			{
	...
	if( umdevice->curcfg.lnkcfg.link_autoneg )
	{
		...
		else
		{
			...
			else if( umdevice->curcfg.lnkcfg.param_1000hdx )
			{
				...
				if( umdevice->curcfg.lnkcfg.param_100fdx == B_TRUE &&
				    umdevice->curcfg.lnkcfg.param_100hdx == B_TRUE &&
				    umdevice->curcfg.lnkcfg.param_10fdx  == B_TRUE &&
				    umdevice->curcfg.lnkcfg.param_10hdx  == B_TRUE )
				{
					lmdevice->params.selective_autoneg = SELECTIVE_AUTONEG_ENABLE_SLOWER_SPEEDS;
				}
			}
			else if( umdevice->curcfg.lnkcfg.param_100fdx )
			{
				...
				if( umdevice->curcfg.lnkcfg.param_100hdx == B_TRUE &&
				    umdevice->curcfg.lnkcfg.param_10fdx  == B_TRUE &&
				    umdevice->curcfg.lnkcfg.param_10hdx  == B_TRUE )
				{
					lmdevice->params.selective_autoneg = SELECTIVE_AUTONEG_ENABLE_SLOWER_SPEEDS;
			...
			else if( umdevice->curcfg.lnkcfg.param_100hdx )
			{
				...
				if( umdevice->curcfg.lnkcfg.param_10fdx == B_TRUE &&
				    umdevice->curcfg.lnkcfg.param_10hdx == B_TRUE )
				{
					lmdevice->params.selective_autoneg = SELECTIVE_AUTONEG_ENABLE_SLOWER_SPEEDS;
				}
			...
			else if( umdevice->curcfg.lnkcfg.param_10fdx )
			{
				...
				if( umdevice->curcfg.lnkcfg.param_10hdx == B_TRUE )
				{
					lmdevice->params.selective_autoneg = SELECTIVE_AUTONEG_ENABLE_SLOWER_SPEEDS;
				}
			}

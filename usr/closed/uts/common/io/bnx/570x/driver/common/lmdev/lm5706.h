...
#define REG_RD(_pdev, _name, _ret) \
    mm_read_barrier(); \
    *(_ret) = readl(&((_pdev)->vars.regview->_name)); \
    LOG_REG_RD( \
        _pdev, \
...
#define REG_WR(_pdev, _name, _val) \
    ...
    writel((_val), &((_pdev)->vars.regview->_name)); \
    mm_write_barrier()
...
#define REG_RD_OFFSET(_pdev, _offset, _ret) \
    mm_read_barrier(); \
    *(_ret) = readl((volatile u32_t *) ((u8_t *) (_pdev)->vars.regview + (_offset))); \
    LOG_REG_RD( \
        _pdev, \
        _offset, \
...
#define REG_WR_OFFSET(_pdev, _offset, _val) \
    ...
    writel((_val), (volatile u32_t *) ((u8_t *) (_pdev)->vars.regview + (_offset))); \
    mm_write_barrier()
...
#define MBQ_WR32(_pdev, _cid, _offset, _val) \
    ...
    writel((_val), (volatile u32_t *) ((u8_t *) (_pdev)->vars.regview + \
...
#define MBQ_WR16(_pdev, _cid, _offset, _val) \
    ...
    writew((_val), (volatile u16_t *) ((u8_t *) (_pdev)->vars.regview + \
                    MB_GET_CID_ADDR(_pdev, _cid) + (_offset))); \
    mm_write_barrier(); \
    if(CHIP_REV(_pdev) == CHIP_REV_IKOS) \
    { \
...
#define MBQ_WR8(_pdev, _cid, _offset, _val) \
    ...
    writeb((_val), (volatile u8_t *) ((u8_t *) (_pdev)->vars.regview + \
                    MB_GET_CID_ADDR(_pdev, _cid) + (_offset))); \
    mm_write_barrier(); \
    if(CHIP_REV(_pdev) == CHIP_REV_IKOS) \
    { \
...
#define REG_RD(_pdev, _name, _ret)                                                                \
    *(_ret) = ddi_get32((_pdev)->vars.dmaRegAccHandle,                                            \
                        (u32_t *)((u8_t *)(_pdev)->vars.regview + OFFSETOF(reg_space_t, _name))); \

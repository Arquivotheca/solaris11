    ...
    u32_t wait_link_timeout_us)
{
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, PHY_CTRL_REG, PHY_CTRL_PHY_RESET);
    for(cnt = 0; cnt < 1000; cnt++)
    {
        mm_wait(pdev, 5);
        
        lm_mread(pdev, pdev->params.phy_addr, PHY_CTRL_REG, &val);
    ...
    /* Get the PHY id. */
    lm_mread(pdev, pdev->params.phy_addr, PHY_ID1_REG, &val);
    pdev->hw_info.phy_id = val << 16;
    DbgMessage1(pdev, INFORM, "Phy Id1 0x%x\n", val);

    lm_mread(pdev, pdev->params.phy_addr, PHY_ID2_REG, &val);
    ...
    if(CHIP_REV(pdev) == CHIP_REV_FPGA)
    {
        ...
        lm_mwrite(pdev, pdev->params.phy_addr, 28, 0x3044);
        lm_mwrite(pdev, pdev->params.phy_addr, 29, 0x0100);
    }
    else
    {
        if(CHIP_NUM(pdev) == CHIP_NUM_5706 || CHIP_NUM(pdev) == CHIP_NUM_5708)
        {
            ...
            lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x0c00);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x17, 0x000a);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x15, 0x310b);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x17, 0x201f);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x15, 0x9506);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x17, 0x401f);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x15, 0x14e2);
            lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x0400);
        }

        /* Enable/Disable Ethernet@WireSpeed. */
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x7007);
        lm_mread(pdev, pdev->params.phy_addr, 0x18, &val);
        ...
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, val | 0x8000 | 0x7);
        ...
        lm_mread(pdev, pdev->params.phy_addr, 0x18, &val);
        val |= BIT_9; /*auto mdix*/
        lm_mwrite(pdev, pdev->params.phy_addr, BCM5401_AUX_CTRL, val | 0x8000 | 0x7);
    ...
    {
        lm_mwrite(pdev, pdev->params.phy_addr, 0x17, 0xf08);
        lm_mread(pdev, pdev->params.phy_addr, 0x15, &val);
        val &= ~0x100;
        lm_mwrite(pdev, pdev->params.phy_addr, 0x15, val);
    }

    /* Configure the PHY for jumbo frame. */
    if(pdev->params.mtu > MAX_ETHERNET_PACKET_SIZE)
    {
        lm_mwrite(pdev, pdev->params.phy_addr, 0x10, 0x0001);
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x4400);
    }
    else
    {
        lm_mwrite(pdev, pdev->params.phy_addr, 0x10, 0x0000);
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x0400);
    }
    ...
    switch(speed)
    {
        case LM_MEDIUM_SPEED_10MBPS:
             ...
             * 10/100 Mb PHY. */
            lm_mwrite(pdev, pdev->params.phy_addr, PHY_1000BASET_CTRL_REG, 0); 
            ...
            lm_mwrite(pdev, pdev->params.phy_addr, PHY_AN_AD_REG, val);
        ...
        case LM_MEDIUM_SPEED_1000MBPS:
            ...
            else
            {
                ...
                lm_mwrite(pdev, pdev->params.phy_addr, BCM5401_AUX_CTRL, 0x7);
                lm_mread(pdev, pdev->params.phy_addr, BCM5401_AUX_CTRL, &val);
                val |= BCM5401_SHDW_NORMAL_EXTERNAL_LOOPBACK;
                lm_mwrite(pdev, pdev->params.phy_addr, BCM5401_AUX_CTRL, val);

                val = PHY_CONFIG_AS_MASTER | PHY_ENABLE_CONFIG_AS_MASTER;
            }

            lm_mwrite(pdev, pdev->params.phy_addr, PHY_1000BASET_CTRL_REG, val);
            break;
    ...
    /* Wait for link. */
    lm_mread(pdev, pdev->params.phy_addr, PHY_STATUS_REG, &val);

    if(CHIP_REV(pdev) != CHIP_REV_FPGA)
    {
        /* Wait for link only if the cable is connected. */
        lm_mwrite(pdev, pdev->params.phy_addr, 0x1c, 0x7c00);
        lm_mread(pdev, pdev->params.phy_addr, 0x1c, &val);
        if(val & 0x20)
        {
            for(; ;)
            {
    ...
    u32_t wait_link_timeout_us)
{
    ...
    {
        set_5708_serdes_pre_emphasis(pdev, pdev->params.serdes_pre_emphasis);
    }
    ...
    lm_medium_t duplex)
{
    ...
    lm_mread(pdev, pdev->params.phy_addr, 0x10, &val);
    val &= ~0x10;
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, 0x10, val);

    /* select combo ieee0 block. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0xffe0);
    ...
    if(speed == LM_MEDIUM_SPEED_2500MBPS)
    {
        ...
        val &= 0xfff0;
        val |= 0x10;
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, val);
    }
} /* force_5709_serdes_link */
    ...
    u32_t selective_autoneg)
{
    ...
    {
        /* disable 2.5g adv */
        lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0x8320);
        lm_mread(pdev, pdev->params.phy_addr, 0x19, &val);
		val &= ~1;
        lm_mwrite(pdev, pdev->params.phy_addr, 0x19, val);
    }
    ...
    if(selective_autoneg)
    {
        lm_mread(pdev, pdev->params.phy_addr, 0x11, &val);
        val &= ~0x1;
        lm_mwrite(pdev, pdev->params.phy_addr, 0x11, val);
    }

    /* select bam next page block. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0x8350);
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0x8370);

    /* enable bam_en, bam_station_mngr_en, bam_np_after_bp_en. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x12, 0xe000);

    /* select ieee1 block. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0x10);

    /* advertise 1000kx. */
    lm_mread(pdev, pdev->params.phy_addr, 0x1, &val);
    val |= 0x20;
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1, val);

    /* select ieee0 block. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0);

    /* enable cl73 aneg. */
    lm_mread(pdev, pdev->params.phy_addr, 0x0, &val);
    val |= 0x1200;
    lm_mwrite(pdev, pdev->params.phy_addr, 0x0, val);
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, 0x14, val);
    ...
    u32_t wait_link_timeout_us)
{
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0xffd0);
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1e, 0x3800);

    /* select combo_ieee0 block. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0xffe0);

    /* reset. */
    lm_mwrite(pdev, pdev->params.phy_addr, 0x10, MII_CTRL_RESET);
    ...
    /* get phy id. */
    lm_mread(pdev, pdev->params.phy_addr, 0x12, &val);
    pdev->hw_info.phy_id = val << 16;
    lm_mread(pdev, pdev->params.phy_addr, 0x13, &val);
    pdev->hw_info.phy_id |= val & 0xffff;
    ...
    u32_t wait_link_timeout_us)
{
    ...
    if(speed == LM_MEDIUM_SPEED_1000MBPS)
    {
        ...
        lm_mwrite(pdev, pdev->params.phy_addr, PHY_CTRL_REG, val);
    }
    else
    {
    ...
    {
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x4400);
        lm_mwrite(pdev, pdev->params.phy_addr, 0x1c, 0xec87);
    }
    else
    {
        lm_mwrite(pdev, pdev->params.phy_addr, 0x18, 0x0400);
        lm_mwrite(pdev, pdev->params.phy_addr, 0x1c, 0xec85);
    }
    ...
    lm_device_t *pdev)
{
    ...
    /* Set the phy into loopback mode. */ 
    lm_mwrite(pdev, pdev->params.phy_addr, 0x10, 0x5140);
    ...
    lm_link_settings_t *rphy_link)
{
    ...
    lm_submit_fw_cmd(pdev, DRV_MSG_CODE_CMD_SET_LINK);
    ...
    u32_t *remote_phy_ad)
{
    ...
    *remote_phy_ad = 0;
    ...
    if(CHIP_NUM(pdev) == CHIP_NUM_5706 || CHIP_NUM(pdev) == CHIP_NUM_5708)
    {
        lm_mread(pdev, pdev->params.phy_addr, PHY_AN_AD_REG, &val);
        ...
        lm_mread(pdev,pdev->params.phy_addr,PHY_LINK_PARTNER_ABILITY_REG,&val);
    ...
    else
    {
        ...
        /* local advertisement. */
        lm_mread(pdev, pdev->params.phy_addr, 0x14, &val);
    ...
    lm_medium_t *medium)
{
    ...
    *medium = LM_MEDIUM_TYPE_UTP |
    ...
    lm_mread(pdev, pdev->params.phy_addr, PHY_STATUS_REG, &phy_status);
    lm_mread(pdev, pdev->params.phy_addr, PHY_STATUS_REG, &phy_status);
    ...
    lm_mread(pdev, pdev->params.phy_addr, PHY_CTRL_REG, &phy_ctrl);
    ...
    u8_t fallback_select)
{
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, 0x17, 0x0f01);
    lm_mread(pdev, pdev->params.phy_addr, 0x15, &intr_exp_status);
    lm_mread(pdev, pdev->params.phy_addr, 0x15, &intr_exp_status);
    ...
    lm_medium_t *medium)
{
    ...
    *medium = LM_MEDIUM_TYPE_FIBER |
    ...
    lm_mwrite(pdev, pdev->params.phy_addr, 0x1f, 0x8120);

    /* get status. */
    lm_mread(pdev, pdev->params.phy_addr, 0x1b, &val);
    lm_mread(pdev, pdev->params.phy_addr, 0x1b, &val);  /* is this needed? */
    ...
    lm_medium_t *medium)
{
    ...
    *medium = LM_MEDIUM_TYPE_FIBER;
    ...
    {
        lm_mwrite(pdev, pdev->params.phy_addr, 0x17, 0x0f03);

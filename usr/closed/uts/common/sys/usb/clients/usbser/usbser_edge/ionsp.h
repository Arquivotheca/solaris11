...
#ifndef _SYS_USB_USBSER_EDGE_IONSP_H
#define	_SYS_USB_USBSER_EDGE_IONSP_H


/*
 * Definitions for I/O Networks Serial Protocol
 *
 * Copyright (c) 1997-1998 Digi International, Inc., Inside Out
 ...
 * Interrupt pipe
 */
typedef struct intr_status_pkt {
	uint16_t	RxBytesAvail;	/* Additional bytes available to */
					/* be read from Bulk IN pipe */
	uint16_t	TxCredits[MAX_RS232_PORTS]; /* Additional space */
					/* available in given port's TxBuffer */
} intr_status_pkt_t;

#define	INTR_STATUS_PKT_SIZE(NumPorts)	\
		(sizeof (uint16_t) + (sizeof (uint16_t) * (NumPorts)))
...
#define	IOSP_PORT_MASK			0x07	/* port number mask */
#define	IOSP_CMD_STAT_BIT		0x80	/* if set, this is */
						/* command/status header */

#define	IS_CMD_STAT_HDR(byte)		((byte) & IOSP_CMD_STAT_BIT)
#define	IS_DATA_HDR(byte)		(!IS_CMD_STAT_HDR(byte))

#define	IOSP_GET_HDR_PORT(byte)		((uint8_t)((byte) & IOSP_PORT_MASK))

#define	IOSP_GET_HDR_DATA_LEN(byte1, byte2)	\
		((uint16_t)(((uint16_t)((byte1) & 0x78)) << 5) | (byte2))

#define	IOSP_GET_STATUS_CODE(byte)	((uint8_t)(((byte) &  0x78) >> 3))

/*
 * These macros build the 1st and 2nd bytes for a data header
 */
#define	IOSP_BUILD_DATA_HDR1(port, len)	\
	((uint8_t)(((port) | ((uint8_t)(((uint16_t)(len)) >> 5) & 0x78))))

#define	IOSP_BUILD_DATA_HDR2(port, len)	((uint8_t)(len))

/*
 * These macros build the 1st and 2nd bytes for a command header
 */
#define	IOSP_BUILD_CMD_HDR1(port, cmd)	\
	((uint8_t)(IOSP_CMD_STAT_BIT | (port) | ((uint8_t)((cmd) << 3))))
 ...
 *
 *		0E-0F	N-byte commands:
 *				P1 = num bytes after P1 (ie, TotalLen - 2)
...
#define	IOSP_CMD_CLOSE_PORT	0x01	/* Disable ints, flush buffers */
#define	IOSP_CMD_CHASE_PORT	0x02	/* Wait for TX buffers to empty */
...
#define	IOSP_CMD_RX_CHECK_REQ	0x07	/* Request Edgeport to insert */
				/* a Checkpoint into the receive data stream */
				/* (Parameter = 1 byte sequence number) */
 ...
 */
#define	MAKE_CMD_WRITE_REG(port, reg)		\
		IOSP_BUILD_CMD_HDR1((port), IOSP_WRITE_UART_REG(reg))
 ...
 */
#define	IOSP_RX_FLOW_RTS		0x01	/* Edgeport drops RTS to stop */
						/* incoming data */
#define	IOSP_RX_FLOW_DTR		0x02	/* Edgeport drops DTR to stop */
						/* incoming data */
#define	IOSP_RX_FLOW_DSR_SENSITIVITY	0x04	/* Ignores Rx data unless DSR */
						/* high. Not currently */
						/* implemented by firmware. */
#define	IOSP_RX_FLOW_XON_XOFF		0x08	/* Edgeport sends XOFF char */
					/* to stop incoming data. Host must */
					/* have previously programmed the */
					/* XON/XOFF values with SET_XON/ */
					/* /SET_XOFF before enabling this bit */
...
#define	IOSP_TX_FLOW_XON_XOFF	0x08	/* Edgeport stops Tx upon */
				/* receiving XOFF char */
...
#define	IOSP_TX_FLOW_XOFF_CONTINUE 0x10	/* If not set, Edgeport stops Tx when */
				...
				/* systems that interpret the next received */
				/* char as XON. If set, Edgeport continues Tx */
...
#define	IOSP_TX_TOGGLE_RTS	0x20	/* Edgeport drives RTS as a true */
				/* half-duplex Request-to-Send signal: */
				/* it is raised before beginning transmission */
				/* and lowered after the last Tx char leaves */
				/* the UART. */
				/* Not currently implemented by firmware. */
 ...
 *
 * ssss:	00-07	2-byte status. ssss is UART register 0-7 that
 *			has changed value, and the new value is in P1.
 *			See 16654.h for definitions of UART register numbers
 *			and contents.
 *
 *		08-0B	3-byte status:
 *						=== P1 ===	=== P2 ===
 ...
 *		0B	available for expansion
 *
 *		0C-0D	4-byte status. P1 = extended status code and
 *			P2,P3 = params. Not currently implemented.
 *
 *		0E-0F	N-byte status:
 *			P1 = num bytes after P1 (ie, TotalLen - 2)
 ...
 */
#define	IOSP_STATUS_LSR_DATA	0x08	/* P1 is new value of LSR register */
					/* (same as STATUS_LSR). P2 is */
					/* errored character read from RxFIFO */
					/* after LSR reported an error */
 ...
 */
#define	IOSP_EXT_STATUS_CHASE_RSP	0	/* Reply to CHASE_PORT cmd */
						/* P2 is outcome: */
...
#define	IOSP_EXT_STATUS_RX_CHECK_RSP	1	/* Reply to RX_CHECK cmd */
						/* P2 is sequence number */
 ...
 */
#define	IOSP_GET_STATUS_LEN(code)	\
				((code) < 8 ? 2 : ((code) < 0x0A ? 3 : 4))
...
#endif	/* _SYS_USB_USBSER_EDGE_IONSP_H */

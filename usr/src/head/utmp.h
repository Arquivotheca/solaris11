...
/*
 * This byte in the ut_id character array is used to generate wildcard
 * IDs.
 */
#define	_UTMP_ID_WILDCARD	((char)0xff)
#define	_UTMP_ID_LEN		4
...
struct utmp {
	...
	char ut_id[_UTMP_ID_LEN]; 	/* /etc/inittab id(usually line #) */
...
struct futmp {
	char ut_user[8];		/* User login name */
	char ut_id[_UTMP_ID_LEN];	/* /etc/inittab id */
	char ut_line[12];		/* device name (console, lnxx) */

...
#define	META_MNSET_NODELIST	_PATH_SYSVOL "/nodelist"
...
#define	MD_IM_DISK_TRANSPLANT		0x02

/* Mask to use to determine disk availability */
#define	MD_IM_DISK_AVAIL_MASK		0x01

/* Macros for disk availability checking */
#define	MD_IS_DISK(m, v)	\
	(((m) & MD_IM_DISK_AVAIL_MASK) == (v))

#define	MD_IS_DISK_AVAILABLE(m)	\
	MD_IS_DISK((m), MD_IM_DISK_AVAILABLE)

#define	MD_IS_DISK_UNAVAILABLE(m)	\
	MD_IS_DISK((m), MD_IM_DISK_NOT_AVAILABLE)

/* Values for set descriptor flags */
#define	MD_IM_SET_INVALID	0x10
#define	MD_IM_SET_REPLICATED	0x20
#define	MD_IM_SET_MULTINODE	0x40
...
extern	int		meta_check_setname(char *setname, md_error_t *ep);
...
extern	int		clnt_mn_sp_update_abr(char *hostname, set_t setno,
			    md_error_t *ep);
extern	int		meta_sp_get_by_component(mdsetname_t *sp,
			    mdname_t *compnp, mdnamelist_t **nlpp,
			    int force, md_error_t *ep);
...
extern	void		metaflushsetno(set_t setno);
...
extern	int		meta_smf_isdisabled(char *svc_name);
...
extern	int		meta_upd_ctdnames(mdsetname_t **sp, set_t setno,
			    side_t sideno, int mnset, mddrivename_t *dnp,
			    char **newname, md_error_t *ep);
extern  int		pathname_reload(mdsetname_t **sp, set_t setno,
			    md_error_t *ep);
extern	int		meta_update_devtree(minor_t mnum);

/* meta_mn_changelog.c */
extern	int		mdmn_update_changelog(set_t setno, md_error_t *ep);
...
extern	int			clnt_imp_adddrvs(char *hostname,
				    ...
				    ulong_t genid, md_error_t *ep);
extern	void			meta_ctds_to_dids(md_im_names_t *);

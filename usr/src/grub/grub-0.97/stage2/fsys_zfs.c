...
static int zio_read_common(blkptr_t *bp, dva_t *dva, void *buf, char *stack);
 ...
 */
static int
zio_read_gang_data(blkptr_t *bp, void *buf, char *stack)
{
	int i;

	/* pick a good dva from the block pointer */
	for (i = 0; i < BP_GET_NDVAS(bp); i++) {
		if (zio_read_common(bp, &bp->blk_dva[i], buf, stack) == 0)
			return (0);
	}
 ...
 */
static int
zio_read_gang(uint64_t bp_blk_birth, dva_t *dva, void *buf, char *stack)
{
	...
	ZIO_SET_CHECKSUM(&tmpbp.blk_cksum, DVA_GET_VDEV(dva),
	    DVA_GET_OFFSET(dva), bp_blk_birth, 0);
	...
	for (i = 0; i < SPA_GBH_NBLKPTRS; i++) {
		...
		if (zio_read_gang_data(&zio_gb->zg_blkptr[i], buf, stack))
			return (1);
 ...
 */
static int
zio_read_common(blkptr_t *bp, dva_t *dva, void *buf, char *stack)
{
	...
	if (DVA_GET_GANG(dva)) {
		if (zio_read_gang(bp->blk_birth, dva, buf, stack) == 0)
			return (0);
	} else {
		uint64_t offset, sector;
		...
		offset = DVA_GET_OFFSET(dva);
		sector = DVA_OFFSET_TO_PHYS_SECTOR(offset);
		if (devread(sector, 0, BP_GET_PSIZE(bp), buf))
			return (0);
	}
...
zio_read_data(blkptr_t *bp, void *buf, char *stack)
{
	...
	for (i = 0; i < BP_GET_NDVAS(bp); i++) {
		...
		if (zio_checksum_verify(bp, buf, BP_GET_PSIZE(bp)) != 0) {

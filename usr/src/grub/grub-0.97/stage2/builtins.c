...
/*
 * List of preferred VESA modes to pick if none specified.
 *
 * We use this file also as a sentinel to detect a BE that supports
 * bitmapped console vs an older one. To simplify the transition from
 * the two, certain commands (splashimage, foreground, background) are
 * ignored when booting a bitmapped console capable BE in graphics mode.
 */
#define	VBE_MODES_FILE	"/boot/grub/default_video_modes.txt"

static int vbematch_func(char *, int);
static void pick_default_vbe();
static int do_default_vbe = 1;
...
/*
 * Check if the target entry supports booting with a bitmapped console.
 * Returns: 0 if support is missing, 1 otherwise.
 */
static int
target_has_bitmap_console_support()
{

  if (!grub_open(VBE_MODES_FILE))
  {
      errnum = 0;
      return 0;
  }
...
/*
 * Algorithm for locating the config file is:
 * 1. If DHCP option 150 is specified and that filename exists, return * it.
 * 2. If menu.lst.01<MAC address> exists, return it.
 * 3. Use the DHCP BootFile value as a base:
 *   - If BootFile is a path to "pxegrub", search up that path for  menu.lst
 *   - Else look for menu.lst.<BootFile>, stripping the "pxegrub." prefix from
 *   <BootFile> (if present) first.
 * 4. Fall back to /boot/grub/menu.lst
 */
static void solaris_config_file (void)
{
#define MENUFILE_LEN		128
#define	CLIENT_MENUFILE_LEN	25
#define	MAX_BOOTFILE_LEN	(MENUFILE_LEN - CLIENT_MENUFILE_LEN)
	static char menufile[MENUFILE_LEN];
	static char client_menufile[CLIENT_MENUFILE_LEN];
	static char hexdigit[] = "0123456789ABCDEF";
	char *c = menufile;
	int i;

	if (grub_strlen(bootfile) > MAX_BOOTFILE_LEN) {	
		if (debug)
			grub_printf("bootfile path too long\n");
		return;
	}
	...
	*c = 0;
	...
	/*
	 * save off the name of this client-specific menufile and we'll
	 * look for it below.
	 */
	grub_strcpy(client_menufile, "/");
	i = grub_strlen(menufile);
	grub_strncat(client_menufile, menufile, i);

	/*
	 * try to find a configfile derived from the DHCP/bootp
	 * BootFile string. There are two forms possible
	 * - If the bootfile is a path to pxegrub, then search along each
	 *   element of the path for <path>/menu.lst (first checking for
	 *   the client-specific menu, as we did above)
	 * - Otherwise, look for menu.lst.<BootFile>
	 */
	if (bootfile != NULL && bootfile[0] != 0) {
		configfile_origin = CFG_BOOTFILE;
		grub_strcpy(menufile, bootfile);
		c = grub_strrchr(menufile, '/');
		i = grub_strlen("/pxegrub");
		if (c != NULL && grub_memcmp(c, "/pxegrub", i) == 0) {
			do {
				grub_strcpy(c, client_menufile);
				if (test_config_file(menufile)) {
					configfile_origin = CFG_MAC;
					return;
				}
				...
				if (test_config_file(menufile))
					return;
				*c = '\0';
			} while ((c = grub_strrchr(menufile, '/')) != NULL);
		} else {
			c = menufile;
			grub_strcpy(c, "menu.lst.");
...
static int splashimage_func(char *arg, int flags) {
    ...
    /*
     * Transition to bitmapped consoles: a leftover splashimage entry would
     * be toxic for us - ignore it.
     */
    if (flags != BUILTIN_MENU && target_has_bitmap_console_support() != 0)
      return 0;
...
{
   /*
    * Transition to bitmapped consoles: a leftover foreground entry would
    * be toxic for us - ignore it.
    */
    if (flags != BUILTIN_MENU && target_has_bitmap_console_support() != 0)
      return 0;
...
{
   /*
    * Transition to bitmapped consoles: a leftover background entry would
    * be toxic for us - ignore it.
    */
    if (flags != BUILTIN_MENU && target_has_bitmap_console_support() != 0)
      return 0;
...
kernel_dollar_func (char *arg, int flags)
{
  ...
  /*
   * If the VGA console is explicitly requested, don't set a VESA mode on boot.
   */
  if (strstr(arg, "force-text"))
    do_default_vbe = 0;
...
pick_default_vbe()
{
  char buffer[80];
  /*
   * If the default video modes file is missing, then we are booting an
   * older BE (or a "corrupted" one). In any case, we can't attempt to set
   * a random hi-res mode, so we simply return here and default to VGA.
   */ 
  if (!grub_open(VBE_MODES_FILE)) {
    grub_printf("Unable to open %s\n", VBE_MODES_FILE);
    errnum = 0;
    return;
  }

  grub_memset(buffer, 0, 80);

  while (get_line_from_config(buffer, 80, 1))
    {
      if (vbematch_func(buffer, 0) == 0)
        break;
...
vbematch_func (char *arg, int flags)
{
  ...
  for (mode_list
	...
	continue;
	  ...
	  {
             mode_number = (unsigned) *mode_list;
  ...
  if (mode_number == -1)
    {
      grub_printf (" Resolution %dx%dx%d  is not found or supported.\n", 
                     width, height, depth);
      return 1;
    }
...
vbeset_func (char *arg, int flags)
{
  ...
  /* VESA mode picked already, no need to automatically choose one. */
  do_default_vbe = 0;

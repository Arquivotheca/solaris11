...
function zip_webrev
{
	...
	if [[ -x $ZIP ]]; then
		print "Compressing webrev to ZIP file"
		$ZIP -rq $WDIR ${WDIR##*/}
		return $?
	else
...
#         '<a href="http://monaco.us.oracle.com/detail.jsp?cr=1234567">
...
function framed_sdiff
{
	...
	url_tname="$(print $TNAME | url_encode)"
	cat >> $WDIR/$DIR/$TNAME.frames.html <<-EOF
	  <frameset rows="*,60">
	    <frameset cols="50%,50%">
	      <frame src="$url_tname.lhs.html" scrolling="auto" name="lhs"></frame>
	      <frame src="$url_tname.rhs.html" scrolling="auto" name="rhs"></frame>
	    </frameset>
...
<body id="SUNWwebrev" bgcolor="#eeeeee" onload="document.diff.real.focus();"
	...
	Either this browser is incompatible or javascript is not enabled</p>
      </center>
    </noscript>
...
Options:
	...
	-z Compress the webrev into ZIP format.
...
#
while getopts "C:Di:I:lnNo:Op:t:Uwz" opt
do
	case $opt in
	C)	Cflag=1
		ITSCONF=$OPTARG;;
...
if [[ -n $nflag && -z $Uflag && -z $zflag ]]; then
...
if [[ -z $tflag && ( -n $Uflag || -n $Dflag ) ]]; then
    echo "remote target must be specified for upload or deletion"
    exit 1
fi
...
do
	...
	if [[ -z $rename ]]; then
		if [ ! -f "$ofile" ]; then
			LC_ALL=C diff -u /dev/null $nfile \
			    | sh -c "$cleanse_newfile" > $WDIR/$DIR/$F.patch
		elif [ ! -f "$nfile" ]; then
			LC_ALL=C diff -u $ofile /dev/null \
			    | sh -c "$cleanse_rmfile" > $WDIR/$DIR/$F.patch
		else
			LC_ALL=C diff -u $ofile $nfile > $WDIR/$DIR/$F.patch
		fi
	else
		LC_ALL=C diff -u $ofile /dev/null | sh -c "$cleanse_rmfile" \
		    > $WDIR/$DIR/$F.patch

		LC_ALL=C diff -u /dev/null $nfile | sh -c "$cleanse_newfile" \
		    >> $WDIR/$DIR/$F.patch
	fi

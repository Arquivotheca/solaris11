...
sub ProcFile {
	...
	# Only look at executables and shared objects
...
sub ProcSymSort {
	...
	my $SORT_FH;
	open($SORT_FH, "elfdump -S $FullPath|") ||
...
sub ProcFindElf {
	...
	my $FIND_ELF_FH;
	my $prefix = OpenFindElf($file, \$FIND_ELF_FH, \$LineNum);

	while ($line = onbld_elfmod::GetLine($FIND_ELF_FH, \$LineNum)) {
...
sub AltObjectConfig {
	my $file = $_[0];
	my ($Crle32, $Crle64, $CRLE32_FH, $CRLE64_FH);
	my $line;
	my $LineNum;
	my $obj_path;
	my $obj_active = 0;
	...
	my $FIND_ELF_FH;
	my $prefix = OpenFindElf($file, \$FIND_ELF_FH);
	...
	while ($line = onbld_elfmod::GetLine($FIND_ELF_FH, \$LineNum)) {
	      ITEM: {
			...
			if ($line =~ /^OBJECT\s/i) {
				...
				if ($type eq 'DYN') {
					...
					# Only want shared objects
					$obj_active = 0;
				}
				last ITEM;
			}

			# We need to follow links to shared objects so
		...
		if ($obj_class == 32) {
			if (!$Crle32) {
				...
				open($CRLE32_FH, "> $Crle32") ||
				    die "$Prog: open failed: $Crle32: $!";
				print $CRLE32_FH "#!/bin/sh\ncrle \\\n";
			}
			print $CRLE32_FH "\t-o $Dir -a /$obj_path \\\n";
		} elsif ($Ena64) {
			if (!$Crle64) {
				$Crle64 = "$Tmpdir/$Prog.crle64.$$";
				open($CRLE64_FH, "> $Crle64") ||
				    die "$Prog: open failed: $Crle64: $!";
				print $CRLE64_FH "#!/bin/sh\ncrle -64\\\n";
			}
			print $CRLE64_FH "\t-o $Dir -a /$obj_path \\\n";
		}
	}
...
# If we are basing this analysis off the shared objects found in
...
if ($opt{D} || $Proto) {
	if (system('ldd -e /usr/lib/lddstub 2> /dev/null')) {
		...
		# shared objects found under $Proto.

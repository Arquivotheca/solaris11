...
main(int argc, char **argv)
{
	...
	if (uniqfile != NULL || withfile != NULL) {
		...
		/*
		 * In case of additive merge, make sure that the label being
		 * added doesn't already exist in withfile.
		 */
		if (withfile != NULL && tdata_label_find(reftd, label) >= 0)
			terminate("Cannot add a duplicate label\n");

 ...
 *		<a>={%auto,func,no%func}
 * -xipo[=<n>]	Enable optimization and inlining across source files,
 *		n={0|1}
 ...
 * -xcrossfile[=<n>]		error (obsolete, see -xipo)
 * -xe				error
 * -xF				error
 * -xhelp=<f>			error
 * -xildoff			ignore
...
do_gcc(cw_ictx_t *ctx)
{
	...
	while (--ctx->i_oldargc > 0) {
		...
		case 'W':
			...
			if (strcmp(arg, "-Wu,-O~Z") == 0) {
				/*
				 * This translates to -mno-red-zone which is
				 * actually covered by the -xmodel=kernel
				 * option.
				 */
				break;
			}
		...
		case 'x':
			...
			case 'i':
				...
				if (strcmp(arg, "-xildon") == 0 ||
					...
					break;
				if (strncmp(arg, "-xipo", 5) == 0)
					/*
					 * No optimization and inlining across
					 * source files; ignore
					 */
					break;
				error(arg);
			...
			case 't':
				...
				break;
			case 'v':
				if (strcmp(arg, "-xvector=none") == 0) {
					/*
					 * Current gcc 3.4.3 doesn't support
					 * the translated option of
					 * 	-fno-tree-vectorize
					 */
					break;
				}
...
do_cc(cw_ictx_t *ctx)
{
	...
	while (--ctx->i_oldargc > 0) {
		...
		case 'S':
			...
			break;
		case 'x':
			if ((strcmp(arg, "-xregs=no%appl") == 0) ||
			    (strcmp(arg, "-xregs=no%float") == 0)) {
				newae(ctx->i_ae, "-Qoption");
				newae(ctx->i_ae, "cg");
			}
		/*FALLTHROUGH*/
		case 'E':
...
main(int argc, char **argv)
{
	...
	if ((dir = getenv("SPRO_VROOT")) != NULL) {
		...
		(void) snprintf(default_dir[CW_C_CC], MAXPATHLEN,
		    "%s/sunstudio12.1/bin", dir);
	} else if ((dir = getenv("BUILD_TOOLS")) != NULL) {
		(void) snprintf(default_dir[CW_C_CC], MAXPATHLEN,
		    "%s/SUNWspro/sunstudio12.1/bin", dir);
	}

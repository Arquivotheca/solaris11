...
int cmd_pcrextend(TSS_HCONTEXT, TSS_HTPM, int argc, char *argv[]);
int cmd_pcrreset(TSS_HCONTEXT, TSS_HTPM, int argc, char *argv[]);
...
cmdtable_t commands[] = {
	...
	{"pcrextend", "pcr [filename]", cmd_pcrextend},
	{"pcrreset", "pcr", cmd_pcrreset},
	{NULL, NULL, NULL},
};
...
static int
SHA1_from_file(TPM_DIGEST *digest, const char *filename)
{
	char buf[BUFSIZ];
	SHA1_CTX ctx;
	int len;
	...
	while ((len = read(fd, buf, sizeof (buf))) > 0) {
		SHA1Update(&ctx, buf, (unsigned int)len);
	}
...
/*ARGSUSED*/
int
cmd_pcrextend(TSS_HCONTEXT hContext, TSS_HTPM hTPM, int argc, char *argv[])
{
	TSS_RESULT ret;
	UINT32 pcrIndex;
	TPM_DIGEST digest;
	...
	if (argc < 2) {
		...
		fprintf(stderr, "pcrextend PCR# [filename]\n");
		return (ERR_FAIL);
	}
	...
	if (SHA1_from_file(&digest, filename) != 0)
		return (ERR_FAIL);

	ret = Tspi_TPM_PcrExtend(hTPM, pcrIndex, sizeof (TPM_DIGEST),
	    digest.digest, NULL, &pcrValueLen, &pcrValue);
	if (ret)
		print_error(ret, "Tspi_TPM_PcrExtend failed");

	return (ret);
}

int
cmd_pcrreset(TSS_HCONTEXT hContext, TSS_HTPM hTPM, int argc, char *argv[])
{
	TSS_RESULT ret;
	UINT32 pcrIndex;
	TSS_HPCRS hPcrs;
	...
	if (ret) {
		print_error(ret, "Tspi_Context_CreateObject(HPCRS) failed");
		return (ret);
	}
	ret = Tspi_PcrComposite_SelectPcrIndexEx(hPcrs, pcrIndex,
	    TSS_PCRS_DIRECTION_RELEASE);
	if (ret) {
		print_error(ret, "Tspi_PcrComposite_SelectPcrIndexEx failed");
		return (ret);
	}

	ret = Tspi_TPM_PcrReset(hTPM, hPcrs);
	if (ret)
		print_error(ret, "PCR Reset failed");

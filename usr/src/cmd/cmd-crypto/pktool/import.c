	...
	if (rv == KMF_OK) {
		...
		for (i = 0; i < nkeys; i++) {
			...
			if (keys[i].label != NULL &&
			    strlen(keys[i].label)) {
				kmf_set_attr_at_index(attrlist, num++,
				    KMF_KEYLABEL_ATTR, keys[i].label,
				    strlen(keys[i].label));
	...
	char *filename)
{
	...
	if (rv == KMF_OK) {
		...
		for (i = 0; rv == KMF_OK && i < ncerts; i++) {
			int num = numattr;
			/*
			 * The command line "label" overrides the
			 * friendlyName found in the PKCS#12 bundle
			 * for the first cert.  If we allow it for
			 * all certs, it results in multiple objects
			 * with the same label, which is probably not the
			 * desired outcome.  99% of the time, the PKCS#12
			 * import operation is for a single key & cert.
			 */
			if (i == 0 && label != NULL) {
				kmf_set_attr_at_index(attrlist, num,
				    KMF_CERT_LABEL_ATTR, label, strlen(label));
				num++;
			} else if (certs[i].kmf_private.label != NULL) {
				kmf_set_attr_at_index(attrlist, num,
				    KMF_CERT_LABEL_ATTR,
				    certs[i].kmf_private.label,
				    strlen(certs[i].kmf_private.label));

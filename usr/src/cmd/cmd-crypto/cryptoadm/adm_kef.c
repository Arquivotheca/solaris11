    ...
    mechlist_t *dislist)
{
	...
	/*
	 * The behavior of disable should match that of enable in the
	 * case of cryptoadm disable provider=<prov> mechanism=all. Since,
	 * enable correctly does not touch the random feature neither
	 * should disable. Note, that when the allflag is set and the
	 * rndflag is not set we have the mechanism=all case.
	 */
	if (allflag && !rndflag && !is_in_list(RANDOM, pent->dislist)) {

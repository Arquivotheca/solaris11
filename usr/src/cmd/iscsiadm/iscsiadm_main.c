...
#define	OPTIONSTRING10	"maximum connection number"
...
optionTbl_t longOptions[] = {
	...
	{"max-connections", required_arg, 'm', OPTIONSTRING10},
	{NULL, 0, 0, 0}
};
...
optionRules_t optionRules[] = {
	...
	{NODE, MODIFY, "NAhdCaRrPHcTm", B_TRUE, "CP"},
	{TARGET_PARAM, MODIFY, "ahdBCpcHTm", B_TRUE, "C"},
...
printLoginParameters(char *prefix, IMA_OID oid, int printOption)
{
	...
	if (printOption == PRINT_NEGOTIATED_PARAMS) {
		propMinMax.currentValue = connProps->maxXmitDataSegmentLength;
	} else {
...
gettext("value must be between 1 and 65535"));
		...
		case MAX_CONNECTIONS:
			...
			if (uintValue < 1 ||
			    uintValue > ISCSI_INI_MAX_CONNECTIONS) {
				(void) fprintf(stderr, "%s: %s\n",
				    cmdName,
gettext("value must be between 1 and 65535"));
				return (1);
...
listNode(int *funcRet)
{
	...
	status = IMA_GetMaxConnectionsProperties(initiatorOid, &propMinMax);
	if (!IMA_SUCCESS(status)) {
		printLibError(status);
		*funcRet = 1;
		return (ret);
	}
	(void) fprintf(stdout, "\t\t%s: ", gettext("Max Connections"));
	printOption = PRINT_CONFIGURED_PARAMS;
	IMAMINMAXPRINT(propMinMax, printOption);
	...
	for (; optionList->optval; optionList++) {
		switch (optionList->optval) {
			...
			case 'T':
				...
				break;
			case 'm':
				if (setLoginParameter(oid, MAX_CONNECTIONS,
				    optionList->optarg) != 0) {
					return (1);
				}
				break;
	...
	for (; optionList->optval; optionList++) {
		switch (optionList->optval) {
			...
			case 'T':
				...
				break;
			case 'm':
				if (setLoginParameter(targetOid,
				    MAX_CONNECTIONS, optionList->optarg) != 0) {
					*funcRet = 1;
					return (1);
				}
				break;

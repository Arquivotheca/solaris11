 ...
 *
 * IPC information is obtained via msgctl64, semctl64 and shmxctl64.
...
static char opts[] = "AabcdiJmopqstD:z:Z";	/* getopt options */
...
static int	bflg,		/* biggest size: */
		...
		dflg,		/* display OSM allocated memory */
...
main(int argc, char *argv[])
{
	...
	if (mflg) {
		...
		if (!qflg || oflg || bflg || dflg || pflg || tflg || iflg)
		...
		for (i = 0; i < n; i++) {
			id = ids[i];
			if (shmxctl64(id, IPC_XSTAT64, &mds) < 0)
				continue;
			/* ignore zone if -Z was used and -z wasn't */
			if ((zflg || !Zflg) &&

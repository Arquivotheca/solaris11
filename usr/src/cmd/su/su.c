...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it wasn't */
	...
	if ((ttyn = ttyname(0)) == NULL)
		if ((ttyn = ttyname(1)) == NULL)
			ttyn = ttyname(2);
	/*  "fake" ttyn for logging but don't set it as PAM_TTY */
	if (ttyn == NULL)
		ttyn = "/dev/???";
	else if (pam_set_item(pamh, PAM_TTY, ttyn) != PAM_SUCCESS)
		exit(1);

	if (getpwuid_r(getuid(), &pwd, pwdbuf, sizeof (pwdbuf)) != NULL) {
		if (pam_set_item(pamh, PAM_AUSER, pwd.pw_name) != PAM_SUCCESS)
			exit(1);
	}
	...
	if ((uid == (uid_t)ROOT) && (strcmp(name, "root") == 0) &&
		...
		switch (errno) {
		case EACCES:
			message(ERR, gettext("Shell not executable"));
			break;
		case EPERM:
			message(ERR, gettext("Insufficient privilege to "
			    "execute shell"));
			break;
		default:
...
audit_success(int pw_change, struct passwd *pwd)
{
	...
	adt_free_event(event);
	/*
	 * The preceding code is a noop if audit isn't enabled,
...
validate(char *usernam, int *pw_change)
{
	...
	if ((error = pam_acct_mgmt(pamh, pam_flags)) != PAM_SUCCESS) {
		...
		if (error == PAM_NEW_AUTHTOK_REQD) {
			tries = 0;
			message(WARN, gettext("Password for user "

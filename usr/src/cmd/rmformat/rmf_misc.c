...
get_device(char *user_supplied, char *node)
{
	...
	/* the device node must be in a /dev tree */

	if ((strncmp(devnode, "/devices/", 9) != 0) &&
	    (strncmp(devnode, "/dev/", 5) != 0))
		return (NULL);
...
/*
 * Follow symbolic links from the logical device name to the /devfs
 * physical device name.  To be complete, we handle the case of multiple
 * links.  If we can't resolve a link, we return the original path.
 *
 * Note that the standard /devices prefix is stripped from the final
 * pathname, if present.  The trailing options are also removed (":c,
 * raw").
 */

 ...
 *
 * (7) By default we randomly choose to enable encryption on datasets.
...
static offset_t zopt_vdev_size = 512 << 20;
...
ztest_info_t ztest_info[] = {
	...
	{ ztest_split_pool,			1,	&zopt_sometimes	},
...
usage(boolean_t requested)
{
	...
	char nice_gang_threshold[10];
	FILE *fp = requested ? stdout : stderr;

	nicenum(zopt_vdev_size, nice_vdev_size);
	nicenum(metaslab_gang_threshold, nice_gang_threshold);
...
process_options(int argc, char **argv)
{
	...
	metaslab_gang_threshold = 32 << 10;

	while ((opt = getopt(argc, argv,
	    "v:s:a:m:r:R:d:t:g:i:k:p:f:VMEUT:P:hF:C")) != EOF) {
		value = 0;
		switch (opt) {
		case 'v':
		case 's':
		...
		case 't':
			...
			break;
		case 'g':
			metaslab_gang_threshold = MAX(
			    SPA_MINBLOCKSIZE << 1, value);
			break;
		case 'i':
			zopt_init = value;
			break;
		...
		case 'C':
			...
			break;
		case 'M':
			io_mvector_tracking_enable = 0;
			zfs_vdev_enable_mvector = 0;
			break;
		case 'h':
			usage(B_TRUE);
			break;
...
static nvlist_t *
make_vdev_file(char *path, char *aux, offset_t size, uint64_t ashift)
{
	...
	if (size != 0) {
		int fd = open64(path, O_RDWR | O_CREAT | O_TRUNC, 0666);
		if (fd == -1)
	...
	VERIFY(nvlist_add_uint64(file, ZPOOL_CONFIG_ASIZE, size) == 0);

	return (file);
}

static nvlist_t *
make_vdev_raidz(char *path, char *aux, offset_t size, uint64_t ashift, int r)
{
...
static nvlist_t *
make_vdev_mirror(char *path, char *aux, offset_t size, uint64_t ashift,
	int r, int m)
{
...
static nvlist_t *
make_vdev_root(char *path, char *aux, offset_t size, uint64_t ashift,
...
ztest_log_write(ztest_ds_t *zd, dmu_tx_t *tx, lr_write_t *lr)
{
	...
	if (lr->lr_length > ZIL_MAX_LOG_DATA(dmu_objset_spa(zd->zd_os)) &&
	    zd->zd_zilog->zl_no_dmu_sync == 0)
...
ztest_replay_write(ztest_ds_t *zd, lr_write_t *lr, boolean_t byteswap)
{
	...
	if (ztest_random(8) == 0 && length == doi.doi_data_block_size &&
		...
		abuf = dmu_request_arcbuf(length);
...
ztest_io(ztest_ds_t *zd, uint64_t object, uint64_t offset)
{
	...
	(void) rw_rdlock(&ztest_shared->zs_io_lock);
	...
	(void) rw_unlock(&ztest_shared->zs_io_lock);
}
...
static void
ztest_async_wait(spa_t *spa, int taskflags)
{
	mutex_enter(&spa->spa_async_lock);
	while ((spa->spa_async_tasks & taskflags) ||
...
ztest_zil_remount(ztest_ds_t *zd, uint64_t id)
{
	...
	VERIFY(rw_wrlock(&zs->zs_io_lock) == 0);

	/*
	 * Mimic the remount behavior of the zil to ensure that
	 * all zil blocks are cleaned up correctly.
	 */
	zil_close(zd->zd_zilog);
	zd->zd_zilog = zil_open(os, ztest_get_data);
	zil_replay(os, zd, ztest_replay_vector);

	(void) rw_unlock(&zs->zs_io_lock);
}
...
ztest_spa_create_destroy(ztest_ds_t *zd, uint64_t id)
{
	...
	nvroot = make_vdev_root("/dev/bogus", NULL, 0, 0, 0, 0, 0, 1);
	VERIFY3U(EEXIST, ==, spa_create(zs->zs_pool, nvroot,
	    NULL, NULL, NULL, NULL));
	nvlist_free(nvroot);
...
grow_vdev(vdev_t *vd, void *arg)
{
	...
	if ((fd = open64(vd->vdev_path, O_RDWR)) == -1)
		return (vd);

	fsize = lseek64(fd, 0, SEEK_END);
	(void) ftruncate64(fd, *newsize);
...
ztest_vdev_LUN_growth(ztest_ds_t *zd, uint64_t id)
{
	...
	metaslab_class_t *mc;
	metaslab_group_t *mg;
	offset_t psize, newsize;
	uint64_t top, topguid;
	 ...
	 */
	VERIFY(mutex_unlock(&zs->zs_vdev_lock) == 0);
	ztest_async_wait(spa, SPA_ASYNC_CONFIG_UPDATE);
	VERIFY(mutex_lock(&zs->zs_vdev_lock) == 0);

	spa_config_enter(spa, SCL_STATE, spa, RW_READER);
	config_ok = (top < spa->spa_root_vdev->vdev_children);

	if (config_ok) {
		tvd = spa->spa_root_vdev->vdev_child[top];
		new_ms_count = tvd->vdev_ms_count;
		new_class_space = metaslab_class_get_space(mc);
		config_ok = (tvd->vdev_mg == mg && mg->mg_class == mc &&
		    tvd->vdev_guid == topguid);
	}
...
ztest_dataset_create(char *dsname, dsl_crypto_ctx_t *dcc)
{
	...
	int err = dmu_objset_create(dsname, DMU_OST_OTHER, 0,
	    dcc, NULL, ztest_objset_create_cb, NULL);
...
ztest_dmu_objset_create_destroy(ztest_ds_t *zd, uint64_t id)
{
	 ...
	 */
	if (zopt_crypto && ztest_random(2) == 0) {
		dcc.dcc_crypt =
	 ...
	 */
	VERIFY3U(EEXIST, ==, dmu_objset_create(name, DMU_OST_OTHER, 0,
	    NULL, NULL, NULL, NULL));
...
ztest_dsl_dataset_promote_busy(ztest_ds_t *zd, uint64_t id)
{
	...
	if (zopt_crypto && zd->zd_crypt != ZIO_CRYPT_OFF) {
	...
	if (error) {
		...
		fatal(0, "dmu_objset_clone(%s) = %d", clone1name, error);
	}
	...
	error = dmu_objset_clone(clone2name, dmu_objset_ds(clone),
	    &dcc, 0);
	dmu_objset_rele(clone, FTAG);
	if (error) {
		if (error == ENOSPC) {
		...
		fatal(0, "dmu_objset_clone(%s) = %d", clone2name, error);
	}
...
ztest_dmu_read_write_zcopy(ztest_ds_t *zd, uint64_t id)
{
	...
	bigbuf_arcbufs = umem_zalloc(2 * s * sizeof (arc_ref_t *), UMEM_NOFAIL);
	...
	for (i = 0; i < 7; i++) {
		...
		for (j = 0; j < s; j++) {
			if (i != 5) {
				bigbuf_arcbufs[j] =
				    dmu_request_arcbuf(chunksize);
			} else {
				bigbuf_arcbufs[2 * j] =
				    dmu_request_arcbuf(chunksize / 2);
		...
		if (txg == 0) {
			...
			for (j = 0; j < s; j++) {
				if (i != 5) {
					dmu_release_arcbuf(bigbuf_arcbufs[j]);
				} else {
					dmu_release_arcbuf(
					    bigbuf_arcbufs[2 * j]);
					dmu_release_arcbuf(
			...
			umem_free(bigbuf_arcbufs, 2 * s * sizeof (arc_ref_t *));
		...
		for (off = bigoff, j = 0; j < s; j++, off += chunksize) {
			...
			if (i != 5) {
				bcopy((caddr_t)bigbuf + (off - bigoff),
				    bigbuf_arcbufs[j]->r_data, chunksize);
			} else {
				bcopy((caddr_t)bigbuf + (off - bigoff),
				    bigbuf_arcbufs[2 * j]->r_data,
				    chunksize / 2);
				bcopy((caddr_t)bigbuf + (off - bigoff) +
				    chunksize / 2,
				    bigbuf_arcbufs[2 * j + 1]->r_data,
				    chunksize / 2);
			}
...
ztest_dsl_crypto(ztest_ds_t *zd, uint64_t id)
{
	...
	if (!zopt_crypto || zd->zd_crypt == 0 || zd->zd_crypt == ZIO_CRYPT_OFF)
		return;
...
ztest_dmu_snapshot_hold(ztest_ds_t *zd, uint64_t id)
{
	...
	if (error && error != ENOENT && error != ESRCH)
		fatal(0, "dsl_dataset_user_release = %d", error);
	error = dmu_objset_destroy(fullname, B_FALSE);
	if (error && error != ENOENT)
		fatal(0, "dmu_objset_destroy(%s) = %d", fullname, error);
	...
	error = dmu_objset_hold(fullname, FTAG, &origin);
	if (error != ENOENT) {
		fatal(0, "dmu_objset_hold(%s) = %d, origin %p",
		    fullname, error, origin);
	}
	...
	error = dsl_dataset_user_hold(osname, snapname, tag, B_FALSE,
	    B_TRUE, -1);
	if (error) {
		fatal(0, "dsl_dataset_user_hold(%s, %s) = %d", fullname, tag,
		    error);
	}
	...
	error = dsl_dataset_user_release(osname, snapname, tag, B_FALSE);
	if (error) {
		fatal(0, "dsl_dataset_user_release(%s, %s) error %d",
		    fullname, tag, error);
	}
...
ztest_fault_inject(ztest_ds_t *zd, uint64_t id)
{
	...
	while (--iters != 0) {
		...
		if (pwrite64(fd, &bad, sizeof (bad), offset) != sizeof (bad))
...
ztest_ddt_repair(ztest_ds_t *zd, uint64_t id)
{
	...
	for (int i = 0; i < copies; i++) {
		...
		VERIFY3U(dmu_buf_hold(os, object, offset, FTAG, &db,
	 ...
	 */
	VERIFY3U(dmu_buf_hold(os, object, 0, FTAG, &db,
	    DMU_READ_NO_PREFETCH), ==, 0);
	dbuf = (dmu_buf_impl_t *)db;
	mutex_enter(&dbuf->db_parent->db_mtx);
	blkid = dbuf->db_blkoff / sizeof (blkptr_t);
	blk = ((blkptr_t *)dbuf->db_parent->db.db_data)[blkid];
	mutex_exit(&dbuf->db_parent->db_mtx);
	dmu_buf_rele(db, FTAG);
...
ztest_spa_import_export(char *oldname, char *newname)
{
	 ...
	 * Try to import it.
	 */
	newconfig = spa_tryimport(config, B_TRUE);
...
ztest_dataset_open(ztest_shared_t *zs, int d)
{
	...
	if (zilog->zl_header->zh_claim_lr_seq != 0 &&
		...
		fatal(0, "%s missing log records: "
		    "claimed %llu < committed %llu",
		    zd->zd_name, zilog->zl_header->zh_claim_lr_seq,
		    committed_seq);
	...
	if (zilog->zl_replaying_seq != 0 &&
		...
		fatal(0, "%s missing log records: "
		    "replayed %llu < committed %llu",
		    zd->zd_name, zilog->zl_replaying_seq, committed_seq);
...
ztest_run(ztest_shared_t *zs)
{
	...
	VERIFY(rwlock_init(&zs->zs_io_lock, USYNC_THREAD, NULL) == 0);
	...
	VERIFY3U(spa_open(zs->zs_pool, &spa, FTAG), ==, 0);
	zs->zs_spa = spa;

	/*
	 * Wait for the async pool cleanup task to finish so ztest functions
	 * that do cleanup of prior runs do not compete with
	 * dsl_dataset_destroy_inconsistent().
	 */
	ztest_async_wait(spa, SPA_ASYNC_POOL_CLEANUP);
	...
	(void) rwlock_destroy(&zs->zs_io_lock);
...
ztest_freeze(ztest_shared_t *zs)
{
	...
	kernel_fini();
	if (zopt_verbose >= 3)
		(void) printf("testing spa_freeze()... done\n");
}
...
ztest_init(ztest_shared_t *zs)
{
	...
	VERIFY3U(0, ==, spa_create(zs->zs_pool, nvroot, props,
	    NULL, NULL, NULL));
	nvlist_free(nvroot);

...
/*
 * Macro and structures which allow rm() to use a loop instead of
 * doing recursion.
 */
...
#define	INIT_VARS(ent, clr) {						\
		entry = ent;						\
		caller = clr;						\
		frame = &v->dlist_frame;				\
	}

#define	RM_ENTRY(ent, clr) {						\
		...
		/* Enter directory */					\
		goto rm_entry;						\
rm_return :	/* Return to here from RM_RETURN */			\
		/* Pop v */						\
		v = stack;						\
		stack = v->next;					\
		RESTORE_VARS();						\
...
/*
 * The RM_ENTRY macro is used to go to the top of a loop that removes an
 * entry in a directory. If the entry is a directory, then it will be entered
 * and its entries will be removed by going to the top of the loop.
 * The RM_RETURN macro is used to return to where the last RM_ENTRY was used.
 * Locale variables will be saved in an area allocated by RM_ENTRY. They
 * will be restored after RM_RETURN returns.
 *
 * The save areas are allocated in user space and they are limitted by the
 * amount of memory available to the running process.
 */

static int
rm(const char *a_entry, struct dlist *a_caller)
{
	struct	dlist	dlist_frame;
	struct	dlist	*frame = &dlist_frame;
	int		flag;
	struct	stat	temp;
	struct	dirent	*dent;
	int		err;
	char		*entry = (char *)a_entry;
	struct	dlist	*caller = a_caller;
...
rm_entry :
	 ...
	 * allocated by readdir and that after return from RM_ENTRY
	 * the memory is no longer valid.  So after the RM_ENTRY
 ...
 */
		...
		/*
		 * We looped and the subdirectory may have set the CANTCLOSE

	...
	nscd_nsw_params_t	*params)
{
	...
	params->compati = -1;
	params->dnsi = -1;
	params->privdb = 0;	/* Assume privileges not required. */
...
/*ARGSUSED*/
void
_nss_db_state_destr(struct nss_db_state *s, nss_db_root_t *r)
{
...
void
nss_psearch(void **bufpp, size_t length)
{
	/* inputs */
	nss_db_initf_t		initf;
	int			dbop;
	...
	status = nss_packed_arg_init(*bufpp, length,
	    NULL, &initf, &dbop, &arg);
	...
	if (!swret.noarg && status != NSS_TRYLOCAL)
		nss_packed_set_status(*bufpp, length, status,  &arg);
	...
	/* remember from which backend the result is from */
	pbuf->nscdpriv = swret.srci;
}

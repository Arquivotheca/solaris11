...
static nscd_rc_t lookup_cache(nsc_lookup_args_t *, nscd_cfg_cache_t *,
    nss_XbyY_args_t *, char *, nsc_entry_t **, int);
...
static int lookup_int_ext(nsc_lookup_args_t *, int, getkey_func, psearch_func);
...
dup_packed_buffer(void *src, void *dst) {
	...
	if (NSCD_GET_STATUS(sphdr) != NSS_SUCCESS) {
		 ...
		 * header plus result
		 */
		slen = sphdr->data_off + sphdr->data_len;
	}
	...
	if (dphdr == NULL) {
		/* Get new buffer, size is slen + 1 for '\0' */
		new_pbufsiz = slen + 1;
		dphdr = calloc(1, new_pbufsiz);
		if (dphdr == NULL)
			return (NSS_ERROR);
		d->buffer = dphdr;
		d->bufsize = new_pbufsiz;
		dphdr->pbufsiz = new_pbufsiz;
	}

	(void) memcpy(dphdr, sphdr, slen);
	/* Make sure the data (not packed header) is null-terminated. */
	if (slen != sphdr->data_off)
		*((char *)dphdr + slen) = '\0';
...
static mutex_t	cache_ctx_lock = DEFAULTMUTEX;

/*
 * Create a new cache for use with nss backends to the list of legal caches.
 *   Cache name, dbname, must be a constant in the form: :BACKEND:cache_name
 *   Example:  ldap dn2uid cache --> ":ldap:dn2uid"
 *   hash_cis - 1 if keys are CIS.  default is CES (string only for now)
 *
 * This allows nsswitch backends the ability to have nscd managed
 * caches without implementing in a managable manner.  nsswitch
 * backends creating their own caches must provide a weak symbol that
 * maps to this function to a function that returns an error if
 * nscd is not currently running.
 */

nss_status_t
_nss_cache_create(char *dbname, int dbop, int hash_cis, int pos_ttl) {
	nsc_ctx_t	*ctx;
	int		i;
	char		*me = "_nss_cache_create";

	(void) mutex_lock(&cache_ctx_lock);
	for (i = 0; i < CACHE_CTX_COUNT; i++) {
		/* previously initialized.  Currently one per db. */
		if (cache_name[i] == NULL)
			break;
		if (strcmp(cache_name[i], dbname) == 0) {
			(void) mutex_unlock(&cache_ctx_lock);
			return (NSS_SUCCESS);	/* nothing else to do */
		}
	}
	if (i >= CACHE_CTX_COUNT) {
		(void) mutex_unlock(&cache_ctx_lock);
		_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_ERROR)
		(me, "no free cache slots for %s\n", dbname);
		return (NSS_ERROR);		/* no free cache slots */
	}
	...
	cache_name[i] = dbname;
	cache_ctx_p[i] = ctx;
	/* updates the cache index tables */
	cache_init_ctx[i] = NULL;
	(void) _nscd_init_cache_sema(&ctx->throttle_sema, cache_name[i]);

	/* init the cache */
	ctx->dbname = dbname;
	if (pos_ttl < 0 || pos_ttl > 600)
		pos_ttl = 600;			/* cap the time to live */
	ctx->cfg.pos_ttl = pos_ttl;
	ctx->cfg.neg_ttl = pos_ttl;
	ctx->cfg.enable = nscd_true;
	ctx->cfg_mtime = time(NULL);
	ctx->backend_cache = nscd_true;
	ctx->db_count = 1;
	ctx->nsc_db[0] = make_cache((hash_cis ? nsc_key_cis : nsc_key_ces),
			dbop, dbname, NULL, NULL, NULL, nsc_ht_default, -1);
...
/*
 * Get an entry from a previously existing cache.
 * This allows backends to create caches and then to be able
 * to fetch or load/fetch cache results.
 * The nscd caching system itself will flush (as necessary)
 * the cache, just as it flushes all other well known caches.
 *
 * **bufpp and length is a previously alloced (malloc) and initialized
 * packed buffer created by the backend and filled by the get operation.
 * The getkey function provided by the nssswitch backend performs
 * the nss_packed_getkey operation and the psearch function
 * performs a nss_psearch on behalf of the system as necessary.
 * The buffer may be dynamically expended by the psearch function.
 *
 * This allows nsswitch backends the ability to have nscd managed
 * caches without implementing everything in the backend.  nsswitch
 * backends creating their own caches must provide a weak symbol that
 * maps to this function to a function that returns an error if
 * nscd is not currently running.
 */
...
nss_status_t
_nss_cache_get(void **bufpp, size_t length, getkey_func getkey,
    psearch_func psearch, int nscd_cache, char *from_backend)
{
	nsc_lookup_args_t	larg;
	nss_pheader_t		*phdr;
	int			ret, i, flag;
	char			*me = "_nss_cache_get";

	larg.buffer = *bufpp;
	larg.bufsize = length;
	larg.ctx = NULL;
	larg.nscdb = NULL;
	phdr = (nss_pheader_t *)*bufpp;

	/* save backend (source) index in header */
	phdr->nscdpriv = -1;
	for (i = 0; i < NSCD_NUM_SRC; i++) {
		if (NSCD_NSW_SRC_NAME(i) == NULL)
			break;
		if (strcmp(from_backend, NSCD_NSW_SRC_NAME(i)) == 0) {
			phdr->nscdpriv = i;
			break;
		}
	}
	/* Error if unknown backend */
	if (phdr->nscdpriv == -1)
		return (NSS_ERROR);

	/* Look up NSCD's own cache or one created by the backend ? */
	if (nscd_cache == 1)
		/* nscd's own */
		flag = NSCDCACHEBIT;
	else
		...
		flag = BECACHEBIT;
	if (getkey == NULL)
		getkey = nss_packed_getkey;
	if (psearch == NULL)
		psearch = nss_psearch;

	ret = lookup_int_ext(&larg, flag, getkey, psearch);
	switch (ret) {
	case SUCCESS:
		/*
		 * If buffer was reallocated, return the new buffer
		 * to caller.
		 */
		if (*bufpp != larg.buffer) {
			_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
			(me, "cache buffer reallocated\n");
			*bufpp = larg.buffer;
		}
		return (NSS_SUCCESS);
	case NOTFOUND:
	...
	/* SERVERERROR, NOSERVER, CONTINUE ... */
	return (NSS_ERROR);
}

/*
 * Given database name 'dbname' find cache index
 * Performs prelim test on initial character before call strcmp
 * and uses faster pointer array walking versus array indexing
 */
int
get_cache_idx(char *dbname) {
	int	i;
	char	**nsc_namep = &cache_name[0];
	char	*nsc_name;
	char	dbch = dbname[0];

	for (i = 0; i < CACHE_CTX_COUNT; i++, nsc_namep++) {
		if ((nsc_name = *nsc_namep) == NULL)
			continue;	/* non-existent cache */
		if (dbch == *nsc_name && strcmp(nsc_name, dbname) == 0)
			return (i);
	}
	return (-1);
}
 ...
 * not a daemon, i.e., when nscd is being an admin
 * client.
 */
void
nsc_info(nsc_ctx_t *ctx, char *dbname, nscd_admin_cfg_cache_t acfg[],
    nscd_cfg_stat_cache_t stats[])
{
	...
	if (acfg == NULL || stats == NULL)
		return;

	for (i = 0; i < CACHE_CTX_COUNT; i++) {
		if (*acfg[i].name == '\0')
			continue;
		ctx2.dbname = acfg[i].name;
		ctx2.cfg = acfg[i].cfg;
		ctx2.stats = stats[i];
...
#define	NSC_LOOKUP_NO_CACHE(str) \
	_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG) \
		...
		str); \
	(*psearch)(&largs->buffer, largs->bufsize); \
 ...
 * for a cache. For internal backend cache, only reaper
 * thread is started.
 */
static void
...
static nss_status_t
copy_result(void **rbufp, void *cbuf, int flag)
{
	nss_pheader_t	*rphdr = (nss_pheader_t *)(*rbufp);
	nss_pheader_t	*cphdr = (nss_pheader_t *)cbuf;
	char		*me = "copy_result";
	nssuint_t	rlen, cpylen;
	boolean_t	has_optdata = B_FALSE;
	char		*dst;

	/*
	 * For requests from a NSS backend, return NSS_NOTFOUND if
	 * looking up an nscd cache and the cached data is not from
	 * that backend.
	 */
	if ((flag & NSCDCACHEBIT) && (rphdr->nscdpriv != cphdr->nscdpriv)) {
		NSCD_SET_STATUS(rphdr, NSS_NOTFOUND, 0)

		_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
		(me, "looking up nscd cache but cache entry is from %s "
		    "and not %s\n",
		    NSCD_NSW_SRC_NAME(cphdr->nscdpriv),
		    NSCD_NSW_SRC_NAME(rphdr->nscdpriv));
		return (NSS_NOTFOUND);
	}

	/*
	 * Determine how much data to return. If looking up built-in
	 * nscd cache and the caller is a backend library, then also
	 * return the optional data. No need to return the optional
	 * data to a door client. The otional data is right after the
	 * regular cache data string, i.e., right after the '\0'. Backend
	 * cache data may not be string based, and all data should be
	 * returned.
	 */
	rlen = cpylen = cphdr->data_len;

	/* check and return the optional data if necessary */
	if (!(flag & BECACHEBIT)) {
		rlen = strlen((char *)cphdr + cphdr->data_off);
		if (rlen != cpylen) { /* there's optional data */
			if (flag & NSCDCACHEBIT) {
				/*
				 * Built-in cache lookup by backend,
				 * return optional data.
				 */
				has_optdata = B_TRUE;
			} else {
				/* Door client does not need optional data. */
				cpylen = rlen;
			}
		}
	}
	...
	/*
	 * Return NSS_ERROR if not enough room to copy result, but
	 * for requests from the backends, which run in nscd, realloc
	 * the buffer to send back the cached data.
	 */
	if (cpylen + 1 > rphdr->data_len) {
		if ((flag & NSCDCACHEBIT) || (flag & BECACHEBIT)) {
			void *tmp;
			nssuint_t newlen = rphdr->data_off + cpylen + 1;
			tmp = realloc(*rbufp, newlen);
			if (tmp == NULL) {
				NSCD_SET_STATUS(rphdr, NSS_NOTFOUND, 0)
				return (NSS_NOTFOUND);
			}
			*rbufp = tmp;
			rphdr = (nss_pheader_t *)(*rbufp);
			_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
			(me, "backend cache buffer reallocated, "
			    "old size is %ld, new size is %ld\n",
			    rphdr->data_off + rphdr->data_len, newlen);
		} else {
	...
	rphdr->data_len = cpylen;

	dst = (char *)rphdr + rphdr->data_off;
	(void) memcpy(dst, (char *)cphdr + cphdr->data_off, cpylen);
	/* some frontend code expects a terminating NULL char */
	dst[cpylen] = '\0';

	_NSCD_LOG_IF(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG) {
		if (flag & BECACHEBIT) {
			int i, j;
			char dbuf[1024];
			int max = sizeof (dbuf) - 1;
			char *buf = (char *)cphdr + cphdr->data_off;

			for (i = 0, j = 0; i < rlen && j < max; i++) {
				if (isprint(buf[i]))
					dbuf[j++] = buf[i];
				else if (j + 4 <= max) {
					/* for non-printables, print \xnn */
					(void) sprintf(dbuf + j, "\\x%02x",
					    (uint8_t)buf[i]);
					j += 4;
				}
			}
			...
			_nscd_logit(me, "BE cache (len = %lld): " ">>%s<<\n",
			    rlen, dbuf);
		} else {
			/* trace the string data */
			_nscd_logit(me,
			    "cache data (len = %lld, from %s): >>%s<<\n",
			    rlen, (cphdr->nscdpriv != -1) ?
			    NSCD_NSW_SRC_NAME(cphdr->nscdpriv) : "UNKNOWN",
			    dst);
		}

		if (has_optdata) {
			char *opt = (char *)cphdr + cphdr->data_off + rlen + 1;
			_nscd_logit(me, "optional cache data (len = %d): "
			    ">>%s<<\n", strlen(opt), opt);
		}
	}
...
check_config(nsc_lookup_args_t *largs, nscd_cfg_cache_t *cfgp,
	char *whoami, int flag, void (*psearch)(void **bufpp, size_t length))
{
	nsc_db_t	*nscdb;
	nsc_ctx_t	*ctx;
	nss_status_t	status;
	...
	*cfgp = nscdb->cfg;
	...
	if (cfgp->enable == nscd_false) {
		...
		if ((UPDATEBIT & flag) || (NSCDCACHEBIT & flag))
			return (NOTFOUND);
		else {
	...
	if (cfgp->pos_ttl <= 0 && cfgp->neg_ttl <= 0) {
		if ((flag & UPDATEBIT) || (flag & NSCDCACHEBIT))
			return (NOTFOUND);
...
/* NOTE: this definition exists in two places, nss:files_common.c */
 ...
 */
static int
repr_stat(const char *filename, struct stat *st)
{
	struct stat statf;
	int ret;
	int fd;
	...
	ret = fstatat(fd, LOCALFILE, &statf, 0);
	(void) close(fd);
	...
	char *whoami, time_t now)
{
	...
	if (repr_stat(ctx->file_name, &buf) == 0) {
		if (ctx->file_mtime == 0) {
...
static int
lookup_int_ext(nsc_lookup_args_t *largs, int flag, getkey_func getkey,
    psearch_func psearch)
{
	...
	status = (*getkey)(largs->buffer, largs->bufsize, &dbname,
	    &dbop, &args);
	...
	if (UPDATEBIT & flag) {
		_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
			(me, "%s: refresh start\n", whoami);
	} else if (NSCDCACHEBIT & flag) {
		_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
			(me, "%s: cache only lookup start\n", whoami);
	} else {
		_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
			(me, "%s: lookup start\n", whoami);
	}

	cfg_rc = check_config(largs, &cfg, whoami, flag, psearch);
	if (cfg_rc != CONTINUE)
		return (cfg_rc);
	...
	if (next_action == _NSC_NSLOOKUP) {
		...
		if (status == NSS_SUCCESS) {
			 ...
			 */
			status = (*getkey)(this_entry->buffer,
			    this_entry->bufsize,
			    &dbname, &dbop, &args);
			if (status != NSS_SUCCESS) {
				delete_entry(nscdb, ctx, this_entry);
		...
		if (NSCD_GET_STATUS((nss_pheader_t *)this_entry->buffer) ==
			...
			if ((nssrc = copy_result(&largs->buffer,
			    this_entry->buffer, flag)) != NSS_SUCCESS) {
				if (nssrc == NSS_NOTFOUND) {
					NSC_LOOKUP_RETURN(NOTFOUND, DEBUG,
					    "%s: positive entry in cache, but "
					    "not from the requested source\n")
				} else {
					NSC_LOOKUP_RETURN(SERVERERROR, ERROR,
					    "%s: response buffer "
					    "insufficient\n")
...
lookup_int(nsc_lookup_args_t *largs, int flag) {
	return (lookup_int_ext(largs, flag, nss_packed_getkey, nss_psearch));
}
...
nsc_lookup(nsc_lookup_args_t *largs, int flag) {
	...
	case SUCCESS:
		...
		/*
		 * the return in the macro makes a break unnecessary here,
		 * a break statement will be required if we replace the macro
		 * with a routine
		 */
...
lookup_cache(nsc_lookup_args_t *largs, nscd_cfg_cache_t *cfgp,
    nss_XbyY_args_t *argp, char *whoami, nsc_entry_t **entry, int flag)
{
	nsc_db_t	*nscdb;
	nsc_ctx_t	*ctx;
	uint_t		hash;
	...
	*entry = (nsc_entry_t *)avl_find(&nscdb->tree, &find_entry, &pos);
	...
	if (flag & NSCDCACHEBIT) {
		_NSCD_LOG(NSCD_LOG_CACHE, NSCD_LOG_LEVEL_DEBUG)
		(me, "%s: cache only requested, avoid name service\n", whoami);
		return (NSCD_DB_ENTRY_NOT_FOUND);
	}

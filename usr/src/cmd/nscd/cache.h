...
/*
 * OR'D in by server to return NSCD cache data only, no backend lookups.
 */
#define	NSCDCACHEBIT	(1<<29)

/*
 * OR'D in by server to look up a cache created by an NSS backend.
 */
#define	BECACHEBIT	(1<<28)
 ...
 */
#define	NSCDMAXNAMELEN	256

/*
 * Name of a cache created by NSS backends starts with ':'
 */
#define	NSCD_BACKEND_CACHE_NAME_FIRST_CHAR	':'
...
typedef struct nscd_admin_cfg_cache {
	char			name[64];
	nscd_cfg_cache_t	cfg;
} nscd_admin_cfg_cache_t;
...
typedef struct nsc_ctx {
	...
	nscd_bool_t	backend_cache;		/* created by nss backend */
} nsc_ctx_t;
...
/*
 * CACHE_CTX_COUNT is the total number of allowed caches
 * in nscd.  This includes both the nscd's built-in ones
 * listed below and and other dynamically created caches
 * that backends may attempt to create, assuming ctx
 * slots are still available.
 */

#define	CACHE_CTX_COUNT	32
...
/* Cache backend info */
extern void nsc_info(nsc_ctx_t *, char *, nscd_admin_cfg_cache_t acfg[],

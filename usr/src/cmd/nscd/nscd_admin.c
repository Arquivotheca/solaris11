...
_nscd_door_getadmin(void *outbuf)
{
	...
	for (i = 0; i < CACHE_CTX_COUNT; i++) {
		if (cache_ctx_p[i] != NULL) {
			...
			admin_c.cache_cfg[i].cfg = cache_ctx_p[i]->cfg;
			(void) strlcpy(admin_c.cache_cfg[i].name,
			    cache_name[i], sizeof (admin_c.cache_cfg[i].name));
			...
			admin_c.cache_cfg[i].cfg = cfg_default;
			(void) memset(&admin_c.cache_stats[i], 0,
			    sizeof (admin_c.cache_stats[0]));
			if (cache_name[i] != NULL) {
				(void) strlcpy(admin_c.cache_cfg[i].name,
				    cache_name[i],
				    sizeof (admin_c.cache_cfg[i].name));
			} else
				*admin_c.cache_cfg[i].name = '\0';
		}
	}
...
_nscd_server_setadmin(nscd_admin_mod_t *set)
{
	...
	for (i = 0; i < set->cache_cfg_num; i++) {
		...
		for (j = 0; j < CACHE_CTX_COUNT; j++) {
			if (cache_name[j] != NULL &&
			    strcmp(set->cache_cfg[i].name, cache_name[j]) == 0)
				break;
		}
		if (j == CACHE_CTX_COUNT)
			goto err_exit;

		new_cfg = &set->cache_cfg[i].cfg;
		dbname = cache_name[j];

		/*
		 * The way the configuration data is populated and handled
		 * is very different between the backend and built-in caches.
		 * Due to this inherent difference between the two,
		 * configuration updates are also processed differently.
		 * For backend caches, the current nscd_cfg_cache_t structure
		 * is replaced as a whole with the one send from the client.
		 * For built-in caches, the configuration data is processed
		 * via the _nscd_cfg_get_handle and _nscd_cfg_set functions,
		 * which only update the marked configuration data.
		 */
		if (cache_ctx_p[j]->backend_cache == nscd_true) {
			ctx = cache_ctx_p[j];
			(void) rw_wrlock(&ctx->cfg_rwlp);
			ctx->cfg = *new_cfg;
			ctx->cfg_mtime = time(NULL) + 1;
			(void) rw_unlock(&ctx->cfg_rwlp);
			rc = NSCD_SUCCESS;
			_NSCD_LOG(NSCD_LOG_ADMIN, NSCD_LOG_LEVEL_ERROR)
			(me, "cache %s config set successfully\n", dbname);
			continue;
		}
...
_nscd_add_admin_mod(char *dbname, char opt,
	...
	if ((cp = strchr(dbn, ',')) != NULL)
		*cp = '\0';
	/*
	 * ask for nscd daemon's admin data only once
	 */
	if (admin_mod.cache_cfg_num == 0 && admin_mod.cache_flush_num == 0 &&
	    _nscd_client_getadmin('g')) {
		(void) snprintf(msg, msglen,
		    gettext("unable to verify cache name \"%s\""), dbn);
		return (-1);
	}
	for (i = 0; i < CACHE_CTX_COUNT; i++) {
		if (strcmp(dbn, admin_c.cache_cfg[i].name) == 0)
			break;
	}
	...
	for (j = 0; j < admin_mod.cache_cfg_num; j++) {
		if (strcmp(admin_mod.cache_cfg[j].name, dbn) == 0)
			break;
	}
	cfg = &admin_mod.cache_cfg[j].cfg;
	...
	if (j == admin_mod.cache_cfg_num) {
		...
		/*
		 * Copy in all the config data returned by the server side.
		 * See the comment in _nscd_server_setadmin() for the
		 * reason why it's necessary.
		 */
		if (*dbn == NSCD_BACKEND_CACHE_NAME_FIRST_CHAR)
			*cfg = admin_c.cache_cfg[i].cfg;
		(void) strlcpy(admin_mod.cache_cfg[j].name, dbn,
		    sizeof (admin_mod.cache_cfg[j].name));

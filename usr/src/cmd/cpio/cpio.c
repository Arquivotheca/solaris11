...
static int		sl_remember_tgt(const struct stat *, int, int);
...
char	*myname,		/* program name */
	...
	**Pat_pp = 0,		/* Pattern strings */
	...
	eos = '\n';		/* Input filename end-of-string terminator */
...
file_out(void)
{
	...
	if (G_p->g_nlink == 1) {
		...
		if (l_p->L_cnt == l_p->L_gen.g_nlink)
			...
			return (FILE_LINKED); /* don't process data yet */
		}
	}
	if (Onecopy && G_p->g_attrnam_p == NULL) {
		tl_p = l_p;
...
/*
 * Reads bytes from the file stream fid into the array pointed to
 * by name, until maxlen-1 bytes are read, or the name separator sep
 * character is read or an end-of-file condition is encountered.
 * The string is then terminated with a newline and null byte.
 *
 * If an end-of-file condition was encountered, return NULL otherwise
 * return name.
 */

static char *
get_namestr(char *name, int maxlen, int sep, FILE *fid)
{
	int n = 0;	/* Current index into the name string */
	int ch = getc(fid);

	while ((ch != sep) && (ch != EOF) && (n < maxlen -1)) {
		name[n++] = ch;
		ch = getc(fid);
	}
	if (ch != EOF) {
...
getname(void)
{
	...
	while (!goodfile) {
		...
		while ((s = get_namestr(Gen.g_nam_p, APATH+1,
		    '\n', In_p)) != NULL) {
			lastchar = strlen(s) - 1;
			issymlink = 0;
	...
	case USTAR: /* TAR and USTAR */
		if (*Buffr.b_out_p == '\0') {
			*Gen.g_nam_p = '\0';
			...
			(void) sscanf(Thdr_p->tbuf.t_size, "%11llo", &size64);
			Gen.g_filesz = size64;
	...
	case TAR:
		if (*Buffr.b_out_p == '\0') {
			*Gen.g_nam_p = '\0';
			...
			(void) sscanf(Thdr_p->tbuf.t_size, "%llo", &size64);
			Gen.g_filesz = size64;
...
setup(int largc, char **largv)
{
...
#if defined(_PC_SATTR_ENABLED)
#ifdef WAITAROUND
	char	*opts_p = "zabcdfiklmoprstuvABC:DE:H:I:LM:O:PR:SV06@/";
#else
	char	*opts_p = "abcdfiklmoprstuvABC:DE:H:I:LM:O:PR:SV06@/";
#endif	/* WAITAROUND */
	...
	while ((option = getopt(largc, largv, opts_p)) != EOF) {
		...
		case 'V':	/* print a dot '.' for each file */
			Args |= OCV;
			break;
		case '0':	/* filenames terminated by a null character */
			eos = '\0';
			break;
...
usage(void)
{
	...
	(void) fprintf(stderr, gettext("USAGE:\n"
	    ...
	    "[-E file] [-H hdr] [-I file [-M msg]] "
	    "[-R id] [patterns]\n"
	    "\tcpio -o[acvABLV0] [-C size] "
	    "[-H hdr] [-O file [-M msg]]\n"
	    "\tcpio -p[adlmuvLV0] [-R id] directory\n"));
#endif
	(void) fflush(stderr);
	exit(EXIT_CODE);
}
...
read_bar_vol_hdr(void)
{
	...
	(void) sscanf(bar_Vhdr->dbuf.size, "%12llo", &size64);
	Gen_bar_vol.g_filesz = size64;
...
read_bar_file_hdr(void)
{
	...
	(void) sscanf(tmp_hdr->dbuf.size, "%12llo", &size64);
	Gen.g_filesz = size64;
...
preview_attrs(char *s, char *attrparent)
{
	...
	while (dp = readdir(dirp)) {
		...
		(void) sl_remember_tgt(&sb, islnk, rw_sysattr);
...
sl_preview_synonyms(void)
{
	...
	while ((s = get_namestr(buf, APATH+1, eos, In_p)) != NULL) {
		size_t lastchar;
		struct stat sb;
		...
		count = sl_remember_tgt(&sb, islnk, 0);

#if defined(O_XATTR)
		/*
		 * if count is greater than one, it means we have
		 * seen a link of this file before. We would have
		 * have retrieved and processed the extended attributes
		 * when first link is encountered. No need to process
		 * them again. Similary, if the is a symbolic link and
		 * we are not following symbolic links, no need to process
		 * extended attributes.
		 */

		if ((Atflag || SysAtflag) && (count == 1) &&
		    (!islnk || (Args & OCL))) {

...
smf_present () {
	[ -r $SMF_SYSVOL_FS/repository_door ] && \
	    [ ! -f $SMF_SYSVOL_FS/repository_door ]
}
...
smf_zonename() {
	echo "${SMF_ZONENAME:=`/usr/sbin/zonename`}"
}
...
#
smf_is_globalzone() {
	[ "${SMF_ZONENAME:=`/usr/sbin/zonename`}" = "global" ] && return 0
	return 1
}
...
#
#  Exits SMF_EXIT_ERR_FATAL if this is a non-global zone.  Useful as a
#  sanity indicator for services which are not expected to ever appear
#  in a non-global zone.
#
smf_assert_globalzone() {
	[ "${SMF_ZONENAME:=`/usr/sbin/zonename`}" = "global" ] && return 0
	echo "This service is not intended to appear in non-global zones." >&2
	echo "Exiting with a fatal error." \
            "The service will go to maintenance." >&2
...
#
smf_configure_ip() {
	[ "${SMF_ZONENAME:=`/usr/sbin/zonename`}" = "global" -o \
	 `/usr/sbin/zonename -t` = exclusive ] && return 0
	return 1
}
...
#
smf_dont_configure_ip() {
	[ "${SMF_ZONENAME:=`/usr/sbin/zonename`}" != "global" -a \
	 `/usr/sbin/zonename -t` = shared ] && return 0
	return 1
}
...
#
# Returns 0 (success) if is a reconfigure boot.
#
is_reconfigure_boot ()
{

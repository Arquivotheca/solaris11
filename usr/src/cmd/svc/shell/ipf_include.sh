...
[ -z "$SMF_SYSVOL_FS" ] && . /lib/svc/share/smf_include.sh

IPFILTER_FMRI="svc:/network/ipfilter:default"
RESTARTER_FMRI="svc:/system/svc/restarter:default"
...
service_is_enabled()
{
	# Incomplete instances are not considered enabled.
	svcprop -c -p general/complete $1 >/dev/null 2>&1 || return 1
...
{
	#
	# Do not process incomplete instances or system/restarter
	#
	svcprop -c -p general/complete $1 >/dev/null 2>&1 || return 1
	[ "$1" = "$RESTARTER_FMRI" ] && return 1
...
create_global_rules()
{
	...
	TEMP=`mktemp $SMF_SYSVOL_FS/ipf.conf.pid$$.XXXXXX`
	...
	*)
	...
	if [ "$GLOBAL_POLICY" = "allow" ]; then
		...
		#
		/usr/sbin/netstrategy | grep dhcp >/dev/null 2>&1
		if [ $? -eq 0 ]; then
...
create_global_ovr_rules()
{
	...
	TEMP=`mktemp $SMF_SYSVOL_FS/ipf_ovr.conf.pid$$.XXXXXX`

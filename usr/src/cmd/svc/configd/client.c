...
#define	AUDIT_OPERATION_IMPORT	"svccfg import"
#define	AUDIT_OPERATION_APPLY	"svccfg apply"
#define	AUDIT_OPERATION_ADMIN	"administrative"
...
static void
entity_avl_remove(repcache_client_t *cp, uint32_t id)
{
...
entity_setup(repcache_client_t *cp, struct rep_protocol_entity_setup *rpr)
{
	...
	ep->re_type = type;
	ep->re_show_masked = cp->rc_show_masked;
...
static int
entity_delete_undelete(repcache_client_t *cp,
    struct rep_protocol_entity_delete *rpr, enum rep_protocol_requestid action)
{
	repcache_entity_t *entity;
	...
	if (entity->re_changeid == rpr->rpr_changeid) {
		...
		result = rc_node_delete_undelete(&entity->re_node, action);
		if (result == REP_PROTOCOL_SUCCESS)
...
{
	return (entity_delete_undelete(cp, rpr, REP_PROTOCOL_ENTITY_DELETE));
}

/*
 * Clear a masked entity and it's decendents so they are again
 * visible.
 */
static int
entity_undelete(repcache_client_t *cp,
    struct rep_protocol_entity_delete *rpr)
{
	return (entity_delete_undelete(cp, rpr, REP_PROTOCOL_ENTITY_UNDELETE));
}

static int
entity_delcust(repcache_client_t *cp,
struct rep_protocol_entity_delete *rpr)
{
	return (entity_delete_undelete(cp, rpr, REP_PROTOCOL_ENTITY_DELCUST));
}

static int
bundle_remove(repcache_client_t *cp, struct rep_protocol_bundle_remove *rpr)
{
	repcache_entity_t *entity;
	...
	if (entity->re_changeid == rpr->rpr_changeid) {
		...
		result = rc_bundle_remove(&entity->re_node,
		    rpr->rpr_bname, NULL, 0, 0);
		if (result == REP_PROTOCOL_SUCCESS)
			entity->re_changeid = rpr->rpr_changeid;
	}
...
/*
 * Destroy an entity with no thought to muting the entity,
 * but actually remove the entity and decorations from the
 * repository.
 *
 * This however does not override keeping entities due to
 * snapshot linkage.
 */
static int
entity_remove(repcache_client_t *cp, struct rep_protocol_entity_delete *rpr)
{
	return (entity_delete_undelete(cp, rpr, REP_PROTOCOL_ENTITY_REMOVE));
}

static rep_protocol_responseid_t
entity_teardown(repcache_client_t *cp, struct rep_protocol_entity_teardown *rpr)
{
	entity_avl_remove(cp, rpr->rpr_entityid);
...
iter_setup(repcache_client_t *cp, struct rep_protocol_iter_request *rpr)
{
	...
	iter->ri_sequence = 0;
	iter->ri_show_masked = cp->rc_show_masked;
	iter_add(cp, iter);
    ...
    void *out_arg, size_t *outsz, void *arg)
{
	...
	case REPCACHE_TX_SETUP:
		if (rpr->rpr_request == REP_PROTOCOL_PROPERTYGRP_TX_COMMIT) {
			out->rpr_response = rc_tx_commit(&tx->re_node,
			    (char *)&rpr->rpr_pg_type, rpr->rpr_pg_flags,
			    rpr->rpr_cmd,
			    insz - REP_PROTOCOL_TRANSACTION_COMMIT_MIN_SIZE);
		} else {
			out->rpr_response = rc_bundle_remove(&tx->re_node,
			    (char *)&rpr->rpr_bname, rpr->rpr_cmd,
			    insz - REP_PROTOCOL_TRANSACTION_COMMIT_MIN_SIZE,
			    rpr->rpr_pg_action);
		}
...
/*ARGSUSED*/
static void
decoration_get_layer(repcache_client_t *cp, const void *in, size_t insz,
    void *out_arg, size_t *outsz, void *arg)
{
	const struct rep_protocol_decoration_request *rpr = in;
	struct rep_protocol_integer_response *out = out_arg;
	repcache_entity_t *ep;
	rep_protocol_decoration_layer_t t = REP_PROTOCOL_DEC_INVALID;
	...
	out->rpr_response = rc_node_get_decoration_layer(&ep->re_node, &t);
...
/*ARGSUSED*/
static void
decoration_get_type(repcache_client_t *cp, const void *in, size_t insz,
    void *out_arg, size_t *outsz, void *arg)
{
	property_get_type(cp, in, insz, out_arg, outsz, arg);
}

/*ARGSUSED*/
static void
decoration_get_value(repcache_client_t *cp, const void *in, size_t insz,
    void *out_arg, size_t *outsz, void *arg)
{
	property_get_value(cp, in, insz, out_arg, outsz, arg);
}

/*
 * Retrieve the path of the bundle file.
 *
 * Fails with:
...
/*ARGSUSED*/
static void
decoration_get_bundle_path(repcache_client_t *cp, const void *in, size_t insz,
    void *out_arg, size_t *outsz, void *unused)
{
	const struct rep_protocol_decoration_request *rpr = in;
	struct rep_protocol_path_response *out = out_arg;
	repcache_entity_t *ep;
	size_t sz = sizeof (out->rpr_path);
	...
	out->rpr_response = rc_node_get_bundle_name(&ep->re_node, out->rpr_path,
	    &sz);
	entity_release(ep);

	/*
	 * If we fail, we only return the response code.  If we succeed,
	 * rc_node_get_bundle_name() has given us the size of the path
	 * which we must include int outsz.
	 */
	if (out->rpr_response == REP_PROTOCOL_SUCCESS)
		*outsz = offsetof(struct rep_protocol_path_response,
		    rpr_path[sz + 1]);
	else
		*outsz = sizeof (out->rpr_response);
}
...
/*
 * This function sets rc_layer_id, rc_operation, rc_file members of the client
 * structure to inform the type of transaction to be performed. Decoration
 * information, rc_layer_id and rc_file, are used to decorate property values.
 * And annotation information, rc_operation and rc_file, are used for audit
 * events.
 *
 * Note that rep_protocol_decoration contains only decoration information, i.e.
 * bundle file and layer id. However, we can infer value for rc_operation from
 * layer id, e.g. REP_PROTOCOL_DEC_MANIFEST layer id implies the operation
 * is "svccfg import".
 *
 * Fails with
 *	_NO_RESOURCES - unable to allocate memory
 *	_BAD_REQUEST - invalid layer id
 */
static rep_protocol_responseid_t
set_tx_decoration(repcache_client_t *cp, struct rep_protocol_decoration *rpr)
{
	...
	/* Set file, layer id, and operation (derived from layer id) */
	if (rpr->rpr_file[0] != 0) {
		/*
	...
	switch (rpr->rpr_layer_id) {
	case REP_PROTOCOL_DEC_MANIFEST:
		if ((operation = strdup(AUDIT_OPERATION_IMPORT)) == NULL)
			goto out;
		break;
	case REP_PROTOCOL_DEC_SYSTEM_PROFILE:
	case REP_PROTOCOL_DEC_SITE_PROFILE:
		if ((operation = strdup(AUDIT_OPERATION_APPLY)) == NULL)
			goto out;
		break;
	case REP_PROTOCOL_DEC_ADMIN:
		if ((operation = strdup(AUDIT_OPERATION_ADMIN)) == NULL)
			goto out;
		break;
	default:
	...
	/* Grab rc_annotate_lock because annotation consumes these members */
	...
	cp->rc_layer_id = rpr->rpr_layer_id;
...
/*
 * This function turns on annotation for auditing events. set_tx_decoration()
 * sets annotation information, e.g. operation to be performed and file being
 * used, thus set_tx_decoration() must be called before set_annotation().
 *
 * Fails with
 *	_BAD_REQUEST - rc_operation and rc_file are not set
 */
/*ARGSUSED*/
static rep_protocol_responseid_t
set_annotation(repcache_client_t *cp, struct rep_protocol_request *rpr)
{
	au_id_t audit_uid;
	au_asid_t sessionid;
	 ...
	 */
	(void) pthread_mutex_lock(&cp->rc_annotate_lock);
	cp->rc_annotate = (cp->rc_operation != NULL) || (cp->rc_file != NULL);
	(void) pthread_mutex_unlock(&cp->rc_annotate_lock);
	...
	/*
	 * Native builds are done to create svc.configd-native.  This program
	 * runs only on Solaris build machines to create the seed repository.
	 * Until the SMF auditing code is distributed to Solaris build machines,
	 * adt_get_unique_id() in the following code is not a global function in
	 * libbsm.  Hence the following conditional compilation.
	 */
#ifndef	NATIVE_BUILD
	/*
...
/*ARGSUSED*/
static rep_protocol_responseid_t
unset_annotation(repcache_client_t *cp, struct rep_protocol_request *rpr)
{
	/* Unset annotation flag */
	(void) pthread_mutex_lock(&cp->rc_annotate_lock);
	cp->rc_annotate = 0;
	(void) pthread_mutex_unlock(&cp->rc_annotate_lock);
...
#ifndef	NATIVE_BUILD
	/*
	 * Annotation is done so restore our client audit session id.
	 */
	adt_set_asid(cp->rc_adt_session, cp->rc_adt_sessionid);
#endif	/* NATIVE_BUILD */
...
{
	repcache_client_t *cp = get_active_client();
	int rc = 0;
...
/*ARGSUSED*/
static void
entity_in_conflict(repcache_client_t *cp, const void *in, size_t insz,
    void *out_arg, size_t *outsz, void *arg)
{
	const struct rep_protocol_entity_request *rpr = in;
	struct rep_protocol_integer_response *out = out_arg;
	uint32_t r;

	repcache_entity_t *entity;

	entity = entity_find(cp, rpr->rpr_entityid);
	...
	out->rpr_response = rc_node_check_decoration_flag(&entity->re_node,
	    DECORATION_CONFLICT, &r);
...
/*
 * Returns:
 *	_FAIL_BAD_REQUEST	entity is not a service, instance, property
 *				group or property
 *	_FAIL_DELETED		entity has been deleted
 *	_FAIL_NOT_SET		client ID has not been associated with an
 *				entity
 *	_FAIL_UNKNOWN_ID	no entity for the specified client ID
 *	_SUCCESS
 */
/*ARGSUSED*/
static void
entity_is_masked(repcache_client_t *cp, const void *in, size_t insz,
    void *out_arg, size_t *outsz, void *arg)
{
	const struct rep_protocol_masked_request *rpr = in;
	struct rep_protocol_integer_response *out = out_arg;
	repcache_entity_t *ep;
	uint32_t masked_flag;
	...
	out->rpr_response = rc_node_check_decoration_flag(&ep->re_node,
	    DECORATION_MASK, &masked_flag);
...
#define	PROTO_PATH_OUT(p, f, in) {					\
		p, #p, &(f), NULL, NULL,				\
		    sizeof (in),					\
		    sizeof (struct rep_protocol_path_response), 0	\
	}
...
static struct protocol_entry {
	...
	PROTO(REP_PROTOCOL_ENTITY_UNDELETE,		entity_undelete,
	    struct rep_protocol_entity_delete),
	PROTO(REP_PROTOCOL_ENTITY_REMOVE,		entity_remove,
	    struct rep_protocol_entity_delete),
	PROTO(REP_PROTOCOL_ENTITY_DELCUST,		entity_delcust,
	    struct rep_protocol_entity_delete),
	PROTO(REP_PROTOCOL_BUNDLE_REMOVE,		bundle_remove,
	    struct rep_protocol_bundle_remove),
	...
	PROTO_VARIN(REP_PROTOCOL_PROP_BUNDLE_REMOVE,	tx_commit,
	    REP_PROTOCOL_TRANSACTION_COMMIT_MIN_SIZE),
	...
	PROTO(REP_PROTOCOL_SET_AUDIT_ANNOTATION,	set_annotation,
	    struct rep_protocol_request),
	PROTO(REP_PROTOCOL_UNSET_AUDIT_ANNOTATION,	unset_annotation,
	    struct rep_protocol_request),

	PROTO(REP_PROTOCOL_SET_TX_DECORATION,		set_tx_decoration,
	    struct rep_protocol_decoration),

	PROTO(REP_PROTOCOL_SWITCH,			repository_switch,
	    struct rep_protocol_switch_request),

	PROTO_UINT_OUT(REP_PROTOCOL_DECORATION_GET_LAYER, decoration_get_layer,
	    struct rep_protocol_decoration_request),
	PROTO_UINT_OUT(REP_PROTOCOL_DECORATION_GET_TYPE, decoration_get_type,
	    struct rep_protocol_property_request),
	PROTO_VALUE_OUT(REP_PROTOCOL_DECORATION_GET_VALUE, decoration_get_value,
	    struct rep_protocol_decoration_request),
	PROTO_PATH_OUT(REP_PROTOCOL_DECORATION_GET_BUNDLE,
	    decoration_get_bundle_path, struct rep_protocol_decoration_request),

	PROTO_UINT_OUT(REP_PROTOCOL_ENTITY_INCONFLICT,	entity_in_conflict,
	    struct rep_protocol_entity_request),
	PROTO_UINT_OUT(REP_PROTOCOL_ENTITY_IS_MASKED, entity_is_masked,
	    struct rep_protocol_masked_request),
...
int
create_client(pid_t pid, uint32_t debugflags, uint32_t show_masked,
    int privileged, int *out_fd)
{
	int fd;
	...
	cp->rc_show_masked = show_masked;

	/*
	 * Set decoration layer id to REP_PROTOCOL_DEC_ADMIN because
	 * transactions are considered administrative changes unless explictly
	 * set to manifest or profile operations.
	 */
	cp->rc_layer_id = REP_PROTOCOL_DEC_ADMIN;

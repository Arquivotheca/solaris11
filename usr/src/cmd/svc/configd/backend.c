...
/*
 * This picture represents the schema of the non-volatile repository.  The
 * volatile repository only has id_tbl, pg_tbl prop_lnk_tbl, schema_version
 * and value_tbl tables.
 *
 * +--------------+          +------------------+        +------------------+
 * | service_tbl  |          | instance_tbl     |        | snapshot_lnk_tbl |
 * |--------------|          |------------------|        |------------------|
 * |*svc_id       |<----+--->|*instance_id      |<----|  |*lnk_id           |
 * |svc_name      | \   |    |instance_name     |     |--|lnk_inst_id       |
 * |svc_conflict  |  \--|----|instance_svc      |        |lnk_snap_name     |
 * |   _cnt       |     |    |instance_conflict |    /---|lnk_snap_id       |
 * |svc_dec_id    |     |    |   _cnt           |   /    +------------------+
 * +--------------+     |    |instance_dec_id   |  /
 *     +------------+   |    +------------------+ |  +----------------------+
 *     |  pg_tbl    |   |                         |  |    snaplevel_tbl     |
 *     |------------|   |      +--------+         |  |----------------------|
 *  |--|*pg_id      |   |      | id_tbl |         |->|snap_id               |
 *  |  |pg_parent_id|---|      |--------|            |snap_level_num        |
 *  |  |pg_name     |          |id_name |         |->|snap_level_id         |
 *  |  |pg_type     |          |id_next |         |  |snap_level_service_id |
 *  |  |pg_flags    |          +--------+         |  |snap_level_service    |
 *  +--|pg_gen_id   |                             |  |snap_level_instance_id|
 *  |  |pg_dec_id   |                             |  |snap_level_instance   |
 *  |  +------------+                             |  +----------------------+
 *  |                                             |
 *  |     +----------------------+                |  +--------------------+
 *  |     |     prop_lnk_tbl     |                |  | snaplevel_lnk_tbl  |
 *  |     |----------------------|                |  |--------------------|
 *  |     |*lnk_prop_id          |                |--|*snaplvl_level_id   |
 *  |--->>|lnk_pg_id             |<<-----+------+----|snaplvl_pg_id       |
 *  |--->>|lnk_gen_id          - |<<-----|      |    |snaplvl_pg_name     |
 *        |lnk_prop_name        \|              |    |snaplvl_pg_type     |
 *  |-----|lnk_prop_type         |\------|      |    |snaplvl_pg_flags    |
 *  |  |--|lnk_val_id            |       |      |----|snaplvl_gen_id      |
 *  |  |  |lnk_decoration_id     |---|   |           |snaplvl_dec_id      |
 ...
 *     |                   |       | |     |----------------------|     |
 *     |  +-----------+    |       |------>|decoration_key        |     |
 *     |  | value_tbl |    |         |---->|decoration_id         |     |
 *     |  |-----------|    --------------->|decoration_entity_type|     |
 *     -->|value_id   |------------------->|decoration_value_id   |     |
 *        |value_value|                    |decoration_gen_id     |<----|
 *        |value_order|                    |decoration_layer      |
 *        +-----------+             |------|decoration_bundle_id  |
 *                                  |      |decoration_type       |
 *        +----------------+        |      |decoration_flags      |
 *        |   bundle_tbl   |        |      |decoration_tv_sec     |
 *        |----------------|        |      |decoration_tv_usec    |
 *        |bundle_id       |<-------|      +----------------------+
 *        |bundle_name     |
 *        |bundle_timestamp|
...
typedef enum be_flight_type {
	...
	BE_FLIGHT_EV_CHECKPOINT_EXISTS,	/* Existing checkpoint detected on */
					/* restart */
	BE_FLIGHT_EV_INTEGRITY_INIT,	/* Start repo integrity check */
	BE_FLIGHT_EV_INTEGRITY_FINI,	/* Finish repo integrity check */
	...
	BE_FLIGHT_EV_REPO_UPGRADE_INIT,	/* Repository upgrade needed */
	BE_FLIGHT_EV_REPO_UPGRADE_FINI,	/* Repository upgrade result */
...
typedef enum be_flight_status {
	...
	BE_FLIGHT_ST_INTEG_BACKGROUND,	/* Integrity check done in background */
	BE_FLIGHT_ST_INTEG_FOREGROUND,	/* Integrity check done in foreground */
 ...
 */
#define	BACKEND_SCHEMA_VERSION		8

#define	NO_SCHEMA_VERSION "no such table: schema_version"
static struct backend_tbl_info tbls_normal[] = { /* BACKEND_TYPE_NORMAL */
	 ...
	 */
	{
		...
		"snaplvl_dec_id INTEGER NOT NULL"
	},
...
static struct backend_idx_info idxs_normal[] = { /* BACKEND_TYPE_NORMAL */
	...
	{ "service_tbl",	"dec", "svc_dec_id" },
	{ "instance_tbl",	"name",	"instance_svc, instance_name" },
	{ "instance_tbl",	"dec", "instance_dec_id" },
	...
	{ "snaplevel_lnk_tbl",	"pg_gen", "snaplvl_pg_id, snaplvl_gen_id" },
	{ "decoration_tbl",	"base", "decoration_id, decoration_gen_id" },
	{ "decoration_tbl",	"id", "decoration_id" },
	{ "decoration_tbl",	"val",	"decoration_value_id" },
	{ "decoration_tbl",	"id_gen", "decoration_id, decoration_gen_id" },
	{ "decoration_tbl",	"id_gen_lyr", "decoration_id, "
	    "decoration_gen_id, decoration_layer" },
	{ "bundle_tbl",		"name", "bundle_name" },
	{ NULL, NULL, NULL }
};
...
static struct backend_tbl_info tbls_common[] = { /* all backend types */
	...
	/*
	 * value_tbl maps a value_id to a set of values.
	 */
	{
		"value_tbl",
...
static struct backend_idx_info idxs_common[] = { /* all backend types */
	...
	{ "pg_tbl",		"id_gen", "pg_id, pg_gen_id" },
	{ "pg_tbl",		"dec", "pg_dec_id" },
	{ "prop_lnk_tbl",	"base",	"lnk_pg_id, lnk_gen_id" },
	{ "prop_lnk_tbl",	"val",	"lnk_val_id" },
	{ "prop_lnk_tbl",	"dec",	"lnk_decoration_id" },
	{ "prop_lnk_tbl",	"gen_dec", "lnk_gen_id, lnk_decoration_id"},
...
/*ARGSUSED*/
static int
run_single_str_callback(void *arg, int columns, char **vals, char **names)
{
	struct run_single_str_info *info = arg;

	assert(info->rs_result != REP_PROTOCOL_SUCCESS);
	assert(columns == 1);

	if ((*info->rs_out = strdup(vals[0])) == NULL)
		info->rs_result = REP_PROTOCOL_FAIL_NO_RESOURCES;
...
/*
 * The result from a backend_tx_run() with this as the call back function
 * is :
 * 	REP_PROTOCOL_SUCCESS if there are NO entries.
 * 	REP_PROTOCOL_DONE if there are entries.
 */
/*ARGSUSED*/
int
...
#define	REPO_UPGRADE "repo_upgrade"
#define	PROG_REPO_UPGRADE "/lib/svc/bin/" REPO_UPGRADE

void
do_upgrade(const char *db_path)
{
	pid_t pid;
	int status;
	int ctfd;
	...
	/*
	 * We fork repo_upgrade in a separate contract so that startd is not
	 * confused by an exit event in configd's process contract.
	 */
	...
	/*
	 * We decorate repo_upgrade process contract so that repo_upgrade can
	 * figure out who's invoked repo_upgrade and in which context.
	 */
	if ((err = ct_pr_tmpl_set_svc_aux(ctfd, CT_AUX_REPO_UPGRADE)) != 0) {
		configd_critical("Could not set process contract term: %s\n",
		    strerror(err));
		return;
	}
	if ((err = ct_tmpl_activate(ctfd)) != 0) {
		configd_critical("Could not activate repo_upgrade process "
		    "contract template: %s\n", strerror(err));
		return;
	}
	...
	if (pid > 0) {
		...
		/*
		 * If repo_upgrade returned a non-zero value,
		 * our upgrade failed.
		 */
		if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
			configd_critical("Failed repo_upgrade\n");
			return;
		}

		return;
	} else if (pid == -1) {
		configd_critical("fork in do_upgrade failed: %s\n",
		    strerror(errno));
		abort();
	}

	(void) execl(PROG_REPO_UPGRADE, REPO_UPGRADE, db_path, NULL);
}

/*
 * Check schema_version of db_path.
 * Return 1 if repository matches the schema_version
 *        0 on no match
 *        -1 on error
 */
int
check_schema_version(const char *db_path)
{
	sqlite *db;
	char *emsg;
	int r;
	...
	if (db_path == NULL)
		db_path = REPOSITORY_DB;

	if ((db = sqlite_open(db_path, 0600, &emsg)) == NULL) {
		configd_critical("Failed to open repository at %s: %s\n",
		    db_path, emsg);
		return (-1);
	}
	...
	r = sqlite_exec(db, "SELECT schema_version FROM schema_version;",
	...
	case SQLITE_OK:
		...
		break;
	case SQLITE_ERROR:
		if (strcmp(NO_SCHEMA_VERSION, emsg) == 0)
			/*
			 * if there is no schema_version, we don't know how
			 * to upgrade.
			 * It can also be that there is no repository, so we
			 * give configd a chance to build it.
			 */
			return (1);
		/*FALLTHROUGH*/
	default:
		configd_critical("Failed to get SMF repository version: %s\n",
		    emsg);
		return (-1);
	}
...
void
check_upgrade(const char *db_path)
{
	int r;

	/*
	 * Set up our temporary sqlite directory before the first
	 * repository access, in check_schema_version function.
	 */
	sqlite_temp_directory = _PATH_SYSVOL;

	if ((r = check_schema_version(db_path)) == 0) {
		flight_recorder_event(BE_FLIGHT_EV_REPO_UPGRADE_INIT,
		    BE_FLIGHT_ST_INFO);
		do_upgrade(db_path);

		if (check_schema_version(db_path) != 1) {
			configd_critical("SMF repository upgrade failed.\n");
			flight_recorder_event(BE_FLIGHT_EV_REPO_UPGRADE_FINI,
			    BE_FLIGHT_ST_FAIL);
		}
		flight_recorder_event(BE_FLIGHT_EV_REPO_UPGRADE_FINI,
		    BE_FLIGHT_ST_SUCCESS);
	} else if (r == -1) {
		configd_critical("Failed to check SMF repository version.\n");
	}
}

/*
 * Must pass both source and copy file names for background
 * integrity check.
 */
#define	INTEGRITY_CHECK_COPY	0	/* File to check */
#define	INTEGRITY_CHECK_SRC	1	/* File to be copied */
#define	INTEGRITY_CHECK_COUNT	2	/* Size of array */
...
static void
backend_report_integrity_failure(char *integrity_results,
    backend_type_t backend_id, const char *db_file, const char *src_file)
{
	const char *fname = _PATH_SYSVOL "/db_errors";
	char *src_file_text = NULL;
	int fd;

	if (src_file)
		src_file_text = uu_msprintf(" (copied from %s)", src_file);
	...
	if ((fd = open(fname, O_CREAT|O_WRONLY|O_APPEND, 0600)) < 0) {
		...
		if (backend_fd_write(fd, "\n\n") < 0 ||
		    ...
		    backend_fd_write(fd, src_file_text) ||
		    backend_fd_write(fd,
	...
	if (!is_main_repository ||
		...
		if (fname != NULL)
			configd_critical(
			    "%s%s: integrity check failed. Details in "
			    "%s\n", db_file, src_file_text, fname);
		else
			configd_critical(
			    "%s%s: integrity check failed.\n",
			    db_file, src_file_text);
	} else {
		(void) fprintf(stderr,
"\n"
...
static int
backend_integrity_check(struct sqlite *db, backend_type_t backend_id,
    const char *db_file, const char *src_file)
{
	char *errp = NULL;
	char *integrity_results = NULL;
	int r = BACKEND_CREATE_SUCCESS;

	/*
	 * run an integrity check
	 */
	r = sqlite_exec(db, "PRAGMA integrity_check;",
	    backend_integrity_callback, &integrity_results, &errp);

	if (r == SQLITE_BUSY || r == SQLITE_LOCKED) {
		free(errp);
		free(integrity_results);
	...
	if (r == SQLITE_ABORT) {
		...
		errp = NULL;
		integrity_results =
		    strdup("out of memory running integrity check\n");
	...
	if (integrity_results != NULL) {
		backend_report_integrity_failure(integrity_results,
		    backend_id, db_file, src_file);
		free(integrity_results);
		r = BACKEND_CREATE_FAIL;
	}
...
static void *
backend_integrity_thread(void *arg)
{
	char *errp;
	struct sqlite *db;
	char **db_names;
	...
	db_names = (char **)arg;
	db = sqlite_open(db_names[INTEGRITY_CHECK_COPY], 0600, &errp);
	if (db == NULL) {
		backend_report_integrity_failure(errp, BACKEND_TYPE_NORMAL,
		    db_names[INTEGRITY_CHECK_COPY],
		    db_names[INTEGRITY_CHECK_SRC]);
		free(errp);
		/* backend_panic() causes configd to exit or abort. */
		backend_panic("Could not open %s for background integrity "
		    "check.\n", db_names[INTEGRITY_CHECK_COPY]);
	}
	flight_recorder_event(BE_FLIGHT_EV_INTEGRITY_INIT,
	    BE_FLIGHT_ST_INTEG_BACKGROUND);
	r = backend_integrity_check(db, BACKEND_TYPE_NORMAL,
	    db_names[INTEGRITY_CHECK_COPY], db_names[INTEGRITY_CHECK_SRC]);
	flight_recorder_event(BE_FLIGHT_EV_INTEGRITY_FINI,
	    BE_FLIGHT_ST_INTEG_BACKGROUND);
	sqlite_close(db);
	(void) unlink(db_names[INTEGRITY_CHECK_COPY]);

	uu_free(db_names[INTEGRITY_CHECK_SRC]);
	uu_free(db_names[INTEGRITY_CHECK_COPY]);
	uu_free(db_names);

	if (r != BACKEND_CREATE_SUCCESS) {
		/* backend_panic() causes configd to exit or abort. */
		backend_panic("Background integrity check failed.\n");
	}
	return (NULL);
}

static rep_protocol_responseid_t
backend_background_integrity_check(const char *db_file)
{
	char **db_names;
	struct stat sbuf;
	pthread_t tid;
	...
	assert(strcmp(db_file, REPOSITORY_DB) == 0);

	/*
	 * Copy the repository before starting the thread.  It doesn't take
	 * long and we can report immediately if there are problems in
	 * making the copy.
	 *
	 * First, check to see if INTEGRITY_DB already exists.  If it does,
	 * we've been restarted and don't need to copy it again.
	 */
	if (stat(INTEGRITY_DB, &sbuf) != 0) {
		r = backend_copy_repository(db_file, INTEGRITY_DB, 0);
		if (r != REP_PROTOCOL_SUCCESS)
			return (r);
	}

	db_names = uu_zalloc(INTEGRITY_CHECK_COUNT * sizeof (const char *));
	if (db_names == NULL)
		return (REP_PROTOCOL_FAIL_NO_RESOURCES);
	db_names[INTEGRITY_CHECK_SRC] = uu_strdup(db_file);
	db_names[INTEGRITY_CHECK_COPY] = uu_strdup(INTEGRITY_DB);
	if ((db_names[INTEGRITY_CHECK_SRC] == NULL) ||
	    (db_names[INTEGRITY_CHECK_COPY] == NULL)) {
		uu_free(db_names[INTEGRITY_CHECK_SRC]);
		uu_free(db_names[INTEGRITY_CHECK_COPY]);
		uu_free(db_names);
		return (REP_PROTOCOL_FAIL_NO_RESOURCES);
	...
	tr = pthread_create(&tid, NULL, backend_integrity_thread,
	    (void *) db_names);
	if (tr == 0) {
		/* Thread will free db_names */
		tr = pthread_detach(tid);
		assert(tr == 0);
		return (REP_PROTOCOL_SUCCESS);
	}
	uu_free(db_names[INTEGRITY_CHECK_SRC]);
	...
	case EAGAIN:
		...
		break;
	case EPERM:
		r = REP_PROTOCOL_FAIL_PERMISSION_DENIED;
		break;
	default:
		r = REP_PROTOCOL_FAIL_BAD_REQUEST;
		break;
	}
    ...
    sqlite_backend_t **bep)
{
	...
	r = sqlite_exec(be->be_db, "SELECT schema_version FROM schema_version;",
	    run_single_int_callback, &info, &errp);
	if (r == SQLITE_ERROR &&
	    strcmp(NO_SCHEMA_VERSION, errp) == 0) {
		free(errp);
		/*
	...
	if (is_main_repository && (backend_id == BACKEND_TYPE_NORMAL)) {
		/* Check the main persistent repository in the background */
		r = backend_background_integrity_check(db_file);
		if (r != REP_PROTOCOL_SUCCESS) {
			/*
			 * Either we couldn't copy the repository or we
			 * couldn't start the thread.  Either way, we'll go
			 * ahead and do the integrity check in the
			 * foreground.
			 */
			flight_recorder_event(BE_FLIGHT_EV_INTEGRITY_INIT,
			    BE_FLIGHT_ST_INTEG_FOREGROUND);
			r = backend_integrity_check(be->be_db, backend_id,
			    db_file, NULL);
			flight_recorder_event(BE_FLIGHT_EV_INTEGRITY_FINI,
			    BE_FLIGHT_ST_INTEG_FOREGROUND);
		}
	} else {
		/* Otherwise do integrity check in the foreground. */
		flight_recorder_event(BE_FLIGHT_EV_INTEGRITY_INIT,
		    BE_FLIGHT_ST_INTEG_FOREGROUND);
		r = backend_integrity_check(be->be_db, backend_id, db_file,
		    NULL);
		flight_recorder_event(BE_FLIGHT_EV_INTEGRITY_FINI,
		    BE_FLIGHT_ST_INTEG_FOREGROUND);
	}
...
int
backend_tx_run_single_str(backend_tx_t *tx, backend_query_t *q, char **c)
{
	struct run_single_str_info info;
	int ret;

	info.rs_out = c;
	info.rs_result = REP_PROTOCOL_FAIL_NOT_FOUND;

	ret = backend_tx_run(tx, q, run_single_str_callback, &info);
...
void
backend_query_reset(backend_query_t *q)
{
	if (q == NULL)
		return;

	if (q->bq_buf != NULL)
		bzero(q->bq_buf, q->bq_size);
}
...
/*
 * Conflict marking and removal
 *
 * Nothing should ever call mark_decoration_conflict() directly without
 * going through backend_mark_xx_conflict().  Because the latter will make
 * sure that all parents are correctly marked.
 *
 * The service is marked when any children are in conflict and is always
 * an inherited conflict.
 *
 * The instance is marked when it's in conflict or a child is in conflict.
 * The pg is in conflict only when it's in conflict.
 * The property is in conflict only when it's in conflict.
 *
 * So really what we need to do is distinguish between an instance being
 * in actual conflict or it being inherited.  I think this can be deduced
 * from the actual data during conflict investigation.
 */
/* ARGSUSED */
static int
mark_decoration_conflict(void *arg, int columns, char **vals, char **names)
{
	struct conflict *cinfo = arg;
	uint32_t cur_decflags;

	string_to_id(vals[0], &cur_decflags, names[0]);

	if (cur_decflags & DECORATION_CONFLICT) {
		cinfo->in_conflict++;
		return (BACKEND_CALLBACK_CONTINUE);
	}

	backend_query_add(cinfo->q, "UPDATE decoration_tbl "
	    "SET decoration_flags = %d WHERE decoration_key = %q; ",
	    cur_decflags | DECORATION_CONFLICT, vals[1]);

	return (BACKEND_CALLBACK_CONTINUE);
}

static void
backend_mark_svc_conflict(backend_tx_t *tx, uint32_t svc_id,
    backend_query_t *propq)
{
	struct conflict cinfo;
	...
	if (propq)
		...
		cinfo.q = backend_query_alloc();

	backend_query_add(q, "SELECT decoration_flags, decoration_key "
	    "FROM decoration_tbl WHERE decoration_id = (SELECT svc_dec_id "
	    "    FROM service_tbl WHERE svc_id = %d); ",
	    svc_id);

	ret = backend_tx_run(tx, q, mark_decoration_conflict, &cinfo);

	if (ret == REP_PROTOCOL_SUCCESS) {
		backend_query_add(cinfo.q, "UPDATE service_tbl "
		    "SET svc_conflict_cnt = svc_conflict_cnt + 1 "
		    "WHERE (svc_id = %d); ", svc_id);

		if (propq == NULL)
			(void) backend_tx_run(tx, cinfo.q, NULL, NULL);
	}

	if (propq == NULL)
		backend_query_free(cinfo.q);

	backend_query_free(q);
}

static void
backend_mark_inst_conflict(backend_tx_t *tx, uint32_t inst_id,
    backend_query_t *propq)
{
	struct conflict cinfo;
	backend_query_t *q = backend_query_alloc();
	...
	backend_query_add(q, "SELECT decoration_flags, decoration_key "
	    "FROM decoration_tbl WHERE decoration_id = (SELECT instance_dec_id "
	    "    FROM instance_tbl WHERE instance_id = %d); ",
	    inst_id);

	ret = backend_tx_run(tx, q, mark_decoration_conflict, &cinfo);

	if (ret == REP_PROTOCOL_SUCCESS) {
		backend_query_add(cinfo.q, "UPDATE instance_tbl "
		    "SET instance_conflict_cnt = instance_conflict_cnt + 1 "
		    "WHERE (instance_id = %d); ", inst_id);

		if (propq == NULL)
			ret = backend_tx_run(tx, cinfo.q, NULL, NULL);
	}
...
void
backend_mark_pg_parent(backend_tx_t *tx, uint32_t pg_id,
    backend_query_t *propq)
{
	backend_query_t *q = backend_query_alloc();
	uint32_t pid;
	...
	(void) backend_tx_run_single_int(tx, q, &pid);

	backend_query_reset(q);

	backend_query_add(q, "SELECT 1 FROM instance_tbl "
	    "WHERE instance_id = %d ", pid);

	if (backend_tx_run(tx, q, backend_fail_if_seen, NULL) ==
	    REP_PROTOCOL_SUCCESS)
		backend_mark_svc_conflict(tx, pid, propq);
	else
		backend_mark_inst_conflict(tx, pid, propq);

	backend_query_free(q);
}

void
backend_mark_pg_conflict(backend_tx_t *tx, uint32_t layer, uint32_t pg_id)
{
	struct conflict cinfo;
	backend_query_t *q = backend_query_alloc();
	int ret;
	...
	/*
	 * If the property group was already in conflict there is
	 * no need to mark the parent again.
	 */
	if (cinfo.in_conflict) {
		backend_query_free(q);
		return;
	}

	/*
	 * If there are higher layers that are not in conflict
	 * there is no need to put the service/instance in conflict
	 * and therefore cause the service/instance to go into
	 * maintenance.
	 */
	backend_query_reset(q);
	...
	backend_mark_pg_parent(tx, pg_id, NULL);
}

/*ARGSUSED*/
static int
clear_decoration_conflict(void *arg, int columns, char **vals, char **names)
{
	struct conflict *cinfo = arg;
	uint32_t cur_decflags;
	...
	if ((cur_decflags & DECORATION_CONFLICT) == 0)
		return (BACKEND_CALLBACK_CONTINUE);
...
/*
 * Decrement the counter by 1, if we are at 0 then mark all decorations out
 * of conflict.
 */
static void
backend_clear_svc_conflict(backend_tx_t *tx, uint32_t svc_id,
    backend_query_t *propq)
{
	backend_query_t *q = backend_query_alloc();
	conflict_t cinfo;
	...
	backend_query_add(q, "SELECT svc_conflict_cnt FROM service_tbl "
	    "WHERE (svc_id = %d); "
	    "UPDATE service_tbl "
	    "    SET svc_conflict_cnt = svc_conflict_cnt - 1 "
	    "        WHERE (svc_id = %d); ", svc_id, svc_id);

	ret = backend_tx_run_single_int(tx, q, &ccnt);
	...
	backend_query_reset(q);
	backend_query_add(q, "SELECT decoration_flags, decoration_key "
	    "FROM decoration_tbl WHERE decoration_id = (SELECT svc_dec_id FROM "
	    "    service_tbl WHERE svc_id = %d); ",
	    svc_id);

	ret = backend_tx_run(tx, q, clear_decoration_conflict, &cinfo);
 ...
 */
static void
backend_clear_inst_conflict(backend_tx_t *tx, uint32_t inst_id,
    backend_query_t *propq)
{
	backend_query_t *q = backend_query_alloc();
	conflict_t cinfo;
	...
	backend_query_add(q, "SELECT instance_conflict_cnt FROM instance_tbl "
	    "WHERE (instance_id = %d); "
	    "UPDATE instance_tbl "
	    "    SET instance_conflict_cnt = instance_conflict_cnt - 1 "
	    "        WHERE (instance_id = %d); ", inst_id, inst_id);
...
void
backend_clear_pg_parent(backend_tx_t *tx, uint32_t pg_id,
    backend_query_t *propq)
{
	backend_query_t *q = backend_query_alloc();
	uint32_t pid;
	...
	if (backend_tx_run(tx, q, backend_fail_if_seen, NULL) ==
	    REP_PROTOCOL_SUCCESS)
		backend_clear_svc_conflict(tx, pid, propq);
	else
		backend_clear_inst_conflict(tx, pid, propq);

	backend_query_free(q);
}

/*
 * remove the conflict flag from the decorations, and then call a clear
 * for the parent.
 */
void
backend_clear_pg_conflict(backend_tx_t *tx, uint32_t layer, uint32_t pg_id)
{
	struct conflict cinfo;
	backend_query_t *q = backend_query_alloc();
	int ret;
	...
	/*
	 * If there are higher layers that are not in conflict
	 * there is no need to clear the service/instance as
	 * this conflict would not have be accounted for in
	 * the conflict counts.
	 *
	 * The admin change took the conflict out of place
	 * should have removed the conflict.
	 */
	backend_query_reset(q);
	backend_query_add(q, "SELECT 1 FROM decoration_tbl "
	    "WHERE decoration_layer > %d AND "
	...
	backend_clear_pg_parent(tx,  pg_id, NULL);
}

void
backend_carry_delcust_flag(backend_tx_t *tx, uint32_t decid, uint32_t layer,
    uint32_t *decflags)
{
	backend_query_t *q;
	uint32_t flags;
	...
	if (layer == REP_PROTOCOL_DEC_ADMIN) {
		*decflags &= ~DECORATION_DELCUSTED;
		return;
	}
	...
	backend_query_add(q, "SELECT decoration_flags FROM decoration_tbl "
	    ...
	    decid, REP_PROTOCOL_DEC_ADMIN);

	r = backend_tx_run_single_int(tx, q, &flags);

	if (r == REP_PROTOCOL_SUCCESS && (flags & DECORATION_DELCUSTED))
		*decflags |= DECORATION_DELCUSTED;
	else
		*decflags &= ~DECORATION_DELCUSTED;
}

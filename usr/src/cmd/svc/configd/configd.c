    ...
    size_t rp_size, int *out_fd)
{
	...
	if (privileged_pid != 0) {
		...
		if (ucred_getpid(uc) != privileged_pid ||
		    pread(privileged_psinfo_fd, &info, sizeof (info), 0) !=
		    sizeof (info))
...
#if (REPOSITORY_DOOR_FLAG_ALL != \
	(REPOSITORY_DOOR_FLAG_DEBUG | REPOSITORY_DOOR_FLAG_SHOW_MASKED))
#error Need to update flag checks
	...
	if (flags & REPOSITORY_DOOR_FLAG_SHOW_MASKED)
		show_masked_flag = 1;

#undef GET_ARG

	return (create_client(ucred_getpid(uc), debugflags, show_masked_flag,
	    privileged, out_fd));
}
...
main(int argc, char *argv[])
{
	...
	/*
	 * before we forfeit the privileges of forking a child,
	 * we check for repository format upgrades, because
	 * check_upgrade may fork/exec to handle the upgrade
	 * process.
	 *
	 * We also set sqlite_temp_directory to _PATH_SYSVOL in check_upgrade,
	 * before the first db access. This is necessary for read-only /,
	 * e.g. miniroot. Future changes that access db before check_upgrade
	 * should make sure to set sqlite_temp_directory accordingly.
	 */
	check_upgrade(dbpath);

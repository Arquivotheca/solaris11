 ...
 * allows us to return the property name even if the file doesn't exist.
 *
 * svccfg uses the deathrow set to true when collecting the manifest file
 * for bundle information but has already realpath'ed the file.
 */
char *
...
/*
 * int mhash_test_file(scf_handle_t *, const char *, char **, uchar_t *)
 ...
 */
int
mhash_test_file(scf_handle_t *hndl, const char *file, char **pnamep,
    uchar_t *hashbuf)
{
	boolean_t do_hash;
	struct stat64 st;

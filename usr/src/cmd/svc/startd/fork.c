...
fork_mount(char *path, char *opts)
{
	...
	(void) execl("/usr/sbin/mount", "mount", "-o", opts, path, NULL);
...
fork_sulogin(boolean_t immediate, const char *format, ...)
{
	...
	(void) execl("/usr/sbin/sulogin", "sulogin", NULL);
...
retry:
	 ...
	 * _PATH_SYSVOL directory, named after the PID to aid in debugging.
	 */
	(void) snprintf(path, sizeof (path), _PATH_SYSVOL "/core.configd.%%p");
...
fork_rc_script(char rl, const char *arg, boolean_t wait)
{
	pid_t pid;
	int tmpl, err, stat;
	char path[20] = "/usr/sbin/rc.", log[20] = "rc..log", timebuf[20];
	time_t now;
	struct tm ltime;
	size_t sz;
	char *pathenv;

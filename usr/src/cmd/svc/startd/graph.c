...
static char be_mountpoint[MAXPATHLEN];
static DIR *be_mountpoint_DIR;
#endif	/* __i386 */
...
boolean_t go_to_unconfigure = B_FALSE; /* Not static so specials.c can reset */
...
static const char * const emsg_env =
	"svc.startd: Adjusted default process environment"
	" due to changes in svc:/system/environment:init."
	" Processes spawned subsequently by svc.startd (1M)"
	" and init (1M) will use the new environment settings."
	" Previously spawned processes will continue with old"
...
static const char * const unconfigure_fmri = SCF_MILESTONE_UNCONFIGURE;


/*
 * These services define the system being "up".  If none of them can come
 * online, then we will run sulogin on the console.  can_come_up()
...
{
	/*
	 * /etc/inittab used to have the initial /usr/sbin/rcS as a 'sysinit'
...
vertex_send_event(graph_vertex_t *v, restarter_event_type_t e)
{
	...
	/*
	 * First check to see if the instance is in conflict
	 * if this is an attempt to enable the instance via
	 * a direct enable, or refresh or restart event.
	 *
	 * If the instance is in conflict then the states
	 * will be reset to maintenance and this even will
	 * be thrown away and just return.
	 */
	if (chkconflict && check_instance_conflict(v->gv_name))
		return;
	...
	if (r != 0) {
		...
		if (r != 0) {
			log_framework(LOG_DEBUG,
			    "%s: instance states read error: %d.\n",
			    v->gv_name, r);
			scf_pg_destroy(pg);
			scf_instance_destroy(scf_inst);
			(void) scf_handle_unbind(h);
		...
		if (next_state == RESTARTER_STATE_OFFLINE ||
...
init_state:
	...
	err = libscf_get_basic_instance_data(h, inst, v->gv_name, &enabled,
	    &enabled_ovr, &restarter_fmri, &incomplete);
	switch (err) {
	case 0:
		if (incomplete)
			return (0);
...
#if defined(__x86)
/*
 * Splash shutdown routines.
 * Determination of whether to do a splash shutdown has been made by other code
 * (see init.c). These routines perform the shutdown animation (in
 * splash_gfx_step()). The console can be reset to framebuffer text mode
 * (e.g. on error) by calling reset_text_console().
 */
#define	GFX_DEVICE	"/dev/fb"

static void
reset_text_console(int fd)
{
	if (fd < 0)
		return;
...
splash_gfx_step(int fd)
{
	...
	(void) ioctl(fd, KDSHUTDOWN, KDS_STEP);
}

static void
splash_gfx_last(int fd)
{
	if (fd < 0)
		return;
	...
	(void) ioctl(fd, KDSHUTDOWN, KDS_LAST);
}

#define	RESET_TEXT_CONSOLE(x)	reset_text_console(x)
#define	SPLASH_GFX_STEP(x)	splash_gfx_step(x)
#define	SPLASH_GFX_LAST(x)	splash_gfx_last(x)

#else

#define	RESET_TEXT_CONSOLE(x)
#define	SPLASH_GFX_STEP(x)
#define	SPLASH_GFX_LAST(x)
...
do_uadmin(void)
{
	const char * const resetting = _PATH_SYSVOL "/resetting";
	int fd;
	struct statvfs vfs;
	time_t now;
	struct tm nowtm;
	...
	/*
	 * Determination of whether to do a splash shutdown has been made by
	 * other code (see init.c).
	 */
	gfx_fd = open(GFX_DEVICE, O_RDONLY);
#endif	/* __x86 */

	SPLASH_GFX_STEP(gfx_fd);
	...
	if (halting == AD_FASTREBOOT) {
#if defined(__i386)
		int rc;
		argbuf = (char *)malloc(BOOTARGS_MAX);
		if ((rc = be_get_boot_args_and_mountpt(-1, argbuf, BOOTARGS_MAX,
		    be_mountpoint, MAXPATHLEN))
		    != 0) {
			/*
			...
			errorstr = be_err_to_str(rc);
			if (errorstr == NULL)
				errorstr = strerror(rc);
		...
		/*
		 * Prevent umountall from unmounting the temporary directory
		 * by opening it.  It's closed below if uadmin() fails.
		 */
		be_mountpoint_DIR = opendir(be_mountpoint);
#else	/* __i386 */
		halting = AD_BOOT;
	...
	SPLASH_GFX_STEP(gfx_fd);
	fork_with_timeout("/usr/sbin/umountall -l", 0, 5);
	fork_with_timeout("/usr/sbin/umount /tmp /var/adm /var "
	    ">/dev/null 2>&1", 0, 5);
	SPLASH_GFX_STEP(gfx_fd);
	/*
	...
	SPLASH_GFX_STEP(gfx_fd);
	fork_with_timeout("/usr/sbin/umount /usr >/dev/null 2>&1", 0, 5);
	...
	SPLASH_GFX_LAST(gfx_fd);

	(void) uadmin(A_SHUTDOWN, halting, (uintptr_t)argbuf);
	uu_warn("uadmin() failed");

	free(argbuf);

	RESET_TEXT_CONSOLE(gfx_fd);
#if defined(__i386)
	/* uadmin failed, so unmount the BE */
	if (halting == AD_FASTREBOOT && strlen(be_mountpoint) > 0 &&
	    strcmp(be_mountpoint, "/") != 0) {
		(void) closedir(be_mountpoint_DIR);
		(void) umount(be_mountpoint);
		(void) rmdir(be_mountpoint);
	}
#endif
    ...
    boolean_t lock_graph)
{
	...
	if (v != NULL) {
		...
		/*
		 * If this instance is incomplete do not add
		 * the vertex.
		 *
		 * This bit of code here is to keep the instances
		 * from going into the vertex that are not complete.
		 */
		if (scf_instance_is_complete(inst) != SCF_COMPLETE) {
			if (lock_graph)
				MUTEX_UNLOCK(&dgraph_lock);
...
dgraph_refresh_instance(graph_vertex_t *v, scf_instance_t *inst)
{
	...
	r = libscf_get_basic_instance_data(scf_instance_handle(inst), inst,
	    v->gv_name, &enabled, NULL, NULL, &incomplete);
	switch (r) {
	case 0:
		if (incomplete)
			return (EINVAL);
    ...
    restarter_instance_state_t old_state, restarter_str_t reason)
{
	...
	if (maint) {
	    ...
	    (IS_ENABLED(v) || to == SCF_STATE_DISABLED)) {
		raise++;
		stev_ct_global++;
	} else {
		stev_ct_noprefs++;
	...
	if ((err = nvlist_alloc(&attr, NV_UNIQUE_NAME, 0)) != 0 ||
	    (err = nvlist_add_string(attr, "fmri", v->gv_name)) != 0 ||
	    (err = nvlist_add_uint32(attr, "reason-version",
	    restarter_str_version())) != 0 ||
	    (err = nvlist_add_string(attr, "reason-short",
	    restarter_get_str_short(reason))) != 0 ||
	    (err = nvlist_add_string(attr, "reason-long",
	    restarter_get_str_long(reason))) != 0 ||
	    (err = nvlist_add_int32(attr, "transition", stn_transition)) != 0) {
		log_framework(LOG_WARNING,
...
/*
 * Signal the unconfigure milestone that we've reached it, and
 * it's time to do work.  The unconfigure milestone is also special
 * insofar as we restart its start method once we've reached the
 * reduced graph state.
 */
void
do_unconfigure()
{
	restarter_event_type_t e = RESTARTER_EVENT_TYPE_ADMIN_RESTART;

	log_framework(LOG_DEBUG,
	    "Transitioned to unconfigure milestone. Restarting %s.\n",
	    milestone->gv_name);
	restarter_protocol_send_event(milestone->gv_name,
	    milestone->gv_restarter_channel, e, milestone->gv_reason);
}
    ...
    restarter_instance_state_t old_state)
{
	...
	if (state != old_state && st->st_load_complete &&
	    !go_single_user_mode && !go_to_level1 && !go_to_unconfigure &&
	    halting == -1) {
		if (!sulogin_thread_running && !can_come_up()) {
			(void) startd_thread_create(sulogin_thread, NULL);
			sulogin_thread_running = B_TRUE;
...
target_milestone_as_runlevel()
{
	...
	if (strcmp(milestone->gv_name, multi_user_fmri) == 0)
		...
		return ('3');
	else if (strcmp(milestone->gv_name, unconfigure_fmri) == 0)
 ...
 * the running of /usr/sbin/rcX scripts but we can no longer make the
 * distinction because the /usr/sbin/rcX scripts no longer exist as punctuation
...
dgraph_set_milestone(const char *fmri, scf_handle_t *h, boolean_t norepository)
{
	...
	if (!isall && !isnone) {
		...
		if (strcmp(cfmri, single_user_fmri) != 0 &&
		    ...
		    strcmp(cfmri, unconfigure_fmri) != 0) {
...
reject:
	...
	if (!isall && !isnone) {
		if (strcmp(cfmri, unconfigure_fmri) == 0)
			go_to_unconfigure = B_TRUE;
	}
...
graph_thread(void *arg)
{
	 ...
	 * state updates.
	 */
	if (!go_single_user_mode && !go_to_level1 && !go_to_unconfigure &&
	    halting == -1) {
		if (!sulogin_thread_running && !can_come_up()) {
			(void) startd_thread_create(sulogin_thread, NULL);
			sulogin_thread_running = B_TRUE;
...
process_actions(scf_handle_t *h, scf_propertygroup_t *pg, scf_instance_t *inst)
{
	...
	while ((a = next_action(action_ts, NACTIONS)) != -1) {
		...
		if (a == ADMIN_EVENT_REFRESH) {
			...
			case 0:
				/*
				 * Reload the SMF environment
				 * if SCF_INSTANCE_ENV is refreshed.
				 */
				if ((vertex == NULL) &&
				    (vertex->gv_name == NULL))
					break;

				if (strcmp(vertex->gv_name,
				    SCF_INSTANCE_ENV) == 0) {
					graph_vertex_t *m_vertex;
					reload_env();
					/*
					 * If single-user SMF service
					 * is online then syslog a
					 * message (including console)
					 */
					m_vertex =
					    vertex_get_by_name(
					    SCF_MILESTONE_SINGLE_USER);
					if (m_vertex != NULL) {
						if (m_vertex->gv_state ==

...
int
check_instance_conflict(char *fmri)
{
	restarter_inst_t *ri;
	scf_handle_t *h;
	scf_instance_t *i;
	...
	i = safe_scf_instance_create(h);
	s = safe_scf_service_create(h);

	/*
	 * If the fmri cannot be decoded into a service and instance
	 * then just return as not in conflict and let the event
	 * handle sort out the problem with the service/instance.
	 */
	if (scf_handle_decode_fmri(h, fmri, NULL, s, i, NULL,
	    NULL, SCF_DECODE_FMRI_EXACT)) {
		r = 0;
		goto out;
	}

	r = scf_instance_in_conflict_composed(i, s);

	/*
	 * if ri is null, it means the instance was deleted which
	 * is not an error here.
	 */
	if (r == 1 && (ri = inst_lookup_by_name(fmri)) != NULL) {
		(void) restarter_instance_update_states(h, ri,
		    RESTARTER_STATE_MAINT, RESTARTER_STATE_NONE, RERR_FAULT,
		    restarter_str_instance_conflict);

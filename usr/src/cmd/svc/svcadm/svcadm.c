    ...
    boolean_t enable)
{
	...
	/*
	 * If the instance is not complete then do not enable the services.
	 *
	 * If the instance is complete but does not have the enabled property
	 * allow for the property to be created.
	 */
	if (scf_instance_is_complete(inst) != SCF_COMPLETE) {
		uu_warn(gettext("%s: is not complete, missing %s/%s "
		    "(see svcs -xv %s for details)\n"),
		    fmri, SCF_PG_GENERAL, SCF_PROPERTY_COMPLETE, fmri);

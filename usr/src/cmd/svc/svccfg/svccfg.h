...
/*
 * Flags for use with lscf_prep_hndl()
 */
#define	LSCF_HNDL_NONE		0x00
#define	LSCF_HNDL_MASKED	0x01
...
typedef enum bundle_type {
	SVCCFG_UNKNOWN_BUNDLE, SVCCFG_MANIFEST, SVCCFG_PROFILE
} bundle_type_t;
...
typedef struct pgroup {
	...
	const char	*sc_pgroup_bundle;	/* Used for dependents */
...
extern boolean_t g_hndl_noreset;
extern int g_exitcode;
extern int g_verbose;
extern int g_restart_mi;
...
extern const char * const complete_attr;
...
void lscf_delpg(char *, int);
void lscf_delhash(char *, int);
int lscf_listprop(const char *, const char *, const char *);
void lscf_addprop(char *, const char *, const uu_list_t *);
void lscf_delprop(char *, int);
...
char *get_bundle_layer(const char *, bundle_type_t, scf_decoration_layer_t *);

...
const char * const complete_attr = "complete";
...
lxml_validate_element(xmlNodePtr n)
{
	...
	if (xmlValidateElement(vcp, n->doc, n) == 0)
		uu_die(gettext("Document is not valid: %s\n"),
		    n->doc->name == NULL ? "<unknown>" : n->doc->name);
...
lxml_store_value(value_t *v, element_t type, const xmlChar *value)
{
	...
	/*
	 * The only reason value is NULL is there was an unknown
	 * type and the type has now been found and the value is
	 * coming from within the value structure as it was already
	 * set to a default of SCF_TYPE_ASTRING.
	 *
	 * There is no assert on the type here, because the type
	 * was already reset by the caller so that the correct
	 * type is used to set the value now.
	 */
	if (value == NULL) {
...
lxml_get_propval(pgroup_t *pgrp, xmlNodePtr propval)
{
	...
	} else if (op == SVCCFG_OP_APPLY || op == SVCCFG_OP_INVENTORY) {
		/*
 ...
 */
static int
lxml_get_dependent(const char *fname, entity_t *entity, xmlNodePtr dependent)
{
	xmlChar *name, *or;
	xmlNodePtr sf;
	xmlChar *fmri, *delete;
...
lxml_get_tm_manpage(entity_t *service, xmlNodePtr manpage)
{
	...
	temp = (char *)lxml_label_to_groupname((const char *)title,
	    (char *)section);
	pgname = (char *)lxml_label_to_groupname(SCF_PG_TM_MAN_PREFIX, temp);
	xmlFree(title);
	xmlFree(section);
	uu_free(temp);
 ...
 *
 * The <name> portion of the preceding proper names requires more
...
lxml_get_default_instance(entity_t *service, xmlNodePtr definst)
{
	...
	uint64_t complete_val = (service->sc_op == SVCCFG_OP_IMPORT);
	...
	/*
	 * If this is an import and the complete flag is set
	 * to false then this is going to fail validation because
	 * the instance is not complete and must be complete in
	 * an import manifest.
	 */
	if (complete_val == 0 && (complete = xmlGetProp(definst,
	    (xmlChar *)complete_attr)) != NULL) {
		complete_val = (strcmp(true, (const char *)complete) == 0) ?
		    1 : 0;
		xmlFree(complete);
	}
	...
	/*
	 * If complete attach the complete property to the general pg.
	 */
	if (complete_val) {
		p = internal_property_create(SCF_PROPERTY_COMPLETE,
		    SCF_TYPE_ASTRING, 0);
 ...
 */
static int
lxml_get_instance(const char *fname, entity_t *service, xmlNodePtr inst,
    bundle_type_t bt)
{
	entity_t *i;
	pgroup_t *pg;
	...
	if (enabled == NULL) {
		...
		p->sc_property_override = (service->sc_op == SVCCFG_OP_APPLY);
		 ...
		 * an import manifest.
		 */
		if (complete_val == 0 && (complete = xmlGetProp(inst,
		    (xmlChar *)complete_attr)) != NULL) {
			complete_val = (strcmp(true,
			    (const char *)complete) == 0) ?  1 : 0;
			xmlFree(complete);
		}
	...
	for (cursor = inst->xmlChildrenNode; cursor != NULL;
		...
		case SC_DEPENDENCY:
			...
			break;
		case SC_DEPENDENT:
			(void) lxml_get_dependent(fname, i, cursor);
			break;
		case SC_METHOD_CONTEXT:
...
lxml_get_service(bundle_t *bundle, xmlNodePtr svc, svccfg_op_t op)
{
	...
	fname = get_bundle_layer(svc->doc->name, bundle->sc_bundle_type,
	    &layer);
	/*
	...
	if (svc->doc->name != NULL &&
	    ...
	    bundle->sc_bundle_type == SVCCFG_PROFILE)) {
		property_t *p;
		char *buf;

		/*
		 * Only record the profiles that are in supported
		 * locations as those are the only profiles that will
		 * be supported by cleanup.  Also, the fact that
		 * profiles applied from outside of the supported ones
		 * will have changes applied at the administrative
		 * layer cleanup will not remove these changes.
		 */
		if (bundle->sc_bundle_type == SVCCFG_MANIFEST ||
		    bundle->sc_bundle_type == SVCCFG_PROFILE) {
			buf = mhash_filename_to_propname(fname, B_TRUE);

			pg = internal_pgroup_create_strict(s,
			    SCF_PG_MANIFESTFILES, SCF_GROUP_FRAMEWORK);

			if (pg == NULL) {
				uu_die(gettext("Property group for "
				    "prop_pattern, \"%s\", already exists "
	...
	for (cursor = svc->xmlChildrenNode; cursor != NULL;
		...
		switch (e) {
		case SC_INSTANCE:
			if (lxml_get_instance(fname, s, cursor,
			    bundle->sc_bundle_type) != 0)
				return (-1);
			break;
		case SC_TEMPLATE:
		...
		case SC_DEPENDENCY:
			...
			break;
		case SC_DEPENDENT:
			(void) lxml_get_dependent(fname, s, cursor);
			break;
		case SC_RESTARTER:
    ...
    xmlNodePtr subbundle, svccfg_op_t op)
{
	...
	case SVCCFG_OP_APPLY:
		...
		break;
	case SVCCFG_OP_INVENTORY:
		if (bundle->sc_bundle_type != SVCCFG_PROFILE &&
		    bundle->sc_bundle_type != SVCCFG_MANIFEST) {
			semerr(gettext("document is not a manifest or "
			    "profile.\n"));
			return (-1);
		}
		break;
 ...
 * operation: manifest for IMPORT, profile for APPLY.
 */
int
lxml_get_bundle_file(bundle_t *bundle, const char *filename, svccfg_op_t op)
{
	...
	if (op == SVCCFG_OP_INVENTORY) {
		bundle_type_t bundle_type;
		xmlChar *type;

		type = xmlGetProp(cursor, (xmlChar *)type_attr);
		bundle_type = lxml_xlate_bundle_type(type);
		if (bundle_type == SVCCFG_UNKNOWN_BUNDLE) {
			semerr(gettext("Unknown bundle type.\n"));
			xmlFreeDoc(document);
			return (-1);
		}

		do_validate = (bundle_type != SVCCFG_PROFILE) &&
		    (getenv("SVCCFG_NOVALIDATE") == NULL);
	...
	if (do_validate) {
		...
		if (r == 0) {
			semerr(gettext("Document is not valid: %s.\n"),
			    filename);
			xmlFreeDoc(document);
			return (-1);
		}
...
lxml_inventory(const char *filename)
{
	...
	if (lxml_get_bundle_file(b, filename, SVCCFG_OP_INVENTORY) != 0) {

...
engine_init()
{
	...
	cp = getenv("SVCCFG_MANIFEST_PREFIX");
	if (cp) {
		if (est->sc_repo_filename == NULL) {
			semerr(gettext("SVCCFG_MANIFEST_PREFIX cannot be set "
			    "without setting SVCCFG_REPOSITORY\n"));
		}
	} else {
		cp = getenv("PKG_INSTALL_ROOT");
	}
	...
	if (cp) {
		...
		est->sc_bundle_prefix = safe_strndup(cp, len);
	} else {
		est->sc_bundle_prefix = NULL;
	}

	cp = getenv("SVCCFG_DOOR_PATH");
	est->sc_repo_doordir = cp ? cp : _PATH_SYSVOL;
    ...
    uint_t flags)
{
	...
	/* Import */
	r = lscf_bundle_import(b, file, flags);

...
#define	DC_CONFLICT	"SMF-8001-02"
...
x_init(void)
{
	...
	if ((g_local_scope = scf_scope_create(g_h)) == NULL ||
	    (g_svc = scf_service_create(g_h)) == NULL ||
	    (g_inst = scf_instance_create(g_h)) == NULL ||
	    (g_snap = scf_snapshot_create(g_h)) == NULL ||
	    (g_pg = scf_pg_create(g_h)) == NULL ||
	    (g_prop = scf_property_create(g_h)) == NULL ||
	    (g_val = scf_value_create(g_h)) == NULL ||
	    (g_iter = scf_iter_create(g_h)) == NULL ||
	    (g_viter = scf_iter_create(g_h)) == NULL)
		scfdie();

	if (scf_handle_get_scope(g_h, SCF_SCOPE_LOCAL, g_local_scope) != 0)
		scfdie();
...
add_instance(const char *svcname, const char *instname, scf_instance_t *inst)
{
	...
	instp->complete = B_FALSE;
	instp->impact_dependents =
	    uu_list_create(svcptrs, instp, UU_LIST_DEBUG);
	if (instp->impact_dependents == NULL)
		uu_die(emsg_nomem);

	if ((dec = scf_decoration_create(g_h)) == NULL)
		uu_die(emsg_nomem);

	if (scf_instance_get_decoration(inst, SCF_DECORATION_TOP,
	    dec) == SCF_SUCCESS) {
		instp->bundle = safe_malloc(MAXPATHLEN);
		if (scf_decoration_get_bundle(dec, instp->bundle,
		    MAXPATHLEN) <= 0) {
			free(instp->bundle);
			instp->bundle = NULL;
		}
	...
	if (scf_instance_is_complete(inst) == SCF_COMPLETE) {
		...
		if (scf_instance_get_pg(inst, SCF_PG_RESTARTER, g_pg) != 0) {
			...
			case SCF_ERROR_NOT_FOUND:
				(void) fprintf(stderr, gettext("svc:/%s:%s has "
				    "no \"%s\" property group; ignoring.\n"),
				    instp->svcname, instp->instname,
				    SCF_PG_RESTARTER);
				return;
		...
		if (pg_get_single_val(g_pg, SCF_PROPERTY_STATE,
		    SCF_TYPE_ASTRING, (void *)instp->state,
		    sizeof (instp->state), 0) != 0)
			return;

		if (pg_get_single_val(g_pg, SCF_PROPERTY_NEXT_STATE,
		    SCF_TYPE_ASTRING, (void *)instp->next_state,
		    sizeof (instp->next_state), 0) != 0)
			return;
		...
		/*
		 * restarter may not set aux_state, allow to continue
		 * in that case
		 */
		if (pg_get_single_val(g_pg, SCF_PROPERTY_AUX_STATE,
		    SCF_TYPE_ASTRING, g_fmri, g_fmri_sz, 0) == 0)
			instp->aux_state = safe_strdup(g_fmri);
		else
			instp->aux_state = safe_strdup(AUX_STATE_INVALID);

		(void) pg_get_single_val(g_pg,
		    SCF_PROPERTY_START_METHOD_WAITSTATUS, SCF_TYPE_INTEGER,
		    &instp->start_method_waitstatus, 0, 0);

		/* Get the optional auxiliary_fmri */
		if (pg_get_single_val(g_pg, SCF_PROPERTY_AUX_FMRI,
		    SCF_TYPE_ASTRING, g_fmri, g_fmri_sz, 0) == 0)
			instp->aux_fmri = safe_strdup(g_fmri);
	} else {
		(void) strcpy(instp->state, "incomplete");
	}
...
load_services(void)
{
	...
	if ((siter = scf_iter_create(g_h)) == NULL ||
	    (iiter = scf_iter_create(g_h)) == NULL)
		scfdie();
...
check_msgbase(void)
{
	if (scf_handle_decode_fmri(g_h, SCF_SERVICE_STARTD, NULL, NULL, g_inst,
...
{
	scf_instance_t *scf_inst = scf_instance_create(g_h);
	if (scf_inst == NULL)
		return;

	if (scf_handle_decode_fmri(g_h, fmri, NULL, NULL, scf_inst,
	    NULL, NULL, SCF_DECODE_FMRI_EXACT) == 0)
		print_logs(scf_inst);
...
static void
print_reasons(const inst_t *svcp, scf_instance_t *inst, int verbose)
{
	int r;
	const char *dc = NULL;
	...
	if (strcmp(svcp->state, SCF_STATE_STRING_UNINIT) == 0) {
		...
		if (strcmp(svcp->aux_state, "administrative_request") == 0) {
			...
			dc = DC_TOOQUICKLY;
		} else if (strcmp(svcp->aux_state, "instance_conflict") == 0) {
			(void) puts(gettext(
			    "Reason: Instance has conflicts."));
			if (smf_inst_print_conflicts(inst) != 0) {
				if (scf_error() != SCF_ERROR_DELETED)
					scfdie();
			}
			dc = DC_CONFLICT;
	...
	} else if (svcp->complete == B_FALSE) {
		if (svcp->bundle != NULL) {
			(void) printf(gettext("Reason: Service is incomplete, "
			    "defined only by profile %s\n"), svcp->bundle);
		} else {
			(void) printf(gettext("Reason: Administratively "
			    "created service is incomplete.  Please set "
			    "complete property using:\n"));
			(void) printf(gettext("       svccfg -s svc:/%s/%s "
			    "setprop general/complete = astring: \"\"\n"),
			    svcp->svcname, svcp->instname);
		}
...
print_service(inst_t *svcp, int verbose)
{
	...
	if (svcp->complete)
		...
		(void) printf(gettext(" State: -\n"));


	free(timebuf);

	/* Reasons */
	print_reasons(svcp, g_inst, verbose);
...
explain(int verbose, int argc, char **argv)
{
	...
	if (argc > 0) {
		...
		err = scf_walk_fmri(g_h, argc, argv, SCF_WALK_MULTIPLE,

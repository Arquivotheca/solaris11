...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it wasn't */
	...
	*Sgrade = NULLCHAR;
	...
	while ((ret = getopt(argc, argv, "Ccdfg:jmn:rs:x:")) != EOF) {
		...
		case 's':
			fopt = optarg;
			/* "m" needed for compatibility */
			if (!Mail) {
				(void) strcat(Optns, "m");
				Mail = 1;
			}
			if (strchr(Optns, 'o') == NULL)
				(void) strcat(Optns, "o");
			break;
	...
	if (split(argv[argc - 1], sys2, fwd2, file2) == -1) {
		(void) fprintf(stderr, gettext("argument too long\n"));
		cleanup(-EX_NOHOST);
	}
	if (*sys2 != NULLCHAR) {
	 ...
	 */
	if (argc - optind > 2) {
		if (strlcat(file2, "/", sizeof (file2)) >= sizeof (file2)) {
			(void) fprintf(stderr,
			    gettext("filename too long: %s\n"), file2);
			cleanup(-EX_NOHOST);
		}
	}
	...
	for (; optind < argc - 1; optind++) {
		if (split(argv[optind], sys1, fwd1, file1) == -1) {
			(void) fprintf(stderr, gettext("argument too long\n"));
			cleanup(-EX_NOHOST);
		}
		if (*sys1 != NULLCHAR) {
		...
		if (*sys1 != NULLCHAR) {
			if ((strchr(file1, '*') != NULL ||
				...
				Rmtname[MAXBASENAME] = NULLCHAR;
				/*
				 * get real Myname - it depends on who I'm
				 * calling--Rmtname
				 */
				(void) mchFind(Rmtname);
				myName(Myname);
				if (*sys2 == NULLCHAR)
					sys2p = Myname;
				if (ruux(sys1, sys1, file1, sys2p, fwd2, file2)
				    != SUCCESS) {
name_toolong:
					(void) fprintf(stderr,
		...
		if (*fwd2 != NULLCHAR) {
			if (ruux(sys2, sys1, file1, "", fwd2, file2) != SUCCESS)
				goto name_toolong;
			continue;
		}
		...
		if (*sys1 == NULLCHAR) {
			...
			/*
			 * get real Myname -
			 * it depends on who I'm calling--Rmtname
			 */
			(void) mchFind(Rmtname);
			myName(Myname);
			if (*sys2 == NULLCHAR)
...
char *s1, *f1, *s2, *f2;
{
	...
	case 0:
		...
		setuid(Uid);
		if (chkperm(file1, file2, strchr(Optns, 'd')) == FAIL) {
		...
		sprintf(command, "/usr/bin/cp %s %s", file1, file2);
	...
	case 1:
		...
		(void) fclose(cfp);
		(void) snprintf(msg, sizeof (msg),
		    "%s!%s --> %s!%s", Rmtname, file1, Myname, file2);
		logent(msg, "QUEUED");
		break;
	case 2:
		...
		(void) fclose(cfp);
		(void) snprintf(msg, sizeof (msg),
		    "%s!%s --> %s!%s", Myname, file1, Rmtname, file2);
		logent(msg, "QUEUED");
		break;
	}
	_Transfer++;
...
char	*rmt, *sys1, *file1, *sys2, *fwd2, *file2;
{
	...
	lim = sizeof (cmd);
	if ((cur = snprintf(cmd, lim, "%s!uucp -C", rmt)) >= lim)
		return (FAIL);

	if (*Uopts != NULLCHAR) {
		if ((len = snprintf(cmd + cur, lim - cur, " (%s) ", Uopts))
		    >= (lim - cur))
			return (FAIL);
		cur += len;
	}
	...
	if (*sys1 == NULLCHAR || EQUALS(sys1, Myname)) {
		if (ckexpf(file1))
			exit(6);
		if ((len = snprintf(cmd + cur, lim - cur, " %s!%s ",
		    sys1, file1)) >= (lim - cur))
			return (FAIL);
	} else if (!EQUALS(rmt, sys1)) {
		if ((len = snprintf(cmd + cur, lim - cur, " (%s!%s) ",
		    sys1, file1)) >= (lim - cur))
			return (FAIL);
	} else {
		if ((len = snprintf(cmd + cur, lim - cur, " (%s) ", file1))
		    >= (lim - cur))
			return (FAIL);
	}
	cur += len;
	...
	if (*fwd2 != NULLCHAR) {
		if (*sys2 != NULLCHAR) {
			if (snprintf(cmd + cur, lim - cur, " (%s!%s!%s) ",
			    sys2, fwd2, file2) >= (lim - cur))
				return (FAIL);
		} else {
			if (snprintf(cmd + cur, lim - cur, " (%s!%s) ",
			    fwd2, file2) >= (lim - cur))
				return (FAIL);
		}
	} else {
	...
	xcmd[0] = NULLCHAR;
	for (i = 0; i < narg; i++) {
		if (strlcat(xcmd, xarg[i], sizeof (xcmd)) >= sizeof (xcmd) ||
		    strlcat(xcmd, " ", sizeof (xcmd)) >= sizeof (xcmd)) {
			return (FAIL);
		}
	}
	DEBUG(2, "cmd: %s\n", xcmd);

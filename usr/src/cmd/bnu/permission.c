...
char *buf;
{
	...
	for (p = line; p && *p; ) {
		...
		if (i >= NUMFLDS) {
			...
			(void) printf("\n*****************************\n");
			(void) printf(gettext("**BAD OPTION in PERMISSIONS"
			    " file: %s\n"), pair.name);
			(void) printf("*****************************\n");
			Uerrors++;
			return (0);
...
char *from, *to, *opt;
{
	...
	if (*(p = LASTCHAR(to)) == '/') {
		if (strlcpy(p+1, BASENAME(from, '/'),
			MAXFULLNAME - strlen(to)) >= MAXFULLNAME - strlen(to)) {
			DEBUG(5, "%s: Internal error.\n", Progname);
			return (FAIL);
		}
	...
	DEBUG(5, "chkperm(): to = %s\n", to);
	DEBUG(5, "chkperm(): from = %s\n", from);

	/*
	 * "to" is now the full path name of the destination file
	 * However, the caveat could be the file to copy over could
	 * actually be an existing directory in the destination.
	 * The "to" should be checked for this reason because
	 * when passed to /usr/bin/cp command, the interpretation
	 * would be to copy the file in the directory with the same name
	 * and not at the level of the directory.
	 *
	 * Ex. uucp /tmp/foo /tmp/bar
	 * where /tmp/bar/foo exists as a directory.
	 *
	 * By the time the code reaches here,
	 * from = /tmp/foo and to = /tmp/bar/foo
	 *
	 * When passed back to the caller, instead of erroring out
	 * because /tmp/bar/foo exists as a directory, the cp command
	 * copies the /tmp/foo file into /tmp/bar/foo directory.
	 * which is legal but the interpretation of the path was
	 * changed above.
	 *
	 * The "to" path must be checked as a directory.
	 */

	/* Check if the path generated exists */
	if (stat(to, &s) == 0) {

		/* Is it a directory? */
		if ((s.st_mode & (S_IFMT)) == S_IFDIR) {
			DEBUG(5, "cannot create %s:"
			    " Is a directory.\n", to);
			return (FAIL);
		}

		/* its a file but is it writable? */
		if (access(to, W_OK) == 0) {
			/* its writable */
			DEBUG(5, "chkperm: %s is a writeable file\n", to);
			return (0);
		}

		/* its not writable file */
		DEBUG(5, "cannot create %s:"
		    " Permission denied\n", to);
		return (FAIL);
	}

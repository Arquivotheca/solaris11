...
#define	FS		' '	/* Field separator for output records. */
...
struct secRexe			/* Data for construction of security record. */
		{
			char	cliSystem[NAMESIZE]; /* client system name */
...

{
	...
	(void) snprintf(Record, sizeof (Record), format,
		Type, FS,
		scptr->reqSystem, FS,
		scptr->reqUser, FS,
		scptr->desSystem, FS,

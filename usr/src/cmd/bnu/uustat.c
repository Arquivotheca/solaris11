...
/* #include "logs.h" */
struct m {
	char	mach[NAMESIZE];		/* machine name */
	char	locked;
	int	ccount, xcount;
	int	count, type;
	long	retrytime;
...
char *file, *dir;
{
	...
	/* if kill done by SA then send user mail */
	   ...
	   {
		snprintf(mailmsg, sizeof (mailmsg), "%s %s", KILLMSG, Jobid);
...
DIR *p;
{
	...
	(void) strlcpy(filename, dp->d_name, NAMESIZE);
	return(TRUE);
}
...
char abuf[BUFSIZ];
FILE *fp;
char accno[15], jobid[15], system[NAMESIZE], loginame[15], time[20], dest[NAMESIZE];
char size[15];
...
long timerange;
{
	...
	char rectype[5],  time[MAXDATE+1], pid[10], wmachine[NAMESIZE];
	char remote[NAMESIZE], device[10], netid[20], jobid[20];

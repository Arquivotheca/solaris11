...
char	*file;
{
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
		...
		if (buf[0] != X_RQDFILE)
			continue;
		(void) sscanf(&buf[1], "%1023s", rqfile);
...
{
	FILE *fp;
	char	buf[BUFSIZ], file[MAXFULLNAME], tfile[NAMESIZE];
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
		if (buf[0] != X_RQDFILE)
			continue;
		if (sscanf(&buf[1], "%1023s%255s", file, tfile) < 2)
			continue;
...
mv_Xfiles()
{
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
		if (buf[0] != X_RQDFILE)
			continue;
		if (sscanf(&buf[1], "%1023s%255s", ffile, tfile) < 2)
			continue;
...
{
	char	ruser[BUFSIZ], msg[2 * BUFSIZ], subj[BUFSIZ];
...
char *dirname;
{
    ...
    char	buf[BUFSIZ], user[BUFSIZ], retaddr[BUFSIZ], retuser[BUFSIZ],
		msgbuf[BUFSIZ];
    char	origsys[NAMESIZE], origuser[MAXFULLNAME];
	...
	*incmd = *retaddr = *retuser = *Jobid = NULLCHAR;
	...
	while (fgets(buf, BUFSIZ, xfp) != NULL) {
		...
		case X_USER:
			 ...
			 */
			(void) sscanf(&buf[1], "%32s%255s", user, origsys);
		...
		case X_STDOUT:
			 ...
			 * standard output
			 */
			(void) sscanf(&buf[1], "%1023s%255s", fout, sysout);
		...
		case X_STDERR:	/* standard error */
			(void) sscanf(&buf[1], "%1023s%255s", ferr, syserr);
		...
		case X_MAILF:	/* put status in _Sfile */
			store_status = 1;
			(void) sscanf(&buf[1], "%1023s", _Sfile);
			break;
 ...
 *	    (void) strlcpy(errDfile, "/dev/null", sizeof (errDfile));
 *	else {
 */

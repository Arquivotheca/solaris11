 ...
 */
	...
	if (Role == MASTER) {
		...
		if(EQUALSN("here=", &msg[1], 5)){
			 ...
			 * untruncated Rmtname but we fear incompatibility.
...
short TM_cnt = 0;
char TM_name[MAXFULLNAME];

void
cleanTM()
{
	int i;
	char tm_name[NAMESIZE];

	DEBUG(7,"TM_cnt: %d\n",TM_cnt);
	for(i=0; i < TM_cnt; i++) {
		(void) snprintf(tm_name, sizeof (tm_name),
		    "%s.%3.3d", TM_name, i);
		DEBUG(7, "tm_name: %s\n", tm_name);
		unlink(tm_name);
	}

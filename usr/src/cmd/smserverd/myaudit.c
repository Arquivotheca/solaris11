...
audit_init(door_data_t *door_dp)
{
	...
	door_dp->audit_namask.am_success = AU_MASK_ALL;
	door_dp->audit_namask.am_failure = AU_MASK_ALL;
...
#endif
	if (au_close(ad, AU_TO_WRITE, door_dp->audit_event,
	    (door_dp == 0) ? 0 : PAD_FAILURE) < 0) {
		(void) au_close(ad, AU_TO_NO_WRITE, 0, 0);
		return (-1);
	}

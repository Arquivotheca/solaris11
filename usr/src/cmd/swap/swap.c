...
#define	ZFS_METADATA_CMD "/usr/sbin/zfs set primarycache=metadata \"%s\""
...
doswap(int flag)
{
	...
	if (flag & HFLAG) {
		...
		(void) printf(gettext("total: %s allocated + "),
		    number_to_scaled_string(numbuf, allocated, factor, scale));
		(void) printf(gettext("%s reserved = "),
		    number_to_scaled_string(numbuf, reserved, factor, scale));
		(void) printf(gettext("%s used, "),
		    number_to_scaled_string(numbuf, allocated + reserved,
		    factor, scale));
		(void) printf(gettext("%s available\n"),
		    number_to_scaled_string(numbuf, available, factor, scale));
	} else {
		(void) printf(gettext("total: %luk bytes allocated + %luk"
		    " reserved = %luk used, %luk available\n"),
		    ctok(allocated), ctok(reserved),
		    ctok(reserved) + ctok(allocated), ctok(available));
	}
	...
	for (i = 0; i < num; i++, swapent++) {
		...
		if (flag & HFLAG) {
			(void) printf(gettext(" %8s"),
			    ...
			    swapent->ste_start, DEV_BSIZE, scale));
			(void) printf(gettext(" %8s"),
			    number_to_scaled_string(numbuf,
...
{
	swapres_t swr;
	int devlen = strlen(ZVOL_FULL_DEV_DIR);
	...
	if ((fd = open("/dev/dump", O_RDONLY)) >= 0) {
		 ...
		 */
		if (strncmp(dumpdev, ZVOL_FULL_DEV_DIR, devlen) == 0) {
	...
	if (strncmp(path, ZVOL_FULL_DEV_DIR, devlen) == 0) {
		/* null-termination is already accounted for here */
		char *p = path + devlen;
		int cmdlen = strlen(ZFS_METADATA_CMD) + strlen(p);
		char *cmd = malloc(cmdlen);

		(void) snprintf(cmd, cmdlen, ZFS_METADATA_CMD, p);
		if (system(cmd) == -1)
			(void) fprintf(stderr, gettext("Warning: failed to "
			    "set %s on swap device\n"),

...
static FILE	*fp;	/* File pointer for recipient's terminal */
static char *rterm, *recipient; /* Pointer to recipient's terminal & name */
static char *thissys;
...
/*	"rterminal".							*/
					...
					if (*rterm == '\0') {
						...
						if (myuid &&
						    !permit(rterminal)) {
							if (bad < 20) {
				(void) strlcpy(badterm[bad++], rterm,
				    sizeof (badterm[bad++]));
							}
							rterm[0] = '\0';
							} else if (bad > 0) {
...
/*	so the user can restart knowing what her/his choices are.	*/
		...
		(void) fwrite(&ubuf->ut_line[0],
		    sizeof (ubuf->ut_line), 1, stderr);
						(void) fprintf(stderr, "\n");
					}
	...
	if (terminal != NULL && *rterm == '\0') {
		if (bad > 0) {
			...
			if (self.ut_pid == 0) {
				...
				(void) strlcpy(&ownname[0],
				    &passptr->pw_name[0], sizeof (ownname));
			} else {
...
/*	utmp file couldn't be found.					*/
		...
		(void) strncpy(&ownname[0], &passptr->pw_name[0],
		    sizeof (ownname));
	} else {
		(void) strncpy(&ownname[0], self.ut_user,
		    sizeof (self.ut_user));
	}
	...
	if (!permit1(1))
		(void) fprintf(stderr, gettext(
		    "Warning: You have your terminal set to \"mesg -n\"."
		    " No reply possible.\n"));
...
/*	<EOT> message to recipient before dying away.			*/
...
/*	Reset the signals to the default actions and exec a shell.	*/
		...
		if (setgid(getgid()) < 0)
			exit(1);
		(void) execl("/usr/bin/sh", "sh", "-c", command, 0);
		exit(0);
	} else {
...
static int readcsi(int d, char *buf, int nbytereq)
{
	...
	if (delta_size) {
		(void) memcpy(buf, delta_pool, delta_size);
		cp = buf + delta_size;
	...
	if (delta_size > 0) {
		if (nextp[delta_size - 1] != '\n') {
			...
			(void) memcpy(delta_pool, nextp, delta_size);
		} else
			nextp = lastp;
	}
	*nextp = '\0';

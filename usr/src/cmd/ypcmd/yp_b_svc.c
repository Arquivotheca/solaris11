	...
	char **argv;
{
	...
	(void) __init_daemon_priv(PU_CLEARLIMITSET, DAEMON_UID, DAEMON_GID,
	    (char *)NULL);

	(void) priv_set(PRIV_OFF, PRIV_ALLSETS, PRIV_FILE_LINK_ANY,
	    PRIV_PROC_EXEC, PRIV_PROC_INFO, PRIV_PROC_SESSION, (char *)NULL);
...
(insecure and transport dependent)\n");
	...
	if (getdomainname(domain, sizeof (domain)) == 0) {
		sprintf(servers, "%s/%s/ypservers", PERM_BINDING, domain);
	...
	if (mkdir(TEMP_BINDING, DAEMON_DIR_MODE) != 0 && errno != EEXIST) {
		(void) fprintf(stderr, "%s: can't create %s - exiting\n",
			Argv[0], TEMP_BINDING);
		exit(1);
	}

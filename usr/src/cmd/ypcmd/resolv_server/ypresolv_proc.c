...
/*
 * Cache entries for storing resolv req info
 */
struct cache_ent {
	struct nres	*nres;
	datum		key;
	...
	datum		keydat;
{
	...
	if (keydat.dptr != NULL)
		...
		if (((char *)keydat.dptr)[keydat.dsize - 1] == '\0')
			keydat.dsize = keydat.dsize - 1;
	chl->key.dsize = keydat.dsize;
	((char *)chl->key.dptr)[keydat.dsize] = '\0';
	chl->val.dptr = NULL;
	...
	SVCXPRT		*transp;
{
	...
	if ((!byname && !byaddr && !byname_v6 && !byaddr_v6) ||
	    keydat.dsize == 0 || (c = ((char *)keydat.dptr)[0]) == '\0' ||
	    !isascii(c) || !isgraph(c)) {
	...
	int errcode;
{
	...
	if (h == NULL) {
		...
		if (valdatp.dsize) {
			((char *)valdatp.dptr)[valdatp.dsize - 1] = '\0';
			valdatp.dsize -= 1;
		}

	...
	char *user;
{
	...
	if (g == NULL) {
		...
		g->groups = malloc(GIDSINC * sizeof (gid_t));
		g->group_len = 1;
		g->group_max = GIDSINC;
		g->groups[0] = gid;
		g->next = groups[val];
		...
		if (g->group_len >= NGROUPS_UMAX) {
			(void) fprintf(stderr, "%s: %s's groups exceed %d\n",
				cmdname, user, NGROUPS_UMAX);
			return;
		}
		if (g->group_len >= g->group_max) {
			g->group_max += GIDSINC;
			g->groups = realloc(g->groups,
			    g->group_max * sizeof (gid_t));
		}

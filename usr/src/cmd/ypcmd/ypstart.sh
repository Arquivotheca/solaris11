...
enable () {
	/usr/sbin/svcadm enable -s -t $1
	ret=$?
	[ $ret = 0 ] || echo "ypstart: unable to enable $1"
...
enable svc:/network/nis/domain:default
if [ $? != 0 ]; then
	echo "ERROR: Could not enable svc:/network/nis/domain."
	exit 1
fi
domain=`domainname`
if [ -z "$domain" ]; then
	echo "ERROR: Default domain is not defined.  \c"
	echo "Configure \"svc:/network/nis/domain\" to set the domain."
	exit 1
fi

echo "starting NIS (YP server) services:\c"

zone=`/usr/sbin/zonename`

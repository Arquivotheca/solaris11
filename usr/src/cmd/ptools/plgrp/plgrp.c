 ...
 */
#define	HDR_PLGRP_PID_LWPID	"     PID/LWPID"
#define	HDR_PLGRP_HOME		"    HOME"
#define	HDR_PLGRP_AFF		"       AFFINITY"
#define	HDR_PLGRP_INHERIT	"    		   INHERIT"

/*
 * Part of the HDR_PLGRP_AFF header used to calculate space needed to
...
/*
 * Strings corresponding to lgroup affinity inheritance
 */
#define	LGRP_AFF_INHERIT_DEFAULT_STR	"default"
#define	LGRP_AFF_INHERIT_NONE_STR	"none"
#define	LGRP_AFF_INHERIT_FUTURE_STR	"future"
 ...
 * plgrp(1) operations
 */
typedef int plgrp_ops_t;

#define	PLGRP_NO_OP		0x0
#define	PLGRP_AFFINITY_GET	0x1
#define	PLGRP_AFFINITY_SET	0x2
#define	PLGRP_HOME_GET		0x4
#define	PLGRP_HOME_SET		0x8
#define	PLGRP_INHERIT_GET	0x10
#define	PLGRP_INHERIT_SET	0x20
#define	PLGRP_EXECUTE_CMD	0x40
...
typedef struct plgrp_args {
	...
	lgrp_aff_inherit_t	aff_inherit;	/* lgroup aff. inheritance */
} plgrp_args_t;
...
{
	(void) fprintf(stderr,
	    gettext("Usage:\t%s [-F] [-h] <pid> | <core> [/lwps] ...\n"),
	    progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] -i <pid>[/lwps] ...\n"),
	    progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] -a <lgroup list> <pid>[/lwps] ...\n"), progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] [-I default|none|future]"
	    " -A <lgroup list>/none|weak|strong[,...] <pid>[/lwps] ...\n"),
	    progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] [-I default|none|future]"
	...
	(void) fprintf(stderr,
	    gettext("\t%s [-F] [-I default|none|future]"
	    " -H <lgroup list> <pid>[/lwps] ...\n"),
	    progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] [-I default|none|future] -H <lgroup list>"
	    " -e <command> [argument(s)] \n"),
	    progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] -I default|none|future <pid>/[/lwps] ...\n"),
	    progname);
	(void) fprintf(stderr,
	    gettext("\t%s [-F] -I default|none|future -e <command> "
	    "[argument(s)\n"), progname);
	(void) fprintf(stderr,
 ...
 */
static void
print_heading(plgrp_ops_t ops)
{
	(void) printf(HDR_PLGRP_PID_LWPID);

	if ((ops & PLGRP_AFFINITY_GET) || (ops & PLGRP_AFFINITY_SET)) {
		(void) printf(HDR_PLGRP_HOME);
		(void) printf(HDR_PLGRP_AFF);
	}

	if ((ops & PLGRP_HOME_GET) || (ops & PLGRP_HOME_SET)) {
		(void) printf(HDR_PLGRP_HOME);
	}

	if ((ops & PLGRP_INHERIT_GET) || (ops & PLGRP_INHERIT_SET)) {
		(void) printf(HDR_PLGRP_INHERIT);
	}
...
static void
print_aff_inherit(lgrp_aff_inherit_t inherit)
{
	switch (inherit) {
	case LGRP_AFF_INHERIT_DEFAULT:
		(void) printf(FMT_INHERIT, LGRP_AFF_INHERIT_DEFAULT_STR);
		break;
	case LGRP_AFF_INHERIT_NONE:
		(void) printf(FMT_INHERIT, LGRP_AFF_INHERIT_NONE_STR);
		break;
	case LGRP_AFF_INHERIT_FUTURE:
		(void) printf(FMT_INHERIT, LGRP_AFF_INHERIT_FUTURE_STR);
		break;
	default:
		break;
	}
...
/*
 * Use /proc to call lgrp_affinity_inherit_get() in another process
 */
static lgrp_aff_inherit_t
Plgrp_aff_inherit_get(struct ps_prochandle *Ph, idtype_t idtype,
    id_t id)
{
	lgrp_affinity_args_t	args;
	argdes_t		Pargd[3];
	...
	/*
	 * Fill in only the arguments needed for
	 * syscall(SYS_lgrpsys, LGRP_SYS_AFF_INHERIT_GET, 0, &args)
	 */
	syscall = SYS_lgrpsys;
	...
	/*
	 * Fill out /proc argument descriptors for syscall(SYS_lgrpsys,
	 * LGRP_SYS_AFF_INHERIT_GET, 0, &args)
	 */
	Pnargs = LGRPSYS_NARGS;
	Pargdp = &Pargd[0];
	Pargdp->arg_value = LGRP_SYS_AFF_INHERIT_GET;
	Pargdp->arg_object = NULL;
	Pargdp->arg_type = AT_BYVAL;
	Pargdp->arg_inout = AI_INPUT;
	Pargdp->arg_size = 0;
...
/*
 * Use /proc to call lgrp_affinity_inherit_set() in another process
 */
static lgrp_aff_inherit_t
Plgrp_aff_inherit_set(struct ps_prochandle *Ph, idtype_t idtype,
    id_t id, lgrp_aff_inherit_t aff_inherit)
{
	lgrp_aff_inherit_args_t	args;
	argdes_t		Pargd[3];
	argdes_t		*Pargdp;
	int			Pnargs;
	int			Pretval;
	...
	Pnargs = LGRPSYS_NARGS;
	Pargdp = &Pargd[0];
	Pargdp->arg_value = LGRP_SYS_AFF_INHERIT_SET;
	Pargdp->arg_object = NULL;
	Pargdp->arg_type = AT_BYVAL;
	Pargdp->arg_inout = AI_INPUT;
	Pargdp->arg_size = 0;
...
/*
 * Do plgrp(1) operation on specified thread. This routine does
 * not enforce syntax checking, so caller must do so before
 * calling it.
 */
static int
    ...
    const lwpsinfo_t *lwpsinfo)
{
	...
	lgrp_aff_inherit_t	aff_inherit;
	...
	if ((ops & PLGRP_HOME_GET)) {
	...
	} else if ((ops & PLGRP_HOME_SET)) {

		if (pid == mypid) {
			/*
			 * Setting home lgroup for self.
			 * Since we are applying the operation on only one
			 * (current) LWP, we only use the first lgroup ID
			 * even if the user specified multiple values.
			 */
			if (lgrp_affinity_set(P_LWPID, P_MYID, lgrps[0],
			    LGRP_AFF_STRONG) < 0) {
				(void) fprintf(stderr,
				    gettext("%s: cannot set home lgroup on"
				    " self for lgroup %d: %s\n"), progname,
				    lgrps[0], strerror(errno));
		...
		/*
		 * Get home lgroup before and after setting it and
		 * display change.  If more than one lgroup and one LWP
		 * are specified, then home LWPs to lgroups in round
		 * robin fashion.
		 */
		...
		if (Plgrp_home_set(Ph, P_LWPID, lwpid, lgrps[i]) != 0) {
			nerrors++;
			(void) fprintf(stderr,
			    gettext("%s: cannot set home lgroup of"
			    " %d/%d to lgroup %d: %s\n"),
			...
			home = Plgrp_home(Ph, P_LWPID, lwpid);
				...
				if (home < 0) {
				(void) fprintf(stderr,
				    gettext("%s cannot get home lgroup"
				    " for %d/%d: %s\n"),
				    progname, pid, lwpid,
				    strerror(errno));
				nerrors++;
			...
			if (! (ops & PLGRP_EXECUTE_CMD)) {
				len = printf(FMT_NEWHOME, (int)old_home,
				    (int)home);
				if (len < width)
					(void) printf("%*c",
					    (int)(width - len), ' ');
...
skip1:
	if ((ops & PLGRP_AFFINITY_GET)) {
		/*
		 * Get and display this LWP's home lgroup and
		 * affinities for specified lgroups
		 */
		home = lwpsinfo->pr_lgrp;
		(void) printf(FMT_HOME, (int)home);
		 ...
		 */
		for (i = 0; i < nlgrps; i++) {
			affs[i] = Plgrp_affinity_get(Ph, P_LWPID,
			    lwpid, lgrps[i]);
			...
			if (affs[i] == LGRP_AFF_INVALID) {
				...
				(void) fprintf(stderr,
				    ...
				    " for lgroup %d for %d/%d:"
				    " %s\n"), progname, lgrps[i], pid,
	...
	} else if ((ops & PLGRP_AFFINITY_SET)) {

		if (pid == mypid) {
			/*
			 * Setting lgroup affinities for self.
			 */
			for (i = 0; i < nlgrps; i++) {
				...
				if (lgrp_affinity_set(P_LWPID, P_MYID,
				    lgrpid, aff) < 0) {
					(void) fprintf(stderr,
					    gettext("%s: cannot set"
					    " %s affinity on self for lgroup %d"
					    " :%s\n"), progname,
					    lgrp_affinity_string(aff),
					    lgrpid, strerror(errno));
					return (EXIT_NONFATAL);
				}
			}
			goto skip2;
		...
		/*
		 * Set affinities for specified lgroups and print old
		 * and new affinities and any resulting change in home
		 * lgroups
		 */
		 ...
		 * affinities and lgroups because user may specify
		 * affinity for same lgroup multiple times....
		 *
		 * Keeping these arrays by lgroup (ID) eliminates any
		 * duplication and makes it easier to just print
		 * initial and final lgroup affinities (instead of
		 * trying to keep a list of lgroups specified which
		...
		for (i = 0; i < nlgrps; i++) {
			...
			/*
			 * If the suggested affinity is the same as the
			 * current one, skip this lgroup.
			 */
			 ...
			 */
			if (Plgrp_affinity_set(Ph, P_LWPID, lwpid,
			    lgrpid, aff) < 0) {
				nerrors++;
				(void) fprintf(stderr,
				    gettext("%s: cannot set"
			...
			/*
			 * Get the new value and verify that it changed
			 * as expected.
			 */
			cur_affs[lgrpid] =
			    Plgrp_affinity_get(Ph, P_LWPID, lwpid,
			    lgrpid);
			...
			if (aff != cur_affs[lgrpid]) {
				(void) fprintf(stderr,
				    ...
				    " lgroup %d is set to %d"
				    " instead of %d for %d/%d\n"),
				    progname, lgrpid, cur_affs[lgrpid],
				    aff, pid, lwpid);
				nerrors++;
			}
		}
		...
		/*
		 * Compare current and initial affinities and mark
		 * lgroups with changed affinities.
		 */
		nchanged = 0;
		for (lgrpid = 0; lgrpid < NLGRPS; lgrpid++) {
		...
		if (nchanged == 0) {
			/*
			 * Nothing changed, so just print current
			 * affinities for specified lgroups.
			 */
			for (i = 0; i < nlgrps; i++) {
			...
			if (! (ops & PLGRP_EXECUTE_CMD)) {
				...
				print_affinities(lgrps_changed,
				    cur_affs, NLGRPS);
			}
		} else {
			int width = strlen(HDR_PLGRP_HOME_CHANGE);
			...
			/*
			 * Some lgroup affinities changed, so display
			 * old and new home lgroups for thread and its
			 * old and new affinities for affected lgroups
			 */
			home = Plgrp_home(Ph, P_LWPID, lwpid);
			if (home < 0) {
				(void) fprintf(stderr,
			...
			if (! (ops & PLGRP_EXECUTE_CMD)) {
				...
				/*
				 * Print change in affinities from old
				 * to new
				 */
				print_affinities(lgrps_changed,
				    init_affs, NLGRPS);
...
skip2:
	if ((ops & PLGRP_INHERIT_GET)) {
		/*
		 * Get and display lgroup affinity inheritance
		 * attribute
		 */
		aff_inherit = Plgrp_aff_inherit_get(Ph, P_LWPID, lwpid);
		if (aff_inherit < 0) {
			(void) fprintf(stderr,
			    gettext("%s: cannot get affinity"
			    " inheritance for %d/%d: %s\n"),
			    progname, pid, lwpid, strerror(errno));
			return (EXIT_NONFATAL);
		}
		print_aff_inherit(aff_inherit);

	} else if ((ops & PLGRP_INHERIT_SET)) {
		/*
		 * Set lgroup affinity inheritance attribute
		 */

		if (pid == mypid) {
			/*
			 * Setting lgroup affinity inheritance for self.
			 */
			if (lgrp_affinity_inherit_set(P_LWPID, P_MYID,
			    plgrp_args->aff_inherit) < 0) {
				(void) fprintf(stderr,
				    gettext("%s: cannot set affinity"
				    " inheritance on self: %s\n"), progname,
				    strerror(errno));
				return (EXIT_NONFATAL);
			}
			goto skip3;
		...
		if (Plgrp_aff_inherit_set(Ph, P_LWPID, lwpid,
		    plgrp_args->aff_inherit)) {
			(void) fprintf(stderr,
			    gettext("%s: cannot set affinity"
			    " inheritance for %d/%d: %s\n"),
			    progname, pid, lwpid, strerror(errno));
		...
		if (! (ops & PLGRP_EXECUTE_CMD)) {
			if ((plgrp_args->op & PLGRP_AFFINITY_SET) &&
			    nchanged > 0)
				print_aff_inherit(plgrp_args->aff_inherit);
			else
				print_aff_inherit(plgrp_args->aff_inherit);
		}
	}
    ...
    const lwpsinfo_t *lwpsinfo)
{
	 ...
	 */
	if (! (plgrp_args->op & PLGRP_EXECUTE_CMD))
...
/*
 * Apply plgrp operation on self and then exec the given command.
 * Note that we don't print anything when -e is used.
 */
static void
exec_cmd(char **argv, int optind, plgrp_args_t *plgrp_args)
{
	int			err;

	/* Call do_op() to apply plgrp(1) operation on self */
	err = do_op(plgrp_args, mypid, P_LWPID, NULL);
	if (err)
		exit(err);
	...
	/* Must have had some error */
	(void) fprintf(stderr,
	    gettext("\n%s: cannot exec command %s\n"), progname,
	    argv[0]);
	usage(EXIT_FAILURE);
}
 ...
 *	plgrp [-F] [-I default|none|future] -H <lgroup>,... <pid>[/lwps] ...
 *	plgrp [-F] [-I default|none|future ] -H <lgroup>,...
 *			-e command [argument(s)]
 *	plgrp [-F] -A <lgroup>,... [/none|weak|strong] ... <pid>[/lwps] ...
 *	plgrp [-F] [-I default|none|future] -A <lgroup>,... [/none|weak|strong]
 *			... <pid>[/lwps] ...
 *	plgrp [-F] [-I default|none|future] -A <lgroup>,... [/none|weak|strong]
 *			... -e command [argument(s)]
 *	plgrp [-F] -I default|none|future <pid>[/lwps] ...
 *	plgrp [-F] -I default|none|future -e command [argument(s)]
 *
...
main(int argc, char *argv[])
{
	...
	opterr = 0;
	Fflag = 0;
	while (!interrupt && (c = getopt(argc, argv, "iea:A:FhH:I:")) != -1) {
		/*
		...
		case 'h':	/* Get home lgroup */
			...
			/* Only allow one occurrence and only one option */
			if (opt_seen) {
				usage(EXIT_FAILURE);
			}
		...
		case 'i':	/* Get lgroup affinity inheritance attribute */
			...
			iflag++;
			opt_seen++;
			plgrp_todo.op = PLGRP_INHERIT_GET;

			break;

		case 'e':	/* Execute command in specified lgroup(s) */

			/*
			 * Fail if already specified option or if wrong
			 * combination of options specified
			 */
			if (eflag || aflag || hflag || iflag) {
				usage(EXIT_FAILURE);
			}

			eflag++;
			opt_seen++;
			plgrp_todo.op |= PLGRP_EXECUTE_CMD;
		...
		case 'H':	/* Set home lgroup */
			...
			/*
			 * Fail if already specified option
			 * or no more arguments or wrong combination of
			 * options specified
			 */
			if (Hflag || Aflag || aflag || hflag || iflag ||
			    optind >= argc) {
				usage(EXIT_FAILURE);
			}
		...
		case 'a':	/* Get lgroup affinity */
			...
			/*
			 * Fail if already specified option or if no more
			 * arguments specified or if any other option
			 * specified
			 */
		...
		case 'A':	/* Set lgroup affinity */
			...
			/*
			 * Fail if already specified option
			 * or if no more arguments specified or if
			 * wrong combination of options specified
			 */
			if (Aflag || Hflag || aflag || hflag ||
			    iflag || optind >= argc) {
				usage(EXIT_FAILURE);
			}
		...
		case 'I':	/* Set lgroup affinity inheritance */

			/*
			 * Fail if already specified option or if wrong
			 * combination of options specified
			 */
			if (Iflag || aflag || hflag || iflag) {
				usage(EXIT_FAILURE);
			}

			if (strcmp(optarg, LGRP_AFF_INHERIT_DEFAULT_STR) == 0) {
				plgrp_todo.aff_inherit =
				    LGRP_AFF_INHERIT_DEFAULT;
			} else if (strcmp(optarg,
			    LGRP_AFF_INHERIT_NONE_STR) == 0) {
				plgrp_todo.aff_inherit =
				    LGRP_AFF_INHERIT_NONE;
			} else if (strcmp(optarg,
			    LGRP_AFF_INHERIT_FUTURE_STR) == 0) {
				plgrp_todo.aff_inherit =
			...
			plgrp_todo.op |= PLGRP_INHERIT_SET;
			Iflag++;
			opt_seen++;
	 ...
	 * or for command and argument(s).
	 */
	if (optind >= argc)
		usage(EXIT_FAILURE);

	/* Set default if no option is specified */
	if (optind <= 1)
		plgrp_todo.op = PLGRP_HOME_GET;
	...
	/* Execute command */
	if (plgrp_todo.op & PLGRP_EXECUTE_CMD)
		exec_cmd(argv, optind, &plgrp_todo);

...
static	int	print_frame(void *, prgregset_t, uint_t, const long *, int,
    int);
    ...
    int frame_flags, int sig)
{
	...
	/* Display signal information if we're passing a signal frame. */
	if (frame_flags & PR_SIGNAL_FRAME) {
		(void) printf(" --- called from signal handler");
		if (frame_flags & PR_FOUND_SIGNAL) {
			if (sig2str(sig, buff) == -1)
				(void) strcpy(buff, " Unknown");
			(void) printf(" with signal %d (SIG%s) ---\n", sig,
			    buff);
		} else {
			(void) printf("; signal information unavailable ---\n");
		}
	}

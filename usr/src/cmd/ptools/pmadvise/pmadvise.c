 ...
 *      free, access_lwp, access_many, access_many_pset, access_default
 *  -v: verbose output
 ...
 *   Group 3: default, accesslwp, accessmany, accessmany_pset
...
#define	GRP3_ADV	(1 << MADV_ACCESS_DEFAULT | 1 << MADV_ACCESS_LWP | \
			1 << MADV_ACCESS_MANY | 1 << MADV_ACCESS_MANY_PSET)
...
usage()
{
	...
	(void) fprintf(stderr,
	    ...
	    "       valid <advice> is one of:\n"
	    "        normal, random, sequential, willneed, dontneed,\n"
	    "        free, access_lwp, access_many, access_many_pset,\n"
	    "	     access_default\n"
	    "    -v: verbose output\n"
	    "    -F: force grabbing of the target process(es)\n"
	    "    -l: show unresolved dynamic linker map names\n"
...
get_advice(char *optarg)
{
	...
	else if (strcmp(optarg, "access_many_pset") == 0)
		return (1 << MADV_ACCESS_MANY_PSET);
...
apply_advice(saddr_t **advicelist)
{
	...
	while (!interrupt && psaddr != NULL) {
		...
		if (psaddr->adv != NO_ADVICE) {
			for (i = MADV_NORMAL; i <= MADV_ACCESS_MANY_PSET; i++) {
				if ((psaddr->adv & (1 << i)) &&

...
#define	zero(p)		for (pp = (p)->beg; pp < (p)->last; pp++) *pp = '\0'
#define	OUTC(x) {(void) printf("%c", x); if (--count == 0)\
			    {(void) printf("\\\n"); count = ll; } }
#define	TEST2	{if ((count -= 2) <= 0) {printf("\\\n"); count = ll; } }
#define	PRINT_MESSAGE	(void) printf(gettext("stack empty\n"))
...
#define	error(p)	do {(void) printf(p); continue; } while (__lintzero)
...
struct	blk *hfree;
struct	blk *dc_getwd(struct blk *);

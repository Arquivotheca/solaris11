...
casediv:
		...
		case 'f':
			if (stkptr == &stack[0])
				(void) printf(gettext("empty stack\n"));
			else {
...
execute:
		...
		case '=':
			...
			continue;
		default:
			(void) printf(gettext("%o is unimplemented\n"), c);
		}
	}
}
...
dcdiv(struct blk *ddivd, struct blk *ddivr)
{
	...
	if (length(ddivr) == 0) {
		pushp(ddivr);
		(void) printf(gettext("divide by 0\n"));
		return (p);
	}
	divsign = remsign = 0;
	divr = ddivr;
...
init(int argc, char **argv)
{
	...
	if (svargc <= 0)
		...
		if (stat(svargv[1], &tsb) < 0) {
			(void) fprintf(stderr,
			    gettext("Cannot stat %s: "), svargv[1]);
			perror("");
			exit(1);
		}
...
{
	if (stkptr == stkend)
		(void) printf(gettext("out of stack space\n"));
	else {
		stkerr = 0;
		*++stkptr = p;
	}
...
hexot(struct blk *p, int flg)
{
	...
	release(p);
	if (c >= 16) {
		(void) printf(gettext("hex digit > 16"));
		return;
	}
...
command(void)
{
	...
	default:
		...
		*sl = 0;
		if ((pid = fork()) == (pid_t)0) {
			(void) execl("/usr/bin/sh", "sh", "-c", line, 0);
			exit(0100);
		}
...
copy(struct blk *hptr, int size)
{
	...
	if (ptr == 0) {
		...
		if ((ptr = nalloc(hptr->beg, (unsigned)size)) == NULL) {
			(void) printf(gettext("copy size %d\n"), size);
			ospace("copy");
		}
	}
	if ((hdr = hfree) == 0)
...
sdump(char *s1, struct blk *hptr)
{
	...
	if (hptr) {
		(void) printf("%s %o rd %o wt %o beg %o last %o\n", s1,
		    (int)hptr, (int)hptr->rd, (int)hptr->wt,
		    (int)hptr->beg, (int)hptr->last);
		p = hptr->beg;
		while (p < hptr->wt)
			(void) printf("%d ", *p++);
		(void) printf("\n");
...
ospace(char *s)
{
	(void) printf(gettext("out of space: %s\n"), s);
	(void) printf(gettext("all %ld rel %ld headmor %ld\n"),
	    all, rel, headmor);
	(void) printf(gettext("nbytes %ld\n"), nbytes);
	sdump("stk", *stkptr);
	abort();
...
garbage(char *s)
{
	...
	(void) printf(gettext("got to garbage %s\n"), s);
	for (i = 0; i < TBLSZ; i++) {
		tmps = stable[i];
		if (tmps != 0) {
			if (i < ARRAYST) {
...
redef(struct blk *p)
{
	...
	if ((int)p->beg & 01) {
		(void) printf(gettext("odd ptr %o hdr %o\n"), p->beg, p);
		ospace("redef-bad");
	}
	free(dummy);
	dummy = dcmalloc(0);

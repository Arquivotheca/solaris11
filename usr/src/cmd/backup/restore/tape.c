...
extractfile(char *name)
{
	...
	case IFIFO:
		...
		/*
		 * Have to swap the device value from x86 to sparc and
		 * vice versa. See i_ordev and di_ordev comments in
		 * sys/fs/ufs_inode.h
		 */
		if (byteorder->Bcvt) {
			daddr32_t addrholder;
			addrholder = curfile.dip->di_db[0];
			curfile.dip->di_db[0] = curfile.dip->di_db[1];
			curfile.dip->di_db[1] = addrholder;
		}
...
metaproc(char *name, char *mdata, size_t msize)
{
	...
	while ((size_t)(c - mdata) < msize) {
		...
		fsd = (ufs_fsd_t *)c;
		/* fsd needs to be byte swapped */
		if (byteorder->Bcvt) {
			swabst("1l", (uchar_t *)&fsd->fsd_type);
			swabst("1l", (uchar_t *)&fsd->fsd_size);
		}

...
%token HELP EXPORT ADD DELETE REMOVE SET INFO VERIFY LIST GET
%token EQUAL OPEN_SQ_BRACKET CLOSE_SQ_BRACKET OPEN_PAREN CLOSE_PAREN COMMA
%token COMMIT REVERT EXIT SEMICOLON TOKEN CLEAR
%token TPNAME DOI HOSTTYPE HOSTADDR
%token DEFLABEL MINLABEL MAXLABEL LABELSET
%token ZONELABEL VISIBLE MLP_SHARED MLP_PRIVATE

%type <strval> TOKEN EQUAL OPEN_SQ_BRACKET CLOSE_SQ_BRACKET
    property_value OPEN_PAREN CLOSE_PAREN COMMA simple_prop_val
%type <ival> property_name TPNAME DOI HOSTTYPE
    HOSTADDR DEFLABEL MINLABEL MAXLABEL LABELSET 
    ZONELABEL VISIBLE MLP_SHARED MLP_PRIVATE
%type <cmd> command
...
get_command: GET
	{
		short_usage(CMD_GET);
		(void) fputs("\n", stderr);
		usage(B_FALSE, CMD_GET, HELP_RES_PROPS);
		YYERROR;
	}
	| GET TOKEN
	{
	...
	| GET property_name EQUAL property_value
	{
		if (($$ = alloc_cmd()) == NULL)
			YYERROR;
		cmd = $$;
...
property_name: TPNAME	{ $$ = PT_TPNAME; }
	| DOI		{ $$ = PT_DOI; }
	| HOSTTYPE	{ $$ = PT_HOSTTYPE; }
	| DEFLABEL	{ $$ = PT_DEFLABEL; }
	| MINLABEL	{ $$ = PT_MINLABEL; }
	| MAXLABEL	{ $$ = PT_MAXLABEL; }
	| LABELSET	{ $$ = PT_LABELSET; }
	| HOSTADDR	{ $$ = PT_HOSTADDR; }
	| ZONELABEL	{ $$ = PT_ZONELABEL; }
	| VISIBLE	{ $$ = PT_VISIBLE; }
	| MLP_SHARED	{ $$ = PT_MLP_SHARED; }
	| MLP_PRIVATE	{ $$ = PT_MLP_PRIVATE; }

/*
 * The grammar builds data structures from the bottom up.  Thus various
 * strings are lexed into TOKENs or commands or property values.
 * Below is where the property values are built up into more

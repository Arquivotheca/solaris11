...
#define	NR_IDENT_TOKENS 4

/*
 * When the client first calls us up, it is expected to send a line giving
 * its "identity"; this consists of the string:
 *
 * IDENT <pid> <locale> [<flags>]
 *
 ...
 *
 * One optional flag is defined: 'd' sets DISCONNECT_ON_HALT.
 *
 * If "pid" is NULL, we're rejecting a client, so just eat the IDENT
 * string.
 */
static int
    ...
    int *flags)
{
	...
	char *tokens[NR_IDENT_TOKENS] = { NULL };
	char c = '\0';
	int i = 0, r;
	...
	for (i = 0; i < NR_IDENT_TOKENS; i++) {
		tokens[i] = strsep(&bufp, " ");
		if (bufp == NULL)
			break;
	}

	if (tokens[0] == NULL || strcmp("IDENT", tokens[0]) != 0)
		return (-1);

	if (tokens[1] == NULL)
		return (-1);
	errno = 0;
	*pid = strtoll(tokens[1], NULL, 10);
	if (errno != 0)
		return (-1);

	if (tokens[2] == NULL ||
	    strlcpy(locale, tokens[2], locale_len) >= locale_len)
		return (-1);

	*flags = 0;

	if (tokens[3] == NULL || tokens[3][0] == '\0')
		return (0);

	if (tokens[3][0] != 'd')
		return (-1);

	*flags |= DISCONNECT_ON_HALT;
...
event_message(int clifd, char *clilocale, zone_evt_t evt)
{
	...
	switch (evt) {
	case Z_EVT_ZONE_BOOTING_W:
		if (*boot_args == '\0') {
			str = "NOTICE: Read-only zone booting up read-write";
			break;
		}
		/*LINTED*/
		(void) snprintf(lmsg, sizeof (lmsg), localize_msg(clilocale,
		    "NOTICE: Read-only zone booting up read-write with"
		    " arguments: %s"), boot_args);
		lstr = lmsg;
		break;
	...
	case Z_EVT_ZONE_HALTED:
		...
		break;
	case Z_EVT_ZONE_REBOOTING_W:
		if (*boot_args == '\0') {
			str = "NOTICE: Read-only zone rebooting read-write";
			break;
		}
		/*LINTED*/
		(void) snprintf(lmsg, sizeof (lmsg), localize_msg(clilocale,
		    "NOTICE: Read-only zone rebooting read-write with"
		    " arguments: %s"), boot_args);
		lstr = lmsg;
		break;
	...
	for (;;) {
		...
		if (pollfds[2].revents &&
			...
			if (clifd != -1) {
			    ...
			    clilocale, sizeof (clilocale), &cliflags)) != -1) {
				pollfds[1].fd = clifd;
		...
		if (pollfds[3].revents) {
			...
			/*
			 * When we return to the serve_console loop, we
			 * will see that the zone is in a state < READY,
			 * and so zoneadmd will shutdown.
			 */
			case Z_EVT_ZONE_UNINSTALLING:
				goto out;

			/*
			 * If zlogin -C asked us to, disconnect when the
			 * zone is no longer running.
			 */
			case Z_EVT_ZONE_HALTED:
			case Z_EVT_ZONE_BOOTFAILED:
				if ((cliflags & DISCONNECT_ON_HALT))
					goto out;
				break;
			}
		}

...
struct pending_shutdown {
	boolean_t ps_waiting;
	zlog_t *ps_log;
	int ps_result;
} pending_shutdown;
...
/* Set in ready and can be used only later than that. */
static boolean_t readonly_zone;
...
static uint32_t rw_boot_flags(uint32_t, boolean_t);
...
z_cmd_name(zone_cmd_t zcmd)
{
	...
	/*
	 * If you trigger this assert, you didn't add your new cmd above.
	 * But if you didn't make a mistake here, lint(1) won't be happy.
	 */
	/*LINTED*/
	assert(sizeof (zcmdstr) / sizeof (*zcmdstr) == Z_AFTER_LAST_CMD);
...
zerror(zlog_t *zlogp, boolean_t use_strerror, const char *fmt, ...)
{
	...
	if (zlogp == &logsys) {
		...
		if (debug)
			syslog(LOG_ERR, "[zone '%s'] %s", zone_name, buf);
...
zone_get_uniqid(uint64_t *idp)
{
	...
	if (zone_getattr(zoneid, ZONE_ATTR_UNIQID, idp, sizeof (*idp)) == -1)
		return (-1);
...
static int
get_mac_profile(zlog_t *zlogp, char *macprofile, size_t len)
{
	zone_dochandle_t handle;
	int err;
	...
	err = zonecfg_get_mac_profile(handle, macprofile, len);
	zonecfg_fini_handle(handle);
...
/*
 * Configure a mostly read-only zone based on its configured value.  The zone's
 ...
 */
static int
setup_zone_mac_profile(zlog_t *zlogp, char *zone_namep, zoneid_t zoneid)
{
	int res;
	zone_dochandle_t handle;
	zone_maclist_t ml;
	...
	readonly_zone = zonecfg_is_readonly(handle);

	/* If not a readonly zone, we're done */
	if (!readonly_zone) {
		zonecfg_fini_handle(handle);
		return (Z_OK);
	}
	...
	if ((res = zonecfg_get_mac_lists(handle, &ml)) != Z_OK) {
		zerror(zlogp, B_FALSE, "failed to get file mac lists");
		zonecfg_fini_handle(handle);
		return (res);
	}
	res = 0;
	if (ml.zmac_size[ZS_BLACK] > 0) {
		res = syscall(SYS_privsys, PRIVSYS_ZONE_MWAC_BLACK_LIST, zoneid,
		    0, ml.zmac_list[ZS_BLACK], ml.zmac_size[ZS_BLACK]);
	}
	if (res == 0) {
		res = syscall(SYS_privsys, PRIVSYS_ZONE_MWAC_WHITE_LIST, zoneid,
		    0, ml.zmac_list[ZS_WHITE], ml.zmac_size[ZS_WHITE]);
	}
	free(ml.zmac_list[ZS_BLACK]);
	free(ml.zmac_list[ZS_WHITE]);

	zonecfg_fini_handle(handle);

	return (res == 0 ? Z_OK : Z_SYSTEM);
}

static int
zone_readonly_enable(zoneid_t zoneid)
{
	/* Will fail if there is no mwac downloaded to the kernel. */
	return (syscall(SYS_privsys, PRIVSYS_ZONE_LOCK, zoneid, 0, NULL, 0));
}

static int
zone_mark_transient_rw(zoneid_t zoneid)
{
	return (syscall(SYS_privsys, PRIVSYS_ZONE_TRANSIENT_RW, zoneid));
}
...
zone_ready(zlog_t *zlogp, zone_mnt_t mount_cmd, int zstate)
{
	...
	/*
	 * Verify the 'zoned' zfs property for the zones platform
	 * datasets. This has to happen before the 'prestate' hook
	 * is executed which mounts the datasets because setting the
	 * 'zoned' property of a mounted dataset will cause an unmount.
	 */
	if (validate_platform_ds_zoned(zlogp) != 0) {
		zerror(zlogp, B_FALSE, "unable to verify platform datasets");
		return (-1);
	}
	...
	if (setup_zone_mac_profile(zlogp, zone_name, zone_id) != Z_OK)
		goto bad;
...
typedef struct mount_data {
	const char *md_spec;
	const char *md_dir;
	const char *md_fstype;
	const char *md_opt;
} mount_data_t;

static int
mount_early_fs_cb(void *data)
{
	mount_data_t *md = (mount_data_t *)data;
	...
	if (md->md_opt != NULL) {
		 ...
		 */
		(void) strlcpy(opt_buf, md->md_opt, sizeof (opt_buf));
		opt = opt_buf;
		optlen = MAX_MNTOPT_STR;
		mflag = MS_OPTIONSTR;
	}

	if (mount(md->md_spec, md->md_dir, mflag, md->md_fstype,
	    NULL, 0, opt, optlen) != 0)
		return (errno);
    ...
    const char *fstype, const char *opt)
{
	...
	rv = zonerun(zlogp, zoneid, mount_early_fs_cb, &md, 0);

	if (rv != 0) {
		zerror(zlogp, B_FALSE, "mount of %s failed: %s",
		    dir, strerror(rv));
		return (-1);
	}
 ...
 */
int
do_subproc(zlog_t *zlogp, const char *cmdbuf, char **retstr)
{
	...
	while (fgets(inbuf, 1024, file) != NULL) {
		if (retstr == NULL) {
			char *c;
			/*
			 * Strip trailing newline, if it exists, to prevent
			 * zerror from double spacing error messages.
			 */
			if ((c = strrchr(inbuf, '\n')) != NULL &&
			    *(c+1) == '\0')
				*c = '\0';
...
static int
zone_bootup(zlog_t *zlogp, const char *bootargs, int zstate, uint32_t flags)
{
	zoneid_t zoneid;
	struct stat st;
	...
	if (readonly_zone) {
		if ((flags & Z_CMD_FLAG_RW) == 0) {
			if (zone_readonly_enable(zoneid) != 0) {
				zerror(zlogp, B_TRUE,
				    "unable to mark zone read-only");
				goto bad;
			}
		} else if ((flags & Z_CMD_FLAG_TRANSIENT_RW) != 0)
			if (zone_mark_transient_rw(zoneid) != 0) {
				zerror(zlogp, B_TRUE, "unable to mark zone to "
				    "boot transient r/w");
				goto bad;
			}
	}
	...
	if (get_mac_profile(zlogp, macprofile, sizeof (macprofile)) != Z_OK ||
	    zone_setattr(zoneid, ZONE_ATTR_MAC_PROFILE, macprofile, 0) == -1) {
		zerror(zlogp, B_TRUE, "could not set file-mac-profile");
		goto bad;
	}
...
static uint32_t
rw_boot_flags(uint32_t flags, boolean_t live)
{
	zone_dochandle_t handle;
	int err;
	char sar[MAXPATHLEN];
	...
	/*
	 * We need to determine whether the _PATH_SELF_AS_REQ exists in
	 * the zone.  If so, we boot with Z_CMD_FLAG_TRANSIENT_RW unless
	 * we're not a ROZR zone or when we're not boot read/write.
	 */
	if ((handle = zonecfg_init_handle()) == NULL)
		return (flags);
	...
	if (live)
		err = zonecfg_get_snapshot_handle(zone_name, handle);
	if (!live || err != Z_OK)
		err = zonecfg_get_handle(zone_name, handle);

	if (err != Z_OK)
		goto out;

	if (!zonecfg_is_readonly(handle)) {
		flags &= ~(Z_CMD_FLAG_RW | Z_CMD_FLAG_TRANSIENT_RW);
		goto out;
	}
	...
	if (zonecfg_get_zonepath(handle, sar, sizeof (sar)) != Z_OK)
		goto out;

	if (strlcat(sar, "/root" _PATH_SELF_AS_REQ, sizeof (sar)) >=
	    sizeof (sar)) {
		goto out;
	}

	if (access(sar, R_OK) == 0)
		flags |= Z_CMD_FLAG_RW | Z_CMD_FLAG_TRANSIENT_RW;
...
/*
 * If there's currently a pending shutdown request, we'll have to account for
 * it.  In particular, notify the thread that we halted (or tried to), and, if
 * this halt was kernel-requested, use its zlog_t.
 */
static int
zone_halt(zlog_t *zlogp, boolean_t unmount_cmd, boolean_t rebooting, int zstate)
{
	...
	if (pending_shutdown.ps_waiting == B_TRUE) {
		if (zlogp == &logsys)
			zlogp = pending_shutdown.ps_log;
	}
	...
	if ((zcerr = zonecfg_destroy_snapshot(zone_name)) != Z_OK)
		zerror(zlogp, B_FALSE, "destroying snapshot: %s",
		    zonecfg_strerror(zcerr));
...
out:
	if (pending_shutdown.ps_waiting == B_TRUE) {
		pending_shutdown.ps_result = err;
		pending_shutdown.ps_waiting = B_FALSE;
	}
...
/*ARGSUSED*/
static int
zone_clean_shutdown_cb(void *data)
{
	return (execl("/usr/sbin/init", "/usr/sbin/init", "0", NULL));
}

/*
 * After requesting a shutdown, we wait until we handle the kernel upcall asking
 * zoneadmd to halt the zone. Since we can wait indefinitely, we must allow
 * other clients to halt the zone; therefore, we can't hold the lock whilst
 * we're waiting.
 */
static int
zone_clean_shutdown(zlog_t *zlogp)
{
	zoneid_t zoneid;
	int rc = -1;

	if (pending_shutdown.ps_waiting == B_TRUE) {
		zerror(zlogp, B_FALSE, "shutdown already pending");
		return (rc);
	}
	...
	if (zonerun(zlogp, zoneid, zone_clean_shutdown_cb, NULL, 10)) {
		zerror(zlogp, B_FALSE, "couldn't cleanly shut down: "
		    "use zoneadm halt");
		return (rc);
	}

	pending_shutdown.ps_waiting = B_TRUE;
	pending_shutdown.ps_result = -1;
	pending_shutdown.ps_log = zlogp;

	while (pending_shutdown.ps_waiting == B_TRUE) {
		(void) mutex_unlock(&lock);
		(void) sleep(1);
		(void) mutex_lock(&lock);
	}

	return (pending_shutdown.ps_result);
}
    ...
    uint_t n_desc)
{
	 ...
	 */
	if (cmd < Z_MIN_CMD || cmd >= Z_AFTER_LAST_CMD ||
	    cmd == Z_UNUSED1) {
	...
	if (kernelcall) {
		 ...
		 */
		if (zone_get_uniqid(&uniqid) != 0 || uniqid != zargp->uniqid) {
			/*
	...
	if (zstate == ZONE_STATE_INCOMPLETE &&
	    (cmd == Z_BOOT || cmd == Z_MOUNT) &&
	    (flags & Z_CMD_FLAG_FORCE) != 0)
	...
	case ZONE_STATE_INSTALLED:
		...
		case Z_BOOT:
			rval = zone_ready(zlogp, Z_MNT_BOOT, zstate);
			if (rval == 0) {
				flags = rw_boot_flags(flags, B_FALSE);
				if (flags & Z_CMD_FLAG_RW)
					eventstream_write(Z_EVT_ZONE_BOOTING_W);
				else
	...
	case ZONE_STATE_DOWN:
		...
		case Z_READY:
			if ((rval = zone_halt(zlogp, B_FALSE, B_TRUE, zstate))
			    != 0) {
				audit_put_record(zlogp, uc, rval, "halt");
				break;
			}
		...
		case Z_BOOT:
			...
			break;
		case Z_SHUTDOWN: {
			boolean_t rebooting = (flags & Z_CMD_FLAG_REBOOT) ?
			    B_TRUE : B_FALSE;

			if (zstate != ZONE_STATE_RUNNING) {
			...
			flags = rw_boot_flags(flags, B_TRUE);
			...
			if (rebooting) {
				...
				if (flags & Z_CMD_FLAG_RW)
					event = Z_EVT_ZONE_REBOOTING_W;
				else
					event = Z_EVT_ZONE_REBOOTING;
				eventstream_write(event);
			}

			if ((rval = zone_clean_shutdown(zlogp)) != 0)
				break;
			...
			/*
			 * Whilst waiting for shutdown, the state may have
			 * changed, so re-check before continuing.
			 */
			if (!rebooting) {
				if (zstate <= ZONE_STATE_INSTALLED) {
					audit_put_record(zlogp, uc, rval,
					    "shutdown-halt");
					eventstream_write(Z_EVT_ZONE_HALTED);
				}
				break;
			} else if (zstate != ZONE_STATE_INSTALLED) {
				rval = -1;
				audit_put_record(zlogp, uc, rval,
				    "shutdown-reboot");
				break;
			...
			rval = zone_bootup(zlogp, zargp->bootbuf, zstate,
			    flags);
			audit_put_record(zlogp, uc, rval, "shutdown-reboot");
			if (rval != 0) {
				(void) zone_halt(zlogp, B_FALSE, B_FALSE,
				    zstate);
				eventstream_write(Z_EVT_ZONE_BOOTFAILED);
		...
		case Z_HALT: {
			boolean_t pending = pending_shutdown.ps_waiting;

			rval = zone_halt(zlogp, B_FALSE, B_FALSE, zstate);
		...
		case Z_REBOOT:
			...
			if (flags & Z_CMD_FLAG_RW)
				eventstream_write(Z_EVT_ZONE_REBOOTING_W);
			else
...
/*
 * Setup the brand's pre and post state change callbacks, if they exist.
 */
static int
...
main(int argc, char *argv[])
{
	 ...
	 * Process options.
	 */
	while ((opt = getopt(argc, argv, "dR:z:")) != EOF) {
		switch (opt) {
		case 'd':
			debug = B_TRUE;
			break;
	...
	zone_isnative = (strcmp(brand_name, SOLARIS_BRAND_NAME) == 0);

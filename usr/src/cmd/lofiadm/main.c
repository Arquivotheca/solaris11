...
static void
lofi_set_blocksize(int lfd, const char *filename, uint_t lbs, uint_t pbs)
{
	struct lofi_ioctl li = { 0 };

	(void) strlcpy(li.li_filename, filename, sizeof (li.li_filename));
	li.li_lbs = lbs;
	li.li_pbs = pbs;
	if (ioctl(lfd, LOFI_SET_BLOCKSIZE, &li) != 0)
		die(gettext("setting blocksize for '%s' failed"),
		    filename);
}
...
main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "a:c:Cd:efk:L:o:P:s:T:U")) != EOF) {
		switch (c) {
		case 'a':
			addflag = B_TRUE;
			...
			break;
		case 'L':
			/* private option to test devices used by zfs */
			lbs = convert_to_num(optarg);
			if (lbs < DEV_BSIZE || !ISP2(lbs))
				die(gettext("block size %s is invalid "
				    "or not a multiple of minimum block "
				    "size %ld\n"), optarg, DEV_BSIZE);
			break;
		case 'P':
			/* private option to test devices used by zfs */
			pbs = convert_to_num(optarg);
			if (pbs < DEV_BSIZE || !ISP2(pbs))
				die(gettext("block size %s is invalid "
				    "or not a multiple of minimum block "
				    "size %ld\n"), optarg, DEV_BSIZE);
			break;
	...
	switch (argc - optind) {
	case 0: /* no more args */
		if (compressflag || uncompressflag || lbs || pbs) {
			/* needs filename */
			usage(pname);
		}
		break;
	...
	if (addflag)
		...
		lofi_set_blocksize(lfd, filename, lbs, pbs);

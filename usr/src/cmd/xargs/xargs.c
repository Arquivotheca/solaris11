...
static int	eof_reached = FALSE;	/* set TRUE when no more input. */
...
static int	null_delim = FALSE;	/* input separated by null char? */
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't 		*/
	...
	while ((c = getopt(mac, mav, "tp0e:E:I:i:L:l:n:s:x")) != EOF) {
		switch (c) {
		...
		case '0':	/* -0: input args separated by null char */
			null_delim = TRUE;
			break;
...
getarg()
{
	...
	for (arg = next; ; c = getwchr()) {
		...
		/*
		 * If the user supplied '-0' on the command line, then
		 * every character is taken literally. There is no special
		 * handling of whitespace, quotes and backslashes. The end
		 * of file string is disabled and treated like any other
		 * argument.
		 *
		 * NOTE: the 'null_delim == TRUE' code has been added as its
		 * own separate chunk here, to reduce the reformatting effect
...
getchr(void)
{
	...
	} while ((null_delim == TRUE) ? FALSE: c == '\0');
	return (c);
}

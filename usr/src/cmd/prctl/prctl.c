...
static	int match_rctl(struct ps_prochandle *Pr, rctlblk_t **rctl, char *name,
			char *valuestringin, uint64_t valuein,
		...
		if (arg_operation != 0) {
			...
			if (arg_operation & ACTION_DELETE) {
				...
				if (match_rctl(p.pr, &rctlblkB, arg_name,
				    arg_valuestring, arg_value,
				    rctlblk_get_privilege(rctlblkA), pid) < 0) {
...
match_rctl(struct ps_prochandle *Pr, rctlblk_t **rctl, char *name,
    char *valuestringin, uint64_t valuein, rctl_priv_t privin, int pidin)
{
	rctlblk_t *next;
	rctlblk_t *last;
	rctlblk_t *tmp;

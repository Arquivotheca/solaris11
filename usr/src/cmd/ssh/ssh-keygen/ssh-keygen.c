...
int convert_from_other_formats = 0;
int print_public = 0;
...
/*
 * Some options do not support PKCS#11 URIs. In such cases if a URI is found the
 * function prints an error message and exits the program. It also exits if
 * 'identity_file' is not a PKCS#11 URI and if taken as a filename it can not be
 * accessed either. Note that 'identity_file' is a global variable.
 */
static void
accept_filename_only(void)
{
	int ret;
	struct stat st;
	pkcs11_uri_t pkcs11_uri;

	ret = pkcs11_parse_uri(identity_file, &pkcs11_uri);
	if (ret == PK11_NOT_PKCS11_URI) {
		...
		error("This option does not support a PKCS#11 URI.");
...
/*
 * The function is used only on filenames, not on PKCS#11 URIs. Note that if
 * used it is always used after the accept_filename_only() function has been
 * called. That is why we use the SSH_KMF_DONT_ASK_FOR_TOKEN_PIN flag here.
 */
static Key *
...
do_convert_to_ssh2(struct passwd *pw)
{
	...
	accept_filename_only();
	k = key_load_public(identity_file, NULL, SSH_KMF_IGNORE_CERT, NULL);
	if (k == NULL) {
...
/*
 * The '-i' option recognizes an SSH2 key format and also a PKCS#11 URI. In the
 * PKCS#11 URI case only the public key is printed since we never try to read
 * the private key into memory. Note that it might not be even technically
 * possible to do that if the private key has a non-extractable flag.
 */
static void
do_convert_from_other_formats(struct passwd *pw)
{
	int ret;
	Key *x509_key;
	pkcs11_uri_t pkcs11_uri;

	if (have_identity == NULL)
		ask_filename(pw, gettext("Enter file in which the key is"));
	...
	/* If we get here we know it is a PKCS#11 URI. */
	x509_key = key_load_public(identity_file, NULL, SSH_KMF_LOAD_CERT,
	    &ret);
	if (x509_key == NULL) {
		error("%s: %s", ssh_pk11_err(ret), identity_file);
		exit(1);
	}

	key_write(x509_key, stdout);
	fprintf(stdout, "\n");
	key_free(x509_key);
	exit(0);
}
...
/*
 * This does not work with the PKCS#11 URIs since we are converting the private
 * key into a public one. We always access a private key using the object
 * handle, we never read its data.
 */
static void
...
do_fingerprint(struct passwd *pw)
{
	...
	int i, ret, skip = 0, num = 1, invalid = 1;
	enum fp_rep rep;
	enum fp_type fptype;
	...
	if (!have_identity)
		ask_filename(pw,
		    gettext("Enter file or PKCS#11 URI in which the key is"));

	public = key_load_public(identity_file, &comment, SSH_KMF_LOAD_CERT,
	    &ret);
	if (public != NULL) {
		...
		/* If it was a PKCS#11 URI, print the proper error message. */
		if (ret != SSH_KMF_NOT_PKCS11_URI) {
			error("%s: %s", ssh_pk11_err(ret), identity_file);
			exit(1);
		}
		/*
		 * errno was set in key_try_load_public called from
		 * key_load_public. It could not be changed after that.
		 */
		perror(identity_file);
		exit(1);
	}
...
/*
 * This is a helper function when working with the known_hosts database. We do
 * not work with X.509 keys here since we just put plain public keys to the file
 * even if those come from a certificate.
 */
static void
...
do_change_passphrase(struct passwd *pw)
{
	...
	if (private == NULL) {
		...
		private = key_load_private(identity_file, old_passphrase,
		    &comment, SSH_KMF_DONT_ASK_FOR_TOKEN_PIN);
...
#define GETOPT_ARGS "BcdeHilpqxXyb:C:f:F:N:P:R:t:"
	...
	if (convert_from_other_formats)
		do_convert_from_other_formats(pw);
	if (print_public)
		do_print_public(pw);

...
get_remote_hostname(int socket, int verify_reverse_mapping)
{
...
#ifdef IPV4_IN_IPV6
	int family = 0;  /* family returned by inet_ntop_native */
#endif
	...
	for (ai = aitop; ai; ai = ai->ai_next) {
		if (getnameinfo(ai->ai_addr, ai->ai_addrlen, ntop2,
			...
			/* take out interface information, so compare works */
			char *tmpptr = strchr(ntop2, '%');
			if (tmpptr != NULL)
				*tmpptr = '\0';
			if (strcmp(ntop, ntop2) == 0) {
				/*
				 * If we dropped the interface (after the %)
				 * then also drop it from the hostname because
				 * we did not confirm past the '%'
				 */
				if (tmpptr != NULL) {
					tmpptr = strchr(name, '%');
					if (tmpptr != NULL)
...
get_socket_address(int socket, int remote, int flags)
{
	...
	result = inet_ntop_native(addr.ss_family,
	    addr6->sin6_addr.s6_addr, abuf, sizeof (abuf), NULL);
 ...
 *
 * The type of the returned address is returned in "type"
 */
static const char *
inet_ntop_native(int af, const void *src, char *dst, size_t size, int *type)
{
	struct in_addr src4;
	const char *result;

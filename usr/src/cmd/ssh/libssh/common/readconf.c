...
static struct {
...
#ifdef GSI
	/* For backwards compatibility with old 1.2.27 client code */
	...
	{ "kmfpolicydatabase", oKMFPolicyDatabase },
	{ "kmfpolicyname", oKMFPolicyName },
	{ "trustedanchorkeystore", oTrustedAnchorKeystore },
	{ NULL, oBadOption }
};
...
parse_flag:
	...
	case oIdentityFile:
		/*
		 * We do not call strdelim() here as usual because we need to
		 * support spaces in the IdentityFile option due to the fact
		 * that we can use PKCS#11 URIs there as well. URIs might
		 * contain spaces. See also servconf.c for a more detailed
		 * explanation.
		 */
		if (s == NULL || *s == '\0')
			fatal("%.200s line %d: Missing argument.", filename, linenum);

...
g11n_getlocales()
{
	...
	count = localelist(&lp, LCLIST_QUERY | LCLIST_INCLUDE_LC_MESSAGES);
	if (count <= 0)
		return (NULL);

	list = (char **) xmalloc(sizeof (char *) * (count + 1));

	for (i = 0; i < count; i++)
		list[i] = xstrdup(lp[i].locale);
	list[i] = NULL;

	localelistfree(lp);

	qsort(list, count, sizeof (char *), locale_cmp);

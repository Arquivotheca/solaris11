...
permanently_set_uid(struct passwd *pw, char *chroot_directory)
{
	 ...
	 * pam_unix_cred(5). Additionally, we also add PRIV_PROC_AUDIT
	 * which is needed for auditing in the in-process SFTP server.
	 * Note that the in-process SFTP server is protected against
	 * tracing and debugging so that the audit data cannot be
	 * compromised or faked by the user.
	 */
	...
	/*
	 * We add PRIV_PROC_AUDIT only if running in compat20 mode
	 * when c2audit module is not explicitely excluded in
	 * /etc/system.
	 */
	if (compat20 && !adt_audit_state(AUC_DISABLED)) {

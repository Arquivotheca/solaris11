...
typedef struct {
	...
	/*
	 * These are for the X.509 user authentication support.
	 */
	...
	/*
	 * This is a non-public option to switch on the SSHv1 protocol support
	 * in the server so that we can test the client side SSHv1 support in
	 * the STC-2 SSH test suite. Note that SSHv1 is officially only not
	 * supported in the server, we keep it supported in the client so that
	 * various existing appliances that support only SSHv1 continue to be
	 * accessible from any Solaris system.
	 *
	 * The name of the option was chosen so that it is clear that should the
	 * option name get out, any customer using it should know they are using
	 * something which is really not officially supported.
	 */
	int	use_unsupported_sshv1;
}       ServerOptions;

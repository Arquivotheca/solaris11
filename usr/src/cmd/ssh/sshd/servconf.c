...
initialize_server_options(ServerOptions *options)
{
	...
	options->kmf_policy_database = NULL;
	options->kmf_policy_name = NULL;
	options->trusted_anchor_keystore = NULL;

	options->use_unsupported_sshv1 = -1;
}
 ...
 * #TIMEOUT=<secs> -> LoginGraceTime=120
 */
static
void
...
fill_default_server_options(ServerOptions *options)
{
	...
	if (options->pam_service_name == NULL)
		options->pam_service_name = NULL;
	if (options->use_unsupported_sshv1 == -1)
		options->use_unsupported_sshv1 = 0;
	/* options->kmf_policy_database should not be set by default */
	/* options->kmf_policy_name should not be set by default */
	/* options->trusted_anchor_keystore should not be set by default */
}
...
typedef enum {
	...
	sMaxStartups, sBanner, sVerifyReverseMapping, sHostbasedAuthentication,
	...
	sPreUserauthHook, sMatch, sForceCommand, sPAMServicePrefix, sPAMServiceName,
	oKMFPolicyDatabase, oKMFPolicyName, oTrustedAnchorKeystore,
	sUseUnsupportedSSHv1,
	sDeprecated
} ServerOpCodes;
...
static struct {
	...
	{ "kmfpolicydatabase", oKMFPolicyDatabase, SSHCFG_GLOBAL },
	{ "kmfpolicyname", oKMFPolicyName, SSHCFG_GLOBAL },
	{ "trustedanchorkeystore", oTrustedAnchorKeystore, SSHCFG_GLOBAL },
	{ "useunsupportedsshv1", sUseUnsupportedSSHv1, SSHCFG_GLOBAL },
	{ NULL, sBadOption, 0 }
};
    ...
    const char *host, const char *address)
{
	...
	case sServerKeyBits:
		log("%s line %d: ignoring ServerKeyBits option value."
		    " SSHv1 protocol is no longer supported in the server, "
...
parse_time:
	...
	case sHostKeyFile:
		...
		/*
		 * Host keys may also contain PKCS#11 URIs which may contain
		 * spaces. To support such IRIs and to keep the change to the
		 * existing code minimal, we reuse as much code as we can, such
		 * as her . We could modify the code so that all files (not just
		 * host keys) used by the server could contain spaces but we
		 * rather do not make such a big change.
		 *
		 * The change is not to use just the first token on the line and
		 * report an error if there is something left but to use the
		 * whole line as a filename or a PKCS#11 URI.
		 */
		arg = cp;
		/*
		 * We read the full line but we do not want the new line
		 * character.
		 */
		if (arg[strlen(arg) - 1] == '\n')
			arg[strlen(arg) - 1] = '\0';
		...
		/*
		 * Skip to the end of the string so that the check immediately
		 * behind the switch command does not result in the
...
parse_flag:
	...
	case sRhostsAuthentication:
		log("%s line %d: ignoring RhostsAuthentication option value."
		    " SSHv1 protocol is no longer supported in the server, "
		    "please remove the option.",
		    filename, linenum);
	...
	case sRhostsRSAAuthentication:
		log("%s line %d: ignoring RhostsRSAAuthentication option "
		    "value. SSHv1 protocol is no longer supported in the "
		    "server, please remove the option.",
		    filename, linenum);
	...
	case sRSAAuthentication:
		log("%s line %d: ignoring RSAAuthentication option value."
		    " SSHv1 protocol is no longer supported in the server, "
		    "please remove the option.",
		    filename, linenum);
	...
	/*
	 * Note that ChallengeResponseAuthentication is still used in the SSHv1
	 * server protocol code which we no longer officially support on the
	 * server side but which we need internally to test the client side in
	 * STC-2. So, just warn the user anytime ChallengeResponseAuthentication
	 * is used.
	 */
	case sChallengeResponseAuthentication:
		log("%s line %d: ChallengeResponseAuthentication has been "
		"deprecated. You should use KbdInteractiveAuthentication "
		"instead (which defaults to \"yes\").", filename, linenum);
	...
	case oKMFPolicyDatabase:
		charptr = &options->kmf_policy_database;
		goto parse_filename;

	case oKMFPolicyName:
		charptr = &options->kmf_policy_name;
		goto parse_filename;

	case oTrustedAnchorKeystore:
		charptr = &options->trusted_anchor_keystore;
		goto parse_filename;
	...
	case sUseUnsupportedSSHv1:
		intptr = &options->use_unsupported_sshv1;
		goto parse_flag;

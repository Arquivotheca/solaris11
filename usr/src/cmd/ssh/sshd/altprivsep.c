...
/* We use the following variable in the alarm signal handler. */
static sig_atomic_t monitor_socket = -1;
...
static void	altprivsep_packet_start(uchar_t);
static int	altprivsep_packet_send(void);
static int	altprivsep_fwd_packet(uchar_t type);
...
static void	altprivsep_packet_put_int(uint_t value);
static void	altprivsep_packet_put_cstring(const char *str);
static void	altprivsep_packet_put_raw(const void *buf, uint_t len);

static uint_t	 altprivsep_packet_get_char(void);
static void	*altprivsep_packet_get_raw(uint_t *length_ptr);
static void	*altprivsep_packet_get_string(uint_t *length_ptr);
...
altprivsep_start_monitor(Authctxt **authctxt)
{
	...
	/*
	 * We need to block SIGALRM to protect child_pid assigment against race
	 * condition with SIGALRM grace_alarm_handler() signal handler.
	 */
	(void) sigemptyset(&signal_mask);
	(void) sigaddset(&signal_mask, SIGALRM);
...
pid_t
altprivsep_get_child_pid(void)
{
	return (child_pid);
}

void
altprivsep_set_socket(int socket)
{
	monitor_socket = socket;
}

int
altprivsep_get_socket(void)
{
 ...
 */
void
altprivsep_rekey(int type, uint32_t seq, void *ctxt)
{
	Kex *kex = (Kex *)ctxt;
...
altprivsep_process_input(fd_set *rset)
{
	...
	debug2("Read packet type %d from pipe to monitor", (uint_t)type);
	...
	if (type == SSH2_MSG_NEWKEYS) {
		debug2("forwarding SSH2_MSG_NEWKEYS packet we got "
		    "from monitor to the client");
...
/* Calls _to_ monitor from unprivileged process */
static int
altprivsep_fwd_packet(uchar_t type)
{
	uint_t len;
	void  *data;
...
altprivsep_get_newkeys(enum kex_modes mode)
{
	...
	altprivsep_packet_put_int((uint_t)mode);
...
/* ARGSUSED */
void
aps_input_altpriv_msg(int type, uint32_t seq, void *ctxt)
{
	uchar_t req_type;

	req_type = packet_get_char();

	debug3("%s: aps message type: %i", __func__, (int)req_type);
...
aps_read_auth_context(void)
{
	...
	session_id2 =
	    (unsigned char *)packet_get_raw((unsigned int *)&session_id2_len);
...
static void
altprivsep_packet_start(uchar_t type)
{
...
static void
altprivsep_packet_put_int(uint_t value)
{
...
static void
altprivsep_packet_put_raw(const void *buf, uint_t len)
{
...
altprivsep_packet_send(void)
{
	...
	uchar_t	plen_buf[sizeof (plen)];
	uchar_t padlen;	/* padding length */
	fd_set *setp;
	int err;
...
void
*altprivsep_packet_get_raw(uint_t *length_ptr)
{
	if (length_ptr != NULL)
...
void
*altprivsep_packet_get_string(uint_t *length_ptr)
{
	...
	int startup_pipe)
{
	...
	if (aps_child > 0) {
		 ...
		 *
		 *  - handle MaxStartups limitation
		 *  - record keeping and auditing
		 *  - PAM cleanup
		 */

		/*
		 * Close startup pipe to notify the server that auth phase
		 * finished and reset the login grace time alarm.
		 */
		if (startup_pipe != -1) {
		...
		debug2("Login grace time reset for child pid=%d",
		    (int)child_pid);
		 ...
		 * monitor could be dangerous. The exception is killing the
		 * child from LoginGraceTime signal handler, but there is no
		 * other way.
		 *
		...
		if (compat20) {
			 ...
			 * descriptor.
			 */
			packet_set_fds(monitor_socket, 0);
			snprintf(sshdpid, sizeof (sshdpid), "sshd-%d",
			    getpid());
			record_login(getpid(), NULL, sshdpid, authctxt->user);
			packet_set_fds(0, 1);
		}
		...
		if (compat20) {
			...
			snprintf(sshdpid, sizeof (sshdpid), "sshd-%d",
			    getpid());
			record_logout(getpid(), NULL, sshdpid, authctxt->user);
		}
		 ...
		 */
		(void) shutdown(monitor_socket, SHUT_RDWR);
		monitor_socket = -1;
		...
		/*
		 * We can close startup_pipe here because monitor takes care
		 * of it.
		 */
		if (startup_pipe != -1) {
			(void) close(startup_pipe);

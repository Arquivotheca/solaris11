...
static int	cmd_install(hp_cmd_t, nvlist_t *, nvlist_t **);
...
static void	audit_changestate(ucred_t *, char *, char *, char *,
		    int, char *, int, int);
static void	audit_setprivate(ucred_t *, char *, char *, char *, char *,
		    int);
static void	audit_install(ucred_t *, char *, char *, char *, hp_cmd_t, int);
...
door_server(void *cookie, char *argp, size_t sz, door_desc_t *dp, uint_t ndesc)
{
	...
	case HP_CMD_GETPRIVATE:
		...
		break;
	case HP_CMD_INSTALL:
	case HP_CMD_UNINSTALL:
		rv = cmd_install(cmd, args, &results);
		break;
	default:
		rv = EINVAL;
		break;
...
cmd_changestate(nvlist_t *args, nvlist_t **resultsp)
{
	...
	if (nvlist_lookup_uint32(args, HPD_FLAGS, (uint32_t *)&flags) != 0)
		flags = 0;
	if (nvlist_lookup_string(args, HPD_OPTIONS, &state_priv) != 0)
		state_priv = NULL;
 ...
 * cmd_install()
 *
 *	Implementation of the door commands to install and uninstall a
 *	hotplug information node and its dependents.
 *
 *	NOTE: requires 'modify' authorization.
 */
static int
cmd_install(hp_cmd_t cmd, nvlist_t *args, nvlist_t **resultsp)
{
	hp_node_t	root = NULL;
	nvlist_t	*results = NULL;
	char		*path, *connection;
	...
	if ((nvlist_lookup_string(args, HPD_PATH, &path) != 0) ||
	    (nvlist_lookup_string(args, HPD_CONNECTION, &connection) != 0)) {
		dprintf("cmd_install: invalid arguments.\n");
		return (EINVAL);
	}
	if (nvlist_lookup_uint32(args, HPD_FLAGS, (uint32_t *)&flags) != 0)
		flags = 0;
	...
	/* Check authorization */
	if (check_auth(uc, HP_MODIFY_AUTH) != 0) {
		dprintf("cmd_install: access denied.\n");
		audit_install(uc, HP_MODIFY_AUTH, path, connection, cmd,
		    ADT_FAIL_VALUE_AUTH);
		ucred_free(uc);
		return (EACCES);
	}

	/* Perform the install or un-install operation */
	if (cmd == HP_CMD_INSTALL) {
		status = install(path, connection, flags, &root);
		dprintf("cmd_install: install() == %d\n", status);
	} else {
		status = uninstall(path, connection, flags, &root);
		dprintf("cmd_install: uninstall() == %d\n", status);
	}

	/* Audit the operation */
	audit_install(uc, HP_MODIFY_AUTH, path, connection, cmd, status);
	 ...
	 *
	 * If any error occurs while packing the results, the original
	 * error code from install() or uninstall() above is still returned.
	 */
	if (root != NULL) {
		char	*buf = NULL;
		size_t	len = 0;

		dprintf("cmd_install: results nvlist required.\n");

		/* Pack and discard the error snapshot */
		rv = hp_pack(root, &buf, &len);
		hp_fini(root);
		if (rv != 0) {
			dprintf("cmd_install: hp_pack() failed (%s).\n",
			    strerror(rv));
			return (status);
		}
		...
		/* Allocate nvlist for results */
		if (nvlist_alloc(&results, NV_UNIQUE_NAME_TYPE, 0) != 0) {
			dprintf("cmd_install: nvlist_alloc() failed.\n");
			free(buf);
			return (status);
		}
		...
		if ((nvlist_add_int32(results, HPD_STATUS, status) != 0) ||
		    (nvlist_add_byte_array(results, HPD_INFO, (uchar_t *)buf,
		    len) != 0)) {
			dprintf("cmd_install: nvlist add failed.\n");
			nvlist_free(results);
			free(buf);
			return (status);
		}
...
audit_changestate(ucred_t *ucred, char *auth, char *path, char *connection,
    int new_state, char *state_priv, int old_state, int result)
{
	adt_session_data_t	*session;
	...
	event->adt_hotplug_state.state_priv = state_priv;
 ...
 * audit_install()
 *
 *	Audit an install or uninstall door command.
 */
static void
audit_install(ucred_t *ucred, char *auth, char *path, char *connection,
    hp_cmd_t cmd, int result)
{
	adt_session_data_t	*session;
	adt_event_data_t	*event;
	int			pass_fail, fail_reason;
	...
	if (cmd == HP_CMD_INSTALL) {
		event = adt_alloc_event(session, ADT_hotplug_install);
	} else {
		event = adt_alloc_event(session, ADT_hotplug_uninstall);
	}
	...
	if (cmd == HP_CMD_INSTALL) {
		event->adt_hotplug_install.auth_used = auth;
		event->adt_hotplug_install.device_path = path;
		event->adt_hotplug_install.connection = connection;
	} else {
		event->adt_hotplug_uninstall.auth_used = auth;
		event->adt_hotplug_uninstall.device_path = path;
		event->adt_hotplug_uninstall.connection = connection;
	}

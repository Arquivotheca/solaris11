...
/*
 * rcm_resources_depends()
 *
 *	Given the root of a hotplug information snapshot, construct a list
 *	of RCM compatible resource names.
 *
 *	This variant of rcm_resources() only includes resources from dependent
 *	hotplug connections.  A hotplug connection can have both a branch of
 *	dependent device nodes and one or more branches of dependent hotplug
 *	connections.  This routine only traverses those hotplug branches, and
 *	excludes device nodes immediately dependent upon the specified root.
 */
int
rcm_resources_depends(hp_node_t root, char ***rsrcsp)
{
	resource_cb_arg_t	arg;
	hp_node_t		dep;
	...
	*rsrcsp = NULL;
	...
	/* Search for dependent hotplug connections */
	for (dep = root->hp_child; dep != NULL; dep = dep->hp_sibling) {

		/* Exclude other types of dependents */
		if ((dep->hp_type != HP_NODE_PORT) &&
		    (dep->hp_type != HP_NODE_CONNECTOR))
			continue;

		/* Traverse the dependent branch */
		arg.error = 0;
		(void) hp_traverse(dep, &arg, resource_callback);
 ...
 * resource_callback()
 *
 *	A callback function for hp_traverse() that builds
 *	an RCM compatible list of resource path names.
 */
static int

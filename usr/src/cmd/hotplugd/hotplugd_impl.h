...
#define	HP_IS_ONLINE(s)		((s == DDI_HP_CN_STATE_ONLINE) || \
				...
				(s == DDI_HP_CN_STATE_MAINTENANCE_SUSPENDED))
...
int		getinfo(const char *path, const char *connection, uint_t flags,
		    hp_node_t *rootp);
int		changestate(const char *path, const char *connection,
		    int state_code, const char *state_priv_string, uint_t flags,
...
int		install(const char *path, const char *connection, uint_t flags,
		    hp_node_t *resultsp);
int		uninstall(const char *path, const char *connection,
		    uint_t flags,
		    hp_node_t *resultsp);
char		*unpack_state_priv(char *buf, size_t len);
int		copy_usage(hp_node_t root);
int		rcm_resources(hp_node_t root, char ***rsrcsp);
int		rcm_resources_depends(hp_node_t root, char ***rsrcsp);

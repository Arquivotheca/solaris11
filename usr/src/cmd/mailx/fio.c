	...
	top:
		...
		if (!hdr && cflg) {	/* nonheader, Content-length seen */
			if (clen > 0 && clen < n) {	/* read too much */
				...
				(void) memcpy(linebuf, linebuf+clen, n+1);
				cflg = 0;
...
{
	(void) fflush(otf);
	if (fseek(itf, mp->m_offset, SEEK_SET) < 0) {
		perror("fseek");
...
expand(char *name)
{
	...
	if (debug)
		(void) fprintf(stderr, "expand(%s)=", name);
	...
	if (name[0] == '+' && getfold(foldbuf) >= 0) {
		(void) snprintf(xname, sizeof (xname),
		    "%s/%s", foldbuf, name + 1);
	...
	if (wrdexp_buf.we_wordc > 1) {
		(void) fprintf(stderr, gettext("\"%s\": Ambiguous\n"), name);
		(void) fflush(stderr);
		return (NOSTR);
	}
	if (debug)
...
getfold(char *name)
{
	...
	if (*folder == '/') {
		...
		(void) snprintf(name, BUFSIZ, "%s/%s", homedir, folder);
	return (0);
}
...
Getf(register char *s)
{
	...
	if (((cp = value(s)) != 0) && *cp) {
		...
		(void) snprintf(defbuf, sizeof (defbuf), "%s/%s", Getf("HOME"),
		    "mbox");
		return (defbuf);

...
static struct name	*gexpand(struct name *nlist,
    struct grouphead *gh, int metoo, int arg_ntype);
static char		*norm(register char *user,
    register char *ubuf, int nbangs);
...
extract(char line[], int arg_ntype)
{
	...
	while ((cp = yankword(cp, nbuf, sizeof (nbuf), comma)) != NOSTR) {
		if (np != NIL && equal(nbuf, "at")) {
			...
			(void) snprintf(abuf, sizeof (abuf),
			    "%s@%s", np->n_name, nbuf);
...
outof(struct name *names, FILE *fo)
{
	...
	for (np = names; np != NIL; np = np->n_flink) {
		...
		if (image < 0) {
			fd = open(tempEdit,
			    O_CREAT|O_EXCL|O_APPEND|O_WRONLY, 0600);
			if ((fd  < 0) && (errno == EEXIST)) {
				if ((fd = open(tempEdit,
				    O_APPEND|O_WRONLY, 0600)) < 0) {
					perror(tempEdit);
					senderr++;
					goto cant;
				}
...
norm(register char *user, register char *ubuf, int nbangs)
{
	...
	user--;
	if (!strchr(user, '!')) {
		(void) snprintf(ubuf, BUFSIZ, "%s!%s", host, user);
		user = ubuf;
		inubuf++;
	}
	if (nbangs) {
		...
		if (cp != NOSTR) {
			if (!inubuf) {
				(void) strncpy(ubuf, user, cp - user);
				ubuf[cp - user] = '\0';
				user = ubuf;
			} else
				*cp = '\0';
...
unpack(struct name *np)
{
	...
	*ap++ = "-i";
	...
	if (hflag) {
		*ap++ = "-h";
		(void) snprintf(hbuf, sizeof (hbuf), "%d", hflag);
		*ap++ = savestr(hbuf);
	}
#endif /* SENDMAIL */
	while (n != NIL) {
...
mapf(register struct name *np, char *from)
{
	...
	if (debug)
		(void) fprintf(stderr, "mapf %s done\n", from);
}

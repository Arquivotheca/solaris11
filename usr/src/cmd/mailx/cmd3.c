...
dosh(char *s)
{
	...
	if (p == 0) {
		...
		for (t = SIGINT; t <= SIGQUIT; t++)
			if (sig[t-SIGINT] != SIG_IGN)
				(void) sigset(t, SIG_DFL);
		(void) execlp(Shell, Shell, (char *)0);
		perror(Shell);
		_exit(1);
	}
	while (wait(0) != p)
...
schdir(char *str)
{
	...
	if (editfile != NOSTR && *editfile != '/') {
		(void) snprintf(file, sizeof (file), "%s/%s", cwd, editfile);
		nstrcpy(efile, sizeof (efile), file);
		editfile = efile;
	}
	if (mailname[0] != '/') {
		(void) snprintf(file, sizeof (file), "%s/%s", cwd, mailname);
...
resp1(int *msgvec, int useauthor)
{
	...
	(void) snprintf(mydomname, sizeof (mydomname), "%s@%s", myname, domain);
	(void) snprintf(mylocalname, sizeof (mylocalname),
	    "%s@%s", myname, host);
...
reedit(char *subj)
{
	...
	if (subj == NOSTR)
		return (NOSTR);
	(void) strncpy(sbuf, subj, 3);
	sbuf[3] = 0;
	if (icequal(sbuf, "re:"))
		return (subj);
	len = (unsigned)(strlen(subj) + 5);
	newsubj = (char *)salloc(len);
	(void) snprintf(newsubj, len, "Re: %s", subj);
	return (newsubj);
}
...
preserve(int *msgvec)
{
	...
	if (edit) {
		(void) printf(gettext("Cannot \"preserve\" in edit mode\n"));
		return (1);
	}
...
rexit(int e)
{
	if (sourcing)
		return (1);
	if (Tflag != NOSTR)
		(void) close(creat(Tflag, TEMPPERM));
	if (!edit)
		Verhogen();
	exit(e ? e : rpterr);
	/* NOTREACHED */
...
ungroup(char **argv)
{
	...
	if (argcount(argv) == 0) {
		(void) printf("Must specify alias or group to remove\n");
		return (1);
	}
	...
	*aedit = 1;
	...
	case '%':
		*aedit = 0;
		...
		if (name[1] != 0) {
			nstrcpy(oldmailname, sizeof (oldmailname),
			    mailname);
			findmail(name+1);
			cp = savestr(mailname);
	...
	case '#':
		if (name[1] != 0)
			goto regular;
		if (prevfile[0] == 0) {
			(void) printf(gettext("No previous file\n"));
			return (NOSTR);
		}
		cp = savestr(prevfile);
		nstrcpy(prevfile, sizeof (prevfile), editfile);
...
ifcmd(char **argv)
{
	...
	if (cond != CANY) {
		(void) printf(gettext("Illegal nested \"if\"\n"));
		return (1);
	}
	cond = CANY;
	cp = argv[0];
	...
	default:
		(void) printf(gettext("Unrecognized if-keyword: \"%s\"\n"), cp);
		return (1);
	}
	return (0);
}
...
elsecmd(void)
{
	...
	switch (cond) {
	case CANY:
		(void) printf(gettext("\"Else\" without matching \"if\"\n"));
		return (1);
	...
	default:
		(void) printf(gettext("invalid condition encountered\n"));
		cond = CANY;
		break;
	}
	return (0);
...
endifcmd(void)
{
	...
	if (cond == CANY) {
		(void) printf(gettext("\"Endif\" without matching \"if\"\n"));
		return (1);
	}
	cond = CANY;
	return (0);

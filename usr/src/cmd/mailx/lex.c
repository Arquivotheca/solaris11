...
setfile(char *name, int isedit)
{
	...
	if (stat(name, &stbuf) < 0 && errno == EOVERFLOW) {
		(void) fprintf(stderr, gettext("mailbox %s is too large to"
	...
	if ((ibuf = fopen(name, "r")) == NULL) {
		...
		if (isedit || filethere)
			...
			char *f = strrchr(name, '/');
			if (f == NOSTR)
				(void) fprintf(stderr, gettext("No mail.\n"));
			else
				(void) fprintf(stderr,
	...
	(void) fseek(ibuf, (long)(BUFSIZ+1), SEEK_SET);	/* flush input buffer */
	(void) fseek(ibuf, 0L, SEEK_SET);
	if (strncmp(fortest, "Forward to ", 11) == 0) {
		if (exitflg)
			goto doexit;	/* no mail, return error */
		(void) fprintf(stderr, gettext(
		    "Your mail is being forwarded to %s"), fortest+11);
		(void) fclose(ibuf);
		goto doret;
	}
	if (exitflg) {
	...
	if (shudclob) {
		 ...
		 */
		if (edit)
			(void) edstop(strcmp(editfile, name) == 0);
		else {
...
Passeren(void)
{
	...
	if ((home = getenv("HOME")) == NULL)
		return (0);
	(void) snprintf(semfn, sizeof (semfn), "%s%s", home, "/.Maillock");
...
#ifdef SIGCONT
	(void) sigset(SIGCONT, SIG_DFL);
#endif
	if (rcvmode && !sourcing) {
		if (sigset(SIGINT, SIG_IGN) != SIG_IGN)
			(void) sigset(SIGINT, stop);
		if (sigset(SIGHUP, SIG_IGN) != SIG_IGN)
			(void) sigset(SIGHUP, hangup);
	}
	for (;;) {
		(void) setjmp(srbuf);
...
top:
		if ((shudprompt = (intty && !sourcing)) != 0) {
			...
			if (intty && value("autoinc") &&
				...
				if (edit || !issysmbox)
					(void) lock(ibuf, "r", 1);
				(void) fseek(ibuf, mailsize, SEEK_SET);
				mailsize = fsize(ibuf);
				setptr(ibuf);
				setmsize(msgCount);
				(void) fclose(ibuf);
...
stop(int s)
{
	...
	if (s) {
		(void) fprintf(stderr, gettext("Interrupt\n"));
		(void) fflush(stderr);
	}
	longjmp(srbuf, 1);
}
...
announce(void)
{
	...
	if (!Hflag && value("quiet") == NOSTR)
		(void) printf(gettext(GREETING), version);
	mdot = newfileinfo(1);
	vec[0] = mdot;
	vec[1] = 0;
	dot = &message[mdot - 1];
...
newfileinfo(int start)
{
	...
	if (getfold(fname) >= 0) {
		...
		if (strncmp(fname, editfile, strlen(fname)) == 0) {
			(void) snprintf(zname, sizeof (zname),
	...
	(void) printf("\"%s\": ", ename);
	if (msgCount == 1)
		(void) printf(gettext("1 message"));
	else
		(void) printf(gettext("%d messages"), msgCount);
	if (n > 0)
		(void) printf(gettext(" %d new"), n);
	if (u-n > 0)
		(void) printf(gettext(" %d unread"), u);
	if (d > 0)
		(void) printf(gettext(" %d deleted"), d);
	if (s > 0)
		(void) printf(gettext(" %d saved"), s);
	if (readonly)
...
inc(void)
{
	...
	if (edit) {
		(void) fprintf(stderr, gettext("Not in system mailbox\n"));
		return (-1);
	}
	if (((ibuf = fopen(mailname, "r")) == NULL) ||
		...
		if (strrchr(mailname, '/') == NOSTR)
			(void) fprintf(stderr, gettext("No new mail.\n"));
		else
			(void) fprintf(stderr, gettext("No new mail for %s\n"),
			    strrchr(mailname, '/')+1);
		if (ibuf != NULL)
			(void) fclose(ibuf);
		return (-1);

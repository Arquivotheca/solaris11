...
netmap(char name[], char from[])
{
	...
	if (debug)
		(void) fprintf(stderr, "netmap(name '%s', from '%s')\n",
		    name, from);
	if (strlen(from) == 0)
	...
	if (value("onehop") && (cp = strchr(name, '!')) && cp > name) {
		...
		*strrchr(from, '!') = 0;
		...
		from[strlen(from)] = '!';
		from = strchr(from, '!') + 1;
		(void) snprintf(nbuf, sizeof (nbuf), "%s!%s", from, name);
	}
	if (debug)
		(void) fprintf(stderr, "before optim, nbuf '%s'\n", name);
#ifdef	OPTIM
...
#endif	/* OPTIM */
	if (debug)
		(void) fprintf(stderr, "after  optim, nbuf '%s', ret '%s'\n",
		    nbuf, ret);
	cp = ret;
	if (debug)
		(void) fprintf(stderr, "wind up with '%s'\n", name);
	if (!icequal(name, cp))
...
tackon(char *sys, char *rest)
{
	while (*rest == '!')
		rest++;
	if (!ishost(sys, rest)) {
		unsigned int len = strlen(sys) + strlen(rest) + 2;
		char *r = (char *)salloc(len);

		(void) snprintf(r, len, "%s!%s", sys, rest);
		rest = r;
	}
	return (rest);
}
...
unuucp(char *name)
{
	...
	*np++ = 0;
	...
	if (cp)
		*cp = 0;
	if (debug)
		(void) fprintf(stderr, "host %s, name %s\n", hp, np);
	(void) snprintf(result, sizeof (result), "%s@%s.UUCP", np, hp);
	if (debug)
		(void) fprintf(stderr, "unuucp returns %s\n", result);
...
netlook(char machine[], int attnet)
{
	 ...
	 */
	for (cp = machine, cp2 = nbuf; *cp && cp2 < &nbuf[BUFSIZ-1];
	    *cp2++ = tolower(*cp++))
		;
	*cp2 = 0;
...
arpafix(char name[], char from[])
{
	...
	if (debug) {
		(void) fprintf(stderr, "arpafix(%s, %s)\n", name, from);
	}
	cp = strrchr(name, '@');
	if (cp == NOSTR)
		cp = strrchr(name, '%');
	...
	if (arpamach == 0) {
		if (debug)
			(void) fprintf(stderr, "machine %s unknown, uses: %s\n",
			    cp, name);
		return (name);
	}
	...
	*cp = 0;
	if (debug)
		(void) fprintf(stderr, "local address, return '%s'\n",
		    newname);
...
makeremote(char name[], char from[])
{
	...
	if (!value("makeremote"))
		return (name);
	if (debug)
		(void) fprintf(stderr, "makeremote(%s, %s) returns ",
		    name, from);
	cp = strrchr(from, '@');
	if (cp == NOSTR)
		cp = strrchr(from, '%');
	(void) snprintf(rbuf, sizeof (rbuf), "%s%s", name, cp);
	if (debug)
...
mstash(char name[], int attnet)
{
	...
	if (xp == (struct xtrahash *)0) {
		(void) printf(gettext("Ran out of machine id spots\n"));
		return (0);
	}
	if (xp->xh_name == NOSTR) {
		if (midfree >= XHSIZE) {
			(void) printf(gettext("Out of machine ids\n"));
			return (0);
		}
		xtab[midfree] = xp;
...
mlook(int mid)
{
	...
	m = mid & 0177;
	if (m >= midfree) {
		(void) printf(gettext("Use made of undefined machine id\n"));
		return (NOSTR);
	}
...
optim(char net[], char name[])
{
	...
	if (debug)
		(void) fprintf(stderr, "optim(%s, %s) called\n", net, name);
	*netstr = '\0';
	cp = net;
	for (;;) {
		/*
		...
		*cp2 = 0;
		...
		if ((c = netlook(netcomp, *cp)) == 0) {
			(void) printf(gettext("No host named \"%s\"\n"),
			    netcomp);
err:
	...
	if (strlen(netcomp) == 0) {
		(void) printf(gettext("net name syntax\n"));
		goto err;
	}
	if (debug)
		(void) fprintf(stderr, "optim1(%s,%s) called\n",
		    netstr, xfstr);
	optim1(netstr, xfstr);
	if (debug)
		(void) fprintf(stderr, "optim1(%s,%s) returns\n",
		    netstr, xfstr);
	...
	*name = '\0';
	while (*cp) {
		if ((cp2 = netname(*cp++)) == NOSTR) {
			(void) printf(gettext("Made up bad net name\n"));
			(void) printf(gettext("Machine code %c (0%o)\n"),
			    cp[-1], cp[-1]);
			(void) printf(gettext("Sorry.\n"));
			goto err;
		}
	...
	if (debug)
		(void) fprintf(stderr, "optim returns %s in name\n", name);
}
	...
	*name  = '\0';
	...
	while (*cp != 0) {
		*path = '\0';
		...
		case IMPLICIT:
			(void) optimimp(path, rpath);
			break;

		case EXPLICIT:
			(void) optimex(path, rpath);
			break;
		}
...
optiboth(char net[])
{
	...
	if (((int)strlen(cp) % 2) != 0) {
		(void) printf(gettext("Strange arg to optiboth\n"));
		return;
	}
	while (*cp) {
		cp2 = rpair(cp+2, *cp);
...
best(int src, int dest)
{
	...
	(void) fflush(stdout);
	if (stype == 0 || dtype == 0) {
		(void) printf(gettext("ERROR:  unknown internal machine id\n"));
		return (0);
	}
	if ((stype & dtype) == 0)
		return (0);

...
static void		statusput(register struct message *mp,
    register FILE *obuf, int doign, int (*fp)(const char *, FILE *));
...
writeit:
	...
	if (ishead && (mailp->m_flag & MSTATUS))
		(void) printf(gettext("failed to fix up status field\n"));
	return (lc);
}
	...
	int (*fp)(const char *, FILE *))
{
	...
	if ((mp->m_flag & (MNEW|MREAD)) == MNEW)
		return;
	(void) strcpy(statout, "Status: ");
	if (mp->m_flag & MREAD)
		(void) strcat(statout, "R");
	if ((mp->m_flag & MNEW) == 0)
		(void) strcat(statout, "O");
	(void) strcat(statout, "\n");
	(*fp)(statout, obuf);
}
...
mail1(struct header *hp, int use_to, char *orig_to)
{
	...
	if (fsize(mtf) == 0 && hp->h_subject == NOSTR) {
		(void) printf(gettext("No message !?!\n"));
		goto out;
	}
	if (intty) {
		(void) printf(gettext("EOT\n"));
		flush();
	}
	...
	senderr = 0;
	to = cat(extract(hp->h_bcc, GBCC), cat(extract(hp->h_to, GTO),
	    extract(hp->h_cc, GCC)));
	...
	if ((mtf = infix(hp, mtf)) == NULL) {
		(void) fprintf(stderr, gettext(". . . message lost, sorry.\n"));
		return;
	}
	rewind(mtf);
	if (askme && isatty(0)) {
	...
	rewind(mtf);
	if (!gotcha && !i) {
		(void) printf(gettext("No recipients specified\n"));
		goto dead;
	}
	if (senderr)
		goto dead;
	...
	if (recfile != NOSTR && *recfile)
		(void) savemail(safeexpand(recfile), hp, mtf);
	if (!gotcha)
		goto out;
	namelist = unpack(to);
	if (debug) {
		(void) fprintf(stderr, "Recipients of message:\n");
...
dead:
		...
		if (fp = fopen(deadletter,
			...
			(void) fseek(mtf, textpos, SEEK_SET);
	...
	if (pid == 0) {
		...
		if ((deliver = value("sendmail")) == NOSTR)
			...
			deliver = MAIL;
#endif
		(void) execvp(safeexpand(deliver), namelist);
		perror(deliver);
		exit(1);
	}
...
puthead(struct header *hp, FILE *fo, int w, long clen)
{
	...
	if (hp->h_defopt != NOSTR && (w & GDEFOPT))
		if (receipt_flg) {
			(void) fprintf(fo, "Return-Receipt-To: %s\n",
			    hp->h_defopt);
			gotcha++;
		} else {
			(void) fprintf(fo, "Default-Options: %s\n",
			    hp->h_defopt);
			gotcha++;
		}
	if ((w & GSUBJECT) && !(int)value("bsdcompat"))
	...
	if (w & GCLEN) {
		(void) fprintf(fo, "Content-Length: %ld\n", clen);
		gotcha++;
	}
	if (gotcha && (w & GNL))
		(void) putc('\n', fo);
...
savemail(char name[], struct header *hp, FILE *fi)
{
	...
	if (debug)
		(void) fprintf(stderr, gettext("save in '%s'\n"), name);
	...
	if (n == NOSTR)
		n = myname;
	(void) fprintf(fo, "From %s %s", n, ctime(&now));
	puthead(hp, fo, GMASK|GCLEN, fsize(fi) - textpos);
	(void) fseek(fi, textpos, SEEK_SET);
	while ((c = getc(fi)) != EOF)
		(void) putc(c, fo);
	(void) putc('\n', fo);
	(void) fflush(fo);

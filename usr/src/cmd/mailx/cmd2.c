...
next(int *msgvec)
{
	...
	if (*msgvec != NULL) {
		if (*msgvec < 0) {
			(void) printf((gettext("Negative message given\n")));
			return (1);
		}
		mp = &message[*msgvec - 1];
		...
		(void) printf(gettext("No applicable message\n"));
		return (1);
	}
...
save1(char str[], int mark)
{
	...
	if (!f) {
		*msgvec = first(0, MMNORM);
		if (*msgvec == NULL) {
			(void) printf(gettext("No messages to %s.\n"), cmd);
			return (1);
		}
		msgvec[1] = NULL;
	}
...
put1(char str[], int doign)
{
	...
	if (!f) {
		*msgvec = first(0, MMNORM);
		if (*msgvec == NULL) {
			(void) printf(gettext("No messages to put.\n"));
			return (1);
		}
		msgvec[1] = NULL;
	}
...
swrite(char str[])
{
	...
	if (!f) {
		*msgvec = first(0, MMNORM);
		if (*msgvec == NULL) {
			(void) printf(gettext("No messages to write.\n"));
			return (1);
		}
		msgvec[1] = NULL;
	}
...
snarf(char linebuf[], int *flag, int erf)
{
	...
	if (!*line_beg) {
		if (erf) {
			(void) printf(gettext("No file specified.\n"));
		}
		*flag = 0;
		return (NOSTR);
	}
	...
	while (*p != '\0') {
		if (any(*p, " \t")) {
			...
			if (*p == '\0') {
				(void) printf(gettext("Syntax error: missing "
				    "%c.\n"), qc);
				*flag = -1;
				return (NOSTR);
			}
			...
			if (pc_type & (SN_DELIM|SN_TOKEN)) {
				...
				(void) printf(gettext("improper quotes"
				    " at \"%s\".\n"), p);
				*flag = -1;
				return (NOSTR);
			}
			if (*p == '\\' && *++p == '\0') {
				(void) printf(gettext("\'\\\' at "
				    "end of line.\n"));
				*flag = -1;
				return (NOSTR);
			}
...
deltype(int msgvec[])
{
	...
	if (delm(msgvec) >= 0) {
		...
		(void) printf(gettext("No more messages\n"));
		return (0);
	}
}
...
retshow(void)
{
	...
	for (h = 0; h < HSHSIZE; h++)
		for (igp = retain[h]; igp != 0; igp = igp->i_link)
			count++;
	if (count == 0) {
		(void) printf(gettext("No fields currently being retained.\n"));
		return (0);
	}
...
igshow(void)
{
	...
	for (h = 0; h < HSHSIZE; h++)
		for (igp = ignore[h]; igp != 0; igp = igp->i_link)
			count++;
	if (count == 0) {
		(void) printf(gettext("No fields currently being ignored.\n"));
		return (0);
	}
	ring = (char **)salloc((count + 1) * sizeof (char *));
	ap = ring;

...
number:
	...
	*np = NOSTR;
	...
	if (star) {
		...
		if (mc == 0) {
			(void) printf(gettext("No applicable messages\n"));
			return (-1);
		}
		return (0);
	}
	...
	if (np > namelist) {
		...
		if (mc == 0) {
			(void) printf(gettext(
			    "No applicable messages from {%s"), namelist[0]);
	...
	if (colmod != 0) {
		...
		if (mp >= &message[msgCount]) {
			...
			(void) printf(gettext("No messages satisfy"));
			for (colp = &coltab[0]; colp->co_char; colp++)
				if (colp->co_bit & colmod)
...
check(int mesg, int f)
{
	...
	if (mesg < 1 || mesg > msgCount) {
		(void) printf(gettext("%d: Invalid message number\n"), mesg);
		return (-1);
	}
	mp = &message[mesg-1];
	if ((mp->m_flag & MDELETED) != f) {
		(void) printf(gettext("%d: Inappropriate message\n"), mesg);
		return (-1);
	}
	return (0);
}
	...
	while (*cp != '\0') {
		...
		*cp2 = '\0';
		if (cp2 == linebuf)
			break;
		if (ap >= last) {
			(void) printf(gettext("Too many elements in the list;"
			    " excess discarded\n"));
			break;
		}
		*ap++ = savestr(linebuf);
...
scan(char **sp)
{
	...
	if (regretp >= 0) {
		(void) copy(stringstack[regretp], lexstring);
...
metamess(int meta, int f)
{
	...
	case '+':
		...
		for (mp = dot + 1; mp < &message[msgCount]; mp++)
			if ((mp->m_flag & MDELETED) == f)
				return (mp - &message[0] + 1);
		(void) printf(gettext("Referencing beyond last message\n"));
		return (-1);
	...
	case '-':
		...
		for (mp = dot - 1; mp >= &message[0]; mp--)
			if ((mp->m_flag & MDELETED) == f)
				return (mp - &message[0] + 1);
		(void) printf(gettext("Referencing before first message\n"));
		return (-1);
	...
	default:
		(void) printf(gettext("Unknown metachar (%c)\n"), c);
		return (-1);
	}
}

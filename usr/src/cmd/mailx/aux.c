...
panic(char *str)
{
	(void) fprintf(stderr, gettext("mailx: Panic - %s\n"), str);
	exit(1);
	/* NOTREACHED */
}
...
source(char name[])
{
	...
	if ((fi = fopen(cp, "r")) == NULL) {
		(void) printf(gettext("Unable to open %s\n"), cp);
		return (1);
	}
...
#endif
			maxfiles = _NFILE;
		sstack = (struct sstack *)calloc(maxfiles,
...
{
	if (ssp < 0) {
		(void) printf(gettext("\"Source\" stack over-pop.\n"));
		sourcing = 0;
		return;
	}
	(void) fclose(input);
	if (cond != CANY)
		(void) printf(gettext("Unmatched \"if\"\n"));
...
alter(char name[])
{
	...
	if (rc != 0) {
		...
		(void) fprintf(stderr, gettext("Errno: %d\n"), errno);
	}
}
	...
	*cp2 = '\0';
	for (;;) {
		...
		if ((wint = substr(cp, "remote from ")) != -1) {
			cp += wint + 12;
			if (first) {
				(void) copy(cp, namebuf);
				first = 0;
			} else {
				tmp = strrchr(namebuf, '!') + 1;
				nstrcpy(tmp,
				    sizeof (namebuf) - (tmp  - namebuf), cp);
			}
	...
	if (value("mustbang") && !strchr(cp, '!')) {
		(void) snprintf(linebuf, sizeof (linebuf), "%s!%s", host, cp);
		cp = linebuf;
	}
...
splice(char *addr, char *hdr)
{
	...
	if (cp = strchr(hdr, '<')) {
		...
		(void) snprintf(buf, sizeof (buf), "%s %s", addr, cp);
	} else
		nstrcpy(buf, sizeof (buf), addr);

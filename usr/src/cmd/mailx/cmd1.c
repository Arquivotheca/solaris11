...
headers(int *msgvec)
{
	...
	if (flag == 0) {
		(void) printf(gettext("No more mail.\n"));
		return (1);
	}
	return (0);
}
...
printhead(int mesg)
{
	 ...
	 */
		...
		if (value("showto") &&
			...
			showto = 1;
			(void) yankword(cp, fromline = name, sizeof (name),
			    docomma(cp));
		}
		if (value("showname"))
	...
	(void) printf("%c%c%3d ", curind, dispc, mesg);
	if ((int)value("showfull")) {
		if (showto)
			(void) printf("To %-15s ", fromline);
		else
			(void) printf("%-18s ", fromline);
	} else {
		if (showto)
			(void) printf("To %-15.15s ", fromline);
		else
			(void) printf("%-18.18s ", fromline);
	}
	if (mp->m_text) {
		(void) printf("%16.16s %4ld/%-5ld %-.25s\n",
		    hl.l_date, mp->m_lines, mp->m_size, subjline);
	} else {
		(void) printf("%16.16s binary/%-5ld %-.25s\n",
...
folders(char **arglist)
{
	...
	if (getfold(dirname) < 0) {
		(void) printf(gettext("No value set for \"folder\"\n"));
		return (-1);
	}
	if (*arglist) {
	...
	(void) snprintf(cmd, sizeof (cmd), "%s %s", LS, dirname);
	return (system(cmd));
}

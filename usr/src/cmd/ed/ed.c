...
#define	PUTM()	if (xcode >= 0) (void) puts(gettext(msgtab[xcode]))
	...
	*argv = p1 + 1;
	...
	for (;;) {
		while ((c = getopt(argc, argv, "sp:qxC")) != EOF) {
			...
			case 'x':
			case 'C':
				(void) fprintf(stderr, gettext("-x and -C "
	...
	if (argc > 0) {
		...
		if (strlen(p1) >= (size_t)FNSIZE) {
			(void) puts(gettext("file name too long"));
			exit(2);
		}
		p2 = savedfile;
		while (*p2++ = *p1++)
...
commands(void)
{
	...
	if (shflg && globp == 0)
		(void) write(1, gettext(prompt), strlen(gettext(prompt)));
	addr1 = 0;
	addr2 = 0;
	if ((c = getchr()) == ',') {
		addr1 = zero + 1;
		...
		if (Xqt) {
			...
			if (c == 'w' && Fl.st_nlink == 1 && ISREG(lFl)) {
				...
				if ((io = creat(funny, FMODE(Fl))) >= 0) {
					(void) chown(funny, Fl.st_uid,
					    Fl.st_gid);
					(void) chmod(funny, FMODE(Fl));
					putfile();
					exfile();
...
exfile(void)
{
	...
	if (vflag) {
		...
		if (nulls) {
			...
			nulls = 0;
			putd();
			(void) puts(gettext(" nulls replaced by '\\0'"));
			count = c;
		}
#endif
	}
...
onintr(int sig)
{
	...
	if (readflg) {
		(void) sprintf(tstring,
		    "\007read may be incomplete - beware!\007");
		(void) puts(gettext(tstring));
		fchange = 0;
	}
	(void) error(26);
}
...
onhup(int sig)
{
	...
	if (dol > zero && fchange == 1) {
		...
		if (io < 0 && home) {
			...
			if (asprintf(&fn, "%s/ed.hup", home) != -1) {
...
getfile(void)
{
	...
	*--lp = 0;
	...
	if (fss.Ffill && fss.Flim && lenchk(linebuf, &fss) < 0) {
		(void) write(1, gettext("line too long: lno = "),
...
unixcom(void)
{
	...
	*pcurcmd++ = 0;
	...
	if ((pid = fork()) == 0) {
		...
		(void) execlp((const char *)shpath, "sh", "-c",
		    curcmd, (char *)0);
		exit(0100);
	}
...
{
	(void) lseek(tfile, (long)b<<9, SEEK_SET);
...
init(void)
{
	...
	if ((tfile = open(tfname, O_CREAT|O_EXCL|O_RDWR,
	    S_IRUSR|S_IWUSR)) < 0) {
		(void) puts(gettext(msgtab[70]));
		exit(2);
	}

	(void) umask(omask);
	(void) brk((char *)fendcore);
...
global(int k)
{
	...
	setall();
	nonzero();
	if (_mbftowc(multic, &l, getchr, &peekc) <= 0)
		(void) error(67);
	if (l == '\n')
		(void) error(19);
	save();
	...
	*p++ = 0;
	if (rhsbuf[0] == '%' && rhsbuf[1] == 0)
		...
		if (remflg == -1)
			...
			(void) strcpy(rhsbuf, remem);
	else {
		(void) strcpy(remem, rhsbuf);
		remflg = 0;
	}
	c = 0;
...
puts(const char *sp)
{
	...
	if (fss.Ffill && (listf == 0)) {
		...
		if ((i = expnd((char *)sp, funny, &sz, &fss)) == -1) {
			(void) write(1, funny, fss.Flim & 0377);
			putchr('\n');
			(void) write(1, gettext("too long"),
		...
		putchr('\n');
		if (i == -2)
			(void) write(1, gettext("tab count\n"),
...
#else
	if (listf) {
	/* xpg6 - ensure that the end of line $ is not preceded with a "\" */
		...
		switch (c) {
			...
			case '\v' :
				...
				*lp++ = 'v';
				...
				break;
#ifdef XPG6
		/* if $ characters are within the line precede with \ */
			case '$' :
				*lp++ = '\\';
				*lp++ = '$';
				col += 2;
...
globaln(int k)
{
	...
	setall();
	nonzero();
	if (_mbftowc(multic, &cl, getchr, &peekc) <= 0)
		(void) error(67);
	if (cl == '\n')
		(void) error(19);
	save();
...
#define	w_or_r(a, b) (rw ? a : b)
	...
	if (!Xqt) {
		if ((io = open(string, rw)) >= 0) {
			if (fflg) {
				(void) read(io, crbuf, LBSIZE);
				if (fspec(crbuf, &fss, 0) < 0) {
					fss.Ffill = 0;
					fflg = 0;
					(void) error(4);
				}
				(void) lseek(io, 0L, SEEK_SET);
			}
		}
		fflg = 0;
		return (io);
...
xerr:		(void) error(0);
	if ((i = fork()) == 0) {
		...
		(void) execlp((const char *)shpath, "sh", "-c", string, NULL);
		exit(1);
	}
	if (i == (pid_t)-1)
		goto xerr;

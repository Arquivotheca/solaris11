 ...
 * sublogin tells us whether we need to insist on utmpx.
 */
...
/* ONC_PLUS EXTRACT END */
	...
	if (geteuid() != 0) {
		(void) fprintf(stderr, "login: Permission denied\n");
		(void) sleep(1);
		exit(1);
	}
...
/* ONC_PLUS EXTRACT END */
	...
	/*
	 * We'll might need to ask the user to change his password in
	 * validate_account or we are done; either way, we need to disable
	 * the timeout.
	 */
	(void) alarm(0);

	/*
	 * Check account expiration and passwd aging; not needed when
	 * logging in from to a non-global zone and with full permission.
	 */
	if (!(zflag && fflag))
		validate_account();
...
get_options(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "u:s:R:f:h:r:pad:t:U:z:")) != -1) {
		...
		case 'z':
			...
			zflag = B_TRUE;
			/* Don't require a utmpx entry for zlogin. */
			sublogin = 1;
			break;
		default:
			errflg++;
	...
	for (j = 0, k = 0, l_index = 0;
	    *envp != NULL && j < (MAXARGS-1);
		...
		if ((endptr = strchr(*envp, '=')) == NULL) {
			...
			if (!legalenvvar(*envp)) { /* this env var permitted? */
				continue;
			} else {

...
"Usage: %s [[-e | -i | -I] | -A ] [-f] [-mvVp] [-c class] [-R root]\n"
	...
	"\t-E  select events that is associated by ENA increment\n";
/* END CSTYLED */
...
static uint64_t *
getenaopt(const char *arg)
{
	uint64_t *argi;
	char *end;

	/*
	 * Use strtoull endptr argument to ensure that the entire
	 * 'arg' string is converted.
	 */
	argi = calloc(1, sizeof (*argi));

	errno = 0;
	*argi = strtoull(arg, &end, 16);

	if (errno != 0 || end == arg || *end != '\0' || *argi > UINT64_MAX)
		fmdump_usage("unexpected hex value of ena %s\n", arg);
...
main(int argc, char *argv[])
{
	...
	while (optind < argc) {
		while ((c =
		    getopt(argc, argv, "Aac:eE:fHiImn:O:pR:t:T:u:vV")) != EOF) {
			switch (c) {
			case 'A':
				opt_A++;
				break;
			...
			case 'n':
				...
				break;
			case 'E':
				fltfv[fltfc].filt_func = fmd_log_filter_ena;
				fltfv[fltfc].filt_arg = getenaopt(optarg);
				allfv[allfc++] = fltfv[fltfc++];
				break;
			case 'v':
				opt_v++;

...
fmd_time_ena2hrt(hrtime_t hrt, uint64_t ena)
{
	...
	case FM_ENA_FMT1:
		/*
		 * An ereport with (gen == 0) is a "primary" ereport and
		 * (ena & ENA_FMT1_TIME_MASK) will give an accurate reflection
		 * of the time (better than hrt). If (gen > 0), then it is
		 * a "secondary" ereport and only hrt will give the time
		 * (slightly less accurately than for the primary)
		 */
		if ((ena & ENA_FMT1_GEN_MASK) > 0)
			return (hrt);

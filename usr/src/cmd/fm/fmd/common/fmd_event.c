    ...
    nvlist_t *nvl, void *data, fmd_log_t *lp, off64_t off, size_t len)
{
	...
	fmd_dprintf(FMD_DBG_EVT, "%s: ep=0x%p, ev_type=%d(%s), ev_nvl=0x%p, "
	    "ev_data=0x%p, ev_hrt=0x%llx\n", __func__, (void *)ep, type,
	    fmd_event_type2str(type), (void *)ep->ev_nvl,
	    (void *)ep->ev_data, ep->ev_hrt);
...
fmd_event_destroy(fmd_event_t *e)
{
	 ...
	 * event has not yet been committed, then attempt to commit it now.
	 */
	...
	case FMD_EVT_CTL:
		...
		break;
	case FMD_EVT_TOPO:
		fmd_topo_rele(ep->ev_data, __func__);
		break;
	}
...
fmd_event_hold(fmd_event_t *e)
{
	...
	fmd_dprintf(FMD_DBG_EVT, "%s: e=0x%p, ev_type=%d, ev_data=0x%p, "
	    "ev_refs=(%d)->(%d)\n", __func__, (void *)e, ep->ev_type,
	    (void *)ep->ev_data, ep->ev_refs, ep->ev_refs + 1);
...
fmd_event_rele(fmd_event_t *e)
{
	...
	if (ep->ev_type == FMD_EVT_CTL)
		fmd_ctl_rele(ep->ev_data, B_TRUE);

	(void) pthread_mutex_lock(&ep->ev_lock);

	fmd_dprintf(FMD_DBG_EVT, "%s: ep=0x%p, ev_type=%d, ev_data=0x%p, "
	    "ev_refs=(%d)->(%d)\n", __func__, (void *)e, ep->ev_type,
	    (void *)ep->ev_data, ep->ev_refs, ep->ev_refs - 1);
...
/*
 * This function is the same as fmd_event_rele(), except that it does
 * not block waiting for a control event reference count to become 0.
 */
void
fmd_event_rele_no_ctl_wait(fmd_event_t *e)
{
	fmd_event_impl_t *ep = (fmd_event_impl_t *)e;

	if (ep->ev_type == FMD_EVT_CTL)
		fmd_ctl_rele(ep->ev_data, B_FALSE);
...
const char *
fmd_event_type2str(uint8_t ev_type)
{
	switch (ev_type)  {
	case FMD_EVT_PROTOCOL:
		return (FMD_EVT_PROTOCOL_STR);
	case FMD_EVT_TIMEOUT:
		return (FMD_EVT_TIMEOUT_STR);
	case FMD_EVT_CLOSE:
		return (FMD_EVT_CLOSE_STR);
	case FMD_EVT_STATS:
		return (FMD_EVT_STATS_STR);
	case FMD_EVT_GC:
		return (FMD_EVT_GC_STR);
	case FMD_EVT_CTL:
		return (FMD_EVT_CTL_STR);
	case FMD_EVT_PUBLISH:
		return (FMD_EVT_PUBLISH_STR);
	case FMD_EVT_TOPO:
		return (FMD_EVT_TOPO_STR);
	default:
		return (FMD_EVT_UNKNOWN_STR);
	}
}

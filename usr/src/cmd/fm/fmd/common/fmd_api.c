...
fmd_module_thrcancel(fmd_idspace_t *ids, id_t id, fmd_module_t *mp)
{
	 ...
	 */
	if (tp->thr_isdoor) {
		fmd_dprintf(FMD_DBG_MOD, "%s: not cancelling %s private door "
		    "thread %u\n", __func__, mp->mod_name, tp->thr_tid);
		fmd_thread_destroy(tp, FMD_THREAD_NOJOIN);
		return;
	}

	fmd_dprintf(FMD_DBG_MOD, "%s: cancelling %s auxiliary thread %u\n",
	    __func__, mp->mod_name, tp->thr_tid);
...
boolean_t
fmd_hdl_unloading(fmd_hdl_t *hdl)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
...
fmd_hdl_topo_hold(fmd_hdl_t *hdl, int v)
{
	...
	fmd_dprintf(FMD_DBG_TOPO, "%s: mod=%s, ftp=0x%p, thp=0x%p, "
	    "refcount=%d\n", __func__, mp->mod_name,
	    (void *)mp->mod_topo_current, (void *)mp->mod_topo_current->ft_hdl,
...
fmd_hdl_vdebug(fmd_hdl_t *hdl, const char *format, va_list ap)
{
	...
	if (fmd.d_fmd_debug & FMD_DBG_TSTAMP)
		hrtime = gethrtime();
	...
	if (fmd.d_hdl_dbout & FMD_DBOUT_STDERR) {
		(void) pthread_mutex_lock(&fmd.d_err_lock);
		if (fmd.d_fmd_debug & FMD_DBG_TSTAMP)
			(void) fprintf(stderr, "%s DEBUG: %llx: %s: %s",
			    fmd.d_pname, hrtime, mp->mod_name, msg);
		else
...
fmd_case_add_suspect(fmd_hdl_t *hdl, fmd_case_t *cp, nvlist_t *nvl)
{
	 ...
	 * property, if it exists, and add it to the fru fmri. Note that the
	 * serial may be on the ident_node rather than on the fru itself.
	 */
	if (fru != NULL) {
		nvlist_t *ident_node = NULL;

		(void) topo_fmri_ident_node(thp, fru, &ident_node, &err);
		fmd_asru_set_serial(thp, fru, ident_node);
		if (ident_node != NULL) {
			(void) nvlist_add_nvlist(nvl, FM_FAULT_IDENT_NODE,
			    ident_node);
			nvlist_free(ident_node);
		}
	}
    ...
    void *cookie)
{
	...
	if (dip && mp->mod_stats->ms_doorthrtotal.fmds_value.ui32 >=
		...
		fmd_module_unlock(mp);
		(void) fmd_dprintf(FMD_DBG_XPRT, "%s: door server %s for %p "
		    "not attemped - at max\n",
		    __func__, dip->di_attributes & DOOR_DEPLETION_CB ?
	...
	if (dip) {
		fmd_dprintf(FMD_DBG_XPRT, "%s: door server startup for %p %s\n",
		    __func__, (void *)dip, new_tp ? "successful" : "failed");
	}
...
ssize_t
fmd_nvl_fmri_nvl2str(fmd_hdl_t *hdl, nvlist_t *nvl, char *buf, size_t buflen)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	ssize_t rv;
	...
	rv = fmd_fmri_nvl2str(nvl, buf, buflen);
	fmd_module_unlock(mp);
...
fmd_nvl_fmri_present(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	...
	ps = fmd_fmri_presence_state(nvl);
	rv = (ps == FMD_OBJ_STATE_STILL_PRESENT ||
	    ps == FMD_OBJ_STATE_UNKNOWN) ? 1 : 0;
	fmd_module_unlock(mp);
...
int
fmd_nvl_fmri_presence_state(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	int rv;
	...
	rv = fmd_fmri_presence_state(nvl);
	fmd_module_unlock(mp);
...
fmd_nvl_fmri_unusable(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	...
	switch (fmd_fmri_service_state(nvl)) {
...
fmd_rsrc_has_fault(fmd_asru_link_t *alp, void *arg)
{
	...
	if (!(fmd_asru_al_getstate(alp) & FMD_ASRU_PRESENT))
		return;
	...
	if (fhfp->class != NULL) {
		if (nvlist_lookup_string(alp->al_event, FM_CLASS, &class)
		    != 0 || !fmd_strmatch(class, fhfp->class))
			return;
	}

	if (fhfp->flags & FMD_HAS_FAULT_RETIRE) {
		if (nvlist_lookup_boolean_value(alp->al_event,
		    FM_SUSPECT_RETIRE, &b) == 0 && !b)
			return;
	}

	if (fhfp->flags & FMD_HAS_FAULT_RESPONSE) {
		if (nvlist_lookup_boolean_value(alp->al_event,
		    FM_SUSPECT_RESPONSE, &b) == 0 && !b)
			return;
	}
...
int
fmd_nvl_fmri_has_fault(fmd_hdl_t *hdl, nvlist_t *nvl, int flags, char *class)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	...
	type = flags & FMD_HAS_FAULT_TYPE_MASK;
...
void
fmd_resolved_asru(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	fmd_asru_hash_t *ahp = fmd.d_asrus;
	char *name;
	...
	fmd_asru_hash_apply_by_asru(ahp, name, fmd_asru_resolved, NULL,
	    B_FALSE);
	if (name != NULL)
		fmd_free(name, namelen + 1);
	fmd_module_unlock(mp);
...
void
fmd_isolated_asru(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	fmd_asru_hash_t *ahp = fmd.d_asrus;
	char *name;
	...
	fmd_asru_hash_apply_by_asru(ahp, name, fmd_asru_isolated, NULL,
	    B_FALSE);
	if (name != NULL)
		fmd_free(name, namelen + 1);
	fmd_module_unlock(mp);
...
void
fmd_resolved_fru(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	fmd_asru_hash_t *ahp = fmd.d_asrus;
	char *name;
	...
	fmd_asru_hash_apply_by_fru(ahp, name, fmd_asru_resolved, NULL,
	    B_FALSE);
	if (name != NULL)
		fmd_free(name, namelen + 1);
	fmd_module_unlock(mp);
...
void
fmd_isolated_fru(fmd_hdl_t *hdl, nvlist_t *nvl)
{
	fmd_module_t *mp = fmd_api_module_lock(hdl);
	fmd_asru_hash_t *ahp = fmd.d_asrus;
	char *name;
	...
	fmd_asru_hash_apply_by_fru(ahp, name, fmd_asru_isolated, NULL,
	    B_FALSE);
	if (name != NULL)
		fmd_free(name, namelen + 1);
	fmd_module_unlock(mp);

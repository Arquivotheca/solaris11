...
static const fmd_conf_mode_t _fmd_debug_modes[] = {
	...
	{ "error", "debug errors", FMD_DBG_ERR },
	...
	{ "startup", "debug fmd startup ", FMD_DBG_STARTUP },
	{ "topo", "debug topo routines ", FMD_DBG_TOPO },
	{ "tstamp", "enable timestamps on debug messages", FMD_DBG_TSTAMP },
	{ "all", "enable all available debug modes", FMD_DBG_ALL },
	{ NULL, NULL, 0 }
};

static const fmd_conf_mode_t _fmd_debug_topo_modes[] = {
	{ "no_snap_load", "don't load snapshot", FMD_DBG_TOPO_NO_SNAP_LOAD },
	{ "no_snap_save", "don't save snapshot", FMD_DBG_TOPO_NO_SNAP_SAVE },
	{ "no_snap_use", "don't use snapshot", FMD_DBG_TOPO_NO_SNAP_USE },
	{ "no_snap_switch", "don't switch snapshot",
	    FMD_DBG_TOPO_NO_SNAP_SWITCH },
	{ "no_startup_delay", "don't delay on startup",
	    FMD_DBG_TOPO_NO_STARTUP_DELAY },
	{ "no_update_delay", "don't delay on topo updates",
	    FMD_DBG_TOPO_NO_UPDATE_DELAY },
	{ "no_dump_clear", "don't clear crash dump of ereports",
	    FMD_DBG_TOPO_NO_DUMP_CLEAR },
	{ "load_on_update", "load a snapshot on topo update",
	    FMD_DBG_TOPO_UPDATE_LOAD },
	{ NULL, NULL, 0 }
};
...
static int
fmd_debug_topo_set(fmd_conf_param_t *pp, const char *value)
{
	int err = fmd_conf_mode_set(_fmd_debug_topo_modes, pp, value);

	if (err == 0)
		fmd.d_fmd_debug_topo = pp->cp_value.cpv_num;
...
static const fmd_conf_ops_t fmd_debug_topo_ops = {
	fmd_debug_topo_set, fmd_conf_mode_get, fmd_conf_notsup, fmd_conf_nop
};
...
{ "topo.dir", &fmd_conf_string, "var/fm/fmd/topo" }, /* topo snap dir path */
{ "topo.dirlim", &fmd_conf_uint32, "5" },	/* topo snap dir limit */
{ "topo.dirmode", &fmd_conf_int32, "0755" },	/* topo snap dir perm mode */
...
{ "debug.topo", &fmd_debug_topo_ops, NULL },	/* topo debug flags */
...
{ "topo.startup_delay", &fmd_conf_time, "10sec" }, /* delay for startup snap */
#ifdef DEBUG
...
{ "dr.filter_class", FMD_TYPE_UINT64,
	"dynamic reconfiguration filter: class" },
{ "dr.filter_time", FMD_TYPE_UINT64,
	"dynamic reconfiguration filter: time" },
{ "dr.filter_cna_dev", FMD_TYPE_UINT64,
	"dynamic reconfiguration filter: cna_dev" },
{ "dr.filter_debug", FMD_TYPE_UINT64,
	"dynamic reconfiguration filter: debug" },
...
fmd_create(fmd_t *dp, const char *arg0, const char *root, const char *conf)
{
	...
	if ((shp = smbios_open(NULL, SMB_VERSION, 0, NULL)) != NULL) {
		...
		if (di_prom_prop_lookup_bytes(promh, rooth, "name",
		    (unsigned char **)&bufp) != -1) {
			fmd_cleanup_auth_str(_fmd_prod, bufp);
		}
	}
...
fmd_destroy(fmd_t *dp)
{
	...
	/*
	 * Call fmd_topo_cleanup in case we end up delayed and killed
	 * while performing other cleanup/destroy operations.
	 */
	fmd_topo_cleanup();
	...
	/*
	 * Close all log files now that modules are no longer active.  We must
	...
	if (dp->d_hvilog != NULL) {
		fmd_log_rele(dp->d_hvilog);
		dp->d_hvilog = NULL;
	}

	if (dp->d_ilog != NULL) {
		fmd_log_rele(dp->d_ilog);
		dp->d_ilog = NULL;
	}
...
/*ARGSUSED*/
static void
fmd_startup_delay(fmd_t *dp, id_t id, hrtime_t hrt)
{
	if (dp->d_mod_start_topo_event == NULL) {
		fmd_dprintf(FMD_DBG_ERR, "%s: NULL delay event!\n", __func__);
		return;
	}

	fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
	    "%s: releasing delay event ep=0x%p\n", __func__,
	    (void *)dp->d_mod_start_topo_event);

	fmd_event_rele(dp->d_mod_start_topo_event);
	dp->d_mod_start_topo_event = NULL;
}
...
{
	fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_XPRT,
...
fmd_run(fmd_t *dp, int pfd)
{
	...
	fmd_dprintf(FMD_DBG_STARTUP, "%s: dp=0x%p, fmd_debug=0x%x, "
	    "fmd_debug_topo=0x%x\n", __func__, (void *)dp,
	    dp->d_fmd_debug, dp->d_fmd_debug_topo);
	...
	fmd_dprintf(FMD_DBG_STARTUP, "%s: initializing topology\n", __func__);
	fmd_topo_init();

	dp->d_xprt_ids = fmd_idspace_create("xprt_ids", 1, INT_MAX);
	fmd_xprt_suspend_all();

	fmd_dprintf(FMD_DBG_STARTUP, "%s: creating door server\n", __func__);
	 ...
	 */
	dp->d_mod_start_sync_event = fmd_event_create(FMD_EVT_CTL,
	    FMD_HRT_NOW, NULL, fmd_ctl_init(NULL));

	fmd_dprintf(FMD_DBG_STARTUP, "%s: created initial control event "
	    "ep=0x%p, ev_hrt=0x%llx\n", __func__,
	    (void*)dp->d_mod_start_sync_event,
	    fmd_event_hrtime(dp->d_mod_start_sync_event));

	fmd_event_hold(dp->d_mod_start_sync_event);
	 ...
	 */
	fmd_dprintf(FMD_DBG_STARTUP, "%s: creating module hash\n", __func__);
	dp->d_mod_hash = fmd_modhash_create();

	fmd_dprintf(FMD_DBG_STARTUP, "%s: loading built-in modules\n",
	    __func__);
	...
	fmd_dprintf(FMD_DBG_STARTUP, "%s: initializing rpc\n", __func__);
	 ...
	 * The presence state for resources in the cache will be based on
	 * the current topology and not the snapshot. This is becasue we
	 * call fmd_fmri_topo_hold() which always returns the current topo.
	 */
	fmd_dprintf(FMD_DBG_STARTUP, "%s: rebuilding resource cache\n",
	    __func__);
	fmd_asru_hash_refresh(dp->d_asrus);

	/*
	 * Create a control event and a topo change event to switch from
	 * the snapshot to the current topology.
	 *
	 * The control event allows us to delay execution of modules for a
	 * little while. This allows old events based on the old topology
	 * to come in and get queued in front of the topo change event
	 * while any events with a later timestamp get queued after.
	 *
	 * We create these events here so that they will have an earlier
	 * timestamp than any timeout events that modules may create during
	 * initialization. This is done so that module timeout functions
	 * will use the current topology rather the previous one.
	 *
	 * Note that the sysevent module which is a builtin was loaded
	 * above therefore its timeout event used to read ereports from
	 * the crash dump will trigger before these events. This will
	 * allow it to queue ereports ahead of the topo change event
	 * before the control event is released.
	 */
	if (!FMD_DEBUG_TOPO_NO_STARTUP_DELAY &&
	    (dp->d_topo_previous != NULL)) {
		dp->d_mod_start_topo_event = fmd_event_create(FMD_EVT_CTL,
		    FMD_HRT_NOW, NULL, fmd_ctl_init(NULL));
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: created control event for module startup "
		    "delay, ep=0x%p, ev_hrt=0x%llx\n", __func__,
		    (void *)dp->d_mod_start_topo_event,
		    fmd_event_hrtime(dp->d_mod_start_topo_event));
		fmd_event_hold(dp->d_mod_start_topo_event);
	} else {
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: not creating delay event\n", __func__);
	}
	if (!FMD_DEBUG_TOPO_NO_SNAP_SWITCH && !FMD_DEBUG_TOPO_NO_SNAP_USE &&
	    (dp->d_topo_previous != NULL)) {
		e_topo = fmd_event_create(FMD_EVT_TOPO, FMD_HRT_NOW, NULL,
		    dp->d_topo_current);
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: created topo event e_topo=0x%p, e_topo_hrt=0x%llx, "
		    "e_topo_data=0x%p\n", __func__, (void *)e_topo,
		    fmd_event_hrtime(e_topo), (void *)dp->d_topo_current);
	} else {
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: not creating topo event\n", __func__);
	}

	fmd_dprintf(FMD_DBG_STARTUP, "%s: loading plugins\n", __func__);
	...
	/*
	 * Enqueue the control and topo change events created above.
	 * Transports are excluded in case some of them process old
	 * events and transform them (e.g. fabric-translate).
	 */
	if (!FMD_DEBUG_TOPO_NO_STARTUP_DELAY &&
	    (dp->d_mod_start_topo_event != NULL)) {
		fmd_modhash_dispatch(fmd.d_mod_hash,
		    dp->d_mod_start_topo_event, FMD_MOD_DISP_NONXPRT, NULL);
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: dispatched control event for module startup "
		    "delay, ep=0x%p, ev_hrt=0x%llx\n", __func__,
		    (void *)dp->d_mod_start_topo_event,
		    fmd_event_hrtime(dp->d_mod_start_topo_event));
	}
	if (!FMD_DEBUG_TOPO_NO_SNAP_SWITCH && !FMD_DEBUG_TOPO_NO_SNAP_USE &&
	    (e_topo != NULL)) {
		fmd_modhash_dispatch(fmd.d_mod_hash, e_topo, NULL, NULL);
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: dispatched topo event e_topo=0x%p, "
		    "e_topo_hrt=0x%llx\n", __func__, (void *)e_topo,
		    fmd_event_hrtime(e_topo));
	}
	 ...
	 * that did not finish processing the last time ran.
	 */
	fmd_dprintf(FMD_DBG_STARTUP, "%s: replaying fault events\n", __func__);
	fmd_asru_hash_replay(dp->d_asrus);

	fmd_dprintf(FMD_DBG_STARTUP, "%s: replaying log events\n", __func__);
	(void) pthread_rwlock_rdlock(&dp->d_log_lock);
	 ...
	 */
	fmd_dprintf(FMD_DBG_STARTUP, "%s: replaying list events\n", __func__);
	fmd_case_repair_replay();

	if (dp->d_topo_previous) {
		fmd_dprintf(FMD_DBG_TOPO, "%s: previous ftp=0x%p, "
		    "thp=0x%p, uuid=%s, refcount=%d\n", __func__,
		    (void *)dp->d_topo_previous,
		    (void *)dp->d_topo_previous->ft_hdl,
		    topo_hdl_uuid(dp->d_topo_current->ft_hdl),
		    dp->d_topo_previous->ft_refcount);
	}
	if (dp->d_topo_current) {
		fmd_dprintf(FMD_DBG_TOPO, "%s: current ftp=0x%p, "
		    "thp=0x%p, uuid=%s, refcount=%d\n", __func__,
		    (void *)dp->d_topo_current,
		    (void *)dp->d_topo_current->ft_hdl,
		    topo_hdl_uuid(dp->d_topo_current->ft_hdl),
		    dp->d_topo_current->ft_refcount);
	}

	/*
	 * Release the global module start barrier by executing a final
	 * rele on d_mod_start_sync_event.
	 */
	fmd_dprintf(FMD_DBG_STARTUP, "%s: releasing startup sync event "
	    "e=0x%p\n", __func__, (void*)dp->d_mod_start_sync_event);
	fmd_event_rele(dp->d_mod_start_sync_event);
	dp->d_mod_start_sync_event = NULL;
	 ...
	 */
	fmd_dprintf(FMD_DBG_STARTUP, "%s: resuming transports\n", __func__);
	fmd_xprt_resume_all();

	fmd_gc(dp, 0, 0);

	fmd_dprintf(FMD_DBG_STARTUP, "%s: clearing aged resources\n",
	    __func__);
	fmd_clear_aged_rsrcs(dp, 0, 0);

	/*
	 * At this point, if we found a previous topo snapshot, then that
	 * is what modules start with.  All modules (except for transports)
	 * will eventually block on the synchronization event, waiting for
	 * FMD and the other modules to release it. Any events that come
	 * in while the module is blocked will be placed on the queue in
	 * chronological order based on event timestamp. This includes fault
	 * events that might be diagnosed by DEs prior to them releasing
	 * the synchronization event. When the synchronization event is
	 * is released by fmd later, modules will consume the topo change
	 * event and switch their active topology to the current topology.
	 *
	 * The module event queues might look like this by the time we
	 * release the control event.
	 *
	 *	FMD_EVT_PROTOCOL	new events from transports (if any)
	 *	FMD_EVT_PROTOCOL	faults diagnosed from old events
	 *	FMD_EVT_PROTOCOL	replayed faults (based on current topo)
	 *	FMD_EVT_TOPO		topology switch event (to current)
	 *	FMD_EVT_PROTOCOL	old events from transports (if any)
	 *	FMD_EVT_PROTOCOL	old events from error log (if any)
	 *	FMD_EVT_CTL		sync event (non-xprts block here)
	 *	<start of module queue>	modules see previous topology
	 */

	/* Install a timer to release the topo change delay event. */
	if ((!FMD_DEBUG_TOPO_NO_STARTUP_DELAY) &&
	    (dp->d_mod_start_topo_event != NULL)) {
		(void) fmd_conf_getprop(dp->d_conf, "topo.startup_delay",
		    &period);
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: installing startup delay timer, period=0x%llx\n",
		    __func__, period);
		(void) fmd_timerq_install(dp->d_timers,
		    dp->d_rmod->mod_timerids, (fmd_timer_f *)fmd_startup_delay,
		    dp, NULL, period);
	} else {
		fmd_dprintf(FMD_DBG_STARTUP | FMD_DBG_TOPO,
		    "%s: not installing startup delay timer\n", __func__);
	}
	...
	fmd_dprintf(FMD_DBG_STARTUP, "%s: fmd startup done\n", __func__);
}

...
{
	fmd_topo_rele_hdl(thp, __func__);
}
...
fmd_fmri_presence_state(nvlist_t *nvl)
{
	...
	(void) pthread_mutex_lock(&sp->sch_opslock);
	rv = sp->sch_ops.sop_presence_state(nvl);
	(void) pthread_mutex_unlock(&sp->sch_opslock);
...
fmd_fmri_contains(nvlist_t *er, nvlist_t *ee)
{
	...
	fmd_dprintf(FMD_DBG_FMRI, "%s: er=0x%p,, ee=0x%p\n",
	    __func__, (void *)er, (void *)ee);

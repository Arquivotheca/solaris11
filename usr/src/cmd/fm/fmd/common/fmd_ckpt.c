...
fmd_ckpt_resv_case(fmd_ckpt_t *ckp, fmd_case_t *cp)
{
	...
	ckp->ckp_strn += strlen(cip->ci_topo_uuid) + 1;
}
...
fmd_ckpt_save_case(fmd_ckpt_t *ckp, fmd_case_t *cp)
{
	...
	fcfc.fcfc_topo_uuid = fmd_ckpt_string(ckp, cip->ci_topo_uuid);
	...
	case FMD_CASE_CLOSE_WAIT:
		if (cip->ci_flags & FMD_CF_REPAIRED)
			fcfc.fcfc_state = FCF_CASE_REPAIR_WAIT;
		else if (cip->ci_flags & FMD_CF_ISOLATED)
			fcfc.fcfc_state = FCF_CASE_CLOSE_WAIT;
		else
			fcfc.fcfc_state = FCF_CASE_DISCARD_WAIT;
		break;
	default:
...
fmd_ckpt_restore_case(fmd_ckpt_t *ckp, fmd_module_t *mp, const fcf_sec_t *sp)
{
	...
	if (uuid == NULL || fcfc->fcfc_state > FCF_CASE_STATE_MAX) {
		fmd_ckpt_error(ckp, EFMD_CKPT_INVAL, "corrupt %u case uuid "
		    "and/or state\n", (uint_t)(sp - ckp->ckp_secp));
	}

	topo_uuid = fmd_ckpt_strptr(ckp, fcfc->fcfc_topo_uuid, NULL);

	if (topo_uuid == NULL)
		fmd_dprintf(FMD_DBG_CKPT | FMD_DBG_ERR | FMD_DBG_TOPO,
		    "%s: NULL topo uuid\n", __func__);
	...
	if ((cp = fmd_case_recreate(mp, NULL,
	    (fcfc->fcfc_state != FCF_CASE_UNSOLVED &&
	    fcfc->fcfc_state != FCF_CASE_DISCARD_WAIT) ? FCF_CASE_SOLVED :
	    FMD_CASE_UNSOLVED, uuid, NULL, topo_uuid)) == NULL) {
		fmd_ckpt_error(ckp, EFMD_CKPT_INVAL,
	 ...
	 */
	if (fmd.d_running && ((fmd_case_impl_t *)cp)->ci_nsuspects == 0)
		(void) fmd_ckpt_restore_suspects(ckp, cp, fcfc->fcfc_suspects);

	if (!fmd.d_running) {
		if (fcfc->fcfc_state != FCF_CASE_UNSOLVED &&
		    fcfc->fcfc_state != FCF_CASE_DISCARD_WAIT)
	...
	else if (fcfc->fcfc_state == FCF_CASE_CLOSE_WAIT)
		fmd_case_transition(cp, FMD_CASE_CLOSE_WAIT, FMD_CF_SOLVED |
		    FMD_CF_ISOLATED);
	else if (fcfc->fcfc_state == FCF_CASE_REPAIR_WAIT)
		fmd_case_transition(cp, FMD_CASE_CLOSE_WAIT, FMD_CF_SOLVED |
		    FMD_CF_REPAIRED);
	else if (fcfc->fcfc_state == FCF_CASE_DISCARD_WAIT)

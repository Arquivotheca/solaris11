...
sysev_legacy(sysevent_t *sep)
{
	...
	/*
	 * If this sysevent resulted in a new topo snapshot then
	 * include some topo info in the payload.
	 */
	if (ftp != NULL) {
		...
		(void) nvlist_add_string(nvl, FM_SUSPECT_TOPO_UUID,
		    topo_hdl_uuid(ftp->ft_hdl));

		fmd_time_sync(&tod0, &hr0, 1);

		fmd_time_hrt2tod(hr0, &tod0, ftp->ft_time_dr, &tod1);
		array[0] = (uint64_t)tod1.ftv_sec;
		array[1] = (uint64_t)tod1.ftv_nsec;
		(void) nvlist_add_uint64_array(nvl, "topo-timeval-dr",
		    array, 2);

		fmd_time_hrt2tod(hr0, &tod0, ftp->ft_time_begin, &tod1);
		array[0] = (uint64_t)tod1.ftv_sec;
		array[1] = (uint64_t)tod1.ftv_nsec;
		(void) nvlist_add_uint64_array(nvl, "topo-timeval-begin",
		    array, 2);

		fmd_time_hrt2tod(hr0, &tod0, ftp->ft_time_end, &tod1);
		array[0] = (uint64_t)tod1.ftv_sec;
		array[1] = (uint64_t)tod1.ftv_nsec;
		(void) nvlist_add_uint64_array(nvl, "topo-timeval-end",
		    array, 2);
	}
...
sysev_replay(fmd_hdl_t *hdl, id_t id, void *arg)
{
	...
	for (;;) {
		...
		fmd_hdl_debug(hdl, "%s: posting ereport, class=%s\n",
		    __func__, class);
...
next:
		 ...
		 *
		 * For debug purposes, allow no clearing of record.
		 */
		if (FMD_DEBUG_TOPO_NO_DUMP_CLEAR)
			fmd_hdl_debug(hdl, "%s: not clearing dump\n", __func__);
		else
			ed.ed_magic = 0;
...
sysev_init(fmd_hdl_t *hdl)
{
	 ...
	 */
	fmd_hdl_debug(hdl, "%s: transport '%s' open\n",
	    __func__, sysev_channel);

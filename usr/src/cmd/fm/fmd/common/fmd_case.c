...
static void
fmd_case_asru_is_faulty(fmd_asru_link_t *alp, void *arg)
{
	int *faultyp = (int *)arg;
...
static void
fmd_case_asru_is_unisolated(fmd_asru_link_t *alp, void *arg)
{
	int *unisolatedp = (int *)arg;

	*unisolatedp |= ((alp->al_flags & FMD_ASRU_FAULTY) &&
	    !(fmd_asru_al_getstate(alp) & FMD_ASRU_UNUSABLE));
}

static void
fmd_case_asru_is_not_faulty(fmd_asru_link_t *alp, void *arg)
{
...
/*
 * Have we got any suspects with an asru that are still unresolved?
 */
static void
fmd_case_asru_is_unresolved(fmd_asru_link_t *alp, void *arg)
{
	int *unresolvedp = (int *)arg;
	int state;
	nvlist_t *asru;

	/*
	 * if this a proxy case and this suspect doesn't have an local asru
	 * then state is unknown so we must assume it may still be unresolved.
	 */
	...
	/*
	 * if no ASRU, nothing to unisolate
	 */
	...
	/*
	 * Consider unresolved if either still faulty, or still unusable but
	 * not removed.
	 */
	*unresolvedp |= ((alp->al_flags & FMD_ASRU_FAULTY) || ((state &
	    FMD_ASRU_UNUSABLE) && (alp->al_reason != FMD_ASRU_REMOVED)));
}

static nvlist_t *
fmd_case_mkevent_cmn(fmd_case_t *cp, const char *class, boolean_t replay)
{
	fmd_case_impl_t *cip = (fmd_case_impl_t *)cp;
	nvlist_t **nva, *nvl;
	uint8_t *ba;
	...
	nvl = fmd_protocol_list(class, cip->ci_diag_de == NULL ?
	    ...
	    nva, ba, msg, &cip->ci_tv, cip->ci_injected, cip->ci_state, replay,
	    cip->ci_topo_uuid);
...
{
	return (fmd_case_mkevent_cmn(cp, class, B_FALSE));
}

nvlist_t *
fmd_case_mkreplayevent(fmd_case_t *cp, const char *class)
{
	return (fmd_case_mkevent_cmn(cp, class, B_TRUE));
}
...
fmd_case_compare_elem(nvlist_t *nvl, nvlist_t *xnvl, const char *elem)
{
	...
	int match = B_FALSE;
	fmd_topo_t *ftp = fmd_topo_hold(__func__);
...
got_match:
	...
	*fccdp->fccd_discard_new = 1;
	 ...
	 */
	if (do_update) {
		if (old_cip->ci_xprt != NULL)
			fmd_case_xprt_updated(old_cp);
		else
...
fmd_case_create(fmd_module_t *mp, const char *uuidstr, void *data)
{
	...
	fmd_topo_t *ftp = mp->mod_topo_current;
	uuid_t uuid;

	fmd_dprintf(FMD_DBG_CASE, "%s: mp=0x%p(%s), uuidstr=%s, data=0x%p, "
	    "topo_uuid=%s\n", __func__, (void *)mp, mp->mod_name,
	    (uuidstr == NULL ? "NULL" : uuidstr), (void *)data,
	...
	cip->ci_topo_uuid = fmd_zalloc(cip->ci_uuidlen + 1, FMD_SLEEP);

	(void) strncpy(cip->ci_topo_uuid, topo_hdl_uuid(ftp->ft_hdl),
	    cip->ci_uuidlen + 1);
	...
	if (uuidstr == NULL) {
		...
		if (fmd_case_hash_insert(fmd.d_cases, cip) != cip) {
			...
			fmd_free(cip->ci_topo_uuid, cip->ci_uuidlen + 1);
			(void) fmd_buf_hash_destroy(&cip->ci_bufs);
			fmd_module_rele(mp);
			pthread_mutex_destroy(&cip->ci_lock);
			fmd_free(cip, sizeof (*cip));
			fmd_dprintf(FMD_DBG_CASE, "%s: case already exists, "
			    "uuid=%s\n", __func__, uuidstr);
			return (NULL);
		}
	}
	fmd_dprintf(FMD_DBG_CASE, "%s: inserted case=0x%p, uuid=%s\n",
	    __func__, (void *)cip, cip->ci_uuid);
...
fmd_case_recreate(fmd_module_t *mp, fmd_xprt_t *xp,
    uint_t state, const char *uuid, const char *code, const char *topo_uuid)
{
	fmd_case_impl_t *cip = fmd_zalloc(sizeof (fmd_case_impl_t), FMD_SLEEP);
	fmd_case_impl_t *eip;

	fmd_dprintf(FMD_DBG_CASE, "%s: mp=0x%p(%s), uuid=%s, code=%s, "
	    "topo_uuid=%s\n", __func__, (void *)mp, mp->mod_name,
	    (uuid == NULL ? "NULL" : uuid), (code == NULL ? "NULL" : code),
	    (topo_uuid == NULL ? "NULL" : topo_uuid));
	...
	cip->ci_topo_uuid = fmd_strdup(topo_uuid, FMD_SLEEP);
...
do_repair:
		 ...
		 * side), or if all suspects are already resolved then
		 * transition straight to RESOLVED state, publishing both the
		 * list.repaired and list.resolved. For a proxy, if we discover
		 * here that all suspects are already resolved, notify the diag
		 * side instead using fmd_xprt_uuresolved().
		 */
		if (!(flags & FMD_CF_RESOLVED)) {
			fmd_asru_hash_apply_by_case(fmd.d_asrus, cp,
			    fmd_case_asru_is_unresolved, &any_not_resolved);
			if (any_not_resolved)
				break;
			if (cip->ci_xprt != NULL) {
	...
	case FMD_CASE_RESOLVED:
		 ...
		 * resolved - this request has come from
		...
		/*
		 * If all suspects are already resolved then
	...
	if (resolved) {
		if (cip->ci_xprt != NULL) {
			...
			cip->ci_flags |= FMD_CF_RES_CMPL | FMD_CF_FLUSH_IMM;
			(void) pthread_mutex_unlock(&cip->ci_lock);
		} else {
			fmd_asru_hash_apply_by_case(fmd.d_asrus, cp,
...
/*ARGSUSED*/
void
fmd_case_set_flush(fmd_case_t *cp, void *arg)
{
	fmd_case_impl_t *cip = (fmd_case_impl_t *)cp;

	(void) pthread_mutex_lock(&cip->ci_lock);
	if (cip->ci_flags & FMD_CF_RES_CMPL)
		cip->ci_flags |= FMD_CF_FLUSH_IMM;
	(void) pthread_mutex_unlock(&cip->ci_lock);
}

/*
 * Discard any case if it is in RESOLVED state (and if FLUSH_IMM is set or
 * if all suspects have passed the rsrc.aged time).
 */
/*ARGSUSED*/
void
fmd_case_discard_resolved(fmd_case_t *cp, void *arg)
{
	 ...
	 * If not, then check if it ought to transition to resolved.
	 */
	(void) pthread_mutex_lock(&cip->ci_lock);
	if (!(cip->ci_flags & FMD_CF_RES_CMPL)) {
		(void) pthread_mutex_unlock(&cip->ci_lock);
		if (cip->ci_state == FMD_CASE_REPAIRED)
			fmd_case_check_resolved(cp);
		return;
	}

	/*
	 * if FLUSH_IMM is not set, see if all suspects have aged.
	 */
	if (!(cip->ci_flags & FMD_CF_FLUSH_IMM)) {
		int aged = 1;
...
fmd_case_transition_update(fmd_case_t *cp, uint_t state, uint_t flags)
{
	...
	int any_unisolated = 0;		/* are any suspects unisolated? */

	ASSERT(state >= FMD_CASE_SOLVED);
	(void) pthread_mutex_lock(&cip->ci_lock);

	fmd_asru_hash_apply_by_case(fmd.d_asrus, cp,
	    fmd_case_asru_is_unisolated, &any_unisolated);
...
fmd_case_update(fmd_case_t *cp)
{
	...
	fmd_asru_hash_apply_by_case(fmd.d_asrus, cp, fmd_case_asru_is_faulty,
	    &faulty);
	(void) pthread_mutex_unlock(&cip->ci_lock);
...
fmd_case_delete(fmd_case_t *cp)
{
	...
	if (cip->ci_flags & FMD_CF_REPAIRED) {
		...
		if (cip->ci_flags & FMD_CF_RESOLVED)
...
fmd_case_repair_replay_case(fmd_case_t *cp, void *arg)
{
	...
	fmd_asru_hash_apply_by_case(fmd.d_asrus, cp, fmd_case_asru_is_faulty,
	    &faulty);
	fmd_asru_hash_apply_by_case(fmd.d_asrus, cp,
	    fmd_case_asru_is_not_faulty, &not_faulty);
	...
	if (cip->ci_state >= FMD_CASE_REPAIRED && !faulty) {
		...
		TRACE((FMD_DBG_CASE, "replay sending list.updated %s",
		    cip->ci_uuid));
		nvl = fmd_case_mkreplayevent(cp, FM_LIST_UPDATED_CLASS);
		(void) nvlist_lookup_string(nvl, FM_CLASS, &class);
		e = fmd_event_create(FMD_EVT_PROTOCOL, FMD_HRT_NOW, nvl, class);
		fmd_dispq_dispatch(fmd.d_disp, e, class);
	}
...
void
fmd_case_check_isolated(fmd_case_t *cp)
{
	fmd_case_impl_t *cip = (fmd_case_impl_t *)cp;
	int any_unisolated = 0;
	uint_t cstate;
	...
	cstate = cip->ci_state;
	(void) pthread_mutex_unlock(&cip->ci_lock);
	if (!any_unisolated && cstate == FMD_CASE_SOLVED)
		fmd_case_transition(cp, FMD_CASE_CLOSE_WAIT, FMD_CF_ISOLATED);
}

void
fmd_case_check_resolved(fmd_case_t *cp)
{
	fmd_case_impl_t *cip = (fmd_case_impl_t *)cp;
	int any_not_resolved = 0;
	uint_t cstate;
	...
	cstate = cip->ci_state;
	(void) pthread_mutex_unlock(&cip->ci_lock);
	if (!any_not_resolved && cstate == FMD_CASE_REPAIRED) {
		if (cip->ci_xprt != NULL)
			fmd_xprt_uuresolved(cip->ci_xprt, cip->ci_uuid);
		else
			fmd_case_transition(cp, FMD_CASE_RESOLVED,
			    FMD_CF_RESOLVED);
	}
}

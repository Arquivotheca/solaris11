...
fmd_eventq_insert_at_head(fmd_eventq_t *eq, fmd_event_t *ep)
{
	...
	fmd_dprintf(FMD_DBG_EVT, "%s: mod_name=%s, eq=0x%p, eq_size=%d, "
	    "ep=0x%p\n", __func__, eq->eq_mod->mod_name, (void *)eq,
	    eq->eq_size, (void *)ep);
...
top:
	...
	fmd_dprintf(FMD_DBG_EVT, "%s: top: eq=0x%p, eq_mod=0x%p(%s), "
	    "eq_size=%d\n", __func__, (void *)eq, (void *)eq->eq_mod,
	    eq->eq_mod->mod_name, eq->eq_size);
	...
	fmd_dprintf(FMD_DBG_EVT, "%s: dequeued: mod_name=%s, eq=0x%p, "
	    "eq_size=%d, ep=0x%p(%s)\n", __func__, eq->eq_mod->mod_name,
	    (void *)eq, eq->eq_size, (void *)ep,
	    fmd_event_type2str(((fmd_event_impl_t *)ep)->ev_type));
...
fmd_eventq_abort(fmd_eventq_t *eq)
{
	...
	while ((eqe = fmd_list_next(&eq->eq_list)) != NULL) {
		fmd_list_delete(&eq->eq_list, eqe);
		fmd_event_rele_no_ctl_wait(eqe->eqe_event);
		fmd_free(eqe, sizeof (fmd_eventqelem_t));
	}

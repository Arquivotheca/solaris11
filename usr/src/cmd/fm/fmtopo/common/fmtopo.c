...
static const char optstr[] = "bCdel:m:P:pR:s:StVx";
static const char *opt_l;
...
usage(FILE *fp)
{
	(void) fprintf(fp,
	    ...
	    "[-R root] [-l file] [-m method] [-s scheme] [fmri]\n", g_pname);
...
static void
print_node(topo_hdl_t *thp, tnode_t *node, const char *fmri)
{
	int err, ret;
	...
	if (opt_S) {
		...
		if (topo_node_fru(node, &fru, NULL, &err) != 0 ||
		    (ret = topo_fmri_presence_state(thp, fru, &err)) < 0)
			(void) printf("\tPresent: -\n");
		else
			(void) printf("\tPresent: %s\n",
			    (ret == FMD_OBJ_STATE_STILL_PRESENT ||
			    ret == FMD_OBJ_STATE_UNKNOWN) ? "true" : "false");

		if (topo_node_asru(node, &asru, NULL, &err) != 0 ||
		    (ret = topo_fmri_service_state(thp, asru, &err)) < 0)
			(void) printf("\tUnusable: -\n");
		else
			(void) printf("\tUnusable: %s\n", ret ==
			    FMD_SERVICE_STATE_UNUSABLE ? "true" : "false");
	}
}
...
set_prop(topo_hdl_t *thp, tnode_t *node, nvlist_t *fmri, struct prop_args *pp)
{
	 ...
	 */
	if (node != NULL) {
		if (topo_prop_getprop(node, pp->group, pp->prop, NULL, &nvl,
		    &err) < 0) {
...
main(int argc, char *argv[])
{
	...
	if (opt_l) {
		if ((uuid = topo_snap_load(thp, opt_l, &err)) == NULL) {
			(void) fprintf(stderr, "%s: failed to load topology "
			    "snapshot from directory %s: %s\n",
			    g_pname, opt_l, topo_strerror(err));
			return (fmtopo_exit(thp, uuid, FMTOPO_EXIT_ERROR));
		}
	} else if ((uuid = topo_snap_hold(thp, NULL, &err)) == NULL) {

...
static struct fmevt_outbound_stats {
	...
	{ "outbound_replays_dropped", FMD_TYPE_UINT64,
	    "replayed list events dropped" }
};
...
fmevt_recv(fmd_hdl_t *hdl, fmd_event_t *ep, nvlist_t *nvl, const char *class)
{
	...
	if (nvlist_lookup_boolean_value(nvl, FM_SUSPECT_REPLAYED,
		...
		fmevt_debug(FMEVT_DBG1, "dropping replayed %s", class);
		return;
	}

	fmevt_debug(FMEVT_DBG1, "forwarding event class %s", class);
	...
	if (sysevent_evc_publish(fmevt_outbound_chan, class, "",
		...
		fmevt_debug(FMEVT_DBG0, "sysevent_evc_publish failed: %s",
		    strerror(errno));
	}
}
...
fmevt_init_outbound(fmd_hdl_t *hdl)
{
	...
	if (fmd_prop_get_int32(hdl, "protocol_forward_disable") == B_TRUE) {
		fmevt_debug(FMEVT_DBG0, "protocol forwarding disabled "
	...
	fmevt_debug(FMEVT_DBG3, "outbound channel is '%s'", channel_name);
	...
	fmevt_debug(FMEVT_DBG3, "outbound channel depth of %d", channel_depth);

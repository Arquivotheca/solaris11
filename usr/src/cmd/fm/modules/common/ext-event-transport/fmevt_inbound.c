    ...
    struct fmevt_ppargs *eap)
{
	...
	if (!fmevt_rs_burst(NULL, rs2burst, &ns, &subsys, B_FALSE)) {
		...
		fmevt_debug(FMEVT_DBG2, "event has bad ruleset");
		return;
	}
	fmevt_debug(FMEVT_DBG3, "ruleset selects namespace %s subsys %s",
	    ns, subsys);
	...
	if (dispf == NULL) {
		if (found) {
			...
			fmevt_debug(FMEVT_DBG3, "ruleset selects drop");
			return;
		} else {
	...
	fmevt_debug(FMEVT_DBG3, "postprocessing fanned out to %d events",
	    expected);

	if (expected > FMEVT_FANOUT_MAX) {
		BUMPSTAT(pp_fanoutmax);
		fmevt_debug(FMEVT_DBG1, "postprocessing fanout too great - %d",
		    expected);
		return;	/* without freeing class and nvl - could leak */
	} else if (expected == 0) {
		BUMPSTAT(pp_intldrop);
		fmevt_debug(FMEVT_DBG2, "postprocessing elects to publish 0 "
		    "events");
		return;
	}
	...
	for (i = 0; i < FMEVT_FANOUT_MAX; i++) {
		...
		if (!class_ok(class[i])) {
			...
			fmevt_debug(FMEVT_DBG1, "bad class %s", class[i]);
			continue;
		}
		 ...
		 */
		if (err == 0) {
			fmevt_debug(FMEVT_DBG2, "posting event number %d "
			    "class %s", i, class[i]);
...
fmevt_cb(sysevent_t *sep, void *arg)
{
	...
	fmevt_debug(FMEVT_DBG1, "received raw event class=%s subclass=%s "
	    "to post-process", rawclass ? rawclass : "<null>",
	    rawsubclass ? rawsubclass : "<null>");

	if (sysevent_get_attr_list(sep, &rawattr) != 0) {
		BUMPSTAT(raw_noattrlist);
		fmevt_debug(FMEVT_DBG1, "raw event lacks attributes");
		goto done;
	}
	...
	if ((dtcr = fmevt_detector(rawattr, ruleset, user, priv,
		...
		fmevt_debug(FMEVT_DBG1, "unable to construct detector fmri");
		goto done;
	}
...
fmevt_init_inbound(fmd_hdl_t *hdl)
{
	...
	/*
	 * Allow post-processing implmentations to configure stats etc.
	 */
	fmevt_pp_smf_init(hdl);
	...
	for (i = 0; i < sizeof (chaninfo) / sizeof (chaninfo[0]); i++) {
		...
		fmevt_debug(FMEVT_DBG3, "binding to %s", channel);
...
fmevt_fini_inbound(fmd_hdl_t *hdl)
{
	...
	for (i = 0; i < sizeof (rulelist) / sizeof (rulelist[0]); i++) {
		...
		if (rsp->rs_namespace) {
			fmd_hdl_strfree(hdl, rsp->rs_namespace);
			rsp->rs_namespace = NULL;
		}
	}

	fmevt_pp_smf_fini(hdl);
}

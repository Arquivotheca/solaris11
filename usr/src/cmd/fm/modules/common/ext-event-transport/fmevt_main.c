...
static int fmevt_debug_level = FMEVT_DBG0;

void
fmevt_debug(int lvl, const char *fmt, ...)
{
	va_list ap;
	...
	if (lvl > fmevt_debug_level)
		return;

	va_start(ap, fmt);
	fmd_hdl_vdebug(fmevt_hdl, fmt, ap);
	va_end(ap);
}
...
_fmd_init(fmd_hdl_t *hdl)
{
	...
	fmevt_debug_level = fmd_prop_get_int32(hdl, "debug_level");

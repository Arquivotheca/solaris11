...
platform_getpath(nvlist_t *nvl)
{
	...
	/*
	 * get the latest topo snapshot to correspond to this ereport.
	 */
	...
	/*
	 * For hc scheme, use the resource from the tolopogy. If it's not
	 * there then we return NULL and we'll get an undiagnosable.
	 */
	if (type == DT_HC) {
		if (topo_fmri_getprop(Eft_topo_hdl, dfmri, TOPO_PGROUP_PROTOCOL,
		    TOPO_PROP_RESOURCE, NULL, &resource, &err) == -1)
			return (NULL);
		if (nvlist_lookup_nvlist(resource, TOPO_PROP_VAL_VAL,
		    &real_fmri) == 0)
	...
	/*
	 * For other schemes look up the path, cpuid, tp or devid in the
	 * topology.
	 */
	lut_free(Usednames, NULL, NULL);
	Usednames = NULL;
	cfg = platform_config_snapshot(B_TRUE);
	switch (type) {
	case DT_DEV:
		if ((ret = config_bydev_lookup(cfg, path)) == NULL)
			out(O_ALTFP, "platform_getpath: no configuration node "
			    "has device path matching \"%s\".", path);
		break;

	case DT_TP:
		if ((ret = config_bytp_lookup(cfg, tp)) == NULL)
			out(O_ALTFP, "platform_getpath: no configuration node "
			    "has tp matching \"%s\".", tp);
		break;

	case DT_DEVID:
		if ((ret = config_bydevid_lookup(cfg, devid)) == NULL)
			out(O_ALTFP, "platform_getpath: no configuration node "
			    "has devid matching \"%s\".", devid);
		break;

	case DT_CPU:
		if ((ret = config_bycpuid_lookup(cfg, cpuid)) == NULL)
			out(O_ALTFP, "platform_getpath: no configuration node "
...
cfgcollect(topo_hdl_t *thp, tnode_t *node, void *arg)
{
	...
	/*
	 * Only include in the raw config those nodes that are used by the rules
	 * remaining after prune_propagations() has been run - ie only those
	 * that could possibly be relevant to the incoming ereport.
	 */
	if (lut_lookup(Usednames, (void *)lastcomp, NULL) == NULL)
		return (TOPO_WALK_NEXT);
...
/*
 * Set up the cpu/dev/devid/tp caches used for looking up ereport paths
 */
/*ARGSUSED*/
static int
cfgcollect_lookups(topo_hdl_t *thp, tnode_t *node, void *arg)
{
	struct cfgdata *rawdata = (struct cfgdata *)arg;
	int err;
	char *propn, *path = NULL;
	...
	newnode = config_lookup(rawdata->cooked, path + 1, 1);
	if (newnode->s == stable(STRAND)) {
		rawdata->cpucache = lut_add(rawdata->cpucache,
		    (void *)newnode->num, (void *)newnode, NULL);
		return (TOPO_WALK_NEXT);
	}
	if (newnode->s == stable(SCSI_DEVICE) ||
	    newnode->s == stable(SMP_DEVICE))
		return (TOPO_WALK_NEXT);
	...
	for (nvp = nvlist_next_nvpair(p_nv, NULL); nvp != NULL;
		...
		for (pg_nvp = nvlist_next_nvpair(pg_nv, NULL); pg_nvp != NULL;
			...
			for (pv_nvp = nvlist_next_nvpair(pv_nv, NULL);
				...
				/*
				 * If this property is a device path, tp or
				 * devid, cache it for quick lookup.
				 */
				if (sn == stable(TOPO_IO_DEV)) {
					(void) nvpair_value_string(pv_nvp,
					    &propv);
					sv = stable(propv);
					out(O_ALTFP|O_VERB3, "caching dev %s",
					    sv);
					rawdata->devcache =
					    lut_add(rawdata->devcache,
					    (void *)sv, (void *)newnode, NULL);
				} else if (sn == stable(TOPO_IO_DEVID) ||
				    sn == stable(TOPO_PROP_SES_DEVID) ||
				    sn == stable(TOPO_PROP_SMP_DEVID)) {
					(void) nvpair_value_string(pv_nvp,
					    &propv);
					sv = stable(propv);
					out(O_ALTFP|O_VERB3, "caching devid %s",
					    sv);
					rawdata->devidcache =
					    lut_add(rawdata->devidcache,
					    (void *)sv, (void *)newnode, NULL);
				} else if (sn ==
				    stable(TOPO_STORAGE_TARGET_PORT_L0IDS)) {
					(void) nvpair_value_string_array(pv_nvp,
					    &propvv, &nelem);
					for (i = 0; i < nelem; i++) {
						sv = stable(propvv[i]);
						out(O_ALTFP|O_VERB3,
...
void
platform_save_config(fmd_hdl_t *hdl, fmd_case_t *fmcase, struct cfgdata *cfg)
{
	size_t cfglen;
	...
	if (cfglen != 0) {
		...
		fmd_buf_write(hdl, fmcase, WOBUF_CFG, cfg->begin, cfglen);
	}
}
 ...
 */
struct cfgdata *
platform_config_snapshot(boolean_t in_getpath)
{
	int err;
	topo_walk_t *twp;
	struct cfgdata *cfg;

	cfg = MALLOC(sizeof (struct cfgdata));
	cfg->begin = cfg->nextfree = cfg->end = NULL;
	cfg->cooked = in_getpath ? newcnode(NULL, 0) : NULL;
	cfg->devcache = NULL;
	cfg->devidcache = NULL;
	cfg->tpcache = NULL;
	cfg->cpucache = NULL;

	if ((twp = topo_walk_init(Eft_topo_hdl, FM_FMRI_SCHEME_HC, in_getpath ?
	    cfgcollect_lookups : cfgcollect, cfg, &err)) == NULL) {
...
platform_path_exists(nvlist_t *fmri)
{
	...
	/*
	 * Get the current topo snapshot using fmd_fmri_topo_hold,
	 * and see if the node is still there.
	 */
	hdl = fmd_fmri_topo_hold(TOPO_VERSION);
	rval = topo_fmri_getprop(hdl, fmri, TOPO_PGROUP_PROTOCOL,
	    TOPO_PROP_RESOURCE, NULL, &resource, &err);
	nvlist_free(resource);
	fmd_fmri_topo_rele(hdl);
	return (rval == 0);
}

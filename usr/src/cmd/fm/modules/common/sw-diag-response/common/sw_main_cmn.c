...
#define	SUBNAME(msinfo, idx) ((*(msinfo)->swms_subinfo)[idx]->swsub_name)
...
static int32_t debug_level = SW_DBG0;

void
sw_debug(fmd_hdl_t *hdl, int lvl, const char *fmt, ...)
{
	va_list ap;
...
sw_recv(fmd_hdl_t *hdl, fmd_event_t *ep, nvlist_t *nvl, const char *class)
{
	...
	sw_debug(hdl, SW_DBG1, "received event class %s", class);
	...
	for (mod = 0; mod < msinfo->swms_dispcnt; mod++) {
		...
		for (dp = (*msinfo->swms_disptbl)[mod];
			...
			if (fmd_nvl_class_match(hdl, nvl, dp->swd_classpat)) {
				dispf = dp->swd_func;
				sw_debug(hdl, SW_DBG1, "dispatching to "
				    "subsidiary %s", SUBNAME(msinfo, mod));
				break;
			}
		}
		if (dispf != NULL) {
    ...
    hrtime_t hrt)
{
	...
	sw_debug(hdl, SW_DBG3, "installed timerid %d in slot %d of %d for %s",
	    timerid, chosen, SW_TIMER_MAX, SUBNAME(msinfo, ID2IDX(who)));
...
sw_timer_remove(fmd_hdl_t *hdl, id_t who, id_t timerid)
{
	...
	sw_debug(hdl, SW_DBG3, "removed timerid %d from slot %d for %s",
	    timerid, slot, SUBNAME(msinfo, ID2IDX(who)));
	(void) pthread_mutex_unlock(&msinfo->swms_timerlock);
}
...
sw_timeout(fmd_hdl_t *hdl, id_t timerid, void *arg)
{
	...
	msinfo->swms_timers[slot].swt_state = SW_TMR_FIRED;
	(void) pthread_mutex_unlock(&msinfo->swms_timerlock);
	...
	sw_debug(hdl, SW_DBG3, "timerid %d from slot %d for %s has fired",
	    timerid, slot, SUBNAME(msinfo, ID2IDX(owner)));
...
static int
sw_prop_count(const fmd_prop_t *propp)
{
	int n = 0;

	while (propp->fmdp_name != NULL) {
		n++;
		propp++;
	}
...
/*
 * Concatenate the shared module properties with those listed for
 * each subsidiary into a NULL-terminated array of all module
 * properties.
 *
 * We cannot use fmd_hdl_alloc since the handle is not yet registered.
 */

static fmd_hdl_info_t sw_hdlinfo;

static const fmd_hdl_info_t *
sw_hdlinfo_alloc(const fmd_hdl_info_t *hdlinfo,
    const struct sw_subinfo *(*subsid)[SW_SUB_MAX])
{
	size_t entsz = sizeof (fmd_prop_t);
	fmd_prop_t *dest;
	int sharedcnt = 0;
	int total = 1;		/* count NULL termination now */
	int i;

	sharedcnt = sw_prop_count(hdlinfo->fmdi_props);
	total += sharedcnt;

	for (i = 0; i < SW_SUB_MAX; i++) {
		const struct sw_subinfo *sip = (*subsid)[i];

		if (sip && sip->swsub_props)
			total += sw_prop_count(sip->swsub_props);
	}

	(void) memcpy(&sw_hdlinfo, hdlinfo, sizeof (*hdlinfo));
	sw_hdlinfo.fmdi_props = calloc(total,  entsz);
	dest = (fmd_prop_t *)sw_hdlinfo.fmdi_props;
	if (dest == NULL)
		return (NULL);

	(void) memcpy((void *)dest, hdlinfo->fmdi_props, sharedcnt * entsz);
	dest += sharedcnt;
	...
	for (i = 0; i < SW_SUB_MAX; i++) {
		...
		if (!sip || sip->swsub_props == NULL)
			continue;

		subcnt = sw_prop_count(sip->swsub_props);

		(void) memcpy((void *)dest, sip->swsub_props, subcnt * entsz);
		dest += subcnt;
	}

	return ((const fmd_hdl_info_t *)&sw_hdlinfo);
}

static void
sw_hdlinfo_free(void)
{
	if (sw_hdlinfo.fmdi_props) {
		free((void *)sw_hdlinfo.fmdi_props);
		sw_hdlinfo.fmdi_props = NULL;
	}
}
...
{
	struct sw_modspecific *msinfo;
	const fmd_hdl_info_t *hdlinfo2;
	int i;

	if ((hdlinfo2 = sw_hdlinfo_alloc(hdlinfo, subsid)) == NULL)
		return (0);

	if (fmd_hdl_register(hdl, FMD_API_VERSION, hdlinfo2) != 0) {
		sw_hdlinfo_free();
		return (0);
	}

	if (fmd_prop_get_int32(hdl, "enable") != B_TRUE) {
		sw_debug(hdl, SW_DBG0, "%s disabled though .conf file setting",
		    hdlinfo->fmdi_desc);
	...
	debug_level = fmd_prop_get_int32(hdl, "debug_level");
	...
	for (i = 0; i < SW_SUB_MAX; i++) {
		...
		case SW_SUB_INIT_SUCCESS:
			...
			sw_debug(hdl, SW_DBG0, "%s initialized", dbgbuf);
			sw_add_callbacks(hdl, sip->swsub_name, dp, nelem,
			    msinfo);
			break;

		case SW_SUB_INIT_FAIL_VOLUNTARY:
			sw_debug(hdl, SW_DBG1, "%s chose not to initialize",
			    dbgbuf);
		...
		case SW_SUB_INIT_FAIL_ERROR:
			sw_debug(hdl, SW_DBG0, "%s failed to initialize "
			    "because of an error", dbgbuf);
			sw_add_callbacks(hdl, sip->swsub_name, NULL, -1,
			    msinfo);
			break;

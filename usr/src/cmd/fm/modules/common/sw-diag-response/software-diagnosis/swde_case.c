...
typedef struct swde_case {
	...
	void *swc_private;		/* for swde_case_{get,set}specific */
...
/*
 * Because we use fmd_case_setspecific to associate a swde_case_t with
 * each case we must provide wrappers for subsidiaries to store and retrieve
 * a per-case private data pointer ala fmd_case_{set,get}specific.
 * Semantics are otherwise unchanged, in particular the data is not
 * checkpointed.
 */
void
swde_case_setspecific(fmd_hdl_t *hdl, fmd_case_t *cp, void *data)
{
	swde_case_t *scp = fmd_case_getspecific(hdl, cp);

	scp->swc_private = data;
}

void *
swde_case_getspecific(fmd_hdl_t *hdl, fmd_case_t *cp)
{
	swde_case_t *scp = fmd_case_getspecific(hdl, cp);

	return (scp->swc_private);
}

...
st_check_component(topo_hdl_t *thp, tnode_t *node, void *arg)
{
	...
	nvlist_t *nvl, *props, *rsrc, *fru, *args = NULL;
	char *fmri;
	int err, ret;
	 ...
	 */
	if ((ret = topo_fmri_presence_state(thp, rsrc, &err)) < 0) {
		fmd_hdl_debug(hdl,
		    "topo_fmri_presence_state() failed for %s=%d",
	...
	/*
	 * See if the simulate mode is set and if so build the args nvlist
	 */
	if (stp->st_sim_fmri != NULL) {
		args = fmd_nvl_alloc(stp->st_hdl, FMD_SLEEP);
		(void) nvlist_add_string(args, "sim_fmri", stp->st_sim_fmri);
		(void) nvlist_add_uint8(args, "sim_state", stp->st_sim_state);
	}
	if (topo_method_invoke(node, TOPO_METH_SENSOR_FAILURE,
	    TOPO_METH_SENSOR_FAILURE_VERSION, args, &nvl, &err) != 0) {
...
static const fmd_hdl_info_t fmd_info = {
	"Sensor Transport Agent", "1.2", &fmd_ops, fmd_props
};
...
_fmd_init(fmd_hdl_t *hdl)
{
	...
	/*
	 * The simulate property can be used to spoof the sensor state for
	 * a given sensor in order to exercise the ereport generation code
	 * paths.  The syntax is as follows:
	 *
	 * setprop simulate "FMRI STATE"
	 *
	 * where FMRI is the FMRI of a sensor facility node
	 * where STATE is the discrete state value we want to spoof.
	 */
	...
	if (simulate[0] != '\0') {
		...
		if (*sep != '\0') {
			...
			stp->st_sim_fmri = fmd_hdl_alloc(hdl,
			    len + 1, FMD_SLEEP);
			(void) memcpy(stp->st_sim_fmri, simulate, len);
			stp->st_sim_fmri[len] = '\0';
		}
		...
		if (*sep != '\0') {
			stp->st_sim_state = atoi(sep);
		} else if (stp->st_sim_fmri) {
			fmd_hdl_strfree(hdl, stp->st_sim_fmri);
			stp->st_sim_fmri = NULL;
		}
	}

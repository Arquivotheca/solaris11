...
#define	CMD_SUNOEM_CORE_TUNNEL		0x44 /* ILOM IPMI core tunnel command */
#define	CORE_TUNNEL_SUBCMD_FMFPCAP	4    /* subcmd to check fp feature */
#define	CORE_TUNNEL_SUBCMD_FMIPCAP	5    /* subcmd to check ip feature */
#define	OEM_DATA_LENGTH			0    /* IPMI cmd payload length */
#define	ILOMCONFIG 			"/usr/lib/ilomconfig.builtin"
#define	ILOMCONIFIG_LIST_INTERCONNECT 	ILOMCONFIG " list interconnect"
...
static int ip_enable;		/* enable the ip transport */
 ...
 * SP/Host interface
 */
static int ip_sp_enable;		/* Enable the sp/host link */
static int ip_sp_capable;		/* SP is ip-transport capable */
static char *ip_sp_server_addr = NULL;	/* SP address */
static char *ip_sp_client_addr = NULL;	/* Host addr */
static char *ip_sp_server_port = "24";	/* SP port */
static char *ip_sp_client_port = "12";	/* Host port */
static hrtime_t ip_sp_sleep;		/* sleep delay for getting addrs */
static hrtime_t ip_sp_sleep_max;	/* max sleep delay */
...
ip_fmdo_send(fmd_hdl_t *hdl, fmd_xprt_t *xp, fmd_event_t *ep, nvlist_t *nvl)
{
	...
	/*
	 * At the connection time, FMD sends a several subscribe events to the
	 * SP. The ILOM fetd is supposed to drop these events. However,
	 * fetd is abnormally terminated. As a workaround to avoid this
	 * problem, FMD does not send these events. This workaround should be
	 * removed when the ILOM fetd is fixed.
	 */
	if (ip_sp_enable && ip_sp_server_addr && ipx && ipx->ipx_cinfo &&
	    (strcmp(ipx->ipx_cinfo->ipc_name, ip_sp_server_addr) == 0) &&
	    (strcmp(ipx->ipx_cinfo->ipc_port, ip_sp_server_port) == 0)) {
		if (nvlist_lookup_string(nvl, FM_CLASS, &class) == 0 &&
		    strcmp(class, "resource.fm.xprt.subscribe") == 0 &&
		    nvlist_lookup_string(nvl, FM_RSRC_XPRT_SUBCLASS,
		    &subclass) == 0) {
			ip_debug(IP_DEBUG_FINER,
			    "Drop %s event, subclass=%s\n", class, subclass);
 ...
 */
int
ip_sp_bind_client(int fd, char *client_addr, char *client_port)
{
	int optval = 1;
	int err;
	struct linger ling;
	...
	ip_debug(IP_DEBUG_FINEST, "Bind client %s, port %s\n", client_addr,
	    client_port);
	err = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &optval,
	    sizeof (optval));
	if (err != 0) {
		ip_debug(IP_DEBUG_FINE, "setsockopt failed err=%d\n", err);
		return (err);
	}
	 ...
	 * The transport will be unable to connect to the SP until this clears.
	 */
	ling.l_onoff = 1;
	ling.l_linger = 0;
	...
	err = getaddrinfo(client_addr, client_port, &aih, &ipc_addr);
	if (err != 0) {
		ip_debug(IP_DEBUG_FINE, "getaddrinfo failed, err=%d\n", err);
		return (err);
	}

	for (aip = ipc_addr; aip != NULL; aip = aip->ai_next) {
		if (aip->ai_family != AF_INET && aip->ai_family != AF_INET6)
			continue; /* ignore anything that isn't IPv4 or IPv6 */
		err = bind(fd, aip->ai_addr, aip->ai_addrlen);
		if (err) {
			ip_debug(IP_DEBUG_FINE, "bind() failed, err=%d\n", err);
			return (err);
		}
	}
...
ip_xprt_setup(fmd_hdl_t *hdl, ip_cinfo_t *cinfo)
{
	...
	for (aip = ail; aip != NULL; aip = aip->ai_next) {
		...
		if (xflags & FMD_XPRT_ACCEPT) {
			...
			if (ip_sp_enable && ip_sp_server_addr &&
			    (strcmp(cinfo->ipc_name, ip_sp_server_addr) == 0) &&
			    (strcmp(cinfo->ipc_port, ip_sp_server_port) == 0)) {
				(void) ip_sp_bind_client(fd, ip_sp_client_addr,
				    ip_sp_client_port);
			}
...
ip_addr_cleanup()
{
	...
	ip_port = NULL;
	if (ip_sp_server_addr) {
		fmd_hdl_strfree(ip_hdl, ip_sp_server_addr);
	}
	if (ip_sp_client_addr) {
		fmd_hdl_strfree(ip_hdl, ip_sp_client_addr);
	}
}
...
static ip_cinfo_t *
ip_create_cinfo(char *server, char *port, boolean_t accept)
{
	int err;
	struct addrinfo aih;
	...
	cinfo->ipc_port = port;
	err = getaddrinfo(server, port, &aih, &cinfo->ipc_addr);
	if (err != 0) {
		fmd_hdl_error(ip_hdl, "failed to resolve host %s port %s: %s\n",
		    cinfo->ipc_name, port, gai_strerror(err));
		cinfo->ipc_addr = NULL;
		cinfo->ipc_port = NULL;
 ...
 */
static int
ip_setup_addr(char *server, char *port, boolean_t accept, boolean_t start)
{
	int err = 0;
	ip_cinfo_t *cinfo = ip_create_cinfo(server, port, accept);
	...
	if (cinfo == NULL) {
		...
		ip_conns = cinfo;
		if (start && (ip_xprt_setup(ip_hdl, cinfo) != 0)) {
			ip_debug(IP_DEBUG_FINE, "ip_xprt_setup() failed\n");
			cinfo->ipc_timer = fmd_timer_install(ip_hdl, cinfo,
			    NULL, ip_sleep);
		}
		(void) pthread_mutex_unlock(&ip_conns_lock);
	}
...
ip_setup_addrs(char *server, boolean_t accept)
{
	...
	for (p = server; *p != '\0'; p++) {
		if (*p == ',') {
			*p = '\0';
			err = ip_setup_addr(addr, ip_port, accept, start);
			*p = ',';
			if (err)
				return (err);
			addr = ++p;
...
get_ipmi_handler()
{
	...
	if ((ipmi_hdl = ipmi_open(&error, &msg, IPMI_TRANSPORT_BMC,
	    NULL)) == NULL) {
		/*
		...
		if (error != EIPMI_BMC_OPEN_FAILED) {
			fmd_hdl_error(ip_hdl, "Failed to initialize IPMI "
			    "connection: %s\n", msg);
		} else {
			fmd_hdl_error(ip_hdl, "Failed to open BMC driver\n");
		}
	...
	if (check_sunoem(ipmi_hdl) != 0) {
		fmd_hdl_error(ip_hdl, "Service Processor does not run "
		    "Sun ILOM\n");
		ipmi_close(ipmi_hdl);
		return (NULL);
	}
...
static void
free_ipmi_handler(ipmi_handle_t *ipmi_hdl)
{
	if (ipmi_hdl) {
		ipmi_close(ipmi_hdl);
	}
...
static int
check_ilom_feature(ipmi_handle_t *ipmi_hdl, uint8_t subcmd)
{
	ipmi_cmd_t cmd, *resp;
	uint8_t oem_data[2], f_enabled;

	oem_data[0] = subcmd;
	cmd.ic_netfn = IPMI_NETFN_OEM;
	cmd.ic_lun = 0;
	cmd.ic_cmd = CMD_SUNOEM_CORE_TUNNEL;
	cmd.ic_dlen =  1 + OEM_DATA_LENGTH;
	cmd.ic_data = &oem_data;

	/*
	 * Failed to send ILOM feature inquiry command to SP
	 */
	if ((resp = ipmi_send(ipmi_hdl, &cmd)) == NULL) {
		return (-1);
	}
	...
	/*
	 * ic_data[1] = 1: the feature inquiried is not supported on ILOM.
	 * ic_data[1] = 0: the feature inquiried is supported on ILOM.
	 */
	f_enabled = ((uint8_t *)resp->ic_data)[0];
	if (f_enabled == 1 || f_enabled == 0) {
		return ((int)f_enabled);
	} else {
		return (-2);
	}
}
...
/*
 * Check if the FMA ip-transport feature is enabled on the SP.
 */
static boolean_t
ip_sp_transport_enable(void)
{
	ipmi_handle_t *ipmi_hdl;
	int support, i;

	ipmi_hdl = get_ipmi_handler();
	if (ipmi_hdl == NULL) {
		return (B_FALSE);
	}

	for (i = 0; i < 4; i++) {
		support = check_ilom_feature(ipmi_hdl,
		    CORE_TUNNEL_SUBCMD_FMIPCAP);
		if (support >= 0)
			break;
	}
	free_ipmi_handler(ipmi_hdl);
	if (support == 1) {
		ip_debug(IP_DEBUG_FINE, "IP transport feature is "
		    "enabled in ILOM\n");
		return (B_TRUE);
	} else {
		ip_debug(IP_DEBUG_FINE, "IP transport feature is not"
		    "enabled in ILOM\n");
		return (B_FALSE);
	}
}
...
/*
 * Use the ilomconfig to check if the LanOverUSB interface is enabled and
 * then get the SP addr and the host addr
 */
static void
ip_sp_get_ilom_addrs(char **saddr, char **haddr)
{
	FILE *fp = NULL;
	char buf[80];
	char *val, *end;
	...
	const char *saddr_prop = "SP Interconnect IP Address: ";
	const char *haddr_prop = "Host Interconnect IP Address: ";

	/*
	 * Parse the ilomconfig output for the connection state, sp address and
	 * the host address.
	 */
	fp = popen(ILOMCONIFIG_LIST_INTERCONNECT, "r");
	if (fp == NULL) {
		ip_debug(IP_DEBUG_FINE, "ilomconfig failed\n");
		return;
	}
	*saddr = NULL;
	*haddr = NULL;
	while ((fgets(buf, sizeof (buf), fp) != NULL) &&
	    (state == NULL || *saddr == NULL || *haddr == NULL)) {
		if (val = strstr(buf, state_prop)) {
			prop = state_prop;
			prop_val = &state;
		} else if (val = strstr(buf, saddr_prop)) {
			prop = saddr_prop;
			prop_val = saddr;
		} else if (val = strstr(buf, haddr_prop)) {
			prop = haddr_prop;
			prop_val = haddr;
		}
		if (val != NULL) {
			/* Go past property string */
			val += strlen(prop);
			end = strchr(val, '\n');
			if (end != NULL) {
				*end = '\0';
				if (*prop_val) {
					fmd_hdl_strfree(ip_hdl, *prop_val);
				}
				*prop_val = fmd_hdl_strdup(ip_hdl, val,
				    FMD_SLEEP);
				ip_debug(IP_DEBUG_FINER,
				    "LanOverUSB: %s %s", prop, *prop_val);
			}
		}
	} /* while */
	(void) pclose(fp);
	...
	if (state == NULL || strcmp(state, "enabled") != 0) {
		ip_debug(IP_DEBUG_FINER, "LanOverUSB is not enabled(state=%s)",
		    state != NULL ? state : "");
		fmd_hdl_strfree(ip_hdl, *saddr);
		fmd_hdl_strfree(ip_hdl, *haddr);
		*saddr = NULL;
		*haddr = NULL;
	}
	if (*saddr == NULL) {
		ip_debug(IP_DEBUG_FINER, "LanOverUSB SP address is null");
		fmd_hdl_strfree(ip_hdl, *haddr);
		*haddr = NULL;
	}
	if (*haddr == NULL) {
		ip_debug(IP_DEBUG_FINER, "LanOverUSB host address is null");
		fmd_hdl_strfree(ip_hdl, *saddr);
		*saddr = NULL;
	}
	fmd_hdl_strfree(ip_hdl, state);
}

/*
 * If the SP has the ip-transport feature enabled, get the SP addr and the
 * host addresses
 */
static void
ip_sp_get_addrs(void)
{
	int err;

	ip_sp_get_ilom_addrs(&ip_sp_server_addr, &ip_sp_client_addr);
	if (ip_sp_server_addr == NULL || ip_sp_client_addr == NULL) {
		fmd_hdl_strfree(ip_hdl, ip_sp_server_addr);
		fmd_hdl_strfree(ip_hdl, ip_sp_client_addr);
		ip_sp_server_addr = NULL;
		ip_sp_client_addr = NULL;
		ip_debug(IP_DEBUG_FINE,
		    "Failed to get the sp address (will retry)\n");

		/* Increase the sleep time */
		if (ip_sp_sleep < ip_sp_sleep_max) {
			ip_sp_sleep = MIN(2 * ip_sp_sleep, ip_sp_sleep_max);
		}

		(void) fmd_timer_install(ip_hdl, NULL, NULL, ip_sp_sleep);
		return;
	}

	/* Upon successfully getting the addresses, reset the sleep time */
	ip_sp_sleep = MIN(ip_sleep, ip_sp_sleep_max);

	err = ip_setup_addr(ip_sp_server_addr, ip_sp_server_port, B_FALSE,
	    B_TRUE);
	if (err) {
		ip_debug(IP_DEBUG_FINE,
		    "Unable to setup sp address %s", ip_sp_server_addr);
		return;
	}
}
 ...
 * (d) arg is NULL, check for a new SP IP address and set up new sp connection
 *
...
ip_timeout(fmd_hdl_t *hdl, id_t id, void *arg) {
	...
	if (arg == NULL) {
		if (ip_sp_enable && ip_sp_capable) {
			ip_sp_get_addrs();
		} else {
			fmd_hdl_error(hdl,
			    "ip_timeout failed because hg arg is NULL");
		}
...
static const fmd_prop_t fmd_props[] = {
	...
	{ "ip_no_remote_repair", FMD_TYPE_BOOL, "false" },
	{ "ip_hconly", FMD_TYPE_BOOL, "true" },
	...
	{ "ip_sp_enable", FMD_TYPE_BOOL, "true" },
	{ "ip_sp_sleep_max", FMD_TYPE_TIME, "10m" },
	{ NULL, 0, NULL }
};
...
{
	char *addr, *auth, *p, *q, *r, *s, isa[MAXNAMELEN];
	int err;

	if (fmd_hdl_register(hdl, FMD_API_VERSION, &fmd_info) != 0)
		return; /* failed to register handle */

	ip_enable = fmd_prop_get_int32(hdl, "ip_enable");
	ip_sp_enable = fmd_prop_get_int32(hdl, "ip_sp_enable");

	/*
	 * Disable the SP link in the non-global zone.
	 * Only enable the SP link on the x86 platforms.
	 * Disable the SP link if the ilomconfig cli is not present because
	 * the LanOverUSB network is not automatically configured.
	 */
	if (getzoneid() != GLOBAL_ZONEID) {
		ip_sp_enable = FMD_B_FALSE;
	} else if ((sysinfo(SI_ARCHITECTURE, isa, sizeof (isa)) == -1) ||
	    (strncmp(isa, "i386", 4) != 0)) {
		ip_sp_enable = FMD_B_FALSE;
	} else if (access(ILOMCONFIG, F_OK|X_OK) != 0) {
		ip_sp_enable = FMD_B_FALSE;
	}

	if (ip_enable == FMD_B_FALSE && ip_sp_enable == FMD_B_FALSE) {
	...
	ip_sp_sleep_max = fmd_prop_get_int64(hdl, "ip_sp_sleep_max");
	...
	if (ip_enable) {
		...
		if (addr != NULL) {
			...
			if (err) {
				fmd_hdl_debug(hdl,
				    "Unable to setup ip_bind_addr %s", addr);

				fmd_prop_free_string(hdl, addr);
				fmd_prop_free_string(hdl, ip_domain_name);
				fmd_hdl_unregister(hdl);
				return;
			}
			fmd_prop_free_string(hdl, addr);
		...
		if (addr != NULL) {
			...
			if (err) {
				fmd_hdl_debug(hdl,
				    "Unable to setup ip_server %s", addr);
				fmd_prop_free_string(hdl, addr);
				fmd_prop_free_string(hdl, ip_domain_name);
				fmd_hdl_unregister(hdl);
				return;
	 ...
	 */
	if (ip_sp_enable) {
		ip_sp_capable = ip_sp_transport_enable();
		if (ip_sp_capable) {
			ip_sp_get_ilom_addrs(&ip_sp_server_addr,
			    &ip_sp_client_addr);
			if (ip_sp_server_addr) {
				/* Set up the conn but do not start it */
				err = ip_setup_addr(ip_sp_server_addr,
				    ip_sp_server_port, B_FALSE, B_FALSE);
				if (err) {
					fmd_prop_free_string(hdl,
					    ip_domain_name);
					fmd_hdl_abort(hdl,
					    "Unable to setup sp address %s",
					    ip_sp_server_addr);
					return;
				}
			}
	...
	/*
	 * When ip_enable is set regardingless whether the ip_sp_enable
	 * property is set or not, if no specific connections configured then
	 * set up general server listening on all network ports.
	 *
	 * When the ip_sp_enable is set without the ip_enable parameter,
	 * the server listening to all networks ports is not setup. If the SP
	 * is ip-transport capable, setup a timer to retry to get the SP
	 * address and then setup the sp/host connection. Otherwise, unload
	 * this module.
	 */
	if (ip_conns == NULL) {
		if (ip_sp_enable && ip_enable == FMD_B_FALSE) {
			if (ip_sp_capable) {
				/*
				 * Fail to get the sp address or setup the
				 * sp/host connection. Retry it later.
				 */
				ip_sp_sleep = MIN(ip_sleep, ip_sp_sleep_max);
				(void) fmd_timer_install(ip_hdl, NULL, NULL,
				    ip_sp_sleep);
			} else {
				ip_debug(IP_DEBUG_FINE,
				    "LanOverUSB is not enabled.");
				ip_addr_cleanup();
				fmd_prop_free_string(hdl, ip_domain_name);
				fmd_hdl_unregister(hdl);
				return;
			}
		} else if (ip_setup_addr(NULL, ip_port, B_TRUE, B_FALSE) != 0) {
			fmd_prop_free_string(hdl, ip_domain_name);

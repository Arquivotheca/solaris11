...
ls_color_init()
{
	...
	/*
	 * We must strdup the color string because it is static
	 * and will be put in read-only memory by the linker
	 * which will cause a SEGV when used as a parameter for
	 * strtok_r below.
	 */
	if ((p = getenv("LS_COLORS")) == NULL)
		colorstr = strdup(default_colorstr);
	else
		colorstr = strdup(p);

	if (colorstr == NULL)
		/* memory error, just return */
		return;

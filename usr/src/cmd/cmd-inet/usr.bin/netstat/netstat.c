...
extern void	*sdp_pr_stats(kstat_ctl_t *kc, boolean_t conn_stats,
		    void *sdp_handle, boolean_t Aflag);



/* Buffer size in kmem_cache_stats() function used for message formatting. */
#define	KCS_BS  256
...
char		*pr_ap(uint_t addr, uint_t port,
			    char *proto, char *dst, uint_t dstlen);
char		*pr_ap6(const in6_addr_t *addr, uint_t port,
			    char *proto, char *dst, uint_t dstlen);
...
boolean_t		family_selected(int family);
...
main(int argc, char **argv)
{
	...
	for (;;) {
		 ...
		 * print SDP stats
		 */
		if (((family_selected(AF_INET_SDP) ||
		    family_selected(AF_INET) || family_selected(AF_INET6)) &&
		    (protocol_selected(PROTO_SDP) ||
		    protocol_selected(IPPROTO_MAX))) &&
		    (!(Dflag || Iflag || Rflag || Mflag ||
		    MMflag || Pflag || Gflag))) {
			sdp_handle = sdp_pr_stats(kc, Sflag, sdp_handle, Aflag);
		}
		/* netstat: AF_UNIX behaviour */
		/*
		 * print AF_UNIX stats if AF_UNIX is specified OR
		 * if no family and no protocol is specified.
		 */
...
#define	ipv6IfStatsEntry_Next(ptr)	\
	(mib2_ipv6IfStatsEntry_t *)((char *)ptr+ipv6IfStatsEntrySize)

/*
 * Computes diffptr->valp[N] = tempp2->valp[N] - tempp1->valp[N] for each N.
 * N is determined as the length value divided by the array element size.
 *
 * The valp is an array of mib2_ipv6IfStatsEntry_t instances.
 * The function computes the difference between the corresponding
 * individual instances.
 */
static int
mib_item_diff_ipv6(mib_item_t *diffptr, mib_item_t *tempp2, mib_item_t *tempp1)
{
	mib2_ipv6IfStatsEntry_t *i2;
	mib2_ipv6IfStatsEntry_t *i1;
	mib2_ipv6IfStatsEntry_t *d_base;
	...
	if (tempp1->length != tempp2->length) {
		(void) fprintf(stderr, "Network configuration changed\n");
		exit(EXIT_FAILURE);
	}
	...
	d_base = (mib2_ipv6IfStatsEntry_t *)calloc(tempp2->length, 1);
	if (d_base == NULL)
		return (0);
	...
	for (
	    d = d_base;
	    ((char *)d + ipv6IfStatsEntrySize) <=
	    (char *)d_base + tempp2->length;
	    /* LINTED: (note 1) */
	    d = ipv6IfStatsEntry_Next(d),
	    /* LINTED: (note 1) */
	    i2 = ipv6IfStatsEntry_Next(i2),
	    /* LINTED: (note 1) */
	    i1 = ipv6IfStatsEntry_Next(i1)) {

		if (i1->ipv6IfIndex != i2->ipv6IfIndex) {
			(void) fprintf(stderr,
			    "Network configuration changed\n");
			exit(EXIT_FAILURE);
		}

		d->ipv6IfIndex = i2->ipv6IfIndex;

		d->ipv6Forwarding = i2->ipv6Forwarding;
		d->ipv6DefaultHopLimit = i2->ipv6DefaultHopLimit;
...
#define	ipv6IfIcmpEntry_Next(ptr)	\
	(mib2_ipv6IfIcmpEntry_t *)((char *)ptr+ipv6IfIcmpEntrySize)

/*
 * Computes diffptr->valp[N] = tempp2->valp[N] - tempp1->valp[N] for each N.
 * N is determined as the length value divided by the array element size.
 *
 * The valp is an array of mib2_ipv6IfIcmpEntry_t instances.
 * The function computes the difference between the corresponding
 * individual instances.
 */
static int
mib_item_diff_icmp6(
	mib_item_t *diffptr, mib_item_t *tempp2, mib_item_t *tempp1)
{
	mib2_ipv6IfIcmpEntry_t *i2;
	mib2_ipv6IfIcmpEntry_t *i1;
	mib2_ipv6IfIcmpEntry_t *d_base;
	...
	d_base = (mib2_ipv6IfIcmpEntry_t *)calloc(tempp2->length, 1);
	if (d_base == NULL)
		return (0);
	...
	for (
	    d = d_base;
	    ((char *)d+ipv6IfIcmpEntrySize) <= (char *)d_base+tempp2->length;
	    /* LINTED: (note 1) */
	    d = ipv6IfIcmpEntry_Next(d),
	    /* LINTED: (note 1) */
	    i2 = ipv6IfIcmpEntry_Next(i2),
	    /* LINTED: (note 1) */
	    i1 = ipv6IfIcmpEntry_Next(i1)) {

		if (i1->ipv6IfIcmpIfIndex != i2->ipv6IfIcmpIfIndex) {
			(void) fprintf(stderr,
			    "Network configuration changed\n");
			exit(EXIT_FAILURE);
		}

		d->ipv6IfIcmpIfIndex = i2->ipv6IfIcmpIfIndex;
...
mib_item_diff(mib_item_t *item1, mib_item_t *item2) {
	...
	for (tempp2 = item2; tempp2 != NULL; tempp2 = tempp2->next_item) {
		...
		for (tempp1 = item1; tempp1 != NULL;
			...
			case MIB2_IP6: {
				if (!mib_item_diff_ipv6(
				    diffptr, tempp2, tempp1))
					goto mibdiff_out_of_memory;
				prevp = diffptr++;
				break;
			}
	...
	for (; item; item = item->next_item) {
			...
			continue; /* 'for' loop 1 */
		{
			...
			if (once_only) {
				...
				mib2_ipAddrEntry_t *ap;
				struct ifstat	t = {0L, 0L, 0L, 0L, 0L};
...
kmem_cache_stats(char *title, char *name, int prefix, uint64_t *total_bytes)
{
	...
	/* buf_total - buf_avail */
	uint64_t buf_inuse = 0;

	/* Statistics summation, total_<var> += <var>. */
	uint64_t total_alloc = 0;
	uint64_t total_alloc_fail = 0;
	uint64_t total_buf_max = 0;
	uint64_t total_buf_inuse = 0;

	/* The sum of "buf_constructed" values of all streams_dblk* caches. */
	uint64_t dblk_constructed = 0;

	/* Buffer to construct verbose user output. */
	char buf[KCS_BS];
	...
	for (ksp = kc->kc_chain; ksp != NULL; ksp = ksp->ks_next) {
		 ...
		 */
		if (strncmp(name, "streams_mblk",
		    sizeof ("streams_mblk")) == 0 &&
		    strncmp(ksp->ks_name, "streams_dblk",
		    sizeof ("streams_dblk") - 1) == 0) {
		...
		if (strncmp(ksp->ks_name, name, name_len) != 0)
			continue;
		...
		if (buf_total < buf_avail) {
			(void) fprintf(stderr,
			    "%s%s: warning, inconsistent statistics: "
			    "buf_total (%llu) < buf_avail (%llu)\n",
			    title, ksp->ks_name + name_len,
			    buf_total, buf_avail);
		}
		...
		if (Vflag && prefix) {
			(void) snprintf(buf, KCS_BS, "%s%s", title,
			    ksp->ks_name + name_len);
	...
	/*
	 * If statistics are requested for streams_mblk cache, then the
	 * number of free and constructed streams_dblk* cache objects
	 * is substracted from the currently computed number of in-use
	 * streams_mblk cache objects. The reason for this is, as the "Hack
	 * alert" message above states, that each constructed streams_dblk*
	 * object contains pointer to one streams_mblk object. Note that there
	 * is one streams_mblk cache and multiple streams_dblk* caches
	 * (name of each is prefixed with streams_dblk string).
	 */
	if (strncmp(name, "streams_mblk", sizeof ("streams_mblk")) == 0) {
		if (total_buf_inuse < dblk_constructed) {
			(void) fprintf(stderr,
			    "%s: warning, inconsistent statistics: "
			    "total_buf_inuse (%llu) < "
			    "dblk_constructed (%llu)\n",
			    title, total_buf_inuse, dblk_constructed);
		} else {
			total_buf_inuse -= dblk_constructed;
		}
	}
 ...
 *
 * Values "addr" and "mask" are in network-byte-order.
 */
static char *
pr_net(uint_t addr, uint_t mask, char *dst, uint_t dstlen)
{
	...
	if (cp != NULL) {
		...
		(void) inet_ntop(AF_INET, (char *)&nbo_addr, dst, dstlen);
	}
	if (hp != NULL)
		freehostent(hp);
	return (dst);
...
process_filter(char *arg)
{
	...
	case FK_AF:
		...
		} else if (strcmp(arg, "sdp") == 0) {
			newf->u.f_family = AF_INET_SDP;
		} else {

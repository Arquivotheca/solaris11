	...
	if (host) {
			...
			if (do_auth() && autoencrypt) {
				...
				if (command("PROT P") == COMPLETE)
					...
					(void) fprintf(stderr,
					    ...
					    argv[0]);
					/*
					 * Unable to encrypt command channel,
					 * too!
					 */
		...
		if (strncmp(reply_string, "215 UNIX Type: L8", 17) == 0) {
			...
			if (overbose &&
				...
				(void) printf(
				    "Remember to set tenex mode when "
				    "transferring "
...
shell(int argc, char *argv[])
{
	...
	if ((pid = fork()) == 0) {
		...
		if (argc > 1) {
			...
			(void) execl(shellstring, namep, (char *)0);
		}
		perror(shellstring);
		code = -1;
		exit(1);
...
macdef(int argc, char *argv[])
{
	...
	(void) strncpy(macros[macnum].mac_name, argv[1], MACRO_NAME_SIZE);
	if (macnum == 0) {

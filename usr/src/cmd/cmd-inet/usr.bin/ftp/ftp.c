...
getreply(int expecteof)
{
	...
	for (;;) {
		...
		while ((c = ibuf[0] ?
			...
			/*
			 * Don't let i to go out of array bounds.
			 * The -2 is because of the one i++ in the
			 * while condition above and one i++ later
			 * in the body below.
			 */
			if (i >= (FTPBUFSIZ-2))
				break;
			...
			if (pflag == 2) {
				if (c != '\r' && c != ')') {
					/*
					 * the mb array is to deal with the
					 * wchar_t
					 */
					char mb[MB_LEN_MAX];
					int avail;
					...
					/*
					 * space available in pasv[],
					 * accounting for trailing NULL
					 */
...
/*
 * Return 1, if fout is a socket descriptor.
 * Return 0, if fout is not a socket descriptor.
 ...
 * Call fsync() on fd.
 *
 * If fd is used by the caller as a stdout and it is a socket descriptor,
 * then fsync() is not called on fd.
 *
 * Returns -1 on error.
 * Returns 0 on success.
 */
static int
fout_fsync(int fd, int is_stdout)
{
	int err;
...
recvrequest(char *cmd, char *local, char *remote, char *mode, int allowpipe)
{
	...
	case TYPE_L:
		...
		if ((d < 0) || ((c == 0) &&
		    (fout_fsync(fileno(fout), fout_is_stdout) == -1))) {
writeerr:
			errflg = 1;
			perror(local);
		}
	...
	*again = 0;
	...
	if (auth_type == AUTHTYPE_GSSAPI) {
		...
		if (maj_stat != GSS_S_COMPLETE) {
			...
			if (GSS_SUPPLEMENTARY_INFO(maj_stat))
				(void) gss_release_buffer(&min_stat, &msg_buf);
			return ('5');
		}
		if (msg_buf.length < ilen - 2 - 1) {
			(void) memcpy(
			    plain_buf, msg_buf.value, msg_buf.length);
			(void) strcpy(
			    (char *)&plain_buf[msg_buf.length], "\r\n");
			(void) gss_release_buffer(&min_stat, &msg_buf);
			*again = 1;
		} else {
			user_gss_error(maj_stat, min_stat,

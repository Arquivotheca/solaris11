...
next:
	...
	case DEFAULT:
		...
		case MACDEF:
			...
			if (macnum == 16) {
				(void) printf("Limit of 16 macros have already "
				    "been defined\n");
				return (-1);
			}
			...
			*tmp++ = c;
			...
			if (c == EOF) {
				(void) printf("Macro definition for `%s` "
				    "missing null line terminator.\n",
			...
			*tmp = '\0';
			...
			while (tmp != macbuf + 4096) {
				if ((c = getc(cfile)) == EOF) {
					(void) printf("Macro definition for "
					    "`%s` missing "
			...
			if (tmp == macbuf + 4096) {
				(void) printf("4K macro buffer exceeded\n");
				return (-1);
			}
			if (*macros[macnum - 1].mac_start == '\n') {
				(void) printf(
				    "Macro definition for `%s` is empty, "
				    "macro not stored.\n",
				    macros[--macnum].mac_name);
			}
			break;

...
secure_determine_constants(void)
{
	...
	if (auth_type == AUTHTYPE_GSSAPI) {
		...
		maj_stat = gss_wrap_size_limit(&min_stat, gcontext,
		    (dlevel == PROT_P), GSS_C_QOP_DEFAULT, msize, &mlen);
...
static int secure_putbuf(int fd, uchar_t *buf, uint_t nbyte)
{
	...
	if (auth_type == AUTHTYPE_GSSAPI) {
		...
		(void) memcpy(outbuf, out_buf.value, length = out_buf.length);
		...
		if (auth_type == AUTHTYPE_GSSAPI) {
			...
			if (maj_stat != GSS_S_COMPLETE) {
				...
				if (GSS_SUPPLEMENTARY_INFO(maj_stat))
					(void) gss_release_buffer(&min_stat,
					    &msg_buf);
				return (ERR);
			}

			(void) memcpy(ucbuf, msg_buf.value,

	...
	if (dsmp == NULL) {
		 ...
		 */
			...
			if (ia.ia_cmd == DHCP_START &&
			    ia.ia_request->data_type == DHCP_TYPE_REQHOST)
				reqhost = (char *)ia.ia_request->buffer;
			lif = attach_lif(ifname, reqhost, isv6, &error);
			if (lif != NULL &&
...
load_option:
	...
	case DHCP_STATUS: {
		...
		status.if_cidlen = dsmp->dsm_cidlen;
		if (dsmp->dsm_cidlen > 0) {
			(void) memcpy(status.if_cid, dsmp->dsm_cid,
			    sizeof (status.if_cid));
		}
		...
		if (dsmp->dsm_reqhost != NULL) {
			(void) strlcpy(status.if_reqhost, dsmp->dsm_reqhost,
			    sizeof (status.if_reqhost));
		} else {
			status.if_reqhost[0] = '\0';
		}

...
insert_pif(const char *pname, boolean_t isv6, int *error)
{
	...
	if (!isv6 && pif->pif_hwtype == 0) {
		...
		if ((rc = dlpi_info(dh, &dlinfo, DLPI_INFO_VERSION)) !=
		    DLPI_SUCCESS) {
 ...
 *	    const char *: the ddns requested hostname
 *	    int *: ignored on input; if insert_pif fails, set to a DHCP_IPC_E_*
 *		   error code with the reason why
...
dhcp_lif_t *
insert_lif(dhcp_pif_t *pif, const char *lname, const char *reqhost, int *error)
{
	dhcp_lif_t *lif;
	int fd;
	struct lifreq lifr;
	...
	if (reqhost != NULL) {
		if (strlcpy(lif->lif_reqhost, reqhost,
		    sizeof (lif->lif_reqhost)) >= sizeof (lif->lif_reqhost)) {
			dhcpmsg(MSG_ERROR, "insert_lif: requested host name "
			    "%s is too long", reqhost);
			*error = DHCP_IPC_E_INVIF;
			goto failure;
		}
	}
...
plumb_lif(dhcp_pif_t *pif, const in6_addr_t *addr)
{
	...
	hold_pif(pif);
	if ((lif = insert_lif(pif, lifr.lifr_name, NULL, &error)) == NULL)
		goto failure;
...
dhcp_lif_t *
attach_lif(const char *lname, const char *reqhost, boolean_t isv6, int *error)
{
	dhcp_pif_t *pif;
	...
	return (insert_lif(pif, lname, reqhost, error));
}

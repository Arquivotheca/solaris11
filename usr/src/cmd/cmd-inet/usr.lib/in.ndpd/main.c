...
static	int	ndpd_create_addrs(const char *, struct sockaddr_in6, int,
    struct sockaddr_in6, boolean_t, boolean_t, const char *);
static int	ndpd_modify_addrs(const char *, struct sockaddr_in6, int,
    struct sockaddr_in6);
...
check_if_removed(struct phyint *pi)
{
	...
	if (!(pi->pi_kernel_state & PI_PRESENT) &&
	    (pi->pi_state & PI_PRESENT)) {
		struct lifreq lifr;
		boolean_t cleanup_ipadm_state = _B_FALSE;
		...
		/*
		 * We only clear ipadm state when the interface is
		 * unplumbed. We differentiate between the interface
		 * being unplumbed and just down by issuing an
		 * SIOCGLIFFLAGS.
		 */
		if (init_ifsock() != 0)
			return;
...
ndpd_process_cmd(int newfd, ipadm_ndpd_msg_t *msg)
{
	...
	case IPADM_CREATE_ADDRS:
		err = ndpd_create_addrs(msg->inm_ifname, msg->inm_intfid,
		    msg->inm_intfidlen, msg->inm_dintfid, msg->inm_stateless,
		    msg->inm_stateful, msg->inm_aobjname);
		break;

	case IPADM_MODIFY_ADDRS:
		err = ndpd_modify_addrs(msg->inm_ifname, msg->inm_intfid,
		    msg->inm_intfidlen, msg->inm_dintfid);
		break;
 ...
 * the given tokens as the interface ids during the next Router Advertisement.
 * Currently, only per interface tokens are supported.
 */
static int
ndpd_create_addrs(const char *ifname, struct sockaddr_in6 intfid, int intfidlen,
    struct sockaddr_in6 dintfid, boolean_t stateless, boolean_t stateful,
    const char *addrobj)
{
	struct phyint *pi;
	struct lifreq lifr;
	...
	if (pi == NULL) {
		...
		if (pi == NULL) {
			if (debug & D_PHYINT)
				logmsg(LOG_ERR, "ndpd_create_addrs: could not "
				    "create phyint for interface %s", ifname);
			return (ENOMEM);
		}
	} else {
		/*
	...
	if (pi->pi_autoconf) {
		logmsg(LOG_ERR, "ndpd_create_addrs: autoconfiguration already "
		    "in progress\n");
		return (EEXIST);
	}
	...
	if (intfidlen == 0) {
		...
		if (debug & D_PHYINT)
			logmsg(LOG_DEBUG, "ndpd_create_addrs: "
			    "Set default token for interface %s\n", ifname);
	} else {
		...
		if (debug & D_PHYINT)
			logmsg(LOG_DEBUG, "ndpd_create_addrs: "
			    "Set non-default token for interface %s\n", ifname);
	}
	if (debug & D_PHYINT)
		logmsg(LOG_DEBUG, "ndpd_create_addrs: token is %s\n",
		    inet_ntop(AF_INET6, (void *)&pi->pi_token, abuf,
		    sizeof (abuf)));

	if (pi->pi_flags & IFF_POINTOPOINT) {
		if (dintfid.sin6_family == AF_UNSPEC) {
			if ((err = init_ifsock()) != 0)
				return (err);
			...
			if (ioctl(ifsock, SIOCGLIFDSTADDR, (char *)&lifr) < 0) {
				err = errno;
				logmsg(LOG_ERR,
				    "unable to get destination address for %s",
				    ifname);
				return (err);
			}
			sin6 = (struct sockaddr_in6 *)&lifr.lifr_addr;
			...
			if (debug & D_PHYINT)
				logmsg(LOG_DEBUG, "ndpd_create_addrs: "
				    "Set default destination token for "
				    "interface %s\n", ifname);
		} else {
			pi->pi_dst_token = dintfid.sin6_addr;
			if (debug & D_PHYINT)
				logmsg(LOG_DEBUG, "ndpd_create_addrs: "
				    "Set non-default destination token for "
				    "interface %s\n", ifname);
		}
	} else {
		pi->pi_dst_token = in6addr_any;
	...
	if (debug & D_PHYINT)
		logmsg(LOG_DEBUG, "ndpd_create_addrs: destination token is "
...
/*
 * Modify the per interface token(s).
 */
static int
ndpd_modify_addrs(const char *ifname, struct sockaddr_in6 intfid,
    int intfidlen, struct sockaddr_in6 dintfid)
{
	struct phyint *pi;
	char abuf[INET6_ADDRSTRLEN];
	...
	pi = phyint_lookup((char *)ifname);
	if (pi == NULL) {
		logmsg(LOG_ERR, "ndpd_modify_addrs: no phyint found for %s",
		    ifname);
		return (ENXIO);
	}
	if (intfidlen > 0) {
		...
		pi->pi_token_length = intfidlen;
		if (debug & D_PHYINT) {
			logmsg(LOG_DEBUG, "ndpd_modify_addrs: "
			    "modified token for interface %s\n", ifname);
			logmsg(LOG_DEBUG, "ndpd_modify_addrs: token is %s\n",
			    inet_ntop(AF_INET6, (void *)&pi->pi_token, abuf,
			    sizeof (abuf)));
		}
	}
	...
	if ((pi->pi_flags & IFF_POINTOPOINT) &&
	    (dintfid.sin6_family == AF_INET6)) {
		pi->pi_dst_token = dintfid.sin6_addr;
		if (debug & D_PHYINT) {
			logmsg(LOG_DEBUG, "ndpd_modify_addrs: "
			    "modified destination token to interface %s\n",
			    ifname);
			logmsg(LOG_DEBUG, "ndpd_modify_addrs: destination "
			    "token is %s\n", inet_ntop(AF_INET6,
			    (void *)&pi->pi_dst_token, abuf, sizeof (abuf)));
		}
	}
	return (0);
}
...
ndpd_delete_addrs(const char *ifname)
{
	...
	pi = phyint_lookup((char *)ifname);
	if (pi == NULL) {
		logmsg(LOG_ERR, "ndpd_delete_addrs: no phyint found for %s",
		    ifname);
		return (ENXIO);
	}
	if (IN6_IS_ADDR_UNSPECIFIED(&pi->pi_token)) {
		logmsg(LOG_ERR, "ndpd_delete_addrs: token does not exist for "
		    " %s", ifname);
		return (EINVAL);
	}
	...
	if (debug & D_PHYINT) {
		logmsg(LOG_DEBUG, "ndpd_delete_addrs: "
		    "removed tokens from interface %s\n", pi->pi_name);
	}
	return (0);
}

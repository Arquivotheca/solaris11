...
void
phyint_cleanup(struct phyint *pi, boolean_t cleanup_ipadm_state)
{
	pi->pi_state = 0;
...
prefix_update_ipadm_addrobj(struct prefix *pr, boolean_t add)
{
	...
	if (status != IPADM_SUCCESS && status != IPADM_DAEMON_IPC_FAILURE) {

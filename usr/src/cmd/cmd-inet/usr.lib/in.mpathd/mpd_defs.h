...
#define	D_XPROBE	0x0100		/* Transitive probing */
/*
...
/*
 * Time interval that defines the size of "same time slot" to resolve failure
 * state flapping for Transitive probing.
 */
#define	XPROBE_SETTLING_TIME	(9000000000LL) /* (In ns) 9s */
...
/*
 * The circular probe stats array for icmp probes should be able to hold enough
...
/*
 * The circular probe stats array for local probes should be able to hold enough
 * samples to detect phyint failure, target failure, phyint repair
 * and target repair. We track this per local probe target (i.e., interface
 * in the group)
 */
#define	LPROBE_STATS_COUNT	\
	((uint16_t)(NUM_PROBE_REPAIRS + 2))
...
#define	XPROBE_INTERVAL		1000	/* Send local probes every 1s */
...
extern	int	xprobe_interval;	/* interval between indirect probes */
...
extern	boolean_t probe_enabled(void);

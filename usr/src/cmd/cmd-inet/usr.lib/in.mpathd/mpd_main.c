...
int	xprobe_interval;		/* interval for  indirect probes. */
					/* Defaults to XPROBE_INTERVAL when */
					/* transitive-probing is enabled */
...
static void	in_xprobe(struct phyint *);
static boolean_t logint_in_global_zone(struct logint *);
...
again:
	...
	/*
	 * Set up the targets available for indirect probing.
	 */
...
/*
 * Return TRUE if the IP address in `li' is currently bound to `under_if' in
 * the kernel.
 */
static boolean_t
verify_bound_phyint(struct phyint *under_if, struct logint *li)
{
	struct lifreq lifr;

	bzero(&lifr, sizeof (lifr));
	(void) strlcpy(lifr.lifr_name, li->li_name, LIFNAMSIZ);
	if (ioctl(ifsock_v4, SIOCGLIFBINDING, &lifr) >= 0 &&
	    strcmp(under_if->pi_name, lifr.lifr_binding) == 0) {
		return (_B_TRUE);
	}
	return (_B_FALSE);
}
...
/*
 * Find a data address that is bound to the underlying interface `under_if'
 */
static struct logint *
bound_logint(struct phyint *ipmp_if, struct phyint *under_if)
{
	struct logint *li;

	for (li = ipmp_if->pi_v4->pii_logint; li != NULL; li = li->li_next) {
		if (!logint_in_global_zone(li))
			continue;
		if (verify_bound_phyint(under_if, li))
			return (li);
	}
	return (NULL);
}
...
/*
 * Walk the logints of this phyint instance, and select the best available test
 * address
 */
struct logint *
best_test_address(struct phyint_instance *pii)
{
	struct logint	*li;
	struct logint	*probe_logint = NULL;
	int		rating;

	for (li = pii->pii_logint; li != NULL; li = li->li_next) {
		/*
		 * Skip 0.0.0.0 addresses, as those are never actually usable.
		 */
		...
		/*
		 * Skip if the address has been assigned to a shared-IP NGZ
		 */
		if (!logint_in_global_zone(li))
			continue;
...
/*
 * Find an UP interface that is not INADDR_ANY or the unspecified address or
 * an IPv6 non-link-local address to be used as the source address of probes
 * on an interface in an anonymous group.
 */
struct logint *
anon_logint(struct phyint_instance *pii)
{
	struct logint	*li;
	struct logint	*probe_logint = NULL;
...
select_test_ifs(void)
{
	...
	for (pii = phyint_instances; pii != NULL; pii = next_pii) {
		...
		next_pii = pii->pii_next;
		probe_logint = NULL;
		if (IS_IPMP(pii->pii_phyint))
			continue;

		if ((pg = pii->pii_phyint->pi_group) != NULL &&
		    !pg->pg_has_testaddrs && xprobe_interval > 0) {
			/*
			 * Find a src address for probing.
			 *
			 * For IPv4, find any data address that is bound to pii.
			 *
			 * For IPv6, even though the link-local address never
			 * actually fails over to another interface in the
			 * group (and is therefore a good candidate
			 * probe_logint	as long as it is UP), we don't
			 * currently use it for ICMP probes unless NOFAILOVER
			 * has been explicitly configured.
			 */
			if (pii->pii_af != AF_INET6) {
				if (pg == phyint_anongroup) {
					probe_logint = anon_logint(pii);
				} else {
					for (pi2 = pg->pg_phyint; pi2 != NULL;
					    pi2 = pi2->pi_pgnext) {
						if (IS_IPMP(pi2)) {
							probe_logint =
							    bound_logint(pi2,
		...
		if (li != NULL) {
			 ...
			 * if it's suboptimal.
			 */
			if (!pg->pg_has_testaddrs && xprobe_interval > 0) {
				if (verify_bound_phyint(pii->pii_phyint, li))
					continue;
			} else if (rate_testflags(li->li_flags) ==
			    BEST_TESTFLAGS) {
				continue;
			}
		}
		 ...
		 */
		if (pg->pg_has_testaddrs)
			probe_logint = best_test_address(pii);
		/*
...
in_signal(int fd)
{
	...
	case SIGQUIT:
		if (phyint_instances != NULL)
			logmsg(LOG_WARNING, "Disabling IP multipathing "
			    "failure and repair detection with IPMP "
...
router_add_common(int af, char *ifname, struct in6_addr nexthop)
{
	...
	if (pi->pi_group == phyint_anongroup) {
		icmp_target_add(pii, nexthop, _B_TRUE);
	} else {
		pi = pi->pi_group->pg_phyint;
		for (; pi != NULL; pi = pi->pi_pgnext)
			icmp_target_add(PHYINT_INSTANCE(pi, af), nexthop,
			    _B_TRUE);
	}
}
...
dup_host_targets(struct phyint_instance	 *desired_pii)
{
	...
	for (pi = desired_pii->pii_phyint; pi != NULL; pi = pi->pi_pgnext) {
		...
		if ((pii == NULL) || (pii == desired_pii) ||
			...
			continue;
		for (tg = pii->pii_targets; tg != NULL; tg = tg->tg_next) {
			icmp_target_create(desired_pii, tg->tg_address,
			    _B_FALSE);
		}
	}
}
...
main(int argc, char *argv[])
{
	...
	xprobe_interval = (probe_enabled() ? XPROBE_INTERVAL : 0);
	...
	for (;;) {
		...
		for (i = 0; i < pollfd_num; i++) {
			...
			for (pi = phyints; pi != NULL; pi = pi->pi_next) {
				if (IS_IPMP(pi))
					continue;
				if (pollfds[i].fd == dlpi_fd(pi->pi_ldh)) {
					assert(xprobe_interval > 0);
					in_xprobe(pi);
					break;
				} else if (pi->pi_notes != 0 &&
...
send_ifinfo(int fd, ipmp_ifinfo_t *ifinfop)
{
	...
	ipmp_iflist_t	*iflist4p = ifinfop->if_targinfo4.it_iflistp;
	ipmp_iflist_t	*iflist6p = ifinfop->if_targinfo6.it_iflistp;
	ipmp_iflist_t	null_iflist = {0};
	unsigned int	retval;
	...
	if (iflist4p != NULL) {
		retval = ipmp_writetlv(fd, IPMP_IFLIST,
		    IPMP_IFLIST_SIZE(iflist4p->il_nif), iflist4p);
		if (retval != IPMP_SUCCESS)
			return (retval);
	} else {
		retval = ipmp_writetlv(fd, IPMP_IFLIST,
		    IPMP_IFLIST_SIZE(null_iflist.il_nif), &null_iflist);
		if (retval != IPMP_SUCCESS)
			return (retval);
	}
	...
	if (iflist6p != NULL) {
		retval = ipmp_writetlv(fd, IPMP_IFLIST,
		    IPMP_IFLIST_SIZE(iflist6p->il_nif), iflist6p);
	} else {
		retval = ipmp_writetlv(fd, IPMP_IFLIST,
		    IPMP_IFLIST_SIZE(null_iflist.il_nif), &null_iflist);
	}
...
#define	NS2MS(ns)	((ns)/(NANOSEC / MILLISEC))
static void
xprobe_print(xprobe_data_t *xpr, struct phyint *from,
    struct probe_stats *prstatp)
{
	...
	if (!(debug & D_PROBE))
		return;
	start = NS2MS(prstatp->pr_hrtime_sent);
	done = NS2MS(prstatp->pr_hrtime_ackproc);
	if (xpr->xp_proto != ORCL_XPROBE_PROTO)
		logdebug("\t protocol %x unrecognized\n", xpr->xp_proto);
	logdebug("\t type (%x) %s from %s\n", xpr->xp_type,
	    (xpr->xp_type == XP_REQUEST ? "req" : "resp"),
	    (from == NULL ? "<unknown>": from->pi_name));
	logdebug("\t ts %llx seq %x\n", xpr->xp_ts, xpr->xp_seq);
	logdebug("\t sent %x rtt %x\n",  start, done - start);
}
#undef NS2MS

/*
 * return TRUE if the receieved sequence number is within the acceptable
 * window.
 */
static boolean_t
local_probe_in_window(uint16_t xp_seq, uint16_t snxt)
{
	uint16_t left, right;

	right = snxt;
	if (snxt > LPROBE_STATS_COUNT) {
		/*
		 * sequence number has to be in [left, right)
		 */
		left = snxt - LPROBE_STATS_COUNT;
		if (xp_seq >= left && xp_seq < right)
			return (_B_TRUE);
		else
			return (_B_FALSE);
	} else {
		/*
		 * seq number has wrapped around. Make sure that xp_seq is
		 * in  [left, MAX] or [MIN, right)
		 */
		left = UINT16_MAX - (LPROBE_STATS_COUNT - snxt) + 1;
		if (xp_seq >= left || xp_seq < right)
			return (_B_TRUE);
		else
			return (_B_FALSE);
...
/*
 * Handle an incoming Transitive Probe request
 */
static void
in_xprobe(struct phyint *pi)
{
	uint8_t pkt[1024];
	int dlerr, lladdrlen;
	xprobe_data_t *xpr;
	...
	dlerr = dlpi_recv(pi->pi_ldh, NULL, NULL, pkt, &pktlen, -1, NULL);
	if (dlerr != DLPI_SUCCESS) {
		logerr("incoming xprobe: %s", dlpi_strerror(dlerr));
		return;
	}
	/* LINTED E_BAD_PTR_CAST_ALIGN */
	xpr = (xprobe_data_t *)pkt;

	cp = (uint8_t *)&xpr[1];
	lladdrlen = *(uint8_t *)cp;
	cp++;
	tg = phyint_lookup_local_target(pi, cp, lladdrlen);
	if (tg == NULL)
		return;

	/* Reject anything that is not a response */
	if (xpr->xp_type != XP_RESPONSE) {
		logerr("unknown probe of type 0x%x\n", xpr->xp_type);
		goto done;
	}

	/* Reject out of window probe replies */
	xp_seq = xpr->xp_seq;
	if (!local_probe_in_window(xp_seq, tg->tg_snxt))
		logtrace("out of window probe seq %u snxt %u on %s from %s",
		    xp_seq, tg->tg_snxt, pi->pi_name, tg->tg_local->pi_name);

	cur_hrtime = gethrtime();
	m = (int64_t)(cur_hrtime - xpr->xp_ts);
	if (m < 0) {
		/*
		 * This is a ridiculously high value of rtt. rtt has wrapped
		 * around. Log a message, and ignore the rtt.
		 */
		logerr("incoming probe reply: rtt wraparound cur_hrtime %lld "
		    "reply timestamp %lld\n", cur_hrtime, xpr->xp_ts);
	}

	/* update the rtt */
	set_crtt(tg, m, _B_TRUE, pi->pi_name, pi->pi_group->pg_probeint);


	/* update the stats */
	pr_ndx = MOD_SUB(tg->tg_lprobe_next, (uint16_t)(tg->tg_snxt - xp_seq),
	    LPROBE_STATS_COUNT);

	prstatp = &tg->tg_lprobes[pr_ndx];
	prstatp->pr_hrtime_ackproc = cur_hrtime;
	prstatp->pr_hrtime_ackrecv = cur_hrtime;

	probe_chstate(prstatp, pi->pi_name, AF_LINK, PR_ACKED);
	xprobe_print(xpr, tg->tg_local, prstatp);

done:
	(void) dlpi_recv(pi->pi_dh, NULL, NULL, NULL, NULL, 0, NULL);
}

/*
 * Return TRUE if the transitive-probing smf property has been set to TRUE
 */
boolean_t
probe_enabled(void)
{
	boolean_t ret = _B_TRUE;
	scf_simple_prop_t *prop;
	uint8_t *retstr;

	prop = scf_simple_prop_get(NULL, "svc:/network/ipmp:default",
	    "config", "transitive-probing");
	if (prop != NULL) {
...
/*
 * Return TRUE if the logical interface `li' has been assigned to a non-global
 * zone, e.g., when shared-IP zones are installed and running.
 */
static boolean_t
logint_in_global_zone(struct logint *li)
{
	struct lifreq lifr;
	int s = (li->li_flags & IFF_IPV4 ? ifsock_v4 : ifsock_v6);

	bzero(&lifr, sizeof (lifr));
	bcopy(li->li_name, lifr.lifr_name, sizeof (lifr.lifr_name));
	if (ioctl(s, SIOCGLIFZONE, (caddr_t)&lifr) == -1)
		return (_B_FALSE);
	return (lifr.lifr_zoneid == ALL_ZONES ||

...
static void icmp_target_print(struct target *tg);
static void icmp_target_insert(struct phyint_instance *pii, struct target *tg);
static struct target *icmp_target_first(struct phyint_instance *pii);
static struct target *icmp_target_select_best(struct phyint_instance *pii);
...
static uchar_t zero_hwaddr[DLPI_PHYSADDR_MAX] = { 0 };
...
phyint_link_notify(dlpi_handle_t dh, dlpi_notifyinfo_t *dnip, void *arg)
{
	...
	/*
	 * Check if either the new or old hardware address is a duplicate.
	 * As a special case, we never consider a zeroed hardware address to be
	 * a duplicate of another zeroed hardware address, though we need to
	 * handle the case where the old (non-zero) hardware address was a
	 * duplicate prior to becoming a zeroed hardware address.
	 */
	if (bcmp(zero_hwaddr, pi->pi_hwaddr, pi->pi_hwaddrlen) != 0)
...
phyint_link_init(struct phyint *pi)
{
	...
	if (xprobe_interval > 0) {
		retval = dlpi_open(pi->pi_name, &pi->pi_ldh, DLPI_NOATTACH);
		if (retval != DLPI_SUCCESS) {
			pi->pi_ldh = NULL;
			errmsg = "cannot open";
			goto failed;
		...
		errno = 0;
		fd = dlpi_fd(pi->pi_ldh);
		while (ioctl(fd, I_PUSH, "xprobe") == -1) {
			if (errno != EINTR) {
				logerr("%s: cannot push xprobe %s\n",
				    pi->pi_name, strerror(errno));
				goto failed;
			}
		}
		retval = dlpi_bind(pi->pi_ldh, ETHERTYPE_ORCL_1, NULL);
		if (retval != DLPI_SUCCESS) {
			dlpi_close(pi->pi_ldh);
			pi->pi_ldh = NULL;
			errmsg = "cannot bind to ETHERTYPE_ORCL_1";
			goto failed;
		}
		/*
		 * The xprobe module expects to intercept a DL_INFO_ACK so
		 * that it can set up physaddr related attributes for the
		 * interface, so we need to invoke dlpi_info() here.
		 */
		retval = dlpi_info(pi->pi_ldh, &dlinfo, DLPI_INFO_VERSION);
		if (retval != DLPI_SUCCESS) {
			dlpi_close(pi->pi_ldh);
			pi->pi_ldh = NULL;
			errmsg = "cannot get DLPI_INFO";
			goto failed;
		}
		(void) poll_add(fd);
		timer_schedule(xprobe_interval);
	}
	return (_B_TRUE);
failed:
...
phyint_link_close(struct phyint *pi)
{
	...
	/* skip IPMP meta interface */
	if (IS_IPMP(pi))
		assert(pi->pi_dh == NULL);
	else
		dlpi_close(pi->pi_dh);
	pi->pi_dh = NULL;
}
    ...
    uint64_t flags)
{
	 ...
	 */
	INIT_LINK_STATE(pi);
	phyint_state_info(pi, PI_UNKNOWN);

	if (!(flags & IFF_IPMP) && !phyint_link_init(pi)) {
		free(pi);
		return (NULL);
	}
...
phyint_chstate(struct phyint *pi, enum pi_state state)
{
	...
	pi->pi_state = state;
	/*
	 * Update the pi_state_info stats for RUNNING and FAILED states.
	 */
	if (pi->pi_state == PI_RUNNING)
		phyint_state_info(pi, PI_SUCCESS);
	else if (pi->pi_state == PI_FAILED) {
		if (LINK_DOWN(pi))
			reason = PI_LINK_DOWN;
		else if (!PROBE_ENABLED(pi->pi_v4) && !PROBE_ENABLED(pi->pi_v6))
			reason = PI_XPROBE_FAILED;
		else
			reason = PI_ICMP_PROBE_FAILED;
		phyint_state_info(pi, reason);
	}
	phyint_changed(pi);
}
...
retry:
	...
	if (!(flags & IFF_MULTICAST) ||
	    (flags & (IFF_VIRTUAL|IFF_POINTOPOINT)))
		return (NULL);
	/*
	 * Note that phyint_inst's are not created for data addresses on IPMP
	 * interfaces.
	 */
	...
	if (pg_name[0] == '\0' && !track_all_phyints) {
		 ...
		 */
		if ((flags &
		    (IFF_FAILED | IFF_INACTIVE | IFF_OFFLINE | IFF_PROBER))) {
	...
	if (pi_created && !IS_IPMP(pi)) {
		...
		/*
		 * If this phyint has an non-zero hardware address and it is not
		 * unique in its group, offline it.
		 */
		if (bcmp(zero_hwaddr, pi->pi_hwaddr, pi->pi_hwaddrlen) != 0 &&
		    phyint_lookup_hwaddr(pi, _B_TRUE) != NULL) {
			pi->pi_hwaddrdup = _B_TRUE;
			(void) phyint_offline(pi, 0);
		}
	}
...
phyint_inst_v4_sockinit(struct phyint_instance *pii)
{
	...
	if (debug & D_XPROBE)
		logdebug("%s: going to IP_BOUND_IF %d\n",
		    pii->pii_name, pii->pii_ifindex);
...
phyint_inst_update_from_k(struct phyint_instance *pii)
{
	...
	pi->pi_flags |= (lifr.lifr_flags & IFF_UP);
	if (pi->pi_v4 != NULL)
	...
	if ((pi->pi_flags & (IFF_UP|IFF_PROBER)) == IFF_PROBER)
...
phyint_inst_delete(struct phyint_instance *pii)
{
	...
	while (pii->pii_targets != NULL)
		icmp_target_delete(pii->pii_targets);
...
logint_init_from_k(struct phyint_instance *pii, char *li_name)
{
	...
	flags = lifr.lifr_flags;
	if (flags & IFF_NOFAILOVER) {
		if (!probe_enabled() &&
		    !pii->pii_phyint->pi_group->pg_has_testaddrs) {
			logerr("At least one NOFAILOVER test address has "
			    "been configured on group `%s'; link-state "
			    "fault-detection setting will be ignored for the "
			    "group", pii->pii_phyint->pi_group->pg_name);
		}
		pii->pii_phyint->pi_group->pg_has_testaddrs = 1;
	}
...
/* Lookup ICMP target on its address */
struct target *
icmp_target_lookup(struct phyint_instance *pii, struct in6_addr addr)
{
	struct target *tg;
	...
	if (debug & D_TARGET) {
		...
		logdebug("icmp_target_lookup(%s %s): addr %s\n",
		    AF_STR(pii->pii_af), pii->pii_name,
		    pr_addr(pii->pii_af, addr, abuf, sizeof (abuf)));
	}

	for (tg = pii->pii_targets; tg != NULL; tg = tg->tg_next) {
		assert(tg->tg_type == TG_ICMP);
 ...
 */
struct target *
icmp_target_next(struct target *tg)
{
...
icmp_target_select_best(struct phyint_instance *pii)
{
	struct target *tg;
 ...
 */
static void
icmp_target_activate_all(struct phyint_instance *pii)
{
	struct target *tg;
	...
	while (pii->pii_ntargets < MIN_PROBE_TARGETS) {
		tg = icmp_target_select_best(pii);
		if (tg == NULL) {
...
static struct target *
icmp_target_first(struct phyint_instance *pii)
{
	struct target *tg;
...
/*
 * Create a default TG_ICMP target entry.
 */
void
icmp_target_create(struct phyint_instance *pii, struct in6_addr addr,
    boolean_t is_router)
{
	struct target *tg;
	struct phyint *pi;
	...
	if (debug & D_TARGET) {
		...
		logdebug("icmp_target_create(%s %s, %s)\n",
		    AF_STR(pii->pii_af), pii->pii_name,
		    pr_addr(pii->pii_af, addr, abuf, sizeof (abuf)));
	}
	...
	if (li == NULL) {
		if (debug & D_TARGET) {
			...
			logdebug("icmp_target_create(%s %s, %s) no test addr\n",
			    AF_STR(pii->pii_af), pii->pii_name,
			    pr_addr(pii->pii_af, addr, abuf, sizeof (abuf)));
		}
		return;
	...
	if (tg == NULL) {
		logperror("icmp_target_create: calloc");
		return;
	}
 ...
 */
void
icmp_target_add(struct phyint_instance *pii, struct in6_addr addr,
    boolean_t is_router)
{
	struct target *tg;
	...
	tg = icmp_target_lookup(pii, addr);

	/*
	 * If the target does not exist, create it; icmp_target_create() will
	 * set tg_in_use to true.  Even if it exists already, if it's a router
	...
	if (tg == NULL || (is_router && !pii->pii_targets_are_routers))
		icmp_target_create(pii, addr, is_router);
	else if (is_router)
		tg->tg_in_use = 1;
}
 ...
 */
static void
icmp_target_insert(struct phyint_instance *pii, struct target *tg)
{
 ...
 */
void
icmp_target_delete(struct target *tg)
{
	int af;
	struct phyint_instance	*pii;
	...
	if (debug & D_TARGET) {
		...
		logdebug("icmp_target_delete(%s %s, %s)\n",
	...
	if (pii->pii_target_next == tg)
		pii->pii_target_next = icmp_target_first(pii);

	if (pii->pii_rtt_target_next == tg)
		pii->pii_rtt_target_next = icmp_target_first(pii);
...
target_flush_hosts(struct phyint_group *pg)
{
	...
	for (pi = pg->pg_phyint; pi != NULL; pi = pi->pi_pgnext) {
		...
		if (pii != NULL && !pii->pii_targets_are_routers) {
			 ...
			 * empty, icmp_target_delete() will set pii->pii_targets
			 * to NULL.
			 */
			while (pii->pii_targets != NULL)
				icmp_target_delete(pii->pii_targets);
	...
	for (i = 0; i < PROBE_STATS_COUNT; i++) {
		if (pii->pii_probes[i].pr_target == tg) {
			if (pii->pii_probes[i].pr_status == PR_UNACKED) {
				probe_chstate(&pii->pii_probes[i],
				    pii->pii_name, pii->pii_af, PR_LOST);
			}
...
static void
icmp_target_print(struct target *tg)
{
	char	abuf[INET6_ADDRSTRLEN];
	char	buf[128];
	char	buf2[128];
...
ifstate(struct phyint *pi)
{
	...
	for (addrp = pi->pi_group->pg_addrs; addrp != NULL;
	    addrp = addrp->al_next) {
		if ((addrp->al_flags & IFF_UP) == 0)
			continue;
		sa = (struct sockaddr *)&addrp->al_addr;
		if (!sockaddrunspec(sa))
			naddr++;
	}
	/*
	 * Transitive probing results are inconclusive if there are no IP
	 * addresses that can be used for ICMP probing. Thus, when transitive
	 * probing is enabled (xprobe_interval > 0 and the group has
	 * no test addresses, or is not an anonymous group), we must return
	 * "unknown" for the state
	 */
	if (xprobe_interval > 0 && naddr == 0 &&
	    pi->pi_group != phyint_anongroup && !pi->pi_group->pg_has_testaddrs)
...
ifprobestate(struct phyint *pi)
{
	...
	if (!PROBE_ENABLED(pi->pi_v4) && !PROBE_ENABLED(pi->pi_v6)) {
		...
		/*
		 * If transitive probing is enabled, verify that probe state
		 * is not failed before reporting OK.
		 */
	}
 ...
 */
int
probe_state_event(struct probe_stats *pr, const char *pi_name, int af,
    const char *tg_name)
{
	nvlist_t *nvl;
	...
	errno = nvlist_add_string(nvl, IPMP_IF_NAME, pi_name);
	if (errno != 0)
		goto failed;
	...
	case TG_ICMP:
		if (tg != NULL)
			addr2storage(af, &tg->tg_address, &ss);
		else
			addr2storage(af, &in6addr_any, &ss);
		break;
	case TG_LOCAL:
		/*
		 * For TG_LOCAL targets, we identify the target by the
		 * interface name only, and the af is ignored.
		 */
		break;
	default:
		goto failed;
	...
	if (tg_name == NULL)
		errno = nvlist_add_byte_array(nvl, IPMP_PROBE_ICMP_TARGET,
		    (uchar_t *)&ss, sizeof (ss));
	else
		errno = nvlist_add_string(nvl, IPMP_PROBE_LOCAL_TARGET,
		    tg_name);
	if (errno != 0)
		goto failed;
...
getgroupinfo(const char *grname, ipmp_groupinfo_t **grinfopp)
{
	...
	for (i = 0, pi = pg->pg_phyint; pi != NULL; pi = pi->pi_pgnext) {
		if (IS_IPMP(pi))
			continue;
		assert(i < nif);
 ...
 */
unsigned int
gettarginfo(struct phyint *pi, struct phyint_instance *pii,
    ipmp_targinfo_t **targinfopp)
{
	...
	char (*ifs)[LIFNAMSIZ];
	unsigned int i, nif = 0;
	const char *name = pi->pi_name;
	...
	/*
	 * Find the list of transitive probe targets: all interfaces in
	 * the group other than pii->pii_phyint itself.
	 */
	ifs = alloca(nif * sizeof (*ifs));
	for (i = 0, tg = pi->pi_loc_all; tg != NULL; tg = tg->tg_next, i++) {
		assert(i < nif);
		(void) strlcpy(ifs[i], tg->tg_local->pi_name, LIFNAMSIZ);
	}
	assert(i == nif);

	*targinfopp = ipmp_targinfo_create(name, iftestaddr(pii, &ss),
	    iftargmode(pii), ntarg, targs, i, ifs);
...
getifinfo(const char *ifname, ipmp_ifinfo_t **ifinfopp)
{
	...
	if ((retval = gettarginfo(pi, pi->pi_v4, &targinfo4)) != 0 ||
	    (retval = gettarginfo(pi, pi->pi_v6, &targinfo6)) != 0)
		goto out;
...
/*
 * Insert local target into the pi_loc_all for the phyint
 */
static void
local_target_insert(struct phyint *pi, struct target *tg)
{
	tg->tg_next = pi->pi_loc_all;
	pi->pi_loc_all = tg;
}

/*
 * Create a TG_LOCAL target entry.
 */
void
local_target_create(struct phyint *pi, struct phyint *loc)
{
	struct target *tg;

	if (debug & D_TARGET) {
		logdebug("local_target_create(%s, %s)\n",
		    pi->pi_name, loc->pi_name);
	}
	...
	tg->tg_num_deferred = 0;
	tg->tg_snxt = 1;
	local_target_insert(pi, tg);

}

static struct target *
local_target_lookup(struct target *tg, struct phyint *loc)
{
	struct target *tgp;

	for (tgp = tg; tgp != NULL; tgp = tgp->tg_next) {
		if (tgp->tg_local == loc)
			return (tgp);
	}
	return (NULL);
...
init_local_targets()
{
	struct phyint *pi, *loc;

	for (pi = phyints; pi != NULL; pi = pi->pi_next) {
		for (loc = phyints; loc != NULL; loc = loc->pi_next) {
			if (IS_IPMP(loc))
				continue;
			if (loc == pi || loc->pi_group != pi->pi_group ||
			    local_target_lookup(pi->pi_loc_all, loc) != NULL)
				continue;
			local_target_create(pi, loc);
		}
	}
}

/*
 * Given a local probe target 'tg' on the phyint src_pi, remove tg from
 * the target list, and also clean up any probe_stats that exist for tg.
 */
void
local_target_delete(struct target *tg)
{
	assert(tg->tg_type == TG_LOCAL);
...
/*
 * clean up any refrerences to the local phyint that is going away.
 */
void
cleanup_local_targets(struct phyint *loc)
{
	struct phyint *pi;
	struct target *tg, *tg_next;

	/* first take loc out of any target lists for other phyints */
	for (pi = phyints; pi != NULL; pi = pi->pi_next) {
		if (loc == pi)
			continue;
		for (tg = pi->pi_loc_all; tg != NULL; tg = tg_next) {
			tg_next = tg->tg_next;
			if (tg->tg_local == loc) {
				if (tg->tg_prev == NULL)
					pi->pi_loc_all = tg_next;
				local_target_delete(tg);
			}
		}
	}
	/* then clean up loc's target list */
	for (tg = loc->pi_loc_all; tg != NULL; tg = tg_next) {
		tg_next = tg->tg_next;
		local_target_delete(tg);
	}
	loc->pi_loc_all = NULL;
...
struct target *
phyint_lookup_local_target(struct phyint *pi, uint8_t *hwaddr, size_t hwaddrlen)
{
	struct target *tg;

	for (tg = pi->pi_loc_all; tg != NULL; tg = tg->tg_next) {
		if (tg->tg_local->pi_hwaddrlen == hwaddrlen &&
		    bcmp(tg->tg_local->pi_hwaddr, hwaddr, hwaddrlen) == 0)
			break;
	}
	return (tg);
}
...
void
phyint_state_info(struct phyint *pi, enum pi_state_reason reason)
{
	pi->pi_state_info.pi_why = reason;
	pi->pi_state_info.pi_when = gethrtime();
}

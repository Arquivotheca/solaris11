        ...
        struct lifreq **best_lifr)
{
    ...
    *best_lifr = NULL;
	...
	/*
	 * If the IP address is not in our zone but is in a different zone we
	 * disregard the address and continue the search. In non-global zones
	 * this check is not needed as SIOCGLIFCONF only returns IP interfaces
	 * part of the zone. In global zone however this check is needed to
	 * ensure we don't use IP addresses assigned to shared IP stack zones.
	 */
        if (getzoneid() == GLOBAL_ZONEID && lifrcopy.lifr_zoneid !=
	    GLOBAL_ZONEID)
            continue;
#endif

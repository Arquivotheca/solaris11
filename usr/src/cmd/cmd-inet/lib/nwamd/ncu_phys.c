...
nwamd_get_link_state(const char *name)
{
	...
	if ((ksp = kstat_lookup(kcp, "link", 0, (char *)name)) == NULL) {
		/*
...
nwamd_set_unset_link_properties(nwamd_ncu_t *ncu, boolean_t set)
{
	...
	if (mtu == 0) {
		...
		if (status != DLADM_STATUS_OK) {
			nlog(LOG_ERR, "nwamd_set_unset_link_properties: "
			    "dladm_get_linkprop failed for %s: %s",
			    ncu->ncu_name, dladm_status2str(status, errmsg));
			return;
		}
	} else {
	...
	if (status != DLADM_STATUS_OK) {
		nlog(LOG_ERR, "nwamd_set_unset_link_properties: "
		    "dladm_set_linkprop failed for %s: %s",
		    ncu->ncu_name, dladm_status2str(status, errmsg));
	}
	...
	if (status != DLADM_STATUS_OK) {
		nlog(LOG_ERR, "nwamd_set_unset_link_properties: "
		    "dladm_set_linkprop failed for autopush property for %s:%s",
		    ncu->ncu_name, dladm_status2str(status, errmsg));
	}
    ...
    uint32_t security_mode, uint_t keyslot, char *raw_key)
{
	...
	nwamd_add_privs(0, PRIV_SYS_DL_CONFIG, PRIV_FILE_DAC_WRITE,
	    PRIV_FILE_CHOWN_SELF, NULL);
	status = dladm_unset_secobj(dld_handle, obj_name,
	    DLADM_OPT_ACTIVE | DLADM_OPT_PERSIST);
	nwamd_drop_privs(0, PRIV_SYS_DL_CONFIG, PRIV_FILE_DAC_WRITE,
	    PRIV_FILE_CHOWN_SELF, NULL);
    ...
    uint32_t security_mode, boolean_t add_to_known_wlans)
{
	...
	nwamd_add_privs(0, PRIV_NET_RAWACCESS, PRIV_SYS_DL_CONFIG, NULL);
	(void) dladm_wlan_disconnect(dld_handle, link->nwamd_link_id);
	nwamd_drop_privs(0, PRIV_NET_RAWACCESS, PRIV_SYS_DL_CONFIG, NULL);
...
find_best_wlan_cb(nwam_known_wlan_handle_t kwh, void *data)
{
	 ...
	 * done from the GUI or select-wifi subcommand of netadm(1M).
	 */
...
nwamd_wlan_connected(nwamd_object_t ncu_obj)
{
	...
	if (strcmp(essid, link->nwamd_link_wifi_essid) == 0) {
		...
		nwamd_add_privs(0, PRIV_NET_RAWACCESS, PRIV_SYS_DL_CONFIG,
		    NULL);
		(void) dladm_wlan_disconnect(dld_handle, link->nwamd_link_id);
		nwamd_drop_privs(0, PRIV_NET_RAWACCESS, PRIV_SYS_DL_CONFIG,
		    NULL);
	...
	if (status != DLADM_STATUS_OK) {
		...
		if (!wireless_autoconf || (status = do_connect(link_id, &attr,
			...
			if ((ncu_obj = nwamd_ncu_object_find(NWAM_NCU_TYPE_LINK,
			    linkname)) == NULL) {
				nlog(LOG_ERR, "wlan_connect_thread: could not "
				    "find object for link %s", linkname);
				goto done_unlocked;
			}

			/*
			 * The connection attempt has failed, switch NCU's
			 * state back to scanning.
			 */
			nwamd_object_set_state_timed(NWAM_OBJECT_TYPE_NCU,
			    ncu_obj->nwamd_object_name,
			    NWAM_STATE_OFFLINE_TO_ONLINE,

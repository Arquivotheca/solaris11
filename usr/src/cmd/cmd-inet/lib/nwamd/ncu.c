...
#define	VBOX_DEV_PREFIX		"vboxnet"
#define	USBECM_DEV_PREFIX	"usbecm"

/*
 * Seconds to wait before changing the state when plumbing fails
 */
#define	NWAMD_STATE_RETRY_INTERVAL		2

/*
 * Max number of retries when plumbing fails
 */
#define	NWAMD_STATE_RETRIES			5

/*
 * Number of plumbing failures
 */
static int state_retries = 0;

/*
 * iSCSI boot-related values.
 */
boolean_t iscsiboot = B_FALSE;
static uchar_t iscsiboot_hwaddr[DLPI_PHYSADDR_MAX];
 ...
 */
static void
nwamd_ncu_state_machine(const char *object_name, nwam_aux_state_t old_aux_state)
{
	...
	case NWAM_AUX_STATE_INITIALIZED:
		if (ncu->ncu_type == NWAM_NCU_TYPE_LINK) {
			...
			if (link->nwamd_link_media == DL_WIFI) {
				...
				if (!link->nwamd_link_wifi_connected &&
					...
					nlog(LOG_DEBUG,
					    ...
					    "disconnecting...");
					nwamd_add_privs(0, PRIV_NET_RAWACCESS,
					    PRIV_SYS_DL_CONFIG, NULL);
					(void) dladm_wlan_disconnect(dld_handle,
					    link->nwamd_link_id);
					nwamd_drop_privs(0, PRIV_NET_RAWACCESS,
					    PRIV_SYS_DL_CONFIG, NULL);
			...
			/*
			 * Plumb interfaces.  If plumbing fails, change state
			 * to DOWN.  From there the NCU will be reinitialized.
			 */
			if (ncu->ncu_if.nwamd_if_ipv4 &&
			    !nwamd_plumb_interface(ncu, AF_INET)) {
				if (state_retries++ < NWAMD_STATE_RETRIES) {
					nwamd_object_set_state_timed
					    (NWAM_OBJECT_TYPE_NCU, object_name,
					    NWAM_STATE_ONLINE_TO_OFFLINE,
					    NWAM_AUX_STATE_DOWN,
					    NWAMD_STATE_RETRY_INTERVAL);
				} else {
					nwamd_object_set_state
					    (NWAM_OBJECT_TYPE_NCU, object_name,
	...
	case NWAM_AUX_STATE_LINK_WIFI_SCANNING:
		...
		/*
		 * Create periodic scan event when moving from
		 * NWAM_AUX_STATE_INITIALIZED state. When moving back from
		 * NWAM_AUX_STATE_LINK_WIFI_CONNECTING, avoid creation of
		 * new periodic scan events, otherwise we end up with multiple
		 * independent scan event chains.
		 */
		if (old_aux_state != NWAM_AUX_STATE_LINK_WIFI_CONNECTING)
	...
	case NWAM_AUX_STATE_LINK_WIFI_NEED_SELECTION:
		...
		/* We are waiting for user intervention, daemonize */
		nwamd_create_daemonize_event();
		break;
	...
	case NWAM_AUX_STATE_LINK_WIFI_CONNECTING:
		...
		/* WiFi has started connecting, don't wait, daemonize */
		nwamd_create_daemonize_event();
		break;
...
nwamd_init_ncus(void)
{
	...
	/*
	 * Enqueue an event to check if nwamd can daemonize.  This is after
	 * all the INIT events, so the check will have NCUs to consider.
	 */
	nwamd_create_daemonize_event();
}
...
add_phys_ncu_to_ncp(nwam_ncp_handle_t ncph, const char *name)
{
	...
	err = nwam_ncu_create(ncph, name, NWAM_NCU_TYPE_LINK,
	    NWAM_NCU_CLASS_PHYS, &ncuh);
	if (err == NWAM_ENTITY_EXISTS) {
		return;
	} else if (err == NWAM_ENTITY_READ_ONLY) {
		nwamd_event_t retry_event;

		/*
		 * Root filesystem may be read-only or nwamd is running as
		 * root, retry in a few seconds.
		 */
		nlog(LOG_DEBUG, "Retrying addition of phys ncu for %s", name);
		...
		nlog(LOG_ERR, "failed to create link ncu for %s: %s",
		    name, nwam_strerror(err));
		return;
	}
...
add_ip_ncu_to_ncp(nwam_ncp_handle_t ncph, const char *name)
{
	...
	err = nwam_ncu_create(ncph, name, NWAM_NCU_TYPE_INTERFACE,
	    NWAM_NCU_CLASS_IP, &ncuh);
	/*
	 * Root filesystem may be read-only or nwamd may be running as root,
	 * but no need to retry here since add_phys_ncu_to_ncp() enqueues a
	 * retry event which will lead to add_ip_ncu_to_ncp() being called.
	 */
	if (err == NWAM_ENTITY_EXISTS || err == NWAM_ENTITY_READ_ONLY) {
		return;
	} else if (err != NWAM_SUCCESS) {
 ...
 * Called with:
 * - hotplug sysevent
 * - when nwam is started and walking the physical interfaces
 * - sysevent for new physical link added by dlmgmtd
 * Add/remove both link and interface NCUs from the Automatic NCP.  Assumes
 * that both link and interface NCUs don't exist.
 */
void
...
/*
 * Retrieves iSCSI boot device MAC address.  If result is all zeros, iSCSI boot
 * is not in use.  Called by main() before privileges are dropped.
 *
 * If iSCSI boot is in use, we need to tell net-loc method not to fall back
 * to NoNet - since doing so would apply IPfilter rules which would block
 * iSSCI traffic - so we specify the Automatic location as the fallback
 * location via the location/fallback property.
 */
extern void
nwamd_get_iscsiboot_hwaddr(void)
{
	...
	if ((fd = open(ISCSI_DRIVER_DEVCTL, O_RDONLY)) == -1) {
		nlog(LOG_DEBUG, "nwamd_get_iscsiboot_hwaddr: open of %s "
	...
	if (bcmp(iscsiboot_hwaddr, null_hwaddr, DLPI_PHYSADDR_MAX) != 0) {
		iscsiboot = B_TRUE;
		if (nwamd_set_string_property(NET_LOC_FMRI, NET_LOC_PG,
		    NET_LOC_FALLBACK_PROP, NWAM_LOC_NAME_AUTOMATIC) == 0) {
			if (smf_refresh_instance(NET_LOC_FMRI) != 0) {
				nlog(LOG_ERR, "nwamd_get_iscsiboot_hwaddr: "
				    "could not refresh %s", NET_LOC_FMRI);
			}
		} else {
			nlog(LOG_ERR, "nwamd_get_iscsiboot_hwaddr: could not "
			    "set %s/%s property to %s for %s", NET_LOC_PG,
			    NET_LOC_FALLBACK_PROP, NWAM_LOC_NAME_AUTOMATIC,
			    NET_LOC_FMRI);
		}
	}
}
...
static boolean_t
nwamd_is_iscsiboot_link(const char *name)
{
	int rc = 0;
	uchar_t hwaddr[DLPI_PHYSADDR_MAX];
	size_t hwlen = DLPI_PHYSADDR_MAX;
	dlpi_handle_t dh;

	if (!iscsiboot)
		return (B_FALSE);

	bzero(hwaddr, DLPI_PHYSADDR_MAX);

	nwamd_add_privs(0, PRIV_NET_RAWACCESS, NULL);
	rc = dlpi_open(name, &dh, 0);
	nwamd_drop_privs(0, PRIV_NET_RAWACCESS, NULL);

	if (rc != DLPI_SUCCESS) {
		nlog(LOG_ERR, "nwamd_is_iscsiboot_link: dlpi_open of %s "
		    "failed: %s", name, dlpi_strerror(rc));
		return (B_FALSE);
	}
	nwamd_add_privs(0, PRIV_SYS_DL_CONFIG, NULL);
	if ((rc = dlpi_get_physaddr(dh, DL_CURR_PHYS_ADDR, hwaddr, &hwlen))
	    != DLPI_SUCCESS) {
		nlog(LOG_ERR, "nwamd_is_iscsi_bootlink(): dlpi_get_physaddr %s "
		    "failed: %s", name, dlpi_strerror(rc));
	}
	nwamd_drop_privs(0, PRIV_SYS_DL_CONFIG, NULL);
	dlpi_close(dh);
	if (rc == DLPI_SUCCESS && bcmp(iscsiboot_hwaddr, hwaddr, hwlen) == 0)
		return (B_TRUE);
	return (B_FALSE);
}

/*
 * Returns B_TRUE for links that nwamd should ignore.
 */
boolean_t
nwamd_ignore_link(const char *name)
{
	datalink_id_t linkid;
	uint32_t media;
	dladm_phys_attr_t pa;

	/* Ignore links used in iSCSI boot */
	if (nwamd_is_iscsiboot_link(name))
		return (B_TRUE);

	/* Ignore links that belong to other zones */
	if (!nwamd_link_belongs_to_this_zone(name))
		return (B_TRUE);

	/* Ignore links that belong to an aggregation */
	if (nwamd_link_belongs_to_an_aggr(name))
		return (B_TRUE);

	/* Get linkid and media type for additional checks */
	if (dladm_name2info(dld_handle, name, &linkid, NULL, NULL, &media)
	    != DLADM_STATUS_OK)
		return (B_FALSE);

	/* Ignore Infiniband links */
	if (media == DL_IB)
		return (B_TRUE);

	/* Use linkid to get device name for additional checks */
	if (dladm_phys_info(dld_handle, linkid, &pa, DLADM_OPT_ACTIVE)
	    != DLADM_STATUS_OK)
		return (B_FALSE);

	/* Ignore VirtualBox vboxnet* interfaces */
	if (strncmp(pa.dp_dev, VBOX_DEV_PREFIX, strlen(VBOX_DEV_PREFIX)) == 0)
		return (B_TRUE);

	/* Ignore ilom usbecm* interfaces */
	if (strncmp(pa.dp_dev, USBECM_DEV_PREFIX, strlen(USBECM_DEV_PREFIX))
	    == 0)
		return (B_TRUE);
...
ncu_create_link_action_event(const char *name, void *data)
{
	...
	/* Do not generate an event for links that nwamd should ignore */
	if (nwamd_ignore_link(name))
		return (DLADM_WALK_CONTINUE);
...
nwamd_destroy_ncu(nwam_ncu_handle_t ncuh, void *data)
{
	...
	/*
	 * Links that exist return DLADM_OPT_ACTIVE flag.  Links that nwamd
	 * should ignore should be removed from the Automatic NCP.
	 */
	if (dladm_name2info(dld_handle, name, NULL, &flags, NULL, NULL)
	    == DLADM_STATUS_OK && (flags & DLADM_OPT_ACTIVE) &&
	    !nwamd_ignore_link(name)) {
		free(name);
		return (0);
	}
...
nwamd_ncu_handle_init_event(nwamd_event_t event)
{
	 ...
	 */
	if (state == NWAM_STATE_INITIALIZED) {
		/*
		...
		/*
		 * Refresh NCU.
		 * - if the NCU is disabled, move it to the disabled state.
		 * - if the link NCU is enabled, bring it online only if it is
		 *   disabled or offline and it is a manual NCU or it
		 *   satisfies priority group conditions.  Otherwise, keep it
		 *   offline.  For wireless links, also do a rescan.
		 * - if the interface NCU is enabled and it is not offline,
		 *   bring it down and let the state machine restart.
		 */
		if (!ncu->ncu_enabled) {
		...
		if (type == NWAM_NCU_TYPE_LINK) {
			...
			actmode = link->nwamd_link_activation_mode;
			priogrp = link->nwamd_link_priority_group;

			if ((state == NWAM_STATE_DISABLED ||
			    state == NWAM_STATE_OFFLINE) &&
			    (actmode == NWAM_ACTIVATION_MODE_MANUAL ||
			    (actmode == NWAM_ACTIVATION_MODE_PRIORITIZED &&
			    priogrp <= c))) {
				/*
				 * DISABLED or OFFLINE NCU is manually enabled
				 * or has priority <= current.  Activate it.
				 */
				nwamd_object_set_state
				    (NWAM_OBJECT_TYPE_NCU,
				    object->nwamd_object_name,
			    ...
			    priogrp > c) {
				/*
				 * NCU is ONLINE and priority has decreased
				 * with priority > current.
				 */
				nwamd_object_set_state
				    (NWAM_OBJECT_TYPE_NCU,
				    object->nwamd_object_name,
			...
			/*
			 * Do a rescan for wireless links.  If the current
			 * state and the active priority-group do not allow
			 * wireless network selection, then it won't happen.
			 */
			...
			/*
			 * If the interface NCU is offline*, online or in
			 * maintenance, mark it down (from there, it will be
			 * reinitialized to reapply its addresses).
			 */
	...
	case NWAM_ACTION_ENABLE:
		...
		if (object->nwamd_object_state == NWAM_STATE_ONLINE) {
			nlog(LOG_DEBUG, "nwamd_ncu_handle_action_event: "
			    "ncu %s already enabled or online, nothing to do",
			    event->event_object);
			nwamd_object_release(object);
			return;
		}
...
nwamd_ncu_handle_state_event(nwamd_event_t event)
{
	...
	nwam_aux_state_t old_aux_state, new_aux_state;
	nwamd_ncu_t *ncu;
	...
	old_aux_state = object->nwamd_object_aux_state;
	new_aux_state =
	...
	case NWAM_STATE_ONLINE_TO_OFFLINE:
		nwamd_ncu_state_machine(event->event_object, old_aux_state);
		break;
	...
	case NWAM_STATE_ONLINE:
		...
		/*
		 * If an interface NCU has gone ONLINE, enqueue a daemonize
		 * event to see if nwamd can daemonize.
		 */
		if (!is_link)
			nwamd_create_daemonize_event();
		break;

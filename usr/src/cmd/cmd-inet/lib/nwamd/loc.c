...
nwamd_loc_check_conditions(void)
{
	 ...
	 * location unless iSCSI booting.  In that case, an IP interface
	 * has been configured during iSCSI boot and we should avoid applying
	 * the NoNet profile to ensure iSCSI traffic can pass.
	 */
	(void) nwamd_walk_objects(NWAM_OBJECT_TYPE_NCU, nwamd_ncu_online_check,
	    &ncu_online);

	if (!ncu_online && !iscsiboot) {
		...
		if (iscsiboot && strcmp(winning_loc, NWAM_LOC_NAME_NO_NET) == 0)
			winning_loc = NWAM_LOC_NAME_AUTOMATIC;
	}
...
nwamd_loc_handle_state_event(nwamd_event_t event)
{
	...
	case NWAM_STATE_MAINTENANCE:
		/*
		 * A location went ONLINE or into MAINTENANCE.  Check if nwamd
		 * can daemonize.
		 */

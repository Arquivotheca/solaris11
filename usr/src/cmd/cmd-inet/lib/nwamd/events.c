...
nwamd_event_name(int event_type)
{
	...
	case NWAM_EVENT_TYPE_DAEMONIZE:
		return ("DAEMONIZE");
...
nwamd_event_send(nwam_event_t event_msg)
{
	...
	nwamd_add_privs(0, PRIV_IPC_DAC_WRITE, NULL);
	err = nwam_event_send(event_msg);
	nwamd_drop_privs(0, PRIV_IPC_DAC_WRITE, NULL);
...
/*
 * Struct for walking interface NCUs and counting the state each one is in.
 * This is later used to determine if nwamd can daemonize or not.
 */
struct nwamd_daemon_check_walk_arg {
	uint64_t online_ncus;
	uint64_t waiting_ncus;
	uint64_t timedout_ncus;
	uint64_t maintenance_ncus;
};

/*
 * Callback function to check the state of each interface NCU.  NCUs in the
 * OFFLINE, UNINITIALIZED, and DISABLED state are not counted.  For WiFi
 * links, if a manual selection is needed, the corresponding interface NCU is
 * in OFFLINE state and thus not counted.  This allows nwamd to daemonize even
 * if no interface is configured.
 */
static int
nwamd_ncu_state_check(nwamd_object_t object, void *data)
{
	struct nwamd_daemon_check_walk_arg *wa = data;
	nwamd_ncu_t *ncu_data = object->nwamd_object_data;
	nwam_state_t state = object->nwamd_object_state;
	nwam_aux_state_t aux_state = object->nwamd_object_aux_state;

	/* Only look at the state of interface NCUs */
	if (ncu_data->ncu_type != NWAM_NCU_TYPE_INTERFACE)
		return (0);

	if (state == NWAM_STATE_ONLINE) {
		wa->online_ncus++;
	} else if (state == NWAM_STATE_MAINTENANCE) {
		wa->maintenance_ncus++;
	} else if (state == NWAM_STATE_OFFLINE_TO_ONLINE) {
		if (aux_state == NWAM_AUX_STATE_IF_DHCP_TIMED_OUT)
			wa->timedout_ncus++;
		else if (aux_state == NWAM_AUX_STATE_IF_WAITING_FOR_ADDR)
			wa->waiting_ncus++;
	}
...
/*
 * Check whether nwamd can daemonize or not.  First, walk all interface NCUs
 * and count the different states they are in.  The following conditions will
 * either successfully daemonize nwamd or will exit with a failure.  By this
 * point, a child thread has already been created and daemonizing involves
 * telling the parent thread to exit.  If none of the conditions below are
 * satisfied, nwamd does not daemonize.
 *
 *  - If at least one NCU is in the ONLINE state, check to make sure that a
 *    location other than NoNet has been selected and then daemonize.
 *
 *  - If there are no NCUs in the ONLINE, WAITING_FOR_ADDR, DHCP_TIMED_OUT or
 *    MAINTENANCE state, it means the NCP has no NCUs or all are OFFLINE or
 *    DISABLED.  nwamd can daemonize in this case.
 *
 *  - If all NCUs are in the DHCP_TIMED_OUT state, then nwamd can daemonize.
 *    dhcpagent will continue to try to acquire a lease.
 *
 *  - If all NCUs are in the MAINTENANCE state, then nwamd will exit with a
 *    failure.
 */
static void
nwamd_daemon_check(void)
{
	struct nwamd_daemon_check_walk_arg wa;
	uint32_t now;
	...
	nlog(LOG_DEBUG, "nwamd_daemon_check: checking if nwamd can daemonize");

	/*
	 * If it is time for nwamd to daemonize (NEXT_FEW_SECONDS after
	 * 'ncu_wait_time' seconds since nwamd started), daemonize regardless
	 * of the NCU states.
	 */
	now = NSEC_TO_SEC(gethrtime());
	if (now >= nwamd_daemon_time) {
		nlog(LOG_INFO, "nwamd_daemon_check: "
		    "nwamd waited for at least %d seconds, daemonizing...",
		    ncu_wait_time);
		nwamd_inform_parent_exit(EXIT_SUCCESS);
		return;
	}

	bzero(&wa, sizeof (wa));
	(void) nwamd_walk_objects(NWAM_OBJECT_TYPE_NCU, nwamd_ncu_state_check,
	    &wa);

	if (wa.online_ncus > 0) {
		/*
		 * At least one NCU online, check if the location is non-emtpy
		 * and not NoNet.
		 */
		(void) pthread_mutex_lock(&active_loc_mutex);
		if (active_loc[0] != '\0' &&
		    strcmp(active_loc, NWAM_LOC_NAME_NO_NET) != 0) {
			nlog(LOG_DEBUG, "nwamd_daemon_check: "
			    "%lld NCU online and %s location active",
			    wa.online_ncus, active_loc);
			nwamd_inform_parent_exit(EXIT_SUCCESS);
		}
		(void) pthread_mutex_unlock(&active_loc_mutex);

	} else if (wa.online_ncus == 0 && wa.waiting_ncus == 0 &&
	    wa.timedout_ncus == 0 && wa.maintenance_ncus == 0) {
		/*
		 * No NCUs other than UNINITIALIZED, OFFLINE OR DISABLED.
		 * Daemonize only if nwamd has selected a location (usually
		 * NoNet).
		 */
		(void) pthread_mutex_lock(&active_loc_mutex);
		if (active_loc[0] != '\0') {
			nlog(LOG_DEBUG, "nwamd_ncu_check: "
			    "no active NCU and %s location active", active_loc);
			nwamd_inform_parent_exit(EXIT_SUCCESS);
		}
		(void) pthread_mutex_unlock(&active_loc_mutex);

	} else if (wa.timedout_ncus > 0 && wa.online_ncus == 0 &&
	    wa.waiting_ncus == 0) {
		/* All NCUs has DHCP has timed out, daemonize */
		nlog(LOG_DEBUG, "nwamd_ncu_check: "
		    "DHCP has timed out on all NCUs, daemonize");
		nwamd_inform_parent_exit(EXIT_SUCCESS);

	} else if (wa.maintenance_ncus > 0 && wa.online_ncus == 0 &&
	    wa.waiting_ncus == 0 && wa.timedout_ncus == 0) {
		/* All NCUs are in MAINTENANCE state, fail */
		nlog(LOG_INFO, "nwamd_ncu_check: all NCUs in maintenance");
		nwamd_inform_parent_exit(EXIT_FAILURE);
		exit(EXIT_FAILURE);
	}
}
...
nwamd_event_handler(void)
{
	...
	boolean_t ncu_check = B_FALSE, daemon_check = B_FALSE;
	...
	while (!got_shutdown_event) {
		...
		if (event->event_object[0] == '\0') {
			...
			case NWAM_EVENT_TYPE_QUEUE_QUIET:
				...
				if (!shutting_down && daemon_check) {
					nwamd_daemon_check();
					daemon_check = B_FALSE;
				}

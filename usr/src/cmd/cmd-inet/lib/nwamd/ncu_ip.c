...
fail:
	nlog(LOG_DEBUG, "get_dhcpinfo_data() of %s failed on %s: %s",
	    sym_name, ifname, errmsg);
	free(entry);
	free(request);
	free(reply);
	return (NULL);
    ...
    boolean_t *do_inform)
{
	...
	if (nifa->ipaddr_atype == IPADM_ADDR_DHCP) {
		...
		nwamd_add_privs(0, PRIV_SYS_IP_CONFIG, PRIV_NET_PRIVADDR,
		    PRIV_NET_RAWACCESS, NULL);
		ipstatus = ipadm_create_addr(ipadm_handle, nifa->ipaddr,
		    IPADM_OPT_ACTIVE | IPADM_OPT_UP | IPADM_OPT_NWAM_OVERRIDE);
		nwamd_drop_privs(0, PRIV_SYS_IP_CONFIG, PRIV_NET_PRIVADDR,
		    PRIV_NET_RAWACCESS, NULL);
...
nwamd_ncu_handle_if_state_event(nwamd_event_t event)
{
	...
	if (evm->nwe_data.nwe_if_state.nwe_addr_valid) {
		...
		if (addr_added) {
			...
			/*
			 * If a v4 dhcp address is added, refresh
			 * network/location since we may also have other DHCP
			 * information.  We might have to restore it first in
			 * case it is in maintenance.
			 */
			if (v4dhcp_running) {
    ...
    const char *ifname)
{
	...
	nwamd_add_privs(0, PRIV_SYS_IP_CONFIG, PRIV_NET_PRIVADDR, NULL);
	ipstatus = ipadm_delete_addr(ipadm_handle, aobjname,
	    IPADM_OPT_ACTIVE | rflags);
	nwamd_drop_privs(0, PRIV_SYS_IP_CONFIG, PRIV_NET_PRIVADDR, NULL);

	if (ipstatus != IPADM_SUCCESS && ipstatus != IPADM_OBJ_NOTFOUND) {
...
/*
 * Return B_TRUE if plumbing/unplumbing succeeds, B_FALSE otherwise.
 */
static boolean_t
nwamd_plumb_unplumb_interface(nwamd_ncu_t *ncu, sa_family_t af, boolean_t plumb)
{
	...
	/*
	 * Both plumb and unplumb needs to open /dev/udp or /dev/udp6.  At
	 * boot time, the device privilege requirements stored in
	 * /etc/security/device_policy may not be loaded yet, it is loaded by
	 * svc:/system/filesystem/root.  If they are not loaded, then nwamd
	 * must have all privileges to open /dev/udp or /dev/udp6, otherwise
	 * EACCES is returned.  Therefore, simply escalate to root to plumb
	 * and unplumb interfaces.  Also see comment in main.c:main() with
	 * regards to opening the dladm handle.
	 */
	if (plumb) {
		nwamd_become_root();
		ipstatus = ipadm_create_ip(ipadm_handle, ifname, af,
		    IPADM_OPT_ACTIVE | IPADM_OPT_NWAM_OVERRIDE);
	} else {
		/* release DHCP address, if any */
		if (af == AF_INET)
			dhcp_release(ifname);
		nwamd_become_root();
		ipstatus = ipadm_delete_ip(ipadm_handle, ifname, af,
		    IPADM_OPT_ACTIVE);
	}
	nwamd_release_root();
	...
	if (ipstatus != IPADM_SUCCESS) {
		if ((plumb && ipstatus != IPADM_IF_EXISTS) ||
		    (!plumb && ipstatus != IPADM_NOSUCH_IF)) {
...
{
	return (nwamd_plumb_unplumb_interface(ncu, af, B_TRUE));
}

void
nwamd_unplumb_interface(nwamd_ncu_t *ncu, sa_family_t af)
{
	(void) nwamd_plumb_unplumb_interface(ncu, af, B_FALSE);
}
...
retry:
	...
	case DHCP_INFORM:
	{
		...
		/*
		 * dhcpagent may have to be started, which requires us to be
		 * root.
		 */
		nwamd_become_root();
	...
	if (ipstatus == IPADM_DHCP_IPC_TIMEOUT) {
		...
		if (type == DHCP_START) {
			...
			/* Since DHCP timed out, check if nwamd can daemonize */
	...
	} else if ((ipstatus == IPADM_DHCP_IPC_FAILURE ||
	    ipstatus == IPADM_CANNOT_START_DHCP ||
	    ipstatus == IPADM_DAEMON_IPC_FAILURE) &&
	    retries++ < NWAMD_DHCP_RETRIES) {
		/*
		 * Retry DHCP request as we may have been unplumbing as part
		 * of the configuration phase.
		 */
		nlog(LOG_DEBUG, "start_dhcp: ipadm_%s_addr on %s returned: %s, "
		    "retrying in %d sec",

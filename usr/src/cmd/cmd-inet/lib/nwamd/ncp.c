...
nwamd_ncp_handle_state_event(nwamd_event_t event)
{
	...
	(void) strlcpy(active_ncp, new_ncp, sizeof (active_ncp));
	(void) pthread_mutex_unlock(&active_ncp_mutex);

	/*
	 * If nwamd has already daemonized, set the netcfg/active_ncp property
	 * and refresh nwamd.  Otherwise, manually refresh nwamd.  In the
	 * latter case, we cannot rely on SMF to refresh nwamd since nwamd has
	 * not daemonized and SMF will wait until nwamd has daemonized to
	 * refresh it.
	 */
	if (daemonized) {
		(void) nwamd_set_string_property(NP_DEFAULT_FMRI, NETCFG_PG,
		    NETCFG_ACTIVE_NCP_PROP, new_ncp);

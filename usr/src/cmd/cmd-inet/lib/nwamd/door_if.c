...
nwamd_door_req_action(nwamd_door_arg_t *req, ucred_t *ucr, struct passwd *pwd)
{
	...
	if (action == NWAM_ACTION_ENABLE || action == NWAM_ACTION_DISABLE) {
		if (chkauthattr(AUTOCONF_SELECT_AUTH, pwd->pw_name) == 0) {
			...
			nwamd_add_privs(0, PRIV_PROC_AUDIT, NULL);
			...
			nwamd_drop_privs(0, PRIV_PROC_AUDIT, NULL);
...
nwamd_door_init(void)
{
	...
	if (stat(NWAM_DOOR, &buf) < 0) {
		...
		nwamd_add_privs(0, PRIV_FILE_DAC_WRITE, NULL);
		...
		nwamd_drop_privs(0, PRIV_FILE_DAC_WRITE, NULL);

		/*
		 * Depending on how we started we might be running as root.
		 * We want the door to be owned by netadm/netadm.
		 */
		nwamd_add_privs(0, PRIV_FILE_CHOWN_SELF, NULL);
		if (chown(NWAM_DOOR, UID_NETADM, GID_NETADM) == -1) {
			nlog(LOG_ERR, "couldn't chown %s to %d/%d: %s",
			    NWAM_DOOR, UID_NETADM, GID_NETADM,
			    strerror(errno));
		}
		nwamd_drop_privs(0, PRIV_FILE_CHOWN_SELF, NULL);
	} else {
	...
	nwamd_add_privs(0, PRIV_SYS_MOUNT, NULL);
	(void) fdetach(NWAM_DOOR);
	nwamd_drop_privs(0, PRIV_SYS_MOUNT, NULL);

	/* Place our door in the file system so that others can find us. */
	nwamd_add_privs(0, PRIV_FILE_OWNER, NULL);
	...
	nwamd_drop_privs(0, PRIV_FILE_OWNER, NULL);
}

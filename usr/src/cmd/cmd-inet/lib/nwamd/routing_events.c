...
routing_events_v4(void *arg)
{
	...
	for (;;) {
		...
		/*
		 * Only RTM_NEWADDR is handled.  If an address is added
		 * without the IFF_UP flag, an RTM_CHGADDR is received.  When
		 * that address has the IFF_UP flag, RTM_NEWADDR is received.
		 * We are only interested once the address comes up.
		 */
		switch (rtm->rtm_type) {
		case RTM_NEWADDR:
		case RTM_DELADDR:
			...
			ip_event = nwamd_event_init_if_state(if_name,
			    ifa->ifam_flags,
			    (rtm->rtm_type == RTM_NEWADDR ? B_TRUE : B_FALSE),
			    addr, netmask);
			if (ip_event != NULL)
...
nwamd_routing_events_init(void)
{
	...
	if (v4_sock == -1)
		pfail("failed to open v4 routing socket: %s", strerror(errno));

	v6_sock = socket(AF_ROUTE, SOCK_RAW, AF_INET6);
	if (v6_sock == -1)
		pfail("failed to open v6 routing socket: %s", strerror(errno));
    ...
    struct sockaddr *gateway, const char *ifname)
{
	...
	if ((rlen = send(af == AF_INET ? v4_sock : v6_sock, rtbuf,
	    rtm->rtm_msglen, 0)) < 0) {

...
kw_add(const char *essid, const char *bssid)
{
	...
	if ((b = calloc(1, sizeof (bssid_t))) == NULL) {
		nlog(LOG_ERR, "kw_add: cannot allocate for bssid_t: %s",
		    strerror(errno));
		return (B_FALSE);
	}
	if ((kw = calloc(1, sizeof (kw_t))) == NULL) {
		nlog(LOG_ERR, "kw_add: cannot allocate for kw_t: %s",
		    strerror(errno));
		free(b);
		return (B_FALSE);
	}
...
kw_update(kw_t *kw, const char *bssid)
{
	...
	if ((b = calloc(1, sizeof (bssid_t))) == NULL) {
		nlog(LOG_ERR, "kw_update: cannot allocate for bssid_t: %s",
		    strerror(errno));
		return (B_FALSE);
	}
...
upgrade_known_wifi_nets_config(void)
{
	...
	for (kw = (kw_t *)kw_list.q_forw, priority = num_kw-1;
		...
		if (bssids == NULL) {
			nwam_known_wlan_free(kwh);
			nlog(LOG_ERR, "upgrade wlan %s: "
			    "could not calloc for bssids: %s", kw->kw_essid,
			    strerror(errno));
			continue;
		}

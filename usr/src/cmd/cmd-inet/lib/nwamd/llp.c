...
/*
 * This file contains the functions needed to upgrade the nwam repository.
 * Current version is NETCFG_DB_VERSION defined in util.h.
 *
 * Versions handled include:
 *	v0: NWAM phase 0/0.5
 *	    no smf version property
 *	    link/interface data is in /etc/nwam/llp
 *	    wlan data is in /etc/nwam/known_wifi_nets
 *
 *	v1: NWAM phase 1
 *	    nwamd/version property of network/physical:nwam has value '1'
 *	    link/interface data is in per-ncp files (/etc/nwam/ncp-<foo>.conf)
 *	    wlan data is in /etc/nwam/known_wlan.conf
 *	    enm and location data in /etc/nwam/enm.conf and /etc/nwam/loc.conf
 *
 *	v2: Instances merge
 *	    netcfg/version property in network/physical:default has value '2'
 *	    nwam instance is obsoleted, smf properties moved to default intance
 *	    active_ncp and version property is moved to netcfg property-group
 */
...
#define	NWAMD_PG			"nwamd"
#define	NWAMD_OLD_DHCP_WAIT_TIME_PROP	"dhcp_wait_time"
#define	NWAMD_OLD_USE_NET_SVC_PROP	"use_net_svc"
#define	NWAMD_OLD_IDLE_TIME_PROP	"idle_time"
...
llp_add(const char *name)
{
	...
	if ((llp = calloc(1, sizeof (llp_t))) == NULL) {
		nlog(LOG_ERR, "llp_add: cannot allocate LLP: %s",
		    strerror(errno));
		return (NULL);
	}
...
parse_llp_config(void)
{
	...
	if (fp == NULL) {
		...
		nlog(LOG_ERR, "parse_llp_config: "
		    "failed to open legacy LLP config file: %s",
		    strerror(errno));
		return (-1);
	}
...
nwamd_handle_upgrade(nwamd_event_t event)
{
	 ...
	 */
	if (nwamd_lookup_count_property(NP_NWAM_FMRI, NWAMD_PG,
	    NWAMD_OLD_DHCP_WAIT_TIME_PROP, &dhcp_wait_time) == 0) {
		(void) nwamd_set_count_property(NP_DEFAULT_FMRI, NETCFG_PG,
		    NWAMD_NCU_WAIT_TIME_PROP, dhcp_wait_time);
		(void) nwamd_delete_scf_property(NP_NWAM_FMRI, NWAMD_PG,
		    NWAMD_OLD_DHCP_WAIT_TIME_PROP);
		nlog(LOG_DEBUG, "nwamd_handle_upgrade: "
	 ...
	 */
	if (nwamd_lookup_count_property(NP_NWAM_FMRI, NWAMD_PG,
	    NWAMD_OLD_IDLE_TIME_PROP, &idle_time) == 0) {
		(void) nwamd_delete_scf_property(NP_NWAM_FMRI, NWAMD_PG,
		    NWAMD_OLD_IDLE_TIME_PROP);
	}
	if (nwamd_lookup_boolean_property(NP_NWAM_FMRI, NWAMD_PG,
	    NWAMD_OLD_USE_NET_SVC_PROP, &use_net_svc) == 0) {
		(void) nwamd_delete_scf_property(NP_NWAM_FMRI, NWAMD_PG,
		    NWAMD_OLD_USE_NET_SVC_PROP);
	}
}

...
static const char policy_conf_file[] = _PATH_SYSVOL "/ipsecpolicy.conf";
static const char lock_file[] = _PATH_SYSVOL "/ipsecconf.lock";
...
#define	CBUF_LEN		65536		/* Maximum size of the cmd */
/*
...
static int ipsecconf_qflag;
static boolean_t smf_managed;	/* Suppress warning about using smf(5). */
...
static boolean_t combined_mode(uint_t, uint_t);
...
#define	WHICH_ALG(type, wild, idx) ((wild)?(known_algs[type][idx].id):(idx))
	...
	for (encr_idx = encr_min; encr_idx <= encr_max; encr_idx++) {
		...
		combined = combined_mode(encr_alg, IPSEC_PROTO_ESP);
...
/*
 * This buffer is large enough to handle the largest combinatorial explosion
 * of all AH and ESP algorithms.
 */
#define	MAX_POL_MSG_LEN	65536
...
get_pf_pol_socket(void)
{
	...
	if (s < 0 && !ipsecconf_qflag) {
		if (errno == EPERM) {
			warn(gettext("Insufficient privileges to open "
			    "PF_POLICY socket"));
		} else {
			warn(gettext("Unable to open PF_POLICY socket"));
		}
	}
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	case IPSEC_CONF_FLUSH:
		...
		/* We dont't care that FLUSH fails (Blatant zones hack). */
...
/*
 * Truncate the POLICY_CONF_FILE for a flush or replace
 */
static int
ipsec_conf_truncate(void)
{
	int pfd;
...
parse_address(int type, char *addr_str)
{
	 ...
	 */
	hp = getipnodebyname(addr_str, AF_INET6, AI_V4MAPPED | AI_ALL,
	    &h_errno);
	if (hp != NULL) {
		/*
...
static boolean_t
combined_mode(uint_t alg_id, uint_t proto)
{
	struct ipsecalgent *alg;
	boolean_t rc;

	alg = getipsecalgbynum(alg_id, proto, NULL);
	if (alg != NULL) {
	...
	for (ap_num = 0; act_props->ap[ap_num].act != NULL; ap_num++) {
		...
		for (i = 0; act_props->ap[ap_num].prop[i]; i++, line_no++) {
			...
			case SPD_ATTR_ESP_ENCR:
				...
				is_combined_mode = combined_mode(
				    iap->iap_eencr.alg_id, IPSEC_PROTO_ESP);
				ipsec_ealg = B_TRUE;
				break;
			case SPD_ATTR_ESP_AUTH:
				/*
				...
				if (combined_mode(iap->iap_eauth.alg_id,
				    IPSEC_PROTO_AH)) {
					warnx(gettext("ERROR: Rule on line %d:"
					    "\nCombined mode algorithm cannot "
					    "be specified for encr_auth_alg."),
					    arg_indices[line_no] == 0 ? 1 :
					    arg_indices[line_no]);
					return (-1);
				}
		...
		if (is_combined_mode) {
			if (ipsec_eaalg) {
				warnx(gettext("ERROR: Rule on line %d: "
				    "Combined mode algorithm specified, "
				    "additional authentication/encryption not "
				    "supported."),
				    arg_indices[line_no] == 0 ? 1 :
				    arg_indices[line_no]);
				return (-1);
		...
		if (first_time) {
			...
			if (replace) {
				...
				if (flushret != 0)
					return (flushret);
				/*
				 * We don't want to fail for cosmetic
				 * reasons, so ignore this return value.
				 */
				(void) ipsec_conf_truncate();
			}
		}
...
ipsec_conf_sub()
{
	...
	while (fgets(rbuf, MAXLEN, remove_fp) != NULL) {
		...
		for (offset = prev_offset = prev_prev_offset = 0;
			...
			/* skip blank lines which separate policy entries */
			if (pbuf[0] == '\n')
				continue;

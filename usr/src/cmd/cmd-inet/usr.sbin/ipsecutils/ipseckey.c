...
static int
parseaddr(char *addr, struct hostent **hpp, char *ebuf)
{
	int hp_errno;
	...
	if (!nflag) {
		...
		hp = getipnodebyname(addr, AF_INET6, AI_V4MAPPED | AI_ALL,
		    &hp_errno);
	} else {
		/*
...
doaddup(int cmd, int satype, char *argv[], char *ebuf)
{
	...
	do {
		...
		case TOK_SRCADDR6:
			...
			sa_len = parseaddr(*argv, &srchp, ebuf);
			if (srchp == NULL) {
				ERROR1(ep, ebuf, gettext(
		...
		case TOK_DSTADDR6:
			...
			sa_len = parseaddr(*argv, &dsthp, ebuf);
			if (dsthp == NULL) {
				ERROR1(ep, ebuf, gettext(
		...
		case TOK_PROXYADDR6:
			...
			sa_len = parseaddr(pstr, &isrchp, ebuf);
			if (isrchp == NULL) {
				ERROR1(ep, ebuf, gettext(
		...
		case TOK_IDSTADDR6:
			...
			sa_len = parseaddr(pstr, &idsthp, ebuf);
			if (idsthp == NULL) {
				ERROR1(ep, ebuf, gettext(
		...
		case TOK_NATLOC:
			...
			sa_len = parseaddr(*argv, &natt_lhp, ebuf);
			if (natt_lhp == NULL) {
				ERROR1(ep, ebuf, gettext(
		...
		case TOK_NATREM:
			...
			sa_len = parseaddr(*argv, &natt_rhp, ebuf);
			if (natt_rhp == NULL) {
				ERROR1(ep, ebuf, gettext(

...
/*
 * netcfg is a lex/yacc based command interpreter used to manage network
 * configurations.  The lexer (see netcfg_lex.l) builds up tokens, which
 * the grammar (see netcfg_grammar.y) builds up into commands, some of
...
/* These *must* match the order of the RT1_ define's from netcfg.h */
...
/* These *must* match the order of the RT2_ define's from netcfg.h */
...
/*
 * No array for NCU_CLASS_.  The #define's in netcfg.h matches the
 * enum nwam_ncu_class_t in libnwam and thus uses libnwam functions to
 * retrieve the string representation.
 */

/* These *MUST* match the order of the PT_ define's from netcfg.h */
static char *pt_types[] = {
	...
	NWAM_LOC_PROP_DNS_NAMESERVICE_SORTLIST,
	NWAM_LOC_PROP_DNS_NAMESERVICE_OPTIONS,
...
static prop_table_entry_t loc_prop_table[] = {
	...
	{ PT_LOC_DNS_SORTLIST, 	NWAM_LOC_PROP_DNS_NAMESERVICE_SORTLIST },
	{ PT_LOC_DNS_OPTIONS, 	NWAM_LOC_PROP_DNS_NAMESERVICE_OPTIONS },
...
do_commit()
{
	...
	if (ret == NWAM_SUCCESS) {
		...
		case NWAM_OBJECT_TYPE_LOC:
			...
			if (verr == NWAM_ENTITY_MULTIPLE_VALUES) {
				nerr("Commit error: properties '%s' and '%s' "
    ...
    boolean_t is_list_prop)
{
	...
	if (ret != NWAM_SUCCESS) {
		nwamerr(ret, "Failed to create property value(s)");
		return (NULL);
	}
...
verify_func(cmd_t *cmd)
{
	...
	case NWAM_OBJECT_TYPE_LOC:
		...
		if (ret == NWAM_ENTITY_MULTIPLE_VALUES) {
			nerr("Verify error: properties '%s' and '%s' "
			    "are mutually-exclusive",
			    NWAM_LOC_PROP_DNS_NAMESERVICE_DOMAIN,
			    NWAM_LOC_PROP_DNS_NAMESERVICE_SEARCH);
			return;
...
/*
 * command-line mode (# netcfg list or # netcfg "select loc test; list")
 */
static int

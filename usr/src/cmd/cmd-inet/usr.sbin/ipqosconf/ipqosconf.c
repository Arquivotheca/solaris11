...
int tab_inserts)
{
	...
	for (x = llimit; x <= ulimit; x++) {
		...
		while (y < size) {
			...
			/*
			 * print entry delimiter (semicolon)? It must be
...
read_enum_nvs(char *line, char *module_name)
{
	...
	/*
	 * loop reading 'name = value' entrys separated by comma until
	 * reach closing brace.
	 */
...
/*
 * Given mapped_list with is a comma separated list of map names, and value,
	...
	for (;;) {
		 ...
		 */
			...
			if ((strcmp(action, IPQOS_CONF_CONT_STR) == 0) ||
				...
				/*
				 * we copy nm to a separate buffer as nv_pair

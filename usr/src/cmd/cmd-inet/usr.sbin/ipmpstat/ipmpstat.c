...
static void probe_output(ipmp_handle_t, char *, ofmt_handle_t);
...
main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "nLPo:agiptx:")) != EOF) {
...
sfunc_probe_id(ofmt_arg_t *ofmtarg, char *buf, uint_t bufsize)
{
	...
	if (nvlist_exists(nvl, IPMP_PROBE_LOCAL_TARGET))
		probe_type = 't';
	else if (nvlist_exists(nvl, IPMP_PROBE_ICMP_TARGET))
		probe_type = 'i';
	else
		return (sfunc_nvwarn("could not determine probe type"));

	if (nvlist_lookup_uint32(nvl, IPMP_PROBE_ID, &probe_id) != 0)
		return (sfunc_nvwarn("IPMP_PROBE_ID"));

	(void) snprintf(buf, bufsize, "%c%u", probe_type, probe_id);
	return (B_TRUE);
}
...
sfunc_probe_target(ofmt_arg_t *ofmtarg, char *buf, uint_t bufsize)
{
	...
	if (nvlist_lookup_byte_array(nvl, IPMP_PROBE_ICMP_TARGET,
	    (uchar_t **)&target, &nelem) == 0) {
		assert(!nvlist_exists(nvl, IPMP_PROBE_LOCAL_TARGET));
		sockaddr2str(target, buf, bufsize);
		return (B_TRUE);
	} else if (nvlist_lookup_string(nvl, IPMP_PROBE_LOCAL_TARGET,
	    &tg) == 0) {
		(void) snprintf(buf, bufsize, "<%s>", tg);
		return (B_TRUE);
	}
...
static void
probe_output(ipmp_handle_t ih, char *ifname, ofmt_handle_t ofmt)
{
	char sub[MAX_SUBID_LEN];
	evchan_t *evch;
	ipmpstat_probe_state_t ps = { ih, ofmt, ifname };
	uint_t nenabled = 0;
...
probe_event(sysevent_t *ev, void *arg)
{
	...
	if (psp->ps_ifname != NULL && strcmp(psp->ps_ifname, ifname) != 0)
		goto out;
...
sfunc_targ_testaddr(ofmt_arg_t *ofmtarg, char *buf, uint_t bufsize)
{
	...
	if (targinfop->it_targmode == IPMP_TARG_TRANSITIVE)
		(void) snprintf(buf, bufsize, "<%s>", targinfop->it_name);
	else if (targinfop->it_targmode != IPMP_TARG_DISABLED)
...
sfunc_targ_targets(ofmt_arg_t *ofmtarg, char *buf, uint_t bufsize)
{
	...
	ipmp_iflist_t		*iflistp = targinfop->it_iflistp;

	if (targinfop->it_targmode == IPMP_TARG_TRANSITIVE) {
		for (i = 0; i < iflistp->il_nif; i++) {
			(void) strlcat(buf, "<", bufsize);
			(void) strlcat(buf, iflistp->il_ifs[i], bufsize);
			(void) strlcat(buf, ">", bufsize);
			if ((i + 1) < iflistp->il_nif)
				(void) strlcat(buf, " ", bufsize);
		}
		return (B_TRUE);
	}
...
static boolean_t
icmp_probe_enabled(ipmp_if_targmode_t targmode)
{
	return (targmode != IPMP_TARG_DISABLED &&
	    targmode != IPMP_TARG_TRANSITIVE);
}
...
targinfo_output_cbfunc(ipmp_handle_t ih, void *infop, void *arg)
{
	 ...
	 */
	if (icmp_probe_enabled(targmode4) || !icmp_probe_enabled(targmode6))
		ofmt_output(arg, ih, &ifinfop->if_targinfo4);
	if (icmp_probe_enabled(targmode6))
...
static ofmt_field_t targ_fields[] = {
	...
	{ "MODE",	11,	0, sfunc_targ_mode		},
	{ "TESTADDR",	20,	0, sfunc_targ_testaddr		},
	{ "TARGETS",	37,	0, sfunc_targ_targets		},
	{ NULL,		0, 	0, NULL				}
};
...
static ipmpstat_enum_t	targ_mode[] = {
	...
	{ "transitive",	IPMP_TARG_TRANSITIVE			},
	{ NULL,		0 					}
};

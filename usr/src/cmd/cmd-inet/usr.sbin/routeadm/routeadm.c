 ...
 *
 * With PSARC 2011/179, the forwarding management is not done under SMF and
 * instead it's done under libipadm. The system-wide ipv4 and ipv6 forwarding
 * is set and retrieved using libipadm APIs. The persistence and reapplying of
 * forwarding configuration, on boot, is provided via the `ipgmtmd' daemon.
 *
 ...
 *	property to switch on (-e) or off (-d) at next boot.
 *
 *	Since this operation must be applied to multiple services in the
 *	routing daemon case,  we make use of the scf_walk_fmri() function,
 *	which applies a callback function to all matching functions.  In the
 *	case of the routing daemons,  we pass in a NULL signifying that all
 ...
 *	called by the ra_set_persistent_opt_cb() function and dispense
 ...
 *
 *	In the case of forwarding, we use IPADM_OPT_PERSIST flag to libipadm
 *	APIs to write to persistent store so that the configuration will be
 ...
 *
 *	In the case of forwarding, we use IPADM_OPT_ACTIVE flag to libipadm
 *	API to indicate that the configuration needs to be applied now.
 */
...
#define	RA_IPADM_FORWARDING_NAME	"forwarding"
...
raopt_t ra_opts[] = {
	...
	B_FALSE },
	{ RA_OPT_IPV4_FORWARDING, NULL, RA_SVC_FLAG_NONE,
	B_FALSE, NULL, NULL, B_FALSE },
	{ RA_OPT_IPV6_FORWARDING, NULL, RA_SVC_FLAG_NONE,
	B_FALSE, NULL, NULL, B_FALSE },
...
static boolean_t	iph_opened = B_FALSE;
...
static int ra_update(boolean_t);
...
/* Function used to set/retrieve forwarding options */
static int ra_get_set_forwarding(raopt_t *, uint_t, boolean_t);

/* Callbacks used to set/retrieve routing options */
...
static int ra_set_prop_from_string(scf_handle_t *, scf_instance_t *,
    ...
    const char **);
static boolean_t ra_get_default_opt(raopt_t *);

static void
usage(void)
{
	(void) fprintf(stderr, gettext(
	    "usage: %1$s [-p [option | var]]\n"
		...
		case 'r':
			...
			if ((raopt = ra_str2opt(optarg)) != NULL) {
				...
				case 'r':
					...
					raopt->opt_enabled =
					    ra_get_default_opt(raopt);
					break;
				}
				/* persist the setting for next boot */
				if (!IS_ROUTING_OPT(optarg)) {
					if (ra_get_set_forwarding(raopt,
					    IPADM_OPT_PERSIST, B_FALSE) != 0)
						exit(EXIT_FAILURE);
				} else if (ra_smf_cb(ra_set_persistent_opt_cb,
...
ra_upgrade_cmd(char opt, int argc, char **argv)
{
	...
	(void) fprintf(fp, "/usr/sbin/routeadm -%c ", opt);
	if (argv != NULL) {
		for (i = 0; i < argc; i++)
 ...
 *
 * if `booting' is B_TRUE then this command is being called in the context of
 * system boot and we have to only update the routing configuration as the
 * forwarding configuration would have already been applied by the `ipmgmtd'
 * daemon.
 */
static int
ra_update(boolean_t booting)
{
	int	i;
	...
	for (i = 0; ra_opts[i].opt_name != NULL; i++) {
		if (!IS_ROUTING_OPT(ra_opts[i].opt_name)) {
			int	err;

			if (booting)
				continue;
			/*
			 * first we need to get the configuration persisted,
			 * via -e , -d or -r, for next boot and then apply that
			 * configuration for the current system.
			 */
			err = ra_get_set_forwarding(&ra_opts[i],
			    IPADM_OPT_PERSIST, B_TRUE);
			if (err == ENOENT)
				continue;
			else if (err != 0)
				return (-1);

			/* opt_enabled will have the right value, now set it */
			if (ra_get_set_forwarding(&ra_opts[i],
			    IPADM_OPT_ACTIVE, B_FALSE) != 0)
				return (-1);
		} else if (ra_smf_cb(ra_set_current_opt_cb, ra_opts[i].opt_fmri,
		    &ra_opts[i]) == -1) {
			return (-1);
		}
	}
...
ra_report(boolean_t parseable, const char *param)
{
	...
	for (i = 0; ra_opts[i].opt_name != NULL; i++) {
		...
		forwarding_opt = !IS_ROUTING_OPT(ra_opts[i].opt_name);
		if (forwarding_opt) {
			if (ra_get_set_forwarding(&ra_opts[i],
			    IPADM_OPT_ACTIVE, B_TRUE) != 0)
				return (-1);
		} else if (ra_smf_cb(ra_get_current_opt_cb,
		...
		if (forwarding_opt) {
			...
			if (err == ENOENT) {
				/*
				 * We didn't find forwarding information
				 * in the persistent store and by default
				 * it is disabled.
				 */
		...
		} else if (ra_smf_cb(ra_get_persistent_opt_cb,
		    ra_opts[i].opt_fmri, &ra_opts[i]) == -1) {
			return (-1);
		}
		...
		if (forwarding_opt) {
			if (ra_get_set_forwarding(&ra_opts[i],
			    IPADM_OPT_DEFAULT, B_TRUE) != 0)
				return (-1);
		} else if (ra_smf_cb(ra_get_default_opt_cb,
...
/*
 * Parse the configuration file and fill the ra_opts array with opt_enabled
 * and opt_default_enabled values, and the ra_vars array with var_value and
...
/*
 * Opens the libipadm handle, if required. Based on how `get' is set,
 * gets/sets ipv4/ipv6 system-wide forwarding configuration.
 */
static int
ra_get_set_forwarding(raopt_t *raopt, uint_t flags, boolean_t get)
{
	...
	if (!iph_opened) {
		if ((status = ipadm_open(&iph, 0)) != IPADM_SUCCESS) {
			(void) fprintf(stderr, gettext(
			    "%s: failed to open an handle to libipadm %s\n"),
			    myname, ipadm_status2str(status));
			return (-1);
		}
		iph_opened = B_TRUE;
	}
	proto = (strcmp(raopt->opt_name, RA_OPT_IPV4_FORWARDING) == 0 ?
	    MOD_PROTO_IPV4 : MOD_PROTO_IPV6);
	if (get) {
		char		buf[MAXPROPVALLEN];
		uint_t		bufsize = sizeof (buf);
		boolean_t	enabled;

		status = ipadm_get_prop(iph, RA_IPADM_FORWARDING_NAME, buf,
		    &bufsize, proto, flags);
		if (status != IPADM_SUCCESS) {
			if (status == IPADM_OBJ_NOTFOUND &&
				...
				(void) fprintf(stderr, gettext(
				    "%s: failed to get value for %s : %s\n"),
				    myname, raopt->opt_name,
				    ipadm_status2str(status));
				return (-1);
			}
		}
		enabled = (strcmp(buf, "on") == 0);
		if (flags & (IPADM_OPT_ACTIVE | IPADM_OPT_PERSIST))
			raopt->opt_enabled = enabled;
		else
			raopt->opt_default_enabled = enabled;
	} else {
		char	*val;

		val = (raopt->opt_enabled ? "on" : "off");
		status = ipadm_set_prop(iph, RA_IPADM_FORWARDING_NAME, val,
		    proto, flags);
		if (status != IPADM_SUCCESS) {
			(void) fprintf(stderr, gettext(
			    "%s: failed to set value for %s : %s\n"),
			    myname, raopt->opt_name, ipadm_status2str(status));
			return (-1);
		}
	}
	return (0);
...
/* retrieves the default value for the given option */
static boolean_t
ra_get_default_opt(raopt_t *raopt)
{
	ra_resetopts();
	if (!IS_ROUTING_OPT(raopt->opt_name)) {
		if (ra_get_set_forwarding(raopt, IPADM_OPT_DEFAULT,
		    B_TRUE) != 0)
			exit(EXIT_FAILURE);
	} else {
		if (ra_smf_cb(ra_get_default_opt_cb,
		    raopt->opt_default_fmri, raopt) == -1)
			exit(EXIT_FAILURE);
		if (raopt->opt_enabled && ra_check_legacy_daemons() == -1)
			exit(EXIT_FAILURE);
	}
	return (raopt->opt_default_enabled);
}

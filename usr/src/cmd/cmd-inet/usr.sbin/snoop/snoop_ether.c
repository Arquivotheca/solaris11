...
interface_t INTERFACES[] = {
	...
	/* Infiniband */
	{ DL_IB, 66560, 0, 2, ETHERTYPE_IP, ETHERTYPE_IPV6,
...
static const struct ether_addr gvrp_addr = GVRP_DEST_ADDR;

#define	PFC_PAUSE_OPCODE 0x0101

uint_t
interpret_mcf(int flags, ushort_t *data, int len)
{
	int 		i;
	ushort_t	opcode, pev;
	ushort_t	*time;
	...
	show_header("MAC:  ", "MAC Control Frame", len);
	show_space();
	(void) sprintf(get_line(0, 0), "Opcode = 0x%4x", opcode);

	switch (opcode) {
	case PFC_PAUSE_OPCODE:
		pev = ntohs(data[1]);
		(void) sprintf(get_line(0, 0), "Priority Eneable Vector = 0x%x",
		    pev);
		time = &data[2];
		for (i = 0; i < 8; i++) {
			sprintf(buf, "%x", ntohs(time[i]));
			(void) snprintf(get_line(0, 0), get_line_remain(),
			    "	%s", getflag(pev, 1 << i, buf, "N/A"));
		}
		break;
	default:
		break;
...
inner_pkt:
	...
	if (len > 0 && (off + len <= (uchar_t *)e + elen)) {
		...
		case ETHERTYPE_AT:
			...
			break;
		case ETHERTYPE_LLDP:
			interpret_lldp(flags, (uint8_t *)data, len);
			break;
		case ETHERTYPE_MAC_CONTROL:
			interpret_mcf(flags, (ushort_t *)data, len);
			break;
		case 0:
			if (ieee8023 == 0)
				break;
			...
			case LLC_BPDU_SAP:
				interpret_bpdu(flags, data, len,
				    memcmp(&e->ether_dhost, &gvrp_addr,
				    sizeof (struct ether_addr)) == 0);
				break;
			}
			break;
...
ETHERTYPE_LLDP, "LLDP",
ETHERTYPE_MAC_CONTROL, "MAC Control Frame",

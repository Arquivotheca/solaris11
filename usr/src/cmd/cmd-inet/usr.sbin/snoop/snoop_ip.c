...
interpret_ipv6(int flags, const ip6_t *ip6h, int fraglen)
{
	...
	/*
	 * Use endian-aware macros to extract traffic class and
	 * flowinfo. Also, flowinfo is now 20 bits and class 8
	 * rather than 24 and 4.
	 */
	class = IPV6_FLOW_TCLASS(ip6h->ip6_vcf);
	flow = IPV6_FLOW_FLOWLABEL(ip6h->ip6_vcf);
	...
	if (!isfrag || flags & F_DTAIL) {
		...
		case IPPROTO_SCTP:
			...
			break;
		case IPPROTO_OSPF:
			interpret_ospf6(flags, (struct ospf6hdr *)data, iplen,
			    fraglen);
			break;
		}
	}

...
main(int argc, char **argv)
{
	...
	if (icapfile) {
		...
		/*
		 * The chunk size needs to be large enough to contain
		 * the largest data packet we can receive plus the
		 * bufmod header.  Infiniband CM mode has a maximum
		 * MTU of 65k, so the chunk size below is based on that.
		 * A possible future enhancement would be to dynamically
		 * set the chunk size based on the link MTU, but for
		 * now the value is static.
		 */
		const int chunksize = 66560;

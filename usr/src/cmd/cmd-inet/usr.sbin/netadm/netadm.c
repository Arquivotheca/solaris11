...
/*
 * netadm is a command interpreter to administer NWAM profiles.  It
...
int get_active_ncp(char *, size_t, scf_error_t *);
int set_active_ncp(const char *, scf_error_t *);
...
/* Structure for "netadm list" output */
...
static boolean_t
nwam_is_enabled(void)
{
	char		activencp[NWAM_MAX_NAME_LEN];
	scf_error_t	serr;

	if (get_active_ncp(activencp, sizeof (activencp), &serr) != 0) {
		die("Failed to retrieve active NCP from SMF: %s",
		    scf_strerror(serr));
	}

	return (!NWAM_NCP_DEFAULT_FIXED(activencp));
}
...
enable_func(int argc, char *argv[])
{
	...
	/* if autoconf is not active, only object we can change is the ncp */
	if (!nwam_is_enabled() && type != NWAM_OBJECT_TYPE_NCP) {
		die("Automatic network management must be enabled to manage "
		    "%s profiles; enable an NCP other than %s to enable "
		    "automatic network management",
		    nwam_object_type_to_string(type),
		    NWAM_NCP_NAME_DEFAULT_FIXED);
	}
	...
	{
		nwam_ncp_handle_t	ncph;
		char			aname[NWAM_MAX_NAME_LEN];
		scf_error_t		serr;
		boolean_t		curisfixed, newisfixed;

		if (get_active_ncp(aname, sizeof (aname), &serr) != 0)
			die("Failed to get the active profile");

		/*
		 * If we are transitioning from DefaultFixed to nwam or vice
		 * versa, stop network/physical:default, set netcfg/active_ncp
		 * property, refresh and start network/physical:default
		 */

		curisfixed = NWAM_NCP_DEFAULT_FIXED(aname);
		newisfixed = NWAM_NCP_DEFAULT_FIXED(name);
		if (curisfixed != newisfixed) {
			boolean_t freename = B_FALSE;

			/*
			 * Do case-insensitive matching.  For the two
			 * well-known names, we have a macro; for all
			 * others, nwam_ncp_read() will handle it and
			 * convert to the canonical case.
			 */
			if (newisfixed) {
				realname = NWAM_NCP_NAME_DEFAULT_FIXED;
			} else if (NWAM_NCP_AUTOMATIC(name)) {
				realname = NWAM_NCP_NAME_AUTOMATIC;
			} else {
				if ((ret = nwam_ncp_read(name, 0, &ncph))
				    != NWAM_SUCCESS) {
					die("Could not find NCP %s: %s",
					    name, nwam_strerror(ret));
				}
			...
			if (smf_disable_instance(NP_DEFAULT_FMRI,
			    SMF_TEMPORARY) != 0) {
				if (freename)
					free(realname);
				die("Failed to temporarily stop %s: %s",
				    NP_DEFAULT_FMRI, scf_strerror(scf_error()));
			}
			if (set_active_ncp(realname, &serr) != 0 ||
			    smf_refresh_instance(NP_DEFAULT_FMRI) != 0) {
				if (serr == SCF_ERROR_NONE)
					serr = scf_error();
				/*
				 * Setting the active_ncp property or
				 * refreshing the service failed.  Enable the
				 * service anyways so that the old state is
				 * restored.
				 */
				(void) smf_enable_instance(NP_DEFAULT_FMRI, 0);
				if (freename)
					free(realname);
				die("Failed to set active_ncp: %s",
				    scf_strerror(serr));
			}
			if (smf_enable_instance(NP_DEFAULT_FMRI, 0) != 0) {
				if (freename)
					free(realname);
				die("Failed to restart %s: %s",
				    NP_DEFAULT_FMRI, scf_strerror(scf_error()));
			}
			(void) printf(gettext("Enabling ncp '%s'\n"), realname);
			if (freename)
				free(realname);
			return;
		} else if (newisfixed && curisfixed) {
			(void) printf("'%s' profile is already enabled\n",
			    name);
			return;
		}
...
disable_func(int argc, char *argv[])
{
	...
	case NWAM_OBJECT_TYPE_NCU:
		...
		break;
	case NWAM_OBJECT_TYPE_NCP:
		die("NCPs cannot be disabled.  Enable a different NCP to "
		    "switch to that NCP");
	}
...
main(int argc, char *argv[])
{
	...
	for (i = CMD_MIN; i <= CMD_MAX; i++) {
		if (strcmp(argv[1], cmd_to_str(i)) == 0) {
			if (cmdtab[i].cmd_needs_nwamd && !nwam_is_enabled()) {
				die("%s NCP is enabled; automatic network "
				    "management is not available.\n"
				    "'%s %s' is only supported when automatic "
				    "network management is active.",
				    NWAM_NCP_NAME_DEFAULT_FIXED,
				    execname, cmd_to_str(cmdtab[i].cmd_num));
			}

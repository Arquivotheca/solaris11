...
static uint32_t		ipadm_flags = IPADM_OPT_ACTIVE;
static uint32_t		dladm_flags = DLADM_OPT_ACTIVE;
static boolean_t	persist_flag = _B_FALSE;

typedef struct ipadm_info {
	char			ii_aobjname[IPADM_AOBJSIZ];
	ipadm_addr_type_t	ii_atype;
} ipadm_info_t;
static ipadm_info_t	ipadm_info;
static boolean_t	update_addr = _B_FALSE;
...
static int	setifup(char *arg, int64_t param);
static int	setifdown(char *arg, int64_t param);
...
static void		ifconfig_get_ipadm_info(const char *, sa_family_t);
...
enum { G_NETMASK_NIL, G_NETMASK_PENDING, G_NETMASK_SET, G_NETMASK_APPLIED }
...
struct	cmd {
	...
	{ "up",		IFF_UP,		setifup,	0,	AF_ANY },
	{ "down",	-IFF_UP,	setifdown,	0,	AF_ANY },
...
#define	ADDRPROP_PREFIXLEN	"prefixlen"
#define	ADDRPROP_DEPRECATED	"deprecated"
#define	ADDRPROP_TRANSMIT	"transmit"
#define	ADDRPROP_PRIVATE	"private"

#define	IFPROP_ARP		"arp"
#define	IFPROP_FORWARDING	"forwarding"
#define	IFPROP_NUD		"nud"
#define	IFPROP_STANDBY		"standby"
#define	IFPROP_METRIC		"metric"
#define	IFPROP_MTU		"mtu"
#define	IFPROP_GROUP		"group"
#define	IFPROP_USESRC		"usesrc"

#define	LINKPROP_MACADDR	"mac-address"
#define	LINKPROP_ENCAPLIMIT	"encaplimit"
#define	LINKPROP_HOPLIMIT	"hoplimit"
...
main(int argc, char *argv[])
{
	...
	/*
	 * The -P flag is a private flag introduced by PSARC/2001/032.
	 * The purpose of the flag is to facilitate the conversion of
	 * the legacy /etc/hostname.<if> files into their ipadm equivalences.
	 * The contents of hostname.<if> files are processed during upgrade
	 * from pre-Solaris11 systems by prepending "ifconfig -P " to each
	 * line in the file. Since ifconfig has been modified to use libipadm
	 * and libdladm as its backend, making configurations persistent is
	 * as simple as enabling the IPADM_OPT_PERSIST and DLADM_OPT_PERSIST
	 * flags when calling the library APIs. Note, ifconfig requires that,
	 * if present, '-P' must be the first argument on the command line.
	 */
	if (strcmp(*argv, "-P") == 0) {
		if (argc < 2) {
			(void) fprintf(stderr, "'-P' requires arguments\n");
			exit(1);
		}
		persist_flag = _B_TRUE;
		ipadm_flags |= IPADM_OPT_PERSIST;
		dladm_flags |= DLADM_OPT_PERSIST;
		argc--, argv++;
	}
	...
	if (name[0] == '-') {
		...
		while ((c = getopt(2, av, "audDXZ46v")) != -1) {
			...
			case '4':
				/*
				 * -4 is not a compatible flag, therefore
		...
		(void) ifconfig_get_ipadm_info(name, af);
...
static void
ifconfig_get_ipadm_info(const char *lifname, sa_family_t af)
{
	ipadm_status_t		istatus;
	...
	if ((cp = strrchr(ifname, ':')) != NULL)
		*cp = '\0';
	/*
	 * Assume that ipadm does not yet have an address object for
	 * the logical interface.
	 */
	ipadm_info.ii_aobjname[0] = '\0';
	ipadm_info.ii_atype = IPADM_ADDR_NONE;

	/*
	 * Get all addresses and search this logical interface among the
	 * active addresses.
	 */
	istatus = ipadm_addr_info(iph, ifname, &ainfo, IPADM_OPT_ZEROADDR,
	    LIFC_DEFAULT);
	if (istatus != IPADM_SUCCESS)
		return;
	for (ainfop = ainfo; ainfop != NULL; ainfop = IA_NEXT(ainfop)) {
		if (ainfop->ia_state != IPADM_ADDRS_DISABLED &&
		    strcmp(ainfop->ia_ifa.ifa_name, lifname) == 0 &&
		    af == ainfop->ia_ifa.ifa_addr->sa_family) {
			(void) strlcpy(ipadm_info.ii_aobjname,
			    ainfop->ia_aobjname,
			    sizeof (ipadm_info.ii_aobjname));
			ipadm_info.ii_atype = ainfop->ia_atype;
			break;
		}
	}
...
/*
 * Create an ipadm address object to be used to set an address
 * or interface id via libipadm.
 */
static ipadm_status_t
create_addrobj_common(int af)
{
	ipadm_addr_type_t	type;
	ipadm_status_t		istatus = IPADM_SUCCESS;

	if (ipadm_info.ii_atype == IPADM_ADDR_NONE)
		(void) ifconfig_get_ipadm_info(name, af);
	if (ipadm_info.ii_aobjname[0] != '\0')
		update_addr = _B_TRUE;

	switch (ipadm_info.ii_atype) {
	case IPADM_ADDR_IPV6_ADDRCONF:
		/*
		 * We only support setting "static addresses" (i.e., tokens)
		 * on lifnum zero.
		 */
		if (lifnum(name) != 0) {
			(void) fprintf(stderr, "ifconfig: cannot set static "
			    "addresses on an ADDRCONF interface.\n");
			istatus = IPADM_OP_NOTSUP;
			break;
		}
	...
	if (istatus == IPADM_SUCCESS)
		istatus = ipadm_create_addrobj(type, name, &ipaddr);
	return (istatus);
}

/*
 * Set an ipadm address object address or interface id. Relies on
 * 'ipaddr' already being initialized by create_addrobj_common().
 */
static ipadm_status_t
set_addr_common(const char *addrstr, int prefixlen, int af)
{
	...
	case IPADM_ADDR_IPV6_ADDRCONF:
		/*
		 * Treat the address as an interface id. ifconfig only
		 * allows setting addresses/tokens on the link-local
		 * interface (i.e. lifnum zero).
		 */
		assert(lifnum(name) == 0);
		if (inet_pton(AF_INET6, addrstr, &sin6.sin6_addr) != 1) {
			(void) fprintf(stderr, "ifconfig: invalid address");
			exit(1);
		}
		if (!IN6_IS_ADDR_LINKLOCAL(&sin6.sin6_addr)) {
			(void) fprintf(stderr, "ifconfig: interface requires "
			    "a link-local address\n");
			exit(1);
		}

		istatus = ipadm_set_interface_id(ipaddr, addrstr);
		break;
	case IPADM_ADDR_NONE:
	case IPADM_ADDR_STATIC:
		...
		break;
	case IPADM_ADDR_DHCP:
		(void) fprintf(stderr, "ifconfig: cannot set a static address "
		    " on a DHCP interface.\n");
		exit(1);
	default:
		assert(_B_FALSE);
...
/*
 * Set an ipadm address object destination address or interface id. Relies
 * on 'ipaddr' already being initialized by create_addrobj_common().
 */
static ipadm_status_t
set_dst_addr_common(const char *addrstr, int af)
{
	...
	case IPADM_ADDR_IPV6_ADDRCONF:
		/*
		 * Treat the address as a destination interface id.
		 * ifconfig only allows setting addresses/tokens on the
		 * link-local interface (i.e. lifnum zero).
		 */
		assert(lifnum(name) == 0);
		if (inet_pton(AF_INET6, addrstr, &sin6.sin6_addr) != 1) {
			(void) fprintf(stderr, "ifconfig: invalid address");
		...
		if (!IN6_IS_ADDR_LINKLOCAL(&sin6.sin6_addr)) {
			(void) fprintf(stderr, "ifconfig: destination address "
			    "must be a link-local address\n");
			exit(1);
		}
		istatus = ipadm_set_dst_interface_id(ipaddr, addrstr);
		break;
	case IPADM_ADDR_NONE:
	case IPADM_ADDR_STATIC:
		istatus = ipadm_set_dst_addr(ipaddr, addrstr, af);
		break;
	case IPADM_ADDR_DHCP:
		(void) fprintf(stderr, "ifconfig: cannot set destination "
		    "address on DHCP interface.\n");
		exit(1);
...
/*
 * Create an address via libipadm. Relies on 'ipaddr' already being
 * initialized by create_addrobj_common() and on set_addr_common() and/or
 * set_dst_addr_common() having set the addresses in 'ipaddr'.
 */
static ipadm_status_t
create_addr_common()
{
	...
	case IPADM_ADDR_IPV6_ADDRCONF:
		assert(lifnum(name) == 0);
		istatus = ipadm_update_interface_id(iph, ipaddr,
		    ipadm_flags);
		break;
	case IPADM_ADDR_NONE:
	case IPADM_ADDR_STATIC:
		if (update_addr) {
			istatus = ipadm_update_addr(iph, ipaddr, ipadm_flags);
		} else {
			istatus = ipadm_create_addr(iph, ipaddr, ipadm_flags);
			(void) ipadm_get_aobjname(ipaddr,
			    ipadm_info.ii_aobjname,
			    sizeof (ipadm_info.ii_aobjname));
		}
		break;
	case IPADM_ADDR_DHCP:
...
ifconfig(int argc, char *argv[], int af, struct ifaddrs *ifa)
{
	...
	while (argc > 0) {
		...
		if (setaddr && ipaddr != NULL) {
			 ...
			 */
			if (p->c_name == 0) {
				istatus = set_dst_addr_common(*argv,
			 ...
			 * Add or update the addresses by calling libipadm.
			 */
			istatus = create_addr_common();
			if (istatus != IPADM_SUCCESS)
				goto createfailed;
			ipadm_destroy_addrobj(ipaddr);
			if (g_netmask_set != G_NETMASK_NIL)
				g_netmask_set = G_NETMASK_APPLIED;
			ipaddr = NULL;
...
/*
 * This function provides a protocol agnostic version of inet_ntop().
 * It accepts a pointer to a sockaddr_storage struct and returns
 * the IPv4 or IPv6 string representation of the address.
 */
static const char *
n2p(struct sockaddr_storage *laddr, char *addrstr, socklen_t size)
{
	struct	sockaddr_in6 *sin6;
	...
	if (af == AF_INET) {
		...
		return (inet_ntop(AF_INET, &sin->sin_addr, addrstr, size));
	} else {
		sin6 = (struct sockaddr_in6 *)laddr;
		return (inet_ntop(AF_INET6, &sin6->sin6_addr, addrstr, size));
	}
}
...
setifaddr(char *addr, int64_t param)
{
	...
	case NO_PREFIX:
		/*
		 * If no netmask or prefixlen is specified, get the
		 * current netmask.
		 */
		if ((afp->af_af == AF_INET && g_netmask_set == G_NETMASK_NIL) ||
		    afp->af_af == AF_INET6) {
			(void) strncpy(lifr.lifr_name, name,
			    sizeof (lifr.lifr_name));
			if (ioctl(s, SIOCGLIFNETMASK, (caddr_t)&lifr) < 0) {
				Perror0_exit("setifaddr: SIOCGLIFNETMASK");
			}
			if (afp->af_af == AF_INET) {
	...
	(void) n2p(&laddr, addrstr, sizeof (addrstr));

	istatus = create_addrobj_common(af);
	if (istatus != IPADM_SUCCESS)
	...
	if (prefixlen < 0) {
		(void) fprintf(stderr, "ifconfig: invalid netmask %s\n",
		    n2p(&lifr.lifr_addr, addrstr, sizeof (addrstr)));
		exit(1);
	}
	istatus = set_addr_common(addrstr, prefixlen, af);
	if (istatus != IPADM_SUCCESS)
...
setifnetmask(char *addr, int64_t param)
{
	...
	if (g_netmask_set == G_NETMASK_APPLIED)
		return (0);
	...
	if (ipadm_info.ii_aobjname[0] != '\0') {
		...
		prefixlen = mask2plen((struct sockaddr *)&netmask);
		if (prefixlen < 0) {
			(void) fprintf(stderr, "ifconfig: invalid netmask %s\n",
			    n2p((struct sockaddr_storage *)&netmask, addrstr,
			    sizeof (addrstr)));
			exit(1);
		}
		(void) snprintf(pval, sizeof (pval), "%d", prefixlen);
		istatus = ipadm_set_addrprop(iph, ADDRPROP_PREFIXLEN, pval,
		    ipadm_info.ii_aobjname, ipadm_flags);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "setifnetmask");
		return (0);
	}
...
setifprefixlen(char *addr, int64_t param)
{
	...
	if (ipadm_info.ii_aobjname[0] != '\0') {
		...
		snprintf(pval, sizeof (pval), "%d", prefixlen);
		istatus = ipadm_set_addrprop(iph, ADDRPROP_PREFIXLEN, pval,
		    ipadm_info.ii_aobjname, ipadm_flags);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "setifprefixlen");
		return (0);
	}
...
setifdstaddr(char *addr, int64_t param)
{
	...
	istatus = create_addrobj_common(af);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "setifdstaddr");

	if (ipadm_info.ii_atype == IPADM_ADDR_STATIC &&
	    ipadm_info.ii_aobjname[0] == '\0') {
		if (af == AF_INET)
			istatus = set_addr_common("0.0.0.0", 0, af);
		else
			istatus = set_addr_common("::", 0, af);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "could not set local address");
	}
	istatus = set_dst_addr_common(addr, af);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "could not set destination address");

	istatus = create_addr_common();
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "could not create address");
 ...
 */
static int
setifup(char *val, int64_t value)
{
	ipadm_status_t  istatus;

	if (ipadm_info.ii_aobjname[0] == '\0' ||
	    ipadm_info.ii_atype == IPADM_ADDR_IPV6_ADDRCONF)
		return (setifflags(val, value));

	istatus = ipadm_up_addr(iph, ipadm_info.ii_aobjname,
	    ipadm_flags);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "up");
	return (0);
}
 ...
 */
static int
setifdown(char *val, int64_t value)
{
	ipadm_status_t istatus;
	...
	istatus = ipadm_down_addr(iph, ipadm_info.ii_aobjname, ipadm_flags);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "down");
	return (0);
}

struct ifflag2ipadmprop {
	uint64_t ifflag;
	char *pname;
};

struct ifflag2ipadmprop flag2addrprop_tbl[] = {
	{ IFF_DEPRECATED,	ADDRPROP_DEPRECATED },
	{ IFF_NOXMIT,		ADDRPROP_TRANSMIT },
	{ IFF_PRIVATE,		ADDRPROP_PRIVATE },
	{ 0,			NULL}
};

struct ifflag2ipadmprop flag2ifprop_tbl[] = {
	{ IFF_NOARP,		IFPROP_ARP },
	{ IFF_ROUTER,		IFPROP_FORWARDING },
	{ IFF_NONUD,		IFPROP_NUD },
	{ IFF_STANDBY,		IFPROP_STANDBY },
	{ 0,			NULL }
};

static boolean_t
is_ipadm_prop(struct ifflag2ipadmprop *tbl, int64_t value, char **pname)
{
	for (; tbl->ifflag != 0; tbl++) {
		if (tbl->ifflag == value) {
			*pname = tbl->pname;
			break;
		}
...
static boolean_t
is_ipadm_addrprop(int64_t value, char **pname)
{
	return (is_ipadm_prop(flag2addrprop_tbl, value, pname));
}

static boolean_t
is_ipadm_ifprop(int64_t value, char **pname)
{
	return (is_ipadm_prop(flag2ifprop_tbl, value, pname));
}

static int
setifprop(char *pname, char *val)
{
	ipadm_status_t istatus;
	char ifname[LIFNAMSIZ];
	char *cp;
	uint_t proto = MOD_PROTO_IP;

	(void) strlcpy(ifname, name, sizeof (ifname));
	if ((cp = strrchr(ifname, ':')) != NULL)
		*cp = '\0';
	if (strcmp(pname, IFPROP_STANDBY) != 0)
		proto = ((af == AF_INET) ? MOD_PROTO_IPV4 : MOD_PROTO_IPV6);
	istatus = ipadm_set_ifprop(iph, ifname, pname, val, proto, ipadm_flags);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "setifprop");
	return (0);
}
...
setifflags(char *val, int64_t value)
{
	...
	if (is_ipadm_addrprop(check_value, &pname)) {
		if (ipadm_info.ii_aobjname[0] == '\0' ||
		    ipadm_info.ii_atype == IPADM_ADDR_IPV6_ADDRCONF)
			goto legacy;
		if (strcmp(pname, ADDRPROP_TRANSMIT) == 0) {
			if (strcmp(pval, "on") == 0)
				pval = "off";
			else
				pval = "on";
		}
		istatus = ipadm_set_addrprop(iph, pname, pval,
		    ipadm_info.ii_aobjname, ipadm_flags);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "setifflags");
		return (0);
	} else if (is_ipadm_ifprop(check_value, &pname)) {
		if (strcmp(pname, IFPROP_ARP) == 0 ||
		    strcmp(pname, IFPROP_NUD) == 0) {
			if (strcmp(pval, "on") == 0)
				pval = "off";
			else
				pval = "on";
		}
		return (setifprop(pname, pval));
	}
...
{
	return (setifprop(IFPROP_METRIC, val));
}
...
{
	return (setifprop(IFPROP_MTU, val));
}
...
setifether(char *addr, int64_t param)
{
	...
	status = ifconfig_dladm_open(name, DATALINK_CLASS_ALL, &linkid);
	if (status != DLADM_STATUS_OK)
	...
	status = dladm_set_linkprop(dlh, linkid, LINKPROP_MACADDR, &addr, 1,
	    dladm_flags);
	if (status != DLADM_STATUS_OK) {
		char errstr[DLADM_STRSIZE];

		(void) fprintf(stderr, "ifconfig: failed setting mac address "
		    "on %s: %s\n", name, dladm_status2str(status, errstr));
	}
...
addif(char *str, int64_t param)
{
	...
	/*
	 * The interface being configured is the newly created logical
	 * interface. Get the right ipadm interface info for that interface.
	 */
	(void) ifconfig_get_ipadm_info(name, af);
	 ...
	 * create_addr_common(), because, with IH_LEGACY, we need
...
setiftoken(char *addr, int64_t param)
{
	...
	if (ipadm_info.ii_aobjname[0] != '\0') {
		istatus = create_addrobj_common(AF_INET6);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "token");
		istatus = ipadm_set_interface_id(ipaddr, addr);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "token");
		istatus = ipadm_update_interface_id(iph, ipaddr,
		    ipadm_flags);
...
remove_ipmp(char *ifname)
{
	...
	istatus = ipadm_get_ifprop(iph, name, IFPROP_GROUP, grname, &bufsize,
	    MOD_PROTO_IP, ipadm_flags);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "Error retrieving IPMP groupname");
	if (grname[0] == '\0')
		return (0);
	...
	istatus = ipadm_remove_ipmp(iph, lifgr.gi_grifname, ifname,
	    ipadm_flags);
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "cannot remove from IPMP group");
	return (0);
}
...
setifgroupname(char *grname, int64_t param)
{
	...
	/* Return if interface does not exist. */
	bzero(&lifrl, sizeof (lifrl));
	(void) strlcpy(lifrl.lifr_name, name, LIFNAMSIZ);
	if (ioctl(s, SIOCGLIFGROUPNAME, &lifrl) == -1)
		Perror0_exit("setifgroupname: SIOCGLIFGROUPNAME");

	/* If the interface is an IPMP interface, change its groupname */
	if (ipadm_is_ipmp(iph, name)) {
		istatus = ipadm_set_ifprop(iph, name, IFPROP_GROUP, grname,
		    MOD_PROTO_IP, ipadm_flags);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "Error setting IPMP groupname");
		return (0);
	}

	if (grname[0] == '\0')
		return (remove_ipmp(name));

	bzero(ipmpif, sizeof (ipmpif));
again:
	/*
	 * Get the ipmp interface name to be fed to ipadm_add_ipmp().
	 */
	(void) strlcpy(lifgr.gi_grname, grname, LIFGRNAMSIZ);
	if (ioctl(s, SIOCGLIFGROUPINFO, &lifgr) == -1) {
		if (errno != ENOENT) {
		...
		/*
		 * For ENOENT, we will create the IPMP interface in
		 * response to the error IPADM_NOSUCH_IPMPIF from
		 * ipadm_add_ipmp().
		 */
	} else {
		(void) strlcpy(ipmpif, lifgr.gi_grifname, sizeof (ipmpif));
	}

	(void) strlcpy(lifrl.lifr_name, name, LIFNAMSIZ);
	while ((istatus = ipadm_add_ipmp(iph, ipmpif, name,
	    ipadm_flags)) != IPADM_SUCCESS) {
		switch (istatus) {
		case IPADM_NOSUCH_IPMPIF:
			/*
			...
			/*
			 * Get the ipmp interface name that was just created
			 * before retrying.
			 */
			goto again;

		case IPADM_ALREADY_IN_GRP:
			/*
			 * The interface is already in another group; must
			 * remove existing membership first.
			 */
			if (ioctl(s, SIOCGLIFGROUPNAME, &lifrl) == -1) {
				Perror2(name, "SIOCGLIFGROUPNAME");
				goto fail;
			}
			(void) strlcpy(lifgr.gi_grname, lifrl.lifr_groupname,
			    LIFGRNAMSIZ);
			if (ioctl(s, SIOCGLIFGROUPINFO, &lifgr) == -1) {
				if (errno == ENOENT)
					continue;
				Perror2(name, "SIOCGLIFGROUPINFO");
				goto fail;
			}
			istatus = ipadm_remove_ipmp(iph, lifgr.gi_grifname,
			    name, ipadm_flags);
			if (istatus != IPADM_SUCCESS) {
				(void) fprintf(stderr, "%s: cannot remove "
				    "existing IPMP group membership: %s", name,
				    ipadm_status2str(istatus));
				goto fail;
			}
			continue;
		case IPADM_UNDERIF_UP_ADDRS:
			 ...
			 */
			istatus = ipadm_down_addrs(iph, name, &downaddrs);
			if (istatus != IPADM_SUCCESS) {
				(void) fprintf(stderr, "%s: cannot bring "
				    "down addresses: %s", name,
				    ipadm_status2str(istatus));
				goto fail;
			}
			continue;
...
retry:
		...
		default:
			ipadmerr_exit(istatus, "cannot join IPMP group");
		}
	}
	/*
	 ...
	 */
	istatus = ipadm_up_addrs(iph, downaddrs);
	if (istatus != IPADM_SUCCESS) {
		(void) fprintf(stderr, "ifconfig: %s: cannot bring back up "
	 ...
	 * the daemon can migrate the data addresses from this interface to the
	 * IPMP interface.
	 */
	istatus = ipadm_migrate_dataaddrs(iph, name, downaddrs);
	if (istatus != IPADM_SUCCESS) {
		(void) fprintf(stderr, "ifconfig: %s: cannot migrate all "
		    "ipadm data address objects: %s\n", name,
		    ipadm_status2str(istatus));
		ipadm_free_addr_info(downaddrs);
		exit(1);
	}
...
fail:
	/*
	 * Attempt to bring back up any addresses that we downed.
	 */
	istatus = ipadm_up_addrs(iph, downaddrs);
	if (istatus != IPADM_SUCCESS) {
		(void) fprintf(stderr, "ifconfig: %s: cannot bring back up "
...
modify_tun(iptun_params_t *params)
{
	...
	if ((status = ifconfig_dladm_open(name, DATALINK_CLASS_IPTUN,
	    &params->iptun_param_linkid)) == DLADM_STATUS_OK)
		status = dladm_iptun_modify(dlh, params, dladm_flags);
	if (status != DLADM_STATUS_OK)
		dladmerr_exit(status, name);
	return (0);
}
...
{
	return (set_tun_prop(LINKPROP_ENCAPLIMIT, arg));
}
...
{
	return (set_tun_prop(LINKPROP_HOPLIMIT, arg));
}
...
setifsrc(char *arg, int64_t param)
{
	...
	return (setifprop(IFPROP_USESRC, arg));
}
...
tun_status(datalink_id_t linkid)
{
	...
	if (dladm_get_linkprop(dlh, linkid, DLADM_PROP_VAL_CURRENT,
	    LINKPROP_HOPLIMIT, (char **)valptr, &valcnt) == DLADM_STATUS_OK) {
		(void) printf("\ttunnel hop limit %s ", propval);
		tabbed = _B_TRUE;
	}

	if (dladm_get_linkprop(dlh, linkid, DLADM_PROP_VAL_CURRENT,
	    LINKPROP_ENCAPLIMIT, (char **)valptr, &valcnt) == DLADM_STATUS_OK) {
		uint32_t elim;
...
/*
 * Print an IPv6 interface's token, if it has one.
 */
static void
print_iftoken(const char *ifname, const char *format)
{
	char			abuf[INET6_ADDRSTRLEN];
	struct sockaddr_in6	*sin6 = (struct sockaddr_in6 *)&lifr.lifr_addr;
	int			*prefixlen = &lifr.lifr_addrlen;

	if (ipadm_info.ii_aobjname[0] != '\0') {
		ipadm_status_t istatus;

		*prefixlen = 0;
		istatus = ipadm_get_interface_id(iph, ipadm_info.ii_aobjname,
		    sin6, prefixlen, IPADM_OPT_ACTIVE);
		if (istatus != IPADM_SUCCESS &&
		    istatus != IPADM_OBJ_NOTFOUND) {
			(void) memset(&lifr.lifr_addr, 0,
			    sizeof (lifr.lifr_addr));
			return;
		}
	...
	(void) strncpy(lifr.lifr_name, ifname,
	    sizeof (lifr.lifr_name));
	if (ioctl(s, SIOCGLIFTOKEN, (caddr_t)&lifr) < 0) {
		if (errno != EADDRNOTAVAIL && errno != EINVAL)
			Perror0_exit("print_iftoken: SIOCGLIFTOKEN");
		(void) memset(&lifr.lifr_addr, 0,
		    sizeof (lifr.lifr_addr));
		return;
	}
printit:
	(void) printf(format, inet_ntop(AF_INET6, (void *)&sin6->sin6_addr,
...
inetunplumb(char *arg, int64_t param)
{
	...
	if (ipadm_is_vni(name)) {
		istatus = ipadm_delete_vni(iph, name, afp->af_af,
		    ipadm_flags);
	} else if (ipadm_is_loopback(name)) {
		istatus = ipadm_delete_loopback(iph, name, afp->af_af,
		    ipadm_flags);
	} else if (ipadm_is_ipmp(iph, name)) {
		istatus = ipadm_delete_ipmp(iph, name, afp->af_af,
		    ipadm_flags);
	} else {
		istatus = ipadm_delete_ip(iph, name, afp->af_af,
		    ipadm_flags);
	}
	if (istatus != IPADM_SUCCESS) {
...
/*
 * Create the interface in `name', using ipadm_create_ip() or
 * ipadm_create_vni() or ipadm_create_loopback(), based on the class of
 * the interface.
 */
/* ARGSUSED */
static int
inetplumb(char *arg, int64_t param)
{
	...
	if (ipadm_is_vni(name)) {
		istatus = ipadm_create_vni(iph, name, afp->af_af, ipadm_flags);
	} else if (ipadm_is_loopback(name)) {
		istatus = ipadm_create_loopback(iph, name, afp->af_af,
		    ipadm_flags);
	} else {
		istatus = ipadm_create_ip(iph, name, afp->af_af, ipadm_flags);
	}
	if (istatus != IPADM_SUCCESS) {
 ...
 * underlying interface in a group. If the IPMP service is non-existent
 * (e.g., S10C), then ifconfig might need to start in.mpathd.
 */
static int
create_ipmp(const char *grname, int af, const char *ifname, boolean_t implicit)
{
	static int ipmp_daemon_started;
	uint32_t flags = ipadm_flags;
	ipadm_status_t istatus;
	int retval;
	...
	/*
	 * ipadm_create_ipmp() creates the IPMP interface and fills in the
	...
	istatus = ipadm_create_ipmp(iph, lifr.lifr_name, af, flags);
	if (istatus != IPADM_SUCCESS) {
	...
	/*
	 * If the caller requested a different group name, set it using
	 * ipadm_set_ifprop().
	 */
	if (strcmp(lifr.lifr_name, grname) != 0) {
		istatus = ipadm_set_ifprop(iph, lifr.lifr_name, "group", grname,
		    MOD_PROTO_IP, ipadm_flags);
		if (istatus != IPADM_SUCCESS) {
			(void) fprintf(stderr, "ifconfig: Error setting IPMP "
			    "groupname %s: %s\n", grname,
			    ipadm_status2str(istatus));
			return (-1);
		}
	...
	retval = ipmp_is_svc_enabled();
	if (retval != IPMP_SUCCESS) {
		/*
		 * Special case needed if the IPMP service does not exist.
		 */
		if (retval == IPMP_ENOSVC) {
			if (ipmp_daemon_started++ == 0)
				start_ipmp_daemon();
		} else {
			(void) fprintf(stderr, "ifconfig: WARNING: the IP "
			    "multipathing service is not enabled.\nIP "
			    "multipathing failure and repair detection is "
			    "inoperable, see in.mpathd(1M)\n");
		}
	}
	return (0);
}
...
/*
 * Start in.mpathd if it's not already running. Note that this should
 * only be done if there is no IPMP service.
 */
static void
...
setifdhcp(const char *caller, const char *ifname, int argc, char *argv[])
{
	...
	for (argv++; --argc > 0; argv++) {
		...
		/*
		 * Setting "reqhost" is only supported with the '-P' flag.
		 */
		if (persist_flag && strcmp(*argv, "reqhost") == 0) {
			if (--argc <= 0) {
				usage();
				return (DHCP_EXIT_BADARGS);
			}
	...
	if (af != AF_INET || type == DHCP_STATUS || type == DHCP_PING ||
	    ((type == DHCP_DROP || type == DHCP_RELEASE ||
	    type == DHCP_INFORM) && ipadm_info.ii_aobjname[0] == '\0')) {
		goto legacy;
	}
	switch (type) {
	case DHCP_RELEASE:
	...
	case DHCP_DROP:
		if (ipadm_info.ii_atype != IPADM_ADDR_DHCP) {
			(void) fprintf(stderr, "ifconfig: %s\n",
			    dhcp_ipc_strerror(DHCP_IPC_E_UNKIF));
			exit(1);
		}
		istatus = ipadm_delete_addr(iph, ipadm_info.ii_aobjname, flags);
		break;
	case DHCP_START:
		istatus = ipadm_create_addrobj(IPADM_ADDR_DHCP, ifname,
		    &ipaddr);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "setifdhcp");
		if (is_primary) {
			istatus = ipadm_set_primary(ipaddr, _B_TRUE);
			if (istatus != IPADM_SUCCESS)
				ipadmerr_exit(istatus, "setifdhcp");
		}
		if (reqhost != NULL) {
			istatus = ipadm_set_reqhost(ipaddr, reqhost);
			if (istatus != IPADM_SUCCESS)
				ipadmerr_exit(istatus, "setifdhcp");
		}
		if (timeout >= 0)
			istatus = ipadm_set_wait_time(ipaddr, timeout);
		else if (timeout == DHCP_IPC_WAIT_FOREVER)
			istatus = ipadm_set_wait_time(ipaddr,
			    IPADM_DHCP_WAIT_FOREVER);
		if (istatus != IPADM_SUCCESS)
			ipadmerr_exit(istatus, "setifdhcp");
		istatus = ipadm_create_addr(iph, ipaddr, flags);
		(void) ipadm_get_aobjname(ipaddr, ipadm_info.ii_aobjname,
		    sizeof (ipadm_info.ii_aobjname));
		ipadm_destroy_addrobj(ipaddr);
		ipaddr = NULL;
		break;
	case DHCP_INFORM:
		istatus = ipadm_refresh_addr(iph, ipadm_info.ii_aobjname,
		    flags | IPADM_OPT_INFORM);
		break;
	default:
		(void) fprintf(stderr, "ifconfig: invalid dhcp option\n");
		exit(1);
	}
	if (istatus != IPADM_SUCCESS)
		ipadmerr_exit(istatus, "setifdhcp");

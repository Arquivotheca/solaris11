    ...
    int flags, ilbadm_key_code_t keyword)
{
	 ...
	 * IPv6 addresses before we can go search for the port separator
...
ports:
	if (port_pref != NULL && is_ports_ok) {
		...
		*port_pref = '\0';
		...
		/*
		 * we fill the '-' back in, but not the port separator,
    ...
    int flags, int *count)
{
	 ...
	 * 2. if we're expecting a list of values (separated by commas)

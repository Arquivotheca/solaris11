...
main(int argc, char *argv[])
{
	...
	if (family_input == AF_INET6 ||
		...
		if (datalen > (IP_MAXPACKET - (sizeof (struct ip6_hdr) +
			...
			/*
			 * In case there is an IPv4 interface, ensure
			 * that the packet size, datalen, is also
			 * checked else ping may segfault in pinger().
			 */
			if (num_v4 != 0)
				family_input = AF_INET;
		}
    ...
    int family)
{
	...
	/*
	 * This loop can lead to a segfault if an invalid packet size is
	 * specified, however, this shouldn't happen because both IPv6
	 * and IPv4 packet size checks are performed in main().
	 */

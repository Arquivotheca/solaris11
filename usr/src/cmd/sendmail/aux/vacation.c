/*
 * Copyright (c) 1985, 2011, Oracle and/or its affiliates. All rights reserved.
 *
...
#define	MAXLINE	2048	/* max size of a line */
	...
	char *from;
{
	register char *p;
	char buf[MAXLINE+1];
	bool inside, onlist, saw_newline, prev_newline, first_pass, skip;
...
#define	Delims " \n\t:,:;()<>@!"
	...
	while (fgets(buf, MAXLINE, stdin) != NULL)
	{
		prev_newline = saw_newline;
		if (buf[0] == '\n')
		{
			/*
			 * We want to break when we hit the blank line that
			 * separates the header from the body.  But if we
			 * hit a long line with no \n and the first character
			 * of the next line is the \n corresponding to the
			 * end of that long line, then we want to continue.
			 */
			if (prev_newline)
				break;
			else
		...
		saw_newline = (strchr(buf, '\n') != NULL);
		/*
		 * If there was no \n in the previous buffer, then this must be
		 * a continuation buffer, in which case none of the tests below
		 * are applicable (i.e., really long headers are a sign of bulk
		 * mail so we don't want to trigger a reply in that case).
		 * Exception for the first pass thru where there was no
		 * previous buffer.
		 */
		skip = (!prev_newline && !first_pass);
		first_pass = FALSE;
		if (skip)
			continue;
		if (buf[0] != ' ' && buf[0] != '\t' && strchr(buf, ':') == NULL)
			continue;			/* no header found */

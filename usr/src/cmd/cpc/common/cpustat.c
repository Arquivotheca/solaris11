...
/*
 * Barrier variables to coordinate sample thread start-up.
 */
static mutex_t		barrier_m;
static cond_t		barrier_cv;
static int		barrier_cnt;
static struct timespec	barrier_busywait = { 0, 1000 };
...
gtick(void *arg)
{
	...
	sgrp = state->sgrp = cpc_setgrp_clone(opts->master);

	if (state->sgrp == NULL)
		return (NULL);

	curset = cpc_setgrp_getset(sgrp);
	...
	/*
	 * Once this thread is initialized, acquire the barrier lock,
	 * bump up the barrier count and then wait for the master thread
	 * to signal 'go time'.
	 */
	(void) mutex_lock(&barrier_m);
	barrier_cnt++;
	(void) cond_wait(&barrier_cv, &barrier_m);
	(void) mutex_unlock(&barrier_m);

	/*
	 * bflags is the flags argument of cpc_bind_cpu() used in the
	 * sampling loop.  It's value is loop invariant until the final
	 * iteration of the loop where the NORELE bit is cleared to
	 * to allow cpc_unbind() to unbind the thread from the CPU.
	 */
	bflags = CPC_FLAGS_NOPBIND | CPC_FLAGS_NORELE;

	if (cpc_bind_cpu(cpc, state->cpuid, curset, CPC_FLAGS_NORELE) == -1)
		goto bad;

	/*
	 * Sample high resolution time for the beginning of our current
	 * sampling interval.
	 */
	ht = gethrtime();
	...
	for (nsamples = opts->nsamples; nsamples; nsamples--, sample_cnt++) {
		...
		if (opts->nsets == 1) {
			...
			*data1 = *data2;
			*data2 = tmp;
			if (cpc_set_sample(cpc, curset, *data1) != 0)
				goto bad;
			...
			/*
			 * On the last iteration of the loop, clear
			 * CPC_FLAGS_NORELE so that cpc_unbind()
			 * releases the thread/CPU binding as
			 * expected.
			 */
			if ((nsamples - 1) == 0)
				bflags ^= CPC_FLAGS_NORELE;

			if (cpc_bind_cpu(cpc, state->cpuid, curset, bflags))
				goto bad;
		}
	}
...
cpustat(void)
{
	...
	(void) mutex_init(&barrier_m, USYNC_THREAD, NULL);
	(void) cond_init(&barrier_cv, USYNC_THREAD, NULL);
	barrier_cnt = 0;
	...
	if (pset_bind(PS_QUERY, P_PID, P_MYID, &mypset) == -1) {
		...
		for (i = 0; i < ncpus; i++) {
			struct tstate *tstate = &gstate[i];

			if (tstate->cpuid == -1)
				continue;

			if (pset_assign(PS_QUERY,
			    tstate->cpuid, &cpupset) == -1) {
				errstr = strerror(errno);
				(void) fprintf(stderr,
	...
	for (i = 0; i < ncpus; i++) {
		...
		if (thr_create(NULL, 0, gtick, tstate,
		    THR_BOUND|THR_NEW_LWP, &tstate->tid) == 0)
			lwps++;
		else {
			(void) fprintf(stderr,
			    gettext("%s: cannot create thread for cpu%d\n"),
			    opts->pgmname, tstate->cpuid);
			tstate->status = 4;
		}
	}

	/*
	 * The master thread waits for all the sampling threads to
	 * initialize themselves.  When all threads check in via
	 * barrier_cnt, signal them to start sampling counters.
	 */
	(void) mutex_lock(&barrier_m);
	while (barrier_cnt < lwps) {
		(void) mutex_unlock(&barrier_m);
		(void) nanosleep(&barrier_busywait, NULL);
		(void) mutex_lock(&barrier_m);
	}
	(void) mutex_unlock(&barrier_m);

	zerotime();		/* mark the beginning of the sampling epoch */

	(void) cond_broadcast(&barrier_cv);
	...
	for (i = 0; i < ncpus; i++) {
		...
		if (tstate->cpuid == -1)
			continue;
		cpc_setgrp_accum(accum, tstate->sgrp);
		cpc_setgrp_free(tstate->sgrp);
		tstate->sgrp = NULL;
		if (tstate->status != 0)
			retval = 1;
	}

...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	if (oflag) {
		if (tempfd == -1) {
			...
			if (tempfd == -1) {
				error(gettext(
				    "Cannot open/create temp file %s"), temp);
				free(temp);
				temp = 0;
			}
		}
		...
		if ((lfd = mkstemp(ltemp)) == -1 ||
			...
			error(gettext("Cannot open/create temp file %s"),
			    ltemp);
	...
	/* Process diff cmd */
		...
		if (oflag == 1 && com != 0) {
			...
			if ((left = fopen(ltemp, "w")) == NULL)
				error(gettext(
				    "main: Cannot open temp file %s"), ltemp);
			if ((right = fopen(rtemp, "w")) == NULL)
				error(gettext(
				    "main: Cannot open temp file %s"), rtemp);
		}
	}
...
cmdin(void)
{
	...
	while (notacc) {
		...
		default:
			(void) fprintf(stderr,
			    gettext("Illegal command reenter\n"));
			break;
		}
	}
}
...
cpp(char *from, FILE *fromdes, FILE *todes)
{
	...
	if ((fromdes = fopen(from, "r")) == NULL)
		error(gettext("cpp: Cannot open %s"), from);
...
filename(char *pa1, char *pa2)
{
	...
	if (stat(a1, &stbuf) != -1 && ((stbuf.st_mode&S_IFMT) == S_IFDIR)) {
		...
		if ((tempfd = mkstemp(temp)) == -1 ||
			...
			error(gettext("Cannot open/create temp file %s"), temp);

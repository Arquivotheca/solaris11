...
beg:
	...
	case '"':
		...
		yylval = (int)&token[0];
		if (c == '\n')
			error(NONFATAL, gettext("missing \" in %.20s"), token);
		return (QTEXT);
	}
	if (c == righteq)
		return (EOF);
	...
	if ((tp = lookup(deftbl, token, NULL)) != NULL) {
		...
		if (dbg)
			(void) printf(".\t%s is not a keyword\n", token);
		return (CONTIG);
...
define(int type)
{
	...
	p2 = strsave(token);
	(void) lookup(deftbl, p1, p2);
	if (dbg)
		(void) printf(".\tname %s defined as %s\n", p1, p2);
}
...
space(void) /* collect line of form "space amt" to replace \x in output */
{
	...
	if (dbg)
		(void) printf(".\tsetting space to %s\n", token);
}
...
include(void)
{
	error(NONFATAL, gettext("Include not yet implemented"));
}
...
bad_str(void)
{
	error(FATAL, gettext("Unexpected string value found\n"));

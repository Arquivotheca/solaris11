...
/*
 * get_running_zones() fetches the list of running zones, then looks
 * up the name of each that it found.  It returns only those zones which
 * it can find names for and which are in the RUNNING state.
 */
int
get_running_zones(uint_t *nzents, zone_entry_t **zents)
{
	...
	if (zone_get_zoneids(&zids, nzents) != 0) {
		...
		warn(gettext("zones will not be capped\n"));
		return (E_ERROR);
	}

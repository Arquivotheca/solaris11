...
static int	rt_priocntlset(procset_t *, int, char *, uintptr_t *);
	...
	for (i = 0; i < numread; i++) {
		...
		if (priocntlset(&pset, PC_GETXPARMS, "RT",
			...
			if (priocntlset(&pset, PC_GETXPARMS, NULL,
				 ...
				 * It has probably changed class while
				 * priocntl(1) command was executing (otherwise
				...
				(void) printf("\tCan't get RT user priority\n");
		}
	}
 ...
 */
static int
rt_priocntlset(procset_t *pset, int cmd, char *clname, uintptr_t *argsp)
{
    ...
    pri_t rtpri, long tqntm, long res, int rtqsig)
{
	...
	(void) strcpy(pcinfo.pc_clname, "RT");
	if (priocntl(0, 0, PC_GETCID, (caddr_t)&pcinfo) == -1)
		fatalerr("%s: Can't get RT class ID, priocntl(2) system call"
		    " failed with errno %d\n", basenm, errno);
	...
	*argsp = 0;
	...
	if (idtype == P_ALL) {
		...
		if (rt_priocntlset(&pset, PC_SETXPARMS, "RT", args) == -1) {
			if (errno == EPERM) {
				(void) fprintf(stderr,
				...
				fatalerr("%s: Can't reset real time parameters"
				    "\npriocntl(2) system call failed with"
				    " errno %d\n", basenm, errno);
			}
		}
	} else if (idargc == 0) {
		...
		for (i = 0; i < idargc; i++) {
			...
			if (rt_priocntlset(&pset, PC_SETXPARMS, "RT", args) ==
			    -1) {
				if (errno == EPERM) {
					(void) fprintf(stderr,

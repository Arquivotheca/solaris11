...
static int fx_priocntlset(procset_t *, int, char *, uintptr_t *);
...
print_fxprocs(void)
{
	...
	(void) printf("%s\n %11s %15s %10s %11s\n",
	    ...
	    "PID[/LWP]", "FXUPRILIM", "FXUPRI", "FXTQNTM");
	...
	for (i = 0; i < numread; i++) {
		...
		/*
		 * Set the procset according to whether this id contains a
		 * LWP or not.
		 */
		if (*item == '/') {
			item++;
			...
			setprocset(&pset, POP_AND, P_PID, pid, P_LWPID, lwp);
			(void) printf("%7ld/%-7ld", pid, lwp);
		} else {
			setprocset(&pset, POP_AND, P_PID, pid, P_ALL, 0);
			(void) printf("%7ld %-7s", pid, " ");
		}

		if (priocntlset(&pset, PC_GETXPARMS, "FX",
		    FX_KY_UPRI, &fx_upri, FX_KY_UPRILIM, &fx_uprilim,
		    FX_KY_TQSECS, &fx_tqsecs, FX_KY_TQNSECS,
		    &fx_tqnsecs, 0) != -1) {
			(void) printf("%9d %12d", fx_uprilim, fx_upri);
			...
			if (priocntlset(&pset, PC_GETXPARMS, NULL,
			    PC_KY_CLNAME, clname, 0) != -1 &&
			    strcmp(clname, "FX"))
				/*
				 * Process from some class other than fixed
				 * priority. It has probably changed class
				 * while priocntl(1) command was executing
				 * (otherwise we wouldn't have been passed its
				 * pid). Print the little we know about it.
				 */
				(void) printf("\tChanged to class %s while"
				    " priocntl(1) command executing\n", clname);
			else
 ...
 */
static int
fx_priocntlset(procset_t *pset, int cmd, char *clname, uintptr_t *argsp)
{
	return (priocntlset(pset, cmd, clname, argsp[0], argsp[1],
    ...
    pri_t fxuprilim, pri_t fxupri, long tqntm, long res)
{
	...
	pri_t		maxupri;
	char		idtypnm[PC_IDTYPNMSZ], *item;
	int		i;
	id_t		id, lwp;
	hrtimer_t	hrtime;
	int 		error = 0;
	...
	(void) strcpy(pcinfo.pc_clname, "FX");
	if (priocntl(0, 0, PC_GETCID, (caddr_t)&pcinfo) == -1)
		fatalerr("%s: Can't get FX class ID, priocntl(2) system call"
		    " failed with errno %d\n", basenm, errno);
	...
	*argsp = 0;
	...
	if (idtype == P_ALL) {
		setprocset(&pset, POP_AND, P_ALL, 0, P_ALL, 0);

		if (fx_priocntlset(&pset, PC_SETXPARMS, "FX", args) == -1) {
			if (errno == EPERM) {
				(void) fprintf(stderr,
			...
			if ((cflags & (FX_DOUPRILIM|FX_DOUPRI)) == FX_DOUPRI) {
				(void) verifyupri(idtype, 0, "FX",
				    FX_KY_UPRILIM, fxupri, basenm);
			}
		}
	} else if (idargc == 0) {
		setprocset(&pset, POP_AND, idtype, P_MYID, P_ALL, 0);
		...
		for (i = 0; i < idargc; i++) {
			...
			case P_CID:
				...
				if (priocntl(0, 0, PC_GETCID,
					...
					fatalerr("%s: Invalid or unconfigured"
					    " class %s, priocntl(2) system call"
					    " failed with errno %d\n",
					    basenm, pcinfo.pc_clname, errno);
				id = pcinfo.pc_cid;

				setprocset(&pset, POP_AND, P_CID, id, P_ALL, 0);
				break;

			case P_PID:
				item = (char *)idargv[i];
				id = (id_t)strtol(item, &item, 10);

				/*
				 * Set the procset according to whether this id
				 * contains a LWP or not.
				 */
				if (*item == '/') {
					item++;
					lwp = (id_t)strtol(item, &item, 10);

					setprocset(&pset, POP_AND, P_PID, id,
					    P_LWPID, lwp);
				} else {
					setprocset(&pset, POP_AND, P_PID, id,
					    P_ALL, 0);
			...
			default:
				...
				setprocset(&pset, POP_AND, idtype, id,
				    P_ALL, 0);
				break;
			}

			if (fx_priocntlset(&pset, PC_SETXPARMS, "FX", args)
			    == -1) {
				if (errno == EPERM) {
					(void) fprintf(stderr,
				...
				if ((cflags & (FX_DOUPRILIM|FX_DOUPRI)) ==
				    FX_DOUPRI) {
					(void) verifyupri(idtype, id, "FX",
					    FX_KY_UPRILIM, fxupri, basenm);
				}
    ...
    long tqntm, long res)
{
	...
	*argsp = 0;
	...
	setprocset(&pset, POP_AND, P_PID, P_MYID, P_ALL, 0);

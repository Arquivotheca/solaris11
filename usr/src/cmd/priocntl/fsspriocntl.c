...
static int fss_priocntlset(procset_t *, int, char *, uintptr_t *);
...
print_fssprocs(void)
{
	...
	for (i = 0; i < numread; i++) {
		...
		if (priocntlset(&pset, PC_GETXPARMS, "FSS",
		    FSS_KY_UPRI, &fssupri,
			...
			if (priocntlset(&pset, PC_GETXPARMS, NULL,
				...
				(void) printf("\tCan't get FSS user"
				    " priority\n");
		}
	}
 ...
 */
static int
fss_priocntlset(procset_t *pset, int cmd, char *clname, uintptr_t *argsp)
{
    ...
    short fssuprilim, short fssupri)
{
	...
	(void) strcpy(pcinfo.pc_clname, "FSS");
	if (priocntl(0, 0, PC_GETCID, (caddr_t)&pcinfo) == -1)
		fatalerr("%s: Can't get FSS class ID, priocntl(2) system call"
	...
	*argsp = 0;
	...
	if (idtype == P_ALL) {
		...
		if (fss_priocntlset(&pset, PC_SETXPARMS, "FSS", args) == -1) {
			if (errno == EPERM) {
			...
			if ((cflags & (FSS_DOUPRILIM|FSS_DOUPRI)) ==
			    FSS_DOUPRI) {
				(void) verifyupri(idtype, 0, "FSS",
		...
		for (i = 0; i < idargc; i++) {
			...
			if (fss_priocntlset(&pset, PC_SETXPARMS, "FSS", args) ==
			    -1) {
				if (errno == EPERM) {
					(void) fprintf(stderr,
	...
	*argsp = 0;
	...
	if (fss_priocntlset(&pset, PC_SETXPARMS, "FSS", args) == -1)
		fatalerr("%s: Can't reset fair sharing parameters\n"
		    "priocntl(2) system call failed (%s)\n",
		    basenm, strerror(errno));

...
static int	ia_priocntlset(procset_t *, int, char *, uintptr_t *);
 ...
 */
static int
ia_priocntlset(procset_t *pset, int cmd, char *clname, uintptr_t *argsp)
{
    ...
    pri_t iauprilim, pri_t iaupri, int iamode)
{
	...
	(void) strcpy(pcinfo.pc_clname, "IA");
	if (priocntl(0, 0, PC_GETCID, (caddr_t)&pcinfo) == -1)
		fatalerr("%s: Can't get IA class ID, priocntl(2) system call"
		    " failed with errno %d\n", basenm, errno);
	...
	*argsp = 0;
	...
	if (idtype == P_ALL) {
		...
		if (ia_priocntlset(&pset, PC_SETXPARMS, "IA", args) == -1) {
			if (errno == EPERM) {
				(void) fprintf(stderr,
		...
		for (i = 0; i < idargc; i++) {
			...
			if (ia_priocntlset(&pset, PC_SETXPARMS, "IA",
			    args) == -1) {
				if (errno == EPERM) {
					(void) fprintf(stderr,
					    "Permissions error"
				...
				if ((cflags & (IA_DOUPRILIM|IA_DOUPRI)) ==
				    IA_DOUPRI) {
					(void) verifyupri(idtype, id, "IA",
					    IA_KY_UPRILIM, iaupri, basenm);
				}

...
static int	ts_priocntlset(procset_t *, int, char *, uintptr_t *);
...
print_tsprocs(void)
{
	...
	for (i = 0; i < numread; i++) {
		...
		if (priocntlset(&pset, PC_GETXPARMS, "TS", TS_KY_UPRI,
 ...
 */
static int
ts_priocntlset(procset_t *pset, int cmd, char *clname, uintptr_t *argsp)
{
    ...
    pri_t tsuprilim, pri_t tsupri)
{
	...
	(void) strcpy(pcinfo.pc_clname, "TS");
	if (priocntl(0, 0, PC_GETCID, (caddr_t)&pcinfo) == -1)
		fatalerr("%s: Can't get TS class ID, priocntl(2) system call"
		    " failed with errno %d\n", basenm, errno);
	...
	*argsp = 0;
	...
	if (idtype == P_ALL) {
		...
		if (ts_priocntlset(&pset, PC_SETXPARMS, "TS", args) == -1) {
			if (errno == EPERM) {
				(void) fprintf(stderr,
		...
		for (i = 0; i < idargc; i++) {
			...
			if (ts_priocntlset(&pset, PC_SETXPARMS, "TS", args) ==
			    -1) {
				if (errno == EPERM) {
					(void) fprintf(stderr,
				...
				if ((cflags & (TS_DOUPRILIM|TS_DOUPRI)) ==
				    TS_DOUPRI) {
					(void) verifyupri(idtype, id, "TS",
					    TS_KY_UPRILIM, tsupri, basenm);
				}

...
get_bootadm_value(char *name, const int quiet)
{
	...
	if (strcmp(name, "boot-file") == 0) {
		is_kernel = 1;
		ptr = "/usr/sbin/bootadm set-menu kernel 2>&1";
	} else if (is_console || (strcmp(name, "boot-args") == 0)) {
		ptr = "/usr/sbin/bootadm set-menu args 2>&1";
	} else {
...
set_bootadm_var(char *name, char *value)
{
	...
	is_console = (strcmp(name, "console") == 0);
	if (strcmp(name, "boot-file") == 0) {
		(void) snprintf(buf, BUFSIZ, "/usr/sbin/bootadm set-menu "
		    "kernel='%s' 2>&1", value);
	} else if (is_console || (strcmp(name, "boot-args") == 0)) {
		if (is_console) {
		...
		if (((args == NULL) || (args[0] == '\0')) &&
			...
			(void) snprintf(buf, BUFSIZ, "/usr/sbin/bootadm "
			    "set-menu args= 2>&1");

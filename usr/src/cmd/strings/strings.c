...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	/*
	 * Check for non-standard "-" and "-number" command line options.
	 * Repeat this operation in case the user has supplied two or more
	 * of them.
	 */
	while (check_nonstd) {
		check_nonstd = 0;
		for (i = 1; i < argc; i++) {
			if (strcmp(argv[i], "-") == 0) {
				...
				errno = 0;
				minlength = strtol(&argv[i][1], &p, 10);
	...
	while ((opt = getopt(argc, argv, "an:N:ot:")) != -1) {
		switch (opt) {
			case 'a':
				aflg++;
				break;
...
long	*cnt;
{
	...
	r_val = 0;
	mbuf[0] = c;
	for (len = i = 1; *cnt > 0 && i < MB_CUR_MAX; i++) {
		int	nc = getc(stdin);
...
_unget:
	for (len--; len >= len1; len--) {
		(void) ungetc((unsigned char)mbuf[len], stdin);
		(*cnt)++;
	}
	return (r_val);
}

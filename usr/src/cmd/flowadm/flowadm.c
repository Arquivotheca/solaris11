...
usage(void)
{
	(void) fprintf(stderr, gettext("usage: flowadm <subcommand>"
	    ...
	    "-a <attr>=<value>[,...]\n"
	    "\t\t   [-p <prop>=<value>,...] <flow>\n"
	    "    remove-flow    [-t] [-R <root-dir>] {-l <link> | <flow>}\n"
	    "    show-flow      [-P] [[-p] -o <field>,...] [-l <link>] "
	    "[<flow>]\n\n"
	    "    set-flowprop   [-t] [-R <root-dir>] -p <prop>=<value>[,...] "
	    "<flow>\n"
	    "    reset-flowprop [-t] [-R <root-dir>] [-p <prop>,...] <flow>\n"
	    "    show-flowprop  [-P] [[-c] -o <field>,...] [-l <link>] "
	    "[-p <prop>,...]\n"
	    "\t\t   [<flow>]\n"));
    ...
    flow_fields_buf_t *fbuf)
{
	...
	if (attr->fa_linkid != DATALINK_INVALID_LINKID) {
		status = dladm_datalink_id2info(handle, attr->fa_linkid, NULL,
		    NULL, NULL, link, sizeof (link));
		if (status != DLADM_STATUS_OK)
			return (status);
	} else {
		/* missing link */
...
set_flowprop_persist(const char *flow, const char *prop_name, char **prop_val,
    uint_t val_cnt, boolean_t reset, const char *root)
{
	dladm_status_t	status;
	char		*errprop;

	status = dladm_set_flowprop(handle, flow, prop_name, prop_val, val_cnt,
	    DLADM_OPT_PERSIST, &errprop, root);
...
set_flowprop(int argc, char **argv, boolean_t reset)
{
	...
	if (proplist == NULL) {
		...
		status = dladm_set_flowprop(handle, flow, NULL, NULL, 0,
		    DLADM_OPT_ACTIVE, &errprop, altroot);
		if (status != DLADM_STATUS_OK) {
			if (errprop != NULL)
	...
	for (i = 0; i < proplist->al_count; i++) {
		...
		s = dladm_set_flowprop(handle, flow, aip->ai_name, val, count,
		    DLADM_OPT_ACTIVE, NULL, altroot);
		if (s == DLADM_STATUS_OK) {
			if (!temp) {
				s = set_flowprop_persist(flow,
				    aip->ai_name, val, count, reset, altroot);
				if (s != DLADM_STATUS_OK)
					status = s;
			}
			continue;
...
{
	show_flowprop_state_t	*state = arg;

	(void) dladm_walk_flow(show_flowprop, dh, linkid, arg,
	    state->fs_persist);
...
show_flowprop_one_flow(void *arg, const char *flow)
{
	...
	/* get flow info from persistent */
	if (statep->fs_persist) {
		status = dladm_get_persist_flow(handle, flow, &attr, &ret_err);
	} else {
		status = dladm_flow_info(handle, flow, &attr);
	}

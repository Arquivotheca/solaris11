 ...
 * main()
 *
 *	The devchassis daemon is designed to be a background daemon
 *	controlled by SMF.  So by default it will daemonize and
 *	do some coordination with its parent process in order to
 *	indicate proper success or failure back to SMF.  When daemonized
 *	all output will be sent to syslog.
 *
 *	For debug, the '-d' command line option will run devchassisd
 *	in the foreground in standalone, debug mode.  In this mode,
 *	errors and additional debug messages will be printed to the
 *	controlling terminal instead of to syslog.
 */
int
main(int argc, char *argv[])
{
	...
	/* Check privileges */
	if (!check_privileges()) {
		log_err("Insufficient privileges.\n");
		return (-1);
	}

	/* Process options (-V is private) */
	...
	if (!debug_flag)
		pfd = daemonize();	/* child deamon returns */

	/* Initialize chassis-receptacal-occupant sysevent receiver */
	if (cro_sysevent_receive_setup()) {
		if (!debug_flag) {
			/* Communicate child failure to parent via write. */
			status = EXIT_FAILURE;
	...
	/* Daemon initialized */
	if (!debug_flag) {
		/* Communicate child success to parent via write. */
		status = 0;
		(void) write(pfd, &status, sizeof (status));
		(void) close(pfd);
	}
	...
	/*
	 * Wait for shutdown signal... while running, daemon processing
	 * occurs off cro_sysevent_recieve_handler().
	 */
	log_info("devchassis daemon started.\n");
	while (!exit_flag)
 ...
 * check_privileges()
 *
 *	Check if the current process has enough privileges to run
 *	devchassisd daemon (should agree with devchassis.xml).
 */
static boolean_t
check_privileges(void)
{
	...
	if ((privset = priv_allocset()) != NULL) {
		if ((getppriv(PRIV_EFFECTIVE, privset) == 0) &&
		    priv_ismember(privset, PRIV_SYS_DEVICES) &&
		    priv_ismember(privset, PRIV_SYS_CONFIG))
			rv = B_TRUE;
 ...
 * signal_handler()
 *
 *	Most signals cause the devchassis daemon to shut down.
...
{
	log_info("devchassis daemon shutting down.\n");

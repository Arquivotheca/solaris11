...
static void	wait_term();			/* wait for audit service off */
...
sig_auditd(int sig)
{
	...
	if (strcmp(SCF_STATE_STRING_ONLINE, state) != 0) {
		(void) fprintf(stderr,
		    gettext("audit: audit service is not enabled\n"));
		free(state);
		return (1);
	}
	free(state);
...
is_audit_config_ok() {
	...
	while (plugin_kva_ll != NULL) {
		...
		if (strcmp((char *)&(*plugin_kva_ll).plugin_name,
			...
			cval_str = kva_match(kvlist, "p_fsize");
			if (!__audit_hrstrtonum(cval_str, &p_fsize)) {
				(void) fprintf(stderr,
				    gettext("%s: audit_binfile(5) \"p_fsize:\" "
				    "attribute invalid\n"), progname);
				state = B_FALSE;
			}
		}
...
start_auditd()
{
	...
	if (strcmp(SCF_STATE_STRING_MAINT, state) == 0) {
		(void) fprintf(stderr, gettext("Could not enable/refresh "
		    "the audit service instance.\nSee 'svcs -xv %s' for more "
		    "information.\n"), AUDITD_FMRI);
		free(state);
		exit(1);
	}
...
{
	char *state;
	boolean_t	inst_enabled;
	scf_propvec_t	prop_vec[2];
	scf_propvec_t	*bad_prop_vec = NULL;
	timespec_t	sleep_spec;
	int		iter;

	sleep_spec.tv_sec = 0;
	sleep_spec.tv_nsec = NANOSEC / 2;	/* 2 times per second */
	iter = 180;				/* 90 seconds upper boundary */

	(void) memset(prop_vec, 0, sizeof (prop_vec));
	prop_vec->pv_prop = SCF_PROPERTY_ENABLED;
	prop_vec->pv_type = SCF_TYPE_BOOLEAN;
	prop_vec->pv_ptr = &inst_enabled;
	...
	do {
		...
		if (strcmp(SCF_STATE_STRING_MAINT, state) == 0) {
			free(state);
			(void) fprintf(stderr, gettext("%s service instance "
			    "entered the maintenance state.\n"), AUDITD_FMRI);
			(void) fprintf(stderr, gettext("See 'svcs -xv %s' for "
			    "more details.\n"), AUDITD_FMRI);
			exit(1);
		}
		free(state);
		...
		/* check for race */
		if (scf_read_propvec(AUDITD_FMRI, SCF_PG_GENERAL, B_FALSE,
		    prop_vec, &bad_prop_vec) != SCF_SUCCESS) {
			display_smf_error();
			if (bad_prop_vec != NULL) {
				(void) fprintf(stderr, "Couldn't read the "
		...
		if (inst_enabled) {
			(void) fprintf(stderr, "%s enabled by other process!\n",
			    AUDITD_FMRI);
			exit(1);
		}

		DPRINT((dbfp, "Waiting for %s to get disabled [remaining "
		    "attempts %d].\n", AUDITD_FMRI, iter));

		if (((iter - 1) % 20) == 0) {
			(void) fprintf(stderr, gettext("Waiting for audit "
			    "service to get disabled.\n"));
		}

	} while (--iter != 0 && nanosleep(&sleep_spec, NULL) != -1);

	if (iter == 0) {
		(void) fprintf(stderr, gettext("Timed out waiting for %s to "
		    "get disabled.\n"), AUDITD_FMRI);
		exit(1);
	}
}

...
static scf_propvec_t prop_option[] = {
	{ GLOBAL_ENABLED, NULL, SCF_TYPE_BOOLEAN, B_FALSE, NULL,
	    CC_GLOBAL_PATH },
	{ PROCESS_ENABLED, NULL, SCF_TYPE_BOOLEAN, B_FALSE, NULL,
	    CC_PROCESS_PATH },
	{ GLOBAL_SETID_ENABLED, NULL, SCF_TYPE_BOOLEAN, B_FALSE, NULL,
	    CC_GLOBAL_SETID },
	{ PROCESS_SETID_ENABLED, NULL, SCF_TYPE_BOOLEAN, B_FALSE, NULL,
	    CC_PROCESS_SETID },
	{ GLOBAL_LOG_ENABLED, NULL, SCF_TYPE_BOOLEAN, B_FALSE, NULL,
	    CC_GLOBAL_LOG },
	{ NULL }
};
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "g:G:i:I:p:P:e:d:uU?")) != EOF) {
		...
		case 'g':
			...
			if (glob_pattern[0] != '/') {
				(void) fprintf(stderr, gettext(
				    "%s: global pattern must start with /.\n"),
				    command);
				return (E_ERROR);
			}
			break;
...
do_modify(boolean_t method)
{
	...
	/* Base behavior on pre-refresh state */
	if (!method)
		state = smf_get_state(COREADM_INST_FMRI);
	...
	if (state != NULL && strcmp(state, SCF_STATE_STRING_MAINT) == 0) {
		if (smf_restore_instance(COREADM_INST_FMRI) == -1) {
			(void) fprintf(stderr, gettext(
			    "%s: failed to clear maintenance from %s\n"),
			    command, COREADM_INST_FMRI);
			goto inactive;
		}
		(void) fprintf(stderr, gettext(
		    "%s: attempting to clear maintenance from %s\n"),
		    command, COREADM_INST_FMRI);
	} else if (state == NULL ||
...
inactive:
	free(state);
	(void) fprintf(stderr,
	    gettext("%s: configuration stored but not made active\n"), command);
	return (E_ERROR);
}

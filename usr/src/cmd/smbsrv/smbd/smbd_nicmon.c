...
smbd_nicmon_start(const char *svc_fmri)
{
	...
	rc = smbd_thread_create("NIC monitor", smbd_nicmon_daemon, NULL);
	if (rc != 0)
		return (-1);
...
smbd_nicmon_setup_rtsock(int af)
{
	...
	if ((sd = socket(PF_ROUTE, SOCK_RAW, af)) == -1) {
		smbd_log(LOG_ERR, "smbd_nicmon: routing socket failed: %d",
		    errno);
		return (-1);
	}

	if ((flags = fcntl(sd, F_GETFL, 0)) < 0) {
		smbd_log(LOG_ERR, "smbd_nicmon: fcntl F_GETFL failed: %d",
		    errno);
		(void) close(sd);
		return (-1);
	}
	...
	if ((fcntl(sd, F_SETFL, flags | O_NONBLOCK)) < 0) {
		smbd_log(LOG_ERR, "smbd_nicmon: fcntl F_SETFL failed: %d",
		    errno);
		(void) close(sd);
		return (-1);
	}
...
smbd_nicmon_setup_eventpipe(int *read_pipe, int *write_pipe)
{
	...
	if ((pipe(fds)) < 0) {
		smbd_log(LOG_ERR, "smbd_nicmon: event pipe failed: %d", errno);
		return (-1);
	}
...
smbd_nicmon_daemon(void *arg)
{
	...
	smbd_online_wait("smbd_nicmon_daemon");
	...
	while (smbd_online()) {
		 ...
		 */
		if (nic_changed && smbd_nicmon_caller_fmri) {

...
typedef struct spoolss_spooldoc {
	uint32_t	sd_magic;
	list_node_t	sd_lnd;
	smb_spooldoc_t	sd_doc;
...
static spoolss_spool_t spoolss_splist;
...
spoolss_initialize(void)
{
	if (!spoolss_splist.sp_initialized) {
		list_create(&spoolss_splist.sp_list,
		    sizeof (spoolss_spooldoc_t),
		    offsetof(spoolss_spooldoc_t, sd_lnd));
...
spoolss_s_OpenPrinter(void *arg, ndr_xa_t *mxa)
{
	...
	if ((pname = strdup(name)) == NULL) {
		bzero(&param->handle, sizeof (spoolss_handle_t));
		param->status = ERROR_NOT_ENOUGH_MEMORY;
		return (NDR_DRC_OK);
	}
	if ((id = ndr_hdalloc(mxa, pname)) == NULL) {
		bzero(&param->handle, sizeof (spoolss_handle_t));
		param->status = ERROR_NOT_ENOUGH_MEMORY;
		free(pname);
		return (NDR_DRC_OK);
...
spoolss_s_StartDocPrinter(void *arg, ndr_xa_t *mxa)
{
	...
	if ((spfile = calloc(1, sizeof (spoolss_spooldoc_t))) == NULL) {
		param->status = ERROR_NOT_ENOUGH_MEMORY;
		return (NDR_DRC_OK);
	}
	...
	if ((rc = smb_share_lookup(SMB_SHARE_PRINT, &si)) != NERR_Success) {
	...
	if (docinfo->doc_name != NULL)
		(void) strlcpy(spdoc->sd_docname,
		    (char *)docinfo->doc_name, MAXNAMELEN);
	else
		(void) strlcpy(spdoc->sd_docname, "document", MAXNAMELEN);

	if (docinfo->printer_name != NULL)
		(void) strlcpy(spdoc->sd_printer,
		    (char *)docinfo->printer_name, MAXPATHLEN);
	else if (hd->nh_data)
		(void) strlcpy(spdoc->sd_printer, hd->nh_data, MAXPATHLEN);
	else
		(void) strlcpy(spdoc->sd_printer, "printer", MAXPATHLEN);

	(void) memcpy(&spdoc->sd_ipaddr, &mxa->pipe->np_user.ui_ipaddr,
	    sizeof (smb_inaddr_t));
	(void) strlcpy(spdoc->sd_username,
	    mxa->pipe->np_user.ui_account, MAXNAMELEN);
	(void) memcpy(&spfile->sd_handle, &param->handle, sizeof (ndr_hdid_t));

	/*
	 * Create temporary spool file in print$
	 */
	(void) snprintf(spdoc->sd_path, MAXPATHLEN, "%s/%s%d", si.shr_path,
	    spdoc->sd_username, spoolss_cnt);
	atomic_inc_32(&spoolss_cnt);

	fd = open(spdoc->sd_path, O_CREAT | O_RDWR, 0600);
	if (fd == -1) {
		smb_tracef("spoolss_s_StartDocPrinter: %s: %s",
		    spdoc->sd_path, strerror(errno));
...
spoolss_s_GetPrinterData(void *arg, ndr_xa_t *mxa)
{
	...
	if (strcasecmp(name, "DefaultSpoolDirectory") == 0) {
		...
		(void) strlcpy(buf, si.shr_winpath, MAXPATHLEN);
		smb_share_free(&si);
		goto encode_string;
	}
...
spoolss_s_GetPrinter(void *arg, ndr_xa_t *mxa)
{
	...
	if ((hdl = ndr_hdlookup(mxa, id)) == NULL) {
	...
	(void) strlcpy(tbuf, hdl->nh_data, MAXPATHLEN);
	pname = strtok(tbuf, "\\");
	while (pname = strtok(NULL, "\\"))
		last_pname = pname;
	if (last_pname == NULL) {
		last_pname = SMBD_PRINTER_NAME_DEFAULT;
		(void) snprintf(printer, MAXPATHLEN, "%s\\%s",
		    server, last_pname);
	} else
		(void) strlcpy(printer, hdl->nh_data, MAXPATHLEN);
	...
	case 2:
		...
		smb_rpc_off(wname, last_pname, &offset,
		    &pinfo2->sharename);

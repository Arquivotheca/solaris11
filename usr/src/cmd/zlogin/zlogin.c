...
/*
 * Solaris 10 (or earlier) Branded Zones also use this, so cannot change
 * to /usr/sbin/sh
 */
...
/*
 * -R is private; do not document.
 */
static void
usage(void)
{
	(void) fprintf(stderr, gettext("usage: %s [ -dCES ] [ -e cmdchar ] "
...
get_console_master(const char *zname)
{
	...
	msglen = snprintf(clientid, sizeof (clientid), "IDENT %lu %s %s\n",
	    getpid(), setlocale(LC_MESSAGES, NULL), dflag ? "d" : "");
 ...
 *
 * We use the zonept() call to let the ptm driver know what we are up
 * to.
 */
static int
init_slave_pty(zoneid_t zoneid)
{
	int slavefd = -1;
	char *slavename;
	 ...
	 * inaccessible to the global zone.
	 *
	...
	if ((slavefd = open(slavename, O_RDWR)) < 0) {
		zerror(gettext("failed to open %s: %s"), slavename,
		    strerror(errno));
		return (-1);
	}
...
setup_utmpx(char *slavename)
{
	...
	ut.ut_pid = getpid();
	ut.ut_id[0] = 'z';
	ut.ut_id[1] = ut.ut_id[2] = ut.ut_id[3] = _UTMP_ID_WILDCARD;
	ut.ut_type = LOGIN_PROCESS;
	(void) time(&ut.ut_tv.tv_sec);

	/*
	 * Now that UTMPX_FILE is located in /system/volatile, it is always
	 * available but this might not be the case in older zones.
	 * Login anyway, as that is more important than having
	 * a utmpx entry.  Older zones (Solaris 10) will require
	 * a utmpx entry and they require it in the old location; so
	 * we create the entry in the old location; for native zones
	 * there is a symlink to the proper utmpx file.
	 */
	(void) utmpxname(_PATH_UTMPX_OLD);
	(void) makeutx(&ut);
...
main(int argc, char **argv)
{
	...
	while ((arg = getopt(argc, argv, "CER:Sde:l:")) != EOF) {
		switch (arg) {
		case 'C':
			console = 1;
			break;
	...
	if (console == 0 && dflag != 0) {
		zerror(gettext(
		    "-d may not be specified for non-console login"));
		usage();
	}
	...
	if (console) {
		...
		if (zonecfg_start_zoneadmd(zonename) == -1)
			return (1);
	...
	if ((child_pid = fork()) == -1) {
		...
		if ((slavefd = init_slave_pty(zoneid)) == -1)
			return (1);

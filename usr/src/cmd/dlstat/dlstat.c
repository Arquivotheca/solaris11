...
typedef struct link_chain_s {
	...
	char			lc_linkname[MAXLINKNAMESPECIFIER];
...
typedef struct show_state {
	...
	zoneid_t	*ls_zones;
	uint32_t	ls_zonecnt;
	const char	*ls_linkname;
	boolean_t	*ls_dumpstattype;
} show_state_t;
...
static cmd_t	cmds[] = {
	{ "",		do_show,
	    ...
	    "       dlstat [-a | -A] [-i <interval>] [-pZ] [ -o field[,...]]\n"
	    "              [-z <zone>,..] [-u R|K|M|G|T|P] [link]"},
	{ "show-phys", do_show_phys,
	    "dlstat show-phys [-r | -t] [-i interval] [-z <zone>,..] [-apZ]\n"
	    "                 [ -o field[,...]] [-u R|K|M|G|T|P] [link]"},
	...
	{ "show-aggr", do_show_aggr,
	    "dlstat show-aggr [-r | -t] [-i interval] [-pZ] [-z <zone>,..]\n"
	    "                 [ -o field[,...]] [-u R|K|M|G|T|P] [link]" }
};
...
typedef struct total_fields_buf_s {
	char t_linkname[MAXLINKNAMESPECIFIER];
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(ring_fields_buf_t, r_zone),	print_default_cb},
{ "TYPE",	5,
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(rx_ring_fields_buf_t, rr_zone),		print_default_cb},
{ "TYPE",	5,
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(tx_ring_fields_buf_t, tr_zone),		print_default_cb},
{ "TYPE",	5,
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(lane_fields_buf_t, l_zone),	print_default_cb},
{ "TYPE",	5,
...
typedef struct rx_lane_fields_buf_s {
	char rl_linkname[MAXLINKNAMESPECIFIER];
	...
	char rl_intrbytes[MAXSTATLEN];
	char rl_polls[MAXSTATLEN];
	char rl_pollbytes[MAXSTATLEN];
	char rl_idrops[MAXSTATLEN];
	char rl_idropbytes[MAXSTATLEN];
	char rl_zone[ZONENAME_MAX];
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(rx_lane_fields_buf_t, rl_zone),		print_default_cb},
{ "TYPE",	5,
...
{ "INTRBYTES",	8,
    offsetof(rx_lane_fields_buf_t, rl_intrbytes),	print_default_cb},
{ "POLLS",	8,
    offsetof(rx_lane_fields_buf_t, rl_polls),		print_default_cb},
{ "POLLBYTES",	8,
    offsetof(rx_lane_fields_buf_t, rl_pollbytes),	print_default_cb},
{ "IDROPS",	8,
    offsetof(rx_lane_fields_buf_t, rl_idrops),		print_default_cb},
{ "IDROPBYTES",	8,
    offsetof(rx_lane_fields_buf_t, rl_idropbytes),	print_default_cb},
{ NULL,		0,		0,		NULL}};
...
typedef struct tx_lane_fields_buf_s {
	char tl_linkname[MAXLINKNAMESPECIFIER];
	...
	char tl_odrops[MAXSTATLEN];
	char tl_odropbytes[MAXSTATLEN];
	char tl_zone[ZONENAME_MAX];
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(tx_lane_fields_buf_t, tl_zone),		print_default_cb},
{ "TYPE",	5,
...
{ "ODROPS",	8,
    offsetof(tx_lane_fields_buf_t, tl_odrops),		print_default_cb},
{ "ODROPBYTES",	8,
    offsetof(tx_lane_fields_buf_t, tl_odropbytes),	print_default_cb},
{ NULL,		0,		0,		NULL}};
...
{ "ZONE",	ZONECOLWIDTH,
    offsetof(aggr_port_fields_buf_t, ap_zone),		print_default_cb},
{ "PORT",	15,
...
static link_chain_t *
get_link_prev_stat(datalink_id_t linkid, const char *linkname, void *arg)
{
	...
	for (link_curr = state->ls_linkchain; link_curr;
		...
		if (link_curr->lc_linkid == linkid &&
		    strcmp(link_curr->lc_linkname, linkname) == 0)
			break;
	}
				/* New link, add it */
	if (link_curr == NULL) {
		...
		(void) strlcpy(link_curr->lc_linkname, linkname,
		    sizeof (link_curr->lc_linkname));
...
void *
print_total_stats(const char *linkname, const char *zname, void *statentry,
...
void *
print_rx_generic_ring_stats(const char *linkname, const char *zname,
...
void *
print_tx_generic_ring_stats(const char *linkname, const char *zname,
    void *statentry, char unit, boolean_t parsable)
{
	ring_stat_entry_t	*sentry = statentry;
	ring_stat_t		*link_stats = &sentry->re_stats;
...
void *
print_rx_ring_stats(const char *linkname, const char *zname, void *statentry,
    char unit, boolean_t parsable)
{
	ring_stat_entry_t	*sentry = statentry;
	ring_stat_t		*link_stats = &sentry->re_stats;
...
void *
print_tx_ring_stats(const char *linkname, const char *zname, void *statentry,
    char unit, boolean_t parsable)
{
	ring_stat_entry_t	*sentry = statentry;
	ring_stat_t		*link_stats = &sentry->re_stats;
...
void *
print_rx_generic_lane_stats(const char *linkname, const char *zname,
    void *statentry, char unit, boolean_t parsable)
{
	...
	if (sentry->rle_id == L_HWLANE)
		...
		(void) snprintf(buf->l_id, sizeof (buf->l_id), "other");
	else
...
void *
print_tx_generic_lane_stats(const char *linkname, const char *zname,
    void *statentry, char unit, boolean_t parsable)
{
...
void *
print_rx_lane_stats(const char *linkname, const char *zname, void *statentry,
    char unit, boolean_t parsable)
{
	rx_lane_stat_entry_t	*sentry = statentry;
	rx_lane_stat_t		*link_stats = &sentry->rle_stats;
	...
	if (sentry->rle_id == L_HWLANE)
		...
		(void) snprintf(buf->rl_id, sizeof (buf->rl_id), "other");
	else
	...
	map_to_units(buf->rl_intrbytes, sizeof (buf->rl_intrbytes),
	    link_stats->rl_intrbytes, unit, parsable);

	map_to_units(buf->rl_polls, sizeof (buf->rl_polls),
	    link_stats->rl_polls, unit, parsable);

	map_to_units(buf->rl_pollbytes, sizeof (buf->rl_pollbytes),
	    link_stats->rl_pollbytes, unit, parsable);

	map_to_units(buf->rl_idrops, sizeof (buf->rl_idrops),
	    link_stats->rl_idrops, unit, parsable);

	map_to_units(buf->rl_idropbytes, sizeof (buf->rl_idropbytes),
	    link_stats->rl_idropbytes, unit, parsable);
...
void *
print_tx_lane_stats(const char *linkname, const char *zname, void *statentry,
    char unit, boolean_t parsable)
{
	tx_lane_stat_entry_t	*sentry = statentry;
	tx_lane_stat_t		*link_stats = &sentry->tle_stats;
	...
	if (sentry->tle_id == L_HWLANE)
		...
		(void) snprintf(buf->tl_id, sizeof (buf->tl_id), "local");
	else if (sentry->tle_id == L_BCAST)
		(void) snprintf(buf->tl_id, sizeof (buf->tl_id), "other");
	else
	...
	map_to_units(buf->tl_odrops, sizeof (buf->tl_odrops),
	    link_stats->tl_odrops, unit, parsable);

	map_to_units(buf->tl_odropbytes, sizeof (buf->tl_odropbytes),
	    link_stats->tl_odropbytes, unit, parsable);
...
void *
print_aggr_port_stats(const char *linkname, const char *zname, void *statentry,
    char unit, boolean_t parsable)
{
...
query_link_stats(dladm_handle_t dh, datalink_id_t linkid, const char *linkname,
    void *arg, dladm_stat_type_t stattype)
{
	...
	link_node = get_link_prev_stat(linkid, linkname, arg);
	if (link_node == NULL)
		goto done;
...
void
walk_dlstat_stats(show_state_t *state, dladm_datalink_info_t *di,
    dladm_stat_type_t stattype, dladm_stat_chain_t *diff_stat)
{
	dladm_stat_chain_t  *curr;
	char zname[ZONENAME_MAX];

	(void) getzonenamebyid(di->di_zoneid, zname, sizeof (zname));
	...
	for (curr = diff_stat; curr != NULL; curr = curr->dc_next) {
		...
		fields_buf = state->ls_stats2str[stattype](di->di_linkname,
		    zname, curr->dc_statentry, state->ls_unit,
		    state->ls_parsable);
		/* Print the stats */
		if (fields_buf != NULL)
...
/*ARGSUSED*/
static int
show_queried_stats(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	...
	if (state->ls_linkname != NULL)
		lname = state->ls_linkname;
	else
		lname = di->di_linkname;
	...
	for (i = 0; i < DLADM_STAT_NUM_STATS; i++) {
		if (state->ls_stattype[i]) {
			 ...
			 */
			diff_stat = query_link_stats(handle, di->di_linkid,
			    lname, arg, i);
			walk_dlstat_stats(state, di, i, diff_stat);
...
static void
show_link_stats(datalink_id_t linkid, const char *name, show_state_t state,
    uint32_t interval)
{
	state.ls_linkname = name;

	/*
	 * A sun studio compiler bug 7032611 optimizes away the infinite loop
	 * and leads to the dlstat bug 7032352. This if block for interval 0
	 * is a workaround for the compiler bug. Once the compiler bug is fixed
	 * we can choose to remove this if block and combine it with the loop
	 * block below.
	 */
	if (interval == 0) {
		(void) dladm_walk_datalinks(handle, linkid, show_queried_stats,
		    &state, DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE,
		    DLADM_OPT_ACTIVE | DLADM_OPT_ALLZONES, state.ls_zonecnt,
		    state.ls_zones);
		return;
	}
...
/*ARGSUSED*/
void
print_all_stats(dladm_handle_t dh, datalink_id_t linkid, const char *linkname,
    dladm_stat_chain_t *stat_chain)
{
	...
	char			prev_linkname[MAXLINKNAMESPECIFIER];
	...
	for (curr = stat_chain; curr != NULL; curr = curr->dc_next) {
		...
		for (curr_stat = stat_entry->nve_stats; curr_stat != NULL;
			...
			(void) printf("\t%15s", curr_stat->nv_statname);
			(void) printf("\t\t%15llu\n", curr_stat->nv_statval);
		}

		(void) strncpy(prev_linkname, linkname, sizeof (prev_linkname));
		stat_printed = B_TRUE;
	}
	if (stat_printed)
		(void) printf("-----------------------------"
...
static int
dump_queried_stats(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	show_state_t		*state = arg;
	boolean_t		*stattype = state->ls_dumpstattype;
	int			i;
	dladm_stat_chain_t	*stat_chain;
	const char		*lname;
	...
	for (i = 0; i < DLADM_STAT_NUM_STATS; i++) {
		if (stattype[i]) {
			stat_chain = dladm_link_stat_query_all(dh,
			    di->di_linkid, i);
			print_all_stats(dh, di->di_linkid, lname, stat_chain);
...
static void
dump_all_link_stats(datalink_id_t linkid, const char *name, show_state_t state,
    boolean_t *stattype)
{
	state.ls_linkname = name;
	state.ls_dumpstattype = stattype;

	(void) dladm_walk_datalinks(handle, linkid, dump_queried_stats,
	    &state, DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE,
...
do_show(int argc, char *argv[], const char *use)
{
	...
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":rtaApi:o:u:z:Z",
	    NULL, NULL)) != -1) {
		switch (option) {
		case 'r':
			if (r_arg)
	...
	if (a_arg &&
	    (p_arg || o_arg || u_arg || i_arg || z_arg || Z_arg)) {
	...
	/* check link name (optional last argument) */
	if (optind == (argc-1)) {
		linkname = argv[optind];
		status = dladm_apply_linknamefilters(handle, linkname, &linkid,
		    state.ls_zonecnt, state.ls_zones, DLADM_OPT_ACTIVE,
		    dlstat_zoneid, errorstr);
		if (status != DLADM_STATUS_OK)
	...
	if (a_arg) {
		...
		dump_all_link_stats(linkid, linkname, state, stattype);
		return;
	}
	...
	show_link_stats(linkid, linkname, state, interval);
...
do_show_phys(int argc, char *argv[], const char *use)
{
	...
	bzero(&state, sizeof (state));
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":rtapi:o:u:z:Z",
	    NULL, NULL)) != -1) {
		switch (option) {
		case 'r':
			if (r_arg)
...
do_show_link(int argc, char *argv[], const char *use)
{
	...
	bzero(&state, sizeof (state));
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":hrtFapi:o:u:z:Z",
	    NULL, NULL)) != -1) {
		switch (option) {
		case 'h':
			if (r_arg || t_arg || i_arg || p_arg ||
			    o_arg || u_arg || a_arg) {
...
do_show_aggr(int argc, char *argv[], const char *use)
{
	...
	bzero(&state, sizeof (state));
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":rtpi:o:u:z:Z",
	    NULL, NULL)) != -1) {
		switch (option) {
		case 'r':
			if (r_arg)

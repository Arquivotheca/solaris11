...
# Solaris/ON.  Also remove the LD_RUN_PATH that is returned by the default
# implementation, as it is not correct when building as part of Solaris/ON.
...
sub ext
{
	...
	foreach my $lp (map({ /^STUBENVLDLIBS\d+$/ ? split(' ', $ENV{$_}) : () }
	    sort(keys(%ENV)))) {

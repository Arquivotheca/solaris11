...
get_start_sector(ig_device_t *device)
{
	...
	for (i = 0; i < FD_NUMPART; i++) {
		...
		if (part->systid == UNUSED)
			continue;

		/*
		 * Skip checking a partition which starts at sector 0, but
		 * emit a warning, since if it's not unused, then it's most
		 * likely bogus.
		 */
		if (part->relsect == 0) {
			(void) fprintf(stderr, BAD_PART, i);
			BOOT_DEBUG("bad partition has id=%x (%d sectors)\n",
			    part->systid, part->numsect);
			continue;
		}
...
read_stage2_from_file(char *path, ig_data_t *dest)
{
	...
	bzero(stage2->buf, stage2->buf_size);
...
#define	IG_STAGE2_MAXSIZE	(5 * 1024 * 1024)
...
read_stage2_from_disk(int dev_fd, ig_stage2_t *stage2)
{
	...
	if (size > IG_STAGE2_MAXSIZE) {
		BOOT_DEBUG("stage2 derived size is too big  (size=%x,"
		    "max=%x)\n", size, IG_STAGE2_MAXSIZE);
		return (BC_ERROR);
	}
...
is_update_necessary(ig_data_t *data, char *updt_str)
{
	...
	/*
	 * If a 'forced' update is requested, do not bother checking
	 * the on-disk stage2.
	 */
	if (force_update) {

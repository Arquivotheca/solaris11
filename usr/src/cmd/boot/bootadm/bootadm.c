...
#define	UFS_SIGNATURE_LIST	_PATH_SYSVOL "/grub_ufs_signatures"
#define	ZFS_LEGACY_MNTPT	"/tmp/bootadm_mnt_zfs_legacy"

#define	BOOTADM_RDONLY_TEST	"BOOTADM_RDONLY_TEST"

/* lock related */
#define	BAM_LOCK_FILE		_PATH_SYSVOL "/bootadm.lock"
...
#define	GRUBDISK_MAP		_PATH_SYSVOL "/solaris_grubdisk.map"
...
#define	INSTALLGRUB		"/usr/sbin/installgrub"
 ...
 */
#define	NEED_UPDATE_FILE _PATH_SYSVOL "/boot_archive_needs_update"

/*
 * svc:/system/boot-archive-update:default checks for this file and
 * updates the boot archive.
 */
#define	NEED_UPDATE_SAFE_FILE _PATH_SYSVOL "/boot_archive_safefile_update"
...
bam_lock(void)
{
	...
	if (bam_lock_fd < 0) {
		 ...
		 * In this case, root is readonly and the kernel
		 * failed to mount _PATH_SYSVOL.
...
update_all(char *root, char *opt)
{
	...
	while (getmntent(fp, &mnt) == 0) {
		if (mnt.mnt_special == NULL)
			continue;
		if (hasmntopt(&mnt, MNTOPT_GLOBAL) != NULL)
			continue;
...
kernel_parser(entry_t *entry, char *cmd, char *arg, int linenum)
{
	...
	if ((strncmp(arg, DIRECT_BOOT_KERNEL,
	    sizeof (DIRECT_BOOT_KERNEL) - 1) == 0) ||
	    (strncmp(arg, COMPAT_BOOT_KERNEL,
	    sizeof (COMPAT_BOOT_KERNEL) - 1) == 0) ||
	    (strncmp(arg, KERNEL_PREFIX, strlen(KERNEL_PREFIX)) == 0 &&
	    ...
	    (strncmp(arg, COMPAT_BOOT_FAILSAFE_KERNEL,
	    sizeof (COMPAT_BOOT_FAILSAFE_KERNEL) - 1) == 0)) {
...
module_parser(entry_t *entry, char *cmd, char *arg, int linenum)
{
	...
	if (strcmp(arg, DIRECT_BOOT_ARCHIVE) == 0 ||
	    ...
	    strcmp(arg, COMPAT_BOOT_ARCHIVE) == 0 ||
	    strcmp(arg, FAILSAFE_ARCHIVE) == 0 ||
	    strcmp(arg, COMPAT_FAILSAFE_ARCHIVE) == 0) {
...
mount_legacy_dataset(char *pool, zfs_mnted_t *mnted)
{
	...
	*mnted = ZFS_MNT_ERROR;
	...
	(void) snprintf(cmd, sizeof (cmd),
	    "/usr/sbin/zfs get -Ho value mounted %s",
	    pool);
...
mount_top_dataset(char *pool, zfs_mnted_t *mnted)
{
	...
	*mnted = ZFS_MNT_ERROR;
	 ...
	 */
	(void) snprintf(cmd, sizeof (cmd),
	    "/usr/sbin/zfs get -Ho value mountpoint %s",
	    pool);
	ret = exec_cmd(cmd, &flist);
...
found:
	...
	/* add the ufs signature to the _PATH_SYSVOL list of signatures */
...
ufs_add_to_sign_list(char *sign)
{
	 ...
	 * We simply unlink the _PATH_SYSVOL file and continue
	 */
...
zfs_get_physical(char *special, char ***physarray, int *n)
{
	...
	(void) snprintf(cmd, sizeof (cmd), "/usr/sbin/zpool status %s", pool);
...
ufs_get_physical(char *special, char ***physarray, int *n)
{
	...
	(void) snprintf(cmd, sizeof (cmd),
	    "/usr/sbin/metastat -p %s", shortname);
	...
	int *entry_num)
{
	...
	for (i = 0, ent = mp->entries; ent; i++, ent = ent->next) {
		...
		if ((strcmp(module, FAILSAFE_ARCHIVE) == 0 &&
		    strcmp(lp->prev->arg, FAILSAFE_ARCHIVE) == 0) ||
		    (strcmp(module, COMPAT_FAILSAFE_ARCHIVE) == 0 &&
		    strcmp(lp->prev->arg, COMPAT_FAILSAFE_ARCHIVE) == 0)) {
 ...
 */
static void
set_archive_line(entry_t *entryp)
{
	...
	size_t bufsize)
{
	...
	if (mp->curdefault) {
		...
		if ((entryp != NULL) && ((entryp->flags & (BAM_ENTRY_BOOTADM |
		    BAM_ENTRY_LU | BAM_ENTRY_LIBBE)) == 0)) {
		...
		if (optnum == ARGS_CMD) {
			...
			*space = '\0';
			if (strcmp(kernelp->arg, DIRECT_BOOT_KERNEL) != 0 &&
			    strcmp(kernelp->arg, COMPAT_BOOT_KERNEL) != 0) {
	...
	if ((path[0] == '\0') ||
	    ...
	    strcmp(path, COMPAT_BOOT_KERNEL) == 0))) {
		...
		if (((optnum == KERNEL_CMD) &&
		    ...
		    sizeof (DIRECT_BOOT_KERNEL) - 1) == 0 ||
		    strncmp(kernelp->arg, COMPAT_BOOT_KERNEL,
		    sizeof (COMPAT_BOOT_KERNEL) - 1) == 0))) {
			kernelp = NULL;

...
find_einfo(char *extra)
{
	...
	/*
	 * We can sanitize the value of 'size' here because currently
	 * we reserve a fixed amount of SECTOR_SIZE bytes for the extra space.
	 * To prevent a segfault we need to account for the 8 bytes
	 * of the bb_header_ext_t.
	 */
	if (ext_header->size > SECTOR_SIZE - sizeof (bb_header_ext_t)) {
		BOOT_DEBUG("extra header size looks corrupted, unable to"
		    "find extended versioning information\n");
		return (NULL);
	}
	...
	if (memcmp(einfo->magic, EINFO_MAGIC, EINFO_MAGIC_SIZE) != 0) {
		BOOT_DEBUG("Unable to read bootblock extended versioning "

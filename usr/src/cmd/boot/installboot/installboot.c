...
/*
 * The ZFS bootblock size is currently around 16K. When reading from the disk,
 * the size of the bootblock is derived from the fake multiboot header that we
 * place along with the versioning information. This size is used to allocate
 * space for the bootblock temporarily in memory. In order to prevent a random
 * or malicious multiboot entry from specifying an extremely large size and
 * consume lots of memory, we state an arbitrary upper limit for the size of
 * 100K to detect such cases. This upper limit is by no means linked to a
 * physical limit for the ZFS bootblock.
 */
#define	IB_ZFS_BBSIZE_UPPER	(100 * 1024)
...
read_bootblock_from_disk(int dev_fd, ib_bootblock_t *bblock)
{
	...
	/*
	 * The layout of the bootblock, plus the fake multiboot structure and
	 * the extended information payload is pretty much fixed and looks like
	 * this:
	 *
	 * [7.5k ZFS bblock][gap][rest of ZFS bblock][fake multiboot][einfo]
	 *
	 * Since we don't expect, nor accept, any other configuration, we use
	 * this information to extra sanity test the found values and avoid
	 * crashing on spurious disk contents.
	 */
	mboot = (multiboot_header_t *)(mboot_scan + mboot_off);

	if (mboot->load_end_addr <= mboot->load_addr) {
		BOOT_DEBUG("Invalid multiboot header contents\n");
		return (BC_ERROR);
	}

	size = mboot->load_end_addr - mboot->load_addr;
	BOOT_DEBUG("size=%x, mboot_off=%x\n", size, mboot_off);

	if (size > IB_ZFS_BBSIZE_UPPER || size <= BBLK_DATA_RSVD_SIZE) {
		BOOT_DEBUG("Invalid bootblock size (%x)\n", size);
		return (BC_ERROR);
	}

	if (mboot_off > size - BBLK_DATA_RSVD_SIZE + SECTOR_SIZE) {
		BOOT_DEBUG("Invalid placement of mboot offset\n");
		return (BC_ERROR);
	}
...
is_update_necessary(ib_data_t *data, char *updt_str)
{
	...
	/* Don't bother getting to the disk on a forced update */
	if (force_update) {

...
install_grub()
{
	...
	if [ $is_pcfs_boot = yes ]; then
		...
		if [ X"$rpcfsdev" != X ]; then
			...
			"$ALT_ROOT"/usr/sbin/installgrub $STAGE1 $STAGE2 $rpcfsdev
		fi
	fi
	...
	do
		if [ X"$rpcfsdev" != X ]; then
			...
			"$ALT_ROOT"/usr/sbin/bootadm update-menu $zfsarg\
			    -R "$ALT_ROOT"/stubboot -o $rootdev,"$ALT_ROOT"
		else
			echo "Creating GRUB menu in ${ALT_ROOT:-/}"
			$ALT_ROOT/usr/sbin/bootadm update-menu -R ${ALT_ROOT:-/} \
			    $zfsarg -o $rootdev
		fi
		print "Installing grub on $rootdev"
		"$ALT_ROOT"/usr/sbin/installgrub $STAGE1 $STAGE2 $rootdev
	done
}

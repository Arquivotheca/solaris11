...
#define	LOCALTIME	"/etc/" TZDEFAULT
#define	SMF_TIMEZONE_FMRI	"svc:/system/timezone:default"
...
static void	sym_link(const char * const fromfield,
...
{
#ifdef LEAPSECOND_SUPPORT
	(void) fprintf(stream, gettext("%s: usage is %s "
	    "[ --version ] [ --help ] [ -s ] [ -v ] [ -l localtime ]"
...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	if (TYPE_BIT(zic_t) < 64) {
		(void) fprintf(stderr, "%s: %s\n", progname,
		    gettext("wild compilation-time specification of zic_t"));
		exit(EXIT_FAILURE);
	}
	for (i = 1; i < argc; ++i)
...
#else
	while ((c = getopt(argc, argv, "d:l:p:vsy:")) != EOF && c != -1)
#endif
		switch (c) {
			default:
...
"%s: More than one -L option specified\n"),
	...
	if (directory != NULL && lcltime != NULL) {
		(void) fprintf(stderr, gettext(
		    "%s: '-d' and '-l' cannot be used together\n"),
		    progname);
		exit(EXIT_FAILURE);
	}
	if (directory == NULL)
	...
	if (lcltime != NULL) {
		eat("command line", 1);
		/* create symlinks */
		sym_link(lcltime, LOCALTIME);
		sym_link(LOCALTIME, TZDEFAULT);
	}
	if (psxrules != NULL) {
		eat("command line", 1);
		...
		if (result != 0 &&
			...
			!itsdir(fromname)) {
				...
				result = symlink(symlinkcontents,
					toname);
				if (result == 0)
warning(gettext("hard link failed, symbolic link used"));
...
#ifdef __sun
		/*
		 * We adjust the charcnt so that the vast 4/8 byte integer
		 * (ats) located on 4/8 byte boundary.
		 *
		 *	tzhead			44 byte
		 *	time_t ats[timecnt]	4 x timecnt
		 *	char type[timecnt]	timecnt
		 *	struct types {
		 *		int32_t offset
		 *		uchar_t isdst
		 *		uchar_t abidx
		 *	}			6 x typecnt
		 *	char chars[charcnt]	charcnt
		 * 	struct leaps {
		 *		time_t t
		 *		int32_t corr
		 *	}			8 x leapcnt
		 *	char issttd[isstdcnt]	typecnt
		 *	char isgmt[isgmtcnt]	typecnt
		 */
		if (pass == 1) {
			/* calculate total size */
			i = sizeof (tzh) + 4 * thistimecnt + thistimecnt +
			    6 * thistypecnt + thischarcnt + 8 * thisleapcnt +
			    thistypecnt + thistypecnt;

			/* round up the end offset to 4 byte boundary */
			j = (i + 3) & ~3;
			/*
			 * Here we need 4 byte alignment but not 8 byte, so
			 * that the ats in 2nd pass could align in 8 byte.
			 */
			if ((j & 7) == 0)
				j += 4;
			rcharcnt = thischarcnt + (j - i);
			if (rcharcnt > TZ_MAX_CHARS) {
				/*
				 * We got too much chars. Give up optimization.
				 */
				rcharcnt = thischarcnt;
			}
		} else {
			/*
			 * Since the 2nd header is aligned in the good
			 * position, we don't need to do much here.
			 */
			rcharcnt = thischarcnt;
		}
#endif
...
#ifdef __sun
		convert(eitol(rcharcnt), tzh.tzh_charcnt);
#else
...
#undef DO
		...
		for (i = 0; i < typecnt; ++i)
			if (writetype[i]) {
				...
				(void) putc((unsigned char)indmap[abbrinds[i]],
				    fp);
			}
		if (thischarcnt != 0)
...
#ifdef __sun
		if ((i = rcharcnt - thischarcnt) != 0) {
			char tmpbuf[8];
			(void) memset(tmpbuf, 0, i);
			(void) fwrite(tmpbuf, 1, i, fp);
		}
#endif
...
set_smf_tz(const char *tz)
{
	...
	/*
	 * Read localime property from timezone SMF service.
	 */
	if ((scf_pg_get_property(pg, "localtime", prop) == -1) ||
	    (scf_property_get_value(prop, val) == -1) ||
	    (scf_value_get_astring(val, l_time, 1024) == -1)) {
		goto cleanup;
	}

	/*
	 * Set the property only if it has changed.
	 */
	if (l_time != NULL) {
		/* Check if the localime property has changed */
		if (strcmp(l_time, tz) != 0) {
			/*
			 * localime property has changed
			 * so set it to new value
			 */
			(void) scf_value_set_astring(val, tz);
		} else {
			/*
			 * localime property has not changed
			 * so cleanup and return success
			 */
			err = 0;
			goto cleanup;
		}
	...
	/* Update the changed localtime property in the SMF service */
...
/*
 * Create a sym_link from 'fromfield' to 'tofield'
 */
static void
sym_link(const char * const fromfield, const char * const tofield)
{
	char *fromname;
	char *toname;
	int result = -1;
	...
	if (access(fromname, F_OK) == 0 && !itsdir(fromname)) {
		const char *s = toname;
		...
		symlinkcontents = ecatalloc(symlinkcontents, fromname + 1);

		/* check if valid symlink exists */
		if (access(toname, F_OK) == 0 && !itsdir(toname)) {
			char buf[PATH_MAX];
			int len = 0;

			if ((len = readlink(toname, buf, sizeof (buf))) >= 0) {
				buf[len] = '\0';
				if (strcmp(buf, symlinkcontents) == 0) {
					ifree(symlinkcontents);
					return;
				}
			}
	...
	if (result != 0) {
		(void) fprintf(stderr, gettext(
		    "%s: Can't link from %s to %s: %s\n"),
		    progname, fromname, toname, strerror(errno));
		exit(EXIT_FAILURE);
	} else {
		/*
		 * Set timezone/localtime property
		 * in timezone:default SMF service.
		 */
		if ((strcmp(lcltime, fromfield)) == 0) {
			if (set_smf_tz(lcltime) == -1) {
				(void) fprintf(stderr, gettext(
				    "%s: Can't update timezone SMF service.\n"),
				    progname);
				exit(EXIT_FAILURE);
			}
		}

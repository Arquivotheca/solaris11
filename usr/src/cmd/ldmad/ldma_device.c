...
/*
 * Open a device path. An open with regular flags might fail for a valid
 * path (for example if we have a path to an empty disk slice). So we first
 * try using regular flags, but if that fails then we retry with the O_NDELAY
 * flag.
 */
static int
open_devpath(const char *path, int oflag)
{
	int fd;
    ...
    size_t request_dlen, ldma_message_header_t **replyp, size_t *reply_dlenp)
{
	...
	if ((fd = open_devpath(path, O_RDWR)) != -1) {
		...
		if ((fd = open_devpath(path, O_RDONLY)) != -1) {

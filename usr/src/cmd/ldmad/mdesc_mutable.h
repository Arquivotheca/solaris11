...
#define	CHAIN_ADD(_chain, _nodep)					\
	do {								\
		...
		(_nodep)->prevp = (_chain).endp;			\
		(_chain).endp = (_nodep);				\
		(_nodep)->nextp = NULL;					\
		(_chain).count++;					\
	NOTE(CONSTCOND) } while (0)

#define	CHAIN_UNLINK(_chain, _nodep)					\
	do {								\
		if ((_chain).startp == (_nodep))			\
			(_chain).startp = (_nodep)->nextp;		\
		else							\
			(_nodep)->prevp->nextp =			\
				(_nodep)->nextp;			\
		if ((_chain).endp == (_nodep))				\
			(_chain).endp = (_nodep)->prevp;		\
		else							\
			(_nodep)->nextp->prevp =			\
				(_nodep)->prevp;			\
		(_chain).count --;					\
		/* ensure chain can no longer be traversed via _nodep */\
		(_nodep)->prevp = NULL;					\
		(_nodep)->nextp = NULL;					\
	NOTE(CONSTCOND) } while (0)
...
void md_delete_node(mmd_t *mdp, md_node_t *nodep);
void md_delete_prop(mmd_t *mdp, md_node_t *nodep, md_prop_t *propp);

...
int
ldma_send_unsolicited_msg(ds_hdl_t hdl, ldma_message_header_t *msg, size_t len)
{
	return (hdl == DS_INVALID_HDL ? EINVAL : ds_send_msg(hdl, msg, len));
}
    ...
    ds_domain_hdl_t dhdl)
{
	...
	/* Invoke the agent-specific registration callback function. */
	if (agent->info->reg_cb != NULL)
		agent->info->reg_cb(ver, dhdl, hdl);
}
...
ldma_unreg_cb(ds_hdl_t hdl, ds_cb_arg_t arg)
{
	...
	if (agent->info->unreg_cb != NULL)
		agent->info->unreg_cb(hdl);
}

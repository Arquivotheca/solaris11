...
/*
 * Delete a node, then all arc references to it.
 */
void
md_delete_node(mmd_t *mdp, md_node_t *nodep)
{
	md_node_t *snp;

	CHAIN_UNLINK(mdp->node_list, nodep);

	for (snp = CHAIN_START(mdp->node_list);
	    snp != NULL;
	    snp = snp->nextp) {
		md_prop_t *propp;
		md_prop_t *next_propp;

		for (propp = CHAIN_START(snp->prop_list);
		    propp != NULL; propp = next_propp) {

			next_propp = propp->nextp;

			if (MDET_PROP_ARC == propp->type) {
				ASSERT(propp->d.arc.is_ptr);
				if (propp->d.arc.val.nodep == nodep) {
					md_delete_prop(mdp, snp, propp);
				}
			}
		}
	}
...
void
md_delete_prop(mmd_t *mdp, md_node_t *nodep, md_prop_t *propp)
{
	CHAIN_UNLINK(nodep->prop_list, propp);
	md_free_prop(mdp, propp);
}
...
md_free_string(mmd_t *mdp, md_string_t *msp)
{
	...
	if (msp->ref_cnt == 0) {
		CHAIN_UNLINK(mdp->string_list, msp);
		free(msp->strp);
		free(msp);
	}
}
...
md_free_data_block(mmd_t *mdp, md_data_block_t *mdbp)
{
	...
	if (mdbp->ref_cnt == 0) {
		CHAIN_UNLINK(mdp->data_block_list, mdbp);
		free(mdbp->datap);
		free(mdbp);
	}
}

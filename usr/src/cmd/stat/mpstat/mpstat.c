...
static int xcalls_index = -1;
static int intr_index = -1;
static int intrthd_index = -1;
static int pswitch_index = -1;
static int inv_switch_index = -1;
static int cpumigrate_index = -1;
static int mutex_adenters_index = -1;
static int rw_rdfails_index = -1;
static int rw_wrfails_index = -1;
static int syscall_index = -1;
static int cpu_ticks_user_index = -1;
static int cpu_ticks_krn_index = -1;
...
#define	TEXT_DOMAIN "SYS_TEST"		/* Use this only if it weren't */
	...
	while (infinite_cycles || iter > 0) {
		...
		if (old && snapshot_has_changed(old, new) == 1) {
			if (!suppress_state) {
				new->snap_changed = 1;
...
print_cpu(struct cpu_snapshot *c1, struct cpu_snapshot *c2)
{
	...
	(void) printf("%3d %4.0f %3.0f %4.0f %5.0f %4.0f "
	    ...
	    "%3.0f %3.0f",
	    c2->cs_id,
	    (kstat_delta(old_vm, &c2->cs_vm, "hat_fault", &hat_fault_index) +
	    kstat_delta(old_vm, &c2->cs_vm, "as_fault",
	    &hat_fault_index)) / etime,
	    kstat_delta(old_vm, &c2->cs_vm, "maj_fault",
	    &maj_fault_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "xcalls", &xcalls_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "intr", &intr_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "intrthread",
	    &intrthd_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "pswitch",
	    &pswitch_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "inv_swtch",
	    &inv_switch_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "cpumigrate",
	    &cpumigrate_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "mutex_adenters",
	    &mutex_adenters_index) / etime,
	    (kstat_delta(old_sys, &c2->cs_sys, "rw_rdfails",
	    &rw_rdfails_index) +
	    kstat_delta(old_sys, &c2->cs_sys, "rw_wrfails",
	    &rw_wrfails_index)) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "syscall",
	    &syscall_index) / etime,
	    kstat_delta(old_sys, &c2->cs_sys, "cpu_ticks_user",
	    &cpu_ticks_user_index) * percent,
	    kstat_delta(old_sys, &c2->cs_sys, "cpu_ticks_kernel",
	    &cpu_ticks_krn_index) * percent,
	    kstat_delta(old_sys, &c2->cs_sys, "cpu_ticks_wait",
	    &cpu_ticks_wait_index) * percent,
	    kstat_delta(old_sys, &c2->cs_sys, "cpu_ticks_idle",
	    &cpu_ticks_idle_index) * percent);
...
static void
agg_stat(kstat_t *k1, kstat_t *k2, char *name, int *index)
{
	kstat_named_t *ksn = stat_data_lookup(k1, name, index);
	kstat_named_t *ksn2 = stat_data_lookup(k2, name, index);
...
agg_vm(struct pset_snapshot *p, kstat_t *ks)
{
	...
	for (i = 1; i < p->ps_nr_cpus; i++) {
		...
		ps_cpus = p->ps_cpus[i];
		agg_stat(ks, &ps_cpus->cs_vm, "hat_fault", &hat_fault_index);
		agg_stat(ks, &ps_cpus->cs_vm, "as_fault", &as_fault_index);
		agg_stat(ks, &ps_cpus->cs_vm, "maj_fault", &maj_fault_index);
	}
...
agg_sys(struct pset_snapshot *p, kstat_t *ks)
{
	...
	for (i = 1; i < p->ps_nr_cpus; i++) {
		...
		cs_sys = &p->ps_cpus[i]->cs_sys;
		agg_stat(ks, cs_sys, "xcalls", &xcalls_index);
		agg_stat(ks, cs_sys, "intr", &intr_index);
		agg_stat(ks, cs_sys, "intrthread", &intrthd_index);
		agg_stat(ks, cs_sys, "pswitch", &pswitch_index);
		agg_stat(ks, cs_sys, "inv_swtch", &inv_switch_index);
		agg_stat(ks, cs_sys, "cpumigrate", &cpumigrate_index);
		agg_stat(ks, cs_sys, "mutex_adenters", &mutex_adenters_index);
		agg_stat(ks, cs_sys, "rw_rdfails", &rw_rdfails_index);
		agg_stat(ks, cs_sys, "rw_wrfails", &rw_wrfails_index);
		agg_stat(ks, cs_sys, "syscall", &syscall_index);
		agg_stat(ks, cs_sys, "cpu_ticks_user", &cpu_ticks_user_index);
		agg_stat(ks, cs_sys, "cpu_ticks_kernel", &cpu_ticks_krn_index);
		agg_stat(ks, cs_sys, "cpu_ticks_wait", &cpu_ticks_wait_index);
		agg_stat(ks, cs_sys, "cpu_ticks_idle", &cpu_ticks_idle_index);
	}
...
print_pset(struct pset_snapshot *p1, struct pset_snapshot *p2)
{
	...
	(void) printf("%3d %4.0f %3.0f %4.0f %5.0f %4.0f "
	    ...
	    "%3.0f %3.0f %3d\n",
	    p2->ps_id,
	    (kstat_delta(&old_vm, &new_vm, "hat_fault", &hat_fault_index) +
	    kstat_delta(&old_vm, &new_vm, "as_fault", &as_fault_index)) / etime,
	    kstat_delta(&old_vm, &new_vm, "maj_fault",
	    &maj_fault_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "xcalls", &xcalls_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "intr", &intr_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "intrthread",
	    &intrthd_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "pswitch", &pswitch_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "inv_swtch",
	    &inv_switch_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "cpumigrate",
	    &cpumigrate_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "mutex_adenters",
	    &mutex_adenters_index) / etime,
	    (kstat_delta(&old_sys, &new_sys, "rw_rdfails", &rw_rdfails_index) +
	    kstat_delta(&old_sys, &new_sys, "rw_wrfails",
	    &rw_wrfails_index)) / etime,
	    kstat_delta(&old_sys, &new_sys, "syscall", &syscall_index) / etime,
	    kstat_delta(&old_sys, &new_sys, "cpu_ticks_user",
	    &cpu_ticks_user_index) * percent,
	    kstat_delta(&old_sys, &new_sys, "cpu_ticks_kernel",
	    &cpu_ticks_krn_index) * percent,
	    kstat_delta(&old_sys, &new_sys, "cpu_ticks_wait",
	    &cpu_ticks_wait_index) * percent,
...
show_cpu_usage(struct snapshot *old, struct snapshot *new, int display_agg)
{
	...
	if (lines_until_reprint == 0 || nr_active_cpus() > 1) {

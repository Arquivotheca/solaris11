...
static int rawch_index = -1;
static int outch_index = -1;
...
#define	TEXT_DOMAIN "SYS_TEST"		/* Use this only if it weren't */
	...
	do {
		...
		if (do_tty || do_cpu) {
			...
			getime = (getime / nr_active_cpus()) / hz;
			if (getime == 0.0)
		...
		if (oldss && snapshot_has_changed(oldss, newss) == 1) {
			oldss->snap_changed = 1;
		}
		newss->snap_changed = 0;
	} while (--iter);

    ...
    snapshot_cb cb, void *data)
{
	...
	while (d1 && d2) {
		rtc = strcmp(d1->is_name, d2->is_name);
		if (rtc < 0) {
			changed = 1;
			cb(d1, NULL, data);
...
snapshot_has_changed(struct snapshot *old, struct snapshot *new)
{
	...
	/*
	 * If snap_changed is zero then we can skip checking the
	 * records.  The only time snap_changed is non-zero is when
	 * we have been given a kstat chain id value from
	 * kstat_chain_update()
	 */

	if (!new->snap_changed && (!(new->s_types & SNAP_PSETS)))
		return (0);
	if (new->snap_changed)
		type = old->s_types;
	else
		type = old->s_types & SNAP_PSETS;

	/*
	 * The snapshot may or may not have changed, depending on
	 * what things show.  To verify this, we need to walk and
	 * compare all the records that we care about.  This can be
	 * a very slow process, but it is the only way to do this.
	 */

	if (type & cpu_mask)
		ret = snapshot_walk(SNAP_CPUS, old, new, dummy_cb, NULL);
	if (!ret && (type & SNAP_PSETS))
		ret = snapshot_walk(SNAP_PSETS, old, new, dummy_cb, NULL);
	if (!ret && (type & iodev_mask))

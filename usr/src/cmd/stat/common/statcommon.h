...
struct iodev_snapshot {
	...
	avl_node_t avl_link;
	/*
	 * Following fields are pieces of data that are constant
	 * across snapshots (unless the snap changes).  have_devinfo
	 * indicates if we have retrieved the information or not.
	 */
	int io_index; /* Index of this record into the stored ksp */
	int have_devinfo;
	int cnst_type;
	int cnst_inst;
	int cnst_disk;
	char cnst_mod[KSTAT_STRLEN];
	char cnst_name[KSTAT_STRLEN];
};
...
struct snapshot_state {
	...
	struct io_information *io_records;
	int io_records_cnt;
	int kstat_cpus_active;
	int kstat_cpu_psets;
	int *kstat_cpu_pset_assign;
	psetid_t *psetid;
	size_t last_psets_count;
	kstat_t **inter_ksp_ptr;
	int sys_interrupts;
	kstat_t *sys_misc_ptr;
};
...
struct kstat_cpu_records {
	kstat_t *cpu_vm_ksp_ptr;
	kstat_t *cpus_sys_ksp_ptr;
	struct io_information *io_records;
	psetid_t cs_pset_id;
	int cs_state;
	int cs_id;
...
void *stat_data_lookup(kstat_t *, char *, int *);

...
build_mnt_list(FILE *mpt)
{
	...
	if (mpt) {
		while (nfs) {
			SAFE_FREE(nfs->device_name, sizeof (char *));
			SAFE_FREE(nfs->mount_point, sizeof (char *));
			SAFE_FREE(nfs->devinfo, sizeof (char *));
			tmp = nfs;
			nfs = nfs->next;
			SAFE_FREE(tmp, sizeof (mnt_t));
		}
		while (ufs) {
			SAFE_FREE(ufs->device_name, sizeof (char *));
			SAFE_FREE(ufs->mount_point, sizeof (char *));
			SAFE_FREE(ufs->devinfo, sizeof (char *));
			tmp = ufs;
			ufs = ufs->next;
			SAFE_FREE(tmp, sizeof (mnt_t));
		}

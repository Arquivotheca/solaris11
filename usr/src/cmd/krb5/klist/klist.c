...
#define KRB_AVAIL_STRING(x) ((x)?"available":"not available")
    ...
    fprintf(stderr, gettext("Usage: %s [-e] [-V] [[-c] [-d] [-f] [-s] [-a [-n]]] %s"),
            progname, gettext("[-k [-t] [-K]] [name]\n"));
    ...
    fprintf(stderr, gettext("\t\t-d shows the submitted authorization data types\n"));
    ...
    char *name;
{
    ...
    if (name == NULL) {
        if ((code = krb5_kt_default(kcontext, &kt))) {
            com_err(progname, code, gettext("while getting default keytab"));
            exit(1);
        }
    } else {
        if ((code = krb5_kt_resolve(kcontext, name, &kt))) {
            com_err(progname, code, gettext("while resolving keytab %s"),
                    name);
            exit(1);
        }
    }
    ...
    if ((code = krb5_kt_get_name(kcontext, kt, buf, BUFSIZ))) {
        com_err(progname, code, gettext("while getting keytab name"));
        exit(1);
    }
    ...
    if ((code = krb5_kt_start_seq_get(kcontext, kt, &cursor))) {
        com_err(progname, code, gettext("while starting keytab scan"));
        exit(1);
    }

    if (show_time) {
        printf(gettext("KVNO Timestamp"));
        fillit(stdout, timestamp_width - sizeof(gettext("Timestamp")) + 2, (int) ' ');
    ...
    while ((code = krb5_kt_next_entry(kcontext, kt, &entry, &cursor)) == 0) {
        if ((code = krb5_unparse_name(kcontext, entry.principal, &pname))) {
            com_err(progname, code, gettext("while unparsing principal name"));
            exit(1);
        }
        printf("%4d ", entry.vno);
        if (show_time) {
    ...
    if (code && code != KRB5_KT_END) {
        com_err(progname, code, gettext("while scanning keytab"));
        exit(1);
    }
    if ((code = krb5_kt_end_seq_get(kcontext, kt, &cursor))) {
        com_err(progname, code, gettext("while ending keytab scan"));
        exit(1);
    }
    exit(0);
}
    ...
    char *name;
{
    ...
    if (name == NULL) {
        ...
        if ((code = krb5_cc_resolve(kcontext, name, &cache))) {
            if (!status_only)
                com_err(progname, code, gettext("while resolving ccache %s"),
                        name);
            exit(1);
        }
    }
    ...
    if ((code = krb5_cc_get_principal(kcontext, cache, &princ))) {
        if (!status_only)
            com_err(progname, code, gettext("while retrieving principal name"));
        exit(1);
    }
    ...
    if (!status_only) {
        ...
        fillit(stdout, timestamp_width - sizeof (gettext("Valid starting")) + 3,
               (int) ' ');
        fputs(gettext("Expires"), stdout);
        fillit(stdout, timestamp_width - sizeof (gettext("Expires")) + 3,
               (int) ' ');
    ...
    if (code == KRB5_CC_END) {
        if ((code = krb5_cc_end_seq_get(kcontext, cache, &cur))) {
            if (!status_only)
                com_err(progname, code,
                        gettext("while finishing ticket retrieval"));
            exit(1);
        }
        flags = KRB5_TC_OPENCLOSE;      /* turns on OPENCLOSE mode */
        if ((code = krb5_cc_set_flags(kcontext, cache, flags))) {
            if (!status_only)
                com_err(progname, code, gettext("while closing ccache"));
            exit(1);
        }
#ifdef KRB5_KRB4_COMPAT
        ...
        if (!status_only)
            com_err(progname, code, gettext("while retrieving a ticket"));
        exit(1);
    }
}
...
{
    /* Solaris Kerberos - may need for space for internationalization */
    static char buf[256];
    krb5_error_code retval;

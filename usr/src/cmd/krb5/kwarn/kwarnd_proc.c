 ...
 */
	...
	if ((config_entry = find_warning_info(argp->warning_name)) == NULL) {
		if (kwarnd_debug)
			printf("kwarn_add_warning_1_svc find_warn_info: "
			    "fails, cWlist=%p\n", cred_warning_list);
		return (TRUE);
	}
 ...
 */
	...
	if (cred_warning = find_cred_warning(argp->warning_name)) {
		...
		cred_warning->cred_warn_time =
		    argp->cred_exp_time - config_entry->seconds_to_warn;
	...
	if (kwarnd_debug)
		printf("kwarn_add_warning_1_svc end: returns true; "
		    "cWlist=%p\n", cred_warning_list);
...
loadConfigFile(void)
{
	...
	if ((cfgfile = fopen(CONF_FILENAME, "r")) == NULL) {
		...
		syslog(LOG_ERR, gettext("could not open config file "
		    "\"%s\"\n"), CONF_FILENAME);
		syslog(LOG_ERR, gettext("using default options "
		    "\"%s\"\n"), DEFAULT_CONFIG);
		def_config = strdup(DEFAULT_CONFIG);
		if (def_config != NULL) {
			retval = parseConfigLine(def_config);
			free(def_config);
		} else {
			retval = FALSE;
		}
...
parseConfigLine(char *buffer)
{
	...
	principal = buffer;
	for (send_to = buffer; *send_to && !isspace(*send_to); send_to++)
		;
	...
	/* find end of send_to */
	for (ends = send_to; *ends && !isspace(*ends); ends++)
		;
	if (*ends)
		*ends = '\0';
	...
	if (strchr(send_to, ':')) {
		...
		*st = '\0';
		...
		if (renew && strchr(st, ',')) {
			while (1) {
				/* loop thru comma separated list-o-opts */
	...
	if (renew) {
		...
		for (send_to = ends+1; *send_to && isspace(*send_to); send_to++)
			;
	...
	/* find first non whitespace after send_to (start of exptime) */
	for (exptime = ends+1; *exptime && isspace(*exptime); exptime++)
		;
	...
	/* find end of emailid */
	if (*emailid) {
		for (ends = emailid; *ends && !isspace(*ends); ends++)
			;
	...
	time_t *new_exp_time) /* out */
{
	...
	if ((code = krb5_parse_name(k5.ctx, princ, &k5.me))) {
	...
	while (1) {
		...
		for (cw = cred_warning_list;
			...
			if (now >= cw->cred_warn_time) {
				...
				/* There is no warning info for this princ */
				if (ce == NULL)
					continue;

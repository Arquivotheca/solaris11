...
static void xusage()
{
    fprintf(stderr, gettext("usage: %s [-C] [-u] [-c ccache] [-e etype]\n"), prog);
...
int main(int argc, char *argv[])
{
    ...
    while ((option = getopt(argc, argv, "uCc:e:hk:qPS:U:")) != -1) {
        ...
        case 'S':
            sname = optarg;
            if (unknown == 1){
                fprintf(stderr, gettext("Options -u and -S are mutually exclusive\n"));
                xusage();
            }
            break;
        case 'u':
    ...
    if (proxy) {
        if (keytab_name == NULL) {
            fprintf(stderr, gettext("Option -P (constrained delegation) "
                    "requires keytab to be specified\n"));
            xusage();
        } else if (for_user == NULL) {
            fprintf(stderr, gettext("Option -P (constrained delegation) requires "
                        ...
                        int proxy)
{
    ...
    if (ret) {
        com_err(prog, ret, gettext("while initializing krb5 library"));
        exit(1);
    }
    ...
    if (etypestr) {
        ...
        if (ret) {
            com_err(prog, ret, gettext("while converting etype"));
            exit(1);
        }
    } else {
        etype = 0;
    ...
    if (ret) {
        com_err(prog, ret, gettext("while opening ccache"));
        exit(1);
    }
    ...
    if (keytab_name) {
        ...
        if (ret) {
            com_err(prog, ret, gettext("resolving keytab %s"), keytab_name);
            exit(1);
        }
    }
    ...
    if (for_user) {
        ret = krb5_parse_name_flags(context, for_user,
                                    ...
                                    &for_user_princ);
        if (ret) {
            com_err(prog, ret, gettext("while parsing principal name %s"), for_user);
            exit(1);
        }
    }
    ...
    if (ret) {
        com_err(prog, ret, gettext("while getting client principal name"));
        exit(1);
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (ret) {
            if (!quiet)
                com_err(prog, ret, gettext("while parsing principal name %s"), names[i]);
            goto error;
        }
        if (unknown == 1) {
        ...
        if (ret) {
            com_err(prog, ret,
                    gettext("while formatting parsed principal name for '%s'"),
                    names[i]);
            goto error;
        }
        ...
        if (for_user) {
            if (!proxy &&
                ...
                com_err(prog, EINVAL,
                        gettext("client and server principal names must match"));
                goto error;
            }
        ...
        if (ret) {
            com_err(prog, ret, gettext("while getting credentials for %s"), princ);
            goto error;
        }
        ...
        if (ret) {
            com_err(prog, ret, gettext("while decoding ticket for %s"), princ);
            goto error;
        }
        ...
        if (keytab) {
            ...
            if (ret) {
                if (!quiet) {
                    fprintf(stderr, gettext("%s: kvno = %d, keytab entry invalid\n"),
                            princ, ticket->enc_part.kvno);
                }
                com_err(prog, ret, gettext("while decrypting ticket for %s"), princ);
                goto error;
            }
            if (!quiet)
                printf(gettext("%s: kvno = %d, keytab entry valid\n"),
                       princ, ticket->enc_part.kvno);
            if (proxy) {
                ...
                if (ret) {
                    com_err(prog, ret,
                            gettext("%s: constrained delegation failed"), princ);
                    goto error;
                }
            }
        } else {

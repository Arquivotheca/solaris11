    ...
    char    **argv;
{
    ...
    if (fd < 0) {
        fprintf(stderr, gettext("%s: %s while opening connection to %s\n"),
    ...
    krb5_context context;
{
     ...
     */
	/* Solaris Kerberos */
	retval = krb5_copy_principal(context, my_principal, &client);
    if (retval) {
	...
	/*
	 * Solaris Kerberos
	 * The pointers clientp and serverp reference the input argument in
	 * creds for get_init_creds and must be freed if the input argument
	 * does not match the output argument, which occurs during a successful
	 * call to get_init_creds.
	 */
	if (client != creds.client)
    ...
    unsigned int     ErrmsgSz;
{
    ...
    if (hp == NULL) {
        (void) snprintf(Errmsg, ErrmsgSz, gettext("%s: unknown host"), host);
        *fd = -1;
        return(0);
    }
    ...
    if (s < 0) {
        (void) snprintf(Errmsg, ErrmsgSz, gettext("in call to socket"));
        return(errno);
    }
    if (connect(s, (struct sockaddr *)&my_sin, sizeof my_sin) < 0) {
        retval = errno;
        close(s);
        (void) snprintf(Errmsg, ErrmsgSz, gettext("in call to connect"));
        return(retval);
    }
    *fd = s;

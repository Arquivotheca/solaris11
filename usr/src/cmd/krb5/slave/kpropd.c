    ...
    char    **argv;
{
    ...
    int iprop_supported; /* Solaris Kerberos */
    krb5_boolean is_master = FALSE; /* Solaris Kerberos */
    ...
    if (log_ctx && (log_ctx->iproprole == IPROP_SLAVE)) {
        ...
        /* Solaris Kerberos: make sure plugin supports iprop */
        ...
        /* Solaris Kerberos */
        tmp_acl_file = fopen(acl_file_name, "rF");
        if (!tmp_acl_file) {
...
retry:
    ...
    if(!port) {
        ...
        if (sp == NULL) {
            com_err(progname, 0, gettext("%s/tcp: unknown service"), KPROP_SERVICE);
    ...
    if ((ret = bind(finet, (struct sockaddr *) &my_sin, sizeof(my_sin))) < 0) {
        if (debug) {
            ...
            if (setsockopt(finet, SOL_SOCKET, SO_REUSEADDR,
                           (char *)&on, sizeof(on)) < 0)
                com_err(progname, errno, gettext("in setsockopt(SO_REUSEADDR)"));
            ret = bind(finet, (struct sockaddr *) &my_sin, sizeof(my_sin));
        }
        if (ret < 0) {
            perror("bind");
            com_err(progname, errno, gettext("while binding listener socket"));
            exit(1);
        }
    }
...
#ifdef PID_FILE
    /* Solaris Kerberos */
    if ((pidfile = fopen(PID_FILE, "wF")) != NULL) {
        fprintf(pidfile, "%d\n", getpid());
        fclose(pidfile);
    } else
        com_err(progname, errno,
                gettext("while opening pid file %s for writing"), PID_FILE);
#endif
    if (listen(finet, 5) < 0) {
    ...
    int     fd;
{
    ...
    if (retval) {
        com_err(progname, retval, gettext("while trying to lock '%s'"),
                temp_file_name);
        exit(1);
    }
    ...
    if (rename(temp_file_name, file)) {
        com_err(progname, errno, gettext("While renaming %s to %s"),
                temp_file_name, file);
        exit(1);
    }
    retval = krb5_lock_file(kpropd_context, lock_fd, KRB5_LOCKMODE_SHARED);
    if (retval) {
        com_err(progname, retval, gettext("while downgrading lock on '%s'"),
                temp_file_name);
        exit(1);
    }
    ...
    if (retval) {
        com_err(progname, retval, gettext("while unlocking '%s'"), temp_file_name);
        exit(1);
    }
    (void)close(lock_fd);
 ...
 * Just call the gen:ed v1 routine should be sufficient.
 */
...
full_resync(CLIENT *clnt)
{
    ...
    return iprop_full_resync_1((void *)&full_resync_arg, clnt);
...
krb5_error_code do_iprop(kdb_log_context *log_ctx)
{
...
#if 0 /* Solaris Kerberos: mem leak fix (PRS already sets def realm) */
    /*
    ...
    if (retval) {
        com_err(progname, retval,
                gettext("while trying to construct host service principal"));
        exit(1);
    }
    ...
    if (krb5_is_referral_realm(krb5_princ_realm(kpropd_context,
        ...
        if (r->data == NULL) {
            com_err(progname, retval,
                    gettext("while determining local service principal name"));
            exit(1);
        }
    ...
    if (retval = krb5_kt_get_entry(kpropd_context, kt, iprop_svc_principal,
        ...
        com_err(progname, retval, gettext("while retrieving entry %s "
                ...
                kt_name[0] ? kt_name : gettext("default keytab"));
...
reinit:
	 ...
	 */
	retval = kadm5_init_with_skey(kpropd_context, iprop_svc_princstr, keytab_name,
				    master_svc_princstr,
				    &params,
    ...
    if (retval) {
        if (retval == KADM5_RPC_ERROR) {
            ...
            /* Solaris Kerberos: our long time test scripts expect fast exit here */
            exit(1);
        }
    }
    ...
    for (;;) {
        ...
        case UPDATE_FULL_RESYNC_NEEDED:
            ...
            case UPDATE_OK:
                ...
                if (ret) {
                    syslog(LOG_WARNING,
                           gettext("kpropd: Full resync, invalid return."));
                    frdone = 0;
                    /*
                     * Exponential backoff immediately after
                     * failure.  If after eight minutes we
                     * still have a failure, output message
                     * for administrator help and exit.
                     *
                     * Solaris Kerberos
                     */
                    backoff_cnt = ++full_resync_fail_count;
                    if (full_resync_fail_count == MAX_ATTEMPTS) {
                        syslog(LOG_ERR,
                               gettext("Repeated full resync "
                                       "failures.  Administrator "
                                       "assistance needed to resolve "
                                       "issue.  Check for existance of "
                                       "Kerberos lock files and delete "
                                       "the files if kpropd and "
                                       "kadmin.local are not running."));
                        exit(ret);
                    }
                } else
                    frdone = 1;
        ...
        case UPDATE_OK:
            ...
            if (retval) {
                ...
                syslog(LOG_ERR,
                       gettext("kpropd: ulog_replay failed (%s), updates not registered."), msg);
        ...
        if (*word == '-') {
            ...
            while (word && (ch = *word++)) {
                ...
                case 'x':
                {
                    ...
                    if (new_db_args == NULL) {
                        com_err(argv[0], errno, gettext("copying db args"));
                        exit(1);
                    }
                    db_args = new_db_args;
                    if (*word)
    ...
    struct sockaddr_in    my_sin;
{
    ...
    if (getsockname(fd, (struct sockaddr *) &r_sin, &sin_length)) {
        com_err(progname, errno, gettext("while getting local socket address"));
        exit(1);
    }
    ...
    if (debug) {
        ...
        if (retval) {
            com_err(progname, retval, gettext("While unparsing client name"));
            exit(1);
        }
    ...
    krb5_data *confmsg;
{
    ...
    if (retval) {
        send_error(context, fd, retval,
                   gettext("while decoding database size"));
    ...
    if (retval) {
        send_error(context, fd, retval,
                   gettext("failed while initializing i_vector"));
...
#define WEXITSTATUS(w) (w).w_retcode
    ...
    /*
     * The filedes and related code is designed to capture any
     * error messages that may be returned from kdb5_util command.
     *
     * Solaris Kerberos
     */
    if (pipe(filedes) != 0) {
        syslog(LOG_ERR, gettext("pipe() failed - error %s"),
               strerror(errno));
        exit(1);
    }
    ...
    case 0:
        ...
        /* Solaris Kerberos */
        if (dup2(filedes[0],2) < 0) {
            syslog(LOG_ERR, gettext("dup2() failed - error %s"),
                   strerror(errno));
            exit(1);
        }
    ...
    default:
        ...
        /*
         * Solaris Kerberos
         * Now we attempt to reap any messages returned by
         * kdb5_util.
         *
         * First, fstat pipe to determine size of message.
         */
        if (fstat(filedes[1], &pipe_info) < 0) {
            syslog (LOG_ERR, gettext("fstat() failed - error %s"),
                    strerror(errno));
            exit(1);
        }
	
        /*
         * If we have data, alloc memory using pipe_info.st_size +1 
         * to take into consideration the null string terminator.
         * Then read the message and print it out.
         * If pipe_info.st_size == zero, no data just exit out.
         */
        if (pipe_info.st_size > 0) {
            if ((kdb5_util_msg = (char *)malloc(pipe_info.st_size + 1))
                == NULL) {
                syslog (LOG_ERR, gettext("malloc() failed -"
                                         " error %s"), strerror(errno));
                exit(1);
            } else {
                (void) memset(kdb5_util_msg, 0, pipe_info.st_size + 1);
                if (read(filedes[1], kdb5_util_msg,
                         pipe_info.st_size) < 0) {
                    syslog (LOG_ERR, gettext("read() failed"
                                             " - error %s"), strerror(errno));
                    free(kdb5_util_msg);
                    exit(1);
                }
                else {
                    /* we reaped kdb5_util message - print it */
                    syslog(LOG_ERR, gettext("kdb5_util message %s"),
                           kdb5_util_msg);
                    free(kdb5_util_msg);
                }
            }
    ...
    if (error_ret) {
        com_err(progname, 0, gettext("%s returned a bad exit status (%d)"),
                kdb_util, error_ret);
        exit(1);
    }
    return;

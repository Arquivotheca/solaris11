    ...
    time_t	RelMonth;
{
    ...
    /*
     * Solaris Kerberos:
     * If today happens to be > day 28 and the relative month requested
     * happens to have less than the current day number Convert() will
     * return an error. The fix: round down the days to the maximum in the
     * relative month requested.
     */
    DaysInMonth[1] = Year % 4 == 0 && (Year % 100 != 0 || Year % 400 == 0)
		    ? 29 : 28;

    if (DaysInMonth[Month - 1] < tm->tm_mday)
        tmp_day = DaysInMonth[Month -1];
    else
        tmp_day = tm->tm_mday;

    ret = Convert(Month, tmp_day, Year,

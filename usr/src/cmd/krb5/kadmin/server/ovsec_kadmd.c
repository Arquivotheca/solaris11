...
/* Solaris Kerberos: so this code will compile */
...
{
    fprintf(stderr,
            gettext("Usage: kadmind [-x db_args]* [-r realm] [-m] [-nofork] [-d]** "
...
#define	TEXT_DOMAIN	"SYS_TEST"	/* Use this only if it weren't */
    ...
    while (argc) {
        if (strcmp(*argv, "-x") == 0) {
            ...
            params.mask |= KADM5_CONFIG_MKEY_FROM_KBD;
                   ...
                   /* Solaris Kerberos: -d deprecated by MIT */
                   (strcmp(*argv, "-d") == 0)) {
            nofork = 1;
            ...
            kadm5_set_use_password_server ();
                   ...
                   /* Solaris Kerberos: -p deprecated by MIT */
                   (strcmp(*argv, "-p") == 0)) {
            argc--; argv++;
            if(!argc)
                usage();
    ...
    if (argc != 0)
     ...
     /* Solaris Kerberos */
     if((ret = kadm5_init2(context, "kadmind", NULL,
			  NULL, &params,
...
kterr:
	...
	if (ret = krb5_klog_list_logs(whoami)) {
     ...
     if (pid_file != NULL) {
	    ...
	    if (ret) {
		    errmsg = krb5_get_error_message(context, ret);
		    krb5_klog_syslog(LOG_ERR,
				    gettext("Cannot create PID file %s: %s"),
				    pid_file, errmsg);
		    kadm5_destroy(global_server_handle);
		    krb5_klog_close(context);
		    exit(1);
	...
	if (log_ctx && (log_ctx->iproprole == IPROP_MASTER)) {
		 ...
		 */
		if (ret = ulog_map(ctx,
				params.iprop_logfile, params.iprop_ulogsize,
				FKADMIND, db_args)) {
			fprintf(stderr,

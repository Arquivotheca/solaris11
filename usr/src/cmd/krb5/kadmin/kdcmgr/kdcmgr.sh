...
if [[ -z $master && -z $slave ]]; then
	query "$(gettext "Is this machine to be configured as a master?"): "
	master=$answer
	
	if [[ $answer = no ]]; then
		query "$(gettext "Is this machine to be configured as a slave?"): "
		slave=$answer
		if [[ $answer = no ]]; then
...
if [[ $slave = yes && -z $master_hn ]]; then
	printf "$(gettext "What is the master KDC's host name?"): "
	read host
	checkval="MASTER"; check_value $host
	check_host
	master_hn=$host

...
char *str_HISTORY_PARSE_NAME = gettext("while parsing admin history principal name.");

char *str_ADMIN_PRINC_EXISTS = gettext("Warning! Admin principal already exists.");

char *str_CHANGEPW_PRINC_EXISTS = gettext("Warning! Changepw principal already exists.");

char *str_HISTORY_PRINC_EXISTS = gettext("Warning! Admin history principal already exists.");
...
char *str_CREATED_PRINC_DB =
    gettext("%s: Admin principal database created (or it already existed).\n"); /* whoami */

char *str_CREATED_POLICY_DB =
    gettext("%s: Admin policy database created (or it already existed).\n"); /* whoami */
...
char *str_CREATING_POLICY_DB = gettext("while creating/opening admin policy database.");

char *str_CLOSING_POLICY_DB = gettext("while closing admin policy database.");

char *str_CREATING_PRINC_DB = gettext("while creating/opening admin principal database.");

char *str_CLOSING_PRINC_DB = gettext("while closing admin principal database.");

char *str_CREATING_PRINC_ENTRY =
    gettext("while creating admin principal database entry for %s."); /* princ_name */

char *str_A_PRINC = gettext("a principal");

char *str_UNPARSE_PRINC = gettext("while unparsing principal.");

char *str_CREATED_PRINC = gettext("%s: Created %s principal.\n"); /* whoami, princ_name */
...
char *str_INIT_RANDOM_KEY = gettext("while initializing random key generator.");
...
char *str_TOO_MANY_CHANGEPW_PRINC =
    gettext("while fetching changepw princ. Can only have one changepw principal.");

char *str_TOO_MANY_HIST_PRINC =
    gettext("while fetching history princ. Can only have one history principal.");

char *str_WHILE_DESTROYING_ADMIN_SESSION = gettext("while closing session with admin server and destroying tickets.");

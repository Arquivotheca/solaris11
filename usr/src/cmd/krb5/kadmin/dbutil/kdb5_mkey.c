...
kdb5_add_mkey(int argc, char *argv[])
{
    ...
    krb5_timestamp now;
    kdb_log_context *log_ctx; /* Solaris Kerberos: iprop */
    ...
    while ((optchar = getopt(argc, argv, "e:s")) != -1) {
        ...
        case 'e':
            if (krb5_string_to_enctype(optarg, &new_master_enctype)) {
                /* Solaris Kerberos */
                com_err(progname, EINVAL, gettext("%s is an invalid enctype"),
                    optarg);
                exit_status++;
                return;
            }
    ...
    if (retval != 0) {
        ...
        com_err(progname, retval,
                gettext("while getting master key principal %s"),
                mkey_fullname);
        exit_status++;
        goto cleanup_return;
    } else if (nentries == 0) {
        com_err(progname, KRB5_KDB_NOENTRY,
                gettext("principal %s not found in Kerberos database"),
                mkey_fullname);
        exit_status++;
        goto cleanup_return;
    } else if (nentries > 1) {
        ...
        com_err(progname, KRB5KDC_ERR_PRINCIPAL_NOT_UNIQUE,
                gettext("principal %s has multiple entries in Kerberos "
    ...
    /* Solaris Kerberos */
    printf(gettext("Creating new master key for master key principal '%s'\n"),
           mkey_fullname);

    /* Solaris Kerberos */
    printf(gettext("You will be prompted for a "
        "new database Master Password.\n"));
    /* Solaris Kerberos */
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("while reading new master key from keyboard"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("adding new master key to master principal"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if ((retval = krb5_timeofday(util_context, &now))) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("while getting current time"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if ((retval = krb5_dbe_update_mod_princ_data(util_context, &master_entry,
        ...
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("while updating the master key principal "
                "modification time"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if ((retval = krb5_db_put_principal(util_context, &master_entry, &nentries))) {
        ...
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("while adding master key entry to the database"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if (do_stash) {
        /*
         * Solaris Kerberos: the MIT code was broken here, the right way to do
         * this is stash all master keys including the latest mkey.
         */
        retval = krb5_db_fetch_mkey_list(util_context,
                                         master_princ,
                                         &master_keyblock,
                                         &master_keylist);
        if (retval) {
            /* Solaris Kerberos */
            com_err(progname, errno, gettext("while getting master key list"));
            printf(gettext("Warning: couldn't stash master key.\n"));
            exit_status++;
            goto cleanup_return;
        }

        retval = krb5_db_store_master_key_list(util_context,
        ...
        if (retval) {
            /* Solaris Kerberos */
            com_err(progname, errno, gettext("while storing master keys"));
            printf(gettext("Warning: couldn't stash master key.\n"));
        }
    }
...
kdb5_use_mkey(int argc, char *argv[])
{
    ...
    if (use_kvno == 0) {
        /* Solaris Kerberos */
        com_err(progname, EINVAL, gettext("0 is an invalid KVNO value"));
        exit_status++;
        return;
    } else {
        ...
        if (!keylist_node) {
            /* Solaris Kerberos */
            com_err(progname, EINVAL, gettext("%d is an invalid KVNO value"),
                use_kvno);
            exit_status++;
            return;
        }
    ...
    if (argc == 3) {
        ...
        if (t == -1) {
            /* Solaris Kerberos */
            com_err(progname, 0,
                    gettext("could not parse date-time string '%s'"),
                    argv[2]);
            exit_status++;
            return;
        } else
    ...
    if (retval != 0) {
        ...
        /* Solaris Kerberos */
        com_err(progname, KRB5KDC_ERR_PRINCIPAL_NOT_UNIQUE,
                gettext("principal %s has multiple entries "
    ...
    if (retval != 0) {
        /* Solaris Kerberos */
        com_err(progname, retval,
                gettext("while looking up active version of master key"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if (!inserted) {
        ...
        if (new_actkvno == NULL) {
            /* Solaris Kerberos */
            com_err(progname, ENOMEM, gettext("while adding new master key"));
            exit_status++;
            goto cleanup_return;
        }
    ...
    if (actkvno_list->act_time > now) {
        ...
        com_err(progname, EINVAL,
            gettext("there must be one master key currently active"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if ((retval = krb5_dbe_update_actkvno(util_context, &master_entry,
        ...
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("while updating actkvno data for master principal entry"));
        exit_status++;
        goto cleanup_return;
    }
...
kdb5_list_mkeys(int argc, char *argv[])
{
    ...
    if (master_keylist == NULL) {
        /* Solaris Kerberos */
        com_err(progname, 0, gettext("master keylist not initialized"));
        exit_status++;
        return;
    }
    ...
    if (retval != 0) {
        ...
        /* Solaris Kerberos */
        com_err(progname, KRB5KDC_ERR_PRINCIPAL_NOT_UNIQUE,
                gettext("principal %s has multiple entries in "
                    "Kerberos database"),
                mkey_fullname);
        exit_status++;
        goto cleanup_return;
    ...
    if (retval != 0) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("while looking up active kvno list"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if (actkvno_list == NULL) {
        ...
        if (retval == KRB5_KDB_NOACTMASTERKEY) {
            ...
            /* Solaris Kerberos */
            com_err(progname, retval,
                gettext("while looking up active master key"));
            exit_status++;
            goto cleanup_return;
        }
    }
    ...
    /* Solaris Kerberos */
    printf(gettext("Master keys for Principal: %s\n"), mkey_fullname);
    ...
    for (cur_kb_node = master_keylist; cur_kb_node != NULL;
        ...
        if ((retval = krb5_enctype_to_string(cur_kb_node->keyblock.enctype,
            ...
            /* Solaris Kerberos */
            com_err(progname, retval,
                gettext("while getting enctype description"));
            exit_status++;
            goto cleanup_return;
        }
        ...
        if (actkvno_list != NULL) {
            ...
            if ((retval = krb5_timeofday(util_context, &act_time))) {
                /* Solaris Kerberos */
                com_err(progname, retval,
                    gettext("while getting current time"));
                exit_status++;
                goto cleanup_return;
            }
        }
        ...
        if (cur_kb_node->kvno == act_kvno) {
            ...
            retval = asprintf(&output_str,
                gettext("KNVO: %d, Enctype: %s, Active on: %s *\n"),
            ...
            if (act_time != -1) {
                ...
                /* Solaris Kerberos */
                retval = asprintf(&output_str,
                    gettext("KNVO: %d, Enctype: %s, No activate time set\n"),
        ...
        if (retval == -1) {
            ...
            com_err(progname, ENOMEM,
                gettext("asprintf could not allocate enough memory "
                "to hold output"));
            exit_status++;
            goto cleanup_return;
        }
...
update_princ_encryption_1(void *cb, krb5_db_entry *ent)
{
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
                gettext("getting string representation of principal name"));
        goto fail;
    }
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
                gettext("determining master key used for principal '%s'"),
                pname);
        goto fail;
    }
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
                gettext("error re-encrypting key for principal '%s'"), pname);
        goto fail;
    }
    if ((retval = krb5_timeofday(util_context, &now))) {
        /* Solaris Kerberos */
    ...
    if ((retval = krb5_dbe_update_mod_princ_data(util_context, ent,
        ...
        /* Solaris Kerberos */
        com_err(progname, retval,
                gettext("while updating principal '%s' "
                "modification time"), pname);
        goto fail;
    }
    ...
    if ((retval = krb5_db_put_principal(util_context, ent, &nentries))) {
        /* Solaris Kerberos */
        com_err(progname, retval,
                gettext("while updating principal '%s' key data "
                "in the database"),
                pname);
        goto fail;
    }
...
kdb5_update_princ_encryption(int argc, char *argv[])
{
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("while formatting master principal name"));
        exit_status++;
        goto cleanup;
    }
    ...
    if (master_keylist == NULL) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("master keylist not initialized"));
        exit_status++;
        goto cleanup;
    }
    ...
    if (glob_to_regexp(name_pattern, "hi", &regexp) != 0) {
        /* Solaris Kerberos */
        com_err(progname, ENOMEM,
                gettext("converting glob pattern '%s' to regular expression"),
                name_pattern);
        exit_status++;
        goto cleanup;
    }
    ...
    if (
        ...
        /* Solaris Kerberos */
        com_err(progname, 0,
            gettext("error compiling converted regexp '%s'"), regexp);
        exit_status++;
        goto cleanup;
    }
    ...
    if (nentries != 1) {
        /* Solaris Kerberos */
        com_err(progname, 0,
                gettext("cannot find master key principal %s in database!"),
                mkey_fullname);
        exit_status++;
        goto cleanup;
    }
    ...
    if (retval) {
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("retrieving the most recent master key"));
        exit_status++;
        goto cleanup;
    }
    ...
    if (!force &&
        !data.dry_run &&
        !are_you_sure(gettext("Re-encrypt all keys not using "
                      "master key vno %u?"),
                      new_mkvno)) {
        /* Solaris Kerberos */
        printf(gettext("OK, doing nothing.\n"));
        exit_status++;
        goto cleanup;
    }
    if (data.verbose) {
        if (data.dry_run)
            /* Solaris Kerberos */
            printf(gettext("Principals whose keys WOULD BE re-encrypted "
                   "to master key vno %u:\n"),
                   new_mkvno);
        else
            /* Solaris Kerberos */
            printf(gettext("Principals whose keys are being re-encrypted "
                   "to master key vno %u if necessary:\n"),
                   new_mkvno);
    }

    retval = krb5_db_iterate(util_context, name_pattern,
                             update_princ_encryption_1, &data,
                             NULL /* Solaris Kerberos */);
    /* If exit_status is set, then update_princ_encryption_1 already
       printed a message.  */
    if (retval != 0 && exit_status == 0) {
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("trying to process principal database"));
        exit_status++;
    }
    (void) krb5_db_fini(util_context);
    if (data.dry_run)
        /* Solaris Kerberos */
        printf(gettext("%u principals processed: %u would be updated, %u "
               "already current\n"),
               data.re_match_count, data.updated, data.already_current);
    else
        /* Solaris Kerberos */
        printf(gettext("%u principals processed: %u updated, %u "
               "already current\n"),
               data.re_match_count, data.updated, data.already_current);
...
kdb5_purge_mkeys(int argc, char *argv[])
{
    ...
    if (!force) {
        /* Solaris Kerberos */
        printf(gettext("Will purge all unused master keys stored in the '%s' "
        ...
        /* Solaris Kerberos */
        if (strcmp(buf, gettext("yes\n"))) {
            exit_status++;
            goto cleanup_return;
        }
        /* Solaris Kerberos */
        printf(gettext("OK, purging unused master keys from '%s'...\n"),
            mkey_fullname);
    }
    ...
    if (old_key_data_count == 1) {
        if (verbose)
            /* Solaris Kerberos */
            printf(gettext("There is only one master key which can not be "
                "purged.\n"));
        goto cleanup_return;
    }
    old_key_data = master_entry.key_data;
    ...
    if (args.kvnos == NULL) {
        retval = ENOMEM;
        /* Solaris Kerberos */
        com_err(progname, ENOMEM, gettext("while allocating args.kvnos"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if ((retval = krb5_db_iterate(util_context,
                                  ...
                                  NULL /* Solaris Kerberos */))) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("while finding master keys in use"));
        exit_status++;
        goto cleanup_return;
    }
    /*
    ...
    if (dry_run)
        /* Solaris Kerberos */
        printf(gettext("Would purge the follwing master key(s) from %s:\n"),
            mkey_fullname);
    else
        /* Solaris Kerberos */
        printf(gettext("Purging the follwing master key(s) from %s:\n"),
            mkey_fullname);
        ...
        if (args.kvnos[i].use_count > 0) {
            ...
            if (args.kvnos[i].kvno == master_kvno) {
                ...
                com_err(progname, KRB5_KDB_STORED_MKEY_NOTCURRENT,
                        gettext("master key stash file needs updating, "
                        "command aborting"));
                exit_status++;
                goto cleanup_return;
            }
    ...
    if (num_kvnos_inuse == args.num_kvnos) {
        /* Solaris Kerberos */
        printf(gettext("All keys in use, nothing purged.\n"));
        goto cleanup_return;
    }
    if (dry_run) {
        /* bail before doing anything else */
        /* Solaris Kerberos */
        printf(gettext("%d key(s) would be purged.\n"), num_kvnos_purged);
        goto cleanup_return;
    }
    ...
    if (retval != 0) {
        /* Solaris Kerberos */
        com_err(progname, retval,
            gettext("while looking up mkey aux data list"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if (master_entry.key_data == NULL) {
        retval = ENOMEM;
        /* Solaris Kerberos */
        com_err(progname, ENOMEM, gettext("while allocating key_data"));
        exit_status++;
        goto cleanup_return;
    }
    ...
    if ((retval = krb5_dbe_update_actkvno(util_context, &master_entry,
                                          actkvno_list))) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("while updating actkvno data for "
    ...
    if ((retval = krb5_dbe_update_mkey_aux(util_context, &master_entry,
                                           mkey_aux_list))) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("while updating mkey_aux data for "
            "master principal entry"));
        exit_status++;
        return;
    }
    ...
    if ((retval = krb5_dbe_update_mod_princ_data(util_context, &master_entry,
                                                 now, master_princ))) {
        /* Solaris Kerberos */
        com_err(progname, retval, gettext("while updating the master key "
            "principal modification time"));
        exit_status++;
        goto cleanup_return;
    }

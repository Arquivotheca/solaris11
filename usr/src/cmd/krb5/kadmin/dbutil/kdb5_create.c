    ...
    char *argv[];
{
    ...
    int strong_random = 1;
#endif
    /* Solaris Kerberos - Disable "-W" option */
    ...
    if (retval) {
        com_err (progname, retval, gettext("Loading random data"));
        exit_status++; return;
    }
#endif
...
master key name '%s'\n"),
    ...
    if ((retval = krb5_c_random_seed(util_context, &seed))) {
        com_err(progname, retval, gettext("while initializing random key generator"));
        exit_status++; return;
    }
    if ((retval = krb5_db_create(util_context,
                                 db5util_db_args))) {
        com_err(progname, retval, gettext("while creating database '%s'"),

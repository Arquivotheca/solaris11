...
#if 0 /* Solaris Kerberos - not used */
    ...
    char *file_name;
{
    ...
    if ((fd = open(file_ok, O_WRONLY|O_CREAT|O_TRUNC, 0600)) < 0) {
        com_err(progname, errno, gettext("while creating 'ok' file, '%s'"),
                file_ok);
        exit_status++;
        free(file_ok);
        return;
    ...
    if (write(fd, "", 1) != 1) {
        com_err(progname, errno, gettext("while writing to 'ok' file, '%s'"),
                file_ok);
        exit_status++;
        free(file_ok);
        return;
...
static krb5_error_code dump_ov_princ(krb5_pointer ptr, krb5_db_entry *kdb)
{
    ...
    for (x = 0; x < adb.old_key_len; x++) {
        ...
        for (y = 0; y < adb.old_keys[x].n_key_data; y++) {
            ...
            if (foundcrc) {
                fprintf(stderr,
                        gettext("Warning!  Multiple DES-CBC-CRC keys "
        ...
        if (!foundcrc)
            fprintf(stderr,
                    gettext("Warning!  No DES-CBC-CRC key for principal "
    ...
    if (mkey_convert) {
        ...
        if (new_mkey_file) {
            ...
            if ((retval = krb5_db_fetch_mkey(util_context, master_princ,
                                             ...
                                             NULL,
                                             &new_master_keyblock))) {
                com_err(progname, retval, gettext("while reading new master key"));
                exit(1);
            }
        } else {
    ...
    if (nmatched == 6) {
        ...
        if ((name = (char *) malloc((size_t) (name_len + 1))) &&
            ...
            if ((pkey->key_data_length[0] > 4)
                ...
                /* Solaris Kerberos - check malloc retval */
                if (shortcopy) {
    ...
    int                 *linenop;
{
    ...
    if (nread == EOF)
        ...
        fprintf(stderr, gettext("cannot parse policy on line %d (%d read)\n"),
                *linenop, nread);
        return 1;
    }
    ...
    int                 *linenop;
{
    ...
    if (strcmp(rectype, "princ") == 0)
        ...
        fprintf(stderr, gettext("unknown record type \")%s\" on line %d\n"),
                rectype, *linenop);
        return 1;
    }
    ...
    char        **argv;
{
    ...
    if (load->updateonly && !(flags & FLAG_UPDATE)) {
        fprintf(stderr,
                gettext("%s: dump version %s can only be loaded with the"
                        " -update flag\n"),
                        progname, load->name);
        exit_status++;
        return;
    ...
    if (! (flags & FLAG_UPDATE)) {
        ...
        if ((kret = kadm5_get_config_params(kcontext, 1,
            ...
            com_err(progname, kret,
                    gettext("while retreiving new configuration parameters"));
            exit_status++;
            return;
        }

        if (!add_db_arg("temporary")) {
            com_err(progname, ENOMEM, gettext("computing parameters for database"));
            exit(1);
        }
    ...
    /*
     * Solaris Kerberos:
     * Instead of calling kadm5_create_magic_princs(), call the relevant
     * functions directly and avoid calling kadm5_destroy() until we exit
     * the function otherwise the db will be cleaned up prematurely.
     */
#if 0
    ...
    if (!(flags & FLAG_UPDATE) && load->create_kadm5) {
        ...
        if ((kret = kadm5_init(kcontext, progname, NULL, NULL, &newparams,
            ...
            exit_status++;
             ...
             kret = add_admin_princs(handle, kcontext, newparams.realm);
             if (kret != 0) {
                 fprintf(stderr,
                     gettext("%s: cannot create default principals\n"),
                     progname);
                 exit_status++;
             }
         }

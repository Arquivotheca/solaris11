...
function discover_domain {
	...
	if [[ -z $realm ]]; then
		set -A DOMs -- `$KLOOKUP _ldap._tcp.dc._msdcs V`
	else
		set -A DOMs -- `$KLOOKUP _ldap._tcp.dc._msdcs.$realm V`
	fi
...
function setSMB {
	...
	domProp=smb/ad_domain
	secProp=smb/security

	svccfg -s $smbFMRI setprop $domProp=$domain

...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	code = k5_profile_init(file, &profile);
	if (code != 0) {
	...
	code = k5_profile_add_realm(profile, realm, master, ret_values,
	    set_change, TRUE);
	if (code != 0) {
	...
	code = k5_profile_add_domain_mapping(profile, domain, realm);
	if (code != 0) {
...
error:
	 ...
	 */
	if ((code =  k5_profile_release(profile)) != NULL)

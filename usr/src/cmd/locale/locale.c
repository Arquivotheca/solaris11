...
#define	CHARMAP_DIRLEN	(sizeof (CHARMAP_DIR))
#define	CHARMAP_NAME	"charmap."
#define	CHARMAP_NAMELEN	(sizeof (CHARMAP_NAME))
...
static void	print_all_charmaps();
...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	if (aflag) {
		c = localelist(&lp, LCLIST_QUERY | LCLIST_INCLUDE_LC_MESSAGES |
		    LCLIST_VALIDATE | LCLIST_KEEP);

		for (aflag = 0; aflag < c; aflag++)
			(void) printf("%s\n", lp[aflag].locale);
...
/*
 * print_all_charmaps(): Print out all the charmaps supported by the system
 */
static void
print_all_charmaps()
{
	...
	(void) memcpy(filename, CHARMAP_DIR, CHARMAP_DIRLEN);
	...
	p = filename + CHARMAP_DIRLEN - 1;

	while ((dep = readdir(dp)) != NULL) {
		(void) strcpy(p, dep->d_name);
		if (stat(filename, &stbuf) < 0)
			continue;

		if (S_ISDIR(stbuf.st_mode) && dep->d_name[0] != '.') {
			if ((dc = opendir(filename)) == NULL)
				exit(0);

			c = p + strlen(dep->d_name);
			*c++ = '/';

			while ((dec = readdir(dc)) != NULL) {
				(void) strcpy(c, dec->d_name);
				if (stat(filename, &stbuf) < 0)
					continue;

				if (S_ISREG(stbuf.st_mode) &&
				    (strncmp(c, CHARMAP_NAME,
				    CHARMAP_NAMELEN - 1) == 0))
					(void) printf("%s\n", p);
			}
 ...
 * The standard says that any control character shall be preceded by

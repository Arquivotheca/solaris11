...
tree_builder(void *arg)
{
	...
	for (;;) {
		...
		while (rebuild_tree == B_FALSE) {
			...
			to.tv_sec = TREE_BUILDER_TIMER;
			to.tv_nsec = 0;
			(void) cond_reltimedwait(&rebuild_tree_cv,
			    &rebuild_tree_lock, &to);
...
read_volprop(ptree_rarg_t *parg, void *buf)
{
	...
	case VPT_BINSENSOR:
		if (propval == ST_TRUE) {
			char	*oid = OID_sunPlatBinarySensorInterpretTrue;
			ret = snmp_get_str(hdl, oid, vol_props[ndx].row,
			    &pstr, &snmp_syserr);
			if (snmp_syserr == ECANCELED) {
				(void) rw_unlock(&stale_tree_rwlp);
				if (pstr)
			...
			char	*oid = OID_sunPlatBinarySensorInterpretFalse;
			ret = snmp_get_str(hdl, oid, vol_props[ndx].row,
			    &pstr, &snmp_syserr);
			if (snmp_syserr == ECANCELED) {
				(void) rw_unlock(&stale_tree_rwlp);

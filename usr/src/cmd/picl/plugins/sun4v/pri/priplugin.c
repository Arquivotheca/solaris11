...
pri_worker_thread(void *arg)
{
	...
	while (1) {
		...
		/*
		 * If there is a PRI update, make updates to the CPU nodes
		 * in the PICL tree if the MD has changed.
		 */
		cpu_update_info();
	}
...
{
	(void) picld_plugin_register(&priplugin_reg);
}

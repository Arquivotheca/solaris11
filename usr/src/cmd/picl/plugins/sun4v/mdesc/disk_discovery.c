...
{
	char	devname[256];
	char	*diskname, *dev_cpy, *temp_ptr;
	char	apid_name[CFGA_AP_LOG_ID_LEN];
	...
	if (diskname != NULL) {
		if ((temp_ptr = strrchr(diskname, 's')) != NULL) {
			*temp_ptr = '\0';
		}
		dev_cpy = strdup(diskname);
		if (dev_cpy != NULL)    {
			if ((temp_ptr = strchr(dev_cpy, 't')) == NULL) {
				free(dev_cpy);
				free(diskname);
				return (NULL);
			}
			*temp_ptr = '\0';
			(void) snprintf(apid_name, sizeof (apid_name),
			    "%s::dsk/%s", dev_cpy, diskname);
			free(dev_cpy);
			free(diskname);

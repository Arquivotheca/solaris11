...
libdevinfo_init(picl_nodehdl_t rooth)
{
	...
	if (is_snapshot_stale(di_root)) {
		...
		di_fini(di_root);
		if ((di_root = di_init("/",
		    DINFOCPYALL | DINFOFORCE | DINFOPATH)) == DI_NODE_NIL) {
			return (PICL_FAILURE);
		}
	}
...
{
	target_data_t	*td = (target_data_t *)arg;
	char		*path;
	...
	if (strcmp(td->target_path, path) == 0) {
...
static di_node_t
find_target_node(di_node_t root_node, char *path)
{
	target_data_t	td;

	td.target_path = path;
	td.target_node = DI_NODE_NIL;
	(void) di_walk_node(root_node, DI_WALK_CLDFIRST, &td,
	    i_find_target_node);
	return (td.target_node);
}

static int
picldevtree_update(char *ptreepath,  char *dipath)
{
	di_node_t	devnode;
	di_node_t	di_root;
	char		*strp;
	...
	if (parh == plafh) {
		ph = di_prom_init();
		/*
		 * take a full snapshot (from "/") so that all multipath nodes
		 * are available.
		 */
		devnode = DI_NODE_NIL;
		di_root = di_init("/", DINFOCPYALL | DINFOPATH);
		if (di_root != DI_NODE_NIL)
			devnode = find_target_node(di_root, dipath);
    ...
    void *cookie)
{
	...
	if (strcmp(ename, PICLEVENT_SYSEVENT_DEVICE_ADDED) == 0) {
		/*
		 * If the node already exists delete it and then add it back
		 * to get the latest properties.
		 */
		if (ptree_get_node_by_path(ptreepath, &nodeh) ==
		    PICL_SUCCESS) {
		...
		if (picldevtree_update(ptreepath, dipath) < 0)
			return;
		...
		if ((ptree_get_propval_by_name(nodeh,
			...
			(void) ptree_destroy_node(nodeh);
		/*
		 * This will add the node back to the picl tree if it is still
		 * in the kernel's tree just like if picl was restarted at
		 * this point.
		 */
		(void) picldevtree_update(ptreepath, dipath);
	}
done:
	(void) setup_cpus(plafh);
	(void) add_ffb_config_info(plafh);

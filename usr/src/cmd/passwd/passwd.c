 ...
 * No special authorization is needed to change password or display
 * password attributes corresponding to the login name.
 */
...
#define	NOCHNG	11	/* Password information unchanged */
...
#define	MSG_NO_CHANGE	"Password information unchanged.\n"
#define	MSG_USE_USERMOD	"Use usermod -d to change homedir.\n"
...
static char		*real_usrname;	/* real username of this program */
...
static	int		check_auth(const char *auth);
static	int		check_manage_auth(char *);
static	int		ckarg(int argc, char **, attrlist **);
static  boolean_t 	change_authorized(char *, pwu_repository_t *);
static  int 		ckauth(void);
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	if (flag == 0) {			/* changing user password */
		...
		/*
		 * User can change his own password without
		 * additional authorization.
		 */
		if (strcmp(usrname, real_usrname) != 0 &&
		    !change_authorized(usrname, &repository)) {
			passwd_exit(NOPERM);
		}

		/* Don't check account expiration for authorized user */
		if (ckauth() != SUCCESS) {
		...
		tries = 1;
		if (check_auth(PASSWD_ASSIGN_AUTH) == SUCCESS) {
		...
		/* Check authorization to change attributes */
		if ((flag & NONAGEFLAG) &&
		    (check_manage_auth(usrname) != SUCCESS)) {
			passwd_exit(NOPERM);
		}
		...
		case HFLAG:
			...
			(void) printf(gettext(MSG_USE_USERMOD));
			exit(NOCHNG);
			/* NOTREACHED */
			break;
		}
...
ckarg(int argc, char **argv, attrlist **attributes)
{
	...
	while ((opt = getopt(argc, argv, "r:aldefghsux:n:w:N")) != EOF) {
		...
		case 'd': /* Delete Auth Token */
			...
			/*
			 * Delete the password - only authorized users
		...
		case 'N': /* set account to be "no login" */
			...
			if ((IS_FILES(repository) || IS_LDAP(repository)) &&
			    ((retval = check_auth(PASSWD_ASSIGN_AUTH)
			    != SUCCESS)))
				return (FAIL);
		...
		case 'l': /* lock the password */
			 ...
			 */
			if ((IS_FILES(repository) || IS_LDAP(repository)) &&
			    ((retval = check_auth(ACCOUNT_SETPOLICY_AUTH))
			    != SUCCESS))
				return (FAIL);
			if (flag & (MUTEXFLAG|NONAGEFLAG)) {
				rusage();	/* exit */
		...
		case 'x': /* set the max date */
			 ...
			 */
			if ((IS_FILES(repository) || IS_LDAP(repository)) &&
			    (check_auth(ACCOUNT_SETPOLICY_AUTH) != SUCCESS)) {
				retval = NOPERM;
				return (FAIL);
			}
		...
		case 's': /* display password attributes */
			...
			/*
			 * Authorized users can execute this
			 * for FILES or LDAP
			 */
			if ((IS_FILES(repository) || IS_LDAP(repository)) &&
			    ((retval = check_auth(ACCOUNT_SETPOLICY_AUTH))
		...
		case 'e': /* change login shell */
			...
			/*
			 * The user name is not yet available, so the check
			 * between solaris.{user,role}.manage is made where
 ...
 *
 * check_auth():
 *	This function returns SUCCESS if the caller has
 *	the authorization auth, else it returns NOPERM.
 */

static int
check_auth(const char *auth)
{
	if (chkauthattr(auth, real_usrname) == 1) {
		return (SUCCESS);
	}
...
/*
 * Check if user has any of the authorization to change
 * password fields.
 */
static int
ckauth()
{
	if ((chkauthattr(ACCOUNT_ACTIVATE_AUTH, real_usrname) == 1) ||
	    (chkauthattr(ACCOUNT_SETPOLICY_AUTH, real_usrname) == 1) ||
	    (chkauthattr(USER_MANAGE_AUTH, real_usrname) == 1) ||
	    (chkauthattr(ROLE_MANAGE_AUTH, real_usrname) == 1) ||
	    (chkauthattr(PASSWD_ASSIGN_AUTH, real_usrname) == 1)) {
		return (SUCCESS);
	}
	...
	while (attributes) {
		...
		case ATTR_PASSWD:
			...
			if (passwd == NULL || *passwd == '\0') {
				...
				status = "NL  ";
			} else if (strncmp(passwd, UNINITPW,
			    sizeof (UNINITPW)-1) == 0) {
				status = "UP  ";
...
static boolean_t
change_authorized(char *username, pwu_repository_t *repo)
{
	int	res;
	int	authorized;
	attrlist *attributes = NULL;

	/* solaris.passwd.assign allows all passwd changes */
	if (chkauthattr(PASSWD_ASSIGN_AUTH, real_usrname) == 1) {
		return (B_TRUE);
	}

	/* If repository is not specified, default to nsswitch.conf */
	if (repo->type == NULL) {
		repo = PWU_DEFAULT_REP;
	}

	attrlist_add(&attributes, ATTR_PASSWD, NULL);
	res = __get_authtoken_attr(username, repo, attributes);

	if (res != PWU_SUCCESS) {
		return (B_FALSE);
	}

	/* Check if the account has an uninitialized password */
	if (attributes->data.val_s != NULL &&
	    strcmp(attributes->data.val_s, UNINITPW) == 0 &&
	    (chkauthattr(ACCOUNT_ACTIVATE_AUTH, real_usrname) == 1)) {
		authorized = B_TRUE;
	} else {
		authorized = B_FALSE;
	}
...
/*
 * Check if the calling program is authorized to change
 * attributes of user as specified by username.
 * This function returns SUCCESS if the caller has
 * the authorization auth, else it returns NOPERM.
 */
static int
check_manage_auth(char *username)
{
	userattr_t *ua = NULL;
	char *val;
	char *auth;
	...
	if (ua != NULL && ua->attr != NULL &&
		...
		auth = ROLE_MANAGE_AUTH; /* type is role */
	} else {
		auth = USER_MANAGE_AUTH; /* type is user */
	}

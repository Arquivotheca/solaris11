...
#define	LINKCOLWIDTH		20
#define	ZONECOLWIDTH		10
...
typedef struct physinfo {
	datalink_id_t		pli_linkid;
	char			pli_linkname[MAXLINKNAMELEN];
	char			pli_media[DLADM_STRSIZE];
	dladm_phys_attr_t	*pli_physattr;
	uint_t			pli_vanity_index;
} physinfo_t;
...
typedef struct show_state {
	...
	boolean_t	ls_hwgrp;
	boolean_t	ls_loc;
	physinfo_t	*ls_physinfo;
	uint32_t	ls_physinfocnt;
	uint32_t	ls_physinfoindex;
	zoneid_t	*ls_zones;
	uint32_t	ls_zonecnt;
} show_state_t;
...
typedef struct show_vnic_state {
	...
	char		vs_vnic[MAXLINKNAMESPECIFIER];
	char		vs_link[MAXLINKNAMESPECIFIER];
...
static ofmt_cb_t print_phys_one_hwgrp_cb, print_phys_one_loc_cb;
static ofmt_cb_t print_wlan_attr_cb, print_wifi_status_cb, print_link_attr_cb;
...
static cmdfunc_t do_rename_link, do_delete_phys, do_init_phys, do_reinit_phys;
...
static int	show_linkprop_onelink(dladm_handle_t, dladm_datalink_info_t *,
		    void *);
...
static dladm_status_t	print_iptun(dladm_handle_t, dladm_datalink_info_t *,
			    show_state_t *);
static int	print_iptun_walker(dladm_handle_t, dladm_datalink_info_t *,
		    void *);

static int	show_etherprop(dladm_handle_t, dladm_datalink_info_t *, void *);
...
static cmd_t	cmds[] = {
	...
	{ "show-linkprop",	do_show_linkprop,
	    "    show-linkprop    [-PZ] [[-c] -o <field>,...] [-p <prop>,...] "
	    "[-z <zone>,...]\n"
	    "\t\t      <name>\n"					},
	{ "show-ether",		do_show_ether,
	    "    show-ether       [-xZ] [[-p] -o <field>,...] [-z <zone>,...] "
	    "[<link>]\n"						},
	...
	{ "show-iptun",		do_show_iptun,
	    "    show-iptun       [-PZ] [[-p] -o <field>,...] [-z <zone>,...] "
	    "[<link>]\n"						},
	...
	{ "show-phys",		do_show_phys,
	    "    show-phys        [-PHLZm] [[-p] -o <field>,...] "
	    "[-z <zone>,...] [<link>]\n"				},
	{ "init-phys",		do_init_phys,		NULL		},
	{ "reinit-phys",	do_reinit_phys,		NULL		},
	...
	{ "show-vnic",		do_show_vnic,
	    "    show-vnic        [-PZ] [[-p] -o <field>,...] [-l <link>]\n"
	    "\t\t     [-s [-i <interval>]] [-z <zone>,...] [<link>]\n"	},
	...
	{ "show-etherstub",	do_show_etherstub,
	    "    show-etherstub   [-tZ] [[-p] -o <field>,...] [-z <zone>,...] "
	    "[<link>]\n"						},
...
{
	char	eth_link[MAXLINKNAMESPECIFIER];
	char	eth_ptype[8];
	char	eth_state[8];
	char	eth_autoneg[5];
	char	eth_spdx[31];
...
{ "ZONE",	ZONECOLWIDTH,
	offsetof(ether_fields_buf_t, eth_zone), print_default_cb},
{ "PTYPE",	9,
...
{ "LINK",	LINKCOLWIDTH,	LINK_S_LINK,	print_link_stats_cb},
{ "ZONE",	ZONECOLWIDTH,	LINK_S_ZONE,	print_link_stats_cb},
...
typedef struct link_args_s {
	const char	*link_s_link;
	char		link_s_zone[ZONENAME_MAX];
...
typedef struct link_fields_buf_s {
	char link_name[MAXLINKNAMESPECIFIER];
	char link_class[DLADM_STRSIZE];
	char link_mtu[11];
	char link_state[DLADM_STRSIZE];
	char link_bridge[MAXLINKNAMESPECIFIER];
	...
	char link_phys_loc[DLD_LOC_STRSIZE];
	char link_flags[6];
	char link_vlan_vid[6];
	char link_zone[ZONENAME_MAX];
} link_fields_buf_t;
...
{ "ZONE",	ZONECOLWIDTH,
	offsetof(link_fields_buf_t, link_zone), print_default_cb},
{ "CLASS",	10,
...
{ "ZONE",	ZONECOLWIDTH,
	offsetof(laggr_fields_buf_t, laggr_zone), print_default_cb},
{ "POLICY",	9,
...
/* name,	field width,	index		callback */
{ "LINK",	11,	AGGR_X_LINK,	print_xaggr_cb},
...
{ "LINK",		14,	AGGR_S_LINK, print_aggr_stats_cb},
...
{ "LINK",		LINKCOLWIDTH,	AGGR_L_LINK,	print_lacp_cb},
...
{ "LOC",	48,
	offsetof(link_fields_buf_t, link_phys_loc), print_default_cb},
{ "FLAGS",	7,
...
/* name,	field width,	offset */
{ "LINK",	LINKCOLWIDTH,	PHYS_M_LINK,	print_phys_one_mac_cb},
...
{ "LINK",	LINKCOLWIDTH,	PHYS_H_LINK,	print_phys_one_hwgrp_cb},
...
/*
 * structures for 'dladm show-phys -L'
 */
...
typedef enum {
	...
	PHYS_L_MEDIA,
	PHYS_L_VANITY
} phys_l_field_index_t;

static const ofmt_field_t phys_l_fields[] = {
{ "LINK",	18,	PHYS_L_LINK,	print_phys_one_loc_cb},
{ "DEVICE",	13,	PHYS_L_DEV,	print_phys_one_loc_cb},
{ "LOC",	48,	PHYS_L_LOC,	print_phys_one_loc_cb},
{ "MEDIA",	21,	PHYS_L_MEDIA,	print_phys_one_loc_cb},
{ "VANITY",	18,	PHYS_L_VANITY,	print_phys_one_loc_cb},
{ NULL,		0,	0,		NULL}}
;
 ...
 */
static ofmt_field_t wifi_show_fields[A_CNT(wifi_common_fields) + 3] = {
{ "ZONE",	ZONECOLWIDTH, UINT_MAX,		NULL},
...
static char *all_show_wifi_fields =
	"link,zone,status,essid,sec,strength,mode,speed,auth,bssid,bsstype";
static char *def_scan_wifi_fields =
	"link,essid,bssid,sec,strength,mode,speed";
static char *def_show_wifi_fields =
	"link,status,essid,sec,strength,mode,speed";
static char *zone_show_wifi_fields =
	"link,zone,status,essid,sec,strength,mode,speed";
...
/* name,	field width,  index */
{ "LINK",	9,	LINKPROP_LINK,	print_linkprop_cb},
{ "ZONE",	ZONECOLWIDTH,	LINKPROP_ZONE,	print_linkprop_cb},
...
typedef struct show_linkprop_state {
	char			ls_link[MAXLINKNAMESPECIFIER];
	char			*ls_line;
	char			**ls_propvals;
...
{
	char vnic_link[MAXLINKNAMESPECIFIER];
	char vnic_over[MAXLINKNAMESPECIFIER];
	char vnic_speed[6];
...
static const ofmt_field_t etherstub_fields[] = {
{ "LINK",		LINKCOLWIDTH,
	offsetof(vnic_fields_buf_t, vnic_link),	print_default_cb},
{ "ZONE",		ZONECOLWIDTH,
	offsetof(vnic_fields_buf_t, vnic_zone),	print_default_cb},
...
{
	char simnet_name[MAXLINKNAMESPECIFIER];
	char simnet_media[DLADM_STRSIZE];
	char simnet_macaddr[18];
	char simnet_otherlink[MAXLINKNAMESPECIFIER];
	char simnet_zone[ZONENAME_MAX];
} simnet_fields_buf_t;
...
{ "ZONE",	ZONECOLWIDTH,
	offsetof(iptun_fields_buf_t, iptun_zone), print_default_cb },
{ "TYPE",	6,
...
typedef struct bridge_fields_buf_s {
	char bridge_name[MAXLINKNAMESPECIFIER]; /* 14.4.1.2.3(b) */
...
typedef struct bridge_link_fields_buf_s {
	char bridgel_link[MAXLINKNAMESPECIFIER];
...
typedef struct bridge_statfields_buf_s {
	char bridges_name[MAXLINKNAMESPECIFIER];
...
typedef struct bridge_link_statfields_buf_s {
	char bridgels_link[MAXLINKNAMESPECIFIER];
...
typedef struct bridge_fwd_fields_buf_s {
	...
	char bridgef_output[MAXLINKNAMESPECIFIER];	/* 14.7.6.3.3(c) */
...
typedef struct bridge_trill_fields_buf_s {
	...
	char bridget_link[MAXLINKNAMESPECIFIER];
...
do_create_aggr(int argc, char *argv[], const char *use)
{
	...
	char			*altroot = NULL;
	char			name[MAXLINKNAMESPECIFIER];
	char			*devs[MAXPORT];
	char			*links[MAXPORT];
	dladm_status_t		status;
	...
	if (!str2int(argv[optind], &key)) {
		if (strlcpy(name, argv[optind], sizeof (name)) >=
		    sizeof (name)) {
...
static dladm_status_t
get_mac_address_prop(datalink_id_t linkid, dladm_prop_type_t type,
    char *mac_address)
{
	char	*valptr[1];
	uint_t	valcnt = 1;

	valptr[0] = mac_address;
	return (dladm_get_linkprop(handle, linkid, type, "mac-address",
	    (char **)valptr, &valcnt));
}
...
do_modify_aggr(int argc, char *argv[], const char *use)
{
	...
	uint8_t			mac_addr[ETHERADDRL];
	boolean_t		mac_addr_fixed = B_FALSE;
	char			mac_addr_pval[DLADM_PROP_VAL_MAX];
	boolean_t		prop_changed = B_FALSE;
	...
	if (modify_mask & DLADM_AGGR_MODIFY_MAC) {
		dladm_prop_type_t type = (flags & DLADM_OPT_PERSIST) ?
		    DLADM_PROP_VAL_PERSISTENT : DLADM_PROP_VAL_CURRENT;

		status = get_mac_address_prop(linkid, type, mac_addr_pval);
		if (status == DLADM_STATUS_OK) {
			status = dladm_set_linkprop(handle, linkid,
			    "mac-address", NULL, 0, flags);
			if (status != DLADM_STATUS_OK)
				goto done;
			prop_changed = B_TRUE;
		}
	...
	status = dladm_aggr_modify(handle, linkid, modify_mask, policy,
	    ...
	    flags);
	if ((status != DLADM_STATUS_OK) && prop_changed) {
		char *val = mac_addr_pval;

		if (dladm_set_linkprop(handle, linkid, "mac-address",
		    &val, 1, flags) == DLADM_STATUS_OK)
			prop_changed = B_FALSE;
	}
	if (prop_changed)
		warn("mac-address property has been modified");
...
do_create_vlan(int argc, char *argv[], const char *use)
{
	...
	if (optind == (argc - 1)) {
		...
		if ((dlpi_parselink(link, drv, sizeof (drv), &ppa) !=
		    DLPI_SUCCESS) || (ppa >= 1000) ||
...
/*ARGSUSED*/
static int
i_dladm_walk_linkmap(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	char			mediabuf[DLADM_STRSIZE];
	char			classbuf[DLADM_STRSIZE];

	(void) printf("%-12s%8d  %-12s%-20s %6d\n", di->di_linkname,
	    di->di_linkid, dladm_class2str(di->di_class, classbuf),
	    dladm_media2str(di->di_media, mediabuf), di->di_flags);
...
do_show_linkmap(int argc, char *argv[], const char *use)
{
	...
	(void) dladm_walk_datalinks(handle, DATALINK_ALL_LINKID,
...
do_init_phys(int argc, char *argv[], const char *use)
{
	...
	/*
	 * dlmgmtd will automatically force all drivers to attach on
	 * reconfiguration boot, so all present network physical devices
	 * are known to the dlmgmtd daemon.  However in case this command
	 * is run on the commandline after a non-reconfiguration boot,
	 * ensure that dlmgmtd knows about all present devices.
	 */
	(void) dladm_walk_phys_info(handle, NULL, NULL, DLADM_OPT_ACTIVE);
	(void) dladm_walk_datalink_id(purge_phys, handle, NULL,
	    DATALINK_CLASS_PHYS, DATALINK_ANY_MEDIATYPE, DLADM_OPT_PERSIST);
}

/*
 * Reinitalize physical configuration.  Door call resets phys-prefix property,
 * then all physical links are removed and the call to dladm_walk_phys_info()
 * triggers a DINFOFORCE snapshot which will trigger reattach of drivers.
 */
/*ARGSUSED*/
static void
do_reinit_phys(int argc, char *argv[], const char *use)
{
	char *prefix = NULL;
	...
	if (dladm_reinit_phys(handle, prefix) != DLADM_STATUS_OK) {
		warn("cannot reinit - complex configuration");
		return;
	}
	(void) dladm_walk_phys_info(handle, NULL, NULL,
	    DLADM_OPT_ACTIVE | DLADM_OPT_FORCE);
}
...
print_link(show_state_t *state, datalink_id_t linkid, link_fields_buf_t *lbuf,
    const char *link, uint32_t flags, datalink_class_t class, zoneid_t linkzid)
{
	uint_t			mtu;
	...
	if (state->ls_flags == DLADM_OPT_ACTIVE) {
		...
		if (class == DATALINK_CLASS_PHYS) {
			...
			if (dlpi_info(dh, &dlinfo, DLPI_INFO_VERSION) !=
			    DLPI_SUCCESS) {
				dlpi_close(dh);
...
link_mtu:
	...
	(void) snprintf(lbuf->link_name, sizeof (lbuf->link_name), "%s", link);
	(void) getzonenamebyid(linkzid, lbuf->link_zone,
	    sizeof (lbuf->link_zone));
...
/* ARGSUSED */
static int
show_link(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	...
	if ((status = print_link(state, di->di_linkid, &lbuf, di->di_linkname,
	    di->di_flags, di->di_class, di->di_zoneid)) == DLADM_STATUS_OK)
...
print_link_stats_cb(ofmt_arg_t *ofarg, char *buf, uint_t bufsize)
{
	...
	case LINK_S_LINK:
		...
		break;
	case LINK_S_ZONE:
		(void) snprintf(buf, bufsize, "%s", largs->link_s_zone);
		break;
	case LINK_S_IPKTS:
...
static int
show_link_stats(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	...
	if (di->di_class == DATALINK_CLASS_PHYS) {
		if (dladm_phys_info(dh, di->di_linkid, &dpa,
		    DLADM_OPT_ACTIVE) != DLADM_STATUS_OK) {
			return (DLADM_WALK_CONTINUE);
		}
		if (dpa.dp_novanity)
			...
			get_link_stats(di->di_linkname, &stats);
	} else {
		get_link_stats(di->di_linkname, &stats);
	}
	dladm_stats_diff(&diff_stats, &stats, &state->ls_prevstats);

	largs.link_s_link = di->di_linkname;
	(void) getzonenamebyid(di->di_zoneid, largs.link_s_zone,
	    sizeof (largs.link_s_zone));
...
print_aggr_info(show_grp_state_t *state, const char *link,
    dladm_aggr_grp_attr_t *ginfop, const char *macaddr, zoneid_t linkzid)
{
	...
	(void) snprintf(lbuf.laggr_name, sizeof (lbuf.laggr_name),
	    "%s", link);
	(void) getzonenamebyid(linkzid, lbuf.laggr_zone,
	    sizeof (lbuf.laggr_zone));

	(void) dladm_aggr_policy2str(ginfop->lg_policy,
	    lbuf.laggr_policy);

	if (macaddr != NULL || ginfop->lg_mac_fixed) {
		if (macaddr == NULL) {
			(void) dladm_aggr_macaddr2str(ginfop->lg_mac,
			    addr_str);
			macaddr = addr_str;
		}
		(void) snprintf(lbuf.laggr_addrpolicy,
		    sizeof (lbuf.laggr_addrpolicy), "fixed (%s)", macaddr);
	} else {
		(void) snprintf(lbuf.laggr_addrpolicy,
...
static dladm_status_t
print_aggr(show_grp_state_t *state, dladm_datalink_info_t *di)
{
	char			mac_addr_pval[DLADM_PROP_VAL_MAX];
	dladm_aggr_grp_attr_t	ginfo;
	dladm_status_t		status;
	...
	if (!(state->gs_flags & di->di_flags))
		return (DLADM_STATUS_NOTFOUND);

	status = dladm_aggr_info(handle, di->di_linkid, &ginfo,
	    state->gs_flags);
	if (status != DLADM_STATUS_OK)
		return (status);

	if (state->gs_lacp)
		status = print_aggr_lacp(state, di->di_linkname, &ginfo);
	else if (state->gs_extended)
		status = print_aggr_extended(state, di->di_linkname, &ginfo);
	else if (state->gs_stats)
		status = print_aggr_stats(state, di->di_linkname, &ginfo);
	else {
		const char *macaddr = NULL;

		if (state->gs_flags & DLADM_OPT_PERSIST) {
			status = get_mac_address_prop(di->di_linkid,
			    DLADM_PROP_VAL_PERSISTENT, mac_addr_pval);
			if (status == DLADM_STATUS_OK)
				macaddr = mac_addr_pval;
		}
		status = print_aggr_info(state, di->di_linkname, &ginfo,
		    macaddr, di->di_zoneid);
	}
...
/* ARGSUSED */
static int
show_aggr(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	show_grp_state_t	*state = arg;

	state->gs_status = print_aggr(state, di);
	return (DLADM_WALK_CONTINUE);
}
...
do_show_link(int argc, char *argv[], const char *use)
{
	...
	boolean_t	o_arg = B_FALSE;
	char		*fields_str = NULL;
	char		*all_fields = "link,zone,class,mtu,state,bridge,over";
	char		*active_zone_fields = "link,zone,class,mtu,state,over";
	...
	char		*allstat_fields =
	    "link,zone,ipackets,rbytes,ierrors,opackets,obytes,oerrors";
	ofmt_handle_t	ofmt;
	ofmt_status_t	oferr;
	uint_t		ofmtflags = 0;
	...
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":pPsSi:o:z:Z",
	    show_lopts, NULL)) != -1) {
		switch (option) {
		case 'p':
			if (p_arg)
		...
		case 'z':
			if (z_arg)
				die_optdup(option);
			z_arg = B_TRUE;
			if (dladm_parse_zonelist(optarg, &state.ls_zonecnt,
			    &state.ls_zones) != DLADM_STATUS_OK)
				die("invalid zone list '%s'", optarg);
			break;
		case 'Z':
			if (Z_arg)
				die_optdup(option);
	...
	if (z_arg && P_arg)
		die("the option -P cannot be used with -z");

	if (Z_arg && (P_arg || o_arg || S_arg))
		die("the option -%c cannot be used with -Z", P_arg ?
		    'P' :(o_arg ?  'o':'S'));

	if (S_arg && (p_arg || P_arg))
		die("the option -%c cannot be used with -S", p_arg ? 'p' : 'P');

	/* get link name (optional last argument) */
	if (optind == (argc-1)) {
		status = dladm_apply_linknamefilters(handle, argv[optind],
		    &linkid, state.ls_zonecnt, state.ls_zones, flags,
		    dladm_zoneid, errorstr);
		if (status != DLADM_STATUS_OK)
			die_dlerr(status, errorstr);
	} else if (optind != argc) {
		die("usage: %s", use);
	}
	...
	if (!o_arg || (o_arg && strcasecmp(fields_str, "all") == 0)) {
		if (s_arg && (Z_arg || o_arg))
			fields_str = allstat_fields;
		else if (s_arg)
			fields_str = stat_fields;
		else if (o_arg)
			fields_str = all_fields;
		else if (Z_arg)
			fields_str = active_zone_fields;
	...
	if (state.ls_parsable)
		ofmtflags |= OFMT_PARSABLE;
	oferr = ofmt_open(fields_str, link_fields, ofmtflags, DLADM_DEFAULT_COL,
	    &ofmt);
	dladm_ofmt_check(oferr, state.ls_parsable, ofmt);
	state.ls_ofmt = ofmt;

	state.ls_status = DLADM_STATUS_OK;
	status = dladm_walk_datalinks(handle, linkid, show_link, &state,
	    DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE,
	    flags | DLADM_OPT_ALLZONES, state.ls_zonecnt, state.ls_zones);
	if (status == DLADM_STATUS_OK)
		status = state.ls_status;
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show all links");
		else
			die_dlerr(status, "failed to show link %s",
			    argv[optind]);
	}
	ofmt_close(ofmt);
}
...
do_show_aggr(int argc, char *argv[], const char *use)
{
	...
	char			*all_fields =
	    "link,zone,policy,addrpolicy,lacpactivity,lacptimer,flags";
	...
	opterr = 0;
	bzero(&state, sizeof (state));
	while ((option = getopt_long(argc, argv, ":LpPxsi:o:z:Z",
	    show_lopts, NULL)) != -1) {
		switch (option) {
		case 'L':
			if (L_arg)
		...
		case 'z':
			if (z_arg)
				die_optdup(option);
			z_arg = B_TRUE;
			if (dladm_parse_zonelist(optarg, &state.gs_zonecnt,
			    &state.gs_zones) != DLADM_STATUS_OK)
				die("invalid zone list '%s'", optarg);
			break;
		case 'Z':
			if (Z_arg)
	...
	if (Z_arg && (P_arg || o_arg))
		die("the option -%c cannot be used with -Z", P_arg ? 'P':'o');
	...
	if (optind == (argc-1)) {
		if (!str2int(argv[optind], &key)) {
			status = dladm_apply_linknamefilters(handle,
			    argv[optind], &linkid, state.gs_zonecnt,
			    state.gs_zones, flags, dladm_zoneid, errorstr);
			if (status != DLADM_STATUS_OK)
				die_dlerr(status, errorstr);
		} else {
			...
			if (status == DLADM_STATUS_OK)
				status = dladm_apply_linknamefilters(handle,
				    NULL, &linkid, state.gs_zonecnt,
				    state.gs_zones, flags, dladm_zoneid,
				    errorstr);
			if (status != DLADM_STATUS_OK)
				die("non-existent aggregation '%s'",
				    argv[optind]);
		}
	} else if (optind != argc) {
		die("usage: %s", use);
	...
	if (state.gs_parsable)
		ofmtflags |= OFMT_PARSABLE;
	oferr = ofmt_open(fields_str, pf, ofmtflags, DLADM_DEFAULT_COL, &ofmt);
	...
	status = dladm_walk_datalinks(handle, linkid, show_aggr, &state,
	    DATALINK_CLASS_AGGR, DATALINK_ANY_MEDIATYPE,
	    flags | DLADM_OPT_ALLZONES, state.gs_zonecnt,
	    state.gs_zones);
	if (status == DLADM_STATUS_OK)
		status = state.gs_status;
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show all aggregations");
		else
			die_dlerr(status, "failed to show aggr %s",
			    argv[optind]);
	}
	ofmt_close(ofmt);
}
...
static dladm_status_t
print_phys_default(show_state_t *state, dladm_datalink_info_t *di)
{
	dladm_phys_attr_t dpa;
	dladm_status_t status;
	link_fields_buf_t pattr;

	status = dladm_phys_info(handle, di->di_linkid, &dpa, state->ls_flags);
	if (status != DLADM_STATUS_OK)
		goto done;

	bzero(&pattr, sizeof (pattr));
	(void) getzonenamebyid(di->di_zoneid, pattr.link_zone,
	    sizeof (pattr.link_zone));
	(void) snprintf(pattr.link_phys_device,
	    sizeof (pattr.link_phys_device), "%s", dpa.dp_dev);
	(void) snprintf(pattr.link_phys_loc,
	    sizeof (pattr.link_phys_loc), "%s", dpa.dp_loc);
	(void) dladm_media2str(di->di_media, pattr.link_phys_media);
	if (state->ls_flags == DLADM_OPT_ACTIVE) {
		boolean_t	islink;

		if (!dpa.dp_novanity) {
			(void) strlcpy(pattr.link_name, di->di_linkname,
	...
	(void) snprintf(pattr.link_flags, sizeof (pattr.link_flags),
	    "%c----", di->di_flags & DLADM_OPT_ACTIVE ? '-' : 'r');
...
static boolean_t
print_phys_one_loc_cb(ofmt_arg_t *ofarg, char *buf, uint_t bufsize)
{
	physinfo_t *pli = ofarg->ofmt_cbarg;

	switch (ofarg->ofmt_id) {
	case PHYS_L_LINK:
		(void) snprintf(buf, bufsize, "%s", pli->pli_linkname);
		break;
	case PHYS_L_DEV:
		(void) snprintf(buf, bufsize, "%s", pli->pli_physattr->dp_dev);
		break;
	case PHYS_L_LOC:
		(void) snprintf(buf, bufsize, "%s", pli->pli_physattr->dp_loc);
		break;
	case PHYS_L_MEDIA:
		(void) snprintf(buf, bufsize, "%s", pli->pli_media);
		break;
	case PHYS_L_VANITY:
		if (strlen(pli->pli_physattr->dp_phys_prefix) > 0) {
			(void) snprintf(buf, bufsize, "%s%d",
			    pli->pli_physattr->dp_phys_prefix,
			    pli->pli_vanity_index);
		} else {
			(void) snprintf(buf, bufsize, "%s", pli->pli_linkname);
		}
		break;
 ...
 */
static dladm_status_t
print_phys_mac(show_state_t *state, datalink_id_t linkid, const char *link)
{
...
static dladm_status_t
print_phys_hwgrp(show_state_t *state, datalink_id_t linkid, const char *link)
{
...
do_show_iptun(int argc, char *argv[], const char *use)
{
	...
	char		*name = NULL;
	dladm_status_t	status;
	const char	*fields_str = "link,type,flags,local,remote";
	show_state_t	state;
	ofmt_handle_t	ofmt;
	ofmt_status_t	oferr;
	uint_t		ofmtflags = 0;
	...
	bzero(&state, sizeof (state));
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":pPo:z:Z",
	    iptun_lopts, NULL)) != -1) {
		switch (option) {
		case 'o':
			if (o_arg)
	...
	if (name != NULL) {
		status = dladm_apply_linknamefilters(handle, name, &linkid,
		    state.ls_zonecnt, state.ls_zones, flags, dladm_zoneid,
		    errorstr);
		if (status != DLADM_STATUS_OK)
			die_dlerr(status, errorstr);
	}

	status = dladm_walk_datalinks(handle, linkid, print_iptun_walker,
	...
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show all IP tunnels");
		else
			die_dlerr(status, "failed to show tunnel %s",
			    argv[optind]);
	}
}
...
/* ARGSUSED */
static dladm_status_t
print_iptun(dladm_handle_t dh, dladm_datalink_info_t *di, show_state_t *state)
{
	dladm_status_t		status;
	iptun_params_t		params;
	...
	params.iptun_param_linkid = di->di_linkid;
	status = dladm_iptun_getparams(dh, &params, state->ls_flags);
	if (status != DLADM_STATUS_OK)
		return (status);

	/* LINK */
	(void) strlcpy(lbuf.iptun_name, di->di_linkname,
	    sizeof (lbuf.iptun_name));

	/* ZONE */
	(void) getzonenamebyid(di->di_zoneid, lbuf.iptun_zone,
	    sizeof (lbuf.iptun_zone));
...
static int
print_iptun_walker(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	((show_state_t *)arg)->ls_status = print_iptun(dh, di, arg);
	return (DLADM_WALK_CONTINUE);
}

static dladm_status_t
print_phys(show_state_t *state, dladm_datalink_info_t *di)
{
	dladm_status_t status;
	...
	if (!(state->ls_flags & di->di_flags)) {
		status = DLADM_STATUS_NOTFOUND;
		goto done;
	}

	if (state->ls_mac)
		status = print_phys_mac(state, di->di_linkid, di->di_linkname);
	else if (state->ls_hwgrp)
		status = print_phys_hwgrp(state, di->di_linkid,
		    di->di_linkname);
	else
		status = print_phys_default(state, di);
done:
	return (status);
}
...
/* ARGSUSED */
static int
show_phys(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	show_state_t		*state = arg;
	physinfo_t		*pli;
	physinfo_t		*plil;
	...
	if (state->ls_loc) {
		/*
		 * We want to display physical locations in physical order,
		 * so simply record information about each physical link here.
		 * Display is then handled by a show_phys_loc(), a callback
		 * to dladm_walk_phys_info(), since this walks devices in
		 * physical order.
		 */
		if ((plil = realloc(state->ls_physinfo,
		    (state->ls_physinfocnt + 1) *
		    sizeof (physinfo_t))) == NULL)
			return (DLADM_WALK_CONTINUE);
		pli = &plil[state->ls_physinfocnt];
		pli->pli_linkid = di->di_linkid;
		(void) strlcpy(pli->pli_linkname, di->di_linkname,
		    MAXLINKNAMELEN);
		(void) dladm_media2str(di->di_media, pli->pli_media);
		state->ls_physinfocnt++;
		state->ls_physinfo = plil;
	} else {
		state->ls_status = print_phys(state, di);
	}
	return (DLADM_WALK_CONTINUE);
}
...
/*
 * Callback for dladm_walk_phys_info()  - walks devices in media/location
 * order, printing location info for each.  Invoked by "dladm show-phys -L".
 */
/*ARGSUSED*/
static int
show_phys_loc(dladm_handle_t dh, datalink_id_t linkid,
    dladm_phys_attr_t *pa, void *arg)
{
	show_state_t		*state = arg;
	physinfo_t		*plil = state->ls_physinfo;
	int			i;

	for (i = 0; i < state->ls_physinfocnt; i++) {
		if (plil[i].pli_linkid == linkid) {
			plil[i].pli_physattr = pa;
			plil[i].pli_vanity_index = state->ls_physinfoindex;
			ofmt_print(state->ls_ofmt, &plil[i]);
			break;
		}
	}
	state->ls_physinfoindex++;
	return (DLADM_WALK_CONTINUE);
}
 ...
 */
static dladm_status_t
print_vlan(show_state_t *state, dladm_datalink_info_t *di, link_fields_buf_t *l)
{
	dladm_vlan_attr_t	vinfo;
	dladm_status_t		status;

	(void) strlcpy(l->link_name, di->di_linkname, sizeof (l->link_name));
	(void) getzonenamebyid(di->di_zoneid, l->link_zone,
	    sizeof (l->link_zone));
	...
	if ((status = dladm_vlan_info(handle, di->di_linkid, &vinfo,
...
/* ARGSUSED */
static int
show_vlan(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	show_state_t		*state = arg;
	dladm_status_t		status;
	link_fields_buf_t	lbuf;
	...
	bzero(&lbuf, sizeof (link_fields_buf_t));
	status = print_vlan(state, di, &lbuf);
	if (status != DLADM_STATUS_OK)
		goto done;
...
do_show_phys(int argc, char *argv[], const char *use)
{
	...
	char		*all_loc_fields = "link,device,loc";
	const ofmt_field_t *pf;
	ofmt_handle_t	ofmt;
	ofmt_status_t	oferr;
	uint_t		ofmtflags = 0;
	boolean_t	z_arg = B_FALSE;
	boolean_t	Z_arg = B_FALSE;
	uint_t		exclusive_args = 0;

	bzero(&state, sizeof (state));
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":pPo:LmHz:Z",
	    show_lopts, NULL)) != -1) {
		switch (option) {
		case 'L':
			L_arg = B_TRUE;
	...
	if (L_arg && !(flags & DLADM_OPT_ACTIVE)) {
		/*
		 * We can only display location information for
		 * active data-links.
		 */
		die("-L not compatible with -P");
	}
	...
	if (!o_arg || (o_arg && strcasecmp(fields_str, "all") == 0)) {
		...
		else if (state.ls_loc)
			fields_str = all_loc_fields;
		else if (Z_arg)
			fields_str = zone_active_fields;
		else if (o_arg && (state.ls_flags & DLADM_OPT_ACTIVE))
			fields_str = zone_active_fields;
	...
	if (state.ls_mac) {
		...
		pf = phys_l_fields;
		state.ls_physinfo = NULL;
		state.ls_physinfocnt = 0;
		state.ls_physinfoindex = 0;
	} else {
		pf = phys_fields;
	}
	...
	dladm_ofmt_check(oferr, state.ls_parsable, ofmt);
	state.ls_ofmt = ofmt;
	status = dladm_walk_datalinks(handle, linkid, show_phys, &state,
	    DATALINK_CLASS_PHYS, DATALINK_ANY_MEDIATYPE,
	    flags | DLADM_OPT_ALLZONES, state.ls_zonecnt,
	    state.ls_zones);
	if (state.ls_loc) {
		status = dladm_walk_phys_info(handle, show_phys_loc, &state,
		    flags);
		if (state.ls_physinfocnt > 0)
			free(state.ls_physinfo);
	}
	...
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show physical links");
		else
			die_dlerr(status, "failed to show physical link %s",
			    argv[optind]);
	}
	ofmt_close(ofmt);
}
...
do_show_vlan(int argc, char *argv[], const char *use)
{
	...
	dladm_status_t  status;
	boolean_t	o_arg = B_FALSE;
	char		*fields_str = "link,vid,over,flags";
	ofmt_handle_t	ofmt;
	ofmt_status_t	oferr;
	uint_t		ofmtflags = 0;
	boolean_t	z_arg = B_FALSE;
	...
	status = dladm_walk_datalinks(handle, linkid, show_vlan, &state,
	...
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show vlans");
		else
			die_dlerr(status, "failed to show vlan %s",
			    argv[optind]);
	}
	ofmt_close(ofmt);
}
...
do_create_vnic(int argc, char *argv[], const char *use)
{
	...
	status = dladm_vnic_create(handle, name, dev_linkid, -1, mac_addr_type,
	    mac_addr, maclen, &mac_slot, mac_prefix_len, NULL, 0, vid, vrid, af,
 ...
 */
static dladm_status_t
print_vnic(show_vnic_state_t *state, dladm_datalink_info_t *di)
{
	...
	boolean_t		use_mac_address_prop = B_FALSE;

	if ((status = dladm_vnic_info(handle, di->di_linkid, vnic,
	    state->vs_flags)) != DLADM_STATUS_OK)
		return (status);
	...
	if (state->vs_stats) {
		...
		if (!state->vs_printstats) {
			 ...
			 * named device.
			 */
			get_link_stats(di->di_linkname, &vnic_stats);
			dladm_stats_total(&state->vs_totalstats, &vnic_stats,
			    &state->vs_prevstats[vnic->va_vnic_id]);
		} else {
			/* get and print vnic statistics */
			get_link_stats(di->di_linkname, &vnic_stats);
			dump_vnic_stat(di->di_linkname, di->di_linkid, state,
			    &vnic_stats, &state->vs_totalstats);
		}
		return (DLADM_STATUS_OK);
	} else {
		(void) snprintf(vbuf.vnic_link, sizeof (vbuf.vnic_link),
		    "%s", di->di_linkname);
		(void) getzonenamebyid(di->di_zoneid, vbuf.vnic_zone,
		...
		if (!is_etherstub) {
			...
			(void) snprintf(vbuf.vnic_speed,
			    ...
			    (uint_t)((get_ifspeed(di->di_linkname, B_TRUE))
			    / 1000000ull));

			if (state->vs_flags & DLADM_OPT_PERSIST) {
				char	mac_addr_pval[DLADM_PROP_VAL_MAX];

				status = get_mac_address_prop(di->di_linkid,
				    DLADM_PROP_VAL_PERSISTENT, mac_addr_pval);
				if (status == DLADM_STATUS_OK) {
					use_mac_address_prop = B_TRUE;
...
/* ARGSUSED */
static int
show_vnic(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	show_vnic_state_t	*state = arg;

	state->vs_status = print_vnic(state, di);
    ...
    boolean_t etherstub)
{
	...
	bzero(&state, sizeof (state));
	opterr = 0;
	while ((option = getopt_long(argc, argv, ":pPl:si:o:z:Z", lopts,
	    NULL)) != -1) {
		switch (option) {
		case 'p':
		...
		case 'l':
			...
			if (strlcpy(state.vs_link, optarg,
			    sizeof (state.vs_link)) >=
			    sizeof (state.vs_link))
				die("link name too long");
		...
		case 'z':
			if (z_arg)
				die_optdup(option);
			z_arg = B_TRUE;
			if (dladm_parse_zonelist(optarg, &state.vs_zonecnt,
			    &state.vs_zones) != DLADM_STATUS_OK)
				die("invalid zone list '%s'", optarg);
			break;
		case 'Z':
			if (Z_arg)
	...
	if (optind == (argc - 1)) {
		status = dladm_apply_linknamefilters(handle, argv[optind],
		    &linkid, state.vs_zonecnt, state.vs_zones, flags,
		    dladm_zoneid, errorstr);
		if (status != DLADM_STATUS_OK)
			die_dlerr(status, errorstr);
		(void) strlcpy(state.vs_vnic, argv[optind],
		    sizeof (state.vs_vnic));
	} else if (optind != argc) {
		die("usage: %s", use);
	}
	...
	if (!o_arg || (o_arg && strcasecmp(fields_str, "all") == 0)) {
		if (etherstub && !Z_arg && !o_arg)
			fields_str = default_e_fields;
		else if (!o_arg && !Z_arg)
			fields_str =
	...
	status = dladm_walk_datalinks(handle, linkid, show_vnic, &state,
	    DATALINK_CLASS_VNIC | DATALINK_CLASS_ETHERSTUB,
	    DATALINK_ANY_MEDIATYPE, flags | DLADM_OPT_ALLZONES,
	    state.vs_zonecnt, state.vs_zones);
	if (status == DLADM_STATUS_OK)
		status = state.vs_status;
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show all vnics");
		else
			die_dlerr(status, "failed to show vnic '%s'",
			    state.vs_vnic);
	}
	ofmt_close(ofmt);
}
...
do_create_etherstub(int argc, char *argv[], const char *use)
{
	...
	status = dladm_vnic_create(handle, name, DATALINK_INVALID_LINKID, -1,
	    VNIC_MAC_ADDR_TYPE_AUTO, NULL, 0, NULL, 0, NULL, 0, 0,
...
static dladm_status_t
print_simnet(show_state_t *state, dladm_datalink_info_t *di)
{
	dladm_simnet_attr_t	slinfo;
	dladm_status_t		status;
	...
	(void) strlcpy(slbuf.simnet_name, di->di_linkname,
	    sizeof (slbuf.simnet_name));
	(void) getzonenamebyid(di->di_zoneid, slbuf.simnet_zone,
	    sizeof (slbuf.simnet_zone));

	if (!(state->ls_flags & di->di_flags))
		return (DLADM_STATUS_NOTFOUND);

	if ((status = dladm_simnet_info(handle, di->di_linkid, &slinfo,
	    state->ls_flags)) != DLADM_STATUS_OK)
		return (status);
...
/* ARGSUSED */
static int
show_simnet(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	show_state_t		*state = arg;

	state->ls_status = print_simnet(state, di);
...
do_show_simnet(int argc, char *argv[], const char *use)
{
	...
	ofmt_handle_t	ofmt;
	ofmt_status_t	oferr;
	char		*all_fields = "link,zone,media,macaddress,otherlink";
	char		*fields_str = "link,media,macaddress,otherlink";
	uint_t		ofmtflags = 0;
	boolean_t	z_arg = B_FALSE;
	boolean_t	Z_arg = B_FALSE;
	...
	status = dladm_walk_datalinks(handle, linkid, show_simnet, &state,
	...
	if (status != DLADM_STATUS_OK) {
		if (linkid == DATALINK_ALL_LINKID)
			die_dlerr(status, "failed to show all simnet links");
		else
			die_dlerr(status, "failed to show simnet %s",
			    argv[optind]);
	}
	ofmt_close(ofmt);
}
    ...
    show_state_t *state)
{
	...
	for (;;) {
		state->ls_donefirst = B_FALSE;
		(void) dladm_walk_datalinks(handle, linkid, show_link_stats,
		    state, DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE,
		    DLADM_OPT_ACTIVE | DLADM_OPT_ALLZONES,
		    state->ls_zonecnt, state->ls_zones);
		if (interval == 0)
			break;
...
aggr_stats(datalink_id_t linkid, show_grp_state_t *state, uint_t interval)
{
	...
	for (;;) {
		...
		(void) dladm_walk_datalinks(handle, linkid, show_aggr, state,
		    DATALINK_CLASS_AGGR, DATALINK_ANY_MEDIATYPE,
		    DLADM_OPT_ACTIVE, state->gs_zonecnt,
		    state->gs_zones);
		if (interval == 0)
			break;
...
vnic_stats(show_vnic_state_t *sp, uint32_t interval)
{
	...
	for (;;) {
		...
		status = dladm_walk_datalinks(handle, sp->vs_vnic_id, show_vnic,
		    &state, DATALINK_CLASS_VNIC, DATALINK_ANY_MEDIATYPE,
		    DLADM_OPT_ACTIVE | DLADM_OPT_ALLZONES,
		    state.vs_zonecnt, state.vs_zones);
		if (status == DLADM_STATUS_OK)
			status = state.vs_status;
		if (status != DLADM_STATUS_OK) {
			if (specific_link)
				...
				die_dlerr(status, "failed to show vnic stats");
		}
...
get_mac_stats(const char *dev, pktsum_t *stats)
{
	...
	if (dlpi_parselink(dev, module, sizeof (module), &instance) !=
	    DLPI_SUCCESS) {
		return;
	}
    ...
    void *val, boolean_t islink)
{
	...
	if (islink) {
		...
		return (query_kstat(module, instance, "phys", stat, type, val));
	}
}
    ...
    boolean_t parsable)
{
	...
	if (cmdtype == WIFI_CMD_SCAN) {
		...
		bcopy(wifi_common_fields, &wifi_show_fields[3],
...
print_wlan_attr_cb(ofmt_arg_t *ofarg, char *buf, uint_t bufsize)
{
	...
	if (ofarg->ofmt_id == UINT_MAX) {
		(void) strlcpy(buf, (char *)statep->ws_zone, bufsize);
		return (B_TRUE);
	}
...
static int
scan_wifi(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	print_wifi_state_t	*statep = arg;
	dladm_status_t		status;

	statep->ws_link = di->di_linkname;
	status = dladm_wlan_scan(dh, di->di_linkid, statep, print_scan_results);
	if (status != DLADM_STATUS_OK)
...
static int
show_wifi(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	print_wifi_state_t	*statep = arg;
	...
	status = dladm_wlan_get_linkattr(dh, di->di_linkid, &attr);
	if (status != DLADM_STATUS_OK)
		die_dlerr(status, "cannot get link attributes for %s",
		    di->di_linkname);

	statep->ws_link = di->di_linkname;
	(void) getzonenamebyid(di->di_zoneid, statep->ws_zone,
	    sizeof (statep->ws_zone));
...
do_display_wifi(int argc, char **argv, int cmd, const char *use)
{
	...
	char			*fields_str = NULL;
	uint_t			flags;
	dladm_walk_datalink_cb_t *callback;
	...
	if (cmd == WIFI_CMD_SCAN) {
		...
		callback = show_wifi;
		shortopts_str = ":o:pz:Z";
		flags = DLADM_OPT_ACTIVE | DLADM_OPT_ALLZONES;
	} else {
		return;
	}
	...
	opterr = 0;
	while ((option = getopt_long(argc, argv, shortopts_str, wifi_longopts,
	    NULL)) != -1) {
		switch (option) {
		case 'o':
			if (o_arg)
		...
		case 'p':
			...
			break;
		case 'z':
			if (dladm_parse_zonelist(optarg, &state.ws_zonecnt,
			    &state.ws_zones) != DLADM_STATUS_OK)
				die("invalid zone list '%s'", optarg);
			break;
		case 'Z':
			if (Z_arg)
				die_optdup(option);
			Z_arg = B_TRUE;
			fields_str = zone_show_wifi_fields;
			break;
		default:
			die_opterr(optopt, option, use);
		}
	...
	if (Z_arg && o_arg)
		die("the option -o cannot be used with -Z");
	...
	if (optind == (argc - 1)) {
		status = dladm_apply_linknamefilters(handle, argv[optind],
		    &linkid, state.ws_zonecnt, state.ws_zones, flags,
		    dladm_zoneid, errorstr);
		if (status != DLADM_STATUS_OK)
			die_dlerr(status, errorstr);
	} else if (optind != argc) {
	...
	(void) dladm_walk_datalinks(handle, linkid, callback, &state,
	    DATALINK_CLASS_PHYS | DATALINK_CLASS_SIMNET,
	    DL_WIFI, flags, state.ws_zonecnt, state.ws_zones);
    ...
    char **pptr)
{
	...
	for (i = 0; i < valcnt; i++) {
		if (propvals[i][0] == '\0' && !statep->ls_parsable) {
			...
			/*
			 * if the returned property value is unknown "--"
			 * then in parsable mode print zero-length string
			 */
...
print_linkprop_cb(ofmt_arg_t *ofarg, char *buf, uint_t bufsize)
{
	...
	case LINKPROP_LINK:
		...
		break;
	case LINKPROP_ZONE:
		(void) snprintf(ptr, lim - ptr, "%s", statep->ls_zone);
		break;
...
do_show_linkprop(int argc, char **argv, const char *use)
{
	...
	while ((option = getopt_long(argc, argv, ":p:cPo:z:Z",
	...
	if (z_arg && state.ls_persist)
		die("the option -P cannot be used with -z");
	if (Z_arg && (state.ls_persist || o_arg))
		die("the option -%c cannot be used with -Z", state.ls_persist ?
		    'P':'o');
	...
	(void) dladm_walk_datalinks(handle, linkid, show_linkprop_onelink,
	    &state, DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE,
	    flags | DLADM_OPT_ALLZONES, state.ls_zonecnt,
	    state.ls_zones);
...
static int
show_linkprop_onelink(dladm_handle_t hdl, dladm_datalink_info_t *di, void *arg)
{
	int			i;
	char			*buf;
	dladm_arg_list_t	*proplist = NULL;
	...
	if ((statep->ls_persist && !(di->di_flags & DLADM_OPT_PERSIST)) ||
	    (!statep->ls_persist && !(di->di_flags & DLADM_OPT_ACTIVE))) {
		statep->ls_status = DLADM_STATUS_BADARG;
		return (DLADM_WALK_CONTINUE);
	}

	(void) strlcpy(statep->ls_link, di->di_linkname,
	    sizeof (statep->ls_link));
	(void) getzonenamebyid(di->di_zoneid, statep->ls_zone,
	    sizeof (statep->ls_zone));
	...
	if (proplist != NULL) {
		for (i = 0; i < proplist->al_count; i++) {
			(void) show_linkprop(hdl, di->di_linkid,
			    proplist->al_info[i].ai_name, statep);
		}
	} else {
		(void) dladm_walk_linkprop(hdl, di->di_linkid, statep,
		    show_linkprop);
	}
	if (dh != NULL)
		dlpi_close(dh);
...
do_show_ether(int argc, char **argv, const char *use)
{
	...
	while ((option = getopt_long(argc, argv, "o:pxz:Z",
		...
		switch (option) {
			...
			case 'z':
				if (z_arg)
					die_optdup(option);
				z_arg = B_TRUE;
				if (dladm_parse_zonelist(optarg,
				    &state.es_zonecnt, &state.es_zones) !=
				    DLADM_STATUS_OK)
					die("invalid zone list '%s'", optarg);
				break;
			case 'Z':
				if (Z_arg)
					die_optdup(option);
				Z_arg = B_TRUE;
				fields_str = "link,zone,ptype,state,auto,"
				    "speed-duplex,pause";
				break;
			default:
				die_opterr(optopt, option, use);
	...
	if (state.es_link != NULL) {
		...
		status = dladm_apply_linknamefilters(handle, argv[optind],
		    &linkid, state.es_zonecnt, state.es_zones, DLADM_OPT_ACTIVE,
		    dladm_zoneid, errorstr);
		if (status != DLADM_STATUS_OK)
			die_dlerr(status, errorstr);
	}

	(void) dladm_walk_datalinks(handle, linkid, show_etherprop, &state,
	    DATALINK_CLASS_PHYS, DL_ETHER,
	    DLADM_OPT_ACTIVE | DLADM_OPT_ALLZONES, state.es_zonecnt,
	    state.es_zones);
...
static int
show_etherprop(dladm_handle_t dh, dladm_datalink_info_t *di, void *arg)
{
	...
	(void) strlcpy(ebuf.eth_link, di->di_linkname, sizeof (ebuf.eth_link));
	(void) getzonenamebyid(di->di_zoneid, ebuf.eth_zone,
	    sizeof (ebuf.eth_zone));

	status = dladm_ether_info(dh, di->di_linkid, &eattr);
	if (status != DLADM_STATUS_OK)
		goto cleanup;
...
altroot_cmd(char *altroot, int argc, char *argv[])
{
	...
	(void) fprintf(fp, "/usr/sbin/dladm ");
	for (i = 0; i < argc; i++) {
		/*

...
/*
 * conf_scrub -- scrub a timestamp-only entry
 */
void
conf_scrub(const char *lhs)
{
	struct confinfo *cp = lut_lookup(Conflut, lhs);
	const char *timestamp;

	if (Canchange == CHG_NONE || cp == NULL)
		return;

	timestamp = opts_optarg(cp->cf_opts, "P");
	if (timestamp != NULL && opts_count(cp->cf_opts, NULL) == 1) {
		cp->cf_flags |= CONFF_DELETED;
		if (Canchange == CHG_BOTH)
			Changed = CHG_BOTH;
		else
...
conf_print(FILE *cstream, FILE *tstream)
{
	...
	for (cp = Confinfo; cp; cp = cp->cf_next) {
		...
		if (cp->cf_entry) {
			...
			if (tstream != NULL && timestamp != NULL) {
				opts_printword(cp->cf_entry, tstream);
				(void) fprintf(tstream, " -P ");
				opts_printword(timestamp, tstream);
				(void) fprintf(tstream, "\n");
				/* suppress conf entry for timestamp only */
				if (opts_count(cp->cf_opts, NULL) == 1)
					showline = 0;
			}
			/* output conf entry to cstream */
			if (showline) {
				opts_printword(cp->cf_entry, cstream);
				if (cp->cf_opts)
					opts_print(cp->cf_opts, cstream,
					    exclude_opts);
			}
		}
		if (cp->cf_com) {
			if (cp->cf_entry && showline)

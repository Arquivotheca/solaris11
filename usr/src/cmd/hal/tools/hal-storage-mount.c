	      ...
	      DBusConnection *system_bus)
{
...
#ifdef sun
	gboolean wants_to_change_gid;
	gboolean wants_to_change_mask;
#endif
	char *mount_dir;
	GError *err = NULL;
	char *sout = NULL;
...
#ifdef sun
	wants_to_change_gid = FALSE;
	wants_to_change_mask = FALSE;
#endif
...
#if sun
				} else if (strcmp(allow, "owner=") == 0) {
					wants_to_change_uid = TRUE;
					goto option_ok;

				} else if (strcmp(allow, "group=") == 0) {
					wants_to_change_gid = TRUE;
					goto option_ok;

				} else if (strcmp(allow, "mask=") == 0) {
					wants_to_change_mask = TRUE;
					goto option_ok;
#endif /* sun */
				} else {
	 ...
	 * (since these drives normally use vfat), except for Solaris which
	 * allows owner/uid setting for vfat/pcfs
	 */
	if (volume != NULL) {
		/* don't consider uid= on vfat, iso9660, udf change-uid for the purpose of policy
		 * (since these doesn't contain uid/gid bits) 
		 */
		if (strcmp (libhal_volume_get_fstype (volume), "iso9660") != 0 &&
		    strcmp (libhal_volume_get_fstype (volume), "udf") != 0) {
#ifndef sun
			if (strcmp (libhal_volume_get_fstype (volume),
			    "vfat") != 0)
#endif /* sun */
	...
	if (strcmp(mount_do_fstype, "pcfs") == 0) {
		char *buf, *cptr, *str;
		int ret;
		struct stat st;
		char dbuf[40];

		if (!wants_to_change_uid || !wants_to_change_gid) {

			if ((buf = strdup(device)) == NULL)
				unknown_error("strdup failed for device path");

			if (ret = stat(buf, &st) != 0) {
				if ((cptr = strstr(buf, ":")) != NULL)
					*cptr = NULL;
				ret = stat(buf, &st);
			}

			if (ret)
				unknown_error("can not stat device");

			if (wants_to_change_uid == FALSE) {
				snprintf(dbuf, sizeof (dbuf), ",owner=%u",
				    st.st_uid);
				g_string_append(mount_option_str, dbuf);
			}

			if (wants_to_change_gid == FALSE) {
				snprintf(dbuf, sizeof (dbuf), ",group=%u",
				    st.st_gid);
				g_string_append(mount_option_str, dbuf);
			}
		}

		if (!wants_to_change_mask) {
			g_string_append(mount_option_str, ",mask=077");
		}
	}
#endif /* sun */

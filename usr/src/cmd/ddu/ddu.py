...
class HDDgui:
    ...
    def __init__(self):
        ...
        status, output = commands.getstatusoutput(
        '%s/scripts/pkg_relate.sh save_p' % ABSPATH)
...
#        _("Feedback alias:driver-utility-feedback@sun.com"))
    ...
    def main_destroy(self, widget, data = None):
        ...
        status, output = commands.getstatusoutput(
        '%s/scripts/pkg_relate.sh restore_p' % ABSPATH)
    ...
    def rescan_clicked(self, widget):
        ...
        self.drv_button.set_sensitive(False)
        self.button_browser.set_sensitive(False)
        self.server_com.set_sensitive(False)
        ...
        gobject.idle_add(self.privilege_dlg)
    ...
    def privilege_dlg(self):
        """
        warning dialog for non-super user
        """
        if os.getuid() == 0:
            self.superuser = True
        elif not self.warning_dlg:
            self.superuser = False	
            self.warning_dlg = True
            msg = _("Insufficient permissions, Please try "
                    "DDU again as a privileged user.")
            self.dialog = gtk.MessageDialog(None, gtk.DIALOG_MODAL, 
                          gtk.MESSAGE_INFO, gtk.BUTTONS_CLOSE, msg)
            self.dialog.set_title(_("DDU"))
            self.dialog.connect("response", self.warning_dlg_exit)

            self.dialog.show() 
             
    def warning_dlg_exit(self, widget, response):
        del widget, response
        self.warning_dlg = True
        self.dialog.destroy()
	...
	if not self.superuser:
            ...
            self.server_com.set_sensitive(True)

...
def ddu_build_repo_list(ips_repo_list):
    ...
    if not isinstance(ips_repo_list, list) or len(ips_repo_list) == 0:
        ...
        #Add it directly in repo_obj_list
...
def ddu_devscan(return_missing_only = True, device_type = "all"):
    ...
    #find specific driver list
        ...
        for device_scan_index in device_scan_cndi:
            ...
            output_lines = output.splitlines()
            for line in output_lines:
            #for loop will get skipped if len(output) == 0
                component_disc = line.split(':')
                #find missing drivers list
                if return_missing_only:
                    #only handle missing driver contrller in
...
def ddu_package_lookup(ddu_dev_instance, repo_list):
    ...
    if repo_search_all == False:
        ...
        status, output = commands.getstatusoutput(
                                 "%s/scripts/comp_lookup.sh \"%s\" \"%s\" || "
                                 "%s/scripts/comp_lookup.sh \"%s\" \"%s\"" % 
                                 (ABSPATH, device_compatible, repo_names,
                                  ABSPATH, device_binding, repo_names))
        if status != 0:
            print >> sys.__stderr__, "ddu_package_lookup: comp_lookup failed."
            print >> sys.__stderr__, output
            try:
                logfile = open('/tmp/ddu_err.log', 'a')
                logfile.write("ddu_package_lookup: comp_lookup failed.")
    ...
    if package_type == "IPS":
        ...
        if status != 0:
            print >> sys.__stderr__, ("ddu_package_lookup: Error getting "
                               "package location from /tmp/%s_dlink.tmp" % 
                               package_location)
            print >> sys.__stderr__, driver_package
            try:
                logfile = open('/tmp/ddu_err.log', 'a')
                logfile.write("ddu_package_lookup: Error getting "
                                "package location from /tmp/%s_dlink.tmp" % 
                                package_location)
                logfile.write(str(driver_package))
                logfile.close()
            except IOError:
                pass
            raise ddu_errors.PackageInvalid("Error getting PKG "
                                                      "package location")
        ...
        if status != 0:
            print >> sys.__stderr__, ("ddu_package_lookup: Error getting "
                               "package information from /tmp/%s_info.tmp" % 
                               package_location)
            print >> sys.__stderr__, driver_repo
            try:
                logfile = open('/tmp/ddu_err.log', 'a')
                logfile.write("ddu_package_lookup: Error getting "
                                "package information from /tmp/%s_info.tmp" % 
                                package_location)
                logfile.write(str(driver_repo))
                logfile.close()
            except IOError:
                pass
            raise ddu_errors.PackageInvalid("Error getting PKG "
        ...
        if status != 0:
            ...
            try:
                ...
                pass
            raise ddu_errors.PackageInvalid("Error getting SVR4 package "
                                              "information link")
        ...
        if status != 0:
            ...
            try:
                ...
                pass
            raise ddu_errors.PackageInvalid("Error getting SVR4 package path")
    ...
    #This type of package are found in driverDB, it indicate the driver
    #information like the website where can download its driver, but its
    #type is not provided for installation, for example it can be source code.
        status, driver_package = commands.getstatusoutput(
                           '/usr/bin/cat /tmp/%s_dlink.tmp' % package_location)
        if status != 0:
            print >> sys.__stderr__, ("ddu_package_lookup: Error getting "
            ...
            try:
                ...
                pass
            raise ddu_errors.PackageInvalid("Error getting UNK package path")

        status, driver_repo = commands.getstatusoutput(
                           '/usr/bin/cat /tmp/%s_info.tmp' % package_location)
        if status != 0:
            print >> sys.__stderr__, ("ddu_package_lookup: Error getting "
                               "package location from /tmp/%s_info.tmp" % 
                               package_location)
            print >> sys.__stderr__, driver_repo
            try:
                logfile = open('/tmp/ddu_err.log', 'a')
                ...
                pass
            raise ddu_errors.PackageInvalid("Error getting UNK package info")
    ...
    #When the package location is blank, there is no driver information
    #available to provide to the user. Log an error. When the GUI finds this
    #kind of object it will indicate to the user that no useful information
...
def ddu_install_package(ddu_package_ob, root_install,
    ...
    if not (pkg_type in ["PKG", "SVR4", "DU", "P5I"]):
        raise ddu_errors.PackageInvalid(pkg_type)

    pkg_location = ddu_package_ob.get_pkg_location()
    if pkg_location == "":
        raise ddu_errors.PackageInvalid("blank location")

    pkg_name = ddu_package_ob.get_pkg_name()
    if pkg_name == "":
    #pkg_name is blank for DU and P5I packages. Extract the name from the
    #pkg_location."
    ...
    if third_party_from_search and install_third_party == False:
        raise ddu_errors.PackageInstallNotAllowed(pkg_name)
    ...
    if pkg_type == "PKG":
        ...
        if status != 0:
            ...
            if not (pkg_location in output.split("\n")):
                raise ddu_errors.InstallAreaUnaccessible(pkg_location)
        status, output = commands.getstatusoutput(
        ...
        if status != 0:
            ...
            try:
                ...
                pass
            raise ddu_errors.InstallPkgFail(pkg_name)
        ...
        if status != 0:
            ...
            try:
                ...
                pass
            raise ddu_errors.InstallUnkFail(pkg_type, pkg_name)		

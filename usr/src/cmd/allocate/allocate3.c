 ...
 */
int
_is_authorized(char *auths, uid_t uid, ae_ticket_t *ae)
{
	...
	if (getpwuid_r(uid, &pw_ent, pw_buf, sizeof (pw_buf)) == NULL) {
		audit_event_set_auth_list(ae, auths, B_FALSE);
		return (0);
	}
	if (strpbrk(auths, DEVICE_AUTH_SEPARATOR) == NULL) {
		if (chkauthattr(auths, pw_ent.pw_name) == 0) {
			audit_event_set_auth_list(ae, auths, B_FALSE);
			return (0);
		}
		audit_event_set_auth_list(ae, auths, B_TRUE);
		return (1);
	}
	...
	if (dcp != NULL) {
		audit_event_set_auth_list(ae, dcp, B_TRUE);
		free(authlist);
		return (1);
	}
	free(authlist);
 ...
 */
int
_is_dev_authorized(devalloc_t *da, uid_t uid, ae_ticket_t *ae)
{
	int	ares;
	...
	if (dcp == NULL)
		return (_is_authorized(auth_list, uid, ae));
	if (_is_local(uid)) {
	...
	ares = _is_authorized(auth_list, uid, ae);
	if (subauth != NULL)
		free(subauth);
...
_list_device(int optflag, uid_t uid, devalloc_t *da, char *zonename)
{
	...
	ae_ticket_t		ae;
	boolean_t		audit_auth = B_FALSE;

	audit_event_init(ADT_da_list_devices, &ae);
	...
	if ((dm = getdmapnam(da->da_devname)) == NULL) {
		...
		dprintf("Unable to find %s in the maps database\n",
		    da->da_devname);
		audit_event_submit(&ae, NODMAPERR);
		return (NODMAPERR);
	}
	enddmapent();
	...
	audit_event_set_dev_attr(&ae);

	is_authorized = _is_dev_authorized(da, uid,
	    (system_labeled) ? &ae : NULL);
...
out:
	if (!audit_auth) {
		/*
		 * Reset the authorization list if device authorization was
		 * actually never subject of interest and the authorization
		 * result was ignored.
		 */
		audit_event_reset_auth_lists(&ae);
		/*
		 * If device listing required no authorization at all,
		 * skip auditing.
		 */
		audit_auth = (AUDIT_EVENT_GET_AUTHS(ae) != 0);
	}
	if (audit_auth) {
		audit_event_submit(&ae, error);
	}
	freedmapent(dm);
	return (error);
}
...
list_devices(int optflag, uid_t uid, char *device, char *zonename)
{
	...
	if (optflag & USERID) {
		 ...
		 */
		audit_set_auth(AUDIT_DEVICE_REVOKE_AUTH);
		if (!_is_authorized(DEVICE_REVOKE_AUTH, getuid(), NULL)) {
			audit_event_submit(&ae, UAUTHERR);
			return (UAUTHERR);
		}
		audit_event_set_def_auth(&ae, AUDIT_DEVICE_REVOKE_AUTH);
	}
	if (system_labeled) {
		if (!(optflag & USERID)) {
			audit_set_auth(AUDIT_DEV_ALLOC_AUTH);
			if (!_is_authorized(DEFAULT_DEV_ALLOC_AUTH,
			    uid, NULL)) {
				/*
			...
			audit_event_set_def_auth(&ae, AUDIT_DEV_ALLOC_AUTH);
		}
		if (optflag & LISTDEFS) {
			/*
			...
			if (device) {
				...
				if (da_defs == NULL) {
					...
					dprintf("No default attributes for "
					    "%s\n", device);
					audit_event_submit(&ae, DEFATTRSERR);
					return (DEFATTRSERR);
				}
    ...
    char *clean_arg)
{
	...
	case 0:
		...
		if (system_labeled && (optflag & WINDOWING)) {
			...
			(void) strlcpy(wdwpath, path, sizeof (wdwpath));
			(void) strlcat(wdwpath, ".windowing", sizeof (wdwpath));
    ...
    char *zonename, int *lock_fd)
{
	...
	audit_event_init((optflag & (FORCE | FORCE_ALL)) ?
	    ADT_da_deallocate_forced : ADT_da_deallocate,
	    &ae);
	...
	is_authorized = _is_dev_authorized(da, uid,
	    (optflag & (FORCE | FORCE_ALL)) ? NULL : &ae);
	if (is_authorized == ALLOC_BY_NONE) {
	...
	AUDIT_EVENT_SET_DEVLIST(ae, dm->dmap_devlist);
	...
	int *lock_fd)
{
	...
	audit_event_init((optflag & FORCE) ?
	    ADT_da_allocate_forced : ADT_da_allocate,
	    &ae);
...
allocate(int optflag, uid_t uid, char *device, char *zonename)
{
	...
	if (optflag & TYPE) {
		...
		if (system_labeled && da_check_logindevperm(device)) {
			freedaent(da);
			audit_event_submit(&ae, LOGINDEVPERMERR);
...
deallocate(int optflag, uid_t uid, char *device, char *zonename)
{
	...
	audit_event_init(ADT_da_deallocate, &ae);

	if (optflag & (FORCE | FORCE_ALL)) {
		audit_event_init(ADT_da_deallocate_forced, &ae);
		audit_set_auth(AUDIT_DEVICE_REVOKE_AUTH);
		if (!_is_authorized(DEVICE_REVOKE_AUTH, getuid(), NULL)) {
			audit_event_submit(&ae, UAUTHERR);
			return (UAUTHERR);
...
_dev_file_name(struct state_file *sfp, devmap_t *dm)
{
	...
	if (dm->dmap_devarray == NULL ||
		...
		return (NODMAPERR);
	(void) strlcpy(sfp->sf_path, dm->dmap_devarray[0],
	    sizeof (sfp->sf_path));
	if (sfp->sf_path[0] == '\0') {
...
create_znode(char *zonename, struct zone_path *zpath, devmap_t *list)
{
	...
	for (; *file != NULL; file++) {
		...
		if (strncmp(tmpfile, "/dev/", strlen("/dev/")) != 0) {
			...
			p = strchr(linkdir + 1, '/');
			*p = '\0';
			(void) strlcpy(dstlinkdir, "/dev", MAXPATHLEN);
			(void) strlcat(dstlinkdir, linkdir, MAXPATHLEN);
			(void) snprintf(srclinkdir, MAXPATHLEN, "%s/root%s",
			    zonepath, tmpfile);
			(void) symlink(dstlinkdir, srclinkdir);
			*p = '/';
			(void) strlcat(dstlinkdir, p, MAXPATHLEN);
			free(tmpfile);
...
remove_znode(char *zonename, devmap_t *dm)
{
	 ...
	 */
	(void) strlcat(zonepath, "/dev", MAXPATHLEN);
	len = strlen(zonepath);
	if (di_prof_init(zonepath, &prof)) {
		dprintf("failed to initialize dev profile at %s\n", zonepath);
		return (1);

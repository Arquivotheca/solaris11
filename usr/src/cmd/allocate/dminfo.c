...
main(int argc, char **argv)
{
	...
	if (update) {
		...
		if ((dmap.dmap_devtype = getdmapfield(NULL)) == NULL) {
			dmapi_err(EINVOKE,
			    gettext("Bad dmap_devtype in entry Argument"));
		}
		if ((dmap.dmap_devlist = getdmapfield(NULL)) == NULL) {
			dmapi_err(EINVOKE,
		...
		if (getdmapnam(dmap.dmap_devname) != NULL) {
			if (verbose) {
				(void) fprintf(stderr,
				    gettext("dmapinfo: Device name (%s) found"
		...
		if (verbose) {
			(void) fprintf(stderr,
			    gettext("dmapinfo: Device name (%s) not found"
			    "in %s.\n"), dmap.dmap_devname, filename);
		}
		/*

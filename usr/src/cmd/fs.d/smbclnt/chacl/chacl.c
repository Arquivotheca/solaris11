...
main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "vu:g:")) != -1) {
		...
		case 'g':
			...
			break;
		case ':':
			(void) fprintf(stderr, "%s: option %c requires arg\n",
			    progname, c);
			usage();
			break;
	...
	if (error) {
		(void) fprintf(stderr, "%s: can not parse ACL: %s\n",
		    progname, acl_arg);
		exit(1);
	}
	if (acl->acl_type != ACE_T) {
		(void) fprintf(stderr, "%s: ACL not ACE_T type: %s\n",
		    progname, acl_arg);
		exit(1);
	}
	...
	selector = 0;
	if (acl)
		selector |= SMB_DACL_SECINFO;
	if (uid != (uid_t)-1)
		selector |= SMB_OWNER_SECINFO;
	if (gid != (gid_t)-1)
		selector |= SMB_GROUP_SECINFO;
...
chacl(char *file, uint32_t selector, uid_t uid, gid_t gid, acl_t *acl)
{
	...
	if (error) {
		(void) fprintf(stderr, "%s: failed to convert ACL\n", progname);
		exit(1);
	}
	...
	if (Vflag) {
		 ...
		 */
		(void) printf("Solaris security data:\n");
		if (uid == (uid_t)-1)
			(void) printf("owner: -1\n");
		else
			(void) printf("owner: %u\n", uid);
		 ...
		 */
		(void) printf("CIFS security data:\n");
	...
	if (error) {
		(void) fprintf(stderr, "%s: ACL set failed, %s\n",
		    file, strerror(error));
		exit(1);
	}

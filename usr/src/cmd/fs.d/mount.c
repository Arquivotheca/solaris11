...
static char	*flags(char *, int);
static char	*remote(char *, FILE *);
 ...
 * the /etc/vfstab file.
 */
static char	mntflags[(_POSIX_MAX_INPUT+1) * 2];

static char	realdir[MAXPATHLEN];	/* buffer for realpath() calls */
static char	*vfstab = VFSTAB;
static char	*mnttab = MNTTAB;
static char	*specific_opts;		/* holds specific mount options */
static char	*generic_opts;		/* holds generic mount options */
static int	maxrun;
static int	nrun;
static int	failcnt;		/* total count of failures */
static int	lofscnt;		/* presence of lofs prohibits */
					/* parallel mounting */
static int	lofsfail;		/* count of failures of lofs mounts */
static int	exitcode;
static int	aflg, cflg, fflg, Fflg, gflg, oflg, pflg, rflg, vflg, Vflg,
		mflg, Oflg, dashflg, questflg, qflg;
...
static vfsent_t	*vfsll, *vfslltail;	/* head and tail of the global */
					/* linked list of vfstab entries */
static vfsent_t	**vfsarray;		/* global array of vfsent_t's */
static int	vfsarraysize;		/* length of the list */
...
static void	doexec(char *, char *, char **);
static void	nomem();
...
flags(char *mntopts, int flag)
{
	...
	if (mntopts == NULL || *mntopts == '\0')
		return ("read/write/setuid/devices/rstchown");
	...
	if (rstchown)
		strcat(mntflags, "/rstchown");
	else
		strcat(mntflags, "/norstchown");
	if (*opts != '\0') {
		strcat(mntflags, "/");
...
static void
doexec(char *special, char *fstype, char *newargv[])
{
	char	specialstr[2 * PATH_MAX];
	...
	specialstr[0] = '\0';
	if (special)
		(void) snprintf(specialstr, sizeof (specialstr),
		    " %s:", special);

	/* build the full pathname of the fstype dependent command. */
	(void) snprintf(full_path, sizeof (full_path),
	    "%s/%s/%s", vfs_path, fstype, myname);
	(void) snprintf(alter_path, sizeof (alter_path),
	    "%s/%s/%s", alt_path, fstype, myname);
	newargv[1] = myname;
	...
	/*
	 * First try to exec the usr/lib/fs/<fstype>/mount portion of the mount.
	 *
	 * In the event that this fails, try to fall back to
	 * etc/fs/<fstype>/mount.  For bundled filesystems, this should be a
	 * no-op.
	 *
	 * If both fail, try to give an accurate error message.
	 */
	(void) execv(full_path, &newargv[1]);
	if (errno == ENOEXEC) {
		newargv[0] = "sh";
		newargv[1] = full_path;
		(void) execv("/bin/sh", &newargv[0]);
		goto err;
	} else if (errno != ENOENT) {
		(void) fprintf(stderr,
		    gettext("%s:%s Cannot execute %s: %s\n"),
		    myname, specialstr, full_path, strerror(errno));
		exit(1);
	}
	(void) execv(alter_path, &newargv[1]);
	if (errno == ENOEXEC) {
		newargv[0] = "sh";
	...
	if (sysfs(GETFSIND, fstype) == -1) {
		(void) fprintf(stderr,
		    gettext("%s:%s FSType %s is not installed "
		    "on this system.\n"), myname, specialstr, fstype);
		exit(1);
	}

err:
	(void) fprintf(stderr,
	    gettext("%s:%s Operation not applicable to FSType %s\n"),
	    myname, specialstr, fstype);
	exit(1);
}

static char *mntopts[] = { MNTOPT_IGNORE, NULL };
#define	IGNORE    0
...
do_mounts(void)
{
	...
	for (vpprev = *vl; vp = *vl; vpprev = vp, vl++, cnt--) {
		...
		if (child == 0) {		/* child */
			...
			setup_output(vp);
			doexec(vp->v.vfs_special, vp->v.vfs_fstype, newargv);
			perror("exec");
			exit(1);
		}

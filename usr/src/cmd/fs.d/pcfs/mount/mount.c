 ...
 */
static char *pcfs_opts[] = { MNTOPT_PCFS_TIMEZONE,
    MNTOPT_PCFS_OWNER, MNTOPT_PCFS_GROUP, NULL };

#define	ARG_PCFS_TIMEZONE	0
#define	ARG_PCFS_OWNER		1
#define	ARG_PCFS_GROUP		2
...
/*
 * Specified owner option can be either a user name or user ID.
 */
static boolean_t
get_owner_id(const char *optowner, char *optbuf, size_t optbuflen)
{
	struct passwd *p;
	char fmtbuf[MAX_MNTOPT_STR];

	if ((p = getpwnam(optowner)) == NULL) {
		/* pass the value without any further validations */
		(void) snprintf(fmtbuf, sizeof (fmtbuf), "owner=%s", optowner);
	} else {
		/* user name was specified */
		(void) snprintf(fmtbuf, sizeof (fmtbuf), "owner=%u", p->pw_uid);
	}

	if (append_opt(optbuf, optbuflen, fmtbuf) == 0)
		return (B_FALSE);
...
/*
 * Specified group option can be either a group name or group ID.
 */
static boolean_t
get_group_id(const char *optgrp, char *optbuf, size_t optbuflen)
{
	struct group *g;
	char fmtbuf[MAX_MNTOPT_STR];

	if ((g = getgrnam(optgrp)) == NULL) {
		/* pass the value without any further validations */
		(void) snprintf(fmtbuf, sizeof (fmtbuf), "group=%s", optgrp);
	} else {
		/* user name was specified */
		(void) snprintf(fmtbuf, sizeof (fmtbuf), "group=%u", g->gr_gid);
	}
...
main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "Vvmr?o:Oq")) != EOF) {
		...
		case 'o':
			...
			while (*in_arg != '\0') {
				...
				case ARG_PCFS_OWNER:
					if (!get_owner_id(val, optbuf,
					    sizeof (optbuf)))
						goto invalarg;
					break;

				case ARG_PCFS_GROUP:
					if (!get_group_id(val, optbuf,
					    sizeof (optbuf)))
						goto invalarg;
					break;
	...
	if (argc - optind != 2 || error) {
		...
		(void) fprintf(stderr, gettext(
		    ...
		    "\t     owner=<valid user name or user ID>"
		    "\t     group=<valid group name or group ID>"
		    "\t     mask=<valid user_mask>"));
		exit(32);
	}

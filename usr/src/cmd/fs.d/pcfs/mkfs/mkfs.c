...
static int seek_nofdisk(int fd, bpb_t *wbpb, off64_t *seekto, ulong_t startsec);
static int ask_nicely(char *special);
static int seek_partn(int fd, char *pn, bpb_t *wbpb, off64_t *seekto);
static int yes(void);
static int check_and_handle_efi(int fd, bpb_t *wbpb, off64_t *seekto);
static int get_geom(int fd, struct dk_geom *dginfo);

#ifdef _FIRMWARE_NEEDS_FDISK
static int check_and_handle_mbrpart(int fd, bpb_t *wbpb, off64_t *seekto,
    char *diskname);
#endif
...
isDosDrive(uchar_t checkMe)
{
	return ((checkMe == DOSOS12) || (checkMe == DOSOS16) ||
	    ...
	    (checkMe == DIAGPART) || (checkMe == EFI_FS));
}
...
static
int
seek_nofdisk(int fd, bpb_t *wbpb, off64_t *seekto, ulong_t startsec)
{
	if (TotSize > 0xffff)
		wbpb->bpb.sectors_in_volume = 0;
	else
	...
	*seekto = RelOffset * BPSEC;
	wbpb->bpb.hidden_sectors = RelOffset + startsec;
	wbpb->sunbpb.bs_offset_high = (RelOffset + startsec) >> 16;
	wbpb->sunbpb.bs_offset_low = (RelOffset + startsec) & 0xFFFF;
...
/*
 * Get/compute geometry info
 * Returns 0 if successful, -1 otherwise
 *
 */
static int
get_geom(int fd, struct dk_geom *dginfo)
{
	diskaddr_t cap;
	struct dk_minfo dkminfo;

	if (ioctl(fd, DKIOCG_VIRTGEOM, dginfo) >= 0 ||
	    ioctl(fd, DKIOCG_PHYGEOM, dginfo) >= 0 ||
	    ioctl(fd, DKIOCGGEOM, dginfo) >= 0) {
		if (Verbose) {
			(void) printf(
			    gettext("Obtained number of heads = %d "
			    "from DKIOCG\n"), dginfo->dkg_nhead);
			(void) printf(gettext("Obtained sectors per track = %d"
			    " from DKIOCG\n"), dginfo->dkg_nsect);
		}
		return (0);
	}
	...
	/*
	 * fake the geometry. AS DKIOCG_PHYGEOM should always succeed
	 * on x86, this is mostly for SPARC. This is the
	 * same algorithm used in sd/cmlb for providing PHYGEOM on x86.
	 */
	if (ioctl(fd, DKIOCGMEDIAINFO, &dkminfo) >= 0) {
		cap = dkminfo.dki_capacity;
	} else {
		perror(gettext("Failed to get media capacity\n"));
		return (-1);
	}
	...
	if (cap < 160) {
		...
		dginfo->dkg_nhead = 2;
		dginfo->dkg_pcyl = 80;
		dginfo->dkg_nsect = cap / (80 * 2);
	}

	if (cap <= 0x200000) {
		dginfo->dkg_nhead = 64;
		dginfo->dkg_nsect = 32;
	} else if (cap <= 0x01000000) {
		dginfo->dkg_nhead = 128;
		dginfo->dkg_nsect = 32;
	} else {
		dginfo->dkg_nhead = 255;

		/* make nsect be smallest multiple of 63 */
		dginfo->dkg_nsect = ((cap +
		    (UINT16_MAX * 255 * 63) - 1) /
		    (UINT16_MAX * 255 * 63)) * 63;

		if (dginfo->dkg_nsect == 0)
			dginfo->dkg_nsect = (UINT16_MAX / 63) * 63;
	}

	dginfo->dkg_pcyl = cap / ((dginfo->dkg_nhead) * (dginfo->dkg_nsect));

	if (Verbose) {
		(void) printf(gettext("Computed number of heads = %d\n"),
		    dginfo->dkg_nhead);
		(void) printf(gettext("Computed sectors per track = %d\n"),
		    dginfo->dkg_nsect);
	}
...
/*
 * Checks whether the disk is EFI/GPT labeled. If it is, it assumes
 * minor nodes other than s0-s6 are not valid, as Solaris only
 * support direct access through minor node to s0-s6 slices on an EFI/GPT
 * labeled disk.
 *
 * Return 1 if not EFI/GPT labeled.
 * Return 0 if handled
 * Return -1 if any error
 */
static int
check_and_handle_efi(int fd, bpb_t *wbpb, off64_t *seekto)
{
	uint64_t	size;
	struct dk_geom dg;
	struct dk_gpt *dkgptp = NULL;
	int ret = -1;
	int partno;

	if ((partno = efi_alloc_and_read(fd, &dkgptp)) < 0) {
		/* Not EFI/GPT labeled */
		ret = 1;
		goto done_efi;
	}
	...
	if (partno > 6 || partno < 0) {
		(void) fprintf(stderr, gettext("Only slice 0 to 6 is valid "
		    "for an EFI/GPT labeled disks\n"));
		goto done_efi;
	}

	if (GetSize) {
		if ((size = dkgptp->efi_parts[partno].p_size) == 0) {
			(void) fprintf(stderr, "Partition size is 0\n");
			goto done_efi;
		}
		if (size > UINT32_MAX) {
			/*
			 * TotSize is a ulong_t, ends up in bpb's
			 * sectors_in_logical_volume which is uint32_t
			 */
			size = UINT32_MAX;
			(void) fprintf(stderr, gettext("Limiting to maximum "
			    "size %u sectors\n"), size);
		}
		TotSize = (ulong_t)size;
	}

	if (dkgptp->efi_parts[partno].p_start > UINT32_MAX) {
		/*
		 * this is what will end up in hidden_sectors which
		 * is a uint32-t
		 */
		goto done_efi;
	}
	...
	if (GetSPT || GetTPC)
		if (get_geom(fd, &dg) < 0) {
			(void) fprintf(stderr, gettext("Can not get drive "
			    "geometry for this EFI/GPT labeled disk\n"));
			goto done_efi;
		}

	wbpb->bpb.heads = (GetTPC ? dg.dkg_nhead: TrkPerCyl);
	wbpb->bpb.sectors_per_track = (GetSPT ? dg.dkg_nsect : SecPerTrk);

	if (Verbose) {
		if (GetTPC)
			(void) printf(gettext("using %d as number of heads\n"),
			    dg.dkg_nhead);
		if (GetSPT)
			(void) printf(
			    gettext("using %d as sectors per track\n"),
			    dg.dkg_nsect);
	}

	/* UEFI 2.1+ spec requires a FAT32 for EFI System partition */
	if (dkgptp->efi_parts[partno].p_tag == V_SYSTEM) {
		MakeFAT32 = 1;
		if (GetResrvd)
	...
	if (seek_nofdisk(fd, wbpb, seekto,
	    dkgptp->efi_parts[partno].p_start) != PART_FOUND)
		goto done_efi;
...
#ifdef _FIRMWARE_NEEDS_FDISK
/*
 * Checks if the device node is not associated with MBR/fdisk partition
 * nodes(p1-p36) . If so, it will try to obtain the needed info.
 *
 * Return 1 if the node did not correspond to an MBR partition.
 * Return 0 if handled
 * Return -1 if any error
 */
static int
check_and_handle_mbrpart(int fd, bpb_t *wbpb, off64_t *seekto,
    char *diskname)
{
	...
	if (ioctl(fd, DKIOCINFO, &dkinfo) < 0) {
		perror(gettext("Can not get partition/slice number\n"));
		return (-1);
	}

	partno = dkinfo.dki_partition;

	if (partno <= NDKMAP ||
	    partno > (NDKMAP + MAX_EXT_PARTS + FD_NUMPART)) {
		/* Not p1-pN */
		return (1);
	}

	if (partno > NDKMAP + FD_NUMPART) {
		...
		index = partno - NDKMAP;
		if (Verbose)
			(void) printf(gettext("Extended partition  %d"),
			    index);

		if ((p0path = strdup(diskname)) == NULL) {
			perror(gettext("diskname string dup failed\n"));
			return (-1);
		}

		if ((strrchr(p0path, ':')  != NULL) ||
		    ((s = strrchr(p0path, 'p')) == NULL) || (*(s + 1) == 0)) {
			free(p0path);
			if (Verbose)
				(void) fprintf(stderr, gettext("bad device "
				    "pathname %s\n"), diskname);
			return (-1);
		}
		...
		*(s + 2) = NULL;
		...
		if (Verbose)
			(void) printf(gettext("devpath %s, p0 path %s\n"),
			    diskname, p0path);

		if ((rval = libfdisk_init(&epp, p0path, NULL, FDISK_READ_DISK))
		    != FDISK_SUCCESS) {
			switch (rval) {
				case FDISK_ENOVGEOM:
				case FDISK_ENOPGEOM:
		...
		rval = fdisk_get_part_info(epp, index, &sysid, &relsect, &size);
		free(p0path);
		libfdisk_fini(&epp);
		if (rval != FDISK_SUCCESS) {
			if (Verbose)
				(void) fprintf(stderr, gettext("can not get "
				    "info on logical drive\n"));
			return (-1);
		}
	} else {
		/* Primary MBR partition */
		int		bufsize = DEV_BSIZE;
		struct mboot 	*bootsect = NULL;
		struct ipart  	*part;
		struct dk_minfo	dkminfo;
		...
		if (ioctl(fd, DKIOCGMEDIAINFO, &dkminfo) != -1)
			if (dkminfo.dki_lbsize != 0)
				bufsize = dkminfo.dki_lbsize;

		index = partno - NDKMAP - 1;
		if ((bootsect = (struct mboot *)calloc(bufsize, 1)) == NULL) {
			perror(gettext("bootsect alloc "));
			return (-1);
		}

		if (ioctl(fd, DKIOCGMBOOT, bootsect) < 0) {
			if (Verbose)
				(void) fprintf(stderr, gettext("DKIOCGMBOOT "
				    "failed\n"));
			free(bootsect);
			return (-1);
		}

		if (ltohs(bootsect->signature) != BOOTSECSIG) {
			if (Verbose)
				(void) fprintf(stderr, gettext("Bad Signature"
		...
		part = (struct ipart *)bootsect->parts + index;
		relsect = ltohi(part->relsect);
		size =  ltohi(part->numsect);
		sysid = part->systid;
		free(bootsect);
	}

	if (Verbose)
		(void) printf(gettext("relsect %u size %u sysid %d\n"),
		    relsect, size, sysid);

	if (relsect == 0) {
		perror(gettext("Overwrites MBR \n"));
		return (-1);
	}

	if (isDosDrive(sysid) == 0) {
		(void) fprintf(stderr, gettext("Invalid Partition "
		    "type %d\n"), sysid);
		return (-1);
	}
	...
	if (GetSize) {
		if (size  == 0) {
			(void) fprintf(stderr, gettext("Fdisk "
			    "Partition size is 0\n"));
			return (-1);
		}
		TotSize = size;
	...
	if (GetSPT || GetTPC) {
		...
		if (Verbose && GetSPT)
			(void) printf(gettext("using %d as sectors per "
			    "track\n"), dg.dkg_nsect);
	}
	...
	if ((FdiskFATsize = lookup_FAT_size(sysid)) == -1)
		return (-1);
	...
	if ((seek_nofdisk(fd, wbpb, seekto, relsect)) != PART_FOUND)
		return (-1);
...
open_and_seek(char *dn, bpb_t *wbpb, off64_t *seekto)
{
	...
	if (!suffix) {
		/*
		 * We have different possibilities.  Chances are we have
		 ...
		 * the 'nofdisk' flag.
		 *
		 * If 'nofdisk' isn't set, do a floppy-specific ioctl.
		 * If the ioctl fails, we have a good idea that they aren't
		 * really on a floppy.  We then check if the request is
		 * for a partition on an EFI/GPT labeled disk. If not,
		 * we check if the request is for an MBR partition (on x86
		 * only). Otherwise they should have given us a partition
		 * specifier.
		 */
		if (DontUseFdisk) {
			if (!(seek_nofdisk(fd, wbpb, seekto, 0))) {
				(void) close(fd);
				exit(2);
			}
			...
			/*
			 * It is possible that we are trying to use
			 * floppy specific FDIOGCHAR ioctl on USB
			 * floppy. Since sd driver, by which USB
			 * floppy is handled, doesn't support it,
			 * we can try to use disk DKIOCGGEOM ioctl
			 * to retrieve data we need. sd driver itself
			 * determines floppy disk by number of blocks
			 * (<=0x1000), then it sets geometry to 80
			 * cylinders, 2 heads.
			 *
			 * Note that DKIOCGGEOM cannot supply us with
			 * type * of media (e.g. 3.5" or 5.25"). We
			 * will set it to 3 (3.5") which is most
			 * probable value.
			 */
			...
			} else if ((ret = check_and_handle_efi(fd, wbpb,
			    seekto)) != 1) {
				/* EFI labeled disk */
				if (ret == -1) {
					/* error condition */

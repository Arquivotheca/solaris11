...
static int nfs_vers_min = NFS_VERSMIN_DEFAULT;
static int nfs_vers_max = NFS_VERSMAX_DEFAULT;
...
main(int argc, char *argv[])
{
	...
	/*
	 * Read in the NFS version values from SMF properties.
	 */
	bufsz = 4;
	...
	if (ret == 0) {
		...
		defvers = strtol(defval, (char **)NULL, 10);
		if (errno == 0)
			nfs_vers_max = defvers;
	}

	/*
	 * Sanity check NFS versions.  Configured values nfs_vers_min
	 * and nfs_vers_max must conform to:
	 * NFS_VERSMIN <= nfs_vers_min <= nfs_vers_max <= NFS_VERSMAX
	 */
	if (nfs_vers_min < NFS_VERSMIN) {
		fprintf(stderr, "server_versmin (%d) is less than minimum NFS "
		    "version (%d)", nfs_vers_min, NFS_VERSMIN);
		exit(1);
	}
	if (nfs_vers_max > NFS_VERSMAX) {
		fprintf(stderr, "server_versmax (%d) is greater than maximum "
		    "NFS version (%d)", nfs_vers_max, NFS_VERSMAX);
		exit(1);
	}
	if (nfs_vers_min > nfs_vers_max) {
		fprintf(stderr, "server_versmin (%d) > server_versmax (%d)",
		    nfs_vers_min, nfs_vers_max);
		exit(1);
	}
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	mnt_vers_max = (nfs_vers_max >= NFS_V3) ? MOUNTVERS3 : MOUNTVERS_POSIX;

	/*
	 * Create datagram and connection oriented services.
	 */
	if (mnt_vers_max >= MOUNTVERS) {
	...
	if (mnt_vers_max >= MOUNTVERS_POSIX) {
...
mount(struct svc_req *rqstp)
{
	...
	transp = rqstp->rq_xprt;
	mnt_vers = rqstp->rq_vers;
	path = NULL;

	/*
	 * Infer NFS version from mount version.
	 */
	nfs_vers = (mnt_vers == MOUNTVERS3) ? NFS_V3 : NFS_VERSION;
	...
	/*
	 * If the mount version being used implies an unsupported NFS version,
	 * reject the request.
	 */
	if (rejecting || nfs_vers < nfs_vers_min) {
		if (verbose)
	...
	/*
	 * Trusted Extension doesn't support NFSv2. To prevent circumventing TX
	 * label policy via using NFSv2 client, reject a mount request made by
	 * NFSv2 client and log an error.
	 */
	if (is_system_labeled()) {
		if (nfs_vers == NFS_VERSION) {
			if (verbose)
				syslog(LOG_ERR,
	 ...
	 */
	/* LINTED pointer alignment */
	while (nfs_getfh(rpath, nfs_vers, &len, fh) < 0) {
		if (errno == EINVAL &&
		    (sh = find_lofsentry(rpath, &lofs_tried)) != NULL) {
			errno = 0;
			continue;
...
check_sharetab(void)
{
	FILE *f;
	struct stat st;
	static volatile timestruc_t last_sharetab_time;
	 ...
	 * write lock check again.
	 *
	 * We need to hold the read lock to avoid the concurrent
	 * last_sharetab_time update.
	 */
	(void) rw_rdlock(&sharetab_lock);
	...
	if (st.st_mtim.tv_sec == prev_sharetab_time.tv_sec &&
	    st.st_mtim.tv_nsec == prev_sharetab_time.tv_nsec) {
		/*
		 * No change.
		 */
		return;
	...
	/*
	 * If another thread already did the update,

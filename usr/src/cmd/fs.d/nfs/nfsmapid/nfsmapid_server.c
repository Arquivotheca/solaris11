...
#define		DIAG_FILE		_PATH_SYSVOL "/nfs4_domain"

/*
 * idmap_kcall() takes a door descriptor as its argument when we
...
static unsigned int nfsmapid_str_unqualified(const char *s, unsigned long *pid,
    boolean_t is_user);
...
nfsmapid_str_uid(struct mapid_arg *argp, size_t arg_size)
{
	...
	if (!extract_domain(argp->str, &user, &domain)) {
		...
		/*
		 * Invalid "user@domain" string. Still, the
		 * user part might be an encoded uid or SID, so do a
		 * final check. Remember, domain part of string
		 * was not set since not a valid string.
		 */
		result.status = nfsmapid_str_unqualified(user, &id, B_TRUE);
...
nfsmapid_uid_str(struct mapid_arg *argp, size_t arg_size)
{
	 ...
	 */
	if (uid > UID_MAX) {
		rc = idmap_nfsv4_getwinnamebyuid(uid, 0, &idmap_buf);
		if (rc != IDMAP_SUCCESS) {
			/*
	...
	if ((pwd_buf = malloc(pwd_buflen)) == NULL || dom_str_len == 0 ||
		 ...
		 */
		if (pwd_buf == NULL) {
			str_size = MAPID_RES_LEN(UID_MAX_STR_LEN);
			if ((pwd_buf = alloca(str_size)) == NULL) {
				resp = &result;
		 ...
		 */
		pw_str = pwd_buf;
		(void) snprintf(pw_str, str_size, "%u", uid);
		pw_str_len = strlen(pw_str);
		at_str_len = dom_str_len = 0;
		at_str = "";
		dom_str[0] = '\0';
...
gen_result:
	...
	(void) snprintf(resp->str, uid_str_len + 1, "%s%s%s", pw_str, at_str,
	    dom_str);
...
nfsmapid_str_gid(struct mapid_arg *argp, size_t arg_size)
{
	...
	if (!extract_domain(argp->str, &group, &domain)) {
		 ...
		 * group part might be an encoded gid or SID, so do a
		 * final check. Remember, domain part of string
		 * was not set since not a valid string.
		 */

		result.status = nfsmapid_str_unqualified(group, &id, B_FALSE);
...
nfsmapid_gid_str(struct mapid_arg *argp, size_t arg_size)
{
	 ...
	 */
	if (gid > UID_MAX) {
		rc = idmap_nfsv4_getwinnamebygid(gid, 0, &idmap_buf);
		if (rc != IDMAP_SUCCESS) {
			/*
	...
	if ((grp_buf = malloc(grp_buflen)) == NULL || dom_str_len == 0 ||
		...
		if (grp_buf == NULL) {
			str_size = MAPID_RES_LEN(UID_MAX_STR_LEN);
			if ((grp_buf = alloca(str_size)) == NULL) {
				resp = &result;
				resp->status = NFSMAPID_INTERNAL;
				resp->u_res.len = 0;
				goto done;
		 ...
		 */
		gr_str = grp_buf;
		(void) snprintf(gr_str, str_size, "%u", gid);
		gr_str_len = strlen(gr_str);
		at_str_len = dom_str_len = 0;
		at_str = "";
		dom_str[0] = '\0';
...
gen_result:
	...
	(void) snprintf(resp->str, gid_str_len + 1, "%s%s%s", gr_str, at_str,
	    dom_str);
...
{
	/*
	 * Caller must ensure that the string is valid
	 */
	*upp = cp;
...
/*
 * Process an unqualified name s.
 * is_user = B_TRUE:  s is a user
 * is_user = B_FALSE: s is a group
 * Return the UID or GID at *pid.
 * Return an appropriate status code.
 */
static unsigned int
nfsmapid_str_unqualified(const char *s, unsigned long *pid, boolean_t is_user)
{
	unsigned long nobody = is_user ? UID_NOBODY : GID_NOBODY;
	idmap_stat rc;

	if (validate_id_str(s)) {
		errno = 0;
		*pid = strtoul(s, (char **)NULL, 10);

		/*
		 * We don't accept ephemeral ids from the wire.
		 */
		if (errno || *pid > UID_MAX) {
			*pid = nobody;
	...
	/* Give idmap a crack at it, to support SIDs */
	if (is_user) {
		uid_t uid;
		rc = idmap_nfsv4_getuidbysid(s, 0, &uid);
		*pid = uid;
	} else {
		gid_t gid;
		rc = idmap_nfsv4_getgidbysid(s, 0, &gid);
		*pid = gid;
	}

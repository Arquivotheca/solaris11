...
static void log_notify(const char *, SVCXPRT *, int);
static char *fmtaddr(struct netconfig *, struct netbuf *);
	...
	SVCXPRT *transp;
{
	...
	if (rqstp->rq_prog == NSM_ADDR_PROGRAM) {
		...
		case SM_SIMU_CRASH:
			/*
			 * If the uid is not of daemon, the credentials cannot
			 * be trusted, drop the request.
			 */
			if (svc_getcallerucred(transp, &uc) != 0 ||
			    ucred_geteuid(uc) != daemon_uid ||
			    ucred_getzoneid(uc) != getzoneid()) {
				calleraddr = (struct sockaddr_in *)
				    svc_getrpccaller(transp)->buf;

				memset(raddr, '\0', INET6_ADDRSTRLEN);
				if (calleraddr->sin_family == AF_INET)
					(void) inet_ntop(AF_INET,
					    (void *)&calleraddr->sin_addr,
					    raddr, INET_ADDRSTRLEN);
				else
					(void) inet_ntop(AF_INET6,
					    (void *)&((struct sockaddr_in6 *)
					    calleraddr)->sin6_addr, raddr,
					    INET6_ADDRSTRLEN);

				syslog(LOG_INFO,
				    "Ignoring SM_SIMU_CRASH received from %s",
				    raddr[0] != '\0' ? raddr : "unknown host");
				ucred_free(uc);
				return;
			}
			ucred_free(uc);
	...
	if (rqstp->rq_proc == SM_NOTIFY) {
		log_notify(argument.ntf_arg.mon_name, transp, LOG_NOTICE);
	}
...
static int
create_srv_service(void (*dispatch) (), ulong_t prognum, ulong_t versnum)
{
	struct SVCXPRT_LIST {
		SVCXPRT *xprt;
		struct SVCXPRT_LIST *next_xprt;
	};
	static struct SVCXPRT_LIST *xprt_list = NULL;
	struct SVCXPRT_LIST *temp;
	...
	if ((handlep = setnetconfig()) == NULL) {
		syslog(LOG_ERR, "create_srv_service: failed to iterate "
		    "over netconfig database\n");
		return (-1);
	}

	/*
	 * Clear existing rpcbind registrations.
	 */
	svc_unreg(prognum, versnum);

	while ((nconf = getnetconfig(handlep)) != NULL) {
		if (nconf->nc_semantics == NC_TPI_RAW)
			continue;

		/*
		 * Check for existing open transport which matches
		 * current netid.
		 */
		for (temp = xprt_list; temp; temp = temp->next_xprt) {
			if (strcmp(temp->xprt->xp_netid, nconf->nc_netid)
			    == 0) {
				if (svc_reg(temp->xprt, prognum, versnum,
				    dispatch, nconf) == FALSE) {
					syslog(LOG_ERR,
					    "create_srv_service: failed to "
					    "register rpc service on %s\n",
					    nconf->nc_netid);
					/*
					 * Do not destroy xprt here as it
					 * will be in use.
					 */
					break;
				}
		...
		if ((fd = t_open(nconf->nc_device, O_RDWR, NULL)) == -1) {
			syslog(LOG_ERR, "create_srv_service: failed to "
			    "open %s\n", nconf->nc_device);
			continue;
		}
		...
		if ((xprt = svc_tli_create(fd, nconf, NULL, 0, 0)) == NULL) {
			(void) t_close(fd);
			continue;
		}
		...
		/*
		 * Add xprt to xprt_list for reusing this transport.
		 */
		temp = (struct SVCXPRT_LIST *)
		    malloc(sizeof (struct SVCXPRT_LIST));
		if (temp == NULL) {
			/*
			 * Destroy xprt as we do not want more than one
			 * xprt open for the same netid.
			 */
			svc_destroy(xprt);
			syslog(LOG_ERR, "create_srv_service: malloc failed\n");
			continue;
		}
		temp->xprt = xprt;
		temp->next_xprt = xprt_list;
		xprt_list = temp;
	}
	...
	/*
	 * There has to be atleast one transport registered for svc_run
	 * to take over.
	 */
	return (ntransp ? 0 : -1);
}
...
main(int argc, char *argv[])
{
	...
	if (create_srv_service(sm_prog_1, SM_PROG, SM_VERS)) {
		syslog(LOG_ERR,
	    "statd: unable to create (SM_PROG, SM_VERS) for netpath.");
		exit(1);
	}

	if (create_srv_service(sm_prog_1, NSM_ADDR_PROGRAM, NSM_ADDR_V1)) {
		syslog(LOG_ERR,
...
/*
 * Generate a syslog message in response to receipt of an SM_NOTIFY call.
 */

static void
log_notify(const char *name, SVCXPRT *transp, int loglevel)
{
	char *sender;			/* hostname of sender */
	char *snd_addr = NULL;
	struct netconfig *nconf;
	struct nd_hostservlist *hlist = NULL;

	nconf = getnetconfigent(transp->xp_netid);
	if (nconf == NULL) {
		sender = "UnknownHost";
	} else {
		snd_addr = fmtaddr(nconf, &transp->xp_rtaddr);
		if (netdir_getbyaddr(nconf, &hlist, &transp->xp_rtaddr) != 0) {
			syslog(LOG_ERR, "Can't look up SM_NOTIFY sender: %s",
			    netdir_sperror());
			sender = "UnknownHost";
		} else {
			sender = hlist->h_hostservs[0].h_host;
		}
	}

	if (snd_addr == NULL) {
		const struct netbuf *nbp = &transp->xp_rtaddr;
		int prefixlen, buflen;
		char *family = NULL;

		if (nconf != NULL)
			family = nconf->nc_protofmly;
		if (family == NULL)
			family = "UnknownFamily";

		/* print the netbuf as a series of hex characters */
		buflen = strlen(family) + strlen(": ") + (2 * nbp->len) + 1;
		snd_addr = malloc(buflen);
		if (snd_addr != NULL) {
			(void) strcpy(snd_addr, family);
			(void) strcat(snd_addr, ": ");
			prefixlen = strlen(snd_addr);
			format_netbuf(snd_addr + prefixlen,
			    buflen - prefixlen, nbp);
		}
	}

	syslog(loglevel, "Received SM_NOTIFY (%s) from %s (%s)", name,
	    sender, snd_addr ? snd_addr : "(out of memory)");

	if (nconf != NULL)
		freenetconfigent(nconf);
	if (hlist != NULL)
		netdir_free(hlist, ND_HOSTSERVLIST);
	free(snd_addr);
}

/*
 * Return the given address as a printable string, omitting port
 * information.  The caller is responsible for freeing the returned
 * string, if non-NULL.
 */

static char *
fmtaddr(struct netconfig *nconf, struct netbuf *addr)
{
	char *result = NULL;
	const char *tr;			/* temp result */
	char ipbuf[INET6_ADDRSTRLEN];

	if (strcmp(nconf->nc_protofmly, NC_INET) == 0) {
		struct sockaddr_in *v4addr = (struct sockaddr_in *)addr->buf;

		tr = inet_ntop(AF_INET, (void *)&v4addr->sin_addr,
		    ipbuf, sizeof (ipbuf));
		if (tr != NULL)
			result = strdup(tr);
	} else if (strcmp(nconf->nc_protofmly, NC_INET6) == 0) {
		struct sockaddr_in6 *v6addr = (struct sockaddr_in6 *)addr->buf;

		tr = inet_ntop(AF_INET6, (void *)&v6addr->sin6_addr,
		    ipbuf, sizeof (ipbuf));
		if (tr != NULL)
			result = strdup(tr);
	} else {
		result = taddr2uaddr(nconf, addr);
	}

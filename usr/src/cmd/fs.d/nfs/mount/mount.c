	...
	bool_t loud_on_mnt_err, struct netconfig **nconfp, ushort_t port)
{
	...
	case MOUNTVERS3:
		*versp = nfsvers_to_use = NFS_V3;
		 ...
		 *
		 * It is possible that some NFS server distros might not return
		 * any security flavors (i.e count == 0). And if "sec=" mount
		 * option was not specified (sec_opt == 0), we make a free fall
		 * and pick clients default security mode.
		 */
		auths =
...
/*
 * Read the NFS Parameters  to determine if the
...
read_default(void)
{
	...
	bufsz = 4;
	ret = wrapped_get_prop(NFS_CLIENT_VERSMIN, value, DEFAULT_INSTANCE,
	    sizeof (value));
	if (ret == 0) {
		errno = 0;
	...
	/* Maximum number of bytes expected. */
	bufsz = 4;
	ret = wrapped_get_prop(NFS_CLIENT_VERSMAX, value, DEFAULT_INSTANCE,
	    sizeof (value));
	if (ret == 0) {
		errno = 0;
		tmp = strtol(value, (char **)NULL, 10);

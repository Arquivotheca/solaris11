...
main(int ac, char *av[])
{
	...
	bufsz = PATH_MAX;
	ret = nfs_smf_get_prop("server_nodeid", value, DEFAULT_INSTANCE,
	    SCF_TYPE_ASTRING, NFSD, &bufsz);
	if (ret == 0) {
		nodeid = atoi(value);
		if (nodeid == 0) {
			(void) fprintf(stderr, "nodeid cannot be zero\n");
			exit(1);
		}
	}
	...
	while ((i = getopt(ac, av, "ac:p:s:t:l:n:")) != EOF) {
		switch (i) {
		case 'a':
			free(df_proto);
			df_proto = NULL;
		...
		case 's':
			if (getzoneid() != GLOBAL_ZONEID) {
				(void) fprintf(stderr,
				    "%s: -s only supported in Global Zone!\n",
				    av[0]);
				exit(1);
			}
	...
	/*
	 * Set the nodeID.  The precedence rules are:
	 * use the value given with -n, if present
	 * use the value of the smf property, if present
	 * use the hostid
	 */
	if (nodeid == 0)
		nodeid = (int)gethostid();

	nodeid &= NFS_MAX_NODEID-1;
	/*
	 * Ignore any error, this is traditional nfsd behavior.
	 */
	(void) _nfssys(NFS_NODEID, &nodeid);

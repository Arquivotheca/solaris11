...
usage()
{
	...
	(void) fprintf(stderr, gettext("\tnfsref [-t type] remove path\n"));
	(void) fprintf(stderr, gettext("\tnfsref [-t type] lookup path\n"));
}
...
{
	int err, i, len, oldlen, notfound = 0;
	char *text, *location;
	nvlist_t *nvl = NULL;
	...
	for (i = optind; i < argc; i++) {
		...
		location[oldlen] = '\0';
		oldlen = len;
		(void) strlcat(location, buf, len);
		(void) strlcat(location, " ", len);
	}
	location[len - 2] = '\0';
	...
	if (notfound)
		(void) printf(gettext("Created reparse point %s\n"), sl_path);
	else
		(void) printf(gettext("Added to reparse point %s\n"), sl_path);
...
delref(char *sl_path, char *svc_type)
{
	...
	(void) printf(gettext("Removed svc_type '%s' from %s\n"), svc_type,
	    sl_path);
	return (err);
}
...
lookup(char *sl_path, char *svc_type, int type_set)
{
	...
	err = lstat(sl_path, &stbuf);
	if (err == -1)
		return (errno);
	if ((stbuf.st_mode & S_IFMT) == S_IFLNK) {
		sz = readlink(sl_path, buf, SYMLINK_MAX);
		if (sz == -1)
			return (errno);
	} else {
		int fd1, fd2;

		fd1 = open(sl_path, O_RDONLY);
		if (fd1 == -1)
			return (errno);
		...
		if (stbuf.st_size > SYMLINK_MAX) {
			(void) close(fd2);
			return (EFBIG);
		}
		sz = read(fd2, buf, stbuf.st_size);
		(void) close(fd2);
		if (sz == -1)
			return (errno);
		if (sz != stbuf.st_size)
	...
	(void) nvpair_value_string(curr, &svc_data);
	...
	if (err != TRUE)
		return (ENOENT);
	(void) printf(gettext("%s points to: "), sl_path);
...
main(int argc, char *argv[])
{
	...
	if (err != 0)
		(void) fprintf(stderr, gettext("Command %s failed: %s\n"),

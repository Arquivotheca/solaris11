...
"       accessible until the system is rebooted with a 64-bit kernel.\n"));
	 ...
	 * geometry style layout because of user specified layout values,
	 * thereby warranting the need for alternate
	 * logic in superblock detection.
	 */
	if (ntrack == -1 || GROW_WITH_DEFAULT_TRAK || DEFAULT_SECT_TRAK_CPG) {
		/*
...
dump_fscmd(char *fsys, int fsi)
{
	...
	if ((sblock.fs_magic == FS_MAGIC && !UFS_VALID_VERSION(&sblock)) ||
	    (sblock.fs_magic == MTB_UFS_MAGIC &&
	    !MTB_UFS_VALID_VERSION(&sblock))) {
		(void) fprintf(stderr, gettext(
...
checksblock(struct fs sb, int proceed)
{
	...
	} else if ((sb.fs_magic == FS_MAGIC && !UFS_VALID_VERSION(&sb)) ||
	    (sb.fs_magic == MTB_UFS_MAGIC && !MTB_UFS_VALID_VERSION(&sb))) {
		err = 2;

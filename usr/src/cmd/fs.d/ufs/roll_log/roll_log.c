...
rl_roll_log(char *bdev)
{
	 ...
	 *
	 * Mounting the device (read only) will populate the in-memory
	 * logmap with the deltas from the log. The log can then be rolled
	 * by calling the flush ioctl (_FIOFFS) via rlflush(). Within the
	 * kernel the flush is handled by ufs_flush() which for a logging
	 * file system will call logmap_roll_dev() resulting in a log roll.
	 * The file system is then unmounted.
	 *
	 * To perform the mount, we need to create a temporary directory,
	 * and then remove it when we are done. There is a very small
	 * opportunity for the umount to fail with EBUSY, in which case
	 * perform a forced umount. This is acceptable as the call to
	 * rlflush() will have rolled the log.
	 */
	rv = make_mp(&li);
	switch (rv) {
	case RL_CORRUPT:
	...
	case RL_SUCCESS:
		...
		if (rv == RL_SUCCESS) {
			rv = rlflush(&li);
			if (((uret = umount(li.li_blkname)) == SYSERR) &&
			    (errno == EBUSY))
				uret = umount2(li.li_blkname, MS_FORCE);
			if (uret)
				(void) fprintf(stderr, "WARNING: "
				    "rl_roll_log(): Can't unmount %s\n",
				    li.li_blkname);
		}
		break;
	}
...
make_mp(log_info_t *lip)
{
	...
	if (rv != RL_SUCCESS) {
		...
		(void) fprintf(stderr, gettext("Unable to create temporary "
		    "directory in any of the directories listed below:\n"));
		for (i = 0; i < list_len; i++) {
			(void) fprintf(stderr, "\t%s\n", tmp_dir_list[i]);
		}
		(void) fprintf(stderr, gettext("Please correct this problem "
...
rlmount(log_info_t *lip, int mntopt)
{
	...
	(void) snprintf(opt, sizeof (opt), "%s,%s,%s", optstr, MNTOPT_NOSUID,
	    MNTOPT_LARGEFILES);
	if (mount(lip->li_blkname, lip->li_tmpmp,
	    optflg | MS_DATA | MS_OPTIONSTR,
	    MNTTYPE_UFS, &args, sizeof (args), opt, MAX_MNTOPT_STR) == SYSERR) {
		rv = RL_SYSERR;
	}
	return (rv);
}
...
rlumount(log_info_t *lip)
{
	...
	if (umount(lip->li_blkname) == SYSERR) {
		(void) fprintf(stderr, gettext("WARNING: rlumount(): "
		    "Can't unmount %s\n"), lip->li_blkname);
		rv = RL_SYSERR;
	}
	return (rv);

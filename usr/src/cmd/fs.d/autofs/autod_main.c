...
#ifdef DEBUG
#define	AUTOFS_DOOR	_PATH_SYSVOL "/autofs_door"
#endif /* DEBUG */
...

{
	...
	/*
	 * Read in the values from File/SMF first before we check
	 * commandline options so the options override the file.
	 */
	bufsz = 6;
	ret = wrapped_get_prop(AUTOMOUNTD_VERBOSE, defval,
	...
	bufsz = 6;
	ret = wrapped_get_prop(AUTOMOUNTD_NOBROWSE, defval,
	    DEFAULT_INSTANCE, sizeof (defval));
	if (ret == 0) {
		if (strncasecmp("true", defval, 4) == 0)
	...
	bufsz = 6;
	ret = wrapped_get_prop(AUTOMOUNTD_TRACE, defval,
	    DEFAULT_INSTANCE, sizeof (defval));
	if (ret == 0) {
		errno = 0;
	...
	wrapped_put_automountd_env(AUTOMOUNTD_ENV);

...
static struct mapent *do_mapent_fedfs(char *, char *);
			...
			char *subdir, uint_t isdirect, bool_t mount_access)
{
	...
	if (strcmp(lp, "-hosts") == 0) {
		...
		if (hierarchical_sort(mapents, &rootnode, key, mapname)
		    != PARSE_OK)
			goto parse_error;
	} else if (strcmp(lp, "-fedfs") == 0) {
		/*
		 * the /nfs4 parser - uses do_mapent_fedfs to build a mapent.
		 * The mapopts are considered default for every entry, so we
		 * don't push options down hierarchies.
		 */
		mapents = do_mapent_fedfs(mapopts, key);
		if (mapents == NULL)		/* nothing to free */
			return (mapents);

		if (trace > 3)
			trace_mapents("do_mapent_fedfs:(return)", mapents);
	 ...
	 */
	if (strcmp(lp, "-fedfs") != 0 &&
	    modify_mapents(&mapents, mapname, mapopts, subdir,
 ...
 * Note that defaultopts may be the same as mapopts.
 * Returns PARSE_OK or appropriate error value.
 */
static int
set_mapent_opts(struct mapent *me, char *opts, char *defaultopts,
	...
	uint_t isdirect;
{
	...
	struct exportnode *exlist = NULL;
	struct exportnode *texlist, **texp, *exnext;
	...
	if (self_check(host)) {
		...
		/*
		 * Call pingnfs. Note that we can't have replicated hosts in
		 * /net.  XXX - we would like to avoid duplicating the across
		 * the wire calls made here in nfsmount(). The pingnfs cache
		 * should help avoid it.
		 */
		if (pingnfs(host, get_retry(mapopts) + 1, &vers, versmin, 0,
		    FALSE, NULL, NULL) != RPC_SUCCESS)
			return ((struct mapent *)NULL);
	}

	/* avoid use of mount protocol for lofs and NFSv4 mounts */
	if (lofs_mount == B_TRUE || vers == NFS_V4) {
		...
		if (ms->map_mounter == NULL)
			goto alloc_failed;
		ms->map_fstype = strdup(MNTTYPE_NFS4);
		...
		if (trace > 1)
			trace_prt(1, "do_mapent_hosts: mount protocol avoided "
			    "for %s\n", host);
...
#define	FEDFS_ROOT_PREFIX "_nfs4._domainroot._tcp"

/*
 * Use DNS lookup of domain to get one or more hosts, and then
 * mount <host[,host]>:/.domainroot-<dns_domain> on /nfs4/<dns_domain>.
 */
struct mapent *
do_mapent_fedfs(mapopts, domain)
	char *mapopts, *domain;
{
	struct mapent *me;
	struct mapfs *mfs, *tmp_mfs;
	char name[MAXPATHLEN];
	char entryopts[MAXOPTSLEN];
	char fstype[32];
	struct mnttab mb;	/* needed for hasmntopt() to get nfs version */
	rpcvers_t vers;		/* version for pingnfs() */
	int foundvers, pinged, num, i;
	char **hostlist, pathbuf[MAXPATHLEN];

	if (trace > 1)
		trace_prt(1, "  do_mapent_fedfs: domain %s\n", domain);

	/*
	 * Look for _nfs4._domainroot._tcp SRV RR records
	 */
	num = dns_srv_list(FEDFS_ROOT_PREFIX, domain, &hostlist);
	if (num < 1) {
		syslog(LOG_ERR,
		    "do_mapent_fedfs: Cannot find entry for %s", domain);
		return ((struct mapent *)NULL);
	}

	/*
	 * Call pingnfs for each host to ensure it is available.
	 */
	mb.mnt_mntopts = mapopts;
	foundvers = nopt(&mb, MNTOPT_VERS, (int *)&vers);
	if (foundvers != 0 && vers != NFS_V4) {
		syslog(LOG_ERR, "do_mapent_fedfs: Incorrect NFS "
		    "version specified for %s", domain);
		free_srv_list(hostlist, num);
		return ((struct mapent *)NULL);
	}
	vers = NFS_V4;

	sprintf(pathbuf, "/.domainroot-%s", domain);
	(void) strcpy(name, "/");
	(void) strcat(name, domain);

	pinged = 0;
	for (i = 0; i < num; i++) {
		if (pingnfs(hostlist[i], get_retry(mapopts) + 1, &vers, vers,
		    0, FALSE, NULL, NULL) != RPC_SUCCESS) {
			syslog(LOG_ERR,
			    "do_mapent_fedfs: Cannot ping %s", hostlist[i]);
			free(hostlist[i]);
			hostlist[i] = NULL;
			continue;
		}
	...
	me->map_fstype = strdup(MNTTYPE_NFS4);
	if (me->map_fstype == NULL)
		goto alloc_failed;
	(void) strcpy(fstype, MNTTYPE_NFS);
	me->map_mounter = strdup(fstype);
	if (me->map_mounter == NULL)
		goto alloc_failed;
	get_opts(mapopts, entryopts, fstype, NULL);
	me->map_mntopts = strdup(entryopts);
	...
	for (i = 0; i < num; i++) {
		...
		if (me->map_fs == NULL)
			...
			me->map_fs->mfs_next = mfs;

		mfs->mfs_host = strdup(hostlist[i]);
		if (mfs->mfs_host == NULL)
			goto alloc_failed;
		mfs->mfs_dir = strdup(pathbuf);
		if (mfs->mfs_dir == NULL)
			goto alloc_failed;
	}
	free_srv_list(hostlist, num);
	...
	if (trace > 1)
		trace_prt(1, "  do_mapent_fedfs: domain %s OK\n", domain);
	trace_mapents("  do_mapent_fedfs:(just built)", me);
...
alloc_failed:
	...
	free(me->map_mounter);
	free(me->map_mntopts);
	for (mfs = me->map_fs; mfs; mfs = tmp_mfs) {
		free(mfs->mfs_host);
		free(mfs->mfs_dir);
		tmp_mfs = mfs->mfs_next;
		free(mfs);
	}
	free(me);
	free_srv_list(hostlist, num);
	syslog(LOG_ERR, "do_mapent_fedfs: Memory allocation failed");
	return ((struct mapent *)NULL);
}

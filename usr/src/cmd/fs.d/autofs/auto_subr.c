...
extern boolean_t check_for_solaris10_brand();
/*
 * If the system is labeled then we need to have a uniquely-named auto_home
 * map for this zone.  The maps are made unique by appending the zonename.
 *
 * For auto_home_<zonename> a default template map is created only if it
 * doesn't exist yet. The default entry is used to declare local home
 * directories created within each zone. For example:
 *
 *	+auto_home_public
...
loadzone_maps(char *mntpnt, char *map, char *opts, char **stack, char ***stkptr)
{
	...
	my_zoneid = getzoneid();
	if (getzonenamebyid(my_zoneid, zonename, ZONENAME_MAX) != -1) {
		...
		/* Next create auto_home_<zone> map if needed using template */
		...
		if ((fd = open(map_path, O_RDWR | O_CREAT | O_EXCL,
			...
			/* Make sure /export/home exists in this zone. */
			(void) mkdir_r("/export/home", 0755);
		}
	}
}
			...
			*pn = '\0';
			...
			if (!s) {
				...
				if (strcmp(namebuf, "ARCH") == 0) {
					...
					if (check_for_solaris10_brand(zid)) {
						s = S10_UTS_RELEASE;
					} else {
						(void) uname(&name);
						s = name.release;
 ...
 * is preceded by the ID of the calling thread.
 */

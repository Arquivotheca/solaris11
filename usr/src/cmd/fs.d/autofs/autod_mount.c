	...
	*alpp = NULL;
	...
	for (me = mapents; me && !err; me = me->map_next) {
		...
		if (strcmp(me->map_fstype, MNTTYPE_NFS) == 0 ||
		    strcmp(me->map_fstype, MNTTYPE_NFS3) == 0 ||
		    strcmp(me->map_fstype, MNTTYPE_NFS4) == 0) {
			...
			/*
			 * Refresh default NFS version based on configuration.
			 * Skip this step if NFS version to be used has already
			 * been determined in do_mapent_hosts().
			 */
			if (strcmp(me->map_fstype, MNTTYPE_NFS) == 0)
				read_default_nfs();
			err = mount_nfs(me, spec_mntpnt, private, overlay, uid,
			    &alp);
			/*

...
try_mnt_slash:
			 ...
			 * If security flavor is in mount options, check
			 * if server supports the flavor. If security
			 * flavor is not in mount options, then look for
			 * default auth (default auth entry in /etc/nfssec.conf)
			 * in the auth list returned from server. If default
			 * auth not found, then use the first supported
			 * security flavor (by the client) in the auth
			 * list returned from the server.
			 *
			 * It is possible that some NFS server distros might
			 * not return any security flavors (i.e count == 0).
			 * And if "sec=" mount option was not specified
			 * (sec_opt == 0), we make a free fall and pick clients
			 ...
			 */
				...
				if (sec_opt) {
					...
					nfs_sec.sc_nfsnum = -1;
					def_sec.sc_nfsnum = -1;
					(void) nfs_getseconfig_default(
					    &def_sec);

					/*
					 * Look for clients default auth in
					 * servers list.
					 */
					if (def_sec.sc_nfsnum != -1) {
						for (i = 0; i < count; i++) {
							if (auths[i] ==
							    def_sec.sc_nfsnum) {
								sec_opt++;
					...
					/*
					 * Could not find clients default auth
					 * in servers list. Pick the first auth
					 * from servers list that is also
...
read_default_nfs(void)
{
	...
	bufsz = 4;
	ret = wrapped_get_prop(NFS_CLIENT_VERSMIN, defval,
	...
	bufsz = 4;
	ret = wrapped_get_prop(NFS_CLIENT_VERSMAX, defval,
	    DEFAULT_INSTANCE, sizeof (defval));
	if (ret == 0) {
		errno = 0;
		tmp = strtol(defval, (char **)NULL, 10);
...
/*
 * This function returns the parent dataset path corresponding
 * to the mountpath, if it exists.
 */
static int
get_zfs_dataset(const char *src, char *dataset, size_t dsetlen)
{
	FILE *mtab;
	struct mnttab mget;
	struct mnttab mref;
	...
	(void) strlcpy(mountpoint, src, sizeof (mountpoint));
	if (slash = strrchr(mountpoint, '/')) {
		*slash = '\0';
	} else {
		return (0);
	}
	...
	if (mtab) {
		...
		if (err == 0 && mget.mnt_special != NULL) {
			(void) strlcpy(dataset, mget.mnt_special, dsetlen);
			return (0);
		} else {
			return (-1);
		}
...
/*
 * create zfs dataset for the user and mount it
 */
static int
zfs_create_dataset(char *user, const char *mountpoint, char *dataset)
{
	nvlist_t *props = NULL;
...
create_homedir(const char *src, const char *dst) {
	...
	/* Check that source is a home directory entry */
	if (src_dir_len < 0 ||
	...
	if (get_zfs_dataset(src, dataset, sizeof (dataset)) == 0) {
		/*
		 * If parent directory is a mounted dataset
		 * create a new dataset for the homedir
		 */
		if (zfs_create_dataset(dst_username, src, dataset) < 0) {
			if (trace > 1) {
				trace_prt(1, "	Couldn't create dataset %s\n",
				    dataset);
			}
			return (0);
		}

...
#define	ERR_USAGE		0x8	/* print the usage line after the */
					/* message */
#define	ERR_CMDOPT		0x10	/* incorrect command line option */
...
{
	errmsg(ERR_NONAME,
	    "Usage: %s [-F FSType] [-abeghklntPVvZ]"
...
parse_options(int argc, char *argv[])
{
	...
	while ((arg = getopt(argc, argv, "F:o:abehkVvtgnlPZ")) != EOF) {
		if (arg == 'F') {
			if (F_option)
				errmsg(ERR_FATAL | ERR_USAGE | ERR_CMDOPT,
	...
	if (k_option && v_option)
		errmsg(ERR_FATAL | ERR_USAGE | ERR_CMDOPT,
		    "-k and -v options are incompatible");
}

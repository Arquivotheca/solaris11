...
/* Existing Mellanox ConnectX drivers */
char *drivername[] = {
	"hermon\0",	/* IB driver */
	"mcxnex\0",	/* nexus driver for the 10GbE driver */
	NULL
};
...
/* helper functions */
static int fw_identify_per_driver(int *start, char *drvname);
static int cnx_identify(struct devicelist *thisdev, char *drvname);
...
static int cnx_set_guids(ib_cnx_encap_ident_t *handle, void *arg1, void *arg2);
...
fw_writefw(struct devicelist *flashdev)
{
	...
	/* check if the two MAC GUIDs are invalid */
	if ((manuf->nguids == 4) || ((manuf->nguids == 6) &&
	    ((manuf->ib_mac[0] & 0xffffffffffffull) == 0xffffffffffffull) &&
	    ((manuf->ib_mac[1] & 0xffffffffffffull) == 0xffffffffffffull))) {
		uint64_t mac;

		for (i = 0; i < 2; i++) {
			mac = (manuf->ibguids[i + 1] >> 40) << 32;
			mac |= (manuf->ibguids[i + 1] & 0xffffff);
			manuf->ib_mac[i] = mac;

			logmsg(MSG_INFO, "hermon: fw_write: UPDATING mac%d to"
			    " %016llx\n", i + 1, mac);
		}
	}
	...
	manuf->state |=
	    ...
	    FWFLASH_ETH_STATE_MACS;
	if (cnx_set_guids(manuf, manuf->ibguids, manuf->ib_mac) !=
	    FWFLASH_SUCCESS) {
...
fw_identify(int start)
{
	...
	for (i = 0; drivername[i] != NULL; i++) {
		if (fw_identify_per_driver(&start_idx, drivername[i]) ==
		    FWFLASH_SUCCESS) {
			rv = FWFLASH_SUCCESS;
		}
	}
...
static int
fw_identify_per_driver(int *start, char *drvname)
{
	int		rv = FWFLASH_FAILURE;
	di_node_t	thisnode;
	...
	thisnode = di_drv_first_node(drvname, rootnode);
	...
	for (; thisnode != DI_NODE_NIL; thisnode = di_drv_next_node(thisnode)) {
		...
		rv = cnx_identify(newdev, drvname);
		...
		if ((newdev->drvname = calloc(1, strlen(drvname) + 1))
		    == NULL) {
			logmsg(MSG_ERROR, gettext("hermon: Unable to allocate"
		...
		(void) strlcpy(newdev->drvname, drvname,
		    strlen(drvname) + 1);
...
fw_devinfo(struct devicelist *thisdev)
{
	...
	fprintf(stdout, gettext("Driver %s\n"), thisdev->drvname);
	...
	if (encap->nguids == 6) {	/* has two MAC GUIDs */
		fprintf(stdout, gettext("\t\tMac 1\t   - %016llx\n"),
		    encap->ib_mac[0]);
		fprintf(stdout, gettext("\t\tMac 2\t   - %016llx\n"),
		    encap->ib_mac[1]);
	}
...
cnx_read_image(ib_cnx_encap_ident_t *handle)
{
	...
	logmsg(MSG_INFO, "%s: fw_size: 0x%x\n", handle->drvname, image_size);
...
cnx_write_file(ib_cnx_encap_ident_t *handle, const char *filename)
{
	...
	fp = fdopen(fd, "w");
	if (fp == NULL) {
		(void) fprintf(stderr, gettext("cnx_write_file: Unknown "
		    "filename %s : %s\n"), filename, strerror(errno));
		return (FWFLASH_FAILURE);
	}
...
static int
cnx_set_guids(ib_cnx_encap_ident_t *handle, void *arg1, void *arg2)
{
	uint32_t	addr;
	uint32_t	*guids, *macs;
	...
	if (handle->state & FWFLASH_ETH_STATE_MACS) {
		/* make sure we include macs to list of guids */
		verifier->fwimage[addr - 4] = htonl(6);

		verifier->fwimage[addr + 8] = macs[0];
		verifier->fwimage[addr + 9] = macs[1];

		verifier->fwimage[addr + 10] = macs[2];
		verifier->fwimage[addr + 11] = macs[3];
	}
#else
	if (handle->state & FWFLASH_IB_STATE_GUIDN) {
	...
	if (handle->state & FWFLASH_ETH_STATE_MACS) {
		...
		verifier->fwimage[addr + 8] = htonl(macs[1]);
		verifier->fwimage[addr + 9] = htonl(macs[0]);

		verifier->fwimage[addr + 10] = htonl(macs[3]);
		verifier->fwimage[addr + 11] = htonl(macs[2]);
	}
 ...
 */
static int
cnx_identify(struct devicelist *thisdev, char *drvname)
{
	int				fd, ret, i;
	...
	snprintf(manuf->drvname, sizeof (manuf->drvname), "%s", drvname);
	...
	if (init_ioctl.af_pn_len != 0) {
		...
		logmsg(MSG_INFO, "hermon: Failed to get Part# from %s "
		    "driver \n", drvname);
		manuf->pn_len = 0;
	}
...
cnx_read_guids(ib_cnx_encap_ident_t *handle, int type)
{
	...
	for (i = 0; i < CNX_GUID_CRC16_SIZE / 4; i++) {
		...
		if (i == 0) {
			handle->nguids = ioctl_info.af_quadlet;
			logmsg(MSG_INFO, "number of guids is %i\n",
			    handle->nguids);
		}

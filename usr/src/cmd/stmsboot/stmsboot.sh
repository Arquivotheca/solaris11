...
# Additional backslashes added to silence keywords check.
...
update_sysfiles()
{
	...
	if [ -n "$response" -a "$response" != "y" -a \
	    "$response" != "Y" ]; then
		for d in $DRVLIST; do
			TMPDRVCONF=$SYSVOL_FS/tmp.$d.conf.$$
	...
	if [ "$cmd" = "enable" -o "$cmd" = "disable" ]; then
		...
		for d in $DRVLIST; do
			DRVCONF=$d.conf
			KDRVCONF=/etc/driver/drv/$d.conf
			TMPDRVCONF=$SYSVOL_FS/tmp.$d.conf.$$
...
configure_mpxio()
{
	...
	KDRVCONF=/etc/driver/drv/$d.conf
	TMPDRVCONF=$SYSVOL_FS/tmp.$d.conf.$$
	TMPDRVCONF_MPXIO_ENTRY=$SYSVOL_FS/tmp.$d.conf.mpxioentry.$$;
	TMPDRVCONF_SATA_ENTRY=$SYSVOL_FS/tmp.$d.conf.sataentry.$$;

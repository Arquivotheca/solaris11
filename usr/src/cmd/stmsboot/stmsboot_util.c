...
main(int argc, char **argv)
{
	...
	errno = 0;
	devinfo_root = di_init("/", DINFOCPYALL|DINFOFORCE|DINFOPATH);
...
validate_devnvl()
{
	...
	curnode = di_drv_first_node("vdc", devinfo_root);
	if (curnode != DI_NODE_NIL)
		rv3 = mpxio_nvl_boilerplate(curnode);
...
popcheck_devnvl(di_node_t thisnode, nvlist_t *devnvl, char *strdevid)
{
	...
	char	path_addr[MAXPATHLEN];
	char	*path_end, *path_devfspath;
	uint32_t path_state;
	...
	if (scsivhciparent) {
		/* save path state in MPxIO-enabled mode */
		path_node = di_path_client_next_path(thisnode, path_node);

		if (DI_PATH_NIL == path_node)
			logmsg(MSG_INFO,
			    gettext("Unable to get path_info node for %s, "
			    "devid %s.\n"),
			    devfspath,
			    strdevid);

		while (path_node != DI_PATH_NIL) {
			path_devfspath = di_devfs_path(di_path_phci_node(
			    path_node));
			if (path_devfspath) {
				(void) nvlist_add_uint32(devnvl,
				    path_devfspath,
				    di_path_state(
				    path_node));
				di_devfs_path_free(path_devfspath);
			}

			path_node = di_path_client_next_path(thisnode,
			    path_node);
		}
	} else {
		/*
		 * In MPXIO-disabled mode, we check to see if this node is
		 * on an active path, (such information is saved in
		 * MPxIO-enabled mode). If not, we will not update the nvlist,
		 * since we have only one nvlist entry for each device of
		 * unique devid,  while in MPxIO-enabled mode we have two
		 * drives (or more) with the same devid.
		 */
		(void) strncpy(path_addr, devfspath, MAXPATHLEN);
		path_end = strrchr(path_addr, '/');
		if (path_end != NULL)
			*path_end = '\0';

		rv = nvlist_lookup_uint32(devnvl, path_addr, &path_state);
		if ((0 == rv) && (path_state != DI_PATH_STATE_ONLINE))
			return (0);
	}

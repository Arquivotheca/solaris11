...
)
{
	...
	if (sem_init(&iscsi_child_sem, 0, 0) == -1) {
		...
		syslog(LOG_DAEMON | LOG_ERR,
		    gettext("Could not create semaphore"));
		if (rval == EPERM) {
	...
	if (iscsi_child_door_handle == -1) {
		...
		syslog(LOG_DAEMON | LOG_ERR,
		    gettext("failed to create door: %s"), strerror(rval));
		exit(SMF_EXIT_ERR_FATAL);
	}
	...
	if (iscsi_kernel_door_handle == -1) {
		rval = errno;
		syslog(LOG_DAEMON | LOG_ERR, gettext("door_create failed: %s"),
		    strerror(rval));
		exit(SMF_EXIT_ERR_FATAL);
	}
	...
	if (iscsi_dev_handle == -1) {
		...
		syslog(LOG_DAEMON | LOG_ERR,
		    gettext("iSCSI device open failed: %s"),
		    strerror(rval));
		if (rval == EPERM) {
			exit(SMF_EXIT_ERR_PERM);
		}
	...
	if (ioctl(
		...
		syslog(LOG_DAEMON | LOG_ERR,
		    gettext("ioctl: failed to enable iSCSI initiator: %s"),
		    strerror(rval));
		exit(SMF_EXIT_ERR_FATAL);
	}
...
)
{
	...
	iscsi_child_smf_exit_code = SMF_EXIT_ERR_FATAL;

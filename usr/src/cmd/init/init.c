...
static char *INITPIPE_DIR = _PATH_SYSVOL;
static char *INITPIPE	  = _PATH_SYSVOL "/initpipe";

#define	INIT_STATE_DIR _PATH_SYSVOL
...
static char *SU	= "/usr/sbin/sulogin";	/* Super-user program for single user */
static char *SH	= "/usr/sbin/sh";	/* Standard shell */

/*
 * Default Path.  /sbin is included in path only during sysinit phase
 */
#define	DEF_PATH	"PATH=/usr/bin:/usr/sbin"
#define	INIT_PATH	"PATH=/usr/sbin:/usr/bin"
...
static int	reload_glob_envp = 0;		/* Whether to reload init env */
...
static void	audit_put_record(int, int, char *);
static void	update_boot_archive(void);
static void	notify_gfx_shutdown(void);
...
main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		/* Reload init environment if necessary */
		if (reload_glob_envp)
			init_env();
...
notify_gfx_shutdown(void)
{
	...
	fd = open(GFX_DEVICE, O_RDONLY);
	if (fd == -1) {
		(void) fprintf(stderr, "init: unable to open /dev/fb"
		    " to load the shutdown image\n");
		return;
	}
	(void) ioctl(fd, KDSHUTDOWN, KDS_START);
}

static void
update_boot_archive(void)
{
	(void) system("/usr/sbin/bootadm -ea update_all");
}
...
init_env()
{
	...
	if ((fp = fopen(ENVFILE, "r")) == NULL) {
		...
		while (fgets(line, MAXCMDL - 1, fp) != NULL &&
			 ...
			 * any non-quoted semicolons to blanks so they
	...
	/*
	 * Reset the reload init variable
	 */
	reload_glob_envp = 0;
 ...
 * it.  _PATH_SYSVOL is mounted by the kernel so it is always available.
 */
void
setup_pipe()
{
...
siglvl(int sig, siginfo_t *sip, ucontext_t *ucp)
{
	...
	/* Set the variable to reload init env on SIGHUP */
	if (sig == LVLQ)
		reload_glob_envp = 1;
...
userinit(int argc, char **argv)
{
	...
	if ((init_signal = lvlname_to_state((char)argv[1][0])) == -1) {
		(void) fprintf(stderr, usage_msg);
		audit_put_record(ADT_FAILURE, ADT_FAIL_VALUE_BAD_CMD, argv[1]);
		exit(1);
	}
	...
	if (init_signal == SINGLE_USER) {
		...
		if (ln == NULL) {
			...
			audit_put_record(ADT_FAILURE, ADT_FAIL_VALUE_BAD_TTY,
			    argv[1]);
			exit(1);
		}
	...
	/*
	 * If we are on the global zone and the system is going down (init
	 * levels 0,5,6), then check if the kernel wants to perform a splash
	 * shutdown and update the archives.
	 */
	if (getzoneid() == GLOBAL_ZONEID && (init_signal == LVL0
	    || init_signal == LVL5 || init_signal == LVL6)) {
		notify_gfx_shutdown();
		update_boot_archive();
	}

	audit_put_record(ADT_SUCCESS, ADT_SUCCESS, argv[1]);
	...
	if (kill(init_pid, init_signal) == FAILURE) {
		...
		audit_put_record(ADT_FAILURE, ADT_FAIL_VALUE_AUTH, argv[1]);
		exit(1);
	}
...
startd_run(const char *cline, int tmpl, ctid_t old_ctid)
{
	 ...
	 */
	do {
		ret = stat(_PATH_SYSVOL "/resetting", &sb);
	} while (ret == -1 && errno == EINTR);
...
/*
 * Generate AUE_init_solaris audit record.
 *

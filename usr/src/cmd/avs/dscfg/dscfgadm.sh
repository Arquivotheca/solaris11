...
#
# NOTE: using "function check_fs_local_grouping" rather than 
# "check_fs_local_grouping()" because ksh93 uses the POSIX standard
# for function_name() which does not use local variables. This caused $svc
# to still have a width of 20 as defined in display_info() and that 
# caused a failure when using ${svc}-local-fs
#
function check_fs_local_grouping
{
	$xopt
	typeset svc=$1

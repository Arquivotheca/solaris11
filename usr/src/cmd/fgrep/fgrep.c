...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	if (errflg || ((argc <= 0) && !fflag && !eflag)) {
		(void) fprintf(stderr, gettext("usage: fgrep [ -bchilnsvx ] "
 ...
 */
	...
	if (w == NULL) {
		(void) fprintf(stderr, gettext("fgrep: could not allocate "
		    "memory for wordlist\n"));
		exit(2);
	}
...
execute(char *file)
{
	...
	for (;;) {
		...
		if (ccount <= 0) {
			...
			if (p > &buf[fw_lBufsiz]) {
				if ((ccount = fread(p, sizeof (char),
				    &buf[fw_lBufsiz + BUFSIZ] - p, fptr)) <= 0)
					break;
...
cfail(void)
{
	...
	struct words *s;
	s = w;
	if ((queue = (struct words **)
	    calloc(qsize, sizeof (struct words *))) == NULL) {
		perror("fgrep");
		exit(2);
	}
	front = rear = queue;

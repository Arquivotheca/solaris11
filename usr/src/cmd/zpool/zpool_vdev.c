...
make_leaf_vdev(const char *arg, uint64_t is_log)
{
	 ...
	 * query interface.
	 */
	if (libzfs_get_cro_hdl(g_zfs) && (lstat64(arg, &statbuf) == 0) &&
		...
		ra = di_cro_reca_create_query((di_cro_hdl_t)
		    libzfs_get_cro_hdl(g_zfs), 0, query);
 ...
 */
static boolean_t
check_tvdevs(nvlist_t **child, uint_t children, replication_level_t *rep,
    boolean_t fatal)
{
	uint64_t vdev_size;
	boolean_t reported;
 ...
 */
static boolean_t
compare_rep_levels(replication_level_t *lastrep, replication_level_t *rep,
    boolean_t fatal)
{
 ...
 */
static boolean_t
check_new_levels(replication_level_t *current, replication_level_t *new)
{
	if (current->zprl_type == NULL || new->zprl_type == NULL)
		return (B_TRUE);

	if (strcmp(current->zprl_type, new->zprl_type) != 0) {
		vdev_error(gettext("mismatched replication level: "
		    "pool uses %s and new vdev is %s\n"),
		    current->zprl_type, new->zprl_type);
	} else if (current->zprl_parity != new->zprl_parity) {
		vdev_error(gettext("mismatched replication level: "
		    "pool uses %llu device parity and new vdev uses %llu\n"),
		    current->zprl_parity, new->zprl_parity);
	} else if (current->zprl_children != new->zprl_children) {
		vdev_error(gettext("mismatched replication level: "
		    "pool uses %llu-way %s and new vdev uses %llu-way %s\n"),
 ...
 */
static boolean_t
valid_replication(nvlist_t *nvroot, replication_level_t *pool,
    replication_level_t *slog, boolean_t fatal)
{
	nvlist_t **top;
	uint_t t, toplevels;
	nvlist_t **child;
	...
	for (t = 0; t < toplevels; t++) {
		...
		if (nvlist_lookup_nvlist_array(nv, ZPOOL_CONFIG_CHILDREN,
			...
			if (!check_tvdevs(child, children, &rep, fatal)) {
				if (!fatal)
					return (B_FALSE);
				ret = B_FALSE;
			}
		...
		if (!compare_rep_levels(prev, &rep, fatal)) {
			if (!fatal)
				return (B_FALSE);
			ret = B_FALSE;
		}
...
check_replication(nvlist_t *config, nvlist_t *newroot)
{
	...
	if (config != NULL) {
		...
		verify(nvlist_lookup_nvlist(config, ZPOOL_CONFIG_VDEV_TREE,
		    &nvroot) == 0);
		if (!valid_replication(nvroot, &p_curr, &s_curr, B_FALSE))
			return (0);
	}
	 ...
	 */
	if (!valid_replication(newroot, &p_new, &s_new, B_TRUE))
		return (-1);
	 ...
	 * the current pool.
	 */
	if (!check_new_levels(&p_curr, &p_new) ||
	    !check_new_levels(&s_curr, &s_new))
		return (-1);

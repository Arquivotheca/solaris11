...
main(int argc, char **argv)
{
	 ...
	 */
	while ((c = getopt(argc, argv, "afi")) != EOF) {
		switch (c) {
			case 'a':
				++allflag;
				break;
	...
	while (argc--) {
		 ...
		 */
		if (!jobexists && !fflag) {
			(void) fprintf(stderr, "atrm: %s: no such job number\n",
			    *argv);
		}
		++argv;
	}
...
{
	(void) fprintf(
	    stderr, "usage: atrm [-f] [-i] [-a] [[job #] [user] ...]\n");
	exit(1);
}
...
removentry(char *filename, struct stat *statptr, uid_t user)
{
	...
	if (user != statptr->st_uid && !cron_admin(login_authchk)) {
		...
		if (iflag) {
			if (cron_admin(login_authchk)) {
				(void) printf("\t(owned by ");
				powner(filename);
				(void) printf(") ");
			}
			(void) printf(gettext("remove it? "));
			if (yes() == 0)
				return (0);
		}
...
powner(char *file)
{
	...
	(void) printf("%s", getname(statb.st_uid));
}
...
{
	(void) fprintf(stderr, "atrm: %s %s: %s\n", msg, name, errmsg(errno));
}

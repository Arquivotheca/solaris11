...
main(int argc, char **argv)
{
	...
	if (argc > 0) {
		...
		if (badarg)
			if (argnum)
				(void) printf("Printing queue information only "
				    "for %s:\n", pr.pw_name);
			else {
				(void) printf(
				    "atq: Non-priviledged user cannot "
...
printqueue(uid_t *uidlist, int nuids)
{
	 ...
	 */
	(void) printf(" Rank	  Execution Date     Owner      Job            "
	    "Queue   Job Name\n");
	...
	for (i = 0; i < numentries; i++) {
		...
		(void) printf("%-10s ", getname(stbuf.st_uid));
		(void) printf("%-14s ", queue[i]->d_name);
		(void) printf("  %c", curqueue);
...
printrank(int n)
{
	...
	if ((n/10) == 1)
		(void) printf("%3d%-5s", n, "th");
	else
		(void) printf("%3d%-5s", n, r[n%10]);
}
...
printdate(char *filename)
{
	 ...
	 */
	(void) sprintf(date, "%3s %2d, %4d %02d:%02d",
...
usage(void)
{
	(void) fprintf(stderr, "usage:	atq [-c] [-n] [name ...]\n");
	exit(1);
}
...
aterror(char *msg)
{
	(void) fprintf(stderr, "atq: %s\n", msg);
}

static void
atperror(char *msg)
{
	(void) fprintf(stderr, "atq: %s: %s\n", msg, errmsg(errno));
}

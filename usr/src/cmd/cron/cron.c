...
#define	PIDERR		"unexpected pid returned %ld (ignored)"
...
begin:
	...
	cron_pid = getpid();
	msg("*** cron started ***   pid = %ld", cron_pid);
...
mail(char *usrname, char *mesg, int format)
{
	...
	if (fork_val == 0) {
		...
		(void) xasprintf(&temp, "%s %s", MAIL, usrname);
		pipe = popen(temp, "w");
		if (pipe != NULL) {
...
ex(struct event *e)
{
	...
	if (e->etype == CRONEVENT) {
		...
		(void) xasprintf(&at_cmdfile, "%s/%s", ATDIR, e->cmd);
	...
	environ = envinit;
	if (chdir(home) == -1) {
		(void) snprintf(bufs.error, sizeof (bufs.error),
		    CANTCDHOME, home);
	...
	(void) snprintf(bufs.error, sizeof (bufs.error), CANTEXECSH, sh);
	mail((e->u)->name, bufs.error,
...
mail_result(struct usr *p, struct runinfo *pr, size_t filesize)
{
	...
	(void) xasprintf(&cmd, "%s %s", MAIL, p->name);

...
check_data(tstat_data_t *data, tstat_data_t *odata)
{
	 ...
	 * correctly.
	 */
	for (i = 0, j = 0; i < g_max_cpus; i++) {
		if (data->tdata_cpuid == -1) {
			/*
			 * There are fewer CPUs in sight now than had
			 * been originally instrumented.
			 */
			ndata->tdata_cpuid = -1;
			return;
		}
		...
		/*
		 * Advance odata, if necessary, to match the current
		 * CPU in the new data. ndata continues to point at
		 * where the CPU should occur in the odata. j counts
		 * slots inspected by odata.
		 */
		while (data->tdata_cpuid != odata->tdata_cpuid) {
			if (++j >= g_max_cpus) {
				/* fell off the end, should never happen */
				fatal("received inconsistent data from driver");
			}
			odata = (tstat_data_t *)((uintptr_t)odata + g_datasize);
		}

		/* odata now leads ndata, copy the slot downward */
		if (ndata != odata)
			bcopy(odata, ndata, g_datasize);

		/* done with this slot, advance everthing to next slot */
		data = (tstat_data_t *)((uintptr_t)data + g_datasize);
		ndata = (tstat_data_t *)((uintptr_t)ndata + g_datasize);
		odata = (tstat_data_t *)((uintptr_t)odata + g_datasize);
		j++;
	}
}
...
main(int argc, char **argv)
{
	...
	if (!tflag)
		g_datasize = sizeof (tstat_data_t);

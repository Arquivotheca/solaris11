...
int notin(int);
int nxtst(int, int);
int range(unsigned char *, unsigned char *, int);
void overflo(void);
...
synerror(void)
{
	(void) fprintf(stderr, gettext("egrep: syntax error\n"));
	exit(2);
}
...
main(int argc, char **argv)
{
	...
	if (errflag || ((argc <= 0) && !fflag && !eflag)) {
		(void) fprintf(stderr, gettext("usage: egrep %s\n"),
		    gettext(USAGE));
		exit(2);
	}
	if(!eflag && !fflag) {
		...
		*ptrend = '\n';
		...
		if(succ) {
			...
			if (cflag) tln++;
				...
				if (nfile > 1 && !hflag) 
					(void) printf(gettext("%s:"), file);
				if (bflag) {
					nchars = blkno - (buf + count - ptrend) - 2;
					if(nlflag)
						nchars++;
					(void) printf("%lld:",
					   (long long) nchars/BLKSIZE);
				}
				if (nflag) 
				...
				if(nlflag)
					...
					nchars = ptrend - ptr;
				(void) fwrite(ptr, (size_t)1,
				    (size_t)nchars, stdout);
			}
		}
		if(!nlflag)
			break;
...
ccl(int type) 
{
	...
	while(current <= last) {
		if(upper[current] == lower[current]) 
			...
			(void) wctomb(multic2, upper[current]);
...
maxmin(wchar_t c, int flag)
{
	...
	if(!minmax1[0]) {
		...
		for(i = 0; i <= 1; i++) {
			...
			(void) mbtowc(&minmax1[i], multic, MB_LEN_MAX);
			if(eucw2) {
				multic[0] = SS2;
				(void) mbtowc(&minmax2[i], multic, MB_LEN_MAX);
			}
			if(eucw3) {
				multic[0] = SS3;
				(void) mbtowc(&minmax3[i], multic, MB_LEN_MAX);
			}
		}
	}
	switch(c) {

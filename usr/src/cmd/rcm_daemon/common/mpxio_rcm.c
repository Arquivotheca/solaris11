...
#define	MPXIO_MSG_USAGE		gettext("IO Multipathing PHCI (%s)")
#define	MPXIO_MSG_USAGEUNKNOWN	gettext("IO Multipathing PHCI (<unknown>)")
...
build_groups(di_node_t dinode, void *arg)
{
	...
	while ((dipath =
		...
		if (phcinode == DI_NODE_NIL) {
			free_phcis(i, phcis);	/* free preceding PHCIs */
			rcm_log_message(RCM_ERROR,

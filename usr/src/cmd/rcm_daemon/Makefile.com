...
COMMON_SHELL_SCRIPT_SRC = SUNW,rdsv3u.sh
...
LDLIBS_MODULES_PRE = -L$(LROOT)/lib -L$(LROOT)/usr/lib
LDLIBS_MODULES =
...
SUNW_ip_rcm.so :=	LDLIBS_MODULES += -linetutil -ldladm \
				-lipmp -lipadm -lnvpair -lsocket -lnsl
...
SUNW_ttymux_rcm.so :=	LDLIBS_MODULES += -ldevinfo
SUNW_sdp_rcm.so :=	LDLIBS_MODULES += -lnvpair
LDLIBS_MODULES_POST = -lrcm -lc
...
SUNW_%.so: %.o
	$(LINK.c) -o $@ $(GSHARED) -h $@ $< $(LDLIBS_MODULES_PRE) \
	    $(LDLIBS_MODULES) $(LDLIBS_MODULES_POST)
	$(POST_PROCESS_SO)

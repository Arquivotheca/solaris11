...
/*
 * separator between multiple items - initialized to empty
 * string, reset to space after the first is printed.
 */
	const char *sep = "";
	const char *space = " ";
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	if (!(sflg || nflg || rflg || vflg || mflg || pflg || iflg || Xflg))
		sflg++;
	if (sflg) {
		(void) fprintf(stdout, "%s%s", sep, un->sysname);
		sep = space;
	}
	if (nflg) {
		(void) fprintf(stdout, "%s%s", sep, un->nodename);
		sep = space;
	}
	if (rflg) {
		(void) fprintf(stdout, "%s%s", sep, un->release);
		sep = space;
	}
	if (vflg) {
		(void) fprintf(stdout, "%s%s", sep, un->version);
		sep = space;
	}
	if (mflg) {
		(void) fprintf(stdout, "%s%s", sep, un->machine);
		sep = space;
	}
	if (pflg) {
		...
		(void) fprintf(stdout, "%s%s", sep, procbuf);
		sep = space;
	}
	if (iflg) {

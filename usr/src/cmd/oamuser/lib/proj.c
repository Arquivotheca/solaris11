...
int
files_edit_projects(char *login, char *new_login, projid_t *projids,
    int overwrite)
{
	char **memptr;
	char t_name[] = "/etc/ptmp.XXXXXX";
	FILE *e_fptr, *t_fptr;
	...
	while (fgets(p_string, NSS_LINELEN_PROJECT - 1, e_fptr)) {
		...
		if (p_ptr == NULL) {
			 ...
			 */
			const char *err_msg = gettext(
			    "WARNING: Project entry exceeds %d "
			    "char: /etc/project entry truncated.\n");
			(void) fprintf(stderr, err_msg, NSS_LINELEN_PROJECT);

 ...
 *		defprof = 0
 *		defrole = 0
 * 		defroleauth = 0	for users or role for roles.
 *
...
getusrdef(char *usertype)
{
	...
	if (is_role(usertype)) {
		if ((defptr = fopen(DEFROLEFILE, "r")) == NULL) {
			...
			defaults.defroleauth = DEFROLEAUTHROLE;
			return (&defaults);
		}
	} else {
...
putusrdef(struct userdefs *defs, char *usertype)
{
	...
	if (is_role(usertype)) {
		...
		skip = offsetof(struct userdefs, defroleauth);
		hdr = FHEADER;
	}
...
import_def(struct userdefs *ud)
{
	...
	for (i = 0; i < NDEF; i++) {
		if (tab[i].uakey != NULL && tab[i].type == STR) {
			...
			if (val[0] != '\0' &&
			    val == getsetdefval(tab[i].uakey, val))
				nkeys ++;
		}
	}
}

...
extern	char	*optarg;
extern	int	get_cpr_config_path(char **);
...
pm_check_suspend(void) {
	 ...
	 */
	ret = uadmin(A_FREEZE, AD_CHECK_SUSPEND, 0);
	if (ret == -1) {
		/*
		 * Some platforms support suspend via AD_CHECK_SUSPEND_TO_RAM
		 * and not via AD_CHECK_SUSPEND.  So if the check above
		 * failed, try AD_CHECK_SUSPEND_TO_RAM.
		 */
		ret = uadmin(A_FREEZE, AD_CHECK_SUSPEND_TO_RAM, 0);
	}
	return (ret);
}
...
is_mou3()
{
	...
	if (cpr_conf == NULL && get_cpr_config_path(&cpr_conf) != 0) {
		(void) printf(gettext("Unable to identify CPR config file"));
		return (-1);
	}

	if ((fd = open(cpr_conf, O_RDONLY)) < 0) {
		(void) printf(gettext("Can't open %s file."), cpr_conf);
		return (found);
	}

	if (read(fd, (void *) &cf, sizeof (cf)) != sizeof (cf)) {
		(void) printf(gettext("Can't read %s file."), cpr_conf);
	} else {

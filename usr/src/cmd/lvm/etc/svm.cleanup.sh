...
IN_CONF=${FLASH_ROOT}/etc/driver/drv/md.conf
...
if [ "${FLASH_TYPE}" = "FULL" ]; then
	...
	if [ -h /etc/driver/drv/md.conf ]; then
		# Remove SVM mddb entries from /etc/driver/drv/md.conf.
		nawk '
		BEGIN {delmddb=0}
		...
		cp /etc/driver/drv/md.conf ${IN_CONF}
	...
	# configured.
	sed -e 's/#.*$//' /etc/driver/drv/md.conf | \
	...
	cp ${SAVE_DIR}/md.conf ${FLASH_ROOT}/etc/driver/drv/md.conf

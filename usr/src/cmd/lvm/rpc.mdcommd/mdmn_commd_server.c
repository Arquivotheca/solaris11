...
setup_debug(void)
{
	...
	if (commdoutfile == (char *)NULL) {
		...
		commdoutfile = _PATH_SYSVOL "/commd.out";
		...
		if ((access(dirname(tmp_dir), X_OK|W_OK)) ||
			...
			syslog(LOG_ERR,
			    ...
			    "using " _PATH_SYSVOL "/commd.out instead\n",
			    commdoutfile);
			free(commdoutfile);
			commdoutfile = _PATH_SYSVOL "/commd.out";
			commdout = fopen(commdoutfile, "a");
...
mdmn_comm_reinit_set_svc_2(set_t *setnop, struct svc_req *rqstp)
{
	...
	/*
	 * Remove the setno from the application setlistp cache (held in
	 * libmeta) so that we do not have stale data polluting our view of
	 * the SVM configuration.
	 */
	(void) rw_wrlock(&set_desc_rwlock[setno]);
	metaflushsetno(setno);
	if (set_descriptor[setno] != NULL) {
		if (setno != set_descriptor[setno]->sd_setno) {
			/*
			 * Aargh! Shouldn't happen, but we've got a
			 * disconnected view of setno vs setdescriptor
			 * set-number. So, free the setdesc one too as a last
			 * ditch effort to clean up...
			 */
			commd_debug(MD_MMV_MISC,
			    "reinit: setno %d mismatched with"
			    "  setdesc->sd_setno %d\n",
			    setno, set_descriptor[setno]->sd_setno);

			metaflushsetno(set_descriptor[setno]->sd_setno);
		}
	}

	/*
	 * Seeing as we've flushed the cache, we need to remove our old
	 * possibly stale set-descriptor reference too.
	 */
	set_descriptor[setno] = NULL;
	(void) rw_unlock(&set_desc_rwlock[setno]);

	commd_debug(MD_MMV_MISC, "reinit: flushed set %d\n", setno);

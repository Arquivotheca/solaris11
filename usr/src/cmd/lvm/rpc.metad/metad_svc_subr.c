...
)
{
	...
	for (p = dd; p != NULL; p = p->dd_next) {
		...
		dr->dr_revision = MD_DRIVE_RECORD_REVISION;
		/*
		 * this flag tells the code that the drive record
		 * is in the process of being added. Once the add
		 * is complete this flag should be removed, otherwise
		 * during the sr_validate it will be removed as the
		 * code assumes that the add of the drive has not
		 * been completed.
		 */

...
report_overlap_recommendation()
{
	...
	for (d = overlap_disks; d != NULL; d = d->overlap) {
		...
		/*
		 * Handle the case where the diskset being scanned does not
		 * have a friendly-name aware replica. This means the set
		 * creation timestamp is not valid. In this case we use the
		 * master-block timestamp instead.
		 */
		if (mbp->mb_setcreatetime.tv_sec == 0 &&
		    mbp->mb_setcreatetime.tv_usec == 0) {
			timestampp = &(mbp->mb_timestamp);
			repstr = gettext("updated at ");
		} else {
			timestampp = &(mbp->mb_setcreatetime);
			repstr = gettext("created at ");
		}
		(void) fprintf(stdout, "%s%s: %s\n",
		    gettext(" - must import with set "), repstr,
		    meta_print_time((md_timeval32_t *)timestampp));
	}
	Free(mbp);
}
...
mddrivenamelist_t	**skiph)
{
	 ...
	 */
	if (MD_IS_DISK_AVAILABLE(d->mid_available)) {
		/*
 ...
 * set_mn_nodeid:
 *
 * Obtain this node's ID from the nodelist file and associate it with the
 * local set. This covers a corner-case where we are attempting to import a
 * multi-owner set with no existing MN sets in the configuration.
 */
static void
set_mn_nodeid()
{
	mndiskset_membershiplist_t	*nlp, *p;
	...
	/*
	 * Handle the non-SC single-node case first. In this case we assign
	 * ourselves to be node-ID 1 as we're the only node in
	 * the configuration.
	 */
	if (meta_mn_singlenode()) {
		snp.sn_nodeid = 1;

		if (metaioctl(MD_MN_SET_NODEID, &snp, &snp.sn_mde,
		    "MD_MN_SET_NODEID") != 0) {
			(void) mdstealerror(ep, &snp.sn_mde);

			mde_perror(ep,
			    gettext("Could not set machine node-ID"));
		}
		return;
	}

	/*
	 * Multi-node (SC) configuration. We need to read the configured
	 * cluster configuration from nodelist and obtain our entry.
	 * Then we stamp the in-kernel node-ID by using the local-set for
	 * the target of the ioctl.
	 */
	if (meta_read_nodelist(&nodecnt, &nlp, ep)) {
		mde_perror(ep, gettext("Could not read nodelist"));
		return;
	}

	for (i = 0, p = nlp; i < nodecnt; i++, p = p->next) {
		if (strcmp(me, p->msl_node_name) == 0) {
			/*
			 * Found our node-name in the list, send a NODEID
			 * request and exit
			 */
			snp.sn_nodeid = p->msl_node_id;
			...
			if (metaioctl(MD_MN_SET_NODEID, &snp, &snp.sn_mde,
				...
				mde_perror(ep,
				    gettext("MD_MN_SET_NODEID failed"));
			}
			return;
		}
	}
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	/*
	 * If we are on a SunCluster system and the did driver supports
	 * devids, then we proceed.
	 */
	if (sdssc_bind_library() != SDSSC_NOT_BOUND) {
		if (meta_devid_supported_in_did() != MD_DEVID_SUPPORTED) {
			(void) printf(gettext(
			    "%s: Import operation only supported SunCluster "
			    "3.2 and newer.\n"),
			    argv[0]);
			exit(0);
		}
	...
	if (argc > optind) {
		...
		for (i = 0; i < cnames.min_count; i++, optind++) {
			...
			if (dnp == NULL) {
				...
				cnames.min_names[i] = Strdup(dnp->rname);
			}
		}
	} else {
	...
	/*
	 * Convert from ctd names to did names if in a SC environment.
	 */
	...
	while (cnames.min_count--) {
		Free(cnames.min_names[cnames.min_count]);
	}
	Free(cnames.min_names);
	...
	if (mdisdserror(ep, MDE_DS_DRIVEINUSE)) {
		...
		for (dp = dnlp->next; dp != NULL; dp = dp->next) {
			sizecnt += 2; /* for the ", " */
			name = dp->drivenamep->cname[0] == 'c' ?
			    dp->drivenamep->cname : dp->drivenamep->rname;
			sizecnt += strlen(name);
		}
		...
		for (dp = dnlp->next; dp != NULL; dp = dp->next) {
			...
			name = dp->drivenamep->cname[0] == 'c' ?
			    dp->drivenamep->cname : dp->drivenamep->rname;
			(void) strlcat(dlist, name, sizecnt);
		}
	...
	if (report_only) {
		...
		for (d = pass1_misp->mis_drives;
			...
			if (d->overlapped_disk)
				/*
				 * Again, choose the cname or rname according
				 * to whether or not this is a ctd device.
				 */
				(void) printf("%s ", d->mid_dnp->cname[0] ==
				    'c' ?  d->mid_dnp->cname :
				    d->mid_dnp->rname);
		}
	...
	if ((sdssc_version(&vers) == SDSSC_OKAY) && (vers.major == 3)) {
		if (misp->mis_flags & MD_IM_SET_MULTINODE) {
			/*
			 * We have to ensure that our node-ID is set within
			 * the kernel. This will normally be done by metaclust
			 * but only if there are any multi-owner sets known to
			 * this system.
			 * We need to explicitly associate the local set with
			 * the value in the nodelist file for ourselves.
			 */
			set_mn_nodeid();

			(void) pclose(popen("/usr/lib/lvm/mddoors", "w"));
			if (meta_set_join(sp, ep) != 0) {
				md_eprintf("%s %s\n", gettext("Failed to "
				    "join set "), sp->setname);
				meta_free_im_set_desc(misp);
				md_exit(sp, 1);
			}
			if (mdmn_update_changelog(sp->setno, ep) != 0) {
				md_eprintf("%s\n", gettext("Failed to "
				    "convert changelog records."));
				meta_free_im_set_desc(misp);
				md_exit(sp, 1);
			}
			(void) pathname_reload(&sp, sp->setno, ep);
			(void) meta_mirror_resync_all(sp, 0, ep);
		} else {
			/*
			 * The following take and release serves the
			 * purpose of creating the minor node links
			 * for the metadevices in this set.  Failure to
			 * do so before calling into libsdssc causes
			 * a SunCluster deadlock in the kernel.
			 */
			(void) meta_set_take(sp, &mhiargs,
			    (misp->mis_partial | TAKE_IMP), 0, &status);
			(void) meta_set_release(sp, &status);

			rval = sdssc_notify_service(setname_new, Make_Primary);
			if (rval == SDSSC_ERROR) {
				md_eprintf("%s\n", gettext("Sun Cluster "
				    "failed to make set primary, releasing "
				    "the set."));

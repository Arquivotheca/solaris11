 ...
 * prune_sp_names:
 *
 * Purpose:	Walk through the list of softparts and discard any which are
 *		not built on top of an underlying mirror metadevice.
 *
 * Returns:
 *		list of soft-part on mirror metadevices
 *		Caller must call metafreenamelist() on returned pointer if
 *		not NULL
 */
static mdnamelist_t *
prune_sp_names(
	...
	md_error_t	*ep)
{
	mdnamelist_t	*devnlp = NULL, *p;
	int		count = 0;
	int		ret = 0;

	for (p = mirnlp; p != NULL; p = p->next) {
		mdname_t	*mirnp = p->namep;
		char		*miscname;

		/* Skip any soft-part reported metas in the mirror list */
		...
		/*
		 * Use meta_sp_get_by_component() to produce
		 * a list of soft-parts which are resident on the given
		 * mirror component.
		 */
		if ((ret = meta_sp_get_by_component(sp, mirnp, &devnlp,
		    1, ep)) != -1) {
			count += ret;
		}
	}

	meta_mc_log(MC_LOG5, gettext("Number of soft-parts on mirrors"
	    " for %s: %d"), sp->setname, count);
	return (devnlp);
}
...
reset_state(uint_t mode, mdsetname_t *sp, char *drivername, md_error_t *ep)
{
	...
	if (mirror_dev) {
		if (meta_get_mirror_names(sp, &devnlp, 0, ep) < 0) {
			mde_perror(ep,
			    gettext("Could not get mirrors for set %s"),
			    sp->setname);
			return (-1);
		}
	} else {
		/*
		 * Get soft-parts built on mirrors in the set.
		 */
		if (meta_get_mirror_names(sp, &mirnlp, 0, ep) < 0) {
			mde_perror(ep, gettext(
			    "Could not get mirrors for set %s"),
			    sp->setname);
			return (-1);
		}

		/*
		 * Prune the metadevice namespace to only those soft-parts
		 * which are built on top of mirror devices.
		 * If we end up with a NULL namelist we can simply return as
		 * there is nothing to process.
		 */
		devnlp = prune_sp_names(sp, mirnlp, ep);
		metafreenamelist(mirnlp);
		if (devnlp == NULL) {
			meta_mc_log(MC_LOG5, gettext("No soft-parts on mirrors"
			    " for %s: %s"), sp->setname,
			    meta_print_hrtime(gethrtime() - start_time));
			return (0);
		}
	...
	for (p = devnlp; (p != NULL); p = p->next) {
		...
		if (mode & RESET_ABR) {
			...
			meta_mc_log(MC_LOG5, gettext("Checking %s/d%u drvr %s"),
 ...
 * how_many_mirrors:
 *
 * Obtains the number of mirror metadevices configured in the specified set.
 */
static int
how_many_mirrors(mdsetname_t *sp, md_error_t *ep)
{
	int		retval = 0;
	mdnamelist_t	*p, *mirnlp = NULL;
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	case MC_STEP4:
		 ...
		 * had the metadevice open and had ABR set has panicked, the
		...
		for (setno = 1; setno < max_sets; setno++) {
			...
			/*
			 * Do *not* bother with any further step4
			 * processing if we have no mirrors configured in this
			 * set. Short-circuits unnecessary soft-part accesses
			 */

			mirrors_present = how_many_mirrors(sp, ep);

			meta_mc_log(MC_LOG5, gettext("Step4 - got %d mirrors"
			    " for set %s"), mirrors_present, sp->setname);
			...
			if (!(sd->sd_mn_am_i_master) && start_step) {
				if (mirrors_present > 0) {
					if (reset_state(UPDATE_ABR, sp,
					    MD_MIRROR, ep) == -1) {
						md_exit(local_sp, 1);
					}
					...
					if (reset_state(GET_MIRROR_STATE, sp,
					    MD_MIRROR, ep) == -1) {
						md_exit(local_sp, 1);
					}
				}

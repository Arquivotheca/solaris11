	...
	if (md_upgd_stat == 0) {
		...
		if (ret == METADEVADM_ERR) {
			...
			if (options & DEV_UPDATE) {
				/*
				 * Report if md_devid_destroy and
				 * md_keep_repl_state are set.
				 */
				if (mdissyserror(ep,  MDDB_E_NODEVID)) {
					md_eprintf("%s\n", gettext(
					    "Cannot update devid while " \
					    "md_devid_destroy and " \
					    "md_keep_repl_state are set."));
				} else {
					mde_perror(ep, "Cannot update devid.");
				}
				closelog();
				(void) meta_unlock(sp, ep);
				md_exit(local_sp, 1);

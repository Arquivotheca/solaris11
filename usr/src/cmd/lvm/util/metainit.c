...
take_set(md_set_record *sr)
{
	...
	/*
	 * We know we are the only host in the set, but in some cases
	 * we are not side 0, so check which side we are.
	 */
	...
	for (dr = sr->sr_drivechain; dr; dr = dr->dr_next) {
		...
		for (rl = rlp; rl != NULL; rl = rl->rl_next) {
			if (strcmp(rl->rl_repp->r_namep->drivenamep->cname,
			    dnp->cname) == 0)
				dbcnt++;
		}
...
auto_take_sets()
{
	...
	/* At boot time the nodename may not be set yet. */
	hostname = mynode();
	...
	for (i = 1; i < max_sets; i++) {
		...
		if (sr->sr_flags & MD_SR_AUTO_TAKE &&
			...
			for (j = 0; j < MD_MAXSIDES; j++) {
				...
				cnt++;
				/* We may not know our nodename yet. */
				if (!hostname_empty && (strcmp(sr->sr_nodes[j],
				    hostname) != 0))
					host_mismatch = 1;
			}
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	if (todo == INIT) {		/* load and take auto-take sets */
		...
		if (pid == (pid_t)-1) {
			...
			/* child */
			if (meta_smf_isdisabled(
			    "network/rpc/metamed:default") == 1) {
				/*
				 * If the service is disabled it is not an
				 * error, so just exit with 0.
				 */
				md_exit(sp, 0);
			}

    ...
    def getAttr(self, at, be, ddh, beobj):
        ...
        if at == 'active':
            if at not in be:
                return '-'
            ret = ''
            # Check to see if the BE is active but unbootable because
            # it's associated with a non-active global zone BE
            if 'active_unbootable' in be and be['active_unbootable']:
                ret += '!'
...
class DatasetList(listBootEnvironment):
    ...
    def __init__(self, ddh):
        ...
        self.hdr = \
            ('BE/Dataset', 'Active', 'Mountpoint', 'Space', 'Policy', \
             'Created'), \
            ('----------', '------', '----------', '-----', '------', \
             '-------')
        if ddh:

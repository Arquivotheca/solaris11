...
class Msgs:
    ...
    mBeadmErr[BEADM_ERR_BE_EXISTS] = _("BE %s already exists. Please choose " \
    "a different BE name.")
    mBeadmErr[BEADM_ERR_BE_DOES_NOT_EXIST] = _("%s does not exist or appear " \
    "to be a valid BE.\nPlease check that the name of the BE provided is " \
    "correct.")
    mBeadmErr[BEADM_ERR_NO_BES_EXIST] = _("No boot environments found on " \
    "this system.")
    mBeadmErr[BEADM_ERR_CREATE] = _("Unable to create %(0)s.\n%(1)s")
    mBeadmErr[BEADM_ERR_DESTROY] = _("Unable to destroy %(0)s.\n%(1)s")
    mBeadmErr[BEADM_ERR_DESTROY_ACTIVE] = _("%(0)s is the currently active " \
    "BE and cannot be destroyed.\nYou must boot from another BE in order to " \
    "destroy %(1)s.")
    mBeadmErr[BEADM_ERR_MSG_SUB] = _("Fatal error. No message associated " \
    "with index %d")
    mBeadmErr[BEADM_ERR_ILL_SUBCOMMAND] = _("Illegal subcommand %s")
    mBeadmErr[BEADM_ERR_INVALID_RESPONSE] = _("Invalid response. Please " \
    "enter 'y' or 'n'.")
    ...
    "'beadm destroy -fF %(2)s'.")
    mBeadmErr[BEADM_ERR_MOUNTPOINT] = _("Invalid mount point %s. Mount " \
    "point must start with a /.")
    mBeadmErr[BEADM_ERR_MUTUALLY_EXCL] = _("Invalid options: %s are " \
    "mutually exclusive.")
    mBeadmErr[BEADM_ERR_NO_MSG] = _("Unable to find message for error code: " \
    "%d")
    ...
    mBeadmErr[BEADM_ERR_SHARED_FS] = _("%s is a shared file system and it " \
    "cannot be unmounted.")
    mBeadmErr[BEADM_ERR_SNAP_DOES_NOT_EXISTS] = _("%s does not exist or " \
    "appear to be a valid snapshot.\nPlease check that the name of the  " \
    "snapshot provided is correct.")
    ...
    mBeadmErr[BE_ERR_ZONES_UNMOUNT] = _("Unable to destroy one of %(0)s's " \
    "zone BE's.\nUse 'beadm destroy -fF %(1)s' or 'zfs -f destroy <dataset>'.")
    mBeadmErr[BEADM_ERR_PERMISSIONS] = _("You have insufficient privileges " \
    "to execute this command.\nEither use 'pfexec' to execute the command " \
    "or become superuser.")
    mBeadmErr[BEADM_ERR_ROFS] = _("Operation not supported in a read-only " \
    "environment.")
    ...
    mBeadmOut[BEADM_MSG_ACTIVE_ON_BOOT] = _("The BE that was just destroyed " \
    "was the 'active on boot' BE.\n%s is now the 'active on boot' BE. Use " \
    "'beadm activate' to change it.\n")
    mBeadmOut[BEADM_MSG_DESTROY] = _("Are you sure you want to destroy %s? " \

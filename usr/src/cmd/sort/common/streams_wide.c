...
stream_wide_prime(stream_t *str)
{
	...
	trip_eof(BF->s_fp);
	if (!feof(BF->s_fp)) {
		if (xfgetws(current_position, end_of_buffer
		    - current_position, BF->s_fp) == NULL) {
			/* We know this is error because stream wasn't EOF */
...
stream_wide_fetch(stream_t *str)
{
	...
	if (xfgetws(graft_pt, dist_to_buf_end, str->s_type.BF.s_fp) == NULL) {
		if (ferror(str->s_type.BF.s_fp) || !feof(str->s_type.BF.s_fp))
			die(EMSG_READ, str->s_filename);
		else
			stream_set(str, STREAM_EOS_REACHED);
	}
...
stream_wide_fetch_overwrite(stream_t *str)
{
	...
	if (xfgetws(str->s_current.l_data.wp, dist_to_buf_end,

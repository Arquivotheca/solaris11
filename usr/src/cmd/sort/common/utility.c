...
/*
 * xfgetws() behaves similaly as fgetws(3c) except that xfgetws()
 * returns NULL when it found illegal multi-byte sequence at the middle
 * of the input line. fgetws(3c) in contrast returns non-NULL pointer with
 * buffer filled with characters which have read upto the illegal sequence.
 */
wchar_t *
xfgetws(wchar_t *buf, size_t size, FILE *fp)
{
	clearerr(fp);
	...
	if (ferror(fp)) {
		/*
		 * If no NL is found and error indicator is set, there
		 * must have been an EILSEQ error at the middle of the
		 * input line.
		 */

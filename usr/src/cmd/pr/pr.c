...
{
	const char *empty_file_msg = gettext("%s -- empty file");
	int	c;
	UNS	len;
	...
	if (f->f_f != NULL) {
		...
		len = (UNS) strlen(f->f_name) + 1 +
		    (UNS) strlen(empty_file_msg);
		s = (char *)getspace(len);
		(void) snprintf(s, len, empty_file_msg, f->f_name);
		(void) fclose(f->f_f);
	}
	Error = 1;
	if (Report)

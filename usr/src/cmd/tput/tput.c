...
#define	VSIZE	32	/* Size of each 'faked' argv[] entry. */

		/* allocate storage for the 'faked' argv[] array	*/
		v = (char **)malloc(10 * sizeof (char *));
		for (i = 0; i < 10; i++)
			v[i] = (char *)malloc(VSIZE * sizeof (char));
...
#define	TAB3		XTABS
	...
	if (reset && reset_1string) {
		(void) putp(reset_1string);
	} else if (init_1string) {
		(void) putp(init_1string);
	}

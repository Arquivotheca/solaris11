...
static char EFIFSstr[] = "EFI System";
...
static int	io_expand = 0;		/* expand Solaris Partition (-x) */
...
static void dev_mboot_write_clear(off_t sect, char *buff, int bootsiz);
static void mboot_read(void);
static void mboot_write(off_t sect, char *buff, int bootsiz);
...
static int expand_solaris_partition();
...
update_disk_and_exit(boolean_t table_changed)
{
	...
	if (table_changed) {
		...
		if (io_expand) {
			/*
			 * then we Expanded the Solaris partition.
			 * Write only the MBR; do not allow
			 * side-effects such as clearing disk
			 * labels.
			 */
			mboot_write(0, Bootsect, sectsiz);
		} else {
			dev_mboot_write_clear(0, Bootsect, sectsiz);
		}
	}
...
main(int argc, char *argv[])
{
	...
	/* Process the options. */
	while ((c = getopt(argc, argv, "o:s:P:F:b:A:D:W:S:tTIhwvrndgGRBEx"))
	    != EOF) {
		switch (c) {
	...
	if (!io_image) {
		...
		if (io_debug) {
			...
			(void) fprintf(stderr,
			    ...
			    /* rearrange to prevent overflow */
			    (Numcyl * heads * sectors) / ((1 << 20) / sectsiz));
	...
	/* Does user want to delete, add, or expand an entry? */
	if (io_Dfdisk) {
 ...
 * clear_vtoc_labels
 *
 * If previous partition table (Old_Table[]) contains a Solaris partition
 * and its size or starting address changed in the new table (Table[]),
 * then overwrite its Primary and Backup disk labels (eg, VTOC data
 ...
 * clear_efi_labels
 *
 * If previous partition table (Old_Table[]) contains an EFI partition of
 * type EFI_PMBR ("Protective MBR") and its size or starting address changed
 * in the new table (Table[]), then overwrite its Primary and Backup disk
 * labels (eg, GPT data structures) with zeroes.
 */
static void
clear_efi_labels()
{
...
/*
 * dev_mboot_write_clear
 *
 * Write the master boot sector to the device.
 *
 * If the device contains either a Solaris partition or an
 * EFI-based "Protective MBR" partition, erase the partition's
 * associated disk labels from the disk before writing the
 * master boot sector.
 */
static void
dev_mboot_write_clear(off_t sect, char *buff, int bootsiz)
{
	if (io_readonly)
		return;
	...
	/*
	 * If partition table contains a Solaris partition, clear its
	 * Primary and Backup VTOC disk labels.
	 */
	clear_vtoc_labels();

	/*
	 * If partition table contains an EFI "Protective MBR"
	 * partition, clear its Primary and Backup EFI disk labels.
	 */
	clear_efi_labels();

	/* write the in-memory MBR to disk */
	mboot_write(sect, buff, bootsiz);
}
 ...
 * mboot_write
 *
 * Write the specified buffer as the Master Boot Record (MBR) to the
 * specified sector address.
 *
 * Do the write using an ioctl; if that fails, use lseek() and write().
 */
static void
mboot_write(off_t sect, char *buff, int bootsiz)
{
	if (io_readonly)
		return;
...
/*
 * partition_find_neighbor
 *
 * Find the "neighbor" of the specified Target partition, 'target_tag'.
 *
 * The Neighbor is the partition (if any) whose starting address is
 * closest to the Target partition's ending address.
 *
 * Return value:
 *  neighbor_index	-1 if a Neighbor was not found, else its
 *                       partition table index
 */
static int
partition_find_neighbor(struct ipart *table, int target_tag)
{
	int		i;
	...
	/* highest possible start-addr of Neighbor */
	neighbor_addr = UINT32_MAX;

	/* ending disk-address of Target partition */
	tag_end_addr = LE_32(table[target_tag].relsect) +
	    LE_32(table[target_tag].numsect) - 1;

	for (i = 0; i < FD_NUMPART; i++) {
		if (i == target_tag) {
			/* then Target cannot be its own Neighbor. */
			continue;
		}
		...
		if (table[i].systid != UNUSED) {
			...
			start_addr = LE_32(table[i].relsect);
			if (start_addr > tag_end_addr &&
			    start_addr < neighbor_addr) {
				neighbor_addr = start_addr;
				neighbor_index = i;
				break;
			}
		}
 ...
 * find_solaris_part
 *
 * Compute whether the current device contains a Solaris partition.
 *
 * The Solaris partition can be directly referenced by the MBR or it
 * can be found within a DOS Extended Partition (which is itself
 * referenced in the MBR).
 *
 * The function's return value is a boolean stating whether a Solaris
 * partition was found.  An "id" for the Solaris partition is also
 * returned; if it is greater than zero, it is an index into the MBR,
 * otherwise it's a negative value that flags that the Solaris
 * partition was found in the Extended Partition.
 *
 * This function leverages libfdisk to iterate the partitions within
 * the Extended Partition.
 *
 * Inputs:
 *  table		- Partition Table to search
 *  epp			- state used in Extended Partition processing
 *
 * Outputs:
 *  return value	- non-zero => "a Solaris partition was found"
 *  *solaris_index	>= 0 if Solaris is found in MBR
 *			< 0 if Solaris is found in Extended Partition
 */
static int
find_solaris_part(struct ipart *table, int *solaris_index)
{
	int	systid;
	int	i;
	int	found_solaris = 0;
#ifdef i386
	int	found_ext_part = 0;
#endif
	*solaris_index = -1;

	/* Iterate MBR partition table. */
	for (i = 0; i < FD_NUMPART; i++) {
		systid = LE_8(table[i].systid);
#ifdef i386
		if (systid == SUNIXOS2 || systid == EFI_PMBR ||
		    (systid == SUNIXOS && fdisk_is_linux_swap(epp,
		    LE_32(table[i].relsect), NULL) != 0)) {
#else
		if (systid == SUNIXOS2 || systid == EFI_PMBR ||
		    systid == SUNIXOS) {
#endif
			/* then Solaris is directly referenced in the MBR. */
			*solaris_index = i;
			found_solaris = 1;
#ifdef DEBUG
			(void) fprintf(stderr, "DEBUG: Solaris partition is "
			    "in MBR[%d]\n", i);
#endif
			break;
		}
#ifdef i386
		if (found_ext_part == 0 && fdisk_is_dos_extended(systid)) {
			int		rval;
			int		part_num;
			uint32_t	start_addr;
			uint32_t	numsect;
			...
			/*
			 * Solaris only supports one Extended Partition
			 * per drive.
			 */
			found_ext_part = 1;

			/* Does ExtPart contain a Solaris partition ? */
			rval = fdisk_get_solaris_part(epp, &part_num,
			    &start_addr, &numsect);
			if (rval == FDISK_SUCCESS) {
				found_solaris = 1;
#ifdef DEBUG
				(void) fprintf(stderr, "DEBUG: Solaris "
				    "partition is in Extended Partition "
				    "p%d\n", part_num);
#endif
				break;
			}
	...
	return (found_solaris);
}

/*
 * expand_mbr_part
 *
 * Expand a Solaris partition that is directly referenced by the MBR
 * (as opposed to being indirectly referenced by an Extended Partition).
 *
 * Use the specified index into an MBR's partition table, solaris_index,
 * to search for a "neighbor" partition.  The Neighbor is an MBR
 * partition whose starting address is closest to the Solaris
 * partition's ending address.  The Neighbor's start-addr is used as
 * the limit for Expanding the Solaris partition.  If a Neighbor is not
 * found, use the current disk's capacity as the expansion limit.
 *
 * Subtract Solaris' start-addr from the expansion limit, and if
 * the size is larger than the Solaris partition's current size,
 * update the specified partition table to have the new size and
 * adjust the associated disk geometry values.
 *
 * Outputs:
 *  return value	- B_TRUE => "Solaris partition was Expanded"
 *  table		- partition table updated with new size of
 *                         Solaris partition
 */
static boolean_t
expand_mbr_part(struct ipart *table, int solaris_index)
{
	int		neighbor_id;
	uint32_t	limit;
	uint32_t	new_numsect;
	boolean_t	was_modified = B_FALSE;

	/*
	 * Search for a "neighbor" partition (if any) whose starting
	 * disk address is closest to the Solaris partition's ending
	 * disk address.
	 */
	neighbor_id = partition_find_neighbor(table, solaris_index);

	/*
	 * If a neighbor of the Solaris partition is not found, use the device's
	 * physical capacity as the limit of expansion.
	 */
	limit = (neighbor_id < 0) ? dev_capacity :
	    LE_32(table[neighbor_id].relsect);

	/*
	 * Compute size of the Solaris partition by subtracting start of Solaris
	 * partition from the Limit.
	 */
	new_numsect = limit - LE_32(table[solaris_index].relsect);

	/* Ensure new size fits within a VTOC-based architectural constraint. */
	if (new_numsect > DK_MAX_2TB) {
		new_numsect = DK_MAX_2TB;
	}

#ifdef DEBUG
	(void) fprintf(stderr, "DEBUG: expand MBR: old-size: %lu  "
	    "new-size: %lu  start: %lu  limit: %lu\n",
	    LE_32(table[solaris_index].numsect), new_numsect,
	    LE_32(table[solaris_index].relsect), limit);
#endif

	if (new_numsect > LE_32(table[solaris_index].numsect)) {
		/* then the new size is larger than its current size. */

		was_modified = B_TRUE;

		if (io_debug) {
			(void) fprintf(stderr, "fdisk: MBR before "
			    "expansion:\n");
			print_Table();
		}

		/* Update in-memory partition table and geometry. */
		table[solaris_index].numsect = LE_32(new_numsect);
		Set_Table_CHS_Values(solaris_index);

		if (io_debug) {
			(void) fprintf(stderr, "fdisk: MBR after "
			    "expansion:\n");
			print_Table();
		}
	}

	/* return whether MBR was modified. */
	return (was_modified);
}
 ...
 * expand_ext_part
 *
 * Expand a Solaris partition (SP) that is found within a DOS Extended
 * Partition (EP).
 *
 * The expansion limit for an SP found within an EP is either the
 * start-addr of a Neighboring partition (NP) found within the EP or
 * the end-addr of the EP.
 *
 * Call a libfdisk function that computes the last free sector
 * (starting at a specified sector).  The function abstracts whether
 * the returned address is the start-addr of an NP or the end-addr of
 * the EP; for our needs, the distinction is not important.
 *
 * Use the returned address to compute the size of the Solaris
 * partition.  If the size is larger than the SP's current size, call
 * a libfdisk function to change the SP's size (as represented in the
 * in-memory ext_part_t structure).  Return a boolean declaring that
 * the EP was modified.  The ext_part_t will be written to disk before
 * fdisk exits.
 *
 * Variables used in computing the Expansion of a Solaris partition
 * (up to a Neighbor's boundary):
 *
 *   start_addr                            last_free_sec
 *        |                first_free_sec   |
 *        |                 |               |
 *        v                 v               v
 *       +---  numsect ------+
 ...
 *
 * Inputs:
 *  epp		- state used by libfdisk to process the Extended
 *		    Partition
 *
 * Outputs:
 *  epp		- updated with new size of Solaris partition
 */
static boolean_t
expand_ext_part()
{
	...
	/* Search for Solaris partition within the Extended Partition. */
	rval = fdisk_get_solaris_part(epp, &part_num, &start_addr,
	    &numsect);
	if (rval != FDISK_SUCCESS) {
		return (was_modified);
	}
	...
	/*
	 * Compute address of first sector that must be part of the Expanded
	 * range, which is the address of the last sector in the Solaris
	 * partition.  If "last_addr + 1" is used, an invalid range could be
	 * computed.
	 */
	first_free_sec = start_addr + numsect - 1;

	/*
	 * Find last free sector of a range that is contiguous with
	 * first_free_sec.
	 */
	last_free_sec = fdisk_ext_find_last_free_sec(epp, first_free_sec);

	/* Compute size of the Solaris partition. */
	new_numsect = last_free_sec - start_addr + 1;

	/* Ensure size fits within a VTOC-based architectural constraint. */
	if (new_numsect > DK_MAX_2TB) {
		new_numsect = DK_MAX_2TB;
	}

#ifdef DEBUG
	(void) fprintf(stderr, "DEBUG: expand ExtPart: old-size: %lu  "
	    "new-size: %lu  start: %lu  limit: %lu\n",
	    numsect, new_numsect, start_addr, last_free_sec + 1);
#endif

	if (new_numsect > numsect) {
		/*
		 * then the new size of the Solaris partition is larger than its
		 * current size.
		 */

		/* Update in-memory representation of Extended Partition. */
		rval = fdisk_change_logical_drive_size(epp, part_num,
		    new_numsect);
		if (rval != 0) {
			if (io_debug) {
				(void) fprintf(stderr, "fdisk: Error changing "
				    "size of logical drive: %d\n", rval);
			}
		} else {
			was_modified = B_TRUE;
		}
	...
	/* return whether Extended Partition was modified. */
	return (was_modified);
}
#endif

/*
 * expand_solaris_partition
 *
 * Expand the Solaris partition at a per-sector granularity by
 * modifying the on-disk MBR or (DOS) Extended Partition contents.
 *
 * If there is unused space contiguous to the end of the Solaris
 * partition, update on-disk data so that the size of the Solaris
 * partition can be "expanded".  The starting address of the Solaris
 * partition is not modified.
 *
 * Since a Solaris partition can be found in either the MBR or a "DOS
 * Extended Partition", this function supports both cases.  In the MBR
 * case, the expansion limit is either the starting address of a
 * neighboring partition or the last sector of the disk.  The global
 * variable Table[] is updated to hold the new size of the Solaris
 * partition.
 *
 * In the case of an Extended Partition (EP), the expansion limit is
 * either a neighboring partition within the EP or the last sector of the
 * EP.  The global variable epp, which is libfdisk's state that's used in
 * processing an Extended Partition, is updated to hold the new size of
 * the Solaris partition.
 *
 * If no Solaris partition is found, the function returns without modifying any
 * on-disk state.
 */
static int
expand_solaris_partition()
{
	int		solaris_index;
	boolean_t	mbr_modified = B_FALSE;
	boolean_t	ext_part_modified = B_FALSE;

	/* Search for a Solaris partition on the current disk. */
	if (find_solaris_part(Table, &solaris_index) == 0) {
		/* then no Solaris partition was found */
		if (io_debug) {
			(void) fprintf(stderr, "fdisk: No Solaris "
			    "partition was found.\n");
		}
		return (1);
	}
	...
	if (solaris_index >= 0) {
		/*
		 * then the Solaris partition is directly referenced
		 * in the MBR.
		 */
		mbr_modified = expand_mbr_part(Table, solaris_index);
	}
#ifdef i386
	if (solaris_index < 0) {
		/* then the Solaris partition is in the ExtPart. */
		ext_part_modified = expand_ext_part();
	}
#endif

	if (mbr_modified == B_TRUE || ext_part_modified == B_TRUE) {
		update_disk_and_exit(mbr_modified);
	}
	...
	for (;;) {
		...
		switch (s[0]) {
			...
			case '6':
				...
				if (TableChanged() == 1) {
					copy_Table_to_Bootblk();
					dev_mboot_write_clear(0, Bootsect,
					    sectsiz);
				}
...
pcreate(void)
{
	...
	while (tsystid == 'z') {
		...
		(void) printf(
		    ...
		    "   5=DOS12\n"
		    "   6=DOS16      7=DOSEXT    8=DOSBIG     9=DOS16LBA  "
		    "   A=x86 Boot\n"
		    "   B=Diagnostic C=FAT32     D=FAT32LBA   E=DOSEXTLBA "
		    "   F=EFI (Protective)\n"
		    "   G=EFI_SYS    0=Exit? ");
	...
	if (tsystid != EFI_PMBR) {
		...
		if (retCode < 0) {
			...
			(void) printf(
			    ...
			    "disk.  You may manually delete\n"
			    "existing partitions, or fdisk can do it."

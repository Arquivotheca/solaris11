...
/* SMF configuration refresh */
#define	LDAP_FMRI		"svc:/network/ldap/client:default"
#define	CMD_NSCFG_IMPORT	"/usr/sbin/nscfg import -f -q "
...
sync_current_with_update_copy(server_info_t *info)
{
	...
	info->sinfo[0].serverType = info->sinfo[1].serverType;
...
getldap_get_rootDSE(void *arg)
{
	...
	serverInfo->sinfo[1].server_status =
	    INFO_SERVER_UNKNOWN;
	serverInfo->sinfo[1].serverType = NS_LDAP_SERVERTYPE_UNKNOWN;
	...
	serverInfo->sinfo[1].serverType		= serverType;
 ...
 *   serveraddress DOORLINESEP serverType DOORLINESEP
 *   [ serveraddress FQDN DOORLINESEP ]
 ...
 *   1.2.3.4|0|supportedControl=1.1.1.1|supportedSASLmechanisms=EXTERNAL|
 *   supportedSASLmechanisms=GSSAPI
 *   2) sasl/GSSAPI bind (self credential):
 *   1.2.3.4|0|foo.sun.com|supportedControl=1.1.1.1|
	...
	*svr_removed = FALSE;
	...
	if (server) {
		...
		if (ret_addrFQDN == NULL)
			(void) asprintf(output, "%s%s%d%s%s",
			    ret_addr, DOORLINESEP,
			    (int)server->sinfo[0].serverType, DOORLINESEP,
			    server->sinfo[0].rootDSE_data);
		else
			(void) asprintf(output, "%s%s%d%s%s%s%s",
			    ret_addr, DOORLINESEP,
			    (int)server->sinfo[0].serverType, DOORLINESEP,
 ...
 *   server: server address or name, type, status:
 *   unknown|up|down|removed DOORLINESEP
 *   for example: ( here | used as DOORLINESEP for visual purposes)
 *   server: 1.2.3.4, ODSEE, status: down|server: 2.2.2.2, status: up|
 *   NOTE: caller should free this buffer when done using it
 */
static int
...
#define	S_FORMAT	"  server: %s, %s, status: %s%s"
#define	S_ERROR		"    error message: %s%s"
	...
	*output = (char *)calloc(1, len1);
	...
	for (info = head; info; info = info->next) {
		...
		switch (info->sinfo[1].serverType) {
		...
		(void) snprintf(*output + strlen(*output), len1,
		    gettext(S_FORMAT), info->sinfo[1].addr, server_type,
		    status, DOORLINESEP);
...
perform_update(void)
{
	...
	/*
	 * re-import updated configuration into SMF.
	 * No need to refresh the service, because ldap_cachemgr
	 * is the service.
	 */
	sysret = system(CMD_NSCFG_IMPORT LDAP_FMRI);
	if (sysret != 0) {
		logit("Error: SMF re-import failed: %d\n", system);
		syslog(LOG_ERR, "Unable to refresh profile consistently, "
		    "svc:/network/ldap/client SMF configuration inconsistent");
	}
...
contact_server(char *addr)
{
	...
	ns_ldap_error_t	*error = NULL;
	int		rc;
	ns_ldap_server_type_t	st;

	if (__ns_ldap_getRootDSE(addr, &st, &rootDSE, &error,
	    SA_ALLOW_FALLBACK) != NS_LDAP_SUCCESS) {
		if (current_admin.debug_level >= DBG_ALL)

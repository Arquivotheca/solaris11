...
#define	ALLOCATE 1001
	...
	switch (callnumber) {
		...
		case SETCACHE:
			...
			state = ALLOCATE;
			break;
		case ADMINMODIFY_ADD:
		case ADMINMODIFY_REP:
		case ADMINMODIFY_DEL:
		case ADMIN_ADDENTRY:
		case ADMIN_DELENTRY:
...
client_killserver()
{
	...
	(void) __ns_ldap_trydoorcall(&dptr, &ndata, &adata);
}
...
new_attr(char *name, char *value)
{
	...
	if (tmp != NULL) {
		tmp->attrname = name;
		tmp->attrvalue = (char **)calloc(1, sizeof (char *));
...
static void
free_ldap_attr_t(ns_ldap_attr_t *attr)
{
	if (attr == NULL)
		return;
	free(attr->attrvalue);
...
str2attrs(ldap_strlist_t *strlist, char **dn)
{
	...
	*dn = (char *)strlist + strlist->ldap_offsets[1];
	 ...
	 */
	for (i = 2, curidx = 0; i < c; i = i + 2) {
		tmp = new_attr((char *)strlist + strlist->ldap_offsets[i],
		...
		if (ret[curidx] == NULL) {
			...
			aptr = ret[curidx];
			if (strcmp(aptr->attrname, tmp->attrname) == 0) {
				char **newptr;
				aptr->value_count++;
				newptr = (char **)realloc(
				    aptr->attrvalue,
				    aptr->value_count * sizeof (char *));
				if (newptr == NULL) {
					err = B_TRUE;
					free_ldap_attr_t(tmp);
					goto end;
				}
				aptr->attrvalue = newptr;
				aptr->attrvalue[aptr->value_count - 1] =
				    tmp->attrvalue[0];
				free_ldap_attr_t(tmp);
			} else {
				/* new attribute, update the index */
				curidx++;
				ret[curidx] = tmp;
			}
		}
	...
	if (err && ret != NULL) {
		/* Clean up any attributes in the 'ret' array. */
		for (i = 0; i <= curidx; i++) {
				free_ldap_attr_t(ret[i]);
		}
		free(ret);
		ret = NULL;
	}
 ...
 *
 * The input ldap_call_t (*in) contains LDAP shadowAccount or RBAC
 * attributes to be modified. The data is a flatten ns_ldap_attr_t
 * array stored in the strlist element of the input ldap_call_t.
...
admin_modify(LineBuf *config_info, ldap_call_t *in)
{
	...
	strlist = &in->ldap_u.ldap_admin.strlist;
	attrs = str2attrs(strlist, &dn);
	if (dn == NULL || *dn == '\0' ||
	    (in->ldap_callnumber != ADMIN_DELENTRY &&
	...
	for (app = authpp; *app; app++) {
		...
		switch (in->ldap_callnumber) {
			case ADMINMODIFY_REP:
				rc = __ns_ldap_repAttr(NS_ADMIN_SHADOW_UPDATE,
				    dn,
				    (const ns_ldap_attr_t * const *)attrs,
				    credp, 0, &error);
				break;
			case ADMINMODIFY_ADD:
				rc = __ns_ldap_addAttr(NS_ADMIN_SHADOW_UPDATE,
				    dn,
				    (const ns_ldap_attr_t * const *)attrs,
				    credp, 0, &error);
				break;
			case ADMINMODIFY_DEL:
				rc = __ns_ldap_delAttr(NS_ADMIN_SHADOW_UPDATE,
				    dn,
				    (const ns_ldap_attr_t * const *)attrs,
				    credp, 0, &error);
				break;
			case ADMIN_ADDENTRY:
			{
				...
				entry.attr_count = 0;
				entry.attr_pair = attrs;
				entry.next = NULL;
				rc = __ns_ldap_addEntry(NS_ADMIN_SHADOW_UPDATE,
				    dn, &entry, credp, 0, &error);
				break;
			}
			case ADMIN_DELENTRY:
				rc = __ns_ldap_delEntry(NS_ADMIN_SHADOW_UPDATE,
				    dn, credp, 0, &error);
				break;
			default:
				break;
		...
		/*
		 * For shadow(4) errors we need to do
		 * additional error handling.  For others, just jump
		 * to the cleanup code regardless of the result.
		 */
		if (rc == NS_LDAP_SUCCESS ||
		    in->ldap_u.ldap_admin.ldap_service != LDAP_SVC_SHADOW)
			goto out;

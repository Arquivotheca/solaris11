...
chkfiles(char *source, char **to)
{
	...
	if ((*statf)(target, &s2) >= 0) {
		...
		if ((*statf)(target, &s2) >= 0) {
			...
			/*
			 * overwrite:
			 * If the user does not have access to the target,
			 * and if it is not silent and the user invoked the
			 * command interactively, ask for permission to
			 * overwrite.
			 *
			 * override:
			 * If not silent, and there's no write access, and
			 * the file isn't a symbolic link, ask for permission
			 * to override.
			 *
			 * For both of these operations, if the conditions
			 * are met, then the user will be prompted
			 * irrespective of whether the input is coming from
			 * a terminal.
			 *
...
rcopy(char *from, char *to)
{
	...
	if (fold == 0 || ((pflg || mve) && fstat(dirfd(fold), &statb) < 0)) {
		Perror(from);
		return (1);
	}
	if (pflg || mve) {
...
chg_time(char *to, struct stat ss)
{
	...
	if (ISLNK(s1))
		rc = utimensat(AT_FDCWD, to, times, AT_SYMLINK_NOFOLLOW);
	else

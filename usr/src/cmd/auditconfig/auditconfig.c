...
static arg_entry_t arg_table[] = {
	...
	{ "-setumask",	" username|auid audit_flags",
						AC_ARG_SETUMASK, B_FALSE},
...
static void do_setumask(char *id_str, au_mask_t *mask);
...
{
	arg_entry_t 	*ae;
	boolean_t	temporary_allowed = B_TRUE;
	...
	for (++argv; *argv; argv++) {
		...
		if (ae->auditconfig_cmd != AC_ARG_SET_TEMPORARY) {
			temporary_allowed = temporary_allowed &&
			    ae->temporary_allowed;
		}
		...
		case AC_ARG_SETAUDIT: {
			...
			if (ae->auditconfig_cmd == AC_ARG_SETAUDIT && cnt < 4) {
				exit_usage(1);
			}
	...
	if (!temporary_allowed && temporary_set) {
		exit_error(gettext("One of the specified subcommands does not "
		    "support -t option."));
	}
}
...
do_audit(char *event, char sorf, int retval, char *audit_str)
{
	...
	if (rtn != 0) {
		...
		if (au_close(rd, AU_TO_WRITE, evp->ae_number,
		    (sorf == 0) ? 0 : PAD_FAILURE) == -1) {
			exit_error(
...
do_getclass(char *event_str)
{
	...
	(void) printf(gettext("audit class mask for event %s(%hu) = 0x%llx\n"),
...
do_getcond(void)
{
	int	cond = AUC_DISABLED;

	(void) auditon(A_GETCOND, (caddr_t)&cond, sizeof (cond));

	(void) printf(gettext("audit condition = "));

	switch (cond) {
	case AUC_AUDITING:
		(void) printf("auditing\n");
		break;
	...
	case AUC_INIT_AUDIT: {
		...
		if ((state = smf_get_state(AUDITD_FMRI)) == NULL) {
			exit_error(gettext("Could not get state of the %s "
			    "service."), AUDITD_FMRI);
		}
		if (strcmp(SCF_STATE_STRING_MAINT, state) == 0) {
			(void) printf(gettext("See 'svcs -xv %s' for more "
			    "details.\n"), AUDITD_FMRI);
		}
		free(state);
		break;
 ...
 * specified user name or audit-ID; valid per zone if AUDIT_PERZONE is set, else
 * only in global zone.
 */
static void
do_setumask(char *id_str, au_mask_t *mask)
{
	struct auditinfo 	ainfo;
	struct passwd		*pwd;

	if (strisnum(id_str)) {
		ainfo.ai_auid = (au_id_t)atoi(id_str);
	} else if ((pwd = getpwnam(id_str)) != NULL) {
		ainfo.ai_auid = (au_id_t)pwd->pw_uid;
	} else {
		exit_usage(1);
	}
...
do_setqctrl(char *hiwater, char *lowater, char *bufsz, char *delay)
{
	...
	if (!temporary_set) {
		...
		if (qctrl.aq_hiwater == 0 || qctrl.aq_lowater == 0 ||
		    qctrl.aq_bufsz == 0 || qctrl.aq_delay == 0) {
			return;
		}
	}

	if (qctrl.aq_hiwater == 0) {
		qctrl.aq_hiwater = AQ_HIWATER;
	}
	if (qctrl.aq_lowater == 0) {
		qctrl.aq_lowater = AQ_LOWATER;
	}
	if (qctrl.aq_bufsz == 0) {
		qctrl.aq_bufsz = AQ_BUFSZ;
	}
	if (qctrl.aq_delay == 0) {
		qctrl.aq_delay = AQ_DELAY;
	}
	eauditon(A_SETQCTRL, (caddr_t)&qctrl, 0);
}

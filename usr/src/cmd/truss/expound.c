...
static void
show_utimensat(private_t *pri, long offset)
{
	struct {
		timespec_t atime;
		timespec_t mtime;
...
show_statfs(private_t *pri)
{
	...
	if (pri->sys_nargs >= 2 && (offset = pri->sys_args[1]) != NULL &&
		...
		(void) printf(
		"%s\tfty=%d bsz=%ld fsz=%ld blk=%ld bfr=%ld fil=%u ffr=%u\n",
		    pri->pname,
		    statfs.f_fstyp,
		    statfs.f_bsize,
		    statfs.f_frsize,
...
static void
show_xshmctl64(private_t *pri, long offset)
{
	struct shmid_xds64 shmds;
	...
	if (offset != NULL &&
		...
		(void) printf(" gra=%-5llu all=%llu\n",
		    (unsigned long long)shmds.shmx_granule_sz,
		    (unsigned long long)shmds.shmx_allocated);
	...
	case 1:			/* shmctl() */
		if (pri->sys_nargs > 3) {
			...
			case IPC_XSTAT64:
				if (pri->Errno)
					break;
				show_xshmctl64(pri, (long)pri->sys_args[3]);
				break;
			case IPC_STAT64:
				if (pri->Errno)
					break;
...
show_iovec32(private_t *pri, long offset, int niov, int showbuf, long count)
{
	...
	if (offset != NULL && niov > 0 &&
		...
		for (ip = &iovec[0]; niov-- && !interrupt; ip++) {
			(void) printf("%s\tiov_base = 0x%.8X  iov_len = %u\n",
			    pri->pname,
			    ip->iov_base,
			    ip->iov_len);
			if ((nb = count) > 0) {
	...
	const char *str, long addroff, long lenoff, long len)
{
	...
	case AF_INET6:
		...
		(void) printf("%s\tscope id = %u  source id = 0x%x\n"
		    ...
		    pri->pname,
		    IPV6_FLOW_TCLASS(sin6->sin6_flowinfo),
		    IPV6_FLOW_FLOWLABEL(sin6->sin6_flowinfo));
		break;
	case AF_INET:
		...
		break;
	case AF_INET_SDP:
		(void) printf("%s\tAF_INET_SDP  %s = %s  port = %u\n",
		    pri->pname, str, inet_ntop(AF_INET, &sin->sin_addr,
		    addrbuf, sizeof (addrbuf)), ntohs(sin->sin_port));
		break;
	case AF_UNIX:
...
expound(private_t *pri, long r0, int raw)
{
	...
	case SYS_utimensat:
		if (pri->sys_nargs > 2)
			show_utimensat(pri, (long)pri->sys_args[2]);
		break;
	case SYS_sockconfig:
		show_sockconfig(pri);
		break;

...
{
	const char *s = raw? NULL : ioctlstr(pri, (int)val);
...
prt_mad(private_t *pri, int raw, long val)	/* print madvise() argument */
{
	...
	if (!raw) {
		...
		case MADV_ACCESS_MANY_PSET: s = "MADV_ACCESS_MANY_PSET"; break;
		}
	}
...
prt_pfm(private_t *pri, int raw, long val)
{
	...
	if ((ulong_t)val <= AF_MAX) {
		outstring(pri, soaf2str((int)val));
	} else {
		prt_dec(pri, 0, val);
	}
}
...
prt_skc(private_t *pri, int raw, long val)
{
	...
	if (!raw) {
		...
		case SOCKCONFIG_GET_NUM_ENTRIES:
			s = "SOCKCONFIG_GET_NUM_ENTRIES"; break;
		case SOCKCONFIG_GET_ENTRIES:
			s = "SOCKCONFIG_GET_ENTRIES"; break;
		}
	}
	if (s == NULL)
		prt_dec(pri, 0, val);
...
prt_skp(private_t *pri, int raw, long val)
{
	...
	if ((s = ipproto2str(pri->sys_args[0], (int)val)) != NULL) {
		outstring(pri, s);
	} else {
		prt_dec(pri, 0, val);
	}
 ...
 */
void
prt_exf(private_t *pri, int raw, long val)
{
#define	EXEC_ALLFLAGS	(EXEC_DESCRIPTOR | EXEC_RETAINNAME | EXEC_ARGVNAME)
	char *str;

	if (val == 0)
		outstring(pri, "0");
	else if (raw || (val & ~EXEC_ALLFLAGS))
		prt_hhx(pri, 0, val);
	else {
		str = pri->code_buf;
		*str = '\0';
		if (val & EXEC_DESCRIPTOR)
			(void) strlcat(str, "|EXEC_DESCRIPTOR",
			    sizeof (pri->code_buf));
		if (val & EXEC_RETAINNAME)
			(void) strlcat(str, "|EXEC_RETAINNAME",
			    sizeof (pri->code_buf));
		if (val & EXEC_ARGVNAME)
			(void) strlcat(str, "|EXEC_ARGVNAME",
			    sizeof (pri->code_buf));
		outstring(pri, str + 1);
	}
#undef EXEC_ALLFLAGS
...
prt_zga(private_t *pri, int raw, long val)
{
	...
	if (!raw) {
		...
		case ZONE_ATTR_MAC_PROFILE: s = "ZONE_ATTR_MAC_PROFILE"; break;
		default:
			if ((int)val >= ZONE_ATTR_BRAND_ATTRS) {
				(void) snprintf(buf, sizeof (buf),
				    "ZONE_ATTR_BRAND_ATTRS#%d",
				    (int)val - ZONE_ATTR_BRAND_ATTRS);
				s = (const char *)&buf;
			}
		}
...
/*
 * Print (some) zone_getattr() attribute values.
 */
void
prt_zgx(private_t *pri, int raw, long val)
{
	int subcode = pri->sys_args[2];
	if (!raw) {
		switch ((int)subcode) {
		case ZONE_ATTR_NAME:
		case ZONE_ATTR_ROOT:
...
void (* const Print[])() = {
	...
	prt_exf,	/* EXF -- print execvex() flags */
	...
	prt_zgx,	/* ZGX -- print zone_getattr attribute values */

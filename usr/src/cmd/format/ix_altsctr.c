...
/*	get alts partition table info					*/
	...
	status = raw_rdwr(DIR_READ, cur_file, altsec_offset,
...
/*	get the alts map						*/
	status = raw_rdwr(DIR_READ, cur_file,
...
wr_altsctr()
{
	...
	if (ap->ap_tblp == NULL)
		return (0);
	status = raw_rdwr(DIR_WRITE, cur_file, altsec_offset,
	    ap->ap_tbl_secsiz / UBSIZE, (char *)ap->ap_tblp, 0, NULL);
	if (status) {
		(void) printf("raw_rdwr status = %d need = %d\n",
		    status, ap->ap_tbl_secsiz / 512);
		perror("Unable to write with raw_rdwr the alt sector part: ");
		return (62);
	}

	if (raw_rdwr(DIR_WRITE, cur_file, (ap->ap_tblp->alts_map_base) +
			altsec_offset, ap->ap_map_secsiz / UBSIZE,
			(char *)ap->ap_mapp, 0, NULL) != SUCCESS) {
	    perror("Unable to write alternate sector partition map: ");
	    return (63);
	}

	if (ap->ap_tblp->alts_ent_used != 0) {
	    if (raw_rdwr(DIR_WRITE, cur_file,
				(ap->ap_tblp->alts_ent_base)+ altsec_offset,
				ap->ap_ent_secsiz / UBSIZE,
				(char *)ap->ap_entp, 0, NULL) != SUCCESS) {

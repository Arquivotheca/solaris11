...
/*
 * Determine whether to show the 'expand' menu item in the 'partition' menu.
 *
 * To Expand a disk is to modify the disk's on-disk metadata so that the
 * metadata comprehends any increase in the disk's physical size.
 */
int
disp_expand(void)
{
	...
	if ((cur_disk->label_type != L_TYPE_EFI) &&
		...
		return (0);
	/*
	 * For VTOC-labeled disks, ensure that cur_disk is not in-use,
	 * as a modification of on-disk metadata while the disk is in
	 * use can cause in-memory and on-disk metadata to become
	 * inconsistent (for any number of currently executing clients
	 * of the device).
	 */
	if ((cur_disk->label_type == L_TYPE_SOLARIS) &&
	    ...
	    (diskaddr_t)-1, 0, 0))) {
		/* then disk is in use; do not allow modifcations. */
		return (0);
	} else {
		/* Show the command only if the disk can be expanded. */
		return (can_label_be_expanded(cur_disk));
	}
}

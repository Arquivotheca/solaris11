 ...
 * Any number preceding the first slash is considered a cylinder value.
 * Any number succeeding the first slash but preceding the second is
	...
	int	num;
{
	...
	if ((type != NULL) && (disk->label_type == L_TYPE_SOLARIS)) {
		...
		print_efi_string(type->disk_id.vendor,
		    type->disk_id.product, type->disk_id.revision,
		    type->disk_id.capacity);
	...
	if (disk->chassis_location && *disk->chassis_location) {
		fmt_print("          %s\n", disk->chassis_location);
		lines_printed++;
	}

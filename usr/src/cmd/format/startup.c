...
static void	search_for_logical_dev(char *devname, char *curdir);
...
static char	*get_chassis_location(char *);
...
static char	*get_chassis_location();
...
#define	OTHER_CTLRS (sizeof (other_ctlrs) / sizeof (other_ctlrs[0]))
...
do_options(int argc, char *argv[])
{
	...
	dev_expert = 0;
	force_label = 0;
	force_label_type = L_TYPE_SOLARIS;
	...
	for (i = 1; i < argc; i = next) {
		...
		if (*argv[i] != '-') {
			/*
			 * If attempt to force a label, the target disk
			 * must be specified and in expert mode.
			 */
			if ((!option_d) && force_label) {
				usage();
			}
			return (i);
		}
		...
		for (ptr = argv[i] + 1; *ptr != '\0'; ptr++) {
			...
			case 'L':
				force_label = 1;
				label_type = argv[next++];
				if (label_type == 0) {
					goto badopt;
				}
				if (strcmp(label_type, "vtoc") == 0) {
					force_label_type = L_TYPE_SOLARIS;
				} else if (strcmp(label_type, "efi") == 0) {
					force_label_type = L_TYPE_EFI;
				} else {
					goto badopt;
				}
				break;
...
usage()
{
	...
	err_print("\n\tformat -L label-type -d disk_name\n");
	fullabort();
}
...
do_search(char *arglist[])
{
	...
	if (arglist != NULL) {
		...
		for (; *arglist != NULL; arglist++) {
			search_for_logical_dev(*arglist, curdir);
		}
	} else {
		/*
		...
		if (sp != NULL) {
			while (*sp != NULL) {
				search_for_logical_dev(*sp++, curdir);
			}
		}
	...
	for (disk = disk_list; disk != NULL; disk = disk->disk_next) {
		...
		if (disk->disk_flags & DSK_AUTO_CONFIG) {
			...
			if (disk->label_type == L_TYPE_SOLARIS)
				...
				scaled = bn2mb(type->disk_id.capacity);
 ...
 */
static void
search_for_logical_dev(char *devname, char *curdir)
{
	char		path[MAXPATHLEN];
	char		cwd[MAXPATHLEN];
	char		*directory = "/dev/rdsk/";
	...
	/*
	 * If the name is an absolute path name, accept it as is after
	 * conversion to a realpath and, if realpath goes into /devices but
	 * has no minor node, the addition of a minor node.
	 */
	if (*devname == '/') {
		if (realpath(devname, path) == NULL)
			(void) strcpy(path, devname);
		if ((strncmp(path, "/devices/", 9) == 0) &&
		    (strchr(path, ':') == NULL))
			(void) strcat(path, minor);
		...
		(void) strcpy(path, directory);
		(void) strcat(path, devname);
		if ((stat(path, &stbuf) < 0) && curdir) {
			/*
			 * Nothing else worked, resolve relative to
			 * curdir so that things like this work:
			 *   cd /dev/chassis/SYS/HD0; format disk
			 */
			(void) strcpy(cwd, curdir);
			(void) strcat(cwd, "/");
			(void) strcat(cwd, devname);
			if (realpath(cwd, path) == NULL)
				(void) strcpy(path, cwd);
			if ((strncmp(path, "/devices/", 9) == 0) &&
			    (strchr(path, ':') == NULL))
				(void) strcat(path, minor);
		}
...
add_device_to_disklist(char *devname, char *devpath)
{
	 ...
	 * and physical chassis location.
	 */
	search_disk->devfs_name = get_physical_name(devpath);
	search_disk->chassis_location =
	    get_chassis_location(search_disk->devfs_name);
	...
	if (search_disk->label_type == L_TYPE_EFI) {
		...
		(void) memcpy(&search_dtype->disk_id, &efi_info.disk_id,
		    sizeof (search_dtype->disk_id));
	...
	if (search_dtype == NULL) {
		...
		if (strncmp(search_label.dkl_asciilabel, "DEFAULT",
		    strlen("DEFAULT")) == 0) {
			struct disk_id diskid;

			(void) strlcpy(disk_name, "unknown",
			    sizeof (disk_name));
			if (get_disk_id(search_file, &diskid) == 0) {
				get_disk_name(disk_name, &diskid);
			} else {
				err_print(
...
/*
 * return the chassis location given a /devices path (without /devices
 * prefix but with leading "/", and without minor name).
 */
static char *
get_chassis_location(char *devicespath)
{
	int		len;
	char		*occupant_devices;
	char		*devchassis_path = NULL;
	di_cro_reca_t	ra;
	di_cro_rec_t	r;
	char		*query;
	int		query_len;
	...
	/* 'occupant_devices' is a di_devfs_path with "/devices" prefix. */
	len = strlen("/devices") + strlen(devicespath) + 1;
	occupant_devices = malloc(len);
	if (occupant_devices == NULL)
		return (NULL);
	(void) snprintf(occupant_devices, len, "/devices%s", devicespath);

	/* find the di_cro record associated with the occupant_devices path */
	query_len = strlen(query_fmt) + strlen(occupant_devices) + 1;
	if ((query = malloc(query_len)) == NULL)
		goto noloc;
	(void) snprintf(query, query_len, query_fmt, occupant_devices);
	ra = di_cro_reca_create_query(cro, 0, query);
	r = di_cro_reca_next(ra, NULL);
	devchassis_path = di_cro_rec_fgeti_devchassis_path(r, 0, NULL, NULL);
	di_cro_reca_destroy(ra);
	free(query);
noloc:	free(occupant_devices);

	return (devchassis_path ? strdup(devchassis_path) : NULL);
}
	...
	const void	*arg2)
{
	...
	s1 = (*((struct disk_info **)arg1))->chassis_location;
	s2 = (*((struct disk_info **)arg2))->chassis_location;
	if (s1 && s2) {
		/*
	...
	/* no location (or same location) - sort by name */
name:	s1 = (*((struct disk_info **)arg1))->disk_name;

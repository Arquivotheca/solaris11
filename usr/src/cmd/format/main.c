...
"Specified table '%s' is not a known table.\n", option_p);
	...
	if (disk != NULL) {
		init_globals(disk);
		/*
		 * If the option "L" is specified with format, the
		 * specified label will be put on disk noninteractively.
		 */
		if (force_label) {
			ret_code = c_label();
			if (ret_code == 0) {
				fmt_print("%s is labeled with %s "
				    "successfully.\n",
				    disk->disk_name,
				    (force_label_type == L_TYPE_SOLARIS ?
				    "VTOC" : "EFI"));
			} else {
				fmt_print("Failed to label %s with %s.\n",
				    disk->disk_name,
				    (force_label_type == L_TYPE_SOLARIS ?
				    "VTOC" : "EFI"));
			}
...
Continue"))
	...
	/*
	 * Check if the disk label represents a size that is
	 * less than what the disk can physically hold.
	 */
	if (can_label_be_expanded(cur_disk) != 0) {
		/*
		 * Lun expansion detected. Inform user now and
		 * do adjustments in <partition> command.
		 */
		if (cur_disk->label_type == L_TYPE_EFI) {
			fmt_print("Note: capacity in disk label is "
			    "less than the disk capacity.\n"
			    "Select <partition> <expand> to adjust "
			    "the label capacity. \n");
		} else {
			fmt_print("Note: detected additional allowable "
			    "expansion storage space that can be \n"
			    "added to current SMI label's computed "
			    "capacity.\nSelect <partition> <expand> "
			    "to adjust the label capacity. \n");
		}
	}
}

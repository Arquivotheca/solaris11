...
auto_efi_sense(int fd, struct efi_info *label)
{
	 ...
	 */
	if (get_disk_id(fd, &label->disk_id) == -1) {
	...
	(void) memcpy(&disk->disk_id, &label->disk_id, sizeof (disk->disk_id));
 ...
 *	- generic sense of disk ID and capacity
	...
	struct dk_label	*label)
{
	 ...
	 */
	if (can_prompt && expert_mode && (!force_label)) {
		deflt = 1;
	...
	if (get_disk_id(fd, &diskid) != 0)
		return ((struct disk_type *)NULL);

	if (option_msg && diag_msg) {
		err_print("Product id: ");
		print_buf(diskid.product, sizeof (diskid.product));
		err_print("\n");
		err_print("blocks:  %llu (0x%llx)\n",
		    diskid.capacity, diskid.capacity);
		err_print("blksize: %u\n", diskid.lbsize);
	}
	 ...
	 */
	if ((disk_type = find_scsi_disk_by_name(diskid.product)) != NULL) {
	...
	return (generic_disk_sense(fd, can_prompt, label, &diskid));
}
	...
	const struct disk_id	*diskid)
{
	 ...
	 * anyway.
	 */
	if (memcmp(diskid->product, "SUN", strlen("SUN")) == 0)
		(void) strlcpy(disk_name, diskid->product, sizeof (disk_name));
	else
		get_disk_name(disk_name, diskid);

	tblocks = diskid->capacity;
	...
	if (setdefault != 1) {
		...
		if (nhead == 0 || nsect == 0) {
			setdefault = 1;
		} else if (pcyl < SUN_MIN_CYL) {
			if (adjust_disk_geometry((diskaddr_t)(diskid->capacity),
			    &pcyl, &nhead, &nsect)) {
				setdefault = 1;
			}
		}
	 ...
	 * those two pages but compute geometry directly. They also
	 * won't work by default inside a zone.
	 */
	if ((setdefault == 1) || (diskid->lbsize != DEV_BSIZE)) {
		/*
	...
	if (((diskaddr_t)pcyl * nhead * nsect) > nblocks) {
		...
		if (can_prompt && expert_mode && !option_f) {
			 ...
			 * can optimize. For compatibility reasons do this
	...
	if ((disk = find_scsi_disk_type(disk_name, label)) == NULL) {
		if (find_scsi_disk_by_name(disk_name) != NULL) {
			...
			(void) get_disk_name(disk_name, diskid);
			if (option_msg && diag_msg) {
				err_print(
 ...
 *
 * NOTE: This function is same as cmlb_convert_geometry() function in
 * cmlb kernel module.
 */
static void

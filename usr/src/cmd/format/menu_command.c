 ...
 */
	...
	for (disk = disk_list; disk != NULL; disk = disk->disk_next) {
		/*
		 * If using more output, account 2-3 lines for each disk.
		 */
		if (more && !more_quit && i && (one_line ||
		    ((lines_printed + 1) % (tty_lines - 3) <= 1))) {
			int	c;
			lines_printed = 0;
			/*
		...
		if (!more || !more_quit)
			lines_printed += pr_diskline(disk, i);
		i++;
	}
	if (more) {
		enter_critical();
...
c_type()
{
	...
	if (index == auto_conf_choice) {
		...
		case L_TYPE_EFI:
			...
			scaled = bn2mb(efi_info.disk_id.capacity);
			if (scaled > 1024.0) {
				fmt_print("%1.2fGB\n", scaled/1024.0);
			} else {
				fmt_print("%1.2fMB\n", scaled);
			}
			print_efi_string(efi_info.disk_id.vendor,
			    efi_info.disk_id.product, efi_info.disk_id.revision,
			    efi_info.disk_id.capacity);
			fmt_print("\n");
...
c_current()
{
	...
	if (cur_disk->devfs_name != NULL) {
		if (cur_dtype == NULL) {
			...
			print_efi_string(cur_dtype->disk_id.vendor,
			    cur_dtype->disk_id.product,
			    cur_dtype->disk_id.revision,
			    cur_dtype->disk_id.capacity);
			fmt_print("\n");
		}
		fmt_print("%s\n", cur_disk->devfs_name);
		if (cur_disk->chassis_location)
			fmt_print("%s\n", cur_disk->chassis_location);
	} else {
...
c_label()
{
	...
	if (expert_mode || force_label) {
		...
		} else if (cur_label == force_label_type) {
			goto expert_end;
		} else {
			choice = force_label_type;
		}
		switch (choice) {
		case 0:
		/*
		 ...
		 */
		if (cur_dtype->disk_id.capacity > INFINITY) {
		...
		if (cur_disk->fdisk_part.systid == EFI_PMBR) {
			if (!force_label) {
				fmt_print("Warning: This disk has an EFI "
				    "label. Changing to SMI label will erase\n"
			...
			/*
			 * If we want to force to label a disk,
			 * use c_fdisk to create a fdisk table and
			 * a solaris partition.
			 */
			if (force_label) {
				return (c_fdisk());
		...
		if (get_disk_id(cur_file, &efinfo.disk_id) != 0) {
			return (-1);
		}
		(void) memset((char *)&label, 0, sizeof (struct dk_label));
		label.dkl_pcyl = pcyl;
...
c_verify_efi()
{
	...
	print_efi_string(efi_info.disk_id.vendor, efi_info.disk_id.product,
	    efi_info.disk_id.revision, efi_info.disk_id.capacity);
	fmt_print("\n");
...
c_inquiry()
{
	if (generic_inquiry(cur_file) != 0)
		return (scsi_inquiry(cur_file));
	return (0);
}

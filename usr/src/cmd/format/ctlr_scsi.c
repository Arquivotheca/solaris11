...
static int is_uscsi_allowed = -1;

/*
 * We presume here that INQUIRY must work if any SCSI command will work.
 */
int
uscsi_allowed(int fd)
{
	if (is_uscsi_allowed == -1) {
		struct scsi_inquiry inquiry;
		/*
		 * As a side-effect, this sets is_uscsi_allowed.
		 */
		(void) uscsi_inquiry(fd, (char *)&inquiry, sizeof (inquiry));
	}

	return (is_uscsi_allowed);
}

/*
 * New disk: inside a zone, SCSI permissions may be per-device, so we must
 * re-probe.
 */
void
reset_uscsi(void)
	...
	int	*xfercntp;
{
	...
	if (!uscsi_allowed(fd)) {
		/*
		 * raw_rdwr() doesn't handle xfer counts.
		 */
		if (xfercntp != NULL)
			return (-1);

		return (raw_rdwr(dir, fd, blkno, secnt, bufaddr, flags, NULL));
	}
...
scsi_inquiry(int fd)
{
	...
	fmt_print("Vendor:   ");
	print_buf(inquiry.inq_vid, sizeof (inquiry.inq_vid));
	fmt_print("\nProduct:  ");
	print_buf(inquiry.inq_pid, sizeof (inquiry.inq_pid));
	fmt_print("\nRevision: ");
	print_buf(inquiry.inq_revision, sizeof (inquiry.inq_revision));
	fmt_print("\n");
	return (0);
}

void
scsi_get_disk_id(int fd, struct disk_id *diskid)
{
	struct scsi_inquiry inquiry;
	...
	/*
	 * size looks weird, but inquiry. isn't nil-terminated, and
	 * diskid-> should be.
	 */
	(void) strlcpy(diskid->vendor, inquiry.inq_vid,
	    sizeof (inquiry.inq_vid) + 1);
	(void) strlcpy(diskid->product, inquiry.inq_pid,
	    sizeof (inquiry.inq_pid) + 1);
	(void) strlcpy(diskid->revision, inquiry.inq_revision,
	    sizeof (inquiry.inq_revision) + 1);
}

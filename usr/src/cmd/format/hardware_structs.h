...
struct disk_info {
	...
	char			*chassis_location; /* device location */
...
/*
 * Macros to access the fields used to represent a Partition (in an
 * architecture-independent manner):
 *
 *   Tag, Flags, Starting Disk Address, and Size (in Sectors)
 */

/*
 * Access a partition's attributes via the "struct dk_vtoc":
 */
#define	PART_TAG_VTOC(_partition, _i)	(_partition)->vtoc.v_part[(_i)].p_tag
#define	PART_FLAG_VTOC(_partition, _i)	(_partition)->vtoc.v_part[(_i)].p_flag
#if defined(_SUNOS_VTOC_16)
#define	PART_ADDR_VTOC(_partition, _i)	(_partition)->vtoc.v_part[(_i)].p_start
#define	PART_SIZE_VTOC(_partition, _i)	(_partition)->vtoc.v_part[(_i)].p_size
#endif
/*
 * Access a partition's attributes via its "struct dk_map32" field:
 */
#define	PART_ADDR_MAP(_partition, _i)	(_partition)->pinfo_map[(_i)].dkl_cylno
#define	PART_SIZE_MAP(_partition, _i)	(_partition)->pinfo_map[(_i)].dkl_nblk

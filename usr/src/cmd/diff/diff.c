...
static int	check_loop(const char *, const char *);
...
talloc(size_t n)
{
	...
	p = malloc(n);
	if (p == NULL)
		error(gettext("files too big, try -h\n"));
	return (p);
}
...
main(int argc, char **argv)
{
	...
	char *diffhargv[5];		/* arguments for the diffh command */
	int diffhargc = 0;
...
/*
 * If you add a new command line option to diff, then you will also need
 * to add it to the diffh: "if (opt || ...flags...)" clause below, so that
 * the diffh program continue to work correctly.
 */
	...
	if (hflag) {
		/*
		 * diffh: flags for any new command line options need to be
		 * added here, to be properly excluded.
		 */
		if (opt || iflag || lflag || rflag ||
		    Sflag || sflag || tflag || wflag) {
			error(gettext("only -b is allowed with the -h option"));
		} else {
			diffhargv[diffhargc++] = "diffh";
			if (bflag)
				diffhargv[diffhargc++] = "-b";
			diffhargv[diffhargc++] = file1;
			diffhargv[diffhargc++] = file2;
			diffhargv[diffhargc] = NULL;

			(void) execv(diffh, &diffhargv[0]);
...
change(int a, int b, int c, int d)
{
	...
	if (anychange == 0) {
		...
		if (opt == D_CONTEXT) {
			...
			if (strftime(time_buf, sizeof (time_buf), dcmsg,
			    localtime(&stb1.st_mtime)) == 0)
			...
			context_vec_start = (struct context_vec *)
			    malloc(MAX_CONTEXT * sizeof (struct context_vec));
...
diffdir(char **argv)
{
	...
	dirname1 = realpath(file1, NULL);
	dirname2 = realpath(file2, NULL);
	if (dirname1 == NULL || dirname2 == NULL)
		error(gettext(NO_MEM_ERR));

	while (d1->d_entry != 0 || d2->d_entry != 0) {
		if (d1->d_entry &&
		    (useless(d1->d_entry) ||
		    check_loop(dirname1, d1->d_entry))) {
			d1++;
			continue;
		}
		...
		if (d2->d_entry &&
		    (useless(d2->d_entry) ||
		    check_loop(dirname2, d2->d_entry))) {
			d2++;
			continue;
		}
...
calldiff(char *wantpr)
{
	...
	(void) fflush(stdout);
	if (wantpr) {
		(void) snprintf(etitle, sizeof (title) - (etitle - title),
		    "%s %s", file1, file2);
		(void) pipe(pv);
		pid = fork();
		if (pid == (pid_t)-1)
...
/* Check that the file really exists and does not form a symbolic link loop. */

static int
check_loop(const char *dir_path, const char *cp)
{
	struct stat buf;
	char file[PATH_MAX];
	char resolved_name[PATH_MAX], *rp;

	if (cp == (char *)NULL || *cp == '\0') {
		return (0);
	}
	(void) snprintf(file, sizeof (file), "%s/%s", dir_path, cp);
	...
	/*
	 * If this is a symbolic link and the realpath points to the
	 * directory containing this link, then we have a loop.
	 */
	if ((buf.st_mode & S_IFMT) == S_IFLNK) {
		if ((rp = realpath(file, resolved_name)) == NULL) {
			(void) fprintf(stderr, "diff: ");
			perror(file);
			return (1);
		}

		if (strcmp(dir_path, rp) == 0) {
			(void) fprintf(stderr, gettext(
			    "diff: loop detected for symbolic link: %s\n"),
			    file);
			return (1);
		}
	}

...
static char *_update_dev(da_deventry_t *, int, const char *, char *, char *);
static int _make_db();
...
_da_check_for_usb(char *link, char *root_dir)
{
	...
	int		hotpluggable = 0;
	char		*p = NULL;
	boolean_t	priv_was_set;
	char		path[MAXPATHLEN + 4];
	char		rpath[MAXPATHLEN + 4];		/* for ",raw" */

	priv_was_set = priv_ineffect(PRIV_FILE_DAC_READ);
	dstsize = sizeof (path);
...
/* ARGSUSED */
void
_update_devalloc_db(da_devlist_t *devlist, int devflag, int action,
    char *devname, char *root_dir)
{
	int		i;
	da_deventry_t	*entry = NULL, *dentry = NULL;
	char 		*typestring;
 ...
 */
static char *
_update_dev(da_deventry_t *dentry, int action, const char *devtype,
    char *devname, char *rm_link)
{
	da_args		dargs;
	da_deventry_t	newentry, *entry;
	int status;

...
/*
 * Devfsadm is the overall administrative tool for /dev.
 * It creates logical links in /dev for corresponding physical
 * device path names in /devices and coordinates updates to
 * /etc/path_to_instance with the kernel.  It can be invoked
 * via command line to handle user or script invoked updates,
 * and as a daemon to handle dynamic link generation for
 * hotplugging support.
 *
 * Note: devfsadm does not play a role with certain dynamic
 * subsets of /dev such as /dev/pts and zones profiles, which
 * are maintained in a consistent state with the kernel.
 */
...
extern void _update_devalloc_db(da_devlist_t *, int, int, char *, char *);
...
/*
 * locking variables to prevent multiples writes
 * to /dev and attempts to start a duplicate
...
/* Global to use cached devinfo snapshot */
...
/*
 * Adjust runtime privileges by removing unnecessary privs
 * from our effective set to allow secure privilege bracketing
 * as needed.
 *
 * Removing it from the inheritable set means that it
 * is not retained across fork.
 *
 * ANY adjustment of privileges on a labeled system sets the
 * priv_aware flag.
 */
static void
adjust_privileges()
{
	(void) priv_set(PRIV_OFF, PRIV_EFFECTIVE, PRIV_FILE_DAC_READ,
	    PRIV_SYS_RESOURCE, NULL);
	(void) priv_set(PRIV_OFF, PRIV_INHERITABLE, PRIV_FILE_DAC_READ,
	    PRIV_SYS_RESOURCE, NULL);
}
/*
 * Set up privileges needed by _reset_devalloc when invoked as devfsadm -e
 * This can be needed even if adjust_privilges was never called, since
 * there are library routines that may adjust privileges.
 */
static int
_db_privileges(int restore)
{
	static priv_set_t *p_inheritable = NULL;
	static priv_set_t *p_effective = NULL;
	static int restorable = 0;


	/*
	 * If calling to restore, restore (if possible) and return
	 * Manipulating privs will automatically set PRIV_AWARE.
	 */
	if (restore == 1) {
		if (restorable == 1) {
			(void) setppriv(PRIV_SET, PRIV_INHERITABLE,
			    p_inheritable);
			(void) setppriv(PRIV_SET, PRIV_EFFECTIVE, p_effective);
			restorable = 0;
		}
		return (0);
	}
	...
	/* Calling to set */
	if (p_inheritable == (priv_set_t *)NULL) {
		p_inheritable = priv_allocset();
		if (p_inheritable == NULL) {
			restorable = 0;
			return (1);
		}
	}
	if (p_effective == (priv_set_t *)NULL) {
		p_effective = priv_allocset();
		if (p_effective == NULL) {
			priv_freeset(p_inheritable);
			p_inheritable = (priv_set_t *)NULL;
			restorable = 0;
			return (1);
		}
	}
	if ((getppriv(PRIV_INHERITABLE, p_inheritable) != 0) ||
	    (getppriv(PRIV_EFFECTIVE, p_effective) != 0)) {
		restorable = 0;
		return (1);
	}
	if (priv_ismember(p_inheritable, PRIV_FILE_DAC_READ) &&
	    priv_ismember(p_effective, PRIV_FILE_DAC_READ)) {
		restorable = 0;
		return (0);   /* already have the privs we need */
	}

	if (priv_set(PRIV_ON, PRIV_INHERITABLE, PRIV_FILE_DAC_READ, NULL) ==
	    -1) {
		restorable = 0;
		return (1);
	}

	if (priv_set(PRIV_ON, PRIV_EFFECTIVE, PRIV_FILE_DAC_READ, NULL) ==
	    -1) {
		(void) setppriv(PRIV_SET, PRIV_INHERITABLE, p_inheritable);
		restorable = 0;
		return (1);
	}
	restorable = 1;
...
main(int argc, char *argv[])
{
	...
	if (daemon_mode == TRUE) {
		...
		if (pid != 0) {
			 ...
			 * of system/devfsadm can be started with full
			 * assurance that the service is fully functional.
			 */
			int status;
		...
		if (devalloc_flag != 0) {
			...
			if (_db_privileges(0) != 0) {
				char tmpbuf[2048];
				(void) snprintf(tmpbuf, sizeof (tmpbuf),
				    "device_allocate,device_maps: %s",
				    strerror(errno));
				err_print(CANT_UPDATE, tmpbuf);
				devfsadm_exit(1);
			}
			_reset_devalloc(devalloc_flag);
			(void) _db_privileges(1);
		}
	}
	return (0);
}
...
retry:
	...
	/*
	 * service is now functional
	 * signal the daemon initiating process to return status now
	 */
	if (kill(getppid(), SIGUSR1) != 0) {
		err_print("devfsadmd: signal to parent failed - %s\n",
		    strerror(errno));
		devfsadm_exit(1);
	}
    ...
    door_desc_t *dp, uint_t ndesc)
{
	...
	/* reject any call by an unprivileged user */
	if (door_ucred(&uc) != 0) {
		dcp = &rdca;
		dcp->dca_error = EPERM;
		goto out;
...
event_handler(sysevent_t *ev)
{
	...
	if (strcmp(subclass, ESC_DEVFS_DEVI_ADD) == 0 ||
		...
		build_and_enq_event(EC_DEV_BRANCH, dev_ev_subclass, path,
		    DI_NODE_NIL, NULL);
		unlock_dev(CACHE_STATE);
	} else if (strcmp(subclass, ESC_DEVFS_PATHI_ADD) == 0 ||
	    strcmp(subclass, ESC_DEVFS_PATHI_REMOVE) == 0) {
		/* pathinfo nodes have no direct namespace */
		err = 0;
		goto out;
	} else
...
devfsadm_mklink(char *link, di_node_t node, di_minor_t minor, int flags)
{
	...
	if (link_exists == TRUE) {
		...
		if (system_labeled && (flags & DA_ADD) &&
			...
			/*
			 * Avoid confusion caused by having the disk_common
			 * link facility call devfsadm_mklink on the raw
			 * devices with DA_CD while the usb_link updater
			 * calls us on the block devices with DA_RMDISK.
			 *
			 * This behavior is seen with usb floppies, usb cd's,
			 * and usb ramdisks.  The more-modern machines use
			 * usb connections even INTERNALLY for peripheral
			 * drives.
			 *
			 * USB CD or floppy vs thumb drives cannot be
			 * distinguished on hotplugging so if we can't tell,
			 * guess "RMDISK" if called from a sysevent.
			 */
			if (flags & DA_CD) {
				chk = _da_check_for_usb(devlink, root_dir);
				if (chk == 1) {
					(void) da_add_list(&devlist, devlink,
					    instance, DA_ADD|DA_RMDISK);
					update_devdb = DA_RMDISK;
				}
			} else if ((linknew == TRUE) && (!flags & DA_AUDIO)) {
			/* devfsadm can't hotplug audio yet, use mkdevalloc */
    ...
    int dangle)
{
	...
	/*
	 * This routine will return the "raw" contents. It is up to the
...
disk_enumerate_int(char *devfs_path, int index, char **buf,
    devfsadm_enumerate_t rules[], int nrules)
{
	char	*scsi_vhci = strstr(devfs_path, "/scsi_vhci/");

	/* If possible, reserve "c0" for scsi_vhci. */
	return (find_enum_id(rules, nrules,
	    devfs_path, index, scsi_vhci ? "0" : "1", INTEGER, buf, 1));
}

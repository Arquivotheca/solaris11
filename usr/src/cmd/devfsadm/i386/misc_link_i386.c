...
static int vga_arbiter(di_minor_t minor, di_node_t node);
...
static devfsadm_create_t misc_cbt[] = {
	...
	{ "pseudo", "ddi_pseudo", NULL,
	    TYPE_EXACT, ILEVEL_0, vga_arbiter
	},
...
drm_node(di_minor_t minor, di_node_t node)
{
	char *minor_nm, *drv_nm;
	char *devfspath;
	char *l_path, *p_path, *buf;
	char *ep;
	long inst;

	devfsadm_enumerate_t drm_rules[1] = {
		"^dri$/^card([0-9]+)$", 1, MATCH_ALL};
	devfsadm_enumerate_t controlD_rules[1] = {
		"^dri$/^controlD([0-9]+)$", 1, MATCH_ALL};
	...
	if (strncmp(minor_nm, "drm", 3) == 0 &&
	    !devfsadm_enumerate_int(p_path, 0, &buf, drm_rules, 1)) {
		(void) snprintf(l_path, PATH_MAX, "dri/%s%s", "card", buf);
	} else if (strncmp(minor_nm, "controlD", 8) == 0 &&
	    !devfsadm_enumerate_int(p_path, 0, &buf, controlD_rules, 1)) {
		inst = strtol(buf, &ep, 10);
		/*
		 * The instance number of controlD starts from 64.
		 * For more details, please see function drm_minor_get_id() in
		 * usr/src/uts/common/io/drm/drm_stub.c.
		 */
		if (inst < 64)
			inst += 64;
		(void) snprintf(l_path, PATH_MAX, "dri/%s%d", "controlD", inst);
	} else {
...
/*
 * Creates devlink for vga_arbiter node
 */
static int
vga_arbiter(di_minor_t minor, di_node_t node)
{
	char *mn;

	if (strcmp(di_node_name(node), "vga_arbiter") != 0)
		return (DEVFSADM_CONTINUE);

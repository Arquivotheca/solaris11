...
static int getuidbyname(char *);
static int getgidbyname(char *);
...
static boolean_t extract_authorized(void);
...
static	char	*tmpdir;
static	const char	*tmp_suffix = "/tarXXXXXX";
static	char	*tname;
...
static const char	*tchar = "";		/* null linkpath */
...
/*
 * Maximum number of compressed header magic bytes. Note that we don't need
 * an extra byte for a '\0' terminator, as comparisons are done with memcmp()
 * rather than strcmp().
 */

#define	MAX_HDR_MAGIC	3
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	for (cp = *argv++; *cp; cp++)
		...
		case 'x':
			/*
			 *  See if user is allowed to extract
			 */
			if (!extract_authorized()) {
				exit(1);
			}
			xflag++;
			break;
	...
	if (rflag) {
		...
		/*
		 * If the input file descriptor is not seekable, then set
		 * Bflag to make sure we correctly handle short reads.
		 */
		if (lseek(mt, 0, SEEK_SET) == -1)
			++Bflag;

		/*
		 * Get a file descriptor to read input from, decompressing
		 * that input if needed.
		 */
		if ((mt = decompress_from_fd(mt)) < 0) {
			fatal(gettext("Could not decompress input"));
		}
...
top:
	...
	(void) sscanf(dblock.dbuf.size, "%12" FMT_off_t_o,
    ...
    int filetype, int lev, int symlink_lev)
{
	...
	case S_IFDIR:
		...
		if (*shortname != '/')
			(void) snprintf(newparent, sizeof (newparent),
			    "%s/%s", parent, shortname);
		else
			(void) snprintf(newparent, sizeof (newparent),
			    "%s", shortname);
...
filedone:
		...
		/*
		 * Because ancillary file precedes the normal file,
	...
	int symflag)	/* true if file is a symbolic link */
{
	...
	if ((checkflag == 1) || (checkflag == 2))
		(void) fchownat(dirfd, name,
		    (uid_t)duid, (gid_t)dgid, symflag);
}
...
tomodes(struct stat *sp)
{
	...
	if ((ulong_t)(gid = sp->st_gid) > (ulong_t)OCTAL7CHAR)
		gid = GID_NOBODY;
	(void) sprintf(dblock.dbuf.gid, "%07lo", (long)gid);
	(void) sprintf(dblock.dbuf.uid, "%07lo", (long)uid);
...
checkupdate(char *arg)
{
	...
	(void) fscanf(tfile, "%" VAL2STR(PATH_MAX) "s" " %ld.%ld",
	    name, &mtime, &nsecs);
	...
	const char		*prefix)
{
	...
	if (Eflag) {
		...
		(void) strcpy(xhdr_buf.dbuf.name, basename((char *)name));
...
gen_num(const char *keyword, const u_longlong_t number)
{
	...
	(void) snprintf(&xrec_ptr[xrec_offset], xrec_size - xrec_offset,
	...
	int		*rlen)
{
	...
	(void) snprintf(hptr->h_version, sizeof (hptr->h_version),
	    "%s", XATTR_ARCH_VERS);
	(void) snprintf(hptr->h_component_len, sizeof (hptr->h_component_len),
	    "%0*d", sizeof (hptr->h_component_len) - 1, complen);
	(void) snprintf(hptr->h_link_component_len,
	    sizeof (hptr->h_link_component_len), "%0*d",
	    sizeof (hptr->h_link_component_len) - 1, linklen);
	(void) snprintf(hptr->h_size, sizeof (hptr->h_size),
	    "%0*d", sizeof (hptr->h_size) - 1, len);
	...
	int typeflag, int filetype, struct linkbuf *lp)
{
	...
	if (asprintf(&sname, "%s.hdr", shortname) == -1)
		fatal(gettext("Out of Memory."));

	if (asprintf(&lname, "/dev/null/%s", sname) == -1)
		fatal(gettext("Out of Memory."));
	...
	free(lname);
	if (asprintf(&lname, "/dev/null/%s", shortname) == -1)
		fatal(gettext("Out of Memory."));
...
add_suffix()
{
	...
	if (strcmp(compress_opt, GZIP) == 0) {
		if ((suffix = gz_suffix()) == NULL) {
			(void) strlcat(tfname, gsuffix[0], sizeof (tfname));
			return (gsuffix[0]);
		}
	} else if (strcmp(compress_opt, COMPRESS) == 0) {
		if ((suffix = gz_suffix()) == NULL) {
			(void) strlcat(tfname, gsuffix[6], sizeof (tfname));
			return (gsuffix[6]);
		}
	} else if (strcmp(compress_opt, BZIP) == 0) {
		if ((suffix = bz_suffix()) == NULL) {
			(void) strlcat(tfname, bsuffix[0], sizeof (tfname));
			return (bsuffix[0]);
		}
	}
	return (NULL);
...
/*
 *      Name: extract_authorized()
 *
 *      Description:
 *              To ensure that only role or a user with Media Restore profile
 *              able to restore/extract
 */
static boolean_t
extract_authorized(void)
{
	struct passwd	*pwd;
	...
	if ((pwd = getpwuid(getuid())) == NULL) {
		fatal(gettext("cannot get user name"));
		return (B_FALSE);
	}
	if (chkauthattr(MEDIA_EXTRACT_AUTH, pwd->pw_name) == 1) {
		return (B_TRUE);
	}

	fatal(gettext("extract not authorized"));
	return (B_FALSE);
}

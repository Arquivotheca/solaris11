 ...
 */
static char *opt_string = ":n:d:i:m:p:rw:o:s:u:t:e:b:d:h:v:Laqlcxgy";
...
get_commandline_args(int argc, char *argv[], pcitool_uiargs_t *parsed_args)
{
	...
	while (((c = getopt(argc, argv, opt_string)) != -1) &&
		...
		case 'i':
			...
			parsed_args->flags |= (INTR_FLAG | SORT_CPU_FLAG);
		...
		/* Size or Interrrupt sort type */
		case 's':

			if (parsed_args->flags & INTR_FLAG) {
				if (strcmp(optarg, "driver") == 0) {
					parsed_args->flags |= SORT_DRV_FLAG;
					parsed_args->flags &= ~ SORT_CPU_FLAG;
				} else if (strcmp(optarg, "cpu") == 0)
					parsed_args->flags |= SORT_CPU_FLAG;
				else {
					(void) fprintf(stderr,
					    "%s: Error in sort argument\n",
					    argv[0]);
					error = B_TRUE;
				}
				break;
		...
		case 'u':
			...
			if (parsed_args->flags & CTO_FLAG) {
				(void) fprintf(stderr, "%s: -u set twice\n",
				    argv[0]);
				error = B_TRUE;
				break;
			}
			parsed_args->flags |= CTO_FLAG;
			if (get_value64(optarg, &recv64, BASE_BY_PREFIX) !=
			    SUCCESS) {
				(void) fprintf(stderr,
				    "%s: Error in completion timeout"
				    "argument\n", argv[0]);
				error = B_TRUE;
				break;
			}

			if (recv64 < 10 || recv64 > 4000) {
				error = B_TRUE;
				(void) fprintf(stderr,
				    "%s: Error in setting completion timeout "
				    "argument, it should be within [10, 4000]"
				    "(10ms ~ 4s)\n", argv[0]);
				break;
			}
			parsed_args->cto = (uint16_t)recv64;
			break;
		/* link equalization */
		case 'L':
			if (!(parsed_args->flags & LEAF_FLAG)) {
			...
			if (parsed_args->flags & EQUA_FLAG) {
				(void) fprintf(stderr, "%s: -L set twice\n",
				    argv[0]);
				error = B_TRUE;
				break;
			}
			parsed_args->flags |= EQUA_FLAG;
			parsed_args->link_speed = 3;
			break;
		...
		case 't':
			...
			if (parsed_args->flags & RETRAIN_FLAG) {
				(void) fprintf(stderr, "%s: -t set twice\n",
				    argv[0]);
				error = B_TRUE;
				break;
			}
			parsed_args->flags |= RETRAIN_FLAG;
			if (get_value64(optarg, &recv64, HEX_ONLY) != SUCCESS) {
				(void) fprintf(stderr,
				    "%s: Error in link retrain argument\n",
				    argv[0]);
				error = B_TRUE;
				break;
			}
			/* only Gen1, Gen2, Gen3 are supported currently */
			if (recv64 < 1 || recv64 > 3) {
				error = B_TRUE;
				(void) fprintf(stderr,
				    "%s: Error in link retrain argument\n",
				    argv[0]);
			...
			parsed_args->link_speed = (uint8_t)recv64;
			break;
		...
		case ':':
			...
			case 'i':
				parsed_args->flags |=
				    (INTR_FLAG | INO_ALL_FLAG);
				break;
			case 'm':
				parsed_args->flags |=
				    (INTR_FLAG | MSI_ALL_FLAG);
				break;
			case 'h':
				parsed_args->flags |= HEX_FLAG;
				break;
			case 'd':
				parsed_args->flags |= DEC_FLAG;
				break;
			default:
				error = B_TRUE;
				break;
	...
	if (!error) {
		...
		if (parsed_args->flags & INTR_FLAG) {
			if (parsed_args->flags &
			    ...
			    HEX_FLAG | DEC_FLAG | SORT_CPU_FLAG |
			    SORT_DRV_FLAG)) {
	...
	if (!(error)) {
		if (!(parsed_args->flags & (READ_FLAG | WRITE_FLAG |
		    RETRAIN_FLAG | EQUA_FLAG | CTO_FLAG))) {
			parsed_args->flags |= READ_FLAG;
		}

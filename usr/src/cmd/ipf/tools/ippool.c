...
#define POOL_ADD_NODE	0
#define	POOL_REM_NODE	1
#define POOL_RESET_NODE	2
#define POOL_STATS_NODE	3
#define POOL_ADD	4
#define POOL_REMOVE	5
...
void	showpools_live(int, int, ip_pool_stat_t *, char *);
...
char *argv[];
{
	...
	switch (getopt(argc, argv, "aAf:FlrRscp"))
	{
	case 'a' :
		err = poolnodecommand(POOL_ADD_NODE, argc, argv);
		break;
	case 'A' :
		err = poolcommand(POOL_ADD, argc, argv);
		break;
	case 'f' :
	...
	case 'l' :
		...
		break;
	case 'r' :
		err = poolnodecommand(POOL_REM_NODE, argc, argv);
		break;
	case 'R' :
		err = poolcommand(POOL_REMOVE, argc, argv);
		break;
	case 's' :
		err = poolstats(argc, argv);
		break;
	case 'c' :
		err = poolnodecommand(POOL_RESET_NODE, argc, argv);
		break;
	case 'p' :
		err = poolnodecommand(POOL_STATS_NODE, argc, argv);
	default :
		exit(1);
	}
...
int poolnodecommand(cmd, argc, argv)
int cmd, argc;
char *argv[];
{
	...
	switch (cmd) {
		...
		case POOL_REM_NODE:
			...
			break;
		case POOL_RESET_NODE:
			err = reset_poolnode(0, poolname, &node, ioctl);
			break;
		case POOL_STATS_NODE:
			err = show_poolnode(0, poolname, &node, ioctl);
			break;
		default:
			err = -1;
	}

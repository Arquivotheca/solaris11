...
static int mdb_sfhmergnmap_isnull(sf_hmeregion_map_t *);
...
sfmmu_vtop_common(struct as *asp, uintptr_t addr, physaddr_t *pap)
{
	...
	struct as *masp = &mas;
	sfmmu_t *hatp, *mhatp;
	void *hatp_inc, *mhatp_inc;
	mdb_ctf_id_t id;
	ssize_t   sfmmu_size;
	ulong_t   cpuset_offset;
	ssize_t   cpuset_size;
	int ret;
	...
	/*
	 * The type definition of sfmmu_cpusran (type cpuset_t) member of
	 * sfmmu_t structure is platform dependent. It is conditionally
	 * defined in usr/src/uts/common/sys/cpuvar.h based on the value
	 * of NCPU for that specific platform. For some non-OPL sun4u
	 * platforms, NCPU is 32 or 64. In this case, cpuset_t is just
	 * one long int. For OPL systems, NCPU is 512. For these systems,
	 * cpuset_t is 8 long ints. But, we are delivering a single
	 * instance of mdb unix module for all sun4u platforms. So, it
	 * is possible that we are using a wrong cpuset_t definition for
	 * sfmmu_cpusran member in the current source file and it may
	 * result in wrong offsets for rest of the members. So, we need
	 * to retrieve the real offset and size from ctf info and do some
	 * manipulation here.
	 */

	/* get the size of sfmmu_t from ctf info */
	if (mdb_ctf_lookup_by_name("sfmmu_t", &id) < 0) {
		SFMMU_VTOP_DBG_DBG("failed to lookup sfmmu_t");
		mdb_warn("failed to look up type %s", "sfmmu_t");
		return (DCMD_ERR);
	}
	sfmmu_size = mdb_ctf_type_size(id);

	/* get the offset and size of sfmmu_cpusran member from ctf info */
	if (mdb_ctf_member_info(id, "sfmmu_cpusran", &cpuset_offset,
	    &id) != 0) {
		mdb_warn("failed to find member %s of type %s", "sfmmu_cpusran",
		    "sfmmu_t");
		return (DCMD_ERR);
	}
	cpuset_size = mdb_ctf_type_size(id);

	if ((mhatp = (sfmmu_t *)mdb_alloc(sfmmu_size,
	    UM_SLEEP|UM_GC)) == NULL) {
	...
	/*
	 * Adjust the pointers to move them past the sfmmu_cpusran member.
	 * We don't require the data in this member for the logic that
	 * follows anyway. So, we skip this member and read only the rest of
	 * the members from sfmmu_t structure.
	 */
	mhatp_inc = (void *)((char *)mhatp +
	    offsetof(sfmmu_t, sfmmu_cpusran) + sizeof (cpuset_t));
	hatp_inc = (void *)((char *)hatp + cpuset_offset + cpuset_size);

	if (mdb_vread(mhatp_inc, sfmmu_size - (cpuset_offset + cpuset_size),
	    (uintptr_t)hatp_inc) == -1) {
	    ...
	    physaddr_t *pap)
{
	...
	if (ret == 0 || mdb_sfhmergnmap_isnull(&(msfmmup->sfmmu_hmeregion_map)))
		return (ret);

	/* Search the shared region hme_blks */

	srdp = msfmmup->sfmmu_srdp;
	SFMMU_VTOP_DBG_DBG("srdp=%p\n", srdp);
	...
	do {
		...
		hmebp = shmebp = HME_HASH_FUNCTION(srdp, addr, hmeshift);
		SFMMU_VTOP_DBG_DBG("hmebp=%p\n", hmebp);
		...
		for (hmeblkp = hmebp->hmeblkp; hmeblkp;
			...
			if (!HTAGS_EQ_SHME(hmeblkp->hblk_tag, hblktag,
			    msfmmup->sfmmu_hmeregion_map)) {
				/* found hme_blk */
				continue;
			}
			if (!hmeblkp->hblk_shared) {
				SFMMU_VTOP_DBG_DBG("hme_blk at %p is not "
				    "a shared hme_blk\n", hmeblkp);
				return (-1);
			}
			...
			if (rid == SFMMU_INVALID_SHMERID ||
				...
				SFMMU_VTOP_DBG_DBG("region id %d is invalid for"
				    " a shared hme_blk\n", rid);
				return (-1);
			}
			...
			sfhmep = mdb_sfmmu_hblktohme(hmeblkp, (caddr_t)addr,
			    &sfhmeinx);
			SFMMU_VTOP_DBG_VRB("sfhmeinx=%d ", sfhmeinx);
...
static int mdb_sfhmergnmap_isnull(sf_hmeregion_map_t *map)
{
	int i;

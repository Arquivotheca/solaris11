...
pt_fpregs(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (has_xregs) {
		for (i = 0; i < 16; i++)
			mdb_printf(
			    "%%ymm%-2d 0x%08x%08x%08x%08x%08x%08x%08x%08x\n",
			    i, xregs.pr_un.pr_xsave.pr_ymm[i]._l[3],
			    xregs.pr_un.pr_xsave.pr_ymm[i]._l[2],
			    xregs.pr_un.pr_xsave.pr_ymm[i]._l[1],
			    xregs.pr_un.pr_xsave.pr_ymm[i]._l[0],
			    fps.xmm[i]._l[3], fps.xmm[i]._l[2],
			    fps.xmm[i]._l[1], fps.xmm[i]._l[0]);
	} else {
		for (i = 0; i < 16; i++)
			mdb_printf("%%xmm%-2d 0x%08x%08x%08x%08x\n", i,
			    fps.xmm[i]._l[3], fps.xmm[i]._l[2],
			    fps.xmm[i]._l[1], fps.xmm[i]._l[0]);
	}

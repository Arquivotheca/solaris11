...
rqueue(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	lower = (uintptr_t)cpu.cpu_m.cpu_rq_va + cpu.cpu_m.cpu_rq_size;
	upper = lower + cpu.cpu_m.cpu_rq_size - Q_ENTRY_SIZE;
...
nrqueue(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	lower = (uintptr_t)cpu.cpu_m.cpu_nrq_va + cpu.cpu_m.cpu_nrq_size;
	upper = lower + cpu.cpu_m.cpu_nrq_size - Q_ENTRY_SIZE;
...
rq_walk_init(mdb_walk_state_t *wsp)
{
	...
	*lower = (uintptr_t)cpu.cpu_m.cpu_rq_va + cpu.cpu_m.cpu_rq_size;
	*upper = (uintptr_t)*lower + cpu.cpu_m.cpu_rq_size - Q_ENTRY_SIZE;
...
nrq_walk_init(mdb_walk_state_t *wsp)
{
	...
	*ao = (uintptr_t)wsp->walk_addr;	/* beginning and end */
	*lower = (uintptr_t)cpu.cpu_m.cpu_nrq_va + cpu.cpu_m.cpu_nrq_size;
	*upper = (uintptr_t)*lower + cpu.cpu_m.cpu_nrq_size - Q_ENTRY_SIZE;

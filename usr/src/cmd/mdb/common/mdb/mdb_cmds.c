...
cmd_dump(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (flags & DCMD_PIPE_OUT) {
		mdb_warn("dump: cannot output to a pipe\n");
		return (DCMD_ERR);
	}
	...
	if (phys)
		error = mdb_dump64(mdb_get_dot(), count, dflags,
		    mdb_partial_pread, NULL);
	else if (file)
		error = mdb_dumpptr(addr, count, dflags,
		    mdb_partial_xread, (void *)mdb_tgt_fread);
	else
		error = mdb_dumpptr(addr, count, dflags,
		    mdb_partial_xread, (void *)mdb_tgt_vread);

	/* If looping, but not from a pipe, we absorb the count by aborting. */
	if (error == -1 || ((flags & (DCMD_LOOP | DCMD_PIPE))) == DCMD_LOOP) {
		return (DCMD_ABORT);
	}
	return (DCMD_OK);
}
...
cmd_head(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (!(flags & DCMD_PIPE)) {
		mdb_warn("head: input must be from pipe.\n");
		return (DCMD_USAGE);
	}
	...
	if (argc == 1 || argc == 2) {
		...
		if (argc == 1) {
			...
			/* Must have at least one digit specified. */
			if (*num == 0) {
				return (DCMD_USAGE);
			}
			...
			if (argv[1].a_type == MDB_TYPE_STRING) {
				...
				num = NULL;
				cnt = argv[1].a_un.a_val;
			} else {
				mdb_warn("head: bad argument to -n\n");
				return (DCMD_USAGE);
			}
		}
		...
		if (num != NULL) {
			...
			if (*c != '\0') {
				mdb_warn("head: bad line count \"%s\"\n", num);
				return (DCMD_USAGE);
			}
		}
...
dump_help(void)
{
	mdb_printf(
	    ...
	    "-n c  Display c bytes of data (defaults to count)\n"
...
const mdb_dcmd_t mdb_dcmd_builtins[] = {
	...
	{ "dump", "?[-eqrstu] [-n count] [-f|-p] [-g bytes] [-w paragraphs]",

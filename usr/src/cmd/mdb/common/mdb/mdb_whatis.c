...
mdb_whatis_unregister_module(mdb_module_t *mod)
{
	...
	/*
	 * If any were removed, compact the array; if the array returns to its
	 * original size then destroy it, allowing it to be recreated if
	 * required.
	 */
	if (found != 0) {
		qsort(whatis_cb, whatis_cb_count, sizeof (*whatis_cb),
		    whatis_cbcmp);
		if ((whatis_cb_count -= found) == NBUILTINS) {
			mdb_free(whatis_cb,
			    sizeof (*whatis_cb) * whatis_cb_size);
			whatis_cb = NULL;
			whatis_cb_count = 0;
		}
	}

...
typedef struct nm_flag_info {
	uint_t		nfi_flag;
	const char	*nfi_name;
} nm_flag_info_t;

typedef struct nm_flags_cbinfo {
	const nm_flag_info_t	*nfc_flags;
	uint_t			*nfc_out;
} nm_flags_cbinfo_t;

/*ARGSUSED*/
static int
nm_flags_cb(char c, char *flag, size_t sz, void *nfc_arg)
{
	const nm_flags_cbinfo_t *const	nfc = nfc_arg;
	const nm_flag_info_t		*nfi;

	for (nfi = nfc->nfc_flags; nfi->nfi_name != NULL; nfi++) {
		if (strcmp(flag, nfi->nfi_name) == 0) {
			(*nfc->nfc_out) |= nfi->nfi_flag;
			return (MDB_LISTCB_OK);
		}
	}
	mdb_warn("invalid option for -%c: \"%s\"\n", c, flag);
	return (MDB_LISTCB_FAIL);
}
...
cmd_nm(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	static const nm_flag_info_t opt_fmt_opts[] = {
		{ NM_FMT_INDEX, "ndx" },
		{ NM_FMT_VALUE, "val" },
		{ NM_FMT_SIZE, "sz" },
		{ NM_FMT_TYPE, "type" },
	...
	static const nm_flag_info_t opt_type_opts[] = {
		{ NM_TYPE_NOTY, "noty" },
		{ NM_TYPE_OBJT, "objt" },
		{ NM_TYPE_FUNC, "func" },
		{ NM_TYPE_SECT, "sect" },
	...
	nm_sym_t *syms, *symp;
	nm_iter_info_t nii;
	nm_flags_cbinfo_t nfc_fmt, nfc_types;

	nfc_fmt.nfc_flags = opt_fmt_opts;
	nfc_fmt.nfc_out = &opt_fmt;

	nfc_types.nfc_flags = opt_type_opts;
	nfc_types.nfc_out = &opt_types;
	...
	i = mdb_getopts(argc, argv,
	    ...
	    'f', MDB_OPT_LISTCB, nm_flags_cb, &nfc_fmt,
	    't', MDB_OPT_LISTCB, nm_flags_cb, &nfc_types,
	    NULL);
	...
	if (opt_fmt == 0) {
		opt_fmt = NM_FMT_VALUE | NM_FMT_SIZE | NM_FMT_TYPE |
		    NM_FMT_BIND | NM_FMT_OTHER | NM_FMT_SHNDX | NM_FMT_NAME;
	}

	/* default output types */
	if (opt_types == 0) {
		opt_types = NM_TYPE_NOTY | NM_TYPE_OBJT | NM_TYPE_FUNC |
		    NM_TYPE_SECT | NM_TYPE_FILE | NM_TYPE_COMM | NM_TYPE_TLS |
		    NM_TYPE_REGI;
	}

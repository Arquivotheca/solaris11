...
static int
argvec_process_listcb(const mdb_opt_t *opt, const mdb_arg_t *arg)
{
	const char *start;
	const char *next;
	char buf[128];
	...
	for (start = arg->a_un.a_str; start != NULL; start = next) {
		next = strchr(start, ',');

		if (next == NULL) {
			len = strlen(start);	/* the rest of the string */
		} else {
			len = next - start;
			next++;			/* skip past the comma */
		}
		if (len + 1 > sizeof (buf)) {
			(void) mdb_snprintf(buf, 32, "%s", start);
			(void) strlcat(buf, "...", sizeof (buf));

			warn("option too long for -%c: \"%s\"\n",
			    opt->opt_char, buf);
			return (-1);
		}
		(void) strlcpy(buf, start, len + 1);

		if ((*opt->opt_listcb)(opt->opt_char, buf, len + 1,
		    opt->opt_valp) != MDB_LISTCB_OK)
			return (-1);
	}
	return (0);
}
...
argvec_process_opt(const mdb_opt_t *opt, const mdb_arg_t *arg)
{
	...
	case MDB_OPT_LISTCB:
		...
		return (argvec_process_listcb(opt, arg));
	default:
	...
	for (i = 0; i < (sizeof (opts) / sizeof (opts[0]) - 1); i++, op++) {
		...
		if (op->opt_type == MDB_OPT_SETBITS ||
			...
			for (sop = op->opt_subopts; sop->sop_flag; sop++)
				sop->sop_index = -1;
		} else if (op->opt_type == MDB_OPT_LISTCB) {
			op->opt_listcb = va_arg(alist, mdb_opt_listcb_f *);
		}

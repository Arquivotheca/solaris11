...
/*
 * Symbol and device names from mem.c. Order is important
 * and mirrors that of logging index number (L_MEM etc)
 * in mem.h.
 */
static const char *log_symnm[LOG_DEVS] = {
	"mm_mem_log",
	"mm_kmem_log",
	"mm_allkmemr_log",
	"mm_allkmemw_log",
};
static const char *log_devnm[LOG_DEVS] = {
	"/dev/mem",
	"/dev/kmem",
	"/dev/allkmem",
	"/dev/allkmem",
...
kt_status_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	mem_log_t log_head[LOG_DEVS] = { 0 };
	const char *cmd_nm[LOG_DEVS] = {
		"memstatus",
		"kmemstatus",
		"allkmemstatus",
		"allkmemstatus",
	...
	/* determine if memory devices have been written to or allkmem read */
	for (log_select = L_MEM; log_select < LOG_DEVS; log_select++) {
		if (mdb_tgt_readsym(mdb.m_target, MDB_TGT_AS_VIRT,
		    &log_head[log_select], sizeof (log_head) / LOG_DEVS,
		    MDB_TGT_OBJ_EXEC, log_symnm[log_select]) < 0) {
			mdb_printf("Cannot determine if %s was accessed.\n",
			    log_devnm[log_select]);
		}
	}
	for (log_select = L_MEM; log_select < LOG_DEVS; log_select++) {
		if (log_head[log_select].m_who != NULL) {
			mdb_printf("WARNING: %s %s, use ::%s to "
			    "view log.\n", log_devnm[log_select],
			    log_select != L_ALLKMEMR ?
...
#define	TITLES	\
"THREAD           HRTIME           RW S ADDRESS          SIZE             SYM\n"
#define	TITLES1	\
"THREAD           HRTIME           RW S ADDRESS          SIZE\n"
#define	FMT	"%-16lx %-16llx %c  %c %-16lx %-16lx %s\n"

static void
kt_log_print(mem_log_t *logp, uint_t start, uint_t end, uint_t log_id)
{
	uint_t count;
	char sym_nm[MDB_TGT_MAPSZ];
	GElf_Sym sym;
	mdb_syminfo_t info;

	for (count = start; count < end; count++) {
		/* symbol lookups are meaningless on /dev/mem */
		if (log_id == L_MEM || kt_lookup_by_addr(mdb.m_target,
		    logp[count].m_where, MDB_TGT_SYM_FUZZY,
		    sym_nm, MDB_TGT_MAPSZ, &sym, &info) == -1) {
			/* just to be sure */
			sym_nm[0] = '\0';
		}
		mdb_printf(FMT,
		    logp[count].m_who,
		    logp[count].m_when,
		    logp[count].m_what == 0 ? 'r' : 'w',
		    logp[count].m_success ? 'y' : 'n',
		    logp[count].m_where,
		    logp[count].m_sz,
		    sym_nm);
	}
...
static int
kt_log_iter(mem_log_t *logp, uint_t log_id)
{
	char *hdgs = NULL;
	uint_t tail = 0;
	u_longlong_t cmltv = 0;
	uint_t log_tail[LOG_DEVS] = { 0 };
	u_longlong_t log_cmltv[LOG_DEVS] = { 0 };

	if (mdb_tgt_readsym(mdb.m_target, MDB_TGT_AS_VIRT, &log_tail,
	    sizeof (log_tail), MDB_TGT_OBJ_EXEC, "mm_log_tail") < 0) {
		mdb_printf("Cannot find log tail.\n");
		return (DCMD_ERR);
	}

	if (mdb_tgt_readsym(mdb.m_target, MDB_TGT_AS_VIRT, &log_cmltv,
	    sizeof (log_cmltv), MDB_TGT_OBJ_EXEC, "mm_log_cmltv") < 0) {
		mdb_printf("Cannot find log head.\n");
		return (DCMD_ERR);
	}

	tail = log_tail[log_id];
	cmltv = log_cmltv[log_id];
	...
	if (cmltv) {
		mdb_printf(hdgs);
		if (cmltv >= LOG_ENTS) {
			kt_log_print(logp, tail, LOG_ENTS, log_id);
		}
		if (tail > 0) {
			kt_log_print(logp, 0, tail, log_id);
		}
		mdb_printf("Total events recorded: %llu\n", cmltv);
	}
...
static int
kt_openlog(uint_t log_select)
{
	mem_log_t log[LOG_ENTS] = { 0 };

	if (log_select >= LOG_DEVS) {
		return (DCMD_ERR);
	}

	if (mdb_tgt_readsym(mdb.m_target, MDB_TGT_AS_VIRT, &log,
	    sizeof (log), MDB_TGT_OBJ_EXEC, log_symnm[log_select]) < 0) {
		mdb_printf("Cannot find log head.\n");
		return (DCMD_ERR);
	}

	return (kt_log_iter(log, log_select));
}

/*ARGSUSED*/
static int
kt_memstatus_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	return (kt_openlog(L_MEM));
}

/*ARGSUSED*/
static int
kt_kmemstatus_dcmd(uintptr_t addr, uint_t flags, int argc,
    const mdb_arg_t *argv)
{
	return (kt_openlog(L_KMEM));
}

/*ARGSUSED*/
static int
kt_allkmemstatus_dcmd(uintptr_t addr, uint_t flags, int argc,
    const mdb_arg_t *argv)
{
	if (kt_openlog(L_ALLKMEMR) == DCMD_ERR) {
		return (DCMD_ERR);
	}
	return (kt_openlog(L_ALLKMEMW));
}
...
static const mdb_dcmd_t kt_dcmds[] = {
	...
	{ "memstatus", NULL, "print /dev/mem change log", kt_memstatus_dcmd },
	{ "kmemstatus", NULL, "print /dev/kmem change log",
	    kt_kmemstatus_dcmd },
	{ "allkmemstatus", NULL, "print /dev/allkmem change log",
	    kt_allkmemstatus_dcmd },
	{ NULL }
};

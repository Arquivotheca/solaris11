...
topo_handle(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	char server[128], psn[128], csn[128];
	topo_hdl_t th;
	...
	if (mdb_readstr(server, sizeof (server), (uintptr_t)th.th_server)
	    < 0) {
		(void) mdb_snprintf(server, sizeof (server), "<%p>",
		    th.th_server);
	}
	if (mdb_readstr(psn, sizeof (psn), (uintptr_t)th.th_psn)
	    < 0) {
		(void) mdb_snprintf(psn, sizeof (psn), "<%p>", th.th_psn);
	}
	if (mdb_readstr(csn, sizeof (csn), (uintptr_t)th.th_csn)
	    < 0) {
		(void) mdb_snprintf(csn, sizeof (csn), "<%p>", th.th_csn);
	}
	...
	mdb_printf("%-12s %-36d %-30s\n", "tm_dbout", th.th_dbout,
	    "Debug channel");
	mdb_printf("%-12s 0x%-34p %-30s\n", "th_cromk_hdl", th.th_cromk_hdl,
	    "Handle to di_cromk_init");
	mdb_printf("%-12s %-36d %-30s\n", "tm_dbout", th.th_dbout,
	    "Debug channel");

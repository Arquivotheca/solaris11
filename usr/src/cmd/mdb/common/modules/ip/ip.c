...
typedef struct {
	uint_t bit_value;	/* bit flag value */
	const char *bit_name;	/* name of bit */
	const char *bit_descr;	/* description of bit's purpose */
} bitname_t;

static const bitname_t squeue_states[] = {
	{ SF(SQS_PROC),			"being processed" },
	{ SF(SQS_WORKER),		"... by a worker thread" },
	{ SF(SQS_ENTER),		"... by an squeue_enter() thread" },
	{ SF(SQS_FAST),			"... in fast-path mode" },

	{ SF(SQS_USER), 		"a non interrupt user" },
	{ SF(SQS_BOUND),		"worker thread bound to CPU" },
	{ SF(SQS_REENTER),		"re-entered thread" },
	{ SF(SQS_TMO_PROG),		"timeout is being set" },

	{ SF(SQS_POLL_CAPAB),		"squeue can control interrupts" },
	{ SF(SQS_ILL_BOUND),		"squeue bound to an ill" },
	{ SF(SQS_GET_PKTS),		"moving pkts from NIC in progress" },
	{ SF(SQS_DEFAULT),		"the default squeue for the CPU" },

	{ SF(SQS_POLLING),		"squeue in polling mode" },
	{ SF(SQS_INTR_BLANK),		"interrupt blanking capability" },
	{ SF(SQS_PROC_HELD),		"SQS_PROC is held by the caller" },
	{ SF(SQS_FORCE_TIMER),		"schedule worker due to B/W control" },

	{ SF(SQS_POLL_CLEANUP),		"disassociating from its rx_ring" },
	{ SF(SQS_POLL_CLEANUP_DONE),	"disassociation done" },
	{ SF(SQS_POLL_QUIESCE),		"stopping squeue from polling" },
	{ SF(SQS_POLL_QUIESCE_DONE),	"polling stop done" },

	{ SF(SQS_POLL_RESTART),		"restart poll thread" },
	{ SF(SQS_POLL_THR_QUIESCED),	"poll thread was quiesced" },
	{ SF(SQS_POLL_THR_RESTART),	"restart the poll thread" },
	{ SF(SQS_POLL_PROC),		"poll thread processing the sq" },

	{ SF(SQS_POLL_RESTART_DONE),	"poll thread restart done" },
	{ SF(SQS_POLL_THR_QUIESCE),	"quiesce the poll thread" },
	{ SF(SQS_PAUSE),		"the squeue has been paused" },
...
static int ipaddr(uintptr_t, uint_t, int, const mdb_arg_t *);
...
static int illif(uintptr_t, uint_t, int, const mdb_arg_t *);
static void illif_help(void);
static int illif_stack_walk_init(mdb_walk_state_t *);
static void illif_stack_walk_fini(mdb_walk_state_t *);
static int illif_stack_walk_step(mdb_walk_state_t *);
static int illif_walk_step(mdb_walk_state_t *);
...
static void ip_squeue_help(void);
static int ip_stacks_common_walk_init(mdb_walk_state_t *);
static int ip_stacks_walk_step(mdb_walk_state_t *);

static int dce(uintptr_t, uint_t, int, const mdb_arg_t *);
static int dce_walk_init(mdb_walk_state_t *);
static int dce_walk_step(mdb_walk_state_t *);

static int ire(uintptr_t, uint_t, int, const mdb_arg_t *);
static int ire_format(uintptr_t addr, const void *, void *);
static int ire_walk_init(mdb_walk_state_t *);
static int ire_walk_step(mdb_walk_state_t *);
static int ire_next_walk_init(mdb_walk_state_t *);
static int ire_next_walk_step(mdb_walk_state_t *);

static int nce(uintptr_t, uint_t, int, const mdb_arg_t *);
static char *nce_l2_addr(const nce_t *, const ill_t *);
static int nce_walk_init(mdb_walk_state_t *);
static int nce_walk_step(mdb_walk_state_t *);
...
static int ns_walk_init(mdb_walk_state_t *);
...
static int squeue(uintptr_t, uint_t, int, const mdb_arg_t *);

static int th_trace(uintptr_t, uint_t, int, const mdb_arg_t *);
static void th_trace_help(void);
static int th_hash_walk_init(mdb_walk_state_t *);
static int th_hash_walk_step(mdb_walk_state_t *);
...
static const mdb_dcmd_t dcmds[] = {
	...
	{ "ipaddr", "?[-d] [-P v4 | v6]", "display an IP address", ipaddr,
	    ipaddr_help },
...
static const mdb_walker_t walkers[] = {
	...
	{ "sctp_listen_fanout", "walk the sctp listen fanout for all stacks",
	    sctp_fanout_walk_init, sctp_fanout_walk_step, NULL,
	    (void *)&sctp_listen_fanout_init },
	{ "sctp_conn_fanout", "walk the sctp conn fanout for all stacks",
	    sctp_fanout_walk_init, sctp_fanout_walk_step, NULL,
	    (void *)&sctp_conn_fanout_init },
	{ "sctp_bind_fanout", "walk the sctp bind fanout for all stacks",
	    sctp_fanout_walk_init, sctp_fanout_walk_step, NULL,
	    (void *)&sctp_bind_fanout_init },
	{ "sctp_stack_listen_fanout",
	    "walk the sctp listen fanout for one stack",
	    sctp_fanout_stack_walk_init, sctp_fanout_stack_walk_step,
	    sctp_fanout_stack_walk_fini,
	    (void *)&sctp_listen_fanout_init },
...
static int
transport_hdr(int proto, uintptr_t addr, uintptr_t end)
{
	mdb_printf("\n");
	switch (proto) {
	case IPPROTO_TCP: {
	...
	case IPPROTO_SCTP: {
		...
		sctphdr_print(&sctph);
		sctp_chunk_print(addr + sizeof (sctph), end);
		break;
	}
	default:
		break;
...
ip6hdr(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	class = IPV6_FLOW_TCLASS(iph->ip6_vcf);
	flow = IPV6_FLOW_FLOWLABEL(iph->ip6_vcf);
...
/* ARGSUSED */
int
ipaddr(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	ipaddr_t ipaddr;
	in6_addr_t ip6addr;
	int ipversion = IPV4_VERSION;
	const char *opt_P = NULL;
	uint_t opt_d = FALSE;
	...
	if (opt_P != NULL) {
		...
		if (opt_d && (ipversion == IPV6_VERSION)) {
			mdb_warn("IPv6 address cannot be passed by value\n");
			return (DCMD_USAGE);
		}
	}
	...
	switch (ipversion) {
	case IPV6_VERSION:
		if (mdb_vread(&ip6addr, sizeof (in6_addr_t), addr) == -1) {
			mdb_warn("failed to read IPv6 address at %p", addr);
			return (DCMD_ERR);
		}
		mdb_printf("%N\n", &ip6addr);
		break;
	case IPV4_VERSION:
		if (opt_d) {
			ipaddr = (ipaddr_t)addr;
		} else {
			if (mdb_vread(&ipaddr, sizeof (ipaddr),
			    addr) == -1) {
				mdb_warn("failed to read IPv4 address at %p",
				    addr);
				return (DCMD_ERR);
			}
		}
...
ipaddr_help(void)
{
	mdb_printf("Prints IP address in a readable form.\n\n");
	mdb_printf("Options:\n");
	mdb_printf("\t-P v4 | v6"
	    "\tread and print IP address of the specified protocol\n");
	mdb_printf("\t-d"
	    "\t\tpass IPv4 address by value\n");
}
...
squeue(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	for (i = 0; squeue_states[i].bit_name != NULL; i++) {
		if (((squeue.sq_state) & (squeue_states[i].bit_value)) == 0)
			continue;
...
mask_to_prefixlen(int af, const in6_addr_t *addr)
{
	...
	if (mask > 0)
		len += (33 - mdb_ffs(ip_ntoh_32(mask)));
	return (len);
}
    ...
    void *private)
{
	...
	if (iw != NULL) {
		/* Use conn buffer from walk init */
		conn = iw->conn;
	} else {
		/* Allocate private conn buffer */
		conn = mdb_alloc(sizeof (conn_t), UM_SLEEP);
		allocated = TRUE;
	}

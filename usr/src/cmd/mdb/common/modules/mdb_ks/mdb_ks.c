...
/*
 * Static copies of nsec_shift, nsec_scale and nsec_unscale from the global
 * 'hrt' structure.
 */
static int mdb_init_hrtime(void);
static uint_t mdb_nsec_shift = 0;
static uint_t mdb_nsec_scale = 0;
static uint_t mdb_nsec_unscale = 0;
...
/*
 * Initialize the internal hrtime variables.
 */
static int
mdb_init_hrtime(void)
{
	ulong_t hrt_shift_off, hrt_scale_off, hrt_unscale_off;
	mdb_ctf_id_t hrt_type_id;
	GElf_Sym sym;
	uint64_t hrt;

	/*
	 * We can return right away if all the internal MDB hrtime fields have
	 * already been initialized.
	 */
	if (mdb_nsec_shift != 0 && mdb_nsec_scale != 0 && mdb_nsec_unscale != 0)
		return (DCMD_OK);

	/*
	 * Find the structure offsets of the nsec_shift, nsec_scale and
	 * nsec_unscale fields of the 'hrt' structure (whose definition is not
	 * visible to the kernel support module).
	 */
	if (mdb_ctf_lookup_by_name("hrt_t", &hrt_type_id) == -1) {
		mdb_warn("Couldn't find hrt_t type.");
		return (DCMD_ERR);
	}

	if (mdb_ctf_offsetof(hrt_type_id, "nsec_shift", &hrt_shift_off) != 0) {
		mdb_warn("Couldn't find field 'nsec_shift' in type 'hrt_t'.");
		return (DCMD_ERR);
	}

	hrt_shift_off /= NBBY;

	if (mdb_ctf_offsetof(hrt_type_id, "nsec_scale", &hrt_scale_off) != 0) {
		mdb_warn("Couldn't find field 'nsec_scale' in type 'hrt_t'.");
		return (DCMD_ERR);
	}

	hrt_scale_off /= NBBY;

	if (mdb_ctf_offsetof(hrt_type_id, "nsec_unscale",
	    &hrt_unscale_off) != 0) {
		mdb_warn("Couldn't find field 'nsec_unscale' in type 'hrt_t'.");
		return (DCMD_ERR);
	}

	hrt_unscale_off /= NBBY;

	/*
	 * Read the variable 'hrt', which is a pointer to that structure.
	 */
	if (mdb_lookup_by_name("hrt", &sym) == -1) {
		mdb_warn("Could not find variable 'hrt'.");
		return (DCMD_ERR);
	}

	if (mdb_vread(&hrt, sizeof (hrt), sym.st_value) == -1) {
		mdb_warn("Could not read 'hrt' pointer.");
		return (DCMD_ERR);
	}

	/*
	 * Read the following three fields from the 'hrt' structure using
	 * their offset within the structure plus the base address.
	 */
	if (mdb_vread(&mdb_nsec_shift, sizeof (uint_t),
	    hrt + hrt_shift_off) == -1) {
		mdb_warn("Could not read 'hrt->nsec_shift' at 0x%p\n.",
		    (void *)(hrt + hrt_shift_off));
		return (DCMD_ERR);
	}

	if (mdb_vread(&mdb_nsec_scale, sizeof (uint_t),
	    hrt + hrt_scale_off) == -1) {
		mdb_warn("Could not read 'hrt->nsec_scale' at 0x%p\n.",
		    (void *)(hrt + hrt_scale_off));
		return (DCMD_ERR);
	}

	if (mdb_vread(&mdb_nsec_unscale, sizeof (uint_t),
	    hrt + hrt_unscale_off) == -1) {
		mdb_warn("Could not read 'hrt->nsec_unscale' at 0x%p\n.",
		    (void *)(hrt + hrt_unscale_off));
		return (DCMD_ERR);
	}
...
mdb_scalehrtime(hrtime_t *tsc)
{
	...
	if (mdb_init_hrtime() != DCMD_OK) {
		mdb_warn("Failed to initialize internal hrtime variables.");
		return (DCMD_ERR);
	}
	...
	/*
	 * Convert tsc to nanoseconds, 32 bits at a time (mind the endianness).
	 */
	h = (uint64_t)BMASK_32(mytsc >> 32);
	l = (uint64_t)BMASK_32(mytsc);

	ns = (h * ((uint64_t)mdb_nsec_scale)) << mdb_nsec_shift;
	ns += (l * ((uint64_t)mdb_nsec_scale)) >> (32 - mdb_nsec_shift);
...
mdb_unscalehrtime(hrtime_t *ns)
{
	...
	/*
	 * Convert ns to native (unscaled) time, 32 bits at a time (mind
	 * the endianness).
	 */
	h = (uint64_t)BMASK_32(myns >> 32);
	l = (uint64_t)BMASK_32(myns);

	unsc = (h * ((uint64_t)mdb_nsec_unscale)) << mdb_nsec_shift;
	unsc += (l * ((uint64_t)mdb_nsec_unscale)) >> (32 - mdb_nsec_shift);

...
#define	SMB_OPT_AUTH		0x00000080
...
static void smb_dcmd_server_help(void);
...
static int smb_share(uintptr_t, uint_t, int, const mdb_arg_t *);
static int smb_share_walk_init(mdb_walk_state_t *);
static int smb_share_walk_step(mdb_walk_state_t *);
...
static const mdb_walker_t walkers[] = {
	...
	{   "smbace_walker",
	    ...
	    NULL },
	{   "smbshare_walker",
	    "walk an AVL tree of smb_share_t structures",
	    smb_share_walk_init,
	    smb_share_walk_step,
	    NULL,
	    NULL },
...
smb_dcmd_list_help(void)
{
	...
	mdb_printf(
	    ...
	    "-a\tDisplay the list of authenticating users\n"
...
static void
smb_dcmd_server_help(void)
{
	mdb_printf(
	    "Display the contents of smb_server_t.\n\n");
	(void) mdb_dec_indent(2);
	mdb_printf("%<b>OPTIONS%</b>\n");
	(void) mdb_inc_indent(2);
	mdb_printf(
...
static const smb_exp_t smb_session_exp[] =
{
	{ SMB_OPT_REQUEST,
	    ...
	    "smbreq", "smb_request"},
	{ SMB_OPT_AUTH,
	    offsetof(smb_session_t, s_auth_list.ll_list),
	    "smbuser", "smb_user"},
...
{
	mdb_printf(
	    "Display the contents of smb_session_t.\n\n");
	(void) mdb_dec_indent(2);
	mdb_printf("%<b>OPTIONS%</b>\n");
	(void) mdb_inc_indent(2);
	mdb_printf(
	...
	if (((opts & SMB_OPT_WALK) && (opts & SMB_OPT_SESSION)) ||
		...
		if (opts & SMB_OPT_VERBOSE) {
			...
			mdb_printf("Session state: %u (%s)\n", se->s_state,
			    state);
			mdb_printf("Number of Auths: %u\n",
			    se->s_auth_list.ll_count);
	...
	if (((opts & SMB_OPT_WALK) && (opts & SMB_OPT_REQUEST)) ||
		...
		if (opts & SMB_OPT_VERBOSE) {
			...
			mdb_printf(
			    ...
			    "running time: %lld\n",
			    sr->sr_command,
			    smb_com[sr->sr_command].smb_com,
			    sr->sr_com_current,
			    smb_com[sr->sr_com_current].smb_com,
			    sr->sr_state, state,
			    sr->sr_tid, sr->tid_tree,
			    sr->sr_uid, sr->uid_user,
...
{
	mdb_printf(
	    "Display the contents of smb_user_t.\n\n");
	(void) mdb_dec_indent(2);
	mdb_printf("%<b>OPTIONS%</b>\n");
	(void) mdb_inc_indent(2);
	mdb_printf(
	    "-v\tDisplay verbose smb_user information\n"
	    "-a\tDisplay authenticating user\n"
	    "-u\tDisplay user (default)\n"
...
smb_dcmd_user(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (((opts & SMB_OPT_WALK) && (opts & (SMB_OPT_USER | SMB_OPT_AUTH))) ||
		...
		if (user->u_state != SMB_USER_STATE_AUTHENTICATING) {
			len = user->u_domain_len + user->u_name_len + 2;
			account = mdb_zalloc(len, UM_SLEEP | UM_GC);
		...
		if (opts & SMB_OPT_VERBOSE) {
			...
			if (DCMD_HDRSPEC(flags))
				mdb_printf(
				    ...
				    "%-32s "
				    "%-16s%</u>%</b>\n",
				    "USER", "UID", "ACCOUNT", "STATE");

			mdb_printf("%-?p %-5u %-32s %-16s\n", addr, user->u_uid,
			    account, state);
		}
	}
...
/*
 * Initialize the smb_vfs_t walker by reading the value of smb_shrmgr
...
smb_vfs_walk_init(mdb_walk_state_t *wsp)
{
	...
	if (mdb_lookup_by_name("smb_shrmgr", &sym) == -1) {
		mdb_warn("failed to find 'smb_shrmgr'");
		return (WALK_ERR);
	}

	wsp->walk_addr = (uintptr_t)sym.st_value +
	    offsetof(smb_shrmgr_t, sm_vfs_list) +
...
{
	mdb_printf(
	    "Display the contents of smb_node_t.\n\n");
	(void) mdb_dec_indent(2);
	mdb_printf("%<b>OPTIONS%</b>\n");
	(void) mdb_inc_indent(2);
	mdb_printf(
	...
	if (DCMD_HDRSPEC(flags)) {
		if (verbose) {
			...
			mdb_printf(
			    ...
			    "%-8s "
			    "%-6s%</u>%</b>\n",
			    "NODE", "VP", "NODE-NAME", "OFILES", "LOCKS",
			    "OPLOCK", "REF");
		}
	}
 ...
 * ::smbshare
 *
 * smbshare dcmd - Prints out smb_share_t structures.
 */
/*ARGSUSED*/
static int
smb_share(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	static char	*shrtype[] = { "Disk", "Print Queue", "Device", "IPC" };
	int		verbose = FALSE;
	smb_share_t	kshr;
	char		*name;
	char		*path;
	...
	/*
	 * If no smb_share address was specified on the command line, we can
	 * print out all smb_share structures by invoking the smb_share walker,
	 * using this dcmd itself as the callback.
	 */
	if (!(flags & DCMD_ADDRSPEC)) {
		if (mdb_walk_dcmd("smbshare_walker", "smbshare",
		    argc, argv) == -1) {
			mdb_warn("failed to walk 'smb_share_t'");
			return (DCMD_ERR);
		}
		return (DCMD_OK);
	}

	if (mdb_vread(&kshr, sizeof (kshr), addr) == -1) {
		mdb_warn("failed to read smb_share_t at %p", addr);
		return (DCMD_ERR);
	}

	name = mdb_zalloc(MAXNAMELEN, UM_SLEEP | UM_GC);
	(void) mdb_vread(name, MAXNAMELEN, (uintptr_t)kshr.shr_name);

	path = mdb_zalloc(MAXPATHLEN, UM_SLEEP | UM_GC);
	(void) mdb_vread(path, MAXPATHLEN, (uintptr_t)kshr.shr_path);

	if (verbose) {
		mdb_printf("  Name: %s\n", name);
		mdb_printf("  Path: %s\n", path);
		mdb_printf("  ID: %u\n", kshr.shr_kid);
		mdb_printf("  Type: %s (0x%08x)\n",
		    shrtype[kshr.shr_type & STYPE_MASK], kshr.shr_type);
		mdb_printf("  Flags: 0x%08x\n", kshr.shr_flags);
		mdb_printf("  Reference Count: %d\n", kshr.shr_refcnt);
		if (kshr.shr_winpath) {
			path = mdb_zalloc(MAXPATHLEN, UM_SLEEP | UM_GC);
			(void) mdb_vread(path, MAXPATHLEN,
			    (uintptr_t)kshr.shr_winpath);
			mdb_printf("  Windows Path: %s\n", path);
		}

		if (kshr.shr_oemname) {
			name = mdb_zalloc(16, UM_SLEEP | UM_GC);
			(void) mdb_vread(name, 16,
			    (uintptr_t)kshr.shr_oemname);
			mdb_printf("  OEM Name: %s\n", name);
		}

		if (kshr.shr_cmnt) {
			item = mdb_zalloc(MAXNAMELEN, UM_SLEEP | UM_GC);
			(void) mdb_vread(item, MAXNAMELEN,
			    (uintptr_t)kshr.shr_cmnt);
			mdb_printf("  Comment: %s\n", item);
		}

		if (kshr.shr_container) {
			item = mdb_zalloc(MAXPATHLEN, UM_SLEEP | UM_GC);
			(void) mdb_vread(item, MAXPATHLEN,
			    (uintptr_t)kshr.shr_container);
			mdb_printf("  AD Container: %s\n", item);
		}

		if (kshr.shr_access_none) {
			item = mdb_zalloc(MAXPATHLEN, UM_SLEEP | UM_GC);
			(void) mdb_vread(item, MAXPATHLEN,
			    (uintptr_t)kshr.shr_access_none);
			mdb_printf("  None Host ACL: %s\n", item);
		}

		if (kshr.shr_access_ro) {
			item = mdb_zalloc(MAXPATHLEN, UM_SLEEP | UM_GC);
			(void) mdb_vread(item, MAXPATHLEN,
			    (uintptr_t)kshr.shr_access_ro);
			mdb_printf("  Read-only Host ACL: %s\n", item);
		}

		if (kshr.shr_access_rw) {
			item = mdb_zalloc(MAXPATHLEN, UM_SLEEP | UM_GC);
			(void) mdb_vread(item, MAXPATHLEN,
			    (uintptr_t)kshr.shr_access_rw);
			mdb_printf("  Read/write Host ACL: %s\n", item);
		}

		if (kshr.shr_flags & SMB_SHRF_AUTOHOME) {
			mdb_printf("  Autohome Count: %d\n", kshr.shr_autocnt);
			mdb_printf("  UID: %d\n", kshr.shr_uid);
			mdb_printf("  GID: %d\n", kshr.shr_gid);
		}
		...
		if (DCMD_HDRSPEC(flags)) {
			mdb_printf(
			    ...
			    "%-s"
			    "%</u>%</b>\n",
			    "SMB_SHARE", "NAME", "PATH", "TYPE", "FLAGS");
		}

		mdb_printf(
		    "%-?p %-10s %-16s %-8s 0x%08x\n", addr, name, path,
		    shrtype[kshr.shr_type & STYPE_MASK], kshr.shr_flags);
	}
 ...
 */
static int
smb_share_walk_init(mdb_walk_state_t *wsp)
{
	smb_shrmgr_t	*smb_shrmgr;
	GElf_Sym	sym;

	if (wsp->walk_addr != NULL) {
		mdb_warn("smb_share_t walk only supports global walks\n");
		return (WALK_ERR);
	}
	...
	smb_shrmgr = mdb_alloc(sizeof (smb_shrmgr_t), UM_SLEEP);
	if (mdb_vread(smb_shrmgr, sizeof (smb_shrmgr_t),
	    (uintptr_t)sym.st_value) == -1) {
		mdb_free(smb_shrmgr, sizeof (smb_shrmgr_t));
		mdb_warn("failed to read smb_shrmgr at %p", sym.st_value);
		return (DCMD_ERR);
	}

	wsp->walk_addr = (uintptr_t)smb_shrmgr->sm_share_avl +
	    offsetof(smb_avl_t, avl_tree);
...
static int
smb_share_walk_step(mdb_walk_state_t *wsp)
{
	smb_share_t kshr;

	if (mdb_vread(&kshr, sizeof (smb_share_t), wsp->walk_addr) == -1) {
		mdb_warn("failed to read smb_share_t at %p", wsp->walk_addr);
		return (WALK_ERR);
	}

	return (wsp->walk_callback(wsp->walk_addr, &kshr, wsp->walk_cbdata));
}
...
smb_dcmd_getopt(uint_t *opts, int argc, const mdb_arg_t *argv)
{
	*opts = 0;
	...
	if (mdb_getopts(argc, argv,
	    ...
	    'a', MDB_OPT_SETBITS, SMB_OPT_AUTH, opts,

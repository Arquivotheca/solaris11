...
#define	SPACER()				\
	...
	if (oflags & TF_DISP) {
		SPACER();
		disp_time = t.t_disp_time;

		if (mdb_scalehrtime(&disp_time) != 0) {
			mdb_warn("Failed to scale t_disp_time");
			return (DCMD_ERR);
		}
		...
		mdb_printf(" %?lx %5d %2d",
		    (clock_t)((mdb_gethrtime() - disp_time)/nsec_per_tick),
		    t.t_bind_cpu, t.t_preempt);
	}

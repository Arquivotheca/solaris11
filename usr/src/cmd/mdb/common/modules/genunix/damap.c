...
damap_free(struct dam *dam, void **kdamda, int kdamda_n)
{
	...
	if (dam) {
		...
		/*
		 * free dam_active/config/unconfig/stable/report/transport_set
		 * embedded in dam.
		 */
		bitset_free(&dam->dam_active_set, 1);
		bitset_free(&dam->dam_cfg_set, 1);
		bitset_free(&dam->dam_uncfg_set, 1);
		bitset_free(&dam->dam_stable_set, 1);
		bitset_free(&dam->dam_report_set, 1);
		bitset_free(&dam->dam_xpterr_set, 1);
...
damap_get(uintptr_t damaddr, void ***pkdamda, int *pkdamda_n)
{
	...
	*dam = kdam;
	...
	dam->dam_cfg_set.bs_set = NULL;
	dam->dam_uncfg_set.bs_set = NULL;
	dam->dam_stable_set.bs_set = NULL;
	dam->dam_report_set.bs_set = NULL;
	dam->dam_xpterr_set.bs_set = NULL;
	...
	/*
	 * fill in dam_active/config/unconfig/stable/report/transport_set
	 * embedded in the dam.
	 */
	...
	bs = bitset_get(damaddr + (offsetof(struct dam, dam_cfg_set)));
	if (bs) {
		dam->dam_cfg_set = *bs;
		mdb_free(bs, sizeof (*bs));
	}
	bs = bitset_get(damaddr + (offsetof(struct dam, dam_uncfg_set)));
	if (bs) {
		dam->dam_uncfg_set = *bs;
		mdb_free(bs, sizeof (*bs));
	}
	...
	bs = bitset_get(damaddr + (offsetof(struct dam, dam_xpterr_set)));
	if (bs) {
		dam->dam_xpterr_set = *bs;
		mdb_free(bs, sizeof (*bs));
	}
...
/*ARGSUSED*/
static void
damap_print(struct dam *dam, void **kdamda, int kdamda_n, uint_t verbose)
{
	struct i_ddi_soft_state	*ss;
	dam_da_t		*da;
	int			i;

	mdb_printf("%s:", dam->dam_name);
	if (dam->dam_error_thresh) {
		mdb_printf(" [Error threshold: %d%%]", dam->dam_error_thresh);
	}
	mdb_printf("\n");
	...
	if (verbose) {
		mdb_printf("    #: %-20s [ACUSRX] ref config-private   "
		    "provider-private %6s %8s %8s %8s %8s %9s\n", "address",
		    "Jitter", "W XPe", "T XPe", "W NXPe", "T NXPe",
		    "ErrWClose");
	} else {
		mdb_printf("    #: %-20s [ACUSRX] ref config-private   "
		    "provider-private\n", "address");
	...
	for (i = 0; i < ss->n_items; i++) {
		...
		/*
		 * Print shorthand of
		 * Active/Config/Unconfig/Stable/Report/Transport error set
		 * membership
		 */
		if (BT_TEST(dam->dam_active_set.bs_set, i))
			...
			mdb_printf(".");
		if (BT_TEST(dam->dam_cfg_set.bs_set, i))
			mdb_printf("C");
		else
			mdb_printf(".");
		if (BT_TEST(dam->dam_uncfg_set.bs_set, i))
			mdb_printf("U");
		else
			mdb_printf(".");
		...
		if (BT_TEST(dam->dam_report_set.bs_set, i))
			...
			mdb_printf(".");
		if (BT_TEST(dam->dam_xpterr_set.bs_set, i))
			mdb_printf("X");
		else
			mdb_printf(".");
		...
		if (verbose) {
			mdb_printf(" %?6d %?8d %?8d %?8d %?8d %?9x",
			    da->da_jitter, da->da_io_wxpe, da->da_io_xpe,
			    da->da_io_wnxpe, da->da_io_nxpe, da->da_err_wclose);
		}
...
damap(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	damap_print(dam, kdamda, kdamda_n, verbose);

...
devinfo2driver(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (devi.devi_node_state < DS_BOUND) {
		/* No driver bound to this devinfo - nothing to do. */
		mdb_warn("%p: No driver bound to this devinfo node\n", addr);
		return (DCMD_ERR);
	}
...
static char *
devinfo_node_state(ddi_node_state_t state)
{
	char *state_str = NULL;
	...
	case DS_BOUND:
		...
		break;
	case DS_INITIALIZED:
		state_str = "DS_INITIALIZED";
		break;
	case DS_PROBED:
		state_str = "DS_PROBED";
		break;
	case DS_ATTACHED:
		state_str = "DS_ATTACHED";
		break;
	case DS_READY:
		state_str = "DS_READY";
		break;
...
devinfo_print_pathing(int mdi_component, void *mdi_client) {
	...
	while (pip != NULL) {
		...
		pathinfo_state = devinfo_pathinfo_state(pi.pi_state);
		if (pathinfo_state)
			mdb_printf("%s#%d, %s\n", binding_name,
			    ph_di.devi_instance, pathinfo_state);
		else
			mdb_printf("%s#%d, UNKNOWN(%x)\n", binding_name,
			    ph_di.devi_instance, pi.pi_state);
...
devinfo(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	static const mdb_bitmask_t devi_state_masks[] = {
	    ...
	    { "DEVICE_LIVESUSPEND",
		DEVI_DEVICE_LIVE_SUSPENDED, DEVI_DEVICE_LIVE_SUSPENDED },
	...
	static const mdb_bitmask_t devi_flags_masks[] = {
	    ...
	    { "CACHED_DEVID",	DEVI_CACHED_DEVID,	DEVI_CACHED_DEVID },
	    { "PHCI_SIGNALS_VHCI",
		DEVI_PHCI_SIGNALS_VHCI,	DEVI_PHCI_SIGNALS_VHCI },
	    { "REBIND",		DEVI_REBIND,		DEVI_REBIND },
	    { "RETIRED",	DEVI_RETIRED,		DEVI_RETIRED },
	    { "RETIRING",	DEVI_RETIRING,		DEVI_RETIRING },
	    { "R_CONSTRAINT",	DEVI_R_CONSTRAINT,	DEVI_R_CONSTRAINT },
	    { "R_BLOCKED",	DEVI_R_BLOCKED,		DEVI_R_BLOCKED },
	    { "CT_NOP",		DEVI_CT_NOP,		DEVI_CT_NOP },
	    { "PCI_DEVICE",	DEVI_PCI_DEVICE,	DEVI_PCI_DEVICE },
	    { "ANNOTATIONS",	DEVI_ANNOTATIONS,	DEVI_ANNOTATIONS },
	    { "ASSIGNED",	DEVI_ASSIGNED,		DEVI_ASSIGNED },
	    { "CONFIG_ALL_CMPLT",
		DEVI_CONFIG_ALL_CMPLT,	DEVI_CONFIG_ALL_CMPLT },
	    { NULL,		0,			0 }
	};
	...
	if (data.di_flags & DEVINFO_SUMMARY) {
		*nodename = '\0';
		...
		node_state = devinfo_node_state(devi.devi_node_state);
		if (node_state)
			mdb_printf("%0?p %5d %?d %-20s %s\n",
			    addr, devi.devi_major, devi.devi_ref,
			    nodename, node_state);
		else
			mdb_printf("%0?p %5d %?d %-20s UNKNOWN(%x)\n",
			    addr, devi.devi_major, devi.devi_ref,
			    nodename, devi.devi_node_state);
...
devinfo_audit(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	node_state = devinfo_node_state(da.da_node_state);
	if (node_state)
		mdb_printf(" %0?p %16llx %0?p %0?p %s\n",
		    addr, da.da_timestamp, da.da_thread, da.da_devinfo,
		    node_state);
	else
		mdb_printf(" %0?p %16llx %0?p %0?p UNKNOWN(%x)\n",
		    addr, da.da_timestamp, da.da_thread, da.da_devinfo,
		    da.da_node_state);

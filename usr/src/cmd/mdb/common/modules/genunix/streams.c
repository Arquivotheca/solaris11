...
static const struct str_flags mbf[] = {
	...
	{ SF(MSGCRED),		"Private: consumer must verify credentials" },
	{ SF(MSGMARKNEXT), 	"Private: b_next's first byte marked"	},
	{ SF(MSGNOTMARKNEXT),	"Private: ... not marked"		},
	{ SF(MSGWAITSYNC),	"Private: waiting for sync squeue enter"},
	{ 0, NULL,		NULL					}
};
...
static void
ioctlstr(uint_t code, char *outstr, size_t length)
{
	const char *str = NULL;
	...
	str = ioctlname(code);
	if (str != NULL) {
		mdb_snprintf(outstr, length, "%s", str);
		return;
	}
	...
	if (isascii(c_lo) && isprint(c_lo) &&
		...
		(void) mdb_snprintf(outstr, length,
		    "(('%c'<<24)|('%c'<<16)|('%c'<<8)|%d)",
		...
		(void) mdb_snprintf(outstr, length, "%8x", code);
}

/* ARGSUSED */
int
iocblk_prt(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	static const mdb_bitmask_t iocblk_f[] = {
		{ "IOC_ILP32", IOC_MODELS, IOC_ILP32 },
		{ "IOC_LP64", IOC_MODELS, IOC_LP64 },
		{ NULL, 0, 0 }
	};
	...
	if (mdb_getopts(argc, argv,
	    'v', MDB_OPT_SETBITS, TRUE, &verbose,
	    'd', MDB_OPT_SETBITS, TRUE, &decode,
	    NULL) != argc)
		return (DCMD_USAGE);

	if (decode) {
		ioctlstr(addr, cmd, sizeof (cmd));
		if ((flags & DCMD_LOOPFIRST) || !(flags & DCMD_LOOP)) {
			mdb_printf("%<u>%10s %s      %</u>\n",
			    "VALUE", "CMD");
		}
		mdb_printf("%10p %s\n", addr, cmd);
	...
	if (mdb_vread(&iocblk, sizeof (iocblk), addr) == -1) {
		mdb_warn("failed to read iocblk structure at %p", addr);
		return (DCMD_ERR);
	}

	ioctlstr(iocblk.ioc_cmd, cmd, sizeof (cmd));

	if ((flags & DCMD_LOOPFIRST) || !(flags & DCMD_LOOP)) {
		if (verbose) {
			mdb_printf("%?s %10s %10s %11s %11s %s\n"
			    "%<u>%?s %s      %</u>\n",
			    "ADDR", "ID", "COUNT", "ERROR", "RVAL", "FLAG",
			    "CRED", "CMD");
		} else {
			mdb_printf("%<u>%?s %s      %</u>\n",
			    "ADDR", "CMD");
	...
	if (verbose) {
		mdb_printf("%?p %10u %10u %11d %11d <%-9b>\n%?p %s\n",
		    addr, iocblk.ioc_id, iocblk.ioc_count, iocblk.ioc_error,
		    iocblk.ioc_rval, iocblk.ioc_flag, iocblk_f, iocblk.ioc_cr,
		    cmd);
	} else {
		mdb_printf("%?p %s\n", addr, cmd);
	}
...
iocblk_help(void)
{
	mdb_printf("Print iocblk structure or decode ioctl code.\n\n"
	    "Options:\n"
	    "   -d:\tdecode given address as ioctl code\n"
	    "   -v:\tbe verbose - print more fields of iocblk structure\n");
}

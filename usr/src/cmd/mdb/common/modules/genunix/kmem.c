...
kmem_walk_step(mdb_walk_state_t *wsp)
{
	...
	if (!(cp->cache_flags & KMF_HASH)) {
		...
		if (mdb_vread(ubase, chunks * chunksize,
			...
			/*
			 * The sfmmu1 cache is used during dump,
			 * unavoidably. Suppress warnings.
			 */
			if (strcmp(cp->cache_name, "sfmmu1_cache") != 0 ||
			    !mdb_prop_postmortem)
				mdb_warn("failed to read slab contents at %p",
				    kbase);
			return (WALK_ERR);
		}
...
whatis_walk_seg(uintptr_t addr, const vmem_seg_t *vs, whatis_info_t *wi)
{
	...
	while (mdb_whatis_match(w, vs->vs_start, size, &cur)) {
		...
		/*
		 * If we're not printing it separately, provide the vmem_seg
...
bufctl(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (history && !in_history) {
		 ...
		 * were in a separate loop, so that the headers group
...
static const char kmem_dump_desc[] =
	"Check kmem usage recorded while the crash dump was being processed.\n"
	"\n"
	"The kernel reserves a small alternate heap which it uses for dump.\n"
	"Should this dump heap overflow, the normal kmem-managed heap\n"
	"will be used instead. In this case, kmem cache state will be\n"
	"changing while memory is being dumped, leading to inconsistent\n"
	"state. ::findleaks and ::kmem_verify may report various errors,\n"
	"such as corrupt buffers, or missing slab entries. The error messages\n"
	"cannot be trusted if ::kmem_dump shows overflows or unsafe\n"
	"allocations for the cache in question.\n"
	"\n"
	"With no options, the command prints a summary of dump heap usage.\n"
	"\n"
	"With -l, the command prints a list of all kmem caches that were\n"
	"accessed during the dump.\n"
	"\n"
	"If addr is provided, the command prints dump activity for the\n"
	"kmem_cache_t represented by addr.\n";


static const char kmem_dump_args[] =
	"-l    list all caches that changed during dump\n";

static const char kmem_dump_headings[] =
	"overflows  # times an alloc was done after dump heap overflowed\n"
	"size       size in bytes of the dump heap\n"
	"hi-water   maximum dump heap needed\n"
	"oversize   larger than normal allocs\n"
	"used       used dump-area size, includes internal fragmentation\n"
	"% used     used dump-area: 0-100%\n"
	"\nCache headings:\n"
	"Addr       address of the kmem_cache_t\n"
	"Flags      U if always unsafe for use at dump time\n"
	"           O if any overflow allocs occurred\n"
	"Alloc      # of dump allocations\n"
	"Free       # of dump frees\n"
	"Overflow   # of allocation dump heap overflows\n"
	"Nondump    # of non-dump-heap frees\n"
	"Unsafe     # of non-dump-heap allocs\n";
...
kmem_dump_help(void)
{
	...
	mdb_printf("%<b>OPTIONS%</b>\n");
	mdb_inc_indent(2);
	mdb_printf("%s", kmem_dump_args);
	mdb_dec_indent(2);
	mdb_printf("\n%<b>HEADINGS%</b>\n");
	mdb_inc_indent(2);
	mdb_printf("%s", kmem_dump_headings);
}

int
kmem_dump(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	kmem_dump_log_t log;
	GElf_Sym kddsym;
	uintptr_t kddaddr;
	...
	size_t used;
	uint_t old, new;
	uint_t listall = FALSE;
	int percent;
	char flg[4];

	if (mdb_getopts(argc, argv,
	    'l', MDB_OPT_SETBITS, TRUE, &listall,
	    NULL) != argc)
		return (DCMD_USAGE);

	if (!(flags & DCMD_ADDRSPEC)) {

		if (mdb_lookup_by_name("kmem_dump", &kddsym) == -1) {
			mdb_warn("kmem_dump is not supported on older kernels");
			return (DCMD_ERR);
		}

		kddaddr = (uintptr_t)kddsym.st_value;

		if (mdb_vread(&kdd, sizeof (kmem_dump_t), kddaddr) == -1) {
			mdb_warn("couldn't read kmem_dump at %p", kddaddr);
			return (DCMD_ERR);
		}

		mdb_printf("%-20s %d\n", "overflows", kdd.kdd_overflows);
		mdb_printf("%-20s 0x%lx\n", "size", kdd.kdd_size);

		if (kdd.kdd_oversize_allocs) {
			mdb_printf("%-20s %d\n", "# oversize allocs",
			    kdd.kdd_oversize_allocs);
			mdb_printf("%-20s 0x%x\n", "max oversize alloc",
			    kdd.kdd_oversize_max);
		}

		used = (char *)kdd.kdd_curr - (char *)kdd.kdd_start;
		percent = (used * 100) / kdd.kdd_size;
		mdb_printf("%-20s 0x%lx\n", "used", used);
		mdb_printf("%-20s %d\n", "% used", percent);
		mdb_printf("%-20s 0x%lx\n", "hi-water", MAX(kdd.kdd_hiwater,
		    used));

		if (percent >= 90 && mdb_readvar(&old, "dump_kmem_heap") !=
		    -1) {
			new = MAX(kdd.kdd_hiwater, 2 * kdd.kdd_size);
			mdb_printf("\nThe dump heap is too small."
			    " Add this to /etc/system:\n"
			    "\tset dump_kmem_heap=0x%x\n",
			    P2ROUNDUP(new, 1024));
			if (listall)
				mdb_printf("\n");
		}
		...
		if (listall) {
			(void) mdb_walk_dcmd("kmem_cache", "kmem_dump", argc,
			    argv);

		} else if (kdd.kdd_overflows) {
			mdb_printf("Warning: some caches may be inconsistent - "
			    "use ::kmem_dump -l\n");
		} else {
			mdb_printf("Dump allocations are safe in all caches - "
			    "no problems found.\n");
		}
		...
		if (DCMD_HDRSPEC(flags)) {
			mdb_printf("%<u>%-25s %?s %3s %7s %7s %7s %7s%</u>",
			    "Cache Name", "Addr", "Flg",
			    "Alloc", "Free", "Ovrflow", "Nondump");
			mdb_printf("\n");
		}

		if (mdb_vread(&c, sizeof (kmem_cache_t), addr) == -1) {
			mdb_warn("couldn't read kmem_cache at %p", addr);
			return (DCMD_ERR);
		}

		if (c.cache_dumplog == NULL)
			return (DCMD_OK);

		flg[0] = ' ';
		flg[1] = c.cache_flags & KMF_DUMPOVERFLOW ? 'O' : ' ';
		flg[2] = c.cache_flags & KMF_DUMPUNSAFE ? 'U' : ' ';
		flg[3] = '\0';

		if (c.cache_dumplog != KMEM_NODUMPLOG && mdb_vread(&log,
		    sizeof (kmem_dump_log_t), (uintptr_t)c.cache_dumplog) !=
		    -1) {
			mdb_printf("%-25s %?p %3s %7d %7d %7d %7d\n",
			    c.cache_name, addr, flg,
			    log.kdl_allocs, log.kdl_frees, log.kdl_overflows,
			    log.kdl_free_nondump);
		} else {
			mdb_printf("%-25s %?p %3s %7s %7s %7s %7s\n",
			    c.cache_name, addr, flg, "?", "?", "?", "?");
		}
	}
...
void
kmem_dump_warn(const kmem_cache_t *cp)
{
	GElf_Sym kddsym;
	uintptr_t kddaddr;
	int overflows = 0;
	...
	if (cp == NULL) {
		...
		kddaddr = (uintptr_t)kddsym.st_value + offsetof(kmem_dump_t,
		    kdd_overflows);

		if (mdb_vread(&overflows, sizeof (int), kddaddr) == -1)
			return;
	...
	} else if (KMEM_DUMP_WARN(cp)) {

		mdb_printf("cache %s is inconsistent - see ::help kmem_dump\n",
		    cp->cache_name);
	}
}

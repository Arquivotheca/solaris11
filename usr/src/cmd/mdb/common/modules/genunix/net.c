...
/*ARGSUSED*/
static int
netstat_sdp_cb(uintptr_t kaddr, const void *walk_data, void *cb_data)
{
	netstat_cb_data_t *ncb = cb_data;
	sdp_ext_conn_t conn;

	if (mdb_vread(&conn, sizeof (sdp_ext_conn_t), kaddr) == -1) {
		mdb_warn("failed to read conn_t at %p", kaddr);
		return (WALK_ERR);
	}

	if (conn.sdp_ext_family != ncb->af)
		return (WALK_NEXT);

	mdb_printf("%0?p %p ", kaddr, conn.sdp_ext_state);
	if (conn.sdp_ext_family == AF_INET) {
		net_ipv4addrport_pr(&conn.sdp_ext_saddr, conn.sdp_ext_src_port);
		mdb_printf(" ");
		net_ipv4addrport_pr(&conn.sdp_ext_faddr, conn.sdp_ext_dst_port);
	} else if (conn.sdp_ext_family == AF_INET6) {
		net_ipv6addrport_pr(&conn.sdp_ext_saddr, conn.sdp_ext_src_port);
		mdb_printf(" ");
		net_ipv6addrport_pr(&conn.sdp_ext_faddr, conn.sdp_ext_dst_port);
	}
#ifdef ZONES_SUPPORT
	mdb_printf(" %5i", ns_to_stackid((uintptr_t)conn.sdp_netstack));
#else
	mdb_printf(" %5i", 0);
#endif
	mdb_printf(" %4i\n", conn.sdp_ext_zoneid);
...
netstat_header_v4(int proto)
{
	...
	else if (proto == PROTO_SDP)
		mdb_printf("%<u>%-?s ", "SDPv4");
    ...
    void *cbdata)
{
	...
	if (mdb_walk(cache, cbfunc, cbdata) == -1) {
		if (strcmp("sdp_conn_cache", cache) != 0) {
...
netstat(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (optP != NULL) {
		if ((strcmp("tcp", optP) != 0) && (strcmp("udp", optP) != 0) &&
		    (strcmp("icmp", optP) != 0)&& (strcmp("sdp", optP) != 0))
			return (DCMD_USAGE);
	...
	if ((optP == NULL) || (strcmp("sdp", optP) == 0)) {
		status = netstat_print_common("sdp_conn_cache", PROTO_SDP,
		    netstat_sdp_cb, cbdata);
	}
out:

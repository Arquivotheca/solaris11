...
sdp_sock_print(struct sonode *socknode)
{
	...
	if (mdb_vread(&conn, sizeof (sdp_ext_conn_t),
	    (uintptr_t)socknode->so_proto_handle) == -1) {
		mdb_warn("failed to read conn_t SDP");
			return (-1);
	}

	mdb_printf("socket:");

	switch (conn.sdp_ext_family) {
	case AF_INET:
		mdb_nhconvert(&port, &conn.sdp_ext_src_port, sizeof (port));
		mdb_printf("AF_INET SDP %I %d ",
		    V4_PART_OF_V6(conn.sdp_ext_saddr), port);
		if (conn.sdp_ext_state != SDP_CONN_ST_LISTEN) {
			mdb_printf("remote: ");
			mdb_nhconvert(&port, &conn.sdp_ext_dst_port,
			    sizeof (port));
			mdb_printf("AF_INET SDP %I %d ",
			    V4_PART_OF_V6(conn.sdp_ext_faddr), port);
		}
		break;
	case AF_INET6:
		mdb_nhconvert(&port, &conn.sdp_ext_src_port, sizeof (port));
		mdb_printf("AF_INET6 SDP %N %d ", &conn.sdp_ext_saddr, port);
		if (conn.sdp_ext_state != SDP_CONN_ST_LISTEN) {
			mdb_printf("remote: ");
			mdb_nhconvert(&port, &conn.sdp_ext_dst_port,
			    sizeof (port));
			mdb_printf("AF_INET6 SDP %N %d ", &conn.sdp_ext_faddr,
			    port);
		}
		break;
	default:

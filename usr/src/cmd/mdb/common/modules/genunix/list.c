    ...
    int (*element_check)(void *, uintptr_t, void *), void *arg)
{
	...
	if (list.list_size == 0) {
		mdb_warn("%s at %#lx is not initialized\n", list_name,
		    wsp->walk_addr);
		return (WALK_ERR);
	}
	if (list.list_size <= list.list_offset ||
	    list.list_size < list.list_offset + sizeof (list_node_t)) {
		mdb_warn("%s at %#lx has a list_offset (%#lx) which doesn't "
		    "fit in list_size (%#lx)\n", list_name, wsp->walk_addr,
		    list.list_offset, list.list_size);
		return (WALK_ERR);
	}
	...
	wsp->walk_addr = (begin == NULL) ?
	    (uintptr_t)list.list_head.list_next : begin + lwd->lw_offset;
	wsp->walk_data = lwd;

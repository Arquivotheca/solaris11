...
#define	TOO_SMALL_FOR_A_STACK	(sizeof (struct rwindow))
...
stacks_findstack(uintptr_t addr, findstack_info_t *fsip, uint_t print_warnings)
{
	...
	fsip->fsi_tstate = state = thr.t_state;
	fsip->fsi_panic = !!(thr.t_flag & T_PANIC);

	/*
	 * Thread should be in a valid state
	 */
	if (state != TS_FREE && state != TS_SLEEP && state != TS_RUN &&
	    state != TS_ONPROC && state != TS_ZOMB && state != TS_STOPPED &&
	    state != TS_WAIT) {
		if (print_warnings)
			mdb_warn(
			    "thread state corrupt for thread %p\n",
			    addr);
	...
	/*
	 * If the stack size is too small, assume that it's bogus
	 */
	if (stksz < TOO_SMALL_FOR_A_STACK) {
		if (print_warnings)
			mdb_warn("stack size for thread %p is too small to be "
			    "reasonable\n", addr);
		fsip->fsi_failed = FSI_FAIL_THREADCORRUPT;
		return (DCMD_ERR);
	}

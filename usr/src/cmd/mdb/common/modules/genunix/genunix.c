...
callout_common_init(callout_data_t *coargs)
{
	...
	if (mdb_readsym(&(coargs->now), sizeof (hrtime_t), "hrtime_last") !=
		...
		if (mdb_lookup_by_name("hrt", &g_hrt) == -1) {
			mdb_warn("Could not lookup symbol 'hrt'");
			return (DCMD_ERR);
		}
		if (mdb_vread(&hrt, sizeof (hrt), g_hrt.st_value) == -1) {
			mdb_warn("could not read symbol 'hrt' at 0x%p\n",
			    (void *)hrt);
			return (DCMD_ERR);
		}
		if (mdb_vread(&(coargs->now), sizeof (coargs->now),
		    hrt + OFFSETOF(hrt_t, hrtime_base)) == -1) {
			mdb_warn("could not read 'hrt->hrtime_base' at 0x%p\n",
			    hrt + OFFSETOF(hrt_t, hrtime_base));
			return (DCMD_ERR);
		}
	}
...
kmastat_cache(uintptr_t addr, const kmem_cache_t *cp, kmastat_args_t *kap)
{
	...
	avail = cp->cache_bufslab + cp->cache_full.ml_total * magsize;
...
cpuinfo_walk_cpu(uintptr_t addr, const cpu_t *cpu, cpuinfo_data_t *cid)
{
	...
	uintptr_t pinned;
	char **flagbuf;
	int nflaglines = 0, flagline = 0, bspl, rval = WALK_NEXT, nsec_per_tick;
	hrtime_t last_swtch;
	...
	if (cpu->cpu_last_swtch) {
		last_swtch = cpu->cpu_last_swtch;

		if (mdb_scalehrtime(&last_swtch) != 0) {
			mdb_warn("Failed to scale cpu_last_swtch");
			return (DCMD_ERR);
		}

		if (mdb_readvar(&nsec_per_tick, "nsec_per_tick") == -1) {
			mdb_warn("Failed to read nsec_per_tick");
			return (DCMD_ERR);
		}

		mdb_printf("t-%-4d ",
		    (clock_t)((mdb_gethrtime() - last_swtch)/nsec_per_tick));
	} else {
		mdb_printf("%-6s ", "-");
	}
...
panicinfo(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (mdb_ctf_lookup_by_name("panic_data_t", &id) == -1) {
		mdb_warn("couldn't find panic_data_t");
		return (DCMD_ERR);
	}
	if (mdb_ctf_offsetof(id, "pd_nvdata", &nv_off) == -1) {
		mdb_warn("couldn't find member pd_nvdata of type panic_data\n");
		return (DCMD_ERR);
	}
	nv_off /= 8; /* offset was in bits */
	...
	if (mdb_lookup_by_name("panicbuf", &sym) != 0) {
		mdb_warn("couldn't read panicbuf symbol\n");
		return (DCMD_ERR);
	}

	if (mdb_ctf_vread(&pd, "panic_data_t", sym.st_value,
	    MDB_CTF_VREAD_IGNORE_ALL) == -1) {
		mdb_warn("couldn't read panic_data at %p\n", addr);
		return (DCMD_ERR);
	}

	mdb_printf("%16s %s\n", "message",  (char *)buf + pd.pd_msgoff);

	pd_nvdata = (panic_nv_t *)((ulong_t)buf + nv_off);
	n = (pd.pd_msgoff - nv_off) / sizeof (panic_nv_t);

	for (i = 0; i < n; i++)
		mdb_printf("%16s %?llx\n",
		    pd_nvdata[i].pnv_name, pd_nvdata[i].pnv_value);
 ...
 * debugger, the lbolt value if used with the -l option, or scale/unscale
 * time with the -s/u options.
 */
/*ARGSUSED*/
static int
time(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	uint_t opt_scale = FALSE;
	uint_t opt_unscale = FALSE;
	...
	if (mdb_getopts(argc, argv,
	    ...
	    's', MDB_OPT_SETBITS, TRUE, &opt_scale,
	    'u', MDB_OPT_SETBITS, TRUE, &opt_unscale,
	    'x', MDB_OPT_SETBITS, TRUE, &opt_hex,
	    NULL) != argc)
		return (DCMD_USAGE);

	if ((flags & DCMD_ADDRSPEC) && !(opt_scale || opt_unscale))
		return (DCMD_USAGE);

	if ((opt_scale && opt_unscale) ||
	    (opt_lbolt && (opt_scale || opt_unscale)) ||
	    (opt_dec && opt_hex))
		return (DCMD_USAGE);

	/*
	 * We require a scaling or unscaling option if the user specified an
	 * input value. If none was given, check if the user requested the
	 * number of clock ticks since boot. Lastly, the user may have requested
	 * the current time in unscaled format.
	 */
	if (flags & DCMD_ADDRSPEC) {
		result = (hrtime_t)addr;

		if (opt_scale) {
			if (mdb_scalehrtime(&result) != DCMD_OK) {
				mdb_warn("Failed to scale time");
				return (DCMD_ERR);
			}
		} else {
			if (mdb_unscalehrtime(&result) != DCMD_OK) {
				mdb_warn("Failed to unscale time");
				return (DCMD_ERR);
			}
		}
	} else {
		if (opt_lbolt) {
			result = mdb_get_lbolt();
		} else {
			result = mdb_gethrtime();
...
static const char *time_desc =
	"::time is used to print the current high-resolution timestamp,\n"
	"and to convert between unscaled and scaled high-res timestamps.\n\n"
	"The modes of operation are:\n\n"
	"  ::time [-[lu]] [-[dx]]\n\n"
	"      Print the current timestamp.  If executed from kmdb(1), this\n"
	"      is the time at which kmdb was entered; if executed on a crash\n"
	"      dump, it is the time at which the crash dump was taken;\n"
	"      if executed on a live system, it is the running value of time\n"
	"      on that system.  By default, the time is printed in units of\n"
	"      nanoseconds since boot.\n\n"
	"  value::time -s [-[dx]]\n\n"
	"      Convert \"value\" from a native (\"unscaled\") high-res time\n"
	"      to a number of nanoseconds (\"scaled\" high-res time).\n\n"
	"  value::time -u [-[dx]]\n\n"
	"      Convert \"value\" from a number of nanoseconds to a\n"
	"      native high-res time.\n";

static const char *time_args =
	"  -l  print current time as clock ticks, rather than nanoseconds\n"
	"  -s  convert input from native time to nanoseconds\n"
	"  -u  convert input from nanoseconds to native time\n"
	"  -d  report time in decimal\n"
	"  -x  report time in hexadecimal\n";
...
/*ARGSUSED*/
static int
deadman_journal(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	boolean_t enabled = B_FALSE, empty = B_TRUE, has_header = B_FALSE;
	uint_t opt_v = FALSE, i, j, k, ncpus, entries;
	uintptr_t dj_cpu, dj_entries;
	deadman_journal_t entry;
	size_t ptr_sz, entry_sz;
	...
	if (mdb_readvar(&enabled, "deadman_enabled") == -1) {
		mdb_warn("Error reading 'deadman_enabled'\n");
		return (DCMD_ERR);
	}

	if (enabled != B_TRUE) {
		mdb_printf("Deadman panic disabled. No content to display.\n");
		return (DCMD_OK);
	}

	if (mdb_readvar(&dj_cpu, "deadman_journal") == -1 ||
	    mdb_readvar(&ncpus, "max_ncpus") == -1 ||
	    mdb_readvar(&entries, "deadman_seconds") == -1) {
		mdb_warn("Error reading deadman variables.\n");
		return (DCMD_ERR);
	}

	ptr_sz = sizeof (deadman_journal_t *);
	entry_sz = sizeof (deadman_journal_t);

	/*
	 * Walk each CPU's journal pointer.
	 */
	for (i = 0; i < ncpus; i++, dj_cpu += ptr_sz) {
		if (mdb_vread(&dj_entries, ptr_sz, dj_cpu) == -1) {
			mdb_warn("Error reading CPU %u's journal.\n", i);
			return (DCMD_ERR);
		}
		...
		/*
		 * Print all the non-empty journal entries of this CPU.
		 */
		for (j = 0; j < entries; j++, dj_entries += entry_sz) {
			if (mdb_vread(&entry, entry_sz, dj_entries) !=
			    entry_sz) {
				mdb_warn("Error reading entry %d for CPU %d.\n",
				    j, i);
				return (DCMD_ERR);
			}

			if (entry.dj_hrtime == 0)
				continue;
			...
			if (has_header == B_FALSE) {
				has_header = B_TRUE;
				mdb_printf("%<u>%5s %5s %16s %9s %8s %16s%"
				    "</u>\n", "CPU", "LOG#", "TIMESTAMP",
				    "INTRACTV", "NRUN", "THREAD");
			}

			mdb_printf("%5d %5d %16llx %9x %8d %16p\n",
			    i, j, entry.dj_hrtime, entry.dj_intr_actv,
			    entry.dj_nrunnable, entry.dj_cpu_thread);
			...
			if (opt_v) {
				...
				for (k = 0; k < HIGH_LEVELS; k++)
					mdb_printf("%4u %16llx\n",
					    k + LOCK_LEVEL + 1,
					    entry.dj_pil_high_start[k]);

				mdb_printf("\n%<u>%4s %16s %16s%</u>\n",
				    "PIL", "ACCUMULATED", "COUNT");

				for (k = 0; k < PIL_MAX; k++)
					mdb_printf("%4u %16llx %16llx\n",
					    k + 1, entry.dj_intrstat[k][0],
					    entry.dj_intrstat[k][1]);
	...
	if (empty == B_TRUE)
		mdb_printf("No entries found.\n");

	return (DCMD_OK);
}

void
deadman_journal_help(void)
{
	mdb_printf("Displays the contents of the deadman journal.\n\n"
	    "Switches:\n"
...
static const mdb_dcmd_t dcmds[] = {
	...
	{ "deadman_journal", "[-v]", "display the deadman journal",
	    deadman_journal, deadman_journal_help },
	...
	{ "time", "[-lsudx]", "display and convert system time", time,
	    time_help },
	...
	{ "kmem_dump", "?[-l]", "print kmem usage during crash dump",
		kmem_dump, kmem_dump_help },
	...
	{ "netstat", "[-arv] [-f inet | inet6 | unix] [-P tcp | udp | icmp | "
	    "sdp ]", "show network statistics", netstat },
	...
	/* from numaio.c */
	{ "numaio_group", "[-c] [-o object-type ]",
		"show numaio groups", numaio_group, numaio_group_help },
	...
	{ "mblk2dblk", ":", "convert mblk_t address to dblk_t address",
		mblk2dblk },
	{ "iocblk", ":[-d|v]", "print iocblk structure or decode ioctl",
		iocblk_prt, iocblk_help },
...
static const mdb_walker_t walkers[] = {
	...
	/* from zone.c */
	{ "zone_deathrow",
		"walk zones on zone_deathrow (<defunct> zones)",
		zone_walk_init, zone_walk_step, NULL, ZONE_WALK_DEATHROW },
	{ "zone", "walk a list of kernel zones",
		zone_walk_init, zone_walk_step, NULL, ZONE_WALK_ACTIVE },

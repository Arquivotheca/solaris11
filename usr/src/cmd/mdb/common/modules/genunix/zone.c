...
zone_walk_init(mdb_walk_state_t *wsp)
{
	...
	if (wsp->walk_addr == NULL) {
		const char *walk_begin;

		if (wsp->walk_arg == ZONE_WALK_DEATHROW)
			walk_begin = "zone_deathrow";
		else
			walk_begin = "zone_active";
		if (mdb_lookup_by_name(walk_begin, &sym) == -1) {
			mdb_warn("failed to find '%s'", walk_begin);
			return (WALK_ERR);
		}
		wsp->walk_addr = (uintptr_t)sym.st_value;
	} else {
		if (wsp->walk_arg == ZONE_WALK_DEATHROW) {
			mdb_warn("zone_deathrow only supports global walks\n");
			return (WALK_ERR);
		}
	}

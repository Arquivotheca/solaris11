    ...
    struct timespec *stime)
{
	...
	secs = (((time_t)fwtime * PMCS_FWLOG_TIMER_DIV) / NSECS_PER_SEC);
	nsecs = (((long)fwtime * PMCS_FWLOG_TIMER_DIV) % NSECS_PER_SEC);
...
static int
display_iport_damap(dev_info_t *pdip, uint_t verbose)
{
	int rval = DCMD_ERR;
	struct dev_info dip;
	scsi_hba_tran_t sht;
	...
	if (dam0 != NULL) {
		rval = mdb_call_dcmd("damap", dam0, DCMD_ADDRSPEC, na, v_argp);
		mdb_printf("\n");
		if (rval != DCMD_OK) {
			return (rval);
		}
	...
	if (dam1 != NULL) {
		rval = mdb_call_dcmd("damap", dam1, DCMD_ADDRSPEC, na, v_argp);
		mdb_printf("\n");
	}
...
display_iport_more(dev_info_t *dip, per_iport_setting_t *pis)
{
	if (pis->pis_damap_info) {
		(void) display_iport_damap(dip, pis->pis_verbose);
	}
...
pmcs_iport_walk_cb(uintptr_t addr, const void *wdata, void *priv)
{
	...
	if (strlen(unit_address) < 3) {
		...
		mdb_printf("UA %-16s %16s %8s %4s %1s %16s", "Iport",
		    "UA State", "PortID", "Phys", "C", "DIP\n");
		mdb_printf("%2s %16p %16s %8s %4d %1d %16p\n", unit_address,
		    addr, ua_state, portid, iport.nphy, iport.iport_changed,
		    iport.dip);
	} else {
		/* Temporary iport unit address */
		mdb_printf("%-32s %16s %20s %8s %4s %1s %16s", "UA", "Iport",
		    "UA State", "PortID", "Phys", "C", "DIP\n");
		mdb_printf("%32s %16p %20s %8s %4d %1d %16p\n", unit_address,
		    addr, ua_state, portid, iport.nphy, iport.iport_changed,
		    iport.dip);
	}
...
display_targets(struct pmcs_hw m, int verbose, int totals_only)
{
	...
	ulong_t		list_head_offset;

	list_head_offset = offsetof(pmcs_xscsi_t, lun_list) +
	...
	for (idx = 0; idx < max_dev; idx++) {
		...
		if (verbose) {
			...
			/* Make sure there's at least one LUN (i.e. not SES) */
			if (((ulong_t)xs.lun_list.list_head.list_next -
			    list_head_offset) != (ulong_t)targets[idx]) {
				struct scsi_device sd;
				pmcs_lun_t lun;

				bzero(&sd, sizeof (struct scsi_device));
				bzero(&lun, sizeof (pmcs_lun_t));
				if (MDB_RD(&lun, sizeof (pmcs_lun_t),
				    xs.lun_list.list_head.list_next) == -1) {
					NOREAD(pmcs_lun_t,
					    xs.lun_list.list_head.list_next);
					continue;
				}
				if ((lun.sd == NULL) ||
				    MDB_RD(&sd, sizeof (struct scsi_device),
				    lun.sd) == -1) {
					NOREAD(scsi_device, lun.sd);
					continue;
				}
    ...
    int totals_only)
{
	...
	/* Process dead phy list */
	pphy = (pmcs_phy_t *)ss.dead_phys;
	if (pphy == NULL)
		return;

	if (!totals_only) {
		mdb_printf("\n\nDead PHY Information\n");
		mdb_printf("--------------------\n");
		mdb_printf("SAS Address      Hdl Phy#  Speed Type ");
...
/*
 * There are actually four dumps, each 16K in length.  In order, they are the
 * register dump for the AAP1, event log for AAP1, register dump for IOP and
 * event log for IOP.
 */
static int
pmcs_dump_regs(struct pmcs_hw *ss, int instance, const char *ofile)
{
	uint8_t *dumpp, *curp;
	int	ofilefd = -1, idx;
	char	ofilename[MAXPATHLEN];
	int	rval = DCMD_OK;
	static char *suffix[4] = { "RegDumpAAP1", "EventLogAAP1",
	    "RegDumpIOP", "EventLogIOP" };

	if (ss->flash_regdumpp == NULL) {
		mdb_warn("No register dumps/event logs for instance %d",
		    instance);
		return (DCMD_OK);
	}

	dumpp = mdb_alloc(PMCS_FLASH_REG_DUMP_SIZE, UM_SLEEP);

	if (MDB_RD(dumpp, PMCS_FLASH_REG_DUMP_SIZE, ss->flash_regdumpp) == -1) {
		NOREAD(dumpp, ss->flash_regdumpp);
		rval = DCMD_ERR;
		goto cleanup;
	}
	...
	idx = 0;
	while (idx < 4) {
		curp = dumpp + (idx * GSM_FLASH_REG_DUMP_SIZE);

		if (snprintf(ofilename, MAXPATHLEN, "%s%d-%s", ofile,
		    instance, suffix[idx]) > MAXPATHLEN) {
			mdb_warn("Output filename is too long for instance %d",
			    instance);
			rval = DCMD_ERR;
			goto cleanup;
		...
		ofilefd = open(ofilename, O_WRONLY | O_CREAT,
		    S_IRUSR | S_IRGRP | S_IROTH);
		if (ofilefd < 0) {
			mdb_warn("Unable to open '%s' for instance %d",
			    ofilename, instance);
			rval = DCMD_ERR;
			goto cleanup;
		}

		if (write(ofilefd, curp, GSM_FLASH_REG_DUMP_SIZE) !=
		    GSM_FLASH_REG_DUMP_SIZE) {
			mdb_warn("Failed to write %d bytes for instance %d",
			    GSM_FLASH_REG_DUMP_SIZE, instance);
			rval = DCMD_ERR;
			goto cleanup;
		}
	...
	mdb_printf("Register dumps/event logs for instance %d complete\n",
	    instance);
...
cleanup:
	...
	mdb_free(dumpp, PMCS_FLASH_REG_DUMP_SIZE);
	return (rval);
}

static int
pmcs_regdump(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	void		*pmcs_state;
	const char	*ofile = NULL;
	struct pmcs_hw	ss;
	...
	if (!(flags & DCMD_ADDRSPEC)) {
		...
		if (mdb_pwalk_dcmd("genunix`softstate", "pmcs`pmcs_regdump",
		    argc, argv, (uintptr_t)pmcs_state) == -1) {
			mdb_warn("mdb_pwalk_dcmd failed for pmcs_regdump");
			return (DCMD_ERR);
		}
		return (DCMD_OK);
	}
	...
	return (pmcs_dump_regs(&ss, dip.devi_instance, ofile));
}
#endif	/* _KMDB */
...
static const mdb_dcmd_t dcmds[] = {
	...
	{ "pmcs_regdump",
	    "?-o output_file",
	    "dump pmcs register dumps and firmware event logs from flash to "
	    "output_file", pmcs_regdump, NULL
	},
#endif	/* _KMDB */
	{ NULL }
};

...
#define	BOOTADM_PROG	"/usr/sbin/bootadm"
...
#if defined(ARCH_X86)
static int gfx_fd = -1;
static void splash_gfx_step();
static void notify_gfx_shutdown();
static void splash_gfx_last();
static void reset_text_console();

#define	NOTIFY_GFX_SHUTDOWN()	notify_gfx_shutdown()
#define	SPLASH_GFX_STEP()	splash_gfx_step()
#define	SPLASH_GFX_LAST()	splash_gfx_last()
#define	RESET_TEXT_CONSOLE()	reset_text_console()

#else

#define	NOTIFY_GFX_SHUTDOWN()
#define	SPLASH_GFX_STEP()
#define	SPLASH_GFX_LAST()
#define	RESET_TEXT_CONSOLE()
...
#define	GDM_STOP_TIMEOUT	15	/* Give gdm 15 seconds to shut down */
...
halt_zones()
{
	...
	/* Get list of zone ids from the kernel */
	if (zone_get_zoneids(&zones, &nz) != 0) {
		(void) fprintf(stderr, gettext("%s: Could not get list of"
...
validate_zfs_pool(char *arg, char *mountpoint)
{
	...
	char *cur_mountpoint = NULL; /* current mount point */
	int rc = 0;
	...
	/* check if dataset is mounted already */
	if (zfs_is_mounted(zhp, &cur_mountpoint) &&
	    (cur_mountpoint != NULL)) {
		/* use the current mount point */
		(void) strlcpy(mountpoint, cur_mountpoint, MAXPATHLEN);
		free(cur_mountpoint);
	} else if (mount(zfs_get_name(zhp), mountpoint,
	    MS_DATA|MS_OPTIONSTR|MS_RDONLY, MNTTYPE_ZFS, NULL, 0,
	    mntopts, sizeof (mntopts)) != 0) { /* perform the mount */
		perror(cmdname);
		(void) fprintf(stderr,
		    gettext("%s: Failed to mount %s\n"), cmdname, arg);
		rc = -1;
    ...
    int *is_dryrun, const char *bename)
{
	...
	if ((bootargs_buf[0] == 0 || isdigit(bootargs_buf[0])) &&
	    bename == NULL && getzoneid() == GLOBAL_ZONEID) {
		/*
		...
		if (rc == 0)
			rc = be_get_boot_args_and_mountpt(entnum, bootargs_buf,
			    buf_size, be_mountpoint, sizeof (be_mountpoint));
	...
	if (newarg && newarg[0] != '-') {
		...
		/*
		 * No unix argument, but mountpoint is not empty, so use
		 * /platform/i86pc/kernel/amd64/unix as default.
		 */

		(void) snprintf(&unixfile[mplen], sizeof (unixfile) - mplen,
 ...
 * We check if 'splash_shutdown' is currently set to true inside
 * system/console-reset and, if so, we ask for a splash shutdown.
 * Kernel code will then load the splash shutdown background and
 * prepare the animation. We step through the animation via
 * splash_gfx_step() at various stages during the shutdown.
 * At any time, console can be reclaimed back (e.g. on error)
 * via reset_text_console().
 */
...
reset_text_console()
{
	...
	(void) ioctl(gfx_fd, KDSETMODE, KD_RESETTEXT);
	(void) close(gfx_fd);
	gfx_fd = -1;
}
...
splash_gfx_step()
{
	...
	(void) ioctl(gfx_fd, KDSHUTDOWN, KDS_STEP);
}
...
splash_gfx_last()
{
	...
	(void) ioctl(gfx_fd, KDSHUTDOWN, KDS_LAST);
}
...
notify_gfx_shutdown()
{
	...
	scf_simple_prop_t 	*load_image = NULL;
	uint8_t			*val;

	if ((load_image = scf_simple_prop_get(NULL, fmri, "config",
	    "splash_shutdown")) == NULL)
		return;

	val = scf_simple_prop_next_boolean(load_image);
	if (val == NULL || *val == 0) {
		scf_simple_prop_free(load_image);
		return;
	}
	scf_simple_prop_free(load_image);
	...
	if (gfx_fd == -1) {
		(void) fprintf(stderr, gettext("%s: unable to open /dev/fb"
		    " for splash shutdown\n"), cmdname);
		return;
	}
	(void) ioctl(gfx_fd, KDSHUTDOWN, KDS_START);
}
#endif	/* ARCH_X86 */
...
main(int argc, char *argv[])
{
	...
	if (bename && zoneid != GLOBAL_ZONEID) {
		(void) fprintf(stderr,
		    gettext("%s: -e only valid from global zone\n"), cmdname);
		return (EINVAL);
	}
	 ...
	 */
	if (fcn == AD_BOOT) {
		(void) audit_reboot_success();
	} else {
		(void) audit_halt_success();
	}
...
#if defined(ARCH_X86)
		/*
		 * Tell to the kernel graphic console that we are shutting down.
		 */
		if (zoneid == GLOBAL_ZONEID)
			NOTIFY_GFX_SHUTDOWN();
#endif	/* ARCH_X86 */
	...
	/* If we get here we probably want the console back... */
	RESET_TEXT_CONSOLE();
...
#if defined(USE_GRUB)
		} else if (strlen(be_mountpoint) != 0 &&

...
main(int argc, char *argv[])
{
	...
	/*
	 * Load the kernel host-id and event to class mappings
	 * before turning on audit.  Turning on audit looks at
	 * all processes to see if they are preselected for kernel
	 * events and sets pre-syscall processing iff kernel events
	 * are preselected.
	 */
	conf_to_kernel();
	...
	as_null.ai_mask.as_success = AU_MASK_NONE;
	as_null.ai_mask.as_failure = AU_MASK_NONE;
	...
	/*
	 * Update the default user preselection mask and the non-attributable
	 * audit mask.
	 */
	if (!scf_to_kernel_flags(&err_str) ||
	    !scf_to_kernel_naflags(&err_str)) {
		err_exit(err_str);
	}
	...
	while (running) {
		...
		if (reset_list || reset_file) {
			if (reset_list) {
				conf_to_kernel();
				if (!scf_to_kernel_qctrl(&err_str) ||
				    !scf_to_kernel_policy(&err_str)) {
					err_exit(err_str);
				}
...
#if DEBUG
	{
		int	acresult = AUC_DISABLED;

		void auditon(A_GETCOND, (caddr_t)&acresult, sizeof (acresult));
...
conf_to_kernel(void)
{
	...
	/*
	 * set kernel instance host-id
	 * ENETDOWN indicates that it is set to loopback
	 * each time __do_sethost is called if the host-id
	 * is loopback, the hostname lookup will happen again.
	 * This allows for auditd refresh to get a new host-id.
	 */
	if (__do_sethost("auditd") != 0) {
		if (errno == ENETDOWN) {
			__audit_dowarn("hostname", "", 0);
		} else {
			__audit_dowarn("nostart", "", 0);
			auditd_exit(1);
		}

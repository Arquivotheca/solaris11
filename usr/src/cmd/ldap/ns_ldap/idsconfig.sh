...
#
#
# idsconfig -- script to setup DS 5.x and DSEE 6.x/7.x and ODSEE 11g
# for LDAP naming services.
#
...
$PROG: Setup of DSEE server ${IDS_SERVER} is complete.
...
Note: idsconfig has created entries for VLV indexes. 
      ...
      For DSEE6.x or later, use dsadm command delivered with DS on ${IDS_SERVER}
...
`
 ...
 10  DSEE Time Limit               : $IDS_TIMELIMIT
 11  DSEE Size Limit               : $IDS_SIZELIMIT
...
HELP - By default DSEE does not store userPassword attribute values using
...
HELP - Enter 'y' to set up the LDAP server for shadow update.
       ...
       to update shadow(4), user_attr(4), exec_attr(4), auth_attr(4),
       prof_attr(4), and project(4) data on the LDAP server. If
...
HELP - Start the setup to add an administrator identity/credential
       ...
       shadow(4), user_attr(4), exec_attr(4), auth_attr(4),
       prof_attr(4), and project(4) data on the LDAP server.
...
#
# get_ids_server(): Prompt for DSEE server name.
#
get_ids_server()
{
    while :
	...
	get_ans "Enter the Directory Server's hostname to setup:" "$IDS_SERVER" 
	IDS_SERVER="$ANS"
...
#
# get_ids_port(): Prompt for DSEE port number.
#
get_ids_port()
{
    # Get a valid DSEE port number.
    while :
    do
	# Enter port number.
	get_number "Enter the port number for DSEE (h=help):" "$IDS_PORT" "port_help"
	IDS_PORT=$ANS
...
chk_ids_version() 
{
    ...
    case "${IDS_MAJVER}" in
        5|6|7|11)  : ;;
        *)   ${ECHO} "ERROR: $PROG only works with DSEE version 5.x, 6.x, 7.x and ODSEE 11g, not ${IDS_VER}."; exit 1;;
    esac
...
get_timelimit()
{
    ...
    get_negone_num "Enter the time limit for DSEE (current=${CURR_TIMELIMIT}):" "-1"
    IDS_TIMELIMIT=$NUM
}
...
get_sizelimit()
{
    ...
    get_negone_num "Enter the size limit for DSEE (current=${CURR_SIZELIMIT}):" "-1"
    IDS_SIZELIMIT=$NUM
}
...
allow_admin_read_write_shadow()
{
    ...
    #
    # if an ACI with ${ADMIN_ACI_NAME} and "write,compare,read,search,add,delete
    # and ${LDAP_ADMINDN} already exists, we are done
    #
    PERMS="write,compare,read,search,add,delete"
    ${EGREP} ".*${ADMIN_ACI_NAME}.*(${PERMS}).*${LDAP_ADMINDN}.*" \
    ...
    #
    # If an ACI with ${ADMIN_ACI_NAME} and "write,compare,read,search" and
    # ${LDAP_ADMINDN} exists, delete it.
    #
    find_and_delete_ACI ".*${ADMIN_ACI_NAME}.*(write,compare,read,search).*${LDAP_ADMINDN}.*" \
	${TMPDIR}/chk_adminwrite_aci ${ADMIN_ACI_NAME}

    # Create the tmp file to add.
    RBAC_ATTRS="objectClass||uid||uidNumber||gidNumber||cn"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisAttrKeyValue||SolarisAttrShortDesc||SolarisAttrLongDesc"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisKernelSecurityPolicy||SolarisProfileType"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisProfileId||SolarisUserQualifier"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisReserved1||SolarisReserved2"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisAttrReserved1||SolarisAttrReserved2"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisProjectID||SolarisProjectName"
    RBAC_ATTRS="${RBAC_ATTRS}||SolarisProjectAttr||memberUid||memberGid||description"
    RBAC_ATTRS="${RBAC_ATTRS}||automountKey||automountMapName||automountInformation"
    RBAC_ATTRS="${RBAC_ATTRS}||ipTnetTemplateName||ipTnetNumber"
...
aci: (target="ldap:///${LDAP_BASEDN}")(targetattr="shadowLastChange
 ...
 ||shadowFlag||userPassword||loginShell||homeDirectory||gecos||${RBAC_ATTRS}")
  (version 3.0; acl ${ADMIN_ACI_NAME}; allow (${PERMS})
...
prompt_config_info()
{
    ...
    # Prompt for DSEE port number.
    get_ids_port

    # Check DSEE version for compatibility.
    chk_ids_version
...
#                LDAP naming services.  Use the idsconfig tool to load it.
#
...
modify_pwd_crypt()
{
    ...
    # DS 5.2 moved passwordchangesceme off to a new data structure.
...
add_vlv_indexes()
{
    ...
    # Get the instance name from DSEE server.
...
aci: (target="ldap:///${LDAP_BASEDN}")(targetattr = "shadowLastChange||
 ...
 deny (write,read,search,compare,add,delete) userdn != "ldap:///${LDAP_ADMINDN}";)
EOF
...
#
# Description:
# This script assumes that the Oracle Directory Server Enterprise
# Edition (ODSEE) is installed and that setup has been run.  This
# script takes the DSEE server from that point and sets up 
# the infrastructure for LDAP Naming Services.  After running this
# script, ldapaddent(1M) or some other tools can be used to populate
# data.
...
then 
    ...
    chk_ids_version    # Check DSEE version for compatibility.
else

...
{
	CELL *x;
	struct ofile *f, *ff = NULL;

	x = execute(b);
	getsval(x);
	for (f = files; f != NULL; f = f->next) {
		if (f->fname && wscmp(x->sval, f->fname) == 0)
			goto doit;
		if (f->fp == NULL && ff == NULL)
			ff = f;
	}
	if (ff == NULL) {
		if ((ff = calloc(1, sizeof (struct ofile))) == NULL)
			error(FATAL, "out of space in redirprint");
		ff->next = files;
		files = ff;
	}
	f = ff;
	if (a == '|')	/* a pipe! */
		f->fp = popen(toeuccode(x->sval), "w");
	else if (a == APPEND)
		f->fp = fopen(toeuccode(x->sval), "a");
	else if (a == GT)
		f->fp = fopen(toeuccode(x->sval), "w");
	else
		error(FATAL, "illegal redirection near line %lld", lineno);
	if (f->fp == NULL)
		error(FATAL, "can't open file %ws", x->sval);
	f->fname = tostring(x->sval);
	f->type = a;
doit:
	fprintf(f->fp, "%ws", s);
#ifndef gcos
	fflush(f->fp);	/* in case someone is waiting for the output */
#endif
	tempfree(x);
}

...
#define	DOORFILE		_PATH_SYSVOL "/syslog_door"
#define	PIDFILE			_PATH_SYSVOL "/syslog.pid"
...
logit(void *ap)
{
	...
	while (f->f_type != F_UNUSED) {
		...
		/*
		 * As duplicate message checking is done at an offset
		 * after the date stamp in the message, overwrite
		 * the whole of the current message buffer with the
		 * new message.
		 */
		(void) strncpy(f->f_current.msg, mp->msg, MAXLINE+1);
...
/*
 * Create the door file and the pid file in _PATH_SYSVOL.  If the filesystem
 * containing /etc is writable, create symlinks /etc/.syslog_door and
 * /etc/syslog.pid to them.  On systems that do not support _PATH_SYSVOL, create
...
init(void)
{
	...
	/*
	 * allocate thread stacks for the persistent threads
	 */
...
/*
 * conf_init - This routine is code separated from the

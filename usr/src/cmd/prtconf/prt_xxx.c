...
#if defined(__i386) || defined(__amd64)
static void isa_printregs(isa_regspec_t *, int);
static void isa_printranges(struct rangespec *, int);
static void isa_printintr(struct intrspec *, int);
static void isa_print(uintptr_t, int);
#endif
...
static struct di_priv_format ppd_format[] = {
...
#else
		"central dma ebus fhc pci pci_pci rootnex",
#endif	/* __sparc */
	...
	},
	{
		...
		sizeof (isa_regspec_t),			/* first pointer */
		offsetof(struct ddi_parent_private_data, par_reg),
		offsetof(struct ddi_parent_private_data, par_nreg),
...
#else
	{ ppd_format[2].drv_name, sbus_print},
	{ ppd_format[3].drv_name, isa_print}
#endif
};
...
#if defined(__i386) || defined(__amd64)
	(void) printf("    Bus Type=0x%x, Address=0x%"PRIx64", "
	    "Size=0x%"PRIx64"\n", rp->regspec_bustype,
...
obio_printranges(struct rangespec *rp, int ilev)
{
	indent_to_level(ilev);
#if defined(__i386) || defined(__amd64)
	(void) printf("    Ch: %.2x,%.8x Pa: %.2x,%.8x, Sz: %x %.8x\n",
	    rp->rng_caddr_high, rp->rng_caddr_low,
	    rp->rng_addr_high, rp->rng_addr_low,
	    rp->rng_size_high, rp->rng_size_low);
#else
...
#if defined(__i386) || defined(__amd64)
static void
isa_printregs(isa_regspec_t *rp, int ilev)
{
	indent_to_level(ilev);
	(void) printf("    Bus Type=0x%x, Address=0x%x, Size=0x%x\n",
	    rp->phys_hi, rp->phys_lo, rp->size);
}

/*
 * isa_parent_private_data uses the same rangespec and intrspec
 * for its ranges and interrupts as does ddi_parent_private_data.
 * If this changes in either structure then unique parsing code is
 * needed for ISA.  Otherwise, just use the obio functions for now.
 */

static void
isa_printranges(struct rangespec *rp, int ilev)
{
	obio_printranges(rp, ilev);
}

static void
isa_printintr(struct intrspec *ip, int ilev)
{
	obio_printintr(ip, ilev);
}

static void
isa_print(uintptr_t data, int ilev)
{
	int i, nreg, nrng, nintr;
	struct isa_parent_private_data *dp;
	isa_regspec_t *reg;
	struct intrspec *intr;
	struct rangespec *rng;

	dp = (struct isa_parent_private_data *)data;
#ifdef DEBUG
	dprintf("isa parent private data: nreg = 0x%x offset = 0x%x"
	    " nintr = 0x%x offset = 0x%x nrng = 0x%x offset = %x\n",
	    dp->par_nreg, *((di_off_t *)(&dp->par_reg)),
	    dp->par_nintr, *((di_off_t *)(&dp->par_intr)),
	    dp->par_nrng, *((di_off_t *)(&dp->par_rng)));
	...
	if (nreg != 0) {
		...
		reg = (isa_regspec_t *)(data + *(di_off_t *)(&dp->par_reg));
		for (i = 0; i < nreg; ++i)
			isa_printregs(reg + i, ilev);
	}

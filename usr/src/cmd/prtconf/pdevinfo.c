...
static const dumpops_t sysprop_dumpops = {
	...
	&prop_dumpops,
	(dump_nextprop_t)di_prop_vendor_next,
	NULL
}, admin_prop_dumpops = {
	&prop_dumpops,
	(dump_nextprop_t)di_prop_admin_next,
	NULL
}, vendor_globprop_dumpops = {
	&prop_dumpops,
	(dump_nextprop_t)di_prop_vendor_global_next,
	NULL
}, admin_globprop_dumpops = {
	&prop_dumpops,
	(dump_nextprop_t)di_prop_admin_global_next,
	NULL
};
...
static void dump_location_data(int, di_arg_t *, di_node_t);
...
prtconf_devinfo(void)
{
	...
	if (opts.o_location || opts.o_verbose)
		cro_hdl = di_cro_init(NULL, 0);

	di_arg.prom_hdl = prom_hdl;
	di_arg.devlink_hdl = devlink_hdl;
	di_arg.cro_hdl = cro_hdl;
...
out:	if (prom_hdl != DI_PROM_HANDLE_NIL)
...
dump_devs(di_node_t node, void *arg)
{
	...
	if (opts.o_location && !opts.o_verbose)
		dump_location_data(0, di_arg, node);
	(void) printf("\n");
	...
	if (opts.o_verbose)  {
		...
		if (opts.o_upgrade_props) {
			(void) dump_prop_list(&admin_prop_dumpops,
			    "Admin",
			    ilev + 1, node, DDI_DEV_T_NONE, NULL);

			(void) dump_prop_list(&admin_globprop_dumpops,
			    "Admin global",
			    ilev + 1, node, DDI_DEV_T_ANY, NULL);

			(void) dump_prop_list(&vendor_prop_dumpops,
			    "Vendor",
			    ilev + 1, node, DDI_DEV_T_NONE, NULL);

			(void) dump_prop_list(&vendor_globprop_dumpops,
			    "Vendor global",
			    ilev + 1, node, DDI_DEV_T_ANY, NULL);
		}

		dump_priv_data(ilev + 1, node);
		dump_location_data(ilev + 1, di_arg, node);
...
/*
 * Print any location data
 */
static void
dump_location_data(int ilev, di_arg_t *di_arg, di_node_t node)
{
	di_cro_hdl_t		cro_hdl = di_arg->cro_hdl;
	di_cro_reca_t		ra;
	di_cro_rec_t		r;
	char			*path;
	int			len;
	...
	if ((cro_hdl == NULL) || (node == DI_PATH_NIL) ||
	    ((path = di_devfs_path(node)) == NULL))
		return;

	/*
	 * di_cro_* 'occupant_devices' is a di_devfs_path with
	 * a "/devices" prefix.
	 */
	len = strlen("/devices") + strlen(path) + 1;
	occupant_devices = malloc(len);
	if (occupant_devices == NULL)
		return;
	(void) snprintf(occupant_devices, len, "/devices%s", path);

	/* find just the di_cro objects that map to occupant_devices */
	...
	ra = di_cro_reca_create_query(cro_hdl, 0, query);
...
noloc:	free(occupant_devices);
	...
	if (devchassis_path) {
		/* Print the devchassis_path location of the device */
		if (ilev)
			indent_to_level(ilev);
		(void) printf("%slocation: %s",
		    ilev ? "" : " ", devchassis_path);
		if (ilev)
			(void) printf("\n");
	}

	/* NOTE: we keep the snapshot handle for next call */
}

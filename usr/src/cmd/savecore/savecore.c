...
#define	PTOB(p)		((offset_t)(p) << dumphdr.dump_pageshift)
#define	DATAOFF(p)	(corehdr.dump_data + PTOB(p))
#define	INVALID_DATAOFF	((offset_t)-1)
...
static const struct {
	 ...
	 */
	{
		...
		_SC_PAYLOAD_CMN | SC_PAYLOAD_DUMPADM_EN |
		    SC_PAYLOAD_PANICSTACK | SC_PAYLOAD_JUSTCHECKING
	},
	...
	{
		"dump_available",
		_SC_PAYLOAD_CMN | SC_PAYLOAD_PANICSTACK |
		    SC_PAYLOAD_INSTANCE | SC_PAYLOAD_ISCOMPRESSED
	},
};
...
read_dumphdr(void)
{
	size_t	dumphdr_size;		/* size of dumphdr to read */
	int	dump_v9;		/* special case version mismatch */
	...
	if (dumphdr.dump_magic != DUMP_MAGIC)
		logprint(SC_SL_NONE | SC_IF_VERBOSE | SC_EXIT_PEND,
		    "bad magic number %x", dumphdr.dump_magic);

	if (dumphdr.dump_version != DUMP_VERSION) {
		if (dumphdr.dump_version == 9 && DUMP_VERSION > 9) {
			/* this is one special case we can handle */
			dump_v9 = 1;
			dumphdr_size = offsetof(dumphdr_t, dump_fm_panic);
		} else {
			logprint(SC_SL_NONE | SC_IF_VERBOSE | SC_EXIT_PEND,
	 ...
	 */
	Pread(dumpfd, &corehdr, dumphdr_size, dumphdr.dump_start);
	...
	if (dump_v9) {
		/*
		 * The v9 dumphdr is smaller, and doesn't contain a UUID
		 * create a NULL uuid.
		 */
		bzero(dumphdr.dump_uuid, sizeof (dumphdr.dump_uuid));
		dumphdr.dump_fm_panic = 0;
		bzero(corehdr.dump_uuid, sizeof (dumphdr.dump_uuid));
		corehdr.dump_fm_panic = 0;
	}
...
static offset_t
pfn_dataoff(const pfn_t *const pfn_table, pfn_t pfn)
{
	long first = 0;
	...
	while (last >= first) {
		middle = (first + last) / 2;
		if (pfn_table[middle] == pfn)
			return (DATAOFF(middle));
		if (pfn_table[middle] < pfn)
			first = middle + 1;
		else
			last = middle - 1;
	}
	return (INVALID_DATAOFF);
}

static void
build_dump_map(int corefd, const pfn_t *const pfn_table)
{
	long i;
	...
	for (i = 0; i < corehdr.dump_nvtop; i++) {
		...
		dataoff = pfn_dataoff(pfn_table, vtop.m_pfn);

		if (dataoff == INVALID_DATAOFF) {
			if (++misses <= 10)
				(void) fprintf(stderr,
 ...
 */
static void
ApplyPatches(FILE *dumpf, int corefd, const pfn_t *const pfn_table)
{
	char *buf = Zalloc(pagesize);
	char *out = Zalloc(pagesize);
	dump_patch_t pat;
	size_t dsize;
	offset_t dataoff;
	...
	(void) fflush(0);
	for (;;) {
		Fread(&pat, sizeof (pat), dumpf);

		if (pat.patch_magic != DUMP_PATCH_MAGIC)
			break;

		if (pat.patch_csize > pagesize) {
			logprint(SC_SL_ERR | SC_EXIT_ERR,
			    "ApplyPatches: csize %ld > %d",
			    pat.patch_csize, pagesize);
			break;
		}

		if (pat.patch_csize == 0)
			break;

		Fread(buf, pat.patch_csize, dumpf);
		if (pat.patch_sum != checksum32(buf, pat.patch_csize))
			logprint(SC_SL_ERR | SC_EXIT_ERR,
			    "ApplyPatches: bad data checksum");

		dsize = decompress(buf, out, pat.patch_csize, pagesize);
		if (dsize != pagesize)
			logprint(SC_SL_ERR | SC_EXIT_ERR,
			    "ApplyPatches: dsize %d != pagesize %d",
			    dsize, pagesize);

		/* lookup pfn offset */
		dataoff = pfn_dataoff(pfn_table, pat.patch_pfn);
		if (dataoff == INVALID_DATAOFF)
			logprint(SC_SL_ERR | SC_EXIT_ERR,
			    "ApplyPatches: pfn %ld not found", pat.patch_pfn);

		/* overwrite older data */
		Pwrite(corefd, out, pat.patch_dsize, dataoff + pat.patch_off);
	}
	free(buf);
	free(out);
}
 ...
 */
static void
decompress_pages(int corefd, const pfn_t *const pfn_table)
{
	char *cpage = NULL;
	char *dpage = NULL;
	char *out;
	...
	/* optional patches follow stream data */
	ApplyPatches(dumpf, corefd, pfn_table);
...
build_corefile(const char *namelist, const char *corefile)
{
	 ...
	 */
	decompress_pages(corefd, pfn_table);
...
main(int argc, char *argv[])
{
	 ...
	 */
	if (!filemode && dumphdr.dump_panicstring[0] != '\0') {

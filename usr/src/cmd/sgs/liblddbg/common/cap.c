    ...
    const char *ptr)
{
	...
	dbg_print(lml, MSG_INTL(MSG_CAP_SEC_ENTRY), Dbg_state_str(dbg_state),
	    conv_cap_tag(tag, 0, &inv_buf), ptr ? ptr : MSG_INTL(MSG_STR_NULL));
}
...
void
Dbg_cap_mapfile_title(Lm_list *lml, const char *file, Lineno lineno)
{
	if (DBG_NOTCLASS(DBG_C_MAP | DBG_C_CAP))
		return;

	dbg_print(lml, MSG_INTL(MSG_MAP_CAP), file, EC_LINENO(lineno));
}

void
Dbg_cap_id(Lm_list *lml, const char *file, Lineno lineno, const char *oid,
    const char *nid)
{
	Dbg_cap_mapfile_title(lml, file, lineno);

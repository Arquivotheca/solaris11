...
Dbg_reloc_doact(Lm_list *lml, int caller, Half mach, Word type, Rel_desc *rdesc,
    uintptr_t off, Xword value, rel_desc_sname_func_t rel_desc_sname_func,
    const char *prefix)
{
	...
	if (prefix == NULL)
		prefix = MSG_ORIG(MSG_STR_EMPTY);

	Elf_reloc_entry_2(lml, caller, prefix, type,
...
/*
 * Relocations that transition TLS instructions, or that are discarded or
 * ignored, are handled by this function to provide consistent disgnostics.
 */
static void
_Dbg_reloc_special(Lm_list *lml, const char *msg1, const char *msg2,
    Half mach, Word type, Rel_desc *rsp, uintptr_t offset,
    rel_desc_sname_func_t rel_desc_sname_func, const char *rstr)
{
	...
	/*
	 * First, indicate the original relocation and it's associated offset.
	 */
	Dbg_reloc_doact(lml, ELF_DBG_LD_ACT, mach, type, rsp, offset, 0,
	    rel_desc_sname_func, msg1);

	isp = rsp->rel_isdesc;
	dbg_print(lml, msg2,
	    conv_reloc_type(mach, rsp->rel_rtype, 0, &inv_buf),
	    dbg_fmt_isec_name(isp, buf, &alloc_mem), isp->is_file->ifl_name,
	    EC_OFF(rsp->rel_roffset), (*rel_desc_sname_func)(rsp), rstr);
	if (alloc_mem != NULL)
		free(alloc_mem);
}

void
Dbg_reloc_transition(Lm_list *lml, Half mach, Word type, Rel_desc *rsp,
    uintptr_t offset, rel_desc_sname_func_t rel_desc_sname_func, Word rtype)
{
	Conv_inv_buf_t		inv_buf;

	_Dbg_reloc_special(lml, MSG_INTL(MSG_STR_TRANSITION),
	    MSG_INTL(MSG_REL_TRANSITION), mach, type, rsp, offset,
	    rel_desc_sname_func, conv_reloc_type(mach, rtype, 0, &inv_buf));
}

void
Dbg_reloc_discard(Lm_list *lml, Half mach, Word type, Rel_desc *rsp,
    uintptr_t offset, rel_desc_sname_func_t rel_desc_sname_func)
{
	_Dbg_reloc_special(lml, MSG_INTL(MSG_STR_DISCARD),
	    MSG_INTL(MSG_REL_DISCARDED), mach, type, rsp, offset,
	    rel_desc_sname_func, NULL);
}

void
Dbg_reloc_ignored(Lm_list *lml, Half mach, Word type, Rel_desc *rsp,
    uintptr_t offset, rel_desc_sname_func_t rel_desc_sname_func)
{
	_Dbg_reloc_special(lml, MSG_INTL(MSG_STR_IGN),
	    MSG_INTL(MSG_REL_IGNORED), mach, type, rsp, offset,
	    rel_desc_sname_func, NULL);
}
...
void
Dbg_reloc_badalign(Lm_list *lml, const char *reloc_name, const char *file,
    const char *sname, uintptr_t offset, Word align)
{
	if (DBG_NOTCLASS(DBG_C_RELOC))
		return;

	dbg_print(lml, MSG_INTL(MSG_REL_NONALIGN), reloc_name, file,
	    Dbg_demangle_name(sname), EC_ADDR(offset), EC_WORD(align));
}


/*
 * Announce that -z relaxreloc has been enabled and the reason for doing so.
 *
 * entry:
 *	lml - Link map control list
 *	type - DBG_RLXREL_* value giving reason.
 *	file - If reason is not DBG_RLXREL_ARG, name of input file triggering
 *		this change. Otherwise ignored.
 *	section - If reason is one of DBG_RLXREL_LINKONCE or
 *		DBG_RLXREL_GASSHDRNAM, pointer to input section descriptor
 *		of the triggering section. Otherwise ignored.
 */
void
Dbg_reloc_relax(Lm_list *lml, int type, const char *file,
    Is_desc *isp)
{
#if DBG_RLXREL_NUM != (DBG_RLXREL_GNUGRP + 1)
#error DBG_RLXREL_NUM has grown
#endif
	...
	if (DBG_NOTCLASS(DBG_C_BASIC | DBG_C_RELOC | DBG_C_SECTIONS))
		return;

	switch (type) {
	case DBG_RLXREL_ARG:
		dbg_print(lml, MSG_INTL(MSG_REL_RLX_ARG));
		break;

	case DBG_RLXREL_LINKONCE:
	case DBG_RLXREL_GASCOMDAT:
		fmt = (type == DBG_RLXREL_LINKONCE) ?
		    MSG_INTL(MSG_REL_RLX_LINKONCE) :
		    MSG_INTL(MSG_REL_RLX_GASCOMDAT);
	...
	case DBG_RLXREL_GRPCMT:
		dbg_print(lml, MSG_INTL(MSG_REL_RLX_GRPCMT), file);
		break;
	}
}

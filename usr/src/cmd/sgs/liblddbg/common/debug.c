...
/*
 * Define a debug descriptor.  Although this provides the default definition to
 * which users can bind, ld.so.1 and libld.so both provide their own definition
 * to ensure liblddbg's loading is deferred unless debugging is explicitly
 * required.  Thus interposition is expected.  This item, and dbg_print(),
 * should be defined NODIRECT.
 */
static Dbg_desc	_dbg_desc = { 0 };
...
dbg_print(Lm_list *lml, const char *format, ...)
{
	 ...
	 * Suppress the lint error by making a dummy assignment.
	 */
	lml = 0;
#endif
	va_start(ap, format);
...
Dbg_state_str(dbg_state_t type)
{
	static const Msg state[DBG_STATE_NUM] = {
		...
		MSG_STR_IGNORED,	/* MSG_INTL(MSG_STR_IGNORED) */

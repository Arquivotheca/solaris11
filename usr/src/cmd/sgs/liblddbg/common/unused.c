...
void
Dbg_unused_strip(Lm_list *lml, const char *name, Ifl_desc *ifl, Word scnndx,
    Xword size, const char *class)
{
	dbg_isec_name_buf_t	buf;
	char			*alloc_mem;
	const char		*fmt;
	...
	if (class)
		fmt = MSG_INTL(MSG_USD_STRIPPED_C);
	else
		fmt = MSG_INTL(MSG_USD_STRIPPED_D);

	dbg_print(lml, fmt, dbg_fmt_isec_name2(name, scnndx, buf, &alloc_mem),
	    EC_XWORD(size), ifl->ifl_name, class);
}

...
void
Dbg_sec_gnu_comdat(Lm_list *lml, int type, Is_desc *isp)
{
#if DBG_GNUCOMDAT_NUM != (DBG_GNUCOMDAT_GROUP + 1)
#error DBG_GNUCOMDAT_NUM has grown
#endif
	...
	switch (type) {
	case DBG_GNUCOMDAT_LINKONCE:
		fmt = MSG_INTL(MSG_SEC_GNU_LINKONCE);
		break;
	case DBG_GNUCOMDAT_GROUP:
		fmt = MSG_INTL(MSG_SEC_GNU_GRPCOMDAT);
		break;
	default:
		return;
	}

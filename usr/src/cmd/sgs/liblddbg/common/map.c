    ...
    const char *file, Lineno lineno)
{
	...
	if (version)
		dbg_print(lml, MSG_INTL(MSG_MAP_SYM_VER_1), file,
		    EC_LINENO(lineno), version, Dbg_demangle_name(name), scp);
	else
		dbg_print(lml, MSG_INTL(MSG_MAP_SYM_VER_2), file,
		    EC_LINENO(lineno), Dbg_demangle_name(name), scp);
}
    ...
    const char *file, Lineno lineno)
{
	...
	dbg_print(lml, MSG_INTL(MSG_MAP_SYM_SIZE), file, EC_LINENO(lineno),
	    segname, Dbg_demangle_name(symname), MSG_INTL(MSG_STR_ADDING));
}
    ...
    const char *file, Lineno lineno)
{
	...
	Dbg_util_nl(lml, DBG_NL_STD);
	dbg_print(lml, MSG_INTL(MSG_MAP_SYM_SIZE), file, EC_LINENO(lineno),
	    segname, sdp->sd_name, MSG_INTL(MSG_STR_UP_1));
 ...
 */
void
Dbg_map_dv(Lm_list *lml, const char *obj_name, const char *file, Lineno lineno)
{
	if (DBG_NOTCLASS(DBG_C_MAP))
		return;

	dbg_print(lml, MSG_INTL(MSG_MAP_DV), file, EC_LINENO(lineno), obj_name);
}
 ...
 */
void
Dbg_map_dv_entry(Lm_list *lml, const char *file, Lineno lineno, int require,
    const char *version)
{
	const char *attr;
	...
	dbg_print(lml, MSG_INTL(MSG_MAP_DV_ENTRY), file, EC_LINENO(lineno),
	    attr, version);
}
...
void
Dbg_map_ent(Lm_list *lml, Ent_desc *enp, Ofl_desc *ofl, const char *file,
    Lineno lineno)
{
	if (DBG_NOTCLASS(DBG_C_MAP | DBG_C_ENTRY))
		return;

	Dbg_util_nl(lml, DBG_NL_STD);
	dbg_print(lml, MSG_INTL(MSG_MAP_EC), file, EC_LINENO(lineno));
...
Dbg_map_seg_os_order(Lm_list *lml, Sg_desc *sgp, const char *sec_name,
    Word ndx, const char *file, Lineno lineno)
{
	if (DBG_NOTCLASS(DBG_C_MAP | DBG_C_SECTIONS))
		return;

	dbg_print(lml, MSG_INTL(MSG_MAP_OS_ORDER), file, EC_LINENO(lineno),
    ...
    const char *file, Lineno lineno)
{
	...
	if (DBG_NOTCLASS(DBG_C_MAP | DBG_C_SEGMENTS))
		return;

	Dbg_util_nl(lml, DBG_NL_STD);
	dbg_print(lml, MSG_INTL(MSG_MAP_SEG), file, EC_LINENO(lineno),
...
Dbg_map_seg_order(Ofl_desc *ofl, uchar_t osabi, Half mach,
    dbg_state_t dbg_state, const char *file, Lineno lineno)
{
	Lm_list	*lml = ofl->ofl_lml;
	Aliste		idx;
	Sg_desc		*sgp;
	...
	Dbg_util_nl(lml, DBG_NL_STD);
	dbg_print(lml, MSG_INTL(MSG_MAP_SEG_ORDER), file, EC_LINENO(lineno),
...
void
Dbg_map_hdr_noalloc(Lm_list *lml, const char *file, Lineno lineno)
{
	if (DBG_NOTCLASS(DBG_C_MAP | DBG_C_SEGMENTS))
		return;

	Dbg_util_nl(lml, DBG_NL_STD);
	dbg_print(lml, MSG_INTL(MSG_MAP_HDR_NOALLOC), file, EC_LINENO(lineno));
}

void
Dbg_map_stub(Lm_list *lml, const char *file, Lineno lineno)
{
	if (DBG_NOTCLASS(DBG_C_MAP))
		return;

	Dbg_util_nl(lml, DBG_NL_STD);
	dbg_print(lml, MSG_INTL(MSG_MAP_STUB_OBJECT), file, EC_LINENO(lineno));
}

...
sections(const char *file, Cache *cache, Word shnum, Ehdr *ehdr, uchar_t osabi)
{
	...
	for (seccnt = 1; seccnt < shnum; seccnt++) {
		...
		if (ehdr->e_type == ET_REL) {
			if (strcmp(secname, MSG_ORIG(MSG_ELF_GOT)) == 0) {
				(void) fprintf(stderr,
		...
		/*
		 * Alignment checks:
		 * -    sh_addralign must be 0 or a power of 2
		 * -    sh_addr should conform to the given alignment
		 */
		if ((shdr->sh_addralign != 0) &&
		    (shdr->sh_addralign & (shdr->sh_addralign - 1)))
			(void) fprintf(stderr, MSG_INTL(MSG_ERR_BADSHADDRAL),
			    file, secname, EC_XWORD(shdr->sh_addralign));
		else if (shdr->sh_addr !=
		    S_ROUND(shdr->sh_addr, shdr->sh_addralign))
			(void) fprintf(stderr, MSG_INTL(MSG_ERR_BADSHADDR),
			    file, secname);
...
/*
 * Locate the GOT section, if present. Returns pointer to cache entry for
 * section if found, and NULL otherwise.
 */
static Cache *
find_got(Cache *cache, Word shnum)
{
	Word		cnt;

	for (cnt = 1; cnt < shnum; cnt++)
		if (strcmp(cache[cnt].c_name, MSG_ORIG(MSG_ELF_GOT)) == 0)
			return (&cache[cnt]);
...
#define	MSG_UNW_BINSRTABENT	MSG_UNW_BINSRTABENT_32
	...
	if ((uphdr && (shdr->sh_addr == uphdr->p_vaddr)) ||
		...
		frame_ptr = elfdump_ehe_extract(file, _cache->c_name,
		    _cache->c_ndx, data, data, &ndx, frame_ptr_enc,
		    ehdr->e_ident, shdr->sh_addr + ndx, shdr->sh_addr);
		if (eh_state->hdr_cnt == 1) {
		...
		fde_cnt = elfdump_ehe_extract(file, _cache->c_name,
		    _cache->c_ndx, data, data, &ndx, fde_cnt_enc, ehdr->e_ident,
		    shdr->sh_addr + ndx, shdr->sh_addr);
		...
		for (tabndx = 0; tabndx < fde_cnt; tabndx++) {
			initloc = elfdump_ehe_extract(file, _cache->c_name,
			    _cache->c_ndx, data, data, &ndx, table_enc,
			    ehdr->e_ident, shdr->sh_addr + ndx, shdr->sh_addr);
			...
			dbg_print(0, MSG_ORIG(MSG_UNW_BINSRTABENT),
			    EC_XWORD(initloc),
			    EC_XWORD(elfdump_ehe_extract(file, _cache->c_name,
			    _cache->c_ndx, data, data, &ndx, table_enc,
			    ehdr->e_ident, shdr->sh_addr+ ndx, shdr->sh_addr)));
			initloc0 = initloc;
		}
		...
		dump_eh_frame(file, _cache->c_name, _cache->c_ndx, data,
		    datasize, shdr->sh_addr, eh_state->got_addr,
    ...
    const char *file, Elf *elf, uint_t flags)
{
	...
	/*
	 * Decoding DW_EH_PE_datarel data items in GNU unwind data
	 * can require the address of the GOT. We have to do this in
	 * a separate pass before we start processing unwind sections,
	 * because the GOT section may come after the frame, and we won't
	 * find it in time.
	 */
	if ((gotcache = find_got(cache, shnum)) != NULL)
		eh_state.got_addr = gotcache->c_shdr->sh_addr;
...
#error "STT_NUM has grown. Update addr_symtype[]"
	 ...
	 * For shared objects, it is the offset relative to
...
group(Cache *cache, Word shnum, const char *file, uint_t flags)
{
	...
	for (scnt = 1; scnt < shnum; scnt++) {
		...
		for (gcnt = 1; gcnt < grpcnt; gcnt++) {
			...
			if (grpdata[gcnt] == 0) {
				(void) printf(MSG_ORIG(MSG_GRP_ENTRY0), index);
			} else {
				...
				(void) printf(MSG_ORIG(MSG_GRP_ENTRY), index,
				    name, EC_XWORD(grpdata[gcnt]));
			}
		}
	}
}

...
static size_t
wrt(Elf *elf, Xword outsz, uint_t fill, int update_cmd)
{
	NOTE(ASSUMING_PROTECTED(*elf))
	Elf_Data		dst, src;
	Xword			hi, sz;
	...
	Elf_Scn			*s;
	Ehdr			*eh = elf->ed_ehdr;
	uint_t			encode, ver = eh->e_version;
	int			byte;
	...
	if ((image = _elf_outmap(elf, outsz)) == NULL)
		return (0);
	...
	for (s = elf->ed_hdscn; s != 0; s = s->s_next) {
		 ...
		 * section and the beginning of this section.
		 */
	...
	if (_elf_outsync(elf, image, outsz) != 0) {
		elf->ed_uflags &= ~ELF_F_DIRTY;
		return (outsz);
	}
	...
	case ELF_C_WRITE:
		...
		if (elf->ed_wrimage) {
			if (elf->ed_myflags & EDF_WRALLOC) {
				...
				/*
				 * The size is still returned even though
				 * nothing is actually written out.  This is
			...
			sz = _elf_outsync(elf, elf->ed_wrimage,
			    elf->ed_wrimagesz);
			elf->ed_wrimage = 0;
			elf->ed_wrimagesz = 0;
			ELFUNLOCK(elf);

...
/* conv_dyn_flag1() */
#define	CONV_DYN_FLAG1_BUFSIZE		371
typedef union {
	Conv_inv_buf_t			inv_buf;
...
/* conv_cnote_sysset() */
#define	CONV_CNOTE_SYSSET_BUFSIZE	3154
typedef union {
	Conv_inv_buf_t			inv_buf;
...
typedef struct {
	...
	Boolean		csl_found;	/* Set to TRUE if item found */
...
extern	conv_iter_ret_t	_conv_iter_vd2(conv_iter_osabi_t, Half,
			    ...
			    const char *);
extern	Boolean		conv_iter_strtol_init(const char *,
			    conv_strtol_uvalue_t *);
...
extern	void		conv_str_to_c_literal(const char *buf, size_t n,
			    ...
			    void *uvalue);
extern	const char	*conv_sym_info_bind(uchar_t, Half, uchar_t,
...
extern	conv_iter_ret_t	conv_iter_sym_info_bind(conv_iter_osabi_t, Half,
			    Conv_fmt_flags_t, conv_iter_cb_t, void *);

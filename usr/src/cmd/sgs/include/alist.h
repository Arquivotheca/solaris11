 ...
 *		each item.
 *	ALIST_TRAVERSE_FROM_IDX: Same as ALIST_TRAVERSE, but allows caller
 *		to specify the start index instead of using 0.
 ...
 *	IDX_START - Initial value to assign to IDX.
...
#define	ALIST_TRAVERSE_FROM_IDX(LIST, IDX, IDX_START, DATA)	\
	(IDX) = (IDX_START),					\
	((LIST) != NULL) && ((DATA) = \
	(void *) (((LIST)->al_size * (IDX)) + (char *)(LIST)->al_data)); \
	\
	((LIST) != NULL) && ((IDX) < (LIST)->al_nitems); \
	\
	(IDX)++, \

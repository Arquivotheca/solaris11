...
/*
 * Determine if a file is a shared object based on its file path.
 ...
 *	dl_hdl - NULL, or a handle to a shared object to release via
 *		dlclose().
 *	dl_path - If dl_hdl is non-NULL, the path to the shared object
 ...
 *	memory or unmapping the shared object. It is therefore safe to
 *	use strings from moddef, or from the shared object (which will
...
/*
 * Load a module shared object for load_module().
 *
 * entry:
...
elfedit_load_module(const char *name, int must_exist, int allow_abs)
{
	...
	if (!is_abs_path) {
		 ...
		 * care of this below, after the shared object is loaded.
		 */
	}
...
/*
 * Load all shared objects found in the specified directory.
 *
 * entry:
...
establish_modpath(const char *cmdline_path)
{
	...
	*src = '\0';
	 ...
	 * add new codes in the future without backward compatibility issues.
	 */
...
elfedit_cpl_command(void *cpldata)
{
	...
	if (modlist != NULL) {
		 ...
		 * of the preceding test that found it. Therefore, we do

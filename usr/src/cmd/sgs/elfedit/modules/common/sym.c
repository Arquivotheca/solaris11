	...
	case SYM_CMD_T_ST_BIND:
		{
			...
			for (sym += ndx; cnt--; sym++) {
				...
				if (outstyle == ELFEDIT_OUTSTYLE_SIMPLE) {
					uchar_t osabi =	argstate->obj_state->
					    os_ehdr->e_ident[EI_OSABI];
					Half mach = 	argstate->obj_state->
					    os_ehdr->e_machine;

					elfedit_printf(MSG_ORIG(MSG_FMT_STRNL),
					    conv_sym_info_bind(osabi, mach,
					    value, CONV_FMT_ALT_CF, &inv_buf));
				} else {
					elfedit_printf(
...
cmd_body_set_st_bind(ARGSTATE *argstate, SYMSTATE *symstate)
{
	...
	uchar_t 	osabi = argstate->obj_state->os_ehdr->e_ident[EI_OSABI];
	Half		mach = argstate->obj_state->os_ehdr->e_machine;
	...
	if (old_bind == bind) {
		elfedit_msg(ELFEDIT_MSG_DEBUG, MSG_INTL(MSG_DEBUG_S_OK),
		    ...
		    conv_sym_info_bind(osabi, mach, bind, CONV_FMT_ALT_CF,
		    &inv_buf1));
	} else {
		/*
		...
		elfedit_msg(ELFEDIT_MSG_DEBUG, MSG_INTL(MSG_DEBUG_S_CHG),
		    symstate->sym.sec->sec_shndx, symstate->sym.sec->sec_name,
		    EC_WORD(symstate->ndx), MSG_ORIG(MSG_CMD_ST_BIND),
		    conv_sym_info_bind(osabi, mach, old_bind, CONV_FMT_ALT_CF,
		    &inv_buf1),
		    conv_sym_info_bind(osabi, mach, bind, CONV_FMT_ALT_CF,
		    &inv_buf2));

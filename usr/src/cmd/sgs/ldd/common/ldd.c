...
elf_check(int nfile, char *fname, char *cname, Elf *elf, int fflag)
{
	...
	for (cnt = 0; cnt < (int)ehdr.e_phnum; cnt++) {
		 ...
		 * does not live under /lib, or /usr/lib, then don't allow ldd
		 * to execute the image.  This prevents someone creating a
	 ...
	 */
	if (dynamic) {
		GElf_Xword	dt_flags_1 = _gelf_getdyndtflags_1(elf);

		if (dt_flags_1 & DF_1_NOHDR) {
			(void) fprintf(stderr, MSG_INTL(MSG_USP_NOHDR), cname,
			    fname);
			return (1);
		}

		if (dt_flags_1 & DF_1_STUB) {
			(void) fprintf(stderr, MSG_INTL(MSG_USP_NOSTUB),
			    cname, fname);
			return (1);
		}
	}

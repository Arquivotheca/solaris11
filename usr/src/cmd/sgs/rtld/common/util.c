...
load_completion(Rt_map *nlmp)
{
	...
	/*
	 * If there's no link map, or this object is involved in constructing
	 * a configuration file, then don't try and fire any .init's.
	 */
	if ((nlmp == NULL) || (MODE(nlmp) & RTLD_CONFGEN))
		return;
	 ...
	 * tsorting may start from the last link-map in existance prior to a
	 * load operation, rather than from the link-map that may have
	 ...
	 * initialization.
	 */
	if (nlml->lm_init && (((nlml->lm_flags & LML_FLG_BASELM) == 0) ||
	 ...
	 */
	if ((nlml->lm_flags & LML_FLG_BASELM) == 0) {
	 ...
	 */
	if (nlml->lm_tls && (((nlml->lm_flags & LML_FLG_BASELM) == 0) ||
	    (rtld_flags2 & (RT_FL2_PLMSETUP | RT_FL2_NOPLM)))) {
		Rt_map	*lmp;
...
veprintf(Lm_list *lml, Error error, const char *format, va_list args)
{
	...
	if (error > ERR_NONE) {
		...
		if (procname) {
			if (bufprint(&prf, MSG_ORIG(MSG_STR_EMSGFOR3),

 ...
 */
static Rt_map *
elf_obj_init(Lm_list *lml, Aliste lmco, const char *oname, Rej_desc *rej)
{
	Ofl_desc	*ofl;
	const char	*name;
	size_t		lmsz;
	...
	ofl->ofl_flags = (FLG_OF_DYNAMIC | FLG_OF_SHAROBJ);
	ofl->ofl_flags1 = (FLG_OF1_RELDYN | FLG_OF1_TEXTOFF | FLG_OF1_MEMORY);
	ofl->ofl_stripflags = FLG_OFS_ALL;
	ofl->ofl_lml = lml;

	/*
	 * Ensure ld.so.1 is initialized for plt relocations, and generate a
	 * symbol probe to test for, and load the necessary support library
	 * (libld.so).  Then configure libld.so to process objects of the
	 * desired target type.
	 */
	if ((elf_rtld_load() == 0) || (dlsym_intn(RTLD_PROBE,
	    MSG_ORIG(MSG_SUP_LDINITTARGET), lml_rtld.lm_head, &dlmp) == NULL) ||
	    (ld_init_target(lml, M_MACH) != 0)) {
		if (rej) {
			rej->rej_type = SGS_REJ_STR;
			rej->rej_str = MSG_INTL(MSG_SUP_FAILED);
			rej->rej_name = name;
		}
		free(ofl);
		return (NULL);
	...
	/*
	 * Obtain a generic set of entrance criteria, and provide an allocation
	 * routine that will be used to create the in-memory ELF image.  libelf
	 * calls calloc() by default, but the memory image needs to execute
	 * access.  Generate a link map place holder and use the ELFPRV()
	 * element to maintain the output file descriptor.
	 */
	lmsz = S_DROUND(sizeof (Rt_map)) + sizeof (Rt_elfp);
	if ((ld_ent_setup(ofl, syspagsz, xcalloc) == S_ERROR) ||
...
elf_obj_file(Lm_list *lml, Aliste lmco, Rt_map *clmp, const char *name,
    mmapobj_result_t *hmpp, mmapobj_result_t *mpp, uint_t mnum, Rej_desc *rej)
{
	Mmap_desc	md;
	 ...
	 */
	if ((olmp == NULL) &&
	    ((olmp = elf_obj_init(lml, lmco, name, rej)) == NULL))
		return (NULL);
...
elf_obj_fini(Lm_list *lml, Rt_map *lmp, Rt_map *clmp, int *in_nfavl)
{
	...
	if ((nlmp = elf_new_lmp(lml, CNTL(olmp), &fd, (Addr)hmpp->mr_addr,
	    ofl->ofl_size, NULL, clmp, &rej, in_nfavl)) == NULL)
		return (NULL);

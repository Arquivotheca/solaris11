    ...
    uint_t rdflags, uint_t cdflags)
{
	...
	if (ghp == NULL) {
		 ...
		 * dlopen(0)), and handles associated with ld.so.1, can have a
		 * reference count of 0.  However, these handles are never
		 * deleted, and therefore are never moved to the orphan list.
		 */
		if ((ghp->gh_refcnt++ == 0) &&
		    ((ghp->gh_flags & (GPH_ZERO | GPH_LDSO)) == 0)) {
			uint_t	ndx;
...
dlclose_intn(Grp_hdl *ghp, Rt_map *clmp)
{
	...
	/*
	 * If objects were loaded as part of processing this request, finish
	 * initializing these objects.
	 */
    ...
    uint_t flags, uint_t orig)
{
	...
	if (ghp) {
		...
		if ((flags & FLG_RT_AUDIT) && (LIST(dlmp) != lml)) {
			remove_lml(lml);
			lml = LIST(dlmp);
		}
	}
...
dlmopen_check(Lm_list *lml, const char *path, int mode, Rt_map *clmp)
{
	...
	if ((ghp = dlmopen_intn(lml, path, mode, clmp, 0, 0)) != NULL)
		load_completion(ghp->gh_ownlmp);
...
dlsym_intn(void *handle, const char *name, Rt_map *clmp, Rt_map **dlmp)
{
	...
	/*
	 * If objects were loaded to satisfy this request, finish initializing
	 * these objects.
	 */
    ...
    int flags)
{
	...
	if ((elf_rtld_load() == 0) || (dlsym_intn(RTLD_PROBE,
	    MSG_ORIG(MSG_SUP_RTDLDUMP), lml_rtld.lm_head, &dlmp) == NULL)) {
		eprintf(lml, ERR_FATAL, MSG_INTL(MSG_SUP_FAILED));
		return (0);
	}

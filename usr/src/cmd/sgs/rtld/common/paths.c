	...
	case RPLENV:
		...
		if (rpl_libpath) {
			...
			if (rpl_libdirs == NULL) {
				...
				/*
				 * If the path was reexpanded for diagnostics,
				 * reinstate the original path to preserve any
				 * flags that might have been set.
				 */
				if (dalp) {
...
{
	char	*str, *olist = NULL, *nlist = (char *)list;
	...
	for (str = nlist; *nlist || fnull; str = nlist) {
		...
		/*
		 * If this is a secure application, validation of the expanded

...
elf_new_lmp(Lm_list *lml, Aliste lmco, Fdesc *fdp, Addr addr, size_t msize,
    void *odyn, Rt_map *clmp, Rej_desc *rej, int *in_nfavl)
{
	...
	if (dyn) {
		...
		for (dynndx = 0, pdyn = NULL, dip = DYNINFO(lmp);
			...
			case DT_FLAGS:
				...
				break;
			case DT_FLAGS_1:
				/* Refuse to load a stub object */
				if (dyn->d_un.d_val & DF_1_STUB) {
					if (rej) {
						rej->rej_type = SGS_REJ_STUB;
						rej->rej_name = fdp->fd_nname;
					}

 ...
 *
 * LAV_VERSION5 (Solaris 10 update 10)
    ...
    int *ndx)
{
	...
	for (APLIST_TRAVERSE(list, idx, alp)) {
		...
		if (flags & LA_FLG_BINDFROM) {
			...
			if ((aip->ai_dynplts = xcalloc(pltcnt,
...
audit_setup(Rt_map *clmp, Audit_desc *adp, uint_t orig, int *in_nfavl)
{
	...
	for (ptr = strtok_r(adp->ad_name, MSG_ORIG(MSG_STR_DELIMIT), &next);
		...
		/*
		 * Having verified that this object is a valid auditor, finish
		 * initializing the objects loaded with this request.
		 */
		load_completion(lmp);

		/*
		 * Now that the auditor is fully relocated and initialized,
		 * interact with the auditing routines.
		 */
		APPLICATION_ENTER(rtldflags);

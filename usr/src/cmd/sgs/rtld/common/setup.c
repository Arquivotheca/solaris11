...
preload(const char *str, Rt_map *mlmp, Rt_map **clmp)
{
	...
	/*
	 * Duplicate any preload string in preparation for strtok() processing.
	 */
	...
	/*
	 * The preload definitions may be a list.  Process each preload object.
	 */
	for (ptr = strtok_r(objs, MSG_ORIG(MSG_STR_DELIMIT), &next);
	    ptr != NULL; ptr = strtok_r(NULL, MSG_ORIG(MSG_STR_DELIMIT),
	    &next)) {
		Rt_map	*nlmp = NULL;
		uint_t	flags;
...
setup(char **envp, auxv_t *auxv, Word _flags, char *_platform, int _syspagsz,
    ...
    char *execname, char **argv, void *aoutdyn, int auxflags, uint_t hwcap_1)
{
	...
	if (fd != -1) {
		...
		if (aoutdyn) {
			...
			if ((mlmp = aout_new_lmp(&lml_main, ALIST_OFF_DATA,
			    &fdm, 0, 0, aoutdyn, NULL, &rej, NULL)) == NULL)
				return (0);
			...
			if ((mlmp = elf_new_lmp(&lml_main, ALIST_OFF_DATA,
				...
				if (rej.rej_type != SGS_REJ_NONE)
					eprintf(&lml_main, ERR_FATAL,
					    MSG_INTL(err_reject[rej.rej_type]),
					    argvname, conv_reject_desc(&rej,
					    &rej_buf, M_MACH));
				return (0);
			}
	...
	if (cap_check_lmp(mlmp, &rej) == 0) {
		...
		if (lml_main.lm_flags & LML_FLG_TRC_ENABLE) {
			...
			(void) printf(MSG_INTL(ldd_warn[rej.rej_type]),
			    NAME(mlmp), conv_reject_desc(&rej,
			    &rej_buf, M_MACH));
		} else {
			/* LINTED */
			eprintf(&lml_main, ERR_FATAL,
			    MSG_INTL(err_reject[rej.rej_type]), NAME(mlmp),
			    conv_reject_desc(&rej, &rej_buf, M_MACH));
			return (0);
		}
	}

...
/*ARGSUSED*/
static void
write_member_header(const char *filename, int fd, int verify_elf_padding,
    ...
    size_t size)
{
	 ...
	 */
	assert(!verify_elf_padding ||
	    (pad(lseek(fd, 0, SEEK_CUR), PADSZ) == 0));
}
...
writefile(Cmd_info *cmd_info)
{
	 ...
	 * preceding special member will be the symbol table, or the long
	...
	for (fptr = listhead; fptr; fptr = fptr->ar_next) {
		...
		write_member_header(name, ar_outfile.fd, prev_is_elf && is_elf,

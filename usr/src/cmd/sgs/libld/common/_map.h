...
/* State of a mapfile symbol ASSERT attribute */
typedef uchar_t msa_flag_t;
typedef uchar_t msa_sh_attr_t;
typedef struct {
	Sym_desc	*msa_sdp;	/* Associated symbol descriptor */
	const char	*msa_mfname;	/* Name of mapfile containing ASSERT */
	Lineno		msa_mflineno;	/* Line number ASSERT starts on */
	msa_flag_t	msa_flags;	/* Asserted symbol attributes */
	uchar_t		msa_bind; 	/* Asserted symbol binding */
	uchar_t 	msa_type;	/* Asserted symbol type (STT_xxx) */
	msa_sh_attr_t	msa_sh_attr;	/* Asserted section attributes */
	Addr		msa_size;	/* Asserted symbol size */
	const char	*msa_alias;	/* Asserted: named symbol has same */
					/*	address, size, and type */
} ld_map_sym_assert_t;

#define	FLG_MSA_ALIAS	0x01	/* assert name of alias symbol */
#define	FLG_MSA_SH_ATTR	0x02	/* assert symbol section attributes */
#define	FLG_MSA_BIND	0x04	/* assert symbol binding */
#define	FLG_MSA_SIZE	0x08	/* assert symbol size */
#define	FLG_MSA_TYPE	0x10	/* assert symbol type */

/*
 * Additional section attributes asserted for symbols (msa_sh_attr)
 */
#define	SH_ATTR_MSA_BITS	0x01	/* assert symbol not in NOBITS sec */
#define	SH_ATTR_MSA_NOBITS	0x02	/* assert symbol is in NOBITS section */
...
extern Boolean		ld_map_sym_enter(Mapfile *, ld_map_ver_t *,
			    ld_map_sym_t *, ld_map_sym_assert_t *);

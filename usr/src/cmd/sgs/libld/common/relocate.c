    ...
    Ofl_desc *ofl)
{
	...
	if ((ssvp = bsearch((void *)&value, (void *)ifl->ifl_sortsyms,
	    ifl->ifl_sortcnt, sizeof (Ssv_desc), &disp_bsearch)) != NULL)
		tsdp = ssvp->ssv_sdp;
	else
		tsdp = NULL;
...
ld_disp_errmsg(const char *msg, Rel_desc *rsp, Ofl_desc *ofl)
{
	...
	if ((sdp = disp_scansyms(ifl, rsp, 0, 1, ofl)) != NULL)
...
disp_inspect(Ofl_desc *ofl, Rel_desc *rld, Boolean rlocal)
{
	...
	if (ifl->ifl_sortsyms == NULL) {
		...
		for (ondx = 0, nndx = 0; ondx < ifl->ifl_symscnt; ondx++) {
			 ...
			 */
			if (((sdp = ifl->ifl_oldndx[ondx]) == NULL) ||
...
ld_add_rel_cache(Ofl_desc *ofl, Rel_cache *rcp)
{
	 ...
	 *
	 * In general, if the output object is an executable, or a shared
...
ld_reloc_plt(Rel_desc *rsp, Ofl_desc *ofl)
{
	...
	if ((ofl->ofl_flags & FLG_OF_EXEC) && (sdp->sd_name[0] == '_') &&
	    (sdp->sd_name[1] == '_')) {
		/*
		 * Various TLS code sequences can be a combination of an
		 * identified TLS relocation followed by a tls_get_addr()
		 * call.  Catch these calls, and identify them as TLS related,
		 * so that any TLS transitions can process, and ignore these
		 * calls as necessary.
		 */
		switch (ld_targ.t_m.m_mach) {
		case EM_AMD64:
			/*
			 * AMD64 TLS code can reference __tls_get_addr().
			 */
			if (strcmp(sdp->sd_name,
			    MSG_ORIG(MSG_SYM_TLSGETADDR_U)) == 0) {
				return (ld_add_actrel((FLG_REL_TLSFIX |
				    FLG_REL_GETADDR), rsp, ofl));
			}
			break;

		case EM_386:
			/*
			 * GNUC IA32 TLS code can reference ___tls_get_addr().
			 */
			if (strcmp(sdp->sd_name,
			    MSG_ORIG(MSG_SYM_TLSGETADDR_UU)) == 0)
				return (ld_add_actrel((FLG_REL_TLSFIX |
				    FLG_REL_GETADDR), rsp, ofl));
			break;
		}
 ...
 */
static uintptr_t
reloc_relobj(Boolean local, Rel_desc *rsp, Ofl_desc *ofl, Is_desc *risp)
{
	Word		rtype = rsp->rel_rtype;
	Sym_desc	*sdp = rsp->rel_sym;
	...
	/*
	 * Determine if we can do any relocations at this point.  To do so,
	 * a relocation must satisfy all of the following:
	 *
	 *  -	the symbol reference is interpreted as local
	 *  -	the relocation is non-got
	 *  -	the relocation is pc-relative
	 *  -	the relocation is not against an SHT_GROUP section
	...
	if (local && !IS_GOT_RELATIVE(rtype) &&
	    ...
	    !(risp->is_shdr->sh_flags & SHF_GROUP) &&
...
/*
 * Determine whether a symbol reference should be treated as local.
 */
static Boolean
is_symref_local(Ofl_desc *ofl, Rel_desc *reld, Sym_desc *sdp)
{
	Word		rtype;
	ofl_flag_t	flags;
	Sym		*sym;
	...
	/*
	 * A symbol that is explicitly defined to be local provides a local
	 * reference.
	 */
	sym = sdp->sd_sym;
	...
	/*
	 * A relocation against a non-loadable section is considered local.
	 * Note, this may also catch external references, SHN_UNDEF.  In this
	 * case, when creating an executable or shared object, the relocation
	 * offset can not be updated at runtime, a warning message is generated,
	 * and the relocation is ignored.  See ld_reloc_local().
	 */
	if ((reld->rel_flags & FLG_REL_LOAD) == 0)
		return (TRUE);

	/*
	 * An undefined symbol, or a definition from an external dependency,
	 * or a capabilities symbol, can only be considered a global reference.
	 */
	if ((sym->st_shndx == SHN_UNDEF) || (sdp->sd_ref != REF_REL_NEED) ||
	    (sdp->sd_flags & FLG_SY_CAP))
		return (FALSE);

	/*
	 * A hidden or protected symbol is considered local.
	 */
	if (sdp->sd_flags & (FLG_SY_HIDDEN | FLG_SY_PROTECT))
		return (TRUE);

	/*
	 * If the relocation is explicitly defined as a local relocation, or
	 * is associated with a SHT_SUNW_dof section, then consider the
	 * reference local.
	 */
	isp = reld->rel_isdesc;
	rtype = reld->rel_rtype;
	if ((isp && (isp->is_shdr->sh_type == SHT_SUNW_dof)) ||
	    (IS_LOCALBND(rtype) || IS_SEG_RELATIVE(rtype)))
		return (TRUE);

	/*
	 * When generating a relocatable object any remaining symbols should
	 * be considered global.  A symbol defined within an executable, or a
	 * shared object that is being built -Bsymbolic (providing the symbol
	 * hasn't been tagged as no-direct), is considered local.
	 */
	flags = ofl->ofl_flags;
	if (((flags & FLG_OF_RELOBJ) == 0) && ((flags & FLG_OF_EXEC) ||
    ...
    const char *isname, Word isscnndx)
{
	 ...
	 */
	local = is_symref_local(ofl, reld, sdp);
	...
	if (flags & FLG_OF_RELOBJ)
		return (reloc_relobj(local, reld, ofl, isp));
    ...
    Rel *reloc)
{
	 ...
	 */
	if (sdp->sd_isc && (sdp->sd_isc->is_osdesc == NULL) &&
	    (ELF_ST_TYPE(sdp->sd_sym->st_info) == STT_SECTION)) {
		...
		if (orsp->rel_flags & FLG_REL_GOT) {
			...
			if (orsp->rel_rtype == ld_targ.t_m.m_r_register) {
				...
				sorted_list[index].rl_key3 = orsp->rel_roffset +
				    (Xword)_elf_getxoff(isp->is_indata) +
...
ld_reloc_process(Ofl_desc *ofl)
{
	...
	else if (!OFL_STRIP(ofl, FLG_SC_SYMBOL))
		/* LINTED */
 ...
 * each architectures build.  Here we add dummy calls to quiet chkmsg.
 *

...
map_cap(Mapfile *mf, Word type, Capmask *capmask)
{
	...
	if (DBG_ENABLED) {
		Dbg_cap_mapfile_title(ofl->ofl_lml, mf->mf_name, mf->mf_lineno);
		Dbg_cap_val_entry(ofl->ofl_lml, DBG_STATE_CURRENT, type,
	...
	DBG_CALL(Dbg_cap_val_entry(ofl->ofl_lml, reset ?
	    DBG_STATE_RESET : DBG_STATE_ADD, type, value, ld_targ.t_m.m_mach));
	...
	DBG_CALL(Dbg_cap_val_entry(ofl->ofl_lml, DBG_STATE_RESOLVED, type,
	    capmask->cm_val, ld_targ.t_m.m_mach));
	return (TRUE);
}
		...
		case TK_EQUAL:		/* Create/modify segment */
			...
			if (!new_segment && DBG_ENABLED) {
				...
				Dbg_map_seg(ofl, DBG_STATE_MOD_BEFORE,
				    ndx, sgp1, mf->mf_name, mf->mf_lineno);
			}
			...
			if (sgp1->sg_phdr.p_type == PT_SUNWSTACK) {
				...
				DBG_CALL(Dbg_map_seg(ofl, DBG_STATE_MOD_AFTER,
				    ndx, sgp1, mf->mf_name, mf->mf_lineno));
				break;
			}

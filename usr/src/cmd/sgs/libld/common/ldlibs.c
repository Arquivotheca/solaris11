 ...
 */
static uintptr_t
find_lib_name(const char *dir, const char *file, Ofl_desc *ofl,
    Boolean defpath, Rej_desc *rej)
{
	int		fd;
	size_t		dlen;
	...
	if (ofl->ofl_flags & FLG_OF_DYNLIBS) {
		...
		if ((fd = open(path, O_RDONLY)) != -1) {
			...
			if ((ofl->ofl_asdeflib != NULL) && (open_ret != 0) &&
			    (open_ret != S_ERROR))
				ld_assert_deflib_test(ofl, _path, defpath);
...
ld_find_library(const char *name, Ofl_desc *ofl)
{
	...
	for (APLIST_TRAVERSE(ofl->ofl_ulibdirs, idx, path)) {
		...
		if ((open_ret = find_lib_name(path, name, ofl,
		    FALSE, &_rej)) == 0) {

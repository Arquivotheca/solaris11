...
ld_group_process(Is_desc *gisc, Ofl_desc *ofl)
{
	...
	/*
	 * Validate the section indices within the group.
	 */
	for (ndx = 1; ndx < gd.gd_cnt; ndx++) {
		...
		/*
		 * If this is a COMDAT group, mark each section as COMDAT.
		 * Ensure a section ndx is provided, and that the section has
		 * been processed.  Stripping might have removed a section.
		 */
		if ((gd.gd_data[0] & GRP_COMDAT) && (gndx > 0) &&
		    ((isc = gifl->ifl_isdesc[gndx]) != NULL))
			isc->is_flags |= FLG_IS_COMDAT;
	}

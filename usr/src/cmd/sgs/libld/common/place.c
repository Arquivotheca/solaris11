    ...
    int ident, const char *alt_os_name)
{
	...
	if (((ofl->ofl_flags & FLG_OF_RELOBJ) == 0) &&
		...
		/*
		 * Identify this section type as COMDAT.  Also, enable
		 * relaxed relocation processing, as this is typically
		 * a requirement with .gnu.linkonce sections.
		 */
		isp->is_flags |= FLG_IS_COMDAT;
		DBG_CALL(Dbg_sec_gnu_comdat(ofl->ofl_lml,
		    DBG_GNUCOMDAT_LINKONCE, isp));
		if ((ofl->ofl_flags1 &
		    (FLG_OF1_RLXREL | FLG_OF1_NRLXREL)) == 0) {
			ofl->ofl_flags1 |= FLG_OF1_RLXREL;
			DBG_CALL(Dbg_reloc_relax(ofl->ofl_lml,
			    DBG_RLXREL_LINKONCE, ifl->ifl_name, isp));
		}
	}
	...
	if (((ofl->ofl_flags & FLG_OF_RELOBJ) == 0) &&
		 ...
		 */
		DBG_CALL(Dbg_sec_gnu_comdat(ofl->ofl_lml,
		    DBG_GNUCOMDAT_GROUP, isp));
		if ((ofl->ofl_flags1 &
		    (FLG_OF1_RLXREL | FLG_OF1_NRLXREL)) == 0) {
			ofl->ofl_flags1 |= FLG_OF1_RLXREL;
			DBG_CALL(Dbg_reloc_relax(ofl->ofl_lml,
			    DBG_RLXREL_GASCOMDAT, ifl->ifl_name, isp));
		}
	}
	...
	if (is_ehframe) {
		/*
		 * Executable or shared objects can have at most a single
	...
	if (is_ehframe) {
		/*
		 * On platforms where the ABI specified section type for
		 * eh_frame sections is not SHT_PROGBITS, we are willing to
		 * accept SHT_PROGBITS input sections, but want the output
		 * section to get the ABI specified type.
		 */

...
/*
 * Create a .bss input section, but do not place it in an output section.
 */
static Is_desc *
ld_make_bss_isec(Ofl_desc *ofl, Xword size, Xword align, uint_t ident)
{
	Shdr		*shdr;
	Elf_Data	*data;
	Is_desc		*isec;
	 ...
	 * arguments are 0.
	 */
	if (new_section(ofl, SHT_NOBITS, NULL, 0, &isec, &shdr, &data) ==
	    S_ERROR)
		return (NULL);
...
ld_make_bss(Ofl_desc *ofl, Xword size, Xword align, uint_t ident)
{
	...
	/* Create the section */
	if ((isec = ld_make_bss_isec(ofl, size, align, ident)) == NULL)
		return (S_ERROR);
...
/*
 * Build an additional data section - used to back OBJT/TLS symbol definitions
 * added with a mapfile.
 */
Is_desc *
ld_make_mapfile_data(Ofl_desc *ofl, size_t size, Boolean tls, Boolean bss)
{
	Shdr		*shdr;
	Elf_Data	*data;
	Is_desc		*isec;
	...
	if (bss) {
		ident = tls ? ld_targ.t_id.id_tlsbss : ld_targ.t_id.id_bss;
		isec = ld_make_bss_isec(ofl, size, ld_targ.t_m.m_word_align,
		    ident);
		if (isec == NULL)
		...
		if (tls) {
			...
			shname = MSG_ORIG(MSG_SCN_TDATA);
			shflags = SHF_WRITE | SHF_TLS;
		} else {
			ident = ld_targ.t_id.id_data;
			shname = MSG_ORIG(MSG_SCN_DATA);
			shflags = SHF_WRITE;
		}

		if (new_section(ofl, SHT_PROGBITS, shname, 0,
		    &isec, &shdr, &data) == S_ERROR)
			return ((Is_desc *)S_ERROR);

		data->d_size = size;
		shdr->sh_size = (Xword)size;
		shdr->sh_flags |= shflags;
	}

	isec->is_keyident = ident;
	return (isec);
}
 ...
 */
Is_desc *
ld_make_mapfile_text(Ofl_desc *ofl, size_t size)
{
	Shdr		*shdr;
	Elf_Data	*data;
	Is_desc		*isec;
	...
	isec->is_keyident = ld_targ.t_id.id_text;
	return (isec);
}


/*
 * Given an output section, and an offset within that output section,
 * return a pointer to the descriptor of the input section placed at
 * that location.
 *
 * entry:
 *	osp - Output section descriptor
 *	off - Offset within output section
 *	is_off - Variable to receive offset within the output section
 *		of the start of the located input section.
 *
 * exit:
 *	On success, *is_off contains the offset of the input
 *	section within the output section, and the input section
 *	descriptor is returned.
 *
 *	On failure, NULL is returned, and the value of *is_off is undefined.
 */

Is_desc *
ld_os_offset_to_isdesc(Os_desc *osp, Off off, Off *is_off)
{
	Aliste	idx;
	int	os_isdescs_idx;
	Is_desc	*isp;
	...
	OS_ISDESCS_TRAVERSE(os_isdescs_idx, osp, idx, isp) {
		...
		*is_off = _elf_getxoff(isp->is_indata);

		if ((off >= *is_off) &&
		    (off < (*is_off + isp->is_shdr->sh_size)))
			return (isp);
	}

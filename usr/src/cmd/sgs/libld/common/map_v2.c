 ...
 */
typedef struct {
	ld_map_ver_t		ss_mv;		/* Version */
	ld_map_sym_t		ss_ms;		/* Symbol */
	ld_map_sym_assert_t	ss_msa;		/* Symbol ASSERT attribute */
} symbol_state_t;
    ...
    Word type, APlist *strs)
{
	...
	DBG_CALL(Dbg_cap_mapfile_title(mf->mf_ofl->ofl_lml, mf->mf_name,
	    mf->mf_lineno));
...
/*
 * Parse a symbol SIZE or ASSERT SIZE value.
 *
 * entry:
 *	mf - Mapfile descriptor
 *	size - Address of variable to receive the resulting size value.
 *
 * exit:
 *	Updates *size, and returns TK_SEMICOLON or TK_RIGHTBKT for success,
 *	and TK_ERROR on error.
 */
static Token
parse_symbol_size(Mapfile *mf, Addr *size)
{
	/*
	 * Map symbolic size_value names to specific values. A value of
	 * 0 triggers a runtime substitution of 4 for ELFCLASS32
	 * or 8 for ELFCLASS64.
	 */
	typedef struct {
		const char	*name;	/* size name string */
		int		size;	/* size value to substitute */
	} szname_t;

	/*
	 * Valid symbolic names for which we substitute numeric values.
	 *
	 * addrsize
	 *	Natural machine word size, capable of holding a memory address.
	 */
	static szname_t szname_list[] = {
		{ MSG_ORIG(MSG_MAPKW_ADDRSIZE),	0 },
	...
	/*
	 * Size of buffer needed to format the names in szname_list[]. Must
	 * be kept in sync with szname_list.
	 */
	static size_t szname_list_bufsize =
	    KW_NAME_SIZE(MSG_MAPKW_ADDRSIZE);

	szname_t	*szname;
	Token		tok, tok2;
	ld_map_tkval_t	tkv, tkv2;
	Conv_inv_buf_t	inv_buf, inv_buf2;

	/*
	 * Expect a numeric value, or a symbolic name to be mapped to
	 * a numeric value.
	 */
	switch (tok = ld_map_gettoken(mf, TK_F_KEYWORD, &tkv)) {
	case TK_ERROR:
	...
	case TK_STRING:
		szname = ld_map_kwfind(tkv.tkv_str, szname_list,
		    SGSOFFSETOF(szname_t, name), sizeof (szname_list[0]));
		if (szname == NULL)
			goto bad_size;
		*size = szname->size;
		if (szname->size == 0)	/* substitute ELFCLASS word size */
			*size =
			    (ld_targ.t_m.m_class == ELFCLASS32) ? 4 : 8;
		break;

	case TK_INT:
		*size = tkv.tkv_int.tkvi_value;
		break;

	default:
	bad_size:
		{
			char buf[VLA_SIZE(szname_list_bufsize)];

			mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMSIZE),
			    ld_map_kwnames(szname_list,
			    SGSOFFSETOF(szname_t, name),
			    sizeof (szname_list[0]), buf, szname_list_bufsize),
			    ld_map_tokenstr(tok, &tkv, &inv_buf));
			return (TK_ERROR);
		}
	}
	...
	/*
	 * Expect a terminator, or a repeat count in [] brackets:
	 */
	switch (tok2 = ld_map_gettoken(mf, 0, &tkv2)) {
	case TK_ERROR:
	case TK_SEMICOLON:
	case TK_RIGHTBKT:
		return (tok2);	/* Error or terminator */

	case TK_LEFTSQBKT:	/* Start of optional count, handled below */
		break;

	default:		/* Anything else is unexpected */
		mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMCNTBKT),
		    ld_map_tokenstr(tok, &tkv, &inv_buf),
		    ld_map_tokenstr(tok2, &tkv2, &inv_buf2));
		return (TK_ERROR);
	}

	/* Multiply the current size by the count */
	tok = gettoken_int(mf, MSG_ORIG(MSG_MAPKW_SIZE), &tkv);
	if (tok == TK_ERROR)
		return (TK_ERROR);
	*size *= tkv.tkv_int.tkvi_value;

	/* Closing ']' following the count */
	switch (tok2 = ld_map_gettoken(mf, 0, &tkv2)) {
	case TK_ERROR:
		return (tok2);
	...
	default:
		mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_RSQBKT),
		    ld_map_tokenstr(tok, &tkv, &inv_buf),
		    ld_map_tokenstr(tok2, &tkv2, &inv_buf2));
		return (TK_ERROR);
	}
...
/*
 * The strtol_XXX functions use the libconv iteration facility to map a
 * string to various ELF constants. This allow us to keep up with any new
 * constants that get added to the system without having to change this
 * code, and enforces uniformity.
 *
 * entry:
 *	str - String to process
 *	conv_uvalue - As per conv_iter_strtol_init()
 *
 * exit:
 *	On success, returns TRUE, and the resulting value is found in
 *	conv_uvalue->csl_value. On failure, returns FALSE.
 */
static Boolean
strtol_sec_type(const char *str, conv_strtol_uvalue_t *conv_uvalue)
{
	conv_iter_ret_t	status;

	if (!conv_iter_strtol_init(str, conv_uvalue))
		return (FALSE);

	/* Look at the canonical form */
	status = conv_iter_sec_type(ELFOSABI_SOLARIS, ld_targ.t_m.m_mach,
	    CONV_FMT_ALT_CF, conv_iter_strtol, conv_uvalue);

	/* Failing that, look at the normal form */
	if (status != CONV_ITER_DONE)
		(void) conv_iter_sec_type(ELFOSABI_SOLARIS, ld_targ.t_m.m_mach,
		    CONV_FMT_ALT_NF, conv_iter_strtol, conv_uvalue);

	return (conv_uvalue->csl_found);
}

static Boolean
strtol_sym_bind(const char *str, conv_strtol_uvalue_t *conv_uvalue)
{
	conv_iter_ret_t	status;
	...
	/* Look at the canonical form */
	status = conv_iter_sym_info_bind(ELFOSABI_SOLARIS, ld_targ.t_m.m_mach,
	    CONV_FMT_ALT_CF, conv_iter_strtol, conv_uvalue);

	/* Failing that, look at the normal form */
	if (status != CONV_ITER_DONE)
		(void) conv_iter_sym_info_bind(ELFOSABI_SOLARIS,
		    ld_targ.t_m.m_mach, CONV_FMT_ALT_NF,
		    conv_iter_strtol, conv_uvalue);

	return (conv_uvalue->csl_found);
}

static Boolean
strtol_sym_type(const char *str, conv_strtol_uvalue_t *conv_uvalue)
{
	conv_iter_ret_t	status;
	 ...
	 * Special cases:
	 *
	 * -	We treat DATA as a synonym for STT_OBJECT, for
	 *	compatibility with the strings accepted by the SYMBOL
	 *	TYPE attribute.
	 *
	 * -	The conversion library does not recognize FUNCTION,
	 *	providing only the abbreviated FUNC. We also match
	 *	the full form.
	 */
	if (strcasecmp(str, MSG_ORIG(MSG_MAPKW_DATA)) == 0) {
		conv_uvalue->csl_found = TRUE;
		conv_uvalue->csl_value = STT_OBJECT;
		return (TRUE);
	}
	if (strcasecmp(str, MSG_ORIG(MSG_MAPKW_FUNCTION)) == 0) {
		conv_uvalue->csl_found = TRUE;
		conv_uvalue->csl_value = STT_FUNC;
		return (TRUE);
	}
	...
	/* Look at the canonical form */
	status = conv_iter_sym_info_type(ld_targ.t_m.m_mach,
	    CONV_FMT_ALT_CF, conv_iter_strtol, conv_uvalue);

	/* Failing that, look at the normal form */
	if (status != CONV_ITER_DONE)
		(void) conv_iter_sym_info_type(ld_targ.t_m.m_mach,
		    CONV_FMT_ALT_NF, conv_iter_strtol, conv_uvalue);
...
dir_capability(Mapfile *mf)
{
	...
	case TK_STRING:
		 ...
		 * be encountered, the last entry wins.
		 */
		DBG_CALL(Dbg_cap_id(mf->mf_ofl->ofl_lml, mf->mf_name,
		    mf->mf_lineno, capstr->cs_str, tkv.tkv_str));
...
dir_hdr_noalloc(Mapfile *mf)
{
	...
	DBG_CALL(Dbg_map_hdr_noalloc(mf->mf_ofl->ofl_lml, mf->mf_name,
	    mf->mf_lineno));
...
at_seg_assign_type(Mapfile *mf, Token eq_tok, void *uvalue)
{
	...
	/* Map to section type */
	if (!strtol_sec_type(tkv.tkv_str, &conv_uvalue)) {
	...
	if (new_segment) {
		...
		if (DBG_ENABLED) {
			...
			DBG_CALL(Dbg_map_seg(mf->mf_ofl, DBG_STATE_MOD_BEFORE,
			    ndx, sgp, mf->mf_name, mf->mf_lineno));
		}
	}
...
dir_segment_order(Mapfile *mf)
{
	...
	DBG_CALL(Dbg_map_seg_order(mf->mf_ofl, ELFOSABI_SOLARIS,
	    ld_targ.t_m.m_mach, DBG_STATE_MOD_BEFORE, mf->mf_name,
	    mf->mf_lineno));
	...
	DBG_CALL(Dbg_map_seg_order(mf->mf_ofl, ELFOSABI_SOLARIS,
	    ld_targ.t_m.m_mach, DBG_STATE_MOD_AFTER, mf->mf_name,
	    mf->mf_lineno));
 ...
 */
static Token
dir_stub_object(Mapfile *mf)
{
	/* This directive is only meaningful when applied to shared objects */
	if (!(mf->mf_ofl->ofl_flags & FLG_OF_SHAROBJ)) {
		mf_fatal0(mf, MSG_INTL(MSG_MAP_STUBDIR_SO));
		return (TK_ERROR);
	}

	mf->mf_ofl->ofl_flags |= FLG_OF_STUBDIR;
	DBG_CALL(Dbg_map_stub(mf->mf_ofl->ofl_lml, mf->mf_name, mf->mf_lineno));

	/* Terminating ';' */
	return (gettoken_semicolon(mf, MSG_ORIG(MSG_MAPKW_STUB_OBJECT)));
}
...
at_sym_size(Mapfile *mf, Token eq_tok, void *uvalue)
{
	...
	return (parse_symbol_size(mf, &ss->ss_ms.ms_size));
}

typedef struct {
	const char	*ast_name;	/* type name */
	Word		ast_shndx;	/* symbol section index */
	uchar_t		ast_type;	/* STT_ symbol type */
} at_sym_type_t;
...
static at_sym_type_t at_sym_type_list[] = {
	...
	{ MSG_ORIG(MSG_MAPKW_TLS),	SHN_ABS,	STT_TLS },
...
gts_efunc_at_sym_type(Mapfile *mf, Token tok, ld_map_tkval_t *tkv)
{
	...
	mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMTYPE),
	    ld_map_kwnames(at_sym_type_list,
	    SGSOFFSETOF(at_sym_type_t, ast_name), sizeof (at_sym_type_list[0]),
	    buf, at_sym_type_list_bufsize),
	    ld_map_tokenstr(tok, tkv, &inv_buf));
}
...
at_sym_type(Mapfile *mf, Token eq_tok, void *uvalue)
{
	...
	type = ld_map_kwfind(tkv.tkv_str, at_sym_type_list,
	    SGSOFFSETOF(at_sym_type_t, ast_name), sizeof (type[0]));
	if (type == NULL) {
		gts_efunc_at_sym_type(mf, TK_STRING, &tkv);
		return (TK_ERROR);
	}

	ss->ss_ms.ms_shndx = type->ast_shndx;
	ss->ss_ms.ms_sdflags |= FLG_SY_SPECSEC;
	ss->ss_ms.ms_type = type->ast_type;
...
/*
 * at_sym_assert_alias(): Value for ALIAS= is not a valid symbol name
 */
static void
gts_efunc_at_sym_assert_alias(Mapfile *mf, Token tok, ld_map_tkval_t *tkv)
{
	Conv_inv_buf_t	inv_buf;

	mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMALIAS),
	    ld_map_tokenstr(tok, tkv, &inv_buf));
}

/*
 * SYMBOL_xxx ... { symbol_name { ASSERT { ALIAS =
 * ------------------------------------------------^
 */
/* ARGSUSED 1 */
static Token
at_sym_assert_alias(Mapfile *mf, Token eq_tok, void *uvalue)
{
	symbol_state_t		*ss = uvalue;
	ld_map_tkval_t		tkv;
	...
	/* value */
	if (gettoken_str(mf, TK_F_KEYWORD, &tkv,
	    gts_efunc_at_sym_assert_alias) == TK_ERROR)
		return (TK_ERROR);

	ss->ss_msa.msa_flags |= FLG_MSA_ALIAS;
	ss->ss_msa.msa_alias = tkv.tkv_str;
...
/*
 * SYMBOL_xxx ... { symbol_name { ASSERT { SH_ATTR =
 * --------------------------------------------------^
 */
/* ARGSUSED 1 */
static Token
at_sym_assert_sh_attr(Mapfile *mf, Token eq_tok, void *uvalue)
{
	typedef struct {
		const char	*name;
		msa_sh_attr_t	value;
	...
	static attr_t attr_list[] = {
		{ MSG_ORIG(MSG_MAPKW_BITS),		SH_ATTR_MSA_BITS },
		{ MSG_ORIG(MSG_MAPKW_NOBITS),		SH_ATTR_MSA_NOBITS },
	...
	/*
	 * If this is the first SH_ATTR attribute, then zero it
	 * and set the flag.
	 */
	if ((ss->ss_msa.msa_flags & FLG_MSA_SH_ATTR) == 0) {
		ss->ss_msa.msa_sh_attr = 0;
		ss->ss_msa.msa_flags |= FLG_MSA_SH_ATTR;
	}
	...
	for (done = 0; done == 0; ) {
		...
		case TK_STRING:
			attr = ld_map_kwfind(tkv.tkv_str, attr_list,
			    SGSOFFSETOF(attr_t, name), sizeof (attr[0]));
			if (attr == NULL)
				goto bad_flag;
			cnt++;
			ss->ss_msa.msa_sh_attr |= attr->value;
			break;
		case TK_RIGHTBKT:
		case TK_SEMICOLON:
			done = 1;
		...
		bad_flag:
			{
				...
				mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMASATTR),
				    ld_map_kwnames(attr_list,
				    SGSOFFSETOF(attr_t, name),
				    sizeof (attr[0]), buf,
				    attr_list_bufsize),
				    ld_map_tokenstr(tok, &tkv, &inv_buf));
			}
...
/*
 * at_sym_assert_bind(): Value for BINDING= is not a valid symbol binding
 */
static void
gts_efunc_at_sym_assert_bind(Mapfile *mf, Token tok, ld_map_tkval_t *tkv)
{
	Conv_inv_buf_t	inv_buf;

	mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMBIND),
	    ld_map_tokenstr(tok, tkv, &inv_buf));
}

/*
 * SYMBOL ... { symbol_name { ASSERT { BINDING =
 * ---------------------------------------------^
 */
/* ARGSUSED 1 */
static Token
at_sym_assert_bind(Mapfile *mf, Token eq_tok, void *uvalue)
{
	symbol_state_t		*ss = uvalue;
	ld_map_tkval_t		tkv;
	conv_strtol_uvalue_t	conv_uvalue;
	...
	/* value */
	if (gettoken_str(mf, TK_F_KEYWORD, &tkv,
	    gts_efunc_at_sym_assert_bind) == TK_ERROR)
		return (TK_ERROR);

	/* Map to symbol binding */
	if (!strtol_sym_bind(tkv.tkv_str, &conv_uvalue)) {
		gts_efunc_at_sym_assert_bind(mf, TK_STRING, &tkv);
		return (TK_ERROR);
	}
	ss->ss_msa.msa_flags |= FLG_MSA_BIND;
	ss->ss_msa.msa_bind = conv_uvalue.csl_value;

	/* terminator */
	return (gettoken_term(mf, MSG_ORIG(MSG_MAPKW_BINDING)));
}

/*
 * SYMBOL ... { symbol_name { ASSERT { SIZE =
 * ---------------------------------------------^
 */
/* ARGSUSED 1 */
static Token
at_sym_assert_size(Mapfile *mf, Token eq_tok, void *uvalue)
{
	symbol_state_t	*ss = uvalue;

	ss->ss_msa.msa_flags |= FLG_MSA_SIZE;
	return (parse_symbol_size(mf, &ss->ss_msa.msa_size));
}

/*
 * at_sym_assert_type(): Value for TYPE= is not a symbol type
 */
static void
gts_efunc_at_sym_assert_type(Mapfile *mf, Token tok, ld_map_tkval_t *tkv)
{
	Conv_inv_buf_t	inv_buf;

	mf_fatal(mf, MSG_INTL(MSG_MAP_EXP_SYMTYPEAS),
	    ld_map_tokenstr(tok, tkv, &inv_buf));
}

/*
 * SYMBOL ... { symbol_name { ASSERT { TYPE =
 * ---------------------------------------------^
 */
/* ARGSUSED 1 */
static Token
at_sym_assert_type(Mapfile *mf, Token eq_tok, void *uvalue)
{
	symbol_state_t	*ss = uvalue;
	ld_map_tkval_t	tkv;
	conv_strtol_uvalue_t	conv_uvalue;
	...
	/* value */
	if (gettoken_str(mf, TK_F_KEYWORD, &tkv,
	    gts_efunc_at_sym_assert_type) == TK_ERROR)
		return (TK_ERROR);

	/* Map to symbol binding */
	if (!strtol_sym_type(tkv.tkv_str, &conv_uvalue)) {
		gts_efunc_at_sym_assert_type(mf, TK_STRING, &tkv);
		return (TK_ERROR);
	}
	ss->ss_msa.msa_flags |= FLG_MSA_TYPE;
	ss->ss_msa.msa_type = conv_uvalue.csl_value;
...
/*
 * Parse the ASSERT attribute for a symbol within a SYMBOL_SCOPE or
 * SYMBOL_VERSION directive.
 *
 * SYMBOL ... { ... ASSERT
 * ----------------------- ^
 */
...
at_sym_assert(Mapfile *mf, Token eq_tok, void *uvalue)
{
	/* Symbol attributes */
	static attr_t attr_list[] = {
		{ MSG_ORIG(MSG_MAPKW_ALIAS),	at_sym_assert_alias,
		    ATTR_FMT_EQ },
		{ MSG_ORIG(MSG_MAPKW_BINDING),	at_sym_assert_bind,
		    ATTR_FMT_EQ },
		{ MSG_ORIG(MSG_MAPKW_SH_ATTR),	at_sym_assert_sh_attr,
		    ATTR_FMT_EQ },
		{ MSG_ORIG(MSG_MAPKW_SIZE),	at_sym_assert_size,
		    ATTR_FMT_EQ },
		{ MSG_ORIG(MSG_MAPKW_TYPE),	at_sym_assert_type,
		    ATTR_FMT_EQ },
	...
	static size_t	attr_list_bufsize =
	    ...
	    KW_NAME_SIZE(MSG_MAPKW_SH_ATTR) +
	    KW_NAME_SIZE(MSG_MAPKW_TYPE) +
	...
	if (gettoken_leftbkt(mf, MSG_ORIG(MSG_MAPKW_ASSERT)) == TK_ERROR)
		return (TK_ERROR);

	/* Parse the attributes */
	if (parse_attributes(mf, MSG_ORIG(MSG_MAPKW_ASSERT),
...
parse_symbol_attributes(Mapfile *mf, const char *dir_name, symbol_state_t *ss)
{
	...
	static attr_t attr_list[] = {
		...
		{ MSG_ORIG(MSG_MAPKW_ASSERT),	at_sym_assert,	ATTR_FMT_NAME },
	...
	for (done = 0; done == 0; ) {
		 ...
		 * complicating the code to prevent.
		 */
		...
		case TK_STRING:
			...
			/*
			 * Reset the symbol ASSERT data. Capture the mapfile
			 * name and current line number for potential use in
			 * reporting assertion failures later.
			 */
			ss->ss_msa.msa_mfname = mf->mf_name;
			ss->ss_msa.msa_mflineno = mf->mf_lineno;
			ss->ss_msa.msa_flags = 0;
			...
			case TK_SEMICOLON:
				/*
				 * The ASSERT ALIAS attribute is mutually
				 * exclusive to SH_ATTR, SIZE, and TYPE, as
				 * those values come from the aliased symbol.
				 * Check that they're not used together.
				 */
				if ((ss->ss_msa.msa_flags & FLG_MSA_ALIAS) &&
				    (ss->ss_msa.msa_flags & (FLG_MSA_SH_ATTR |
				    FLG_MSA_SIZE | FLG_MSA_TYPE))) {
					eprintf(mf->mf_ofl->ofl_lml, ERR_FATAL,
					    MSG_INTL(MSG_MAP_SYMAS_ALIASME),
					    ss->ss_msa.msa_mfname,
					    EC_LINENO(ss->ss_msa.msa_mflineno),
					    ss->ss_ms.ms_name,
					    ss->ss_msa.msa_alias);
					ss->ss_mv.mv_errcnt++;
				}
				...
				/*
				 * Add the new symbol. All symbols added by a
				 * mapfile start out with global scope and then
				 * pass through the normal symbol resolution
				 * process.  If scope reduction is specified,
				 * it is applied after input file processing
				 * is complete.
				 */
	...
	/*
	 * In the SYMBOL directives, we keep parsing in the face of
...
ld_map_parse_v2(Mapfile *mf)
{
	...
	tldir_t dirlist[] = {
		...
		{ MSG_ORIG(MSG_MAPKW_STUB_OBJECT),	dir_stub_object },
	...
	static size_t dirlist_bufsize =
	    ...
	    KW_NAME_SIZE(MSG_MAPKW_STACK) +
	    KW_NAME_SIZE(MSG_MAPKW_STUB_OBJECT) +

...
#define	ADD_TO_DYNSORT(_sdp, _sym, _type, _sym_ndx) \
	...
	Aliste		idx1;
	Boolean		assert_data, assert_data_err;
	 ...
	 */
	if (!OFL_STRIP(ofl, FLG_SC_SYMBOL)) {
	...
	if (OFL_ALLOW_DYNSYM(ofl)) {
		...
		if (ofl->ofl_osldynsym) {
			...
			/*
			 * If there is a .SUNW_ldynsym, then there may also be
			 * .SUNW_dynsymsort and/or .SUNW_dyntlssort sections,
			 * used to collect indices of function and data symbols
	...
	if (!(flags & FLG_OF_RELOBJ)) {
		...
		if (sgp) {
			...
			if (((alp = sgp->sg_osdescs) != NULL) &&
				 ...
				 */
				osp = alp->apl_data[alp->apl_nitems - 1];
				/* LINTED */
		...
		for (lndx = 1; lndx < local; lndx++) {
			...
			if (((isc = sdp->sd_isc) != NULL) && !update_done) {
				...
				if ((flags & FLG_OF_RELOBJ) == 0) {
					...
					/*
					 * If a TLS segment is created, any TLS
					 * symbol is made relative to that
					 * segment.
					 */
					if ((type == STT_TLS) &&
					    ofl->ofl_tlsphdr)
	...
	for (sav = avl_first(&ofl->ofl_symavl); sav;
		...
		if ((sdp->sd_flags & FLG_SY_PAREXPN) ||
			...
			if (sdp->sd_flags & FLG_SY_PAREXPN) {
				...
				/*
				 * If a TLS segment is created, any TLS symbol
				 * is made relative to that segment.
				 */
				if (ofl->ofl_tlsphdr)
					symptr->st_value -=
					    ofl->ofl_tlsphdr->p_vaddr;
	...
	/*
	 * Determine if we will need to check global data symbols to
	 * ensure they have an explicit mapfile ASSERT attributes.
	 */
	assert_data = (ofl->ofl_flags & FLG_OF_STUBDIR) &&
	    !(ofl->ofl_dtflags_1 & DF_1_STUB);
	assert_data_err = FALSE;
	...
	for (ssndx = 0; ssndx < (ofl->ofl_elimcnt + ofl->ofl_scopecnt +
		...
		if (dynsym && (!local || dynlocal)) {
			...
			/*
			 * If this is not a stub object, and the STUB_OBJECT
			 * mapfile directive was seen, then all global data must
			 * have an explicit mapfile ASSERT explicitly stating
			 * a data symbol type. This ensures that all exported
			 * global data is visible in the mapfile assert list,
			 * and thus, that the stub and real objects will have
			 * fully compatible data symbols.
			 *
			 * These errors tend to come in clusters, so keep going
			 * in order to flush out as many as possible in one
			 * invocation, using assert_data_err to indicate
			 * that we must exit after the loop completes.
			 */
			if (assert_data && ((sdp->sd_flags &
			    (FLG_SY_LDSTD | FLG_SY_MFASDATA)) == 0) &&
			    (sdp->sd_sym->st_shndx != SHN_UNDEF) &&
			    assert_data_symtype[ELF_ST_TYPE(sym->st_info)] &&
			    (sdp->sd_ref == REF_REL_NEED) &&
			    (sym->st_shndx != SHN_ABS)) {
				Conv_inv_buf_t	inv_buf;

				eprintf(ofl->ofl_lml, ERR_FATAL,
				    MSG_INTL(MSG_MAP_SYMAS_DATARQ),
				    conv_sym_info_type(ld_targ.t_m.m_mach,
				    ELF_ST_TYPE(sym->st_info), 0, &inv_buf),
				    demangle(name), sdp->sd_file->ifl_name);
				assert_data_err = TRUE;
			}
		}
	...
	/*
	 * If we threw any symbol assert missing errors, quit now.
	 */
	if (assert_data_err)
		return ((Addr)S_ERROR);
...
ld_update_outfile(Ofl_desc *ofl)
{
	...
	/*
	 * Process any symbol ASSERT attributes seen in mapfiles.
	 */
	if (!ld_map_sym_assert(ofl))
		return (S_ERROR);

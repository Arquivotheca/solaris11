...
#define	SYM_LOC_BADADDR(_sdp, _sym, _type) \
	...
	if (ifl->ifl_flags & FLG_IF_HSTRTAB) {
		Is_desc	*sisp;
		Word	sndx = shdr->sh_link;

		if ((sndx == 0) || (sndx >= ifl->ifl_shnum)) {
			ld_eprintf(ofl, ERR_FATAL,
			    MSG_INTL(MSG_FIL_INVSHLINK), ifl->ifl_name,
			    EC_WORD(symsecndx), symsecname, EC_XWORD(sndx));
			return (S_ERROR);
		}
		sisp = ifl->ifl_isdesc[sndx];
		strsize = sisp->is_shdr->sh_size;
		strs = sisp->is_indata->d_buf;
		if ((strsecname = sisp->is_name) == NULL)
	...
	if (local) {
		...
		for (sym++, ndx = 1; ndx < local; sym++, ndx++) {
			...
			/*
			 * Now that we have the name, validate the section
			 * index.
			 */
			...
			if ((sym->st_shndx != SHN_UNDEF) &&
				...
				/*
				 * If this symbol has no associated section, or
				 * the section is going to be discarded, then
				 * discard the symbol.
				 */
				if ((sdp->sd_isc == NULL) ||
				    (sdp->sd_isc->is_flags & FLG_IS_DISCARD)) {
					sdp->sd_flags |= FLG_SY_ISDISC;
					DBG_CALL(Dbg_syms_discarded(
					    ofl->ofl_lml, sdp));
					continue;
				}
			}
	...
	for (ndx = local; ndx < total; sym++, ndx++) {
		const char	*name;
		sd_flag_t	sdflags = 0;
		Word		shndx;
		Sym		*nsym = sym;
		...
		/*
		 * Now that we have the name, validate the section index.
		 */
		if (((sdflags & FLG_SY_SPECSEC) == 0) &&
		    (shndx > ifl->ifl_shnum)) {
		...
		if ((bind != STB_GLOBAL) && (bind != STB_WEAK)) {
			ld_eprintf(ofl, ERR_WARNING, MSG_INTL(MSG_SYM_NONGLOB),
			    ...
			    ld_targ.t_m.m_mach, bind, 0, &inv_buf));
			continue;
		}
		if (bind == STB_WEAK)
			weak++;
	...
	if (weak && (OFL_ALLOW_LDYNSYM(ofl) || (etype == ET_DYN)) &&
		...
		for (ndx = 0; ndx < (total - local); ndx++) {
			...
			for (sndx = ndx + 1; sndx < (total - local); sndx++) {
				...
				/*
				 * If a shared object, set link fields so
 ...
 */
Ld_ret
ld_sym_add_u(const char *name, Ofl_desc *ofl, Msg mid, Sym_desc **sdpp)
{
	Sym		*sym;

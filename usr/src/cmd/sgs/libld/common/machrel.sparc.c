...
tls_fixups(Ofl_desc *ofl, Rel_desc *arsp)
{
	...
	if (sdp->sd_ref == REF_DYN_NEED) {
		...
		case R_SPARC_TLS_GD_ADD:
			DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH,
			    SHT_RELA, arsp, offset, ld_reloc_sym_name,
			    R_SPARC_NONE));
			w = bswap ? ld_bswap_Word(*addr) : *addr;
			w = (TLS_GD_IE_LD |
			    (w & (FM3_REG_MSK_RS1 | FM3_REG_MSK_RS2)));
			*addr = bswap ? ld_bswap_Word(w) : w;
			return (FIX_DONE);
	...
	case R_SPARC_TLS_LDO_LOX10:
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_SPARC_TLS_LE_LOX10));
	...
	case R_SPARC_TLS_IE_LDX:
		 ...
		 */
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_SPARC_NONE));
		w = bswap ? ld_bswap_Word(*addr) : *addr;
...
gotop_fixups(Ofl_desc *ofl, Rel_desc *arsp)
{
	...
	case R_SPARC_GOTDATA_OP_HIX22:
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_SPARC_GOTDATA_HIX22));
		arsp->rel_rtype = R_SPARC_GOTDATA_HIX22;
		return (FIX_RELOC);

	case R_SPARC_GOTDATA_OP_LOX10:
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_SPARC_GOTDATA_LOX10));
...
ld_do_activerelocs(Ofl_desc *ofl)
{
	...
	REL_CACHE_TRAVERSE(&ofl->ofl_actrels, idx, rcbp, arsp) {
		...
		if ((arsp->rel_flags & FLG_REL_GOT) &&
			...
			/* LINTED */
			R1addr = (uintptr_t)((-neggotoffset * M_GOT_ENTSIZE) +
...
ld_reloc_TLS(Boolean local, Rel_desc *rsp, Ofl_desc *ofl)
{
	...
	if ((rtype == R_SPARC_TLS_GD_CALL) || (rtype == R_SPARC_TLS_LDM_CALL)) {
		...
		if (ld_sym_add_u(MSG_ORIG(MSG_SYM_TLSGETADDR_U),

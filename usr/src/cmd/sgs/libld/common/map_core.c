 ...
 *		initialization call.
 *	test_dup - TRUE if it is ncessary to test whether the identifier
 *		is already entered, FALSE if the caller knows that
 ...
 */
static int
cexp_ident_add(Mapfile *mf, Boolean test_dup, const char *name)
{
	cexp_id_node_t	*node;
	...
	if (test_dup && cexp_ident_test(name))
		return (1);
...
/*
 * Add a boolean identifier from the command line (-z mapfile-add=name)
 * to the list of names to be added to the conditional expression identifier
 * AVL tree.
 *
 * Such names cannot be added until cexp_ident_init() is run, and that
 * has to wait until all of the command line arguments have been processed
 * and we know the type of object we are producing.
 */
Ld_ret
ld_map_cexp_cmdline(Ofl_desc *ofl, const char *name)
{
	const char	*s = name;
	...
	for (s = name; *s != '\0'; s++) {
		if (isspace(*s)) {
			ld_eprintf(ofl, ERR_FATAL, MSG_INTL(MSG_ARG_BADMFCEXP),
			    name);
			return (LD_FALSE);
		}
	}

	/* Add it to the list for cexp_ident_init() to deal with */
	if (aplist_append(&(ofl->ofl_mapcladd), name,
	    AL_CNT_OFL_MAPCLADD) == NULL)
		return (LD_ERROR);
...
cexp_ident_init(Ofl_desc *ofl)
{
	...
	/* ELFCLASS */
	if (cexp_ident_add(NULL, FALSE, (ld_targ.t_m.m_class == ELFCLASS32) ?
	...
	case EM_386:
	case EM_AMD64:
		if (cexp_ident_add(NULL, FALSE, MSG_ORIG(MSG_STR_UX86)) == 0)
			return (FALSE);
		break;

	case EM_SPARC:
	case EM_SPARCV9:
		if (cexp_ident_add(NULL, FALSE, MSG_ORIG(MSG_STR_USPARC)) == 0)
			return (FALSE);
		break;
	}
	...
	if (ofl->ofl_flags & FLG_OF_SHAROBJ)
		str = MSG_ORIG(MSG_STR_UET_DYN);
	else if (ofl->ofl_flags & FLG_OF_RELOBJ)
		str = MSG_ORIG(MSG_STR_UET_REL);
	else
		str = MSG_ORIG(MSG_STR_UET_EXEC);
	if (cexp_ident_add(NULL, FALSE, str) == 0)
		return (FALSE);


	/* true is always defined */
	if (cexp_ident_add(NULL, FALSE, MSG_ORIG(MSG_STR_TRUE)) == 0)
		return (FALSE);

	/* Add names entered on command line via -z mapefile-add */
	for (APLIST_TRAVERSE(ofl->ofl_mapcladd, idx, str))
		if (cexp_ident_add(NULL, TRUE, str) == 0)
			return (FALSE);
...
restart:
	...
	case CDIR_T_ADD:
		if (pass && !cexp_ident_add(&arg_mf, TRUE, arg_mf.mf_next))
			return (FALSE);
		break;
...
ld_map_tokenstr(Token tok, ld_map_tkval_t *tkv, Conv_inv_buf_t *inv_buf)
{
	...
	case TK_LEFTSQBKT:
		return (MSG_ORIG(MSG_QSTR_LEFTSQBKT));
	case TK_RIGHTSQBKT:
		return (MSG_ORIG(MSG_QSTR_RIGHTSQBKT));
	default:
		assert(0);
		break;
	}
...
ld_map_parse(const char *mapfile, Ofl_desc *ofl)
{
	...
	case MFV_SOLARIS:
		 ...
		 */
		if ((lms.lms_cexp_id == NULL) && !cexp_ident_init(ofl))
			return (FALSE);
...
ld_map_post_process(Ofl_desc *ofl)
{
	 ...
	 */
	if (ofl->ofl_mapbcksec) {
		Is_desc	*isp;

		DBG_CALL(Dbg_sec_backing(ofl->ofl_lml));

		for (APLIST_TRAVERSE(ofl->ofl_mapbcksec, idx, isp))
			if (ld_place_section(ofl, isp, NULL, isp->is_keyident,
			    NULL) == (Os_desc *)S_ERROR)
				return (FALSE);
	}

	/*
	 * Stub objects are built entirely from mapfiles. The real and
	 * stub versions of a given object must export exactly the same
	 * symbols, with the same types. It is particularly important for
	 * the proper functioning of copy relocations that exported data
	 * aliasing (weak/strong pairs) be reproduced accurately. All of
	 * this needs to happen even though the two objects are built by
	 * separate link-editor invocations. We combine the STUB_OBJECT
	 * mapfile directive with symbol scope reduction to achive this:
	 *
	 * -	When a stub object is built (-zstub), ensure that the
	 *	STUB_OBJECT directive is used, and ensure that symbol
	 *	scope auto-reduction ('*') is employed.
	 *
	 * -	When STUB_OBJECT is seen, and -zstub is not specified,
	 *	ensure that symbol scope reduction ('*') is employed.
	 *
	 * The net result is that the possible exported symbols in both cases
	 * are limited to those specified in the mapfiles. ASSERT attributes
	 * can then be used, and verified, to ensure the exported symbols
	 * are compatible.
	 */
	if (ofl->ofl_dtflags_1 & DF_1_STUB) {
		Boolean	ok = TRUE;

		if ((ofl->ofl_flags & FLG_OF_STUBDIR) == 0) {
			eprintf(ofl->ofl_lml, ERR_FATAL,
			    MSG_INTL(MSG_MAP_STUBDIRREQ));
			ok = FALSE;
		}
		if ((ofl->ofl_flags & (FLG_OF_AUTOLCL | FLG_OF_AUTOELM)) == 0) {
			eprintf(ofl->ofl_lml, ERR_FATAL,
			    MSG_INTL(MSG_MAP_STUBOPTAUTRED));
			ok = FALSE;
		}
		if (!ok)
			return (FALSE);
	} else {
		if ((ofl->ofl_flags & FLG_OF_STUBDIR) &&
		    (ofl->ofl_flags & (FLG_OF_AUTOLCL | FLG_OF_AUTOELM)) == 0) {
			eprintf(ofl->ofl_lml, ERR_FATAL,
			    MSG_INTL(MSG_MAP_STUBDIRAUTRED));
			return (FALSE);
		}
	}
	return (TRUE);
...
/*
 * Process any SYMBOL_SCOPE/SYMBOL_VERSION ASSERT attributes.
 *
 * entry:
 *	Symbol processing must be complete, and output symbol tables done.
 *
 * exit:
 *	If all assert conditions are met, returns TRUE. If any assert
 *	conditions fail, issues the necessary diagnostic message(s),
 *	and returns FALSE.
 */
Boolean
ld_map_sym_assert(Ofl_desc *ofl)
{
	ld_map_sym_assert_t	*msap;
	Boolean		r = TRUE;
	Aliste		idx;
	Conv_inv_buf_t	buf1, buf2;
	Boolean		stubdir = (ofl->ofl_flags & FLG_OF_STUBDIR) != 0;


	/*
	 * Process all ASSERT records
	 */
	for (ALIST_TRAVERSE(ofl->ofl_mapassert, idx, msap)) {
		Sym_desc	*sdp = msap->msa_sdp;
		Boolean		has_alias = (msap->msa_flags & FLG_MSA_ALIAS);

		/*
		 * If the STUB_OBJECT mapfile directive is in force, and
		 * the symbol is for a data type, then we supply some
		 * default assertions for symbols that represent data.
		 *
		 * -	GLOBAL binding
		 * -	BITS attribute
		 *
		 * These defaults are necessary to ensure that a stub
		 * object and the real object it represents have compatible
		 * data symbols with regards to copy relocations, and
		 * symbol binding.
		 */
		if (stubdir && !has_alias &&
		    assert_data_symtype[ELF_ST_TYPE(sdp->sd_sym->st_info)]) {
			if ((msap->msa_flags & FLG_MSA_BIND) == 0) {
				msap->msa_bind = STB_GLOBAL;
				msap->msa_flags |= FLG_MSA_BIND;
			}

			if ((msap->msa_flags & FLG_MSA_SH_ATTR) == 0) {
				msap->msa_sh_attr = 0;
				msap->msa_flags |= FLG_MSA_SH_ATTR;
			}
			if ((msap->msa_sh_attr &
			    (SH_ATTR_MSA_BITS | SH_ATTR_MSA_NOBITS)) == 0)
				msap->msa_sh_attr |= SH_ATTR_MSA_BITS;
		}

		/* ASSERT binding */
		if ((msap->msa_flags & FLG_MSA_BIND) &&
		    (msap->msa_bind != ELF_ST_BIND(sdp->sd_sym->st_info))) {
			eprintf(ofl->ofl_lml, ERR_FATAL,
			    MSG_INTL(MSG_MAP_SYMAS_BIND), msap->msa_mfname,
			    EC_LINENO(msap->msa_mflineno), sdp->sd_name,
			    conv_sym_info_bind(ELFOSABI_SOLARIS,
			    ld_targ.t_m.m_mach, ELF_ST_BIND(
			    sdp->sd_sym->st_info), CONV_FMT_ALT_CFNP, &buf1),
			    conv_sym_info_bind(ELFOSABI_SOLARIS,
			    ld_targ.t_m.m_mach, msap->msa_bind,
			    CONV_FMT_ALT_CFNP, &buf2));
			r = FALSE;
		}

		/* ASSERT size */
		if ((msap->msa_flags & FLG_MSA_SIZE) &&
		    (msap->msa_size != sdp->sd_sym->st_size)) {
			eprintf(ofl->ofl_lml, ERR_FATAL,
			    MSG_INTL(MSG_MAP_SYMAS_SIZE), msap->msa_mfname,
			    EC_LINENO(msap->msa_mflineno), sdp->sd_name,
			    sdp->sd_sym->st_size, msap->msa_size);
			r = FALSE;
		}

		/* ASSERT type */
		if ((msap->msa_flags & FLG_MSA_TYPE) &&
		    (msap->msa_type != ELF_ST_TYPE(sdp->sd_sym->st_info))) {
			eprintf(ofl->ofl_lml, ERR_FATAL,
			    MSG_INTL(MSG_MAP_SYMAS_TYPE), msap->msa_mfname,
			    EC_LINENO(msap->msa_mflineno), sdp->sd_name,
			    conv_sym_info_type(ld_targ.t_m.m_mach,
			    ELF_ST_TYPE(sdp->sd_sym->st_info), 0, &buf1),
			    conv_sym_info_type(ld_targ.t_m.m_mach,
			    msap->msa_type, 0, &buf2));
			r = FALSE;
		}

		/* ASSERT attributes */
		if (msap->msa_flags & FLG_MSA_SH_ATTR) {
			/* ASSERT BITS section */
			if ((msap->msa_sh_attr & SH_ATTR_MSA_BITS) &&
			    sdp->sd_isc &&
			    (sdp->sd_isc->is_shdr->sh_type == SHT_NOBITS)) {
				eprintf(ofl->ofl_lml, ERR_FATAL,
				    MSG_INTL(MSG_MAP_SYMAS_BITS),
				    msap->msa_mfname,
				    EC_LINENO(msap->msa_mflineno),
				    sdp->sd_name);
				r = FALSE;
			}

			/* ASSERT NOBITS section */
			if ((msap->msa_sh_attr & SH_ATTR_MSA_NOBITS) &&
			    sdp->sd_isc &&
			    (sdp->sd_isc->is_shdr->sh_type != SHT_NOBITS)) {
				eprintf(ofl->ofl_lml, ERR_FATAL,
				    MSG_INTL(MSG_MAP_SYMAS_NOBITS),
				    msap->msa_mfname,
				    EC_LINENO(msap->msa_mflineno),
				    sdp->sd_name);
				r = FALSE;
		...
		/*
		 * ASSERT alias symbol: An alias symbol must have the same
		 * value, size, and type as the main symbol. It is not necessary
		 * to check the attributes, because if the address and size
		 * match, they occupy the same spot and will therefore by
		 * necessity already have the right attributes.
		 */
		if (has_alias) {
			Sym_desc	*asdp;

			if ((asdp = ld_sym_find(msap->msa_alias, SYM_NOHASH,
			    NULL, ofl)) == NULL) {
				eprintf(ofl->ofl_lml, ERR_FATAL,
				    MSG_INTL(MSG_MAP_SYMAS_ALIASNF),
				    msap->msa_mfname,
				    EC_LINENO(msap->msa_mflineno),
				    sdp->sd_name, msap->msa_alias);
				r = FALSE;
			} else {
				if (sdp->sd_sym->st_value !=
				    asdp->sd_sym->st_value) {
					eprintf(ofl->ofl_lml, ERR_FATAL,
					    MSG_INTL(MSG_MAP_SYMAS_ALIASVL),
					    msap->msa_mfname,
					    EC_LINENO(msap->msa_mflineno),
					    sdp->sd_name, msap->msa_alias,
					    EC_XWORD(sdp->sd_sym->st_value),
					    EC_XWORD(asdp->sd_sym->st_value));
					r = FALSE;
				}

				if (sdp->sd_sym->st_size !=
				    asdp->sd_sym->st_size) {
					eprintf(ofl->ofl_lml, ERR_FATAL,
					    MSG_INTL(MSG_MAP_SYMAS_ALIASSZ),
					    msap->msa_mfname,
					    EC_LINENO(msap->msa_mflineno),
					    sdp->sd_name, msap->msa_alias,
					    EC_XWORD(sdp->sd_sym->st_size),
					    EC_XWORD(asdp->sd_sym->st_size));
					r = FALSE;
				}

				if (ELF_ST_TYPE(sdp->sd_sym->st_info) !=
				    ELF_ST_TYPE(asdp->sd_sym->st_info)) {
					eprintf(ofl->ofl_lml, ERR_FATAL,
					    MSG_INTL(MSG_MAP_SYMAS_ALIASTP),
					    msap->msa_mfname,
					    EC_LINENO(msap->msa_mflineno),
					    sdp->sd_name, msap->msa_alias,
					    conv_sym_info_type(
	...
	/*
	 * If any errors were issued above, or if we are building a stub
	 * object, then stop here. Otherwise, continue on to do data symbol
	 * alias verification.
	 */
	if (!r || (ofl->ofl_dtflags_1 & DF_1_STUB))
		return (r);

	/*
	 * We require all global data to have an ASSERT. Hence, all global
	 * data symbols are visible in the assert record list.
	 *
	 * Compare each ASSERT record for a given data symbol to every other
	 * assert record with a symbol that aliases it. Verify that the mapfile
	 * alias assertions match those found in the actual objects. In each
	 * such alias group, there should be one symbol defined without the
	 * ALIAS attribute, and all the others should be using ALIAS to point
	 * at the main item, or each other. If there is more than one symbol
	 * not using ALIAS in the group, that means that the ASSERT mapfile
	 * attributes defined them independently of each other instead of
	 * using ALIAS to tie them together, and that the mapfile needs
	 * to be fixed.
	 */
	for (ALIST_TRAVERSE(ofl->ofl_mapassert, idx, msap)) {
		ld_map_sym_assert_t	*msap2;
		Aliste		idx2;
		Sym_desc	*sdp = msap->msa_sdp;
		Sym		*sym = sdp->sd_sym;
		...
		/*
		 * assert_data_symtype[] catches plain data, common, and
		 * tls. At this point in the link, common variables have
		 * been converted to plain data, so though we use
		 * assert_data_symtype[], we know and rely on the fact that
		 * only plain data or TLS can appear here.
		 */

		if (!assert_data_symtype[type])
			continue;

		for (ALIST_TRAVERSE_FROM_IDX(ofl->ofl_mapassert, idx2,
		    idx + 1, msap2)) {
			Sym_desc	*sdp2 = msap2->msa_sdp;
			Sym		*sym2 = sdp2->sd_sym;
			uchar_t		type2 = ELF_ST_TYPE(sym2->st_info);

			/*
			 * If the symbols are not aliases, then skip
			 * the check.
			 */
			if ((type != type2) ||
			...
			/*
			 * These symbols are aliases of each other. If neither
			 * ASSERT has an ALIAS attribute, then they have been
			 * defined independently, which is an error.
			 */
			if (((msap->msa_flags & FLG_MSA_ALIAS) == 0) &&
			    ((msap2->msa_flags & FLG_MSA_ALIAS) == 0)) {
				eprintf(ofl->ofl_lml, ERR_FATAL,
				    MSG_INTL(MSG_MAP_SYMAS_ALIASRQ),
				    msap->msa_mfname,
				    EC_LINENO(msap->msa_mflineno), sdp->sd_name,
				    msap2->msa_mfname,
				    EC_LINENO(msap2->msa_mflineno),
				    sdp2->sd_name);
				r = FALSE;
			}
		}

 ...
 * in which the object contains a signed negative value. dwarf_ehe_extract()
 * cannot accurately return such values, and using them will result in a bad
 * object. As we believe this doesn't happen in real objects, we test for
 * the situation and fail the link it if occurs. Should a legitimate need
 * for it arise, we'll need to design a better interface for
 * dwarf_ehe_extract().
 *
 * entry:
 *	data, dotp, ehe_flags, pcrel_base, datarel_base - As defined by
 *		dwarf_ehe_extract().
 *
 *	ofl - Output file descriptor
 *	osp - Output section descriptor
 *	base_data - Address of start of data for output section
 *	ret_value - Address of variable to receive result value
 *		from dwarf_ehe_extract().
 *
 * exit:
 *	On success, returns TRUE, and *value contains the result value.
 *	On failure, and error has been issued, and FALSE is returned.
 */

static Boolean
unwind_ehe_extract(Ofl_desc *ofl, Os_desc *osp, uchar_t *base_data,
    uchar_t *data, uint64_t *dotp, uint_t ehe_flags, uint64_t pcrel_base,
    uint64_t datarel_base, uint64_t *ret_value)
{
	Boolean		sneg;
	uint64_t	value;
	Is_desc		*isp;
	...
	value = dwarf_ehe_extract(data, dotp, ehe_flags,
	    ofl->ofl_dehdr->e_ident, &sneg, pcrel_base, datarel_base);
	if (sneg) {
		const char	*filename, *secname;
		uint64_t	data_off = data + dot - base_data;

		isp = ld_os_offset_to_isdesc(osp, data_off, &is_off);
		if (isp == NULL) {
			filename = secname = MSG_INTL(MSG_STR_UNKNOWN);
			is_off = 0;
		} else {
			filename = isp->is_file->ifl_name;
			secname = isp->is_name;
		}
		ld_eprintf(ofl, ERR_FATAL, MSG_INTL(MSG_UNW_NEGINT),
		    filename, secname, EC_OFF(data_off - is_off));
		return (FALSE);
	}
...
ld_unwind_populate_hdr(Ofl_desc *ofl)
{
	...
	gotaddr = (ofl->ofl_osgot != NULL) ?
	    ofl->ofl_osgot->os_shdr->sh_addr : 0;
	...
	for (APLIST_TRAVERSE(ofl->ofl_unwind, idx, osp)) {
		...
		while (off < size) {
			...
			if (id == 0) {
				...
				if (!unwind_ehe_extract(ofl, osp, data,
				    &data[off], &ndx, cieRflag, shdr->sh_addr +
				    off + ndx, gotaddr, &initloc))
					return (S_ERROR);

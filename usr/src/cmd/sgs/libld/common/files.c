...
/*
 * If a non-command line input file is rejected, offer guidance, and diagnose
 * the rejection.
 */
static void
ifl_rejected(Ofl_desc *ofl, Rej_desc *rej, Half mach, Word flags,
    const char *name)
{
	/*
	 * Guidence: Use correct search paths (-L).
	 */
	if (OFL_GUIDANCE(ofl, FLG_OFG_NO_LIBPATH) &&
	    ((flags & FLG_IF_CMDLINE) == 0)) {
		Conv_reject_desc_buf_t	reject_desc_buf;
		const char		*str;

		switch (rej->rej_type) {
		case SGS_REJ_UNKFILE:
			/*
			 * An unknown file is unusual, but can be caused due to
			 * a dependency being corrupt, or because of a build
			 * race condition.
			 */
			rej->rej_str = MSG_INTL(MSG_STR_UNKFILE);
			/* FALLTHROUGH */
		case SGS_REJ_STR:
			str = MSG_INTL(MSG_GUIDE_UNKNOWN);
			break;
		default:
			/*
			 * Other failures indicate that the dependency is
			 * invalid, perhaps because the user has mistakenly
			 * pointed at the wrong class of directory.
			 */
			str = MSG_INTL(MSG_GUIDE_LIBPATH);
			break;
		}
		ld_eprintf(ofl, ERR_GUIDANCE, str, name,
		    conv_reject_desc(rej, &reject_desc_buf, mach));
	}
	DBG_CALL(Dbg_file_rejected(ofl->ofl_lml, rej, mach));
}
    ...
    Rej_desc *rej)
{
	...
	if (ifl_verify(ehdr, ofl, &_rej) == 0) {
		...
		ifl_rejected(ofl, &_rej, ld_targ.t_m.m_mach, flags, name);
...
/*
 * Determine whether a section can be stripped, based upon:
 *
 *  -	this section is non-allocatable,
 *  -	a specific strip class is requested, or
 *  -	the nonallocatable strip class is requested, and the specific strip
 *	class has not been prevented.
 *
 * Return the class name that triggered the strip, for use in future
 * diagnostics.
 */
static const char *
can_strip(Ofl_desc *ofl, Shdr *shdr, int scndx)
{
	ofl_stripflag_t	flag;

	if (shdr->sh_flags & SHF_ALLOC)
		return (NULL);

	flag = strip_cls_items[scndx].flag;
	if (OFL_STRIP(ofl, flag))
		return (MSG_ORIG(strip_cls_items[scndx].class));

	flag |= strip_cls_items[SC_NONALLOC].flag;
	if (OFL_STRIP(ofl, flag))
		return (MSG_ORIG(strip_cls_items[SC_NONALLOC].class));
...
process_section(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	Is_desc	*isp;
	...
	isp->is_name = name;
	isp->is_scnndx = ndx;
	isp->is_flags = (FLG_IS_EXTERNAL | *isflags);
	isp->is_keyident = ident;
	if (((*isflags & FLG_IS_DISCARD) == 0) &&
	    ((isp->is_indata = elf_getdata(scn, NULL)) == NULL)) {
...
/*
 * Strip a section.  Diagnose the removal and generate an input section
 * descriptor to track this section.
 */
static inline uintptr_t
section_strip(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, Ofl_desc *ofl, Word *isflags, const char *class)
{
	DBG_CALL(Dbg_unused_strip(ofl->ofl_lml, name, ifl, ndx,
	    shdr->sh_size, class));

	*isflags |= FLG_IS_DISCARD;
	return (process_section(name, ifl, shdr, scn, ndx,
	    ld_targ.t_id.id_null, ofl, isflags));
}
...
#define	FP_FLAGS	(SF1_SUNW_FPKNWN | SF1_SUNW_FPUSED)
	...
	/*
	 * An SF1_SUNW_FPUSED by itself is viewed as bad practice.
	 */
    ...
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	...
	/*
	 * Never include .stab.excl sections in any output file.  If debugging
	 * information is being stripped, then remove any .stab sections.
	 */
	if (ident &&
	    (strncmp(name, MSG_ORIG(MSG_SCN_STAB), MSG_SCN_STAB_SIZE) == 0)) {
		const char	*class;

		if (((class = can_strip(ofl, shdr, SC_DEBUG)) != NULL) ||
		    (strncmp((name + MSG_SCN_STAB_SIZE),
		    MSG_ORIG(MSG_SCN_STABEXCL), MSG_SCN_STABEXCL_SIZE) == 0))
			return (section_strip(name, ifl, shdr, scn, ndx, ofl,
			    isflags, class));
	}
	 ...
	 */
	if (ident && (strcmp(name, MSG_ORIG(MSG_SCN_STRTAB)) == 0))
		ident = ld_targ.t_id.id_null;

	ret = process_section(name, ifl, shdr, scn, ndx, ident, ofl, isflags);
	if ((ret == 0) || (ret == S_ERROR))
		return (ret);
 ...
 *
 * entry:
 *	name, ident - As passed to process_progbits().
...
process_progbits_alloc(const char *name, int *ident, Word *isflags)
{
	if (name[0] == '.') {
		switch (name[1]) {
		case 'e':
			/*
			 * Non-amd64 platforms don't have a special section type
			 * for .eh_frame, and use SHT_PROGBITS. The amd64
			 * ABI specifies SHT_AMD64_UNWIND, and the compilers use
			 * it properly, but we sometimes encounter hand written
			 * assembly that uses SHT_PROGBITS on that platform.
			 * We therefore accept .eh_frame sections of type
			 * SHT_PROGBITS, matched by name, on all platforms.
			 * We are following the precedent set by the GNU
			 * toolchain in this matter.
			 */
			if (is_name_cmp(name, MSG_ORIG(MSG_SCN_EHFRAME),
			    MSG_SCN_EHFRAME_SIZE)) {
				*ident = ld_targ.t_id.id_unwind;
				*isflags |= FLG_IS_EHFRAME;
				return (TRUE);
			}
			break;
		case 'g':
	...
	if (*isflags & FLG_IS_STABS) {
		/*
		 * This is a work-around for x86 compilers that have set
		 * SHF_ALLOC for the .stab.index section.
		 *
		 * Because of this, make sure that the .stab.index does not
		 * end up as the last section in the text segment.  Older
		 * link-editors can produce segmentation violations when they
		 * strip (ld -s) against a shared object whose last section in
		 * the text segment is a .stab.
		 */
...
/*
 * Helper routine for process_progbits() to process comment sections.
 *
 * entry:
 *	ifl, scn, ofl - As passed to process_progbits().
 *
 * exit:
 *	Returns TRUE if the comment section was processed, and FALSE if
 *	an allocation error prevented it.
 */
static Boolean
process_progbits_comment(Ifl_desc *ifl, Elf_Scn *scn, Ofl_desc *ofl)
{
	Elf_Data	*dp;
	size_t		n;
	const char	*str;
	...
	/*
	 * Under some circumstances, we wish to enable -z relaxreloc
	 * for gcc/gnu generated objects, but it is not always possible
	 * to identify them easily. One stable watermark that we know about
	 * is their .comment sections. If relaxreloc has not been specified
	 * nor disabled, and we have not yet detected a GNU object, then look
	 * at the comment text to to see if it was produced by a GNU
	 * compiler/assembler.
	 */
	if (((ofl->ofl_flags1 & (FLG_OF1_RLXREL | FLG_OF1_NRLXREL)) != 0) ||
	    ((ifl->ifl_flags & FLG_IF_GNUCMT) != 0))
		return (TRUE);
	...
	/*
	 * By convention, a .comment section contains 1 or more
	 * NULL-terminated lines of text. Unlike strnstr(), this loop
	 * searches the entire section, across NULLs.
	 */
	while (n >= MSG_STR_GNUCMT_SIZE) {
		while ((n >= MSG_STR_GNUCMT_SIZE) && (*str != 'G')) {
			str++;
			n--;
		}
		if (n >= MSG_STR_GNUCMT_SIZE) {
			if (strncmp(MSG_ORIG(MSG_STR_GNUCMT),
			    str, MSG_STR_GNUCMT_SIZE) == 0) {
				ifl->ifl_flags |= FLG_IF_GNUCMT;
				return (TRUE);
			}
			str++;
			n--;
...
process_progbits(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	Word		sflags = shdr->sh_flags;
	uintptr_t	ret;
	const char	*class = NULL;

	/*
	 * Data can be discarded from the output file if:
	 *
	 *  -	.stab.excl sections are never included in any output file.
	 *  -	.stab, .debug, .line or .compcom sections can be removed if
	 *	debug stripping is requested.
	 *  -	.stab, .debug, .line or .compcom sections can be removed if
	 *	nonallocatable stripping is requested, and debugging stripping
	 *	hasn't been explicitly prevented, and these sections are
	 * 	non-allocatable.
	 *  -	.comment sections can be removed if comment stripping is
	 *	requested.
	 *  -	.comment sections can be removed if nonallocatable stripping is
	 *	requested, and comment stripping hasn't been explicitly
	 *	prevented, and these sections are non-allocatable.
	 *
	 * We believe that none of these sections require additional processing
	 * or have other sections that make reference to them, hence a section
	 * descriptor is not generated for them.
	 */
	if (ident &&
	    (strncmp(name, MSG_ORIG(MSG_SCN_STAB), MSG_SCN_STAB_SIZE) == 0)) {
		if ((strncmp((name + MSG_SCN_STAB_SIZE),
		    MSG_ORIG(MSG_SCN_STABEXCL), MSG_SCN_STABEXCL_SIZE) == 0) ||
		    ((class = can_strip(ofl, shdr, SC_DEBUG)) != NULL))
			return (section_strip(name, ifl, shdr, scn, ndx, ofl,
			    isflags, class));

		/*
		 * Identify any retained .stabs section for possible future
		 * processing.
		 */
		*isflags |= FLG_IS_STABS;
		...
		if ((class = can_strip(ofl, shdr, SC_DEBUG)) != NULL) {
			if ((strncmp(name, MSG_ORIG(MSG_SCN_DEBUG),
			    ...
			    (strcmp(name, MSG_ORIG(MSG_SCN_COMPCOM)) == 0))
				return (section_strip(name, ifl, shdr, scn, ndx,
				    ofl, isflags, class));
		}

		if ((name[0] == '.') && (name[1] == 'c') &&
		    (strcmp(name, MSG_ORIG(MSG_SCN_COMMENT)) == 0)) {
			if (!process_progbits_comment(ifl, scn, ofl))
				return (S_ERROR);
			if ((class = can_strip(ofl, shdr, SC_COMMENT)) != NULL)
				return (section_strip(name, ifl, shdr, scn, ndx,
				    ofl, isflags, class));
		}
	}
	...
	if (ident) {
		...
		} else if ((sflags & ~ALL_SHF_IGNORE) ==
		    (SHF_ALLOC | SHF_EXECINSTR)) {
			ident = ld_targ.t_id.id_text;
		} else if (sflags & SHF_ALLOC) {
			if (process_progbits_alloc(name, &ident,
			    isflags) == FALSE)
				return (S_ERROR);
		} else {
	 ...
	 */
	if (*isflags && (ret == 1))
		ifl->ifl_isdesc[ndx]->is_flags |= *isflags;

	return (ret);
}

/*
 * Process a generic strippable section.
 */
static uintptr_t
process_strip(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags, int scndx)
{
	const char	*class;

	if (ident && ((class = can_strip(ofl, shdr, scndx)) != NULL))
		return (section_strip(name, ifl, shdr, scn, ndx, ofl, isflags,
		    class));
...
process_debug(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	return (process_strip(name, ifl, shdr, scn, ndx, ident, ofl,
	    isflags, SC_DEBUG));
}

/*
 * Handles the SHT_SUNW_ANNOTATE sections.
 */
static uintptr_t
process_annotate(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	return (process_strip(name, ifl, shdr, scn, ndx, ident, ofl,
	    isflags, SC_ANNOTATE));
...
/*
 * Handles the SHT_NOTE sections.
 */
static uintptr_t
process_note(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	return (process_strip(name, ifl, shdr, scn, ndx, ident, ofl,
	    isflags, SC_NOTE));
...
process_sym_shndx(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	if (process_input(name, ifl, shdr, scn, ndx, ident, ofl,
	    isflags) == S_ERROR)
		return (S_ERROR);
...
static char *
expand(const char *caller, const char *name, char **next)
{
	...
	while (*optr) {
		...
		if (strncmp(optr, MSG_ORIG(MSG_STR_ORIGIN),
			...
			int	rlen;
			char	*eptr, rname[PATH_MAX];

			/*
			 * For $ORIGIN, first ensure we have the callers
			 * resolved name.  Expansion is effectively a
			 * concatenation of the callers directory name.  For
			 * example, a caller foo/bar/lib1.so with a dependency
			 * on $ORIGIN/lib2.so would be expanded to
			 * foo/bar/lib2.so.
			 */
			if ((rlen = resolvepath(caller, rname,
			    (PATH_MAX - 1))) >= 0) {
				rname[rlen] = '\0';
				caller = rname;
			}

			if ((eptr = strrchr(caller, '/')) == NULL) {
				*nptr++ = '.';
				nrem--;
			} else {
				size_t	dlen = eptr - caller;

				if (dlen >= nrem)
					return ((char *)name);

				(void) strncpy(nptr, caller, dlen);
				nptr = nptr + dlen;
				nrem -= dlen;
			}
 ...
 * versioning scheme. Use this fact to determine if the shared object
...
process_dynamic_isgnu(const char *name, Ifl_desc *ifl, Shdr *shdr,
    Elf_Scn *scn, Word ndx, int ident, Ofl_desc *ofl, Word *isflags)
{
	Dyn		*dyn;
	Elf_Data	*dp;
	uintptr_t	ret;
 ...
 * This is used to recognize SHT_PROGBITS .eh_frame sections.
 */
/*ARGSUSED1*/
static uintptr_t
...
/*
 * SHF_EXCLUDE flags is set for this section.  SHF_EXCLUDE sections are never
 * propagated to an executable or shared object.  These sections can also be
 * explicitly removed via the -z strip-class option.
 */
static uintptr_t
process_exclude(const char *name, Ifl_desc *ifl, Shdr *shdr, Elf_Scn *scn,
    Word ndx, Ofl_desc *ofl, Word *isflags)
{
	Word		sflags = shdr->sh_flags;
	const char	*class = NULL;
	...
	if ((sflags & SHF_EXCLUDE) &&
	    ...
	    ((class = can_strip(ofl, shdr, SC_EXCLUDE)) != NULL))) {
		/*
		 * Sections SHT_SYMTAB and SHT_DYNDYM, even with SHF_EXCLUDE,
		 * might be needed for future processing.  These sections need
		 * to be added to the input file descriptor, so return to the
		 * section processing engine to capture them as required.
		 */
		if (shdr->sh_type == SHT_SYMTAB || shdr->sh_type == SHT_DYNSYM)
			return (0);

		/*
		 * SHF_EXCLUDE sections aren't supposed to be allocatable.  If
		 * this is, issue a warning and return to the section processing
		 * engine to capture them as required.
		 */
		if (sflags & SHF_ALLOC) {
	...
	/*
	 * This section needs additional processing.
	 */
	return (0);
}
...
typedef uintptr_t	(* initial_func_t)(const char *, Ifl_desc *, Shdr *,
			    Elf_Scn *, Word, int, Ofl_desc *, Word *);
...
process_elf(Ifl_desc *ifl, Elf *elf, Ofl_desc *ofl)
{
	Elf_Scn		*scn;
	Shdr		*shdr;
	Word		ndx, sndx, ordndx = 0, ordcnt = 0, isflags = 0;
	char		*str, *name;
	Word		row, column;
	int		ident;
	uintptr_t	ret;
	...
	ret = process_strtab(name, ifl, shdr, scn, sndx, ld_targ.t_id.id_null,
	    ofl, &isflags);
	if ((ret == 0) || (ret == S_ERROR))
		return (ret);
	...
	while (scn = elf_nextscn(elf, scn)) {
		...
		/*
		 * Process any SHF_EXCLUDE or empty sections.
		 */
		if (ident) {
			if ((ret = process_exclude(name, ifl, shdr, scn,
			    ndx, ofl, &isflags)) == S_ERROR)
				return (S_ERROR);
			if (ret == 1)
				continue;
		}
		...
		if (row < SHT_NUM) {
			...
			case SHT_SUNW_cap:
				/*
				 * The bulk of capabilities are obtained from
				 * input relocatable objects, however some
				 * dependency software capabilities may need
				 * validating.
				 */
				if (process_section(name, ifl, shdr, scn,
				    ndx, ld_targ.t_id.id_null, ofl,
				    &isflags) == S_ERROR)
					return (S_ERROR);
			...
			case SHT_SUNW_ANNOTATE:
				if (process_annotate(name, ifl, shdr, scn,
				    ndx, ident, ofl, &isflags) == S_ERROR)
					return (S_ERROR);
				break;
 ...
 * entry:
 *	name - File name
 *	soname - SONAME for needed shared object, as described above
    ...
    Word flags, Ofl_desc *ofl, Rej_desc *rej, Ifl_desc **ifl_ret)
{
	...
	case ELF_K_AR:
		...
		if (ifl_ret != NULL) {
			...
			ifl_rejected(ofl, &_rej, ld_targ.t_m.m_mach,
			    flags, name);
	...
	/*
	 * GNU (gcc) objects have a history of producing incomplete COMDAT
	 * groups, requiring -z relaxreloc to link. If this object contains
	 * SHT_GROUP sections and has a .comment section that identifies
	 * it as being produced by gcc, then turn it on, unless specifically
	 * requested not to.
	 */
	if (((ofl->ofl_flags1 & (FLG_OF1_RLXREL | FLG_OF1_NRLXREL)) == 0) &&
	    (ifl->ifl_flags & FLG_IF_GNUCMT) &&
	    (alist_nitems(ifl->ifl_groups) > 0)) {
		ofl->ofl_flags1 |= FLG_OF1_RLXREL;
		DBG_CALL(Dbg_reloc_relax(ofl->ofl_lml, DBG_RLXREL_GRPCMT,
		    ifl->ifl_name, NULL));
	}
 ...
 *
 * Note, to simplify the interaction with callers, the input file descriptor is
    ...
    Word flags, Rej_desc *rej, Ifl_desc **ifl_ret)
{
	...
	if ((elf = elf_begin(nfd, ELF_C_READ, NULL)) == NULL) {
	 ...
	 *
	 * A file descriptor of -1, or a NULL ELF descriptor indicates the
	 * file should be ignored.
	 */
	ld_sup_open(ofl, &npath, &nfile, &nfd, flags, &elf, NULL, 0,
	    elf_kind(elf));
	...
	open_ret = ld_process_ifl(npath, nfile, nfd, elf, flags, ofl,
	    rej, ifl_ret);

	/*
	 * Determine whether the input file we're processing has been externally
	 * modified while it has been processed.  The intent is to warn against,
	 * and perhaps point to, parallel build problems, where files are being
	 * read while someone is still updating them.  Obtaining the files
	 * status at this point is convenient, as the file descriptor is still
	 * available.  Most of the processing of the files data has been
	 * carried out, or the data has been cached.  This might not be
	 * foolproof, as the files memory image might still be read to gather
	 * additional information.
	 *
	 * Ignore any files that have been exchanged by a support library, as
	 * the initial fstat() recorded in the input file descriptor will have
	 * been the status of the original file.
	 */
	if (ifl_ret && (nfd == fd) && ((ifl = *ifl_ret) != NULL)) {
		struct stat	status;

		(void) fstat(fd, &status);

		if (ifl->ifl_mtime != status.st_mtime) {
			ld_eprintf(ofl, ERR_WARNING,
			    MSG_INTL(MSG_FIL_MODIFIED), npath);
		}
	}
	(void) close(nfd);
    ...
    Ofl_desc *ofl, Rej_desc *rej)
{
	...
	if ((fd = open(path, O_RDONLY)) == -1)
		...
		if ((open_ret = ld_process_open(_path, &_path[dlen], fd, ofl,
		    0, rej, &ifl)) != 1)
			return ((Ifl_desc *)open_ret);
		return (ifl);
	}
	...
	for (APLIST_TRAVERSE(ofl->ofl_soneed, idx1, sdf)) {
		...
		if (slash) {
			...
			if ((fd = open(file, O_RDONLY)) == -1) {
				...
				ifl = NULL;
				if (ld_process_open(file, ++slash,
				    fd, ofl, 0, &_rej, &ifl) == S_ERROR)
					return (S_ERROR);

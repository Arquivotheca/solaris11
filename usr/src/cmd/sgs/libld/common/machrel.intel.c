	...
	if (orsp->rel_isdesc && ((orsp->rel_flags &
		...
		if (DBG_ENABLED) {
			...
			DBG_CALL(Dbg_reloc_discard(ofl->ofl_lml, M_MACH,
			    SHT_REL, orsp, offset, ld_reloc_sym_name));
		}
		return (1);
	}
...
tls_fixups(Ofl_desc *ofl, Rel_desc *arsp)
{
	...
	if (sdp->sd_ref == REF_DYN_NEED) {
		...
		case R_386_TLS_GD:
			...
			DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH,
			    SHT_REL, arsp, offset, ld_reloc_sym_name,
			    R_386_TLS_GOTIE));
			 ...
			 */
			addr -= 3;
			reg = (addr[2] & SIB_MSK_IND) >> 3;
			(void) memcpy(addr, tlsinstr_gd_ie,
			    sizeof (tlsinstr_gd_ie));

			/*
			 * set register %r1 into the addl instruction.
			 */
			addr[0x7] |= reg;
			return (FIX_RELOC);
		...
		case R_386_TLS_GD_PLT:
			/*
			 * tls_get_addr() references are ignored as part of the
			 * TLS_GD transition.
			 */
			DBG_CALL(Dbg_reloc_ignored(ofl->ofl_lml, M_MACH,
			    SHT_REL, arsp, offset, ld_reloc_sym_name));
			return (FIX_DONE);
		}
	}
	...
	case R_386_TLS_GD:
		 ...
		 *	0xc
		 */
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_REL,
		    arsp, offset, ld_reloc_sym_name, R_386_TLS_LE));
	...
	case R_386_TLS_LDM_PLT:
		/*
		 * Transition:
		 *	0x00 call __tls_get_addr()
		 * to:
		 *	0x00 nop, nop, nop, nop, nop
		 */
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_REL,
		    arsp, offset, ld_reloc_sym_name, R_386_NONE));
	...
	case R_386_TLS_GOTIE:
		/*
		 * These transitions are a little different than the others,
		 * in that we could have multiple instructions pointed to by a
		 * single relocation.  Depending upon the instruction, we
		 * perform a different code transition.  Known transitions:
		 *
	...
	case R_386_TLS_IE:
		/*
		 * These transitions are a little different than the others,
		 * in that the relocation can point at one of several possible
		 * instructions.  Depending upon the instruction, we perform a
		 * different code transition.  Known transitions:
		 *
...
ld_do_activerelocs(Ofl_desc *ofl)
{
	...
	REL_CACHE_TRAVERSE(&ofl->ofl_actrels, idx, rcbp, arsp) {
		...
		DBG_CALL(Dbg_reloc_doact(ofl->ofl_lml, ELF_DBG_LD_ACT,
		    M_MACH, SHT_REL, arsp, offset, value, ld_reloc_sym_name,
		    NULL));
...
ld_reloc_TLS(Boolean local, Rel_desc * rsp, Ofl_desc * ofl)
{
	...
	if ((rtype == R_386_TLS_GD_PLT) || (rtype == R_386_TLS_LDM_PLT)) {
		...
		if (ld_sym_add_u(MSG_ORIG(MSG_SYM_TLSGETADDR_UU),
		    ofl, MSG_STR_TLSREL, &tlsgetsym) == LD_ERROR)
			return (S_ERROR);

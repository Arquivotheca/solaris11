...
ld_perform_outreloc(Rel_desc *orsp, Ofl_desc *ofl, Boolean *remain_seen)
{
	...
	if (orsp->rel_isdesc && ((orsp->rel_flags &
		...
		if (DBG_ENABLED) {
			...
			offset = (uintptr_t)orsp->rel_roffset + (uintptr_t)
			    _elf_getxoff(orsp->rel_isdesc->is_indata);

			DBG_CALL(Dbg_reloc_discard(ofl->ofl_lml, M_MACH,
			    SHT_RELA, orsp, offset, ld_reloc_sym_name));
		}
		return (1);
	}
...
static uchar_t tlsinstr_ie_le_m[] = {
	/*
	 *	0x00 movq x@gottpoff(%rip), %rax
	 */
	0x48, 0xc7, 0xc0, 0x00, 0x00,
	0x00, 0x00
};
static uchar_t tlsinstr_ie_le_l[] = {
	/*
	 *	0x00 leaq x@gottpoff(%rip), %rax
	 */
	0x48, 0x8d, 0x80, 0x00, 0x00,
	0x00, 0x00
};
...
tls_fixups(Ofl_desc *ofl, Rel_desc *arsp)
{
	...
	offset = (uintptr_t)arsp->rel_roffset +
	    (uintptr_t)_elf_getxoff(arsp->rel_isdesc->is_indata);
	addr = (uchar_t *)(offset +
	    (uintptr_t)RELAUX_GET_OSDESC(arsp)->os_outdata->d_buf);

	/*
	 * Although individual TLS relocations are caught in the cases below,
	 * tls_get_addr() references can also be associated with these
	 * transitions, and these references can be identified with generic
	 * relocations.  These tls_get_addr() references are caught in
	 * ld_reloc_plt() and ignored here.
	 */
	if (arsp->rel_flags & FLG_REL_GETADDR) {
		DBG_CALL(Dbg_reloc_ignored(ofl->ofl_lml, M_MACH, SHT_REL,
		    arsp, offset, ld_reloc_sym_name));
		return (FIX_DONE);
	}
	...
	if (sdp->sd_ref == REF_DYN_NEED) {
		...
		case R_AMD64_TLSGD:
			...
			DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH,
			    SHT_RELA, arsp, offset, ld_reloc_sym_name,
			    R_AMD64_GOTTPOFF));
			...
			/*
			 * Adjust the address to be the beginning of the
			 * instruction sequence.
			 */
			addr -= 4;
			(void) memcpy(addr, tlsinstr_gd_ie,
	...
	case R_AMD64_TLSGD:
		 ...
		 *	0x10
		 */
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_AMD64_TPOFF32));
		arsp->rel_rtype = R_AMD64_TPOFF32;
		arsp->rel_roffset += 8;
		arsp->rel_raddend = 0;

		/*
		 * Adjust the address and write the new instruction sequence.
		 */
		addr -= 4;
		(void) memcpy(addr, tlsinstr_gd_le, sizeof (tlsinstr_gd_le));
		return (FIX_RELOC);
	...
	case R_AMD64_GOTTPOFF:
		 ...
		 * IE -> LE
		 *
		 * In this transition only the instruction at 0x9 is modified.
		 *
		 * Transition:
		 *	0x00 movq %fs:0, %rax
		 ...
		 *	0x10
		 *
		 * In this transition only the instruction at 0x0 is modified.
		 *
		 * Transition:
		 *	0x00 movq x@gottopoff(%rip), %rax
		 *	0x07 movq %fs:(%rax), %rax
		 * To:
		 *	0x00 movq x@tpoff(%rip), %rax
		 *	0x07 movq %fs:(%rax), %rax
		 *
		 * The amd64 ABI states that the register used will be %rax,
		 * but the wording is ambiguous as to whether %rax is the
		 * only register allowed, or simply an example of a wider family
		 * of possible registers.  The GNU compilers make the later
		 * assumption, and can use other registers, so we decode the
		 * instruction to determine which register is really used, and
		 * apply that to the transitioned instruction.
		 */
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_AMD64_TPOFF32));
		arsp->rel_rtype = R_AMD64_TPOFF32;
		...
		/*
		 * Determine for the addq or movq instruction, the register
		 * being used.
		 *
		 *	48 83 (15) 00 00 00  addq   x@gottopoff(%rip),%rax
		 *	48 8b (15) 00 00 00  movq   x@gottopoff(%rip),%rax
		 *
		 *	(*) byte contains register definition
		 */
		addr -= 1;
		reg = (*addr >> 3) & 0x7;

		/*
		 * Determine from the second byte whether a addq or movq
		 * instructions must be modified, and then adjust the address
		 * to be the beginning of the instruction sequence from which
		 * to copy the new instruction.
		 */
		addr -= 1;
		if (*addr & 0x8)
			(void) memcpy(--addr, tlsinstr_ie_le_m,
			    sizeof (tlsinstr_ie_le_m));
		else
			(void) memcpy(--addr, tlsinstr_ie_le_l,
			    sizeof (tlsinstr_ie_le_l));

		/*
		 * Apply the original register to the new instruction.
		 */
		addr += 2;
		*addr = *addr | reg;
		return (FIX_RELOC);
	...
	case R_AMD64_TLSLD:
		 ...
		 */
		DBG_CALL(Dbg_reloc_transition(ofl->ofl_lml, M_MACH, SHT_RELA,
		    arsp, offset, ld_reloc_sym_name, R_AMD64_NONE));

		addr -= 3;
		(void) memcpy(addr, tlsinstr_ld_le, sizeof (tlsinstr_ld_le));
		return (FIX_DONE);
...
ld_do_activerelocs(Ofl_desc *ofl)
{
	...
	REL_CACHE_TRAVERSE(&ofl->ofl_actrels, idx, rcbp, arsp) {
		...
		if ((arsp->rel_flags & FLG_REL_GOT) &&
			...
			R1addr = (uintptr_t)(gotndx * M_GOT_ENTSIZE);
		...
		/*
		 * Get the address of the data item we need to modify.  Note,
		 * the offset is recomputed, as it may have been modified when
		 * performing any TLS fixups.
		 */
		offset = (uintptr_t)arsp->rel_roffset +
		    (uintptr_t)_elf_getxoff(arsp->rel_isdesc->is_indata);
		addr = (uchar_t *)(offset + (uintptr_t)osp->os_outdata->d_buf);

		DBG_CALL(Dbg_reloc_doact(ofl->ofl_lml, ELF_DBG_LD_ACT,
		    M_MACH, SHT_RELA, arsp, offset, value, ld_reloc_sym_name,
		    NULL));

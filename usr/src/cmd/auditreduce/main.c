...
/*
 * Solaris Audit post-selection and audit.log(4) merging tool.
 *
...
extern void	init_tokens(void);	/* shared with praudit(1M) */
...
main(int argc, char **argv)
{
	...
	/*
	 * Initialize signals, process cmd line options, setup root process
	 * output stream.
	 */
	if (init_sig() == -1 ||
	...
	if (mfork(pcb, pcbnum, 0, pcbnum - 1) == -1) {
		/*
	...
	/* free per-process allocated memory for pcbs/fcbs */
	while (pcb->pcb_lo <= pcb->pcb_hi) {
		fcb = pcb->pcb_below[pcb->pcb_lo].pcb_dfirst;
		while (fcb != NULL) {
			fcbn = fcb->fcb_next;
			free(fcb);
			fcb = fcbn;
		...
		if (pcb->pcb_flags & PF_LEAF) {
			free(pcb->pcb_below[pcb->pcb_lo].pcb_rec);
		}
		pcb->pcb_lo++;
	}
	free(pcb->pcb_below);
...
mfork(audit_pcb_t *pcb, int nsp, int lo, int hi)
{
	 ...
	 */
		...
		pcb->pcb_below = a_calloc(tofork, sizeof (*pcb));
		...
		for (i = 0; i < tofork; i++) {
			...
			if (pipe(fildes) == -1) {
				perror(gettext(
				    "auditreduce: could not get a pipe"));
				return (-1);
			}

			/* Convert descriptors to streams. */
			if ((pcbn->pcb_fpr = fdopen(fildes[0], "r")) == NULL) {
				perror(gettext("auditreduce: could not get "
				    "read stream for pipe"));
				return (-1);
			}
...
trim_mem(audit_pcb_t *pcb)
{
	...
	if (pcb->pcb_flags & PF_LEAF) {
		...
		pcb->pcb_below = a_calloc(count, size);
		/* save this pcb's portion */
		(void) memcpy(pcb->pcb_below, &audit_pcbs[pcb->pcb_lo],
		    count * size);
		rm_mem(pcb);
 ...
 * 	pcb in audit_pcbs[] free the record buffer and all of the fcbs.
 * 	Then free audit_pcbs[].
 * .call	rm_mem(pcbr).
...
rm_mem(audit_pcb_t *pcbr)
{
	...
	for (i = 0; i < pcbsize; i++) {
		/*
		 * Don't free the record buffer and fcbs
		 * for the pcbs this process is using.
		 */
...
c_close(audit_pcb_t *pcb, int	i)
{
	...
	for (j = 0; j <= i; j++) {
		...
		if (fclose(pcbt->pcb_fpr) == EOF && !f_quiet) {
			perror(gettext(
			    "auditreduce: initial close on pipe failed"));
		}
...
{
	if (fclose(pcbn->pcb_fpw) == EOF && !f_quiet) {
...
audit_stats(void)
{
	...
	if (getrlimit(RLIMIT_NOFILE, &rl) != -1) {
		(void) fprintf(stderr,
		    gettext("%s The system allows %llu files per process.\n"),
		    ar, rl.rlim_cur);
	}
	(void) fprintf(stderr, gettext("%s There were %d file(s) %d file "
	    "group(s) %d process(es) %d layer(s).\n"), ar, filenum, pcbnum,
	    total_procs, total_layers);
}
...
rm_outfile(void)
{
	...
	if (f_outfile != NULL) {
		if (unlink(f_outtemp) == -1) {
			(void) sprintf(errbuf,
			    gettext("%s delete on %s failed"), ar, f_outtemp);
			perror(errbuf);
		}
	}
#else
	(void) fprintf(stderr, gettext("%s Warning: Incomplete audit file "
	    "may have been generated - %s\n"), ar,
...
close_outfile(void)
{
	if (fclose(stdout) == EOF) {
		(void) snprintf(errbuf, sizeof (errbuf), gettext("%s close on "
		    "%s failed"), ar,
		    f_outfile != NULL ? f_outfile : "standard output");
		perror(errbuf);
		return (-1);
	}
	...
	if (fwrite(for_adr, sizeof (char), adr_count(&adr), stdout) !=
		...
		if (when == f_start) {
			(void) snprintf(errbuf, sizeof (errbuf),
			    gettext("%s error writing header to %s. "), ar,
			    f_outfile != NULL ? f_outfile :
			    gettext("standard output"));
		} else {
			(void) snprintf(errbuf, sizeof (errbuf),
			    gettext("%s error writing trailer to %s. "), ar,
			    f_outfile != NULL ? f_outfile :
			    gettext("standard output"));
		}
		perror(errbuf);
...
open_outfile(void)
{
	...
	if (f_outfile != NULL) {
		if (asprintf(&f_outtemp, "%sXXXXXX", f_outfile) == -1) {
			(void) snprintf(errbuf, sizeof (errbuf),
			    gettext("%s could not allocate memory"), ar);
			perror(errbuf);
			return (-1);
		}
		if ((tmpfd = mkstemp(f_outtemp)) == -1) {
			(void) snprintf(errbuf, sizeof (errbuf),
			    gettext("%s could not create temporary file"), ar);
			perror(errbuf);
			free(f_outtemp);
			return (-1);
		}
		...
		if (tmpfd != fileno(stdout)) {
			if ((dup2(tmpfd, fileno(stdout))) == -1) {
				(void) snprintf(errbuf, sizeof (errbuf),
				    gettext("%s could not assign %s to the "
...
init_options(void)
{
	...
	pcbsize = PCB_INITSIZE;

	audit_pcbs = a_calloc(pcbsize, sizeof (*audit_pcbs));
...
/*
 * .func	set_err - sets the error string.
 * .ret	void.
 */
/* PRINTFLIKE1 */
void
set_err(char *format_str, ...)
{
	va_list	vl;

	errbuf[0] = '\0';
	va_start(vl, format_str);
	(void) vsnprintf(errbuf, sizeof (errbuf), format_str, vl);
	va_end(vl);

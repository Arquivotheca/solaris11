...
static size_t	buf_resize(char **, size_t);
 ...
 */
int
mproc(register audit_pcb_t *pcbr)
{
	int	i, ret, junk;
	...
	while ((pcb = aget()) != NULL) {	/* get oldest record */
		if (write_recs(pcbr, pcb, &nprecs) == -1) {
			return (-1);
		}
	...
	if (pcbr->pcb_flags & PF_ROOT) {
		if (nprecs == 0) {		/* write header if no records */
			if (write_header() == -1) {
				return (-1);
			}
		}
		...
		if (write_recs(pcbr, pcb, &junk) == -1) {
			return (-1);
		}
		if (fclose(pcbr->pcb_fpw) == EOF && !f_quiet) {
			(void) fprintf(stderr,
			    gettext("%s could not close pipe.\n"), ar);
		}
	}
 ...
 */
static void
asort(register audit_pcb_t *pcb)
{
 ...
 */
static int
get_file(register audit_pcb_t *pcb)
{
	FILE	*fp;
	audit_fcb_t	*fcb;

	/* Process file list until a good one is found or empty. */
	while (pcb->pcb_fpr == NULL) {
		if ((fcb = pcb->pcb_first) == NULL) {
			...
			if (check_header(fp, fcb->fcb_name) == -1) {
				if (!f_quiet) {
					(void) fprintf(stderr,
					    "%s %s:\n  %s.\n",
 ...
 */
static int
write_recs(register audit_pcb_t *pcbr, register audit_pcb_t *pcb, int *nprecs)
{
	adr_t	adr;
	char	id;
	int32_t	size, szw;
	int	retry = 10;			/* amount of retries on EINTR */
	...
	for (;;) {
		szw = fwrite(pcb->pcb_rec, sizeof (char), size, pcbr->pcb_fpw);
		if (szw != size) {
			/* retry on EINTR on full system/shell pipe buffer */
			if (errno == EINTR && retry != 0) {
				retry--;
				size = size - szw;
				(void) memmove(pcb->pcb_rec, pcb->pcb_rec + szw,
				    size);
				continue;
			}
			if (pcbr->pcb_flags & PF_ROOT) {
				(void) snprintf(errbuf, sizeof (errbuf),
				    gettext("%s write failed to %s"), ar,
				    f_outfile != NULL ? f_outfile :
				    gettext("stdout"));
				perror(errbuf);
			} else {
				perror(gettext("auditreduce: write failed "
				    "to pipe"));
			}
			return (-1);
		}
 ...
 */
static int
get_recs(register audit_pcb_t *pcb, int	*nr)
{
	adr_t	adr;
	time_t	secs;
	int	tmp;
	...
	while (getrec) {
		ret = get_record(pcb->pcb_fpr, &pcb->pcb_rec, &pcb->pcb_size,
		    pcb->pcb_cur == NULL ? "pipe" : pcb->pcb_cur->fcb_name);
		if (ret > 0) {
			adrm_start(&adr, pcb->pcb_rec);
	...
	if (alldone == TRUE) {
		...
		get_trace(pcb);
#endif
		/* Leaf pcb and Error in record read. Display messages. */
		if (ret < 0 && pcb->pcb_flags & PF_LEAF || ret2 == -2) {
			...
			if (!f_quiet) {
				if (pcb->pcb_flags & PF_USEFILE) {
					...
					if (!strstr((pcb->pcb_cur)->fcb_file,
					    "not_terminated")) {
						(void) fprintf(stderr,
						    gettext("%s read error in "
						    "%s at record %d.\n"), ar,
						    (pcb->pcb_cur)->fcb_file,
						    pcb->pcb_nrecs);
					}
				} else {
					(void) fprintf(stderr, gettext("%s read"
					    " error in pipe at record %d.\n"),
					    ar, pcb->pcb_nrecs);
				}
			}

		} else if (pcb->pcb_flags & PF_USEFILE) {
			/*
 ...
 */
static int
check_rec(register audit_pcb_t *pcb)
{
	adr_t	adr;
	uint_t	bytes;
	char	version;
	...
	if (flags & M_CLASS) {
		...
		if (cacheauevent(&ev, event_type) <= 0) {
			(void) fprintf(stderr, gettext("Warning: invalid event "
			    "no %d in audit trail.\n"), event_type);
			    return (-1);
		}
	...
	if (!f_all) {
		if (m_after > tv.tv_sec || m_before <= tv.tv_sec) {
			return (-1);
		}
	}
 ...
 */
static void
check_order(register audit_pcb_t *pcb)
{
	...
	if (pcb->pcb_otime > pcb->pcb_time) {
		if (!f_quiet) {
			(void) memcpy(cptr1, ctime(&pcb->pcb_otime), 26);
			cptr1[24] = ' ';
			(void) memcpy(cptr2, ctime(&pcb->pcb_time), 26);
			cptr2[24] = ' ';
			(void) fprintf(stderr, gettext("%s %s had records out "
			    "of order: %s was followed by %s.\n"),
...
check_header(FILE *fp, char *fn)
{
	...
	if (adrf_char(&adrf, &id, 1) == -1) {
		set_err(gettext("%s is empty"), fn);
		return (-1);
	}
	if (!(id == AUT_OTHER_FILE32 || id == AUT_OTHER_FILE64)) {
		set_err(gettext("%s not an audit file "), fn);
		return (-1);
	}
	...
	if (pathlength != 0) {
		fname = a_calloc(1, (size_t)pathlength);
		if ((fread(fname, sizeof (char), pathlength, fp)) !=
		    pathlength) {
			set_err(gettext("error in header/filename read in %s"),
			    fn);
			free(fname);
			return (-1);
		}
 ...
 * .call	ret = get_record(fp, buf, cur_size, fn).
...
get_record(FILE *fp, char **buf, size_t *cur_size, char *fn)
{
	...
	/* Get the token type. It will be either a header or a file token. */
	(void) adrf_start(&adrf, &adr, fp);
	if (adrf_char(&adrf, &id, 1) == -1) {
		set_err(gettext("record expected but not found in %s"), fn);
		return (-1);
	}
	switch (id) {
	case AUT_HEADER32:
	...
	case AUT_HEADER64_EX:
		...
		if (*cur_size < lsize + leadin) {
			*cur_size = buf_resize(buf, lsize + leadin);
		}
		adr_start(&adr, *buf);
		adr_char(&adr, &id, 1);
		...
		if (fread(*buf + leadin, sizeof (char), lsize - leadin, fp) !=
		    lsize - leadin) {
			set_err(gettext("header token read failure in %s"), fn);
			return (-1);
		}
		return (lsize + leadin);
	case AUT_OTHER_FILE32: {
		int32_t secs, msecs;
		leadin =  2 * sizeof (int32_t) + sizeof (short) + sizeof (char);
		(void) adrf_int32(&adrf, (int32_t *)&secs, 1);
		(void) adrf_int32(&adrf, (int32_t *)&msecs, 1);
		(void) adrf_short(&adrf, &ssize, 1);
		if (*cur_size < (ssize + leadin)) {
			*cur_size = buf_resize(buf, ssize + leadin);
		}
		adr_start(&adr, *buf);
		adr_char(&adr, &id, 1);
	...
	case AUT_OTHER_FILE64: {
		int64_t secs, msecs;
		leadin =  2 * sizeof (int64_t) + sizeof (short) + sizeof (char);
		(void) adrf_int64(&adrf, (int64_t *)&secs, 1);
		(void) adrf_int64(&adrf, (int64_t *)&msecs, 1);
		(void) adrf_short(&adrf, &ssize, 1);
		if (*cur_size < (ssize + leadin)) {
...
/*
 * .func        resize_buf - resize buffer and store its new size.
 * .ret created buffer size.
 */
static size_t
buf_resize(char **buf, size_t buf_sz)
{
	free(*buf);
	*buf = a_calloc(1, buf_sz);

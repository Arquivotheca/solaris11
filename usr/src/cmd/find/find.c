...
enum Command
{
	...
	PRUNE, NOUSER, NOGRP, FSTYPE, LS, XATTR, ACL, MMIN, AMIN, CMIN, PRINT0,
	INAME
};
...
struct FTW *state;
{
	...
	while (np) {
		...
		case NAME: {
			...
			/*
			 * XPG4 find -name should not treat a leading '.'
			 * in a filename specially for pattern matching.
			 * /usr/bin/find -name will not pattern match a
			 * leading '.' in a filename, unless '.' is
			 * explicitly specified.
			 *
			 * Both /usr/bin/find -iname and XPG4 find -iname
			 * treat a leading '.' in the same way as XPG4 find
			 * -name.
			 */
#ifndef XPG4
			if (np->action == NAME)

 ...
 */
static CK_MECHANISM_TYPE compute_chksum = CKM_SHA256;
static CK_SESSION_HANDLE p11_session;
...
bart_create(int argc, char **argv)
{
	...
	if (compute_chksum != 0 && SUNW_C_GetMechSession(compute_chksum,
	    &p11_session) != CKR_OK) {
		(void) fprintf(stderr, "%s not supported on this system",
		    optarg);
	}
...
output_manifest(void)
{
	...
	if (compute_chksum == CKM_MD5 || compute_chksum == 0) {
		(void) printf("%s", MANIFEST_VER1_0);
	} else {
		(void) printf("%s", MANIFEST_VER1_1);
...
/*
 *
 * description:	This routine reads fdin in BUF_SIZE chunks, uses the bits
 *		to update the hash buffer, and outputs the chunks
 *		to stdout.  When fdin is exhausted, the hash is computed,
...
generate_hash(int fdin, char *hash_str)
{
	...
	int i;
	ssize_t amtread;
	CK_MECHANISM p11_mech = { 0 };
	CK_RV rv;

	p11_mech.mechanism = compute_chksum;

	if ((rv = C_DigestInit(p11_session, &p11_mech)) != CKR_OK) {
		(void) fprintf(stderr, "C_DigestInit failed: %lu\n", rv);
		return (1);
	}
	...
	for (;;) {
		...
		if (C_DigestUpdate(p11_session, buf, amtread) != CKR_OK)
			return (1);
	}

	if (C_DigestFinal(p11_session, hash, &hash_len) != CKR_OK)
		return (1);

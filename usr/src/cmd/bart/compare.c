...
static void
check_version(char *control, FILE *control_fd, char *test, FILE *test_fd)
{
	char *control_buf = NULL;
	char *test_buf = NULL;
	uint_t n = BUF_SIZE;

	(void) getline(&control_buf, &n, control_fd);
	(void) getline(&test_buf, &n, test_fd);

	if (control_buf == NULL)
		(void) fprintf(stderr, MISSING_VER, control);
	if (test_buf == NULL)
		(void) fprintf(stderr, MISSING_VER, test);

	if (strcmp(control_buf, MANIFEST_VER1_0) != 0 &&
	    strcmp(control_buf, MANIFEST_VER1_1) != 0)
		(void) fprintf(stderr, MISSING_VER, control);

	if (strcmp(test_buf, MANIFEST_VER1_0) != 0 &&
	    strcmp(test_buf, MANIFEST_VER1_1) != 0)
		(void) fprintf(stderr, MISSING_VER, test);

	if (strcmp(control_buf, test_buf) != 0) {
		(void) fprintf(stderr, MANIFEST_DIFF);
		exit(FATAL_EXIT);
	}
	...
	/*
	 * >= version 1.1 manifest check the hash too, note that we don't
	 * actually have to check the hash algorithm is valid just that
	 * the control and test manifests are using the same algorithm.
	 */
	(void) getline(&control_buf, &n, control_fd);
	(void) getline(&test_buf, &n, test_fd);
	if (strncmp(control_buf, HASH_HDR, strlen(HASH_HDR)) != 0) {
		(void) fprintf(stderr, HASH_MISSING, control);
		exit(FATAL_EXIT);
	}
	if (strncmp(test_buf, HASH_HDR, strlen(HASH_HDR)) != 0) {
		(void) fprintf(stderr, HASH_MISSING, test);
		exit(FATAL_EXIT);
	}
	if (strcmp(control_buf, test_buf) != 0) {
		(void) fprintf(stderr, HASH_DIFF);
		exit(FATAL_EXIT);
	}
out:
	free(control_buf);
...
boolean_t prog_fmt, uint_t flags)
{
	...
	check_version(control, control_fd, test, test_fd);

 ...
 */
static void
list_getsize(list_t *list, char *sortk)
{
	id_info_t *id;
	...
	for (id = list->l_head; id != NULL; id = id->id_next) {
		...
		if (match != NULL) {
			...
			/*
			 * The sort key needs to be updated to reflect
			 * proper rss/size values.
			 */
			if (strcmp("size", sortk) == 0) {
				id->id_key = id->id_size;
			} else if (strcmp("rss", sortk) == 0) {
				id->id_key = id->id_rssize;
			}
		}
		/*
...
prstat_scandir(DIR *procdir)
{
	...
	for (rewinddir(procdir); (direntp = readdir(procdir)); ) {
		...
		if (opts.o_outpmode & (OPT_LWPS | OPT_PSETS)) {
			int rep_lwp = 0;
...
main(int argc, char **argv)
{
	...
	/*
	 * Use sigaction to establish signal handlers so we can make write
	 * syscalls restartable on interrupt.  This allows us in turn to use
	 * ferror() to detect failed writes caused by stdout being closed.
	 */
	...
	(void) sigaction(SIGTSTP, &sigact, NULL);
	if (opts.o_outpmode & OPT_FULLSCREEN)
	...
	do {
		...
		if (opts.o_outpmode & OPT_USERS) {
			/*
			 * list_getsize gets the user wide statistics.
			 * Using -a with -k, -p, -P, -j, -C, -u should
			 * avoid the user wide statistics to print
			 * proper values computed by /proc aggregation.
			 */
			if (!(opts.o_outpmode & OPT_SPLIT))
				list_getsize(&users, sortk);
			list_sort(&users);
			list_print(&users);
			list_clear(&users);
		}
		if (opts.o_outpmode & OPT_TASKS) {
			/*
			 * list_getsize gets the task wide statistics.
			 * Using -T with -p, -P, -j, -C, -u, -U should
			 * avoid the user wide statistics to print
			 * proper values computed by /proc aggregation.
			 */
			if (!(opts.o_outpmode & OPT_SPLIT))
				list_getsize(&tasks, sortk);
			list_sort(&tasks);
			list_print(&tasks);
			list_clear(&tasks);
		}
		if (opts.o_outpmode & OPT_PROJECTS) {
			/*
			 * list_getsize gets the project wide statistics.
			 * Using -J with -k, -p, -P, -C, -u, -U should
			 * avoid the project wide statistics to print
			 * proper values computed by /proc aggregation.
			 */
			if (!(opts.o_outpmode & OPT_SPLIT))
				list_getsize(&projects, sortk);
			list_sort(&projects);
			list_print(&projects);
			list_clear(&projects);
		}
		if (opts.o_outpmode & OPT_ZONES) {
			/*
			 * list_getsize gets the zone wide statistics.
			 * Using -Z with -k, -p, -P, -j, -C, -u, -U should
			 * avoid the zone wide statistics to print
			 * proper values computed by /proc aggregation.
			 */
			if (!(opts.o_outpmode & OPT_SPLIT))
				list_getsize(&zones, sortk);
			list_sort(&zones);
			list_print(&zones);
			list_clear(&zones);
		}
		...
		if (opts.o_outpmode & OPT_TTY) {
			...
			errno = 0;
			pollset.revents = 0;
			stat = poll(&pollset, (nfds_t)1, timeout);
			if (! (errno == 0 || errno == EINTR) ||
			    pollset.revents & (POLLERR | POLLHUP | POLLNVAL))
				Die("read from stdin failed");
			if (stat > 0 && read(STDIN_FILENO, &key, 1) == 1 &&
			    tolower(key) == 'q')
				break;
		} else {

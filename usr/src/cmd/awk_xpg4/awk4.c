...
{
	register uint_t na;
	register uint_t seed;
	static uint_t oldseed = 0;

	if ((na = nargs(np)) > 1)
		awkerr(nargerr, s_srand);
	if (na == 0)
		seed = (uint_t)time((time_t *)0);
	else
		seed = (uint_t)exprint(getlist(&np));
	srand(seed);
	na = oldseed;
	oldseed = seed;
...
f_split(NODE *np)
{
	...
	for (;;) {
		...
		*ep = '\0';
		...
		(void) assign(tnp,
		    stringnode(cp, FALLOC|FSENSE, (size_t)(ep - cp)));
		*ep = c;
	}
	/*
...
f_close(NODE *np)
{
	...
	for (op = ofiles; op != NULL; op = op->f_next) {
...
f_asort(NODE *np)
{
	...
	if (array->n_type != ARRAY)
		awkerr(gettext("%s function requires an array"), s_asort);

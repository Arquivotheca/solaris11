...
#endif
#define	muloverflow()	if (((i2 != 0 && iresult/i2 != i1) ||		\
	...
	leaf:
			...
			return (STRING)lltoa((long long)np->n_int);
		{
			...
			(void) wsprintf(numbuf,
			    (const char *)
			    (tmp = wcstombsdup(exprstring(varCONVFMT))),
			    (double)np->n_real);
			if (tmp != NULL)
				free(tmp);
		}

...
uint_t	npattern;			/* Number of non-BEGIN patterns */
uint_t	nfield;				/* Number of fields (if splitdone) */
uint_t	fcount;				/* Field counter (used by blackfield) */
uint_t	phase;				/* BEGIN, END, or 0 */
uint_t	running = 0;			/* Set if not in compile phase */
uchar_t	catterm;			/* Can inject concat or ';' */
uint_t	lexlast = '\n';			/* Last lexical token */
uint_t	lineno = 0;			/* Current programme line number */
...
ushort_t slevel;			/* Scope level (0 == root) */
ushort_t loopexit;			/* Short circuit loop with keyword */

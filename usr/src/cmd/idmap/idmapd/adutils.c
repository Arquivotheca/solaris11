...
typedef struct idmap_q {
	...
	idmap_id_type		want_unix_type;	/* desired UNIX type */
	/* results */
	...
	LDAPMessage *res, LDAP *ld)
{
	...
	if (state->directory_based_mapping == DIRECTORY_MAPPING_NAME &&
		 ...
		 * its value.
		 *
		 * If the caller specified a desired type, use the
		 * corresponding attribute.  If not, use the attribute
		 * appropriate to the AD object found.
		 */

		if (q->want_unix_type == IDMAP_UID)
			attr = state->ad_unixuser_attr;
		else if (q->want_unix_type == IDMAP_GID)
			attr = state->ad_unixgroup_attr;
		else if (sid_type == IDMAP_USID)
			attr = state->ad_unixuser_attr;
		else if (sid_type == IDMAP_GSID)
			attr = state->ad_unixgroup_attr;
...
idmap_batch_add1(idmap_query_state_t *state, const char *filter,
	char *ecanonname, char *edomain, idmap_id_type want_unix_type,
	...
	idmap_retcode *rc)
{
	...
	q->edomain = edomain;
	q->want_unix_type = want_unix_type;
...
idmap_name2sid_batch_add1(idmap_query_state_t *state,
	const char *name, const char *dname, idmap_id_type want_unix_type,
	char **dn, char **attr, char **value,
	...
	posix_id_t *pid, idmap_retcode *rc)
{
	...
	retcode = idmap_batch_add1(state, filter, ecanonname, edomain,
	    want_unix_type, dn, attr, value, canonname, NULL, sid, rid,
	    sid_type, unixname, pid, rc);
...
idmap_sid2name_batch_add1(idmap_query_state_t *state,
	const char *sid, const rid_t *rid, idmap_id_type want_unix_type,
	char **dn, char **attr, char **value,
	...
	char **unixname, posix_id_t *pid, idmap_retcode *rc)
{
	...
	retcode = idmap_batch_add1(state, filter, NULL, NULL, want_unix_type,

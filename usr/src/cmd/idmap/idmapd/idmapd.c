 ...
 * with others.  We putenv("KRB5CCNAME=_PATH_SYSVOL/idmap/ccache") in main()
...
main(int argc, char **argv)
{
	...
	if ((rc = idmap_cfg_start_updates()) < 0) {
		/* Total failure */
		idmapdlog(LOG_ERR, "could not start config updater");
		exit(rc < -2 ? SMF_EXIT_ERR_CONFIG : 1);
	}
...
static void
trace_vsprintf(nvlist_t *entry, char *n1, char *n2, char *fmt, va_list va)
{
	char *buf;

	(void) vasprintf(&buf, fmt, va);
	if (buf != NULL) {
		trace_str(entry, n1, n2, buf);
		free(buf);
	}
}

static void
trace_sprintf(nvlist_t *entry, char *n1, char *n2, char *fmt, ...)
{
	va_list va;

	va_start(va, fmt);
	trace_vsprintf(entry, n1, n2, fmt, va);
	va_end(va);
}
...
static void
trace_id(nvlist_t *entry, char *fromto, idmap_identity_t *id)
{
	trace_int(entry, fromto, IDMAP_TRACE_TYPE, (int64_t)id->type);
	if (IS_ID_SID(*id)) {
		if (id->name != NULL) {
			trace_sprintf(entry, fromto, IDMAP_TRACE_NAME,
			    "%s%s%s", id->name,
			    id->domain == NULL ? "" : "@",
			    id->domain == NULL ? "" : id->domain);
		}
		if (id->u.sid.prefix != NULL) {
			trace_sprintf(entry, fromto, IDMAP_TRACE_SID,
			    "%s-%u", id->u.sid.prefix, id->u.sid.rid);
		}
	} else if (IS_ID_POSIX(*id)) {
		if (id->name != NULL)
			trace_str(entry, fromto, IDMAP_TRACE_NAME, id->name);
		if (id->u.pid != IDMAP_SENTINEL_PID) {
 ...
 */
int
trace(idmap_request_t *rq, char *fmt, ...)
{
	va_list va;
	int err;
	nvlist_t *entry;
	...
	trace_id(entry, "from", &rq->from);
	trace_id(entry, "to", &rq->to);

	if (IDMAP_ERROR(rq->retcode)) {
		trace_int(entry, IDMAP_TRACE_ERROR, NULL,
		    (int64_t)rq->retcode);
	}

	va_start(va, fmt);
	trace_vsprintf(entry, IDMAP_TRACE_MESSAGE, NULL, fmt, va);
	va_end(va);
	...
	if (rq->do_trace) {
		...
		if (rq->trace == NULL) {
			err = nvlist_alloc(&rq->trace, 0, 0);
			if (err != 0) {
				rq->trace = NULL; /* just in case */
				(void) fprintf(stderr,
		...
		(void) nvlist_add_nvlist(rq->trace, "", entry);

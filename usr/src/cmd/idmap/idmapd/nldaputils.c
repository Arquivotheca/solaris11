...
/* ARGSUSED */
idmap_retcode
nldap_lookup_batch(lookup_state_t *state)
{
...
out:
	for (i = 0; i < state->nreq; i++) {
		...
		if (rq->retcode == IDMAP_SUCCESS &&
		    ...
		    (IS_ID_UID(rq->from) || IS_ID_GID(rq->from))) {

			rc1 = lookup_name2sid(state,
			    rq->to.name, rq->to.domain, IDMAP_SID,
			    NULL, NULL,
			    &rq->to.u.sid.prefix,
			    &rq->to.u.sid.rid,
			    &rq->to.type,
		 ...
		 */
		if (rq->retcode != IDMAP_SUCCESS &&
		    rq->retcode != IDMAP_ERR_NS_LDAP_BAD_WINNAME &&
		    !(IDMAP_FATAL_ERROR(rq->retcode))) {
			idmap_how_clear(&rq->how);
			rq->retcode = IDMAP_SUCCESS;
		}
	}

...
/*
 * This file is included in multiple utilties, and the set of routines
 * used it not uniform.  Therefore we mark it with LINTLIBRARY to allow
 * more effective linting.
 */
/*LINTLIBRARY*/
...
read_ttydefs(const char *id, int check)
{
	...
	char oldc;
	static char d_id[MAXID+1], d_nextid[MAXID+1], d_autobaud[MAXID+1];
	static char d_if[BUFSIZ], d_ff[BUFSIZ];
	static char *states[] = {
	    "", "tty label", "Initial flags",
	    "Final flags", "Autobaud", "Next label"
	};
...
dump_ttydefs(void)
{
	...
	struct	Gdef	*gptr;
	gptr = &Gdef[0];
	debug("********** dumping ttydefs table **********");
	debug("Ndefs = %d", Ndefs);
	debug(" ");
	for (i = 0; i < Ndefs; i++, gptr++) {
		debug("----------------------------------------");
		debug("ttylabel:\t%s", gptr->g_id);
		debug("initial flags:\t%s", gptr->g_iflags);
		debug("final flags:\t%s", gptr->g_fflags);
		if (gptr->g_autobaud & A_FLAG)
			debug("autobaud:\tyes");
		else
			debug("Autobaud:\tno");
		debug("nextlabel:\t%s", gptr->g_nextid);
		debug(" ");
	}
	debug("********** end dumping ttydefs table **********");
}

...
/*
 * This file is borrowed out of the uucp source code.  We pass the LINTLIBRARY
 * directive here because not all of the function in this file are used by
 * ttymon and friends.  This lets us lint more effectively the rest of the
 * code.
 */
/*LINTLIBRARY*/
...
mklock(char *name)
{
	...
	if (pid[0] == '\0') {
		...
		(void) snprintf(tempfile, sizeof (tempfile),
		    "%s/LTMP.%ld", X_LOCKDIR, (long)getpid());
	}
...
delock(char *pre, char *s)
{
	...
	(void) snprintf(ln, sizeof (ln), "%s.%s", pre, s);

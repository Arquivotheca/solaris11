...
const char *requestfile = REQUESTFILE;	/* default lookup file */

void dumprequest(const char *, const char *, FILE *);
 ...
 */
	...
	if ( (request[nextreq].file = strtok(NULL, ": ")) == NULL )
	    request[nextreq].file = (char *)requestfile;
	nextreq++;
...
void
dumprequest(const char *want, const char *file, FILE *fp_out)

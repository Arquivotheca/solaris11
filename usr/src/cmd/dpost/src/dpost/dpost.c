 ...
 *	    the default state is set up. An 'x res' command must precede the
 *	    'x init' command!
 *
 *	pagesetup
 *
 ...
 *	    preceded by a number that's expected to be added to the width of a
...
const char	*prologue = DPOST;	/* the basic PostScript prologue */
const char	*colorfile = COLOR;	/* things needed for color support */
const char	*drawfile = DRAW;	/* and drawing */
const char	*formfile = FORMFILE;	/* stuff for multiple pages per sheet */
const char	*baselinefile = BASELINE;

const char	*fontdir = FONTDIR;	/* binary device directories found here */
...
const char	*realdev = DEVNAME;	/* a good description of target printer */
...
static void loadfont(int, const char *, const char *);
...
extern void	beginpath(char *, int);
extern void	drawarc(int, int, int, int, int);
extern void	drawcirc(int);
extern void	drawellip(int, int);
extern void	drawline(int, int);
extern void	drawpath(char *, int);
extern void	drawspline(FILE *, int);
extern void	drawtext(char *);
extern void	getdraw(void);
extern void	inlinepic(FILE *, char *);
extern void	newbaseline(char *);
extern void	newcolor(char *);
extern void	picture(char *);
...
static void
loadfont(int n, const char *s, const char *s1)
 ...
 */
    ...
    fprintf(tf,
	"base=0x%p, nchars=%d, spec=%d, name=%s, widtab=0x%p, fitab=0x%p\n",
	(void *)p, n, fontbase[i]->specfont, fontbase[i]->namefont,
	(void *)widthtab[i], (void *)fitab[i]);

...
install(char *from, char *to)
{
	...
	devnull = (strcmp(from, "/dev/null") == 0);
	...
	if (!stat(path = to, &to_sb)) {
		if ((to_sb.st_mode&S_IFMT) == S_IFDIR) {
			...
			(void) sprintf(path = pbuf, "%s/%s", to,
			    (C = strrchr(from, '/')) ? ++C : from);
			if (stat(path, &to_sb))
				goto nocompare;
		}
...
nocompare:
	...
	if ((to_fd = open(path, O_CREAT|O_WRONLY|O_TRUNC, 0600)) < 0) {
		fprintf(stderr, gettext("install: %s: %s\n"),
		    path, strerror(errno));
		return (1);
	}
 ...
 */
int
copy(char *from_name, int to_fd, char *to_name)
{
	int n, from_fd;
	int status = 0;
	char buf[MAXBSIZE];
...
usage()
{
	fputs(gettext("usage: install [-cs] [-g group] [-m mode] "
	    "[-o owner] file ...  destination\n"), stderr);
	fputs(gettext("       install  -d   [-g group] [-m mode] "
	    "[-o owner] dir\n"), stderr);
	exit(1);
}

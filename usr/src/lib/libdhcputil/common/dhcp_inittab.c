...
/*
 * get_dhcpopt_code(): converts a string representation of a given DHCP option
 *		     into the DHCP option code. Returns -1 if the given DHCP
 *		     option string wasn't found in dhcp_inittab(5).
 *   input: const char *: the DHCP option string to convert
 *  output: int: the DHCP option code on success and -1 on failure.
 */

static int
get_dhcpopt_code(const char *option)
{
	dhcp_symbol_t	*dhcpsym;
	int optcode;

	dhcpsym = inittab_getbyname(ITAB_CAT_STANDARD, ITAB_CONS_INFO, option);
	if (dhcpsym == NULL)
		return (-1);

	optcode = dhcpsym->ds_code;
	free(dhcpsym);
	return (optcode);
}

/*
 * validate_servervalue(): validate the DHCP option value received from the
 *		     DHCP server. Only checks hostname related options.
 *   input: ushort_t: the DHCP option code of the value to be checked.
 *	    const char *: the DHCP option value to check.
 *  output: boolean_t: B_TRUE on success and B_FALSE on failure.
 */

static boolean_t
validate_servervalue(ushort_t optcode, const char *result)
{
	boolean_t valid = B_TRUE;

	/* Validate any hostname or domain name option value */
	if (optcode == get_dhcpopt_code("Hostname") ||
	    optcode == get_dhcpopt_code("DNSdmain") ||
	    optcode == get_dhcpopt_code("TFTPsrvN"))
		valid = valid_hostname(result);
    ...
    uint16_t length, boolean_t just_payload, int *ierrnop)
{
	...
	*ierrnop = 0;
	...
	case DSYM_ASCII:
		...
		if (!validate_servervalue(ie->ds_code, result)) {
			*ierrnop = ITAB_BAD_STRING;
			inittab_msg("inittab_decode: invalid option value %s "
			    "for code %d from server", result, ie->ds_code);
			free(result);
			return (NULL);
		}
		break;

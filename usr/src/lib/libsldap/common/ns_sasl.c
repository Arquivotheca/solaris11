 ...
 *
 * Defining SOFTTOKEN_DIR=/dev/null stops the crypto module pkcs11_softtoken
 * calling naming services.
 *
 * Defining KMSTOKEN_DIR=/dev/null stops the crypto module pkcs11_kms calling
 * naming services.
 *
 * Defining PKCS11_TPM_DIR=/dev/null stops the crypto module pkcs11_tpm calling
 * naming services. Crypto module pkcs11_tpm also calls library libtspi.so.1.
 *
 * Defining TSS_USER_PS_FILE=/dev/null stops the library libtspi.so.1 from
 * calling naming services.
 */
int
__s_api_sasl_gssapi_init(void)
{
	...
	if (!sasl_gssapi_inited) {
		if (setenv("SOFTTOKEN_DIR", "/dev/null", 1) != 0) {
			syslog(LOG_ERR, "libsldap: %s",
			    gettext("Can't setenv SOFTTOKEN_DIR to /dev/null "
			    "for self credential mode"));
			rc = NS_LDAP_INTERNAL;
			goto exit;
		}
		if (setenv("KMSTOKEN_DIR", "/dev/null", 1) != 0) {
			syslog(LOG_ERR, "libsldap: %s",
			    gettext("Can't setenv KMSTOKEN_DIR to /dev/null "
			    "for self credential mode"));
			rc = NS_LDAP_INTERNAL;
			goto exit;
		}
		if (setenv("PKCS11_TPM_DIR", "/dev/null", 1) != 0) {
			syslog(LOG_ERR, "libsldap: %s",
			    gettext("Can't setenv "
			    "PKCS11_TPM_TOKEN_DIR to /dev/null "
			    "for self credential mode"));
			rc = NS_LDAP_INTERNAL;
			goto exit;
		}
		if (setenv("TSS_USER_PS_FILE", "/dev/null", 1) != 0) {
			syslog(LOG_ERR, "libsldap: %s",
			    gettext("Can't setenv TSS_USER_PS_FILE to "
			    "/dev/null for self credential mode"));
			rc = NS_LDAP_INTERNAL;
			goto exit;
		}
		syslog(LOG_WARNING, "libsldap: %s",
		...
		ns_ldap_error_t **errpp)
{
	...
	if ((rc = __ns_ldap_check_dns_preq(foreground, mode_verbose,
	    mode_quiet, NSSWITCH_CONF, config, errpp)) != NS_LDAP_SUCCESS)
		return (rc);
	if ((rc = __ns_ldap_check_gssapi_preq(foreground, mode_verbose,
	    mode_quiet, config, errpp)) != NS_LDAP_SUCCESS)
		return (rc);

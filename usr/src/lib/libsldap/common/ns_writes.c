...
static int send_to_cachemgr(const char *,
    ns_ldap_attr_t **, ns_ldap_error_t **, int, ldap_service_t);

static int do_cachemgr_operation(const char *, ns_ldap_attr_t **,
    ns_ldap_error_t **, int, ldap_service_t);

static int escape_str(char *, char *);
static int get_admin_passwd(ns_cred_t *, ns_ldap_error_t **);

/*
 * Check to see if the UPDATE_SHADOW flag is set and the
 * service is one of the services that are allowed to be modified
 * with the UPDATE_SHADOW privilege.
 */
static boolean_t
privileged_operation_check(const int flags, const char *service)
{
	return ((flags & NS_LDAP_UPDATE_SHADOW) != 0 &&
	    (strcmp(service, "shadow") == 0 ||
	    strcmp(service, "passwd") == 0 ||
	    strcmp(service, "user_attr") == 0 ||
	    strcmp(service, "auth_attr") == 0 ||
	    strcmp(service, "exec_attr") == 0 ||
	    strcmp(service, "prof_attr") == 0 ||
	    strcmp(service, "project") == 0 ||
	    strcmp(service, "group") == 0 ||
	    strncmp(service, NS_LDAP_TYPE_AUTOMOUNT,
	    sizeof (NS_LDAP_TYPE_AUTOMOUNT) - 1) == 0 ||
	    strcmp(service, "tnrhdb") == 0 ||
	    strcmp(service, "tnrhtp") == 0));
}

static ldap_service_t
map_service_to_type(const char *service)
{
	ldap_service_t svc;
	...
	if (strcmp(service, "shadow") == 0)
		...
		svc = LDAP_SVC_PASSWD;
	else if (strcmp(service, "user_attr") == 0)
		svc = LDAP_SVC_USERATTR;
	else if (strcmp(service, "auth_attr") == 0)
		svc = LDAP_SVC_AUTHATTR;
	else if (strcmp(service, "exec_attr") == 0)
		svc = LDAP_SVC_EXECATTR;
	else if (strcmp(service, "prof_attr") == 0)
		svc = LDAP_SVC_PROFATTR;
	else if (strcmp(service, "project") == 0)
		svc = LDAP_SVC_PROJECT;
...
static int
do_cachemgr_operation(const char *dn, ns_ldap_attr_t **attr,
	ns_ldap_error_t ** errorp, int opcode, ldap_service_t svc)
{
	boolean_t	priv;
	...
	/* privileged modify requires euid 0 or all zone privs */
	priv = (geteuid() == 0);
	if (!priv) {
	...
	return (send_to_cachemgr(dn, attr, errorp, opcode, svc));
}
	...
	ns_ldap_error_t ** errorp)
{
	...
	*errorp = NULL;
	...
	/* Privileged modify? */
	if (privileged_operation_check(flags, service)) {
		rc = do_cachemgr_operation(dn, (ns_ldap_attr_t **)attr, errorp,
		    ADMINMODIFY_ADD, map_service_to_type(service));
		return (rc);
	}
	...
	ns_ldap_error_t ** errorp)
{
	...
	*errorp = NULL;
	...
	if (privileged_operation_check(flags, service)) {
		rc = do_cachemgr_operation(dn, (ns_ldap_attr_t **)attr, errorp,
		    ADMINMODIFY_DEL, map_service_to_type(service));
		return (rc);
	}
 ...
 * enabled', RBAC related entries can only be modified by privileged users.
 * Such users use the NS_LDAP_UPDATE_SHADOW flag to indicate the call is
 * for such an update, which would be forwarded to ldap_cachemgr
	...
	ns_ldap_error_t ** errorp)
{
	...
	*errorp = NULL;
	...
	if (privileged_operation_check(flags, service)) {
		rc = do_cachemgr_operation(dn, (ns_ldap_attr_t **)attr, errorp,
		    ADMINMODIFY_REP, map_service_to_type(service));
		return (rc);
	}
	...
	ns_ldap_error_t ** errorp)
{
	...
	*errorp = NULL;
	...
	/* Privileged modify? */
	if (privileged_operation_check(flags, service)) {
		rc = do_cachemgr_operation(dn, (ns_ldap_attr_t **)attr,
		    errorp, ADMIN_ADDENTRY, map_service_to_type(service));
		return (rc);
	}
	...
	ns_ldap_error_t ** errorp)
{
	...
	*errorp = NULL;
	/* Privileged modify? */
	if (privileged_operation_check(flags, service)) {
		rc = do_cachemgr_operation(dn, NULL, errorp,
		    ADMIN_DELENTRY, map_service_to_type(service));
		return (rc);
	}
	...
	ns_ldap_error_t **errorp)
{
	...
	if (create != 1) {
		...
		if ((__s_cvtlist[s].flags & AE) != 0) {
			if (flags & NS_LDAP_UPDATE_ATTR) {
				rc = __ns_ldap_repAttr(service, fulldn,
 ...
 */
static int
attr2list(const char *dn, int opcode, ns_ldap_attr_t **attr,
    char *buf, int bufsize)
{
	...
	/* count number of attributes */
	while (aptr != NULL && *aptr != NULL) {
		a = *aptr;
		/*
		 * Some attributes have multiple values (ex: objectClass)
		 * For some operations we do not send ObjectClass attributes
		 * because the server will not let it be modified.
		 */
		if (!(opcode == ADMINMODIFY_REP &&
		    strcasecmp(a->attrname, "objectclass") == 0))
			c += a->value_count;
		aptr++;
	}
	al->ldap_count = 2 + c * 2;
	...
	aptr = attr;
	for (c = 2; c < al->ldap_count && aptr && *aptr != NULL; aptr++) {
		int i;
		...
		/*
		 * Don't pass objectClass attributes when modifying
		 * an existing entry.
		 */
		if (opcode == ADMINMODIFY_REP &&
		    strcasecmp(a->attrname, "objectclass") == 0)
		...
		/*
		 * For each value, write the attrname + attrvalue
		 * so they can be parsed later by the cachemgr.
		 */
		for (i = 0; i < a->value_count; i++) {
			...
			al->ldap_offsets[c] = ao;
			c++;
			ao += strlen(a->attrvalue[i]) + 1;
			(void) strlcpy(ap, a->attrvalue[i], bufsize);
			ap = buf + ao;
		}
	...
	ldap_service_t svc)
{
	...
	*errorp = NULL;
	...
	len = attr2list(dn, opcode, attr,
	    (char *)&space.s_d.ldap_call.ldap_u.ldap_admin.strlist,
	...
	ndata = sizeof (space);
	space.s_d.ldap_call.ldap_callnumber = opcode;
	space.s_d.ldap_call.ldap_u.ldap_admin.ldap_service = svc;
	sptr = &space.s_d;
	...
	case NS_CACHE_NOTFOUND:
		(void) snprintf(errstr, sizeof (errstr),
		    gettext("Door call %d to "
		    "ldap_cachemgr failed - error: %d"),
		    opcode, space.s_d.ldap_ret.ldap_errno);

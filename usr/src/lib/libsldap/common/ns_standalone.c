...
getDirBaseDN(LDAP *ld, const char *domain_name, char **dir_base_dn)
{
	...
	do {
		...
		if ((entry = ldap_first_entry(ld, resultMsg)) == NULL) {
			(void) ldap_msgfree(resultMsg);
			resultMsg = NULL;
			continue;
		}

		if ((a = ldap_get_dn(ld, entry)) != NULL) {
...
set_ldap_server_type(char *root_dse)
{
	char	*rs, *attr, *val, *rest;
	ns_ldap_server_type_t	type = NS_LDAP_SERVERTYPE_UNKNOWN;
	...
	for (attr = strtok_r(rs, DOORLINESEP, &rest);
	    attr != NULL;
	    attr = strtok_r(NULL, DOORLINESEP, &rest)) {
		if ((val = strchr(attr, '=')) == NULL) {
			continue;
		}
		*val++ = '\0';

		if (strcasecmp(attr, _VENDORNAME) == 0) {
			if (strcasecmp(val, _VENDORSUN) == 0) {
				type = NS_LDAP_SERVERTYPE_ODSEE;
				break;
			}
		}

		if (strcasecmp(attr, _ISGLOBALCATALOGREADY) == 0) {
			type = NS_LDAP_SERVERTYPE_AD;
			break;
		}

		if (strcasecmp(attr, "objectclass") == 0) {
			if (strcasecmp(val, _OPENLDAPROOTDSE) == 0) {
				type = NS_LDAP_SERVERTYPE_OPENLDAP;
				break;
			}
			continue;
		}
 ...
 * on whether a conneciton would/would not be successful.  That data
...
__ns_ldap_getRootDSE(const char *server_addr,
		ns_ldap_server_type_t *server_type,
		char **root_dse,
		ns_ldap_error_t **errorp,
		int anon_fallback)
{
	...
	if (server_addr == NULL || root_dse == NULL || errorp == NULL)
		return (NS_LDAP_INVALID_PARAM);
	...
	*errorp = NULL;
	 ...
	 */
	ret_code = __s_api_getConnection(server_addr, NS_LDAP_NEW_CONN,
	    NULL, &sessionId, &session, errorp, 0, 0, NULL);
	if (ret_code != NS_LDAP_SUCCESS) {
		...
		*errorp = NULL;
		...
		ret_code = __s_api_getConnection(server_addr, NS_LDAP_NEW_CONN,
		    &anon, &sessionId, &session, errorp, 0, 0, NULL);
		if (ret_code != NS_LDAP_SUCCESS) {
			if (*errorp != NULL)
				(void) __ns_ldap_freeError(errorp);
			*errorp = NULL;
	...
	rc = __ns_ldap_getParam(NS_LDAP_SEARCH_TIME_P, &paramVal, errorp);
	...
	ldap_rc = ldap_search_ext_s(session->ld, "", LDAP_SCOPE_BASE,
	    "(objectclass=*)", attrs, 0, NULL, NULL, &tv, 0, &resultMsg);
	...
	if (ldap_rc != LDAP_SUCCESS) {
		...
		MKERROR(LOG_ERR, *errorp, NS_LDAP_OP_FAILED,
		    strdup(errmsg), NS_LDAP_MEMORY);
	 ...
	 */
	ret_code = convert_to_door_line(session->ld, resultMsg,
	    INCLUDE_ATTR_NAMES, NOT_PROFILE, root_dse);
	if (ret_code == NS_LDAP_NOTFOUND) {
		(void) snprintf(errmsg, sizeof (errmsg),
		    gettext("No root DSE data for server %s returned."),
		    server_addr);
		MKERROR(LOG_ERR, *errorp, NS_LDAP_NOTFOUND,
		    strdup(errmsg), NS_LDAP_MEMORY);
	}

	*server_type = set_ldap_server_type(*root_dse);
...
#define	CHUNK_SIZE 16
	...
	*retCode = __ns_ldap_getRootDSE(server->ip,
	...
	if (server->controls == NULL || server->saslMech == NULL) {
		free(rootDSE);
		free(retCode);
		if (server->controls != NULL)
			free(server->controls);
		if (server->saslMech != NULL)
			free(server->saslMech);
		return (NULL);
	}
		...
		ns_ldap_error_t	**error)
{
	...
	ret->serverType = server->serverType;

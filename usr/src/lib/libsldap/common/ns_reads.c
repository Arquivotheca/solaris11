...
/*
 * The following are from libldap. Used to check if an attribute value
 * should be base64-encoded when displayed by tools such as ldaplist.
 */
...
static char *__s_api_remove_rdn_space(char *rdn);
...
/*
 * If requested, convert attribute type in a RDN that has an attribute
 * mapping to the original mappped type. Either way, the DN will be
 * normalized (i.e., remove unnecessary spaces).
 *
 * For example:
 *
 * e.g.
 ...
 *         rdn: RDN
 *         map_attr: if TRUE, map attribute
 ...
 */
static char *
_cvtRDN(const char *service, const char *rdn, boolean_t map_attr)
{
	char	**attrs, **mapped_attrs, *type, *value, *attr;
	char	**mapp = NULL;
	char	*new_rdn = NULL;
	int	nAttr = 0, i, attr_mapped, len = 0;

	/* Break down "type = value\0" pairs. */
	if ((attrs = ldap_explode_rdn(rdn, 0)) == NULL)
		return (NULL);

	for (nAttr = 0; attrs[nAttr] != NULL; nAttr++) {
		/* remove leading/trailing and those spaces around '=' */
		attrs[nAttr] = __s_api_remove_rdn_space(attrs[nAttr]);
	}
	...
	for (i = 0; i < nAttr; i++) {
		...
		if (map_attr && (mapp =
		    __ns_ldap_getOrigAttribute(service, type)) != NULL &&
		    mapp[0] != NULL) {
...
/*
 * If requested, convert attribute type in a DN that has an attribute
 * mapping to the original mappped type. Either way, the DN will be
 * normalized (i.e., remove unnecessary spaces).
 *
 * For example:
 ...
 */
static char *
_cvtDN(const char *service, const char *dn, boolean_t map_attr) {
	char	**mapped_rdns;
	char	**rdns, *new_rdn, *new_dn = NULL;
	int	nRdn = 0, i, len = 0;
	boolean_t rdn_mapped;

	if (dn == NULL)
		return (NULL);

	/* Attribute mapping requires 'service' */
	if (map_attr) {
		if (service == NULL)
			return (NULL);
	} else {
		/* attribute mapping not needed and no space in DN, done */
		if (strchr(dn, SPACETOK) == NULL)
			return (strdup(dn));
	}
	...
	for (i = 0; i < nRdn; i++) {
		if ((new_rdn = _cvtRDN(service, rdns[i], map_attr)) != NULL) {
...
/*
 * Checks which ones of the internal operational attributes
 * (cookie->i_extra_info_attr) are requested and returns
 * the values. Currently only NS_LDAP_OP_ATTR_SERVER_TYPE
 * is supported.
 */
void
set_extra_info(ns_ldap_cookie_t *cookie)
{
	const char * const * iattr;
	char		*val;
	int		i;
	int		attr_cnt = 0;
	ns_ldap_entry_t	*ep;
	ns_ldap_attr_t	**apair;

	iattr = cookie->i_extra_info_attr;
	...
	apair = (ns_ldap_attr_t **)calloc(attr_cnt, sizeof (ns_ldap_attr_t *));
	if (apair == NULL) {
		free(ep);
		return;
	}
	...
	i = 0;
	for (iattr = cookie->i_extra_info_attr; *iattr != NULL; iattr++) {
		if (strcmp(*iattr, NS_LDAP_OP_ATTR_SERVER_TYPE) == 0) {
			if (cookie->conn == NULL)
				continue;
			switch (cookie->conn->serverType) {
				case NS_LDAP_SERVERTYPE_ODSEE:
					val = NS_LDAP_ATTR_VAL_SERVER_ODSEE;
					break;
				case NS_LDAP_SERVERTYPE_AD:
					val = NS_LDAP_ATTR_VAL_SERVER_AD;
					break;
				case NS_LDAP_SERVERTYPE_OPENLDAP:
					val = NS_LDAP_ATTR_VAL_SERVER_OPENLDAP;
					break;
				case NS_LDAP_SERVERTYPE_OID:
					val = NS_LDAP_ATTR_VAL_SERVER_OID;
					break;
				default:
					val = NS_LDAP_ATTR_VAL_SERVER_UNKNOWN;
					break;
			}
			apair[i] = (ns_ldap_attr_t *)calloc(1,
			...
			apair[i]->attrvalue = (char **)calloc(2,
			    sizeof (char *));
			if (apair[i]->attrvalue == NULL) {
				__ns_ldap_freeEntry(ep);
				return;
			}
			apair[i]->attrvalue[0] = strdup(val);
			if (apair[i]->attrvalue[0] == NULL) {
				__ns_ldap_freeEntry(ep);
				return;
			}
			apair[i]->attrname =
			    strdup(NS_LDAP_OP_ATTR_SERVER_TYPE);
			if (apair[i]->attrname == NULL) {
				__ns_ldap_freeEntry(ep);
				return;
			}
			apair[i]->value_count = 1;
	...
	cookie->extra_info = ep;
}

/*
 * Free the data structure used to track the processing of attributes
 * with range option, such as member.
 */
static void
free_range_info(ns_ldap_range_info_t **info)
{
	ns_ldap_range_attr_t	*attr, *next;

	if ((*info)->dn != NULL)
		free((*info)->dn);
	if ((*info)->attr_to_search != NULL)
		free((*info)->attr_to_search);

	for (attr = (*info)->range_attr; attr != NULL; attr = next) {
		if (attr->mapped_name != NULL)
			free(attr->mapped_name);
		if (attr->attr_and_range != NULL)
			free(attr->attr_and_range);
		next = attr->next;
		free(attr);
	}
	free(*info);
...
/*
 * Create/add/update the necessary info for the search state
 * machine to collect the next range of the attribute values.
 *
 * 'first' indicates if the attribute range option has
 * just be detected and if the control structure should
 * be created and added to the range_attr list in the
 * range info control structure, *info_p. If it's not
 * TRUE, the control structure will be updated to have
 * the attribute's range value for the next search. Either
 * way, 'attr_type' should always contain the attribute
 * name and the range option specification. The updated
 * attr_and_range string will also have the attribute
 * name and range info together, so that it can be
 * readily used in the attribute list for the next
 * ldap search.
 *
 * See the comments in ns_internal.h for info about
 * ns_ldap_range_attr_t and ns_ldap_range_info_t
 * when used for processing attributes with range
 * options.
 */
static int
update_range_info(boolean_t first, char *attr_type,
    ns_ldap_range_info_t **info_p)
{
	ns_ldap_range_attr_t	*attr;
	ns_ldap_range_info_t	*info;
	int	attr_len;
	long	next_range_start;
	char	*endptr;
	char	*attr_end;
	char	*last_entry_s;
	char	attr_and_range[NS_LDAP_ATTR_AND_RANGE_SIZE];
	boolean_t range_done = B_FALSE;

	/*
	 * Attribute type with range option has the form:
	 * <attr type>;range=n1-n2, where n2 may be '*' (last)
	 */
	attr_end = strstr(attr_type, ";range=");
	attr_len = attr_end - attr_type;
	/* get end of range */
	last_entry_s = strchr(attr_end + 7, '-');
	if (last_entry_s == NULL) {
		/* ignore the range setting */
		return (NS_LDAP_SUCCESS);
	}

	/*
	 * End of range reached and first search result ?
	 * No more to be done.
	 */
	last_entry_s++;
	if (*last_entry_s == '*') {
		...
		next_range_start = strtol(last_entry_s, &endptr, 10);
		if (next_range_start == 0 || (*endptr != ';' &&
		    *endptr != '\0'))
			/* ignore the range setting */
			return (NS_LDAP_SUCCESS);
		/*
		 * generate the "attribute type + option" for the next search:
		 * <attr type>;range=start-end
		 */
		(void) snprintf(attr_and_range, sizeof (attr_and_range),
		    "%.*s;range=%ld-*", attr_len, attr_type,
		    next_range_start + 1);
	}

	info = *info_p;
	if (info == NULL) {
		*info_p = info = (ns_ldap_range_info_t *)calloc(1,
		    sizeof (ns_ldap_range_info_t));
		if (info == NULL)
			return (NS_LDAP_MEMORY);
	}

	if (first) {
		attr = (ns_ldap_range_attr_t *)calloc(1,
		    sizeof (ns_ldap_range_attr_t));
		if (attr == NULL) {
			free_range_info(info_p);
			return (NS_LDAP_MEMORY);
		}
		attr->attr_len = attr_len;
		attr->attr_and_range = malloc(NS_LDAP_ATTR_AND_RANGE_SIZE);
		if (attr->attr_and_range == NULL) {
			free_range_info(info_p);
			free(attr);
			return (NS_LDAP_MEMORY);
		...
		attr->next = info->range_attr;
		info->range_attr = attr;
	} else {
		for (attr = info->range_attr; attr != NULL; attr = attr->next) {
			if (strncasecmp(attr_type, attr->attr_and_range,
			    attr->attr_len + 1) == 0)
				break;
		}
		if (attr == NULL)
			/* ignore this attribute type */
			return (NS_LDAP_SUCCESS);

		if (range_done) {
			attr->range_done = B_TRUE;
			return (NS_LDAP_SUCCESS);
		}
	}

	(void) strcpy(attr->attr_and_range, attr_and_range);
...
/*
 * Take a binary value and return the base64 encoded string.
 * Tag the output string with NS_LDAP_BASE64_TAG, "\1{base64}".
 */
static char *
b64_encode(char *value, size_t vlen)
{
	char	*b64s;
	size_t	mvlen;

	/* Encode 'value', output looks like "\1{base64}:: <encoded string>" */
	b64s = ldif_type_and_value_with_options(NS_LDAP_BASE64_TAG, value,
	    vlen, LDIF_OPT_NOWRAP);
	if (b64s == NULL)
		return (NULL);
	mvlen = strlen(b64s) - NS_LDAP_BASE64_TAG_LDIF_LEN;

	/*
	 * change "\1{base64}:: <encoded string>" into
	 * "\1{base64}<encoded string>
	 */
	(void) memmove(b64s + NS_LDAP_BASE64_TAG_LEN,
	    b64s + NS_LDAP_BASE64_TAG_LDIF_LEN, mvlen);
	/* remove the newline character at the end */
	b64s[NS_LDAP_BASE64_TAG_LEN + mvlen - 1] = '\0';
...
__s_api_cvtEntry(LDAP	*ld,
	...
	ns_ldap_range_info_t **range_info_p)
{
	...
	boolean_t	schema_mapping_existed = B_FALSE;
	boolean_t	gecos_mapping_existed = B_FALSE;
	boolean_t	gecos_attr_matched;
	boolean_t	auto_service = B_FALSE;
	boolean_t	group_service = B_FALSE;
	boolean_t	passwd_service = B_FALSE;
	boolean_t	cvtdn = B_FALSE;
	boolean_t	first_range = B_TRUE;
	boolean_t	proc_range = B_FALSE;
	char		attr_and_range[NS_LDAP_ATTR_AND_RANGE_SIZE];
	char		*attr_to_get;
	...
	*error = NULL;
	...
	if (service != NULL) {
		if (strncasecmp(service, "auto_", 5) == 0 ||
			...
			auto_service = B_TRUE;
		else if (strcasecmp(service, "group") == 0)
			group_service = B_TRUE;
		else if (strcasecmp(service, "passwd") == 0)
			passwd_service = B_TRUE;
	}
	...
	nAttrs = 1;  /* start with 1, 0 is for the DN attr */
	for (attr = ldap_first_attribute(ld, e, &ber); attr != NULL;
	    attr = ldap_next_attribute(ld, e, ber)) {
		nAttrs++;
		if (strstr(attr, ";range=") != NULL) {
			rc = update_range_info(first_range, attr,
			    range_info_p);
	...
	if (!proc_range && *range_info_p != NULL)
		proc_range = B_TRUE;
	...
	if (ap[0] == NULL)
		goto free_dn_and_return;
	ap[0]->attrname = strdup("dn");
	if (ap[0]->attrname == NULL)
		goto free_dn_and_return;
	if (proc_range && first_range) {
		(*range_info_p)->dn = strdup(dn);
		if ((*range_info_p)->dn == NULL)
			goto free_dn_and_return;
	}
	...
	/* attribute-map and normalize the entry DN if needed */
	if ((flags & NS_LDAP_NOT_CVT_DN) == 0) {
		if (schema_mapping_existed)
			ap[0]->attrvalue[0] = _cvtDN(service, dn, B_TRUE);
	} else {
		/*
		 * To speed up group member DN processing, we need to
		 * normalize the entry DN of a passwd or group entry.
		 * Such DNs will later be stored in cache if needed.
		 */
		if (group_service || passwd_service)
			ap[0]->attrvalue[0] = _cvtDN(service, dn, B_FALSE);
	}
	...
	for (attr = ldap_first_attribute(ld, e, &ber), j = 1;
		...
		/*
		 * Range result ? Not all values returned ? Then
		 * attr should looks like: <attr type>;range=n1-n2
		 * where n2 may be '*', i.e., end of range.
		 */
		range_attr = NULL;
		if (proc_range) {
			ns_ldap_range_attr_t	*ra;
			char *cp;

			ra = (*range_info_p)->range_attr;
			while (ra != NULL) {
				if (strncasecmp(attr, ra->attr_and_range,
				    ra->attr_len + 1) == 0) {
					range_attr = ra;
					/*
					 * For now, ra->received_ranges will
					 * have the first range of values. For
					 * subsequent range searches, values
					 * found in ra->current_range will be
					 * appended to it in __s_api_getEntry.
					 * So it would represent the
					 * accumulation of all of the ranges
					 * received so far.
					 */
					if (first_range)
						ra->received_ranges = ap[j];
					else
						ra->current_range = ap[j];

					/* copy attribute type + range */
					(void) strlcpy(attr_and_range, attr,
					    NS_LDAP_ATTR_AND_RANGE_SIZE);
					attr_to_get = attr_and_range;
					/* no need for the range part now */
					cp = strchr(attr, ';');
					*cp = '\0';
				}
				ra = ra->next;
		...
		if ((flags & NS_LDAP_NOMAP) || !schema_mapping_existed)
			mapping = NULL;
		else
		...
		if (mapping == NULL) {
			...
			if (range_attr != NULL) {
				range_attr->mapped_name = strdup(mapping[0]);
				if (range_attr->mapped_name == NULL) {
					free(ap[j]->attrname);
					ap[j]->attrname = NULL;
					rc = NS_LDAP_MEMORY;
					goto err_out;
			...
			if (strcasecmp(mapping[0], "gecos") == 0) {
				 ...
				 */
				gecos_attr_matched = B_FALSE;
				for (i = 0; i < 3 && gecos_mapping[i]; i++) {
					if (gecos_mapping[i] &&
					    strcasecmp(gecos_mapping[i],
					    attr) == 0) {
						gecos_val_index[i] = j;
						gecos_attr_matched = B_TRUE;
						break;
					}
				}
				if (!gecos_attr_matched) {
					/*
					 * Not match found.
					 * This should never happens,
					 * syslog the error
		...
		/*
		 * Need to process group member or memberof in DN format ?
		 */
		cvtdn = B_FALSE;
		if ((group_service &&
		    (strcasecmp(ap[j]->attrname, "member") == 0 ||
		    strcasecmp(ap[j]->attrname, "uniquemember") == 0 ||
		    strcasecmp(ap[j]->attrname, "memberof") == 0)) ||
		    (passwd_service &&
		    strcasecmp(ap[j]->attrname, "memberof") == 0)) {
			cvtdn = B_TRUE;
		}

		if ((bvals = ldap_get_values_len(ld, e, attr_to_get)) == NULL) {
			ldap_memfree(attr);
			attr = NULL;
			continue;
		}

		if ((ap[j]->value_count = ldap_count_values_len(bvals)) == 0) {
		...
		for (i = 0; i < ap[j]->value_count; i++) {
			boolean_t binary_data = B_FALSE;
			/* check for binary data if length not zero */
			if ((flags & NS_LDAP_BASE64_BINARY_ATTR_VALUE) != 0) {
				cp = bvals[i]->bv_val;
				for (k = 1; k <= bvals[i]->bv_len;
				    k++, cp++) {
					if (!LDIF_CONSERVATIVE_CHAR(*cp)) {
						binary_data = B_TRUE;
						break;
					}
				}
			...
			if (binary_data) {
				...
				ap[j]->attrvalue[i] =
				    b64_encode(bvals[i]->bv_val,
				    bvals[i]->bv_len);
				if (ap[j]->attrvalue[i] == NULL) {
					rc = NS_LDAP_MEMORY;
					goto err_out;
				}
			} else {
				ap[j]->attrvalue[i] = calloc(1,
				    bvals[i]->bv_len + 1);
				if (ap[j]->attrvalue[i] == NULL) {
					rc = NS_LDAP_MEMORY;
					goto err_out;
		...
		if ((flags & NS_LDAP_NOMAP) == 0 &&
			...
			for (k = 0; k < ap[j]->value_count; k++) {
				mapping = __ns_ldap_getOrigObjectClass(
				    service, ap[j]->attrvalue[k]);
				...
				if (mapping == NULL && auto_service)
					...
					mapping =
					    ...
					    "automount", ap[j]->attrvalue[k]);

				if (mapping != NULL) {
					free(ap[j]->attrvalue[k]);
					ap[j]->attrvalue[k] =
					    strdup(mapping[0]);
					__s_api_free2dArray(mapping);
					mapping = NULL;
			...
			/*
			 * Normalize group member or memberof
			 * DN but don't map attributes.
			 */
			if (cvtdn) {
				for (k = 0; k < ap[j]->value_count; k++) {
					char *old, *new;
					old = ap[j]->attrvalue[k];
					new = _cvtDN(service, old, B_FALSE);
					if (new != old) {
						free(old);
						ap[j]->attrvalue[k] = new;
					}
...
/*
 * The ns_ldap_entry_t generated by __s_api_cvtEntry() may contain
 * duplicate attributes for an range attribute. These duplicate
 * ones are caused by libldap when it returns attributes such
 * as "member" and "member;range=0-1499" together. This function
 * keeps the short one and removes the one with option.
 */
static void
remove_dup_range_attrs(ns_ldap_entry_t *entry, ns_ldap_range_info_t *info)
{
	ns_ldap_attr_t		*eAttr;
	ns_ldap_range_attr_t	*attr;
	int			i, j, cnt;
	char			*attrname;

	/*
	 * Remove duplicate empty attributes, such as member or memberof,
	 * in the result ns_ldap_entry_t entry.
	 */
	attr = info->range_attr;
	while (attr != NULL) {
		/*
		 * Attribute name in entry may be mapped to a different name
		 * due to attribute mapping. The one containing range
		 * option info is an unmapped one, so use the mapped
		 * one saved in ns_ldap_range attr_t to find duplicate.
		 */
		if (attr->mapped_name != NULL) {
			attrname = attr->mapped_name;
		} else {
			attrname = attr->attr_and_range;
			/* No need for the range option info now */
			attrname[attr->attr_len] = '\0';
		}

		cnt = entry->attr_count;
		for (i = 0; i < cnt; i++) {
			eAttr = entry->attr_pair[i];
			if (eAttr->value_count != 0)
				continue;
			if (strcasecmp(attrname, eAttr->attrname) == 0) {
				free(eAttr->attrname);
				free(eAttr);
				for (j = i; j < cnt - 1; j++) {
					entry->attr_pair[j] =
				...
				entry->attr_pair[j] = NULL;
				entry->attr_count--;
				break;
			}
		}

		/* restore the range option info if needed */
		if (attr->mapped_name == NULL) {
			attrname[attr->attr_len] = ';';
		}
...
__s_api_getEntry(ns_ldap_cookie_t *cookie)
{
	...
	ret = __s_api_cvtEntry(cookie->conn->ld, cookie->service,
	    ...
	    &curEntry, &cookie->errorp, &cookie->range_info);
	if (ret != NS_LDAP_SUCCESS)
		goto out;

	if (!cookie->get_next_range) {
	...
	if (cookie->i_extra_info_attr != NULL &&
	    cookie->extra_info == NULL)
		set_extra_info(cookie);

	if (cookie->range_info == NULL)
		return (NS_LDAP_SUCCESS);

	info = cookie->range_info;
	/*
	 * If just gotten the first range, remove duplicate attribute types,
	 * i.e., keep "member" and not "member;range=0-1499". Otherwise,
	 * merge this range of values with the previous ones.
	 */
	if (!cookie->get_next_range) {
		cookie->get_next_range = B_TRUE;
		remove_dup_range_attrs(curEntry, info);
		info->attr_to_search = (char **)calloc(info->attr_count + 1,
		    sizeof (char *));
		if (info->attr_to_search == NULL) {
			ret = NS_LDAP_MEMORY;
			goto out;
		...
		uint_t	old_count, new_count;
		char	**tmp, **new_values;
		ns_ldap_range_attr_t	*prev, *next;

		/*
		 * Reallocate the received_ranges value array and add the
		 * current one to the end of it.
		 */
		for (prev = NULL, attr = info->range_attr; attr != NULL;
		    attr = next) {
			old_count = attr->received_ranges->value_count;
			new_count = old_count +
			    attr->current_range->value_count;
			tmp = (char **)realloc(attr->received_ranges->attrvalue,
			    new_count * sizeof (char *));
			if (tmp == NULL) {
				ret = NS_LDAP_MEMORY;
				goto out;
			}
			attr->received_ranges->attrvalue = tmp;
			new_values = tmp + old_count;
			(void) memcpy(new_values,
			    attr->current_range->attrvalue,
			    attr->current_range->value_count * sizeof (char *));
			attr->received_ranges->value_count = new_count;

			/*
			 * Free the current (not first) ns_ldap_entry.
			 * The attribute value array itself needs to be
			 * freed, but not the value strings. Since the
			 * pointers to these strings have just been
			 * copied into the first ns_ldap_entry's value
			 * array.
			 */
			free(attr->current_range->attrvalue);
			attr->current_range->attrvalue = NULL;
			attr->current_range->value_count = 0;
			attr->current_range = NULL;
			__ns_ldap_freeEntry(curEntry);
			curEntry = NULL;

			/* clean up the range control info if last range */
			if (attr->range_done) {
				next = attr->next;
				if (attr->attr_and_range != NULL) {
					free(attr->attr_and_range);
					attr->attr_and_range = NULL;
				}
				if (info->range_attr == attr)
					info->range_attr = next;
				else
					prev->next = next;
				free(attr);
				info->attr_count--;
		...
		/*
		 * Generate only once the extra infomation
		 * (operational attributes) if requested.
		 * Then exit with NS_LDAP_SUCCESS.
		 */
	...
	/*
	 * set up the attributes (with updated range numbers)
	 * for the next search of the same DN
	 */
	attr = info->range_attr;
	for (i = 0; attr != NULL; attr = attr->next, i++) {
		info->attr_to_search[i] = attr->attr_and_range;
	}
	info->attr_to_search[i] = NULL;

	return (NS_LDAP_SUCCESS);
out:
	if (cookie->range_info != NULL)
		free_range_info(&cookie->range_info);
	if (ret != NS_LDAP_SUCCESS && cookie->extra_info != NULL) {
		__ns_ldap_freeEntry(cookie->extra_info);
		cookie->extra_info = NULL;
	}
	if (curEntry != NULL)
		__ns_ldap_freeEntry(curEntry);
...
delete_search_cookie(ns_ldap_cookie_t *cookie)
{
	...
	if (cookie->extra_info)
		(void) __ns_ldap_freeEntry(cookie->extra_info);
	cookie->extra_info = NULL;
	if (cookie->attribute)
...
search_state_machine(ns_ldap_cookie_t *cookie, ns_state_t state, int cycle)
{
	...
	struct timeval	tv;
	char		*basedn_save = NULL;
	int		scope_save = NS_LDAP_SCOPE_ONELEVEL;
	char		*filter_save = NULL;
	char		**attrs_save = NULL;
	int		msgId_save = 0;
	ns_state_t	state_save = NEXT_RESULT;
	LDAPControl	**p_serverctrls_save = NULL;
	...
	for (;;) {
		...
		case NEXT_RANGE:
			/*
			 * Do search if more range of values to get.
			 * Otherwise, restore search related data in
			 * cookie and go to the NEXT_RESULT state to
			 * remove the last LDAP_RES_SEARCH_RESULT
			 * of this range search and have the result
			 * processed.
			 */
			if (cookie->range_info == NULL) {
				cookie->basedn = basedn_save;
				cookie->scope = scope_save;
				cookie->filter = filter_save;
				cookie->attribute = attrs_save;
				cookie->p_serverctrls = p_serverctrls_save;

				basedn_save = NULL;
				filter_save = NULL;
				attrs_save = NULL;
				p_serverctrls_save = NULL;

				cookie->new_state = NEXT_RESULT;
				break;
			}

			/*
			 * Change/save the search related data in cookie to
			 * get the next range of values.
			 */
			if (basedn_save == NULL) {
				basedn_save = cookie->basedn;
				scope_save = cookie->scope;
				filter_save = cookie->filter;
				attrs_save = cookie->attribute;
				msgId_save = cookie->msgId;
				p_serverctrls_save = cookie->p_serverctrls;

				cookie->basedn = cookie->range_info->dn;
				cookie->scope = LDAP_SCOPE_BASE;
				cookie->filter = "(objectclass=*)";
				cookie->msgId = 0;
				cookie->p_serverctrls = NULL;
			}
			cookie->attribute = cookie->range_info->attr_to_search;
		...
		case DO_SEARCH:
			...
			if (rc != LDAP_SUCCESS) {
				if (rc == LDAP_BUSY ||
					...
					} else if (cookie->get_next_range) {
						/*
						 * The search of a range of
						 * attribute values should
						 * not be retried by itself.
						 * It could possibily cause
						 * duplicate results.
						 */
						cookie->new_state =
						    LDAP_ERROR;
			...
			break;
		case NEXT_RANGE_RESULT:
			/*
			 * If all range of attribute values
			 * have been retrieved, go back to
			 * normal result processing. Otherwise,
			 * get the next range.
			 */
			if (cookie->range_info != NULL) {
				cookie->new_state = NEXT_RANGE;
				break;
			}

			cookie->new_state = PROCESS_RESULT;
			cookie->next_state = state_save;
			cookie->msgId = msgId_save;
			msgId_save = 0;
			cookie->get_next_range = B_FALSE;
			rc = 0;
			break;
		case NEXT_RESULT:
			/*
			...
			if (rc == LDAP_RES_SEARCH_RESULT) {
				if (cookie->get_next_range) {
					cookie->new_state = NEXT_RANGE_RESULT;
				} else {
					cookie->new_state = END_RESULT;
					/* check and process referrals info */
					if (cookie->followRef)
						proc_result_referrals(cookie);
				}
			...
			/*
			 * If not all values of certain attributes,
			 * such as member or memberof, are returned,
			 * as indicated by the 'range' option attached
			 * to the attribute type, then search the
			 * same DN entry again to get the entire
			 * range of the attribute values. If this
			 * is not the first such search, then go
			 * to the NEXT_RESULT state to remove the
			 * remaining LDAP_RES_SEARCH_RESULT from the
			 * result message to prevent memory leaks.
			 */
			if (cookie->get_next_range) {
				cookie->new_state = NEXT_RANGE;
				state_save = cookie->state;
				break;
			}
		...
		case MULTI_RESULT:
			 ...
			 * as indicated by the 'range' option attached
			 * to the attribute type, then search the
			 * same entry again to get the rest of the
			 * attribute values.
			 */
			if (cookie->range_info != NULL) {
				cookie->new_state = NEXT_RANGE;
		...
		case LDAP_ERROR:
			if (cookie->range_info != NULL) {
				cookie->new_state = NEXT_RANGE_RESTORE;
				cookie->next_state = LDAP_ERROR;
				break;
			}
	...
	ns_ldap_entry_t **extra_info)
{
	...
	*rcp = NS_LDAP_SUCCESS;
	...
	cookie->i_extra_info_attr = extra_info_attr;
	cookie->i_auth = auth;
	...
	const void *userdata)
{
	 ...
	 * __ns_ldap_list_sort() which has the sort attribute.
	 */
...
/*
 * This function performs the same processing as __ns_ldap_list, but
 * handles 'extra_info_attr' and returns 'extra_info'. This is to allow
 * internal libsldap data, such as server type, to be requested/retured.
 */
int
__ns_ldap_list_ext(
	const char *service,
	...
	ns_ldap_entry_t **extra_info)
{
	...
	for (;;) {
		if (__s_api_setup_retry_search(&cu, NS_CONN_USER_SEARCH,
		    &try_cnt, &rc, errorp) == 0)
			break;
		rc = ldap_list(NULL, service, filter, NULL, init_filter_cb,
		    attribute, auth, mod_flags, rResult, errorp, &trc, callback,
		    userdata, cu, extra_info_attr, extra_info);
	}
...
/*
 * internal function for __ns_ldap_read_dn
 */
static int
ldap_read_dn(
	const char *dn,
	...
	ns_ldap_entry_t **extra_info)
{
	...
	*rResult = NULL;
	...
	/* no need to perform attribute mapping on the DN */
	cookie->i_flags |= NS_LDAP_NOT_CVT_DN;
	cookie->i_filter = strdup("(objectClass=*)");
	if (cookie->i_filter == NULL) {
		rc = NS_LDAP_MEMORY;
		goto err_out;
	}
...
/*
 * __ns_ldap_read_dn reads a specific DN using LDAP search to a given
 * directory server with service search descriptors and schema
 * mapping as appropriate. The operation may be retried a
 * couple of times in error situations.
 */
	...
	ns_ldap_entry_t **extra_info)
{
	...
	for (;;) {
		if (__s_api_setup_retry_search(&cu, NS_CONN_USER_SEARCH,
		    &try_cnt, &rc, errorp) == 0)
			break;
		rc = ldap_read_dn(dn, service, init_filter_cb,
		    attribute, auth, flags, rResult, errorp, &trc,
		    callback, userdata, cu, extra_info_attr, extra_info);
	}
...
__ns_ldap_firstEntry(
    ...
    const void *userdata)
{
		return (__ns_ldap_firstEntry_ext(service, filter, vlv_sort,
		    init_filter_cb, attribute, auth, flags, vcookie,
		    result, errorp, userdata, NULL, NULL));
}

/*
 * This function performs the same processing as __ns_ldap_firstEntry, but
 * handles 'extra_info_attr' and returns 'extra_info'. This is to allow
 * internal libsldap data, such as server type, to be requested/retured.
 */
int
__ns_ldap_firstEntry_ext(
    const char *service,
    const char *filter,
    const char *vlv_sort,
    int (*init_filter_cb)(const ns_ldap_search_desc_t *desc,
...
__s_api_replace_mapped_attr_in_dn(
	...
	*new_dn = NULL;
	...
	/*
	 * separate dn into individual components
	 * e.g.

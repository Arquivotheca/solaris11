...
access_conn_mgmt(int op)
{
	...
	/* Quick out for other threads, if already shutting down */
 ...
 */
static ns_conn_mt_t *
init_conn_mt(ns_conn_mgmt_t *cmg, const char *server,
	const ns_cred_t *cred, ns_ldap_error_t **ep)
{
	ns_conn_mt_t	*cm;
	...
	if (server == NULL) {		/* Any server will do */
		cm->conn_server = NULL;
	} else {
		cm->conn_server = strdup(server);
		if (cm->conn_server == NULL) {
			if (ep != NULL)
				*ep = __s_api_make_error(NS_LDAP_MEMORY, NULL);
			free(cm);
			return (NULL);
	...
	if (cred == NULL) {
		...
		cm->conn_cred = __ns_ldap_dupAuth(cred);
		if (cm->conn_cred == NULL) {
			if (ep != NULL)
				*ep = __s_api_make_error(NS_LDAP_MEMORY, NULL);
			if (cm->conn_server)
				free(cm->conn_server);
	...
	if (cmg_a != cmg) {
		...
		if (ep != NULL && *ep != NULL)
			*ep = __s_api_make_error(NS_LDAP_MEMORY, NULL);
		(void) __ns_ldap_freeCred(&cm->conn_cred);
		if (cm->conn_server)
			free(cm->conn_server);
		free(cm);
		return (NULL);
	...
	ns_conn_mt_t *cm)
{
	...
	if (server != NULL && *server != 0) {
		/* No conn yet, check conn_server instead */
		if (cm->state == NS_CONN_MT_CONNECTING) {
			if (cm->conn_server == NULL ||
			    strcasecmp(server, cm->conn_server) != 0)
				return (B_FALSE);
		} else {
	...
	/* server passed muster, so check cred */
	if (cm->state == NS_CONN_MT_CONNECTING)
		cr = cm->conn_cred;
	else
		cr = cp->auth;
	if (cr == NULL)
		return (B_FALSE);
	return (__s_api_is_auth_matched(cr, cred));
}
	...
	Connection **session, ns_ldap_error_t **errorp, ns_conn_user_t *cu)
{
	...
	*session = NULL;
	...
	cn = init_conn_mt(cmg, server, cred, errorp);
	if (cn == NULL) {
		(void) mutex_unlock(&cmg->lock);

...
/*
 * Obtain an LSA policy handle using OpenPolicy2.  A policy handle is
 * required to access LSA resources on a remote server.  The server
 * name supplied does not need the double backslash prefix; it will
 * be added here.
 *
 * If username is NULL, an anonymous connection will be established.
    ...
    mlsvc_handle_t *lsa_handle)
{
	...
	arg.servername = ndr_rpc_derive_nbhandle(lsa_handle, server);

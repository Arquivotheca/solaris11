    ...
    DWORD recno, logr_record_t *rec)
{
	...
	r_length = sizeof (logr_record_t);
	r_resv = LOGR_RECORD_SIGNATURE;
	r_strnum = 1;

	rec->Length1 = LE_IN32(&r_length);
	rec->Reserved = LE_IN32(&r_resv);
	rec->RecordNumber = LE_IN32(&recno);
	rec->TimeGenerated = LE_IN32(&le->le_timestamp.tv_sec);
	rec->TimeWritten = LE_IN32(&le->le_timestamp.tv_sec);
	logr_set_event_typeid(le->le_pri, &rec->EventType, &rec->EventID);
	rec->EventID = LE_IN32(&rec->EventID);
	rec->EventType = LE_IN16(&rec->EventType);
	rec->NumStrings = LE_IN16(&r_strnum);
	rec->EventCategory = 0;
	rec->ReservedFlags = 0;
	rec->ClosingRecordNumber = 0;
	rec->StringOffset = LE_IN32(&str_offs);
	rec->UserSidLength = 0;
	rec->UserSidOffset = 0;
	rec->DataLength = 0;
	rec->DataOffset = 0;
	...
	if (len > 0)
		...
		(void) ndr_mbstowcs(NULL, (smb_wchar_t *)(rec->info + sh_len),
		    le->le_msg, len);

	rec->Length2 = LE_IN32(&r_length);
}

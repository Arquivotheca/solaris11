...
{
	return (string_init(prop, c_default));
}
...
{
	return (elem_is_default(TO_ELEM(res)));
}
 ...
 */
int
elem_is_default(const pool_elem_t *elem)
{
	pool_value_t val = POOL_VALUE_INITIALIZER;
	uchar_t bval = 0;

	if (pool_get_ns_property(elem, c_default, &val) != POC_BOOL) {
		return (0);
	}

	if (pool_value_get_bool(&val, &bval) != PO_SUCCESS) {
		return (0);
	}

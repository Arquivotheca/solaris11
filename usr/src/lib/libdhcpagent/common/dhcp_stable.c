...
#define	DUID_FILE	"/var/dhcp/duid"
#define	IAID_FILE	"/var/dhcp/iaid"

/* We only write to the new pathnames and we prefer the new names. */
#define	OLD_DUID_FILE	"/etc/dhcp/duid"
#define	OLD_IAID_FILE	"/etc/dhcp/iaid"
...
read_stable_duid(size_t *duidlen)
{
	...
	if ((fd = open(DUID_FILE, O_RDONLY)) == -1 &&
	    (fd = open(OLD_DUID_FILE, O_RDONLY)) == -1)
		return (NULL);
...
make_stable_iaid(const char *intf, uint32_t hint)
{
	...
	if ((fd = open(IAID_FILE, O_RDONLY)) == -1 &&
	    (fd = open(OLD_IAID_FILE, O_RDONLY)) == -1)
		return (hint);
	maxid = 0;
	minunused = 1;
	/*

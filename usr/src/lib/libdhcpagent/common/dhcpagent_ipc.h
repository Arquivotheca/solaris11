...
typedef struct dhcp_status {
	 ...
	 */
	uint32_t	if_cidlen;
	uchar_t		if_cid[UCHAR_MAX];
	char		if_reqhost[MAXNAMELEN];
} dhcp_status_t;

#define	DHCP_STATUS_VER		3	/* current version of dhcp_status_t */
#define	DHCP_STATUS_VER1_SIZE	(offsetof(dhcp_status_t, if_bad_offers) + \
				    sizeof (uint32_t))
#define	DHCP_STATUS_VER2_SIZE	(offsetof(dhcp_status_t, if_cid) + UCHAR_MAX)
#define	DHCP_STATUS_VER3_SIZE	(offsetof(dhcp_status_t, if_reqhost) + \
				    MAXNAMELEN)

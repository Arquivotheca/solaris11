 ...
 */
static const uint32_t v9_asr_rdmask = 0x0fcb007d;
static const uint32_t v9_asr_wrmask = 0x0ffb004d;
...
/* ARGSUSED3 */
int
fmt_cbcond(dis_handle_t *dhp, uint32_t instr, const inst_t *inp, int idx)
{
	const char *name = inp->in_data.in_def.in_name;
	...
	if ((dhp->dh_debug & DIS_DEBUG_PRTFMT) != 0) {
		prt_field("op", f->f2d.op, 2);
		prt_field("c_hi", f->f2d.c_hi, 2);
		prt_field("c_lo", f->f2d.c_lo, 3);
		prt_field("cc2", f->f2d.cc2, 1);
		prt_field("d10_hi", f->f2d.d10_hi, 2);
		prt_field("i", f->f2d.i, 1);
		prt_field("rs1", f->f2d.rs1, 5);
		prt_field("d10_lo", f->f2d.d10_lo, 8);
		prt_field("rs2", f->f2d.rs2, 5);
	}

	disp = sign_extend((f->f2d.d10_hi << 8)|(f->f2d.d10_lo), 10);
	disp *= 4;

	prt_name(dhp, name, 1);

	regname = get_regname(dhp, FLG_RS1_VAL(flags), f->f2d.rs1);
	bprintf(dhp, "%s, ", regname);

	if (f->f2d.i == 1) {
		prt_imm(dhp, sign_extend(f->f2d.rs2, 5), IMM_SIGNED);
	} else {
		regname = get_regname(dhp, FLG_RS1_VAL(flags), f->f2d.rs2);
		bprintf(dhp, "%s", regname);
	}

	bprintf(dhp, ", %s0x%-5lx <", (disp < 0) ? "-" : "+",
	    (disp < 0) ? (-disp) : disp);
...
/* ARGSUSED3 */
int
fmt_crypto(dis_handle_t *dhp, uint32_t instr, const inst_t *inp, int idx)
{
	ifmt_t *f = (ifmt_t *)&instr;

	if ((dhp->dh_debug & DIS_DEBUG_PRTFMT) != 0) {
		prt_field("op", f->fcrypto.op, 2);
		prt_field("rd", f->fcrypto.rd, 5);
		prt_field("op3", f->fcrypto.op3, 6);
		prt_field("rs1", f->fcrypto.rs1, 5);
		prt_field("opf", f->fcrypto.opf, 9);
		prt_field("imm5", f->fcrypto.imm5, 5);
	}

	prt_name(dhp, inp->in_data.in_def.in_name, 1);
	prt_imm(dhp, f->fcrypto.imm5, 0);
...
/* ARGSUSED3 */
int
fmt_crypto4op(dis_handle_t *dhp, uint32_t instr, const inst_t *inp, int idx)
{
	ifmt_t *f = (ifmt_t *)&instr;
	int flags = inp->in_data.in_def.in_flags;

	prt_name(dhp, inp->in_data.in_def.in_name, 1);
	bprintf(dhp, "%s, %s, ", get_regname(dhp, FLG_P1_VAL(flags),
	    f->fused.rs1), get_regname(dhp, FLG_P1_VAL(flags), f->fused.rs2));

	if (idx == 0x8) {
		/* aes_kexpand1 */
		prt_imm(dhp, f->fused.rs3, 0);
	} else {
		bprintf(dhp, "%s", get_regname(dhp, FLG_P1_VAL(flags),
		    f->fused.rs3));
	}

	bprintf(dhp, ", %s", get_regname(dhp, FLG_P1_VAL(flags), f->fused.rd));

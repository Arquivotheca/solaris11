...
zonecfg_set_iptype(zone_dochandle_t handle, zone_iptype_t iptype)
{
	...
	if (xmlSetProp(cur, DTD_ATTR_IPTYPE, (const xmlChar *)(iptype ==
	    ZS_EXCLUSIVE ? "exclusive" : "shared")) == NULL)
		return (Z_INVAL);
	return (Z_OK);
}
...
zonecfg_lookup_linkname(zone_dochandle_t handle, const char *linkname)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		if (xmlStrcmp(cur->name, DTD_ELEM_NET) == 0) {
			if ((fetchprop(cur, DTD_ATTR_PHYSICAL,
			    str, sizeof (str)) == Z_OK &&
			    strcmp(linkname, str) == 0))
				return (B_TRUE);
		}
		if (xmlStrcmp(cur->name, DTD_ELEM_ANET) == 0) {
			if ((fetchprop(cur, DTD_ATTR_LINKNAME,
			    str, sizeof (str)) == Z_OK &&
			    strcmp(linkname, str) == 0))
				return (B_TRUE);
		}
...
zonecfg_lookup_net(zone_dochandle_t handle, struct zone_nettab *tabptr)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		...
		if (defrouterspec != 0 && (fetchprop(cur, DTD_ATTR_DEFROUTER,
		    ...
		    !zonecfg_same_net_address(tabptr->zone_net_defrouter,
		    address)))
			continue;
	...
	if ((err = fetchprop(cur, DTD_ATTR_PHYSICAL, tabptr->zone_net_physical,
	    sizeof (tabptr->zone_net_physical))) != Z_OK)
		return (err);

	if (iptype == ZS_SHARED &&
	    (err = fetchprop(cur, DTD_ATTR_ADDRESS, tabptr->zone_net_address,
	...
	tabptr->zone_net_configure_allowed_addr = B_TRUE;
	if ((err = fetchprop(cur, DTD_ATTR_CONFIGURE_ALLOWED_ADDRESS,
	    config_allowed_addr, sizeof (config_allowed_addr))) != Z_OK)
		return (err);
	if (strlen(config_allowed_addr) > 0) {
		tabptr->zone_net_configure_allowed_addr =
		    (strcmp(config_allowed_addr, DTD_ENTITY_TRUE) == 0);
	}
...
zonecfg_add_net_core(zone_dochandle_t handle, struct zone_nettab *tabptr)
{
	...
	if ((err = newprop(newnode, DTD_ATTR_CONFIGURE_ALLOWED_ADDRESS,
	    tabptr->zone_net_configure_allowed_addr ? DTD_ENTITY_TRUE :
	    DTD_ENTITY_FALSE)) != Z_OK)
		return (err);
...
zonecfg_delete_net_core(zone_dochandle_t handle, struct zone_nettab *tabptr)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		...
		allowed_addr_match = match_prop(cur,
...
#define	_FETCH_CMP_PROPERTY(pname, dtdattr, cmpfunc) \
	if (tabptr->pname[0] != '\0' && (fetchprop(cur, dtdattr, \
...
zonecfg_lookup_anet(zone_dochandle_t handle, struct zone_anettab *tabptr)
{
	...
	if ((err = fetchprop(cur, DTD_ATTR_LINKNAME,
	    tabptr->zone_anet_linkname,
	    sizeof (tabptr->zone_anet_linkname))) != Z_OK)
		return (err);

	if ((err = fetchprop(cur, DTD_ATTR_LOWER_LINK,
	...
	tabptr->zone_anet_configure_allowed_addr = B_TRUE;
	if ((err = fetchprop(cur, DTD_ATTR_CONFIGURE_ALLOWED_ADDRESS,
	    config_allowed_addr, sizeof (config_allowed_addr))) != Z_OK)
		return (err);
	if (strlen(config_allowed_addr) > 0) {
...
zonecfg_add_anet_core(zone_dochandle_t handle, struct zone_anettab *tabptr)
{
	...
	newnode = xmlNewTextChild(cur, NULL, DTD_ELEM_ANET, NULL);

	/* mandatory */
	if ((err = newprop(newnode, DTD_ATTR_LOWER_LINK,
	    tabptr->zone_anet_lower_link)) != Z_OK)
		return (err);

	/* optional */
	if (strlen(tabptr->zone_anet_linkname) > 0 &&
	    (err = newprop(newnode, DTD_ATTR_LINKNAME,
	...
	if ((err = newprop(newnode, DTD_ATTR_CONFIGURE_ALLOWED_ADDRESS,
	    tabptr->zone_anet_configure_allowed_addr ? DTD_ENTITY_TRUE :
	    DTD_ENTITY_FALSE)) != Z_OK)
		return (err);
...
zonecfg_delete_anet_core(zone_dochandle_t handle, struct zone_anettab *tabptr)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		 ...
		 */
		if (match_prop(cur, DTD_ATTR_LINKNAME,
		    tabptr->zone_anet_linkname)) {
			xmlUnlinkNode(cur);
			xmlFreeNode(cur);
			return (Z_OK);
...
zonecfg_lookup_dev(zone_dochandle_t handle, struct zone_devtab *tabptr)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		...
		if (strlen(tabptr->zone_dev_match) > 0 &&
		    (fetchprop(cur, DTD_ATTR_MATCH, val,
		    sizeof (val)) == Z_OK)) {
		...
		if (strlen(tabptr->zone_dev_partition) > 0 &&
		    (fetchprop(cur, DTD_ATTR_ALLOW_PARTITION, val,
		    sizeof (val)) == Z_OK)) {
		...
		if (strlen(tabptr->zone_dev_raw_io) > 0 &&
		    (fetchprop(cur, DTD_ATTR_ALLOW_RAW_IO, val,
		    sizeof (val)) == Z_OK)) {
	...
	if ((err = fetchprop(cur, DTD_ATTR_MATCH, tabptr->zone_dev_match,
		...
		return (err);
	if ((err = fetchprop(cur, DTD_ATTR_ALLOW_PARTITION,
	    tabptr->zone_dev_partition,
	    sizeof (tabptr->zone_dev_partition))) != Z_OK)
		return (err);
	if ((err = fetchprop(cur, DTD_ATTR_ALLOW_RAW_IO,
...
zonecfg_add_dev_core(zone_dochandle_t handle, struct zone_devtab *tabptr)
{
	...
	if ((err = newprop(newnode, DTD_ATTR_MATCH,
		...
		return (err);
	if ((err = newprop(newnode, DTD_ATTR_ALLOW_PARTITION,
	    tabptr->zone_dev_partition)) != Z_OK)
		return (err);
	if ((err = newprop(newnode, DTD_ATTR_ALLOW_RAW_IO,
...
zonecfg_delete_dev_core(zone_dochandle_t handle, struct zone_devtab *tabptr)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		...
		partition_match = match_prop(cur, DTD_ATTR_ALLOW_PARTITION,
		    tabptr->zone_dev_partition);
		raw_io_match = match_prop(cur, DTD_ATTR_ALLOW_RAW_IO,
...
nm_to_dtd(char *nm)
{
	...
	if (strcmp(nm, "writable-path") == 0)
		return (DTD_ELEM_WRP);
	if (strcmp(nm, "readonly-path") == 0)
		return (DTD_ELEM_ROP);
...
zonecfg_getnetent(zone_dochandle_t handle, struct zone_nettab *tabptr)
{
	...
	if ((err = fetchprop(cur, DTD_ATTR_ADDRESS, tabptr->zone_net_address,
	    sizeof (tabptr->zone_net_address))) != Z_OK) {
		handle->zone_dh_cur = handle->zone_dh_top;
		return (err);
	}
	...
	if ((err = fetchprop(cur, DTD_ATTR_CONFIGURE_ALLOWED_ADDRESS,
		...
		handle->zone_dh_cur = handle->zone_dh_top;
		return (err);
	} else if (strlen(config_allowed_addr) > 0) {
		tabptr->zone_net_configure_allowed_addr =
		    (strcmp(config_allowed_addr, DTD_ENTITY_TRUE) == 0);
	}
...
zonecfg_lookup_ds(zone_dochandle_t handle, struct zone_dstab *tabptr)
{
	...
	for (cur = cur->xmlChildrenNode; cur != NULL; cur = cur->next) {
		...
		if (strlen(tabptr->zone_dataset_alias) > 0) {
			if ((fetchprop(cur, DTD_ATTR_ALIAS, name,
			    sizeof (name)) == Z_OK) &&
...
zonecfg_getdsent(zone_dochandle_t handle, struct zone_dstab *tabptr)
{
	...
	if (fetchprop(cur, DTD_ATTR_ALIAS, tabptr->zone_dataset_alias,
	    sizeof (tabptr->zone_dataset_alias)) != Z_OK) {
 ...
 * [<altroot>]_PATH_SYSVOL/zones/<zonename>.zoneadm.lock.  If a zoneadm instance
...
zonecfg_set_mac_profile(zone_dochandle_t handle, char *profile)
{
	...
	if ((ret = verify_mac_profile(handle, profile)) == Z_OK) {
		...
		/* Clear the profile if we can't establish it. */
		(void) setrootattr(handle, DTD_ATTR_MAC_PROFILE, "");
		return (ret);
	}
}

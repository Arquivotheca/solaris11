...
typedef enum {mit_any, mit_nis, mit_ldap}	__nis_mapping_item_type_t;
 ...
 * Mapping item
 *
 * The mapping item is a single LDAP attribute such as
 *	ldap:gidNumber:ou=Group, ?one?cn=staff
 * (Note: spaces around "=" and after "," to make cstyle happy; such spaces
 * are not usually part of the syntax, but they are allowed.)
 *
 * type		mit_ldap or mit_nis
 * name		Attribute/column name
 * searchSpec	LDAP search triple

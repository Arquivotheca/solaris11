...
appendString2SingleVal(char *str, __nis_single_value_t *v, int *newLen) {
	void	*s;
	int	l, nl;
	const char *myself = "appendString2SingleVal";
...
void
c2buf(const char *msg, void *buf, int len) {
	bc2buf(msg, buf, len, &pb);
}

/* Like c2buf(), but remove trailing NUL bytes */
void
sc2buf(const char *msg, void *buf, int len) {
...
checkIPaddress(char *addr, int len, char **newaddr) {
	...
	char		*buffer;
	int		s, e;
	const char	*myself = "checkIPaddress";
...
trimWhiteSpaces(char *str, int *len, int deallocate) {
	...
	int	olen = 0;
	int	first = 1, i;
	const char *myself = "trimWhiteSpaces";
...
escapeSpecialChars(__nis_value_t *val) {
	int	i, j, k, count;
	char	*newval, *s;
	const char *myself = "escapeSpecialChars";

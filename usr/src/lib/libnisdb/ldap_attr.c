...
makeFilterComp(char *filter, int *numComps) {
	...
	char	**comp = 0, **new, *str;
	int	len;
	const char *myself = "makeFilterComp";
...
addFilterComp(char *new, char **comp, int *numComps) {
	char	**tmp, *str;
	const char *myself = "addFilterComp";
...
concatenateFilterComps(int numComps, char **comp) {
	...
	const char	*myself = "concatenateFilterComps";
 ...
 */
char **
findDNs(const char *msg, __nis_rule_value_t *rv, int nrv, char *defBase,
		int *numDN) {
	char	**dn;
	int	irv, iv, ndn;
	const char *myself = "findDNs";

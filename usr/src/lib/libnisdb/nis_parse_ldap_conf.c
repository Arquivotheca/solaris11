	...
	__nisdb_table_mapping_t	*table_info)
{
	...
	/*
	 * preferrred servers and auth method are in nisplusLDAPconfig
	 * object class.
	 */
	...
	char				**attributes)
{
	...
	if (nis_config->numberOfServiceThreads == (int)NO_VALUE_SET)
		attributes[n_attrs++] = NUMBER_THREADS;
 ...
 *	    the sequence. The corresponding macro (IS_BIND_INFO or
 *	    IS_OPER_INFO) may need to be adjusted. These are used
 *	    to partition the attributes into smaller chunks.

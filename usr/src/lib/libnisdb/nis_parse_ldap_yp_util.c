	...
	__nis_table_mapping_t   **table_mapping)
{
	...
	const char *myself = "second_parser_pass";
	int	i = 0, len;
	int	remove_t = 0;
	int	add_t = 0;
	...
	__yp_domain_context_t   *ypDomains)
{
	...
	__nis_table_mapping_t   *prev = NULL;
	int			i;
	const char		*myself = "final_parser_pass";
	int			nm;
	bool_t			r;
	int			del_tbl_flag = 0;
...
{
	__nis_table_mapping_t *t;
	const char *myself = "check_domain_specific_order";
	char	*type;
	char	*dbId = 0;
	int 	i, len;
	int		match = 0;
				...
				switch (attrib_num) {
					...
					case key_yp_repeated_field_separators:
						if (t->separatorStr !=
						    DEFAULT_SEP_STRING)
							match = 1;
					type = YP_REPEATED_FIELD_SEPARATORS;
						break;
...
char   *DomainLabel, char *Domain)
{
	...
	const char *myself = "append_domainContext";

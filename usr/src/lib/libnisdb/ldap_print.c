...
printMappingFormat(__nis_mapping_format_t *f) {
	...
	int		i;
	const char	*myself = "printMappingFormat";
...
printMappingFormatArray(__nis_mapping_format_t *a) {
	...
	const char	*myself = "printMappingFormatArray";
...
printObjSpec(__nis_obj_spec_t *o) {
	...
	const char *myself = "printObjSpec";
...
printSearchTriple(__nis_search_triple_t *s) {
	...
	const char *myself = "printSearchTriple";
...
printMappingItem(__nis_mapping_item_t *i, __nis_mapping_item_type_t native) {
	...
	int		j;
	const char	*myself = "printMappingItem";
...
printMappingSubElement(__nis_mapping_sub_element_t *e,
	...
	int	i;
	const char *myself = "printMappingSubElement";
...
printMappingElement(__nis_mapping_element_t *e,
			__nis_mapping_item_type_t native) {
	int	i;
	const char *myself = "printMappingElement";
...
printMappingRLHS(__nis_mapping_rlhs_t *m, __nis_mapping_item_type_t native) {
	int	i;
	const char *myself = "printMappingRLHS";
...
printMappingRule(__nis_mapping_rule_t *r,
	...
	const char	*myself = "printMappingRule";
...
printObjName(__nis_index_t *index, char *name) {
	const char	*myself = "printObjName";
...
printobjectDN(__nis_object_dn_t *o) {
	const char	*myself = "printobjectDN";
	int		i;
	...
	case dd_perDbId:
		...
		for (i = 0; i < o->numDbIds; i++) {
			p2buf(myself, "\t\t\t");
			printMappingRule(o->dbId[i], mit_ldap, mit_nis);
		}
		break;
	case dd_never:
		p2buf(myself, ":\n\t\tnever");
...
printTableMapping(__nis_table_mapping_t *t) {
	__nis_object_dn_t	*o;
	int			i;
	const char		*myself = "printTableMapping";
	...
	for (i = 0; i < t->numRulesFromLDAP; i++) {
		p2buf(myself, "\t\t");
		printMappingRule(t->ruleFromLDAP[i], mit_nis, mit_ldap);
		p2buf(myself, "\n");
	}
	...
	for (i = 0; i < t->numRulesToLDAP; i++) {
		p2buf(myself, "\t\t");
		printMappingRule(t->ruleToLDAP[i], mit_ldap, mit_nis);
		p2buf(myself, "\n");
	}
}
...
printRuleValue(__nis_rule_value_t *rv) {
	...
	const char	*myself = "printRuleValue";
...
printLdapMod(LDAPMod **mods, __nis_buffer_t *b) {
	LDAPMod		*m;
	char		*s;
	const char	*myself = "printLdapMod";
...
static void
printObjRights(const char *msg, void *access) {
	uchar_t	*a = access;
	int	i;
...
printObjAttr(__nis_obj_attr_t *attr) {
	const char *myself = "printObjAttr";

...
get_ttl_value(map_ctrl *map, TTL_TYPE type)
{
	...
	int interval, res;
	const char *myself = "get_ttl_value";
...
{
	const char *myself = "get_passwd_list";
...
update_map_from_dit(map_ctrl *map, bool_t log_flag) {
	...
	map_ctrl		temp_map;
	datum			key;
	const char		*myself = "update_map_from_dit";
	bool_t			secure_flag;
	int			entry_count = 1;
	...
	const char		*xlate_to_lcase_maps[] = {
		"hosts.byname",
		"ipnodes.byname",
		NULL
	};
...
{
	const char *myself = "get_mapping_map_list";
	__nis_hash_item_mt *it;
	int	i, j, size;
	char 	*end_ptr;
	char	**res;		/* Result array */
...
make_nis_container(char *map_name, char *domain, bool_t init_containers) {
	...
	__nis_table_mapping_t	*t;
	char			*dn;
	const char		*myself = "make_nis_container";
...
update_netgroup_byxxx(map_ctrl *map) {
	...
	DBM			*temp_ttl_db;
	map_ctrl		temp_map;
	const char		*myself = "update_netgroup_byxxx";
	char			*cmdbuf;
	int			cmd_length;
	datum			key;

...
{
	const char *myself = "proc_map_list";
	char	*map_name;
	char	cmdbuf[BUFSIZ];
	int	map_name_len = 0;
	int	index, ans = 0;
...
{
	const char *myself = "decode_pwd_entry";
	char *dptr = data->dptr;
	char *p, *str_end, *temp;

	/* Work out last location in string */
	str_end = dptr + data->dsize;

	/* Name */
	if (NULL == (p = get_next_token(dptr, &temp, str_end)))
		return (FAILURE);
	if (adjunct) {
...
{
	const char *myself = "get_next_token";
...
{
	const char *myself = "create_pwd_str";
	char *s;
	int len;
...
{
	const char *myself = "get_old_info";
...
{
	const char *myself = "get_old_shadow";
	struct spwd *sp;
	datum key, data;
	suc_code res;
...
decode_shadow_entry(datum *data, struct spwd *sp)
{
	...
	/* Name */
	if (NULL == (p = get_next_token(dptr, &(sp->sp_namp), str_end)))
		return (FAILURE);
...
{
	const char *myself = "write_shadow_info";
	datum key, data;
	char *str;
	suc_code res;
	int len;

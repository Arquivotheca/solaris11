...
getFullMapName(char *map, char *domain) {
	const char	*myself = "getFullMapName";
	char	*objPath;
	if (map == 0 || domain == 0) {
		return (0);
	}
...
__nis_value_t *stringToValue(char *dptr, int dsize) {
	const char	*myself = "stringToValue";
	char		*emptystr = "";
	__nis_value_t	*val;
...
processSplitField(__nis_table_mapping_t *sf, __nis_value_t *inVal,
	...
	char			*str, *oldstr;
	const char		*myself = "processSplitField";
...
datumToRuleValue(datum *key, datum *value, __nis_table_mapping_t *t,
	...
	if (value != 0 && t->e) {
		...
		for (; valueLen > 0 && (dptr[valueLen - 1] == ' ' ||
			dptr[valueLen - 1] == '\t'); valueLen--);
		 ...
		 * into fields.
		 */
		if ((val = stringToValue(dptr, valueLen)) == 0) {
			freeRuleValue(rvq, 1);
			*statP = MAP_NO_MEMORY;
			return (0);
		}
...
addSplitFieldValues(__nis_table_mapping_t *t, __nis_rule_value_t *rv,
	...
	char			delim[2] = {0, 0};
	char			*emptystr = "";
	const char		*myself = "addSplitFieldValues";
...
ruleValueToDatum(__nis_table_mapping_t *t, __nis_rule_value_t *rv, int *statP) {
	...
	const char	*myself = "ruleValueToDatum";
	...
	*statP = 0;
	...
	if (val && val->val && val->val->value) {
		...
		if (t->commentChar != '\0' &&
		    (str = findVal(N2LCOMMENT, rv, mit_nis)) != 0 &&
		    *str != '\0') {
    ...
    int *statP, bool_t xlate_to_lcase)
{
	...
	char	*dptr;
	char	*str;
	const char	*myself = "getKeyFromRuleValue";
...
singleReadFromDIT(char *map, char *domain, datum *key, datum *value,
	...
	const char		*myself = "singleReadFromDIT";
...
singleWriteToDIT(char *map, char *domain, datum *key, datum *value,
	...
	const char		*myself = "singleWriteToDIT";
	char			*dptr;
	...
	for (i = 0; i < key->dsize && ((dptr = key->dptr)[i] == 0 ||
		dptr[i] == ' ' || dptr[i] == '\t'); i++);
	if (i >= key->dsize)
		return (SUCCESS);
...
makeNISObject(char *domain, char *dn) {
	...
	int			i, rc, nr, add_rc;
	char			*val;
	const char		*myself = "makeNISObject";
...
addNISObject(char *domain, char *dn, int *ldap_rc) {
	...
	const char		*myself = "addNISObject";

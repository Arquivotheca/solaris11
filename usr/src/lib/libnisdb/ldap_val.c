...
cloneMappingFormat(__nis_mapping_format_t *m) {
	...
	int			i, nf, err;
	const char		*myself = "cloneMappingFormat";
...
cloneValue(__nis_value_t *val, int count) {
	...
	for (c = 0; c < count; c++) {
		...
		for (i = 0; i < n[c].numVals; i++) {
			 ...
			 * LDAP has a tendency to store strings without a
			 * NUL, so the value length may not include the NUL
			 * (even though it's there). In order to preserve that
			 * NUL, we add a byte to the length if the type is
			 * vt_string, and there isn't already a NUL at the
			 * end. The memory allocation function (am()) will
			 * take care of actually putting the NUL in place,
			 * since it allocates zero-initialized memory.
			 */
		...
		for (i = 0; i < numName; i++) {
			...
			if (cmp == 0) {
				...
				if (val != 0) {
					...
					/*
					 * If we are here, it indicates
					 * that the 'val has a valid
...
getMappingFormat(__nis_mapping_format_t *f, __nis_rule_value_t *rv,
	...
	const char	*myself = "getMappingFormat";
	__nis_value_t	*val = 0;
...
explodeValues(__nis_value_t *v1, __nis_value_t *v2) {
	...
	__nis_value_t	*v;
	__nis_buffer_t	b = {0, 0};
	const char	*myself = "explodeValues";
...
getSearchTriple(__nis_search_triple_t *s, int *len) {
	...
	char		*a;
	int		l;
	const char	*myself = "getSearchTriple";
...
getMappingItem(__nis_mapping_item_t *i, __nis_mapping_item_type_t native,
		__nis_rule_value_t *rv, char *berstring, int *np_ldap_stat) {
	const char	*myself = "getMappingItem";
	__nis_value_t	*val = 0;
	__nis_buffer_t	b = {0, 0};
	int		len = 0;
	char		*buf;
...
cloneObjSpec(__nis_obj_spec_t *old) {
	const char		*myself = "cloneObjSpec";
	int			err = 0;
...
copySearchTriple(__nis_search_triple_t *old, __nis_search_triple_t *new,
		int *err) {
	const char		*myself = "copySearchTriple";
...
cloneSearchTriple(__nis_search_triple_t *old) {
	const char		*myself = "cloneSearchTriple";
	int			err = 0;
...
cloneTripleOrObj(__nis_mapping_item_type_t type, __nis_triple_or_obj_t *old) {
	const char		*myself = "cloneTripleOrObj";
	int			err = 0;
...
copyItem(__nis_mapping_item_t *old, __nis_mapping_item_t *new, int *err) {
	...
	*err = 0;
	...
	if (old->type == mit_nis || old->type == mit_ldap)
...
concatenateMappingItem(__nis_mapping_item_t *old, int numItems,
	...
	__nis_mapping_item_t	*new;
	int			i, err = 0;
	const char		*myself = "concatenateMappingItem";
...
concatenateValues(__nis_value_t *v1, __nis_value_t *v2) {
	int		i, n, a;
	__nis_value_t	*v;
	const char	*myself = "concatenateValues";
...
scanMappingFormat(__nis_mapping_format_t *f, int curf, int nf, char *str,
	...
	int	i, len;
	bool_t	match;
	const char	*myself = "scanMappingFormat";
	/* N2L variables */
	int	af, skipspaces = 0;
	bool_t	ipaddr = FALSE;
	...
	/*
	 * If separator list is NULL, then simply use empty string.
	 */
	if (sepset == 0)
		sepset = emptystr;
...
matchMappingItem(__nis_mapping_format_t *f, __nis_value_t *inVal,
	...
	char			*str, *valstr;
	__nis_mapping_format_t	*ftmp;
	const char		*myself = "matchMappingItem";
...
getMappingElement(__nis_mapping_element_t *e, __nis_mapping_item_type_t native,
	...
	int *np_ldap_stat;
	const char	*myself = "getMappingElement";
	...
	case me_print:
		 ...
		 * then we had enough column values which can produce
		 * value for this rule, but didn't. So return

    ...
    unsigned char *lm_rsp)
{
	...
	rc = smb_auth_DES(lm_rsp, SMBAUTH_RESP_SZ, S21, 21, challenge, clen);
	return (rc);
}
    ...
    unsigned char *ntlm_rsp)
{
	...
	bcopy(hash, S21, SMBAUTH_HASH_SZ);
	bzero(&S21[SMBAUTH_HASH_SZ], 5);
	if (smb_auth_DES((unsigned char *)ntlm_rsp, SMBAUTH_RESP_SZ,
...
static int
smb_auth_ntlm2_session_hash(
    const unsigned char *nonce, int nlen, unsigned char *ntlm2_sess_hash)
{
	uint8_t digest[MD_DIGEST_LEN];

	if (smb_auth_md5(digest, nonce, nlen) != SMBAUTH_SUCCESS)
		return (SMBAUTH_FAILURE);

	bcopy(digest, ntlm2_sess_hash, SMBAUTH_SESSION_HASH_SZ);
 ...
 */
static int
smb_auth_ntlm2_session_response(unsigned char *session_nonce, int slen,
    const unsigned char *ntlm_hash, int hlen, unsigned char *ntlm2_sess_rsp,
    int rlen)
{
	unsigned char ntlm2_sess_hash[SMBAUTH_SESSION_HASH_SZ];
	unsigned char S21[21];

	if (smb_auth_ntlm2_session_hash(session_nonce, slen,
	    ntlm2_sess_hash) == SMBAUTH_FAILURE)
		return (SMBAUTH_FAILURE);

	bzero(S21, 21);
	bcopy(ntlm_hash, S21, hlen);

	return (smb_auth_DES(ntlm2_sess_rsp, rlen, S21, 21, ntlm2_sess_hash,
	    SMBAUTH_SESSION_HASH_SZ));
}
...
{
	unsigned char lm_resp[SMBAUTH_RESP_SZ];
	int rc;
	...
	return (bcmp(lm_resp, passwd, SMBAUTH_RESP_SZ) == 0);
}
...
{
	unsigned char ntlm_resp[SMBAUTH_RESP_SZ];
	int rc;
	boolean_t ok;
	...
	ok = (bcmp(ntlm_resp, passwd, SMBAUTH_RESP_SZ) == 0);
	if (ok && (session_key)) {
 ...
 */
int
smb_auth_ntlm2_usr_ssn_key(
    const unsigned char *ntlm_hash, const unsigned char *sess_nonce,
    unsigned char *result)
{
	unsigned char ntlm_usr_sess_key[SMBAUTH_HASH_SZ];

	if (smb_auth_md4(ntlm_usr_sess_key, ntlm_hash, SMBAUTH_HASH_SZ)
	    != SMBAUTH_SUCCESS)
		return (SMBAUTH_FAILURE);

	return (SMBAUTH_HMACT64(sess_nonce, SMBAUTH_SESSION_NONCE_SZ,
	    ntlm_usr_sess_key, SMBAUTH_SESSION_KEY_SZ, result));

}

static boolean_t
smb_ntlm2_session_password_ok(
    unsigned char *challenge,
    uint32_t clen,
    unsigned char *ntlm_hash,
    unsigned char *passwd,
    ...
    uchar_t *session_key)
{
	...
	unsigned char ntlm2_sess_resp[SMBAUTH_RESP_SZ];
	unsigned char ntlm2_sess_nonce[SMBAUTH_SESSION_NONCE_SZ];

	bcopy(challenge, ntlm2_sess_nonce, clen);
	bcopy(nonce, ntlm2_sess_nonce + clen, nlen);

	if (smb_auth_ntlm2_session_response(ntlm2_sess_nonce,
	    SMBAUTH_SESSION_NONCE_SZ, ntlm_hash, SMBAUTH_HASH_SZ,
	    ntlm2_sess_resp, SMBAUTH_RESP_SZ) != SMBAUTH_SUCCESS)
		return (B_FALSE);

	if (smb_auth_ntlm2_usr_ssn_key(ntlm_hash, ntlm2_sess_nonce,
	    session_key) != SMBAUTH_SUCCESS)
		return (B_FALSE);

	return (bcmp(passwd, ntlm2_sess_resp, pwdlen) == 0);
}
    ...
    char *username)
{
	...
	unsigned char ntlmv2_hash[SMBAUTH_HASH_SZ];
	unsigned char lmv2_resp[SMBAUTH_RESP_SZ];
	boolean_t ok = B_FALSE;
	char *dest[3];
	int i;
	...
	for (i = 0; i < (sizeof (dest) / sizeof (char *)); i++) {
		...
		ok = (bcmp(passwd, lmv2_resp, SMBAUTH_RESP_SZ) == 0);
		if (ok)
			break;
	}
    ...
    char *username)
{
	...
	if (smb_config_getnum(SMB_CI_SVR_LM_LEVEL, &lmlevel) != SMBD_SMF_OK)
		return (B_FALSE);
 ...
 *
 * Validates given NTLM/NTLMv2/NTLM 2 session response sent by the client,
    ...
    uchar_t *session_key)
{
	...
	if ((lmlevel == 5) && (pwdlen <= SMBAUTH_RESP_SZ))
		return (B_FALSE);
	...
	if (!ok && (nonce != NULL))
		ok = smb_ntlm2_session_password_ok(challenge, clen,
		    smbpw->pw_nthash, passwd, pwdlen, nonce, nlen,
		    session_key);
	return (ok);
}

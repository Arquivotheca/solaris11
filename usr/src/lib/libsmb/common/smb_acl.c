...
static acl_t *smb_fsacl_null_empty(smb_acl_t *);
...
smb_acl_to_zfs(smb_acl_t *acl, uint32_t flags, int which_acl, acl_t **fs_acl)
{
	...
	if ((acl == NULL) || (acl->sl_acecnt == 0)) {
		if (which_acl == SMB_DACL_SECINFO)
			*fs_acl = smb_fsacl_null_empty(acl);
	...
	if ((zacl = smb_fsacl_alloc(acl->sl_acecnt, flags)) == NULL)
		return (NT_STATUS_INTERNAL_ERROR);
 ...
 */
static acl_t *
smb_fsacl_null_empty(smb_acl_t *acl)
{
	acl_t *zacl;
	ace_t *zace;

	if ((zacl = smb_fsacl_alloc(1, ACL_AUTO_INHERIT)) == NULL)
		return (NULL);

...
static int smb_lgrp_dtbl_fuid(sqlite *, smb_sid_t *, uint16_t,
    uint32_t *, uint32_t *);
 ...
 */
int
smb_lgrp_add(char *name, char *cmnt)
{
	smb_wka_t *wka;
	struct group *pxgrp;
	smb_group_t grp;
	...
	(void) strlcpy(gname, name, MAXNAMELEN);
 ...
 */
int
smb_lgrp_rename(char *name, char *new_name)
{
	smb_group_t grp;
	sqlite *db;
	int rc;
	char gname[MAXNAMELEN], new_gname[MAXNAMELEN];
	...
	(void) strlcpy(new_gname, new_name, MAXNAMELEN);
	if (!smb_lgrp_normalize_name(new_gname))
		return (SMB_LGRP_INVALID_NAME);
 ...
 */
int
smb_lgrp_setcmnt(char *name, char *cmnt)
{
	smb_group_t grp;
	sqlite *db;
	int rc;
 ...
 */
int
smb_lgrp_getcmnt(char *name, char **cmnt)
{
	smb_group_t grp;
	sqlite *db;
	int rc;
 ...
 */
int
smb_lgrp_setpriv(char *name, uint8_t priv_lid, boolean_t enable)
{
	sqlite *db;
	int rc;
	char gname[MAXNAMELEN];
 ...
 */
int
smb_lgrp_getpriv(char *name, uint8_t priv_lid, boolean_t *enable)
{
	sqlite *db;
	smb_group_t grp;
	int rc;
 ...
 */
int
smb_lgrp_add_member(char *name, smb_sid_t *msid, uint16_t sid_type)
{
	sqlite *db;
	smb_gsid_t mid;
	int rc;
 ...
 */
int
smb_lgrp_del_member(char *name, smb_sid_t *msid, uint16_t sid_type)
{
	sqlite *db;
	smb_gsid_t mid;
	int rc;
 ...
 */
int
smb_lgrp_getbyname(char *name, smb_group_t *grp)
{
	sqlite *db;
	int rc;
	char gname[MAXNAMELEN];
 ...
 * arg 'count'. Local groups are either in BUILTIN domain
 * or in machine/local domain. Groups in the machine domain
 * MUST have a corresponding Solaris group at the time they
 * are created, which means there is a valid GID mapping for
 * their SID, therefore SMB_LGRP_POSIX_IDX is used to lookup
 * groups in the machine domain.
 */
int
...
smb_lgrp_start(void)
{
	...
	for (i = 0; i < ngrp; i++) {
		...
		if (!smb_lgrp_exists(wka->wka_name)) {
			...
			if (rc != SMB_LGRP_SUCCESS)
				syslog(LOG_DEBUG, "failed to add %s group",
				    wka->wka_name);
		}
	}
...
smb_lgrp_gtbl_insert(sqlite *db, smb_group_t *grp)
{
	...
	dom_idx = (grp->sg_domain == SMB_DOMAIN_LOCAL)
	    ? SMB_LGRP_POSIX_IDX : SMB_LGRP_BUILTIN_IDX;
    ...
    int flags)
{
	...
	rc = smb_lgrp_dtbl_fuid(db, member->gs_sid, mid.m_type,
	    &mid.m_idx, &mid.m_rid);
	if (rc != SMB_LGRP_SUCCESS) {
		sqlite_free_table(result);
		return (rc);
...
/*
 * Returns the FUID for the given member SID. FUID's upper
 * part is returned in 'dom_idx' and the lower part in 'rid'.
 *
 * If the given SID is mapped to a valid UID/GID then
 *   dom_idx = 0
 *   rid = UID/GID
 *
 * otherwise the given SID is split into a domain SID and
 * a RID. The RID is returned in 'rid'. dom_idx will be the
 * index of the domain SID in the domain table whether it's
 * already there or it's inserted if it doesn't exist.
 */
static int
smb_lgrp_dtbl_fuid(sqlite *db, smb_sid_t *sid, uint16_t sid_type,
    uint32_t *dom_idx, uint32_t *rid)
{
	...
	id_type = (sid_type == SidTypeUser) ? SMB_IDMAP_USER : SMB_IDMAP_GROUP;
	if (smb_idmap_getid(sid, rid, &id_type) == IDMAP_SUCCESS) {
		if (!IDMAP_ID_IS_EPHEMERAL(*rid)) {
			*dom_idx = SMB_LGRP_POSIX_IDX;
			return (SMB_LGRP_SUCCESS);
		}
	}
    ...
    sqlite *db)
{
	...
	m_ids = calloc(m_num, sizeof (smb_lgmid_t));
	if (m_ids == NULL)
		return (SMB_LGRP_NO_MEMORY);
...
smb_lgrp_decode(smb_group_t *grp, char **values, int infolvl, sqlite *db)
{
	...
	if (infolvl & SMB_LGRP_INFO_SID) {
		...
		grp->sg_domain = (sid_idx == SMB_LGRP_POSIX_IDX)
    ...
    sqlite *db, smb_sid_t **sid)
{
	...
	*sid = NULL;
	if (dom_idx == SMB_LGRP_POSIX_IDX) {
		...
		/*
		 * Make sure the returned SID is local if the SID is of type
		 * Alias
		 */
		if ((sid_type == SidTypeAlias) &&
		    !smb_sid_indomain(smb_localgrp.lg_machine_sid, res_sid)) {
			smb_sid_tostr(res_sid, strsid);
			syslog(LOG_ERR, "smb_lgrp_getsid: "
			    "local group is mapped to a non-local SID: %d->%s",
			    *rid, strsid);

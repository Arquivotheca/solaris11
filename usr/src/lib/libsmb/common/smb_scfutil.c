...
/* helper functions for using libscf */
    ...
    char *inst_name)
{
	...
	if (serv_prefix == NULL || inst_name == NULL)
		return (SMBD_SMF_SYSTEM_ERR);
...
smb_smf_delete_instance_pgroup(smb_scfhandle_t *handle, char *pgroup)
{
	 ...
	 */
	if (scf_instance_get_pg(handle->scf_instance, pgroup,
	    handle->scf_pg) == 0) {
		...
		if (scf_pg_delete(handle->scf_pg) != 0) {
			...
			err = scf_error();
			if (err != SCF_ERROR_NONE) {
				smb_smf_scf_log_error("SMF pg delete "
				    "problem: %s\n");
			}
		}
	} else {
		err = scf_error();
		if (err != SCF_ERROR_NONE)
			smb_smf_scf_log_error("SMF pg get problem: %s\n");
		ret = SMBD_SMF_SYSTEM_ERR;
	}

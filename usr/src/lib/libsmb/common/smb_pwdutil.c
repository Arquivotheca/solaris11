...
static smb_passwd_t *smb_pwd_getpwnam_core(const char *, smb_passwd_t *,
    boolean_t);
...
smb_pwd_getpwnam(const char *name, smb_passwd_t *smbpw)
{
	...
	pw = smb_pwd_getpwnam_core(name, smbpw, B_TRUE);
	if (pw == NULL)
		pw = smb_pwd_getpwnam_core(name, smbpw, B_FALSE);
...
smb_pwd_update(const char *name, const char *password, int control)
{
	...
	if (smb_config_getnum(SMB_CI_SVR_LM_LEVEL, &lm_level) != SMBD_SMF_OK)
		lm_level = 4;
 ...
 */
static smb_passwd_t *
smb_pwd_getpwnam_core(const char *name, smb_passwd_t *smbpw, boolean_t cs)
{
	boolean_t found = B_FALSE;
	smb_pwbuf_t pwbuf;
	FILE *fp;
	...
	while (smb_pwd_fgetent(fp, &pwbuf, SMB_PWD_GETF_ALL) != NULL) {
		if (cs)
			err = strcmp(name, smbpw->pw_name);
		else
			err = strcasecmp(name, smbpw->pw_name);
...
smb_pwd_lock(void)
{
	...
	if (smb_file_lock(SMB_PASSLCK, &lockinfo)) {
		switch (errno) {
		case EINTR:
			res = SMB_PWE_BUSY;
			break;

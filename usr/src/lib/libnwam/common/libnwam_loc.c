...
static nwam_error_t valid_resolv_sortlist(nwam_value_t);
...
struct nwam_prop_table_entry loc_prop_table_entries[] = {
	...
	{NWAM_LOC_PROP_DNS_NAMESERVICE_SEARCH, NWAM_VALUE_TYPE_STRING, B_FALSE,
	    ...
	    NWAM_TYPE_ANY, NWAM_CLASS_ANY},
	{NWAM_LOC_PROP_DNS_NAMESERVICE_SORTLIST, NWAM_VALUE_TYPE_STRING,
	    B_FALSE, 0, NWAM_MAX_NUM_VALUES, valid_resolv_sortlist,
	    ...
	    NWAM_TYPE_ANY, NWAM_CLASS_ANY},
	{NWAM_LOC_PROP_DNS_NAMESERVICE_OPTIONS, NWAM_VALUE_TYPE_STRING, B_FALSE,
...
static nwam_error_t
valid_resolv_sortlist(nwam_value_t value)
{
	char **sortvalues, *copy, *addr = NULL;
	uint_t i, numvalues;
	nwam_value_t val = NULL;
	nwam_error_t err;

	if (nwam_value_get_string_array(value, &sortvalues, &numvalues)
	    != NWAM_SUCCESS)
		return (NWAM_ENTITY_INVALID_VALUE);

	for (i = 0; i < numvalues; i++) {
		copy = strdup(sortvalues[i]);

		/* may include netmask, which is separated by a slash */
		addr = strsep(&copy, "/");

		if ((err = nwam_value_create_string(addr, &val))
		    != NWAM_SUCCESS)
			goto fail;
		if ((err = nwam_valid_host_any(val)) != NWAM_SUCCESS)
			goto fail;
		nwam_value_free(val);
		val = NULL;
		...
		if (copy != NULL) {
			if ((err = nwam_value_create_string(copy, &val))
			    != NWAM_SUCCESS)
				goto fail;
			if ((err = nwam_valid_host_any(val)) != NWAM_SUCCESS)
				goto fail;
    ...
    nwam_value_t nameservicesval, const char **errpropp)
{
	 ...
	 */
	for (i = 0; i < numvalues; i++) {
		nwam_value_t dom = NULL, srch = NULL;
		...
		case NWAM_NAMESERVICES_DNS:
			...
			if (nwam_loc_get_prop_value(loch,
			    NWAM_LOC_PROP_DNS_NAMESERVICE_DOMAIN, &dom)
			    != NWAM_ENTITY_NOT_FOUND &&
			    nwam_loc_get_prop_value(loch,
			    NWAM_LOC_PROP_DNS_NAMESERVICE_SEARCH, &srch)
			    != NWAM_ENTITY_NOT_FOUND) {
				nwam_value_free(dom);
				nwam_value_free(srch);

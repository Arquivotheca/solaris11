...
changelist_prefix(prop_changelist_t *clp)
{
	...
	for (cn = uu_list_first(clp->cl_list); cn != NULL;
		...
		/*
		 * If the pool is imported read-only then all mounts must be
		 * read-only.
		 */
		if (zpool_get_prop_int(cn->cn_handle->zpool_hdl,
		    ZPOOL_PROP_READONLY, NULL) != 0)
			continue;
		...
		if (!ZFS_IS_VOLUME(cn->cn_handle) &&
			...
			err = zfs_unmount(cn->cn_handle, NULL, clp->cl_mflags);
			if (err != 0) {
				ret = -1;
				cn->cn_needpost = B_FALSE;
			}
...
changelist_postfix(prop_changelist_t *clp)
{
	...
	for (cn = uu_list_last(clp->cl_list); cn != NULL;
		...
		/*
		 * Shadow changes are handled separately.
		 */
		if (clp->cl_prop == ZFS_PROP_SHADOW) {
			(void) zfs_shadow_change(cn->cn_handle);
			continue;
		}
		...
		else if (cn->cn_shared_nfs || clp->cl_waslegacy)
			errors += zfs_unshare_nfs(cn->cn_handle, NULL);
		if (sharesmb && mounted)
			errors += zfs_share_smb(cn->cn_handle);
		else if (cn->cn_shared_smb || clp->cl_waslegacy)
...
/*
 * Check if there is any child in use in a local zone in a given changelist.
...
change_one(zfs_handle_t *zhp, void *data)
{
	...
	if (clp->cl_alldependents || clp->cl_allchildren ||
		...
		if (!ZFS_IS_VOLUME(zhp)) {
			cn->cn_shared_nfs = zfs_is_shared_nfs(zhp, NULL);
			cn->cn_shared_smb = zfs_is_shared_smb(zhp, NULL);
		}
		cn->cn_zoned = zfs_prop_get_int(zhp, ZFS_PROP_ZONED);
		cn->cn_needpost = B_TRUE;

		/* Indicate if any child is in use in a local zone. */
		if (getzoneid() == GLOBAL_ZONEID && cn->cn_zoned &&
		    cn->cn_mounted)
    ...
    int mnt_flags)
{
	...
	} else if (clp->cl_prop != ZFS_PROP_SHADOW &&
	    zfs_iter_children(zhp, change_one, clp) != 0) {
		changelist_free(clp);
		return (NULL);
	}
	...
	if (!ZFS_IS_VOLUME(temp)) {
		cn->cn_shared_nfs = zfs_is_shared_nfs(temp, NULL);
		cn->cn_shared_smb = zfs_is_shared_smb(temp, NULL);
	}
	cn->cn_zoned = zfs_prop_get_int(zhp, ZFS_PROP_ZONED);
	cn->cn_needpost = B_TRUE;

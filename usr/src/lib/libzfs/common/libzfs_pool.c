...
static boolean_t zpool_has_duplicate_vdev(zpool_handle_t *zhp, nvlist_t *tree);
    ...
    nvlist_t *props, nvlist_t *fsprops)
{
	...
	if (zfs_crypto_zckey(hdl, ZFS_CRYPTO_PCREATE, zc_fsprops, &zc) != 0)
		goto create_failed;
...
zpool_add(zpool_handle_t *zhp, nvlist_t *nvroot)
{
	...
	if (zpool_has_duplicate_vdev(zhp, nvroot)) {
		zfs_error_aux(hdl, dgettext(TEXT_DOMAIN, "one or more "
		    "devices is already present in the pool"));
		return (zfs_error(hdl, EZFS_BADDEV, msg));
	}
    ...
    boolean_t *l2cache, boolean_t *log)
{
	...
	if (guid != 0 && *end == '\0') {
		...
		/*
		 * If 'arg' is a symlink, read the link and try to convert it
		 * into a whole-disk ctd name.  This is done to support
		 * references to /dev/chassis/.../disk, where the link value
		 * resolves to a whole-disk /devices path. If the di_cro
		 * interfaces can convert the link value into a disk ctd name,
		 * then we proceed with that ctd name as whole-disk 'path'.
		 */
		if (libzfs_get_cro_hdl(hdl) &&
		    (lstat64(path, &statbuf) == 0) &&
			...
			ctd = di_cro_rec_fgeti_occupant_compdev(r,
			    0, NULL, NULL);
...
noloc:			if (ctd)
		...
		if (path[0] != '/') {
			(void) snprintf(buf, sizeof (buf), "%s%s",
			    "/dev/dsk/", path);
			verify(nvlist_add_string(search,
			    ZPOOL_CONFIG_PATH, buf) == 0);
		} else {
			verify(nvlist_add_string(search,
...
static boolean_t
zpool_has_duplicate_vdev(zpool_handle_t *zhp, nvlist_t *cfg)
{
	nvlist_t **child;
	nvpair_t *elem = NULL;
	uint_t count;

	if (nvlist_lookup_nvlist_array(cfg, ZPOOL_CONFIG_CHILDREN,
	    &child, &count) == 0) {
		for (int i = 0; i < count; i++) {
			if (zpool_has_duplicate_vdev(zhp, child[i]))
				return (B_TRUE);
		}
		return (B_FALSE);
	}

	while ((elem = nvlist_next_nvpair(cfg, elem)) != NULL) {
		if (strcmp(nvpair_name(elem), ZPOOL_CONFIG_PATH) == 0) {
			boolean_t as, l2;
			char *path;

			verify(nvpair_value_string(elem, &path) == 0);
			if (zpool_find_vdev(zhp, path, &as, &l2, NULL) != NULL)
				return (B_TRUE);
		}
	}
    ...
    vdev_state_t *newstate)
{
	...
	/* put in-use spare device back to available when possible */
	zc.zc_obj = flags | ZFS_ONLINE_UNSPARE;
    ...
    const char *old_disk, const char *new_disk, nvlist_t *nvroot, int replacing)
{
	...
	if ((newname = zpool_vdev_name(NULL, NULL, child[0],
	 ...
	 */
	new_vdev = zpool_find_vdev(zhp, newname, &avail_spare, &l2cache, NULL);
	if (replacing &&
	    nvlist_lookup_uint64(tgt, ZPOOL_CONFIG_IS_SPARE, &val) == 0 &&
	    (new_vdev == NULL || !avail_spare) &&
	...
	/*
	 * If the target is not a hot spare but we can still find 'newname' in
	 * the vdev tree, then it cannot be used to replace another disk.
	 */
	if (ret == 0 && new_vdev != NULL && !avail_spare) {
		uint64_t old_guid, new_guid;

		verify(nvlist_lookup_uint64(tgt, ZPOOL_CONFIG_GUID,
		    &old_guid) == 0);
		verify(nvlist_lookup_uint64(new_vdev, ZPOOL_CONFIG_GUID,
		    &new_guid) == 0);

		/*
		 * If the guids match, then we're doing an in-place replace,
		 * which is allowed.  If the guids don't match, then we're
		 * attempting to re-use the device illegally.
		 */
		if (old_guid != new_guid) {
			zfs_error_aux(hdl, dgettext(TEXT_DOMAIN,
			    "is already part of pool %s."), zhp->zpool_name);
			ret = EZFS_BADTARGET;
		}
	}
...
zpool_vdev_clear(zpool_handle_t *zhp, uint64_t guid)
{
	...
	(void) strlcpy(zc.zc_name, zhp->zpool_name, sizeof (zc.zc_name));
	zc.zc_guid = guid;
	zc.zc_cookie = ZPOOL_NORMAL_LOAD;
	...
	if (nvlist_lookup_uint64(nv, ZPOOL_CONFIG_NOT_PRESENT,
		...
		if (nvlist_lookup_uint64(nv, ZPOOL_CONFIG_WHOLE_DISK,
			...
			if (CTD_CHECK(tmp)) {
				...
				/*
				 * If asking for location, use di_cro interfaces
				 * to translate a ctd name into a /dev/chassis
				 * path describing location.
				 */
				if (location && !old) {
					di_cro_reca_t ra;
					di_cro_rec_t r;
					...
					query_len = strlen(query_fmt) +
					    strlen("disk") + strlen(tmp) + 1;
					if ((query = malloc(query_len)) == NULL)
						goto noloc;
					(void) snprintf(query, query_len,
					    query_fmt, "disk", tmp);
					ra = di_cro_reca_create_query(
					    (di_cro_hdl_t)
					    libzfs_get_cro_hdl(hdl), 0, query);
					r = di_cro_reca_next(ra, NULL);
					dp = di_cro_rec_fgeti_devchassis_path(r,
					    0, NULL, NULL);
					di_cro_reca_destroy(ra);
					free(query);

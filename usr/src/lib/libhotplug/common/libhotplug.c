 ...
 */
char *
hp_state_priv(hp_node_t node)
{
	i_hp_dprintf("hp_state_priv: node=%p\n", (void *)node);
 ...
 */
char *
hp_driver(hp_node_t node)
{
	i_hp_dprintf("hp_driver: node=%p\n", (void *)node);
 ...
 */
int
hp_instance(hp_node_t node)
{
	i_hp_dprintf("hp_instance: node=%p\n", (void *)node);
 ...
 */
int
hp_set_state(hp_node_t node, uint_t flags, int state, const char *state_priv,
    hp_node_t *resultsp)
{
	hp_node_t	root = NULL;
	nvlist_t	*args;
	...
	i_hp_dprintf("hp_set_state: node=%p, flags=0x%x, state=0x%x, "
	    "state_priv=%s, resultsp=%p\n", (void *)node, flags, state,
	    state_priv ? state_priv : "NULL", (void *)resultsp);
 ...
 */
int
hp_install(hp_node_t node, uint_t flags, hp_node_t *resultsp)
{
	hp_node_t	root = NULL;
	nvlist_t	*args;
	nvlist_t	*results;
	...
	/* Build arguments for door call */
	if ((args = i_hp_set_args(HP_CMD_INSTALL, path, connection, flags,
	    NULL, 0)) == NULL)
		return (ENOMEM);
 ...
 */
int
hp_uninstall(hp_node_t node, uint_t flags, hp_node_t *resultsp)
{
	hp_node_t	root = NULL;
	nvlist_t	*args;
	nvlist_t	*results;
	...
	/* Build arguments for door call */
	if ((args = i_hp_set_args(HP_CMD_UNINSTALL, path, connection, flags,
	    NULL, 0)) == NULL)
		return (ENOMEM);
...
i_hp_pack_node(hp_node_t node, char **bufp, size_t *lenp)
{
	...
	if ((node->hp_driver) &&
	    ((rv = nvlist_add_string(nvl, HP_INFO_DRIVER,
	    node->hp_driver)) != 0))
		goto fail;

	if ((rv = nvlist_add_uint32(nvl, HP_INFO_INSTANCE,
	    (uint32_t)node->hp_instance)) != 0)
		goto fail;
	...
	if ((node->hp_state_priv) &&
	    ((rv = nvlist_add_string(nvl, HP_INFO_STATE_PRIV,
...
i_hp_unpack_node(char *buf, size_t len, hp_node_t parent, hp_node_t *retp)
{
	...
	*retp = NULL;
	...
	for (nvp = NULL; nvp = nvlist_next_nvpair(nvl, nvp); ) {
		...
		} else if ((strcmp(nvpair_name(nvp), HP_INFO_STATE_PRIV)
		    == 0) && (nvpair_type(nvp) == DATA_TYPE_STRING)) {

			(void) nvpair_value_string(nvp, &val_string);
			if ((node->hp_state_priv = strdup(val_string))
			    == NULL) {
				rv = ENOMEM;
				break;
			}
		...
		} else if ((strcmp(nvpair_name(nvp), HP_INFO_DRIVER) == 0) &&
		    (nvpair_type(nvp) == DATA_TYPE_STRING)) {

			(void) nvpair_value_string(nvp, &val_string);
			if ((node->hp_driver = strdup(val_string)) == NULL) {
				rv = ENOMEM;
				break;
			}

		} else if ((strcmp(nvpair_name(nvp), HP_INFO_INSTANCE) == 0) &&
		    (nvpair_type(nvp) == DATA_TYPE_UINT32)) {

			(void) nvpair_value_uint32(nvp, &val_uint32);
			node->hp_instance = val_uint32;

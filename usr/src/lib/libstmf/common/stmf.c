...
#define	MGMT_URL_SIZE	1024
#define	SBD_PROPS_SIZE	MAXPATHLEN + MAX_SERIAL_SIZE + MAX_LU_ALIAS_SIZE + \
    MGMT_URL_SIZE
	...
	switch (stmfError) {
		...
		case SBD_RET_ACCESS_STATE_FAILED:
			*ret = STMF_ERROR_ACCESS_STATE_SET;
			break;
		case SBD_RET_WRITE_PROTECT_SET_FAILED:
			*ret = STMF_ERROR_WRITE_PROTECT_SET;
			break;
		default:
			*ret = STMF_STATUS_ERROR;
			break;
...
getDiskAllProps(stmfGuid *luGuid, luResource *hdl)
{
	...
	int ioctlRet;
	int savedErrno;
	int sbdPropsSize = sizeof (*sbdProps) + SBD_PROPS_SIZE;
	stmf_iocdata_t sbdIoctl = {0};
...
loadDiskPropsFromDriver(luResourceImpl *hdl, sbd_lu_props_t *sbdProps)
{
	...
	if (sbdProps->slp_mgmt_url_valid) {
		...
		diskLu->luMgmtUrl = strdup(
		    (char *)&(sbdProps->slp_buf[sbdProps->slp_mgmt_url_off]));
		if (diskLu->luMgmtUrl == NULL)
			return (STMF_ERROR_NOMEM);
	}
...
setDiskProp(luResourceImpl *hdl, uint32_t resourceProp, const char *propVal)
{
	...
	switch (resourceProp) {
		...
		case STMF_LU_PROP_META_FILENAME:
			...
			break;
		case STMF_LU_PROP_MGMT_URL:
			diskLu->luMgmtUrl = strdup(propVal);
			if (diskLu->luMgmtUrl == NULL)
				return (STMF_ERROR_NOMEM);
			diskLu->luMgmtUrlValid = B_TRUE;
			break;
...
stmfFreeLuResource(luResource hdl)
{
	...
	if (hdlImpl->type == STMF_DISK) {
		diskResource *dr = (diskResource *)hdlImpl->resource;
		if (dr->luMgmtUrlValid && dr->luMgmtUrl) {
			free(dr->luMgmtUrl);
		}
	}
...
/*
 * stmfCheckTargetGroupInUse
 *
 * Purpose: check whether a given target group is in use
 *
 * groupName - Name of the target group for which we need to check
 * if it's in use
 *
 * inUse - B_TRUE - if group in use, B_FALSE otherwise
 *
 */

int
stmfCheckTargetGroupInUse(stmfGroupName *groupName, boolean_t *inUse)
{
	int fd;
	int ret = STMF_STATUS_SUCCESS;
	int ioctlRet;
	...
	/*
	 * Issue ioctl to check if group in use
	 */
	stmfIoctl.stmf_version = STMF_VERSION_1;
	stmfIoctl.stmf_ibuf_size = sizeof (iGroupName);
	stmfIoctl.stmf_ibuf = (uint64_t)(unsigned long)&iGroupName;
	stmfIoctl.stmf_obuf_size = sizeof (boolean_t);
	stmfIoctl.stmf_obuf = (uint64_t)(unsigned long)inUse;
	ioctlRet = ioctl(fd, STMF_IOCTL_CHECK_TARGET_GROUP_INUSE, &stmfIoctl);
	if (ioctlRet != 0) {
		switch (errno) {
			case EPERM:
			case EACCES:
...
/*
 * stmfCheckHostGroupInUse
 *
 * Purpose: check whether a given  host group name is in use
 *
 * groupName - Name of the host group for which we need to check if its in use
 *
 * inUse - B_TRUE - if group in use, B_FALSE otherwise
 *
 */
...
int
stmfCheckHostGroupInUse(stmfGroupName *groupName, boolean_t *inUse)
{
	int fd;
	int ret = STMF_STATUS_SUCCESS;
	int ioctlRet;
	...
	stmfIoctl.stmf_obuf_size = sizeof (boolean_t);
	stmfIoctl.stmf_obuf = (uint64_t)(unsigned long)inUse;
	ioctlRet = ioctl(fd, STMF_IOCTL_CHECK_HOST_GROUP_INUSE, &stmfIoctl);
	if (ioctlRet != 0) {
		switch (errno) {
			case EPERM:
			case EACCES:

...
{
	if (extbuf && (((int)extlen < 0) || (extlen % 8))) {
		return (-1);
	}
...
{
	(void) memcpy((uint8_t *)databuf + offset, val, vallen);
...
{
	(void) memcpy(val, (uint8_t *)databuf + offset, vallen);
	return (offset + vallen);
}

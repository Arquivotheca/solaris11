                         ...
                         char *password)
{
    ...
    /*
     * Solaris Kerberos: Make sure we have the latest active mkey.
     * First, update the global master_keylist in case a new mkey has been
     * added.
     */
    ret = krb5_db_fetch_mkey_list(handle->context, master_princ,
                                  &master_keyblock, &master_keylist);
    if (ret) {
        krb5_db_free_principal(handle->context, &kdb, 1);
        if (mask & KADM5_POLICY)
            (void) kadm5_free_policy_ent(handle->lhandle, &polent);
    ...
    /*
     * Then update the active mkey list in case the new key has been activated
     */
    ...
    /* Solaris Kerberos: Make sure we have the latest active mkey END */
                         ...
                         char *password)
{
    ...
    if (hist_added == 1) {
        kdb.mask = KADM5_KEY_DATA | KADM5_ATTRIBUTES | KADM5_KEY_HIST |
            KADM5_FAIL_AUTH_COUNT /* | KADM5_CPW_FUNCTION */;
    } else

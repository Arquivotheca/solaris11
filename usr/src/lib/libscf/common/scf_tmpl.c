...
/*
 * Message format:
 *     Conflicting value: FMRI="svc:/system/postrun:default"; Name of
 *   conflicting property="foo/bar"; Value="X" from
 *   file="/etc/svc/profile/site/a.xml"; Value="Y" from
 *   file="/etc/svc/profile/site/b.xml"
 *
 * Can set error to:
 *    SCF_ERROR_CONNECTION_BROKEN
 *    SCF_ERROR_DELETED
 ...
 */
int
smf_pg_print_conflicts(scf_propertygroup_t *pg, const char *fmri)
{
	scf_handle_t *h = pg->rd_d.rd_handle;
	scf_iter_t *iter, *dec_iter;
	scf_property_t *prop;
	scf_decoration_t *dec;
	char bundle[MAXPATHLEN];
	int namelen;
	int ret = -1;
	...
	if (iter == NULL || dec_iter == NULL || prop == NULL || dec == NULL)
		goto pg_conflicts_cleanup;

	namelen = scf_limit(SCF_LIMIT_MAX_NAME_LENGTH) + 1;
	assert(namelen > 0);
	if (((pg_name = malloc(namelen)) == NULL) ||
	    ((prop_name = malloc(namelen)) == NULL)) {
		(void) scf_set_error(SCF_ERROR_NO_MEMORY);
		goto pg_conflicts_cleanup;
	}

	if (scf_pg_get_name(pg, pg_name, namelen) < 0) {
		assert(scf_error() != SCF_ERROR_NOT_SET);
		goto pg_conflicts_cleanup;
	}
	...
	if (scf_pg_in_conflict(pg)) {
		(void) printf("%s: FMRI=\"%s\";",
		    dgettext(TEXT_DOMAIN, "Conflicting property group"), fmri);

		(void) printf(" %s=\"%s\";", dgettext(TEXT_DOMAIN,
		    "Name of conflicting property group"), pg_name);

		if (scf_iter_pg_decorations(dec_iter, pg,
		...
		while (scf_iter_next_decoration(dec_iter, dec) > 0) {
			if (scf_decoration_get_bundle(dec, bundle,
			    MAXPATHLEN) < 0) {
				assert(scf_error() !=
				    SCF_ERROR_HANDLE_MISMATCH);
				assert(scf_error() != SCF_ERROR_NOT_SET);
			...
			(void) printf(" %s=\"%s\";",
			    dgettext(TEXT_DOMAIN, "from file"), bundle);
		}

		(void) printf("\n");
	}

	scf_iter_reset(dec_iter);
	...
	while (scf_iter_next_property(iter, prop) > 0) {
		...
		if (scf_property_get_name(prop, prop_name, namelen) < 0) {
			assert(scf_error() != SCF_ERROR_NOT_SET);
			goto pg_conflicts_cleanup;
		}

		(void) printf("%s: FMRI=\"%s\";",
		    dgettext(TEXT_DOMAIN, "Conflicting value"), fmri);

		(void) printf(" %s=\"%s/%s\";",
		    dgettext(TEXT_DOMAIN, "Name of conflicting property"),
		    pg_name, prop_name);

		if (scf_iter_property_decorations(dec_iter, prop,
		    SCF_ITER_CONFLICTS_ONLY) < 0) {
			assert(scf_error() != SCF_ERROR_HANDLE_MISMATCH);
			assert(scf_error() != SCF_ERROR_NOT_SET);
			(void) printf("\n");
 ...
 */
int
smf_inst_print_conflicts(scf_instance_t *inst)
{
	...
	while (scf_iter_next_pg(iter, pg) > 0) {
		if (smf_pg_print_conflicts(pg, fmri_buf) == -1)
			goto inst_conflicts_cleanup;
	}
	...
	if (scf_instance_to_fmri(inst, fmri_buf, fmri_sz) == -1) {
		assert(scf_error() != SCF_ERROR_NOT_SET);
		goto inst_conflicts_cleanup;
	}

...
#define	SMF_SPEEDY_FILES_PATH		_PATH_SYSVOL "/"
    ...
    scf_handle_t *h)
{
	...
	for (numvals = 0; (iterret = scf_iter_next_value(iter, val)) == 1;
		...
		case SCF_TYPE_OPAQUE:
			...
			if (scf_value_get_opaque(val, NULL, 0) == -1)
				goto error1;
			if ((vallist[numvals].pv_opaque.o_value =
			    malloc(val->value_size)) == NULL) {
				(void) scf_set_error(SCF_ERROR_NO_MEMORY);
				goto error1;
			}
			vallist[numvals].pv_opaque.o_size = val->value_size;
			if (scf_value_get_opaque(val,
			    vallist[numvals].pv_opaque.o_value,
			    vallist[numvals].pv_opaque.o_size) == -1) {
 ...
 */
static void
scf_clean_propvec_mval(scf_propvec_t *prop)
{
	scf_propvec_mval_t	*pv_ptr_node = prop->pv_ptr;
	scf_propvec_mval_t	*pv_ptr_node_prev;
	...
	while (pv_ptr_node != NULL) {
		if (prop->pv_type == SCF_TYPE_OPAQUE) {
			scf_opaque_t *o = pv_ptr_node->pv_ptr;
			free(o->so_addr);
		}
		free(pv_ptr_node->pv_ptr);
		pv_ptr_node_prev = pv_ptr_node;
		pv_ptr_node = pv_ptr_node->next;
		free(pv_ptr_node_prev);
	}
}
 ...
 */
static int
scf_read_propvec_mval(scf_handle_t *h, scf_propvec_t *prop,
    scf_property_t *p)
{
	...
	scf_propvec_mval_t	*pv_ptr_ll_end = NULL;

	if ((pi = scf_iter_create(h)) == NULL ||
	    (piv = scf_value_create(h)) == NULL) {
		scf_iter_destroy(pi);
		return (SCF_FAILED);
	}
	...
	while (scf_iter_next_value(pi, piv) == 1) {
		int ret = 0;

		if ((pv_ptr_node = calloc(1, sizeof (*pv_ptr_node))) == NULL) {
			error = SCF_ERROR_NO_MEMORY;
			goto out;
		}
		if (pv_ptr_ll_end != NULL) {
			pv_ptr_ll_end->next = pv_ptr_node;
		} else {
			prop->pv_ptr = pv_ptr_node;
			pv_ptr_ll_end = pv_ptr_node;
		}
		...
		case SCF_TYPE_BOOLEAN: {
			...
			if ((ret = scf_value_get_boolean(piv, &b)) == -1) {
				break;
			}

			pv_ptr_node->pv_ptr = malloc(sizeof (boolean_t));
			if (pv_ptr_node->pv_ptr == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			*(boolean_t *)pv_ptr_node->pv_ptr =
			    b ? B_TRUE : B_FALSE;
			break;
		}
		case SCF_TYPE_COUNT:
			pv_ptr_node->pv_ptr = malloc(sizeof (uint64_t));
			if (pv_ptr_node->pv_ptr == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			ret = scf_value_get_count(piv, pv_ptr_node->pv_ptr);
			break;
		case SCF_TYPE_INTEGER:
			pv_ptr_node->pv_ptr = malloc(sizeof (int64_t));
			if (pv_ptr_node->pv_ptr == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			ret = scf_value_get_integer(piv, pv_ptr_node->pv_ptr);
			break;
		case SCF_TYPE_TIME: {
			pv_ptr_node->pv_ptr = malloc(sizeof (scf_time_t));
			if (pv_ptr_node->pv_ptr == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			ret = scf_value_get_time(piv,
			    &((scf_time_t *)pv_ptr_node->pv_ptr)->t_seconds,
			    &((scf_time_t *)pv_ptr_node->pv_ptr)->t_ns);
			break;
		}
		case SCF_TYPE_OPAQUE: {
			scf_opaque_t *opaque;

			if (scf_value_get_opaque(piv, NULL, 0) == -1) {
				error = scf_error();
				goto out;
			}

			pv_ptr_node->pv_ptr = malloc(sizeof (scf_opaque_t));
			if (pv_ptr_node->pv_ptr == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			opaque = pv_ptr_node->pv_ptr;

			opaque->so_addr = malloc(piv->value_size);
			if (opaque->so_addr == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			opaque->so_size = piv->value_size;
			ret = scf_value_get_opaque(piv, opaque->so_addr,
			    opaque->so_size);
			break;
		}
		default: {
			...
			size = scf_value_get_astring(piv, NULL, 0);
			if (size == -1) {
				error = scf_error();
				goto out;
			}
			if ((pv_ptr_node->pv_ptr = malloc(++size)) == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				goto out;
			}
			ret = scf_value_get_astring(piv,
			    pv_ptr_node->pv_ptr, size);
			break;
		}
    ...
    scf_propvec_t *properties, scf_propvec_t **badprop)
{
	...
	for (prop = properties; prop->pv_prop != NULL; prop++) {
		if (prop->pv_mval)
			prop->pv_ptr = NULL;
		else if (prop->pv_type == SCF_TYPE_OPAQUE)
	...
	for (prop = properties; prop->pv_prop != NULL; prop++) {
		...
		if (prop->pv_mval) {
			if (scf_read_propvec_mval(h, prop, p) != SCF_SUCCESS) {
				*badprop = prop;
				goto scferror;
			}
			continue;
		...
		case SCF_TYPE_BOOLEAN: {
			...
			if ((ret = scf_value_get_boolean(v, &b)) == -1) {
				break;
			}
			if (prop->pv_aux != 0) {
		...
		case SCF_TYPE_OPAQUE: {
			...
			if (scf_value_get_opaque(v, NULL, 0) == -1) {
				*badprop = prop;
				goto scferror;
			}
			if ((opaque->so_addr = malloc(v->value_size)) == NULL) {
				error = SCF_ERROR_NO_MEMORY;
				scf_clean_propvec(properties);
				goto out;
			}
			opaque->so_size = v->value_size;
			ret = scf_value_get_opaque(v, opaque->so_addr,
			    opaque->so_size);
			break;
		}
		default: {
 ...
 */
int
scf_write_propvec_mval(scf_handle_t *h, scf_propvec_t *prop,
    scf_transaction_entry_t *e)
{
	...
	for (; pv_ptr_node != NULL;  pv_ptr_node = pv_ptr_node->next) {
		...
		switch (prop->pv_type) {
		case SCF_TYPE_BOOLEAN:
			scf_value_set_boolean(v,
			    *(boolean_t *)pv_ptr_node->pv_ptr ? 1 : 0);
			break;
		case SCF_TYPE_COUNT:
			scf_value_set_count(v,
			    *(uint64_t *)pv_ptr_node->pv_ptr);
			break;
		case SCF_TYPE_INTEGER:
			scf_value_set_integer(v,
			    *(int64_t *)pv_ptr_node->pv_ptr);
			break;
		case SCF_TYPE_TIME: {
		...
		case SCF_TYPE_OPAQUE: {
			ret = scf_value_set_opaque(v,
			    ((scf_opaque_t *)pv_ptr_node->pv_ptr)->so_addr,
			    ((scf_opaque_t *)pv_ptr_node->pv_ptr)->so_size);
			break;
		}
		case SCF_TYPE_ASTRING:
			...
			break;
		default:
			ret = scf_value_set_from_string(v, prop->pv_type,
			    (const char *)pv_ptr_node->pv_ptr);
		}

		if (ret == -1 || scf_entry_add_value(e, v) == -1) {
			scf_value_destroy(v);
			return (SCF_FAILED);
		}
	}
...
top:
	...
	for (prop = properties, i = 0; prop->pv_prop != NULL; prop++, i++) {
		...
		ret = scf_transaction_property_change(tx, e, prop->pv_prop,
		    prop->pv_type);
		if (ret == -1 && scf_error() == SCF_ERROR_NOT_FOUND)
			ret = scf_transaction_property_new(tx, e,
		...
		if (prop->pv_mval) {
			if (scf_write_propvec_mval(h, prop, e) != SCF_SUCCESS) {
				*badprop = prop;
				goto scferror;
			}
			continue;
		...
		case SCF_TYPE_COUNT:
			scf_value_set_count(v, *(uint64_t *)prop->pv_ptr);
			break;
		case SCF_TYPE_INTEGER:
			scf_value_set_integer(v, *(int64_t *)prop->pv_ptr);
			break;
		case SCF_TYPE_TIME: {
			scf_time_t *time = prop->pv_ptr;

			ret = scf_value_set_time(v, time->t_seconds,
			    time->t_ns);
			break;
		}
		case SCF_TYPE_OPAQUE: {
			...
			ret = scf_value_set_opaque(v, opaque->so_addr,
			    opaque->so_size);
			break;
		}
		case SCF_TYPE_ASTRING:

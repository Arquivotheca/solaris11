...
static int datael_get_child(const scf_datael_t *, const char *, uint32_t,
    scf_datael_t *, boolean_t);
...
static const struct {
	...
	rep_protocol_decoration_layer_t li_proto_layer;
	const char *li_name;
	const char *li_rep_name;
} scf_layer_info[] = {
	{SCF_DECORATION_MANIFEST, REP_PROTOCOL_DEC_MANIFEST,
	    SCF_LAYER_MANIFEST, REP_PROTOCOL_DECORATION_MANIFEST},
	{SCF_DECORATION_SYSTEM_PROFILE, REP_PROTOCOL_DEC_SYSTEM_PROFILE,
	    SCF_LAYER_SYSTEM_PROFILE, REP_PROTOCOL_DECORATION_SYSTEM_PROFILE},
	{SCF_DECORATION_SITE_PROFILE, REP_PROTOCOL_DEC_SITE_PROFILE,
	    SCF_LAYER_SITE_PROFILE, REP_PROTOCOL_DECORATION_SITE_PROFILE},
	{SCF_DECORATION_ADMIN,	REP_PROTOCOL_DEC_ADMIN,
	    SCF_LAYER_ADMIN, REP_PROTOCOL_DECORATION_ADMIN},
	{SCF_DECORATION_TOP,	REP_PROTOCOL_DEC_TOP,
	    SCF_LAYER_TOP, REP_PROTOCOL_DECORATION_TOP}
};

#define	SCF_LAYER_INFO_COUNT (sizeof (scf_layer_info) / \
	    sizeof (*scf_layer_info))

static rep_protocol_decoration_layer_t
scf_layer_to_protocol_layer(scf_decoration_layer_t t)
{
	int i;

	for (i = 0; i < SCF_LAYER_INFO_COUNT; i++)
		if (scf_layer_info[i].li_layer == t)
			return (scf_layer_info[i].li_proto_layer);

	return (REP_PROTOCOL_DEC_INVALID);
}

static scf_decoration_layer_t
scf_protocol_layer_to_layer(rep_protocol_decoration_layer_t t)
{
	int i;

	for (i = 0; i < SCF_LAYER_INFO_COUNT; i++)
		if (scf_layer_info[i].li_proto_layer == t)
			return (scf_layer_info[i].li_layer);

	return (SCF_DECORATION_INVALID);
}

const char *
scf_decoration_layer_to_string(scf_decoration_layer_t li)
{
	int i;

	for (i = 0; i < SCF_LAYER_INFO_COUNT; i++)
		if (scf_layer_info[i].li_layer == li)
			return (scf_layer_info[i].li_name);

	return ("unknown");
}

scf_decoration_layer_t
scf_decoration_layer_from_string(const char *ln)
{
	int i;

	for (i = 0; i < SCF_LAYER_INFO_COUNT; i++)
		if (strcmp(scf_layer_info[i].li_name, ln) == 0)
			return (scf_layer_info[i].li_layer);

	if (strcmp(SCF_LAYER_ALL, ln) == 0)
		return (SCF_DECORATION_ALL);

	return (SCF_DECORATION_INVALID);
}

const char *
scf_decoration_layer_to_rep_string(scf_decoration_layer_t li)
{
	int i;

	for (i = 0; i < SCF_LAYER_INFO_COUNT; i++)
		if (scf_layer_info[i].li_layer == li)
			return (scf_layer_info[i].li_rep_name);

	return ("unknown");
}

static int
scf_get_decoration_common(const scf_datael_t *dp, scf_decoration_layer_t layer,
    scf_decoration_t *dec)
{
	const char *layer_name = scf_decoration_layer_to_rep_string(layer);

	return (datael_get_child(dp, layer_name,
	    REP_PROTOCOL_ENTITY_DECORATION, dec ? &dec->rd_d : NULL, 0));
}
...
proto_error(rep_protocol_responseid_t e)
{
	...
	case REP_PROTOCOL_CONFLICT:
		return (SCF_ERROR_CONFLICT);
...
scf_handle_create(scf_version_t v)
{
	...
	(void) scf_handle_decorate(ret, SCF_SHOW_MASKED, SCF_DECORATE_CLEAR);
...
{
	if (v != SCF_DECORATE_CLEAR && v != SCF_DECORATE_TRUE &&
	...
	if (strcmp(name, SCF_SHOW_MASKED) == 0) {
		if (v == SCF_DECORATE_CLEAR) {
			(void) pthread_mutex_lock(&handle->rh_lock);
			handle->rh_show_masked = 0;
			(void) pthread_mutex_unlock(&handle->rh_lock);
		} else {
			(void) pthread_mutex_lock(&handle->rh_lock);
			handle->rh_show_masked =
			    REPOSITORY_DOOR_FLAG_SHOW_MASKED;
			(void) pthread_mutex_unlock(&handle->rh_lock);
		}
		return (0);
	}
...
scf_handle_bind(scf_handle_t *handle)
{
	...
	request.rdr_flags = handle->rh_flags | handle->rh_show_masked;
    ...
    uint32_t type, scf_datael_t *out, scf_iter_t *iter)
{
	...
	if (response.rpr_response == REP_PROTOCOL_FAIL_TRUNCATED)
		return (scf_set_error(SCF_ERROR_CONSTRAINT_VIOLATED));
...
static int
datael_undelete(const scf_datael_t *dp)
{
	scf_handle_t *h = dp->rd_handle;
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	request.rpr_request = REP_PROTOCOL_ENTITY_UNDELETE;
	request.rpr_entityid = dp->rd_entity;
...
/*
 * Return 1 if the entity represented by dp is masked.  Return 0
 * if the entity is not masked.  Return -1 if an error is encountered and
 * sets SCF error to one of the following.
 *
 * Returns:
 *	_CONNECTION_BROKEN
 *	_DELETED
 ...
 */
static int
datael_is_masked(const scf_datael_t *dp)
{
	scf_handle_t *h;
	struct rep_protocol_masked_request request;
	struct rep_protocol_integer_response response;
	int r;

	request.rpr_request = REP_PROTOCOL_ENTITY_IS_MASKED;

	h = dp->rd_handle;
	(void) pthread_mutex_lock(&h->rh_lock);
	request.rpr_entityid = dp->rd_entity;
	datael_finish_reset(dp);
	r = make_door_call(h, &request, sizeof (request), &response,
	    sizeof (response));
	(void) pthread_mutex_unlock(&h->rh_lock);
    ...
    scf_datael_t *cp)
{
	...
	if (response.rpr_response != REP_PROTOCOL_SUCCESS) {
		/*
		 * If the child exists and is currently masked simply
		 * undelete the child removing the masking.
		 */
		if (response.rpr_response == REP_PROTOCOL_FAIL_EXISTS) {
			r = datael_get_child(dp, name, type, cp, 0);
			if (r != SCF_SUCCESS)
				return (r);

			if (datael_is_masked(cp))
				return (datael_undelete(cp));
		}
...
static int
datael_remove(const scf_datael_t *dp)
{
	scf_handle_t *h = dp->rd_handle;
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	request.rpr_request = REP_PROTOCOL_ENTITY_REMOVE;
	request.rpr_entityid = dp->rd_entity;
...
static int
datael_delcust(const scf_datael_t *dp)
{
	scf_handle_t *h = dp->rd_handle;
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	request.rpr_request = REP_PROTOCOL_ENTITY_DELCUST;
	request.rpr_entityid = dp->rd_entity;
 ...
 * _BACKEND_ACCESS, _IN_USE, _NOT_FOUND.
 */
static int
datael_remove_bundle(const scf_datael_t *dp, char *bname)
{
	scf_handle_t *h = dp->rd_handle;

	struct rep_protocol_bundle_remove request;
	struct rep_protocol_response response;
	ssize_t r;

	(void) pthread_mutex_lock(&h->rh_lock);
	if (dp->rd_type != REP_PROTOCOL_ENTITY_SERVICE &&
	    dp->rd_type != REP_PROTOCOL_ENTITY_INSTANCE) {
		(void) pthread_mutex_unlock(&h->rh_lock);
		return (scf_set_error(SCF_ERROR_INVALID_ARGUMENT));
	}

	request.rpr_request = REP_PROTOCOL_BUNDLE_REMOVE;
	request.rpr_entityid = dp->rd_entity;

	if (bname == NULL || strlcpy(request.rpr_bname, bname,
	    sizeof (request.rpr_bname)) >= sizeof (request.rpr_bname)) {
		(void) pthread_mutex_unlock(&h->rh_lock);
		return (scf_set_error(SCF_ERROR_INVALID_ARGUMENT));
	}

	/*
	 * If the type is a property group then there is a
	 * transaction that should have come along that a
	 * transaction request is going to be stored in
	 * with the bundle_remove request for a list of
	 * properties.
	 */
...
static int
datael_in_conflict(const scf_datael_t *dp)
{
	scf_handle_t *h = dp->rd_handle;

	struct rep_protocol_entity_request request;
	struct rep_protocol_integer_response response;
	ssize_t r;

	(void) pthread_mutex_lock(&h->rh_lock);
	request.rpr_request = REP_PROTOCOL_ENTITY_INCONFLICT;
	request.rpr_entityid = dp->rd_entity;
	response.rpr_value = 0;
...
{
	scf_handle_t *h = dp->rd_handle;
	boolean_t composed = (flags & SCF_ITER_COMPOSED);
	boolean_t conflicts = (flags & SCF_ITER_CONFLICTS_ONLY);
	...
	request.rpr_flags =
	    ...
	    (conflicts ? RP_ITER_START_CONFLICT : RP_ITER_START_ALL);
    ...
    const scf_snapshot_t *snap)
{
	...
	return (datael_setup_iter(iter, snap ? &snap->rd_d : &inst->rd_d,
	    REP_PROTOCOL_ENTITY_PROPERTYGRP, SCF_ITER_COMPOSED));
}
...
int
scf_iter_service_decorations(scf_iter_t *iter, const scf_service_t *svc,
    int flags)
{
	return (datael_setup_iter(iter, &svc->rd_d,
	    REP_PROTOCOL_ENTITY_DECORATION, flags));
}

int
scf_iter_instance_decorations(scf_iter_t *iter, const scf_instance_t *inst,
    int flags)
{
	return (datael_setup_iter(iter, &inst->rd_d,
	    REP_PROTOCOL_ENTITY_DECORATION, flags));
...
/* Property group decoration iterator */
int
scf_iter_pg_decorations(scf_iter_t *iter, const scf_propertygroup_t *pg,
    int flags)
{
	return (datael_setup_iter(iter, &pg->rd_d,
	    REP_PROTOCOL_ENTITY_DECORATION, flags));
...
/* Property decoration iterator */
int
scf_iter_property_decorations(scf_iter_t *iter, const scf_property_t *prop,
    int flags)
{
...
int
scf_iter_next_decoration(scf_iter_t *iter, scf_decoration_t *out)
{
	return (datael_iter_next(iter, &out->rd_d));
}
...
/*
 * These will basically remove an entity without consideration for muting.
 */
int
scf_service_remove(scf_service_t *svc)
{
	return (datael_remove(&svc->rd_d));
}

int
scf_instance_remove(scf_instance_t *inst)
{
	return (datael_remove(&inst->rd_d));
}

int
scf_pg_remove(scf_propertygroup_t *pg)
{
	return (datael_remove(&pg->rd_d));
}

int
scf_service_delcust(scf_service_t *svc)
{
	return (datael_delcust(&svc->rd_d));
}

int
scf_instance_delcust(scf_instance_t *inst)
{
	return (datael_delcust(&inst->rd_d));
}

int
scf_pg_delcust(scf_propertygroup_t *pg)
{
	return (datael_delcust(&pg->rd_d));
}

/*
 * Remove a bundle's presence from the repository.
 */
int
scf_service_bundle_remove(scf_service_t *svc, char *bname)
{
	return (datael_remove_bundle(&svc->rd_d, bname));
}
 ...
 */
int
scf_instance_bundle_remove(scf_instance_t *inst, char *bname)
{
	return (datael_remove_bundle(&inst->rd_d, bname));
}
    ...
    struct rep_protocol_transaction_cmd *out)
{
	...
	for (child = cur->entry_head; child != NULL;
		...
		assert(cur->entry_action != REP_PROTOCOL_TX_ENTRY_REMOVE);
		assert(cur->entry_action != REP_PROTOCOL_TX_ENTRY_UNDELETE);
		assert(cur->entry_action != REP_PROTOCOL_TX_ENTRY_DELCUST);
		if (out != NULL) {
...
static struct rep_protocol_transaction_commit *
prep_transaction(scf_transaction_t *tran, enum rep_protocol_requestid requestid,
    int *err)
{
	...
	if (tran->tran_state != TRAN_STATE_SETUP ||
	    tran->tran_invalid) {
		(void) pthread_mutex_unlock(&h->rh_lock);
		*err = scf_set_error(SCF_ERROR_INVALID_ARGUMENT);
		return (NULL);
	}
	...
	for (cur = uu_list_first(tran->tran_props); cur != NULL;
		...
		if (size == BAD_SIZE) {
			(void) pthread_mutex_unlock(&h->rh_lock);
			*err = scf_set_error(SCF_ERROR_INTERNAL);
			return (NULL);
		}
	...
	request->rpr_request = requestid;
	request->rpr_entityid = tran->tran_pg.rd_d.rd_entity;
	request->rpr_size = request_size;
	request->rpr_pg_action = tran->tran_pg_action;
	cmd = (uintptr_t)request->rpr_cmd;

	if (tran->tran_pg_type) {
		if (strlcpy(request->rpr_pg_type, tran->tran_pg_type,
		    sizeof (request->rpr_pg_type)) >
		    sizeof (request->rpr_pg_type)) {
			(void) pthread_mutex_unlock(&h->rh_lock);
			*err = scf_set_error(SCF_ERROR_INVALID_ARGUMENT);
			uu_free(request);
		...
		(void) strcpy(request->rpr_pg_type, tran->tran_pg_type);
		request->rpr_pg_flags = tran->tran_pg_flags;
	}
...
int
scf_prop_bundle_remove(scf_transaction_t *tran, char *bname)
{
	scf_handle_t *h = tran->tran_pg.rd_d.rd_handle;
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	request = prep_transaction(tran, REP_PROTOCOL_PROP_BUNDLE_REMOVE, &r);

	if (request == NULL)
		return (r);

	request_size = request->rpr_size;
...
/*
 * Undelete an entity, that has previously been masked.
 */
int
scf_service_undelete(scf_service_t *svc)
{
	return (datael_undelete(&svc->rd_d));
}

int
scf_instance_undelete(scf_instance_t *inst)
{
	return (datael_undelete(&inst->rd_d));
}

int
scf_pg_undelete(scf_propertygroup_t *pg)
{
	return (datael_undelete(&pg->rd_d));
}
...
int
scf_service_get_decoration(const scf_service_t *svc,
    scf_decoration_layer_t layer, scf_decoration_t *dec)
{
	return (scf_get_decoration_common(&svc->rd_d, layer, dec));
}
...
int
scf_instance_get_decoration(const scf_instance_t *inst,
    scf_decoration_layer_t layer, scf_decoration_t *dec)
{
	return (scf_get_decoration_common(&inst->rd_d, layer, dec));
}
...
int
scf_pg_get_decoration(const scf_propertygroup_t *pg,
    scf_decoration_layer_t layer, scf_decoration_t *dec)
{
	return (scf_get_decoration_common(&pg->rd_d, layer, dec));
}
...
static int
decoration_type_locked(const scf_decoration_t *dec,
    rep_protocol_value_type_t *out)
{
	scf_handle_t *h = dec->rd_d.rd_handle;
	...
	request.rpr_request = REP_PROTOCOL_DECORATION_GET_TYPE;
	request.rpr_entityid = dec->rd_d.rd_entity;

	datael_finish_reset(&dec->rd_d);
	r = make_door_call(h, &request, sizeof (request),
	    &response, sizeof (response));
...
int
scf_decoration_type(const scf_decoration_t *dec, scf_type_t *out)
{
	scf_handle_t *h = dec->rd_d.rd_handle;
	rep_protocol_value_type_t out_raw;
	int ret;
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	ret = decoration_type_locked(dec, &out_raw);
	(void) pthread_mutex_unlock(&h->rh_lock);
...
int
scf_decoration_is_type(const scf_decoration_t *dec, scf_type_t base_arg)
{
	scf_handle_t *h = dec->rd_d.rd_handle;
	rep_protocol_value_type_t base = scf_type_to_protocol_type(base_arg);
	rep_protocol_value_type_t type;
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	ret = decoration_type_locked(dec, &type);
	(void) pthread_mutex_unlock(&h->rh_lock);
...
int
scf_instance_is_complete(scf_instance_t *inst)
{
	scf_propertygroup_t *pg;
	scf_property_t *prop;
	scf_handle_t *h;
	...
	if (scf_pg_get_property(pg, SCF_PROPERTY_COMPLETE, prop) != 0) {
		scf_pg_destroy(pg);
		scf_property_destroy(prop);
...
static int
decoration_layer_locked(const scf_decoration_t *prop,
    rep_protocol_decoration_layer_t *out)
{
	scf_handle_t *h = prop->rd_d.rd_handle;

	struct rep_protocol_decoration_request request;
	struct rep_protocol_integer_response response;
	...
	request.rpr_request = REP_PROTOCOL_DECORATION_GET_LAYER;
	request.rpr_entityid = prop->rd_d.rd_entity;
 ...
 */
ssize_t
scf_decoration_get_bundle(const scf_decoration_t *dec, char *out, size_t len)
{
	scf_handle_t *h;
	struct rep_protocol_decoration_request request;
	struct rep_protocol_path_response response;
	ssize_t r;

	h = dec->rd_d.rd_handle;
	request.rpr_request = REP_PROTOCOL_DECORATION_GET_BUNDLE;
	...
	return (strlcpy(out, response.rpr_path, len));
}
 ...
 */
int
scf_decoration_get_layer(const scf_decoration_t *dec,
    scf_decoration_layer_t *out)
{
	scf_handle_t *h = dec->rd_d.rd_handle;
	rep_protocol_decoration_layer_t out_raw;
	int ret;

	(void) pthread_mutex_lock(&h->rh_lock);
	ret = decoration_layer_locked(dec, &out_raw);
	(void) pthread_mutex_unlock(&h->rh_lock);

	if (ret == SCF_SUCCESS)
		*out = scf_protocol_layer_to_layer(out_raw);
    ...
    const char *prop, rep_protocol_value_type_t type)
{
	...
	if (action == REP_PROTOCOL_TX_ENTRY_DELETE ||
	    action == REP_PROTOCOL_TX_ENTRY_UNDELETE ||
	    action == REP_PROTOCOL_TX_ENTRY_REMOVE ||
	    action == REP_PROTOCOL_TX_ENTRY_DELCUST)
 ...
 */
int
scf_transaction_pg_bundle_remove(scf_transaction_t *tran)
{
	scf_handle_t *h = tran->tran_pg.rd_d.rd_handle;
	scf_error_t error = SCF_ERROR_NONE;

	(void) HANDLE_HOLD_PG(h);
	...
	tran->tran_pg_action = REP_PROTOCOL_BUNDLE_REMOVE;
 ...
 */
int
scf_transaction_pg_modify(scf_transaction_t *tran, const char *type,
    uint32_t flags)
{
	scf_handle_t *h = tran->tran_pg.rd_d.rd_handle;
	scf_error_t error = SCF_ERROR_NONE;
	...
	if ((tran->tran_pg_type = strdup(type)) == NULL) {
		error = SCF_ERROR_NO_MEMORY;
		goto error;
	}

	tran->tran_pg_flags = flags;
...
int
scf_transaction_property_remove(scf_transaction_t *tx,
    scf_transaction_entry_t *entry, const char *prop)
{
	return (transaction_add(tx, entry, REP_PROTOCOL_TX_ENTRY_REMOVE,
	    prop, REP_PROTOCOL_TYPE_INVALID));
}

int
scf_transaction_property_undelete(scf_transaction_t *tx,
    scf_transaction_entry_t *entry, const char *prop)
{
	return (transaction_add(tx, entry, REP_PROTOCOL_TX_ENTRY_UNDELETE,
	    prop, REP_PROTOCOL_TYPE_INVALID));
}

int
scf_transaction_property_delcust(scf_transaction_t *tx,
    scf_transaction_entry_t *entry, const char *prop)
{
	return (transaction_add(tx, entry, REP_PROTOCOL_TX_ENTRY_DELCUST,
	    prop, REP_PROTOCOL_TYPE_INVALID));
}
...
scf_transaction_commit(scf_transaction_t *tran)
{
	...
	(void) pthread_mutex_lock(&h->rh_lock);
	request = prep_transaction(tran, REP_PROTOCOL_PROPERTYGRP_TX_COMMIT,
	    &r);
...
transaction_reset(scf_transaction_t *tran)
{
	...
	free(tran->tran_pg_type);
	tran->tran_pg_type = NULL;
	tran->tran_pg_flags = 0;
	tran->tran_pg_action = 0;
...
scf_entry_add_value(scf_transaction_entry_t *entry, scf_value_t *v)
{
	...
	if (entry->entry_action == REP_PROTOCOL_TX_ENTRY_DELETE ||
	    entry->entry_action == REP_PROTOCOL_TX_ENTRY_REMOVE ||
	    entry->entry_action == REP_PROTOCOL_TX_ENTRY_UNDELETE ||
	    entry->entry_action == REP_PROTOCOL_TX_ENTRY_DELCUST) {
		(void) pthread_mutex_unlock(&h->rh_lock);
		return (scf_set_error(SCF_ERROR_INVALID_ARGUMENT));
	}
 ...
 */
scf_decoration_t *
scf_decoration_create(scf_handle_t *handle)
{
	scf_decoration_t *ret;

	ret = uu_zalloc(sizeof (*ret));
	if (ret != NULL) {
		if (datael_init(&ret->rd_d, handle,
		    REP_PROTOCOL_ENTITY_DECORATION) == -1) {
			uu_free(ret);
			return (NULL);
		}
	} else {
...
scf_handle_t *
scf_decoration_handle(const scf_decoration_t *val)
{
	return (datael_handle(&val->rd_d));
}

void
scf_decoration_destroy(scf_decoration_t *val)
{
	if (val == NULL)
		return;
...
/*
 * Decoration value iterator.
 */
int
scf_iter_decoration_values(scf_iter_t *iter, const scf_decoration_t *dec)
{
	return (datael_setup_iter(iter, &dec->rd_d,
...
int
scf_property_get_decoration(const scf_property_t *prop,
    scf_decoration_layer_t layer, scf_decoration_t *dec)
{
	return (scf_get_decoration_common(&prop->rd_d, layer, dec));
}

/*
 * Get scf_value_t for the given property decoration.
 */
/* ARGSUSED */
int
scf_decoration_get_value(const scf_decoration_t *dec, scf_value_t *v)
{
	scf_handle_t *h = dec->rd_d.rd_handle;
	struct rep_protocol_property_request request;
	struct rep_protocol_value_response response;
	...
	request.rpr_request = REP_PROTOCOL_DECORATION_GET_VALUE;
	request.rpr_entityid = dec->rd_d.rd_entity;
...
int
scf_property_get_value_at_layer(const scf_property_t *prop,
    scf_decoration_layer_t layer, scf_value_t *v)
{
	int r;
	scf_handle_t *h = prop->rd_d.rd_handle;
	scf_decoration_t *dec = scf_decoration_create(h);

	if (dec == NULL)
		return (scf_error());

	if ((r = scf_property_get_decoration(prop, layer, dec)))
		return (r);

	r = scf_decoration_get_value(dec, v);
...
nolegacy:
	...
	for (i = 0; i < argc; i++) {
		...
		if (pattern[i].sp_matchcount == 0) {
			...
			/*
			 * If the empty flag is present, just continue
			 */
			if (flags & SCF_WALK_EMPTY)
				continue;
 ...
 */
int
_scf_set_annotation(scf_handle_t *h)
{
	struct rep_protocol_request request;
	struct rep_protocol_response response;
	int r;
...
int
_scf_unset_annotation(scf_handle_t *h)
{
	struct rep_protocol_request request;
	struct rep_protocol_response response;
	int r;
	...
	request.rpr_request = REP_PROTOCOL_UNSET_AUDIT_ANNOTATION;
...
int
_scf_set_tx_decoration(scf_handle_t *h, const char *file,
    scf_decoration_layer_t layer_id)
{
	struct rep_protocol_decoration request;
	struct rep_protocol_response response;
	size_t copied;
	int r;
	...
	request.rpr_request = REP_PROTOCOL_SET_TX_DECORATION;
	request.rpr_layer_id = scf_layer_to_protocol_layer(layer_id);

	copied = strlcpy(request.rpr_file,
	    (file == NULL) ? "" : file, sizeof (request.rpr_file));
...
int
_scf_unset_tx_decoration(scf_handle_t *h)
{
	return (_scf_set_tx_decoration(h, NULL, SCF_DECORATION_ADMIN));
}
 ...
 */
int
scf_service_in_conflict(scf_service_t *svc)
{
	return (datael_in_conflict(&svc->rd_d));
}

int
scf_instance_in_conflict(scf_instance_t *inst)
{
	return (datael_in_conflict(&inst->rd_d));
}

int
scf_pg_in_conflict(scf_propertygroup_t *pg)
{
	return (datael_in_conflict(&pg->rd_d));
}

int
scf_property_in_conflict(scf_property_t *prop)
{
	return (datael_in_conflict(&prop->rd_d));
}

int
scf_instance_in_conflict_composed(scf_instance_t *inst, scf_service_t *svc)
{
	int r;

	if ((r = scf_instance_in_conflict(inst)))
		return (r);

	return (scf_service_in_conflict(svc));
}

int
scf_service_is_masked(const scf_service_t *svc)
{
	return (datael_is_masked(&svc->rd_d));
}

int
scf_instance_is_masked(const scf_instance_t *inst)
{
	return (datael_is_masked(&inst->rd_d));
}

int
scf_pg_is_masked(const scf_propertygroup_t *pg)
{
	return (datael_is_masked(&pg->rd_d));
}

int
scf_property_is_masked(const scf_property_t *prop)
{
	return (datael_is_masked(&prop->rd_d));
}

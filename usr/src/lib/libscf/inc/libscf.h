...
#include <paths.h>
#else
#define	_PATH_SYSVOL	"/system/volatile"
#endif
#include <libnvpair.h>
...
typedef struct scf_decoration scf_decoration_t;
 ...
 * Decoration layers
 *
 * The MANIFEST, SYSTEM_PROFILE, SITE_PROFILE, and ADMIN layers correspond
 * to repository layers.
 *
 * SCF_DECORATION_TOP is not an actual layer but it points to the highest
 * available layer.
 *
 * SCF_DECORATION_ALL is also not an actual layer but references all layers.
 */
typedef enum {
	SCF_DECORATION_INVALID		= 0,
	SCF_DECORATION_MANIFEST		= 10000,
	SCF_DECORATION_SYSTEM_PROFILE	= 20000,
	SCF_DECORATION_SITE_PROFILE	= 30000,
	SCF_DECORATION_ADMIN		= 40000,
	SCF_DECORATION_TOP		= 500000,
	SCF_DECORATION_ALL		= 550000
...
typedef enum scf_error {
	...
	SCF_ERROR_CONFLICT,		/* There is a conflict in the entity */
...
#define	SCF_INSTANCE_ENV	((const char *) \
...
#define	SCF_MILESTONE_UNCONFIGURE \
	((const char *) "svc:/milestone/unconfig:default")
 ...
 * Layer names.
 *
 * SCF_LAYER_TOP and SCF_LAYER_ALL are specifically for svccfg listprop -l
 */
#define	SCF_LAYER_MANIFEST		((const char *)"manifest")
#define	SCF_LAYER_SYSTEM_PROFILE	((const char *)"system-profile")
#define	SCF_LAYER_SITE_PROFILE		((const char *)"site-profile")
#define	SCF_LAYER_ADMIN			((const char *)"admin")
#define	SCF_LAYER_TOP			((const char *)"current")
#define	SCF_LAYER_ALL			((const char *)"all")
...
#define	SCF_PROPERTY_COMPLETE		((const char *)"complete")
...
/*
 * An scf_handle flag that enables libscf consumers to retrieve
 * masked entities.
 */
#define	SCF_SHOW_MASKED		((const char *)"show-masked")
#define	SCF_DECORATE_TRUE	((scf_value_t *)1)
...
int scf_iter_service_decorations(scf_iter_t *, const scf_service_t *, int);
...
int scf_iter_instance_decorations(scf_iter_t *, const scf_instance_t *, int);
...
int scf_iter_pg_decorations(scf_iter_t *, const scf_propertygroup_t *, int);
int scf_iter_property_decorations(scf_iter_t *, const scf_property_t *, int);
int scf_iter_property_values(scf_iter_t *, const scf_property_t *);
int scf_iter_decoration_values(scf_iter_t *, const scf_decoration_t *);
...
int scf_iter_next_decoration(scf_iter_t *, scf_decoration_t *);
...
int scf_service_get_decoration(const scf_service_t *, scf_decoration_layer_t,
    scf_decoration_t *);
...
int scf_service_undelete(scf_service_t *);
int scf_service_delcust(scf_service_t *);
...
int scf_instance_get_decoration(const scf_instance_t *, scf_decoration_layer_t,
    scf_decoration_t *);
...
int scf_service_get_instance(const scf_service_t *, const char *,
    scf_instance_t *);
int scf_instance_is_complete(scf_instance_t *);
int scf_service_add_instance(const scf_service_t *, const char *,
    scf_instance_t *);
int scf_instance_delete(scf_instance_t *);
int scf_instance_undelete(scf_instance_t *);
int scf_instance_delcust(scf_instance_t *);
...
int scf_pg_get_decoration(const scf_propertygroup_t *, scf_decoration_layer_t,
    scf_decoration_t *);
...
int scf_pg_undelete(scf_propertygroup_t *);
int scf_pg_delcust(scf_propertygroup_t *);
...
int scf_property_get_value_at_layer(const scf_property_t *,
    scf_decoration_layer_t layer, scf_value_t *);
...
scf_decoration_t *scf_decoration_create(scf_handle_t *);
scf_handle_t *scf_decoration_handle(const scf_decoration_t *);
void scf_decoration_destroy(scf_decoration_t *);
int scf_decoration_is_type(const scf_decoration_t *, scf_type_t);
int scf_decoration_type(const scf_decoration_t *, scf_type_t *);
int scf_property_get_decoration(const scf_property_t *,
    scf_decoration_layer_t layer, scf_decoration_t *);
ssize_t scf_decoration_get_bundle(const scf_decoration_t *, char *, size_t);
int scf_decoration_get_value(const scf_decoration_t *, scf_value_t *);
int scf_decoration_get_layer(const scf_decoration_t *,
    scf_decoration_layer_t *);
const char *scf_decoration_layer_to_string(scf_decoration_layer_t);
scf_decoration_layer_t scf_decoration_layer_from_string(const char *);

int scf_instance_is_masked(const scf_instance_t *);
int scf_pg_is_masked(const scf_propertygroup_t *);
int scf_property_is_masked(const scf_property_t *);
int scf_service_is_masked(const scf_service_t *);
...
int scf_transaction_property_undelete(scf_transaction_t *,
    scf_transaction_entry_t *, const char *);
int scf_transaction_property_delcust(scf_transaction_t *,
    scf_transaction_entry_t *, const char *);

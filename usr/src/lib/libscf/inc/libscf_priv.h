...
/*
 * Standard Directory locations for manifests
 */
...
/*
 * Process contract creator and auxiliary terms
 */
#define	CT_CREATOR_STARTD	"svc.startd"
#define	CT_CREATOR_CONFIGD	"svc.configd"
#define	CT_CREATOR_CTRUN	"ctrun"
#define	CT_AUX_REPO_UPGRADE	"repo_upgrade"
...
/*
 * Iterator setup flags used by datael_setup_iter.
 */
#define	SCF_ITER_COMPOSED		0x01
#define	SCF_ITER_CONFLICTS_ONLY		0x02
 ...
 *				conjunction with SCF_WALK_SERVICE.
 *
 * 	SCF_WALK_EMPTY		Allow walking FMRIs with no instances without
 *				issuing an error.
 *
...
#define	SCF_WALK_EMPTY		0x40

/*
 * The default locations of the repository dbs
 */
#define	REPOSITORY_DB		"/etc/svc/repository.db"
#define	NONPERSIST_DB		_PATH_SYSVOL "/svc_nonpersist.db"
#define	FAST_REPOSITORY_DB	_PATH_SYSVOL "/fast_repository.db"
#define	REPOSITORY_CHECKPOINT	_PATH_SYSVOL "/checkpoint_repository.db"
#define	INTEGRITY_DB		_PATH_SYSVOL "/smf_integrity.db"
 ...
 * svccfg import.
 */
int _scf_set_annotation(scf_handle_t *h);
int _scf_unset_annotation(scf_handle_t *h);

/* Set transaction decoration information */
int _scf_set_tx_decoration(scf_handle_t *, const char *,
    scf_decoration_layer_t);
int _scf_unset_tx_decoration(scf_handle_t *);
 ...
 *   pv_mval - multi valued property - if set pv_ptr points to
 *             scf_propvec_mval_t, the linked list of property values.
 *             pv_aux in scf_propvec_mval_t has no meaning.
 ...
 *             where pv_aux indicates the bit holding the truth/false value.
 *             if pv_mval is set, the pv_ptr of scf_propvec_mval_t is always
 *             pointer to a boolean_t
...
struct scf_propvec_mval {
	void			*pv_ptr;
	struct scf_propvec_mval	*next;
};
typedef struct scf_propvec_mval scf_propvec_mval_t;
 ...
 * Conflict check
 */
int scf_service_in_conflict(scf_service_t *);
int scf_instance_in_conflict(scf_instance_t *);
int scf_pg_in_conflict(scf_propertygroup_t *);
int scf_property_in_conflict(scf_property_t *);
int scf_instance_in_conflict_composed(scf_instance_t *, scf_service_t *);
int smf_inst_print_conflicts(scf_instance_t *);
int smf_pg_print_conflicts(scf_propertygroup_t *, const char *);


/*
 * Entity Remove
 */
int scf_service_remove(scf_service_t *);
int scf_instance_remove(scf_instance_t *);
int scf_pg_remove(scf_propertygroup_t *);
int scf_transaction_property_remove(scf_transaction_t *,
    scf_transaction_entry_t *, const char *);
int scf_service_bundle_remove(scf_service_t *, char *);
int scf_instance_bundle_remove(scf_instance_t *, char *);
int scf_prop_bundle_remove(scf_transaction_t *, char *);
...
/*
 * Function to modify a property group in line as opposed
 * to having to remove the property group and readd it.
 */
int scf_transaction_pg_modify(scf_transaction_t *, const char *, uint32_t);

/*
 * Function to flag pg transaction to drop bundle support.
 */
int scf_transaction_pg_bundle_remove(scf_transaction_t *);

...
{
	const char *fmt = "%-14s %-8s %-11s %-8s %-s";
...
clock_freq:
	...
	if (status == PICL_SUCCESS) {
		...
		if (status != PICL_SUCCESS) {
			...
			/* Running frequency */
			(void) snprintf(freq_str, sizeof (freq_str), "%u MHz",

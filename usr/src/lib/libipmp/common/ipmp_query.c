...
{
	int retval;
	ipmp_addrlist_t *tlist4p = NULL, *tlist6p = NULL;
	ipmp_iflist_t	*iflist4p = NULL, *iflist6p = NULL;
	...
	retval = ipmp_readinfo(statep, IPMP_IFLIST, (void **)&iflist4p, endtp);
	if (retval != IPMP_SUCCESS)
		goto fail;

	retval = ipmp_readinfo(statep, IPMP_IFLIST, (void **)&iflist6p, endtp);
	if (retval != IPMP_SUCCESS)
		goto fail;

	ifinfop->if_targinfo4.it_targlistp = tlist4p;
	ifinfop->if_targinfo6.it_targlistp = tlist6p;
	ifinfop->if_targinfo4.it_iflistp = iflist4p;
	ifinfop->if_targinfo6.it_iflistp = iflist6p;
	return (IPMP_SUCCESS);

fail:
	ipmp_freeaddrlist(tlist4p);
	ipmp_freeaddrlist(tlist6p);
	ipmp_freeiflist(iflist4p);
	ipmp_freeiflist(iflist6p);
	return (retval);
}
...
ipmp_targinfo_create(const char *name, struct sockaddr_storage *testaddrp,
    ...
    uint_t nif, char (*ifs)[LIFNAMSIZ])
{
	ipmp_targinfo_t *targinfop;

	targinfop = calloc(1, sizeof (ipmp_targinfo_t));
	if (targinfop == NULL)
		return (NULL);
	...
	if (targmode == IPMP_TARG_TRANSITIVE) {
		targinfop->it_iflistp = ipmp_iflist_create(nif, ifs);
		if (targinfop->it_iflistp == NULL) {
			ipmp_freetarginfo(targinfop);
			return (NULL);
		}
	}
...
static ipmp_iflist_t *
ipmp_iflist_clone(ipmp_iflist_t *iflist)
{
	if (iflist == NULL)
		return (NULL);
	return (ipmp_iflist_create(iflist->il_nif, iflist->il_ifs));
}
    ...
    ipmp_targinfo_t *targinfo4p, ipmp_targinfo_t *targinfo6p)
{
	...
	ifinfop->if_targinfo4.it_iflistp =
	    ipmp_iflist_clone(targinfo4p->it_iflistp);
	ifinfop->if_targinfo6.it_iflistp =
	    ipmp_iflist_clone(targinfo6p->it_iflistp);

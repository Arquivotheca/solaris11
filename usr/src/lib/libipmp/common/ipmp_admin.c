...
int
ipmp_is_svc_enabled(void)
{
	char 		*s;
	int		retval = IPMP_ESVCDISABLED;
	scf_error_t	serr;

	if ((s = smf_get_state("svc:/network/ipmp:default")) != NULL) {
		if (strcmp(s, SCF_STATE_STRING_ONLINE) == 0)
			retval = IPMP_SUCCESS;
		free(s);
	} else if ((serr = scf_error()) == SCF_ERROR_NOT_FOUND ||
	    serr == SCF_ERROR_VERSION_MISMATCH) {
		retval = IPMP_ENOSVC;
	}
	return (retval);
}

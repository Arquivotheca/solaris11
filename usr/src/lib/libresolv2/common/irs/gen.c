...
#ifdef	SUNW_CHECK_MAP_ACC
static int 		init_map_rules(struct gen_p *, const char *conf_file);
#else	/* SUNW_CHECK_MAP_ACC */
static void 		init_map_rules(struct gen_p *, const char *conf_file);
#endif	/* SUNW_CHECK_MAP_ACC */
...
#ifdef	SUNW_CHECK_MAP_ACC
	acc->private = irs;
	if (init_map_rules(irs, conf_file) == -1) {
		gen_close(acc);
		return (NULL);
	}
#else	/* SUNW_CHECK_MAP_ACC */
	 ...
	 const char *options)
{
	...
	if (inst->acc != NULL) {
		...
		if (inst->gr == NULL && inst->acc->gr_map != NULL)
			if (!(inst->gr = (*inst->acc->gr_map)(inst->acc)))
				return (-1);
		if (inst->pw == NULL && inst->acc->pw_map != NULL)
			if (!(inst->pw = (*inst->acc->pw_map)(inst->acc)))
				return (-1);
		if (inst->sv == NULL && inst->acc->sv_map != NULL)
			if (!(inst->sv = (*inst->acc->sv_map)(inst->acc)))
				return (-1);
		if (inst->pr == NULL && inst->acc->pr_map != NULL)
			if (!(inst->pr = (*inst->acc->pr_map)(inst->acc)))
				return (-1);
		if (inst->ho == NULL && inst->acc->ho_map != NULL)
			if (!(inst->ho = (*inst->acc->ho_map)(inst->acc)))
				return (-1);
		if (inst->nw == NULL && inst->acc->nw_map != NULL)
			if (!(inst->nw = (*inst->acc->nw_map)(inst->acc)))
				return (-1);
		if (inst->ng == NULL && inst->acc->ng_map != NULL)
			if (!(inst->ng = (*inst->acc->ng_map)(inst->acc)))
				return (-1);
#else	/* SUNW_CHECK_MAP_ACC */
		if (inst->gr == NULL && inst->acc->gr_map != NULL)
...
default_map_rules(struct gen_p *irs) {
...
#ifdef WANT_IRS_GR
	if (add_rule(irs, irs_gr, irs_lcl, "") == -1)
		return (-1);
#endif
#ifdef WANT_IRS_PW
	if (add_rule(irs, irs_pw, irs_lcl, "") == -1)
		return (-1);
#endif
	if (add_rule(irs, irs_sv, irs_lcl, "") == -1)
		return (-1);
	if (add_rule(irs, irs_pr, irs_lcl, "") == -1)
		return (-1);
#ifdef SUNW_HOSTS_FALLBACK
	if (__res_no_hosts_fallback()) {
		if (add_rule(irs, irs_ho, irs_dns, "") == -1)
			return (-1);
	} else {
		if (add_rule(irs, irs_ho, irs_dns, "continue") == -1)
			return (-1);
		if (add_rule(irs, irs_ho, irs_lcl, "") == -1)
			return (-1);
	}
	...
	if (add_rule(irs, irs_ho, irs_lcl, "") == -1)
		return (-1);
#endif /* SUNW_HOSTS_FALLBACK */
	if (add_rule(irs, irs_nw, irs_dns, "continue") == -1)
		return (-1);
	if (add_rule(irs, irs_nw, irs_lcl, "") == -1)
		return (-1);
	if (add_rule(irs, irs_ng, irs_lcl, "") == -1)
		return (-1);
	return (0);
}
...
#ifdef	SUNW_CHECK_MAP_ACC
		if (add_rule(irs, map, acc, options) == -1) {
			fclose(conf);
			return (-1);
		}
#else	/* SUNW_CHECK_MAP_ACC */

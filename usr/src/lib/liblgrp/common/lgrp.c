...
/*
 * Get affinity inheritance for given process or thread
 */
lgrp_aff_inherit_t
lgrp_affinity_inherit_get(idtype_t idtype, id_t  id)
{
	lgrp_aff_inherit_args_t	args;

	args.idtype = idtype;
	args.id = id;
	return (_lgrpsys(LGRP_SYS_AFF_INHERIT_GET, 0, (void *)&args));
}

/*
 * Set affinity inheritance for given lgroup
 */
int
lgrp_affinity_inherit_set(idtype_t idtype, id_t  id,
    lgrp_aff_inherit_t inherit_flags)
{
	...
	args.idtype = idtype;
	args.id = id;
	args.aff_inherit = inherit_flags;
	return (_lgrpsys(LGRP_SYS_AFF_INHERIT_SET, 0, (void *)&args));
}

...
be_create_snapshot(nvlist_t *be_attrs)
{
	...
	if (nvlist_lookup_pairs(be_attrs, NV_FLAG_NOENTOK,
		...
		be_print_err(gettext("be_create_snapshot: failed to lookup "
	...
	if ((ret = _be_create_snapshot(be_name, altpool, &snap_name, policy))
	    == BE_SUCCESS) {
		if (autoname == B_TRUE) {
			/*
 ...
 */
int
_be_create_snapshot(char *be_name, char *altpool, char **snap_name,
    char *policy)
{
	be_transaction_data_t	bt = { 0 };
	zfs_handle_t		*zhp = NULL;
	...
	/*
	 * Check to see if this is an attempt to create a BE
	 * snapshot of a zone BE that is not associated with the
	 * currently active global zone.
	 */
	if (in_ngz) {
		if (!be_zone_is_bootable(bt.obe_root_ds)) {
			be_print_err(gettext("be_create_snapshot: Operation "
			    "not supported on unbootable BE\n"));
	...
	if (!in_ngz) {
		 ...
		 * cleanup policy there.  Otherwise don't set one - this
		 * snapshot will always inherit the cleanup policy from
		 * its parent.
		 */
...
_be_destroy_snapshot(char *be_name, char *snap_name)
{
	...
	/*
	 * Detect if the BE snapshot to destroy has the 'active on boot'
	 * property set.
	 */
	if (getzoneid() != GLOBAL_ZONEID) {
		if (!be_zone_is_bootable(bt.obe_root_ds)) {
			be_print_err(gettext("be_destroy_snapshot: Operation "
			    "not supported on unbootable BE\n"));
			return (BE_ERR_NOTSUP);
		}
	}

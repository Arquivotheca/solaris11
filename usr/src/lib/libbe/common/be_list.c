...
be_get_list_callback(zpool_handle_t *zlp, void *data)
{
	 ...
	 */
	if (!zfs_dataset_exists(g_zfs, open_ds, ZFS_TYPE_FILESYSTEM)) {
		/*
	...
	char *be_ds)
{
	...
	if (!in_ngz) {
		if ((zphp = zpool_open(g_zfs, rpool)) == NULL) {
			be_print_err(gettext("be_get_node_data: failed "
			    "to open pool (%s): %s\n"), rpool,
			    libzfs_error_description(g_zfs));
		...
		(void) zpool_get_prop(zphp, ZPOOL_PROP_BOOTFS, prop_buf,
		    ZFS_MAXPROPLEN, NULL);
		/*
		 * It is incumbent on the caller to make sure that the boot menu
		 * doesn't contradict this ZFS property.  If that's the case,
		 * the boot menu wins.
		 */
		if (prop_buf != NULL && strcmp(prop_buf, be_ds) == 0)
			be_node->be_active_on_boot = B_TRUE;
		else
			be_node->be_active_on_boot = B_FALSE;

		/*
		 * TODO - we might want to set be_active_unbootable = B_TRUE for
		 * BEs that are stored on alternate pools.  For now, we set it
		 * to false.  Because we currently don't support multiple root
		 * pools in the global zone context, a BE could be considered
		 * to be 'unbootable' if it resides on an alternate pool that is
		 * not the root pool.  Similarly, in a solaris container (aka
		 * Zone) a BE is unbootable if it's parent BE in the global zone
		 * is not the currently active global zone BE.  In both cases,
		 * the admin would have to do *something* in order to boot these
		 * BEs above and beyond just beadm activate.
		 */
		be_node->be_active_unbootable = B_FALSE;

		zpool_close(zphp);
	} else {
		/*
		 * For nested BEs that are not associated with the currently
		 * active global zone BE we mark them as unbootable since they
		 * can not be booted under the currently active global zone.
		 */
		if (be_zone_is_bootable(be_node->be_root_ds))
			be_node->be_active_unbootable = B_FALSE;
		else
			be_node->be_active_unbootable = B_TRUE;
	 ...
	 */
	be_node->be_mounted = zfs_is_mounted(zhp, &(be_node->be_mntpt));
	...
	if ((userprops = zfs_get_user_props(zhp)) == NULL) {
		...
		/*
		 * Get the uuid for global zone bes and
		 * for solaris container bes we get the parentbe uuid
		 */
		if (!in_ngz) {

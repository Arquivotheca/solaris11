...
#define	S10_AUDITSYS(rv, cmd, a0, a1, a2) \
    __systemcall((rv), SYS_auditsys + 1024, (cmd), (a0), (a1), (a2))

#define	S10_AUDIT_HMASK	0xffffffc0
#define	S10_AUDIT_LMASK	0x3f
#define	S10_AUDIT_L32(v) ((v) & UINT64_C(0xffffffff))

#define	S10_AUT_AUDITINFO_ADDR	1
#define	S10_AUT_AUDITINFO	2
#define	S10_AUT_AUDITPINFO_ADDR	3
#define	S10_AUT_AUDITPINFO	4
#define	S10_AUT_EVCLASSMAP	5
#define	S10_AUT_AU_MASK		6
#define	S10_AUT_POLICY		7

typedef unsigned int s10_au_class_t;

typedef struct s10_au_mask {
	s10_au_class_t am_success;
	s10_au_class_t am_failure;
} s10_au_mask_t;

typedef struct s10_auditinfo {
	au_id_t		ai_auid;
	s10_au_mask_t	ai_mask;
	au_tid_t	ai_termid;
	au_asid_t	ai_asid;
} s10_auditinfo_t;

typedef struct s10_auditinfo_addr {
	au_id_t		ai_auid;
	s10_au_mask_t	ai_mask;
	au_tid_addr_t	ai_termid;
	au_asid_t	ai_asid;
} s10_auditinfo_addr_t;

typedef struct s10_auditinfo64_addr {
	au_id_t		ai_auid;
	s10_au_mask_t	ai_mask;
	au_tid64_addr_t	ai_termid;
	au_asid_t	ai_asid;
} s10_auditinfo64_addr_t;

typedef struct s10_auditpinfo {
	pid_t		ap_pid;
	au_id_t		ap_auid;
	s10_au_mask_t	ap_mask;
	au_tid_t	ap_termid;
	au_asid_t	ap_asid;
} s10_auditpinfo_t;

typedef struct s10_auditpinfo_addr {
	pid_t		ap_pid;
	au_id_t		ap_auid;
	s10_au_mask_t	ap_mask;
	au_tid_addr_t	ap_termid;
	au_asid_t	ap_asid;
} s10_auditpinfo_addr_t;

typedef struct s10_evclass_map {
	au_event_t	ec_number;
	s10_au_class_t	ec_class;
} s10_evclass_map_t;

typedef union s10_asys_types {
	s10_auditinfo_t		ai;
	s10_auditinfo_addr_t	ai_a;
	s10_auditpinfo_t	pai;
	s10_auditpinfo_addr_t	pai_a;
	s10_au_mask_t		m;
	s10_evclass_map_t	evm;
	uint32_t		p;
} s10_asys_types_t;

typedef union native_asys_types {
	auditinfo_t		    ai;
	auditinfo_addr_t	    ai_a;
	struct auditpinfo	    pai;
	struct auditpinfo_addr	    pai_a;
	au_mask_t		    m;
	au_evclass_map_t	    evm;
	uint32_t		    p;
} native_asys_types_t;

static int
s10_audit_get_native(int dtype, intptr_t from10, native_asys_types_t *t)
{
	s10_asys_types_t t10;

	switch (dtype) {
	case S10_AUT_AUDITINFO_ADDR:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.ai_a)) != 0)
			return (EFAULT);
		t->ai_a.ai_auid	    = t10.ai_a.ai_auid;
		t->ai_a.ai_termid   = t10.ai_a.ai_termid;
		t->ai_a.ai_asid	    = t10.ai_a.ai_asid;
		t->ai_a.ai_mask.am_success =
		    (au_class_t)(t10.ai_a.ai_mask.am_success);
		t->ai_a.ai_mask.am_failure =
		    (au_class_t)(t10.ai_a.ai_mask.am_failure);
		break;
	case S10_AUT_AUDITINFO:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.ai)) != 0)
			return (EFAULT);
		t->ai.ai_auid	    = t10.ai.ai_auid;
		t->ai.ai_termid	    = t10.ai.ai_termid;
		t->ai.ai_asid	    = t10.ai.ai_asid;
		t->ai.ai_mask.am_success =
		    (au_class_t)(t10.ai.ai_mask.am_success);
		t->ai.ai_mask.am_failure =
		    (au_class_t)(t10.ai.ai_mask.am_failure);
		break;
	case S10_AUT_AUDITPINFO_ADDR:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.pai_a)) != 0)
			return (EFAULT);
		t->pai_a.ap_pid	    = t10.pai_a.ap_pid;
		t->pai_a.ap_auid    = t10.pai_a.ap_auid;
		t->pai_a.ap_asid    = t10.pai_a.ap_asid;
		t->pai_a.ap_termid  = t10.pai_a.ap_termid;
		t->pai_a.ap_mask.am_success =
		    (au_class_t)(t10.pai_a.ap_mask.am_success);
		t->pai_a.ap_mask.am_failure =
		    (au_class_t)(t10.pai_a.ap_mask.am_failure);
		break;
	case S10_AUT_AUDITPINFO:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.pai)) != 0)
			return (EFAULT);
		t->pai.ap_pid	    = t10.pai.ap_pid;
		t->pai.ap_auid	    = t10.pai.ap_auid;
		t->pai.ap_asid	    = t10.pai.ap_asid;
		t->pai.ap_termid    = t10.pai.ap_termid;
		t->pai.ap_mask.am_success =
		    (au_class_t)(t10.pai.ap_mask.am_success);
		t->pai.ap_mask.am_failure =
		    (au_class_t)(t10.pai.ap_mask.am_failure);
		break;
	case S10_AUT_EVCLASSMAP:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.evm)) != 0)
			return (EFAULT);
		t->evm.ec_class	    = (au_class_t)(t10.evm.ec_class);
		t->evm.ec_number    = t10.evm.ec_number;
		break;
	case S10_AUT_AU_MASK:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.m)) != 0)
			return (EFAULT);
		t->m.am_success = (au_class_t)(t10.m.am_success);
		t->m.am_failure = (au_class_t)(t10.m.am_failure);
		break;
	case S10_AUT_POLICY:
		if (brand_uucopy((const void *)from10, (void *)&t10,
		    sizeof (t10.p)) != 0)
			return (EFAULT);
		t->p = ((t10.p >> 1) & S10_AUDIT_HMASK) |
		    (t10.p & S10_AUDIT_LMASK);
		break;
	default:
		break;
	}
...
static int
s10_audit_get_s10(int dtype, native_asys_types_t *t, intptr_t to10)
{
	s10_asys_types_t	t10;
	size_t			size;
	...
	switch (dtype) {
	case S10_AUT_AUDITINFO_ADDR:
		t10.ai_a.ai_auid    = t->ai_a.ai_auid;
		t10.ai_a.ai_termid  = t->ai_a.ai_termid;
		t10.ai_a.ai_asid    = t->ai_a.ai_asid;
		t10.ai_a.ai_mask.am_success =
		    (s10_au_class_t)S10_AUDIT_L32(t->ai_a.ai_mask.am_success);
		t10.ai_a.ai_mask.am_failure =
		    (s10_au_class_t)S10_AUDIT_L32(t->ai_a.ai_mask.am_failure);
		size = sizeof (t10.ai_a);
		break;
	case S10_AUT_AUDITINFO:
		t10.ai.ai_auid	    = t->ai.ai_auid;
		t10.ai.ai_termid    = t->ai.ai_termid;
		t10.ai.ai_asid	    = t->ai.ai_asid;
		t10.ai.ai_mask.am_success =
		    (s10_au_class_t)S10_AUDIT_L32(t->ai.ai_mask.am_success);
		t10.ai.ai_mask.am_failure =
		    (s10_au_class_t)S10_AUDIT_L32(t->ai.ai_mask.am_failure);
		size = sizeof (t10.ai);
		break;
	case S10_AUT_AUDITPINFO:
		t10.pai.ap_pid	    = t->pai.ap_pid;
		t10.pai.ap_auid	    = t->pai.ap_auid;
		t10.pai.ap_asid	    = t->pai.ap_asid;
		t10.pai.ap_termid   = t->pai.ap_termid;
		t10.pai.ap_mask.am_success =
		    (s10_au_class_t)S10_AUDIT_L32(t->pai.ap_mask.am_success);
		t10.pai.ap_mask.am_failure =
		    (s10_au_class_t)S10_AUDIT_L32(t->pai.ap_mask.am_failure);
		size = sizeof (t10.pai);
		break;
	case S10_AUT_AUDITPINFO_ADDR:
		t10.pai_a.ap_pid    = t->pai_a.ap_pid;
		t10.pai_a.ap_auid   = t->pai_a.ap_auid;
		t10.pai_a.ap_asid   = t->pai_a.ap_asid;
		t10.pai_a.ap_termid = t->pai_a.ap_termid;
		t10.pai_a.ap_mask.am_success =
		    (s10_au_class_t)S10_AUDIT_L32(t->pai_a.ap_mask.am_success);
		t10.pai_a.ap_mask.am_failure =
		    (s10_au_class_t)S10_AUDIT_L32(t->pai_a.ap_mask.am_failure);
		size = sizeof (t10.pai_a);
		break;
	case S10_AUT_AU_MASK:
		t10.m.am_success =
		    (s10_au_class_t)S10_AUDIT_L32(t->m.am_success);
		t10.m.am_failure =
		    (s10_au_class_t)S10_AUDIT_L32(t->m.am_failure);
		size = sizeof (t10.m);
		break;
	case S10_AUT_EVCLASSMAP:
		t10.evm.ec_class =
		    (s10_au_class_t)S10_AUDIT_L32(t->evm.ec_class);
		t10.evm.ec_number = S10_AUDIT_L32(t->evm.ec_number);
		size = sizeof (t10.evm);
		break;
	case S10_AUT_POLICY:
		t10.p = ((t->p & S10_AUDIT_HMASK) << 1) |
		    (t->p & S10_AUDIT_LMASK);
		size = sizeof (t10.p);
		break;
	default:
	...
	return (brand_uucopy((const void *)&t10, (void *)to10, size));
}
...
s10_auditsys(sysret_t *rval, int bsmcmd, intptr_t a0, intptr_t a1, intptr_t a2)
{
	...
	intptr_t	    arg1 = a1;
	intptr_t	    arg2 = a2;
	native_asys_types_t t;

	if (bsmcmd != BSM_AUDITCTL) {
		switch (bsmcmd) {
		case BSM_GETAUDIT:
			err = S10_AUDITSYS(rval, bsmcmd, &t.ai, sizeof (t.ai),
			    arg2);
			if (err != 0)
				return (err);
			return (s10_audit_get_s10(S10_AUT_AUDITINFO, &t, arg0));
		case BSM_GETAUDIT_ADDR:
			if ((size_t)arg1 < sizeof (s10_auditinfo_addr_t))
				return (EOVERFLOW);
			err = S10_AUDITSYS(rval, bsmcmd, &t.ai_a,
			    sizeof (t.ai_a), arg2);
			if (err != 0)
				return (err);
			return (s10_audit_get_s10(S10_AUT_AUDITINFO_ADDR, &t,
			    arg0));
		case BSM_SETAUDIT:
			err = s10_audit_get_native(S10_AUT_AUDITINFO, arg0, &t);
			if (err != 0)
				return (err);
			arg0 = (intptr_t)&t.ai;
			arg1 = (intptr_t)sizeof (t.ai);
			break;
		case BSM_SETAUDIT_ADDR:
			if ((size_t)arg1 < sizeof (s10_auditinfo_addr_t))
				return (EOVERFLOW);
			err = s10_audit_get_native(S10_AUT_AUDITINFO_ADDR, arg0,
			    &t);
			if (err != 0)
				return (err);
			arg0 = (intptr_t)&t.ai_a;
			arg1 = (intptr_t)sizeof (t.ai_a);
			break;
		default:
			break;
		}
	...
	switch ((int)arg0) {
	case A_GETKMASK:
		err = S10_AUDITSYS(rval, bsmcmd, arg0, &t.m, sizeof (t.m));
		if (err != 0)
			return (err);
		return (s10_audit_get_s10(S10_AUT_AU_MASK, &t, arg1));
	case A_GETKAUDIT:
		if ((size_t)arg2 < sizeof (s10_auditinfo_addr_t))
			return (EOVERFLOW);
		err = S10_AUDITSYS(rval, bsmcmd, arg0, &t.ai_a,
		    sizeof (t.ai_a));
		if (err != 0)
			return (err);
		return (s10_audit_get_s10(S10_AUT_AUDITINFO_ADDR, &t, arg1));
	case A_GETCLASS:
		err = s10_audit_get_native(S10_AUT_EVCLASSMAP, arg1, &t);
		if (err != 0)
			return (err);
		err = S10_AUDITSYS(rval, bsmcmd, arg0, &t.evm, sizeof (t.evm));
		if (err != 0)
			return (err);
		return (s10_audit_get_s10(S10_AUT_EVCLASSMAP, &t, arg1));
	case A_GETPINFO:
		err = s10_audit_get_native(S10_AUT_AUDITPINFO, arg1, &t);
		if (err != 0)
			return (err);
		err = S10_AUDITSYS(rval, bsmcmd, arg0, &t.pai, sizeof (t.pai));
		if (err != 0)
			return (err);
		return (s10_audit_get_s10(S10_AUT_AUDITPINFO, &t, arg1));
	case A_GETPINFO_ADDR:
		if ((size_t)arg2 < sizeof (s10_auditpinfo_addr_t))
			return (EOVERFLOW);
		err = s10_audit_get_native(S10_AUT_AUDITPINFO_ADDR, arg1, &t);
		if (err != 0)
			return (err);
		err = S10_AUDITSYS(rval, bsmcmd, arg0, &t.pai_a,
		    sizeof (t.pai_a));
		if (err != 0)
			return (err);
		return (s10_audit_get_s10(S10_AUT_AUDITPINFO_ADDR, &t, arg1));
	case A_GETPOLICY:
		err = S10_AUDITSYS(rval, bsmcmd, arg0, &t.p, sizeof (t.p));
		if (err)
			return (err);
		return (s10_audit_get_s10(S10_AUT_POLICY, &t, arg1));
	case A_SETKMASK:
		err = s10_audit_get_native(S10_AUT_AU_MASK, arg1, &t);
		if (err != 0)
			return (err);
		arg1 = (intptr_t)&t.m;
		arg2 = (intptr_t)sizeof (t.m);
		break;
	case A_SETKAUDIT:
		err = s10_audit_get_native(S10_AUT_AUDITINFO_ADDR, arg1, &t);
		if (err != 0)
			return (err);
		arg1 = (intptr_t)&t.ai_a;
		arg2 = (intptr_t)sizeof (t.ai_a);
		break;
	case A_SETUMASK:
	case A_SETSMASK:
		err = s10_audit_get_native(S10_AUT_AUDITINFO, arg1, &t);
		if (err != 0)
			return (err);
		arg1 = (intptr_t)&t.ai;
		arg2 = (intptr_t)sizeof (t.ai);
		break;
	case A_SETPMASK:
		err = s10_audit_get_native(S10_AUT_AUDITPINFO, arg1, &t);
		if (err != 0)
...
syscall:
	return (S10_AUDITSYS(rval, bsmcmd, arg0, arg1, arg2));
}

static int
s10_ucredsys(sysret_t *rval, int code, int obj, void *buf)
{
	auditinfo64_addr_t	*ai;
	s10_auditinfo64_addr_t	ai10;
	ucred_t			*ucp;
	int			err;
	size_t			size;
	priv_impl_info_t	ip;
	...
	/* Get priv implementation header */
	if ((err = __systemcall(&rv, SYS_privsys + 1024, PRIVSYS_GETIMPLINFO,
	    0, 0, (void *)&ip, sizeof (ip), 0)) != 0) {
		return (err);
	}

	/* Get number of supplemental groups */
	rv.sys_rval1 = rv.sys_rval2 = 0;
	(void) __systemcall(&rv, SYS_sysconfig + 1024, _CONFIG_NGROUPS);

	/*
	 * Count the estimated ucred size like UCRED_SIZE() does and allocate
	 * the buffer. The internal size of MAC label is used here because
	 * cross library calls to blabel_size could cause conflicts.
	 */
	size = (sizeof (struct ucred_s) + sizeof (prcred_t) +
	    ((int)rv.sys_rval1 - 1) * sizeof (gid_t) +
	    sizeof (prpriv_t) +
	    sizeof (priv_chunk_t) *
	    ...
	    sizeof (bslabel_t));
	if ((ucp = malloc(size)) == NULL) {
		return (ENOMEM);
	}

	/* ucredsys system call */
	if ((err = __systemcall(rval, SYS_ucredsys + 1024, code, obj,
	    (void *)ucp)) != 0) {
		free(ucp);
		return (err);
	}
	...
	/* LINTED: alignment */
	if ((ai = UCAUD(ucp)) == NULL) {
		goto ucredsys_finish;
	}

	/*
	 * Substitute native audit info data located at ai pointer with
	 * S10 audit info
	 */
	size = (size_t)(ucp->uc_size - sizeof (auditinfo64_addr_t) +
	    sizeof (s10_auditinfo64_addr_t));
	ai10.ai_mask.am_success = (s10_au_class_t)ai->ai_mask.am_success_lo;
	ai10.ai_mask.am_failure = (s10_au_class_t)ai->ai_mask.am_failure_lo;
	ai10.ai_auid = ai->ai_auid;
	ai10.ai_asid = ai->ai_asid;
	ai10.ai_termid = ai->ai_termid;

	/*
	 * Move data located behind the original audit info we pulled
	 * out of the kernel at the end of the S10 audit info.
	 */
	(void) memcpy((char *)ai, (char *)&ai10, sizeof (ai10));
	(void) memmove((char *)ai + sizeof (ai10),
	    (char *)ai + sizeof (auditinfo64_addr_t),
	    (size_t)(ucp->uc_size - ucp->uc_audoff -
	    sizeof (auditinfo64_addr_t)));
	ucp->uc_size = (uint32_t)size;
	size = sizeof (auditinfo64_addr_t) - sizeof (s10_auditinfo64_addr_t);

	/*
	 * Update offsets when needed. Note that uc_auditoff > 0, otherwise
	 * UCAUD(ucp) would return NULL.
	 */
	if (ucp->uc_labeloff > ucp->uc_audoff) {
		ucp->uc_labeloff -= size;
	}
	if (ucp->uc_credoff > ucp->uc_audoff) {
		ucp->uc_credoff -= size;
	}
	if (ucp->uc_privoff > ucp->uc_audoff) {
		ucp->uc_privoff -= size;
	}

ucredsys_finish:
	err = brand_uucopy((const void *)ucp, (void *)buf, ucp->uc_size);
	free(ucp);
	return (err);
}
...
/*
 * The brand of a global zone in s10 was "native".
 */
#define	S10_NATIVE_BRAND_NAME "native"
...
brand_sysent_table_t brand_sysent_table[] = {
	...
	NOSYS,					/*  82 */
	EMULATE(s10_ucredsys, 3	| RV_DEFAULT),	/*  83 */
	NOSYS,					/*  84 */
	NOSYS,					/*  85 */
	NOSYS,					/*  86 */
	...
	NOSYS,					/* 109 */
	EMULATE(s10_utimesys, 5 | RV_DEFAULT),	/* 110 */
	NOSYS,					/* 111 */
	NOSYS,					/* 112 */
	NOSYS,					/* 113 */
	NOSYS,					/* 114 */

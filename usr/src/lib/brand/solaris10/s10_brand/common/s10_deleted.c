...
s10_utime(sysret_t *rval, const char *path, const struct utimbuf *times)
{
	...
	return (__systemcall(rval, SYS_utimensat + 1024,
	    AT_FDCWD, path, tsp, 0));
}
    ...
    int fd, const char *path, const struct timeval times[2])
{
	...
	return (__systemcall(rval, SYS_utimensat + 1024, fd, path, tsp, 0));
}

/*
 * This was implemented in Solaris Nevada, then patched back to Solaris 10,
 * then changed in Solaris Nevada to a better implementation of utimensat().
 * Since it is in Solaris 10 (some patch level), we must emulate it here.
 */
int
s10_utimesys(sysret_t *rval, int code,
    uintptr_t arg1, uintptr_t arg2, uintptr_t arg3, uintptr_t arg4)
{
	switch (code) {
	case 0:		/* futimens */
		return (__systemcall(rval, SYS_utimensat + 1024,
		    (int)arg1, NULL, (timespec_t *)arg2, 0));
	case 1:		/* utimensat */
		return (__systemcall(rval, SYS_utimensat + 1024,
		    (int)arg1, (char *)arg2, (timespec_t *)arg3, (int)arg4));
	default:
		return (EINVAL);
	}
}

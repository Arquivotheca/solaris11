...
case $cmd in
	0)
		# If we're readying the zone, update the zone index file
		# to look like this is the global zone.
		echo "global:installed:/" > $zonepath/root/etc/zones/index
		;;
	4)
		# unmount/remount the zone's root dataset when the zone
		# is halted
		mount_active_be -c zone || exit $ZONE_SUBPROC_FATAL
		;;
esac

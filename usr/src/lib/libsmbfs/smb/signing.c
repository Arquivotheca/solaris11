	...
	uint32_t seqno, uchar_t *signature)
{
	 ...
	 */
	bcopy(m->m_data, smbhdr.r.raw, SMB_HEADER_LEN);
	...
	MD5Update(&md5, smbhdr.r.raw, SMB_HEADER_LEN);

	/* Digest the rest of the first mbuf */
	if (m->m_len > SMB_HEADER_LEN) {
		MD5Update(&md5, m->m_data + SMB_HEADER_LEN,
		    m->m_len - SMB_HEADER_LEN);
	}
	m = m->m_next;
	 ...
	 */
	if (signature)
		bcopy(digest, signature, SMB_SIG_SIZE);

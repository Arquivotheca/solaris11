...
int  smb_ctx_parseunc(struct smb_ctx *, const char *, int, int, int,
	const char **);
int  smb_ctx_parsedomuser(char *, char **, char **);
...
int  smb_ctx_setsigning(struct smb_ctx *, boolean_t required);
...
int  smb_fh_open(const char *, int);
int  smb_fh_read(int, void *, size_t, off_t);
int  smb_fh_write(int, const void *, size_t, off_t);

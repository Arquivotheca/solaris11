...
static int files_update_history(char *name, char *history);
	...
	for (p = items; p != NULL; p = p->next) {
		...
		case ATTR_PASSWD_SERVER_POLICY:
			...
			if (p->data.val_s == NULL) {
				spw->sp_pwdp = "";
			} else if ((strcmp(spw->sp_pwdp, NOLOGINSTRING) == 0) ||
			    (strcmp(spw->sp_pwdp, LOCKSTRING) == 0)) {
				/*
				 * Disallow setting a password on system
				 * accounts.
				 * System accounts are denoted either
				 * by the no login string or the lock
				 * string without any password following.
				 */
				...
				free(pwbuf->history);
				pwbuf->history = strdup(hash);
				if (pwbuf->history == NULL) {
					errno = ENOMEM;
					return (PWU_NOMEM);
				}
				if ((strncmp(spw->sp_pwdp, LOCKSTRING,
					...
					(void) strlcat(pword, hash, len);
					spw->sp_pwdp = pword;
				} else {
					spw->sp_pwdp = pwbuf->history;
				}
				aging_needed = 1;
			}
...
files_putpwnam(char *name, char *oldpw, pwu_repository_t *rep, void *buf)
{
	...
	if (result == PWU_SUCCESS && pwbuf->spwd) {
		if (pwbuf->history != NULL) {
			debug("update_history = %s", pwbuf->history);
			result = files_update_history(name, pwbuf->history);
		} else {
 ...
 */
int
files_update_history(char *name, char *history)
{
	int	histsize;
	int	tmpfd;
	...
	debug("files_update_history(%s, %s) histsize = %d", name,
	    history, histsize);
	...
	while ((src != NULL) &&
		...
		if (strcmp(user, name) == 0) {
			...
			/* found user, update */
			found++;
			(void) fprintf(dst, "%s:%s:", name, history);
...
#ifdef	DEBUG
			debug("files_update_history: copy line %s", user);
#endif	/* DEBUG */
		}
	}
	...
	/* If something messed up in file system, lose the update */
	if (fclose(dst) != 0) {
	...
	if (stat64(OHISTORY, &statbuf) == 0 ||
		...
		/* old history won't go away, lose the update */

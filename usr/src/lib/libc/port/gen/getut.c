...
_compat_makeut(struct utmp *utmp)
{
	...
	unsigned char saveid[_UTMP_ID_LEN];

	wild = 0;
	for (i = 0; i < _UTMP_ID_LEN; i++)
		if (utmp->ut_id[i] == _UTMP_ID_WILDCARD) {
			wild = 1;
			break;
		}
	...
	if (wild) {
		...
		(void) memset(saveid, i, _UTMP_ID_LEN);
		...
		while ((utp = _compat_getutent()) != 0) {
			...
			if (utp->ut_type == DEAD_PROCESS)
				break;
			(void) memcpy(saveid, utp->ut_id, _UTMP_ID_LEN);
		}
		...
		if (utp) {
			 ...
			 */
			(void) memcpy(utmp->ut_id, utp->ut_id, _UTMP_ID_LEN);

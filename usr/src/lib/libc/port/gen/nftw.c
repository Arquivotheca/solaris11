    ...
    int depth, struct Save *last, struct Var *vp)
{
	...
	*component = 0;
	if (vp->curflags & FTW_CHDIR) {
		...
		if ((cdval = fchdir(dirfd(this.fd))) >= 0) {
			this.comp = component;
		} else {
			type = FTW_DNR;
			rc = (*fn)(vp->tmppath, &statb, type, &vp->state);

    ...
    int maxgids, int numgids)
{
	 ...
	 * we need to change the default to be 1 (TRUE) as we now
	 * need the admin to decided to use netid, setting
	 * NETID_AUTHORITATIVE in /etc/default/nss to TRUE gets us
	 * a value of 0 for force_slow_way - don't you just love
	 * double negatives ;-)
	 *

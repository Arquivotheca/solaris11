...
/*
 * The nss_status_t values NSS_FRONTRETRY and NSS_BACKRETRY are private to
 * this module.
 *
 * NSS_FRONTRETRY is used to indicate that the back (private) door will not
 * perform the request and the request should be sent to the front (fattached)
 * door. This can happen because either the back door has no file descriptor
 * (-1) or the db is not a member of the back door db list.
 *
 * NSS_BACKRETRY is used to indicate that the back (private) door should be
 * retried. This can happen if the back (private) door file descriptor has
 * changed.
 */
#define	NSS_FRONTRETRY	-1
#define	NSS_BACKRETRY	-2

/*
 * Check for a valid and matching db in the list.
 * The db list must be locked by the caller.
 */
...
static nss_status_t
_nsc_initdoor_fp(nsc_door_t *dp, char *db, int *fd)
{
	...
	*fd = -1;
	...
	/* Check that the private back door is appropiate */
	if (dp == &nsc_door[1] &&
	    (dp->doorfd == -1 || _nsc_use_backdoor(db) == 0)) {
		lmutex_unlock(&dp->door_lock);
		return (NSS_FRONTRETRY);
	}
			...
			size_t *adata, int *pdesc)
{
	...
	param.desc_ptr = NULL;
	param.desc_num = 0;
	ret = __door_call(fd, &param);
	if (ret < 0) {
		if (dp == &nsc_door[1] && dp->doorfd != -1 &&
		    dp->doorfd != fd) {
			/* New back door which we can retry */
			return (NSS_BACKRETRY);
		} else if (dp == &nsc_door[1] && dp->doorfd == -1) {
			/* Back door has closed so try to obtain a new one */
			return (NSS_FRONTRETRY);
		} else
			return (NSS_ERROR);
	}
...
_nsc_trydoorcall_ext(void **dptr, size_t *ndata, size_t *adata)
{
	...
	char		*db_list;
	int		fb2frontd = 0;	/* Fallback to front door */
	int		reset_frontd = 0;
	size_t		ndata_save = *ndata, adata_save = *adata;
	void		*dptr_save = *dptr;
	boolean_t	restore_headers = B_FALSE;
	nss_status_t	server_status = NSS_SUCCESS;
	...
	while (ret == NSS_ALTRETRY || ret == NSS_ALTRESET ||
	    ret == NSS_FRONTRETRY || ret == NSS_BACKRETRY) {
		if (restore_headers) {
			if (*dptr != dptr_save)
				(void) munmap((void *)*dptr, *ndata);
			/*
			 * Restore the buffer size and header
			 * data so that the door will
			...
			*dptr = dptr_save;
			ph =  (nss_pheader_t *)*dptr;
			*ph = ph_save;
			ph->p_status = server_status;
			restore_headers = B_FALSE;
		}
		/* Try private (back) door first */
		if (fb2frontd == 0 && ret != NSS_FRONTRETRY) {
			...
			if (ret == NSS_ALTRESET) {
				 ...
				 */
				fb2frontd = 1;
				restore_headers = B_TRUE;
				/*
				 * tell the front door server, this is
				 * a fallback call
				 */
				server_status = NSS_ALTRETRY;
				continue;
			} else if (ret == NSS_FRONTRETRY ||
			    ret == NSS_BACKRETRY) {
				/*
				 * Retry the door after restoring buffer
				 * and headers
				 */
				restore_headers = B_TRUE;
				server_status = NSS_SUCCESS;
				continue;
			} else {
		...
		ph =  (nss_pheader_t *)*dptr;
		db_list = ((char *)ph) + ph->data_off;

		if (_nsc_init_private_db(db_list) == 0) {

...
#define	IPIPE	_PATH_SYSVOL "/initpipe"	/* FIFO to send pids to init */
#define	UPIPE	_PATH_SYSVOL "/utmppipe"	/* FIFO to send pids to utmpd */

/* old file only, for sanity check */
#define	ETC_UTMPX_FILE	"/etc/utmpx"
...
utmpxname(const char *newfile)
{
	...
	/*
	 * If the file is being changed to UTMPX_FILE, /etc/utmpx or
	 * /var/adm/utmpx then we clear the flag so pututxline invokes
	 * utmp_update.  Otherwise we set the flag indicating that they
	 * changed to another name.
	 */
	if (strcmp(utmpxfile, UTMPX_FILE) == 0 ||
	    strcmp(utmpxfile, ETC_UTMPX_FILE) == 0 ||
	    strcmp(utmpxfile, _PATH_UTMPX_OLD) == 0)
		changed_name = 0;
	else
		changed_name = 1;
...
modutx(const struct utmpx *utp)
{
	...
	for (i = 0; i < _UTMP_ID_LEN; ++i) {
		if (utp->ut_id[i] == _UTMP_ID_WILDCARD)
			return (NULL);
	}
...
allocid(char *srcid, unsigned char *saveid)
{
	...
	int changed;		/* flag to indicate that a new id has */
				/* been generated */
	char copyid[_UTMP_ID_LEN];	/* work area */

	(void) memcpy(copyid, srcid, _UTMP_ID_LEN);
	changed = 0;
	for (i = 0; i < _UTMP_ID_LEN; ++i) {
	 ...
	 */
	if (changed) {
		(void) memcpy(srcid, copyid, _UTMP_ID_LEN);
		return (0);
	} else {
		return (-1);
	}
...
lockutx(void)
{
	...
	if ((lockfd = open(utmpxfile, O_RDWR|O_CREAT, 0644)) < 0)
		return (-1);
...
makeutx(const struct utmpx *utmp)
{
	...
	unsigned char saveid[_UTMP_ID_LEN]; /* the last id we matched that */
					/* was NOT a dead proc */
	 ...
	 */
	if (memchr(utmp->ut_id, _UTMP_ID_WILDCARD, _UTMP_ID_LEN) != NULL) {
		/*
		 ...
		 * used in allocid
		 */
		(void) memset(saveid, falphanum, _UTMP_ID_LEN);
		...
		if (ut) {
			 ...
			 */
			(void) memcpy((caddr_t)utmp->ut_id, ut->ut_id,
			    _UTMP_ID_LEN);

...
readdir(DIR *dirp)
{
	...
	if (dirp->d_size != 0) {
		...
		saveloc = dirp->d_loc;		/* save for possible EOF */
		dirp->d_loc += (int)dp->d_reclen;
	}
	if (dirp->d_loc >= dirp->d_size)
		dirp->d_loc = dirp->d_size = 0;

	if (dirp->d_size == 0 && 	/* refill buffer */
	    (dirp->d_size = getdents(dirp->d_fd,
	    (dirent_t *)(uintptr_t)dirp->d_buf, DIRBUF)) <= 0) {
		if (dirp->d_size == 0)		/* This means EOF */
			dirp->d_loc = saveloc;	/* so save for telldir */
		return (NULL);		/* error or EOF */
	}

	return ((dirent_t *)(uintptr_t)&dirp->d_buf[dirp->d_loc]);
}
...
readdir64(DIR *dirp)
{
	...
	if (dirp->d_size != 0) {
		...
		saveloc = dirp->d_loc;		/* save for possible EOF */
		dirp->d_loc += (int)dp64->d_reclen;
	}
	if (dirp->d_loc >= dirp->d_size)
		dirp->d_loc = dirp->d_size = 0;

	if (dirp->d_size == 0 && 	/* refill buffer */
	    (dirp->d_size = getdents64(dirp->d_fd,
	    (dirent64_t *)(uintptr_t)dirp->d_buf, DIRBUF)) <= 0) {
		if (dirp->d_size == 0)		/* This means EOF */
			dirp->d_loc = saveloc;	/* so save for telldir */
		return (NULL);		/* error or EOF */
	}

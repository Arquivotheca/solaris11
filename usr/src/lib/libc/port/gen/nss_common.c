 ...
 * search/setent/getent the lock can be left set in the child,
...
static array_entry_t *lock_array = NULL;
static int lock_array_size = 0;
static int lock_array_nxt_free = 0;
static mutex_t lock_array_mutex = DEFAULTMUTEX;
...
nss_reinitlocks(void)
{
	...
	(void) mutex_init(&lock_array_mutex, USYNC_THREAD, NULL);
	...
	for (i = 0; i < lock_array_nxt_free; i++) {
		 ...
		 */
		if (dladdr(lock_array[i].addr, &cur_dli) != 0) {
			if (bcmp(&cur_dli, &lock_array[i].dli,
			    sizeof (Dl_info_t)) == 0) {
					(void) mutex_init(lock_array[i].addr,
					    USYNC_THREAD, NULL);
			}
		}
	}
...
static void storelockaddr(void *r)
{
	...
	if (lock_array == NULL) {
		size = 25;
		tmp = libc_malloc(size * sizeof (array_entry_t));
		if (tmp == NULL) {
			(void) mutex_unlock(&lock_array_mutex);
			return;
		}
		lock_array = tmp;
		lock_array_size = size;
	...
	for (i = 0; i < lock_array_nxt_free; i++) {
		if (lock_array[i].addr == r) {
			...
			if (dladdr(lock_array[i].addr, &newdli) != 0) {
				/*
				...
				if (i < lock_array_nxt_free - 1) {
					lock_array[i] =
					    lock_array[lock_array_nxt_free - 1];
				}
				lock_array_nxt_free--;
				(void) mutex_unlock(&lock_array_mutex);
				return;
	...
	if (lock_array_nxt_free >= lock_array_size) {
		size = lock_array_size + 10;
		tmp = libc_realloc(lock_array, size * sizeof (array_entry_t));
		if (tmp == NULL) {
			(void) mutex_unlock(&lock_array_mutex);
			return;
		}
	...
	/* add new entry */
	if (dladdr(r, &newdli) != 0) {
		lock_array[lock_array_nxt_free].dli = newdli;
		lock_array[lock_array_nxt_free].addr = r;
		lock_array_nxt_free++;
	}
...
get_userattr_backend(const char *config_name)
{
	struct __nsw_switchconfig_v1 *config;
	enum __nsw_parse_err	err;

	config = __nsw_getconfig_v1(NSS_DBNAM_PASSWD_COMPAT, &err);

	/* The falls back to the default backend below. */
	if (config == 0)
		return (0);

	if (config->num_lookups >= 1) {
		struct __nsw_lookup_v1 *look;
		int len = sizeof ("files ");
		char *buf;

		for (look = config->lookups; look != NULL; look = look->next)
			len += strlen(look->service_name) + 1;

		buf = alloca(len);

		(void) strlcpy(buf, "files", len);
		for (look = config->lookups; look != NULL; look = look->next) {
			(void) strlcat(buf, " ", len);
			(void) strlcat(buf, look->service_name, len);
		}
		config = _nsw_getoneconfig_v1(config_name, buf, &err);
		return (config);
	}
...
_nss_db_state_constr(nss_db_initf_t initf, nss_db_root_t *rootp)
{
...
/* NSS_OPTIONS is undocumented and should be used without nscd running. */
	...
	if (! (s->p.flags & NSS_USE_DEFAULT_CONFIG)) {
		...
		if (config != 0 && strcmp(s->p.name, NSS_DBNAM_USERATTR) == 0 &&
		    config->num_lookups == 1 &&
		    strcmp(config->lookups[0].service_name, "compat") == 0) {
			(void) __nsw_freeconfig_v1(config);
			config = get_userattr_backend(config_name);
		}
	}
	if (config == 0) {
...
void
_nss_db_state_destr(struct nss_db_state *s, nss_db_root_t *rootp)
{

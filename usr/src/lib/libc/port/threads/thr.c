...
libc_init(void)
{
	 ...
	 * the stack size from /proc/self/rmap.
	 * If that fails, we just set it to 8 Mbytes.
	 */
	if (self->ul_stksiz == RLIM_INFINITY) {
		if ((self->ul_stksiz = proc_stack_size()) == 0)
			self->ul_stksiz = 8 * 1024 * 1024;
		self->ul_stk = (caddr_t)(self->ul_stktop - self->ul_stksiz);
	}
...
proc_stack_size(void)
{
	...
	if ((fd = __open("/proc/self/rmap", O_RDONLY, 0)) < 0 ||
	    fstat(fd, &statb) < 0)
		goto out;
	prmap = alloca(statb.st_size);
	if ((nmap = __read(fd, prmap, statb.st_size)) <= 0)
		goto out;
	for (nmap /= sizeof (prmap_t); nmap != 0; prmap++, nmap--) {
		if (prmap->pr_mflags & MA_STACK) {
			size = prmap->pr_size;
			break;
		}
	}

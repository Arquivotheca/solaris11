...
/* value in aio_returned to indicate that aio_return() has *not* been called */
#define	AIO_NOTRETURNED	0x6f
...
aio_read(aiocb_t *aiocbp)
{
	...
	if (_aio_sigev_thread(aiocbp) != 0)
		return (-1);
	aiocbp->aio_returned = AIO_NOTRETURNED;
...
aio_error(const aiocb_t *aiocbp)
{
	...
	sig_mutex_lock(&__aio_mutex);
	if (aiocbp->aio_returned != AIO_NOTRETURNED) {
		/* aio_return() was called or aiocb is uninitialized */
		sig_mutex_unlock(&__aio_mutex);
		errno = EINVAL;
		return (-1);
	}
...
aio_return(aiocb_t *aiocbp)
{
	 ...
	 * calls to aio_return() or aio_error() will return the fact that
	 * the result has already been returned.
	 */
	sig_mutex_lock(&__aio_mutex);
	if (aiocbp->aio_returned != AIO_NOTRETURNED) {
		/* aio_return() already called or aiocb is uninitialized */
		sig_mutex_unlock(&__aio_mutex);
		errno = EINVAL;
		return (-1);
	}
	aiocbp->aio_returned = 0;
...
aio_error64(const aiocb64_t *aiocbp)
{
	...
	if ((reqp = _aio_hash_del((aio_result_t *)resultp)) == NULL) {
		((aiocb64_t *)aiocbp)->aio_state = CHECKED;
		sig_mutex_unlock(&__aio_mutex);
	} else {
		((aiocb64_t *)aiocbp)->aio_state = NOCHECK;
		ASSERT(reqp->req_head == NULL);
		(void) _aio_req_remove(reqp);
		sig_mutex_unlock(&__aio_mutex);
		_aio_req_free(reqp);

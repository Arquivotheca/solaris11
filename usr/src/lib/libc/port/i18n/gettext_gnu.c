...
handle_lang(struct msg_pack *mp)
{
	...
	mid = prepare_msgfile(mp, &totallen, suffix, &suffix_len);
	if (mid == NULL)
		return (NULL);
	...
	while (*p) {
		...
		if (locale_len < 1 || totallen + locale_len >= MAXPATHLEN)
			continue;

		(void) memcpy(mid, op, locale_len);
		(void) memcpy(mid + locale_len, suffix, suffix_len);
	...
	while (*p) {
		...
		if (locale_len < 1 ||
		    totallen - suffix_len + locale_len >= MAXPATHLEN)
			continue;

		(void) memcpy(mid, op, locale_len);
		mid[locale_len] = '\0';

		alternative_locales(mid, &canonical, &start, &end, 1);

		if (start != -1) {
			for (; start <= end; start++) {
				locale_len =
				    strlen(__lc_obs_msg_lc_list[start]);

				if (totallen + locale_len > MAXPATHLEN)
					continue;

				(void) memcpy(mid,
				    __lc_obs_msg_lc_list[start], locale_len);
				(void) memcpy(mid + locale_len, suffix,
				    suffix_len);
		...
		if (canonical != NULL) {
			locale_len = strlen(canonical);

			if (totallen + locale_len > MAXPATHLEN)
				continue;

			(void) memcpy(mid, canonical, locale_len);
			(void) memcpy(mid + locale_len, suffix, suffix_len);
 ...
 */
static uint32_t *
do_conv(iconv_t fd, char *src, uint32_t srclen)
{
	uint32_t	tolen;
	uint32_t	*ptr, *optr;
    ...
    struct msg_pack *mp)
{
	...
	conv_dst = do_conv(gmnp->fd, msgstr, msgstr_len);

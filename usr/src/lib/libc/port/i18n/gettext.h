...
/*
 * this structure is used for preserving nlspath templates at
 * each Nls_node and also to use at _real_gettext_u().
 */
typedef struct nlstmp {
	char	*pathname;		/* the full pathname to file */
...
typedef struct nls_node {
	...
	Nlstmp	*ppaths;		/* value: expanded path */
...
#define	MSGFILESUFFIXLEN	(sizeof (MSGFILESUFFIX))
...
extern char	*prepare_msgfile(struct msg_pack *, size_t *, char *, size_t *);

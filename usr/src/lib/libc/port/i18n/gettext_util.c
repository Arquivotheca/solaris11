...
mk_msgfile(struct msg_pack *mp)
{
	...
	totallen = strlen(mp->binding);
	(void) memcpy(p, mp->binding, totallen);
	p += totallen;
	...
	catlen = category_name_len[mp->category];

	totallen += loclen + catlen + mp->domain_len + MSGFILESUFFIXLEN;
	if (totallen > MAXPATHLEN)
		return (NULL);

	(void) memcpy(p, mp->locale, loclen);
	p += loclen;

	(void) memcpy(p, category_name[mp->category], catlen);
	p += catlen;

	(void) memcpy(p, mp->domain, mp->domain_len);
	p += mp->domain_len;

	(void) memcpy(p, MSGFILESUFFIX, MSGFILESUFFIXLEN);
...
char *
prepare_msgfile(struct msg_pack *mp, size_t *totallen, char *suffix,
    size_t *suffix_len)
{
	char *p;
	size_t catlen;
	...
	*suffix_len = catlen + mp->domain_len + MSGFILESUFFIXLEN;

	*totallen += *suffix_len;
	if (*totallen > MAXPATHLEN)
		return (NULL);

	(void) memcpy(suffix, category_name[mp->category], catlen);
	suffix += catlen;

	(void) memcpy(suffix, mp->domain, mp->domain_len);
	suffix += mp->domain_len;

	(void) memcpy(suffix, MSGFILESUFFIX, MSGFILESUFFIXLEN);

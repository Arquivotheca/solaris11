...
/*
 * When we reach EOF on a file opened for update, we can change from
 * read to write without an intervening file positioning function;
 * _IOREAD, _IOEOF and _IORW must all be set.
 * We need to make sure that _cnt == 0, _base == _ptr and that _IOREAD
 * is unset.  When we reach EOF, _cnt and _ptr are properly set but in
 * the non XPG4 mode, EOF is not sticky and more chars may have been read
 * into the buffer so we need to reset _cnt and _ptr.
 * As we start to write, _IOEOF and _IOREAD need to be cleared and _IOWRT
 * needs to be set.
 */
#define	_READ2WRT(p)	do { \
				if ((p->_flag & (_IOREAD | _IOEOF | _IORW)) == \
				    (_IOREAD | _IOEOF | _IORW)) { \
					p->_cnt = 0; \
					p->_ptr = p->_base; \
				} \
				p->_flag &= ~(_IOREAD | _IOEOF); \
				p->_flag |= _IOWRT; \
			} while (__CHECK(p))

#ifdef __lint
#define	__CHECK(p)	(!(p->_flag & _IOWRT))
#else
#define	__CHECK(p)	(0)
#endif

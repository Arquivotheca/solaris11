		    ...
		    OM_uint32 *expire_rec /* The expire time received*/)
{
	...
	*cred = GSS_C_NO_CREDENTIAL;
	...
	if (desired_mechs != GSS_C_NO_OID_SET &&
	    !__OID_is_member(desired_mechs, &OID))
	 ...
	 * Diffie-Hellman mechanism
	 */
	if (mechs && (*minor = __OID_to_OID_set(mechs, &OID))) {
		Free(dh_cred);
		Free(netname);
		return (GSS_S_FAILURE);
	}
	...
	*minor = DH_SUCCESS;
	...
	if (cred == GSS_C_NO_CREDENTIAL) {
		if (!(*dh_keyopts.key_secretkey_is_set)())
			return (GSS_S_NO_CRED);
		if (name)
			*name = (gss_name_t)(*dh_keyopts.get_principal)();
		if (lifetime)
	...
	if (mechs &&
	    (*minor = __OID_to_OID_set(mechs, &OID)) != DH_SUCCESS) {
		free(name);
		return (GSS_S_FAILURE);
	}
			    ...
			    gss_cred_usage_t *usage /* cred usage */)
{
	...
	if (!__OID_equal(mech, &OID)) {
		*minor = DH_BAD_CONTEXT;
		return (GSS_S_FAILURE);
	}

	/* Fetch cred info */
	major = __dh_gss_inquire_cred(minor, cred, name,
	    &lifetime, &use, NULL);

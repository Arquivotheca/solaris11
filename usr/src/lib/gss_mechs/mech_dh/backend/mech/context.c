...
OM_uint32
/*ARGSUSED*/
__dh_gss_export_sec_context(OM_uint32 *minor,
...
OM_uint32
/*ARGSUSED*/
__dh_gss_import_sec_context(OM_uint32 *minor,
...
OM_uint32
/*ARGSUSED*/
__dh_gss_process_context_token(OM_uint32 *minor,
    gss_ctx_id_t context, gss_buffer_t token)
{
	return (GSS_S_UNAVAILABLE);
}
			...
			OM_uint32 *input_size /* Input size returned */)
{
	...
	if ((major = stat | __dh_gss_context_time(minor, context, &left))
	    != GSS_S_COMPLETE)
		return (major | stat);

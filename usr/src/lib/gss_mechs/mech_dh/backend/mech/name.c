		    ...
		    gss_name_t *output /* The converted internal name */)
{
	...
	*minor = DH_BADARG_FAILURE;
	...
	if ((mechoidlen = get_der_length(&p, currlen, &bytes)) < 0)
		return (GSS_S_DEFECTIVE_TOKEN);
	if (mechoidlen != OID.length)
		return (GSS_S_DEFECTIVE_TOKEN);
	if (memcmp(p, OID.elements, mechoidlen) != 0)
		return (GSS_S_DEFECTIVE_TOKEN);
	p += mechoidlen;
		    ...
		    gss_name_t *output /* The converted name */)
{
	...
	*output = GSS_C_NO_NAME;
	...
	if (__OID_equal(name_type, GSS_C_NT_MACHINE_UID_NAME)) {
		...
		stat = do_exported_netname(minor, input, output);
		return (stat);
	}
 ...
 */
OM_uint32
__dh_gss_release_name(OM_uint32 *minor, gss_name_t *name)
{
 ...
 */
OM_uint32
__dh_gss_inquire_names_for_mech(OM_uint32 *minor,
		...
		uid_t *uid  /* where to put the uid */)
{
	...
	*uid = UID_NOBODY;
	...
	glen = sysconf(_SC_NGROUPS_MAX);
	if (glen < NGRPS)
		glen = NGRPS;

	glist = alloca(glen * sizeof (gid_t));
		    ...
		    gss_buffer_t exported_name /* Exported name goes here */)
{
	...
	*minor = DH_SUCCESS;
	...
	namelen = OIDlen_len + __DH_GSS_C_NT_NETNAME->length
	    + strlen(pname)+1;
	oid_der_len = der_length_size(OID.length);
	/* Find the total length */
	len = tokid_len + OIDlen_len + mechoid_tag_len + oid_der_len
	    + OID.length + namelen_len + namelen;
	...
	*p++ = ((mechoid_tag_len + oid_der_len + OID.length)
	    & 0xff00) >> 8;
	*p++ = ((mechoid_tag_len + oid_der_len + OID.length)
	    & 0x00ff);
	...
	*p++ = 0x06; /* Universal Tag for OID */
	...
	if (!put_der_length(OID.length, &p, currlen) == 0) {
		return (GSS_S_FAILURE);
	}
 ...
 */
OM_uint32
__dh_gss_internal_release_oid(OM_uint32 *minor, gss_OID *oid)
{
	if (minor == 0)
		return (GSS_S_CALL_INACCESSIBLE_WRITE);

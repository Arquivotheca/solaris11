	    ...
	    dh_principal *netname /* Cred owner */)
{
	...
	*netname = NULL;
	...
	if ((*netname = dh_keyopts.get_principal()) == NULL) {
 ...
 */
static OM_uint32
establish_session_keys(const char *remote,
		    dh_key_set_t keys, dh_signature_t sig, dh_token_t token)
{
	...
	for (i = 0; key_was_from_cache && i < 2; i++) {
		...
		if (dh_keyopts.key_decryptsessions(remote,
		    keys->dh_key_set_val,
		    keys->dh_key_set_len,
		    &key_was_from_cache)) {
			Free(saved_keys);
			    ...
			    gss_cred_id_t *del_cred /* Delegated credential */)
{
	...
	*minor = 0;
	...
	if (*minor = __get_ap_token(input, &OID, &token, &sig)) {
		free(netname);
	...
	if ((*minor = establish_session_keys(clnt->cntx.remote,
	    &clnt->keys,
	    ...
	    gss_buffer_t result /* The serialized token to send */)
{
	...
	if (dh_keyopts.key_encryptsessions(cntx->remote,
	    keys.dh_key_set_val,
	    cntx->no_keys)) {
	 ...
	 * the token.
	 */
	stat =  __make_ap_token(result, &OID, &token, &ukeys);
	    ...
	    gss_buffer_t results /* Output token for the other side */)
{
	...
	if (dh_keyopts.key_gendeskeys(dh_gss_ctx->keys, 3)) {
		*minor = DH_NOMEM_FAILURE;
		goto cleanup;
	}
	...
	*minor = gen_init_token(dh_gss_ctx, channel, results);
	if (*minor != DH_SUCCESS)
		goto cleanup;
			...
			OM_uint32 *time_rec /* Actual life time received */)
{
	...
	*minor = DH_SUCCESS;
	...
	if (mech_rec)
		*mech_rec = &OID;   /* Note this should not be duped. */

	/* Check that were the right mechanism */
	if ((mech != GSS_C_NULL_OID) &&
	    (!__OID_equal(mech, &OID))) {
		return (GSS_S_BAD_MECH);
	}

	/* Validate the cred and obtain our netname in the process. */
	stat = validate_cred(minor, dh_cred, GSS_C_INITIATE, &netname);
	if (stat != GSS_S_COMPLETE)
		return (stat);
	...
	if (dh_gss_ctx == (dh_gss_context_t)GSS_C_NO_CONTEXT) {
		...
		stat =  create_context(minor, &dh_gss_ctx, netname,

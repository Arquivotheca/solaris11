			    ...
			    unsigned int *count, krb5_cksumtype **cksumtypes)
{
	...
	*count = 1;
	...
	if ((krb5int_cksumtypes_list[i].keyhash &&
	     etype_match(krb5int_cksumtypes_list[i].keyed_etype, enctype)) ||
	    (krb5int_cksumtypes_list[i].flags & KRB5_CKSUMFLAG_DERIVE)) {
	    c++;
	}
    }
    ...
    *count = c;
	...
	if ((krb5int_cksumtypes_list[i].keyhash &&
	     etype_match(krb5int_cksumtypes_list[i].keyed_etype, enctype)) ||
	    (krb5int_cksumtypes_list[i].flags & KRB5_CKSUMFLAG_DERIVE)) {
	    (*cksumtypes)[c] = krb5int_cksumtypes_list[i].ctype;
	    c++;
	}
    }

	...
	*output_cred_handle = NULL;
	...
	if ((cred_usage == GSS_C_INITIATE) ||
		...
		if ((ret = acquire_init_cred_with_pw(context, minor_status,
				cred->name ? cred->name : (krb5_gss_name_t)
	...
	if (!cred->name)
		if ((code = kg_duplicate_name(context, (krb5_gss_name_t)
				desired_name, 0, &cred->name))) {
			if (cred->ccache)
...
OM_uint32
gssspi_acquire_cred_with_password(minor_status, desired_name,

...
#define BOUNDS_CHECK(cp, end, n) do { if ((end) - (cp) < (n)) goto fail_name; } while (0)
            ...
            /*
             * Solaris Kerberos: this assert is inappropriate because the
             * length of the exported name object (cp points to the end of it)
             * may be smaller than the input_name_buffer->value.
             */
            /* assert(cp == end); */
        } else {
            xfree(tmp);
...
fail_name:
            ...
            krb5_set_error_message(context, *minor_status,
                                   dgettext(TEXT_DOMAIN,
                                            "Failed to convert the specified "
                                            "principal to GSS-API internal "
                                            "format"));
            save_error_info(*minor_status, context);

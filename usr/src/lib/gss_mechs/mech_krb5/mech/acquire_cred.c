                    ...
                    krb5_gss_cred_id_rec *cred)
{
    ...
    *output_name = NULL;
    ...
    if (code) {
        *minor_status = code;
        /*
         * Solaris Kerberos: GSS_S_CRED_UNAVAIL is not RFC 2743 compliant,
         * changed throughout this file
         */
    ...
    if (desired_name != NULL) {
        ...
        if ((code = krb5_kt_get_entry(context, kt, princ, 0, 0, &entry))) {
            /* Solaris Kerberos: better error msgs */
            if (code == KRB5_KT_NOTFOUND) {
                char *s_name;
                if (krb5_unparse_name(context, princ, &s_name) == 0) {
                    char kt_name[PATH_MAX];
                    if (krb5_kt_get_name(context, kt, kt_name, PATH_MAX) != 0)
			strlcpy(kt_name, "<unknown>", PATH_MAX);
                  ...
                  krb5_gss_cred_id_rec *cred)
{
    ...
    /*
     * Solaris Kerberos: Added calls to krb5_cc_set_flags(...
     * KRB5_TC_OPENCLOSE) on the error returns cuz the 1.4 krb5_cc_close does
     * not always close the file like it used to and caused STC test gss.27 to
     * fail.
     */

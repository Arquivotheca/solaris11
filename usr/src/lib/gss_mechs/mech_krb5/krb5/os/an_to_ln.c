...
default_an_to_ln(krb5_context context, krb5_const_principal aname, const unsigned int lnsize, char *lname)
{
    ...
    /*
     * Solaris Kerberos:  compare against default realm and
     * auth_to_local_realm(s)
     */
    if (!data_eq_string(*krb5_princ_realm(context, aname), def_realm) &&
        !an_to_ln_realm_chk(context, aname, def_realm, realm_length)) {
        free(def_realm);
        return KRB5_LNAME_NOTRANS;
    }
...
{
    krb5_error_code     kret;
    char                *realm = NULL; /* Solaris Kerberos */
    char                *pname;
    char                *mname;
    ...
    /*
     * Solaris Kerberos
     * Obtain the realm of the principal else use the default realm.
     */
    rsize = krb5_princ_realm(context, aname)->length;
    if (rsize > 0) {
	if ((realm = malloc(rsize + 1)) == NULL)
		return ENOMEM;
	memcpy(realm, krb5_princ_realm(context, aname)->data, rsize);
	realm[rsize] = '\0';
    } else
	kret = krb5_get_default_realm(context, &realm);
    if (realm != NULL) {
        /* Flatten the name */

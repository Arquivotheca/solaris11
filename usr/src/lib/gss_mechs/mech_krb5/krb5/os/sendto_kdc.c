...
/* Solaris Kerberos - dtrace probes */
                           ...
                           void *msg_handler_data)
{
    ...
    *retval = 0;
    ...
    if (krb5_rd_error(context, reply, &err_reply) == 0) {
...
/*
 * Solaris Kerberos An extra arg to return the FQDN of the KDC (hostname_used)
 * that sent the request and an arg for a DTrace probe (request).  Caller (at
 * top of stack) needs to free hostname_used.
 */
krb5_error_code
krb5_sendto_kdc (krb5_context context, const krb5_data *message,
		 const krb5_kdc_req *request, const krb5_data *realm,
		 krb5_data *reply, int *use_master, int tcp_only,
		 char **hostname_used)
{
    ...
    /* Solaris Kerberos */
    KERBEROS_PROBE_KRB_KDC_REQ(MAKE, message, request);
                  ...
                  krb5_data* callback_buffer)
{
    ...
    if (ai->ai_socktype == SOCK_DGRAM) {
	    ...
	    /* Solaris Kerberos */
	    k5_trace_message_send(state->fd, SG_BUF(sg), SG_LEN(sg));
            state->state = READING;
        }
    }
#ifdef DEBUG
            ...
            krb5_data* callback_buffer)
{
    ...
    /* Solaris Kerberos */
    k5_trace_message_send(conn->fd, SG_BUF(sg), SG_LEN(sg));
                ...
                int ssflags)
{
    ...
    try_writing:
	    ...
	    /* Solaris Kerberos */
	    k5_trace_message_send(conn->fd, SG_BUF(&conn->x.out.sgbuf[1]),
	        SG_LEN(&conn->x.out.sgbuf[1]));
    ...
    case READING:
	    ...
	    /* Solaris Kerberos */
	    k5_trace_message_recv(conn->fd, conn->x.in.buf, nread);
                /* We win!  */
                return 1;
            }
        } else {

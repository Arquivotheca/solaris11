...
{
    struct stat statbuf; /* Solaris Kerberos */

    /* Solaris Kerberos: return error if keytab doesn't exist sooner */
    if (stat(name, &statbuf) != 0) {
	return(errno);
    }

    return ktfile_common_resolve(context, name, id, &krb5_ktf_ops);
}
...
krb5_ktfileint_internal_read_entry(krb5_context context, krb5_keytab id, krb5_keytab_entry *ret_entry, krb5_int32 *delete_point)
{
	...
	/* Solaris Kerberos - Allow for empty components */
	if (u_princ_size > 0) {
...
/*
 * Solaris Kerberos
 * 183 resync - major changes from MIT so not merging in our changes.
 */
static krb5_error_code

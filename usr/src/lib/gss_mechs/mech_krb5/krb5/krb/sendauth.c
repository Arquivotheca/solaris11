              ...
              krb5_creds **out_creds)
{
    ...
    if (ap_req_options & AP_OPTS_USE_SUBKEY) {
        ...
        if (getpeername (*(int*)fd, (GETPEERNAME_ARG2_TYPE *) rnd_data,
                         &len2) == 0) {
            d.length = len2;
            /* Solaris Kerberos: don't need to add entropy  */
    ...
    if (retval == 0)
        k5_trace_message_send(*((int *)fd), outbuf[0].data, outbuf[0].length);
    free(outbuf[0].data);
    if (retval)
        goto error_return;
    ...
    if (inbuf.length) {
        /* Solaris Kerberos */
        k5_trace_message_recv(*((int *)fd), inbuf.data, inbuf.length);
        if (error) {

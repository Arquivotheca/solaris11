...
/* Solaris Kerberos */
krb5_error_code
krb5_copy_tickets(krb5_context context, krb5_ticket * const *in,
    krb5_ticket ***out)
{
    krb5_ticket **tkt;
    unsigned int i;
    ...
    tkt = calloc(i + 1, sizeof (krb5_ticket *));
    if (tkt == NULL)
        return ENOMEM;

    for (i = 0; in[i] != NULL; i++) {
        retval = krb5_copy_ticket(context, in[i], &tkt[i]);
        if (retval)
            break;
    }

    if (retval)
        krb5_free_tickets(context, tkt);
    else
        *out = tkt;

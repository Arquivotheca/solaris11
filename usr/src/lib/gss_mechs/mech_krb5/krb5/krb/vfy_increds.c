                       ...
                       krb5_verify_init_creds_opt *options)
{
    ...
    if ((ret = krb5_kt_get_entry(context, keytab, server, 0, 0, &kte))) {
         ...
         * Make sure krb5_verify_init_creds() returns an error by default.  We
         * differ from MIT in that their default value for
         * verify_ap_req_nofail/KRB5_CONF_VERIFY_AP_REQ_NOFAIL is false/0 while
         * Solaris treats verify_ap_req_nofail as true/1 by default.  Take care
         * to maintain Solaris behavior here.
         */
        int nofail = 1;
        ...
        if (options &&
            ...
            krb5int_libdefault_boolean(context,
                                       &creds->client->realm,
                                       KRB5_CONF_VERIFY_AP_REQ_NOFAIL,
                                       &nofail);
        }
        /*
         * nofail will be 0 ONLY if verify_ap_req_nofail is set to false
    ...
    if (krb5_principal_compare(context, server, creds->server)) {
        ...
        char *temp_realm = NULL; /* Solaris Kerberos */
        ...
        ret = profile_get_string(context->profile, KRB5_CONF_LIBDEFAULTS,
                                 KRB5_CONF_DEFAULT_REALM, NULL, NULL,
                                 &temp_realm);
        if (ret)
            goto cleanup;
        ...
        if (temp_realm == NULL) {
             ...
             * the server's.
             */
            if (!krb5_realm_compare(context, creds->client, server)) {

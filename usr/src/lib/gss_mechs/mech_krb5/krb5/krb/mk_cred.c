...
krb5_mk_ncred_basic(krb5_context context,
                    ...
                    krb5_cred *pcred,
                    krb5_cred_enc_part **ret_credenc)
{
...
cleanup:
    ...
    if (retval == 0 && ret_credenc != NULL) {
        *ret_credenc = malloc (sizeof (krb5_cred_enc_part));
        if (*ret_credenc != NULL)
            **ret_credenc = credenc;
    } else
              ...
              krb5_replay_data *outdata)
{
    ...
    /* Solaris Kerberos */
    krb5_cred_enc_part *credenc = NULL;
    ...
    if ((retval = krb5_mk_ncred_basic(context, ppcreds, ncred, key,
                                      ...
                                      premote_fulladdr, pcred, &credenc))) {
        goto error;
    }
    ...
    if (retval == 0)
        KERBEROS_PROBE_KRB_CRED(MAKE, *ppdata, pcred, credenc);

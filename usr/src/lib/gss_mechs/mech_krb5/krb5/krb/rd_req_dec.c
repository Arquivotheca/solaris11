...
rd_req_decoded_opt(krb5_context context, krb5_auth_context *auth_context,
                   const krb5_ap_req *req, const krb5_data *raw_req,
                   ...
                   krb5_ticket **ticket, int check_valid_flag)
{
    ...
    /* Solaris Kerberos */
    memset(&decrypt_key, 0, sizeof (krb5_keyblock));
    ...
    if ((*auth_context)->key) { /* User to User authentication */
        if ((retval = krb5_decrypt_tkt_part(context,
            ...
            KERBEROS_PROBE_KRB_AP_REQ(READ, raw_req, req, NULL, req->ticket);
            goto cleanup;
        }
        if (check_valid_flag) {
    ...
    if ((retval = decrypt_authenticator(context, req,
        ...
        goto cleanup;
    /* Solaris Kerberos dtrace support */
    KERBEROS_PROBE_KRB_AP_REQ(READ, raw_req, req, (*auth_context)->authentp,
                              req->ticket);
#endif
    ...
    if (retval == KRB5KRB_AP_ERR_TKT_NYV) {
        ...
        char *s_name = NULL;
        char *c_name = NULL;
	krb5_error_code serr, cerr, terr;
	char fill = ' ';
	char timestring[17];
	terr = krb5_timestamp_to_sfstring(req->ticket->enc_part2->times.starttime,
					timestring,
					sizeof (timestring),
					&fill);
	...
	char fill = ' ';
	char timestring[17];
	terr = krb5_timestamp_to_sfstring(req->ticket->enc_part2->times.endtime,
					timestring,
					sizeof (timestring),
					&fill);
	serr = krb5_unparse_name(context, req->ticket->server, &s_name);
...
/* Solaris Kerberos */
krb5_error_code
krb5_rd_req_decoded_ext(krb5_context context, krb5_auth_context *auth_context,
                        const krb5_ap_req *req, const krb5_data *raw_req,
                        krb5_const_principal server,
                        krb5_keytab keytab, krb5_flags *ap_req_options,
                        krb5_ticket **ticket)
...
/* Solaris Kerberos */
krb5_error_code
krb5_rd_req_decoded_anyflag_ext(krb5_context context,

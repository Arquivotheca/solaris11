...
{
    krb5_error_code retval;
    krb5_timestamp saved_endtime; /* Solaris Kerberos */

    TR_DBG(ts, "try_ccache");
    /*
     * Solaris Kerberos:
     * Ensure the retrieved cred isn't stale.  Set endtime to now so
     * krb5_cc_retrieve_cred won't return an expired ticket.
     */
...
find_nxt_kdc(struct tr_state *ts)
{
     ...
     * Solaris Kerberos:
     *
     * The following assertion is not be true for the case when ts->nxt points
     * to a cached ticket and not to a freshly fetched TGT in ts->kdc_tgts. See
     * changes in try_kdc()
     */
        ...
        if (!err) {
            krb5_set_error_message(ts->ctx, KRB5_KDCREP_MODIFIED,
                                   dgettext(TEXT_DOMAIN,
                                            ...
                                            "expectations: server '%s' "
                                            "principal size should be 2"),
                                   s_name);
    ...
    if (r1 != NULL && r2 != NULL &&
        ...
        krb5_set_error_message(ts->ctx, KRB5_KDCREP_MODIFIED,
                               dgettext(TEXT_DOMAIN,
                                        ...
                                        " KDC not found.  Probably got an "
                                        "unexpected realm referral"));
        return KRB5_KDCREP_MODIFIED;
    }
        ...
        if (server->length >= 2) {
            ...
            krb5_set_error_message(context, KRB5_ERR_HOST_REALM_UNKNOWN,
                                   dgettext(TEXT_DOMAIN,
                                            ...
                                            "Referral specified but no "
                                            "fallback realm available. Client "
                                            "is '%s' and Server is '%s'"),
            ...
            /* Solaris Kerberos end better errors */

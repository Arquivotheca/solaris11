...
# In addition to libcurses, and its compilation symlink, we also
# deliver libtermcap and libtermlib. These libraries are symlinks to
# libcurses.so.1.
#
EXTRA_LINKNAMES_NV=	libtermcap.so		libtermlib.so
EXTRA_LINKNAMES_V=	$(EXTRA_LINKNAMES_NV:%=%$(VERS))

EXTRA_ROOTLINKS_NV=	$(EXTRA_LINKNAMES_NV:%=$(ROOTLIBDIR)/%)
EXTRA_ROOTLINKS_V=	$(EXTRA_LINKNAMES_V:%=$(ROOTLIBDIR)/%)
EXTRA_ROOTLINKS=	$(EXTRA_ROOTLINKS_V) $(EXTRA_ROOTLINKS_NV)

EXTRA_ROOTLINKS64_NV=	$(EXTRA_LINKNAMES_NV:%=$(ROOTLIBDIR64)/%)
EXTRA_ROOTLINKS64_V=	$(EXTRA_LINKNAMES_V:%=$(ROOTLIBDIR64)/%)
EXTRA_ROOTLINKS64=	$(EXTRA_ROOTLINKS64_V) $(EXTRA_ROOTLINKS64_NV)

EXTRA_STUBROOTLINKS_NV=	$(EXTRA_LINKNAMES_NV:%=$(STUBROOTLIBDIR)/%)
EXTRA_STUBROOTLINKS_V=	$(EXTRA_LINKNAMES_V:%=$(STUBROOTLIBDIR)/%)
EXTRA_STUBROOTLINKS=	$(EXTRA_STUBROOTLINKS_V) $(EXTRA_STUBROOTLINKS_NV)

EXTRA_STUBROOTLINKS64_NV= $(EXTRA_LINKNAMES_NV:%=$(STUBROOTLIBDIR64)/%)
EXTRA_STUBROOTLINKS64_V= $(EXTRA_LINKNAMES_V:%=$(STUBROOTLIBDIR64)/%)
EXTRA_STUBROOTLINKS64=	$(EXTRA_STUBROOTLINKS64_V) $(EXTRA_STUBROOTLINKS64_NV)

$(EXTRA_ROOTLINKS_V) : $(ROOTLIBDIR)/libcurses.so$(VERS)
	$(RM) $@
	$(SYMLINK) libcurses.so$(VERS) $@

$(EXTRA_ROOTLINKS64_V) : $(ROOTLIBDIR64)/libcurses.so$(VERS)
	$(RM) $@
	$(SYMLINK) libcurses.so$(VERS) $@

$(EXTRA_STUBROOTLINKS_V) : $(STUBROOTLIBDIR)/libcurses.so$(VERS)
	$(RM) $@
	$(SYMLINK) libcurses.so$(VERS) $@

$(EXTRA_STUBROOTLINKS64_V) : $(STUBROOTLIBDIR64)/libcurses.so$(VERS)
	$(RM) $@
	$(SYMLINK) libcurses.so$(VERS) $@

$(ROOTLIBDIR)/libterm%.so : $(ROOTLIBDIR)/libterm%.so$(VERS)
	$(RM) $@
	$(SYMLINK) $(<F) $@

$(ROOTLIBDIR64)/libterm%.so : $(ROOTLIBDIR64)/libterm%.so$(VERS)
	$(RM) $@
	$(SYMLINK) $(<F) $@

$(STUBROOTLIBDIR)/libterm%.so : $(STUBROOTLIBDIR)/libterm%.so$(VERS)
	$(RM) $@
	$(SYMLINK) $(<F) $@

$(STUBROOTLIBDIR64)/libterm%.so : $(STUBROOTLIBDIR64)/libterm%.so$(VERS)
	$(RM) $@
	$(SYMLINK) $(<F) $@


# install rule for lint library target
$(ROOTLINTDIR)/%	$(ROOTLINTDIR64)/% : ../screen/%
	$(INS.file)

# We also deliver lint libraries for libtermcap and libtermlib that
# are really links to those for libcurses.
EXTRA_LINTNAMES=	llib-ltermcap	llib-ltermcap.ln \
			llib-ltermlib	llib-ltermlib.ln

EXTRA_ROOTLINTLINKS=	$(EXTRA_LINTNAMES:%=$(ROOTLINTDIR)/%)
EXTRA_ROOTLINTLINKS64=	$(EXTRA_LINTNAMES:%=$(ROOTLINTDIR64)/%)

$(ROOTLINTDIR)/llib-ltermcap% \
$(ROOTLINTDIR)/llib-ltermlib% : $(ROOTLINTDIR)/llib-lcurses%
	$(RM) $@
	$(SYMLINK) $(<F) $@

$(ROOTLINTDIR64)/llib-ltermcap% \
$(ROOTLINTDIR64)/llib-ltermlib% : $(ROOTLINTDIR64)/llib-lcurses%
	$(RM) $@
	$(SYMLINK) $(<F) $@

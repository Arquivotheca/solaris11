...
i_ipadm_ngz_addr(ipadm_handle_t iph, char *link, char *buf)
{
	...
	ipstatus = ipadm_create_ip(iph, link, AF_UNSPEC, IPADM_OPT_ACTIVE);
	if (ipstatus != IPADM_SUCCESS && ipstatus != IPADM_IF_EXISTS) {
		return (ipstatus);
	}
 ...
 * were made available for exclusive IP non-global zones by zoneadmd from the
 * global zone.
 */
static ipadm_status_t
...
i_ipadm_zone_network_attr(dladm_handle_t dh, datalink_id_t linkid, void *arg)
{
	...
	int rtsock = iph->ih_rtsock;
	char *ifname = nwd->ngz_ifname;
	boolean_t s10c = nwd->ngz_s10c;
	boolean_t is_ipmgmtd = (iph->ih_flags & IH_IPMGMTD);
    ...
    void (*persist_if)(char *, boolean_t, boolean_t))
{
	...
	nwd.ngz_iph = iph;
	nwd.ngz_zoneid = iph->ih_zoneid;
	nwd.ngz_ifname = ifname;
	nwd.ngz_persist_if = persist_if;
	nwd.ngz_s10c = i_ipadm_zone_is_s10c(iph->ih_zoneid);

 ...
 *
 *  IPADM_NVP_DINTFID - value is a single entity of type DATA_TYPE_UINT8_ARRAY
 *	       that represents a destination token for stateless
 *	       auto-configuration.
 *
...
static ipadm_conf_ent_t ipadm_conf_ent[] = {
	...
	{ IPADM_NVP_DINTFID, i_ipadm_dintfid_nvp2dbline,
	    i_ipadm_dintfid_dbline2nvl },
...
/*
 * Converts IPADM_NVP_DINTFID nvpair to a string representation for writing to
 * the DB. The converted string format:
 *	IPADM_NVP_DINTFID=dintfid
 */
static size_t
i_ipadm_dintfid_nvp2dbline(nvpair_t *nvp, char *buf, size_t buflen)
{
	char		addrbuf[IPADM_STRSIZE];
	struct in6_addr	in6addr;
	uint8_t		*addr6;
	...
	assert(nvpair_type(nvp) == DATA_TYPE_UINT8_ARRAY &&
	    strcmp(nvpair_name(nvp), IPADM_NVP_DINTFID) == 0);

	(void) snprintf(buf, buflen, "%s=", IPADM_NVP_DINTFID);
	if (nvpair_value_uint8_array(nvp, &addr6, &n) != 0)
		goto fail;
	bcopy(addr6, in6addr.s6_addr, n);
	(void) inet_ntop(AF_INET6, &in6addr, addrbuf, sizeof (addrbuf));
	return (strlcat(buf, addrbuf, buflen));
fail:
	buf[0] = '\0';
	return (0);
 ...
 */
static void
i_ipadm_str_dbline2nvl(nvlist_t *nvl, const char *name, char *value)
{
 ...
 */
static void
i_ipadm_ip4_dbline2nvl(nvlist_t *nvl, const char *name, char *value)
{
	char			*cp, *hname;
 ...
 */
static void
i_ipadm_ip6_dbline2nvl(nvlist_t *nvl, const char *name, char *value)
{
	char			*cp, *hname;
	struct ipadm_addrobj_s	ipaddr;
 ...
 */
static void
i_ipadm_intfid_dbline2nvl(nvlist_t *nvl, const char *name, char *value)
{
	char			*cp;
	struct ipadm_addrobj_s	ipaddr;
	char			*endp;
...
/*
 * `name' = IPADM_NVP_DINTFID and `value' = intfid
 * This function will add an nvlist with the address object information in
 * nvpairs to the nvlist in `nvl'.
 */
static void
i_ipadm_dintfid_dbline2nvl(nvlist_t *nvl, const char *name, char *value)
{
	struct ipadm_addrobj_s	ipaddr;

	assert(strcmp(name, IPADM_NVP_DINTFID) == 0 && value != NULL);
	bzero(&ipaddr, sizeof (ipaddr));
	ipaddr.ipadm_dintfid.sin6_family = AF_INET6;
	(void) inet_pton(AF_INET6, value, &ipaddr.ipadm_dintfid.sin6_addr);

	/* Add it to the nvlist */
	(void) nvlist_add_uint8_array(nvl, IPADM_NVP_DINTFID,
 ...
 */
static void
i_ipadm_dhcp_dbline2nvl(nvlist_t *nvl, const char *name, char *value)
{
	char		*cp;
	char		*endp;
	long		wait_time;

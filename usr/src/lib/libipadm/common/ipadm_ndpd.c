...
ipadm_status_t
i_ipadm_set_linklocal(ipadm_handle_t iph, ipadm_addrobj_t addr)
{
	struct sockaddr_in6	*sin6;
	struct lifreq		lifr;
	int			err;
	ipadm_status_t		status;
	in6_addr_t		ll_template = {0xfe, 0x80, 0x0, 0x0, 0x0, 0x0,
	...
	if ((err = ioctl(iph->ih_sock6, SIOCGLIFFLAGS, (caddr_t)&lifr)) < 0)
		goto fail;
	plen = (lifr.lifr_flags & IFF_POINTOPOINT) ? IPV6_ABITS :
	    PREFIXLEN_LINKLOCAL;

	/* Create the link-local address */
	bzero(&lifr.lifr_addr, sizeof (lifr.lifr_addr));
	(void) plen2mask(plen, AF_INET6, (struct sockaddr *)&lifr.lifr_addr);
	if ((err = ioctl(iph->ih_sock6, SIOCSLIFNETMASK, (caddr_t)&lifr)) < 0)
		goto fail;
	if (addr->ipadm_intfidlen == 0) {
		...
		err = ioctl(iph->ih_sock6, SIOCSLIFPREFIX, (caddr_t)&lifr);
		if (err < 0)
			goto fail;
	} else {
		/* Make a linklocal address in sin6 and set it */
		i_ipadm_make_linklocal(sin6, &addr->ipadm_intfid.sin6_addr);
		err = ioctl(iph->ih_sock6, SIOCSLIFADDR, (caddr_t)&lifr);
		if (err < 0)
			goto fail;
	}

	/* Set the destination address, if one is given. */
	if (addr->ipadm_dintfid.sin6_family != AF_UNSPEC) {
		sin6 = (struct sockaddr_in6 *)&lifr.lifr_addr;
		i_ipadm_make_linklocal(sin6, &addr->ipadm_dintfid.sin6_addr);
		err = ioctl(iph->ih_sock6, SIOCSLIFDSTADDR, (caddr_t)&lifr);
...
fail:
	...
	(void) ioctl(iph->ih_sock6, SIOCSLIFFLAGS, (caddr_t)&lifr);
	sin6->sin6_family = AF_INET6;
	sin6->sin6_addr = in6addr_any;
	(void) ioctl(iph->ih_sock6, SIOCSLIFADDR, (caddr_t)&lifr);
...
retry:
	...
	i_ipadm_addrobj2lifname(addr, lifname, sizeof (lifname));

	status = i_ipadm_set_linklocal(iph, addr);
	if (status != IPADM_SUCCESS)
		goto fail;

	status = i_ipadm_set_flags(iph, lifname, AF_INET6, IFF_UP, 0);
	if (status != IPADM_SUCCESS)
		goto fail;
...
fail:
	/*
	 * Remove the logical interface if one was created.
	 */
	if (addr->ipadm_lifnum != 0) {
		...
		(void) ioctl(iph->ih_sock6, SIOCLIFREMOVEIF, (caddr_t)&lifr);
	}
    ...
    int cmd)
{
	...
	if (addr == NULL && (cmd == IPADM_CREATE_ADDRS ||
	    cmd == IPADM_DELETE_ADDRS || cmd == IPADM_MODIFY_ADDRS)) {
	...
	if (addr != NULL) {
		...
		msg.inm_dintfid = addr->ipadm_dintfid;
		if (cmd != IPADM_MODIFY_ADDRS) {
	...
	if (ipadm_ndpd_write(fd, &msg, sizeof (msg)) < 0 ||
	    ipadm_ndpd_read(fd, &retval, sizeof (retval)) < 0)
		goto fail;
	(void) close(fd);
...
fail:
	...
	case ETIME:
	case ETIMEDOUT:
		status = IPADM_NDPD_TIMEOUT;
		break;
	case EIO:
		status = IPADM_NDPD_IO_FAILURE;
		break;
	default:
		status = ipadm_errno2status(errno);
		break;

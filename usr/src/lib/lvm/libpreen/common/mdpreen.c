...
get_major_from_n2m(char *modname, int *major)
{
	...
	while ((fgets(line, sizeof (line), fp) != NULL) && status == 0) {
...
)
{
	...
	if (md_major == -1 && get_major_from_n2m(MD_MODULE, &md_major) != 0)
		return;
	...
	for (p = nlp; (p != NULL); p = p->next) {
		...
		if (((md_did = meta_getdidbykey(sp->setno, sideno,
			 ...
			 */
			if (strstr(minor_name, ",blk") != NULL) {
				tmpstr = strrchr(minor_name, ',');
				*tmpstr = '\0';
			}

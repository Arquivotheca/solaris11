...
)
{
		...
		if (misp->mis_partial) {
			...
			if (midp == NULL) {
				(void) mddserror(ep, MDE_DS_SETNOTIMP,
				    MD_SET_BAD, nodename, NULL, sp->setname);
				rval = -1;
				goto out;
			}
		}
	 ...
	 */
	(void) memset(&mhiargs, '\0', sizeof (mhiargs));
	if (clnt_gtimeout(nodename, sp, &mhiargs, ep) == -1) {
		rval = -1;
		goto out;
	}
	...
	if (mnset != 0) {
		if (clnt_mnsetmaster(nodename, sp,
		    sd->sd_mn_mynode->nd_nodename,
		    sd->sd_mn_mynode->nd_nodeid, ep))
			goto rollback;
		...
		(void) strncpy(sd->sd_mn_master_nodenm,
		    sd->sd_mn_mynode->nd_nodename,
		    MD_MAX_NODENAME_PLUS_1);

		if (clnt_adddrvs(nodename, sp, dd, now, genid, ep))
			goto rollback;

		if (clnt_upd_nr_flags(nodename, sp, sd->sd_nodelist,
		    MD_NR_OK, NULL, ep))
			goto rollback;

		if (clnt_upd_dr_flags(nodename, sp, dd, MD_DR_OK, ep))
			goto rollback;
	 ...
	 */
	if (clnt_imp_adddrvs(nodename, sp, dd, now, genid, ep) == -1)
		goto rollback;
...
rollback:
	...
	/* level 1 */
	if (rb_level > 0) {
		if (clnt_deldrvs(nodename, sp, dd, &xep) == -1) {
			mdclrerror(&xep);
		}
		max_genid += 2;
		resync_genid(sp, sd, max_genid, 0, NULL);

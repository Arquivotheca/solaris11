...
)
{
	...
	crc_skip_t	*skiptail = NULL;
	int		is_32bit_record;
	int		mnset = (mbp->mb_revision == MDDB_REV_MNMB);
	...
	if (mnset != 0) {
		skiptail = (crc_skip_t *)Malloc(sizeof (crc_skip_t));
		skiptail->skip_next = NULL;
		skiptail->skip_offset = offsetof(mddb_rb_t, rb_timestamp);
		skiptail->skip_size = sizeof (struct timeval32);
		skip->skip_next = skiptail;

		if (dep->de_type1 >= MDDB_FIRST_MODID) {
			switch (dep->de_flags) {
				case MDDB_F_STRIPE:
					skiptail->skip_next =
					    stripe_skip_ts((void *)
					    (((mddb_rb_t *)rbp)->rb_data),
					    ((mddb_rb_t *)rbp)->rb_revision);
					break;
				case MDDB_F_MIRROR:
					skiptail->skip_next =
					    mirror_skip_ts(

...
)
{
	...
	if (options & MDCMD_DOIT) {
		...
		if (! mdisok(ep))
			return (-1);
		shs.shs_hot_spare_pool = meta_gethspnmentbyname(sp->setno,
		    thisside, hspnp->hspname, ep);
		if (! mdisok(ep)) {
			/*
			...
			if (is_existing_metadevice(sp, hspnp->hspname))
				return (mderror(ep, MDE_NAME_IN_USE,
				    hspnp->hspname));
			if ((shs.shs_hot_spare_pool = add_hsp_name(sp,
			    hspnp->hspname, ep)) == MD_HSPID_WILD) {
				return (-1);
			}
		}
	}
	...
	if (hsnlp == NULL) {
		...
		if (metaioctl(MD_IOCSET_HS, &shs, &shs.mde,
		    hspnp->hspname) != 0) {
			if (options & MDCMD_DOIT) {
				(void) del_hsp_keys(sp, shs.shs_hot_spare_pool,
				    &ignore_error);
			}

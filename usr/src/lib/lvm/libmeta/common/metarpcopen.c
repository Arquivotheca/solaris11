...
)
{
	...
	(*cachep)[i]->cc_priv_ip = Strdup(priv_ip);
	(*cachep)[i]->cc_clp = clntp;
	(*cachep)[i]->cc_ttl = now;
...
)
{
	...
	(void) strlcpy(host_priv, hostname, strlen(hostname) +1);
	/*
	...
	if (sdssc_bind_library() != SDSSC_NOT_BOUND) {
		if (sdssc_get_priv_ipaddr(hostname, &p_ip) == SDSSC_OKAY) {
			...
			if (host_sc != NULL) {
				...
				(void) strlcpy(host_priv, host_sc, size);
			}
		}
	}
	 ...
	 */
	clntp = meta_client_create_retry(host_priv, client_create_vers_retry,
	...
	if (clntp == NULL) {
		...
		cc_add(&client_header, hostname, host_priv, NULL, ep);
		return (NULL);
	} else {
		auth_destroy(clntp->cl_auth);
	...
	cc_add(&client_header, hostname, host_priv, clntp, ep);

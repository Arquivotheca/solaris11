...
/*
 * During metaimport of a MN set, it's possible that the old set
 * number and the new one will not match.  In this case, the selfid
 * field needs to be updated to reflect the new set number otherwise
 * messages won't be logged.
 *
 * Called from metaimport
 * Return values:
 *	0 - success
 *	1 - failure
 */
int
mdmn_update_changelog(set_t setno, md_error_t *ep)
{
	mdmn_changelog_record_od_t	*ondisk;
	mddb_recid_t			id;

	id = 0;
	while ((ondisk = (mdmn_changelog_record_od_t *)get_ur_rec(setno,
	    MD_UR_GET_NEXT, MDDB_UR_LR, &id, ep)) != NULL) {
		mdmn_changelog_record_t		incore;

		copy_changelog(&incore, ondisk, MD_MN_COPY_TO_INCORE);
		incore.lr_selfid = MAKERECID(setno, id);

		if (mdmn_log_it(setno, ep, &incore) != 0)
			return (1);
	}

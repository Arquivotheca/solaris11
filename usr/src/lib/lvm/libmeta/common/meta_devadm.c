...
)
{
	...
	if (!metaislocalset(sp)) {
		mda_debug("Updating the ctd names for set: %s\n", sp->setname);
		...
		mda_debug("Updating the ctd names for the local set\n");
	}
	...
	for (snp = dnp->side_names; snp != NULL; snp = snp->next) {
		...
		/*
		 * Only use SKEW for the local replica and for traditional
		 * disksets.
		 */
		if (setno == 0 && mn_set == 0) {
	...
	mda_debug("Side name: sideno: %d dname: %s cname: %s minor: %lx\n",
	    snp->sideno, snp->dname, snp->cname, snp->mnum);
	...
	if ((nm.devname = (uintptr_t)meta_getnmentbykey(setno, sideno,
		...
		mda_debug("No devname with setno: %d sideno: %d key %d\n",
		    setno, sideno, dnp->side_names_key);
		return (METADEVADM_ERR);
	}
	...
	if (ret != 0) {
		 ...
		 * Failed to encode the devid.
		 */
		mda_debug("Failed to decode the devid string: %s",
		    (char *)(uintptr_t)nm.devname);
		devid_free(devidp);
		return (METADEVADM_ERR);
	}
...
)
{
	...
	dev_options |= options;
	mda_debug("dev_options: %x mn_set: %d\n", dev_options, mn_set);
	if (dev_options & DEV_RELOAD) {
		...
		if (setno != MD_LOCAL_SET) {
			...
			/*
			 * Only add a SKEW to traditional disksets because
			 * multi-owner disksets use the node id for the side.
			 */
			if (mn_set)
				sideno = getmyside(sp, ep);
			else
			...
			mda_debug("meta_fixdevid: sideno: %d\n", sideno);

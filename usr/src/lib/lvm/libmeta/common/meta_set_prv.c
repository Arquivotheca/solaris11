 ...
 */
int
meta_devid_supported_in_did(void)
{
	static int idevid = MD_DEVID_NO_INIT;
	md_error_t mderr;
	...
	mderr.extra = NULL;
	mderr.name = NULL;
	if (metaioctl(MD_DID_DEVID, &idevid, &mderr, NULL) != 0)
		idevid = MD_DEVID_NOT_SUPPORTED;
...
setup_db_bydd(mdsetname_t *sp, md_drive_desc *dd, int force, md_error_t *ep)
{
	...
	for (p = dd; p != NULL; p = p->dd_next) {
		 ...
		 */
		if (dnp->devid == NULL || meta_devid_supported_in_did()
		    == MD_DEVID_NOT_SUPPORTED)
			use_devid = 0;

...
extern	int	meta_devid_supported_in_did(void);
...
getmyside(mdsetname_t *sp, md_error_t *ep)
{
	...
	/*
	 * We may not have a nodename yet if early in boot and using
	 * auto take sets, so find side based on name in set.
	 */
	node = mynode();

	if ((strcmp(node, "") == 0) && (sd->sd_flags & MD_SR_AUTO_TAKE)) {
		int j;

		if ((sr = metad_getsetbynum(sp->setno, ep)) == NULL)
			return (MD_SIDEWILD);
		/*
		 * We know there can only be one host in an auto take set
		 * but there are cases where it is not side 0, so check.
		 */
...
)
{
	 ...
	 * call would have found any issues with the device.
	 */
...
)
{
	 ...
	 */
	if (MD_MNSET_DESC(sd)) {
		devidp = meta_getdidbykey(MD_LOCAL_SET, sideno, key, ep);
	} else {
		devidp = meta_getdidbykey(MD_LOCAL_SET, sideno+SKEW, key, ep);
	}
	...
	if (devidp != NULL) {
		...
		if (MD_MNSET_DESC(sd)) {
			nm = meta_getnmbykey(MD_LOCAL_SET, sideno, key, ep);
		} else {
			nm = meta_getnmbykey(MD_LOCAL_SET, sideno+SKEW, key,
			    ep);
		}
		if (nm == NULL)
			return (NULL);
		...
		dnp->side_names_key = key;
		/*
		 * Skip the following devid set/check if dnp is a did device
		 * and the Sun Cluster did driver does not support devids.
		 */
		if (clboot & CLUSTER_CONFIGURED &&
		    meta_devid_supported_in_did() == MD_DEVID_NOT_SUPPORTED) {
			goto out;
		}
		...
		/*
		 * The devid is missing.  This means that we have just
		 * upgraded from a configuration where devids were not
		 * used so try to add in the devid and requery. If the
		 * devid still isn't there, that's OK. dnp->devid will
		 * be null as it is in any configuration with no devids.
		 */

		if (MD_MNSET_DESC(sd)) {
			ret = meta_setdid(MD_LOCAL_SET, sideno, key, ep);
		} else {
			ret = meta_setdid(MD_LOCAL_SET, sideno + SKEW, key, ep);
		}
		if (ret < 0)
			return (NULL);

		if (MD_MNSET_DESC(sd)) {
			devidp = meta_getdidbykey(MD_LOCAL_SET, sideno, key,
			    ep);
		} else {
			devidp = meta_getdidbykey(MD_LOCAL_SET, sideno + SKEW,
			    key, ep);
		}
...
)
{
	...
	if (!(sd->sd_mn_mynode->nd_flags & MD_MN_NODE_OWN)) {
		...
		/*
		 * The following call will solve a problem that has
		 * been present but harmless in the post S10 development
		 * release.  The set creation times stored in the master
		 * blocks of MN sets all match except for the first one,
		 * which was set to the epoch.  If the mbs don't all have
		 * matching set create times, metaimport reports bogus
		 * conflicts by matching on the epoch.
		 */
		(void) meta_update_mb(sp, dd, ep);

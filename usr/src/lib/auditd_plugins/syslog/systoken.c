...
path_attr_token(parse_context_t *ctx)
{
	...
	adrm_int32(&(ctx->adr), &string_cnt, 1);
	...
	path_lens = p - ctx->adr.adr_now;

	if ((p = realloc(ctx->out.sf_atpath, path_lens)) == NULL) {
		return (-1);
	}
	ctx->out.sf_atpath = p;
	adrm_char(&(ctx->adr), ctx->out.sf_atpath, (int)path_lens);
	ctx->out.sf_atpathlen = path_lens;

...
setmask(const char *flags)
{
	...
	if ((mask.am_success | mask.am_failure) == AU_MASK_NONE) {
		rc = AUDITD_INVALID;
 ...
 * Here, non-zero and -1 are considered equivalent.
 * tossit() returns 1 for delete it and 0 for keep it.
 */
    ...
    size_t in_len, size_t out_len)
{
	...
	while ((ctx.adr.adr_now - ctx.adr.adr_stream) < in_len) {
		...
		if ((parse_rc = parse_token(&ctx)) != 0) {
			...
			DPRINT((dbfp, "error before token %d (previous "
			    "token=%d) of record type %s%s\n", token_count,
			    parse_rc, event_name, sequence_str));

			__audit_syslog("audit_syslog.so",
			    LOG_PID | LOG_ODELAY | LOG_CONS,
			    LOG_DAEMON, LOG_ALERT, "error before token %d "

...
static int smb_ads_get_dsattr(LDAP *, const char *);
...
static int smb_ads_update_spn_attr(smb_ads_handle_t *, char *, const char *);
static void smb_ads_upgrade_failure(smb_ads_handle_t *, char *);
static char *smb_ads_get_principal(const char *, const char *);

/*
 * Transient thread performs the following upgrade for supporting Kerberos
 * user authentication:
 *
 * Service principals introduced by PSARC/2009/673 case will be registered with
 * AD and the associated keys will also be added to the local keytab.
 *
 * On error, smb/server will enter maintenance mode; administrator intervention
 * will be required.
 */
/*ARGSUSED*/
void *
smb_ads_upgrade(void *args)
{
	char		fqdn[MAXHOSTNAMELEN];
	krb5_kvno	kvno;
	int		dclevel, domainlevel, level;
	smb_ads_handle_t *ah = NULL;
	char		dn[SMB_ADS_DN_MAX];
	uint32_t	flags;

	(void) smb_getdomainname_ad(fqdn, sizeof (fqdn));
	if ((ah = smb_ads_open()) == NULL) {
		(void) fprintf(stderr,
		    "smbd: unable to connect to domain controller\n");
		smb_ads_upgrade_failure(ah, fqdn);
		return (NULL);
	}

	if (smb_ads_find_computer(ah, dn) != SMB_ADS_STAT_FOUND) {
		(void) fprintf(stderr, "smbd: no workstation trust account\n");
		smb_ads_upgrade_failure(ah, fqdn);
		return (NULL);
	}

	if (smb_ads_update_spn_attr(ah, dn, fqdn) == -1) {
		(void) fprintf(stderr,
		    "smbd: unable to update SPN attribute of the system's "
	...
	dclevel = smb_ads_get_dsattr(ah->ld, DS_ATTR_DCLEVEL);
	domainlevel = smb_ads_get_dsattr(ah->ld, DS_ATTR_DOMAINLEVEL);

	if ((dclevel == -1) && (domainlevel == -1)) {
		(void) fprintf(stderr,
		    "smbd: unknown domain controller level and domain level\n");
		smb_ads_upgrade_failure(ah, fqdn);
		return (NULL);
	}
	...
	if (dclevel == -1) {
		...
		level = (dclevel < domainlevel) ? dclevel : domainlevel;
	}

	flags = DS_SUPPORT_AES(level) ? K5_KT_FLAG_AES_SUPPORT : 0;
	if (smb_krb5_kt_update_startup(fqdn, kvno, flags) != 0) {
		(void) fprintf(stderr,
		    "smbd: unable to update Kerberos keytab\n");
		smb_ads_upgrade_failure(ah, fqdn);
		return (NULL);
	}
...
static void
smb_ads_upgrade_failure(smb_ads_handle_t *ah, char *fqdn)
{
	(void) fprintf(stderr, "smbd: authentication upgrade failed. "
	    "Please rejoin domain %s\n", fqdn);
...
/*
 * Update SPN attribute of the machine account in the AD to ensure CIFS
 * Servcie can be run under the system account security context.
 */
static int
smb_ads_update_spn_attr(smb_ads_handle_t *ah, char *dn, const char *fqdn)
{
	smb_krb5_pn_set_t spns;
	uint32_t flags = SMB_PN_SPN_ATTR;
	LDAPMod *attrs[2];
	int rc = 0;

	if (smb_krb5_get_pn_set(&spns, flags, (char *)fqdn) == 0)
		return (-1);

	if (smb_ads_alloc_attr(attrs, 2) != 0) {
		smb_krb5_free_pn_set(&spns);
		return (-1);
	}

	attrs[0]->mod_op = LDAP_MOD_REPLACE;
	attrs[0]->mod_type = SMB_ADS_ATTR_SPN;
	attrs[0]->mod_values = spns.s_pns;

	if ((rc = ldap_modify_s(ah->ld, dn, attrs)) != LDAP_SUCCESS) {
		syslog(LOG_NOTICE, "SPN attribute update failed: %s",
		    ldap_err2string(rc));
		rc = -1;
	}
    ...
    const char *hostname, const char *shareUNC)
{
	...
	if (smb_getdomainname_dns(my_domain, sizeof (my_domain)) != 0)
		return (-1);
...
smb_ads_validate_cache_host(char *domain, char *srv)
{
	...
	if (smb_strcasecmp(smb_ads_cached_host_info->domain, domain, 0) != 0)
		return (B_FALSE);
...
{
	char site[SMB_ADS_SITE_MAX];

	(void) smb_config_getstr(SMB_CI_ADS_SITE,
	    site, SMB_ADS_SITE_MAX);

	if (*site == '\0')
		*site_service = '\0';
	else
		(void) snprintf(site_service, len,
		    SMB_ADS_MSDCS_SRV_SITE_RR, site);
}
...
update_cache:
	if (host) {
		(void) strlcpy(host->domain, domain, sizeof (host->domain));
		(void) mutex_lock(&smb_ads_cached_host_mtx);
		if (!smb_ads_cached_host_info)
...
/*
 * Discovers DCs via SRV RR lookup and return an array of DCs terminated by a
 * NULL pointer.
 */
char **
smb_ads_get_kdcs(char *fqdomain)
{
	smb_ads_host_list_t *list;
	int i;
	char **dcs;

	list = smb_ads_query_dns_server(fqdomain, "_kerberos._tcp");

	if ((list == NULL) &&
	    (list = smb_ads_query_dns_server(fqdomain,
	    SMB_ADS_MSDCS_SRV_DC_RR)) == NULL)
			return (NULL);

	if ((dcs = calloc(list->ah_cnt + 1, sizeof (char *))) == NULL)
		return (NULL);

	for (i = 0; i < list->ah_cnt; i++)
		dcs[i] = strdup(list->ah_list[i].name);

	smb_ads_hlist_free(list);
	return (dcs);
...
smb_ads_open_main(char *domain, char *user, char *password)
{
	...
	if (user != NULL) {
		if (strchr(user, '@') == NULL) {
			principal = smb_ads_get_principal(domain, user);
			if (principal == NULL)
				return (NULL);

			rc = smb_kinit(principal, password);
			free(principal);

		} else {
			rc = smb_kinit(user, password);
		}
	...
	if (rc != LDAP_SUCCESS) {
		syslog(LOG_ERR, "ldap_sasl_interactive_bind_s failed (%s)",
		    ldap_err2string(rc));
		smb_ads_close(ah);
		free(ads_host);
		return (NULL);
 ...
 */
static int
smb_ads_escape_search_filter_chars(const char *src, char *dst, size_t dst_len)
{
	int avail = dst_len - 1; /* reserve a space for NULL char */
    ...
    const char *adsContainer, char *unc_name)
{
	...
	if (smb_ads_escape_search_filter_chars(unc_name, tmpbuf,
	    sizeof (tmpbuf)) != 0) {
		free(share_dn);
		return (-1);
	}

	cnt = snprintf(filter, sizeof (filter),
	    "(&(objectClass=volume)(uNCName=%s))", tmpbuf);
	if (cnt < 0 || cnt >= sizeof (filter)) {
		free(share_dn);
		return (-1);
	}
...
static int
smb_ads_update_dsattr(const char *hostname, int port)
{
	LDAP	*ld;
	...
	if ((ld = ldap_init(hostname, port)) == NULL) {
		syslog(LOG_DEBUG, "smb_ads_update_dsattr: ldap_init failed");
		return (-1);
	}
	...
	rc = ldap_sasl_interactive_bind_s(ld, "", "GSSAPI", NULL, NULL,
	    LDAP_SASL_INTERACTIVE, &smb_ads_saslcallback, NULL);
	if (rc != LDAP_SUCCESS) {
		syslog(LOG_ERR,
		    "smb_ads_update_dsattr: ldap_sasl_interactive_bind_s %s",
		    ldap_err2string(rc));
	...
	(void) smb_ads_get_dsattr(ld, DS_ATTR_DCLEVEL);
	(void) smb_ads_get_dsattr(ld, DS_ATTR_DOMAINLEVEL);
	(void) ldap_unbind(ld);
	return (0);
}

/*
 * Get the AD DS functional level for the DC, domain or forest.
 * These values are not populated on Windows 2000 server.
 *
 * On success, the level is stored locally in SMF and returned.
 * Returns -1 on error.
 */
static int
smb_ads_get_dsattr(LDAP *ld, const char *ds_attr)
{
	...
	rc = ldap_search_s(ld, "", LDAP_SCOPE_BASE, NULL, attr, 0, &res);
	if (rc != LDAP_SUCCESS) {
		(void) ldap_msgfree(res);
		return (-1);
	}
	...
	if ((values = ldap_get_values(ld, entry, ds_attr)) == NULL) {
		(void) ldap_msgfree(res);
		smb_config_setdsattr(ds_attr, DS_BEHAVIOR_WIN2000);
		return (DS_BEHAVIOR_WIN2000);
	}

	if (values[0] != NULL) {
		level = atoi(values[0]);
		smb_config_setdsattr(ds_attr, (int64_t)level);
	}
...
{
	char dns_suffix[MAXHOSTNAMELEN];

	(void) smb_get_dns_suffix(dns_suffix, sizeof (dns_suffix));

	if (smb_gethostname(fqhost, len, SMB_CASE_LOWER) != 0)
		return (-1);

	(void) snprintf(fqhost, len, "%s.%s", fqhost,
	    *dns_suffix == '\0' ? ah->domain : dns_suffix);
...
smb_ads_computer_op(smb_ads_handle_t *ah, int op, int dclevel, char *dn)
{
	...
	if (smb_config_getbool(SMB_CI_SVR_EXTSEC))
		flags = SMB_PN_SPN_ATTR;
	else
		flags = SMB_PN_SPN_ATTR | SMB_PN_NONKERBERIZED_CIFS;

	if (smb_krb5_get_pn_set(&spn, flags, ah->domain) == 0)
		return (-1);
	...
	max = (SMB_ADS_COMPUTER_NUM_ATTR - ((op != LDAP_MOD_ADD) ? 1 : 0))
	    - (dclevel >= DS_BEHAVIOR_WIN2008 ?  0 : 1);
	...
	if (dclevel > DS_BEHAVIOR_WIN2003) {
		attrs[++j]->mod_op = op;
		attrs[j]->mod_type = DS_ATTR_ENCTYPES;
    ...
    int scope, char *dn)
{
	...
	LDAPMessage *res;
	char sam_acct[SMB_SAMACCT_MAXLEN];
	char sam_acct2[SMB_SAMACCT_MAXLEN * 2];
	smb_ads_qstat_t rc;
	int cnt;
	...
	if (smb_ads_escape_search_filter_chars(sam_acct, sam_acct2,
	    sizeof (sam_acct2)) != 0)
		return (SMB_ADS_STAT_ERR);
    ...
    size_t len)
{
	...
	boolean_t new_acct;
	int dclevel, status, usrctl_flags = 0;
	...
	if ((dclevel = smb_ads_get_dsattr(ah->ld, DS_ATTR_DCLEVEL)) == -1) {
		smb_ads_close(ah);
		return (SMB_ADJOIN_ERR_GET_DCLEVEL);
	}

	(void) smb_ads_get_dsattr(ah->ld, DS_ATTR_DOMAINLEVEL);
	...
	status = smb_krb5_kt_update_adjoin(ctx, ah->domain, kvno,
	    machine_passwd, DS_SUPPORT_AES(dclevel) ?
	    K5_KT_FLAG_AES_SUPPORT : 0);
	rc = (status == 0) ? SMB_ADJOIN_SUCCESS : SMB_ADJOIN_ERR_WRITE_KEYTAB;

adjoin_cleanup:
	/*
	 * Upon failure, clean up the machine account created during domain
	 * join.
	 */
	if (new_acct && rc != SMB_ADJOIN_SUCCESS)
...
smb_ads_join_errmsg(smb_adjoin_status_t status)
{
	...
	static const struct xlate_table {
		...
		{ SMB_ADJOIN_ERR_GET_HANDLE,
		    "Unable to connect to an Active Directory server" },
		{ SMB_ADJOIN_ERR_GEN_PWD,
		    "Machine password generation failed" },
		{ SMB_ADJOIN_ERR_GET_DCLEVEL, "Unknown domain controller "
		    "functional level:  The rootDSE attribute named "
		    "\"domainControllerFunctionality\" is missing from "
		    "Active Directory" },
		{ SMB_ADJOIN_ERR_ADD_TRUST_ACCT,
		    "Workstation trust account create failed" },
		{ SMB_ADJOIN_ERR_MOD_TRUST_ACCT,
		    "Workstation trust account update failed" },
		{ SMB_ADJOIN_ERR_DUP_TRUST_ACCT,
		    "Workstation trust account update failed: "
		    "The name is in use" },
		{ SMB_ADJOIN_ERR_TRUST_ACCT,
		    "Workstation trust account query failed" },
		{ SMB_ADJOIN_ERR_INIT_KRB_CTX,
		    "Kerberos context initialization failed" },
		{ SMB_ADJOIN_ERR_GET_SPNS,
		    "Unable to get Kerberos principals." },
		{ SMB_ADJOIN_ERR_KSETPWD, "Machine password update failed" },
		{ SMB_ADJOIN_ERR_UPDATE_CNTRL_ATTR,
		    "Workstation trust account userAccountControl attribute "
		    "update failed" },
		{ SMB_ADJOIN_ERR_WRITE_KEYTAB, "keytab update failed" }
	};
...
smb_ads_match_pdc(smb_ads_host_info_t *host)
{
	...
	(void) smb_config_getip(SMB_CI_DOMAIN_SRV, &pdc);
	if (smb_inet_equal(&host->ipaddr, &pdc))
		match = B_TRUE;
 ...
 *
 * fqdn	  - fully-qualified AD domain name
 * server - fully-qualifed hostname of the DC to be sought
 * buf    - fully-qualified hostname of the discovered DC
 */
boolean_t
smb_ads_lookup_msdcs(char *fqdn, char *server, char *buf, uint32_t buflen)
{
	...
	*buf = '\0';
	...
	(void) smb_ads_update_dsattr(hinfo->name, hinfo->port);
...
/*
 * user principal name format: <username>@<REALM>\
 */
static char *
smb_ads_get_principal(const char *domain, const char *user)
{
	char *realm;
	char *principal;

	if ((realm = smb_krb5_domain2realm(domain)) == NULL)
		return (NULL);

	(void) asprintf(&principal, "%s@%s", user, realm);
	free(realm);
	return (principal);
}

...
#define	NDR_MULTI_FRAGSZ_MIN	(60 * 1024)
#define	NDR_MULTI_FRAGSZ_MAX	(1024 * 1024)

static void ndr_clnt_grow_pdu(ndr_xa_t *);
...
static void
ndr_clnt_grow_pdu(ndr_xa_t *mxa)
{
	ndr_response_hdr_t	*hdr =  &mxa->recv_hdr.response_hdr;
	uint32_t		alloc_size;

	if (hdr->alloc_hint < NDR_MULTI_FRAGSZ_MIN)
		alloc_size = NDR_MULTI_FRAGSZ_MIN;
	else if (hdr->alloc_hint > NDR_MULTI_FRAGSZ_MAX)
		alloc_size = NDR_MULTI_FRAGSZ_MAX;
	else
		alloc_size = hdr->alloc_hint + NDR_DEFAULT_FRAGSZ;

	smb_tracef("ndr_clnt_grow_pdu: alloc_hint=%u, alloc_size=%u",
	    hdr->alloc_hint, alloc_size);

	(void) NDS_GROW_PDU(&mxa->recv_nds, alloc_size, NULL);
}
 ...
 * as a series of fragments: 5680 bytes for Windows NT and 4280 bytes for

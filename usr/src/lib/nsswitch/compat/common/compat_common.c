...
/*
 * str2auuser_s is very simple version of the str2auuser() that can be found in
 * libc. They only copy the user name into the au_user_str_t structure
 * (so check on user name can be performed).
 */
static int
str2auuser_s(
	const char	*instr,
    ...
    nss_dbop_t op_num;
{
	...
	if (res != NSS_SUCCESS) {
		...
		if (op_num == NSS_DBOP_AUDITUSER_BYNAME) {

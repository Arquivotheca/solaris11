...
__td_thr_getxregsize(td_thrhandle_t *th_p, int *xregsize)
{
	...
	err = ps_lgetxregsize(ph_p, thr_to_lwpid(th_p), xregsize);
	if (err != PS_OK) {
...
__td_thr_getxregs(td_thrhandle_t *th_p, void *xregset)
{
	...
	err = ps_lgetxregs(ph_p, thr_to_lwpid(th_p), (caddr_t)xregset);
	if (err != PS_OK) {
#if defined(__i386) || defined(__amd64)
		if (err == PS_NOXREGS) {
			return_val = TD_NOXREGS;
...
__td_thr_setxregs(td_thrhandle_t *th_p, const void *xregset)
{
	...
	err = ps_lsetxregs(ph_p, thr_to_lwpid(th_p), (caddr_t)xregset);
	if (err != PS_OK) {
#if defined(__i386) || defined(__amd64)
		if (err == PS_NOXREGS) {
			return_val = TD_NOXREGS;

...
#define	DTD_ELEM_POSTMOVE	((const xmlChar *) "postmove")
...
#define	DTD_ELEM_MAC_PROFILE	((const xmlChar *) "file-mac-profile")
...
static int
get_zonepath_ds(const char *zonepath, char *dataset, size_t len)
{
	FILE *mnttab;
	struct mnttab query, result;
	...
	query.mnt_mountp = (char *)zonepath;
	query.mnt_fstype = "zfs";

	if (getmntany(mnttab, &result, &query) != 0) {
		(void) fclose(mnttab);
		return (-1);
	}

	(void) fclose(mnttab);
	if (strlcpy(dataset, result.mnt_special, len) >= len) {
		return (-1);
	}
	return (0);
}
...
{
	int dst, src;
	char ds[MAXNAMELEN];	/* Dataset, used by %D */
	...
	for (src = 0; src < strlen((char *)sbuf) && dst < dbuf_size; src++) {
		...
		case 'D':
			...
			if (get_zonepath_ds(zonepath, ds, sizeof (ds)) != 0) {
				/*
				 * The zonepath doesn't exist yet.  If we
				 * fill in nothing in place of %D, a bogus
				 * value that means nothing is returned.  If
				 * we fill in some descriptive text, then
				 * a bogus value is still returned but it
				 * is more recognizable at being bogus and
				 * as such helps with messaging.
				 */
				dst += strlcpy(dbuf + dst, "<zonepath_dataset>",
				    dbuf_size - dst);
				break;
			}
			dst += strlcpy(dbuf + dst, ds, dbuf_size - dst);
			break;
		case 'R':
			if (zonepath == NULL)
				break;
...
int
brand_get_postmove(brand_handle_t bh, const char *zonename,
    const char *zonepath, char *buf, size_t len)
{
	struct brand_handle *bhp = (struct brand_handle *)bh;
	return (brand_get_value(bhp, zonename, zonepath, NULL, NULL,
	    buf, len, DTD_ELEM_POSTMOVE, B_TRUE, B_TRUE));
}
...
i_brand_platform_iter_mounts(struct brand_handle *bhp, const char *zonepath,
    const char *zonename, int (*func)(void *, const char *, const char *,
...
brand_platform_iter_gmounts(brand_handle_t bh, const char *zonepath,
    const char *zonename, int (*func)(void *, const char *,
    const char *, const char *, const char *), void *data)
{
	struct brand_handle *bhp = (struct brand_handle *)bh;
	return (i_brand_platform_iter_mounts(bhp, zonepath, zonename,
	    func, data, DTD_ELEM_GLOBAL_MOUNT));
}

/*
 * Iterate over datasets
 *
 * Walks the platform, searching for <dataset> elements, calling the
 * specified callback for each.  Returns 0 on success, or -1 on failure.
 *
 * Perform the following substitutions as necessary:
 *
 ...
 */
int
brand_platform_iter_datasets(brand_handle_t bh, const char *zonepath,
    int (*func)(void *, const char *, const char *), void *data)
{
	xmlNodePtr node;
	xmlChar *name, *alias;
	char name_exp[MAXPATHLEN], alias_exp[MAXPATHLEN];
	int ret;		/* return values from called functions */
	int fn_ret = 0;		/* return value from this function */
	struct brand_handle *bhp = (struct brand_handle *)bh;
	...
	for (node = node->xmlChildrenNode; node != NULL; node = node->next) {
		...
		if (xmlStrcmp(node->name, DTD_ELEM_DATASET) != 0)
			continue;

		if ((name = xmlGetProp(node, DTD_ATTR_NAME)) == NULL) {
			ret = -1;
			goto next;
		}

		/* Substitute token values as needed. */
		if ((ret = i_substitute_tokens((char *)name, name_exp,
		    sizeof (name_exp), NULL, zonepath, NULL, NULL)) != 0)
			goto next;

		/*
		 * If there is no alias, use the default alias, which is
		 * equivalent to basename(name).
		 */
		if ((alias = xmlGetProp(node, DTD_ATTR_ALIAS)) == NULL) {
			alias_exp[0] = '\0';
		} else {
			if ((ret = i_substitute_tokens((char *)alias,
			    alias_exp, sizeof (alias_exp), NULL, zonepath,
			    NULL, NULL)) != 0)
				alias_exp[0] = '\0';
		}

		ret = func(data, (char *)name_exp, (char *)alias_exp);
...
static xmlNodePtr
brand_find_profile_node(brand_handle_t bh, const char *prof)
{
	xmlNodePtr		node;
	xmlChar			*name;
	int			ret;
	...
	for (node = node->xmlChildrenNode; node != NULL; node = node->next) {
		if (xmlStrcmp(node->name, DTD_ELEM_MAC_PROFILE) != 0)
			continue;

		/* Returns that we support at least one file-mac-profile. */
		if (prof == NULL)
			return (node);
		...
		ret = xmlStrcmp(name, (xmlChar *)prof);
		xmlFree(name);
...
boolean_t
brand_allow_readonly(brand_handle_t bh)
{
	return (brand_find_profile_node(bh, NULL) != NULL);
}

boolean_t
brand_verify_mac_profile(brand_handle_t bh, const char *prof)
{
	xmlNodePtr	node;

	node = brand_find_profile_node(bh, prof);

	return (node != NULL);
}

/*
 * Iterate over brand writable-path for a particular file-mac-profile.
 *
 * Walks the brand config, searching for <writable-path> or <readonly-path>
 * elements for the specified profile, calling the specified callback for each.
 * Returns 0 on success, or -1 on failure or the non-zero return from ret.
 */
int
brand_config_iter_rwp(brand_handle_t bh, int (*func)(void *, const char *),
    const char *prof, void *data, boolean_t w)
{
	xmlNodePtr		node;
	xmlChar			*path;
	int			ret = 0;
	const xmlChar		*elem = w ? DTD_ELEM_WRP : DTD_ELEM_ROP;

	node = brand_find_profile_node(bh, prof);

	/* Profile not found, this is an error. */
	if (node == NULL)
		return (-1);

	for (node = node->xmlChildrenNode; node != NULL; node = node->next) {
		if (xmlStrcmp(node->name, elem) != 0)
			continue;

		path = xmlGetProp(node, DTD_ATTR_PATH);

		if (path == NULL)
			continue;

		ret = func(data, (char *)path);
		xmlFree(path);

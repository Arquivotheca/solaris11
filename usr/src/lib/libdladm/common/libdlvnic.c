 ...
 */
static char dladm_vnic_def_prefix[] = VNIC_DEF_PREFIX;
...
i_dladm_vnic_create_sys(dladm_handle_t handle, dladm_vnic_attr_t *attr)
{
	...
	ioc.vc_zone_id = attr->va_zone_id;
    ...
    uint32_t flags)
{
	...
	if ((status = dladm_create_datalink_id_zone(handle, name, class,
	    media, flags, zoneid, &vnic_id)) != DLADM_STATUS_OK)
		return (status);

	if ((flags & DLADM_OPT_PREFIX) != 0) {
		(void) snprintf(name + strlen(name), sizeof (name),
		    "%llu", vnic_id);
		flags &= ~DLADM_OPT_PREFIX;
	}

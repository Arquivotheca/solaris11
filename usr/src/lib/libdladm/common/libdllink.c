...
void
dladm_get_ring_index_list(dladm_handle_t handle, datalink_id_t linkid,
    uint_t rxringlist[], uint_t *p_nrxrings,
    uint_t txringlist[], uint_t *p_ntxrings)
{
	dld_ioc_ringidget_t	iom;
	uint_t			nrxrings = 0, ntxrings = 0;
	int			i;

	iom.dir_linkid = linkid;

	if (ioctl(dladm_dld_fd(handle), DLDIOC_GETRINGIDS, &iom) < 0)
		goto done;

	if (iom.dir_rxdedicated == B_TRUE)
		nrxrings = iom.dir_nrxrings;

	if (iom.dir_txdedicated == B_TRUE)
		ntxrings = iom.dir_ntxrings;

done:
	if (p_nrxrings) {
		*p_nrxrings = nrxrings;
		for (i = 0; i < *p_nrxrings; i++)
			rxringlist[i] = iom.dir_rxringids[i];
	}
	if (p_ntxrings) {
		*p_ntxrings = ntxrings;
		for (i = 0; i < *p_ntxrings; i++)
			txringlist[i] = iom.dir_txringids[i];
	}
}


void
dladm_get_ring_counts(dladm_handle_t handle, datalink_id_t linkid,
    uint_t *p_nrxrings, uint_t *p_ntxrings)
{
	int			bufsize;
	...
	if (ioctl(dladm_dld_fd(handle), DLDIOC_GETHWGRP, iomp) < 0) {
		free(iomp);
		goto done;
	}

	nrxrings = iomp->dih_nrxrings;
	ntxrings = iomp->dih_ntxrings;
...
void
dladm_query_primary_linkid(dladm_handle_t handle, datalink_id_t linkid,
    datalink_id_t *ulinkid)
{
	dladm_status_t	status = DLADM_STATUS_OK;
	datalink_class_t	class;

	*ulinkid = DATALINK_INVALID_LINKID;
	...
	case DATALINK_CLASS_VLAN: {
		...
		status = dladm_vnic_info(handle, linkid, &vinfo,
		    DLADM_OPT_ACTIVE);
		if (status != DLADM_STATUS_OK)
			break;

		*ulinkid = vinfo.va_link_id;
		break;
	}
	}
}
    ...
    boolean_t (*fn)(void *, dladm_hwgrp_attr_t *))
{
	...
	if (ret == 0) {
		...
		for (i = 0; i < iomp->dih_ngroups; i++) {
			...
			attr.hg_nrings = dhip->dhi_nrings;
			for (j = 0; j < dhip->dhi_nrings; j++)
				attr.hg_rings[j] = dhip->dhi_rings[j];
			dladm_sort_index_list(attr.hg_rings, attr.hg_nrings);
			attr.hg_nclnts = dhip->dhi_nclnts;
    ...
    dladm_phys_attr_t *dpap, uint32_t flags)
{
	...
	case DLADM_OPT_PERSIST: {
		...
		dpap->dp_loc[0] = '\0';
		if (status == DLADM_STATUS_OK) {
			(void) dladm_get_conf_field(handle, conf, FPHYLOC,
			    dpap->dp_loc, DLD_LOC_STRSIZE);
		}
		dladm_destroy_conf(handle, conf);
		return (status);
	}
	case DLADM_OPT_ACTIVE: {
		...
		(void) strlcpy(dpap->dp_loc, dip.dip_loc,
		    DLD_LOC_STRSIZE);
		if (dladm_get_phys_prefix(handle, dpap->dp_phys_prefix)
		    != DLADM_STATUS_OK || dip.dip_novanity)
			dpap->dp_phys_prefix[0] = '\0';
		return (DLADM_STATUS_OK);
	}
	default:

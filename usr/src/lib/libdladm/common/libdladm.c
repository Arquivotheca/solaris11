...
dladm_status2str(dladm_status_t status, char *buf)
{
	...
	case DLADM_STATUS_PERSIST_FLOW_EXISTS:
		...
		break;
	case DLADM_STATUS_FLOW_WRONG_ZONE:
	...
	case DLADM_STATUS_PORT_NOPROTO:
		...
		break;
	case DLADM_STATUS_PORT_LLDP_ENABLED:
		s = "port has LLDP enabled";
		break;
	case DLADM_STATUS_PORT_DCB_ENABLED:
		s = "port has DCB enabled";
		break;
	case DLADM_STATUS_PORT_LLDP_NOT_ENABLED:
...
{
	return (dlparse_drvppa(link, NULL, 0, NULL));
}
 ...
 */
dladm_status_t
dladm_parse_zonelist(char *optarg, uint32_t *zonecnt, zoneid_t **zonelistp)
{
	char *zoneoptstr;
	char *zone;
	char *last;
	char *sepstr = ",";
	zoneid_t *zonelist = NULL;
	zoneid_t *zonelistnew;
	zoneid_t zid;
	uint32_t zidcnt = 0;

	zoneoptstr = strdupa(optarg);
	if ((zone = strtok_r(zoneoptstr, sepstr, &last)) == NULL)
		return (DLADM_STATUS_BADARG);
	...
	do {
		...
		if (zone_get_id(zone,  &zid) != 0) {
			free(zonelist);
			return (DLADM_STATUS_BADARG);
		}

		zonelistnew = realloc(zonelist, (zidcnt + 1) *
		    sizeof (zoneid_t));
		if (zonelistnew == NULL) {
			free(zonelist);
			return (DLADM_STATUS_NOMEM);
		}
		zonelist = zonelistnew;
		zonelist[zidcnt] = zid;
		zidcnt++;
	} while ((zone = strtok_r(NULL, sepstr, &last)) != NULL);
 ...
 */
dladm_status_t
dladm_apply_linknamefilters(dladm_handle_t handle, const char *linkname,
    datalink_id_t *linkidp, uint32_t zonecnt, zoneid_t *zonelist,
    uint32_t searchflags, zoneid_t caller_zoneid, char *errstr)
{
	uint32_t flags;
	zoneid_t linkzoneid;
	dladm_status_t status = DLADM_STATUS_BADARG;
	zoneid_t linknamezid = ALL_ZONES;
	 ...
	 */
	if (linkname != NULL && (!dlparse_zonelinkname(linkname, NULL,
	    &linknamezid) || (caller_zoneid != GLOBAL_ZONEID && linknamezid !=
	    ALL_ZONES) || (status = dladm_name2info(handle, linkname, linkidp,
	    &flags, NULL, NULL)) != DLADM_STATUS_OK)) {
		(void) snprintf(errstr, DLADM_STRSIZE, "link %s is not valid",
		    linkname);
		return (status);
	} else if (linkname == NULL && (*linkidp == DATALINK_ALL_LINKID ||
	    (status = dladm_datalink_id2info(handle, *linkidp, &flags, NULL,
	...
	if (!(flags & searchflags)) {
		(void) snprintf(errstr, DLADM_STRSIZE, "link %s is %s",
		    linkname, (searchflags & DLADM_OPT_PERSIST) ?
	...
	if ((status = dladm_datalink_id2linkinfo(handle, *linkidp, NULL, NULL,
	    NULL, NULL, 0, &linkzoneid)) != DLADM_STATUS_OK) {
		(void) snprintf(errstr, DLADM_STRSIZE, "link %s is not valid",
		    linkname);
		return (status);
	}
	 ...
	 */
	if (linknamezid != ALL_ZONES && dladm_search_zonelist(linknamezid,
	    zonecnt, zonelist) != DLADM_STATUS_OK) {
		(void) snprintf(errstr, DLADM_STRSIZE, "link %s is not valid "
	 ...
	 */
	if ((flags & DLADM_OPT_ONLOAN) && caller_zoneid == GLOBAL_ZONEID) {
		if (dladm_search_zonelist(GLOBAL_ZONEID, zonecnt, zonelist) ==
		    DLADM_STATUS_OK)
			return (DLADM_STATUS_OK);
	}

	if (dladm_search_zonelist(linkzoneid, zonecnt, zonelist) ==
	    DLADM_STATUS_OK)
		return (DLADM_STATUS_OK);
	(void) snprintf(errstr, DLADM_STRSIZE, "link %s is not valid in "

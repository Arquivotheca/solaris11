...
static pd_getf_t	get_zone, get_autopush, get_rate_mod, get_rate,
			...
			get_duplex, get_link_state, get_binary, get_uint8,
			get_uint32, get_flowctl, get_maxbw, get_cpus,
			get_priority, get_tagmode, get_range, get_stp,
			get_bridge_forward, get_bridge_pvid, get_protection,
			get_rxrings, get_txrings, get_cntavail,
			get_allowedips, get_allowedcids, get_pool,
			get_rings_range, get_vlan_announce, get_mac_addr,
			get_linkmode_prop, get_pfcmap, get_rxfanout;

static pd_setf_t	set_zone, set_rate, set_powermode, set_radio,
			set_public_prop, set_resource, set_stp_prop,
			set_bridge_forward, set_bridge_pvid, set_vlan_announce,
			set_mac_addr;
...
static pd_checkf_t	check_zone, check_autopush, check_rate, check_hoplimit,
			...
			check_allowedcids, check_rings, check_mac_addr,
			check_pfcmap, check_pool, check_prop, check_rxfanout;
...
#define	PD_DONT_PERSIST	0x8	/* do not persist into datalink.conf */
#define	PD_RESET	0x10  	/* reset property */
	/*
...
static link_attr_t link_attr[] = {
	...
	{ MAC_PROP_GVRP_TIMEOUT, sizeof (uint32_t), "gvrp-timeout"},
	...
	{ MAC_PROP_MAC_ADDR,	sizeof (dld_mac_addr_t), "mac-address"},

	{ MAC_PROP_PFC,	sizeof (uint8_t),	"pfcmap"},

	{ MAC_PROP_NTCS,	sizeof (uint8_t),	"ntcs"},
...
static val_desc_t	vlan_announce_vals[] =	{
...
static prop_desc_t	prop_table[] = {
	...
	{ "flowctrl-effective", { "--", 0 },
	    NULL, 0, NULL, NULL, get_flowctl, NULL,
	    0, DATALINK_CLASS_PHYS, DL_ETHER },
	...
	{ "rxfanout", { "", 0 }, NULL, 0,
	    set_resource, NULL,
	    get_rxfanout, check_rxfanout, PD_RESET,
	    DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE },
	...
	{ "pool", { "--", RESET_VAL }, NULL, 0,
	    set_resource, NULL,
	    get_pool, check_pool, PD_CHECK_ALLOC,
	    DATALINK_CLASS_ALL, DATALINK_ANY_MEDIATYPE },
	...
	{ "vlan-announce", { "off", MAC_VLAN_ANNOUNCE_OFF }, vlan_announce_vals,
	    VALCNT(vlan_announce_vals), set_vlan_announce, NULL,
	    get_vlan_announce, NULL,
	    0, DATALINK_CLASS_PHYS|DATALINK_CLASS_AGGR|
	    DATALINK_CLASS_SIMNET, DL_ETHER },

	{ "gvrp-timeout", { "250", 250 }, NULL, 0,
	    set_public_prop, get_range,
	...
	{ "mac-address", { "", 0 },
	    NULL, 0, set_mac_addr, NULL,
	    get_mac_addr, check_mac_addr, PD_CHECK_ALLOC,
	    DATALINK_CLASS_PHYS | DATALINK_CLASS_VLAN | DATALINK_CLASS_AGGR |
	    DATALINK_CLASS_VNIC | DATALINK_CLASS_PART, DATALINK_ANY_MEDIATYPE },
	...
	{ "pfcmap", { "00000000", 0 }, NULL, 0,
	    set_public_prop, NULL, get_pfcmap, check_pfcmap, 0,
	    DATALINK_CLASS_PHYS, DL_ETHER },

	{ "ntcs", { "", 0 }, NULL, 0, NULL, NULL, get_uint8, NULL,
    ...
    uint_t val_cnt, uint_t flags)
{
	...
	if (prop_val != NULL) {
		...
		} else if ((pdp->pd_flags & PD_RESET) != 0) {
			if ((vdp = calloc(1, sizeof (val_desc_t))) == NULL)
				return (DLADM_STATUS_NOMEM);
			vdp->vd_val = RESET_VAL;
		} else {
    ...
    const char *prop_name, char **prop_val, uint_t val_cnt, uint_t flags)
{
	...
	if (flags & DLADM_OPT_PERSIST) {
		...
		if (status == DLADM_STATUS_OK && (flags & DLADM_OPT_ACTIVE)) {
			if (pdp != NULL && (pdp->pd_flags & PD_AFTER_PERM)) {
    ...
    uint_t *val_cntp)
{
	...
	case DLADM_PROP_VAL_PERSISTENT:
		if (pdp->pd_flags & PD_TEMPONLY) {
			return (DLADM_STATUS_TEMPONLY);
		} else if (pdp->pd_flags & PD_DONT_PERSIST) {
			status = pdp->pd_get(handle, pdp, linkid, prop_val,
			    val_cntp, media, DLADM_OPT_PERSIST, &perm_flags);
		} else {
...
/* ARGSUSED */
static dladm_status_t
check_pfcmap(dladm_handle_t handle, prop_desc_t *pdp, datalink_id_t linkid,
    char **prop_val, uint_t *val_cntp, uint_t flags, val_desc_t **vdpp,
    datalink_media_t media)
{
	uint8_t		pfcmap;
	...
	ptr = prop_val[0] + strlen(prop_val[0]) - 1;
	i = 0;
	pfcmap = 0;
	while (ptr >= prop_val[0]) {
		if (*ptr == '1')
			pfcmap |= 1 << i;
		else if (*ptr != '0')
			goto err;
    ...
    datalink_media_t media)
{
	 ...
	 */
	if (val_cnt <= ncpus) {
		newvdp = calloc(ncpus, sizeof (val_desc_t));
		if (newvdp == NULL) {
			status = DLADM_STATUS_NOMEM;
			goto done1;
		}
	...
	for (i = 0; i < ncpus; i++) {
		for (j = i + 1; j < ncpus; j++) {
			if (vdp[i].vd_val == vdp[j].vd_val) {
				status = DLADM_STATUS_DUPCPUID;
				goto done2;
			}
		}
	}
...
/* ARGSUSED */
static dladm_status_t
get_rxfanout(dladm_handle_t handle, prop_desc_t *pdp, datalink_id_t linkid,
    char **prop_val, uint_t *val_cnt, datalink_media_t media,
    uint_t flags, uint_t *perm_flags)
{
	mac_resource_props_t	mrp;
	...
	if (strcmp(pdp->pd_name, "rxfanout-effective") == 0) {
		status = i_dladm_get_public_prop(handle, linkid,
		    "resource-effective", flags, perm_flags, &mrp,
		    sizeof (mrp));
	...
	rxfanout = mrp.mrp_rxfanout;
	*val_cnt = 1;
	(void) snprintf(*prop_val, DLADM_PROP_VAL_MAX, "%d", rxfanout);
	return (DLADM_STATUS_OK);
}

/* ARGSUSED */
static dladm_status_t
check_rxfanout(dladm_handle_t handle, prop_desc_t *pdp,
...
/* ARGSUSED */
dladm_status_t
extract_rxfanout(val_desc_t *vdp, uint_t cnt, void *arg)
{
	mac_resource_props_t	*mrp = (mac_resource_props_t *)arg;

	if (vdp->vd_val == RESET_VAL)
		mrp->mrp_rxfanout = MRP_RXFANOUT_RESETVAL;
	else
		mrp->mrp_rxfanout = vdp->vd_val;
    ...
    datalink_media_t media)
{
	...
	if ((flags & DLADM_OPT_BOOT) == 0) {
		...
		/* Look for pool name */
		if (pool_get_pool(poolconf, *prop_val) == NULL) {
...
/* ARGSUSED */
static dladm_status_t
check_mac_addr(dladm_handle_t handle, prop_desc_t *pdp, datalink_id_t linkid,
    char **prop_val, uint_t *val_cntp, uint_t flags, val_desc_t **vdpp,
    datalink_media_t media)
{
	datalink_class_t	class;
	...
	if (class == DATALINK_CLASS_VLAN || class == DATALINK_CLASS_PART)
		return (DLADM_STATUS_PROPRDONLY);

	if (prop_val != NULL) {
		dld_mac_addr_t	*mac_addr;
		uchar_t		*addr;

		if ((mac_addr = calloc(1, sizeof (dld_mac_addr_t))) == NULL)
			return (DLADM_STATUS_NOMEM);

		if ((addr = _link_aton(prop_val[0], &len)) == NULL) {
			status = (len == -1) ? DLADM_STATUS_BADVAL :
			    DLADM_STATUS_NOMEM;

			free(mac_addr);
			return (status);
		}
		bcopy(addr, mac_addr->dm_addr, len);
		free(addr);
		mac_addr->dm_len = len;
		vdp->vd_val = (uintptr_t)mac_addr;
	} else {
		vdp->vd_val = 0;
	}
...
/* ARGSUSED */
static dladm_status_t
set_mac_addr(dladm_handle_t handle, prop_desc_t *pdp,
...
/* ARGSUSED */
static dladm_status_t
get_mac_addr(dladm_handle_t handle, prop_desc_t *pdp,
    datalink_id_t linkid, char **prop_val, uint_t *val_cnt,
    datalink_media_t media, uint_t flags, uint_t *perm_flags)
{
	datalink_class_t	class;
	...
	status = i_dladm_get_public_prop(handle, linkid, "mac-address",
	    flags, NULL, &mac_addr, sizeof (dld_mac_addr_t));
	if (status != DLADM_STATUS_OK)
		return (status);

	str = _link_ntoa(mac_addr.dm_addr, NULL, mac_addr.dm_len,
	    IFT_OTHER);
	(void) strncpy(*prop_val, str, DLADM_PROP_VAL_MAX);
	free(str);
	*val_cnt = 1;
    ...
    const char *prop_name, char **prop_val, uint_t val_cnt)
{
	...
	buf[0] = '\0';
	for (i = 0; i < val_cnt; i++) {
		if (strlcat(buf, prop_val[i], MAXLINELEN) >= MAXLINELEN)
...
/* ARGSUSED */
static dladm_status_t
get_pfcmap(dladm_handle_t handle, prop_desc_t *pdp,
    datalink_id_t linkid, char **prop_val, uint_t *val_cnt,
    datalink_media_t media, uint_t flags, uint_t *perm_flags)
{
	dladm_status_t	status;
...
/* ARGSUSED */
static dladm_status_t
get_uint8(dladm_handle_t handle, prop_desc_t *pdp,
    datalink_id_t linkid, char **prop_val, uint_t *val_cnt,
    datalink_media_t media, uint_t flags, uint_t *perm_flags)
{
	dladm_status_t	status;
	...
	(void) snprintf(*prop_val, DLADM_PROP_VAL_MAX, "%hhu", v);
	*val_cnt = 1;
	return (DLADM_STATUS_OK);
}
    ...
    datalink_media_t media, uint_t flags, uint_t *perm_flags)
{
	...
	case LINK_FLOWCTRL_PFC:
		(void) sprintf(*prop_val, "pfc");
		break;
	case LINK_FLOWCTRL_AUTO:
		(void) sprintf(*prop_val, "auto");
		break;
	}
	*val_cnt = 1;
	return (DLADM_STATUS_OK);
...
/* ARGSUSED */
static dladm_status_t
set_vlan_announce(dladm_handle_t handle, prop_desc_t *pd, datalink_id_t linkid,
    val_desc_t *vdp, uint_t val_cnt, uint_t flags, datalink_media_t media)
{
	dladm_status_t status;
	dld_ioc_macprop_t *dip;
	mac_vlan_announce_t announce_type;

	dip = i_dladm_buf_alloc_by_id(sizeof (mac_vlan_announce_t),
	    linkid, MAC_PROP_VLAN_ANNOUNCE, 0, &status);
	if (dip == NULL)
		return (status);
	announce_type = vdp->vd_val;
	(void) memcpy(dip->pr_val, &announce_type, sizeof (announce_type));
	status = i_dladm_macprop(handle, dip, B_TRUE);
	free(dip);
	if (status != DLADM_STATUS_OK)
		return (status);
...
/* ARGSUSED */
static dladm_status_t
get_vlan_announce(dladm_handle_t handle, struct prop_desc *pd,
    datalink_id_t linkid, char **prop_val, uint_t *val_cnt,
    datalink_media_t media, uint_t flags, uint_t *perm_flags)
{
	dladm_status_t status;
	dld_ioc_macprop_t *dip;
	mac_vlan_announce_t announce_type = 0;

	if (flags != 0)
		return (DLADM_STATUS_NOTSUP);
	*perm_flags = MAC_PROP_PERM_RW;
	*val_cnt = 1;
	dip = i_dladm_buf_alloc_by_id(sizeof (mac_vlan_announce_t), linkid,
	    MAC_PROP_VLAN_ANNOUNCE, 0, &status);
	if (dip == NULL)
		return (status);
	status = i_dladm_macprop(handle, dip, B_FALSE);
	if (status == DLADM_STATUS_OK) {
		(void) memcpy(&announce_type, dip->pr_val,
		    sizeof (mac_vlan_announce_t));
		(void) strlcpy(*prop_val,
		    vlan_announce_vals[announce_type].vd_name,
		    DLADM_PROP_VAL_MAX);
	} else {
		(void) strlcpy(*prop_val, "?", DLADM_PROP_VAL_MAX);
	}
...
static prop_desc_t *
i_dladm_get_prop_desc(const char *pname)
{
	prop_desc_t	*pdp;
	int		i;
	...
	for (i = 0; i < DLADM_MAX_PROPS; i++, pdp++) {
		if (strcasecmp(pname, pdp->pd_name) == 0)
			return (pdp);
	}
	return (NULL);
}

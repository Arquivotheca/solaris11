...
static boolean_t
i_dladm_aggr_is_dcbmode(dladm_handle_t handle, datalink_id_t linkid)
{
	...
	ntcs = (uint8_t)strtol(propval, NULL, 10);
	return (ntcs > 0);
}
...
{
	assert(flags & (DLADM_OPT_ACTIVE|DLADM_OPT_PERSIST));
    ...
    uint32_t nports, dladm_aggr_port_attr_db_t *ports, uint32_t flags, int cmd)
{
	 ...
	 */
	for (i = 0; i < nports; i++) {
		status = dladm_datalink_id2info(handle, ports[i].lp_linkid,
		    NULL, &class, &media, linkname, sizeof (linkname));
		if (status != DLADM_STATUS_OK ||
		...
		/*
		 * Aggregation is not allowed over links with DCB mode.
		 */
		if (i_dladm_aggr_is_dcbmode(handle, ports[i].lp_linkid))
			return (DLADM_STATUS_PORT_DCB_ENABLED);

		/*
		 * If any port has LLDP enabled, we have to disable LLDP
		 * before creating an aggr.
		 */
		if (lldp_is_enabled(linkname))
			return (DLADM_STATUS_PORT_LLDP_ENABLED);
	}
...
i_dladm_aggr_up(dladm_handle_t handle, datalink_id_t linkid, void *arg)
{
	...
	for (i = 0, j = 0; i < attr.lg_nports; i++) {
		...
		if (i_dladm_aggr_is_dcbmode(handle, ports[i].lp_linkid)) {
			status = DLADM_STATUS_PORT_DCB_ENABLED;
			goto done;
		}
    ...
    aggr_lacp_mode_t lacp_mode, aggr_lacp_timer_t lacp_timer, uint32_t flags)
{
	...
	for (i = 0; i < nports; i++) {
		...
		/*
		 * If LLDP is enabled on any ports, it has to be
		 * disabled first.
		 */
		if (lldp_is_enabled(linkname))
			return (DLADM_STATUS_PORT_LLDP_ENABLED);
...
/* Check if any port in the aggr has LLDP enabled */
static boolean_t
aggr_port_lldp_enabled(dladm_handle_t handle, datalink_id_t linkid)
{
	dladm_aggr_grp_attr_t	attr;
	dladm_status_t		status;
	char			linkname[MAXLINKNAMELEN];
	...
	status = dladm_aggr_info(handle, linkid, &attr, DLADM_OPT_ACTIVE);
	if (status != DLADM_STATUS_OK)
		return (B_FALSE);

	/* If LLDP is enabled on any port, return true. */
	for (i = 0; i < attr.lg_nports; i++) {
		status = dladm_datalink_id2info(handle,
		    attr.lg_ports[i].lp_linkid, NULL, NULL, NULL,
...
dladm_aggr_delete(dladm_handle_t handle, datalink_id_t linkid, uint32_t flags)
{
	...
	/*
	 * If LLDP is enabled on any port we disallow deleting the
	 * aggr.
	 */
	if (aggr_port_lldp_enabled(handle, linkid))
		return (DLADM_STATUS_PORT_LLDP_ENABLED);

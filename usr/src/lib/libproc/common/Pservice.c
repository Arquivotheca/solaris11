...
ps_lgetxregsize(struct ps_prochandle *P, lwpid_t lwpid, int *xrsize)
{
			...
			if (lwp->lwp_id == lwpid) {
				if (lwp->lwp_xregs != NULL)
					...
					*xrsize = 0;
					...
					/*
					 * In x86 we return PS_NOXREGS when AVX
	...
	*xrsize = (int)statb.st_size;
	...
	if (*xrsize == 0) {
		/*
		 * In x86 we return PS_NOXREGS when AVX extension is not found.
		 */
		return (PS_NOXREGS);
	}
#endif
...
#if defined(__i386) || defined(__amd64)
	err = ps_lgetxregsize(P, lwpid, &xrsize);
	if (err == PS_NOXREGS || (err == PS_OK && xrsize == 0)) {
		return (PS_NOXREGS);
	}
#endif

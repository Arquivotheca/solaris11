	...
	proc_stack_f *func, void *arg)
{
	...
	(void) Plookup_by_name(P, "libc.so", "__sighndlr", &sigh);
	...
	for (;;) {
		...
		if (gregs[R_PC] - sigh.st_value < sigh.st_size) {
			frame_flags = PR_FOUND_SIGNAL;
			sig = gregs[R_I0];
		}

		uc_addr = (P->status.pr_dmodel == PR_MODEL_ILP32) ?
		    fp + SA32(sizeof (struct frame32)) :
		    fp + STACK_BIAS + SA(sizeof (struct frame));
		if (find_uclink(&ucl, uc_addr))
		...
		for (i = 0; i < 6; i++)
			args[i] = gregs[R_I0 + i];
		if ((rv = func(arg, gregs, 6, args, frame_flags, sig)) != 0)
			break;
		...
		if (P->status.pr_dmodel == PR_MODEL_ILP32) {
			...
			if (uc_addr != NULL &&
			    Pread(P, &uc32, sizeof (uc32), uc_addr) ==
			    sizeof (uc32)) {

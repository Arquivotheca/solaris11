...
perms_subcmd = dict(
    ...
    snapshot=_("Allows taking of snapshots"),
    rollback=_("Allows rolling back datasets to previously-taken snapshots"),
...
\t\t\t\tability in the new parent"""),
    ...
    send=_("Allows sending of snapshots"),
...
perms_other = dict(
    ...
    key=_("Allows load/unload of dataset key"),
    keychange=_("Allows key change operations"),
)
...
def print_perms():
	...
	print(_("The following delegated permissions are supported:\n"))
	fmt = "%-16s %-14s\t%s"
	...
	# The list of properties is printed by zfs_do_help() in zfs_main.c
...
def do_allow():
	...
	def usage(msg=None):
		if msg:
			print(msg)
			if un:
				parser.exit(_("For more info, run: zfs help unallow"))
			else:
				parser.exit(_("For more info, run: zfs help allow"))
		else:
			parser.print_help()
	...
	parser = zfs.util.ZFSOptionParser(usage=u, prog="zfs")
	...
	def mungeargs(expected_len):
		if un and len(args) == expected_len-1:
			...
			p = [e.strip() for e in args[expected_len-2].split(",")]
			if '' in p:
				usage(_("badly formed comma separated list"))
			return (p, args[expected_len-1])
		else:
	...
	if options.set:
		...
		(perms, fsname) = mungeargs(3)
		who = [w.strip() for w in args[0].split(",")]
		if '' in who:
			usage(_("badly formed list of users"))

...
again:
	myzc = *zc;
	nvbuf = malloc(nvsz);
	myzc.zc_nvlist_dst_size = nvsz;
	myzc.zc_nvlist_dst = (uintptr_t)nvbuf;

	if (ioctl(zfsdevfd, ioc, &myzc) == 0) {
		nvlist_t *nvl;

		errno = nvlist_unpack(nvbuf, myzc.zc_nvlist_dst_size, &nvl, 0);
		if (errno == 0) {
			pynv = nvl2py(nvl);
			nvlist_free(nvl);
		}

...
static const char *ecache_parity[] = {
	"Bits 7:0 ",
	"Bits 15:8 ",
	"Bits 21:16 ",
	"Bits 24:22 "
...
{
	int group;
	const char *status_str[] = { "Unknown", " Empty ",
	    " Failed", " Active", " Spare " };
	const char *cond_str[] = { " Unknown  ", "    OK    ",
	    " Failing  ", "  Failed  ", " Uninit.  ", " Unusable " };
...
disp_env_status(struct system_kstat_data *kstats)
{
	 ...
	 * (enum temp_state).
	 */
	static const char *temp_trend_msg[] = {	"unknown",
						"rapidly falling",
						"falling",
						"stable",
						"rising",
	...
	static const char *temp_state_msg[] = {	"   OK    ",
						"WARNING  ",
						" DANGER  "
					};
...
sunfire_disp_asic_revs(Sys_tree *tree, struct system_kstat_data *kstats)
{
	...
	Prom_node *pnode;
	int isplusbrd;
	const char *board_str[] = {   "Uninitialized", "Unknown", "CPU",

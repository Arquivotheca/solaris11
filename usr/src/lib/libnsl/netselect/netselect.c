	...
	if ((netpath = getenv(NETPATH)) == NULL ||
		...
		for (tpp = netpp; *tpp; tpp++) {
			if ((*tpp)->nc_flag & NC_VISIBLE) {
				(void) strlcat(valid_netpath,
				    (*tpp)->nc_netid, BUFSIZ);
				(void) strlcat(valid_netpath, ":", BUFSIZ);
				count++;
			}
		}
	} else {
 ...
 *	the directory lookup libraries, given as a comma-separated list
 *	in the argument "cp".
 */
 ...
 *	preceded by a '\' are taken literally).
 */

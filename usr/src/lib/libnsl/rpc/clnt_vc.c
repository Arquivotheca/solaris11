...
set_blocking_connection(struct ct_data *ct, bool_t blocking)
{
	...
	if ((flag = fcntl(ct->ct_fd, F_GETFL, 0)) < 0) {
		syslog(LOG_ERR, "set_blocking_connection : %s",
		    no_fcntl_getfl_str);
		return (FALSE);
	}
	...
	rpcvers_t vers, uint_t sendsz, uint_t recvsz, const struct timeval *tp)
{
	...
	if ((flag = fcntl(fd, F_GETFL, 0)) < 0) {
		syslog(LOG_ERR, "_clnt_vc_create_timed : %s",
		    no_fcntl_getfl_str);
		rpc_createerr.cf_stat = RPC_SYSTEMERROR;
						...
						const struct timeval *tp)
{
	...
	case T_IDLE:
		...
		if (tp != NULL) {
			...
			/* TCP_CONN_ABORT_THRESHOLD: int value in millisecs */
			ms = tp->tv_sec * SECS_TO_MS +
			    tp->tv_usec * USECS_TO_MS;
			if (((curr_timeout = _get_tcp_conntime(fd)) != -1) &&
			    (_set_tcp_conntime(fd, ms) == 0)) {
				/* EMPTY */
#ifdef DEBUG
				fprintf(stderr,
				    "set_up_connection: set tcp ");
				fprintf(stderr,
				    "connection timeout to %d ms\n", ms);
#endif
			}
		}
		 ...
		 */
		if (curr_timeout) {
			(void) _set_tcp_conntime(fd, curr_timeout);
		}
...
read_vc(void *ct_tmp, caddr_t buf, int len)
{
	...
	if (pfdp == NULL) {
		syslog(LOG_ERR, clnt_vc_errstr, clnt_read_vc_str, __no_mem_str);

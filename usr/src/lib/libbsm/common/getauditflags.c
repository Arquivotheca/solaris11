...
static int
match_class(char *s, char *prefix, au_class_t m, int v)
{
...
{
	char	    *prefix;		/* +, -, or null */
	au_class_t  m;			/* for masking with masks */
	...
	*auditstring = '\0';
	...
	if ((masks->am_success | masks->am_failure) == AU_MASK_NONE) {
		if (match_class(auditstring, "", AU_MASK_NONE, verbose) != 0)
			return (-1);
	...
	all.am_success = AU_MASK_NONE;
	all.am_failure = AU_MASK_NONE;
	...
	for (m = 0x8000000000000000ULL; m != 0x0ULL; m >>= 1) {

 ...
 * AUC_INIT_AUDIT, AUC_AUDITING, AUC_NOAUDIT
 *
 ...
 * adt_audit_state()
 *
 * returns true if the kernel audit state matches with at least one
 * of audit states set in the argument.
 *
 */
...
adt_get_mask_from_user(uid_t uid, au_mask_t *mask)
{
	...
	if (auditstate & AUC_DISABLED) {
		/* c2audit excluded */
		mask->am_success = AU_MASK_NONE;
		mask->am_failure = AU_MASK_NONE;
	} else if (uid <= MAXUID) {
...
static int
adt_get_hostIP(const char *hostname, boolean_t local, au_tid_addr_t *p_term)
{
	...
	/*
	 * If local request try to use the current system instance
	 * local terminal ID
	 */
	if (local) {
		...
		if ((audit_info.ai_termid.at_addr[0] |
	...
	while ((tries-- > 0) &&
		 ...
		 */
		__auditd_debug("getaddrinfo(%s) failed[%s]\n", hostname,
	...
	if (ai != NULL) {
		...
		freeaddrinfo(ai);
		return (0);
	} else if (auditstate & AUC_AUDITING) {
		/*
		 * auditd is running see if there's an
		 * kernel audit context.  It could be that
		 * auditd is being refreshed from a loopback
		 * situation.  See __do_sethost().
		 */
		if (auditon(A_GETKAUDIT, (caddr_t)&audit_info,
		    sizeof (audit_info)) < 0) {
			adt_write_syslog("unable to get kernel audit context",
		...
		__auditd_debug("setting Audit IP address to kernel for %s\n",
		    hostname);
		*p_term = audit_info.ai_termid;
		return (0);
	}
try_interface:
	{
		...
		if (adt_get_local_address(family, &al) != 0) {
			...
			if (adt_get_local_address(family, &al) != 0) {
				__auditd_debug("adt_get_local_address failed, "
				    "no Audit IP address available, "
				    "faking loopback for %s and "
				    "error %s\n", hostname,
				    strerror(errno));
				al.addr.addr.s_addr = htonl(INADDR_LOOPBACK);
...
adt_load_hostname(const char *hostname, adt_termid_t **termid)
{
	...
	if (adt_get_hostIP(hostname, local, p_term) == 0) {
		*termid = (adt_termid_t *)p_term;
		return (0);
	} else {
		*termid = (adt_termid_t *)p_term;
...
adt_load_ttyname(const char *ttyname, adt_termid_t **termid)
{
	...
	if (adt_get_hostIP(localhost, B_TRUE, p_term) == 0) {
		*termid = (adt_termid_t *)p_term;
		return (0);
	} else {
		*termid = (adt_termid_t *)p_term;
    ...
    const adt_export_data_t *external)
{
	...
	if (version == PROTOCOL_VERSION_1) {
		...
		adrm_int32(&context,
		    (int *)&(internal->as_info.ai_mask.am_success), 1);
		adrm_int32(&context,
		    (int *)&(internal->as_info.ai_mask.am_failure), 1);
		adrm_int32(&context,
		...
		adrm_int32(&context, (int *)&(internal->as_info.ai_auid), 1);
		adrm_int64(&context,
		    (int64_t *)&(internal->as_info.ai_mask.am_success), 2);
		adrm_int32(&context,
		    (int *)&(internal->as_info.ai_termid.at_port), 1);
		adrm_int32(&context,
		    (int *)&(internal->as_info.ai_termid.at_type), 1);
    ...
    adt_internal_state_t *internal)
{
	...
	adrm_putint32(&context, (int *)&(internal->as_rgid), 1);
	adrm_putint32(&context, (int *)&(internal->as_info.ai_auid), 1);
	mask = (uint32_t)(internal->as_info.ai_mask.am_success &
	    UINT64_C(0xFFFFFFFF));
	adrm_putint32(&context, (int *)&mask, 1);
	mask = (uint32_t)(internal->as_info.ai_mask.am_failure &
	    UINT64_C(0xFFFFFFFF));
	adrm_putint32(&context, (int *)&mask, 1);
	adrm_putint32(&context,
	    (int *)&(internal->as_info.ai_termid.at_port), 1);
...
adt_init(adt_internal_state_t *state, int use_proc_data)
{
	...
	if (use_proc_data) {
		...
		if (!(state->as_audit_state & AUC_DISABLED)) {
			...
			if (ucred == NULL) {
				...
				if (mask != NULL) {
					state->as_info.ai_mask.as_success =
					    AU_CLASS_64(mask->am_success_lo,
					    mask->am_success_hi);
					state->as_info.ai_mask.as_failure =
					    AU_CLASS_64(mask->am_failure_lo,
					    mask->am_failure_hi);
				} else {
					ucred_free(ucred);
					return (-1);
	...
	if ((state->as_audit_state & AUC_AUDITING) &&
...
adt_changeuser(adt_internal_state_t *state, uid_t ruid)
{
	...
	DPRINTF(("changed mask to %08llX/%08llX for ruid=%d\n",
    ...
    enum adt_user_context user_context)
{
	...
	case ADT_SETTID:
		...
		state->as_info.ai_auid = AU_NOAUDITID;
		state->as_info.ai_asid = 0;
		state->as_info.ai_mask.am_failure = AU_MASK_NONE;
		state->as_info.ai_mask.am_success = AU_MASK_NONE;
    ...
    enum adt_user_context user_context)
{
	...
	case ADT_NEW:
		...
		if (ucred_getauid(ucred) == AU_NOAUDITID) {
			...
			state->as_info.ai_auid = ucred_getauid(ucred);
			state->as_info.ai_asid = ucred_getasid(ucred);
			m = *ucred_getamask(ucred);
			state->as_info.ai_mask.as_success =
			    AU_CLASS_64(m.am_success_lo, m.am_success_hi);
			state->as_info.ai_mask.as_failure =
			    AU_CLASS_64(m.am_failure_lo, m.am_failure_hi);
...
adt_put_event(const adt_event_data_t *event, int status, int return_val)
{
	...
	if ((event_state->ae_session == NULL) ||
	    !(event_state->ae_session->as_audit_state & AUC_AUDITING)) {
		return (0);
	}
	...
	event_state->ae_emod |= status == ADT_SUCCESS ? 0 : PAD_FAILURE;
...
static int
adt_is_selected(au_event_t e, au_mask_t *m, int status, int mode)
{
	return (au_preselect(e, m,
	    status == ADT_SUCCESS ? AU_PRS_SUCCESS : AU_PRS_FAILURE, mode));
}
...
adt_selected(struct adt_event_state *event, au_event_t actual_id, int status)
{
	...
	mode = (sp->as_session_model == ADT_SESSION_MODEL) ?
	    AU_PRS_USECACHE : AU_PRS_REREAD;

	/* non-attributable? */
	if ((sp->as_info.ai_auid == AU_NOAUDITID) ||
	    (sp->as_info.ai_auid == ADT_NO_AUDIT) ||
	    (sp->as_info.ai_auid == ADT_NO_ATTRIB)) {
		if (auditon(A_GETKMASK, (caddr_t)&namask,
		    sizeof (namask)) != 0) {
			adt_write_syslog("auditon failure", errno);
			return (1);
		}
		event->ae_emod |= PAD_NONATTR;
		return (adt_is_selected(actual_id, &namask, status, mode));
	}
	if ((rc = adt_is_selected(actual_id, &(sp->as_info.ai_mask), status,
	    mode)) == 2) {
		/* if forced preselection */
		event->ae_emod |= PAD_NONATTR;
	}
	return (rc);
}
...
adt_get_local_address(int family, struct ifaddrlist *al)
{
	...
	if ((ifal_count = ifaddrlist(&ifal, family, 0, errbuf)) < 0) {
		...
		__auditd_debug("adt_get_local_address couldn't get %d "

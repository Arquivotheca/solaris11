...
au_open(void)
{
	...
	if (au_d_required_length > au_d_length) {
		au_d_new = calloc(au_d_required_length, sizeof (au_d));
...
au_write(int d, token_t *m)
{
	...
	for (mp = au_d[d]; mp->tt_next != NULL; mp = mp->tt_next) {
		continue;
	}
	mp->tt_next = m;
	(void) mutex_unlock(&mutex_au_d);
 ...
 */
int
au_close(int d, int right, au_event_t e_type, au_emod_t e_mod)
{
	...
	for (record = dchain; record != NULL; record = record->tt_next) {

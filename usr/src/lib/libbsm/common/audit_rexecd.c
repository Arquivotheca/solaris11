...
int	sf;
{
	...
	mask.am_success = mask.am_failure = AU_MASK_NONE;
	if (uid > MAXEPHUID) {
...
char	*cmdbuf;	/* command line to be executed locally */
{
	...
	if ((tbuf = malloc(tlen)) == NULL) {
		(void) au_close(rd, AU_TO_NO_WRITE, 0, 0);
		return;
	}
	...
	/* write audit record */
	if (au_close(rd, AU_TO_WRITE, event, PAD_FAILURE) < 0) {
		(void) au_close(rd, AU_TO_NO_WRITE, 0, 0);
		return;
	}
}
...
char	*cmdbuf;	/* command line to be executed locally */
{
	...
	if ((tbuf = malloc(tlen)) == NULL) {
		...
		/* write audit record */
		if (au_close(rd, AU_TO_WRITE, event, 0) < 0) {
			(void) au_close(rd, AU_TO_NO_WRITE, 0, 0);
		}
	}

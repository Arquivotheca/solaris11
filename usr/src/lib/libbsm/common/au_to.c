...
au_to_attr(struct stat64 *attr)
{
	...
	value = (int32_t)attr->st_mode;
	adr_int32(&adr, &value, 1);
	value = (int32_t)attr->st_uid;
	adr_int32(&adr, &value, 1);
	value = (int32_t)attr->st_gid;
	adr_int32(&adr, &value, 1);
	adr_int32(&adr, (int32_t *)&(attr->st_dev), 1);
	adr_int64(&adr, (int64_t *)&(attr->st_ino), 1);
	value64 = (int64_t)attr->st_rdev;
	adr_int64(&adr, &value64, 1);

 ...
 */
static boolean_t
get_prop_str_singleval(char *attr_string, char *key_buf, scf_property_t *prop,
    scf_value_t *prop_val)
{
	...
	case SCF_TYPE_BOOLEAN: {
		...
		len = snprintf(attr_buf, PLUGIN_MAXATT, "%s=%d;", key_buf,
		    pval_bool);
		...
		if (strlcat(attr_string, attr_buf, PLUGIN_MAXATT) >=
		    PLUGIN_MAXATT) {
			prt_error(gettext("Too long attribute string: %s\n"),
			    key_buf);
			return (B_FALSE);
		}
		break;
	...
	case SCF_TYPE_ASTRING: {
		...
		len = snprintf(attr_buf, PLUGIN_MAXATT, "%s=%s;", key_buf,
		    val_buf);
		if (len < 0 || len >= PLUGIN_MAXATT) {
			prt_error(gettext("Too long attribute: %s\n"), key_buf);
			return (B_FALSE);
	...
	case SCF_TYPE_COUNT: {
		...
		len = snprintf(attr_buf, PLUGIN_MAXATT, "%s=%llu;", key_buf,
		    pval_count);
		if (len < 0 || len >= PLUGIN_MAXATT) {
			prt_error(gettext("Too long attribute: %s\n"), key_buf);
			return (B_FALSE);
 ...
 */
static boolean_t
get_prop_str_multival(char *attr_string, char *key_buf, asi_scfhandle_t *handle)
{
	...
	while (scf_iter_next_value(handle_iter_prop.prop,
		...
		if (!get_prop_str_singleval(attr_string, key_buf, handle->prop,
		    handle_iter_prop.prop_val)) {
			scf_free_iter_prop(&handle_iter_prop);
			return (B_FALSE);
		}
	}
    ...
    kva_t **plugin_kva)
{
	...
	while (scf_iter_next_property(handle_iter->prop, handle->prop) == 1) {
		...
		if (scf_property_get_value(handle->prop,
			...
			if (!get_prop_str_singleval((char *)&attr_string,
			    (char *)&key_buf, handle->prop,
			    handle_iter->prop_val)) {
				return (B_FALSE);
			}
		} else {
			...
			if (!get_prop_str_multival((char *)&attr_string,
			    (char *)&key_buf, handle)) {
				return (B_FALSE);
			}
		}

...
cache_free_alias(alias_t *aliasp)
{
	...
	if (aliasp->alias_cxtydz != NULL)
		free(aliasp->alias_cxtydz);
	if (aliasp->alias_did != NULL)
		free(aliasp->alias_did);
...
del_drive_by_name(char *name)
{
	...
	for (listp = disk_listp; listp != NULL; listp = listp->next) {
		...
		for (ap = listp->aliases; ap; ap = ap->next) {
			if (libdiskmgt_str_eq(name, ap->alias_cxtydz) ||
			    libdiskmgt_str_eq(name, ap->alias_did)) {
				del_drive(listp);
				return;
			}
		}
...
{
	if ((ap->alias_cxtydz == NULL) && (ap->alias_did == NULL)) {
		return (0);
	}

	while (listp != NULL) {
		if (libdiskmgt_str_eq(ap->alias_cxtydz, listp->alias_cxtydz) ||
		    libdiskmgt_str_eq(ap->alias_cxtydz, listp->alias_did) ||
		    libdiskmgt_str_eq(ap->alias_did, listp->alias_cxtydz) ||
		    libdiskmgt_str_eq(ap->alias_did, listp->alias_did)) {
			return (1);
		}
		listp = listp->next;
	}

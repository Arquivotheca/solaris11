...
clean_paths(struct search_args *args)
{
	...
	for (cp = args->controller_listp; cp != NULL; cp = cp->next) {
		path_t	**pp;

		for (pp = cp->paths; pp && *pp; pp++)
			remove_invalid_controller((*pp)->name, cp, args);
	}
}
	...
	if (ap != NULL) {
		...
		if (!ap->cluster) {
			...
			*namep = 0;
			...
			if ((ap->alias_did = strdup(alias)) == NULL)
				args->dev_walk_status = ENOMEM;

			ap->cluster = 1;

			size = sizeof (cldevpath) - 1;
			len = basep - devlink_path;
			for (cnt = 0; (cnt < len) && (cnt < size); cnt ++)
				cldevpath[cnt] = devlink_path[cnt];
			cldevpath[cnt] = '\0';

			if (new_devpath(ap, cldevpath) != 0)
				args->dev_walk_status = ENOMEM;
		}
...
get_str_prop(char *prop_name, di_node_t node)
{
	 ...
	 */
	if (di_prop_lookup_strings(DDI_DEV_T_ANY, node, prop_name, &str) > 0) {
		return (str);
	}
	...
	struct search_args *args)
{
	...
	bzero(aliasp, sizeof (alias_t));

	get_disk_name_from_path(devlink_path, alias, sizeof (alias));

	aliasp->alias_cxtydz = strdup(alias);
	if (aliasp->alias_cxtydz == NULL) {
		cache_free_alias(aliasp);
		return (ENOMEM);
	}

	aliasp->alias_did = strdup(alias);
	if (aliasp->alias_did == NULL) {
		cache_free_alias(aliasp);
		return (ENOMEM);
	}
...
static void
remove_controller_reference(char *name, controller_t **ctra)
{
	int cnt;

	if ((name == NULL) || (ctra == NULL))
		return;

	for (cnt = 0; ctra[cnt] != NULL; cnt++) {
		if (libdiskmgt_str_eq(ctra[cnt]->name, name)) {
			int tcnt;
			 ...
			 * (it is a path)
			 */
			for (tcnt = cnt; ctra[tcnt] != NULL; tcnt++)
				ctra[tcnt] = ctra[tcnt + 1];
		}
	}
}
    ...
    struct search_args *args)
{
	 ...
	 */
	for (dp = args->disk_listp; dp != NULL; dp = dp->next)
		remove_controller_reference(name, dp->controllers);

	for (bp = args->bus_listp; bp != NULL; bp = bp->next)
		remove_controller_reference(name, bp->controllers);

	/*
	 * loop through the controllers and remove the controller itself.
	 * The controller being removed is a 'path'.
	 */
	for (pcp = NULL, cp = args->controller_listp; cp != NULL;
	    /* CSTYLED */) {

...
get_attrs(disk_t *dp, int fd, nvlist_t *attrs)
{
	...
	/* check for disks with EFI/GPT label */
	...
	if (efi_alloc_and_read(fd, &efip) >= 0) {
		...
		/*
		 * Install relies on availability of DM_NHEADS and
		 * DM_NSECTORS to be able to re-write a GPT labeled disk
		 * with SMI/VTOC.
		 */
		...
		if (ioctl(fd, DKIOCG_PHYGEOM, &geometry) >= 0) {
			...
			if (nvlist_add_uint32(attrs, DM_NSECTORS,
			    geometry.dkg_nsect) != 0)
				return (ENOMEM);
		}
	} else if (ioctl(fd, DKIOCG_PHYGEOM, &geometry) >= 0) {
#else
	} else if (ioctl(fd, DKIOCGGEOM, &geometry) >= 0) {
#endif
		struct extvtoc	vtoc;

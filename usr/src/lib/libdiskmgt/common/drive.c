...
int
drive_get_part_format_by_name(char *name, int *errp)
{
	struct extvtoc vtoc;
	struct dk_gpt *efip;
	descriptor_t *dp;
	int part_format = FMT_UNKNOWN;
	int status;
	int fd;

	dp = slice_get_descriptor_by_name(name, errp);
	if (dp == NULL) {
		*errp = ENODEV;
		return (part_format);
	}

	if ((fd = drive_open_disk(dp->p.disk, NULL, 0)) < 0) {
		*errp = ENODEV;
		return (part_format);
	}
...
get_assoc_alias(disk_t *diskp, int *errp)
{
	...
	*errp = 0;
	...
	while (aliasp != NULL) {
	    ...
	    if (aliasp->cluster && (aliasp->alias_did != NULL)) {
		cnt++;
	    }
	    aliasp = aliasp->next;
	}

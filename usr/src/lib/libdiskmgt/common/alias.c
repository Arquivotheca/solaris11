...
alias_get_attributes(descriptor_t *dp, int *errp)
{
	...
	*errp = ENODEV;
	for (ap = dp->p.disk->aliases; ap != NULL; ap = ap->next) {
	    if (libdiskmgt_str_eq(dp->name, ap->alias_cxtydz) ||
		libdiskmgt_str_eq(dp->name, ap->alias_did)) {
...
alias_make_descriptors()
{
	...
	while (dp != NULL) {
	    ...
	    ap = dp->aliases;
	    while (ap != NULL) {
		if (ap->alias_cxtydz != NULL) {
		    cache_load_desc(DM_ALIAS, dp, ap->alias_cxtydz, NULL,
			&error);
		    if (error != 0) {
			return (error);
		    }
		}
		if (ap->cluster && (ap->alias_did != NULL)) {
		    cache_load_desc(DM_ALIAS, dp, ap->alias_did, NULL, &error);
		    if (error != 0) {
			return (error);
		    }
		}

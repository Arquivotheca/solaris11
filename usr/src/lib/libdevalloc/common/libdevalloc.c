...
/*
 * da_check_link -
 *	check if the link should be included in device_maps
 *	return 1 if true, 0 if false.
 */
int
da_check_link(const char *link)
{
	if (strchr(link, ':') != NULL)
		return (0);
	return (1);
...
/*
 * Utility routine to make it easy to make type-based decisions in
 * a switch statement.
 */

int
da_str2flag(char *devtype)
{

	if (strcmp(DA_AUDIO_TYPE, devtype) == 0)
		return (DA_AUDIO);

	if (strcmp(DA_CD_TYPE, devtype) == 0)
		return (DA_CD);

	if (strcmp(DA_FLOPPY_TYPE, devtype) == 0)
		return (DA_FLOPPY);

	if (strcmp(DA_TAPE_TYPE, devtype) == 0)
		return (DA_TAPE);

	if (strcmp(DA_RMDISK_TYPE, devtype) == 0)
		return (DA_RMDISK);
...
da_std_type(da_args *dargs, char *namebuf)
{
	...
	type = da_str2flag(dargs->devinfo->devtype);
    ...
    strentry_t **head_devmapp)
{
	...
	uint_t		tmp = 0;
	int		suffix;	/* number at end of devname, i.e. rmdisk1 */
	int		found = 0;
	int		stdtype = 1;
	...
	while ((devmapp = getdmapent()) != NULL) {
		...
		if ((rc = dmap_matchtype(devmapp, dargs->devinfo->devtype))
			...
			if (dargs->optflag & DA_REMOVE) {
				...
				/* no need to track suffix on remove */
				rc = dmap_exact_dev(devmapp,
				    dargs->devinfo->devlist, (int *)NULL);
				...
				if (rc == 0) {
					...
					if ((dargs->optflag & DA_EVENT) == 0)
						log_level = LOG_DEBUG;
					/*
					 * Match allocatable on add is an error
					 * unless just looping through on a
					 * brainless loopy daemon.
			...
			int log_level = LOG_NOTICE;
			/*
			 * Cut down on noise caused by busy daemon.
			 */
	...
	if (dargs->optflag & DA_ADD) {
		 ...
		 * we know the passed-in devname is a useless guess,
		 ...
		 *
		 * So, above, we've been setting bits in tmp_bitmap for each
		 * device of this type found as we loop through DEVMAPS_FILE,
		 * to make sure we don't change the entry for an existing
		 * allocatable device.
		 *
		 */
		 ...
		 *
		 * If the caller chooses a specialized name, then we
		 * trust the caller to keep track of the numbers
		 * (such as sraudio1, printer23) so we ignore the
		 * bitmap and therefore have no need for a limit on
		...
		if (stdtype &&
			...
			/*
			 * Future: support > 63 hotplug devices per type?
			 * DA_MAX_DEVNO was chosen as a handy number for
			 * a bitmap, and is much much higher than the number
			 * of hotpluggable devices we ever expect to see added
			 * by the devfsadmd daemon.
			 */
			if (tmp > DA_MAX_DEVNO)
				return (2);
...
_write_new_defattrs(FILE *fp, da_args *dargs)
{
	...
	char		*lasts;
	da_devinfo_t	*devinfo = dargs->devinfo;
 ...
 */
int
da_add_list(da_devlist_t *dlist, char *link, int new_instance, int flag)
{
	int		instance;
	int		nlen, plen;
	int		new_entry = 0;
	...
	kva_t		*kva;
	da_deventry_t	*dentry = NULL, *nentry = NULL, *pentry = NULL;
	da_defs_t	*da_defs;
	...
	if (nentry == NULL) {
		...
		if ((nentry = (da_deventry_t *)malloc(sizeof (da_deventry_t)))
		    == NULL)
			return (-1);
		if (pentry != NULL)
			pentry->next = nentry;
 ...
 */
int
da_remove_list(da_devlist_t *dlist, char *link, int type, char *devname,
    int size)
{
	int		flag;
	int		remove_dev = 0;
	...
	struct stat	rmstat;
	da_deventry_t	*dentry, *current, *prev;
...
/*ARGSUSED*/
int
da_rm_list_entry(da_devlist_t *dlist, char *link, int type, char *devname)
{
	int		retval = 0;
	da_deventry_t	**dentry, *current, *prev;
 ...
 */
void
da_print_device(int flag, da_devlist_t *devlist)
{
	da_deventry_t	*entry, *dentry;
	da_devinfo_t	*devinfo;

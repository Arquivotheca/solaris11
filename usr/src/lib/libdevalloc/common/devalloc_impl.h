...
#ifndef	_DEVALLOC_IMPL_H
#define	_DEVALLOC_IMPL_H
...
#define	DA_DEFAULT_AUDIO_CLEAN	"/etc/security/lib/audio_clean"
...
typedef struct _da_deventry_t {
	da_devinfo_t		devinfo;
	struct _da_deventry_t	*next;
} da_deventry_t;

typedef struct _da_devlist_t {
	da_deventry_t	*audio;
	da_deventry_t	*cd;
	da_deventry_t	*floppy;
	da_deventry_t	*tape;
	da_deventry_t	*rmdisk;
} da_devlist_t;
...
int da_add_list(da_devlist_t *, char *, int, int);
int da_remove_list(da_devlist_t *, char *, int, char *, int);
int da_rm_list_entry(da_devlist_t *, char *, int, char *);
void da_print_device(int, da_devlist_t *);
int da_str2flag(char *);

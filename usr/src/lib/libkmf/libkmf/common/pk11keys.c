		...
		KMF_DATA *signature)
{
	 ...
	 * mechanisms.
	 */
	rv = kmf_create_pk11_session(&ckSession, mechtype, CKF_VERIFY);
	if (rv != KMF_OK)
		return (rv);
	...
	if (hashmech != 0 &&
		...
		CK_SESSION_HANDLE hashSession = 0;

		hashData.Data = digest;
		hashData.Length = sizeof (digest);

		/* Get handle for the digest operation */
		rv = kmf_create_pk11_session(&hashSession, hashmech,
		    CKF_DIGEST);
		if (rv != KMF_OK)
			goto cleanup;

		rv = PKCS_DigestData(handle, hashSession,
		    hashmech, data, &hashData,
		    (mechtype == CKM_RSA_PKCS));

		(void) C_CloseSession(hashSession);
		if (rv)
			goto cleanup;

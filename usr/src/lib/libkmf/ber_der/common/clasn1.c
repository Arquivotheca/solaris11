...
encode_rdn(BerElement *asn1, KMF_X509_NAME *name)
{
	...
	for (i = 0; i < name->numberOfRDNs; i++) {
		...
		for (j = 0; j < rdn->numberOfPairs; j++) {
			attrtvpair = &rdn->AttributeTypeAndValue[j];
...
static KMF_RETURN
add_new_avpair(KMF_X509_RDN *rdn, KMF_X509_TYPE_VALUE_PAIR *avpair)
{
	if (rdn->AttributeTypeAndValue == NULL) {
		rdn->AttributeTypeAndValue = calloc(1,
		    sizeof (KMF_X509_TYPE_VALUE_PAIR));
		if (rdn->AttributeTypeAndValue == NULL)
			return (KMF_ERR_MEMORY);

		(void) memcpy(rdn->AttributeTypeAndValue, avpair,
		    sizeof (KMF_X509_TYPE_VALUE_PAIR));
		rdn->numberOfPairs = 1;
	} else {
		KMF_X509_TYPE_VALUE_PAIR *ptr;
		KMF_X509_TYPE_VALUE_PAIR *newpair;
		...
		rdn->AttributeTypeAndValue = ptr;
		newpair = &rdn->AttributeTypeAndValue[rdn->numberOfPairs - 1];

		(void) memcpy(newpair, avpair,
		    sizeof (KMF_X509_TYPE_VALUE_PAIR));
...
get_rdn(BerElement *asn1, KMF_X509_NAME *name)
{
	...
	while ((tag = kmfber_next_element(asn1, &size, end)) ==
		 ...
		 * it gets reused.
		 */
		bzero(&newrdn, sizeof (KMF_X509_RDN));
		...
		while ((tag = kmfber_next_element(asn1, &size, end)) ==
			...
			if (kmfber_scanf(asn1, "Dtl", &AttrOID,
			    &tag, &size) == -1) {
				ret = KMF_ERR_BAD_CERT_FORMAT;
				goto cleanup;
			}

	...
	KMF_DATA *output)
{
	...
	/*
	 * Create a new session for doing the digest operation.
	 * This insures that we find a slot that supports the
	 * hash algorithm.  Some hardware devices have limited
	 * support for hashing.
	 */
	if (hashmech != 0) {
		rv = kmf_create_pk11_session(&hSession, hashmech, CKF_DIGEST);
		if (rv != KMF_OK)
			return (rv);
		...
		rv = PKCS_DigestData(handle, hSession, hashmech, tobesigned,
		    &hashData, (mechtype == CKM_RSA_PKCS));
		(void) C_CloseSession(hSession);
		if (rv != KMF_OK)
			return (rv);
	} else {
		/* No hash alg specified, just sign the data we were given */
		hashData.Data = tobesigned->Data;
		hashData.Length = tobesigned->Length;
	}

	/*
	 * Now use the original handle to sign the hash with the
	 * private key.
	 */
	hSession = kmfh->pk11handle;

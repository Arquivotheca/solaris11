 ...
 */
#define	MAX_DAEMON_ATTEMPTS	60
#define	DAEMON_BACKOFF_DELAY	1
...
devlink_create(const char *root, const char *name, int dca_devlink_flag)
{
	...
	if (!priv_ismember(eset, PRIV_SYS_CONFIG)) {
		ucred_free(uc);
		return (EPERM);
	}
	...
	for (;;) {
		...
		if (daemon_call(root, &dca) == 0) {
			 ...
			 */
			if (priv_ismember(eset, PRIV_PROC_FORK) &&
			    priv_ismember(eset, PRIV_PROC_EXEC) &&
			    priv_ismember(eset, PRIV_SYS_DEVICES) &&
			    ucred_geteuid(uc) == 0 && ucred_getegid(uc) == 0) {
				exec_devfsadm(root, &dca);
				rv = 0;
				goto out;
			}
			/*
...
daemon_call(const char *root, struct dca_off *dcp)
{
	...
	(void) snprintf(synch_door, sizeof (synch_door),
	    "%s/etc/dev/%s", root, DEVFSADM_SYNCH_DOOR);
	if ((stat(synch_door, &sb) == -1) || !S_ISDOOR(sb.st_mode)) {
		return (0);
	}
...
static void
exec_devfsadm(const char *root, struct dca_off *dcp)
{
	int i;
	char *argv[8];

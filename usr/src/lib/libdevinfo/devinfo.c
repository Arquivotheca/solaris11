...
/* Return the Configuration Numeric Association of the snapshot. */
uint64_t
di_cna_dev(di_node_t node)
{
	caddr_t pa;

	pa = (caddr_t)node - DI_NODE(node)->self;

	return (DI_ALL(pa)->cna_dev);
}
...
/* Return the Configuration Numeric Association of the node. */
uint64_t
di_node_cna_dev(di_node_t node)
{
	return (DI_NODE(node)->node_cna_dev);
}
...
di_prop_next(di_node_t node, di_prop_t prop)
{
	...
	do {
		...
		case DI_PROP_GLB_LIST:
			...
			break;
		case DI_PROP_VENDOR_GLB_LIST:
			prop = di_prop_vendor_global_next(node, prop);
			break;
		case DI_PROP_ADMIN_GLB_LIST:
			prop = di_prop_admin_global_next(node, prop);
			break;
		case DI_PROP_HW_LIST:
			prop = di_prop_hw_next(node, prop);
			break;
		case DI_PROP_VENDOR_LIST:
			prop = di_prop_vendor_next(node, prop);
			break;
		case DI_PROP_ADMIN_LIST:
			prop = di_prop_admin_next(node, prop);
			break;
		default:	/* shouldn't happen */
			errno = EFAULT;
			return (DI_PROP_NIL);
		}
	} while ((prop == DI_PROP_NIL) && (list < DI_PROP_NLISTS));
...
enum prop_type {
	...
	PROP_TYPE_GLOB,
	PROP_TYPE_HW,
	PROP_TYPE_VENDOR_GLOBAL,
	PROP_TYPE_ADMIN_GLOBAL,
	PROP_TYPE_VENDOR,
	PROP_TYPE_ADMIN
};
...
di_prop_next_common(di_node_t node, di_prop_t prop, int prop_type)
{
	 ...
	 *
	 * Global properties: a device may have no properties
	 * or the properties could differ from those currently
	 * in devnames if forced to reload a driver.conf with
	 * attached instances.  Otherwise, we can simply
	 * use the global properties associated with devnames.
	 */
	pa = (caddr_t)node - DI_NODE(node)->self;
	switch (prop_type) {
	case PROP_TYPE_DRV:
	...
	case PROP_TYPE_HW:
		...
		break;
	case PROP_TYPE_VENDOR_GLOBAL:
		prop_off = DI_NODE(node)->vendor_global_prop;
		if (prop_off == -1) {
			prop_off = 0;		/* no global property */
		} else if ((prop_off == 0) && (DI_NODE(node)->drv_major >= 0)) {
			struct di_devnm *devnm = DI_DEVNM(pa +
			    DI_ALL(pa)->devnames + (DI_NODE(node)->drv_major *
			    sizeof (struct di_devnm)));
			prop_off = devnm->vendor_global_prop;
		}
		break;
	case PROP_TYPE_ADMIN_GLOBAL:
		prop_off = DI_NODE(node)->admin_global_prop;
		if (prop_off == -1) {
			prop_off = 0;		/* no global property */
		} else if ((prop_off == 0) && (DI_NODE(node)->drv_major >= 0)) {
			struct di_devnm *devnm = DI_DEVNM(pa +
			    DI_ALL(pa)->devnames + (DI_NODE(node)->drv_major *
			    sizeof (struct di_devnm)));
			prop_off = devnm->admin_global_prop;
		}
		break;
	case PROP_TYPE_VENDOR:
		prop_off = DI_NODE(node)->vendor_prop;
		break;
	case PROP_TYPE_ADMIN:
		prop_off = DI_NODE(node)->admin_prop;
		break;
	}
...
di_prop_t
di_prop_vendor_global_next(di_node_t node, di_prop_t prop)
{
	return (di_prop_next_common(node, prop, PROP_TYPE_VENDOR_GLOBAL));
}

di_prop_t
di_prop_admin_global_next(di_node_t node, di_prop_t prop)
{
	return (di_prop_next_common(node, prop, PROP_TYPE_ADMIN_GLOBAL));
}
...
di_prop_t
di_prop_vendor_next(di_node_t node, di_prop_t prop)
{
	return (di_prop_next_common(node, prop, PROP_TYPE_VENDOR));
}

di_prop_t
di_prop_admin_next(di_node_t node, di_prop_t prop)
{
	return (di_prop_next_common(node, prop, PROP_TYPE_ADMIN));
}
...
di_walk_hp_callback(di_node_t node, void *argp)
{
	...
	for (hp = DI_HP_NIL; (hp = di_hp_next(node, hp)) != DI_HP_NIL; ) {
		...
		if (!(arg->flag & DI_HP_PORT) &&
		    ((di_hp_type(hp) & DDI_HP_CN_TYPE_CONNECTOR_MASK) == 0))
			continue;
...
size_t
di_hp_state_priv_size(di_hp_t hp)
{
	/*
	 * paranoid error checking
	 */
	...
	return ((size_t)DI_HP(hp)->hp_state_priv_size);
}

char *
di_hp_state_priv(di_hp_t hp)
{
	caddr_t pa;
	...
	if (DI_HP(hp)->hp_state_priv_size == 0) {
		errno = ENXIO;
		return (NULL);
	}

	return ((char *)(pa + DI_HP(hp)->hp_state_priv));
}
...
/* Return the Configuration Numeric Association of the path. */
uint64_t
di_path_cna_dev(di_path_t path)
{
	return (DI_PATH(path)->path_cna_dev);
}

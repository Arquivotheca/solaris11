...
/*
 * Constant info per dir_dev_access() invocation
 */
typedef struct devperm_info {
	di_node_t	cache_snapshot;
	uid_t		uid;
	gid_t		gid;
	mode_t		mode;
	char		*devperm_line;
	void		(*ferror)(char *);
	char		drvname[MAX_LINELEN];
	int		node_matched;
	char		*devfs_path;
} devperm_info_t;
...
static int dir_dev_acc(char *, char *, devperm_info_t *);
 ...
 */
static int
logindevperm(const char *ttyn, uid_t uid, gid_t gid, void (*ferror)(char *))
{
	int err = 0, lineno = 0;
	...
	/* may be null in which case we fall back to DINFOCPYONE */
	info.cache_snapshot = di_init("/", DINFOCACHE);
	...
	while (fgets(line, MAX_LINELEN, fp) != NULL) {
		...
		info.mode = (mode_t)strtol(mode_str, &ptr, 8);
		if (info.mode > 0777 || *ptr != '\0') {
		...
		/* include line from logindevperm for additional parsing */
		info.devperm_line = saveline;

		device = strtok_r(dev_list, ":", &last);
		while (device != NULL) {
			if ((device[0] != '/') || (strlen(device) <= 1))  {
				err = -1;
			} else if (dir_dev_acc("/", &device[1], &info)) {
				err = -1;
			}
			device = strtok_r(last, ":", &last);
		}
	}
	if (info.cache_snapshot)
		di_fini(info.cache_snapshot);
	(void) fclose(fp);
...
{
	devperm_info_t *info = (devperm_info_t *)arg;
	char *path;
	int matched;

	path = di_devfs_path(node);
	matched = strcmp(info->devfs_path, path);
	di_devfs_path_free(path);
	if (matched == 0) {
		info->node_matched = 1;
		(void) strncpy(info->drvname, di_driver_name(node),
		    sizeof (info->drvname));
...
/*
 * Use the cached devinfo snapshot to match the devfs device
 * path and return the driver name for the node.
 */
static int
getdrvname(char *devfs_path, devperm_info_t *info)
{
	di_node_t node;
	int rv;

	info->devfs_path = devfs_path;

	/* first try the cached snapshot */
	if (info->cache_snapshot) {
		info->node_matched = 0;
		rv = di_walk_node(info->cache_snapshot, DI_WALK_CLDFIRST,
		    info, walk_nodes);
		if (rv == 0 && info->node_matched)
			return (1);
	}

	/* fall back to looking up this node directly */
	node = di_init(devfs_path, DINFOCPYONE);
	if (node == NULL)
		return (0);
	(void) strncpy(info->drvname, di_driver_name(node),
	    sizeof (info->drvname));
 ...
 */
static int
check_driver_match(char *path, devperm_info_t *info)
{
	char *driver, *lasts;
	...
	/*
	 * get path and truncate on : to take a snapshot
	 */
	...
	*p = '\0';
	...
	rv = getdrvname(devfs_path, info);
	free(devfs_path);
	if (rv == 0)
		return (0);

	(void) strlcpy(saveline, info->devperm_line, sizeof (saveline));
	...
	if (driver) {
		if (strcmp(driver, "driver") == 0) {
			...
			while (driver) {
				if (strcmp(driver, info->drvname) == 0)
					return (0);
				driver = strtok_r(NULL, ", \t\n", &lasts);
			}
		}
 ...
 */
static int
dir_dev_acc(char *path, char *left_to_do, devperm_info_t *info)
{
	...
	if (stat(path, &stat_buf) == -1) {
		...
		if (!S_ISDIR(stat_buf.st_mode)) {
			if (strlen(left_to_do) == 0) {
				...
				if (check_driver_match(path, info) == 0) {
					/*
	...
	if (snprintf(match, MAXPATHLEN + 2, "^%.*s$",
	    (int)(p ? ((int)(p - left_to_do)) : strlen(left_to_do)),
	    left_to_do) >= MAXPATHLEN + 2) {
		finddev_close(handle);
		free(newpath);
		free(match);
		return (-1);
	...
	while ((name = (char *)finddev_next(handle)) != NULL) {
		if (alwaysmatch ||
			...
			if (dir_dev_acc(newpath, remainder_path, info)) {
				err = -1;
			}
		}
	}
    ...
    void (*ferror)(char *))
{
	...
	if (ttyn == NULL) {
		(*ferror)("di_devperm_login: NULL tty device\n");
		return (-1);
	}
	...
	err = setdevaccess((char *)ttyn, uid, tty_gid,
	    S_IRUSR|S_IWUSR|S_IWGRP, ferror);
	if (err) {
		return (err);
	}

	/* set the device permissions */
	return (logindevperm(ttyn, uid, gid, ferror));
}

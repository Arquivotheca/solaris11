    ...
    int page_size, struct scsi_ms_header *header, void *rqbuf, int *rqblen)
{
	...
	union scsi_cdb cdb;
	int status;
	int maximum = page_size - sizeof (struct mode_page);
	char *pc;
	uchar_t page_data_len;
	...
	if (page_code == MODEPAGE_ALLPAGES) {
		...
		/*
		 * Check that the amount of data that we are copying does not
		 * exceed the page_size.  Note that if the left side of the
		 * equation turns out negative, it will be treated as a large
		 * positive and we will correctly treat that as a failure.
		 */
		page_data_len = (hdr->length +
		    sizeof (header->ms_header.length)) -
		    (MODE_HEADER_LENGTH + hdr->bdesc_length);
		if (page_data_len > page_size) {
			dprintf("Mode sense page 0x%x: incorrect page length "
		...
		(void) memcpy(page_data, (caddr_t)pg, page_data_len);
    ...
    void *rqbuf, int *rqblen)
{
	...
	if (page_code == MODEPAGE_ALLPAGES) {
		 ...
		 * positive and we will correctly treat that as a failure.
		 */
		page_data_len = (length + sizeof (header->ms_header.length)) -
		    (MODE_HEADER_LENGTH_G1 + bdesc_length);
		if (page_data_len > page_size) {
			dprintf("Mode sense page 0x%x: incorrect page length "

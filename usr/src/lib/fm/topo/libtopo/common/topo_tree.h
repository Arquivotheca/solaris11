...
/* WARNING: fma libtopo testsuite code has dependencies on field ordering. */
struct topo_hdl {
	...
	char *th_snap_dir;		/* directory for snapshot file */
	time_t th_timestamp;		/* time when snapshot was created */
	char *th_server;		/* server name */
	char *th_psn;			/* psn */
	char *th_csn;			/* csn */
	di_cromk_hdl_t th_cromk_hdl;	/* CRO create/make handle */
};

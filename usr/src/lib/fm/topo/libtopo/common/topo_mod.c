    ...
    topo_version_t version)
{
	...
	topo_dprintf(pmod->tm_hdl, TOPO_DBG_MODSVC, "%s: name=%s\n",
	    __func__,  name);
	...
	if (topo_is_excluded("TOPOMODEXCLUDE", name))
		return (NULL);

	(void) snprintf(file, PLUGIN_PATH_LEN, "%s/%d/%s.so",
	    PLUGIN_PATH, sizeof (void *) * 8, name);

	path = topo_search_path(pmod, (const char *)file);
	if (path == NULL ||
    ...
    const char *name, topo_instance_t min, topo_instance_t max, void *data)
{
	...
	topo_dprintf(mod->tm_hdl, TOPO_DBG_MODSVC, "%s: enum_name=%s, "
	...
	topo_dprintf(mod->tm_hdl, TOPO_DBG_MODSVC, "%s: module=%s, "
	...
	if (err != 0) {
		...
		topo_dprintf(mod->tm_hdl, TOPO_DBG_ERR,
		    ...
		    __func__, (char *)mod->tm_name, (char *)node->tn_name,
		    node->tn_instance);
		topo_node_rele(node);
		return (-1);
	}
...
di_cromk_hdl_t
topo_mod_cromk_hdl(topo_mod_t *mod)
{
	return (topo_hdl_cromk_hdl(mod->tm_hdl));
}
...
{
	return (topo_mod_strdup(mod, mod->tm_hdl->th_psn));
}

static char *
topo_mod_csn(topo_mod_t *mod)
{
	return (topo_mod_strdup(mod, mod->tm_hdl->th_csn));
}

static char *
topo_mod_server(topo_mod_t *mod)
{
	return (topo_mod_strdup(mod, mod->tm_hdl->th_server));
}

/*ARGSUSED*/
void
topo_mod_auth_update_psn(topo_mod_t *mod, tnode_t *pnode, char *psn)
{
	/* th_psn is not scoped by mod, so don't use top_mod_str  */
	if (mod->tm_hdl->th_psn)
		topo_hdl_strfree(mod->tm_hdl, mod->tm_hdl->th_psn);
	mod->tm_hdl->th_psn = topo_hdl_strdup(mod->tm_hdl, psn);
}

/*ARGSUSED*/
void
topo_mod_auth_update_csn(topo_mod_t *mod, tnode_t *pnode, char *csn)
{
	/* th_csn is not scoped by mod, so don't use top_mod_str  */
	if (mod->tm_hdl->th_csn)
		topo_hdl_strfree(mod->tm_hdl, mod->tm_hdl->th_csn);
	mod->tm_hdl->th_csn = topo_hdl_strdup(mod->tm_hdl, csn);
}

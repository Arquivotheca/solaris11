    ...
    xmlNodePtr n, const char *propname, uint64_t *value)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: propname=%s\n",
	    __func__, propname);
	if ((str = xmlGetProp(n, (xmlChar *)propname)) == NULL)
		return (topo_mod_seterrno(mp, ETOPO_PRSR_NOATTR));
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: str=%s\n", __func__, str);
    ...
    xmlNodePtr xn, const char *propname, nvlist_t **rnvl)
{
	...
	if (topo_mod_str2nvl(mp, (const char *)str, rnvl) < 0) {
		topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: "
		    "topo_mod_str2nvl() failed, str=%s\n", __func__, str);
		xmlFree(str);
		return (-1);
	}
	xmlFree(str);
...
xmlattr_to_type(topo_mod_t *mp, xmlNodePtr xn, xmlChar *attr)
{
	...
	if ((str = xmlGetProp(xn, (xmlChar *)attr)) == NULL) {
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR,
		    "%s: %s attribute missing", __func__, attr);
...
const char *name)
{
	...
	xmlNodePtr cn;
	char *token = NULL;
	boolean_t array_workaround = B_FALSE;

	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: name=%s\n",
	    __func__, name);
	switch (ptype) {
	...
	case TOPO_TYPE_FMRI:
		if (xmlattr_to_fmri(mp, xn, Value, &fmri) < 0) {
			topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR, "%s: "
	...
	case TOPO_TYPE_UINT64_ARRAY:
		...
		if (nelems == 0) {
			topo_dprintf(mp->tm_hdl,
			    (ptype == TOPO_TYPE_UINT32_ARRAY ?
			    TOPO_DBG_SNAP : TOPO_DBG_ERR | TOPO_DBG_SNAP),
			    "%s: no <propitem> or <argitem> elements found "
			    "for array val, name=%s", __func__, name);
			...
			if ((str = xmlGetProp(xn, (xmlChar *)Value)) == NULL) {
				...
				char *str2;
				topo_dprintf(mp->tm_hdl, TOPO_DBG_SNAP,
				 ...
				 * a duplicate string.
				 */
				str2 = (char *)alloca(strlen((char *)str));
				(void) strcpy(str2, (char *)str);
				for (token = strtok(str2, " "); token != NULL;
				    token = strtok(NULL, " "))
					nelems++;
				topo_dprintf(mp->tm_hdl, TOPO_DBG_SNAP,
				    "%s: nelem=%d\n", __func__, nelems);
	...
	case TOPO_TYPE_UINT32_ARRAY:
		...
		if (array_workaround == B_TRUE) {
			...
			for (token = strtok((char *)str, " "); token != NULL;
			    token = strtok(NULL, " ")) {
				((uint32_t *)arrbuf)[i++]
				    = strtoul((const char *)token,
				    (char **)NULL, 16);
			}
			xmlFree(str);
	...
	case TOPO_TYPE_UINT64_ARRAY:
		...
		rv = nvlist_add_uint64_array(nvl, name, arrbuf, nelems);
		topo_mod_free(mp, arrbuf, (nelems * sizeof (uint64_t)));
		break;
	case TOPO_TYPE_STRING_ARRAY:
		for (cn = xn->xmlChildrenNode; cn != NULL; cn = cn->next) {
	...
	case TOPO_TYPE_FMRI_ARRAY:
		...
		rv = nvlist_add_nvlist_array(nvl, name, nvlarrbuf,
		    nelems);
		topo_mod_free(mp, arrbuf, (nelems * sizeof (nvlist_t *)));
		break;
	}
    ...
    tf_info_t *xinfo, tf_pad_t *pad, xmlNodePtr dxn, tnode_t *ptn)
{
	...
	if ((np = topo_xml_walk(mp, xinfo, dxn, ptn)) == NULL) {
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR,
		    ...
		    __func__, topo_strerror(topo_mod_errno(mp)));
		return (-1);
	}
	if (pp != NULL)
		pp->rd_next = np;
    ...
    topo_type_t ptype, int flag)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: pgrp=%s, prop=%s\n",
	    __func__, gnm, pnm);
	switch (ptype) {
	case TOPO_TYPE_INT32:
	...
	if (e != 0 && err != ETOPO_PROP_DEFD) {
		 ...
		 */
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR, "%s: prop set "
		    "failed %s/%s:%s\n", __func__, gnm, pnm,
		    topo_strerror(err));
    ...
    tnode_t *ptn, const char *gnm, nvlist_t **props, int nprops)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: pgrp = %s\n",
	    __func__, gnm);
...
pgroups_create(topo_mod_t *mp, tf_pad_t *pad, tnode_t *ptn)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: %s=%d\n", __func__,
...
register_method(topo_mod_t *mp, tnode_t *ptn, struct propmeth_data *meth)
{
	...
	if (topo_prop_method_version_register(ptn, meth->pg_name,
		...
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR, "%s: failed to register "
    ...
    const char *rname, const char *ppgrp_name)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: %s=%d (pgrp=%s)\n",
	    __func__, topo_node_name(tn), topo_node_instance(tn), pg_name);
    ...
    tf_pad_t *rpad, int pi, const char *ppgrp_name)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: pgroup=%s\n",
	    __func__, (char *)name);
	for (cn = pxn->xmlChildrenNode; cn != NULL; cn = cn->next) {
    ...
    tf_pad_t *rpad, const char *ppgrp)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: pxn->name=%s\n",
	    __func__, pxn->name);
	for (cn = pxn->xmlChildrenNode; cn != NULL; cn = cn->next) {
...
set_contains(topo_mod_t *mp, char *key, char *set)
{
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: set_contains(key = %s, "
	    "setlist = %s)\n", __func__, key, set);
    ...
    tf_pad_t **rpad)
{
	...
	if (new == NULL) {
		...
		topo_dprintf(mp->tm_hdl, TOPO_DBG_XML,
		    ...
		    __func__, dcnt, pgcnt, ecnt, joined_set);
		/*
...
fac_process(topo_mod_t *mp, xmlNodePtr pn, tf_rdata_t *rd, tnode_t *ptn)
{
	...
	for (cn = pn->xmlChildrenNode; cn != NULL; cn = cn->next) {
		...
		topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: done with "
...
node_process(topo_mod_t *mp, xmlNodePtr nn, tf_rdata_t *rd)
{
	...
	if (topo_mod_enumerate(rd->rd_mod, rd->rd_pn,
	    rd->rd_finfo->tf_scheme, rd->rd_name, inst, inst, NULL) < 0) {
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR,
	...
	rv = 0;
nodedone:
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: done with node %s\n",
	    __func__, rd->rd_name);

	if (rv < 0)
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR,
		    "%s: returning error, rv=%d\n", __func__, rv);
...
enum_run(topo_mod_t *mp, tf_rdata_t *rd)
{
	 ...
	 */
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: enumerate request. (%s)\n",
	    __func__, rd->rd_einfo->te_name);
	e = topo_mod_enumerate(rd->rd_mod, rd->rd_pn, rd->rd_einfo->te_name,
	    rd->rd_name, rd->rd_min, rd->rd_max, NULL);
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: back from enumeration. "
	    "%d\n", __func__, e);
	if (e != 0) {
		topo_dprintf(mp->tm_hdl, TOPO_DBG_ERR,
...
fac_enum_run(topo_mod_t *mp, tnode_t *node, const char *name)
{
	...
	topo_dprintf(thp, TOPO_DBG_XML, "%s:\n", __func__);
	/*
	 * Check if the enumerator module is already loaded.
	 * Module loading is single-threaded at this point so there's
	 * no need to worry about the module going away or bumping the
	 ...
	 * We're live, so let's enumerate.
	 */
	topo_dprintf(thp, TOPO_DBG_XML, "%s: fac enumerate request. (%s)\n",
	    __func__, name);
	e = topo_mod_enumerate(fmod, node, name, name, 0, 0, NULL);
	topo_dprintf(thp, TOPO_DBG_XML, "%s: back from enumeration. %d\n",
	    __func__, e);
	if (e != 0) {
		topo_dprintf(thp, TOPO_DBG_ERR,
...
topo_xml_range_process(topo_mod_t *mp, xmlNodePtr rn, tf_rdata_t *rd)
{
	 ...
	 */
	if (pmap_node != NULL) {
		topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: found a propmap "
		    "element\n", __func__);
		if ((pmap_name = xmlGetProp(pmap_node, (xmlChar *)Name))
			...
			if (topo_file_load(mp, rd->rd_pn,
			    (const char *)pmap_name, rd->rd_finfo->tf_scheme,
			    B_TRUE) < 0) {
	...
	topo_dprintf(mp->tm_hdl, TOPO_DBG_XML, "%s: end range process %s\n",
	    __func__, rd->rd_name);
...
topo_xml_enum(topo_mod_t *tmp, tf_info_t *xinfo, tnode_t *troot)
{
	...
	topo_dprintf(tmp->tm_hdl, TOPO_DBG_XML, "%s:\n", __func__);
    ...
    int fd, const char *filenm, const char *escheme)
{
	...
	topo_dprintf(tmp->tm_hdl, TOPO_DBG_XML | TOPO_DBG_SNAP,
	    "%s: tmp=0x%p, thp=0x%p, filenm=%s, escheme=%s, "
	    "load_snapshot=%s\n", __func__, (void *)tmp, (void *)thp,
	    (char *)filenm, (char *)escheme,
	...
	if ((document = xmlReadFd(fd, filenm, NULL, readflags)) == NULL) {
		topo_dprintf(tmp->tm_hdl, TOPO_DBG_ERR,
		    "%s: txml_file_parse: couldn't parse document\n",
		    __func__);
		return (NULL);
	}
	...
	if (load_snapshot == B_TRUE) {
		 ...
		 * Lookup UUID.
		 */
		if ((uuid = xmlGetProp(cursor, (xmlChar *)UUID)) == NULL) {
			topo_dprintf(tmp->tm_hdl, TOPO_DBG_ERR | TOPO_DBG_SNAP,
		...
		if ((thp->th_uuid = topo_hdl_zalloc(thp,
		    TOPO_UUID_SIZE)) == NULL) {
			topo_dprintf(thp, TOPO_DBG_ERR, "%s: unable "
			    "to allocate uuid\n", __func__);
			(void) topo_mod_seterrno(tmp, ETOPO_NOMEM);
			xmlFree(uuid);
			xmlFree(scheme);
			xmlFreeDoc(document);
			return (NULL);
		}
		(void) strncpy(thp->th_uuid, (char *)uuid, TOPO_UUID_SIZE);
		topo_dprintf(thp, TOPO_DBG_ERR, "%s: th_uuid=0x%p(%s)\n",
		    __func__, (void *)thp->th_uuid, thp->th_uuid);
		xmlFree(uuid);

		/*
		 * Lookup timestamp.
		 */
		if ((timestamp = xmlGetProp(cursor, (xmlChar *)Timestamp))
		    == NULL) {
			topo_dprintf(tmp->tm_hdl, TOPO_DBG_ERR | TOPO_DBG_SNAP,
		...
		topo_dprintf(thp, TOPO_DBG_SNAP, "%s: timestamp=%s\n",
		    __func__, timestamp);
		thp->th_timestamp = strtoul((char *)timestamp, NULL, 0);
		topo_dprintf(thp, TOPO_DBG_SNAP, "%s: snapshot th_uuid=%s, "
		    "timestamp=0x%lx=%s=%s\n", __func__, thp->th_uuid,
		    (ulong_t)thp->th_timestamp, timestamp,
		    ctime(&thp->th_timestamp));
		xmlFree(timestamp);
	}
...
topo_xml_read(topo_mod_t *tmp, const char *path, const char *escheme)
{
	...
	if ((fd = open(path, O_RDONLY)) < 0) {
		topo_dprintf(tmp->tm_hdl, TOPO_DBG_SNAP,
		    "%s: failed to open %s\n", __func__, path);
		return (NULL);
	}
	topo_dprintf(tmp->tm_hdl, TOPO_DBG_SNAP, "%s: opened %s\n",
	    __func__, path);

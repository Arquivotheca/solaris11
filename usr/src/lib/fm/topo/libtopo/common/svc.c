 ...
 * state (service state, and presence_state).
 */
...
static int svc_fmri_str2nvl(topo_mod_t *, tnode_t *, topo_version_t,
    nvlist_t *, nvlist_t **);
static int svc_fmri_presence_state(topo_mod_t *, tnode_t *, topo_version_t,
    nvlist_t *, nvlist_t **);
...
/*
 * This common function is shared by all consumers (presence_state
 * and service state).
 *
 ...
 *
 * If none of the above apply and this is a call from the
 * "presence_state" method (presence_only == B_TRUE) then
 * svc_get_state returns FMD_SERVICE_STATE_OK.
 *
 *
 * The "presence_state" methods maps a return of UNKNOWN to
 * FMD_OBJ_STATE_NOT_PRESENT and OK to FMD_OBJ_STATE_UNKNOWN.
 *
 * For the "service state" method svc_get_state goes on
 * to return the instance state as below, and the method maps that
 * result as in the last column of the following table:
 *
 *			svc_get_state succeeds		Service
 * Instance state	with FMD_SERVICE_STATE_*	State
 * --------------	-------------------------------	---------------
 * none			OK				OK
 * uninitialized	OK				OK
 * maintenance		UNUSABLE			UNUSABLE
 * offline		OK				OK
 * disabled		OK				OK
 * online		OK				OK
 ...
 * That's because a service entering maintenance state
...
/*ARGSUSED*/
static int
svc_fmri_presence_state(topo_mod_t *mod, tnode_t *node, topo_version_t version,
    nvlist_t *in, nvlist_t **out)
{
	int state;
	uint8_t fmversion;
	...
	if (nvlist_add_uint32(*out, TOPO_METH_PRESENCE_STATE_RET,

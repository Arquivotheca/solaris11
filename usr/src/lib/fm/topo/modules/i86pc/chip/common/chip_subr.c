 ...
 */
static int
fmri_presence_state(topo_mod_t *mod, tnode_t *node, nvlist_t *unum, int *errp)
{
	tnode_t *dimmnode;
	nvlist_t *resource;
	int err;
	...
	if (topo_prop_get_string(dimmnode, TOPO_PGROUP_PROTOCOL,
		...
		if (err == ETOPO_PROP_NOENT) {
			...
			*errp = EMOD_NVL_INVAL;
			whinge(mod, NULL, "fmri_presence_state: Unexpected "
...
int
rank_fmri_presence_state(topo_mod_t *mod, tnode_t *node, topo_version_t version,
    nvlist_t *in, nvlist_t **out)
{
	int presence_state, err;

	if (version > TOPO_METH_PRESENCE_STATE_VERSION)
		return (topo_mod_seterrno(mod, EMOD_VER_NEW));

	presence_state = fmri_presence_state(mod, node, in, &err);
	if (presence_state == -1)
		return (topo_mod_seterrno(mod,  err));

	fmri_dprint(mod, "rank_fmri_presence_state", presence_state, in);

	return (set_retnvl(mod, out, TOPO_METH_PRESENCE_STATE_RET,
	    presence_state));
}

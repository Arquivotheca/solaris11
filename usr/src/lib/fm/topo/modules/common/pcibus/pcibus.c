    ...
    int board, int bridge, int rc, int devno, int fnno, int depth)
{
	...
	} else if ((subclass == PCI_SERIAL_USB)) {
		if (topo_mod_load(mod, USBTOPO, TOPO_VERSION) == NULL) {
			topo_mod_dprintf(mod, "pcibus enum"
			    "could not load usb enum\n");
			(void) topo_mod_seterrno(mod, EMOD_PARTIAL_ENUM);
		} else {

			if (topo_mod_enumerate(mod, fn, USBTOPO, USB_BUS, fnno,
			    fnno, fn) == 0) {
				topo_mod_dprintf(mod,
				    "pcibus enum usb succeed");
			} else {
				topo_mod_dprintf(mod, "pcibus enum usb fail");
			}
		}
	}
}

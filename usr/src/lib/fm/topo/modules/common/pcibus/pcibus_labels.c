...
pci_label_missing_lookup(topo_mod_t *mod, char *platform, did_t *dp)
{
	...
	for (p = 0; p < Missing_Names->mn_nplats; p++) {
		...
		for (i = 0; i < Missing_Names->mn_names[p].pdl_nnames; i++) {
			...
			if ((m.dl_board ==  -1 || m.dl_board == board) &&
			    (m.dl_bridge == -1 || m.dl_bridge == bridge) &&
			    (m.dl_rc == -1 || m.dl_rc == rc) &&
...
pci_slot_label_lookup(topo_mod_t *mod, tnode_t *node, did_t *dp, did_t *pdp)
{
	...
	int err, b, d, f, done = 0, label_override;
	size_t len;
	...
	if (!label_override) {
		...
		topo_mod_dprintf(mod, "%s: node=%p: node has a slot label "
		    "%s, checking ancestors for slot labels\n", __func__, node,
		    l);

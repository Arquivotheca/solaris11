 ...
 * module registration
 */
static topo_modops_t	pi_ops	= {pi_enum, pi_release};
 ...
 */
static void
pi_release(topo_mod_t *mod, tnode_t *tnp)
{
	if (strncmp(topo_node_name(tnp), BAY, strlen(BAY)) == 0) {
		bay_t	*bp;
		if ((bp = (bay_t *)topo_node_getspecific(tnp)) != NULL) {
			topo_node_setspecific(tnp, NULL);
			topo_mod_free(mod, bp, sizeof (bay_t));
		}
	}
}

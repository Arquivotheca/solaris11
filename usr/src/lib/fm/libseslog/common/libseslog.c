...
fill_host_id(uint8_t *host_id)
{
	...
	if ((prop = scf_simple_prop_get(NULL,
	    "svc:/appliance/kit/identity:default", "factory", "asn")) != NULL) {
		if ((str = scf_simple_prop_next_astring(prop)) != NULL) {
			if (!uuid_parse(str, host_id)) {
				/* Worked okay */
	...
	/* Use host id */
	myhostid = gethostid();
	host_id[12] = (myhostid & 0xff000000) >> 24;
	host_id[13] = (myhostid & 0xff0000) >> 16;
	host_id[14] = (myhostid & 0xff00) >> 8;
	host_id[15] = myhostid & 0xff;
}
...
clear_log(int sg_fd, ses_log_call_t *data)
{
	...
	fill_host_id((uint8_t *)&clear_data.host_id);

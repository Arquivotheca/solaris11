 ...
 */
static boolean_t
role_auth_as_user(char *username)
{
	char *utype, *auth;
	userattr_t *ue;
	...
	ue = getusernam(username);
	if (ue == NULL)
		return (B_FALSE);
	utype = kva_match((kva_t *)ue->attr, USERATTR_TYPE_KW);
	if (utype == NULL || (strcmp(utype, USERATTR_TYPE_ROLE) != 0))
		return (B_FALSE);
	auth = kva_match((kva_t *)ue->attr, USERATTR_ROLE_AUTH_KW);
	if (auth == NULL)
		return (B_FALSE);
	return (strcmp(auth, USERATTR_ROLE_AUTH_USER) == 0);
}
...
pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv)
{
	...
	if (auser != NULL && *auser != '\0' &&
	    strcmp(auser, user) != 0 && role_auth_as_user(user)) {
		user = auser;
	}

...
static boolean_t
is_role(const char *username)
{
	boolean_t	res = B_FALSE;
	userattr_t	*ue;
	char		*utype;

	if ((ue = getusernam(username)) != NULL) {
		utype = kva_match((kva_t *)ue->attr, USERATTR_TYPE_KW);
		if ((utype != NULL) &&
		    (strcmp(utype, USERATTR_TYPE_ROLE) == 0)) {
			res = B_TRUE;
		}
		free_userattr(ue);
	}
...
pam_sm_acct_mgmt(pam_handle_t *pamh, int flags, int argc, const char **argv)
{
	...
	for (i = 0; i < argc; ++i) {
		if (strncasecmp(argv[i], "debug", sizeof ("debug")) == 0) {
			...
			check_exact = B_TRUE;
		} else if (strncasecmp(argv[i], "role", sizeof ("role")) == 0) {
			check_role = B_TRUE;
		} else if (strncasecmp(argv[i], "norole",
		    sizeof ("norole")) == 0) {
			check_role = B_FALSE;
	...
	if (debug) {
		__pam_log(LOG_AUTH | LOG_DEBUG,
		    ...
		    check_user, check_host, check_exact, check_role);
	 ...
	 */
	if ((check_role == B_FALSE) && (is_role(username))) {
		if (debug) {
			__pam_log(LOG_AUTH | LOG_DEBUG,

...
static void
create_varuser(struct passwd *pwd)
{
	char *userdir;

	(void) asprintf(&userdir, "/var/user/%s", pwd->pw_name);
	if (userdir == NULL) { /* memory error */
		syslog(LOG_AUTH | LOG_ERR,
		    "pam_unix_cred: error allocating userdir buffer space.");
		return;
	}
	if (mkdir(userdir, 0700) != 0) {
		/* Don't complain if /var/user is not writable */
		if (errno == EROFS) {
			free(userdir);
			return;
		} else if (errno != EEXIST) {
			syslog(LOG_AUTH | LOG_ERR,
			    "pam_unix_cred: error creating %s: %m", userdir);
		} else {
			/* dir exists, but chmod 0700 to be safe */
			if (chmod(userdir, 0700) != 0 && errno != EROFS)
				syslog(LOG_AUTH | LOG_ERR,
				    "pam_unix_cred: chmod error on %s: %m",
				    userdir);
		}
	}
	if (chown(userdir, pwd->pw_uid, pwd->pw_gid) != 0 && errno != EROFS)
		syslog(LOG_AUTH | LOG_ERR,
		    "pam_unix_cred: chown error on %s: %m",
		    userdir);
...
pam_sm_setcred(pam_handle_t *pamh, int flags, int argc, const char **argv)
{
	...
	if (debug) {
		int	state = AUC_DISABLED;

		(void) auditon(A_GETCOND, (caddr_t)&state, sizeof (state));
		syslog(LOG_AUTH | LOG_DEBUG,
		    "pam_unix_cred: state = %d, auid = %d", state, auid);
	}
...
adt_done:
	...
	/*
	 * Create the /var/user/$USER area if it is not already
	 * present.  pwd buffer refers to PAM_USER.
	 */
	if ((flags & PAM_ESTABLISH_CRED) && ret == PAM_SUCCESS)
		create_varuser(&pwd);
out:
	free(deflim.lim);
	free(deflim.def);

 ...
 * number of parameters to seq.  It also increases the
 * cost appropriately.
 *
...
row(struct Sequence *outseq, int orow, int nrow)
{
	...
	/* try out parameterized up or down motion */
	zero_seq(try);
...
simp_col(struct Sequence *outseq, int oc, int nc)
{
	...
	if (oc < nc) {
		...
		if (0 < init_tabs) {
			...
			/* Distance from tabstop preceding nc in columns. */
			tabstop = nc % init_tabs;
		}
...
column(struct Sequence *outseq, int ocol, int ncol)
{
	...
	/* try out parameterized left or right motion */
	if (ncol < ocol) {

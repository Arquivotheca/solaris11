 ...
 */
int
bigint_bitlen(biginteger_t *bigint)
{
	int		l = 0;
	CK_BYTE		c;
	int		b = 8;

	while ((l < bigint->big_value_len) && (bigint->big_value[l] == 0)) {
		l++;
	}
	...
	c = bigint->big_value[l];
	while ((c & 0x80) == 0) {
		c <<= 1;
		b--;
	}
	l = bigint->big_value_len - l - 1;
    ...
    uint_t mode)
{
	...
	case CKK_RSA:
		...
		/* set the modulus bits derived from modulus for create op */
		if (mode == KERNEL_CREATE_OBJ) {
			KEY_PUB_RSA_MOD_BITS(pbk) =
			    bigint_bitlen(KEY_PUB_RSA_MOD(pbk));
		}
		break;
	...
	if (mode == KERNEL_GEN_KEY) {
		attr_mask |= LOCAL_BOOL_ON;
	}
    ...
    uint_t mode)
{
	...
	case CKK_DH:
		...
		if (mode == KERNEL_CREATE_OBJ) {
			KEY_PRI_DH_VAL_BITS(pvk) =
			    bigint_bitlen(KEY_PRI_DH_VALUE(pvk));
		} else {
			KEY_PRI_DH_VAL_BITS(pvk) =
			    (isValueBits) ? value_bits : 0;
		}

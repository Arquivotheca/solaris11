...
C_GetSlotInfo(CK_SLOT_ID slotID, CK_SLOT_INFO_PTR pInfo)
{
	...
	rv = FUNCLIST(slotID)->C_GetSlotInfo(TRUEID(slotID), pInfo);
...
C_GetTokenInfo(CK_SLOT_ID slotID, CK_TOKEN_INFO_PTR pInfo)
{
	...
	rv = FUNCLIST(slotID)->C_GetTokenInfo(TRUEID(slotID), pInfo);
    ...
    CK_ULONG_PTR pulCount)
{
	...
	if (policyfastpath) {
		...
		true_id = TRUEID(slotID);
		prov_funcs = FUNCLIST(slotID);
	}
	...
	for (i = 0; i < tmpmech_count; i++) {
		...
		if (pkcs11_is_dismech(slotID, pmech_list[i])) {
			continue;
		}
    ...
    CK_MECHANISM_INFO_PTR pInfo)
{
	...
	if (pkcs11_is_dismech(slotID, type)) {
    ...
    CK_UTF8CHAR_PTR pLabel)
{
	...
	rv = FUNCLIST(slotID)->C_InitToken(TRUEID(slotID), pPin, ulPinLen,
	    pLabel);

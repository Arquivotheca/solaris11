...
#define	META_DESTROY_KEY(session, hObject) \
	if ((session) && \
	    ((hObject) != CK_INVALID_HANDLE)) { \
		CK_SLOT_ID fw_st_id = (session)->fw_st_id; \
		CK_SESSION_HANDLE hSession = (session)->hSession; \
		(void)  FUNCLIST(fw_st_id)->C_DestroyObject(hSession, \
		    hObject); \
	}

#define	META_SSL3_MAT_OUT(mech) \
	((CK_SSL3_KEY_MAT_OUT *)((CK_SSL3_KEY_MAT_PARAMS*) \
	    (mech)->pParameter)->pReturnedKeyMaterial)
...
finish:
	...
	if (slot_key1) {
		META_DESTROY_KEY(gen_session, slot_key1->hObject);
		meta_slot_object_dealloc(slot_key1);
	}
	if (slot_key2) {
		META_DESTROY_KEY(gen_session, slot_key2->hObject);
...
finish:
	if (slot_unwrapped_key) {
		META_DESTROY_KEY(unwrap_session, hUnwrappedKey);
...
finish:
	if (slotkey1) {
		if (isSSL) {
			META_DESTROY_KEY(derive_session,
			    META_SSL3_MAT_OUT(pMechanism)->hClientMacSecret);
		} else if (!isTLSPRF) {
			META_DESTROY_KEY(derive_session, hDerivedKey);
		}
		meta_slot_object_dealloc(slotkey1);
	}
	if (slotkey2) {
		META_DESTROY_KEY(derive_session,
		    META_SSL3_MAT_OUT(pMechanism)->hServerMacSecret);
		meta_slot_object_dealloc(slotkey2);
	}
	if (slotkey3) {
		META_DESTROY_KEY(derive_session,
		    META_SSL3_MAT_OUT(pMechanism)->hClientKey);
		meta_slot_object_dealloc(slotkey3);
	}
	if (slotkey4) {
		META_DESTROY_KEY(derive_session,
		    META_SSL3_MAT_OUT(pMechanism)->hServerKey);

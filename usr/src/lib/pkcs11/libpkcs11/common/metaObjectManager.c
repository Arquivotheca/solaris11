 ...
 * process prevents others from seeing the object until fully initialized.
 *
 */
CK_RV
 ...
 * Since slot_object_t is allocated with calloc(), we can assume that
 * hObject is initialized to CK_INVALID_HANDLE which is 0.
 */
CK_RV
...
/*
 * Go through the list of wrapping mechanisms, and return the first
 * one that is supported by both the source and the destination slots.
 * If none of the mechanisms are supported by both slots, return the

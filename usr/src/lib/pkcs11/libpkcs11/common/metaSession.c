    ...
    CK_ULONG_PTR pulOperationStateLen)
{
	...
	if (session->op1.type != 0) {
		...
		opstate.state[0].op_mech = session->init.pMech->mechanism;
		rv = FUNCLIST(slot_session->fw_st_id)->C_GetOperationState(
    ...
    CK_OBJECT_HANDLE hAuthenticationKey)
{
	...
	if (opstate.state[0].op_type != 0) {
		...
		session->init.pMech = calloc(1, sizeof (CK_MECHANISM));
		if (session->init.pMech == NULL) {
			rv = CKR_HOST_MEMORY;
			goto cleanup;
		}
		session->init.pMech->mechanism = opstate.state[0].op_mech;

    ...
    CK_NOTIFY Notify, CK_SESSION_HANDLE_PTR phSession)
{
	...
	if (slotID == METASLOT_FRAMEWORK_ID) {
		...
		rv = FUNCLIST(slotID)->C_OpenSession(true_id, flags,
		    pApplication, Notify, &prov_sess);
	}
	...
	if (slotID == METASLOT_FRAMEWORK_ID) {
		...
		rv = pkcs11_session_add(slottable->st_slots[slotID],
		    slotID, phSession, prov_sess);
	}
...
C_CloseAllSessions(CK_SLOT_ID slotID)
{
	...
	/* Check that slotID is valid is not metaslot's keystore */
	if (slotID == metaslot_keystore_slotid) {
		return (CKR_SLOT_ID_INVALID);
	}

	slotp = slottable->st_slots[slotID];
	true_id = TRUEID(slotID);

	rv = FUNCLIST(slotID)->C_CloseAllSessions(true_id);

    ...
    find_context_t *fcontext)
{
	...
	if (((token_flag_val) || (!token_specified)) &&
		...
		/* acquire soft_giant_mutex - needed in refresh_token_objects */
		(void) pthread_mutex_lock(&soft_giant_mutex);
 ...
 *
 * It is assumed that the caller holds the soft_giant_mutex lock as well
 * as the lock for the token slot
 */
CK_RV

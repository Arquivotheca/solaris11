...
ecb_common:
		...
		/* Allocate a context */
		soft_des_ctx->mode_ctx = (void *)des_ecb_ctx_init(
		    soft_des_ctx->key_sched, soft_des_ctx->keysched_len,
		    key_p->key_type);

		if (soft_des_ctx->mode_ctx == NULL) {
...
cbc_common:
		...
		soft_des_ctx->mode_ctx = (void *)des_cbc_ctx_init(
		    soft_des_ctx->key_sched, soft_des_ctx->keysched_len,
	...
	case CKM_AES_ECB:
		...
		if (1) {
			 ...
			 * AES/ECB mode optimization.  Otherwise, the
			 * block-level AES encrypt functions are
			 * called directly.
			 */
			(void) pthread_mutex_lock(&session_p->session_mutex);
			...
			soft_aes_ctx->mode_ctx = (void *)aes_ecb_ctx_init(
		...
		if (pLastEncryptedPart == NULL) {
			...
			*pulLastEncryptedPartLen = out_len;
			...
			rc = des_encrypt_contiguous_blocks(
			    (des_ctx_t *)soft_des_ctx->mode_ctx,
	...
	boolean_t lock_held)
{
	...
	case CKM_DES3_ECB:
	{
		...
		if (soft_des_ctx != NULL) {
			des_ctx = (des_ctx_t *)soft_des_ctx->mode_ctx;
			if (des_ctx != NULL) {

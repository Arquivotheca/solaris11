...
/*
 * enc_key and hmac_key point at specific objects
 * in token_session's object list.
 */
static soft_object_t	*enc_key = NULL;
static soft_object_t	*hmac_key = NULL;
static char		keystore_path[MAXPATHLEN] = { '\0' };
...
get_keystore_path()
{
	...
	struct passwd pwd, *user_info = NULL;
	char pwdbuf[PWD_BUFFER_SIZE];

	if (strcmp(keystore_path, "") != 0) {
	 ...
	 * from the environment.  Note that if we are running a setuid
	 * program we want to use effective uid to determine
	 * the keystore_path.
	 *
	...
	if ((alt != NULL) && (strcmp(alt, "") != 0)) {
		...
		if ((getpwuid_r(geteuid(), &pwd, pwdbuf, PWD_BUFFER_SIZE,
		    &user_info) == 0) && (user_info != NULL) &&
		    (user_info->pw_dir != NULL)) {
			(void) snprintf(keystore_path, MAXPATHLEN,
			    "%s/%s/%s", user_info->pw_dir,
			    SUNW_PATH, KEYSTORE_PATH);
		}
...
create_keystore()
{
	...
	/*
	 * The keystore doesn't exist, try to create keystore directory.
	 * get_keystore_path() either returns a path to create keystore
	 * directory or NULL string when path could not be determined.
	 */
	if (strcmp(get_keystore_path(), "") != 0) {
		if (mkdirp(keystore_path, S_IRUSR|S_IWUSR|S_IXUSR) < 0) {
			/*
			 * Even though the standard requires that keys and data
			 * be deleted on an existing keystore when C_InitToken()
			 * is called, we have chosen to leave the keystore
			 * intact.  It seems, otherwise, the punishment for an
			 * extraneous call to C_InitToken() would be too severe,
			 * particularly in the case where the keystore is
			 * persistent but the callers are transitory.
			 */
			if (errno == EEXIST) {
				return (0);
			}

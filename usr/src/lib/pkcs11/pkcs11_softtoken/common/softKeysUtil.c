...
errorexit:
	if (IS_TOKEN_OBJECT(public_key)) {
		...
		soft_delete_object(session_p, public_key, B_FALSE, B_FALSE);
	}
	if (IS_TOKEN_OBJECT(private_key)) {
		soft_delete_token_object(private_key, B_FALSE, B_FALSE);
	} else {
		soft_delete_object(session_p, private_key, B_FALSE, B_FALSE);
	}
	return (rv);
	...
	CK_BYTE *blockdata, CK_ULONG blocklen)
{
	...
	for (i = 0; i < params->iterations; i++) {
		/*
		 * The key doesn't change, it's always the
		 * password initially given.
		 */
		...
		CK_OBJECT_HANDLE_PTR phKey)
{
	 ...
	 * ALWAYS_SENSITIVE_BOOL_ON; if they are TRUE, the template must
	...
	if (new_objp->class == CKO_SECRET_KEY) {
		...
		if (OBJ_SEC_VALUE_LEN(new_objp) == 0) {
			...
			goto cleanup_unwrap;
		} else {	/* between 0 and plain_len, hence padded */

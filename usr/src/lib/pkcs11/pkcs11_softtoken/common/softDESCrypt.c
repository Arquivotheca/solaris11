    ...
    soft_session_op_t op)
{
	...
	soft_set_session_context(session_p, op, soft_des_ctx, pMechanism);
 ...
 */
void *
des_ecb_ctx_init(void *key_sched, size_t size, CK_KEY_TYPE type)
{
	...
	if (type == CKK_DES)
		...
		ecb_ctx->ecb_flags |= ECB_MODE | DES3_STRENGTH;
	...
	boolean_t sign_op, boolean_t Final)
{
	...
	if (Final) {
		...
		if (soft_des_ctx_encrypt->remain_len != 0) {
			 ...
			 * soft_des_ctx_encrypt->mode_ctx->des_ctx->dc_lastp
			 * Copy that data to last_block
			 */
			soft_des_ctx_encrypt =
			    (soft_des_ctx_t *)session_p->encrypt.context;
			des_ctx = (des_ctx_t *)soft_des_ctx_encrypt->mode_ctx;

...
soft_dh_genkey_pair(soft_object_t *pubkey, soft_object_t *prikey)
{
	...
	OBJ_PRI_DH_VAL_BITS(prikey) = k.x_len_bits;
    ...
    void *publicvalue, size_t publicvaluelen)
{
	...
	k.x_len_bits = CRYPTO_BYTES2BITS(privatevaluelen);
	k.public_y = publicvalue;
	k.y_len_bits = CRYPTO_BYTES2BITS(publicvaluelen);
	k.rfunc = NULL;

	...
	soft_object_t *new_object, CK_ULONG mode, CK_KEY_TYPE key_type)
{
	...
	case CKK_DH:
		...
		if (mode == SOFT_GEN_KEY) {
			...
			KEY_PRI_DH_VAL_BITS(pvk) = bigint_bitlen(&value);
		}
...
fail_cleanup:
	if (rv != CKR_OK) {
		free(cert);
		new_object->object_class_u.certificate = NULL;

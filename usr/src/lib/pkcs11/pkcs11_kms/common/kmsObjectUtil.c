    ...
    CK_ULONG ulCount, find_context_t *fcontext)
{
	...
	for (i = 0; i < ulCount; i++) {
		...
		if (pTemplate[i].type == CKA_CLASS) {
			findclass = *((CK_OBJECT_CLASS *)pTemplate[i].pValue);
		}
	}
	...
	if (token_flag_val || !token_specified) {
		 ...
		 * local list.
		 */
		if (!found && label != NULL && findclass == CKO_SECRET_KEY) {
			kms_object_t *pObj = NULL;
			rv = KMS_RetrieveKeyObj(sp, label, &pObj);
			if (rv == CKR_OK && pObj != NULL) {
				/*
				 * If we DID find the object, add it to the
				 * slot token object list and label list.
				 */
				rv = add_to_search_result(pObj, fcontext,
				    &num_result_alloc);
				if (rv != CKR_OK) {
					kms_free_object(pObj);
					goto token_done;
				}

				add_label_node(&sp->objlabel_tree, label);
				rv = kms_update_label_file(sp);
				if (rv != CKR_OK) {
					kms_free_object(pObj);
					goto token_done;
				}

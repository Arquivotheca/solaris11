	...
	nvlist_t *attr_list)
{
	...
	/*
	 * Calculate the publisher size plus string separators and maximum
	 * pid characters
	 */
 ...
 *			It is created as a separate thread in each subscriber
...
sysevent_open_channel(const char *channel)
{
	...
	/*
	 * Check that _PATH_SYSVOL is mounted as tmpfs before allowing a channel
	 * to be opened.
	 */
	if (statvfs(_PATH_SYSVOL, &vfsbuf) != 0 ||
	    strcmp(vfsbuf.f_basetype, MNTTYPE_TMPFS) != 0) {
		errno = EACCES;
		return (NULL);
	}

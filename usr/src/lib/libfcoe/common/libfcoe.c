...
FCOE_STATUS
FCOE_Set_Priority(FCOE_PORT_PROP *fpp, uint8_t pvalue)
{
	if (fpp == NULL)
		return (FCOE_STATUS_ERROR_INVAL_ARG);

	fpp->port_pmask |= FCOE_DCB_PRIORITY;
	fpp->port_priority = pvalue;

	return (FCOE_STATUS_OK);
}

FCOE_STATUS
FCOE_SetpropPort(const FCOE_UINT8 *macLinkName, const FCOE_PORT_PROP *fpp)
{
	FCOE_STATUS	status = FCOE_STATUS_OK;
	int		fcoe_fd;
	fcoeio_t	fcoeio;
	...
	datalink_id_t	linkid;
	int		io_ret = 0;
	fcoeio_setprop_port_t fsp;

	/* the only supported property is the DCB priority */
	if (macLinkName == NULL || fpp == NULL ||
	    fpp->port_pmask != FCOE_DCB_PRIORITY) {
		return (FCOE_STATUS_ERROR_INVAL_ARG);
	}
	...
	fsp.fsp_mac_linkid = linkid;
	fsp.fsp_mask = fpp->port_pmask;
	fsp.fsp_priority = fpp->port_priority;

	(void) memset(&fcoeio, 0, sizeof (fcoeio));
	fcoeio.fcoeio_cmd = FCOEIO_SETPROP_FCOE_PORT;
	fcoeio.fcoeio_ilen = sizeof (fcoeio_setprop_port_t);
	fcoeio.fcoeio_xfer = FCOEIO_XFER_WRITE;
	fcoeio.fcoeio_ibuf = (uintptr_t)&fsp;

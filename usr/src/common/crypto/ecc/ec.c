...
#ifndef EC_USE_MONTGOMERY
	if (params->name != ECCurve_noName) {
		group = ECGroup_fromName(params->name, kmflag);
	}
#else /* EC_USE_MONTGOMERY */
	group = NULL;
	if (params->name != ECCurve_noName) {
		...
		if (params->fieldID.type == ec_field_GFp) {
			...
			group = ECGroup_consGFp_mont(&irreducible, &a, &b, &Gx, &Gy, &order, params->cofactor);
		} else {
...
EC_ValidatePublicKey(ECParams *ecParams, SECItem *publicValue, int kmflag)
{
    ...
    /* Initialize Px and Py */
    CHECK_MPI_OK( mp_read_unsigned_octets(&Px, publicValue->data + 1,
	(mp_size) len) );
    CHECK_MPI_OK( mp_read_unsigned_octets(&Py, publicValue->data + 1 + len,
	(mp_size) len) );
	    ...
	    int kmflag)
{
    ...
    if ((rv = EC_ValidatePublicKey(ecParams, publicValue, kmflag)) !=
	SECSuccess) {
	    return rv;
    }
...
void
EC_FreePrivateKey(ECPrivateKey *key, boolean_t freeit)
{
	if (key == NULL)
		return;

	SECITEM_FreeItem(&key->publicValue, freeit);
	SECITEM_ZfreeItem(&key->privateValue, freeit);
	SECITEM_FreeItem(&key->version, freeit);

	if (freeit)
		PORT_Free(key, sizeof (ECPrivateKey));
}

void
EC_FreePublicKey(ECPublicKey *key, boolean_t freeit)
{
	if (key == NULL)
		return;
	...
	SECITEM_FreeItem(&key->publicValue, freeit);
	if (freeit)
		PORT_Free(key, sizeof (ECPublicKey));
}

void
EC_FreeDerivedKey(SECItem *secret, boolean_t freeit)
{
	SECITEM_FreeItem(secret, freeit);
}

void
EC_FreeParams(ECParams *ecParams, boolean_t freeit)
{
	if (ecParams == NULL)
		return;

	SECITEM_FreeItem(&ecParams->fieldID.u.prime, freeit);
	SECITEM_FreeItem(&ecParams->fieldID.u.poly, freeit);
	SECITEM_FreeItem(&ecParams->curve.a, freeit);
	SECITEM_FreeItem(&ecParams->curve.b, freeit);
	SECITEM_FreeItem(&ecParams->curve.seed, freeit);
	SECITEM_FreeItem(&ecParams->base, freeit);
	SECITEM_FreeItem(&ecParams->order, freeit);
	SECITEM_FreeItem(&ecParams->DEREncoding, freeit);
	SECITEM_FreeItem(&ecParams->curveOID, freeit);

	if (freeit)
		PORT_Free(ecParams, sizeof (ECParams));
}

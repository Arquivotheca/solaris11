...
static void Encode(uint8_t *_RESTRICT_KYWD output,
    const uint32_t *_RESTRICT_KYWD input, size_t input_len);

#ifdef	__amd64

#define	MD5_MULTIBLOCK(ctx, input, len)	\
				md5_block_asm_host_order(ctx, input, len)
void md5_block_asm_host_order(MD5_CTX *ctx, const void *inpp,
    unsigned int input_length_in_blocks);

#elif defined(sun4v)

void yf_md5_multiblock(MD5_CTX *ctx, const uint8_t *input,
    unsigned int input_length_in_blocks);

#ifdef	HWCAP_MD5

#define	MD5_MULTIBLOCK(ctx, input, len)	yf_md5_multiblock(ctx, input, len)

#elif defined(_KERNEL)
static boolean_t yf_md5_instruction_present(void);
#define	MD5_MULTIBLOCK(ctx, input, len)	\
	if (yf_md5_instruction_present()) { \
		yf_md5_multiblock(ctx, input, len); \
	} else { \
		base_md5_multiblock(ctx, input, len); \
	}

static void MD5Transform(MD5_CTX *ctx, const uint8_t input[64]);
static void base_md5_multiblock(MD5_CTX *ctx, const uint8_t *input,
    unsigned int input_length_in_blocks);

#else	/* sun4v && !HWCAP_MD5 && !_KERNEL */

#define	MD5_MULTIBLOCK(ctx, input, len)	base_md5_multiblock(ctx, input, len)
...
#if defined(sun4v) && defined(HWCAP_MD5)
	ctx->state[0] = 0x01234567U;
	ctx->state[1] = 0x89abcdefU;
	ctx->state[2] = 0xfedcba98U;
	ctx->state[3] = 0x76543210U;
#else
...
{
	uint32_t		i, buf_index, remaining_len;
#if defined(sun4v) && !defined(HWCAP_MD5)
	uint32_t		old_asi;
#endif /* sun4v && !HWCAP_MD5 */
#if defined(sun4v) && defined(_KERNEL)
	...
	if (input_len >= remaining_len) {
...
#if defined(sun4v) && defined(_KERNEL)
		save_fp = yf_md5_instruction_present();
		SAVE_FP;
#endif
		/*
...
#if defined(sun4v) && !defined(HWCAP_MD5)
		/*
		 * For T1-3 use %asi register. However, costly to repeatedly
		 *  setin MD5Transform. Therefore, set once here.
		...
		if (buf_index) {
			...
			MD5_MULTIBLOCK(ctx, ctx->buf_un.buf8, 1);
			i = remaining_len;
		}

		block_count = (input_len - i) >> 6;
		if (block_count > 0) {
			MD5_MULTIBLOCK(ctx, &input[i], block_count);
			i += block_count << 6;
		}
...
#if defined(sun4v) && defined(HWCAP_MD5)
	bcopy((char *)ctx->state, (char *)digest, 16);
#else
#if defined(sun4v) && defined(_KERNEL)
	if (!yf_md5_instruction_present()) {
...
#if !(defined(__amd64) || (defined(sun4v) && defined(HWCAP_MD5)))
...
static void
MD5Transform(MD5_CTX *ctx, const uint8_t block[64])
{
...
#pragma inline(base_md5_multiblock)
static
void base_md5_multiblock(MD5_CTX *ctx, const uint8_t *input,
    unsigned int input_length_in_blocks)
{
	int	i;

	for (i = 0; i < input_length_in_blocks; i++)
		MD5Transform(ctx, &input[i << 6]);

}
#endif /*  !(defined(__amd64) || (defined(sun4v) && defined(HWCAP_MD5))) */
...
#if defined(sun4v) && defined(_KERNEL)
/*
 * Return true if executing on an architecture (like T4) with md5 instruction,
 * otherwise false.
 */
#pragma inline(yf_md5_instruction_present)
static boolean_t
yf_md5_instruction_present(void)
{
	return ((cpu_hwcap_flags & AV_SPARC_MD5) != 0);
}
#endif /* sun4v */

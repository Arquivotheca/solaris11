...
#define	MAX_DH_KEYLENGTH_IN_BYTES	1024
#define	DH_MIN_KEY_LEN		64	/* DH min key length in bits */
#define	DH_MAX_KEY_LEN		8192	/* DH max key length in bits */
...
typedef struct {
	...
	uchar_t *base;		/* g */
	uchar_t	*private_x;	/* x */
	uint32_t x_len_bits;	/* length for x */
	uchar_t *public_y;	/* y */
	uint32_t y_len_bits;	/* length for y */

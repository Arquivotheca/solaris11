    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	lastp = (uint8_t *)ctx->ccm_cb; /* saved partial-block output buffer */
	if (out != NULL)
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	...
	if ((ctx->ccm_flags & NO_CTR_FINAL) == 0) {
		if (out->cd_length <
		...
		if (ctx->ccm_remainder_len > 0) {
			...
			lastp = (uint8_t *)ctx->ccm_tmp;
			encrypt_block(ctx->ccm_keysched,
			    (uint8_t *)ctx->ccm_cb, lastp);
			...
			if (out_data_2 == NULL) {
				...
				if (out_data_1_len < ctx->ccm_remainder_len) {
					...
					if (out_data_1_len ==
						 ...
						 * out_data_1, part of the mac
						 * will be in out_data_2
						 */
						bcopy(ccm_mac_p,
						    out_data_1 +
						    ctx->ccm_remainder_len,
		...
		if (out->cd_length < ctx->ccm_mac_len) {
			return (CRYPTO_DATA_LEN_RANGE);
		}

		if (ctx->ccm_processed_data_len != ctx->ccm_data_len) {
			return (CRYPTO_DATA_LEN_RANGE);
		}
		...
		if (ctx->ccm_remainder_len > 0) {
			...
			/*
			 * ccm_mac_input_buf holds the unencrypted
			 * last partial block
			 */
			macp = (uint8_t *)ctx->ccm_mac_input_buf;
			bzero(macp + ctx->ccm_remainder_len,
			    block_size - ctx->ccm_remainder_len);
		...
		rv = crypto_put_output_data(ccm_mac_p, out, ctx->ccm_mac_len);

		if (rv != CRYPTO_SUCCESS) {
			return (rv);
		}

		out->cd_offset += ctx->ccm_mac_len;
		ctx->ccm_remainder_len = 0;
		return (CRYPTO_SUCCESS);
	}
}
    ...
    void (*xor_block)(uint8_t *, uint8_t *))
{
	 ...
	 */
	if (ctx->ccm_processed_data_len + length >
	    ctx->ccm_data_len + ctx->ccm_mac_len) {
		return (CRYPTO_ENCRYPTED_DATA_LEN_RANGE);
	}

	bcopy(data,
	    (&((uchar_t *)ctx->ccm_pt_buf)[ctx->ccm_processed_data_len]),
	    length);
	ctx->ccm_processed_data_len += length;
	return (CRYPTO_SUCCESS);
}
    ...
    void (*cbcmac_multiblock)(void *ctx, uint64_t *data, size_t length))
{
	...
	ASSERT(ctx->ccm_processed_data_len ==
	    ctx->ccm_data_len + ctx->ccm_mac_len);

	pt_len = ctx->ccm_data_len;
	blockp = ctx->ccm_pt_buf;
	tmpout.cd_format = CRYPTO_DATA_RAW;
	tmpout.cd_offset = 0;
	tmpout.cd_length = pt_len;
	tmpout.cd_raw.iov_base = blockp;
	tmpout.cd_raw.iov_len = pt_len;

	flags = ctx->ccm_flags;
	ctx->ccm_flags = (ctx->ccm_flags & (~MODE_MASK)) | CTR_MODE;
	ctx->ccm_cb[1] = (ctx->ccm_cb[1] & ~(ctx->ccm_counter_mask)) |
	    htonll(1);

	decrypt_contiguous_blocks(ctx, (char *)blockp, pt_len, &tmpout);
	...
	if (remainder >= block_size) {
		...
		cbcmac_multiblock(ctx, blockp, num_blocks);
	...
	if (bcmp(&(((uchar_t *)(ctx->ccm_pt_buf))[ctx->ccm_data_len]),
	    ccm_mac_p, ctx->ccm_mac_len)) {
		/* They don't match */
		return (CRYPTO_INVALID_MAC);
	} else {
		rv = crypto_put_output_data((uchar_t *)(ctx->ccm_pt_buf),
		    out, pt_len);
		if (rv != CRYPTO_SUCCESS)
			return (rv);
    ...
    ulong_t authDataSize, uint8_t *b0, ccm_ctx_t *aes_ctx)
{
	...
	aes_ctx->ccm_ctr.ctr_lower_mask = aes_ctx->ccm_counter_mask;
	aes_ctx->ccm_ctr.ctr_upper_mask = 0;
}
...
ccm_init(ccm_ctx_t *ctx, unsigned char *nonce, size_t nonce_len,
    ...
    int (*encrypt_block)(const void *, const uint8_t *, uint8_t *),
    void (*xor_block)(uint8_t *, uint8_t *), boolean_t save_fp)
{
...
NOTE(ARGUNUSED(kmflag))
	...
	if (ccm_init(ccm_ctx, ccm_param->nonce, ccm_param->ulNonceSize,
	    ...
	    encrypt_block, xor_block, save_fp) != 0) {
		rv = CRYPTO_MECHANISM_PARAM_INVALID;
		goto out;
	}
	if (!is_encrypt_init) {
		/* allocate buffer for storing decrypted plaintext */
		ccm_ctx->ccm_pt_buf = CRYPTO_ALLOC(ccm_ctx->ccm_data_len +
		    ccm_ctx->ccm_mac_len, kmflag);
...
NOTE(ARGUNUSED(kmflag))
	...
	if ((ccm_ctx = CRYPTO_ZALLOC(sizeof (ccm_ctx_t), kmflag)) == NULL)
		return (NULL);

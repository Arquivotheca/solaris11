...
mb_put_ntsd(mbchain_t *mbp, i_ntsd_t *sd)
{
	...
	/* The SD is "self-relative" on the wire. */
	flags = sd->sd_flags | SE_SELF_RELATIVE;
...
smbfs_zaf2naf[] = {
	{ ACE_FILE_INHERIT_ACE,		OBJECT_INHERIT_ACE },
	{ ACE_DIRECTORY_INHERIT_ACE,	CONTAINER_INHERIT_ACE },
	{ ACE_NO_PROPAGATE_INHERIT_ACE,	NO_PROPAGATE_INHERIT_ACE },
	{ ACE_INHERIT_ONLY_ACE,		INHERIT_ONLY_ACE },
	{ ACE_INHERITED_ACE,		INHERITED_ACE },
	...
	uid_t *uidp, gid_t *gidp)
{
	...
	if (acl_info) {
		...
		if (acl_info->acl_type != ACE_T ||
			...
			return (EINVAL);
#endif /* _KERNEL */
		if ((sd->sd_flags & SE_DACL_PRESENT) == 0)
			return (EINVAL);
	}
	...
	if (sd->sd_group)
		mapcnt++;
	if ((sd->sd_flags & SE_SACL_PRESENT) &&
	    (sd->sd_sacl != NULL))
		mapcnt += sd->sd_sacl->acl_acecount;
	if ((sd->sd_flags & SE_DACL_PRESENT) &&
	    (sd->sd_dacl != NULL))
	...
	/* NB, have: (sd->sd_flags & SE_DACL_PRESENT) */
	if ((sd->sd_dacl != NULL) &&
	...
	i_ntsd_t **sdp)
{
	 ...
	 */
	mapcnt = 0;
	if (selector & SMB_OWNER_SECINFO) {
		if (own_uid == (uid_t)-1)
			return (EINVAL);
		mapcnt++;
	}
	if (selector & SMB_GROUP_SECINFO) {
		if (own_gid == (gid_t)-1)
			return (EINVAL);
		mapcnt++;
	}
	if (selector & (SMB_DACL_SECINFO | SMB_SACL_SECINFO)) {
		if (acl_info == NULL)
			return (EINVAL);
		if (own_uid == (uid_t)-1)
			return (EINVAL);
	...
	if (selector & SMB_DACL_SECINFO) {
		...
		sd->sd_dacl = acl;
		acl = NULL;
		sd->sd_flags |= SE_DACL_PRESENT;
	}

	if (selector & SMB_SACL_SECINFO) {
		/*

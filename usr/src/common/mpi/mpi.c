...
/*
 * converts an mp_int with 32-bit "digits" to an array
 * of 64-bit numbers (least significant 64 bits in
 * *digits[0], etc.)
 * The caller may supply a pre-allocated array by giving
 * its address in *digits and its size (#of uint64_t's
 * allocated) in *numdigits. If a bigger size is required,
 * this function will allocate it and change *digits and
 * *numdigits accordingly. In this case, the caller should
 * deallocate the new buffer (and the old one, too).
 */
mp_err
conv_mp32_to_uint64arr(const mp_int *a, int maxused,
    uint64_t **digits, int *numdigits)
{
	int		i, n, useda;
	uint64_t	*d;
	mp_digit	*ad;
	...
	useda = MP_USED(a);
	n = (maxused + 1) / 2;
	if ((*digits == NULL) || (n > *numdigits)) {
		d = s_mp_alloc(n, sizeof (uint64_t), MP_FLAG(a));
	} else {
		d = *digits;
	}
...
void
conv_uint64arr_to_mp32(uint64_t *d, mp_int *c)
{
	mp_digit	*cd;
	int		n, i;
	...
	n = MP_USED(c);
	for (i = 0; i < (n + 1) / 2 - 1; i++) {
		cd[2 * i] = d[i] & 0xffffffff;
		cd[2 * i + 1] = d[i] >> 32;
	}
	if (n & 1) {
		cd[n - 1] =  d[(n - 1) / 2] & 0xffffffff;
	} else {
		cd[n - 2] =  d[(n - 1) / 2] & 0xffffffff;
		cd[n - 1] =  d[(n - 1) / 2] >> 32;
	}
}
...
#error this only works with BIG_CHUNK_SIZE == 64
#endif
		uint64_t	*adigits = NULL;
		uint64_t	*bdigits = NULL;
		uint64_t	*cdigits = NULL;
#ifdef MP_USE_UINT_DIGIT
		int		asize, bsize;
		
		if((res = conv_mp32_to_uint64arr(a, useda,
		    &adigits, &asize)) != MP_OKAY) {
			goto CLEANUP;
		}
		if((res = conv_mp32_to_uint64arr(b, useda,
		    &bdigits, &bsize)) != MP_OKAY) {
			s_mp_free(adigits, asize * sizeof (uint64_t));
			goto CLEANUP;
		}
		
		/*
		 * since useda is in units of 32-bit words,
		 * the following allocates enough memory for the
		 * product
		 */
		if ((cdigits = s_mp_alloc((useda + 1), sizeof (uint64_t),
		    MP_FLAG(a))) == NULL) {
			s_mp_free(adigits, asize * sizeof (uint64_t));
			s_mp_free(bdigits, bsize * sizeof (uint64_t));
			goto CLEANUP;	
		}
		big_mul_arr_64(cdigits, adigits, bdigits, asize);
		conv_uint64arr_to_mp32(cdigits, c);
		s_mp_free(adigits, asize * sizeof (uint64_t));
		s_mp_free(bdigits, bsize * sizeof (uint64_t));
		s_mp_free(cdigits, (useda + 1) * sizeof (uint64_t));
		...
		bdigits = MP_DIGITS(b);
		cdigits = MP_DIGITS(c);
		if ((adigits = s_mp_alloc(useda, sizeof (uint64_t),
		    MP_FLAG(a))) == NULL) {
			goto CLEANUP;	
		}
		if ((bdigits = s_mp_alloc(useda, sizeof (uint64_t),
		    MP_FLAG(a))) == NULL) {
			s_mp_free(adigits, useda * sizeof (uint64_t));
			goto CLEANUP;	
		}
		if ((cdigits = s_mp_alloc(2 * useda, sizeof (uint64_t),
		    MP_FLAG(a))) == NULL) {
			s_mp_free(adigits, useda * sizeof (uint64_t));
			s_mp_free(bdigits, useda * sizeof (uint64_t));
			goto CLEANUP;	
		}
		for (i = 0; i < MP_USED(a); i++) {
			adigits[i] = MP_DIGIT(a, i);
		}
		for (i =  MP_USED(a); i < useda; i++) {
			adigits[i] = 0;
		}
		for (i = 0; i < MP_USED(b); i++) {
			bdigits[i] = MP_DIGIT(b, i);
		}
		for (i =  MP_USED(b); i < useda; i++) {
			bdigits[i] = 0;
		}
		big_mul_arr_64(cdigits, adigits, bdigits, useda);
		for (i = 0; i < 2 * useda; i++) {
			MP_DIGIT(c, i) = cdigits[i];
		}
		s_mp_free(adigits, useda * sizeof (uint64_t));
		s_mp_free(bdigits, useda * sizeof (uint64_t));
		s_mp_free(cdigits, 2 * useda  * sizeof (uint64_t));
#endif
		res = MP_OKAY;
		goto DONE;
	}

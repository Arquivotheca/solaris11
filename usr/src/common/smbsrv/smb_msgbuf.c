 ...
 * smb_msgbuf_seek
 *
 * Seek the smb_msgguf buffer to the specified byte offset relative to
 * the base.
 */
boolean_t
smb_msgbuf_seek(smb_msgbuf_t *mb, size_t offset)
{
	if (offset > mb->max)
		return (B_FALSE);

	mb->scan = mb->base + offset;
	return (B_TRUE);
}

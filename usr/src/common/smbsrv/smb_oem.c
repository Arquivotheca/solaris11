...
static rwlock_t oemcpg_lock;
#define	OEMCPG_RWINIT()	(void) rwlock_init(&oemcpg_lock, USYNC_THREAD, NULL)
#define	OEMCPG_RDLOCK()	(void) rw_rdlock(&oemcpg_lock)
#define	OEMCPG_WRLOCK()	(void) rw_wrlock(&oemcpg_lock)
#define	OEMCPG_UNLOCK()	(void) rw_unlock(&oemcpg_lock)

#else

static krwlock_t oemcpg_lock;
#define	OEMCPG_RWINIT()	rw_init(&oemcpg_lock, NULL, RW_DEFAULT, NULL)
#define	OEMCPG_RDLOCK()	rw_enter(&oemcpg_lock, RW_READER)
#define	OEMCPG_WRLOCK()	rw_enter(&oemcpg_lock, RW_WRITER)
#define	OEMCPG_UNLOCK()	rw_exit(&oemcpg_lock);
 ...
 *
 * Must be called with oemcpg lock held for reading
 */
static oempage_t *
...
/*
 * initialize the oem codepage rwlock
 */
int
smb_oemcpg_init(void)
{
	OEMCPG_RWINIT();
	return (0);
}
...
/*
 * Must be called with oem codepage lock held for writing.
 */
static void
...
/*
 * Cleanup all oem pages in the oem table.
 */
void
smb_oemcpg_fini(void)
{
	uint32_t	cpid;

	OEMCPG_WRLOCK();

	for (cpid = 0; cpid < MAX_OEMPAGES; cpid++) {
		if (oemcpg_table[cpid].valid) {
			MEM_FREE("oem", oemcpg_table[cpid].oempage.value);
			oemcpg_table[cpid].oempage.value = NULL;
			MEM_FREE("oem", oemcpg_table[cpid].ucspage.value);
			oemcpg_table[cpid].ucspage.value = NULL;
			oemcpg_table[cpid].valid = B_FALSE;
		}
	}

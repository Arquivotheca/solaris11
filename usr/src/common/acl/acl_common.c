...
void
acl_trivial_access_masks(boolean_t dir, mode_t mode, uint32_t *allow0,
    uint32_t *deny1, uint32_t *deny2, uint32_t *owner, uint32_t *group,
    uint32_t *everyone)
{
	...
	*group = ACE_READ_ACL|ACE_READ_ATTRIBUTES| ACE_READ_NAMED_ATTRS|
	...
	if (mode & S_IRGRP)
		*group |= ACE_READ_DATA;
	if (mode & S_IWGRP) {
		*group |= ACE_WRITE_DATA|ACE_APPEND_DATA|ACE_WRITE_ATTRIBUTES|
		    ACE_WRITE_NAMED_ATTRS;
	}
	if (mode & S_IXGRP) {
		*group |= ACE_EXECUTE;
		if (dir && (mode & S_IWGRP))
			*group |= ACE_DELETE_CHILD;
	}
	...
	*everyone = ACE_READ_ACL|ACE_READ_ATTRIBUTES| ACE_READ_NAMED_ATTRS|
	...
	if (mode & S_IXOTH) {
		*everyone |= ACE_EXECUTE;
		if (dir && (mode & S_IWOTH))
			*everyone |= ACE_DELETE_CHILD;
	}
}

int
acl_trivial_create(boolean_t dir, mode_t mode, ace_t **acl, int *count)
{
	...
	*count = 3;
	acl_trivial_access_masks(dir, mode, &allow0, &deny1,
	    &deny2, &owner, &group, &everyone);
    ...
    uint16_t *, uint16_t *, uint32_t *))
{
	...
	while (cookie = walk(acep, cookie, aclcnt, &flags, &type, &mask)) {
		...
		/*
		 * The delete_child permission is allowed when write and
		 * execute are set.
		 */
		if ((mask & ACE_DELETE_CHILD) &&
		    (((mask & ACE_WRITE_DATA) == 0) ||
		    ((mask & ACE_EXECUTE) == 0)))
			return (1);

		/*
		 * The delete permission is never set by default.
		 */
		if (mask & ACE_DELETE)
			return (1);
		...
		/*
		 * write_attributes/write_xattr are considered trivial if the
		 * write permission is allowed.  The exception is that the
		 * owner@ can have write_attributes and write_xattr
		 * even in the case that write permission is not allowed.
		 * (e.g. chmod 444 will create an owner@ ACE that does not
		 * have write permissions, but does have write_attributes
		 * write_xattr permissions).
		 */
		if (type == ACE_ACCESS_ALLOWED_ACE_TYPE &&
		    !(flags & ACE_OWNER) &&
		    (mask & (ACE_WRITE_ATTRIBUTES|ACE_WRITE_NAMED_ATTRS)) &&

...
#ifdef	DEBUG
/*
 * Support corruption the devid for testing.
 *
 * NOTE: This mechanism is similar to the devid_register_corrupt mechanism
 * except it occurs early enough to effect a guid derived from a devid too.
 */
static int	devid_scsi_encode_corrupt = 0;
static int	devid_scsi_encode_corrupt_hint = 0;
#endif	/* DEBUG */
	...
	ddi_devid_t	*ret_devid)
{
	...
	if (devid_scsi_encode_corrupt)
		i_devid->did_id[0] += devid_scsi_encode_corrupt;
	if (devid_scsi_encode_corrupt_hint)
		i_devid->did_driver[0] += devid_scsi_encode_corrupt_hint;
#endif	/* DEBUG */

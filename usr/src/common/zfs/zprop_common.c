...
zprop_register_impl(int prop, const char *name, zprop_type_t type,
    ...
    boolean_t rightalign, boolean_t visible, boolean_t size,
    ...
    const char *colname)
{
	zprop_register_impl(prop, name, PROP_TYPE_NUMBER, def, NULL, attr,
	    objset_types, values, colname, B_TRUE, B_TRUE, B_FALSE, NULL);
}

void
zprop_register_size(int prop, const char *name, uint64_t def,
    zprop_attr_t attr, int objset_types, const char *values,
    const char *colname)
{
	zprop_register_impl(prop, name, PROP_TYPE_NUMBER, def, NULL, attr,
	    objset_types, values, colname, B_TRUE, B_TRUE, B_TRUE, NULL);
}
    ...
    const char *colname, const zprop_index_t *idx_tbl)
{
	zprop_register_impl(prop, name, PROP_TYPE_INDEX, def, NULL, attr,
	    objset_types, values, colname, B_TRUE, B_TRUE, B_FALSE, idx_tbl);
}
    ...
    zprop_attr_t attr, int objset_types, const char *colname)
{
	zprop_register_impl(prop, name, type, 0, NULL, attr,
	    objset_types, NULL, colname, B_FALSE, B_FALSE, B_FALSE, NULL);
}


void
zprop_register_hidden_index(int prop, const char *name, zprop_attr_t attr,
    int objset_types, const char *colname, const zprop_index_t *idx_tbl)
{
	zprop_register_impl(prop, name, PROP_TYPE_INDEX, 0, NULL, attr,
	    objset_types, NULL, colname, B_FALSE, B_FALSE, B_FALSE, idx_tbl);
}
...
zprop_width(int prop, boolean_t *fixed, zfs_type_t type)
{
	...
	*fixed = pd->pd_size;
	...
	case PROP_TYPE_NUMBER:
		 ...
		 * ("20.4M", for example). Non-"size" properties are numbers
		 * with no fixed max length; libzfs will need to work it out
		 * dynamically.
		 */
		if (*fixed && ret < 5)
			ret = 5;
...
boolean_t
zprop_is_size(int prop, zfs_type_t type)
{
	ASSERT(prop < zprop_get_numprops(type));
	return ((zprop_get_proptable(type))[prop].pd_size);
}

const char *
zprop_to_name(int prop, zfs_type_t type)
{
	ASSERT(prop < zprop_get_numprops(type));
	return (type == ZFS_TYPE_POOL ?
	    zpool_prop_to_name(prop) : zfs_prop_to_name(prop));
}

zprop_type_t
zprop_get_type(int prop, zfs_type_t type)
{
	ASSERT(prop < zprop_get_numprops(type));
	return (type == ZFS_TYPE_POOL ?
	    zpool_prop_get_type(prop) : zfs_prop_get_type(prop));
}

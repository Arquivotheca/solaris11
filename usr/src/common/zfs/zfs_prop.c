...
const char **
zfs_prop_get_quota_prefixes(void)
{
	return (zfs_userquota_prop_prefixes);
}
...
zfs_prop_init(void)
{
	...
	zprop_register_index(ZFS_PROP_ZONED, "zoned", 0, PROP_INHERIT,
	    ZFS_TYPE_FILESYSTEM | ZFS_TYPE_VOLUME, "on | off", "ZONED",
	    boolean_table);
	...
	zprop_register_index(ZFS_PROP_NBMAND, "nbmand", 0, PROP_INHERIT,
	    ...
	    boolean_table);
	zprop_register_index(ZFS_PROP_RSTCHOWN, "rstchown", 1, PROP_INHERIT,
	    ZFS_TYPE_FILESYSTEM | ZFS_TYPE_SNAPSHOT, "on | off", "RSTCHOWN",
	    boolean_table);

	/* default index properties */
	zprop_register_index(ZFS_PROP_VERSION, "version", 0, PROP_DEFAULT,
	    ZFS_TYPE_FILESYSTEM | ZFS_TYPE_SNAPSHOT,
	    "<version> | current", "VERSION", version_table);
	...
	zprop_register_index(ZFS_PROP_CASE, "casesensitivity",
	    ZFS_CASE_MIXED, PROP_ONETIME, ZFS_TYPE_FILESYSTEM |
	    ZFS_TYPE_SNAPSHOT,
	...
	zprop_register_string(ZFS_PROP_SHARENFS, "sharenfs", "off",
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "on | off", "SHARENFS");
	zprop_register_string(ZFS_PROP_TYPE, "type", NULL, PROP_READONLY,
	    ZFS_TYPE_DATASET, "filesystem | volume | snapshot", "TYPE");
	zprop_register_string(ZFS_PROP_SHARESMB, "sharesmb", "off",
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "on | off", "SHARESMB");
	zprop_register_string(ZFS_PROP_MLSLABEL, "mlslabel",
	    ZFS_MLSLABEL_NONE, PROP_DEFAULT, ZFS_TYPE_FILESYSTEM,
	    "<sensitivity label>", "MLSLABEL");
	zprop_register_string(ZFS_PROP_KEYSOURCE, "keysource", "none",
	    PROP_INHERIT, ZFS_TYPE_DATASET,
	    "raw | hex | passphrase,"
	    "prompt | file://<path> | pkcs11: | https://<path>", "KEYSOURCE");
	zprop_register_string(ZFS_PROP_SHADOW, "shadow", "none", PROP_DEFAULT,
	    ZFS_TYPE_FILESYSTEM, "<uri> | none", "SHADOW");
	...
	zprop_register_size(ZFS_PROP_VOLBLOCKSIZE, "volblocksize",
	    ZVOL_DEFAULT_BLOCKSIZE, PROP_ONETIME,
	    ZFS_TYPE_VOLUME, "512 to 1MB, power of 2",	"VOLBLOCK");
	...
	zprop_register_size(ZFS_PROP_QUOTA, "quota", 0, PROP_DEFAULT,
	    ZFS_TYPE_FILESYSTEM, "<size> | none", "QUOTA");
	zprop_register_size(ZFS_PROP_RESERVATION, "reservation", 0,
	    PROP_DEFAULT, ZFS_TYPE_FILESYSTEM | ZFS_TYPE_VOLUME,
	    "<size> | none", "RESERV");
	zprop_register_size(ZFS_PROP_VOLSIZE, "volsize", 0, PROP_DEFAULT,
	    ZFS_TYPE_VOLUME, "<size>", "VOLSIZE");
	zprop_register_size(ZFS_PROP_REFQUOTA, "refquota", 0, PROP_DEFAULT,
	    ZFS_TYPE_FILESYSTEM, "<size> | none", "REFQUOTA");
	zprop_register_size(ZFS_PROP_REFRESERVATION, "refreservation", 0,
	    PROP_DEFAULT, ZFS_TYPE_FILESYSTEM | ZFS_TYPE_VOLUME,
	    "<size> | none", "REFRESERV");

	/* inherit number properties */
	zprop_register_size(ZFS_PROP_RECORDSIZE, "recordsize",
	    SPA_128KBLOCKSIZE, PROP_INHERIT,
	    ZFS_TYPE_FILESYSTEM, "512 to 1MB, power of 2", "RECSIZE");
	...
	zprop_register_hidden(ZFS_PROP_SHARE2, "$share2", PROP_TYPE_NUMBER,
	    PROP_DEFAULT, ZFS_TYPE_DATASET, "SHARE2");
	/*
	 * $hasrecvd is inheritable so it continues to work on datasets received
	 * when an earlier bug interfered with setting the property recursively.
	 * PROP_ONETIME conveniently makes $hasrecvd both inheritable and
	 * readonly. We want it readonly to prevent the property from being sent
	 * and received (not to mention being set on the command line). From the
	 * user perspective, the property is readonly, even though it remains
	 * settable internally because dsl_prop_set_sync() ignores the readonly
	 * attribute.
	 */
	zprop_register_impl(ZFS_PROP_HASRECVD, "$hasrecvd", PROP_TYPE_NUMBER,
	    -1ULL, NULL, PROP_ONETIME, ZFS_TYPE_DATASET, NULL, "HASRECVD",
	...
	zprop_register_hidden_index(ZFS_PROP_TMPMOUNTED, "tmpmounted",
	    PROP_READONLY, ZFS_TYPE_FILESYSTEM, "TMPMOUNTED", boolean_table);

	/* oddball properties */
	zprop_register_impl(ZFS_PROP_CREATION, "creation", PROP_TYPE_NUMBER, 0,
	    NULL, PROP_READONLY, ZFS_TYPE_DATASET,
	    "<date>", "CREATION", B_FALSE, B_TRUE, B_FALSE, NULL);
	zprop_register_impl(ZFS_PROP_REKEYDATE, "rekeydate",
	    PROP_TYPE_NUMBER, 0, NULL, PROP_READONLY,
	    ZFS_TYPE_FILESYSTEM | ZFS_TYPE_VOLUME,
	    "<date>", "REKEYDATE", B_FALSE, B_TRUE, B_FALSE, NULL);
}
...
boolean_t
zfs_prop_readonly_userquota(const char *name)
{
	zfs_userquota_prop_t prop;
	...
	return (prop == ZFS_PROP_USERUSED || prop == ZFS_PROP_GROUPUSED);
}

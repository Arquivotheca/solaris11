...
void
zpool_get_load_policy(nvlist_t *nvl, zpool_load_policy_t *zrpp)
{
	nvlist_t *policy;
	nvpair_t *elem;
	char *nm;
	...
	/* Defaults */
	zrpp->zrp_request = ZPOOL_NORMAL_LOAD;
	zrpp->zrp_maxmeta = 0;
	...
	while ((elem = nvlist_next_nvpair(nvl, elem)) != NULL) {
		...
		if (strcmp(nm, ZPOOL_LOAD_POLICY) == 0) {
			if (nvpair_value_nvlist(elem, &policy) == 0)
				zpool_get_load_policy(policy, zrpp);
			return;
		} else if (strcmp(nm, ZPOOL_LOAD_REWIND) == 0) {
			if (nvpair_value_uint32(elem, &zrpp->zrp_request) == 0)
				if (zrpp->zrp_request & ~ZPOOL_LOAD_POLICIES)
					zrpp->zrp_request = ZPOOL_NORMAL_LOAD;
		} else if (strcmp(nm, ZPOOL_LOAD_REWIND_TXG) == 0) {
		...
		} else if (strcmp(nm, ZPOOL_LOAD_RETRY) == 0) {
			zrpp->zrp_request = ZPOOL_RETRY_LOAD;
		}
	}
 ...
 * $SRC/uts/common/sys/fs/zfs.h: enum history_internal_events
 */

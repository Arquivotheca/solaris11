 ...
 * ----------------------
 * The structure of a scope is shown in this picture.  DEC means decoration
 * and PROP means property.
 *
 *	 _______
 *	| SCOPE |
 *	|_______|
 ...
 *	./   |.		  /. |.	 \ .
 *     ./    |.		 /.  |.	  \ .
 *  ______   _____  ____/   _____  \__________
 * | PROP | | DEC || PG |  | DEC | | SNAPSHOT |
 * |______| |_____||____|  |_____| |__________|
 *    |.	  ./   |.		\ .
 *    |.	 ./    |.		 \ .
 *   _____    ______   _____		  \___________
 *  | DEC |  | PROP | | DEC |		  | SNAPLEVEL |
 *  |_____|  |______| |_____|		  |___________|
 *		|.			     /.
 *		|.			    /.
 *	       _____		       ____/
 ...
 *	Deletes (masks) the entity represented by entity_id.
 *
 * ENTITY_UNDELETE(entity_id, changeid) -> result
 * 	Undeletes a masked entity represented by entity_id.
 *
 * ENTITY_REMOVE(entity_id, changeid) -> result
 *      Deletes the entity represented by entity_id without regard for masking
 *
 * ENTITY_DELCUST(entity_id, changeid) -> result
 *      Deletes admin customizations on entity and masks site-profile
 *      customizations on entity
 *
 * BUNDLE_REMOVE(svc_entity_id, changeid, bname) -> result
 *      Deletes the bundle and its associated entites from the repository
 *
 ...
 *
 * ENTITY_IS_MASKED(entity_id) -> boolean
 *	Returns 1 if the entity is masked and 0 if it is not.  Can only be
 *	applied to service, instance, property group and property
 *	entities.
 *
 ...
 *	in entity_id.  Note that if an error occurs, you can lose
 ...
 *
 * BACKUP(name) -> result
 *	Backs up the persistent repository with a particular name.
 *
 ...
 *	default location (/etc/svc) to the tmpfs location (_PATH_SYSVOL).
 *	When it is set to 'perm', the switch is reversed.
 *
 * DECORATION_GET_BUNDLE(entity_id) -> result, bundle path
 *	entity_id must be associated with a decoration entity.  The request
 *	will return the path of the bundle file associated with this
 *	decoration.  Note that the path will be empty if the decoration was
 *	not created by a bundle.
 */
...
#define	REPOSITORY_DOOR_NAME	_PATH_SYSVOL "/repository_door"
 ...
 * doubt, bump it.
 */
#define	REPOSITORY_DOOR_VERSION			(22 + REPOSITORY_DOOR_BASEVER)

/*
 * flags for rdr_flags
 */
#define	REPOSITORY_DOOR_FLAG_DEBUG		0x00000001	/* rdr_debug */
#define	REPOSITORY_DOOR_FLAG_SHOW_MASKED	0x00000010	/* mask aware */

#define	REPOSITORY_DOOR_FLAG_ALL		0x00000011	/* all flags */
...
enum rep_protocol_requestid {
	...
	REP_PROTOCOL_ENTITY_REMOVE,
	REP_PROTOCOL_ENTITY_DELCUST,
	REP_PROTOCOL_BUNDLE_REMOVE,
	...
	REP_PROTOCOL_PROP_BUNDLE_REMOVE,
	...
	REP_PROTOCOL_UNSET_AUDIT_ANNOTATION,

	REP_PROTOCOL_SET_TX_DECORATION,

	REP_PROTOCOL_SWITCH,

	REP_PROTOCOL_DECORATION_GET_LAYER,
	REP_PROTOCOL_DECORATION_GET_TYPE,
	REP_PROTOCOL_DECORATION_GET_VALUE,
	REP_PROTOCOL_DECORATION_GET_BUNDLE,

	REP_PROTOCOL_ENTITY_INCONFLICT,
	REP_PROTOCOL_ENTITY_IS_MASKED,
...
/*
 * This enumeration of decoration layers mirrors scf_decoration_layer_t in
 * libscf. decoration_layer_info_t array in rc_node.c and scf_layer_info
 * array in lowlevel.c also reference this enumeration. In short, adding
 * new element in this enumeration requires corresponding changes in those
 * components.
 *
 * REP_PROTOCOL_DEC_TOP is not an actual layer but it points to the
 * highest available layer.
 */
typedef enum rep_protocol_decoration_layer {
	REP_PROTOCOL_DEC_INVALID	= 0,
	REP_PROTOCOL_DEC_MANIFEST	= 10000,
	REP_PROTOCOL_DEC_SYSTEM_PROFILE	= 20000,
	REP_PROTOCOL_DEC_SITE_PROFILE	= 30000,
	REP_PROTOCOL_DEC_ADMIN		= 40000,
	REP_PROTOCOL_DEC_TOP		= 500000
...
#define	REP_PROTOCOL_DECORATION_TOP		"top_layer"
#define	REP_PROTOCOL_DECORATION_ADMIN		"administrative_layer"
#define	REP_PROTOCOL_DECORATION_SITE_PROFILE	"site_profile_layer"
#define	REP_PROTOCOL_DECORATION_SYSTEM_PROFILE	"system_profile_layer"
#define	REP_PROTOCOL_DECORATION_MANIFEST	"manifest_layer"
...
#define	RP_ITER_START_CONFLICT	0x00000004	/* conflicts only */
#define	RP_ITER_START_FILT_MASK	0x00000007
#define	RP_ITER_START_COMPOSED	0x00000008	/* composed */
...
#define	RP_ENTITY_NAME_DECORATION_BUNDLE	7
...
struct rep_protocol_bundle_remove {
	enum rep_protocol_requestid rpr_request; /* REMOVE BUNDLE */
	uint32_t rpr_entityid;
	uint32_t rpr_changeid;
	char	rpr_bname[MAXPATHLEN];
};
...
struct rep_protocol_transaction_commit {
	enum rep_protocol_requestid rpr_request; /* TX_COMMIT/BUNDLE_REMOVE */
	char rpr_pg_type[REP_PROTOCOL_NAME_LEN];
	uint32_t rpr_pg_flags;
	uint32_t rpr_pg_action;
	uint32_t rpr_entityid;
...
enum rep_protocol_transaction_action {
	...
	REP_PROTOCOL_TX_ENTRY_REMOVE,	/* remove prop (no values, no mute) */
	REP_PROTOCOL_TX_ENTRY_UNDELETE,	/* undelete property (no values) */
	REP_PROTOCOL_TX_ENTRY_DELCUST	/* remove admin layer (no values) */
};
...
struct rep_protocol_decoration {
	enum rep_protocol_requestid rpr_request;	/* SET_DECORATION */
	char rpr_file[MAXPATHLEN];
	rep_protocol_decoration_layer_t rpr_layer_id;
};
...
struct rep_protocol_masked_request {
	enum rep_protocol_requestid rpr_request;	/* ENTITY_IS_MASKED */
	uint32_t rpr_entityid;
};
...
struct rep_protocol_path_response {
	rep_protocol_responseid_t rpr_response;
	char rpr_path[MAXPATHLEN];
};

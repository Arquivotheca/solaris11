...
/*
 * NOTE: two MDB modules read the tick_per_msec and msec_per_tick
 * variables via mdb_readvar().  Those modules are: nca and kssl.  If
 * you want to remove or modify the semantics of either of these two
 * variables then you must also update those modules.
 */
 ...
 * of data and getting the kernel stuck in kmem_alloc.  Limit to 1MB of
 * data plus 4K in door results overhead.
 */
size_t door_max_upcall_reply = (1024 * 1024) + 4096;

...
/* Solaris Kerberos changes in here to support kernel and user space mechs */

#include "k5-int.h"
/* #include <stdio.h> */ /* Solaris Kerberos: not needed */
...
/*
 * Solaris Kerberos
 * Care should be taken when modifing the k5_unparse_name() as
 * krb5_unparse_name_no_ctx will call this function with context == NULL (and
 * flags == 0).  With flags == 0 the only functions called which "use" the
 * context are krb5_princ_realm, krb5_princ_size and krb5_princ_component which
 * don't actually use the context at all.
 */
/* ARGSUSED */
...
{
    /* Solaris Kerberos lint - cp/length not used */
    /* char *cp; */
    char *q;
    int i;
    /* int     length; */
...
#ifndef _KERNEL
    /* Solaris Kerberos princ in kernel will always have realm */
    ...
    if (size) {
        if (*name && (*size < totalsize)) {
            /* Solaris Kerberos works for both kernel&user */
...
/*
 * Solaris Kerberos
 * Another variant of unparse_name which doesn't require a krb5_context.
 */
krb5_error_code
krb5_unparse_name_no_ctx(krb5_const_principal principal, register char **name)
{
    if (name != NULL) /* name == NULL will return error from _ext */
        *name = NULL;

    return k5_unparse_name(NULL, principal, 0, name, NULL);
}

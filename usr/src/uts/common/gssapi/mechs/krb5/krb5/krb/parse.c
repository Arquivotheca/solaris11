...
/*
 * Solaris Kerberos: there are mods in here to support use in kernel as well as
 * user space.
 */
              ...
              int flags, krb5_principal *nprincipal)
{
    ...
    *nprincipal = NULL;
    ...
    if (principal->data == NULL) {
        FREE(principal, sizeof(krb5_principal_data));
        return ENOMEM;
    }
    ...
    if (tmpdata == 0) {
        FREE(principal->data, principal->length);
        FREE(principal, sizeof(krb5_principal_data));
#ifndef _KERNEL
        free(default_realm);
#endif
        ...
        if (tmpdata2 == NULL) {
            for (i--; i >= 0; i--)
                FREE(krb5_princ_component(context, principal, i)->data,
                     krb5_princ_component(context, principal, i)->length + 1);
            FREE(krb5_princ_realm(context, principal)->data,
                 krb5_princ_realm(context, principal)->length + 1);
            FREE(principal->data, principal->length);
            FREE(principal, sizeof(krb5_principal_data));
#ifndef _KERNEL
            free(default_realm);

		     ...
		     const krb5_data *input, krb5_checksum *cksum)
{
    ...
    if (cksumtype == 0) {
        ret = krb5int_c_mandatory_cksumtype(context, key->enctype,
                                            &cksumtype);
        if (ret != 0)
            return ret;
    }
	...
	cksumlen = krb5int_cksumtypes_list[i].keyhash->hashsize;
    else
	cksumlen = krb5int_cksumtypes_list[i].hash->hashsize;

#ifdef _KERNEL
    context->kef_cksum_mt = krb5int_cksumtypes_list[i].kef_cksum_mt;
#endif
	...
	if (krb5int_cksumtypes_list[i].keyed_etype) {
	    for (e1=0; e1<krb5_enctypes_length; e1++) 
		if (krb5_enctypes_list[e1].etype ==
		    krb5int_cksumtypes_list[i].keyed_etype)
		    break;
	...
	ret = (*(krb5int_cksumtypes_list[i].keyhash->hash))(context, key,
						usage, 0, input, &data);
    } else if (krb5int_cksumtypes_list[i].flags & KRB5_CKSUMFLAG_DERIVE) {
#ifdef _KERNEL
	...
	ret = krb5_dk_make_checksum(context,
				krb5int_cksumtypes_list[i].hash,
	    ...
	    context->kef_hash_mt = context->kef_cksum_mt;
#endif /* _KERNEL */
	    ret = (*(krb5int_cksumtypes_list[i].hash->hash))(context, 1,
							input, &data);
    }
	...
	if (krb5int_cksumtypes_list[i].trunc_size) {
	    krb5_octet *trunc;
             ...
             */
            cksum->length = krb5int_cksumtypes_list[i].trunc_size;
...
krb5_k_make_checksum(krb5_context context, krb5_cksumtype cksumtype,
                     ...
                     krb5_checksum *cksum)
{
  return(krb5_c_make_checksum(context, cksumtype, &key->keyblock,
			    usage, input, cksum));
}

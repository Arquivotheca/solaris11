...
/* Solaris Kerberos support for kernel throughout this file */
#ifdef _KERNEL
...
#ifndef _KERNEL
        krb5_ser_context_init, krb5_ser_ccache_init, krb5_ser_rcache_init,
    ...
    gss_ctx_id_t        *context_handle;
{
    ...
    kret = KGSS_INIT_CONTEXT(&context); /* Solaris Kerberos */
    if (kret) {
        *minor_status = kret;
    ...
    *minor_status = 0;
    ...
    /*
     * Solaris Kerberos: Previous versions of MIT(1.2ish)/Solaris did not
     * serialize the k5_context but MIT 1.5 does.  But we don't need all the
     * userspace junk in the kernel so we continue to not serialize it. So we
     * keep this context live here (see it's use in kg_ctx_internalize) and it
     * will get freed by delete_sec_context.
     */
    /* krb5_free_context(context); */
...
#ifdef _KERNEL
        /* Solaris Kerberos gssd_ctx_verifier parameter needed */
        (void)krb5_gss_delete_sec_context(minor_status,
                                          (gss_ctx_id_t *) &ctx, NULL, 0);
#else
        (void)krb5_gss_delete_sec_context(minor_status,

...
#include <memory.h>
#endif
/* Solaris Kerberos support for kernel throughout */
#ifndef _KERNEL
#include <assert.h>
...
kg_unseal_v1(context, minor_status, ctx, ptr, bodysize, message_buffer,
    ...
    int *conf_state;
    gss_qop_t *qop_state; /* Solaris Kerberos: fix type nit */
    int toktype;
{
    ...
    OM_uint32 retval;
    size_t sumlen;
    size_t blocksize; /* Solaris Kerberos */
    int tmsglen; /* Solaris Kerberos */
    ...
    *minor_status = 0;
     ...
     */
    /*
     * Solaris Kerberos: XXX need to revisit this as it's ugly to access the
     * keyblock directly.
     */
    ...
    if (toktype == KG_TOK_SEAL_MSG) {
        tmsglen = bodysize-(14+cksum_len); /* Solaris Kerberos */
        if (sealalg != 0xffff) {
...
#endif
                /* Solaris Kerberos: added context */
        ...
        if ((sealalg == 0xffff) && ctx->big_endian) {
             ...
             */
            code = krb5_c_block_size(context, ctx->enc->keyblock.enctype, &blocksize);
            if (code != 0) {
                if (sealalg != 0xffff)
                    xfree_wrap(plain, tmsglen);
                *minor_status = code;
    ...
    if (code) {
        /* Solaris Kerberos: prevent mem leak */
        if (toktype == KG_TOK_SEAL_MSG && token.value != NULL)
    ...
    /* Solaris Kerberos: moved this down for CR 5012599 */
    ...
    if ((ctx->initiate && direction != 0xff) ||
        ...
        if (toktype == KG_TOK_SEAL_MSG) {
            ...
            token.value = NULL;
            token.length = 0;
            /* Solaris Kerberos for CR 5012599 */
            /* message_buffer->value = NULL; */
            /* message_buffer->length = 0; */
        }

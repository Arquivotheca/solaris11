...
extern rfs4_database_t *rfs4_database_create(uint32_t, fsh_entry_t *);
extern void		rfs4_database_shutdown(rfs4_database_t *);
extern void		rfs4_database_destroy(rfs4_database_t *);

extern rfs4_table_t	*rfs4_table_create(rfs_inst_t *, rfs4_database_t *,
...
extern void		rfs4_fse_db_create(rfs_inst_t *, fsh_entry_t *);
extern fsh_entry_t	*rfs4_dbe_getfse(rfs4_dbe_t *);
extern rfs_inst_t	*rfs4_dbe_rip(rfs4_dbe_t *);
 ...
 * stateid4.seqid.
 *
 * NB:  The meta-cluster node ID is 12 bits in length.  It would be nice to
 * have it as one, 12-bit long bit field.  However, when the stateid was
 * formed that way, it worked fine for a 64-bit kernel, but not for a 32-bit
 * kernel.  If 32-bit kernel support is dropped, it may be worth changing
 * the structure to remove this complexity.
 */
typedef union {
	stateid4 stateid;
	struct {
		uint32_t chgseq;	/* State changes / protocol's seqid */
		uint32_t boottime:24;	/* boot time  */
		uint32_t type:4;	/* stateid_type_t as define below */
		uint32_t mcid:4;	/* meta-cluster nodeid is 12 bits */
		uint32_t clnodeid:8;	/* mcid cont or cluster server nodeid */
		uint32_t ident:24;	/* 2^24-1 openowner x fhs */
		pid_t	 pid;		/* pid of corresponding lock owner */
	} bits;
} stateid_t;
/*
 * Note that the way the type field above is defined, this enum must
 * not have more than 16 members.
 */
...
/*
 * A grace period instance. We can associate sets of clients - via a pointer in
 ...
 */
typedef struct rfs4_grace {
	int			rg_dss_npaths;
	krwlock_t		rg_rwlock;
	krwlock_t		rg_oldstate_lock;
	time_t			rg_start_time;
	time_t			rg_grace_period;
	rfs4_oldstate_t		rg_oldstate;
	struct rfs4_dss_path	**rg_dss_paths;
	struct rfs4_grace	*rg_next;
	struct rfs4_grace	*rg_prev;
} rfs4_grace_t;
...
typedef struct rfs4_dss_path {
	struct rfs4_dss_path	*ds_next; /* for insque/remque */
	struct rfs4_dss_path	*ds_prev; /* for insque/remque */
	char			*ds_path;
	rfs4_grace_t		*ds_sip;
	unsigned		ds_index; /* offset in rfs4_grace_t's array */
} rfs4_dss_path_t;
...
typedef struct rfs4_lemo_entry {
	list_node_t		rle_ln;
	fsid_t			rle_fsid;
	time_t			rle_time_seen;
} rfs4_lemo_entry_t;
 ...
 * sw - used to serialize access to the open_seqid/reply handling
 ...
 * sw - used to serialize update seqid/reply/stateid handling
 * node - node for state struct list of lo_states
 * reply - last reply sent in relation to this lockowner/state
 *
 * NOTE: the embedded pid in 'rls_lockid' can not be used to search
 * for the lock_owner of this lock if this lock state was migrated
 * from another system as part of a filesystem migration. This pid
 * and the rl_pid do not match.
 * See comment in rfs4_hydrate_mig_lo_state() for additional details.
 */
...
extern int		rfs4_in_grace(rfs4_client_t *, struct compound_state *);
extern void		rfs4_fse_grace_start(rfs_inst_t *, fsh_entry_t *);
extern void		rfs4_grace_create(rfs_inst_t *, int, int, char **);
extern void		rfs4_grace_destroy_all(rfs_inst_t *);
extern void		rfs4_clnt_grace_assign(rfs4_client_t *, rfs4_grace_t *);
extern rfs4_grace_t	*rfs4_clnt_grace(rfs4_client_t *);
extern int		rfs4_clnt_in_grace(rfs4_client_t *);
extern int		rfs4_grace_in(rfs4_grace_t *);
extern int		rfs4_grace_new(rfs4_grace_t *);
extern void		rfs4_grace_start(rfs4_grace_t *);
extern void		rfs4_inst_grace_start(rfs_inst_t *);
extern void		rfs4_inst_grace_reset(rfs_inst_t *);
extern void		rfs4_ss_oldstate(rfs4_oldstate_t *, char *, char *);
extern void		rfs4_dss_readstate(rfs_inst_t *, int, char **);
...
extern void rfs4_disable_delegation(rfs_inst_t *);
extern void rfs4_enable_delegation(rfs_inst_t *);
 ...
 * above
 */
extern	void		rfs4_clean_state_exi(struct exportinfo *);
extern	void		rfs4_clean_state_fse(fsh_entry_t *);
extern	void		rfs4_free_reply(nfs_resop4 *);
extern	void		rfs4_copy_reply(nfs_resop4 *, nfs_resop4 *);

/* rfs4_client_t handling */
extern  rfs4_client_t   *rfs4_findclient_db(rfs4_inst_t *, rfs4_index_t *,
					void *, bool_t *, void *, bool_t);
extern	rfs4_client_t	*rfs4_findclient(rfs_inst_t *, nfs_client_id4 *,
					bool_t *, rfs4_client_t *);
extern	rfs4_client_t	*rfs4_findclient_by_id(rfs_inst_t *, clientid4, bool_t);
...
extern	bool_t		rfs4_lease_moved(rfs4_client_t *,
			    struct compound_state *);
extern	rfs4_lemo_entry_t *rfs4_lemo_insert(rfs4_client_t *, fsid_t);
extern	void		rfs4_lemo_remove(rfs4_client_t *, rfs4_lemo_entry_t *);
extern	nfsstat4	rfs4_check_clientid(rfs_inst_t *, clientid4 *, int);

/* rfs4_clntip_t handling */
extern	rfs4_clntip_t	*rfs4_find_clntip(rfs_inst_t *, struct sockaddr *,
    bool_t *);
extern	void		rfs4_invalidate_clntip(rfs_inst_t *, struct sockaddr *);

/* rfs4_openowner_t handling */
extern	rfs4_openowner_t *rfs4_findopenowner(rfs_inst_t *, open_owner4 *,
    bool_t *, seqid4);
...
extern	rfs4_lockowner_t *rfs4_findlockowner(rfs_inst_t *, lock_owner4 *,
    bool_t *);
extern	rfs4_lockowner_t *rfs4_findlockowner_by_pid(rfs_inst_t *rip, pid_t);
extern	void		rfs4_lockowner_rele(rfs4_lockowner_t *);

/* rfs4_state_t handling */
extern	rfs4_state_t	*rfs4_findstate_by_owner_file(struct compound_state *,
    rfs4_openowner_t *, rfs4_file_t *, bool_t *);
...
extern	void		rfs4_release_share_lock_state(rfs4_state_t *,
					cred_t *, bool_t);
extern	void		rfs4_close_all_state(struct compound_state *,
    rfs4_file_t *);
extern void 		rfs4_state_rele_nounlock(rfs4_state_t *sp);

/* rfs4_lo_state_t handling */
extern	rfs4_lo_state_t *rfs4_findlo_state_by_owner(struct compound_state *,
...
extern	rfs4_file_t	*rfs4_findfile(fsh_entry_t  *, vnode_t *, nfs_fh4 *,
    bool_t *);
extern	rfs4_file_t	*rfs4_findfile_withlock(fsh_entry_t *, vnode_t *,
    nfs_fh4 *, bool_t *);
...
extern	nfsstat4	rfs4_get_deleg_state(struct compound_state *,
    stateid4 *, rfs4_deleg_state_t **);
extern	nfsstat4	rfs4_get_lo_state(struct compound_state *, stateid4 *,
    rfs4_lo_state_t **, bool_t);
extern	nfsstat4	rfs4_check_stateid(int, struct compound_state *,
    vnode_t *, stateid4 *, bool_t, bool_t *, bool_t, caller_context_t *);
...
extern	rfs4_deleg_state_t *rfs4_finddeleg(struct compound_state *,
    rfs4_state_t *, bool_t *);
extern	rfs4_deleg_state_t *rfs4_finddelegstate(fsh_entry_t *, stateid_t *);
...
extern void rfs4_set_deleg_policy(rfs_inst_t *, srv_deleg_policy_t);
#ifdef DEBUG
...
/*
 * could test volatile fh in DEBUG kernels here
 */
...
typedef struct compound_state {
	...
	fsh_entry_t	*fse;		/* FS hash table entry */
	...
	bool_t 		mandlock;	/* Is mandatory locking in effect */
					/* for vp */
	bool_t		got_fs_loca;	/* compound has gotten fs_locations */
	bool_t		lease_moved;	/* state check saw LEASE_MOVED */
	...
	rfs_inst_t	*rip;		/* nfs server instance */
	clientid4	clientid;	/* short for client id from RENEW */
	bool_t		renew_seen;
} compound_state_t;
...
/*
 * The functions for getting attributes can return:
 * 0  - OK
 * >0 - Protocol error
 * -1 - Error can be ignored if rdattr_error is set
 * -2 - Error can silently be ignored
 */
#define	NFS4_ATTR_ERR_RDATTR_OKAY	-1
#define	NFS4_ATTR_ERR_SILENT_IGNORE	-2
...
extern fs_locations4 *fetch_referral(rfs_inst_t *, vnode_t *, cred_t *);
extern fs_locations4 *fetch_replicas(rfs_inst_t *, vfs_t *, cred_t *);
extern int	vfs_replica_parse(vfs_t *vfsp, nvlist_t *nvl);

extern int	stateid4_cmp(stateid4 *, stateid4 *);
extern int	netbuf_cmp(struct netbuf *, struct netbuf *);
...
extern void rfs4_attr_init(rfs_inst_t *);
...
extern void	rfs4_global_init(void);
extern void	rfs4_global_fini(void);
extern void	rfs4_srvrinit(rfs_inst_t *);
extern void	rfs4_srvrfini(rfs_inst_t *);
extern void	rfs4_state_init(rfs_inst_t *);
extern void	rfs4_state_fini(rfs_inst_t *);

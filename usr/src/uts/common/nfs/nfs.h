...
/*
 * Maximum value for the NFS nodeID, embedded in state tokens
 */
#define	NFS_MAX_NODEID	4096
/*
...
extern void rfs_srvrinit(rfs_inst_t *);
extern void rfs_srvrfini(rfs_inst_t *);
 ...
 */
struct nfs_stats {
	kstat_named_t		  *svstat_ptr[NFS_VERSMAX + 1];
	struct nfs_version_stats  nfs_stats[NFS_VERSMAX + 1];
};
...
extern void nfsstat_inc_svstat(rfs_inst_t *, int, int);
extern void nfsstat_inc_proc_cnt(rfs_inst_t *, int, int);
...
struct READDIR3resok {
	...
	cookieverf3 cookieverf;
	dirlist3 reply;
	uint_t size;	/* size in bytes of valid data in reply.entries */
	uint_t count;	/* limit on size of whole reply */
	uint_t freecount; /* size of allocated reply.entries buffer */
	cookie3 cookie;
};
...
struct READDIRPLUS3resok {
	...
	cookieverf3 cookieverf;
	dirlistplus3 reply;
	uint_t nents;		/* number of dirents in reply.entries */
	uint_t freecount;	/* size of allocated reply.entries buffer */
	entryplus3_info *infop;
	uint_t infosize;	/* size of allocated infop buffer */
};
...
extern void rfs3_global_init(void);
extern void rfs3_global_fini(void);
extern void rfs_global_init(void);
extern void rfs_global_fini(void);
...
extern int	rfs_publicfh_mclookup(rfs_inst_t *, char *, vnode_t *, cred_t *,
			vnode_t **, struct exportinfo **, struct sec_ol *);
extern int	rfs_pathname(rfs_inst_t *, char *, vnode_t **, vnode_t **,
			vnode_t *, cred_t *, int);
 ...
 */
extern bool_t rfs4_check_delegated(int, vnode_t *, bool_t);
extern bool_t rfs4_check_delegated_byrip(rfs_inst_t *, int, vnode_t *, bool_t);
...
extern void rfs4_hold_deleg_policy(rfs_inst_t *);
extern void rfs4_rele_deleg_policy(rfs_inst_t *);

	...
	val_t *bootaux)
{
	 ...
	 *
	 * We do this early as DTrace static probes call undefined
	 * references.  We have to process those relocations before
	 * calling any of them.
	 *
...
kobj_fstat(intptr_t descr, struct bootstat *buf)
{
	...
	if (_modrootloaded) {
		 ...
		 * from the contents of vattr (transferring only the ones that
		 * are obvious.
		 */

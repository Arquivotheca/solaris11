...
#define	MSG_REL_NONALIGN	"offset 0x%llx is non-aligned; expected" \
				    " alignment %u"
...
#define	REL_ERR_NONALIGN(lml, file, sym, rtype, off, align)		     \
	_kobj_printf(ops, MSG_REL_PREGEN, CONV_RELOC_TYPE((rtype))); \
	_kobj_printf(ops, MSG_REL_FILE, (file)); \
	_kobj_printf(ops, MSG_REL_SYM, ((sym) ? (sym) : MSG_STR_UNKNOWN)); \
	_kobj_printf(ops, MSG_REL_NONALIGN, EC_OFF((off)), EC_WORD((align)))
...
#define	REL_ERR_NONALIGN(lml, file, sym, rtype, off, align)	     \
	DBG_CALL(Dbg_reloc_badalign(lml, conv_reloc_type_static(M_MACH, \
	    rtype, 0), file, (sym) ? demangle(sym) : \
	    MSG_INTL(MSG_STR_UNKNOWN), (uintptr_t)off, align));

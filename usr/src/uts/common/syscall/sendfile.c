	...
	size32_t *xferred, int fildes)
{
	...
	if (reg)	/* serialize with lseek(), read(), write() */
		mutex_enter(&fp->f_rwlock);
	(void) VOP_RWLOCK(vp, V_WRITELOCK_TRUE, NULL);
...
int
sendvec_small_chunk(struct sonode *so, file_t *fp, u_offset_t *fileoff,
    struct sendfilevec *sfv, int copy_cnt, ssize_t total_size, int maxblk,
    ssize_t *count)
{
	struct vnode *vp;
	struct uio auio;
	...
	head = dmp = so_allocb_premapped(so, buf_left + extra);
	if (head == NULL)
		return (ENOMEM);
...
uint_t sendvec_small_chunk_size = 8192;
		...
		if (is_sock) {
			if (!SOCK_IS_NONSTR(so) &&
				...
				error = nl7c_sendfilev(so, &fileoff,
				    sfv, copy_cnt, &count);
			} else if (total_size < sendvec_small_chunk_size &&
			    error == 0) {
				error = sendvec_small_chunk(so, fp,

    ...
    offset_t *retoff)
{
	...
	if (reg) {
		/*
		 * This is a POSIX requirement for atomicity
		 * of calls to read[v](), write[v]() and lseek()
		 * when operating on regular files.
		 */
	...
	case SEEK_CUR:
		if (reg && off == 0) {	/* regular file, query only */
			error = 0;
			(*retoff) = curoff;
			goto out;
		}
	...
	if ((error = VOP_SEEK(vp, curoff, &newoff, NULL)) == 0) {
		/*
		 * lseek(fd, 0, SEEK_CUR) should not reset the file offset
		 * even if this is not a regular file.
		 */
		if (off != 0 || stype != SEEK_CUR)
			fp->f_offset = newoff;
		(*retoff) = newoff;
	}
out:
...
out:
	if (reg)
		mutex_exit(&fp->f_rwlock);
	releasef(fdes);
	return (error? (off_t)set_errno(error) : new_off);
}
#endif	/* _LP64 */

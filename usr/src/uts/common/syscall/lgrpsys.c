 ...
 */
void
lgrp_affinity_init(lgrp_aff_t **aff)
{
	ASSERT(aff != NULL);

	if (*aff == NULL)
		*aff = kmem_zalloc(sizeof (lgrp_aff_t), KM_SLEEP);

	(*aff)->aff = NULL;
	(*aff)->aff_inherit = LGRP_AFF_INHERIT_DEFAULT;
}
 ...
 */
void
lgrp_affinity_free(lgrp_aff_t **aff)
{
	if (aff && *aff) {
		if ((*aff)->aff) {
			kmem_free((*aff)->aff,
		...
		kmem_free(*aff, sizeof (lgrp_aff_t));
		*aff = NULL;
	}
}
    ...
    boolean_t prefer_start)
{
	...
	if (t->t_lgrp_aff->aff == NULL)
		return (NULL);

	affs = t->t_lgrp_aff->aff;
...
/*
 * Find LWP with given ID in specified process and get its affinity
 * inheritance
 */
lgrp_aff_inherit_t
lgrp_aff_inherit_get_thread(proc_t *p, id_t lwpid)
{
	...
	do {
		if (t->t_tid == lwpid || lwpid == P_ANY) {
			thread_lock(t);
			/*
			 * Check to see whether caller has permission to get
			 * affinity inheritance for LWP
			 */
			if (t->t_cid == 0 || !hasprocperm(t->t_cred, CRED())) {
				thread_unlock(t);
			...
			if (t->t_lgrp_aff->aff_inherit) {
				aff_inherit = t->t_lgrp_aff->aff_inherit;
				found = 1;
			}
	...
	if (!found)
		aff_inherit = set_errno(ESRCH);

	return (aff_inherit);
}

/*
 * Get lgroup affinity inheritance for given LWP
 */
lgrp_aff_inherit_t
lgrp_aff_inherit_get(lgrp_aff_inherit_args_t *ap)
{
	lgrp_aff_inherit_t	aff_inherit;
	 ...
	 * Copyin arguments
	 */
	if (copyin(ap, &args, sizeof (lgrp_aff_inherit_args_t)) != 0)
		return (set_errno(EFAULT));
	...
	/*
	 * Get lgroup affinity inheritance for given LWP or process
	 */
	switch (idtype) {
	case P_LWPID:
		/*
		...
		if (id != P_MYID)	/* different thread */
			aff_inherit = lgrp_aff_inherit_get_thread(p, id);
		else {			/* current thread */
			t = curthread;
			thread_lock(t);
			if (t->t_lgrp_aff->aff_inherit)
	...
	case P_PID:
		...
		mutex_enter(&p->p_lock);
		aff_inherit = lgrp_aff_inherit_get_thread(p, P_ANY);
		mutex_exit(&p->p_lock);
		mutex_exit(&pidlock);
		break;

	default:
		aff_inherit = set_errno(EINVAL);
		break;
	}
...
/*
 * Set thread's affinity inheritance
 */
int
lgrp_aff_inherit_set_thread(kthread_t *t, lgrp_aff_inherit_t inherit_flags)
{
	ASSERT(t != NULL);
	ASSERT(MUTEX_HELD(&ttoproc(t)->p_lock));
	...
	/*
	 * Check to see whether caller has permission to set affinity
	 * inheritance for thread
	 */
	if (t->t_cid == 0 || !hasprocperm(t->t_cred, CRED())) {
		thread_unlock(t);
		return (set_errno(EACCES));
	...
	if (inherit_flags < LGRP_AFF_INHERIT_DEFAULT ||
	    inherit_flags > LGRP_AFF_INHERIT_FUTURE) {
		thread_unlock(t);
		return (set_errno(EINVAL));
	}

	t->t_lgrp_aff->aff_inherit = inherit_flags;
...
/*
 * Set LWP's or process' affinity inheritance
 *
 * When setting affinities, pidlock, process p_lock, and thread_lock()
 * need to be held in that order to protect target thread's pset, process,
 * process contents, and thread contents.
 */
int
lgrp_aff_inherit_set(lgrp_aff_inherit_args_t *ap)
{
	lgrp_aff_inherit_args_t	args;
	...
	idtype = args.idtype;
	id = args.id;
	inherit_flags = args.aff_inherit;
	...
	/*
	 * Set given LWP's or process' affinity inheritance
	 */
	switch (idtype) {
	case P_LWPID:
		p = curproc;
		...
		mutex_enter(&p->p_lock);
		if (id == P_MYID) {		/* current thread */
			retval = lgrp_aff_inherit_set_thread(curthread,
			    inherit_flags);
		} else if (p->p_tlist == NULL) {
			retval = set_errno(ESRCH);
		} else {			/* other thread */
			...
			do {
				if (t->t_tid == id) {
					retval = lgrp_aff_inherit_set_thread(t,
					    inherit_flags);
					found = 1;
					break;
				}
	...
	case P_PID:
		...
		/*
		 * Set lgroup affinity inheritance for threads in
		 * process
		 */
		t = p->p_tlist;
		if (t != NULL) {
			do {
				/*
				 * Set lgroup affinity inheritance
				 * for thread
				 */
				err = lgrp_aff_inherit_set_thread(t,
				    inherit_flags);
...
lgrpsys(int subcode, long ia, void *ap)
{
	...
	case LGRP_SYS_AFF_INHERIT_GET:
		return (lgrp_aff_inherit_get((lgrp_aff_inherit_args_t *)ap));

	case LGRP_SYS_AFF_INHERIT_SET:
		return (lgrp_aff_inherit_set((lgrp_aff_inherit_args_t *)ap));

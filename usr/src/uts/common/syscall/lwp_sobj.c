...
top:
	...
	/*
	 * We have added a new entry for addr in the lwpchan cache.
	 * Now mark the segment that contains addr with the S_LWPCHAN
	 * flag to enable as_unmap() to call lwpchan_delete_mapping()
	 * when appropriate (not on every munmap()).
	 */
	as_setlwpchan(as, addr);
	return (1);
}

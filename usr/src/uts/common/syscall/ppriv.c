    ...
    int itype)
{
	...
	case PRIVSYS_KLPD_REG:
		if (bufsize < sizeof (priv_set_t))
			return (set_errno(ENOMEM));
		return (klpd_reg((int)op, (idtype_t)itype, (id_t)type, buf));
	case PRIVSYS_KLPD_UNREG:
		return (klpd_unreg((int)op, (idtype_t)itype, (id_t)type));
	case PRIVSYS_PFEXEC_REG:
		return (pfexec_reg((int)op));
	case PRIVSYS_PFEXEC_UNREG:
		return (pfexec_unreg((int)op));
	case PRIVSYS_ZONE_LOCK:
	case PRIVSYS_ZONE_LOCK_TEMP:
		return (priv_mwac_enable(code == PRIVSYS_ZONE_LOCK,
		    (zoneid_t)op));
	case PRIVSYS_ZONE_UNLOCK:
		return (priv_mwac_disable((zoneid_t)op));
	case PRIVSYS_ZONE_MWAC_WHITE_LIST:
		return (priv_mwac_list((zoneid_t)op, bufsize, buf, B_TRUE));
	case PRIVSYS_ZONE_MWAC_BLACK_LIST:
		return (priv_mwac_list((zoneid_t)op, bufsize, buf, B_FALSE));
	case PRIVSYS_ZONE_TRANSIENT_RW:
		return (priv_mwac_mark_transient((zoneid_t)op));
	}
	return (set_errno(EINVAL));
}

...
#define	AUDIT_KEVENTS(c, m) ((c->auk_auditstate == AUC_AUDITING) && \
	    (((m.am_success | m.am_failure) & c->auk_classes) != 0))
...
setaudit(caddr_t info_p)
{
	...
	/* See if there are kernel events to be audited */
	if (AUDIT_KEVENTS(kctx, ainfo->ai_mask)) {
...
setqctrl(caddr_t data)
{
	...
	/* enforce sane values */
	if (!CHK_BDRY_QBUFSZ(qctrl_tmp.aq_bufsz) ||
	    !CHK_BDRY_QDELAY(qctrl_tmp.aq_delay) ||
	    !CHK_BDRY_QLOWATER(qctrl_tmp.aq_lowater, qctrl_tmp.aq_hiwater) ||
	    !CHK_BDRY_QHIWATER(qctrl_tmp.aq_lowater, qctrl_tmp.aq_hiwater)) {
		return (EINVAL);
	}
...
setclass(caddr_t data)
{
	...
	/* determine the set of kernel event classes */
	if (event.ec_number == AUE_NULL) {
		/* clear the class set */
		kctx->auk_classes = AU_MASK_NONE;
	} else {
		/* accumulate kernel event classes */
		kctx->auk_classes |= event.ec_class;
	}

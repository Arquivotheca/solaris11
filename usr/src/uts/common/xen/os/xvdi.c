    ...
    domid_t dom, int vdev)
{
	...
	/*
	 * In HVM domain, do not create PV device node for cdrom device,
	 * because for certain "xvd*:cdrom" devices, two I/O paths will be
	 * exported for the same device(one PV and one HVM), we only support
	 * HVM I/O path at this point.
	 */
#ifdef XPV_HVM_DRIVER
	if ((xdcp->xsdev != NULL) && (xsname != NULL) &&
	    strcmp(xdcp->xsdev, "vbd") == 0) {
		char *str = NULL;
		if (xenbus_read_str(xsname, XBP_DEV_TYPE, &str) != 0) {
			cmn_err(CE_WARN, "can not read device-type\n");
			return (NULL);
		}

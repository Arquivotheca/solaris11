...
struct authdes_cache_entry {
	...
	void *localcred;		/* generic local credential */
...
struct bsdcred {
	credgrp_t *credgrps;	/* cached gids */
...
kauthdes_getucred(const struct authdes_cred *adc, cred_t *cr)
{
	...
	mutex_exit(&authdes_lock);
	cred = nickentry->localcred;
	if (!cred->valid) {
		/*
		 * not in cache: lookup
		 */
		if (knetname2user(adc->adc_fullname.name, &i_uid, &i_gid,
		    &cred->credgrps) != RPC_SUCCESS) {
			/*
	...
	if (cred->credgrps != NULL)
		crgrphold(cred->credgrps);
	crsetcredgrp(cr, cred->credgrps);
	mutex_exit(&nickentry->lock);
	return (1);
}
...
authdes_cache_new(char *fullname, des_block *sessionkey, uint32_t window) {
	...
	if (!(ucred = kmem_alloc(sizeof (struct bsdcred), KM_NOSLEEP))) {
...
authdes_cache_reclaim(void *pdata) {
	...
	for (i = 0; i < n; i++) {
		...
		lcred = p->localcred;
		if (lcred->credgrps != NULL)
			crgrprele(lcred->credgrps);
		kmem_free(lcred, sizeof (struct bsdcred));

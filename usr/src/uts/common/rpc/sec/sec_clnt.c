	...
	*ap = (AUTH *)NULL;
	...
	for (;;) {
		...
		case AUTH_DES:
			...
			/*
			 *  A better way would be to have a cred parameter to
			 *  authdes_create.
			 */
		...
		if (desauthtab != NULL) {
			for (da = desauthtab;
				...
				if (uid == da->da_uid &&
					...
					/* Make sure it is not found. */
					da->da_data = NULL;
					da->da_uid = 0;
					da->da_zoneid = INVALID_ZONE;
				}
			}
		}
		mutex_exit(&desauthtab_lock);

...
xdrrdma_destroy(XDR * xdrs)
{
	...
	if (xdrp->xp_wcl) {
		if (xdrp->xp_flags &
		    (XDR_RDMA_WLIST_REG|XDR_RDMA_WLIST_PREREG)) {
 ...
 * bytes contain no chunks to separate out, and if the bytes do not fit in
...
xdrrdma_control(XDR *xdrs, int request, void *info)
{
	...
	case XDR_RDMA_ADD_CHUNK:
		...
		case RCI_WRITE_UIO_CHUNK:
			...
			if (rcip->rci_len <= rdma_prereg_bufsize) {
				bzero(&rdmabuf, sizeof (rdmabuf));
				rdmabuf.type = RDMA_LONG_BUFFER;
				rdmabuf.len = rdma_prereg_bufsize;
				if (rdma_buf_alloc(xdrp->xp_conn, &rdmabuf)) {
					DTRACE_PROBE1(
					    krpc__i__xdrrdma_control_noregbuf,
					    int, rdma_prereg_bufsize);
				}
				memp = rdmabuf.addr;
				xdrp->xp_flags |= XDR_RDMA_WLIST_PREREG;
			}
			...
			for (i = 0; i < uiop->uio_iovcnt; i++) {
				...
				if (first == NULL) {
					first = rwl;
					if (memp)
						first->rb_longbuf = rdmabuf;
				}

				if (iolen >= uiop->uio_iov[i].iov_len)
				...
				if (memp) {
					...
					/*
					 * if userspace address, put adspace
					 * ptr in clist. If not, then do
					 * nothing since it's already set to
					 * NULL (from kmem_zalloc)
					 */
					if (uiop->uio_segflg ==
					    UIO_USERSPACE) {
						rwl->c_adspc =
						    ttoproc(curthread)->p_as;
					}
				}
	...
	case XDR_RDMA_GET_WCINFO:
		...
		return (TRUE);
	case XDR_RDMA_SET_PREREGBUF:
		wlist_xdrp = (XDR *)info;
		wlist_private = (xrdma_private_t *)wlist_xdrp->x_private;
		if (xdrp->xp_wcl == NULL)
			return (TRUE);

		xdrp->xp_wcl->rb_longbuf = wlist_private->xp_wcl->rb_longbuf;
		xdrp->xp_flags |= XDR_RDMA_WLIST_PREREG;

		/*
		 * We have now transfered control of the preregistered longbuf
		 * to the xdrp, from the wlist_xdrp.  Make sure to zero the
		 * longbuf in the wlist_xdrp or else the XDR_DESTROY of
		 * wlist_xdrp and xdrp would end up freeing the longbuf
		 * twice.
		 */
		bzero(&wlist_private->xp_wcl->rb_longbuf, sizeof (rdma_buf_t));
		wlist_private->xp_flags &= ~XDR_RDMA_WLIST_PREREG;
		return (TRUE);

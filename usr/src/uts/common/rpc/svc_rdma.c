    ...
    rdma_xprt_group_t *started_xprts)
{
	...
	for (rmod = rdma_mod_head; rmod != NULL; rmod = rmod->r_next) {
		...
		xprt->xp_sct = sct;
		xprt->xp_type = T_RDMA;
		mutex_init(&xprt->xp_ref_lock, NULL, MUTEX_DEFAULT, NULL);
		xprt->xp_ref = 0;
		 ...
		 */
		rd->rd_data.svcid = id;
		error = stp_xprt_register(xprt, id);
		if (error) {
...
svc_rdma_krecv(SVCXPRT *clone_xprt, mblk_t *mp, struct rpc_msg *msg)
{
	...
	if (cl != NULL && op == RDMA_NOMSG) {
		...
		/*
		 * set cl to NULL, for cleanup if errors occur below.
		 */
		cl = NULL;
	}
...
cll_malloc_err:
	/*
	 * This cl represents any additional cl's from a NOMSG buffer
	 * type that exist beyond the initial cl that holds the rpcmsg
	 * buffer.  The cl that holds the rpcmsg, and for MSG bytes is
	 * in the xp->rcl of the xdrs and will be freed with the
	 * XDR_DESTROY below.
	 */
	if (cl)
		clist_free(cl);

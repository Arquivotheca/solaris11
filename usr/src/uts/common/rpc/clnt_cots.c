    ...
    caddr_t argsp, xdrproc_t xdr_results, caddr_t resultsp, struct timeval wait)
{
	...
	clock_t ticks, now;
	clock_t start_disp_time, stop_disp_time;
...
call_again:
	...
	start_disp_time = ddi_get_lbolt();
	if (clnt_dispatch_send(wq, mp, call, p->cku_xid,
	    (p->cku_flags & CKU_ONQUEUE), max_waitsecs) != RPC_SUCCESS) {
		p->cku_err.re_status = call->call_status;
	...
	stop_disp_time = ddi_get_lbolt();
	waitsecs = (drv_hztousec(stop_disp_time - start_disp_time)/1000000);
	if (waitsecs) {
...
start_retry_loop:
	...
	if (cm_entry->x_early_disc) {
		...
		cm_entry->x_dead = (cm_entry->x_connected == FALSE);
	}
	...
	cred_t			*cr)
{
	 ...
	 */
	if (clnt_dispatch_send(wq, mp, e, 0, 0, waitp->tv_sec) != RPC_SUCCESS) {
    ...
    calllist_t *e, cred_t *cr)
{
	 ...
	 */
	if (clnt_dispatch_send(wq, mp, e, 0, 0, clnt_cots_min_tout)
	    != RPC_SUCCESS) {
...
clnt_dispatch_send(queue_t *q, mblk_t *mp, calllist_t *e, uint_t xid,
			uint_t queue_flag, long timeout)
{
	int done = 0;
	...
	/*
	 * Krpc bypasses normal streams flow control methods by
	 * pushing messages directly onto rpcmod's queue. Traditional
	 * style of pushing messages onto streams head would have
	 * allowed the streams framework to take care of flow control.
	 * With current rpc design, we need to implement flow control
	 * on write queue by using QWANTWSYNC flag. If the write
	 * queue is full, we wait for stream framework to signal us
	 * via qbackenable() once flow control has been relieved on
	 * the write queue.
	 */
	if (!canput(q)) {

		/*
		 * Don't delay if the zone is shutting down
		 */
		...
		/*
		 * Set QWANTWSYNC to indicate we want to be
		 * signaled by the STREAM once the queue counts
		 * drop below q_lowat
		 */
		...
		/*
		 * Recheck while holding the sd_lock to ensure we didn't
		 * miss the wakeup from qbackenable().
		 */
		mutex_enter(&STREAM(q)->sd_lock);
		if ((q->q_flag & QWANTWSYNC) && (strwaitq(STREAM(q), WRITEWAIT,
		    (ssize_t)0, 0, (clock_t)timeout*1000, &done) || done)) {
			mutex_exit(&STREAM(q)->sd_lock);
			e->call_status = RPC_CANTSEND;
			e->call_reason = ENOBUFS;
			return (RPC_CANTSEND);
		}

...
#define	XDR_RDMA_WLIST_PREREG		0x8

#define	LONG_REPLY_LEN	65536
#define	WCL_BUF_LEN	32768
#define	RCL_BUF_LEN	32768

#define	RDMA_CHUNK_HDR  0x10	/* size of encoded chunk list in rdma header */
/* Addition bytes in the chunk list portion of the rdma header */
/* to represent the number of segments and the ending boolean FALSE. */
#define	RDMA_CHUNK_LIST_OVERHEAD	0x8
#define	RDMA_HDR	0x20	/* RDMA header size not including chunks */
...
struct clist {
	...
	uint32		c_flags;	/* chunk list flags */
	struct clist	*c_next;	/* Next chunk */
};

#define	CLIST_FORCE_DEREG 0x01	/* Force deregistration of the clist longbuf */
...
struct rdma_data {
	...
	timeout_id_t	rd_fctid;	/* timeout id for async disable */
					/* of flow control */
};
...
#define	C_CONN_FLOWCONTROLLED	0x00000004	/* rpc server in flow control */
...
typedef struct conn {
	...
	mblk_t 		*c_msgs;	/* flow controlled message queue */
} CONN;
...
typedef struct rdmaops {
	...
	rdma_stat	(*rdma_deregmemsync)(CONN *, caddr_t, struct mrc,
			    void *, void *);
	rdma_stat	(*rdma_force_dereg)(CONN *, rdma_buf_t *rbuf);
	...
	/* MISC */
	rdma_stat	(*rdma_getinfo)(rdma_info_t *);
	rdma_stat	(*rdma_ctrl)(struct rdma_data *, int);
} rdmaops_t;
...
#define	RDMA_FORCE_DEREG(conn, rdma_buf) \
	(*(conn)->c_rdmamod->rdma_ops->rdma_force_dereg)(conn, rdma_buf)
...
#define	RDMA_CONTROL(rdma_mod, rdma_data, cmd)	\
	((*(rdma_mod)->rdma_ops->rdma_ctrl)(rdma_data, cmd))

/* Flags for rdma_ctrl */
#define	RDMA_DISABLE_FLOWCTRL	0x0001

#ifdef _KERNEL
extern int rdma_prereg_bufsize;
...
extern kcondvar_t rdma_modload_wait;    /* wait for rdma modload to finish */
...
extern void rdma_svc_unblock(SVCMASTERXPRT *);

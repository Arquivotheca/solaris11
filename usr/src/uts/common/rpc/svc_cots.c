 ...
 * cots ZSD
 */
typedef struct svc_cots_globals {
	list_node_t	sg_gnode;	/* link for svc_xprt.sxg_cots_list */
	drc_globals_t	sg_drc;		/* cots dupcache globals */
	zoneid_t	sg_zoneid;
} svc_cots_globals_t;
 ...
 * Kept in xprt->xp_p2.
 */
typedef struct svc_cots_master_data {
	char		*smd_src_addr;		/* client's address */
	int		smd_xprt_started;	/* flag for clone routine to */
						/* call rpcmod start routine */
	struct rpc_cots_server	*smd_stats;	/* stats for zone */
	svc_cots_globals_t	*smd_globals;	/* per-zone xprt data */
} svc_cots_master_data_t;
...
#define	CLONE2STATS(clone_xprt)	\
	((svc_cots_master_data_t *)(clone_xprt)->xp_master->xp_p2)->smd_stats
...
static void *svc_cots_zoneinit(zoneid_t);
static void svc_cots_zonefini(zoneid_t, void *);
    ...
    SVCMASTERXPRT **nxprt)
{
	...
	ASSERT(rpcstat != NULL);
	sgp = zone_getspecific(svc_xprt.sxg_cots_key, curproc->p_zone);
	ASSERT(sgp != NULL);

	xprt = kmem_zalloc(sizeof (SVCMASTERXPRT), KM_SLEEP);

	smd = kmem_zalloc(sizeof (*smd) + sizeof (*ack_p)
	    + (2 * sizeof (sin6_t)), KM_SLEEP);

	ack_p = (struct T_addr_ack *)&smd[1];
	...
	smd->smd_xprt_started = 0;
	smd->smd_stats = rpcstat->rpc_cots_server;
	smd->smd_globals = sgp;
	...
	if (err) {
		kmem_free(smd, sizeof (*smd) + sizeof (*ack_p) +
		    (2 * sizeof (sin6_t)));
	...
	smd->smd_src_addr = xprt->xp_rtaddr.buf =
	...
	/*
	 * for async sendreply()'s.  Only applicable in case
	 * of T_COTS/T_COTS_ORD right now.
	 */
	list_create(&xprt->xp_async_msgs.msg_list, sizeof (rpc_async_msg_t),
	    offsetof(rpc_async_msg_t, ram_next));
	mutex_init(&xprt->xp_async_msgs.msg_lock, NULL, MUTEX_DEFAULT, NULL);
...
{
	svc_cots_master_data_t *smd = (svc_cots_master_data_t *)xprt->xp_p2;
	...
	list_destroy(&xprt->xp_async_msgs.msg_list);
	mutex_destroy(&xprt->xp_async_msgs.msg_lock);
	mutex_destroy(&xprt->xp_ref_lock);

	kmem_free(smd, sizeof (*smd) + sizeof (struct T_addr_ack) +
	    (2 * sizeof (sin6_t)));
...
svc_cots_kstart(SVCMASTERXPRT *xprt)
{
	...
	if (smd->smd_xprt_started == 0) {
		/*
		 * Acquire the xp_ref_lock in order to use xp_wq
...
static void *
svc_cots_zoneinit(zoneid_t zoneid)
{
	svc_cots_globals_t *sgp;
	...
	sgp->sg_drc.dg_maxdrq = cotsmaxdupreqs;

	mutex_enter(&svc_xprt.sxg_lock);
	list_insert_tail(&svc_xprt.sxg_cots_list, sgp);
	svc_xprt.sxg_cots_cnt++;
	mutex_exit(&svc_xprt.sxg_lock);
	return (sgp);
}

static void
svc_cots_zonefini(zoneid_t zoneid, void *arg)
{
	svc_cots_globals_t *sgp = arg;

	ASSERT(sgp->sg_zoneid == zoneid);

	mutex_enter(&svc_xprt.sxg_lock);
	list_remove(&svc_xprt.sxg_cots_list, sgp);
	svc_xprt.sxg_cots_cnt--;
	mutex_exit(&svc_xprt.sxg_lock);
...
svc_cots_init(void)
{
	...
	svc_xprt.sxg_cots_cnt = 0;
	list_create(&svc_xprt.sxg_cots_list, sizeof (svc_cots_globals_t),
	    offsetof(svc_cots_globals_t, sg_gnode));
	zone_key_create(&svc_xprt.sxg_cots_key, svc_cots_zoneinit, NULL,
	    svc_cots_zonefini);
}
...
svc_cots_fini(void)
{
	(void) zone_key_delete(svc_xprt.sxg_cots_key);
	mutex_enter(&svc_xprt.sxg_lock);
	list_destroy(&svc_xprt.sxg_cots_list);
	mutex_exit(&svc_xprt.sxg_lock);
	mutex_destroy(&cots_kcreate_lock);
}

...
_init(void)
{
	...
	cv_init(&rdma_modload_wait, NULL, CV_DEFAULT, NULL);
		 ...
		 */
			...
			case RPC_SERVER:
				...
				if (rmp->rm_krpc_cell) {
					 ...
					 * This is UDP, in case of failure,
					 * just free the message.
					 */
					if (stp_queuereq(q, mp)) {
						mutex_enter(&rmp->rm_lock);
						rmp->rm_ref--;
						mutex_exit(&rmp->rm_lock);
						freemsg(mp);
...
/*
 * Called when the pool's p_reqs drop below
 * low water mark. Enable the service procedure
 * to drain backed up requests if we are not
 * blocked on the write side.
 */
void
mir_unblock(queue_t *q)
{
	mir_t *mir = (mir_t *)q->q_ptr;
	if (!mir->mir_inwservice) {
		qenable(RD(q));
		mir->mir_hold_inbound = 0;
		MIR_CLEAR_INRSRV(mir);
	}

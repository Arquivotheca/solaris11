...
static int
dtrace_safe_phdr(Phdr *phdrp, uarg_t *args, uintptr_t base)
{
...
mapexec_brand(vnode_t *vp, uarg_t *args, Ehdr *ehdr, Addr *uphdr_vaddr,
    intptr_t *voffset, caddr_t exec_file, int *interp, execenv_t *envp,
    uintptr_t *lddatap)
{
	size_t		len;
	struct vattr	vat;
	caddr_t		phdrbase = NULL;
	caddr_t		*bssbase = NULL;
	caddr_t		*brkbase = NULL;
	size_t		*brksize = NULL;
	ssize_t		phdrsize;
	...
	if (envp) {
		bssbase = &envp->ex_bssbase;
		brkbase = &envp->ex_brkbase;
		brksize = &envp->ex_brksize;
	}
    ...
    int brand_action)
{
	...
	*execsz = btopr(SINCR) + btopr(SSIZE) + btopr(NCARGS-1);
	 ...
	 */
	if ((level < 2) &&
	    (brand_action != EBA_NATIVE) && (PROC_HAS_BRANDOPS(p))) {
	...
	if ((brand_action != EBA_NATIVE) && (PROC_HAS_BRANDOPS(p))) {
		branded = 1;
		/*
...
/*
 * Phdr resolves to either Elf64_Phdr or Elf32_Phdr, each requiring a different
 * alignment.
 */
#define	PHDRALIGNBITS	3
#else
size_t elf_nphdr_max = 1000;
#define	PHDRALIGNBITS	7
#endif
    ...
    caddr_t *phbasep, ssize_t *phsizep)
{
	 ...
	 * array of program headers, it must be appropriately aligned or else
	 * we might cause a misaligned access. We use all members through
	 * p_flags on 32-bit ELF files and p_memsz on 64-bit ELF files so
	 * e_phentsize must be at least large enough to include those
	 * members. We also ensure that the calculation of the PHT's size does
	 * not overflow.
	 */
...
#endif
	if (ehdr->e_phentsize < minsize ||
	    (ehdr->e_phentsize & PHDRALIGNBITS) ||
	    nphdrs > INT_MAX / ehdr->e_phentsize)
		return (EINVAL);
...
/*
 * Shdr resolves to either Elf64_Shdr or Elf32_Shdr, each requiring a different
 * alignment.
 */
#define	SHDRALIGNBITS	3
#else
size_t elf_nshdr_max = 10000;
size_t elf_shstrtab_max = 100 * 1024;
#define	SHDRALIGNBITS	7
#endif
    ...
    char **shstrbasep, ssize_t *shstrsizep)
{
	 ...
	 * array of section headers, it must be appropriately aligned or else
	 * we might cause a misaligned access. We use all members through
	 * sh_entsize (on both 32- and 64-bit ELF files) so e_shentsize
	 * must be at least large enough to include that member. The index
	 * of the string table section must also be valid. We also ensure that
	 * the calculation of the SHT's size does not overflow.
	 */
	minsize = offsetof(Shdr, sh_entsize) + sizeof (shdr->sh_entsize);
	if (ehdr->e_shentsize < minsize ||
	    (ehdr->e_shentsize & SHDRALIGNBITS) || shstrndx >= nshdrs ||
	    nshdrs > INT_MAX / ehdr->e_shentsize)
		return (EINVAL);

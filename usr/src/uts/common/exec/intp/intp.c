 ...
 */
static int
getintphead(vnode_t *vp, intpdata_t *idatap)
{
	int error;
	...
	int brand_action)
{
	 ...
	 * We do the same thing when running execve(..., EXEC_DESCRIPTOR).
	 */
	if ((setid & EXECSETID_PRIVS) != 0 ||
	    (setid & (EXECSETID_UGIDS|EXECSETID_SETID)) ==
	    (EXECSETID_UGIDS|EXECSETID_SETID) ||
	    uap->execfd >= 0) {
		if ((error = VOP_ACCESS(vp, VREAD, 0, cred, NULL)) != 0)
			goto done;

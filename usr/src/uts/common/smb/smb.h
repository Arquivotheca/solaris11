...
/*
 * Protocol magic value as a 32-bit number and as a string.
 * This is '0xff 0x53 0x4d 0x42' on the wire.
 */
#define	SMB_PROTOCOL_MAGIC	0x424d53ff
#define	SMB_MAGIC_SIG		"\xFFSMB"
#define	SMB_MAGIC_LEN		4

#define	SMB_SIG_SIZE		8	/* SMB security signature size */
#define	SMB_HEADER_LEN		32

/*
 * Size of the GUID returned in an extended security negotiate response.
 */
#define	SMB_GUID_LEN		16

#define	SMB_UID_RESERVED	0xffff
#define	SMB_TID_RESERVED	0xffff
#define	SMB_FID_RESERVED	0xffff

/*
 * Protocol Dialects
 *
 * PC NETWORK PROGRAM 1.0	The original MSNet SMB protocol, referred to
 *				as the Core protocol.
 * PCLAN 1.0			Alternate name for the MSNet SMB Core protocol.
 * MICROSOFT NETWORKS 1.03	SmbLockAndRead, SmbWriteAndUnlock, SmbReadRaw
 * 				and SmbWriteRaw extensions.
 * Microsoft Networks 3.0	Equivalent to DOS LANMAN 1.0 except that OS/2
 * 				errors are mapped to DOS errors.
 * LANMAN 1.0			The original DOS LANMAN protocol.
 * Windows for Workgroups 3.1a	SMB for Windows for Workgroups Version 1.0
 * LANMAN 2.0			LM1.2x002 - LANMAN 2.0 protocol.
 * DOS LANMAN 2.0		DOS LM1.2x002 - Equivalent to LM1.2x002 but
 *				errors must be mapped to DOS errors.
 * DOS LANMAN 2.1		DOS version of LANMAN 2.1
 * LANMAN 2.1			OS/2 version of LANMAN 2.1
 * NT LM 0.12			Windows NT specific extensions to support NT
 * 				semantics over the network.
 */
...
#define	LANMAN1_0			5
#define	Windows_for_Workgroups_3_1a	6
#define	LANMAN2_0			7
#define	DOS_LANMAN2_0			8
#define	DOS_LANMAN2_1			9
#define	LANMAN2_1			10
#define	NTLM_0_12			11
 ...
 *
 * ISMB_FLAGS_REPLY               This bit indicates that the SMB is being
 * (SMB_FLAGS_SERVER_TO_REDIR)    sent from server to (client) redirector.
 */
...
#define	SMB_FLAGS2_COMPRESEED				0x0008
 ...
 *
 * Data buffer format codes are used to identify the type and format
 * of the fields that immediately follow them in the data flock of
 * SMB messages.
 * In Core Protocol commands, every field in the data block (following
 * the ByteCount field) is preceded by a one-byte buffer format field.
 * Commands introduced in dialects subsequent to the Core protocol
 * typically do not include buffer format fields.
 */
#define	SMB_FMT_DATA		0x01
#define	SMB_FMT_DIALECT		0x02
#define	SMB_FMT_PATHNAME	0x03
#define	SMB_FMT_ASCII		0x04
#define	SMB_FMT_VARIABLE	0x05
...
/*
 * Negotiated Capabilities (CIFS/1.0 4.1.1 and [MS-SMB] 2.2.4.5)
 *
 ...
 *
 * CAP_RAW_MODE		The server supports SMB_COM_READ/WRITE_RAW
 * CAP_MPX_MODE		The server supports SMB_COM_READ/WRITE_MPX
 * CAP_UNICODE		The server supports Unicode strings
 * CAP_LARGE_FILES	The server supports large files with 64 bit offsets
 * CAP_NT_SMBS		The server supports the NT LM 0.12 SMB dialect
 *			Implies CAP_NT_FIND
 * CAP_RPC_REMOTE_APIS	The server supports the DCE RPC remote admin API
 * CAP_STATUS32		The server can respond with 32 bit status codes
 * CAP_LEVEL_II_OPLOCKS	The server supports level 2 oplocks
 * CAP_LOCK_AND_READ	The server supports SMB_COM_LOCK_AND_READ SMB
 * CAP_DFS		The server is DFS aware
 * CAP_INFOLEVEL_PASSTHRU	The server supports passthru information
 *				level processing capability
 * CAP_LARGE_READX	The server supports large SMB_COM_READ_ANDX
 * CAP_LARGE_WRITEX	The server supports large SMB_COM_WRITE_ANDX
 * CAP_EXTENDED_SECURITY	The server supports extended security exchanges
 *
...
#define	CAP_COMPRESSED_BULK		0x00000800
...
/*
 * The following bits may be set in the SecurityMode field
 * of the SMB_COM_NEGOTIATE response.
 *
 * NEGOTIATE_SECURITY_SHARE_LEVEL is a local invention.
 *
...
#define	NEGOTIATE_SECURITY_ENCRYPT_PASSWORDS	    0x02
...
#define	SMB_OACT_LOCK			SMB_OACT_OPLOCK

/*
 * NtCreateX CreateDisposition flags
 *
...
/*
 * NtCreateX and NtTransactCreate creation flags: CIFS section 4.2.2
 *
 * NT_CREATE_REQUEST_OPLOCK		Level I oplock requested
 * NT_CREATE_REQUEST_OPBATCH		Batch oplock requested
 * NT_CREATE_OPEN_TARGET_DIR		Target for open is a directory
 */
...
#define	NT_CREATE_FLAG_EXTENDED		0x10

/*
 * NtCreateX Impersonation levels
 */
...
/*
 * File action type codes for NtNotifyChangeDirectoryFile
 */
 ...
 * [MS-CIFS] section 2.2.8.2.6 and [MS-SMB] section 2.2.1.2.2
 *
 * 0x00000008 is reserved for use by the old DOS VOLID (volume ID)
 * and is therefore not considered valid in NT.
 *
 * 0x00000010 is reserved for use for the old DOS SUBDIRECTORY flag
 * and is therefore not considered valid in NT.  This flag has
 * been disassociated with file attributes since the other flags are
 * protected with READ_ and WRITE_ATTRIBUTES access to the file.
 *
 * Note also that the order of these flags is set to allow both the
 ...
 *
 * Characteristics are the sum of any of the following:
 */
...
/*
 * Transaction2 setup[0] sub-commands
 */
#define	TRANS2_OPEN2			0x00	/* Create file, extended attr */
#define	TRANS2_FIND_FIRST2		0x01	/* Begin search for files */
#define	TRANS2_FIND_NEXT2		0x02	/* Resume search for files */
#define	TRANS2_QUERY_FS_INFORMATION 	0x03	/* Get file system info */
#define	TRANS2_SET_FS_INFORMATION	0x04	/* Set file system info. */
#define	TRANS2_QUERY_PATH_INFORMATION	0x05	/* Get info by named file/dir */
#define	TRANS2_SET_PATH_INFORMATION	0x06	/* Set info by named file/dir */
#define	TRANS2_QUERY_FILE_INFORMATION	0x07	/* Get info by handle */
#define	TRANS2_SET_FILE_INFORMATION	0x08	/* Set info by handle */
#define	TRANS2_FSCTL			0x09	/* Not implemented by NT svr */
#define	TRANS2_IOCTL2			0x0A	/* Not implemented by NT svr */
#define	TRANS2_FIND_NOTIFY_FIRST	0x0B	/* Not implemented by NT svr */
#define	TRANS2_FIND_NOTIFY_NEXT		0x0C	/* Not implemented by NT svr */
#define	TRANS2_CREATE_DIRECTORY		0x0D	/* Create dir, extended attr */
#define	TRANS2_SESSION_SETUP		0x0E	/* Session setup ext security */

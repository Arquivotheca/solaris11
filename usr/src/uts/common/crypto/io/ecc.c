...
static crypto_kcf_provider_handle_t	ecc_prov_handle;
static boolean_t			is_ecc_registered = B_FALSE;
...
_init(void)
{
	...
	if (ret = crypto_register_provider(&ecc_prov_info, &ecc_prov_handle)) {
...
_fini(void)
{
	...
	if (is_ecc_registered) {
		if (crypto_unregister_provider(ecc_prov_handle))
			return (EBUSY);
		is_ecc_registered = B_FALSE;
	}
    ...
    crypto_req_handle_t req)
{
	...
	if (kcf_get_fips140_state() == FIPS140_MODE_ENABLED) {

...
int			dca_mindma = 2500;
static boolean_t	is_dca1_registered = B_FALSE;
static boolean_t	is_dca2_registered = B_FALSE;
...
failed:
	...
	if (is_dca1_registered) {
		(void) crypto_unregister_provider(
		    WORKLIST(dca, MCR1)->dwl_prov);
		is_dca1_registered = B_FALSE;
	}
	if (is_dca2_registered) {
		(void) crypto_unregister_provider(
		    WORKLIST(dca, MCR2)->dwl_prov);
		is_dca2_registered = B_FALSE;
	}
	if (intr_added) {
...
dca_destroyreq(dca_request_t *reqp)
{
	...
	/*
	 * These two DMA handles should have been unbound in
...
dca_gather(crypto_data_t *in, char *dest, int count, int reverse)
{
	...
	case CRYPTO_DATA_RAW:
		...
		if (reverse)
			dca_reverse((char *)in->cd_raw.iov_base + off,
			    dest, count, count);
		else
...
dca_bufdaddr_out(crypto_data_t *data)
{
	...
	switch (data->cd_format) {
	case CRYPTO_DATA_RAW:
		return ((char *)data->cd_raw.iov_base + offset);
	case CRYPTO_DATA_UIO:
		return ((char *)data->cd_uio->uio_iov[0].iov_base + offset);
	case CRYPTO_DATA_MBLK:

...
/* We will use fixed parity size based on MAXMEMBLOCKS */
#define	PARITYSIZE		howmany(MAXMEMBLOCKS, 8)
...
typedef struct physmem_entsrc_s {
	uint8_t parity[PARITYSIZE];	/* parity bit vector */
...
/* Output argument for physmem_block2pfn(). */
typedef struct physmem_pfn_data_s {
	pfn_t pfn;		/* page frame number */
	offset_t pg_offset;	/* page offset */
} physmem_pfn_data_t;
...
static void
physmem_init_parity(physmem_entsrc_t *entsrc)
{
	uint8_t *ptr = entsrc->parity;
	int i = howmany(entsrc->nblocks, 8);
...
/*
 * Converts memory block number to page frame number and page offset.  pfn_buf
 * is pointer where to store result. Returns -1 on failure, 0 on success.
 */
static int
physmem_block2pfn(uint32_t block, physmem_pfn_data_t *pfn_buf)
{
	struct memseg *seg;
	int	ret_val = 0;		/* assume success */
	unsigned int cur_seg_blocks;	/* number of blocks in curr. segment */

	kpreempt_disable();

	/*
	 * We will iterate through list of physical memory segments until we
	 * find segment, where block fits.
	 */
	seg = memsegs;
	while (seg != NULL) {
		cur_seg_blocks = (MSEG_NPAGES(seg) * PAGESIZE) / MEMBLOCKSIZE;

		/*
		 * If desired block is found in current segment, break loop.
		 */
		if (block <= cur_seg_blocks) {
			/* Store base pfn for segment with our desired block. */
			pfn_buf->pfn = seg->pages_base;
			break;
		}

		/*
		 * Block is not in the current segment, let's move to next
		 * segment in the list.
		 */
		block -= cur_seg_blocks;
		seg = seg->next;
	}
	...
	/*
	 * We found a segment with our block. Now we have to find desired page
	 * frame number in segment.
	 */
	if (seg != NULL) {
		/* Add offset in segment to reach pfn with our desired block. */
		pfn_buf->pfn += (block * MEMBLOCKSIZE) / PAGESIZE;
		/* Calculate page offset for given block number. */
		pfn_buf->pg_offset = (block * MEMBLOCKSIZE) % PAGESIZE;
	} else {
		ret_val = -1;
	}
...
/*
 * There are two sources to get random bits from:
 *	- digest of 4k data block found in memory
 *	- timedelta the digest took to compute
 * Function iterates through srndpool array of unsigned integers.  Each integer
 * there represents block number of physical memory to compute digest from.
 * Block number must be converted to page frame number (pfn) and offset in
 * given page.  Once we have a page and offset, we can compute digest from data
 * in the page. The resulting digest is used as entropy bits, which are added
 * to entropy pool together with timedelta.
 */
static void
physmem_ent_gen(physmem_entsrc_t *entsrc)
{
	physmem_pfn_data_t pfn_data;
	page_t *pp;
	...
	for (i = 0; i < RNDPOOLSIZE/4; i++) {
		...
		block = (srndpool[i] % entsrc->nblocks);
		if (physmem_block2pfn(block, &pfn_data) == -1)
			continue;

		/*
		 * Do an initial check to see if the address is safe.
		 */
		if (plat_hold_page(pfn_data.pfn,
		    PLAT_HOLD_NO_LOCK, NULL) == PLAT_HOLD_FAIL)
			continue;

		/*
		 * Load the page and compute the hash of the memory block.
		 */
		len = MEMBLOCKSIZE;
		ts1 = gethrtime();
		HashInit(&ctx);
		while (len > 0) {
			poffset = pfn_data.pg_offset & PAGEOFFSET;
			 ...
			 */
			if (plat_hold_page(pfn_data.pfn,
			    PLAT_HOLD_LOCK, &pp) == PLAT_HOLD_FAIL)
				break;

			hat_devload(kas.a_hat, entsrc->pmbuf,
			    PAGESIZE, pfn_data.pfn, PROT_READ,
			    HAT_LOAD_NOCONSIST | HAT_LOAD_LOCK);

			HashUpdate(&ctx,
			    (uint8_t *)entsrc->pmbuf + poffset, nbytes);
		...
		/*
		 * Compute the time it took to load and hash the block and
		 * compare it against the previous measurement. The delta of
		 * the time values provides a small amount of entropy.  The
		 * minimum of the first, second, and third order delta is used
		 * to estimate how much entropy is present.
		 */
		diff = ts2 - ts1;
		...
		/*
		 * If the memory block has changed, credit the pool with the
		 * entropy estimate.  The entropy estimate is doubled because
		 * the single-bit checksum misses half the change on average.
		 */
		if (physmem_parity_update(entsrc->parity, block,
		    digest[0] & 1))
...
physmem_count_blocks()
{
	...
	for (memseg = memsegs; memseg != NULL; memseg = memseg->next)
		pg_cnt += MSEG_NPAGES(memseg);

	kpreempt_enable();

	entsrc.nblocks = (pg_cnt * PAGESIZE) / MEMBLOCKSIZE;
	entsrc.nblocks = (entsrc.nblocks > MAXMEMBLOCKS) ?
	    MAXMEMBLOCKS : entsrc.nblocks;
}

static void
rnd_dr_handler(void)
{
	/* Memory size has changed, so update entsrc->nblocks. */
	mutex_enter(&srndpool_lock);
	physmem_count_blocks();
	mutex_exit(&srndpool_lock);
}

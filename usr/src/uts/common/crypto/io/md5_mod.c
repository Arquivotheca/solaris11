...
static crypto_kcf_provider_handle_t	md5_prov_handle;
static boolean_t			is_md5_registered;
...
_init(void)
{
	 ...
	 * available.
	 */
	if (crypto_register_provider(&md5_prov_info, &md5_prov_handle) ==
	    CRYPTO_SUCCESS) {
		is_md5_registered = B_TRUE;
	}
...
md5_digest_update_uio(MD5_CTX *md5_ctx, crypto_data_t *data)
{
	...
	while (vec_idx < data->cd_uio->uio_iovcnt && length > 0) {
		...
		MD5Update(md5_ctx,
		    (char *)data->cd_uio->uio_iov[vec_idx].iov_base + offset,
		    cur_len);
	...
	if (offset + digest_len <=
		...
		while (vec_idx < digest->cd_uio->uio_iovcnt && length > 0) {
			...
			bcopy(digest_tmp + scratch_offset,
			    (char *)digest->cd_uio->uio_iov[vec_idx].iov_base +
			    offset, cur_len);
    ...
    crypto_req_handle_t req)
{
	...
	case CRYPTO_DATA_RAW:
		MD5Update(&PROV_MD5_CTX(ctx)->mc_md5_ctx,
		    (char *)data->cd_raw.iov_base + data->cd_offset,
		    data->cd_length);
		break;
	...
	case CRYPTO_DATA_UIO: {
		...
		while (vec_idx < mac->cd_uio->uio_iovcnt && length > 0) {
			...
			if (bcmp(digest + scratch_offset,
			    (char *)mac->cd_uio->uio_iov[vec_idx].iov_base +
			    offset, cur_len) != 0) {
				ret = CRYPTO_INVALID_MAC;
				break;
			}

...
static crypto_kcf_provider_handle_t	blowfish_prov_handle;
static boolean_t			is_blowfish_registered = B_FALSE;
...
_fini(void)
{
	...
	if (is_blowfish_registered) {
		if (crypto_unregister_provider(blowfish_prov_handle))
			return (EBUSY);

		is_blowfish_registered = B_FALSE;
	}

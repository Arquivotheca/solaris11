...
#define	SETUP_CRYPTO_DATA_RAW(data, len)			\
...
#define	SETUP_CRYPTO_DATA_UIO(data, buf, len, _iov, _uio)	\
	(_iov)[0].iov_base = buf;				\
	(_iov)[0].iov_len = len;				\
 ...
 *
 * If KCF_PROV_COPYIN_DATA() returns CRYPTO_SUCCESS then
 * we don't do the copyin of the data the provider does.
 */
static int
    ...
    crypto_call_req_t *))
{
	...
	boolean_t prov_copyin = B_TRUE;
	iovec_t diov[1], eiov[1];
	uio_t duio = { 0 };
	uio_t euio = { 0 };
	...
	prov_copyin = KCF_PROV_COPYIN_DATA(sp->sd_provider) == CRYPTO_SUCCESS;

	if (prov_copyin) {
		/*
		 * if the provider can do the copyin/copyout, inplace operation
		 * can't be used as the provider needs to know the actual
		 * output address.
		 */
		do_inplace = B_FALSE;
	} else {
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(data, STRUCT_FGETP(encrypt, ce_databuf),
		    datalen, diov, duio);
	} else {
		SETUP_CRYPTO_DATA_RAW(data, datalen);
	...
	if (do_inplace) {
		...
		if (prov_copyin) {
			SETUP_CRYPTO_DATA_UIO(encr,
			    STRUCT_FGETP(encrypt, ce_encrbuf),
			    encrlen, eiov, euio);
		} else {
			SETUP_CRYPTO_DATA_RAW(encr, encrlen);
		}
	}
	...
	if (rv == CRYPTO_SUCCESS) {
		...
		if (!prov_copyin && encr.cd_length != 0 &&
		    copyout(encr.cd_raw.iov_base,
    ...
    crypto_call_req_t *))
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(data,
		    STRUCT_FGETP(encrypt_update, eu_databuf),
		    datalen, diov, duio);
	} else {
		SETUP_CRYPTO_DATA_RAW(data, datalen);
		if (datalen != 0 && copyin(STRUCT_FGETP(encrypt_update,
		    eu_databuf), data.cd_raw.iov_base, datalen) != 0) {
			error = EFAULT;
			goto out;
		}
	}
	...
	if (do_inplace) {
		...
		if (prov_copyin) {
			SETUP_CRYPTO_DATA_UIO(encr, STRUCT_FGETP(encrypt_update,
			    eu_encrbuf), encrlen, eiov, euio);
		} else {
			SETUP_CRYPTO_DATA_RAW(encr, encrlen);
		}
	}
	...
	if (rv == CRYPTO_SUCCESS || rv == CRYPTO_BUFFER_TOO_SMALL) {
		if (rv == CRYPTO_SUCCESS) {
			ASSERT(encr.cd_length <= encrlen);
			if (!prov_copyin && encr.cd_length != 0 &&
			    copyout(encr.cd_raw.iov_base, encrbuf,
			    encr.cd_length) != 0) {
				error = EFAULT;
				goto out;
			}
    ...
    int (*final)(crypto_context_t, crypto_data_t *, crypto_call_req_t *))
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(encr, encrbuf, encrlen, eiov, euio);
	} else {
		SETUP_CRYPTO_DATA_RAW(encr, encrlen);
	}
...
release_minor:
	...
	if (!prov_copyin && encr.cd_raw.iov_base != NULL) {
		kmem_free(encr.cd_raw.iov_base, encrlen);
	}
	if (error != 0)
		return (error);
...
digest_update(dev_t dev, caddr_t arg, int mode, int *rval)
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(data,
		    STRUCT_FGETP(digest_update, du_databuf),
		    datalen, diov, duio);
	} else {
		SETUP_CRYPTO_DATA_RAW(data, datalen);
		if (datalen != 0 && copyin(STRUCT_FGETP(digest_update,
		    du_databuf), data.cd_raw.iov_base, datalen) != 0) {
			error = EFAULT;
			goto release_minor;
		}
	}
...
release_minor:
	...
	if (!prov_copyin && data.cd_raw.iov_base != NULL)
		kmem_free(data.cd_raw.iov_base, datalen);
    ...
    crypto_call_req_t *))
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(data,
		    STRUCT_FGETP(crypto_digest, cd_databuf),
		    datalen, diov, duio);

		SETUP_CRYPTO_DATA_UIO(digest, digestbuf,
		    digestlen, eiov, euio);
	} else {
		SETUP_CRYPTO_DATA_RAW(data, datalen);
		if (datalen != 0 && copyin(STRUCT_FGETP(crypto_digest,
		    cd_databuf), data.cd_raw.iov_base, datalen) != 0) {
			error = EFAULT;
			goto release_minor;
		}
		SETUP_CRYPTO_DATA_RAW(digest, digestlen);
	}
	...
	if (rv == CRYPTO_SUCCESS) {
		...
		if (!prov_copyin && digest.cd_length != 0 &&
		    copyout(digest.cd_raw.iov_base,
...
verify(dev_t dev, caddr_t arg, int mode, int *rval)
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(data,
		    STRUCT_FGETP(verify, cv_databuf), datalen, diov, duio);
		SETUP_CRYPTO_DATA_UIO(sign,
		    STRUCT_FGETP(verify, cv_signbuf), signlen, eiov, euio);
	} else {
		SETUP_CRYPTO_DATA_RAW(data, datalen);
		...
		SETUP_CRYPTO_DATA_RAW(sign, signlen);
    ...
    int (*update)(crypto_context_t, crypto_data_t *, crypto_call_req_t *))
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(data,
		    STRUCT_FGETP(sign_update, su_databuf), datalen, diov, duio);
	} else {
		SETUP_CRYPTO_DATA_RAW(data, datalen);
		if (datalen != 0 && copyin(STRUCT_FGETP(sign_update,
		    su_databuf), data.cd_raw.iov_base, datalen) != 0) {
			error = EFAULT;
			goto release_minor;
		}
	}
...
verify_final(dev_t dev, caddr_t arg, int mode, int *rval)
{
	...
	if (prov_copyin) {
		SETUP_CRYPTO_DATA_UIO(sign,
		    STRUCT_FGETP(verify_final, vf_signbuf),
		    signlen, eiov, euio);
	} else {
		SETUP_CRYPTO_DATA_RAW(sign, signlen);
		if (signlen != 0 && copyin(STRUCT_FGETP(verify_final,
		    vf_signbuf), sign.cd_raw.iov_base, signlen) != 0) {
			error = EFAULT;
			goto release_minor;
		}
	}
...
release_minor:
	...
	if (!prov_copyin && sign.cd_raw.iov_base != NULL)
		kmem_free(sign.cd_raw.iov_base, signlen);

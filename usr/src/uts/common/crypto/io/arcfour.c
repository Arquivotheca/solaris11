...
static crypto_kcf_provider_handle_t	rc4_prov_handle;
static boolean_t			is_rc4_registered = B_FALSE;
...
_init(void)
{
	...
	if (ret = crypto_register_provider(&rc4_prov_info, &rc4_prov_handle)) {
...
_fini(void)
{
	...
	if (is_rc4_registered) {
		if (crypto_unregister_provider(rc4_prov_handle))
			return (EBUSY);

		is_rc4_registered = B_FALSE;
	}
...
/* EXPORT DELETE START */
	...
	if (output == NULL) {
		...
		case CRYPTO_DATA_RAW: {
			char *start, *end;
			start = (char *)input->cd_raw.iov_base +
			    input->cd_offset;

			end = (char *)input->cd_raw.iov_base +
			    input->cd_raw.iov_len;
	...
	case CRYPTO_DATA_RAW: {
		char *start, *end;
		start = (char *)input->cd_raw.iov_base + input->cd_offset;

		end = (char *)input->cd_raw.iov_base + input->cd_raw.iov_len;
	...
	switch (out->cd_format) {
		...
		case CRYPTO_DATA_UIO: {
			...
			while (vec_idx < uiop->uio_iovcnt && len > 0) {
				...
				start = (uchar_t *)iovp->iov_base + offset;
				arcfour_crypt(key, in, start + offset, cur_len);

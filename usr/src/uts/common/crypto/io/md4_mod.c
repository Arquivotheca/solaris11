...
static crypto_kcf_provider_handle_t	md4_prov_handle;
static boolean_t			is_md4_registered;
...
_init(void)
{
	...
	if (ret = crypto_register_provider(&md4_prov_info, &md4_prov_handle)) {
...
_fini(void)
{
	...
	if (is_md4_registered) {
		if (crypto_unregister_provider(md4_prov_handle))
			return (EBUSY);

		is_md4_registered = B_FALSE;
	}

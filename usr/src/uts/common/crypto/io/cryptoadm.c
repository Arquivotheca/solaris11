...
get_soft_info(dev_t dev, caddr_t arg, int mode, int *rval)
{
	...
	/* get mechanism names from the core module */
	if ((rv = crypto_get_soft_info(name, &count, &entries)) != 0) {
		soft_info.si_return_value = rv;
		if (copyout(&soft_info, arg, sizeof (soft_info)) != 0) {
			return (EFAULT);
		}
		return (0);
...
/*
 * This function gets the current FIPS 140 mode status
 */
/* ARGSUSED */
static int
fips140_actions(dev_t dev, caddr_t arg, int mode, int *rval, int cmd)
{
	...
	case CRYPTO_FIPS140_STATUS:
		fips140_info.fips140_status = kcf_get_fips140_state();
		break;
	default:
		rv = CRYPTO_ARGUMENTS_BAD;
	}

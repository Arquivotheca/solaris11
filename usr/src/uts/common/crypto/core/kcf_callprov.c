 ...
 *
 * This function assumes that the mech_type is supported by the provider
 * which can be ensured by using is_valid_provider_for_mech() beforehand.
 */
static int
	...
	crypto_key_t *key)
{
	...
	ASSERT(KCF_TO_PROV_MECH_INDX(provider, mech_type) != KCF_INVALID_INDX);
 ...
 *
 * If second mechanism is specified, the resulting provider must support that
 * mechanism as well. Both mechanisms are expected to belong into the same
 * function group.
 *
    ...
    kcf_provider_desc_t *old, kcf_provider_desc_t **new, crypto_func_group_t fg)
{
	...
	kcf_mech_entry_t *me, *me2 = NULL;
	int len, gqlen = INT_MAX, rv = CRYPTO_SUCCESS;
	kcf_lock_withpad_t *mp;

	/* When mech_type1 is a valid mechanism, me will be its mech_entry. */
	if (kcf_get_mech_entry(mech_type_1, &me) != KCF_SUCCESS) {
		return (CRYPTO_MECHANISM_INVALID);
	}
	if (mech_type_2 != CRYPTO_MECH_INVALID) {
		if (kcf_get_mech_entry(mech_type_2, &me2) != KCF_SUCCESS)
			return (CRYPTO_MECHANISM_INVALID);
	}
	...
	if (old->pd_prov_type == CRYPTO_LOGICAL_PROVIDER) {
		...
		while (p != NULL) {
			...
			if (mech_type_2 != CRYPTO_MECH_INVALID) {
				if (!is_valid_provider_for_mech(provider, me2,
				    fg)) {
					p = p->pl_next;
					continue;
				}
    ...
    crypto_func_group_t fg, size_t data_size)
{
	...
	if ((prov_chain != NULL) &&
		...
		while (prov_chain != NULL) {
			...
			if (!is_valid_provider_for_mech(pd, me, fg)) {
				prov_chain = prov_chain->pm_next;
				continue;
			}
 ...
 * a dual operation with both mech1 and mech2.
 *
 * If a non-NULL key structure is supplied, the provider will be checked
 * to see if the key length falls into the limits of given mechanism
 * for that provider. This is done for both key structures and mechanisms.
    ...
    size_t data_size)
{
	...
	kcf_mech_entry_t *me1, *me2;
	kcf_lock_withpad_t *mp;
	int error_val = CRYPTO_MECH_NOT_SUPPORTED; /* default error value */

	/* When mech is a valid mechanism, me will be its mech_entry. */
	if (kcf_get_mech_entry(mech1->cm_type, &me1) != KCF_SUCCESS) {
		*error = CRYPTO_MECHANISM_INVALID;
		return (NULL);
	}
	if (kcf_get_mech_entry(mech2->cm_type, &me2) != KCF_SUCCESS) {
		*error = CRYPTO_MECHANISM_INVALID;
		return (NULL);
	}
	...
	*prov_mt2 = CRYPTO_MECH_INVALID;
	...
	prov_chain = me1->me_hw_prov_chain;
	/*
	 ...
	 */
	if ((prov_chain != NULL) &&
	    ((data_size == 0) || (me1->me_threshold == 0) ||
	    (data_size >= me1->me_threshold) ||
	    ((mdesc = me1->me_sw_prov) == NULL) ||
	    (!IS_FG_SUPPORTED(mdesc, fg1)) ||
	    (!KCF_IS_PROV_USABLE(mdesc->pm_prov_desc)))) {
		/* there is at least one provider */
		ASSERT(me1->me_num_hwprov > 0);
		...
		while (prov_chain != NULL) {
			...
			if (!is_valid_provider_for_mech(pd, me1, fg1)) {
				prov_chain = prov_chain->pm_next;
				continue;
			}
...
#define	MIL_MECH_NUM(mil)	(mil)->ml_mech_info.cm_mech_number
				...
				DTRACE_PROBE2(mil_search,
				    crypto_mech_type_t, mil->ml_kcf_mechid,
				    crypto_mech_type_t, m2id);
				if (mil->ml_kcf_mechid == m2id) {
					/*
					 * It is not necessary to check if
					 * provider pd supports mechanism m2id
					 * before doing key len check because
					 * the mil list only contains mechanisms
					 * supported/allowed by the provider.
					 */
					if ((key2 != NULL) &&
	...
	/* no HW provider for this mech, is there a SW provider? */
	if (pd == NULL && (mdesc = me1->me_sw_prov) != NULL) {
		pd = mdesc->pm_prov_desc;
		if (!IS_FG_SUPPORTED(mdesc, fg1) ||
			...
			/* See if pd can do me2 too */
			for (mil = me1->me_sw_prov->pm_mi_list;
			    mil != NULL; mil = mil->ml_next) {
				if ((mil->ml_mech_info.cm_func_group_mask &
				    fg2) == 0)
					continue;
			...
			*prov_mt1 = PMD_MECH_NUM(me1->me_sw_prov);
		}
	}

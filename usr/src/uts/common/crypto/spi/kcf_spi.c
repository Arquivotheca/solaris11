...
static void
copy_ops_vector_v5(crypto_ops_t *src_ops, crypto_ops_t *dst_ops)
{
	dst_ops->co_uio_userspace_ok = src_ops->co_uio_userspace_ok;
}

/*
 * This routine is used to add cryptographic providers to the kCF framework.
    ...
    crypto_kcf_provider_handle_t *handle)
{
	...
	if (info->pi_interface_version > CRYPTO_SPI_VERSION_5) {
	...
	if (info->pi_provider_type != CRYPTO_LOGICAL_PROVIDER) {
		...
		if (info->pi_interface_version >= CRYPTO_SPI_VERSION_5) {
			copy_ops_vector_v5(info->pi_ops_vector,
			    prov_desc->pd_ops_vector);
		}
	}
...
errormsg:
	if (ret != CRYPTO_SUCCESS && sys_shutdown == 0) {
		...
		case CRYPTO_MECH_NOT_SUPPORTED:
			/*
			 * The provider was not registered because all
			 * mechanisms for the provider was disabled. This is
			 * commonly seen with 'cryptoadm list' command and there
			 * is no need to log an error message.
			 */
			break;

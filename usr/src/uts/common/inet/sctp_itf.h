...
extern int sctp_disconnect(struct sctp_s *conn, cred_t *);
extern int sctp_get_opt(struct sctp_s *conn, int level, int opt, void *opts,
    socklen_t *optlen, cred_t *);
...
extern int sctp_listen(struct sctp_s *conn, cred_t *);
extern void sctp_recvd(struct sctp_s *conn, int len);
extern int sctp_sendmsg(struct sctp_s *conn, mblk_t *mp, int flags, cred_t *);
extern int sctp_set_opt(struct sctp_s *conn, int level, int opt,
    const void *opts, socklen_t optlen, cred_t *);

/* Flags for sctp_create(), sctp_alloc_hdr() */
#define	SCTP_CAN_BLOCK			0x01

/* Flag values in b_flag of the mblk_t passed to upcall su_recv() */
#define	SCTP_IS_NOTIFICATION		0x01	/* message is a notification */
#define	SCTP_IS_PARTIAL_DATA		0x02	/* not a full message */

/* Use by sockfs to do sctp_peeloff(). */
#define	SCTP_UC_SWAP			255

/* Private SCTP message header creation function. */
extern int sctp_alloc_hdr_copy(mblk_t **, const void *, int, void **,
    int, uint_t, boolean_t);

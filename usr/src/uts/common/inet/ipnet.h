...
typedef struct ipnet {
	...
	kmutex_t	ipnet_reflock;	/* protects ipnet_refcnt */
	int		ipnet_refcnt;	/* ipnet_reflock */
	kcondvar_t	ipnet_refcv;
	boolean_t	ipnet_closing;	/* Protected by ips_list_lock */
} ipnet_t;
 ...
 * - ips_str_list is a list of open client streams.  ips_list_lock
 *   synchronizes access to the list.  Walkers such as ipnet_dispatch() also use
 *   ipnet_refcnt to prevent an ipnet from closing while it does a putnext.
 ...
 * and any other lock such as ips_list_lock, ips_avl_lock, or if_addr_lock,
...
typedef struct ipnet_stack {
	...
	kcondvar_t	ips_event_cv;

	/* Protected by ips_event_lock */
	uint32_t	ips_ntasks;
	boolean_t	ips_unregistering;

	kmutex_t	ips_avl_lock;
	avl_tree_t	ips_avl_by_index;
	avl_tree_t	ips_avl_by_name;
	kmutex_t	ips_list_lock;		/* Protects ips_str_list */
	list_t		ips_str_list;		/* Protected by ips_list_lock */
	kstat_t		*ips_kstatp;
	ipnet_kstats_t	ips_stats;
	avl_tree_t	ips_avl_by_shared;
	hook_t		*ips_hook;		/* Protected by ips_list_lock */
} ipnet_stack_t;

...
/*
 * For a socket using SO_PASSIVE_CONNECT, its internal state is TCPS_SYN_SENT
 * since the tcp_t reacts as if it is in that state.  From a higher layer
 * perspective, it may be more "natural" to see that this connection is in
 * TCPS_LISTEN state.  So we re-map the state.
 */
#define	TCP_SNMP_REMAP_STATE(connp, tcp)				\
	(((connp)->conn_passive_connect &&				\
	    ((tcp)->tcp_state == TCPS_SYN_SENT)) ? TCPS_LISTEN :	\
	    (tcp)->tcp_state)
...
tcp_snmp_state(tcp_t *tcp)
{
	...
	case TCPS_SYN_SENT:
		/* See TCP_SNMP_REMAP_STATE() comments */
		if (tcp->tcp_connp->conn_passive_connect)
			return (MIB2_TCP_listen);
		else
	...
	for (i = 0; i < CONN_G_HASH_SIZE; i++) {
		...
		while ((connp =
			...
			tce6.tcp6ConnEntryInfo.ce_state =
			    TCP_SNMP_REMAP_STATE(connp, tcp);

...
/*
 * If port is already bound to INADDR_ANY and another app tries to bind() to
 * the specific local address with SO_REUSEADDR set (or vice versa),
 * we allow bind() to succeed only if both bind()s were done using same uid.
 * Setting this variable to 0 disables uid check in this case.
 */
uint32_t tcp_bind_uid_check = 1;
 ...
 * address will be inserted before those bound to INADDR_ANY.
 */
void
...
tcp_update_next_port(in_port_t port, const tcp_t *tcp, boolean_t random)
{
	...
	if (random && tcp_random_anon_port != 0) {
		 ...
		 */
		if (tcps->tcps_largest_anon_port ==
...
tcp_get_next_priv_port(const tcp_t *tcp)
{
	...
	nextport = tcps->tcps_next_priv_port;
retry:
	if (nextport < tcps->tcps_min_anonpriv_port ||
	    nextport >= IPPORT_RESERVED) {
		nextport = IPPORT_RESERVED - 1;
		tcps->tcps_next_priv_port = nextport;
		if (restart)
			return (0);
		restart = B_TRUE;
	}
	...
	/*
	 * tsol_next_port() returns 0 if the port is known to be SLP,
	 * and as the comment above says we're only interested in SLP,
	 * skipping multilevel ports. The value returned here can also
	 * be greater than IPPORT_RESERVED thus the need to check for
	 * this under retry.
	 */
	if (is_system_labeled() &&
	    (nexttsport = tsol_next_port(crgetzone(tcp->tcp_connp->conn_cred),
	    nextport, IPPROTO_TCP, B_FALSE)) != 0) {
		nextport = nexttsport;
		goto retry;
	}
	tcps->tcps_next_priv_port--;
	return (nextport);
}
    ...
    boolean_t bind_to_req_port_only, cred_t *cr)
{
	...
	/*
	 * Remember the uid before calling tcp_bindi(). We do this without
	 * regard to SO_REUSEPORT or SO_REUSEADDR being set because the app
	 * can set them after the bind().
	 */
    ...
    boolean_t bind_to_req_port_only, boolean_t user_specified)
{
	...
	do {
		...
		for (; ltcp != NULL; ltcp = ltcp->tcp_bind_hash_port) {
			 ...
			 * with IP, since IP needs to have that code to
			 * support userland TCP implementations.
			 */
			if (quick_connect &&
			...
			/*
			 * If existing port is bound to a non-wildcard
			 * IP address and the requesting stream is
			 * bound to a distinct different IP address
			 * (non-wildcard, also), it does not conflict
			 * with our bind request. Proceed with checking
			 * of next port.
			 */
			...
			if (!reuseaddr) {
				 ...
				 * binding tcp_t.
				 *
				 * We refuse binding, regardles of SO_REUSEADDR
				 * setting, if:
				 * 1) Two streams are bound to same IP address
				 * or both addr and bound source are wildcards
				 * (INADDR_ANY), or
				 * 2) One bind() was done to INADDR_ANY and the
				 * other to the specified address (or vice
				 * versa), but these were done using different
				 * uids.
				 *
				 * If we found a match of IP address and source
				 * port, or we found that this bind() is done
				 * using different uid, we break.
				 */

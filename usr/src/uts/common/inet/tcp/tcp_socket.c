...
static int	tcp_accept(sock_lower_handle_t, sock_lower_handle_t,
		    sock_upper_handle_t, cred_t *, pm_handle_t *);
...
static int	tcp_connect(sock_lower_handle_t, const struct sockaddr *,
		    socklen_t, sock_connid_t *, cred_t *, pm_handle_t *);
...
tcp_newconn_notify(tcp_t *tcp, ip_recv_attr_t *ira)
{
	...
	/*
	 * Notify the ULP about the newconn, but first place an extra
	 * reference on the listener which could otherwise disappear if
	 * tcp_accept() is called during the execution of the upcall.
	 *
	 * It is guaranteed that no tcp_accept() call will be made for
	 * the eager if the notification fails.
	 */
	CONN_INC_REF(lconnp);
	...
	/* The listener can go away once this reference is dropped */
	CONN_DEC_REF(lconnp);

...
/*
 * Prevent compiler optimization by not using 'static' for system tunables.
 */
boolean_t tcp_outbound_squeue_switch = B_TRUE;
...
tcp_mss_set(tcp_t *tcp, uint32_t mss)
{
	...
	tcp->tcp_cwnd_cnt = 0;
	DTRACE_PROBE3(tcp_cwnd_update, tcp_t *, tcp, uint32_t, tcp->tcp_cwnd,
	    int, TCPCONG_D_CWND);
...
tcp_parse_options(tcpha_t *tcpha, tcp_opt_t *tcpopt)
{
	...
	int		found = 0;
	int32_t		sack_len;
	tcp_seq		sack_begin, sack_end, snxt;
	tcp_t		*tcp;
	...
	while (up < endp) {
		...
		case TCPOPT_SACK:
			if (len <= TCPOPT_HEADER_LEN ||
			    up[1] <= TCPOPT_HEADER_LEN || len < up[1]) {
				break;
			}
			...
			/* Truncated option... */
			if (sack_len < 2 * sizeof (tcp_seq)) {
				break;
			}
			up += TCPOPT_HEADER_LEN;

			/*
			 * Need to take into account that tcp_snxt may be set
			 * beyond tcp_fss.
			 */
			snxt = (tcp->tcp_valid_bits & TCP_FSS_VALID) ?
...
tcp_process_options(tcp_t *tcp, tcpha_t *tcpha)
{
	...
	DTRACE_PROBE3(tcp_cwnd_update, tcp_t *, tcp,
	    uint32_t, tcp->tcp_cwnd, int, TCPCONG_D_CWND);
}
...
tcp_input_listener(void *arg, mblk_t *mp, void *arg2, ip_recv_attr_t *ira)
{
	...
	if (listener->tcp_listen_cnt != NULL) {
		...
		if (atomic_add_64_nv(&tlc->tlc_cnt, 1) > tlc->tlc_max + 1) {
			mutex_exit(&listener->tcp_eager_lock);
			now = ddi_get_lbolt64();
			atomic_add_64(&tlc->tlc_cnt, -1);
			...
			if (now - tlc->tlc_report_time >
				...
				zcmn_err(lconnp->conn_zoneid, CE_WARN,
				    "Listener (port %d) connection max (%lu) "
				    "reached: %lu attempts dropped total\n",
	...
	/*
	 * Set the xmit_hint based on the ring info from GLD
	 * This gives a 1-1 mapping from Rx to Tx rings and
	 * helps in reducing contention.
	 */
	if (ira->ira_xmit_hint != (uint32_t)-1)
		econnp->conn_ixa->ixa_xmit_hint = ira->ira_xmit_hint;
	...
	tcp_cong_init(eager, listener, NULL);

	/*
	 * The eager's conn_rcvbuf should already be inherited from the
	 * listener, or from the per route metrics.  First round up the
	 * eager's tcp_rwnd to the nearest MSS (the MSS value has been
	 * adjusted in tcp_process_options()).  Also make sure that the rwnd
	 * size is at least the required multiple of MSS.  We need to check
	 * before calculating the window scale factor since once the factor
	 * is set, size of tcp_rwnd is bounded.  Then find out the window
	 * scale option value if needed.  Call tcp_rwnd_set() to finish the
	 * setting.
	 */
	eager->tcp_rwnd = MAX(MSS_ROUNDUP(econnp->conn_rcvbuf, eager->tcp_mss),
	    tcps->tcps_recv_hiwat_minmss * eager->tcp_mss);
	...
	if (econnp->conn_sqp == lconnp->conn_sqp) {
		...
		/*
		 * The block comment in squeue.c explains the rationale
		 * for the appropriate flag argument to squeue_enter()
		 */
		SQUEUE_ENTER_ONE(econnp->conn_sqp, mp1, tcp_send_synack,
		    econnp, NULL, SQ_NODRAIN, SQTAG_TCP_SEND_SYNACK);
	}
	return;
error:
	freemsg(mp1);
...
error2:
	freemsg(mp);
	if (tlc_set)
		atomic_add_64(&listener->tcp_listen_cnt->tlc_cnt, -1);
}
...
tcp_input_data(void *arg, mblk_t *mp, void *arg2, ip_recv_attr_t *ira)
{
	...
	/*
	 * Check the CE bit in the IP header.  Later on the first mblk may
	 * be freed and the IP header is no longer accessible.
	 */
	if (tcp->tcp_ecn_ok) {
		if (connp->conn_ipversion == IPV4_VERSION) {
	...
	case TCPS_SYN_SENT:
		if (connp->conn_final_sqp == NULL && !tcp->tcp_loopback &&
		    tcp_outbound_squeue_switch && ira->ira_sqp != NULL &&
		    (flags & (TH_SYN | TH_ACK)) == (TH_SYN | TH_ACK)) {
			ASSERT(connp->conn_initial_sqp == connp->conn_sqp);
			connp->conn_final_sqp = ira->ira_sqp;
			connp->conn_ixa->ixa_sqp = ira->ira_sqp;
			if (ira->ira_xmit_hint != (uint32_t)-1) {
				connp->conn_ixa->ixa_xmit_hint =
				    ira->ira_xmit_hint;
			}
			if (connp->conn_final_sqp != connp->conn_sqp) {
				...
				/*
				 * The block comment in squeue.c explains the
				 * rationale for the appropriate flag argument
				 * to squeue_enter().
				 */
				SQUEUE_ENTER_ONE(connp->conn_sqp, mp,
				    tcp_input_data, connp, ira, SQ_FILL,
		...
		if (flags & TH_ACK) {
			...
			if (tcp->tcp_rexmit) {
				...
				DTRACE_PROBE3(tcp_cwnd_update, tcp_t *, tcp,
				    ...
				    int, TCPCONG_D_CWND_REXMIT);
			}

			tcp->tcp_swl1 = seg_seq;
			tcp->tcp_swl2 = seg_ack;

			new_swnd = ntohs(tcpha->tha_win) << tcp->tcp_snd_ws;
	 ...
	 * data down below.
	 */
	if (TCP_IS_DETACHED_NONEAGER(tcp) && (seg_len > 0 &&
	    SEQ_GT(seg_seq + ((flags & TH_SYN) ? seg_len + 1 : seg_len),
	    tcp->tcp_rnxt))) {
...
try_again:;
	...
	if (gap < 0) {
		...
		if (seg_len <= 0) {
			 ...
			 * But continue processing this segment in order to
			 * use the ACK information it contains, but skip all
			 * other sequence-number processing.  Processing the
			 * ACK information is necessary in order to
			...
			/*
			 * Make sure to check for unack'd FIN when rest of
			 * data has been previously ack'd.  This is the case
			 * when the adjusted seg_len == 0 and the FIN flag is
			 * set.
			 *
			 * We clear the other flag fields related to
			...
			/* No sequence number processing should be done. */
			seg_len = 0;

			/* Now continue the ACK processing. */
			goto process_ack;
		}
...
process_ack:
	...
	if (tcp->tcp_state == TCPS_SYN_RCVD) {
		 ...
		 * hand shake of a active open'ed or passive connect
		 * connection, so we must send up a T_CONN_CON
		 */
		if (tcp->tcp_active_open || connp->conn_passive_connect) {
	...
	if (tcp->tcp_ecn_ok && (flags & TH_ECE)) {
		if (!tcp->tcp_cwr) {
			...
			DTRACE_PROBE3(tcp_cwnd_ssthresh_update, tcp_t *, tcp,
			    uint32_t, tcp->tcp_cwnd,
			...
			if (mp1 && tcp->tcp_suna != tcp->tcp_snxt &&
				...
				if ((dupack_cnt = ++tcp->tcp_dupack_cnt) <
					...
					if (tcp->tcp_unsent > 0 &&
						...
						flags |= TH_LIMIT_XMIT;
						DTRACE_PROBE3(tcp_cwnd_update,
						    tcp_t *, tcp, uint32_t,
						    tcp->tcp_cwnd, int,
						    TCPCONG_D_CWND_LIM_REXMIT);
					}
				} else if (dupack_cnt ==
				    tcps->tcps_dupack_fast_retransmit) {
				...
				if (!tcp->tcp_cwr) {
					TCP_CONG_LOSS(tcp);
					DTRACE_PROBE3(tcp_cwnd_ssthresh_update,
					    tcp_t *, tcp,
					    uint32_t, tcp->tcp_cwnd,
					    uint32_t, tcp->tcp_cwnd_ssthresh);
				}
				...
				if (tcp->tcp_snd_sack_ok) {
					...
					DTRACE_PROBE3(tcp_cwnd_update, tcp_t *,
					    tcp, uint32_t, tcp->tcp_cwnd,
					    int, TCPCONG_D_CWND_FR_ENTER);
				}
			}
	...
	if (tcp->tcp_snd_sack_ok && tcp->tcp_notsack_list != NULL) {
		...
		if (SEQ_GT(seg_ack, tcp->tcp_fack))
			tcp->tcp_fack = seg_ack;
	}
	 ...
	 *
	 * We do this before updating the congestion window, as some
	 * congestion control algorithms use RTT in their calculations.
	 */
	if (tcp->tcp_snd_ts_ok) {
	 ...
	 */
	if (!tcp->tcp_ecn_ok || !(flags & TH_ECE)) {
		tcp->tcp_cong_args.ca_bytes_acked = &bytes_acked;
		TCP_CONG_ACK(tcp);
		DTRACE_PROBE3(tcp_cwnd_ssthresh_update, tcp_t *, tcp,
		    uint32_t, tcp->tcp_cwnd,
		    uint32_t, tcp->tcp_cwnd_ssthresh);
...
xmit_check:
	...
	if ((flags & (TH_REXMIT_NEEDED|TH_XMIT_NEEDED|TH_NEED_SACK_REXMIT|
		...
		if (flags & TH_REXMIT_NEEDED) {
			uint32_t snd_size = TCP_NOT_ACKED(tcp);
...
tcp_set_rto(tcp_t *tcp, clock_t rtt)
{
	...
	if (sv < tcp_sd_min) {
		/*
		 * We do not know that if sv captures the delay ACK effect as
		 * in a long train of segments, a receiver does not delay its
		 * ACKs, except possibly the last odd segment.  So set the
		 * minimum of sv to be tcp_sd_min, which is default to 200 ms,
		 * the value of BSD DATO.  That means the minimum of calculated
		 * mean deviation is 50 ms.
		 *
		 */
		sv = tcp_sd_min;
	}
	 ...
	 */
	rto = (sa >> 3) + sv + tcps->tcps_rexmit_interval_extra;
...
tcp_icmp_input(void *arg1, mblk_t *mp, void *arg2, ip_recv_attr_t *ira)
{
	 ...
	 * close of this tcp which would be disastrous for tcp_refcnt.
	 * tcp_close_detached / tcp_clean_death / tcp_closei_local must
	 * be called at most once on a TCP. In this case we can just
	 * drop the packet.
	 */
	if (tcp->tcp_state == TCPS_CLOSED || tcp->tcp_state == TCPS_BOUND) {
		freemsg(mp);
		return;
	}

...
tcp_wput_data(tcp_t *tcp, mblk_t *mp, boolean_t urgent)
{
	...
	if (tcpstate < TCPS_SYN_SENT || tcpstate > TCPS_CLOSE_WAIT ||
	    ...
	    (tcpstate == TCPS_SYN_SENT && connp->conn_passive_connect)) {
...
data_null:
	...
	if ((tcp->tcp_suna == snxt) && !tcp->tcp_localnet &&
		...
		TCP_CONG_AFTER_IDLE(tcp);
		DTRACE_PROBE3(tcp_cwnd_update, tcp_t *, tcp,
		    uint32_t, tcp->tcp_cwnd, int, TCPCONG_D_CWND_AFTER_IDLE);
	}
...
tcp_output(void *arg, mblk_t *mp, void *arg2, ip_recv_attr_t *dummy)
{
	...
	db = mp1->b_datap;
	if (((db->db_ref != 2) || rptr < db->db_base + tcps->tcps_wroff_xtra) ||
	    (!OK_32PTR(rptr))) {
	...
	/* Need to update the not-sack'ed list after sending new data. */
	if (tcp->tcp_snd_sack_ok && tcp->tcp_notsack_list != NULL) {
		tcp->tcp_pipe += len;
		tcp_notsack_update(&(tcp->tcp_notsack_list), snxt,
		    tcp->tcp_snxt, &(tcp->tcp_num_notsack_blk),
		    &(tcp->tcp_cnt_notsack_list));
	}
...
tcp_close_output(void *arg, mblk_t *mp, void *arg2, ip_recv_attr_t *dummy)
{
	...
	case TCPS_LISTEN:
		break;
	case TCPS_SYN_SENT:
		/* Don't sent out a RST if this is a passive connect. */
	...
	for (;;) {
		...
		*tail_unsent -= len;
		...
		if (!OK_32PTR(rptr) ||
		    ...
		    rptr < db->db_base + tcps->tcps_wroff_xtra) {
			/* NOTE: we assume allocb returns an OK_32PTR */
	...
	while (usable_swnd > 0) {
		...
		if (notsack_blk == NULL) {
			...
			if (usable_swnd <= 0 || tcp->tcp_unsent == 0) {
				tcp->tcp_cwnd = TCP_NOT_ACKED(tcp);
			...
			DTRACE_PROBE3(tcp_cwnd_update, tcp_t *, tcp,
			    ...
			    int, TCPCONG_D_CWND_SACK_REXMIT);
			return;
		}

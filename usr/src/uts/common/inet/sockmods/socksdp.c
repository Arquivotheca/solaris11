 ...
 * Socket upcalls
 */
static void *sdp_sock_newconn(sock_upper_handle_t parenthandle, void *connind);
static void sdp_sock_connected(sock_upper_handle_t handle);
static void sdp_sock_disconnected(sock_upper_handle_t handle, int error);
static void sdp_sock_connfail(sock_upper_handle_t handle, int error);
static int sdp_sock_recv(sock_upper_handle_t handle, mblk_t *mp, int flags);
static void sdp_sock_xmitted(sock_upper_handle_t handle, int txqueued);
static void sdp_sock_urgdata(sock_upper_handle_t handle);
static void sdp_sock_ordrel(sock_upper_handle_t handle);
...
sosdp_init(struct sonode *so, struct sonode *pso, struct cred *cr, int flags)
{
	...
	so->so_proto_handle = (sock_lower_handle_t)sdp_create(
	    (sock_upper_handle_t)so, NULL, so->so_family, SDP_CAN_BLOCK,
	    upcalls, cr, &error);
    ...
    struct cred *cr)
{
	 ...
	 * connected) then SS_CANTRCVMORE is set and we return the EOF.
	 */
 ...
 */
static void *
sdp_sock_newconn(sock_upper_handle_t parenthandle, void *connind)
{
	 ...
	 */
	if (lso->so_acceptq_len >= lso->so_backlog) {
		return (NULL);
	}
	...
	mutex_enter(&lso->so_acceptq_lock);
	nso->so_listener = lso;
	lso->so_acceptq_len++;
	list_insert_tail(&lso->so_acceptq_list, nso);
	cv_signal(&lso->so_acceptq_cv);
	mutex_exit(&lso->so_acceptq_lock);
 ...
 */
static void
sdp_sock_connected(sock_upper_handle_t handle)
{
	struct sonode *so = (struct sonode *)handle;
 ...
 */
static void
sdp_sock_disconnected(sock_upper_handle_t handle, int error)
{
	struct sonode *so = (struct sonode *)handle;
...
/*ARGSUSED*/
static int
sdp_sock_recv(sock_upper_handle_t handle, mblk_t *mp, int flags)
{
	struct sonode *so = (struct sonode *)handle;
 ...
 */
static void
sdp_sock_xmitted(sock_upper_handle_t handle, int writeable)
{
	struct sonode *so = (struct sonode *)handle;
 ...
 */
static void
sdp_sock_urgdata(sock_upper_handle_t handle)
{
	struct sonode *so = (struct sonode *)handle;
 ...
 */
static void
sdp_sock_ordrel(sock_upper_handle_t handle)
{
	struct sonode *so = (struct sonode *)handle;
...
static void
sdp_sock_connfail(sock_upper_handle_t handle, int error)
{
	struct sonode *so = (struct sonode *)handle;

...
sendalert:
	*decrmp = ssl->alert_sendbuf;
	...
	mutex_exit(&ssl->kssl_lock);
	if (ssl->sendalert_level == alert_fatal)
		return (KSSL_CMD_SEND_FATAL);
	else
	...
	*decrmp = NULL;
	...
	if (content_type == content_handshake_v2) {
		if (ssl->hs_waitstate == wait_client_hello) {
			...
			/* We don't support "pure" SSLv2 */
				...
				/* MSB follows LSB in the packet */
				ssl->major_version = mp->b_rptr[4];
				ssl->minor_version = mp->b_rptr[3];
	...
	case content_change_cipher_spec:
		...
		if (ssl->hs_waitstate != wait_change_cipher) {
			...
			mp->b_rptr = real_recend;
			DTRACE_PROBE2(kssl__state__change, ssl_t *, ssl,
			    SSL3WaitState, wait_finished);
    ...
    kssl_ctx_t *kssl_ctxp)
{
	...
	if (sin->sin_family == AF_INET) {
		...
		ssl->fport = sin->sin_port;
	} else {
		/* struct assignment */
		ssl->faddr = ((struct sockaddr_in6 *)addr)->sin6_addr;
		ssl->fport = ((struct sockaddr_in6 *)addr)->sin6_port;
	}
	ssl->tcp_mss = mss;
...
/*
 * Build handshake_failure fatal alert record and return pointer to the
 * read-to-send mblk.
 */
mblk_t *
kssl_build_alert_fatal(kssl_ctx_t ctx)
{
	mblk_t *mp;
	ssl_t *ssl = (ssl_t *)ctx;

	mp = kssl_prepare_alert(ssl, alert_fatal, handshake_failure);
 ...
 * Takes a copy of the message before encrypting it if it has another reference.
 * In case of failure, NULL is returned, and the message will be freed by the
 * caller.
...
kssl_build_record(kssl_ctx_t ctx, mblk_t *mp)
{
	...
	/* Note that we might be called with zeroed ssl_t structure. */
	ASSERT(ssl != NULL);

	mutex_enter(&ssl->kssl_lock);

	/* Trying to send data after close_notify was sent. */
	if (mp != NULL && ssl->close_notify_srvr) {
		KSSL_COUNTER(internal_errors, 1);
		DTRACE_PROBE(kssl_err__data_after_close_notify);
		mutex_exit(&ssl->kssl_lock);
		return (NULL);
	}
	 ...
	 * or shutdown() followed by close().
	 */
	if (mp == NULL) {
		/*
		 * close_notify should only be sent after the initial
		 * handshake has been finalized. This also serves as a guard
		 * for the checks below to protect them from accessing
		 * zeroed out ssl structure.
		 */
		if (ssl->hs_waitstate != idle_handshake) {
			DTRACE_PROBE1(kssl_close_before_handshake_done,
			    ssl_t *, ssl);
			mutex_exit(&ssl->kssl_lock);
			return (NULL);
		}

		/* Only SSLv3/TLS have close_notify alert. */
		if (ssl->major_version != 0x03) {
			mutex_exit(&ssl->kssl_lock);
			return (NULL);
		}

		/* close_notify was already sent. */
		if (ssl->close_notify_srvr) {
			DTRACE_PROBE1(kssl_duplicate_close_notify,
			    ssl_t *, ssl);
			mutex_exit(&ssl->kssl_lock);
			return (NULL);
		}
		...
		DTRACE_PROBE1(kssl_sending_close_notify, ssl_t *, ssl);
	}

	/* Drop the mutex so that incoming records can be processed. */
	mutex_exit(&ssl->kssl_lock);
 ...
 *
 * The function assumes it is called with ssl->kssl_lock held.
 */
static kssl_status_t
kssl_build_single_record(ssl_t *ssl, mblk_t *mp)
{
	int len; /* length of data not including header, padding and MAC */
	int reclen; /* record length as present on the wire in SSL header */
	...
	/* Only initialized structures should reach us. */
	ASSERT(ssl->major_version != 0);
	...
	/*
	 * kssl_send_alert() which sets the close_notify_srvr flag constructs
	 * the SSL header by itself so the header size needs to be subtracted.
	 */
	if (ssl->close_notify_srvr)
	...
	DTRACE_PROBE4(kssl_lengths, int, reclen, int, len, int, mac_sz,
	    int, pad_sz);
...
void
kssl_kstat_premature_bump(int64_t delta)
{
	KSSL_COUNTER(premature_close, delta);
}

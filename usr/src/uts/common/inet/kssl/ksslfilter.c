...
typedef struct ksslf {
	boolean_t	ksslf_pending;		/* !received 1st data chunk */
	...
	boolean_t	ksslf_accepted;		/* past accept() */
} ksslf_t;
	...
	*lenp = 0;
	do {
		...
		case KSSL_CMD_SEND: {
			...
			bzero(&msg, sizeof (msg));
			ret_out = sof_inject_data_out(handle, outmp, &msg,
			    &flowctrld);
			/*
			 * We (server side) sent a fatal alert during handshake
			 * phase or failed to send handshake message.
			 * Terminate the connection.
			 */
			if (kssl->ksslf_inhandshake && ((ret_out != 0) ||
			    (kssl_cmd == KSSL_CMD_SEND_FATAL))) {
				/*
				 * sof_inject_data_out() has free'd the outmp
				 * for us in case of failure.
				 */
		...
		case KSSL_CMD_NOT_SUPPORTED: {
			...
			if (kssl->ksslf_pending) {
				...
				/*
				 * Move the connection to the fallback point.
				 */
				if (fallback != NULL &&
					...
					/*
					 * No fallback: send fatal alert and
					 * terminate the connection.
					 */
					outmp = kssl_build_alert_fatal(
					    kssl->ksslf_ctx);
					if (outmp != NULL) {
						bzero(&msg, sizeof (msg));
						(void) sof_inject_data_out(
						    handle, outmp, &msg,
						    &flowctrld);
					}
...
sof_rval_t
kssl_accept_cb(sof_handle_t handle, void *cookie, cred_t *cr)
{
	ksslf_t *kssl = (ksslf_t *)cookie;
	...
	/* From now on it is up to the application to close() the descriptor. */
	kssl->ksslf_accepted = B_TRUE;

	return (SOF_RVAL_CONTINUE);
}

void
kssl_notify_cb(sof_handle_t handle, void *cookie, sof_event_t event,
    uintptr_t arg)
{
	ksslf_t *kssl = (ksslf_t *)cookie;
	mblk_t *outmp;
	...
	switch (event) {
		/*
		 * Ideally the application should call shutdown() after it is
		 * done writing data but in case it calls just close() attempt
		 * to send close_notify just like in the shutdown() case.
		 */
		case SOF_EV_CLOSING:
			/* Check if it is okay to inject data. */
			if ((boolean_t)arg != B_TRUE)
				return;

			/* return if no record was built. */
			if ((outmp = kssl_build_record(kssl->ksslf_ctx, NULL))
			    == NULL)
				return;
			...
			break;
		/*
		 * If the TCP connection is closed before or during initial
		 * SSL handshake perform cleanup of the connection gracefully.
		 */
		case SOF_EV_CANTRECVMORE:
		case SOF_EV_DISCONNECTED:
			/* Make sure we are not past accept(). */
			if (!kssl->ksslf_accepted) {
				/*
				 * Cleanup of KSSL structures will be done via
				 * socket filter path.
				 */
				DTRACE_PROBE1(kssl_premature_close,
				    kssl_ctx_t, kssl->ksslf_ctx);
				sof_newconn_close(handle);
				kssl_kstat_premature_bump(1);
			}
			break;
		default:
			break;
...
sof_ops_t ksslf_ops = {
	...
	.sofop_accept = kssl_accept_cb,
	.sofop_connect = kssl_connect_cb,
	.sofop_notify = kssl_notify_cb,

...
suite_found:
	...
	if (ssl->sid.cached == B_TRUE) {
		...
		DTRACE_PROBE2(kssl__state__change, ssl_t *, ssl,
		    SSL3WaitState, wait_change_cipher);
...
/*
 * Produce SSL alert record and set members of ssl accordingly.
 * The alert message is saved into ssl->alert_sendbuf.
 */
void
kssl_send_alert(ssl_t *ssl, SSL3AlertLevel level, SSL3AlertDescription desc)
{
	...
	ssl->alert_sendbuf = kssl_prepare_alert(ssl, level, desc);
}

/*
 * Produce SSL alert record (SSLv3/TLS) or error record (SSLv2) and make all
 * preparations for it to be sent out. For SSLv2, it is only done to tear down
 * the SSL connection so it has fixed encoding.
 *
 * This function does not set any direct members of ssl structure.
 */
mblk_t *
kssl_prepare_alert(ssl_t *ssl, SSL3AlertLevel level, SSL3AlertDescription desc)
{
	mblk_t *mp;
	uchar_t *buf;
	size_t len;
	...
	mp = allocb(len + spec->mac_hashsz + spec->cipher_bsize, BPRI_HI);
	if (mp == NULL) {
		KSSL_COUNTER(alloc_fails, 1);
		return (NULL);
	}
    ...
    kssl_callback_t cbfn, void *arg)
{
	...
	pms_data->cd_raw.iov_base =
	    (char *)wrapped_pms_data->cd_raw.iov_base + msglen;
	...
	case CRYPTO_QUEUED:
		...
		DTRACE_PROBE2(kssl__state__change, ssl_t *, ssl,
		    SSL3WaitState, wait_client_key_done);
...
/* Re-set the tail by going from the head. */
#define	TAIL_ADJUST(ssl)						\
	if (((ssl)->rec_ass_tail = (ssl)->rec_ass_head) != NULL) {	\
		while ((ssl)->rec_ass_tail->b_cont != NULL) {		\
			(ssl)->rec_ass_tail =				\
			    (ssl)->rec_ass_tail->b_cont; 		\
		}							\
	}
	...
	if (MBLKL(mp) < rhsz) {
		...
		/* pullupmsg() might have freed the tail, re-set the pointer. */
		TAIL_ADJUST(ssl);
	}

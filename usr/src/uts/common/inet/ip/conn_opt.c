	...
	case SOL_SOCKET:
		...
		case SO_DOMAIN:
			*i1 = connp->conn_family;
			break;
		case SO_REUSEPORT:
			*i1 = connp->conn_reuseport ? SO_REUSEPORT : 0;
			break;  /* goto sizeof (int) option return */
		case SO_PASSIVE_CONNECT:
			*i1 = connp->conn_passive_connect ?
			    SO_PASSIVE_CONNECT : 0;
			break;  /* goto sizeof (int) option return */
		default:
			return (-1);
		}
    ...
    uchar_t *invalp, boolean_t checkonly, cred_t *cr)
{
	...
	case SO_REUSEPORT:
		...
		break;
	case SO_PASSIVE_CONNECT:
		connp->conn_passive_connect = onoff;
		break;
	}
...
static uint32_t	cksum_massage_options_v4(ipha_t *, netstack_t *, boolean_t);
 ...
 */
static uint32_t
cksum_massage_options_v4(ipha_t *ipha, netstack_t *ns, boolean_t overwrite)
{
	in_addr_t	dst;
	uint32_t	cksum;

	/* Get last hop then diff against first hop */
	cksum = ip_massage_options(ipha, ns, overwrite);
    ...
    const in6_addr_t *dst, in6_addr_t *nexthop)
{
	...
	if (ixa->ixa_flags & IXAF_IS_IPV4) {
		...
		v4nexthop = ip_pkt_source_route_v4(ipp, ixa->ixa_ipst);
...
conn_connect(conn_t *connp, iulp_t *uinfo, uint32_t flags)
{
	...
	if ((flags & IPDF_ZCOPY) &&
		...
		ixa->ixa_flags |= IXAF_ZCOPY_CAPAB;
		ixa->ixa_xmit_hint = ixa->ixa_nce->nce_ill->ill_xmit_hint++;
	}
	return (0);
}
...
conn_inherit_parent(conn_t *lconnp, conn_t *econnp)
{
	...
	/*
	 * Preserve fields that are unique to the 'eager' by stashing
	 * them away and then restoring them after doing ixa_safe_copy().
	 */
	notify_cookie = econnp->conn_ixa->ixa_notify_cookie;
	xmit_hint = econnp->conn_ixa->ixa_xmit_hint;
	ixa_sqp = econnp->conn_ixa->ixa_sqp;
	...
	econnp->conn_ixa->ixa_sqp = ixa_sqp;

 ...
 */
void
ip_input_v6(ill_t *ill, void *arg, mblk_t *mp_chain,
    struct mac_header_info_s *mhip)
{
	(void) ip_input_common_v6(ill, arg, mp_chain, mhip, NULL, NULL,
	    NULL);
}
 ...
 */
mblk_t *
ip_input_common_v6(ill_t *ill, void *arg, mblk_t *mp_chain,
 ...
 */
static boolean_t
ip6_bad_address(in6_addr_t *addr, boolean_t is_src, boolean_t loopback_copy)
{
	if (IN6_IS_ADDR_V4MAPPED(addr)) {
		ip1dbg(("ip_input_v6: pkt with IPv4-mapped addr"));
		return (B_TRUE);
	}
	if (!loopback_copy && IN6_IS_ADDR_LOOPBACK(addr)) {
...
#define	rptr	((uchar_t *)ip6h)
	...
	if (ip6h->ip6_dst.s6_addr32[0] == 0 &&
		...
		if (ip6_bad_address(&ip6h->ip6_dst, B_FALSE, B_FALSE)) {
	...
	case IPPROTO_TCP:
		...
		if (iraflags & IRAF_TARGET_SQP) {
			...
			SQUEUE_ENTER_ONE(connp->conn_sqp, mp, connp->conn_recv,
			    ...
			    SQ_NODRAIN : ip_squeue_flag, SQTAG_IP6_TCP_INPUT);
		}
		ira->ira_ill = ill;
		ira->ira_rill = rill;
		return;

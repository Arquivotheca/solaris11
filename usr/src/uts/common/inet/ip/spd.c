...
static char *ipsec_policy_failure_msgs[] = {
	...
	"%s: Dropping inbound %s datagram because it does not match "
	"the policy; Source %s, Destination %s.\n",
...
static ipsec_kstats_t ipsec_kstats_template = {
	{ "esp_stat_in_requests",	KSTAT_DATA_UINT64, 0 },
	{ "esp_stat_in_discards",	KSTAT_DATA_UINT64, 0 },
	{ "esp_stat_lookup_failure",	KSTAT_DATA_UINT64, 0 },
	{ "ah_stat_in_requests",	KSTAT_DATA_UINT64, 0 },
	{ "ah_stat_in_discards",	KSTAT_DATA_UINT64, 0 },
	{ "ah_stat_lookup_failure",	KSTAT_DATA_UINT64, 0 },
	{ "sadb_acquire_maxpackets",	KSTAT_DATA_UINT64, 0 },
	{ "sadb_acquire_qhiwater",	KSTAT_DATA_UINT64, 0 },
};
...
ipsec_kstat_init(ipsec_stack_t *ipss)
{
	ipss->ipsec_ksp = kstat_create_netstack("ip", 0, "ipsec_stat", "net",
	    KSTAT_TYPE_NAMED, NUM_OF_FIELDS(ipsec_kstats_t), 0,
	...
	*ipss->ipsec_kstats = ipsec_kstats_template;
    ...
    const conn_t *connp, ip_xmit_attr_t *ixa)
{
	...
	ASSERT(!(ixa->ixa_flags & IXAF_IPSEC_CLEARFLAGS));
...
ipsec_conn_cache_policy(conn_t *connp, boolean_t isv4)
{
	...
	if (connp->conn_proto == IPPROTO_ICMP ||
		...
		connp->conn_ixa->ixa_flags &= ~IXAF_IPSEC_CLEARFLAGS;
		return (0);
	}
    ...
    netstack_t *ns)
{
	 ...
	 */
	if (alg_type == IPSEC_ALG_ENCR && alg->alg_id == SADB_EALG_NULL)
		return;
	...
	if (alg_type == IPSEC_ALG_ENCR) {
		/*
		 * For backwards compatibility, assume that the IV length
		 * is the same as the data length.
		 */
		if (alg->alg_ivlen == 0)
			alg->alg_ivlen = alg->alg_datalen;
		if ((alg->alg_flags & ~ALG_FLAG_VALID) == 0)
			alg->alg_flags |= ALG_FLAG_CBC;
	}

	/* For auth algs, the ICV length is the data length. */
	if (alg_type == IPSEC_ALG_AUTH) {
		if (alg->alg_icvlen == 0)
			alg->alg_icvlen = alg->alg_datalen;
	}
...
{
	uint_t cm_flags = ALG_FLAG_CCM | ALG_FLAG_GCM | ALG_FLAG_GMAC;
	uint_t all_flags = cm_flags | ALG_FLAG_CBC;

	alg->alg_flags &= ~ALG_FLAG_VALID;

	/*
	 * Check that only a single mode flag is set (EG: ALG_FLAG_GCM).
	 * Check that AES-CCM, AES-GCM and AES-GMAC are defined as
	 * ALG_FLAG_COMBINED and ALG_FLAG_COUNTERMODE.
	 * Check that AES-CBC is NOT defined as ALG_FLAG_COMBINED.
	 */
	if (!ISP2(alg->alg_flags & all_flags))
		return;

	if (alg->alg_flags & cm_flags) {
		if (!(alg->alg_flags & ALG_FLAG_COUNTERMODE))
			return;
		if (!(alg->alg_flags & ALG_FLAG_COMBINED))
			return;
	}
	if ((alg->alg_flags & (ALG_FLAG_COUNTERMODE | ALG_FLAG_COMBINED)) &&
	    !(alg->alg_flags & cm_flags))
		return;
	/*
    ...
    ip_xmit_attr_t *ixa)
{
	...
	/* Make sure we've left nothing dangling... */
	if (ixa->ixa_ipsec_policy != NULL) {
		IPPOL_REFRELE(ixa->ixa_ipsec_policy);
		ixa->ixa_flags &= ~IXAF_IPSEC_CLEARFLAGS;
	}

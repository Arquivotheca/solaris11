    ...
    uint32_t flags, uint_t mac_mode)
{
	...
	if (!(flags & IPDF_SELECT_SRC) && IN6_IS_ADDR_V4MAPPED(src_addrp))
	...
	if (!(ire->ire_flags & (RTF_REJECT|RTF_BLACKHOLE))) {
		/* Get an nce to cache. */
		nce = ire_to_nce(ire, NULL, firsthop, ixa->ixa_ifindex);
		if (nce == NULL) {
			/* Allocation failure? */
...
ip_source_routed_v6(ip6_t *ip6h, mblk_t *mp, ip_stack_t *ipst)
{
	...
	/* if a routing hdr is preceded by HOPOPT or DSTOPT */

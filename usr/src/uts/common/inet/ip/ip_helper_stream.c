...
ip_helper_share_conn(queue_t *q, mblk_t *mp, cred_t *crp)
{
	...
	helper = *((ip_helper_stream_info_t **)mp->b_cont->b_rptr);
	connp = helper->iphs_conn;
	ASSERT(connp != NULL);

	mutex_enter(&connp->conn_lock);
	if (connp->conn_helper_info != NULL) {
		mutex_exit(&connp->conn_lock);
		miocnak(q, mp, 0, EALREADY);
		return;
	}
...
ip_create_helper_stream(conn_t *connp, ldi_ident_t li)
{
	...
	error = 0;
	helper = kmem_zalloc(sizeof (ip_helper_stream_info_t), KM_SLEEP);
	...
	helper->iphs_conn = connp;

	/*
	 * Share connp with the helper stream. SIOCSQPTR handler will hold the
	 * conn_lock, so we don't grab the lock here.
	 */
	error = ldi_ioctl(helper->iphs_handle, SIOCSQPTR, (intptr_t)helper,
	    FKIOCTL, kcred, &ret);

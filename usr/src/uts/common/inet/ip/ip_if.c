...
static int	ipif_set_values(queue_t *q, mblk_t *mp, int ipi_cmd,
    char *interf_name, uint_t *ppa);
static int	ipif_set_values_restart(ipif_t *ipif, queue_t *q, mblk_t *mp,
    int ipi_cmd, char *ifname, uint_t *ppa);
...
static void	ill_dlpi_clear_deferred(ill_t *);

static int	ill_alloc_frag_hash_tbl(ill_t *, size_t);
static void	ill_free_frag_hash_tbl(ill_t *);
...
ipsq_xopq_mp_cleanup(ill_t *ill, conn_t *connp)
{
	...
	for (prev = NULL, curr = ipsq->ipsq_xopq_mphead; curr != NULL;
		...
		if (connp == NULL ||
			...
			/*
			 * Do not flush ioctl mp's queued by the other ill.
			 */
			if (connp == NULL && curr->b_queue->q_next != NULL &&
			    curr->b_queue->q_ptr != ill) {
				prev = curr;
				continue;
			}
			if (curr->b_queue->q_next == NULL) {
				/*
				 * ioctls from connp on the other ill should
				 * not be flushed.
				 */
				connp = (conn_t *)(curr->b_queue->q_ptr);
				isv6 = (connp->conn_family == AF_INET6);
				if (isv6 != ill->ill_isv6) {
					prev = curr;
					continue;
				}
			}
...
ill_capability_direct_enable(ill_t *ill)
{
	...
	if (rc == 0) {
		...
		idd->idd_tx_pmh = (pm_handle_t)direct.di_tx_pmh;
		...
		ill->ill_flownotify_mh = idd->idd_tx_cb_df(idd->idd_tx_cb_dh,
		    ill_flowctl_enable, ill);
		ill->ill_capabilities |= ILL_CAPAB_DLD_DIRECT;
		ill->ill_pmh = idd->idd_tx_pmh;
...
ill_capability_poll_enable(ill_t *ill)
{
	...
	poll.poll_ring_add_cf = (uintptr_t)ip_gld_squeue_add;
	poll.poll_ring_remove_cf = (uintptr_t)ip_gld_squeue_remove;
...
ill_taskq_dispatch(ip_stack_t *ipst)
{
	...
	ipst->ips_capab_taskq_flags &= ~IPS_ILL_TASKQ_DISP_DONE;
	for (;;) {
		...
		if (ipst->ips_capab_taskq_flags & IPS_CAPAB_TASKQ_QUIT)
			break;
...
static int
ill_alloc_frag_hash_tbl(ill_t *ill, size_t extra_size)
{
	int	count;
	 ...
	 * the device name.
	 */
	ill->ill_frag_ptr = (uchar_t *)mi_zalloc(ILL_FRAG_HASH_TBL_SIZE +
	    extra_size);
	if (ill->ill_frag_ptr == NULL)
		return (ENOMEM);
	ill->ill_frag_free_num_pkts = 0;
	ill->ill_last_frag_clean_time = 0;
	ill->ill_frag_hash_tbl = (ipfb_t *)(ill->ill_frag_ptr);
...
static void
ill_free_frag_hash_tbl(ill_t *ill)
{
	int	count;
...
ill_init(queue_t *q, ill_t *ill)
{
	 ...
	 * the device name.
	 */
	if ((err = ill_alloc_frag_hash_tbl(ill, 2 * LIFNAMSIZ)) != 0) {
		freemsg(info_mp);
		return (err);
	}
	ill->ill_name = (char *)(ill->ill_frag_ptr + ILL_FRAG_HASH_TBL_SIZE);
    ...
    boolean_t *did_alloc, ip_stack_t *ipst)
{
	...
	*ill = ill_null;
	...
	/*
	 * Allocate the fragment hash table
	 */
	if (ill_alloc_frag_hash_tbl(ill, 0) != 0)
		goto done;
    ...
    zoneid_t zoneid, ip_stack_t *ipst)
{
	...
	boolean_t no_duplicate = (match_flags & IPIF_MATCH_UP);
 ...
 * except that we will only return an address if it is IPIF_UP
 */
ipif_t *
ipif_lookup_up_addr(ipaddr_t addr, ill_t *match_ill, zoneid_t zoneid,
    ip_stack_t *ipst)
{
	return (ipif_lookup_addr_common(addr, match_ill,
	    (IPIF_MATCH_ILLGRP | IPIF_MATCH_UP), zoneid, ipst));
}
...
ip_sioctl_getsetprop(queue_t *q, mblk_t *mp)
{
	...
	case MOD_PROTO_TCP:
		...
		cong = &tcps->tcps_cong_enabled;
		cong_lock = &tcps->tcps_cong_lock;
		cbarg = tcps;
		break;
	case MOD_PROTO_UDP:
	...
	case MOD_PROTO_SCTP:
		...
		cong = &sctps->sctps_cong_enabled;
		cong_lock = &sctps->sctps_cong_lock;
		cbarg = sctps;
		break;
	default:
		miocnak(q, mp, 0, EINVAL);
	...
	/*
	 * If nothing was found, search private property tables exported
	 * (optionally) by congestion control modules. They are stored
	 * in a linked list in the protocol's stack structure.
	 */
	if (pinfo->mpi_name == NULL && cong != NULL) {
		mutex_enter(cong_lock);
		for (tce = list_head(cong); tce != NULL; tce = list_next(cong,
		    tce)) {
			if ((pinfo = tce->tce_propinfo) == NULL)
				continue;
			for (; pinfo->mpi_name != NULL; pinfo++) {
				if (strcmp(pinfo->mpi_name, pioc->mpr_name) ==
				    0 && pinfo->mpi_proto == pioc->mpr_proto) {
					mutex_exit(cong_lock);
					goto found;
				}
			}
		}
	...
	ASSERT(pinfo == NULL || pinfo->mpi_name == NULL);
...
found:
	...
	if (set && pinfo->mpi_setf != NULL) {
		...
		uint_t	psize = pioc->mpr_valsize;

		err = pinfo->mpi_getf(cbarg, pinfo, pioc->mpr_ifname,
		    pioc->mpr_val, &pioc->mpr_valsize, pioc->mpr_flags);
		if (err == 0 && pioc->mpr_valsize >= psize) {
			/*
			 * buffer provided is insufficient and pioc->mprvalsize
			 * contains the correct size and we need to return that
			 * size.
			 */
			miocnak(q, mp, iocp->ioc_count, ENOBUFS);
			return;
		}
	} else {
		err = EPERM;
...
ip_sioctl_garp_reply(mblk_t *mp, ill_t *ill, void *hwaddr, int flags)
{
	...
	*flagsp = ATF_INUSE;
	/*
	 * If /usr/sbin/arp told us we are the authority using the "permanent"
	 * flag, or if this is one of my addresses print "permanent"
	 * in the /usr/sbin/arp output.
	 */
    ...
    ip_ioctl_cmd_t *dummy_ipip, void *dummy_ifreq)
{
	...
	/*
	 * If the address is not unspecified, verify that the address being
	 * configured is permitted by the ill_allowed_ips[] for the interface.
	 */
	if (((ipif->ipif_isv6 && !IN6_IS_ADDR_UNSPECIFIED(&v6addr)) ||
	    (!ipif->ipif_isv6 && addr != INADDR_ANY)) &&
	    ill->ill_allowed_ips_cnt > 0) {
...
/*
 * Get current interface flags values.
 */
static uint64_t
ip_sioctl_flags_current(ipif_t *ipif)
{
	ill_t *ill = ipif->ipif_ill;
	phyint_t *phyi = ill->ill_phyint;

	return (ipif->ipif_flags | ill->ill_flags | phyi->phyint_flags);
}
    ...
    uint64_t *offp)
{
	...
	intf_flags = ip_sioctl_flags_current(ipif);
...
/*
 * Check whether the initial flags specified in the input `lifreq' structure
 * chosen by `lcr_origmask' match the current flags.
 */
static boolean_t
ip_sioctl_flags_match(ipif_t *ipif, struct lifreq *lifr)
{
	uint64_t orig_mask;
	uint64_t curr_flags;
	lif_cflags_req_t *lcr = &lifr->lifr_cflags;

	curr_flags = ip_sioctl_flags_current(ipif);
	orig_mask = lcr->lcr_origmask & IFF_ALL_FLAGS;
	return (((curr_flags ^ lcr->lcr_origflags) & orig_mask) == 0);
}

/*
 * Get the requested flags value after applying the changes as per the input to
 * the ioctl (SIOCSIFFLAGS, SIOCSLIFFLAGS or SIOCCLIFFLAGS)
 */
static uint64_t
ip_sioctl_flags_requested(ipif_t *ipif, ip_ioctl_cmd_t *ipip, void *if_req)
{
	struct ifreq	*ifr;
	struct lifreq	*lifr;
	uint64_t	flags;
	...
	/*
	 * For SIOCSLIFFLAGS and SIOCSLIFFLAGS, simply return the
	 * flags value in `lifr_flags'. For SIOCCLIFFLAGS, apply the
	 * modifications from `lifr_cflags.lcr_modflags' masked by
	 * `lifr_cflags.lcr_modmask' to the current interface flags
	...
	if (ipip->ipi_cmd_type == IF_CMD) {
		...
		lifr = if_req;
		if (ipip->ipi_cmd == SIOCCLIFFLAGS) {
			lif_cflags_req_t *lcr = &lifr->lifr_cflags;
			uint64_t modflags = lcr->lcr_modflags;
			uint64_t modmask = lcr->lcr_modmask & IFF_ALL_FLAGS;
			uint64_t intf_flags;

			intf_flags = ip_sioctl_flags_current(ipif);
			flags = (intf_flags & ~modmask) | (modflags & modmask);
		} else {
...
/*
 * Set/change interface flags.  Many flags require special handling (e.g.,
    ...
    ip_ioctl_cmd_t *ipip, void *if_req)
{
	...
	if (ipip->ipi_cmd == SIOCCLIFFLAGS &&
	    !ip_sioctl_flags_match(ipif, if_req))
		return (EINVAL);

	flags = ip_sioctl_flags_requested(ipif, ipip, if_req);
	...
	if (!(ipif->ipif_flags & IPIF_UP) &&
		...
		/*
		 * PHYI_FAILED, PHYI_INACTIVE, PHYI_PROBER and PHYI_OFFLINE
		 * are all the same to the kernel: if any of them has been
		 * set by userland, the interface cannot be used for data
		 * traffic.
		 */
		if ((turn_on|turn_off) & (PHYI_FAILED | PHYI_INACTIVE |
		    PHYI_OFFLINE | PHYI_PROBER)) {
			ASSERT(!IS_IPMP(ill));
			/*
...
ip_sioctl_flags_tail(ipif_t *ipif, uint64_t flags, queue_t *q, mblk_t *mp)
{
	...
	/*
	 * PHYI_FAILED, PHYI_INACTIVE, PHYI_PROBER and PHYI_OFFLINE are all
	 * the same to the kernel: if any of them has been set by userland,
	 * the interface cannot be used for data traffic.
	 */
	if ((turn_on|turn_off) &
	    (PHYI_FAILED | PHYI_INACTIVE | PHYI_OFFLINE | PHYI_PROBER)) {
		ASSERT(!IS_IPMP(ill));
		if (IS_UNDER_IPMP(ill) && ((turn_on|turn_off) & PHYI_PROBER)) {
			struct phyint *ipmp_phyi;

			ipmp_phyi = ill->ill_grp->ig_ipmp_ill->ill_phyint;
			if (turn_on & PHYI_PROBER)
				ipmp_phyi->phyint_grp->gr_probing = B_TRUE;
			else
				ipmp_phyi->phyint_grp->gr_probing = B_FALSE;
		}
		/*
    ...
    ip_ioctl_cmd_t *ipi, void *if_req)
{
	 ...
	 * /usr/sbin/dladm) and dynamically discovered MTUs (e.g., those
	 * discovered for tunnels) we conservatively choose the
	 * ill_current_frag as the upper-bound.
	 */
	if (lir->lir_maxmtu != 0 &&
...
ill_dl_down(ill_t *ill)
{
	...
	if (mp != NULL) {
		 ...
		 *
		 * Re-enable all conns blocked due to flow control.
		 */
...
ill_mcast_send_queued(ill_t *ill)
{
	...
	while ((mp = ill->ill_mcast_deferred) != NULL) {
		...
		mutex_enter(&ill->ill_phyint->phyint_lock);
		if (release_ill != NULL && !ipmp_ill_is_active(ill)) {
			/*
...
ipif_add_ires_v4(ipif_t *ipif, boolean_t loopback)
{
	 ...
	 * bunch, since ire_add returns with a referenced ire.
	 */
	for (irep1 = irep; irep1 > ire_array; ) {
		ire_t **tmp;
		...
		*irep1 = ire_add(*irep1);
		if (*irep1 == NULL) {
			err = ENOMEM;
			/*
			 * We need to do both ire_delete() and ire_refrele()
			 * to clean up the ire's that got added thus far. The
			 * rest can be cleaned up by ire_delete() at `bad2'
			...
			DTRACE_PROBE1(ire__add__fail, int, irep1 - ire_array);
			goto bad2;
		}
	}
...
ipif_delete_ires_v4(ipif_t *ipif)
{
	...
	if (ire != NULL) {
		/*
		 * Move count to ipif so we don't lose the count due to
		 * a down/up dance.
		 */
...
static void
if_unitsel_ifname(queue_t *q, uint_t ppa, char *ifname)
{
	do {
		q = q->q_next;
	} while (q->q_next);

	(void) snprintf(ifname, LIFNAMSIZ, "%s%d", Q2NAME(q), ppa);
}
	...
	ip_ioctl_cmd_t *ipip, void *dummy_ifreq)
{
	...
	ppa = *(uint_t *)mp->b_cont->b_cont->b_rptr;
	if_unitsel_ifname(q, ppa, interf_name);

	return (ipif_set_values_restart(ipif, q, mp, IF_UNITSEL, interf_name,
	    &ppa));
}
    ...
    ip_ioctl_cmd_t *ipip, void *dummy_ifreq)
{
	 ...
	 */
	return (ipif_set_values(q, mp, IF_UNITSEL, interf_name, &ppa));
}
...
ipif_create_bcast_ires(ipif_t *ipif, ire_t **irep)
{
	...
	ip1dbg(("ipif_create_bcast_ires(%p %s:%d): creating broadcast IREs\n",
	    (void *)ipif, ipif->ipif_ill->ill_name, ipif->ipif_id));
    ...
    ip_ioctl_cmd_t *ipip, void *if_req)
{
	...
	return (ipif_set_values(q, mp, SIOCSLIFNAME, lifr->lifr_name,
	    &lifr->lifr_ppa));
}
    ...
    ip_ioctl_cmd_t *ipip, void *if_req)
{
	...
	return (ipif_set_values_restart(ipif, q, mp, SIOCSLIFNAME,
	    lifr->lifr_name, &lifr->lifr_ppa));
}
 ...
 */
static int
ipif_set_values(queue_t *q, mblk_t *mp, int ipi_cmd, char *interf_name,
    uint_t *new_ppa_ptr)
{
	ill_t	*ill;
	ipif_t	*ipif;
	...
	char	*old_ptr;
	int	error;
	char	ill_ifname[LIFNAMSIZ];
	ip_stack_t	*ipst;
	phyint_t	*phyi;
	ill_t		*ill_mate = NULL;
	boolean_t	ipsq_started = B_FALSE;
	...
	(void) strlcpy(ill_ifname, interf_name, sizeof (ill_ifname));
	/* The ppa is sent down by ifconfig or is chosen */
	if ((ppa_ptr = ill_get_ppa_ptr(ill_ifname)) == NULL) {
		return (EINVAL);
	}
	...
	/*
	 * We need to become writer on the ipsq of `ill_mate' since we will
	 * be updating phyi->phyint_illv6 later in ill_glist_insert()->
	 * ill_phyint_reinit().
	 */
	ill_refhold(ill);
	rw_enter(&ipst->ips_ill_g_lock, RW_WRITER);
	phyi = avl_find(&ipst->ips_phyint_g_list->phyint_list_avl_by_name,
	    interf_name, NULL);
	if (phyi != NULL) {
		ill_mate = phyi->phyint_illv4;
		if (ill_mate == NULL)
			ill_mate = phyi->phyint_illv6;
		ASSERT(ill_mate != NULL);
		ill_refhold(ill_mate);
		ipsq = ipsq_try_enter_internal(ill_mate, q, mp,
		    ip_reprocess_ioctl, NEW_OP, B_TRUE);
		if (ipsq == NULL) {
			/* Operation enqueued on the ipsq of the other ILL */
			error = EINPROGRESS;
			goto out;
		}
		if (ipsq->ipsq_xop->ipx_current_ipif == NULL) {
			ipsq_current_start(ipsq, ipif, ipi_cmd);
			ipsq_started = B_TRUE;
		} else {
			/*
			 * If we came here by means of restart,
			 * ip_reprocess_ioctl() should have set
			 * ipx_current_ipif by calling
			 * ipsq_current_start().
			 */
	...
	if ((error = ill_glist_insert(ill, ill_ifname,
	...
	if (*new_ppa_ptr == UINT_MAX) {
		...
		*new_ppa_ptr = ill->ill_ppa;
		...
		ASSERT(strncmp(ill->ill_name, interf_name, LIFNAMSIZ) == 0);
	}
	...
	rw_exit(&ipst->ips_ill_g_lock);
	/*
	 * We had to postpone refrele of ill_mate until we dropped
	 * ips_ill_g_lock to avoid recursive mutex panic.
	 */
	if (ill_mate != NULL)
		ill_refrele(ill_mate);
	ill_refrele(ill);
...
/*
 * ipif_set_values() tried to become a writer on the other ill's ipsq
 * while doing an SIOCSLIFNAME or IF_UNITSEL.  We might not have been
 * able to complete the ioctl in ipif_set_values, if we could not become
 * exclusive, in which case it is restarted here.
 */
static int
ipif_set_values_restart(ipif_t *ipif, queue_t *q, mblk_t *mp, int ipi_cmd,
    char *ifname, uint_t *ppa)
{
	int error;
	ipsq_t *ipsq;

	/*
	 * In order to free the ipsq of the current ill after merging
	 * it's phyint with the other ill in ill_phyint_reinit, we need
	 * to become a writer on the ipsq of the current ill once again,
	 * but this should be trivial, since the ill is not globally
	 * visible yet.
	 */
	ipsq = ipsq_try_enter(NULL, ipif->ipif_ill, NULL, NULL, NULL,
	    CUR_OP, B_TRUE);
	ASSERT(ipsq != NULL);
	error = ipif_set_values(q, mp, ipi_cmd, ifname, ppa);
	if (error != 0 && error != EINPROGRESS)
		ipsq_current_finish(ipsq);
	ipsq_exit(ipsq);
	return (error);
...
retry:
		...
		default:
			ASSERT(nce == NULL);
			ip0dbg(("ipif_arp_up_done_tail: got err %d for %s:%d\n",
			    err, ipif->ipif_ill->ill_name, ipif->ipif_id));
			goto arp_up_done;
		}
		if (arp_no_defense) {
			if ((ipif->ipif_flags & IPIF_UP) &&
...
/*
 * Return B_TRUE if `ill' has at least one NOFAILOVER address that is UP.
 */
boolean_t
ill_has_testaddr(ill_t *ill)
{
	ipif_t *ipif;
	...
	ipif = ill->ill_ipif;
	for (ipif = ill->ill_ipif; ipif != NULL; ipif = ipif->ipif_next) {
		if ((ipif->ipif_flags & (IPIF_UP|IPIF_NOFAILOVER)) !=
		    (IPIF_UP|IPIF_NOFAILOVER))
			continue;
		retval = B_TRUE;
		break;
	}

...
ipmp_ill_leave_illgrp(ill_t *ill)
{
	...
	/*
	 * if the interface was the designated prober for the group, reset the
	 * needed state for the IPMP meta-interface
	 */
	if (ill->ill_phyint->phyint_flags & PHYI_PROBER)
		ipmp_ill->ill_phyint->phyint_grp->gr_probing = B_FALSE;
	...
	/*
	 * Clean up any nce_t entries that are still attached to `ill',
	 * leaving out those for the local addresses.
	 * See comments in ipmp_ill_deactivate()
	 */
	nce_flush_nonlocal(ill);
...
ipmp_ill_deactivate(ill_t *ill)
{
	 ...
	 *
	 * Even though `ill' has been deactivated, NCEs associated with probe
	 * traffic may continue to use it -- and thus potentially create NCECs
	 * on the IPMP interface that are associated with that probe traffic.
	 * But, since NCEs e.g., for data traffic may have been created in the
	 * interval between nce_flush() below and the reset of ill_grp in
	 * ipmp_ill_leave_illgrp(), NCEs will need to be flushed again as part
	 * of removing `ill' from the group to release those NCECs references.
	 * If this is not done, references to those NCECs will remain even
	 * after the `ill' has left the group and the IPMP interface associated
	 * with those NCECs will not be able to be unplumbed until some other
	 * operation triggers a flush of the ill's NCEs (if ever).
	 *
	 * Note that the NCEs for the local addresses should not be flushed,
	 * as these are tied to the underlying ill and will not cause creation
	 * of NCECs on the IPMP interface. These NCEs will be deleted when the
	 * last ipif that points to them goes down. Otherwise, we will leave
	 * an ipif which doesn't have a corresponding NCE_F_MYADDR NCE.
	 * Thus, the ipmp group leave code path should let the ipif_nce_* paths
	 * clean up the local NCEs and should not blindly flush them here.
	 */
	nce_flush_nonlocal(ill);
...
ipmp_ill_is_active(ill_t *ill)
{
	 ...
	 *
	 * An interface that has PHYI_PROBER set is the designated prober of
	 * a failed IPMP group when transitive probing is used, and is
	 * considered active so that data addresses can be bound to the PROBER
	 * and used as source address for IPMP probes. Note that data packets
	 * themselves will not be sent out on the failed IPMP group due to
	 * the checks in ire_to_nce()
	 */
	return (!(ill->ill_ipif_up_count == 0 ||
	    ((phyi->phyint_flags & PHYI_PROBER) == 0 &&
...
ipmp_ncec_refresh_nce(ncec_t *ncec)
{
	...
	/*
	 * purge the ones on the inactive/failed/standby interfaces
	 */

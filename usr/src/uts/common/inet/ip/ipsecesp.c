...
static esp_kstats_t esp_kstats_template = {
	...
	{ "num_ealgs",			KSTAT_DATA_UINT64, 0 },
	{ "bad_decrypt",		KSTAT_DATA_UINT64, 0 },
	{ "sa_port_renumbers",		KSTAT_DATA_UINT64, 0 },
};
...
esp_kstat_init(ipsecesp_stack_t *espstack, netstackid_t stackid)
{
	espstack->esp_ksp = kstat_create_netstack("ipsecesp", 0, "esp_stat",
	    "net", KSTAT_TYPE_NAMED, NUM_OF_FIELDS(esp_kstats_t), 0,
	    stackid);
	...
	*espstack->esp_kstats = esp_kstats_template;
...
esp_in_done(mblk_t *data_mp, ip_recv_attr_t *ira, ipsec_crypto_t *ic)
{
	...
	esph = (esph_t *)(data_mp->b_rptr + ic->ic_espstart);
	...
	if (!esp_age_bytes(assoc, ic->ic_processed_len, B_TRUE)) {
	...
	if (esp_strip_header(data_mp, (ira->ira_flags & IRAF_IS_IPV4),
	    assoc->ipsa_iv_len, &counter, espstack)) {
    ...
    ipsa_t *assoc, uint_t esph_offset)
{
	...
	if (do_auth) {
		...
		if (!do_encr) {
			...
			ic->ic_espstart = ic->ic_crypto_data.cd_offset;
			ic->ic_processed_len = ic->ic_crypto_data.cd_length;
	...
	if (do_encr) {
		...
		(assoc->ipsa_noncefunc)(assoc, encr_len, ic,
		    esp_mp, esph_offset, B_FALSE);
    ...
    uchar_t *icv_buf, uint_t payload_len)
{
	...
	if (do_encr) {
		...
		(assoc->ipsa_noncefunc)(assoc, payload_len, ic,
		    data_mp, esph_offset, B_TRUE);

		/* call the crypto framework */
		if (do_auth) {
			kef_rc = crypto_encrypt_mac(&assoc->ipsa_emech,
			    &assoc->ipsa_amech, NULL, &assoc->ipsa_kcfencrkey,
			    &assoc->ipsa_kcfauthkey, encr_ctx_tmpl,
			    auth_ctx_tmpl, &ic->ic_crypto_dual_data,
			    &ic->ic_crypto_mac, callrp);
...
esp_outbound(mblk_t *data_mp, ip_xmit_attr_t *ixa)
{
	...
	if (!update_iv((uint8_t *)iv_ptr, assoc, espstack, NULL)) {
		ip_drop_packet(data_mp, B_FALSE, ill,

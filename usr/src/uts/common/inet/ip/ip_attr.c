...
ip_recv_attr_to_mblk(ip_recv_attr_t *ira)
{
	...
	irm->irm_xmit_hint = ira->ira_xmit_hint;
...
ip_recv_attr_from_mblk(mblk_t *iramp, ip_recv_attr_t *ira)
{
	...
	ira->ira_xmit_hint = irm->irm_xmit_hint;
...
#endif
	/* Clear all the IPsec pointers and the flags as well. */
	ixa->ixa_flags &= ~IXAF_IPSEC_CLEARFLAGS;
...
ixa_cleanup(ip_xmit_attr_t *ixa)
{
	...
	/*
	 * The 'refrele' can recurse through ipif_ill_refrele_tail and
	 * and enter or reenter the ipsq, potentially triggering an ipcl_walk
	 * and callback and hit the ire/nce/dce which has already been
	 * refrele'd. So we need to first set the ixa_ire/nce/dce to NULL
	 * before doing the refrele.
	 */
	if (ixa->ixa_ire != NULL) {
		ire = ixa->ixa_ire;
		ixa->ixa_ire = NULL;
    ...
    ip_recv_attr_t *dummy)
{
	...
	/*
	 * Cleanup any cached conn_pmh or so_pmh now along with the
	 * cleanup of the cached ire/nce/dce pointers.
	 */
	nce = connp->conn_ixa->ixa_nce;
	if (nce == NULL || nce->nce_is_condemned)
		conn_change_pmh(NULL, connp);
	...
	if (mp != NULL) {
		ASSERT(connp->conn_sq_cleanup_mp == NULL);
		mutex_enter(&connp->conn_lock);
		connp->conn_sq_cleanup_mp = mp;
		cv_signal(&connp->conn_sq_cleanup_cv);
		mutex_exit(&connp->conn_lock);
	}
}
...
conn_ixa_cleanup(conn_t *connp, void *arg)
{
	...
	if (IPCL_IS_TCP(connp)) {
		...
		if (connp->conn_sqp->sq_run == curthread) {
			/* Already on squeue */
			tcp_ixa_cleanup(connp, NULL, NULL, NULL);
		} else {
			mutex_enter(&connp->conn_lock);
			mp = connp->conn_sq_cleanup_mp;
			if (mp == NULL && !tryhard) {
				mutex_exit(&connp->conn_lock);
				return;
			}
			while ((mp = connp->conn_sq_cleanup_mp) == NULL)
				cv_wait(&connp->conn_sq_cleanup_cv,
				    &connp->conn_lock);
			connp->conn_sq_cleanup_mp = NULL;
			CONN_INC_REF_LOCKED(connp);
			mutex_exit(&connp->conn_lock);
			/*
			 * We don't wait for the cleanup to complete.
			 * We rely on the unplumb path waiting for the
			 * appropriate ire/nce ref counts to drop to
			 * quiescent values which in turn can happen only
			 * after the conn cleanup has completed and the conn
			 * has droppped its ref on the ire/nce.
			 */
			SQUEUE_ENTER_ONE(connp->conn_sqp, mp, tcp_ixa_cleanup,
			    connp, NULL, SQ_FILL, SQTAG_TCP_IXA_CLEANUP);
		}
...
ixa_check_drain_insert(conn_t *connp, ip_xmit_attr_t *ixa)
{
	...
	/*
	 * At unbind time, in ill_dl_down, all flow controlled conns are
	 * re-enabled. We should not re-insert again in any flow control
	 * list subsequently.
	 */
	ASSERT(!(ill->ill_state_flags & ILL_DL_UNBIND_IN_PROGRESS) &&
	    ill->ill_dl_up);
	if (ill->ill_dld_capab != NULL)
		idd = &ill->ill_dld_capab->idc_direct;

	idl_hash = &ill->ill_idl_hash[IDLHASHINDEX(cookie)];
	idlpp = &idl_hash->idh_idl;
	mutex_enter(&idl_hash->idh_lock);
	mutex_enter(&connp->conn_lock);
	/*
	 * If the conn is closing or if it is already in a drain list,
	 * then bail out. Multiple threads sending concurrently on the
	 * same connection can cause the conn to be already present in
	 * a drain list.
	 */
	...
	for (idl = *idlpp; idl != NULL; idl = idl->idl_next) {
		if (idl->idl_cookie == cookie)
			break;
		idlpp = &idl->idl_next;
	}
	if (idl == NULL) {
		idl = kmem_zalloc(sizeof (idl_t), KM_NOSLEEP);
		if (idl == NULL) {
			mutex_exit(&idl_hash->idh_lock);
			mutex_exit(&connp->conn_lock);
			return;
		...
		*idlpp = idl;
		idl->idl_cookie = cookie;
		idl->idl_hashp = idl_hash;
		idl->idl_ill = ill;
		idl->idl_conn_head = connp;
		idl->idl_conn_tail = connp;
	} else {
		/*
		 * Tail insert to maintain fairness when conns are enabled
		 * on flow control relief
		 */
		if (idl->idl_conn_head != NULL) {
			ASSERT(idl->idl_conn_tail != NULL);
			idl->idl_conn_tail->conn_drain_next = connp;
		} else {
			idl->idl_conn_head = connp;
		}
		idl->idl_conn_tail = connp;
	...
	/* Mark the `idl' as blocked */
	idl->idl_ndrainers = 0;

	ill_conn_blocked_cnt_adjust(ill, 1);

	/*
	 * Add ref for being in the drain list. DEC_REF happens in
	 * conn_list_drain. Connection close will wait till ref count
	 * drops to zero.
	 */
	CONN_INC_REF_LOCKED(connp);
	mutex_exit(&connp->conn_lock);
	...
	/*
	 * Since the check for flow control and subsequent enqueue is not
	 * atomic, recheck for flow control now that the connection has been
	 * enqueued on the idl_t list. idd_tx_fctl_df (mac_tx_is_flow_blocked)
	 * indicates whether flow control is still in place. If so relief
	 * will happen subsequently. Else we re-enable the conns right away.
	 */
	if ((cookie != NULL) &&
	    (idd->idd_tx_fctl_df(idd->idd_tx_fctl_dh, cookie) == B_TRUE))
		return;
	else if (cookie == NULL && !canputnext(ill->ill_wq))
		return;

	/*
	 * Re-enable the conns since we are not flow controlled
	 * idl's are deleted only after interface quiesce in the unbind/unplumb
	 * logic. So it is safe to access the idl.
	 */
	mutex_enter(&idl_hash->idh_lock);

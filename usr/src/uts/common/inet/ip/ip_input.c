 ...
 */
void
ip_input(ill_t *ill, void *arg, mblk_t *mp_chain,
    struct mac_header_info_s *mhip)
{
	(void) ip_input_common_v4(ill, arg, mp_chain, mhip, NULL, NULL,
	    NULL);
}
 ...
 */
mblk_t *
ip_input_common_v4(ill_t *ill, void *arg, mblk_t *mp_chain,
    ...
    mblk_t **last, uint_t *cnt)
{
	...
	if (mras != NULL) {
		iras.ira_sqp = mras->ra_sqp;
		iras.ira_xmit_hint = mras->ra_xmit_hint;
	} else {
		iras.ira_sqp = NULL;
		iras.ira_xmit_hint = (uint32_t)-1;
	}
	...
	for (mp = mp_chain; mp != NULL; mp = mp_chain) {
		 ...
		 */
		if ((dst = ipha->ipha_dst) == INADDR_ANY) {
		 ...
		 */
		(*ill->ill_inputfn)(mp, ipha, &dst, &iras, &rtc);
...
#define	rptr	((uchar_t *)ipha)
	 ...
	 * Source address can be loopback address when multicast is being
	 * looped back
	 */
	if (((*(uchar_t *)&ipha->ipha_dst) == IN_LOOPBACKNET) ||
	    (!(ira->ira_flags & IAF_LOOPBACK_COPY) &&
...
after_ilb:
	...
	if (ira->ira_flags & IRAF_SYSTEM_LABELED) {
		...
		boolean_t ire_reused;

		ire_reused = B_FALSE;
		/* Update the route cache */
		if (CLASSD(nexthop)) {
			ire = ire_multicast_ext(ill, rtc->rtc_ire, &ire_reused);
		} else {
			/* Just match the destination */
			ire = ire_route_recursive_dstonly_v4(nexthop,
			    irr_flags, ira->ira_xmit_hint, ipst,
			    rtc->rtc_ire, &ire_reused);
		}

		/*
		 * No need to call ire_refrele() if we are using the same ire.
		 * ire_route_recursive_dstonly_v4()/ire_multicast_ext() will
		 * set the reused flag and skip incrementing the ire_refcnt
		 * for this case.
		 */
		if (rtc->rtc_ire != NULL && !ire_reused)
			ire_refrele(rtc->rtc_ire);
 ...
 *
 * This routine optimizes for the case where all or a sub-chain of the packets
 * in the packet chain being forwarded, use the same nce. We hold the nce once
 * for the first packet in the chain/sub-chain and skip holding it for the rest.
 *
 * The caller sets ira->ira_old_nce to NULL once before the loop
 * (looping over the chain of packets) that calls this routine and releases
 * the nce in ira->ira_old_nce after the loop.
 */
void
ire_recv_forward_v4(ire_t *ire, mblk_t *mp, void *iph_arg, ip_recv_attr_t *ira)
{
	...
	if (nce == NULL || (nce != ira->ira_old_nce)) {
		if (nce == NULL) {
			/* Not yet set up - try to set one up */
			(void) ire_revalidate_nce(ire);
		}
		if (ira->ira_old_nce != NULL) {
			nce_refrele(ira->ira_old_nce);
			ira->ira_old_nce = NULL;
		}
	...
	/* The caller will release this hold */
	ira->ira_old_nce = nce;
}
...
ip_input_local_v4(ire_t *ire, mblk_t *mp, ipha_t *ipha, ip_recv_attr_t *ira)
{
	...
	if (ipha->ipha_fragment_offset_and_flags &
		...
		/*
		 * Make sure we have ira_l2src before we lose the original
		 * mblk
		 */
...
#define	rptr	((uchar_t *)ipha)
	...
	case IPPROTO_TCP:
		...
		break;
	case IPPROTO_SCTP:
		min_ulp_header_length = SCTP_MIN_HDR_LENGTH;
		break;
	case IPPROTO_UDP:
	...
	case IPPROTO_TCP:
		...
		if (offset != TCP_MIN_HEADER_LENGTH_IN_WORDS) {
			if (offset < TCP_MIN_HEADER_LENGTH_IN_WORDS)
				goto discard;
		...
		if (iraflags & IRAF_TARGET_SQP) {
			...
			/*
			 * Block comment in squeue.c explains the rationale
			 * for the selection of the SQ_NODRAIN versus
			 * ip_squeue_flag below.
			 */
			SQUEUE_ENTER_ONE(connp->conn_sqp, mp, connp->conn_recv,
			    connp, ira, (ira->ira_flags & IRAF_LOOPBACK) ?
			    SQ_NODRAIN : ip_squeue_flag, SQTAG_IP_TCP_INPUT);
		}
		ira->ira_ill = ill;
		ira->ira_rill = rill;
		return;
	...
	case IPPROTO_SCTP: {
		...
		ASSERT(MBLKL(mp) >= ip_hdr_length + SCTP_MIN_HDR_LENGTH);

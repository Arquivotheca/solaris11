...
#define	DCE_CONDEMNED_LIMIT	4096
/*
...
ip_dce_reclaim_stack(ip_stack_t *ipst)
{
	...
	if ((freemem > lotsfree + needfree) &&
	    (ipst->ips_num_dce_condemned < DCE_CONDEMNED_LIMIT))
		return;
...
dce_stack_init(ip_stack_t *ipst)
{
	...
	def_dce = ipst->ips_dce_default;
	bzero(def_dce, sizeof (dce_t));
	def_dce->dce_flags = DCEF_DEFAULT;
	def_dce->dce_generation = DCE_GENERATION_INITIAL;
	def_dce->dce_last_change_time = TICK_TO_SEC(ddi_get_lbolt64());
	def_dce->dce_refcnt = 1;	/* Should never go away */
	def_dce->dce_ipst = ipst;
	/*
	 * Set the default initial fragment ID to a random number so that
	 * the number of fragmented packets sent since stack start up is not
	 * revealed in the ID.
	 */
	(void) random_get_pseudo_bytes((uint8_t *)&def_dce->dce_ident,
	    sizeof (def_dce->dce_ident));
...
static void
dce_cleanup_all(ip_stack_t *ipst)
{
	uint_t  i;
	dcb_t   *dcb;
	dce_t   *dce, *nextdce;
...
dce_stack_destroy(ip_stack_t *ipst)
{
	...
	dce_cleanup_all(ipst);
	for (i = 0; i < ipst->ips_dce_hashsize; i++) {
		ASSERT(ipst->ips_dce_hash_v4[i].dcb_cnt == 0);
		rw_destroy(&ipst->ips_dce_hash_v4[i].dcb_lock);
		ASSERT(ipst->ips_dce_hash_v6[i].dcb_cnt == 0);
...
/*
 * Set the initial fragment ID.  Need to make sure that the fragment ID is
 * random and not the same as the last used default fragment ID.
 */
#define	DCE_SET_IDENT(ipst, dce)					\
{									\
	(void) random_get_pseudo_bytes((uint8_t *)&(dce)->dce_ident,	\
	    sizeof ((dce)->dce_ident));					\
	if ((dce)->dce_ident == 0)					\
		(dce)->dce_ident = 1;					\
	(dce)->dce_ident += (ipst)->ips_dce_default->dce_ident;		\
}

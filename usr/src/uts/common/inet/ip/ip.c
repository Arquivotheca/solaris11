 ...
 * these Tx rings. mac_tx() is called (via str_mdata_fastpath_put()) to send
 ...
 * cookie points to a specific Tx ring that is blocked. The cookie is used
 * to hash into the ill_idl_hash[] array and locate a matching idl_t.
 * Each idl_t corresponds to a unique cookie. Legacy NICs that don't expose
 * Tx rings have a single idl_t corresponding to a zero cookie, while NICs
 * with 1 or more  Tx rings have one idl_t correspnding to each Tx ring or
 * cookie. The connections (conn_t) are anchored off the idl_t.
 *
 * The diagram below shows the ill_idl_hash array and its drain_lists.
 * The ill_idl_hash array is anchored off of the ill and the size of the
 * array is TX_FANOUT_SIZE (127). Multiple idl_t list entries off of a
 * single ill_idl_hash[] array element implies that multiple cookies hashed
 * to the same value. The drain list points to the list of connections that
 * are flow controlled.
 *
 * +---------------+	+------+	+-----+
 * |ill_idl_hash[0]|--->| idl_t|------->|idl_t| ...
 * +---------------+	+------+	+-----+
 *			   |		   |
 *			+------+	+------+
 *			|conn_t|	|conn_t| ...
 ...
 * Flow control block
 *
 * ip_xmit returns EWOULDBLOCK to the transport when it attempts to send a
 * packet and detects a flow control block condition. This may happen with
 * mac_tx() returning a cookie in the direct call case or as a result of a
 * failure of canputnext in the STREAMS case. In either case, the transport
 * then calls ixa_check_drain_insert to insert the conn into the idl_t
 * corresponding to the cookie in the hash list. A zero cookie
 * corresponds to a flow control block due to a legacy NIC with no Tx rings.
 * The conn_t is then inserted into the idl_t list depicted above. Upper level
 * flow control is achieved by setting QFULL on the conn_wq for streams
 * end points and by calling su_txqfull for non-streams sockets.
 *
 * Flow control relief
 *
 ...
 * blocked Tx ring. This cookie is used to locate the right idl_t and
 * enable the blocked connections hanging off the idl_t. For legacy NICs,
 * ip_wsrv is back enabled via streams which in turn calls ill_flow_enable
 * QFULL is cleared for streams end points and su_clrqfull is called for
 * non-streams sockets.
 *
 * IPQOS notes:
 *
...
static void	ill_idl_fini(ill_t *);

static void	conn_walk_sctp(pfv_t, void *, zoneid_t, netstack_t *);
static void	conn_idl_cleanup(conn_t *);
...
static void	ill_flowctl_timer(void *);
...
#define	SIOCLIFADDR_NDX 112
	...
	/* 147 */ { SIOCCLIFFLAGS, sizeof (struct lifreq), IPI_PRIV | IPI_WR,
			LIF_CMD, ip_sioctl_flags, ip_sioctl_flags_restart },
    ...
    ip_xmit_attr_t *ixa, iulp_t *uinfo, uint32_t flags, uint_t mac_mode)
{
	...
	if (!(ire->ire_flags & (RTF_REJECT|RTF_BLACKHOLE))) {
		/* Get an nce to cache. */
		nce = ire_to_nce(ire, firsthop, NULL, ixa->ixa_ifindex);
		if (nce == NULL) {
			/* Allocation failure? */
...
/*
 * Carve "len" bytes out of an mblk chain, dupb/copy mblks that can not be used
 * completely. Return a pointer to the first mblk in the result, and update
 * the pointer to the next mblk to chew on.
 * If anything goes wrong, we waste everything in sight and return a NULL
 * pointer.
 */
mblk_t *
ip_carve_mp(mblk_t **mpp, ssize_t len)
{
	...
	if (mp0->b_wptr - mp0->b_rptr > len) {
		mp1 = dupb(mp0);
		if (mp1 == NULL) {
			mp1 = allocb_tmpl(len, mp0);
			if (mp1 == NULL)
				return (mp1);
			bcopy(mp0->b_rptr, mp1->b_rptr, len);
		}
	...
	for (mp2 = mp1 = mp0; (mp2 = mp2->b_cont) != 0 && len; mp1 = mp2) {
		if (mp2->b_wptr - mp2->b_rptr > len) {
			...
			if (mp1->b_cont == NULL) {
				...
				newmp = allocb_tmpl(len, mp2);
				if (newmp == NULL) {
					/*
					 * Trouble. Rather than go to a lot of
					 * trouble to clean up, we free the
					 * messages. This won't be any worse
					 * than losing it on the wire.
					 */
					freemsg(mp0);
					freemsg(mp2);
					*mpp = NULL;
				...
				bcopy(newmp->b_rptr, mp2->b_rptr, len);
				mp1->b_cont = newmp;
			}
			mp1 = mp1->b_cont;
			mp1->b_wptr = mp1->b_rptr + len;
...
ip_modclose(ill_t *ill)
{
	 ...
	 * /usr/sbin/ifconfig plumbing a stream twice, and encountering
	 * an error
	 */
	if (ai != NULL) {
		ASSERT(!ill->ill_isv6);
...
{
	boolean_t	conn_idl_cleanup_reqd = B_FALSE;
	 ...
	 * inserted in future into any list. Eg. ixa_check_drain_insert
	 * won't insert this conn into the idl_t list
	 *
	 ...
	 */
	if (conn_idl_cleanup_reqd)
		conn_idl_cleanup(connp);
...
ip_stack_shutdown(netstackid_t stackid, void *arg)
{
	...
	/* Check if we have already shutdown this IP stack */
	if ((ipst->ips_capab_taskq_flags & IPS_CAPAB_TASKQ_QUIT) != 0)
		return;
	...
	ipst->ips_capab_taskq_flags |= IPS_CAPAB_TASKQ_QUIT;
	/* Wake up the ill_taskq_dispatch thread */
	cv_signal(&ipst->ips_capab_taskq_cv);
	/*
	 * Wait for confirmation that ill_taskq_dispatch() is done
	 * before proceeding. This ensures that no one frees the
	 * ipst before ill_taskq_dispatch() is done with it
	 */
	while (!(ipst->ips_capab_taskq_flags & IPS_ILL_TASKQ_DISP_DONE))
...
ip_ddi_init(void)
{
	...
	ipsec_policy_g_init();
	tcpcong_ddi_g_init();
	newreno_ddi_g_init();	/* must always be after tcpcong_ddi_g_init() */
	tcp_ddi_g_init();
	sctp_ddi_g_init();
	dce_g_init();
...
ip_stack_init(netstackid_t stackid, netstack_t *ns)
{
	...
	/*
	 * Create the taskq dispatcher thread after initializing related stuff.
	 */
...
ip_type_v4(ipaddr_t addr, ip_stack_t *ipst)
{
	...
	ire = ire_ftable_lookup_simple_v4(addr, 0, ipst, NULL, NULL,
	    NULL, NULL);
	ASSERT(ire != NULL);
	if (ire->ire_flags & (RTF_REJECT|RTF_BLACKHOLE))
		result = IRE_NOROUTE;
 ...
 *
 * We overwrite an existing source-route option if, and only if, it has
 * been set by an application on a local end-point. TCP listen
 * endpoints that receive source-route options would have already reversed
 * these options in ip_pkt_source_route_v4()
 */
ipaddr_t
ip_massage_options(ipha_t *ipha, netstack_t *ns, boolean_t overwrite)
{
	ipoptp_t	opts;
	uchar_t		*opt;
	uint8_t		optval;
...
ip_fix_dbref(mblk_t *mp, ip_recv_attr_t *ira)
{
	...
	/* Make sure we have ira_l2src before we lose the original mblk */
	if (!(ira->ira_flags & IRAF_L2SRC_SET))
		ip_setl2src(mp, ira, ira->ira_rill);
		...
		case DL_NOTE_LINK_DOWN: {
			...
			if (IS_IPMP(ill)) {
				new_phyint_flags = went_up ?
				    ...
				    new_phyint_flags | PHYI_FAILED;
				if (ill->ill_phyint->phyint_grp->gr_probing) {
					new_phyint_flags |= PHYI_FAILED;
					new_phyint_flags &= ~PHYI_RUNNING;
				}
			}
...
/*
 * Following block of functions are related to IP - device flow control.
 * The big block comment at the start of ip.c under the section
 * 'IP flow control notes' explains the mechanisms.
 */
void
ill_idl_init(ill_t *ill)
{
	int i;
	idl_hash_list_t *idl_hash;

	ill->ill_idl_hash =
	    kmem_zalloc(TX_FANOUT_SIZE * sizeof (idl_hash_list_t), KM_SLEEP);
	for (i = 0; i < TX_FANOUT_SIZE; i++) {
		idl_hash =  &ill->ill_idl_hash[i];
		mutex_init(&idl_hash->idh_lock, NULL, MUTEX_DEFAULT, NULL);
	}
}

static void
ill_idl_fini(ill_t *ill)
{
	int i;
	idl_hash_list_t *idl_hash;

	(void) untimeout(ill->ill_flowctl_tid);

	for (i = 0; i < TX_FANOUT_SIZE; i++) {
		idl_hash =  &ill->ill_idl_hash[i];
		ASSERT(idl_hash->idh_idl == NULL);
		mutex_destroy(&idl_hash->idh_lock);
	}
	kmem_free(ill->ill_idl_hash,
	    TX_FANOUT_SIZE * sizeof (idl_hash_list_t));
	ill->ill_idl_hash = NULL;
}

/*
 * Enable the blocked connections for transmit. Called on flow control relief
 */
void
conn_list_drain(conn_t *connp)
{
	conn_t	*next;

	while (connp != NULL) {
		next = connp->conn_drain_next;
		...
		/*
		 * Release the reference held in ixa_check_drain_insert
		 * when the conn was added to the idl_t list of conns.
		 */
		CONN_DEC_REF(connp);
		connp = next;
	}
...
uint_t idl_g_ndrainers = 8;

/*
 * Called on flow control relief from the underlying NIC driver.
 * Drain the list of connections anchored off the idl_t that are blocked
 * from sending packets down due to flow control. To avoid the thundering
 * herd problem, we enable only a limited number of conns up to
 * 'idl_ndrainers' and double this number every tick in idl_timer until
 * we are blocked again, at which point we reset it to zero.
 * Blocked connections go to the tail of the list to ensure fairness.
 * On interface unplumb, `all' is true  and we drain all the connections.
 */
conn_t *
idl_drain(idl_t *idl, boolean_t all, conn_t **tailp)
{
	int	cnt = 0;
	conn_t *head, *cur, *prev = NULL;

	ASSERT(MUTEX_HELD(&idl->idl_hashp->idh_lock));

	/*
	 * Set the number of drainers the first time we are enabled
	 * after a flow control block
	 */
	if (idl->idl_ndrainers == 0)
		idl->idl_ndrainers = idl_g_ndrainers;
	head = cur = idl->idl_conn_head;
	while (cur != NULL && (all || cnt < idl->idl_ndrainers)) {
		cnt++;
		mutex_enter(&cur->conn_lock);
		cur->conn_idl = NULL;
		/*
		 * If the conn is closing, it may be waiting and needs to
		 * be woken up.
		 */
		if (cur->conn_state_flags & CONN_CLOSING)
			cv_signal(&cur->conn_idl_cv);
		mutex_exit(&cur->conn_lock);
		prev = cur;
		cur = cur->conn_drain_next;
	}
	if (prev != NULL) {
		idl->idl_conn_head = prev->conn_drain_next;
		prev->conn_drain_next = NULL;
	}
	if (idl->idl_conn_head == NULL)
		idl->idl_conn_tail = NULL;

	if (tailp != NULL)
		*tailp = prev;

	ill_conn_blocked_cnt_adjust(idl->idl_ill, -cnt);

	return (head);
}

static void
conn_idl_cleanup(conn_t *connp)
{
	idl_t	*idl;
	ill_t	*ill;
	conn_t	*prev, *cur;
	...
	ill = idl->idl_ill;
	idl_hash = idl->idl_hashp;

	if (ill_check_and_refhold(ill)) {
		mutex_exit(&connp->conn_lock);
		mutex_enter(&idl_hash->idh_lock);
		mutex_enter(&connp->conn_lock);
		if (connp->conn_idl == NULL) {
			mutex_exit(&idl_hash->idh_lock);
			mutex_exit(&connp->conn_lock);
			ill_refrele(ill);
			return;
		}
		ASSERT(connp->conn_idl == idl);
		prev = NULL;
		cur = idl->idl_conn_head;
		while (cur != NULL && cur != connp) {
			prev = cur;
			cur = cur->conn_drain_next;
		}
		ASSERT(cur == connp);
		if (prev != NULL)
			prev->conn_drain_next = connp->conn_drain_next;
		if (idl->idl_conn_head == connp)
			idl->idl_conn_head = connp->conn_drain_next;
		if (idl->idl_conn_tail == connp)
			idl->idl_conn_tail = prev;
		ASSERT(((idl->idl_conn_head != NULL) ^
		    (idl->idl_conn_tail != NULL)) == B_FALSE);
		connp->conn_idl = NULL;
		connp->conn_drain_next = NULL;

		ill_conn_blocked_cnt_adjust(ill, -1);

		mutex_exit(&idl_hash->idh_lock);
		mutex_exit(&connp->conn_lock);

		conn_list_drain(connp);
		ill_refrele(ill);
	} else {
		while (connp->conn_idl != NULL)
			cv_wait(&connp->conn_idl_cv, &connp->conn_lock);
		mutex_exit(&connp->conn_lock);
	}
}

#define	ILL_NEED_FLOWCTL_TIMER(ill)				\
	(!((ill)->ill_state_flags & ILL_CONDEMNED) &&		\
	    (ill)->ill_conn_blocked_cnt != 0)
/*
 * Called on flow control relief from the underlying driver. There are 2 cases.
 *
 * i. DLD_CAPAB_DIRECT capability is enabled. This function is then called
 *    directly from GLD.
 * ii. DLD_CAPAB_DIRECT is not enabled. Called from ip_wsrv which in turn
 *    is via STREAMS back-enabling.
 */
void
ill_flowctl_enable(void *arg, ip_mac_tx_cookie_t cookie)
{
	ill_t *ill = (ill_t *)arg;
	idl_hash_list_t *idl_hash;
	idl_t	*idl;
	...
	if (cookie == (ip_mac_tx_cookie_t)-1) {
		ill_flowctl_cleanup(ill);
		return;
	}
	idl_hash = &ill->ill_idl_hash[IDLHASHINDEX(cookie)];
	mutex_enter(&idl_hash->idh_lock);
	for (idl = idl_hash->idh_idl; idl != NULL; idl = idl->idl_next) {
		if (idl->idl_cookie == cookie) {
			conn_list = idl_drain(idl, B_FALSE, NULL);
			mutex_exit(&idl_hash->idh_lock);
			conn_list_drain(conn_list);
			goto out;
		}
	}
	mutex_exit(&idl_hash->idh_lock);
...
out:
	/*
	 * Enable the timer if we have blocked connections and if
	 * the timer is not already running
	 */
	mutex_enter(&ill->ill_lock);
	if (ill->ill_flowctl_tid == 0 && ILL_NEED_FLOWCTL_TIMER(ill))
		ill->ill_flowctl_tid = timeout(ill_flowctl_timer, arg, 1);
	mutex_exit(&ill->ill_lock);
}

/*
 * Called on unplumb or down of the ill, to cleanup the flow control lists.
 * All conns are unhooked and re-enabled. The idl_t's  are destroyed.
 * This is called after the interface is quiesced. So there  can't be any
 * threads trying to insert conns or recreate an idl_t for the ill that is
 * going down.
 */
void
ill_flowctl_cleanup(ill_t *ill)
{
	idl_hash_list_t *idl_hash;
	idl_t	*idl;
	conn_t	*conn_list;
	...
	for (i = 0; i < TX_FANOUT_SIZE; i++) {
		idl_hash = &ill->ill_idl_hash[i];
		mutex_enter(&idl_hash->idh_lock);
		while (idl_hash->idh_idl != NULL) {
			idl = idl_hash->idh_idl;
			conn_list = idl_drain(idl, B_TRUE, NULL);
			idl_hash->idh_idl = idl->idl_next;
			idl->idl_next = NULL;
			mutex_exit(&idl_hash->idh_lock);
			conn_list_drain(conn_list);
			kmem_free(idl, sizeof (idl_t));
			mutex_enter(&idl_hash->idh_lock);
		}
		mutex_exit(&idl_hash->idh_lock);
	}
...
void
ill_conn_blocked_cnt_adjust(ill_t *ill, int cnt)
{
	mutex_enter(&ill->ill_lock);
	ill->ill_conn_blocked_cnt += cnt;
	ASSERT(ill->ill_conn_blocked_cnt >= 0);
	mutex_exit(&ill->ill_lock);
}

/*
 * Write service routine. The device queue's messages has fallen below the
 * low water mark and STREAMS has backenabled the ill_wq.
 */
void
ip_wsrv(queue_t *q)
{
	...
	if (ill->ill_state_flags == 0) {
		/*
		 * The device flow control has opened up. Start draining
		 * the list of blocked conns. This makes sense only
		 * if the stream is fully plumbed and setup.
		 * Hence the ill_state_flags check above.
		 */
		ill_flowctl_enable(ill, 0);
		enableok(ill->ill_wq);
	}
}

/*
 * Single timer per `ill' instance. Activated on the first flow control
 * relief that happens on the `ill'. Continues to re-arm itself as long as
 * there are flow controlled connections on the `ill' and the ill is not
 * being unplumbed.
 */
static void
ill_flowctl_timer(void *arg)
{
	ill_t *ill = (ill_t *)arg;
	int i;
	idl_hash_list_t	*idl_hash;
	...
	for (i = 0; i < TX_FANOUT_SIZE; i++) {
		...
		mutex_enter(&idl_hash->idh_lock);
		/*
		 * idl_ndrainers == 0 implies the idl is flow control blocked.
		 * Otherwise double the number of drainers every tick,
		 * Please see the block comment above ill_idl_drain.
		 */
		for (idl = idl_hash->idh_idl; idl != NULL &&
		    idl->idl_conn_head != NULL && idl->idl_ndrainers != 0;
		    idl = idl->idl_next) {
			idl->idl_ndrainers <<= 1;

			tmp_head = idl_drain(idl, B_FALSE, &tmp_tail);
			if (head == NULL) {
				head = tmp_head;
				tail = tmp_tail;
			} else {
				tail->conn_drain_next = tmp_head;
				if (tmp_head != NULL)
					tail = tmp_tail;
			}
		}
		mutex_exit(&idl_hash->idh_lock);
		if (head != NULL)
			conn_list_drain(head);
	}

	mutex_enter(&ill->ill_lock);
	if (ILL_NEED_FLOWCTL_TIMER(ill)) {
		ill->ill_flowctl_tid = timeout(ill_flowctl_timer, arg, 1);
	} else {
		ill->ill_flowctl_tid = 0;
	}
	mutex_exit(&ill->ill_lock);
}
...
/*
 * If the packet cannot be sent out in ip_xmit() because of pending address
 * resolution, the existing state of transmit attributes is saved in an mblk
 * that is prepended to the packet chain in ip_xmit_state_prepend(), by calling
 * ip_xmit_attr_to_mblk(). The prepended state will then be retrieved after
 * address resolution to re-invoke ip_xmit() from nce_resolv_ok() or
 * nce_resolv_ipmp_ok()
 */
/* ARGSUSED */
static mblk_t *
ip_xmit_state_prepend(nce_t *nce, iaflags_t ixaflags, uint_t pkt_len,
    uint32_t xmit_hint, zoneid_t szone, zoneid_t nolzid, uintptr_t *ixacookie)
{
	...
	bzero(&ixas, sizeof (ixas));
	ixas.ixa_flags = ixaflags;
	ill = nce->nce_ill;
	ixas.ixa_nce = nce;
	ixas.ixa_ifindex = ill->ill_phyint->phyint_ifindex;
	ixas.ixa_xmit_hint = xmit_hint;
	ixas.ixa_zoneid = szone;
	ixas.ixa_pktlen = pkt_len;
	ixas.ixa_ipst = nce->nce_ill->ill_ipst;
	ixas.ixa_no_loop_zoneid = nolzid;
	return (ip_xmit_attr_to_mblk(&ixas));
}
...
sendit:
	...
	case ND_INCOMPLETE:
		 ...
		 */
		asyncmp = ip_xmit_state_prepend(nce, ixaflags, pkt_len,
		    xmit_hint, szone, nolzid, ixacookie);
		mutex_enter(&ncec->ncec_lock);
		if (NCE_ISREACHABLE(ncec)) {
			mutex_exit(&ncec->ncec_lock);
			freemsg(asyncmp);
		...
		if (asyncmp == NULL) {
			...
			ip_drop_output("ipIfStatsOutDiscards "
			    "- no memory for asyncmp with ND_INCOMPLETE nce",
			    mp, ill);
			freemsg(mp);
			return (ENOMEM);
		}
		...
		linkb(asyncmp, mp);
		mp = asyncmp;
		is_probe = (IS_IPMP(ncec->ncec_ill) &&
		    (ixaflags & IXAF_IPMP_PROBE) != 0);
	...
	case ND_INITIAL:
		...
		if (asyncmp == NULL) {
			...
			BUMP_MIB(ill->ill_ip_mib, ipIfStatsOutDiscards);
			ip_drop_output("ipIfStatsOutDiscards "
			    "- no memory for asyncmp with ND_INITIAL nce",
			    mp, ill);
			freemsg(mp);
			return (ENOMEM);
		}
 ...
 */
ipaddr_t
ip_pkt_source_route_v4(const ip_pkt_t *ipp, ip_stack_t *ipst)
{
	...
	for (optval = ipoptp_first2(&opts, totallen, ipp->ipp_ipv4_options);
		...
		redo_srr:
			 ...
			 * for source route?
			 */
			if (ip_type_v4(nexthop, ipst) == IRE_LOCAL) {
				off += IP_ADDR_LEN;
				goto redo_srr;
			}
			break;
...
ip_pkt_source_route_reverse_v4(ip_pkt_t *ipp)
{
	...
	for (optval = ipoptp_first2(&opts, totallen, ipp->ipp_ipv4_options);
		...
		case IPOPT_LSRR:
			...
			off2 = opt[IPOPT_OFFSET] - IP_ADDR_LEN - 1;
			/*
			 * The last address in the source route option will
			 * become ipha_dst. Reverse all IP addresses
			 * before the last address.
			 */
			off2 -= IP_ADDR_LEN;
			while (off2 > off1) {
				bcopy(opt + off2, &tmp, IP_ADDR_LEN);
				bcopy(opt + off1, opt + off2, IP_ADDR_LEN);
				bcopy(&tmp, opt + off1, IP_ADDR_LEN);
				off2 -= IP_ADDR_LEN;
				off1 += IP_ADDR_LEN;
			}
			opt[IPOPT_OFFSET] = IPOPT_MINOFF_SR;
			ipp->ipp_flags |= IPP_SR_REVERSED;
			break;
		}
	}
}

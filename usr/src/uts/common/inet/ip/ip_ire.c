 ...
 *	  walking. We also hold irb_lock when modifying to allow the data path
 *	  to only acquire irb_lock.
 *
...
#define	IRE_CONDEMNED_LIMIT	4096
 ...
 *
 * Any suppression of unneeded ones is done in ire_add_v4.
...
matchit:
	...
	if (match_flags & MATCH_IRE_ILL) {
		 ...
		 * - we loopback packets even sent to test addresses on
		 *   on differnt interfaces in the group,
		 * - we loopback packets sent to any locally defined addresses
		...
		if ((match_flags & MATCH_IRE_TESTHIDDEN) &&
			 ...
			 * NULL
			 */
			if (!(ire->ire_type & IRE_LOCAL) || !IS_LOOPBACK(ill)) {
				if (ire_ill == NULL ||
				    !IS_ON_SAME_LAN(ill, ire_ill))
					return (B_FALSE);
			}
		}
	}
...
ip_ire_reclaim_stack(ip_stack_t *ipst)
{
	...
	if ((freemem > lotsfree + needfree) &&
	    (ipst->ips_num_ire_condemned < IRE_CONDEMNED_LIMIT))
		return;

	IP_STAT(ipst, ip_ire_reclaim_calls);

	fraction = ipst->ips_ip_ire_reclaim_fraction;
...
retry:
	...
	/*
	 * If we are trying to create nce for the data path (in which case
	 * the in_ill passed to ire_nce_init would have been the IPMP
	 * meta-interface) make sure that the underlying transmit interface,
	 * ill, is still an active interface under-ill in the IPMP group
	 * Hold the ill_g_lock to prevent IPMP group changes while we confirm
	 * that the ill is active, since this is the data path.
	 */
	rw_enter(&ill->ill_ipst->ips_ill_g_lock, RW_READER);
	mutex_enter(&ill->ill_lock);
	mutex_enter(&ill->ill_phyint->phyint_lock);
	if (need_refrele && !ipmp_ill_is_active(ill)) {
		/*
		 * need_refrele implies that an under ill was selected by
...
/*
 * Return a held IRE_MULTICAST. The caller may specify
 * a non-NULL prev_ire that it holds, to skip the hold here.
 */
ire_t *
ire_multicast_ext(ill_t *ill, ire_t *prev_ire, boolean_t *ire_reused)
{
	...
	else if (ire != prev_ire)
		ire_refhold(ire);
	else {
		ASSERT(ire_reused != NULL);
		*ire_reused = B_TRUE;
	}
	return (ire);
}
...
{
	return (ire_multicast_ext(ill, NULL, NULL));
}
...
ire_handle_condemned_nce(nce_t *nce, ire_t *ire, ipha_t *ipha, ip6_t *ip6h,
    boolean_t fail_if_better, uint_t ifindex)
{
	...
	if (ire->ire_ipversion == IPV4_VERSION) {
		ASSERT(ipha != NULL);
		nce = ire_to_nce(ire, ipha->ipha_dst, NULL, ifindex);
	} else {
		ASSERT(ip6h != NULL);
		nce = ire_to_nce(ire, INADDR_ANY, &ip6h->ip6_dst, ifindex);
	}
...
ire_revalidate_nce(ire_t *ire)
{
	...
	if (ire->ire_type & (IRE_LOCAL|IRE_LOOPBACK)) {
		...
		if (ire->ire_ipversion == IPV4_VERSION) {
			nce = arp_nce_init(nexthop->ire_ill,
			    nexthop->ire_addr, nexthop->ire_type);
		} else {
 ...
 * If an ifindex has been specified, and that ifindex identifies an
 * underlying interface in an IPMP group, ire_to_nce() will ensure that the
 * returned nce is created with nce->nce_ill pointing at the interface
 * with the specified ifindex.
 */
nce_t *
ire_to_nce(ire_t *ire, ipaddr_t v4nexthop, const in6_addr_t *v6nexthop,
    uint_t ifindex)
{
	nce_t	*nce, *under_nce = NULL;
	ill_t	*ill;
	boolean_t isv6, ifindex_is_ipmp = B_FALSE;
	...
	if (nce != NULL) {
		/*
		 * If the nce matches any ifindex constraints, we can return
		 * the nce after verifying that we are not sending IPMP data
		 * packet on a failed IPMP group. Data packets are those that
		 * don't have an ifindex corresponding to an underlying
		 * interface, i.e., ifindex is 0 or has been explicitly
		 * specified to be an ifindex that corresponds to an
		 * IPMP meta-interface.
		 */
		ill = nce->nce_ill;
		if (ifindex == 0 ||
		    ill->ill_phyint->phyint_ifindex == ifindex) {
			if (ifindex != 0)
				ifindex_is_ipmp = IS_IPMP(ill);
			goto done;
		}
	}
	/*
	 * Either nce is null, or we did not meet the ifindex constraints.
	 * If ifindex is 0 (no ifindex constraints), and the requeste ire_type
	 * is IRE_MULTICAST, invoke ire_nce_init() to create a multicast nce
	 * based on ire_ill. If ifindex is not 0, create an nce on the
	 * requested ifindex. Note that unicast ire's with ifindex 0 must have
	 * had their nce created via ire_revalidate_nce(), which would typically
	 * have been invoked from ip_select_route()
	 */
	if (ifindex == 0) {
		ill = ire->ire_ill;
		if (ill == NULL) {
		...
		/*
		 * Must create on the requested ifindex, if it still exists.
		 */
		isv6 = (ire->ire_ipversion != IPV4_VERSION);
		ill = ill_lookup_on_ifindex(ifindex, isv6, ire->ire_ipst);
		if (ill == NULL) {
			if (nce != NULL)
				nce_refrele(nce);
			return (NULL);
		} else if (IS_IPMP(ill)) {
			ifindex_is_ipmp = B_TRUE;
			if (nce != NULL) {
				ill_refrele(ill);
				goto done;
			}
		}
		/* create an nce on the underlying send interface */
		if (nce != NULL) {
			under_nce = nce_fastpath_on_ill(ill, nce->nce_common);
			ill_refrele(ill);
			nce_refrele(nce);
			nce = under_nce;
			goto done;
	...
	if (ire->ire_type & IRE_MULTICAST) {
		...
		if ((ifindex == 0 || ifindex_is_ipmp) && IS_IPMP(ill) &&
		...
		if (ire->ire_ipversion == IPV4_VERSION) {
			...
			nce = arp_nce_init(ill, v4nexthop, ire->ire_type);
		} else {
			ASSERT(v6nexthop != NULL);
			ASSERT(v4nexthop == 0);
			nce = ndp_nce_init(ill, v6nexthop, ire->ire_type);
		}
		ill_refrele(ill);
		goto done;
	}
	...
	/*
	 * unicast nce's are created with the nce_common pointing at the
	 * IPMP meta-interface. If the ifindex is 0, or was selected to the
	 * meta-interface when the group has failed, we force the packet to
	 * drop by returning NULL here.
	 */
	if (nce != NULL) {
		ill = nce->nce_common->ncec_ill;
		if (IS_IPMP(ill) && (ifindex == 0 || ifindex_is_ipmp) &&
		    ill->ill_phyint->phyint_grp->gr_probing) {
			nce_refrele(nce);
			return (NULL);
		}
...
nce_t *
ire_to_nce_pkt(ire_t *ire, mblk_t *mp, uint_t ifindex)
{
	ipha_t		*ipha;
	ip6_t		*ip6h;
	...
	if (IPH_HDR_VERSION(mp->b_rptr) == IPV4_VERSION) {
		...
		return (ire_to_nce(ire, ipha->ipha_dst, NULL, ifindex));
	} else {
		ip6h = (ip6_t *)mp->b_rptr;
		return (ire_to_nce(ire, INADDR_ANY, &ip6h->ip6_dst, ifindex));
	}
}

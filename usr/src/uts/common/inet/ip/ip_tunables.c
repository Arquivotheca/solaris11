...
ip_get_forwarding(void *cbarg, mod_prop_info_t *pinfo, const char *ifname,
    void *pval, uint_t *pr_size, uint_t ptype)
{
	boolean_t 		value;
	ill_walk_context_t 	ctx;
	ill_t 			*ill;
	...
	case MOD_PROP_POSSIBLE:
		nbytes = snprintf(pval, *pr_size, "%u,%u", B_FALSE, B_TRUE);
		break;
	case MOD_PROP_ACTIVE:
		...
		if (ifname == NULL || ifname[0] == '\0') {
			nbytes = snprintf(pval, *pr_size, "%d",
			    pinfo->prop_cur_bval);
			break;
		}
    ...
    void *pval, uint_t *psize, uint_t ptype)
{
	...
	switch (ptype) {
	case MOD_PROP_PERM:
		*psize = snprintf(pval, *psize, "%u", MOD_PROP_PERM_RW);
		break;
	case MOD_PROP_POSSIBLE:
	...
	case MOD_PROP_DEFAULT:
		*psize = snprintf(pval, *psize, "%u", pinfo->prop_def_uval);
		break;
	case MOD_PROP_ACTIVE:
		*psize = snprintf(pval, *psize, "%u", ip_debug);
		break;
	default:
		return (EINVAL);
	}
    ...
    void *pval, uint_t *psize, uint_t ptype)
{
	...
	if (ptype != MOD_PROP_DEFAULT && ptype != MOD_PROP_POSSIBLE)
		return (ENOTSUP);
	...
	switch (ptype) {
	case MOD_PROP_DEFAULT:
		*psize = snprintf(pval, *psize, "%u", def_mtu);
		break;
	case MOD_PROP_POSSIBLE: {
		uint32_t	min_mtu;

		min_mtu = isv6 ? IPV6_MIN_MTU : IP_MIN_MTU;
		*psize = snprintf(pval, *psize, "%u-%u", min_mtu, max_mtu);
		break;
	}
	default:
		return (EINVAL);
    ...
    void *pval, uint_t *psize, uint_t ptype)
{
	...
	/*
	 * we come here for only current property, everything else is
	 * available in the library.
	 */
	if (ptype != MOD_PROP_ACTIVE)
		return (ENOTSUP);
	if (*psize < sizeof (hostmodel)) {
		*psize = sizeof (hostmodel);
		return (ENOBUFS);
	}
	bzero(pval, *psize);
	if (!isv6) {

...
static void	ipnet_refhold(ipnet_t *);
static void	ipnet_refrele(ipnet_t *);
...
static ipnet_kstats_t stats_template = {
	...
	{ "dispatchPutDrop",    KSTAT_DATA_UINT64 },
...
ipnet_populate_if(net_handle_t nd, ipnet_stack_t *ips, boolean_t isv6)
{
	...
	for (phyif = net_phygetnext(nd, 0); phyif != 0 && phyif != (phy_if_t)-1;
		...
		for (lif = net_lifgetnext(nd, phyif, 0);
		    lif != 0 && lif != (lif_if_t)-1;
...
ipnet_open(queue_t *rq, dev_t *dev, int oflag, int sflag, cred_t *crp)
{
	...
	mutex_init(&ipnet->ipnet_reflock, NULL, MUTEX_DEFAULT, NULL);
	cv_init(&ipnet->ipnet_refcv, NULL, CV_DEFAULT, NULL);
	...
	*dev = makedevice(getmajor(*dev), ipnet->ipnet_minor);
	...
	if (err != 0) {
		...
		mutex_destroy(&ipnet->ipnet_reflock);
		cv_destroy(&ipnet->ipnet_refcv);
...
ipnet_close(queue_t *rq)
{
	...
	if (list_link_active(&ipnet->ipnet_next)) {
		/*
		 * Need to handle walkers that use list_head and list_next.
		 * Note that we can't do a list_remove until we are certain
		 * that no other thread will call list_next, since list_next
		 * returns garbage if the item is not on the list.
		 */
		mutex_enter(&ips->ips_list_lock);
		ipnet->ipnet_closing = B_TRUE;
		mutex_exit(&ips->ips_list_lock);

		/*
		 * Wait for any current walkers with a ref. Ref is atleast one
		 * since the ipnet is on the list.
		 */
		mutex_enter(&ipnet->ipnet_reflock);
		ASSERT(ipnet->ipnet_refcnt >= 1);
		while (ipnet->ipnet_refcnt != 1)
			cv_wait(&ipnet->ipnet_refcv, &ipnet->ipnet_reflock);
		mutex_exit(&ipnet->ipnet_reflock);
		...
		/* One refcnt for being on the list */
		ipnet_refrele(ipnet);
	}
	ASSERT(ipnet->ipnet_refcnt == 0);
...
/*
 * If this ipnet is not already on ips_str_list, then we add it.
 */
static void
ipnet_add_str_list(ipnet_t *ipnet)
{
	ipnet_stack_t	*ips = ipnet->ipnet_ns->netstack_ipnet;
	...
	/*
	 * Only register our callback if we're the first client.
	 * We call unregister in close() for the last client.
	 */
	if (list_head(&ips->ips_str_list) == list_tail(&ips->ips_str_list)) {
		ips->ips_hook = ipobs_register_hook(ipnet->ipnet_ns,
		    ipnet_dispatch);
	}
	mutex_exit(&ips->ips_list_lock);
}
...
{
	struct copyresp *crp = (struct copyresp *)mp->b_rptr;
	ipnet_t	*ipnet = q->q_ptr;
...
ipnet_join_allmulti(ipnetif_t *ipnetif, ipnet_stack_t *ips)
{
	...
	/*
	 * ip_join_allmulti() returns ENETDOWN if the interface is plumbed and
	 * there are no [IFF_UP] addresses configured on the interface. In that
	 * case we will try to join later from ipnet_lifup_ev() for this
	 * interface, when at least one of the addresses comes up later on.
	 *
	 * We will not return an error, if ip_join_allmulti() fails, because
	 * we still need to provide observability for the plumbed interface.
	 */
	mutex_enter(&ips->ips_event_lock);
	if (ipnetif->if_multicnt == 0) {
		...
		if (ipnetif->if_flags & IPNETIF_IPV4PLUMBED) {
			if (ip_join_allmulti(index, B_FALSE, ipst) == 0)
				ipnetif->if_flags |= IPNETIF_IPV4ALLMULTI;
		}
		if (ipnetif->if_flags & IPNETIF_IPV6PLUMBED) {
			if (ip_join_allmulti(index, B_TRUE, ipst) == 0)
...
ipnet_dispatch(void *arg)
{
	...
	ipnet = list_head(list);
	while (ipnet != NULL && ipnet->ipnet_closing)
	...
	while (ipnet != NULL) {
		mutex_enter(&ips->ips_list_lock);
		ipnet1 = list_next(list, ipnet);
		while (ipnet1 != NULL && ipnet1->ipnet_closing)
			ipnet1 = list_next(list, ipnet1);
		if (ipnet1 != NULL)
			ipnet_refhold(ipnet1);
		mutex_exit(&ips->ips_list_lock);
    ...
    ipnet_stack_t *ips, boolean_t isv6)
{
	...
	/* check to see if we have to join allmulti */
	if (ipnetif->if_multicnt != 0)  {
		boolean_t	join = B_FALSE;
		uint_t		flag = 0;
		ip_stack_t	*ipst = ips->ips_netstack->netstack_ip;

		if (isv6 && !(ipnetif->if_flags & IPNETIF_IPV6ALLMULTI)) {
			join = B_TRUE;
			flag = IPNETIF_IPV6ALLMULTI;
		} else if (!isv6 &&
		    !(ipnetif->if_flags & IPNETIF_IPV4ALLMULTI)) {
			join = B_TRUE;
			flag = IPNETIF_IPV4ALLMULTI;
		}
		if (join &&
		    ip_join_allmulti(ipnetif->if_index, isv6, ipst) == 0) {
			ipnetif->if_flags |= flag;
		}
	}
	ipnetif_refrele(ipnetif);
}
...
ipnet_nicevent_cb(hook_event_token_t token, hook_data_t info, void *arg)
{
	...
	if (ddi_taskq_dispatch(ipnet_nicevent_taskq, ipnet_nicevent_task,
	    ipne, DDI_NOSLEEP) != DDI_SUCCESS) {
		mutex_enter(&ips->ips_event_lock);
		ips->ips_ntasks--;
		mutex_exit(&ips->ips_event_lock);
...
ipnet_nicevent_task(void *arg)
{
	...
	ipnet_stack_t		*ips = ipne->ipne_ips;
	boolean_t		isv6;

	mutex_enter(&ips->ips_event_lock);
	if (ips->ips_unregistering)
		goto done;
...
ipnet_stack_init(netstackid_t stackid, netstack_t *ns)
{
	...
	mutex_init(&ips->ips_list_lock, NULL, MUTEX_DEFAULT, 0);
	mutex_init(&ips->ips_event_lock, NULL, MUTEX_DEFAULT, 0);
	cv_init(&ips->ips_event_cv, NULL, CV_DEFAULT, NULL);
...
ipnet_stack_fini(netstackid_t stackid, void *arg)
{
	...
	/*
	 * Since the nicevent hooks have been unregistered, no more
	 * nicevent_tasks will be dispatched. Set the unregistering flag and
	 * wait for all the already dispatched nicevent_tasks to quiesce.
	 */
	mutex_enter(&ips->ips_event_lock);
	ips->ips_unregistering = B_TRUE;
	while (ips->ips_ntasks != 0)
		cv_wait(&ips->ips_event_cv, &ips->ips_event_lock);
	mutex_exit(&ips->ips_event_lock);
	...
	mutex_destroy(&ips->ips_list_lock);
	mutex_destroy(&ips->ips_event_lock);
	cv_destroy(&ips->ips_event_cv);
...
static void
ipnet_refrele(ipnet_t *ipnet)
{
	mutex_enter(&ipnet->ipnet_reflock);
	ASSERT(ipnet->ipnet_refcnt > 0);
	ipnet->ipnet_refcnt--;
	cv_broadcast(&ipnet->ipnet_refcv);
	mutex_exit(&ipnet->ipnet_reflock);
}
...
ipnetif_zonecheck(ipnetif_t *ipnetif, ipnet_stack_t *ips)
{
	...
	while (ipnet != NULL) {
		...
		if (ipnet->ipnet_if == ipnetif &&
		    !ipnetif_in_zone(ipnetif, ipnet->ipnet_zoneid, ips))
			(void) putnextctl(ipnet->ipnet_rq, M_HANGUP);

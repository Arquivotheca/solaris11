...
/* Flags for ips_capab_taskq_flags */
#define	IPS_ILL_TASKQ_DISP_DONE		0x01
#define	IPS_CAPAB_TASKQ_QUIT		0x02
 ...
 */
#define	IPSEC_POLICY_MISMATCH		0
#define	IPSEC_POLICY_AUTH_NOT_NEEDED	1
#define	IPSEC_POLICY_ENCR_NOT_NEEDED	2
#define	IPSEC_POLICY_SE_NOT_NEEDED	3
#define	IPSEC_POLICY_MAX		4	/* Always max + 1. */
...
/*
 * The IP drain list structure (idl_t) represents the list of connections
 * blocked due to flow-control on a specific Tx ring of the ill, or the
 * entire ill in the case when the Tx rings haven't been exposed.
 * The idl_cookie is the opaque handle of the Tx ring on which  the
 * connections are blocked. It is zero if the Tx rings haven't been exposed.
 *
 * idh_lock protects the idh_idl list of idl_t and the conn_t list off
 * of each idl_t.
 *
 * For complete details regarding the flow control scheme please see the
 * big block comment at the start of ip.c
 */

#define	IDL_NDRAINERS	8			/* Concurrent drains */

typedef uintptr_t ip_mac_tx_cookie_t;

typedef struct idl_hash_list_s {		/* Array of hash list */
	kmutex_t	idh_lock;
	struct idl_s	*idh_idl;
} idl_hash_list_t;

typedef struct idl_s {
	ip_mac_tx_cookie_t	idl_cookie;	/* Bloced on this Tx cookie */
	idl_hash_list_t		*idl_hashp;	/* Back pointer */
	struct ill_s		*idl_ill;
	struct idl_s		*idl_next;
	conn_t			*idl_conn_head;	/* List of blocked conn_t */
	conn_t			*idl_conn_tail;
	uint_t			idl_ndrainers;	/* concurrent drainer count */
} idl_t;
...
typedef struct ipmp_grp_s {
	...
	boolean_t	gr_probing;	/* group has failed and has a */
					/* designated PROBER  */
...
#define	ill_last_mp_to_free	ill_mcast_deferred
	...
	pm_handle_t	ill_pmh; /* pmh is set only for ILL_CAPAB_DLD_DIRECT */
	...
	idl_hash_list_t	*ill_idl_hash;	/* conn drain hash list */
	timeout_id_t	ill_flowctl_tid;
	int		ill_conn_blocked_cnt; /* flow controlled conn count */
	uint32_t	ill_xmit_hint;
} ill_t;
...
typedef enum {
	...
	IXAN_PMH	/* Pre-mapped DMA handle change */
} ixa_notify_type_t;
...
/* Flags to clear when we detach ixa_ipsec_policy. */
#define	IXAF_IPSEC_CLEARFLAGS	(IXAF_IPSEC_SECURE | IXAF_IPSEC_TUNNEL)

#define	IXAF_IPMP_PROBE		0x1000000000	/* IPMP probe packet */
#define	IXAF_VERIFY_PMH		0x2000000000	/* Pre-mapped cache handle */
 ...
 * The shift amount is based on the size of the struct to discard the
 * low order bits which don't have much if any variation
...
#define	CONN_TO_XMIT_HINT(connp) \
	HASH_HINT((uint32_t)(((uintptr_t)connp) >> 11))

/*
 * Set ira_xmit_hint for forwarded traffic. We use random_get_pseudo_bytes()
 * rather than hashing on a pointer, to get a better distribution of packets
 * among outgoing rings. While random_get_pseudo_bytes() is slightly more
 * expensive, we are computing it only once and caching it in a
 * long living ill_ring_t structure.
 *
 * Basing the hint for forwarded traffic on rr_ring_xmit_hint of the
 * ill_ring_t means that packets from the same NIC+ring are likely to use
 * the same outbound ring hence we get low contention on the ring in the
 * transmitting driver.
 */
#define	SET_ILL_RING_XMIT_HINT(ring)    ((void) random_get_pseudo_bytes( \
	(uint8_t *)&(ring)->rr_ring_xmit_hint, \
	sizeof ((ring)->rr_ring_xmit_hint)))
#define	ILL_RING_TO_XMIT_HINT(ring)	(((ring) != NULL) ? \
	(ring)->rr_ring_xmit_hint : 0)
...
struct ip_recv_attr_s {
	...
	nce_t		*ira_old_nce;	/* nce used for previous pkt */
...
#define	ipp_addr_v4	V4_PART_OF_V6(ipp_addr)
	...
	uint16_t	ipp_flags;
#define	IPP_SR_REVERSED	0x0001 /* source-route option has been reversed */
...
extern ipaddr_t ip_pkt_source_route_v4(const ip_pkt_t *, ip_stack_t *);
...
struct	mac_ip_sqinfo_s;

extern conn_t	*idl_drain(idl_t *, boolean_t, conn_t **);
...
extern void	ill_idl_init(ill_t *);
...
extern void	ip_input(ill_t *, void *, mblk_t *,
    struct mac_header_info_s *);
extern void	ip_input_v6(ill_t *, void *, mblk_t *,
    struct mac_header_info_s *);
extern mblk_t	*ip_input_common_v4(ill_t *, void *, mblk_t *,
    struct mac_header_info_s *, squeue_t *, mblk_t **, uint_t *);
extern mblk_t	*ip_input_common_v6(ill_t *, void *, mblk_t *,
    struct mac_header_info_s *, squeue_t *, mblk_t **, uint_t *);
...
extern void	ixa_check_drain_insert(conn_t *, ip_xmit_attr_t *);
...
extern boolean_t ip_output_verify_local(conn_t *);
...
extern ipaddr_t ip_massage_options(ipha_t *, netstack_t *, boolean_t);
...
extern void	conn_drain_list(conn_t *);
extern void	conn_drain_idl(idl_t *);
extern void	conn_setqfull(conn_t *, boolean_t *);
extern void	conn_clrqfull(conn_t *, boolean_t *);
extern void	conn_list_drain(conn_t *);
...
struct ill_rx_ring {
	...
	int			rr_ring_xmit_hint;	/* for ira_xmit_hint */
};
...
typedef struct ill_dld_direct_s {		/* DLD provided driver Tx */
	...
	pm_handle_t		idd_tx_pmh;	/* pm_handle_t */
} ill_dld_direct_t;
...
extern squeue_t *ip_squeue_get(void *);
...
extern void ip_gld_squeue_add(struct mac_ip_sqinfo_s *);
extern void ip_gld_squeue_remove(struct mac_ip_sqinfo_s *);
...
extern void	conn_change_pmh(ill_t *, conn_t *);

...
static int fr_stgettable __P((char *, ipf_stack_t *));
...
ipf_stack_t *ifs;
{
	...
	ifs->ifs_ips_stats.iss_tcptab = ifs->ifs_ips_tqtqb;
...
ipf_stack_t *ifs;
{
	...
	case SIOCGTQTAB :
		error = fr_outobj(data, ifs->ifs_ips_tqtqb, IPFOBJ_STATETQTAB);
		break;
...
ipstate_t *is;
{
	 ...
	 * entry to be created with a retransmitted SYN packet.
	 */
	if ((tcp->th_flags & TH_OPENING) == TH_SYN) {
...
/* ------------------------------------------------------------------------ */
static int fr_stgettable(data, ifs)
char *data;
ipf_stack_t *ifs;
{
	ipftable_t table;
	...
	error = COPYOUT(ifs->ifs_ips_stats.iss_bucketlen, table.ita_table,
		ifs->ifs_fr_statesize * sizeof(u_long));
	if (error != 0)
		error = EFAULT;
	return error;

...
struct ipf_stack {
	...
	net_handle_t	ifs_ipf_ipv6;
	    ...
	    /* ip_auth.c */
	int			ifs_auth_opened;
	u_32_t			ifs_fr_authsize;
	int			ifs_fr_authcnt;
	int			ifs_fr_defaultauthage;
	int			ifs_fr_auth_lock;
	int			ifs_fr_auth_init;
	int			ifs_fr_authmax;
	int			ifs_authwait_entered;
	int			ifs_auth_unloading;
	fr_authstat_t		ifs_fr_authstats;
	ipftq_t			ifs_auth_wq;	/* wait queue */
	ipftq_t			ifs_auth_rq;	/* resolved queue */
	auth_list_head_t	ifs_auth_list; /* global list */
	frauth_htab_t	*ifs_auth_req_htab;
	frauth_htab_t	*ifs_auth_fin_htab;
	u_32_t			ifs_fr_authq_ttl;

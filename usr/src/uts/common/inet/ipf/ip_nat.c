...
static	int	nat_gettable __P((char *, ipf_stack_t *));
...
ipf_stack_t *ifs;
{
	...
	case SIOCGNATS :
		...
		ifs->ifs_nat_stats.ns_ticks = ifs->ifs_fr_ticks;
	...
	case SIOCIPFDELTOK :
		...
		break;
	case SIOCGTQTAB :
		error = fr_outobj(data, &ifs->ifs_nat_tqb, IPFOBJ_STATETQTAB);
		break;
	case SIOCGTABL :
		error = nat_gettable(data, ifs);
		break;
...
maskloop:
		...
		for (np = ifs->ifs_nat_rules[hv]; np; np = npnext) {
			...
			natfailed = -1;
			/*
			 * We need to update npnext here because it is
			 * possible somebody could modify the list during
			 * lock upgrade above.
			 */
			npnext = np->in_mnext;
...
/* ------------------------------------------------------------------------ */
static int nat_gettable(data, ifs)
char *data;
ipf_stack_t *ifs;
{
	ipftable_t table;
	...
	case IPFTABLE_BUCKETS_NATIN :
		error = COPYOUT(ifs->ifs_nat_stats.ns_bucketlen[0], table.ita_table,
				ifs->ifs_ipf_nattable_sz * sizeof(u_long));
		break;

	case IPFTABLE_BUCKETS_NATOUT :
		error = COPYOUT(ifs->ifs_nat_stats.ns_bucketlen[1], table.ita_table,
				ifs->ifs_ipf_nattable_sz * sizeof(u_long));
		break;

...
static int iplookup_resetnode __P((caddr_t, ipf_stack_t *));
static int iplookup_flush __P((caddr_t, ipf_stack_t *));
static int iplookup_node __P((caddr_t, ipf_stack_t *));
...
ipf_stack_t *ifs;
{
	...
	case SIOCLOOKUPRESETNODE :
	case SIOCLOOKUPRESETNODEW :
		READ_ENTER(&ifs->ifs_ip_poolrw);
		err = iplookup_resetnode(data, ifs);
		RWLOCK_EXIT(&ifs->ifs_ip_poolrw);
		break;

	case SIOCLOOKUPNODE :
	case SIOCLOOKUPNODEW :
		READ_ENTER(&ifs->ifs_ip_poolrw);
		err = iplookup_node(data, ifs);
		RWLOCK_EXIT(&ifs->ifs_ip_poolrw);
		break;
	default :
		err = EINVAL;
...
/* ------------------------------------------------------------------------ */
/* Function:	iplookup_node						    */
/* Returns:	int     - 0 = success, else error			    */
/* Parameters:	data(I) - pointer to data from ioctl call		    */
/* 		ifs	- pointer to IP stack instance 			    */
/*									    */
/* Find a matching node and copy out a data. The parameters for look up	    */
/* operation and buffer for output node data are gathered from ioctl()	    */
/* argument.								    */
/* ------------------------------------------------------------------------ */
static int iplookup_node(data, ifs)
caddr_t data;
ipf_stack_t *ifs;
{
	ip_pool_node_t node, *m;
	...
	case IPLT_POOL :
		...
		err = COPYOUT(m, op.iplo_struct, sizeof (node));
		
		break;

	case IPLT_HASH :
		/*
		 * this will be implemented as a part of fix for
		 * CR 6933613
		 *
		 */
		err = EINVAL;
...
/* ------------------------------------------------------------------------ */
/* Function:	iplookup_reset_node					    */
/* Returns:	int     - 0 = success, else error			    */
/* Parameters:	data(I) - pointer to data from ioctl call		    */
/*									    */
/* Resets packets and bytes counters in pool node			    */
/* ------------------------------------------------------------------------ */
static int iplookup_resetnode(data, ifs)
caddr_t data;
ipf_stack_t *ifs;
{
	ip_pool_node_t node, *m;
	...
	case IPLT_HASH :
		/*
		 * There is no meaning trying to implement it until
		 * CR 6933613 will get fixed.
		 */
		err = EINVAL;
		break;

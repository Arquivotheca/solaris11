...
cred_t *cred;
{
	...
	if (min == IPL_LOGAUTH) {
		min = 0;
		ifs = ipf_find_stack(crgetzoneid(cred));
		if (ifs == NULL)
			return (EAGAIN);

		WRITE_ENTER(&ifs->ifs_ipf_global);
		if (ifs->ifs_auth_opened == 0)
			ifs->ifs_auth_opened = 1;
		else
			min = EIO;
		RWLOCK_EXIT(&ifs->ifs_ipf_global);
	} else
...
/* Parameters:  fin - pointer to fr_info_t instance with packet data	    */
/*              m - pointer to packet to be reinjected to IP packet inbound */
/*		queue							    */
/*                                                                          */
/* Function inserts packet m back to inbound queue. This is used solely by  */
/* ip_auth.c module, which needs to reinsert inbound packet to ingress	    */
/* queue in order to further proceed with fr_check() operation. fr_check()  */
/* will be stepped second time for such packet. It will find out the result */
/* of authorisation operation for packet on its second entry.		    */
/* ------------------------------------------------------------------------ */
...
mb_t *m;
{
	...
	ipf_stack_t *ifs = fin->fin_ifs;
	net_handle_t net_data;
	int dir = (fin->fin_out == 0) ? NI_QUEUE_IN : NI_QUEUE_OUT;
	...
	return (net_inject(net_data, dir, &inject));
}

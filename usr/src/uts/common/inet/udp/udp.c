...
int		udp_opt_get(conn_t *connp, int level, int name,
		    uchar_t *ptr);
static int	udp_output_connected(conn_t *connp, mblk_t *mp, uio_t *uio,
		    cred_t *cr, pid_t pid);
static int	udp_output_lastdst(conn_t *connp, mblk_t *mp, uio_t *uio,
    cred_t *cr, pid_t pid, ip_xmit_attr_t *ixa);
static int	udp_output_newdst(conn_t *connp, mblk_t *data_mp, uio_t *uio,
		    sin_t *sin, sin6_t *sin6, ushort_t ipversion, cred_t *cr,
		    pid_t, ip_xmit_attr_t *ixa);
...
static int	udp_do_connect(conn_t *, const struct sockaddr *, socklen_t,
    cred_t *, pid_t);
static int udp_copyin(conn_t *connp, uio_t *uio, ip_xmit_attr_t *ixa,
    mblk_t **out_mp);
...
{
	in_port_t nextport, nexttsport;
	boolean_t restart = B_FALSE;
	udp_stack_t *us = udp->udp_us;

	nextport = us->us_next_priv_port;
retry:
	if (nextport < us->us_min_anonpriv_port ||
	    nextport >= IPPORT_RESERVED) {
		nextport = IPPORT_RESERVED - 1;
		us->us_next_priv_port = nextport;
		if (restart)
			return (0);
		restart = B_TRUE;
	}
	...
	if (is_system_labeled() &&
	    (nexttsport = tsol_next_port(crgetzone(udp->udp_connp->conn_cred),
	    nextport, IPPROTO_UDP, B_FALSE)) != 0) {
		nextport = nexttsport;
		goto retry;
	}

	us->us_next_priv_port--;
	return (nextport);
}
 ...
 * without setting SO_REUSEADDR/SO_REUSEPORT. This is needed so that they
    ...
    uint32_t flowinfo, mblk_t *data_mp, int *errorp)
{
	...
	if (ixa->ixa_flags & IXAF_IS_IPV4) {
		...
		/*
		 * Setting IXAF_SET_ULP_CKSUM enables IP layer checksumming.
		 * IP uses UDP pseudo-header checksum passed via uha_checksum,
		 * 0 means "checksum disabled" and
		 * 0xffff means "checksum enabled with value 0".
		 */
		...
		/* For IPv6 UDP checksum cannot be disabled. See RFC2460 */
		if (cksum == 0)
			udpha->uha_checksum = 0xffff;
		else
			udpha->uha_checksum = htons(cksum);
...
udp_update_next_port(udp_t *udp, in_port_t port, boolean_t random)
{
	...
	if (random && udp_random_anon_port != 0) {
		/*
		 * If smallest_anon_port and largest_anon_port are equal,
		 * then the only random port is the smallest_anon_port itself.
		 */
		if (us->us_largest_anon_port == us->us_smallest_anon_port) {
...
udp_output_ancillary(conn_t *connp, sin_t *sin, sin6_t *sin6, mblk_t *mp,
    uio_t *uio, mblk_t *tudr_mp, struct nmsghdr *msg, cred_t *cr, pid_t pid)
{
	udp_t		*udp = connp->conn_udp;
	udp_stack_t	*us = udp->udp_us;
	int		error;
	...
	/* Exactly one of mp and uio must be non-null */
	ASSERT((mp != NULL) ^ (uio != NULL));
	...
	if (uio != NULL) {
		error = udp_copyin(connp, uio, ixa, &uio_mp);
		if (error != 0) {
			UDPS_BUMP_MIB(us, udpOutErrors);
			goto done;
		}
	...
	case 0:
		break;
	case EWOULDBLOCK:
		ixa_check_drain_insert(connp, ixa);
		error = 0;
		break;
	case EADDRNOTAVAIL:
		/*
...
static int
udp_copyin(conn_t *connp, uio_t *uio, ip_xmit_attr_t *ixa, mblk_t **out_mp)
{
	pm_handle_t	pmh = NULL;
	mblk_t	*head = NULL;
	mblk_t	**tail = &head;
	size_t	wroff;
	ssize_t	iosize, maxblk;
	nce_t	*nce;
	int error = 0;

	nce = ixa->ixa_nce;
	if (nce != NULL)
		pmh = nce->nce_ill->ill_pmh;

	iosize = uio->uio_resid;
	wroff = connp->conn_wroff;
	maxblk = dma_block_size;
	if (iosize + wroff > maxblk)
		wroff = 0;
	...
	do {
		...
		ASSERT(blocksize >= 0);
		/*
		 * The reference on the 'nce' ensures that the unplumb of
		 * the ill can't complete which in turn ensures that the
		 * 'pmh' is valid in the call below.
		 */
		mp = allocb_premapped(pmh, wroff + blocksize, B_FALSE,
		    NULL, NULL);
		if (mp == NULL) {
			error = ENOMEM;
			break;
		...
		*tail = mp;
		...
		if ((error = uiomove(mp->b_rptr, (size_t)blocksize,
		    UIO_WRITE, uio)) != 0) {
			ASSERT(error != ENOMEM);
			freemsg(head);
			break;
 ...
 */
static int
udp_output_connected(conn_t *connp, mblk_t *mp, uio_t *uio, cred_t *cr,
    pid_t pid)
{
	udp_t		*udp = connp->conn_udp;
	udp_stack_t	*us = udp->udp_us;
 ...
 */
static int
udp_output_lastdst(conn_t *connp, mblk_t *mp, uio_t *uio, cred_t *cr, pid_t pid,
    ip_xmit_attr_t *ixa)
{
	udp_t		*udp = connp->conn_udp;
	udp_stack_t	*us = udp->udp_us;
...
udp_wput(queue_t *q, mblk_t *mp)
{
	...
	case M_DATA:
		...
		error = udp_output_connected(connp, mp, NULL, cr, pid);
		if (error != 0) {
 ...
 */
static int
udp_output_newdst(conn_t *connp, mblk_t *data_mp, uio_t *uio, sin_t *sin,
    sin6_t *sin6, ushort_t ipversion, cred_t *cr, pid_t pid,
    ip_xmit_attr_t *ixa)
{
	uint_t		srcid;
	uint32_t	flowinfo;
	...
	/* Exactly one of data_mp and uio must be non-null */
	ASSERT((data_mp != NULL) ^ (uio != NULL));
	ASSERT(MUTEX_HELD(&connp->conn_lock));
	ASSERT(ixa != NULL);
	/*
...
udp_stack_init(netstackid_t stackid, netstack_t *ns)
{
	...
	us->us_next_priv_port = IPPORT_RESERVED - 1;
    ...
    boolean_t bind_to_req_port_only)
{
	...
	/*
	 * If conn_reuseaddr/conn_reuseport is not set, then we have to make
	 * sure that the IP address and port number the application requested
	 ...
	 * changes, if conn_reuseaddr/conn_reuseport is set, then we allow
	 * multiple binds to the same port independent of the local IP address.
	 *
	 ...
	 * SO_REUSEPORT in order to guarantee a unique port.
	 */
	...
	for (;;) {
		...
		for (udp1 = udpf->uf_udp; udp1 != NULL;
			...
			if (connp->conn_ipversion != connp1->conn_ipversion) {
				 ...
				 * both and get each separately. On subsequent
		...
		/*
		 * SO_REUSEADDR and SO_REUSEPORT have the same semantics for
		 * a UDP endpoint.
		 */
		if (!found_exclbind &&
    ...
    cred_t *cr, pid_t pid)
{
	...
	connp->conn_lastsrcid = srcid;
	/* Also remember a source to use together with lastdst */
	connp->conn_v6lastsrc = (IN6_IS_ADDR_UNSPECIFIED(&v6src) ||
	    IN6_IS_ADDR_V4MAPPED_ANY(&v6src)) ? connp->conn_saddr_v6 : v6src;
...
udp_connect(sock_lower_handle_t proto_handle, const struct sockaddr *sa,
    socklen_t len, sock_connid_t *id, cred_t *cr, pm_handle_t *pmh)
{
	conn_t	*connp = (conn_t *)proto_handle;
	udp_t	*udp = connp->conn_udp;
	int	error;
...
int
udp_send_cmn(sock_lower_handle_t proto_handle, mblk_t *mp, uio_t *uio,
    struct nmsghdr *msg, cred_t *cr)
{
	...
	if (msg->msg_name == NULL) {
		...
		if (msg->msg_controllen != 0) {
			...
			error = udp_output_connected(connp, mp, uio, cr, pid);
		}
	...
	case AF_INET6:
		...
		if (msg->msg_controllen != 0) {
		    ...
		    ipsec_outbound_policy_current(ixa)) {
			/* udp_output_lastdst drops conn_lock */
			error = udp_output_lastdst(connp, mp, uio, cr, pid,
			    ixa);
		} else {
			/* udp_output_newdst drops conn_lock */
			error = udp_output_newdst(connp, mp, uio, NULL, sin6,
	...
	case AF_INET:
		...
		if (msg->msg_controllen != 0) {
			mutex_exit(&connp->conn_lock);
			ASSERT(ixa == NULL);
			error = udp_output_ancillary(connp, sin, NULL, mp, uio,
			    NULL, msg, cr, pid);
		} else if (conn_same_as_last_v4(connp, sin) &&
		    ipsec_outbound_policy_current(ixa)) {
			/* udp_output_lastdst drops conn_lock */
			error = udp_output_lastdst(connp, mp,
			    uio, cr, pid, ixa);
		} else {
			/* udp_output_newdst drops conn_lock */
			error = udp_output_newdst(connp, mp, uio, sin, NULL,
			    ipversion, cr, pid, ixa);
		}
		ASSERT(MUTEX_NOT_HELD(&connp->conn_lock));
		if (us->us_sendto_ignerr)
    ...
    cred_t *cr)
{
	...
	return (udp_send_cmn(proto_handle, mp, NULL, msg, cr));
}

int
udp_send_uio(sock_lower_handle_t proto_handle, uio_t *uio, struct nmsghdr *msg,
    cred_t *cr)
{
	int	err;

	err = udp_send_cmn(proto_handle, NULL, uio, msg, cr);
	if (err == 0) {
		/*
		 * If we don't want to return an error for whatever reason,
		 * make sure that we have consumed the data. Otherwise
		 * the caller may invoke us again in a loop.
		 */
		uio->uio_resid = 0;
	}
	return (err);

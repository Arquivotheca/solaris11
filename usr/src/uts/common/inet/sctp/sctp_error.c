...
sctp_user_abort(sctp_t *sctp, mblk_t *data)
{
	...
	if (mp == NULL) {
		...
		/*
		 * Although we cannot send out the abort, we still proceed
		 * to kill the association since there is no timer involved
		 * here.  Just pretend that the ABORT is dropped...
		 */
		goto out;
	}

...
sctp_send_shutdown(sctp_t *sctp, int rexmit)
{
	 ...
	 */
	if (sctp->sctp_unsent > 0 || sctp->sctp_unacked > 0)
		return;
    ...
    boolean_t rexmit, sctp_faddr_t *fp)
{
	 ...
	 * The tsn received should be within sctp_lastack_rxd and
	 * sctp_ltsn - 1.
	 */
	if (sch != NULL) {
		cum_tsn = ntohl(*(uint32_t *)(sch + 1));
		if (!SEQ_LT(cum_tsn, sctp->sctp_lastack_rxd) &&
		    !SEQ_GEQ(cum_tsn, sctp->sctp_ltsn)) {
			trysend = sctp_cumack(sctp, cum_tsn, &samp);
		}
	}

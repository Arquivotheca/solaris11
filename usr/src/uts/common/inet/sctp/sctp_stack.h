 ...
 * SCTP private kstat
 *
 * When adding a counter here, sctp_kstat_counter_t, sctp_clr_kstats2() and
 * sctp_add_kstats2() also need to be updated.
 */
typedef struct sctp_kstat_s {
	...
	kstat_named_t	sctp_data_drop;
} sctp_kstat_t;

/*
 * This struct contains only the counter part of sctp_kstat_t.  It is used
 * in sctp_stats_cpu_t instead of sctp_kstat_t to save memory space.  Need to
 * be kept in sync with sctp_kstat_t.
 */
...
#define	SCTP_NUM_EPRIV_PORTS	64
	...
	/* congestion control */
	kmutex_t		sctps_cong_lock;
	list_t			sctps_cong_enabled;	/* enabled algs list */
	tcpcong_list_ent_t	*sctps_cong_default_ent; /* default alg entry */
};

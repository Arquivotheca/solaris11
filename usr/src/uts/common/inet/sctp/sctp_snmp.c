...
/*
 * For a socket using SO_PASSIVE_CONNECT, its internal state is
 * SCTPS_COOKIE_WAIT since the sctp_t reacts as if it is in that state.
 * From a higher layer perspective, it may be more "natural" to see that
 * this association is in SCTPS_LISTEN state.  So we re-map the state.
 */
#define	SCTP_SNMP_REMAP_STATE(connp, sctp)				\
	(((connp)->conn_passive_connect &&				\
	    ((sctp)->tcp_state == SCTPS_COOKIE_WAIT)) ? SCTPS_LISTEN :	\
	    (sctp)->sctp_state)
...
sctp_clr_kstats2(sctp_kstat_t *stats)
{
	...
	stats->sctp_data_drop.value.ui64 = 0;
}
...
sctp_add_kstats2(sctp_kstat_counter_t *from, sctp_kstat_t *to)
{
	...
	to->sctp_reclaim_cnt.value.ui64 += from->sctp_reclaim_cnt;
	to->sctp_listen_cnt_drop.value.ui64 += from->sctp_listen_cnt_drop;
	to->sctp_data_drop.value.ui64 += from->sctp_data_drop;
}
...
sctp_kstat2_init(netstackid_t stackid)
{
	...
	sctp_kstat_t template = {
		...
		{ "sctp_data_drop",			KSTAT_DATA_UINT64 }
	};
...
sctp_snmp_get_mib2(queue_t *q, mblk_t *mpctl, sctp_stack_t *sctps)
{
	...
	int			idx;
	mib2_sctp_t		sctp_mib;
	boolean_t		set_local_prime = B_FALSE;
	...
	while (sctp != NULL) {
		...
		sce.sctpAssocLocalPort = ntohs(connp->conn_lport);
		sce.sctpAssocRemPort = ntohs(connp->conn_fport);
		...
		if (sctp->sctp_primary != NULL) {
			...
			/*
			 * There is no peer address yet.  Check if there is
			 * any bound source address.  If yes, return it.  If
			 * not, return wildcard address.
			 */
			...
			if (sctp->sctp_nsaddrs == 0 ||
				...
				/* Just use the socket endpoint family. */
				...
				/*
				 * If set_local_prim is B_TRUE, later when
				 * we go through the source hash table, we
				 * will fill in the sctpAssocLocPrimAddr.
				 */
				set_local_prime = B_TRUE;
			}
		}
		...
		for (i = 0; i < SCTP_IPIF_HASH; i++) {
			...
			for (l = 0; l < sctp->sctp_saddrs[i].ipif_count; l++) {
				...
				if (set_local_prime) {
					...
					sce.sctpAssocLocPrimAddr = addr;
					set_local_prime = B_FALSE;
				}
...
sctp_snmp_state(sctp_t *sctp)
{
	...
	case SCTPS_COOKIE_WAIT:
		/* See SCTP_SNMP_REMAP_STATE() comments */
		if (sctp->sctp_connp->conn_passive_connect)
			return (MIB2_SCTP_listen);
		else
...
sctp_sum_mib(sctp_stack_t *sctps, mib2_sctp_t *sctp_mib)
{
	...
	/* Static components of mib2_sctp_t. */

    ...
    iaflags_t iraflags)
{
	...
	/*
	 * INIT-ACK must always be the first chunk in the packet and IP
	 * guarantees that the chunk header is in the first mblk.
	 */
	ich = (sctp_chunk_hdr_t *)(sctph + 1);
	if (ich->sch_id != CHUNK_INIT_ACK)
		return (NULL);

	/*
	 * If the whole chunk is not contained in the first mblk, just copy
	 * the whole chunk into a dedicated buffer to simplify the following
	 * code.  Note that we must not do pullupmsg() since the caller of
	 * this function may have references, such as sctph and ipha, on the
	 * passed in mblk.
	 */
	remaining = mp->b_wptr - (uint8_t *)ich;
	if (remaining < ntohs(ich->sch_len)) {
		if ((new_mp = msgpullup(mp, -1)) == NULL)
			return (NULL);

		/*
		 * Use the original chunk header offset to find the chunk
		 * header in the new mblk.
		 */
		ich = (sctp_chunk_hdr_t *)(new_mp->b_rptr + ((uint8_t *)ich -
		    mp->b_rptr));
		remaining = new_mp->b_wptr - (uint8_t *)ich;
		mp = new_mp;
	}
	/* Make sure that the INIT-ACK chunk and parameters are OK. */
	if (!sctp_check_input(ich, remaining, B_TRUE)) {
		DTRACE_PROBE2(sctp_check_input, sctp_chunk_hdr_t *, ich,
		    ssize_t, remaining);
		goto out;
	}

	ph = (sctp_parm_hdr_t *)((uint8_t *)(ich + 1) +
	    sizeof (sctp_init_chunk_t));
	remaining = mp->b_wptr - (uint8_t *)ph;

	/*
	 * Find out at most how many address parameter there can be.  This
	 * is the upper bound.
	 */
	totaddr = remaining / PARM_ADDR4_LEN;

	addrbuf = (in6_addr_t **)kmem_zalloc(totaddr * sizeof (in6_addr_t *),
	    KM_NOSLEEP);
	if (addrbuf == NULL)
		goto out;
	faddrpp = addrbuf;
	...
	while (ph != NULL) {
		 ...
		 * network byte order, unless otherwise stated.  Therefore
		 * convert the param type to host byte order.  Also do not add
		 * src address present in IP header as it has already been
		 * thru sctp_conn_match() above.
		 */
		...
		case PARM_ADDR4:
			...
			if (IN6_ARE_ADDR_EQUAL(&src, srcp))
				break;
			*faddrpp = (in6_addr_t *)
			    kmem_zalloc(sizeof (in6_addr_t), KM_NOSLEEP);
			if (*faddrpp == NULL)
				break;
			IN6_INADDR_TO_V4MAPPED((struct in_addr *)(ph + 1),
			    *faddrpp);
	...
	if (nfaddr > 0) {
		sctp = sctp_conn_match(addrbuf, nfaddr, dstp, ports, zoneid,
		    iraflags, sctps);
...
void
sctp_conn_hash_insert(sctp_tf_t *tf, sctp_t *sctp, boolean_t caller_holds_lock)
{
 ...
 */
void
sctp_bind_hash_insert(sctp_tf_t *tbf, sctp_t *sctp, boolean_t caller_holds_lock)
{
	sctp_t	**sctpp;
	sctp_t	*sctpnext;

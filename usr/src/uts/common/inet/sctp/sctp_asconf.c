...
#define	SCTP_CLR_SENT_FLAG(mp)	((mp)->b_flag &= ~SCTP_CHUNK_FLAG_SENT)
	...
	DTRACE_PROBE3(sctp_cwnd_ssthresh_update, sctp_faddr_t *, ofp,
	    uint32_t, ofp->sf_cwnd, uint32_t, ofp->sf_ssthresh);
	/*
    ...
    sctp_faddr_t *fp, int *cont, int act, in6_addr_t *raddr)
{
	...
	*cont = 1;
	...
	if (type == PARM_ADD_IP) {
		...
		/* Add the new address */
		err = sctp_add_faddr(sctp, NULL, &addr, NULL,
		...
		if (sctp->sctp_current == nfp) {
			/*
			 * fp is the source address of this request.  It may
			 * not be alive (unreachable because of asymmetric
			 * route).
			 */
			if (fp->sf_state == SCTP_FADDRS_ALIVE) {
				sctp_set_faddr_current(sctp, fp);
			} else {
				sctp_set_faddr_current(sctp,
				    sctp_rotate_faddr(sctp, fp));
			}
		}
...
{
	const struct sctp_setpeerprim	*prim = inp;

...
static void	sctp_cap_notify(void *, ip_xmit_attr_t *, ixa_notify_type_t,
    ixa_notify_arg_t);

static void	sctp_cong_stack_init(sctp_stack_t *);
static void	sctp_cong_stack_fini(sctp_stack_t *);
...
sctp_clean_death(sctp_t *sctp, int err)
{
	...
	if ((sctp->sctp_state >= SCTPS_COOKIE_WAIT) &&
		...
		if (sctp->sctp_pd_on)
			sctp_partial_delivery_failure(sctp);
...
/* ARGSUSED */
int
sctp_disconnect(sctp_t *sctp, cred_t *cr)
{
	int		error = 0;
...
/* ARGSUSED */
void
sctp_close(sctp_t *sctp, cred_t *cr)
{
...
sctp_closei_local(sctp_t *sctp)
{
	...
	/*
	 * Release all ixa references since we will not send anything anymore.
	 * We don't know when the upper layer will close this sctp_t.  So
	 * we should not hold on to those ixa because it will block some
	 * IP actions, such as removing an interface.  We still keep the
	 * addresses in case we need them for debugging a dump.
	 */
...
{
	mblk_t		*meta, *nxt_meta, *mp, *nxt_mp;

	for (meta = sctp->sctp_xmit_head; meta != NULL; meta = nxt_meta) {
		...
		ASSERT(DB_REF(meta) == 1);
		ASSERT(meta->b_cont == NULL);
		freeb(meta);
	}
...
/*
 * Clean up all the messages in the stream queue and the reassembly lists.
 * Also clean up the sctp_rx_ready list which holds blocked messages because
 * of partial delivery.  If 'free' is true, then delete the streams as well.
 */
void
sctp_instream_cleanup(sctp_t *sctp, boolean_t free)
{
	...
	while ((mp = sctp->sctp_rx_ready) != NULL) {
		sctp->sctp_rx_ready = mp->b_next;
		freemsg(mp);
	}
 ...
 * Called from SCTP_REFRELE() -> CONN_DEC_REF().
 */
void
sctp_conn_destroy(conn_t *connp, netstack_t *ns)
{
	...
	if (ns != NULL) {
		/*
		 * Release the hold on netstack_t only after we are done
		 * updating the sctp_stack_t stats.
		 */
		netstack_rele(ns);
...
sctp_init_values(sctp_t *sctp, sctp_t *psctp, int sleep)
{
	...
	if (psctp != NULL) {
		...
		sctp->sctp_max_burst = psctp->sctp_max_burst;
		...
		sctp->sctp_faddr_policy = psctp->sctp_faddr_policy;
		sctp->sctp_faddr_chk_cnt = psctp->sctp_faddr_chk_cnt;
		...
		sctp->sctp_explicit_eor = psctp->sctp_explicit_eor;
		sctp->sctp_frag_interleave = psctp->sctp_frag_interleave;

		sctp->sctp_recvrcvinfo = psctp->sctp_recvrcvinfo;
		sctp->sctp_recvnxtinfo = psctp->sctp_recvnxtinfo;

		/* Inherit all event subscriptions. */
		sctp->sctp_events = psctp->sctp_events;

		sctp->sctp_sack_max = psctp->sctp_sack_max;
		sctp->sctp_sack_interval = psctp->sctp_sack_interval;

		(void) strlcpy(sctp->sctp_cong_alg_name,
		    psctp->sctp_cong_alg_name,
		...
		sctp->sctp_max_burst = sctps->sctps_maxburst;
		...
		sctp->sctp_faddr_policy = sctps->sctps_faddr_policy;
		switch (sctp->sctp_faddr_policy) {
		case SCTP_FADDR_POLICY_STICKY_PRIMARY:
			sctp->sctp_faddr_chk_cnt =
			    sctps->sctps_faddr_sticky_prim_cnt;
			break;
		case SCTP_FADDR_POLICY_PREF_PRIMARY:
			sctp->sctp_faddr_chk_cnt =
			    sctps->sctps_faddr_pref_prim_cnt;
			break;
		case SCTP_FADDR_POLICY_ROTATE:
		default:
			/* sctp_faddr_chk_cnt should be 0. */
			break;
		}

		(void) strlcpy(sctp->sctp_cong_alg_name,
		    sctps->sctps_cong_default,
		    sizeof (sctp->sctp_cong_alg_name));
		...
		sctp->sctp_sack_max = sctps->sctps_deferred_acks_max;
		sctp->sctp_sack_interval = MSEC_TO_TICK(
		    sctps->sctps_deferred_ack_interval);
	}
...
sctp_update_pmtu(sctp_t *sctp, sctp_faddr_t *fp, boolean_t decrease_only)
{
	...
	/* Make sure that mss is a multiple of SCTP_ALIGN. */
	mss &= ~(SCTP_ALIGN - 1);
	...
	DTRACE_PROBE4(sctp_update_pmtu, sctp_t *, sctp, sctp_faddr_t *, fp,
	    int32_t, fp->sf_pmss, uint32_t, mss);
	...
	/*
	 * PMTU becomes bigger, check to see if we can increase
	 * sctp_mss, which is the MIN of all peer addresses' PMTU.
	 */
	if (bigger)
		sctp_min_fp_mss(sctp);
	else if (mss < sctp->sctp_mss)
		sctp->sctp_mss = mss;
...
/* ARGSUSED1 */
static void
sctp_cap_notify(void *arg, ip_xmit_attr_t *ixa, ixa_notify_type_t ntype,
 ...
 */
sctp_t *
sctp_create(void *ulpd, sctp_t *psctp, int family, int type, int flags,
    ...
    cred_t *credp)
{
	...
	/*
	 * sctp_mss is the minimum of all peer addresses' path MSS.  Initialize
	 * to the maximum value.  When a peer address is added, that peer's
	 * path MSS will be compared with sctp_mss.  If sctp_mss is bigger,
	 * sctp_mss is set to that peer's path MSS.
	 */
	sctp->sctp_mss = IP_MAXPACKET;
	...
	if (psctp != NULL) {
		...
		/*
		 * Inherit various info from parent. Parent is either in
		 * SCTPS_BOUND, or SCTPS_LISTEN state.
		 */
		ASSERT((psctp->sctp_state == SCTPS_BOUND) ||
		    (psctp->sctp_state == SCTPS_LISTEN));
		if (sctp_init_values(sctp, psctp, sleep) != 0 ||
		    sctp_dup_saddrs(psctp, sctp, sleep) != 0) {
			WAKE_SCTP(psctp);
			SCTP_CONDEMNED(sctp);
			ipcl_conn_destroy(connp);
			return (NULL);
		...
		if (sctp_init_values(sctp, NULL, sleep) != 0) {
			SCTP_CONDEMNED(sctp);
			ipcl_conn_destroy(connp);
			return (NULL);
		}
...
#define	IPVER(ip6h)	((((uint32_t *)ip6h)[0] >> 4) & 0x7)
	...
	while ((mp = sctp->sctp_recvq) != NULL) {
		...
		/*
		 * In the case where the state is now idle we free
		 * the message rather than passing it to sct_input_data().
		 */
		if (sctp->sctp_state == SCTPS_IDLE) {
			ip_drop_input("sctp_process_recvq - SCTPS_IDLE",
			    mp, NULL);
			freemsg(mp);
			mutex_enter(&sctp->sctp_recvq_lock);
			continue;
...
sctp_conn_cache_constructor(void *buf, void *cdrarg, int kmflags)
{
	...
	bzero(buf, sizeof (conn_t) + sizeof (sctp_t));
...
sctp_conn_cache_destructor(void *buf, void *cdrarg)
{
	...
	ASSERT(sctp->sctp_xmit_last_sent == NULL);

	ASSERT(sctp->sctp_rx_ready == NULL);
	ASSERT(sctp->sctp_rx_ready_tail == NULL);
...
sctp_conn_init(conn_t *connp)
{
	 ...
	 */
	connp->conn_ixa->ixa_notify = sctp_cap_notify;
 ...
 */
static void
sctp_cong_stack_init(sctp_stack_t *sctps)
{
	tcpcong_list_ent_t *tce;

	/* Initialize congestion control */
	mutex_init(&sctps->sctps_cong_lock, NULL, MUTEX_DEFAULT, NULL);
	list_create(&sctps->sctps_cong_enabled, sizeof (tcpcong_list_ent_t),
	...
	TCPCONG_PROP_INFO_ALLOC(tce, MOD_PROTO_SCTP);
	list_insert_tail(&sctps->sctps_cong_enabled, tce);
	sctps->sctps_cong_default_ent = tce;
}

static void
sctp_cong_stack_fini(sctp_stack_t *sctps)
{
	tcpcong_list_ent_t *tce;

	while ((tce = list_head(&sctps->sctps_cong_enabled)) != NULL) {
		TCPCONG_PROP_INFO_FREE(tce, MOD_PROTO_SCTP);
		tcpcong_unref(tce->tce_hdl);
		list_remove(&sctps->sctps_cong_enabled, tce);
		kmem_free(tce, sizeof (*tce));
	}
	sctps->sctps_cong_default_ent = NULL;
	list_destroy(&sctps->sctps_cong_enabled);
	mutex_destroy(&sctps->sctps_cong_lock);
}

...
validate_init_params(sctp_t *sctp, sctp_chunk_hdr_t *ch,
    ...
    mblk_t **errmp, int *supp_af, uint_t *sctp_options, uint32_t *al_val,
    ip_recv_attr_t *ira)
{
	sctp_parm_hdr_t		*cph;
	...
	*sctp_options = 0;
	...
	while (cph != NULL) {
		...
		case PARM_ADAPT_LAYER_IND:
			*sctp_options |= SCTP_AL_OPTION;
			/* Return the value in host byte order. */
			if (al_val != NULL)
				*al_val = ntohl(*(uint32_t *)(cph + 1));
			break;
    ...
    mblk_t *initmp, ip_recv_attr_t *ira)
{
	...
	if (validate_init_params(sctp, ch, init, initmp, NULL, &errmp,
	    &supp_af, &sctp_options, NULL, ira) == 0) {
		return;
	}
	if (errmp != NULL)
	 ...
	 */
	if ((pad = SCTP_PAD_LEN(ipsctplen)) != 0) {
		ipsctplen += pad;
		if (errmp != NULL)
			iacklen += pad;
	}
...
sctp_find_al_ind(sctp_parm_hdr_t *sph, ssize_t len, uint32_t *adaptation_code)
{
	...
	while (sph != NULL) {
		if (sph->sph_type == htons(PARM_ADAPT_LAYER_IND) &&
		    ...
		    sizeof (uint32_t))) {
			/* Adaptation code is stored in host byte order. */
			*adaptation_code = ntohl(*(uint32_t *)(sph + 1));
			return (0);
		}
...
/*
 * Got an INIT-ACK, echo back the COOKIE chunk.
 */
void
    ...
    ip_recv_attr_t *ira)
{
	...
	/*
	 * Call sctp_get_addrparams() to add all the peer addresses.  Note
	 * that when each peer address is added, the conn fanout lock is held
	 * (and released) to check for address overlap with existing
	 * associations.  We don't hold the lock here as the lock may then be
	 * held for too long.
	 *
	 * This sctp_t isn't a listener so we don't need to hold listen fanout
	 * lock.
	 */
	...
	if (sctp_options & SCTP_AL_OPTION) {
		sctp->sctp_rx_adaptation_code = al_val;
		sctp->sctp_recv_adaptation = B_TRUE;
	}
	...
	if (sctp->sctp_unsent > 0) {
		...
		do {
			...
			/*
			 * Before the association is established, the upper
			 * layer can send down data and it will be queued.
			 * At that time, we cannot be sure that the stream
			 * number used is valid.  Now that we know what
			 * stream numbers can be used, we need to check the
			 * stream number of queued messaage and remove
			 * those queued messsages with invalid stream number.
			 */
			if (msg_hdr->smh_sid >= sctp->sctp_num_ostr) {
				unsent += msg_hdr->smh_msglen;
				if (prev != NULL)
					prev->b_next = meta->b_next;
				else
					sctp->sctp_xmit_head = meta->b_next;
				if (meta->b_next != NULL)
					meta->b_next->b_prev = prev;
				meta->b_prev = meta->b_next = NULL;
				sctp_sendfail_event(sctp, meta,
				    SCTP_ERR_BAD_SID, B_TRUE);
				if (prev != NULL)
					meta = prev->b_next;
				else
					meta = sctp->sctp_xmit_head;
		...
		/* No valid data, just send out the cookie. */
	...
	mp = sctp_get_next_chunk(sctp, sctp->sctp_xmit_last_sent, &meta,
	    &msg_hdr);
	if (mp == NULL)
		goto sendcookie;
	...
	/* Only bundle one chunk... */
	if ((ceclen + seglen) > fp->sf_pmss ||
	    (seglen - sizeof (*sdc)) > cansend) {
		goto sendcookie;
	}
	/*
	 * OK, if this fails, just don't remember that we've tried to send
	 * something.
	 */
...
sendcookie:
	...
	if (sctp->sctp_cookie_mp == NULL) {
		/*
		 * Just freemsg() the data chunk since we've not updated
		 * sctp_xmit_last_sent and smh_xmit_last_sent.
		 */
		if (cemp->b_cont != NULL) {
			freemsg(cemp->b_cont);
			cemp->b_cont = NULL;
...
/*
 * Listener gets a COOKIE chunk, verify its validity and find the INIT-ACK
 * embedded.
 */
int
sctp_process_cookie(sctp_t *sctp, sctp_chunk_hdr_t *ch, mblk_t *cmp,
    sctp_init_chunk_t **iackpp, int *recv_adaptation,
	...
	*recv_adaptation = 0;
	...
	if (sctp->sctp_state >= SCTPS_COOKIE_WAIT) {
		if (sctp->sctp_state == SCTPS_ESTABLISHED &&
		    ...
		    *fttag == 0 && *lttag == 0) {
			/*
			 * Duplicate Cookie-Echo, our Cookie-ACK is lost,
			 * resend it.
			 */
		...
		if (init->sic_inittag != sctp->sctp_fvtag &&
		    ...
		    *lttag == sctp->sctp_lvtag) {
			...
			if (sctp->sctp_state < SCTPS_ESTABLISHED) {
				/*
				 * Use the INIT stored in the COOKIE to get
				 * peer's addresses.
				 */
				if (sctp_get_addrparams(sctp, NULL, cmp,
				    initch, NULL) != 0) {
					return (-1);
				}

...
sctp_process_heartbeat(sctp_t *sctp, sctp_chunk_hdr_t *cp)
{
	...
	/*
	 * If this is the primary address and we can reach it, check if we
	 * need to switch back to use it.
	 *
	 * Note that sf_chk_cnt is set to a positive value when sctp_current
	 * is switched from sctp_primary.  If sf_chk_cnt is 0, we don't need
	 * to do anything.
	 */
	if (fp->sf_chk_cnt > 0 && fp->sf_state == SCTP_FADDRS_ALIVE &&
	    --fp->sf_chk_cnt == 0) {
#if DEBUG
		int policy = sctp->sctp_faddr_policy;

		ASSERT(policy == SCTP_FADDR_POLICY_STICKY_PRIMARY ||
		    policy == SCTP_FADDR_POLICY_PREF_PRIMARY);
		ASSERT(sctp->sctp_primary == fp);
#endif

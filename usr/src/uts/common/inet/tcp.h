...
#define	TCP_MIN_HEADER_LENGTH_IN_WORDS	5
...
#define	tcp_ip_addr_cache	tcp_reass_tail
	...
	tcpcong_handle_t tcp_cong_hdl;	/* current module handle */
	tcpcong_ops_t	*tcp_cong_ops;	/* current algorithm ops */
	tcpcong_args_t	tcp_cong_args;	/* args passed to the alg */
	/* preallocated algorithm state buffer */
	uint64_t	tcp_cong_state_buf[TCPCONG_MAX_STATE_SIZE];
} tcp_t;
...
extern void	tcp_cong_destroy(tcp_t *);
...
/* Unacked bytes */
#define	TCP_NOT_ACKED(tcp)	((tcp)->tcp_snxt - (tcp)->tcp_suna)

/* Congestion control ops */
#define	TCP_CONG_STATE_SIZE(tcp)				\
	(tcp)->tcp_cong_ops->co_state_size(TCPCONG_PROTO_TCP)
#define	TCP_CONG_STATE_INIT(tcp)				\
	(tcp)->tcp_cong_ops->co_state_init(&(tcp)->tcp_cong_args)
#define	TCP_CONG_STATE_FINI(tcp)				\
	(tcp)->tcp_cong_ops->co_state_fini(&(tcp)->tcp_cong_args)
#define	TCP_CONG_ACK(tcp)					\
	(tcp)->tcp_cong_ops->co_ack(&(tcp)->tcp_cong_args);
#define	TCP_CONG_DEC_FUNC(tcp, func) {				\
	uint32_t flight_size = TCP_NOT_ACKED(tcp);		\
	(tcp)->tcp_cong_args.ca_flight_size = &flight_size;	\
	(tcp)->tcp_cong_ops->func(&(tcp)->tcp_cong_args);	\
	(tcp)->tcp_cong_args.ca_flight_size = NULL;		\
}
#define	TCP_CONG_LOSS(tcp)		TCP_CONG_DEC_FUNC(tcp, co_loss)
#define	TCP_CONG_CONGESTION(tcp)	TCP_CONG_DEC_FUNC(tcp, co_congestion)
#define	TCP_CONG_RTO(tcp, flight_size)				\
	(tcp)->tcp_cong_args.ca_flight_size = &(flight_size);	\
	(tcp)->tcp_cong_ops->co_rto(&(tcp)->tcp_cong_args);	\
	(tcp)->tcp_cong_args.ca_flight_size = NULL;
#define	TCP_CONG_AFTER_IDLE(tcp)				\
	(tcp)->tcp_cong_ops->co_after_idle(&(tcp)->tcp_cong_args)

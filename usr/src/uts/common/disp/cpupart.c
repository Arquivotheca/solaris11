 ...
 * haltset to prevent contention due to false sharing.
 */
...
cpupart_move_cpu(cpu_t *cp, cpupart_t *newpp, int forced)
{
	...
	if (disp_bound_check(cp, B_FALSE, DISP_BOUND_PART) == NULL) {
		/*
...
again:
	if (move_threads) {
		int loop_count;
		/*
		 * Check for threads strong or weak bound to this CPU. We
		 * are not interested in bound threads that have affinity set,
		 * unless they are also user bound to this CPU.
		 */
		for (loop_count = 0; disp_bound_check(cp, B_FALSE,
		    DISP_BOUND_CPU | DISP_BOUND_SKIP_AFF) != NULL;
		    loop_count++) {
			if (loop_count >= 5) {
	...
	if (move_threads) {
		 ...
		 * again.
		 */
		if (disp_bound_check(cp, B_TRUE, DISP_BOUND_CPU) != NULL) {
			start_cpus();
			goto again;
		}
	}
	...
	cpupart_update_online_ids(newpp, oldpp, cp);
    ...
    void *projbuf, void *zonebuf)
{
	...
	if (oldpp != newpp) {
		...
		if (LGRP_CPUS_IN_PART(tp->t_lpl->lpl_lgrpid, newpp) &&
		    tp->t_lgrp_aff && tp->t_lgrp_aff->aff == NULL) {
			/*
...
/*
 * Create a new partition. On MP systems, this also allocates a kernel
 * preemption queue for that partition.
 */
int
cpupart_create(psetid_t *psid)
{
	...
	pp->cp_gen = 0;
	pp->cp_online_ids[CP_ONLINE_LOWER] = UINT32_MAX;
	pp->cp_online_ids[CP_ONLINE_UPPER] = 0;
...
cpupart_destroy(psetid_t psid)
{
	...
	mutex_enter(&cpu_lock);
	err = cpupart_destroy_locked(psid);
	mutex_exit(&cpu_lock);
	return (err);
}
 ...
 */
int
cpupart_destroy_locked(psetid_t psid)
{
	cpu_t	*cp, *first_cp;
	cpupart_t *pp, *newpp;
	int	err = 0;
...
/*
 * This routine updates the cp_online_id[] array in the given CPU partitions,
 * which holds the lowest and highest sequential id's of the online processors
 * in such partitions. This routine must be called with cpu_lock held and
 * whenever a CPU enters/exits a partition or is on/offlined.
 */
void
cpupart_update_online_ids(cpupart_t *newpp, cpupart_t *oldpp, cpu_t *cp)
{
	cpu_t *list;
	uint_t id;

	ASSERT(MUTEX_HELD(&cpu_lock));
	ASSERT(newpp != NULL || oldpp != NULL);
	ASSERT(cp != NULL);

	/*
	 * Updating the online id's for a CPU entering a partition is simple,
	 * just check whether its sequential id is lower/higher than the
	 * ones currently indicated in the cpupart structure.
	 */
	if (newpp != NULL) {
		if (newpp->cp_online_ids[CP_ONLINE_LOWER] > cp->cpu_seqid)
			newpp->cp_online_ids[CP_ONLINE_LOWER] = cp->cpu_seqid;

		if (newpp->cp_online_ids[CP_ONLINE_UPPER] < cp->cpu_seqid)
			newpp->cp_online_ids[CP_ONLINE_UPPER] = cp->cpu_seqid;
	}

	/*
	 * If the exiting CPU has the lowest and/or highest order sequential id
	 * in the old partition we need to take a trip through the CPU list to
	 * find out the next lowest/highest.
	 */
	if (oldpp != NULL && oldpp->cp_ncpus > 0) {
		if (oldpp->cp_online_ids[CP_ONLINE_LOWER] == cp->cpu_seqid) {
			list = oldpp->cp_cpulist;
			id = UINT32_MAX;
			...
			do {
				...
				if (list->cpu_seqid < id)
					id = list->cpu_seqid;

			} while ((list = list->cpu_next_part)
			    != oldpp->cp_cpulist);

			oldpp->cp_online_ids[CP_ONLINE_LOWER] = id;
		}

		if (oldpp->cp_online_ids[CP_ONLINE_UPPER] == cp->cpu_seqid) {
			list = oldpp->cp_cpulist;
			id = 0;
			...
			oldpp->cp_online_ids[CP_ONLINE_UPPER] = id;
		}
	}
}

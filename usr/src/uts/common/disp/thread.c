...
/*
 * Allow all threads in proc_pageout, proc_fsflush, and proc_sched to tap
 * the pageout reserve.
 */
boolean_t all_kernel_pushpage = B_TRUE;

/*
 * Thresholds and tunables for workload characterization, see thread.h
 * for more details.
 */
uint_t thread_transience_pct = T_TRANSIENCE_PCT;
uint_t thread_transience_thresh = T_TRANSIENCE_THRESH;
uint_t thread_cpu_intensive_pct = T_CPU_INTENSIVE_PCT;

boolean_t thread_transience_user = B_TRUE;
boolean_t thread_transience_kernel = B_TRUE;
...
thread_init(void)
{
	...
	/*
	 * Idle threads are not transient.
	 */
	tp->t_wkld_flags |= T_WKLD_PERMANENT;
	tp->t_transience = 0;
...
/*
 * Called to enable the current thread to tap the pageout reserve when
 * allocating memory.
 */
void
thread_setpushpage(void)
{
	...
	pri_t	pri)
{
	...
	/*
	 * If all_kernel_pushpage is set, we allow threads associated with
	 * processes 0, 2, and 3 to tap the pageout reserve.
	 */
	if (all_kernel_pushpage &&
	    (pp == proc_pageout || pp == proc_fsflush || pp == &p0)) {
		t->t_flag |= T_PUSHPAGE;
	}

	/*
	 * Kernel threads are flagged as transient by default.
	 */
	if (thread_transience_kernel == B_TRUE)
		t->t_transience = thread_transience_thresh;
	 ...
	 */
	proj = project_hold(proj0p);
	/*
	 * The following fields are initialized to defaults here. They are
	 * correctly set in lwp_create() for an LWP.
	 */
	t->t_proj = proj;
	t->t_task = task0p;
	t->t_zone = &zone0;

	lgrp_affinity_init(&t->t_lgrp_aff);
...
thread_free(kthread_t *t)
{
	...
	lgrp_affinity_free(&t->t_lgrp_aff);
...
/*
 * Transient threads are simply threads that run for a very short amount of
 * time. These threads switch in and out of context very quickly and can
 * interfere with scheduling decisions much like background noise interferes
 * with a conversation. We identify these as they are enqueued, right after
 * leaving a processor, and we do so in a very conservative manner to prevent
 * mischaracterizing an important thread. The algorithm below is very simple:
 * the t_transience kthread field indicates how transient a thread is, and
 * is updated based on the thread's t_pctcpu value. If it is below the
 * thread_transience_pct threshold, we increment t_transience. If it's above,
 * we immediately reset t_transience to zero signaling a non-transient
 * behavior.
 */
void
thread_transience_update(kthread_t *tp)
{
	ASSERT(THREAD_LOCK_HELD(tp));
	ASSERT(tp != tp->t_cpu->cpu_idle_thread);

	/*
	 * This thread has a fixed t_wkld_flags value, no need to go further.
	 */
	if (T_WKLD_IS_PERMANENT(tp))
		return;

	/*
	 * Check if thread transience optimizations should be turned off
	 * for user or kernel threads.
	 */
	if (tp->t_lwp == NULL) {
		if (thread_transience_kernel == B_FALSE) {
			tp->t_transience = 0;
			return;
		}
	} else {
		if (thread_transience_user == B_FALSE) {
			tp->t_transience = 0;
			return;
		}
	}
	...
	/*
	 * If the thread's t_pctcpu value is above the percentage at which
	 * it's considered transient, we reset the transience counter to
	 * reflect the change in behavior. Otherwise, we increase it while
	 * it is below the transience threshold.
	 */
	if (tp->t_pctcpu >= thread_transience_pct) {
		tp->t_transience = 0;

		/*
		 * Now that we know that the thread is not transient
		 * check if it's considered CPU intensive.
		 */
		if (tp->t_pctcpu >= thread_cpu_intensive_pct)
			tp->t_wkld_flags |= T_WKLD_CPU_INTENSIVE;
		else
			tp->t_wkld_flags &= ~T_WKLD_CPU_INTENSIVE;

	} else {
		if (tp->t_transience < thread_transience_thresh)
			tp->t_transience++;
	}
}

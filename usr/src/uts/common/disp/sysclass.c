...
/*
 * Class specific code for the sys class. There are no class specific data
 * structures associated with this class and the scheduling policy is trivially
...
static int	sys_pri_util(kthread_t *);

static boolean_t sys_pri_util_disable = B_TRUE;
...
/* ARGSUSED */
pri_t
sys_init(id_t cid, int clparmsz, classfuncs_t **clfuncspp)
{
...
/* ARGSUSED */
static int
sys_enterclass(kthread_t *tp, id_t cid, void *parmsp, cred_t *reqpcredp,
    void *bufp)
{
	return (0);
}
...
/* ARGSUSED */
static int
sys_fork(kthread_t *tp, kthread_t *ct, void *bufp)
{
	/*
...
/* ARGSUSED */
static void
sys_forkret(kthread_t *tp, kthread_t *ct)
{
	register proc_t *pp = ttoproc(tp);
...
/* ARGSUSED */
static pri_t
sys_swappri(kthread_t *tp, int flags)
{
	return (-1);
}
...
/* ARGSUSED */
static int
sys_pri_util(kthread_t *tp)
{
	if (sys_pri_util_disable == B_FALSE && T_WKLD_IS_CPU_INT(tp))

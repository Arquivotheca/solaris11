...
#define	FX_QUANTUM_SET(fxpp)						\
{									\
	int quantum;							\
	uint64_t clock;							\
									\
	quantum = (fxpp)->fx_pquantum;					\
	if (quantum == FX_TQINF) {					\
		(fxpp)->fx_timer = UINT64_MAX;				\
	} else {							\
		LWP_AC_CLOCK((fxpp)->fx_lwp, 0, clock);			\
		(fxpp)->fx_timer = clock + quantum;			\
	}								\
}

#define	FX_QUANTUM_LEFT(fxpp, left)					\
{									\
	uint64_t clock;							\
									\
	if ((fxpp)->fx_pquantum == FX_TQINF) {				\
		...
		LWP_AC_CLOCK((fxpp)->fx_lwp, 0, clock);			\
		left = (fxpp)->fx_timer - clock;			\
	}								\
}

#define	FX_QUANTUM_EXPIRED(fxpp, expired)				\
{									\
	uint64_t clock;							\
									\
	LWP_AC_CLOCK((fxpp)->fx_lwp, 0, clock);				\
	expired = ((fxpp)->fx_timer <= clock);				\
}
...
static boolean_t fx_pri_util_disable = B_FALSE;
...
static int	fx_pri_util(kthread_t *);
...
fx_fork(kthread_t *t, kthread_t *ct, void *bufp)
{
	 ...
	 */
	cfxpp->fx_pquantum = pfxpp->fx_pquantum;
	...
	cfxpp->fx_lwp = ttolwp(ct);
	ct->t_acflag |= TA_NO_PROCESS_LOCK;
	FX_QUANTUM_SET(cfxpp);
...
fx_preempt(kthread_t *t)
{
	...
	if (t->t_schedctl && schedctl_get_nopreempt(t)) {
		FX_QUANTUM_LEFT(fxpp, timeleft);
		if (fxpp->fx_pquantum == FX_TQINF || timeleft > -SC_MAX_TICKS) {
...
fx_setrun(kthread_t *t)
{
	...
	if ((gethrtime_unscaled() - t->t_disp_time) > disp_cache_warmth_delta)
		setbackdq(t);
	else
		setfrontdq(t);
}
...
fx_tick(kthread_t *t)
{
	...
	if (FX_HAS_CB(fxpp)) {
		...
		thread_unlock_nopreempt(t);	/* LWP A/C can't be preempted */
	}

	FX_QUANTUM_EXPIRED(fxpp, expired);
...
/*
 * Map the given thread's priority to a utilization level if it's above the
 * CPU utilization threshold.
 */
static int
fx_pri_util(kthread_t *tp)
{
	if (fx_pri_util_disable == B_FALSE && T_WKLD_IS_CPU_INT(tp))
	...
	pri_t pri, clock_t quantum)
{
	...
	fxpp->fx_tp->t_acflag &= ~TA_NO_PROCESS_LOCK;

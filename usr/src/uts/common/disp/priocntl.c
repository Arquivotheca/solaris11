    ...
    caddr_t arg2, uio_seg_t seg)
{
		...
		if (cur_inset_only(&procset) == B_TRUE) {
			...
			if ((procset.p_lidtype == P_LWPID) ||
				...
				error = dotolwp(&procset, parmsset,
				    (char *)&pcparms, &retprocp);

				if (retprocp != NULL) {
					/*
					 * dotolwp() can return with p_lock
					 * held. This is required for the
					 * priocntl GETPARMS case. So, here
					 * we just release the p_lock.
					 */
					if (MUTEX_HELD(&retprocp->p_lock))
						mutex_exit(&retprocp->p_lock);
				}
			}
	...
	case PC_GETXPARMS:
		...
		if (cur_inset_only(&procset) == B_TRUE) {
			...
			if (retthreadp != NULL &&
			    retprocp != NULL &&
			    (ttoproc(retthreadp) != retprocp) &&
			    MUTEX_HELD(&retprocp->p_lock))
				mutex_exit(&retprocp->p_lock);
			...
			if (error) {
				...
				if (retprocp != NULL)
				    /* CSTYLED */
				    ASSERT(MUTEX_NOT_HELD(&retprocp->p_lock));
		 ...
		 */
		if (retthreadp == NULL && retprocp != NULL) {
			ASSERT(MUTEX_NOT_HELD(&retprocp->p_lock));
			return (set_errno(ESRCH));
		}
		...
		if (cmd == PC_GETPARMS) {
			...
			if ((error = vaparmsout(arg, &pcparms, &vaparms,
			    seg)) != 0)
				return (set_errno(error));
		}
...
donice(procset_t *procset, pcnice_t *pcnice)
{
	...
	if (procset->p_lidtype == P_LWPID || procset->p_ridtype == P_LWPID) {
		...
		if (retprocp != NULL) {
			/*
			 * dotolwp() can return with p_lock held. This is
			 * required for the priocntl GETPARMS case. So, here
			 * we just release the p_lock.
			 */
			if (MUTEX_HELD(&retprocp->p_lock))
				mutex_exit(&retprocp->p_lock);

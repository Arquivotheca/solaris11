...
static boolean_t fss_pri_util_disable = B_TRUE;
...
static int	fss_pri_util(kthread_t *);
...
fss_tick(kthread_t *t)
{
	...
	/*
	 * The LWP accounting code guarantees that we can access
	 * fsspset and fssproj safely here.
	 */
...
static int
fss_pri_util(kthread_t *tp)
{
	if (fss_pri_util_disable == B_FALSE && T_WKLD_IS_CPU_INT(tp))

...
#define	RT_QUANTUM_SET(rtpp)						\
{									\
	int quantum;							\
	uint64_t clock;							\
									\
	quantum = (rtpp)->rt_pquantum;					\
	if (quantum == RT_TQINF) {					\
		(rtpp)->rt_timer = UINT64_MAX;				\
	} else {							\
		LWP_AC_CLOCK((rtpp)->rt_lwp, 0, clock);			\
		(rtpp)->rt_timer = clock + quantum;			\
	}								\
}

#define	RT_QUANTUM_LEFT(rtpp, left)					\
{									\
	uint64_t clock;							\
									\
	if ((rtpp)->rt_pquantum == RT_TQINF) {				\
		(left) = INT_MAX;					\
	} else {							\
		LWP_AC_CLOCK((rtpp)->rt_lwp, 0, clock);			\
		left = (rtpp)->rt_timer - clock;			\
	}								\
}

#define	RT_QUANTUM_EXPIRED(rtpp, expired)				\
{									\
	uint64_t clock;							\
									\
	LWP_AC_CLOCK((rtpp)->rt_lwp, 0, clock);				\
	expired = ((rtpp)->rt_timer <= clock);				\
}
...
static int	rt_pri_util(kthread_t *);
...
static boolean_t rt_pri_util_disable = B_FALSE;
    ...
    void *bufp)
{
	...
	t->t_cid = cid;
	rtpp->rt_lwp = ttolwp(t);
	if (rtpp->rt_tqsignal == 0)
...
rt_fork(kthread_t *t, kthread_t *ct, void *bufp)
{
	...
	thread_lock(t);
	crtpp->rt_pquantum = prtpp->rt_pquantum;
	...
	crtpp->rt_lwp = ttolwp(ct);
	if (crtpp->rt_tqsignal == 0)
		ct->t_acflag |= TA_NO_PROCESS_LOCK;
	RT_QUANTUM_SET(crtpp);
...
rt_parmsset(kthread_t *tx, void *prmsp, id_t reqpcid, cred_t *reqpcredp)
{
	...
	if (rtkprmsp->rt_tqntm == RT_TQINF)
		...
		rtpp->rt_pquantum = rtkprmsp->rt_tqntm;
	RT_QUANTUM_SET(rtpp);

	if ((rtkprmsp->rt_cflags & RT_DOSIG) != 0) {
		rtpp->rt_tqsignal = rtkprmsp->rt_tqsig;
		if (rtpp->rt_tqsignal == 0)
			rtpp->rt_tp->t_acflag |= TA_NO_PROCESS_LOCK;
		else
			rtpp->rt_tp->t_acflag &= ~TA_NO_PROCESS_LOCK;
	}
...
rt_tick(kthread_t *t)
{
	rtproc_t *rtpp = (rtproc_t *)(t->t_cldata);
	proc_t *p;
	int timeleft, expired;

	ASSERT(LWP_AC_LOCK_HELD(t) || MUTEX_HELD(&ttoproc(t)->p_lock));

	RT_QUANTUM_EXPIRED(rtpp, expired);
	if (!expired)
		return;

	thread_lock(t);
	RT_QUANTUM_LEFT(rtpp, timeleft);
	if ((timeleft <= 0) ||
	    (t->t_state == TS_ONPROC && DISP_MUST_SURRENDER(t))) {
		if ((timeleft <= 0) && rtpp->rt_tqsignal) {
			thread_unlock(t);
			p = ttoproc(t);
			ASSERT(MUTEX_HELD(&p->p_lock));
			sigtoproc(p, t, rtpp->rt_tqsignal);
			thread_lock(t);
		}
		rtpp->rt_flags |= RTBACKQ;
		cpu_surrender(t);
...
/* ARGSUSED */
static int
rt_pri_util(kthread_t *tp)
{
	if (rt_pri_util_disable == B_FALSE && T_WKLD_IS_CPU_INT(tp))

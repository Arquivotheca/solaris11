...
static boolean_t ts_pri_util_disable = B_TRUE;
...
#define	TS_QUANTUM_SET(tspp)						\
{									\
	int quantum;							\
	uint64_t clock;							\
									\
	quantum = ts_dptbl[(tspp)->ts_cpupri].ts_quantum;		\
	if ((tspp)->ts_flags & TSKPRI) {				\
		(tspp)->ts_timeleft = quantum;				\
	} else {							\
		LWP_AC_CLOCK((tspp)->ts_lwp, 0, clock);			\
		(tspp)->ts_timer = clock + quantum;			\
	}								\
}

#define	TS_QUANTUM_LEFT(tspp, left)					\
{									\
	uint64_t clock;							\
									\
	if ((tspp)->ts_flags & TSKPRI) {				\
		(left) = (tspp)->ts_timeleft;				\
	} else {							\
		LWP_AC_CLOCK((tspp)->ts_lwp, 0, clock);			\
		left = (tspp)->ts_timer - clock;			\
	}								\
}

#define	TS_QUANTUM_EXPIRED(tspp, expired)				\
{									\
	uint64_t clock;							\
									\
	LWP_AC_CLOCK((tspp)->ts_lwp, 0, clock);				\
	expired = ((tspp)->ts_timer <= clock);				\
}

#define	TS_QUANTUM_RAISE(tspp)						\
{									\
	uint64_t clock;							\
									\
	if ((tspp)->ts_timer != UINT64_MAX) {				\
		LWP_AC_CLOCK((tspp)->ts_lwp, 0, clock);			\
		(tspp)->ts_timeleft = (tspp)->ts_timer - clock;		\
		(tspp)->ts_timer = UINT64_MAX;				\
	}								\
}

#define	TS_QUANTUM_LOWER(tspp)						\
{									\
	uint64_t clock;							\
									\
	if ((tspp)->ts_timer == UINT64_MAX) {				\
		LWP_AC_CLOCK((tspp)->ts_lwp, 0, clock);			\
		(tspp)->ts_timer = (tspp)->ts_timeleft + clock;		\
	}								\
}
...
static int	ts_pri_util(kthread_t *);
	...
	cred_t *reqpcredp, void *bufp)
{
	...
	tspp->ts_lwp = ttolwp(t);
	t->t_acflag |= TA_NO_PROCESS_LOCK;
	TS_QUANTUM_SET(tspp);
...
ts_fork(kthread_t *t, kthread_t *ct, void *bufp)
{
	...
	ctspp->ts_lwp = ttolwp(ct);
	ct->t_acflag |= TA_NO_PROCESS_LOCK;
	TS_QUANTUM_SET(ctspp);
...
ts_preempt(kthread_t *t)
{
	...
	if (t->t_schedctl && schedctl_get_nopreempt(t)) {
		TS_QUANTUM_LEFT(tspp, timeleft);
		if (timeleft > -SC_MAX_TICKS) {
...
ts_setrun(kthread_t *t)
{
	...
	if (tspp->ts_flags & TSIA) {
		...
		if ((gethrtime_unscaled() - t->t_disp_time) >
		    disp_cache_warmth_delta)
			setbackdq(t);
		else
			setfrontdq(t);
	}
...
ts_tick(kthread_t *t)
{
	...
	pri_t	oldpri;
	int timeleft, expired, capped;

	ASSERT(LWP_AC_LOCK_HELD(t) || MUTEX_HELD(&ttoproc(t)->p_lock));

	TS_QUANTUM_EXPIRED(tspp, expired);
	capped = CPUCAPS_ON() && THREAD_IS_CAPPED(t);
	if (!expired && !capped)
		return;
	...
	/* If thread priority is kernel level, exit */
	if ((tspp->ts_flags & TSKPRI) != 0) {
	...
	if (timeleft <= 0) {
		 ...
		 * preempting this thread, just note that the thread
		 * should yield soon and let it keep running (unless it's
		 * been a while).
		 */
		if (t->t_schedctl && schedctl_get_nopreempt(t)) {
			if (timeleft > -SC_MAX_TICKS) {
		 ...
		 * necessary to adjust its position on a sleep queued
	...
	thread_unlock_nopreempt(t);	/* LWP AC can't be preempted */
}
...
ts_yield(kthread_t *t)
{
	...
	if (timeleft <= 0) {
		 ...
		 */
		DTRACE_SCHED1(schedctl__yield, int, -timeleft);
		tspp->ts_cpupri = ts_dptbl[tspp->ts_cpupri].ts_tqexp;
		TS_NEWUMDPRI(tspp);
		TS_QUANTUM_SET(tspp);
		tspp->ts_dispwait = 0;
...
/* ARGSUSED */
static int
ts_pri_util(kthread_t *tp)
{
	if (ts_pri_util_disable == B_FALSE && T_WKLD_IS_CPU_INT(tp))
		return ((int)disp_pri_util_map[tp->t_pri]);
	else
		return (1);
}

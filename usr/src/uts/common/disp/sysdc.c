 ...
 * - Sleeping
 *
 *	While an SDC thread is sleeping, its duty cycle is not changing,
 *	since it is not RUNNABLE or ONPROC.  If we slept at the current
 *	priority, this could lead to priority inversion issues (for example,
 *	a priority-zero SDC thread would sleep at the bottom of the CV's
 *	wait queue).  To prevent this, SDC threads always sleep at their
 *	maximum priority, and restore their actual priority upon wakeup.
 *
 * - Why not FSS?
 *
...
static boolean_t sysdc_pri_util_disable = B_TRUE;
    ...
    hrtime_t *O, hrtime_t *R, hrtime_t *newO)
{
	...
	*O = *R = 0;
	...
	if (sdc->sdc_asleep && sdc->sdc_sleep_updates != sdc->sdc_nupdates) {
...
sysdc_update_pri(sysdc_t *sdc, uint_t flags)
{
	...
	if (sysdc_compute_pri(sdc, flags)) {
		/*
		 * Do not change priority if the thread is sleeping; his
		 * priority will be adjusted at wakeup time.
		 */
		if (sdc->sdc_asleep) {
			return;
		}
...
sysdc_tick(kthread_t *t)
{
	...
	IMPLY(sdc->sdc_asleep, t->t_state != TS_ONPROC && t->t_state != TS_RUN);
...
sysdc_setrun(kthread_t *t)
{
	...
	/*
	 * See if T needs to change priority, activating it first if necessary.
	 *
	 * Since we are in transition, there's no need to use the full
	 * thread_change_pri().
	 */
	if (sdc->sdc_next == NULL) {
		(void) sysdc_compute_pri(sdc, 0);
		sysdc_activate(sdc);
		ASSERT(sdc->sdc_next != NULL);
	}
	if (t->t_pri != sdc->sdc_epri) {
...
sysdc_sleep(kthread_t *t)
{
	...
	/*
	 * Since our priority will not adjust while we are asleep (by
	 * definition of "duty cycle"), we always sleep at our maximum
	 * priority.  When we wake, we'll readjust it if necessary.
	 */
	sdc->sdc_asleep = 1;
	sdc->sdc_sleep_updates = sdc->sdc_nupdates;
	if (t->t_pri != sdc->sdc_maxpri) {
		THREAD_CHANGE_PRI(t, sdc->sdc_maxpri);
	}
}
    ...
    void *bufp)
{
	...
	/*
	 * SYSDC threads are permanently marked as non-transient.
	 */
	if (thread_transience_kernel == B_TRUE) {
		t->t_wkld_flags |= T_WKLD_PERMANENT;
		t->t_transience = 0;
	}
...
/* ARGSUSED */
static int
sysdc_pri_util(kthread_t *tp)
{
	if (sysdc_pri_util_disable == B_FALSE && T_WKLD_IS_CPU_INT(tp))

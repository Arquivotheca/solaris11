...
static void
tavor_override_nodedesc(tavor_state_t *state, char *resp)
{
	char	*node_desc;
	size_t	nlen;

	node_desc = ibc_read_nodedesc(state->ts_dip, &nlen);
	if (!node_desc)
		return;

    ...
    offset_t off, size_t len, void **pvtp)
{
	...
	if (type == MLNX_UMAP_CQMEM_RSRC) {
		...
		indx = key & state->hs_cqnmask;
		cq = hermon_icm_num_to_hdl(state, HERMON_CQC, indx);
		if (cq == NULL)
			goto umem_map_fail;
		...
		if (cq->cq_umap_dhp == NULL) {
			...
			hermon_icm_dec_hdl_refcnt(state, HERMON_CQC, indx);
		...
		indx = key & state->hs_qpnmask;
		qp = hermon_icm_num_to_hdl(state, HERMON_QPC, indx);
		if (qp == NULL)
			goto umem_map_fail;
		...
		if (qp->qp_umap_dhp == NULL) {
			...
			hermon_icm_dec_hdl_refcnt(state, HERMON_QPC, indx);
		} else {
			mutex_exit(&qp->qp_lock);
			hermon_icm_dec_hdl_refcnt(state, HERMON_QPC, indx);
			goto umem_map_fail;
		...
		indx = key & state->hs_srqnmask;
		srq = hermon_icm_num_to_hdl(state, HERMON_SRQC, indx);
		if (srq == NULL)
			goto umem_map_fail;
		...
		if (srq->srq_umap_dhp == NULL) {
			...
			hermon_icm_dec_hdl_refcnt(state, HERMON_SRQC, indx);
		} else {
			mutex_exit(&srq->srq_lock);
			hermon_icm_dec_hdl_refcnt(state, HERMON_SRQC, indx);
			goto umem_map_fail;
    ...
    offset_t off, size_t len, void **pvtp)
{
	...
	if (type == MLNX_UMAP_CQMEM_RSRC) {
		...
		/* Use "key" (CQ number) to do fast lookup of CQ handle */
		indx = key & state->hs_cqnmask;
		cq = hermon_icm_num_to_hdl(state, HERMON_CQC, key);
		if (cq == NULL)
			goto umem_map_fail;

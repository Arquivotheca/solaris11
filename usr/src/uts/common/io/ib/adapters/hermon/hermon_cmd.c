    ...
    uint_t sleepflag)
{
	hermon_mbox_info_t	mbox_info;
	hermon_cmd_post_t	cmd;
	uint64_t		in_mapaddr, out_mapaddr;
	...
	if (opcode == RTS2SQD_QP) {
		...
		for (i = 0; i < (size >> 2); i++) {
			ddi_put32(mbox_info.mbi_in->mb_acchdl,
			    ((uint32_t *)mbox_info.mbi_in->mb_addr + i + 2),
			    ((uint32_t *)(void *)qp)[i]);
		}
		ddi_put32(mbox_info.mbi_in->mb_acchdl,
 ...
 *    QUERY_HCA, QUERY_MPT, QUERY_EQ, and QUERY_CQ.
 *    With support of FCoIB, this also supports QUERY_FC.
 *    The QUERY_QP command is handled by hermon_qp_query_cmd_post().
 */
int
...
/*
 * hermon_qp_query_cmd_post()
 *    Context: Can be called from interrupt or base context.
 *
 *    This is the specific function for posting the Hermon query QP command.
 *    An "Out" mailbox is allocated for the resulting queried data.
 */
int
hermon_qp_query_cmd_post(hermon_state_t *state, uint_t opcode, uint_t opmod,
    uint_t queryindx, void *query, uint_t size, uint_t sleepflag)
{
	hermon_mbox_info_t	mbox_info;
	hermon_cmd_post_t	cmd;
	...
	for (i = 0; i < (size >> 2); i++) {
		data = ddi_get32(mbox_info.mbi_out->mb_acchdl,
		    ((uint32_t *)mbox_info.mbi_out->mb_addr + i + 2));
		((uint32_t *)query)[i] = data;
	}

	...
	case HCA_IBA_ERR:
		...
		case HCA_ERR_FATAL:
			...
			ddi_fm_service_impact(state->hs_dip,
			    DDI_SERVICE_LOST);
			state->hs_fm_async_fatal = B_TRUE;
			/* dump the error buffer if it's the internal error */
			if (state->hs_fm_internal_err != 0) {
				uint32_t buf_size =
				    state->hs_fw.error_buf_sz;	/* in #dwords */
				int i;
				ddi_acc_handle_t cmdhdl
				    = hermon_get_cmdhdl(state);
				if (state->hs_fm_err_buf == NULL) {
					state->hs_fm_err_buf =
					    kmem_alloc(buf_size *
					    sizeof (uint32_t), KM_NOSLEEP);
				}
				for (i = 0; i < buf_size; i++) {
					uint32_t *base_addr =
					    (state->hs_cmd_regs.fw_err_buf + i);
					uint32_t err_buf =
					    ddi_get32(cmdhdl, base_addr);
					/* record the error buffer in memory */
					if (state->hs_fm_err_buf != NULL) {
						state->hs_fm_err_buf[i] =
						    err_buf;
					}
					cmn_err(CE_CONT,
					    "hermon%d: "
					    "internal error buf[%02x]: %08X\n",
					    state->hs_instance, i, err_buf);
				}
				/*
				 * If the Hermon internal error is detected
				 * during the attach, this should not be fatal.
				 * Just print out the message instead of panic.
				 */
				if (hermon_get_state(state) & HCA_ATTCH_FM) {
					cmn_err(CE_WARN,
					    "Hermon Fatal Internal Error. "
					    "Hermon state pointer=0x%p, "
					    "error=0x%x, error buffer size=%d",
					    (void *)state,
					    state->hs_fm_internal_err,
					    buf_size *
...
hermon_inter_err_chk(void *arg)
{
...
#ifdef FMA_TEST
	uint32_t *fw_err_buf, *saved_fw_err_buf;
	boolean_t internal_error_test = B_FALSE;
#endif
...
#ifdef FMA_TEST
	if (hermon_test_num != 0 && hermon_internal_error_test <= 0) {
		return;
	} else if (hermon_internal_error_test > 0) {
		internal_error_test = B_TRUE;
		hermon_internal_error_test--; /* decrease the counter */
	}
#endif
...
#ifdef FMA_TEST
	if (internal_error_test == B_TRUE) {
		uint32_t value = 0x11111111;
		int i;

		fw_err_buf = kmem_alloc(state->hs_fw.error_buf_sz *
		    sizeof (uint32_t), KM_SLEEP);

		/* set the dummy error buffer */
		saved_fw_err_buf = state->hs_cmd_regs.fw_err_buf;
		state->hs_cmd_regs.fw_err_buf = fw_err_buf;

		/* create the error buffer for test */
		for (i = 0; i < state->hs_fw.error_buf_sz; i++) {
				uint32_t err_value = value * (i + 1);
				ddi_put32(cmdhdl, fw_err_buf + i, err_value);
		}
	}
#endif
	...
	state->hs_fm_internal_err =
	    ddi_get32(cmdhdl, state->hs_cmd_regs.fw_err_buf);
	...
	if (state->hs_fm_internal_err != 0) {
		...
		if (state->hs_fm_disable) {
			...
			if (state->hs_fm_err_buf == NULL) {
				state->hs_fm_err_buf =
				    kmem_alloc(buf_size * sizeof (uint32_t),
				    KM_NOSLEEP);
			}
			/* dump the error buffer */
			for (i = 0; i < buf_size; i++) {
				base_addr =
				    (state->hs_cmd_regs.fw_err_buf + i);
			...
			cmn_err(CE_PANIC,
			    ...
			    "Hermon state pointer=0x%p, "
			    "error=0x%x, error buffer size=%d",
			    (void *)state, state->hs_fm_internal_err,
			    buf_size * (uint32_t)sizeof (uint32_t));
		} else {
...
#ifdef FMA_TEST
	if (internal_error_test == B_TRUE) {
		ASSERT(fw_err_buf != NULL);
		state->hs_cmd_regs.fw_err_buf = saved_fw_err_buf;
		kmem_free(fw_err_buf, state->hs_fw.error_buf_sz *
		    sizeof (uint32_t));
		fw_err_buf = NULL;
	}
#endif
...
int hermon_internal_error_test;	/* internal error case. used as counter */

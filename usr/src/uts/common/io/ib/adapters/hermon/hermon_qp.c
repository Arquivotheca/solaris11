...
static void hermon_qp_sgl_to_logwqesz(hermon_state_t *state, uint_t num_sgl,
    uint_t real_max_sgl, uint_t *inline_bytesp, hermon_qp_wq_type_t wq_type,
    ...
    uint_t sleepflag)
{
	...
	int				status, flag;
	uint_t				sq_sgl, inline_bytes, max_inline_bytes;
	 ...
	 *
	 * 1. FEXCH is only allocated via qp_range.
	 */
	if (alloc_flags & IBT_QP_USES_FEXCH) {
	...
	switch (alloc_flags & (IBT_QP_USES_RSS |
	...
	if ((attr_p->qp_ibc_rcq_hdl == NULL)) {
		if ((alloc_flags & (IBT_QP_USES_FCMD | IBT_QP_USES_RSS)) == 0) {
			status = IBT_CQ_HDL_INVALID;
			goto qpalloc_fail1;
		}
	...
	if (type == IBT_UD_RQP) {
		...
		max_inline_bytes =
		    state->hs_ibtfinfo.hca_attr->hca_ud_send_inline_sz;
	} else {
		max_sgl = state->hs_ibtfinfo.hca_attr->hca_conn_send_sgl_sz;
		swq_type = HERMON_QP_WQ_TYPE_SENDQ_CONN;
		max_inline_bytes =
		    state->hs_ibtfinfo.hca_attr->hca_conn_send_inline_sz;
	}
	...
	if (qp_srq_en) {
		...
		hermon_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_rq_sgl,
		    max_recv_sgl, NULL, HERMON_QP_WQ_TYPE_RECVQ,
		    &qp->qp_rq_log_wqesz, &qp->qp_rq_sgl);
	}
	sq_sgl = attr_p->qp_sizes.cs_sq_sgl;
	if ((inline_bytes = attr_p->qp_sizes.cs_inline) != 0) {
		if (alloc_flags & IBT_QP_USES_INLINE) {
			if (inline_bytes > max_inline_bytes) {
				cmn_err(CE_CONT, "!hermon_qp_alloc: cs_inline "
	...
	hermon_qp_sgl_to_logwqesz(state, sq_sgl, max_sgl, &inline_bytes,
	    swq_type, &qp->qp_sq_log_wqesz, &qp->qp_sq_sgl);
	...
	if (queuesz_p != NULL) {
		...
		queuesz_p->cs_inline	= inline_bytes;
    ...
    uint_t sleepflag)
{
	 ...
	 */
	hermon_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_rq_sgl,
	    max_sgl, NULL, HERMON_QP_WQ_TYPE_RECVQ,
	    &qp->qp_rq_log_wqesz, &qp->qp_rq_sgl);
	if (type == IBT_SMI_SQP) {
		hermon_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_sq_sgl,
		    max_sgl, NULL, HERMON_QP_WQ_TYPE_SENDMLX_QP0,
		    &qp->qp_sq_log_wqesz, &qp->qp_sq_sgl);
	} else {
		hermon_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_sq_sgl,
		    max_sgl, NULL, HERMON_QP_WQ_TYPE_SENDMLX_QP1,
		    &qp->qp_sq_log_wqesz, &qp->qp_sq_sgl);
	}
    ...
    hermon_qphdl_t *qphdl, uint_t sleepflag)
{
	 ...
	 */
	if (alloc_flags & IBT_QP_USER_MAP) {
...
for_each_qp:
	...
	bzero(qp, sizeof (struct hermon_sw_qp_s));
	_NOTE(NOW_INVISIBLE_TO_OTHER_THREADS(*qp))
	qp->qp_alloc_flags = alloc_flags & ~IBT_QP_USES_RSS;
	...
	if ((inline_bytes = attr_p->qp_sizes.cs_inline) != 0) {
		if (alloc_flags & IBT_QP_USES_INLINE) {
			if (inline_bytes >
			    state->hs_ibtfinfo.hca_attr->hca_max_inline_size) {
				cmn_err(CE_CONT, "!hermon_qp_alloc: cs_inline "
				    "is invalid or uninitialized\n");
				status = IBT_HCA_SGL_EXCEEDED;
				goto qpalloc_fail7;
	 ...
	 * and the rest for the headroom definition of prefetching.
	 */
	if ((alloc_flags & IBT_QP_USES_FEXCH) == 0) {
		wqesz_shift = qp->qp_sq_log_wqesz;
		thewqesz    = 1 << wqesz_shift;
		thewqe = (uint64_t *)(void *)(qp->qp_sq_buf);
		for (i = 0; i < sq_depth; i++) {
	...
	if (qp->qp_is_special) {
		...
		if (free_qp_flags == IBC_FREE_QP_ONLY) {
			...
			*qpnh = (ibc_qpn_hdl_t)entry;
			...
			hermon_qp_release_qpn(state, qp->qp_qpn_hdl,
			    HERMON_QPC_FREE | HERMON_QPN_RELEASE);
		}
	}
    ...
    ibt_qp_query_attr_t *attr_p)
{
	...
	/* QUERY_QP does not always provide a correct sched_q */
	 ...
	 */
	status = hermon_qp_query_cmd_post(state, QUERY_QP, 0, qp->qp_qpnum,
	...
	if (qpc->pri_addr_path.sched_q == 0)	/* restore tmp if query is 0 */
	...
	if (qp->qp_type == IBT_UD_RQP) {
		...
		path_ptr->cep_adds_vect.av_srvl =
		    (qpc_path->sched_q >> 2) & 0xF;
		...
		alt_path_ptr->cep_adds_vect.av_srvl =
		    (qpc_alt_path->sched_q >> 2) & 0xF;
    ...
    hermon_rsrc_t *qpc)
{
	...
	if (entry == NULL) {
		...
		entry->qpn_statep = state;
		/*
		 * Use of kmem_zalloc negates the need to do:
		 * entry->qpn_timeout_id = 0;
		 * entry->qpn_refcnt  = 0;
		 * entry->qpn_counter = 0;
		 */
	 ...
	 */
	qp->qp_qpn_hdl	= entry;
	qp->qp_qpnum	= (HERMON_QPN_UPPER_BITS(state, entry) | qpc->hr_indx) &
...
void
hermon_qpn_timeout(void *arg)
{
	hermon_qpn_entry_t *entry = (hermon_qpn_entry_t *)arg;
	hermon_state_t *state = entry->qpn_statep;
    ...
    int flags)
{
	...
	if (flags & HERMON_QPC_FREE) {
		 ...
		 * we want to free it up for use by other consumers.
		 * if all the floating upper bits of the QPN have been used,
		 * then we do not free the context here, but do that after
		 * the refxnt of QPN usage goes to zero (farther below).
		 */
		if (HERMON_QPN_UPPER_BITS(state, entry) <=
		    HERMON_QP_MAXNUMBER_MSK) {
			hermon_rsrc_free(state, &entry->qpn_qpc);
		}
	}
	...
	/*
	 * If we are releasing the QP number, then we decrement the
	 * reference count.  If there are zero references and no more
	 * value of the upper bits that can be used, then we paranoidly
	 * (to avoid IBCM TIMEWAIT (stale) problems) clean up after a
	 * 10 second timeout.
	 */
	if (flags & HERMON_QPN_RELEASE) {
		 ...
		 * context if it has more QPNs that it can use.  If not,
		 * it is passed to timeout() so that it is not usable for
		 * some time to avoid IBCM TIMEWAIT (stale) problems.
		 */
		if (HERMON_QPN_UPPER_BITS(state, entry) >
		    HERMON_QP_MAXNUMBER_MSK) {
			if (entry->qpn_refcnt == 0) {
				entry->qpn_timeout_id = timeout(
				    hermon_qpn_timeout, entry,
				    drv_usectohz(10 * 1000000)); /* 10 sec. */
			}
		}
	}
	mutex_exit(&state->hs_qpn_avl_lock);
...
hermon_qpn_avl_fini(hermon_state_t *state)
{
	...
	/* Make sure there are no pending timeouts before proceeding */
	avl_tree = &state->hs_qpn_avl;
restart:
	mutex_enter(&state->hs_qpn_avl_lock);
	for (entry = avl_first(avl_tree); entry != NULL;
	    entry = avl_walk(avl_tree, entry, AVL_AFTER)) {
		if ((timeout_id = entry->qpn_timeout_id) != 0) {
			entry->qpn_timeout_id = 0;
			mutex_exit(&state->hs_qpn_avl_lock);
			ret = untimeout(timeout_id);

			/* if timeout() did not run yet, run it now */
			if (ret != (clock_t)-1)
				hermon_qpn_timeout(entry);
			goto restart;
		}
    ...
    uint_t *logwqesz, uint_t *max_sgl)
{
	...
	uint_t	inline_bytes, i;

	inline_bytes = (inline_bytesp != NULL) ? *inline_bytesp : 0;
	...
	case HERMON_QP_WQ_TYPE_SENDQ_UD:
		...
		if (HERMON_SGL_TO_INLINE(num_sgl) < inline_bytes) {
			for (i = 0; i < HERMON_SQ_SIZES; i++)
				if (inline_bytes <= state->hs_ud_max_inline[i])
					break;
		} else {
			for (i = 0; i < HERMON_SQ_SIZES; i++)
				if (max_size <= state->hs_max_size[i])
					break;
		}
		max_size = state->hs_max_size[i];
		inline_bytes = state->hs_ud_max_inline[i];
		if (inline_bytesp)
			*inline_bytesp = inline_bytes;
		log2 = i + 6;
	...
	case HERMON_QP_WQ_TYPE_SENDQ_CONN:
		...
		/* Make sure not to break RDMA_WRITE on older drivers */
		if (num_sgl == 3)
			num_sgl = 4;	/* 3 + 1 (Remote Address Segment) */

		max_size = (sizeof (hermon_hw_snd_wqe_ctrl_t) + (num_sgl << 4));
		if (HERMON_SGL_TO_INLINE(num_sgl) < inline_bytes) {
			for (i = 0; i < HERMON_SQ_SIZES; i++)
				if (inline_bytes <=
				    state->hs_conn_max_inline[i])
					break;
		} else {
			for (i = 0; i < HERMON_SQ_SIZES; i++)
				if (max_size <= state->hs_max_size[i])
		...
		max_size = state->hs_max_size[i];
		inline_bytes = state->hs_conn_max_inline[i];
		if (inline_bytesp)
			*inline_bytesp = inline_bytes;
		log2 = i + 6;

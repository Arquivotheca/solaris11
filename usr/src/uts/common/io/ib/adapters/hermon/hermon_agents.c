...
hermon_agent_handle_req(void *cb_args)
{
	...
	if (HERMON_IS_SPECIAL_TRAP_MAD(msgp)) {
		...
		if (status != HERMON_CMD_SUCCESS) {
			...
			if (status == HERMON_CMD_BAD_PKT) {
#ifndef	__lock_lint
				madhdrp->Status = MAD_STATUS_INVALID_FIELD;
				madhdrp->R_Method = MAD_METHOD_GET_RESPONSE;
#endif
			} else {
				goto hermon_agent_handle_req_skip_response;
			}
...
static void
hermon_override_nodedesc(hermon_state_t *state, char *resp)
{
	char	*node_desc;
	size_t	nlen;

	node_desc = ibc_read_nodedesc(state->hs_dip, &nlen);
	if (!node_desc)
		return;
	(void) strncpy(resp, node_desc, nlen);
	kmem_free(node_desc, nlen);
}
    ...
    uint_t port)
{
	...
	if (HERMON_MAD_IS_SUBNET_CLASS(rmadhdrp) &&
	    rmadhdrp->R_Method == MAD_METHOD_GET_RESPONSE &&
	    b2h16(rmadhdrp->AttributeID) == MAD_ATTR_ID_NODEDESC) {
		hermon_override_nodedesc(state, (char *)resp);
	}

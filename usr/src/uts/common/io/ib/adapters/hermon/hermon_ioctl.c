    ...
    int *rvalp)
{
	...
	uint32_t	ibc_nodedesc_upd_flg = 0;
	hermon_nodedesc_ioctl_t	node_desc_info;
	dev_info_t	*dip = NULL;
	size_t		read_len;
	char		*nd_str;
	...
	case HERMON_IOCTL_SET_NODEDESC:
		if (ddi_copyin((void *)arg, (void *)&node_desc_info,
		    sizeof (hermon_nodedesc_ioctl_t), mode) != 0) {
			status = EFAULT;
			break;
		}
		 ...
		 * after solaris_set_nodedesc is modified not to pass
		 * 0xffffffff when it wants to just update the common
		 * nodedescriptor.
		 */
		if (node_desc_info.node_desc_update_flag == 0xffffffff)
			node_desc_info.node_desc_update_flag =
			    HERMON_NODEDESC_UPDATE_STRING;

		if (node_desc_info.node_desc_update_flag &
		    HERMON_NODEDESC_UPDATE_STRING)
			ibc_nodedesc_upd_flg  |= IBC_NODEDESC_UPDATE_STRING;
		if (node_desc_info.node_desc_update_flag &
		    HERMON_NODEDESC_UPDATE_HCA_STRING) {
			ibc_nodedesc_upd_flg  |= IBC_NODEDESC_UPDATE_HCA_STRING;
			dip = state->hs_dip;
		}

		ibt_status = ibc_write_nodedesc(dip,
		    (char *)node_desc_info.node_desc_str, ibc_nodedesc_upd_flg);
	...
	case HERMON_IOCTL_GET_NODEDESC:
		dip = state->hs_dip;
		nd_str = ibc_read_nodedesc(dip, &read_len);
		if (nd_str == NULL) {
			status = EINVAL;
			break;
		}
		(void) strncpy((char *)node_desc_info.node_desc_str, nd_str,
		    read_len);
		status = 0;
		kmem_free(nd_str, read_len);

		if (ddi_copyout((void *) &node_desc_info,
		    (void *) arg, sizeof (node_desc_info),
		    mode) != 0) {
			status = EFAULT;
			break;
		}

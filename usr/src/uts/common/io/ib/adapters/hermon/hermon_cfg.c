...
hermon_cfg_profile_init_phase2(hermon_state_t *state)
{
	...
	cp->cp_hca_max_rdma_in_qp = cp->cp_hca_max_rdma_out_qp =
	    1 << min(hermon_log_num_rdb_per_qp, devlim->log_max_ra_req_qp);
#endif
	state->hs_cqnmask = (1 << state->hs_cfg_profile->cp_log_num_cq) - 1;
	state->hs_qpnmask = (1 << state->hs_cfg_profile->cp_log_num_qp) - 1;
	state->hs_srqnmask = (1 << state->hs_cfg_profile->cp_log_num_srq) - 1;

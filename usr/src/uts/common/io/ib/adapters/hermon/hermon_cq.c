...
static void hermon_cq_free_step2(hermon_state_t *state, hermon_cqhdl_t cq);

/*
 * hermon_cq_freelist_init()
 *	Context: driver attach
 *
 * Initialize the data used to manage delayed "CQ free".
 */
void
hermon_cq_freelist_init(hermon_state_t *state)
{
	mutex_init(&state->hs_cq_freelist_lock, NULL, MUTEX_DRIVER, NULL);
	state->hs_cq_freelist = NULL;
	state->hs_cq_freelist_tail = NULL;
	state->hs_cq_freelist_cnt = 0;
	state->hs_cq_freelist_cnt_limit = 1024;	/* What seems to work. */
	state->hs_cq_alloc_generation = 0;
	state->hs_cq_freelist_task_running = 0;
}


/*
 * hermon_cq_freelist_fini()
 *	Context: driver detach
 *
 * Completely free all entries on the CQ free list.
 */
void
hermon_cq_freelist_fini(hermon_state_t *state)
{
	hermon_cqhdl_t cq, next;
	int cnt;

	mutex_enter(&state->hs_cq_freelist_lock);
	cq = state->hs_cq_freelist;
	cnt = state->hs_cq_freelist_cnt;

	state->hs_cq_freelist = NULL;
	state->hs_cq_freelist_tail = NULL;
	state->hs_cq_freelist_cnt = 0;
	mutex_exit(&state->hs_cq_freelist_lock);

	_NOTE(NOW_INVISIBLE_TO_OTHER_THREADS(*cq))
	while (--cnt >= 0) {
		next = cq->cq_link;
		cq->cq_link = NULL;
		hermon_cq_free_step2(state, cq);
		cq = next;
	}
	mutex_destroy(&state->hs_cq_freelist_lock);
}
    ...
    uint_t sleepflag)
{
	...
	mutex_enter(&state->hs_cq_freelist_lock);
	cq->cq_cqnum = (cqc->hr_indx + (state->hs_cq_alloc_generation <<
	    state->hs_cfg_profile->cp_log_num_cq)) & HERMON_CQ_MAXNUMBER_MSK;
	mutex_exit(&state->hs_cq_freelist_lock);
...
/*
 * hermon_cq_freelist_task()
 *	Context: taskq/kernel context.
 *
 * Keep freeing CQCs while the size of the list is beyond the limit.
 * Any time the list is grabbed for freeing, increment the generation
 * used for allocation to avoid using recently used CQ numbers any
 * time soon.
 */
void
hermon_cq_freelist_task(void *arg)
{
	...
	mutex_enter(&state->hs_cq_freelist_lock);
	while (state->hs_cq_freelist_cnt >= state->hs_cq_freelist_cnt_limit) {
		state->hs_cq_alloc_generation++;
		cq = state->hs_cq_freelist;
		cnt = state->hs_cq_freelist_cnt;
...
hermon_cq_free(hermon_state_t *state, hermon_cqhdl_t *cqhdl, uint_t sleepflag)
{
	...
	/*
	 * Put NULL into the CQNum-to-CQHdl list.  This will allow any
	...
	mutex_enter(&state->hs_cq_freelist_lock);
	if (state->hs_cq_freelist)
		state->hs_cq_freelist_tail->cq_link = cq;
	else
		state->hs_cq_freelist = cq;
	state->hs_cq_freelist_tail = cq;
	if ((++state->hs_cq_freelist_cnt >= state->hs_cq_freelist_cnt_limit) &&
	    state->hs_cq_freelist_task_running == 0) {
		state->hs_cq_freelist_task_running = 1;
		(void) taskq_dispatch(system_taskq, hermon_cq_freelist_task,
		    state, TQ_SLEEP);
	}
	mutex_exit(&state->hs_cq_freelist_lock);
...
static void
hermon_cq_free_step2(hermon_state_t *state, hermon_cqhdl_t cq)
{
	hermon_rsrc_t		*cqc, *rsrc;
	hermon_pdhdl_t		pd;
    ...
    uint_t count, uint_t usec, ibt_cq_handler_id_t hid, uint_t sleepflag)
{
	...
	if (hid && (hid - 1 != cq->cq_eqnum)) {
		if (!HERMON_HID_VALID(state, hid)) {
			IBTF_DPRINTF_L2("CQmodify", "bad handler id 0x%x",
			    hid);
			mutex_exit(&cq->cq_lock);
...
{
	hermon_cqhdl_t		cq;
	uint32_t		cqnum, cqindx;

	/* Get the CQ handle from CQ number in event descriptor */
	cqnum = HERMON_EQE_CQNUM_GET(eq, eqe);
	cqindx = cqnum & state->hs_cqnmask;
	cq = hermon_icm_num_to_hdl(state, HERMON_CQC, cqindx);
	 ...
	 */
	if (cq != NULL) {
		if ((cq->cq_cqnum == cqnum) && (state->hs_ibtfpriv != NULL)) {
			hermon_arm_cq_dbr_update(cq->cq_arm_ci_vdbr);
			HERMON_DO_IBTF_CQ_CALLB(state, cq);
		}
		hermon_icm_dec_hdl_refcnt(state, HERMON_CQC, cqindx);
	}

    ...
    uint_t page)
{
	...
	if (page >= HERMON_MAX_DBR_PAGES_PER_USER)
		return (NULL);
    ...
    uint64_t *mapoffset)
{
	...
	for (i3 = 0; i3 < sizeof (uint64_t) / sizeof (uint8_t); i3++)
		if ((u64 & ((uint64_t)1 << (i3 + 8 * i2))) == 0)
			break;

	/* Mark it as allocated */
	pagep->upg_free[i1] |= ((uint64_t)1 << (i3 + 8 * i2));
...
hermon_user_dbr_free(hermon_state_t *state, uint_t index, hermon_dbr_t *record)
{
	...
	i1 = dbr_index / 64;
	i2 = dbr_index % 64;
	ASSERT((pagep->upg_free[i1] & ((uint64_t)1 << i2)) ==
	    ((uint64_t)1 << i2));
	pagep->upg_free[i1] &= ~((uint64_t)1 << i2);
	pagep->upg_nfree++;
	mutex_exit(&state->hs_dbr_lock);
}
    ...
    ibt_adds_vect_t *attr_p, hermon_ahhdl_t *ahhdl, uint_t sleepflag)
{
	 ...
	 */
	status = hermon_set_udav(state, attr_p, udav);
	if (status != DDI_SUCCESS) {
    ...
    ibt_adds_vect_t *attr_p)
{
	...
	*pd = ah->ah_pdhdl;
	...
	/*
	 * Fill in "ibt_adds_vect_t".  We call hermon_get_udav() to fill
	 ...
	 */
	hermon_get_udav(state, ah->ah_udav, attr_p);
    ...
    ibt_adds_vect_t *attr_p)
{
	 ...
	 */
	status = hermon_set_udav(state, attr_p, ah->ah_udav);
	if (status != DDI_SUCCESS) {
		mutex_exit(&ah->ah_lock);
		return (status);
	}
 ...
 *    Context: Can be called from interrupt or base context.
 *
 * This routine is used to fill in the Hermon UDAV struct from the ibt struct.
 */

int hermon_srate_override = -1;	/* allows ease of testing */

int
hermon_set_udav(hermon_state_t *state, ibt_adds_vect_t *av,
    hermon_hw_udav_t *udav)
{
	uint_t		gidtbl_sz;
	...
	udav->mgid_index = ((av->av_port_num - 1) * gidtbl_sz) +
	    av->av_sgid_ix;
	 ...
	 */
	if (udav->grh) {
		udav->flow_label = av->av_flow;
		udav->tclass	 = av->av_tclass;
		udav->hop_limit	 = av->av_hop;
		bcopy(&(av->av_dgid.gid_prefix), &(udav->rgid_h),
		    sizeof (uint64_t));
		bcopy(&(av->av_dgid.gid_guid), &(udav->rgid_l),
		    sizeof (uint64_t));
	} else {
		udav->rgid_l	 = 0x2;
		udav->flow_label = 0;
 ...
 *    Context: Can be called from interrupt or base context.
 *
 * This routine is used to fill in the Hermon Address Path (primary or
 * alternate) from the ibt struct.
 */
int
hermon_set_addr_path(hermon_state_t *state, ibt_adds_vect_t *av,
    hermon_hw_addr_path_t *path)
{
	uint_t		gidtbl_sz;
	...
	path->rgid[0] = av->av_dgid.gid_prefix >> 32;
	path->rgid[1] = av->av_dgid.gid_prefix & 0xffffffff;
	path->rgid[2] = av->av_dgid.gid_guid >> 32;
	path->rgid[3] = av->av_dgid.gid_guid & 0xffffffff;
 ...
 *    Context: Can be called from interrupt or base context.
 *
 * This routine is used to fill in the ibt struct from the Hermon UDAV struct.
 */
void
hermon_get_udav(hermon_state_t *state, hermon_hw_udav_t *udav,
    ibt_adds_vect_t *av)
{
	uint_t		gidtbl_sz;
	...
	av->av_src_path	= udav->ml_path;
	av->av_dlid	= udav->rlid;

	/* Set "av_ipd" value from max_stat_rate */
	switch (udav->max_stat_rate) {
	 ...
	 */
	gidtbl_sz = (1 << state->hs_queryport.log_max_gid);
	av->av_sgid_ix = udav->mgid_index - ((av->av_port_num - 1) *
	    gidtbl_sz);

	av->av_port_num = udav->portnum;

	av->av_flow		= udav->flow_label;
	av->av_tclass		= udav->tclass;
	av->av_hop		= udav->hop_limit;
}
 ...
 *    Context: Can be called from interrupt or base context.
 *
 * This routine is used to fill in the ibt struct from the Hermon
 * Address Path (primary or alternate).
 */
/* ARGSUSED */
void
    ...
    ibt_adds_vect_t *av)
{
	...
	av->av_dgid.gid_prefix = (uint64_t)path->rgid[0] << 32 | path->rgid[1];
	av->av_dgid.gid_guid   = (uint64_t)path->rgid[2] << 32 | path->rgid[3];
}
...
fmr_alloc_done:
	...
	status = hermon_mr_register_physical_fmr(state, mem_pattr, fmr->fmr,
	    mem_desc_p);
	ASSERT(status == DDI_SUCCESS);	/* There is no failure case. */
...
hermon_deregister_fmr(hermon_state_t *state, hermon_mrhdl_t mr)
{
	...
	_NOTE(NOW_INVISIBLE_TO_OTHER_THREADS(*mr))
	*(uint8_t *)(mr->mr_mpt_table) = 0xF0;

	if (++fmr->fmr_remaps < fmrpool->fmr_max_remaps) {
		/* add to remap list */
#ifdef DEBUG
		if (hermon_rdma_debug & 0x4)

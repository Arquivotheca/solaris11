    ...
    hermon_pdhdl_t pd, ibt_smr_attr_t *mr_attr, hermon_mrhdl_t *mrhdl_new)
{
	 ...
	 */
	mr->mr_rkey = mr->mr_lkey = hermon_mr_keycalc(state, mpt->hr_indx);
    ...
    hermon_fmrhdl_t fmr_pool, hermon_mrhdl_t *mrhdl)
{
	...
	mr->mr_mpt_table  = (uint64_t *)mpt->hr_addr;
	mr->mr_fmr_indx   = (uint32_t)mpt->hr_indx;
    ...
    ibt_pmr_attr_t *mem_pattr_p, hermon_mrhdl_t mr, ibt_pmr_desc_t *mem_desc_p)
{
	...
	*(uint8_t *)mpt_table = 0xF0;
	 ...
	 * in.  Otherwise both Rkey and LKey are given the same value.
	 */
	key = mr->mr_fmr_indx | (mr->mr_fmr_key++ << 24);
    ...
    hermon_mwhdl_t *mwhdl)
{
	 ...
	 */
	mw->mr_rkey = hermon_mr_keycalc(state, mpt->hr_indx);
 ...
 *    where K == the arbitrary/floating bits and I == the index
 */
uint32_t
hermon_mr_keycalc(hermon_state_t *state, uint32_t indx)
{
	/* Use the saved mpt_floating_bits in the icm table for MPTs. */

	return ((indx & 0xffffff) |
	    (hermon_icm_mpt_floating_bits(state, indx) << 24));
}
    ...
    hermon_mpt_rsrc_type_t mpt_type)
{
	...
	if (mpt)
		mr->mr_rkey = mr->mr_lkey =
		    hermon_mr_keycalc(state, mpt->hr_indx);
    ...
    ibt_lkey_flags_t flags, uint_t nummtt, hermon_mrhdl_t *mrhdl)
{
	...
	mr->mr_lkey = hermon_mr_keycalc(state, mpt->hr_indx);

...
static int hermon_rsrc_hw_entry_icm_confirm(hermon_rsrc_pool_info_t *pool_info,
    uint_t num, hermon_rsrc_t *hdl, int icm_kind);
    ...
    uint_t num, uint_t sleepflag, hermon_rsrc_t *hdl)
{
	...
	if (status != DDI_SUCCESS) {
		status = hermon_rsrc_hw_entry_icm_free(qpc_pool, hdl);
		vmem_xfree(vmp, addr, num);
		return (DDI_FAILURE);
	}
...
hermon_rsrc_hw_entry_icm_confirm(hermon_rsrc_pool_info_t *pool_info, uint_t num,
    hermon_rsrc_t *hdl, int icm_kind)
{
	...
	hermon_bitmap(bitmap, dma_info, icm_table, index1, icm_kind);
	while (num) {
#ifndef __lock_lint
    ...
    hermon_rsrc_t *hdl)
{
	...
	rindx = hdl->hr_indx;
	hermon_index(index1, index2, rindx, icm_table, span_offset);
	dma_info = icm_table->icm_dma[index1];
	if (dma_info == NULL)	/* reserved rsrcs for FCoIB */
		return (DDI_SUCCESS);

    ...
    ibt_chan_sizes_t *sizes)
{
	...
	if (flags & IBT_ACHAN_USES_INLINE)
		qp_attr.qp_alloc_flags |= IBT_QP_USES_INLINE;
    ...
    ibt_rc_chan_query_attr_t *chan_attrs)
{
	...
	chan_attrs->rc_chan_sizes.cs_inline = 0; /* not returned by CI */
    ...
    ibt_channel_hdl_t *ud_chan_p, ibt_chan_sizes_t *sizes)
{
	...
	if (flags & IBT_ACHAN_USES_SRQ) {
		...
		qp_attr.qp_alloc_flags |= IBT_QP_USES_SRQ;
		qp_attr.qp_ibc_srq_hdl = args->ud_srq->srq_ibc_srq_hdl;
	}
    ...
    ibt_ud_chan_query_attr_t *ud_chan_attrs)
{
	...
	ud_chan_attrs->ud_chan_sizes.cs_inline = 0; /* not returned by CI */

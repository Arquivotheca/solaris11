...
/*
 *  Insert src list before dst list. Empty src list thereafter.
 */
void
rdsv3_list_move_to_head(list_t *dst, list_t *src)
{
	...
	srcnode->list_prev->list_next = dstnode->list_next;
	dstnode->list_next->list_prev = srcnode->list_prev;
	srcnode->list_next->list_prev = dstnode;
	dstnode->list_next = srcnode->list_next;
    ...
    rdsv3_delayed_work_t *dwp, uint_t delay)
{
	...
	if (dwp->timeid == 0) {
		dwp->wq = wq;
		dwp->timeid = timeout(rdsv3_work_timeout_handler, dwp, delay);
		mutex_exit(&dwp->lock);
	} else {
		mutex_exit(&dwp->lock);
...
rdsv3_sock_init_data(struct rsock *sk)
{
	...
	sk->sk_sndbuf = (rdsv3_xmit_hiwat <= rdsv3_max_buf) ?
	    rdsv3_xmit_hiwat : rdsv3_max_buf;
	sk->sk_rcvbuf = (rdsv3_recv_hiwat <= rdsv3_max_buf) ?
	    rdsv3_recv_hiwat : rdsv3_max_buf;
}
...
rdsv3_conn_destructor(void *buf, void *arg)
{
	...
	mutex_destroy(&conn->c_cm_lock);
}

int
rdsv3_conn_compare(const void *addr, const void *conn)
{
	if (*(ipaddr_t *)addr ==
	    ((struct rdsv3_connection *)conn)->c_faddr)
		return (0);
...
rdsv3_ib_frag_constructor(void *buf, void *arg, int kmflags)
{
	...
	frag->f_page = kmem_alloc(RDSV3_FRAG_SIZE, kmflags);
	if (frag->f_page == NULL) {
		RDSV3_DPRINTF2("rdsv3_ib_frag_constructor",
		    "kmem_alloc for %d failed", RDSV3_FRAG_SIZE);
		return (-1);
	}
	frag->f_refcnt = 0;
	...
	if (ibt_map_mem_iov(ib_get_ibt_hca_hdl(rds_ibdev->dev),
		...
		kmem_free(frag->f_page, RDSV3_FRAG_SIZE);
		return (-1);
	}
...
/*
 * ib_send_slab cache
 * Uses rdsv3_ib_frag_destructor.
 */
/* ARGSUSED */
int
rdsv3_ib_send_constructor(void *buf, void *arg, int kmflags)
{
	struct rdsv3_page_frag *frag = (struct rdsv3_page_frag *)buf;
	struct rdsv3_ib_device *rds_ibdev = (struct rdsv3_ib_device *)arg;
	ibt_iov_attr_t iov_attr;
	...
	frag->f_page = kmem_alloc(RDSV3_FRAG_SIZE, kmflags);
	if (frag->f_page == NULL) {
		RDSV3_DPRINTF2("rdsv3_ib_send_constructor",
		    "kmem_alloc for %d failed", RDSV3_FRAG_SIZE);
		return (-1);
	}
	frag->f_refcnt = 0;

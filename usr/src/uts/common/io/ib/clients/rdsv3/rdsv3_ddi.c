...
/* Set to 176 (224-48), and can be tuned to 56 (104-48) */
uint_t		rdsv3_max_inline = RDSV3_MAX_INLINE;
...
rdsv3_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (rdsv3_max_inline >= RDSV3_MAX_INLINE)
		rdsv3_max_inline = RDSV3_MAX_INLINE;
...
rdsv3_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	/* the driver detach is not supported currently */
	return (DDI_FAILURE);
#endif
}

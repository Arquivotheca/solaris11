...
int ibd_port_settle_time = 30;		/* seconds */

ib_mtu_t ibd_multicast_mtu = IB_MTU_2K;
 ...
 */
static mblk_t *ibd_acache_tx(ibd_state_t *, ibd_ace_t *,  mblk_t *);
 ...
 */
static boolean_t ibd_send(ibd_state_t *, ibd_ace_t *, mblk_t *);
 ...
 * id_ac_rwlock
 */
_NOTE(RWLOCK_PROTECTS_DATA(ibd_state_t::id_ac_rwlock,
...
_NOTE(SCHEME_PROTECTS_DATA("Hot cache copy, it is okay to change",
...
ibd_queue_work_slot(ibd_state_t *state, ibd_req_t *ptr, int op)
{
	...
	if (op == IBD_ASYNC_FLUSH) {
		list_insert_head(&state->id_req_list, ptr);
	} else {
...
ibd_async_work(ibd_state_t *state)
{
	...
	for (;;) {
		...
		if (ptr && ptr->rq_op == IBD_ASYNC_FLUSH) {
			/*
			 * This is an "abort async work" signal.
			 * Purge the rest of the queue by freeing all the
			 * async work requests and signal the thread waiting on
			 * the flush cv.
			 */
			kmem_cache_free(state->id_req_kmc, ptr);
			while (ptr = list_get_head(&state->id_req_list)) {
				kmem_cache_free(state->id_req_kmc, ptr);
			}
			mutex_enter(&state->id_worker_flush_lock);
			state->id_worker_flushed = 1;
			cv_signal(&state->id_worker_flush_cv);
			mutex_exit(&state->id_worker_flush_lock);
		}
...
ibd_acache_init(ibd_state_t *state)
{
	...
	rw_init(&state->id_ac_rwlock, NULL, RW_DRIVER, NULL);
	mutex_init(&state->id_mc_mutex, NULL, MUTEX_DRIVER, NULL);
	rw_enter(&state->id_ac_rwlock, RW_WRITER);
	...
	state->id_ah_inprog_hash = mod_hash_create_extended(
	    "IBD in progress AH hash", state->id_hash_size,
	    mod_hash_null_keydtor, mod_hash_null_valdtor, ibd_hash_by_id, NULL,
	    ibd_hash_key_cmp, KM_SLEEP);
	...
	/*
	 * Create the taskq to do the acache entry creation.
	 */
	(void) snprintf(tqname, MAXNAMELEN, "acache taskq %s",
	    state->id_link_name);
	state->id_ac_taskq = ddi_taskq_create(state->id_dip, tqname,
	    IBD_NUM_ACACHE_TASKQ_THREADS, TASKQ_DEFAULTPRI, 0);
	if (state->id_ac_taskq == NULL) {
		rw_exit(&state->id_ac_rwlock);
		ibd_acache_fini(state);
		return (DDI_FAILURE);
	}
 ...
 */
ibd_ace_t *
ibd_acache_find(mod_hash_t *hash, ipoib_mac_t *mac, boolean_t hold, int num)
{
	ibd_ace_t *ptr;
	 ...
	 * Do hash search.
	 */
	if (mod_hash_find(hash, (mod_hash_key_t)mac, (mod_hash_val_t)&ptr)
	    == 0) {
		if (hold)
			INC_REF(ptr, num);
		return (ptr);
...
void
ibd_acache_ace_create(void *arg)
{
	ibd_state_t *state;
	ibd_req_t *req = arg;
	ipoib_mac_t *mac;
	...
	ace = req->rq_ptr2;
	mac = &ace->ac_mac;
	kmem_cache_free(state->id_req_kmc, req);

	DPRINT(4, "ibd_acache_ace_create :  %08X:%08X:%08X:%08X:%08X",
	...
	if ((state->id_link_state != LINK_STATE_UP) ||
		...
		rw_enter(&state->id_ac_rwlock, RW_WRITER);
		IBD_ACACHE_PULLOUT_INPROG(state, ace);
		mutex_enter(&ace->ac_mutex);
		mp = ace->ac_mblk_head;
		ace->ac_mblk_head = NULL;
		ace->ac_mblk_tail = NULL;
		ace->ac_mblk_cnt = 0;
		mutex_exit(&ace->ac_mutex);
	...
	if (mac->ipoib_qpn == htonl(IB_MC_QPN)) {
		...
		bcopy((char *)mac, &(nmac), IPOIB_ADDRL);
		/*
		 * If we can not find or join the group or even
		 * redirect, error out.
		 */
		if ((mce = ibd_async_mcache(state, &nmac, &redirected)) ==
		    NULL) {
			rw_enter(&state->id_ac_rwlock, RW_WRITER);
			IBD_ACACHE_PULLOUT_INPROG(state, ace);
			mutex_enter(&ace->ac_mutex);
		 ...
		 */
		if (redirected) {
			DPRINT(4, "ibd_acache_ace_create :  redirected to "
			    "%08X:%08X:%08X:%08X:%08X",
			    htonl(mac->ipoib_qpn), htonl(mac->ipoib_gidpref[0]),
			...
			rw_enter(&state->id_ac_rwlock, RW_WRITER);
			if ((rce = ibd_acache_find(state->id_ah_active_hash,
			    &nmac, B_FALSE, 0)) != NULL) {
				mblk_t *rmps;
				...
				ace->ac_mblk_head = NULL;
				ace->ac_mblk_tail = NULL;
				mblk_cnt = ace->ac_mblk_cnt;
				ace->ac_mblk_cnt = 0;
				mutex_exit(&ace->ac_mutex);
				IBD_ACACHE_INSERT_FREE(state, ace);
				INC_REF(rce, mblk_cnt);
				rw_exit(&state->id_ac_rwlock);
				rmps = ibd_acache_tx(state, rce, mp);
				if (rmps)
					freemsgchain(rmps);
				DPRINT(4, "ibd_acache_ace_create : router AH "
				    "found");
				return;
			}
			rw_exit(&state->id_ac_rwlock);
	...
	ibd_n2h_gid(&ace->ac_mac, &destgid);
	...
	if (ibt_get_paths(state->id_ibt_hdl, IBT_PATH_PKEY, &path_attr, 1,
		...
		DPRINT(10, "ibd_acache_ace_create : failed in ibt_get_paths");
		goto error;
	}
	if (ibt_modify_ud_dest(ace->ac_dest, state->id_mcinfo->mc_qkey,
	    ntohl(ace->ac_mac.ipoib_qpn),
	    &path_info.pi_prim_cep_path.cep_adds_vect) != IBT_SUCCESS) {
		DPRINT(10, "ibd_acache_ace_create : failed in "
		    "ibt_modify_ud_dest");
		goto error;
	}
	 ...
	 */
	if (ace->ac_type == IBD_RC_CAPABLE) {
		ASSERT(ace->ac_chan == NULL);
		DPRINT(10, "ibd_acache_ace_create: calling ibd_rc_try_connect"
		    "(ace=%p)", ace);
		ibd_rc_try_connect(state, ace, &path_info);
		if (ace->ac_chan == NULL) {
			DPRINT(10, "ibd_acache_ace_create: failed to setup RC"
			    " channel");
			state->rc_conn_fail++;
			goto error;
		}
	...
	if (mce != NULL) {
		...
		ibd_n2h_gid(&nmac, &mgid);

		nmce = IBD_MCACHE_FIND_FULL(state, mgid);
		if (nmce == NULL) {
			rw_exit(&state->id_ac_rwlock);
			goto error;
		}
	...
	mblk_cnt = ace->ac_mblk_cnt;
	ace->ac_mblk_cnt = 0;
	ace->ac_state = IBD_AC_LKUP_SUCCESS;
	mutex_exit(&ace->ac_mutex);
	INC_REF(ace, mblk_cnt);
	IBD_ACACHE_INSERT_ACTIVE(state, ace);
	rw_exit(&state->id_ac_rwlock);

	rmps = ibd_acache_tx(state, ace, mp);
	if (rmps)
		freemsgchain(rmps);
...
#endif
	*err = IBD_AC_LKUP_SUCCESS;

	rw_enter(&state->id_ac_rwlock, RW_READER);

	/*
	 * This is the hotpath. Consecutive send operations to the same
	 * destination will result in immediate retreval of ACE from the
	 * id_ac_hot_ace.
	 */
	...
	/*
	 * Fast path. We have the ACE in the active hash but it was not the most
	 * recently used destination. Get it from the acache and return it.
	 */
	if (((ptr = ibd_acache_find(state->id_ah_active_hash, mac, B_TRUE,
	    numwqe)) != NULL)) {
	...
	/*
	 * ACE lookup in progress. This destination does not have a ACE in the
	 * active hash but the address resolution is already in progress. We
	 * return the in progress ACE and tell the caller to chain up the mblk
	 * in the ACE.
	 */
	if (((ptr = ibd_acache_find(state->id_ah_inprog_hash, mac, B_FALSE,
	    numwqe)) != NULL)) {
		*err = IBD_AC_LKUP_INPROG;
		rw_exit(&state->id_ac_rwlock);
		return (ptr);
	}
	...
	/*
	 * The actual operation to lookup the IB paths and create the ace is
	 * currently synchronous. We need to upgrade the rwlock to RW_WRITER
	 * here. Calling rw_tryupgrade will not work since there can be many
	 * threads entering this path with the rwlock held as READER and if
	 * all of them try to upgrade here then they will deadlock.
	 * Alternatively id_ah_op and friends can be protected by id_ah_mutex
	 * and the lookup path alone can be moved under rwlock.
	 */
	rw_exit(&state->id_ac_rwlock);
	rw_enter(&state->id_ac_rwlock, RW_WRITER);

	/*
	 * The acache entry could have been inserted into the active hash while
	 * we were upgrading the rwlock to writer. Check the cache again.
	 */
	if (((ptr = ibd_acache_find(state->id_ah_active_hash, mac, B_TRUE,
	    numwqe)) != NULL)) {
		state->id_ac_hot_ace = ptr;
	...
	/*
	 *  Similarly check the in progress cache again.
	 */
	if (((ptr = ibd_acache_find(state->id_ah_inprog_hash, mac, B_FALSE,
	    numwqe)) != NULL)) {
		*err = IBD_AC_LKUP_INPROG;
	...
	if ((ptr = IBD_ACACHE_GET_FREE(state)) == NULL) {
		/*
		 ...
		 */
		if ((ptr = ibd_acache_get_unref(state)) == NULL) {
			/*
			 * Pretty serious shortage now.
			 */
			rw_exit(&state->id_ac_rwlock);
			*err = IBD_AC_LKUP_FAILED;
			DPRINT(10, "ibd_acache_lookup : failed to find AH "
			    "slot\n");
			return (NULL);
		}
		ptr->ac_mce = NULL;
	...
	bcopy((char *)mac, &(ptr->ac_mac), IPOIB_ADDRL);
	if (state->id_enable_rc && (mac->ipoib_qpn != htonl(IB_MC_QPN)) &&
	    (htonl(mac->ipoib_qpn) & IBD_MAC_ADDR_RC)) {
		ptr->ac_type = IBD_RC_CAPABLE;
	} else {
		ptr->ac_type = IBD_UD_CAPABLE;
	}

	req = kmem_cache_alloc(state->id_req_kmc, KM_NOSLEEP);
	if (req == NULL) {
		IBD_ACACHE_INSERT_FREE(state, ptr);
		rw_exit(&state->id_ac_rwlock);
		*err = IBD_AC_LKUP_FAILED;
		DPRINT(10, "ibd_acache_lookup : failed to allocate req slot\n");
		return (NULL);
	}
	req->rq_ptr = state;
	req->rq_ptr2 = ptr;
	req->rq_op = IBD_ASYNC_GETAH;

	ptr->ac_state = IBD_AC_LKUP_INPROG;
	IBD_ACACHE_INSERT_INPROG(state, ptr);

	if (ddi_taskq_dispatch(state->id_ac_taskq, ibd_acache_ace_create, req,
	    DDI_NOSLEEP) != DDI_SUCCESS) {
		IBD_ACACHE_PULLOUT_INPROG(state, ptr);
		IBD_ACACHE_INSERT_FREE(state, ptr);
		rw_exit(&state->id_ac_rwlock);
		kmem_cache_free(state->id_req_kmc, req);
		*err = IBD_AC_LKUP_FAILED;
		DPRINT(10, "ibd_acache_lookup : failed to dispatch to acache "
		    "taskq\n");
		return (NULL);
	}
	rw_exit(&state->id_ac_rwlock);
...
ibd_acache_get_unref(ibd_state_t *state)
{
	...
	ASSERT(rw_write_held(&state->id_ac_rwlock));
	...
	while (ptr != NULL) {
		/*
		 * Note that it is possible that the "cycle" bit is set on the
		 * AH w/o any reference count. The mcg must have been deleted,
		 * and the tx cleanup just decremented the reference count to 0,
		 * but hasn't gotten around to grabbing the id_ac_rwlock to move
		 * the AH into the free list.
		 */
		if (GET_REF(ptr) == 0) {
			if (ptr->ac_chan != NULL) {
...
ibd_acache_recycle(ibd_state_t *state, ipoib_mac_t *acmac, boolean_t force)
{
	...
	if ((acactive = ibd_acache_find(state->id_ah_active_hash, acmac,
	    B_FALSE, 0)) != NULL) {
...
ibd_async_link(ibd_state_t *state, ibd_req_t *req)
{
	...
	state->id_link_state_change_in_progress--;
	if (state->id_link_state_change_in_progress == 0) {
...
update_link_state:
	...
	/*
	 * Since the same async thread processes all events as well as other
	 * operations serially, it is possible that it may take some time
	 * for the async thread to get to the processing of an event.
	 * In high availability configurations, it is critical that we
	 * report LINK_DOWN events quickly to the mac layer so that upper
	 * layers can quickly failover to another working interface.
	 * So here we report LINK_DOWN event immediately to the mac layer.
	 * We also enqueue the operation for later processing by the async
	 * thread to preserve the ordering of the event with respect to the
	 * other events.
	 *
	 * The check for state->id_mh != NULL is probably not required.
	 * It is kept here for just in case.
	 */
	if (new_link_state == LINK_STATE_DOWN && state->id_mh != NULL) {
		mac_link_update(state->id_mh, LINK_STATE_DOWN);
	}
...
ibd_part_busy(ibd_state_t *state)
{
	...
	if (state->id_ah_num_inprog != 0) {
		DPRINT(10, "ibd_part_busy: failed: Connection eshtablisment in "
		    "progress\n");
		return (DDI_FAILURE);
	}
	return (DDI_SUCCESS);
...
ibd_state_init(ibd_state_t *state, dev_info_t *dip)
{
	...
	mutex_init(&state->id_worker_flush_lock, NULL, MUTEX_DRIVER, NULL);
	cv_init(&state->id_worker_flush_cv, NULL, CV_DEFAULT, NULL);
	...
	state->id_rc_rx_rwqe_thresh_orig = IBD_DEF_RC_RX_RWQE_THRESH;
...
ibd_state_fini(ibd_state_t *state)
{
	...
	cv_destroy(&state->id_worker_flush_cv);
	mutex_destroy(&state->id_worker_flush_lock);
...
ibd_iba_join(ibd_state_t *state, ib_gid_t mgid, ibd_mce_t *mce)
{
	...
	mcg_attr.mc_rate_req.r_srate = state->id_mcinfo->mc_adds_vect.av_srate;
	mcg_attr.mc_rate_req.r_selector = IBT_EQU;
	mcg_attr.mc_hop = state->id_mcinfo->mc_adds_vect.av_hop;
...
ibd_join_group(ibd_state_t *state, ib_gid_t mgid, uint8_t jstate)
{
	...
	if (((jstate == IB_MC_JSTATE_FULL) || (jstate ==
	    IB_MC_JSTATE_SEND_ONLY_NON)) && ((omce = IBD_MCACHE_FIND_FULL(state,
	    mgid)) != NULL)) {
	...
	if (jstate == IB_MC_JSTATE_NON) {
		 ...
		 */
		mutex_enter(&state->id_mc_mutex);
		omce = IBD_MCACHE_FIND_FULL(state, mgid);
		if (omce != NULL) {
			omce->mc_fullreap = B_FALSE;
			if (omce->mc_jstate == IB_MC_JSTATE_SEND_ONLY_NON) {
    ...
    uint8_t jstate)
{
	...
	if (jstate == IB_MC_JSTATE_FULL) {
		...
		if (IBD_MCACHE_FIND_NON(state, mgid) != NULL)
			do_detach = B_FALSE;
		/*
		 * We could have come here in Tx completion path even for
		 * send-only non-member MCE. we should not detach in that case
		 */
		if (mce->mc_jstate == IB_MC_JSTATE_SEND_ONLY_NON)
			do_detach = B_FALSE;
	 ...
	 * the mcg and are about to lose track of this information.
	 */
	if (do_detach) {
...
query_bcast_grp:
	...
	if (!found) {
		if (state->id_create_broadcast_group) {
			...
			mcg_attr.mc_mtu_req.r_mtu = ibd_multicast_mtu;
			mcg_attr.mc_mtu_req.r_selector = IBT_EQU;
			_NOTE(NOW_INVISIBLE_TO_OTHER_THREADS(state->id_mgid))
			state->id_mgid.gid_prefix =
			    (((uint64_t)IB_MCGID_IPV4_PREFIX << 32) |
    ...
    ibt_subnet_event_t *event)
{
	...
	switch (code) {
		case IBT_SM_EVENT_UNAVAILABLE:
			 ...
			 */
			DPRINT(10, "IBA multicast support "
...
ibd_m_getcapab(void *arg, mac_capab_t cap, void *cap_data)
{
	...
	case MAC_CAPAB_LINKID: {
		...
		cap_lid->mpl_linkid = state->id_plinkid;
		break;
	}
	default:
		return (B_FALSE);
    ...
    uint_t pr_valsize, const void *pr_val)
{
	...
	if (strcmp(pr_name, "_ibd_rc_num_rwqe") == 0) {
		...
		if (result < IBD_MIN_RC_NUM_RWQE ||
			...
			if (state->id_rc_rx_rwqe_thresh > state->id_rc_num_rwqe)
				...
				state->id_rc_rx_rwqe_thresh_orig =
				    state->id_rc_rx_rwqe_thresh;
	...
	if (strcmp(pr_name, "_ibd_rc_rx_rwqe_thresh") == 0) {
		...
		if (result < IBD_MIN_RC_RX_RWQE_THRESH ||
			...
			state->id_rc_rx_rwqe_thresh_orig = (uint32_t)result;
		}
		return (err);
	}
    ...
    void *pr_val)
{
	...
	if (strcmp(pr_name, "_ibd_rc_rx_rwqe_thresh") == 0) {
		value = state->id_rc_rx_rwqe_thresh_orig;
		err = 0;
		goto done;
	}
	if (strcmp(pr_name, "_ibd_rc_tx_comp_count") == 0) {
...
ibd_undo_start(ibd_state_t *state, link_state_t cur_link_state)
{
	...
	/* Stop ACE creation */
	if (state->id_ac_taskq != NULL) {
		ddi_taskq_wait(state->id_ac_taskq);
		ddi_taskq_destroy(state->id_ac_taskq);
		state->id_ac_taskq = NULL;
	}
	...
	req = kmem_cache_alloc(state->id_req_kmc, KM_SLEEP);
	/*
	 * Tell the worker thread to purge the remaining tasks that it
	 * has on its list. We are going away. There is no way of getting new
	 * requests in the worker queue after this, since we are in the unplumb
	 * path. All the possible work generators have now been quiesced.
	 */
	state->id_worker_flushed = 0;
	ibd_queue_work_slot(state, req, IBD_ASYNC_FLUSH);

	mutex_enter(&state->id_worker_flush_lock);
	while (!state->id_worker_flushed) {
		cv_wait(&state->id_worker_flush_cv,
		    &state->id_worker_flush_lock);
	}
	mutex_exit(&state->id_worker_flush_lock);
	...
	if (progress & IBD_DRV_BCAST_GROUP_JOINED) {
		...
		/*
		 * Make sure that the id_mc_non list is empty before
		 * ibd_acache_fini() gets called.
		 */
		ibd_async_unsetprom(state);
	...
	if (progress & IBD_DRV_ACACHE_INITIALIZED) {
		...
		mod_hash_destroy_hash(state->id_ah_inprog_hash);
		rw_exit(&state->id_ac_rwlock);
		ibd_acache_fini(state);
...
ibd_start(ibd_state_t *state)
{
	...
	/*
	 * get link_name again just in case if it has changed after the
	 * the partition creation.
	 */
	(void) dls_mgmt_get_linkinfo(state->id_plinkid, state->id_link_name,
	    NULL, NULL, NULL);
 ...
 */
static boolean_t
ibd_send(ibd_state_t *state, ibd_ace_t *ace, mblk_t *mp)
{
	ibd_swqe_t *node;
	ipoib_mac_t *dest;
	...
	if (ace->ac_type == IBD_RC_CAPABLE) {
		if (ace->ac_chan == NULL) {
			...
			if (ace->ac_chan->chan_state ==
				...
				ibd_print_warn(state, "ibd_send: RC Channel not"
				    " in ESTABLISHED state. Will use UD channel"
				    " for this send\n");
			}
		}
	}
	...
	/* Send in RC mode */
	if ((ace->ac_type == IBD_RC_CAPABLE) && rc_chan) {
		ibt_iov_t iov_arr[IBD_MAX_SQSEG];
		ibt_iov_attr_t iov_attr;
		uint_t		i;
		size_t	blksize;
...
ibd_m_tx(void *arg, mblk_t *mp)
{
	...
	while (mp != NULL) {
		...
		dest = (ipoib_mac_t *)&ipibp->ib_dst;
		if ((ntohl(dest->ipoib_qpn) & IB_QPN_MASK) == IB_MC_QPN)
			IBD_FILL_SCOPE_PKEY(dest, state->id_scope,
			    state->id_pkey);

		ace = ibd_acache_lookup(state, dest, &ret, 1);
		/*
		 * When ibd_acache_lookup returns IBD_AC_LKUP_SUCCESS, we have
		 * incremented the ref count by 1, so the state is not going to
		 * change, however more care is needed in handling in progress
		 * state.
		 */
		if (ret == IBD_AC_LKUP_SUCCESS) {
			if (ibd_send(state, ace, mp) == B_FALSE) {
				/* Send fail */
				mp->b_next = next;
				break;
			}
		} else if (ret == IBD_AC_LKUP_INPROG) {
			/*
			 * enter the ac_mutex and check the current ace state.
			 * This is the slow path where we are waiting for the
			 * address cache entry for the IB destination to be
			 * created. We take the mutex only in this slow path.
			 */
			mutex_enter(&ace->ac_mutex);
			if (ace->ac_state == IBD_AC_LKUP_INPROG &&
			    ace->ac_mblk_cnt <= IBD_DEF_RC_NUM_SWQE) {
				if (ace->ac_mblk_head == NULL)
					ace->ac_mblk_head = mp;
				else
					ace->ac_mblk_tail->b_next = mp;
				ace->ac_mblk_tail = mp;
				ace->ac_mblk_cnt++;
				mutex_exit(&ace->ac_mutex);
				/*
				 * If the ac_state had changed to LKUP_SUCCESS
				 * between lookup and taking the mutex, we need
				 * to send the mblk instead of dropping.
				 */
			} else if (ace->ac_state == IBD_AC_LKUP_SUCCESS) {
				INC_REF(ace, 1);
				mutex_exit(&ace->ac_mutex);
				if (ibd_send(state, ace, mp) == B_FALSE) {
					/* Send fail */
			...
			/*
			 * If ace lookup failed, then we have to drop the mp.
			 */
	...
	if (tofreemps)
		freemsgchain(tofreemps);

	return (mp);
}

static mblk_t *
ibd_acache_tx(ibd_state_t *state, ibd_ace_t *ace,  mblk_t *mp)
{
	mblk_t *next;
	...
	if (DEC_REF_DO_CYCLE(ace)) {
		...
		if (GET_REF_CYCLE(ace) == 0) {
			...
			if (mce != NULL) {
				...
				/*
				 * If we don't get a req structure system is
				 * very low in memory. The MCE will be freed
				 * during unplumb if we don't free now.
				 *
				 * A full member MCG leave will happen either
				 * via ibd_m_multicst entry point or via
				 * ibd_undo_start. If we have done a full
				 * member join, we are interested in sending as
				 * well as receiving from the MCG. If we leave
				 * a full member join in the Tx ACE reference
				 * decrement path, we will not be able to
				 * receive any more data from this MCG but the
				 * MAC layer might still think we should be
				 * able to since it did not do a ibd_m_multicst
				 * call to leave this MCG.
				 *
				 * A send only MCG membership is created on the
				 * first send to the MCG MAC address. If a full
				 * member join MCE already exists that is
				 * reused. In the Tx reference path, we should
				 * only leave a send-only MCG. A full member MCG
				 * join will be removed in ibd_m_multicst. A
				 * receive only non-member MCG will not come
				 * here because this is Tx path.
				 */
				req = kmem_cache_alloc(state->id_req_kmc,
				    KM_NOSLEEP);
				if (req != NULL) {
...
static void
ibd_port_settle_wait(ibd_state_t *state, int64_t port_attach_time)
{
	int64_t			end_time;
	clock_t			delay_time;
	...
	end_time = port_attach_time +
	    drv_usectohz(ibd_port_settle_time * 1000000);
	delay_time = drv_usectohz(200 * 1000);
	pinfop = NULL;

	while (ddi_get_lbolt64() < end_time) {
		if ((ibt_query_hca_ports(state->id_hca_hdl,
		    state->id_port, &pinfop, &psize, &pinfosz)
		    == IBT_SUCCESS) &&
		    (psize == 1) &&
		    (pinfop->p_linkstate == IBT_PORT_ACTIVE)) {
			break;
		}
...
/*
 * This function gets invoked when the command "dladm up-part [linkname]" is
 * run. The usual functionality is to create a partition. But in case the
 * partition has already been created and plumbed when this command was run, it
 * tries to bring up the partition if the previous attempt to bring up
 * resulted in the partition state being down. The partition state can be down
 * for a few reasons. When the partition is plumbed, if
 * - The physical link is down OR
 * - The IPoIB broadcast group is not present OR
 * - The PKEY over which this partition is created is absent
 * plumb succeeds but the link state will be marked as "down".
 * Generally, subsequent availability of any of these will be indicated by an
 * event which gets handled and an attempt to bring up the interface is made.
 * But in case we miss the event (may be due to heavy traffic on the fabric),
 * "dladm up-part [linkname]" could be used to attempt an interface bring-up.
 */
    ...
    int *rvalp)
{
	...
	ibt_status_t 		ibt_status;
	uint_t 			psize, pinfosz;
	boolean_t		force_create = B_FALSE, try_plumb = B_FALSE;
	...
	for (p = ibd_objlist_head; p; p = p->id_next) {
		if ((p->id_port_inst == cmd->ibdioc.ioc_port_inst) &&
			...
			if (p->id_mac_state & IBD_DRV_IN_LATE_HCA_INIT) {
				try_plumb = B_TRUE;
				break;
			}
	...
	if (try_plumb) {
		ibd_set_mac_progress(p, IBD_DRV_START_IN_PROGRESS);
		if (p->id_mac_state & IBD_DRV_IN_DELETION) {
			ibd_clr_mac_progress(p, IBD_DRV_START_IN_PROGRESS);
			rval = EIO;
		} else {
			rval = ibd_start(p);
			ibd_clr_mac_progress(p, IBD_DRV_START_IN_PROGRESS);
	...
	state->id_attach_time	= ddi_get_lbolt64();

	mutex_init(&state->id_macst_lock, NULL, MUTEX_DRIVER, NULL);
	cv_init(&state->id_macst_cv, NULL, CV_DEFAULT, NULL);

	if (dls_mgmt_get_linkinfo(state->id_plinkid, state->id_link_name,
	    NULL, NULL, NULL) != 0) {
		(void) strlcpy(state->id_link_name, "unknown", MAXLINKNAMELEN);
	}
	...
	ibd_port_settle_wait(state, port_state->id_attach_time);
...
static void
ibdpd_async_link(void *arg)
{
	ibd_state_t	*state = (ibd_state_t *)arg;
	link_state_t	lstate;

	DPRINT(10, "ibdpd_async_link()");

	mutex_enter(&state->id_mt_lock);
	if (state->id_mt_state == IBD_MT_PORT_UNATTACH_IN_PROGRESS)
		goto out;
	...
	if (state->id_link_state != lstate) {
		/* id_link_mutex is held here to make warlock happy */
		mutex_enter(&state->id_link_mutex);
		state->id_link_state = lstate;
		mac_link_update(state->id_mh, lstate);
		mutex_exit(&state->id_link_mutex);
    ...
    ibt_async_code_t code, ibt_async_event_t *event)
{
	...
	case IBT_EVENT_PORT_UP:
	case IBT_ERROR_PORT_DOWN:
		(void) ddi_taskq_dispatch(state->id_async_event_taskq,
		    ibdpd_async_link, state, DDI_SLEEP);
		break;
...
ibd_port_attach(dev_info_t *dip)
{
	...
	/* initialize the id_link_name field just in case some one uses it */
	(void) strlcpy(state->id_link_name, "unknown", MAXLINKNAMELEN);
	...
	state->id_async_event_taskq = ddi_taskq_create(dip,
	    "ibp-port-async_event-taskq", 1, TASKQ_DEFAULTPRI, 0);
	if (state->id_async_event_taskq == NULL) {
		DPRINT(10, "ibd_port_attach: failed to create async event "
		    "task queue: dip = 0x%p", (void *)dip);
		ddi_soft_state_free(ibd_list, instance);
		return (DDI_FAILURE);
	}
	state->id_mac_state |= IBD_DRV_ASYNC_EVENT_TASKQ_CREATED;

	mutex_init(&state->id_link_mutex, NULL, MUTEX_DRIVER, NULL);
	mutex_init(&state->id_mt_lock, NULL, MUTEX_DRIVER, NULL);

	/*
	 * Acquire this lock to block the processing of async events such as
	 * IBT_EVENT_PORT_UP/DOWN until the port attach is done.
	 */
	mutex_enter(&state->id_mt_lock);
	...
	state->id_mt_state = IBD_MT_PORT_UNATTACH_IN_PROGRESS;
	mutex_exit(&state->id_mt_lock);
...
ibd_port_unattach(ibd_state_t *state, dev_info_t *dip)
{
	...
	if (progress & IBD_DRV_ASYNC_EVENT_TASKQ_CREATED) {
		ddi_taskq_destroy(state->id_async_event_taskq);
		state->id_mac_state &= (~IBD_DRV_ASYNC_EVENT_TASKQ_CREATED);
	}

	mutex_destroy(&state->id_mt_lock);
	mutex_destroy(&state->id_link_mutex);

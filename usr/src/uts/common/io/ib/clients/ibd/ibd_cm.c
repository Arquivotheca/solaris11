...
#include <sys/ib/ibtl/ibvti.h>	/* for oc_cm_retry_cnt */
...
ibd_rc_process_rx(ibd_rc_chan_t *chan, ibd_rwqe_t *rwqe, ibt_wc_t *wc)
{
	...
	if (rxcnt < state->id_rc_rx_rwqe_thresh) {
...
#endif
		/*
		 * Check whether memory is getting tight. This is a workaround
		 * to avoid memory exhaustion.
		 */
		if (freemem < lotsfree) {
			if (freemem < desfree) {
				if (freemem < minfree) {
					state->rc_mem_extr_short++;
					if (state->id_rc_rx_rwqe_thresh >
					    IBD_RC_RWQE_MEM_EX_SHORT) {
						state->id_rc_rx_rwqe_thresh =
						    IBD_RC_RWQE_MEM_EX_SHORT;
					}
				} else {
					state->rc_mem_very_short++;
					if (state->id_rc_rx_rwqe_thresh >
					    IBD_RC_RWQE_MEM_VERY_SHORT) {
						state->id_rc_rx_rwqe_thresh =
						    IBD_RC_RWQE_MEM_VERY_SHORT;
					}
				}
		...
		if (state->id_rc_rx_rwqe_thresh !=
			...
			if (freemem > (lotsfree + desfree)) {
				state->id_rc_rx_rwqe_thresh =
				    state->id_rc_rx_rwqe_thresh_orig;
				state->rc_rwqe_thresh_recovered++;
			} else {
				if (freemem > lotsfree) {
					if (state->id_rc_rx_rwqe_thresh <
					    IBD_RC_RWQE_MEM_SHORT) {
...
ibd_rc_stop_listen(ibd_state_t *state)
{
	...
	if (state->rc_listen_hdl != NULL) {
		if (state->rc_listen_bind) {
			ret = ibt_unbind_service(state->rc_listen_hdl,
			    state->rc_listen_bind);
			if (ret != IBT_SUCCESS) {
				DPRINT(40, "ibd_rc_stop_listen:"
				    "ibt_unbind_service() failed for "
				    "rc_listen_bind: ret=%d", ret);
			} else {
				state->rc_listen_bind = NULL;
			}
		}

		if (state->rc_listen_bind == NULL) {
			ret = ibd_deregister_service(state->rc_listen_hdl);
			if (ret != 0) {
				DPRINT(40, "ibd_rc_stop_listen:"
				    "ibd_deregister_service() failed, ret=%d",
				    ret);
			} else {
	...
	if (state->rc_listen_hdl_OFED_interop != NULL) {
		if (state->rc_listen_bind_OFED_interop) {
			ret = ibt_unbind_service(
			    state->rc_listen_hdl_OFED_interop,
			    state->rc_listen_bind_OFED_interop);
			if (ret != 0) {
				DPRINT(40, "ibd_rc_stop_listen:"
				    "ibt_unbind_service() failed for "
				    "rc_listen_bind_OFED_interop: ret=%d", ret);
			} else {
				state->rc_listen_bind_OFED_interop = NULL;
			}
		}

		if (state->rc_listen_bind_OFED_interop == NULL) {
			ret = ibd_deregister_service(
			    state->rc_listen_hdl_OFED_interop);
			if (ret != 0) {
				DPRINT(40, "ibd_rc_stop_listen:"
    ...
    uint64_t ietf_cm_service_id)
{
	...
	open_args.oc_path_rnr_retry_cnt	= IBD_PATH_RNR_RETRY_COUNT;
	open_args.oc_path_retry_cnt = IBD_PATH_RETRY_COUNT;
	open_args.oc_cm_retry_cnt = IBD_CM_RETRY_COUNT;
	...
	status = ibt_open_rc_channel(chan->chan_hdl, IBT_OCHAN_CM_RETRY,
	...
	if (status == IBT_CM_FAILURE &&
	    open_returns.rc_status == IBT_CM_TIMEOUT) {
		status = IBT_IBMF_TIMEOUT;
	}
...
ibd_rc_act_close(ibd_rc_chan_t *chan, boolean_t is_close_rc_chan)
{
	...
	case IBD_RC_STATE_ACT_ESTAB:
		...
		if (is_close_rc_chan) {
			/*
			 * Closing the RC channel with IBT_NOCALLBACKS will
			 * result in a non blocking close with no CM callbacks
			 * on completion (either successful or timeout). The
			 * ibt_close_hca will wait till the qpn is free to be
			 * reused. This will include close time (including CM
			 * timeout if remote side does not respond) and the
			 * IBCM TIMEWAIT period (time to wait before reusing
			 * the qpn).
			 */
			ret = ibt_close_rc_channel(chan->chan_hdl,
			    IBT_NOCALLBACKS, NULL, 0, NULL, NULL,
			    0);
			if (ret != IBT_SUCCESS) {
    ...
    ibt_priv_data_len_t ret_len_max)
{
	...
	case IBT_CM_EVENT_CONN_CLOSED:
		...
		if ((rc_chan->chan_state == IBD_RC_STATE_ACT_ESTAB) &&
		    ((ace = ibd_acache_find(state->id_ah_active_hash,
		    &ace->ac_mac, B_FALSE, 0)) != NULL) &&
		    (ace == rc_chan->ace)) {

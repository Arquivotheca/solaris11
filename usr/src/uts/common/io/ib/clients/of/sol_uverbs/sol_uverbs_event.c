...
/* ARGSUSED */
void
uverbs_async_qp_event_handler(ibt_hca_hdl_t hca_hdl,
...
/* ARGSUSED */
void
uverbs_async_cq_event_handler(ibt_hca_hdl_t hca_hdl,
	enum ib_event_type code, ibt_async_event_t *event)
{
...
/* ARGSUSED */
void
uverbs_async_srq_event_handler(ibt_hca_hdl_t hca_hdl,
	enum ib_event_type code, ibt_async_event_t *event)
{
...
/* ARGSUSED */
void
uverbs_async_unaff_event_handler(ibt_hca_hdl_t hca_hdl,
	enum ib_event_type code, ibt_async_event_t *event)
{
	...
	for (i = 0; i < uo_tbl->uobj_tbl_used_blks; i++) {
		...
		for (j = 0; j < SOL_OFS_UO_BLKSZ; j++) {
			 ...
			 * to the idicated hca.
			 */
			if (uctxt->hca && uctxt->hca->drv_hca_hdl == hca_hdl &&
			    uctxt->async_evfile) {
...
/* ARGSUSED */
void
uverbs_async_event_handler(struct ib_event_handler *hdlr,
	struct ib_event *ib_eventp)
{
...
#ifdef DEBUG
	if (hdlr->handler != uverbs_async_event_handler) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "uverbs_async_event_handler: invalid ptr");
		return;
	}
#endif
	ofa_type = ib_eventp->event;
	hca_hdl = ib_eventp->element.sol_ib_evt.sol_evt_hca_hdl;
	event = ib_eventp->element.sol_ib_evt.sol_evt_event;

	SOL_OFS_DPRINTF_L5(sol_uverbs_dbg_str,
	    "ASYNNC EVENT HANDLER: entry, ofa_event=%d", ofa_type);
	...
	switch (ofa_type) {
		...
		case IB_EVENT_PATH_MIG:
			 ...
			 */
			uverbs_async_qp_event_handler(hca_hdl,
			    ofa_type, event);
			break;
		...
		case IB_EVENT_CQ_ERR:
			 ...
			 */
			uverbs_async_cq_event_handler(hca_hdl,
			    ofa_type, event);
			break;
		...
		case IB_EVENT_SRQ_LIMIT_REACHED:
			 ...
			 */
			uverbs_async_srq_event_handler(hca_hdl,
			    ofa_type, event);
			break;
		...
		case IB_EVENT_PATH_MIG_ERR:
			 ...
			 */
			uverbs_async_unaff_event_handler(hca_hdl,
			    ofa_type, event);
			break;

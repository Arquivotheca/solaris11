    ...
    int in_len, int out_len)
{
	...
	rc = ibt_alloc_cq(uctxt->hca->drv_hca_hdl, &cq_attr,
	    &ucq->cq, &real_size);
	...
	rc = ibt_ci_data_out(uctxt->hca->drv_hca_hdl, IBT_CI_NO_FLAGS,
	    IBT_HDL_CQ, (void *) ucq->cq,  &resp.drv_out,
	    sizeof (resp.drv_out));
	if (rc != IBT_SUCCESS) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
...
err_cq_destroy:
	if ((status = ibt_free_cq(ucq->cq)) != IBT_SUCCESS)
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "create_cq: ibt_free_cq() failed, "
		    "status %d", status);
    ...
    int in_len, int out_len)
{
	...
	/*
	 * Allocate response buffer to include all WCs requested.
	 */
	alloc_resp = (uchar_t *)kmem_zalloc(
	    (sizeof (struct ib_uverbs_wc) * cmd.ne) +
	    sizeof (struct ib_uverbs_poll_cq_resp), KM_NOSLEEP);
	resp = (struct ib_uverbs_poll_cq_resp *)alloc_resp;
	if (resp == NULL) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "poll_cq: Resp Allocation Error");
		rc = ENOMEM;
		goto err_poll;
	}
	...
	/*
	 * WC array will be at the end of cq_resp struct. Total length
	 * to copyout
	 * 	sizeof (resp) + (num WCs returned * sizeof (WC))
	 */
	ofed_wc = (struct ib_uverbs_wc *)(alloc_resp +
	    sizeof (struct ib_uverbs_poll_cq_resp));
	copyout_size = sizeof (struct ib_uverbs_poll_cq_resp) +
	    (sizeof (struct ib_uverbs_wc) * (resp->count));

	for (i = 0; i < resp->count; i++)
		uverbs_convert_wc(uctxt, &completions[i], &ofed_wc[i]);

#ifdef	_LP64
	rc = copyout((void*)&resp, (void*)cmd.response.r_laddr, copyout_size);
#else
	rc = copyout((void*)&resp, (void*)cmd.response.r_addr, copyout_size);
#endif
	if (rc != 0) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
	...
	kmem_free(alloc_resp, (sizeof (struct ib_uverbs_wc) * cmd.ne) +
	    sizeof (struct ib_uverbs_poll_cq_resp));
	sol_ofs_uobj_put(&ucq->uobj);

    ...
    int in_len, int out_len)
{
	...
	uint8_t				do_qp_free = 1;

	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "create_qp: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
	...
	if ((cmd.sq_sig_all & LIB_RDMACM_QP_BIT) &&
	    cmd.qp_type  == IB_QPT_RC)
		uqp->is_rdmacm_rc_qp = B_TRUE;
	else
		uqp->is_rdmacm_rc_qp = B_FALSE;

	cmd.sq_sig_all &= ~LIB_RDMACM_QP_BIT;
	...
	qp_attr.qp_sizes.cs_inline	= cmd.max_inline_data;
	 ...
	 */
	rc = ibt_alloc_qp(uctxt->hca->drv_hca_hdl, qp_type, &qp_attr,
	    &qp_sizes, &uqp->qp_num, &uqp->qp);
	if (rc != IBT_SUCCESS) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
	...
	rc = ibt_ci_data_out(uctxt->hca->drv_hca_hdl, IBT_CI_NO_FLAGS,
	    IBT_HDL_CHANNEL, (void *)uqp->qp, &resp.drv_out,
	    sizeof (resp.drv_out));
	if (rc != IBT_SUCCESS) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
	...
	if (qp_sizes.cs_inline)
		resp.max_inline_data	= qp_sizes.cs_inline;
...
err_qp_destroy:
	/*
	 * ibt_free_qp() should not fail here, as the qp, is not yet used.
	 * In the unlikely event that it is fails, do not free uqp.
	 */
	if (ibt_free_qp(uqp->qp) != IBT_SUCCESS) {
		do_qp_free = 0;
		goto err_put;
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "destroy_qp: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "modify_qp: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "query_qp: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
...
#ifdef	_LP64
	rc = copyout((void*)&resp, (void*)cmd.response.r_laddr,
	    sizeof (resp));
#else
    ...
    int in_len, int out_len)
{
	...
	int					rc;
	ibt_status_t				status;
	uint_t					srq_max_sgl, srq_max_wrs;

	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "create_srq: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
	...
	rc = ib_get_hca_srq_szs(uctxt->hca->hca_ib_devp,
	    &srq_max_wrs, &srq_max_sgl);
	if (rc) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "create_srq(): ib_get_hca_srq_szs failed");
		goto err_out;

	}
	if (attr.srq_wr_sz > srq_max_wrs ||
	    attr.srq_sgl_sz > srq_max_sgl) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
	...
	rc = ibt_alloc_srq(uctxt->hca->drv_hca_hdl, flags,
	    upd->pd, &attr, &usrq->srq, &real_attr);
	 ...
	 */
	rc = ibt_ci_data_out(uctxt->hca->drv_hca_hdl, IBT_CI_NO_FLAGS,
	    IBT_HDL_SRQ, (void *)usrq->srq,  &resp.drv_out,
	    sizeof (resp.drv_out));
...
err_srq_destroy:
	if ((status = ibt_free_srq(usrq->srq)) != IBT_SUCCESS)
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "create_srq: ibt_free_srq() failed, "
		    "status %d", status);
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "modify_srq: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "query_srq: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "destroy_srq: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "attach_mcast: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}
    ...
    int in_len, int out_len)
{
	...
	if (in_len < sizeof (cmd)) {
		SOL_OFS_DPRINTF_L2(sol_uverbs_dbg_str,
		    "detach_mcast: in_len %x, cmd size %lx", in_len,
		    sizeof (cmd));
		rc = EINVAL;
		return (rc);
	}

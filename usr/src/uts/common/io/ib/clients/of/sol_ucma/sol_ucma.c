...
static int
ucma_check_privilege(struct sockaddr *sockaddrp, cred_t *credp)
{
	struct sockaddr_in	*sock_in4p;
	struct sockaddr_in6 	*sock_in6p;
	in_port_t		inp_port;
	int			rc;

	sock_in4p = (struct sockaddr_in *)sockaddrp;
	sock_in6p = (struct sockaddr_in6 *)sockaddrp;
	if (sockaddrp->sa_family == AF_INET)
		inp_port = ntohs(sock_in4p->sin_port);
	else if (sockaddrp->sa_family == AF_INET6)
		inp_port = ntohs(sock_in6p->sin6_port);
	else {
		return (0);
	}

	if (inp_port == 0 || inp_port >= 1024)
		return (0);

	rc = priv_policy(credp, PRIV_NET_PRIVADDR, B_FALSE, EACCES, NULL);
	if (rc != 0)
		SOL_OFS_DPRINTF_L3(sol_ucma_dbg_str,
		    "check_privilege : priv_policy() ret %d", rc);
	return (rc);
...
sol_ucma_write(dev_t dev, struct uio *uio,  cred_t *credp)
{
	...
	if (user_hdrp->cmd == RDMA_USER_CM_CMD_BIND_ADDR) {
		sol_ucma_bind_addr_t	*bind_addrp;
		struct sockaddr		*sockaddrp;

		bind_addrp = (sol_ucma_bind_addr_t *)data_buf;
		sockaddrp = (struct sockaddr *)&bind_addrp->addr;
		ret = ucma_check_privilege(sockaddrp, credp);

	}
	if (user_hdrp->cmd == RDMA_USER_CM_CMD_RESOLVE_ADDR) {
		sol_ucma_resolve_addr_t	*resolve_addrp;
		struct sockaddr		*sockaddrp;

		resolve_addrp = (sol_ucma_resolve_addr_t *)data_buf;
		sockaddrp = (struct sockaddr *)&resolve_addrp->src_addr;
		ret = ucma_check_privilege(sockaddrp, credp);
	}
...
sol_ucma_destroy_id(dev_t dev, void *io_buf, struct uio *uio)
{
	...
	/*
	 * Event handling, return number of events acked for this chan.
	 */
	ASSERT(filep);
	mutex_enter(&filep->file_mutex);
	if (chanp) {

...
extern struct rdma_cm_id	*sol_cma_listen_special;
extern boolean_t	cma_listen_rescan_required;
extern int		cma_tmout_val;
extern kmutex_t		sol_cma_glob_mutex;
extern kmutex_t		sol_cma_dev_mutex;
extern int		cma_tmout_val;
extern llist_head_t	sol_cma_dev_list;

/* Tunables for Multicast MTU and Rate */
ib_mtu_t    rdma_mcast_mtu  = 0;
ibt_srate_t rdma_mcast_rate = 0;
 ...
 */
int
ibcma_ibt_to_kernel_status(ibt_status_t status)
{
	int err;
...
rdma_ib_bind_addr(struct rdma_cm_id *idp, struct sockaddr *addr)
{
	...
	if (ret == 0) {
		...
		/* Interop with Linux IPOIB port space */
		if (idp->ps == RDMA_PS_IPOIB)
			ibchanp->chan_sid &= ~IB_SID_IPADDR_PREFIX;
		SOL_OFS_DPRINTF_L5(sol_rdmacm_dbg_str,
    ...
    int *qp_attr_mask)
{
	...
	if (idp->ps == RDMA_PS_TCP && qpstate == IB_QPS_INIT) {
		...
		*qp_attr_mask = IB_QP_STATE | IB_QP_AV | IB_QP_PATH_MTU  |
		...
		return (0);
	} else if (idp->ps == RDMA_PS_TCP && qpstate == IB_QPS_RTS) {
...
ibcma_append_listen_list(struct rdma_cm_id  *root_idp)
{
	...
	if (root_ibchanp->chan_devp &&
		...
		sol_cma_add_hca_list(ep_chanp,
		    ntohll(devp->dev_node_guid), NULL);
...
ibcma_init_root_chan(sol_cma_chan_t *root_chanp, sol_cma_glbl_listen_t *listenp)
{
	...
	if (status != IBT_SUCCESS) {
		...
		return (ibcma_ibt_to_kernel_status(status));
	}
...
ibcma_init_root_sid(sol_cma_chan_t *root_chanp)
{
	...
	if (root_ibchanp->chan_sid == 0) {
		...
		root_ibchanp->chan_sid = ibt_get_ip_sid(root_idp->ps,
		    root_ibchanp->chan_port);
		/* Interop with Linux IPOIB port space */
		if (root_idp->ps == RDMA_PS_IPOIB)
			root_ibchanp->chan_sid &= ~IB_SID_IPADDR_PREFIX;
...
rdma_ib_disconnect(struct rdma_cm_id *idp)
{
	...
	if (chanp->chan_req_state == REQ_CMID_NOTIFIED ||
		...
		session_id = chanp->chan_session_id;
		mutex_exit(&chanp->chan_mutex);

		if (IS_UDP_CMID(idp)) {
			status = ibt_cm_ud_proceed(session_id,
			    NULL, IBT_CM_NO_CHANNEL, NULL, NULL, 0);
			if (status != IBT_SUCCESS) {
				SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
				    "ib_disconnect(%p) Reject for incoming REQ "
				    "failed, status %d", idp, status);
				return (ibcma_ibt_to_kernel_status(status));
			}
		} else {
			ibt_cm_proceed_reply_t	cm_reply;
			...
			status = ibt_cm_proceed(IBT_CM_EVENT_REQ_RCV,
			    session_id, IBT_CM_REJECT, &cm_reply,
			    NULL, 0);
			if (status != IBT_SUCCESS) {
				SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
				    "ib_disconnect(%p) Reject for incoming REQ "
    ...
    void *context)
{
	...
	uint_t		num_mcg_info = 0;
	ibt_mcg_info_t	*mcg_query_infop = NULL;
	...
	if (sol_cma_any_addr(addr)) {
		...
		ipaddr2mgid(addr, &mcast_gid_horder, devp->dev_pkey, idp->ps);
	...
	mcg_attr.mc_qkey = (idp->ps == RDMA_PS_UDP) ?
	    RDMA_UDP_QKEY : SOL_IPOIB_DEFAULT_QKEY;

	/* Try to query attributes from an existing MCG */
	if (ibt_query_mcg(devp->dev_sgid, &mcg_attr, 1,
	    &mcg_query_infop, &num_mcg_info) == IBT_SUCCESS) {
		mcg_attr.mc_rate_req.r_selector = IBT_EQU;
		mcg_attr.mc_rate_req.r_srate =
		    mcg_query_infop->mc_adds_vect.av_srate;
		mcg_attr.mc_flow = mcg_query_infop->mc_adds_vect.av_flow;
		mcg_attr.mc_hop = mcg_query_infop->mc_adds_vect.av_hop;
		mcg_attr.mc_tclass = mcg_query_infop->mc_adds_vect.av_tclass;
		mcg_attr.mc_sl = mcg_query_infop->mc_adds_vect.av_srvl;
		mcg_attr.mc_mtu_req.r_selector = IBT_EQU;
		mcg_attr.mc_mtu_req.r_mtu = mcg_query_infop->mc_mtu;
		ibt_free_mcg_info(mcg_query_infop, num_mcg_info);
	} else {
		/* No MCGs exist */
		mcg_attr.mc_flow = 0;
		...
		if (idp->ps == RDMA_PS_UDP) {
			/*
			 * Check the tunable for a user specified rate.
			 * If not, use the best possible rate.
			 */
			if ((rdma_mcast_rate >= IBT_SRATE_2) &&
			    (rdma_mcast_rate <= IBT_SRATE_120)) {
				mcg_attr.mc_rate_req.r_selector = IBT_EQU;
				mcg_attr.mc_rate_req.r_srate = rdma_mcast_rate;
			} else {
				mcg_attr.mc_rate_req.r_selector = IBT_BEST;
			}
		} else {
			/* Always use 10Gb/s for IPoIB to interop with Linux */
			mcg_attr.mc_rate_req.r_selector = IBT_EQU;
			mcg_attr.mc_rate_req.r_srate = IBT_SRATE_10;
		}

		/*
		 * Assign MTU from a user specified tunable.
		 * If not specified, use MTU < 4K to interop with Linux.
		 */
		if ((rdma_mcast_mtu >= IB_MTU_256) &&
		    (rdma_mcast_mtu <= IB_MTU_4K)) {
			mcg_attr.mc_mtu_req.r_selector = IBT_EQU;
			mcg_attr.mc_mtu_req.r_mtu = rdma_mcast_mtu;
		} else {
			mcg_attr.mc_mtu_req.r_selector = IBT_LT;
			mcg_attr.mc_mtu_req.r_mtu = IB_MTU_4K;
		}
	}
    ...
    struct rdma_conn_param *conn_paramp)
{
	...
	chan_qp_hdl = chanp->chan_qp_hdl;
	mutex_exit(&chanp->chan_mutex);
	status = ibt_open_rc_channel(chan_qp_hdl, flags, IBT_NONBLOCKING,
	    &args, NULL);
	mutex_enter(&chanp->chan_mutex);
	if (status != IBT_SUCCESS) {
		SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
    ...
    uint8_t num_paths, ibt_path_ip_src_t *src_ip_p)
{
	...
	routep = &idp->route;
	ib_addr_set_sgid(&routep->addr.dev_addr,
	    &((routep->path_rec[0]).sgid));
	ib_addr_set_dgid(&routep->addr.dev_addr,
	    &((routep->path_rec[0]).dgid));
    ...
    ibt_priv_data_len_t priv_datalen)
{
	...
	if (eventp->cm_type == IBT_CM_UD_EVENT_SIDR_REQ) {
		...
		if (ibt_status != IBT_SUCCESS) {
			...
			return (ibcma_ibt_to_kernel_status(ibt_status));
		}
		...
		event_devp = event_ibchanp->chan_devp;
		event_devp->dev_pkey = sidr_req->sreq_pkey;
		if (ibt_pkey2index((event_idp->device)->hca_hdl,
		    event_devp->dev_port_num, event_devp->dev_pkey,
		    &(event_devp->dev_pkey_ix))
		    != IBT_SUCCESS) {
			SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
			    "create_new_id failed!!");
    ...
    enum rdma_cm_event_type *event, int *evt_status)
{
	...
	*evt_status = 0;
	...
	routep = &event_idp->route;
	routep->num_paths = reqp->req_alt_hca_port ? 2 : 1;
	routep->path_rec = kmem_zalloc(
	    sizeof (struct ib_sa_path_rec) * (routep->num_paths),
	    KM_SLEEP);
	...
	ibt_addsvect2sa_path(&reqp->req_prim_addr,
	    &(routep->path_rec[0]), base_lid);
	(routep->path_rec[0]).mtu = (uint8_t)rtr_data.req_path_mtu;
	ib_addr_set_sgid(&routep->addr.dev_addr,
	    &((routep->path_rec[0]).sgid));
	ib_addr_set_dgid(&routep->addr.dev_addr,
	    &((routep->path_rec[0]).dgid));
	...
	*event_id_ptr = event_idp;
	...
	ipaddr2sockaddr(&info.dst_addr,
	    &(event_idp->route.addr.src_addr), &info.src_port);
	event_devp = event_ibchanp->chan_devp;
	event_devp->dev_pkey = reqp->req_pkey;
	if (ibt_pkey2index((event_idp->device)->hca_hdl,
	    event_devp->dev_port_num, event_devp->dev_pkey,
	    &(event_devp->dev_pkey_ix))
	    != IBT_SUCCESS) {
    ...
    enum rdma_cm_event_type *event, int *evt_status)
{
	...
	*evt_status = 0;
	...
	case IBT_CM_FAILURE_REJ_SENT :
		/*
		 * Pass on Stale connections on active side.
		 * Stale connections on passive side are not
		 * notified as IBT_CM_FAILURE_REJ_SENT.
		 *
		 * For Reject initiated by rdma_reject() do
		 * not pass the event back to consumer.
		 */
		if (failedp->cf_reason == IBT_CM_CONN_STALE &&
		    !root_idp) {
			*event_id_ptr  = idp;
...
static void
ibcma_set_port_mask(sol_cma_chan_t *chanp, uint_t num_ports,
    cma_device_t *cma_devp)
{
	uint32_t	mask;

	ASSERT(MUTEX_HELD(&sol_cma_glob_mutex));
	if (!cma_devp)
		return;
	if (chanp != (sol_cma_chan_t *)sol_cma_listen_special)
		return;

	for (mask = 0x1; num_ports; num_ports--, mask *= 2)
		cma_devp->cma_port_mask |= mask;
	cma_devp->cma_port_mask |= CMA_PORT_MASK_INITED;
}

static void
ibcma_set_listen_port(sol_cma_chan_t *chanp, uint_t port,
    cma_device_t *cma_devp)
{
	uint32_t	mask;
	...
	for (mask = 0x1; port > 1; port--, mask *= 2)
		;
	cma_devp->cma_port_listen_mask |= mask |
	    CMA_PORT_MASK_INITED;
}
    ...
    int num_hcas, genlist_t *ret_devlist, boolean_t with_ipaddr_only)
{
	...
	for (i = 0; i < num_hcas; i++) {
		...
		cma_devp = cma_get_hca_device(htonll(hca_guidp[i]));
		if (!cma_devp) {
			mutex_exit(&sol_cma_dev_mutex);
			mutex_exit(&sol_cma_glob_mutex);
			SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
			    "get_devlist: HCA GUID 0x%llx not yet in "
			    "common list", hca_guidp[i]);
			continue;
		}
		mutex_exit(&sol_cma_dev_mutex);

		if (root_chanp !=
		    (sol_cma_chan_t *)sol_cma_listen_special) {
			mutex_exit(&sol_cma_glob_mutex);
		} else
			mutex_exit(&sol_cma_glob_mutex);

		status = ibt_query_hca_ports(
		    (cma_devp->cma_device)->hca_hdl, 0,
		    &port_info, &num_ports, &port_size);
		if (status !=  IBT_SUCCESS) {
			SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
			    "ibt_query_hca_ports failed %d", status);
			continue;
		}
		mutex_enter(&sol_cma_glob_mutex);
		if (cma_devp)
			ibcma_set_port_mask(root_chanp, num_ports,
			    cma_devp);
		mutex_exit(&sol_cma_glob_mutex);
		...
		for (p = 0, tmp = port_info; p < num_ports; p++, tmp++) {
			...
			for (s = 0; s < num_sgids; s++) {
				...
				for (pk = 0; pk < num_pkeys; pk++) {
					...
					/*
					 * We will add only 1 Pkey for same
					 * IB port SGID, ibt_bind_service()
					 * fails when called more than once for
					 * IB port SGID for a service. The Pkey
					 * in the Req data will be used to fill
					 * Pkey info for the CMID created on
					 * connection request.
					 */
					break;
				}
		...
		if (cma_devp && cma_devp->cma_port_listen_mask !=
		    cma_devp->cma_port_mask) {
			mutex_enter(&sol_cma_glob_mutex);
...
extern void sol_cma_release_device(struct rdma_cm_id *);

void
cma_tmout_hdlr(void *arg)
{
	struct rdma_cm_id	*listen_cmid, *ep_idp;
	sol_cma_chan_t		*listen_chanp, *ep_chanp;
	ibcma_chan_t		*ep_ibchanp;
	cma_device_t		*cma_devp;
	boolean_t		local_rescan_req = B_FALSE;
	ibt_status_t		status;
	llist_head_t		*entry;
	ibt_hca_portinfo_t	*port_info, *tmp;
	uint_t			num_ports, p;
	...
	listen_cmid = (struct rdma_cm_id *)arg;
	listen_chanp = (sol_cma_chan_t *)listen_cmid;
	...
	if (cma_listen_rescan_required == B_FALSE) {
		mutex_exit(&sol_cma_glob_mutex);
		goto resched_tmout;
	}
	mutex_exit(&sol_cma_glob_mutex);
	...
	list_for_each(entry, &sol_cma_dev_list) {
		...
		/* Listening on all HCA ports, skip */
		if ((cma_devp->cma_port_mask &
		    CMA_PORT_MASK_INITED) &&
		    cma_devp->cma_port_mask ==
		    cma_devp->cma_port_listen_mask)
			continue;
		...
		if (status !=  IBT_SUCCESS) {
			SOL_OFS_DPRINTF_L2(sol_rdmacm_dbg_str,
			    "ibt_query_hca_ports_byguid failed %d",
			    status);
			continue;
		}
		if (cma_devp->cma_port_mask == 0) {
			mutex_enter(&sol_cma_glob_mutex);
			ibcma_set_port_mask(listen_chanp, num_ports,
			    cma_devp);
			mutex_exit(&sol_cma_glob_mutex);
		}

		for (mask = 0x01, p = 0, tmp = port_info;
		    p < num_ports; p++, tmp++, mask *= 2) {
			if (tmp->p_linkstate != IBT_PORT_ACTIVE)
				continue;
			if (mask & cma_devp->cma_port_listen_mask)
				continue;

			num_sgids = tmp->p_sgid_tbl_sz /
			    sizeof (ib_gid_t);
			for (s = 0; s < num_sgids; s++) {
				if (tmp->p_sgid_tbl[s].gid_guid == 0x0LL)
					continue;
				...
				devp = kmem_zalloc(
				    sizeof (ibcma_dev_t), KM_SLEEP);
				devp->dev_node_guid =
				    (cma_devp->cma_device)->node_guid;
				devp->dev_port_num = p + 1;
				devp->dev_pkey_ix = 0;
				devp->dev_pkey = tmp->p_pkey_tbl[0];
				devp->dev_sgid = tmp->p_sgid_tbl[s];

				ep_idp = ibcma_create_new_id(
				    listen_cmid);
				ASSERT(ep_idp);

				ep_chanp = (sol_cma_chan_t *)ep_idp;
				ep_chanp->chan_xport_type =
				    SOL_CMA_XPORT_IB;
				...
				ASSERT(ep_ibchanp->chan_devp == NULL);
				ep_ibchanp->chan_devp = devp;
				ep_ibchanp->chan_port = p + 1;

				if (rdma_ib_listen(ep_idp,
				    listen_chanp->chan_listen_bklog)) {
					SOL_OFS_DPRINTF_L2(
					    sol_rdmacm_dbg_str,
					    "cma_tmout_hdlr: listen(%p)"
					    " failed!!", ep_chanp);

					if (ibcma_fini_ep_chan(ep_chanp))
						continue;
					kmem_free(
					    ep_chanp->chan_listenp,
					    sizeof (sol_cma_listen_info_t));
				...
				listenp->listen_ep_root_entry =
				    ...
				    listen_cmid);
				sol_cma_add_hca_list(ep_chanp,
				    ntohll(devp->dev_node_guid),
				    cma_devp);
			}
			cma_devp->cma_port_listen_mask |=
			    mask | CMA_PORT_MASK_INITED;
		}
		if (cma_devp->cma_port_listen_mask !=
		    cma_devp->cma_port_mask)
			local_rescan_req = B_TRUE;
		ibt_free_portinfo(port_info, port_size);
	}
	mutex_exit(&sol_cma_dev_mutex);

resched_tmout:
	if (local_rescan_req == B_TRUE) {
		mutex_enter(&sol_cma_glob_mutex);
		cma_listen_rescan_required = B_TRUE;
		mutex_exit(&sol_cma_glob_mutex);
	}
	(void) timeout(cma_tmout_hdlr, arg,

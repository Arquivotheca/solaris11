...
static int eib_fip_send_update(eib_t *, eib_vnic_t *, eib_wqe_t *, uint_t,
    int *);
...
eib_fip_heartbeat(eib_t *ss, eib_vnic_t *vnic, int *err)
{
	...
	do {
		/*
		 * Even if we're running low on the wqe resource, we want
		 * to be able to grab a wqe to send the keepalive, to avoid
		 * getting logged out by the gateway, so we use a higher
		 * priority.
		 */
...
eib_fip_parse_ctl_pkt(uint8_t *pkt, eib_vnic_t *vnic)
{
	...
	if (ret == EIB_E_SUCCESS) {
		...
		ss->ei_ka_gw_heartbeat = ddi_get_lbolt64();
		if (vnic_state == EIB_LOGIN_TBL_DONE ||
		    vnic_state == EIB_LOGIN_DONE) {
			ss->ei_ka_gw_eport_state =
...
eib_fip_send_login(eib_t *ss, eib_vnic_t *vnic, eib_wqe_t *swqe, int *err)
{
	...
	ret = ibt_modify_ud_dest(swqe->qe_dest, EIB_FIP_QKEY,
	    ss->ei_gw_props->pp_gw_ctrl_qpn, &swqe->qe_av);
	if (ret != IBT_SUCCESS) {
		*err = EINVAL;
    ...
    uint_t nxt_state, int *err)
{
	...
	bzero(av, sizeof (ibt_adds_vect_t));
	av->av_srate = IBT_SRATE_10;
	av->av_srvl = ld->ld_gw_sl;
	av->av_port_num = ss->ei_props->ep_port_num;
	av->av_send_grh = B_FALSE;
	av->av_dlid = ld->ld_gw_lid;
	av->av_src_path = 0;

	/*
	 * Modify the UD destination handle to the destination
	 */
	ret = ibt_modify_ud_dest(swqe->qe_dest, EIB_FIP_QKEY,
	    ld->ld_gw_ctl_qpn, &swqe->qe_av);
	if (ret != IBT_SUCCESS) {
		*err = EINVAL;
    ...
    eib_vhub_update_t *upd, boolean_t tbl_done, uint8_t eport_state)
{
	...
	if (eport_state_changed) {
		EIB_DPRINTF_DEBUG(ss->ei_instance,
		    "eib_fip_update_eport_state: "
		    "eport state changed to %d", eport_state);
	}
}

...
eibnx_name_child(dev_info_t *child, char *name, size_t namesz)
{
	...
	if ((node_name = ddi_get_parent_data(child)) == NULL) {
		ENX_DPRINTF_DEBUG("ddi_get_parent_data(child=0x%llx) "
		    "returned NULL", child);
...
eibnx_start_port_monitor(eibnx_hca_t *hca, eibnx_port_t *port)
{
	...
	if ((hca_dip = ibtl_ibnex_hcaguid2dip(ti->ti_hca_guid)) == NULL) {
		...
		ENX_DPRINTF_DEBUG("hca driver instance (%d) invalid for "
		    "hca_guid=0x%llx, hca_dip=0x%llx",
		    ti->ti_hca_guid, hca_dip);
	} else {
    ...
    dev_info_t **childp)
{
	...
	if (ret != NDI_SUCCESS) {
		ENX_DPRINTF_DEBUG("ndi_devi_online(node_name=%s) failed "
...
eibnx_locate_unconfigured_node(eibnx_thr_info_t **ti_p, eibnx_gw_info_t **gwi_p)
{
	...
	for (ti = ss->nx_thr_info; ti; ti = ti->ti_next) {
		...
		for (ch = ti->ti_child; ch; ch = ch->ch_next) {
			if (ch->ch_dip == NULL) {
				/*
				 * In linux-compatibility-mode, if this child
				 * was originally configured, but the vnic had
				 * been deleted subsequently on the nm2gw, we
				 * will now have the gateway info indicate
				 * that num_net_vnics is 0.  In this case, we
				 * don't return this as an unconfigured node
				 * worthy of configuring.
				 */
				if ((enx_linux_compat_mode) &&
				    (ch->ch_gwi->gw_num_net_vnics == 0)) {
					continue;
				}

				/*
				 * Otherwise, return this is as the next
				 * unconfigured node that may be configured.
				 */
				*ti_p = ti;
				*gwi_p = ch->ch_gwi;

...
eib_rb_adm_setup_ud_channel(eib_t *ss)
{
	...
	if (chan->ch_chan) {
		...
		/*
		 * We've now marked the channel in the error state. We'll now
		 * wait for all the CQEs to be generated and the channel to
		 * get to the "last wqe reached" state.
		 */
		...
		/*
		 * Now we'll mark the wait flag so that any completion handler
		 * run after this point will know that we'll be waiting
		 * for the cq to be drained.  Of course, we also need to make
		 * sure at least one more pass of the cq handler is done.
		 */
		mutex_enter(&chan->ch_cqstate_lock);

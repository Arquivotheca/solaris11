...
/*
 * Linux compatibility mode for eoib datalink instantiation.
 *
 * With this mode set, eoib datalinks will be instantiated only
 * when the gateway unicast advertisements also carry a non-zero
 * 'num-net-vnics' value in addition to a set 'gw-available' flag.
 *
 * With this mode not set, every gateway that declares itself
 * available via the 'gw-available' flag in an unicast advertisement
 * will result in an eoib datalink instantiation.
 */
int enx_linux_compat_mode = 1;

/*
 * Link Training delay affects instantiation of eoib datalinks
 * during bus-config.
 */
uint_t enx_link_training_time = ENX_DFL_LNK_TRNG_TIME;
    ...
    ddi_bus_config_op_t op, void *arg, dev_info_t **childp)
{
	...
	if (ret == NDI_SUCCESS) {
		ret = ndi_busop_bus_config(parent, flags, op, arg, childp,
		    wait_time);
	}
...
eibnx_config_all_children(dev_info_t *parent)
{
	...
	if (!ss->nx_monitors_up) {
		...
		/*
		 * Wait till atleast one port monitor has detected its port
		 * to be up (or a maximum time of enx-link-training-time has
		 * elapsed) before declaring that the port monitors are up
		 * and allowing this bus config to continue.  We don't really
		 * care how we're woken up, as long as we wake up.
		 */
		if (enx_link_training_time > ENX_MAX_LNK_TRNG_TIME)
			enx_link_training_time = ENX_MAX_LNK_TRNG_TIME;

		deadline = ddi_get_lbolt() +
		    SEC_TO_TICK(enx_link_training_time);
		(void) cv_timedwait(&ss->nx_monitors_cv, &ss->nx_lock,
		    deadline);
...
eibnx_config_child(char *devname, dev_info_t **childp)
{
	...
	if (eibnx_locate_node_name(devname, &ti, &gwi) == ENX_E_FAILURE) {
		ENX_DPRINTF_VERBOSE("eibnx_config_child: invalid eoib "
		    "nodename '%s', no such address", devname);
		return (ENX_E_FAILURE);
	}

	if ((enx_linux_compat_mode) && (gwi->gw_num_net_vnics == 0))
		return (ENX_E_FAILURE);

...
static boolean_t eib_rsrc_ok_to_free_pool(eib_t *, eib_wqe_pool_t *);
...
static void eib_rb_rsrc_setup_txbufs(eib_t *);
static void eib_rb_rsrc_setup_rxbufs(eib_t *);
static void eib_rb_rsrc_setup_lsobufs(eib_t *);
...
eib_rsrc_setup_bufs(eib_t *ss, int *err)
{
	...
	if (ss->ei_caps->cp_lso_maxlen && ss->ei_caps->cp_cksum_flags &&
		...
		if (eib_rsrc_setup_lsobufs(ss, err) != EIB_E_SUCCESS) {
			eib_rb_rsrc_setup_txbufs(ss);
			return (EIB_E_FAILURE);
		}
	}

	if (eib_rsrc_setup_rxbufs(ss, err) != EIB_E_SUCCESS) {
		eib_rb_rsrc_setup_lsobufs(ss);
		eib_rb_rsrc_setup_txbufs(ss);
		return (EIB_E_FAILURE);
	}
...
void
eib_rsrc_return_swqe(eib_t *ss, eib_wqe_t *wqe)
{
	ASSERT(ss->ei_tx != NULL);
...
void
eib_rsrc_return_rwqe(eib_t *ss, eib_wqe_t *wqe)
{
	ASSERT(ss->ei_rx != NULL);
...
eib_wqe_t *
eib_rsrc_nxt_rwqe_to_free(eib_t *ss)
{
	eib_wqe_pool_t *wp = ss->ei_rx;
	eib_wqe_t *wqe = NULL;
	uint64_t busy_wqes_in_blk;
	int blk;
	int ndx;
	int wqe_ndx;

	mutex_enter(&wp->wp_lock);
	for (blk = 0; blk < EIB_BLKS_PER_POOL; blk++) {
		busy_wqes_in_blk = ~(wp->wp_free_wqes[blk]);
		if (busy_wqes_in_blk) {
			ndx = EIB_FIND_LSB_SET(busy_wqes_in_blk);
			wqe_ndx = blk * EIB_WQES_PER_BLK + ndx;
			wqe = &wp->wp_wqe[wqe_ndx];
			break;
		}
	}
...
void
eib_rb_rsrc_setup_bufs(eib_t *ss)
{
	eib_rb_rsrc_setup_rxbufs(ss);
	eib_rb_rsrc_setup_lsobufs(ss);
	eib_rb_rsrc_setup_txbufs(ss);
}
...
eib_rsrc_setup_txbufs(eib_t *ss, int *err)
{
	...
	for (i = 0, blk = 0; blk < EIB_BLKS_PER_POOL; blk++) {
		for (ndx = 0; ndx < EIB_WQES_PER_BLK; ndx++, i++) {
			 ...
			 * encapsulation header for non-TCP_UDP/IP packets
			 * (no checksum bits, no multiple segments, no FCS).
			 */
			encap_hdr = (uint32_t *)(void *)wqe->qe_payload_hdr;
			*encap_hdr = htonl(EIB_TX_ENCAP_HDR);
...
/*ARGSUSED*/
static boolean_t
eib_rsrc_ok_to_free_pool(eib_t *ss, eib_wqe_pool_t *wp)
{
	uint64_t free_blks;
	int i;
...
static void
eib_rb_rsrc_setup_txbufs(eib_t *ss)
{
	eib_wqe_pool_t *wp = ss->ei_tx;
	eib_wqe_t *wqe;
	ibt_ud_dest_hdl_t dest;
	 ...
	 */
	if (eib_rsrc_ok_to_free_pool(ss, wp)) {
...
static void
eib_rb_rsrc_setup_rxbufs(eib_t *ss)
{
	 ...
	 */
	if (eib_rsrc_ok_to_free_pool(ss, rx)) {
		if (rx->wp_mr) {
			if ((ret = ibt_deregister_mr(ss->ei_hca_hdl,
...
static void
eib_rb_rsrc_setup_lsobufs(eib_t *ss)
{
	eib_lsobkt_t *bkt;
	ibt_status_t ret;
	...
	/*
	 * Try to stop the lso bufs monitor thread.
	 */
	if (eib_stop_monitor_lso_bufs(ss) != EIB_E_SUCCESS)
		return;

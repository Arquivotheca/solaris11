...
eibnx_state_init(void)
{
	...
	cv_init(&ss->nx_monitors_cv, NULL, CV_DEFAULT, NULL);
...
eibnx_setup_cq(eibnx_thr_info_t *info)
{
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_DEBUG("ibt_query_hca(hca_hdl=0x%llx) failed, "
		    "ret=%d", info->ti_hca, ret);
		return (ENX_E_FAILURE);
	}
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_DEBUG("ibt_alloc_cq(hca_hdl=0x%llx, cq_sz=0x%lx) "
...
eibnx_setup_ud_channel(eibnx_thr_info_t *info)
{
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_DEBUG("ibt_pkey2index(hca_hdl=0x%llx, "
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_DEBUG("ibt_alloc_ud_channel(hca_hdl=0x%llx, "
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_DEBUG("ibt_query_ud_channel(chan_hdl=0x%llx) "
...
eibnx_setup_txbufs(eibnx_thr_info_t *info)
{
	...
	if ((ret = ibt_register_mr(info->ti_hca, info->ti_pd, &attr,
		...
		ENX_DPRINTF_DEBUG("ibt_register_mr() failed for tx "
	...
	for (i = 0; i < ENX_NUM_SWQE; i++) {
		...
		ret = ibt_alloc_ud_dest(info->ti_hca, IBT_UD_DEST_NO_FLAGS,
		    info->ti_pd, &dest);
		if (ret != IBT_SUCCESS) {
			ENX_DPRINTF_DEBUG("ibt_alloc_ud_dest(hca_hdl=0x%llx) "
...
eibnx_setup_rxbufs(eibnx_thr_info_t *info)
{
	...
	if ((ret = ibt_register_mr(info->ti_hca, info->ti_pd, &attr,
		...
		ENX_DPRINTF_DEBUG("ibt_register_mr() failed for rx "
		    "region (0x%llx, 0x%llx) with ret=%d",
		    attr.mr_vaddr, attr.mr_len, ret);
...
eibnx_setup_bufs(eibnx_thr_info_t *info)
{
	...
	for (i = 0; i < ENX_NUM_RWQE; i++) {
		...
		if (ret != IBT_SUCCESS) {
			ENX_DPRINTF_DEBUG("ibt_post_recv(chan_hdl=0x%llx) "
			    "failed, ret=%d", info->ti_chan, ret);
...
eibnx_join_solicit_mcg(eibnx_thr_info_t *info)
{
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_WARN("ibt_join_mcg() failed for solicit "
...
eibnx_join_advertise_mcg(eibnx_thr_info_t *info)
{
	...
	ret = ibt_join_mcg(rgid, &mcg_attr, &mcg_info, NULL, NULL);
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_WARN("ibt_join_mcg() failed for advertise "
	...
	if (ret != IBT_SUCCESS) {
		ENX_DPRINTF_WARN("ibt_attach_mcg(chan_hdl=0x%llx, "

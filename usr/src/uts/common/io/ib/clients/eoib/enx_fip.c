...
eibnx_fip_make_solicit_pkt(eibnx_thr_info_t *info, eibnx_wqe_t *swqe)
{
	...
	if (pktsz < solicit_sz) {
		ENX_DPRINTF_DEBUG("swqe bufsize too small for pkt, "
    ...
    eibnx_gw_addr_t *gw_addr)
{
	...
	if (gw_addr == NULL) {
		...
		if (ret != IBT_SUCCESS) {
			ENX_DPRINTF_DEBUG("ibt_modify_ud_dest() failed with "
	...
	if ((ret != IBT_SUCCESS) || (path.pi_hca_guid == 0)) {
		ENX_DPRINTF_DEBUG("ibt_get_paths() failed with "
    ...
    eibnx_gw_addr_t *gw_addr)
{
	...
	if (ret != IBT_SUCCESS) {
		mutex_exit(&swqe->qe_lock);
		ENX_DPRINTF_DEBUG("ibt_post_send() failed for solicit, "
		    "ret=%d", ret);
		return (ENX_E_FAILURE);
	}

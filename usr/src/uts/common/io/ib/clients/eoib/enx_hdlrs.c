...
static int eibnx_offline_children(eibnx_thr_info_t *);
...
eibnx_port_monitor(eibnx_thr_info_t *info)
{
	...
	/*
	 * Let the bus-config path know that we've detected the port
	 * to be up and it can continue with config
	 */
	mutex_enter(&ss->nx_lock);
	cv_signal(&ss->nx_monitors_cv);
	...
	if (eibnx_setup_cq(info) != ENX_E_SUCCESS) {
		ENX_DPRINTF_DEBUG("eibnx_setup_cq() failed, terminating "
	...
	if (eibnx_setup_ud_channel(info) != ENX_E_SUCCESS) {
		ENX_DPRINTF_DEBUG("eibnx_setup_ud_channel() failed, "
		"terminating port monitor for (hca_guid=0x%llx, port_num=0x%x)",
		    info->ti_hca_guid, info->ti_pi->p_port_num);
		goto port_monitor_exit;
	}
	...
	if (eibnx_setup_bufs(info) != ENX_E_SUCCESS) {
		ENX_DPRINTF_DEBUG("eibnx_setup_bufs() failed, terminating "
		    "port monitor for (hca_guid=0x%llx, port_num=0x%x)",
		    info->ti_hca_guid, info->ti_pi->p_port_num);
		goto port_monitor_exit;
	}
	...
	if (eibnx_setup_cq_handler(info) != ENX_E_SUCCESS) {
		ENX_DPRINTF_DEBUG("eibnx_setup_cq_handler() failed, "
		"terminating port monitor for (hca_guid=0x%llx, port_num=0x%x)",
		    info->ti_hca_guid, info->ti_pi->p_port_num);
		goto port_monitor_exit;
	}
	...
	if (eibnx_join_mcgs(info) != ENX_E_SUCCESS) {
		ENX_DPRINTF_DEBUG("eibnx_join_mcgs() failed, terminating ",
		    "port monitor for (hca_guid=0x%llx, port_num=0x%x)",
		    info->ti_hca_guid, info->ti_pi->p_port_num);
		goto port_monitor_exit;
	}
	...
	if (eibnx_fip_solicit_mcast(info) != ENX_E_SUCCESS) {
		ENX_DPRINTF_DEBUG("eibnx_fip_solicit_mcast() failed, "
		"terminating port monitor for (hca_guid=0x%llx, port_num=0x%x)",
		    info->ti_hca_guid, info->ti_pi->p_port_num);
		goto port_monitor_exit;
	}
...
periodic_solicit:
	...
	if (info->ti_event & ENX_EVENT_TIMED_OUT) {
		if (eibnx_fip_solicit_ucast(info,
			...
			ENX_DPRINTF_DEBUG("failed to send solicit ucast to "
    ...
    boolean_t gwi_changed)
{
	...
	/*
	 * We're here because we received a unicast advertisement from
	 * a gateway. If the linux-compatibility-mode is set, this also
	 * means that there is atleast one vnic available from this gw
	 * for us to login.
	 *
	 * It is possible that the gateway has come back alive from dead
	 * (we won't know for sure because eibnx may not catch all cases
	 * of gateway disappearing and coming back) and become aware
	 * of us.  We may need to inform any EoIB children waiting for
	 * notification.  Note that if this gateway is being discovered
	 * for the first time now, we wouldn't have created the binding
	 * eoib node for it (we will do that when we return from this
	 * routine), so the "rebirth" and "gw info update" event postings
	 * will be NOPs.
	 */
	mutex_enter(&gwi->gw_adv_lock);
	gwi->gw_adv_last_lbolt = ddi_get_lbolt64();
...
eibnx_process_rx(eibnx_thr_info_t *info, ibt_wc_t *wc, eibnx_wqe_t *wqe)
{
	...
	if (orig_gwi == NULL) {
		if (gwi->gw_flag_available == 0) {
		    ...
		    ((enx_linux_compat_mode) && (gwi->gw_num_net_vnics == 0))) {
...
eibnx_handle_login_ack(eibnx_thr_info_t *info, uint8_t *pkt)
{
	...
	if ((rdip = eibnx_find_child_dip_by_inst(info, inst)) == NULL) {
		ENX_DPRINTF_VERBOSE("no eoib child with instance 0x%x found "
	...
	if (ret != NDI_SUCCESS) {
		ENX_DPRINTF_VERBOSE("no login-ack cookie for (hca_guid=0x%llx, "
...
eibnx_handle_gw_rebirth(eibnx_thr_info_t *info, uint16_t portid)
{
	...
	if ((rdip = eibnx_find_child_dip_by_gw(info, portid)) == NULL) {
		ENX_DPRINTF_VERBOSE("no eoib child bound to gw portid 0x%x "
	...
	if (ret != NDI_SUCCESS) {
		ENX_DPRINTF_VERBOSE("no gw-available cookie for "
		    "(hca_guid=0x%llx, port_num=0x%x, gw_portid=0x%x), ret=%d",
		    info->ti_hca_guid, info->ti_pi->p_port_num, portid, ret);
		return;
	}
    ...
    void *new_gw_info)
{
	...
	if (ret != NDI_SUCCESS) {
		ENX_DPRINTF_VERBOSE("no gw-info-update cookie for "
...
/*
 * Offlines eoib children device instances during the handling
 * of a HCA DR event.
 */
static int
eibnx_offline_children(eibnx_thr_info_t *ti_stop_list)
{
	eibnx_t *ss = enx_global_ss;
	eibnx_thr_info_t *ti;
	eibnx_thr_info_t *trb;
	eibnx_child_t *ch;
	eibnx_child_t *rch;
	int circular;
	int ret;
	...
	/*
	 * Offline each eoib child dip in the "ti_stop_list".  If
	 * offlining fails at any point, rollback the ones that were
	 * successfully offlined and return failure.
	 */
	ndi_devi_enter(ss->nx_dip, &circular);
	for (ti = ti_stop_list; ti; ti = ti->ti_next) {
		for (ch = ti->ti_child; ch; ch = ch->ch_next) {
			if (ch->ch_dip) {
				ENX_DPRINTF_VERBOSE("offlining %s \n",
				    ch->ch_node_name);
				ret = ndi_devi_offline(ch->ch_dip,
				    NDI_UNCONFIG);
				if (ret != NDI_SUCCESS) {
					ENX_DPRINTF_VERBOSE(
					    "eibnx_offline_children: "
					    "ndi_devi_offline failed for "
					    "node_name=%s with ret 0x%x\n",
					    ch->ch_node_name, ret);
					goto rollback;
				}
			}
...
rollback:
	for (trb = ti_stop_list; trb; trb = trb->ti_next) {
		for (rch = trb->ti_child; (rch != NULL) && (rch != ch);
		    rch = rch->ch_next) {
			ret = ndi_devi_online(rch->ch_dip, 0);
			if (ret != NDI_SUCCESS) {
				/*
				 * If node cannot be brought back online,
				 * do the same thing as we normally do for
				 * eibnx_unconfig_child(), but don't call
				 * it directly - this child is off the main
				 * list, so we won't find it there. The child
				 * will probably be recreated on a subsequent
				 * config.
				 */
				ENX_DPRINTF_VERBOSE(
				    "eibnx_offline_children: failed to "
				    "rollback node %s to online state, "
				    "leaving it unconfigured, ret=0x%x\n",
				    rch->ch_node_name, ret);

				(void) eibnx_update_child(trb,
				    rch->ch_gwi, NULL);
			}
		}
		if (rch == ch)
...
eibnx_handle_hca_detach(ib_guid_t del_hca_guid)
{
	...
	eibnx_thr_info_t *stop_list_tail = NULL;
	...
	/*
	 * Offline the children in the stop list first.
	 */
	if ((ti_stop_list != NULL) &&
	    (eibnx_offline_children(ti_stop_list) != ENX_E_SUCCESS)) {
		/*
		 * Offline failed, add the stop list back to the
		 * main list and return.
		 */
		for (ti = ti_stop_list; ti; ti = ti->ti_next)
			stop_list_tail = ti;

		mutex_enter(&ss->nx_lock);
		stop_list_tail->ti_next = ss->nx_thr_info;
		ss->nx_thr_info = ti_stop_list;
		mutex_exit(&ss->nx_lock);
		return;
	}

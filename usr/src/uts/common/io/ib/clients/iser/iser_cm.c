    ...
    ibt_cm_return_args_t *rargsp, void *rcmp, ibt_priv_data_len_t rcmp_len)
{
	...
	/* iser_ib_alloc_channel_nopathlookup() acquired a hold on the HCA */
	...
	/* Get the IB path used by this channel and save it in ic_ibt_path */
	(void) bzero(&ipattr, sizeof (ibt_ip_path_attr_t));
	ipattr.ipa_dst_ip	= &(ipcm_info.src_addr);
	ipattr.ipa_src_ip	= ipcm_info.dst_addr;
	ipattr.ipa_hca_guid	= evp->cm_event.req.req_hca_guid;
	ipattr.ipa_hca_port_num	= evp->cm_event.req.req_prim_hca_port;
	ipattr.ipa_max_paths	= 1;
	ipattr.ipa_ndst		= 1;

	(void) bzero(&ibt_path, sizeof (ibt_path_info_t));
	ISER_LOG(CE_NOTE, "iser_ib_handle_cm_req: Get IB path "
	    "HCA(%llx) port (%d)",
	    (longlong_t)evp->cm_event.req.req_hca_guid,
	    evp->cm_event.req.req_prim_hca_port);

	status = ibt_get_ip_paths(chan->ic_hca->hca_clnt_hdl, IBT_PATH_NO_FLAGS,
	    &ipattr, &ibt_path, NULL, NULL);
	if (status != IBT_SUCCESS) {
		ISER_LOG(CE_NOTE, "iser_ib_handle_cm_req: "
		    "ibt_get_ip_paths HCA(%llx) port (%d) failed: status (%d)",
		    (longlong_t)evp->cm_event.req.req_hca_guid,
		    evp->cm_event.req.req_prim_hca_port, status);
	}
	chan->ic_ibt_path = ibt_path;

	iser_ib_hca_rele(chan->ic_hca); /* hold acquired in alloc channel */
...
iser_handle_cm_event_failure(ibt_cm_event_t *evp)
{
	...
	case ISER_CONN_STAGE_ALLOCATED:
		/*
		 * We blew up or were offlined during connection
		 * establishment. Ignore this failure on the iSER
		 * initiator, the function ibt_open_rc_channel()
		 * will also fail and its caller will handle the
		 * cleanup.
		 * On the iSER target, teardown the iSER conn and
		 * channel handles.
		 */
		mutex_exit(&chan->ic_conn->ic_lock);
		if (chan->ic_conn->ic_type == ISER_CONN_TYPE_TGT) {
			iser_internal_conn_destroy(chan->ic_conn);
		}
		break;

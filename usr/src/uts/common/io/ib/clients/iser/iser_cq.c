...
iser_ib_poll_send_completions(ibt_cq_hdl_t cq_hdl, iser_chan_t *iser_chan)
{
	...
	for (i = 0; i < npoll; i++) {
		...
		wrid = wc[i].wc_id;
		/*
		 * An odd work ID indicates this CQE is associated
		 * with an intermediate post of a RDMA data transfer
		 * operation.  These only get posted on an error condition.
		 */
		if (((uintptr_t)wrid & 1) != 0) {
			if (wc[i].wc_status != IBT_WC_SUCCESS &&
			    wc[i].wc_status != IBT_WC_WR_FLUSHED_ERR) {
				ISER_LOG(CE_NOTE,
				    "iser_ib_poll_send_completions:"
				    " wrid 0x%p gets error code %d",
				    (void *)((uintptr_t)wrid & ~1),
				    wc[i].wc_status);
			}
			continue;
		}
		...
		iser_chan->ic_sq_post_count -= wr->iw_nwrs;
		...
		if (wc[i].wc_status != IBT_WC_SUCCESS) {
			...
			if (wr->iw_buf != NULL) {
				...
				if (idb->idb_bufalloc != IDB_ZCOPY) {
					bcopy(&wc[i],
					    &((iser_buf_t *)idb->
					    idb_buf_private)->buf_wc,
					    sizeof (ibt_wc_t));
				}
#endif
				idt = idb->idb_task_binding;
				iser_hdr =
				    (iser_ctrl_hdr_t *)idt->idt_transport_hdr;

...
static ibt_status_t ibcm_get_single_pathrec(ibt_path_attr_t *,
    ibt_path_flags_t, ibtl_cm_port_list_t *, ibcm_dinfo_t *, uint8_t,
    uint8_t *, ibt_path_info_t *);

static ibt_status_t ibcm_get_multi_pathrec(ibt_path_attr_t *,
    ibt_path_flags_t, ibtl_cm_port_list_t *, ibcm_dinfo_t *dinfo,
    uint8_t *, ibt_path_info_t *);
    ...
    ibcm_dinfo_t *dinfo, uint8_t *max_count)
{
	...
	for (idx = 0; idx < dinfo->num_dest; idx++) {
		...
		if ((dgid.gid_prefix >> 56ULL & 0xFF) == 0xFF) {
			...
			retval = ibcm_get_single_pathrec(&p_arg->attr,
			    p_arg->flags, sl, dinfo, idx, &num_path_plus,
	...
	if ((unicast_dgid_present != 0) && (num_path_plus > 0)) {
		...
		if ((sl->p_multi != IBTL_CM_SIMPLE_SETUP) ||
			 ...
			 */
			retval = ibcm_get_single_pathrec(&p_arg->attr,
			    p_arg->flags, sl, dinfo, 0xFF, &num_path_plus,
			    &p_arg->paths[rec_found]);
		} else {
			uint8_t old_num_path_plus = num_path_plus;

			/* MultiPathRec will be used for other queries. */
			retval = ibcm_get_multi_pathrec(&p_arg->attr,
			    p_arg->flags, sl, dinfo, &num_path_plus,
			    &p_arg->paths[rec_found]);
			if ((retval != IBT_SUCCESS) &&
			    (retval != IBT_INSUFF_DATA) &&
			    (sl->p_count > 0) &&
				...
				retval = ibcm_get_single_pathrec(&p_arg->attr,
				    p_arg->flags, &sl_tmp, &dinfo_tmp, 0xFF,
...
static ibt_status_t
ibcm_get_single_pathrec(ibt_path_attr_t *attrp, ibt_path_flags_t flags,
    ibtl_cm_port_list_t *sl, ibcm_dinfo_t *dinfo, uint8_t idx,
    uint8_t *num_path, ibt_path_info_t *paths)
{
	...
	IBTF_DPRINTF_L3(cmlog, "ibcm_get_single_pathrec(%p, %lX, %p, %p, %d)",
	    attrp, flags, sl, dinfo, *num_path);
...
static ibt_status_t
ibcm_get_multi_pathrec(ibt_path_attr_t *attrp, ibt_path_flags_t flags,
    ibtl_cm_port_list_t *sl, ibcm_dinfo_t *dinfo, uint8_t *num_path,
    ibt_path_info_t *paths)
{
	...
	IBTF_DPRINTF_L3(cmlog, "ibcm_get_multi_pathrec(%p, %lX, %p, %p, %d)",
	    attrp, flags, sl, dinfo, *num_path);
	...
	if (flags & (IBT_PATH_AVAIL | IBT_PATH_APM)) {
	...
	if ((results_p != NULL) && (num_rec > 0)) {
		...
		if (flags & (IBT_PATH_APM | IBT_PATH_AVAIL)) {
...
static ibt_status_t ibcm_get_ip_pkey(zoneid_t myzid, char *linkname,
    ib_guid_t port_guid, ib_guid_t hca_guid, uint8_t port_num,
    ib_pkey_t *pkey_p);
    ...
    ibcm_dinfo_t *dinfo, uint8_t *max_count)
{
	...
	if ((sl->p_multi != IBTL_CM_SIMPLE_SETUP) ||
		 ...
		 */
		retval = ibcm_get_single_pathrec(&p_arg->attr,
		    p_arg->flags, sl, dinfo, 0xFF, &num_path,
		    &p_arg->paths[rec_found]);
	} else {
		/* MultiPathRec will be used for other queries. */
		retval = ibcm_get_multi_pathrec(&p_arg->attr, p_arg->flags,
		    sl, dinfo, &num_path, &p_arg->paths[rec_found]);
	}
	if ((retval != IBT_SUCCESS) && (retval != IBT_INSUFF_DATA))
...
ibcm_process_get_ip_paths(void *tq_arg)
{
	...
	ibcm_hca_info_t		*hcap;
	ibmf_saa_handle_t	saa_handle;
	ibcm_ibaddr_t		ibaddr, ibaddr2;
	zoneid_t		myzoneid;
	ib_pkey_t		my_pkey = 0, my_pkey2 = 0;


	IBTF_DPRINTF_L3(cmlog, "ibcm_process_get_ip_paths(%p, 0x%X) ",
	    p_arg, p_arg->flags);

	max_paths = num_path = p_arg->ip_attr.ipa_max_paths;
	 ...
	 */
	bzero(&ibaddr, sizeof (ibcm_ibaddr_t));
	bzero(&ibaddr2, sizeof (ibcm_ibaddr_t));

	myzoneid = (p_arg->ip_attr.ipa_zoneid == ALL_ZONES) ?
	    ALL_ZONES : GLOBAL_ZONEID;

	retval = ibcm_arp_get_ibaddr(myzoneid, p_arg->ip_attr.ipa_src_ip,
	    p_arg->ip_attr.ipa_dst_ip[0], &ibaddr);
	if (retval) {
	...
	retval = ibcm_get_ip_pkey(p_arg->ip_attr.ipa_zoneid,
	    &ibaddr.ifname[0], (ibaddr.src_mismatch ? 0 : ibaddr.sgid.gid_guid),
	    p_arg->ip_attr.ipa_hca_guid,
	    p_arg->ip_attr.ipa_hca_port_num, &my_pkey);
	if (retval) {
		IBTF_DPRINTF_L2(cmlog, "ibcm_process_get_ip_paths: "
		    "ibcm_get_ip_pkey failed: %d", retval);
		goto ippath_error;
	}

	_NOTE(NOW_INVISIBLE_TO_OTHER_THREADS(*p_arg))

	p_arg->attr.pa_sgid = ibaddr.sgid;
	p_arg->attr.pa_hca_guid = p_arg->ip_attr.ipa_hca_guid;
	p_arg->attr.pa_hca_port_num = p_arg->ip_attr.ipa_hca_port_num;
	p_arg->attr.pa_sl = p_arg->ip_attr.ipa_sl;

	bcopy(&p_arg->ip_attr.ipa_mtu, &p_arg->attr.pa_mtu,
	    sizeof (ibt_mtu_req_t));
	bcopy(&p_arg->ip_attr.ipa_srate, &p_arg->attr.pa_srate,
	    sizeof (ibt_srate_req_t));
	bcopy(&p_arg->ip_attr.ipa_pkt_lt, &p_arg->attr.pa_pkt_lt,
	    sizeof (ibt_pkt_lt_req_t));
	p_arg->attr.pa_flow = p_arg->ip_attr.ipa_flow;
	p_arg->attr.pa_hop = p_arg->ip_attr.ipa_hop;
	p_arg->attr.pa_tclass = p_arg->ip_attr.ipa_tclass;
	_NOTE(NOW_VISIBLE_TO_OTHER_THREADS(*p_arg))

	retval = ibtl_cm_get_active_plist(&p_arg->attr, p_arg->flags, &sl);
	if (retval == IBT_SUCCESS) {
		bcopy(&ibaddr.src_ip, &sl->p_src_ip, sizeof (ibt_ip_addr_t));
	} else {
		IBTF_DPRINTF_L2(cmlog, "ibcm_process_get_ip_paths: "
	...
	IBTF_DPRINTF_L4(cmlog, "ibcm_process_get_ip_paths: SGID %llX:%llX, "
	    ...
	    ibaddr.dgid.gid_prefix, ibaddr.dgid.gid_guid);

	if (p_arg->ip_attr.ipa_ndst > 1) {

		/* Get DGID for all specified Dest IP Addr */
		for (i = 1; i < p_arg->ip_attr.ipa_ndst; i++) {
			retval = ibcm_arp_get_ibaddr(myzoneid,
			    p_arg->ip_attr.ipa_src_ip,
			    p_arg->ip_attr.ipa_dst_ip[i], &ibaddr2);
			if (retval) {
				IBTF_DPRINTF_L2(cmlog,
			...
			retval = ibcm_get_ip_pkey(p_arg->ip_attr.ipa_zoneid,
			    &ibaddr2.ifname[0],
			    (ibaddr2.src_mismatch ? 0 : ibaddr.sgid.gid_guid),
			    p_arg->ip_attr.ipa_hca_guid,
			    p_arg->ip_attr.ipa_hca_port_num, &my_pkey2);
			if (retval) {
				IBTF_DPRINTF_L2(cmlog,
				    "ibcm_process_get_ip_paths: "
				    "ibcm_get_ip_pkey failed: %d", retval);
			...
			IBTF_DPRINTF_L4(cmlog, "ibcm_process_get_ip_paths: "
			    "%d, DGID%d: %llX:%llX, PKey2 %lX", i,
			    ibaddr2.dgid.gid_prefix, ibaddr2.dgid.gid_guid,
			    my_pkey2);
		}
	}

	len = p_arg->ip_attr.ipa_ndst - 1;
	if (p_arg->flags & IBT_PATH_APM) {
		retval = ibcm_get_comp_pgids(ibaddr.dgid, ibaddr2.dgid, 0,
		    &d_gids_p, &dnum);
		if ((retval != IBT_SUCCESS) &&
		    (retval != IBT_GIDS_NOT_FOUND)) {
			IBTF_DPRINTF_L2(cmlog,
	...
	dinfo->dest[0].d_gid = ibaddr.dgid;
	dinfo->p_key = dinfo->dest[0].d_pkey = my_pkey;
	dinfo->num_dest = num_dest;

	if (p_arg->ip_attr.ipa_ndst > 1) {
		dinfo->dest[1].d_gid = ibaddr2.dgid;
		if (!(p_arg->flags & IBT_PATH_APM) && my_pkey &&
		    my_pkey2 && my_pkey != my_pkey2) {
			dinfo->p_key = 0;
			dinfo->dest[1].d_pkey = my_pkey2;
		}
	} else if (dnum) {
		dinfo->dest[1].d_gid = d_gids_p[0];
	}
...
static ibt_status_t
ibcm_get_ip_pkey(zoneid_t myzid, char *linkname, ib_guid_t port_guid,
    ib_guid_t hca_guid, uint8_t port_num, ib_pkey_t *pkey_p)
{
	ibt_part_attr_t	*attr_list, *pa_attr;
	int		nparts, i;
	zoneid_t	zoneid;
	ibt_status_t	ret;
	char		ifname[MAXLINKNAMELEN];

	IBTF_DPRINTF_L4(cmlog, "ibcm_get_ip_pkey(%d, %s, %llX, %llX, %d)",
	    myzid, linkname, port_guid, hca_guid, port_num);

	ret = ibt_get_all_part_attr(&attr_list, &nparts);
	if ((ret != IBT_SUCCESS) || (nparts == 0)) {
		IBTF_DPRINTF_L2(cmlog, "ibcm_get_ip_pkey: Failed to "
		    "IB Part List - %d", nparts);
		return (ret);
	}
	IBTF_DPRINTF_L4(cmlog, "ibcm_get_ip_pkey: Found %d IB Part List",
	    nparts);
	pa_attr = attr_list;

	/* In case we do not find the match */
	ret = IBT_PATH_RECORDS_NOT_FOUND;

	for (i = 0; i < nparts; i++, pa_attr++) {
		/* Check whether is this the IB Partition we r interested in */
		if (port_guid && (pa_attr->pa_port_guid != port_guid))
			continue;
		if (hca_guid && (pa_attr->pa_hca_guid != hca_guid))
			continue;
		if (port_num && (pa_attr->pa_port != port_num))
			continue;

		if (dls_devnet_get_active_linkname(pa_attr->pa_plinkid,
		    &zoneid, ifname, MAXLINKNAMELEN) != 0) {
			IBTF_DPRINTF_L3(cmlog, "ibcm_get_ip_pkey: "
			    "dls_devnet_get_active_linkname %d failed",
			    pa_attr->pa_plinkid);
			continue;
		}

		if (strncmp(ifname, linkname, sizeof (ifname)))
			continue;

		if ((myzid != ALL_ZONES) && (myzid != zoneid))
			continue;

		IBTF_DPRINTF_L4(cmlog, "ibcm_get_ip_pkey: ifname %s, PKey %lX",
		    ifname, pa_attr->pa_pkey);
		/* Retrieve the PKey */
		*pkey_p = pa_attr->pa_pkey;
		ret = IBT_SUCCESS;
		break;
	}
...
static ibt_status_t
ibcm_val_ipaddr(boolean_t is_src_addr, ibt_ip_addr_t *ip_addr)
{
	/*
	 * For source point, INADDR_ANY is legal, but for destination
	 * it is not a valid input attributes.
	 */
	if (ip_addr->family == AF_INET) {
		if (ip_addr->un.ip4addr == INADDR_ANY) {
			if (!is_src_addr) {
				IBTF_DPRINTF_L2(cmlog, "ibcm_val_ipattr: "
				    "Dest IPv4 Addr must be specified: ERROR");
				return (IBT_NOT_SUPPORTED);
			} else {
				ip_addr->family = AF_UNSPEC;
			}
		} else if (ip_addr->un.ip4addr == INADDR_LOOPBACK) {
			if (is_src_addr)
				ip_addr->family = AF_UNSPEC;
		}
	} else if (ip_addr->family == AF_INET6) {
		if (!is_src_addr &&
		    IN6_IS_ADDR_UNSPECIFIED(&ip_addr->un.ip6addr)) {
			IBTF_DPRINTF_L2(cmlog, "ibcm_val_ipattr: Dest "
			    "IPv6 Addr must be specified: ERROR");
			return (IBT_NOT_SUPPORTED);
		}

		if (ibcm_ip6_linklocal_addr_ok &&
		    IN6_IS_ADDR_LINKLOCAL(&ip_addr->un.ip6addr)) {
			IBTF_DPRINTF_L2(cmlog, "ibcm_val_ipattr: IPv6 "
			    "Addr specified is Link Local Address: "
			    "NOT SUPPORTED");
			return (IBT_NOT_SUPPORTED);
		}

		if (is_src_addr &&
		    IN6_IS_ADDR_LOOPBACK(&ip_addr->un.ip6addr)) {
			ip_addr->un.ip6addr = ipv6_all_zeros;
			ip_addr->family = AF_UNSPEC;
		}
	}
	return (IBT_SUCCESS);
...
ibcm_val_ipattr(ibt_ip_path_attr_t *attrp, ibt_path_flags_t flags)
{
	...
	if (ibcm_val_ipaddr(1, &attrp->ipa_src_ip) != IBT_SUCCESS)
		return (IBT_NOT_SUPPORTED);
	...
	for (i = 0; i < attrp->ipa_ndst; i++) {
		...
		if (dst_ip.family == AF_UNSPEC) {
			IBTF_DPRINTF_L2(cmlog, "ibcm_val_ipattr: ERROR: "
			    "DstIP NOT specified: family AF_UNSPEC");
			return (IBT_INVALID_PARAM);
		}

		if (ibcm_val_ipaddr(0, &dst_ip) != IBT_SUCCESS)
			return (IBT_NOT_SUPPORTED);
		...
		if (attrp->ipa_src_ip.family != AF_UNSPEC) {
			 ...
			 */
			if (attrp->ipa_src_ip.family != dst_ip.family) {
				IBTF_DPRINTF_L2(cmlog, "ibcm_val_ipattr: "
				    "ERROR: Specified SrcIP (%d) and "
				    "DstIP(%d) family diffs.",
				    attrp->ipa_src_ip.family, dst_ip.family);
				return (IBT_INVALID_PARAM);
			}

			/*
			 * If DstIP specified is LOOPBACK, and SrcIP is
			 * specified then make sure we get info for that
			 * end point.
			 */
			if (((dst_ip.family == AF_INET) &&
			    (dst_ip.un.ip4addr == INADDR_LOOPBACK)) ||
			    ((dst_ip.family == AF_INET6) &&
			    IN6_IS_ADDR_LOOPBACK(&dst_ip.un.ip6addr))) {
				bcopy(&attrp->ipa_src_ip, &attrp->ipa_dst_ip[i],
				    sizeof (ibt_ip_addr_t));
			}
		}
	}
    ...
    ibt_path_ip_src_t *src_ip_p, ibt_ip_path_handler_t func, void  *arg)
{
	...
	_NOTE(NOW_INVISIBLE_TO_OTHER_THREADS(*path_tq))
	bcopy(attrp, &path_tq->ip_attr, sizeof (ibt_ip_path_attr_t));

	path_tq->ip_attr.ipa_dst_ip = (ibt_ip_addr_t *)(((uchar_t *)path_tq) +
	    sizeof (ibcm_ip_path_tqargs_t));
	bcopy(attrp->ipa_dst_ip, path_tq->ip_attr.ipa_dst_ip,
...
{
	ibt_alt_path_attr_t	attr;

	bzero(&attr, sizeof (ibt_alt_path_attr_t));

	/* If optional attributes are specified, validate them. */
	if (attrp) {
		boolean_t	src_ip_ok, dst_ip_ok;

		if ((attrp->apa_src_ip.family != AF_UNSPEC) &&
		    ibcm_val_ipaddr(1, &attrp->apa_src_ip) == IBT_SUCCESS)
			src_ip_ok = B_TRUE;
		else
			src_ip_ok = B_FALSE;

		if ((attrp->apa_dst_ip.family != AF_UNSPEC) &&
		    ibcm_val_ipaddr(0, &attrp->apa_dst_ip) == IBT_SUCCESS)
			dst_ip_ok = B_TRUE;
		else
			dst_ip_ok = B_FALSE;
		...
		/*
		 * Only if desired alternate ip-addr are specified, then
		 * perform ip2mac lookup to get appropriate GID info.
		 */
		if (dst_ip_ok || src_ip_ok) {
			ibcm_ibaddr_t	ibaddr;
			zoneid_t	myzoneid;
			ibt_status_t	retval;

			bzero(&ibaddr, sizeof (ibcm_ibaddr_t));

			myzoneid = (attrp->apa_zoneid == ALL_ZONES) ?
			    ALL_ZONES : GLOBAL_ZONEID;

			/* Get SGID and DGID for the specified input ip-addr */
			retval = ibcm_arp_get_ibaddr(myzoneid,
			    attrp->apa_src_ip,
			    dst_ip_ok ? attrp->apa_dst_ip : attrp->apa_src_ip,
			    &ibaddr);
			if (retval) {
				IBTF_DPRINTF_L2(cmlog, "ibt_get_ip_alt_path: "
				    "ibcm_arp_get_ibaddr() failed: %d \n"
				    "desired inputs ignored", retval);
			} else {
				if (src_ip_ok)
					attr.apa_sgid = ibaddr.sgid;
				if (dst_ip_ok)
					attr.apa_dgid = ibaddr.dgid;
			}
		}
		bcopy(&attrp->apa_srate, &attr.apa_srate,
		    sizeof (ibt_srate_req_t));
		bcopy(&attrp->apa_pkt_lt, &attr.apa_pkt_lt,
		    sizeof (ibt_pkt_lt_req_t));
		attr.apa_flow = attrp->apa_flow;
		attr.apa_sl = attrp->apa_sl;
		attr.apa_hop = attrp->apa_hop;
		attr.apa_tclass = attrp->apa_tclass;
	}

	return (ibt_get_alt_path(rc_chan, flags,
	    ((attrp == NULL) ? NULL : &attr), api_p));
}

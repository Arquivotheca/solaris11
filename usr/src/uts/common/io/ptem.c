...
ptemwmsg(queue_t *q, mblk_t *mp)
{
	...
	case M_IOCTL:
		 ...
		 * message is made and passed downstream. Eventually the PCKT
		 * module, if it has been pushed, should pick up this message.
		 * If the PCKT module has not been pushed the master side
		 * stream head will free it.
		 * ptemrput can't always mop up the response (if we close while
		 * it's in-flight) so we generate a fresh ioc_id. That way our
		 * stream head will discard any IOCACK/IOCNAKs we might miss.
		 */
		iocp = (struct iocblk *)mp->b_rptr;
		switch (iocp->ioc_cmd) {
		...
		case TCSETSW:
			...
			if ((cflags & CBAUD) == B0) {
				...
				/*
				 * Make a copy of this message with a fresh
				 * ioc_id and pass it on to the PCKT module.
				 */
				if ((pckt_msgp = copymsg(mp)) == NULL) {
					miocnak(q, mp, 0, EAGAIN);
					break;
				}
				iocp = (struct iocblk *)pckt_msgp->b_rptr;
				iocp->ioc_id = getiocseqno();
				putnext(q, pckt_msgp);
			}
			/*
			 * Send ACK upstream.
...
out:
		...
		case TIOCSTI:
			...
			/*
			 * Need a copy of this message with a fresh ioc_id
			 * to pass on to the PCKT module.
			 */
			if ((pckt_msgp = copymsg(mp)) == NULL) {
				miocnak(q, mp, 0, EAGAIN);
				break;
			...
			if ((ntp->state & REMOTEMODE) == 0) {
				...
				/*
				 * Inherit the writer's credentials.
				 */
				bp->b_flag |= MSGCRED;
				mblk_copycred(bp, mp);
				/*
...
ptioc(queue_t *q, mblk_t *mp, int qside)
{
	...
	int error;
	int sig;
	int onoff;		/* remote mode enabled / disabled */
	mblk_t *mctlp;

...
/*
 * Max bytes returned per read from /dev/random and /dev/urandom. Must be
 * a multiple of HASHSIZE
 */
#define	MAXRETBYTES_RANDOM	1040
#define	MAXRETBYTES_URANDOM	(128 * 1040)
...
rnd_read(dev_t dev, struct uio *uiop, cred_t *credp)
{
	...
	while (error == 0 && uiop->uio_resid > 0) {
		...
		if (error == 0) {
			...
			if (devno == DEVRANDOM) {
				if (nbytes >= MAXRETBYTES_RANDOM)
					break;
			} else {
				if (nbytes >= MAXRETBYTES_URANDOM)
					break;
			}

...
matchmac(dev_info_t *dip, void *arg)
{
	...
	/*
	 * Special handling for Mellanox Ethernet device:
	 *	mcxnex is bound to the device. It's not a NIC driver but
	 *	the nexus driver for mcxe NIC driver. We need to configure
	 *	its mcxe child driver node(s) here.
	 */
	if (strcmp(ddi_driver_name(dip), "mcxnex") == 0) {
		/* configure mcxe node(s) */

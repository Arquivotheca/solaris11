...
#if defined(sun4v)
extern void fm_clr_attach(dev_info_t *);
extern void fm_clr_detach();
extern int fm_ioctl_cache_retire(int, nvlist_t *, nvlist_t **);
#endif /* sun4v */
...
#endif	/* __x86 */
#if defined(sun4v)
	{ FM_IOC_CACHE_RETIRE, B_TRUE, FM_CACHE_OP_VERSION,
	    fm_ioctl_cache_retire },
	{ FM_IOC_CACHE_STATUS, B_FALSE, FM_CACHE_OP_VERSION,
	    fm_ioctl_cache_retire },
	{ FM_IOC_CACHE_UNRETIRE, B_TRUE, FM_CACHE_OP_VERSION,
	    fm_ioctl_cache_retire },
#endif /* sun4v */
	{ -1, B_FALSE, NULL, NULL },
};
...
void
fm_debug(unsigned int cat, const char *fmt, ...)
{
	va_list ap;

	if (fm_debug_flags & cat) {
		va_start(ap, fmt);

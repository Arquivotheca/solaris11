...
proto_promiscon_req(dld_str_t *dsp, mblk_t *mp)
{
	...
	switch (dlp->dl_level) {
	case DL_PROMISC_SAP:
		promisc_add = DLS_PROMISC_SAP;
		break;

	case DL_PROMISC_MULTI:
		promisc_add = DLS_PROMISC_MULTI;
		break;

	case DL_PROMISC_PHYS:
		promisc_add = DLS_PROMISC_PHYS;
		break;
	...
	if ((dsp->ds_promisc == 0) && (err = dls_active_set(dsp)) != 0) {
		dl_err = DL_SYSERR;
		goto failed2;
	}
	 ...
	 */
	err = dls_promisc(dsp, promisc_add, B_TRUE);
...
proto_promiscoff_req(dld_str_t *dsp, mblk_t *mp)
{
	...
	case DL_PROMISC_SAP:
		...
		promisc_remove = DLS_PROMISC_SAP;
		break;
	...
	case DL_PROMISC_MULTI:
		...
		promisc_remove = DLS_PROMISC_MULTI;
		break;
	...
	case DL_PROMISC_PHYS:
		...
		promisc_remove = DLS_PROMISC_PHYS;
		break;
	 ...
	 * Adjust channel promiscuity.
	 */
	err = dls_promisc(dsp, promisc_remove, B_FALSE);
...
dld_capab_direct(dld_str_t *dsp, void *data, uint_t flags)
{
	...
	case DLD_ENABLE:
		...
		direct->di_tx_fctl_df = (uintptr_t)mac_tx_is_blocked;
		direct->di_tx_fctl_dh = dsp->ds_mch;
		direct->di_tx_pmh =
		    (uintptr_t)mac_client_pmh_tx_get(dsp->ds_mch);
...
dld_capab_poll_enable(dld_str_t *dsp, dld_capab_poll_t *poll)
{
	...
	mac_rx_bypass_set(dsp->ds_mch, (mac_direct_rx_t)dsp->ds_rx,
	    dsp->ds_rx_arg);
...
dld_capab_poll_disable(dld_str_t *dsp, dld_capab_poll_t *poll)
{
	...
	mac_rx_bypass_set(dsp->ds_mch, NULL, NULL);

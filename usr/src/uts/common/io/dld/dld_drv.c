...
drv_ioc_phys_attr(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	dev_t			phydev;
	zoneid_t		zoneid = crgetzoneid(cred);
	size_t			lsz = sizeof (dipp->dip_loc);
	...
	/*
	 * Get the physical location from the FPHYLOC phys attr.
	 */
	dipp->dip_loc[0] = '\0';
	(void) dls_mgmt_get_linkattr(dipp->dip_linkid, FPHYLOC,
	    &dipp->dip_loc, &lsz);
...
/* ARGSUSED */
static int
drv_ioc_ringidget(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	dld_ioc_ringidget_t	*ringidp = karg;
	datalink_id_t		linkid = ringidp->dir_linkid;
	...
	mac_get_ringids(dlp->dl_mch, ringidp->dir_rxringids,
	    &ringidp->dir_nrxrings, &ringidp->dir_rxdedicated,
	    ringidp->dir_txringids, &ringidp->dir_ntxrings,
	    &ringidp->dir_txdedicated);
...
drv_ioc_hwgrpget(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	int			totgrps = 0;
	zoneid_t		zoneid = crgetzoneid(cred);
	uint_t			rxgids[MAX_GROUPS_PER_NIC];
	uint_t			txgids[MAX_GROUPS_PER_NIC];

	if (zoneid != GLOBAL_ZONEID &&
	    zone_check_datalink(&zoneid, hwgrpp->dih_linkid) != 0)
		return (ENOENT);

	hwgrpp->dih_ngroups = 0;
	hwgrpp->dih_nrxrings = 0;
	hwgrpp->dih_ntxrings = 0;
	...
	mac_get_hwrxgrp_ids(mh, rgrpnum, rxgids);
	...
	if (rgrpnum > 0) {
		...
		mac_get_hwrxgrp_info(mh, rxgids[0], &hwgrp.dhi_grp_num,
		    &hwgrp.dhi_nrings, hwgrp.dhi_rings, &hwgrp.dhi_grp_type,
		    &hwgrp.dhi_nclnts, hwgrp.dhi_clnts);
		hwgrpp->dih_nrxrings += hwgrp.dhi_nrings;
		if (hwgrp.dhi_nrings != 0) {
	...
	mac_get_hwtxgrp_ids(mh, tgrpnum, txgids);
	...
	if (tgrpnum > 0) {
		...
		bcopy(mac_name(mh), hwgrp.dhi_link_name,
		    sizeof (hwgrp.dhi_link_name));
		mac_get_hwtxgrp_info(mh, txgids[0], &hwgrp.dhi_grp_num,
		    &hwgrp.dhi_nrings, hwgrp.dhi_rings, &hwgrp.dhi_grp_type,
		    &hwgrp.dhi_nclnts, hwgrp.dhi_clnts);
		hwgrpp->dih_ntxrings += hwgrp.dhi_nrings;
		if (hwgrp.dhi_nrings != 0) {
			if (copyout(&hwgrp, hip, sizeof (hwgrp)) != 0) {
				err = EFAULT;
				goto done;
	...
	for (i = 1; i < rgrpnum; i++) {
		...
		bcopy(mac_name(mh), hwgrp.dhi_link_name,
		    sizeof (hwgrp.dhi_link_name));
		mac_get_hwrxgrp_info(mh, rxgids[i], &hwgrp.dhi_grp_num,
		    &hwgrp.dhi_nrings, hwgrp.dhi_rings, &hwgrp.dhi_grp_type,
		    &hwgrp.dhi_nclnts, hwgrp.dhi_clnts);
		hwgrpp->dih_nrxrings += hwgrp.dhi_nrings;
		if (hwgrp.dhi_nrings == 0)
	...
	for (i = 1; i < tgrpnum; i++) {
		...
		bcopy(mac_name(mh), hwgrp.dhi_link_name,
		    sizeof (hwgrp.dhi_link_name));
		mac_get_hwtxgrp_info(mh, txgids[i], &hwgrp.dhi_grp_num,
		    &hwgrp.dhi_nrings, hwgrp.dhi_rings, &hwgrp.dhi_grp_type,
		    &hwgrp.dhi_nclnts, hwgrp.dhi_clnts);
		hwgrpp->dih_ntxrings += hwgrp.dhi_nrings;
		if (hwgrp.dhi_nrings == 0)
    ...
    cred_t *cred, int mode)
{
	...
	/*
	 * Don't allow a process to set properties of a link if that
	 * link doesn't belong to that zone.
	 */
	if (set && zoneid != dls_devnet_getownerzid(dlh)) {
		err = ENOENT;
		goto done;
	}
	...
	case MAC_PROP_MAC_ADDR: {
		mac_handle_t mh;
		dld_mac_addr_t *mac_addr = (dld_mac_addr_t *)kprop->pr_val;

		err = mac_open(dls_devnet_mac(dlh), &mh);
		if (err != 0)
			break;

		if (set) {
			uchar_t defaddr[MAXMACADDRLEN];
			uchar_t *addr;

			if (zoneid != GLOBAL_ZONEID &&
			    mac_protect_enabled(dlp->dl_mch, MPT_MACNOSPOOF)) {
				dld_mac_close(mh);
				err = EACCES;
				break;
			}
			...
			/*
			 * No new value represents a request to reset
			 * the primary address to its default.
			 */
			if (kprop->pr_valsize == 0) {
				mac_unicast_default_get(mh, defaddr);
				addr = defaddr;
			} else {
				addr = mac_addr->dm_addr;
			}
			err = mac_unicast_primary_set(mh, addr);
			dld_mac_close(mh);
			if (err != 0)
				break;
		} else {
			mac_addr->dm_len = mac_addr_len(mh);
			if (kprop->pr_flags & DLD_PROP_DEFAULT)
				mac_unicast_default_get(mh, mac_addr->dm_addr);
			else
				mac_unicast_primary_get(mh, mac_addr->dm_addr);
			dld_mac_close(mh);
			err = 0;
		}
		break;
...
drv_ioc_addflow(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	afp->af_flow_desc.fd_zoneid = crgetzoneid(cred);
	return (dld_add_flow(afp->af_linkid, afp->af_name,
	    &afp->af_flow_desc, &afp->af_resource_props));
}

/*
 * Process a DLDIOC_CREATEKSTAT request.
 */
/* ARGSUSED */
static int
drv_ioc_createkstat(void *karg, intptr_t arg, int mode, cred_t *cred,
    int *rvalp)
{
	dld_ioc_createkstat_t	*ckp = karg;

	return (dld_create_kstat(ckp->ck_flow, ckp->ck_zoneid));
}
...
drv_ioc_removeflow(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	return (dld_remove_flow(rfp->rf_name, rfp->rf_zoneid));
}
...
static dld_ioc_info_t drv_ioc_list[] = {
	...
	{DLDIOC_ADDFLOW, DLDCOPYIN, sizeof (dld_ioc_addflow_t),
	    drv_ioc_addflow, secpolicy_flow_config},
	{DLDIOC_CREATEKSTAT, DLDCOPYIN, sizeof (dld_ioc_createkstat_t),
	    drv_ioc_createkstat, secpolicy_flow_config},
	{DLDIOC_REMOVEFLOW, DLDCOPYIN, sizeof (dld_ioc_removeflow_t),
	    drv_ioc_removeflow, secpolicy_flow_config},
	{DLDIOC_MODIFYFLOW, DLDCOPYIN, sizeof (dld_ioc_modifyflow_t),
	    drv_ioc_modifyflow, secpolicy_flow_config},
	...
	{DLDIOC_USAGELOG, DLDCOPYIN, sizeof (dld_ioc_usagelog_t),
	    drv_ioc_usagelog, secpolicy_flow_config},
	...
	{DLDIOC_GETRINGIDS, DLDCOPYINOUT, sizeof (dld_ioc_ringidget_t),
	    drv_ioc_ringidget, NULL},
};

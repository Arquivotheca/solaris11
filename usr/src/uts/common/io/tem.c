...
tem_info_init(char *pathname, cred_t *credp)
{
	...
	bzero(&temargs, sizeof (struct vis_devinit));
    ...
    struct vis_devinit *devinit)
{
	...
	if (diff == 0) {
		...
		tems.ts_color_map = devinit->color_map;
...
/*
 * This function is used to allow the driver to optimize the operation of
 * clearing the screen
 */
int
tems_clearscr_layered(
	...
	cred_t *credp)
{
	...
	(void) ldi_ioctl(tems.ts_hdl, VIS_CONSCLEAR,
	    (intptr_t)cls, FKIOCTL, credp, &rval);
...
tem_set_fbmode(tem_vt_state_t tem_arg, uchar_t fbmode, cred_t *credp)
{
	...
	/*
	 * From the tem point of view, a "reset" of the text (for example done
	 * by sulogin) is just a setting of KD_TEXT. On the other hand, the
	 * underlying graphic driver needs to know that it might have to stop
	 * any boot-time graphics related thread.
	 */
	if (fbmode == KD_RESETTEXT)
		fbmode = KD_TEXT;

	tem->tvs_fbmode = fbmode;

	if (tem->tvs_isactive) {
		tem_kdsetmode(fbmode_saved, credp);
		if (fbmode == KD_TEXT)

...
static int aggr_pseudo_disable_intr(mac_ring_driver_t);
static int aggr_pseudo_enable_intr(mac_ring_driver_t);
static int aggr_pseudo_start_ring(mac_ring_driver_t, uint64_t);
static void aggr_pseudo_stop_ring(mac_ring_driver_t);
static int aggr_addmac(void *, const uint8_t *, uint64_t);
static int aggr_remmac(void *, const uint8_t *);
static mblk_t *aggr_rx_poll(void *, int, int);
...
aggr_grp_attach_port(aggr_grp_t *grp, aggr_port_t *port)
{
	 ...
	 *
	 * An aggr port is put in promiscuous mode if it runs out of mac
	 * address slots. For such ports, all the packets are passed up
	 * through promiscuous code path and the port's callback function
	 * is set to drop all the packets. The callback function is left
	 * unchanged for such ports.
	 */
	if (!port->lp_promisc_on)
...
aggr_add_pseudo_tx_group(aggr_port_t *port, aggr_pseudo_tx_group_t *tx_grp)
{
	...
	if (hw_rh_cnt == 0) {
		...
		VERIFY(mac_deftxring_get(port->lp_mch, hw_rh) == 0);
	} else {
	...
	for (i = 0; i < port->lp_tx_ring_cnt; i++) {
		err = aggr_add_pseudo_tx_ring(port, tx_grp, hw_rh[i],
		    &pseudo_rh);
		if (err != 0)
			break;
...
static int
aggr_pseudo_disable_intr(mac_ring_driver_t rh)
{
	aggr_pseudo_rx_ring_t *rr_ring = (aggr_pseudo_rx_ring_t *)rh;
	return (mac_hwring_disable_intr(rr_ring->arr_hw_rh));
}

static int
aggr_pseudo_enable_intr(mac_ring_driver_t rh)
{
	aggr_pseudo_rx_ring_t *rr_ring = (aggr_pseudo_rx_ring_t *)rh;
    ...
    laioc_port_t *ports)
{
	...
	for (i = 0; i < nports; i++) {
		...
		/*
		 * Check whether the sdu and margin of the device been added
		 * to the aggregation are compatible
		 */
		if (!aggr_grp_sdu_check(grp, port) ||
	...
	/* Update the group's capabilities. */
	aggr_grp_capab_set(grp);
	mac_capab_update(grp->lg_mh);
...
aggr_grp_modify_common(aggr_grp_t *grp, uint8_t update_mask, uint32_t policy,
    boolean_t mac_fixed, boolean_t default_mac, const uchar_t *mac_addr,
    aggr_lacp_mode_t lacp_mode, aggr_lacp_timer_t lacp_timer)
{
	...
	if (default_mac)
		(void) mac_unicast_default_set(grp->lg_mh, grp->lg_addr);
    ...
    aggr_lacp_timer_t lacp_timer)
{
	...
	err = aggr_grp_modify_common(grp, update_mask, policy, mac_fixed,
	    B_TRUE, mac_addr, lacp_mode, lacp_timer);
...
aggr_m_capab_get(void *arg, mac_capab_t cap, void *cap_data)
{
	...
	case MAC_CAPAB_AGGR: {
		...
		if (cap_data != NULL) {
			...
			aggr_cap->mca_select_ring = aggr_select_tx_ring;
	...
	case MAC_CAPAB_LINKID: {
		...
		cap_lid->mpl_linkid = grp->lg_linkid;
		break;
	}
	default:
		return (B_FALSE);
    ...
    mac_group_info_t *infop, mac_group_handle_t gh)
{
	...
	/*
	 * Aggr does not have more than one RX or TX ring group.
	 */
	ASSERT(index == 0);
	...
	if (rtype == MAC_RING_TYPE_RX) {
		...
		tx_group->atg_gh = gh;
		tx_group->atg_grp = grp;

		infop->mgi_driver = (mac_group_driver_t)tx_group;
		infop->mgi_count = tx_group->atg_ring_cnt;
	}
}
    ...
    const int index, mac_ring_info_t *infop, mac_ring_handle_t rh)
{
	...
	case MAC_RING_TYPE_RX: {
		 ...
		 */
		bzero(&aggr_mac_intr, sizeof (aggr_mac_intr));
		aggr_mac_intr.mi_enable = aggr_pseudo_enable_intr;
		aggr_mac_intr.mi_disable = aggr_pseudo_disable_intr;

		hwinfop = mac_hwring_getinfo(rx_ring->arr_hw_rh);
		aggr_mac_intr.mi_ddi_handle = hwinfop->mri_intr.mi_ddi_handle;
		mac_hwring_setup(rx_ring->arr_hw_rh,
		    (mac_resource_handle_t)rx_ring, rh);
	...
	case MAC_RING_TYPE_TX: {
		 ...
		 */
		hwinfop = mac_hwring_getinfo(tx_ring->atr_hw_rh);
		infop->mri_flags = hwinfop->mri_flags;
		infop->mri_intr.mi_ddi_handle = hwinfop->mri_intr.mi_ddi_handle;
		mac_hwring_setup(tx_ring->atr_hw_rh,
		    (mac_resource_handle_t)tx_ring, rh);
		break;
	}
	default:
		break;
...
static mblk_t *
aggr_rx_poll(void *arg, int bytes_to_pickup, int pkts_to_pickup)
{
...
static int
aggr_addmac(void *arg, const uint8_t *mac_addr, uint64_t flags)
{
...
aggr_m_unicst(void *arg, const uint8_t *macaddr)
{
	...
	mac_perim_enter_by_mh(grp->lg_mh, &mph);
	err = aggr_grp_modify_common(grp, AGGR_MODIFY_MAC, 0, B_TRUE, B_FALSE,
	    macaddr, 0, 0);
	mac_perim_exit(mph);
	return (err);
}

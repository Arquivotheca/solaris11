    ...
    mbx_data_t *p_results)
{
	...
	max_wait = 50; /* wait upto 50 PI interrupt */
	...
	while ((done != B_TRUE) && (max_wait--)) {
		...
		if (ql_poll_processor_intr(qlge, (uint8_t)mbx_cmd->timeout)
			...
			QL_PRINT(DBG_MBX, ("%s(%d) PI Intr received",
			    __func__, qlge->instance));
			if (ql_read_mailbox_cmd(qlge, p_results, 0)) {
				/*EMPTY*/
				QL_PRINT(DBG_MBX,
				...
				/*
				 * Sometimes, the incoming messages is not what
				 * we are waiting for, ie. async events, then,
				 * continue to wait. If it is the result * of
				 * previous mailbox command, then Done.
				 * No matter what, send
				 * HOST_CMD_CLEAR_RISC_TO_HOST_INTR to clear
				 * each PI interrupt
				 */
				if (ql_async_event_parser(qlge, p_results) ==
				    B_FALSE) {
					/*
...
ql_async_event_parser(qlge_t *qlge, mbx_data_t *mbx_cmds)
{
	...
	case MBA_LINK_UP /* 8011h */:
		 ...
		 */
		ql_get_and_report_link_state(qlge, LS_UP);
	...
	case MBA_LINK_DOWN /* 8012h */:
		...
		ql_get_and_report_link_state(qlge, LS_DOWN);
		break;
...
ql_do_mpi_intr(qlge_t *qlge)
{
	...
	if (ql_read_mailbox_cmd(qlge, &qlge->received_mbx_cmds,
	    qlge->max_read_mbx)) {
		goto exit;
	}
	/*
    ...
    struct qlnic_mpi_version_info *mpi_version_ptr)
{
	...
	if (mbx_cmds.mb[0] != MBS_COMMAND_COMPLETE /* 0x4000 */) {
		...
		qlge->phy_version_info.sub_minor_version =
		    LSB(LSW(mbx_cmds.mb[2]));
		cmn_err(CE_NOTE, "!qlge(%d) firmware version: %d.%d.%d\n",
		    qlge->instance,
		    qlge->fw_version_info.major_version,
		    qlge->fw_version_info.minor_version,
		    qlge->fw_version_info.sub_minor_version);

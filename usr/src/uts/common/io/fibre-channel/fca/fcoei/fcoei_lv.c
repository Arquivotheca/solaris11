...
static void fcoei_xch_abort(fcoei_exchange_t *xch);
...
fcoei_port_manage(void * fca_handle, fc_fca_pm_t *pm)
{
	...
	case FC_PORT_REPORT_FLOGI:
	{
		ss->ss_eport->eport_flags |= EPORT_FLAG_FLOGI_DONE;
		rval = FC_SUCCESS;
		break;
	}
	default:
...
fcoei_initiate_fcp_cmd(fcoei_exchange_t *xch)
{
	...
	/*
	 * FCoE offload setup LRO
	 */
	max_lro_size = xch->xch_ss->ss_eport->offload_capab.fcoe_max_lro_size;

	if (SS_LRO_ENABLED(xch->xch_ss) &&
	    (fpkt->pkt_tran_type == FC_PKT_FCP_READ) &&
	    (IS_SCSI_READ(fcp_cmd_iu->fcp_cdb[0])) &&
	    (fpkt->pkt_datalen > xch->xch_ss->ss_fcp_data_payload_size) &&
	    (fpkt->pkt_datalen <= max_lro_size)) {
		if (xch->xch_ss->ss_eport->eport_fcoe_setup_lro
		    (xch->xch_ss->ss_eport,
		    MAC_FCOE_XCHG_ON_INITIATOR, xch->xch_oxid,
		    fpkt->pkt_datalen) == -1) {
			FCOEI_LOG(__FUNCTION__,
			    "setup_lro fail: xch/%p, %x/%x",
 ...
 */
static void
fcoei_xch_abort(fcoei_exchange_t *xch)
{
...
fcoei_process_event_reset(fcoei_event_t *ae)
{
	...
	for (i = ss->ss_min_sol_oxid; i <= ss->ss_max_sol_oxid; i++) {
		if (ss->ss_sol_oxid_table[i] != NULL) {
			fcoei_xch_abort(ss->ss_sol_oxid_table[i]);
		}
	}
	for (i = 0; i < FCOEI_UNSOL_TABLE_SIZE; i++) {
		if (ss->ss_unsol_rxid_table[i] != NULL) {
			fcoei_xch_abort(ss->ss_unsol_rxid_table[i]);
		}
	}

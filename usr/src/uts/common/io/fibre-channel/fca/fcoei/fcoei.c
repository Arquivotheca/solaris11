 ...
 */
static void fcoei_xch_check(fcoei_soft_state_t *ss, boolean_t checkSolTable);
static void fcoei_xch_check_one(
	uint16_t xid, fcoei_exchange_t *xch, fcoei_soft_state_t *ss);
...
fcoei_attach_init(fcoei_soft_state_t *ss)
{
	...
	if (SS_LRO_ENABLED(ss)) {
		...
		ss->ss_min_sol_oxid = 0;
		ss->ss_max_sol_oxid = FCOEI_SOL_TABLE_SIZE - 1;
	}
	ss->ss_sol_oxid_table = kmem_zalloc((ss->ss_max_sol_oxid + 1)
	    * sizeof (fcoei_exchange_t *), KM_SLEEP);
	ss->ss_unsol_rxid_table = kmem_zalloc(FCOEI_UNSOL_TABLE_SIZE
	    * sizeof (fcoei_exchange_t *), KM_SLEEP);
...
fcoei_detach_uninit(fcoei_soft_state_t *ss)
{
	...
	for (i = ss->ss_min_sol_oxid;
	    i <= ss->ss_max_sol_oxid; i++) {
		if (ss->ss_sol_oxid_table[i] != NULL) {
			return (FCOE_FAILURE);
		}
	}
	for (i = 0; i < FCOEI_UNSOL_TABLE_SIZE; i++) {
		if (ss->ss_unsol_rxid_table[i] != NULL) {
			return (FCOE_FAILURE);
		}
	}
	kmem_free(ss->ss_sol_oxid_table,
	    sizeof (fcoei_exchange_t *) * (ss->ss_max_sol_oxid + 1));
	kmem_free(ss->ss_unsol_rxid_table,
	    sizeof (fcoei_exchange_t *) * FCOEI_UNSOL_TABLE_SIZE);
...
fcoei_watchdog(void *arg)
{
	...
	FCOEI_LOG(__FUNCTION__, "sol_table %p", ss->ss_sol_oxid_table);
	FCOEI_LOG(__FUNCTION__, "unsol_table %p", ss->ss_unsol_rxid_table);
 ...
 *
 * Input:
 *	xid = oxid of the solicited exchange or
 *	      rxid of the unsolicited exchange
 *	xch = the exchange
 *	ss  = the soft state
 *
 * Return:
 ...
 */
static void
fcoei_xch_check_one(uint16_t xid, fcoei_exchange_t *xch, fcoei_soft_state_t *ss)
{
	ASSERT(xch->xch_ss == ss);
	if ((xch->xch_end_tick < CURRENT_CLOCK) &&
	    (xch->xch_ss->ss_link_state != FC_STATE_OFFLINE)) {
		if (xch->xch_flags & XCH_FLAG_IN_SOL_TABLE) {
			ASSERT(xch->xch_oxid == xid);
			/*
...
static void
fcoei_xch_check(fcoei_soft_state_t *ss, boolean_t checkSolTable)
{
	uint16_t	xid;
	if (checkSolTable == B_TRUE) {
		for (xid = ss->ss_min_sol_oxid;
		    xid <= ss->ss_max_sol_oxid; xid++) {
			if (ss->ss_sol_oxid_table[xid] == NULL) {
				continue;
			}
			fcoei_xch_check_one(xid,
			    ss->ss_sol_oxid_table[xid], ss);
		}
	} else {
		for (xid = 0; xid < FCOEI_UNSOL_TABLE_SIZE; xid++) {
			if (ss->ss_unsol_rxid_table[xid] == NULL) {
				continue;
			}
			fcoei_xch_check_one(xid,
			    ss->ss_unsol_rxid_table[xid], ss);
		}
	}
...
fcoei_abts_exchange(fcoei_exchange_t *xch)
{
	...
	/*
	 * FCoE offload Cancel LRO
	 */
	if (SS_LRO_ENABLED(xch->xch_ss) &&
	    (xch->xch_flags & XCH_FLAG_LRO)) {
		xch->xch_ss->ss_eport->eport_fcoe_cancel_lro(
		    xch->xch_ss->ss_eport,
		    MAC_FCOE_XCHG_ON_INITIATOR, xch->xch_oxid);
	}
    ...
    uint8_t pkt_state, uint8_t pkt_reason)
{
	...
	/*
	 * If xch is in exchange table, we need remove it
	 */
	if (xch->xch_flags & XCH_FLAG_IN_SOL_TABLE) {
		val = atomic_swap_ptr(
		    &xch->xch_ss->ss_sol_oxid_table[xch->xch_oxid], NULL);
		ASSERT(val != NULL);
		xch->xch_flags &= ~XCH_FLAG_IN_SOL_TABLE;
	} else if (xch->xch_flags & XCH_FLAG_IN_UNSOL_TABLE) {
		val = atomic_swap_ptr(
		    &xch->xch_ss->ss_unsol_rxid_table[xch->xch_rxid], NULL);
		ASSERT(val != NULL);
		xch->xch_flags &= ~XCH_FLAG_IN_UNSOL_TABLE;
	} else {
		FCOEI_LOG(__FUNCTION__, "xch not in any table: %p", xch);
	}

...
#pragma ident	"Copyright 2011 QLogic Corporation; ql_xioctl.c"
...
ql_diagnostic_loopback(ql_adapter_state_t *ha, EXT_IOCTL *cmd, int mode)
{
	...
	if (CFG_IST(ha, CFG_CTRL_8081)) {
		if (loop_up && opt == MBC_LOOPBACK_POINT_EXTERNAL) {
			...
			rval = ql_diag_echo(ha, bp, plbreq.TransferCount,
			    MBC_ECHO_ELS, &mr);
		} else {
	...
	ql_restart_driver(ha);
	if (CFG_IST(ha, CFG_CTRL_8021) && loop_up &&
	    opt == MBC_LOOPBACK_POINT_INTERNAL) {
		timer = 30;
		do {
			delay(100);

...
fp_attach_handler(dev_info_t *dip)
{
	...
	/*
	 * Unwind any/all preceding allocations in the event of an error.
	 */
...
fp_port_startup(fc_local_port_t *port, job_request_t *job)
{
	...
	if (state == FC_STATE_LOOP) {
		...
		if (rval == FC_SUCCESS) {
			...
			if (strcmp(port->fp_hba_port_attrs.model,
			    "FCoE Virtual FC HBA") == 0) {
				pm.pm_cmd_flags = FC_FCA_PM_WRITE;
				pm.pm_cmd_code = FC_PORT_REPORT_FLOGI;
				pm.pm_data_len = 0;
				pm.pm_data_buf = NULL;
				(void) port->fp_fca_tran->fca_port_manage(
				    port->fp_fca_handle, &pm);
			}
...
fp_plogi_intr(fc_packet_t *pkt)
{
	...
	if ((pd = pkt->pkt_pd) == NULL) {
		pd = fctl_get_remote_port_by_pwwn(port, &pwwn);
		if (pd == NULL) {
			FP_TRACE(FP_NHEAD1(1, 0), "fp_plogi_intr: fp %x pd %x",
    ...
    job_request_t *job)
{
	...
	case R_CTL_EXTENDED_SVC:
		...
		case LA_ELS_LOGO:
			pd = fctl_get_remote_port_by_did(port, s_id);
			/*
			 * The domain controller range is also considered
			 * not a REAL device therefore no remote port is
			 * registered. Therefore, special handling for that
			 * range to directly send LS_ACC for LOGO command.
			 */
			if (pd == NULL && s_id >= NPORT_ID_DOM_CTLR_START &&
			    s_id <= NPORT_ID_DOM_CTLR_END) {
				if (FP_IS_CLASS_1_OR_2(buf->ub_class)) {
					break;
				}
				if ((cmd = fp_alloc_pkt(port,
				    FP_PORT_IDENTIFIER_LEN, 0,
				    KM_SLEEP, NULL)) == NULL) {
					break;
				}
...
fp_ns_intr(fc_packet_t *pkt)
{
	...
	if (FP_IS_PKT_ERROR(pkt)) {
		...
		FP_TRACE(FP_NHEAD1(1, 0), "%x NS failure pkt state=%x "

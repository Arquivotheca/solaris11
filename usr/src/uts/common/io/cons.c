...
/*
 * Happy face boot may have to be interrupted by a console event waiting on
 * a read or a poll.
 */
#if defined(__x86)
extern boolean_t	check_reset_console;
extern void 		splash_reset_text_console();
#endif	/* __x86 */
...
#if defined(__x86)
	/*
	 * The first time we read from the console, check if we need
	 * to reclaim the console back from happyface boot.
	 */
	if (check_reset_console == B_TRUE) {
...
#if defined(__x86)
	/*
	 * The first time we poll from the console, check if we need
	 * to reclaim the console back from happyface boot.
	 */
	if (check_reset_console == B_TRUE) {
		check_reset_console = B_FALSE;

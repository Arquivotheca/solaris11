...
static int		update_valid_replica(side_t side, mddb_locator_t *lp,
				mddb_set_t *s, int li, char *devname,
				char *pathname, md_dev64_t devt);
extern crc_skip_t	*stripe_skip_ts(void *un, uint_t revision);
extern crc_skip_t	*mirror_skip_ts(uint_t revision);
extern crc_skip_t	*hotspare_skip_ts(uint_t revision);
	...
	if (lbp->lb_flags & MDDB_MNSET) {
		if (master_only == MDDB_WR_ONLY_MASTER) {
			...
			if (!(md_set[s->s_setno].s_am_i_master)) {
				/* Unless we're in the process of importing */
...
)
{
	...
	if ((lbp->lb_flags & MDDB_MNSET) &&
		...
		/*
		 * The MDDB_DEVID_CONV flag indicates that this set
		 * is a MN set which is being converted from a non
		 * devid format to using devids.  In this case, we
		 * do not want to trigger a parse message because it
		 * will happen during the reconfig cycle, when parse
		 * messages are blocked, causing a deadlock.
		 */
		if (!(lbp->lb_flags & MDDB_DEVID_CONV))
...
)
{
	...
	if (lbp->lb_flags & MDDB_MNSET) {
		...
		if ((dep->de_owner_nodeid == MD_MN_INVALID_NID) ||
			...
			/*
			 * Has ownership but is the slave node.
			 * In this case, slave node already failed to access
			 * the records and the master node had moved records
			 * to other locations. Set bits and takes over
			 */
			orp = (struct optim_resync *)dep->de_rb->rb_data;
			for (i = 0; i < howmany(orp->or_num, NBBY); i++)
				orp->or_rr[i] = 0xFF;
			rec_owner = 1;
		}
	} else {
	...
	int		flag)
{
	...
	while (r) {
		if (match_mddb(r, (ddi_devid_t)(uintptr_t)clp->l_devid,
			...
			if (clp->l_flags & MDDB_DEVID_TRANSPLANT) {
				r->ri_transplant = 1;
			}
...
static int
md_low_side_in_lb(mddb_mnlb_t *mnlbp, int li, side_t skip)
{
	int			i;
	int			index = -1;
	mddb_mnsidelocator_t	*mnslp;
	...
	VERIFY(mnlbp->lb_flags & MDDB_MNSET);

	for (i = 0; i < MD_MNMAXSIDES; i++) {
		mnslp = &mnlbp->lb_mnsidelocators[i][li];
		if (mnslp->mnl_sideno == 0 || mnslp->mnl_sideno == skip)
			continue;
		if (mnslp->mnl_sideno < side) {
			side = mnslp->mnl_sideno;
			index = i;
		}
	}
...
static void
md_alter_sides_in_lb(mddb_mnlb_t *mnlbp, int index, side_t side)
{
	int li;

	VERIFY(mnlbp->lb_flags & MDDB_MNSET);

	for (li = 0; li < MDDB_NLB; li++) {
		mddb_mnsidelocator_t	*mnslp;

		mnslp = &mnlbp->lb_mnsidelocators[index][li];
		if (mnslp->mnl_sideno != 0)
			mnslp->mnl_sideno = side;
	}
}
...
)
{
	...
	for (rip = s->s_rip; rip != NULL; rip = rip->ri_next) {
		...
		/*
		 * We may be importing a set from a cluster to a standalone
		 * node or vice versa.  If that is the case, then the minor
		 * name will need to be converted accordingly or the locator
		 * blocks will not be found, and the import will fail.
		 */
		if ((md_get_setstatus(setno) & MD_SET_IMPORT)) {
			for (li = 0; li < lbp->lb_loccnt; li++) {
				...
				for (trip = s->s_rip; trip != NULL;
					...
					rep = (md_get_setstatus(setno) &
					    MD_SET_REPLICATED_IMPORT);
					if (rep != 0) {

						/*
						 * Check for configured s_rip
						 * entries
						 */
					...
					if (ddi_lyr_get_minor_name(trip->ri_dev,
					    S_IFBLK, &tmin) != DDI_SUCCESS)
						continue;

					MIN_NM_TYPE(tmin, ttype);
					/*
					 * Handle minor-name transplanting here.
					 * This is a form of replication which
					 * can occur if the device enumeration
					 * differs from the original set device
					 * enumeration (e.g., via scdidadm -t
					 * remapping of drives).
					 */
					if ((mtype != ttype) || (rep != 0)) {
						if (strlen(tmin) >=
						    MDDB_MINOR_NAME_MAX) {
							if (tmin != NULL)
								kmem_free(tmin,
			...
			for (li = 0; li < lbp->lb_loccnt; li++) {
				...
				for (trip = s->s_rip; trip != NULL;
					...
					if (i == MD_MNMAXSIDES) {
						...
						mnlbp->lb_mnsidelocators[i][li].
						    mnl_mnum = md_getminor(
						    trip->ri_dev);
					}
				}
			}
		...
		if ((rip->ri_devid != NULL) &&
			...
			for (li = 0; li < lbp->lb_loccnt; li++) {
				...
				if (md_compare_minors(rip->ri_minor_name,
		...
		if ((md_get_setstatus(setno) & MD_SET_IMPORT) && mn_set) {
			mddb_mnlb_t		*mnlbp = (mddb_mnlb_t *)lbp;
			mddb_mnsidelocator_t	*mnslp;
			int			index;

			index = checklocator(lbp, li, s->s_sideno);
			mnslp = &mnlbp->lb_mnsidelocators[index][li];
			if (mnslp->mnl_sideno == 0) {
				if ((index = md_low_side_in_lb(mnlbp, li,
				    s->s_sideno)) == -1) {
					continue;
				}
				md_alter_sides_in_lb(mnlbp, index,
				    s->s_sideno);
			}
		}
...
)
{
	...
	if (md_get_setstatus(setno) & MD_SET_IMPORT &&
		...
		mddb_mnln_t 		*mnlnp = (mddb_mnln_t *)s->s_lnp;

		index = checklocator(lbp, 0, s->s_sideno);
		if (index == -1) {
			retval = MDDB_E_NOLOCNMS;
			goto errout;
		}
		...
		for (li = 0; li < MDDB_NLB; li++) {
			...
			if (mnsn->mn_ln_sideno == 0)
				continue;
			mnsn->mn_ln_sideno = s->s_sideno;
		}
	}

	/*
	 * Update the locator names if we're doing an import, because
	 * we may have to convert to or from did device nomenclature.
	 */
	if (md_get_setstatus(setno) & MD_SET_IMPORT) {
		for (li = 0; li < lbp->lb_loccnt; li++) {
			lp = &lbp->lb_locators[li];
			...
			for (rip = s->s_rip; rip != NULL; rip = rip->ri_next) {
				int err = 0;
				char *dname, *tname, *pname, *isdid;

				if (match_mddb(rip, NULL, NULL,
				    md_expldev(lp->l_dev), lp->l_blkno)) {
					dname = md_strdup(rip->ri_devname);

					/*
					 * Handle NULL ri_devname by looking
					 * for SC did driver in the ri_driver
					 * field. If found, the target disk
					 * name is the ri_minor_name field.
					 * For 'ssd' (non-SC disk) drivers
					 * we have to process the ri_devname
					 * field to get the minor device name
					 */

					isdid = strstr(rip->ri_driver, "did");
					if (isdid != NULL) {
						pname = md_strdup(
						    "/dev/did/dsk");
						tname = rip->ri_minor_name;
					} else {
						pname = md_strdup("/dev/dsk");
						tname = strrchr(dname, '/');
						if (tname == NULL) {
							freestr(pname);
							freestr(dname);
					...
					err = update_valid_replica(s->s_sideno,
					    lp, s, li, tname, pname,
					    md_expldev(lp->l_dev));
...
)
{
	 ...
	 *
	 * For some versions of the SunCluster environment, device id mode
	 * is disabled. We must query the did driver to see if it supports
	 * device ids.
	 *
	 * For localset, the behavior will remain the intact and run in
	 * device id mode.
	 */
	devid_flag = 1;
...
)
{
	...
	md_clr_setstatus(setno, MD_SET_ACCOK | MD_SET_ACCEPT |
	    ...
	    MD_SET_IMPORT | MD_SET_REPLICATED_IMPORT | MD_SET_MN_START_RC);
...
)
{
	...
	lnp = s->s_lnp;
	mnset = md_get_setstatus(s->s_setno) & MD_SET_MNSET;

	if (mnset == 0) {
		sn = &lnp->ln_suffixes[side][li];
		pre_index = sn->suf_prefix;
	} else {
		int	idx = checklocator(s->s_lbp, li, side);
		mnlnp = (mddb_mnln_t *)lnp;

		if (idx < 0 || idx > MD_MNMAXSIDES)
			return (1);

		mnsn = &mnlnp->ln_mnsuffixes[idx][li];

		/*
		 * checklocator may have returned a free index number,
		 * rather than one which already was populated.  In
		 * that case, adjust the sideno here.
		 */
		if (mnsn->mn_ln_sideno == 0)
			mnsn->mn_ln_sideno = s->s_sideno;

		pre_index = mnsn->mn_ln_suffix.suf_prefix;
	}
	...
	if (mnset == 0) {
		...
		if ((mnsn->mn_ln_suffix.suf_len != suf_len) ||
		    (bcmp(mnsn->mn_ln_suffix.suf_data, devname,
		    suf_len) != 0)) {
			bcopy(devname, mnsn->mn_ln_suffix.suf_data, suf_len);
			mnsn->mn_ln_suffix.suf_len = suf_len;
		}
	}
...
)
{
	...
	if (rip->ri_mbip != (mddb_mb_ic_t *)NULL) {
		...
		if (devidptr != (ddi_devid_t)NULL) {
			...
			mb->mb_revision = MD_MNSET_SETNO(s->s_setno) ?
			    MDDB_REV_MNMB : MDDB_REV_MB;
			mb2free = 1;
		} else {
			goto out;
		}
...
)
{
	...
	if ((setno != MD_LOCAL_SET) && (s->s_lbp->lb_flags & MDDB_MNSET) &&
	    (md_set[setno].s_am_i_master == 0)) {
		/* The exception is if we're currently importing the set */
		if ((md_get_setstatus(setno) & MD_SET_IMPORT) == 0) {
			single_thread_end(s);
			mddb_setexit(s);
			return (0);
		}
	...
	for (li = 0; li < lbp->lb_loccnt; li++) {
		...
		if ((did_info->info_flags & MDDB_DID_EXISTS) &&
			...
			if (MD_MNSET_SETNO(s->s_setno)) {
				...
				mnlbp = (mddb_mnlb_t *)lbp;
				for (idx = 0; idx < MD_MNMAXSIDES; idx++) {
					mnslp = &mnlbp->
					    lb_mnsidelocators[idx][li];
					if (mnslp->mnl_sideno == s->s_sideno)
						break;
				}
				/* No matching side found */
				if (idx == MD_MNMAXSIDES)
					continue;

				mnlnp = (mddb_mnln_t *)s->s_lnp;
				sn = (md_name_suffix *)&mnlnp->
				    ln_mnsuffixes[idx][li];
			} else {
...
)
{
	...
	/*
	 * It's probable that the setname will remain unchanged.
	 * If so, don't bother updating the shared namespace.
	 */
	if (strcmp(shrname, shn->sn_name) == 0)
		goto out;
...
)
{
	...
	/*
	 * In MN sets, it's possible that the side on which this
	 * set is being imported didn't exist in the previous
	 * configuration.  If matching side information is present,
	 * we can simply reuse it; if not, find a side that we can
	 * alter for reuse.
	 */
	if (MD_MNSET_SETNO(setno)) {
		side_t	side = md_side_in_namespace(s);

		if (side == MD_SIDEWILD) {
			goto cleanup;
		} else if (side != s->s_sideno) {
			md_alter_sidenos(s, side);
		}
	}

	/*
	 * Remove entries from now defunct sides
	 */
	md_cull_sides(s);

	/*
	 * Translation of directory names and minor names for use when
	 * importing sets between normal Solaris and SunCluster.
	 */
	if ((err = md_xlate_namespace(s)) != 0)
		goto cleanup;
	if ((err = md_xlate_minors(s)) != 0)
		goto cleanup;

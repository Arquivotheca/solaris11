...
md_base_ioctl(md_dev64_t dev, int cmd, caddr_t data, int mode, IOLOCK *lockp)
{
	...
	{
		/*
		 * Is the sun cluster did driver using devids?
		 */
		if (! (mode & FWRITE))
			return (EACCES);
		sz = sizeof (int);
		d = (int *)kmem_alloc(sz, KM_SLEEP);

		if (md_devid_supported_in_did() == MD_DEVID_SUPPORTED) {
			*(int *)d = MD_DEVID_SUPPORTED;
		} else {
			*(int *)d = MD_DEVID_NOT_SUPPORTED;
		}
		break;
	}

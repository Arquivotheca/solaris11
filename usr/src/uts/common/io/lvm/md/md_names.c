...
extern int	mddb_devid_get(mddb_set_t *, uint_t, ddi_devid_t *, char **);
		...
		if (nm != NULL) {
			...
			if (strncmp(nm, ((struct nm_shared_name *)shn)->sn_name,
				...
				if (devid_nm & NM_DEVID) {
					if (ddi_devid_compare((ddi_devid_t)nm,
					    (ddi_devid_t)(((struct
					    did_shr_name *)shn)->did_devid))
					    == 0)
						return ((void *)shn);
				} else {
					if (strcmp(nm, ((struct
					    nm_shared_name *)shn)->sn_name)
					    == 0)
						return ((void *)shn);
				}
			}
...
{
	struct nm_next_hdr	*shr_nh, *nh, *did_shr_nh, *did_nh = NULL;
	struct nm_name		*n;
	struct did_min_name	*did_n;
	struct did_shr_name	*did_shr_n;
	...
	if (devid_nm) {
		if (((did_nh = get_first_record(setno, 1, NM_DEVID |
		    NM_NOTSHARED)) == NULL) || ((did_shr_nh = get_first_record(
		    setno, 1, NM_DEVID | NM_SHARED)) == NULL) || ((shr_nh =
		    get_first_record(setno, 1, NM_SHARED)) == NULL)) {
			devid_nm = 0;
		}
	}
	...
	if (did_found) {
		...
		if (strcmp(drv, drvnm) != 0) {
			(void) remove_shared_entry(shr_nh, 0, drv, NM_SHARED);
...
/*
 * This function looks through the primary namespace to determine which
 * sides are present.  If a side matching the side of the importing node
 * is present, that side is returned.  If there is no match, the lowest
 * side is returned, or MD_SIDEWILD in the case of an error.
 *
 * The namespace must be already be loaded when calling this function.
 */
side_t
md_side_in_namespace(mddb_set_t *s)
{
	struct nm_next_hdr	*nh;
	side_t			myside = s->s_sideno;
	side_t			side;
	set_t			setno = s->s_setno;
	mdkey_t			key = MD_KEYWILD;
	side_t			ret = MD_SIDEWILD;

	VERIFY(md_get_setstatus(setno) & MD_SET_NM_LOADED);
	VERIFY(MD_MNSET_SETNO(setno));

	if ((nh = get_first_record(setno, 0, NM_NOTSHARED)) == NULL)
		return (ret);

	for (side = 0; side < MD_MNMAXSIDES; side++) {
		while ((key = md_getnextkey(setno, side, key, NULL)) !=
		    MD_KEYWILD) {
			struct nm_name		*name;

			rw_enter(&nm_lock.lock, RW_WRITER);
			if ((name = (struct nm_name *)lookup_entry(nh, setno,
			    side, key, NODEV64, 0L)) == NULL) {
				rw_exit(&nm_lock.lock);
				continue;
			}

			if (myside == name->n_side) {
				rw_exit(&nm_lock.lock);
				return (s->s_sideno);
			} else if (name->n_side < ret) {
				ret = name->n_side;
			}
			rw_exit(&nm_lock.lock);
		}
...
/*
 * This function goes through all the entries in the unshared namespaces
 * where 'side' matches the side passed in, and replaces the side information
 * with the side from the mddb_set passed in.
 *
 * The namespace must be already be loaded when calling this function,
 * and the side specified must not be the side specified by the mddb_set
 * passed in.
 */
void
md_alter_sidenos(mddb_set_t *s, side_t side)
{
	struct nm_next_hdr	*did_nh, *nh;
	set_t			setno = s->s_setno;
	mdkey_t			key = MD_KEYWILD;

	VERIFY(md_get_setstatus(setno) & MD_SET_NM_LOADED);
	VERIFY(MD_MNSET_SETNO(setno));
	VERIFY(side != s->s_sideno);

	nh = get_first_record(setno, 0, NM_NOTSHARED);
	did_nh = get_first_record(setno, 0, NM_DEVID | NM_NOTSHARED);
	if (nh == NULL || did_nh == NULL)
		return;

	while ((key = md_getnextkey(setno, side, key, NULL)) != MD_KEYWILD) {
		struct did_min_name	did_new, *did_name;
		struct nm_name		new, *name;
		mddb_recid_t		recids[3];
		char			*string;
		int			len;
		...
		name = (struct nm_name *)lookup_entry(nh, setno, side,
		    key, NODEV64, NM_NOTSHARED);
		did_name = (struct did_min_name *)lookup_entry(did_nh,
		    setno, side, key, NODEV64, NM_DEVID | NM_NOTSHARED);

		if (name != NULL) {
			string = md_strdup(name->n_name);
			len = strlen(string);
			new = *name;
			new.n_side = s->s_sideno;
			(void) remove_entry(nh, side, key, NM_NOTSHARED |
			    NM_KEY_RECYCLE);
			name = (struct nm_name *)alloc_entry(nh,
			    md_set[setno].s_nmid, len + 1, NM_NOTSHARED,
			    &recids[0]);
			*name = new;
			(void) strncpy(name->n_name, string, len);
			recids[1] = md_set[setno].s_nmid;
			recids[2] = 0;
			mddb_commitrecs_wrapper(recids);
			freestr(string);
		...
		if (did_name != NULL) {
			string = md_strdup(did_name->min_name);
			len = strlen(string);
			did_new = *did_name;
			did_new.min_side = s->s_sideno;
			(void) remove_entry(did_nh, side, key, NM_DEVID |
			    NM_NOTSHARED | NM_KEY_RECYCLE);
			did_name = (struct did_min_name *)alloc_entry(did_nh,
			    md_set[setno].s_did_nmid, len + 1, NM_DEVID |
			    NM_NOTSHARED, &recids[0]);
			*did_name = did_new;
			(void) strncpy(did_name->min_name, string, len);
			recids[1] = md_set[setno].s_did_nmid;
			recids[2] = 0;
			mddb_commitrecs_wrapper(recids);
			freestr(string);
...
/*
 * On import, a set will necessarily only have one host associated with
 * it.  If in its previous life this set had multiple hosts in it, this
 * routine will remove the namespace records holding now extraneous side
 * information.
 *
 * The namespace must be already be loaded when calling this function.
 */
void
md_cull_sides(mddb_set_t *s)
{
	struct nm_next_hdr	*did_nh, *did_shr_nh, *nh, *shr_nh;
	mdkey_t			key = MD_KEYWILD;
	set_t			setno = s->s_setno;
	side_t			myside = s->s_sideno;
	side_t			side;
	...
	nh = get_first_record(setno, 0, NM_NOTSHARED);
	shr_nh = get_first_record(setno, 0, NM_SHARED);
	did_nh = get_first_record(setno, 0, NM_DEVID | NM_NOTSHARED);
	did_shr_nh = get_first_record(setno, 0, NM_DEVID | NM_SHARED);
	if (nh == NULL || shr_nh == NULL || did_nh == NULL ||
	    did_shr_nh == NULL)
			return;
	...
	for (side = 0; side < MD_MNMAXSIDES; side++) {
		...
		while ((key = md_getnextkey(setno, side, key, NULL)) !=
		    MD_KEYWILD) {
			struct did_min_name	*did_name;
			struct nm_name		*name;


			rw_enter(&nm_lock.lock, RW_WRITER);

			name = (struct nm_name *)lookup_entry(nh,
			    setno, side, key, NODEV64, 0L);
			did_name = (struct did_min_name *)lookup_entry(
			    did_nh, setno, side, key, NODEV64, NM_DEVID);

			if (name != NULL) {
				mdkey_t	dirkey = name->n_dir_key;
				mdkey_t	drvkey = name->n_drv_key;

				(void) remove_entry(nh, side, key, 0L);
				(void) remove_shared_entry(shr_nh, dirkey,
				    (char *)0, 0L);
				(void) remove_shared_entry(shr_nh, drvkey,
				    (char *)0, 0L);
			}

			if (did_name != NULL) {
				mdkey_t	minkey = did_name->min_devid_key;

				(void) remove_entry(did_nh,
				    side, key, NM_DEVID);
				(void) remove_shared_entry(did_shr_nh,
				    minkey, (char *)0, NM_DEVID);
			}
...
/*
 * Convert minor names in the namespace (if required) from did style
 * minors to ctd style and vice versa.  This is done by comparing the
 * style of each entry to the style found in the in core replica list.
 * If they differ, the entry needs to be converted.
 *
 * The namespace must be already be loaded when calling this function.
 */
int
md_xlate_minors(mddb_set_t *s)
{
	struct nm_next_hdr	*did_nh, *did_shr_nh, *nh;
	struct did_min_name	*new;
	mddb_recid_t		recid, ids[3];
	set_t			setno = s->s_setno;
	side_t			side = s->s_sideno;
	mdkey_t			key = MD_KEYWILD;
	char			*xlated_minor, *tmpstr;
	int			riptype, entrytype, replicated;
	int			xlated_minor_len;
	int			ret = 0;
	...
	did_nh = get_first_record(setno, 0, NM_DEVID | NM_NOTSHARED);
	did_shr_nh = get_first_record(setno, 0, NM_DEVID | NM_SHARED);
	if (nh == NULL || did_nh == NULL || did_shr_nh == NULL)
		return (ENOENT);

	/*
	 * If we can't determine whether the replica list minors are
	 * did or ctd style, simply return without error, and without
	 * modifying any entries.
	 */
	MIN_NM_TYPE(s->s_rip->ri_minor_name, riptype);
	if (riptype == UNK_MIN_NM)
		return (ret);

	replicated = md_get_setstatus(setno) & MD_SET_REPLICATED_IMPORT;
	while ((key = md_getnextkey(setno, side, key, NULL)) != MD_KEYWILD) {
		struct did_shr_name	*dsn;
		struct did_min_name	*dmn;
		int			match;

		rw_enter(&nm_lock.lock, RW_WRITER);

		if ((dmn = (struct did_min_name *)lookup_entry(did_nh,
		    setno, side, key, NODEV64, NM_DEVID)) == NULL)
			goto next;

		if ((dsn = (struct did_shr_name *)lookup_shared_entry(
		    did_shr_nh, dmn->min_devid_key, (char *)0, NULL,
		    NM_DEVID)) == NULL)
			goto next;

		MIN_NM_TYPE(dmn->min_name, entrytype);
		match = (entrytype == riptype);

		if (match != 0 && replicated != 0 && entrytype == DID_MIN_NM) {
			mddb_ri_t	*rip;

			for (rip = s->s_rip; rip != (mddb_ri_t *)NULL;
			    rip = rip->ri_next) {
				/* char *didstrp = NULL; */

				if (rip->ri_old_devid == NULL) {
					/*
					 * Handle devid replacement...
					 */
					if (ddi_devid_compare(
					    (ddi_devid_t)dsn->did_devid,
					    rip->ri_devid) == 0) {
						char *xptr, *sptr;

						if (strncmp(rip->ri_minor_name,
						    dmn->min_name,
						    MAX(strlen(
						    rip->ri_minor_name),
						    strlen(dmn->min_name)))
						...
						/* Devid replacement... */
						xlated_minor = md_strdup(
						    rip->ri_minor_name);
						xptr = strrchr(xlated_minor,
						    's');
						sptr = strrchr(dmn->min_name,
						    's');
						(void) strcpy(xptr, sptr);
						break;
					}
					continue;
				}
				...
				if (ddi_devid_compare(
					...
					if (strncmp(rip->ri_minor_name,
					    dmn->min_name,
					    MAX(strlen(rip->ri_minor_name),
					    strlen(dmn->min_name))) == 0)
						goto next;
					...
					/*
					 * didstrp = strrchr(dmn->min_name,
					 * 's');
					 * didstrp++;
					 *
					 * tmpstr = strrchr(xlated_minor, 's');
					 * tmpstr++;
					 * *tmpstr = *didstrp;
					 */
					break;
			...
			goto next;
		} else if (match == 0 && entrytype == DID_MIN_NM) {
			struct nm_name	*n;
			/*
			 * n->name is did style (d20s0, or the like)
			 * and we want a translated minor like "a"
			 */
			n = lookup_entry(nh, setno, side, dmn->min_key,
			    NODEV64, 0L);
			if (n == NULL) {
				rw_exit(&nm_lock.lock);
				ret = ENOENT;
			...
			xlated_minor = md_strdup(tmpstr);
			*xlated_minor += 0x31;
		} else if (match == 0 && entrytype == STD_MIN_NM) {
			ddi_devid_t	icdevid;
			char		*icminor;
			int		li;

			/*
			 * Here, we have a regular minor name, and
			 * want it converted to a did driver style
			 * minor: "h" -> "20,20s7,blk"
			 *
			 * To do this, we need the number of the did
			 * device as well as the converted minor name.
			 * Use the devid to get it.
			 */
			for (li = 0; li < s->s_lbp->lb_loccnt; li++) {
				if (mddb_devid_get(s, li, &icdevid,
				    &icminor) == 0)
					continue;

				if (ddi_devid_compare((ddi_devid_t)
				    dsn->did_devid, icdevid) == 0) {
					xlated_minor = md_strdup(icminor);
					tmpstr = strrchr(xlated_minor, 's');
					if (tmpstr == NULL) {
						freestr(xlated_minor);
						continue;
					}
					tmpstr++;
					*tmpstr = *(dmn->min_name) - 0x31;
				}
			}
		}

		xlated_minor_len = strlen(xlated_minor) + 1;

		new = (struct did_min_name *)alloc_entry(did_nh,
		    md_set[setno].s_did_nmid, xlated_minor_len,
		    NM_DEVID | NM_NOTSHARED | NM_NOCOMMIT, &recid);

		new->min_namlen = xlated_minor_len;
		(void) strcpy(new->min_name, xlated_minor);
		if (xlated_minor) {
			freestr(xlated_minor);
			xlated_minor = NULL;
		}
		...
		new->min_side = side;
		new->min_key = dmn->min_key;
		new->min_count = dmn->min_count;
		new->min_devid_key = dmn->min_devid_key;

		ids[0] = recid;
		ids[1] = md_set[setno].s_did_nmid;
		ids[2] = 0;
		mddb_commitrecs_wrapper(ids);

		(void) remove_entry(did_nh, dmn->min_side, dmn->min_key,
		    NM_DEVID);
...
/*
 * For each entry in the namespace, examine its shared
 * namespace directory entry to determine if we need to
 * convert it from '/dev/dsk' to '/dev/did/dsk' or vice
 * versa.
 *
 * The namespace must be already be loaded when calling this function.
 */
int
md_xlate_namespace(mddb_set_t *s)
{
	struct nm_next_hdr	*shr_nh, *nh;
	set_t			setno = s->s_setno;
	side_t			side = s->s_sideno;
	mdkey_t			key = MD_KEYWILD;
	int			riptype;
	...
	while ((key = md_getnextkey(setno, side, key, NULL)) != MD_KEYWILD) {
		struct nm_name	*name;
		mdkey_t		dirkey, o_dirkey;
		char		*dir, *getp, *setp;
		char		*didp = "/dev/did/dsk/";
		char		*dskp = "/dev/dsk/";

		if (riptype == STD_MIN_NM) {
			getp = didp;
			setp = dskp;
		} else if (riptype == DID_MIN_NM) {
			getp = dskp;
			setp = didp;
		}

		rw_enter(&nm_lock.lock, RW_WRITER);
		name = lookup_entry(nh, setno, side, key, NODEV64, 0L);
		if (name == NULL) {
			rw_exit(&nm_lock.lock);
			continue;
		}

		dir = (char *)getshared_name(setno, name->n_dir_key, 0L);
		if ((dir != NULL) && strcmp(dir, getp) == 0) {
			dirkey = setshared_name(setno, setp, MD_KEYWILD, 0L);
			if (dirkey == MD_KEYBAD) {
				rw_exit(&nm_lock.lock);
				return (ENOENT);
			}
			o_dirkey = name->n_dir_key;
			name->n_dir_key = dirkey;
			if (update_entry(nh, side, name->n_key, 0L) != 0) {
				rw_exit(&nm_lock.lock);
				return (ENOENT);
			}
			(void) remove_shared_entry(shr_nh, o_dirkey, NULL, 0);
		}
		rw_exit(&nm_lock.lock);
	}

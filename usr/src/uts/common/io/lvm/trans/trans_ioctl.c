...
/*
 * This used to be the implementation of page_create();  now that that
 * interface is gone, we've moved a copy of the old implementation here,
 * to keep this compiling.
 */
static page_t *
trans_page_create(vnode_t *vp, u_offset_t off, size_t bytes, uint_t flags)
{
...
trans_test_trypage(void *d, int mode, IOLOCK *lock)
{
	 ...
	 */
	test = 2;
	if ((pp = trans_page_create(cvp, 0, 1, PG_WAIT)) == NULL)
		goto errout;

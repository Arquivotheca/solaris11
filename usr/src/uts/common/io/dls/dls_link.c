    ...
    boolean_t loopback)
{
	...
	/*
	 * Perform necessary multicast address and SAP value checks
	 * to filter out irrelevant packets.
	 */
...
static int
i_dls_link_create(const char *name, dls_link_t **dlpp, zoneid_t zid)
{
	dls_link_t		*dlp;
	int			err;
    ...
    zoneid_t zid)
{
	 ...
	 */
	if ((err = i_dls_link_create(name, &dlp, zid)) != 0)
		return (err);
...
int
dls_link_hold_create(const char *name, dls_link_t **dlpp, zoneid_t zid)
{
	return (dls_link_hold_common(name, dlpp, B_TRUE, zid));
}

int
dls_link_hold(const char *name, dls_link_t **dlpp)
{
	return (dls_link_hold_common(name, dlpp, B_FALSE, ALL_ZONES));
}
...
dls_link_setzid(const char *name, zoneid_t zid)
{
	...
	if ((err = dls_link_hold_create(name, &dlp, zid)) != 0)
		return (err);
	...
	dlp->dl_zid = zid;
	(void) mac_link_setlinkzid(dlp->dl_mh, zid);

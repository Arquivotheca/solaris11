...
#define	DD_AUTOMATIC_LINK	0x8 /* Automatically created link for NGZ */
...
typedef struct dls_devnet_s {
	...
	kstat_t		*dd_gzone_ksp;	/* in global zone if owner_zid != 0 */
	uint32_t	dd_ref;
	kmutex_t	dd_mutex;
	kcondvar_t	dd_cv;
	uint32_t	dd_tref;
...
dls_zone_remove(datalink_id_t linkid, void *arg)
{
	...
	if (dls_devnet_hold_tmp(linkid, &ddp) == 0) {
		/*
		 * Automatically created data links are destroyed
		 * by zoneadmd when the zone is removed, and not
		 * assigned back to the global zone. The deletion
		 * of automatic links is done by zoneadmd (and not
		 * here) so that any error in deleting a link can
		 * be reported back to the user attempting to halt
		 * or shutdown the zone.
		 */
		if (!(ddp->dd_flags & DD_AUTOMATIC_LINK))
 ...
 *     - novanity	vanity naming is not supported
 */
int
dls_mgmt_create(const char *devname, dev_t dev, datalink_class_t class,
    uint32_t media, boolean_t persist, boolean_t novanity,
...
int
dls_mgmt_get_linkattr(const datalink_id_t linkid, const char *attr,
...
dls_mgmt_get_phydev(datalink_id_t linkid, dev_t *devp)
{
	...
	if (dls_mgmt_get_linkattr(linkid, FPHYMAJ, &maj, &attrsz) != 0 ||
	    attrsz != sizeof (uint64_t) ||
	    dls_mgmt_get_linkattr(linkid, FPHYINST, &inst, &attrsz) != 0 ||
	    attrsz != sizeof (uint64_t)) {
		return (EINVAL);
	}
...
/*
 * Send a call to the datalink management about a virtual link that's
 * created or going to be deleted.
 */
void
dls_mgmt_update_vlink(datalink_id_t linkid, datalink_id_t vlinkid,
    uint16_t vid, boolean_t is_primary, boolean_t added)
{
	dlmgmt_door_vlink_update_t	uvlink;
	dlmgmt_retval_t			retval;

	uvlink.ld_cmd = DLMGMT_CMD_VLINK_UPDATE;
	uvlink.ld_linkid = linkid;
	uvlink.ld_vlinkid = vlinkid;
	uvlink.ld_vid = vid;
	uvlink.ld_isprim = is_primary;
	uvlink.ld_added = added;

	(void) i_dls_mgmt_upcall(&uvlink, sizeof (uvlink), &retval,
	    sizeof (retval));
}
 ...
 *
 * There are four cases for links and zones:
 *
 * Case 1: Links created by GZ
 *         owner and current zone = GZ
 *         dd_ksp -> create kstat for owner
 *         dd_zone_ksp -> NULL
 *         dd_gzone_ksp -> NULL
 * Case 2: Links created by GZ and currently assigned to another zone
 *         owner zone = GZ
 *         current zone = NGZ
 *         dd_ksp -> create kstat for owner
 *         dd_zone_ksp -> create kstat for the current zone
 *         dd_gzone_ksp -> make kstat for the current NGZ visible in GZ
 * Case 3: Links created by NGZ (e.g. iptuns)
 *         owner and current zone = NGZ
 *         dd_ksp -> create kstat for owner
 *         dd_zone_ksp -> NULL
 *         dd_gzone_ksp -> make kstat for the current NGZ visible in GZ
 * Case 4: Links created by GZ for NGZ (e.g. anets)
 *         owner zone = GZ
 *         current zone = NGZ
 *         dd_ksp -> NULL (owner doesn't need separate kstat object)
 *         dd_zone_ksp -> create kstat for the current zone
 *         dd_gzone_ksp -> make kstat for the current NGZ visible in GZ
 */
static void
dls_devnet_stat_create(dls_devnet_t *ddp, zoneid_t zoneid)
{
	...
	if (dls_stat_create("link", 0, ddp->dd_linkname, zoneid,
		...
		/*
		 * Datalinks in NGZs have "link" kstats visible in the GZ
		 * under instance set to the zone ID. Using the kstat instance
		 * avoids any kstat collisions. We assume the max zone ID is
		 * always lower than max int value of ks_instance field.
		 */
		if (zoneid != GLOBAL_ZONEID) {
			kstat_t *gksp = NULL;

			(void) dls_stat_create("link", zoneid, ddp->dd_linkname,
			    GLOBAL_ZONEID, dls_devnet_stat_update, ddp, &gksp);
			ASSERT(ddp->dd_gzone_ksp == NULL);
			ddp->dd_gzone_ksp = gksp;
		}
	}
}
...
dls_devnet_stat_destroy(dls_devnet_t *ddp, zoneid_t zoneid)
{
	...
	if (zoneid != GLOBAL_ZONEID) {
		if (ddp->dd_gzone_ksp != NULL) {
			kstat_delete(ddp->dd_gzone_ksp);
			ddp->dd_gzone_ksp = NULL;
		}
	}
}
...
dls_devnet_stat_rename(dls_devnet_t *ddp)
{
	...
	if (ddp->dd_owner_zid != GLOBAL_ZONEID && ddp->dd_gzone_ksp != NULL) {
		kstat_delete(ddp->dd_gzone_ksp);
		ddp->dd_gzone_ksp = NULL;
	}
 ...
 */
static int
dls_devnet_set(const char *macname, datalink_id_t linkid,
    zoneid_t owner_zoneid, zoneid_t target_zoneid, dls_devnet_t **ddpp)
{
	...
	if ((err = mod_hash_find(i_dls_devnet_hash,
		...
		ddp->dd_ref++;
		/*
		 * ddp->dd_zid is initialized here to owner_zoneid
		 * Later below it is assigned to the target_zoneid
		 * by i_dls_devnet_setzid.
		 */
		ddp->dd_zid = owner_zoneid;
		ddp->dd_owner_zid = owner_zoneid;
	...
	if (linkid != DATALINK_INVALID_LINKID) {
		...
		/*
		 * Set flag on datalink to indicate that the datalink
		 * has been created by one zone for another (E.g. using
		 * anet resource from zonecfg command). This is
		 * different from assigning an existing link from
		 * one zone to another. In the former case, the link
		 * should not be re-assigned back to the owner during
		 * dls_devnet_unset.
		 */
		if (owner_zoneid != target_zoneid)
			ddp->dd_flags |= DD_AUTOMATIC_LINK;
	}
	err = 0;
done:
	/*
	...
	if (err == 0) {
		if (target_zoneid != GLOBAL_ZONEID &&
		    (err = i_dls_devnet_setzid(ddp,
		    target_zoneid, B_FALSE)) != 0)
		 ...
		 *
		 * We create the stats for the target zone.
		 * dls_devnet_stat_create() will also create an entry in the
		 * GZ if the target zone is not GZ. Both the kstats refer
		 * to the same underlying kstat object allowing GZ to
		 * access kstats created inside a NGZ.
		 */
		if (stat_create)
			dls_devnet_stat_create(ddp, target_zoneid);
		if (ddpp != NULL)
			*ddpp = ddp;
	}
	return (err);
...
dls_devnet_unset(const char *macname, datalink_id_t *id, boolean_t wait)
{
	...
	*id = ddp->dd_linkid;
	...
	/*
	 * The locking order is kstat lock -> dd_mutex.
	 *
	 * So, we need drop the dd_mutex lock here before deleting the kstats
	 * to prevent a deadlock with a thread calling the kstat update
	 * routine - dls_devnet_stat_update. This is safe since
	 * dls_devnet_stat_update checks for DD_CONDEMNED flag before
	 * proceeding and we set the flag above.
	 */
	mutex_exit(&ddp->dd_mutex);

	/*
	 * Delete all stats created for this link
	 */
	if (ddp->dd_linkid != DATALINK_INVALID_LINKID) {
	...
	if (ddp->dd_zid != ddp->dd_owner_zid) {
		if ((ddp->dd_flags & DD_AUTOMATIC_LINK) == 0) {
			/*
			 * If this link was created by its owner for
			 * another zone (example: using anet resource
			 * in zonecfg command), then don't reassign this
			 * link back to its owner because this was
			 * created solely for the current zone
			 * its current zoneid (mi_zoneid) is still
			 * required for later stages such as mac_unregister.
			 *
			 * If this link was assigned by its owner to another
			 * zone during dls_devnet_set, reassign it back to its
			 * owner. This will also release the additional
			 * reference that was acquired during dls_devnet_set
			 * (See dls_link_setzid).
			 */
			err = i_dls_devnet_setzid(ddp, ddp->dd_owner_zid,
			    B_FALSE);
		}
		if (err != 0) {
			return (err);
...
struct dls_devnet_walk_state {
	char		ddw_linkname[MAXLINKNAMELEN];
	zoneid_t	ddw_zoneid;
	datalink_id_t	ddw_linkid;
};

/*ARGSUSED*/
static uint_t
i_dls_devnet_linkname2linkid(mod_hash_key_t key, mod_hash_val_t *valp,
    void *arg)
{
	struct dls_devnet_walk_state	*ddwp = arg;
	dls_devnet_t			*ddp = (dls_devnet_t *)valp;

	if (ddp->dd_zid == ddwp->ddw_zoneid && strcmp(ddp->dd_linkname,
	    ddwp->ddw_linkname) == 0) {
		ddwp->ddw_linkid = ddp->dd_linkid;
...
/*
 * Get the linkid of an active link in a given zone (that exists in /dev/net)
 */
int
dls_devnet_get_active_linkid(const char *linkname, zoneid_t zoneid,
    datalink_id_t *linkidp)
{
	struct dls_devnet_walk_state	ddw;

	(void) strlcpy(ddw.ddw_linkname, linkname, MAXLINKNAMELEN);
	ddw.ddw_zoneid = zoneid;
	ddw.ddw_linkid = DATALINK_INVALID_LINKID;
	rw_enter(&i_dls_devnet_lock, RW_READER);
	mod_hash_walk(i_dls_devnet_hash, i_dls_devnet_linkname2linkid, &ddw);
	*linkidp = ddw.ddw_linkid;
	rw_exit(&i_dls_devnet_lock);
	return ((*linkidp) == DATALINK_INVALID_LINKID ? ENOENT : 0);
}

/*
 * Get the link name and zoneid of an an active link (that exists in /dev/net)
 * by the given linkid
 */
int
dls_devnet_get_active_linkname(datalink_id_t linkid, zoneid_t *zidp,
    char *linkname, size_t len)
{
	...
	if (strlcpy(linkname, ddp->dd_linkname, len) >= len)
		err = EINVAL;
	else
		*zidp = ddp->dd_zid;
...
/*
 * This function is called when the owner zone != target zone.
 * This function returns onloan = false if it is a automatic vnic
 * otherwise it returns onloan = true.
 */
int
dls_devnet_get_onloan(mac_handle_t mh, boolean_t *onloan)
{
	dls_devnet_t	*ddp;

	rw_enter(&i_dls_devnet_lock, RW_READER);
	if (mod_hash_find(i_dls_devnet_hash, (mod_hash_key_t)mac_name(mh),
	    (mod_hash_val_t *)&ddp) != 0) {
		rw_exit(&i_dls_devnet_lock);
		return (ENOENT);
	}

	*onloan = (ddp->dd_flags & DD_AUTOMATIC_LINK) ? B_FALSE : B_TRUE;
	rw_exit(&i_dls_devnet_lock);
	return (0);
}
...
dls_devnet_rename(datalink_id_t id1, datalink_id_t id2, const char *link)
{
	...
	/* Re-naming Auto VNICs is not allowed */
	if ((ddp->dd_flags & DD_AUTOMATIC_LINK) != 0) {
		err = EINVAL;
		goto done;
	}
	...
	/*
	 * Increment dd_tref to prevent the link from being deleted
	 * until we finish.
	 */
	ddp->dd_tref++;
	tref_held = B_TRUE;
	...
	mac_rename_linkname(mh, ddp->dd_linkname);
...
/*
 * Note: We assume the caller did a dls_devnet_hold() or a
 * dls_devnet_hold_tmp() to get the 'ddh' handle. This hold prevents
 * a race with any dls_devnet_unset() in progress.
 */
int
dls_devnet_setzid(dls_dl_handle_t ddh, zoneid_t new_zid)
{
	...
	/*
	 * If the datalink was created inside a NGZ then we don't allow
	 * the datalink to be re-assigned to another zone.
	 */
	if (ddh->dd_owner_zid != GLOBAL_ZONEID)
		return (EINVAL);
...
boolean_t
dls_devnet_cansetzone(dls_dl_handle_t ddh)
{
	dls_devnet_t	*ddp = ddh;

	/* Cannot change zone property on Auto VNICs */
	if ((ddp->dd_flags & DD_AUTOMATIC_LINK) != 0)
		return (B_FALSE);
	return (B_TRUE);
}
 ...
 * zone, or if it is currently assigned to the zone or if the caller is in the
 * global zone.
 */
boolean_t
...
dls_devnet_open(const char *link, dls_dl_handle_t *dhp, dev_t *devp)
{
	...
	err = dls_link_hold_create(ddp->dd_mac, &dlp, ddp->dd_zid);
	mac_perim_exit(mph);
...
{
	return (dls_devnet_create_zone(mh, linkid, zoneid, zoneid));
}

int
dls_devnet_create_zone(mac_handle_t mh, datalink_id_t linkid,
    zoneid_t owner_zoneid, zoneid_t target_zoneid)
{
	dls_link_t	*dlp;
	dls_devnet_t	*ddp;
	int		err;
	 ...
	 */
	if ((err = dls_devnet_set(mac_name(mh), linkid,
	    owner_zoneid, target_zoneid, &ddp)) == 0) {
		if ((err = dls_link_hold_create(mac_name(mh), &dlp,
		    target_zoneid)) != 0) {
			mac_perim_exit(mph);
...
dls_devnet_recreate(mac_handle_t mh, datalink_id_t linkid)
{
	...
	return (dls_devnet_set(mac_name(mh), linkid,
	    GLOBAL_ZONEID, GLOBAL_ZONEID, NULL));
}
...
dls_devnet_destroy(mac_handle_t mh, datalink_id_t *idp, boolean_t wait)
{
	...
	*idp = DATALINK_INVALID_LINKID;
	...
	if (err != 0) {
		...
		(void) dls_devnet_set(mac_name(mh), *idp, zoneid, zoneid,
		    NULL);
	}
	return (err);
}
    ...
    datalink_id_t *linkid)
{
	...
	err = dls_mgmt_create((char *)linkname, 0, DATALINK_CLASS_IPTUN, media,
	    B_FALSE, B_FALSE, &ik.iptun_kparam_linkid);
	if (err != 0) {

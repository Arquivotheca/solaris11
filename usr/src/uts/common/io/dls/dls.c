...
#define	DLS_SAP_ADDRESS_CHECK(promisc, dsp, mhip) {			\
	if (promisc && (dsp->ds_promisc & DLS_PROMISC_SAP) == 0) {	\
		if (dsp->ds_sap != mhip->mhi_bindsap) {			\
			if (dsp->ds_mip->mi_media == DL_TPR &&		\
			    dsp->ds_sap != DL_TPR_ANY_SAP)		\
				goto refuse;				\
			else if (dsp->ds_sap != DL_ANY_SAP)		\
				goto refuse;				\
		}							\
	}								\
}
...
dls_close(dld_str_t *dsp)
{
	 ...
	 */
	(void) dls_promisc(dsp, DLS_PROMISC_ALL, B_FALSE);
...
int
dls_promisc(dld_str_t *dsp, uint32_t new_flags, boolean_t is_promisc)
{
	int		err = 0;
	uint32_t	old_flags;

	ASSERT(MAC_PERIM_HELD(dsp->ds_mh));
	ASSERT(!(dsp->ds_promisc & ~DLS_PROMISC_ALL));
	...
	if (is_promisc) {
		...
		err = mac_promisc_add(dsp->ds_mch, MAC_CLIENT_PROMISC_ALL,
		    ...
		    ((old_flags | new_flags) != DLS_PROMISC_SAP) ? 0 :
		...
		/*
		 * It's possible that mac_promisc_add() can fail.  If so,
		 * then ds_mph will be NULL which means we return with
    ...
    void **ds_rx_arg, boolean_t promisc, boolean_t promisc_loopback)
{
	...
	case MAC_ADDRTYPE_BROADCAST:
		/*
		 * Perform SAP value check if DLS_PROMISC_SAP is not set
		 * If SAP check fails, check if stream is registered
		 * to DLPI_ANY_SAP mode.
		 */
		DLS_SAP_ADDRESS_CHECK(promisc, dsp, mhip)
		goto accept;
	case MAC_ADDRTYPE_MULTICAST:
		 ...
		 * addresses.
		 * If DLS_PROMISC_MULTI is not set perform SAP value check
		 * with special handling for DLPI_ANY_SAP mode.
		 */
...
dls_active_set(dld_str_t *dsp)
{
	...
	/*
	 * Callers in the GZ can observe NGZ datalinks under /dev/net/zone
	 * (ds_local is true) but are not allowed to issue DLPI primitives on
	 * a NGZ datalink that can result in the DLPI stream becoming "active".
	 * Such callers must use passive mode.
	 */
	if (!dsp->ds_local) {
		ASSERT(dsp->ds_dlp->dl_zid != GLOBAL_ZONEID);
		return (EINVAL);
	}

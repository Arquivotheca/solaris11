...
int
vnic_check_args(vnic_mac_addr_type_t mac_addr_type, int mac_len, int mac_slot,
    int mac_prefix_len, vnic_ioc_diag_t *diag)
{
	int err = 0;
...
vnic_ioc_create(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	if ((err = vnic_check_args(mac_addr_type, mac_len, mac_slot,
	    mac_prefix_len, &diag)) != 0) {
			goto bail;
	}
...
create:
	err = vnic_dev_create(create_arg->vc_vnic_id,
	    create_arg->vc_link_id, create_arg->vc_zone_id,
...
vnic_ioc_modify(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	rv = vnic_check_args(modify_arg->vm_mac_addr_type,
	    modify_arg->vm_mac_len, modify_arg->vm_mac_slot,
	    modify_arg->vm_mac_prefix_len, &modify_arg->vm_diag);
	if (rv != 0) {
		return (rv);
	}
	...
	return (vnic_dev_modify(modify_arg->vm_vnic_id,
	    ...
	    &modify_arg->vm_mac_slot, modify_arg->vm_mac_prefix_len,
	    &modify_arg->vm_resource_props, &modify_arg->vm_diag));
}

...
ldtermclose(queue_t *q, int cflag, cred_t *crp)
{
	...
	if (!(cflag & (FNDELAY|FNONBLOCK)) && !(tp->t_state & TS_POSIX)) {
		struct iocblk *iocb;
...
ldtermrput(queue_t *q, mblk_t *mp)
{
	...
	/*
	 * This high priority message cannot be called from service routine,
	 * needs immediate processing. For fifo, a close at one end needs to
	 * send immediate hang-up to the other end.
	 * Call ldtermrmsg() to do the actual M_HANGUP processing.
	 */
	case M_HANGUP:
...
ldtermrmsg(queue_t *q, mblk_t *mp)
{
	...
	if (mp->b_datap->db_type <= QPCTL && !bcanputnext(q, mp->b_band)) {
		 ...
		 * Note that you will lose the data in this
...
ldterm_do_ioctl(queue_t *q, mblk_t *mp)
{
	...
	case PTSSTTY:
		...
		/*
		 * ldterm uses this ioctl for the lower levels
		 * it signals that the lower modules should
		 * set the proper flag but they should return
		 * EEXIST so that libc won't push ldterm again.
		 */
		iocp->ioc_cmd = _PTSSTTY;
		break;

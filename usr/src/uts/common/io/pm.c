...
/*
 * number of properties that are dependent upon admin authority
 */
#define	NO_DEPENDENT_PROPS	2


extern kmutex_t	pm_prop_lock;	/* protects pm[gs]et_props() */
...
extern pm_kernel_config_t pm_kernel_config;
...
static	int pm_set_props(intptr_t arg, int mode);
static	int pm_get_props(intptr_t arg, int mode);
static	int pm_disable_power_management();
static	int pm_apply_props(uint64_t ttfc, uint64_t ttmr);
static	pm_authority_t pm_get_auth(const char *);
static	const char *pm_get_authstr(pm_authority_t);
static	int pm_apply_suspend(boolean_t pm_suspend_enabled);
static	int pm_apply_suspend_enable(void);
static	int pm_apply_suspend_disable(void);
static	int pm_apply_plat_props(void);
static	int pm_apply_smf_props(nvlist_t *list);
static	int pm_apply_all_props(nvlist_t *list);
static	int pm_apply_independent_props(nvlist_t *list);
static	int pm_apply_dependent_props(nvlist_t *list);
...
pm_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		/*
		 * Fill in the "S0" element for pm_syscap[] if one doesn't
		 * already exist.
		 */
		if (pm_syscap[SYSTEM_POWER_S0] == NULL) {
			pmcp = (pm_capability_t *)kmem_zalloc(
			    sizeof (pm_capability_t), KM_SLEEP);
			/*
			 * Note as we'zalloc'd above, only set
			 * non-zero values.
			 */
			pm_syscap[SYSTEM_POWER_S0] = pmcp;
			pmcp->cp_capacity = PM_CAP_CAPACITY_MAX;
			pmcp->cp_power = PM_CAP_POWER_MAX;
		}
		ddi_report_dev(dip);
		return (DDI_SUCCESS);
...
struct pm_cmd_info {
	...
	int str_type;		/* PM_REQ, NOSTRUCT, PM_CONF */
...
pm_perms(int perm, cred_t *cr)
{
	...
	if (perm == CHECKPERMS) {	/* check for PRIV_SYS_CONFIG */
...
static struct pm_cmd_info pmci[] = {
	...
	{PM_GET_AUTOSUSPEND_STATE, "PM_GET_AUTOSUSPEND_STATE", 1, NOSTRUCT},
	{PM_GET_SUSPEND_SUPPORT_STATE, "PM_GET_SUSPEND_SUPPORT_STATE", 1,
	    NOSTRUCT},
	...
	{PM_START_AUTOSUSPEND, "PM_START_AUTOSUSPEND", 1, NOSTRUCT, 0, 0, 0,
	    SU},
	{PM_STOP_AUTOSUSPEND, "PM_STOP_AUTOSUSPEND", 1, NOSTRUCT, 0, 0, 0, SU},
	{PM_ENABLE_SUSPEND, "PM_ENABLE_SUSPEND", 1, NOSTRUCT, 0, 0, 0, SU},
	{PM_DISABLE_SUSPEND, "PM_DISABLE_SUSPEND", 1, NOSTRUCT, 0, 0, 0, SU},
	{PM_ENTER_SUSPEND, "PM_ENTER_SUSPEND", 1, NOSTRUCT, 0, 0, 0, SU},
	...
	{PM_DEFAULT_CPU_DEEP_IDLE, "PM_DFLT_CPU_DEEP_IDLE", 1, NOSTRUCT, 0,
	    0, 0, SU},
	{PM_GET_PROPS, "PM_GET_PROPS", 1, PM_CONF},
	{PM_SET_PROPS, "PM_SET_PROPS", 1, PM_CONF, 0, 0, 0, CHECKPERMS},
	{0, NULL}
};
...
pm_ioctl(dev_t dev, int cmd, intptr_t arg, int mode, cred_t *cr, int *rval_p)
{
	...
	case NOSTRUCT:
	{
		...
		case PM_GET_AUTOSUSPEND_STATE:
		{
			if (autosuspend_enabled) {
				*rval_p = PM_AUTOSUSPEND_ENABLED;
			} else {
				*rval_p = PM_AUTOSUSPEND_DISABLED;
			}
			ret = 0;
			break;
		}

		case PM_GET_SUSPEND_SUPPORT_STATE:
		{
			if (pm_suspend_enabled || cpr_platform_enable) {
				*rval_p = PM_SUSPEND_SUPPORT_ENABLED;
			} else {
				*rval_p = PM_SUSPEND_SUPPORT_DISABLED;
			}
			ret = 0;
			break;
		}

		case PM_ENABLE_SUSPEND:
		{
			ret = pm_apply_suspend_enable();
			break;
		}

		case PM_DISABLE_SUSPEND:
		{
			ret = pm_apply_suspend_disable();
			break;
		}

		case PM_START_AUTOSUSPEND:
		{
			mutex_enter(&pm_scan_lock);
			if (autosuspend_enabled) {
				mutex_exit(&pm_scan_lock);
	...
	{
		/*
		 * Commands that require pm_config_t as arg
		 */
		switch (cmd) {
		case PM_GET_PROPS:
			ret = pm_get_props(arg, mode);
			break;
		case PM_SET_PROPS:
			ret = pm_set_props(arg, mode);
			break;
		default:
			/*
			 * Internal error, invalid ioctl description
...
int
pm_set_props(intptr_t arg, int mode)
{

#ifdef	PMDDEBUG
	char		*cmdstr = "SET_PROPS";
#endif	/*	PMDDEBUG	*/
	...
	int		ret = 0;
	int		err = 0;
	pm_config32_t	pmconf32;

	PMD(PMD_IOCTL, ("%s called\n", cmdstr))

	/*
	 * Only permit setting properties in global zone
	 */
...
#ifdef	_MULTI_DATAMODEL
	if ((mode & DATAMODEL_MASK) == DATAMODEL_ILP32) {
		if (ddi_copyin((caddr_t)arg, &pmconf32,
		    sizeof (pmconf32), mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: ddi_copyin "
			    "EFAULT\n\n", cmdstr))
			return (EFAULT);
		}
		pmconf.buf = (caddr_t)(uintptr_t)pmconf32.buf;
		pmconf.size = pmconf32.size;
	} else
#endif /* _MULTI_DATAMODEL */
	{
		if (ddi_copyin((caddr_t)arg, &pmconf,
		    sizeof (pmconf), mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: ddi_copyin "
			    "EFAULT\n\n", cmdstr))
			return (EFAULT);
		}
	...
	packed = kmem_alloc(pmconf.size, KM_NOSLEEP);
	if (packed == NULL) {
		PMD(PMD_IOCTL, ("%s returned EFAULT #1\n", cmdstr))
		return (EFAULT);
	}

	ret = copyin(pmconf.buf, packed, pmconf.size);
	if (ret != 0) {
		PMD(PMD_ERROR, ("%s returned EFAULT #2\n", cmdstr))
		kmem_free(packed, pmconf.size);
		return (EFAULT);
	}

	err =  nvlist_unpack(packed, pmconf.size, &list, KM_NOSLEEP);
	if (err != 0) {
		PMD(PMD_IOCTL, ("%s returned EFAULT nvlist_unpack() failed\n",
		    cmdstr))
		kmem_free(packed, pmconf.size);
		return (err);
	}

	mutex_enter(&pm_prop_lock);

	/*
	 * apply all the properties
	 */
	ret = pm_apply_all_props(list);

	mutex_exit(&pm_prop_lock);
	...
	PMD(PMD_IOCTL, ("%s returned normally %d\n", cmdstr, ret))
	return (ret);
}

int
pm_get_props(intptr_t arg, int mode)
{

#ifdef	PMDDEBUG
	char		*cmdstr = "GET_PROPS";
#endif	/*	PMDDEBUG	*/

	int		ret = 0;
	const char	*aa_string;
	nvlist_t	*list = NULL;
	pm_config_t	pmconf;
	pm_config_t	*pmconfp;
	pm_config_t	conf;
	pm_config32_t	pmconf32;
	pm_config32_t	*pmconfp32;

	PMD(PMD_IOCTL, ("%s called\n", cmdstr))

#ifdef	_MULTI_DATAMODEL
	if ((mode & DATAMODEL_MASK) == DATAMODEL_ILP32) {
		pmconfp32 = (pm_config32_t *)arg;
		if (ddi_copyin((caddr_t)arg, &pmconf32,
		    sizeof (pmconf32), mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: ddi_copyin "
			    "EFAULT\n\n", cmdstr))
	...
	PMD(PMD_IOCTL, ("%s pmconf.size %ld\n", cmdstr, pmconf.size))
	PMD(PMD_IOCTL, ("%s PM_SMF_TTFC = %s\n", cmdstr, PM_SMF_TTFC))

	ret = nvlist_alloc(&list, NV_UNIQUE_NAME_TYPE, DDI_NOSLEEP);

	if (ret != 0) {
		PMD(PMD_IOCTL, ("%s returned ret= %d\n", cmdstr, ret))
		return (ret);
	}

	mutex_enter(&pm_prop_lock);
	/*
	 * All of the current values are returned as normal property names.
	 * Values that represent the SMF or PLATFORM authorities take the
	 * form:
	 *	authority_name/property_name
	 */
	ret = nvlist_add_uint64(list, PM_PLAT_TTFC,
	    pm_kernel_config.pm_plat_ttfc);
	if (ret != 0)
		goto err;

	ret = nvlist_add_uint64(list, PM_PLAT_TTMR,
	    pm_kernel_config.pm_plat_ttmr);
	if (ret != 0)
		goto err;

	ret = nvlist_add_uint64(list, PM_PROP_TTFC,
	    pm_kernel_config.pm_curr_ttfc);
	if (ret != 0)
		goto err;

	ret = nvlist_add_uint64(list, PM_PROP_TTMR,
	    pm_kernel_config.pm_curr_ttmr);
	if (ret != 0)
		goto err;

	aa_string = pm_get_authstr(pm_kernel_config.pm_auth);
	if (aa_string == NULL) {
		PMD(PMD_IOCTL, ("%s returned EINVAL #2\n", cmdstr))
		ret = EINVAL;
		goto err;
	}

	PMD(PMD_IOCTL, ("%s aa_string %s\n", cmdstr, aa_string))

	ret = nvlist_add_string(list, PM_PROP_AUTHORITY, aa_string);
	if (ret != 0)
		goto err;
	PMD(PMD_IOCTL, ("%s aa_string added to nvlist OK\n", cmdstr))

	ret = nvlist_add_boolean_value(list, PM_PROP_SUSPEND_ENABLE,
	    pm_kernel_config.pm_suspend_enabled);
	if (ret != 0)
		goto err;

	ret = nvlist_add_boolean_value(list, PM_PLAT_DISABLED,
	    pm_kernel_config.pm_plat_disabled);
	if (ret != 0)
		goto err;

	PMD(PMD_IOCTL, ("%s packing list\n", cmdstr))
	...
	conf.buf = NULL;
	conf.size = 0;
	ret = nvlist_pack(list, &conf.buf, &conf.size, NV_ENCODE_XDR,
	    KM_NOSLEEP);
	if (ret != 0) {
		PMD(PMD_IOCTL, ("%s nvlist_pack() returned %d\n", cmdstr, ret))
		goto err;
	}

	/*
	 * inform the caller the minimum required size
	 */

#ifdef	_MULTI_DATAMODEL
	if ((mode & DATAMODEL_MASK) == DATAMODEL_ILP32) {
		pmconf32.size = conf.size;
		if (ddi_copyout(&pmconf32.size, &pmconfp32->size,
		    sizeof (pmconf32.size), mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: copyout "
			    "failed--EFAULT\n", cmdstr))
		}
	} else
#endif	/* _MULTI_DATAMODEL */
	{
		if (ddi_copyout(&conf.size, &pmconfp->size,
		    sizeof (conf.size), mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: copyout "
			    "failed--EFAULT\n", cmdstr))
		}
	}
	...
	/*
	 * size for the buffer we were given is too small, so return EOVERFLOW
	 */
	if (conf.size > pmconf.size) {
		PMD(PMD_IOCTL, ("%s returned EOVERFLOW conf.size = %ld, "
		    "pmconf.size = %ld\n", cmdstr, conf.size, pmconf.size))
		ret = EOVERFLOW;
		goto err;
	}

	/*
	 * copy out the packed buffer
	 */
#ifdef	_MULTI_DATAMODEL
	if ((mode & DATAMODEL_MASK) == DATAMODEL_ILP32) {
		pmconf32.buf = (caddr32_t)(uintptr_t)pmconf.buf;
		if (ddi_copyout(conf.buf, (void *)(uintptr_t)pmconf32.buf,
		    conf.size, mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: copyout failed--EFAULT\n",
			    cmdstr))
			ret = EFAULT;
		}
	} else
#endif	/* _MULTI_DATAMODEL */
	{
		if (ddi_copyout(conf.buf, pmconf.buf, conf.size, mode) != 0) {
			PMD(PMD_ERROR, ("ioctl: %s: copyout failed--EFAULT\n",
			    cmdstr))
			ret = EFAULT;
		}
...
/*
 *  pm_disable_power_management() -
 *
 * At present, this simply means that we disable CPUPM
 */
...
{
/*
 * Call in to each resource manager to disable their doing
 * any power management
 *
 * For the present, we simply disable CPU PM
 */
	pm_kernel_config.pm_curr_disabled = B_TRUE;
	return (pm_apply_props(0, 0));
}
/*
 * pm_apply_props(ttfc, ttmr) -
 *
 * This function transmits the current responsiveness constraints
 * to each of the system resource managers (e.g. CPU, I/O, VM,
 * Filesystem etc.)
 *
 * For the present just use P-states C1 and "deep" C-states
 */

/*ARGSUSED1*/	/* only 1st arg used for now	*/
static	int
pm_apply_props(uint64_t ttfc, uint64_t ttmr)
{

#ifdef	PMDDEBUG
	char		*cmdstr = "APPLY_PROPS";
#endif	/*	PMDDEBUG	*/

	int		ret = 0;
	pm_cpupm_t	prev_cpupm = cpupm;



	/*
	 * 1. Use of P-states and C1:
	 *
	 * If time to full capacity (ttfc) is greater than the time
	 * it takes to change CPU P-states, then we can enable
	 * their use by the Dispatcher (via the cpupm code).
	 */
	if (ttfc >= pm_pstate_latency_val) {
		cpupm = PM_CPUPM_EVENT;
		if (cpupm_set_policy(CPUPM_POLICY_ELASTIC) < 0) {
			PMD(PMD_IOCTL, ("%s failed to enable PAD\n", cmdstr))
			cpupm = prev_cpupm;
			ret = EINVAL;
		}
	} else {
		cpupm = PM_CPUPM_DISABLE;
		if (cpupm_set_policy(CPUPM_POLICY_DISABLED) < 0) {
			PMD(PMD_IOCTL, ("%s failed to enable PAD\n", cmdstr))
			cpupm = prev_cpupm;
			ret = EINVAL;
		}
	...
	/*
	 * 2. Use of "deep" C-states:
	 *
	 * For now, we just check to see when the time to full capacity
	 * is greater than a particular responsiveness value,
	 * and we enable the use of deep C-states (C2 and C3) if so.
	 *
	 * If ttfc is less than that, we disable the use of deep
	 * C-states
	 */

	if (ttfc >= pm_cpu_deep_idle_latency_val) {
		if (callb_execute_class(CB_CL_CPU_DEEP_IDLE,
		    PM_ENABLE_CPU_DEEP_IDLE) == NULL)
			ret = 0;
		else
...
static pm_authority_t
pm_get_auth(const char *authstr)
{
	int		i;
	int		nel;
	...
	nel = sizeof (pm_authmap) / sizeof (struct pm_authmap_s);
	for (i = 0; i < nel && result == PM_AUTHORITY_INVALID; i++) {
		if (strncmp(authstr, pm_authmap[i].a_name,
		    strlen(pm_authmap[i].a_name)) == 0) {
			result = pm_authmap[i].a_auth;
		}
	}
...
static const char *
pm_get_authstr(pm_authority_t auth)
{
	int		i;
	int		nel;
	const char	*result;
	...
	nel = sizeof (pm_authmap) / sizeof (struct pm_authmap_s);
	result = NULL;
	for (i = 0; i < nel && result == NULL; i++) {
		if (auth == pm_authmap[i].a_auth) {
			result = pm_authmap[i].a_name;
		}
	}
...
static	int
pm_apply_suspend(boolean_t pm_se)
{
	int	ret;

#ifdef	PMDDEBUG
	char	*cmdstr = "APPLY_SUSPEND";
#endif	/*	PMDDEBUG	*/

	PMD(PMD_IOCTL, ("%s called\n", cmdstr))

	/*
	 * Always try to enable or disable suspend.  It isn't often
	 * called, and there could have been transient reasons as
	 * to why the call may have previously failed (though the
	 * current implementation of pm_apply_suspend_disable()
	 * always returns 0).
	 */


	if (pm_se == B_TRUE) {
		PMD(PMD_IOCTL, ("%s pm_se B_TRUE\n", cmdstr))
		ret = pm_apply_suspend_enable();
		if ((ret == 0) || (ret == EBUSY)) {
			/*
			 * Everything was OK, set local value to B_TRUE,
			 * and return.
			 * Note that EBUSY indicates we have previously
			 * enabled suspend.
			 */
			pm_kernel_config.pm_suspend_enabled = B_TRUE;
			PMD(PMD_IOCTL,
			    ("%s pm_kernel_config.pm_suspend_enabled = %d\n",
			    cmdstr, pm_kernel_config.pm_suspend_enabled))
			return (0);
		}
	...
	/*
	 * Either pm_se is B_FALSE, or pm_apply_suspend_enable()
	 * above failed. In any case call pm_apply_suspend_disable()
	 * and set pm_suspend_enabled to B_FALSE
	 */
	if (pm_se == B_FALSE)
		PMD(PMD_IOCTL, ("%s pm_se B_FALSE\n", cmdstr))

	/*
	 * Note that pm_apply_suspend_disable() always succeeds.
	 * We ignore the return value here, but it is used else where
	 */
	(void) pm_apply_suspend_disable();
	pm_kernel_config.pm_suspend_enabled =  B_FALSE;

	PMD(PMD_IOCTL, ("%s ret = %d\n", cmdstr, ret))

	PMD(PMD_IOCTL, ("%s pm_kernel_config.pm_suspend_enabled = %d\n",
	    cmdstr, pm_kernel_config.pm_suspend_enabled))
...
static	int
pm_apply_suspend_enable(void)
{

#ifdef	PMDDEBUG
	char	*cmdstr = "APPLY_SUSPEND_ENABLE";
#endif	/*	PMDDEBUG	*/

	int	i;

	/*
	 * If this platform is specifically denied the ability to suspend,
	...
	mutex_enter(&pm_scan_lock);
	/*
	 * Don't enable suspend on platforms that are unsupported
	 * (must be one suspend state in the pm_syscap array).
	 */
	for (i = SYSTEM_POWER_S4; i > 0; i--)
		if (pm_syscap[i] != NULL) {
			break;
	}
	/*
	 * Set the "default" state to the highest level. Do this before
	 * checking for SYSTEM_POWER_S0 to ensure this is properly set.
	 */
	pm_cpr_state = i;

	/*
	 * If the only supported state is S0, suspend is not supported.
	 */
	if (SYSTEM_POWER_S0 == i) {
		mutex_exit(&pm_scan_lock);
		return (ENOTSUP);
	}

	/* if suspend already enabled, return EBUSY */
	if (pm_suspend_enabled) {
		mutex_exit(&pm_scan_lock);
		PMD(PMD_ERROR, ("ioctl: %s: EBUSY\n", cmdstr))
		return (EBUSY);
	}
	pm_suspend_enabled = 1;
	mutex_exit(&pm_scan_lock);
...
static	int
pm_apply_smf_props(nvlist_t *list)
{

#ifdef	PMDDEBUG
	char		*cmdstr = "APPLY_SMF_PROPS";
#endif	/*	PMDDEBUG	*/

	int		ret = 0;
	int		err = 0;
	int		nprops = 0;
	uint64_t	pm_smf_ttfc = 0;
	uint64_t	pm_smf_ttmr = 0;

	PMD(PMD_IOCTL, ("%s pm_smf_ttfc #1 %ld\n", cmdstr, pm_smf_ttfc))

	err = nvlist_lookup_uint64(list, PM_PROP_TTFC, &pm_smf_ttfc);
	if (err == 0)
		nprops++;
	else
		PMD(PMD_IOCTL, ("%s %s not found\n", cmdstr, PM_PROP_TTFC))

	PMD(PMD_IOCTL, ("%s pm_smf_ttfc #2 %ld\n", cmdstr, pm_smf_ttfc))

	err =  nvlist_lookup_uint64(list, PM_PROP_TTMR, &pm_smf_ttmr);
	if (err == 0)
		nprops++;
	else
		PMD(PMD_IOCTL, ("%s %s not found\n", cmdstr, PM_PROP_TTMR))
	/*
	 * we did not find ALL matches, return ENOENT
	 */
	PMD(PMD_IOCTL, ("%s nprops set to %d\n", cmdstr, nprops))
	if (nprops != NO_DEPENDENT_PROPS)
		return (ENOENT);

	/*
	 * now that we have all the required SMF properties, transmit these
	 * values to each of the resource-management subsystems
	 */
	ret =  pm_apply_props(pm_smf_ttfc, pm_smf_ttmr);

	if (ret != 0)
		return (ret);

	/*
	 * finally make the SMF properties current
	 */
	pm_kernel_config.pm_curr_ttfc = pm_smf_ttfc;
	pm_kernel_config.pm_curr_ttmr = pm_smf_ttmr;
	pm_kernel_config.pm_curr_disabled = B_FALSE;

	PMD(PMD_IOCTL, ("%s returning 0\n", cmdstr))

	return (0);
}

static	int
pm_apply_plat_props(void)
{

#ifdef	PMDDEBUG
	char	*cmdstr = "APPLY_PLAT_PROPS";
#endif	/*	PMDDEBUG	*/

	int	ret;

	/*
	 * pm_auth == "Platform" means that we
	 * must now transmit the platform values of ttfc and ttmr
	 * to the various resource management subsystems
	 *
	 */
	ret = pm_apply_props(pm_kernel_config.pm_plat_ttfc,
	    pm_kernel_config.pm_plat_ttmr);
	...
	/*
	 * finally make the platform properties current
	 */
	pm_kernel_config.pm_curr_ttfc = pm_kernel_config.pm_plat_ttfc;
	pm_kernel_config.pm_curr_ttmr = pm_kernel_config.pm_plat_ttmr;
	pm_kernel_config.pm_curr_disabled = B_FALSE;
...
static	int
pm_apply_all_props(nvlist_t *list)
{
	int	ret = 0;

	/*
	 * apply the properties that are independent of admin athority
	 */
	(void) pm_apply_independent_props(list);

	/*
	 * apply the properties that are dependent upon admin athority
	 */
	ret = pm_apply_dependent_props(list);

	return (ret);
}

/*
 * currently there is only one independent property ie suspend_enable
 * that is a, propertiy that is always set independent of the
 * adininstrative_authority
 *
 * this function return 0 if all the independ properties have been found
 * and applied
 */
static	int
pm_apply_independent_props(nvlist_t *list)
{

#ifdef	PMDDEBUG
	char		*cmdstr = "APPLY_INDEPENDENT_PROPS";
#endif	/*	PMDDEBUG	*/
	...
	/*
	 * All properties are sent using just the property name with no
	 * authority decoration.  They are ALL assumed to come from SMF.
	 */
	ret =  nvlist_lookup_boolean_value(list, PM_PROP_SUSPEND_ENABLE,
	    &pm_se);

	if (ret != 0) {
		PMD(PMD_IOCTL, ("%s %s not found\n", cmdstr,
		    PM_PROP_SUSPEND_ENABLE))
		return (ret);
	}

	ret = pm_apply_suspend(pm_se);

	return (ret);
}

static	int
pm_apply_dependent_props(nvlist_t *list)
{

#ifdef	PMDDEBUG
	char	*cmdstr = "APPLY_DEPENDENT_PROPS";
#endif	/*	PMDDEBUG	*/
	 ...
	 * authority decoration.  They are ALL assumed to come from SMF.
	 */
	ret = nvlist_lookup_string(list, PM_PROP_AUTHORITY, &pm_aa);
	if (ret != 0) {
		PMD(PMD_IOCTL, ("%s %s not found\n", cmdstr,
		    PM_PROP_AUTHORITY))
		...
		PMD(PMD_IOCTL, ("%s pm_aa = %s, %d\n",
		    cmdstr, pm_aa, pm_kernel_config.pm_auth))

		/* Convert the given string to an authority value */
		pm_kernel_config.pm_auth = pm_get_auth(pm_aa);
		switch (pm_kernel_config.pm_auth) {
		case PM_AUTHORITY_PLATFORM:
			PMD(PMD_IOCTL, ("%s pm_aa = %s, %d\n",
			    cmdstr, pm_aa, PM_AUTHORITY_PLATFORM))
			ret = pm_apply_plat_props();
			break;

		case PM_AUTHORITY_SMF:
			PMD(PMD_IOCTL, ("%s pm_aa = %s, %d\n",
			    cmdstr, pm_aa, PM_AUTHORITY_SMF))
			ret = pm_apply_smf_props(list);
			break;

		case PM_AUTHORITY_NONE:
			PMD(PMD_IOCTL, ("%s pm_aa = %s, %d\n",
			    cmdstr, pm_aa, PM_AUTHORITY_NONE))
			/*
			 * pm_auth == "None" means that we
			 * now disable power management
			 */

			ret = pm_disable_power_management();
			break;

		default:
			PMD(PMD_IOCTL, ("%s returned EINVAL\n", cmdstr))
			ret = EINVAL;
			break;
		}
	}

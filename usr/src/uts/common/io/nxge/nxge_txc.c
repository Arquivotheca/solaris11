...
static nxge_status_t nxge_txc_handle_port_errors(p_nxge_t, uint32_t);
static void nxge_txc_inject_port_err(uint8_t, txc_int_stat_dbg_t *,
    uint8_t istats);
...
nxge_txc_init_fail:
	NXGE_ERROR_MSG((nxgep, NXGE_ERR_CTL,
	    "nxge_txc_init: Failed to initialize txc on port %d", port));
 ...
 */
nxge_status_t
nxge_txc_tdc_bind(p_nxge_t nxgep, int channel)
{
	uint8_t		port;
	uint64_t	bitmap;
	npi_handle_t	handle;
	...
	TXC_FZC_CNTL_REG_READ64(handle, TXC_PORT_DMA_ENABLE_REG, port, &bitmap);
 ...
 */
nxge_status_t
nxge_txc_tdc_unbind(p_nxge_t nxgep, int channel)
{
	uint8_t		port;
	uint64_t	bitmap;
	npi_handle_t	handle;

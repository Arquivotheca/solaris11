...
void
nxge_guest_regs_map_free(nxge_t *nxge)
{
 ...
 * -------------------------------------------------------------
 */
static nxge_hio_dc_t *nxge_guest_dc_alloc(nxge_t *, nxge_hio_vr_t *,
    nxge_grp_type_t);
static void res_map_parse(nxge_t *, nxge_grp_type_t, uint64_t);
static void nxge_check_guest_state(nxge_hio_vr_t *);
static void nxge_guest_dc_free(nxge_t *, nxge_hio_vr_t *, nxge_grp_type_t);
...
nxge_hio_vr_add(nxge_t *nxge)
{
	...
	vr->size = vr_size;
	vr->region = vr_index;
	vr->rx_group.type = NXGE_VR_GROUP_RX;
	vr->tx_group.type = NXGE_VR_GROUP_TX;
	...
	if (nxge_mac_register(nxge) != NXGE_OK) {
...
nxge_hio_vr_add_exit:
	nxge_guest_dc_free(nxge, vr, NXGE_TRANSMIT_GROUP);
	nxge_guest_dc_free(nxge, vr, NXGE_RECEIVE_GROUP);
 ...
 */
nxge_hio_dc_t *
nxge_guest_dc_alloc(nxge_t *nxge, nxge_hio_vr_t *vr, nxge_grp_type_t type)
{
	nxge_hio_data_t *nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
	nxge_hio_dc_t *dc;
	int limit, i;
...
static void
nxge_guest_dc_free(nxge_t *nxge, nxge_hio_vr_t *vr, nxge_grp_type_t type)
{
	nxge_hio_data_t *nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
	nxge_hio_dc_t *dc;
	int limit, i;
 ...
 */
void
res_map_parse(nxge_t *nxge, nxge_grp_type_t type, uint64_t res_map)
{
...
nxge_hio_vr_release(nxge_t *nxge)
{
	 ...
	 * Clean up.
	 */
	nxge_guest_dc_free(nxge, nxge->hio_vr, NXGE_TRANSMIT_GROUP);
	nxge_guest_dc_free(nxge, nxge->hio_vr, NXGE_RECEIVE_GROUP);

	MUTEX_ENTER(&nhd->lock);
	nxge->hio_vr->rx_group.type = NXGE_NO_GROUP_TYPE;
	nxge->hio_vr->tx_group.type = NXGE_NO_GROUP_TYPE;
	for (vr_index = 0; vr_index < FUNC_VIR_MAX; vr_index++) {
 ...
 */
nxge_status_t
nxge_tdc_lp_conf(p_nxge_t nxge, int channel, boolean_t configure)
{
	nxge_hio_dc_t		*dc;
	...
	if (configure) {
		hv_rv = hv_niu_vrtx_logical_page_conf(dc->cookie,
		    (uint64_t)channel, 0, ring->hv_tx_buf_base_ioaddr_pp,
		    ring->hv_tx_buf_ioaddr_size);
	} else {
		hv_rv = hv_niu_vrtx_logical_page_conf(dc->cookie,
		    (uint64_t)channel, 0, 0, 0);
	}
	...
	NXGE_DEBUG_MSG((nxge, HIO_CTL,
	    "==> nxge_tdc_lp_conf: channel %d (page 0 data buf) hv_rv 0x%llx "
	...
	if (configure) {
		...
		hv_rv = hv_niu_vrtx_logical_page_conf(dc->cookie,
		    (uint64_t)channel, (uint64_t)1, 0, 0);
	}
 ...
 */
nxge_status_t
nxge_rdc_lp_conf(p_nxge_t nxge, int channel, boolean_t configure)
{
	nxge_hio_dc_t		*dc;
	nxge_dma_common_t	*data;
	nxge_dma_common_t	*control;
	...
	if (configure) {
		hv_rv = hv_niu_vrrx_logical_page_conf(dc->cookie,
		    (uint64_t)channel, 0, ring->hv_rx_buf_base_ioaddr_pp,
 ...
 */
void
nxge_hio_start_timer(nxge_t *nxge)
{
	nxge_hio_data_t	*nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
	nxge_hio_vr_t	*vr;
	int		region;
 ...
 */
void
nxge_check_guest_state(nxge_hio_vr_t *vr)
{
...
nxge_status_t
nxge_hio_rdc_intr_arm(p_nxge_t nxge, uint32_t channel, boolean_t arm)
{
	nxge_grp_t	*group;
	nxge_hio_dc_t	*dc;
	nxge_ldg_t	*ldgp;
...
nxge_status_t
nxge_hio_rdc_enable(p_nxge_t nxge, uint32_t channel)
{
	nxge_grp_t	*group;
	npi_handle_t	handle;
	npi_status_t	rval;

...
/*
 * For now: we hard coded the DMA configurations and assume for one
 * partition only.
 *
...
{
	nxge_status_t	status = NXGE_OK;
	int		ddi_status, num_ports = nxgep->nports;
	int		port, bits, j;
	...
	case FAIR:
		...
		for (port = 0; port < num_ports; port++) {
			custom_num_grp[port] = (num_ports == 4) ?
			    p4_rdcgrp_fair[port] : p2_rdcgrp_fair[port];
	...
	case EQUAL:
		...
		for (port = 0; port < num_ports; port++) {
			custom_num_grp[port] = (num_ports == 4) ?
			    p4_rdcgrp_equal[port] : p2_rdcgrp_equal[port];
			custom_start_grp[port] = start_grp;
			start_grp += custom_num_grp[port];
		}
		break;
	...
	case CUSTOM:
		...
		for (port = 0; port < num_ports; port++) {
			...
			if ((custom_start_grp[port] == -1) ||
			    (custom_start_grp[port] >= NXGE_MAX_RDC_GRPS)) {
				bad_config = B_TRUE;
				break;
			}
			custom_num_grp[port] =
			    ddi_prop_get_int(DDI_DEV_T_NONE, s_dip[port],
			    DDI_PROP_DONTPASS, num_prop, -1);

			if ((custom_num_grp[port] == -1) ||
			    (custom_num_grp[port] > NXGE_MAX_RDC_GRPS) ||
			    ((custom_num_grp[port] + custom_start_grp[port]) >=
			    NXGE_MAX_RDC_GRPS)) {
				bad_config = B_TRUE;
				break;
			}
			...
			grp_bitmap[port] = 0;
			for (bits = 0; bits < custom_num_grp[port]; bits++) {
				grp_bitmap[port] |=
		...
		if (bad_config == B_FALSE) {
			...
			for (port = 0; port < num_ports - 1; port++) {
				for (j = port + 1; j < num_ports; j++) {
					if (grp_bitmap[port] & grp_bitmap[j]) {
						bad_config = B_TRUE;
						break;
					}
				}
	...
	for (port = 0; port < num_ports; port++) {
		...
		if (ddi_status != DDI_PROP_SUCCESS) {
			NXGE_ERROR_MSG((nxgep, NXGE_ERR_CTL,
			    " property %s not updating", cfg_prop));
		...
		if (ddi_status != DDI_PROP_SUCCESS) {
			NXGE_ERROR_MSG((nxgep, NXGE_ERR_CTL,
			    " property %s not updating", num_prop));
			status |= NXGE_DDI_FAILED;
		}
		ddi_status = ddi_prop_update_int(DDI_DEV_T_NONE, s_dip[port],
		...
		if (ddi_status != DDI_PROP_SUCCESS) {
			NXGE_ERROR_MSG((nxgep, NXGE_ERR_CTL,
			    " property %s not updating", start_prop));
			status |= NXGE_DDI_FAILED;
		}
	}
    ...
    dev_info_t *s_dip[])
{
	...
	case EQUAL:
		...
		for (port = 0; port < num_ports; port++) {
			custom_num_rdc[port] = (num_ports == 4) ?
			    p4_rx_equal[port] : p2_rx_equal[port];
			custom_start_rdc[port] = start_rdc;
			start_rdc += custom_num_rdc[port];
		}
		break;
	...
	case CUSTOM:
		...
		num_rdc = 0;
		for (port = 0; port < num_ports; port++) {
			ddi_status = ddi_prop_lookup_int_array(DDI_DEV_T_ANY,
			    s_dip[port], 0, start_rdc_prop, &prop_val,
			    &prop_len);
			...
			if ((custom_start_rdc[port] == -1) ||
			    (custom_start_rdc[port] >= NXGE_MAX_RDCS)) {
				NXGE_DEBUG_MSG((nxgep, CFG_CTL,
				    " %s custom start %d out of range %x ",
				    " rxdma-cfg",
				    port, custom_start_rdc[port]));
				bad_config = B_TRUE;
				break;
			}
			ddi_status =
			    ddi_prop_lookup_int_array(DDI_DEV_T_ANY,
			    s_dip[port], 0, num_rdc_prop, &prop_val, &prop_len);
			if (ddi_status == DDI_SUCCESS)
				custom_num_rdc[port] = *prop_val;
			else {
				NXGE_DEBUG_MSG((nxgep, CFG_CTL,
				    " %s custom num port %d read failed ",
				    "rxdma-cfg", port));
				bad_config = B_TRUE;
				status |= NXGE_DDI_FAILED;
			}
			...
			if ((custom_num_rdc[port] == -1) ||
			    (custom_num_rdc[port] > NXGE_MAX_RDCS) ||
			    ((custom_num_rdc[port] + custom_start_rdc[port]) >
			    NXGE_MAX_RDCS)) {
				NXGE_DEBUG_MSG((nxgep, CFG_CTL,
				    " %s custom num %d out of range %x ",
				    " rxdma-cfg", port, custom_num_rdc[port]));
				bad_config = B_TRUE;
				break;
			}
...
{
	nxge_status_t	status = NXGE_OK;
	int		ddi_status = DDI_SUCCESS, num_ports = nxgep->nports;
	int		port, bits, j;
	...
	case CUSTOM:
		...
		for (port = 0; port < num_ports; port++) {
			...
			if ((custom_num_tdc[port] == -1) ||
			    (custom_num_tdc[port] > NXGE_MAX_TDCS) ||
			    ((custom_num_tdc[port] + custom_start_tdc[port]) >
			    NXGE_MAX_TDCS)) {
				NXGE_DEBUG_MSG((nxgep, CFG_CTL,
				    " %s custom num %d"
    ...
    uint64_t known_cfg, uint32_t override, dev_info_t *c_dip[])
{
	...
	for (i = 0; i < nxgep->nports; i++) {
		...
		if (ddi_prop_lookup_int_array(DDI_DEV_T_ANY, c_dip[i], 0,
		    prop, &cfg_val, &prop_len) == DDI_PROP_SUCCESS) {
			cfg_value[i] = *cfg_val;
...
nxge_class_get_known_cfg(p_nxge_t nxgep, int class_prop, int rx_quick_cfg)
{
	...
	case CFG_L3_DISTRIBUTE:
		cfg_value = nxge_classify_get_cfg_value(nxgep, rx_quick_cfg,
...
nxge_get_config_properties(p_nxge_t nxgep)
{
	...
	default:
		...
		MUTEX_ENTER(&hw_p->nxge_cfg_lock);
		if ((hw_p->flags & COMMON_CFG_VALID) != COMMON_CFG_VALID) {
			status = nxge_cfg_verify_set(nxgep, COMMON_TXDMA_CFG);
			status |= nxge_cfg_verify_set(nxgep, COMMON_RXDMA_CFG);
			status |= nxge_cfg_verify_set(nxgep,
			    COMMON_RXDMA_GRP_CFG);
			status |= nxge_cfg_verify_set(nxgep, COMMON_CLASS_CFG);
			if (status != NXGE_OK)
				return (NXGE_ERROR);
...
nxge_use_default_dma_config_n2(p_nxge_t nxgep)
{
	...
	if (ddi_prop_lookup_int_array(DDI_DEV_T_ANY, nxgep->dip, 0,
	    "tx-dma-channels", (int **)&prop_val, &prop_len) ==
	    DDI_PROP_SUCCESS) {
	...
	if (nxge_fzc_rdc_tbl_bind(nxgep,
	    p_cfgp->def_mac_rxdma_grpid, B_TRUE) < 0) {
...
nxge_set_hw_mac_class_config(p_nxge_t nxgep)
{
	...
	char			*prop;
	p_nxge_class_pt_cfg_t	p_class_cfgp;
	int			good_count = 0, good_cfg[NXGE_MAX_MACS];
...
{
	int			i, maxldvs, maxldgs, nldvs, ldv, endldg;
	uint8_t			func, channel, chn_start;
	...
	NXGE_DEBUG_MSG((nxgep, INT_CTL,
	    "==> nxge_ldgv_init_n2: maxldvs %d maxldgs %d", maxldvs, maxldgs));
	...
	for (i = 0; i < maxldgs; i++) {
		...
		NXGE_DEBUG_MSG((nxgep, INT_CTL,
		    ...
		    "ldg %d ldgptr $%p", maxldvs, maxldgs, ptr->ldg, ptr));
		ptr++;
	}
	...
	*nrequired_p = 0;
	...
	if (own_sys_err && p_cfgp->ser_ldvid) {
		...
		(void) nxge_fzc_sys_err_mask_set(nxgep, SYS_ERR_SMX_MASK |
		    SYS_ERR_IPP_MASK | SYS_ERR_TXC_MASK | SYS_ERR_ZCP_MASK);
...
{
	int			i, maxldvs, maxldgs, nldvs, ldv, ldg;
	int			endldg, ngrps;
	uint8_t			func, channel;
	boolean_t		own_sys_err = B_FALSE, own_fzc = B_FALSE;
	p_nxge_dma_pt_cfg_t	p_dma_cfgp;
	p_nxge_hw_pt_cfg_t	p_cfgp;
	p_nxge_ldgv_t		ldgvp;
...
nxge_intr_mask_mgmt(p_nxge_t nxgep)
{
	...
	if (nxgep->niu_type != N2_NIU) {
		...
		for (i = 0; i < ldgvp->ldg_intrs; i++, ldgp++) {
			...
			for (j = 0; j < ldgp->nldvs; j++, ldvp++) {
				...
				NXGE_DEBUG_MSG((nxgep, INT_CTL,
				    "==> nxge_intr_mask_mgmt: set mask OK "
				    " rs 0x%x ldv %d mask 0x%x",
	...
	for (i = 0; i < nxgep->ldgvp->ldg_intrs; i++, ldgp++) {
		...
		if (rs != NPI_SUCCESS) {
			NXGE_ERROR_MSG((nxgep, NXGE_ERR_CTL,
			    "<== nxge_intr_mask_mgmt: "
			    "set timer failed rs 0x%x dg %d timer 0x%x",
			    rs, ldgp->ldg, ldgp->ldg_timer));
			return (NXGE_ERROR | rs);
		}
		NXGE_DEBUG_MSG((nxgep, INT_CTL,
		    "==> nxge_intr_mask_mgmt: set timer OK "
...
nxge_intr_mask_mgmt_set(p_nxge_t nxgep, boolean_t on)
{
	...
	if (nxgep->niu_type == N2_NIU) {
		NXGE_DEBUG_MSG((nxgep, INT_CTL,
		    "<== nxge_intr_mask_mgmt_set (%d) not set (N2/NIU)", on));
		return (NXGE_ERROR);
	}
	...
	for (i = 0; i < ldgvp->ldg_intrs; i++, ldgp++) {
		...
		for (j = 0; j < ldgp->nldvs; j++, ldvp++) {
			...
			NXGE_DEBUG_MSG((nxgep, INT_CTL,
			    "==> nxge_intr_mask_mgmt_set: flag %d set mask OK "
			    " ldv %d mask 0x%x",
...
nxge_get_mac_addr_properties(p_nxge_t nxgep)
{
	...
	if (!nxge_is_valid_local_mac(nxgep->factaddr)) {
		...
		if (!nxge_is_valid_local_mac(nxgep->factaddr)) {
			...
			NXGE_ERROR_MSG((nxgep, NXGE_ERR_CTL,
			    "EEPROM version [%s] invalid...please update",
			    nxgep->vpd_info.ver));
			return (NXGE_ERROR);
		}
	}
    ...
    uint8_t endldg, int *ngrps)
{
	...
	if ((*ldgp)->ldg == (endldg - 1)) {
		...
		NXGE_DEBUG_MSG((NULL, INT_CTL,
		    "==> nxge_ldgv_setup: ldvp $%p ngrps %d", *ldvp, *ngrps));
		++*ldvp;
	} else {
		(*ldgp)->ldvp = *ldvp;
		*ngrps += 1;

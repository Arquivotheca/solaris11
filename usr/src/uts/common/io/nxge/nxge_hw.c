...
static void nxge_rtrace_ioctl(p_nxge_t, queue_t *, mblk_t *, struct iocblk *);
...
nxge_intr(void *arg1, void *arg2)
{
	...
	if (!(nxgep->drv_state & STATE_HW_INITIALIZED)) {
		cmn_err(CE_CONT, "nxge(%d): shared interrupt not ready\n",
		    nxgep->instance);
		goto nxge_intr_rearm;
	}
	...
	for (i = 0; i < nintrs; i++, t_ldgp++) {
		...
		/*
		 * Find the logical devices that need to be serviced.
		 */
		for (j = 0; j < nldvs; j++, t_ldvp++) {
			ldv = t_ldvp->ldv;

			if (((ldv < NXGE_MAC_LD_START) &&
			    (LDV_ON(ldv, vector0) | (LDV_ON(ldv, vector1)))) ||
			    (ldv >= NXGE_MAC_LD_START &&
			    ((LDV2_ON_1(ldv, vector2)) ||
			    (LDV2_ON_2(ldv, vector2))))) {
				/*
				 * Call the interrupt handler for one of
				 * the logical devices sharing this
				 * interrupt.
				 */
...
nxge_intr_hw_enable(p_nxge_t nxgep)
{
	...
	if (nxgep->niu_type == N2_NIU)
		(void) nxge_intr_mask_mgmt(nxgep);
	NXGE_DEBUG_MSG((nxgep, INT_CTL, "<== nxge_intr_hw_enable"));
...
nxge_set_lb(p_nxge_t nxgep, queue_t *wq, p_mblk_t mp)
{
	...
	if (nxge_lb_dbg) {
		cmn_err(CE_NOTE, "!nxge%d: Adapter now in %s loopback mode",
...
nxge_set_lb_exit:
	NXGE_DEBUG_MSG((nxgep, DDI_CTL, "<== nxge_set_lb status = 0x%08x",
	    status));
	return (status);
nxge_set_lb_err:
	status = B_FALSE;
	cmn_err(CE_NOTE, "!nxge%d: Failed to put adapter in %s loopback mode",
	    nxgep->instance, lb_info->key);
	return (status);
}
...
nxge_put_mii(p_nxge_t nxgep, p_mblk_t mp)
{
	...
	reg = (uint8_t)buf[0];
	NXGE_DEBUG_MSG((nxgep, IOC_CTL,
	    "reg = 0x%08X index = 0x%08X value = 0x%08X", reg, buf[0], buf[1]));
...
nxge_check_hw_state(p_nxge_t nxgep)
{
	...
	if (nxgep->nxge_mac_state == NXGE_MAC_STOPPING)

...
typedef enum xmac_rx_iconfig_e {
	...
	ICFG_XMAC_RX_ALL = (XMAC_RX_FRAME_RCVD | XMAC_RX_OVERFLOW |
		...
		XMAC_RX_LEN_ERR_CNT_EXP | XMAC_RX_VIOL_ERR_CNT_EXP |
		...
		XMAC_RX_ALIGNERR_CNT_EXP | XMAC_RX_LINK_FLT_CNT_EXP |
		XMAC_RX_HST_CNT7_EXP | XMAC_RX_REMOTE_FLT_DET |
		XMAC_RX_LOCAL_FLT_DET)
} xmac_rx_iconfig_t;
 ...
 */
#define	NPI_MAC_PORT_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | PORT_INVALID | IS_PORT | \
	(portn << 12))
#define	NPI_MAC_OPCODE_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | OPCODE_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_MAC_HASHTAB_ENTRY_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | MAC_HASHTAB_ENTRY_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_MAC_HOSTINFO_ENTRY_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | MAC_HOSTINFO_ENTRY_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_MAC_ALT_ADDR_ENTRY_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | MAC_ALT_ADDR_ENTRY_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_MAC_PORT_ATTR_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | MAC_PORT_ATTR_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_MAC_RESET_MODE_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | MAC_RESET_MODE_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_MAC_PCS_REG_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | REGISTER_INVALID | \
	IS_PORT | (portn << 12))
#define	NPI_TXMAC_RESET_FAILED(portn) \
	((TXMAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | RESET_FAILED | IS_PORT | \
	(portn << 12))
#define	NPI_RXMAC_RESET_FAILED(portn) \
	((RXMAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | RESET_FAILED | IS_PORT | \
	(portn << 12))
#define	NPI_MAC_CONFIG_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | CONFIG_INVALID | IS_PORT | \
	(portn << 12))
#define	NPI_MAC_REG_INVALID(portn) \
	((MAC_BLK_ID << NPI_BLOCK_ID_SHIFT) | REGISTER_INVALID | IS_PORT | \
	(portn << 12))
#define	NPI_MAC_MII_READ_FAILED(portn) \
	((MIF_BLK_ID << NPI_BLOCK_ID_SHIFT) | READ_FAILED | IS_PORT | \
	(portn << 12))
#define	NPI_MAC_MII_WRITE_FAILED(portn) \
	((MIF_BLK_ID << NPI_BLOCK_ID_SHIFT) | WRITE_FAILED | IS_PORT | \
	(portn << 12))

/* library functions prototypes */
/* general mac functions */
npi_status_t npi_mac_hashtab_entry(npi_handle_t, io_op_t, uint8_t, uint8_t,
    uint16_t *);
npi_status_t npi_mac_hostinfo_entry(npi_handle_t, io_op_t, uint8_t, uint8_t,
    hostinfo_t *);
npi_status_t npi_mac_altaddr_enable(npi_handle_t, uint8_t, uint8_t);
npi_status_t npi_mac_altaddr_disable(npi_handle_t, uint8_t, uint8_t);
npi_status_t npi_mac_altaddr_entry(npi_handle_t, io_op_t, uint8_t,
    uint8_t, npi_mac_addr_t *);
npi_status_t npi_mac_port_attr(npi_handle_t, io_op_t, uint8_t, npi_attr_t *);
npi_status_t npi_mac_get_link_status(npi_handle_t, uint8_t, boolean_t *);
npi_status_t npi_mac_get_10g_link_status(npi_handle_t, uint8_t, boolean_t *);
npi_status_t npi_mac_mif_mii_read(npi_handle_t, uint8_t, uint8_t, uint16_t *);
npi_status_t npi_mac_mif_mii_write(npi_handle_t, uint8_t, uint8_t, uint16_t);
npi_status_t npi_mac_mif_link_intr_enable(npi_handle_t, uint8_t,
    uint8_t, uint16_t);
npi_status_t npi_mac_mif_mdio_read(npi_handle_t, uint8_t, uint8_t, uint16_t,
    uint16_t *);
npi_status_t npi_mac_mif_mdio_write(npi_handle_t, uint8_t, uint8_t, uint16_t,
    uint16_t);
npi_status_t npi_mac_mif_mdio_link_intr_enable(npi_handle_t, uint8_t, uint8_t,
    uint16_t, uint16_t);
npi_status_t npi_mac_mif_link_intr_disable(npi_handle_t, uint8_t);
npi_status_t npi_mac_pcs_mii_read(npi_handle_t, uint8_t, uint8_t, uint16_t *);
npi_status_t npi_mac_pcs_mii_write(npi_handle_t, uint8_t, uint8_t, uint16_t);
 ...
 * xmac functions
 */
npi_status_t npi_xmac_reset(npi_handle_t, uint8_t, npi_mac_reset_t);
npi_status_t npi_xmac_xif_config(npi_handle_t, config_op_t, uint8_t,
    xmac_xif_config_t);
npi_status_t npi_xmac_tx_config(npi_handle_t, config_op_t, uint8_t,
    xmac_tx_config_t);
npi_status_t npi_xmac_rx_config(npi_handle_t, config_op_t, uint8_t,
    xmac_rx_config_t);
npi_status_t npi_xmac_tx_iconfig(npi_handle_t, config_op_t, uint8_t,
    xmac_tx_iconfig_t);
npi_status_t npi_xmac_rx_iconfig(npi_handle_t, config_op_t, uint8_t,
    xmac_rx_iconfig_t);
npi_status_t npi_xmac_ctl_iconfig(npi_handle_t, config_op_t, uint8_t,
    xmac_ctl_iconfig_t);
...
npi_status_t npi_xmac_xpcs_read(npi_handle_t, uint8_t, uint8_t, uint32_t *);
npi_status_t npi_xmac_xpcs_write(npi_handle_t, uint8_t, uint8_t, uint32_t);
...
npi_status_t npi_xmac_xif_led(npi_handle_t, uint8_t, boolean_t);
npi_status_t npi_xmac_zap_tx_counters(npi_handle_t, uint8_t);
npi_status_t npi_xmac_zap_rx_counters(npi_handle_t, uint8_t);

/* bmac functions */
npi_status_t npi_bmac_reset(npi_handle_t, uint8_t, npi_mac_reset_t mode);
npi_status_t npi_bmac_tx_config(npi_handle_t, config_op_t, uint8_t,
    bmac_tx_config_t);
npi_status_t npi_bmac_rx_config(npi_handle_t, config_op_t, uint8_t,
    bmac_rx_config_t);
npi_status_t npi_bmac_rx_iconfig(npi_handle_t, config_op_t, uint8_t,
    bmac_rx_iconfig_t);
npi_status_t npi_bmac_xif_config(npi_handle_t, config_op_t, uint8_t,
    bmac_xif_config_t);
npi_status_t npi_bmac_tx_iconfig(npi_handle_t, config_op_t, uint8_t,
    bmac_tx_iconfig_t);
npi_status_t npi_bmac_ctl_iconfig(npi_handle_t, config_op_t, uint8_t,
    bmac_ctl_iconfig_t);
...
npi_status_t npi_bmac_ctl_get_istatus(npi_handle_t, uint8_t,
    bmac_ctl_iconfig_t *);
npi_status_t npi_bmac_send_pause(npi_handle_t, uint8_t, uint16_t);

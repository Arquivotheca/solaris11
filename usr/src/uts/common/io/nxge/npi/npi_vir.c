...
npi_vir_dump_pio_fzc_regs_one(npi_handle_t handle)
{
	...
	for (i = 0; i < num_regs; i++) {
		...
		NPI_REG_DUMP_MSG((handle.function, NPI_REG_CTL, "0x%08llx "
		    "%s\t 0x%08llx \n", fzc_pio_offset[i], fzc_pio_name[i],
		    value));
	}
	...
	for (ldv = 0; ldv < NXGE_INT_MAX_LDS; ldv++) {
		for (i = 0; i < num_regs; i++) {
			...
			NPI_REG_DUMP_MSG((handle.function, NPI_REG_CTL,
			    ...
			    "%s\t %d\n", ldv, offset, fzc_pio_ldgnum_name[i],
			    value));
		}
	}
...
npi_dev_func_sr_init(npi_handle_t handle)
{
	...
	if (!sr.bits.ldw.tas) {
		...
		NPI_DEBUG_MSG((handle.function, NPI_VIR_CTL,
		    " npi_dev_func_sr_init sr <0x%x>", sr.bits.ldw.sr));
	} else {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_dev_func_sr_init tas busy <0x%x>", sr.bits.ldw));
	...
	if (!sr.bits.ldw.tas) {
		...
		case NPI_DEV_SR_LOCK_ST_FREE:
			...
			sr.bits.ldw.sr |= (NPI_DEV_SR_LOCK_ST_BUSY |
			    (sr.bits.ldw.funcid << NPI_DEV_SR_LOCK_FID_SHIFT));
			NXGE_REG_WR64(handle, DEV_FUNC_SR_REG, sr.value);
			break;

		case NPI_DEV_SR_LOCK_ST_RESET:
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " npi_dev_func_sr_lock_enter reset state <0x%x>",
			    sr.bits.ldw.sr));
		...
		case NPI_DEV_SR_LOCK_ST_BUSY:
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " npi_dev_func_sr_lock_enter busy <0x%x>",
			    sr.bits.ldw.sr));
		...
		default:
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " npi_dev_func_sr_lock_enter invalid state",
			    sr.bits.ldw.sr));
		...
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_dev_func_sr_lock_enter tas busy", sr.bits.ldw));
		status = NPI_VIR_TAS_BUSY(sr.bits.ldw.funcid);
	}
...
npi_dev_func_sr_lock_free(npi_handle_t handle)
{
	...
	if (sr.bits.ldw.tas) {
		if (sr.bits.ldw.funcid == NPI_GET_LOCK_OWNER(sr.bits.ldw.sr)) {
			...
			NPI_DEBUG_MSG((handle.function, NPI_VIR_CTL,
			    " npi_dev_func_sr_lock_free not owner <0x%x>",
			    sr.bits.ldw.sr));
			status = NPI_VIR_SR_NOTOWNER(sr.bits.ldw.funcid);
		}
	} else {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_dev_func_sr_lock_free invalid tas state <0x%x>",
		    sr.bits.ldw.tas));
...
npi_status_t
npi_fzc_dma_bind_get(npi_handle_t handle, fzc_dma_bind_t dma_bind,
    uint64_t *pValue)
{
...
npi_fzc_ldg_num_set(npi_handle_t handle, uint8_t ld, uint8_t ldg)
{
	...
	if (!LD_VALID(ld)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fzc_ldg_num_set ld <0x%x>", ld));
	...
	if (!LDG_VALID(ldg)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fzc_ldg_num_set ldg <0x%x>", ldg));
    ...
    uint64_t *ldf_p)
{
	...
	if (!LDG_VALID(ldg)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_ldsv_get Invalid Input ldg <0x%x>", ldg));
	...
	default:
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_ldsv_get Invalid Input:  ldsv type <0x%x>", vector));
    ...
    ldsv_type_t vector, ldf_type_t ldf_type, boolean_t *flag_p)
{
	...
	if (!LDG_VALID(ldg)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_ldsv_ld_get Invalid Input: ldg <0x%x>", ldg));
		return (NPI_FAILURE | NPI_VIR_LDG_INVALID(ldg));
	}
	...
	if (!LD_VALID(ld)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_ldsv_ld_get Invalid Input: ld <9x%x>", ld));
		return (NPI_FAILURE | NPI_VIR_LD_INVALID(ld));
	} else if (vector == VECTOR2 && ld < NXGE_MAC_LD_START) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_ldsv_ld_get Invalid Input: ld-vector2 <0x%x>", ld));
		return (NPI_FAILURE | NPI_VIR_LD_INVALID(ld));
	}
...
npi_fzc_sid_set(npi_handle_t handle, fzc_sid_t sid)
{
	...
	if (!LDG_VALID(sid.ldg)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fzc_sid_set Invalid Input: ldg <0x%x>", sid.ldg));
	...
	if (!sid.niu) {
		...
		if (!FUNC_VALID(sid.func)) {
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " npi_fzc_sid_set Invalid Input: func <0x%x>",
			    sid.func));
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		...
		if (!SID_VECTOR_VALID(sid.vector)) {
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " npi_fzc_sid_set Invalid Input: vector <0x%x>",
			    sid.vector));
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
...
npi_fzc_sid_get(npi_handle_t handle, p_fzc_sid_t sid_p)
{
	...
	if (!LDG_VALID(sid_p->ldg)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fzc_sid_get Invalid Input: ldg <0x%x>", sid_p->ldg));

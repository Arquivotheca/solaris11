...
#define	GET_HASHTBL_PART_OFFSET(handle, partid, reg) \
	(handle.is_vraddr ? (((partid & 0x1) * HASHTBL_PART_REG_VIR_STEP) + \
...
npi_fflp_tcam_check_completion(npi_handle_t handle, tcam_op_t op_type)
{
	...
	case TCAM_RWC_STAT:
		...
		if (!try_counter) {
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " TCAM RWC_STAT operation failed to complete \n"));
	...
	case TCAM_RWC_MATCH:
		...
		if (!try_counter) {
			NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
			    " TCAM Match operation failed to find match \n"));
	...
	default:
		...
		return (NPI_FFLP_ERROR | NPI_TCAM_ERROR | OPCODE_INVALID);
	}
	return (tctl.value);
}
 ...
 */
npi_status_t
npi_fflp_tcam_entry_read(npi_handle_t handle, tcam_location_t location,
 ...
 */
npi_status_t
npi_fflp_tcam_entry_write(npi_handle_t handle, tcam_location_t location,
 ...
 */
npi_status_t
npi_fflp_tcam_asc_ram_entry_write(npi_handle_t handle, tcam_location_t location,
    uint64_t ram_data)
{
	uint64_t	tcam_stat = 0;
	tcam_ctl_t	tctl;
 ...
 */
npi_status_t
npi_fflp_tcam_asc_ram_entry_read(npi_handle_t handle, tcam_location_t location,
    uint64_t *ram_data)
{
	uint64_t	tcam_stat;
	tcam_ctl_t	tctl;
...
npi_fflp_fcram_entry_write(npi_handle_t handle, part_id_t partid,
    uint32_t location, fcram_entry_t *fcram_ptr, fcram_entry_format_t format)
{
	int		num_subareas = 0;
	...
	default:
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_entry_write: unknown format param location %llx\n",
		    location));
    ...
    uint32_t location, fcram_entry_t *fcram_ptr, fcram_entry_format_t format)
{
	...
	default:
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_entry_read: unknown format param location %llx\n",
		    location));
    ...
    uint32_t location)
{
	...
	if (location % 8) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " FCRAM_ENTRY_Invalidate: unaligned location %llx \n",
		    location));
    ...
    uint32_t location, uint64_t *data)
{
	...
	if (location % 8) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_subarea_read: unaligned location %llx \n",
		    location));
		/* need to be 8 byte alligned */
		return (NPI_FFLP_FCRAM_LOC_INVALID);
	}
...
/*
 * The following are function zero fflp configuration functions.
 */
    ...
    uint8_t base_mask, uint8_t base_reloc)
{
	...
	if (!FCRAM_PARTITION_VALID(partid)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_fcram_partition: Invalid Partition %d \n",
		    partid));
 ...
 * Return
 *      0 Successful
 *      Non zero error code and enable failed, and reason.
 */
npi_status_t
npi_fflp_cfg_fcram_partition_enable  (npi_handle_t handle, part_id_t partid)
{
	...
	if (!FCRAM_PARTITION_VALID(partid)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_partition enable: Invalid Partition %d \n",
		    partid));
		return (NPI_FFLP_FCRAM_PART_INVALID);
	}
...
npi_fflp_cfg_fcram_partition_disable(npi_handle_t handle, part_id_t partid)
{
	...
	if (!FCRAM_PARTITION_VALID(partid)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_partition disable: Invalid Partition %d \n",
		    partid));
		return (NPI_FFLP_FCRAM_PART_INVALID);
	}
...
npi_fflp_cfg_cam_errorcheck_disable(npi_handle_t handle)
{
	...
	uint64_t	offset = FFLP_CFG_1_REG;
    ...
    hash_pio_err_log_t *err_stat)
{
	...
	if (!FCRAM_PARTITION_VALID(partid)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_get_pio_err_log: Invalid Partition %d \n", partid));
		return (NPI_FFLP_FCRAM_PART_INVALID);
	}
...
npi_fflp_fcram_clr_pio_err_log(npi_handle_t handle, part_id_t partid)
{
	...
	if (!FCRAM_PARTITION_VALID(partid)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fcram_clr_pio_err_log: Invalid Partition %d \n", partid));
		return (NPI_FFLP_FCRAM_PART_INVALID);
	}
 ...
 * Input
 *      handle: opaque handle interpreted by the underlying OS
 *	err_stat: structure to report various TCAM errors.  Will be
 *		updated if there are TCAM errors.
 * Return
 *	NPI_SUCCESS	Success
 */
npi_status_t
 ...
 *	Tests the FCRAM error detection logic.  The error detection logic
 *	for the syndrome is tested. tst0->synd (8bits) are set to select
 *	the syndrome bits to be XOR'ed
 * Input
 *      handle:	opaque handle interpreted by the underlying OS
...
{
	uint64_t		t0_offset = FFLP_FCRAM_ERR_TST0_REG;
 ...
 *	Tests the FCRAM error detection logic.  The error detection logic for
 *	the datapath is tested.  bits [63:0] are set to select the data
 *	bits to be xor'ed
 * Input
 *      handle:	opaque handle interpreted by the underlying OS
...
npi_fflp_cfg_enet_vlan_table_assoc(npi_handle_t handle, uint8_t mac_portn,
    vlan_id_t vlan_id, uint8_t rdc_table, uint8_t priority)
{
	...
	if (!FFLP_VLAN_VALID(vlan_id)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fflp_cfg_enet_vlan_table: Invalid vlan ID %d \n",
		    vlan_id));
	...
	if (!FFLP_PORT_VALID(mac_portn)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fflp_cfg_enet_vlan_table: Invalid port num %d \n",
		    mac_portn));
	...
	if (!FFLP_RDC_TABLE_VALID(rdc_table)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " fflp_cfg_enet_vlan_table: Invalid RDC Table %d \n",
		    rdc_table));
    ...
    vlan_id_t vlan_id, uint8_t priority)
{
	...
	if (!FFLP_VLAN_VALID(vlan_id)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " enet_vlan_table set pri: Invalid vlan ID %d \n",
		    vlan_id));
		return (NPI_FFLP_VLAN_INVALID);
	}
	...
	if (!FFLP_PORT_VALID(mac_portn)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " enet_vlan_table set pri: Invalid port num %d \n",
		    mac_portn));
		return (NPI_FFLP_PORT_INVALID);
	}
...
npi_fflp_cfg_vlan_table_clear(npi_handle_t handle, vlan_id_t vlan_id)
{
	...
	if ((vlan_id < start_vlan) || (vlan_id >= NXGE_MAX_VLANS)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " enet_vlan_table clear: Invalid vlan ID %d \n", vlan_id));
		return (NPI_FFLP_VLAN_INVALID);
	}
 ...
 *	Reports VLAN Table errors.  If there are VLAN Table errors as
 *	indicated by err bit set by HW, then the SW will clear it by
 *	clearing the bit.
 * Input
 *      handle:	opaque handle interpreted by the underlying OS
 *	err_stat: structure to report various VLAN table errors. Will be
 *		updated if there are errors.
 * Return
 *	NPI_SUCCESS	Success
 */
npi_status_t
 ...
 *	Clears VLAN Table PIO error status.  If there are VLAN Table
 *	errors as indicated by err bit set by HW, then the SW will clear
 *	it by clearing the bit.
 * Input
 *         handle: opaque handle interpreted by the underlying OS
 * Return
 *	NPI_SUCCESS	Success
 ...
 *                        [13:14] in the frame.
 * 		by default, the class will be disabled until
 *		explicitly enabled.
 * Return
 *	NPI success/failure status code
 */
    ...
    tcam_class_t class, uint16_t enet_type)
{
	...
	if (!TCAM_L2_USR_CLASS_VALID(class)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_enet_usr_cls_set: Invalid class %d \n",
		    class));
...
npi_fflp_cfg_enet_usr_cls_enable(npi_handle_t handle, tcam_class_t class)
{
	...
	if (!TCAM_L2_USR_CLASS_VALID(class)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_enet_usr_cls_enable: Invalid class %d \n",
		    class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
...
npi_fflp_cfg_enet_usr_cls_disable(npi_handle_t handle, tcam_class_t class)
{
	...
	if (!TCAM_L2_USR_CLASS_VALID(class)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_enet_usr_cls_disable: Invalid class %d \n",
		    class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
...
npi_fflp_cfg_ip_usr_cls_set(npi_handle_t handle, tcam_class_t class,
    uint8_t tos, uint8_t tos_mask, uint8_t proto, uint8_t ver)
{
	uint64_t		offset;
	...
	if (!TCAM_L3_USR_CLASS_VALID(class)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_usr_cls_set: Invalid class %d \n",
		    class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
 ...
 *	Configures the TCAM user configurable IP classes. This function sets
 *	the new fields that were added for IP tunneling support
 * Input
 *      handle:		opaque handle interpreted by the underlying OS
 *      class:       IP Class  class
 *		     (TCAM_CLASS_IP_USER_4 <= class <= TCAM_CLASS_IP_USER_7)
...
npi_fflp_cfg_ip_usr_cls_set_iptun(npi_handle_t handle, tcam_class_t class,
    uint8_t l4b0_val, uint8_t l4b0_msk, uint16_t l4b23_val, uint8_t l4b23_sel)
{
	uint64_t		offset, val;
	tcam_class_prg_ip_t	ip_cls_cfg;
...
npi_fflp_cfg_ip_usr_cls_get_iptun(npi_handle_t handle, tcam_class_t class,
    uint8_t *l4b0_val, uint8_t *l4b0_msk, uint16_t *l4b23_val,
    uint8_t *l4b23_sel)
{
	uint64_t		offset;
	tcam_class_prg_ip_t	ip_cls_cfg;
...
npi_fflp_cfg_ip_usr_cls_enable(npi_handle_t handle, tcam_class_t class)
{
	...
	if (!TCAM_L3_USR_CLASS_VALID(class)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_usr_cls_enable: Invalid class %d \n",
		    class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
...
npi_fflp_cfg_ip_usr_cls_disable(npi_handle_t handle, tcam_class_t class)
{
	...
	if (!TCAM_L3_USR_CLASS_VALID(class)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_usr_cls_disable: Invalid class %d \n",
		    class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
 ...
 */
npi_status_t
npi_fflp_cfg_ip_cls_tcam_key(npi_handle_t handle, tcam_class_t l3_class,
    tcam_key_cfg_t *cfg)
{
	uint64_t		offset;
	...
	if (!(TCAM_L3_CLASS_VALID(l3_class))) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_cls_tcam_key: Invalid class %d \n",
		    l3_class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}

	if ((cfg->use_ip_daddr) && (cfg->use_ip_saddr == cfg->use_ip_daddr)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_cls_tcam_key:"
    ...
    flow_key_cfg_t *cfg)
{
	...
	if (!(TCAM_L3_CLASS_VALID(l3_class))) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_cls_flow_key: Invalid class %d \n",
		    l3_class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
...
npi_status_t
npi_fflp_cfg_ip_cls_flow_key_get(npi_handle_t handle, tcam_class_t l3_class,
    flow_key_cfg_t *cfg)
{
	uint64_t		offset;
	flow_class_key_ip_t	flow_cfg_reg;
 ...
 *	Configures the flow key generation for the IP classes.
 *	Flow key is used to generate the H1 hash function value.
 *	The fields used for the generation are configured using this NPI
 *	function.
 * Input
 *      handle:	opaque handle interpreted by the underlying OS
 *      l3_class:        IP class to configure flow key generation
    ...
    flow_key_cfg_t *cfg)
{
	...
	if (!(TCAM_L3_CLASS_VALID_RFNL(l3_class))) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_cls_flow_key_rfnl: Invalid class %d \n",
		    l3_class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
    ...
    boolean_t enable)
{
	...
	if (!(TCAM_L3_CLASS_VALID_RFNL(l3_class))) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_sym_ip_cls_flow_key: Invalid class %d \n",
		    l3_class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
...
npi_fflp_cfg_ip_cls_flow_key_get_rfnl(npi_handle_t handle,
    tcam_class_t l3_class, flow_key_cfg_t *cfg)
{
	uint64_t		offset;
	flow_class_key_ip_t	flow_cfg_reg;
...
npi_status_t
npi_fflp_cfg_ip_cls_tcam_key_get(npi_handle_t handle, tcam_class_t l3_class,
    tcam_key_cfg_t *cfg)
{
	uint64_t		offset;
	tcam_class_key_ip_t	tcam_cls_cfg;
	...
	if (!(TCAM_L3_CLASS_VALID(l3_class))) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_ip_cls_tcam_key_get: Invalid class %d \n",
		    l3_class));
		return (NPI_FFLP_TCAM_CLASS_INVALID);
	}
...
npi_fflp_cfg_fcram_access(npi_handle_t handle, uint8_t access_ratio)
{
	...
	if (access_ratio > 0xf) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_fcram_access: Invalid access ratio %d \n",
		    access_ratio));
...
npi_fflp_cfg_tcam_access(npi_handle_t handle, uint8_t access_ratio)
{
	...
	if (access_ratio > 0xf) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_cfg_tcram_access: Invalid access ratio %d \n",
		    access_ratio));
		return (NPI_FFLP_ERROR | NPI_FFLP_SW_PARAM_ERROR);
	}
...
npi_fflp_cfg_hash_h1poly(npi_handle_t handle, uint32_t init_value)
{
	...
	uint64_t	offset = FFLP_H1POLY_REG;
...
npi_fflp_cfg_hash_h2poly(npi_handle_t handle, uint16_t init_value)
{
	...
	uint64_t	offset = FFLP_H2POLY_REG;
    ...
    fflp_event_mask_cfg_t *mask_cfgp)
{
	...
	default:
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_fflp_event_mask_config eventmask <0x%x>", op_mode));
		return (NPI_FFLP_ERROR | NPI_FFLP_SW_PARAM_ERROR);
	}
 ...
 */
void
npi_fflp_fcram_error_get(npi_handle_t handle, p_hash_tbl_data_log_t p_err,
    uint8_t partition)
{
	uint64_t	offset;
...
{
	REG_PIO_READ64(handle, HASH_LKUP_ERR_LOG1_REG, &log1->value);
}
...
{
	REG_PIO_READ64(handle, HASH_LKUP_ERR_LOG2_REG, &log2->value);
}

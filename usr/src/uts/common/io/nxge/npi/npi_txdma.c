...
#define	NUM_TDC_DMC_REGS \
	(sizeof (tdc_dmc_offset) / sizeof (uint64_t))
#define	NUM_TX_FZC_REGS \
	(sizeof (tx_fzc_offset) / sizeof (uint64_t))
...
npi_txdma_dump_tdc_regs(npi_handle_t handle, uint8_t tdc)
{
	...
	if (!TXDMA_CHANNEL_VALID(tdc)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    "npi_txdma_dump_tdc_regs Invalid TDC number %d \n", tdc));
...
npi_txdma_tdc_regs_zero(npi_handle_t handle, uint8_t tdc)
{
	...
	if (!TXDMA_CHANNEL_VALID(tdc)) {
		NPI_REG_DUMP_MSG((handle.function, NPI_REG_CTL,
		    "npi_txdma_tdc_regs_zero InvaliInvalid TDC number %d \n",
		    tdc));
		return (NPI_FAILURE | NPI_TXDMA_CHANNEL_INVALID(tdc));
	}
    ...
    p_dma_log_page_t cfgp)
{
	...
	if (status) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_txdma_log_page_set npi_status <0x%x>", status));
		return (status);
	}
	...
	NPI_DEBUG_MSG((handle.function, NPI_REG_CTL,
	    ...
	    vld.value, vld.bits.ldw.func, (cfgp->valid << cfgp->page_num)));
    ...
    p_dma_log_page_t cfgp)
{
	...
	if (status) {
		NPI_ERROR_MSG((handle.function, NPI_REG_CTL,
		    " npi_txdma_log_page_get npi_status <0x%x>", status));
		return (status);
	}
...
npi_txdma_log_page_config(npi_handle_t handle, io_op_t op_mode,
    txdma_log_cfg_t type, uint8_t channel, p_dma_log_page_t cfgp)
{
	int		status = NPI_SUCCESS;
	uint64_t	val;
	...
	case OP_GET:
		...
		case TXDMA_LOG_PAGE_ALL:
			return (npi_txdma_log_page_get(handle, channel, cfgp));
	...
	case OP_CLEAR:
		...
		switch (type) {
		case TXDMA_LOG_PAGE_ALL:
			return (npi_txdma_log_page_set(handle, channel, cfgp));
		case TXDMA_LOG_PAGE_MASK:
			if (!cfgp->page_num) {
...
{
	NPI_DEBUG_MSG((handle.function, NPI_TDC_CTL,
	    " npi_txdma_channel_reset" " RESETTING", channel));
    ...
    uint8_t channel)
{
	...
	if (!TXDMA_CHANNEL_VALID(channel)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_txdma_channel_control Invalid Input: channel <0x%x>",
		    channel));
		return (NPI_FAILURE | NPI_TXDMA_CHANNEL_INVALID(channel));
	}
		...
		uint8_t channel, p_tx_cs_t cs_p)
{
	...
	if (!TXDMA_CHANNEL_VALID(channel)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_txdma_control_status Invalid Input: channel <0x%x>",
		    channel));
		return (NPI_FAILURE | NPI_TXDMA_CHANNEL_INVALID(channel));
	}
    ...
    uint8_t channel, txdma_ent_msk_cfg_t *mask_cfgp)
{
	...
	default:
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_txdma_event_mask_config"
		    " Invalid Input: eventmask <0x%x>", op_mode));
		return (NPI_FAILURE | NPI_TXDMA_OPCODE_INVALID(channel));
	}
    ...
    uint8_t channel, uint64_t *reg_data)
{
	...
	default:
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_txdma_ring_config Invalid Input: ring_config <0x%x>",
		    op_mode));
		return (NPI_FAILURE | NPI_TXDMA_OPCODE_INVALID(channel));
	}
    ...
    uint8_t channel, uint64_t *mbox_addr)
{
	...
	if (!TXDMA_CHANNEL_VALID(channel)) {
		NPI_ERROR_MSG((handle.function, NPI_ERR_CTL,
		    " npi_txdma_mbox_config Invalid Input: channel <0x%x>",
		    channel));
		return (NPI_FAILURE | NPI_TXDMA_CHANNEL_INVALID(channel));
	}
 ...
 */
npi_status_t
npi_txdma_desc_gather_set(npi_handle_t handle, p_tx_desc_t desc_p,
 ...
 */
npi_status_t
npi_txdma_desc_gather_sop_set(npi_handle_t handle, p_tx_desc_t desc_p,
...
npi_status_t
npi_txdma_desc_gather_sop_set_1(npi_handle_t handle, p_tx_desc_t desc_p,
...
npi_status_t
npi_txdma_desc_set_xfer_len(npi_handle_t handle, p_tx_desc_t desc_p,
    uint32_t transfer_len)
{
	...
	NPI_DEBUG_MSG((handle.function, NPI_TDC_CTL,
	    "npi_set_xfer_len: len %d (%d)", desc_p->bits.hdw.tr_len,
	    transfer_len));
    ...
    uint16_t tail_index, boolean_t wrap)
{
	...
	NPI_DEBUG_MSG((handle.function, NPI_TDC_CTL,
	    " npi_txdma_desc_kick_reg_set: KICKING channel %d", channel));

...
nxge_kstat_index_t nxge_tdc_stats[] = {
	...
	{TDC_STAT_TX_RING_OFLOW, KSTAT_DATA_ULONG, "tdc_tx_ring_oflow"},
	{TDC_STAT_PREF_BUF_ECC_ERR, KSTAT_DATA_ULONG, "tdc_pref_buf_err_err"},
	{TDC_STAT_NACK_PREF, KSTAT_DATA_ULONG, "tdc_nack_pref"},
	{TDC_STAT_NACK_PKT_RD, KSTAT_DATA_ULONG, "tdc_nack_pkt_rd"},
	{TDC_STAT_CONF_PART_ERR, KSTAT_DATA_ULONG, "tdc_conf_part_err"},
...
nxge_kstat_index_t nxge_xmac_stats[] = {
	...
	{XMAC_STAT_RX_FRAME_ALIGN_ERR_CNT, KSTAT_DATA_ULONG,
	    "rxmac_alignment_err"},
	{XMAC_STAT_RX_LINKFAULT_ERR_CNT, KSTAT_DATA_ULONG,
	    "rxmac_linkfault_errs"},
	{XMAC_STAT_RX_REMOTEFAULT_ERR, KSTAT_DATA_ULONG, "rxmac_remote_faults"},
	{XMAC_STAT_RX_LOCALFAULT_ERR, KSTAT_DATA_ULONG, "rxmac_local_faults"},
	...
	{XMAC_STAT_XPCS_DESKEW_ERR_CNT, KSTAT_DATA_ULONG,
	    "xpcs_deskew_err_cnt"},
#ifdef NXGE_DEBUG_SYMBOL_ERR
	{XMAC_STAT_XPCS_SYMBOL_L0_ERR_CNT,
...
nxge_setup_rdc_kstats(p_nxge_t nxgep, int channel)
{
	...
	nxgep->statsp->rdc_ksp[channel] = nxge_setup_local_kstat(nxgep,
	    nxgep->instance, stat_name, nxge_rdc_stats, RDC_STAT_END,
...
nxge_setup_tdc_kstats(p_nxge_t nxgep, int channel)
{
	...
	nxgep->statsp->tdc_ksp[channel] = nxge_setup_local_kstat(nxgep,
	    nxgep->instance, stat_name, nxge_tdc_stats, TDC_STAT_END,
...
nxge_setup_kstats(p_nxge_t nxgep)
{
	...
	nxgep->statsp->rdc_sys_ksp = nxge_setup_local_kstat(nxgep,
	    nxgep->instance, "RDC System Stats", &nxge_rdc_sys_stats[0],
	    RDC_SYS_STAT_END, nxge_rdc_sys_stat_update);

	/* Setup IPP statistics */
	nxgep->statsp->ipp_ksp = nxge_setup_local_kstat(nxgep,
	    nxgep->instance, "IPP Stats", &nxge_ipp_stats[0],
	    IPP_STAT_END, nxge_ipp_stat_update);
#ifdef NXGE_DEBUG_ERROR
...
nxge_xmac_init_kstats(struct kstat *ksp)
{
	...
	kstat_named_init(&nxgekp->rx_remote_fault_err_cnt,
	    "rxmac_remote_faults", KSTAT_DATA_ULONG);
	...
	kstat_named_init(&nxgekp->xpcs_ln0_symbol_err_cnt,
	    "xpcs_ln0_symbol_err_cnt", KSTAT_DATA_ULONG);
	kstat_named_init(&nxgekp->xpcs_ln1_symbol_err_cnt,
	    "xpcs_ln1_symbol_err_cnt", KSTAT_DATA_ULONG);
	kstat_named_init(&nxgekp->xpcs_ln2_symbol_err_cnt,
	    "xpcs_ln2_symbol_err_cnt", KSTAT_DATA_ULONG);
	kstat_named_init(&nxgekp->xpcs_ln3_symbol_err_cnt,
	    "xpcs_ln3_symbol_err_cnt", KSTAT_DATA_ULONG);
#endif
}
...
nxge_save_cntrs(p_nxge_t nxgep)
{
	...
	if (nxgep->mac.porttype == PORT_TYPE_XMAC) {
		...
		/*
		 * Hardware designer:
		 * The link fault register must read twice
		 * and take the second read since link fault register
		 * is a high/low latch.
		 */
		...
		/*
		 * xpcs descw register must be written 0 before read
		 * so the register content will be valid.
		 */
		(void) npi_xmac_xpcs_write(handle, portn,
...
uint64_t
nxge_m_rx_stat(nxge_t *nxgep, uint_t stat)
{
	p_nxge_stats_t		statsp;
...
uint64_t
nxge_m_tx_stat(nxge_t *nxgep, uint_t stat)
{
	p_nxge_stats_t	statsp;
...
nxge_m_stat(void *arg, uint_t stat, uint64_t *value)
{
	...
	default:
		...
		NXGE_ERROR_MSG((nxgep, KST_CTL,
		    "nxge_m_stat: unrecognized parameter value = 0x%x", stat));
#endif
		return (ENOTSUP);
	}

 ...
 */
extern nxge_os_mutex_t nxge_common_lock;
...
extern int nxge_m_mmac_add_g(void *arg, const uint8_t *maddr, int rdctbl,
    boolean_t usetbl);
#if defined(sun4v)
extern int nxge_tx_ring_start(mac_ring_driver_t rdriver, uint64_t mr_gen_num);
extern int nxge_rx_ring_do_start(mac_ring_driver_t rdriver,
    uint64_t mr_gen_num, p_rx_rcr_ring_t ring);
#endif
...
/*
 * void
 * nxge_hio_uninit() -- Cleanup the HIO shared data.  Assumes
 *	that nxge_common_lock is held.
 * Return(s): NONE.
 */
void
nxge_hio_uninit(nxge_t *nxge)
{
	...
	ASSERT(mutex_owned(&nxge_common_lock));
 ...
 */
nxge_grp_t *
nxge_grp_add(nxge_t *nxge, nxge_grp_type_t type)
{
	nxge_grp_set_t	*set;
	nxge_grp_t	*group;
	int		i;
	...
	NXGE_DEBUG_MSG((nxge, HIO_CTL, "nxge_grp_add: %cgroup = %d.%d",
...
void
nxge_grp_remove(nxge_t *nxge, nxge_grp_t *group)
{
	nxge_grp_set_t	*set;
	vpc_type_t	type;
 ...
 * Arguments:
 *	nxge, instance of the nxge device.
 *	group, The group to add the channel too.
 *	type, the group type.
 *	channel, a physical/logical channel number
 * Context:
 *	Any domain
 */
/* ARGSUSED */
int
nxge_grp_dc_add(nxge_t *nxge, nxge_grp_t *group, vpc_type_t type, int channel)
{
	nxge_hio_data_t	*nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
	nxge_hio_dc_t	*dc;
	nxge_grp_set_t	*set;
	...
	if ((*dc->init)(nxge, channel) != NXGE_OK) {
		NXGE_ERROR_MSG((nxge, NXGE_ERR_CTL,
...
nxge_grp_dc_remove(nxge_t *nxge, vpc_type_t type, int channel)
{
	...
	NXGE_DEBUG_MSG((nxge, HIO_CTL, "DC remove: group = %d.%d.%d, %cdc %d",
...
nxge_hio_dc_t *
nxge_grp_dc_find(nxge_t *nxge, vpc_type_t type,	int channel)
{
	nxge_hio_data_t *nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
 ...
 */
static void
nxge_grp_dc_append(nxge_t *nxge, nxge_grp_t *group, nxge_hio_dc_t *dc)
{
	MUTEX_ENTER(&nxge->group_lock);
 ...
 */
void
nxge_grp_dc_map(nxge_grp_t *group)
{
...
nxge_hio_init(nxge_t *nxge)
{
	...
	if ((nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio) == NULL) {
...
#if defined(sun4v)
	if ((nxge->environs == SOLARIS_DOMAIN) && (nxge->niu_type == N2_NIU)) {
		if (nxge->niu_hsvc_available == B_TRUE) {
			...
			if ((niu_hsvc->hsvc_major >= NIU_MAJOR_VER) ||
				...
				cmn_err(CE_CONT,
				    "!nxge(%d): HV services version %d.%d\n",
				    nxge->instance, (int)niu_hsvc->hsvc_major,
				    (int)niu_hsvc->hsvc_minor);
			}
		}
	}
#endif
	...
	for (i = 0; i < NXGE_MAX_TDC_GROUPS; i++) {
		...
		nxge->tx_hio_groups[i].sindex = NXGE_NO_HIO_SHARE;
	}
...
/*ARGSUSED*/
static int
nxge_hio_add_mac(void *arg, const uint8_t *mac_addr, uint64_t flags)
{
...
#if defined(sun4v)
	/*
	 * For the N2NIU, if hybrid IO is enabled for the group
	 * (sindex != NXGE_NO_HIO_SHARE), then only one mac address for
	 * the group may be added at this time.
	 */
	if ((nxge->niu_type == N2_NIU) &&
	    (group->sindex != NXGE_NO_HIO_SHARE)) {
...
nxge_hio_group_start(mac_group_driver_t gdriver)
{
	 ...
	 */
	if ((group->gindex != 0) && (group->nxgep->niu_type == N2_NIU)) {
		if (group->sindex == NXGE_NO_HIO_SHARE) {
			...
			shp = &group->nxgep->shares[group->sindex];
			rdctbl = shp->rxgroup;
		}
	} else if (group->gindex != 0) {
		rdctbl = nxge_fzc_rdc_tbl_bind(group->nxgep,
		    dev_gindex, B_TRUE);
		if (rdctbl < 0) {
			mutex_exit(group->nxgep->genlock);
	...
	group->rdctbl = rdctbl;
	if (nxge_init_fzc_rdc_tbl(group->nxgep, rdc_grp_p, rdctbl)) {
		mutex_exit(group->nxgep->genlock);
		return (EIO);
	}
...
nxge_hio_group_stop(mac_group_driver_t gdriver)
{
	...
	if (group->gindex != 0) {
		...
		group->rdctbl = NXGE_NO_RDC_TABLE;
	}
    ...
    mac_group_info_t *infop, mac_group_handle_t ghdl)
{
	...
	case MAC_RING_TYPE_TX:
		group = &nxgep->tx_hio_groups[groupid];
		group->nxgep = nxgep;
		group->ghandle = ghdl;
		group->gindex = groupid;
		group->sindex = NXGE_NO_HIO_SHARE;
...
#if defined(sun4v)
static int
nxge_hio_share_assign(nxge_t *nxge, uint64_t cookie, res_map_t *tmap,
    res_map_t *rmap, nxge_hio_vr_t *vr)
{
	nxge_hio_data_t	*nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
	uint64_t	slot, hv_rv;
	nxge_hio_dc_t	*dc;
	...
	NXGE_DEBUG_MSG((nxge, HIO_CTL, "nxge_hio_share_assign: major %d "
...
void
nxge_hio_share_unassign(nxge_hio_vr_t *vr)
{
    ...
    mac_group_driver_t ghandle)
{
	...
	if (rg->sindex != NXGE_NO_HIO_SHARE) {
	...
	case MAC_RING_TYPE_RX:
		 ...
		 * VR1: RDC2 & RDC3, etc.
		 */
		dgindex = nxge->pt_config.hw_config.def_mac_rxdma_grpid +
		    rg->gindex;
		...
		if (rg->rdctbl == NXGE_NO_RDC_TABLE) {
			vr->rdc_tbl = nxge_fzc_rdc_tbl_bind(nxge,
			    dgindex, B_TRUE);
			if (vr->rdc_tbl < 0)
				return (EIO);
		} else
	...
	case MAC_RING_TYPE_TX:
		 ...
		 * VR1: TDC2 & TDC3, etc.
		 */
		dgindex = nxge->pt_config.hw_config.def_mac_txdma_grpid +
		    rg->gindex;
...
void
nxge_hio_unshare(nxge_hio_vr_t *vr)
{
	nxge_t		*nxge = (nxge_t *)vr->nxge;
	nxge_hio_data_t	*nhd;
...
/* ARGSUSED */
static void
nxge_hio_remres(nxge_hio_vr_t *vr, mac_ring_type_t type, res_map_t res_map)
{
	nxge_t		*nxge = (nxge_t *)vr->nxge;
	nxge_grp_t	*group;
 ...
 */
int
nxge_hio_tdc_share(nxge_t *nxge, int channel)
{
	nxge_hio_data_t	*nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
 ...
 */
int
nxge_hio_rdc_share(nxge_t *nxge, nxge_hio_vr_t *vr, int channel)
{
	nxge_grp_set_t *set = &nxge->rx_set;
	nxge_rdc_grp_t *rdc_grp;

	ASSERT(nxge != NULL);
	ASSERT(isLDOMservice(nxge));
	...
	if (nxge_rxdma_disable(nxge, channel) != NXGE_OK) {
		NXGE_ERROR_MSG((nxge, NXGE_ERR_CTL, "nxge_hio_rdc_share: "
 ...
 */
static int
nxge_hio_dc_share(nxge_t *nxge, nxge_hio_vr_t *vr, mac_ring_type_t type,
    int channel)
{
	nxge_hio_data_t *nhd = (nxge_hio_data_t *)nxge->nxge_hw_p->hio;
	nxge_hio_dc_t *dc;
	...
	/* -------------------------------------------------- */
	slot = (type == MAC_RING_TYPE_TX) ? nxge_hio_tdc_share(nxge, channel) :
 ...
 */
void
nxge_hio_tdc_unshare(nxge_t *nxge, int dev_grpid, int channel)
{
	nxge_grp_set_t *set = &nxge->tx_set;
	nxge_grp_t *group;
	int i, grpid;
	...
	for (i = 0; i < NXGE_MAX_TDCS; i++) {
		if (nxge->tx_ring_handles[i].channel == channel) {
			(void) nxge_tx_ring_start(
			    (mac_ring_driver_t)&nxge->tx_ring_handles[i],
			    nxge->tx_ring_handles[i].ring_gen_num);
			break;
		}
	}
 ...
 */
void
nxge_hio_rdc_unshare(nxge_t *nxge, int dev_grpid, int channel)
{
	nxge_grp_set_t		*set = &nxge->rx_set;
	nxge_grp_t		*group;
	int			grpid;
 ...
 */
void
nxge_hio_dc_unshare(nxge_t *nxge, nxge_hio_vr_t *vr, mac_ring_type_t type,
    int channel)
{
	nxge_grp_t *group;
	nxge_hio_dc_t *dc;

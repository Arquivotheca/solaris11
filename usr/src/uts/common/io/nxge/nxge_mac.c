...
/* Recover alternate MAC address settings in HW, which is lost after reset. */

nxge_status_t
nxge_recover_altmac(p_nxge_t nxgep)
{
	int		slot;
	...
	NXGE_DEBUG_MSG((nxgep, MAC_CTL, "==> nxge_recover_altmac: port<%d>",
	    portn));
	mmac_info = &nxgep->nxge_mmac_info;
	for (slot = 0; slot < mmac_info->num_mmac; slot++) {
		if (mmac_info->mac_pool[slot].flags & MMAC_SLOT_USED &&
		    (mmac_info->mac_pool[slot].flags & MMAC_VENDOR_ADDR) == 0) {
			if (nxge_altmac_set(nxgep,
			    (uint8_t *)&mmac_info->mac_pool[slot].addr, slot, 0,
			    B_FALSE) != 0) {
				NXGE_DEBUG_MSG((nxgep, MAC_CTL,
				    "nxge_recover_altmac: ", "failed to "
				    "recover altmac in slot %d on port<%d>",
				    slot, portn));
				return (NXGE_ERROR);
			}
		}
...
nxge_mac_init(p_nxge_t nxgep)
{
	...
	if ((status = nxge_recover_altmac(nxgep)) != NXGE_OK)
		goto fail;
...
/*
 * nxge_status_t
 * nxge_mac_lfs_disable(nxgep) -- disable link fault check and enable
 * 	TX interanl loopback.  This allows us during unplumb time to properly
 *	drain the TX dma channels.
 *
 * Returns(s): NXGE_OK, if succesful.
 *	NXGE_ERROR, if failed.
 */
nxge_status_t
nxge_mac_lfs_disable(p_nxge_t nxgep)
{
	uint32_t		xif_config = 0;
	uint8_t			portn;
	...
	xif_config = CFG_XMAC_XIF_LFS | CFG_XMAC_XIF_LOOPBACK;

	if (nxgep->mac.porttype == PORT_TYPE_XMAC)
		rs = npi_xmac_xif_config(handle, ENABLE, portn, xif_config);
	else
		rs = npi_bmac_xif_config(handle, ENABLE, portn, xif_config);
	if (rs != NPI_SUCCESS)
		return (NXGE_ERROR);
...
nxge_nlp2020_xcvr_init(p_nxge_t nxgep)
{
	...
	/*
	 * According to Netlogic, two registers must be
	 * set for PMA system loopback if the connector
	 * type is fiber and five registers must be set
	 * if the connector type is copper.
	 * Except the NLP2020_PMA_PMD_CTL_REG, other
	 * additional register settings are not described
	 * in the AEL 2020 data sheet!.
	 *
	 * If the connector type is fiber:
	 * (no cable connected is treated as fiber mode):
	 * 1.   1.CD40 = 0x00  (1 = MDIO device address (NLP2020_PMA_PMD_ADDR)
	 *			CD40 = register address)
	 * 2.   1.0000 = Set the desired loopback bit
	 * 		1    = MDIO device address (NLP2020_PMA_PMD_ADDR)
	 * 		0000 = PMA/PMD control register address
	 * 		Set to 0x1 to enable PMA system loopback.
	 *
	 * If the connector type is copper:
	 * (this mode is currently not supported).
	 * A list of loopback steps.
	 * 1.   1.d092.0 = 1
	 * 2.   1.c00c = 0
	 * 3.   1.c20c = 0x1880
	 * 4.   1.CD40 = 0
	 * 5.   Set the desired loopback bit
	 */
	if (nxgep->statsp->port_stats.lb_mode == nxge_lb_phy10g) {
		/* Step 1: Write 0x0 to register 0xCD40 */
		if ((status = nxge_mdio_write(nxgep, phy_port_addr,
		    NLP2020_PMA_PMD_ADDR, 0xCD40, 0x0))
		    != NXGE_OK) {
			goto fail;
		}
		NXGE_DEBUG_MSG((nxgep, MAC_CTL, "nxge_nlp2020_xcvr_init: "
		    "LB mode %d, wrote 0x0 to NLP2020_PMA_PMD 0xCD40",
		    nxgep->statsp->port_stats.lb_mode));

		/*  Step 2: Set control register bit 0 to 1. */
		pmd_ctl |= 0x0001;
	} else {
		pmd_ctl &= 0xfffe;
	}
	...
	NXGE_DEBUG_MSG((nxgep, MAC_CTL, "nxge_nlp2020_xcvr_init: "
	    "LB mode %d, wrote NLP2020_PMA_PMD_CTL_REG[0x%x]",
	    nxgep->statsp->port_stats.lb_mode, pmd_ctl));

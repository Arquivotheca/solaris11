...
static int pciehpc_change_slot_state(pcie_hp_slot_t *slot_p,
    ddi_hp_cn_state_code_t target_state);
static int pciehpc_slot_poweron(pcie_hp_slot_t *slot_p,
    ddi_hp_cn_state_code_t *result);
static int pciehpc_slot_poweroff(pcie_hp_slot_t *slot_p,
    ddi_hp_cn_state_code_t *result);
...
pciehpc_init(dev_info_t *dip, caddr_t arg)
{
	...
	/*
	 * For non-empty hotplug capable slots, enable the errors.
	 * For empty hotplug capable slots, keep to disable the
	 * errors.
	 */
    ...
    void *arg, void *result)
{
	...
	case DDI_HPOP_CN_GET_STATE:
	{
		...
		*((ddi_hp_cn_state_code_t *)result) = slot_p->hs_info.cn_state;
	...
	case DDI_HPOP_CN_CHANGE_STATE:
	{
		ddi_hp_cn_change_state_arg_t *kargp
		    = (ddi_hp_cn_change_state_arg_t *)arg;
		ddi_hp_cn_state_code_t target_state
...
pciehpc_get_slot_state(pcie_hp_slot_t *slot_p)
{
	...
	ddi_hp_cn_state_code_t curr_state = slot_p->hs_info.cn_state;
...
/*ARGSUSED*/
static int
pciehpc_slot_poweron(pcie_hp_slot_t *slot_p, ddi_hp_cn_state_code_t *result)
{
	pcie_hp_ctrl_t	*ctrl_p = slot_p->hs_ctrl;
	pcie_bus_t	*bus_p = PCIE_DIP2BUS(ctrl_p->hc_dip);
	uint16_t	status, control;
...
/*ARGSUSED*/
static int
pciehpc_slot_poweroff(pcie_hp_slot_t *slot_p, ddi_hp_cn_state_code_t *result)
{
	pcie_hp_ctrl_t	*ctrl_p = slot_p->hs_ctrl;
	pcie_bus_t	*bus_p = PCIE_DIP2BUS(ctrl_p->hc_dip);
	uint16_t	status, control;
...
pciehpc_slot_probe(pcie_hp_slot_t *slot_p)
{
	...
	/*
	 * Disable ACS Source Validation before probe device
	 * A work around to avoid ACS Violation Error during probe
	 */
	pcie_acs_control(ctrl_p->hc_dip, PCIE_ACS_DISABLE,
	...
	if (ret != DDI_SUCCESS) {
		...
		/* Enable ACS Source Validation after probe device */
		pcie_acs_control(ctrl_p->hc_dip, PCIE_ACS_ENABLE,
		    PCIE_ACS_CTL_SOURCE_VAL_EN);

...
{
	ASSERT(MUTEX_HELD(&lsp->ls_io_lock));
	...
	lsp->ls_crypto_enabled = B_FALSE;
}

static void
lofi_free_compress(struct lofi_state *lsp)
{
	ASSERT(MUTEX_HELD(&lsp->ls_io_lock));

	if (strlen(lsp->ls_comp_algorithm) == 0)
		return;
	...
	if (lsp->ls_uncomp_seg_sz > 0) {
		...
		lsp->ls_comp_index_data = NULL;
		lsp->ls_comp_index_data_sz = 0;
...
static void
lofi_destroy_dev(struct lofi_state *lsp)
{
	char namebuf[50];
	dev_t dev = lsp->ls_dev;
	...
	mutex_destroy(&lsp->ls_io_lock);

	list_remove(&lofi_list, lsp);
	lofi_zone_unbind(lsp);
	ddi_soft_state_free(lofi_statep, minor);
	lofi_free_minor(minor);
}

static void
lofi_detach_file(struct lofi_state *lsp, cred_t *credp)
{
	ASSERT(mutex_owned(&lsp->ls_io_lock));
	...
	(void) ddi_prop_update_int64(lsp->ls_dev, lofi_dip, SIZE_PROP_NAME, 0);
	(void) ddi_prop_update_int64(lsp->ls_dev, lofi_dip,
	    NBLOCKS_PROP_NAME, 0);

	lofi_free_crypto(lsp);
	lofi_free_compress(lsp);
	...
	lsp->ls_openflag = 0;
	lsp->ls_open_rw_err = 0;
	lsp->ls_vp_size = lsp->ls_vp_comp_size = 0;
	lsp->ls_close_req = B_FALSE;
	cv_broadcast(&lsp->ls_io_cv);
}

static void
lofi_kickoff_cleanup(struct lofi_state *lsp)
{
	ASSERT(mutex_owned(&lofi_lock));
	 ...
	 * They should leave since the device will be
	 * gone soon.
	 */
	cv_broadcast(&lsp->ls_io_cv);
}
...
lofi_zone_shutdown(zoneid_t zoneid, void *arg)
{
	...
	for (lsp = list_head(&lofi_list); lsp != NULL; lsp = next) {
		 ...
		 */
		if (is_opened(lsp)) {
			lofi_kickoff_cleanup(lsp);
		} else {
			mutex_enter(&lsp->ls_io_lock);
			lofi_detach_file(lsp, kcred);
			mutex_exit(&lsp->ls_io_lock);
			lofi_destroy_dev(lsp);
		}
	}
...
lofi_open(dev_t *devp, int flag, int otyp, struct cred *credp)
{
	...
	/* If the underlying file isn't opened r/w, return the saved error. */
	if ((flag & FWRITE) && !(lsp->ls_openflag & FWRITE)) {
		mutex_exit(&lofi_lock);
		return (lsp->ls_open_rw_err);
	}
	...
	if (flag & FWRITE)
		rwflag |= VWRITE;
	if ((error = VOP_ACCESS(lsp->ls_vp, rwflag, 0, credp, NULL)) != 0) {
		mutex_exit(&lofi_lock);
		return (error);
	}
done:
...
lofi_close(dev_t dev, int flag, int otyp, struct cred *credp)
{
	...
	/*
	 * If we are asked for cleanup (li_cleanup), finish up if
	 * we're the last out of the door.
	 */
	if (!is_opened(lsp) && lsp->ls_cleanup) {
		mutex_enter(&lsp->ls_io_lock);
		lofi_detach_file(lsp, credp);
		mutex_exit(&lsp->ls_io_lock);
		lofi_destroy_dev(lsp);
	}
    ...
    struct lofi_state *lsp)
{
	...
	/* Writing but not opened with FWRITE, return an error */
	if (!isread && (lsp->ls_openflag & FWRITE) == 0)
		return (lsp->ls_open_rw_err);
...
lofi_strategy(struct buf *bp)
{
	...
	mutex_enter(&lsp->ls_io_lock);
	if (lsp->ls_vp == NULL || lsp->ls_close_req) {
		mutex_exit(&lsp->ls_io_lock);
		bioerror(bp, EIO);
		biodone(bp);
		return (0);
...
lofi_init_compress(struct lofi_state *lsp)
{
	...
	if (error != 0) {
		kmem_free(lsp->ls_comp_bufs, sizeof (struct compbuf));
		list_destroy(&lsp->ls_comp_cache);
		mutex_destroy(&lsp->ls_comp_cache_lock);
		mutex_destroy(&lsp->ls_comp_bufs_lock);
		lsp->ls_comp_algorithm[0] = '\0';
...
/* Will pick a minor if *minorp == -1. */
static int
lofi_alloc_minor(minor_t *minorp)
{
	minor_t minor = *minorp;
	...
	if (minor == (minor_t)-1) {
		...
		if (ddi_get_soft_state(lofi_statep, minor) != NULL) {
			...
			minor = (minor_t)id_alloc_specific_nosleep(
			    lofi_minor_id, minor);
...
lofi_zone_bind(struct lofi_state *lsp)
{
	...
	error = rctl_incr_lofi(curproc, curproc->p_zone, 1);
...
static int
lofi_create_dev(minor_t *minorp)
{
	int	error;
	dev_t	newdev;
	...
	cv_init(&lsp->ls_io_cv, NULL, CV_DRIVER, NULL);
	mutex_init(&lsp->ls_io_lock, NULL, MUTEX_DRIVER, NULL);
	...
	/* Create properties. */
	lsp->ls_dev = newdev = makedevice(ddi_driver_major(lofi_dip), minor);
	...
	/* Return lofi dev we just created to caller. */
	*minorp = minor;
...
static int
lofi_attach_file(struct lofi_state *lsp, struct lofi_ioctl *klip,
    struct cred *credp)
{
	int	flag = FREAD | FWRITE | FOFFMAX | FEXCL;
	int	error = 0;
	int	openrwerr = 0;
	struct lofi_state *existing_lsp = NULL;
	...
	if (klip->li_filename[0] == '\0')
		return (EINVAL);

	if (lsp->ls_vp != NULL)
		return (EBUSY);

	if ((file_to_lofi_nocheck(klip->li_filename, &existing_lsp) == 0))
		return (EBUSY);
	...
	if ((strlen(lsp->ls_comp_algorithm) > 0) && klip->li_crypto_enabled) {
		error = ENOTSUP;
		goto err;
	}
	...
	lsp->ls_pbs = DEV_BSIZE;
	lsp->ls_lbs = DEV_BSIZE;

	/* Update DDI properties. */
	if ((ddi_prop_update_int64(lsp->ls_dev, lofi_dip, SIZE_PROP_NAME,
	    lsp->ls_vp_size - lsp->ls_crypto_offset)) != DDI_PROP_SUCCESS) {
		error = EINVAL;
		goto err;
	}
	if ((ddi_prop_update_int64(lsp->ls_dev, lofi_dip, NBLOCKS_PROP_NAME,
 ...
 */
static int
lofi_map_file(struct lofi_ioctl *ulip, int pickminor,
    int *rvalp, struct cred *credp, int ioctl_flag)
{
	...
	if ((error = lofi_create_dev(&minor)) != 0)
		goto err;

	/* Associate file to this lofi device. */
	lsp = ddi_get_soft_state(lofi_statep, minor);
	ASSERT(lsp != NULL);
	mutex_enter(&lsp->ls_io_lock);
	error = lofi_attach_file(lsp, klip, credp);
	mutex_exit(&lsp->ls_io_lock);
	if (error != 0)
		goto fileerr;
    ...
    struct cred *credp, int ioctl_flag)
{
	...
	/* Get lsp in question and make sure we can access it. */
	if (byfilename) {
	...
	/* Found lofi device we need to take action on. */
	 ...
	 * will return DKIO_DEV_GONE.  The lofi device will be left and has
	 * to be removed manually later.
	 *
	 ...
	 * (ls_io_count), as well as a flag to indicate that no new I/Os
	 * should be dispatched (ls_close_req).
	 *
	...
	if (is_opened(lsp)) {
		if (klip->li_force) {
			mutex_enter(&lsp->ls_io_lock);
			lsp->ls_close_req = B_TRUE;
			while (lsp->ls_io_count > 0)
				cv_wait(&lsp->ls_io_cv, &lsp->ls_io_lock);
			lofi_detach_file(lsp, credp);
			mutex_exit(&lsp->ls_io_lock);
 ...
 */
static int
lofi_get_info(struct lofi_ioctl *ulip, int which, int ioctl_flag)
{
	struct lofi_ioctl *klip;
	struct lofi_state *lsp;
	int	error;
	...
	case LOFI_GET_FILENAME:
		...
		if (lsp->ls_stacked_vp != NULL) {
			if (vnodetopath(NULL, lsp->ls_stacked_vp,
			    klip->li_filename, sizeof (klip->li_filename),
			    CRED()) != 0) {
				(void) strlcpy(klip->li_filename, "?",
				    sizeof (klip->li_filename));
			}
		} else {
			klip->li_filename[0] = '\0';
		}
...
static int
lofi_set_info(struct lofi_ioctl *ulip, int which, int ioctl_flag)
{
	struct lofi_ioctl *klip;
	struct lofi_state *lsp;
	int	error;
	...
	switch (which) {
	case LOFI_SET_BLOCKSIZE:
		/* for testing devices used by zfs */
		mutex_enter(&lofi_lock);
		error = file_to_lofi(klip->li_filename, &lsp);
		if (error != 0) {
			mutex_exit(&lofi_lock);
		...
		klip->li_minor = getminor(lsp->ls_dev);
		if (klip->li_lbs)
			lsp->ls_lbs = klip->li_lbs;
		if (klip->li_pbs)
			lsp->ls_pbs = klip->li_pbs;
		mutex_exit(&lofi_lock);
		free_lofi_ioctl(klip);
		return (0);
	default:
    ...
    int *rvalp)
{
	...
	if (minor == 0) {
		...
		case LOFI_MAP_FILE:
			if ((flag & FWRITE) == 0)
				return (EPERM);
			return (lofi_map_file(lip, 1, rvalp, credp, flag));
		case LOFI_MAP_FILE_MINOR:
			if ((flag & FWRITE) == 0)
				return (EPERM);
			return (lofi_map_file(lip, 0, rvalp, credp, flag));
		...
		case LOFI_GET_FILENAME:
			return (lofi_get_info(lip, LOFI_GET_FILENAME,
			    flag));
		case LOFI_GET_MINOR:
			return (lofi_get_info(lip, LOFI_GET_MINOR,
			    flag));
		...
		case LOFI_CHECK_COMPRESSED:
			return (lofi_get_info(lip, LOFI_CHECK_COMPRESSED,
			    flag));
		case LOFI_SET_BLOCKSIZE:
			return (lofi_set_info(lip, LOFI_SET_BLOCKSIZE,
			    flag));
		default:
			return (EINVAL);
		}
	...
	case DKIOCSTATE:
		...
		while (((dkstate == DKIO_INSERTED && lsp->ls_vp != NULL) ||
			 ...
			 */
			if (!cv_wait_sig(&lsp->ls_io_cv, &lsp->ls_io_lock)) {
				mutex_exit(&lsp->ls_io_lock);
				return (EINTR);
			}
		}
	...
	case DKIOCGMEDIAINFOEXT:
	{
		...
		mie.dki_media_type = DK_FIXED_DISK;
		mie.dki_lbsize = lsp->ls_lbs;
		mie.dki_capacity = lsp->ls_vp_size - lsp->ls_crypto_offset;
		mie.dki_capacity /= mie.dki_lbsize;
		mie.dki_pbsize = lsp->ls_pbs;
		if (ddi_copyout(&mie, (void *)arg,
		    sizeof (struct dk_minfo_ext), flag) != 0)
			return (EFAULT);
		return (0);
	}
	default:

...
static void
splash_key_abort_thread(struct kbtrans *upper)
{
	ldi_ident_t li;
	extern void splash_key_abort(ldi_ident_t);

	if (ldi_ident_from_stream(upper->kbtrans_streams_readq, &li) != 0) {
		cmn_err(CE_NOTE, "!ldi_ident_from_stream failed");
	} else {
		mutex_enter(&upper->splash_key_abort_lock);
		while (upper->splash_key_abort_flag == 0)
			cv_wait(&upper->splash_key_abort_cv,
			    &upper->splash_key_abort_lock);
		if (upper->splash_key_abort_flag == 1) {
			mutex_exit(&upper->splash_key_abort_lock);
			splash_key_abort(li);
		} else {
			mutex_exit(&upper->splash_key_abort_lock);
		}
	...
	int 				initial_led_mask)
{
	...
	*ret_kbd = upper;
	...
	upper->splash_key_abort_flag = 0;
	cv_init(&upper->splash_key_abort_cv, NULL, CV_DEFAULT, NULL);
	/* this counts on no keyboards being above ipl 12 */
	mutex_init(&upper->splash_key_abort_lock, NULL, MUTEX_SPIN,
	    (void *)ipltospl(12));
	tid = thread_create(NULL, 0, splash_key_abort_thread, upper,
	    0, &p0, TS_RUN, minclsyspri);
	upper->splash_key_abort_t_did = tid->t_did;
...
kbtrans_streams_fini(struct kbtrans *upper)
{
	...
	mutex_enter(&upper->splash_key_abort_lock);
	if (upper->splash_key_abort_flag == 0) {
		upper->splash_key_abort_flag = 2;
		cv_signal(&upper->splash_key_abort_cv);
		mutex_exit(&upper->splash_key_abort_lock);
		thread_join(upper->splash_key_abort_t_did);
	} else {
		mutex_exit(&upper->splash_key_abort_lock);
	}
	cv_destroy(&upper->splash_key_abort_cv);
	mutex_destroy(&upper->splash_key_abort_lock);

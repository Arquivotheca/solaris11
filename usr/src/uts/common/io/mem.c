...
/*
 * Log control. Indexes into LOG_DEVS arrays is governed by the logging
 * index macros in mem.h
 */
static kmutex_t mm_log_lock[LOG_DEVS];
u_longlong_t mm_log_cmltv[LOG_DEVS];	/* cumulative update counts */
uint_t mm_log_tail[LOG_DEVS];		/* last entry in each */
mem_log_t mm_mem_log[LOG_ENTS];
mem_log_t mm_kmem_log[LOG_ENTS];
mem_log_t mm_allkmemr_log[LOG_ENTS];	/* reads log */
mem_log_t mm_allkmemw_log[LOG_ENTS];	/* writes log */
...
mm_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	for (i = 0; i < LOG_DEVS; i++) {
		mutex_init(&mm_log_lock[i], NULL, MUTEX_DEFAULT, NULL);
	}
...
mmrw(dev_t dev, struct uio *uio, enum uio_rw rw, cred_t *cred)
{
	...
	uint_t idx;
	uint_t log_select;
	mem_log_t *log_buf = NULL;
	u_offset_t start_off = (u_offset_t)uio->uio_loffset;
	...
	/* if appropriate commit the operation to transaction log */
	if ((rw == UIO_WRITE && (minor == M_MEM || minor == M_KMEM)) ||
	    minor == M_ALLKMEM) {
		if (rw == UIO_READ) {
			/* must be read on /dev/allkmem */
			log_select = L_ALLKMEMR;
		} else {
			log_select = (uint_t)minor;
		}

		mutex_enter(&mm_log_lock[log_select]);
		idx = mm_log_tail[log_select];

		switch (log_select) {
		case L_MEM:
			log_buf = &(mm_mem_log[idx]);
			break;
		case L_KMEM:
			log_buf = &(mm_kmem_log[idx]);
			break;
		case L_ALLKMEMR:
			log_buf = &(mm_allkmemr_log[idx]);
			break;
		case L_ALLKMEMW:
			log_buf = &(mm_allkmemw_log[idx]);
			break;
		}

		log_buf->m_who = curthread;
		log_buf->m_when = gethrtime();
		log_buf->m_where = start_off;
		log_buf->m_what = (int)rw;
		log_buf->m_sz = oresid;
		log_buf->m_success = 1;		/* presume success */

		mm_log_cmltv[log_select]++;

		if (++mm_log_tail[log_select] == LOG_ENTS) {
			mm_log_tail[log_select] = 0;
		}
		...
		mutex_exit(&mm_log_lock[log_select]);
	}

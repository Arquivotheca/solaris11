...
struct ixgbe_hw;
uint16_t ixgbevf_read_pci_cfg(struct ixgbe_hw *, uint32_t);
void ixgbevf_write_pci_cfg(struct ixgbe_hw *, uint32_t, uint32_t);

#define	usec_delay(x)		drv_usecwait(x)
#define	msec_delay(x)		drv_usecwait(x * 1000)

#define	OS_DEP(hw)		((struct ixgbevf_osdep *)((hw)->back))

#define	false		B_FALSE
#define	true		B_TRUE

#define	IXGBE_READ_PCIE_WORD 		ixgbevf_read_pci_cfg
#define	IXGBE_WRITE_PCIE_WORD 		ixgbevf_write_pci_cfg

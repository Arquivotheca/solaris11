...
/* IntelVersion: 1.182 scm_120210_003723 */
 ...
 */
static bool
e1000_sgmii_uses_mdio_82575(struct e1000_hw *hw)
{
	u32 reg = 0;
	bool ext_mdio = false;
...
e1000_init_phy_params_82575(struct e1000_hw *hw)
{
	...
	(void) e1000_reset_mdicnfg_82580(hw);
 ...
 */
s32
e1000_set_d3_lplu_state_82580(struct e1000_hw *hw, bool active)
{
	struct e1000_phy_info *phy = &hw->phy;
	s32 ret_val = E1000_SUCCESS;
	u16 data;
 ...
 */
static void
e1000_power_up_serdes_link_82575(struct e1000_hw *hw)
{
	u32 reg;
 ...
 */
static void
e1000_config_collision_dist_82575(struct e1000_hw *hw)
{
	u32 tctl_ext;
 ...
 */
void
e1000_vmdq_set_anti_spoofing_pf(struct e1000_hw *hw, bool enable, int pf)
{
	u32 dtxswc;
 ...
 */
static s32
e1000_reset_mdicnfg_82580(struct e1000_hw *hw)
{
	s32 ret_val = E1000_SUCCESS;
	u32 mdicnfg;
	u16 nvm_data;
...
static s32 e1000_validate_nvm_checksum_82580(struct e1000_hw *hw)
{
	...
	for (j = 0; j < eeprom_regions_count; j++) {
		...
		checksum = 0;
		for (i = nvm_offset;
		    i < ((NVM_CHECKSUM_REG + nvm_offset) + 1); i++) {
 ...
 */
static s32
e1000_update_nvm_checksum_82580(struct e1000_hw *hw)
{
	s32 ret_val;
	u16 checksum = 0;
	u16 i, j, nvm_data;
	...
	for (j = 0; j < 4; j++) {
		nvm_offset = NVM_82580_LAN_FUNC_OFFSET(j);
		checksum = 0;
		for (i = nvm_offset; i < (NVM_CHECKSUM_REG + nvm_offset); i++) {
			ret_val = hw->nvm.ops.read(hw, i, 1, &nvm_data);
			if (ret_val) {
				DEBUGOUT("NVM Read Error"
				    " while updating checksum.\n");
				goto out;
			}
			checksum += nvm_data;
		}
		checksum = (u16) NVM_SUM - checksum;
		ret_val = hw->nvm.ops.write(hw, (NVM_CHECKSUM_REG + nvm_offset),
		    1, &checksum);
		if (ret_val) {
			DEBUGOUT("NVM Write Error while updating checksum.\n");
			goto out;
...
e1000_set_eee_i350(struct e1000_hw *hw)
{
	...
	if ((hw->mac.type != e1000_i350) ||
	    (ctrl_ext & E1000_CTRL_EXT_LINK_MODE_GMII))
		goto out;
	...
	(void) E1000_READ_REG(hw, E1000_IPCNFG);
	(void) E1000_READ_REG(hw, E1000_EEER);
out:

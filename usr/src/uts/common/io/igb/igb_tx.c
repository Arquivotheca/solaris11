...
igb_tx_ring_send(void *arg, mblk_t *mp)
{
	...
	if ((igb->igb_state & (IGB_STARTED | IGB_ERROR)) != IGB_STARTED) {
		freemsg(mp);
		return (NULL);
	}
    ...
    tx_context_t *ctx, size_t mbsize)
{
	...
	while (tcb != NULL) {
		...
		tcb->time_stamp = ddi_get_lbolt64();
...
igb_tx_recycle_legacy(igb_tx_ring_t *tx_ring)
{
	...
	while (tcb != NULL) {
		...
		if (desc_done) {
			...
			if (tcb->time_stamp < igb->resume_stamp)
				tcb->time_stamp = igb->resume_stamp;
			delta = ddi_get_lbolt64() - tcb->time_stamp;
			break;
		}
	}
	...
	if (desc_num == 0) {
		 ...
		 */
		if ((delta > igb->stall_threshold) &&
		    !(E1000_READ_REG(&igb->hw, E1000_STATUS) &
		    E1000_STATUS_TXOFF)) {
			igb->stall_flag = B_TRUE;
		}
		mutex_exit(&tx_ring->recycle_lock);
		return (0);
	}

	...
	if (ref_cnt == 0) {
		...
		if ((rx_data->flag & IGB_RX_STOPPED) &&
			...
			if (IGB_IS_SUSPENDED(igb) &&
				...
				rx_data->next = igb->pending_rx_data;
				igb->pending_rx_data = rx_data;
			} else {
...
igb_rx_copy(igb_rx_data_t *rx_data, uint32_t index, uint32_t pkt_len)
{
	...
	if (mp == NULL) {
		igb_error(igb, "igb_rx_copy: allocate buffer failed");
		return (NULL);
	}
...
igb_rx_bind(igb_rx_data_t *rx_data, uint32_t index, uint32_t pkt_len)
{
	...
	if (current_rcb->mp == NULL) {
		DESBALLOC(current_rcb);

		/*
		 * If we failed to build a mblk using the current
...
mblk_t *
igb_rx_ring_poll(void *arg, int bytes, int pkts)
{
	...
	if ((igb->igb_state & (IGB_STARTED_TX_RX | IGB_ERROR))
	    != IGB_STARTED_TX_RX) {
		mutex_exit(&rx_ring->rx_lock);
		return (NULL);
	}

	mp = igb_rx(rx_ring, bytes, pkts);
 ...
 */
mblk_t *
igb_rx(igb_rx_ring_t *rx_ring, int poll_bytes, int poll_pkts)
{
...
rx_discard:
	...
	E1000_WRITE_REG(&igb->hw, E1000_RDT(rx_ring->queue), rx_tail);

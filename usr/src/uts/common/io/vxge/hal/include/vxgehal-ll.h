 ...
 * All right Reserved.
 *
 * FileName :	vxgehal-ll.h
 *
 * Description:  API and data structures used by LL driver
 *
...
#include "vxgehal-version.h"
#include "vxge-defs.h"
#include "vxge-os-pal.h"
#include "vxgehal-status.h"
#include "vxgehal-types.h"
#include "vxge-debug.h"
#include "vxge-list.h"
#include "vxge-queue.h"
#include "vxgehal-config.h"
#include "vxgehal-stats.h"
#include "vxgehal-mgmt.h"
#include "vxgehal-mgmtaux.h"
...
/*
 * vxge_hal_rnicif_init - Initializes the vxge_rnicif structure
 *
 * @rnicif: Handle to rnic_if structure
 *
 * This function is called by Link Layer driver to initialize rnic interface
 * structure to be exported to iWARP/RDMA modules.
 */
void vxge_hal_rnicif_init(void *rnicif);
 ...
 *
 */
vxge_hal_status_e vxge_hal_driver_initialize(
 ...
 *
 */
void vxge_hal_driver_terminate(void);
 ...
 * @rth_it_hit: Set, If RTH hash value calculated by the X3100 II hardware
 *	    has a matching entry in the Indirection table.
 * @rth_spdm_hit: Set, If RTH hash value calculated by the X3100 II hardware
 ...
 * @rth_value: Receive Traffic Hashing(RTH) hash value. Produced by X3100 II
 ...
 */
u32 vxge_hal_ring_rxd_size_get(u32 buf_mode);
#pragma inline(vxge_hal_ring_rxd_size_get)
/*
...
u32 vxge_hal_ring_rxds_per_block_get(
			u32 buf_mode);
#pragma inline(vxge_hal_ring_rxds_per_block_get)
/*
 ...
 *
 */
vxge_hal_status_e vxge_hal_ring_rxd_reserve(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_rxd_h *rxdh,
			void **rxd_priv);
...
void vxge_hal_ring_rxd_1b_set(
			...
			u32 size);
#pragma inline(vxge_hal_ring_rxd_1b_set)
...
void vxge_hal_ring_rxd_3b_set(
			...
			u32 sizes[]);
#pragma inline(vxge_hal_ring_rxd_3b_set)
/*
...
void vxge_hal_ring_rxd_5b_set(
			...
			dma_addr_t dma_pointers[],
			u32 sizes[]);
#pragma inline(vxge_hal_ring_rxd_5b_set)
 ...
 *
 */
vxge_hal_status_e vxge_hal_ring_rxd_post(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_rxd_h rxdh);
 ...
 */
vxge_hal_status_e vxge_hal_ring_is_next_rxd_completed(
			vxge_hal_vpath_h vpath_handle);
 ...
 */
vxge_hal_status_e vxge_hal_ring_rxd_next_completed(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_rxd_h *rxdh,
			void **rxd_priv,
			u8 *t_code);
 ...
 */
vxge_hal_status_e vxge_hal_ring_handle_tcode(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_rxd_h rxdh,
			u8 t_code);
...
void vxge_hal_ring_rxd_1b_get(
			...
			dma_addr_t *dma_pointer,
			u32 *pkt_length);
#pragma inline(vxge_hal_ring_rxd_1b_get)
/*
 * xge_hal_ring_rxd_3b_get - Get data from the completed 3-buf
 * descriptor.
 * @vpath_handle: Virtual Path handle.
 * @rxdh: Descriptor handle.
...
void vxge_hal_ring_rxd_3b_get(
			...
			dma_addr_t dma_pointers[],
			u32 sizes[]);
#pragma inline(vxge_hal_ring_rxd_3b_get)
/*
...
void vxge_hal_ring_rxd_5b_get(
			...
			dma_addr_t dma_pointers[],
			int sizes[]);
#pragma inline(vxge_hal_ring_rxd_5b_get)
/*
...
#pragma inline(vxge_hal_ring_rxd_1b_info_get)
...
void vxge_hal_ring_rxd_3b_5b_info_get(
			...
			vxge_hal_rxd_h rxdh,
			vxge_hal_ring_rxd_info_t *rxd_info);
#pragma inline(vxge_hal_ring_rxd_3b_5b_info_get)
 ...
 *
 */
void *vxge_hal_ring_rxd_private_get(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_rxd_h rxdh);
...
/*
 * struct vxge_hal_tcp_hdr_t - Standard TCP Header
 * @source: Source port
 * @dest: Destination port
 * @seq: Sequence Number
 * @ack_seq: Acknowledgement sequence number
 * @doff_res: Data offset
 * @ctrl: Control
 * @window: Window size
 * @check: Checksum
 * @urg_ptr: Priority
 *
 */
typedef struct vxge_hal_tcp_hdr_t {
	u16   source;
	u16   dest;
	u32   seq;
	u32   ack_seq;
	u8	doff_res;
#define	VXGE_HAL_TCP_DATA_OFFSET_MASK				0x50
	u8	ctrl;
#define	VXGE_HAL_TCP_ECN_NS_MASK	0x100
#define	VXGE_HAL_TCP_ECN_CWR_MASK	   0x80
#define	VXGE_HAL_TCP_ECN_ECE_MASK	   0x40
#define	VXGE_HAL_TCP_CTRL_URG_MASK	  0x20
#define	VXGE_HAL_TCP_CTRL_ACK_MASK	  0x10
#define	VXGE_HAL_TCP_CTRL_PSH_MASK	  0x8
#define	VXGE_HAL_TCP_CTRL_RST_MASK	  0x4
#define	VXGE_HAL_TCP_CTRL_SYN_MASK	  0x2
#define	VXGE_HAL_TCP_CTRL_FIN_MASK	  0x1
	u16   window;
	u16   check;
	u16   urg_ptr;
} vxge_hal_tcp_hdr_t;
...
/*
 * struct vxge_hal_ip_hdr_t - Standard IP Header
 * @version_ihl: IP Version
 * @tos: TOS
 ...
 *
 */
typedef struct vxge_hal_ip_hdr_t {
	u8	version_ihl;
#define	VXGE_HAL_IP_VERSION_IHL_MASK				0x45
	u8	tos;
	u16   tot_len;
	u16   id;
	u16   frag_off;
	u8	ttl;
	u8	protocol;
#define	VXGE_HAL_IP_HDR_PROTOCOL_IGMP				2
#define	VXGE_HAL_IP_HDR_PROTOCOL_TCP				6
#define	VXGE_HAL_IP_HDR_PROTOCOL_UDP				17
#define	VXGE_HAL_IP_HDR_PROTOCOL_OSPF				89
#define	VXGE_HAL_IP_HDR_PROTOCOL_SCTP				132
	u16   check;
	u32   saddr;
	u32   daddr;
...
/*
 * struct vxge_hal_sw_lro_t - Software LRO Structure
 *
 * @os_buf: Contains scatter-gather list of xframe-mapped received buffers
 * @os_buf_end: End of scatter-gather list of xframe-mapped received buffers
 * @ll_hdr: link layer header of the first frame; remains intack throughout
 *		the processing
 * @ip_hdr: IP header - gets _collapsed_
 * @tcp_hdr: Transport header - gets _collapsed_
 * @tcp_next_seq_num: Next tcp sequence number
 * @tcp_seq_num: Current tcp seq
 * @tcp_ack_num: Current tcp ack
 * @sg_num: Total number of accumulated (so far) frames
 * @total_length: Total data length
 * @rth_value: Receive side hash value, available from Hercules
 * @frags_len: Total length of the fragments clubbed with the inital frame
 * @ts_off: LRO frame contains time stamp, if (ts_off != -1)
 * @cur_tsval: Current tsval
 * @cur_tsecr: Current tsecr
 * @saw_ts: TBD
 * @window: Window size
 */
typedef struct vxge_hal_sw_lro_t {
	OS_NETSTACK_BUF		os_buf;
	OS_NETSTACK_BUF		os_buf_end;
	u8			*ll_hdr;
	vxge_hal_ip_hdr_t	*ip_hdr;
	vxge_hal_tcp_hdr_t	*tcp_hdr;
...
/*
 * vxge_hal_sw_lro_rx_process: Process Rx Buffer
 * @vpath_handle: Virtual Path handle.
 * @rxd_info: Please see vxge_hal_ring_rxd_info_t {}
 * @eth_hdr: Ethernet header (start of frame buffer)
 * @seglen: Buffer to return segment length
 * @ip: Ip Header. Please see vxge_hal_ip_hdr_t {}
 * @tcp: Buffer to return pointer to vxge_hal_tcp_hdr_t {}
 * @sw_lro: Buffer to return pointer to vxge_hal_sw_lro_t {}
 * @new_sw_lro: Buffer to return pointer to new vxge_hal_sw_lro_t {}
 *
 * Processes the rx buffer and either creates new lro session or
 * appends to the existing lro session
 */
vxge_hal_status_e
vxge_hal_sw_lro_rx_process(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_ring_rxd_info_t *rxd_info,
			u8 *eth_hdr,
			u32 *seglen,
			vxge_hal_sw_lro_t **sw_lro);

/*
 * vxge_hal_update_L3L4_header: Update L3 L4 header
 * @vpath_handle: Virtual Path handle.
 * @lro: Buffer to return pointer to vxge_hal_sw_lro_t {}
 *
 * Updates L3 and L4 headers.
 */
void vxge_hal_update_L3L4_header(vxge_hal_vpath_h vpath_handle,
			vxge_hal_sw_lro_t *lro);

/*
 * vxge_hal_sw_lro_next_session_get: get the next LRO session
 * @vpath_handle: Virtual Path handle.
 ...
 */
vxge_hal_sw_lro_t *
vxge_hal_sw_lro_next_session_get(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_sw_lro_t *sw_lro);

/*
 * vxge_hal_sw_lro_session_close: Close LRO session
 * @vpath_handle: Virtual Path handle.
 * @sw_lro: LRO Session. Please see vxge_hal_sw_lro_t {}
 *
 * Closes the LRO session
 */
void
vxge_hal_sw_lro_session_close(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_sw_lro_t *sw_lro);

#endif /* VXGE_HAL_USE_SW_LRO */
 ...
 *
 */
vxge_hal_status_e vxge_hal_fifo_txdl_reserve(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h *txdlh,
			void **txdl_priv);
...
void vxge_hal_fifo_txdl_cksum_set_bits(
			vxge_hal_txdl_h txdlh,
			u64 cksum_bits);
#pragma inline(vxge_hal_fifo_txdl_cksum_set_bits)
/*
...
void vxge_hal_fifo_txdl_interrupt_type_set(
			vxge_hal_txdl_h txdlh,
			u64 interrupt_type);
#pragma inline(vxge_hal_fifo_txdl_interrupt_type_set)
/*
...
void vxge_hal_fifo_txdl_lso_set(
			...
			u32 encap,
			u32 mss);
#pragma inline(vxge_hal_fifo_txdl_lso_set)
...
u32 vxge_hal_fifo_txdl_lso_bytes_sent(
			vxge_hal_txdl_h txdlh);
#pragma inline(vxge_hal_fifo_txdl_lso_bytes_sent)
...
void vxge_hal_fifo_txdl_vlan_set(
			vxge_hal_txdl_h txdlh,
			u16 vlan_tag);
#pragma inline(vxge_hal_fifo_txdl_vlan_set)
/*
 * vxge_hal_fifo_txdl_buffer_set - Set transmit buffer pointer in the
 * descriptor.
 * @vpath_handle: virtual path handle.
 * @txdlh: Descriptor handle.
 * @frag_idx: Index of the data buffer in the caller's scatter-gather listß
 *	   (of buffers).
 ...
 *
 */
void vxge_hal_fifo_txdl_buffer_set(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h txdlh,
			u32 frag_idx,
			dma_addr_t dma_pointer,
 ...
 *
 */
vxge_hal_status_e vxge_hal_fifo_txdl_buffer_set_aligned(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h txdlh,
			u32 frag_idx,
			void *vaddr,
 ...
 *
 */
void vxge_hal_fifo_txdl_new_frame_set(
	vxge_hal_vpath_h vpath_handle,
	vxge_hal_txdl_h txdlh,
	u32 tagged);
 ...
 *
 */
vxge_hal_status_e vxge_hal_fifo_txdl_post(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h txdlh,
			u32 tagged);
 ...
 * @vpath_handle: Virtual path handle.
 */
vxge_hal_status_e vxge_hal_fifo_is_next_txdl_completed(
			vxge_hal_vpath_h vpath_handle);
 ...
 * @vpath_handle: Virtual path handle.
 */
u32 vxge_hal_fifo_free_txdl_count_get(
			vxge_hal_vpath_h vpath_handle);
 ...
 *
 */
vxge_hal_status_e vxge_hal_fifo_txdl_next_completed(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h *txdlh,
			void **txdl_priv,
			vxge_hal_fifo_tcode_e *t_code);
 ...
 * VXGE_HAL_ERR_CRITICAL	- when encounters critical error.
 */
vxge_hal_status_e vxge_hal_fifo_handle_tcode(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h txdlh,
			vxge_hal_fifo_tcode_e t_code);
 ...
 */
void *vxge_hal_fifo_txdl_private_get(
			vxge_hal_vpath_h vpath_handle,
			vxge_hal_txdl_h txdlh);
 ...
 * @VXGE_HAL_CARD_TITAN: X3100 (PCI-266Mhz) device.
 *
...
/*
 * enum vxge_hal_device_lag_mode_e - VX3100 adapter lag mode
 ...
 *
 * Enumerates VX3100 adapter lag modes.
 *
 */
...
#define	VXGE_HAL_VH_NORMAL_FUNCTION				7
	u64			 function_mode;
#define	VXGE_HAL_FUNCTION_MODE_MULTI_FUNCTION			0
#define	VXGE_HAL_FUNCTION_MODE_SINGLE_FUNCTION			1
#define	VXGE_HAL_FUNCTION_MODE_SRIOV				2
#define	VXGE_HAL_FUNCTION_MODE_MRIOV				3
	u32			 func_id;
	u64			 vpath_mask;
 ...
 * each vpath
 */
vxge_hal_status_e vxge_hal_device_hw_info_get(
			pci_dev_h	pdev,
			pci_reg_h	regh0,
			u8		*bar0,
 ...
 */
vxge_hal_status_e vxge_hal_device_config_default_get(
 ...
 * vxge_hal_device_attr_t {}.
 */
vxge_hal_status_e vxge_hal_device_initialize(
			vxge_hal_device_h *devh,
			vxge_hal_device_attr_t *attr,
			vxge_hal_device_config_t *device_config);

/*
 * vxge_hal_device_intrmode_set - Set intr mode X3100 device.
 * @devh: HAL device handle.
 * @intr_mode: Current intr mode.
 *
 * Set HAL device intr_mode.
 *
 * See also: vxge_hal_device_initialize().
 */
void
vxge_hal_device_intrmode_set(vxge_hal_device_h *devh, u32 intr_mode);
...
vxge_hal_card_e vxge_hal_device_check_id(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_check_id)
/*
...
u32 vxge_hal_device_revision_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_revision_get)
/*
...
u8 *vxge_hal_device_bar0_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_bar0_get)
/*
...
u8 *vxge_hal_device_bar1_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_bar1_get)
/*
...
u8 *vxge_hal_device_bar2_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_bar2_get)
/*
...
void vxge_hal_device_bar0_set(
			vxge_hal_device_h devh,
			u8 *bar0);
#pragma inline(vxge_hal_device_bar0_set)
/*
...
void vxge_hal_device_bar1_set(
			vxge_hal_device_h devh,
			u8 *bar1);
#pragma inline(vxge_hal_device_bar1_set)
/*
...
void vxge_hal_device_bar2_set(
			vxge_hal_device_h devh,
			u8 *bar2);
#pragma inline(vxge_hal_device_bar2_set)
/*
 ...
 *
 */
vxge_hal_status_e vxge_hal_device_enable(
			vxge_hal_device_h devh);
 ...
 *
 */
vxge_hal_status_e vxge_hal_device_disable(
			vxge_hal_device_h devh);
...
vxge_hal_status_e vxge_hal_device_pci_info_get(
			...
			vxge_hal_pci_e_signalling_rate_e *signalling_rate,
			vxge_hal_pci_e_link_width_e *link_width);
#pragma inline(vxge_hal_device_pci_info_get)
/*
 ...
 * Returns: link state.
 */
vxge_hal_device_link_state_e vxge_hal_device_link_state_test(
				vxge_hal_device_h devh);
...
vxge_hal_device_link_state_e vxge_hal_device_link_state_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_link_state_get)
/*
 ...
 * Returns: link state.
 */
vxge_hal_device_link_state_e vxge_hal_device_link_state_poll(
			vxge_hal_device_h devh);
...
vxge_hal_device_data_rate_e vxge_hal_device_data_rate_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_data_rate_get)
/*
 ...
 * Returns: data rate.
 */
vxge_hal_device_data_rate_e vxge_hal_device_data_rate_poll(
			vxge_hal_device_h devh);
 ...
 */
vxge_hal_device_lag_mode_e vxge_hal_device_lag_mode_get(
			vxge_hal_device_h devh);
 ...
 *
 */
vxge_hal_status_e vxge_hal_device_reset(
			vxge_hal_device_h devh);
...
#pragma inline(vxge_hal_device_terminating)
/*
...
void vxge_hal_device_private_set(
			vxge_hal_device_h devh,
			void *data);
#pragma inline(vxge_hal_device_private_set)
/*
...
void *vxge_hal_device_private_get(
			vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_private_get)
/*
 ...
 */
u64 vxge_hal_device_is_traffic_interrupt(u64 reason, u32 vp_id);
#pragma inline(vxge_hal_device_is_traffic_interrupt)
 ...
 * frame initialization sequence.
 *
 * See also: vxge_hal_device_intr_disable()
 */
void vxge_hal_device_intr_enable(
			vxge_hal_device_h devh);
 ...
 */
void vxge_hal_device_intr_disable(
			vxge_hal_device_h devh);
 ...
 */
void vxge_hal_device_unmask_all(
			vxge_hal_device_h devh);
 ...
 */
vxge_hal_status_e vxge_hal_device_begin_irq(
			vxge_hal_device_h devh,
			u32 skip_alarms,
			u64 *reason);
 ...
 * vxge_hal_fifo_callback_f {}.
 */
vxge_hal_status_e vxge_hal_device_continue_irq(
			vxge_hal_device_h devh);
 ...
 */
vxge_hal_status_e vxge_hal_device_handle_irq(
			vxge_hal_device_h devh,
			u32 skip_alarms);
 ...
 */
void vxge_hal_device_unmask_tx(
			vxge_hal_device_h devh);
 ...
 */
void vxge_hal_device_unmask_rx(
			vxge_hal_device_h devh);
 ...
 */
void vxge_hal_device_mask_tx_rx(
			vxge_hal_device_h devh);
 ...
 */
void vxge_hal_device_clear_tx_rx(
			vxge_hal_device_h devh);
 ...
 */
void vxge_hal_device_unmask_tx_rx(
			vxge_hal_device_h devh);
 ...
 * non-zero otherwise.
 */
int vxge_hal_device_msix_mode(vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_msix_mode)
 ...
 * This routine is used to dynamically change the debug output
 */
void vxge_hal_device_debug_set(
	vxge_hal_device_h devh,
	vxge_debug_level_e level,
	u32 mask);
 ...
 * This routine returns the current debug level set
 */
u32 vxge_hal_device_debug_level_get(vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_debug_level_get)
 ...
 */
u32 vxge_hal_device_debug_mask_get(vxge_hal_device_h devh);
#pragma inline(vxge_hal_device_debug_mask_get)
/*
 ...
 * status
 */
vxge_hal_status_e vxge_hal_device_getpause_data(
				vxge_hal_device_h devh,
				u32 port,
				u32 *tx,
				u32 *rx);
 ...
 * @VXGE_HAL_PCIE_FUNC_MODE_MF4_VP4: Multi Function
 *             - 4 functions, 4 Paths/Function ( funcs 4-7 have no resources )
 */
...
enum vxge_hal_nwif_cmds {
	...
	VXGE_HAL_XMAC_NWIF_Cmd_CfgDualPort_L2SwitchEnabl	= 0x9,
 ...
 *
 */
vxge_hal_status_e vxge_hal_mrpcim_pcie_func_mode_set(
			vxge_hal_device_h devh,
 ...
 *
 */
vxge_hal_status_e vxge_hal_mrpcim_vpath_qos_set(
			vxge_hal_device_h devh,
			u32 vp_id,
			u32 priority,
			u32 min_bandwidth,
 ...
 *
 */
vxge_hal_status_e vxge_hal_mrpcim_vpath_qos_get(
			vxge_hal_device_h devh,
			u32 vp_id,
			u32 *priority,
			u32 *min_bandwidth,
 ...
 * status
 */
vxge_hal_status_e vxge_hal_mrpcim_getpause_data(
				vxge_hal_device_h devh,
				u32 port,
				u32 *tx,
				u32 *rx);
...
vxge_hal_status_e vxge_hal_mrpcim_setpause_data(
				vxge_hal_device_h devh,
				u32 port,
				u32 tx,
				u32 rx);
 ...
 */
vxge_hal_status_e vxge_hal_vpath_close(
			vxge_hal_vpath_h vpath_handle);
 ...
 */
vxge_hal_status_e vxge_hal_vpath_obj_count_get(
			vxge_hal_vpath_h vpath_handle,
 ...
 * 0 on success.
 */
vxge_hal_status_e vxge_hal_rldram_test(
			vxge_hal_device_h devh,
			u64 *data);
...
vxge_hal_read_eeprom
vxge_hal_write_eeprom
vxge_hal_register_test(hldev, &data[0]))
vxge_hal_rldram_test(hldev, &data[3]))
vxge_hal_eeprom_test(hldev, &data[1]))
 ...
 * @dma_h: Physical address of the block
...
#pragma inline(vxge_hal_isfifoenable)


__EXTERN_END_DECLS

#include "vxge-os-debug.h"

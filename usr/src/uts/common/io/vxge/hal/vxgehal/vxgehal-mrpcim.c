 ...
 * All right Reserved.
 *
 * FileName :   vxgehal-mrpcim.c
 *
 * Description:  Implementation of API to program MRPCIM
 *
...
vxge_hal_mrpcim_serial_number_get(vxge_hal_device_h devh)
{
	...
	if (!(hldev->access_rights & VXGE_HAL_DEVICE_ACCESS_RIGHT_MRPCIM)) {
		vxge_hal_trace_log_stats(hldev, NULL_VPID,
		    "<==  %s:%s:%d Result = %d",
			...
			u32 func)
{
	...
	val64 = vxge_os_pio_mem_read64(pdev, regh0,
	    &toc->toc_vpath_pointer[0]);

	vpath_reg = (void *)(bar0 + val64);

	val64 = __vxge_hal_vpath_vpath_map_get(
	    pdev,
	    regh0,
	    0,
	    0,
			...
			vxge_hal_pcie_function_mode_e func_mode)
{
	...
	vxge_hal_trace_log_driver(hldev, NULL_VPID,
	    "devh = 0x"VXGE_OS_STXFMT", func_mode = %d",
	    (ptr_t)devh, func_mode);
	...
	status =  __vxge_hal_vpath_pcie_func_mode_set(hldev,
	    hldev->first_vp_id,
	    fmode);
			...
			u32 length)
{
	...
	for (index = 0; index < VXGE_HAL_MAX_VIRTUAL_PATHS; index++) {
		...
		if (!((vpath_mask) & mBIT(index)))
			continue;

		val64 = vxge_os_pio_mem_read64(pdev, regh0,
		    &toc->toc_vpmgmt_pointer[index]);
		...
		if (!(__vxge_hal_device_access_rights_get(host_type, func_id) &
		    VXGE_HAL_DEVICE_ACCESS_RIGHT_MRPCIM)) {

			vxge_hal_trace_log_mrpcim(NULL_HLDEV, NULL_VPID,
			    "<==  %s:%s:%d Result = %d",
			    __FILE__, __func__, __LINE__,
			    VXGE_HAL_ERR_PRIVILAGED_OPEARATION);
		...
		val64 = vxge_os_pio_mem_read64(pdev, regh0,
		    &toc->toc_vpath_pointer[index]);

		vpath_reg = (void *)(bar0 + val64);

		status = __vxge_hal_vpath_fw_upgrade(
		    pdev,
		    regh0,
		    index,
		    vpath_reg,
	...
	if (status == VXGE_HAL_OK) {
		...
		val64 =
		    vxge_os_pio_mem_read64(pdev,
		    regh0, &toc->toc_mrpcim_pointer);
			...
			u32 max_bandwidth)
{
	...
	if ((status =
	    __vxge_hal_vpath_qos_config_check(&config)) != VXGE_HAL_OK) {
		vxge_hal_trace_log_mrpcim(hldev, NULL_VPID,
		    "<==  %s:%s:%d Result = %d",
		    __FILE__, __func__, __LINE__, status);
...
/*
 * __vxge_hal_mrpcim_mdio_access - Access the MDIO device
 * @devh: HAL Device handle.
 * @port: Port id
...
vxge_hal_mrpcim_reset(vxge_hal_device_h devh)
{
	...
	(void) __vxge_hal_srpcim_reset(hldev);
...
vxge_hal_mrpcim_reset_poll(vxge_hal_device_h devh)
{
	...
	status =  __vxge_hal_mrpcim_mac_configure(hldev);
	...
	status =  __vxge_hal_mrpcim_lag_configure(hldev);
	...
	if ((hldev->header.config.mrpcim_config.lag_config.lag_en ==
	    VXGE_HAL_LAG_LAG_EN_DISABLE) ||
	    ((hldev->header.config.mrpcim_config.lag_config.lag_en ==
	    VXGE_HAL_LAG_LAG_EN_ENABLE) &&
	    (hldev->header.config.mrpcim_config.lag_config.lag_mode ==
	    VXGE_HAL_LAG_LAG_MODE_LAG) &&
	    (hldev->header.config.mrpcim_config.lag_config.la_mode_config.
	    distrib_alg_sel == VXGE_HAL_LAG_DISTRIB_ALG_SEL_SRC_VPATH) &&
	    (hldev->header.config.mrpcim_config.lag_config.la_mode_config.
	    distrib_dest == 0))) {

		vxge_os_pio_mem_write64(hldev->header.pdev,
		    hldev->header.regh0,
		    hldev->header.config.mrpcim_config.vpath_to_wire_port_map,
		    &hldev->mrpcim_reg->asic_ntwk_cfg_port_num);

		vxge_os_pio_mem_write64(hldev->header.pdev,
		    hldev->header.regh0,
		    hldev->
		    header.config.mrpcim_config.vpath_to_wire_port_map_en,
		    &hldev->mrpcim_reg->asic_ntwk_cfg_show_port_info);
	...
	(void) __vxge_hal_srpcim_reset_poll(hldev);
...
/*
 * __vxge_hal_mrpcim_xpak_counter_check -  check the Xpak error count
 * and log the msg
 * @hldev: pointer to __vxge_hal_device_t structure
 * @port: Port number
					...
					u32 port, u32 type, u32 value)
{
	...
	if (value == 3) {
		...
		case VXGE_HAL_XPAK_ALARM_EXCESS_LASER_OUTPUT:
			...
			break;
		default:
			vxge_hal_info_log_stats(hldev, NULL_VPID, "%s",
 ...
 */
vxge_hal_status_e vxge_hal_mrpcim_xpak_stats_poll(
	vxge_hal_device_h devh, u32 port)
{
	...
	vxge_hal_trace_log_stats(hldev, NULL_VPID,
	    "hldev = 0x"VXGE_OS_STXFMT", port = %d",
	    (ptr_t)hldev, port);
	...
	if (val &
		...
		__vxge_hal_mrpcim_xpak_counter_check(hldev, port,
		    VXGE_HAL_XPAK_ALARM_EXCESS_TEMP,
		    hldev->mrpcim->xpak_stats[port].excess_temp);
	} else {
		hldev->mrpcim->xpak_stats[port].excess_temp = 0;
			...
			vxge_hal_device_h devh)
{
	...
	if (hldev->header.config.stats_read_method ==
		...
		status = __vxge_hal_mrpcim_stats_get(
		    hldev,
		    hldev->mrpcim->mrpcim_stats);
	}
 ...
 */
vxge_hal_status_e vxge_hal_device_mrpcim_stats_disable(
			vxge_hal_device_h devh)
{
	u64 val64;
	vxge_hal_status_e status = VXGE_HAL_OK;
 ...
 */
vxge_hal_status_e vxge_hal_mrpcim_stats_get(
			vxge_hal_device_h devh,
 ...
 */
vxge_hal_status_e __vxge_hal_mrpcim_stats_get(
 ...
 *
 */
vxge_hal_status_e vxge_hal_mrpcim_stats_clear(
	vxge_hal_device_h devh)
{
	u32 i;
	u64 stat;
...
/*
 * __vxge_hal_mrpcim_mac_configure - Initialize mac
 * @hldev: hal device.
 *
 * Initializes mac
 *
...
/*
 * __vxge_hal_mrpcim_lag_configure - Initialize LAG registers
 * @hldev: hal device.
 *
...
__vxge_hal_mrpcim_lag_configure(__vxge_hal_device_t *hldev)
{
	...
	if (lag_config->ap_mode_config.pref_active_port !=
		...
		val64 |= VXGE_HAL_LAG_ACTIVE_PASSIVE_CFG_PREF_ACTIVE_PORT_NUM;
		else
		val64 &= ~VXGE_HAL_LAG_ACTIVE_PASSIVE_CFG_PREF_ACTIVE_PORT_NUM;
	}
	...
	if (lag_config->
	    ap_mode_config.alt_aggr != VXGE_HAL_LAG_ALT_AGGR_DEFAULT) {
		if (lag_config->
		    ap_mode_config.alt_aggr == VXGE_HAL_LAG_ALT_AGGR_1)
...
/*
 * __vxge_hal_mrpcim_get_vpd_data - Getting vpd_data.
 *
...
__vxge_hal_mrpcim_get_vpd_data(__vxge_hal_device_t *hldev)
{
	...
	for (index = 0; index < VXGE_HAL_VPD_BUFFER_SIZE; index += 4) {
		...
		if (count >= max_count) {
			vxge_hal_info_log_device(hldev, NULL_VPID,
			    "%s:ERR, Reading VPD data failed", __func__);
			fail = 1;
			break;
		}
	...
	if (!fail) {
		...
		if (vpd_data[1] < VXGE_HAL_VPD_LENGTH) {
			(void *) memset(hldev->mrpcim->vpd_data.product_name,
			    0, vpd_data[1]);
			(void *) memcpy(hldev->mrpcim->vpd_data.product_name,
	...
	vxge_hal_trace_log_mrpcim(hldev, NULL_VPID,
	    "<==  %s:%s:%d Result = %d", __FILE__, __func__, __LINE__, fail);
}
			...
			vxge_hal_lag_config_t *lconfig)
{
	...
	status =  __vxge_hal_device_lag_config_check(lconfig);
...
/*
 * __vxge_hal_mrpcim_initialize - Initialize mrpcim
 * @hldev: hal device.
 *
 * Initializes mrpcim
 *
 * See also: __vxge_hal_mrpcim_terminate()
 */
vxge_hal_status_e
__vxge_hal_mrpcim_initialize(__vxge_hal_device_t *hldev)
{
	...
	hldev->mrpcim = (__vxge_hal_mrpcim_t *)
	    vxge_os_malloc(hldev->header.pdev, sizeof (__vxge_hal_mrpcim_t));
	...
	vxge_os_memzero(hldev->mrpcim, sizeof (__vxge_hal_mrpcim_t));

	__vxge_hal_mrpcim_get_vpd_data(hldev);

	hldev->mrpcim->mrpcim_stats_block =
	    __vxge_hal_blockpool_block_allocate(hldev, VXGE_OS_HOST_PAGE_SIZE);
...
/*
 * __vxge_hal_mrpcim_terminate - Terminates mrpcim
 * @hldev: hal device.
 *
 * Terminates mrpcim.
 *
 * See also: __vxge_hal_mrpcim_initialize()
 */
vxge_hal_status_e
__vxge_hal_mrpcim_terminate(__vxge_hal_device_t *hldev)
{
	...
	if (hldev->mrpcim->mrpcim_stats_block != NULL) {
		__vxge_hal_blockpool_block_free(devh,
		    hldev->mrpcim->mrpcim_stats_block);
		hldev->mrpcim->mrpcim_stats_block = NULL;
	}

	vxge_os_free(hldev->header.pdev,
	    hldev->mrpcim, sizeof (__vxge_hal_mrpcim_t));

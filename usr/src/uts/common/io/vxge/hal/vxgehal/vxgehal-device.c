...
/*ARGSUSED*/
void
__vxge_hal_pio_mem_write32_upper(pci_dev_h pdev,
	pci_reg_h regh, u32 val, void *addr)
{
...
/*
 * __vxge_hal_pio_mem_write32_lower
 *
 * Endiann-aware implementation of vxge_os_pio_mem_write32().
 * Since X3100 has 64bit registers, we differintiate uppper and lower
 * parts.
...
/*ARGSUSED*/
void
__vxge_hal_pio_mem_write32_lower(pci_dev_h pdev, pci_reg_h regh, u32 val,
			    void *addr)
{
#if defined(VXGE_OS_HOST_BIG_ENDIAN) && !defined(VXGE_OS_PIO_LITTLE_ENDIAN)
	...
	void *val)
{
	...
	vxge_hal_trace_log_device(devh, NULL_VPID,
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		status = __vxge_hal_vpath_pci_read(hldev, i,
		    offset, length, val);
...
/*
 * __vxge_hal_device_pci_caps_list_process
 * @hldev: HAL device handle.
 *
 * Process PCI capabilities and initialize the offsets
 */
void
__vxge_hal_device_pci_caps_list_process(__vxge_hal_device_t *hldev)
{
	u8 cap_id;
	u16 ext_cap_id;
	u16 next_ptr;
	...
	while (next_ptr != 0) {
		...
		case VXGE_HAL_PCI_CAP_ID_SECDEV:
			vxge_hal_info_log_device(hldev, NULL_VPID,
			    "Unexpected Capability = %d", cap_id);
			break;
		default:
			vxge_hal_info_log_device(hldev, NULL_VPID,
			    "Unknown capability = %d", cap_id);
			break;
		}
	...
	while (next_ptr != 0) {
		...
		default:
			vxge_hal_info_log_device(hldev, NULL_VPID,
			    "Unknown capability = %d", ext_cap_id);
			break;
		}
 ...
 */
void
__vxge_hal_device_pci_e_init(__vxge_hal_device_t *hldev)
{
	int i;
	u16 cmd;
	u32 *ptr_32;
	...
	ptr_32 = (void *)&hldev->pci_config_space_bios;
	...
	__vxge_hal_device_pci_caps_list_process(hldev);
	...
	if (hldev->header.config.latency_timer &&
	    (hldev->header.config.latency_timer !=
	    VXGE_HAL_USE_BIOS_DEFAULT_LATENCY)) {
		vxge_os_pci_write8(hldev->header.pdev, hldev->header.cfgh,
		    vxge_offsetof(vxge_hal_pci_config_le_t,
		    latency_timer),
		    (u8)hldev->header.config.latency_timer);
	}
	/* Read back latency timer to reflect it into user level */
	(void) __vxge_hal_vpath_pci_read(hldev,
	    hldev->first_vp_id,
	    vxge_offsetof(vxge_hal_pci_config_le_t, latency_timer),
	    1,
	    &latency_timer);
	hldev->header.config.latency_timer = latency_timer;

	/* TODO: Program PCI-E config space */
...
/*
 * __vxge_hal_device_bus_master_enable
 * @hldev: HAL device handle.
 *
 * Enable bus mastership.
 */
static void
__vxge_hal_device_bus_master_enable(__vxge_hal_device_t *hldev)
{
	u16 cmd;
	u16 bus_master = 4;
...
/*
 * __vxge_hal_device_register_poll
...
/*
 * __vxge_hal_device_reg_addr_get
 ...
 */
vxge_hal_status_e
__vxge_hal_device_reg_addr_get(__vxge_hal_device_t *hldev)
{
	u64 val64;
	u32 i;
	vxge_hal_status_e status = VXGE_HAL_OK;
	...
	hldev->legacy_reg = (void *)hldev->header.bar0;

	status = __vxge_hal_legacy_swapper_set(hldev->header.pdev,
	    hldev->header.regh0,
	    hldev->legacy_reg);
	...
	hldev->toc_reg = (void *)(hldev->header.bar0+val64);
	...
	for (i = 0; i < VXGE_HAL_TITAN_PCICFGMGMT_REG_SPACES; i++) {
		...
		hldev->pcicfgmgmt_reg[i] =
		    (void *)(hldev->header.bar0 + val64);


		vxge_hal_info_log_device(hldev, NULL_VPID, "PCICFG_MGMT[%d] = "
	...
	vxge_hal_info_log_device(hldev, NULL_VPID,
	    "MEM REPAIR = 0x"VXGE_OS_STXFMT, (ptr_t)hldev->mrpcim_reg);
	...
	for (i = 0; i < VXGE_HAL_TITAN_SRPCIM_REG_SPACES; i++) {
		...
		vxge_hal_info_log_device(hldev, NULL_VPID, "SRPCIM[%d] ="
		    "0x"VXGE_OS_STXFMT, i, (ptr_t)hldev->srpcim_reg[i]);
	...
	for (i = 0; i < VXGE_HAL_TITAN_VPMGMT_REG_SPACES; i++) {
		...
		vxge_hal_info_log_device(hldev, NULL_VPID, "VPMGMT[%d] ="
		    "0x"VXGE_OS_STXFMT, i, (ptr_t)hldev->vpmgmt_reg[i]);
	...
	for (i = 0; i < VXGE_HAL_TITAN_VPATH_REG_SPACES; i++) {
		...
		vxge_hal_info_log_device(hldev, NULL_VPID, "VPATH[%d] ="
		    "0x"VXGE_OS_STXFMT, i, (ptr_t)hldev->vpath_reg[i]);
	...
	default:
		vxge_hal_info_log_device(hldev, NULL_VPID,
		    "Invalid BIR = 0x"VXGE_OS_STXFMT,
		    (ptr_t)VXGE_HAL_TOC_GET_KDFC_INITIAL_BIR(val64));
		break;
	}

	vxge_hal_info_log_device(hldev, NULL_VPID,
	    "KDFC = 0x"VXGE_OS_STXFMT, (ptr_t)hldev->kdfc);
	...
	vxge_hal_info_log_device(hldev, NULL_VPID,
	    "USDC = 0x"VXGE_OS_STXFMT, (ptr_t)hldev->usdc);
	...
	if (status != VXGE_HAL_OK) {
		/*EMPTY*/
		vxge_hal_err_log_device(hldev, NULL_VPID,
		    "%s:vpath_rst_in_prog is not cleared", __func__);
	}
 ...
 */
void
__vxge_hal_device_id_get(__vxge_hal_device_t *hldev)
{
	vxge_assert(hldev);
...
/*
 * __vxge_hal_device_access_rights_get: Get Access Rights of the driver
 * @host_type: Host type.
 * @func_id: Function Id
 *
 ...
 */
u32
__vxge_hal_device_access_rights_get(u32 host_type, u32 func_id)
{
...
/*
 * __vxge_hal_device_host_info_get
 * @hldev: HAL Device object.
 *
 * This routine returns the host type assignments
 */
void
__vxge_hal_device_host_info_get(__vxge_hal_device_t *hldev)
{
	u64 val64;
	u32 i;
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		hldev->access_rights = __vxge_hal_device_access_rights_get(
		...
		if (hldev->access_rights &
			...
			hldev->manager_up =
			    __vxge_hal_ifmsg_is_manager_up(val64);
		}
...
/*
 * __vxge_hal_device_pci_e_info_get - Get PCI_E bus informations such as
 * link_width and signalling rate
 * @hldev: HAL device.
...
__vxge_hal_device_pci_e_info_get(
...

{
	...
	vxge_hal_trace_log_device(hldev, NULL_VPID,
	    "hldev = 0x"VXGE_OS_STXFMT", signalling_rate = 0x"VXGE_OS_STXFMT", "
...
/*
 * __vxge_hal_device_hw_initialize
 * @hldev: HAL device handle.
 *
 * Initialize X3100-V hardware.
 */
vxge_hal_status_e
__vxge_hal_device_hw_initialize(__vxge_hal_device_t *hldev)
{
	vxge_hal_status_e status = VXGE_HAL_OK;
	...
	__vxge_hal_device_pci_e_init(hldev);

	/* update the pci mode, frequency, and width */
	if (__vxge_hal_device_pci_e_info_get(hldev,
		 ...
		 */
		vxge_hal_err_log_device(hldev, NULL_VPID,
		    "unable to get pci info == > %s : %d", __func__, __LINE__);
	}

	if (hldev->access_rights & VXGE_HAL_DEVICE_ACCESS_RIGHT_SRPCIM) {
		status = __vxge_hal_srpcim_initialize(hldev);
	}

	if (hldev->access_rights & VXGE_HAL_DEVICE_ACCESS_RIGHT_MRPCIM) {
		status = __vxge_hal_mrpcim_initialize(hldev);
	}
...
vxge_hal_device_reset(vxge_hal_device_h devh)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		if (status != VXGE_HAL_OK) {
			/*EMPTY*/
			vxge_hal_err_log_device(devh, NULL_VPID,
			    "vpath %d Reset Failed", i);
		}
...
vxge_hal_device_mrpcim_reset_poll(vxge_hal_device_h devh)
{
	...
	status =  __vxge_hal_ifmsg_srpcim_to_vpath_reset_end_poll(
...
vxge_hal_device_status(vxge_hal_device_h devh, u64 *hw_status)
{
	...
	*hw_status = vxge_os_pio_mem_read64(hldev->header.pdev,
	...
	vxge_hal_trace_log_device(devh, NULL_VPID,
	    "Adapter_Status = 0x"VXGE_OS_LLXFMT, *hw_status);
	...
	vxge_hal_trace_log_device(devh, NULL_VPID,
	    "<==  %s:%s:%d Result = 0x"VXGE_OS_STXFMT,
...
vxge_hal_device_is_slot_freeze(vxge_hal_device_h devh)
{
	...
	if (adapter_status == VXGE_HAL_ALL_FOXES) {
		u64 dummy;
		dummy = vxge_os_pio_mem_read64(hldev->header.pdev,
		    hldev->header.regh0,
		    &hldev->common_reg->adapter_status);

		vxge_os_printf(">>> Slot is frozen!\n");
		brkpoint(0);
	}
#endif
	vxge_hal_trace_log_device(devh, NULL_VPID,
			...
			vxge_hal_device_h devh)
{
	...
	vxge_hal_trace_log_device_irq(devh, NULL_VPID,
	    "==>  %s:%s:%d", __FILE__, __func__, __LINE__);
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		(void) __vxge_hal_vpath_intr_enable(&hldev->virtual_paths[i]);
	}
			...
			vxge_hal_device_h devh)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		(void) __vxge_hal_vpath_intr_disable(&hldev->virtual_paths[i]);
	}
			...
			u64 *reason)
{
	...
	if (vxge_os_unlikely(!val64)) {
		...
		*reason	= 0;
		...
		vxge_hal_info_log_device_irq(devh, NULL_VPID,
		    "wrong_isr general_int_status = 0x%llx", val64);
		vxge_hal_trace_log_device_irq(devh, NULL_VPID,
		    "<==  %s:%s:%d Result = %d",
...
/*
 * __vxge_hal_device_handle_link_up_ind
 * @hldev: HAL device handle.
 *
 ...
 */
vxge_hal_status_e
__vxge_hal_device_handle_link_up_ind(__vxge_hal_device_t *hldev)
{
	vxge_assert(hldev);

	vxge_hal_trace_log_device_irq(hldev, NULL_VPID,
	    "==>  %s:%s:%d", __FILE__, __func__, __LINE__);
...
/*
 * __vxge_hal_device_handle_link_down_ind
 * @hldev: HAL device handle.
 *
 ...
 */
vxge_hal_status_e
__vxge_hal_device_handle_link_down_ind(__vxge_hal_device_t *hldev)
{
	vxge_assert(hldev);
			...
			vxge_hal_device_h devh)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		status =
		    __vxge_hal_vpath_link_state_test(&hldev->virtual_paths[i]);
			...
			vxge_hal_device_h devh)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		link_state =
		    __vxge_hal_vpath_link_state_poll(&hldev->virtual_paths[i]);
			...
			vxge_hal_device_h devh)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		data_rate =
		    __vxge_hal_vpath_data_rate_poll(&hldev->virtual_paths[i]);
			...
			vxge_hal_device_h devh)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		lag_mode =
		    __vxge_hal_vpath_lag_mode_get(&hldev->virtual_paths[i]);
...
/*
 * __vxge_hal_device_handle_error - Handle error
 * @hldev: HAL device
 * @vp_id: Vpath Id
 ...
 */
void
__vxge_hal_device_handle_error(
		__vxge_hal_device_t *hldev,
		u32 vp_id,
		vxge_hal_event_e type)
{
	...
	vxge_hal_trace_log_device_irq(hldev, vp_id,
	    "==>  %s:%s:%d", __FILE__, __func__, __LINE__);
	...
	switch (type) {
		...
		case VXGE_HAL_EVENT_KDFCCTL:
			break;
		case VXGE_HAL_EVENT_RESET_START:
			break;
		case VXGE_HAL_EVENT_RESET_COMPLETE:
			break;
			...
			vxge_hal_device_hw_info_t *hw_info)
{
	...
	legacy_reg = (void *)bar0;

	status = __vxge_hal_legacy_swapper_set(pdev, regh0, legacy_reg);
	...
	val64 =
	    vxge_os_pio_mem_read64(pdev, regh0, &toc->toc_common_pointer);

	common_reg = (void *)(bar0 + val64);
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		val64 = vxge_os_pio_mem_read64(pdev, regh0,
		    &toc->toc_vpmgmt_pointer[i]);

		vpmgmt_reg = (void *)(bar0 + val64);
		...
		if (__vxge_hal_device_access_rights_get(hw_info->host_type,
		    hw_info->func_id) & VXGE_HAL_DEVICE_ACCESS_RIGHT_MRPCIM) {

			val64 = vxge_os_pio_mem_read64(pdev, regh0,
			    &toc->toc_mrpcim_pointer);

			mrpcim_reg = (void *)(bar0 + val64);
		...
		val64 = vxge_os_pio_mem_read64(pdev, regh0,
		    &toc->toc_vpath_pointer[i]);

		vpath_reg = (void *)(bar0 + val64);

		(void) __vxge_hal_vpath_fw_flash_ver_get(pdev, regh0, i,
		    vpath_reg,
		    &hw_info->fw_version,
		    &hw_info->fw_date,
		...
		(void) __vxge_hal_vpath_card_info_get(pdev, regh0, i, vpath_reg,
		    hw_info->serial_number,
		    hw_info->part_number,
		    hw_info->product_description);

		(void) __vxge_hal_vpath_pmd_info_get(pdev, regh0, i, vpath_reg,
		    &hw_info->ports,
		    &hw_info->pmd_port0,
		    &hw_info->pmd_port1);

		hw_info->function_mode =
		    __vxge_hal_vpath_pci_func_mode_get(pdev, regh0, i,
		    vpath_reg);
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		status =  __vxge_hal_vpath_hw_addr_get(pdev, regh0, i,
		    vpath_reg, hw_info->mac_addrs[i],
 ...
 *
 * When done, the ULD allocates sizeof (__vxge_hal_device_t) bytes for HAL
 ...
 *
 * See also: __vxge_hal_device_terminate(), vxge_hal_status_e {}
	...
	vxge_hal_device_config_t *device_config)
{
	...
	status = __vxge_hal_device_config_check(device_config);
	...
	hldev = (__vxge_hal_device_t *)vxge_os_malloc(attr->pdev,
	    sizeof (__vxge_hal_device_t));
	...
	vxge_os_memzero(hldev, sizeof (__vxge_hal_device_t));

	hldev->header.magic = VXGE_HAL_DEVICE_MAGIC;

	__vxge_hal_channel_init_pending_list(hldev);
	...
	vxge_hal_trace_log_device(hldev, NULL_VPID, "==>  %s:%s:%d",
	    __FILE__, __func__, __LINE__);
#endif

	vxge_hal_info_log_device(hldev, NULL_VPID,
	    "device 0x"VXGE_OS_STXFMT" is initializing",
	    (ptr_t)hldev);
	...
	if ((status = __vxge_hal_device_reg_addr_get(hldev)) != VXGE_HAL_OK) {
		vxge_hal_trace_log_device(hldev, NULL_VPID,
		    "<==  %s:%s:%d Result = %d",
		    __FILE__, __func__, __LINE__, status);
	...
	__vxge_hal_device_id_get(hldev);

	__vxge_hal_device_host_info_get(hldev);
	...
	if (__vxge_hal_blockpool_create(hldev,
	    &hldev->block_pool,
		...
		vxge_hal_info_log_device(hldev, NULL_VPID,
		    "%s:__vxge_hal_blockpool_create failed", __func__);
		vxge_hal_trace_log_device(hldev, NULL_VPID,
		    "<==  %s:%s:%d Result = %d",
		    __FILE__, __func__, __LINE__,
		    VXGE_HAL_ERR_OUT_OF_MEMORY);
	...
	status = __vxge_hal_device_hw_initialize(hldev);
	...
	if (hldev->dump_buf == NULL) {
		vxge_hal_info_log_device(hldev, NULL_VPID,
		    "%s:vxge_os_malloc failed ", __func__);
		vxge_hal_trace_log_device(hldev, NULL_VPID,
		    "<==  %s:%s:%d Result = %d",
		    __FILE__, __func__, __LINE__,
		    VXGE_HAL_ERR_OUT_OF_MEMORY);
...
{
	__vxge_hal_device_t *hldev = (__vxge_hal_device_t *)devh;
	hldev->header.config.intr_mode = intr_mode;
}
...
vxge_hal_device_terminate(vxge_hal_device_h devh)
{
	...
	if (hldev->srpcim != NULL) {
		(void) __vxge_hal_srpcim_terminate(hldev);
	}

	if (hldev->mrpcim != NULL) {
		(void) __vxge_hal_mrpcim_terminate(hldev);
	}

	__vxge_hal_channel_destroy_pending_list(hldev);


	__vxge_hal_blockpool_destroy(&hldev->block_pool);
	...
	vxge_os_free(hldev->header.pdev, hldev, sizeof (__vxge_hal_device_t));
			...
			vxge_hal_device_h devh)
{
	...
	__vxge_hal_device_bus_master_enable(hldev);
 ...
 */
vxge_hal_status_e vxge_hal_device_hw_stats_enable(
				vxge_hal_device_h devh)
{
	u32 i;
	u64 val64;
 ...
 */
vxge_hal_status_e vxge_hal_device_hw_stats_disable(
			vxge_hal_device_h devh)
{
	u32 i;
	u64 val64;
 ...
 */
vxge_hal_status_e vxge_hal_device_hw_stats_get(
			vxge_hal_device_h devh,
 ...
 */
vxge_hal_status_e vxge_hal_device_sw_stats_get(
			vxge_hal_device_h devh,
 ...
 */
vxge_hal_status_e vxge_hal_device_stats_get(
			vxge_hal_device_h devh,
				...
				vxge_hal_device_xmac_stats_t *xmac_stats)
{
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		status = __vxge_hal_vpath_xmac_tx_stats_get(
		    &hldev->virtual_paths[i],
		...
		status = __vxge_hal_vpath_xmac_rx_stats_get(
		    &hldev->virtual_paths[i],
		    ...
		    u32 on_off)
{
	...
	vxge_hal_trace_log_device(devh, NULL_VPID,
	    "devh = 0x"VXGE_OS_STXFMT", port = %d, on_off = %d",
	    (ptr_t)devh, port, on_off);
	...
	status = __vxge_hal_vpath_flick_link_led(
	    hldev,
	    hldev->first_vp_id,
	    port,
	    on_off);
				...
				u32 *rx)
{
	...
	if (port >= VXGE_HAL_MAC_MAX_PORTS) {
		vxge_hal_trace_log_device(devh, NULL_VPID,
		    "<==  %s:%s:%d  Result: %d", __FILE__, __func__,
		    __LINE__, VXGE_HAL_ERR_INVALID_PORT);

...
/*
 * __vxge_hal_driver_config_check - Check driver configuration.
...
/*
 * __vxge_hal_device_wire_port_config_check - Check wire port configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_wire_port_config_check
	(vxge_hal_wire_port_config_t *port_config)
{
...
/*
 * __vxge_hal_device_switch_port_config_check - Check switch port configuration.
 * @port_config: Port configuration information
 *
 ...
 */
vxge_hal_status_e
__vxge_hal_device_switch_port_config_check(
...
/*
 * __vxge_hal_device_mac_config_check - Check mac port configuration.
...
__vxge_hal_device_mac_config_check(vxge_hal_mac_config_t *mac_config)
{
	...
	status = __vxge_hal_device_wire_port_config_check(
	...
	status = __vxge_hal_device_switch_port_config_check(
...
/*
 * __vxge_hal_device_lag_port_config_check - Check LAG port configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_lag_port_config_check(vxge_hal_lag_port_config_t *port_config)
{
...
/*
 * __vxge_hal_device_lag_aggr_config_check - Check aggregator configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_lag_aggr_config_check(vxge_hal_lag_aggr_config_t *aggr_config)
{
...
/*
 * __vxge_hal_device_lag_la_config_check - Check LAG link aggregation
 *                          mode configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_lag_la_config_check(vxge_hal_lag_la_config_t *la_config)
{
...
/*
 * __vxge_hal_device_lag_ap_config_check - Check LAG a/p mode configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_lag_ap_config_check(vxge_hal_lag_ap_config_t *ap_config)
{
...
/*
 * __vxge_hal_device_lag_sl_config_check - Check LAG Single Link mode
 * configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_lag_sl_config_check(vxge_hal_lag_sl_config_t *sl_config)
{
...
/*
 * __vxge_hal_device_lag_lacp_config_check - Check LACP configuration.
 ...
 */
vxge_hal_status_e
__vxge_hal_device_lag_lacp_config_check(vxge_hal_lag_lacp_config_t *lacp_config)
{
...
/*
 * __vxge_hal_device_lag_config_check - Check link aggregation configuration.
...
__vxge_hal_device_lag_config_check(vxge_hal_lag_config_t *lag_config)
{
	...
	status =
	    __vxge_hal_device_lag_la_config_check(&lag_config->la_mode_config);
	if (status == VXGE_HAL_OK)
		return (status);

	status =
	    __vxge_hal_device_lag_ap_config_check(&lag_config->ap_mode_config);
	if (status == VXGE_HAL_OK)
		return (status);

	status =
	    __vxge_hal_device_lag_sl_config_check(&lag_config->sl_mode_config);
	if (status == VXGE_HAL_OK)
		return (status);

	status =
	    __vxge_hal_device_lag_lacp_config_check(&lag_config->lacp_config);
	if (status == VXGE_HAL_OK)
		return (status);
	...
	for (i = 0; i < VXGE_HAL_LAG_PORT_MAX_PORTS; i++) {
		...
		if ((status = __vxge_hal_device_lag_port_config_check(
	...
	for (i = 0; i < VXGE_HAL_LAG_AGGR_MAX_PORTS; i++) {
		...
		if ((status = __vxge_hal_device_lag_aggr_config_check(
...
/*
 * __vxge_hal_vpath_qos_config_check - Check vpath QOS configuration.
...
/*
 * __vxge_hal_mrpcim_config_check - Check mrpcim configuration.
...
__vxge_hal_mrpcim_config_check(vxge_hal_mrpcim_config_t *config)
{
	...
	if ((status = __vxge_hal_device_mac_config_check(
	    &config->mac_config)) != VXGE_HAL_OK)
		return (status);

	if ((status = __vxge_hal_device_lag_config_check(
	    &config->lag_config)) != VXGE_HAL_OK)
		return (status);

	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {

		if ((status = __vxge_hal_vpath_qos_config_check(
...
/*
 * __vxge_hal_device_ring_config_check - Check ring configuration.
...
__vxge_hal_device_ring_config_check(vxge_hal_ring_config_t *ring_config)
{
	...
	if ((ring_config->sw_lro_sessions < VXGE_HAL_SW_LRO_MIN_SESSIONS) ||
	    (ring_config->sw_lro_sessions > VXGE_HAL_SW_LRO_MAX_SESSIONS))
		return (VXGE_HAL_BADCFG_SW_LRO_SESSIONS);

	if ((ring_config->sw_lro_sg_size < VXGE_HAL_SW_LRO_MIN_SG_SIZE) ||
	    (ring_config->sw_lro_sg_size > VXGE_HAL_SW_LRO_MAX_SG_SIZE))
		return (VXGE_HAL_BADCFG_SW_LRO_SG_SIZE);

	if ((ring_config->sw_lro_frm_len < VXGE_HAL_SW_LRO_MIN_FRM_LEN) ||
	    (ring_config->sw_lro_frm_len > VXGE_HAL_SW_LRO_MAX_FRM_LEN))
		return (VXGE_HAL_BADCFG_SW_LRO_FRM_LEN);
...
/*
 * __vxge_hal_device_fifo_config_check - Check fifo configuration.
...
/*
 * __vxge_hal_device_tim_intr_config_check - Check tim intr configuration.
...
/*
 * __vxge_hal_device_vpath_config_check - Check vpath configuration.
...
__vxge_hal_device_vpath_config_check(vxge_hal_vp_config_t *vp_config)
{
	...
	if ((vp_config->bandwidth_limit >
	    VXGE_HAL_VPATH_BANDWIDTH_LIMIT_MAX) &&
	    (vp_config->bandwidth_limit !=
	...
	if ((status = __vxge_hal_device_ring_config_check(&vp_config->ring))
	    != VXGE_HAL_OK)
		return (status);

	if ((status = __vxge_hal_device_fifo_config_check(&vp_config->fifo))
	    != VXGE_HAL_OK)
		return (status);


	if ((status = __vxge_hal_device_tim_intr_config_check(&vp_config->tti))
	    != VXGE_HAL_OK)
		return (status);

	if ((status = __vxge_hal_device_tim_intr_config_check(&vp_config->rti))
	    != VXGE_HAL_OK)
		return (status);
...
/*
 * __vxge_hal_device_config_check - Check device configuration.
...
__vxge_hal_device_config_check(vxge_hal_device_config_t *new_config)
{
...
#if 0
	if (new_config->dma_blockpool_min < VXGE_HAL_MIN_DMA_BLOCK_POOL_SIZE)
		return (VXGE_HAL_BADCFG_BLOCKPOOL_MIN);

	if (new_config->dma_blockpool_initial <
	    VXGE_HAL_INITIAL_DMA_BLOCK_POOL_SIZE)
		return (VXGE_HAL_BADCFG_BLOCKPOOL_INITIAL);
#endif
	...
	if ((status = __vxge_hal_mrpcim_config_check(
	    &new_config->mrpcim_config)) != VXGE_HAL_OK)
		return (status);

	if (new_config->isr_polling_cnt > VXGE_HAL_MAX_ISR_POLLING_CNT)
		return (VXGE_HAL_BADCFG_ISR_POLLING_CNT);

	if (((new_config->latency_timer < VXGE_HAL_MIN_LATENCY_TIMER) ||
	    (new_config->latency_timer > VXGE_HAL_MAX_LATENCY_TIMER)) &&
	    (new_config->latency_timer != VXGE_HAL_USE_BIOS_DEFAULT_LATENCY))
		return (VXGE_HAL_BADCFG_LATENCY_TIMER);
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		if ((status = __vxge_hal_device_vpath_config_check(
	    ...
	    vxge_hal_device_config_t *device_config)
{
	...
	vxge_hal_trace_log_driver(NULL_HLDEV, NULL_VPID,
	    "device_config = 0x"VXGE_OS_STXFMT, (ptr_t)device_config);
	...
	device_config->latency_timer = VXGE_HAL_USE_BIOS_DEFAULT_LATENCY;
	...
	for (i = 0; i < VXGE_HAL_MAX_VIRTUAL_PATHS; i++) {
		...
		device_config->vp_config[i].bandwidth_limit =
		    VXGE_HAL_VPATH_BANDWIDTH_LIMIT_USE_FLASH_DEFAULT;
		...
		device_config->vp_config[i].ring.sw_lro_sessions =
		    VXGE_HAL_SW_LRO_DEFAULT_SESSIONS;

		device_config->vp_config[i].ring.sw_lro_sg_size =
		    VXGE_HAL_SW_LRO_DEFAULT_SG_SIZE;

		device_config->vp_config[i].ring.sw_lro_frm_len =
		    VXGE_HAL_SW_LRO_MAX_FRM_LEN;

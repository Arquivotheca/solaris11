...
/*
 * __vxge_hal_ring_block_memblock_idx - Return the memblock index
 * @block: Virtual address of memory block
 *
 * This function returns the index of memory block
 */
static inline u32
__vxge_hal_ring_block_memblock_idx(
	vxge_hal_ring_block_t block)
{
	u64 *ptr_64;
	ptr_64 = (void *)((u8 *)block +
	    VXGE_HAL_RING_MEMBLOCK_IDX_OFFSET);
	return (u32)(*ptr_64);
}

/*
 * __vxge_hal_ring_block_memblock_idx_set - Sets the memblock index
 * @block: Virtual address of memory block
 ...
 */
static inline void
__vxge_hal_ring_block_memblock_idx_set(
	vxge_hal_ring_block_t block,
	u32 memblock_idx)
{
	u64 *ptr_64;
	ptr_64 = (void *)((u8 *)block +  VXGE_HAL_RING_MEMBLOCK_IDX_OFFSET);
	*ptr_64 = memblock_idx;
}

/*
 * __vxge_hal_ring_block_next_pointer - Returns the dma address of next block
 * @block: RxD block
 *
	...
	vxge_hal_ring_block_t *block)
{
	...
	return (dma_addr_t)(*ptr_64);
}

/*
 * __vxge_hal_ring_block_next_pointer_set -
...
/*
 * __vxge_hal_ring_first_block_address_get - Returns the dma address of the
 *		first block
...
{
	__vxge_hal_ring_t *ring = (__vxge_hal_ring_t *)ringh;
	vxge_hal_mempool_dma_t *dma_object;

	dma_object = __vxge_hal_mempool_memblock_dma(ring->mempool, 0);
...
/*
 * __vxge_hal_ring_item_dma_offset - Return the dma offset of an item
 ...
 */
static ptrdiff_t
__vxge_hal_ring_item_dma_offset(
	...
	void *item)
{
	...
	vxge_hal_trace_log_ring(hldev, NULL_VPID,
	...
	memblock_idx = __vxge_hal_ring_block_memblock_idx(item);

	/* get owner memblock by memblock index */
	memblock = __vxge_hal_mempool_memblock(mempoolh, memblock_idx);
...
/*
 * __vxge_hal_ring_item_dma_addr - Return the dma address of an item
 * @mempoolh: Handle to the memory pool of the ring
 * @item: Item for which to get the dma offset
	...
	pci_dma_h *dma_handle)
{
	...
	/* get owner memblock index */
	memblock_idx = __vxge_hal_ring_block_memblock_idx((u8*)item);

	/* get owner memblock by memblock index */
	memblock = __vxge_hal_mempool_memblock(
	    (vxge_hal_mempool_t *)mempoolh, memblock_idx);

	/* get memblock DMA object by memblock index */
	memblock_dma_object = __vxge_hal_mempool_memblock_dma(
	    (vxge_hal_mempool_t *)mempoolh, memblock_idx);

	/* calculate offset in the memblock of this item */
	dma_item_offset = _PTRDIFF(item, memblock);
...
/*
 * __vxge_hal_ring_rxdblock_link - Link the RxD blocks
 * @mempoolh: Handle to the memory pool of the ring
 * @ring: ring
	...
	u32 to)
{
	...
	/* hldev = (__vxge_hal_device_t *)ring->channel.devh; */

	vxge_hal_trace_log_ring(hldev,
	    ((__vxge_hal_vpath_handle_t *)ring->channel.vph)->vpath->vp_id,
	    "==>  %s:%s:%d", __FILE__, __func__, __LINE__);
	...
	/* get "from" RxD block */
	from_item = (vxge_hal_ring_block_t *)__vxge_hal_mempool_item(
	    (vxge_hal_mempool_t *)mempoolh, from);
	vxge_assert(from_item);

	/* get "to" RxD block */
	to_item = (vxge_hal_ring_block_t *)__vxge_hal_mempool_item(
	    (vxge_hal_mempool_t *)mempoolh, to);
	vxge_assert(to_item);

	/* return address of the beginning of previous RxD block */
	to_dma =
	    __vxge_hal_ring_item_dma_addr(mempoolh, to_item, &to_dma_handle);

	/*
	 * set next pointer for this RxD block to point on
	 * previous item's DMA start address
	 */
	__vxge_hal_ring_block_next_pointer_set(from_item, to_dma);

	/* return "from" RxD block's DMA start address */
	from_dma = __vxge_hal_ring_item_dma_addr(
	...
	vxge_os_dma_sync(ring->channel.pdev,
	    ...
	    __vxge_hal_ring_item_dma_offset(mempoolh, from_item) +
	    VXGE_HAL_RING_NEXT_BLOCK_POINTER_OFFSET,
	    sizeof (u64),
	    VXGE_OS_DMA_DIR_TODEVICE);

	if (vxge_check_dma_handle(ring->channel.pdev,
	    from_dma_handle) != VXGE_HAL_OK)
		return (VXGE_HAL_FAIL);
#endif

	vxge_hal_info_log_ring(hldev,
	    ((__vxge_hal_vpath_handle_t *)ring->channel.vph)->vpath->vp_id,
...
/*
 * __vxge_hal_ring_mempool_item_alloc - Allocate List blocks for RxD block
 * callback
 ...
 *
 * This function is callback passed to __vxge_hal_mempool_create to create
 * memory pool for RxD block
 */
static vxge_hal_status_e
__vxge_hal_ring_mempool_item_alloc(
	vxge_hal_mempool_h mempoolh,
	void *memblock,
	u32 memblock_index,
	vxge_hal_mempool_dma_t *dma_object,
...
{
	u32 i;
	__vxge_hal_ring_t *ring = (__vxge_hal_ring_t *)userdata;
	/*LINTED*/
	__vxge_hal_device_t *hldev;
	...
	vxge_hal_trace_log_pool(hldev,
	    ...
	    "item = 0x"VXGE_OS_STXFMT", index = %d, is_last = %d, "
	    "userdata = 0x"VXGE_OS_STXFMT, (ptr_t)mempoolh, (ptr_t)memblock,
	    memblock_index, (ptr_t)dma_object, (ptr_t)item, index, is_last,
	    (ptr_t)userdata);

	/* format rxds array */
	for (i = 0; i < ring->rxds_per_block; i++) {
		void *rxdblock_priv;
		__vxge_hal_ring_rxd_priv_t *rxd_priv;
		void *uld_priv;
		vxge_hal_ring_rxd_1_t *rxdp;
		u32 dtr_index = index * ring->rxds_per_block + i;
		u32 memblock_item_idx;

		ring->channel.dtr_arr[dtr_index].dtr = ((u8 *)item) +
		    i * ring->rxd_size;
		 ...
		 */
		rxdblock_priv = __vxge_hal_mempool_item_priv(
		...
		rxd_priv->dma_offset = _PTRDIFF(rxdp, memblock);
	...
	__vxge_hal_ring_block_memblock_idx_set((u8*)item, memblock_index);

	if (is_last) {
		/* link last one with first one */
		(void) __vxge_hal_ring_rxdblock_link(mempoolh, ring, index, 0);
	}

	if (index > 0) {
		/* link this RxD block with previous one */
		(void) __vxge_hal_ring_rxdblock_link(
		    mempoolh, ring, index-1, index);
	}
...
/*
 * __vxge_hal_ring_mempool_item_free - Free RxD blockt callback
 * @mempoolh: Handle to memory pool
 * @memblock: Address of this memory block
 * @memblock_index: Index of this memory block
 * @dma_object: dma object for this block
 ...
 * @userdata: Specific data of user
 *
 * This function is callback passed to __vxge_hal_mempool_free to destroy memory
 * pool for RxD block
 */
/*ARGSUSED*/
static vxge_hal_status_e
__vxge_hal_ring_mempool_item_free(
	vxge_hal_mempool_h mempoolh,
	void *memblock,
	u32 memblock_index,
	vxge_hal_mempool_dma_t *dma_object,
...
/*
 * __vxge_hal_ring_initial_replenish - Initial replenish of RxDs
 * @ring: ring
	...
	vxge_hal_reopen_e reopen)
{
	...
	vxge_hal_trace_log_ring(hldev,
	    ((__vxge_hal_vpath_handle_t *)ring->channel.vph)->vpath->vp_id,
	    "ring = 0x"VXGE_OS_STXFMT", reopen = %d", (ptr_t)ring, reopen);
	...
	while (vxge_hal_ring_rxd_reserve(ring->channel.vph, &rxd, &uld_priv) ==
		...
		if (ring->rxd_init) {
			...
			if (status != VXGE_HAL_OK) {
				...
				vxge_hal_trace_log_ring(hldev,
				    ((__vxge_hal_vpath_handle_t *)ring->channel.
				    vph)->vpath->vp_id,
				    "<== %s:%s:%d Result: %d",
		...
		(void) vxge_hal_ring_rxd_post(ring->channel.vph, rxd);
	}
...
/*
 * __vxge_hal_ring_create - Create a Ring
	...
	vxge_hal_ring_attr_t *attr)
{
	...
	if ((vpath_handle == NULL) || (attr == NULL)) {
		vxge_hal_err_log_ring(hldev, vp->vpath->vp_id,
		    "null pointer passed == > %s : %d", __func__, __LINE__);
		vxge_hal_trace_log_ring(hldev, vp->vpath->vp_id,
	...
	ring = (__vxge_hal_ring_t *)__vxge_hal_channel_allocate(
	...
	ring->rxd_priv_size =
	    sizeof (__vxge_hal_ring_rxd_priv_t) + attr->per_rxd_space;
	...
	ring->rxd_mem_avail =
	    ...
	    ring->channel.vph)->vpath->rxd_mem_size;

	ring->db_byte_count = 0;

	ring->mempool = __vxge_hal_mempool_create(
	    (vxge_hal_device_h)vp->vpath->hldev,
	...
	status = __vxge_hal_channel_initialize(&ring->channel);
	if (status != VXGE_HAL_OK) {
		(void) __vxge_hal_ring_delete(vpath_handle);
		vxge_hal_trace_log_ring(hldev, vp->vpath->vp_id,
		    "<==  %s:%s:%d  Result: %d",
	...
	if (vp->vpath->hldev->header.config.lro_enable) {
		status = __vxge_hal_sw_lro_init(ring);
		if (status != VXGE_HAL_OK) {
			__vxge_hal_ring_delete(vpath_handle);
			vxge_hal_trace_log_ring(hldev, vp->vpath->vp_id,
			    "<==  %s:%s:%d  Result: %d",
			    __FILE__, __func__, __LINE__, status);
			return (status);
	 ...
	 */
	if (ring->rxd_init) {
		if ((status = __vxge_hal_ring_initial_replenish(
		    ring,
...
/*
 * __vxge_hal_ring_abort - Returns the RxD
	...
	vxge_hal_reopen_e reopen)
{
	...
	vxge_hal_trace_log_ring(hldev,
	    ((__vxge_hal_vpath_handle_t *)ring->channel.vph)->vpath->vp_id,
	    "ring = 0x"VXGE_OS_STXFMT", reopen = %d", (ptr_t)ringh,
	    reopen);

	if (ring->rxd_term) {
		__vxge_hal_channel_for_each_dtr(&ring->channel, rxdh, index) {
			if (!__vxge_hal_channel_is_posted_dtr(&ring->channel,
			    index)) {
	...
	for (;;) {
		__vxge_hal_channel_dtr_try_complete(&ring->channel, &rxdh);

		if (rxdh == NULL)
			break;

		__vxge_hal_channel_dtr_complete(&ring->channel);
		...
		__vxge_hal_channel_dtr_free(&ring->channel,
...
/*
 * __vxge_hal_ring_reset - Resets the ring
 * @ringh: Ring to be reset
 *
	...
	vxge_hal_ring_h ringh)
{
	...
	vxge_hal_status_e status;
	/*LINTED*/
	__vxge_hal_vpath_handle_t *vph =
	    (__vxge_hal_vpath_handle_t *)ring->channel.vph;
	...
	vxge_hal_trace_log_ring(hldev,
	    ((__vxge_hal_vpath_handle_t *)ring->channel.vph)->vpath->vp_id,
	    "ring = 0x"VXGE_OS_STXFMT, (ptr_t)ringh);

	(void) __vxge_hal_ring_abort(ringh, VXGE_HAL_RESET_ONLY);

	status = __vxge_hal_channel_reset(&ring->channel);

	if (status != VXGE_HAL_OK) {

		vxge_hal_trace_log_ring(hldev,
		    ((__vxge_hal_vpath_handle_t *)
		    ring->channel.vph)->vpath->vp_id,
		    "<==  %s:%s:%d  Result: %d", __FILE__, __func__, __LINE__,
		    status);
		return (status);
...
#if defined(VXGE_HAL_USE_SW_LRO)
	if (vph->vpath->hldev->header.config.lro_enable) {
		status = __vxge_hal_sw_lro_reset(ring);
		if (status != VXGE_HAL_OK) {
			vxge_hal_trace_log_ring(hldev,
			    ((__vxge_hal_vpath_handle_t *)ring->channel.vph)->
			    vpath->vp_id, "<==  %s:%s:%d  Result: %d",
			    __FILE__, __func__, __LINE__, status);
			return (status);
		}
	}
...
/*
 * __vxge_hal_ring_delete - Removes the ring
	...
	vxge_hal_vpath_h vpath_handle)
{
	...
	ring = (__vxge_hal_ring_t *)vp->vpath->ringh;

	vxge_assert(ring != NULL);

	vxge_assert(ring->channel.pdev);

	(void) __vxge_hal_ring_abort(vp->vpath->ringh, VXGE_HAL_OPEN_NORMAL);

#if defined(VXGE_HAL_USE_SW_LRO)
	if (vp->vpath->hldev->header.config.lro_enable)
		__vxge_hal_sw_lro_terminate(ring);
#endif

	if (ring->mempool) {
		__vxge_hal_mempool_destroy(ring->mempool);
	}

	__vxge_hal_channel_terminate(&ring->channel);
	...
	__vxge_hal_channel_free(&ring->channel);
 ...
 * Set the maximum frame length of recv frames.
 * @vpath: virtual Path
	...
	u32 new_frmlen)
{
	...
	vxge_hal_trace_log_ring(hldev, vpath->vp_id,
	    "==>  %s:%s:%d", __FILE__, __func__, __LINE__);
	...
	void **rxd_priv)
{
	...
	status = __vxge_hal_channel_dtr_reserve(&ring->channel, rxdh);
	...
	vxge_hal_rxd_h	rxdh)
{
	...
	if (TRUE) {
		if (VXGE_HAL_RING_RXD_INDEX(rxdp) != 0) {
			...
			__vxge_hal_ring_rxd_priv_t	*rxdp_priv;
			u32 index;
	...
	__vxge_hal_channel_dtr_post(&ring->channel,
	    VXGE_HAL_RING_RXD_INDEX(rxdh));
	...
	vxge_hal_rxd_h rxdh)
{
	...
	defined(VXGE_HAL_DMA_RXD_STREAMING)
	__vxge_hal_ring_rxd_priv_t	*priv;
#endif
	__vxge_hal_vpath_handle_t *vp =
	    (__vxge_hal_vpath_handle_t *)vpath_handle;
	/*LINTED*/
...
#if	defined(VXGE_OS_DMA_REQUIRES_SYNC) &&\
	defined(VXGE_HAL_DMA_RXD_STREAMING)
	priv = __vxge_hal_ring_rxd_priv(ring, rxdp);
	vxge_os_dma_sync(ring->channel.pdev,
	    priv->dma_handle,
	    priv->dma_addr,
	    priv->dma_offset,
	...
	vxge_hal_rxd_h	rxdh)
{
	...
	if (vxge_check_dma_handle(ring->channel.pdev,
	    rxdp_priv->dma_handle) != VXGE_HAL_OK)
		return (VXGE_HAL_FAIL);
#endif
	...
	if (TRUE) {
		if (VXGE_HAL_RING_RXD_INDEX(rxdp) != 0) {
			...
			prev_rxdh =
			    ...
			    VXGE_HAL_RING_RXD_INDEX(rxdp) - 1].dtr;
			...
			vxge_hal_rxd_h rxdh)
{
	...
	(void) vxge_hal_ring_rxd_post_post(vpath_handle, rxdh);
			...
			vxge_hal_vpath_h vpath_handle)
{
	...
	if (ring->db_byte_count <= ring->rxd_mem_avail) {
		__vxge_hal_rxd_db_post(vpath_handle, ring->db_byte_count);
		ring->rxd_mem_avail -= ring->db_byte_count;
		ring->db_byte_count = 0;
	} else {
		__vxge_hal_rxd_db_post(vpath_handle, ring->rxd_mem_avail);
			...
			u8 *t_code)
{
	...
	*rxd_priv = NULL;
	...
	/* check whether it	is not the end */
		own = (rxdp->control_0 & VXGE_HAL_RING_RXD_LIST_OWN_ADAPTER);
		t_code_tmp = (u8)VXGE_HAL_RING_RXD_T_CODE_GET(rxdp->control_0);
	...
	u8 t_code)
{
	...
	case 0x7:
		/*
		 * 0x7: Buffer size error—the RxD buffer(
		 *	s) were not appropriately sized and
		 *	data loss occurred.
		 */
		break;
	case 0x8:
		/* 0x8: Internal ECC error—RxD corrupted. */
		__vxge_hal_device_handle_error(vp->vpath->hldev,
		    vp->vpath->vp_id,
		    VXGE_HAL_EVENT_ECCERR);
		break;
	case 0x9:
		/*
			...
			vxge_hal_rxd_h rxdh)
{
	...
	return (VXGE_HAL_RING_ULD_PRIV(
	    ((__vxge_hal_ring_t *)vp->vpath->ringh), rxdh));

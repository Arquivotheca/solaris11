...
struct __vxge_hal_mrpcim_t;
struct __vxge_hal_srpcim_t;
...
/*
 * __vxge_hal_tracebuf_t
 *
 * HAL trace buffer object.
 */
typedef struct __vxge_hal_tracebuf_t {
	u8		*data;
	u64		wrapped_count;
	volatile u32	offset;
	u32		size;
...
/*
 * __vxge_hal_msix_map_t
 *
 * HAL msix to vpath map.
 */
typedef struct __vxge_hal_msix_map_t {
	u32		vp_id;
	u32		int_num;
} __vxge_hal_msix_map_t;
 ...
 */
typedef struct __vxge_hal_device_t {
...
#define	VXGE_HAL_DEVICE_ACCESS_RIGHT_MRPCIM	0x4
	...
	struct __vxge_hal_srpcim_t			*srpcim;
	struct __vxge_hal_mrpcim_t			*mrpcim;
	__vxge_hal_blockpool_t			block_pool;
	...
	char					*dump_buf;
#ifdef	VXGE_TRACE_INTO_CIRCULAR_ARR
	__vxge_hal_tracebuf_t			trace_buf;
#endif
...
#define	VXGE_HAL_DEVICE_MANAGER_STATE_SET(hldev, wmsg) {	\
	((__vxge_hal_device_t *)hldev)->manager_up =	  \
		__vxge_hal_ifmsg_is_manager_up(wmsg);	\
}
...
#define	VXGE_HAL_DEVICE_TIM_INT_MASK_SET(hldev, i) {			       \
	if (i < 16) {							       \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask0[0] |= vBIT(0x8,      \
	    (i*4), 4);							       \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask0[1] |= vBIT(0x4,      \
	    (i*4), 4);							       \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask0[3] |= vBIT(0x1,      \
	    (i*4), 4);							       \
	} else {							       \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask1[0] = 0x80000000;     \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask1[1] = 0x40000000;     \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask1[3] = 0x10000000;     \
	}								       \
}

#define	VXGE_HAL_DEVICE_TIM_INT_MASK_RESET(hldev, i) {			      \
	if (i < 16) {							      \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask0[0] &=		      \
							~vBIT(0x8, (i*4), 4); \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask0[1] &=		      \
							~vBIT(0x4, (i*4), 4); \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask0[3] &=		      \
							~vBIT(0x1, (i*4), 4); \
	} else {							      \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask1[0] = 0;	      \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask1[1] = 0;	      \
	    ((__vxge_hal_device_t *)hldev)->tim_int_mask1[3] = 0;	      \
	}								      \
}
...
void
__vxge_hal_pio_mem_write32_lower(pci_dev_h pdev,
			    pci_reg_h regh,
			    u32 val,
			    void *addr);

void
__vxge_hal_device_event_queued(void *data,
			    u32 event_type);
...
vxge_hal_status_e
__vxge_hal_device_register_poll(pci_dev_h	pdev,
			pci_reg_h	regh,
			u64		*reg,
			u32		op,
			u64		mask,
...
vxge_hal_status_e
__vxge_hal_device_reset(__vxge_hal_device_t *hldev);

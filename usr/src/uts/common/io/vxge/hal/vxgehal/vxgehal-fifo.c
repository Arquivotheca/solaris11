...
/*
 * __vxge_hal_fifo_mempool_item_alloc - Allocate List blocks for
 * TxD list callback @mempoolh: Handle to memory pool
 ...
 *
 * This function is callback passed to __vxge_hal_mempool_create
 * to create memory pool for TxD list
 */
/*ARGSUSED*/
static vxge_hal_status_e
__vxge_hal_fifo_mempool_item_alloc(
	...
	void *userdata)
{
	...
	__vxge_hal_fifo_t *fifo = (__vxge_hal_fifo_t *)userdata;
	/*LINTED*/
	__vxge_hal_device_t *hldev = (__vxge_hal_device_t *)fifo->channel.devh;

	vxge_assert(fifo != NULL);

	vxge_hal_trace_log_pool(hldev,
	    ((__vxge_hal_vpath_handle_t *)fifo->channel.vph)->vpath->vp_id,
	...
	vxge_hal_trace_log_pool(hldev,
	    ...
	    "dma_object = 0x"VXGE_OS_STXFMT", item = 0x"VXGE_OS_STXFMT", "
	    "index = %d, is_last = %d, userdata = 0x"VXGE_OS_STXFMT,
	    (ptr_t)mempoolh, (ptr_t)memblock, memblock_index,
	    (ptr_t)dma_object, (ptr_t)item, index, is_last,
	    (ptr_t)userdata);

	vxge_assert(item);

	block_priv =
	    __vxge_hal_mempool_item_priv((vxge_hal_mempool_t *)mempoolh,
	...
	for (i = 0; i < fifo->txdl_per_memblock; i++) {
		...
		int dtr_index = index * fifo->txdl_per_memblock + i;
		...
		if (fifo->config->alignment_size) {
			status =
			    __vxge_hal_fifo_txdl_align_alloc_map(fifo, txdp);
			if (status != VXGE_HAL_OK)  {
				vxge_hal_err_log_pool(hldev,
				    ((__vxge_hal_vpath_handle_t *)
				    fifo->channel.vph)->vpath->vp_id,
				    "align buffer[%d] %d bytes, status %d",
				    (index * fifo->txdl_per_memblock + i),
...
/*
 * __vxge_hal_fifo_mempool_item_free - Free List blocks for TxD list callback
...
/*ARGSUSED*/
static vxge_hal_status_e
__vxge_hal_fifo_mempool_item_free(
	vxge_hal_mempool_h mempoolh,
	void *memblock,
	u32 memblock_index,
	vxge_hal_mempool_dma_t *dma_object,
...
#ifdef	VXGE_HAL_ALIGN_XMIT
	if (fifo->config->alignment_size) {
		for (int i = 0; i < fifo->txdl_per_memblock; i++) {
			txdp = (void *)
			    ((char *)item + i * fifo->txdl_size);
			__vxge_hal_fifo_txdl_align_free_unmap(fifo,
			    (vxge_hal_fifo_txd_t *)txdp);
		}
	}
#endif
...
/*
 * __vxge_hal_fifo_create - Create a FIFO
	...
	vxge_hal_fifo_attr_t *attr)
{
	...
	if ((vpath_handle == NULL) || (attr == NULL)) {
		vxge_hal_err_log_fifo(hldev, vp->vpath->vp_id,
	...
	fifo = (__vxge_hal_fifo_t *)__vxge_hal_channel_allocate(
	 ...
	 *
	 * Pool of memory memblocks never shrinks except __vxge_hal_fifo_close
	 * routine which will essentially stop channel and free the resources.
	 */

	/* TxDL common private size == TxDL private + ULD private */
	fifo->txdl_priv_size =
	    sizeof (__vxge_hal_fifo_txdl_priv_t) + attr->per_txdl_space;
	fifo->txdl_priv_size =
	...
	fifo->mempool =
	    __vxge_hal_mempool_create((vxge_hal_device_h)vp->vpath->hldev,
	    fifo->memblock_size,
	    fifo->memblock_size,
	    fifo->txdlblock_priv_size,
	...
	status = __vxge_hal_channel_initialize(&fifo->channel);
	if (status != VXGE_HAL_OK) {
		(void) __vxge_hal_fifo_delete(vpath_handle);
		vxge_hal_trace_log_fifo(hldev, vp->vpath->vp_id,
		    "<==  %s:%s:%d  Result: %d",
...
/*
 * __vxge_hal_fifo_abort - Returns the TxD
 ...
 */
vxge_hal_status_e
__vxge_hal_fifo_abort(
	vxge_hal_fifo_h fifoh,
	vxge_hal_reopen_e reopen)
{
	u32 index = 0;
	__vxge_hal_fifo_t *fifo = (__vxge_hal_fifo_t *)fifoh;
	/*LINTED*/
	__vxge_hal_device_t *hldev;
	...
	hldev = (__vxge_hal_device_t *)fifo->channel.devh;

	vxge_hal_trace_log_fifo(hldev,
	    ((__vxge_hal_vpath_handle_t *)fifo->channel.vph)->vpath->vp_id,
	    "==>  %s:%s:%d", __FILE__, __func__, __LINE__);

	vxge_hal_trace_log_fifo(hldev,
	    ((__vxge_hal_vpath_handle_t *)fifo->channel.vph)->vpath->vp_id,
	    "fifo = 0x"VXGE_OS_STXFMT", reopen = %d", (ptr_t)fifoh, reopen);

	if (fifo->txdl_term) {
		__vxge_hal_channel_for_each_dtr(&fifo->channel, txdlh, index) {
			if (!__vxge_hal_channel_is_posted_dtr(&fifo->channel,
			    index)) {
	...
	for (;;) {
		__vxge_hal_channel_dtr_try_complete(&fifo->channel, &txdlh);

		if (txdlh == NULL)
			break;

		__vxge_hal_channel_dtr_complete(&fifo->channel);
		...
		__vxge_hal_channel_dtr_free(&fifo->channel,
...
/*
 * __vxge_hal_fifo_reset - Resets the fifo
 * @fifoh: Fifo to be reset
 *
 * This function resets the fifo during vpath reset operation
 */
vxge_hal_status_e
__vxge_hal_fifo_reset(
	vxge_hal_fifo_h fifoh)
{
	vxge_hal_status_e status;
	/*LINTED*/
	...
	vxge_hal_trace_log_fifo(hldev,
	    ((__vxge_hal_vpath_handle_t *)fifo->channel.vph)->vpath->vp_id,
	    "fifo = 0x"VXGE_OS_STXFMT, (ptr_t)fifoh);

	(void) __vxge_hal_fifo_abort(fifoh, VXGE_HAL_RESET_ONLY);

	status = __vxge_hal_channel_reset(&fifo->channel);
	...
	vxge_hal_vpath_h vpath_handle)
{
	...
	fifo = (__vxge_hal_fifo_t *)vp->vpath->fifoh;

	vpath = ((__vxge_hal_vpath_handle_t *)fifo->channel.vph)->vpath;

	status =  __vxge_hal_non_offload_db_reset(fifo->channel.vph);

	if (status != VXGE_HAL_OK) {
		vxge_hal_trace_log_fifo(hldev, vpath->vp_id,
		    "<==  %s:%s:%d  Result: 0", __FILE__, __func__, __LINE__);
		return (status);
	}

	__vxge_hal_channel_for_each_posted_dtr(&fifo->channel, txdlh, index) {

		txdl_priv = VXGE_HAL_FIFO_HAL_PRIV(fifo, txdlh);

		__vxge_hal_non_offload_db_post(fifo->channel.vph,
	...
	vxge_hal_vpath_h vpath_handle)
{
	...
	(void) __vxge_hal_fifo_abort(vp->vpath->fifoh, VXGE_HAL_OPEN_NORMAL);

	if (fifo->mempool) {
		__vxge_hal_mempool_destroy(fifo->mempool);
	}

	__vxge_hal_channel_terminate(&fifo->channel);
	...
	__vxge_hal_channel_free(&fifo->channel);
...
#ifdef	VXGE_HAL_ALIGN_XMIT
/*
 * __vxge_hal_fifo_txdl_align_free_unmap - Unmap the alignement buffers
 * @fifo: Fifo
 * @txdp: txdl
 *
 ...
 */
void
__vxge_hal_fifo_txdl_align_free_unmap(
	__vxge_hal_fifo_t *fifo,
	vxge_hal_fifo_txd_t *txdp)
{
	/*LINTED*/
	...
	if (txdl_priv->align_vaddr != NULL) {
		(void) __vxge_hal_blockpool_free(fifo->channel.devh,
...
/*
 * __vxge_hal_fifo_txdl_align_alloc_map - Maps the alignement buffers
 * @fifo: Fifo
 * @txdp: txdl
 *
 ...
 */
vxge_hal_status_e
__vxge_hal_fifo_txdl_align_alloc_map(
	__vxge_hal_fifo_t *fifo,
	vxge_hal_fifo_txd_t *txdp)
{
	/*LINTED*/
	...
	txdl_priv->align_vaddr =
	    (u8*)__vxge_hal_blockpool_malloc(fifo->channel.devh,
	    fifo->align_size,
	    &txdl_priv->align_dma_addr,
	    &txdl_priv->align_dma_handle,
	    &txdl_priv->align_dma_acch);
			...
			vxge_hal_vpath_h vpath_handle)
{
	...
	return __vxge_hal_channel_free_dtr_count(&((__vxge_hal_fifo_t *)
	    ((__vxge_hal_vpath_handle_t *)
	    vpath_handle)->vpath->fifoh)->channel);
}
...
void *vxge_hal_fifo_txdl_private_get(
			...
			vxge_hal_txdl_h txdlh)
{
	return VXGE_HAL_FIFO_ULD_PRIV(((__vxge_hal_fifo_t *)
	    ((__vxge_hal_vpath_handle_t *)vpath_handle)->vpath->fifoh), txdlh);
}
	...
	void **txdl_priv)
{
	...
	status = __vxge_hal_channel_dtr_reserve(&fifo->channel, txdlh);
	...
	if (status == VXGE_HAL_OK) {
		...
		__vxge_hal_fifo_txdl_priv_t *priv;
	...
	u32 misaligned_size)
{
	...
	/* setup new buffer */
	prev_boff = _PTRDIFF(txdl_priv->align_vaddr_start,
	    txdl_priv->align_vaddr);
	...
	if (vxge_check_dma_handle(fifo->channel.pdev,
	    txdl_priv->align_dma_handle) != VXGE_HAL_OK)
	...
	u32 size)
{
	...
	used = _PTRDIFF(txdl_priv->align_vaddr_start, txdl_priv->align_vaddr);
	...
	u32 tagged)
{
	 ...
	 */
	__vxge_hal_channel_dtr_post(&fifo->channel,
	    VXGE_HAL_FIFO_TXDL_INDEX(txdlh));

	__vxge_hal_non_offload_db_post(vpath_handle,
	    list_ptr,
	    txdl_priv->frags - 1,
	    vp->vpath->vp_config->fifo.no_snoop_bits);

#ifdef	VXGE_HAL_FIFO_DUMP_TXD
	vxge_hal_info_log_fifo(hldev, vp->vpath->vp_id,
...
/*
 * __vxge_hal_fifo_txdl_free_many - Free the fragments
 * @fifo: FIFO
 ...
 */
void __vxge_hal_fifo_txdl_free_many(
	__vxge_hal_fifo_t *fifo,
	vxge_hal_fifo_txd_t *txdp,
	u32 list_size,
	u32 frags)
{
	__vxge_hal_fifo_txdl_priv_t *current_txdl_priv;
	__vxge_hal_fifo_txdl_priv_t *next_txdl_priv;
	u32 invalid_frags = frags % list_size;
	__vxge_hal_vpath_handle_t *vp =
	    (__vxge_hal_vpath_handle_t *)fifo->channel.vph;
	/*LINTED*/
	__vxge_hal_device_t *hldev;
	...
	while (txdp) {
		vxge_hal_trace_log_fifo(hldev, vp->vpath->vp_id,
		    "freeing linked txdlh 0x"VXGE_OS_STXFMT", "
		    "fragments %d list size %d",
		    (ptr_t)txdp, frags, list_size);
	...
	vxge_hal_txdl_h txdlh)
{
	...
	if (txdl_priv->alloc_frags > max_frags) {
		...
		if (dang_txdp) {
			...
			__vxge_hal_fifo_txdl_free_many(fifo, dang_txdp,
		...
		__vxge_hal_fifo_txdl_free_many(fifo,

...
e1000g_rx_setup(struct e1000g *Adapter)
{
	...
	/* Workaround errata in large frame configuration with 82579 PHY */
	if (hw->mac.type == e1000_pch2lan) {
		hw->phy.ops.reset(hw);
		(void) e1000_setup_link(hw);
		if (Adapter->default_mtu > ETHERMTU)
			(void) e1000_lv_jumbo_workaround_ich8lan(hw, true);
		else
			(void) e1000_lv_jumbo_workaround_ich8lan(hw, false);
	}
 ...
 */
mblk_t *
e1000g_receive(e1000g_rx_ring_t *rx_ring, mblk_t **tail, int sz,
    int poll_pkts)
{
	struct e1000_hw *hw;
	mblk_t *nmp;
	...
	while ((current_desc->status & E1000_RXD_STAT_DD) &&
	    ...
	    (chain_sz <= sz && pkt_count <= poll_pkts)) {

...
/* LINTED E_STATIC_UNUSED */
static char e1000g_version[] = "Driver Ver. 5.3.31";
...
#else
	/* CRC must be stripped for 82579 jumbo frame workaround */
...
e1000g_set_bufsize(struct e1000g *Adapter)
{
	 ...
	 * potential risk where we may lose data if buffer is not
...
e1000g_init(struct e1000g *Adapter)
{
	 ...
	 */
	if (hw->mac.type == e1000_80003es2lan || hw->mac.type == e1000_pchlan)
	...
	default:
		...
		break;
	case e1000_pchlan:
		/* Workaround for PCH LOM adapter hangs  */
	...
	/*
	 * The following is the workaround for the issue that e1000g
	 * shows link up wrongly with no cable attached when working
	 * in forced link mode.
	 */
	if ((!hw->mac.autoneg) &&
		...
		switch (hw->mac.type) {
		/*
		 * The workaround is only applied to the adapters
		 * listed here, which have been verified. Other
		 * adapters are not tested.
		 */
		case e1000_82546:
		case e1000_82571:
    ...
    int pkts_to_pickup)
{
	...
	mp = e1000g_receive(rx_ring, &tail, bytes_to_pickup, pkts_to_pickup);
	...
	if (icr & E1000_ICR_RXT0) {
		...
		if (!rx_ring->poll_flag)
			mp = e1000g_receive(rx_ring, &tail,
			    E1000G_CHAIN_NO_LIMIT, E1000G_CHAIN_NO_LIMIT);
		mutex_exit(&rx_ring->rx_lock);
		rw_exit(&Adapter->chip_lock);
		if (mp != NULL)
 ...
 */
static int
e1000g_rx_ring_intr_enable(mac_ring_driver_t rh)
{
...
static int
e1000g_rx_ring_intr_disable(mac_ring_driver_t rh)
{
	e1000g_rx_ring_t	*rx_ring = (e1000g_rx_ring_t *)rh;
	struct e1000g 		*adapter = rx_ring->adapter;
	struct e1000_hw 	*hw = &adapter->shared;
 ...
 */
static int
e1000g_addmac(void *arg, const uint8_t *mac_addr, uint64_t flags)
{
...
e1000g_set_internal_loopback(struct e1000g *Adapter)
{
	...
	case e1000_pch2lan:
		...
		(void) e1000_read_phy_reg(hw, PHY_REG(0, 21), &phy_reg);
		(void) e1000_write_phy_reg(hw, PHY_REG(0, 21),
		    phy_reg & ~(1 << 3));
		/* Disable full chip energy detect */
		(void) e1000_read_phy_reg(hw, PHY_REG(776, 18), &phy_reg);
		(void) e1000_write_phy_reg(hw, PHY_REG(776, 18), phy_reg | 1);
		/* Enable loopback on PHY */
		(void) e1000_write_phy_reg(hw, I82577_PHY_LBK_CTRL, 0x8001);
		break;
	}

...
/*
 * IntelVersion: 1.256 v2010-08-31
 */
...
static s32 e1000_set_ich8_mdio_slow_mode_hv(struct e1000_hw *hw);
...
e1000_init_phy_params_pchlan(struct e1000_hw *hw)
{
	...
	if ((phy->id == 0) || (phy->id == PHY_REVISION_MASK)) {
		 ...
		 */
		ret_val = e1000_set_ich8_mdio_slow_mode_hv(hw);
		if (ret_val)
			goto out;
		ret_val = e1000_get_phy_id(hw);
		if (ret_val)
...
/*
 * e1000_set_ich8_mdio_slow_mode_hv - Set slow MDIO access mode
 * @hw: pointer to the HW structure
 */
static s32
e1000_set_ich8_mdio_slow_mode_hv(struct e1000_hw *hw)
{
	s32 ret_val;
	u16 data;

	DEBUGFUNC("e1000_set_ich8_mdio_slow_mode_hv");
...
e1000_disable_gig_wol_ich8lan(struct e1000_hw *hw)
{
	...
	if (hw->mac.type >= e1000_pchlan) {
		(void) e1000_oem_bits_config_ich8lan(hw, true);
		ret_val = hw->phy.ops.acquire(hw);
		if (ret_val)
			return;
		(void) e1000_write_smbus_addr(hw);
		hw->phy.ops.release(hw);
	}
}

...
unm_process_rcv(unm_adapter *adapter, statusDesc_t *desc)
{
	...
	if (rx_buffer->inhw == B_TRUE) {
		rx_buffer->inhw = B_FALSE;
	} else {
		/*
		 * Invalid reference handle.  The corresponding rx buffer was
		 * not posted to hw.
		 */
		adapter->stats.rxinvhdl++;
		return (NULL);
	}
...
unm_kstat_update(kstat_t *ksp, int flag)
{
	...
	(knp++)->value.ui64 = adapter->stats.rxinvhdl;

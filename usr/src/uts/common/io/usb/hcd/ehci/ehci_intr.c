	...
	ehci_state_t		*ehcip)
{
	...
	while (curr_qtd) {
		...
		/*
		 * Don't free the Ctrl TW if it's going wrong, and
		 * its last qTD. Otherwise, the TW's timeout handler
		 * will have a NULL pointer dereference.
		 */
		phase = Get_QTD(curr_qtd->qtd_ctrl_phase);
		if (((phase == EHCI_CTRL_DATA_PHASE) ||
		    (phase == EHCI_CTRL_SETUP_PHASE)) &&
		    (tw->tw_num_qtds == 0) &&
		    (error == USB_CR_OK)) {
			USB_DPRINTF_L2(PRINT_MASK_INTR, ehcip->ehci_log_hdl,
			    "ehci_traverse_active_qtd_list: last qtd(%d) is "
			    "not Status, skip", phase);
			curr_qtd = next_qtd;
	...
	ehci_qtd_t		*qtd)
{
	...
	/*
	 * If the last qTD of a control req's TW is Data or Setup, we will
	 * not stop the TW's timer. The Status qTD has arrived prematurely.
	 * There's no chance to return the ctrl req in Status phase. We
	 * heavily depend on the timer, which is now the only executor that
	 * can hand the control request back to USBA. If we stop the timer,
	 * the requester will never get a response and hang forever. See
	 * ehci_handle_ctrl_qtd() for how Ctrl qTDs get handled.
	 */
	if (((Get_QTD(qtd->qtd_ctrl_phase) == EHCI_CTRL_DATA_PHASE) ||
	    (Get_QTD(qtd->qtd_ctrl_phase) == EHCI_CTRL_SETUP_PHASE)) &&
	    (tw->tw_num_qtds == 1)) {
		USB_DPRINTF_L2(PRINT_MASK_INTR, ehcip->ehci_log_hdl,
		    "ehci_parse_error: Ctrl Status arrived prematurely,"
		    "tw=0x%p, last qtd is %d", (void *)tw,
	...
	usb_cr_t		error)
{
	...
	if (length) {
		...
		if (ehci_check_dma_handle(ehcip, tw->tw_dmahandle) !=
		    DDI_FM_OK) {
			USB_DPRINTF_L4(PRINT_MASK_INTR, ehcip->ehci_log_hdl,
			    "ehci_sendup_qtd_message: sync IO buffer error");

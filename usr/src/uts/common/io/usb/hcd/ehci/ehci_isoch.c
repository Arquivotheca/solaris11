	...
	if (pipe_dir == USB_EP_DIR_IN) {
		...
		if (itw->itw_length) {
			...
			Sync_IO_Buffer_for_device(itw->itw_dmahandle,
			    itw->itw_length);
			if (ehci_check_dma_handle(ehcip, itw->itw_dmahandle) !=
			    DDI_FM_OK) {
				USB_DPRINTF_L2(PRINT_MASK_LISTS,
				    ehcip->ehci_log_hdl,
				    "ehci_allocate_isoc_resources: "
				    "Sync IO Error "
				    "request data may needs re-copy "
				    "to dma buffer");
	...
	ehci_state_t		*ehcip)
{
	...
	/* Sync ITD pool */
	Sync_ITD_Pool(ehcip);
	if (ehci_check_dma_handle(ehcip, ehcip->ehci_itd_pool_dma_handle) !=
	    DDI_FM_OK) {
		USB_DPRINTF_L2(PRINT_MASK_INTR, ehcip->ehci_log_hdl,
		    "ehci_traverse_active_isoc_list: Sync IO buffer error ");
	...
	usb_cr_t		error)
{
	...
	if (length) {
		...
		if (ehci_check_dma_handle(ehcip, itw->itw_dmahandle) !=
		    DDI_FM_OK) {
			USB_DPRINTF_L3(PRINT_MASK_INTR, ehcip->ehci_log_hdl,
			    "ehci_sendup_itd_message: Sync IO Buffer error ");

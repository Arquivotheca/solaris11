...
/*
 * EHCI live suspend/resume callback
 */
static int	ehci_cbfunc(dev_info_t *dip, ddi_cb_action_t cbaction,
	...
	ddi_attach_cmd_t	cmd)
{
	...
	if (ddi_cb_register(dip, DDI_CB_FLAG_LSR, ehci_cbfunc, ehcip,
	    NULL, &ehcip->ehci_cb_hdl) != DDI_SUCCESS) {

		USB_DPRINTF_L4(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
		    "ehci_attach - failed to register callback");
	...
	ehcip->ehci_fm_cap = ddi_prop_get_int(DDI_DEV_T_ANY, dip,
...
/* ARGSUSED */
static int
ehci_cbfunc(dev_info_t *dip, ddi_cb_action_t cbaction,
    void *cbarg, void *arg1, void *arg2)
{
	...
	case DDI_CB_LSR_SUSPEND:
		...
		USB_DPRINTF_L4(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
		    "ehci_cbfunc - receives live suspend "
		...
		ehcip->ehci_hc_lsr_state = EHCI_LSR_SUSPENDED;

		ehcip->ehci_hc_lsr_activities = plsr->activities;
		ehcip->ehci_hc_lsr_impacts = plsr->impacts;

		mutex_exit(&ehcip->ehci_int_mutex);

		/*
		 * ehci host controller live suspend
		 */
		if (ehci_live_suspend(ehcip) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
			    "ehci_cbfunc - failed to do live suspend");

			mutex_enter(&ehcip->ehci_int_mutex);

			ehcip->ehci_hc_lsr_state = 0;
			ehcip->ehci_hc_lsr_activities = 0;
			ehcip->ehci_hc_lsr_impacts = 0;
	...
	case DDI_CB_LSR_RESUME:
		...
		USB_DPRINTF_L4(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
		    "ehci_cbfunc - receives live resume: "
		    "activities is %llx, impacts is %llx",
		    (unsigned long long)plsr->activities,
		    (unsigned long long)plsr->impacts);
		...
		if ((plsr->activities != ehcip->ehci_hc_lsr_activities) ||
		    (plsr->impacts != ehcip->ehci_hc_lsr_impacts)) {
			mutex_exit(&ehcip->ehci_int_mutex);
		...
		/*
		 * ehci host controller live resume
		 */
		if (ehci_live_resume(ehcip) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
			    "ehci_cbfunc - failed to do live resume");
	...
	case DDI_CB_LSR_QUERY_CAPABILITY:
		...
		USB_DPRINTF_L4(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
		    "ehci_cbfunc - LSR Query Capability");

		/*
		 * query LSR capability
		 */
		if (ehci_lsr_capability(ehcip, plsr) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
			    "ehci_cbfunc - failed to query capability");

			return (USB_FAILURE);
		}

		USB_DPRINTF_L4(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
		    "ehci_cbfunc - descendants have capabilities: "
		    "activities is %llx, impacts is %llx",
		    (unsigned long long)plsr->activities,
		    (unsigned long long)plsr->impacts);
	...
	default:
		USB_DPRINTF_L2(PRINT_MASK_LSR, ehcip->ehci_log_hdl,
		    "ehci_cbfunc - unknown lsr action %x", cbaction);
...
ehci_intr(caddr_t arg1, caddr_t arg2)
{
	...
	if (intr & EHCI_INTR_FRAME_LIST_ROLLOVER) {
		...
		/* VIA VT6202 loses EHCI_INTR_USB interrupts, workaround. */

...
/* OHCI live suspend/resume callback */
static int	ohci_cbfunc(dev_info_t			*dip,
...
static void	ohci_dma_cleanup(ohci_state_t		*ohcip);
static int	ohci_cpr_suspend(ohci_state_t		*ohcip);
static int	ohci_cpr_resume(ohci_state_t		*ohcip);
static int	ohci_live_suspend(ohci_state_t		*ohcip);
static int	ohci_live_resume(ohci_state_t		*ohcip);
static int	ohci_lsr_capability(ohci_state_t	*ohcip,
				void			*cbarg);
...
static void	ohci_hcdi_callback(
				...
				usb_cr_t		completion_reason);
/* FM functions */
static int	ohci_fm_error_cb(
				dev_info_t	*dip,
				ddi_fm_error_t	*err,
				const void	*impl);
static void	ohci_fm_init(ohci_state_t *ohcip);
static void	ohci_fm_fini(ohci_state_t *ohcip);
	...
	ddi_attach_cmd_t	cmd)
{
	...
	if (ddi_cb_register(dip, DDI_CB_FLAG_LSR, ohci_cbfunc, ohcip,
	    NULL, &ohcip->ohci_cb_hdl) != 0) {

		USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_attach - failed to register callback");

		return (DDI_FAILURE);
	}

	/* must be called before set_dma_attribuites */
	ohcip->ohci_fm_cap = ddi_prop_get_int(DDI_DEV_T_ANY, dip,
...
/*ARGSUSED*/
static int
ohci_cbfunc(dev_info_t *dip, ddi_cb_action_t cbaction,
    void *cbarg, void *arg1, void *arg2)
{
	...
	case DDI_CB_LSR_SUSPEND:
		...
		USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_cbfunc - receives live suspend: "
		    "activities is %llx, impacts is %llx",
		    (unsigned long long)plsr->activities,
		    (unsigned long long)plsr->impacts);
		...
		if (ohcip->ohci_hc_lsr_state == OHCI_LSR_SUSPENDED) {
			mutex_exit(&ohcip->ohci_int_mutex);

			return (DDI_EALREADY);
		}

		ohcip->ohci_hc_lsr_state = OHCI_LSR_SUSPENDED;

		ohcip->ohci_hc_lsr_activities = plsr->activities;
		ohcip->ohci_hc_lsr_impacts = plsr->impacts;

		mutex_exit(&ohcip->ohci_int_mutex);

		/*
		 * ohci host controller live suspend
		 */
		if (ohci_live_suspend(ohcip) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
			    "ohci_cbfunc - failed to do live suspend");

			mutex_enter(&ohcip->ohci_int_mutex);

			ohcip->ohci_hc_lsr_state = 0;
			ohcip->ohci_hc_lsr_activities = 0;
			ohcip->ohci_hc_lsr_impacts = 0;
	...
	case DDI_CB_LSR_RESUME:
		...
		USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_cbfunc - receives live resume: "
		    "activities is %llx, impacts is %llx",
		    (unsigned long long)plsr->activities,
		    (unsigned long long)plsr->impacts);
		...
		if ((plsr->activities != ohcip->ohci_hc_lsr_activities) ||
		    (plsr->impacts != ohcip->ohci_hc_lsr_impacts)) {
			mutex_exit(&ohcip->ohci_int_mutex);
		...
		/*
		 * ohci host controller live resume
		 */
		if (ohci_live_resume(ohcip) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
			    "ohci_cbfunc - failed to do live resume");
	...
	case DDI_CB_LSR_QUERY_CAPABILITY:
		...
		USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_cbfunc - LSR Query Capability");

		/*
		 * Query descendant's lsr capability
		 */
		if ((ohci_lsr_capability(ohcip, plsr)) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
			    "ohci_cbfunc - failed to query its descendants");

			return (DDI_FAILURE);
		}

		USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_cbfunc - and its descendants have capabilities - "
		    "activities is %llx, impacts is %llx",
		    (unsigned long long)plsr->activities,
		    (unsigned long long)plsr->impacts);
	...
	default:
		USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_cbfunc - unknown lsr action %x", cbaction);
...
ohci_set_dma_attributes(ohci_state_t	*ohcip)
{
	...
	ohcip->ohci_dma_attr.dma_attr_addr_hi = 0xffffffffull;
	...
	if (DDI_FM_DMA_ERR_CAP(ohcip->ohci_fm_cap)) {
		ohcip->ohci_dma_attr.dma_attr_flags |= DDI_DMA_FLAGERR;
	} else {
...
ohci_map_regs(ohci_state_t	*ohcip)
{
	...
	if (DDI_FM_ACC_ERR_CAP(ohcip->ohci_fm_cap)) {
...
ohci_cleanup(ohci_state_t	*ohcip)
{
	...
	(void) ddi_cb_unregister(ohcip->ohci_cb_hdl);
 ...
 */
static void
ohci_dma_cleanup(ohci_state_t	*ohcip)
{
	int i, state;
	ohci_td_t *td;
	ohci_trans_wrapper_t *tw;
...
ohci_cpr_suspend(ohci_state_t	*ohcip)
{
	...
	if (usba_hubdi_detach(ohcip->ohci_dip, DDI_SUSPEND) != DDI_SUCCESS) {
		USB_DPRINTF_L2(PRINT_MASK_ATTA, ohcip->ohci_log_hdl,
		    "ohci_cpr_suspend: root hub fails to suspend");
	...
	mutex_enter(&ohcip->ohci_int_mutex);
	ASSERT(ohcip->ohci_open_pipe_count == 0);
...
ohci_cpr_resume(ohci_state_t	*ohcip)
{
	...
	if (ohci_init_ctlr(ohcip) != DDI_SUCCESS) {
		...
		ohci_fm_ereport(ohcip, DDI_FM_DEVICE_INVAL_STATE);
		ddi_fm_service_impact(ohcip->ohci_dip, DDI_SERVICE_LOST);
 ...
 * ohci_live_suspend
 *
 * This function will put ohci host controller to SUSPEND
 * state first, then do additional operation according to
 * arguments passed down from registered DDI callback.
 */
static int
ohci_live_suspend(ohci_state_t *ohcip)
{
	usba_cb_lsr_t usba_lsr;

	USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
	    "ohci_live_suspend:entry");

	mutex_enter(&ohcip->ohci_int_mutex);

	usba_lsr.usba_lsr_cbaction = DDI_CB_LSR_SUSPEND;
	usba_lsr.usba_lsr_cbarg.activities = ohcip->ohci_hc_lsr_activities;
	usba_lsr.usba_lsr_cbarg.impacts = ohcip->ohci_hc_lsr_impacts;
	usba_lsr.usba_lsr_cbret = USB_SUCCESS;

	mutex_exit(&ohcip->ohci_int_mutex);

	/* Call into the root hub and suspend all descendants */
	if (usba_hubdi_post_lsr(ohcip->ohci_dip, &usba_lsr) != USB_SUCCESS) {
		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_live_suspend - failed to suspend descendants");

		return (USB_FAILURE);
	}

	/* Now suspend ohci host controller */
	mutex_enter(&ohcip->ohci_int_mutex);

	ASSERT(ohcip->ohci_open_pipe_count == 0);

	USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
	    "ohci_live_suspend: Disable HC ED list processing");
	...
	USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
	    "ohci_live_suspend: Disable HC interrupts");

	/* Disable all HC interrupts */
	Set_OpReg(hcr_intr_disable, ~(HCR_INTR_MIE|HCR_INTR_SOF));

	USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
	    "ohci_live_suspend: Wait for the next SOF");

	/* Wait for the next SOF */
	if (ohci_wait_for_sof(ohcip) != USB_SUCCESS) {

		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_live_suspend: ohci host controller suspend failed");
	...
	USB_DPRINTF_L4(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
	    "ohci_live_suspend: Disable Master interrupt");
	...
	if (!(ohcip->ohci_hc_lsr_impacts & (DDI_CB_LSR_IMP_DMA_ADDR_CHANGE |
 ...
 * ohci_live_resume
 *
 * This function will resume ohci driver from live suspend.
 */
static int
ohci_live_resume(ohci_state_t *ohcip)
{
	usba_cb_lsr_t usba_lsr;
	...
	if (ohci_allocate_pools(ohcip) != USB_SUCCESS) {

		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_live_resume: failed to allocate pools");
	...
	if (ohci_init_hcca(ohcip) != USB_SUCCESS) {

		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_live_resume: failed to allocate hcca");
	...
	/* Set the HcHCCA to the physical address of the HCCA block */
	Set_OpReg(hcr_HCCA,
	    (uint_t)ohcip->ohci_hcca_cookie.dmac_address);

lr_end:
	/* Now resume ohci host controller */
	ohcip->ohci_fno = 0;
	...
	if (ohci_init_ctlr(ohcip) != DDI_SUCCESS) {
		...
		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_live_resume: ohci host controller resume failed ");
	...
	/* Now resume the root hub and all descendents */
	if (usba_hubdi_post_lsr(ohcip->ohci_dip, &usba_lsr) != USB_SUCCESS) {

		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_live_resume: root hub and descendants resume failed");
 ...
 */
static int
ohci_lsr_capability(ohci_state_t *ohcip, void *cbarg)
{
	ddi_cb_lsr_t *plsr;
	usba_cb_lsr_t usba_lsr;
	...
	usba_lsr.usba_lsr_cbarg.activities = OHCI_CB_LSR_ACTIVITIES;
	usba_lsr.usba_lsr_cbarg.impacts = OHCI_CB_LSR_IMPACTS;
	usba_lsr.usba_lsr_cbret = USB_SUCCESS;

	/*
	 * Query descendant's lsr capability
	 */
	if ((usba_hubdi_post_lsr(ohcip->ohci_dip, &usba_lsr)) !=
	    USB_SUCCESS) {
		USB_DPRINTF_L2(PRINT_MASK_LSR, ohcip->ohci_log_hdl,
		    "ohci_lsr_capability - failed to query lsr capability");
	...
	usb_flags_t		flag)
{
	...
	if (action == CLEAR_sKip) {
		...
		if (flag & OHCI_FLAGS_DMA_SYNC) {
			...
			if ((ohci_check_dma_handle(ohcip,
			    ohcip->ohci_td_pool_dma_handle) != DDI_FM_OK) ||
			    (ohci_check_dma_handle(ohcip,
			    ohcip->ohci_ed_pool_dma_handle) != DDI_FM_OK)) {
				USB_DPRINTF_L2(PRINT_MASK_INTR,
				    ohcip->ohci_log_hdl,
				    "ohci_modify_sKip_bit: FM check dma"
				    " handle error");

				ddi_fm_service_impact(ohcip->ohci_dip,
	...
	usb_flags_t		flags)
{
	...
	if (ohci_check_acc_handle(ohcip,
	    ohcip->ohci_regs_handle) != DDI_FM_OK) {
		ddi_fm_service_impact(ohcip->ohci_dip,
	...
	if (tw && (flags & OHCI_FLAGS_DMA_SYNC)) {
		...
		if ((ohci_check_dma_handle(ohcip,
			...
			USB_DPRINTF_L2(PRINT_MASK_INTR, ohcip->ohci_log_hdl,
			    "ohci_xfer_timeout_handler:FM check dma hdl error");
...
ohci_intr(caddr_t arg1, caddr_t arg2)
{
	...
	if (ohcip->ohci_hccap) {
		/* Sync HCCA area */
		Sync_HCCA(ohcip);
		if (ohci_check_dma_handle(ohcip, ohcip->ohci_hcca_dma_handle)
		    != DDI_FM_OK) {
			USB_DPRINTF_L3(PRINT_MASK_INTR, ohcip->ohci_log_hdl,
			    "ohci_intr: FM check dma hdl error");
	...
	ohci_td_t		*head_done_list)
{
	...
	/* Sync ED and TD pool */
	Sync_ED_TD_Pool(ohcip);
	if ((ohci_check_dma_handle(ohcip, ohcip->ohci_td_pool_dma_handle)
	    != DDI_FM_OK) ||
	    (ohci_check_dma_handle(ohcip, ohcip->ohci_ed_pool_dma_handle)
	    != DDI_FM_OK)) {
		USB_DPRINTF_L2(PRINT_MASK_INTR, ohcip->ohci_log_hdl,
		    "ohci_traverse_done_list: FM check dma handle error");

		ddi_fm_service_impact(ohcip->ohci_dip, DDI_SERVICE_DEGRADED);
	}
	...
	usb_cr_t		error)
{
	...
	if (length) {
		...
		if ((eptd->bmAttributes & USB_EP_ATTR_MASK) ==
			...
			if (ohci_check_dma_handle(ohcip, tw->tw_dmahandle) !=
			    DDI_FM_OK) {
				USB_DPRINTF_L3(PRINT_MASK_INTR,
				    ohcip->ohci_log_hdl,
				    "ohci_sendup_qtd_message: "
...
static void
ohci_fm_init(ohci_state_t *ohcip)
{
	if (ohcip->ohci_fm_cap) {
		/* we do not care the iblk cookie */
		ddi_fm_init(ohcip->ohci_dip, &ohcip->ohci_fm_cap, NULL);

		/*
		 * Initialize pci ereport capabilities if ereport capable
		 */
		if (DDI_FM_EREPORT_CAP(ohcip->ohci_fm_cap) ||
		    DDI_FM_ERRCB_CAP(ohcip->ohci_fm_cap))
			pci_ereport_setup(ohcip->ohci_dip);

		/*
		 * Register error callback if error callback capable
		 */
		if (DDI_FM_ERRCB_CAP(ohcip->ohci_fm_cap))
			ddi_fm_handler_register(ohcip->ohci_dip,
			    ohci_fm_error_cb, (void*) ohcip);
	}
}

static void
ohci_fm_fini(ohci_state_t *ohcip)
{
	if (ohcip->ohci_fm_cap) {
		if (DDI_FM_EREPORT_CAP(ohcip->ohci_fm_cap) ||
		    DDI_FM_ERRCB_CAP(ohcip->ohci_fm_cap))
			pci_ereport_teardown(ohcip->ohci_dip);

		if (DDI_FM_ERRCB_CAP(ohcip->ohci_fm_cap))
			ddi_fm_handler_unregister(ohcip->ohci_dip);

		ddi_fm_fini(ohcip->ohci_dip);
	}
}

/*
 * report a device IO error.
	...
	char		*detail)
{
	...
	if (DDI_FM_EREPORT_CAP(ohcip->ohci_fm_cap)) {
		(void) snprintf(buf, FM_MAX_CLASS, "%s.%s",
		    DDI_FM_DEVICE, detail);
		ena = fm_ena_generate(0, FM_ENA_FMT1);
		ddi_fm_ereport_post(ohcip->ohci_dip, buf, ena, DDI_NOSLEEP,
		    FM_VERSION, DATA_TYPE_UINT8, FM_EREPORT_VERS0, NULL);
	}
}

/* We're not going to handle PIO access errors */
int
ohci_check_acc_handle(
	ohci_state_t		*ohcip,

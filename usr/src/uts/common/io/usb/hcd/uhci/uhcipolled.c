...
uhci_polled_fini(uhci_polled_t *uhci_polledp, uhci_state_t *uhcip)
{
	 ...
	 */
	uhci_free_tw(uhcip, uhci_polledp->uhci_polled_tw);
	uhci_polledp->uhci_polled_tw = NULL;
	...
	usba_pipe_handle_data_t *ph)
{
	...
	/*
	 * Save the transfer wrapper in uhci_polledp to prevent kmdb from
	 * indicating it's a memory leak mistakenly.
	 */
	uhci_polledp->uhci_polled_tw = tw;

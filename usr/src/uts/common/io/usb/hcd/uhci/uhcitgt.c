...
uhci_hcdi_pipe_open(usba_pipe_handle_data_t *ph, usb_flags_t flags)
{
	...
	rval = uhci_state_is_operational(uhcip, ph);
	...
	/* needn't allocate qh for root hub */
	if ((UHCI_XFER_TYPE(&ph->p_ep) != USB_EP_ATTR_ISOCH) &&
	    (usb_addr != ROOT_HUB_ADDR)) {
	...
	usb_flags_t		flags)
{
	...
	mutex_enter(&uhcip->uhci_int_mutex);
	error = uhci_state_is_operational(uhcip, ph);
    ...
    usb_bulk_req_t *bulk_reqp, usb_flags_t usb_flags)
{
	...
	error = uhci_state_is_operational(uhcip, pipe_handle);
	...
	size_t		*size)
{
	...
	mutex_enter(&uhcip->uhci_int_mutex);
	rval = uhci_state_is_operational(uhcip, NULL);
	...
	usb_flags_t		flags)
{
	...
	rval = uhci_state_is_operational(uhcip, pipe_handle);

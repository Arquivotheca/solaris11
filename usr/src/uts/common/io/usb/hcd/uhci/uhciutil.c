...
uhci_set_dma_attributes(uhci_state_t *uhcip)
{
	...
	uhcip->uhci_dma_attr.dma_attr_addr_hi = 0xffffffffull;
...
uhci_wait_for_sof(uhci_state_t *uhcip)
{
	...
	error = uhci_state_is_operational(uhcip, NULL);
	...
	for (n = 0; n < MAX_SOF_WAIT_COUNT; n++) {
		...
		if ((rval == -1) &&
			...
			if (uhcip->uhci_hc_soft_state ==
			    UHCI_CTLR_STOPPED_STATE)
				uhcip->uhci_hc_soft_state =
 ...
 * In UHCI_CTLR_STOPPED_STATE state, the root hub is still
 * operational.
 */
int
uhci_state_is_operational(uhci_state_t	*uhcip, usba_pipe_handle_data_t *ph)
{
	int	val;

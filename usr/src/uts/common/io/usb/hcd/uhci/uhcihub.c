...
static	void	uhci_handle_port_suspend(
			...
			usb_port_t		port,
			uint_t			on);
static void uhci_handle_clrchng_port_suspend(
			uhci_state_t		*uhcip,
			usb_port_t		port);
...
static	void	uhci_rh_hcdi_callback(
			...
			usb_opaque_t		req,
			usb_cr_t		cr);
static int	uhci_is_any_port_active(
			uhci_state_t		*uhcip);
static void	uhci_stop_host_ctlr(
			uhci_state_t		*uhcip);
static void	uhci_restart_host_ctlr(
			uhci_state_t    *uhcip);
	...
	usb_port_t		port)
{
	...
	case USB_REQ_CLEAR_FEATURE:
		...
		case CFS_C_PORT_SUSPEND:
			uhci_handle_clrchng_port_suspend(uhcip, port);
...
/*
 * uhci_handle_clrchng_port_suspend:
 */
static void
uhci_handle_clrchng_port_suspend(
	uhci_state_t		*uhcip,
	usb_port_t		port)
{
	...
	USB_DPRINTF_L4(PRINT_MASK_ROOT_HUB, uhcip->uhci_log_hdl,
	    "uhci_handle_clrchng_port_suspend: port=%d",
	    port);


	uhcip->uhci_root_hub.rh_port_changes[port] &= ~PORT_CHANGE_PSSC;
...
uhci_handle_root_hub_status_change(void *arg)
{
	...
	for (port = 0; port < uhcip->uhci_root_hub.rh_num_ports; port++) {
		...
		/* See if a device was inserted/removed, or resumed */
		if (change_status & (PORT_STATUS_CCS|PORT_STATUS_PSS)) {
	...
	/* Device connected. Restart host controller. */
	port_active = uhci_is_any_port_active(uhcip);
	hc_status = Get_OpReg16(USBSTS);
	if ((uhcip->uhci_hc_soft_state == UHCI_CTLR_STOPPED_STATE) &&
	    (port_active == USB_SUCCESS || all_ports_status)) {
		uhci_restart_host_ctlr(uhcip);
	}
	...
	/*
	 * No device connected and no outstanding tds. Stop host controller.
	 */
	if ((port_active == USB_FAILURE) && !all_ports_status &&
	    (uhcip->uhci_outst_tds_head == NULL) &&
	    !(hc_status & USBSTS_REG_HC_HALTED)) {
		uhci_stop_host_ctlr(uhcip);
	}
	...
	usb_port_t	port)
{
	...
	/*
	 * When a resume event is detected on the port, HCR_PORT_RESUME_DETECT
	 * is set, but HCR_PORT_SUSPEND doesn't get cleared automatically.
	 * In this case we should clear the bits and report a resume event.
	 */
	if (port_status & HCR_PORT_SUSPEND) {
...
/*
 * uhci_is_any_port_active
 */
static int
uhci_is_any_port_active(uhci_state_t    *uhcip)
{
	usb_port_t	port;
	ushort_t	port_status;
	int		val = USB_FAILURE;
	...
	/*
	 * Check Port Enable/Disable Change, Connect Status Change
	 * and Current Connect Status bits for each port.
	 */
	for (port = 0; port < uhcip->uhci_root_hub.rh_num_ports; port ++) {
		port_status = Get_OpReg16(PORTSC[port]);
		if (port_status & (HCR_PORT_CSC |
		    HCR_PORT_ENDIS_CHG | HCR_PORT_CCS)) {
			val = USB_SUCCESS;
			break;
		}
	}
 ...
 */
static void
uhci_stop_host_ctlr(uhci_state_t    *uhcip)
{
	uint16_t	cmd_reg;
	int		i;
	...
	USB_DPRINTF_L3(PRINT_MASK_ATTA, uhcip->uhci_log_hdl,
	    "uhci_stop_host_ctlr: waited %d milliseconds for hc to halt", i);

	/* Host controller cannot halt */
	if (i > UHCI_TIMEWAIT / 1000) {
		USB_DPRINTF_L2(PRINT_MASK_ATTA, uhcip->uhci_log_hdl,
		    "uhci_stop_host_ctlr: Controller cannot be stopped.");
	...
	/* Set host controller soft state to stopped */
	uhcip->uhci_hc_soft_state = UHCI_CTLR_STOPPED_STATE;
}

/*
 *
 * uhci_restart_host_ctlr
 */
static void
uhci_restart_host_ctlr(uhci_state_t    *uhcip)
{
	uint_t	cmd_reg;
	...
	Set_OpReg16(USBCMD, (USBCMD_REG_MAXPKT_64 |
	    USBCMD_REG_CONFIG_FLAG | USBCMD_REG_HC_RUN));

	cmd_reg = Get_OpReg16(USBCMD);
	if (!(cmd_reg & (USBCMD_REG_HC_RUN | USBCMD_REG_MAXPKT_64 |
	    USBCMD_REG_CONFIG_FLAG))) {
		USB_DPRINTF_L2(PRINT_MASK_ATTA, uhcip->uhci_log_hdl,
		    "uhci_restart_host_ctlr: Controller restarted failed");

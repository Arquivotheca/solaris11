...
static int usb_mid_bus_fm_init(dev_info_t *self,
			dev_info_t *child,
			int cap,
...
static struct bus_ops usb_mid_busops = {
	...
	usb_mid_bus_fm_init,		/* bus_fm_init */
	NULL,				/* bus_fm_fini */
...
/* ARGSUSED */
static int
usb_mid_bus_fm_init(dev_info_t *self, dev_info_t *child, int cap,
    ddi_iblock_cookie_t *ibc)
{
	usba_device_t *self_ud = usba_get_usba_device(self);

	/*
	 * Interfaces occupied the same usba_device, so use the
	 * unique usba_device here
	 */
	return (self_ud? self_ud->usb_fm_capable : usba_fm_capable);
}

...
int	hid_pm_mouse = 0; /* enable power management D0/D3 states for mouse */
...
static void hid_default_pipe_exception_callback(usb_pipe_handle_t,
		usb_ctrl_req_t *);
static void hid_lsr_event_callback(dev_info_t *,
...
static int hid_live_suspend(hid_state_t *hidp);
static int hid_live_resume(hid_state_t *hidp);
...
/* HID callback for live suspend/resume */
static int	hid_cbfunc(hid_state_t *, ddi_cb_action_t, void *);
...
/*
 * hid live suspend/resume routine
 */
/*ARGSUSED*/
static int
hid_cbfunc(hid_state_t *hidp, ddi_cb_action_t cbaction, void *cbarg)
{
	ddi_cb_lsr_t *plsr;
	...
	switch (cbaction) {
		case DDI_CB_LSR_SUSPEND:
			...
			USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_cbfunc - receive live suspend: "
			    "activities is %llu, impacts is %llu\n",
			...
			if (hidp->hid_lsr_state == HID_LSR_SUSPENDED) {
				mutex_exit(&hidp->hid_mutex);

				return (USB_SUCCESS);
			}

			hidp->hid_lsr_state = HID_LSR_SUSPENDED;
			hidp->hid_lsr_activities = plsr->activities;
			hidp->hid_lsr_impacts = plsr->impacts;
			 ...
			 */
			USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_cbfunc - do live suspend\n");

			if (hid_live_suspend(hidp) != USB_SUCCESS) {
				USB_DPRINTF_L2(PRINT_MASK_LSR,
				    hidp->hid_log_handle,
				    "hid_cbfunc - failed to do live suspend");

				mutex_enter(&hidp->hid_mutex);

				hidp->hid_lsr_state = 0;
				hidp->hid_lsr_activities = 0;
				hidp->hid_lsr_impacts = 0;
		...
		case DDI_CB_LSR_RESUME:
			...
			USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_cbfunc - receives live resume: "
			    "activities is %llu, impacts is %llu\n",
			    (unsigned long long)plsr->activities,
			    (unsigned long long)plsr->impacts);
			...
			if ((plsr->activities != hidp->hid_lsr_activities) ||
			    (plsr->impacts != hidp->hid_lsr_impacts)) {
				mutex_exit(&hidp->hid_mutex);
			 ...
			 */
			USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_cbfunc - do live resume\n");

			if (hid_live_resume(hidp) != USB_SUCCESS) {
				USB_DPRINTF_L2(PRINT_MASK_LSR,
				    hidp->hid_log_handle,
				    "hid_cbfunc - failed to do live resume");
		...
		case DDI_CB_LSR_QUERY_CAPABILITY:
			USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_cbfunc - do query cabability\n");

			plsr = (ddi_cb_lsr_t *)cbarg;

			plsr->activities &= HID_CB_LSR_ACTIVITIES;
			plsr->impacts &= HID_CB_LSR_IMPACTS;

			break;
		default:
			USB_DPRINTF_L2(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_cbfunc - unknown lsr action %x", cbaction);
 ...
 * hid_live_suspend
 *
 * This function will put hid driver to SUSPEND state first,
 * then do additional operation according to arguments
 * passed down from Host Controller driver.
 */
static int
hid_live_suspend(hid_state_t *hidp)
{
	USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
	    "hid_live_suspend - dip=0x%p", (void *)hidp->hid_dip);

	if (hid_cpr_suspend(hidp) != USB_SUCCESS) {
		USB_DPRINTF_L2(PRINT_MASK_LSR, hidp->hid_log_handle,
		    "hid_live_suspend - failed to do suspend");
	...
	/*
	 * In polled-in mode, hid will drive DMA operation.
	 * So if hid gets DMA impacts during live suspend,
	 * it will release allocated DMA resource.
	 */
	if (!(hidp->hid_lsr_impacts & (DDI_CB_LSR_IMP_DMA_ADDR_CHANGE |
	...
	if (hidp->hid_polled_console_info) {
		mutex_enter(&hidp->hid_mutex);
		hidp->hid_lsr_polled = B_TRUE;
		mutex_exit(&hidp->hid_mutex);

		if (hid_polled_input_fini(hidp) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_live_suspend - failed to release memory");

			hid_cpr_resume(hidp);

			mutex_enter(&hidp->hid_mutex);
			hidp->hid_lsr_polled = B_FALSE;
			mutex_exit(&hidp->hid_mutex);
 ...
 * hid_live_resume
 *
 * This function will resume driver from live suspend.
 */
static int
hid_live_resume(hid_state_t *hidp)
{
	USB_DPRINTF_L4(PRINT_MASK_LSR, hidp->hid_log_handle,
	    "hid_live_resume - dip=0x%p", (void *)hidp->hid_dip);

	mutex_enter(&hidp->hid_mutex);

	/* If the device is disconnected, we keep the state */
	...
	if (hidp->hid_lsr_polled == B_TRUE) {
		mutex_exit(&hidp->hid_mutex);

		if (hid_polled_input_init(hidp) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_LSR, hidp->hid_log_handle,
			    "hid_live_resume - failed to alloc memory");

			(void) hid_cpr_suspend(hidp);
...
hid_disconnect_event_callback(dev_info_t *dip)
{
	...
	case USB_DEV_SUSPENDED:
		/*
		 * If device is disconnected after live suspend,
		 * we need to keep the disconnect state, and
		 * check the state when doing live resume.
		 * We remain cpr suspended.
		 */
		if (hidp->hid_lsr_state == HID_LSR_SUSPENDED) {
			hidp->hid_dev_state = USB_DEV_DISCONNECTED;
		}
...
/*
 * hid lsr event callback
 */
/*ARGSUSED*/
static void
hid_lsr_event_callback(dev_info_t *dip,
    ddi_eventcookie_t cookie, void *arg, void *bus_impldata)
{
	...
	if ((hid_cbfunc(hidp, cbaction, cbarg)) != USB_SUCCESS) {
		USB_DPRINTF_L2(PRINT_MASK_LSR, hidp->hid_log_handle,
		    "hid_lsr_event_callback - "
		    "failed to execute lsr callback\n");
...
hid_create_pm_components(dev_info_t *dip, hid_state_t *hidp)
{
	...
	case MOUSE_PROTOCOL:
		...
		/*
		 * remote wakeup is always enabled on all kbd/mouse
		 * so that they can resume a system in S3 state
		 */
		...
		/*
		 * by default, pm-components is only created for kbd, not for
		 * mouse, consistent with the hid(7d) man page
		 */
		if (hid_is_pm_enabled(dip) == USB_SUCCESS) {
...
hid_is_pm_enabled(dev_info_t *dip)
{
	hid_state_t	*hidp = ddi_get_soft_state(hid_statep,
	    ddi_get_instance(dip));
	boolean_t	hid_pm_keyboard = B_TRUE;
	...
	if (strcmp(ddi_node_name(dip), "mouse") == 0) {
	...
	} else if ((strcmp(ddi_node_name(dip), "keyboard") == 0) &&
	    (strcmp(ddi_driver_name(pdip), "usb_mid") == 0)) {
		/*
		 * USB KVM switch has built-in usb mid per port,the mid has its
		 * upstream port connected to a internal hub, and in downstream
		 * side, there are two interfaces to which keyboard and mice is
		 * connected respectively.
		 * When keyboard/mice exist simultaneously,keyboard will switch
		 * from D0 to D3 state when autopm is enabled,but mice is still
		 * alive. Under this circumstances the mid won't get into power
		 * management state and port suspend status won't be set on the
		 * port the mid is connecting to, as a result,no suspend/resume
		 * event can be detected by host controler,keyboard will always
		 * in power off state.
		 * To avoid this situation, just disable the keyboard's pm when
		 * using USB KVM.
		 */
		 ...
		 * serialize access
		 */
		ndi_devi_enter(pdip, &pcirc);

		for (cdip = ddi_get_child(pdip); cdip;
		    cdip = ddi_get_next_sibling(cdip)) {
			if (strcmp(ddi_node_name(cdip), "mouse") == 0) {
				hid_pm_keyboard = B_FALSE;
				break;
			}
		}

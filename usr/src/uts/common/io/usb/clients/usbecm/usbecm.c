			...
			if (ecmp->ecm_rcv_queue == NULL) {
				 ...
				 */
					USB_DPRINTF_L2(PRINT_MASK_CB,
					    ecmp->ecm_lh, "usbecm_bulkin_cb: "
					    "Drop big packet, rcvlen=%d, new "
					    "data_len=%d",
					    (int)msgsize(ecmp->ecm_rcv_queue),
					    data_len);
...
usbecm_send_data(usbecm_state_t *ecmp, mblk_t *data)
{
	...
	if (rval != USB_SUCCESS) {
		 ...
		 * packet to device if the total data length is multiple of
...
usbecm_restore_device_state(usbecm_state_t *ecmp)
{
	...
	if (ecmp->ecm_mac_state == USBECM_MAC_STARTED) {
		...
		if (usbecm_open_pipes(ecmp) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_OPS, ecmp->ecm_lh,
			    "usbecm_restore_device_state: open pipes fail");
		...
		if (rval != USB_SUCCESS) {
			USB_DPRINTF_L3(PRINT_MASK_OPS, ecmp->ecm_lh,
			    "usbecm_restore_device_state: set packet filters"
			    " fail, rval=%d, continue", rval);
		}

		if (ECM_DS_OP_VALID(ecm_ds_start)) {
			if (ecmp->ecm_ds_ops->ecm_ds_start(ecmp) !=
			    USB_SUCCESS) {
				USB_DPRINTF_L2(PRINT_MASK_OPS, ecmp->ecm_lh,
				    "usbecm_restore_device_state: Can't start"
				    " hardware");
				usbecm_close_pipes(ecmp);
		...
		if (usbecm_rx_start(ecmp) != USB_SUCCESS) {
			USB_DPRINTF_L2(PRINT_MASK_OPS, ecmp->ecm_lh,
			    "usbecm_restore_device_state: fail to start_rx");
			ecmp->ecm_dev_state = state;
			mutex_exit(&ecmp->ecm_mutex);
		...
		/*
		 * To mark the link as RUNNING. And to tell upper
		 * layer we can accept packets.
		 */
		mac_link_update(ecmp->ecm_mh, LINK_STATE_UP);
		mac_tx_update(ecmp->ecm_mh);
	...
	USB_DPRINTF_L4(PRINT_MASK_OPS, ecmp->ecm_lh,
	    "usbecm_restore_device_state: end");

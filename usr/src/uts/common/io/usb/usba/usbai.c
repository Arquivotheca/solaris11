	...
	usb_flags_t flags)
{
	...
	if (usb_evdata->lsr_event_handler != NULL) {
		if (usba_device->lsr_cookie == NULL) {
			if (ddi_get_eventcookie(dip, USBA_LSR_EVENT,
			    &usba_device->lsr_cookie) != DDI_SUCCESS) {

				goto fail;
			}
		}
		if (ddi_add_event_handler(dip, usba_device->lsr_cookie,
		    (peh_t)usb_evdata->lsr_event_handler,
		    NULL, &evdata->ev_lsr_cb_id) != DDI_SUCCESS) {

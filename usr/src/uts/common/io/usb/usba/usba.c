...
usba_free_usba_device(usba_device_t *usba_device)
{
	...
	if (usba_device->usb_fm_err_count) {
		kmem_free(usba_device->usb_fm_err_count,
		    usba_device->usb_n_ifs * sizeof (int));
	}
	/*
 ...
 *	Remove duplicate strings if mfg is the same with product.
 *
 * Arguments:
    ...
    int		buflen)
{
	...
	if ((usba_device->usb_product_str) &&
		...
		if (strcmp(buffer, usba_device->usb_product_str) != 0) {
			if ((return_len > 0) && (return_len < (buflen - 1))) {
				buffer[return_len++] = ' ';
			}

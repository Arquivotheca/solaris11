...
/* default USB client driver "fm-capable" property value */
int	usba_fm_capable = DDI_FM_EREPORT_CAPABLE;
...
/*
 * Set up basic FM capability for a client driver. If this
 * procedure fails, just skip.
 */
static void
usba_setup_client_fm(dev_info_t *dip, usba_device_t *usba_device)
{
	int	capable;
	ddi_iblock_cookie_t icookie;

	/*
	 * scsa2usb driver has been setup with basic fm capability in
	 * SCSA framework. See scsa_nexus_setup().
	 */
	if (strcmp(ddi_driver_name(dip), "scsa2usb") == 0) {

		return;
	}

	/*
	 * NOTE: USBA maintains an 'fm-capable' domain, in usb_fm_capable,
	 * that is not dependent (limited by) on the capabilities of its
	 * clients.
	 *
	 * Get 'fm-capable' property from driver.conf, if present. If not
	 * present, default to the usba_fm_capable global (which has
	 * DDI_FM_EREPORT_CAPABLE set by default).
	 */

	mutex_enter(&usba_device->usb_mutex);
	if (usba_device->usb_fm_capable == DDI_FM_NOT_CAPABLE)
		usba_device->usb_fm_capable = ddi_prop_get_int(DDI_DEV_T_ANY,
		    dip, DDI_PROP_NOTPROM | DDI_PROP_DONTPASS,
		    "fm-capable", usba_fm_capable);

	/*
	 * If a client is *not* doing its own fma support by calling
	 * ddi_fm_init() prior to usb_client_attach(), we provide a
	 * minimal common USBA implementation so that USBA children
	 * can generate ereports via usb_fm_ereport_post().  We use
	 * ddi_fm_capable() to detect a USBA client calling ddi_fm_init()
	 * prior to usb_client_attach().
	 */
	if (usba_device->usb_fm_capable &&
		 ...
		 *
		 * NOTE: use parent host controller's soft interrupt iblock
		 * cookies.
		 */
		capable = usba_device->usb_fm_capable;
		icookie = usba_hcdi_get_hcdi(usba_device->usb_root_hub_dip)->
		    hcdi_soft_iblock_cookie;
		ddi_fm_init(dip, &capable, &icookie);
		/*
		 * Set USBA_CLIENT_FLAG_FM_INIT bit to mark us as using the
		 * common minimal USBA fm implementation -  we called
		 * ddi_fm_init(), so we are responsible for calling
		 * ddi_fm_fini() in usb_client_detach().
		 * NOTE: if ddi_fm_init fails in any reason, SKIP.
		 * Skip usb_mid and usb_ia device here because interface0
		 * share the same usb_client_flags with usb_mid or usb_ia
		 */
		if (strcmp(ddi_driver_name(dip), "usb_mid") != 0 &&
		    strcmp(ddi_driver_name(dip), "usb_ia") != 0) {
			if (DEVI(dip)->devi_fmhdl) {
				usba_device->
				    usb_client_flags[usba_get_ifno(dip)] |=
				    USBA_CLIENT_FLAG_FM_INIT;
				usba_device->
				    usb_fm_err_count[usba_get_ifno(dip)] = 0;
			}
		} else {
			if (DEVI(dip)->devi_fmhdl) {
				usba_device->
				    usb_fm_err_ext_count = 0;
			}
		}
	}
...
static void
usba_teardown_client_fm(dev_info_t *dip, usba_device_t *usba_device)
{
	if ((dip == NULL) || (usba_device == NULL)) {
		return;
	}

	/*
	 * scsa2usb driver not handle here. See scsa_nexus_teardown
	 */
	if (strcmp(ddi_driver_name(dip), "scsa2usb") == 0) {
	...
	if (DEVI(dip)->devi_fmhdl) {
		/*
		 * usb_client_flags not available for usb_mid and
		 * usb_ia, See it in usba_setup_client_fm
		 */
		if (strcmp(ddi_driver_name(dip), "usb_mid") == 0 ||
		    strcmp(ddi_driver_name(dip), "usb_ia") == 0) {
			ddi_fm_fini(dip);
		...
		if (usba_device->usb_client_flags[usba_get_ifno(dip)] &
		    USBA_CLIENT_FLAG_FM_INIT) {
			ddi_fm_fini(dip);
			usba_device->
			    usb_client_flags[usba_get_ifno(dip)] &=
...
usb_client_attach(dev_info_t *dip, uint_t version, usb_flags_t flags)
{
	...
	usba_setup_client_fm(dip, usba_device);
...
usb_client_detach(dev_info_t *dip, usb_client_dev_data_t *reg)
{
	...
	if (dip) {
		...
		usba_teardown_client_fm(dip, usba_device);
	}
...
usba_process_if_descr(usba_reg_state_t *state, boolean_t *requested_if)
{
	...
	/* No config precedes this interface. */
...
usba_process_ep_descr(usba_reg_state_t *state)
{
	...
	/* No interface precedes this endpoint. */
...
usba_process_ep_comp_descr(usba_reg_state_t *state)
{
	...
	/* No endpoint descr precedes this descr */

		...
		size_t		buflen)
{
	...
	if ((dip == NULL) || (buf == NULL) || (buflen == 0) ||
	    (index == 0) || (langid != USB_LANG_ID)) {
		if (langid != USB_LANG_ID)
			USB_DPRINTF_L2(DPRINT_MASK_USBA, usbai_log_handle,
			    "langid 0x%x not supported", langid);
		return (USB_INVALID_ARGS);
	}
...
usb_common_pwrlvl3(dev_info_t *dip, uint8_t *pm, int *dev_state)
{
	...
	case USB_DEV_PWRED_DOWN:
		...
		*pm = USB_DEV_OS_FULL_PWR;
		...
		/*
		 * A hid device in D3 state must have stopped its interrupt
		 * polling, but it may trigger a remote wakeup event on the
		 * parent hub port. Thus hubd_hotplug_thread() fires and may
		 * call pm_raise_power() to powerup the child dip, which can
		 * be usb_mid. That's why we're here. We must relay the call
		 * down to the hid children, otherwise the hids won't be
		 * notified to restart interrupt polling and will remain in
		 * D3 state.
		 */
		...
		for (cdip = ddi_get_child(dip); cdip;
			...
			(void) pm_raise_power(cdip, 0, USB_DEV_OS_FULL_PWR);
		}
...
usba_common_unregister_events(dev_info_t *dip, uint_t if_num)
{
	...
	if (evdata->ev_lsr_cb_id != NULL) {
		(void) ddi_remove_event_handler(evdata->ev_lsr_cb_id);
		evdata->ev_lsr_cb_id = NULL;
	}

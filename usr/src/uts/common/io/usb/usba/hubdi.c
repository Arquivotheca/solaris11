...
static int hubd_bus_fm_init_child(dev_info_t *self, dev_info_t *child,
...
static int hubd_cbfunc(dev_info_t *, ddi_cb_action_t, void *);
...
struct bus_ops usba_hubdi_busops =	{
	...
	hubd_bus_fm_init_child,		/* bus_fm_init */
	NULL,				/* bus_fm_fini */
...
extern int		usba_fm_capable; /* Default usb bus fm capability */
	...
	usb_dev_descr_t *root_hub_device_descriptor)
{
	...
	usba_device->usb_fm_err_count = kmem_zalloc(
	    usba_device->usb_n_ifs * sizeof (int), KM_SLEEP);
...
static void hubd_lsr_event_cb(dev_info_t *dip, ddi_eventcookie_t cookie,
...
static int hubd_wait_for_hotplug_exit(hubd_t *hubd);
...
static ndi_event_definition_t hubd_ndi_event_defs[] = {
	...
	{USBA_EVENT_TAG_PRE_SUSPEND, USBA_PRE_SUSPEND_EVENT, EPL_KERNEL,
						NDI_EVENT_POST_TO_ALL},
	{USBA_EVENT_TAG_LSR, USBA_LSR_EVENT, EPL_KERNEL,
						NDI_EVENT_POST_TO_ALL}
};
...
/*
 * bus_fm_init entry point :
 *     Return the fm capbility of the usb bus
 *     This func is called only for usb leaf driver.
 *     There's no limitation for the usb children device
 *     The minimal is capbility of ereport.
 */
/* ARGSUSED */
static int
hubd_bus_fm_init_child(dev_info_t *self, dev_info_t *child, int cap,
    ddi_iblock_cookie_t *ibc)
{
	usba_device_t *child_ud = usba_get_usba_device(child);

	/* Use the cap from children(client driver)'s usba_device */
	return (child_ud? child_ud->usb_fm_capable : usba_fm_capable);
}
	...
	for (port = 1; port <= hubd->h_hub_descr.bNbrPorts; port++) {
		...
		if (ch_dip) {
			...
			if (status & PORT_STATUS_CCS) {
				...
				/*
				 * This is a workaround to cleanup children
				 * when child-devices are disconnected before
				 * live resume.
				 */
				if (hubd->h_lsr_state == HUBD_LSR_SUSPENDED) {
					mutex_exit(HUBD_MUTEX(hubd));
					...
					/*
					 * Mark this dip for deletion as the
					 * device is not physically present,
					 * and schedule cleanup thread upon
					 * post resume.
					 */
					mutex_exit(HUBD_MUTEX(hubd));
    ...
    uint_t config_index)
{
	...
	child_ud->usb_fm_err_count = kmem_zalloc(
	    child_ud->usb_n_ifs * sizeof (int), KM_SLEEP);
...
hubd_pre_suspend_event_cb(dev_info_t *dip)
{
	...
	if (hubd_wait_for_hotplug_exit(hubd) == USB_FAILURE) {
		...
		USB_DPRINTF_L2(DPRINT_MASK_EVENTS,
		    hubd->h_log_handle, "hubd_pre_suspend_event_cb: "
		    "Time out for waiting for hotplug_thread or ioctl exit.");
		mutex_exit(HUBD_MUTEX(hubd));
...
/*
 * hubd_lsr_event_cb
 * 	trigger hubd lsr callback
 */
/* ARGSUSED */
static void
hubd_lsr_event_cb(dev_info_t *dip, ddi_eventcookie_t cookie,
    void *arg, void *bus_impldata)
{
	hubd_t *hubd;
	usba_cb_lsr_t *plsr;
	...
	hubd = (hubd_t *)hubd_get_soft_state(dip);
	ASSERT(hubd != NULL);

	plsr = (usba_cb_lsr_t *)bus_impldata;
	cbaction = plsr->usba_lsr_cbaction;
	cbarg = &plsr->usba_lsr_cbarg;
	cbret = &plsr->usba_lsr_cbret;

	if ((hubd_cbfunc(dip, cbaction, cbarg)) != USB_SUCCESS) {
		USB_DPRINTF_L2(DPRINT_MASK_LSR, hubd->h_log_handle,
		    "hubd_lsr_even_cb - failed to execute lsr callback");
...
hubd_cpr_post_user_callb(void *arg, int code)
{
	...
	case CB_CODE_CPR_CHKPT:
		...
		return ((hubd_pre_suspend_event_cb(hubd->h_dip) == USB_SUCCESS)?
		    B_TRUE : B_FALSE);
...
/*
 * hubd live suspend/resume routine
 */
/* ARGSUSED */
static int
hubd_cbfunc(dev_info_t *dip, ddi_cb_action_t cbaction, void *cbarg)
{
	hubd_t *hubd;
	ddi_cb_lsr_t *plsr;
	...
	usba_lsr.usba_lsr_cbaction = cbaction;
	usba_lsr.usba_lsr_cbarg = *(ddi_cb_lsr_t *)cbarg;
	...
	case DDI_CB_LSR_SUSPEND:
		...
		USB_DPRINTF_L4(DPRINT_MASK_LSR, hubd->h_log_handle,
		    "hubd_cbfunc - receives live suspend: "
		...
		hubd->h_lsr_state = HUBD_LSR_SUSPENDED;
		hubd->h_lsr_activities = plsr->activities;
		hubd->h_lsr_impacts = plsr->impacts;

		mutex_exit(HUBD_MUTEX(hubd));

		/*
		 * Post LSR Suspend event to children
		 */
		if (usba_hubdi_post_lsr(dip, &usba_lsr) !=
		    USB_SUCCESS) {
			USB_DPRINTF_L2(DPRINT_MASK_LSR, hubd->h_log_handle,
			    "hubd_cbfunc - failed to post suspend");

			usba_lsr.usba_lsr_cbaction = DDI_CB_LSR_RESUME;
			(void) usba_hubdi_post_lsr(dip, &usba_lsr);

			mutex_enter(HUBD_MUTEX(hubd));

			hubd->h_lsr_state = 0;
			hubd->h_lsr_activities = 0;
			hubd->h_lsr_impacts = 0;
		 ...
		 * Do hubd suspend
		 */
		if (hubd_cpr_suspend(hubd) != USB_SUCCESS) {
			USB_DPRINTF_L2(DPRINT_MASK_LSR, hubd->h_log_handle,
			    "hubd_cbfunc - failed to do live suspend");
	...
	case DDI_CB_LSR_RESUME:
		...
		USB_DPRINTF_L4(DPRINT_MASK_LSR, hubd->h_log_handle,
		    "hubd_cbfunc - receives live resume: "
		    "activities is %llx, impacts is %llx",
		    (unsigned long long)plsr->activities,
		    (unsigned long long)plsr->impacts);
		...
		if ((plsr->activities != hubd->h_lsr_activities) ||
		    (plsr->impacts != hubd->h_lsr_impacts)) {
			mutex_exit(HUBD_MUTEX(hubd));
		 ...
		 * Do hubd live resume
		 */
		hubd_cpr_resume(hubd->h_dip);

		/*
		 * Post LSR Resume event to children
		 */
		if (usba_hubdi_post_lsr(dip, &usba_lsr) !=
		    USB_SUCCESS) {
			USB_DPRINTF_L2(DPRINT_MASK_LSR, hubd->h_log_handle,
			    "hubd_cbfunc - failed to post live resume");

			usba_lsr.usba_lsr_cbaction = DDI_CB_LSR_SUSPEND;
			(void) usba_hubdi_post_lsr(dip, &usba_lsr);
			(void) hubd_cpr_suspend(hubd);
	...
	case DDI_CB_LSR_QUERY_CAPABILITY:
		USB_DPRINTF_L4(DPRINT_MASK_LSR, hubd->h_log_handle,
		    "hubd_cbfunc - receives LSR QUERY CAPABILITY");

		/*
		 * Post LSR Query Capability event to children
		 */
		if ((usba_hubdi_post_lsr(dip, &usba_lsr)) !=
		    USB_SUCCESS) {
			USB_DPRINTF_L2(
			    DPRINT_MASK_LSR, hubd->h_log_handle,
			    "hubd_cbfunc - failed to query capability");

			return (USB_FAILURE);
		}

		/*
		 * Return the supported activiteis and impacts
		 */
		plsr = (ddi_cb_lsr_t *)cbarg;
	...
	default:
		USB_DPRINTF_L2(DPRINT_MASK_LSR, hubd->h_log_handle,
		    "hubd_cbfunc - unknown lsr action %x", cbaction);
...
hubd_wait_for_hotplug_exit(hubd_t *hubd)
{
	...
	while (hubd->h_hotplug_thread) {
		...
		if (cv_reltimedwait(&hubd->h_cv_hotplug_dev,
		    &hubd->h_mutex, until, TR_CLOCK_TICK) <= 0) {
			/*
			 * The timeout time was reached, but the condition
			 * was not signaled.
			 */
...
/*
 * Post LSR to descendants
 */
int
usba_hubdi_post_lsr(dev_info_t *dip, void *bus_impldata)
{
	hubd_t *hubd;
	usb_port_t port;
	dev_info_t *cdip;
	...
	cookie = ndi_event_tag_to_cookie(hubd->h_ndi_event_hdl,
	    USBA_EVENT_TAG_LSR);
	plsr = (usba_cb_lsr_t *)bus_impldata;

	/* root hub resumes before its descendants */
	if (usba_is_root_hub(dip) &&
	    (plsr->usba_lsr_cbaction == DDI_CB_LSR_RESUME)) {

		USB_DPRINTF_L4(DPRINT_MASK_LSR, hubdi_log_handle,
		    "root hub live resume");
	...
	/* post lsr event to all descendants */
	mutex_enter(HUBD_MUTEX(hubd));
	for (port = 1; port <= hubd->h_hub_descr.bNbrPorts; port++) {
		if (hubd->h_children_dips[port]) {
			...
			USB_DPRINTF_L4(DPRINT_MASK_LSR, hubdi_log_handle,
			    "usba_hubdi_post_lsr - "
			    "port %d posts lsr", port);

			plsr->usba_lsr_cbret = USB_FAILURE;

			if (ndi_event_do_callback(
			    hubd->h_ndi_event_hdl, cdip, cookie,
			    bus_impldata) != DDI_SUCCESS) {
				USB_DPRINTF_L2(
				    DPRINT_MASK_LSR, hubdi_log_handle,
				    "usba_hubdi_post_lsr - "
				    "port %d's descendants "
				    "%s#%d - event is not defined",
				    port, ddi_driver_name(cdip),
				    ddi_get_instance(cdip));

				return (USB_FAILURE);
			}

			if (plsr->usba_lsr_cbret != USB_SUCCESS) {
				USB_DPRINTF_L2(
				    DPRINT_MASK_LSR, hubdi_log_handle,
				    "usba_hubdi_post_lsr - "
				    "port %d faied to "
	...
	/* root hub suspend after its descendants */
	if (usba_is_root_hub(dip) &&
	    (plsr->usba_lsr_cbaction == DDI_CB_LSR_SUSPEND)) {

		USB_DPRINTF_L4(DPRINT_MASK_LSR, hubdi_log_handle,
		    "root hub live suspend");
		...
		hubd->h_lsr_state = HUBD_LSR_SUSPENDED;
		hubd->h_lsr_activities = plsr->usba_lsr_cbarg.activities;
		hubd->h_lsr_impacts = plsr->usba_lsr_cbarg.impacts;

		mutex_exit(HUBD_MUTEX(hubd));

		if (hubd_cpr_suspend(hubd) != USB_SUCCESS) {
			USB_DPRINTF_L2(DPRINT_MASK_LSR, hubdi_log_handle,
			    "root hub failed to live suspend");

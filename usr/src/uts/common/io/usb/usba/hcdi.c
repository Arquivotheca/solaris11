...
/* fm error report frequency */
uint_t	hcdi_fm_err_freq = 30;
...
void
usba_hcdi_post_ereport(dev_info_t *dip, usb_cr_t completion_reason)
{
	char *err;
	...
	case USB_CR_FLUSHED:
		/* FALLTHROUGH */
		/*
		 * Pipe close/stop polling/reset/flushed errors are expected.
		 * Client drivers are allowed to do such operations on their
		 * pipes when there are still outstanding requests on the pipes
		 */
	default:
		return;
	}
	...
	if (strcmp(ddi_driver_name(dip), "usb_mid") == 0 ||
		...
		usba_device->
		    usb_fm_err_ext_count ++;
		if (usba_device->
		    usb_fm_err_ext_count < hcdi_fm_err_freq) {
	...
	} else if (usba_device->usb_fm_err_count) {
		usba_device->
		    usb_fm_err_count[usba_get_ifno(dip)] ++;
		if (usba_device->
		    usb_fm_err_count[usba_get_ifno(dip)] < hcdi_fm_err_freq) {
...
ereport:
	/*
	 * Determine the dip status to avoid report error
	 * when the topology is not valid.
	 */
	if (DEVI_IS_DEVICE_REMOVED(dip) || !DEVI_EVADD(dip)) {

		return;
	}
	usb_ereport_post(dip, err);
}
    ...
    usba_hcdi_t *hcdi)
{
	...
	if (completion_reason == USB_CR_OK) {
		...
		dev_info_t *cdip = ph_data->p_dip;

		USB_DPRINTF_L4(DPRINT_MASK_HCDI, hcdi->hcdi_log_handle,
		    "exception callback handling: attrs=0x%x", attrs);

		/*
		 * Report errors to fm framework if USB FM is enabled.
		 */
		if (usba_fm_capable) {
			usba_hcdi_post_ereport(cdip, completion_reason);
		}

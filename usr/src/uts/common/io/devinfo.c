...
static uint_t	di_chunk = 0;	/* I/O chunk size in pages */
...
di_ioctl(dev_t dev, int cmd, intptr_t arg, int mode, cred_t *credp, int *rvalp)
{
	...
	struct di_all	*all;
	major_t		i;
	size_t		size, size_no_root_path, map_size;
	struct di_mem	*dcp;
	int		ndi_flags;
	...
	case DINFOLODRV:
		...
		i_ddi_di_cache_invalidate(NULL, NULL);
	 ...
	 */
	size_no_root_path = offsetof(struct di_all, root_path);
	size = size_no_root_path + sizeof (dio->root_path);
	if (size < PAGESIZE)
		size = PAGESIZE;
	off = di_checkmem(st, 0, size);
	all = DI_ALL_PTR(st);
	off += size_no_root_path;

	/* Copyin ioctl arg requested path */
	if (copyinstr(&dio->root_path[0], all->req_path,
	    sizeof (all->req_path), &size) != 0) {
		di_freemem(st);
		(void) di_setstate(st, IOC_IDLE);
		return (EFAULT);
	}
	...
	if (st->command & DINFOPRIVDATA) {
		...
		case DDI_MODEL_NONE:
			if ((off = di_copyformat(off, st,
			    (intptr_t)&dio->priv, mode)) == 0) {
				di_freemem(st);
				(void) di_setstate(st, IOC_IDLE);
				return (EFAULT);
			}
...
di_cache2mem(struct di_cache *cache, struct di_state *st)
{
	...
	bcopy(cache->cache_data, DI_ALL_PTR(st), cache->cache_size);
...
di_copy_aliases(struct di_state *st, alias_pair_t *apair, di_off_t *offp)
{
	...
	off = *offp;
	size = offsetof(struct di_alias, alias);
...
got_root:
	...
	/*
	 * Establish the CNA_DEV of the snapshot as the cna_dev of the root
	 * and release the root.
	 */
	all->cna_dev = ddi_cna_dev(rootnode);
	...
	*off_p = off = di_checkmem(st, *off_p, size);
	...
	for (i = 0; i < devcnt; i++) {
		...
		if ((DINFOPROP & st->command)) {
			...
			if (devnamesp[i].dn_vdr_global_props) {
				dnp[i].vendor_global_prop = off;
				off = di_getprop(DI_PROP_VENDOR_GLB_LIST,
				    &devnamesp[i].dn_vdr_global_props->
				    prop_list, &dnp[i].vendor_global_prop,
				    st, NULL);
			}
			if (devnamesp[i].dn_adm_global_props) {
				dnp[i].admin_global_prop = off;
				off = di_getprop(DI_PROP_ADMIN_GLB_LIST,
				    &devnamesp[i].dn_adm_global_props->
				    prop_list, &dnp[i].admin_global_prop,
				    st, NULL);
			}
		}
...
di_copynode(struct dev_info *node, struct di_stack *dsp, struct di_state *st)
{
	...
	*(TOP_OFFSET(dsp)) = off = di_checkmem(st, *(TOP_OFFSET(dsp)), size);
	...
	me->node_cna_dev = ddi_cna_dev((dev_info_t *)node);
	...
	if (ddi_devid_str_get((dev_info_t *)node)) {
		ddi_devid_t	devid;
		 ...
		 */
		if (ddi_devid_str_decode(ddi_devid_str_get((dev_info_t *)node),
		    &devid, NULL) == DDI_SUCCESS) {
...
property:
	...
	if (node->devi_vdr_prop_ptr) {	/* vendor property list */
		me->vendor_prop = off;
		off = di_getprop(DI_PROP_VENDOR_LIST, &node->devi_vdr_prop_ptr,
		    &me->vendor_prop, st, node);
	}

	if (node->devi_adm_prop_ptr) {	/* admin property list */
		me->admin_prop = off;
		off = di_getprop(DI_PROP_ADMIN_LIST, &node->devi_adm_prop_ptr,
		    &me->admin_prop, st, node);
	}

	/*
	 * Make a copy of any per-device global properties if this
	 * devinfo refers to global properties different from what's
	 * on the devnames array, to support forced driver.conf
	 * update via update_drv(1M).
	 */
	if (node->devi_global_prop_list == NULL) {
		me->glob_prop = (di_off_t)-1;	/* no global property list */
	} else {
	...
	if (node->devi_vdr_global_props == NULL) {
		me->vendor_global_prop = (di_off_t)-1;
	} else {
		ASSERT(me->drv_major != -1);
		if (node->devi_vdr_global_props !=
		    devnamesp[me->drv_major].dn_vdr_global_props) {
			me->vendor_global_prop = off;
			off = di_getprop(DI_PROP_VENDOR_GLB_LIST,
			    &node->devi_vdr_global_props->prop_list,
			    &me->vendor_global_prop, st, node);
		}
	}

	if (node->devi_adm_global_props == NULL) {
		me->admin_global_prop = (di_off_t)-1;
	} else {
		ASSERT(me->drv_major != -1);
		if (node->devi_adm_global_props !=
		    devnamesp[me->drv_major].dn_adm_global_props) {
			me->admin_global_prop = off;
			off = di_getprop(DI_PROP_ADMIN_GLB_LIST,
			    &node->devi_adm_global_props->prop_list,
			    &me->admin_global_prop, st, node);
		}
	}
...
i_lnode_walker(mod_hash_key_t key, mod_hash_val_t *val, void *arg)
{
	 ...
	 * devinfo node.  (could occur when multiple minor nodes
    ...
    struct di_state *st, int get_client)
{
	...
	while (pip = (*next_pip)(dip, pip)) {
		...
		dcmn_err((CE_WARN, "marshaling pip = %p", (void *)pip));
		...
		*off_p = off = di_checkmem(st, off, size);
		...
		me->path_cna_dev = mdi_pi_get_cna_dev(pip);
    ...
    di_off_t off, di_off_t **off_pp)
{
	...
	if (prop_op) {
		...
		if (rv == DDI_PROP_SUCCESS) {
			 ...
			 * len information until after prop_op(9E) to ensure
 ...
 * To prevent invalid format from panicking the system, we call on_fault().
    ...
    struct di_state *st)
{
	...
	*off_p = off;
	...
	do {
		...
		me->hp_state_priv_size = 0;
		me->hp_state_priv = 0;
		if ((ddihp_cn_get_state_priv(hp_hdl, &state_priv)
		    == DDI_SUCCESS) && (state_priv.buf_size <= UINT32_MAX)) {
			me->hp_state_priv_size = (uint32_t)state_priv.buf_size;
		}
		...
		/* Add state private nvlist to the snapshot */
		if (me->hp_state_priv_size) {
			me->hp_state_priv = off
			    = di_checkmem(st, off, me->hp_state_priv_size);
			bcopy(state_priv.nvlist_buf, di_mem_addr(st, off),
			    me->hp_state_priv_size);
			kmem_free(state_priv.nvlist_buf, state_priv.buf_size);
			off += me->hp_state_priv_size;
		}

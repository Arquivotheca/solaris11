...
nge_alloc_bufs(nge_t *ngep)
{
	 ...
	 */
	for (split = 0; split < ngep->tx_desc; ++split) {
...
nge_init_send_ring(nge_t *ngep)
{
	...
	/*
	 * Now initialise tx buffer
	 */
	for (split = 0, ssbdp = srp->sw_sbds; split < ngep->tx_desc;
	    ++ssbdp, ++split) {
		pbuf = &srp->buf[split];
		ssbdp->pbuf = pbuf;
		ssbdp->pbuf->nslots = 1;
		ssbdp->pbuf->size = pbuf->size;
		ssbdp->pbuf->alength = pbuf->alength;
	}
...
nge_m_start(void *arg)
{
	...
	ngep->link_state_timeout = gethrtime() + NGE_PHY_STABLE_TIME;
...
nge_attach(dev_info_t *devinfo, ddi_attach_cmd_t cmd)
{
	...
	/*
	 * param_recv_max_packet is max packet received per interrupt.

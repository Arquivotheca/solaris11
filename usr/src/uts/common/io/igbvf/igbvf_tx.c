...
static boolean_t igbvf_tx(igbvf_tx_ring_t *, mblk_t *);
static int igbvf_tx_copy(igbvf_tx_ring_t *, tx_control_block_t *, mblk_t *,
    uint32_t, boolean_t);
static int igbvf_tx_bind(igbvf_tx_ring_t *, tx_control_block_t *, mblk_t *,
    uint32_t);
static int igbvf_tx_fill_ring(igbvf_tx_ring_t *, link_list_t *, tx_context_t *,
    size_t);
static void igbvf_save_desc(tx_control_block_t *, uint64_t, size_t);
static tx_control_block_t *igbvf_get_free_list(igbvf_tx_ring_t *);
static int igbvf_get_tx_context(mblk_t *, tx_context_t *);
static boolean_t igbvf_check_tx_context(igbvf_tx_ring_t *, tx_context_t *);
static void igbvf_fill_tx_context(struct e1000_adv_tx_context_desc *,
    tx_context_t *, uint32_t);

#ifndef IGBVF_DEBUG
#pragma inline(igbvf_save_desc)
#pragma inline(igbvf_get_tx_context)
#pragma inline(igbvf_check_tx_context)
#pragma inline(igbvf_fill_tx_context)
#endif

mblk_t *
igbvf_tx_ring_send(void *arg, mblk_t *mp)
{
	igbvf_tx_ring_t *tx_ring = (igbvf_tx_ring_t *)arg;
	igbvf_t *igbvf = tx_ring->igbvf;

	if (igbvf->igbvf_state & IGBVF_SUSPENDED_TX_RX)
		return (mp);
	...
	return ((igbvf_tx(tx_ring, mp)) ? NULL : mp);
}

/*
 * igbvf_tx - Main transmit processing
 *
 * Called from igbvf_m_tx with an mblk ready to transmit. this
 ...
 */
static boolean_t
igbvf_tx(igbvf_tx_ring_t *tx_ring, mblk_t *mp)
{
	igbvf_t *igbvf = tx_ring->igbvf;
	...
	copy_thresh = igbvf->tx_copy_thresh;
	...
	if (igbvf->tx_hcksum_enable) {
		 ...
		 * context descriptor.
		 */
		if (igbvf_get_tx_context(mp, ctx) != TX_CXT_SUCCESS) {
			freemsg(mp);
			return (B_TRUE);
		}

		if ((ctx->lso_flag &&
		    (mbsize > (ctx->mac_hdr_len + IGBVF_LSO_MAXLEN))) ||
		    (!ctx->lso_flag &&
		    (mbsize > (igbvf->max_frame_size - ETHERFCSL)))) {
			freemsg(mp);
			IGBVF_DEBUGLOG_0(igbvf, "igbvf_tx: packet oversize");
			return (B_TRUE);
		}
	} else {
		ctx = NULL;
		if (mbsize > (igbvf->max_frame_size - ETHERFCSL)) {
			freemsg(mp);
			IGBVF_DEBUGLOG_0(igbvf, "igbvf_tx: packet oversize");
			return (B_TRUE);
		}
	 ...
	 */
	if (tx_ring->tbd_free < igbvf->tx_recycle_thresh)
	 ...
	 */
	if (tx_ring->tbd_free < igbvf->tx_overload_thresh) {
		tx_ring->reschedule = B_TRUE;
		IGBVF_DEBUG_STAT(tx_ring->stat_overload);
		return (B_FALSE);
	}
	...
	if (ctx && ctx->lso_flag) {
		...
		if ((hdr_current_mp != mp) ||
		    (P2NPHASE((uintptr_t)hdr_current_mp->b_rptr,
		    igbvf->page_size) < hdr_len)) {
			/*
...
adjust_threshold:
	...
	while (current_mp) {
		...
		if (copy_done) {
			 ...
			 */
			tcb = igbvf_get_free_list(tx_ring);

			if (tcb == NULL) {
				IGBVF_DEBUG_STAT(tx_ring->stat_fail_no_tcb);
				goto tx_failure;
			}
		...
		if (current_flag == USE_COPY) {
			...
			desc_num = igbvf_tx_copy(tx_ring, tcb, current_mp,
			...
			desc_num = igbvf_tx_bind(tx_ring, tcb, current_mp,
			    current_len);
		}
	 ...
	 * parallel.
	 *
	 * Do not use up all the tx descriptors. Otherwise tx recycle will
	 * fail and cause false hang.
	 */
	if (tx_ring->tbd_free <= (desc_total + 1)) {
		IGBVF_DEBUG_STAT(tx_ring->stat_fail_no_tbd);
		mutex_exit(&tx_ring->tx_lock);
		goto tx_failure;
	}

	desc_num = igbvf_tx_fill_ring(tx_ring, &pending_list, ctx, mbsize);
 ...
 */
static int
igbvf_tx_copy(igbvf_tx_ring_t *tx_ring, tx_control_block_t *tcb, mblk_t *mp,
    uint32_t len, boolean_t copy_done)
{
	...
	if (copy_done) {
		 ...
		 */
		igbvf_save_desc(tcb, tx_buf->dma_address, tx_buf->len);
		desc_num++;
	}
 ...
 */
static int
igbvf_tx_bind(igbvf_tx_ring_t *tx_ring, tx_control_block_t *tcb, mblk_t *mp,
    uint32_t len)
{
	int status, i;
	...
	if (status != DDI_DMA_MAPPED) {
		IGBVF_DEBUG_STAT(tx_ring->stat_fail_dma_bind);
		return (-1);
	}
...
/*
 * igbvf_get_tx_context
 *
 * Get the tx context information from the mblk
 */
static int
igbvf_get_tx_context(mblk_t *mp, tx_context_t *ctx)
{
	uint32_t start;
	uint32_t flags;
	uint32_t lso_flag;
	...
	if (ctx->lso_flag) {
		if (!((ctx->hcksum_flags & HCK_PARTIALCKSUM) &&
			...
			IGBVF_DEBUGLOG_0(NULL, "igbvf_tx: h/w "
	...
	case ETHERTYPE_IP:
		...
		if (ctx->lso_flag) {
			*((uint16_t *)(uintptr_t)(pos + offsetof(ipha_t,
			...
			/*
			 * To utilize igbvf LSO, here need to fill
	...
	default:
		/* Unrecoverable error */
		IGBVF_DEBUGLOG_0(NULL, "Ethernet type field error with "
		    "tx hcksum flag set");
...
/*
 * igbvf_check_tx_context
 *
 * Check if a new context descriptor is needed
 */
static boolean_t
igbvf_check_tx_context(igbvf_tx_ring_t *tx_ring, tx_context_t *ctx)
{
	tx_context_t *last;
...
/*
 * igbvf_fill_tx_context
 *
 * Fill the context descriptor with hardware checksum informations
 */
static void
igbvf_fill_tx_context(struct e1000_adv_tx_context_desc *ctx_tbd,
    tx_context_t *ctx, uint32_t queue)
{
	/*
	...
	if (ctx->hcksum_flags & HCK_PARTIALCKSUM) {
		...
		default:
			/* Unrecoverable error */
			IGBVF_DEBUGLOG_0(NULL, "L4 type error with tx hcksum");
			break;
		}
	}

	ctx_tbd->seqnum_seed = 0;
	ctx_tbd->mss_l4len_idx = queue << 4;
	if (ctx->lso_flag) {
 ...
 */
static int
igbvf_tx_fill_ring(igbvf_tx_ring_t *tx_ring, link_list_t *pending_list,
    tx_context_t *ctx, size_t mbsize)
{
	...
	if (ctx != NULL) {
		 ...
		 */
		load_context = igbvf_check_tx_context(tx_ring, ctx);
		if (load_context) {
	...
	if (hcksum_flags != 0) {
		...
		first_tbd->read.olinfo_status |= tx_ring->queue << 4;
	}
	...
	IGBVF_DEBUG_STAT(tx_ring->stat_pkt_cnt);
	 ...
	 * the tx recycling.
	 */
	i = igbvf_atomic_reserve(&tx_ring->tbd_free, desc_num);
	ASSERT(i >= 0);
	 ...
	 */
	E1000_WRITE_REG(hw, E1000_TDT(tx_ring->queue), index);
 ...
 */
static void
igbvf_save_desc(tx_control_block_t *tcb, uint64_t address, size_t length)
{
	sw_desc_t *desc;
 ...
 */
uint32_t
igbvf_tx_recycle_legacy(igbvf_tx_ring_t *tx_ring)
{
...
/*
 * igbvf_free_tcb - free up the tx control block
 *
 ...
 */
void
igbvf_free_tcb(tx_control_block_t *tcb)
{
	switch (tcb->tx_type) {
	case USE_COPY:
		/*
...
/*
 * igbvf_get_free_list - Get a free tx control block from the free list
 *
 ...
 * the routine igbvf_put_check_list.
 */
static tx_control_block_t *
igbvf_get_free_list(igbvf_tx_ring_t *tx_ring)
{
	tx_control_block_t *tcb;
	 ...
	 * in the free list.
	 */
	if (igbvf_atomic_reserve(&tx_ring->tcb_free, 1) < 0)
		return (NULL);
 ...
 * between igbvf_get_free_list and igbvf_put_free_list is implemented with
 * the atomic operation on the counter tcb_free.
 */
void
igbvf_put_free_list(igbvf_tx_ring_t *tx_ring, link_list_t *pending_list)
{
	uint32_t index;
	int tcb_num;
	tx_control_block_t *tcb;

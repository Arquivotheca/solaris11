...
#ifndef _CXGE_COMMON_H
#define	_CXGE_COMMON_H

#ifdef __cplusplus
extern "C" {
#endif

#include "cxge_osdep.h"
...
unsigned int t3_mc5_size(const struct mc5 *p);
...
unsigned int t3_mc7_size(const struct mc7 *p);
...
int mdio_read(struct cphy *phy, int mmd, int reg, unsigned int *valp);
int mdio_write(struct cphy *phy, int mmd, int reg, unsigned int val);

/* Convenience initializer */
void cphy_init(struct cphy *phy, p_adapter_t adapter, pinfo_t *pinfo,
...
int uses_xaui(const adapter_t *adap);
int is_10G(const adapter_t *adap);
int is_offload(const adapter_t *adap);
unsigned int core_ticks_per_usec(const adapter_t *adap);
unsigned int dack_ticks_to_usec(const adapter_t *adap, unsigned int ticks);
unsigned int is_pcie(const adapter_t *adap);
...
int t3_wait_op_done(adapter_t *adapter, int reg, u32 mask,
...
void t3_phy_intr_handler(adapter_t *adapter);

void t3_link_changed(adapter_t *adapter, int port_id);
void t3_link_start(struct cphy *phy, struct cmac *mac, struct link_config *lc);

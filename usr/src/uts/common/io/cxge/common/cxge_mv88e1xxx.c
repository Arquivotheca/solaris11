	...
	int *speed, int *duplex, int *fc)
{
	...
	(void) mdio_read(cphy, 0, MV88E1XXX_SPECIFIC_STATUS, &status);
...
static int mv88e1xxx_intr_handler(struct cphy *cphy)
{
	...
	(void) mdio_read(cphy, 0, MV88E1XXX_INTR_STATUS, &cause);
	...
	const struct mdio_ops *mdio_ops)
{
	...
	cphy_init(phy, pinfo->adapter, pinfo, phy_addr, &mv88e1xxx_ops,
	    mdio_ops, SUPPORTED_10baseT_Full | SUPPORTED_100baseT_Full |

 ...
 *
 * Copyright (C) 2004-2010 Chelsio Communications.  All rights reserved.
 *
...
#ifndef _CXGE_T3_CPL_H
#define	_CXGE_T3_CPL_H
...
enum CPL_opcode {
	...
	/* Fake opcode for async notifications */
...
#define	G_CONG_CONTROL_FLAVOR(x) \
	(((x) >> S_CONG_CONTROL_FLAVOR) & M_CONG_CONTROL_FLAVOR)
...
struct tx_pkt_coalesce_wr {
	WR_HDR;
	struct cpl_tx_pkt_coalesce cpl[1];
};
...
#define	G_RTE_WRITE_REQ_LUT_IX(x) \
	(((x) >> S_RTE_WRITE_REQ_LUT_IX) & M_RTE_WRITE_REQ_LUT_IX)
...
#define	G_RTE_WRITE_REQ_LUT_BASE(x) \
	(((x) >> S_RTE_WRITE_REQ_LUT_BASE) & M_RTE_WRITE_REQ_LUT_BASE)

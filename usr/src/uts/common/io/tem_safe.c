...
#define	BITMASK_FIVE	(0x1f)
#define	BITMASK_SIX	(0x3f)

#define	PIX4TO16(pix4)	(uint16_t)(					       \
	((cmap4_to_24.red[pix4] >> 3) & BITMASK_FIVE) << 11 | 		       \
	((cmap4_to_24.green[pix4] >> 2) & BITMASK_SIX) << 5 | 		       \
	((cmap4_to_24.blue[pix4] >> 3) & BITMASK_FIVE))
 ...
 * It is also entered when the kernel is panicking.
 */
void
...
/*
 * Allow the driver to optimize the operation of clearing the screen.
 * This ioctl is not supported in polled i/o mode.
 */
static int
tems_safe_clearscr(
	struct vis_consclear *cls,
	cred_t *credp,
	enum called_from called_from)
{
	if (called_from == CALLED_FROM_STANDALONE)
		...
		return (tems_clearscr_layered(cls, credp));
}
	...
	enum called_from called_from)
{
	...
	case A_VT:
		/*
		 * If output ends exactly at the last column, ignore
		 * a subsequent NL/VT since it would just leave a blank
		 * line on its own on the terminal.
		 */
		if (tem->tvs_linewrap) {
			tem->tvs_linewrap = B_FALSE;
			break;
		}
		tem_safe_send_data(tem, credp, called_from);
    ...
    cred_t *credp, enum called_from called_from)
{
	...
	if (tem->tvs_linewrap && (ch != A_VT && ch != A_LF && ch != A_CR))
		tem->tvs_linewrap = B_FALSE;
...
/*
 * Give a chance to the underlying driver to optimize the clearing of the
 * screen.
 */
static int
tem_safe_pix_clearscr(struct tem_vt_state *tem, cred_t *credp,
    enum called_from called_from)
{
	text_color_t		fg_color, bg_color;
	struct vis_consclear	cls;

	tem_safe_get_color(tem, &fg_color, &bg_color, TEM_ATTR_SCREEN_REVERSE);
	cls.bg_color = bg_color;

	return (tems_safe_clearscr(&cls, credp, called_from));
}
    ...
    enum called_from called_from)
{
	...
	if (tem_safe_pix_clearscr(tem, credp, called_from) != 0) {
		/*
		 * If the underlying driver does not support an optimized way
		 * to clear the screen, or if it failed for any other
		 * reason, revert to the slower approach.
		 */
		width = tems.ts_font.width;
		height = tems.ts_font.height;
    ...
    cred_t *credp, enum called_from called_from)
{
	...
	} else if (tems.ts_pdepth == 15 || tems.ts_pdepth == 16) {
		if (tem->tvs_flags & TEM_ATTR_REVERSE) {
			ca.fg_color.sixteen[0] = TEM_TEXT_WHITE16_LOWER;
			ca.fg_color.sixteen[1] = TEM_TEXT_WHITE16_UPPER;
			ca.bg_color.sixteen[0] = TEM_TEXT_BLACK16_LOWER;
			ca.bg_color.sixteen[1] = TEM_TEXT_BLACK16_UPPER;
		} else {
			ca.fg_color.sixteen[0] = TEM_TEXT_BLACK16_LOWER;
			ca.fg_color.sixteen[1] = TEM_TEXT_BLACK16_UPPER;
			ca.bg_color.sixteen[0] = TEM_TEXT_WHITE16_LOWER;
			ca.bg_color.sixteen[1] = TEM_TEXT_WHITE16_UPPER;
		}
    ...
    text_color_t bg_color)
{
	...
	generic_bit_to_pix4(&tems.ts_font, dest, c, fg_color, bg_color);
}
...
{
	uint8_t fg_color8, bg_color8, *dest = (uint8_t *)tem->tvs_pix_data;

	ASSERT(fg_color4 < 16 && bg_color4 < 16);

	/*
	 * XXX - Is this right? -- Or do we need to check if THIS tem is the
	 * XXX - framebuffer console (the one that needs the mapping)?
	 */
	if (tems.ts_color_map) {
		fg_color8 = tems.ts_color_map(fg_color4);
		bg_color8 = tems.ts_color_map(bg_color4);
		generic_bit_to_pix8(&tems.ts_font, dest, c, fg_color8,
		    bg_color8);
	} else {
		generic_bit_to_pix8(&tems.ts_font, dest, c, fg_color4,
		    bg_color4);
	}
}
...
bit_to_pix16(
	...
	text_color_t bg_color4)
{
	uint16_t fg_color16, bg_color16, *destp;

	ASSERT(fg_color4 < 16 && bg_color4 < 16);

	fg_color16 = PIX4TO16(fg_color4);
	bg_color16 = PIX4TO16(bg_color4);

	destp = (uint16_t *)tem->tvs_pix_data;
	generic_bit_to_pix16(&tems.ts_font, destp, c, fg_color16, bg_color16);
}
...
bit_to_pix24(
	...
	text_color_t bg_color4)
{
	uint32_t fg_color32, bg_color32, *destp;
	...
	destp = (uint32_t *)tem->tvs_pix_data;
	generic_bit_to_pix24(&tems.ts_font, destp, c, fg_color32, bg_color32);
}

 ...
 */
int asy_trig_level = FIFO_TRIG_8;	/* tunable for PCI serial card */
...
static void	asy_parse_generic_mode(dev_info_t *devi, struct asycom *asy);
...
static int	asy_getproperty(dev_info_t *devi, struct asycom *asy,
		    const char *property);
static int	asy_get_generic_property(dev_info_t *devi,
    const char *property);
...
asyattach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	case DDI_RESUME:
	{
		...
		if (async->async_flags & ASYNC_ISOPEN) {
			...
			dosoft = (asy->asy_flags & ASY_DOINGSOFT)? 0:1;
		}
		mutex_exit(&asy->asy_excl_hi);
		mutex_exit(&asy->asy_excl);
		mutex_exit(&asy->asy_soft_sr);
	...
	if (asy->asy_com_port != 0) {
		...
		/* Look for generic properties */

		switch (asy_get_generic_property(devi, "ignore-cd")) {
		case 0:				/* *-ignore-cd=False */
			DEBUGCONT1(ASY_DEBUG_MODEM,
			    "asy%dattach: clear ASY_IGNORE_CD\n", instance);
			asy->asy_flags &= ~ASY_IGNORE_CD; /* wait for cd */
		...
		/* Property for not raising DTR/RTS */
		switch (asy_get_generic_property(devi, "rts-dtr-off")) {
		case 0:				/* *-rts-dtr-off=False */
			asy->asy_flags |= ASY_RTS_DTR_OFF;	/* OFF */
			mcr = asy->asy_mcr;		/* rts/dtr on */
			DEBUGCONT1(ASY_DEBUG_MODEM, "asy%dattach: "
		...
		/* Parse property for tty modes */
		asy_parse_generic_mode(devi, asy);
	}
...
static int
asy_get_generic_property(dev_info_t *devi, const char *property)
{
	int len;
	char val[40];
	char name[40];
	...
	(void) snprintf(name, sizeof (name), "%s%d-%s", ddi_driver_name(devi),
	    ddi_get_instance(devi), property);
	len = sizeof (val);
	if (GET_PROP(devi, name, DDI_PROP_CANSLEEP, val, &len)
	    != DDI_PROP_SUCCESS)
...
out:
	...
	/*
	 * Clear out device state, except persistent device property flags.
	 */
...
check_looplim:
	...
	if ((async->async_flags & ASYNC_SERVICEIMM) || needsoft ||
		...
		asy->asy_flags |= ASY_NEEDSOFT;	/* need a soft interrupt */
}
...
asysoftintr(caddr_t intarg)
{
	...
	mutex_enter(&asy->asy_excl_hi);
	if (asy->asy_flags & ASY_NEEDSOFT || asy->asy_rsrv_needsoft == 1)
		rv = DDI_INTR_CLAIMED;
	else
		rv = DDI_INTR_UNCLAIMED;

	/* If we are already running punt */
	...
	while (1) {
		...
		mutex_enter(&asy->asy_excl_hi);
		if (asy->asy_flags & ASY_NEEDSOFT ||
		    asy->asy_rsrv_needsoft == 1) {
			asy->asy_flags &= ~ASY_NEEDSOFT;
			asy->asy_rsrv_needsoft = 0;
			mutex_exit(&asy->asy_excl_hi);
			mutex_exit(&asy->asy_soft_lock);
			async_softint(asy);
			mutex_enter(&asy->asy_soft_lock);
...
async_softint(struct asycom *asy)
{
	...
	/*
	 * Try to avoid delays in messages moving upstream by bypassing
	 * our read service routine if there is room in the next
	 * queue. Otherwise we have no choice just use our local queue.
	 */
	...
	if (cc) {
		...
		/* finish cc chars */
		mutex_enter(&asy->asy_excl_hi);
		async->async_common->asy_flags |= ASY_NEEDSOFT;
		mutex_exit(&asy->asy_excl_hi);
	}
	mutex_enter(&asy->asy_excl);
	mutex_enter(&asy->asy_excl_hi);
...
asyrsrv(queue_t *q)
{
	...
	mutex_enter(&async->async_common->asy_soft_lock);
	async->async_common->asy_rsrv_needsoft = 1;
	mutex_exit(&async->async_common->asy_soft_lock);

	ddi_trigger_softintr(async->async_common->asy_softintr_id);
...
/*
 * asy_parse_mode_impl(dev_info_t *devi, struct asycom *asy)
 ...
 */
static int
asy_parse_mode_impl(const char *prefix, dev_info_t *devi, struct asycom *asy)
{
	char		name[40];
	char		val[40];
	int		len;
	 ...
	 */
	(void) snprintf(name, sizeof (name), "%s-mode", prefix);
	len = sizeof (val);
	ret = GET_PROP(devi, name, DDI_PROP_CANSLEEP, val, &len);
	if (ret != DDI_PROP_SUCCESS) {
		return (ret);
...
static void
asy_parse_generic_mode(dev_info_t *devi, struct asycom *asy)
{
	char	prefix[40];

	(void) snprintf(prefix, sizeof (prefix), "%s%d",
	    ddi_driver_name(devi), ddi_get_instance(devi));

	(void) asy_parse_mode_impl(prefix, devi, asy);
}
...
asy_parse_mode(dev_info_t *devi, struct asycom *asy)
{
	...
	(void) snprintf(prefix, sizeof (prefix), "tty%c",
	    asy->asy_com_port + 'a' - 1);

	ret = asy_parse_mode_impl(prefix, devi, asy);

	if (ret != DDI_PROP_SUCCESS) {
		(void) snprintf(prefix, sizeof (prefix), "com%c",
		    asy->asy_com_port + '0');

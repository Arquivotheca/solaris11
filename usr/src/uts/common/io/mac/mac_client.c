...
static void mac_tunables_init(mac_client_impl_t *);
...
i_mac_client_impl_ctor(void *buf, void *arg, int kmflag)
{
	...
	bzero(buf, sizeof (mac_client_impl_t));
	...
	mcip->mci_tx_pcpu = kmem_zalloc((mac_tx_percpu_cnt + 1) *
	    sizeof (mac_tx_percpu_t), KM_SLEEP);
...
i_mac_client_impl_dtor(void *buf, void *arg)
{
	...
	kmem_free(mcip->mci_tx_pcpu, (mac_tx_percpu_cnt + 1) *
	    sizeof (mac_tx_percpu_t));
...
mac_client_init(void)
{
	...
	mac_client_impl_cache = kmem_cache_create("mac_client_impl_cache",
	    sizeof (mac_client_impl_t), 64, i_mac_client_impl_ctor,
	...
	mac_promisc_impl_cache = kmem_cache_create("mac_promisc_impl_cache",
	    sizeof (mac_promisc_impl_t), 64, i_mac_promisc_impl_ctor,
...
/*
 * Return the VID associated with a MAC client. We get the VID from
 * the flow entry associated with the MAC client (mci_flent). If
 * the datapath setup is complete, the return value is the VID
 * configured for the MAC client (which could be 0 if this is not a
 * VLAN MAC client). If the datapath setup is not complete, the return
 * will be 0.
 */
uint16_t
...
/*
 * pm_handle_t
 * mac_client_pmh_tx_get() - Return the Pre-Mapped Buffer Cache Handle
 *     for this MAC client.
 * Return(s):
 *	NULL, if pre-mapped handle for the underlying device.
 */
pm_handle_t
mac_client_pmh_tx_get(mac_client_handle_t mch)
{
	mac_client_impl_t 	*mcip = (mac_client_impl_t *)mch;
	mac_impl_t 		*mip = mcip->mci_mip;
...
mac_client_stat_get(mac_client_handle_t mch, uint_t stat)
{
	...
	case MAC_STAT_MULTIRCV:
		val = mcip->mci_stat.ms_misc.mms_multircv;
		break;
	case MAC_STAT_BRDCSTRCV:
		val = mcip->mci_stat.ms_misc.mms_brdcstrcv;
		break;
	case MAC_STAT_MULTIXMT:
		val = mcip->mci_stat.ms_misc.mms_multixmt;
		break;
	case MAC_STAT_BRDCSTXMT:
		val = mcip->mci_stat.ms_misc.mms_brdcstxmt;
		break;
...
/* ARGSUSED */
static void
mac_rx_direct_drop(void *arg0, void *arg1, mblk_t *mp_chain,
 ...
 */
int
mac_vnic_unicast_set(mac_client_handle_t mch, const uint8_t *addr, size_t *len)
{
	mac_client_impl_t *mcip = (mac_client_impl_t *)mch;
	mac_impl_t *mip = mcip->mci_mip;
	mac_address_t *map = mcip->mci_unicast;
...
/*
 * Program the new default unicast address of the specified MAC. The only
 * consumers of this function are the VNIC and aggr modify routines. They
 * reset the default address after successfully returning from
 * mac_unicast_primary_set().
 */
int
mac_unicast_default_set(mac_handle_t mh, const uint8_t *addr)
{
	mac_impl_t *mip = (mac_impl_t *)mh;
	...
	bcopy(addr, mip->mi_defaddr, mip->mi_type->mt_addr_length);
...
/*
 * Return the default primary MAC address of the specified MAC.
 */
void
mac_unicast_default_get(mac_handle_t mh, uint8_t *addr)
{
	mac_impl_t *mip = (mac_impl_t *)mh;

	rw_enter(&mip->mi_rw_lock, RW_READER);
	bcopy(mip->mi_defaddr, addr, mip->mi_type->mt_addr_length);
	rw_exit(&mip->mi_rw_lock);
}
...
mac_client_open(mac_handle_t mh, mac_client_handle_t *mchp, char *name,
    uint16_t flags)
{
	return (mac_client_open_zone(mh, mchp, name, flags,
	    GLOBAL_ZONEID));
}

int
mac_client_open_zone(mac_handle_t mh, mac_client_handle_t *mchp, char *name,
    uint16_t flags, zoneid_t zoneid)
{
	mac_impl_t		*mip = (mac_impl_t *)mh;
	mac_client_impl_t	*mcip;
	int			err = 0;
	...
	*mchp = NULL;
	...
	if (mip->mi_state_flags & MIS_IS_AGGR) {
		mac_capab_aggr_t	capab;

		mcip->mci_state_flags |= MCIS_IS_AGGR;
		VERIFY(i_mac_capab_get((mac_handle_t)mip, MAC_CAPAB_AGGR,
		    &capab));
		mcip->mci_select_ring = capab.mca_select_ring;
		mcip->mci_select_ring_arg = capab.mca_arg;
	}
	...
	mac_bwctl_init(mcip);
	mac_cpu_group_init(mcip);
	mac_tunables_init(mcip);
	...
	bzero(&mcip->mci_stat, sizeof (mcip->mci_stat));
	...
	*mchp = (mac_client_handle_t)mcip;
	 ...
	 * for mac_unicast_add. We will do this only if the underlying
	 * MAC is not already exclusively reserved.
	 */
	if (!(mip->mi_state_flags & MIS_EXCLUSIVE) &&
	    mcip->mci_state_flags & MCIS_NO_UNICAST_ADDR) {
		if ((err = mac_client_datapath_setup(mcip, VLAN_ID_NONE,
		    NULL, NULL, B_TRUE, NULL, 0)) != 0) {
			goto done;
		}
	}
	i_mac_perim_exit(mip);
...
mac_client_close(mac_client_handle_t mch, uint16_t flags)
{
	...
	if (MCIP_DATAPATH_SETUP(mcip)) {
		/*
		 * Any multicast groups added by the client must be removed
	...
	mac_bwctl_fini(mcip);
	mac_cpu_group_fini(mcip);
	mac_client_remove(mcip);

	i_mac_perim_exit(mip);

	mcip->mci_select_ring = NULL;
	mcip->mci_select_ring_arg = NULL;
	mcip->mci_subflow_tab = NULL;
	mcip->mci_state_flags = 0;
	mcip->mci_tx_flag = 0;
	for (i = 0; i <= mac_tx_percpu_cnt; i++) {
		mcip->mci_tx_pcpu[i].pcpu_tx_obytes = 0;
		mcip->mci_tx_pcpu[i].pcpu_tx_opackets = 0;
	}
...
void
mac_rx_resource_add(mac_client_impl_t *mcip, mac_rx_fanout_t *rf)
{
	mcip->mci_resource_add(&rf->rf_sqinfo);
	ASSERT(rf->rf_squeue_obj == NULL);
	rf->rf_squeue_obj = numaio_object_create_thread(rf->rf_sqinfo.mis_tid,
	    "SQUEUE", 0);
}

void
mac_rx_resource_remove(mac_client_impl_t *mcip, mac_rx_fanout_t *rf)
{
	mcip->mci_resource_remove(&rf->rf_sqinfo);
	numaio_object_destroy(rf->rf_squeue_obj);
	rf->rf_squeue_obj = NULL;
}

/*
 * int
 * mac_client_vf_bind() -- binds mac client to the RX and TX
 *	groups associated with a VF.
 *
 * Return(s):
 *	0, succes.
 *	ENXIO, on failure.
 */
int
mac_client_vf_bind(mac_client_handle_t mch, uint16_t vf_index)
{
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;
	mac_impl_t		*mip = mcip->mci_mip;
	mac_group_t		*txgrps, *rxgrps;

	/*
	 * Check to see if the client has been opened exclusive.
	 */
	ASSERT(mip != NULL);

	/*
	 * Lock the MAC client while changing it's state.
	 */
	 ...
	 */
	if (!mac_sriov_ready(mip)) {
		rw_exit(&mcip->mci_rw_lock);
		return (ENXIO);
	}

	/*
	 * Search for RX and TX groups that correspond the named VF
	 * index.
	 */
	if ((txgrps = mac_search_group(mip, vf_index, MAC_GROUP_TX)) == NULL) {
		rw_exit(&mcip->mci_rw_lock);
		return (ENXIO);
	}
	if ((rxgrps = mac_search_group(mip, vf_index, MAC_GROUP_RX)) == NULL) {
		rw_exit(&mcip->mci_rw_lock);
		return (ENXIO);
	}
	 ...
	 */
	if (mac_bind_group(txgrps, mch)) {
		rw_exit(&mcip->mci_rw_lock);
		return (ENXIO);
	}
	if (mac_bind_group(rxgrps, mch)) {
		rw_exit(&mcip->mci_rw_lock);
		return (ENXIO);
	}

	mcip->mci_tx_groups = txgrps;
	mcip->mci_rx_groups = rxgrps;
	mcip->mci_state_flags |= MCIS_IS_VF;
	mcip->mci_vf_index = vf_index;

	/*
	 * Let go of the mac client lock.
	 */
	rw_exit(&mcip->mci_rw_lock);
 ...
 */
void
mac_rx_bypass_set(mac_client_handle_t mch, mac_direct_rx_t rx_fn, void *arg)
{
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;
	mac_impl_t		*mip = mcip->mci_mip;
	mac_rx_fanout_t		*rf;
	 ...
	 */
	mcip->mci_direct_rx_fn = (rx_fn != NULL) ? rx_fn : mac_rx_direct_drop;
	mcip->mci_direct_rx_arg = arg;
	...
	for (i = 0; i < mcip->mci_rx_fanout_cnt; i++) {
		...
		if (rx_fn != NULL) {
			if (mcip->mci_resource_add != NULL &&
				...
				mac_cpu_modify(mcip, MAC_CPU_SQUEUE_ADD, rf);
			}
		} else {
			if (mcip->mci_resource_remove != NULL &&
			    rf->rf_sqinfo.mis_sqp != NULL) {
				mac_cpu_modify(mcip, MAC_CPU_SQUEUE_REMOVE, rf);
	...
	if (rx_fn != NULL)
		mcip->mci_state_flags |= MCIS_RX_BYPASS_OK;
	else
		mcip->mci_state_flags &= ~MCIS_RX_BYPASS_OK;
}
...
mac_rx_set(mac_client_handle_t mch, mac_rx_t rx_fn, void *arg)
{
	 ...
	 * the mac client. We then change the receive function without
	 * interference from any receive data thread and then reenable
...
{
	mac_rx_set(mch, mac_pkt_drop_old, NULL);
}
...
mac_resource_ctl_set(mac_client_handle_t mch, mac_resource_props_t *mrp)
{
	...
	/*
	 * The bandwidth can be updated without setting up the datapath.
	 */
	if ((mrp->mrp_mask & MRP_MAXBW) != 0) {
		if (mrp->mrp_maxbw == MRP_MAXBW_RESETVAL)
			mcip->mci_feature &= ~MAC_BWCTL_LINK;
		else
			mcip->mci_feature |= MAC_BWCTL_LINK;
    ...
    flow_entry_t **flent, mac_resource_props_t *mrp)
{
	...
	flow_desc.fd_zoneid = (mcip->mci_flent != NULL) ?
	    mcip->mci_flent->fe_flow_desc.fd_zoneid : GLOBAL_ZONEID;
	...
	if ((err = mac_flow_create(&flow_desc, mrp, flowname, NULL,
	    flent_flags, flent, flow_desc.fd_zoneid)) != 0)
		return (err);
...
mac_client_datapath_setup(mac_client_impl_t *mcip, uint16_t vid,
    ...
    mac_unicast_impl_t *muip, uint16_t flags)
{
	mac_impl_t	*mip = mcip->mci_mip;
	...
	flow_entry_t	*flent = NULL;
	int		err = 0;
	boolean_t	no_unicast = B_FALSE;

	/*
	 * Add the flow to a client that is attached to a VF?
	 */
	if (MCI_IS_VF(mcip)) {
		/*
		 * Add the unicast flow for mac client that has no
		 * datapath (e.g. VF).
		 */
		mac_group_t *grp = mcip->mci_rx_groups;
		boolean_t usehw = B_FALSE;
		boolean_t group_primary = B_FALSE;
		...
		/*
		 * Is the address and vlan already in use?
		 */
		if (mac_addr_in_use(mip, mac_addr, vid))
			return (EINVAL);

		/*
		 * Is the VF's primary address.
		 */
		if (flags & MAC_UNICAST_VF_PRIMARY)
			group_primary = B_TRUE;

		/*
		 * Program the H/W Classifier with the MAC address and
		 * vlan, if there is a vlan.
		 */
		usehw = ((mcip->mci_state_flags & MCIS_UNICAST_HW) != 0);
		if ((err = mac_add_macaddr(mip, grp, mac_addr,
		    usehw, group_primary)) != 0) {
			return (err);
		}

		/*
		 * Populate the mac unicast handle.
		 */
		muip->mui_map = mac_find_macaddr(mip, mac_addr);
		ASSERT(muip->mui_map != NULL);

		/*
		 * Program a vlan?
		 */
		if (vid != VLAN_ID_NONE) {
			err = mac_add_macvlan(mip, grp, vid, flags);
			if (err != 0)  {
				(void) mac_remove_macaddr(mip, mac_addr);
				return (err);
			}
			muip->mui_vid = vid;
		}
	...
	/*
	 * Client is not a VF client. Hence, setup the datapath.
	 */
	...
	if (!MCIP_DATAPATH_SETUP(mcip)) {
		 ...
		 */
		if ((err = mac_datapath_setup(mcip, flent, flags)) != 0)
			goto bail;
    ...
    mac_unicast_handle_t *mah, uint16_t vid, mac_diag_t *diag)
{
	...
	mac_resource_props_t	*mrp = NULL;
	boolean_t		passive_client = B_FALSE;
	mac_unicast_impl_t	*muip = NULL;
	boolean_t		is_vnic_primary =
	    (flags & MAC_UNICAST_VNIC_PRIMARY);
	uint32_t		mi_state_flags = mip->mi_state_flags;
	uint32_t		mci_flags = mcip->mci_flags;
	uint32_t		mci_state_flags = mcip->mci_state_flags;
	 ...
	 */
	if (!MCI_IS_VF(mcip) && (mip->mi_pvid != 0) &&
	    (vid == mip->mi_pvid))
		return (EBUSY);

	/*
	 * Is the mac client one that does not implement a datapath
	 * (e.g. an SR-IOV VF).
	 */
	if (MCI_IS_VF(mcip)) {
		...
		if ((flags & MAC_UNICAST_HW) != 0)
			mcip->mci_state_flags |= MCIS_UNICAST_HW;

		/*
		 * Create a mac unicast handle
		 */
	...
	/*
	 * is_vnic_primary is true when we come here as a VLAN VNIC which
	 * uses the primary mac client's address but with a non-zero VID.
	 * In this case the MAC address is not specified by an upper
	 * MAC client.
	 */
	 ...
	 * and then use it.
	 */
	if (!(MCI_IS_VF(mcip)) && !MCIP_DATAPATH_SETUP(mcip)) {
		if (is_unicast_hw) {
			/*
	...
	if ((err = mac_client_datapath_setup(mcip, vid, mac_addr, mrp,
	    is_primary || is_vnic_primary, muip, flags)) != 0)
		goto bail_out;
	...
	*mah = (mac_unicast_handle_t)muip;
	...
	if (vid != VLAN_ID_NONE)
		(void) mac_vlan_announce_register((mac_handle_t *)mip, vid);
	return (0);

bail_out:
	/* Revert back to the original state */
	mip->mi_state_flags = mi_state_flags;
	mcip->mci_flags = mci_flags;
	mcip->mci_state_flags = mci_state_flags;
    ...
    flow_entry_t *flent)
{
	...
	/*
	 * Remove a flow for a VF based client.
	 */
	if (MCI_IS_VF(mcip)) {
		mac_group_t *grp = mcip->mci_rx_groups;
		ASSERT(grp != NULL);
		...
		/*
		 * Remove it from the H/W classifier both mac address and
		 * vlan.
		 */
		(void) mac_remove_macaddr(mip, muip->mui_map->ma_addr);
		if (muip->mui_vid != VLAN_ID_NONE)
			(void) mac_remove_macvlan(mip, grp, muip->mui_vid);

		/*
		 * Free the mac unicast address handle.
		 */
		kmem_free(muip, sizeof (mac_unicast_impl_t));
		return;
	}
	...
	if (!no_unicast) {
		 ...
		 * from the active flow tables. But the flow entry itself
	 ...
	 * flow as we have a already cleaned up resources via the teadown
...
mac_unicast_remove(mac_client_handle_t mch, mac_unicast_handle_t mah)
{
	...
	if (muip->mui_vid != VLAN_ID_NONE)
		(void) mac_vlan_announce_deregister((mac_handle_t *)mip,
		    muip->mui_vid);
	...
	if (!mac_client_single_rcvr(mcip) && !MCI_IS_VF(mcip)) {
		/*
		 * This MAC client is shared by more than one unicast
		 * addresses, so we will just remove the flent corresponding
		 * to the address being removed.
		 */
	...
	if (mcip != NULL) {
		...
		if (mac_client_datapath_setup(mcip, VLAN_ID_NONE,
		    mip->mi_addr, mrp, B_TRUE, muip, 0) == 0) {
			if (mcip->mci_rx_p_fn != NULL) {
				mac_rx_set(mch, mcip->mci_rx_p_fn,
				    mcip->mci_rx_p_arg);
				mcip->mci_rx_p_fn = NULL;
...
mac_multicast_add(mac_client_handle_t mch, const uint8_t *addr)
{
	...
	/*
	 * Multicast mac addresses will be programmed by the PF driver at
	 * the direction of the VF driver.
	 */
	if (MCI_IS_VF(mcip))
		return (ENOTSUP);
	...
	if (!(mpip->mpi_no_phys)) {
		if ((rv = i_mac_promisc_set(mip, B_FALSE)) != 0) {
			cmn_err(CE_WARN, "%s: failed to switch OFF promiscuous"
			    " mode because of error 0x%x", mip->mi_linkname,
			    rv);
		}
	}
	mcbi = &mip->mi_promisc_cb_info;
...
mac_resource_set_common(mac_client_handle_t mch, mac_resource_add_t add,
    mac_resource_remove_t remove, void *arg)
{
	mac_client_impl_t *mcip = (mac_client_impl_t *)mch;
...
mac_resource_set(mac_client_handle_t mch, mac_resource_add_t add, void *arg)
{
	...
	mac_resource_set_common(mch, add, NULL, arg);
}

/*
 * Associate CPUs/rxfanout specified by the given property with a MAC client.
 */
int
mac_cpu_rxfanout_set(mac_client_handle_t mch, mac_resource_props_t *mrp)
{
	mac_client_impl_t *mcip = (mac_client_impl_t *)mch;
	mac_impl_t *mip = mcip->mci_mip;
	int err = 0;
...
mac_client_set_resources(mac_client_handle_t mch, mac_resource_props_t *mrp)
{
	...
	if (mrp->mrp_mask & (MRP_MAXBW|MRP_PRIORITY|MRP_USRPRI)) {
		err = mac_resource_ctl_set(mch, mrp);
		if (err != 0)
			goto done;
	}

	if (mrp->mrp_mask & (MRP_CPUS|MRP_POOL|MRP_RXFANOUT)) {
		err = mac_cpu_rxfanout_set(mch, mrp);
		if (err != 0)
			goto done;
	}
...
int
mac_client_set_maxbw(mac_client_handle_t mch, uint64_t maxbw)
{
	int			rv;
	mac_resource_props_t	*mrp;
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;

	ASSERT(!MCI_IS_VF(mcip));
	if (MCI_IS_VF(mcip))
		return (ENOTSUP);

	mrp = kmem_zalloc(sizeof (mac_resource_props_t), KM_SLEEP);
	mrp->mrp_mask = MRP_MAXBW;
	mrp->mrp_maxbw = maxbw;

	rv = mac_client_set_resources(mch, mrp);
	kmem_free(mrp, sizeof (mac_resource_props_t));

	return (rv);
}

int
mac_client_get_maxbw(mac_client_handle_t mch, uint64_t *maxbw)
{
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;
	mac_resource_props_t	*mcip_mrp = MCIP_RESOURCE_PROPS(mcip);
	...
	if (mcip_mrp->mrp_mask & MRP_MAXBW)
		*maxbw = mcip_mrp->mrp_maxbw;
	else
		return (ENOENT);
...
int
mac_client_reset_maxbw(mac_client_handle_t mch)
{
	return (mac_client_set_maxbw(mch, MRP_MAXBW_RESETVAL));
}

int
mac_client_set_priority(mac_client_handle_t mch, int level)
{
	int rv;
	mac_resource_props_t *mrp;
	...
	switch (level) {
	case MAC_CLIENT_PRI_LOW:
		mrp->mrp_priority = MPL_LOW;
		break;
	case MAC_CLIENT_PRI_MEDIUM:
		mrp->mrp_priority = MPL_MEDIUM;
		break;
	case MAC_CLIENT_PRI_HIGH:
		mrp->mrp_priority = MPL_HIGH;
		break;
	default:
		kmem_free(mrp, sizeof (mac_resource_props_t));
		return (EINVAL);
...
int
mac_client_get_priority(mac_client_handle_t mch, int *level)
{
	mac_client_impl_t *mcip = (mac_client_impl_t *)mch;
	mac_resource_props_t *mcip_mrp = MCIP_RESOURCE_PROPS(mcip);
	...
	/* Use default priority, if the priority is not set */
	pri = ((mcip_mrp->mrp_mask & MRP_PRIORITY) == 0) ?
	    MPL_LINK_DEFAULT : mcip_mrp->mrp_priority;

	switch (pri) {
	case MPL_LOW:
		*level = MAC_CLIENT_PRI_LOW;
		break;
	case MPL_MEDIUM:
		*level = MAC_CLIENT_PRI_MEDIUM;
		break;
	case MPL_HIGH:
		*level = MAC_CLIENT_PRI_HIGH;
		break;
	default:
		return (ENOENT);
	}
...
int
mac_client_reset_priority(mac_client_handle_t mch)
{
	int rv;
	mac_resource_props_t *mrp;
...
int
mac_client_set_usrpri(mac_client_handle_t mch, uint8_t priority)
{
	int rv;
	mac_resource_props_t *mrp;

	mrp = kmem_zalloc(sizeof (mac_resource_props_t), KM_SLEEP);
	mrp->mrp_mask = MRP_USRPRI;
	mrp->mrp_usrpri = priority;
...
void
mac_rx_fanout_set_effective(mac_client_impl_t *mcip)
{
	mac_resource_props_t *mrp = MCIP_EFFECTIVE_PROPS(mcip);

	mrp->mrp_rxfanout = mcip->mci_rx_fanout_cnt;
}
    ...
    mac_resource_props_t *mrp)
{
	...
	if (mcip->mci_numa_grp_cnt != 0)
		mac_cpu_set_effective(mcip);

	if (mcip->mci_rx_fanout_cnt != 0)
		mac_rx_fanout_set_effective(mcip);


	mcip_mrp = MCIP_EFFECTIVE_PROPS(mcip);
	bcopy(mcip_mrp, mrp, sizeof (mac_resource_props_t));
}
...
i_mac_capab_get(mac_handle_t mh, mac_capab_t cap, void *cap_data)
{
	...
	if ((mip->mi_bridge_link != NULL) && (cap != MAC_CAPAB_BUFFER_MGMT))
...
mac_capab_get(mac_handle_t mh, mac_capab_t cap, void *cap_data)
{
	...
	if (mip->mi_nactiveclients > 1) {
		...
		case MAC_CAPAB_LSO:
			/* Query the driver for these capabilities */
			break;
		default:
			return (B_FALSE);
		}
...
mac_sap_verify(mac_handle_t mh, uint32_t sap, uint32_t *bind_sap)
{
	...
	retval = mip->mi_type->mt_ops.mtops_sap_verify(sap, bind_sap,
	    mip->mi_pdata);
    ...
    size_t extra_len)
{
	...
	retval = mip->mi_type->mt_ops.mtops_header(mip->mi_addr, hdr_daddr, sap,
...
mac_header_info(mac_handle_t mh, mblk_t *mp, mac_header_info_t *mhip)
{
	...
	retval =  mip->mi_type->mt_ops.mtops_header_info(mp, mip->mi_pdata,
	    mhip);
...
mac_header_cook(mac_handle_t mh, mblk_t *mp)
{
	...
	if (mip->mi_type->mt_ops.mtops_ops & MTOPS_HEADER_COOK) {
		...
		mutex_enter(&mip->mi_pdata_lock);
		retval = mip->mi_type->mt_ops.mtops_header_cook(mp,
		    mip->mi_pdata);
		mutex_exit(&mip->mi_pdata_lock);
		return (retval);
	}
...
mac_header_uncook(mac_handle_t mh, mblk_t *mp)
{
	...
	if (mip->mi_type->mt_ops.mtops_ops & MTOPS_HEADER_UNCOOK) {
		...
		mutex_enter(&mip->mi_pdata_lock);
		retval = mip->mi_type->mt_ops.mtops_header_uncook(mp,
		    mip->mi_pdata);
		mutex_exit(&mip->mi_pdata_lock);
		return (retval);
	}
    ...
    boolean_t is_user_flow)
{
	if (nmrp != NULL && cmrp != NULL) {
		...
		if (nmrp->mrp_mask & MRP_RXFANOUT) {
			if (nmrp->mrp_rxfanout == MRP_RXFANOUT_RESETVAL) {
				cmrp->mrp_mask &= ~MRP_RXFANOUT;
				cmrp->mrp_rxfanout = 0;
			} else {
				cmrp->mrp_mask |= MRP_RXFANOUT;
				cmrp->mrp_rxfanout = nmrp->mrp_rxfanout;
			}
		}
		...
		if (nmrp->mrp_mask & MRP_RX_RINGS) {
			if (nmrp->mrp_mask & MRP_PROP_RESET) {
		...
		if (nmrp->mrp_mask & MRP_USRPRI) {
			if (nmrp->mrp_mask & MRP_PROP_RESET) {
				cmrp->mrp_mask &= ~MRP_USRPRI;
				cmrp->mrp_usrpri = 0;
			} else {
				cmrp->mrp_mask |= MRP_USRPRI;
				cmrp->mrp_usrpri = nmrp->mrp_usrpri;
			}
		}
	}
}
...
i_mac_set_resources(mac_handle_t mh, mac_resource_props_t *mrp)
{
	...
	if (mcip != NULL && (mcip->mci_state_flags & MCIS_IS_AGGR_PORT) == 0) {
		...
		if (vmcip != NULL) {
			...
			i_mac_setup_enter(vmcip->mci_mip);
			...
			i_mac_setup_exit(vmcip->mci_mip);
			kmem_free(omrp, sizeof (*omrp));
		}
	}
...
int
mac_set_vlan_announce(mac_handle_t mh, mac_vlan_announce_t val)
{
	int err = 0;
	mac_impl_t *mip = (mac_impl_t *)mh;
	...
	if (val == mip->mi_vlan_announce)
		goto done;

	switch (val) {
	case MAC_VLAN_ANNOUNCE_OFF:
		mac_gvrp_disable(mh);
		break;
	case MAC_VLAN_ANNOUNCE_GVRP:
		mac_gvrp_enable(mh);
		break;
	default:
		err = EINVAL;
...
mac_vlan_announce_t
mac_get_vlan_announce(mac_handle_t mh)
{
	mac_impl_t *mip = (mac_impl_t *)mh;

	return (mip->mi_vlan_announce);
}
...
mac_rename_primary(mac_handle_t mh, const char *new_name)
{
	...
	for (cur_clnt = mip->mi_clients_list; cur_clnt != NULL;
		...
		if (cur_clnt->mci_state_flags & MCIS_IS_AGGR_PORT) {
			...
			mac_link_stat_rename(fep->fe_mcip);
...
{
	mac_address_t *map = muip->mui_map;
	flow_entry_t *flent;
	uint16_t vid;
	flow_desc_t flow_desc;

	ASSERT(map->ma_addr != NULL);
	ASSERT(MAC_PERIM_HELD((mac_handle_t)mcip->mci_mip));

	for (flent = mcip->mci_flent_list; flent != NULL;
	    flent = flent->fe_client_next) {
		/*
		 * Compare the mac address first.
		 */
		mac_flow_get_desc(flent, &flow_desc);
...
/*
 * we replace the flow_desc_t in mci_flent with that of an existing
 * flent and then remove that flent instead of mci_flent.
 */
static flow_entry_t *
...
mac_validate_props(mac_impl_t *mip, mac_resource_props_t *mrp)
{
	...
	if (mrp->mrp_mask & MRP_RXFANOUT) {
		if (mrp->mrp_rxfanout != MRP_RXFANOUT_RESETVAL &&
		    mrp->mrp_rxfanout > ncpus)
			return (EINVAL);
	}
	...
	/*
	 * For the mip == NULL we should have validated it before
	 * getting here - similar to the "rings" cases below.
	 */
	if (mrp->mrp_mask & MRP_USRPRI && mip != NULL) {
		if (mip->mi_dcb_flags == 0)
			return (ENOTSUP);
		if (mrp->mrp_usrpri >= mip->mi_ntcs)
			return (EINVAL);
	}
	...
	reset = mrp->mrp_mask & MRP_PROP_RESET;
	/*
    ...
    mac_resource_props_t *mrp)
{
	...
	if (mip != NULL && mcip->mci_mip->mi_dcb_flags != 0)
		mip->mi_dcb_flags = mcip->mci_mip->mi_dcb_flags;
...
/*
 * int
 * mac_client_set_mtu() -- Set the mtu for the mac client.
 *
 * Return(s)):
 *	0 on success.
 */
int
mac_client_set_mtu(mac_client_handle_t mch, uint_t new_mtu, uint_t *old_mtu)
{
	mac_client_impl_t *mcip = (mac_client_impl_t *)mch;
	int err = 0;

	ASSERT(mcip != NULL);
	ASSERT(mcip->mci_mip != NULL);
	ASSERT(MCI_IS_VF(mcip));
	ASSERT(mcip->mci_tx_groups != NULL);
	ASSERT(mcip->mci_rx_groups != NULL);
	ASSERT(new_mtu != 0);
	ASSERT(old_mtu != NULL);

	/*
	 * Set the mtu for the RX group.
	 */
	i_mac_perim_enter(mcip->mci_mip);
	err = mac_group_set_mtu(mcip->mci_rx_groups, new_mtu, old_mtu);
	...
	/*
	 * Set the mtu for the TX group.
	 */
	err = mac_group_set_mtu(mcip->mci_tx_groups, new_mtu, old_mtu);
	if ((err != 0) && (err != ENOTSUP)) {
		i_mac_perim_exit(mcip->mci_mip);
		return (err);
	}
...
/*
 * Return the RX h/w group indexes.
 */
void
mac_get_hwrxgrp_ids(mac_handle_t mh, int ngrps, uint_t *grpids)
{
	mac_impl_t	*mi = (mac_impl_t *)mh;
	int		i;
	mac_group_t	*grp;

	ASSERT(mi != NULL);

	for (i = 0, grp = mi->mi_rx_groups; i < ngrps; i++) {
		ASSERT(grp != NULL);
		grpids[i]  = grp->mrg_index;
		grp = grp->mrg_next;
	}
}
    ...
    char *clnts_name)
{
	...
	grp = mac_group_find(mip, MAC_RING_TYPE_RX, grp_index);
	ASSERT(grp != NULL);
	*grp_num = grp->mrg_index;
	*type = grp->mrg_type;
	*n_rings = grp->mrg_cur_count;
	ring = grp->mrg_rings;
	for (index = 0; index < grp->mrg_cur_count; index++) {
	...
	index = 0;
	if (grp == mip->mi_rx_defaultgrp) {
		(void) strlcpy(clnts_name, "<default,mcast>,",
		    MAXCLIENTNAMELEN);
		index += strlen("<default,mcast>,");
	}

	for (mcip = grp->mrg_clients; mcip != NULL; mcip = mcip->mgc_next) {
...
/*
 * Return the TX h/w group indexes.
 */
void
mac_get_hwtxgrp_ids(mac_handle_t mh, int ngrps, uint_t *grpids)
{
	mac_impl_t	*mi = (mac_impl_t *)mh;
	int		i;
	mac_group_t	*grp;
	...
	for (i = 0, grp = mi->mi_tx_groups; i < ngrps; i++) {
		ASSERT(grp != NULL);
		grpids[i]  = grp->mrg_index;
		grp = grp->mrg_next;
	}
    ...
    char *clnts_name)
{
	...
	grp = mac_group_find(mip, MAC_RING_TYPE_TX, grp_index);
	ASSERT(grp != NULL);
	...
	*n_rings = grp->mrg_cur_count;
	...
	if (mip->mi_tx_defaultgrp == grp) {
		(void) strlcpy(clnts_name, "<default>,", MAXCLIENTNAMELEN);
...
mac_hwgrp_num(mac_handle_t mh, int type)
{
	...
	return (type == MAC_RING_TYPE_RX ? mip->mi_rx_group_count :
	    mip->mi_tx_groups != NULL ? mip->mi_tx_group_count : 0);
}
...
boolean_t
mac_capab_fcoe_get(mac_client_handle_t mch, uint16_t *fcoe_flags,
    uint32_t *fcoe_max_lso_size, uint32_t *fcoe_max_lro_size,
    uint16_t *fcoe_min_lro_xchgid, uint16_t *fcoe_max_lro_xchgid)
{
	boolean_t		ret;
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;
	mac_impl_t		*mip = mcip->mci_mip;
	mac_capab_fcoe_t	*cap_data = &mip->mi_fcoe_cap;

	ret = i_mac_capab_get((mac_handle_t)mip, MAC_CAPAB_FCOE, cap_data);
	if (!ret)
		return (ret);

	if (fcoe_flags != NULL) {
		*fcoe_flags = cap_data->mac_capab_fcoe_flags;
		if (*fcoe_flags & MAC_FCOE_FLAG_LRO) {
			ASSERT(cap_data->mac_fcoe_setup_lro != NULL);
			ASSERT(cap_data->mac_fcoe_cancel_lro != NULL);
		}
	}
	if (fcoe_max_lso_size != NULL)
		*fcoe_max_lso_size = cap_data->mac_fcoe_max_lso_size;
	if (fcoe_max_lro_size != NULL)
		*fcoe_max_lro_size = cap_data->mac_fcoe_max_lro_size;
	if (fcoe_min_lro_xchgid != NULL)
		*fcoe_min_lro_xchgid = cap_data->mac_fcoe_min_lro_xchgid;
	if (fcoe_max_lro_xchgid != NULL)
		*fcoe_max_lro_xchgid = cap_data->mac_fcoe_max_lro_xchgid;
	return (ret);
}

int
mac_client_fcoe_setup_lro(mac_client_handle_t mch,
    mac_fcoe_lro_params_t *mflp)
{
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;
	mac_impl_t		*mip = mcip->mci_mip;

	return (mip->mi_fcoe_cap.mac_fcoe_setup_lro(mip->mi_driver, mflp));
}

void
mac_client_fcoe_cancel_lro(mac_client_handle_t mch,
    mac_fcoe_lro_params_t *mflp)
{
	mac_client_impl_t	*mcip = (mac_client_impl_t *)mch;
	mac_impl_t		*mip = mcip->mci_mip;

	mip->mi_fcoe_cap.mac_fcoe_cancel_lro(mip->mi_driver, mflp);
}

/*
 * Allocate mblk for an FCoE frame, It in turn calls allocb()
 * with the size of mac_fcoe_tx_params_t plus the real frame size.
 */
mblk_t *
mac_fcoe_allocb(size_t fcoe_frame_size)
{
	mblk_t	*mp;

	mp = allocb(fcoe_frame_size + sizeof (mac_fcoe_tx_params_t), NULL);
	if (mp != NULL) {
		mp->b_rptr += sizeof (mac_fcoe_tx_params_t);
		mp->b_wptr += sizeof (mac_fcoe_tx_params_t);
	}
	return (mp);
}

/*
 * Used for FCoE software to associate offload flags/parameters with the
 * given FCoE frame.
 */
void
mac_fcoe_set(mblk_t *mp, mac_fcoe_tx_params_t *mftp)
{
	ASSERT(DB_TYPE(mp) == M_DATA);

	bcopy(mftp, mp->b_datap->db_base, sizeof (mac_fcoe_tx_params_t));
}

/*
 * MAC client tunables initialization
 */
static void
mac_tunables_init(mac_client_impl_t *mcip)
{
	uint_t		media = mcip->mci_mip->mi_info.mi_nativemedia;
	mac_tunables_t	*t = &mcip->mci_tunables;

	if (media == DL_IB) {
		t->mt_rx_fanout_inline_max = mac_ib_rx_fanout_inline_max;
	} else {
		t->mt_rx_fanout_inline_max = mac_rx_fanout_inline_max;
	}
}

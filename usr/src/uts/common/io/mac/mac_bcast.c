 ...
 * ring if it was received from the wire.
 */
/* ARGSUSED */
void
mac_bcast_send(void *arg1, void *arg2, mblk_t *mp_chain, uint_t flags)
{
	mac_bcast_grp_t		*grp = arg1;
	mac_client_impl_t	*src_mcip = NULL, *dst_mcip;
	...
	for (i = 0; i < grp->mbg_nclients_alloc; i++) {
		 ...
		 */
		if ((src_mcip != NULL) &&
		    (mp_chain1 = mac_fix_cksum(mp_chain1, B_FALSE, 0)) == NULL)
			break;
		...
		if (grp->mbg_addrtype == MAC_ADDRTYPE_MULTICAST)
			addrtype = MAC_RX_MULTICAST;
		else if (grp->mbg_addrtype == MAC_ADDRTYPE_BROADCAST)
			addrtype = MAC_RX_BROADCAST;

		dst_mcip->mci_flent->fe_cb_fn(dst_mcip->mci_flent->fe_cb_arg1,
		    ring, mp_chain1, flags | addrtype);
		FLOW_REFRELE(flent);
	...
	if (src_mcip != NULL) {
		...
		/* update stats */
		if (grp->mbg_addrtype == MAC_ADDRTYPE_MULTICAST) {
			MCIP_MISC_STAT(src_mcip, multixmtbytes) +=
			    msgdsize(mp_chain);
			MCIP_MISC_STAT(src_mcip, multixmt) += 1;
		} else {
			MCIP_MISC_STAT(src_mcip, brdcstxmtbytes) +=
			    msgdsize(mp_chain);
			MCIP_MISC_STAT(src_mcip, brdcstxmt) += 1;
		}
    ...
    mac_addrtype_t addrtype)
{
	...
	if (grp == NULL) {
		...
		/*
		 * Create the flow for this group in the GZ because
		 * MAC clients of this group can be in multiple
		 * zones. (Example: autovnics can be created for
		 * multiple zones over a single underlying physical link).
		 * Don't create this flow in a NGZ otherwise it will be
		 * destroyed when the zone halts.
		 */
		flow_desc.fd_zoneid = GLOBAL_ZONEID;
		rc = mac_flow_create(&flow_desc, NULL, flow_name,
		    grp, FLOW_MCAST, &grp->mbg_flow_ent, flow_desc.fd_zoneid);
		if (rc != 0) {
		 ...
		 * by the underlying NIC, mac_rx_classify's flow lookup
		 * will return mbg_flow_ent. mac_bcast_send is then
		 * invoked to send a copy of the packet(s) to every MAC
		 * client opened on top of the underlying MAC.
		 *

 ...
 */
ulong_t	dhcp_max_pending_txn = 512;
ulong_t	dhcp_max_completed_txn = 512;
time_t	txn_cleanup_interval = 60;
...
#define	BUMP_STAT(m, s)	(m)->mci_stat.ms_misc.mms_##s++
...
mac_protect_set(mac_client_handle_t mch, mac_resource_props_t *mrp)
{
	...
	if (mrp->mrp_protect.mp_types == MPT_RESET)
		mcip->mci_feature &= ~MAC_PROTECT;
	else
		mcip->mci_feature |= MAC_PROTECT;

	mac_update_resources(mrp, MCIP_RESOURCE_PROPS(mcip), B_FALSE);
	notify_mip = ((mcip->mci_state_flags & MCIS_IS_VNIC) != 0 ?
	    mcip->mci_upper_mip : mip);
	if (notify_mip != NULL)
		i_mac_notify(notify_mip, MAC_NOTE_ALLOWED_IPS);
	return (0);
}

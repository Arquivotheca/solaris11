 ...
 *
 * int ptms_minor_valid(zoneid_t zoneid, minor_t minor,
 *    uid_t *ruid, gid_t *rgid)
 *
 *	Check if minor refers to an allocated pty in the given zone.
 *	Returns
 *		 0 if not allocated or not for this zone.
 *		 1 if an allocated pty in the given zone.
 ...
 *
 * void ptms_set_owner(zoneid_t zoneid, minor_t minor, uid_t ruid, gid_t rgid)
 *
 ...
 */
void
ptms_set_owner(zoneid_t zoneid, minor_t dminor, uid_t ruid, gid_t rgid)
{
	struct pt_ttys *pt;
	...
	if (pt != NULL && pt->pt_zoneid == zoneid) {
		pt->pt_ruid = ruid;
		pt->pt_rgid = rgid;
	}
	mutex_exit(&ptms_lock);
 ...
 * returns:
 *	1 if the pty is allocated to the given zone.
 *	0 otherwise
 *
 * If the pty is allocated to the given zone, it also returns the owner.
 */
int
ptms_minor_valid(zoneid_t zoneid, minor_t dminor, uid_t *ruid, gid_t *rgid)
{
	struct pt_ttys *pt;
	int ret;

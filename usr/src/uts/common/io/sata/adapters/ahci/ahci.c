...
/*
 * The below global variables are tunable via ahci.conf or /etc/system
 *
...
ahci_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	ahci_msi_enabled = ddi_getprop(DDI_DEV_T_ANY, dip, 0,
	    "ahci-msi-enabled", ahci_msi_enabled);
...
intr_done:
	...
	/* Check the size of the PRDT table */
	ahci_dma_prdt_number = ddi_getprop(DDI_DEV_T_ANY, dip, 0,
	    "ahci-dma-prdt-number", AHCI_PRDT_NUMBER);
	 ...
	 */
	ahci_buf_64bit_dma = ddi_getprop(DDI_DEV_T_ANY, dip, 0,
	    "ahci-buf-64bit-dma", ahci_buf_64bit_dma);
	sb600_buf_64bit_dma_disable = ddi_getprop(DDI_DEV_T_ANY, dip, 0,
	    "sb600-buf-64bit-dma-disable", sb600_buf_64bit_dma_disable);
	 ...
	 * and SP5100 if sbxxx_commu_64bit_dma_disable is B_FALSE
	 */
	ahci_commu_64bit_dma = ddi_getprop(DDI_DEV_T_ANY, dip, 0,
	    "ahci-commu-64bit-dma", ahci_commu_64bit_dma);
	sbxxx_commu_64bit_dma_disable = ddi_getprop(DDI_DEV_T_ANY, dip, 0,
	    "sbxxx-commu-64bit-dma-disable", sbxxx_commu_64bit_dma_disable);
	if ((ahci_commu_64bit_dma == B_FALSE) ||
	    ...
	    ahci_ctlp->ahcictl_devid == 0x4394)))) {
    ...
    ahci_port_t *ahci_portp, ahci_addr_t *addrp)
{
	...
	if (ahci_restart_port_wait_till_ready(ahci_ctlp,
	    ahci_portp, port, AHCI_PORT_RESET,
	    NULL) != AHCI_SUCCESS) {
...
ahci_config_space_init(ahci_ctl_t *ahci_ctlp)
{
	...
	/*
	 * AMD/ATI SB700/710/750/800 AHCI chipset share the same
	...
	/*
	 * SP5100 (0x1002, 0x4394) doesn't support 64-bit DMA addressing for
	 * communication memory descriptors though S64A bit of CAP register
	...
	if (ahci_ctlp->ahcictl_venid == 0x1002 &&
		...
		AHCIDBG(AHCIDBG_INIT, ahci_ctlp,
		    "ATI SP5100 cannot do 64-bit DMA for communication, "
		    "forcing use of 32-bit DMA", NULL);
	}
    ...
    ahci_addr_t *addrp)
{
	...
	do {
		...
		/* We are effectively timing out after 10 seconds. */

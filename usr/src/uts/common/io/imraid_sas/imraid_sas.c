...
/*
 * imraid_sas.c: source for imraid_sas driver
 *
 ...
 * Local static data
 */
static void	*imraidsas_state = NULL;
static volatile boolean_t	imraidsas_relaxed_ordering = B_TRUE;
...
static dma_obj_t	drv_ver_dma_obj;
...
static ddi_dma_attr_t imraidsas_generic_dma_attr = {
...
int32_t imraidsas_max_cap_maxxfer = 0x1000000;

/*
 * cb_ops contains base level routines
 */
static struct cb_ops imraidsas_cb_ops = {
	imraidsas_open,		/* open */
	imraidsas_close,	/* close */
	nodev,			/* strategy */
	nodev,			/* print */
	nodev,			/* dump */
	nodev,			/* read */
	nodev,			/* write */
	imraidsas_ioctl,	/* ioctl */
	nodev,			/* devmap */
	nodev,			/* mmap */
	nodev,			/* segmap */
	nochpoll,		/* poll */
 ...
 */
static struct dev_ops imraidsas_ops = {
	DEVO_REV,		/* rev, */
	0,			/* refcnt */
	imraidsas_getinfo,	/* getinfo */
	nulldev,		/* identify */
	nulldev,		/* probe */
	imraidsas_attach,	/* attach */
	imraidsas_detach,	/* detach */
	nodev,
	&imraidsas_cb_ops,	/* char/block ops */
	NULL,			/* bus ops */
	NULL,			/* power */
	imraidsas_quiesce	/* quiesce */
...
_init(void)
{
	...
	ret = ddi_soft_state_init(&imraidsas_state,
	    sizeof (struct imraidsas_instance), 0);

	if (ret != DDI_SUCCESS) {
		con_log(CL_ANN, (CE_WARN, "imraid_sas: could not init state"));
		return (ret);
	}

	if ((ret = scsi_hba_init(&modlinkage)) != DDI_SUCCESS) {
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas:could not init scsi hba"));
		ddi_soft_state_fini(&imraidsas_state);
		return (ret);
	}
	...
	if (ret != DDI_SUCCESS) {
		con_log(CL_ANN, (CE_WARN, "imraid_sas: mod_install failed"));
...
static int
imraidsas_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	int		instance_no;
	int		nregs;
	...
	struct imraidsas_instance	*instance;
	...
	if (ddi_slaveonly(dip) == DDI_SUCCESS) {
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas%d: Device in slave-only slot, unused",
		    instance_no));
		return (DDI_FAILURE);
	}
	...
	switch (cmd) {
		case DDI_ATTACH:
			con_log(CL_DLEVEL1, (CE_NOTE,
			    "imraid_sas: DDI_ATTACH"));
			/* allocate the soft state for the instance */
			if (ddi_soft_state_zalloc(imraidsas_state, instance_no)
			    != DDI_SUCCESS) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas%d: Failed to"
				    " allocate soft state",
				    instance_no));
			...
			instance = (struct imraidsas_instance *)
			    ddi_get_soft_state(imraidsas_state, instance_no);

			if (instance == NULL) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas%d: Bad soft state",
				    instance_no));

				ddi_soft_state_free(imraidsas_state,
				    instance_no);
			...
			bzero((caddr_t)instance,
			    sizeof (struct imraidsas_instance));

			instance->func_ptr = kmem_zalloc(
			    sizeof (struct imraidsas_func_ptr), KM_SLEEP);
			...
			if (pci_config_setup(dip, &instance->pci_handle) !=
			    DDI_SUCCESS) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas%d: pci config setup failed ",
				    instance_no));

				kmem_free(instance->func_ptr,
				    sizeof (struct imraidsas_func_ptr));
				ddi_soft_state_free(imraidsas_state,
				    instance_no);
			...
			if (ddi_dev_nregs(dip, &nregs) != DDI_SUCCESS) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas: failed to get registers."));
			...
			con_log(CL_DLEVEL1, (CE_CONT, "imraid_sas%d: "
			...
			if (!(command & PCI_COMM_ME)) {
				...
				con_log(CL_ANN, (CE_CONT, "imraid_sas%d: "
			...
			if ((device_id == PCI_DEVICE_ID_LSI_DRAKE_4PORT) ||
			    (device_id == PCI_DEVICE_ID_LSI_DRAKE_8PORT)) {
				con_log(CL_DLEVEL1, (CE_CONT, "imraid_sas[%d]: "
				...
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas: Invalid device detected"));
			...
			if (reglength > DEFAULT_MFI_MEM_SZ) {
				...
				con_log(CL_DLEVEL1, (CE_NOTE,
				    "imraid_sas: register length to map is "
			...
			if (ddi_prop_lookup_string(DDI_DEV_T_ANY, dip, 0,
			    "imraidsas-enable-msi", &data) == DDI_SUCCESS) {
			...
			if (msi_enable && (intr_types & DDI_INTR_TYPE_MSIX)) {
				if (imraidsas_add_intrs(instance,
			...
			if (init_mfi(instance) != DDI_SUCCESS) {
				con_log(CL_ANN, (CE_WARN, "imraid_sas: "
			...
			mutex_init(&instance->issue_cmd_mtx, "issue_cmd_mtx",
			    MUTEX_DRIVER, DDI_INTR_PRI(instance->intr_pri));
			...
			if (instance->isr_level == HIGH_LEVEL_INTR) {
				if (ddi_add_softintr(dip, DDI_SOFTINT_HIGH,
				    ...
				    imraidsas_softintr, (caddr_t)instance) !=
				    DDI_SUCCESS) {
					con_log(CL_ANN, (CE_WARN,
			...
			tran->tran_tgt_init	= imraidsas_tran_tgt_init;
			tran->tran_tgt_probe	= scsi_hba_probe;
			tran->tran_tgt_free	= imraidsas_tran_tgt_free;
			tran->tran_init_pkt	= imraidsas_tran_init_pkt;
			tran->tran_start	= imraidsas_tran_start;
			tran->tran_abort	= imraidsas_tran_abort;
			tran->tran_reset	= imraidsas_tran_reset;
			tran->tran_bus_reset	= imraidsas_tran_bus_reset;
			tran->tran_getcap	= imraidsas_tran_getcap;
			tran->tran_setcap	= imraidsas_tran_setcap;
			tran->tran_destroy_pkt	= imraidsas_tran_destroy_pkt;
			tran->tran_dmafree	= imraidsas_tran_dmafree;
			tran->tran_sync_pkt	= imraidsas_tran_sync_pkt;
			tran->tran_bus_config	= imraidsas_tran_bus_config;

			if (imraidsas_relaxed_ordering)
				imraidsas_generic_dma_attr.dma_attr_flags |=
				    DDI_DMA_RELAXED_ORDERING;


			tran_dma_attr = imraidsas_generic_dma_attr;
			...
			if (ddi_create_minor_node(dip, "scsi", S_IFCHR,
			    ...
			    DDI_FAILURE) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas: failed to create scsi node."));
			...
			if ((instance->taskq = ddi_taskq_create(dip,
			    "imraidsas_dr_taskq", 1,
			    TASKQ_DEFAULTPRI, 0)) == NULL) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas: failed to create taskq "));
				instance->taskq = NULL;
				goto fail_attach;
			}
			...
			if (start_mfi_aen(instance)) {
				con_log(CL_ANN, (CE_WARN,
				    "imraid_sas: failed to initiate AEN."));
			...
			if (imraidsas_check_acc_handle(
			    instance->regmap_handle) !=
			    DDI_SUCCESS) {
				goto fail_attach;
			}
			if (imraidsas_check_acc_handle(instance->pci_handle) !=
			    DDI_SUCCESS) {
				goto fail_attach;
			}

			instance->mr_ld_list =
			    kmem_zalloc(MRDRV_MAX_LD *
			    sizeof (struct imraidsas_ld),
			    KM_SLEEP);
			ASSERT(instance->mr_ld_list);

			instance->mr_pd_max = MRSAS_PD_TGT_MAX;
			instance->mr_pd_list =
			    kmem_zalloc(MRSAS_GET_PD_MAX(instance)
			    * sizeof (struct imraidsas_pd), KM_SLEEP);
			ASSERT(instance->mr_pd_list);
			for (i = 0; i < instance->mr_pd_max; i++) {
				instance->mr_pd_list[i].lun_type = MRSAS_PD_LUN;
				instance->mr_pd_list[i].dev_id = (uint8_t)i;
			}
			break;
		case DDI_RESUME:
			con_log(CL_ANN, (CE_NOTE,
			    "imraid_sas: DDI_RESUME"));
			break;
		default:
			con_log(CL_ANN, (CE_WARN,
			    "imraid_sas: invalid attach cmd=%x", cmd));
			return (DDI_FAILURE);
	}
...
fail_attach:
	...
	imraidsas_fm_ereport(instance, DDI_FM_DEVICE_NO_RESPONSE);
	ddi_fm_service_impact(instance->dip, DDI_SERVICE_LOST);

	imraidsas_fm_fini(instance);

	pci_config_teardown(&instance->pci_handle);

	ddi_soft_state_free(imraidsas_state, instance_no);

	con_log(CL_ANN, (CE_NOTE,
	    "imraid_sas: return failure from imraidsas_attach"));
...
/*ARGSUSED*/
static int
imraidsas_getinfo(dev_info_t *dip, ddi_info_cmd_t cmd,
    void *arg, void **resultp)
{
	int	rval;
	int	imraidsas_minor = getminor((dev_t)arg);
	...
	switch (cmd) {
		case DDI_INFO_DEVT2DEVINFO:
			instance = (struct imraidsas_instance *)
			    ddi_get_soft_state(imraidsas_state,
			    MINOR2INST(imraidsas_minor));
...
static int
imraidsas_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	int	instance_no;
	...
	if (!instance) {
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas:%d could not get instance in detach",
		    instance_no));
	...
	con_log(CL_ANN, (CE_NOTE,
	    "imraid_sas%d: detaching device 0x%4x:0x%4x:0x%4x:0x%4x",
	...
	case DDI_DETACH:
		con_log(CL_ANN, (CE_NOTE,
		    "imraidsas_detach: DDI_DETACH"));

		if (scsi_hba_detach(dip) != DDI_SUCCESS) {
			con_log(CL_ANN, (CE_WARN,
			    "imraid_sas:%d failed to detach",
			    instance_no));
		...
		if (imraidsas_free_dma_obj(instance, drv_ver_dma_obj)
		    != DDI_SUCCESS)
			return (DDI_FAILURE);
		...
		if (abort_aen_cmd(instance, instance->aen_cmd)) {
			con_log(CL_ANN, (CE_WARN, "imraidsas_detach: "
		...
		if (untimeout(instance->timeout_id) == -1) {
			con_log(CL_ANN, (CE_WARN,
			    "imraid_sas:%d Timer not found ",
			    instance_no));
		} else {
			con_log(CL_ANN, (CE_WARN,
			    "imraid_sas:%d Stopped Timer Successfully",
			    instance_no));
		}
		...
		kmem_free(instance->mr_pd_list, MRSAS_GET_PD_MAX(instance)
		    * sizeof (struct imraidsas_pd));
...
static  int
imraidsas_open(dev_t *dev, int openflags, int otyp, cred_t *credp)
{
	int	rval = 0;
	...
	if (drv_priv(credp) != 0) {
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas: Non-root ioctl access denied!"));
		return (EPERM);
	}
	...
	if (otyp != OTYP_CHR) {
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas: ioctl node must be a char node"));
		return (EINVAL);
	}

	if (ddi_get_soft_state(imraidsas_state, MINOR2INST(getminor(*dev)))
	    == NULL) {
		return (ENXIO);
	}
...
static  int
imraidsas_close(dev_t dev, int openflags, int otyp, cred_t *credp)
{
	int	rval = 0;
...
static int
imraidsas_ioctl(dev_t dev, int cmd, intptr_t arg, int mode, cred_t *credp,
    int *rvalp)
{
	int	rval = 0;

	struct imraidsas_instance	*instance;
	struct imraidsas_ioctl	*ioctl;
	struct imraidsas_aen	aen;

	con_log(CL_ANN1, (CE_NOTE, "chkpnt:%s:%d", __func__, __LINE__));

	instance = ddi_get_soft_state(imraidsas_state,
	    MINOR2INST(getminor(dev)));

	if (instance == NULL) {
		/* invalid minor number */
		con_log(CL_ANN, (CE_WARN, "imraid_sas: adapter not found."));
		return (ENXIO);
	}

	ioctl = (struct imraidsas_ioctl *)kmem_zalloc(
	    sizeof (struct imraidsas_ioctl),
	    KM_SLEEP);
	ASSERT(ioctl);
	...
	switch ((uint_t)cmd) {
		case MRSAS_IOCTL_FIRMWARE:
			if (ddi_copyin((void *)arg, ioctl,
			    sizeof (struct imraidsas_ioctl), mode)) {
				con_log(CL_ANN, (CE_WARN, "imraidsas_ioctl: "
				    "ERROR IOCTL copyin"));
				kmem_free(ioctl,
				    sizeof (struct imraidsas_ioctl));
				return (EFAULT);
			...
			if (ddi_copyout((void *)ioctl, (void *)arg,
			    (sizeof (struct imraidsas_ioctl) - 1), mode)) {
				con_log(CL_ANN, (CE_WARN,
				    "imraidsas_ioctl: copy_to_user failed"));
				rval = 1;
			}
		...
		case MRSAS_IOCTL_AEN:
			if (ddi_copyin((void *) arg, &aen,
			    sizeof (struct imraidsas_aen), mode)) {
				con_log(CL_ANN, (CE_WARN,
				    "imraidsas_ioctl: ERROR AEN copyin"));
				kmem_free(ioctl,
				    sizeof (struct imraidsas_ioctl));
				return (EFAULT);
			}
		...
		default:
			...
			con_log(CL_DLEVEL1, (CE_NOTE, "imraidsas_ioctl: "
			    "scsi_hba_ioctl called, ret = %x.", rval));
	}

	kmem_free(ioctl, sizeof (struct imraidsas_ioctl));
	return (rval);
}
...
/*ARGSUSED*/
static int
imraidsas_quiesce(dev_info_t *dip)
{
	int	instance_no;
	struct	imraidsas_instance   *instance;
	instance_no = ddi_get_instance(dip);
	instance = (struct imraidsas_instance *)ddi_get_soft_state
	    (imraidsas_state, instance_no);
	con_log(CL_ANN1, (CE_NOTE, "chkpnt:%s:%d", __func__, __LINE__));
	if (!instance) {
		con_log(CL_ANN1, (CE_WARN,
		    "imraid_sas:%d could not get adapter "
		    "in quiesce", instance_no));
		return (DDI_FAILURE);
	}
	if (instance->deadadapter || instance->adapterresetinprogress) {
		con_log(CL_ANN1, (CE_WARN, "imraid_sas:%d adapter is not in "
		    "healthy state", instance_no));
		return (DDI_FAILURE);
	}
	if (abort_aen_cmd(instance, instance->aen_cmd)) {
		con_log(CL_ANN1, (CE_WARN, "imraidsas_quiesce: "
...
/*ARGSUSED*/
static int
imraidsas_tran_tgt_init(dev_info_t *hba_dip, dev_info_t *tgt_dip,
		scsi_hba_tran_t *tran, struct scsi_device *sd)
{
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_tgt_init target %d lun %d",
	    tgt, lun));

	instance = ADDR2MR(&sd->sd_address);

	if (ndi_dev_is_persistent_node(tgt_dip) == 0) {
		(void) ndi_merge_node(tgt_dip, imraidsas_name_node);
		ddi_set_name_addr(tgt_dip, NULL);
		con_log(CL_ANN1, (CE_NOTE, "imraidsas_tgt_init in ndi_per "
		    "DDI_FAILURE t = %d l = %d", tgt, lun));
		return (DDI_FAILURE);
	}

	con_log(CL_ANN1, (CE_NOTE, "imraidsas_tgt_init dev_dip %p tgt_dip %p",
	...
	if (tgt < MRDRV_MAX_LD && lun == 0) {
		...
		if (instance->mr_pd_list[tgt].dip == NULL) {
			mutex_enter(&instance->config_dev_mtx);
			instance->mr_pd_list[tgt].dip = tgt_dip;
			instance->mr_pd_list[tgt].flag = MRDRV_TGT_VALID;
			mutex_exit(&instance->config_dev_mtx);
			con_log(CL_ANN1, (CE_NOTE, "imraidsas_tran_tgt_init:"
			    "t%xl%x", tgt, lun));
		}
	}
	return (DDI_SUCCESS);
...
/*ARGSUSED*/
static void
imraidsas_tran_tgt_free(dev_info_t *hba_dip, dev_info_t *tgt_dip,
    scsi_hba_tran_t *hba_tran, struct scsi_device *sd)
{
	...
	if (tgt < MRDRV_MAX_LD && lun == 0) {
		...
		mutex_enter(&instance->config_dev_mtx);
		instance->mr_pd_list[tgt].dip = NULL;
		mutex_exit(&instance->config_dev_mtx);
...
static dev_info_t *
imraidsas_find_child(struct imraidsas_instance *instance,
    uint16_t tgt, uint8_t lun)
{
	...
	for (child = ddi_get_child(instance->dip); child;
		...
		if (imraidsas_name_node(child, tmp, MAXNAMELEN) !=
		    DDI_SUCCESS) {
			continue;
		}
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_find_child: return child = %p",
	    (void *)child));
	return (child);
}

static int
imraidsas_name_node(dev_info_t *dip, char *name, int len)
{
	int tgt, lun;
	...
	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_name_node: dip %p tgt %d", (void *)dip, tgt));
	if (tgt == -1) {
		return (DDI_FAILURE);
	}
	lun = ddi_prop_get_int(DDI_DEV_T_ANY, dip, DDI_PROP_DONTPASS,
	    "lun", -1);
	con_log(CL_ANN1,
	    (CE_NOTE, "imraidsas_name_node: tgt %d lun %d", tgt, lun));
	if (lun == -1) {
		return (DDI_FAILURE);
	}
...
imraidsas_tran_init_pkt(struct scsi_address *ap, register struct scsi_pkt *pkt,
	...
	int flags, int (*callback)(), caddr_t arg)
{
	...
	if (bp && bp->b_bcount != 0) {
		if (acmd->cmd_dmahandle == NULL) {
			if (imraidsas_dma_alloc(instance, pkt, bp, flags,
			...
			if (imraidsas_dma_move(instance, pkt, bp) ==
			    DDI_FAILURE) {
				return ((struct scsi_pkt *)NULL);
			}
		}
...
static int
imraidsas_tran_start(struct scsi_address *ap, register struct scsi_pkt *pkt)
{
	uchar_t 	cmd_done = 0;

	struct imraidsas_instance	*instance = ADDR2MR(ap);
	struct imraidsas_cmd	*cmd;

	if (instance->deadadapter == 1) {
		con_log(CL_ANN1, (CE_WARN,
		    "imraidsas_tran_start: return TRAN_FATAL_ERROR "
	...
	*pkt->pkt_scbp = STATUS_GOOD; /* clear arq scsi_status */
	...
	/*
	 * Check if the command is already
	 * completed by the imraidsas_build_cmd()
	 * routine. In which case the busy_flag
	 * would be clear and scb will be
	...
	if ((pkt->pkt_flags & FLAG_NOINTR) == 0) {
		if (instance->fw_outstanding > instance->max_fw_cmds) {
			con_log(CL_ANN, (CE_CONT, "imraid_sas:Firmware busy"));
		...
		struct imraidsas_header *hdr = &cmd->frame->hdr;
		...
		(void) imraidsas_common_check(instance, cmd);
...
/*ARGSUSED*/
static int
imraidsas_tran_abort(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	con_log(CL_ANN1, (CE_NOTE, "chkpnt:%s:%d", __func__, __LINE__));
...
/*ARGSUSED*/
static int
imraidsas_tran_reset(struct scsi_address *ap, int level)
{
	struct imraidsas_instance *instance = ADDR2MR(ap);
...
/* ARGSUSED */
static int
imraidsas_tran_bus_reset(dev_info_t *dip, int level)
{
	int	instance_no = ddi_get_instance(dip);
	struct imraidsas_instance *instance = ddi_get_soft_state(
	    imraidsas_state,
	    instance_no);
...
/*ARGSUSED*/
static int
imraidsas_tran_getcap(struct scsi_address *ap, char *cap, int whom)
{
	int	rval = 0;
	...
	case SCSI_CAP_DMA_MAX:
		...
		rval = imraidsas_max_cap_maxxfer;
		break;
...
/*ARGSUSED*/
static int
imraidsas_tran_setcap(struct scsi_address *ap, char *cap, int value, int whom)
{
	int		rval = 1;
...
static void
imraidsas_tran_destroy_pkt(struct scsi_address *ap, struct scsi_pkt *pkt)
{
...
/*ARGSUSED*/
static void
imraidsas_tran_dmafree(struct scsi_address *ap, struct scsi_pkt *pkt)
{
...
/*ARGSUSED*/
static void
imraidsas_tran_sync_pkt(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	register struct scsa_cmd	*acmd = PKT2CMD(pkt);
 ...
 */
static uint_t
imraidsas_isr(struct imraidsas_instance *instance)
{
	int		need_softintr;
	uint32_t	producer;
	uint32_t	consumer;
	...
	if (imraidsas_check_dma_handle(
	    instance->mfi_internal_dma_obj.dma_handle)
	    != DDI_SUCCESS) {
		imraidsas_fm_ereport(instance, DDI_FM_DEVICE_NO_RESPONSE);
		ddi_fm_service_impact(instance->dip, DDI_SERVICE_LOST);
		con_log(CL_ANN1, (CE_WARN,
		    "imraid_sas_isr(): FMA check, "
		    "returning DDI_INTR_UNCLAIMED"));
	...
	while (consumer != producer) {
		...
		hdr = (struct imraidsas_header *)&cmd->frame->hdr;
		if (hdr) {
	...
	if (instance->isr_level == HIGH_LEVEL_INTR) {
		 ...
		 */
		(void) imraidsas_softintr(instance);
	}
 ...
 */
static struct imraidsas_cmd *
get_mfi_pkt(struct imraidsas_instance *instance)
{
	mlist_t 		*head = &instance->cmd_pool_list;
	struct imraidsas_cmd	*cmd = NULL;

	mutex_enter(&instance->cmd_pool_mtx);
	ASSERT(mutex_owned(&instance->cmd_pool_mtx));

	if (!mlist_empty(head)) {
		cmd = mlist_entry(head->next, struct imraidsas_cmd, list);
...
static struct imraidsas_cmd *
get_mfi_app_pkt(struct imraidsas_instance *instance)
{
 ...
 */
static void
return_mfi_pkt(struct imraidsas_instance *instance, struct imraidsas_cmd *cmd)
{
	mutex_enter(&instance->cmd_pool_mtx);
	ASSERT(mutex_owned(&instance->cmd_pool_mtx));
...
static void
return_mfi_app_pkt(struct imraidsas_instance *instance,
    struct imraidsas_cmd *cmd)
{
	mutex_enter(&instance->app_cmd_pool_mtx);
	ASSERT(mutex_owned(&instance->app_cmd_pool_mtx));
...
static void
push_pending_mfi_pkt(struct imraidsas_instance *instance,
    struct imraidsas_cmd *cmd)
{
	struct scsi_pkt *pkt;
	struct imraidsas_header	*hdr;
...
static int
imraidsas_print_pending_cmds(struct imraidsas_instance *instance)
{
	...
	struct scsi_pkt *pkt;
	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_print_pending_cmds(): Called"));
	while (flag) {
		mutex_enter(&instance->cmd_pend_mtx);
		tmp	=	tmp->next;
		if (tmp == head) {
			...
			cmd = mlist_entry(tmp, struct imraidsas_cmd, list);
			mutex_exit(&instance->cmd_pend_mtx);
			if (cmd) {
				if (cmd->sync_cmd == MRSAS_TRUE) {
				hdr = (struct imraidsas_header *)&
				    cmd->frame->hdr;
					if (hdr) {
					con_log(CL_ANN1, (CE_CONT,
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_print_pending_cmds(): Done\n"));
	return (DDI_SUCCESS);
}


static int
imraidsas_complete_pending_cmds(struct imraidsas_instance *instance)
{
	...
	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_complete_pending_cmds(): Called"));


	mutex_enter(&instance->cmd_pend_mtx);
	mlist_for_each_safe(pos, next, &instance->cmd_pend_list) {
		cmd = mlist_entry(pos, struct imraidsas_cmd, list);
		if (cmd) {
			pkt = cmd->pkt;
			if (pkt) { /* for IO */
			...
			con_log(CL_ANN1, (CE_NOTE,
			    "imraidsas_complete_pending_cmds:"
			    "NULL command\n"));
		}
		con_log(CL_ANN1, (CE_NOTE,
		    "imraidsas_complete_pending_cmds:"
	...
	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_complete_pending_cmds(): DONE\n"));
	return (DDI_SUCCESS);
}


static int
imraidsas_issue_pending_cmds(struct imraidsas_instance *instance)
{
	mlist_t *head	=	&instance->cmd_pend_list;
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_issue_pending_cmds(): Called"));
	while (tmp != head) {
		mutex_enter(&instance->cmd_pend_mtx);
		cmd = mlist_entry(tmp, struct imraidsas_cmd, list);
		tmp = tmp->next;
		...
		if (cmd) {
			...
			if (cmd->retry_count_for_ocr > IO_RETRY_COUNT) {
				...
				(void) imraidsas_kill_adapter(instance);
				return (DDI_FAILURE);
			}
			pkt = cmd->pkt;
			if (pkt) {
			...
			con_log(CL_ANN1, (CE_NOTE,
			    "imraidsas_issue_pending_cmds: NULL command\n"));
		}
		con_log(CL_ANN1, (CE_NOTE,
		    "imraidsas_issue_pending_cmds:"
		    "looping for more commands"));
	}
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_issue_pending_cmds(): DONE\n"));
	return (DDI_SUCCESS);
}
 ...
 */
static void
destroy_mfi_frame_pool(struct imraidsas_instance *instance)
{
	int		i;
	...
	for (i = 0; i < max_cmd+1; i++) {
		...
		if (cmd->frame_dma_obj_status == DMA_OBJ_ALLOCATED)
			(void) imraidsas_free_dma_obj(instance,
			    cmd->frame_dma_obj);
 ...
 */
static int
create_mfi_frame_pool(struct imraidsas_instance *instance)
{
	int		i = 0;
	int		cookie_cnt;
	uint16_t	max_cmd;
	...
	sge_sz	= sizeof (struct imraidsas_sge_ieee);
	...
	while (i < max_cmd+1) {
		...
		cmd->frame_dma_obj.dma_attr = imraidsas_generic_dma_attr;
		...
		cookie_cnt = imraidsas_alloc_dma_obj(instance,
		    &cmd->frame_dma_obj,
		...
		cmd->frame = (union imraidsas_frame *)cmd->frame_dma_obj.buffer;
		cmd->frame_phys_addr =
		...
		if (!cmd->frame || !cmd->sense) {
			con_log(CL_ANN, (CE_NOTE,
			    "imraid_sas: pci_pool_alloc failed"));
 ...
 */
static void
free_additional_dma_buffer(struct imraidsas_instance *instance)
{
 ...
 */
static int
alloc_additional_dma_buffer(struct imraidsas_instance *instance)
{
	uint32_t	reply_q_sz;
	...
	instance->mfi_internal_dma_obj.dma_attr	= imraidsas_generic_dma_attr;
	...
	if (imraidsas_alloc_dma_obj(instance, &instance->mfi_internal_dma_obj,
	    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas: could not alloc reply queue"));
		return (DDI_FAILURE);
	}
	...
	/* allocate evt_detail */
	instance->mfi_evt_detail_obj.size =
	    sizeof (struct imraidsas_evt_detail);
	instance->mfi_evt_detail_obj.dma_attr = imraidsas_generic_dma_attr;
	...
	if (imraidsas_alloc_dma_obj(instance, &instance->mfi_evt_detail_obj,
	    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
 ...
 */
static void
free_space_for_mfi(struct imraidsas_instance *instance)
{
	int		i;
	uint32_t	max_cmd = instance->max_fw_cmds;
	...
	kmem_free(instance->cmd_list,
	    sizeof (struct imraidsas_cmd *) * (max_cmd+1));
 ...
 */
static int
alloc_space_for_mfi(struct imraidsas_instance *instance)
{
	int		i;
	uint32_t	max_cmd;
	...
	sz = sizeof (struct imraidsas_cmd *) * (max_cmd+1);

	/*
	 * instance->cmd_list is an array of struct imraidsas_cmd pointers.
	...
	for (i = 0; i < max_cmd+1; i++) {
		instance->cmd_list[i] = kmem_zalloc(
		    sizeof (struct imraidsas_cmd),
		    KM_SLEEP);
 ...
 */
static int
get_ctrl_info(struct imraidsas_instance *instance,
    struct imraidsas_ctrl_info *ctrl_info)
{
	int	ret = 0;

	struct imraidsas_cmd		*cmd;
	struct imraidsas_dcmd_frame	*dcmd;
	struct imraidsas_ctrl_info	*ci;
	...
	(void) memset((char *)&cmd->frame[0], 0,
	    sizeof (union imraidsas_frame));
	ddi_put32(cmd->frame_dma_obj.acc_handle, &cmd->frame->hdr.context,
	    cmd->index);

	dcmd = &cmd->frame->dcmd;

	ci = (struct imraidsas_ctrl_info *)instance->internal_buf;
	...
	(void) memset(ci, 0, sizeof (struct imraidsas_ctrl_info));
	...
	ddi_put32(cmd->frame_dma_obj.acc_handle, &dcmd->data_xfer_len,
	    sizeof (struct imraidsas_ctrl_info));
	...
	if (imraidsas_common_check(instance, cmd) != DDI_SUCCESS) {
		ret = -1;
	}
	return_mfi_pkt(instance, cmd);
 ...
 */
static int
abort_aen_cmd(struct imraidsas_instance *instance,
    struct imraidsas_cmd *cmd_to_abort)
{
	int	ret = 0;

	struct imraidsas_cmd		*cmd;
	struct imraidsas_abort_frame	*abort_fr;
	uint32_t	max_cmd = instance->max_fw_cmds;
 ...
 */
static int
init_mfi(struct imraidsas_instance *instance)
{
	struct imraidsas_cmd		*cmd;
	struct imraidsas_ctrl_info		ctrl_info;
	struct imraidsas_init_frame		*init_frame;
	struct imraidsas_init_queue_info	*initq_info;
	struct imraidsas_drv_ver    	drv_ver_info;

	/* we expect the FW state to be READY */
	if (mfi_state_transition_to_ready(instance)) {
		con_log(CL_ANN, (CE_WARN, "imraid_sas: F/W is not ready"));
	...
	init_frame = (struct imraidsas_init_frame *)cmd->frame;
	initq_info = (struct imraidsas_init_queue_info *)
	    ((unsigned long)init_frame + 64);

	(void) memset(init_frame, 0, MRMFI_FRAME_SIZE);
	(void) memset(initq_info, 0, sizeof (struct imraidsas_init_queue_info));
	...
	drv_ver_dma_obj.size = sizeof (drv_ver_info.drv_ver);
	drv_ver_dma_obj.dma_attr = imraidsas_generic_dma_attr;
	drv_ver_dma_obj.dma_attr.dma_attr_addr_hi = 0xFFFFFFFFU;
	drv_ver_dma_obj.dma_attr.dma_attr_count_max = 0xFFFFFFFFU;
	drv_ver_dma_obj.dma_attr.dma_attr_sgllen = 1;
	drv_ver_dma_obj.dma_attr.dma_attr_align = 1;

	if (imraidsas_alloc_dma_obj(instance, &drv_ver_dma_obj,
	    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
		con_log(CL_ANN, (CE_WARN,
	...
	(void) memset(drv_ver_dma_obj.buffer, 0, sizeof (drv_ver_info.drv_ver));
	ddi_rep_put8(cmd->frame_dma_obj.acc_handle,
	    (uint8_t *)drv_ver_info.drv_ver,
	    (uint8_t *)drv_ver_dma_obj.buffer,
	...
	ddi_put64(cmd->frame_dma_obj.acc_handle,
	    &init_frame->driverversion,
	    drv_ver_dma_obj.dma_cookie[0].dmac_address);

	ddi_put32(cmd->frame_dma_obj.acc_handle, &init_frame->data_xfer_len,
	    sizeof (struct imraidsas_init_queue_info));
	...
	if (instance->func_ptr->read_fw_status_reg(instance) & 0x04000000) {
		con_log(CL_ANN, (CE_NOTE, "imraid_sas: IEEE SGL's supported"));
...
static int
imraidsas_issue_init_mfi(struct imraidsas_instance *instance)
{
	struct imraidsas_cmd		*cmd;
	struct imraidsas_init_frame		*init_frame;
	struct imraidsas_init_queue_info	*initq_info;
 ...
 */
	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_issue_init_mfi: entry\n"));
	cmd = get_mfi_app_pkt(instance);

	if (!cmd) {
		con_log(CL_ANN1, (CE_NOTE,
		    "imraidsas_issue_init_mfi: get_pkt failed\n"));
		return (DDI_FAILURE);
	}
	...
	if (instance->func_ptr->issue_cmd_in_poll_mode(instance, cmd)) {
		con_log(CL_ANN1, (CE_WARN,
		    "imraidsas_issue_init_mfi():failed to "
		    "init firmware"));
		return_mfi_app_pkt(instance, cmd);
		return (DDI_FAILURE);
	}
	return_mfi_app_pkt(instance, cmd);
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_issue_init_mfi: Done"));
	return (DDI_SUCCESS);
}
/*
 ...
 */
static int
mfi_state_transition_to_ready(struct imraidsas_instance *instance)
{
	int		i;
	uint8_t		max_wait;
	uint32_t	fw_ctrl;
	...
	while (fw_state != MFI_STATE_READY) {
		...
		case MFI_STATE_FAULT:
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: FW in FAULT state!!"));

			return (ENODEV);
		case MFI_STATE_WAIT_HANDSHAKE:
			/* set the CLR bit in IMR0 */
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: FW waiting for HANDSHAKE"));
			/*
		...
		case MFI_STATE_BOOT_MESSAGE_PENDING:
			/* set the CLR bit in IMR0 */
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: FW state boot message pending"));
			/*
			 * PCI_Hot Plug: MFI F/W requires
			 * (MFI_INIT_CLEAR_HANDSHAKE|MFI_INIT_HOTPLUG)
			 * to be set
		...
		case MFI_STATE_OPERATIONAL:
			...
			instance->func_ptr->disable_intr(instance);
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: FW in OPERATIONAL state"));
			/*
			 * PCI_Hot Plug: MFI F/W requires
		...
		default:
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: Unknown state 0x%x", fw_state));
			return (ENODEV);
		}
 ...
 */
static int
get_seq_num(struct imraidsas_instance *instance,
    struct imraidsas_evt_log_info *eli)
{
	int	ret = DDI_SUCCESS;
	...
	struct imraidsas_cmd		*cmd;
	struct imraidsas_dcmd_frame		*dcmd;
	struct imraidsas_evt_log_info *eli_tmp;
	cmd = get_mfi_pkt(instance);

	if (!cmd) {
		cmn_err(CE_WARN, "imraid_sas: failed to get a cmd");
	...
	dcmd_dma_obj.size = sizeof (struct imraidsas_evt_log_info);
	dcmd_dma_obj.dma_attr = imraidsas_generic_dma_attr;
	...
	if (imraidsas_alloc_dma_obj(instance, &dcmd_dma_obj,
	    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
		con_log(CL_ANN, (CE_WARN,
	...
	(void) memset(dcmd_dma_obj.buffer, 0,
	    sizeof (struct imraidsas_evt_log_info));
	...
	if (instance->func_ptr->issue_cmd_in_sync_mode(instance, cmd)) {
		...
		eli_tmp = (struct imraidsas_evt_log_info *)dcmd_dma_obj.buffer;
		eli->newest_seq_num = ddi_get32(cmd->frame_dma_obj.acc_handle,
		    &eli_tmp->newest_seq_num);
		ret = DDI_SUCCESS;
	}

	if (imraidsas_free_dma_obj(instance, dcmd_dma_obj) != DDI_SUCCESS)
		ret = DDI_FAILURE;
 ...
 */
static int
start_mfi_aen(struct imraidsas_instance *instance)
{
	int	ret = 0;

	struct imraidsas_evt_log_info	eli;
	union imraidsas_evt_class_locale	class_locale;

	/* get the latest sequence number from FW */
	(void) memset(&eli, 0, sizeof (struct imraidsas_evt_log_info));
 ...
 */
static void
flush_cache(struct imraidsas_instance *instance)
{
	struct imraidsas_cmd		*cmd = NULL;
	struct imraidsas_dcmd_frame		*dcmd;
	uint32_t	max_cmd = instance->max_fw_cmds;
 ...
 */
static void
service_mfi_aen(struct imraidsas_instance *instance, struct imraidsas_cmd *cmd)
{
	uint32_t	seq_num;
	struct imraidsas_evt_detail *evt_detail =
	    (struct imraidsas_evt_detail *)instance->mfi_evt_detail_obj.buffer;
	int		rval = 0;
	int		tgt = 0;
	uint8_t		dtype;
	imraidsas_pd_address_t *pd_addr;
	...
	if (ddi_log_sysevent(instance->dip, DDI_VENDOR_LSI, "LSIMEGA", "SAS",
		...
		con_log(CL_ANN, (CE_WARN,
		    "imraid_sas%d: Failed to log AEN event", instance_no));
	}
	/*
	...
	case MR_EVT_CFG_CLEARED: {
		for (tgt = 0; tgt < MRDRV_MAX_LD; tgt++) {
			if (instance->mr_ld_list[tgt].dip != NULL) {
				...
				mutex_exit(&instance->config_dev_mtx);
				rval = imraidsas_service_evt(instance, tgt, 0,
				    MRSAS_EVT_UNCONFIG_TGT, NULL);
				con_log(CL_ANN1, (CE_WARN,
				    "imraid_sas: CFG CLEARED AEN rval = %d "
				    "tgt id = %d", rval, tgt));
			}
		}
		break;
	...
	case MR_EVT_LD_DELETED: {
		...
		mutex_exit(&instance->config_dev_mtx);
		rval = imraidsas_service_evt(instance,
		    ddi_get16(acc_handle, &evt_detail->args.ld.target_id), 0,
		    MRSAS_EVT_UNCONFIG_TGT, NULL);
		con_log(CL_ANN1, (CE_WARN,
		    "imraid_sas: LD DELETED AEN rval = %d "
	...
	case MR_EVT_LD_CREATED: {
		...
		con_log(CL_ANN1, (CE_WARN,
		    "imraid_sas: LD CREATED AEN rval = %d "
		    "tgt id = %d index = %d", rval,
		    ddi_get16(acc_handle, &evt_detail->args.ld.target_id),
		    ddi_get8(acc_handle, &evt_detail->args.ld.ld_index)));
		break;
	...
	case MR_EVT_PD_REMOVED_EXT: {
		...
		mutex_enter(&instance->config_dev_mtx);
		instance->mr_pd_list[tgt].flag = (uint8_t)~MRDRV_TGT_VALID;
		mutex_exit(&instance->config_dev_mtx);
		rval = imraidsas_service_evt(instance,
		    ddi_get16(acc_handle, &evt_detail->args.pd.device_id),
		    1, MRSAS_EVT_UNCONFIG_TGT, NULL);
		con_log(CL_ANN1, (CE_WARN, "imraid_sas: PD_REMOVED:"
	...
	case MR_EVT_PD_INSERTED_EXT: {
		...
		con_log(CL_ANN1, (CE_WARN,
		    "imraid_sas: PD_INSERTED_EXT: rval = %d "
		    "tgt id = %d ", rval,
		    ddi_get16(acc_handle, &evt_detail->args.pd.device_id)));
		break;
 ...
 */
static void
complete_cmd_in_sync_mode(struct imraidsas_instance *instance,
    struct imraidsas_cmd *cmd)
{
...
/*
 * Call this function inside imraidsas_softintr.
 * imraidsas_initiate_ocr_if_fw_is_faulty  -
 * Initiates OCR if FW status is faulty
 * @instance:			Adapter soft state
 */

static uint32_t
imraidsas_initiate_ocr_if_fw_is_faulty(struct imraidsas_instance *instance)
{
	uint32_t	cur_abs_reg_val;
	uint32_t	fw_state;
...
/*
 * imraidsas_softintr - The Software ISR
 ...
 */
static uint_t
imraidsas_softintr(struct imraidsas_instance *instance)
{
	struct scsi_pkt		*pkt;
	struct scsa_cmd		*acmd;
	struct imraidsas_cmd	*cmd;
	...
	con_log(CL_ANN1, (CE_CONT, "imraidsas_softintr called"));
	...
	mlist_for_each_safe(pos, next, &process_list) {
		...
		if (imraidsas_check_dma_handle(cmd->frame_dma_obj.dma_handle) !=
		    DDI_SUCCESS) {
			imraidsas_fm_ereport(instance,
		...
		case MFI_CMD_OP_LD_WRITE:
			...
			con_log(CL_ANN1, (CE_CONT, "CDB[0] = %x completed"
			    "%s: size %lx context %x"
			    "cmd_status %x",
			    pkt->pkt_cdbp[0], ((acmd->islogical) ? "LD" : "PD"),
			    acmd->cmd_dmacount, hdr->context, hdr->cmd_status));
			...
			case MFI_STAT_DEVICE_NOT_FOUND:
				con_log(CL_ANN1, (CE_CONT,
				"imraidsas_softintr:device not found error"));
				pkt->pkt_reason	= CMD_DEV_GONE;
				pkt->pkt_statistics  = STAT_DISCON;
				break;
...
/*
 * imraidsas_alloc_dma_obj
 *
 * Allocate the memory and other resources for an dma object.
 */
static int
imraidsas_alloc_dma_obj(struct imraidsas_instance *instance, dma_obj_t *obj,
    uchar_t endian_flags)
{
	...
	if (imraidsas_check_dma_handle(obj->dma_handle) != DDI_SUCCESS) {
		ddi_fm_service_impact(instance->dip, DDI_SERVICE_LOST);
		return (-1);
	}

	if (imraidsas_check_acc_handle(obj->acc_handle) != DDI_SUCCESS) {
		ddi_fm_service_impact(instance->dip, DDI_SERVICE_LOST);
		return (-1);
	}
...
/*
 * imraidsas_free_dma_obj(struct imraidsas_instance *, dma_obj_t)
 *
 * De-allocate the memory and other resources for an dma object, which must
 * have been alloated by a previous call to imraidsas_alloc_dma_obj()
 */
static int
imraidsas_free_dma_obj(struct imraidsas_instance *instance, dma_obj_t obj)
{
...
/*
 * imraidsas_dma_alloc(instance_t *, struct scsi_pkt *, struct buf *,
 * int, int (*)())
 *
 * Allocate dma resources for a new scsi command
 */
static int
imraidsas_dma_alloc(struct imraidsas_instance *instance, struct scsi_pkt *pkt,
    struct buf *bp, int flags, int (*callback)())
{
	...
	ddi_dma_attr_t	tmp_dma_attr = imraidsas_generic_dma_attr;
	struct scsa_cmd	*acmd = PKT2CMD(pkt);
...
/*
 * imraidsas_dma_move(struct imraidsas_instance *,
 ...
 */
static int
imraidsas_dma_move(struct imraidsas_instance *instance, struct scsi_pkt *pkt,
    struct buf *bp)
{
	int	i = 0;
 ...
 */
static struct imraidsas_cmd *
build_cmd(struct imraidsas_instance *instance, struct scsi_address *ap,
    struct scsi_pkt *pkt, uchar_t *cmd_done)
{
	...
	ddi_acc_handle_t acc_handle;
	struct imraidsas_cmd		*cmd;
	struct imraidsas_sge64		*mfi_sgl;
	struct imraidsas_sge_ieee		*mfi_sgl_ieee;
	struct scsa_cmd			*acmd = PKT2CMD(pkt);
	struct imraidsas_pthru_frame 	*pthru;
	struct imraidsas_io_frame		*ldio;
	...
	*cmd_done = 0;
	...
	case SCMD_WRITE_G1:
		if (acmd->islogical) {
			ldio = (struct imraidsas_io_frame *)cmd->frame;
			...
			if (instance->flag_ieee) {
				mfi_sgl_ieee =
				    (struct imraidsas_sge_ieee *)&ldio->sgl;
			} else {
				mfi_sgl = (struct imraidsas_sge64 *)&ldio->sgl;
			}
	...
	default:
		...
		case SCMD_MODE_SENSE_G1: {
			...
			case 0x3:
			case 0x4:
				(void) imraidsas_mode_sense_build(pkt);
				return_mfi_pkt(instance, cmd);
				*cmd_done = 1;
				return (NULL);
			}
		...
		pthru	= (struct imraidsas_pthru_frame *)cmd->frame;
		...
		if (instance->flag_ieee) {
			mfi_sgl_ieee = (struct imraidsas_sge_ieee *)&pthru->sgl;
		} else {
			mfi_sgl	= (struct imraidsas_sge64 *)&pthru->sgl;
		}
	...
	if (instance->flag_ieee) {
		...
		sge_bytes = sizeof (struct imraidsas_sge_ieee)*
		    acmd->cmd_cookiecnt;
	} else {
		...
		sge_bytes = sizeof (struct imraidsas_sge64)*acmd->cmd_cookiecnt;
	}
...
static int
wait_for_outstanding(struct imraidsas_instance *instance)
{
	int		i;
 ...
 */
static int
issue_mfi_pthru(struct imraidsas_instance *instance,
    struct imraidsas_ioctl *ioctl,
    struct imraidsas_cmd *cmd, int mode)
{
	void		*ubuf;
	...
	struct imraidsas_pthru_frame	*kpthru;
	struct imraidsas_pthru_frame	*pthru;
	int i;
	pthru = &cmd->frame->pthru;
	kpthru = (struct imraidsas_pthru_frame *)&ioctl->frame[0];
	...
	if (xferlen) {
		...
		pthru_dma_obj.dma_attr = imraidsas_generic_dma_attr;
		...
		if (imraidsas_alloc_dma_obj(instance, &pthru_dma_obj,
		    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
	...
	if (xferlen) {
		/* free kernel buffer */
		if (imraidsas_free_dma_obj(instance,
		    pthru_dma_obj) != DDI_SUCCESS)
			return (DDI_FAILURE);
	}
 ...
 */
static int
issue_mfi_dcmd(struct imraidsas_instance *instance,
    struct imraidsas_ioctl *ioctl,
    struct imraidsas_cmd *cmd, int mode)
{
	void		*ubuf;
	...
	uint32_t	model;
	dma_obj_t	dcmd_dma_obj;
	struct imraidsas_dcmd_frame	*kdcmd;
	struct imraidsas_dcmd_frame	*dcmd;
	ddi_acc_handle_t	acc_handle = cmd->frame_dma_obj.acc_handle;
	int i;
	dcmd = &cmd->frame->dcmd;
	kdcmd = (struct imraidsas_dcmd_frame *)&ioctl->frame[0];
	if (instance->adapterresetinprogress) {
		con_log(CL_ANN1, (CE_NOTE, "Reset flag set, "
		"returning mfi_pkt and setting TRAN_BUSY\n"));
		return (DDI_FAILURE);
 ...
 */
static int
issue_mfi_smp(struct imraidsas_instance *instance,
    struct imraidsas_ioctl *ioctl,
    struct imraidsas_cmd *cmd, int mode)
{
	void		*request_ubuf;
	...
	ddi_acc_handle_t	acc_handle = cmd->frame_dma_obj.acc_handle;
	struct imraidsas_smp_frame		*ksmp;
	struct imraidsas_smp_frame		*smp;
	struct imraidsas_sge32		*sge32;
#ifndef _ILP32
	struct imraidsas_sge64		*sge64;
#endif
	int i;
	uint64_t			tmp_sas_addr;

	smp = &cmd->frame->smp;
	ksmp = (struct imraidsas_smp_frame *)&ioctl->frame[0];
	...
	if (request_xferlen) {
		...
		request_dma_obj.dma_attr = imraidsas_generic_dma_attr;
		...
		/* allocate kernel buffer for DMA */
		if (imraidsas_alloc_dma_obj(instance, &request_dma_obj,
		    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
	...
	if (response_xferlen) {
		...
		response_dma_obj.dma_attr = imraidsas_generic_dma_attr;
		...
		/* allocate kernel buffer for DMA */
		if (imraidsas_alloc_dma_obj(instance, &response_dma_obj,
		    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
			con_log(CL_ANN, (CE_WARN, "issue_mfi_smp: "
			    "could not allocate data transfer buffer."));
			return (DDI_FAILURE);
	...
	if (request_xferlen) {
		/* free kernel buffer */
		if (imraidsas_free_dma_obj(instance, request_dma_obj) !=
		    DDI_SUCCESS)
			return (DDI_FAILURE);
	}
	...
	if (response_xferlen) {
		/* free kernel buffer */
		if (imraidsas_free_dma_obj(instance, response_dma_obj) !=
		    DDI_SUCCESS)
			return (DDI_FAILURE);
	}
 ...
 */
static int
issue_mfi_stp(struct imraidsas_instance *instance,
    struct imraidsas_ioctl *ioctl,
    struct imraidsas_cmd *cmd, int mode)
{
	void		*fis_ubuf;
	...
	struct imraidsas_stp_frame	*kstp;
	struct imraidsas_stp_frame	*stp;
	ddi_acc_handle_t	acc_handle = cmd->frame_dma_obj.acc_handle;
	int i;

	stp = &cmd->frame->stp;
	kstp = (struct imraidsas_stp_frame *)&ioctl->frame[0];
	...
	if (fis_xferlen) {
		...
		fis_dma_obj.dma_attr = imraidsas_generic_dma_attr;
		...
		/* allocate kernel buffer for DMA */
		if (imraidsas_alloc_dma_obj(instance, &fis_dma_obj,
		    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
	...
	if (data_xferlen) {
		...
		data_dma_obj.dma_attr = imraidsas_generic_dma_attr;
		...
		/* allocate kernel buffer for DMA */
		if (imraidsas_alloc_dma_obj(instance, &data_dma_obj,
		    (uchar_t)DDI_STRUCTURE_LE_ACC) != 1) {
			con_log(CL_ANN, (CE_WARN, "issue_mfi_stp: "
			    "could not allocate data transfer buffer."));
			return (DDI_FAILURE);
 ...
 */
static void
fill_up_drv_ver(struct imraidsas_drv_ver *dv)
{
	(void) memset(dv, 0, sizeof (struct imraidsas_drv_ver));

	(void) memcpy(dv->signature, "$LSI LOGIC$", strlen("$LSI LOGIC$"));
	(void) memcpy(dv->os_name, "Solaris", strlen("Solaris"));
	(void) memcpy(dv->drv_name, "imraid_sas", strlen("imraid_sas"));
 ...
 */
static int
handle_drv_ioctl(struct imraidsas_instance *instance,
    struct imraidsas_ioctl *ioctl,
    int mode)
{
	int	i;
	...
	uint_t		model;
	struct imraidsas_dcmd_frame	*kdcmd;
	struct imraidsas_drv_ver	dv;
	struct imraidsas_pci_information pi;
	...
	case MRSAS_DRIVER_IOCTL_PCI_INFORMATION:
		...
		for (i = 0; i < (sizeof (struct imraidsas_pci_information) -
		    offsetof(struct imraidsas_pci_information, pciHeaderInfo));
		    i++) {
			pci_conf_buf[i] =
 ...
 */
static int
handle_mfi_ioctl(struct imraidsas_instance *instance,
    struct imraidsas_ioctl *ioctl,
    int mode)
{
	int	rval = DDI_SUCCESS;
	...
	hdr = (struct imraidsas_header *)&ioctl->frame[0];
 ...
 */
static int
handle_mfi_aen(struct imraidsas_instance *instance, struct imraidsas_aen *aen)
{
	int	rval = 0;
...
static int
register_mfi_aen(struct imraidsas_instance *instance, uint32_t seq_num,
    uint32_t class_locale_word)
{
	int	ret_val;

	struct imraidsas_cmd	*cmd, *aen_cmd;
	struct imraidsas_dcmd_frame	*dcmd;
	union imraidsas_evt_class_locale	curr_aen;
	union imraidsas_evt_class_locale	prev_aen;
...
{
	struct scsi_pkt *pkt;
	struct imraidsas_instance *instance = arg;
	struct imraidsas_cmd	*cmd = NULL;
	struct imraidsas_header	*hdr;
	int time = 0;
	int counter = 0;
	...
	if (imraidsas_initiate_ocr_if_fw_is_faulty(instance) ==  1) {
		con_log(CL_ANN1, (CE_NOTE,
		    "Fw Fault state Handling in io_timeout_checker"));
		if (instance->adapterresetinprogress == 0) {
			(void) imraidsas_reset_ppc(instance);
		}
		instance->timeout_id = timeout(io_timeout_checker,
		    (void *) instance, drv_usectohz(MRSAS_1_SECOND));
		return;
	...
	if (counter) {
		...
		if (instance->disable_online_ctrl_reset == 1) {
			con_log(CL_ANN1, (CE_NOTE, "imraidsas: "
			...
			if (cmd->retry_count_for_ocr <=  IO_RETRY_COUNT) {
				...
				con_log(CL_ANN1, (CE_NOTE, "imraidsas: "
				    ...
				    "triggering  imraidsas_reset_ppc"));
				(void) imraidsas_reset_ppc(instance);
				}
			} else {
				con_log(CL_ANN1, (CE_NOTE,
...
static int
read_fw_status_reg_ppc(struct imraidsas_instance *instance)
{
	return ((int)RD_OB_SCRATCH_PAD_0(instance));
}

static void
issue_cmd_ppc(struct imraidsas_cmd *cmd, struct imraidsas_instance *instance)
{
	struct scsi_pkt *pkt;
	atomic_add_16(&instance->fw_outstanding, 1);
	...
	mutex_enter(&instance->issue_cmd_mtx);
	WR_IB_HIGH_QPORT(0, instance);
	WR_IB_LOW_QPORT((cmd->frame_phys_addr) |
	    (((cmd->frame_count - 1) << 1) | 1), instance);
	mutex_exit(&instance->issue_cmd_mtx);
}
 ...
 */
static int
issue_cmd_in_sync_mode_ppc(struct imraidsas_instance *instance,
struct imraidsas_cmd *cmd)
{
	int	i;
 ...
 */
static int
issue_cmd_in_poll_mode_ppc(struct imraidsas_instance *instance,
    struct imraidsas_cmd *cmd)
{
	int		i;
	uint16_t	flags;
	uint32_t	msecs = MFI_POLL_TIMEOUT_SECS * MILLISEC;
	struct imraidsas_header *frame_hdr;

	con_log(CL_ANN1, (CE_NOTE, "issue_cmd_in_poll_mode_ppc: called"));

	frame_hdr = (struct imraidsas_header *)cmd->frame;
...
static void
enable_intr_ppc(struct imraidsas_instance *instance)
{
	uint32_t	mask;

	con_log(CL_ANN1, (CE_NOTE, "enable_intr_ppc: called"));

	WR_OB_INTR_MASK(~(MFI_REPLY_NOVA_MESSAGE_INTR), instance);
...
static void
disable_intr_ppc(struct imraidsas_instance *instance)
{
	uint32_t	mask;
...
static int
intr_ack_ppc(struct imraidsas_instance *instance)
{
	uint32_t	status;
	...
	if (!(status & MFI_REPLY_NOVA_MESSAGE_INTR)) {
		return (DDI_INTR_UNCLAIMED);
	}
...
static int
imraidsas_kill_adapter(struct imraidsas_instance *instance)
{
	if (instance->deadadapter == 1)
		return (DDI_FAILURE);

	con_log(CL_ANN1, (CE_NOTE, "imraidsas_kill_adapter: "
	...
	(void) imraidsas_complete_pending_cmds(instance);
	return (DDI_SUCCESS);
}


static int
imraidsas_reset_ppc(struct imraidsas_instance *instance)
{
	uint32_t status;
	uint32_t retry = 0;
	uint32_t cur_abs_reg_val;
	...
	if (instance->deadadapter == 1) {
		con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	...
	mutex_exit(&instance->ocr_flags_mtx);
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: adpterresetinprogress "
...
retry_reset:
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: magic number written "
	...
	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_reset_ppc: Adapter reset complete"));
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	...
	if (mfi_state_transition_to_ready(instance) ||
		...
		con_log(CL_ANN1, (CE_NOTE,
		    "imraidsas_reset_ppc :before fake: FW is not ready "
		    "FW state = 0x%x", fw_state));
		if (debug_fw_faults_after_ocr_g == 1)
			fw_state = MFI_STATE_FAULT;

		con_log(CL_ANN1, (CE_NOTE,
		    "imraidsas_reset_ppc : FW is not ready "
		    "FW state = 0x%x", fw_state));
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	    "Calling imraidsas_issue_init_mfi"));
	(void) imraidsas_issue_init_mfi(instance);
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	    "imraidsas_issue_init_mfi Done"));
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	    "Calling imraidsas_print_pending_cmd\n"));
	(void) imraidsas_print_pending_cmds(instance);
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	    "imraidsas_print_pending_cmd done\n"));
	instance->func_ptr->enable_intr(instance);
	instance->fw_outstanding = 0;
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	    "Calling imraidsas_issue_pending_cmds"));
	(void) imraidsas_issue_pending_cmds(instance);
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	"Complete"));
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc: "
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_reset_ppc done\n"));
	return (DDI_SUCCESS);
}
static int
imraidsas_common_check(struct imraidsas_instance *instance,
    struct  imraidsas_cmd *cmd)
{
	int ret = DDI_SUCCESS;
	...
	if (imraidsas_check_dma_handle(
	    instance->mfi_evt_detail_obj.dma_handle) != DDI_SUCCESS) {
		ddi_fm_service_impact(instance->dip, DDI_SERVICE_UNAFFECTED);
		if (cmd->pkt != NULL) {
			cmd->pkt->pkt_reason = CMD_TRAN_ERR;
			cmd->pkt->pkt_statistics = 0;
	...
	if (imraidsas_check_acc_handle(instance->regmap_handle) !=
	    DDI_SUCCESS) {
		ddi_fm_service_impact(instance->dip, DDI_SERVICE_UNAFFECTED);
...
/*ARGSUSED*/
static int
imraidsas_fm_error_cb(dev_info_t *dip, ddi_fm_error_t *err,
...
static void
imraidsas_fm_init(struct imraidsas_instance *instance)
{
	...
	if (instance->fm_capabilities) {
		...
		imraidsas_generic_dma_attr.dma_attr_flags = DDI_DMA_FLAGERR;
		...
		if (DDI_FM_ERRCB_CAP(instance->fm_capabilities)) {
			ddi_fm_handler_register(instance->dip,
			    imraidsas_fm_error_cb, (void*) instance);
		}
	} else {
		endian_attr.devacc_attr_access = DDI_DEFAULT_ACC;
		imraidsas_generic_dma_attr.dma_attr_flags = 0;
	}
}

static void
imraidsas_fm_fini(struct imraidsas_instance *instance)
{
...
int
imraidsas_check_acc_handle(ddi_acc_handle_t handle)
{
	ddi_fm_error_t de;
...
int
imraidsas_check_dma_handle(ddi_dma_handle_t handle)
{
	ddi_fm_error_t de;
...
void
imraidsas_fm_ereport(struct imraidsas_instance *instance, char *detail)
{
	uint64_t ena;
...
static int
imraidsas_add_intrs(struct imraidsas_instance *instance, int intr_type)
{
	...
	con_log(CL_DLEVEL1, (CE_WARN, "imraidsas_add_intrs: intr_type = %x",
	    intr_type));
	...
	con_log(CL_DLEVEL1, (CE_WARN,
	    "imraidsas_add_intrs: count = %d ", count));
	...
	con_log(CL_DLEVEL1, (CE_WARN,
	    "imraidsas_add_intrs: avail = %d ", avail));
	...
	if ((ret != DDI_SUCCESS) || (actual == 0)) {
		con_log(CL_ANN, (CE_WARN, "imraidsas_add_intrs: "
		    "avail = %d", avail));
	...
	con_log(CL_DLEVEL1, (CE_NOTE, "imraidsas_add_intrs: intr_pri = 0x%x ",
	    instance->intr_pri));

	/* Call ddi_intr_add_handler() */
	for (i = 0; i < actual; i++) {
		ret = ddi_intr_add_handler(instance->intr_htable[i],
		    (ddi_intr_handler_t *)imraidsas_isr, (caddr_t)instance,
		    (caddr_t)(uintptr_t)i);
...
static void
imraidsas_rem_intrs(struct imraidsas_instance *instance)
{
	int i;

	con_log(CL_ANN, (CE_NOTE, "imraidsas_rem_intrs called"));
...
static int
imraidsas_tran_bus_config(dev_info_t *parent, uint_t flags,
    ddi_bus_config_op_t op, void *arg, dev_info_t **childp)
{
	...
	if ((instance = ddi_get_soft_state(imraidsas_state,
	...
	case BUS_CONFIG_ONE: {
		...
		if (imraidsas_parse_devname(arg, &tgt, &lun) != 0) {
			rval = NDI_FAILURE;
			break;
		}

		if (lun == 0) {
			rval = imraidsas_config_ld(instance, tgt, lun, childp);
		} else {
			rval = imraidsas_config_pd(instance, tgt, lun, childp);
		}
	...
	case BUS_CONFIG_ALL: {
		...
		rval = imraidsas_config_all_devices(instance);
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_tran_bus_config: rval = %x",
	    rval));
	return (rval);
}

static int
imraidsas_config_all_devices(struct imraidsas_instance *instance)
{
	int rval, tgt;

	for (tgt = 0; tgt < MRDRV_MAX_LD; tgt++) {
		(void) imraidsas_config_ld(instance, tgt, 0, NULL);

	}

	/* Config PD devices connected to the card */
	for (tgt = 0; tgt < instance->mr_pd_max; tgt++) {
		(void) imraidsas_config_pd(instance, tgt, 1, NULL);
	}
...
static int
imraidsas_parse_devname(char *devnm, int *tgt, int *lun)
{
...
static int
imraidsas_config_ld(struct imraidsas_instance *instance, uint16_t tgt,
    uint8_t lun, dev_info_t **ldip)
{
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_config_ld: t = %d l = %d",
	    tgt, lun));

	if ((child = imraidsas_find_child(instance, tgt, lun)) != NULL) {
		if (ldip) {
			*ldip = child;
		}
		if (instance->mr_ld_list[tgt].flag != MRDRV_TGT_VALID) {
			rval = imraidsas_service_evt(instance, tgt, 0,
			    MRSAS_EVT_UNCONFIG_TGT, NULL);
			con_log(CL_ANN1, (CE_WARN,
			    "imraid_sas: DELETING STALE ENTRY rval = %d "
			    "tgt id = %d ", rval, tgt));
			return (NDI_FAILURE);
		}
		return (NDI_SUCCESS);
	...
	if (scsi_hba_probe(sd, NULL) == SCSIPROBE_EXISTS)
		rval = imraidsas_config_scsi_device(instance, sd, ldip);
	else
		rval = NDI_FAILURE;
	...
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_config_ld: return rval = %d",
	    rval));
	return (rval);
}

static int
imraidsas_config_pd(struct imraidsas_instance *instance, uint16_t tgt,
    uint8_t lun, dev_info_t **ldip)
{
	struct scsi_device *sd;
	dev_info_t *child;
	int rval, dtype;
	struct imraidsas_pd_info *pds = NULL;

	con_log(CL_ANN1, (CE_NOTE, "imraidsas_config_pd: t = %d l = %d",
	    tgt, lun));
	...
	if ((child = imraidsas_find_child(instance, tgt, lun)) != NULL) {
		...
		if (instance->mr_pd_list[tgt].flag != MRDRV_TGT_VALID) {
			rval = imraidsas_service_evt(instance, tgt, 1,
			    MRSAS_EVT_UNCONFIG_TGT, NULL);
			con_log(CL_ANN1, (CE_WARN,
			    "imraid_sas:DELETING STALE ENTRY  rval = %d "
			    "tgt id = %d ", rval, tgt));
	...
	pds = (struct imraidsas_pd_info *)
	    kmem_zalloc(sizeof (struct imraidsas_pd_info), KM_SLEEP);
	imraidsas_get_pd_info(instance, pds, tgt);
	...
	if ((dtype == DTYPE_DIRECT)) {
		if ((dtype == DTYPE_DIRECT) &&
			...
			kmem_free(pds, sizeof (struct imraidsas_pd_info));
			return (NDI_FAILURE);
		}
		sd = kmem_zalloc(sizeof (struct scsi_device), KM_SLEEP);
		sd->sd_address.a_hba_tran = instance->tran;
	...
	kmem_free(pds, sizeof (struct imraidsas_pd_info));
	con_log(CL_ANN1, (CE_NOTE, "imraidsas_config_pd: return rval = %d",
	    rval));
	return (rval);
}

static int
imraidsas_config_scsi_device(struct imraidsas_instance *instance,
    struct scsi_device *sd, dev_info_t **dipp)
{
	...
	con_log(CL_ANN1, (CE_WARN, "imraid_sas: scsi_device t%dL%d", tgt, lun));
	scsi_hba_nodename_compatible_get(sd->sd_inq, NULL, dtype,
	    NULL, &nodename, &compatible, &ncompatible);

	if (nodename == NULL) {
		con_log(CL_ANN1, (CE_WARN,
		    "imraid_sas: Found no compatible driver "
		    "for t%dL%d", tgt, lun));
		rval = NDI_FAILURE;
		goto finish;
	}
	...
	con_log(CL_ANN1, (CE_WARN,
	    "imraid_sas: Childname = %2s nodename = %s", childname, nodename));

	/* Create a dev node */
	rval = ndi_devi_alloc(instance->dip, childname, DEVI_SID_NODEID, &ldip);
	con_log(CL_ANN1, (CE_WARN,
	    "imraid_sas_config_scsi_device: ndi_devi_alloc rval = %x", rval));
	if (rval == NDI_SUCCESS) {
		if (ndi_prop_update_int(DDI_DEV_T_NONE, ldip, "target", tgt) !=
		    DDI_PROP_SUCCESS) {
			con_log(CL_ANN1, (CE_WARN,
			    "imraid_sas: unable to create "
		...
		if (rval != NDI_SUCCESS) {
			con_log(CL_ANN1, (CE_WARN,
			    "imraid_sas: unable to online t%dl%d",
			    tgt, lun));
			ndi_prop_remove_all(ldip);
			(void) ndi_devi_free(ldip);
		} else {
			con_log(CL_ANN1, (CE_WARN, "imraid_sas: online Done :"
			    "0 t%dl%d", tgt, lun));
		}
...
finish:
	...
	con_log(CL_DLEVEL1, (CE_WARN,
	    "imraid_sas: config_scsi_device rval = %d t%dL%d",
	    rval, tgt, lun));
...
/*ARGSUSED*/
static int
imraidsas_service_evt(struct imraidsas_instance *instance,
    int tgt, int lun, int event,
    uint64_t wwn)
{
	struct imraidsas_eventinfo *mrevt = NULL;

	con_log(CL_ANN1, (CE_NOTE,
	    "imraidsas_service_evt called for t%dl%d event = %d",
	    tgt, lun, event));

	if ((instance->taskq == NULL) || (mrevt =
	    kmem_zalloc(sizeof (struct imraidsas_eventinfo),
	    KM_NOSLEEP)) == NULL) {
		return (ENOMEM);
	}
	...
	if ((ddi_taskq_dispatch(instance->taskq,
	    (void (*)(void *))imraidsas_issue_evt_taskq, mrevt, DDI_NOSLEEP)) !=
	    DDI_SUCCESS) {
		con_log(CL_ANN1, (CE_NOTE,
		    "imraid_sas: Event task failed for t%dl%d event = %d",
		    tgt, lun, event));
		kmem_free(mrevt, sizeof (struct imraidsas_eventinfo));
		return (DDI_FAILURE);
	}
...
static void
imraidsas_issue_evt_taskq(struct imraidsas_eventinfo *mrevt)
{
	struct imraidsas_instance *instance = mrevt->instance;
	dev_info_t *dip, *pdip;
	int circ1 = 0;
	char *devname;

	con_log(CL_ANN1, (CE_NOTE, "imraidsas_issue_evt_taskq: called for"
	    " tgt %d lun %d event %d",
	...
	if (mrevt->tgt < MRDRV_MAX_LD && mrevt->lun == 0) {
		...
		mutex_enter(&instance->config_dev_mtx);
		dip = instance->mr_pd_list[mrevt->tgt].dip;
		mutex_exit(&instance->config_dev_mtx);
	}
	...
	case MRSAS_EVT_CONFIG_TGT:
		if (dip == NULL) {
			...
			if (mrevt->lun == 0) {
				(void) imraidsas_config_ld(instance, mrevt->tgt,
				    0, NULL);
			} else {
				(void) imraidsas_config_pd(instance, mrevt->tgt,
				    1, NULL);
			}
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: EVT_CONFIG_TGT called:"
			    " for tgt %d lun %d event %d",
			    mrevt->tgt, mrevt->lun, mrevt->event));

		} else {
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: EVT_CONFIG_TGT dip != NULL:"
			    " for tgt %d lun %d event %d",
			    mrevt->tgt, mrevt->lun, mrevt->event));
		}
		break;
	case MRSAS_EVT_UNCONFIG_TGT:
		if (dip) {
			...
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: EVT_UNCONFIG_TGT called:"
			    " for tgt %d lun %d event %d",
			    mrevt->tgt, mrevt->lun, mrevt->event));
		} else {
			con_log(CL_ANN1, (CE_NOTE,
			    "imraid_sas: EVT_UNCONFIG_TGT dip == NULL:"
			    " for tgt %d lun %d event %d",
			    mrevt->tgt, mrevt->lun, mrevt->event));
		}
		break;
...
static void
imraidsas_get_pd_info(struct imraidsas_instance *instance,
    struct imraidsas_pd_info *pds,
    int tgt)
{
	struct imraidsas_cmd	*cmd;
	struct imraidsas_dcmd_frame	*dcmd;
	...
	dcmd = &cmd->frame->dcmd;
	dcmd_dma_obj.size = sizeof (struct imraidsas_pd_info);
	dcmd_dma_obj.dma_attr = imraidsas_generic_dma_attr;
	...
	(void) imraidsas_alloc_dma_obj(instance, &dcmd_dma_obj,
	    DDI_STRUCTURE_LE_ACC);
	(void) memset(dcmd_dma_obj.buffer,
	    0,
	    sizeof (struct imraidsas_pd_info));
	...
	ddi_rep_get8(cmd->frame_dma_obj.acc_handle, (uint8_t *)pds,
	    (uint8_t *)dcmd_dma_obj.buffer, sizeof (struct imraidsas_pd_info),
	    DDI_DEV_AUTOINCR);
	(void) imraidsas_free_dma_obj(instance, dcmd_dma_obj);
	return_mfi_pkt(instance, cmd);
	(void) imraidsas_common_check(instance, cmd);
}
...
static int
imraidsas_mode_sense_build(struct scsi_pkt *pkt)
{
	union scsi_cdb		*cdbp;
	uint16_t 		page_code;
	struct scsa_cmd		*acmd;

...
/*
 * imraid_sas.h: header for imraid_sas
 *
...
#ifndef	_IMRAID_SAS_H_
#define	_IMRAID_SAS_H_
...
#include "imraid_sas_list.h"

/*
 * MegaRAID SAS2.0 Driver meta data
 */
#define	MRSAS_VERSION				"03.03.00"
#define	MRSAS_RELDATE				"Dec 27, 2010"
...
/*
 * Add Device ID's for Skinny Controllers.
 */

#define	PCI_DEVICE_ID_LSI_DRAKE_4PORT		0x0073
#define	PCI_DEVICE_ID_LSI_DRAKE_8PORT		0x0073
...
#define	MR_DCMD_CTRL_MFC_DEFAULTS_GET		0x010e0201
...
typedef struct imraidsas_instance {
	uint32_t	*producer;
	uint32_t	*consumer;
	...
	struct imraidsas_func_ptr *func_ptr;
	...
	ddi_taskq_t	*taskq;
	struct imraidsas_ld	*mr_ld_list;
	struct imraidsas_pd *mr_pd_list;
	uint32_t	mr_pd_max;
	kmutex_t	config_dev_mtx;
	kmutex_t	ocr_flags_mtx;
} imraidsas_t;

struct imraidsas_func_ptr {
	int (*read_fw_status_reg)(struct imraidsas_instance *);
	void (*issue_cmd)(struct imraidsas_cmd *, struct imraidsas_instance *);
	int (*issue_cmd_in_sync_mode)(struct imraidsas_instance *,
	    struct imraidsas_cmd *);
	int (*issue_cmd_in_poll_mode)(struct imraidsas_instance *,
	    struct imraidsas_cmd *);
	void (*enable_intr)(struct imraidsas_instance *);
	void (*disable_intr)(struct imraidsas_instance *);
	int (*intr_ack)(struct imraidsas_instance *);
};
...
#define	TRAN2MR(tran)	(struct imraidsas_instance *)(tran)->tran_hba_private)
...
#define	SCP2ADAPTER(scp)				\
	(struct imraidsas_instance *)SCSIHOST2ADAP(SCP2HOST(scp))
...
struct scsa_cmd {
	...
	uchar_t			islogical;
	uchar_t			lun;
	struct imraidsas_device	*imraidsas_dev;
};


struct imraidsas_cmd {
	union imraidsas_frame	*frame;
...
struct imraidsas_mgmt_info {

	uint16_t			count;
	struct imraidsas_instance		*instance[MAX_MGMT_ADAPTERS];
...
struct imraidsas_ctrl_info {
	...
	struct imraidsas_ctrl_prop		properties;
...
#define	IB_NOVA_DOORBELL_OFF		0x00	/* Nova */
...
#define	WR_IB_WRITE_SEQ(v, instance) 	ddi_put32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap + 0x004), (v))

#define	RD_OB_DRWE(instance) 		ddi_get32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap + 0x008))

#define	WR_IB_DRWE(v, instance) 	ddi_put32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap + 0x008), (v))

/*
 * All MFI register set macros accept imraidsas_register_set*
 */
...
#define	WR_IB_DOORBELL(v, instance)	ddi_put32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap +			     \
	    IB_NOVA_DOORBELL_OFF), (v))

#define	RD_IB_DOORBELL(instance)	ddi_get32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap +			     \
	IB_NOVA_DOORBELL_OFF))
...
#define	WR_IB_LOW_QPORT(v, instance)    ddi_put32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap + 0xC0), (v))

#define	WR_IB_HIGH_QPORT(v, instance)   ddi_put32((instance)->regmap_handle, \
	(uint32_t *)((uintptr_t)(instance)->regmap + 0xC4), (v))
...
/*
 * Skinny specific:
 * The interrupt notification bit is 0
 * of the controller. This is same as 2108
 * controller. But handled as separate case.
 */
#define	MFI_REPLY_NOVA_MESSAGE_INTR		0x00000001
...
struct imraidsas_sge32 {
	uint32_t	phys_addr;
	uint32_t	length;
};

struct imraidsas_sge64 {
	uint64_t	phys_addr;
	uint32_t	length;
};

struct imraidsas_sge_ieee {
	uint64_t 	phys_addr;
	uint32_t	length;
	uint32_t	flag;
};
...
union imraidsas_sgl {
	struct imraidsas_sge32	sge32[1];
	struct imraidsas_sge64	sge64[1];
	struct imraidsas_sge_ieee	sge_ieee[1];
};

struct imraidsas_header {
	uint8_t		cmd;				/* 00h */
...
union imraidsas_sgl_frame {
	struct imraidsas_sge32	sge32[8];
	struct imraidsas_sge64	sge64[5];
};

struct imraidsas_init_frame {
	uint8_t		cmd;				/* 00h */
	...
	uint32_t	reserved_4[4];			/* 28h */
};

struct imraidsas_init_queue_info {
...
struct imraidsas_io_frame {
	...
	union imraidsas_sgl	sgl;			/* 28h */
};

struct imraidsas_pthru_frame {
	uint8_t			cmd;			/* 00h */
	uint8_t			sense_len;		/* 01h */
	uint8_t			cmd_status;		/* 02h */
	uint8_t			scsi_status;		/* 03h */
	...
	uint8_t			cdb[16];		/* 20h */
	union imraidsas_sgl	sgl;			/* 30h */
};

struct imraidsas_dcmd_frame {
	uint8_t			cmd;			/* 00h */
	uint8_t			reserved_0;		/* 01h */
	uint8_t			cmd_status;		/* 02h */
...
struct imraidsas_smp_frame {
	...
	union imraidsas_sgl	sgl[2];			/* 28h */
};

struct imraidsas_stp_frame {
	uint8_t		cmd;				/* 00h */
	uint8_t		reserved_1;			/* 01h */
	uint8_t		cmd_status;			/* 02h */
	uint8_t		connection_status;		/* 03h */
	...
	union imraidsas_sgl	sgl;			/* 40 */
};

union imraidsas_frame {
	struct imraidsas_header		hdr;
	struct imraidsas_init_frame	init;
	struct imraidsas_io_frame	io;
	struct imraidsas_pthru_frame	pthru;
	struct imraidsas_dcmd_frame	dcmd;
	struct imraidsas_abort_frame	abort;
...
typedef struct imraidsas_pd_address {
	uint16_t	device_id;
	uint16_t	encl_id;
	...
	uint64_t		sas_addr[2];
} imraidsas_pd_address_t;

union imraidsas_evt_class_locale {
	struct {
		uint16_t	locale;
		uint8_t		reserved;
		int8_t		class;
...
struct imraidsas_progress {
	uint16_t	progress;
	uint16_t	elapsed_seconds;
};

struct imraidsas_evtarg_ld {
	uint16_t	target_id;
	uint8_t		ld_index;
	uint8_t		reserved;
};

struct imraidsas_evtarg_pd {
	uint16_t	device_id;
	uint8_t		encl_index;
	uint8_t		slot_number;
};

struct imraidsas_evt_detail {
	uint32_t	seq_num;
	uint32_t	time_stamp;
	uint32_t	code;
	union imraidsas_evt_class_locale	cl;
	uint8_t		arg_type;
	uint8_t		reserved1[15];

	union {
		struct {
			struct imraidsas_evtarg_pd	pd;
			uint8_t			cdb_length;
		...
		struct {
			struct imraidsas_evtarg_ld	ld;
			struct imraidsas_progress	prog;
		} ld_prog;
...
struct imraidsas_pci_common_header {
...
struct imraidsas_pci_link_capability {
	union {
	    struct {
...
struct imraidsas_pci_link_status_capability {
	union {
	    struct {
...
struct imraidsas_pci_capabilities {
	struct imraidsas_pci_link_capability	linkCapability;
	struct imraidsas_pci_link_status_capability linkStatusCapability;
};

struct imraidsas_pci_information
{
	uint32_t		busNumber;
	...
	uint8_t			reserved;
	struct imraidsas_pci_common_header pciHeaderInfo;
	struct imraidsas_pci_capabilities capability;
	uint8_t			reserved2[32];
};

struct imraidsas_ioctl {
	uint16_t	version;
	...
	uint8_t		frame[64];
	union imraidsas_sgl_frame sgl_frame;
	uint8_t		sense_buff[MRSAS_MAX_SENSE_LENGTH];
	uint8_t		data[1];
};

struct imraidsas_aen {
	uint16_t	host_no;
	uint16_t	cmd_status;
	uint32_t	seq_num;
...
static int	imraidsas_getinfo(dev_info_t *, ddi_info_cmd_t,
		    void *, void **);
static int	imraidsas_attach(dev_info_t *, ddi_attach_cmd_t);

static int	imraidsas_quiesce(dev_info_t *);

static int	imraidsas_detach(dev_info_t *, ddi_detach_cmd_t);
static int	imraidsas_open(dev_t *, int, int, cred_t *);
static int	imraidsas_close(dev_t, int, int, cred_t *);
static int	imraidsas_ioctl(dev_t, int, intptr_t, int, cred_t *, int *);

static int	imraidsas_tran_tgt_init(dev_info_t *, dev_info_t *,
		    scsi_hba_tran_t *, struct scsi_device *);
static struct scsi_pkt *imraidsas_tran_init_pkt(struct scsi_address *, register
		    struct scsi_pkt *, struct buf *, int, int, int, int,
		    int (*)(), caddr_t);
static int	imraidsas_tran_start(struct scsi_address *,
		    register struct scsi_pkt *);
static int	imraidsas_tran_abort(struct scsi_address *, struct scsi_pkt *);
static int	imraidsas_tran_reset(struct scsi_address *, int);
static int	imraidsas_tran_bus_reset(dev_info_t *, int);
static int	imraidsas_tran_getcap(struct scsi_address *, char *, int);
static int	imraidsas_tran_setcap(struct scsi_address *, char *, int, int);
static void	imraidsas_tran_destroy_pkt(struct scsi_address *,
		    struct scsi_pkt *);
static void	imraidsas_tran_dmafree(struct scsi_address *,
		    struct scsi_pkt *);
static void	imraidsas_tran_sync_pkt(struct scsi_address *,
		    struct scsi_pkt *);
static uint_t	imraidsas_isr();
static uint_t	imraidsas_softintr();

static int	init_mfi(struct imraidsas_instance *);
static int	imraidsas_free_dma_obj(struct imraidsas_instance *, dma_obj_t);
static int	imraidsas_alloc_dma_obj(struct imraidsas_instance *,
		    dma_obj_t *,
		    uchar_t);
static struct imraidsas_cmd *get_mfi_pkt(struct imraidsas_instance *);
static void	return_mfi_pkt(struct imraidsas_instance *,
		    struct imraidsas_cmd *);

static void	free_space_for_mfi(struct imraidsas_instance *);
static void	free_additional_dma_buffer(struct imraidsas_instance *);
static int	alloc_additional_dma_buffer(struct imraidsas_instance *);
static int	read_fw_status_reg_ppc(struct imraidsas_instance *);
static void	issue_cmd_ppc(struct imraidsas_cmd *,
		    struct imraidsas_instance *);
static int	issue_cmd_in_poll_mode_ppc(struct imraidsas_instance *,
		    struct imraidsas_cmd *);
static int	issue_cmd_in_sync_mode_ppc(struct imraidsas_instance *,
		    struct imraidsas_cmd *);
static void	enable_intr_ppc(struct imraidsas_instance *);
static void	disable_intr_ppc(struct imraidsas_instance *);
static int	intr_ack_ppc(struct imraidsas_instance *);
static int	mfi_state_transition_to_ready(struct imraidsas_instance *);
static void	destroy_mfi_frame_pool(struct imraidsas_instance *);
static int	create_mfi_frame_pool(struct imraidsas_instance *);
static int	imraidsas_dma_alloc(struct imraidsas_instance *,
		    struct scsi_pkt *,
		    struct buf *, int, int (*)());
static int	imraidsas_dma_move(struct imraidsas_instance *,
			struct scsi_pkt *, struct buf *);
static void	flush_cache(struct imraidsas_instance *instance);
static void	display_scsi_inquiry(caddr_t);
static int	start_mfi_aen(struct imraidsas_instance *instance);
static int	handle_drv_ioctl(struct imraidsas_instance *instance,
		    struct imraidsas_ioctl *ioctl, int mode);
static int	handle_mfi_ioctl(struct imraidsas_instance *instance,
		    struct imraidsas_ioctl *ioctl, int mode);
static int	handle_mfi_aen(struct imraidsas_instance *instance,
		    struct imraidsas_aen *aen);
static void	fill_up_drv_ver(struct imraidsas_drv_ver *dv);
static struct imraidsas_cmd *build_cmd(struct imraidsas_instance *instance,
		    struct scsi_address *ap, struct scsi_pkt *pkt,
		    uchar_t *cmd_done);
static int	wait_for_outstanding(struct imraidsas_instance *instance);
static int	register_mfi_aen(struct imraidsas_instance *instance,
		    uint32_t seq_num, uint32_t class_locale_word);
static int	issue_mfi_pthru(struct imraidsas_instance *instance, struct
		    imraidsas_ioctl *ioctl, struct imraidsas_cmd *cmd,
		    int mode);
static int	issue_mfi_dcmd(struct imraidsas_instance *instance, struct
		    imraidsas_ioctl *ioctl, struct imraidsas_cmd *cmd,
		    int mode);
static int	issue_mfi_smp(struct imraidsas_instance *instance, struct
		    imraidsas_ioctl *ioctl, struct imraidsas_cmd *cmd,
		    int mode);
static int	issue_mfi_stp(struct imraidsas_instance *instance, struct
		    imraidsas_ioctl *ioctl,
		    struct imraidsas_cmd *cmd,
		    int mode);
static int	abort_aen_cmd(struct imraidsas_instance *instance,
		    struct imraidsas_cmd *cmd_to_abort);

static int	imraidsas_common_check(struct imraidsas_instance *instance,
		    struct  imraidsas_cmd *cmd);
static void	imraidsas_fm_init(struct imraidsas_instance *instance);
static void	imraidsas_fm_fini(struct imraidsas_instance *instance);
static int	imraidsas_fm_error_cb(dev_info_t *, ddi_fm_error_t *,
		    const void *);
static void	imraidsas_fm_ereport(struct imraidsas_instance *instance,
		    char *detail);
static int	imraidsas_check_dma_handle(ddi_dma_handle_t handle);
static int	imraidsas_check_acc_handle(ddi_acc_handle_t handle);

static void	imraidsas_rem_intrs(struct imraidsas_instance *instance);
static int	imraidsas_add_intrs(struct imraidsas_instance *instance,
    int intr_type);

static void	imraidsas_tran_tgt_free(dev_info_t *, dev_info_t *,
		    scsi_hba_tran_t *, struct scsi_device *);
static int	imraidsas_tran_bus_config(dev_info_t *, uint_t,
		    ddi_bus_config_op_t, void *, dev_info_t **);
static int	imraidsas_parse_devname(char *, int *, int *);
static int	imraidsas_config_all_devices(struct imraidsas_instance *);
static int 	imraidsas_config_scsi_device(struct imraidsas_instance *,
		    struct scsi_device *, dev_info_t **);
static int 	imraidsas_config_ld(struct imraidsas_instance *, uint16_t,
			uint8_t, dev_info_t **);
static int 	imraidsas_config_pd(struct imraidsas_instance *, uint16_t,
			uint8_t, dev_info_t **);
static dev_info_t *imraidsas_find_child(struct imraidsas_instance *, uint16_t,
			uint8_t);
static int	imraidsas_name_node(dev_info_t *, char *, int);
static void	imraidsas_issue_evt_taskq(struct imraidsas_eventinfo *);
static int	imraidsas_service_evt(struct imraidsas_instance *, int, int,
		    int, uint64_t);
static void	imraidsas_get_pd_info(struct imraidsas_instance *,
			struct imraidsas_pd_info *, int);

static int	imraidsas_mode_sense_build(struct scsi_pkt *);
static void	push_pending_mfi_pkt(struct imraidsas_instance *,
			struct imraidsas_cmd *);
static int 	imraidsas_issue_init_mfi(struct imraidsas_instance *);
static int 	imraidsas_issue_pending_cmds(struct imraidsas_instance *);
static int 	imraidsas_print_pending_cmds(struct imraidsas_instance *);
static int  	imraidsas_complete_pending_cmds(struct imraidsas_instance *);
static int	imraidsas_reset_ppc(struct imraidsas_instance *);
static uint32_t imraidsas_initiate_ocr_if_fw_is_faulty(
		    struct imraidsas_instance *);
static int	imraidsas_kill_adapter(struct imraidsas_instance *);
static void 	io_timeout_checker(void *instance);
static void 	complete_cmd_in_sync_mode(struct imraidsas_instance *,
		struct imraidsas_cmd *);

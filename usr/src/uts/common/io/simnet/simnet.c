...
simnet_init_wifi(simnet_dev_t *sdev, mac_register_t *mac)
{
	...
	mac->m_min_sdu = IEEE80211_MTU_MIN;
...
simnet_init_mac(simnet_dev_t *sdev)
{
	...
	if (sdev->sd_type == DL_ETHER) {
		sdev->sd_min_mtu_limit = ETHERMIN;
		sdev->sd_max_mtu_limit = SIMNET_ETHER_MAX_MTU;
		err = simnet_init_ether(sdev, mac);
	} else if (sdev->sd_type == DL_WIFI) {
		sdev->sd_min_mtu_limit = IEEE80211_MTU_MIN;
		sdev->sd_max_mtu_limit = IEEE80211_MTU_MAX;
		err = simnet_init_wifi(sdev, mac);
	} else {
		err = EINVAL;
	}

	sdev->sd_min_mtu_propval = sdev->sd_min_mtu_limit;
	sdev->sd_max_mtu_propval = sdev->sd_max_mtu_limit;
...
simnet_m_tx(void *arg, mblk_t *mp_chain)
{
	...
	while ((mp = mpnext) != NULL) {
		...
		if ((mp = mac_fix_cksum(mp, B_FALSE, 0)) == NULL) {
			sdev->sd_stats.xmit_errors++;
			continue;
		}
...
{
	simnet_wifidev_t *wdev = sdev->sd_wifidev;
	uint32_t min_mtu, max_mtu, mtu;
	long result;
	unsigned long uresult;

	if (sdev->sd_type == DL_WIFI && strcmp(pr_name, "_wl_esslist") == 0) {
		if (pr_val == NULL)
			return (EINVAL);
		return (set_wl_esslist_priv_prop(wdev, pr_valsize, pr_val));
	} else if (sdev->sd_type == DL_WIFI &&
	    strcmp(pr_name, "_wl_connected") == 0) {
		if (pr_val == NULL)
			return (EINVAL);
		...
		return (0);
	} else if (strcmp(pr_name, "_min_mtu") == 0) {
		if (pr_val == NULL)
			return (EINVAL);
		(void) ddi_strtoul((char *)pr_val, NULL, 10, &uresult);
		min_mtu = (uint32_t)uresult;
		mac_sdu_get(sdev->sd_mh, NULL, &mtu);
		if (min_mtu < sdev->sd_min_mtu_limit || min_mtu > mtu)
			return (EINVAL);
		sdev->sd_min_mtu_propval = min_mtu;
		return (0);
	} else if (strcmp(pr_name, "_max_mtu") == 0) {
		if (pr_val == NULL)
			return (EINVAL);
		(void) ddi_strtoul((char *)pr_val, NULL, 10, &uresult);
		max_mtu = (uint32_t)uresult;
		mac_sdu_get(sdev->sd_mh, NULL, &mtu);
		if (max_mtu > sdev->sd_max_mtu_limit || max_mtu < mtu)
			return (EINVAL);
		sdev->sd_max_mtu_propval = max_mtu;
		return (0);
	}
...
static int
simnet_m_setprop(void *arg, const char *pr_name, mac_prop_id_t pr_num,
    uint_t pr_length, const void *pr_buf)
{
	simnet_dev_t *sdev = arg;
	simnet_wifidev_t *wdev = sdev->sd_wifidev;
	int err = 0;
	...
	switch (pr_num) {
	case MAC_PROP_MTU:
		(void) memcpy(&mtu, pr_buf, sizeof (mtu));
		if (mtu < sdev->sd_min_mtu_propval ||
		    mtu > sdev->sd_max_mtu_propval)
			return (EINVAL);
		return (mac_maxsdu_update(sdev->sd_mh, mtu));

	case MAC_PROP_PRIVATE:
		return (simnet_set_priv_prop(sdev, pr_name, pr_length,
		    pr_buf));
	default:
		break;
	}
	...
	case MAC_PROP_WL_ESSID: {
		...
		(void) memcpy(&wdev->swd_essid, pr_buf,
		    sizeof (wl_essid_t));
	...
	case MAC_PROP_WL_BSSID: {
		(void) memcpy(&wdev->swd_bssid, pr_buf,
		    sizeof (wl_bssid_t));
		break;
	}
    ...
    uint_t pr_valsize, void *pr_val)
{
	...
	if (sdev->sd_type == DL_WIFI && strcmp(pr_name, "_wl_esslist") == 0) {
		...
		value = sdev->sd_min_mtu_propval;
	} else if (strcmp(pr_name, "_max_mtu") == 0) {
		value = sdev->sd_max_mtu_propval;
	} else {
		err = ENOTSUP;
	}
...
static int
simnet_m_getprop(void *arg, const char *pr_name, mac_prop_id_t pr_num,
    uint_t pr_length, void *pr_buf)
{
	simnet_dev_t *sdev = arg;
	simnet_wifidev_t *wdev = sdev->sd_wifidev;
	int err = 0;
	...
	if (pr_num == MAC_PROP_PRIVATE) {
		return (simnet_get_priv_prop(sdev, pr_name, pr_length,
		    pr_buf));
	}
	...
	switch (pr_num) {
	case MAC_PROP_WL_ESSID:
		(void) memcpy(pr_buf, &wdev->swd_essid,
		    sizeof (wl_essid_t));
		break;
	case MAC_PROP_WL_BSSID:
		(void) memcpy(pr_buf, &wdev->swd_bssid,
		    sizeof (wl_bssid_t));
		break;
	case MAC_PROP_WL_PHY_CONFIG:
	case MAC_PROP_WL_AUTH_MODE:
	...
	case MAC_PROP_WL_LINKSTATUS:
		(void) memcpy(pr_buf, &wdev->swd_linkstatus,
		    sizeof (wdev->swd_linkstatus));
		break;
	case MAC_PROP_WL_ESS_LIST: {
		wl_ess_conf_t *w_ess_conf;

		((wl_ess_list_t *)pr_buf)->wl_ess_list_num =
		    wdev->swd_esslist_num;
		/* LINTED E_BAD_PTR_CAST_ALIGN */
		w_ess_conf = (wl_ess_conf_t *)((char *)pr_buf +
	...
	case MAC_PROP_WL_RSSI:
		*(wl_rssi_t *)pr_buf = wdev->swd_rssi;
		break;
	case MAC_PROP_WL_RADIO:
		*(wl_radio_t *)pr_buf = B_TRUE;
		break;
...
static void
simnet_priv_propinfo(simnet_dev_t *sdev, const char *pr_name,
    mac_prop_info_handle_t prh)
{
	...
	if (strcmp(pr_name, "_wl_esslist") == 0) {
		...
		mac_prop_info_set_default_uint32(prh, sdev->sd_min_mtu_limit);
		mac_prop_info_set_range_uint32(prh, sdev->sd_min_mtu_limit,
		    sdev->sd_max_mtu_limit);
	} else if (strcmp(pr_name, "_max_mtu") == 0) {
		mac_prop_info_set_default_uint32(prh, sdev->sd_max_mtu_limit);
		mac_prop_info_set_range_uint32(prh, sdev->sd_min_mtu_limit,
		    sdev->sd_max_mtu_limit);
	}
}
...
static void
simnet_m_propinfo(void *arg, const char *pr_name, mac_prop_id_t pr_num,
    mac_prop_info_handle_t prh)
{
	simnet_dev_t *sdev = arg;
	...
	switch (pr_num) {
	case MAC_PROP_PRIVATE:
		simnet_priv_propinfo(sdev, pr_name, prh);
		return;
	case MAC_PROP_MTU:
		mac_prop_info_set_range_uint32(prh, sdev->sd_min_mtu_propval,
		    sdev->sd_max_mtu_propval);
		return;
	}

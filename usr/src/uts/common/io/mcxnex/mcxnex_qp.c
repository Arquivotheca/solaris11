...
/*
 * mcxnex_qp.c
 *    Mcxnex Queue Pair Processing Routines
 *
 *    Implements all the routines necessary for allocating, freeing, and
 *    querying the Mcxnex queue pairs.
 */
...
static int mcxnex_qp_create_qpn(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    mcxnex_rsrc_t *qpc);
static int mcxnex_qpn_avl_compare(const void *q, const void *e);
static int mcxnex_special_qp_rsrc_alloc(mcxnex_state_t *state,
    ibt_sqp_type_t type, uint_t port, mcxnex_rsrc_t **qp_rsrc);
static int mcxnex_special_qp_rsrc_free(mcxnex_state_t *state,
    ibt_sqp_type_t type, uint_t port);
static void mcxnex_qp_sgl_to_logwqesz(mcxnex_state_t *state, uint_t num_sgl,
    uint_t real_max_sgl, mcxnex_qp_wq_type_t wq_type,
 ...
 */
int
mcxnex_qp_alloc(mcxnex_state_t *state, mcxnex_qp_info_t *qpinfo,
    uint_t sleepflag)
{
	mcxnex_rsrc_t			*qpc, *rsrc;
	mcxnex_umap_db_entry_t		*umapdb;
	mcxnex_qphdl_t			qp;
	ibt_qp_alloc_attr_t		*attr_p;
	ibt_qp_type_t			type;
	mcxnex_qp_wq_type_t		swq_type;
	ibtl_qp_hdl_t			ibt_qphdl;
	ibt_chan_sizes_t		*queuesz_p;
	ib_qpn_t			*qpn;
	mcxnex_qphdl_t			*qphdl;
	ibt_mr_attr_t			mr_attr;
	mcxnex_mr_options_t		mr_op;
	mcxnex_srqhdl_t			srq;
	mcxnex_pdhdl_t			pd;
	mcxnex_cqhdl_t			sq_cq, rq_cq;
	mcxnex_mrhdl_t			mr;
	...
	/*
	 * Extract the necessary info from the mcxnex_qp_info_t structure
	 */
	...
	if (qp_srq_en) {
		...
		srq = (mcxnex_srqhdl_t)attr_p->qp_ibc_srq_hdl;
	}
	...
	if ((type != IBT_UD_RQP) && (type != IBT_RC_RQP) &&
	    (type != IBT_UC_RQP) && (type != IBT_RAWETHER_RQP)) {
	...
	pd = (mcxnex_pdhdl_t)attr_p->qp_pd_hdl;
	...
	sq_cq = (mcxnex_cqhdl_t)attr_p->qp_ibc_scq_hdl;
	rq_cq = (mcxnex_cqhdl_t)attr_p->qp_ibc_rcq_hdl;
	 ...
	 */
	status = mcxnex_cq_refcnt_inc(sq_cq, MCXNEX_CQ_IS_NORMAL);
	if (status != DDI_SUCCESS) {
		status = IBT_CQ_HDL_INVALID;
		goto qpalloc_fail1;
	}
	status = mcxnex_cq_refcnt_inc(rq_cq, MCXNEX_CQ_IS_NORMAL);
	if (status != DDI_SUCCESS) {
		status = IBT_CQ_HDL_INVALID;
		goto qpalloc_fail2;
	}
	 ...
	 * other Mcxnex hardware resources, ownership is not immediately
	 ...
	 */
	status = mcxnex_rsrc_alloc(state, MCXNEX_QPC, 1, sleepflag, &qpc);
	if (status != DDI_SUCCESS) {
	 ...
	 * (i.e. the Mcxnex Queue Pair handle).  If we fail here, we must
	 * undo the reference counts and the previous resource allocation.
	 */
	status = mcxnex_rsrc_alloc(state, MCXNEX_QPHDL, 1, sleepflag, &rsrc);
	if (status != DDI_SUCCESS) {
		status = IBT_INSUFF_RESOURCE;
		goto qpalloc_fail4;
	}
	qp = (mcxnex_qphdl_t)rsrc->hr_addr;
	bzero(qp, sizeof (struct mcxnex_sw_qp_s));
	 ...
	 */
	status = mcxnex_qp_create_qpn(state, qp, qpc);
	if (status != DDI_SUCCESS) {
		status = IBT_INSUFF_RESOURCE;
		goto qpalloc_fail5;
	}
	 ...
	 */
	if (qp_is_umap) {
		umapdb = mcxnex_umap_db_alloc(state->hs_instance, qp->qp_qpnum,
	...
	/*
	 * Allocate the doorbell record.  Mcxnex just needs one for the RQ,
	 * if the QP is not associated with an SRQ, and use uarpg (above) as
	 * the uar index
	 */

	if (!qp_srq_en) {
		status = mcxnex_dbr_alloc(state, uarpg, &qp->qp_rq_dbr_acchdl,
	...
	if (attr_p->qp_alloc_flags & IBT_QP_VLAN_STRIP_OFF) {
		if (state->hs_vlan_strip_off_cap)
			qp->qp_vlan_strip_off = 1;
		else {
			status = IBT_INVALID_PARAM;
			goto qpalloc_fail6;
		}
	...
	if (attr_p->qp_alloc_flags & IBT_QP_LINK_TYPE_ETH)
		qp->qp_link_type_eth = 1;
	 ...
	 */
	if (type == IBT_UD_RQP || type == IBT_RAWETHER_RQP) {
		max_sgl = state->hs_ibtfinfo.hca_attr->hca_ud_send_sgl_sz;
		swq_type = MCXNEX_QP_WQ_TYPE_SENDQ_UD;
	} else {
		max_sgl = state->hs_ibtfinfo.hca_attr->hca_conn_send_sgl_sz;
		swq_type = MCXNEX_QP_WQ_TYPE_SENDQ_CONN;
	}
	...
	if (qp_srq_en) {
		...
		mcxnex_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_rq_sgl,
		    max_recv_sgl, MCXNEX_QP_WQ_TYPE_RECVQ,
		    &qp->qp_rq_log_wqesz, &qp->qp_rq_sgl);
	}
	mcxnex_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_sq_sgl,
	...
	if (qp->qp_no_prefetch)
		...
		qp->qp_sq_headroom = sq_wqe_size + MCXNEX_QP_OH_SIZE;
	/*
	 * hdrm wqes must be integral since both sq_wqe_size &
	 * MCXNEX_QP_OH_SIZE are power of 2
	 */
	 ...
	 * Note:  All Mcxnex QP work queues must be a power-of-2 in size.  Also
	 * they may not be any smaller than MCXNEX_QP_MIN_SIZE.  This step is
	...
	/*
	 * Allocate the memory for QP work queues. Since Mcxnex work queues
	 ...
	 * that they would never cross the 4GB boundary (Mcxnex work queues
	 ...
	 * to the queue memory (as a result of the mcxnex_mr_register() call)
	 * is offset from zero.
	...
	if (qp_is_umap) {
		qp->qp_wqinfo.qa_location = MCXNEX_QUEUE_LOCATION_USERLAND;
	} else {
		qp->qp_wqinfo.qa_location = MCXNEX_QUEUE_LOCATION_NORMAL;
	}
	status = mcxnex_queue_alloc(state, &qp->qp_wqinfo, sleepflag);
	if (status != DDI_SUCCESS) {
		status = IBT_INSUFF_RESOURCE;
		goto qpalloc_fail7;
	}
	...
	if (qp_is_umap == 0) {
		qp->qp_sq_wqhdr = mcxnex_wrid_wqhdr_create(sq_depth);
		...
		if (qp_srq_en) {
			...
			qp->qp_rq_wqhdr = mcxnex_wrid_wqhdr_create(rq_depth);
		...
		qp->qp_sq_wqavl.wqa_type = MCXNEX_WR_SEND;
		qp->qp_sq_wqavl.wqa_wq = qp->qp_sq_wqhdr;
		qp->qp_rq_wqavl.wqa_qpn = qp->qp_qpnum;
		qp->qp_rq_wqavl.wqa_type = MCXNEX_WR_RECV;
	}

	/*
	 * Register the memory for the QP work queues.  The memory for the
	 * QP must be registered in the Mcxnex cMPT tables.  This gives us the
	 * LKey to specify in the QP context later.  Note: The memory for
	 * Mcxnex work queues (both Send and Recv) must be contiguous and
	 ...
	 * flag in the call to mcxnex_mr_register(). This guarantees that the
	...
	/*
	 * Fill in the rest of the Mcxnex Queue Pair handle.
	 */
	qp->qp_qpcrsrcp		= qpc;
	qp->qp_rsrcp		= rsrc;
	qp->qp_state		= MCXNEX_QP_RESET;
	qp->qp_pdhdl		= pd;
	qp->qp_mrhdl		= mr;
	qp->qp_sq_sigtype	= (attr_p->qp_flags & IBT_WR_SIGNALED) ?
	    MCXNEX_QP_SQ_WR_SIGNALED : MCXNEX_QP_SQ_ALL_SIGNALED;
	qp->qp_is_special	= 0;
	...
	if (qp_srq_en) {
		qp->qp_srqhdl = srq;
		qp->qp_srq_en = MCXNEX_QP_SRQ_ENABLED;
		mcxnex_srq_refcnt_inc(qp->qp_srqhdl);
	} else {
		qp->qp_srqhdl = NULL;
		qp->qp_srq_en = MCXNEX_QP_SRQ_DISABLED;
	}

	/* Determine the QP service type */
	if (type == IBT_RC_RQP) {
		qp->qp_serv_type = MCXNEX_QP_RC;
	} else if (type == IBT_UD_RQP) {
		qp->qp_serv_type = MCXNEX_QP_UD;
	} else if (type == IBT_RAWETHER_RQP) {
		qp->qp_serv_type = MCXNEX_QP_MLX;
	} else {
		qp->qp_serv_type = MCXNEX_QP_UC;
	}
	...
	bzero(&qp->qpc, sizeof (mcxnex_hw_qpc_t));

	/*
	 * Put QP handle in Mcxnex QPNum-to-QPHdl list.  Then fill in the
 ...
 */
qpalloc_fail9:
	mcxnex_queue_free(&qp->qp_wqinfo);
qpalloc_fail8:
	if (qp->qp_sq_wqhdr)
		mcxnex_wrid_wqhdr_destroy(qp->qp_sq_wqhdr);
	if (qp->qp_rq_wqhdr)
		mcxnex_wrid_wqhdr_destroy(qp->qp_rq_wqhdr);
qpalloc_fail7:
	if (qp_is_umap) {
		mcxnex_umap_db_free(umapdb);
	}
	if (!qp_srq_en) {
		mcxnex_dbr_free(state, uarpg, qp->qp_rq_vdbr);
	}
...
qpalloc_fail6:
	 ...
	 */
	mcxnex_qp_release_qpn(state, qp->qp_qpn_hdl, MCXNEX_QPN_RELEASE);
	qpc = NULL;
qpalloc_fail5:
	mcxnex_rsrc_free(state, &rsrc);
qpalloc_fail4:
	if (qpc) {
		mcxnex_rsrc_free(state, &qpc);
	}
qpalloc_fail3:
	mcxnex_cq_refcnt_dec(rq_cq);
qpalloc_fail2:
	mcxnex_cq_refcnt_dec(sq_cq);
qpalloc_fail1:
...
/*
 * mcxnex_special_qp_alloc()
 *    Context: Can be called only from user or kernel context.
 */
int
mcxnex_special_qp_alloc(mcxnex_state_t *state, mcxnex_qp_info_t *qpinfo,
    uint_t sleepflag)
{
	mcxnex_rsrc_t		*qpc, *rsrc;
	mcxnex_qphdl_t		qp;
	 ...
	 */
	if (!mcxnex_portnum_is_valid(state, port)) {
	 ...
	 */
	status = mcxnex_cq_refcnt_inc(sq_cq, MCXNEX_CQ_IS_SPECIAL);
	if (status != DDI_SUCCESS) {
		status = IBT_CQ_HDL_INVALID;
		goto spec_qpalloc_fail1;
	}
	status = mcxnex_cq_refcnt_inc(rq_cq, MCXNEX_CQ_IS_SPECIAL);
	if (status != DDI_SUCCESS) {
		status = IBT_CQ_HDL_INVALID;
	 ...
	 * mcxnex_qp_alloc() above, ownership for this QP context is not
	 * immediately given to hardware in the final step here.  Instead, we
	 * wait until the QP is later transitioned to the "Init" state before
	 * passing the QP to hardware.  If we fail here, we must undo all
	 * the reference count (CQ and PD).
	 */
	status = mcxnex_special_qp_rsrc_alloc(state, type, port, &qpc);
	if (status != DDI_SUCCESS) {
	 ...
	 * pair (i.e. the Mcxnex Queue Pair handle).  If we fail here, we
	...
	/*
	 * Allocate the doorbell record.  Mcxnex uses only one for the RQ so
	...
	attr_p->qp_sizes.cs_sq =
	    max(attr_p->qp_sizes.cs_sq, MCXNEX_QP_MIN_SIZE);
	attr_p->qp_sizes.cs_rq =
	    max(attr_p->qp_sizes.cs_rq, MCXNEX_QP_MIN_SIZE);
	 ...
	 */
	mcxnex_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_rq_sgl,
	    max_sgl, MCXNEX_QP_WQ_TYPE_RECVQ,
	    &qp->qp_rq_log_wqesz, &qp->qp_rq_sgl);
	if (type == IBT_SMI_SQP) {
		mcxnex_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_sq_sgl,
		    max_sgl, MCXNEX_QP_WQ_TYPE_SENDMLX_QP0,
		    &qp->qp_sq_log_wqesz, &qp->qp_sq_sgl);
	} else {
		mcxnex_qp_sgl_to_logwqesz(state, attr_p->qp_sizes.cs_sq_sgl,
		    max_sgl, MCXNEX_QP_WQ_TYPE_SENDMLX_QP1,
		    &qp->qp_sq_log_wqesz, &qp->qp_sq_sgl);
	}
	...
	qp->qp_rq_wqhdr = mcxnex_wrid_wqhdr_create(1 << log_qp_rq_size);
	if (qp->qp_rq_wqhdr == NULL) {
		status = IBT_INSUFF_RESOURCE;
		goto spec_qpalloc_fail6;
	}
	 ...
	 * the special QP must be registered in the Mcxnex cMPT tables.  This
	 * gives us the LKey to specify in the QP context later.  Note: The
	 * memory for Mcxnex work queues (both Send and Recv) must be contiguous
	 ...
	 * the call to mcxnex_mr_register(). This guarantees that the resulting
	...
	status = mcxnex_mr_register(state, pd, &mr_attr, &mr, &mr_op,
	    MCXNEX_QP_CMPT);
	if (status != DDI_SUCCESS) {
		status = IBT_INSUFF_RESOURCE;
		goto spec_qpalloc_fail6;
	...
	/*
	 * Fill in the rest of the Mcxnex Queue Pair handle.  We can update
...
spec_qpalloc_fail5:
	mcxnex_rsrc_free(state, &rsrc);
spec_qpalloc_fail4:
	if (mcxnex_special_qp_rsrc_free(state, type, port) != DDI_SUCCESS) {
		MCXNEX_WARNING(state, "failed to free special QP rsrc");
	}
spec_qpalloc_fail3:
	mcxnex_cq_refcnt_dec(rq_cq);
spec_qpalloc_fail2:
 ...
 *    a subsequent call to mcxnex_qp_release_qpn().
 *
 *    Context: Can be called only from user or kernel context.
 */
/* ARGSUSED */
int
mcxnex_qp_free(mcxnex_state_t *state, mcxnex_qphdl_t *qphdl,
    ibc_free_qp_flags_t free_qp_flags, ibc_qpn_hdl_t *qpnh,
    uint_t sleepflag)
{
	mcxnex_rsrc_t		*qpc, *rsrc;
	mcxnex_umap_db_entry_t	*umapdb;
	mcxnex_qpn_entry_t	*entry;
	mcxnex_pdhdl_t		pd;
	mcxnex_mrhdl_t		mr;
	mcxnex_cqhdl_t		sq_cq, rq_cq;
	mcxnex_srqhdl_t		srq;
	...
	/*
	 * Pull all the necessary information from the Mcxnex Queue Pair
	 ...
	 */
	if (qp->qp_state != MCXNEX_QP_RESET) {
		if (mcxnex_qp_to_reset(state, qp) != DDI_SUCCESS) {
			mutex_exit(&qp->qp_lock);
			MCXNEX_WARNING(state, "failed to reset QP context");
			status = DDI_FAILURE;
			goto qpfree_fail;
		}
		qp->qp_state = MCXNEX_QP_RESET;
		 ...
		 */
		if (mcxnex_wrid_to_reset_handling(state, qp) != DDI_SUCCESS) {
			mutex_exit(&qp->qp_lock);
			MCXNEX_WARNING(state, "failed to reset QP WRID list");
			status = DDI_FAILURE;
			goto qpfree_fail;
		}
	}
	 ...
	 */
	if (qp->qp_is_umap) {
		status = mcxnex_umap_db_find(state->hs_instance, qp->qp_qpnum,
		    MLNX_UMAP_QPMEM_RSRC, &value, MCXNEX_UMAP_DB_REMOVE,
		    &umapdb);
		if (status != DDI_SUCCESS) {
			mutex_exit(&qp->qp_lock);
		...
		if (qp->qp_umap_dhp != NULL) {
			...
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				MCXNEX_WARNING(state, "failed in QP memory "
				    "devmap_devmem_remap()");
				return (DDI_FAILURE);
			}
			qp->qp_umap_dhp = (devmap_cookie_t)NULL;
	...
	/*
	 * Put NULL into the Mcxnex QPNum-to-QPHdl list.  This will allow any
	...
	status = mcxnex_mr_deregister(state, &mr, MCXNEX_MR_DEREG_ALL,
	    sleepflag);
	if (status != DDI_SUCCESS) {
		MCXNEX_WARNING(state, "failed to deregister QP memory");
		status = DDI_FAILURE;
		goto qpfree_fail;
	}
	...
	/* Free the dbr */
	if (!qp_srq_en) {
		mcxnex_dbr_free(state, qp->qp_uarpg, qp->qp_rq_vdbr);
	}
	 ...
	 */
	if (qp->qp_is_special) {
		type = (qp->qp_is_special == MCXNEX_QP_SMI) ?
		    IBT_SMI_SQP : IBT_GSI_SQP;

		/* Free up resources for the special QP */
		status = mcxnex_special_qp_rsrc_free(state, type, port);
		if (status != DDI_SUCCESS) {
			MCXNEX_WARNING(state, "failed to free special QP rsrc");
			status = DDI_FAILURE;
			goto qpfree_fail;
		...
		if (free_qp_flags == IBC_FREE_QP_ONLY) {
			entry = qp->qp_qpn_hdl;
			mcxnex_qp_release_qpn(state, qp->qp_qpn_hdl,
			    MCXNEX_QPN_FREE_ONLY);
	...
	/* Free the Mcxnex Queue Pair handle */
	mcxnex_rsrc_free(state, &rsrc);
	...
	mcxnex_cq_refcnt_dec(sq_cq);
	mcxnex_pd_refcnt_dec(pd);
	if (qp_srq_en == MCXNEX_QP_SRQ_ENABLED) {
		mcxnex_srq_refcnt_dec(srq);
	}
 ...
 */
int
mcxnex_qp_query(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_qp_query_attr_t *attr_p)
{
	...
	mcxnex_hw_addr_path_t	*qpc_path, *qpc_alt_path;
	...
	/* Convert the current Mcxnex QP state to IBTF QP state */
	switch (qp->qp_state) {
	case MCXNEX_QP_RESET:
	...
	if (qp->qp_is_special) {
		attr_p->qp_qpn = (qp->qp_is_special == MCXNEX_QP_SMI) ? 0 : 1;
	} else {
	 ...
	 *
	 * We do a DDI_NOSLEEP here because we are holding the "qp_lock".
	...
	status = mcxnex_cmn_query_cmd_post(state, QUERY_QP, 0, qp->qp_qpnum,
	    qpc, sizeof (*qpc), DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		mutex_exit(&qp->qp_lock);
		cmn_err(CE_WARN, "QUERY_QP command failed: %08x", status);
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
	...
	} else if (qp->qp_serv_type == MCXNEX_QP_RC) {
		...
		if (qpc->pm_state == MCXNEX_QP_PMSTATE_MIGRATED) {
			rc->rc_mig_state = IBT_STATE_MIGRATED;
		} else if (qpc->pm_state == MCXNEX_QP_PMSTATE_REARM) {
		...
		path_ptr = &rc->rc_path;
		mcxnex_get_addr_path(state, qpc_path, &path_ptr->cep_adds_vect,
		    MCXNEX_ADDRPATH_QP);
		...
		mcxnex_get_addr_path(state, qpc_alt_path,
		    &alt_path_ptr->cep_adds_vect, MCXNEX_ADDRPATH_QP);
	...
	} else if (qp->qp_serv_type == MCXNEX_QP_UC) {
	...
	} else  if (qp->qp_serv_type != MCXNEX_QP_MLX) {
		MCXNEX_WARNING(state, "unexpected QP transport type");
		mutex_exit(&qp->qp_lock);
		return (DDI_FAILURE);
	}

	/*
	 * Under certain circumstances it is possible for the Mcxnex hardware
	 ...
	 */
	if (qpc->state == MCXNEX_QP_SQERR) {
		attr_p->qp_info.qp_state = IBT_STATE_SQE;
		qp->qp_state = MCXNEX_QP_SQERR;
	}
	if (qpc->state == MCXNEX_QP_ERR) {
		attr_p->qp_info.qp_state = IBT_STATE_ERROR;
		qp->qp_state = MCXNEX_QP_ERR;
	}
	mutex_exit(&qp->qp_lock);
...
/*
 * mcxnex_qp_create_qpn()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_qp_create_qpn(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    mcxnex_rsrc_t *qpc)
{
	mcxnex_qpn_entry_t	query;
	mcxnex_qpn_entry_t	*entry;
	avl_index_t		where;
	...
	entry = (mcxnex_qpn_entry_t *)avl_find(&state->hs_qpn_avl,
	    &query, &where);
	if (entry == NULL) {
		/*
		 ...
		 */
		entry = (mcxnex_qpn_entry_t *)kmem_zalloc(
		    sizeof (mcxnex_qpn_entry_t), KM_NOSLEEP);
		if (entry == NULL) {
...
/*
 * mcxnex_qp_release_qpn()
 *    Context: Can be called only from user or kernel context.
 */
void
mcxnex_qp_release_qpn(mcxnex_state_t *state, mcxnex_qpn_entry_t *entry,
    int flags)
{
	ASSERT(entry != NULL);
	 ...
	 * already been freed during a MCXNEX_QPN_FREE_ONLY free, i.e. for
	...
	if (flags == MCXNEX_QPN_RELEASE) {
		...
		if (entry->qpn_refcnt == 0) {
			...
			if (entry->qpn_counter >= (1 <<
				...
				kmem_free(entry, sizeof (mcxnex_qpn_entry_t));
			}
		}

	} else if (flags == MCXNEX_QPN_FREE_ONLY) {
		/*
...
/*
 * mcxnex_qpn_avl_compare()
 *    Context: Can be called from user or kernel context.
 */
static int
mcxnex_qpn_avl_compare(const void *q, const void *e)
{
	mcxnex_qpn_entry_t	*entry, *query;

	entry = (mcxnex_qpn_entry_t *)e;
	query = (mcxnex_qpn_entry_t *)q;
 ...
 */
void
mcxnex_qpn_avl_init(mcxnex_state_t *state)
{
	...
	avl_create(&state->hs_qpn_avl, mcxnex_qpn_avl_compare,
	    sizeof (mcxnex_qpn_entry_t),
	    offsetof(mcxnex_qpn_entry_t, qpn_avlnode));
}
 ...
 */
void
mcxnex_qpn_avl_fini(mcxnex_state_t *state)
{
	mcxnex_qpn_entry_t	*entry;
	void			*cookie;
	 ...
	 */
	cookie = NULL;
	while ((entry = (mcxnex_qpn_entry_t *)avl_destroy_nodes(
 ...
 */
mcxnex_qphdl_t
mcxnex_qphdl_from_qpnum(mcxnex_state_t *state, uint_t qpnum)
{
...
/*
 * mcxnex_special_qp_rsrc_alloc
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_special_qp_rsrc_alloc(mcxnex_state_t *state, ibt_sqp_type_t type,
    uint_t port, mcxnex_rsrc_t **qp_rsrc)
{
	uint_t		mask, flags;
	int		status;
	...
	if (type == IBT_SMI_SQP) {
		 ...
		 * to instruct the Mcxnex firmware to handle all incoming
		 ...
		 * behaviors (e.g. Mcxnex SMA becomes unresponsive).
		 */
		 ...
		 */
		if ((flags & MCXNEX_SPECIAL_QP0_RSRC_MASK) == 0) {
			status = mcxnex_conf_special_qp_cmd_post(state,
			    state->hs_spec_qp0->hr_indx, MCXNEX_CMD_QP_SMI,
			    DDI_NOSLEEP,
			    MCXNEX_CMD_SPEC_QP_OPMOD(
			    state->hs_cfg_profile->cp_qp0_agents_in_fw,
			    state->hs_cfg_profile->cp_qp1_agents_in_fw));
			if (status != MCXNEX_CMD_SUCCESS) {
				mutex_exit(&state->hs_spec_qplock);
				cmn_err(CE_NOTE, "mcxnex%d: CONF_SPECIAL_QP "
				    "command failed: %08x\n",
		 ...
		 */
		mask = (1 << (MCXNEX_SPECIAL_QP0_RSRC + port));
		if (flags & mask) {
			mutex_exit(&state->hs_spec_qplock);
			return (IBT_QP_IN_USE);
		}
		...
		*qp_rsrc = state->hs_spec_qp0;
		 ...
		 * a CONF_SPECIAL_QP firmware command
		 */
		if ((flags & MCXNEX_SPECIAL_QP1_RSRC_MASK) == 0) {
			status = mcxnex_conf_special_qp_cmd_post(state,
			    state->hs_spec_qp1->hr_indx, MCXNEX_CMD_QP_GSI,
			    DDI_NOSLEEP,
			    MCXNEX_CMD_SPEC_QP_OPMOD(
			    state->hs_cfg_profile->cp_qp0_agents_in_fw,
			    state->hs_cfg_profile->cp_qp1_agents_in_fw));
		 ...
		 * whether the allocation was successful
		 */
		mask = (1 << (MCXNEX_SPECIAL_QP1_RSRC + port));
		if (flags & mask) {
			mutex_exit(&state->hs_spec_qplock);
			return (IBT_QP_IN_USE);
		}
...
/*
 * mcxnex_special_qp_rsrc_free
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_special_qp_rsrc_free(mcxnex_state_t *state, ibt_sqp_type_t type,
    uint_t port)
{
	uint_t		mask, flags;
	int		status;
	...
	if (type == IBT_SMI_SQP) {
		 ...
		 */
		if (flags) {
			status = mcxnex_conf_special_qp_cmd_post(state, 0,
			    MCXNEX_CMD_QP_SMI, DDI_NOSLEEP, 0);
			if (status != MCXNEX_CMD_SUCCESS) {
				mutex_exit(&state->hs_spec_qplock);
				cmn_err(CE_NOTE, "mcxnex%d: CONF_SPECIAL_QP "
				    "command failed: %08x\n",
		...
		if (flags) {
			status = mcxnex_conf_special_qp_cmd_post(state, 0,
			    MCXNEX_CMD_QP_GSI, DDI_NOSLEEP, 0);
			if (status != MCXNEX_CMD_SUCCESS) {
				mutex_exit(&state->hs_spec_qplock);
				cmn_err(CE_NOTE, "mcxnex%d: CONF_SPECIAL_QP "
				    "command failed: %08x\n",
...
/*
 * mcxnex_qp_sgl_to_logwqesz()
 *    Context: Can be called from interrupt or base context.
 */
static void
mcxnex_qp_sgl_to_logwqesz(mcxnex_state_t *state, uint_t num_sgl,
    uint_t real_max_sgl, mcxnex_qp_wq_type_t wq_type,
    uint_t *logwqesz, uint_t *max_sgl)
{
	...
	switch (wq_type) {
	case MCXNEX_QP_WQ_TYPE_SENDQ_UD:
		/*
		 ...
		 */
		max_size = (MCXNEX_QP_WQE_MLX_SND_HDRS + (num_sgl << 4));
		...
		log2 = max(log2, MCXNEX_QP_WQE_LOG_MINIMUM);

		/* Calculate actual number of SGL (given WQE size) */
		actual_sgl = ((1 << log2) -
		    sizeof (mcxnex_hw_snd_wqe_ctrl_t)) >> 4;
		break;

	case MCXNEX_QP_WQ_TYPE_SENDQ_CONN:
		/*
		 * Use requested maximum SGL to calculate max descriptor size
		 * (while guaranteeing that the descriptor size is a
		 * power-of-2 cachelines).
		...
		/* Calculate actual number of SGL (given WQE size) */
		actual_sgl = ((1 << log2) - MCXNEX_QP_WQE_MLX_SND_HDRS) >> 4;
		break;

	case MCXNEX_QP_WQ_TYPE_RECVQ:
		/*
		 * Same as above (except for Recv WQEs)
		 */
		max_size = (MCXNEX_QP_WQE_MLX_RCV_HDRS + (num_sgl << 4));
		log2 = highbit(max_size);
		if ((max_size & (max_size - 1)) == 0) {
			log2 = log2 - 1;
		}
		...
		/* Calculate actual number of SGL (given WQE size) */
		actual_sgl = ((1 << log2) - MCXNEX_QP_WQE_MLX_RCV_HDRS) >> 4;
		break;

	case MCXNEX_QP_WQ_TYPE_SENDMLX_QP0:
		/*
		 ...
		 */
		max_size = (MCXNEX_QP_WQE_MLX_QP0_HDRS + (num_sgl << 4));
		log2 = highbit(max_size);
		if ((max_size & (max_size - 1)) == 0) {
			log2 = log2 - 1;
		}
		...
		/* Calculate actual number of SGL (given WQE size) */
		actual_sgl = ((1 << log2) - MCXNEX_QP_WQE_MLX_QP0_HDRS) >> 4;
		break;

	case MCXNEX_QP_WQ_TYPE_SENDMLX_QP1:
		/*
		 ...
		 */
		max_size = (MCXNEX_QP_WQE_MLX_QP1_HDRS + (num_sgl << 4));
		log2 = highbit(max_size);
		if ((max_size & (max_size - 1)) == 0) {
			log2 = log2 - 1;
		}
		...
		/* Calculate actual number of SGL (given WQE size) */
		actual_sgl = ((1 << log2) - MCXNEX_QP_WQE_MLX_QP1_HDRS) >> 4;
		break;

	default:
		MCXNEX_WARNING(state, "unexpected work queue type");
		break;
	}

...
/*
 * mcxnex_rsrc.c
 *    Mcxnex Resource Management Routines
 *
 *    Implements all the routines necessary for setup, teardown, and
 *    alloc/free of all Mcxnex resources, including those that are managed
 *    by Mcxnex hardware or which live in Mcxnex's direct attached DDR memory.
 */
 ...
 * the resource pools used by the Mcxnex resource allocation routines.
 * They consist of four classes of object:
 *
 * Mailboxes:  The "In" and "Out" mailbox types are used by the Mcxnex
 *    command interface routines.  Mailboxes are used to pass information
 *    back and forth to the Mcxnex firmware.  Either type of mailbox may
 *    be allocated from Mcxnex's direct attached DDR memory or from system
 *    memory (although currently all "In" mailboxes are in DDR and all "out"
 *    mailboxes come from system memory.
 *
 * HW entry objects:  These objects represent resources required by the Mcxnex
 ...
 *
 * SW handle objects: These objects represent resources required by Mcxnex
...
static int mcxnex_rsrc_mbox_init(mcxnex_state_t *state,
    mcxnex_rsrc_mbox_info_t *info);
static void mcxnex_rsrc_mbox_fini(mcxnex_state_t *state,
    mcxnex_rsrc_mbox_info_t *info);

static int mcxnex_rsrc_sw_handles_init(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info);
static void mcxnex_rsrc_sw_handles_fini(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info);

static int mcxnex_rsrc_pd_handles_init(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info);
static void mcxnex_rsrc_pd_handles_fini(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info);
 ...
 */
static int mcxnex_rsrc_mbox_alloc(mcxnex_rsrc_pool_info_t *pool_info,
    uint_t num, mcxnex_rsrc_t *hdl);
static void mcxnex_rsrc_mbox_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl);

static int mcxnex_rsrc_hw_entry_alloc(mcxnex_rsrc_pool_info_t *pool_info,
    uint_t num, uint_t num_align, ddi_acc_handle_t acc_handle,
    uint_t sleepflag, mcxnex_rsrc_t *hdl);
static void mcxnex_rsrc_hw_entry_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl);

static int mcxnex_rsrc_hw_entry_icm_confirm(mcxnex_rsrc_pool_info_t *pool_info,
    uint_t num, mcxnex_rsrc_t *hdl);
static int mcxnex_rsrc_hw_entry_icm_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl);

static int mcxnex_rsrc_swhdl_alloc(mcxnex_rsrc_pool_info_t *pool_info,
    uint_t sleepflag, mcxnex_rsrc_t *hdl);
static void mcxnex_rsrc_swhdl_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl);

static int mcxnex_rsrc_pdhdl_alloc(mcxnex_rsrc_pool_info_t *pool_info,
    uint_t sleepflag, mcxnex_rsrc_t *hdl);
static void mcxnex_rsrc_pdhdl_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl);
 ...
 */
static int mcxnex_rsrc_pdhdl_constructor(void *pd, void *priv, int flags);
static void mcxnex_rsrc_pdhdl_destructor(void *pd, void *state);
static int mcxnex_rsrc_cqhdl_constructor(void *cq, void *priv, int flags);
static void mcxnex_rsrc_cqhdl_destructor(void *cq, void *state);
static int mcxnex_rsrc_qphdl_constructor(void *cq, void *priv, int flags);
static void mcxnex_rsrc_qphdl_destructor(void *cq, void *state);
static int mcxnex_rsrc_srqhdl_constructor(void *srq, void *priv, int flags);
static void mcxnex_rsrc_srqhdl_destructor(void *srq, void *state);
static int mcxnex_rsrc_refcnt_constructor(void *rc, void *priv, int flags);
static void mcxnex_rsrc_refcnt_destructor(void *rc, void *state);
static int mcxnex_rsrc_ahhdl_constructor(void *ah, void *priv, int flags);
static void mcxnex_rsrc_ahhdl_destructor(void *ah, void *state);
static int mcxnex_rsrc_mrhdl_constructor(void *mr, void *priv, int flags);
static void mcxnex_rsrc_mrhdl_destructor(void *mr, void *state);
 ...
 */
static int mcxnex_rsrc_mcg_entry_get_size(mcxnex_state_t *state,
 ...
 */
int
mcxnex_rsrc_alloc(mcxnex_state_t *state, mcxnex_rsrc_type_t rsrc, uint_t num,
    uint_t sleepflag, mcxnex_rsrc_t **hdl)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	mcxnex_rsrc_t		*tmp_rsrc_hdl;
	 ...
	 */
	flag = (sleepflag == DDI_SLEEP) ? KM_SLEEP : KM_NOSLEEP;
	tmp_rsrc_hdl = (mcxnex_rsrc_t *)kmem_cache_alloc(state->hs_rsrc_cache,
	    flag);
	...
	/*
	 * Set rsrc_hdl type.  This is later used by the mcxnex_rsrc_free call
	...
	case MCXNEX_IN_MBOX:
	case MCXNEX_OUT_MBOX:
	case MCXNEX_INTR_IN_MBOX:
	case MCXNEX_INTR_OUT_MBOX:
		status = mcxnex_rsrc_mbox_alloc(rsrc_pool, num, tmp_rsrc_hdl);
		break;
	...
	case MCXNEX_EQC:
		/*
		 * Because these objects are NOT accessed by Mcxnex driver
		 * software, we set the acc_handle parameter to zero.
		 */
		status = mcxnex_rsrc_hw_entry_alloc(rsrc_pool, num, 1, 0,
		    sleepflag, tmp_rsrc_hdl);
		break;

	case MCXNEX_DMPT:
		/*
		 * Because these objects are sometimes accessed by Mcxnex
		 ...
		 */
		status = mcxnex_rsrc_hw_entry_alloc(rsrc_pool, num, num,
		    0, sleepflag, tmp_rsrc_hdl);
		break;

	case MCXNEX_MCG:
		/*
		 * Mcxnex MCG entries are also NOT accessed by Mcxnex driver
	...
	case MCXNEX_MTT:
		 ...
		 */
		status = mcxnex_rsrc_hw_entry_alloc(rsrc_pool, num, 1,
		    0, sleepflag, tmp_rsrc_hdl);
		break;

	case MCXNEX_UARPG:
		/*
		 * Because UAR pages are written by Mcxnex driver software (for
		 * doorbells), we set the acc_handle parameter to point to
		 * the ddi_acc_handle_t for the Mcxnex UAR memory.
		 */
		status = mcxnex_rsrc_hw_entry_alloc(rsrc_pool, num, 1,
		    mcxnex_rsrc_alloc_uarhdl(state), sleepflag, tmp_rsrc_hdl);
		break;
	...
	case MCXNEX_QPHDL:
	case MCXNEX_REFCNT:
		status = mcxnex_rsrc_swhdl_alloc(rsrc_pool, sleepflag,
		    tmp_rsrc_hdl);
		break;

	case MCXNEX_PDHDL:
		status = mcxnex_rsrc_pdhdl_alloc(rsrc_pool, sleepflag,
		    tmp_rsrc_hdl);
		break;

	case MCXNEX_RDB:	/* handled during MCXNEX_QPC */
	case MCXNEX_ALTC:	/* handled during MCXNEX_QPC */
	case MCXNEX_AUXC:	/* handled during MCXNEX_QPC */
	case MCXNEX_CMPT_QPC:	/* handled during MCXNEX_QPC */
	case MCXNEX_CMPT_SRQC:	/* handled during MCXNEX_SRQC */
	case MCXNEX_CMPT_CQC:	/* handled during MCXNEX_CPC */
	case MCXNEX_CMPT_EQC:	/* handled during MCXNEX_EPC */
	default:
		MCXNEX_WARNING(state, "unexpected resource type in alloc ");
 ...
 */
void
mcxnex_rsrc_free(mcxnex_state_t *state, mcxnex_rsrc_t **hdl)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	...
	case MCXNEX_INTR_IN_MBOX:
	case MCXNEX_INTR_OUT_MBOX:
		mcxnex_rsrc_mbox_free(rsrc_pool, *hdl);
		break;
	...
	case MCXNEX_MTT:
	case MCXNEX_UARPG:
		mcxnex_rsrc_hw_entry_free(rsrc_pool, *hdl);
		break;
	...
	case MCXNEX_QPHDL:
	case MCXNEX_REFCNT:
		mcxnex_rsrc_swhdl_free(rsrc_pool, *hdl);
		break;

	case MCXNEX_PDHDL:
		mcxnex_rsrc_pdhdl_free(rsrc_pool, *hdl);
		break;
	...
	default:
		MCXNEX_WARNING(state, "unexpected resource type in free");
		break;
	}
...
/*
 * mcxnex_rsrc_init_phase1()
 *
 *    Completes the first phase of Mcxnex resource/configuration init.
 *    This involves creating the kmem_cache for the "mcxnex_rsrc_t"
 *    structs, allocating the space for the resource pool handles,
 *    and setting up the "Out" mailboxes.
 *
 *    When this function completes, the Mcxnex driver is ready to
 ...
 */
int
mcxnex_rsrc_init_phase1(mcxnex_state_t *state)
{
	mcxnex_rsrc_pool_info_t		*rsrc_pool;
	mcxnex_rsrc_mbox_info_t 		mbox_info;
	mcxnex_rsrc_cleanup_level_t	cleanup;
	mcxnex_cfg_profile_t		*cfgprof;
	uint64_t			num, size;
	int				status;
	char				*rsrc_name;
	...
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL0;

	/* Build kmem cache name from Mcxnex instance */
	rsrc_name = (char *)kmem_zalloc(MCXNEX_RSRC_NAME_MAXLEN, KM_SLEEP);
	MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_RSRC_CACHE);

	/*
	 * Create the kmem_cache for "mcxnex_rsrc_t" structures
	 * (kmem_cache_create will SLEEP until successful)
	 */
	state->hs_rsrc_cache = kmem_cache_create(rsrc_name,
	    sizeof (mcxnex_rsrc_t), 0, NULL, NULL, NULL, NULL, NULL, 0);

	/*
	 * Allocate an array of mcxnex_rsrc_pool_info_t's (used in all
	 * subsequent resource allocations)
	 */
	state->hs_rsrc_hdl = kmem_zalloc(MCXNEX_NUM_RESOURCES *
	    sizeof (mcxnex_rsrc_pool_info_t), KM_SLEEP);
	...
	rsrc_pool = &state->hs_rsrc_hdl[MCXNEX_OUT_MBOX];
	rsrc_pool->rsrc_type	  = MCXNEX_OUT_MBOX;
	rsrc_pool->rsrc_loc	  = MCXNEX_IN_SYSMEM;
	rsrc_pool->rsrc_pool_size = (size * num);
	rsrc_pool->rsrc_shift	  = cfgprof->cp_log_outmbox_size;
	rsrc_pool->rsrc_quantum	  = (uint_t)size;
	rsrc_pool->rsrc_align	  = MCXNEX_MBOX_ALIGN;
	rsrc_pool->rsrc_state	  = state;
	mbox_info.mbi_num	  = num;
	mbox_info.mbi_size	  = size;
	mbox_info.mbi_rsrcpool	  = rsrc_pool;
	status = mcxnex_rsrc_mbox_init(state, &mbox_info);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp1_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL1;

	/* Initialize the mailbox list */
	status = mcxnex_outmbox_list_init(state);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp1_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL2;

	/* Initialize the resource pool for "interrupt out" mailboxes */
	num  =  ((uint64_t)1 << cfgprof->cp_log_num_intr_outmbox);
	size =  ((uint64_t)1 << cfgprof->cp_log_outmbox_size);
	rsrc_pool = &state->hs_rsrc_hdl[MCXNEX_INTR_OUT_MBOX];
	rsrc_pool->rsrc_type	  = MCXNEX_INTR_OUT_MBOX;
	rsrc_pool->rsrc_loc	  = MCXNEX_IN_SYSMEM;
	rsrc_pool->rsrc_pool_size = (size * num);
	rsrc_pool->rsrc_shift	  = cfgprof->cp_log_outmbox_size;
	rsrc_pool->rsrc_quantum	  = (uint_t)size;
	...
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL3;

	/* Initialize the mailbox list */
	status = mcxnex_intr_outmbox_list_init(state);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp1_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL4;
	...
	rsrc_pool = &state->hs_rsrc_hdl[MCXNEX_IN_MBOX];
	rsrc_pool->rsrc_type	  = MCXNEX_IN_MBOX;
	rsrc_pool->rsrc_loc	  = MCXNEX_IN_SYSMEM;
	rsrc_pool->rsrc_pool_size = (size * num);
	...
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL5;

	/* Initialize the mailbox list */
	status = mcxnex_inmbox_list_init(state);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp1_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL6;

	/* Initialize the resource pool for "interrupt in" mailboxes */
	num  =  ((uint64_t)1 << cfgprof->cp_log_num_intr_inmbox);
	size =  ((uint64_t)1 << cfgprof->cp_log_inmbox_size);
	rsrc_pool = &state->hs_rsrc_hdl[MCXNEX_INTR_IN_MBOX];
	rsrc_pool->rsrc_type	  = MCXNEX_INTR_IN_MBOX;
	rsrc_pool->rsrc_loc	  = MCXNEX_IN_SYSMEM;
	rsrc_pool->rsrc_pool_size = (size * num);
	rsrc_pool->rsrc_shift	  = cfgprof->cp_log_inmbox_size;
	rsrc_pool->rsrc_quantum	  = (uint_t)size;
	...
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL7;

	/* Initialize the mailbox list */
	status = mcxnex_intr_inmbox_list_init(state);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp1_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_PHASE1_COMPLETE;
	kmem_free(rsrc_name, MCXNEX_RSRC_NAME_MAXLEN);
	return (DDI_SUCCESS);
...
/*
 * mcxnex_rsrc_init_phase2()
 *    Context: Only called from attach() path context
 */
int
mcxnex_rsrc_init_phase2(mcxnex_state_t *state)
{
	mcxnex_rsrc_sw_hdl_info_t	hdl_info;
	mcxnex_rsrc_hw_entry_info_t	entry_info;
	mcxnex_rsrc_pool_info_t		*rsrc_pool;
	mcxnex_rsrc_cleanup_level_t	cleanup, ncleanup;
	mcxnex_cfg_profile_t		*cfgprof;
	...
	/* Build the ICM vmem arena names from Mcxnex instance */
	rsrc_name = (char *)kmem_zalloc(MCXNEX_RSRC_NAME_MAXLEN, KM_SLEEP);
	...
	for (i = 0; i < MCXNEX_NUM_ICM_RESOURCES; i++) {
		...
		case MCXNEX_MTT:
			...
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_MTT_VMEM);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL9;
			break;

		case MCXNEX_DMPT:
			max = ((uint64_t)1 << devlim->log_max_dmpt);
			num_prealloc = ((uint64_t)1 << devlim->log_rsvd_dmpt);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_DMPT_VMEM);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL10;
			break;

		case MCXNEX_QPC:
			max = ((uint64_t)1 << devlim->log_max_qp);
			num_prealloc = ((uint64_t)1 << devlim->log_rsvd_qp);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_QPC_VMEM);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL11;
			break;

		case MCXNEX_CQC:
			max = ((uint64_t)1 << devlim->log_max_cq);
			num_prealloc = ((uint64_t)1 << devlim->log_rsvd_cq);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_CQC_VMEM);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL13;
			break;

		case MCXNEX_SRQC:
			max = ((uint64_t)1 << devlim->log_max_srq);
			num_prealloc = ((uint64_t)1 << devlim->log_rsvd_srq);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_SRQC_VMEM);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL16;
			break;

		case MCXNEX_EQC:
			max = ((uint64_t)1 << devlim->log_max_eq);
			num_prealloc = devlim->num_rsvd_eq;
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_EQC_VMEM);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL18;
			break;

		case MCXNEX_MCG:	/* handled below */
		case MCXNEX_AUXC:
		case MCXNEX_ALTC:
		case MCXNEX_RDB:
		case MCXNEX_CMPT_QPC:
		...
		rsrc_pool->rsrc_state	  = state;
		rsrc_pool->rsrc_loc	  = MCXNEX_IN_ICM;
		...
		status = mcxnex_rsrc_hw_entries_init(state, &entry_info);
		if (status != DDI_SUCCESS) {
			mcxnex_rsrc_fini(state, cleanup);
			status = DDI_FAILURE;
	 ...
	 */
	status = mcxnex_rsrc_mcg_entry_get_size(state, &mcg_size_shift);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp2_fail;
	}
	mcg_size = MCXNEX_MCGMEM_SZ(state);
	 ...
	 * number of MCGs must be set aside for Mcxnex firmware use (they
	...
	rsrc_pool		  = &state->hs_rsrc_hdl[MCXNEX_MCG];
	rsrc_pool->rsrc_type	  = MCXNEX_MCG;
	rsrc_pool->rsrc_loc	  = MCXNEX_IN_ICM;
	...
	rsrc_pool->rsrc_state	  = state;
	MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_MCG_VMEM);
	...
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL19;
	...
	/*
	 * Initialize the Mcxnex command handling interfaces.  This step
	 * sets up the outstanding command tracking mechanism for easy access
	 * and fast allocation (see mcxnex_cmd.c for more details).
	 */
	status = mcxnex_outstanding_cmdlist_init(state);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp2_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL20;

	/* Initialize the resource pool and vmem arena for the PD handles */
	rsrc_pool		 = &state->hs_rsrc_hdl[MCXNEX_PDHDL];
	rsrc_pool->rsrc_type	 = MCXNEX_PDHDL;
	rsrc_pool->rsrc_loc	 = MCXNEX_IN_SYSMEM;
	rsrc_pool->rsrc_quantum	 = sizeof (struct mcxnex_sw_pd_s);
	rsrc_pool->rsrc_state	 = state;
	MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_PDHDL_CACHE);
	...
	hdl_info.swi_constructor = mcxnex_rsrc_pdhdl_constructor;
	hdl_info.swi_destructor	 = mcxnex_rsrc_pdhdl_destructor;
	hdl_info.swi_rsrcname	 = rsrc_name;
	hdl_info.swi_flags	 = MCXNEX_SWHDL_KMEMCACHE_INIT;
	status = mcxnex_rsrc_pd_handles_init(state, &hdl_info);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
		goto rsrcinitp2_fail;
	}
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL21;
	 ...
	 */
	for (i = MCXNEX_NUM_ICM_RESOURCES; i < MCXNEX_NUM_RESOURCES; i++) {
		rsrc_pool = &state->hs_rsrc_hdl[i];

		/* Set the resource-specific attributes */
		switch (i) {
		case MCXNEX_MRHDL:
			rsrc_pool->rsrc_type    = MCXNEX_MRHDL;
			rsrc_pool->rsrc_quantum =
			    sizeof (struct mcxnex_sw_mr_s);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_MRHDL_CACHE);
			hdl_info.swi_num =
			...
			hdl_info.swi_constructor =
			    mcxnex_rsrc_mrhdl_constructor;
			hdl_info.swi_destructor	 = mcxnex_rsrc_mrhdl_destructor;
			hdl_info.swi_flags	 = MCXNEX_SWHDL_KMEMCACHE_INIT;
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL22;
			break;

		case MCXNEX_EQHDL:
			rsrc_pool->rsrc_type    = MCXNEX_EQHDL;
			rsrc_pool->rsrc_quantum =
			    sizeof (struct mcxnex_sw_eq_s);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_EQHDL_CACHE);
			hdl_info.swi_num = MCXNEX_NUM_EQ;
			hdl_info.swi_max = ((uint64_t)1 << devlim->log_max_eq);
			hdl_info.swi_constructor = NULL;
			hdl_info.swi_destructor	 = NULL;
			hdl_info.swi_flags	 = MCXNEX_SWHDL_KMEMCACHE_INIT;
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL23;
			break;

		case MCXNEX_CQHDL:
			rsrc_pool->rsrc_type    = MCXNEX_CQHDL;
			rsrc_pool->rsrc_quantum =
			    sizeof (struct mcxnex_sw_cq_s);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_CQHDL_CACHE);
			hdl_info.swi_num =
			    (uint64_t)1 << cfgprof->cp_log_num_cq;
			hdl_info.swi_max = (uint64_t)1 << devlim->log_max_cq;
			hdl_info.swi_constructor =
			    mcxnex_rsrc_cqhdl_constructor;
			hdl_info.swi_destructor	 = mcxnex_rsrc_cqhdl_destructor;
			hdl_info.swi_flags = (MCXNEX_SWHDL_KMEMCACHE_INIT |
			    MCXNEX_SWHDL_TABLE_INIT);
			hdl_info.swi_prealloc_sz = sizeof (mcxnex_cqhdl_t);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL24;
			break;

		case MCXNEX_SRQHDL:
			rsrc_pool->rsrc_type    = MCXNEX_SRQHDL;
			rsrc_pool->rsrc_quantum =
			    sizeof (struct mcxnex_sw_srq_s);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_SRQHDL_CACHE);
			hdl_info.swi_num =
			    (uint64_t)1 << cfgprof->cp_log_num_srq;
			hdl_info.swi_max = (uint64_t)1 << devlim->log_max_srq;
			hdl_info.swi_constructor =
			    mcxnex_rsrc_srqhdl_constructor;
			hdl_info.swi_destructor = mcxnex_rsrc_srqhdl_destructor;
			hdl_info.swi_flags = (MCXNEX_SWHDL_KMEMCACHE_INIT |
			    MCXNEX_SWHDL_TABLE_INIT);
			hdl_info.swi_prealloc_sz = sizeof (mcxnex_srqhdl_t);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL25;
			break;

		case MCXNEX_AHHDL:
			rsrc_pool->rsrc_type	= MCXNEX_AHHDL;
			rsrc_pool->rsrc_quantum	=
			    sizeof (struct mcxnex_sw_ah_s);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_AHHDL_CACHE);
			hdl_info.swi_num =
			    (uint64_t)1 << cfgprof->cp_log_num_ah;
			hdl_info.swi_max = MCXNEX_NUM_AH;
			hdl_info.swi_constructor =
			    mcxnex_rsrc_ahhdl_constructor;
			hdl_info.swi_destructor	 = mcxnex_rsrc_ahhdl_destructor;
			hdl_info.swi_flags	 = MCXNEX_SWHDL_KMEMCACHE_INIT;
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL26;
			break;

		case MCXNEX_QPHDL:
			rsrc_pool->rsrc_type    = MCXNEX_QPHDL;
			rsrc_pool->rsrc_quantum =
			    sizeof (struct mcxnex_sw_qp_s);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_QPHDL_CACHE);
			hdl_info.swi_num =
			    (uint64_t)1 << cfgprof->cp_log_num_qp;
			hdl_info.swi_max = (uint64_t)1 << devlim->log_max_qp;
			hdl_info.swi_constructor =
			    mcxnex_rsrc_qphdl_constructor;
			hdl_info.swi_destructor	= mcxnex_rsrc_qphdl_destructor;
			hdl_info.swi_flags = (MCXNEX_SWHDL_KMEMCACHE_INIT |
			    MCXNEX_SWHDL_TABLE_INIT);
			hdl_info.swi_prealloc_sz = sizeof (mcxnex_qphdl_t);
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL27;
			break;

		case MCXNEX_REFCNT:
			rsrc_pool->rsrc_type	 = MCXNEX_REFCNT;
			rsrc_pool->rsrc_quantum	 = sizeof (mcxnex_sw_refcnt_t);
			MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_REFCNT_CACHE);
			hdl_info.swi_num =
			    (uint64_t)1 << cfgprof->cp_log_num_dmpt;
			hdl_info.swi_max = (uint64_t)1 << devlim->log_max_dmpt;
			hdl_info.swi_constructor =
			    mcxnex_rsrc_refcnt_constructor;
			hdl_info.swi_destructor = mcxnex_rsrc_refcnt_destructor;
			hdl_info.swi_flags	 = MCXNEX_SWHDL_KMEMCACHE_INIT;
			ncleanup = MCXNEX_RSRC_CLEANUP_LEVEL28;
			break;
		...
		hdl_info.swi_rsrcpool    = rsrc_pool;
		hdl_info.swi_rsrcname    = rsrc_name;
		status = mcxnex_rsrc_sw_handles_init(state, &hdl_info);
		if (status != DDI_SUCCESS) {
			mcxnex_rsrc_fini(state, cleanup);
			status = DDI_FAILURE;
			goto rsrcinitp2_fail;
	...
	hdl_info.swi_flags	 = MCXNEX_SWHDL_TABLE_INIT;
	hdl_info.swi_prealloc_sz = sizeof (struct mcxnex_sw_mcg_list_s);
	status = mcxnex_rsrc_sw_handles_init(state, &hdl_info);
	if (status != DDI_SUCCESS) {
		mcxnex_rsrc_fini(state, cleanup);
		status = DDI_FAILURE;
	...
	cleanup = MCXNEX_RSRC_CLEANUP_LEVEL29;
	...
	rsrc_pool		  = &state->hs_rsrc_hdl[MCXNEX_UARPG];
	rsrc_pool->rsrc_type	  = MCXNEX_UARPG;
	rsrc_pool->rsrc_loc	  = MCXNEX_IN_UAR;
	...
	MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_UAR_PAGE_VMEM_ATTCH);
	entry_info.hwi_num	  = num;
	entry_info.hwi_max	  = max;
	entry_info.hwi_prealloc	  = num_prealloc;
	entry_info.hwi_rsrcpool	  = rsrc_pool;
	...
	cleanup = MCXNEX_RSRC_CLEANUP_ALL;
 ...
 */
void
mcxnex_rsrc_fini(mcxnex_state_t *state, mcxnex_rsrc_cleanup_level_t clean)
{
	mcxnex_rsrc_sw_hdl_info_t	hdl_info;
	mcxnex_rsrc_hw_entry_info_t	entry_info;
	mcxnex_rsrc_mbox_info_t		mbox_info;
	 ...
	 * ensure that MCXNEX_RSRC_CLEANUP_ALL is still the first entry (i.e.
	 * corresponds to the last resource allocated).
	 */

	case MCXNEX_RSRC_CLEANUP_ALL:
	case MCXNEX_RSRC_CLEANUP_LEVEL31:
		/* Cleanup the UAR page resource pool, first the dbr pages */
		if (state->hs_kern_dbr) {
			mcxnex_dbr_kern_free(state);
			state->hs_kern_dbr = NULL;
		}

		/* NS then, the pool itself */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_UARPG];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);

		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL30:
		/* Cleanup the central MCG handle pointers list */
		hdl_info.swi_rsrcpool  = NULL;
		hdl_info.swi_table_ptr = state->hs_mcghdl;
		hdl_info.swi_num = ((uint64_t)1 << cfgprof->cp_log_num_mcg);
		hdl_info.swi_prealloc_sz = sizeof (struct mcxnex_sw_mcg_list_s);
		mcxnex_rsrc_sw_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL29:
		/* Cleanup the reference count resource pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_REFCNT];
		hdl_info.swi_table_ptr = NULL;
		mcxnex_rsrc_sw_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL28:
		/* Cleanup the QP handle resource pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_QPHDL];
		hdl_info.swi_table_ptr = state->hs_qphdl;
		hdl_info.swi_num = ((uint64_t)1 << cfgprof->cp_log_num_qp);
		hdl_info.swi_prealloc_sz = sizeof (mcxnex_qphdl_t);
		mcxnex_rsrc_sw_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */
	case MCXNEX_RSRC_CLEANUP_LEVEL27:
		/* Cleanup the address handle resrouce pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_AHHDL];
		hdl_info.swi_table_ptr = NULL;
		mcxnex_rsrc_sw_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL26:
		/* Cleanup the SRQ handle resource pool. */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_SRQHDL];
	...
	case MCXNEX_RSRC_CLEANUP_LEVEL25:
		/* Cleanup the CQ handle resource pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_CQHDL];
	...
	case MCXNEX_RSRC_CLEANUP_LEVEL24:
		/* Cleanup the EQ handle resource pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_EQHDL];
		hdl_info.swi_table_ptr = NULL;
		mcxnex_rsrc_sw_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL23:
		/* Cleanup the MR handle resource pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_MRHDL];
		hdl_info.swi_table_ptr = NULL;
		mcxnex_rsrc_sw_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL22:
		/* Cleanup the PD handle resource pool */
		hdl_info.swi_rsrcpool  = &state->hs_rsrc_hdl[MCXNEX_PDHDL];
		hdl_info.swi_table_ptr = NULL;
		mcxnex_rsrc_pd_handles_fini(state, &hdl_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL21:
		/* Currently unused - FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL20:
		/* Cleanup the outstanding command list  */
		mcxnex_outstanding_cmdlist_fini(state);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL19:
		/* Cleanup the EQC table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_EQC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL18:
		/* Cleanup the MCG table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_MCG];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL17:
		/* Currently Unused - fallthrough */
	case MCXNEX_RSRC_CLEANUP_LEVEL16:
		/* Cleanup the SRQC table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_SRQC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL15:
		/* Cleanup the AUXC table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_AUXC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL14:
		/* Cleanup the ALTCF table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_ALTC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL13:
		/* Cleanup the CQC table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_CQC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL12:
		/* Cleanup the RDB table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_RDB];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL11:
		/* Cleanup the QPC table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_QPC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL10EQ:
		/* Cleanup the cMPTs for the EQs, CQs, SRQs, and QPs */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_CMPT_EQC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL10CQ:
		/* Cleanup the cMPTs for the EQs, CQs, SRQs, and QPs */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_CMPT_CQC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL10SRQ:
		/* Cleanup the cMPTs for the EQs, CQs, SRQs, and QPs */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_CMPT_SRQC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL10QP:
		/* Cleanup the cMPTs for the EQs, CQs, SRQs, and QPs */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_CMPT_QPC];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL10:
		/* Cleanup the dMPT table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_DMPT];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL9:
		/* Cleanup the MTT table resource pool */
		entry_info.hwi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_MTT];
		mcxnex_rsrc_hw_entries_fini(state, &entry_info);
		break;

	/*
	 * The cleanup below comes from the "Phase 1" initialization step.
	 * (see mcxnex_rsrc_init_phase1() above)
	 */
	case MCXNEX_RSRC_CLEANUP_PHASE1_COMPLETE:
		/* Cleanup the "In" mailbox list  */
		mcxnex_intr_inmbox_list_fini(state);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL7:
		/* Cleanup the interrupt "In" mailbox resource pool */
		mbox_info.mbi_rsrcpool =
		    &state->hs_rsrc_hdl[MCXNEX_INTR_IN_MBOX];
		mcxnex_rsrc_mbox_fini(state, &mbox_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL6:
		/* Cleanup the "In" mailbox list  */
		mcxnex_inmbox_list_fini(state);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL5:
		/* Cleanup the "In" mailbox resource pool */
		mbox_info.mbi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_IN_MBOX];
		mcxnex_rsrc_mbox_fini(state, &mbox_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL4:
		/* Cleanup the interrupt "Out" mailbox list  */
		mcxnex_intr_outmbox_list_fini(state);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL3:
		/* Cleanup the "Out" mailbox resource pool */
		mbox_info.mbi_rsrcpool =
		    &state->hs_rsrc_hdl[MCXNEX_INTR_OUT_MBOX];
		mcxnex_rsrc_mbox_fini(state, &mbox_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL2:
		/* Cleanup the "Out" mailbox list  */
		mcxnex_outmbox_list_fini(state);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL1:
		/* Cleanup the "Out" mailbox resource pool */
		mbox_info.mbi_rsrcpool = &state->hs_rsrc_hdl[MCXNEX_OUT_MBOX];
		mcxnex_rsrc_mbox_fini(state, &mbox_info);
		/* FALLTHROUGH */

	case MCXNEX_RSRC_CLEANUP_LEVEL0:
		/* Free the array of mcxnex_rsrc_pool_info_t's */

		kmem_free(state->hs_rsrc_hdl, MCXNEX_NUM_RESOURCES *
		    sizeof (mcxnex_rsrc_pool_info_t));

		kmem_cache_destroy(state->hs_rsrc_cache);
		break;

	default:
		MCXNEX_WARNING(state, "unexpected resource cleanup level");
		break;
	}
}
...
/*
 * mcxnex_rsrc_mbox_init()
 *    Context: Only called from attach() path context
 */
static int
mcxnex_rsrc_mbox_init(mcxnex_state_t *state, mcxnex_rsrc_mbox_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	mcxnex_rsrc_priv_mbox_t	*priv;
	...
	priv = kmem_zalloc(sizeof (mcxnex_rsrc_priv_mbox_t), KM_SLEEP);
	 ...
	 */
	mcxnex_dma_attr_init(state, &priv->pmb_dmaattr);
	priv->pmb_dmaattr.dma_attr_align  = MCXNEX_MBOX_ALIGN;
	priv->pmb_dmaattr.dma_attr_sgllen = 1;
	priv->pmb_dmaattr.dma_attr_flags = 0;
	rsrc_pool->rsrc_private = priv;

	ASSERT(rsrc_pool->rsrc_loc == MCXNEX_IN_SYSMEM);
...
/*
 * mcxnex_rsrc_mbox_fini()
 *    Context: Only called from attach() and/or detach() path contexts
 */
/* ARGSUSED */
static void
mcxnex_rsrc_mbox_fini(mcxnex_state_t *state, mcxnex_rsrc_mbox_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	...
	kmem_free(rsrc_pool->rsrc_private, sizeof (mcxnex_rsrc_priv_mbox_t));
}


/*
 * mcxnex_rsrc_hw_entries_init()
 *    Context: Only called from attach() path context
 */
int
mcxnex_rsrc_hw_entries_init(mcxnex_state_t *state,
    mcxnex_rsrc_hw_entry_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	mcxnex_rsrc_t		*rsvd_rsrc = NULL;
	vmem_t			*vmp;
	...
	if (num_hwentry > max_hwentry) {
		cmn_err(CE_WARN, "Too many resource entries. "
		    "(num %llu, max %llu, type %u)",
		    (unsigned long long)num_hwentry,
		    (unsigned long long)max_hwentry, rsrc_pool->rsrc_type);
		return (DDI_FAILURE);
	}
	 ...
	 * won't be allocated/freed through mcxnex_rsrc_alloc/free).  This
	...
	if (num_prealloc != 0) {
		status = mcxnex_rsrc_alloc(state, rsrc_pool->rsrc_type,
		    num_prealloc, DDI_SLEEP, &rsvd_rsrc);
		if (status != DDI_SUCCESS) {
	...
	DTRACE_PROBE2(mcxnex__rsrc_hw_entries_init,
...
/*
 * mcxnex_rsrc_hw_entries_fini()
 *    Context: Only called from attach() and/or detach() path contexts
 */
void
mcxnex_rsrc_hw_entries_fini(mcxnex_state_t *state,
    mcxnex_rsrc_hw_entry_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	mcxnex_rsrc_t		*rsvd_rsrc;
	...
	rsvd_rsrc = (mcxnex_rsrc_t *)rsrc_pool->rsrc_private;
	if (rsvd_rsrc != NULL) {
		mcxnex_rsrc_free(state, &rsvd_rsrc);
	}
...
/*
 * mcxnex_rsrc_sw_handles_init()
 *    Context: Only called from attach() path context
 */
/* ARGSUSED */
static int
mcxnex_rsrc_sw_handles_init(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	 ...
	 */
	if (info->swi_flags & MCXNEX_SWHDL_KMEMCACHE_INIT) {
	...
	if (info->swi_flags & MCXNEX_SWHDL_TABLE_INIT) {
...
/*
 * mcxnex_rsrc_sw_handles_fini()
 *    Context: Only called from attach() and/or detach() path contexts
 */
/* ARGSUSED */
static void
mcxnex_rsrc_sw_handles_fini(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
...
/*
 * mcxnex_rsrc_pd_handles_init()
 *    Context: Only called from attach() path context
 */
static int
mcxnex_rsrc_pd_handles_init(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	vmem_t			*vmp;
	char			vmem_name[MCXNEX_RSRC_NAME_MAXLEN];
	int			status;
	...
	status = mcxnex_rsrc_sw_handles_init(state, info);
	if (status != DDI_SUCCESS) {
		return (DDI_FAILURE);
	}

	/* Build vmem arena name from Mcxnex instance */
	MCXNEX_RSRC_NAME(vmem_name, MCXNEX_PDHDL_VMEM);
	...
	if (vmp == NULL) {
		...
		mcxnex_rsrc_sw_handles_fini(state, info);
		return (DDI_FAILURE);
	}
	rsrc_pool->rsrc_vmp = vmp;
...
/*
 * mcxnex_rsrc_pd_handles_fini()
 *    Context: Only called from attach() and/or detach() path contexts
 */
static void
mcxnex_rsrc_pd_handles_fini(mcxnex_state_t *state,
    mcxnex_rsrc_sw_hdl_info_t *info)
{
	mcxnex_rsrc_pool_info_t	*rsrc_pool;
	...
	/* Destroy the "mcxnex_sw_pd_t" kmem_cache */
	mcxnex_rsrc_sw_handles_fini(state, info);
}


/*
 * mcxnex_rsrc_mbox_alloc()
 *    Context: Only called from attach() path context
 */
static int
mcxnex_rsrc_mbox_alloc(mcxnex_rsrc_pool_info_t *pool_info, uint_t num,
    mcxnex_rsrc_t *hdl)
{
	mcxnex_rsrc_priv_mbox_t	*priv;
	caddr_t			kaddr;
...
/*
 * mcxnex_rsrc_mbox_free()
 *    Context: Can be called from interrupt or base context.
 */
static void
mcxnex_rsrc_mbox_free(mcxnex_rsrc_pool_info_t *pool_info, mcxnex_rsrc_t *hdl)
{
	ASSERT(pool_info != NULL);
	ASSERT(hdl != NULL);
...
/*
 * mcxnex_rsrc_hw_entry_alloc()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_rsrc_hw_entry_alloc(mcxnex_rsrc_pool_info_t *pool_info, uint_t num,
    ...
    mcxnex_rsrc_t *hdl)
{
	...
	/*
	 * Mcxnex hardware entries (QPC, CQC, EQC, MPT, etc.) do not
	 ...
	 */
	flag = (sleepflag == DDI_SLEEP) ? VM_SLEEP : VM_NOSLEEP;
	...
	if (pool_info->rsrc_loc == MCXNEX_IN_ICM) {
		/* confirm ICM is mapped, and allocate if necessary */
		status = mcxnex_rsrc_hw_entry_icm_confirm(pool_info, num, hdl);
		if (status != DDI_SUCCESS) {
			return (DDI_FAILURE);
		}
	}
...
/*
 * mcxnex_rsrc_hw_entry_free()
 *    Context: Can be called from interrupt or base context.
 */
static void
mcxnex_rsrc_hw_entry_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl)
{
	void			*addr;
	uint64_t		offset;
	...
	if (pool_info->rsrc_loc == MCXNEX_IN_ICM) {
		...
		status = mcxnex_rsrc_hw_entry_icm_free(pool_info, hdl);
		if (status != DDI_SUCCESS)
			MCXNEX_WARNING(pool_info->rsrc_state,
			    "failure in hw_entry_free");
	}
}

/*
 * mcxnex_rsrc_hw_entry_icm_confirm()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_rsrc_hw_entry_icm_confirm(mcxnex_rsrc_pool_info_t *pool_info, uint_t num,
    mcxnex_rsrc_t *hdl)
{
	mcxnex_state_t		*state;
	 ...
	 * backing the ICM resources allocated via mcxnex_rsrc_hw_entry_alloc().
	...
	rindx = hdl->hr_indx;
	mcxnex_index(index1, index2, rindx, icm_table, span_offset);

	mutex_enter(&icm_table->icm_table_lock);
	mcxnex_bitmap(bitmap, dma_info, icm_table, index1);
	while (num) {
#ifndef __lock_lint
...
#endif
		if (!MCXNEX_BMAP_BIT_ISSET(bitmap, index2)) {
			/* Allocate ICM for this span */
			icm_table->icm_busy = 1;
			mutex_exit(&icm_table->icm_table_lock);
			status = mcxnex_icm_alloc(state, type, index1, index2);
			mutex_enter(&icm_table->icm_table_lock);
...
fail_alloc:
	...
	while (index-- != start) {
		 ...
		 */
		MCXNEX_WARNING(state, "unable to allocate ICM memory: "
		    "UNIMPLEMENTED HANDLING!!");
	}
#else
	cmn_err(CE_WARN,
	    "unimplemented fail code in mcxnex_rsrc_hw_entry_icm_alloc");
#endif
	mutex_exit(&icm_table->icm_table_lock);
...
/*
 * mcxnex_rsrc_hw_entry_icm_free()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_rsrc_hw_entry_icm_free(mcxnex_rsrc_pool_info_t *pool_info,
    mcxnex_rsrc_t *hdl)
{
	mcxnex_state_t		*state;
	mcxnex_icm_table_t	*icm_table;
...
/*
 * mcxnex_rsrc_swhdl_alloc()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_rsrc_swhdl_alloc(mcxnex_rsrc_pool_info_t *pool_info, uint_t sleepflag,
    mcxnex_rsrc_t *hdl)
{
	void	*addr;
	int	flag;
...
/*
 * mcxnex_rsrc_swhdl_free()
 *    Context: Can be called from interrupt or base context.
 */
static void
mcxnex_rsrc_swhdl_free(mcxnex_rsrc_pool_info_t *pool_info, mcxnex_rsrc_t *hdl)
{
	ASSERT(pool_info != NULL);
	ASSERT(hdl != NULL);
...
/*
 * mcxnex_rsrc_pdhdl_alloc()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_rsrc_pdhdl_alloc(mcxnex_rsrc_pool_info_t *pool_info, uint_t sleepflag,
    mcxnex_rsrc_t *hdl)
{
	mcxnex_pdhdl_t	addr;
	void		*tmpaddr;
	...
	status = mcxnex_rsrc_swhdl_alloc(pool_info, sleepflag, hdl);
	if (status != DDI_SUCCESS) {
		return (DDI_FAILURE);
	}
	addr = (mcxnex_pdhdl_t)hdl->hr_addr;
	...
	if (tmpaddr == NULL) {
		...
		mcxnex_rsrc_swhdl_free(pool_info, hdl);
		return (DDI_FAILURE);
	}
...
/*
 * mcxnex_rsrc_pdhdl_free()
 *    Context: Can be called from interrupt or base context.
 */
static void
mcxnex_rsrc_pdhdl_free(mcxnex_rsrc_pool_info_t *pool_info, mcxnex_rsrc_t *hdl)
{
	ASSERT(pool_info != NULL);
	ASSERT(hdl != NULL);
...
/*
 * mcxnex_rsrc_pdhdl_constructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static int
mcxnex_rsrc_pdhdl_constructor(void *pd, void *priv, int flags)
{
	mcxnex_pdhdl_t	pdhdl;
	mcxnex_state_t	*state;

	pdhdl = (mcxnex_pdhdl_t)pd;
	state = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_pdhdl_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_pdhdl_destructor(void *pd, void *priv)
{
	mcxnex_pdhdl_t	pdhdl;
...
/* ARGSUSED */
static int
mcxnex_rsrc_cqhdl_constructor(void *cq, void *priv, int flags)
{
	mcxnex_cqhdl_t	cqhdl;
	mcxnex_state_t	*state;

	cqhdl = (mcxnex_cqhdl_t)cq;
	state = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_cqhdl_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_cqhdl_destructor(void *cq, void *priv)
{
	mcxnex_cqhdl_t	cqhdl;
...
/* ARGSUSED */
static int
mcxnex_rsrc_qphdl_constructor(void *qp, void *priv, int flags)
{
	mcxnex_qphdl_t	qphdl;
	mcxnex_state_t	*state;

	qphdl = (mcxnex_qphdl_t)qp;
	state = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_qphdl_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_qphdl_destructor(void *qp, void *priv)
{
	mcxnex_qphdl_t	qphdl;
...
/* ARGSUSED */
static int
mcxnex_rsrc_srqhdl_constructor(void *srq, void *priv, int flags)
{
	mcxnex_srqhdl_t	srqhdl;
	mcxnex_state_t	*state;

	srqhdl = (mcxnex_srqhdl_t)srq;
	state = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_srqhdl_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_srqhdl_destructor(void *srq, void *priv)
{
	mcxnex_srqhdl_t	srqhdl;
...
/* ARGSUSED */
static int
mcxnex_rsrc_refcnt_constructor(void *rc, void *priv, int flags)
{
	mcxnex_sw_refcnt_t	*refcnt;
	mcxnex_state_t		*state;

	refcnt = (mcxnex_sw_refcnt_t *)rc;
	state  = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_refcnt_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_refcnt_destructor(void *rc, void *priv)
{
	mcxnex_sw_refcnt_t	*refcnt;
...
/* ARGSUSED */
static int
mcxnex_rsrc_ahhdl_constructor(void *ah, void *priv, int flags)
{
	mcxnex_ahhdl_t	ahhdl;
	mcxnex_state_t	*state;

	ahhdl = (mcxnex_ahhdl_t)ah;
	state = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_ahhdl_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_ahhdl_destructor(void *ah, void *priv)
{
	mcxnex_ahhdl_t	ahhdl;
...
/* ARGSUSED */
static int
mcxnex_rsrc_mrhdl_constructor(void *mr, void *priv, int flags)
{
	mcxnex_mrhdl_t	mrhdl;
	mcxnex_state_t	*state;

	mrhdl = (mcxnex_mrhdl_t)mr;
	state = (mcxnex_state_t *)priv;
...
/*
 * mcxnex_rsrc_mrhdl_destructor()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static void
mcxnex_rsrc_mrhdl_destructor(void *mr, void *priv)
{
	mcxnex_mrhdl_t	mrhdl;
...
/*
 * mcxnex_rsrc_mcg_entry_get_size()
 */
static int
mcxnex_rsrc_mcg_entry_get_size(mcxnex_state_t *state, uint_t *mcg_size_shift)
{

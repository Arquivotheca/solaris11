...
/*
 * mcxnex_qpmod.c
 *    Mcxnex Queue Pair Modify Routines
 *
...
static int mcxnex_qp_reset2init(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_qp_info_t *info_p);
static int mcxnex_qp_init2init(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
static int mcxnex_qp_init2rtr(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
static int mcxnex_qp_rtr2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
static int mcxnex_qp_rts2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
#ifdef MCXNEX_NOTNOW
static int mcxnex_qp_rts2sqd(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags);
#endif
static int mcxnex_qp_sqd2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
static int mcxnex_qp_sqd2sqd(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
static int mcxnex_qp_sqerr2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p);
static int mcxnex_qp_to_error(mcxnex_state_t *state, mcxnex_qphdl_t qp);
static int mcxnex_qp_reset2err(mcxnex_state_t *state, mcxnex_qphdl_t qp);

static uint_t mcxnex_check_rdma_enable_flags(ibt_cep_modify_flags_t flags,
    ibt_qp_info_t *info_p, mcxnex_hw_qpc_t *qpc);
static int mcxnex_qp_validate_resp_rsrc(mcxnex_state_t *state,
    ibt_qp_rc_attr_t *rc, uint_t *rra_max);
static int mcxnex_qp_validate_init_depth(mcxnex_state_t *state,
    ibt_qp_rc_attr_t *rc, uint_t *sra_max);
static int mcxnex_qp_validate_mtu(mcxnex_state_t *state, uint_t mtu);
...
/* ARGSUSED */
int
mcxnex_qp_modify(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p,
    ibt_queue_sizes_t *actual_sz)
{
	 ...
	 * type.
	 */
	if (!(MCXNEX_QP_TYPE_VALID(info_p->qp_trans, qp->qp_serv_type))) {
	 ...
	 * in "info_p->qp_current_state" to an Mcxnex QP state.  Otherwise, we
	...
	if (flags & IBT_CEP_SET_RTR_RTS) {
		cur_state = MCXNEX_QP_RTR;		/* Ready to Receive */

	} else if ((flags & IBT_CEP_SET_STATE) &&
	    (mod_state == IBT_STATE_RTS)) {

		/* Convert the current IBTF QP state to an Mcxnex QP state */
		switch (info_p->qp_current_state) {
		case IBT_STATE_RTR:
			cur_state = MCXNEX_QP_RTR;	/* Ready to Receive */
			break;
		case IBT_STATE_RTS:
			cur_state = MCXNEX_QP_RTS;	/* Ready to Send */
			break;
		case IBT_STATE_SQE:
			cur_state = MCXNEX_QP_SQERR;	/* Send Queue Error */
			break;
		case IBT_STATE_SQD:
			cur_state = MCXNEX_QP_SQD;	/* SQ Drained */
			break;
		default:
			mutex_exit(&qp->qp_lock);
	...
	case MCXNEX_QP_RESET:
		...
		if ((flags & IBT_CEP_SET_RESET_INIT) &&
			 ...
			 */
			status = mcxnex_qp_reset2init(state, qp, info_p);
			 ...
			 *    already be set to MCXNEX_QP_RESET.
			 *
			 * Note: We return here because we do not want to fall
			 *    through to the mcxnex_wrid_from_reset_handling()
			 ...
			 */
			status = mcxnex_qp_reset2err(state, qp);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
		 ...
		 * lists).  Note: If mcxnex_wrid_from_reset_handling() fails,
		 ...
		 */
		status = mcxnex_wrid_from_reset_handling(state, qp);
		if (status != DDI_SUCCESS) {
			if (mcxnex_qp_to_reset(state, qp) != DDI_SUCCESS) {
				MCXNEX_WARNING(state, "failed to reset QP");
			}
	...
	case MCXNEX_QP_INIT:
		...
		if ((flags & IBT_CEP_SET_INIT_RTR) &&
			 ...
			 */
			status = mcxnex_qp_init2rtr(state, qp, flags, info_p);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			 ...
			 */
			status = mcxnex_qp_init2init(state, qp, flags, info_p);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			 ...
			 */
			status = mcxnex_qp_to_reset(state, qp);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			 ...
			 * workQ WRID lists)
			 */
			status = mcxnex_wrid_to_reset_handling(state, qp);
			 ...
			 */
			status = mcxnex_qp_to_error(state, qp);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
	...
	case MCXNEX_QP_RTR:
		...
		if ((flags & IBT_CEP_SET_RTR_RTS) &&
			 ...
			 */
			status = mcxnex_qp_rtr2rts(state, qp, flags, info_p);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			qp->qp_state = MCXNEX_QP_RTS;
	...
	case MCXNEX_QP_RTS:
		...
		if ((flags & IBT_CEP_SET_STATE) &&
			 ...
			 */
			status = mcxnex_qp_rts2rts(state, qp, flags, info_p);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			 ...
			 */
			status = mcxnex_qp_rts2sqd(state, qp, flags);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			qp->qp_state = MCXNEX_QP_SQD;
#else
	...
	case MCXNEX_QP_SQERR:
		...
		if ((flags & IBT_CEP_SET_STATE) &&
			 ...
			 */
			status = mcxnex_qp_sqerr2rts(state, qp, flags, info_p);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
	...
	case MCXNEX_QP_SQD:
		...
		if ((flags & IBT_CEP_SET_STATE) &&
			 ...
			 */
			status = mcxnex_qp_sqd2sqd(state, qp, flags, info_p);
			if (status != DDI_SUCCESS) {
				mutex_exit(&qp->qp_lock);
				goto qpmod_fail;
			}
			...
			status = mcxnex_qp_sqd2rts(state, qp,
	...
	case MCXNEX_QP_ERR:
		...
		if ((flags & IBT_CEP_SET_STATE) &&
			 ...
			 *    Nothing to do here really... just drop the lock
			 *    and return success.  The qp->qp_state should
			 *    already be set to MCXNEX_QP_ERR.
			 *
			 */
			mutex_exit(&qp->qp_lock);
			return (DDI_SUCCESS);
	...
	default:
		 ...
		 */
		mutex_exit(&qp->qp_lock);
		MCXNEX_WARNING(state, "unknown QP state in modify");
		status = IBT_QP_STATE_INVALID;
		goto qpmod_fail;
	}
...
/*
 * mcxnex_qp_reset2init()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_qp_reset2init(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	...
	if (qp->qp_is_special) {
		qpc->serv_type	= MCXNEX_QP_MLX;
	} else {
		qpc->serv_type	= qp->qp_serv_type;
	}
	qpc->pm_state		= MCXNEX_QP_PMSTATE_MIGRATED;
	...
	/* MCXNEX: sched_q is now in the address vector(s) */
	qpc->pri_addr_path.sched_q = MCXNEX_QP_SCHEDQ_GET(qp->qp_portnum,
	    0, qp->qp_is_special);
	qpc->alt_addr_path.sched_q = MCXNEX_QP_SCHEDQ_GET(qp->qp_portnum,
	    0, qp->qp_is_special);
	...
	if (qp->qp_vlan_strip_off)
		qpc->vlan_strip_off = 1;

	if (qp->qp_link_type_eth)
		qpc->pri_addr_path.link_type = MCXNEX_QP_LINK_TYPE_ETH;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		...
		if (mcxnex_pkeyindex_is_valid(state, pkeyindx)) {
		 ...
		 *
		 */
		qpc->mtu = MCXNEX_ETH_MTU;

		qp->qp_portnum = portnum - 1;

		qpc->pri_addr_path.ack_timeout = 1 & 0x07;
		qpc->pri_addr_path.sched_q = 0x83 | ((portnum - 1) << 6);
		qpc->pri_addr_path.cntr_idx = 0;
		qpc->pri_addr_path.cvlan = 0;

		qpc->msg_max = MCXNEX_ETH_MSG_MAX;
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in rst2init");
		return (DDI_FAILURE);
	}

	/*
	 * Post the RST2INIT_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, RST2INIT_QP, qpc, qp->qp_qpnum,
	    0, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		cmn_err(CE_NOTE, "mcxnex%d: RST2INIT_QP command failed: %08x\n",
 ...
 */
static int
mcxnex_qp_init2init(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	 ...
	 */
		 ...
		 */
		opmask |= mcxnex_check_rdma_enable_flags(flags, info_p, qpc);
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in init2init");
		return (DDI_FAILURE);
	}

	/*
	 * Post the INIT2INIT_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, INIT2INIT_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: INIT2INIT_QP command "
			    "failed: %08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
 ...
 */
static int
mcxnex_qp_init2rtr(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	ibt_qp_ud_attr_t	*ud;
	ibt_qp_uc_attr_t	*uc;
	mcxnex_hw_addr_path_t	*qpc_path;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		 ...
		 * the MTU is 256 bytes.  However, Mcxnex checks the QPC
		 ...
		 */
		status = mcxnex_set_addr_path(state, adds_vect, qpc_path,
		    MCXNEX_ADDRPATH_QP);
		if (status != DDI_SUCCESS) {
			return (status);
		}
		...
		if (mcxnex_portnum_is_valid(state, portnum)) {
			qpc->pri_addr_path.sched_q  =
			    MCXNEX_QP_SCHEDQ_GET(qp->qp_portnum,
		...
		if (mcxnex_qp_validate_mtu(state, mtu) != DDI_SUCCESS) {
		 ...
		 * optionally specifiable in Mcxnex.  So we force the
		 * optional flag here.
		 */
		qpc->min_rnr_nak = rc->rc_min_rnr_nak;
		opmask |= MCXNEX_CMD_OP_MINRNRNAK;

		/*
		 * Check that the number of specified "incoming RDMA resources"
		 * is valid.  And if it is, then setup the "rra_max
		 */
		if (mcxnex_qp_validate_resp_rsrc(state, rc, &rra_max) !=
		    DDI_SUCCESS) {
			return (IBT_INVALID_PARAM);
		}
		qpc->rra_max = rra_max;

		/* don't need to set up ra_buff_indx, implicit for mcxnex */
		...
		if (flags & IBT_CEP_SET_ALT_PATH) {
			...
			status = mcxnex_set_addr_path(state, adds_vect,
			    qpc_path, MCXNEX_ADDRPATH_QP);
			if (status != DDI_SUCCESS) {
				return (status);
			}
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in init2rtr");
		return (DDI_FAILURE);
	}

	/*
	 * Post the INIT2RTR_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, INIT2RTR_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: INIT2RTR_QP command "
			    "failed: %08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_rtr2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		 ...
		 * appropriately
		 */
		if (mcxnex_qp_validate_init_depth(state, rc, &sra_max) !=
		    DDI_SUCCESS) {
			return (IBT_INVALID_PARAM);
		}
		qpc->sra_max = sra_max;
		...
		if (flags & IBT_CEP_SET_MIG) {
			if (rc->rc_mig_state == IBT_STATE_MIGRATED) {
				...
				qpc->pm_state = MCXNEX_QP_PMSTATE_REARM;
			} else {
				return (IBT_QP_APM_STATE_INVALID);
			}
			opmask |= MCXNEX_CMD_OP_PM_STATE;
		}
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in rtr2rts");
		return (DDI_FAILURE);
	}

	/*
	 * Post the RTR2RTS_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, RTR2RTS_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		cmn_err(CE_WARN, "failed modify to rss\n");
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: RTR2RTS_QP command failed: "
			    "%08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_rts2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in rts2rts");
		return (DDI_FAILURE);
	}

	/*
	 * Post the RTS2RTS_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, RTS2RTS_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: RTS2RTS_QP command failed: "
			    "%08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_rts2sqd(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags)
{
	int			status;
	...
	/*
	 * Post the RTS2SQD_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, RTS2SQD_QP, NULL, qp->qp_qpnum,
	    0, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: RTS2SQD_QP command failed: "
			    "%08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_sqd2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		...
		if (flags & IBT_CEP_SET_PORT) {
			...
			opmask |= MCXNEX_CMD_OP_PRIM_PORT;
		}
		...
		if (flags & IBT_CEP_SET_RDMARA_OUT) {
			if (mcxnex_qp_validate_init_depth(state, rc,
			    &sra_max) != DDI_SUCCESS) {
				return (IBT_INVALID_PARAM);
			}
			qpc->sra_max = sra_max;
			opmask |= MCXNEX_CMD_OP_SRA_SET;
		}
		...
		if (flags & IBT_CEP_SET_RDMARA_IN) {
			if (mcxnex_qp_validate_resp_rsrc(state, rc,
			    &rra_max) != DDI_SUCCESS) {
				return (IBT_INVALID_PARAM);
			}
			qpc->rra_max = rra_max;
			opmask |= MCXNEX_CMD_OP_RRA_SET;
		}
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in sqd2rts");
		return (DDI_FAILURE);
	}

	/*
	 * Post the SQD2RTS_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, SQD2RTS_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: SQD2RTS_QP command failed: "
			    "%08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_sqd2sqd(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_rc_attr_t	*rc;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		...
		if (flags & IBT_CEP_SET_PORT) {
			...
			opmask |= MCXNEX_CMD_OP_SCHEDQUEUE;
		}
		...
		if (flags & IBT_CEP_SET_ADDS_VECT) {
			...
			opmask |= (MCXNEX_CMD_OP_PRIM_PATH |
			    MCXNEX_CMD_OP_RETRYCNT | MCXNEX_CMD_OP_ACKTIMEOUT |
			    MCXNEX_CMD_OP_PRIM_RNRRETRY);
		}
		...
		if (flags & IBT_CEP_SET_TIMEOUT) {
			qpc_path = &qpc->pri_addr_path;
			qpc_path->ack_timeout = rc->rc_path.cep_timeout;
			opmask |= MCXNEX_CMD_OP_ACKTIMEOUT;
		}
		...
		if (flags & IBT_CEP_SET_RETRY) {
			qpc->retry_cnt = rc->rc_retry_cnt;
			opmask |= MCXNEX_CMD_OP_PRIM_RNRRETRY;
		}
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in sqd2sqd");
		return (DDI_FAILURE);
	}

	/*
	 * Post the SQD2SQD_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, SQD2SQD_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: SQD2SQD_QP command failed: "
			    "%08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_sqerr2rts(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_cep_modify_flags_t flags, ibt_qp_info_t *info_p)
{
	mcxnex_hw_qpc_t		*qpc;
	ibt_qp_ud_attr_t	*ud;
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in sqerr2rts");
		return (DDI_FAILURE);
	}

	/*
	 * Post the SQERR2RTS_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, SQERR2RTS_QP, qpc, qp->qp_qpnum,
	    opmask, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		if (status != MCXNEX_CMD_BAD_QP_STATE) {
			cmn_err(CE_NOTE, "mcxnex%d: SQERR2RTS_QP command "
			    "failed: %08x\n", state->hs_instance, status);
			if (status == MCXNEX_CMD_INVALID_STATUS) {
				mcxnex_fm_ereport(state, HCA_SYS_ERR,
				    HCA_ERR_SRV_LOST);
 ...
 */
static int
mcxnex_qp_to_error(mcxnex_state_t *state, mcxnex_qphdl_t qp)
{
	int	status;
	...
	/*
	 * Post the TOERR_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, TOERR_QP, NULL, qp->qp_qpnum,
	    0, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		cmn_err(CE_NOTE, "mcxnex%d: TOERR_QP command failed: %08x\n",
		    state->hs_instance, status);
		if (status == MCXNEX_CMD_INVALID_STATUS) {
			mcxnex_fm_ereport(state, HCA_SYS_ERR, HCA_ERR_SRV_LOST);
		}
 ...
 */
int
mcxnex_qp_to_reset(mcxnex_state_t *state, mcxnex_qphdl_t qp)
{
	mcxnex_hw_qpc_t	*qpc;
	int		status;
	...
	/*
	 * Post the TORST_QP command to the Mcxnex firmware
	 *
	 * We do a MCXNEX_NOSLEEP here because we are still holding the
	 * "qp_lock".  If we got raised to interrupt level by priority
	 * inversion, we do not want to block in this routine waiting for
	 * success.
	 */
	status = mcxnex_cmn_qp_cmd_post(state, TORST_QP, qpc, qp->qp_qpnum,
	    0, DDI_NOSLEEP);
	if (status != MCXNEX_CMD_SUCCESS) {
		cmn_err(CE_NOTE, "mcxnex%d: TORST_QP command failed: %08x\n",
		    state->hs_instance, status);
		if (status == MCXNEX_CMD_INVALID_STATUS) {
			mcxnex_fm_ereport(state, HCA_SYS_ERR, HCA_ERR_SRV_LOST);
		}
 ...
 */
static int
mcxnex_qp_reset2err(mcxnex_state_t *state, mcxnex_qphdl_t qp)
{
	mcxnex_hw_qpc_t	*qpc;
	int		status;
	uint32_t	cqnmask;
	 ...
	 * context to the Mcxnex hardware.  This is accomplished by
	 ...
	 * the Mcxnex hardware and will be in the "Error" state.
	 */
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD) {
		...
		qpc->qkey = 0;
		qpc->pri_addr_path.sched_q =
		    MCXNEX_QP_SCHEDQ_GET(0, 0, qp->qp_is_special);
		qpc->pri_addr_path.pkey_indx = 0;
		 ...
		 * and return failure
		 */
		MCXNEX_WARNING(state, "unknown QP transport type in rst2err");
		return (DDI_FAILURE);
	}
	...
	/*
	 * Now post the TOERR_QP command to the Mcxnex firmware
	 *
	 * We still do a MCXNEX_NOSLEEP here because we are still holding the
...
/*
 * mcxnex_check_rdma_enable_flags()
 *    Context: Can be called from interrupt or base context.
 */
static uint_t
mcxnex_check_rdma_enable_flags(ibt_cep_modify_flags_t flags,
    ibt_qp_info_t *info_p, mcxnex_hw_qpc_t *qpc)
{
	uint_t	opmask = 0;
...
/*
 * mcxnex_qp_validate_resp_rsrc()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_qp_validate_resp_rsrc(mcxnex_state_t *state, ibt_qp_rc_attr_t *rc,
    uint_t *rra_max)
{
	uint_t	rdma_ra_in;
...
/*
 * mcxnex_qp_validate_init_depth()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_qp_validate_init_depth(mcxnex_state_t *state, ibt_qp_rc_attr_t *rc,
    uint_t *sra_max)
{
	uint_t	rdma_ra_out;
...
/*
 * mcxnex_qp_validate_mtu()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_qp_validate_mtu(mcxnex_state_t *state, uint_t mtu)
{
	/*

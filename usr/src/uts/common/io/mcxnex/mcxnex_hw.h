...
/*
 * mcxnex_hw.h
 *    Contains all the structure definitions and #defines for all Mcxnex
 *    hardware resources and registers (as defined by the Mcxnex register
 *    specification).  Wherever possible, the names in the Mcxnex spec
...
#define	PCI_DEVID_MCXNEX_SDR	0x6340	/* Mellanox MT25208-SDR PCIe Gen1 */
#define	PCI_DEVID_MCXNEX_DDR	0x634A	/* Mellanox MT25208-DDR PCIe Gen1 */
#define	PCI_DEVID_MCXNEX_EN10G  0x6368  /* Mellanox MT25448 PCIe 10G ETH  */
#define	PCI_DEVID_MCXNEX_DDRG2	0x6732	/* Mellanox MT25208-DDR PCIe Gen2 */
#define	PCI_DEVID_MCXNEX_QDRG2	0x673C	/* Mellanox MT25208-QDR PCIe Gen2 */
#define	PCI_DEVID_MCXNEX_CX2	0x6750	/* XXX: Some value.... */
#define	PCI_DEVID_MCXNEX_ENVIRT 0x6764
#define	PCI_DEVID_MCXNEX_MAINT	0x0191  /* Maintenance/Mem Controller Mode */

/*
 * Native page size of the adapter
 */
#define	MCXNEX_PAGESIZE		0x1000	/* 4Kb */
#define	MCXNEX_PAGEMASK		(MCXNEX_PAGESIZE - 1)
#define	MCXNEX_PAGESHIFT	0xC		/* 12  */
...
#define	MCXNEX_POW2(x)		(1 << highbit(x))
/*
 * given a size in bytes, return the minimum number of
 * *HCA PAGES* needed to hold it
 */
#define	MCXNEX_HCA_PAGES(x)	\
	(((x + MCXNEX_PAGESIZE) & MCXNEX_PAGEMASK) >> MCXNEX_PAGESHIFT)

/*
 * given a size in bytes, return the power of two number of
 * *HCA PAGES* needed to hold it
 */
#define	MCXNEX_HCA_POW2_PAGES(x)	(MCXNEX_POW2(MCXNEX_HCA_PAGES(x)))
/*
 ...
 */
#define	MCXNEX_CMD_HCR_OFFSET		0x80680 /* PRM */
#define	MCXNEX_CMD_SW_RESET_OFFSET	0xF0010 /* PRM */
#define	MCXNEX_CMD_SW_SEMAPHORE_OFFSET	0xF03FC /* PRM */
#define	MCXNEX_CMD_OFFSET_MASK		0xFFFFF /* per MLX instruction */


/*
 * Ownership flags used to define hardware or software ownership for
 * various Mcxnex resources
 */
#define	MCXNEX_HW_OWNER			0x1
#define	MCXNEX_SW_OWNER			0x0

/*
 * Determines whether or not virtual-to-physical address translation is
 * required.  Several of the Mcxnex hardware structures can be optionally
 * accessed by Mcxnex without going through the TPT address translation
 * tables.
 */
#define	MCXNEX_VA2PA_XLAT_ENABLED	0x1
#define	MCXNEX_VA2PA_XLAT_DISABLED	0x0
 ...
 *    primary mechanism through which mailboxes may be posted to Mcxnex
...
#define	MCXNEX_HCR_TOKEN_MASK		0xFFFF0000
#define	MCXNEX_HCR_TOKEN_SHIFT		16

#define	MCXNEX_HCR_CMD_STATUS_MASK	0xFF000000
#define	MCXNEX_HCR_CMD_GO_MASK		0x00800000
#define	MCXNEX_HCR_CMD_E_MASK		0x00400000
#define	MCXNEX_HCR_CMD_T_MASK		0x00200000
#define	MCXNEX_HCR_CMD_OPMOD_MASK	0x0000F000
#define	MCXNEX_HCR_CMD_OPCODE_MASK	0x00000FFF
#define	MCXNEX_HCR_CMD_STATUS_SHFT	24
#define	MCXNEX_HCR_CMD_GO_SHFT		23
#define	MCXNEX_HCR_CMD_E_SHFT		22
#define	MCXNEX_HCR_CMD_T_SHFT		21
#define	MCXNEX_HCR_CMD_OPMOD_SHFT	12

/*
 * Arbel "QUERY_DEV_LIM" command - Mcxnex, "QUERY_DEV_CAP" - Same hex code
 ...
 *    supported by the Mcxnex device.  This command must be run before
...
struct mcxnex_hw_querydevlim_s {
	uint32_t	rsrv0[4];
 ...
 *    version check (see mcxnex_fw_version_check() for more details).
 */

#ifdef	_LITTLE_ENDIAN
struct mcxnex_hw_queryfw_s {
...
#define	MCXNEX_FW_VER_MAJOR		0x0002	/* TBD for Mcxnex */
#define	MCXNEX_FW_VER_MINOR		0x0005
#define	MCXNEX_FW_VER_SUBMINOR		0x0000

/*
 * Mcxnex "QUERY_ADAPTER" command
 ...
 */
#ifdef	_LITTLE_ENDIAN
struct mcxnex_hw_queryadapter_s {
	uint32_t	rsrv0[4];
	...
	uint32_t	vsd_vend_id	:16;		/* added v35 mcxnex */
	uint32_t			:16;
...
#endif
#define	MCXNEX_REV_A0	0xA0
#define	MCXNEX_REV_A1	0xA1
...
/*
 * Mcxnex "INIT_HCA" and "QUERY_HCA" commands
 ...
 *    mcxnex_hw_qp_ee_cq_eq_rdb_t for "QPC/EEC/CQC/EQC/RDB Parameters",
 *    mcxnex_udav_mem_param_t for "Memory Access Parameters for UDAV Table",
 *    mcxnex_multicast_param_t for "Multicast Support Parameters",
 *    mcxnex_tpt_param_t for "Translation and Protection Table Parameters",
 *    and mcxnex_uar_param_t for Mcxnex "UAR Parameters".
 */

/*
 *  need to consider removing any ref to "ee", mcxnex doesn't support
 *       ee/rd stuff, and they've taken away the pretense
 */


#ifdef	_LITTLE_ENDIAN
typedef struct mcxnex_hw_qp_ee_cq_eq_rdb_s {
	uint32_t	rsrv0[4];
...
#ifdef	_LITTLE_ENDIAN
typedef struct mcxnex_multicast_param_s {
...
#define	MCXNEX_MCG_DEFAULT_HASH_FN	0x0

#ifdef	_LITTLE_ENDIAN
typedef struct mcxnex_tpt_param_s {
...
#ifdef	_LITTLE_ENDIAN
typedef struct mcxnex_uar_param_s {
	uint32_t	rsvd0[2];
...
#ifdef _LITTLE_ENDIAN
typedef struct mcxnex_qp_alloc_param_s {
	uint32_t			:32;
...
#ifdef	_LITTLE_ENDIAN
struct mcxnex_hw_initqueryhca_s {
	uint32_t			:32;
	...
	mcxnex_hw_qp_ee_cq_eq_rdb_t	context;

	uint32_t	rsrv2[8];

	mcxnex_multicast_param_t		multi;

	uint32_t	rsrv3[4];

	mcxnex_tpt_param_t		tpt;

	uint32_t	rsrv4[4];

	mcxnex_uar_param_t		uar;

	uint32_t	rsrv5[4];

	mcxnex_qp_alloc_param_t		qp_alloc;
...
#endif
#define	MCXNEX_UDAV_PROTECT_DISABLED	0x0
#define	MCXNEX_UDAV_PROTECT_ENABLED	0x1
#define	MCXNEX_UDAV_PORTCHK_DISABLED	0x0
#define	MCXNEX_UDAV_PORTCHK_ENABLED	0x1


/*
 * Mcxnex "INIT_IB"/"INIT_PORT" command
 ...
 *    through the Mcxnex MAD_IFC command.
 *
...
/*
 * MCXNEX query_port and set_port commands.  QUERY_PORT is new for mcxnex,
...
struct mcxnex_hw_query_port_s {
	...
	uint64_t	def_mac;		/* default mac address */
	uint32_t	rsvd1[504];
};
#else /* BIG ENDIAN */
struct mcxnex_hw_query_port_s {
	...
	/* setting value for different mac */
	uint64_t	def_mac;
	uint32_t	rsvd1[504];
...
/*
 * Mcxnex Memory Protection Table (MPT) entries
 *
...
#define	MCXNEX_MEM_CYCLE_GENERATE	0x1
#define	MCXNEX_IO_CYCLE_GENERATE	0x0

#define	MCXNEX_MPT_IS_WINDOW		0x0
#define	MCXNEX_MPT_IS_REGION		0x1

#define	MCXNEX_MPT_DEFAULT_VERSION	0x0

#define	MCXNEX_UNLIMITED_WIN_BIND	0x0

#define	MCXNEX_PHYSADDR_ENABLED		0x1
#define	MCXNEX_PHYSADDR_DISABLED	0x0


/*
 * Mcxnex Memory Translation Table (MTT) entries
 *    After accessing the MPT table (above) and validating the access rights
 *    to the region/window, Mcxnex address translation moves to the next step
...
#endif
#define	MCXNEX_MTT_ENTRY_NOTPRESENT	0x0
#define	MCXNEX_MTT_ENTRY_PRESENT	0x1


/*
 * Mcxnex Event Queue Context Table (EQC) entries
 *    Mcxnex supports 512 Event Queues, and the status of Event Queues is stored
 ...
 * 	NOTE that in Mcxnex (as opposed to earlier HCAs),
...
#endif
#define	MCXNEX_EQ_STATUS_OK		0x0
#define	MCXNEX_EQ_STATUS_OVERFLOW	0x9
#define	MCXNEX_EQ_STATUS_WRITE_FAILURE	0xA

#define	MCXNEX_EQ_ARMED			0x9
#define	MCXNEX_EQ_FIRED			0xA
#define	MCXNEX_EQ_ALWAYS_ARMED		0xB


/*
 * Mcxnex Event Queue Entries (EQE)
 ...
 *    of the various kinds of events that the Mcxnex hardware will generate.
 *    Note: The mcxnex_hw_eqe_t below is the generic "Event Queue Entry".  All
 ...
 *    mcxnex_hw_eqe_cq_t for "Completion Queue Events"
 *    mcxnex_hw_eqe_qp_evt_t for "Queue Pair Events" such as Path Migration
 ...
 *    mcxnex_hw_eqe_cqerr_t for "Completion Queue Error Events"
 *    mcxnex_hw_eqe_portstate_t for "Port State Change Events"
 *    mcxnex_hw_eqe_gpio_t for "GPIO State Change Events"
 *    mcxnex_hw_eqe_cmdcmpl_t for "Command Interface Completion Events"
 *    mcxnex_hw_eqe_operr_t for "Operational and Catastrophic Error Events"
 *        such as EQ Overflow, Misbehaved UAR page, Internal Parity Error,
 *        Uplink bus error, and DDR data error.
 *    mcxnex_hw_eqe_pgflt_t for "Not-present Page Fault on WQE or Data
...
typedef struct mcxnex_hw_eqe_cq_s {
	uint32_t			:8;
	uint32_t	cqn		:24;
	uint32_t	rsrv0[5];
} mcxnex_hw_eqe_cq_t;
...
typedef struct mcxnex_hw_eqe_qp_evt_s {
	uint32_t			:8;
	uint32_t	qpn		:24;

	uint32_t	rsrv0[5];
} mcxnex_hw_eqe_qpevt_t;


typedef struct mcxnex_hw_eqe_cqerr_s {
	uint32_t			:8;
	uint32_t	cqn		:24;
	...
	uint32_t	rsrv0[3];
} mcxnex_hw_eqe_cqerr_t;
#define	MCXNEX_CQERR_OVERFLOW		0x1
#define	MCXNEX_CQERR_ACCESS_VIOLATION	0x2


typedef struct mcxnex_hw_eqe_portstate_s {
	uint32_t	rsrv0[2];
	...
	uint32_t	rsrv1[3];
} mcxnex_hw_eqe_portstate_t;
#define	MCXNEX_PORT_LINK_ACTIVE		0x4
#define	MCXNEX_PORT_LINK_DOWN		0x1


typedef struct mcxnex_hw_eqe_gpio_s {
	uint32_t	rsrv0[3];
	...
	uint32_t		:32;
} mcxnex_hw_eqe_gpio_t;


typedef struct mcxnex_hw_eqe_cmdcmpl_s {
	uint32_t			:16;
	uint32_t	token		:16;
	...
	uint32_t			:32;
} mcxnex_hw_eqe_cmdcmpl_t;


typedef struct mcxnex_hw_eqe_operr_s {
	uint32_t	rsrv0[2];
	...
	uint32_t	rsrv1[2];
} mcxnex_hw_eqe_operr_t;
#define	MCXNEX_ERREVT_EQ_OVERFLOW	0x1
#define	MCXNEX_ERREVT_BAD_UARPG		0x2
#define	MCXNEX_ERREVT_UPLINK_BUSERR	0x3
#define	MCXNEX_ERREVT_DDR_DATAERR	0x4
#define	MCXNEX_ERREVT_INTERNAL_PARITY	0x5


typedef struct mcxnex_hw_eqe_pgflt_s {
	uint32_t	rsrv0[2];
	uint32_t			:24;
	...
	uint32_t	vaddr_l;
	uint32_t	mem_key;
} mcxnex_hw_eqe_pgflt_t;
#define	MCXNEX_PGFLT_PG_NOTPRESENT	0x8
#define	MCXNEX_PGFLT_PG_WRACC_VIOL	0xA
#define	MCXNEX_PGFLT_UNSUP_NOTPRESENT	0xE
#define	MCXNEX_PGFLT_UNSUP_WRACC_VIOL	0xF
#define	MCXNEX_PGFLT_WQE_CAUSED		0x1
#define	MCXNEX_PGFLT_DATA_CAUSED		0x0
#define	MCXNEX_PGFLT_REMOTE_CAUSED	0x1
#define	MCXNEX_PGFLT_LOCAL_CAUSED	0x0
#define	MCXNEX_PGFLT_SEND_CAUSED		0x1
#define	MCXNEX_PGFLT_RECV_CAUSED		0x0
#define	MCXNEX_PGFLT_DESC_CONSUMED	0x1
#define	MCXNEX_PGFLT_DESC_NOTCONSUMED	0x0

struct mcxnex_hw_eqe_s {
	uint32_t			:8;
	uint32_t	event_type	:8;
	uint32_t			:8;
	uint32_t	event_subtype	:8;
	union {
		mcxnex_hw_eqe_cq_t		eqe_cq;
		mcxnex_hw_eqe_qpevt_t		eqe_qpevt;
		mcxnex_hw_eqe_cqerr_t		eqe_cqerr;
		mcxnex_hw_eqe_portstate_t	eqe_portstate;
		mcxnex_hw_eqe_gpio_t		eqe_gpio;
		mcxnex_hw_eqe_cmdcmpl_t		eqe_cmdcmpl;
		mcxnex_hw_eqe_operr_t		eqe_operr;
		mcxnex_hw_eqe_pgflt_t		eqe_pgflt;
	} event_data;
	uint32_t			:24;
	uint32_t	owner		:1;
	uint32_t			:7;
 ...
 */
#define	MCXNEX_EQE_CQNUM_MASK		0x00FFFFFF
#define	MCXNEX_EQE_CQNUM_SHIFT		0
#define	MCXNEX_EQE_QPNUM_MASK		0x00FFFFFF
#define	MCXNEX_EQE_QPNUM_SHIFT		0
#define	MCXNEX_EQE_PORTNUM_MASK		0x30
#define	MCXNEX_EQE_PORTNUM_SHIFT	4
#define	MCXNEX_EQE_OWNER_MASK		0x00000080
#define	MCXNEX_EQE_OWNER_SHIFT		7

#define	MCXNEX_EQE_EVTTYPE_GET(eq, eqe)					\
	(((uint8_t *)(eqe))[1])
#define	MCXNEX_EQE_EVTSUBTYPE_GET(eq, eqe)				\
	(((uint8_t *)(eqe))[3])
#define	MCXNEX_EQE_CQNUM_GET(eq, eqe)					\
	((htonl(((uint32_t *)(eqe))[1]) & MCXNEX_EQE_CQNUM_MASK) >>	\
	    MCXNEX_EQE_CQNUM_SHIFT)
#define	MCXNEX_EQE_QPNUM_GET(eq, eqe)					\
	((htonl(((uint32_t *)(eqe))[1]) & MCXNEX_EQE_QPNUM_MASK) >>	\
	MCXNEX_EQE_QPNUM_SHIFT)
#define	MCXNEX_EQE_PORTNUM_GET(eq, eqe)					\
	(((((uint8_t *)(eqe))[12]) & MCXNEX_EQE_PORTNUM_MASK) >>	\
	    MCXNEX_EQE_PORTNUM_SHIFT)
#define	MCXNEX_EQE_CMDTOKEN_GET(eq, eqe)				\
	htons(((uint16_t *)(eqe))[3])
#define	MCXNEX_EQE_CMDSTATUS_GET(eq, eqe)				\
	(((uint8_t *)(eqe))[0xf])
#define	MCXNEX_EQE_CMDOUTP0_GET(eq, eqe)				\
	htonl(((uint32_t *)(eqe))[4])
#define	MCXNEX_EQE_CMDOUTP1_GET(eq, eqe)				\
	htonl(((uint32_t *)(eqe))[5])
#define	MCXNEX_EQE_OPERRTYPE_GET(eq, eqe)				\
	(((uint8_t *)(eqe))[0xf])
#define	MCXNEX_EQE_OPERRDATA_GET(eq, eqe)				\
	htonl(((uint32_t *)(eqe))[4])
/*
 * Mcxnex does ownership of CQ and EQ differently from Arbel & Tavor.
...
#define	MCXNEX_EQE_OWNER_IS_SW(eq, eqe)					\
	((((uint8_t *)(eqe))[0x1f] & MCXNEX_EQE_OWNER_MASK) ==		\
	    (((eq->eq_nexteqe) & eq->eq_bufsz) >>			\
	    (eq->eq_log_eqsz - MCXNEX_EQE_OWNER_SHIFT)))

/*
 * Mcxnex Completion Queue Context Table (CQC) entries
...
struct mcxnex_hw_cqc_s {
	...
	uint64_t	rsrv1[8];		/* mcxnex, match DEV_CAP size */
};
#else
struct mcxnex_hw_cqc_s {
	uint32_t	status	:4;
...
#endif
#define	MCXNEX_CQ_STATUS_OK		0x0
#define	MCXNEX_CQ_STATUS_OVERFLOW	0x9
#define	MCXNEX_CQ_STATUS_WRITE_FAILURE	0xA

#define	MCXNEX_CQ_DISARMED		0x0
#define	MCXNEX_CQ_ARMED			0x1
#define	MCXNEX_CQ_ARMED_SOLICITED	0x4
#define	MCXNEX_CQ_FIRED			0xA

/*
 * Mcxnex Completion Queue Entries (CQE)
...
#define	MCXNEX_COMPLETION_RECV		0x0
#define	MCXNEX_COMPLETION_SEND		0x1

#define	MCXNEX_CQE_DEFAULT_VERSION	0x0

/*
 * The following macros are used for extracting (and in some cases filling in)
 * information from CQEs
 */
#define	MCXNEX_CQE_QPNUM_MASK		0x00FFFFFF
#define	MCXNEX_CQE_QPNUM_SHIFT		0


#define	MCXNEX_CQE_DQPN_MASK		0x00FFFFFF
#define	MCXNEX_CQE_DQPN_SHIFT		0


#define	MCXNEX_CQE_SL_SHIFT		4
#define	MCXNEX_CQE_GRH_MASK		0x80
#define	MCXNEX_CQE_PATHBITS_MASK	0x7F
#define	MCXNEX_CQE_SLID_15_8		0xe
#define	MCXNEX_CQE_SLID_7_0		0xf
#define	MCXNEX_CQE_OPCODE_MASK		0x1F
#define	MCXNEX_CQE_SENDRECV_MASK	0x40
#define	MCXNEX_CQE_SENDRECV_SHIFT	6
#define	MCXNEX_CQE_OWNER_MASK		0x80
#define	MCXNEX_CQE_OWNER_SHIFT		7
#define	MCXNEX_CQE_WQECNTR_15_8		0x18
#define	MCXNEX_CQE_WQECNTR_7_0		0x19
/* Byte offsets for IPoIB Checksum Offload fields */
#define	MCXNEX_CQE_CKSUM_15_8		0x1a
#define	MCXNEX_CQE_CKSUM_7_0		0x1b
#define	MCXNEX_CQE_IPOK			0x10	/* byte 0x10 in cqe */
#define	MCXNEX_CQE_IPOK_BIT		0x10	/* bitmask for OK bit */
#define	MCXNEX_CQE_TCPUDP_BITS		0x0c	/* bitmask for TCP/UDP bits */

#define	MCXNEX_CQE_IS_VLAN(cq, cqe)					\
	(((uint8_t *)(cqe))[0] & 0x20)
#define	MCXNEX_CQE_VLANID(cq, cqe)					\
	((uint16_t *)(cqe))[6]

#define	MCXNEX_CQE_IS_IPOK(cq, cqe)					\
	(((uint8_t *)(cqe))[MCXNEX_CQE_IPOK] & MCXNEX_CQE_IPOK_BIT)

#define	MCXNEX_CQE_IS_TCPUDP(cq, cqe)					\
	(((uint8_t *)(cqe))[MCXNEX_CQE_IPOK] & MCXNEX_CQE_TCPUDP_BITS)

#define	MCXNEX_CQE_CKSUM(cq, cqe)					\
	((((uint8_t *)(cqe))[MCXNEX_CQE_CKSUM_15_8] << 8) |		\
	    (((uint8_t *)(cqe))[MCXNEX_CQE_CKSUM_7_0]))

#define	MCXNEX_CQE_IPOIB_STATUS(cq, cqe)				\
	htonl((((uint32_t *)(cqe)))[4])

#define	MCXNEX_CQE_QPNUM_GET(cq, cqe)					\
	((htonl((((uint32_t *)(cqe)))[0]) & MCXNEX_CQE_QPNUM_MASK) >>	\
	    MCXNEX_CQE_QPNUM_SHIFT)

#define	MCXNEX_CQE_IMM_ETH_PKEY_CRED_GET(cq, cqe)			\
	htonl(((uint32_t *)(cqe))[1])

#define	MCXNEX_CQE_DQPN_GET(cq, cqe)					\
	((htonl(((uint32_t *)(cqe))[2]) & MCXNEX_CQE_DQPN_MASK) >>	\
	    MCXNEX_CQE_DQPN_SHIFT)

#define	MCXNEX_CQE_GRH_GET(cq, cqe)					\
	(((uint8_t *)(cqe))[8] & MCXNEX_CQE_GRH_MASK)

#define	MCXNEX_CQE_PATHBITS_GET(cq, cqe)				\
	(((uint8_t *)(cqe))[8] & MCXNEX_CQE_PATHBITS_MASK)

#define	MCXNEX_CQE_DLID_GET(cq, cqe)					\
	((((uint8_t *)(cqe))[MCXNEX_CQE_SLID_15_8] << 8) |		\
	    (((uint8_t *)(cqe))[MCXNEX_CQE_SLID_7_0]))

#define	MCXNEX_CQE_SL_GET(cq, cqe)					\
	((((uint8_t *)(cqe))[12]) >> MCXNEX_CQE_SL_SHIFT)

#define	MCXNEX_CQE_BYTECNT_GET(cq, cqe)					\
	htonl(((uint32_t *)(cqe))[5])

#define	MCXNEX_CQE_WQECNTR_GET(cq, cqe)					\
	((((uint8_t *)(cqe))[MCXNEX_CQE_WQECNTR_15_8] << 8) |		\
	    (((uint8_t *)(cqe))[MCXNEX_CQE_WQECNTR_7_0]))

#define	MCXNEX_CQE_ERROR_SYNDROME_GET(cq, cqe)				\
	(((uint8_t *)(cqe))[27])

#define	MCXNEX_CQE_OPCODE_GET(cq, cqe)					\
	((((uint8_t *)(cqe))[31]) & MCXNEX_CQE_OPCODE_MASK)

#define	MCXNEX_CQE_SENDRECV_GET(cq, cqe)				\
	(((((uint8_t *)(cqe))[31]) & MCXNEX_CQE_SENDRECV_MASK) >>	\
	    MCXNEX_CQE_SENDRECV_SHIFT)

/* See Comment above for EQE - ownership of CQE is handled the same */

#define	MCXNEX_CQE_OWNER_IS_SW(cq, cqe, considx)			\
	(((((uint8_t *)(cqe))[31] & MCXNEX_CQE_OWNER_MASK) >>		\
	    MCXNEX_CQE_OWNER_SHIFT) == 					\
	    (((considx) & cq->cq_bufsz) >> cq->cq_log_cqsz))

/*
 * Mcxnex Shared Receive Queue (SRQ) Context Entry Format
 */

#ifdef _LITTLE_ENDIAN
struct mcxnex_hw_srqc_s {
...
/*
 * Mcxnex MOD_STAT_CFG input mailbox structure
 */


#ifdef _LITTLE_ENDIAN
struct mcxnex_hw_mod_stat_cfg_s {
	uint32_t	rsvd0;
...
/*
 * Mcxnex MOD_STAT_CFG input modifier structure
 */
struct mcxnex_hw_msg_in_mod_s {
#ifdef _LITTLE_ENDIAN
	uint32_t	offset			:8;
	uint32_t	port_num		:8;
	uint32_t	lane_num		:4;
...
typedef union {
	uint32_t			data;
	struct mcxnex_hw_msg_in_mod_s	mod;
} mcxnex_hw_msg_in_mod_t;

/*
 * Mcxnex UD Address Vector (UDAV)
 *    Mcxnex UDAV are used in conjunction with Unreliable Datagram (UD) send
...
#endif
#define	MCXNEX_UDAV_MODIFY_MASK0		0xFCFFFFFFFF000000ULL
#define	MCXNEX_UDAV_MODIFY_MASK1		0xFF80F00000000000ULL


/*
 * Mcxnex Queue Pair Context Table (QPC) entries
 ...
 *
 *    Below we first define the mcxnex_hw_addr_path_t or "Mcxnex Address Path"
 ...
 *    structure is _very_ similar to the mcxnex_hw_udav_t structure above,
 *    we are able to leverage the similarity with filling in and reading from
 *    the two types of structures.  See mcxnex_get_addr_path() and
 *    mcxnex_set_addr_path() in mcxnex_misc.c for more details.
 */
...
struct mcxnex_hw_addr_path_s {
	...
	uint32_t	cvlan		:1;	/* but may be used for enet */
	uint32_t	force_lb	:1;

	uint32_t	flow_label	:20;
	uint32_t	tclass		:8;
	uint32_t	sniff_s_ing	:1;
	uint32_t	sniff_s_egr	:1;
	uint32_t	sniff_r_ing	:1;
	uint32_t	sniff_r_egr	:1;
...
struct mcxnex_hw_qpc_s {
	...
	mcxnex_hw_addr_path_t	pri_addr_path;

	mcxnex_hw_addr_path_t	alt_addr_path;
	...
	uint32_t			:1;	/* may use one bit for enet */
	uint32_t	vlan_strip_off	:1;
...
#define	MCXNEX_QP_LINK_TYPE_IB		0x0
#define	MCXNEX_QP_LINK_TYPE_ETH		0x1

#define	MCXNEX_QP_RESET			0x0
#define	MCXNEX_QP_INIT			0x1
#define	MCXNEX_QP_RTR			0x2
#define	MCXNEX_QP_RTS			0x3
#define	MCXNEX_QP_SQERR			0x4
#define	MCXNEX_QP_SQD			0x5
#define	MCXNEX_QP_ERR			0x6
#define	MCXNEX_QP_SQDRAINING		0x7

#define	MCXNEX_QP_RC			0x0
#define	MCXNEX_QP_UC			0x1
#define	MCXNEX_QP_UD			0x3
#define	MCXNEX_QP_MLX			0x7

#define	MCXNEX_QP_PMSTATE_MIGRATED	0x3
#define	MCXNEX_QP_PMSTATE_ARMED		0x0
#define	MCXNEX_QP_PMSTATE_REARM		0x1

#define	MCXNEX_QP_DESC_EVT_DISABLED	0x0
#define	MCXNEX_QP_DESC_EVT_ENABLED	0x1

#define	MCXNEX_QP_FLIGHT_LIM_UNLIMITED	0xF

#define	MCXNEX_QP_SQ_ALL_SIGNALED	0x1
#define	MCXNEX_QP_SQ_WR_SIGNALED		0x0
#define	MCXNEX_QP_RQ_ALL_SIGNALED	0x1
#define	MCXNEX_QP_RQ_WR_SIGNALED		0x0

#define	MCXNEX_QP_SRQ_ENABLED	0x1
#define	MCXNEX_QP_SRQ_DISABLED	0x0

#define	MCXNEX_QP_WQE_BASE_SHIFT		0x6

/*
 * Mcxnex Multicast Group Member (MCG)
 *    Mcxnex MCG are organized in a virtually-contiguous memory table (the
 ...
 *    Note:  An MCG actually consists of a single mcxnex_hw_mcg_t and some
 *    number of mcxnex_hw_mcg_qp_list_t (such that the combined structure is a
 *    power-of-2).
 *
...
#ifdef	_LITTLE_ENDIAN
struct mcxnex_hw_mcg_qp_list_s {
	uint32_t	qpn		:24;
	uint32_t			:6;
	uint32_t	blk_lb		:1;
	uint32_t			:1;
...
#define	MCXNEX_MCG_QPN_BLOCK_LB		0x40000000
...
#ifdef _LITTLE_ENDIAN
struct mcxnex_hw_sm_perfcntr_s {
	uint32_t	linkdown	:8;
...
#ifdef _LITTLE_ENDIAN
struct mcxnex_hw_sm_extperfcntr_s {
	uint32_t	rsvd;
	uint32_t	cntrsel		:16;
	uint32_t	portsel		:8;
	uint32_t			:8;
 ...
 *		UCE
 *
 * [es] and change it even further for mcxnex
 ...
 */
typedef struct mcxnex_hw_send_db_reg_s {
	uint32_t			:32;

	uint32_t	snd_q_num	:24;
	uint32_t			:8;
} mcxnex_hw_send_db_reg_t;

#define	MCXNEX_QPSNDDB_QPN_SHIFT		0x8

/* Max descriptors per Mcxnex doorbell */
#define	MCXNEX_QP_MAXDESC_PER_DB		256

/*
 * CQ doorbell register structure
 */
typedef struct mcxnex_hw_cq_db_reg_s {
	uint32_t			:2;
	uint32_t	cmd_sn		:2;
	uint32_t			:2;
	uint32_t	cmd		:2;
	...
	uint32_t	cq_ci		:24;
} mcxnex_hw_cq_db_reg_t;

#define	MCXNEX_CQDB_CMD_SHIFT		0x18	/* dec 24 */
#define	MCXNEX_CQDB_CMDSN_SHIFT		0x1C	/* dec 28 */


#define	MCXNEX_CQDB_NOTIFY_CQ		0x02
#define	MCXNEX_CQDB_NOTIFY_CQ_SOLICIT	0x01

/* Default value for use in NOTIFY_CQ doorbell */
#define	MCXNEX_CQDB_DEFAULT_PARAM	0xFFFFFFFF

typedef struct mcxnex_hw_guest_eq_ci_s {	/* guest op eq consumer index */
	uint32_t	armed		:1;
	uint32_t			:7;
	uint32_t	guestos_ci	:24;

	uint32_t			:32;
} mcxnex_hw_guest_eq_ci_t;
 ...
 */
struct mcxnex_hw_uar_s {
	uint32_t		rsrv0[4];

	mcxnex_hw_send_db_reg_t	send;

	uint32_t		rsrv1[2];

	mcxnex_hw_cq_db_reg_t	cq;

	uint32_t		rsrv2[502];	/* next is at offset 0x800 */

	mcxnex_hw_guest_eq_ci_t	g_eq0;
	mcxnex_hw_guest_eq_ci_t	g_eq1;
	mcxnex_hw_guest_eq_ci_t	g_eq2;
	mcxnex_hw_guest_eq_ci_t	g_eq3;
 ...
 *	ease of management.  Mcxnex defines its usage in the QP chapter.
 */
typedef struct mcxnex_hw_qp_db_s {
	uint32_t			:16;
 ...
 */
typedef struct mcxnex_hw_cq_arm_db_s {
	uint32_t			:8;
...
#define	MCXNEX_CQ_DB_CMD_SOLICTED	0x01
#define	MCXNEX_CQ_DB_CMD_NEXT		0x02


/*
 * Mcxnex Blue Flame (BF)
 *	Mcxnex has the ability to do a low-latency write of successive WQEs
...
/*
 * Mcxnex Send Work Queue Element (WQE)
 *    A Mcxnex Send WQE is built of the following segments, each of which is a
...
struct mcxnex_hw_snd_wqe_ctrl_s {
	uint32_t	owner		:1;
...
struct mcxnex_hw_srq_wqe_next_s {
	uint32_t			:16;
...
#define	MCXNEX_WQE_SEND_FENCE_MASK	0x40

#define	MCXNEX_WQE_SEND_NOPCODE_NOP	0x00
#define	MCXNEX_WQE_SEND_NOPCODE_SND_INV 0x01
#define	MCXNEX_WQE_SEND_NOPCODE_RDMAW	0x8
#define	MCXNEX_WQE_SEND_NOPCODE_RDMAWI	0x9
#define	MCXNEX_WQE_SEND_NOPCODE_SEND	0xA
#define	MCXNEX_WQE_SEND_NOPCODE_SENDI	0xB
#define	MCXNEX_WQE_SEND_NOPCODE_LSO	0xE
#define	MCXNEX_WQE_SEND_NOPCODE_RDMAR	0x10
#define	MCXNEX_WQE_SEND_NOPCODE_ATMCS	0x11
#define	MCXNEX_WQE_SEND_NOPCODE_ATMFA	0x12
#define	MCXNEX_WQE_SEND_NOPCODE_ATMCSE 0x14
#define	MCXNEX_WQE_SEND_NOPCODE_ATMFAE 0x15
#define	MCXNEX_WQE_SEND_NOPCODE_BIND	0x18
#define	MCXNEX_WQE_SEND_NOPCODE_FRWR	0x19
#define	MCXNEX_WQE_SEND_NOPCODE_LCL_INV 0x1B
#define	MCXNEX_WQE_SEND_NOPCODE_CONFIG 0x1F		/* for ccq only */

#define	MCXNEX_WQE_SEND_SIGNALED_MASK	0x0000000C00000000ull
#define	MCXNEX_WQE_SEND_SOLICIT_MASK	0x0000000200000000ull
#define	MCXNEX_WQE_SEND_IMMEDIATE_MASK	0x0000000100000000ull

struct mcxnex_hw_snd_wqe_ud_s {
	struct mcxnex_hw_udav_s		ud_addr_v;
...
#define	MCXNEX_WQE_SENDHDR_UD_AV_MASK	0xFFFFFFFFFFFFFFE0ull
#define	MCXNEX_WQE_SENDHDR_UD_DQPN_MASK	0xFFFFFF

struct mcxnex_hw_snd_wqe_bind_s {
	uint32_t	ae		:1;
	uint32_t	rw		:1;
	uint32_t	rr		:1;
	uint32_t			:3;
...
#define	MCXNEX_WQE_SENDHDR_BIND_ATOM	0x8000000000000000ull
#define	MCXNEX_WQE_SENDHDR_BIND_WR	0x4000000000000000ull
#define	MCXNEX_WQE_SENDHDR_BIND_RD	0x2000000000000000ull

struct mcxnex_hw_snd_wqe_remaddr_s {
	uint64_t	vaddr;
	uint32_t	rkey;
	uint32_t			:32;
};

struct mcxnex_hw_snd_wqe_atomic_s {
	uint64_t	swap_add;
	uint64_t	compare;
};

struct mcxnex_hw_snd_wqe_atomic_ext_s {
	uint64_t	swap_add;
	uint64_t	compare;
	uint64_t	swapmask;
	uint64_t	cmpmask;
...
struct mcxnex_hw_snd_wqe_local_inv_s {
	uint32_t			:6;
...
struct mcxnex_hw_snd_wqe_frwr_s {
...
/*
 * NOTE:  Some mcxnex-PRM defined Send WQE segments are not defined here
...
/*
 * Mcxnex "MLX transport" Work Queue Element (WQE)
...
struct mcxnex_hw_mlx_wqe_nextctrl_s {
	uint32_t	owner		:1;
	uint32_t			:23;
	uint32_t			:3;
...
#define	MCXNEX_WQE_MLXHDR_VL15_MASK	0x0002000000000000ull
#define	MCXNEX_WQE_MLXHDR_SLR_MASK	0x0001000000000000ull
#define	MCXNEX_WQE_MLXHDR_SRATE_SHIFT	44
#define	MCXNEX_WQE_MLXHDR_SL_SHIFT	40
#define	MCXNEX_WQE_MLXHDR_SIGNALED_MASK	0x0000000800000000ull
#define	MCXNEX_WQE_MLXHDR_RLID_SHIFT	16


/*
 * Mcxnex Receive Work Queue Element (WQE)
...
#define	MCXNEX_WQE_SGL_BYTE_CNT_MASK	0x7FFFFFFF
#define	MCXNEX_WQE_SGL_INLINE_MASK	0x80000000

/*
 * The following defines are used when building descriptors for special QP
 * work requests (i.e. MLX transport WQEs).  Note: Because Mcxnex MLX transport
 * requires the driver to build actual IB packet headers, we use these defines
 * for the most common fields in those headers.
 */


#define	MCXNEX_MLX_VL15_LVER		0xF0000000
#define	MCXNEX_MLX_VL0_LVER		0x00000000
#define	MCXNEX_MLX_IPVER_TC_FLOW	0x60000000
#define	MCXNEX_MLX_TC_SHIFT		20
#define	MCXNEX_MLX_DEF_PKEY		0xFFFF
#define	MCXNEX_MLX_GSI_QKEY		0x80010000
#define	MCXNEX_MLX_UDSEND_OPCODE	0x64000000
#define	MCXNEX_MLX_DQPN_MASK		0xFFFFFF

/*
 * The following macros are used for building each of the individual
 * segments that can make up a Mcxnex WQE.  Note: We try not to use the
 ...
 * write operations.
 *
 *    MCXNEX_WQE_BUILD_UD		- Builds Unreliable Datagram Segment
 *
 *    MCXNEX_WQE_BUILD_REMADDR		- Builds Remote Address Segment using
 *					    RDMA info from the work request
 *    MCXNEX_WQE_BUILD_RC_ATOMIC_REMADDR	- Builds Remote Address Segment
 *					    for RC Atomic work requests
 *    MCXNEX_WQE_BUILD_ATOMIC		- Builds Atomic Segment using atomic
 *					    info from the work request
 *    MCXNEX_WQE_BUILD_BIND		- Builds the Bind Memory Window
 *					    Segment using bind info from the
 *					    work request
 *    MCXNEX_WQE_BUILD_DATA_SEG		- Builds the individual Data Segments
 *					    for Send, Receive, and MLX WQEs
 *    MCXNEX_WQE_BUILD_INLINE		- Builds an "inline" Data Segment
 *					    (primarily for MLX transport)
 *    MCXNEX_WQE_BUILD_INLINE_ICRC	- Also builds an "inline" Data Segment
 *					    (but used primarily in the ICRC
 *					    portion of MLX transport WQEs)
 *    MCXNEX_WQE_LINKNEXT		- Links the current WQE to the
 *					    previous one
 *    MCXNEX_WQE_LINKFIRST		- Links the first WQE on the current
 *					    chain to the previous WQE
 *    MCXNEX_WQE_BUILD_MLX_LRH		- Builds the inline LRH header for
 *					    MLX transport MADs
 *    MCXNEX_WQE_BUILD_MLX_GRH		- Builds the inline GRH header for
 *					    MLX transport MADs
 *    MCXNEX_WQE_BUILD_MLX_BTH		- Builds the inline BTH header for
 *					    MLX transport MADs
 *    MCXNEX_WQE_BUILD_MLX_DETH		- Builds the inline DETH header for
 *					    MLX transport MADs
 */
#define	MCXNEX_WQE_BUILD_UD(qp, ud, ah, dest)				\
{									\
	uint64_t		*tmp;					\
	uint64_t		*udav;					\
									\
	...
	tmp[4]	= ntohll((((uint64_t)((dest)->ud_dst_qpn &		\
	    MCXNEX_WQE_SENDHDR_UD_DQPN_MASK) << 32) |			\
	    (dest)->ud_qkey));						\
	tmp[5] = 0;							\
}

#define	MCXNEX_WQE_BUILD_LSO(qp, ds, mss, hdr_sz)			\
	*(uint32_t *)(ds) = htonl(((mss) << 16) | hdr_sz);

#define	MCXNEX_WQE_BUILD_REMADDR(qp, ra, wr_rdma)			\
{									\
	uint64_t		*tmp;					\
									\
	tmp	= (uint64_t *)(ra);					\
...
#define	MCXNEX_WQE_BUILD_RC_ATOMIC_REMADDR(qp, rc, wr)	\
{									\
	uint64_t		*tmp;					\
									\
	tmp	= (uint64_t *)(rc);					\
...
#define	MCXNEX_WQE_BUILD_ATOMIC(qp, at, wr_atom)		\
{									\
	uint64_t		*tmp;					\
									\
	tmp	= (uint64_t *)(at);					\
...
#define	MCXNEX_WQE_BUILD_BIND(qp, bn, wr_bind)			\
{									\
	uint64_t		*tmp;					\
	uint64_t		bn0_tmp;				\
	...
	bn0_tmp	   = (bind_flags & IBT_WR_BIND_ATOMIC) ?		\
	    MCXNEX_WQE_SENDHDR_BIND_ATOM : 0;				\
	bn0_tmp	  |= (bind_flags & IBT_WR_BIND_WRITE) ?			\
	    MCXNEX_WQE_SENDHDR_BIND_WR : 0;				\
	bn0_tmp	  |= (bind_flags & IBT_WR_BIND_READ) ?			\
	    MCXNEX_WQE_SENDHDR_BIND_RD : 0;				\
...
#define	MCXNEX_WQE_BUILD_DATA_SEG_RECV(ds, sgl)		\
{									\
	uint64_t		*tmp;					\
									\
	tmp	= (uint64_t *)(ds);					\
	tmp[0] = htonll((((uint64_t)((sgl)->ds_len & \
	    MCXNEX_WQE_SGL_BYTE_CNT_MASK) << 32) | (sgl)->ds_key));	\
	tmp[1] = htonll((sgl)->ds_va); \
}
#define	MCXNEX_WQE_BUILD_DATA_SEG_SEND(ds, sgl)		\
{									\
	((uint64_t *)(ds))[1] = htonll((sgl)->ds_va);			\
	((uint32_t *)(ds))[1] = htonl((sgl)->ds_key);			\
	membar_producer();						\
	((uint32_t *)(ds))[0] =						\
	    htonl((sgl)->ds_len & MCXNEX_WQE_SGL_BYTE_CNT_MASK);	\
}

#define	MCXNEX_WQE_BUILD_INLINE(qp, ds, sz)				\
	*(uint32_t *)(ds) = htonl(MCXNEX_WQE_SGL_INLINE_MASK | (sz))

#define	MCXNEX_WQE_BUILD_INLINE_ICRC(qp, ds, sz, icrc)	\
{									\
	uint32_t		*tmp;					\
									\
	tmp = (uint32_t *)(ds);						\
	tmp[0] = htonl(MCXNEX_WQE_SGL_INLINE_MASK | (sz));		\
	tmp[1] = htonl(icrc);						\
}

#define	MCXNEX_WQE_SET_CTRL_SEGMENT(desc, desc_sz, fence,	 	\
...
#define	MCXNEX_WQE_SET_MLX_CTRL_SEGMENT(desc, desc_sz, sig, maxstat, 	\
		lid, qp, sl)						\
{									\
	uint32_t		*tmp;					\
	uint32_t		cntr_tmp;				\
	...
	cntr_tmp &= 0x80000000;						\
	cntr_tmp |= MCXNEX_WQE_SEND_NOPCODE_SEND;			\
	tmp[0] = ntohl(cntr_tmp);					\
	tmp[1] = ntohl(desc_sz);					\
	cntr_tmp = ((maxstat << 4) | (sl & 0xff)) << 8;			\
	if (qp->qp_is_special == MCXNEX_QP_SMI)				\
		cntr_tmp |= (0x02 << 16);				\
...
#define	MCXNEX_WQE_BUILD_MLX_LRH(lrh, qp, udav, pktlen)	\
{									\
	uint32_t		*tmp;					\
	uint32_t		lrh_tmp;				\
									\
	tmp	 = (uint32_t *)(void *)(lrh);				\
									\
	if ((qp)->qp_is_special == MCXNEX_QP_SMI) {			\
		lrh_tmp = MCXNEX_MLX_VL15_LVER;				\
	} else {							\
		lrh_tmp = MCXNEX_MLX_VL0_LVER | ((udav)->sl << 20);	\
	}								\
	if ((udav)->grh) {						\
 ...
 */
#define	MCXNEX_WQE_BUILD_MLX_GRH(state, grh, qp, udav, pktlen)	\
{									\
	uint32_t		*tmp;					\
	uint32_t		grh_tmp;				\
	ib_gid_t		sgid;					\
	...
	tmp	 = (uint32_t *)(grh);					\
									\
	grh_tmp	 = MCXNEX_MLX_IPVER_TC_FLOW;				\
	grh_tmp |= (udav)->tclass << MCXNEX_MLX_TC_SHIFT;		\
...
#define	MCXNEX_WQE_BUILD_MLX_BTH(state, bth, qp, wr)		\
{									\
	uint32_t		*tmp;					\
	uint32_t		bth_tmp;				\
									\
	...
	tmp[1] = htonl((wr)->wr.ud.udwr_dest->ud_dst_qpn &		\
	    MCXNEX_MLX_DQPN_MASK);					\
	tmp[2] = 0x0;							\
}

#define	MCXNEX_WQE_BUILD_MLX_DETH(deth, qp)			\
{									\
	uint32_t		*tmp;					\
									\
	tmp	 = (uint32_t *)(deth);					\
	...
	if ((qp)->qp_is_special == MCXNEX_QP_SMI) {			\
		...
		tmp[0] = htonl(MCXNEX_MLX_GSI_QKEY);			\
		tmp[1] = htonl(0x1);					\
	}								\
}
 ...
 *    access, offsets within Mcxnex CR space for accessing flash-specific
 *    information or settings, masks used for flash settings, and
 *    timeout values for flash operations.
 */
#define	MCXNEX_HW_FLASH_CFG_HWREV			8
#define	MCXNEX_HW_FLASH_CFG_ADDR			88
#define	MCXNEX_HW_FLASH_CFG_DATA			92

#define	MCXNEX_HW_FLASH_RESET_AMD			0xF0
#define	MCXNEX_HW_FLASH_RESET_INTEL		0xFF
#define	MCXNEX_HW_FLASH_CPUMODE			0xF0150
#define	MCXNEX_HW_FLASH_ADDR			0xF01A4
#define	MCXNEX_HW_FLASH_DATA			0xF01A8
#define	MCXNEX_HW_FLASH_GPIO_SEMA		0xF03FC
#define	MCXNEX_HW_FLASH_WRCONF_SEMA		0xF0380
#define	MCXNEX_HW_FLASH_GPIO_DATA			0xF0040
#define	MCXNEX_HW_FLASH_GPIO_MOD1			0xF004C
#define	MCXNEX_HW_FLASH_GPIO_MOD0			0xF0050
#define	MCXNEX_HW_FLASH_GPIO_DATACLEAR		0xF00D4
#define	MCXNEX_HW_FLASH_GPIO_DATASET		0xF00DC
#define	MCXNEX_HW_FLASH_GPIO_LOCK		0xF0048
#define	MCXNEX_HW_FLASH_GPIO_UNLOCK_VAL		0xD42F
#define	MCXNEX_HW_FLASH_GPIO_PIN_ENABLE		0x1E000000

#define	MCXNEX_HW_FLASH_CPU_MASK			0xC0000000
#define	MCXNEX_HW_FLASH_CPU_SHIFT		30
#define	MCXNEX_HW_FLASH_ADDR_MASK		0x0007FFFC
#define	MCXNEX_HW_FLASH_CMD_MASK			0xE0000000
#define	MCXNEX_HW_FLASH_BANK_MASK		0xFFF80000

#define	MCXNEX_HW_FLASH_SPI_BUSY			0x40000000
#define	MCXNEX_HW_FLASH_SPI_WIP			0x01000000
#define	MCXNEX_HW_FLASH_SPI_READ_OP		0x00000001
#define	MCXNEX_HW_FLASH_SPI_USE_INSTR		0x00000040
#define	MCXNEX_HW_FLASH_SPI_NO_ADDR		0x00000020
#define	MCXNEX_HW_FLASH_SPI_NO_DATA		0x00000010
#define	MCXNEX_HW_FLASH_SPI_TRANS_SZ_4B		0x00000200

#define	MCXNEX_HW_FLASH_SPI_SECTOR_ERASE		0xD8
#define	MCXNEX_HW_FLASH_SPI_READ		0x03
#define	MCXNEX_HW_FLASH_SPI_PAGE_PROGRAM		0x02
#define	MCXNEX_HW_FLASH_SPI_READ_STATUS_REG	0x05
#define	MCXNEX_HW_FLASH_SPI_WRITE_ENABLE		0x06
#define	MCXNEX_HW_FLASH_SPI_READ_ESIGNATURE	0xAB

#define	MCXNEX_HW_FLASH_SPI_GW			0xF0400
#define	MCXNEX_HW_FLASH_SPI_ADDR			0xF0404
#define	MCXNEX_HW_FLASH_SPI_DATA			0xF0410
#define	MCXNEX_HW_FLASH_SPI_DATA4		0xF0414
#define	MCXNEX_HW_FLASH_SPI_DATA8		0xF0418
#define	MCXNEX_HW_FLASH_SPI_DATA12		0xF041C
#define	MCXNEX_HW_FLASH_SPI_ADDR_MASK		0x00FFFFFF
#define	MCXNEX_HW_FLASH_SPI_INSTR_PHASE_OFF	0x04
#define	MCXNEX_HW_FLASH_SPI_ADDR_PHASE_OFF	0x08
#define	MCXNEX_HW_FLASH_SPI_DATA_PHASE_OFF	0x10
#define	MCXNEX_HW_FLASH_SPI_ENABLE_OFF		0x2000
#define	MCXNEX_HW_FLASH_SPI_CS_OFF		0x800
#define	MCXNEX_HW_FLASH_SPI_INSTR_OFF		0x10000
#define	MCXNEX_HW_FLASH_SPI_INSTR_SHIFT		0x10
#define	MCXNEX_HW_FLASH_SPI_BOOT_ADDR_REG	0xF0000

#define	MCXNEX_HW_FLASH_TIMEOUT_WRITE		300
#define	MCXNEX_HW_FLASH_TIMEOUT_ERASE		1000000
#define	MCXNEX_HW_FLASH_TIMEOUT_GPIO_SEMA	1000
#define	MCXNEX_HW_FLASH_TIMEOUT_CONFIG		50

#define	MCXNEX_HW_FLASH_ICS_ERASE		0x20
#define	MCXNEX_HW_FLASH_ICS_ERROR		0x3E
#define	MCXNEX_HW_FLASH_ICS_WRITE		0x40
#define	MCXNEX_HW_FLASH_ICS_STATUS		0x70
#define	MCXNEX_HW_FLASH_ICS_READY		0x80
#define	MCXNEX_HW_FLASH_ICS_CONFIRM		0xD0
#define	MCXNEX_HW_FLASH_ICS_READ			0xFF

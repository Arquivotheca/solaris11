...
#ifndef	_MCXNEX_CFG_H
#define	_MCXNEX_CFG_H

/*
 * mcxnex_cfg.h
 *    Contains some prototypes and the structure needed to provided the
 *    Mcxnex Configuration Profile variables.
 */
...
#define	MCXNEX_RO_DISABLED	0
#define	MCXNEX_RO_ENABLED	1

/*
 * Configuration profiles
 */
#define	MCXNEX_CFG_MEMFREE	0x0001

#define	MCXNEX_MAX_PORTS		2

#define	MCXNEX_LOG_CMPT_PER_TYPE	24	/* for now, allot 2**24 per */

/*
 * The mcxnex_cfg_profile_t structure is used internally by the Mcxnex driver
 * to hold all of the configuration information for the driver.  It contains
 * information such as the maximum number (and size) of Mcxnex's queue pairs.
 * completion queues, translation tables, etc.  It also contains configuration
 * information such as whether the device is using agents in the Mcxnex
 ...
 */
typedef struct mcxnex_cfg_profile_s {
	...
	/*
	 * Number of supported Mcxnex mailboxes ("In" and "Out") and their
	 ...
	 * specified by the Mcxnex device node properties
	 */
	uint64_t	cp_sysimgguid;
	uint64_t	cp_nodeguid;
	uint64_t	cp_portguid[MCXNEX_MAX_PORTS];

} mcxnex_cfg_profile_t;

int mcxnex_cfg_profile_init_phase1(mcxnex_state_t *state);
int mcxnex_cfg_profile_init_phase2(mcxnex_state_t *state);
void mcxnex_cfg_profile_fini(mcxnex_state_t *state);

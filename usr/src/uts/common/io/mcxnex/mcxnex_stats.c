...
/*
 * mcxnex_stats.c
 *    Mcxnex IB Performance Statistics routines
 *
...
static kstat_t *mcxnex_kstat_picN_create(mcxnex_state_t *state, int num_pic,
    int num_evt, mcxnex_ks_mask_t *ev_array);
static kstat_t *mcxnex_kstat_cntr_create(mcxnex_state_t *state, int num_pic,
    int (*update)(kstat_t *, int));
static int mcxnex_kstat_cntr_update(kstat_t *ksp, int rw);

void mcxnex_kstat_perfcntr64_create(mcxnex_state_t *state, uint_t port_num);
static int mcxnex_kstat_perfcntr64_read(mcxnex_state_t *state, uint_t port,
    int reset);
static void mcxnex_kstat_perfcntr64_thread_exit(mcxnex_ks_info_t *ksi);
static int mcxnex_kstat_perfcntr64_update(kstat_t *ksp, int rw);

/*
 * Mcxnex IB Performance Events structure
 *    This structure is read-only and is used to setup the individual kstats
 *    and to initialize the tki_ib_perfcnt[] array for each Mcxnex instance.
 */
mcxnex_ks_mask_t mcxnex_ib_perfcnt_list[MCXNEX_CNTR_NUMENTRIES] = {
	{"port_xmit_data", 0, 0},
	{"port_recv_data", 0, 0},
	{"port_xmit_pkts", 0, 0},
	{"port_recv_pkts", 0, 0},
 ...
 */
int
mcxnex_kstat_init(mcxnex_state_t *state)
{
	mcxnex_ks_info_t		*ksi;
	uint_t			numports;
	int			i;

	/* Allocate a kstat info structure */
	ksi = (mcxnex_ks_info_t *)kmem_zalloc(sizeof (mcxnex_ks_info_t),
	    KM_SLEEP);
	if (ksi == NULL) {
		return (DDI_FAILURE);
	}
	 ...
	 * Enable all of the events specified in the "mcxnex_ib_perfcnt_list"
	 * structure.
	 */
	numports = state->hs_cfg_profile->cp_num_ports;
	for (i = 0; i < numports; i++) {
		ksi->hki_picN_ksp[i] = mcxnex_kstat_picN_create(state, i,
		    MCXNEX_CNTR_NUMENTRIES, mcxnex_ib_perfcnt_list);
		if (ksi->hki_picN_ksp[i] == NULL) {
			goto kstat_init_fail;
		}

		mcxnex_kstat_perfcntr64_create(state, i + 1);
	...
	ksi->hki_cntr_ksp = mcxnex_kstat_cntr_create(state, numports,
	    mcxnex_kstat_cntr_update);
	...
	/*
	 * Initialize the Mcxnex hki_ib_perfcnt[] array values using the
	 * default values in mcxnex_ib_perfcnt_list[]
	 */
	for (i = 0; i < MCXNEX_CNTR_NUMENTRIES; i++) {
		ksi->hki_ib_perfcnt[i] = mcxnex_ib_perfcnt_list[i];
	}
...
kstat_init_fail:
	...
	kmem_free(ksi, sizeof (mcxnex_ks_info_t));
 ...
 */
void
mcxnex_kstat_fini(mcxnex_state_t *state)
{
	mcxnex_ks_info_t	*ksi;
	uint_t			numports;
	int			i;
	...
	mcxnex_kstat_perfcntr64_thread_exit(ksi);
...
/*
 * mcxnex_kstat_picN_create()
 *    Context: Only called from attach() path context
 */
static kstat_t *
mcxnex_kstat_picN_create(mcxnex_state_t *state, int num_pic, int num_evt,
    mcxnex_ks_mask_t *ev_array)
{
	kstat_t			*picN_ksp;
	...
	for (i = 0; i < num_evt - 1; i++) {
		pic_named_data[i].value.ui64 =
		    ((uint64_t)i << (num_pic * MCXNEX_CNTR_SIZE));
	...
	pic_named_data[i].value.ui64 =
	    ~((uint64_t)MCXNEX_CNTR_MASK << (num_pic * MCXNEX_CNTR_SIZE));
	kstat_named_init(&pic_named_data[i], ev_array[i].ks_evt_name,
	    KSTAT_DATA_UINT64);
...
/*
 * mcxnex_kstat_cntr_create()
 *    Context: Only called from attach() path context
 */
static kstat_t *
mcxnex_kstat_cntr_create(mcxnex_state_t *state, int num_pic,
    int (*update)(kstat_t *, int))
{
	...
	/*
	 * Store the Mcxnex softstate pointer in the kstat's private field so
 ...
 */
static int
mcxnex_kstat_cntr_update(kstat_t *ksp, int rw)
{
	mcxnex_state_t		*state;
	mcxnex_ks_mask_t		*ib_perf;
	mcxnex_ks_info_t		*ksi;
	...
	int			status;
	mcxnex_hw_sm_perfcntr_t	sm_perfcntr;

	/*
	 * Extract the Mcxnex softstate pointer, kstat data, pointer to the
	...
	if (rw == KSTAT_WRITE) {
		...
		status = mcxnex_getperfcntr_cmd_post(state, 1,
		    DDI_NOSLEEP, &sm_perfcntr, 0);
		 ...
		 */
		if (numports == MCXNEX_MAX_PORTS) {
			indx   = pcr >> MCXNEX_CNTR_SIZE;
			oldval = ib_perf[indx].ks_old_pic1;

			status = mcxnex_getperfcntr_cmd_post(state, 2,
			    DDI_NOSLEEP, &sm_perfcntr, 0);
			if (status != MCXNEX_CMD_SUCCESS) {
				return (-1);
			}
...
/*
 * mcxnex_kstat_perfcntr64_create()
 *    Context: Only called from attach() path context
 *
 * Create "port#/perf_counters" kstat for the specified port number.
 */
void
mcxnex_kstat_perfcntr64_create(mcxnex_state_t *state, uint_t port_num)
{
	mcxnex_ks_info_t	*ksi = state->hs_ks_info;
	struct kstat		*cntr_ksp;
	struct kstat_named	*cntr_named_data;
	int			drv_instance;
	char			*drv_name;
	...
	status = mcxnex_is_ext_port_counters_supported(state, port_num,
	    DDI_NOSLEEP, &ext_width_supported);
	if (status == MCXNEX_CMD_SUCCESS) {
	...
	cntr_ksp = kstat_create(drv_name, drv_instance, kname, "ib",
	    KSTAT_TYPE_NAMED, MCXNEX_PERFCNTR64_NUM_COUNTERS,
	    KSTAT_FLAG_WRITABLE);
	if (cntr_ksp == NULL) {
		return;
	}
	...
	kstat_named_init(&cntr_named_data[MCXNEX_PERFCNTR64_ENABLE_IDX],
	    "enable", KSTAT_DATA_UINT32);
	kstat_named_init(&cntr_named_data[MCXNEX_PERFCNTR64_XMIT_DATA_IDX],
	    "xmit_data", KSTAT_DATA_UINT64);
	kstat_named_init(&cntr_named_data[MCXNEX_PERFCNTR64_RECV_DATA_IDX],
	    "recv_data", KSTAT_DATA_UINT64);
	kstat_named_init(&cntr_named_data[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX],
	    "xmit_pkts", KSTAT_DATA_UINT64);
	kstat_named_init(&cntr_named_data[MCXNEX_PERFCNTR64_RECV_PKTS_IDX],
	...
	cntr_ksp->ks_update  = mcxnex_kstat_perfcntr64_update;
 ...
 */
static int
mcxnex_kstat_perfcntr64_read(mcxnex_state_t *state, uint_t port, int reset)
{
	mcxnex_ks_info_t	*ksi = state->hs_ks_info;
	mcxnex_perfcntr64_ks_info_t *ksi64 = &ksi->hki_perfcntr64[port - 1];
	int			status, i;
	uint32_t		tmp;
	mcxnex_hw_sm_perfcntr_t	sm_perfcntr;
	...
	status = mcxnex_getperfcntr_cmd_post(state, port,
	    DDI_NOSLEEP, &sm_perfcntr, 0);
	if (status != MCXNEX_CMD_SUCCESS) {
		return (status);
	}
	...
	if (reset) {
		...
		tmp = MAX(sm_perfcntr.portxmdata,
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_XMIT_DATA_IDX]);
		ksi64->hki64_counters[MCXNEX_PERFCNTR64_XMIT_DATA_IDX] += tmp;

		tmp = MAX(sm_perfcntr.portrcdata,
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_RECV_DATA_IDX]);
		ksi64->hki64_counters[MCXNEX_PERFCNTR64_RECV_DATA_IDX] += tmp;

		tmp = MAX(sm_perfcntr.portxmpkts,
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX]);
		ksi64->hki64_counters[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX] += tmp;

		tmp = MAX(sm_perfcntr.portrcpkts,
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_RECV_PKTS_IDX]);
		ksi64->hki64_counters[MCXNEX_PERFCNTR64_RECV_PKTS_IDX] += tmp;

		for (i = 0; i < MCXNEX_PERFCNTR64_NUM_COUNTERS; i++)
...
/*
 * mcxnex_kstat_perfcntr64_update_thread()
 ...
 */
static void
mcxnex_kstat_perfcntr64_update_thread(void *arg)
{
	 ...
	 * for all ports. Exit if MCXNEX_PERFCNTR64_THREAD_EXIT flag is set.
	 */
	while (!(ksi->hki_perfcntr64_flags & MCXNEX_PERFCNTR64_THREAD_EXIT)) {
		for (i = 0; i < state->hs_cfg_profile->cp_num_ports; i++) {
			if (ksi->hki_perfcntr64[i].hki64_enabled) {
				(void) mcxnex_kstat_perfcntr64_read(state,
				    i + 1, 1);
			}
		}
		/* sleep for a second */
...
/*
 * mcxnex_kstat_perfcntr64_thread_create()
 *    Context: Called from the kstat context
 *
 * Create a thread that maintains 64 bit performance counters in software.
 */
static void
mcxnex_kstat_perfcntr64_thread_create(mcxnex_state_t *state)
{
	mcxnex_ks_info_t	*ksi = state->hs_ks_info;
	kthread_t		*thr;
	...
	/*
	 * One thread per mcxnex instance. Don't create a thread if already
	 * created.
	 */
	if (!(ksi->hki_perfcntr64_flags & MCXNEX_PERFCNTR64_THREAD_CREATED)) {
		thr = thread_create(NULL, 0,
		    mcxnex_kstat_perfcntr64_update_thread,
		    state, 0, &p0, TS_RUN, minclsyspri);
		ksi->hki_perfcntr64_thread_id = thr->t_did;
		ksi->hki_perfcntr64_flags |= MCXNEX_PERFCNTR64_THREAD_CREATED;
	}
}

/*
 * mcxnex_kstat_perfcntr64_thread_exit()
 *    Context: Called from attach, detach or kstat context
 */
static void
mcxnex_kstat_perfcntr64_thread_exit(mcxnex_ks_info_t *ksi)
{
	kt_did_t	tid;
	...
	if (ksi->hki_perfcntr64_flags & MCXNEX_PERFCNTR64_THREAD_CREATED) {
		 ...
		 */
		ksi->hki_perfcntr64_flags |= MCXNEX_PERFCNTR64_THREAD_EXIT;
...
/*
 * mcxnex_kstat_perfcntr64_update_ext()
 *    Context: Called from the kstat context
 *
 ...
 */
static int
mcxnex_kstat_perfcntr64_update_ext(mcxnex_perfcntr64_ks_info_t *ksi64, int rw,
    struct kstat_named *data)
{
	mcxnex_hw_sm_extperfcntr_t	sm_extperfcntr;
	 ...
	 */
	if (mcxnex_getextperfcntr_cmd_post(ksi64->hki64_state,
	    ksi64->hki64_port_num, DDI_NOSLEEP, &sm_extperfcntr) !=
	    MCXNEX_CMD_SUCCESS) {
		return (EIO);
	}

	data[MCXNEX_PERFCNTR64_ENABLE_IDX].value.ui32 = 1;

	data[MCXNEX_PERFCNTR64_XMIT_DATA_IDX].value.ui64 =
	    sm_extperfcntr.portxmdata;

	data[MCXNEX_PERFCNTR64_RECV_DATA_IDX].value.ui64 =
	    sm_extperfcntr.portrcdata;

	data[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX].value.ui64 =
	    sm_extperfcntr.portxmpkts;

	data[MCXNEX_PERFCNTR64_RECV_PKTS_IDX].value.ui64 =
...
/*
 * mcxnex_kstat_perfcntr64_update()
 *    Context: Called from the kstat context
 *
 * See the general comment on 64 bit kstats for performance counters:
 */
static int
mcxnex_kstat_perfcntr64_update(kstat_t *ksp, int rw)
{
	mcxnex_state_t			*state;
	struct kstat_named		*data;
	mcxnex_ks_info_t		*ksi;
	mcxnex_perfcntr64_ks_info_t	*ksi64;
	int				i, thr_exit;
	int				rv;
	...
	if (ksi64->hki64_ext_port_counters_supported) {
		rv = mcxnex_kstat_perfcntr64_update_ext(ksi64, rw, data);
		mutex_exit(&ksi->hki_perfcntr64_lock);
		return (rv);
	}
	 ...
	 */
	if (rw == KSTAT_WRITE) {
		if (data[MCXNEX_PERFCNTR64_ENABLE_IDX].value.ui32) {
			if (ksi64->hki64_enabled == 0) {
				 ...
				 * mcxnex_kstat_perfcntr64_update_thread.
				 */
				if (mcxnex_getperfcntr_cmd_post(state,
				...
				for (i = 0;
				    i < MCXNEX_PERFCNTR64_NUM_COUNTERS; i++) {
					ksi64->hki64_counters[i] = 0;
					ksi64->hki64_last_read[i] = 0;
				}
				mcxnex_kstat_perfcntr64_thread_create(state);
			}
		 ...
		 * Read the counters and update kstats.
		 */
		if (mcxnex_kstat_perfcntr64_read(state, ksi64->hki64_port_num,
		    0) != MCXNEX_CMD_SUCCESS) {
			mutex_exit(&ksi->hki_perfcntr64_lock);
			return (EIO);
		}
		...
		data[MCXNEX_PERFCNTR64_XMIT_DATA_IDX].value.ui64 =
		    ksi64->hki64_counters[MCXNEX_PERFCNTR64_XMIT_DATA_IDX] +
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_XMIT_DATA_IDX];

		data[MCXNEX_PERFCNTR64_RECV_DATA_IDX].value.ui64 =
		    ksi64->hki64_counters[MCXNEX_PERFCNTR64_RECV_DATA_IDX] +
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_RECV_DATA_IDX];

		data[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX].value.ui64 =
		    ksi64->hki64_counters[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX] +
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_XMIT_PKTS_IDX];

		data[MCXNEX_PERFCNTR64_RECV_PKTS_IDX].value.ui64 =
		    ksi64->hki64_counters[MCXNEX_PERFCNTR64_RECV_PKTS_IDX] +
		    ksi64->hki64_last_read[MCXNEX_PERFCNTR64_RECV_PKTS_IDX];

	} else {
		/* return 0 in kstats if not enabled */
		data[MCXNEX_PERFCNTR64_ENABLE_IDX].value.ui32 = 0;
		for (i = 1; i < MCXNEX_PERFCNTR64_NUM_COUNTERS; i++)
			data[i].value.ui64 = 0;
	}

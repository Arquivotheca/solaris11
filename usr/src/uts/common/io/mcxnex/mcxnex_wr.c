...
static uint32_t mcxnex_wr_get_immediate(ibt_send_wr_t *wr);
static int mcxnex_wr_bind_check(mcxnex_state_t *state, ibt_send_wr_t *wr);
static int mcxnex_wqe_send_build(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint64_t *desc, uint_t *size);
static int mcxnex_wqe_mlx_build(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint64_t *desc, uint_t *size);
static void mcxnex_wqe_headroom(uint_t from, mcxnex_qphdl_t qp);
static int mcxnex_wqe_recv_build(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_recv_wr_t *wr, uint64_t *desc);
static int mcxnex_wqe_srq_build(mcxnex_state_t *state, mcxnex_srqhdl_t srq,
    ibt_recv_wr_t *wr, uint64_t *desc);
static void mcxnex_wqe_sync(void *hdl, uint_t sync_from,
    uint_t sync_to, uint_t sync_type, uint_t flag);
static mcxnex_workq_avl_t *mcxnex_wrid_wqavl_find(mcxnex_cqhdl_t cq, uint_t qpn,
    uint_t send_or_recv);
static void mcxnex_cq_workq_add(mcxnex_cqhdl_t cq, mcxnex_workq_avl_t *wqavl);
static void mcxnex_cq_workq_remove(mcxnex_cqhdl_t cq,
    mcxnex_workq_avl_t *wqavl);

static	ibt_wr_ds_t	null_sgl = { 0, 0x00000100, 0 };

static int
mcxnex_post_send_ud(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint_t num_wr, uint_t *num_posted)
{
	mcxnex_hw_snd_wqe_ud_t		*ud;
...
post_next:
	...
	desc = MCXNEX_QP_SQ_ENTRY(qp, tail);

	ud = (mcxnex_hw_snd_wqe_ud_t *)((uintptr_t)desc +
	    sizeof (mcxnex_hw_snd_wqe_ctrl_t));
	ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)ud +
	    sizeof (mcxnex_hw_snd_wqe_ud_t));
	nds = wr->wr_nds;
	sgl = wr->wr_sgl;
	num_ds = 0;
	...
	if (wr->wr_opcode == IBT_WRC_SEND_LSO) {
		...
		ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)ds + total_len);
		i = 0;
	} else if (wr->wr_opcode == IBT_WRC_SEND) {
		if (wr->wr_flags & IBT_WR_SEND_IMMED) {
			nopcode = MCXNEX_WQE_SEND_NOPCODE_SENDI;
			immed_data = wr->wr.ud.udwr_immed;
		} else {
			nopcode = MCXNEX_WQE_SEND_NOPCODE_SEND;
		}
	...
	for (j = nds; --j >= i; ) {
		 ...
		 */
		last_ds--;
		MCXNEX_WQE_BUILD_DATA_SEG_SEND(&ds[last_ds], &sgl[j]);
	}

	membar_producer();

	if (wr->wr_opcode == IBT_WRC_SEND_LSO) {
		MCXNEX_WQE_BUILD_LSO(qp, old_ds, wr->wr.ud_lso.lso_mss,
		    wr->wr.ud_lso.lso_hdr_sz);
	}

	fence = (wr->wr_flags & IBT_WR_SEND_FENCE) ? 1 : 0;

	signaled_dbd = ((qp->qp_sq_sigtype == MCXNEX_QP_SQ_ALL_SIGNALED) ||
	    (wr->wr_flags & IBT_WR_SEND_SIGNAL)) ? 1 : 0;

	solicited = (wr->wr_flags & IBT_WR_SEND_SOLICIT) ? 1 : 0;

	MCXNEX_WQE_SET_CTRL_SEGMENT(desc, desc_sz, fence, immed_data,
	...
	MCXNEX_SET_SEND_WQE_OWNER(qp, (uint32_t *)desc, nopcode);

	posted_cnt++;
	if (--num_wr > 0) {
		/* do the invalidate of the headroom */
		wqe_start = (uint32_t *)MCXNEX_QP_SQ_ENTRY(qp,
	...
	if (posted_cnt != 0) {
		...
		MCXNEX_UAR_DOORBELL(state, uarhdl,
...
static int
mcxnex_post_send_mlx(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint_t num_wr, uint_t *num_posted)
{
	mcxnex_workq_hdr_t		*wq;
	uint64_t			*desc;
...
post_next:
	...
	ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)desc +
	    sizeof (mcxnex_hw_snd_wqe_ctrl_t));
	nds = wr->wr_nds;
	sgl = wr->wr_sgl;
	num_ds = 0;
	 ...
	 */
	if (wr->wr_opcode == IBT_WRC_SEND_LSO) {
		int total_len, copy_len, mblen;
		mblk_t *bp;
		uint8_t *pdst;

		nopcode = MCXNEX_WQE_SEND_NOPCODE_LSO;
		/* always post re-read bit for lso */
		nopcode |= (1 << 6);
		...
		pdst = (uint8_t *)old_ds;
		pdst += 4;
		bp = (mblk_t *)(void *)wr->wr.ud_lso.lso_hdr;
		for (copy_len = wr->wr.ud_lso.lso_hdr_sz; copy_len != 0;
		    bp = bp->b_cont) {
			mblen = MBLKL(bp);
			if (copy_len > mblen) {
				bcopy(bp->b_rptr, pdst, mblen);
				copy_len -= mblen;
				pdst += mblen;
			} else {
				bcopy(bp->b_rptr, pdst, copy_len);
				copy_len = 0;
			}
		}
	...
	for (last_ds = num_ds, j = 0; j < nds; j++) {
		if (sgl[j].ds_len != 0)
			last_ds++;	/* real last ds of wqe to fill */
	}
...
static int
mcxnex_post_send_rc(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint_t num_wr, uint_t *num_posted)
{
	uint64_t			*desc;
	mcxnex_workq_hdr_t		*wq;
	...
	int				status;
	uint32_t			nopcode, fence, immed_data = 0;
	mcxnex_hw_snd_wqe_remaddr_t	*rc;
	mcxnex_hw_snd_wqe_atomic_t	*at;
	mcxnex_hw_snd_wqe_bind_t	*bn;
	mcxnex_hw_wqe_sgl_t		*ds;
	ibt_wr_ds_t			*sgl;
	uint32_t			nds;
...
post_next:
	...
	case IBT_WRC_RDMAW:
		if (wr->wr_flags & IBT_WR_SEND_IMMED) {
			nopcode = MCXNEX_WQE_SEND_NOPCODE_RDMAWI;
			immed_data = wr->wr.rc.rcwr.rdma.rdma_immed;
		} else {
			nopcode = MCXNEX_WQE_SEND_NOPCODE_RDMAW;
		}
		/* FALLTHROUGH */
	case IBT_WRC_RDMAR:
		if (wr->wr_opcode == IBT_WRC_RDMAR)
			nopcode = MCXNEX_WQE_SEND_NOPCODE_RDMAR;
		rc = (mcxnex_hw_snd_wqe_remaddr_t *)((uintptr_t)desc +
		    sizeof (mcxnex_hw_snd_wqe_ctrl_t));
		 ...
		 */
		MCXNEX_WQE_BUILD_REMADDR(qp, rc, &wr->wr.rc.rcwr.rdma);

		/* Update "ds" for filling in Data Segments (below) */
		ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)rc +
		    sizeof (mcxnex_hw_snd_wqe_remaddr_t));
		break;
	 ...
	 */
	case IBT_WRC_CSWAP:
		nopcode = MCXNEX_WQE_SEND_NOPCODE_ATMCS;
		/* FALLTHROUGH */
	case IBT_WRC_FADD:
		if (wr->wr_opcode == IBT_WRC_FADD)
			nopcode = MCXNEX_WQE_SEND_NOPCODE_ATMFA;
		rc = (mcxnex_hw_snd_wqe_remaddr_t *)((uintptr_t)desc +
		    sizeof (mcxnex_hw_snd_wqe_ctrl_t));
		at = (mcxnex_hw_snd_wqe_atomic_t *)((uintptr_t)rc +
		    sizeof (mcxnex_hw_snd_wqe_remaddr_t));
		 ...
		 * work request.
		 */
		MCXNEX_WQE_BUILD_RC_ATOMIC_REMADDR(qp, rc, wr);
		MCXNEX_WQE_BUILD_ATOMIC(qp, at, wr->wr.rc.rcwr.atomic);

		/* Update "ds" for filling in Data Segments (below) */
		ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)at +
		    sizeof (mcxnex_hw_snd_wqe_atomic_t));
	 ...
	 * mcxnex_wr_bind_check() routine to validate the request and
	 * to generate the updated RKey.  If this is successful, then
	 * we fill in the WQE's "Bind" header fields.
	 */
	case IBT_WRC_BIND:
		nopcode = MCXNEX_WQE_SEND_NOPCODE_BIND;
		status = mcxnex_wr_bind_check(state, wr);
		if (status != DDI_SUCCESS)
			goto done;

		bn = (mcxnex_hw_snd_wqe_bind_t *)((uintptr_t)desc +
		    sizeof (mcxnex_hw_snd_wqe_ctrl_t));
		 ...
		 * window work request.
		 */
		MCXNEX_WQE_BUILD_BIND(qp, bn, wr->wr.rc.rcwr.bind);
		 ...
		 * (below).
		 */
		ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)bn +
		    sizeof (mcxnex_hw_snd_wqe_bind_t));
		nds = 0;
	}
	...
	for (i = nds; --i >= 0; ) {
		 ...
		 */
		last_ds--;
		MCXNEX_WQE_BUILD_DATA_SEG_SEND(&ds[last_ds], &sgl[i]);
	}
	...
	MCXNEX_WQE_SET_CTRL_SEGMENT(desc, desc_sz, fence, immed_data, solicited,
 ...
 */
int
mcxnex_post_send(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint_t num_wr, uint_t *num_posted)
{
	...
	if ((qp->qp_state == MCXNEX_QP_RESET) ||
	    (qp->qp_state == MCXNEX_QP_INIT) ||
	    (qp->qp_state == MCXNEX_QP_RTR)) {
	...
	if (qp->qp_serv_type == MCXNEX_QP_UD)
		return (mcxnex_post_send_ud(state, qp, wr,
		    num_wr, num_posted));

	if (qp->qp_serv_type == MCXNEX_QP_MLX)
		return (mcxnex_post_send_mlx(state, qp, wr,
		    num_wr, num_posted));

	if (qp->qp_serv_type == MCXNEX_QP_RC)
		return (mcxnex_post_send_rc(state, qp, wr,
		    num_wr, num_posted));

	if (qp->qp_serv_type == MCXNEX_QP_UC)
		goto post_many;
...
post_many:
	 ...
	 */
		 ...
		 */
		chainlen = (numremain > MCXNEX_QP_MAXDESC_PER_DB) ?
		    MCXNEX_QP_MAXDESC_PER_DB : numremain;
		numremain -= chainlen;
		...
		for (currindx = 0; currindx < chainlen; currindx++, wrindx++) {
			...
			desc = MCXNEX_QP_SQ_ENTRY(qp, tail);
			/*
			 * Call mcxnex_wqe_send_build() to build the WQE
			 ...
			 */
			status = mcxnex_wqe_send_build(state, qp,
			...
			case IBT_WRC_RDMAW:
				if (curr_wr->wr_flags & IBT_WR_SEND_IMMED) {
					nopcode =
					    MCXNEX_WQE_SEND_NOPCODE_RDMAWI;
					immed_data =
					    mcxnex_wr_get_immediate(curr_wr);
				} else {
					nopcode = MCXNEX_WQE_SEND_NOPCODE_RDMAW;
				}
				break;
			...
			if ((qp->qp_sq_sigtype == MCXNEX_QP_SQ_ALL_SIGNALED) ||
			...
			if (qp->qp_is_special) {
				...
				MCXNEX_WQE_SET_MLX_CTRL_SEGMENT(desc, desc_sz,
				    signaled_dbd, maxstat, ah->ah_udav->rlid,
				    qp, ah->ah_udav->sl);
				mutex_exit(&ah->ah_lock);
			} else {
				MCXNEX_WQE_SET_CTRL_SEGMENT(desc, desc_sz,
		...
		if (posted_cnt != 0) {
			...
			mcxnex_wqe_headroom(tail, qp);

			/* Do a DMA sync for current send WQE(s) */
			mcxnex_wqe_sync(qp, sync_from, sync_to, MCXNEX_WR_SEND,
			    DDI_DMA_SYNC_FORDEV);
			 ...
			 * one in the chain
			 */
			MCXNEX_SET_SEND_WQE_OWNER(qp, (uint32_t *)prev,
			    prev_nopcode);

			/* the FMA retry loop starts for Mcxnex doorbell. */
			mcxnex_pio_start(state, uarhdl, pio_error, fm_loop_cnt,
			    fm_status, fm_test);
 ...
 */
int
mcxnex_post_recv(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_recv_wr_t *wr, uint_t num_wr, uint_t *num_posted)
{
	...
	for (wrindx = 0; wrindx < num_wr; wrindx++) {
		...
		desc = MCXNEX_QP_RQ_ENTRY(qp, tail);
		status = mcxnex_wqe_recv_build(state, qp, &wr[wrindx], desc);
		if (status != DDI_SUCCESS) {
			break;
		}
	...
	if (posted_cnt != 0) {
		...
		mcxnex_wqe_sync(qp, sync_from, sync_to, MCXNEX_WR_RECV,
		    DDI_DMA_SYNC_FORDEV);
		...
		MCXNEX_UAR_DB_RECORD_WRITE(qp->qp_rq_vdbr,
 ...
 */
int
mcxnex_post_srq(mcxnex_state_t *state, mcxnex_srqhdl_t srq,
    ibt_recv_wr_t *wr, uint_t num_wr, uint_t *num_posted)
{
	uint64_t			*desc;
	mcxnex_workq_hdr_t		*wq;
	 ...
	 */
	if (srq->srq_state == MCXNEX_SRQ_STATE_ERROR) {
		mutex_exit(&srq->srq_lock);
		return (IBT_QP_STATE_INVALID);
	}
	...
	for (wrindx = 0; wrindx < num_wr; wrindx++) {
		...
		desc = MCXNEX_SRQ_WQE_ADDR(srq, indx);

		wq->wq_wrid[indx] = wr[wrindx].wr_id;

		status = mcxnex_wqe_srq_build(state, srq, &wr[wrindx], desc);
		if (status != DDI_SUCCESS) {
			break;
		}

		mcxnex_wqe_sync(srq, indx, indx + 1,
		    MCXNEX_WR_SRQ, DDI_DMA_SYNC_FORDEV);
		posted_cnt++;
	...
	if (posted_cnt != 0) {
		...
		MCXNEX_UAR_DB_RECORD_WRITE(srq->srq_wq_vdbr,
...
/*
 * mcxnex_wqe_send_build()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_wqe_send_build(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint64_t *desc, uint_t *size)
{
	mcxnex_hw_snd_wqe_ud_t		*ud;
	mcxnex_hw_snd_wqe_remaddr_t	*rc;
	mcxnex_hw_snd_wqe_atomic_t	*at;
	mcxnex_hw_snd_wqe_remaddr_t	*uc;
	mcxnex_hw_snd_wqe_bind_t	*bn;
	mcxnex_hw_wqe_sgl_t		*ds, *old_ds;
	ibt_ud_dest_t			*dest;
	ibt_wr_ds_t			*sgl;
	...
	case IBT_UD_SRV:
		 ...
		 * build MLX WQEs instead.  So jump to mcxnex_wqe_mlx_build()
		...
		if (qp->qp_is_special) {
			...
			status = mcxnex_wqe_mlx_build(state, qp,
			    wr, desc, size);
			return (status);
		}
		 ...
		 * all the fields in the Mcxnex UD header for the WQE.  Note:
		...
		/* mutex_enter(&ah->ah_lock); */
		if (wr->wr_opcode == IBT_WRC_SEND) {
			MCXNEX_WQE_BUILD_UD(qp, ud, ah, wr->wr.ud.udwr_dest);
		} else {	/* IBT_WRC_SEND_LSO */
			MCXNEX_WQE_BUILD_UD(qp, ud, ah,
		...
		if (wr->wr_opcode == IBT_WRC_SEND_LSO) {
			...
			for (; i < nds; i++) {
				if (sgl[i].ds_len == 0)
					continue;
				MCXNEX_WQE_BUILD_DATA_SEG_SEND(&ds[num_ds],
				    &sgl[i]);
				num_ds++;
				i++;
				break;
	...
	case IBT_UC_SRV:
		...
		if (wr->wr_opcode == IBT_WRC_RDMAW) {
			uc = (mcxnex_hw_snd_wqe_remaddr_t *)((uintptr_t)desc +
			    sizeof (mcxnex_hw_snd_wqe_ctrl_t));
			 ...
			 */
			MCXNEX_WQE_BUILD_REMADDR(qp, uc, &wr->wr.uc.ucwr.rdma);

			/* Update "ds" for filling in Data Segments (below) */
			ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)uc +
			    sizeof (mcxnex_hw_snd_wqe_remaddr_t));
			break;
		}
		...
		if (wr->wr_opcode == IBT_WRC_BIND) {
			 ...
			 * window work request.
			 */
			MCXNEX_WQE_BUILD_BIND(qp, bn, wr->wr.uc.ucwr.bind);
	 ...
	 * segments.  Note: We skip any SGL with zero size because Mcxnex
	 ...
	 * For Mcxnex, we want them (for now) to be on stride size
...
/*
 * mcxnex_wqe_mlx_build()
 *    Context: Can be called from interrupt or base context.
 */
static int
mcxnex_wqe_mlx_build(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_send_wr_t *wr, uint64_t *desc, uint_t *size)
{
	mcxnex_ahhdl_t		ah;
	...
	if ((udav->rlid == IB_LID_PERMISSIVE) &&
	    (qp->qp_is_special == MCXNEX_QP_GSI)) {
		mutex_exit(&ah->ah_lock);
		return (IBT_AH_HDL_INVALID);
	}
	 ...
	 */
	MCXNEX_WQE_BUILD_INLINE(qp, &ds[0], desc_sz);
	desc_sz += 4;
	...
	MCXNEX_WQE_BUILD_MLX_LRH(lrh, qp, udav, pktlen);
	...
	if (udav->grh) {
		...
		MCXNEX_WQE_BUILD_MLX_GRH(state, grh, qp, udav, pktlen);
	 ...
	 */
	MCXNEX_WQE_BUILD_MLX_BTH(state, bth, qp, wr);

	/*
	 * Build Datagram Extended Transport Header (DETH)
	 */
	deth = (ib_deth_hdr_t *)((uintptr_t)bth + sizeof (ib_bth_hdr_t));
	MCXNEX_WQE_BUILD_MLX_DETH(deth, qp);

	/* Ensure that the Data Segment is aligned on a 16-byte boundary */
	ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)deth + sizeof (ib_deth_hdr_t));
	ds = (mcxnex_hw_wqe_sgl_t *)(((uintptr_t)ds + 0xF) & ~0xF);
	nds = wr->wr_nds;
	sgl = wr->wr_sgl;
	num_ds = 0;
	 ...
	 * MCXNEX_WQE_SGL_BYTE_CNT_MASK (so that 2GB will end up encoded as
	 * zero.)
	 */
	...
	for (i = 0; i < nds; i++) {
		 ...
		 * the work request.
		 */
		MCXNEX_WQE_BUILD_DATA_SEG_SEND(&ds[num_ds], &sgl[i]);

		/*
		 * Search through the contents of all MADs posted to QP0 to
		 * initialize pointers to the places where Directed Route "hop
		 * pointer", "hop count", and "mgmtclass" would be.  Mcxnex
		 * needs these updated (i.e. incremented or decremented, as
		 * necessary) by software.
		 */
		if (qp->qp_is_special == MCXNEX_QP_SMI) {

			MCXNEX_SPECIAL_QP_DRMAD_GET_MGMTCLASS(mgmtclass,
			    offset, sgl[i].ds_va, sgl[i].ds_len);

			MCXNEX_SPECIAL_QP_DRMAD_GET_HOPPOINTER(hpoint,
			    offset, sgl[i].ds_va, sgl[i].ds_len);

			MCXNEX_SPECIAL_QP_DRMAD_GET_HOPCOUNT(hcount,
			    offset, sgl[i].ds_va, sgl[i].ds_len);
	...
	/*
	 * Mcxnex's Directed Route MADs need to have the "hop pointer"
	 ...
	 */
	if (qp->qp_is_special == MCXNEX_QP_SMI) {
		MCXNEX_SPECIAL_QP_DRMAD_DO_HOPPOINTER_MODIFY(*mgmtclass,
		    *hpoint, *hcount);
	}
	 ...
	 */
	MCXNEX_WQE_BUILD_INLINE_ICRC(qp, &ds[num_ds], 4, 0);
	num_ds++;
...
/*
 * mcxnex_wqe_recv_build()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static int
mcxnex_wqe_recv_build(mcxnex_state_t *state, mcxnex_qphdl_t qp,
    ibt_recv_wr_t *wr, uint64_t *desc)
{
	mcxnex_hw_wqe_sgl_t	*ds;
	int			i, num_ds;
	 ...
	 * recv queue for mcxnex, but will need to put an invalid
	...
	for (i = 0; i < wr->wr_nds; i++) {
		 ...
		 * work request.
		 */
		MCXNEX_WQE_BUILD_DATA_SEG_RECV(&ds[num_ds], &wr->wr_sgl[i]);
		num_ds++;
	}

	/* put the null sgl pointer as well if needed */
	if (num_ds < qp->qp_rq_sgl) {
		MCXNEX_WQE_BUILD_DATA_SEG_RECV(&ds[num_ds], &null_sgl);
	}
...
/*
 * mcxnex_wqe_srq_build()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static int
mcxnex_wqe_srq_build(mcxnex_state_t *state, mcxnex_srqhdl_t srq,
    ibt_recv_wr_t *wr, uint64_t *desc)
{
	mcxnex_hw_wqe_sgl_t	*ds;
	int			i, num_ds;
	...
	ds = (mcxnex_hw_wqe_sgl_t *)((uintptr_t)desc +
	    sizeof (mcxnex_hw_srq_wqe_next_t));
	num_ds = 0;
...
/*
 * mcxnex_wr_get_immediate()
 *    Context: Can be called from interrupt or base context.
 */
static uint32_t
mcxnex_wr_get_immediate(ibt_send_wr_t *wr)
{
	/*
...
static void
mcxnex_wqe_headroom(uint_t from, mcxnex_qphdl_t qp)
{
	...
	wqe_base  = (uint32_t *)MCXNEX_QP_SQ_ENTRY(qp, 0);
	wqe_top	  = (uint32_t *)MCXNEX_QP_SQ_ENTRY(qp, qsize);
	wqe_start = (uint32_t *)MCXNEX_QP_SQ_ENTRY(qp, from);
 ...
 */
static void
mcxnex_wqe_sync(void *hdl, uint_t sync_from, uint_t sync_to,
    uint_t sync_type, uint_t flag)
{
	...
	if (sync_type == MCXNEX_WR_SRQ) {
		srq = (mcxnex_srqhdl_t)hdl;
	...
	if (sync_type == MCXNEX_WR_SEND) {
		wqe_from = MCXNEX_QP_SQ_ENTRY(qp, sync_from);
		wqe_to   = MCXNEX_QP_SQ_ENTRY(qp, sync_to);
		qsize	 = qp->qp_sq_bufsz;

		wq_base = MCXNEX_QP_SQ_ENTRY(qp, 0);
		wq_top	 = MCXNEX_QP_SQ_ENTRY(qp, qsize);
	} else if (sync_type == MCXNEX_WR_RECV) {
		wqe_from = MCXNEX_QP_RQ_ENTRY(qp, sync_from);
		wqe_to   = MCXNEX_QP_RQ_ENTRY(qp, sync_to);
		qsize	 = qp->qp_rq_bufsz;

		wq_base = MCXNEX_QP_RQ_ENTRY(qp, 0);
		wq_top	 = MCXNEX_QP_RQ_ENTRY(qp, qsize);
	} else {
		wqe_from = MCXNEX_SRQ_WQ_ENTRY(srq, sync_from);
		wqe_to   = MCXNEX_SRQ_WQ_ENTRY(srq, sync_to);
		qsize	 = srq->srq_wq_bufsz;

		wq_base = MCXNEX_SRQ_WQ_ENTRY(srq, 0);
		wq_top	 = MCXNEX_SRQ_WQ_ENTRY(srq, qsize);
	}
...
/*
 * mcxnex_wr_bind_check()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
static int
mcxnex_wr_bind_check(mcxnex_state_t *state, ibt_send_wr_t *wr)
{
	...
	mw = (mcxnex_mwhdl_t)wr->wr.rc.rcwr.bind->bind_ibt_mw_hdl;
	if (mw == NULL) {
		return (IBT_MW_HDL_INVALID);
	}

	/* Check for a valid Memory Region handle in the WR */
	mr = (mcxnex_mrhdl_t)wr->wr.rc.rcwr.bind->bind_ibt_mr_hdl;
	if (mr == NULL) {
	...
	mpt = mw->mr_mptrsrcp;
	new_rkey = mcxnex_mr_keycalc(mpt->hr_indx);
	new_rkey = mcxnex_mr_key_swap(new_rkey);
...
/*
 * mcxnex_wrid_from_reset_handling()
 *    Context: Can be called from interrupt or base context.
 */
/* ARGSUSED */
int
mcxnex_wrid_from_reset_handling(mcxnex_state_t *state, mcxnex_qphdl_t qp)
{
	mcxnex_workq_hdr_t	*swq, *rwq;
	uint_t			qp_srq_en;
	...
	mcxnex_cq_workq_add(qp->qp_sq_cqhdl, &qp->qp_sq_wqavl);
...
#endif
	mcxnex_cq_workq_add(qp->qp_rq_cqhdl, &qp->qp_rq_wqavl);
...
/*
 * mcxnex_wrid_to_reset_handling()
 *    Context: Can be called from interrupt or base context.
 */
int
mcxnex_wrid_to_reset_handling(mcxnex_state_t *state, mcxnex_qphdl_t qp)
{
	uint_t			qp_srq_en;
	 ...
	 */
	mcxnex_cq_entries_flush(state, qp);
	...
	mcxnex_cq_workq_remove(qp->qp_rq_cqhdl, &qp->qp_rq_wqavl);
	mcxnex_cq_workq_remove(qp->qp_sq_cqhdl, &qp->qp_sq_wqavl);
...
/*
 * mcxnex_wrid_get_entry()
 *    Context: Can be called from interrupt or base context.
 */
uint64_t
mcxnex_wrid_get_entry(mcxnex_cqhdl_t cq, mcxnex_hw_cqe_t *cqe)
{
	mcxnex_workq_avl_t	*wqa;
	mcxnex_workq_hdr_t	*wq;
	uint64_t		wrid;
	 ...
	 * entry on success.
	 */
	indx = MCXNEX_CQE_WQEADDRSZ_GET(cq, cqe) & wq->wq_mask;
	wrid = wq->wq_wrid[indx];
	if (wqa->wqa_srq_en) {
		struct mcxnex_sw_srq_s	*srq;
		uint64_t		*desc;
...
int
mcxnex_wrid_workq_compare(const void *p1, const void *p2)
{
	mcxnex_workq_compare_t	*cmpp;
	mcxnex_workq_avl_t	*curr;

	cmpp = (mcxnex_workq_compare_t *)p1;
	curr = (mcxnex_workq_avl_t *)p2;
...
/*
 * mcxnex_wrid_workq_find()
 *    Context: Can be called from interrupt or base context.
 */
static mcxnex_workq_avl_t *
mcxnex_wrid_wqavl_find(mcxnex_cqhdl_t cq, uint_t qpn, uint_t wq_type)
{
	mcxnex_workq_avl_t	*curr;
	mcxnex_workq_compare_t	cmp;
...
#ifdef __lock_lint
	mcxnex_wrid_workq_compare(NULL, NULL);
#endif
...
/*
 * mcxnex_wrid_wqhdr_create()
 *    Context: Can be called from base context.
 */
/* ARGSUSED */
mcxnex_workq_hdr_t *
mcxnex_wrid_wqhdr_create(int bufsz)
{
	mcxnex_workq_hdr_t	*wqhdr;

	/*
	 * Allocate space for the wqhdr, and an array to record all the wrids.
	 */
	wqhdr = (mcxnex_workq_hdr_t *)kmem_zalloc(sizeof (*wqhdr), KM_NOSLEEP);
	if (wqhdr == NULL) {
		return (NULL);
	}
...
void
mcxnex_wrid_wqhdr_destroy(mcxnex_workq_hdr_t *wqhdr)
{
 ...
 */
static void
mcxnex_cq_workq_add(mcxnex_cqhdl_t cq, mcxnex_workq_avl_t *wqavl)
{
	mcxnex_workq_compare_t	cmp;
	avl_index_t		where;
...
/*
 * mcxnex_cq_workq_remove()
 *    Context: Can be called from interrupt or base context.
 */
static void
mcxnex_cq_workq_remove(mcxnex_cqhdl_t cq, mcxnex_workq_avl_t *wqavl)
{
#ifdef __lock_lint
	mcxnex_wrid_workq_compare(NULL, NULL);
#endif

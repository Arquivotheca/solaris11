...
/*
 * mcxnex_fm.c
 *    Mcxnex (InfiniBand) HCA Driver Fault Management Routines
 *
 * [Mcxnex FM Implementation]
 *
 * Mcxnex FM recovers the system from a HW error situation and/or isolates a
 ...
 * isolate the faulted HW, Mcxnex FM issues Solaris FMA ereports as follows.
 *
 *  * PIO transient error
 ...
 *
 *  * Mcxnex HCA firmware error
 *         invalid_state => degraded
 *
 *  * Other Mcxnex HCA specific errors
 ...
 *
 * See the Mcxnex FMA portfolio in detail.
 */
...
/*
 * Mcxnex driver has to disable its FM functionality
 * if this "fm_capable" variable is defined or has a value
 * in /kernel/drv/mcxnex.conf.
 */
static char *fm_cap = "fm-capable";	/* FM capability */

static mcxnex_hca_fm_t hca_fm;		/* Mcxnex HCA FM Structure */
...
/* forward declaration for mcxnex_fm_{init, fini}() */
#ifdef FMA_TEST
 ...
 * defined below, but specific to the Mcxnex HCA FM capabilities.
 */

/*
 *  void
 *  mcxnex_hca_fm_init(mcxnex_state_t *state, mcxnex_hca_fm_t *hca)
 *
 *  Overview
 *      mcxnex_hca_fm_init() initializes the FM resources.
 *
 *  Argument
 *      state: pointer to state structure
 *      hca: pointer to FM structure
 *
 *  Return value
 *      Nothing
 *
 *  Caller's context
 *      Can be called in user or kernel context only.
 */
static void
mcxnex_hca_fm_init(mcxnex_state_t *state, mcxnex_hca_fm_t *hca_fm)
{
...
/*
 *  void
 *  mcxnex_hca_fm_fini(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_hca_fm_fini() releases the FM resources.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *
 *  Return value
 *      Nothing
 *
...
/*
 *  void
 *  mcxnex_clr_state_nolock(mcxnex_state_t *state, int fm_state)
 *
 *  Overview
 *      mcxnex_clr_state() drops the specified state from Mcxnex FM state
 *      without the mutex locks.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *      fm_state: Mcxnex FM state, which is composed of:
 ...
 *
 *  Caller's context
 *      mcxnex_clr_state() can be called in user, kernel, interrupt context
...
/*
 *  void
 *  mcxnex_clr_state(mcxnex_state_t *state, int fm_state)
 *
 *  Overview
 *      mcxnex_clr_state() drops the specified state from Mcxnex FM state.
 *
 ...
 *
 *  Caller's context
 *      mcxnex_clr_state() can be called in user, kernel or interrupt context.
 */
static void
mcxnex_clr_state(mcxnex_state_t *state, int fm_state)
{
	...
	mcxnex_clr_state_nolock(state, fm_state);
...
/*
 *  void
 *  mcxnex_set_state(mcxnex_state_t *state, int fm_state)
 *
 *  Overview
 *      mcxnex_set_state() sets Mcxnex FM state.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *      fm_state: Mcxnex FM state, which is composed of:
 ...
 *
 *  Caller's context
 *      mcxnex_set_state() can be called in user, kernel or interrupt context.
 */
static void
mcxnex_set_state(mcxnex_state_t *state, int fm_state)
{
...
/*
 *  int
 *  mcxnex_get_state(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_get_state() returns the current Mcxnex FM state.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *
 ...
 *
 *  Caller's context
 *      mcxnex_get_state() can be called in user, kernel or interrupt context.
 */
int
mcxnex_get_state(mcxnex_state_t *state)
{
...
/*
 *  void
 *  mcxnex_fm_init(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_fm_init() is an FM initialization function which registers
 *      some FMA functions such as the ereport and the acc check capabilities.
 *      If the "fm_disable" property in /kernel/drv/mcxnex.conf is
 *      defined (and/or its value is set), then the FM capabilities will
...
mcxnex_fm_init(mcxnex_state_t *state)
{
	 ...
	 * use the Solaris FMA default action for Mcxnex.
	 */
	...
	if (state->hs_fm_disable) {
		mcxnex_set_state(state, HCA_NO_FM);
		return;
	}

	/* Set the Mcxnex FM attach mode */
	mcxnex_set_state(state, HCA_ATTCH_FM);

	/* Initialize the Solaris FMA capabilities for the Mcxnex FM support */
	state->hs_fm_capabilities = ddi_prop_get_int(DDI_DEV_T_ANY,
	    state->hs_dip, DDI_PROP_DONTPASS, fm_cap,
	    DDI_FM_EREPORT_CAPABLE | DDI_FM_ACCCHK_CAPABLE);

	/*
	 * The Mcxnex FM uses the ereport and acc check capabilites only,
	...
	if (state->hs_fm_capabilities !=
		...
		MCXNEX_ATTACH_MSG(state->hs_attach_buf,
		    "Mcxnex FM capability fails");
		return;
	}

	/* Initialize the HCA FM resources */
	mcxnex_hca_fm_init(state, &hca_fm);
	...
	/* Set the Mcxnex FM state */
	mcxnex_set_state(state, HCA_PIO_FM | HCA_EREPORT_FM);
...
/*
 *  void
 *  mcxnex_fm_fini(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_fm_fini() is a FM finalization function which de-registers
 *      Solaris FMA functions set to the device.
 *
 *  Argument
 *      state: pointer to state structure
 *
 ...
 *
 *  Caller's context
 *      mcxnex_fm_fini() can be called in user or kernel context only.
 */
void
mcxnex_fm_fini(mcxnex_state_t *state)
{
	/*
	 * If mcxnex_fm_diable is set or there is no FM service provided,
	 * then skip the rest.
	 */
	if (state->hs_fm_disable || mcxnex_get_state(state) == HCA_NO_FM) {
		return;
	}

	ASSERT(!(mcxnex_get_state(state) & HCA_ERRCB_FM));

#ifdef FMA_TEST
	i_hca_test_fini(&state->hs_fm_test_hash, &state->hs_fm_id_hash);
#endif /* FMA_TEST */

	/* Set the Mcxnex FM state to no support */
	mcxnex_set_state(state, HCA_NO_FM);

	/* Release HCA FM resources */
	mcxnex_hca_fm_fini(state);
	...
	/* De-register the Mcxnex FM from the IO fault services */
...
/*
 *  int
 *  mcxnex_fm_ereport_init(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_fm_ereport_init() changes the Mcxnex FM state to the ereport
 ...
 *
 *  Caller's context
 *      mcxnex_fm_ereport_init() can be called in user or kernel context only.
 */
int
mcxnex_fm_ereport_init(mcxnex_state_t *state)
{
	...
	/*
	 * Disable the Mcxnex interrupt after the interrupt capability flag
	 * is checked.
	 */
	...
	/*
	 * Now that the Mcxnex FM uses the ereport capability only,
	...
	if (state->hs_fm_capabilities != DDI_FM_EREPORT_CAPABLE) {
		MCXNEX_ATTACH_MSG(state->hs_attach_buf,
		    "Mcxnex FM ereport fails (ereport mode)");
		goto error;
	}
	...
	/* Allocate the regular access handle for Mcxnex CMD I/O space */
	if (ddi_regs_map_setup(state->hs_dip, MCXNEX_CMD_BAR,
	...
	offset = state->hs_fw.clr_intr_offs & MCXNEX_CMD_OFFSET_MASK;
	...
	/* Allocate the regular access handle for Mcxnex UAR I/O space */
	if (ddi_regs_map_setup(state->hs_dip, MCXNEX_UAR_BAR,
	...
	/* Drop the Mcxnex FM Attach Mode */
	mcxnex_clr_state(state, HCA_ATTCH_FM);

	/* Set the Mcxnex FM Runtime Mode */
	mcxnex_set_state(state, HCA_RUNTM_FM);

	/* Free up Mcxnex UAR page #1 */
	...
	MCXNEX_RSRC_NAME(rsrc_name, MCXNEX_UAR_PAGE_VMEM_RUNTM);
	...
	if (mcxnex_rsrc_hw_entries_init(state, &entry_info) != DDI_SUCCESS) {
	...
	if (mcxnex_rsrc_alloc(state, MCXNEX_UARPG, 1, DDI_SLEEP,
	...
	/* Now drop the the Mcxnex PIO FM */
	mcxnex_clr_state(state, HCA_PIO_FM);
	...
	if (state->hs_fm_pcihdl) {
		mcxnex_regs_map_free(state, &state->hs_fm_pcihdl);
	...
	/* Enable the Mcxnex interrupt again */
	if (state->hs_intrmsi_cap & DDI_INTR_FLAG_BLOCK) {
...
/*
 *  int
 *  mcxnex_regs_map_setup(mcxnex_state_t *state, uint_t rnumber, caddr_t *addrp,
 *	offset_t offset, offset_t len, ddi_device_acc_attr_t *accattrp,
 *	ddi_acc_handle_t *handle)
 *
 *  Overview
 *      This is a wrapper function of i_hca_regs_map_setup() for Mcxnex FM so
 ...
 *      in /kernel/drv/mcxnex.conf, the function calls ddi_regs_map_setup()
 *      directly instead.
 ...
 *
 *  Caller's context
 *      mcxnex_regs_map_setup() can be called in user or kernel context only.
 */
int
mcxnex_regs_map_setup(mcxnex_state_t *state, uint_t rnumber, caddr_t *addrp,
	offset_t offset, offset_t len, ddi_device_acc_attr_t *accattrp,
...
/*
 *  void
 *  mcxnex_regs_map_free(mcxnex_state_t *state, ddi_acc_handle_t *handlep)
 *
 *  Overview
 *      This is a wrapper function of i_hca_regs_map_free() for Mcxnex FM so
 *      that it calls i_hca_regs_map_free() inside after it checks the
 *      "fm_disable" configuration property. If the "fm_disable" is described
 *      in /kernel/drv/mcxnex.conf, the function calls ddi_regs_map_fre()
 ...
 *
 *  Caller's context
 *      mcxnex_regs_map_free() can be called in user or kernel context only.
 *
 *  Note that the handle passed to mcxnex_regs_map_free() is NULL-cleared
 *  after this function is called.
 */
void
mcxnex_regs_map_free(mcxnex_state_t *state, ddi_acc_handle_t *handle)
{
	if (state->hs_fm_disable) {
...
/*
 *  int
 *  mcxnex_pci_config_setup(mcxnex_state_t *state, ddi_acc_handle_t *handle)
 *
 *  Overview
 *      This is a wrapper function of i_hca_pci_config_setup() for Mcxnex FM so
 ...
 *      in /kernel/drv/mcxnex.conf, the function calls pci_config_setup()
 ...
 *
 *  Caller's context
 *      mcxnex_pci_config_setup() can be called in user or kernel context only.
 */
int
mcxnex_pci_config_setup(mcxnex_state_t *state, ddi_acc_handle_t *handle)
{
	if (state->hs_fm_disable) {
		...
		/* Check Mcxnex FM and Solaris FMA capability flags */
		ASSERT((mcxnex_get_state(state) & HCA_PIO_FM &&
		    DDI_FM_ACC_ERR_CAP(ddi_fm_capable(state->hs_dip))) ||
		    (!(mcxnex_get_state(state) & HCA_PIO_FM) &&
		    !DDI_FM_ACC_ERR_CAP(ddi_fm_capable(state->hs_dip))));
...
/*
 *  void
 *  mcxnex_pci_config_teardown(mcxnex_state_t *state, ddi_acc_handle_t *handle)
 *
 *  Overview
 *      This is a wrapper function of i_hca_pci_config_teardown() for Mcxnex
 ...
 *      described in /kernel/drv/mcxnex.conf, the function calls
 ...
 *
 *  Caller's context
 *      mcxnex_pci_config_teardown() can be called in user or kernel context
 *      only.
 */
void
mcxnex_pci_config_teardown(mcxnex_state_t *state, ddi_acc_handle_t *handle)
...
/*
 *  boolean_t
 *  mcxnex_init_failure(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_init_failure() tells if HW errors are detected in
 *      the Mcxnex driver attach.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *
 ...
 *
 *  Caller's context
 *      mcxnex_init_failure() can be called in user, kernel, interrupt
...
/*
 *  void
 *  mcxnex_fm_ereport(mcxnex_state_t *state, int type, int detail)
 *
 *  Overview
 *      mcxnex_fm_ereport() is a Mcxnex FM ereport function used
 *      to issue a Solaris FMA ereport. See Mcxnex FM comments at the
 ...
 * 		HCA_ERR_DEGRADED	Mcxnex driver and/or uDAPL degraded
 * 					due to HW error
 ...
 *
 *  Caller's context
 *      mcxnex_fm_ereport() can be called in user, kernel, interrupt context
 *      or high interrupt context.
 */
void
mcxnex_fm_ereport(mcxnex_state_t *state, int type, int detail)
{
	/*
	 * If mcxnex_fm_diable is set or there is no FM ereport service
	 * provided, then skip the rest.
	 */
	if (state->hs_fm_disable ||
	    !(mcxnex_get_state(state) & HCA_EREPORT_FM)) {
		return;
	}
	...
	case HCA_SYS_ERR:
		...
		case HCA_ERR_FATAL:
			...
			break;
		default:
			cmn_err(CE_WARN, "mcxnex_fm_ereport: Unknown error. "
			    "type = %d, detail = %d\n.", type, detail);
		}
		break;
	...
	case HCA_IBA_ERR:
		...
		case HCA_ERR_TRANSIENT:
			...
			break;
		case HCA_ERR_SRV_LOST:
			cmn_err(CE_WARN, "mcxnex_fm_ereport: not supported "
		...
		case HCA_ERR_FATAL:
			if (mcxnex_get_state(state) & HCA_PIO_FM) {
				...
				ddi_fm_service_impact(state->hs_dip,
				    DDI_SERVICE_LOST);
				cmn_err(CE_PANIC,
				    "Mcxnex Fatal Internal Error. "
				    "Mcxnex state=0x%p", (void *)state);
			}
			break;
		default:
			cmn_err(CE_WARN, "mcxnex_fm_ereport: Unknown error. "
	...
	default:
		cmn_err(CE_WARN, "mcxnex_fm_ereport: Unknown type "
		    "type = %d, detail = %d\n.", type, detail);
		break;
	}
}
...
/*
 *  uchar_t
 *  mcxnex_devacc_attr_version(mcxnex_state_t *)
 *
 *  Overview
 *      mcxnex_devacc_attr_version() returns the ddi device attribute
 *      version.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 ...
 *      	DDI_DEVICE_ATTR_V0	Mcxnex FM disabled
 *      	DDI_DEVICE_ATTR_V1	Mcxnex FM enabled
 *
 *  Caller's context
 *      mcxnex_devacc_attr_version() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 */
ushort_t
mcxnex_devacc_attr_version(mcxnex_state_t *state)
{
	if (state->hs_fm_disable) {
...
/*
 *  uchar_t
 *  mcxnex_devacc_attr_access(mcxnex_state_t *)
 *
 *  Overview
 *      mcxnex_devacc_attr_access() returns devacc_attr_access error
 *      protection types.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 ...
 *      	DDI_DEFAULT_ACC		Mcxnex FM disabled for PIO
 *      	DDI_FLAGERR_ACC		Mcxnex FM enabled for PIO
 *
 *  Caller's context
 *      mcxnex_devacc_attr_access() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 */
uchar_t
mcxnex_devacc_attr_access(mcxnex_state_t *state)
{
	if (state->hs_fm_disable) {
...
/*
 *  int
 *  mcxnex_PIO_start(mcxnex_state_t *state, ddi_acc_handle_t handle,
 *      mcxnex_test_t *tst)
 *
 *  Overview
 *      mcxnex_PIO_start() should be called before Mcxnex driver issues PIOs
 *      against I/O space. If Mcxnex FM is disabled, this function returns
 ...
 *
 *  Caller's context
 *      mcxnex_PIO_start() can be called in user, kernel or interrupt context.
 */
int
mcxnex_PIO_start(mcxnex_state_t *state, ddi_acc_handle_t handle,
    mcxnex_test_t *tst)
...
/*
 *  int
 *  mcxnex_PIO_end(mcxnex_state_t *state, ddi_acc_handle_t handle, int *cnt,
 *      mcxnex_test_t *tst)
 *
 *  Overview
 *      mcxnex_PIO_end() should be called after Mcxnex driver issues PIOs
 *      against I/O space. If Mcxnex FM is disabled, this function returns
 ...
 *
 *  Caller's context
 *      mcxnex_PIO_end() can be called in user, kernel or interrupt context.
 */
int
mcxnex_PIO_end(mcxnex_state_t *state, ddi_acc_handle_t handle, int *cnt,
    mcxnex_test_t *tst)
...
/*
 *  ddi_acc_handle_t
 *  mcxnex_get_cmdhdl(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_get_cmdhdl() returns either the fma-protected access handle or
 *      the regular ddi-access handle depending on the Mcxnex FM state for
 *      Mcxnex command I/O space.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *
 ...
 *
 *  Caller's context
 *      mcxnex_get_cmdhdl() can be called in user, kernel, interrupt context
 *      or high interrupt context.
 */
ddi_acc_handle_t
mcxnex_get_cmdhdl(mcxnex_state_t *state)
{
	return (state->hs_fm_disable || mcxnex_get_state(state) & HCA_PIO_FM ?
...
/*
 *  ddi_acc_handle_t
 *  mcxnex_get_uarhdl(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_get_uarhdl() returns either the fma-protected access handle or
 *      the regular ddi-access handle depending on the Mcxnex FM state for
 *      Mcxnex UAR I/O space.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 *
 ...
 *
 *  Caller's context
 *      mcxnex_get_uarhdl() can be called in user, kernel, interrupt context
 *      or high interrupt context.
 */
ddi_acc_handle_t
mcxnex_get_uarhdl(mcxnex_state_t *state)
...
/*
 *  ddi_acc_handle_t
 *  mcxnex_rsrc_alloc_uarhdl(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_rsrc_alloc_uarhdl() returns either the fma-protected access
 *      handle or the regular ddi-access handle depending on the Mcxnex FM
 *      state for Mcxnex UAR I/O space as well as mcxnex_get_uarhdl(), but
 ...
 *
 *  Caller's context
 *      mcxnex_rsrc_alloc_uarhdl() can be called in user, kernel, interrupt
 *      or high interrupt context.
 */
ddi_acc_handle_t
mcxnex_rsrc_alloc_uarhdl(mcxnex_state_t *state)
{
	return (state->hs_fm_disable || mcxnex_get_state(state) & HCA_ATTCH_FM ?
	    state->hs_fm_uarhdl : state->hs_reg_uarhdl);
}

/*
 *  ddi_acc_handle_t
 *  mcxnex_get_pcihdl(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_get_pcihdl() returns either the fma-protected access
 ...
 *
 *  Caller's context
 *      mcxnex_get_pcihdl() can be called in user, kernel, interrupt
 *      or high interrupt context.
 */
ddi_acc_handle_t
mcxnex_get_pcihdl(mcxnex_state_t *state)
...
/*
 *  ddi_acc_handle_t
 *  mcxnex_get_msix_tblhdl(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_get_msix_tblhdl() returns either the fma-protected access
 ...
 *
 *  Caller's context
 *      mcxnex_get_msix_tblhdl() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 */
ddi_acc_handle_t
mcxnex_get_msix_tblhdl(mcxnex_state_t *state)
...
/*
 *  ddi_acc_handle_t
 *  mcxnex_get_msix_pbahdl(mcxnex_state_t *state)
 *
 *  Overview
 *      mcxnex_get_msix_pbahdl() returns either the fma-protected access
 ...
 *
 *  Caller's context
 *      mcxnex_get_msix_pbahdl() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 */
ddi_acc_handle_t
mcxnex_get_msix_pbahdl(mcxnex_state_t *state)
...
/*
 *  void
 *  mcxnex_inter_err_chk(void *arg)
 *
 *  Overview
 *      mcxnex_inter_err_chk() periodically checks the internal error buffer
 *      to pick up a Mcxnex asynchronous internal error.
 *
 ...
 *      it cannot be raised so that Mcxnex RPM recommeds to poll this internal
 ...
 *
 *  Argument
 *      arg: pointer to Mcxnex state structure
 *
 *  Return value
 *  	Nothing
 *
 *  Caller's context
 *      mcxnex_inter_err_chk() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 *
 */
void
mcxnex_inter_err_chk(void *arg)
{
	...
	/* Get the access handle for Mcxnex CMD I/O space */
	cmdhdl = mcxnex_get_cmdhdl(state);
	...
	if (word != 0) {
		...
		/* if fm_disable is on, FM functions don't work */
		if (state->hs_fm_disable) {
			cmn_err(CE_PANIC,
			    "ConnectX-2 Fatal Internal Error.  state=0x%p",
			    (void *)state);
		} else {
			mcxnex_fm_ereport(state, HCA_IBA_ERR, HCA_ERR_FATAL);
		}
	}
...
/*
 *  boolean_t
 *  mcxnex_cmd_retry_ok(mcxnex_cmd_post_t *cmd, int status)
 *
 *  Overview
 *  	In the case that a HW error is detected, if it can be isolated
 *  	enough, Mcxnex FM retries the operation which caused the error.
 ...
 *  	was set inside the Mcxnex HW already in the previous operation, the
 ...
 *  	but a harmless result. mcxnex_cmd_retry_ok() checks this kind of
 *  	situation then returns if the state Mcxnex CMD returns is OK or not.
 *
 *  Argument
 *      cmd: pointer to mcxnex_cmd_post_t structure
 *      status: Mcxnex CMD status
 *
 *  Return value
 ...
 *
 *  Caller's context
 *      mcxnex_cmd_retry_ok() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 *
 *  Note that status except for MCXNEX_CMD_SUCCESS shouldn't be accepted
...
mcxnex_cmd_retry_ok(mcxnex_cmd_post_t *cmd, int status)
{
	...
	/*
	 * The wrong status such as MCXNEX_CMD_BAD_SYS_STATE or
	 * MCXNEX_CMD_BAD_RES_STATE can return as a side effect
	 * because of the Mcxnex FM operation retry when a PIO
	 * error is detected during the I/O transaction. In the
	 * case, the driver may set the same value in Mcxnex
	 * though it was set already, then Mcxnex returns MCXNEX_
	...
	case INIT_HCA:
		/*
		 * MCXNEX_CMD_BAD_SYS_STATE can be gotten in case of
		 * ICM not mapped or HCA already initialized.
		 */
		if (status == MCXNEX_CMD_BAD_SYS_STATE)
			return (B_TRUE);
		return (B_FALSE);

	case CLOSE_HCA:
		/*
		 * MCXNEX_CMD_BAD_SYS_STATE can be gotten in case of Firmware
	...
	case CLOSE_PORT:
		/*
		 * MCXNEX_CMD_BAD_SYS_STATE can be gotten in case of HCA not
	...
	case SW2HW_MPT:
		/*
		 * MCXNEX_CMD_BAD_RES_STATE can be gotten in case of MPT
		 * entry already in hardware ownership.
		 */
		if (status == MCXNEX_CMD_BAD_RES_STATE)
			return (B_TRUE);
		return (B_FALSE);
	...
	case SW2HW_EQ:
		/*
		 * MCXNEX_CMD_BAD_RES_STATE can be gotten in case of EQ
		 * entry already in hardware ownership.
		 */
		if (status == MCXNEX_CMD_BAD_RES_STATE)
			return (B_TRUE);
	...
	case SW2HW_CQ:
		/*
		 * MCXNEX_CMD_BAD_RES_STATE can be gotten in case of CQ
		 * entry already in hardware ownership.
		 */
		if (status == MCXNEX_CMD_BAD_RES_STATE)
			return (B_TRUE);
	...
	case SW2HW_SRQ:
		/*
		 * MCXNEX_CMD_BAD_RES_STATE can be gotten in case of SRQ
		 * entry already in hardware ownership.
		 */
		if (status == MCXNEX_CMD_BAD_RES_STATE)
			return (B_TRUE);
...
/*
 * Mcxnex FMA test variables
 */
#define	FMA_TEST_HASHSZ	64
int mcxnex_test_num;			/* predefined testset */
...
static void mcxnex_trigger_pio_error(mcxnex_test_t *, ddi_fm_error_t *);

/*
 * Mcxnex FMA Function Test Interface
 */
...
/*
 *  void
 *  mcxnex_trigger_pio_error(mcxnex_test_t *tst, ddi_fm_error_t *derr)
 *
 *  Overview
 *      mcxnex_trigger_pio_error() is a PIO error injection function
 ...
 *
 *  Caller's context
 *      mcxnex_trigger_pio_error() can be called in user, kernel, interrupt
 *      context or high interrupt context.
 */
static void
mcxnex_trigger_pio_error(mcxnex_test_t *tst, ddi_fm_error_t *derr)
{
	mcxnex_state_t *state = (mcxnex_state_t *)tst->private;
...
/*
 *  struct mcxnex_fm_test *
 *  mcxnex_test_register(mcxnex_state_t *state, char *filename, int linenum,
 *      int type)
 *
 *  Overview
 *      mcxnex_test_register() registers a Mcxnex FM test item for the
 *      function test.
 *
 *  Argument
 *      state: pointer to Mcxnex state structure
 ...
 *
 *  Return value
 *      pointer to Mcxnex FM function test structure registered.
 *
 *  Caller's context
 *      mcxnex_test_register() can be called in user, kernel or interrupt
 *      context.
 *
 *  Note that no test item is registered if Mcxnex FM is disabled.
 */
mcxnex_test_t *
mcxnex_test_register(mcxnex_state_t *state, char *filename, int linenum,
    int type)
{
	void (*pio_injection)(struct i_hca_fm_test *, ddi_fm_error_t *) =
	    (void (*)(struct i_hca_fm_test *, ddi_fm_error_t *))
	    mcxnex_trigger_pio_error;

	if (state->hs_fm_disable)
		return (NULL);

	return ((mcxnex_test_t *)i_hca_test_register(filename, linenum, type,
	    pio_injection, (void *)state, state->hs_fm_test_hash,
	    state->hs_fm_id_hash, mcxnex_test_num));
}
#endif /* FMA_TEST */
 ...
 * Customized functions should have the driver name prefix such as
 * mcxnex_xxxx() and be defined separately but whose functions should
 ...
 * mcxnex_xxxx() and be defined separately but whose function should
 * call the common interface inside.
 */
    ...
    void *private, mod_hash_t *strHash, mod_hash_t *idHash, int preTestNum)
{
	...
	case MH_ERR_NOTFOUND:
		...
		/* Set the pre-defined mcxnex test item */
...
i_hca_test_set_item(int num, struct i_hca_fm_test *t_item)
{
	if (num < 0 || num >= sizeof (testset) / sizeof (mcxnex_test_t) ||

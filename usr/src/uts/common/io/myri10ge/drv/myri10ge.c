...
myri10ge_clean_rx_done(struct myri10ge_slice_state *ss,
    struct myri10ge_mblk_list *mbl, int byte_limit,
    int pkt_limit, boolean_t *stop)
{
	...
	while (rx_done->entry[rx_done->idx].length != 0) {
		...
		byte_limit -= length;
		pkt_limit--;
		if (unlikely(byte_limit < 0) || unlikely(pkt_limit < 0))
			break;
...
myri10ge_intr_rx(struct myri10ge_slice_state *ss)
{
	...
	if (mutex_tryenter(&ss->rx_lock) == 0)
		return;
	gen = ss->rx_gen_num;
	myri10ge_clean_rx_done(ss, &mbl, MYRI10GE_POLL_NULL, MYRI10GE_POLL_NULL,
	    &ss->rx_polling);
	if (mbl.head != NULL)
...
static mblk_t *
myri10ge_poll_rx(void *arg, int bytes, int pkts)
{
	...
	if (ss->rx_polling)
		myri10ge_clean_rx_done(ss, &mbl, bytes, pkts, &dummy);
	else
...
static int
myri10ge_addmac(void *arg, const uint8_t *mac_addr, uint64_t flags)
{
...
static int
myri10ge_rx_ring_intr_disable(mac_ring_driver_t rh)
{
	struct myri10ge_slice_state *ss;
...
static int
myri10ge_rx_ring_intr_enable(mac_ring_driver_t rh)
{
	struct myri10ge_slice_state *ss;

...
static idm_pdu_t *idm_pdu_alloc_common(uint_t hdrlen, uint_t datalen,
    int sleepflag);
static idm_buf_t *idm_buf_alloc_int(idm_conn_t *ic, void *bufptr,
...
idm_status_t
idm_tgt_svc_port_up(idm_svc_req_t *sr, idm_svc_t **new_svc)
{
	idm_svc_t	*svc = NULL;
	idm_status_t	rc;
	...
	/*
	 * Lookup the list of existing services for a service instance
	 * listening on the specified port. If the service is found,
	 * mark it as being used and increment the is_online count.
	 */
	for (svc = list_head(&idm.idm_tgt_svc_list);
	    svc != NULL;
	    svc = list_next(&idm.idm_tgt_svc_list, svc)) {
		if (svc->is_svc_req.sr_port == sr->sr_port) {
			ASSERT(svc->is_online > 0);
	...
	/* Service is not found, Create a service on the given port */
	if ((rc = idm_tgt_svc_create(sr, new_svc)) != IDM_STATUS_SUCCESS) {
		mutex_exit(&idm.idm_global_mutex);
		return (rc);
	}

	idm_tgt_svc_hold(*new_svc);

	/* Start accepting requests on the given port */
	if ((rc = idm_tgt_svc_online(*new_svc)) != IDM_STATUS_SUCCESS) {
		idm_tgt_svc_rele_and_destroy(*new_svc);
		mutex_exit(&idm.idm_global_mutex);
		return (rc);
	}
	list_insert_tail(&idm.idm_tgt_svc_list, *new_svc);
	/*
	 * Increment the service online count indicating
	 * that a portal is using this service
	 */
	(*new_svc)->is_online++;
	mutex_exit(&idm.idm_global_mutex);
...
{
	mutex_enter(&idm.idm_global_mutex);
	idm_tgt_svc_offline(svc);

	/* remove this service from the global list */
	if (svc->is_online == 0) {
		list_remove(&idm.idm_tgt_svc_list, svc);
	}

	/* If the service is unreferenced, destroy it too */
...
idm_tgt_svc_create(idm_svc_req_t *sr, idm_svc_t **new_svc)
{
	...
	*new_svc = NULL;
	...
	for (type = 0; type < IDM_TRANSPORT_NUM_TYPES; type++) {
		...
		if (rc != IDM_STATUS_SUCCESS) {
			/* Teardown any configured service */
			while (type--) {
				it = &idm_transport_list[type];
				if (it->it_ops == NULL) {
					continue;
 ...
 *
 * This is only called once when the portal is brought online at creation time.
 */
idm_status_t
...
idm_buf_alloc_sgl(idm_conn_t *ic, struct stmf_sglist_ent *sglp,
    uint64_t buflen, uint32_t numbufs, uint32_t flags)
{
...
idm_buf_alloc_int(idm_conn_t *ic, void *bufptr, uint64_t buflen,
    struct stmf_sglist_ent *sglp, uint32_t numbufs,
    uint32_t flags) {
	idm_buf_t	*buf;
	int		rc;
	...
	/* Buffers do not start out zeroed, so initialize all fields */
	buf->idb_ic		= ic;
	...
	if (bufptr == NULL && sglp == NULL) {
		 ...
		 */
		ASSERT(buf->idb_bufalloc != 0);
	} else {
		/* ZCOPY */
		ASSERT(sglp != NULL);
		rc = ic->ic_transport_ops->it_buf_alloc_sgl(buf, sglp,
		    numbufs, flags);
		if (rc != IDM_STATUS_SUCCESS) {
			idm_conn_rele(ic);
			kmem_cache_free(idm.idm_buf_cache, buf);
...
idm_buf_free(idm_buf_t *buf)
{
	...
	if (buf->idb_bufalloc == IDB_IDMALLOC) {
		ic->ic_transport_ops->it_buf_free(buf);
	} else if (buf->idb_bufalloc == IDB_ZCOPY) {
		ic->ic_transport_ops->it_buf_free_sgl(buf);
	} else if (buf->idb_bufalloc == IDB_REG_MEM) {
...
idm_bufpat_set(idm_buf_t *idb)
{
	...
	ASSERT(idb->idb_bufalloc == IDB_IDMALLOC);
	len = idb->idb_buflen;
...
idm_bufpat_check(idm_buf_t *idb, int check_len, idm_bufpat_check_type_t type)
{
	 ...
	 * to double-buffer)
	 */
	if (idb->idb_bufalloc != IDB_IDMALLOC)
		return (B_FALSE);
...
idm_task_alloc(idm_conn_t *ic)
{
	...
	mutex_enter(&idm.idm_taskid_table[idt->idt_tt].task_bkt_lock);
	idm.idm_taskid_table[idt->idt_tt].task = idt;
	mutex_exit(&idm.idm_taskid_table[idt->idt_tt].task_bkt_lock);
...
idm_task_free(idm_task_t *idt)
{
	...
	/*
	 * Clear the pointer to the task from the idm_taskid_table
	 * so that when we check for the existance of this
	 * pointer in idm_task_find_common we will not be able to
	 * access the task if it has been freed.
	 */
	mutex_enter(&idm.idm_taskid_table[idt->idt_tt].task_bkt_lock);
	idm.idm_taskid_table[idt->idt_tt].task = NULL;
	mutex_exit(&idm.idm_taskid_table[idt->idt_tt].task_bkt_lock);
    ...
    boolean_t complete)
{
	...
	mutex_enter(&idm.idm_taskid_table[tt].task_bkt_lock);
	idt = idm.idm_taskid_table[tt].task;
	...
	if (idt != NULL) {
		...
		if ((idt->idt_state != TASK_ACTIVE) ||
			...
			mutex_exit(&idt->idt_mutex);
			mutex_exit(&idm.idm_taskid_table[tt].task_bkt_lock);
			return (NULL);
		}
		idm_task_hold(idt);
		/*
...
idm_task_find(idm_conn_t *ic, uint32_t itt, uint32_t ttt)
{
	if (IDM_CONN_ISINI(ic)) {
		return (idm_task_find_common(ic, itt & IDM_ITT_MASK,
		    ttt, B_FALSE));
	} else {
...
idm_task_find_by_handle(idm_conn_t *ic, uintptr_t handle)
{
	...
	for (idx = 0; idx < idm.idm_taskid_max; idx++) {
		mutex_enter(&idm.idm_taskid_table[idx].task_bkt_lock);
		idt = idm.idm_taskid_table[idx].task;

		if (idt == NULL) {
			mutex_exit(&idm.idm_taskid_table[idx].task_bkt_lock);
			continue;
		}
...
idm_task_abort(idm_conn_t *ic, idm_task_t *idt, idm_abort_type_t abort_type)
{
	...
	if (idt == NULL) {
		...
		for (idx = 0; idx < idm.idm_taskid_max; idx++) {
			mutex_enter(&idm.idm_taskid_table[idx].task_bkt_lock);
			task = idm.idm_taskid_table[idx].task;
			if (task == NULL) {
				mutex_exit(
				    &idm.idm_taskid_table[idx].task_bkt_lock);
				continue;
			}
...
idm_conn_get_transport_type(idm_conn_t *ic)
{
	return (ic->ic_transport_type);
}
...
_idm_init(void)
{
	...
	idm.idm_taskid_table = (idm_taskid_bucket_t *)
	    kmem_zalloc(idm.idm_taskid_max*
	    sizeof (idm_taskid_bucket_t), KM_SLEEP);
	idm.idm_taskid_space = id_space_create(
	    "idm_taskid_space", 0, idm.idm_taskid_max);

	/* Initialize the task_bkt_lock mutex */
	for (int i = 0; i < idm_max_taskids; i++) {
		mutex_init(&idm.idm_taskid_table[i].task_bkt_lock,
...
_idm_fini(void)
{
	...
	/* Destroy the task_bkt_lock mutex */
	for (int i = 0; i < idm_max_taskids; i++) {
		mutex_destroy(&idm.idm_taskid_table[i].task_bkt_lock);
	}

	kmem_free(idm.idm_taskid_table,
	    idm.idm_taskid_max * sizeof (idm_taskid_bucket_t));

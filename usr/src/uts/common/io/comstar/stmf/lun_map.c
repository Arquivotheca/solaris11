...
stmf_i_local_port_t *stmf_targetident_to_ilport(uint8_t *target_ident,
		uint16_t ident_size);
boolean_t stmf_session_has_hostident(stmf_i_scsi_session_t *iss,
...
uint16_t
stmf_alua_get_next_free_lun(stmf_lun_map_t *sm, uint8_t *lun)
{
	uint16_t luNbr;
	uint16_t firstNum;

	firstNum = (stmf_state.stmf_alua_node == 0) ? 2 : 1;
	if (sm->lm_nentries > 0 && sm->lm_plus[0] != NULL) {
		if (sm->lm_nluns < 0x4000 / 2) {
			for (luNbr = firstNum; luNbr < sm->lm_nentries;
			    luNbr += 2) {
...
stmf_get_next_free_lun(stmf_lun_map_t *sm, uint8_t *lun)
{
	...
	if (stmf_state.stmf_alua_state == 1) {
		return (stmf_alua_get_next_free_lun(sm, lun));
	}
...
/*
 * Check whether given group has any view entry related to it
 */
int
stmf_check_group_inuse(uint8_t *grpname, uint16_t grpname_size,
    stmf_id_type_t group_type, uint32_t *err_detail, boolean_t *in_use)
{
	stmf_id_data_t *id;
	...
	*in_use = (id->id_refcnt != 0)?B_TRUE:B_FALSE;
	return (0);
}
		...
		stmf_id_type_t entry_type, uint32_t *err_detail)
{
	...
	if (id_grp == NULL) {
		*err_detail = (entry_type == STMF_ID_TYPE_HOST)?
		    STMF_IOCERR_INVALID_HG:STMF_IOCERR_INVALID_TG;
		return (ENODEV); /* group not found */
	}

	/* Check whether this member is already bound to a group. */
	if (id_grp_tmp) {
		if (id_grp_tmp != id_grp) {
			*err_detail = (entry_type == STMF_ID_TYPE_HOST)?
			    STMF_IOCERR_HG_ENTRY_EXISTS:
			    STMF_IOCERR_TG_ENTRY_EXISTS;
			return (EEXIST); /* already in another group */
		} else {
			return (0);
		}
	}

	/* The target needs to be offline if we are modifying a target group. */
	...
	/* Code specific for adding new member to a target group. */
	if (entry_type == STMF_ID_TYPE_TARGET) {
		ilport = stmf_targetident_to_ilport(entry_ident, entry_size);
		if (ilport)
	...
	/*
	 * Code specific for adding new member to a host group.
	 * Some LUs might become accessible by the host, so we need to add
	 * them into the host's sessions.
	 * There are no sessions when the STMF service is not running.
	 */
	if (!stmf_state.stmf_service_running)
		return (0);
	...
	/*
	 * Go through the list of sessions and update the ones that may be
	 * affected.
	 */
	for (ilport = stmf_state.stmf_ilportlist; ilport;
	    ilport = ilport->ilport_next) {
		...
		for (iss = ilport->ilport_ss_list; iss; iss = iss->iss_next) {
			stmf_id_data_t *tgid;
			if (stmf_session_has_hostident(iss, entry_ident,
			    entry_size) != B_TRUE)
				continue;
			...
			/*
			 * Consider views for the user-defined target group
			 * and this host group.
			 */
			if (tgid) {
			...
			/*
			 * Also consider views for the default target group
			 * with all targets and this host group.
			 */
			if (vemap_alltgt)
		...
		stmf_id_type_t entry_type, uint32_t *err_detail)
{
	...
	/* Code specific for removing the member from a target group. */
	if (entry_type == STMF_ID_TYPE_TARGET) {
		ilport = stmf_targetident_to_ilport(entry_ident, entry_size);
		if (ilport)
			ilport->ilport_tg = NULL;
	...
	/*
	 * Code specific for removing the member from a host group.
	 * Some LUs might no longer be accessible by the host, so we need
	 * to remove them from the host's sessions.
	 * There are no sessions when the STMF service is not running.
	 */
	if (!stmf_state.stmf_service_running)
		return (0);
...
boolean_t
stmf_session_has_hostident(stmf_i_scsi_session_t *iss, uint8_t *host_ident,
    uint16_t ident_size)
{
	uint8_t *id;

	id = (uint8_t *)iss->iss_ss->ss_rport_id;
	if (id[3] == ident_size && bcmp(id + 4, host_ident, ident_size) == 0)
		return (B_TRUE);
	else
		return (B_FALSE);
}

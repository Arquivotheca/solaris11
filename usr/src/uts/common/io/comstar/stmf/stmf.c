...
void stmf_task_lu_kill_for_sessions(scsi_task_t *tm_task, stmf_status_t s,
...
volatile int stmf_max_nworkers = 8192;
volatile int stmf_min_nworkers = 64;
	...
	cred_t *credp, int *rval)
{
	...
	case STMF_IOCTL_REMOVE_TARGET_GROUP:
		...
		mutex_exit(&stmf_state.stmf_lock);
		break;
	case STMF_IOCTL_CHECK_HOST_GROUP_INUSE:
		idtype = STMF_ID_TYPE_HOST_GROUP;
		/* FALLTHROUGH */
	case STMF_IOCTL_CHECK_TARGET_GROUP_INUSE:
		if (stmf_state.stmf_config_state == STMF_CONFIG_NONE) {
			ret = EACCES;
			iocd->stmf_error = STMF_IOCERR_UPDATE_NEED_CFG_INIT;
			break;
		...
		grpname = (stmf_group_name_t *)ibuf;
		if (cmd == STMF_IOCTL_CHECK_TARGET_GROUP_INUSE)
			idtype = STMF_ID_TYPE_TARGET_GROUP;
		if ((ibuf == NULL) ||
		    (iocd->stmf_ibuf_size < sizeof (stmf_group_name_t))) {
			ret = EINVAL;
		...
		if ((obuf == NULL) ||
		    (iocd->stmf_obuf_size < sizeof (boolean_t))) {
			ret = EINVAL;
			break;
		}
		if (grpname->name[0] == '*') {
		...
		mutex_enter(&stmf_state.stmf_lock);
		ret = stmf_check_group_inuse(grpname->name,
		    grpname->name_size, idtype, &iocd->stmf_error,
		    (boolean_t *)obuf);
		mutex_exit(&stmf_state.stmf_lock);
		break;
...
stmf_ic_rx_scsi_data(stmf_ic_scsi_data_msg_t *msg)
{
	...
	dbuf->db_flags = DB_DIRECTION_TO_RPORT | DB_LU_PRIVATE_DATA_BUF;
 ...
 * We need to be able to index the array by PROTOCOL_ANY.
 */
char *protocol_ident[PROTOCOL_ANY + 1] = {
	"Fibre Channel",
	"Parallel SCSI",
	"SSA",
	"IEEE_1394",
...
/*
 * Kills all tasks on a lu if its session id matches one in session_ids array
 */
void
stmf_task_lu_kill_for_sessions(scsi_task_t *tm_task, stmf_status_t s,
    uint64_t *session_ids)
{
	stmf_i_lu_t *ilu = (stmf_i_lu_t *)tm_task->task_lu->lu_stmf_private;
	stmf_i_scsi_task_t	*itask;
	int			i;
	...
	for (itask = ilu->ilu_tasks; itask != NULL;
		...
		if (itask->itask_task == tm_task)
			continue;
		for (i = 0; session_ids[i] != 0; i++) {
			if (itask->itask_task->task_session->ss_session_id ==
			    session_ids[i]) {
				stmf_abort(STMF_QUEUE_TASK_ABORT,
				    itask->itask_task, s, NULL);
				break;
			}
		}
	}
	...
	for (i = 0; i < 4; i++) {
		if (map & 1) {
			...
			if ((dbuf->db_flags & DB_LU_DATA_BUF) ||
			    (dbuf->db_flags & DB_LU_PRIVATE_DATA_BUF)) {
				/*
				...
				if ((itask->itask_flags
				    & ITASK_DEFAULT_HANDLING) == 0)
					lu = task->task_lu;
				else
					lu = dlun0;
...
stmf_xfer_data(scsi_task_t *task, stmf_data_buf_t *dbuf, uint32_t ioflags)
{
	...
	/*
	 * Port provider may have already called the buffer callback.
	 */
	if (ret != STMF_SUCCESS) {
		dbuf->db_flags &= ~DB_LPORT_XFER_ACTIVE;
	}
...
stmf_abort(int abort_cmd, scsi_task_t *task, stmf_status_t s, void *arg)
{
	...
	case STMF_QUEUE_ABORT_LU:
		...
		return;
	case STMF_QUEUE_ABORT_LU_SESS_TASK:
		stmf_task_lu_kill_for_sessions(task, s, (uint64_t *)arg);
		return;
...
stmf_handle_lun_reset(scsi_task_t *task)
{
	...
	do {
		...
		new |= ITASK_DEFAULT_HANDLING | ITASK_CAUSING_LU_RESET;
	} while (atomic_cas_32(&itask->itask_flags, old, new) != old);
...
stmf_handle_target_reset(scsi_task_t *task)
{
	...
	/*
	 * Mark this task as the one causing target reset so that we know who
	 * was responsible for setting the ISS_RESET_ACTIVE. In case this
	 * task itself gets aborted, we will clear ISS_RESET_ACTIVE.
	 */
	do {
		new = old = itask->itask_flags;
		if (new & ITASK_BEING_ABORTED) {
		...
		new |= ITASK_DEFAULT_HANDLING | ITASK_CAUSING_TARGET_RESET;
	} while (atomic_cas_32(&itask->itask_flags, old, new) != old);
...
void
stmf_dlun0_dbuf_free(struct scsi_task *task, struct stmf_data_buf *dbuf)
{
	stmf_xfer_data_t *xd;
...
stmf_dlun_init()
{
	...
	dlun0->lu_dbuf_free = stmf_dlun0_dbuf_free;
				...
				uint16_t *tptid_sz)
{
	...
	case PROTOCOL_iSCSI:
		...
		/* If the format_code is 1 check for ISID separator */

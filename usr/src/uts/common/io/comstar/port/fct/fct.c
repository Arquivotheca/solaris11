...
fct_cmd_free(fct_cmd_t *cmd)
{
	...
	if (do_abts_acc) {
		...
		/*
		 * If a failure happens, let the initiator handle the timeout.
		 * We are not taking any action here other than logging the
		 * error, since even if there is a fatal error in the FCA layer,
		 * subsequent transport would catch it and handle it
		 */
		if (port->port_send_cmd_response(lcmd,
		    FCT_IOF_FORCE_FCA_DONE) != FCT_SUCCESS) {
			stmf_trace(iport->iport_alias, "fct_cmd_free: "
			    "iport-%p, ABTS_ACC port_send_cmd_response failed",
			    (void *)iport);
		}
		fct_cmd_free(lcmd);
		cmd->cmd_link = NULL;
...
fct_handle_rcvd_abts(fct_cmd_t *cmd)
{
	...
	mutex_exit(&iport->iport_worker_lock);
	rw_exit(&irp->irp_lock);
	if (c->cmd_type != FCT_CMD_RCVD_ELS) {
		/*
		 * Queue cmd for termination only if it's not an RCVD ELS,
		 * Otherwise it will be queued in both irp_els_list and
		 * iport_abort_queue which will lead to trouble.
		 */

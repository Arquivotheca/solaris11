...
#define	FCOET_VERSION	"v20110331-1.04"
...
#define	FCOET_OXID_TABLE_SIZE	512
#define	FCOET_RXID_TABLE_SIZE	512
...
typedef struct fcoet_soft_state {
	 ...
	 * oxid/rxid
	 */
	struct fcoet_exchange	**ss_sol_oxid_table;
	struct fcoet_exchange	**ss_unsol_rxid_table;
	uint16_t		 ss_next_sol_oxid;
	uint16_t		 ss_next_unsol_rxid;
	uint16_t		 ss_min_unsol_rxid;
	uint16_t		 ss_max_unsol_rxid;
...
#define	FCOET_DBUF_COPY_THRESHOLD	0x1000	/* 4k */
#define	FCOET_MAX_SGL_XFER_LEN		0x20000
#define	FCOET_MAX_DBUF_PER_XCHG		1
/*
 * exchange - cmd alias
 */
typedef struct fcoet_exchange {
	...
	stmf_data_buf_t		*xch_dbufs[FCOET_MAX_DBUF_PER_XCHG];
	uint8_t			xch_buf_map;
 ...
 * get the exchange from xid table in fcoet_rx_frame.
...
#define	XCH_FLAG_STATUS_SENT_BY_FCOET	0x0020
#define	XCH_FLAG_LRO			0x0040
...
void fcoet_copy_between_dbuf_and_frm(stmf_data_buf_t *dbuf,
    void *frm_payload, uint32_t offset, uint32_t data_size,
    boolean_t from_dbuf);
#define	FCOET_COPY_FROM_DBUF(dbuf, dst, offset, data_size)	\
    fcoet_copy_between_dbuf_and_frm(dbuf, dst, offset,		\
    data_size, B_TRUE);
#define	FCOET_COPY_TO_DBUF(dbuf, dst, offset, data_size)	\
    fcoet_copy_between_dbuf_and_frm(dbuf, dst, offset,		\
    data_size, B_FALSE);
...
#define	FCOET_DB_GET_LEFT_SIZE(x_db)			\
	((intptr_t)(x_db)->db_port_private)
#define	FCOET_DB_SET_LEFT_SIZE(x_db, size)		\
	(x_db)->db_port_private = (void *)(intptr_t)(size)

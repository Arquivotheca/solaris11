    ...
    idm_pdu_t *pdu)
{
	...
	/*
	 * If we are in the ILS_LOGIN_ERROR or ILS_LOGIN_DONE state,
	 * drop all additional events. The mutex should be held for
	 * this check to prevent any race condition.
	 */
...
login_sm_session_bind(iscsit_conn_t *ict)
{
	...
	iscsit_conn_t		*existing_ict = NULL, *leading_ict = NULL;
	uint8_t			error_class;
	uint8_t			error_detail;
	idm_transport_type_t	ict_tt, leading_tt;
	...
	if (existing_sess == NULL) {
		...
		if (lsm->icl_tsih == ISCSI_UNSPEC_TSIH) {
			...
			new_sess = iscsit_sess_reinstate(tgt, existing_sess,
			    ict, lsm->icl_cmdsn, &error_class, &error_detail);
			ASSERT(new_sess != NULL);
			 ...
			 */
			IDM_SM_LOG(CE_NOTE, "login_sm_session_bind: add new "
			    "conn/sess continue");
			if (existing_ict != NULL) {
				...
				/*
				 * iSER-assisted mode is negotiated during the
				 * iSCSI login for each session, and an
				 * entire iSCSI session can only operate in
				 * one mode (i.e., a connection in a session
				 * cannot operate in iSER-assisted mode if a
				 * different connection of the same sesiion is
				 * already in Full Feature Phase in the
				 * Traditional iSCSI mode
				 */
				ict_tt =
				    idm_conn_get_transport_type(ict->ict_ic);
				leading_ict =
				    list_head(&existing_sess->ist_conn_list);
				leading_tt = idm_conn_get_transport_type(
				    leading_ict->ict_ic);
				IDM_SM_LOG(CE_NOTE, "login_sm_session_bind: "
				    "Transport type[new conn = %d], "
				    "Transport type[leading conn = %d]",
				    ict_tt, leading_tt);
				if (ict_tt != leading_tt) {
					IDM_SM_LOG(CE_NOTE,
					    "login_sm_session_bind: "
					    "conn using a different transport");
					SET_LOGIN_ERROR(ict,
					    ISCSI_STATUS_CLASS_INITIATOR_ERR,
					    ISCSI_LOGIN_STATUS_INIT_ERR);
					goto session_bind_error;
...
iscsit_process_negotiated_values(iscsit_conn_t *ict)
{
	...
	if ((nvrc = nvlist_lookup_string(lsm->icl_negotiated_values,
		...
		ASSERT(nvrc == 0);
		if (ict->ict_sess->ist_initiator_alias != NULL) {
			kmem_free(ict->ict_sess->ist_initiator_alias,
			    strlen(ict->ict_sess->ist_initiator_alias) + 1);
		}
	...
	if ((nvrc = nvlist_lookup_string(lsm->icl_negotiated_values,
		...
		ASSERT(nvrc == 0);
		if (ict->ict_sess->ist_target_alias != NULL) {
			kmem_free(ict->ict_sess->ist_target_alias,
			    strlen(ict->ict_sess->ist_target_alias) + 1);
		}

...
iscsit_tpg_modify(iscsit_tpg_t *tpg, it_tpg_t *cfg_tpg)
{
	...
	for (cfg_portal = cfg_tpg->tpg_portal_list;
		...
		if ((portal = iscsit_tpg_portal_lookup_locked(tpg,
			...
			portal = iscsit_portal_create(tpg,
			    &cfg_portal->portal_addr);
			if (tpg->tpg_online_tgt_count > 0) {
				(void) iscsit_portal_online(portal);
				/*
				 * If iscsit_portal_online fails,
				 * tpg_online_tgt_count will be set
				 * to 1, but portal_online will be
				 * B_FALSE indicating that the
				 * service on portal_addr is not
				 * up for accepting connections.
				 */
			}
		} else {
	...
	for (portal = avl_first(&tpg->tpg_portal_list);
		...
		/*
		 * A request to delete the last portal of a TPG is
		 * considered to be an error. Typically libiscsit
		 * should prevent such cases. To avoid the problem
		 * of partial removal of an unwanted configuration,
		 * the new portal configuration is first added and
		 * then the old configuration is deleted.
		 */
		if (avl_numnodes(&tpg->tpg_portal_list) > 1) {

			if (it_portal_lookup(cfg_tpg, &portal->portal_addr)
			    == NULL) {
...
iscsit_tpg_online(iscsit_tpg_t *tpg)
{
	...
	mutex_enter(&tpg->tpg_mutex);
	ASSERT(tpg->tpg_online_tgt_count >= 0);
	tpg->tpg_online_tgt_count++;

	for (portal = avl_first(&tpg->tpg_portal_list);
	    portal != NULL;
	    portal = AVL_NEXT(&tpg->tpg_portal_list, portal)) {
		if (portal->portal_online == B_FALSE) {
			/* No service exists on the given portal-port */
...
tpg_online_fail:
	...
	for (portal = avl_first(&tpg->tpg_portal_list);
	    portal != portal_fail;
	    portal = AVL_NEXT(&tpg->tpg_portal_list, portal)) {
		if (portal->portal_online == B_TRUE) {
...
iscsit_portal_online(iscsit_portal_t *portal)
{
	...
	ASSERT(portal->portal_online == B_FALSE);
	...
	/* Populate the service request parameters */
	sr.sr_port				= port;
	...
	sr.sr_conn_ops.icb_client_notify 	= &iscsit_client_notify;
	sr.sr_conn_ops.icb_build_hdr		= &iscsit_build_hdr;
	sr.sr_conn_ops.icb_update_statsn	= &iscsit_update_statsn;
	sr.sr_conn_ops.icb_keepalive		= &iscsit_keepalive;

	ASSERT(portal->portal_svc == NULL);

	/*
	 * Portal is not associated with a service on the specified
	 * portal addr. Lookup the list of existing services for a
	 * service instance listening on the specified port. If the
	 * service is found, it is used by this portal as well. If
	 * a service is not found, a new service is created.
	 */

	if ((rc = idm_tgt_svc_port_up(&sr, &svc)) != IDM_STATUS_SUCCESS) {
		return (rc);
	}

	/* portal is ready to accept requests on the specified portal */
	portal->portal_svc = svc;
	portal->portal_online = B_TRUE;
...
iscsit_portal_offline(iscsit_portal_t *portal)
{
	...
	ASSERT(portal->portal_online == B_TRUE);

	iscsit_isns_portal_offline(portal);

	/* Offline the service and destroy it */
	idm_tgt_svc_port_down(portal->portal_svc);

	portal->portal_svc = NULL;
	portal->portal_online = B_FALSE;
}

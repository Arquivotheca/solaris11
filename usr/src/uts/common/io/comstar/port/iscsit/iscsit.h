...
#define	PDU_ACTIVE_MISSING	((idm_pdu_t *)1)
typedef struct {
	int		cb_num_elems;
	struct cb_buf {
		idm_pdu_t	*cb_pdu;
		clock_t		cb_queue_time;
	} cb_buffer[ISCSIT_RXPDU_QUEUE_LEN];
} iscsit_cbuf_t;
...
typedef struct {
	...
	/* Count of online targets/tpgts using this tpg */
	int		tpg_online_tgt_count;
...
typedef struct {
	...
	/* service exists on the portal addr */
	boolean_t		portal_online;
...
iscsit_sess_reinstate(iscsit_tgt_t *tgt, iscsit_sess_t *ist, iscsit_conn_t *ict,
    uint32_t cmdsn, uint8_t *error_class, uint8_t *error_detail);

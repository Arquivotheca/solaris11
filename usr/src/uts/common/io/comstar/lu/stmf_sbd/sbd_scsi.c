...
static stmf_status_t sbd_copy_rdwr(scsi_task_t *, uint64_t,
    stmf_data_buf_t *, int, int);
...
sbd_do_sgl_read_xfer(struct scsi_task *task, sbd_cmd_t *scmd, int first_xfer)
{
	...
	while (scmd->len && scmd->nbufs < task->task_max_nbufs) {
		...
		if (final_xfer) {
			/*
			 * For the final xfer of a SCSI READ, we let the port
			 * provider do the status sending. In case the PP does
			 * not succeed in sending the status, we set the
			 * DB_DONT_EMULATE_STATUS_GOOD flag to let ut know
			 * that SBD will take over the job of sending the
			 * status.
			 */
			dbuf->db_flags |= (DB_SEND_STATUS_GOOD |
			    DB_DONT_EMULATE_STATUS_GOOD);
		}
				...
				struct stmf_data_buf *dbuf)
{
	...
	/*
	 * Completion notifications may be received out of order
	 * w.r.t the data transfer order, so the DB_STATUS_GOOD_SENT
	 * bit may appear in any one of the dbuf db_flags fields.
	 */
	if (dbuf->db_flags & DB_STATUS_GOOD_SENT) {
		ASSERT((scmd->flags & SBD_SCSI_CMD_STATUS_SENT) == 0);
		scmd->flags |= SBD_SCSI_CMD_STATUS_SENT;
	}
	...
	if (!scmd_err) {
		...
		if (scmd->nbufs == 0 && scmd->len == 0) {
			 ...
			 */
			scmd->flags &= ~SBD_SCSI_CMD_ACTIVE;
			if (scmd->flags & SBD_SCSI_CMD_STATUS_SENT) {
				/*
				 * Status was sent along with data, so no
				 * status completion will occur. Tell stmf
				 * we are done.
				 */
				...
				struct stmf_data_buf *dbuf)
{
	...
	if ((dbuf->db_flags & DB_LU_DATA_BUF) == 0) {
		...
		/*
		 * Handle immediate data passed in with a dbuf from the PP
		 * Just write the data to zvol and kick-off
		 * more xfers. We should only get here if there
		 * is more data after this dbuf to xfer, so no need
		 * to do a commit here.
		 */
		stmf_status_t ret;

		ASSERT(zvio == NULL);
		ASSERT(task->task_flags & TF_INITIAL_BURST);
		ASSERT(scmd->len);
		ASSERT(dbuf->db_relative_offset == 0);

		ret = sbd_copy_rdwr(task,
		    scmd->addr, dbuf, SBD_CMD_SCSI_WRITE, 0);
		/* Release the dbuf */
		stmf_free_dbuf(task, dbuf);
		if (ret == STMF_SUCCESS) {
			task->task_nbytes_transferred += data_size;
			...
			/*
			 * The only possible failure is the zvol write
			 */
			rw_exit(&sl->sl_access_state_lock);
			scmd->flags &= ~SBD_SCSI_CMD_ACTIVE;
			stmf_scsilib_send_status(task, STATUS_CHECK,
	...
	DTRACE_PROBE5(backing__store__write__end, sbd_lu_t *, sl,
	    uint8_t *, NULL, uint64_t, data_size,
	    uint64_t, zvio->zvio_offset, int, ret);
...
sbd_zcopy_write_useful(scsi_task_t *task, uint64_t laddr, uint32_t len,
    uint64_t blksize, stmf_data_buf_t *initial_dbuf)
{

	/*
	 * If there is a passed in dbuf, that data must be copied since
	 * it is not in an ARC buffer. Remove it from consideration.
	 */
	if (initial_dbuf) {
		ASSERT(initial_dbuf->db_data_size <= len);
		len -= initial_dbuf->db_data_size;
		laddr += initial_dbuf->db_data_size;
	}
...
sbd_handle_write(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	...
	sbd_cmd_t *scmd;
	stmf_data_buf_t *dbuf;
	uint8_t	sync_wr_flag = 0, do_zcopy = 0;

	if (sl->sl_flags & SL_READ_ONLY) {
		stmf_scsilib_send_status(task, STATUS_CHECK,
	...
	if (sbd_zcopy & (4|1) &&		/* Debug switch */
	    ...
	    (task->task_additional_flags &
	    TASK_AF_ACCEPT_LU_DBUF) &&		/* PP allows it */
	    sbd_zcopy_write_useful(task, laddr, len, sl->sl_blksize, dbuf)) {

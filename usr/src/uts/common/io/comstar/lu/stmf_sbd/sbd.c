...
extern sbd_status_t sbd_pgr_meta_load(sbd_lu_t *sl, int trans_to_active);
...
static int sbd_is_store_rdonly(sbd_lu_t *sl);
static void sbd_process_rdonly(sbd_lu_t *sl);
...
odf_over_open:
	...
	sl->sl_data_vtype = sl->sl_data_vp->v_type;
	if ((sl->sl_data_vtype != VREG) && (vattr.va_size == 0)) {
		/*
    ...
    int no_register, sbd_lu_t **slr)
{
	...
	if ((sl->sl_flags & SL_ZFS_META) || (sl->sl_meta_vtype == VREG)) {
		sl->sl_meta_blocksize_shift = 0;
	} else {
		sl->sl_meta_blocksize_shift = 9;
	}
...
sim_sli_loaded:
	...
	sret = sbd_pgr_meta_load(sl, standby);
	if (sret != SBD_SUCCESS) {
		*err_ret = SBD_RET_NO_META;
		ret = EIO;
		goto sim_err_out;
	...
	*err_ret = 0;
	...
	if (mlu->mlu_write_protected_valid) {
		...
		if (mlu->mlu_write_protected) {
			...
			if (sbd_is_store_rdonly(sl)) {
				mutex_exit(&sl->sl_lock);
				*err_ret = SBD_RET_WRITE_PROTECT_SET_FAILED;
				ret = EINVAL;
				goto smm_err_out;
			} else if (sl->sl_flags & SL_WRITE_PROTECTED) {
				sl->sl_flags &= ~SL_WRITE_PROTECTED;
			}
		}
		mutex_exit(&sl->sl_lock);
		sbd_process_rdonly(sl);
	}
...
/*
 * determine WP mode of the backing store;
 */
int
sbd_is_store_rdonly(sbd_lu_t *sl)
{
	int store_rdonly = 0;

	if (sbd_is_zvol(sl->sl_data_filename)) {
		int ret = sbd_zvol_get_rdonly(sl, &store_rdonly);
		if (ret) {
			store_rdonly = 0;
		}
	}
...
/*
 * check for UNIT ATTENTION due to readonly mode change
 */
void
sbd_check_rdonly_ua(sbd_lu_t *sl)
{
	sbd_process_rdonly(sl);
}

/*
 * set effective read-only condition if either sbd or zvol is read-only
 * if effective read-only has changed set UNIT ATTENTION
 */
static void
sbd_process_rdonly(sbd_lu_t *sl)
{
	sbd_it_data_t *it;
	int new_flags;
	...
	if (sl->sl_flags & SL_WRITE_PROTECTED || sbd_is_store_rdonly(sl)) {
		new_flags = sl->sl_flags | SL_READ_ONLY;
	} else {
		new_flags = sl->sl_flags & ~SL_READ_ONLY;
	}
	if (new_flags != sl->sl_flags) {
		sl->sl_flags = new_flags;

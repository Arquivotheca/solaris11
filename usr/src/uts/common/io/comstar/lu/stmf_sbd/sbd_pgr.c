...
sbd_status_t sbd_pgr_meta_load(sbd_lu_t *, int);
...
static sbd_status_t sbd_get_preempt_its(scsi_task_t *, uint64_t, boolean_t,
	uint64_t **, int *);
...
sbd_status_t
sbd_pgr_meta_load(sbd_lu_t *slu, int trans_to_active)
{
	...
	/* If APTPL bit is not set and it is not a transition from standby */
	if (!(pgr->pgr_flags & SBD_PGR_APTPL) && (trans_to_active == 0)) {
		PGR_CLEAR_RSV_FLAG(pgr->pgr_flags);
		kmem_free(spi, spi->pgr_sms_header.sms_size);
		return (ret);
	}

	/* If APTPL bit is  set or it is transition from standby to active */
...
sbd_pgr_meta_write(sbd_lu_t *slu)
{
	...
	if (ret != SBD_SUCCESS) {
		...
		pgr->pgr_keylist = NULL;
		if (sbd_pgr_meta_load(slu, 0) != SBD_SUCCESS) {
			char *lun_name = sbd_get_devid_string(slu);
...
/*
 * Get session ID for all the preempted ITs depends on boolean variable "match"
 * match = B_TRUE  ==>	Find all ITs whose service key matches the given
 *			svc_key, except for the IT which task belongs to.
 * match = B_FALSE ==>	Find all ITs whose service key does not match the
 *			svc_key, except for the IT which task belongs to.
 * *it_session_ids will hold an array of all the session ID for preempted ITs,
 * end with 0. Caller is responsible for releasing it.
 * *count will hold the size of returned array.
 */
static sbd_status_t
sbd_get_preempt_its(scsi_task_t *task, uint64_t svc_key, boolean_t match,
    uint64_t **it_session_ids, int *size)
{
	sbd_it_data_t	*it;
	sbd_it_data_t	*my_it = task->task_lu_itl_handle;
	sbd_lu_t	*slu = (sbd_lu_t *)task->task_lu->lu_provider_private;
	uint64_t	*sbd_it_session_ids = NULL;
	uint32_t	i = 0;

	if (((scsi_cdb_prout_t *)task->task_cdb)->action !=
	    PR_OUT_PREEMPT_ABORT)
		return (SBD_SUCCESS);

	ASSERT(size != NULL && it_session_ids != NULL);
	...
	sbd_it_session_ids = (uint64_t *)kmem_zalloc(
	    i * sizeof (uint64_t), KM_NOSLEEP);
	if (sbd_it_session_ids == NULL) {
		mutex_exit(&slu->sl_lock);
		return (SBD_FAILURE);
	}
	...
	*size = i;
	...
	for (i = 0, it = slu->sl_it_list; it != NULL; it = it->sbd_it_next) {
		if (it == my_it)
			continue;
		if (it->pgr_key_ptr &&
		    (match && it->pgr_key_ptr->pgr_key == svc_key ||
		    !match && it->pgr_key_ptr->pgr_key != svc_key)) {
			sbd_it_session_ids[i++] = it->sbd_it_session_id;
		}
	}
	mutex_exit(&slu->sl_lock);

	if (i == 0) {
		kmem_free((void *)sbd_it_session_ids,
		    (*size) * sizeof (uint64_t));
		*size = 0;
		return (SBD_SUCCESS);
	}
	*it_session_ids = sbd_it_session_ids;
	return (SBD_SUCCESS);
}
...
sbd_pgr_out_preempt(scsi_task_t *task, stmf_data_buf_t *dbuf)
{
	...
	if (SBD_PGR_RSVD_NONE(pgr)) {
		if (svc_key != 0 && sbd_get_preempt_its(task, svc_key, B_TRUE,
		    &sbd_it_session_ids, &it_alloc_size) != SBD_SUCCESS) {
			stmf_scsilib_send_status(task, STATUS_CHECK,
			    STMF_SAA_INSUFFICIENT_REG_RESOURCES);
			return;
		}
		...
		if (pgr->pgr_rsvholder == key &&
			...
			if (sbd_get_preempt_its(task, svc_key,
			    B_TRUE, &sbd_it_session_ids,
			    &it_alloc_size) != SBD_SUCCESS) {
				stmf_scsilib_send_status(task, STATUS_CHECK,
				    STMF_SAA_INSUFFICIENT_REG_RESOURCES);
				return;
			}
		...
		if (svc_key == 0) {
			...
			sbd_pgr_do_release(slu, it,
			    SBD_UA_REGISTRATIONS_PREEMPTED);
			if (sbd_get_preempt_its(task, 0,
			    B_FALSE, &sbd_it_session_ids,
			    &it_alloc_size) != SBD_SUCCESS) {
				stmf_scsilib_send_status(task, STATUS_CHECK,
				    STMF_SAA_INSUFFICIENT_REG_RESOURCES);
				return;
	...
	if (pr_out->action == PR_OUT_PREEMPT_ABORT &&
	    sbd_it_session_ids != NULL) {
		stmf_abort(STMF_QUEUE_ABORT_LU_SESS_TASK, task, STMF_ABORTED,
		    (void *)sbd_it_session_ids);
	}
	stmf_scsilib_send_status(task, STATUS_GOOD, 0);
preempt_return:
	if (sbd_it_session_ids != NULL) {
		kmem_free((void *)sbd_it_session_ids,
		    it_alloc_size * sizeof (uint64_t));
	}
}

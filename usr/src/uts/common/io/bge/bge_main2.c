...
bge_reinit_buff_ring(buff_ring_t *brp, uint32_t ring)
{
	...
	srbdp = brp->sw_rbds;
	if ((DEVICE_5717_SERIES_CHIPSETS(brp->bgep)) &&
	    (ring == BGE_JUMBO_BUFF_RING)) {
		hw_ext_rbd_p = DMA_VPTR(brp->desc);
		for (slot = 0; slot < nslots; ++hw_ext_rbd_p, ++srbdp, ++slot) {
			hw_rbd_p = &(hw_ext_rbd_p->std);
			...
			hw_rbd_p->flags |=
			    ring_type_flag[ring] | RBD_FLAG_PACKET_END;
		}
	} else {
 ...
 */
static int
bge_addmac(void *arg, const uint8_t *mac_addr, uint64_t flags)
{
...
static int
bge_flag_intr_enable(mac_ring_driver_t rh)
{
	recv_ring_t *rrp = (recv_ring_t *)rh;
	bge_t *bgep = rrp->bgep;
...
static int
bge_flag_intr_disable(mac_ring_driver_t rh)
{
	recv_ring_t *rrp = (recv_ring_t *)rh;
	bge_t *bgep = rrp->bgep;
...
bge_alloc_bufs(bge_t *bgep)
{
	...
	if (DEVICE_5717_SERIES_CHIPSETS(bgep)) {
		...
		rxbuffdescsize +=
		    bgep->chipid.jumbo_slots * sizeof (bge_ext_rbd_t);
	} else {
		rxbuffdescsize = BGE_STD_SLOTS_USED;
...
bge_quiesce(dev_info_t *devinfo)
{
	...
	/* Enable MEMORY_ARBITER_MODE_REG to prevent chip hang */
	if (bgep->bge_chip_state == BGE_CHIP_STOPPED &&

...
bge_recycle_ring(bge_t *bgep, send_ring_t *srp)
{
	...
	for (n = 0, slot = srp->tc_next; slot != *srp->cons_index_p;
		...
		/*
		 * As the consumer index may be reset when the chipset is
		 * stopped (e.g., BCM5718), we just break when pbuf is NULL.
		 * Don't be afraid of memory leaks, bge_reinit_send_ring()
		 * will take care of it.
		 */
		if ((buf_item = ssbdp->pbuf) == NULL)
			break;
...
start_tx:
	for (count = 0; count < bgep->param_drain_max; ++count) {
		...
		if ((DEVICE_5717_SERIES_CHIPSETS(bgep)) &&
		    (hw_sbd_p->len > BGE_DEFAULT_MTU))
			hw_sbd_p->flags |= SBD_FLAG_JUMBO_PKT;
		if (pktp->vlan_tci != 0) {
...
bge_send_drain(caddr_t arg)
{
	...
	if ((bgep->bge_chip_state != BGE_CHIP_RUNNING) ||
	    !(bgep->param_link_up))
		BGE_DEBUG(("bge_send_drain: chip not running or link down"));
...
bge_m_tx(void *arg, mblk_t *mp)
{
	...
	/*
	 * Require running chip and link up unless internal loopback
	 * mode is set.
	 */
	if ((bgep->bge_chip_state != BGE_CHIP_RUNNING) ||
	    (!bgep->param_link_up && !BGE_LOOP_INTERNAL_ENABLED(bgep))) {

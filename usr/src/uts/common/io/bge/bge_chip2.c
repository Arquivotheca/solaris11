	...
	if (!(cidp->flags & CHIP_FLAG_NO_JUMBO) &&
		...
		if (DEVICE_5714_SERIES_CHIPSETS(bgep)) {
			...
			cidp->mbuf_lo_water_rdma =
			    RDMA_MBUF_LOWAT_5714_JUMBO;
			cidp->mbuf_lo_water_rmac =
			    MAC_RX_MBUF_LOWAT_5717_JUMBO;
			cidp->mbuf_hi_water = MBUF_HIWAT_5717_JUMBO;
	...
	uint32_t mask, uint32_t val)
{
	...
	BGE_DEBUG(("bge_chip_poll_engine(%p) failed: regno = 0x%lx",
	    (void *)bgep, regno));
...
bge_poll_firmware(bge_t *bgep)
{
	...
	uint32_t gen, val;
	uint32_t i;
	boolean_t fw_finished = B_FALSE;

	/*
	 * The firmware didn't complete initialization in past.  Most likely it
	 * will time out again with no result, but we want to avoid hogging the
	 * cpu in busy wait.
	 */
	if (bgep->poll_firmware_timedout)
		return (0ULL);
	...
	if (! fw_finished)
		bgep->poll_firmware_timedout = B_TRUE;
...
bge_chip_start(bge_t *bgep, boolean_t reset_phys)
{
	...
	if (DEVICE_5704_SERIES_CHIPSETS(bgep)) {
		...
		if (bgep->chipid.default_mtu > BGE_DEFAULT_MTU) {
			buff_ring_t *brp = &bgep->buff[BGE_JUMBO_BUFF_RING];
			bge_reg_put64(bgep, JUMBO_RCV_BD_RING_RCB_REG,
			    brp->desc.cookie.dmac_laddress);
			bge_reg_put32(bgep, JUMBO_RCV_BD_RING_RCB_REG + 8,
			    0);
		} else {
			bge_reg_put32(bgep, JUMBO_RCV_BD_RING_RCB_REG + 8,
	...
	if (DEVICE_5717_SERIES_CHIPSETS(bgep)) {
		if (bgep->chipid.default_mtu > BGE_DEFAULT_MTU)
			bge_reg_put32(bgep, JUMBO_RCV_BD_REPLENISH_REG,
			    bge_replenish_jumbo);
		bge_reg_put32(bgep, STD_REPLENISH_LWM_REG,
		    STD_REPLENISH_LWM_DEFAULT);
		bge_reg_put32(bgep, JUMBO_REPLENISH_LWM_REG,
		    JUMBO_REPLENISH_LWM_DEFAULT);
	}
	...
	if (bgep->chipid.tx_rings <= COALESCE_64_BYTE_RINGS &&
		...
		coalmode = 0;
	if (DEVICE_5717_SERIES_CHIPSETS(bgep))
		coalmode = COALESCE_CLR_TICKS_RX | COALESCE_32_BYTE_STATUS;
	...
	if (!bge_chip_enable_engine(bgep, WRITE_DMA_MODE_REG,
		...
		retval = DDI_FAILURE;
	/*
	 * Enables the fix for CQ25155, where a DMA FIFO overrun occurs
	 * if a large number of Rx BDs are fetched while the Tx MBUF
	 * is full and the Read DMA Fifo is empty.
	 */
	if (DEVICE_5717_SERIES_CHIPSETS(bgep))
		bge_reg_set32(bgep, READ_DMA_RESERVED_CTL_REG,
		    DMA_FIFO_OVERRUN_FIX_CQ25155);
	if (DEVICE_5723_SERIES_CHIPSETS(bgep) ||
	    DEVICE_5717_SERIES_CHIPSETS(bgep))
		bge_dma_rdprio = 0;
	...
	if (bgep->intr_type == DDI_INTR_TYPE_FIXED)
		...
		/*
		 * In some cases, after bge_intr_disable()/bge_intr_enable()
		 * are called, PCI_MSI_ENABLE_BIT could not be recovered.
		 * So we set this bit here to workaround it.
		 */
		if (DEVICE_5717_SERIES_CHIPSETS(bgep)) {
			msi_ctl = pci_config_get16(bgep->cfg_handle,
			    PCI_CONF_BGE_MSI_CTRL);
			pci_config_put16(bgep->cfg_handle,
			    PCI_CONF_BGE_MSI_CTRL,
			    msi_ctl | PCI_MSI_ENABLE_BIT);
		}
	}

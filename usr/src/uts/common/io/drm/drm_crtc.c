...
void *drm_mode_object_find(struct drm_device *dev,
		   ...
		   const struct drm_crtc_funcs *funcs)
{
	...
	(void) drm_mode_object_get(dev, &crtc->base, DRM_MODE_OBJECT_CRTC);
		     ...
		     int connector_type)
{
	...
	(void) drm_mode_object_get(dev, &connector->base, DRM_MODE_OBJECT_CONNECTOR);
		      ...
		      int encoder_type)
{
	...
	(void) drm_mode_object_get(dev, &encoder->base, DRM_MODE_OBJECT_ENCODER);
...
struct drm_display_mode *drm_mode_create(struct drm_device *dev)
{
	...
	(void) drm_mode_object_get(dev, &nmode->base, DRM_MODE_OBJECT_MODE);
	return nmode;
}
...
static int drm_mode_create_standard_connector_properties(struct drm_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(drm_dpms_enum_list); i++)
		(void) drm_property_add_enum(dpms, i, drm_dpms_enum_list[i].type,
...
int drm_mode_create_dvi_i_properties(struct drm_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(drm_dvi_i_select_enum_list); i++)
		(void) drm_property_add_enum(dvi_i_selector, i,
	...
	for (i = 0; i < ARRAY_SIZE(drm_dvi_i_subconnector_enum_list); i++)
		(void) drm_property_add_enum(dvi_i_subconnector, i,
				  ...
				  char *modes[])
{
	...
	for (i = 0; i < ARRAY_SIZE(drm_tv_select_enum_list); i++)
		(void) drm_property_add_enum(tv_selector, i,
	...
	for (i = 0; i < ARRAY_SIZE(drm_tv_subconnector_enum_list); i++)
		(void) drm_property_add_enum(tv_subconnector, i,
...
int drm_mode_create_scaling_mode_property(struct drm_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(drm_scaling_mode_enum_list); i++)
		(void) drm_property_add_enum(scaling_mode, i,
...
int drm_mode_create_dithering_property(struct drm_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(drm_dithering_mode_enum_list); i++)
		(void) drm_property_add_enum(dithering_mode, i,
...
int drm_mode_create_dirty_info_property(struct drm_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(drm_dirty_info_enum_list); i++)
		(void) drm_property_add_enum(dirty_info, i,
...
int drm_mode_detachmode_crtc(struct drm_device *dev, struct drm_display_mode *mode)
{
	...
	list_for_each_entry(connector, struct drm_connector, &dev->mode_config.connector_list, head) {
		(void) drm_mode_detachmode(dev, connector, mode);
	}
	return 0;
}
					 ...
					 const char *name, int num_values)
{
	...
	(void) drm_mode_object_get(dev, &property->base, DRM_MODE_OBJECT_PROPERTY);
							  ...
							  void *data)
{
	...
	(void) drm_mode_object_get(dev, &blob->base, DRM_MODE_OBJECT_BLOB);
			       ...
			       struct drm_property_blob *blob)
{
	...
	list_del(&blob->head);
	kfree(blob, sizeof(struct drm_property_blob) + 128);
}
...
int drm_mode_connector_property_set_ioctl(DRM_IOCTL_ARGS)
{
	...
	if (!ret)
		(void) drm_connector_property_set_value(connector, property, out_resp->value);
out:
	mutex_unlock(&dev->mode_config.mutex);
	return ret;
}
...
int drm_mode_page_flip_ioctl(DRM_IOCTL_ARGS)
{
	...
	if (page_flip->flags & DRM_MODE_PAGE_FLIP_EVENT) {
		...
		e->base.destroy =
                       (void (*) (struct drm_pending_event *, size_t)) kfree;
	}

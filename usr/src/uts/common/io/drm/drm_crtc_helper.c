...
void drm_encoder_helper_add(struct drm_encoder *encoder,
				  const struct drm_encoder_helper_funcs *funcs)
{
	encoder->helper_private = (void *)funcs;
}

int drm_connector_helper_add(struct drm_connector *connector,
			  ...
			  int n, int width, int height)
{
	...
	list_for_each_entry(crtc, struct drm_crtc, &dev->mode_config.crtc_list, head) {
		...
		crtcs[n] = crtc;
		(void) memcpy(crtcs, best_crtcs, n * sizeof(struct drm_crtc *));
...
static void drm_setup_crtcs(struct drm_device *dev)
{
	...
	(void) drm_pick_crtcs(dev, crtcs, modes, 0, width, height);
...
int drm_crtc_helper_set_config(struct drm_mode_set *set)
{
	...
	DRM_DEBUG_KMS("crtc: %p %d fb: %p connectors: %p num_connectors:"
			" %d (x, y) (%i, %i)\n",
	          (void *)set->crtc, set->crtc->base.id, (void *)set->fb,
	          (void *)set->connectors,
	...
	list_for_each_entry(connector, struct drm_connector, &dev->mode_config.connector_list, head) {
		...
		DRM_DEBUG_KMS("setting connector %d crtc to %p\n",
			  connector->base.id, (void *)new_crtc);
	}
...
bool drm_helper_plugged_event(struct drm_device *dev)
{
	...
	(void) drm_helper_probe_connector_modes(dev, dev->mode_config.max_width,
...
{
	(void) drm_helper_plugged_event(dev);

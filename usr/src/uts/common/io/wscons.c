...
/*
 * Locking Policy:
 * This module has a D_MTQPAIR inner perimeter which means STREAMS
 * allows mutltiple threads to enter this module through STREAMS entry
 * points -- open() close() put() srv() qtimeout().  But only one thread
 * enters into a specific stream, queue pair or minor instance.
 * So for the most time we do not need locking for the stream specific
 * global data like most of data members in "vc_state_t" variables.
 * But global data shared by all streams or other modules need locking:
 *
 *   - wc shares four global variables ("wc_dip", "vc_active_console",
 *     "vc_cons_user", "vc_avl_root") with virtual console devname part
 *    (fs/dev/sdev_vtops.c) which get compiled into genunix. Especially,
 *    "wc_dip" is only be modified in wc_attach() which is serialized by
 *    the frame work along with other module entry points. And all
 *    the other accesses to "wc_dip" are read only.
 *
 *   - wc_modechg_cb() is a callback function which will be triggered when
 *     the framebuffer display mode is changed.
 *
 *   - vt_send_hotkeys() is triggered by timeout() which is not STREAMS MT
 *     safe.
 *
 *   - wc global variables which are shared by all stream  threads
 *     in the module like "vc_last_console", "vc_inuse_max_minor",
 *     "vc_target_console" and "vc_waitactive_list".
 *
 * Based on the analysis above, all access to the global variables
 * in THIS WC MODULE or in either DEVNAME PART or the CALLBACK need
 * locking with a few exceptions:
 * 1) "wc_dip" is only be modified in wc_attach() which is serialized
 *    along with other stream entry points by the framework.  So read
 *    access to "wc_dip" in this module doesn't need locking.
 *
...
/*
 * "wscons_state" variables are used when consconfig doing the
 * configurations work or polled io on "WC" driver.
 * And all the configurations work originate from kernel
 * thread(consconfig) which are safely serialized already.
 * Also, polled ios are safely single thread.
 */
_NOTE(SCHEME_PROTECTS_DATA("scheme serialized data", wscons))
_NOTE(SCHEME_PROTECTS_DATA("scheme serialized data", wscons_state))
/*
 * Function specific local variables
 */
_NOTE(SCHEME_PROTECTS_DATA("lock free data", vt_stat))
/*
 * Wait active message variables are referenced in
 * vc_waitactive_reply(), ioctl(VT_WAITACTIVE).
 * The data reference in vc_waitactive_reply() will be serialized by
 * the lock protecting both the list and message data.
 * ioctl(VT_WAITACTIVE) creates and intializes the new message data
 * which is invisiable to others unless it is inserted into the list.
 *
 */
_NOTE(SCHEME_PROTECTS_DATA("scheme serialized data", vc_waitactive_msg))
/*
 * All "tty_common" variables are linked with a queue specific "vc_state_t"
 * which are serialized by the perimeters.
 */
_NOTE(SCHEME_PROTECTS_DATA("scheme serialized data", tty_common))
/*
 * Function specific local variables
 */
_NOTE(SCHEME_PROTECTS_DATA("scheme serialized data", vt_mode))
/*
 * Function specific local variables
 */
_NOTE(SCHEME_PROTECTS_DATA("lock free data", vt_dispinfo))

/*
 * All "winsize" variables are linked with a queue specific "vc_state_t"
 * which are serialized by the perimeters.
 */
_NOTE(SCHEME_PROTECTS_DATA("lock free data", winsize))
/*
 * "vc_last_console" was referenced in  vt_open(), wcclose(),
 * vc_activate(), vt_switch(), vt_check_hotkeys(). They are
 * all protected by "vc_lock".
 */
_NOTE(MUTEX_PROTECTS_DATA(vc_lock, vc_last_console))
_NOTE(MUTEX_PROTECTS_DATA(vc_lock, vc_active_console vc_cons_user))
...
DDI_DEFINE_STREAM_OPS(wc_ops, nulldev, nulldev, wc_attach, nodev, nodev,
    wc_info, D_MTQPAIR | D_MP, &wcinfo, ddi_quiesce_not_supported);
...
wc_polled_putchar(cons_polledio_arg_t arg, unsigned char c)
{
	...
	/*
	 * Only one thread for polled mode
	 */

	pvc = vt_minor2vc_locked(VT_ACTIVE, B_TRUE);
...
wc_modechg_cb(tem_modechg_cb_arg_t arg)
{
	...
	for (index = 0; index < VC_INSTANCES_COUNT; index++) {
		pvc = vt_minor2vc_locked(index, B_FALSE);

...
#define	IXGBE_TX_METHOD(mp, len, copy_threshold)		\
	(DBLK_IS_PREMAPPED((mp)->b_datap) ? USE_DMA :		\
	((len) <= (copy_threshold) ? USE_COPY : USE_DMA))
...
ixgbe_ring_tx(void *arg, mblk_t *mp)
{
	...
	mblk_t *hdr_nmp = NULL;
	boolean_t hdr_mp_premap_ok = B_TRUE;
	...
	if ((ixgbe->tx_hcksum_enable) || (ixgbe->fcoe_txcrc_enable)) {
		/*
		...
		if ((ctx->lso_flag &&
		    ...
		    (ctx->fcoe_lso_flag &&
		    ((mbsize - ctx->fcoe_hdr_len - IXGBE_FC_TRAILER_LEN) >
		    IXGBE_FCOE_LSO_MAXLEN)) ||
		    (!ctx->lso_flag && !ctx->fcoe_lso_flag &&
	...
	if ((ctx != NULL) && ctx->lso_flag) {
		...
		if (len < hdr_len || P2NPHASE((uintptr_t)mp->b_rptr,
		    ixgbe->sys_page_size) < hdr_len)
			hdr_mp_premap_ok = B_FALSE;
...
adjust_threshold:
	 ...
	 */
	if (hdr_mp_premap_ok && DBLK_IS_PREMAPPED(current_mp->b_datap)) {
		current_flag = USE_DMA;
	} else {
	...
	while (current_mp) {
		...
		/*
		 * Excluding the FC trailer for FCoE TX CRC offload
		 */
		if (eop &&
		    (ctx != NULL) &&
		    (ctx->fcoe_flags & MAC_FCOE_ENABLE_TXCRC)) {
			current_len -= IXGBE_FC_TRAILER_LEN;
		}
		...
		if ((current_len == 0) && (copy_done)) {
			...
			current_flag = IXGBE_TX_METHOD(current_mp,
			    current_len, copy_thresh);
			continue;
		}
		...
		if (current_flag == USE_COPY) {
			...
			if (eop) {
				...
				next_flag = USE_NONE;
				copy_done = B_TRUE;
			} else if (DBLK_IS_PREMAPPED(next_mp->b_datap)) {
				next_flag = USE_DMA;
				copy_done = B_TRUE;
			 ...
			 */
			if (!eop) {
				next_flag = IXGBE_TX_METHOD(next_mp,
				    next_len, copy_thresh);
			}
	...
	/*
	 * 82598/82599/X540 chipset has a limitation that no more than 32 tx
	 * descriptors can be transmitted out at one time.
	 *
    ...
    uint32_t len)
{
	...
	uint_t ncookies;
	int desc_num;
	boolean_t premapped = B_FALSE;
	ddi_dma_cookie_t cookies[DMAB_NCOOKIES];
	ddi_dma_handle_t dma_handle;

	if (mp->b_datap->db_flags & DBLK_DMA_PREMAP) {
		status = dblk_dma_info_get(
		    mac_pmh_tx_get(tx_ring->ixgbe->mac_hdl), mp->b_rptr, len,
		    mp->b_datap, &cookies[0], &ncookies, &dma_handle);
	...
	if (premapped) {
		...
		tx_ring->stat_opackets_premap++;
		tcb->tx_type = USE_NONE;
		for (j = 0; j < ncookies; j++) {
			ixgbe_save_desc(tcb, cookies[j].dmac_laddress,
			    (int)cookies[j].dmac_size);
			desc_num++;
		}
	} else {
		tx_ring->stat_opackets_bind++;
		...
		for (i = ncookies; i > 0; i--) {
			 ...
			 */
			ixgbe_save_desc(tcb, dma_cookie.dmac_laddress,
			    dma_cookie.dmac_size);

			desc_num++;

			if (i > 1)
				ddi_dma_nextcookie(tcb->tx_dma_handle,
				    &dma_cookie);
		}
	}
...
ixgbe_get_context(mblk_t *mp, ixgbe_tx_context_t *ctx)
{
	...
	bzero(&mftp, sizeof (mac_fcoe_tx_params_t));
	mac_fcoe_get(mp, &mftp, &fcoeflags);

	if ((hckflags == 0) && (fcoeflags != HW_FCOE)) {
		return (0);
	}
	...
	case ETHERTYPE_IPV6:
		...
		break;
	case ETHERTYPE_FCOE:
		ctx->fcoe_flags = mftp.flags;
		if (ctx->fcoe_flags & MAC_FCOE_ENABLE_TXCRC) {
			ctx->fcoe_sof = mftp.sof;
			ctx->fcoe_eof = mftp.eof;
			ctx->fcoe_fc_hdr_len = mftp.fc_hdr_len;
			/*
			 * ether_hdr + fcoe_hdr + fc_hdr
			 */
			fcoe_hdr_len = mac_hdr_len + IXGBE_FCOE_HDR_LEN +
			    ctx->fcoe_fc_hdr_len;
			/*
			 * ether_hdr + fcoe_hdr - 4
			 */
			fcoe_mac_len = mac_hdr_len + IXGBE_FCOE_HDR_LEN - 4;
		}
		if (ctx->fcoe_flags & MAC_FCOE_ENABLE_LSO) {
			if (!(ctx->fcoe_flags & MAC_FCOE_ENABLE_TXCRC)) {
				IXGBE_DEBUGLOG_0(NULL, "ixgbe_tx: h/w "
				    "crc flag is not specified for FCoE LSO");
				return (-1);
			}
			ctx->fcoe_lso_flag = B_TRUE;
			ctx->fcoe_mss = (uint32_t)(mftp.mss);
			if (ctx->fcoe_flags & MAC_FCOE_REL_OFFSET)
				ctx->fcoe_fc_f_ctl = B_TRUE;
			else
				ctx->fcoe_fc_f_ctl = B_FALSE;
		}
		break;
	default:
		/* Unrecoverable error */
	...
	if (ctx->fcoe_flags) {
		ctx->fcoe_hdr_len = fcoe_hdr_len;
		ctx->fcoe_mac_len = fcoe_mac_len;
	} else {
...
ixgbe_check_context(ixgbe_tx_ring_t *tx_ring, ixgbe_tx_context_t *ctx)
{
	...
	if ((ctx->hcksum_flags != last->hcksum_flags) ||
	    ...
	    (ctx->mss != last->mss) ||
	    (ctx->fcoe_flags != last->fcoe_flags) ||
	    (ctx->fcoe_fc_f_ctl != last->fcoe_fc_f_ctl) ||
	    (ctx->fcoe_mss != last->fcoe_mss) ||
	    (ctx->fcoe_hdr_len != last->fcoe_hdr_len) ||
	    (ctx->fcoe_mac_len != last->fcoe_mac_len) ||
	    (ctx->fcoe_sof != last->fcoe_sof) ||
	    (ctx->fcoe_eof != last->fcoe_eof) ||
	    (ctx->fcoe_fc_hdr_len != last->fcoe_fc_hdr_len)) {
		return (B_TRUE);
	}
...
{
	uint32_t fcoe_sof_eof = 0;

	/*
	 * Fill the context descriptor with the context
	 * information we've got.
	 */

	if (ctx->fcoe_flags) {
		ctx_tbd->vlan_macip_lens = ctx->fcoe_hdr_len;
		ctx_tbd->vlan_macip_lens |= ctx->fcoe_mac_len <<
	...
	if (ctx->fcoe_flags & MAC_FCOE_ENABLE_TXCRC) {
		ctx_tbd->type_tucmd_mlhl |= IXGBE_ADVTXT_TUCMD_FCOE;
	}
	...
	if (ctx->lso_flag) {
	...
	} else if ((ctx->fcoe_flags & MAC_FCOE_ENABLE_TXCRC) ||
	    ctx->fcoe_lso_flag) {
		switch (ctx->fcoe_sof) {
		case IXGBE_FC_SOF_N2:
			break;
		case IXGBE_FC_SOF_N3:
			fcoe_sof_eof |= IXGBE_ADVTXD_FCOEF_SOF;
			break;
		case IXGBE_FC_SOF_I2:
			fcoe_sof_eof |= IXGBE_ADVTXD_FCOEF_ORIS;
			break;
		case IXGBE_FC_SOF_I3:
			fcoe_sof_eof |= IXGBE_ADVTXD_FCOEF_SOF;
			fcoe_sof_eof |= IXGBE_ADVTXD_FCOEF_ORIS;
			break;
		default:
			/* Unknown sof */
			IXGBE_DEBUGLOG_1(NULL, "Error sof 0x%x",
			    ctx->fcoe_sof);
			break;
		}
		...
		default:
			/* Unknown eof */
			IXGBE_DEBUGLOG_1(NULL, "Error eof 0x%x",
			    ctx->fcoe_eof);
			break;
		}

		if (ctx->fcoe_lso_flag && ctx->fcoe_fc_f_ctl)
			fcoe_sof_eof |= IXGBE_ADVTXD_FCOEF_PARINC;

		ctx_tbd->seqnum_seed = fcoe_sof_eof;

		ctx_tbd->mss_l4len_idx =
		    (ctx->fcoe_mss << IXGBE_ADVTXD_MSS_SHIFT) |
    ...
    ixgbe_tx_context_t *ctx, size_t mbsize)
{
	 ...
	 * 82599/X540 requires the packet length in paylen field with or without
	...
	case ixgbe_mac_X540:
		if (ctx != NULL && ctx->lso_flag) {
			...
			first_tbd->read.olinfo_status |=
			    (mbsize - ctx->mac_hdr_len - ctx->ip_hdr_len
			    - ctx->l4_hdr_len) << IXGBE_ADVTXD_PAYLEN_SHIFT;
		} else if (ctx != NULL && ctx->fcoe_lso_flag) {
			/*
			 * For FCoE offload, context idx 1 is used
			 */
			tbd->read.cmd_type_len |= IXGBE_ADVTXD_DCMD_IFCS;
			first_tbd->read.cmd_type_len |= IXGBE_ADVTXD_DCMD_TSE;
			first_tbd->read.olinfo_status |=
			    ((mbsize - ctx->mac_hdr_len - IXGBE_FCOE_HDR_LEN
			    - ctx->fcoe_fc_hdr_len - IXGBE_FC_TRAILER_LEN)
	...
	if (tx_ring->ixgbe->sriov_pf)
		first_tbd->read.olinfo_status |= IXGBE_ADVTXD_CC;

	/*
	 * The last descriptor of packet needs End Of Packet (EOP),
	 * and Report Status (RS) bits set. Do not set the RS bit if
	 * WTHRESH is set to a non-zero value.
	 */
	ASSERT(tbd != NULL);
	tbd->read.cmd_type_len |= IXGBE_ADVTXD_DCMD_EOP |
	    (tx_ring->wthresh_nonzero ? 0x0 : IXGBE_ADVTXD_DCMD_RS);
	 ...
	 */
	IXGBE_WRITE_REG(hw, IXGBE_TDT(tx_ring->hw_index), index);
...
ixgbe_tx_recycle_legacy(ixgbe_tx_ring_t *tx_ring)
{
	...
	while (tcb != NULL) {
		 ...
		 * For 82599/X540, LSO descriptors can not be recycled

...
ixgbe_update_stats(kstat_t *ks, int rw)
{
	...
	ixgbe_ks->ampfc.value.ui64 = ixgbe->alloc_mp_fail;
	...
	for (i = 0; i  < MAX_TRAFFIC_CLASS; i++) {
		...
		ixgbe_ks->pxofftxc[i].value.ui64 += IXGBE_READ_REG(hw,
		    IXGBE_PXOFFTXC(i));
		/* Priority XOFF Received */
		ixgbe_ks->pxoffrxc[i].value.ui64 += IXGBE_READ_REG(hw,
		    IXGBE_PXOFFRXCNT(i));
	}

	ixgbe_ks->fccrc.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCCRC);
	ixgbe_ks->fcoerpdc.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCOERPDC);
	ixgbe_ks->fclast.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCLAST);
	ixgbe_ks->fcoeprc.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCOEPRC);
	ixgbe_ks->fcoedwrc.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCOEDWRC);
	ixgbe_ks->fcoeptc.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCOEPTC);
	ixgbe_ks->fcoedwtc.value.ui64 += IXGBE_READ_REG(hw, IXGBE_FCOEDWTC);
...
ixgbe_init_stats(ixgbe_t *ixgbe)
{
	...
	kstat_named_init(&ixgbe_ks->pxofftxc[0], "link_pxoff_xmitd[0]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[1], "link_pxoff_xmitd[1]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[2], "link_pxoff_xmitd[2]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[3], "link_pxoff_xmitd[3]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[4], "link_pxoff_xmitd[4]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[5], "link_pxoff_xmitd[5]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[6], "link_pxoff_xmitd[6]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxofftxc[7], "link_pxoff_xmitd[7]",
	    KSTAT_DATA_UINT64);

	kstat_named_init(&ixgbe_ks->pxoffrxc[0], "link_pxoff_recvd[0]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[1], "link_pxoff_recvd[1]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[2], "link_pxoff_recvd[2]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[3], "link_pxoff_recvd[3]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[4], "link_pxoff_recvd[4]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[5], "link_pxoff_recvd[5]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[6], "link_pxoff_recvd[6]",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->pxoffrxc[7], "link_pxoff_recvd[7]",
	    KSTAT_DATA_UINT64);
	...
	kstat_named_init(&ixgbe_ks->lroc, "lro_pkt_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->ampfc, "alloc_mp_fail_count",
	    KSTAT_DATA_UINT64);

	kstat_named_init(&ixgbe_ks->fccrc, "fc_crc_error_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->fcoerpdc, "fcoe_rx_pkt_drop_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->fclast, "fc_last_error_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->fcoeprc, "fcoe_pkt_recvd_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->fcoedwrc, "fcoe_dword_recvd_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->fcoeptc, "fcoe_pkt_xmitd_count",
	    KSTAT_DATA_UINT64);
	kstat_named_init(&ixgbe_ks->fcoedwtc, "fcoe_dword_xmitd_count",
	    KSTAT_DATA_UINT64);

...
ixgbe_dump_interrupt(void *adapter, char *tag)
{
	...
	while (j >= 0) {
		hw_index = ixgbe->tx_rings[j].hw_index;
		ixgbe_log(ixgbe, "tx %d  ivar %d  txdctl: 0x%x\n",
		    hw_index, IXGBE_IVAR_TX_QUEUE(hw_index),
		    IXGBE_READ_REG(hw, IXGBE_TXDCTL(hw_index)));
	...
	ixgbe_log(ixgbe, "-- mpsar --\n");
	for (i = 0; i < 16; i++) {
		if (reg = IXGBE_READ_REG(hw, IXGBE_MPSAR_LO(i))) {
			ixgbe_log(ixgbe, "lo(%d): 0x%x  hi(%d): 0x%x\n",
			    i, reg, i, IXGBE_READ_REG(hw, IXGBE_MPSAR_HI(i)));
		}
	}
...
ixgbe_dump_regs(void *adapter)
{
	...
	reg_val = IXGBE_READ_REG(hw, IXGBE_MTQC);
	ixgbe_log(ixgbe, "\tMTQC=%x\n", reg_val);
	for (i = 0; i < ixgbe->num_tx_rings; i++) {
		hw_index = ixgbe->tx_rings[i].hw_index;
		reg_val = IXGBE_READ_REG(hw, IXGBE_TXDCTL(hw_index));
		ixgbe_log(ixgbe, "\tTXDCTL(%d)=%x\n", hw_index, reg_val);
		reg_val = IXGBE_READ_REG(hw, IXGBE_TDWBAL(hw_index));
		ixgbe_log(ixgbe, "\tTDWBAL(%d)=%x\n", hw_index, reg_val);
		reg_val = IXGBE_READ_REG(hw, IXGBE_TDWBAH(hw_index));
		ixgbe_log(ixgbe, "\tTDWBAH(%d)=%x\n", hw_index, reg_val);
		reg_val = IXGBE_READ_REG(hw, IXGBE_TXPBSIZE(hw_index));
		ixgbe_log(ixgbe, "\tTXPBSIZE(%d)=%x\n", hw_index, reg_val);
	}

	ixgbe_log(ixgbe, "SR-IOV registers");
	reg_val = IXGBE_READ_REG(hw, IXGBE_VT_CTL);
	ixgbe_log(ixgbe, "\tVT_CTL=%x\n", reg_val);

	reg_val = IXGBE_READ_REG(hw, IXGBE_GCR_EXT);
	ixgbe_log(ixgbe, "\tGCR_EXT=%x\n", reg_val);

	reg_val = IXGBE_READ_REG(hw, IXGBE_VFRE(0));
	ixgbe_log(ixgbe, "\tIXGBE_VFRE(0)=%x\n", reg_val);
	reg_val = IXGBE_READ_REG(hw, IXGBE_VFRE(1));
	ixgbe_log(ixgbe, "\tIXGBE_VFRE(1)=%x\n", reg_val);
	reg_val = IXGBE_READ_REG(hw, IXGBE_VFTE(0));
	ixgbe_log(ixgbe, "\tIXGBE_VFTE(0)=%x\n", reg_val);
	reg_val = IXGBE_READ_REG(hw, IXGBE_VFTE(1));
	ixgbe_log(ixgbe, "\tIXGBE_VFTE(1)=%x\n", reg_val);
}

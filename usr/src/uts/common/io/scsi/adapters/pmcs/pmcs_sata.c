...
static int pmcs_run_sata_cmd(pmcs_hw_t *, pmcs_phy_t *, fis_t, uint32_t,
...
pmcs_sata_special_work(pmcs_hw_t *pwp, pmcs_xscsi_t *xp)
{
	...
	if (pkt->pkt_cdbp[0] == SCMD_INQUIRY ||
		...
		retval = pmcs_sata_identify(pwp, pptr, scratch_hdl);
		...
		id = (ata_identify_t *)pmcs_get_scratch_area(pwp, scratch_hdl);
 ...
 */
int
pmcs_sata_identify(pmcs_hw_t *pwp, pmcs_phy_t *pptr, int scratch_hdl)
{
	fis_t fis;
	...
	return (pmcs_run_sata_cmd(pwp, pptr, fis, SATA_PROTOCOL_PIO,
	    ...
	    pmcs_get_scratch_dma_area(pwp, scratch_hdl)));
}
...
pmcs_run_sata_cmd(pmcs_hw_t *pwp, pmcs_phy_t *pptr, fis_t fis, uint32_t mode,
    uint32_t ddir, uint32_t dlen, uint64_t scratch_dma)
{
	struct pmcwork *pwrk;
...
/*
 * Complete PHY up processing for SATA PHYs.  First, get the IDENTIFY
 * information and find out if it has a valid WWN.
 */
void
pmcs_sata_phy_up(pmcs_hw_t *pwp)
{
	pmcs_phy_t *phyp;
	pmcs_iport_t *iport;
	ata_identify_t *id;
	...
	while (phyp != NULL) {
		...
		if ((phyp->dtype == NOTHING) && (phyp->pend_dtype == SATA)) {
			phyp->dtype = SATA;

			if (pmcs_register_device(pwp, phyp)) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, phyp, NULL,
				    "%s: pmcs_register_phy failed for phy 0x%p",
				    __func__, (void *)phyp);
				goto phy_up_failed;
			}

			if (pmcs_sata_identify(pwp, phyp, scratch_hdl)) {
				pmcs_prt(pwp, PMCS_PRT_INFO, phyp, NULL,
				    "%s: ATA IDENTIFY failed", __func__);
				goto phy_up_failed;
			}

			if ((LE_16(id->word87) & 0x100) &&
			    (LE_16(id->word108) >> 12) == 5)  {
				phyp->sas_address[0] = LE_16(id->word108) >> 8;
				phyp->sas_address[1] = LE_16(id->word108);
				phyp->sas_address[2] = LE_16(id->word109) >> 8;
				phyp->sas_address[3] = LE_16(id->word109);
				phyp->sas_address[4] = LE_16(id->word110) >> 8;
				phyp->sas_address[5] = LE_16(id->word110);
				phyp->sas_address[6] = LE_16(id->word111) >> 8;
				phyp->sas_address[7] = LE_16(id->word111);
				pmcs_prt(pwp, PMCS_PRT_DEBUG, phyp, NULL,
				    "%s: Device has WWN " SAS_ADDR_FMT,
				    __func__, SAS_ADDR_PRT(phyp->sas_address));
				wwn = pmcs_barray2wwn(phyp->sas_address);
			} else {
				pmcs_prt(pwp, PMCS_PRT_DEBUG, phyp, NULL,
				    "%s: Device doesn't have WWN", __func__);
				wwn = pwp->sas_wwns[8 + phyp->phynum];
				pmcs_wwn2barray(wwn, phyp->sas_address);
			}

			pmcs_set_changed(pwp, phyp, B_TRUE, 0);
			mutex_exit(&phyp->phy_lock);

			/* Add this PHY to the phymap */
			if (sas_phymap_phy_add(pwp->hss_phymap, phyp->phynum,
			    pwp->sas_wwns[0], wwn) != DDI_SUCCESS) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG, phyp, NULL,
				    "Unable to add phy %u for 0x%" PRIx64 ".0x%"
				    PRIx64, phyp->phynum,
				    pwp->sas_wwns[phyp->phynum], wwn);
			}
			...
			if (iport) {
				...
				if (iport->ua_state == UA_ACTIVE) {
					pmcs_add_phy_to_iport(iport, phyp);
					phyp->iport = iport;
					ASSERT(iport->nphy == 1);
					iport->nphy = 1;
				}
			...
			pmcs_update_phy_pm_props(phyp, (1ULL << phyp->phynum),
			    1ULL, B_TRUE);
			phyp->sysevent |= PMCS_SAS_PHY_ONLINE;

			pwp->ports[phyp->portid] = phyp;
		}
		mutex_exit(&phyp->phy_lock);
		phyp = phyp->sibling;
	}

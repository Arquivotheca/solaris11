...
pmcs_complete_work(pmcs_hw_t *pwp, pmcwork_t *pwrk, uint32_t *iomb, size_t amt)
{
	...
	pmcs_complete_work_impl(pwp, pwrk, iomb, amt, B_FALSE);
}
...
pmcs_process_sas_hw_event(pmcs_hw_t *pwp, void *iomb, size_t amt)
{
	...
	case IOP_EVENT_SAS_PHY_UP:
	{
		...
		pmcs_update_phy_pm_props(pptr, (1ULL << phynum),
		    (1ULL << phy_id), B_TRUE);
		pptr->sysevent |= PMCS_SAS_PHY_ONLINE;
		pmcs_unlock_phy(pptr);
	...
	case IOP_EVENT_SATA_PHY_UP: {
		...
		/*
		 * Kick off work to go get the IDENTIFY information for this
		 * device and finish this process (add to phymap, etc.)
		 */
		SCHEDULE_WORK(pwp, PMCS_WORK_SATA_PHY_UP);
	...
	case IOP_EVENT_PHY_DOWN: {
		...
		if (pptr == NULL) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG, subphy, NULL,
		...
		if (IOP_EVENT_PORT_STATE(w3) == IOP_EVENT_PS_VALID) {
			...
			if (pptr == subphy) {
				...
				if (pptr) {
					...
					pmcs_prt(pwp, PMCS_PRT_DEBUG, pptr,
					    ...
					    pptr->phynum);
					/*
					 * Make sure the new PHY gets
					 * registered if necessary
					 */
					mutex_enter(&tphyp->phy_lock);
					if (!tphyp->valid_device_id) {
						pmcs_prt(pwp, PMCS_PRT_DEBUG,
						    pptr, NULL,
						    "PortID 0x%x: PHY 0x%x "
						    "was already deregistered",
						    portid,
						    tphyp->phynum);
						SCHEDULE_WORK(pwp,
						    PMCS_WORK_REGISTER_DEV);
						(void) ddi_taskq_dispatch(
						    pwp->tq, pmcs_worker, pwp,
						    DDI_NOSLEEP);
					}
					mutex_exit(&tphyp->phy_lock);
				} else {
			...
			pmcs_prt(pwp, PMCS_PRT_INFO, subphy, NULL,
		...
		pmcs_update_phy_pm_props(pptr, pptr->att_port_pm_tmp,
		    pptr->tgt_port_pm_tmp, B_FALSE);
		pptr->sysevent |= PMCS_SAS_PHY_OFFLINE;
		mutex_exit(&pptr->phy_lock);
	...
	case IOP_EVENT_BROADCAST_CHANGE:
		...
		need_ack = 1;
		mutex_enter(&pwp->lock);
		PMCS_SET_BC_PROCESS(pwp, phynum);
		mutex_exit(&pwp->lock);
		break;
	case IOP_EVENT_BROADCAST_SES:
		...
		if (pptr) {
			pmcs_lock_phy(pptr);
			pptr->sysevent |= PMCS_SAS_PORT_BROADCAST_SES;
			pmcs_unlock_phy(pptr);
		}
		break;
	...
	case IOP_EVENT_BROADCAST_EXP:
		...
		if (pptr) {
			pmcs_lock_phy(pptr);
			pptr->sysevent |= PMCS_SAS_PORT_BROADCAST_D01_4;
			pmcs_unlock_phy(pptr);
		}
		break;
	...
	case IOP_EVENT_BROADCAST_ASYNC_EVENT:
		...
		if (pptr) {
			pmcs_lock_phy(pptr);
			pptr->sysevent |= PMCS_SAS_PORT_BROADCAST_D04_7;
			pmcs_unlock_phy(pptr);
		}
		break;
	default:
	...
	if (need_ack) {
		...
		PMCS_SET_ACK(pwp, phynum, PMCS_ACK_INFO(w1));
    ...
    boolean_t worker)
{
	pmcs_phy_t *pptr;
	struct pmcwork *pwrk;
	boolean_t lock_phy = B_TRUE;
	pmcs_abort_comp_t *pac_iomb = NULL;
	uint32_t htag = LE_32(((uint32_t *)iomb)[1]);
	...
	/*
	 * If discovery is running, it is possible for this interrupt thread to
	 * get stuck behind the  discovery thread and cause a livelock.
	 * So, schedule abort completion for later.
	 */
	mutex_enter(&pwp->config_lock);
	if ((pwp->configuring) && (worker == B_FALSE)) {
		mutex_exit(&pwp->config_lock);
		pac_iomb = kmem_cache_alloc(pwp->abort_comp_cache, KM_NOSLEEP);
		if (pac_iomb == NULL) {
			pmcs_prt(pwp, PMCS_PRT_WARN, NULL, NULL,
		...
		bcopy(iomb, &pac_iomb->iomb[0], amt);
		pac_iomb->amt = amt;
		mutex_enter(&pwp->abort_comp_list_lock);
		list_insert_tail(&pwp->abort_comp_list, pac_iomb);
		mutex_exit(&pwp->abort_comp_list_lock);

		pmcs_prt(pwp, PMCS_PRT_DEBUG1, NULL, NULL, "%s: Deferred "
		    " processing of abort_compl in %s thread ", __func__,
		    worker ? "worker" : "interrupt");
		SCHEDULE_WORK(pwp, PMCS_WORK_ABORT_COMPLETION);
		(void) ddi_taskq_dispatch(pwp->tq, pmcs_worker,
		    pwp, DDI_NOSLEEP);
		return;
	}
	mutex_exit(&pwp->config_lock);
default_process:
	pmcs_prt(pwp, PMCS_PRT_DEBUG1, NULL, NULL, "%s: Processing abort_"
	    "compl in %s thread ", __func__, worker ? "worker" : "interrupt");

	pwrk = pmcs_tag2wp(pwp, htag, lock_phy);
	if (pwrk == NULL) {
		pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL,
...
/*
 * Called by worker thread for offline abort processing
 */
void
pmcs_abort_completion_work(pmcs_hw_t *pwp)
{
	list_t pac_list;
	pmcs_abort_comp_t *pac_iomb, *next_pac_iomb;

	list_create(&pac_list, sizeof (pmcs_abort_comp_t),
	    offsetof(pmcs_abort_comp_t, list_node));

	mutex_enter(&pwp->abort_comp_list_lock);
	list_move_tail(&pac_list, &pwp->abort_comp_list);
	mutex_exit(&pwp->abort_comp_list_lock);

	pac_iomb = list_head(&pac_list);
	while (pac_iomb != NULL) {
		next_pac_iomb = list_next(&pac_list, pac_iomb);
		pmcs_process_abort_completion(pwp,
		    (void *)&pac_iomb->iomb[0], pac_iomb->amt, B_TRUE);
		list_remove(&pac_list, (void *)pac_iomb);
		bzero(pac_iomb, sizeof (pmcs_abort_comp_t));
		kmem_cache_free(pwp->abort_comp_cache, pac_iomb);
		pac_iomb = next_pac_iomb;
	}

	list_destroy(&pac_list);
}
...
pmcs_general_intr(pmcs_hw_t *pwp)
{
	...
	while (ci != pi) {
		...
		case PMCOUT_SMP_ABORT:
			pmcs_process_abort_completion(pwp, local, amt, B_FALSE);
			break;
...
pmcs_iodone_intr(pmcs_hw_t *pwp)
{
	...
	uint32_t w0, ci, pi, nbuf, niodone = 0, iomb_opcode;
	size_t amt;
	uint32_t *ptr;
	...
	DTRACE_PROBE1(pmcs__iodone, uint32_t, niodone);

...
pmcs_iport_attach(dev_info_t *dip)
{
	...
	mutex_init(&iport->refcnt_lock, NULL, MUTEX_DRIVER,
	    DDI_INTR_PRI(pwp->intr_pri));
	mutex_init(&iport->iport_changed_lock, NULL, MUTEX_DRIVER,
	    DDI_INTR_PRI(pwp->intr_pri));
...
iport_attach_fail2:
	...
	mutex_destroy(&iport->iport_changed_lock);
...
pmcs_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	pmcs_hw_t *pwp;
	pmcs_phy_t *phyp;
	uint32_t num_threads, bpp;
	char buf[64];
	char *fwl_file;
	pmcs_scratch_t *scratchp;
	...
	list_create(&pwp->abort_comp_list, sizeof (pmcs_abort_comp_t),
	    offsetof(pmcs_abort_comp_t, list_node));
	...
	pwp->sgllen = ddi_prop_get_int(DDI_DEV_T_ANY, dip,
	    DDI_PROP_DONTPASS | DDI_PROP_NOTPROM, "pmcs-sgllen", PMCS_SGLLEN);
	...
	pwp->iqp_dma_attr = pwp->oqp_dma_attr =
	    ...
	    pwp->fwlog_dma_attr = pwp->scratch_dma_attr = pmcs_dattr;
	 ...
	 */
	(void) pmcs_add_more_chunks(pwp, bpp * PMCS_MIN_CHUNK_PAGES);
	...
	/*
	 * Cache of abort completion IOMBs to be processed off worker thread
	 */
	(void) snprintf(buf, sizeof (buf), "pmcs_abort_comp_cache%d", inst);
	pwp->abort_comp_cache = kmem_cache_create(buf,
	    sizeof (pmcs_abort_comp_t), 16, NULL, NULL, NULL, NULL, NULL, 0);
	...
	pwp->cq_info.cq_threads_running = 0;
	...
	/*
	 * Alloc memory for pending HW ACKs and Broadcast processing
	 */
	pwp->ack_info = kmem_zalloc(pwp->nphy * sizeof (uint32_t), KM_SLEEP);
	pwp->ack_bitmap = 0;
	...
	for (i = 0; i < pwp->max_cmd; i++) {
		...
		mutex_init(&pwrk->lock, NULL, MUTEX_DRIVER,
		    DDI_INTR_PRI(pwp->intr_pri));
		mutex_init(&pwrk->sleep_lock, NULL, MUTEX_DRIVER,
		    DDI_INTR_PRI(pwp->intr_pri));
	...
	/*
	 * Allocate DMA area for "scratch", one per PHY
	 */
	pwp->scratch_dma_attr.dma_attr_align = sizeof (uint32_t);
	if (pmcs_dma_setup(pwp, &pwp->scratch_dma_attr, &pwp->scratch_acchdls,
	    &pwp->scratch_handle, pwp->nphy * bpp,
	    (caddr_t *)&pwp->scratch_addr, &pwp->scratch_dma) == B_FALSE) {
		pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL,
		    "Failed to setup scratch regions");
		goto failure;
	}
	bzero(pwp->scratch_addr, pwp->nphy * bpp);

	pwp->scratch = kmem_zalloc(sizeof (pmcs_scratch_t) * pwp->nphy,
	    KM_SLEEP);
	for (i = 0; i < pwp->nphy; i++) {
		scratchp = pwp->scratch + i;
		scratchp->scratch_dma = pwp->scratch_dma + (bpp * i);
		scratchp->scratch_addr = (void *)
		    ((uint8_t *)pwp->scratch_addr + (bpp * i));
		scratchp->scratch_lock = 0;
	}
	...
	/*
	 * If there are register dumps available in the flash, read them
	 * into memory so they can be retrieved if required.
	 */
	(void) pmcs_load_register_dumps(pwp, NULL, 0, B_TRUE);
...
pmcs_unattach(pmcs_hw_t *pwp)
{
	...
	mutex_enter(&pwp->abort_comp_list_lock);
	ASSERT(list_is_empty(&pwp->abort_comp_list));
	list_destroy(&pwp->abort_comp_list);
	mutex_exit(&pwp->abort_comp_list_lock);
	...
	if (pwp->scratch_handle) {
		if (ddi_dma_unbind_handle(pwp->scratch_handle) != DDI_SUCCESS) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL,
			    "Condition check failed "
			    "at %s():%d", __func__, __LINE__);
		}
		ddi_dma_free_handle(&pwp->scratch_handle);
		ddi_dma_mem_free(&pwp->scratch_acchdls);
		pwp->scratch_handle = 0;
		kmem_free(pwp->scratch, sizeof (pmcs_scratch_t) * pwp->nphy);
		pwp->scratch = NULL;
	}
	...
	/*
	 * Free abort completion IOMB cache
	 */
	if (pwp->abort_comp_cache) {
		kmem_cache_destroy(pwp->abort_comp_cache);
	}
	 ...
	 */
	if (pwp->ack_info) {
		kmem_free(pwp->ack_info, pwp->nphy * sizeof (uint32_t));
		pwp->ack_info = NULL;
	}
	...
	if (pwp->flash_regdumpp) {
		kmem_free(pwp->flash_regdumpp, PMCS_FLASH_REG_DUMP_SIZE);
		pwp->flash_regdumpp = NULL;
	}
	...
	if (pwp->locks_initted) {
		...
		mutex_destroy(&pwp->ack_lock);
		mutex_destroy(&pwp->dead_phylist_lock);
		mutex_destroy(&pwp->abort_comp_list_lock);
#ifdef	DEBUG
...
/*
 * Called with xp->statlock and PHY lock.
 */
static int
pmcs_add_sata_device(pmcs_hw_t *pwp, pmcs_xscsi_t *xp)
{
	ata_identify_t *ati;
	int result, i, scratch_hdl = -1;
	pmcs_phy_t *pptr;
	uint16_t *a;
	union {
		uint8_t nsa[8];
	...
	result = pmcs_sata_identify(pwp, pptr, scratch_hdl);
	...
	ati = pmcs_get_scratch_area(pwp, scratch_hdl);
	a = &ati->word108;
	for (i = 0; i < 4; i++) {
...
pmcs_worker(void *arg)
{
	...
	if (work_flags & PMCS_WORK_FLAG_BC_PROCESS) {
		pmcs_process_broadcast_event(pwp);
	}
	...
	if (work_flags & PMCS_WORK_FLAG_REGISTER_DEV) {
		pmcs_register_device_work(pwp);
	}

	if (work_flags & PMCS_WORK_FLAG_DEREGISTER_DEV) {
		pmcs_deregister_device_work(pwp);
	}
	...
	if (work_flags & PMCS_WORK_FLAG_ABORT_COMPLETION) {
		pmcs_abort_completion_work(pwp);
	}
	...
	if (work_flags & PMCS_WORK_FLAG_SATA_PHY_UP) {
		pmcs_sata_phy_up(pwp);
	}
...
pmcs_add_more_chunks(pmcs_hw_t *pwp, unsigned long nsize)
{
	...
	dc = NULL;
	for (dl = 0; dl < (nsize / PMCS_SGL_CHUNKSZ(pwp)); dl++) {
		pmcs_dmachunk_t *tmp;
	...
	pmcs_idma_chunks(pwp, dc, pchunk);
	pwp->nchunks++;
...
hot_reset:
	 ...
	 */
	rval = ddi_taskq_dispatch(pwp->tq, pmcs_hot_reset, pwp, DDI_NOSLEEP);

	/*
	 * Flush out the tgt queues & the completion queue,
	 * so that upper layers wont be waiting for I/O response from dead chip.
	 */
	mutex_enter(&pwp->lock);
	if (rval != DDI_SUCCESS) {
		pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL, "%s: "
		    "Failed ddi_taskq_dispatch to hot reset chip", __func__);
...
pmcs_watchdog(void *arg)
{
	...
	/* Check for a stall condition on the chip */
	...
	mutex_enter(&pwp->lock);
	/* Dispatch the worker if necessary */
	...
	/* Clean up commands and dead phys */
	pmcs_check_commands(pwp);
	pmcs_handle_dead_phys(pwp);

	/* Finally, set the watchdog to run again */
	mutex_enter(&pwp->lock);
	pwp->wdhandle = timeout(pmcs_watchdog, pwp,
	    drv_usectohz(PMCS_WATCH_INTERVAL));
	mutex_exit(&pwp->lock);
...
pmcs_setup_intr(pmcs_hw_t *pwp)
{
	...
	mutex_init(&pwp->ack_lock, NULL, MUTEX_DRIVER, DDI_INTR_PRI(pri));
	mutex_init(&pwp->dead_phylist_lock, NULL, MUTEX_DRIVER,
	    DDI_INTR_PRI(pri));
	mutex_init(&pwp->abort_comp_list_lock, NULL, MUTEX_DRIVER,
	    DDI_INTR_PRI(pri));
#ifdef	DEBUG
...
/*
 * Called with softstate lock and phy lock held.
 * Returns B_TRUE if removal is success, otherwise returns B_FALSE.
 */
boolean_t
pmcs_remove_device(pmcs_hw_t *pwp, pmcs_phy_t *pptr)
{
	...
	pmcs_xscsi_t *xp;
	static const uint8_t cleared[8] = {0};
	unsigned int vtgt;
	...
	for (vtgt = 0; vtgt < pwp->max_dev; vtgt++) {
		...
		mutex_enter(&xp->statlock);
		if ((xp->phy == pptr) && (pptr->target == xp)) {

			if (rval = pmcs_phy_target_match(pptr)) {
				/*
				 * This is the target on this PHY.  If the
				 * addresses match, go ahead with removal.
				 */
				if (xp->new) {
					xp->new = 0;
			...
			} else if (memcmp(pptr->sas_address, cleared, 8) == 0) {
				/*
				 * Since we enforce the PHY sas address match
				 * to the target's unit address above, we need
				 * to explicitly check for a zeroed PHY address.
				 */
				pmcs_clear_xp(pwp, xp);
				pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, pptr, xp,
				    "PHY sas_address is cleared, removing tgt "
				    "0x%p vtgt %u", (void *)xp, vtgt);
			}
 ...
 *
 * Return values >= 0 are "handles" that need to be passed in to
 * pmcs_release_scratch.  If the return value is < 0, scratch could not be
 * acquired.
 */
int
pmcs_acquire_scratch(pmcs_hw_t *pwp)
{
	volatile uint8_t *scratch_lock;
	int rval, idx;

	for (idx = 0; idx < pwp->nphy; idx++) {
		scratch_lock = &(pwp->scratch + idx)->scratch_lock;
		rval = atomic_swap_8(scratch_lock, 1);
		if (!rval) {
			(pwp->scratch + idx)->scratch_held_by = curthread;
			(pwp->scratch + idx)->scratch_lock_time = gethrtime();
			return (idx);
		}
	}

	/* No free scratch areas */
	return (-1);
}

void
pmcs_release_scratch(pmcs_hw_t *pwp, int handle)
{
	ASSERT((handle >= 0) && (handle < pwp->nphy));
	(pwp->scratch + handle)->scratch_lock = 0;
}

void *
pmcs_get_scratch_area(pmcs_hw_t *pwp, int handle)
{
	ASSERT((handle >= 0) && (handle < pwp->nphy));
	return ((pwp->scratch + handle)->scratch_addr);
}

uint64_t
pmcs_get_scratch_dma_area(pmcs_hw_t *pwp, int handle)
{
	ASSERT((handle >= 0) && (handle < pwp->nphy));
	return ((pwp->scratch + handle)->scratch_dma);
}

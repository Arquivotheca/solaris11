...
/*
 * Dump the register dumps and event logs in flash
 */
uint32_t
pmcs_load_register_dumps(pmcs_hw_t *pwp, caddr_t buf, uint32_t size_left,
    boolean_t raw_dump)
{
	uint8_t slice = 0;
	int n = 0;
	uint32_t bytes_written = 0;
	uint32_t req_dump_size;

	if (buf == NULL) {
		if (pwp->flash_regdumpp == NULL) {
			pwp->flash_regdumpp =
			    kmem_zalloc(PMCS_FLASH_REG_DUMP_SIZE, KM_NOSLEEP);
			if (pwp->flash_regdumpp == NULL) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL,
				    "%s: Could not allocate memory for flash "
				    "register dumps", __func__);
				return (bytes_written);
			}
		}

		buf = pwp->flash_regdumpp;
		size_left = PMCS_FLASH_REG_DUMP_SIZE;
	}

	/*
	 * Make sure we have enough room depending on whether this is a "raw"
	 * dump or not
	 */
	if (raw_dump) {
		req_dump_size = PMCS_FLASH_REG_DUMP_SIZE;
	} else {
		req_dump_size = PMCS_FLASH_REG_DUMP_SIZE * 2;
	}

	if (size_left < req_dump_size) {
		pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL,
		    "%s: Not enough buffer space for register dumps and event "
		    "logs", __func__);
		return (bytes_written);
	}

	slice = (GSM_FLASH_REG_DUMP_SIZE / PMCS_FLASH_CHUNK_SIZE);
...
pmcs_register_dump_int(pmcs_hw_t *pwp)
{
	...
	mutex_exit(&pwp->lock);
	n = pmcs_load_register_dumps(pwp, buf, size_left, B_FALSE);
	ASSERT(size_left >= n);
	buf += n; size_left -= n;

 ...
 * Local static data
 */
static int pmcs_tgtmap_stable_usec = MICROSEC;		/* 1 second */
static int pmcs_tgtmap_csync_usec = 10 * MICROSEC;	/* 10 seconds */
static int pmcs_tgtmap_error_usec = 10 * MICROSEC;	/* 10 seconds */
static int pmcs_tgtmap_error_thresh = 25;		/* 25% */
static int pmcs_tgtmap_error_iops_thresh = 100;		/* 100 IOPs avg */
...
pmcs_setup(pmcs_hw_t *pwp)
{
	...
	/*
	 * Enable fairness queue algorithm in the firmware
	 * and IOST consistency check which will trigger
	 * fatal intr before IBQ Stall occurs
	 */
	PMCS_SET_CUSTOM(pwp, (PMCS_IB_FAIRQ_BIT | PMCS_OB_FAIRQ_BIT |
	    PMCS_IOST_CHECK_BIT));
...
{
	uint32_t s1, s2, sfrbits, gsm, rapchk, wapchk, wdpchk, spc, tsmode;
	pmcs_phy_t *pptr;
	char *msg = NULL;
	int i;
	...
	/* If we are called by pmcs_hot_reset, wait for 10 seconds */
	if (pwp->state == STATE_IN_RESET) {
		if (pwp->locks_initted) {
			mutex_exit(&pwp->lock);
		}
		pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL, "%s: "
		    "Waiting for 10 seconds before proceeding...", __func__);
	...
	/*
	 * Step 0: Required only for soft reset due to fatal error
	 */
	if (pwp->fatal_error) {
		if (pwp->locks_initted) {
			mutex_exit(&pwp->lock);
		}
		/* We poll for a maximum of 8 seconds */
		for (i = 0; i < PMCS_SOFT_RST_POLL_TIME * 10; i++) {
			s1 = pmcs_rd_msgunit(pwp, PMCS_MSGU_SCRATCH1) &
			    PMCS_MSGU_AAP_CPU_SOFT_RST_RDY;
			s2 = pmcs_rd_msgunit(pwp, PMCS_MSGU_SCRATCH2) &
			    PMCS_IOP_CPU_SOFT_RST_RDY;
		...
		if ((s1 & s2) == 0) {
			...
			pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL, "%s: "
			    "PMCS_MSGU_CPU_SOFT_RESET_READY not ready. "
			    "Bailing out of soft reset.", __func__);
			return (-1);
		}
		pwp->fatal_error = 0;
	}
	 ...
	 */
	s2 = pmcs_rd_msgunit(pwp, PMCS_MSGU_SCRATCH2);
	if ((s2 & PMCS_IOP_HOST_SOFT_RST_RDY) == 0) {
		pmcs_wr_gsm_reg(pwp, RB6_ACCESS, RB6_NMI_SIGNATURE);
		pmcs_wr_gsm_reg(pwp, RB6_ACCESS, RB6_NMI_SIGNATURE);
		for (i = 0; i < 100; i++) {
			s2 = pmcs_rd_msgunit(pwp, PMCS_MSGU_SCRATCH2) &
			    PMCS_IOP_HOST_SOFT_RST_RDY;
			if (s2) {
				break;
			}
			drv_usecwait(10000);
		...
		if (s2 == 0) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL,
			    ...
			    "ready. Proceeding with soft reset.", __func__);
			pmcs_register_dump(pwp);
		}
	}
	 ...
	 */
	pmcs_flush_all_tgts_queues(pwp);

	/*
	 * Zero out the ports list and clear root phys.
	 */
 ...
 * restore the state back to pre-reset context.
 * softstate lock should not be held.
 */
void
pmcs_hot_reset(void *ss)
{
	pmcs_hw_t	*pwp = (pmcs_hw_t *)ss;
	pmcs_iport_t	*iport;

	ASSERT(!mutex_owned(&pwp->lock));

	/* Deactivate watchdog - softstate lock cannot be held */
	...
	if (pmcs_soft_reset(pwp, B_FALSE)) {
		...
		mutex_enter(&pwp->lock);
		pwp->state = STATE_DEAD;
		/* Flush all tgts' queues since chip will remain dead */
		pmcs_flush_all_tgts_queues(pwp);
		/*
		 * For any iports on this HBA,
		 * report empty target sets and phymaps to SCSA
		 */
		...
		ddi_fm_service_impact(pwp->dip, DDI_SERVICE_LOST);
		pmcs_prt(pwp, PMCS_PRT_ERR, NULL, NULL, "%s: soft reset failed",
		    __func__);
		return;
	}
	...
	/*
	 * For any iports on the HBA, teardown the phys
	 */
	rw_enter(&pwp->iports_lock, RW_READER);
	for (iport = list_head(&pwp->iports); iport != NULL;
	    iport = list_next(&pwp->iports, iport)) {
	...
	pmcs_prt(pwp, PMCS_PRT_DEBUG, NULL, NULL, "%s: success", __func__);
}
...
{
	scsi_tgtmap_params_t tgtmap_params;
	...
	tgtmap_params.csync_usec = pmcs_tgtmap_csync_usec;
	tgtmap_params.stable_usec = pmcs_tgtmap_stable_usec;
	tgtmap_params.error_usec = pmcs_tgtmap_error_usec;
	tgtmap_params.error_thresh = pmcs_tgtmap_error_thresh;
	tgtmap_params.error_iops_thresh = pmcs_tgtmap_error_iops_thresh;

	/* create target map */
	if (scsi_hba_tgtmap_create(iport->dip, SCSI_TM_FULLSET,
	    &tgtmap_params, (void *)iport, pmcs_tgtmap_activate_cb,
	    pmcs_tgtmap_deactivate_cb, &iport->iss_tgtmap) != DDI_SUCCESS) {
...
pmcs_phymap_activate(void *arg, char *ua, void **privp)
{
	...
	*privp = (void *)pwp;
	...
	mutex_enter(&pwp->lock);
	(void) pmcs_process_sysevent(pwp);
	mutex_exit(&pwp->lock);
}
...
pmcs_begin_observations(pmcs_hw_t *pwp)
{
	...
	for (iport = list_head(&pwp->iports); iport != NULL;
	    iport = list_next(&pwp->iports, iport)) {
		/*
		 * Skip this iport if it has no PHYs that are up or if it
		 * isn't marked as having changed.  pmcs_clear_iport_changed
		 * will return the current value.  This way, we can tell later
		 * whether the iport changed during discovery.
		 * Note we don't need to lock the iport for report_skip
		 * since it is only used during discovery, which is single-
		 * threaded.  For the "no PHYs up" case, the skip is so that
...
pmcs_flush_observations(pmcs_hw_t *pwp)
{
	...
	for (iport = list_head(&pwp->iports); iport != NULL;
	    iport = list_next(&pwp->iports, iport)) {
		/*
		 * Skip this iport if it has no PHYs up, or it was skipped
		 * during begin_observations.
		 */
		if (iport->report_skip ||
		    !sas_phymap_uahasphys(pwp->hss_phymap, iport->ua)) {
			continue;
		}

		/*
		 * Set the iport back to changed state so the observations that
		 * are being flushed now will be reported in a subsequent
		 * attempt.
		 */
		pmcs_set_iport_changed(iport);
		tgtmap = iport->iss_tgtmap;
		ASSERT(tgtmap);
...
pmcs_report_observations(pmcs_hw_t *pwp)
{
	...
	while (pptr) {
		 ...
		 * to report observations for this iport's targets.  If there
		 * is no iport, move on to the next PHY.
		 */
		iport = pptr->iport;
		pmcs_unlock_phy(pptr);
		if (iport == NULL) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, pptr, NULL,
			    "%s: no iport for PHY %d", __func__, pptr->phynum);
			pptr = pptr->sibling;
			continue;
		}
		pmcs_hold_iport(iport);
		...
		/*
		 * If this iport is marked to be skipped, just move on.
		 */
		if (iport->report_skip) {
			pmcs_rele_iport(iport);
			pptr = pptr->sibling;
		...
		/*
		 * If the PHY is marked changed, skip it.
		 */
		pmcs_lock_phy(pptr);
		if (pptr->changed) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, pptr, NULL,
			    "%s: PHY %s changed; restart discovery",
			    __func__, pptr->path);
			iport_status = B_FALSE;
			goto next_phy;
		}

		/*
		 * If the iport is marked as having changed, skip it.  We'll
		 * ultimately be returning FALSE, which will trigger discovery
		 * to run again.
		 */
		if (!iport->report_skip) {
			if (pmcs_get_iport_changed(iport) ||
			    pmcs_report_iport_observations(pwp, iport, pptr)
			    == B_FALSE) {
				iport_status = B_FALSE;
				goto next_phy;
			}
		...
		/*
		 * If the iport has changed, flush the observation, otherwise
		 * indicate the end of this report.
		 */
		if (pmcs_get_iport_changed(iport)) {
			iport_status = B_FALSE;
			goto next_phy;
		} else if (scsi_hba_tgtmap_set_end(tgtmap, 0) != DDI_SUCCESS) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG_MAP, NULL, NULL,
		...
		/*
		 * The observation report is complete. Set up properties for
		 * this iport.
		 */
...
next_phy:
		/*
		 * If this iport failed, set the overall status to return
		 * FALSE, and flush this observation
		 */
		if (iport_status == B_FALSE) {
			if (scsi_hba_tgtmap_set_flush(tgtmap) != DDI_SUCCESS) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG_MAP, NULL, NULL,
				    "%s: Failed set_flush on tgtmap 0x%p",
    ...
    pmcs_phy_t *phyp)
{
	...
	while (lphyp) {
		...
		if (scsi_hba_tgtmap_set_add(tgtmap, tgt_type, ua, NULL,
		    lphyp->phy_cc_delta) != DDI_SUCCESS) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG_MAP,  NULL, NULL,
...
pmcs_configure_new_devices(pmcs_hw_t *pwp, pmcs_phy_t *pptr)
{
	...
	while (pptr) {
		...
		if (iport == NULL) {
			pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, pptr, NULL,
			    "%s: PHY %s iport not yet configured, skipping",
			    __func__, pptr->path);
			pmcs_lock_phy(pptr);
			goto next_phy;
		}
...
pmcs_configure_expander(pmcs_hw_t *pwp, pmcs_phy_t *pptr, pmcs_iport_t *iport)
{
	...
	/*
	 * Step 1- If the PHY is a root PHY and is currently marked as having
	 * changed, reset the config_stop timer as well.
	 */
	...
	/*
	 * Set the new children pointer for this expander. Also,
	 * We can now clear this PHY's changed flag.
	 */
	pptr->children = clist;
	clist = NULL;
	pptr->ncphy = nphy;
...
pmcs_check_expander(pmcs_hw_t *pwp, pmcs_phy_t *pptr)
{
	...
	/*
	 * Figure out how many phys are in this expander. If
	...
	/*
	 * If the number of phys don't agree, kill the old sub-tree.
	 */
	if (nphy != pptr->ncphy) {
		pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, pptr, NULL,
	...
	/*
	 * If we're at the bottom of the stack, we're done
	 * (we can't have any levels below us). Clear the changed flag.
	 */
	if (pptr->level == PMCS_MAX_XPND-1) {
		pptr->changed = 0;
		return;
	...
	/*
	 * Discover things about each phy in this expander.  We do
	...
	/*
	 * Compare the local PHY's contents to our current PHY. If
	...
	while (ctmp) {
		...
		if (changed) {
			...
			if (local->dtype == NOTHING) {
				...
				ctmp->phy_cc_delta = local->phy_cc_delta;
			}

			/*
			 * Rather than copy the entire discovery response,
			 * just copy the important part(s). Currently, that
			 * is just the PHY change count
			 */
			ctmp->disc_resp.sdr_phy_change_count =
			    local->disc_resp.sdr_phy_change_count;
		}
...
pmcs_expander_get_nphy(pmcs_hw_t *pwp, pmcs_phy_t *pptr)
{
	...
	ASSERT(pwp->config_sh != -1);
	scratch = pmcs_get_scratch_area(pwp, pwp->config_sh);
	scratch_dma = pmcs_get_scratch_dma_area(pwp, pwp->config_sh);
...
again:
	...
	(void) memset(scratch, 0x77, PMCS_SCRATCH_SIZE);
	pwrk->arg = scratch;
	pwrk->dtype = pptr->dtype;
	pwrk->xp = pptr->target;
	...
	srf = (smp_response_frame_t *)&((uint32_t *)scratch)[rdoff >> 2];
	srgr = (smp_report_general_resp_t *)
	    &((uint32_t *)scratch)[(rdoff >> 2)+1];
	...
	if (status != PMCOUT_STATUS_OK) {
		...
		/*
		 * For the NON_OPERATIONAL/RECOVERY cases, we need to kick off
		...
		case PMCOUT_STATUS_IO_DS_NON_OPERATIONAL: {
			...
			pmcs_prt(pwp, PMCS_PRT_DEBUG, pptr, xp,
			    "%s: expander %s device state %s",
			    __func__, pptr->path, pmcs_status_str(status));
    ...
    pmcs_phy_t *pptr)
{
	...
	const uint_t rdoff = 0x100;	/* returned data offset */
	uint8_t *roff, phy_chg_delta;
	uint32_t status, *ptr, msg[PMCS_MSG_SIZE], htag;
	int result = 0;
	uint8_t	ini_support;
	uint8_t	tgt_support;
	void *scratch = pmcs_get_scratch_area(pwp, pwp->config_sh);
	uint64_t scratch_dma = pmcs_get_scratch_dma_area(pwp, pwp->config_sh);
	...
	/*
	 * Determine the PHY change count delta (jitter) and save it.
	 */
	if (sdr->sdr_phy_change_count > pptr->disc_resp.sdr_phy_change_count) {
		phy_chg_delta = sdr->sdr_phy_change_count -
		    pptr->disc_resp.sdr_phy_change_count;
	} else {
		phy_chg_delta = 256 - pptr->disc_resp.sdr_phy_change_count -
		    sdr->sdr_phy_change_count;
	}
	if (pptr->pend_dtype != NEW) {
		pptr->phy_cc_delta = phy_chg_delta;
	}
    ...
    int wait)
{
	...
	default:
		if ((tag) &&
		    ((tag_pwrk = pmcs_tag2wp(pwp, tag, B_FALSE)) != NULL)) {
			switch (tag_pwrk->dtype) {
			case SAS:
				abt_type = PMCIN_SSP_ABORT;
				mutex_exit(&tag_pwrk->lock);
				break;
			case SATA:
				abt_type = PMCIN_SATA_ABORT;
				mutex_exit(&tag_pwrk->lock);
			...
			default:
				mutex_exit(&tag_pwrk->lock);
				pmcs_prt(pwp, PMCS_PRT_DEBUG, pptr, NULL,
				    "%s: phy and pwrk dtype unavailable. "
				    "Not aborting (htag 0x%x) for phy(%s)",
				    __func__, tag, pptr->path);
			return (0);

			}
		} else {
			pmcs_prt(pwp, PMCS_PRT_DEBUG, pptr, NULL,
			    "%s: phy dtype unavailable. "
			    "Not aborting all cmds for phy(%s))", __func__,
			    pptr->path);
			return (0);
		}
	}
    ...
    uint64_t lun, uint32_t *response)
{
	...
	/*
	 * 20secs is long. Check if the phy got freed while we were waiting
	 * xp's phy gets cleared when phy is freed (during pmcs_soft_reset)
	 * Return code ENXIO indicates this situation to callers.
	 */
	if (pwrk->xp && (pwrk->xp->phy != pwrk->phy)) {
		pmcs_pwork(pwp, pwrk);
		return (ENXIO);
	}
	pmcs_pwork(pwp, pwrk);
...
pmcs_sata_abort_ncq(pmcs_hw_t *pwp, pmcs_phy_t *pptr)
{
	...
	if (scratch_hdl == -1) {
		pmcs_prt(pwp, PMCS_PRT_DEBUG, pptr, pptr->target,
		    "%s: Unable to allocate scratch", __func__);
		return (ENOMEM);
	}
	fp = pmcs_get_scratch_area(pwp, scratch_hdl);
	...
	msg[10] = 0;
	msg[11] = 0;
	msg[12] = LE_32(DWORD0(scratch_dma));
	msg[13] = LE_32(DWORD1(scratch_dma));
	msg[14] = LE_32(512);
	msg[15] = 0;
...
/*
 * Process & Log sysevents for root level phys of the HBA.
 * Called with softstate lock held.
 */
void
pmcs_process_sysevent(pmcs_hw_t *pwp)
{
	pmcs_phy_t	*pptr;
	uint16_t	etype;
	...
	for (pptr = pwp->root_phys; pptr; pptr = pptr->sibling) {
		...
		if (etype & PMCS_SAS_PHY_ONLINE) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_PHY_EVENT,
			    SAS_PHY_ONLINE, pptr);
		}
		if (etype & PMCS_SAS_PHY_OFFLINE) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_PHY_EVENT,
			    SAS_PHY_OFFLINE, pptr);
		}
		if (etype & PMCS_SAS_PHY_REMOVE) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_PHY_EVENT,
			    SAS_PHY_REMOVE, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_CHANGE) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_CHANGE, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_SES) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_SES, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_D24_0) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_D24_0, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_D27_4) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_D27_4, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_D01_4) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_D01_4, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_D04_7) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_D04_7, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_D16_7) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_D16_7, pptr);
		}
		if (etype & PMCS_SAS_PORT_BROADCAST_D29_7) {
			pmcs_smhba_log_sysevent(pwp, ESC_SAS_HBA_PORT_BROADCAST,
			    SAS_PORT_BROADCAST_D29_7, pptr);
		}
		/* Clear all events that were processed */
		pptr->sysevent = 0;
skip:
...
pmcs_kill_devices(pmcs_hw_t *pwp, pmcs_phy_t *phyp)
{
	...
	while (phyp) {
		...
		ret = B_TRUE;
		if (remove_device && (ret = pmcs_remove_device(pwp, phyp))) {
			mutex_exit(&pwp->lock);
			...
			mutex_exit(&pwp->lock);
			if (ret == B_FALSE) {
				/* phy/target ptr or sas_address mismatch */
 ...
 * This is only needed for root PHYs.
 */
void
pmcs_ack_events(pmcs_hw_t *pwp)
{
	uint32_t msg[PMCS_MSG_SIZE], ack_info = 0, *ptr;
	struct pmcwork *pwrk;
	int idx;

	for (idx = 0; idx < pwp->nphy; idx++) {
		mutex_enter(&pwp->ack_lock);
		if (!PMCS_NEED_ACK(pwp, idx)) {
			mutex_exit(&pwp->ack_lock);
			continue;
		}
		ack_info = pwp->ack_info[idx];
		ASSERT(ack_info != 0);
		PMCS_CLEAR_ACK(pwp, idx);
		pwp->ack_info[idx] = 0;
		mutex_exit(&pwp->ack_lock);
		...
		msg[1] = LE_32(pwrk->htag);
		msg[2] = LE_32(ack_info);

		pwrk->dtype = SAS;   /* Doesn't really matter */
		mutex_exit(&pwrk->lock);
...
/*
 * Handle brocast event processing in worker thread context
 */
void
pmcs_process_broadcast_event(pmcs_hw_t *pwp)
{
	pmcs_phy_t *pptr;
	...
	while (pptr) {
		pmcs_lock_phy(pptr);
		if (PMCS_NEED_BC_PROCESS(pwp, pptr->phynum)) {
			pmcs_set_changed(pwp, pptr, B_TRUE, 0);
			PMCS_CLEAR_BC_PROCESS(pwp, pptr->phynum);
			pptr->sysevent |= PMCS_SAS_PORT_BROADCAST_CHANGE;
			handled = B_TRUE;
		}
		pmcs_unlock_phy(pptr);
		pptr = pptr->sibling;
...
pmcs_dma_load(pmcs_hw_t *pwp, pmcs_cmd_t *sp, uint32_t *msg)
{
	...
	/*
	 * Otherwise, we'll need an external S/G list.
	 */
	...
	sp->cmd_clist = tc;
	sgl = tc->chunks;
	(void) memset(tc->chunks, 0, PMCS_SGL_CHUNKSZ(pwp));
	sgl_addr = tc->addr;
	...
	for (seg = 0; seg < CMD2PKT(sp)->pkt_numcookies; seg++) {
		sgl[seg].sglal = LE_32(DWORD0(sg->dmac_laddress));
		sgl[seg].sglah = LE_32(DWORD1(sg->dmac_laddress));
		sgl[seg].sglen = LE_32(sg->dmac_size);
		sgl[seg].flags = 0;
		sg++;
	}
	return (0);
 ...
 */
void
pmcs_idma_chunks(pmcs_hw_t *pwp, pmcs_dmachunk_t *dcp, pmcs_chunk_t *pchunk)
{
	unsigned long off, n;
	pmcs_dmachunk_t *np = dcp, *nptail;
	pmcs_chunk_t *tmp_chunk;
	uint32_t chunk_size = PMCS_SGL_CHUNKSZ(pwp);
...
pmcs_flush_target_queues(pmcs_hw_t *pwp, pmcs_xscsi_t *tgt, uint8_t queues)
{
	...
	if (queues & PMCS_TGT_ACTIVE_QUEUE) {
		...
		while (sp) {
			...
			CMD2PKT(sp)->pkt_reason = CMD_DEV_GONE;
			CMD2PKT(sp)->pkt_state = STATE_GOT_BUS;
			pmcs_complete_work_impl(pwp, pwrk, NULL, 0, B_TRUE);
			pmcs_dma_unload(pwp, sp);
			mutex_enter(&pwp->cq_lock);
			STAILQ_INSERT_TAIL(&pwp->cq, sp, cmd_next);
			mutex_exit(&pwp->cq_lock);
...
/*
 * Flush all targets' queues. Called with softstate lock held.
 */
void
pmcs_flush_all_tgts_queues(pmcs_hw_t *pwp)
{
	int i = 0;
	pmcs_phy_t *phy;
	pmcs_xscsi_t *xp;
	...
	if (pwp->targets) {
		for (i = 0; i < pwp->max_dev; i++) {
			...
			pmcs_flush_target_queues(pwp, xp, PMCS_TGT_ALL_QUEUES);
			/*
			 * Clear out the tgt's phy ptr except while going
			 * through pmcs_soft_reset because we will reap the
			 * dead phy right away.
			 */
			if (pwp->state != STATE_IN_RESET) {
				xp->phy = NULL;
	...
	for (i = 0; i < pwp->max_cmd; i++) {
		...
		if (p->htag & PMCS_TAG_NONIO_CMD) {
			...
			pmcs_prt(pwp, PMCS_PRT_DEBUG, p->phy, p->xp,
			    ...
			    __func__, p->htag);
			pmcs_complete_work_impl(pwp, p, NULL, 0, B_TRUE);
		} else {
			mutex_exit(&p->lock);
		}
	}
...
pmcs_complete_work_impl(pmcs_hw_t *pwp, pmcwork_t *pwrk, uint32_t *iomb,
    size_t amt, boolean_t phy_locked)
{
...
#endif
		pptr = pwrk->phy;
		abt_htag = pwrk->abt_htag;
		pmcs_pwork(pwp, pwrk);

		/* If this was an abort all, clean up if needed */
		if ((abt_htag == PMCS_ABT_HTAG_ALL) && (pptr != NULL)) {
			if (!phy_locked)
				mutex_enter(&pptr->phy_lock);
			if (pptr->abort_all_start) {
				pptr->abort_all_start = 0;
...
pmcs_reap_dead_phy(pmcs_phy_t *phyp)
{
	...
	while (ctmp) {
		...
		if (((iport_cmp) && (iport_cmp != phyp->iport)) ||
		    (memcmp((void *)&ctmp->sas_address[0],
		    (void *)&phyp->sas_address[0], 8) != 0)) {
			ctmp = ctmp->dead_next;
			continue;
		}
...
pmcs_phy_target_match(pmcs_phy_t *phyp)
{
	...
	if (memcmp((void *)unit_address, (void *)phyp->target->unit_address,
		...
		pmcs_prt(phyp->pwp, PMCS_PRT_DEBUG, phyp, phyp->target, "%s: "
		    "mismatch: phy sas_address (%s),  tgt "
		    "unit_address (%s)", __func__, unit_address,
		    phyp->target->unit_address);
	}
...
/*
 * It's possible when a root PHY gets promoted for the PHY going down to have
 * already been deregistered.  If that happens, the newly promoted PHY will not
 * be registered, thus nothing behind it will ever be discovered.
 */
void
pmcs_register_device_work(pmcs_hw_t *pwp)
{
	pmcs_phy_t	*pptr;
	...
	for (pptr = pwp->root_phys; pptr; pptr = pptr->sibling) {
		pmcs_lock_phy(pptr);
		/*
		 * If the PHY is primary, is not dead, and is not configured,
		 * it needs to be registered.
		 */
		if (!pptr->subsidiary && !pptr->dead && !pptr->configured) {
			if (pmcs_get_device_handle(pwp, pptr)) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG_CONFIG, pptr,
				    NULL, "%s: Failed to register %s",
				    __func__, pptr->path);
			}
		}
		pmcs_unlock_phy(pptr);
...
void
pmcs_deregister_device_work(pmcs_hw_t *pwp)
{
	pmcs_phy_t	*pptr;
    ...
    scsi_tgtmap_deact_rsn_t tgt_deact_rsn)
{
	...
	/*
	 * If the reason for deactivation was instability (detected by SCSA),
	 * then do not re-run discovery.
	 */
	if (tgt_deact_rsn == SCSI_TGT_DEACT_RSN_UNSTBL) {
		rediscover = B_FALSE;
	}
...
pmcs_add_dead_phys(pmcs_hw_t *pwp, pmcs_phy_t *phyp)
{
	...
	while (phyp) {
		...
		/*
		 * Make sure that phy's tgt is cleared (& queues are flushed)
		 */
		if (phyp->target) {
			mutex_enter(&phyp->target->statlock);
			if (!STAILQ_EMPTY(&phyp->target->wq)) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG, phyp,
				    phyp->target, "%s: Clearing non-empty "
				    "WAIT queue for tgt 0x%p ", __func__,
				    (void *)phyp->target);
			}
			if (!STAILQ_EMPTY(&phyp->target->aq)) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG, phyp,
				    phyp->target, "%s: Clearing non-empty "
				    "ACTIVE queue for tgt 0x%p ", __func__,
				    (void *)phyp->target);
			}
			if (!STAILQ_EMPTY(&phyp->target->sq)) {
				pmcs_prt(pwp, PMCS_PRT_DEBUG, phyp,
				    phyp->target, "%s: Clearing non-empty "
				    "SPECIAL queue for tgt 0x%p ", __func__,
				    (void *)phyp->target);
			}
			pmcs_clear_xp(pwp, phyp->target);
			mutex_exit(&phyp->target->statlock);
		}
...
boolean_t
pmcs_get_iport_changed(pmcs_iport_t *iport)
{
	boolean_t changed;

	ASSERT(iport);
	mutex_enter(&iport->iport_changed_lock);
	changed = iport->iport_changed;
	mutex_exit(&iport->iport_changed_lock);
	return (changed);
}

void
pmcs_set_iport_changed(pmcs_iport_t *iport)
{
	ASSERT(iport);
	mutex_enter(&iport->iport_changed_lock);
	iport->iport_changed = B_TRUE;
	mutex_exit(&iport->iport_changed_lock);
}

/*
 * pmcs_clear_iport_changed returns the current value of iport_changed prior
 * to clearing it.  This ensures that the caller knows the current state if
 * need be.
 */
boolean_t
pmcs_clear_iport_changed(pmcs_iport_t *iport)
{
	boolean_t changed;

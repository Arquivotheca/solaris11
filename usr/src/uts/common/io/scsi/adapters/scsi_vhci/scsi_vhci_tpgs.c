...
retry:
	ap = &sd->sd_address;
	if (retval = vhci_tpgs_inquiry(ap, bp, mode)) {
		VHCI_DEBUG(1, (CE_NOTE, NULL, "!vhci_tpgs_get_target_fo_mode: "
		    " failed vhci_tpgs_inquiry\n"));

		if (retval == EAGAIN) {
			if (retry_cnt++ >= STD_FO_MAX_RETRIES) {
				(void) ddi_pathname(
				    ap->a_hba_tran->tran_hba_dip, phci_path);
				cmn_err(CE_WARN,
				    "vhci_tpgs_get_target_fo_mode:"
				    "inquiry timeout on %s %s%d\n",
				    phci_path, ddi_driver_name(sd->sd_dev),
				    ddi_get_instance(sd->sd_dev));
				retval = 1;
				goto done;
			} else {
	...
	if (vhci_tpgs_page83(ap, bp, &rel_tgt_port, &tgt_port, &lu)) {
		VHCI_DEBUG(1, (CE_NOTE, NULL, "!vhci_tpgs_get_target_fo_mode: "
...
try_again:
	...
	while (i++ < 6) {
		/* try up to 6 times */
		if (vhci_do_scsi_cmd(pkt) == 0) {
			/* pkt transport fails, try again with delay 1s */
			VHCI_DEBUG(1, (CE_WARN, NULL,
			    "!vhci_tpgs_report_target_groups: vhci_do_scsi_cmd"
			    " failed pkt=0x%p,reason=0x%x,retry=%d",
			    (void *)pkt, pkt->pkt_reason, i));
	...
	if (i > 0) {
		/* pkt transport fails all 6 times, so return failure */
		VHCI_DEBUG(1, (CE_WARN, NULL, "!vhci_tpgs_report_target_groups:"
		    " vhci_do_scsi_cmd failed\n"));
		kmem_free((void *)bufp, len);
		scsi_destroy_pkt(pkt);

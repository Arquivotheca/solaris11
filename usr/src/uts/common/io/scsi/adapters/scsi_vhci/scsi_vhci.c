...
/*
 * Export vhci_debug to support debug plugin modules with non-debug
 * scsi_vhci core.
 */
...
/*
 * global mutex for synchronizing scsi_vhci_lun allocation/deallocation
 */
static kmutex_t		vhci_svl_mutex;
...
static struct scsi_vhci_lun *vhci_lun_lookup_hold(dev_info_t *);
static struct scsi_vhci_lun *vhci_lun_lookup_alloc_hold(dev_info_t *,
    char *, int *);
static void vhci_lun_rele(struct scsi_vhci_lun *dvlp, struct scsi_device *sd);
...
static int vhci_send_release(dev_info_t *, mdi_pathinfo_t *);
...
_init(void)
{
	...
	mutex_init(&vhci_svl_mutex, NULL, MUTEX_DRIVER, NULL);

	if ((rval = mod_install(&modlinkage)) != 0) {
		VHCI_DEBUG(1, (CE_NOTE, NULL, "!_init: mod_install failed\n"));
		mutex_destroy(&vhci_svl_mutex);
...
vhci_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 *
	 * Regarding dma attributes: We start with scsi_alloc_attr, but
	 * scsi_vhci's tran_dma_attr gets updated with the
	 * ddi_dma_attr_merge() result of all pHCIs as pHCIs
	 * scsi_hba_attach_setup()/scsi_hba_detach() calls
	 * mdi_devi_set_dma_attr()->ddi_dma_attr_merge().
	 */
	...
	scsi_hba_tran_t *hba_tran, struct scsi_device *sd)
{
	...
	vlun = vhci_lun_lookup_hold(tgt_dip);
	 ...
	 * XXX is that possible that we have vlun allocated but no paths
	 * available?
	 */
	while ((vlun == NULL) || (mdi_client_get_path_count(tgt_dip) == 0) ||
		...
		/*
		 * Decrease the vlun reference count first.
		 */
		vhci_lun_rele(vlun, NULL);
...
vhci_scsi_start(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	...
	if ((pkt->pkt_cdbp[0] == SCMD_RESERVE) ||
		 ...
		 * of this pkt set the VLUN_QUIESCED_FLG and wait till any
		 * pending IO is completed before proceeding.
	...
	if ((vpkt->vpkt_flags & CFLAG_DMA_PARTIAL) == 0) {
		...
		rval = vhci_bind_transport(ap, vpkt,
		    vpkt->vpkt_tgt_init_pkt_flags, NULL_FUNC);
		if (rval != TRAN_ACCEPT) {
			...
			/*
			 * In the case of TRAN_FATAL_ERROR, if the client
			 * is failed, then TRAN_ACCEPT the packet and return
			 * it with reason CMD_DEV_GONE.
			 */
			if ((rval == TRAN_FATAL_ERROR) &&
			    mdi_client_is_failed(cdip)) {
...
pkt_cleanup:
	...
	if ((resrv_instance != 0) && (resrv_instance !=
		...
		if ((rval == MDI_SUCCESS) && (pip != NULL)) {
			if (vhci_send_release(cdip, pip) == 0) {
				if (restore_lbp)
					(void) mdi_set_lb_policy(cdip, lbp);
	...
	if (scsi_pkt_allocated_correctly(vpkt->vpkt_hba_pkt)) {
		...
		/*
		 * Pass in pkt_path_instance in case someone may need this
		 * before vhci_intr return.
		 */
		vpkt->vpkt_tgt_pkt->pkt_path_instance =
		    vpkt->vpkt_hba_pkt->pkt_path_instance;
	}
    ...
    int (*func)(caddr_t))
{
	...
	time_t			tnow;
	int			path_instance = 0;
	int			least_desired_pi = 0;
	int			no_online_path = 0;
	...
	/*
	 * Get path_instance or the least desired path_instance depending on
	 * the pkt_flags specified by upper layer drivers.
	 *
	 * Non-zero path_instance with FLAG_PKT_PATH_INSTANCE set indicates
	 * that mdi_select_path should be called to select a specific path,
	 *
	 * Non-zero path_instance with FLAG_PKT_PATH_INSTANCE_AVOID_LD
	 * set indicates mdi_select_path should try to avoid a specific path
	 * (i.e. the specified path is least_desired).
	 *
	...
	if (scsi_pkt_allocated_correctly(vpkt->vpkt_tgt_pkt)) {
		...
		if (vpkt->vpkt_tgt_pkt->pkt_flags &
...
try_again:
		/*
		 * FLAG_PKT_PATH_INSTANCE takes higher precedence than
		 * FLAG_PKT_PATH_INSTANCE_AVOID_LD.
		 */
		if (path_instance) {
		...
		} else if (least_desired_pi) {
			VHCI_DEBUG(4, (CE_NOTE, NULL, "!least_desired_pi=%d "
			    "pkt=%p cdb=0x%x b_blkno=0x%llx\n",
			    least_desired_pi,
			    (void *)vpkt->vpkt_tgt_pkt,
			    vpkt->vpkt_tgt_pkt->pkt_cdbp[0],
			    (unsigned long long)(vpkt->vpkt_tgt_init_bp ?
			    vpkt->vpkt_tgt_init_bp->b_blkno :
			    0xffffffffffffffff)));

			rval = mdi_select_path(cdip, vpkt->vpkt_tgt_init_bp,
			    MDI_SELECT_PATH_INSTANCE_AVOID_LD,
			    (void *)(intptr_t)least_desired_pi, &pip);
		...
		if (rval == MDI_BUSY) {
			...
			/*
			 * First we will try to select an ONLINE path to
			 * attach. If no ONLINE path available,
			 * svl_waiting_for_activepath will be set to 1.
			 */
			rval = mdi_select_path(cdip, vpkt->vpkt_tgt_init_bp,
			    mps_flag, NULL, &pip);
			if ((rval != MDI_SUCCESS) || (pip == NULL)) {
			...
			/*
			 * If least_desired_pi is set, we will skip over it
			 * to the next ONLINE/STANDBY path for attaching,
			 * otherwise we will continue to use the ONLINE path
			 * just been selected above.
			 */
			if (least_desired_pi) {
				rval = mdi_select_path(cdip,
				    vpkt->vpkt_tgt_init_bp,
				    MDI_SELECT_PATH_INSTANCE,
				    (void *)(intptr_t)least_desired_pi, &tpip);
				if (rval != MDI_SUCCESS) {
					tpip = NULL;
				}
				...
				if (pip != NULL) {
					/*
					 * Pip is previousely used for ONLINE
					 * path existence check, and should be
					 * released before next path selection.
					 */
					mdi_rele_path(pip);
					pip = NULL;
				}
...
vhci_update_pathstates(void *arg)
{
	...
	sps_flags = (MDI_SELECT_ONLINE_PATH |
	    MDI_SELECT_STANDBY_PATH |
	    MDI_SELECT_USER_DISABLE_PATH |
	    MDI_SELECT_NO_PREFERRED);

	sps = mdi_select_path(dip, NULL, sps_flags, NULL, &npip);
	if ((npip == NULL) || (sps != MDI_SUCCESS)) {
		goto done;
	}

	/*
	 * Send a release to the path which was previously reserved.  It
	 * prevents a possible race condition during re-reservation process.
	 * Also, there are some devices which keep reservations on access state
	 * change and we want to avoid sending unnecessary recovery resets to
	 * them.
	 */
	if (vlun->svl_flags & VLUN_RESERVE_ACTIVE_FLG) {
		(void) vhci_send_release(dip, vlun->svl_resrv_pip);
	}
	...
	do {
		...
		if (fo->sfo_path_get_opinfo(psd, &opinfo,
			...
			sps = mdi_select_path(dip, NULL, sps_flags, pip, &npip);
			mdi_rele_path(pip);
			continue;
		}
		...
		if ((opinfo.opinfo_path_state == SCSI_PATH_ACTIVE ||
			...
			if (!(MDI_PI_IS_ONLINE(pip))) {
				...
				vhci_log(CE_NOTE, ddi_get_parent(dip), "!%s "
				    "(%s%d): path %s is now ONLINE "
				    "because a failover occured",
			...
			/*
			 * We expect that there should be no reserved path
			 * belonging to the vlun after path states have changed.
			 * Let's ping each path to confirm no reservation has
			 * been preserved.
			 */
			if (vlun->svl_flags & VLUN_RESERVE_ACTIVE_FLG) {
				bp = scsi_alloc_consistent_buf(
				    &svp->svp_psd->sd_address, (struct buf *)
				    NULL, DEV_BSIZE, B_READ, NULL, NULL);
				if (!bp) {
					VHCI_DEBUG(1, (CE_NOTE, NULL,
					    "!vhci_update_pathstates: No "
					    "resources (buf)\n"));
					mdi_rele_path(pip);
					goto done;
				}
				...
				if (pkt) {
					(void) scsi_setup_cdb((union scsi_cdb *)
					    (uintptr_t)pkt->pkt_cdbp, SCMD_READ,
					    1, 1, 0);
					pkt->pkt_time = 3*30;
	...
	if (vlun->svl_flags & VLUN_KEEP_HELD) {
		vlun->svl_flags &= ~VLUN_KEEP_HELD;
	} else {
		VHCI_RELEASE_LUN(vlun);
	}
}
...
vhci_pathinfo_init(dev_info_t *vdip, mdi_pathinfo_t *pip, int flags)
{
	...
	vlun = vhci_lun_lookup_alloc_hold(tgt_dip, guid, &vlun_alloced);
	ddi_prop_free(guid);
	...
	scsi_device_damap_nxpe_bind(psd);
	svp->svp_new_path = 1;
...
failure:
	...
	/*
	 * Release the referencing of scsi_vhci_lun.
	 */
	vhci_lun_rele(vlun, NULL);
...
vhci_pathinfo_uninit(dev_info_t *vdip, mdi_pathinfo_t *pip, int flags)
{
	...
	scsi_device_damap_nxpe_unbind(psd);
	mutex_destroy(&psd->sd_mutex);
	...
	/*
	 * release referencing of the scsi_vhci_lun
	 */
	vhci_lun_rele(svp->svp_svl, NULL);
...
vhci_pathinfo_online(dev_info_t *vdip, mdi_pathinfo_t *pip, int flags)
{
	...
	mpapi_lu_data_t			*ld = NULL;
	char				*update_path_on_reset;
	...
	vlun = vhci_lun_lookup_hold(cdip);
	ASSERT(vlun != NULL);
	...
	/*
	 * If the target does not support UNIT ATTENTION queuing, then
	 * the tunable scsi-vhci-update-pathstate-on-reset
	 * needs to be set accordingly in scsi_vhci.conf so that scsi_vhci
	 * would update path states on any UA reset condition.
	 *
	 ...
	 * By default it is "no".
	 */
	update_path_on_reset = scsi_get_device_type_string(
	    "scsi-vhci-update-pathstate-on-reset", vdip, psd);
	if (update_path_on_reset != NULL) {
		if (strcmp(update_path_on_reset, "yes") == 0)
			vlun->svl_flags |= VLUN_UPDATE_PATH_ON_RESET;
		kmem_free(update_path_on_reset,
		    strlen(update_path_on_reset) +1);
	}
	...
	if (svp->svp_new_path) {
		...
		if ((vlun->svl_flags & VLUN_RESERVE_ACTIVE_FLG) == 0) {
			 ...
			 * host reboot.
			 */
			if (vhci_send_release(vlun->svl_dip, pip) == 0) {
				rval = MDI_FAILURE;
				goto failure;
			}
		}
	...
	if (list_ptr != NULL) {
		...
		if (ld != NULL) {
			bcopy(psd->sd_inq->inq_vid,
			    ld->prop.prodInfo.vendor, 8);
			bcopy(psd->sd_inq->inq_pid,
			    ld->prop.prodInfo.product, 16);
			bcopy(psd->sd_inq->inq_revision,
			    ld->prop.prodInfo.revision, 4);
		} else {
		...
		cmn_err(CE_WARN,
		    "vhci_pathinfo_online: list_ptr is NULL "
		    "pip:%p (GUID %s)",
		    (void *)pip, svp->svp_svl->svl_lun_wwn);
		rval = MDI_FAILURE;
		goto failure;
	}
...
check_path_again:
		...
		if (activation_done) {
			/*
			 * confirm the path state again by "ping"-ing it.
			 */
		...
		if (sfo->sfo_path_activate(svp->svp_psd, pclass2,
		    vlun->svl_fops_ctpriv) == 0) {
			/*
			 * For devices using tpgs, we need a double check since
			 * READ could be done on STANDBY paths for certain
			 * devices.
			 */
ping_path_again:
			if (sfo->sfo_path_ping(svp->svp_psd,
			    vlun->svl_fops_ctpriv) == 1) {
				/*
				 * We're good, this is an ACTIVE path.
				 */
				activation_done = 1;
				mdi_rele_path(npip);
				VHCI_DEBUG(1, (CE_NOTE, NULL,
				...
				/*
				 * This is not an ACTIVE path, let's ping again.
				 */
				if (ping_count++ < 3) {
					VHCI_DEBUG(1, (CE_NOTE, NULL,
					    "!vhci_failover(7.1)(%s): "
					    "sfo_path_ping on path 0x%p\n",
					    guid, (void *)npip));
					drv_usecwait(1000000);
					goto ping_path_again;
				} else {
					/*
					 * We're done with retrying, need to
					 * check the next path.
					 */
					cmn_err(CE_WARN,
					    "!vhci_failover(7.2)(%s) failed on "
					    "path 0x%p\n", guid, (void *)npip);
					activation_done = 0;
					ping_count = 0;
				}
	 ...
	 * pathclass pclass2; let us update all path states, this may
	 * re-assign the pathclass for all paths.
	 */
	vlun->svl_flags |= (VLUN_UPDATE_TPG | VLUN_KEEP_HELD);
	vhci_update_pathstates((void *)vlun);

	/*
	 * verify if there is an active path again.
	 */
	npip = NULL;
	sps = mdi_select_path(cdip, NULL, MDI_SELECT_ONLINE_PATH, NULL, &npip);
	if (sps != MDI_SUCCESS) {
		vhci_log(CE_NOTE, NULL, "!Failover operation failed for "
		    "device %s (GUID %s): no path is reported as active.\n",
		    ddi_node_name(cdip), guid);
		goto done;
	} else if (npip != NULL) {
		mdi_rele_path(npip);
...
/*
 * If the scsi_vhci_lun exists, just increase the reference count.
 */
static struct scsi_vhci_lun *
vhci_lun_lookup_hold(dev_info_t *tgt_dip)
{
	struct scsi_vhci_lun *svl = NULL;
	mutex_enter(&vhci_svl_mutex);
	if (svl = (struct scsi_vhci_lun *)
	    mdi_client_get_vhci_private(tgt_dip)) {
		svl->svl_ref_cnt++;
	}
	mutex_exit(&vhci_svl_mutex);
	return (svl);
}

/*
 * The life cycle of scsi_vhci_lun is the union of:
 * - the life cycle of all the paths
 * - the life cycle of the devinfo node of mdi_client
 * We're using reference couting to address this life cycle issue.
 */
static struct scsi_vhci_lun *
vhci_lun_lookup_alloc_hold(dev_info_t *tgt_dip, char *guid, int *didalloc)
{
	struct scsi_vhci_lun *svl = NULL;
	struct scsi_vhci_lun *svl_mdi = NULL;

	/*
	 * Firstly, let's check whether the scsi_vhci_lun already allocated.
	 */
	if (svl = vhci_lun_lookup_hold(tgt_dip)) {
		return (svl);
	}

	/*
	 * Let's allocate a new scsi_vhci_lun before aquiring the big lock
	 * (vhci_svl_mutex).
	 */
	...
	/*
	 * Double check whether the scsi_vhci_lun already allocated during
	 * this window, if not, just use the one we created above.
	 */
	mutex_enter(&vhci_svl_mutex);
	if (svl_mdi = (struct scsi_vhci_lun *)
	    mdi_client_get_vhci_private(tgt_dip)) {
		svl_mdi->svl_ref_cnt++;
	} else {
	...
	if (svl_mdi) {
		/*
		 * The scsi_vhci_lun was already allocated by another thread,
		 * we need to free the one we just created.
		 */
		sema_destroy(&svl->svl_pgr_sema);
		cv_destroy(&svl->svl_cv);
		mutex_destroy(&svl->svl_mutex);
		kmem_free(svl->svl_lun_wwn, strlen(guid) + 1);
		kmem_free(svl, sizeof (*svl));
		svl = NULL;
	}
	VHCI_DEBUG(1, (CE_NOTE, NULL,
	    "vhci_lun_lookup_alloc_hold: guid %s vlun 0x%p alloced %d\n",
	    guid, (void *)svl, *didalloc));
	return (svl_mdi ? svl_mdi : svl);
}

/*
 * This function should be called when:
 * - a path is being uninitialized
 * - the devinfo node of the mdi_client is being uninit-ing.
 */
static void
vhci_lun_rele(struct scsi_vhci_lun *dvlp, struct scsi_device *sd)
{
	char *guid;
	...
	ASSERT(guid != NULL);
	VHCI_DEBUG(4, (CE_NOTE, NULL, "!vhci_lun_rele: %s\n", guid));

	/*
	 * Check whether there are other owners.
	 */
	mutex_enter(&vhci_svl_mutex);
	if (--dvlp->svl_ref_cnt) {
		mutex_exit(&vhci_svl_mutex);
	...
	if (mdi_component_is_client(dvlp->svl_dip, NULL) == MDI_SUCCESS) {
		/*
		 * If enter here, this must be a pathinfo node who is the last
		 * owner of scsi_vhci_lun structure. We would clear the
		 * mdi_client::ct_vprivate member so we have everything clean.
		 */
		mdi_client_set_vhci_private(dvlp->svl_dip, NULL);
	}
	mutex_exit(&vhci_svl_mutex);
	...
	/*
	 * vhci_lun_rele may be called before the tgt_dip
	...
	if (err) {
		...
		if (retry_cnt++ < 6) {
			...
			if ((pkt->pkt_reason == CMD_CMPLT) &&
				...
				VHCI_DEBUG(1, (CE_WARN, NULL,
				    "!v_s_do_s_c:retry "
				    "packet 0x%p  sense data %s "
				    "asc 0x%x ascq 0x%x", (void *)pkt,
				    scsi_sname(skey), scsi_sense_asc(sns),
				    scsi_sense_ascq(sns)));
			}
			goto retry;
		}
...
/*
 * Issues RELEASE(6) to a path.
 */
static int
vhci_send_release(dev_info_t *cdip, mdi_pathinfo_t *pip)
{
	scsi_vhci_priv_t	*svp;
	struct scsi_pkt		*pkt;

	svp = (scsi_vhci_priv_t *)mdi_pi_get_vhci_private(pip);
	pkt = scsi_init_pkt(&svp->svp_psd->sd_address, NULL, NULL,
	    CDB_GROUP0, sizeof (struct scsi_arq_status), 0, 0, SLEEP_FUNC,
	    NULL);

	if (pkt == NULL) {
		/*
		 * This is very unlikely.
		 * scsi_init_pkt(SLEEP_FUNC) does not fail because of resources.
		 * But in theory it could fail for some other reason.  There is
		 * not an easy way how to recover though.  Log a warning and
		 * return.
		 */
		vhci_log(CE_WARN, ddi_get_parent(cdip),
		    "!Sending RELEASE(6) to %s failed.",
		    mdi_pi_spathname(pip));
		return (0);
	}
	...
	/*
	 * Ignore the return value.  If it fails then the path is probably no
	 * longer reserved anyway.
	 */
	(void) vhci_do_scsi_cmd(pkt);
	VHCI_DEBUG(1, (CE_NOTE, NULL, "!vhci_send_release: path 0x%p, issued "
	    "SCSI-2 RELEASE\n", (void *)pip));
	scsi_destroy_pkt(pkt);
	return (1);
}

...
vhci_mpapi_validate(void *udata, mp_iocdata_t *mpioc, int mode, cred_t *credp)
{
	...
	case MP_GET_LU_LIST:
	case MP_GET_INIT_PORT_LIST:
		/*
		 * XXX: MP_GET_INIT_PORT_LIST won't come;
    ...
    void *input_data, void *output_data, int mode)
{
	...
	while (mplu_path_list != NULL) {
		...
		if (!(mppathp->hide) && (mppathp->valid != 0)) {
			 ...
			 * and doesn't report it through ioctl response.
    ...
    void *input_data, void *output_data, int mode)
{
	...
	/*
	 * ld->resp can not be accessed unless ld is valid.
	 */
	if (!ld->valid) {
		VHCI_DEBUG(1, (CE_WARN, NULL, "vhci_set_tpg_access_state: "
		    "OID NOT VALID"));
    ...
    void *tp)
{
	...
	VHCI_DEBUG(4, (CE_WARN, NULL, "vhci_mpapi_get_alua_item: "
	    "Returns NULL"));
	return (NULL);
}
	...
	if (!SCSI_FAILOVER_IS_TPGS(vlun->svl_fops)) {
		...
		if (vhci_get_mpapi_item(vhci, tpg_data->lu_list,
			...
			item_list = vhci_get_mpapi_item(vhci, NULL,
			    MP_OBJECT_TYPE_MULTIPATH_LU, (void *)vlun);
			if (item_list != NULL) {
				tpg_lu_list = kmem_zalloc(
				    sizeof (mpapi_item_list_t), KM_SLEEP);
				tpg_lu_list->item = item_list->item;
				(void) vhci_mpapi_add_to_list(
				    tpg_data->lu_list, tpg_lu_list);
			} else {
				VHCI_DEBUG(1, (CE_WARN, NULL,
				    "vhci_mpapi_synthesize_tpg_data: "
				    "vlun(%p) not found", (void*)vlun));
			}
		}
		...
		if (vhci_get_mpapi_item(vhci, tpg_data->tport_list,
			...
			if (item_list != NULL) {
				...
				(void) vhci_mpapi_add_to_list(
				    tpg_data->tport_list, tpg_tport_list);
			} else {
				VHCI_DEBUG(1, (CE_WARN, NULL,
				    "vhci_mpapi_synthesize_tpg_data: "
				    "wwn(%p) not found", (void*)tmp_wwn));
			}
		}
		...
		if (lu_tpg_list == NULL) {
			if (lu_list != NULL) {
				lu_tpg_list = kmem_zalloc(
				    sizeof (mpapi_item_list_t), KM_SLEEP);
				lu_tpg_list->item = tpg_list->item;
				(void) vhci_mpapi_add_to_list(
				    ((mpapi_lu_data_t *)
				    (lu_list->item->idata))->tpg_list,
				    lu_tpg_list);
			} else {
				VHCI_DEBUG(1, (CE_WARN, NULL,
				    "vhci_mpapi_synthesize_tpg_data: "
				    "lu_list vlun(%p) not found",
				    (void*)vlun));
			}
		}
    ...
    int rel_tgt_port)
{
	...
	/*
	 * It is possible for this code to be called without the sd_pathinfo
 ...
 * There is no existing sysevent that would announce this.
 ...
 *
 * NOTE: EC_DEVFS / ESC_DEVFS_PATHI_{ADD|REMOVE} events are now
 * generated by sunmdi.c code for pathinfo nodes/paths.
 */
...
vhci_mpapi_set_path_state(dev_info_t *vdip, mdi_pathinfo_t *pip, int state)
{
	...
	if (ilist != NULL) {
		...
		/*
		 * MP API does not distinguish between ACTIVE and PASSIVE
	...
	if ((state == MP_DRVR_PATH_STATE_REMOVED) || (state ==
		 ...
		 */
		(void) vhci_mpapi_update_tpg_acc_state_for_lu(vhci, svl);

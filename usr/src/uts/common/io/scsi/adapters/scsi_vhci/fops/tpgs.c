...
static int std_path_ping(struct scsi_device *sd, void *ctpriv)
{
	...
	if (vhci_tpgs_get_target_fo_mode(sd, &mode, &state, &xlf, &preferred)) {
		VHCI_DEBUG(1, (CE_NOTE, NULL, "!std_path_ping:"
		    " failed vhci_tpgs_get_target_fo_mode\n"));
		return (0);
	}
	if ((state == STD_ACTIVE_OPTIMIZED) ||
	    (state == STD_ACTIVE_NONOPTIMIZED)) {
		VHCI_DEBUG(4, (CE_NOTE, NULL, "!path state active for %p\n",
		    (void *)sd));
		return (1);
	}
    ...
    void *ctpriv)
{
	...
	/*
	 * Get the vlun pointer. We need to look if target
	 * supports UNIT ATTENTION queuing or not.
	 */
	...
	if ((skey == KEY_UNIT_ATTENTION) &&
	    ...
	    (skey == KEY_UNIT_ATTENTION) && (asc == STD_ASC_RESET) &&
	    ((inq->inq_tpgs == TPGS_FAILOVER_IMPLICIT) ||
	    (inq->inq_tpgs == TPGS_FAILOVER_BOTH))) {
		/*

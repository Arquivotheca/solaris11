...
clean_failed_sess:
		...
		if ((strcmp((char *)isp->sess_name, target_name) == 0) &&
			...
			if (isp->sess_tpgt_conf == tpgt) {
				...
				*oid = isp->sess_oid;
				...
				if (isp->sess_tx_thread == NULL) {
					th_name = kmem_zalloc(
					    ISCSI_TH_MAX_NAME_LEN, KM_SLEEP);
					if (snprintf(th_name,
					    (ISCSI_TH_MAX_NAME_LEN - 1),
					    ISCSI_SESS_TX_NAME_FORMAT,
					    ihp->hba_oid, isp->sess_oid) <
					    ISCSI_TH_MAX_NAME_LEN) {
						isp->sess_tx_thread =
						    iscsi_thread_create(
				...
				if (isp->sess_chk_cmd_thread == NULL) {
					th_name = kmem_zalloc(
					    ISCSI_TH_MAX_NAME_LEN, KM_SLEEP);
					if (snprintf(th_name,
					    (ISCSI_TH_MAX_NAME_LEN - 1),
					    ISCSI_SESS_CHK_CMD_NAME_FORMAT,
					    ihp->hba_oid, isp->sess_oid) <
					    ISCSI_TH_MAX_NAME_LEN) {
						isp->sess_chk_cmd_thread =
						    iscsi_thread_create(
						    ihp->hba_dip,
						    th_name,
						    iscsi_chk_cmd_thread,
						    isp);
					}
					kmem_free(th_name,
					    ISCSI_TH_MAX_NAME_LEN);
	...
	*oid = isp->sess_oid;
	...
	/* startup iscsi tx thread */
	th_name = kmem_zalloc(ISCSI_TH_MAX_NAME_LEN, KM_SLEEP);
	if (snprintf(th_name, (ISCSI_TH_MAX_NAME_LEN - 1),
	    ISCSI_SESS_TX_NAME_FORMAT, ihp->hba_oid, isp->sess_oid) <
	    ISCSI_TH_MAX_NAME_LEN) {
		isp->sess_tx_thread = iscsi_thread_create(ihp->hba_dip,
		    th_name, iscsi_tx_thread, isp);
	}
	...
	th_name = kmem_zalloc(ISCSI_TH_MAX_NAME_LEN, KM_SLEEP);
	if (snprintf(th_name, (ISCSI_TH_MAX_NAME_LEN - 1),
	    ISCSI_SESS_CHK_CMD_NAME_FORMAT, ihp->hba_oid, isp->sess_oid) <
	    ISCSI_TH_MAX_NAME_LEN) {
		isp->sess_chk_cmd_thread = iscsi_thread_create(ihp->hba_dip,
		    th_name, iscsi_chk_cmd_thread, isp);
		(void) iscsi_thread_start(isp->sess_chk_cmd_thread);
	}
	...
	if (type == ISCSI_SESS_TYPE_NORMAL) {
		...
		isp->sess_flags = isp->sess_flags | ISCSI_SESS_FLAGS_CHK_MCS;
		isp->sess_conn_count = 0;
		isp->sess_conn_index = 0;
	}
	...
	isp->sess_state_destroy = B_FALSE;
...
iscsi_sess_cleanup2:
	...
	if (isp->sess_tx_thread != NULL) {
		iscsi_thread_destroy(isp->sess_tx_thread);
		isp->sess_tx_thread = NULL;
	}
	if (isp->sess_chk_cmd_thread != NULL) {
		iscsi_thread_destroy(isp->sess_chk_cmd_thread);
		isp->sess_chk_cmd_thread = NULL;
	}
...
iscsi_sess_online(void *arg)
{
	...
	iscsi_conn_t	*icp;
	int		idx;
	uint32_t	event_count, conn_count;
	...
	conn_count = iscsi_get_sess_max_conns(isp);
	if (icp == NULL) {
		if (conn_count == 1) {
			...
			isp->sess_conn_tx = icp;
		} else {
			icp = isp->sess_conn_list;
			isp->sess_conn_act = icp;
			isp->sess_conn_tx = icp;
		}
		icp->conn_sess_leading = B_TRUE;
	}
	...
	if (icp->conn_state == ISCSI_CONN_STATE_FREE) {
		/*
		 * attempt to login into the first connection in
		 * our connection list.  If this fails, we will try
		 * the next connection in our list until end of
		 * the list.
		 */
		while (icp != NULL) {
			if ((icp->conn_state == ISCSI_CONN_STATE_FREE) &&
			    (iscsi_conn_online(icp) == ISCSI_STATUS_SUCCESS)) {
				icp->conn_sess_leading = B_TRUE;
				isp->sess_conn_act = icp;
				isp->sess_conn_tx = icp;
	...
	if (conn_count <= 1 || isp->sess_max_conn_nego <= 1 ||
	    isp->sess_conn_act == NULL) {
		return;
	}

	/*
	 * MC/S try login connection list from
	 * sess_conn_act->next if avaliable,
	 * if reach end of connection list,
	 * then online connection from
	 * sess_conn_list
	 */
	if (conn_count > isp->sess_max_conn_nego) {
		conn_count = isp->sess_max_conn_nego;
	}

	/* Online connection from sess_conn_act->next */
	icp = isp->sess_conn_act->conn_next;
	for (idx = 1; idx < conn_count; idx++) {
		if (icp == NULL) {
			icp = isp->sess_conn_list;
		}
		mutex_enter(&icp->conn_state_mutex);
		if (icp->conn_state == ISCSI_CONN_STATE_FREE) {
			(void) iscsi_conn_online(icp);
		}
		mutex_exit(&icp->conn_state_mutex);
		icp = icp->conn_next;
	}
...
iscsi_sess_destroy(iscsi_sess_t *isp)
{
	...
	if (isp->sess_type == ISCSI_SESS_TYPE_NORMAL) {
		/*
		 * Here we first logout all the non-leading connections with
		 * close connection. Then we logout the leading connection with
		 * close session.
		 */
		icp = isp->sess_conn_list;
		while (icp != NULL) {
			if (icp->conn_sess_leading == B_FALSE) {
				icp->conn_logout_reason =
		...
		icp = isp->sess_conn_act;
		if (icp != NULL) {
			ASSERT(icp->conn_sess_leading == B_TRUE);
	...
	/*
	 * At this point, we should wait all the connections become to be
	 * FREE state. This will push the session to a FREE state.
	 */
	if (isp->sess_wd_thread != NULL && isp->sess_ic_thread != NULL) {
		while (wait == B_TRUE) {
	...
	/* Stop TX thread */
	if (isp->sess_tx_thread) {
		iscsi_thread_destroy(isp->sess_tx_thread);
		isp->sess_tx_thread = NULL;
	}

	/* Stop check command thread */
	if (isp->sess_chk_cmd_thread) {
		iscsi_thread_destroy(isp->sess_chk_cmd_thread);
		isp->sess_chk_cmd_thread = NULL;
	}
...
/*
 * iscsi_sess_reserve_scsi_itt - Used to reserve an ITT hash slot
 * The scsi itt range from 0x4000h ~ 0xFFFFFFFFh
 */
iscsi_status_t
iscsi_sess_reserve_scsi_itt(iscsi_cmd_t *icmdp)
{
	...
	/* Keep scsi itt values within range */
	if ((isp->sess_scsi_itt < IDM_TASKIDS_MAX) ||
	    (isp->sess_scsi_itt == (uint32_t)~IDM_ITT_MASK)) {
		isp->sess_scsi_itt = IDM_TASKIDS_MAX;
	}
	icmdp->cmd_itt = isp->sess_scsi_itt ^ itp->idt_tt;
	isp->sess_scsi_itt += IDM_TASKIDS_MAX;
...
/*
 * iscsi_sess_reserve_iscsi_itt - Used to reserve an ITT hash slot
 * The iscsi itt range from 0x0000h ~ 0x3FFFh
 */
iscsi_status_t
iscsi_sess_reserve_iscsi_itt(iscsi_sess_t *isp, iscsi_cmd_t *icmdp)
{
	 ...
	 */
	if (isp->sess_iscsi_itt >= IDM_TASKIDS_MAX)
		isp->sess_iscsi_itt = 0;

	/*
	 * Find the next available slot.  Normally its the
	 * slot pointed to by the session's sess_iscsi_itt value.
	 ...
	 */
	while (isp->sess_cmd_table[isp->sess_iscsi_itt %
	    ISCSI_CMD_TABLE_SIZE] != NULL) {
		isp->sess_iscsi_itt++;
	}

	/* reserve slot and update counters */
	icmdp->cmd_itt = isp->sess_iscsi_itt & IDM_ITT_MASK;
	isp->sess_cmd_table[isp->sess_iscsi_itt %
...
/*
 * iscsi_sess_release_iscsi_itt - Used to release ITT hash slot
 */
void
iscsi_sess_release_iscsi_itt(iscsi_sess_t *isp, iscsi_cmd_t *icmdp)
{
    ...
    uint32_t event_count)
{
	...
	case ISCSI_SESS_EVENT_N5:
		 ...
		 */
		if (iscsi_sess_conns_active(isp) == B_TRUE) {
			break;
		}
...
iscsi_sess_enum_request(iscsi_sess_t *isp, boolean_t wait,
	...
	if (isp->sess_state_destroy == B_TRUE) {
		kmem_free(itp, sizeof (iscsi_task_t));
		return (ISCSI_SESS_ENUM_GONE);
	}
	mutex_enter(&isp->sess_enum_lock);
...
boolean_t
iscsi_sess_conns_active(iscsi_sess_t *isp) {
	iscsi_conn_t	*icp;
	...
	icp = isp->sess_conn_list;
	while (icp != NULL) {
		if (ISCSI_CONN_STATE_FULL_FEATURE(icp->conn_state) == B_TRUE) {
			break;
		}
		icp = icp->conn_next;
	}

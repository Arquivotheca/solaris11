...
static void iscsi_flush_cmd_after_reset(uint32_t cmd_sn, uint16_t lun_num,
    iscsi_conn_t *icp);
static iscsi_conn_t *iscsi_conn_round_robin(iscsi_sess_t *isp);
    ...
    iscsi_cmd_t **icmdp)
{
	...
	cmd_table_idx = (ihp->itt & IDM_ITT_MASK) % ISCSI_CMD_TABLE_SIZE;
...
iscsi_tx_thread(iscsi_thread_t *thread, void *arg)
{
	...
	while (ret != 0) {
		...
		while ((isp->sess_window_open == B_TRUE) &&
			...
			if (isp->sess_type == ISCSI_SESS_TYPE_NORMAL) {
				/* MC/S, select next connect to send PDU */
				icp = iscsi_conn_round_robin(isp);
				if (icp == NULL) {
					icp = isp->sess_conn_act;
				}
			}
			if (icmdp->cmd_conn != NULL) {
				icp = icmdp->cmd_conn;
			}
...
iscsi_tx_scsi_init_pkt(iscsi_cmd_t *icmdp, iscsi_scsi_cmd_hdr_t *ihp)
{
	...
	/*
	 * Update all values before transferring.
	 * We should never touch the icmdp after
	 * transferring if there is no more data
...
iscsi_tx_logout(iscsi_sess_t *isp, iscsi_cmd_t *icmdp)
{
	...
	ilh->flags	= ISCSI_FLAG_FINAL | icmdp->cmd_un.logout.logout_reason;
	ilh->itt		= icmdp->cmd_itt;
	ilh->cid		= htons(icp->conn_cid);
	mutex_enter(&isp->sess_cmdsn_mutex);
	icmdp->cmd_sn	= isp->sess_cmdsn;
...
iscsi_handle_reset(iscsi_sess_t *isp, int level, iscsi_lun_t *ilp)
{
	...
	icmdp->cmd_lun		= ilp;
	icmdp->cmd_un.reset.level = level;
	icmdp->cmd_result	= ISCSI_STATUS_SUCCESS;
	icmdp->cmd_completed	= B_FALSE;
	icmdp->cmd_un.reset.response = SCSI_TCP_TM_RESP_COMPLETE;
	...
	if (rval == ISCSI_STATUS_SUCCESS) {
		...
		while (icp != NULL) {
			...
			while (t_icmdp != NULL) {
				...
				if (level == RESET_LUN) {
					if (icmdp->cmd_lun == NULL ||
					    t_icmdp->cmd_lun == NULL ||
					    (icmdp->cmd_lun->lun_num !=
				...
				if (icmdp->cmd_sn == t_icmdp->cmd_sn) {
					/*
					...
					mutex_exit(&t_icmdp->cmd_mutex);
				} else if ((icmdp->cmd_sn > t_icmdp->cmd_sn) ||
				    ((t_icmdp->cmd_sn - icmdp->cmd_sn) >
				    ISCSI_CMD_SN_WRAP)) {
					/*
...
iscsi_handle_logout(iscsi_conn_t *icp)
{
	...
	icmdp->cmd_result	= ISCSI_STATUS_SUCCESS;
	icmdp->cmd_completed	= B_FALSE;
	icmdp->cmd_un.logout.logout_reason = icp->conn_logout_reason;
	icmdp->cmd_conn		= icp;
...
/*
 * A watch dog thread to check pending command.
 *
 */
void
iscsi_chk_cmd_thread(iscsi_thread_t *thread, void *arg)
{
	iscsi_sess_t	*isp = (iscsi_sess_t *)arg;
	iscsi_cmd_t	*icmdp	= NULL;
	iscsi_cmd_t	*nicmdp	= NULL;
	clock_t		now;
	int		rc	= 1;
	...
	while (rc != NULL) {
		...
		for (icmdp = isp->sess_queue_pending.head;
			...
			/*
			 * Skip pending queue entries for cmd_type values that
			 * depend on having an open cmdsn window for successfull
			 * transition from pending to the active (i.e. ones that
			 * depend on sess_cmdsn .vs. sess_maxcmdsn). For them,
			 * the timer starts when they are successfully moved to
			 * the active queue by iscsi_cmd_state_pending() code.
			 */
			/*
			 * If the cmd is stuck, at least give it a chance to
			 * timeout
			 */
			if (((icmdp->cmd_type == ISCSI_CMD_TYPE_SCSI) ||
			    (icmdp->cmd_type == ISCSI_CMD_TYPE_TEXT)) &&
...
iscsi_nop_checks(iscsi_sess_t *isp)
{
	...
	while (icp != NULL) {
		...
		if ((ISCSI_CONN_STATE_FULL_FEATURE(icp->conn_state)) &&
		    (ddi_get_lbolt() > icp->conn_rx_lbolt +
		    SEC_TO_TICK(iscsi_nop_delay)) && (ddi_get_lbolt() >
		    icp->conn_nop_lbolt + SEC_TO_TICK(iscsi_nop_delay))) {
			 ...
			 */
			mutex_enter(&isp->sess_queue_pending.mutex);
			iscsi_handle_nop(icp, 0, ISCSI_RSVD_TASK_TAG);
			mutex_exit(&isp->sess_queue_pending.mutex);
		}
		mutex_exit(&icp->conn_state_mutex);
...
/*
 * Helper function
 * A rough round robin function identify which
 * connection used for send iscsi PDU
 */
static iscsi_conn_t *
iscsi_conn_round_robin(iscsi_sess_t *isp)
{
	iscsi_conn_t	*icp;
	ASSERT(isp != NULL);
	ASSERT(isp->sess_conn_tx != NULL);
	icp = isp->sess_conn_tx->conn_next;
	...
	do {
		...
		icp = icp->conn_next;
		if (icp == isp->sess_conn_tx->conn_next) {
			icp = NULL;
			break;
		}
	} while (icp != NULL);

...
/*
 * This structure is somewhat bogus. As a 'software' transport, we represent
 * a very-capable pseudo-DMA engine.  This is done, in part, because we don't
 * want 'software' transport attributes to limit scsi_vhci when they are
 * combined/merged with 'hardware' pHCI transport attributesp by
 * ddi_dma_attr_merge().
 */
static ddi_dma_attr_t iscsi_dma_attr = {
	...
	0xFFFFFFFFull,		/* counter upper bound */
	8,			/* alignment requirements */
	0x3f,			/* burst sizes */
	1,			/* minimum DMA access */
	0xFFFFFFFFull,		/* maximum DMA access */
	0xFFFFFFFFull,		/* segment boundary restrictions */
	0xFFFF,			/* scater/gather list length */
	512,			/* device granularity */
	DDI_DMA_FORCE_PHYSICAL |	/* DMA flags */
	DDI_DMA_FLAGERR |
	...
	case DDI_ATTACH:
		...
		if (ddi_create_minor_node(dip, ISCSI_DEVCTL, S_IFCHR, 0,
			...
			if (mdi_phci_register(MDI_HCI_CLASS_SCSI, dip, 0) !=
				...
				/*
				 * NOTE: Since we called mdi_phci_register()
				 * after scsi_hba_attach_setup(), we are
				 * responsible for calling
				 * mdi_devi_set_dma_attr().
				 */
				mdi_devi_set_dma_attr(dip,
				    &tran->tran_dma_attr);

				ihp->hba_mpxio_enabled = B_TRUE;
			}

			ihp->hba_load_balance = ISCSI_LB_ROUND_ROBIN;
			if (ddi_prop_lookup_string(DDI_DEV_T_ANY,
			    ihp->hba_dip, 0, ISCSI_LOAD_BALANCE_STR,
			    &data) == DDI_SUCCESS) {
				if (strcmp(data,
				    ISCSI_LOAD_BALANCE_NONE_STR) == 0) {
					ihp->hba_load_balance = ISCSI_LB_NONE;
				}
				ddi_prop_free(data);
			}
...
#undef STRING_FFOMD
	...
	case ISCSI_PARAM_SET:
		...
		if (ils->s_param == ISCSI_LOGIN_PARAM_MAX_CONNECTIONS) {
			if (!ISCSI_SUCCESS(iscsi_reconfig_sess_connections(
			    ihp, ils->s_oid))) {
				rtn = EINVAL;
				kmem_free(ils, sizeof (*ils));
				break;
...
if (t_param.p_bitmap & (1 << ISCSI_LOGIN_PARAM_MAX_CONNECTIONS)) {
							/*
							 * MC/S connection
							 * param need clear,
							 * reconfig session
			...
			if (t_param.p_bitmap &
			    (1 << ISCSI_LOGIN_PARAM_MAX_CONNECTIONS)) {
				if (!ISCSI_SUCCESS(
				    iscsi_reconfig_sess_connections(ihp,
				    e.e_oid))) {
					rtn = EINVAL;
					break;
				}
...
#undef ISCSIIOCOLGC
	...
	case ISCSI_TARGET_REENUM:
		...
		if (isp->sess_type == ISCSI_SESS_TYPE_NORMAL) {
			...
			if (isp->sess_state == ISCSI_SESS_STATE_LOGGED_IN) {
				rw_exit(&isp->sess_state_rwlock);
				if (iscsi_sess_enum_request(isp, B_TRUE,
	...
	case ISCSI_BOOT_MAC_ADDR_GET:
		mac_addr = (uchar_t *)kmem_zalloc(ETHERADDRL, KM_SLEEP);
		/*
		 * Check whether we are using iSCSI Boot. If we are not using
		 * iSCSI Boot, will pass all zero to the buffer
		 */
		if (iscsiboot_prop != NULL) {
			for (i = 0; i < ETHERADDRL; i++) {
				mac_addr[i] =
				    iscsiboot_prop->boot_nic.nic_mac[i];
			}
		}
		if (ddi_copyout(mac_addr, (caddr_t)arg, ETHERADDRL, mode)
		    != 0) {
			rtn = EFAULT;
		}
		kmem_free(mac_addr, ETHERADDRL);
		break;

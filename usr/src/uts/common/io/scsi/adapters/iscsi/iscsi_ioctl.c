...
#define	ISCSI_SENDTGTS_REQ_STR		"SendTargets=All"
	...
	status = iscsi_conn_create(&addr_snd.sin, isp, &icp, B_TRUE);
	rw_exit(&isp->sess_conn_list_rwlock);
...
iscsi_set_param(iscsi_login_params_t *params, iscsi_param_set_t *ipsp)
{
	...
	case ISCSI_LOGIN_PARAM_MAX_CONNECTIONS:
		params->max_connections = ipsp->s_value.v_integer;
		break;
...
iscsi_ioctl_set_tunable_param(iscsi_hba_t *ihp, iscsi_tunable_object_t *tpss)
{
	...
	if (tpss->t_value.v_integer > ISCSI_TUNABLE_MAX) {
		return (EINVAL);
	}
	...
	if ((param_id == ISCSI_TUNABLE_PARAM_CONN_LOGIN_MAX) ||
	    (param_id == ISCSI_TUNABLE_PARAM_LOGIN_POLLING_DELAY)) {
		if (pparam->p_params.conn_login_max <
		    pparam->p_params.polling_login_delay) {
...
int
iscsi_get_sess_max_conns(iscsi_sess_t *isp)
{
	persistent_param_t	*pp;
	int conns = 0;

	pp = kmem_zalloc(sizeof (persistent_param_t), KM_SLEEP);
	if (persistent_param_get((char *)isp->sess_name, pp)) {
		if (pp->p_bitmap & (1 << ISCSI_LOGIN_PARAM_MAX_CONNECTIONS)) {
			conns = pp->p_params.max_connections;
		}
	}
	kmem_free(pp, sizeof (persistent_param_t));
	...
	if (conns == 0) {
		/*
		 * iscsi target's max connection not set or
		 * default value, try use initiator node's max
		 * connection param
		 */
		conns = isp->sess_params.max_connections;
	}

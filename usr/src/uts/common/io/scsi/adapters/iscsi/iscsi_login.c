...
static void iscsi_declare_key_values(iscsi_conn_t *icp);
static void iscsi_update_leading_conn(iscsi_conn_t *icp);
...
iscsi_login_end(iscsi_conn_t *icp, iscsi_status_t status, iscsi_task_t *itp)
{
	...
	if (status == ISCSI_STATUS_SUCCESS) {
		...
		(void) iscsi_declare_key_values(icp);

		/* We are now logged in */
		iscsi_conn_update_state(icp, ISCSI_CONN_STATE_LOGGED_IN);

		/* startup TX thread */
		(void) iscsi_thread_start(isp->sess_tx_thread);
		...
		case ISCSI_CONN_STATE_FAILED:
			...
			mutex_exit(&icp->conn_state_mutex);
			if ((iscsi_get_sess_max_conns(isp) == 1) ||
			    (iscsi_sess_conns_active(isp) == B_FALSE)) {
				event_count = atomic_inc_32_nv(
				    &isp->sess_state_event_count);
				iscsi_sess_enter_state_zone(isp);
				iscsi_sess_state_machine(isp,
...
iscsi_login(iscsi_conn_t *icp, uint8_t *status_class, uint8_t *status_detail)
{
	...
	do {
		...
		if (icp->conn_login_state != LOGIN_RX && ilrhp->opcode == 0) {
			...
			if (timeout_result == -1) {
				/* wait login response timeout */
				rval = ISCSI_STATUS_RX_TIMEOUT;
			} else {
				if (icp->conn_login_state == LOGIN_ERROR) {
					/*
					 * login error, connection down,
					 * with no login response.
					 */
					rval = (ISCSI_STATUS_LOGIN_FAILED);
				} else {
					rval = (ISCSI_STATUS_INTERNAL_ERROR);
				}
			}
    ...
    char *data, int max_data_length)
{
	...
	ilhp->cid = htons(ilhp->cid);
	bcopy(&isp->sess_isid[0], &ilhp->isid[0], sizeof (isp->sess_isid));
	rw_enter(&isp->sess_state_rwlock, RW_READER);
	if ((iscsi_get_sess_max_conns(isp) > 1) && (isp->sess_tsid != 0) &&
	    (isp->sess_state == ISCSI_SESS_STATE_LOGGED_IN)) {
		ilhp->tsid = htons(isp->sess_tsid);
	} else {
		ilhp->tsid = 0;
	}
...
more_text:
	...
	return (ISCSI_STATUS_LOGIN_TPGT_NEGO_FAIL);
				...
				if (isp->sess_type ==
				    ISCSI_SESS_TYPE_DISCOVERY) {
					/* EMPTY */
				} else if (ddi_strtoul(value, &tmpe, 0,
				    &tmp) != 0) {
					cmn_err(CE_WARN, "iscsi connection(%u) "
					    "login failed - can't accept "
					    "MaxConnections %s",
					...
					if (tmp_conn <= 0) {
						cmn_err(CE_WARN,
						    "iscsi connection(%u) "
						    "login failed - can't "
						    "accept MaxConnections %s",
						    icp->conn_oid, value);
						return (ISCSI_STATUS_NEGO_FAIL);
					}
					icp->conn_params.max_connections =
					    tmp_conn;
					isp->sess_max_conn_nego = tmp_conn;
				}
				text = value_end;
	...
	if (rval != IDM_STATUS_SUCCESS) {
		if (addr->sa_family == AF_INET) {
			struct sockaddr_in *addr_in =
			    &icp->conn_curr_addr.sin4;
			sin_ptr = &addr_in->sin_addr.s_addr;
		} else {
			struct sockaddr_in6 *addr_in6 =
			    &icp->conn_curr_addr.sin6;
			sin_ptr = &addr_in6->sin6_addr.s6_addr;
		}
		ip_str = inet_ntop(addr->sa_family, sin_ptr, ip_buf,
		    INET6_ADDRSTRLEN);
		cmn_err(CE_NOTE, "iscsi connection(%u) unable to "
		    "connect to target %s, target address %s", icp->conn_oid,
		    icp->conn_sess->sess_name, ip_str);
		idm_conn_rele(icp->conn_ic);
	}
...
iscsi_notice_key_values(iscsi_conn_t *icp)
{
	...
	rc = nvlist_add_uint64(neg_nvl, "MaxRecvDataSegmentLength",
	    (uint64_t)icp->conn_params.max_xmit_data_seg_len);
	ASSERT(rc == 0);
...
/*
 * iscsi_declare_key_values - Create an nvlist containing the declarative
 * key value pairs that are important to IDM and pass them down to IDM,
 * for example the MaxRecvDataSegmentLength is used by IDM to verify the
 * received PDU length.
 */
static void
iscsi_declare_key_values(iscsi_conn_t *icp)
{
	nvlist_t	*cfg_nvl;
	int		rc;
	kv_status_t	kvrc;

	rc = nvlist_alloc(&cfg_nvl, NV_UNIQUE_NAME, KM_SLEEP);
	ASSERT(rc == 0);

	rc = nvlist_add_uint64(cfg_nvl, "MaxRecvDataSegmentLength",
	    (uint64_t)icp->conn_params.max_recv_data_seg_len);
	ASSERT(rc == 0);

	kvrc = idm_declare_key_values(icp->conn_ic, cfg_nvl, NULL);
	ASSERT(kvrc == KV_HANDLED);
...
static void
iscsi_update_leading_conn(iscsi_conn_t *icp)
{
	iscsi_conn_t *t_icp = NULL;
	iscsi_sess_t *isp = icp->conn_sess;

	t_icp = icp->conn_sess->sess_conn_list;
	while (t_icp != NULL) {
		mutex_enter(&t_icp->conn_state_mutex);
		if (t_icp->conn_sess_leading == B_TRUE) {
			t_icp->conn_sess_leading = B_FALSE;
		}
		if (t_icp == icp) {
			t_icp->conn_sess_leading = B_TRUE;
			isp->sess_conn_act = icp;
		}
		mutex_exit(&t_icp->conn_state_mutex);

...
typedef enum iscsi_load_balance {
	ISCSI_LB_NONE = 0,
	ISCSI_LB_ROUND_ROBIN,	/* Round Robin */
	ISCSI_LB_MAX
} iscsi_load_balance_t;

#define	ISCSI_LOAD_BALANCE_STR		"load-balance"
#define	ISCSI_LOAD_BALANCE_RR_STR	"round-robin"
#define	ISCSI_LOAD_BALANCE_NONE_STR	"none"
...
typedef struct iscsi_conn {
	...
	/* MC/S leading connection */
	boolean_t		conn_sess_leading;

	uchar_t			conn_logout_reason;
} iscsi_conn_t;
...
#define	ISCSI_SESS_FLAGS_CHK_MCS	0x01
...
typedef struct iscsi_sess {
	...
	/* MC/S active connection to end PDU in TX thread */
	struct iscsi_conn	*sess_conn_tx;
	...
	uint32_t		sess_iscsi_itt;
	uint32_t		sess_scsi_itt;
	/*
	...
	int			sess_max_conn_nego;

	iscsi_thread_t		*sess_chk_cmd_thread;
} iscsi_sess_t;
...
typedef struct iscsi_hba {
	...
	iscsi_load_balance_t	hba_load_balance;
} iscsi_hba_t;
...
void iscsi_chk_cmd_thread(iscsi_thread_t *thread, void *arg);
...
iscsi_status_t iscsi_sess_reserve_iscsi_itt(iscsi_sess_t *isp,
    iscsi_cmd_t *icmdp);
void iscsi_sess_release_iscsi_itt(iscsi_sess_t *isp, iscsi_cmd_t *icmdp);
...
boolean_t iscsi_sess_conns_active(iscsi_sess_t *isp);

/* iscsi_conn.c */
iscsi_status_t iscsi_conn_create(struct sockaddr *addr, iscsi_sess_t *isp,
    iscsi_conn_t **icpp, boolean_t unique);
...
int iscsi_get_sess_max_conns(iscsi_sess_t *isp);
...
iscsi_status_t iscsi_reconfig_sess_connections(iscsi_hba_t *ihp,
    uint32_t target_oid);

...
static void iscsid_check_mcs(iscsi_hba_t *ihp, iSCSIDiscoveryMethod_t method);
static iscsi_status_t iscsid_del_match_conn(iscsi_sess_t *isp,
    struct sockaddr *addr_dsc);
...
retry_sendtgts:
	...
	kmem_free(stl_hdr, stl_sz);
	iscsid_check_mcs(ihp, iSCSIDiscoveryMethodSendTargets);
}
    ...
    struct sockaddr *addr_tgt)
{
	...
	for (isid = 0; isid < ics->ics_out; isid++) {
		...
		/* create or find matching connections for this session */
		if (!ISCSI_SUCCESS(iscsi_conn_create(addr_tgt, isp, &icp,
		    B_TRUE))) {
			/*
			 * MC/S. No connection created for this session,
	...
	while (isp != NULL) {
		...
		if (((target_name == NULL) ||
			...
			if ((method == iSCSIDiscoveryMethodISNS) ||
				...
				int	conn_max = 1;
				conn_max = iscsi_get_sess_max_conns(isp);
				if ((isp->sess_max_conn_nego != 0) &&
				    (conn_max > isp->sess_max_conn_nego)) {
					conn_max = isp->sess_max_conn_nego;
				}
				...
				if (addr_dsc == NULL) {
					...
					if (conn_max == 1) {
						...
						/*
						 * MC/S, try delete
						 * connection match with
						 * addr_dsc.
						 */
						if (!ISCSI_SUCCESS(
						    iscsid_del_match_conn(isp,
						    addr_dsc))) {
							cmn_err(CE_WARN,
							    "Static session(%s)"
							    " - fail to delete "
...
iscsid_thread_static(iscsi_thread_t *thread, void *p)
{
	...
	while (iscsi_thread_wait(thread, -1) != 0) {
		...
		iscsid_check_mcs(ihp, iSCSIDiscoveryMethodStatic);
    ...
    iscsi_param_set_t *ipsp)
{
	...
	case ISCSI_LOGIN_PARAM_MAX_CONNECTIONS:
		ipsp->s_value.v_integer =  params->max_connections;
		break;
    ...
    isns_portal_group_list_t *pg_list)
{
	...
	iscsid_check_mcs(ihp, iSCSIDiscoveryMethodISNS);
}
...
/*
 * Decrease the connections for the session.
 */
iscsi_status_t
iscsi_reconfig_sess_connections(iscsi_hba_t *ihp, uint32_t target_oid)
{
	...
	int		conn_count  = 1;
	int		tmp	    = 0;
	boolean_t	try_poke    = B_FALSE;
	iSCSIDiscoveryMethod_t	poke_discovery = iSCSIDiscoveryMethodUnknown;

	rw_enter(&ihp->hba_sess_list_rwlock, RW_READER);
	isp = ihp->hba_sess_list;
	while (isp != NULL) {
		if ((target_oid == ihp->hba_oid) ||
		    (target_oid == isp->sess_oid) ||
		    (target_oid == isp->sess_target_oid)) {
			rw_enter(&isp->sess_conn_list_rwlock, RW_WRITER);
			conn_count = iscsi_get_sess_max_conns(isp);
			if (conn_count >= isp->sess_conn_count) {
				/* No need to destroy connection */
				rw_exit(&isp->sess_conn_list_rwlock);
				try_poke = B_TRUE;
				if (target_oid != ihp->hba_oid) {
					poke_discovery =
			...
			tmp = isp->sess_conn_count - conn_count;
			for (idx = 0; idx < tmp; idx++) {
				icp = isp->sess_conn_list_last_ptr;

				if ((icp->conn_sess_leading == B_TRUE) ||
				    (icp == isp->sess_conn_act)) {
...
icp->conn_logout_reason = ISCSI_LOGOUT_REASON_CLOSE_CONNECTION;
	...
	rw_exit(&ihp->hba_sess_list_rwlock);
	if (ISCSI_SUCCESS(rval) && (try_poke == B_TRUE)) {
		if (target_oid == ISCSI_INITIATOR_OID) {
			poke_discovery = iSCSIDiscoveryMethodUnknown;
		}
		iscsid_poke_discovery(ihp, poke_discovery);
		/* lock so only one config operation occurs */
...
/*
 * Increase the connections for session.
 */
static void
iscsid_check_mcs(iscsi_hba_t *ihp, iSCSIDiscoveryMethod_t method)
{
	iscsi_sess_t	*isp		= NULL;
	iscsi_conn_t	*icp		= NULL;
	iscsi_conn_t	*tmp_icp	= NULL;
	...
	rw_enter(&ihp->hba_sess_list_rwlock, RW_WRITER);
	for (isp = ihp->hba_sess_list; isp != NULL; isp = isp->sess_next) {
		...
		if (isp->sess_conn_count >= conn_max) {
			continue;
		}

		icp = isp->sess_conn_list;
		while ((icp != NULL) && (i < isp->sess_conn_index)) {
			i++;
			icp = icp->conn_next;
		}
		i = conn_max - isp->sess_conn_count;
		rw_enter(&isp->sess_conn_list_rwlock, RW_WRITER);
		while ((i > 0) && (icp != NULL)) {
			status =
			    iscsi_conn_create(
			    (struct sockaddr *)&icp->conn_base_addr, isp,
			    &tmp_icp, B_FALSE);
...
iscsid_del_match_conn(iscsi_sess_t *isp, struct sockaddr *addr_dsc)
{
	iscsi_conn_t	*icp, *t_icp;
	iscsi_status_t  rval    = ISCSI_STATUS_SUCCESS;
	...
	rw_enter(&isp->sess_conn_list_rwlock, RW_WRITER);
	for (icp = isp->sess_conn_list; icp; icp = t_icp) {
		t_icp = icp->conn_next;
		if (bcmp(addr_dsc, &icp->conn_base_addr.sin,
		    SIZEOF_SOCKADDR(&icp->conn_base_addr.sin)) == 0) {
			if (icp->conn_sess_leading == B_TRUE) {
				cmn_err(CE_NOTE, "Can't delete leading "
				    "connection(%u) from session %s",
				    icp->conn_oid, isp->sess_name);
				continue;
			}

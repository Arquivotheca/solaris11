 ...
 */
iscsi_status_t
iscsi_conn_create(struct sockaddr *addr, iscsi_sess_t *isp,
    iscsi_conn_t **icpp, boolean_t unique)
{
	...
	/* See if this connection has already existed */
	for (icp = isp->sess_conn_list; icp; icp = icp->conn_next) {

		/*
		 * Query connection list in session to see if
		 * there is a match for this connection. If we are
		 * not using MC/S, this can be done by making sure
		 * the IPs are of the same size and then they are
		 * the same value. If MC/S is enabled, we should
		 * also check the number of connections created.
		 * If there are enough connections, we won't create
		 * the another connection with this IP address.
		 */
...
iscsi_conn_offline(iscsi_conn_t *icp)
{
	 ...
	 */
	delay = ddi_get_lbolt() +
	    SEC_TO_TICK(icp->conn_tunable_params.polling_login_delay +
...
iscsi_conn_destroy(iscsi_conn_t *icp)
{
	...
	/*
	 * MC/S check removed connection is session's active TX
	 * connection.
	 */
	if (isp->sess_conn_tx == icp) {
		isp->sess_conn_tx = icp->conn_next;
		if (isp->sess_conn_tx == NULL) {
			isp->sess_conn_tx = isp->sess_conn_act;
		}
	}

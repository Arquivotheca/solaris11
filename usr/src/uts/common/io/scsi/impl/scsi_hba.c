...
/* number of probe serialization messages */
...
typedef struct impl_scsi_tgtmap {
	...
	int		tgtmap_error_usec;
	int		tgtmap_error_thresh;
	int		tgtmap_error_iops_thresh;
	int		tgtmap_sync_cnt;
} impl_scsi_tgtmap_t;
...
scsi_initialize_hba_interface()
{
	...
	/* We need "scsa,probe" and "scsa,nodev" as an alias or a driver. */
...
/* scsi_hba_attach(9F): Obsolete, use scsi_hba_attach_setup(9F). */
/*ARGSUSED*/
int
scsi_hba_attach(
	dev_info_t		*self,
	...
	void			*hba_options)
{
	...
	/* Start with restrictive scsi_alloc_attr attributes */
	hba_dma_attr = scsi_alloc_attr;
...
#define	CONFIG_INT_PROP(s, p, dv)	{			\
	...
	/*
	 * Register the tran's ddi_dma_attr(9S) with mdi. If self is not a
	 * vHCI or pHCI, this call is a noop. This call needs to occur after
	 * mdi_phci_register().
	 */
	mdi_devi_set_dma_attr(self, &tran->tran_dma_attr);
...
scsi_hba_attach_setup(
	...
	int			flags)
{
	extern int		scsa_hba_pkt_setup(scsi_hba_tran_t *);
	extern ddi_dma_attr_t	scsi_alloc_attr;
	...
	/*
	 * The SCSA tran_setup_pkt(9E) and scsi_ifgetcap(9F) implementation code
	 * expect hba_dma_attr to be accurate for HBA data buf(9S) transfers.
	 * NOTE: If a more restrictive ddi_dma_attr_t is used, we can suffer
	 * from poor performance (allocation/copy-buffer) or operational
	 * failures (tape large records).
	 */
	tran->tran_dma_attr = *hba_dma_attr;

	/*
	 * We may encounter old HBA drivers, from a time when the hba_dma_attr
	 * passed into scsi_hba_attach_setup(9E) did not need to be accurate.
	 * We need to detect this situation and fallback to using
	 * scsi_alloc_attr, otherwise scsi_alloc_consistent_buf can loop
	 * forever retrying an i_ddi_mem_alloc that will never succeed.
	 */
	if (hba_dma_attr->dma_attr_addr_lo == hba_dma_attr->dma_attr_addr_hi) {
		SCSI_HBA_LOG((_LOG(WARN), self, NULL, "driver exports bogus "
		    "ddi_dma_attr(9S) to scsi_hba_attach_setup(9F)"));
		tran->tran_dma_attr = scsi_alloc_attr;
	}
#ifdef	DEBUG
#ifdef	_x86
	/*
	 * To identify performance problems, on debug kernels we produce a
	 * warning. We check dma_attr_addr_hi directly instead of using
	 * i_ddi_copybuf_required so the warning occurs independent of the
	 * amount of memory installed on the machine.
	 */
	if (hba_dma_attr->dma_attr_addr_hi <= 0xFFFFFFFFU)
		SCSI_HBA_LOG((_LOG(WARN), self, NULL,
		    "driver operations may require copy buffer use"));
#endif	/* __x86 */
#endif	/* DEBUG */

	/* Establish scsi_hba_pkt_xxx/tran_setup_pkt(9E) environment. */
	if (!scsa_hba_pkt_setup(tran))
		goto fail;
...
scsi_busctl_initchild(dev_info_t *child)
{
	...
	*addr = '\0';
	...
	scsi_device_damap_nxpe_bind(sd);
...
scsi_busctl_uninitchild(dev_info_t *child)
{
	...
	scsi_device_damap_nxpe_unbind(sd);
    ...
    char ***compatiblep, int *ncompatiblep)
{
	...
	*csp = NULL;			/* NULL terminate array of pointers */
	 ...
	 * ddi_driver_compatible_major() binding that will be established by
	 * bind_node().
	 *
	 * NOTE: we can't fully special-case, and 'pretend' to have
	 * an alias like '"scsiclass" nulldriver' because bind_node()
	 * can't pretend: the '"scsiclass" nulldriver' alias needs to
	 * be in the base /etc/driver_aliases file.
	 */
	...
	/* Partial repair for missing '"scsiclass" nulldriver' alias. */
	if (dname == NULL)
		dname = "nulldriver";

	/*
	 * Corner-case, last ditch effort to come up with a nodename. If no
	 * nodename_aliases mapping exists then use the drivername as a
	 * nodename. The unfortunate thing about using a drivername as a
	 * nodename is the nodename can change as a result of
	 * 'update_drv -a -i <alias> <dname>'. After such a change, correct
	 * enumeration, with updated nodename-drivername, will require a
	 * reboot (or possibly a hotplug).
	 */
	if (nname == NULL)
		nname = dname;

	/* If the nodename is "nulldriver", switch nodename to "nodriver". */
	if (strcmp(nname, "nulldriver") == 0)
		nname = "nodriver";
	...
	*dchildp = NULL;
	...
	for (dchild = ddi_get_child(self); dchild;
		...
		if (daddr && (strcmp(addr, daddr) == 0) &&
			 ...
			 * to find the 'probe' node. The existence of
			 * a probe node on a 'name == NULL' search should
			 * fail.  This will trigger slow-path code where
			 * we explicitly look for, and synchronize against,
    ...
    struct scsi_device *sdprobe, dev_info_t **dchildp, mdi_pathinfo_t **pchildp)
{
	...
	if ((guid == NULL) ||
		...
		/* Watch out for inconsistencies in devids. */
...
scsi_hba_pkt_comp(struct scsi_pkt *pkt)
{
	 ...
	 * determine the scsi_device given the scsi_address embedded in
	 * scsi_pkt (via scsi_address_device(9F)), we also require the use of
	...
	pkt_stat = *pkt->pkt_scbp & STATUS_MASK;
	pkt_reason = pkt->pkt_reason;
	 ...
	 */
	if ((pkt_reason == CMD_CMPLT) && (pkt_stat == STATUS_CHECK) &&
	...
	/*
	 * Check for success or failure of the transport.
	 *
	 * We only care about transport failures (as opposed to other types
	 * of failures; e.g. hardware). This gathering of statistics needs to
	 * be as fast as possible since it is the performance path.  Thus, the
	 * counters may not be 100% accurate, nor may be the detection.
	 *
	 * For the non-transport error case, a counter is simply incremented.
	 *
	 * In the case of a transport error, call into the DAM.  This is an
	 * indication to the DAM that there may be work to do with regard to
	 * this device.  The DAM will keep track of the count of transport
	 * errors associated with this device via the call to damap_xpe_report.
	 */
	sd = scsi_address_device(&(pkt->pkt_address));
	if (sd->sd_io_nxpe) {
		if ((pkt_reason == CMD_TRAN_ERR) ||
		    (pkt_reason == CMD_TIMEOUT) ||
		    (pkt_reason == CMD_DEV_GONE)) {
			damap_xpe_report(sd->sd_damapp, sd->sd_damap_id);
		} else {
			(*sd->sd_io_nxpe)++;
		}
	}
    ...
    int *circp, int *ppi)
{
	 ...
	 * a state usable state for issuing our "probe" commands. We establish
	 * this barrier node with a node name of "probe" and compatible
	 * property of "scsa,probe". The compatible property must be associated
	...
	for (;;) {
		 ...
		 * previously "scsa,nodev" LUN0 coming into existence (or a
	 ...
	 * implementation. We also add a "compatible" property of "scsa,probe"
	...
	if (child) {
		 ...
		 * for devices which are currently device_isremoved in order to
...
scsi_device_unconfig(dev_info_t *self, char *name, char *addr, int *circp)
{
	...
	if (child) {
		...
		chg = mdi_pi_device_remove(path);
		SCSI_HBA_LOG((_LOGUNCFG, self, NULL,
		    "pathinfo%s %s note device_remove%s",
		    (chg == 2) ? "/devinfo" : "",
		    spathname, chg ? "" : "ed already"));
    ...
    dev_info_t *self, char *taddr, int pi, int mt, scsi_enum_t se)
{
	...
	if (tgtmap) {
		...
		tgtid = damap_lookup(tgtdam, taddr, B_FALSE);
		if (tgtid != NODAM) {
    ...
    ddi_bus_config_op_t op, void *arg, dev_info_t **childp)
{
	 ...
	 * we need the framework to wait for children that are physically
    ...
    ddi_bus_config_op_t op, void *arg, dev_info_t **childp)
{
	...
	if (tran->tran_hba_flags & SCSI_HBA_HBA) {
		 ...
		 * BUS_CONFIG_ONE operation down a level to resolve an
    ...
    damap_config_rsn_t reason)
{
	...
	/*
	 * If the reason is CONFIG_CMPLT, all we have to do is signal completion
	 */
	if (reason == DAMAP_CONFIG_RSN_CONFIG_ALL_CMPLT) {
		SCSI_HBA_LOG((_LOGIPT, self, NULL,
		    "%s bus config complete", damap_name(mapp)));

		ndi_busop_bus_config_all_complete(self);
		return (DAM_SUCCESS);
	}
...
scsi_hba_tgtmap_create(dev_info_t *self, scsi_tgtmap_mode_t mode,
    scsi_tgtmap_params_t *tgtmap_params, void *tgtmap_priv,
    ...
    scsi_hba_tgtmap_t **handle)
{
	...
	if (self == NULL || tgtmap_params == NULL || handle == NULL)
		return (DDI_FAILURE);
	...
	*handle = NULL;
	...
	tgtmap->tgtmap_create_csync_usec = tgtmap_params->csync_usec;
	tgtmap->tgtmap_settle_usec = tgtmap_params->stable_usec;
	tgtmap->tgtmap_error_usec = tgtmap_params->error_usec;
	tgtmap->tgtmap_error_thresh = tgtmap_params->error_thresh;
	tgtmap->tgtmap_error_iops_thresh = tgtmap_params->error_iops_thresh;
	...
	if (damap_create(context, rpt_style, optflags,
	    tgtmap_params->stable_usec, tgtmap_params->error_usec,
	    tgtmap_params->error_thresh, tgtmap_params->error_iops_thresh,
	    tgtmap, scsi_tgtmap_scsi_activate, scsi_tgtmap_scsi_deactivate,
	    tran, scsi_tgtmap_scsi_config, scsi_tgtmap_scsi_unconfig, &mapp)
	    != DAM_SUCCESS) {
	...
	if (damap_create(context, rpt_style, optflags,
	    tgtmap_params->stable_usec, tgtmap_params->error_usec,
	    tgtmap_params->error_thresh, tgtmap_params->error_iops_thresh,
	    tgtmap, scsi_tgtmap_smp_activate, scsi_tgtmap_smp_deactivate, tran,
	    scsi_tgtmap_smp_config, scsi_tgtmap_smp_unconfig,
	    &mapp) != DAM_SUCCESS) {
    ...
    int jitter)
{
	...
	return ((damap_addrset_add(tgtmap->tgtmap_dam[tgt_type], tgt_addr,
	    NULL, NULL, tgt_priv, jitter) == DAM_SUCCESS)
	    ? DDI_SUCCESS : DDI_FAILURE);
}
    ...
    char *tgt_addr, scsi_tgtmap_tgt_type_t *r_type)
{
	...
	for (i = 0; i < SCSI_TGT_NTYPES; i++) {
		tgtid = damap_lookup(tgtmap->tgtmap_dam[i], tgt_addr, B_FALSE);
		if (tgtid != NODAM) {
			*r_type = i;
			SCSI_HBA_LOG((_LOG(3), self, NULL,
			    "%s @%s found: type %d",
 ...
 * scsi bus_config implementation to dynamically enumerate the specified iport
    ...
    void *arg, dev_info_t **childp)
{
	...
	for (i = 0; i < num_iports; i++) {
		...
		if (pdip = scsi_hba_bus_config_port(self, addr, SE_BUSCONFIG)) {
			...
			/*
			 * Try to configure child under iport see whether
    ...
    scsi_hba_iportmap_t **handle)
{
	...
	*handle = NULL;
	...
	if (damap_create(context, DAMAP_REPORT_PERADDR, DAMAP_SERIALCONFIG,
	    settle_usec, 0, 0, 0, NULL, NULL, NULL, self,
    ...
    char *iport_addr)
{
	...
	iportid = damap_lookup(iportmap->iportmap_dam, iport_addr, B_FALSE);
	if (iportid != NODAM) {
		SCSI_HBA_LOG((_LOG(3), self, NULL,
		    "%s @%s found",
    ...
    damap_config_rsn_t reason)
{
	...
	/*
	 * If the reason is CONFIG_CMPLT, there's nothing more to do
	 */
	if (reason == DAMAP_CONFIG_RSN_CONFIG_ALL_CMPLT) {
		return (DAM_SUCCESS);
	}
...
scsi_lunmap_create(dev_info_t *self, impl_scsi_tgtmap_t *tgtmap, char *taddr)
{
	...
	/* create lundam */
	if (damap_create(context, DAMAP_REPORT_FULLSET, optflags, 1, 0, 0, 0,
    ...
    char *taddr, scsi_lun64_t lun64, int sfunc)
{
	...
	return ((damap_addrset_add(lundam, ua, NULL, NULL,
	    NULL, 1) == DAM_SUCCESS) ? DDI_SUCCESS : DDI_FAILURE);
}
...
scsi_lunmap_lookup(dev_info_t *self, damap_t *lundam, char *addr)
{
	...
	if ((lunid = damap_lookup(lundam, addr, B_FALSE)) != NODAM) {
		SCSI_HBA_LOG((_LOG(3), self, NULL,
    ...
    sas_phymap_t **handlep)
{
	...
	if (damap_create(context, DAMAP_REPORT_PERADDR, DAMAP_SERIALCONFIG,
	    settle_usec, 0, 0, 0, NULL, NULL, NULL,
...
sas_phymap_lookup_uapriv(sas_phymap_t *handle, char *ua)
{
	...
	name = ddi_soft_state_bystr_get(phymap->phymap_ua2name, ua);
	if (name) {
		phyid = damap_lookup(phymap->phymap_dam, name, B_FALSE);
		if (phyid != NODAM) {
...
void
scsi_device_damap_nxpe_bind(struct scsi_device *sd)
{
	dev_info_t	*self;
	scsi_hba_tran_t	*tran;
	damap_t		*damapp;
	...
	ASSERT(sd && sd->sd_dev);
	self = sd->sd_dev;
	tran = sd->sd_address.a_hba_tran;

	if (tran->tran_tgtmap == NULL) {
		SCSI_HBA_LOG((_LOGTGT, NULL, self, "No target map"));
		return;
	}

	damapp = ((impl_scsi_tgtmap_t *)tran->tran_tgtmap)->
	    tgtmap_dam[SCSI_TGT_SCSI_DEVICE];

	if (scsi_device_prop_lookup_string(sd, SCSI_DEVICE_PROP_PATH,
	    SCSI_ADDR_PROP_TARGET_PORT, &taddr) != DDI_PROP_SUCCESS) {
		SCSI_HBA_LOG((_LOGTGT, NULL, self,
		    "%s failed to get target port property",
		    damap_name(damapp)));
		return;
	}

	mapid = damap_lookup(damapp, taddr, B_TRUE);
	if (mapid == NODAM) {
		SCSI_HBA_LOG((_LOGTGT, NULL, self, "%s @%s damap_lookup failed",
		    damap_name(damapp), taddr));
		scsi_device_prop_free(sd, SCSI_DEVICE_PROP_PATH, taddr);
		return;
	}

	damap_nxpe_bind(damapp, mapid, &sd->sd_io_nxpe);

	/*
	 * Store the pointer to the damap and the damap_id so that I/Os
	 * completed without transport errors can be reported without
	 * requiring expensive locking and/or lookups.
	 */
	if (sd->sd_io_nxpe) {
		sd->sd_damapp = damapp;
		sd->sd_damap_id = mapid;
	} else {
		SCSI_HBA_LOG((_LOGTGT, NULL, self,
		    "%s @%s damap_stat_bind failed",
		    damap_name(damapp), taddr));
	}

	scsi_device_prop_free(sd, SCSI_DEVICE_PROP_PATH, taddr);
	damap_id_rele(damapp, mapid);
}

void
scsi_device_damap_nxpe_unbind(struct scsi_device *sd)
{
	scsi_hba_tran_t	*tran;
	damap_t		*damapp;
	damap_id_t	mapid;
	...
	if (tran->tran_tgtmap == NULL) {
		SCSI_HBA_LOG((_LOGTGT, NULL, sd->sd_dev, "No target map"));
		return;
	}

	damapp = sd->sd_damapp;
	mapid = sd->sd_damap_id;
	if (!damapp || (mapid == NODAM)) {
		SCSI_HBA_LOG((_LOGTGT, NULL, sd->sd_dev,
		    "No target map or addrid. Already unbound? (sd = %p)",
		    (void *)sd));
		sd->sd_damapp = NULL;
		sd->sd_damap_id = NODAM;
		return;
	}
	damap_nxpe_unbind(damapp, mapid, &sd->sd_io_nxpe);
	sd->sd_damapp = NULL;
	sd->sd_damap_id = NODAM;
}

/*
 * Wrapper for checking for a pending unconfiguration.
 */
int
scsi_device_pending_unconfig(struct scsi_device *sd)
{
	return (dam_addrid_pending_uncfg(sd->sd_damapp, sd->sd_damap_id));
}

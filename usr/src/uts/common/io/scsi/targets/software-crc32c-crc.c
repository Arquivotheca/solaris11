...
/*
 * CRC-32C table cloned from iscsi_crc32c_table.
 */
...
static uint32_t
crc32c_crc(dp_setable const *set, caddr_t datap, uint32_t blk_len)
{
	uint8_t		*blk_addr;
	uint32_t	crc;
	...
	/* Do Reflected CRC */
	crc = set->dp_seed;
	blk_addr = (uint8_t *)datap;

	while (blk_len--) {
		crc = crctable[(crc ^ *blk_addr++) & 0xFF] ^ (crc >> 8);
	}
...
crc32c_crcadd(dp_setable const *set, caddr_t datap, uint32_t blk_len)
{
	...
	crc = crc32c_crc(set, datap, blk_len);
	*crcpnt = BE_32(crc);
}
...
crc32c_crcchk(dp_setable const *set, caddr_t datap, uint32_t blk_len)
{
	...
	crc = crc32c_crc(set, datap, blk_len - set->dp_len);
	if (BE_32(*crcpnt) == crc)
		return (B_TRUE);
	return (B_FALSE);
}

const dp_funcs crc32c_software = {
	crc32c_name,
	crc32c_len,
	crc32c_crcadd,
	crc32c_crcchk,

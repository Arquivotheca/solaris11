...
static int ses_alloc_rqs(ses_softc_t *ssc);
static void ses_free_rqs(ses_softc_t *ssc);
...
ses_probe(dev_info_t *dip)
{
	 ...
	 *    uses the same target as raid controller.
...
is_enc_dev(ses_softc_t *ssc, struct scsi_inquiry *inqp, int iqlen, enctyp *ep)
{
	...
	if (inqp->inq_encserv && inqp->inq_rdf >= RDF_SCSI2) {
		/*
		 * PassThrough Device.
		 */
		*ep = SES_TYPE;
...
ses_open(dev_t *dev_p, int flag, int otyp, cred_t *cred_p)
{
	...
	/*
	 * If the device is suspended, fail open.
	 */
...
ses_uscsi_cmd(ses_softc_t *ssc, Uscmd *Uc, int Uf)
{
	...
	/*
	 * If the device is suspended, fail.
...
ses_get_pkt(struct buf *bp, int (*func)())
{
	...
	(void) scsi_uscsi_pktinit(scmd, pkt);
}
...
ses_callback(struct scsi_pkt *pkt)
{
	...
	(void) scsi_uscsi_pktfini(pkt, scmd);
...
CHECK_PKT:
	...
	case QUE_SENSE:
		...
		if (ssc->ses_retries > SES_NO_RETRY) {
			...
			/*
			 * Allocate the buf and pkt for request sense.
			 * Do not allocate, if it is retrying the
			 * Request Sense command.
			 */
			if (!request_sense) {
				if (ses_alloc_rqs(ssc) == 0) {
				SES_LOG(ssc, CE_NOTE,
				    "Allocate Request Sense pkt Failed");
					ses_free_rqs(ssc);
					break;
				}
			}
...
static int
ses_alloc_rqs(ses_softc_t *ssc)
{

	ASSERT(ssc->ses_rqbp == NULL);
	ASSERT(ssc->ses_rqpkt == NULL);

	ssc->ses_rqbp = scsi_alloc_consistent_buf(SES_ROUTE(ssc),
	    NULL, MAX_SENSE_LENGTH, B_READ, NULL_FUNC, NULL);
...
static void
ses_free_rqs(ses_softc_t *ssc)
{
	if (ssc->ses_rqpkt != NULL) {

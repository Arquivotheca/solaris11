...
#define	sd_wmr_taskq_threadnum		ssd_wmr_taskq_threadnum
...
#if defined(__fibre) || defined(__amd64)
...
#endif	/* __fibre || __amd64 */

#if (defined(__sparc) && !defined(__fibre)) || defined(__amd64)
...
#define	sd_process_flash_dev_table	ssd_process_flash_dev_table
...
#define	sd_check_arq			ssd_check_arq
...
static void sd_process_flash_dev_table(struct sd_lun *un);
...
static void sd_check_arq(struct sd_lun *un);
...
static int sd_fault_path_instance = 0;
...
/* Our use of B_PRIVATE buf.h b_flags flag */
#define	SD_B_ABSOLUTE		B_PRIVATE
...
/* Macro to check if the un is enumerated in MPXIO context. */
#define	SD_MPXIO_ENABLED(un) \
	(DEVI(SD_DEVINFO(un))->devi_mdi_client != NULL)
...
sd_set_properties(struct sd_lun *un, char *name, char *value)
{
	...
	if (strcasecmp(name, "emulation-rmw") == 0) {
		if (ddi_strtol(value, &endptr, 0, &val) == 0) {
			un->un_f_enable_rmw = val;
		} else {
			goto value_invalid;
		}
		SD_INFO(SD_LOG_ATTACH_DETACH, un, "sd_set_properties: "
		    "read modify write flag set to %d\n", un->un_f_enable_rmw);
	}
...
sdpower(dev_info_t *devi, int component, int level)
{
	...
	if (SD_PM_STOP_MOTOR_NEEDED(un, level) &&
		 ...
		 * the powercycle counts have gone beyond the threshold.
		 */
		log_page_size = START_STOP_CYCLE_COUNTER_PAGE_SIZE;
		log_page_data = kmem_zalloc(log_page_size, KM_SLEEP);
...
sd_unit_attach(dev_info_t *devi)
{
	...
	/*
	 * Check if the HBA can support auto-request sense
	 */
	sd_check_arq(un);
	...
	/*
	 * Enable RMW for the devices in sd_flash_dev_table
	 */
	sd_process_flash_dev_table(un);
...
sd_set_errstats(struct sd_lun *un)
{
	 ...
	 * (4376302))
	 * Set the "Serial No" kstat for non Sun qualified drives, using
	 * INQUIRY_SERIAL_NO properties.
	 */
	if (bcmp(&SD_INQUIRY(un)->inq_pid[9], "SUN", 3) == 0) {
		...
		if (ddi_prop_lookup_string(DDI_DEV_T_ANY, SD_DEVINFO(un),
		    DDI_PROP_NOTPROM | DDI_PROP_DONTPASS,
		    INQUIRY_SERIAL_NO, &inq_serial_no)
		    == DDI_PROP_SUCCESS) {
			/*
			 * Current limit of kstat_name.value.c is 16 bytes.
			 */
			if (strlen(inq_serial_no) < 16) {
				bcopy(inq_serial_no,
				    stp->sd_serial.value.c,
				    (strlen(inq_serial_no) + 1));
			}
			ddi_prop_free(inq_serial_no);
			inq_serial_no = NULL;
		}
	}
 ...
 *		state is changed to OFFLINE which can be used to suppress
...
sdclose(dev_t dev, int flag, int otyp, cred_t *cred_p)
{
	...
	if (cp == &un->un_ocmap.chkd[OCSIZE]) {
		...
		/*
		 * We avoid persistence upon the last close, and set
 ...
 *
 * Description: Test if device is ready and has a valid geometry. If
 * part is -1, then we're doing a SD_B_ABSOLUTE I/O, and only care about
 * readiness.
 *
...
sd_ready_and_valid(sd_ssc_t *ssc, int part)
{
	...
	if (un->un_f_has_removable_media) {
		...
		if (status != 0) {
			...
			/* Ignore all failed status for removable media */
			sd_ssc_assessment(ssc, SD_FMT_IGNORE);
	...
	if (un->un_f_format_in_progress == FALSE && part != -1) {
		mutex_exit(SD_MUTEX(un));
...
#define	SD_WMR_TASKQ_MINALLOC	1024
#define	SD_WMR_TASKQ_MAXALLOC	1024
...
static int	sd_wmr_taskq_minalloc = SD_WMR_TASKQ_MINALLOC;
static int	sd_wmr_taskq_maxalloc = SD_WMR_TASKQ_MAXALLOC;

/*
 * The following task queue is being created for the write part of
 * read-modify-write of non-512 block size devices.
 * Set it to 32 for the reason that now the consumer of this taskq
 * includes 4K sector size disk drive and SSDs in emulation mode which
 * may have higher IOPS.
 */
#define	SD_WMR_TASKQ_NUMTHREADS 8
static int	sd_wmr_taskq_threadnum = SD_WMR_TASKQ_NUMTHREADS;
 ...
 *		  (definitely NOT recommended 3rd-party drivers!) :-)
...
sd_taskq_create(void)
{
	...
	sd_wmr_tq = (taskq_create(taskq_name, sd_wmr_taskq_threadnum,
	    (v.v_maxsyspri - 2), sd_wmr_taskq_minalloc, sd_wmr_taskq_maxalloc,
	    TASKQ_PREPOPULATE));
}
 ...
 *              sd_ssc_fini.
 *
 *              The calling sequence will look like:
 *              sd_ssc_init->
 *
 *                  ...
 *
	...
	enum uio_seg dataspace, int path_flag)
{
	...
	/*
	 * Do not send an internally generated command to a removed device.
	 */
	if ((dataspace == UIO_SYSSPACE) &&
	    ndi_devi_device_isremoved(SD_DEVINFO(un))) {
		mutex_exit(SD_MUTEX(un));
		SD_TRACE(SD_LOG_IO, un, "sd_ssc_send: un:0x%p device is "
		    "removed, skipping internal IO\n", un);
		return (EIO);
	}
	mutex_exit(SD_MUTEX(un));
 ...
 *                 converted to physical sector address in
 *		   sd_mapblocksize_iostart.
 *              3. I/O is misaligned but is aligned in "overrun" buf
 *                 xp->xb_blkno represents the absolute logical block address
 *                 based on DEV_BSIZE. The logical block address will be
 *                 converted to physical sector address in
 *		   sd_mapblocksize_iostart. But no RMW will be issued in
 *		   this case.
 *
 *     Context: Can sleep
 *
...
sd_mapblockaddr_iostart(int index, struct sd_lun *un, struct buf *bp)
{
	...
	int	partition;
	diskaddr_t	part_nblocks = 0;
	diskaddr_t	part_offset = 0;
	struct sd_xbuf *xp;
	...
	/*
	 * Determine the partition from the dev_t, unless SD_B_ABSOLUTE
	 * is set (which implies an absolute device offset instead
	 * a partition offset).
	 */
	if (bp->b_flags & SD_B_ABSOLUTE)
		partition = -1;
	else
	...
	if (partition != -1) {
		if ((cmlb_partinfo(un->un_cmlbhandle, partition,
		    &part_nblocks, &part_offset, NULL, NULL,
		    (void *)SD_PATH_DIRECT)) != 0) {
			bp->b_flags |= B_ERROR;
			goto error_exit;
		}
	} else if (un->un_f_blockcount_is_valid) {
		part_nblocks = un->un_blockcount;
	}
	...
	if (!(NOT_DEVBSIZE(un)) || un->un_f_enable_rmw) {
		...
		if (is_aligned) {
			...
			part_nblocks = SD_TGT2SYSBLOCK(un, part_nblocks);
			part_offset = SD_TGT2SYSBLOCK(un, part_offset);
		}
	}
	 ...
	 */
	if ((blocknum < 0) || (blocknum >= part_nblocks) ||
	...
	/*
	 * If the requested # blocks exceeds the available # blocks, that
	...
	available_nblocks = (size_t)(part_nblocks - blocknum);
	ASSERT(part_nblocks >= blocknum);
...
sd_mapblocksize_iostart(int index, struct sd_lun *un, struct buf *bp)
{
	 ...
	 */
	if (!(NOT_DEVBSIZE(un))) {
		first_byte = SD_TGTBLOCKS2BYTES(un, (offset_t)xp->xb_blkno);
	} else {
...
sd_mapblocksize_iodone(int index, struct sd_lun *un, struct buf *bp)
{
	...
	if ((un->un_tgt_blocksize == DEV_BSIZE && !un->un_f_enable_rmw) ||
		...
		if ((bp->b_bcount == 0) &&
			...
			/*
			 * If bp->b_iodone is sd_mapblocksize_iodone, it
			 * means shadow buf has been allocated in
			 * sd_mapblocksize_iostart, so shadow buf
			 * needs to be freed.
			 */
			goto freebuf_done;
		}
	...
	if (has_wmap) {
		 ...
		 */
		rc = bp_copyin(orig_bp, bp->b_un.b_addr + copy_offset, 0,
		    copy_length);
		VERIFY(rc == 0);
		 ...
		 */
		rc = bp_copyout(bp->b_un.b_addr + copy_offset, orig_bp, 0,
		    copy_length);
		VERIFY(rc == 0);
	}
	...
	int code)
{
	 ...
	 *   received, and reset to zero whenever a TRAN_ACCEPT is returned
	 *   from scsi_transport().
	 *
	 * The FLAG_SILENT in the scsi_pkt must be CLEARED in ALL of
	 * the preceding cases in order for the message to be printed.
	 */
...
got_pkt:
		...
		case TRAN_BUSY:
			 ...
			 * unless we are retrying a command. And avoid
			 * registering the timeout to restore the throttle
			 * if the detach routine is running.
			 */
			mutex_enter(&sd_detach_mutex);
			if ((bp != un->un_retry_bp) &&
			    (un->un_detach_count == 0)) {
...
sd_return_command(struct sd_lun *un, struct buf *bp)
{
	...
	if (bp->b_error == 0) {
		...
		if (xp->xb_ena > 0) {
			...
			/* zero xb_ena after successful command */
			xp->xb_ena = 0;
		}
	} else {
		/*
	...
	int errcode)
{
	 ...
	 * failfast bp encountered a fatal error when we attempted to
	...
	void (*statp)(kstat_io_t *))
{
	...
	if (retry_check_flag & SD_RETRIES_FAILFAST) {
		if (un->un_failfast_state == SD_FAILFAST_ACTIVE) {
			...
			if (un->un_failfast_bp == NULL) {
				...
				/*
				 * If neither of the preceding conditionals
				 * was true, it means that there is some
				 * *other* bp that has met an initial failfast
	...
	if (retry_delay == 0) {
		...
		if (un->un_ncmds_in_transport >= un->un_throttle) {
			...
			/*
			 * To avoid retrying an i/o on the same physical path,
			 * we use the FLAG_PKT_PATH_INSTANCE_AVOID_LD flag with
			 * a scsi_pkt pkt_path_instance value of the
			 * pre-retry-path. This setup acts as a hint to
			 * scsi_vhci to select, if possible, a different
			 * retry-path (i.e. try to avoid the pkt_path_instance
			 * least-desired path).
			 *
			 * NOTE: For typical no-single-point-of-failure (NSPOF)
			 * configs, where each pHCI initiator-port ends up at a
			 * different device target-port, a simple least-desired
			 * path implementation in scsi_vhci results in the
			 * selection of a retry-path with maximal hardware
			 * separation from the least-desired pre-retry-path.
			 * For configs where multiple pHCI initiator-ports end
			 * up at the same device target-port, a complex
			 * implementation in scsi_vhci would be needed to select
			 * the best retry-path.
			 */
			if (!(pktp->pkt_flags & (FLAG_PKT_PATH_INSTANCE |
			    FLAG_PKT_PATH_INSTANCE_AVOID_LD)) &&
			    scsi_pkt_allocated_correctly(pktp) &&
			    pktp->pkt_path_instance &&
			    failure_code) {
				pktp->pkt_flags |=
				    FLAG_PKT_PATH_INSTANCE_AVOID_LD;
			}
	...
	void (*statp)(kstat_io_t *))
{
	...
	/*
	 * As described in sd_retry_command, we use
	 * FLAG_PKT_PATH_INSTANCE_AVOID_LD to get scsi_vhci to select a
	...
	if (!(pktp->pkt_flags & (FLAG_PKT_PATH_INSTANCE |
	    ...
	    scsi_pkt_allocated_correctly(pktp) &&
	    pktp->pkt_path_instance) {
		pktp->pkt_flags |= FLAG_PKT_PATH_INSTANCE_AVOID_LD;
	}
	...
	struct scsi_pkt *pktp)
{
	...
	/*
	 * Alloc the buf and pkt, and setup the cdb for the
	 * request sense command, if they are not initialized
	 * for this LUN.
	 */
	if (sd_alloc_rqs(un->un_sd, un) != DDI_SUCCESS) {
		SD_TRACE(SD_LOG_IO_CORE | SD_LOG_ERROR, un,
		    "sd_send_request_sense_command: "
		    "alloc request sense, exit\n");
		return;
	}
...
/*
 *    Function: sd_check_arq
 *
 * Description: Check if the HBA supports auto request sense data
 *
 *     Context: Called under attach(9E) context
 */

static void
sd_check_arq(struct sd_lun *un)
{
	ASSERT(un != NULL);
	ASSERT(!mutex_owned(SD_MUTEX(un)));
	...
	if (un->un_f_is_fibre == TRUE) {
		...
		case 0:
			...
			if (scsi_ifsetcap(SD_ADDRESS(un), "auto-rqsense", 1, 1)
				...
				SD_INFO(SD_LOG_ATTACH_DETACH, un,
				    "sd_check_arq: ARQ enabled\n");
				un->un_f_arq_enabled = TRUE;
			} else {
				/* Could not enable ARQ in the HBA */
				SD_INFO(SD_LOG_ATTACH_DETACH, un,
				    "sd_check_arq: failed ARQ enable\n");
		...
		case 1:
			 ...
			 */
			SD_INFO(SD_LOG_ATTACH_DETACH, un,
			    "sd_check_arq: ARQ already enabled\n");
			un->un_f_arq_enabled = TRUE;
			break;
		default:
			/*
			 ...
			 */
			SD_INFO(SD_LOG_ATTACH_DETACH, un,
			    "sd_check_arq: HBA does not support ARQ\n");
			un->un_f_arq_enabled = FALSE;
			break;
		}
	}
 ...
 * Return Code: DDI_SUCCESS or DDI_FAILURE
 *
 *    Context: Called while Request Sense is needed
 */
...
sd_alloc_rqs(struct scsi_device *devp, struct sd_lun *un)
{
	 ...
	 */
	xp = kmem_alloc(sizeof (struct sd_xbuf), KM_NOSLEEP);
	if (xp == NULL) {
		sd_free_rqs(un);
		return (DDI_FAILURE);
	}
...
sdintr(struct scsi_pkt *pktp)
{
	...
	/* SD FaultInjection */
	if (sd_fault_path_instance == -1) {
		sd_faultinjection(pktp);
	} else if (sd_fault_path_instance > 0) {
		if (scsi_pkt_allocated_correctly(pktp) &&
		    pktp->pkt_path_instance == sd_fault_path_instance) {
			sd_faultinjection(pktp);
		}
	}
...
not_successful:
	...
	default:
		SD_TRACE(SD_LOG_IO_CORE | SD_LOG_ERROR, un,
		    "sdintr: default\n");
		/*
		 * Mark the ssc_flags for detecting invalid pkt_reason.
		 */
		if (!(xp->xb_pkt_flags & SD_XB_USCSICMD)) {
	...
	struct sd_xbuf *xp, struct scsi_pkt *pktp)
{
	...
	/*
	 * See if we can retry the read/write, preferably immediately.
 ...
 *   Arguments: un - ptr to softstate
 *		comp - component mask
 *		title - "title" string to preced data when printed
	...
	struct buf *bp, struct sd_xbuf *xp, struct scsi_pkt *pktp)
{
	...
	case 0x04:	/* LOGICAL UNIT NOT READY */
		...
		case 0x88:
			 ...
			 * path problem in a multi-pathed config, but as far as
		 ...
		 * unnecessarily for the duration of the delay.
		 *
	...
	struct buf *bp, struct sd_xbuf *xp, struct scsi_pkt *pktp)
{
	...
	/* Do NOT do a RESET when it is MEDUIM ERROR. */
	...
	struct sd_xbuf *xp, struct scsi_pkt *pktp)
{
	...
	asc = scsi_sense_asc(xp->xb_sense_data);

	/*
	 * If the ASC: LOGICAL LUN NOT SUPPORTED is returned,
	 * this command should be retried on another path in case of
	 * MPXIO enabled.
	 */
	if ((asc == 0x25) && (SD_MPXIO_ENABLED(un))) {
		sd_retry_command(un, bp, SD_RETRIES_STANDARD,
		    sd_print_sense_msg, &si, EIO, (clock_t)0, NULL);
	} else {
		/*
		 * Pointless to retry if the target thinks
 ...
 *		lbap - ptr to unsigned 32-bit variable to receive the
	...
	uint32_t *lbap, uint32_t *psp, int path_flag)
{
	...
	struct	uscsi_cmd	ucmd_buf;
	union	scsi_cdb	cdb;
	struct	scsi_capacity_16	*capacity16_buf;
	uint64_t		capacity;
	uint32_t		lbasize;
	uint32_t		pbsize;
	uint32_t		lbpb_exp;
	...
	version = SD_INQUIRY(un)->inq_ansi;
	...
	case 0:
		 ...
		 */
		capacity = BE_64(capacity16_buf->sc_capacity);
		lbasize = BE_32(capacity16_buf->sc_lbasize);

		/*
		 * logical-blocks-per-physical-block-exponent can be
		 * extracted only when the target supports SBC-3.
		 */
		if (version >= 0x06) {
			lbpb_exp = capacity16_buf->sc_lb_p_pbe;
			pbsize = lbasize << lbpb_exp;
		} else {
			/*
			 * For versions earlier than SBC-3, physical block size
			 * can not be got from the logical-blocks-per-physical-
			 * exponent in the read capacity_16 data.
			 * Set physical block size as the default 512.
			 *
			 * If the device is native 4K, we assume the physical
			 * block size equals to target block size for HDD since
			 * there is no disk which its physical block size is
			 * less than its logical block size on the market.
			 */
			if ((!NOT_DEVBSIZE(un)) &&
			    (lbasize == un->un_tgt_blocksize) &&
			    (ISP2(un->un_tgt_blocksize / DEV_BSIZE))) {
				pbsize = un->un_tgt_blocksize;
			} else {
				pbsize = DEV_BSIZE;
			}
		}
...
sd_start_stop_unit_task(void *arg)
{
	...
	if (un->un_retry_bp != NULL) {
		xp = (struct sd_xbuf *)(un->un_retry_bp->b_private);
		if (!(xp->xb_pkt_flags & SD_XB_XFERRED))
			sd_start_cmds(un, un->un_retry_bp);
	} else {
		sd_start_cmds(un, un->un_retry_bp);
	}
 ...
 *		residp:  Pointer to variable to receive the residual of the
...
/*
 * This is an incomplete implementation, with all that's necessary for
 * format(1m): specifically, dadkio_status is not fully filled in.
 */
static int
sd_dioctl_rwcmd(struct sd_lun *un, dev_t dev, caddr_t arg, int flag)
{
	struct dadkio_rwcmd rwcmd;
	struct iovec aiov;
	struct uio auio;
	int rw, rval, status = 0;
	struct buf *buf;
	sd_ssc_t *ssc;

	if ((rval = cmlb_rwcmd_copyin(&rwcmd, arg, flag)) != 0)
		return (rval);
	...
	ssc = sd_ssc_init(un);
	rval = sd_ready_and_valid(ssc, -1);
	sd_ssc_fini(ssc);

	bzero(&rwcmd.status, sizeof (rwcmd.status));

	/*
	 * Note that for failures here, we're still returning zero for
	 * the ioctl() syscall itself.
	 */
	switch (rval) {
	case SD_READY_VALID:
		rwcmd.status.status = DADKIO_STAT_NO_ERROR;
		break;
	case SD_NOT_READY_VALID:
		rwcmd.status.status = DADKIO_STAT_NOT_READY;
		goto out;
	case SD_RESERVED_BY_OTHERS:
		rwcmd.status.status = DADKIO_STAT_RESERVED;
		goto out;
	}
	...
	auio.uio_loffset = rwcmd.blkaddr * un->un_tgt_blocksize;
	auio.uio_resid  = rwcmd.buflen;
	auio.uio_segflg = flag & FKIOCTL ? UIO_SYSSPACE : UIO_USERSPACE;

	/*
	 * Set SD_B_ABSOLUTE/B_PRIVATE in b_flags to indicate an absolute
	 * device offset and call physio to issue and wait for IO completion.
	 * NOTE: physio will preserve B_PRIVATE.
	 */
	buf = getrbuf(KM_SLEEP);
	buf->b_flags = SD_B_ABSOLUTE;
	status = physio(sdstrategy, buf, dev, rw, sdmin, &auio);
	freerbuf(buf);

out:
	if (status == 0)
		status = cmlb_rwcmd_copyout(&rwcmd, arg, flag);

	return (status);
}

static int
sd_dkio_getdiskid(struct sd_lun *un, caddr_t arg, int flag)
{
	struct dk_disk_id diskid;
	struct scsi_inquiry *inq = SD_INQUIRY(un);
	int err = 0;

	/* also ensure null termination */
	bzero(&diskid, sizeof (diskid));

	diskid.dkd_dtype = DKD_SCSI_TYPE;
	(void) strlcpy(diskid.disk_id.scsi_disk_id.dkd_svendor,
	    inq->inq_vid, DKD_SCSI_VENDOR);
	(void) strlcpy(diskid.disk_id.scsi_disk_id.dkd_sproduct,
	    inq->inq_pid, DKD_SCSI_PRODUCT);
	(void) strlcpy(diskid.disk_id.scsi_disk_id.dkd_sfwver,
	    inq->inq_revision, DKD_SCSI_REVLEVEL);
	(void) strlcpy(diskid.disk_id.scsi_disk_id.dkd_sserial,
	    inq->inq_serial, DKD_SCSI_SERIAL);

	if (ddi_copyout(&diskid, (void *)arg, sizeof (diskid), flag) != 0)
		err = EFAULT;
...
sdioctl(dev_t dev, int cmd, intptr_t arg, int flag, cred_t *cred_p, int *rval_p)
{
	...
	/*
	 * If the device has been marked removed, ensure that un_mediastate
	 * is updated as well.
	 */
	if (ndi_devi_device_isremoved(un->un_sd->sd_dev) &&
	    (un->un_mediastate != DKIO_DEV_GONE)) {
		un->un_mediastate = DKIO_DEV_GONE;
		cv_broadcast(&un->un_state_cv);
	}
	...
	err = cmlb_ioctl_secpolicy(dev, cred_p, cmd);
	if (err != 0) {
		switch (cmd) {
#ifdef SD_FAULT_INJECTION
		case SDIOCINSERTARQ:
...
skip_ready_valid:
	...
	switch (cmd) {
	case DIOCTL_RWCMD:
		SD_TRACE(SD_LOG_IOCTL, un, "DIOCTL_RWCMD\n");
		err = sd_dioctl_rwcmd(un, dev, (caddr_t)arg, flag);
		break;

	case DKIOC_GETDISKID:
		SD_TRACE(SD_LOG_IOCTL, un, "DKIOC_GETDISKID\n");
		err = sd_dkio_getdiskid(un, (caddr_t)arg, flag);
		break;
	...
	case CDROMSBLKMODE:
		...
		if (!ISCD(un) || (un->un_f_cfg_is_atapi == TRUE)) {
			 ...
			 * maintain consistency with the original mmc
			 * driver update.
			 */
			err = EINVAL;
		} else {
	...
	case CDROMSDRVSPEED:
		...
		if (!ISCD(un)) {
			 ...
			 *
			 * Note: EINVAL is returned for consistency of original
...
sd_check_media(dev_t dev, enum dkio_state state)
{
	...
	if (state == un->un_mediastate || un->un_mediastate == DKIO_NONE) {
		 ...
		 * we will not be signaled unless mediastate == state but it is
 ...
 *		(sd_check_media) is then signaled.
 *
...
sd_resv_reclaim_thread()
{
	...
	while ((sd_tr.srq_thr_cur_req = sd_tr.srq_thr_req_head) != NULL) {
		...
		SD_TRACE(SD_LOG_IOCTL_MHD, un,
		    "sd_resv_reclaim_thread: cv_signaling current request \n");
	}
...
sr_read_tocentry(dev_t dev, caddr_t data, int flag)
{
	...
	if ((entry->cdte_ctrl & CDROM_DATA_TRACK) &&
		...
		if (rval == 0) {
			 ...
			 * -1 for an invalid track so that we can still
    ...
    struct scsi_pkt *pkt, struct sd_xbuf *xp)
{
	 ...
	 *
	 * How much data NOT transferred to the HBA yet.
	 */
	bytes_not_xfered = xp->xb_dma_resid;

	/*
	 * figure how many blocks NOT transferred to the HBA yet.
	 */
	num_blks_not_xfered = SD_BYTES2TGTBLOCKS(un, bytes_not_xfered);

	/*
	 * set starting block number to the end of what WAS transferred.
	 */
 ...
 *		content based on error injection scenario.
 *
...
sd_faultinjection(struct scsi_pkt *pktp)
{
	...
	/*
	 * kmem_free does not guarantee to set to NULL
    ...
    struct buf *bp, struct sd_xbuf *xp)
{
	...
	/*
	 * When a single command produces multiple ereports, we
	 * use ENA to associate all the ereports with the same
	 * command.
	 *
	 * If xb_ena is zero then we are generating the first
	 * ereport, and we use the fm_ena_generate() result
	 * as the ereport ENA value.
	 *
	 * If xb_ena is non-zero then we are generating an
	 * additional ereport for the same command, and we use
	 * the fm_ena_increment result as the ereport ENA value.
	 */
	if (xp->xb_ena == 0)
		xp->xb_ena = fm_ena_generate(0, FM_ENA_FMT1);
	else
		xp->xb_ena = fm_ena_increment(xp->xb_ena);
...
sd_check_emulation_mode(sd_ssc_t *ssc)
{
	...
	/*
	 * In case the SSD in flash dev table can not
	 * report physical block size correctly, default
	 * the sector size to SSD_SECSIZE
	 */
	...
	/*
	 *  If disk is not in emulation mode, turn off
	 *  RMW for emulation mode disk.
	 */
	if (un->un_phy_blocksize == un->un_tgt_blocksize)
		un->un_f_enable_rmw = FALSE;
...
/*
 *          Function: sd_process_flash_dev_table
 *
 *       Description: Go through sd_flash_dev_table to
 *                    turn on RMW for devices in the table.
 *
 *           Context: Kernel thread or interrupt context.
 */
static void
sd_process_flash_dev_table(struct sd_lun *un)
{
	int i, devid_len;

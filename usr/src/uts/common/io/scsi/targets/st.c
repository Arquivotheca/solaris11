...
static int st_gen_mode_sense(struct scsi_tape *un, ubufunc_t ubf, uchar_t page,
    uchar_t subpage, struct seq_mode *page_data, int page_size);
...
static int st_read_block_limits(struct scsi_tape *un, ubufunc_t ubf,
    struct read_blklim *read_blk);
static void st_update_read_blk_limits_info(struct scsi_tape *un,
    struct read_blklim *rbl);
static void st_get_read_blk_limits(struct scsi_tape *un, buf_t *bp);
...
static uint64_t st_get_cdb_verify_cnt(uchar_t *cdb);
static int st_reset(struct scsi_tape *un, int reset_type);
static void st_reset_notification(caddr_t arg);
static void st_update_mode_log(struct scsi_tape *un, bufunc_t bf);
...
static int st_make_sure_mode_data_is_correct(struct scsi_tape *un,
    ubufunc_t ubf);
static int st_alloc_rqs(struct scsi_device *devp, struct scsi_tape *un);
static void st_free_rqs(struct scsi_tape *un);
static int st_set_dadp(struct scsi_tape *un, tape_dadp_state new_mode);
static int st_set_dadp_support(struct scsi_tape *un);
...
st_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	 ...
	 */
	mutex_enter(ST_MUTEX);
	un->un_dev = makedevice(ddi_driver_major(devi), MTMINOR(instance));
	st_known_tape_type(un);

	(void) st_set_dadp_support(un);
	...
	for (node_ix = 0; node_ix < ST_NUM_MEMBERS(st_minor_data); node_ix++) {
		...
		if (ddi_create_minor_node(devi, name, S_IFCHR, minor,
			...
			/* device doesn't support DADP */
			if (un->un_sd->sd_inq->inq_protect == 0)
				continue; /* Happy path continue adding nodes */

			(void) sprintf(dadp_name, "d%s", name);

			/* Device doesn't have a entry in st.conf */
			if (un->un_dadp.dps.dp_method == 0) {
				/* Remove any previous data protect nodes */
				ddi_remove_minor_node(devi, dadp_name);
				continue; /* Happy path continue adding nodes */
			}
			minor |= MT_DADP;
			name = dadp_name;
		...
		/* Sad path, creating any minor node failed free every thing. */
		...
		if (un->un_mode_data) {
			kmem_free(un->un_mode_data, un->un_mode_data_len);
			un->un_mode_data = NULL;
			un->un_mode_data_len = 0;
		}
		if (un->un_mspl) {
		...
		if (devp->sd_sense != NULL) {
			kmem_free(devp->sd_sense, MAX_SENSE_LENGTH);
			devp->sd_sense = NULL;
		}
...
st_doattach(struct scsi_device *devp, int (*canwait)())
{
	...
	un->un_rqs_bp = NULL;
	un->un_rqs = NULL;
	devp->sd_sense = (struct scsi_extended_sense *)
	    kmem_zalloc(MAX_SENSE_LENGTH, KM_SLEEP);
	if (devp->sd_sense == NULL) {
		goto error;
	}
    ...
    struct st_drivetype *dp)
{
	...
	rval = st_read_block_limits(un, st_uscsi_cmd, blklim);
	if (rval) {
		ST_DEBUG2(ST_DEVINFO, st_label, SCSI_DEBUG,
		    "st_get_conf_from_tape_drive(): "
...
st_tape_init(struct scsi_tape *un)
{
	...
	if (un->un_sd->sd_inq->inq_protect != 0) {
		tape_dadp_state newstate;
		/*
		 * If opened in DADP mode enable, otherwise disable.
		 */
		if (un->un_dadp.dps.dp_method == 0)
			newstate = DADP_DISABLE;
		else if (getminor(un->un_dev) & MT_DADP)
			newstate = DADP_RW_ENABLE;
		else
			newstate = DADP_DISABLE;

		rval = st_set_dadp(un, newstate);
	...
	if (un->un_maxbsize == MAXBSIZE_UNKNOWN) {
		...
		if (!err) {
			 ...
			 */
			st_update_read_blk_limits_info(un, un->un_rbl);
			if ((un->un_maxbsize == 0) ||
	...
	if (err != 0) {
		...
		if (un->un_status != KEY_UNIT_ATTENTION) {
			...
			if (un->un_oflags & (FNONBLOCK|FNDELAY)) {
				if (un->un_mediastate != MTIO_UNTHREADED) {
...
st_close(dev_t dev, int flag, int otyp, cred_t *cred_p)
{
	...
	if (un->un_sd->sd_inq->inq_protect != 0)
		(void) st_set_dadp(un, DADP_DISABLE);
...
st_rw(dev_t dev, struct uio *uio, int flag)
{
	...
	len = uio->uio_resid;
	loc_uio.uio_iov = NULL;

	mutex_enter(ST_MUTEX);

	/*
	 * Check to see if DADP is enabled of this I/O.
	 */
	if (((flag == B_WRITE) && (un->un_dadp_state & DADP_WT_ENABLE)) ||
	    ((flag == B_READ) && (un->un_dadp_state & DADP_RD_ENABLE))) {
		dp_sz = un->un_dadp.dpf->dp_len(&un->un_dadp.dps, len);
		if (un->un_bsize)
			nblks = len / (un->un_bsize - dp_sz);
		else
			nblks = 1;
	} else {
		dp_sz = 0;
	...
	/* More than 1 vector with more than 1 block seems undoable. */
	if (uio->uio_iovcnt > 1 && nblks > 1) {
		scsi_log(ST_DEVINFO, st_label, CE_WARN,
		    "Fixed block mode does not support more than one vector");
		mutex_exit(ST_MUTEX);
		return (EINVAL);
	}

	/*
	 * If DADP is enabled and/or the caller sent more than one vector,
	 * copy all vectors into a single buffer to emulate real scatter-
	 * gather.
	 */
	if ((dp_sz > 0) || (uio->uio_iovcnt > 1)) {

		ASSERT(nblks);

		bzero(&loc_vec, sizeof (loc_vec));
		bzero(&loc_uio, sizeof (loc_uio));

		loc_vec.iov_len = len + (dp_sz * nblks);
		buf = kmem_zalloc(loc_vec.iov_len, KM_SLEEP);
		loc_vec.iov_base = buf;

		/*
		 * Writing one block that can have one or more vector,
		 * with or without data protection.
		 */
		if (flag == B_WRITE && (dp_sz == 0 || nblks == 1)) {
			rval = uiomove(buf, len, UIO_WRITE, uio);
			if (rval == 0 && dp_sz) {
				un->un_dadp.dpf->dp_add(&un->un_dadp.dps, buf,
				    len);
			}

		/*
		 * Writing more than one block that can have only one vector,
		 ...
		 */
			...
			char *dest;
			char *source;
			int bsize = un->un_bsize - dp_sz;

			source = uio->uio_iov->iov_base;

			/* One block at a time */
			for (i = 0, dest = buf; i < nblks; i++) {

				/* Copy write data in */
				mutex_exit(ST_MUTEX);
				rval = ddi_copyin(source, dest, bsize,
				    uio->uio_segflg);
				mutex_enter(ST_MUTEX);
				if (rval) {
					scsi_log(ST_DEVINFO, st_label, CE_WARN,
					    "Failed to copyin data[%d]", i);
					rval = EFAULT;
					break;
				}
				/* Add CRC */
				un->un_dadp.dpf->dp_add(&un->un_dadp.dps, dest,
				    bsize);
				source += bsize;	/* user block size */
				dest += un->un_bsize;	/* dadp block size */
			}
		} else {
			/* Should have handled all the write cases */
			ASSERT(flag == B_READ);
		}

		if (rval == 0) {
			loc_uio.uio_iov = &loc_vec;
			loc_uio.uio_iovcnt = 1;
			loc_uio.uio_segflg = UIO_SYSSPACE;
			loc_uio.uio_fmode = uio->uio_fmode;
			loc_uio.uio_extflg = uio->uio_extflg;
			loc_uio._uio_limit = uio->_uio_limit;
			loc_uio.uio_resid = loc_vec.iov_len;
		}
		/* allows freeing if it failed */
		len = loc_vec.iov_len;
		uiop = &loc_uio;
	}
	...
	rval = physio(st_queued_strategy, NULL, dev, flag, st_minphys, uiop);

	mutex_enter(ST_MUTEX);
	if (uiop == &loc_uio) {
		int amount = len - uiop->uio_resid;

		if (amount <= 0 || rval != 0) {
			/*
			 * Read of zero is valid with no error
			 * as in a filemark. No data read, no
			 * data to send to the caller.
			 */
			/* EMPTY */;
		} else if (flag == B_READ && (dp_sz == 0 || nblks == 1)) {
			/*
			 * If read was good copy read data to callers uio struct
			 */
			amount -= dp_sz;
			if (amount) {
				rval = uiomove(buf, amount, UIO_READ, uio);
			}
			if (rval != 0) {
				scsi_log(ST_DEVINFO, st_label, CE_WARN,
				    "Failed to uiomove read data");
			}
		} else if (flag == B_READ) {
			int i;
			char *source;
			char *dest = uio->uio_iov->iov_base;
			int bsize = un->un_bsize - dp_sz;


			/* One block at a time */
			for (i = 0, source = buf; i < nblks; i++) {

				/* Copy read data out */
				mutex_exit(ST_MUTEX);
				rval = ddi_copyout(source, dest, bsize,
				    uio->uio_segflg);
				mutex_enter(ST_MUTEX);
				if (rval) {
					scsi_log(ST_DEVINFO, st_label, CE_WARN,
					    "Failed to copyout data[%d]", i);
					rval = EFAULT;
					break;
				}
				dest += bsize;		/* user block size */
				source += un->un_bsize;	/* dadp block size */
				uio->uio_resid -= bsize;
			}
		} else {
			/* Should have handled all the read cases. */
			ASSERT(flag == B_WRITE);
			if (nblks > 1) {
				int actblks = amount / (un->un_bsize - dp_sz);
				if (actblks == nblks)
					uio->uio_resid = 0;
				else
					uio->uio_resid =
	...
	if (un->un_pos.eof > ST_NO_EOF) {
		ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
		    "eof=%d resid=%lx\n", un->un_pos.eof, uiop->uio_resid);
	}
	if (un->un_pos.eof >= ST_EOM && (flag == B_WRITE)) {
		if ((uiop->uio_resid != len) && (uiop->uio_resid != 0)) {
			un->un_pos.eof = ST_EOM;
		} else if (uiop->uio_resid == len) {
			un->un_pos.eof = ST_NO_EOF;
		}
	}

	if (un->un_silent_skip && uiop->uio_resid != len) {
	...
	case MTIOCRELEASE:
		{
			 ...
			 * persistent reservation we now need to drop
...
st_do_mtioctop(struct scsi_tape *un, struct mtlop *mtop)
{
	...
	case MTSRSZ:
		...
		/*
		 * If they are setting a fixed block size after enabling DADP
		 * there will CRC bytes that they don't know about.
		 */
		if ((mtop->mt_count) && (un->un_dadp_state & DADP_RW_ENABLE)) {
			mtop->mt_count +=
			    un->un_dadp.dpf->dp_len(&un->un_dadp.dps,
			    mtop->mt_count);
		}
	...
	case MTDADP:
		if (un->un_dadp.dps.dp_method != 0)
			rval = st_set_dadp(un, mtop->mt_count);
		else
			rval = ENOTTY;
		if (rval)
			ST_DEBUG2(ST_DEVINFO, st_label, CE_WARN,
			    "Attempt to set DADP to 0x%X returned 0x%X\n",
			    (unsigned)mtop->mt_count, rval);
		break;

	case MTVERIFY:
	{
		int64_t cnt = mtop->mt_count;

		/*
		 * If variable block size and DADP enabled add CRC size to
		 * requested bytes.
		 */
		if ((un->un_bsize == 0) &&
		    (un->un_dadp_state & DADP_RD_ENABLE)) {
			cnt += un->un_dadp.dpf->dp_len(&un->un_dadp.dps, cnt);
		}
		rval = st_cmd(un, SCMD_VERIFY_G0, cnt, SYNC_CMD);
		mtop->mt_count = un->un_err_resid;
		if (un->un_bsize != 0) {
			mtop->mt_count /=  un->un_bsize;
		}
...
exit_free:
	...
	if (uscmd->uscsi_cdb != NULL) {
		...
		if (((uscmd->uscsi_cdb[0] == SCMD_SPACE) ||
		    (uscmd->uscsi_cdb[0] == (char)SCMD_SPACE_G4) ||
		    (uscmd->uscsi_cdb[0] == SCMD_LOCATE) ||
		    (uscmd->uscsi_cdb[0] == (char)SCMD_LOCATE_G4)) &&
		    (un->un_pos.pmode != invalid)) {
			un->un_running.pmode = invalid;
		...
		/*
		 * After any mode select that might change our mode data
		 * read all the mode pages in.
		 */
		if ((uscmd->uscsi_cdb[0] == (char)SCMD_MODE_SELECT) ||
		    (uscmd->uscsi_cdb[0] == (char)SCMD_MODE_SELECT_G1)) {
			st_update_mode_log(un, st_cmd);
		}
		/*
		 * When a MTLOAD is done st_tape_init() gets called.
		 * So when a uscsi load command is completed this will do it.
		 */
		if ((uscmd->uscsi_cdb[0] == SCMD_LOAD) && (err == 0) &&
		    ((uscmd->uscsi_cdb[4] & (LD_LOAD | LD_HOLD)) == LD_LOAD)) {
			if (st_tape_init(un) == 0) {
				un->un_laststate = un->un_state;
				offline_state = 0;
			}
		}
...
st_cmd(struct scsi_tape *un, int com, int64_t count, int wait)
{
	...
	if ((com == (char)SCMD_MODE_SELECT) ||
	    (com == (char)SCMD_MODE_SELECT_G1)) {
		st_update_mode_log(un, st_cmd);
	}
...
st_setup_cmd(struct scsi_tape *un, buf_t *bp, int com, int64_t count)
{
	...
	case SCMD_MODE_SENSE:
		...
		if (count == MSIZE)
			...
			bp->b_un.b_addr = (caddr_t)(un->un_mode_data);
		break;
	...
	case SCMD_MODE_SELECT:
		ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
		    "mode select(6)\n");
		if (count != MSIZE) {
			un->un_mode_data->six.data_len = 0;
			un->un_mode_data->six.media_type = 0;
			un->un_mode_data->six.wp = 0;
			bp->b_un.b_addr = (caddr_t)(un->un_mode_data);
		} else {
			bp->b_un.b_addr = (caddr_t)(un->un_mspl);
		}
	...
	case SCMD_MODE_SELECT_G1:
		ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
		    "mode select(10)\n");
		/* Clear fields that are valid for mode sense only */
		un->un_mode_data->ten.data_len[0] = 0;
		un->un_mode_data->ten.data_len[1] = 0;
		un->un_mode_data->ten.media_type = 0;
		un->un_mode_data->ten.wp = 0;
		bp->b_bcount = count;
		bp->b_un.b_addr = (caddr_t)(un->un_mode_data);
		break;
	...
	case SCMD_VERIFY_G0:
		ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG, "Verify tape\n");
		bp->b_bcount = count;
		count = 0;
		break;
...
st_set_devconfig_page(struct scsi_tape *un, int compression_on)
{
	...
	if (un->un_mspl->page_code != ST_DEV_CONFIG_PAGE) {
		rval = st_gen_mode_sense(un, st_uscsi_cmd, ST_DEV_CONFIG_PAGE,
		    0, un->un_mspl, sizeof (struct seq_mode));
		if (rval)
			return (rval);
	}
...
st_modesense(struct scsi_tape *un)
{
	...
	case ST_DEV_CONFIG_PAGE: /* FALLTHROUGH */
		rval = st_gen_mode_sense(un, st_uscsi_cmd, page, 0, un->un_mspl,
		    sizeof (struct seq_mode));
		break;
	...
	case ST_DEV_DATACOMP_PAGE | ST_DEV_CONFIG_PAGE:
		if (un->un_dp->options & ST_MODE_SEL_COMP) {
			...
			rval = st_gen_mode_sense(un, st_uscsi_cmd, page, 0,
 ...
 */
static int
st_gen_mode_sense(struct scsi_tape *un, ubufunc_t ubf, uchar_t page,
    uchar_t subpage, struct seq_mode *page_data, int page_size)
{
	int r;
	char	cdb[CDB_GROUP1];
	...
	/* If reading all pages always use 10 byte mode sense command. */
	if ((page_size > 0xf0 || (page & 0x3f) == 0x3f) &&
	    (un->un_mode_data_cmd == SCMD_MODE_SENSE_G1)) {
		cdb[0] = SCMD_MODE_SENSE_G1;
		cdb[2] = (char)page;
		cdb[3] = (char)subpage;
		cdb[7] = (char)(page_size >> 8);
		cdb[8] = (char)(page_size >> 0);
...
static int
st_read_block_limits(struct scsi_tape *un, ubufunc_t ubf,
    struct read_blklim *read_blk)
{
	int rval;
	char cdb[CDB_GROUP0];
...
static void
st_update_read_blk_limits_info(struct scsi_tape *un, struct read_blklim *rbl)
{
	un->un_maxbsize =
	    (rbl->max_hi << 16) +
	    (rbl->max_mid << 8) +
	...
	un->un_minbsize =
	    (rbl->min_hi << 8) +
	    rbl->min_lo;
	un->un_data_mod = 1 << rbl->granularity;
}

static void
st_get_read_blk_limits(struct scsi_tape *un, buf_t *bp)
{
	size_t d_sz;
	struct uscsi_cmd *cmd = (struct uscsi_cmd *)bp->b_back;

	ST_FUNC(ST_DEVINFO, st_get_read_blk_limits);
	...
	if (bp_mapin_common(bp, VM_NOSLEEP) == NULL) {
		...
		scsi_log(ST_DEVINFO, st_label, SCSI_DEBUG,
		    "bp_mapin_common(Read Blk Limits) failed");

		return;
	}

	d_sz = bp->b_bcount - bp->b_resid;
	if (d_sz == sizeof (struct read_blklim)) {
		bcopy(bp->b_un.b_addr, un->un_rbl, d_sz);
		st_update_read_blk_limits_info(un, un->un_rbl);
	}
}
...
st_change_block_size(struct scsi_tape *un, uint32_t nblksz)
{
	...
	rval = st_gen_mode_sense(un, st_uscsi_cmd, 0, 0, current, MSIZE);
	if (rval != 0) {
		scsi_log(ST_DEVINFO, st_label, SCSI_DEBUG,
...
st_make_cmd(struct scsi_tape *un, struct buf *bp, int (*func)(caddr_t))
{
	...
	if ((bp != un->un_sbufp) && (bp != un->un_recov_buf)) {
		...
		case SCMD_MODE_SENSE_G1:
			...
			allocbp = bp;
			fixbit = 0;
			/* read all pages NOT in subpage format */
			address = 0x3f000000;
			cdb_len = CDB_GROUP1;
			tval = un->un_dp->non_motion_timeout;
			un->un_lastop = ST_OP_CTL;
...
st_intr(struct scsi_pkt *pkt)
{
	 ...
	 */
		...
		request_sense = 1;
		scsi_sync_pkt(pkt);
		bcopy(un->un_rqs_bp->b_un.b_addr, ST_RQSENSE, MAX_SENSE_LENGTH);
...
again:
	...
	case COMMAND_DONE_ERROR_RECOVERED:
		...
		/*FALLTHROUGH*/
	case COMMAND_DONE:
		if ((un->un_dadp_state & DADP_RD_ENABLE) &&	/* DADP read */
		    (rcv->cmd_attrib->transfers_data == TRAN_READ) &&
		    (rcv->cmd_attrib->chg_tape_pos) &&		/* from tape */
		    (bp->b_bcount - bp->b_resid > 0)) {
			if (rcv->pkt_retry_cnt++ >= st_retry_count) {
				action = COMMAND_DONE_ERROR;
				if (st_error_level >= SCSI_ERR_FATAL)
					goto again; /* Don't display message */
				scsi_log(ST_DEVINFO, st_label, CE_NOTE,
				    "Read Data Protection checksum error:"
				    " Retries exhausted");
				goto again;
			}
			if (st_error_level < SCSI_ERR_RETRYABLE)
				scsi_log(ST_DEVINFO, st_label, CE_NOTE, "Read "
				    "Data Protection checksum error: Retrying");
			bp_mapin(bp);
			if (un->un_bsize == 0) {
				if (un->un_dadp.dpf->dp_chk(&un->un_dadp.dps,
				    bp->b_un.b_addr,
				    bp->b_bcount - bp->b_resid) != B_TRUE) {
					/*
					 * Read may have worked but the CRC
					 * check might fail. Setting no data
					 * tranfered should cause reread.
					 */
				...
				int i;
				char *data = bp->b_un.b_addr;
				int nblks = (bp->b_bcount - bp->b_resid) /
				    un->un_bsize;

				/*
				 * Fixed block may have more than 1 block.
				 */
				for (i = 0; i < nblks; i++) {
					if (un->un_dadp.dpf->dp_chk(
					    &un->un_dadp.dps, data,
					    un->un_bsize) != B_TRUE) {
						/* See above comment no data */
						pkt->pkt_state &=
						    ~STATE_XFERRED_DATA;
						action = ATTEMPT_RETRY;
						goto again;
	...
	case QUE_SENSE:
		...
		/* Allocate the Request Sense pkt */
		if ((un->un_rqs_bp == NULL) && (un->un_rqs == NULL)) {
			if (st_alloc_rqs(un->un_sd, un) != DDI_SUCCESS) {
				goto sense_error;
			}
		}
...
last_command_error:
	...
	/* Free the Request Sense resource */
	if ((request_sense == 1) && (un->un_rqs_bp != NULL)) {
		st_free_rqs(un);
	}
    ...
    struct scsi_arq_status *statusp, tapepos_t *pos)
{
	...
	if ((attrib->transfers_data != TRAN_NONE) &&	/* Reads or Writes */
	    (attrib->chg_tape_pos != 0) &&		/* data from/to tape */
	    (bp != un->un_recov_buf)) {			/* not for recovery */
		if (sensep->es_valid) {
			resid =
			    (sensep->es_info_1 << 24) |
			    (sensep->es_info_2 << 16) |
		...
		ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
		    ...
		    (sensep->es_info_4),
		    pkt->pkt_resid);
	} else if ((attrib->transfers_data == TRAN_NONE) &&	/* no data */
	    (attrib->chg_tape_pos != 0) &&			/* moves tape */
	    (sensep->es_valid)) {

		/*
		 * If the command has done tape positioning, we need to get the
		...
		resid =
		    ...
		    (sensep->es_info_4);
		ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
		    "st_decode_sense(positioning): resid=%ld\n", resid);
	} else {
		/*
		 * If the special command is non-media read/write,
		 * non-positioning.
		 */
	...
	if (resid) {
		 ...
		 * If no pkt resid set what we got.
		 */
		if (pkt->pkt_resid == 0)
			pkt->pkt_resid = resid;
		bp->b_resid = resid;
...
common:
		...
		if (sensep->es_filmk && !sensep->es_eom) {
			...
			if (un->un_bsize == 0 && resid > 0) {
				...
				/*
				 * Fixed block mode with a non-negitive resid.
				 * Totally valid.
				 */
				pkt->pkt_resid = resid;
				un->un_running.pmode = invalid;
...
check_keys:
	...
	case KEY_HARDWARE_ERROR:
		/* This is the drive detected DADP error */
		if ((sensep->es_class == CLASS_EXTENDED_SENSE) &&
		    (sensep->es_code == ST_DEFERRED_ERROR) &&
		    (sensep->es_add_code == 0x10) &&	/* Drive Reported */
		    (sensep->es_qual_code == 1)) {	/* DADP CRC ERROR */
			/* Check retries against limit */
			if (ri->pkt_retry_cnt++ >= st_retry_count) {
				rval = COMMAND_DONE_ERROR;
				severity = SCSI_ERR_FATAL;
				if (st_error_level >= SCSI_ERR_FATAL)
					break; /* Don't log error message */
				scsi_log(ST_DEVINFO, st_label, CE_NOTE, "%s "
				    "Data Protection checksum error: Retrying",
				    ri->cmd_attrib->chg_tape_data ? "Write" :
				    "Read");
				break;
			}
			if (st_error_level < SCSI_ERR_RETRYABLE) {
	...
	case KEY_NOT_READY:
		...
		if (ST_RQSENSE->es_add_code == 0x3a) {
			...
			if (ST_RQSENSE->es_qual_code == 0x04)
				un->un_mediastate = MTIO_UNTHREADED;
			else
				un->un_mediastate = MTIO_EJECTED;
			cv_broadcast(&un->un_state_cv);
		}
...
st_calc_bnum(struct scsi_tape *un, struct buf *bp, struct scsi_pkt *pkt)
{
	...
	if (attrib->transfers_data != TRAN_NONE) {
		if (attrib->transfers_data == SUDO_READ) {

			/*
			 * Above b_bcount has no value due to no data transfer.
			 */
			count = st_get_cdb_verify_cnt(pkt->pkt_cdbp);
			if (un->un_bsize) {
			...
			if (pkt->pkt_resid) {
				ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
				    "SUDO_READ count 0x%"PRIX64" resid 0x%lX",
				    count, pkt->pkt_resid);
				count -= pkt->pkt_resid;
			}
		}
		if (count == 0) {
			...
			/*
			 * So its possable to read some blocks and a filemark.
			 * Example reading in fixed block mode where more than
			 * one block at a time is requested. In this case
			 * because the filemark is hit something less then the
			 * requesed number of blocks is read.
			 */
			if (un->un_pos.eof == ST_EOF_PENDING && bp->b_resid) {
				ST_DEBUG6(ST_DEVINFO, st_label, SCSI_DEBUG,
				    "counting filemark line %d\n", __LINE__);
				nfiles = 1;
			} else {
				nfiles = 0;
			}
...
st_set_state(struct scsi_tape *un, struct buf *bp)
{
	...
	if (ri->privatelen == sizeof (recov_info)) {
		...
		attrib = st_lookup_cmd_attribute(sp->pkt_cdbp[0]);
	}
	...
	if (bp != un->un_sbufp) {
		...
		case SCMD_WRITE_FILE_MARK:
		{
			...
			fmdone = (attrib->get_cnt(sp->pkt_cdbp) - bp->b_resid);
			if (fmdone > 0) {
		...
		case SCMD_LOAD:
		{
			...
			type = attrib->get_cnt(sp->pkt_cdbp);

			/* filter for valid load types */
			type &= LD_LOAD | LD_RETEN | LD_EOT | LD_HOLD;

			switch (type & (LD_LOAD | LD_HOLD)) {
			case LD_UNLOAD | LD_HOLD:
				un->un_mediastate = MTIO_UNTHREADED;
				un->un_state = ST_STATE_OFFLINE;
				un->un_pos.pmode = invalid;
				break;
			...
			/*
			 * If we are we loading or unloading expect the
			 * media id to change.
			 * Setting unit attention allows re-read of id.
			 */
...
st_is_sony_worm(struct scsi_tape *un)
{
	...
	result = st_gen_mode_sense(un, st_uscsi_cmd, AIT_DEV_PAGE, 0,
...
st_space_records(struct scsi_tape *un, int64_t count)
{
	...
	if (un->un_pos.pmode == logical) {
		...
		if (rval != 0 || un->un_sbufp->b_resid != 0 ||
		    un->un_status == SUN_KEY_EOF ||
		    un->un_status == SUN_KEY_BOT) {
			rval = EIO;
		}
	...
	if (un->un_pos.blkno < dblk || (un->un_dp->options & ST_BSR)) {
		...
		} else if (un->un_status == SUN_KEY_BOT) {
			rval = EIO;
		}
	} else {
		/*
...
st_recover(void *arg)
{
	...
	case DEVICE_TAMPER:
		...
		/*
		 * Media change could result in block limits changing.
		 */
		(void) st_rcmd(un, SCMD_READ_BLKLIM, RBLSIZE, SYNC_CMD);
		/*
	 ...
	 */
		...
		ASSERT(errinfo->ei_failed_pkt.pkt_cdbp[0] ==
		    SCMD_REQUEST_SENSE);
		status = COMMAND_DONE_ERROR;
	}
    ...
    struct scsi_pkt *pkt)
{
	...
	if (rinfo->cmd_attrib->chg_tape_pos) {
		 ...
		 * except if doing a rewind to establish a valid position
		 * or doing an unload where nobody cares what the position is.
		 * assumption here is that since the state says the media is
		 * inserted the load command is doing a unload. A load on a
		 * loaded cartridge does nothing.
		 */
		if ((un->un_mediastate == MTIO_INSERTED) &&
		    (pkt->pkt_cdbp[0] != SCMD_REWIND) &&
		    (pkt->pkt_cdbp[0] != SCMD_LOAD)) {
			ASSERT(rinfo->pos.pmode != invalid);
		}
...
st_make_sure_mode_data_is_correct(struct scsi_tape *un, ubufunc_t ubf)
{
	...
	if (rval) {
		...
		if (un->un_mode_data_cmd == SCMD_MODE_SENSE_G1)
			cmd = SCMD_MODE_SELECT_G1;
		else
			cmd = SCMD_MODE_SELECT;

		if (ubf == st_uscsi_cmd)
			bf = st_cmd;
		else if (ubf == st_uscsi_rcmd)
			bf = st_rcmd;
		else {
			ASSERT(0);
			return (EINVAL);
		...
		rval = bf(un, cmd, un->un_mode_data_len, SYNC_CMD);
	}
...
{
	mode_parameter_header *current;
	mode_parameter_header *changeable;
	int rval;
	int size;
	int i;
	caddr_t this;
	...
	current = kmem_zalloc(size, KM_SLEEP);

	rval = st_gen_mode_sense(un, ubf, 0x3f, 0,
	    (struct seq_mode *)current, size);
	if (rval != 0) {
		ST_RECOV(ST_DEVINFO, st_label, CE_NOTE,
		    "Mode Sense for mode verification failed");
		kmem_free(current, size);
		return (rval);
	}

	changeable = kmem_zalloc(size, KM_SLEEP);

	/*
	 * there is no point in flagging a change in mode sense data that
	 * is read only and therefor NOT changeable.
	 */
	rval = st_gen_mode_sense(un, ubf, 0x7f, 0,
	    (struct seq_mode *)changeable, size);
	if (rval != 0) {
		ST_RECOV(ST_DEVINFO, st_label, CE_NOTE,
		    "Mode Sense changable for mode verification failed");
		kmem_free(changeable, size);
		changeable = NULL;
	}

	this = (caddr_t)current;
	that = (caddr_t)un->un_mode_data;
	mask = (caddr_t)changeable;

	rval = bcmp(this, that, min(size, un->un_mode_data_len));
	if (rval == 0) {
		ST_RECOV(ST_DEVINFO, st_label, CE_NOTE,
		    "Found no changes in mode data");
		if (size != un->un_mode_data_len)
			rval = 1;	/* Force update of un_mode_data */
	} else {
		for (i = 1; i < min(un->un_mode_data_len, size); i++) {
			if (this[i] != that[i]) {
				if (mask == NULL) {
#ifdef STDEBUG
					ST_RECOV(ST_DEVINFO, st_label, CE_CONT,
					    "sense data changed at byte %d was "
					    "0x%x now 0x%x with NO mask", i,
					    (uchar_t)that[i], (uchar_t)this[i]);
#endif
				} else if ((mask[i] & this[i]) !=
				    (mask[i] & that[i])) {
#ifdef STDEBUG
					ST_RECOV(ST_DEVINFO, st_label, CE_CONT,
					    "sense data changed at byte %d was "
					    "0x%x now 0x%x with mask 0x%x", i,
					    (uchar_t)that[i], (uchar_t)this[i],
					    (uchar_t)mask[i]);
#endif
				} else {
					ST_RECOV(ST_DEVINFO, st_label, CE_CONT,
					    "DON'T CARE sense data change byte "
					    " %d was 0x%x now 0x%x with mask "
					    "0x%x", i, (uchar_t)that[i],
					    (uchar_t)this[i], (uchar_t)mask[i]);
					that[i] &= ~mask[i];
					that[i] |= (~mask[i] & this[i]);
					ST_RECOV(ST_DEVINFO, st_label, CE_CONT,
					    "DON'T CARE sense data after "
					    "changed byte %d is 0x%x was "
					    "0x%x with mask 0x%x", i,
					    (uchar_t)that[i], (uchar_t)this[i],
					    (uchar_t)mask[i]);
				}
			}
	...
	if (cmd_att->recov_pos_type == POS_EXPECTED) {
		...
		if (cmd_att->chg_tape_direction == DIR_FORW) {
			 ...
			 */
			    ...
			    (cmd_att->transfers_data == SUDO_READ)) &&
...
/*
 * Special get count for verify command.
 * Needed since no data is transfered and b_bcount is zero.
 * In this case count returned is in the same unit (bytes/Blocks) as resid
 * so count calculations are correct.
 */
static uint64_t
st_get_cdb_verify_cnt(uchar_t *cdb)
{
	uint64_t count;
...
{
	return ((uint64_t)(cdb[4] | (LD_HOLD << 8)));
}
...
static uint64_t
st_get_cdb_g4g5_lba(uchar_t *cdb)
{
	uint64_t lba;
...
static const cmd_attribute cmd_attributes[] = {
	...
	{ SCMD_READ_G4,
	    ...
	    0, 0, 0, st_get_cdb_g5_rw_cnt, st_get_cdb_g4g5_lba },
	{ SCMD_WRITE_G4,
	...
	{ SCMD_WRITE_FILE_MARK_G4,
	    ...
	    0, 0, 0, st_get_cdb_g5_count, st_get_cdb_g4g5_lba },
	{ SCMD_SPACE,
	    1, 0, 1, 0, 0, DIR_EITH, TRAN_NONE, POS_STARTING,
	    0, 0, 0, st_get_cdb_g0_sign_count },
	{ SCMD_SPACE_G4,
	    1, 0, 1, 0, 0, DIR_EITH, TRAN_NONE, POS_STARTING,
	    0, 0, 0, st_get_cdb_g4g5_lba },
	{ SCMD_INQUIRY,
	    0, 1, 0, 0, 0, DIR_NONE, TRAN_READ, POS_EXPECTED,
	    0, 0, 0 },
	{ SCMD_VERIFY_G0,
	    1, 0, 1, 0, 0, DIR_FORW, SUDO_READ, POS_EXPECTED,
	    0, 0, 0, st_get_cdb_g0_rw_count },
	{ SCMD_VERIFY_G4,
	    1, 0, 1, 0, 1, DIR_FORW, SUDO_READ, POS_EXPECTED,
	    0, 0, 0, st_get_cdb_g5_rw_cnt, st_get_cdb_g4g5_lba },
	{ SCMD_RECOVER_BUF,
	    1, 0, 1, 1, 0, DIR_REVC, TRAN_READ, POS_EXPECTED,
	    0, 0, 0 },
	...
	{ SCMD_LOCATE_G4,
	    1, 1, 1, 0, 0, DIR_EITH, TRAN_NONE, POS_EXPECTED,
	    0, 0, 0, NULL, st_get_cdb_g4g5_lba },
	{ SCMD_REPORT_LUNS,
	    0, 1, 0, 0, 0, DIR_NONE, TRAN_READ, POS_EXPECTED,
	    0, 0, 0 },
...
st_set_target_TLR_mode(struct scsi_tape *un, ubufunc_t ubf)
{
	...
	ret = st_gen_mode_sense(un, ubf, 0x18, 0, mode_data, amount);
	if (ret != DDI_SUCCESS) {
		if (ret != EACCES)
			un->un_tlr_flag = TLR_NOT_SUPPORTED;
		goto out;
...
static void
st_update_mode_log(struct scsi_tape *un, bufunc_t bf)
{
	int result;
	ushort_t size;

	ST_FUNC(ST_DEVINFO, st_update_mode_log);

	/*
	 * Mode change could cause Read Block Limits change.
	 */
	(void) bf(un, SCMD_READ_BLKLIM, RBLSIZE, SYNC_CMD);

	/*
	 * Read header to get overall length.
	 */
	if (un->un_mode_data_len == 0) {
		un->un_mode_data_len = sizeof (mode_parameter_header);
		un->un_mode_data = kmem_zalloc(un->un_mode_data_len, KM_SLEEP);
		un->un_mode_data_cmd = SCMD_MODE_SENSE_G1;
	}
upsize:
	result = bf(un, un->un_mode_data_cmd, un->un_mode_data_len, SYNC_CMD);
	if (result) {
		struct scsi_extended_sense *sp = un->un_sd->sd_sense;

		/*
		 * If device reports invalid command operation code that means
		 * it doesn't know SCMD_MODE_SENSE_G1 use so SCMD_MODE_SENSE.
		 */
		if ((sp->es_key == KEY_ILLEGAL_REQUEST) &&
		    (sp->es_add_code == 0x20) &&
		    (un->un_mode_data_cmd == SCMD_MODE_SENSE_G1)) {
			un->un_mode_data_cmd = SCMD_MODE_SENSE;
			goto upsize;
		}
		kmem_free(un->un_mode_data, un->un_mode_data_len);
		un->un_mode_data = NULL;
	...
	if (un->un_mode_data_cmd == SCMD_MODE_SENSE_G1) {
		size = TWO_BYTE(un->un_mode_data->ten.data_len) +
		    sizeof (un->un_mode_data->ten.data_len);
	} else {
		size = un->un_mode_data->six.data_len +
		    sizeof (un->un_mode_data->six.data_len);
	}

	if (size > un->un_mode_data_len) {
		kmem_free(un->un_mode_data, un->un_mode_data_len);
		un->un_mode_data_len = size;
		un->un_mode_data = kmem_zalloc(size, KM_SLEEP);
		goto upsize;
	}
}

static int
st_alloc_rqs(struct scsi_device *devp, struct scsi_tape *un)
{
	...
	ASSERT(un->un_rqs == NULL);

	ST_FUNC(ST_DEVINFO, st_alloc_rqs);

	un->un_rqs_bp = scsi_alloc_consistent_buf(&devp->sd_address, NULL,
	    MAX_SENSE_LENGTH, B_READ, NULL_FUNC, NULL);
	...
	un->un_rqs = scsi_init_pkt(&devp->sd_address, NULL, un->un_rqs_bp,
	    CDB_GROUP0, 1, st_recov_sz, PKT_CONSISTENT, NULL_FUNC, NULL);
...
static void
st_free_rqs(struct scsi_tape *un)
{
	ASSERT(un);
	ASSERT(mutex_owned(ST_MUTEX));

	ST_FUNC(ST_DEVINFO, st_free_rqs);
...
/*
 * st_interpret_dadp_mode_data()
 * Given a dadp mode page will return the protection mode.
 * Optionally will return the protection method and data length.
 */
static tape_dadp_state
st_interpret_dadp_mode_data(data_protect_mode_t *dadp_page, dp_setable *method)
{
	tape_dadp_state dadp_mode = DADP_DISABLE;
	...
	/* Check for correct page, subpage, and length */
	if (dadp_page->page_code != 0xa || dadp_page->subpf == 0 ||
	    dadp_page->subpage_code != 0xf0 ||
	    dadp_page->page_len[0] != 0 || dadp_page->page_len[1] != 4)
		return (DADP_ERROR);
	...
	if (dadp_page->method) {
		if (dadp_page->RBDP)
			dadp_mode |= DADP_RBDP;
		if (dadp_page->LBP_R)
			dadp_mode |= DADP_RD_ENABLE;
		if (dadp_page->LBP_W)
			dadp_mode |= DADP_WT_ENABLE;
	}

	if (method != NULL) {
		method->dp_method = dadp_page->method;
		method->dp_len = dadp_page->prot_len;
	}
...
/*
 * st_get_dadp_state()
 * Reads dadp mode page and returns that page and settings.
 */
static tape_dadp_state
st_get_dadp_state(struct scsi_tape *un, mode_parameter_header6 *page, int size,
    dp_setable *method)
{
	int result;
	data_protect_mode_t *dadp;
	mode_block_descriptor *blk_desc;
	tape_dadp_state dadp_mode = DADP_DISABLE;

	ST_FUNC(ST_DEVINFO, st_get_dadp_state);

	/*
	 * Get current DADP Mode state.
	 */
	result = st_gen_mode_sense(un, st_uscsi_cmd, 0x0A, 0xf0,
	    (struct seq_mode *)page, size);
	if (result)
		return (DADP_ERROR);

	/* Mode Sense command for this page failed. Not supported? */
	if ((page->data_len < sizeof (mode_parameter_header6)) ||
	    (page->bd_len != sizeof (mode_block_descriptor)))
		return (DADP_ERROR);

	blk_desc = (mode_block_descriptor *)&page[1];
	dadp = (data_protect_mode_t *)&blk_desc[1];

	dadp_mode = st_interpret_dadp_mode_data(dadp, method);
...
/* ARGSUSED */
static uint32_t
off_len(dp_setable const *set, uint32_t data_len)
{
	return (0);
}
...
/* ARGSUSED */
uint32_t
no_crc(dp_setable const *set, const caddr_t nu, uint32_t len)
{
	ASSERT(0);
	return (0);
}
...
/* ARGSUSED */
static void
off_add(dp_setable const *set, caddr_t nu, uint32_t len)
{
	ASSERT(0);
	;
}
...
/* ARGSUSED */
static boolean_t
off_chk(dp_setable const *set, caddr_t nu, uint32_t len)
{
	ASSERT(0);
	return (B_TRUE);
}

const dp_funcs off_funcs = {
	off_name,	/* name */
	off_len,	/* len */
	off_add,	/* add */
	off_chk,	/* check */
...
/*
 * st_set_dadp()
 * Gets and then sets dadp mode page.
 */
static int
st_set_dadp(struct scsi_tape *un, tape_dadp_state new_mode)
{
	int sz;
	int result;
	dp_setable cur_setting;
	tape_dadp_state cur_mode;
	mode_parameter_header6 *mode_data;
	data_protect_mode_t *dadp;
	const dadp_info *setting;

	ST_FUNC(ST_DEVINFO, st_set_dadp);

	setting = new_mode == DADP_DISABLE ? &dadp_off : &un->un_dadp;

	/* if disabled only allow disable settings. */
	if (un->un_dadp.dpf == &off_funcs && new_mode != DADP_DISABLE)
		return (ENOTTY);

	/*
	 * Check for valid requests.
	 */
	if ((new_mode < DADP_DISABLE) || (new_mode > DADP_RBDP_RW_ENABLE))
		return (EINVAL);

	sz = sizeof (mode_parameter_header6) + sizeof (mode_block_descriptor) +
	    sizeof (data_protect_mode_t);
	mode_data = (mode_parameter_header6 *)kmem_zalloc(sz, KM_SLEEP);

	/*
	 * Read current DADP setting.
	 */
	cur_mode = st_get_dadp_state(un, mode_data, sz, &cur_setting);
	if (cur_mode == DADP_ERROR) {
		kmem_free(mode_data, sz);
		return (EIO);
	}

	/*
	 * If already set to what is being asked for just return.
	 */
	if ((cur_mode == new_mode) &&
	    (cur_setting.dp_method == setting->dps.dp_method) &&
	    (cur_setting.dp_len == setting->dps.dp_len)) {
		kmem_free(mode_data, sz);
		un->un_dadp_state = new_mode;
		return (EALREADY);
	}

	switch (mode_data->bd_len) {
		mode_block_descriptor *blk_desc;

	case sizeof (mode_block_descriptor):
		/* Should be getting block descriptors */
		blk_desc = (mode_block_descriptor *)&mode_data[1];
		dadp = (data_protect_mode_t *)&blk_desc[1];
		break;
	case 0:
		dadp = (data_protect_mode_t *)&mode_data[1];
		break;
	}

	if (new_mode != DADP_DISABLE &&
	    setting->dps.dp_method &&
	    setting->dps.dp_len) {
		dadp->method = setting->dps.dp_method;
		dadp->prot_len = setting->dps.dp_len;

		if (new_mode & DADP_WT_ENABLE)
			dadp->LBP_W = 1;
		else
			dadp->LBP_W = 0;

		if (new_mode & DADP_RD_ENABLE)
			dadp->LBP_R = 1;
		else
			dadp->LBP_R = 0;

		if (new_mode & DADP_RBDP)
			dadp->RBDP = 1;
		else
			dadp->RBDP = 0;
	} else {
	 ...
	 */
	result = st_gen_mode_select(un, st_uscsi_cmd,
	    (struct seq_mode *)mode_data, sz);

	/* If mode select worked read back and verify settings */
	if (result == 0) {
		ST_DEBUG(ST_DEVINFO, st_label, CE_WARN,
		    "Set DADP mode %s:%s", un->un_dadp.dpf->dp_name(),
		    new_mode == DADP_DISABLE ? "Disabled" : "Enabled");
		cur_mode = st_get_dadp_state(un, mode_data, sz, &cur_setting);
	}

	/* If settings did not take disable */
	if ((result) ||					/* mode select failed */
	    (cur_mode != new_mode) ||			/* read/write mode */
	    (cur_setting.dp_method != setting->dps.dp_method) || /* method */
	    (cur_setting.dp_len != setting->dps.dp_len)) {
		...
		/* select dadp disabled mode */
		result = st_gen_mode_select(un, st_uscsi_cmd,
		    (struct seq_mode *)mode_data, sz);

		/*
		 * Hope this mode select disables.
		 * Even if it works report err since requested action failed.
		 */
		if (result == 0)
			result = EIO;
		un->un_dadp_state = DADP_DISABLE;
	} else
		un->un_dadp_state = new_mode;
	kmem_free(mode_data, sz);
...
static int
st_set_dadp_support(struct scsi_tape *un)
{
	caddr_t config_list = NULL;
	int config_list_len;
	struct scsi_inquiry *inq = un->un_sd->sd_inq;

	ST_FUNC(ST_DEVINFO, st_set_dadp_support);

	/* Firmware doesn't support data protection. */
	if (inq->inq_protect == 0)
		goto exit;

	if (ddi_getlongprop(DDI_DEV_T_ANY, ST_DEVINFO, DDI_PROP_DONTPASS,
	    "st_dadp_settings", (caddr_t)&config_list, &config_list_len) ==
	    DDI_PROP_SUCCESS) {
		int len;
		int tuplelen;
		char *vidptr;
		...
		for (len = config_list_len, vidptr = config_list;
		    len > 0;
		    vidptr += tuplelen, len -= tuplelen) {
			alg_name = vidptr + strlen(vidptr) + 1;
			conf_name = alg_name + strlen(alg_name) + 1;
			tuplelen = strlen(vidptr) + strlen(alg_name) +
			    strlen(conf_name) + 3;
			...
			/*
			 * If inquiry vid dosen't match this douples vid,
			 * try the next.
			 */
			if (strncasecmp(vidptr, inq->inq_vid, strlen(vidptr))) {
				continue;
			}
			if (strncmp(dadp_off.dpf->dp_name(), alg_name,
			    strlen(alg_name)) == 0) {
				un->un_dadp = dadp_off;
				return (0);
#ifndef __sparc
			} else if (strncmp(crc32c_xhardware.dp_name(), alg_name,
			    strlen(alg_name)) == 0) {
				un->un_dadp.dpf = &crc32c_xhardware;
#endif
			} else if (strncmp(crc32c_software.dp_name(), alg_name,
			    strlen(alg_name)) == 0) {
				un->un_dadp.dpf = &crc32c_software;
			} else {
				continue;
			}
			 ...
			 */
			if (ddi_getlongprop(DDI_DEV_T_ANY, ST_DEVINFO, 0,
			    conf_name, (caddr_t)&dadp_list, &dadp_list_len) !=
			    DDI_PROP_SUCCESS) {
				/*
				 * Error in getting property value
				 * print warning!
				 */
				scsi_log(ST_DEVINFO, st_label, CE_WARN,
				    "dadp property (%s) has no values\n",
				    conf_name);
				goto exit;
			}
			if (dadp_list_len != sizeof (int) * 4) {
				scsi_log(ST_DEVINFO, st_label, CE_WARN,
				    "dadp property (%s) has %d values but"
				    " should have 4\n",
				    conf_name, dadp_list_len);
					goto exit;
			}
			un->un_dadp.dps.dp_method = dadp_list[0];
			un->un_dadp.dps.dp_len = dadp_list[1];
			un->un_dadp.dps.dp_mod = 1 << (dadp_list[2]);
			un->un_dadp.dps.dp_seed = dadp_list[3];
			return (1);
		}
	}
exit:

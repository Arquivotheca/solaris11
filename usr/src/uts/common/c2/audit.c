 ...
 *		the error returned by the lookup facility for
 *		the last component (error).
 * TODO:
 * QUESTION:
 */
...
audit_savepath(
	...
	int    error,			/* error returned by the lookup */
	cred_t *cr)			/* cred of requestor */
{
	...
	/*
	 * Noise elimination in audit trails - this event will be discarded iff:
	 * 1)  the public policy is not active AND
	 * 2)  the system call is being audited AND
	 * 3)  the system call is a public operation AND
	 * 4)  the VFS lookup failed for some reason AND
	 * 5a) the object being accessed does exist and is public OR
	 * 5b) the object is missing and it would have been located in a public
	 * directory owned by root if it had existed.
	 */
	if (tad->tad_flag != 0 && error != 0 &&
	    (tad->tad_ctrl & TAD_PUBLIC_EV) &&
	    !(kctx->auk_policy & AUDIT_PUBLIC)) {
		struct vattr attr;
		struct vnode *vn = (vp != NULL) ? vp : pvp;

		/*
		 * Conditions 1-4 are true. Lets check if either 5a or 5b is
		 * true as well.
		 */
		if (vn != NULL) {
			attr.va_mask = AT_ALL;
			if (VOP_GETATTR(vn, &attr, 0, CRED(), NULL) == 0) {
				if (object_is_public(&attr)) {
					/* Do not audit this event */
	...
	/*
	 * Is this event being audited or do we need path information later?
	 * This might be for a chdir/chroot or open (add path to file pointer).
	 * If the path has already been found for an open/creat then we don't
	 * need to process the path.
	 *
	...
	/*
	 * Prevent possible reentry. We may be recursively called again
	 * from the lookup done by:
	 * audit_attributes()->...->getflabel()->vnodetopath()->...
	 */
	tad->tad_ctrl |= TAD_NOPATH;
	...
	if (tad->tad_flag != 0) {
		/*
		 * If an error appeared during the lookup, the file may be
		 * missing and the openat() syscall called with FCREAT (without
		 * FEXCL) will try to create the file. Lets make a note on this
		 * so audit_setf() knows which openat() event subtype should be
		 * audited.
		 */
		if (error != 0 &&
		...
		if (vp != NULL) {
			/*
			 * Capture attributes when there is no error. The lookup
			 * usually does not return the vnode of the failing
			 * component. If there was a lookup error, then don't
			 * add attributes. When called during the vn_create()
			 * execution, then don't add attributes. They are added
			 * at the end of vn_create().
			 */
			if (error == 0 && !(tad->tad_ctrl & TAD_NOATTRB))
				audit_attributes(vp);
		}
	}
	...
	/*
	 * The system call may perform multiple lookups if TAD_MLD is set. Save
	 * the first one only.
	 */
...
audit_exit(int code, int what)
{
	...
	if (tad->tad_scid == SYS_exit) {
		...
		if (tad->tad_flag && tad->tad_event == AUE_EXIT)
			(void) audit_finish(0, SYS_exit, 0, NULL);
		return;
	}
	...
	if (tad->tad_flag) {
		...
		(void) audit_finish(0, tad->tad_scid, 0, NULL);
	}

	/*
	 * Generate an audit record for process exit if preselected.
	 */
	(void) audit_start(0, SYS_exit, AUC_UNSET, NULL);
	(void) audit_finish(0, SYS_exit, 0, NULL);
}
...
audit_core_start(int sig)
{
	...
	if (kctx->auk_auditstate & ~(AUC_AUDITING | AUC_INIT_AUDIT)) {
		tad->tad_flag = 0;
		tad->tad_ctrl = 0;
		return;
	}
 ...
 */
void
audit_vncreate_finish(struct vnode *vp, int flag, int error)
{
	t_audit_data_t *tad;
	...
	if ((tad->tad_ctrl & TAD_TRUE_CREATE) ||
	    ((flag & FCREAT) && (tad->tad_scid == SYS_openat ||
	    tad->tad_scid == SYS_openat64))) {
		audit_attributes(vp);
	}
	...
	if ((tad->tad_event == AUE_MKNOD) || (tad->tad_event == AUE_MKDIR)) {
		audit_attributes(vp);
	}
...
/*
 * ROUTINE:	audit_filepath
 * PURPOSE:	create the audit pathname from a file pointer
 * CALLBY:	exece(..., EXEC_DESCRIPTOR)
 * NOTE:	file structure already initialized
 * TODO:
 * QUESTION:
 */
void
audit_filepath(struct file *fp)
{
	t_audit_data_t *tad = U2A(u);
	f_audit_data_t *fad;

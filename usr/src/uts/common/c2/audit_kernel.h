 ...
 *   what the original parameter was before as well as after the system call.
...
struct p_audit_data {
	...
	struct _pad_data {
		...
		uint32_t		pad_flags;   /* process audit flags */
	} pad_data;
};
...
#define	PAD_KAUDIT 	0x00000002	/* system call auditing enabled */
...
struct audit_queue {
	...
	timeout_id_t coid_stop;	/* audit_dont_stop() callout id */
};
...
struct au_kcontext {
	...
	au_class_t		auk_classes;	/* set of kernel classes */
};

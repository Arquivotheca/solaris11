...
typedef uint64_t au_class_t;
typedef uint32_t au_class32_t;
 ...
 * An audit event mask.
 */
#define	AU_MASK_ALL	UINT64_MAX	/* all bits on for uint64_t */
#define	AU_MASK_NONE	UINT64_C(0x0)	/* all bits off = no:invalid class */

struct au_mask {
	au_class_t  am_success;	    /* success bits */
	au_class_t  am_failure;	    /* failure bits */
};
typedef struct au_mask au_mask_t;

/*
 * We need to store the 64-bit audit class as two 32-bit values in ucred_t.
 * The reason is that ucred_t needs to be 32-bit aligned. Using uint64_t
 * would make ucred_t 64-bit aligned.
 */
struct au_mask32 {
	au_class32_t  am_success_lo;	/* low 32 success bits */
	au_class32_t  am_success_hi;	/* high 32 success bits */
	au_class32_t  am_failure_lo;	/* low 32 failure bits */
	au_class32_t  am_failure_hi;	/* high 32 failure bits */
};
typedef struct au_mask32 au_mask32_t;
...
#define	A_GETAMASK	31	/* get user default audit event mask */
#define	A_SETAMASK	32	/* set user default audit event mask */
...
#define	AQ_MINDELAY	1

/*
 * boundary checking macros for the queuectrl parameters
 */
#define	CHK_BDRY_QBUFSZ(x)	((x) >= AQ_BUFSZ && (x) <= AQ_MAXBUFSZ)
#define	CHK_BDRY_QDELAY(x)	((x) >= AQ_MINDELAY && (x) <= AQ_MAXDELAY)
#define	CHK_BDRY_QLOWATER(low, high)	((low) >= AQ_MINLOW && (low) < (high))
#define	CHK_BDRY_QHIWATER(low, high)	((high) > (low) && \
					    (high) >= AQ_LOWATER && \
					    (high) <= AQ_MAXHIGH)

#define	AU_CLASS_64(lo, hi) \
	((au_class_t)((lo) | (((uint64_t)(hi)) << 32)))
#define	AU_CLASS_LO(c)	((au_class32_t)((c) & UINT64_C(0xFFFFFFFF)))
#define	AU_CLASS_HI(c)	((au_class32_t)(((c) >> 32) & UINT64_C(0xFFFFFFFF)))
...
void	audit_filepath(struct file *);
...
void	audit_vncreate_finish(struct vnode *, int, int);
...
int	audit_start(unsigned int, unsigned int, uint32_t, klwp_t *);
int	audit_finish(unsigned int, unsigned int, int, union rval *);
...
uint32_t    audit_getpflags(void);
void	    audit_setpflags(proc_t *);
int	    au_zone_getstate(const au_kcontext_t *);
au_class_t  au_zone_getkclasses(proc_t *);

/* The audit mask defining in which case is auditing enabled */
#define	AU_AUDIT_MASK	(AUC_AUDITING)

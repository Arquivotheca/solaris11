...
struct au_output_context {
	au_kcontext_t	*zone_kctx;
	kmutex_t	lock;
};
...
auditdoor(int fd)
{
	...
	if (!kctx->auk_output_active) {
		kctx->auk_output_active = B_TRUE;
		do_create = B_TRUE;
	}
	mutex_exit(&(kctx->auk_svc_lock));
	if (do_create) {
		if (kctx->auk_taskq == NULL) {
			kctx->auk_taskq = taskq_create_proc("audit_taskq", 1,
			    minclsyspri, 1, 1, curzone->zone_zsched, 0);
		} else {
			/*
			 * Wait for a previous output thread to finish its job.
			 * There should be at most one output thread active per
			 * a zone. Although the thread should be already gone,
			 * wait, just in case.
			 */
			taskq_wait(kctx->auk_taskq);
		}
		/* Dispatch a new output thread for execution */
		(void) taskq_dispatch(kctx->auk_taskq,
		    (task_func_t *)au_output_thread, kctx, TQ_SLEEP);
	}
	return (0);
}
...
{
	au_kcontext_t *ctx = (au_kcontext_t *)kctx;

	if (ctx->auk_auditstate == AUC_NOAUDIT) {
		return;
	}

	mutex_enter(&ctx->auk_queue.lock);
	cv_broadcast(&ctx->auk_queue.write_cv);
	ctx->auk_queue.coid_stop = 0;
	mutex_exit(&ctx->auk_queue.lock);
}
 ...
 */
static void
au_queue_kick(void *context)
{
	struct au_output_context *kctx = (struct au_output_context *)context;

	mutex_enter(&kctx->lock);
	/*
	 * wakeup reader if its not running and there is something
	 * to do.
	 */
	if (kctx->zone_kctx == NULL) {
		/*
		 * au_output_thread is down. There will be no one
		 * to wake up. Exit and do not schedule another
		 * callout.
		 */
		mutex_exit(&kctx->lock);
		/* It is up to us to free the memory */
		mutex_destroy(&kctx->lock);
		kmem_free(context, sizeof (struct au_output_context));
		return;
	}

	if (kctx->zone_kctx->auk_queue.cnt &&
	    kctx->zone_kctx->auk_queue.rd_block)
		cv_broadcast(&kctx->zone_kctx->auk_queue.read_cv);

	/* fire off timeout event to kick audit queue awake */
	(void) timeout(au_queue_kick, kctx, kctx->zone_kctx->auk_queue.delay);
	mutex_exit(&kctx->lock);
}
...
{
	int error = 0;
	struct au_output_context *kick_kctx =
	    kmem_alloc(sizeof (struct au_output_context), KM_SLEEP);

	/*
	 * Set up callouts.
	 */
	mutex_enter(&(kctx->auk_queue.lock));
	kctx->auk_queue.coid_stop = 0;
	mutex_exit(&(kctx->auk_queue.lock));
	kick_kctx->zone_kctx = kctx;
	mutex_init(&kick_kctx->lock, NULL, MUTEX_DEFAULT, NULL);
	(void) timeout(au_queue_kick, kick_kctx, kctx->auk_queue.delay);
	...
	while (!error) {
		if (kctx->auk_auditstate == AUC_AUDITING) {
			...
			while (kctx->auk_queue.head == NULL) {
				...
				if (kctx->auk_auditstate != AUC_AUDITING) {
					mutex_exit(&(kctx->auk_queue.lock));
					kctx->auk_queue.coid_stop =
					    timeout(audit_dont_stop, kctx,
					    au_resid);
					goto output_exit;
				}
			}
...
output_exit:
	...
	kctx->auk_output_active = B_FALSE;
	mutex_exit(&(kctx->auk_svc_lock));
	/*
	 * We are exiting, so there will be no one to wake up. Tell
	 * au_queue_kick callout to shut down.
	 */
	mutex_enter(&kick_kctx->lock);
	kick_kctx->zone_kctx = NULL;
	mutex_exit(&kick_kctx->lock);
}

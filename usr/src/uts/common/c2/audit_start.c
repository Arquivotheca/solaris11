...
audit_init_module()
{
	...
	au_mem_init();
	/* The minimum for the c2audit module is ready */
...
/*ARGSUSED*/
int
audit_start(unsigned type, unsigned scid, uint32_t audit_state, klwp_t *lwp)
{
	...
	ai = crgetauinfo(curproc->p_cred);
	if ((ai == NULL) ||
	    (((ai->ai_mask.am_success | ai->ai_mask.am_failure) &
	    au_zone_getkclasses(curproc)) == 0)) {
		p_audit_data_t	*pad = P2A(curproc);

		/* kernel audit is no longer selected */

		/* clear the audit state */
		tad->tad_audit = AUC_UNSET;
		mutex_enter(&pad->pad_lock);
		pad->pad_flags &= ~PAD_KAUDIT;
	...
	if (audit_s2e[scid].au_init == (au_event_t)AUE_NULL) {
		...
		/* get specific event */
		tad->tad_event =
		    (*audit_s2e[scid].au_init)(audit_s2e[scid].au_event);
	}
...
/*ARGSUSED*/
int
audit_finish(unsigned type, unsigned scid, int error, rval_t *rval)
{
	struct t_audit_data *tad;
	int	flag;

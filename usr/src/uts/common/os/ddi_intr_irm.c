...
extern boolean_t	ddi_msix_alloc_tuned;
...
{
	/*
	 * Initialize ddi_msix_alloc_limit.
	 *
	 * On SPARC, begin with DDI_MIN_MSIX_ALLOC currently defined as 8.
	 * On x86, begin with DDI_DEFAULT_MSIX_ALLOC currently defined as 2.
	 * The APIX module on x86 may later upgrade it to DDI_MIN_MSIX_ALLOC.
	 *
	 * If the value was tuned to a non-zero value, preserve the tuning
	 * but impose a cap of DDI_MIN_MSIX_ALLOC on the tunable value.
	 *
	 * NOTE: this is done regardless if IRM is enabled, supported, etc.
	 */
	if (ddi_msix_alloc_limit == 0) {
#ifdef	__amd64
		...
		ddi_msix_alloc_tuned = B_TRUE;
		ddi_msix_alloc_limit = MIN(ddi_msix_alloc_limit,
		    DDI_MIN_MSIX_ALLOC);
	}
    ...
    ddi_irm_pool_t **pool_retp)
{
	...
	pool_p->ipool_defsz = MIN(DDI_MAX_MSIX_ALLOC, MAX(ddi_msix_alloc_limit,
...
ndi_irm_resize_pool(ddi_irm_pool_t *pool_p, uint_t new_size)
{
	...
	if ((pool_p->ipool_totsz < new_size) ||
		...
		pool_p->ipool_defsz = MIN(DDI_MAX_MSIX_ALLOC,
		    MAX(ddi_msix_alloc_limit,
		    new_size / DDI_MSIX_ALLOC_DIVIDER));
...
i_ddi_irm_balance(ddi_irm_pool_t *pool_p)
{
	 ...
	 * Also, check for the REDUCED and NEW flags.  Clear the
	 * REDUCED flag if a request got upgraded at or above the
	 * default.  Always clear the NEW flag.
	 */
	while (req_p = list_remove_head(&pool_p->ipool_scratch_list)) {
		req_p->ireq_flags &= ~(DDI_IRM_FLAG_NEW);
		if (req_p->ireq_navail > req_p->ireq_scratch) {
			(void) i_ddi_irm_notify(pool_p, req_p);
			if ((req_p->ireq_flags & DDI_IRM_FLAG_REDUCED) &&
			    (req_p->ireq_navail >= pool_p->ipool_defsz)) {
				req_p->ireq_flags &= ~(DDI_IRM_FLAG_REDUCED);
			}
		}
	}
 ...
 *	order is preserved.  It avoids reducing requests below one interrupt
 *	per device.
 *
...
i_ddi_irm_reduce_by_policy(ddi_irm_pool_t *pool_p, int imbalance, int policy)
{
	...
	while (imbalance > 0) {
		...
		if ((policy == DDI_IRM_POLICY_LARGE) && req_p &&
			...
			reduction = last_p->ireq_navail - 1;
 ...
 *
 *	When this reduces a new request, DDI_IRM_FLAG_REDUCED is set.
 *	The flag is only cleared by i_ddi_irm_balance() if it is ever
 *	successfully upgraded at or above the default size.  All the
 *	previously reduced requests are analyzed first before reducing
 *	the newest request.
 *
...
i_ddi_irm_reduce_new(ddi_irm_pool_t *pool_p, int imbalance)
{
	...
	/* Try to solve the imbalance by restoring previous reductions */
	for (req_p = list_head(&pool_p->ipool_scratch_list); req_p;
	    req_p = list_next(&pool_p->ipool_scratch_list, req_p)) {
		if ((req_p->ireq_flags & DDI_IRM_FLAG_REDUCED) &&
		    (req_p->ireq_navail > req_p->ireq_scratch)) {
			reduction = req_p->ireq_navail - req_p->ireq_scratch;
			reduction = MIN(imbalance, reduction);
			req_p->ireq_navail -= reduction;
	...
	/* If still imbalanced, reduce the newest request */
	for (req_p = list_head(&pool_p->ipool_scratch_list); req_p;
	    req_p = list_next(&pool_p->ipool_scratch_list, req_p)) {

...
stop(int why, int what)
{
	...
	if (why != PR_JOBCONTROL && why != PR_CHECKPOINT) {
		...
		/*
		 * The case of lwp executing PR_REQUESTED stop while other lwp's
		 * hitting the watch point simultaneously can result in missing
		 * a signal on the p_holdlwps leading to process hang. proc tool
		 * utilities also hangs.
		 */
		if (!procstop && (p->p_flag & SHOLDWATCH))
			cv_broadcast(&p->p_holdlwps);
	}
...
void
sigcld_list_create(proc_t *p)

{
	list_create(&p->p_cldpend_head, sizeof (proc_t),
	    offsetof(proc_t, p_cldpend_list));
	list_link_init(&p->p_cldpend_list);

	list_create(&p->p_ns_head, sizeof (proc_t),
	    offsetof(proc_t, p_ns_list));
	list_link_init(&p->p_ns_list);
}

void
sigcld_insert(proc_t *cp)

{
	ASSERT(MUTEX_HELD(&pidlock));

	if ((cp->p_pidflag & CLDPEND) && (cp->p_parent != NULL)) {
		ASSERT(!list_link_active(&cp->p_cldpend_list));
		cp->p_pidflag |= CLDPEND;
		list_insert_tail(&(CLDPEND_LIST_HEAD(cp)), cp);
	}
}

void
sigcld_remove(proc_t *cp)
{
	ASSERT(MUTEX_HELD(&pidlock));

	if ((cp->p_pidflag & CLDPEND) && (cp->p_parent != NULL)) {
		ASSERT(!list_is_empty(&CLDPEND_LIST_HEAD(cp)));
		ASSERT(list_link_active(&cp->p_cldpend_list));
		cp->p_pidflag &= ~CLDPEND;
		list_remove(&(CLDPEND_LIST_HEAD(cp)), cp);
	}
}
...
sigcld_repost()
{
	...
	sqp = kmem_zalloc(sizeof (sigqueue_t), KM_SLEEP);
	mutex_enter(&pidlock);
	cp = (proc_t *)list_head(&pp->p_cldpend_head);
	if (cp) {
		post_sigcld(cp, sqp);
		mutex_exit(&pidlock);
		return;
...
sigsendset(procset_t *psp, sigsend_t *pv)
{
	...
	error = dotoprocs(psp, sigsendproc, (char *)pv, &retprocp);

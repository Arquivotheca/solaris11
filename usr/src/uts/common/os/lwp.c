...
uint64_t	lwp_ac_bticks;		/* buffered ticks for data clients */
    ...
    int state, int pri, const k_sigset_t *smask, int cid, id_t lwpid)
{
	...
	if (CLASS_KERNEL(cid)) {
		...
		if (lwp_reapcnt > 0) {
			...
			if ((t = lwp_deathrow) != NULL) {
				...
				ctx = t->t_ctx;
				/*
				 * Get the thread free lock to sync with
				 * LWP accounting.
				 */
				LWP_AC_LOCK(t);
				t->t_swap = NULL;
	...
	/*
	 * We initially flag threads as transient in thread_create() because
	 * kernel threads are expected to be short running. Here we check if
	 * we're creating a user thread and undo the initial characterization
	 * since we don't expect such threads to be transient.
	 */
	if (thread_transience_user == B_TRUE && !CLASS_KERNEL(cid))
		t->t_transience = 0;
...
grow:
	...
	/*
	 * Normally, we inherit processor and processor set bindings from
	 * curthread.  For kernel LWPs, we do one of:
	 *
	 *	1. Attach using default bindings if the process is new (i.e.
	 *	   p_tlist is NULL),
	 *	2. Inherit from curthread, if we are also in process p, or
	 *	3. Inherit from an arbitrary thread in the process (p->p_tlist)
	 *	   otherwise.
	 * This ensures kernel lwps for system processes in zones are
	 * bound to the right pset.
	 *
	 * lgroup affinities are inherited only if the inherit flag is
	 * set in the parent.
	 */
	tx = curthread;
	...
	if (tx == NULL) {
		/* New kernel process; start with default bindings */
		...
		/* Inherit from tx */
		binding = tx->t_bind_cpu;
		t->t_bind_cpu = binding;
		oldpart = t->t_cpupart;
		t->t_cpupart = tx->t_cpupart;
		t->t_bind_pset = tx->t_bind_pset;
		t->t_bindflag = tx->t_bindflag |
		    (uchar_t)default_binding_mode;

		if (curthread->t_lgrp_aff &&
		    curthread->t_lgrp_aff->aff_inherit ==
			...
			/*
			 * Copy lgroup affinities from parent. Also set
			 * the affinity inheritance flag so that children
			 * of this new LWP will inherit lgroup affinities.
			 */
			t->t_lgrp_aff->aff =
			    kmem_zalloc(nlgrpsmax * sizeof (lgrp_affinity_t),
			    KM_SLEEP);

			for (i = 0; i < nlgrpsmax; i++) {
				t->t_lgrp_aff->aff[i] =
				    curthread->t_lgrp_aff->aff[i];
			}

			t->t_lgrp_aff->aff_inherit =
			    curthread->t_lgrp_aff->aff_inherit;
		}
	}
	 ...
	 * If lgroup affinity inheritance flag is set, home lgroup is
	 * inherited for threads that are not kernel threads and for threads
	 * that are not bound.
	 */
	ASSERT(oldpart != NULL);
	if (binding != PBIND_NONE && t->t_affinitycnt == 0) {
		...
		lpl_t *lpl_ptr;
		lgrp_id_t home = curthread->t_lpl->lpl_lgrpid;

		if (curthread->t_lgrp_aff &&
		    curthread->t_lgrp_aff->aff_inherit ==
		    LGRP_AFF_INHERIT_FUTURE) {
			lpl_ptr = &t->t_cpupart->cp_lgrploads[home];
		} else {
			lpl_ptr = lgrp_choose(t, t->t_cpupart);
		}

		lgrp_move_thread(t, lpl_ptr, 1);
	}
	...
	/*
	 * Propagate the task from the process to the thread.
	 */
	t->t_task = p->p_task;
	...
	/*
	 * Propagate the zone from the process to the thread.
	 */
	t->t_zone = p->p_zone;
...
lwp_exit(void)
{
	 ...
	 * (See the block comment preceding lwp_wait().)
	 */
...
forklwp(klwp_t *lwp, proc_t *cp, id_t lwpid)
{
	...
	*clwp = *lwp;
	...
	/*
	 * Restore LWP accounting information.
	 */
	clwp->lwp_ac = lac;
	...
	for (i = 0; i < 3; i++)
		clwp->lwp_ac_timer[i].lac_timer = UINT64_MAX;
...
lwp_ac_init(void)
{
	lwp_ac_bticks = NSEC_TO_TICK(LWP_AC_BTIME);
...
void
lwp_ac_start(klwp_t *lwp)
{
	int i;

	proc_ac_create(lwptoproc(lwp));
	LWP_AC_LOCK(lwptot(lwp));
	for (i = 0; i < 3; i++)
		lwp->lwp_ac_timer[i].lac_timer = UINT64_MAX;
	LWP_AC_UNLOCK(lwptot(lwp));
}

void
lwp_ac_end(klwp_t *lwp)
{
	LWP_AC_LOCK(lwptot(lwp));
	lwp->lwp_ac.ac_flags |= LWP_AC_EXIT;
	LWP_AC_UNLOCK(lwptot(lwp));
}

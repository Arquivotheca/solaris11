...
netstack_unregister(int moduleid)
{
	...
	while (ns != NULL) {
		...
		while (ns->netstack_flags & NSF_ZONE_CREATE) {
			/*
			 * If the zone is in the process of creation, wait
			 * for that to be done (wait for netstack_zone_create()
			 * to clear the NSF_ZONE_CREATE flag).
			 *
			 * Since the locks are to be released, in case of the
			 * changed netstack list, we set restart to be true
			 * so that the loop will later be started over from
			 * netstack_head.
			 */
			restart = B_TRUE;
			mutex_exit(&netstack_g_lock);
...
netstack_zone_create(zoneid_t zoneid)
{
	...
	zone = curproc->p_zone;
	/*
	 * The ZSD is registered early in boot, so the creator callback will
	 * always be executed (by zsched) in the context of the zone which
	 * it is initializing.
	 */
	ASSERT(zoneid == zone->zone_id);
...
netstack_zone_shutdown(zoneid_t zoneid, void *arg)
{
	...
	/*
	 * Determine the set of stacks that exist before we drop the lock.
	 * Set NSS_SHUTDOWN_NEEDED for each of those. We support calling
	 * shutdown on previously shutdown netstacks (zone shutdown callbacks
	...
	for (i = 0; i < NS_MAX; i++) {
		...
		if ((ns_reg[i].nr_flags & NRF_REGISTERED) &&
		    ...
		    (nms->nms_flags & NSS_CREATE_COMPLETED) &&
		    (nms->nms_flags & NSS_SHUTDOWN_INPROGRESS) == 0) {
			nms->nms_flags |= NSS_SHUTDOWN_NEEDED;
			DTRACE_PROBE2(netstack__shutdown__needed,
			    netstack_t *, ns, int, i);
		}

...
brand_register(brand_t *brand)
{
	...
	for (scan = brand_list; scan != NULL; scan = scan->bl_next) {
		if (strcmp(brand->b_modname, scan->bl_brand->b_modname) == 0) {
			cmn_err(CE_WARN,
			    "Invalid attempt to load a second instance of "
			    "brand %s", brand->b_modname);
...
brand_unregister(brand_t *brand)
{
	...
	if (list == NULL) {
		cmn_err(CE_WARN, "Brand %s wasn't registered",
		    brand->b_modname);
		mutex_exit(&brand_list_lock);
		return (EINVAL);
	}
...
/*
 * Load the brand module if required.
 */
brand_t *
brand_modopen(char *modname)
{
	 ...
	 * already loaded brand module.  If the brand module hasn't been
	 * loaded, we call ddi_modopen() to force it to be loaded and then
	 * make a second pass through the list of brand modules.
	 */
	for (;;) {
		/* Search list of loaded brand modules */
		mutex_enter(&brand_list_lock);
		for (l = brand_list; l != NULL; l = l->bl_next)
			if (strcmp(modname, l->bl_brand->b_modname) == 0)
				break;
		if ((l != NULL) || (hdl != NULL))
			break;
		mutex_exit(&brand_list_lock);
		...
		/*
		 * We didn't find that the requested brand module, so
		 * trigger the load and search the list again.
		 */
		modpath = kmem_alloc(MAXPATHLEN, KM_SLEEP);
		(void) strcpy(modpath, "brand/");
		(void) strcat(modpath, modname);
		hdl = ddi_modopen(modpath, KRTLD_MODE_FIRST, &err);
...
void
brand_modclose(struct brand *bp)
{
    ...
    struct brand *pbrand, int brandvers)
{
	...
	/*
	 * Fail right away if we don't recognize this command; the brand
	 * may implement its own subcodes, and our safety checks do not
	 * apply to those.
	 */
	switch ((cmd)) {
	case B_EXEC_NATIVE:
	...
	case B_TRUSS_POINT:
		break;
	default:
		return (-1);	/* We don't handle this command */
	}

	/*
	 * For all of the generic brandsys() subcodes defined thus far, this
	 * must be a branded process; a native process in a branded zone could
	 * (in theory) get here if it tried calling brandsys(2).
	 * This check rejects any such attempts for the generic subcodes.
	 *
	 * Note that for brand subcodes in the brand-specific category
	 * (B_BRANDSPECIFIC_BASE and above), this does not apply.
	 */
	...
	switch ((cmd)) {
	case B_EXEC_NATIVE:
		err = exec_common(-1, (char *)arg1, (const char **)arg2,
		    (const char **)arg3, EBA_NATIVE, 0);
		return (err);
...
static void
restoreexecenv(execenv_t *ep, stack_t *sp)
{
    ...
    char *brandlib, char *brandlib32, char *brandlinker, char *brandlinker32)
{
	...
	if (args->to_model == DATAMODEL_NATIVE) {
		err = mapexec_brand(vp, args, &ehdr, &uphdr_vaddr,
		    &voffset, exec_file, &interp, &env, NULL);
	}
#if defined(_LP64)
	else {
		Elf32_Ehdr ehdr32;

 ...
 *
 * 'ispeek' is used to signal from the caller that this is only a peek, not a
 * read, and this indication is passed through to getq_noenab to instruct it not
 * to fragment any mblks. In this case, it is possible for getq_noenab to return
 * more bytes than requested, so the caller must putback any unused part of the
 * message.
 */
mblk_t *
strget(struct stdata *stp, queue_t *q, struct uio *uiop, int first,
    int *errorp, boolean_t ispeek)
{
	mblk_t *bp;
	int error;
	ssize_t rbytes = 0;
	...
	*errorp = 0;
	...
	return (getq_noenab(q, rbytes, !ispeek));
}
...
#define	_LASTMARK	0x8000	/* Distinct from MSG*MARK */
	...
	for (;;) {
		...
		delim = 0;
		first = 1;
		while ((bp = strget(stp, q, uiop, first, &error, B_FALSE)) ==
		    NULL) {
			int done = 0;
...
ismdata:
			...
			/*
			 * If our privileges have increased since the message
			 * was written, we must discard it.
			 */
			if ((bp->b_flag & MSGCRED) &&
			    !hasprocperm(CRED(), DB_CRED(bp))) {
				freemsg(bp);
				mutex_enter(&stp->sd_lock);
				continue;
			}
...
strrput(queue_t *q, mblk_t *bp)
{
	...
	uchar_t		band = 0;
	int		hipri_sig;
	boolean_t	link_to_qlast = B_FALSE;
	...
	case M_DATA:
		...
		if ((rput_opt & SR_CONSOL_DATA) &&
			...
			if ((db_type == M_DATA || db_type == M_PROTO ||
				...
				/*
				 * Here we mark bp as needing to be appended by
				 * putq_common. Since stp->sd_lock was acquired
				 * above and will not be released until after
				 * the call to putq_common, the MSG*NEXT flags
				 * are safe to set below because they cannot be
				 * consumed until sd_lock is dropped.
				 */
				link_to_qlast = B_TRUE;
	...
	if (bp == NULL) {
		...
		case M_PASSFP:
			...
			if (bp->b_flag & MSGMARK)
				stp->sd_mark = bp;
			(void) putq_common(q, bp, link_to_qlast);
...
strrput_nondata(queue_t *q, mblk_t *bp)
{
	...
	case M_HANGUP:
		...
		strhup(stp);
		/*
		 * If this is the controlling tty send signal to registered
		 * processes / process groups (unless the downstream
		 * device wishes to silently hang up).
		 */
		if ((!stp->sd_sidp) ||
		    (((MBLKL(bp)) == 1) && (*bp->b_rptr == NOSIGNAL))) {
			mutex_exit(&stp->sd_lock);
			freemsg(bp);
			return (0);
		}
	...
	default:
		...
		if (((cmd & IOCTYPE) == LDIOC) ||
			...
			case TIOCSTI:
				...
				strioc.ic_dp = (char *)arg;
				/*
				 * We mustn't send STI to a more privileged
				 * reader. STRIOCTL_CRED causes our
				 * credentials to be carried in the message.
				 */
				return (strdoioctl(stp, &strioc, flag,
				    copyflag | STRIOCTL_CRED, crp, rvalp));
	...
	int *rvalp)
{
	...
	int credflag = (flag & STRIOCTL_CRED);
	int errs;
	uint_t waitflags;
...
#define	_LASTMARK	0x8000	/* Distinct from MSG*MARK */
	...
	for (;;) {
		...
		if (!(*flagsp & (MSG_HIPRI|MSG_BAND))) {
			...
			bp = strget(stp, q, uiop, first, &error, B_FALSE);
			ASSERT(MUTEX_HELD(&stp->sd_lock));
			if (bp != NULL) {
				if (DB_TYPE(bp) == M_SIG) {
		 ...
		 */
		    ...
		    (bp = getq_noenab(q, 0, B_TRUE)) != NULL) {

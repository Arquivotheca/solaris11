 ...
 * 	Create a hash using strings as keys
 *
 *	The string hashing algorithm is from Fowler / Noll / Vo (FNV-1a)
...
{
	/*
	 * hash needs to have an offset, otherwise all keys consisting only of
	 * zeros will end up in a hash = 0. It's not terribly important in this
	 * particular case but it doesn't hurt either. The authors of the FNV
	 * hash suggest using a hash with offset = 0 (FNV-0a) of any string
	 * where the number of characters matches the number of hash bits. Here
	 * we use the FNV-0a hash of "At night it's colder than abroad"
	 */
	uint_t hash = 3694316771;
	uint_t fnv_prime = (1<<24) + (1<<8) + 0x93;
	char *p, *k = (char *)key;
...
/*
 * Interface used by vm_getusage() to destroy hash and clear
 * without freeing mod_hash_entry.
 */
void
i_mod_hash_destroy_clear(mod_hash_t *hash)
{
	mod_hash_t *mhp, *mhpp;
	...
	i_mod_hash_clear_nosync(hash, 1);
 ...
 */
void
i_mod_hash_clear_nosync(mod_hash_t *hash, uint_t free)
{
	int i;
...
mod_hash_clear(mod_hash_t *hash)
{
	ASSERT(hash);
	rw_enter(&hash->mh_contents, RW_WRITER);
	i_mod_hash_clear_nosync(hash, 0);
	rw_exit(&hash->mh_contents);
}

ulong_t
mod_hash_nelems(mod_hash_t *hash)
{
	ulong_t nelems;

	ASSERT(hash);
	rw_enter(&hash->mh_contents, RW_READER);
	nelems = hash->mh_stat.mhs_nelems;
	rw_exit(&hash->mh_contents);

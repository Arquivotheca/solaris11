    ...
    cred_t *fcred)
{
	...
	/*
	 * We don't support special files such as /dev/null.
	 */
	if (vn_matchops(vp, spec_getvnodeops())) {
		MOBJ_STAT_ADD(map_flat_enodev);
		return (ENODEV);
	}
    ...
    uint_t *num_mapped, struct exec *hdr, cred_t *fcred)
{
	 ...
	 */
	osize = P2ROUNDUP(hdr->a_text, (uint32_t)PAGESIZE) + hdr->a_data +
	    hdr->a_bss;

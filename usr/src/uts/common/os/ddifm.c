 ...
 * the IO framework will initialize FM state and return a bit mask of the
 ...
 * via the 'devid' argument. The caller needs to control whether the
 ...
 * whether MPXIO is enabled, and whether a transport or non-transport
    ...
    nvlist_t *pl, va_list ap)
{
	...
	fm_fmri_dev_set(detector, FM_DEV_SCHEME_VERSION,
	    ddi_cna_dev(dip), NULL, path, devid, tpl0);
 ...
 * When called with the following capabilities, data structures necessary
...
ddi_fm_init(dev_info_t *dip, int *fmcap, ddi_iblock_cookie_t *ibcp)
{
	...
	i_ddi_fm_init(dip, fmcap, ibcp);
}

void
i_ddi_fm_init(dev_info_t *dip, int *fmcap, ddi_iblock_cookie_t *ibcp)
{
	...
	if ((instance = ddi_get_instance(dip)) != -1) {

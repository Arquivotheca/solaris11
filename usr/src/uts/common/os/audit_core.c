...
audit_init()
{
	...
	au_pathhold(pad0->pad_cwd);

	/*
	 * The kernel allocates a bunch of threads make sure they have
	 * a valid tad.  There may also be processes already running.
	 * Make sure they have a valid pad.
	 */
...
audit_newproc(struct proc *cp)	/* initialized child proc structure */
{
	...
	if (((t_audit_data_t *)T2A(curthread))->tad_flag) {
		 ...
		 */
		(void) audit_finish(0,
		    ((t_audit_data_t *)T2A(curthread))->tad_scid, 0, NULL);
	}
}
...
{
	return ((audit_active == C2AUDIT_LOADED) &&
	    (P2A(curproc)->pad_flags & PAD_KAUDIT));
}

void
audit_setpflags(proc_t *p)
{
	if (audit_active == C2AUDIT_LOADED) {
		p_audit_data_t *pad = P2A(p);

...
int kmem_sysdc_disabled = 0;	/* sysdc taskqs enabled by default */
uint_t kmem_dc = 75;		/* kmem sysdc taskq duty cycle */
...
static taskq_t		*kmem_taskq = NULL;
...
static kmutex_t		kmem_taskq_lock;
static kcondvar_t	kmem_taskq_cv;
 ...
 * changing kmem cache state while memory is being saved to the dump
 * device.
 *
 * If the alternate heap overflows then fall back to normal
 * alloc. However, mdb will report problems, such as:
 *	::kmem_verify may report "corrupt buffers"
 *	::findleaks may report "failed to read slab"
 *
 * The leaks are usually real, even though the apparent corruption may
 * not be. The heap size is set with dump_kmem_heap, which may be set
 * via /etc/system. Increase this value until the heap overflow
 * situation can be diagnosed.
 *
 * Note that there are no locks because only one CPU calls kmem during
 * a crash dump. To enable this feature, first create the associated
 * vmem arena with VMC_DUMPSAFE.
 */
kmem_dump_t kmem_dump;
kmem_dump_log_t kmem_dump_log[KMEM_DUMP_LOGS];

#define	KMEM_DUMPBUF(cp, buf)						\
	(KMEM_DUMP(cp) &&						\
	    (char *)(buf) >= (char *)kmem_dump.kdd_start &&		\
	    (char *)(buf) < (char *)kmem_dump.kdd_end)

#define	KDI_LOG(kdd, cp, stat)						\
{									\
	kmem_dump_log_t *kdl;						\
	if ((kdl = (cp)->cache_dumplog) != NULL) {			\
		if (kdl != KMEM_NODUMPLOG) {				\
			kdl->stat++;					\
		}							\
	} else if ((kdd)->kdd_log_idx < KMEM_DUMP_LOGS) {		\
		kdl = &kmem_dump_log[(kdd)->kdd_log_idx++];		\
		kdl->stat++;						\
		kdl->kdl_cache = (cp);					\
		(cp)->cache_dumplog = kdl;				\
	} else {							\
		(cp)->cache_dumplog = KMEM_NODUMPLOG;			\
	}								\
}

#define	KDI_LOG_ALLOC(kdd, cp, stat, used)				\
{									\
	KDI_LOG(kdd, cp, stat);						\
	(kdd)->kdd_used += (used);					\
	if ((kdd)->kdd_used > (kdd)->kdd_hiwater)			\
		(kdd)->kdd_hiwater = (kdd)->kdd_used;			\
}

#define	KDI_LOG_FREE(kdd, cp, stat, used)				\
{									\
	KDI_LOG(kdd, cp, stat);						\
	if ((used) <= (kdd)->kdd_used)					\
		(kdd)->kdd_used -= (used);				\
	else								\
		(kdd)->kdd_used = 0;					\
}

#define	KDI_LOG_OVERSIZED(kdd, size)					\
{									\
	(kdd)->kdd_oversize_allocs++;					\
	if ((size) > (kdd)->kdd_oversize_max)				\
		(kdd)->kdd_oversize_max = (size);			\
}

#define	KDI_LOG_OVERFLOW(kdd, cp)					\
{									\
	(kdd)->kdd_overflows++;						\
...
{
	kmem_dump_t *kdd = &kmem_dump;

	if (kdd->kdd_start != NULL)
		kmem_free(kdd->kdd_start, kdd->kdd_size);

	kdd->kdd_start = kmem_alloc(size, KM_SLEEP);

	if (kdd->kdd_start != NULL) {
		kdd->kdd_size = size;
		kdd->kdd_curr = kdd->kdd_start;
		kdd->kdd_end = (void *)((char *)kdd->kdd_start + size);
		copy_pattern(KMEM_UNINITIALIZED_PATTERN, kdd->kdd_start, size);
	} else {
		kdd->kdd_size = 0;
		kdd->kdd_curr = NULL;
		kdd->kdd_end = NULL;
 ...
 * create patch records in the dump file so that the crash image is up to date
 */
void
kmem_dump_patch(void (*patch)(void *, size_t), size_t *shortage)
{
	kmem_dump_t *kdd = &kmem_dump;
	kmem_cache_t *cp;

	patch(kdd, sizeof (kmem_dump_t));
	patch(&kmem_dump_log, sizeof (kmem_dump_log));

	for (cp = list_head(&kmem_caches); cp != NULL;
	    cp = list_next(&kmem_caches, cp)) {
		if (cp->cache_dumplog || (cp->cache_flags & KMF_DUMPOVERFLOW))
			patch(cp, sizeof (kmem_cache_t));
	}

	if (kdd->kdd_start != NULL)
		patch(kdd->kdd_start, (char *)kdd->kdd_curr -
		    (char *)kdd->kdd_start);

	*shortage = kdd->kdd_hiwater > kdd->kdd_size ?
	    kdd->kdd_hiwater - kdd->kdd_size : 0;
}
...
kmem_cache_alloc_dump(kmem_cache_t *cp, int kmflag)
{
	...
	if ((buf = cp->cache_dumpfreelist) != NULL) {
		...
		KDI_LOG_ALLOC(kdd, cp, kdl_allocs, cp->cache_bufsize);
		return (buf);
	}
	...
	if (bufend > (char *)kdd->kdd_end) {
		kdd->kdd_curr = kdd->kdd_end;
		KDI_LOG_ALLOC(kdd, cp, kdl_overflows, cp->cache_bufsize);
		KDI_LOG_OVERFLOW(kdd, cp);
		return (NULL);
	}
	...
	if (cp->cache_constructor != NULL &&
		...
		if (kdd->kdd_curr == bufend)
			kdd->kdd_curr = curr;

		/* fall back to normal alloc if the constructor fails */
		KDI_LOG(kdd, cp, kdl_overflows);
		KDI_LOG_OVERFLOW(kdd, cp);
		return (NULL);
	}
...
kmem_cache_free_dump(kmem_cache_t *cp, void *buf)
{
	...
	if ((char *)buf >= (char *)kdd->kdd_start &&
	    (char *)buf < (char *)kdd->kdd_end) {
		KMEM_DUMPCTL(cp, buf)->kdc_next = cp->cache_dumpfreelist;
		cp->cache_dumpfreelist = buf;
		KDI_LOG_FREE(kdd, cp, kdl_frees, cp->cache_bufsize);
		return (0);
	}

	/* count all non-dump buf frees */
	KDI_LOG_FREE(kdd, cp, kdl_free_nondump, cp->cache_bufsize);

	/* just drop buffers that were allocated before dump started */
	if (kdd->kdd_curr < kdd->kdd_end)
		return (0);
		...
		if (ccp->cc_rounds > 0) {
			...
			if (ccp->cc_flags & (KMF_BUFTAG | KMF_DUMPUNSAFE)) {
				if (ccp->cc_flags & KMF_DUMPUNSAFE) {
					ASSERT(!(ccp->cc_flags &
					    KMF_DUMPDIVERT));
					KDI_LOG(&kmem_dump, cp, kdl_allocs);
				}
...
kmem_cpucache_magazine_alloc(kmem_cpu_cache_t *ccp, kmem_cache_t *cp)
{
	 ...
	 *
	 * In low-memory situations, it is better to fail the magazine
	 * allocation and fall back to the slab layer, than to allocate
	 * the last page on the system and use it to cache even more
	 * memory.  We use KM_NORMALPRI to undo the implicit "high priority"
	 * semantic which comes with KM_NOSLEEP, as well as any ability
	 * to tap the pageout reserve.
	 */
	mtp = cp->cache_magtype;
	mutex_exit(&ccp->cc_lock);
	emp = kmem_cache_alloc(mtp->mt_cache, KM_NOSLEEP | KM_NORMALPRI);
	mutex_enter(&ccp->cc_lock);
...
kmem_cache_free(kmem_cache_t *cp, void *buf)
{
	...
	if (ccp->cc_flags & (KMF_BUFTAG | KMF_DUMPDIVERT | KMF_DUMPUNSAFE)) {
		if (ccp->cc_flags & KMF_DUMPUNSAFE) {
			ASSERT(!(ccp->cc_flags & KMF_DUMPDIVERT));
			/* log it so that we can warn about it */
			KDI_LOG(&kmem_dump, cp, kdl_frees);
...
kmem_alloc(size_t size, int kmflag)
{
	...
	if ((index = ((size - 1) >> KMEM_ALIGN_SHIFT)) < KMEM_ALLOC_TABLE_MAX) {
		...
		if (buf == NULL)
			...
			KDI_LOG_OVERSIZED(&kmem_dump, size);
		}
		return (buf);
	}
...
kmem_free(void *buf, size_t size)
{
	...
	if ((cp->cache_flags & KMF_BUFTAG) && !KMEM_DUMPBUF(cp, buf)) {
		kmem_buftag_t *btp = KMEM_BUFTAG(cp, buf);
...
static void
kmem_cache_purge(kmem_cache_t *cp)
{
...
void
kmem_cache_reap_all_now(kmem_cache_t *cp)
{
	ASSERT(list_link_active(&cp->cache_link));
	...
	(void) taskq_dispatch(kmem_taskq,
	    (task_func_t *)kmem_cache_purge, cp, TQ_SLEEP);
	taskq_wait(kmem_taskq);
}
...
kmem_cache_magazine_resize(kmem_cache_t *cp)
{
	...
	if (cp->cache_chunksize < mtp->mt_maxbuf &&
	    cp->cache_magsize_max > mtp[1].mt_magsize) {
		kmem_cache_magazine_purge(cp);
		mutex_enter(&cp->cache_depot_lock);
...
kmem_cache_update(kmem_cache_t *cp)
{
	...
	if (cp->cache_chunksize < cp->cache_magtype->mt_maxbuf &&
	    cp->cache_magsize_max > cp->cache_magtype[1].mt_magsize &&
	...
	int cflags)		/* cache creation flags */
{
	 ...
	 * is aligned on a FALSE_SHARING_ALIGN boundary to prevent
	 * false sharing of per-CPU data.
	 */
	cp = vmem_xalloc(kmem_cache_arena, csize, FALSE_SHARING_ALIGN,
	    P2NPHASE(csize, FALSE_SHARING_ALIGN), 0, NULL, NULL, VM_SLEEP);
	bzero(cp, csize);
...
void
kmem_cache_set_magsize_max(kmem_cache_t *cp, short magsize_max)
{
	cp->cache_magsize_max = magsize_max;
}
...
kmem_init(void)
{
	...
	/* LINTED */
	ASSERT3U(P2PHASE(sizeof (kmem_cpu_cache_t), FALSE_SHARING_ALIGN), ==,
	    0);
	...
	mutex_init(&kmem_taskq_lock, NULL, MUTEX_DEFAULT, NULL);
	cv_init(&kmem_taskq_cv, NULL, CV_DEFAULT, NULL);
	 ...
	 * KMF_LITE records separate from the tunable.  We arbitrarily clamp
...
kmem_sysdc_pinit()
{
	...
	(void) strncpy(pu->u_psargs, KMEM_SYSDC_NAME, sizeof (pu->u_psargs));
	(void) strncpy(pu->u_comm, KMEM_SYSDC_NAME, sizeof (pu->u_comm));

	ntq = taskq_create_sysdc("kmem_taskq", 1, 300, INT_MAX,
	    curproc, kmem_dc, TASKQ_PREPOPULATE);

	mutex_enter(&kmem_taskq_lock);
	kmem_taskq = ntq;
	cv_signal(&kmem_taskq_cv);
	mutex_exit(&kmem_taskq_lock);
...
kmem_sysdc_init()
{
	if (kmem_sysdc_disabled) {
		kmem_taskq = taskq_create_instance("kmem_taskq", 0, 1,
		    minclsyspri, 300, INT_MAX, TASKQ_PREPOPULATE);
		return;
	}

	if (newproc(kmem_sysdc_pinit, NULL, syscid, minclsyspri, NULL,
	    0) != 0) {
		panic("kmem_sysdc_init: unable to fork for pinit");
	}

	/* Wait for kmem process to initalize the taskq */
	mutex_enter(&kmem_taskq_lock);
	while (kmem_taskq == NULL)
		cv_wait(&kmem_taskq_cv, &kmem_taskq_lock);
	mutex_exit(&kmem_taskq_lock);
}
...
kmem_cache_move_notify(kmem_cache_t *cp, void *buf)
{
	...
	if (args != NULL) {
		args->kmna_cache = cp;
		args->kmna_buf = buf;
		if (kmem_taskq == NULL || !taskq_dispatch(kmem_taskq,
...
kmem_cache_scan(kmem_cache_t *cp)
{
	...
	if (kmem_cache_is_fragmented(cp, &reap)) {
		...
		if (!avl_is_empty(&cp->cache_partial_slabs)) {
			...
			(void) random_get_pseudo_bytes((uint8_t *)&debugrnd, 2);
			if (!kmem_move_noreap &&
			    ((debugrnd % kmem_mtb_reap) == 0)) {
				mutex_exit(&cp->cache_lock);
				KMEM_STAT_ADD(kmem_move_stats.kms_debug_reaps);
				kmem_cache_reap(cp);
				return;
			} else if ((debugrnd % kmem_mtb_move) == 0) {
				KMEM_STAT_ADD(kmem_move_stats.kms_scans);

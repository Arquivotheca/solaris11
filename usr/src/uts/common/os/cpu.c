 ...
 */
void
thread_affinity_set(kthread_t *t, int cpu_id)
{
	cpu_t *cp;
	int id;
...
cpu_pause_alloc(cpu_t *cp)
{
	...
	/*
	 * Pause threads are not transient.
	 */
...
cpu_online(cpu_t *cp)
{
	...
	if (error == 0) {
		...
		/*
		 * This has to be called only after cyclic_online() as it
		 * uses cyclics.
		 */
	...
	cpupart_update_online_ids(cp->cpu_part, NULL, cp);
...
cpu_offline(cpu_t *cp, int flags)
{
	...
	int		loop_count;
	int		no_quiesce = 0;
	uint_t		bound_flags = DISP_BOUND_CPU;
	kthread_t	*t;
	lpl_t		*cpu_lpl;
	proc_t		*p;
	int		lgrp_diff_lpl;
	 ...
	 * state). If the test finds some bound threads, wait a few ticks
	 ...
	 */
	if (no_quiesce == 0)
		bound_flags |= DISP_BOUND_INTR;

again:	for (loop_count = 0; disp_bound_check(cp, B_FALSE, bound_flags) != NULL;
	    loop_count++) {
		if (loop_count >= 5) {
		...
		/*
		 * Give bound threads a chance to complete their work or
		 * get scheduled on a different CPU. Note that we still hold
		 * cpu_lock, which is okay since it isn't needed for any
		 * performance critical work. This is necessary to prevent
		 * another p_online(2) from offlining the last processor.
		 */
		delay(hz/100);
	}
	...
	if (error == 0) {
		...
		if ((!no_quiesce && cp->cpu_intr_actv != 0) ||
		    disp_bound_check(cp, B_TRUE, bound_flags)) {
			start_cpus();
...
out:
	...
	cpupart_update_online_ids(NULL, cp->cpu_part, cp);
...
cpu_list_init(cpu_t *cp)
{
	...
	cp_default.cp_online_ids[CP_ONLINE_LOWER] = cp->cpu_seqid;
	cp_default.cp_online_ids[CP_ONLINE_UPPER] = cp->cpu_seqid;
	cp->cpu_next_part = cp;
	cp->cpu_prev_part = cp;
...
cpu_add_active_internal(cpu_t *cp)
{
	...
	cpupart_update_online_ids(pp, NULL, cp);
...
/*
 * Remove a CPU from the list of active CPUs. This routine should not block
 * since CPUs are paused.
 */
static void
cpu_remove_active(cpu_t *cp)
{
	...
	/*
	 * The given CPU must not be the last online in the system.
	 */
	ASSERT(cp->cpu_next_onln != cp);
	ASSERT(cp->cpu_prev_onln != cp);
...
cpu_configure(int cpuid)
{
	...
	/*
	 * Notify the PG framework that a new CPU is coming online so
	 * any new class can configure the priority to utilization mapping,
	 * or in case the existing mapping needs to be updated.
	 */
    ...
    int *error)
{
	...
	if (tp->t_affinitycnt == 0) {
		if (binding == PBIND_NONE) {
			 ...
			 * after unbinding
			 */
			if (tp->t_lgrp_aff && tp->t_lgrp_aff->aff)
				lgrp_move_thread(tp,
...
cpu_get_state(cpu_t *cpu)
{
	...
	if (cpu->cpu_flags & CPU_POWEROFF)
		...
		return (P_SPARE);
	else if (cpu->cpu_flags & CPU_POWERIDLE)
		return (P_POWERIDLE);

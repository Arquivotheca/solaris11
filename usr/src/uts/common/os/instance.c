...
e_ddi_borrow_instance(dev_info_t *cdip, in_node_t *cnp)
{
	...
	/*
	 * During the initial path_to_inst discovery the system is
	 * being discovered for the first time. Translations don't
	 * make sense as there is no previous path to translate from.
	 *
	 * The dynamic case (currently unimplemented) will provide an
	 * explicit end to this state at the time of the first DR.
	 */
	if (pti_rebuild)
		return;
	...
	/* bail here if the alias matches itself */
	...
	if (alias && (anp = e_ddi_path_to_instance(alias)) != NULL) {
		 ...
		 * to borrow an instance for them. However since they do

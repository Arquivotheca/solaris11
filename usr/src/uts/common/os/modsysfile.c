...
struct hwc_parse_mt {
	...
	int		use_path;	/* use module search path */
	int		rv;		/* return value */
};

static int hwc_parse_now(char *, struct par_list **, ddi_prop_t **, int);
static void hwc_parse_thread(struct hwc_parse_mt *);
static struct hwc_parse_mt *hwc_parse_mtalloc(char *,
    struct par_list **, ddi_prop_t **, int);
...
hwc_free(struct hwc_spec *hwcp)
{
	...
	i_ddi_prop_list_delete(hwcp->hwc_devi_vdr_prop_ptr);
	i_ddi_prop_list_delete(hwcp->hwc_devi_adm_prop_ptr);
 ...
 */
int
hwc_parse(char *fname, struct par_list **pl, ddi_prop_t **props, int use_path)
{
	int ret;
	struct hwc_parse_mt *pltp;

	pltp = hwc_parse_mtalloc(fname, pl, props, use_path);
	...
	if (curthread != &t0) {
		...
		pltp->rv = hwc_parse_now(fname, pl, props, use_path);
	}
	ret = pltp->rv;
...
struct _buf *pciconf_file = (struct _buf *)(-1);
int64_t	pciconf_file_last_atime = 0;
void
read_pciconf_file()
{
	char	*pciconf_buf;
	...
	file = kobj_open_file(PCICONF_FILE);
	if (file == (struct _buf *)-1) {
		return;
	}
	pciconf_buf = kmem_zalloc(MAX_PCICONF_FILE_SIZE, KM_SLEEP);
	for (i = 0; i < MAX_PCICONF_FILE_SIZE; i++) {
		c = kobj_getc(file);
		if (c == -1)
			break;
		count++;
		pciconf_buf[i] = (char)c;
	}
	pciconf_file = kmem_zalloc(sizeof (struct _buf), KM_SLEEP);
	pciconf_file->_name = kmem_alloc(strlen(PCICONF_FILE) + 1, KM_SLEEP);
	pciconf_file->_cnt = pciconf_file->_size = count;
	pciconf_file->_off = 0;
	pciconf_file->_ln = 1;
	pciconf_file->_ptr = pciconf_file->_base = pciconf_buf;
	(void) strcpy(pciconf_file->_name, PCICONF_FILE);
	pciconf_file->_bsize = 0;
	pciconf_file->_iscmp = 0;
	kobj_close_file(file);
	if (modrootloaded) {
		mutex_enter(&tod_lock);
		ts = tod_get();
		mutex_exit(&tod_lock);
		pciconf_file_last_atime = ts.tv_sec;
	} else {
		pciconf_file_last_atime = 0;
	}
}
...
hwc_parse_thread(struct hwc_parse_mt *pltp)
{
	...
	pltp->rv = hwc_parse_now(pltp->name,
	    pltp->pl, pltp->props, pltp->use_path);
	sema_v(&pltp->sema);
	mutex_enter(&cpr_lk);
...
hwc_parse_mtalloc(char *name,
    struct par_list **pl, ddi_prop_t **props, int use_path)
{
    ...
    struct par_list **pl, ddi_prop_t **props, int use_path)
{
	...
	/*
	 * Don't use kobj_open_path's use_moddir_suffix option.
	 * We only expect to find the appropriate conf file for
	 * a driver via the default module search path, or
	 * a fixed location, but not an ISA-specific subdirectory.
	 */
	if ((file = kobj_open_path(fname, use_path, 0)) ==
	    (struct _buf *)-1) {
...
add_spec(struct hwc_spec *spec, struct par_list **par)
{
	 ...
	 * class is not NULL, we match the pl by class name.
	 */

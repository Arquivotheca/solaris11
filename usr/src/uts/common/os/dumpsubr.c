...
uint_t dump_helpwait = 5;	/* how many secs to wait for helpers */
...
uint_t dump_kmem_heap = 128*1024;
...
/*
 * Compression metrics are accumulated nano-second subtotals. When
 * the dump completes the metrics are sub-totaled into dumpsync. A
 * patch record is generated which updates dumpsync in the crash
 * image, so that metrics may be retrieved with mdb.
 */
#define	PERPAGES \
 ...
 * performance information. The values are recorded in dumpsync_t and
 * helper_t instances. After the memory dump completes a patch record
 * is generated so that the final values make their way into the dump
 * image.
 */
...
dump_update_clevel()
{
	 ...
	 * created, and the I/O path allocates buffers and data
	 * structs.
	 *
	 * syslog messages are saved in kmem (log_consq) until
	 * recorded by dump_messages.
	 */
	kmem_dump_init((new->ncmap * dump_kmem_permap) + dump_kmem_heap);
		...
		if (VOP_OPEN(&cdev_vp, FREAD | FWRITE, kcred, NULL) == 0) {
			...
			if (strcmp(dki.dki_dname, ZVOL_DRIVER) == 0) {
				if (IS_SWAPVP(common_specvp(cvp)))
				    ...
				    DKIOCPREALLOCINIT, NULL, FKIOCTL,
				    kcred, NULL, NULL)) != 0)
					dumpfini();
			}
...
dumpfini(void)
{
	...
	if (is_zfs &&
		...
		if (VOP_OPEN(&cdev_vp, FREAD | FWRITE, kcred, NULL) == 0) {
			(void) VOP_IOCTL(cdev_vp, DKIOCPREALLOCFINI, NULL,
...
/*
 * Create patch records for size bytes starting at buf.
 */
void
dump_patch(void *buf, size_t size)
{
	char *cbuf = dumpcfg.cbuf[0].buf;
	char *dbuf = cbuf + PAGESIZE;
	caddr_t addr;
	size_t n;
	size_t off;
	pfn_t pfn;
	...
	ASSERT(dumpcfg.cbuf[0].size >= (PAGESIZE *2));

	/* write terminator */
	if (buf == NULL) {
		bzero(&pat, sizeof (pat));
		pat.patch_magic = DUMP_PATCH_MAGIC;
		dumpvp_write(&pat, sizeof (pat));
		return;
	}

	/* compress data by pages */
	for (addr = buf; size > 0; size -= n, addr += n) {
		...
		bcopy(addr, dbuf, n);
		bzero(dbuf + n, PAGESIZE - n);
		pat.patch_csize = compress(dbuf, cbuf, PAGESIZE);
		pat.patch_sum = checksum32(cbuf, pat.patch_csize);
		...
		dumpvp_write(&pat, sizeof (pat));
		dumpvp_write(cbuf, pat.patch_csize);
	}
}
...
dumpsys_main_task(void *arg)
{
	 ...
	 *
	 * Enter with helper_lock held. Helpers can be spinning
	 * already, or come later, or never show. We can get by with
	 * at least one helper, but have to go to serial mode if there
	 * are none. If we do switch to serial mode, late arrivers go
	 * away because helpers_wanted=0. If we stay in parallel mode,
	 * it is OK if other helpers show up late. They will be
	 * controlled by the queues at that point.
	 */
	dumpserial = 1;
	dumpcfg.helpers_wanted = dumpcfg.clevel > 0;

	if (dumpcfg.helpers_wanted && dump_plat_mincpu != 0) {

		dumpsys_spinunlock(&dumpcfg.helper_lock);
		delay(1);
		dumpsys_spinlock(&dumpcfg.helper_lock);

		for (tries = 0; tries < dump_helpwait; tries++) {
...
dumpsys(void)
{
	...
	/*
	 * kmem_alloc may allocate new kernel pages. Divert kmem to
	 * use the dump heap before building the bitmap. Any pages
	 * allocated after the bitmap is set will not have translations
	 * in the dump file (mdb: no mapping for address.)
	 */
	if (panicstr) {
		kmem_dump_begin();
	}
	...
	/* mark end of stream blocks */
	datatag = 0;
	dumpvp_write(&datatag, sizeof (datatag));

	/* sub-total metrics counters */
#ifdef COLLECT_METRICS
#define	PERPAGE(x) dumpsync.perpage.x += hp->perpage.x;
	for (hp = dumpcfg.helper; hp != hpend; hp++) {
		PERPAGES;
	}
#undef PERPAGE
	...
	/* append patch records after the data stream */
	if (panicstr) {
		size_t shortage;

		dump_patch(&dumpsync, sizeof (dumpsync));
		dump_patch(&dumpcfg, sizeof (dumpcfg));
		dump_patch(dumpcfg.helper, sizeof (helper_t) * dumpcfg.nhelper);
		dump_patch(dumpcfg.helpermap, BT_SIZEOFMAP(NCPU));
		kmem_dump_patch(&dump_patch, &shortage);
		dump_patch(NULL, 0);	/* flag end of patches */

		if (shortage > 0) {
			uprintf("Warning: dump heap is too small: "
			    "add this to /etc/system: "
			    "set dump_kmem_heap=0x%lx\n",
			    P2ROUNDUP(dump_kmem_heap + shortage, 1024));
		}
	}

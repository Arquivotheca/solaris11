 ...
 */
static int ddihp_copyin_changestate_arg(uintptr_t arg,
...
static int ddihp_cn_post_change_state(ddi_hp_cn_handle_t *hdlp,
    ddi_hp_cn_state_code_t new_state);
    ...
    uintptr_t rval)
{
	...
	if (hp_op == DDI_HPOP_CN_CREATE_PORT) {
		...
		if (hp_op == DDI_HPOP_CN_CHANGE_STATE) {
			...
			/*
			 * Here we only copy in the state_code and the pointer
			 * to the state_priv info, bus nexus driver will copy
			 * in the state_priv data itself
			 */
			if (ddihp_copyin_changestate_arg(arg, &karg) != 0) {
				cmn_err(CE_WARN, "ddihp_modctl: changestate "
				    "copyin failed\n");
				error = EINVAL;
				goto done;
			}

			DDIHP_CN_OPS(hdlp, op, (void *)&karg,
			    (void *)&result_state, rv);

			DDI_HP_IMPLDBG((CE_CONT, "ddihp_modctl: target_state="
			    "%x, result_state=%x, rv=%x \n",
			    karg.target_state.state_code, result_state, rv));
		} else {
...
/*
 * If current state has bus specific state private data, get it.
 * Currently, only SUSPENDED state has such data.
 *
 * rval is an input/output argument points to ddi_hp_state_priv_t.
 */
int
ddihp_cn_get_state_priv(ddi_hp_cn_handle_t *hdlp, void *rval)
{
	(void) ddihp_cn_getstate(hdlp);

	return (ddihp_connector_ops(hdlp,
	    DDI_HPOP_CN_GET_STATE_PRIV, NULL, rval));
}
...
/*
 * Process the hotplug operations for Connectors, and also some Port
 * operations (create port, check the existence of port's device, etc.)
 * upon user command.
 */
int
    ...
    void *arg, void *result)
{
	...
	if (op == DDI_HPOP_CN_CHANGE_STATE) {
		...
		rv = ddihp_cn_pre_change_state(hdlp, kargp);
		if (rv != DDI_SUCCESS) {
	...
	if (op == DDI_HPOP_CN_CHANGE_STATE) {
		...
		DDI_HP_IMPLDBG((CE_CONT, "ddihp_connector_ops: "
		    ...
		    hdlp->cn_info.cn_state, *(ddi_hp_cn_state_code_t *)result,
		    rv));
    ...
    void *arg, void *result)
{
	...
	case DDI_HPOP_CN_GET_STATE:
	{
		...
		if (hdlp->cn_info.cn_child == NULL) {
			...
			*((ddi_hp_cn_state_code_t *)result) = state;

			break;
		}

		/* There is a child of this Port */
		...
		if (state == DDI_HP_CN_STATE_ONLINE) {
			dev_info_t *cdip = hdlp->cn_info.cn_child;

			if (DEVI_IS_DEVICE_LIVE_SUSPENDED(cdip))
				state = DDI_HP_CN_STATE_MAINTENANCE_SUSPENDED;
			else if (ddi_get_devstate(cdip) != DDI_DEVSTATE_UP)
				state = DDI_HP_CN_STATE_MAINTENANCE;
		}
	...
	{
		ddi_hp_cn_change_state_arg_t *kargp;
		ddi_hp_cn_state_code_t curr_state = hdlp->cn_info.cn_state;

		(void) ddihp_cn_getstate(hdlp);

		kargp = (ddi_hp_cn_change_state_arg_t *)arg;

		ret = ddihp_port_change_state(hdlp, kargp);
		if (curr_state != hdlp->cn_info.cn_state) {
			ddihp_update_last_change(hdlp);
		}
		*((ddi_hp_cn_state_code_t *)result) = hdlp->cn_info.cn_state;
	...
	{
		/* Probe the port's dependent devices */
		(void) ddihp_cn_getstate(hdlp);

		if (hdlp->cn_info.cn_state < DDI_HP_CN_STATE_ONLINE) {
			/*
			 * There is no device node created for this port yet,
			 * so can not probe its dependent devices.
			 */
			ret = DDI_FAILURE;
		...
		ret =
		    (*(DEVI(hdlp->cn_dip)->devi_ops->devo_bus_ops->bus_hp_op))(
		    hdlp->cn_dip, hdlp->cn_info.cn_name, DDI_HPOP_CN_PROBE,
		    NULL, NULL);
	...
	{
		/* Unprobe the port's dependent devices */
		(void) ddihp_cn_getstate(hdlp);

		if (hdlp->cn_info.cn_state < DDI_HP_CN_STATE_ONLINE) {
			/*
			 * There is no device node created for this port yet,
			 * so there must be no dependent devices for this port.
			 */
			ret = DDI_SUCCESS;
		...
		ret =
		    (*(DEVI(hdlp->cn_dip)->devi_ops->devo_bus_ops->bus_hp_op))(
		    hdlp->cn_dip, hdlp->cn_info.cn_name, DDI_HPOP_CN_UNPROBE,
		    NULL, NULL);
...
static int
ddihp_copyin_changestate_arg(uintptr_t arg,
    ddi_hp_cn_change_state_arg_t *kargp)
{
#ifdef _SYSCALL32_IMPL
	ddi_hp_cn_state32_t	target_state32;
#endif

	if (get_udatamodel() == DATAMODEL_NATIVE) {
		if (copyin((void *)arg, &kargp->target_state,
		    sizeof (ddi_hp_cn_state_t)) != 0) {
			cmn_err(CE_WARN, "ddihp_copyin_changestate_arg: "
			    "DATAMODEL_NATIVE copyin failed\n");
			return (DDI_EINVAL);
		}
	}
#ifdef _SYSCALL32_IMPL
	else {
		bzero(&kargp->target_state, sizeof (ddi_hp_cn_state_t));
		if (copyin((void *)arg, &target_state32,
		    sizeof (ddi_hp_cn_state32_t))) {
			cmn_err(CE_WARN, "ddihp_copyin_changestate_arg: "
			    "32-bit copyin failed\n");
			return (DDI_EINVAL);
		}
		kargp->target_state.state_code =
		    (ddi_hp_cn_state_code_t)target_state32.state_code;
		kargp->target_state.state_priv =
		    (void *)(uintptr_t)target_state32.state_priv;
	}
#endif

	kargp->type = ARG_PRIV_NVLIST;
    ...
    ddi_hp_cn_change_state_arg_t *kargp)
{
	...
	if (curr_state > kargp->target_state.state_code &&
    ...
    ddi_hp_cn_state_code_t new_state)
{
	...
	if (new_state == DDI_HP_CN_STATE_ENABLED) {
		if (curr_state == new_state) {
			/*
			 * Already enabled, so only try to online the devices.
			 * Ignore the return value here because the online
			 * status does not impact the ENABLED state.
			 * In the failure case, a warning message is printed
			 * to console by sub-routine.
			 */
...
ddihp_cn_handle_state_change(ddi_hp_cn_handle_t *hdlp)
{
	...
	if (rv != DDI_SUCCESS) {
		...
		karg.target_state.state_code = DDI_HP_CN_STATE_POWERED;
		karg.target_state.state_priv = NULL;
...
ddihp_cn_change_children_state(ddi_hp_cn_handle_t *hdlp, boolean_t online)
{
	...
	for (h = DEVI(dip)->devi_hp_hdlp; h; h = h->next) {
		if (h->cn_info.cn_type & DDI_HP_CN_TYPE_CONNECTOR_MASK)
			continue;
		...
		if (online) {
			...
			if (ndi_devi_offline(cdip, NDI_UNCONFIG) !=
				...
				cmn_err(CE_WARN,
				    ...
				    hdlp->cn_info.cn_name);
				/*
				 * Re-online children in case this sub-tree left
				 * in partial configured state (some offlined
				 * while some others still online).
				 */
				if (ndi_devi_online(cdip, NDI_ONLINE_ATTACH |
				    NDI_CONFIG) != NDI_SUCCESS) {
					cmn_err(CE_WARN, "(%s%d): failed to"
					    " re-attach driver for the device"
					    " (%s%d) under the Connection %s\n",
					    ddi_driver_name(dip),
					    ddi_get_instance(dip),
					    ddi_driver_name(cdip),
...
{
	ddi_hp_cn_state_code_t curr_state = hdlp->cn_info.cn_state;
	ddi_hp_cn_state_code_t target_state = kargp->target_state.state_code;

	if (target_state < DDI_HP_CN_STATE_PORT_EMPTY ||
	    target_state > DDI_HP_CN_STATE_ONLINE) {
		DDI_HP_IMPLDBG((CE_WARN, "invalid hotplug target state: %d\n",
		    (int)target_state));
		return (DDI_EINVAL);
	}

	if (curr_state == target_state) {
		DDI_HP_IMPLDBG((CE_NOTE, "target state equals to current state,"
		    " no hotplug state change.\n"));
		return (DDI_SUCCESS);
	}

	if (target_state == DDI_HP_CN_STATE_MAINTENANCE_SUSPENDED &&
	    curr_state != DDI_HP_CN_STATE_ONLINE) {
		DDI_HP_IMPLDBG((CE_WARN, "invalid hotplug target state(%d), "
		    "SUSPENDED state can only be reached from ONLINE state\n",
		    (int)target_state));
		return (DDI_EINVAL);
	}

	if (curr_state < target_state)
		return (ddihp_port_upgrade_state(hdlp, kargp));

	return (ddihp_port_downgrade_state(hdlp, kargp));
}
...
ddihp_port_upgrade_state(ddi_hp_cn_handle_t *hdlp,
    ddi_hp_cn_change_state_arg_t *kargp)
{
	ddi_hp_cn_state_code_t	curr_state, result_state;
	ddi_hp_cn_state_code_t	target_state;
	ddi_hp_cn_change_state_arg_t	tmp_arg;
	dev_info_t		*cdip;
	int			rv = DDI_SUCCESS;
	...
	while (curr_state < target_state) {
		...
		case DDI_HP_CN_STATE_PORT_EMPTY:
			...
			tmp_arg.target_state.state_code
			    = DDI_HP_CN_STATE_PORT_PRESENT;
			tmp_arg.target_state.state_priv = NULL;
			tmp_arg.type = kargp->type;
			rv = ddihp_connector_ops(hdlp,
			    DDI_HPOP_CN_CHANGE_STATE,
			    (void *)&tmp_arg, (void *)&result_state);
			if (rv == DDI_SUCCESS) {
				hdlp->cn_info.cn_state =
				    result_state;
			}
		...
		case DDI_HP_CN_STATE_MAINTENANCE_SUSPENDED:
			ASSERT(target_state == DDI_HP_CN_STATE_ONLINE);

			rv = ddihp_connector_ops(hdlp,
			    DDI_HPOP_CN_CHANGE_STATE,
			    (void *)kargp, (void *)&result_state);
			if (rv == DDI_SUCCESS) {
				hdlp->cn_info.cn_state =
				    result_state;
			}
    ...
    ddi_hp_cn_change_state_arg_t *kargp)
{
	...
	if (curr_state == DDI_HP_CN_STATE_ONLINE &&
	    target_state == DDI_HP_CN_STATE_MAINTENANCE_SUSPENDED) {
		rv = ddihp_connector_ops(hdlp,
		    DDI_HPOP_CN_CHANGE_STATE,
		    (void *)kargp, (void *)&result_state);
		if (rv == DDI_SUCCESS) {
...
{
	hdlp->cn_info.cn_last_change = (time_t)ddi_get_time();
}

...
#pragma align FALSE_SHARING_ALIGN(callout_table)
...
callout_cpu_online(cpu_t *cp)
{
	...
	if (cache == NULL) {
		...
		cache->cc_cache = kmem_cache_create(s, sizeof (callout_t),
		    FALSE_SHARING_ALIGN, NULL, NULL, NULL, NULL, NULL, 0);
...
callout_init(void)
{
	...
	size += FALSE_SHARING_ALIGN;
	buf = (uintptr_t)kmem_zalloc(size, KM_SLEEP);
	callout_table = (callout_table_t *)P2ROUNDUP(buf, FALSE_SHARING_ALIGN);

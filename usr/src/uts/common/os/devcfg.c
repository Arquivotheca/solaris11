...
/*
 * The cached devinfo snapshot and related variables.
...
/*
 * Add flag so behavior of preventing attach for retired persistent nodes
 * can be disabled.
 */
...
/*
 * Allocate/free per-device prop lists derived from parent spec
 */
static int
i_ddi_alloc_devi_sys_props(struct dev_info *devi, hwc_spec_t *specp, int flag)
{
	ddi_prop_t *props;

	ASSERT(specp != NULL);
	ASSERT(devi->devi_sys_prop_ptr == NULL);
	ASSERT(devi->devi_vdr_prop_ptr == NULL);
	ASSERT(devi->devi_adm_prop_ptr == NULL);

	if (specp->hwc_devi_sys_prop_ptr) {
		if ((props = i_ddi_prop_list_dup(
		    specp->hwc_devi_sys_prop_ptr, flag)) == NULL)
			return (0);
		devi->devi_sys_prop_ptr = props;
	}

	if (specp->hwc_devi_vdr_prop_ptr) {
		if ((props = i_ddi_prop_list_dup(
		    specp->hwc_devi_vdr_prop_ptr, flag)) == NULL)
			return (0);
		devi->devi_vdr_prop_ptr = props;
	}

	if (specp->hwc_devi_adm_prop_ptr) {
		if ((props = i_ddi_prop_list_dup(
		    specp->hwc_devi_adm_prop_ptr, flag)) == NULL)
			return (0);
		devi->devi_adm_prop_ptr = props;
	}
	return (1);
}

static void
i_ddi_free_devi_sys_props(struct dev_info *devi)
{
	if (devi->devi_sys_prop_ptr)
		i_ddi_prop_list_delete(devi->devi_sys_prop_ptr);
	if (devi->devi_vdr_prop_ptr)
		i_ddi_prop_list_delete(devi->devi_vdr_prop_ptr);
	if (devi->devi_adm_prop_ptr)
		i_ddi_prop_list_delete(devi->devi_adm_prop_ptr);
}
...
i_ddi_alloc_node(dev_info_t *pdip, char *node_name, pnode_t nodeid,
    int instance, hwc_spec_t *specp, int flag)
{
	...
	/*
	 * Make a copy of system and other properties
	 * if this node is derived via conf spec.
	 */
	if (specp &&
	    i_ddi_alloc_devi_sys_props(devi, specp, flag) == 0)
		goto fail;
...
sid:		devi->devi_node_attributes |= DDI_PERSISTENT;
	...
	cv_init(&(devi->devi_config_cv), NULL, CV_DEFAULT, NULL);
	...
	/*
	 * Establish an initial CNA, so we have some form of CNA even when
	 * there are bugs related to missing i_ddi_di_cache_invalidate calls.
	 *
	 * NOTE: nodes that are allocated early in boot will have a CNA
	 * value of zero.
	 */
	devi->devi_cna_dev = fm_cna_dev_generate();
...
fail:
	i_ddi_free_devi_sys_props(devi);
...
i_ddi_free_node(dev_info_t *dip)
{
	...
	cv_destroy(&(devi->devi_config_cv));
	...
	if (devi->devi_ev_devidstr)
		ddi_devid_str_free(devi->devi_ev_devidstr);
...
ndi_devi_set_nodename(dev_info_t *dip, char *name, int flags)
{
	...
	/*
	 * In rare instances the node name is changed after node creation.
	 * In the unlikely event that a BUS_CONFIG_ONE is waiting for the
	 * new node name, signal devi_config_cv.
	 */
...
link_node(dev_info_t *dip)
{
	...
	/*
	 * Signal devi_config_cv now as there may be BUS_CONFIG_ONEs waiting
	 * for the node that was just linked. Thus, devi_config_one() ->
	 * find_child_by_xxx() -> find_sibling()/FIND_ADDR_BY_INIT node
	 * promotion can now find this newly linked node. This is necessary
	 * because a thread linking this node will not necessarily promote it
	 * on its own.
	 */
	mutex_enter(&DEVI(dip)->devi_lock);
	cv_broadcast(&DEVI(dip)->devi_config_cv);
...
init_node(dev_info_t *dip)
{
...
#ifdef __x86
	DEVI(dip)->devi_iommu = DEVI(pdip)->devi_iommu;
#endif
	 ...
	 *
	 * On successful return, since all find_addr_by_xxx() calls off
	 * devi_config_one use FIND_ADDR_BY_INIT searches, we don't need to
	 * signal devi_config_cv here.
	 */
	...
	if (find_duplicate_child(pdip, dip) != NULL) {
		 ...
		 * will release initial hold of parent using ndi_rele_devi().
		 */
...
uninit_node(dev_info_t *dip)
{
	...
	/* Keep assigned nodes in the initialized state */
...
detach_node(dev_info_t *dip, uint_t flag)
{
	...
	mutex_enter(&(DEVI(dip)->devi_lock));
	DEVI_CLR_NEED_RESET(dip);
	DEVI_CLR_DEVICE_LIVE_SUSPENDED(dip);
	mutex_exit(&(DEVI(dip)->devi_lock));
	...
	while ((i_ddi_node_state(dip) < state) && (rv == DDI_SUCCESS)) {
		...
		case DS_PROBED:
			...
			if (i_ddi_check_retire(dip) == 1 &&
			    !i_ndi_dev_is_auto_assigned_node(dip) &&
...
ndi_devi_exit_and_wait(dev_info_t *dip, int circular, clock_t end_time)
{
	...
	(void) cv_timedwait(&devi->devi_config_cv, &(devi->devi_lock),
	    end_time);
	mutex_exit(&(devi->devi_lock));
}
    ...
    int (*callback)(dev_info_t *, char *, int))
{
	...
	/* if no list, return before kmem_alloc */
	if (head == NULL)
		return (NULL);
...
i_ddi_prop_list_dup(ddi_prop_t *prop, uint_t flag)
{
	...
	for (; prop != NULL; prop = prop->prop_next) {
		...
		copy->prop_name = ddi_strdup(prop->prop_name, flag);
		if (copy->prop_name == NULL) {
			kmem_free(copy, sizeof (*copy));
			goto fail;
		}
...
ndi_merge_node(dev_info_t *dip, int (*make_ua)(dev_info_t *, char *, int))
{
	...
	DEVI(hwdip)->devi_vdr_prop_ptr = DEVI(dip)->devi_vdr_prop_ptr;
	DEVI(hwdip)->devi_adm_prop_ptr = DEVI(dip)->devi_adm_prop_ptr;
	DEVI(dip)->devi_sys_prop_ptr = NULL;
	DEVI(dip)->devi_drv_prop_ptr = NULL;
	DEVI(dip)->devi_vdr_prop_ptr = NULL;
	DEVI(dip)->devi_adm_prop_ptr = NULL;
	mutex_exit(&DEVI(dip)->devi_lock);
	mutex_exit(&DEVI(hwdip)->devi_lock);
...
ndi_merge_wildcard_node(dev_info_t *dip)
{
	...
	for (hwdip = ddi_get_child(pdip); hwdip;
		...
		DEVI(hwdip)->devi_sys_prop_ptr = i_ddi_prop_list_dup(
		    DEVI(dip)->devi_sys_prop_ptr, KM_SLEEP);
		DEVI(hwdip)->devi_drv_prop_ptr = i_ddi_prop_list_dup(
		    DEVI(dip)->devi_drv_prop_ptr, KM_SLEEP);
		DEVI(hwdip)->devi_vdr_prop_ptr = i_ddi_prop_list_dup(
		    DEVI(dip)->devi_vdr_prop_ptr, KM_SLEEP);
		DEVI(hwdip)->devi_adm_prop_ptr = i_ddi_prop_list_dup(
		    DEVI(dip)->devi_adm_prop_ptr, KM_SLEEP);
		mutex_exit(&DEVI(hwdip)->devi_lock);
	}
	mutex_exit(&DEVI(dip)->devi_lock);
}
...
ddi_compatible_driver_major(dev_info_t *dip, char **formp)
{
	...
	/*
	 * If an assigned device does not bind to any driver, bind it to
	 * nulldriver.  It will not attach.
	 */
	if (DEVI_IS_ASSIGNED(devi))
		return (ddi_name_to_major("nulldriver"));
...
add_global_props(dev_info_t *dip)
{
	...
	ddi_prop_list_t *fplist;
	ddi_prop_list_t *eplist;
	...
	dnp = &devnamesp[DEVI(dip)->devi_major];
	LOCK_DEV_OPS(&dnp->dn_lock);
	if ((plist = dnp->dn_global_prop_ptr) != NULL)
		i_ddi_prop_list_hold(plist, dnp);
	if ((fplist = dnp->dn_vdr_global_props) != NULL)
		i_ddi_prop_list_hold(fplist, dnp);
	if ((eplist = dnp->dn_adm_global_props) != NULL)
		i_ddi_prop_list_hold(eplist, dnp);
	UNLOCK_DEV_OPS(&dnp->dn_lock);

	mutex_enter(&DEVI(dip)->devi_lock);
	DEVI(dip)->devi_global_prop_list = plist;
	DEVI(dip)->devi_vdr_global_props = fplist;
	DEVI(dip)->devi_adm_global_props = eplist;
	mutex_exit(&DEVI(dip)->devi_lock);
}
...
remove_global_props(dev_info_t *dip)
{
	...
	ddi_prop_list_t *fproplist;
	ddi_prop_list_t *eproplist;

	mutex_enter(&DEVI(dip)->devi_lock);
	proplist = DEVI(dip)->devi_global_prop_list;
	fproplist = DEVI(dip)->devi_vdr_global_props;
	eproplist = DEVI(dip)->devi_adm_global_props;
	DEVI(dip)->devi_global_prop_list = NULL;
	DEVI(dip)->devi_vdr_global_props = NULL;
	DEVI(dip)->devi_adm_global_props = NULL;
	mutex_exit(&DEVI(dip)->devi_lock);

	if (proplist || fproplist || eproplist) {
		major_t major;
		struct devnames *dnp;
		...
		if (fproplist)
			i_ddi_prop_list_rele(fproplist, dnp);
		if (eproplist)
			i_ddi_prop_list_rele(eproplist, dnp);
		UNLOCK_DEV_OPS(&dnp->dn_lock);
	}
}
...
/*
 * report device status, and invalidate the cached devinfo snapshot.
 */
static void
i_ndi_devi_report_status_change(dev_info_t *dip, char *path)
{
	...
	/* Invalidate the cached devinfo snapshot. */
...
/*
 * Produce event notification that devinfo-or-pathinfo-or-minor branch-or-node
 * was configured/unconfigured (added/removed).
 *
 * NOTE: When the caller specifies non-NULL dip or pip, as a courtesy, the
 * code below will derive many of the arguments values not explicitly specified.
 * For 'remove' subclass events that occur after node destruction, dip and pip
 * must be NULL and the caller is responsible for specifying event attributes
 * explicitly, saving attribute values prior to node destruction as needed.
 *
 * Return 0 on success (or if called too early).
 */
int
i_ddi_log_event(char *subclass, dev_info_t *dip, mdi_pathinfo_t *pip,
    uint64_t cna_dev, char *pathname, char *minorname, char *devidstr,
    char *class, char *driver_name, int instance, uint_t flags)
{
	...
	if (dip || pip)
		i_ddi_di_cache_invalidate(dip, pip);

	branch = strstr(subclass, "branch") ? 1 : 0;
	minor = strstr(subclass, "minor") ? 1 : 0;

	/* Allocate and fill in event attributes. */
	ev = sysevent_alloc(EC_DEVFS, subclass, EP_DDI, SE_SLEEP);

	/*
	 * Add the node's CNA_DEV as an attribute.  As a courtesy to the
	 * caller, if cna_dev is zero we derive cna_dev from the dip or
	 * pip argument.
	 */
	if (cna_dev == 0) {
		if (pip)
			cna_dev = mdi_pi_get_cna_dev(pip);
		else if (dip)
			cna_dev = ddi_cna_dev(dip);
	}
	if (cna_dev) {
		se_val.value_type = SE_DATA_TYPE_UINT64;
		se_val.value.sv_uint64 = cna_dev;
		if (sysevent_add_attr(&ev_attr_list,
		    EA_CNA_DEV, &se_val, SE_SLEEP) != 0)
			goto fail;
	}

	/*
	 * Add the node's pathname as an attribute.  As a courtesy to the
	 * caller, if pathname is null we derive pathname from the dip or
	 * pip argument.
	 */
	if (pathname == NULL) {
		if (dip) {
			...
			pathname_free = 1;
			pathname = kmem_alloc(MAXPATHLEN, KM_SLEEP);
			(void) strcpy(pathname, mdi_pi_pathname(pip));
		}
	} else
		pathname_free = 0;
	if (pathname) {
		...
		if (sysevent_add_attr(&ev_attr_list,
		    DEVFS_PATHNAME, &se_val, SE_SLEEP) != 0)
			goto fail;

		/* add minorname attribute if non-null */
		if (minorname) {
			se_val.value_type = SE_DATA_TYPE_STRING;
			se_val.value.sv_string = minorname;
	...
	/* add the devid attribute */
	if (dip && (devidstr == NULL))
		devidstr = ddi_devid_str_get(dip);
	if (devidstr) {
		se_val.value_type = SE_DATA_TYPE_STRING;
		se_val.value.sv_string = devidstr;
		if (sysevent_add_attr(&ev_attr_list,
		    DEVFS_DEVID, &se_val, SE_SLEEP) != 0)
			goto fail;
	}

	/* add the device class attribute (but not on branch events) */
	if (dip && (class == NULL))
		class = i_ddi_devi_class(dip);
	if (!branch && class) {
		se_val.value_type = SE_DATA_TYPE_STRING;
		se_val.value.sv_string = class;
		if (sysevent_add_attr(&ev_attr_list,
		    DEVFS_DEVI_CLASS, &se_val, SE_SLEEP) != 0)
			goto fail;
	}

	/* add the driver_name attribute (but not on branch events) */
	if (dip && (driver_name == NULL))
		driver_name = (char *)ddi_driver_name(dip);
	if (!branch && driver_name) {
		se_val.value_type = SE_DATA_TYPE_STRING;
	...
	/* add the instance attribute (but not on branch events) */
	if (pip && (instance < 0))
		instance = mdi_pi_get_path_instance(pip);
	else if (dip && (instance < 0))
		instance = ddi_get_instance(dip);
	if (!branch && (instance >= 0)) {
	...
	/*
	 * For non-branch, non-minor events against devinfo nodes, we add an
	 * DEVFS_BRANCH_EVENT attribute unless NDI_BRANCH_EVENT_OP is set.
	 * When NDI_BRANCH_EVENT_OP is set the caller will generate a
	 * separate branch-subclass sysevent.
	 */
	if (!branch && !minor && ((flags & NDI_BRANCH_EVENT_OP) == 0)) {
		/*
		 * Instead of logging a separate branch event just add
		 * DEVFS_BRANCH_EVENT attribute. It indicates to devfsadmd to
	...
	if (sysevent_attach_attributes(ev, ev_attr_list) == 0) {
		ev_attr_list = NULL;	/* attributes now attached to event */

		if ((se_err = log_sysevent(ev, SE_SLEEP, &eid)) == 0)
			fail_msg = 0;		/* success */
		else if (se_err == SE_NO_TRANSPORT)
			no_transport = 1;
	}

fail:	if (fail_msg)
		cmn_err(CE_WARN, "failed to log EC_DEVFS %s event for %s%s",
		    subclass, pathname ? pathname : "unknown-path",
	...
	sysevent_free(ev);
	return (fail_msg ? 1 : 0);		/* return 0 on success */
}
...
i_ddi_log_devfs_device_remove(dev_info_t *dip)
{
	...
	/* Generate ESC_DEVFS_DEVI_REMOVE sysevent. */
	(void) i_ddi_log_event(ESC_DEVFS_DEVI_REMOVE, dip, NULL,
	    0LL, NULL, NULL, NULL, NULL, NULL, -1, 0);
}
...
i_ddi_log_devfs_device_insert(dev_info_t *dip)
{
	...
	(void) i_ddi_log_event(ESC_DEVFS_DEVI_ADD, dip, NULL,
	    0LL, NULL, NULL, NULL, NULL, NULL, -1, 0);
}
    ...
    struct brevq_node *child)
{
	...
	brn->brn_deviname = i_ddi_strdup(ddi_deviname(dip, deviname), KM_SLEEP);
	kmem_free(deviname, MAXNAMELEN);
	i_ddi_di_cache_invalidate(dip, NULL);

	brn->brn_cna_dev = ddi_cna_dev(dip);	/* save branch cna */
	brn->brn_child = child;
 ...
 * associated memory.
 *
 * pathname must have been allocated with at least MAXPATHLEN bytes.
 */
static void
log_and_free_brevq(char *pathname, struct brevq_node *brevq)
{
	struct brevq_node *brn;
	char *p;
	...
	for (brn = brevq; brn != NULL; brn = brn->brn_sibling) {
		...
		/* NOTE: i_ddi_di_cache_invalidate was done by brevq_enqueue */
		(void) i_ddi_log_event(ESC_DEVFS_BRANCH_REMOVE, NULL, NULL,
		    brn->brn_cna_dev, pathname, NULL, NULL, NULL, NULL, -1, 0);
	}
	*p = '\0';
...
devi_attach_node(dev_info_t *dip, uint_t flags)
{
	...
	/* report attach and invalidate devinfo cache */
    ...
    uint_t flags, clock_t timeout)
{
	...
	*cdipp = NULL;
	...
	for (;;) {
		...
		/*
		 * We have either a vHCI (with devinfo children) or a pHCI
		 * (with mixed devinfo and pathinfo children). Either way,
		 * we need to look for devinfo children.
		 */
...
again:		(void) i_ndi_make_spec_children(pdip, flags);
		...
		if ((cdip == NULL) && drivername)
			cdip = find_child_by_driver(pdip, drivername, addr);
		 ...
		 */
		if (probed && i_ddi_io_initialized() &&
		...
		/*
		 * Break out of for(;;) if time expired or if we know there's
		 * no point in continuing to wait (i.e. all existing children
		 * have been configured).
		 */
		if ((timeout <= 0) || (ddi_get_lbolt() >= end_time) ||
		    (DEVI(pdip)->devi_flags & DEVI_CONFIG_ALL_CMPLT)) {
			NDI_DEBUG(flags, (CE_CONT,
			    "%s%d: not waiting for child %s@%s (%s)",
			    ddi_driver_name(pdip), ddi_get_instance(pdip),
			    name, addr,
			    (DEVI(pdip)->devi_flags & DEVI_CONFIG_ALL_CMPLT) ?
			    "config complete" : "time expired"));
			break;
		}
...
ndi_devi_config_one(dev_info_t *pdip, char *devnm, dev_info_t **dipp, int flags)
{
	...
	*dipp = NULL;
	...
	if (branch_event)
		(void) i_ddi_log_event(ESC_DEVFS_BRANCH_ADD, *dipp, NULL,
		    0LL, NULL, NULL, NULL, NULL, NULL, -1, 0);
...
devi_detach_node(dev_info_t *dip, uint_t flags)
{
	...
	if (i_ddi_node_state(dip) == DS_INITIALIZED) {
		...
		if (need_remove_event(dip, flags)) {
			/*
			 * path, devid, and instance are lost with demotion
			 * associated with ddi_uninitchild() below, so we
			 * need to save them now for use in successful remove
			 * event generation below.
			 */
			if (devi->devi_ev_path == NULL) {
				devi->devi_ev_path = kmem_alloc(MAXPATHLEN,
				    KM_SLEEP);
				(void) ddi_pathname(dip, devi->devi_ev_path);
			}
			if ((devi->devi_ev_devidstr == NULL) &&
			    ddi_devid_str_get(dip))
				devi->devi_ev_devidstr =
				    ddi_devid_str_get_dup(dip);
		...
		if (ret == NDI_SUCCESS) {
			...
			if (flags & NDI_DEVI_REMOVE) {
				...
				if (DEVI_EVREMOVE(dip)) {
					...
					/*
					 * We are responsible for path and
					 * devidstr.
					 */
					path = devi->devi_ev_path;
					devi->devi_ev_path = NULL;
					devidstr =  devi->devi_ev_devidstr;
					devi->devi_ev_devidstr = NULL;

					instance = devi->devi_ev_instance;

					/* save class */
					class = i_ddi_strdup(
					    i_ddi_devi_class(dip), KM_SLEEP);
				...
				if (post_event && (ret == NDI_SUCCESS)) {
					 ...
					 * i_ddi_log_event does not do that
					 * when both dip and pip are NULL.
					 */
					i_ddi_di_cache_invalidate(pdip, NULL);

					/* Generate ESC_DEVFS_DEVI_REMOVE */
					(void) i_ddi_log_event(
					    ESC_DEVFS_DEVI_REMOVE, NULL, NULL,
					    ddi_cna_dev(pdip), path, NULL,
					    devidstr, class, driver, instance,
					    flags);

					/*
					 * Remove successful, free path and
					 * devidstr.
					 */
					if (path)
						kmem_free(path, MAXPATHLEN);
					 ...
					 * responsibility for path and
					 * devidstr back to devi_ev_*.
					 */
					devi->devi_ev_path = path;
					devi->devi_ev_devidstr = devidstr;
				}
	...
	major_t major)
{
	...
	while (child) {
		next = ddi_get_next_sibling(child);
		persistent_node = ndi_dev_is_persistent_node(child);
		...
		if (devi_detach_node(child, flags) != NDI_SUCCESS) {
			...
			/*
			 * Detaching of a PF node could have deleted some of
			 * its sibling VF nodes. The sibling looked up prior to
			 * node detach may not be valid.
			 * If this node is being removed lookup for next child
			 * from the parent.
			 * Else, get the sibling of the current detached
			 * node.
			 */
			if ((flags & NDI_DEVI_REMOVE) ||
			    ((flags & NDI_UNCONFIG) && (!persistent_node)))
				child = ddi_get_child(dip);
			else
				child = ddi_get_next_sibling(child);
...
ndi_devi_online(dev_info_t *dip, uint_t flags)
{
	...
	if ((rv = devi_attach_node(dip, flags)) == NDI_SUCCESS) {
		...
		if (branch_event)
			(void) i_ddi_log_event(ESC_DEVFS_BRANCH_ADD, dip, NULL,
			    0LL, NULL, NULL, NULL, NULL, NULL, -1, 0);
	}
...
void
e_ddi_mark_annotations(dev_info_t *dip)
{
	DEVI(dip)->devi_flags |= DEVI_ANNOTATIONS;
}
    ...
    void *arg, dev_info_t **child, clock_t timeout)
{
	...
	/*
	 * For SCSAv3 devices, a timeout of 0 implies there is no need to
	 * wait, which means it's time to clear the DEVI_CONFIG_ALL_CMPLT
	 * flag.  For non-SCSAv3 devices, the flag would never be set anyway.
	 */
	if (timeout == 0) {
		mutex_enter(&DEVI(pdip)->devi_lock);
		DEVI(pdip)->devi_flags &= ~DEVI_CONFIG_ALL_CMPLT;
	...
	case BUS_CONFIG_ALL:
		...
		if (timeout > 0) {
			clock_t expire = ddi_get_lbolt64() + timeout;
			...
			NDI_DEBUG(flags, (CE_CONT,
			    ...
			    timeout));
			mutex_enter(&DEVI(pdip)->devi_lock);
			while (!(DEVI(pdip)->devi_flags &
			    DEVI_CONFIG_ALL_CMPLT)) {
				if (cv_timedwait(&DEVI(pdip)->devi_config_cv,
				    &DEVI(pdip)->devi_lock, expire) == -1) {
					break;		/* time expired */
				}
			}
			mutex_exit(&DEVI(pdip)->devi_lock);
...
void
ndi_busop_bus_config_all_complete(dev_info_t *pdip)
{
	mutex_enter(&DEVI(pdip)->devi_lock);
	DEVI(pdip)->devi_flags |= DEVI_CONFIG_ALL_CMPLT;
	cv_broadcast(&DEVI(pdip)->devi_config_cv);
	mutex_exit(&DEVI(pdip)->devi_lock);
}
...
mt_config_children(struct mt_config_handle *hdl)
{
	...
	while (dip) {
		 ...
		 * removed. The corresponding ndi_rele_devi() for children
...
void
i_ddi_di_cache_invalidate(dev_info_t *dip, mdi_pathinfo_t *pip)
{
	uint64_t	cna_dev;
	int		cache_valid;
	...
	if (dip || pip)  {
		/* Compute current dev Configuration Numeric Association. */
		cna_dev = fm_cna_dev_generate();

		/*
		 * Update devi_cna_dev on hw twig up to (and including) the
		 * root.
		 * NOTE: ddi_get_parent(ddi_root_node())) == ddi_root_node()
		 */
		if (pip) {
			mdi_pi_set_cna_dev(pip, cna_dev);
			dip = mdi_pi_get_phci(pip);	/* up pHCI stick */
		}
		if (dip) {
			DEVI(dip)->devi_cna_dev = cna_dev;
			while (dip && (dip != ddi_root_node())) {
	...
	/* Verify that we are initialized to perform cache invalidate */
...
ndi_devi_device_insert(dev_info_t *dip)
{
	...
	/* report insert (as 'online') and invalidate cached snapshot */
	i_ndi_devi_report_status_change(dip, NULL);

	/*
	 * Generate sysevent for those interested in insertion (either directly
...
int
ndi_devi_device_is_live_suspended(dev_info_t *dip)
{
	return (DEVI_IS_DEVICE_LIVE_SUSPENDED(dip));
}

int
ndi_devi_device_live_suspend(dev_info_t *dip)
{
	ASSERT(dip && ddi_get_parent(dip) &&
	    DEVI_BUSY_OWNED(ddi_get_parent(dip)));

	/* Return if already marked suspended. */
	if (ndi_devi_device_is_live_suspended(dip))
		return (0);

	/* Mark suspended */
	mutex_enter(&(DEVI(dip)->devi_lock));
	DEVI_SET_DEVICE_LIVE_SUSPENDED(dip);
	mutex_exit(&(DEVI(dip)->devi_lock));

	return (1); /* DEVICE_SUSPENDED state changed */
}

int
ndi_devi_device_live_resume(dev_info_t *dip)
{
	ASSERT(dip && ddi_get_parent(dip) &&
	    DEVI_BUSY_OWNED(ddi_get_parent(dip)));

	/* Return if not marked suspended. */
	if (!ndi_devi_device_is_live_suspended(dip))
		return (0);
...
e_ddi_retire_device(char *path, char **cons_array)
{
	...
	/* Check for assigned device which should not be retired */
	if (dev_path_match_assigned(path)) {
		RIO_DEBUG((CE_NOTE, "Cannot retire assigned device %s.", path));
		return (DDI_FAILURE);
	}

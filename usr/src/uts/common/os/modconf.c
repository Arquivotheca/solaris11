 ...
 */
static int mod_infotcpcong(struct modltcpcong *, struct modlinkage *, int *);
static int mod_installtcpcong(struct modltcpcong *, struct modlinkage *);
static int mod_removetcpcong(struct modltcpcong *, struct modlinkage *);

struct mod_ops mod_tcpcongops = {
	mod_installtcpcong, mod_removetcpcong, mod_infotcpcong
};
...
{
	brand_t *brand = modl->brand_branddef;

	/* Sanity checks */
	if (brand == NULL) {
		cmn_err(CE_WARN, "brand module malformed: %s",
		    modl->brand_linkinfo);
		return (EINVAL);
	}

	/* Version check */
	if (brand->b_version != BRAND_VER_REV) {
		if (brand->b_version < BRAND_VER_REV) {
			cmn_err(CE_WARN,
			    "brand module unsupported, too old: %s",
			    modl->brand_linkinfo);
		} else {
			cmn_err(CE_WARN,
			    "brand module unsupported, too new: %s",
			    modl->brand_linkinfo);
		}
		return (EINVAL);
	}

	/* version dependent sanity checks */
	if (brand->b_modname == NULL || brand->b_ops == NULL ||
...
/*
 * Get status of a tcpcong module.
 */
/*ARGSUSED*/
static int
mod_infotcpcong(struct modltcpcong *modl, struct modlinkage *modlp, int *p0)
{
	*p0 = -1;	/* no useful info */
	return (0);
}
...
/*
 * Install a tcpcong module.
 */
/*ARGSUSED*/
static int
mod_installtcpcong(struct modltcpcong *modl, struct modlinkage *modlp)
{
	return (tcpcong_mod_register(modl->tcpcong_ops));
}

/*
 * Remove a tcpcong module.
 */
/*ARGSUSED*/
static int
mod_removetcpcong(struct modltcpcong *modl, struct modlinkage *modlp)
{
	return (tcpcong_mod_unregister(modl->tcpcong_ops));
}

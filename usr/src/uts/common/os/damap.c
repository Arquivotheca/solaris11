...
static void damap_new_error_window(dam_t *, dam_da_t *, id_t, int64_t);
static void damap_id_hold(dam_t *, dam_da_t *);
...
/*
 * A device must have had a minimum of damap_iops_threshold I/Os per second
 * within a given error window in order for the DAM to make a determination
 * of whether or not to unconfigure based on jitter or transport errors seen
 * during that window.
 */
int	damap_iops_threshold = DAMAP_DEFAULT_IOPS_THRESHOLD;
 ...
 * error_win_usec:	Size of transport error detection window (in usecs)
 * error_thresh:	Error threshold ([0..100])
 *			0 = No threshold (Never unconfig based on xp errors)
 *			100 = Every completion within the window must have had
 *			    a transport error to trigger deactivation
 * error_iops_thresh:	Must be non-negative.  This indicates the minimum
 *			average I/Os per second in a window in order to trigger
 *			any action based on errors.  If this is 0, the DAM will
...
damap_create(char *name, damap_rptmode_t mode, int map_opts,
    int stable_usec, int error_win_usec, int error_thresh,
    int error_iops_thresh, void *activate_arg, damap_activate_cb_t activate_cb,
    damap_deactivate_cb_t deactivate_cb, void *config_arg,
    damap_configure_cb_t configure_cb, damap_unconfig_cb_t unconfig_cb,
    damap_t **damapp)
{
	dam_t *mapp;
	...
	mapp->dam_error_ticks = drv_usectohz(error_win_usec);
	if (error_thresh < 0)
		mapp->dam_error_thresh = 0;
	else if (error_thresh > 100)
		mapp->dam_error_thresh = 100;
	else
		mapp->dam_error_thresh = error_thresh;
	mapp->dam_error_iops_thresh = error_iops_thresh;
	if (error_iops_thresh <= 0) {
		mapp->dam_error_iops_thresh = damap_iops_threshold;
	}
	mapp->dam_size = 0;
	...
	bitset_init(&mapp->dam_cfg_set);
	bitset_init(&mapp->dam_uncfg_set);
	bitset_init(&mapp->dam_stable_set);
	bitset_init(&mapp->dam_report_set);
	bitset_init(&mapp->dam_xpterr_set);
	*damapp = (damap_t *)mapp;
...
dam_map_alloc(dam_t *mapp)
{
	...
	bitset_resize(&mapp->dam_cfg_set, mapp->dam_size);
	bitset_resize(&mapp->dam_uncfg_set, mapp->dam_size);
	bitset_resize(&mapp->dam_stable_set, mapp->dam_size);
	bitset_resize(&mapp->dam_report_set, mapp->dam_size);
	bitset_resize(&mapp->dam_xpterr_set, mapp->dam_size);
	return (DAM_SUCCESS);
}
...
damap_destroy(damap_t *damapp)
{
	...
	bitset_fini(&mapp->dam_cfg_set);
	bitset_fini(&mapp->dam_uncfg_set);
	bitset_fini(&mapp->dam_stable_set);
	bitset_fini(&mapp->dam_report_set);
	bitset_fini(&mapp->dam_xpterr_set);
...
again:	while ((mapp->dam_flags & WAITFOR_FLAGS) ||
	    ...
	    (mapp->dam_tid != 0)) {
		mapp->dam_sync_reason = 0;
		if (mapp->dam_flags & WAITFOR_FLAGS) {
			mapp->dam_sync_reason |= DAM_SYNC_RSN_FLAGS;
		}
		if (!bitset_is_null(&mapp->dam_report_set)) {
			mapp->dam_sync_reason |= DAM_SYNC_RSN_REPORT_SET;
		}
		if (mapp->dam_tid != 0) {
			mapp->dam_sync_reason |= DAM_SYNC_RSN_TID;
		}
		DTRACE_PROBE3(damap__map__sync__waiting,
		    char *, mapp->dam_name, dam_t *, mapp,
		    int, mapp->dam_sync_reason);
 ...
 * jitter:	amount of jitter since last addrset_add
 *
 * Returns:	DAM_SUCCESS
 *		DAM_EINVAL	Invalid argument(s)
 *		DAM_MAPFULL	address map exhausted
...
damap_addrset_add(damap_t *damapp, char *address, damap_id_t *ridx,
    nvlist_t *nvl, void *addr_priv, int jitter)
{
	dam_t *mapp = (dam_t *)damapp;
	id_t addrid;
	dam_da_t *passp;
	...
	/*
	 * If this map contains an error window, record the provided jitter
	 * regardless of whether the address is already in the report set
	 */
	if (mapp->dam_error_ticks) {
		passp->da_jitter += jitter;
		DTRACE_PROBE4(damap__addrset__add__jitter,
		    char *, mapp->dam_name, dam_t *, mapp,
		    char *, address, int, passp->da_jitter);

		ts = ddi_get_lbolt64();
		if (ts > passp->da_err_wclose) {
			damap_new_error_window(mapp, passp, addrid, ts);
		}
	}
...
/*
 * Given a map and an address ID, populate the passed in pointers with the
 * addresses of the statistics for the requested address.  This also increases
 * the reference count on that address. The caller is required to release that
 * reference by calling damap_id_rele.
 *
 * damapp:	address map handle
 * addrid:	address ID
 * io_nxpe:	Address of pointer to populate with non-transport error counter
 *
 * Returns:	None
 *		Success: io_xpe and io_nxpe will be non-NULL
 *		Failure: io_xpe and io_nxpe will be NULL
 */
void
damap_nxpe_bind(damap_t *damapp, damap_id_t addrid, uint32_t **io_nxpe)
{
	dam_t *mapp = (dam_t *)damapp;
	dam_da_t *passp;
	...
	if (passp == NULL) {
		*io_nxpe = (uint32_t *)NULL;
		return;
	}

	DTRACE_PROBE4(damap__stat__bind, dam_t *, mapp, char *, mapp->dam_name,
	    char *, passp->da_addr, int, passp->da_ref);

	mutex_enter(&mapp->dam_lock);
	damap_id_hold(mapp, passp);
	*io_nxpe = &passp->da_io_nxpe;
	mutex_exit(&mapp->dam_lock);
}

void
damap_nxpe_unbind(damap_t *damapp, damap_id_t addrid, uint32_t **io_nxpe)
{
	dam_t *mapp = (dam_t *)damapp;
	dam_da_t *passp;
	...
	passp = ddi_get_soft_state(mapp->dam_da, (id_t)addrid);
	if (passp != NULL) {
		DTRACE_PROBE4(damap__stat__unbind, dam_t *, mapp,
		    char *, mapp->dam_name, char *, passp->da_addr,
		    int, passp->da_ref);
...
static void
damap_new_error_window(dam_t *mapp, dam_da_t *passp, id_t addrid,
    int64_t timestamp)
{
	uint32_t	xpe_pct;
	char		*addr;
	boolean_t	jitter = B_FALSE;

	ASSERT(mapp && passp);
	ASSERT(mutex_owned(&mapp->dam_lock));
	addr = damap_id2addr((damap_t *)mapp, addrid);

	/*
	 * If this address is not in the stable set, just open a new error
	 * window.
	 */
	if (!bitset_in_set(&mapp->dam_stable_set, addrid)) {
		DTRACE_PROBE2(damap__new__error__window__only,
		    dam_t *, mapp, char *, addr);
		goto criteria_fail;
	}

	/*
	 * Check PHY jitter.
	 * The symptom of a "jittery" PHY would be numerous address set reports
	 * that alternate between containing an address and not containing it.
	 */
	if (passp->da_jitter) {
		if (passp->da_jitter >
		    (drv_hztousec(mapp->dam_error_ticks) / MICROSEC)) {
			jitter = B_TRUE;
		}
	}

	/*
	 * Is there enough data in this window to make a reasonable assessment
	 * as to the device's condition?
	 */
	if ((((passp->da_io_wnxpe + passp->da_io_wxpe) * drv_hztousec(1)) /
	    mapp->dam_error_ticks) < damap_iops_threshold) {
		DTRACE_PROBE4(damap__not__enough__ios,
		    dam_t *, mapp, char *, addr,
		    uint32_t, passp->da_io_wnxpe, uint32_t, passp->da_io_wxpe);
		goto criteria_fail;
	}

	/*
	 * Check the error window's error counts and the threshold to see if
	 * this device needs to be unconfigured.
	 */
	if (passp->da_io_nxpe == passp->da_io_wnxpe) {
		if (passp->da_io_xpe == passp->da_io_wxpe)
			xpe_pct = 0;
		else
			xpe_pct = 100;
	} else {
		xpe_pct = ((passp->da_io_xpe - passp->da_io_wxpe) * 100 /
		    (passp->da_io_nxpe - passp->da_io_wnxpe));
	}

	if (jitter || (xpe_pct >= mapp->dam_error_thresh)) {
		/*
		 * Only schedule stabilization if the address isn't currently
		 * in the xpterr bitset.
		 */
		if (!DAM_XPE_EXCEEDED(mapp, addrid)) {
			DTRACE_PROBE5(damap__error__thresh__exceeded,
			    char *, mapp->dam_name, dam_t *, mapp,
			    char *, addr, uint32_t, xpe_pct,
			    int, passp->da_jitter);
			bitset_add(&mapp->dam_xpterr_set, (id_t)addrid);
			mapp->dam_last_update = gethrtime();
			dam_sched_timeout(dam_addrset_stable_cb, mapp,
			    mapp->dam_stable_ticks);
			return;
...
criteria_fail:
	/* Initialize the new error window */
	passp->da_jitter = 0;
	passp->da_err_wclose = timestamp + mapp->dam_error_ticks;
	passp->da_io_wnxpe = passp->da_io_nxpe;
	passp->da_io_wxpe = passp->da_io_xpe;
	DTRACE_PROBE4(damap__new__error__window,
	    char *, mapp->dam_name, dam_t *, mapp,
	    char *, addr, int64_t, passp->da_err_wclose);
}

/*
 * This function is called from scsi_hba_pkt_comp in the case of an I/O
 * with a transport error.  damap_new_error_window will check thresholds
 * and transport error counts to determine if any action  needs to be taken.
 */
void
damap_xpe_report(damap_t *damapp, damap_id_t addrid)
{
	dam_t		*mapp = (dam_t *)damapp;
	dam_da_t	*passp;
	char		*addr;
	...
	DTRACE_PROBE4(damap__report__transport__error,
	    char *, mapp->dam_name, dam_t *, mapp,
	    char *, addr, uint32_t, passp->da_io_xpe);

	if (!mapp->dam_error_ticks) {
		/* This map doesn't have an error window configured */
		mutex_exit(&mapp->dam_lock);
		return;
	}
	...
	ts = ddi_get_lbolt64();
	if (ts > passp->da_err_wclose)
		damap_new_error_window(mapp, passp, (id_t)addrid, ts);

	mutex_exit(&mapp->dam_lock);
}

/*
 * Establish address reference in map
 *
 * mapp:	address map pointer
 * passp:	softstate pointer for address in mapp
 */
static void
damap_id_hold(dam_t *mapp, dam_da_t *passp)
{
	ASSERT(mapp && passp);
	ASSERT(mutex_owned(&mapp->dam_lock));

	DTRACE_PROBE4(damap__id__hold,
	    char *, mapp->dam_name, dam_t *, mapp,
	    char *, passp->da_addr, int, passp->da_ref);
	passp->da_ref++;
}
 ...
 * allow_unstable:	If set, allow addresses not yet stabilized
 *
 ...
 */
damap_id_t
damap_lookup(damap_t *damapp, char *address, boolean_t allow_unstable)
{
	dam_t *mapp = (dam_t *)damapp;
	id_t addrid = 0;
	dam_da_t *passp = NULL;
	...
	if (addrid) {
		is_stable = DAM_IS_STABLE(mapp, addrid);
		if (is_stable || allow_unstable) {
			passp = ddi_get_soft_state(mapp->dam_da, addrid);
			if (passp) {
				damap_id_hold(mapp, passp);
			} else {
	...
	mutex_exit(&mapp->dam_lock);
	DTRACE_PROBE5(damap__lookup__return,
	    char *, mapp->dam_name, dam_t *, mapp,
	    char *, address, int, addrid, boolean_t, is_stable);
...
dam_addr_activate(dam_t *mapp, id_t addrid)
{
	...
	config_rv = (*mapp->dam_configure_cb)(mapp->dam_config_arg, mapp,
	    addrid, DAMAP_CONFIG_RSN_CONFIG_ONE);
...
{
	char			*addrstr;
	damap_deact_rsn_t	rsn = DAMAP_DEACT_RSN_GONE;
    ...
    damap_deact_rsn_t deact_rsn)
{
	...
	/*
	 * clear the active and xpterr bit and free the backing info for
	 * this address
	 */
	mutex_enter(&mapp->dam_lock);
	bitset_del(&mapp->dam_active_set, addrid);
	bitset_del(&mapp->dam_xpterr_set, addrid);
	passp->da_ppriv = NULL;
	if (passp->da_nvl)
...
dam_addrset_activate(dam_t *mapp, bitset_t *activate)
{
	...
	/*
	 * Call the configure callback again to indicate that all addresses
	 * have been configured
	 */
	(void) (*mapp->dam_configure_cb)(mapp->dam_config_arg, mapp, NODAM,
	    DAMAP_CONFIG_RSN_CONFIG_ALL_CMPLT);
}
...
dam_addr_release(dam_t *mapp, id_t addrid)
{
	...
	addrstr = ddi_strid_id2str(mapp->dam_addr_hash, addrid);
	DTRACE_PROBE4(damap__addr__release,
	    char *, mapp->dam_name, dam_t *, mapp,
...
dam_stabilize_map(void *arg)
{
	...
	bitset_copy(&cfg, &mapp->dam_cfg_set);
	bitset_copy(&uncfg, &mapp->dam_uncfg_set);
	/*
	 ...
	 */
	mutex_enter(&mapp->dam_lock);
	bitset_zero(&mapp->dam_cfg_set);
	bitset_zero(&mapp->dam_uncfg_set);
	bitset_zero(&mapp->dam_stable_set);
...
dam_addrset_stable_cb(void *arg)
{
	...
	bitset_init(&unstable);
	bitset_resize(&unstable, mapp->dam_size);

	/*
	 * If DAM_SETADD is set, then we're here as a result of a full set
	 * report.  However, there could also be bits in the xpterr_set.
	 * That case needs to be handled differently from the case where
	 * there are bits in xpterr_set but DAM_SETADD is not set.
	 */
	if (mapp->dam_flags & DAM_SETADD) {
		if (bitset_and(&mapp->dam_report_set, &mapp->dam_xpterr_set,
		    &unstable)) {
			/*
			 * Anything set in the report set and the xpterr set
			 * needs to be removed in the stable set.  Any bits
			 * set in both places will now be in the unstable set.
			 */
			(void) bitset_xor(&mapp->dam_report_set, &unstable,
			    &mapp->dam_stable_set);
		} else {
			/* Nothing in the report set needs to be removed */
			bitset_copy(&mapp->dam_report_set,
			    &mapp->dam_stable_set);
		}
	} else {
		/*
		 * There is no report set.  The stable set becomes the active
		 * set minus anything in xpterr_set.
		 */
		(void) bitset_xor(&mapp->dam_active_set, &mapp->dam_xpterr_set,
		    &mapp->dam_stable_set);
	}
	...
	mapp->dam_flags |= DAM_SPEND;
	mapp->dam_flags &= ~DAM_SETADD;
	/* NOTE: don't need cv_broadcast since DAM_SPEND is still set */

	DTRACE_PROBE2(damap__map__addrset__stable__start,
	    char *, mapp->dam_name, dam_t *, mapp);
	mutex_exit(&mapp->dam_lock);

	bitset_fini(&unstable);
}
...
dam_get_addrid(dam_t *mapp, char *address)
{
	...
	if ((addrid = ddi_strid_str2id(mapp->dam_addr_hash, address)) == 0) {
		...
		DTRACE_PROBE4(damap__addrid__alloc,
		    char *, mapp->dam_name, dam_t *, mapp,
		    char *, address, int, passp->da_ref);
	}
	return (addrid);
}
...
/*
 * Check whether the specifid address ID is pending on unconfig
 *
 * damapp:	address map handle
 * addrid:	address ID
 *
 ...
 */
int
dam_addrid_pending_uncfg(damap_t *damapp, damap_id_t addrid)
{
	dam_t *mapp = (dam_t *)damapp;
	int ret = 0;

	mutex_enter(&mapp->dam_lock);
	if (DAM_IN_UNCFG(mapp, addrid)) {
		ret = 1;
	}
	mutex_exit(&mapp->dam_lock);
	return (ret);

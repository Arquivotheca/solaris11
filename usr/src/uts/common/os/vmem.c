...
vmem_populate(vmem_t *vmp, int vmflag)
{
	...
	else if ((vmflag & VM_PUSHPAGE) || NOMEMWAIT())
...
do_alloc:
		...
		if (vmp->vm_source_alloc != NULL && nocross == 0 &&
			...
			if (asize < size) {	/* overflow */
				...
				vaddr = NULL;
			} else if (vmp->vm_cflags & VMC_XALLOC) {
				size_t oasize = asize;
...
{
	int i;
	size_t nqcache_arg = qcache_max >> (highbit(quantum) - 1);
	size_t nqcache = (vmflag & VMC_NO_QCACHE) ? 0 : nqcache_arg;
	size_t vmem_sz = VMEM_SIZE(nqcache);
	vmem_t *vmp, *cur, **vmpp;
	vmem_seg_t *vsp;
	vmem_freelist_t *vfp;
	...
	if (vmem_vmem_arena != NULL) {
		vmp = vmem_alloc(vmem_vmem_arena, vmem_sz,
		    vmflag & VM_KMFLAGS);
	} else {
		ASSERT(id <= VMEM_INITIAL);
		ASSERT(vmem_sz == sizeof (vmem_t));
		vmp = &vmem0[id - 1];
	}
	...
	if (vmp->vm_cflags & VMC_NO_QCACHE) {
		ASSERT(nqcache == 0);
		vmp->vm_min_import =
		    VMEM_QCACHE_SLABSIZE(nqcache_arg << vmp->vm_qshift);
		nqcache = 0;
	}
...
vmem_destroy(vmem_t *vmp)
{
	...
	size_t nqcache = (vmp->vm_qcache_max >> vmp->vm_qshift);
	size_t vmem_sz = VMEM_SIZE(nqcache);
	size_t leaked;
	int i;
...
/*
 * Accessor functions for vm_private. Subsystems may find it helpful to
 * anchor private data structures off the vm_private during vmem creation
 * time. This may aid in quick retrieval of private data associated
 * with that vmem in the vm_source callback
 */
void *
vmem_get_private(vmem_t *vmp)
{
	return (vmp->vm_private);
}

void
vmem_set_private(vmem_t *vmp, void *ptr)
{
	vmp->vm_private = ptr;
}

 ...
 * Return UINT32_MAX (-1) if no set bit was found.
 */
static uint_t
bitset_find_in_word(ulong_t w, uint_t seed)
{
	uint_t rotate_bit, elt = UINT32_MAX;
...
/*
 * Find a bit that is set in the bitset in a non-victimizing fashion (e.g.
 * doesn't bias the low/high order bits/words). The 'lower' and 'upper'
 * arguments indicate lower and upper boundaries for the search. Passing zero
 * for both of these arguments indicates that the caller doesn't want to limit
 * the search space. We return UINT32_MAX (-1) if no set bit was found.
 */
uint_t
bitset_find(bitset_t *b, uint_t lower, uint_t upper)
{
	uint_t start, seed, idx, l, u, elt = UINT32_MAX;

	ASSERT(b->bs_words > 0);
	ASSERT(lower <= upper);

	/*
	 * Both 'lower' and 'upper' are specified in terms of elements, and
	 * then mapped to which ulong contains that element. This implies
	 * that the boundaries have a coarser granularity than the interface
	 * leads the caller to believe, but should be enough for most purposes.
	 */
	if (lower > 0) {
		l = BT_BITOUL(lower << b->bs_fanout) - 1;
		ASSERT3U(l, >=, 0);
	} else {
		l = 0;
	}
	...
	if (upper > 0) {
		u = BT_BITOUL(upper << b->bs_fanout) - 1;
		ASSERT3U(u, <=, b->bs_words);
	} else {
		u = b->bs_words - 1;
	}
	...
	if (u == l) {
		...
		/*
		 * We add one here since % requires the total number of ulongs,
		 * and not the difference between lower and upper bounds.
		 */
		idx = start = l + (seed % (u - l + 1));
	}

	do {
		elt = bitset_find_in_word(b->bs_set[idx], seed);
...
/*
 * Return 1 if bs1 contains bs2.
 */
int
bitset_contains(bitset_t *bs1, bitset_t *bs2)
{
	int i;
	ulong_t set;
	...
	for (i = 0; i < bs2->bs_words; i++) {
		set = bs2->bs_set[i];

		if (set == 0)
			continue;

		if ((bs1->bs_set[i] & set) != set)
			return (0);
	}

...
log_flushq(queue_t *q)
{
	...
	while ((mp = getq_noenab(q, 0, B_TRUE)) != NULL)
...
log_init(void)
{
	...
	printf("Copyright (c) 1983, 2011, Oracle and/or its affiliates. "
	    "All rights reserved.\n");
#ifdef DEBUG
...
log_conswitch(log_t *src, log_t *dst)
{
	...
	while ((mp = getq_noenab(src->log_q, 0, B_TRUE)) != NULL) {
	...
	size_t size, int on_intr)
{
	...
	if (size <= LOG_MSGSIZE &&
		...
		mp = getq_noenab(log_freeq, 0, B_TRUE);
	...
	if ((flags & SL_CONSOLE) && (lc->pri & LOG_FACMASK) == LOG_KERN) {
		...
		if ((lc->flags & SL_CONSONLY) == 0 &&
			...
			if (log_recentq->q_flag & QFULL)
				freemsg(getq_noenab(log_recentq, 0, B_TRUE));

    ...
    int flag, int wstat)
{
	...
	pu->pu_wstat = wstat;
	/*
	 * Compute average RSS in Kb.  The denominator is the number of
	 * samples:  the number of clock ticks plus the initial value.
	 */
	proc_rss_aggregate(p, &rss, &rss_max, &rss_samples);
	pu->pu_mem_rss_avg = (rss / (rss_samples + 1)) * (PAGESIZE / 1024);
	pu->pu_mem_rss_max = rss_max * (PAGESIZE / 1024);

...
devi_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	/* initialize NUMA IO info */
	if ((options_dip != NULL) && !is_pseudo_device(devi))
		numaio_dev_init(devi);
...
devi_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	...
	ASSERT(cmd == DDI_SUSPEND || cmd == DDI_DETACH);

	if (cmd == DDI_SUSPEND && e_ddi_parental_suspend_resume(devi)) {

...
pci_post_suspend(dev_info_t *dip)
{
	...
	/*
	 * Though we would have previously bailed if PPCF_NOPMCAP, we could
	 * have arrived her by fetching the "saved" pmcap values.  So
	 * check again and goto don if set.
	 */
	...
	/*
	 * XXX - Could the ppc_cap_offset change either across a
	 * s/r cycle, or even a hotplug cycle?  I wonder if we are
	 * best to always lookup the capabilities, rather than saving
	 * them as a device property.
	 */
...
#if defined(__x86)
	/*
	 * We only should do a setwake if this device has PME capability.
	 */
	if (pci_enable_wakeup &&
	    (p->ppc_suspend_level & PCI_PMCSR_PME_EN) != 0) {
		/*
		 * For x86, we also want to call the ACPI _DSW or _PRW
		 * methods, and only display a message if this fails.
		 */
		ret = acpi_ddi_setwake(dip, SYSTEM_POWER_S3);
		if (ret) {
	...
	if (p) {
		 ...
		 *
		 * Couldn't we make sure we are in D0 while performing
		 * the SLP_EN????
		 */
		...
		/*
		 * HACK ALLERT
		 * One problem of setting "D" states, is that they can
		 * render a disk controller unusable, and we won't be able
		 * to save the system state to storage.  The best answer,
		 * is to identify controllers that are used for this purpose,
		 * but this hack specifically prevents this in all PCI
		 * hardware.
		 */
		if (pm_cpr_save_state) {
			/*
			 * Don't disable i/o, memory access and bus mastering
			 * if saving the system state.
			 */
			pmcsr &= ~PCI_PMCSR_STATE_MASK;
		} else {
			/*
			 * According to 8.2.2 of "PCI Bus Power Management
			 * Interface Specification Revision 1.2":
			 * "When placing a function into D3, the operating
			 * system software is required to disable I/O and
			 * memory space as well as bus mastering via the
...
pci_pre_resume(dev_info_t *dip)
{
	...
	if (pci_enable_wakeup) {
		int retval;
		/*
		 * Note, that even though we only set PME devices
		 * for wakeup in pci_post_suspend, we wish to disable
		 * _any_ ability of a wakeup here.
		 */
		retval = acpi_ddi_setwake(dip, SYSTEM_POWER_S0);
		if (retval) {
...
int (*pciv_get_pf_list_p)(nvlist_t **);
int pciv_get_pf_list(nvlist_t **);
int (*pciv_get_vf_list_p)(char *, nvlist_t **);
int pciv_get_vf_list(char *, nvlist_t **);
int (*pciv_get_numvfs_p)(char *, uint_t *);
int pciv_get_numvfs(char *, uint_t *);
int (*pciv_param_get_p)(char *, nvlist_t **);
int pciv_param_get(char *, nvlist_t **);
int (*pciv_plist_lookup_p)(pci_plist_t, va_list);
int pciv_plist_lookup(pci_plist_t, ...);
int (*pciv_plist_getvf_p)(nvlist_t *, uint16_t, pci_plist_t *);
int pciv_plist_getvf(nvlist_t *, uint16_t, pci_plist_t *);
void (*pciv_class_config_completed_p)(char *);
void pciv_class_config_completed(char *);


int
pciv_get_pf_list(nvlist_t **nvlp)
{
	if (pciv_get_pf_list_p)
		return (*pciv_get_pf_list_p)(nvlp);
	return (DDI_FAILURE);
}

int
pciv_get_vf_list(char *pf_path, nvlist_t **nvlp)
{
	if (pciv_get_vf_list_p)
		return (*pciv_get_vf_list_p)(pf_path, nvlp);
	return (DDI_FAILURE);
}

int
pciv_get_numvfs(char *pf_path, uint_t *numvfs_p)
{
	if (pciv_get_numvfs_p)
		return (*pciv_get_numvfs_p)(pf_path, numvfs_p);
	return (DDI_FAILURE);
}

int
pciv_param_get(char *pf_path, nvlist_t **nvlp)
{
	if (pciv_param_get_p)
		return (*pciv_param_get_p)(pf_path, nvlp);
	return (DDI_FAILURE);
}

int
pciv_plist_lookup(pci_plist_t plist, ...)
{
	va_list alist;
	int	ret;

	if (pciv_plist_lookup_p) {
		va_start(alist, plist);
		ret = (*pciv_plist_lookup_p)(plist, alist);
		va_end(alist);
		return (ret);
	}
	return (DDI_FAILURE);
...
int
pciv_plist_getvf(nvlist_t *nvl, uint16_t vf_index, pci_plist_t *vfplist_p)
{
	if (pciv_plist_getvf_p) {
		return (*pciv_plist_getvf_p)(nvl, vf_index, vfplist_p);
	}
	return (DDI_FAILURE);
}

void
pciv_class_config_completed(char *pf_path)
{
	if (pciv_class_config_completed_p)
		(*pciv_class_config_completed_p)(pf_path);
}

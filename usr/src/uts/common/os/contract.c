 ...
 * structures representing the process bundle queues, p_ct_equeue.
 * p_ct_equeue and the queues it points to are allocated on demand
 * (i.e. contract creation or adoption, or ctfs pbundle file open) and
 * are not changed or freed until the process exits.
 *
...
static void cte_trim(ct_equeue_t *, contract_t *, int);
...
contract_abandon(contract_t *ct, proc_t *p, int explicit)
{
	...
	ASSERT(p->p_ct_equeue);
	ASSERT(p->p_ct_equeue[ct->ct_type->ct_type_index]);
	ct_equeue_t *q = p->p_ct_equeue[ct->ct_type->ct_type_index];
...
contract_adopt(contract_t *ct, proc_t *p)
{
	...
	/*
	 * Multiple contract locks are taken contract -> subcontract.
	 * ct_evtlock must be taken before any other.
	 */
...
contract_destroy(contract_t *ct)
{
	...
	cte_trim(&ct->ct_type->ct_type_events, ct, 0);
 ...
 *   listeners.
 *
 *   When a contract is implicitly abandoned, it is essential that all
 *   its events are removed from the owner's bundle queue.  In this
 *   case we forcibly kick listeners off the event rather than rely on
 *   lazy reclamation.
 *
 ...
 */
static void
cte_trim(ct_equeue_t *q, contract_t *ct, int force)
{
	ct_kevent_t *e, *next;
	ct_listener_t *l;
	int flags, stopper;
	...
	ASSERT(MUTEX_HELD(&q->ctq_lock));
	IMPLY(force, q->ctq_listno == CTEL_PBUNDLE);
	...
	for (e = list_head(&q->ctq_events); e != NULL; e = next) {
		...
		} else if ((e->cte_contract == ct) && force) {
			/*
			 * Force all listeners off this event immediately.
			 * Moving the last listener should cause e to
			 * be freed.  Since we hold q->ctq_lock, even
			 * if the pointer is reused it should be safe
			 * to continue comparing it against the rest of
			 * the queue's listeners' positions.
			 */
...
cte_queue_drain(ct_equeue_t *q, int ack)
{
	...
	/*
	 * Kick listeners off of any events they may be on.
	 */
	for (l = list_head(&q->ctq_listeners); l;
	    l = list_next(&q->ctq_listeners, l)) {

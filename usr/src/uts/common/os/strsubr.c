...
strinit(void)
{
	...
	pm_percpu_cnt = (1 << highbit(ncpus - 1));
	ASSERT(pm_percpu_cnt >= 1);
	/*
	 * Make it of the form 2**N - 1 in the range [0 .. ncpus - 1]
	 */
	pm_percpu_cnt--;
...
reset_nfsrv_ptr(queue_t *rqp, queue_t *wqp)
{
	...
	if ((rqp->q_flag & _QREMOVING) && (wqp->q_qinfo->qi_srvp != NULL)) {
		for (tmp_qp = backq(wqp);
		    ...
		    tmp_qp = backq(tmp_qp)) {
			tmp_qp->q_nfsrv = wqp->q_next->q_nfsrv;
		}
	}
...
str_stack_shutdown(netstackid_t stackid, void *arg)
{
	...
	/*
	 * Check if there is anything to shutdown first as
	 * we can be called multiple times.
	 */
	if (ss->ss_mux_nodes == NULL)
		return;

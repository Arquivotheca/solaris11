...
static int execsetid(vnode_t *, struct vattr *, uid_t *, uid_t *,
...
exece(uintptr_t file, const char **argp, const char **envp, int flags)
{
	...
	if (flags & ~(EXEC_DESCRIPTOR | EXEC_RETAINNAME | EXEC_ARGVNAME)) {
		error = EINVAL;
	} else if (flags & EXEC_DESCRIPTOR) {
		fname = NULL;
		if (file > INT_MAX)
			error = EBADF;
		else
		...
		fname = (const char *)file;
		execfd = -1;
	}
	if (error == 0)
		error = exec_common(execfd, fname, argp, envp, EBA_NONE, flags);
	return (error ? (set_errno(error)) : 0);
}

/*
 * Called from exec_common() when there is a non-NULL fname (!EXEC_DESCRIPTOR).
 *
 ...
 */
static int
exec_byname(const char *fname, pathname_t *pnp, pathname_t *resolvepnp,
    vnode_t **vpp, vnode_t **dirvpp)
{
	int error;

	if ((error = pn_get((char *)fname, UIO_USERSPACE, pnp)) != 0)
		return (error);
	pn_alloc(resolvepnp);
	if ((error = lookuppn(pnp, resolvepnp, FOLLOW, dirvpp, vpp)) == 0)
		return (0);
	pn_free(resolvepnp);
	pn_free(pnp);
	if (error != EINVAL)
	...
	*dirvpp = NULLVP;
	if ((error = pn_get((char *)fname, UIO_USERSPACE, pnp)) != 0)
		return (error);
	pn_alloc(resolvepnp);
	if ((error = lookuppn(pnp, resolvepnp, FOLLOW, NULLVPP, vpp)) == 0)
		return (0);
	pn_free(resolvepnp);
	pn_free(pnp);
	return (error);
...
/*
 * Called from exec_common() when there is a NULL fname (EXEC_DESCRIPTOR).
 *
 * Look up path name and remember last component for later.
 * Since we can't know the containg directory of the file,
 * we don't bother with it.  p_execdir will be NULL,
 * but coreadm is allowed to expand %d to the empty string and
 * there are other cases in which that failure may occur.
 */
static int
exec_byfd(int execfd, pathname_t *pnp, pathname_t *resolvepnp,
    vnode_t **vpp, vnode_t *rootvp)
{
	file_t *fp;
	vnode_t *vp;
	char *comp;
	...
	if (AU_AUDITING())
		audit_filepath(fp);
	vp = fp->f_vnode;
	if (vp->v_type != VREG && !pr_isobject(vp)) {
		releasef(execfd);
		pn_free(resolvepnp);
		pn_free(pnp);
		return (EACCES);
	...
	if ((path = vp->v_path) != NULL) {
		/*
		 * Advance over the rootdir portion of the vnode's pathname.
		 * It should not be visible to exec() in a chroot environment.
		 */
		if (rootvp != NULL &&
		    (rpath = rootvp->v_path) != NULL &&
		    strcmp(rpath, "/") != 0) {
			size_t len = strlen(rpath);
			if (strncmp(rpath, path, len) == 0) {
				if (*(path + len) == '/')
					path += len;
			}
		}
		if ((comp = strrchr(path, '/')) != NULL)
			...
			comp = path;
		if (strlen(path) < resolvepnp->pn_bufsize)
			(void) pn_set(resolvepnp, path);
		else
			(void) pn_set(resolvepnp, comp);
		(void) pn_set(pnp, comp);
...
int
exec_common(int execfd, const char *fname, const char **argp, const char **envp,
    int brand_action, int flags)
{
	vnode_t *vp;
	vnode_t *dir;
	vnode_t *tmpvp;
	...
	if (brand_action != EBA_NONE) {
		 ...
		 */
		if (ZONE_HAS_BRANDOPS(p->p_zone) && !PROC_HAS_BRANDOPS(p))
			brandme = B_TRUE;
	}

	if (flags & (EXEC_DESCRIPTOR | EXEC_ARGVNAME)) {
		/* get the process name from argv[0] */
		caddr_t argv0;

		if (p->p_model == DATAMODEL_NATIVE) {
			ulong_t ptr;
			if (fulword((caddr_t)argp, &ptr) != 0)
				return (EFAULT);
			argv0 = (caddr_t)ptr;
		} else {
			uint32_t ptr;
			if (fuword32((caddr_t)argp, &ptr) != 0)
				return (EFAULT);
			argv0 = (caddr_t)(uintptr_t)ptr;
		}
		/* pretend that argv[0] points to a pathname */
		if ((error = pn_get(argv0, UIO_USERSPACE, &pn)) != 0)
			return (error);
		if ((argv0 = strrchr(pn.pn_path, '/')) != NULL)
			argv0++;
		else
			argv0 = pn.pn_path;
		bzero(argv_name, MAXCOMLEN+1);
		(void) strncpy(argv_name, argv0, MAXCOMLEN);
		pn_free(&pn);
	}
	/*
	...
	/* while we're holding p_lock, get the rootvp for exec_byfd(), below */
	if (execfd >= 0 && (tmpvp = up->u_rdir) != NULL) {
		if (tmpvp == rootdir)
			tmpvp = NULL;
		else
			VN_HOLD(tmpvp);
	...
	if (execfd >= 0) {
		dir = NULL;
		error = exec_byfd(execfd, &pn, &resolvepn, &vp, tmpvp);
		if (tmpvp != NULL)
			VN_RELE(tmpvp);
	} else {
		error = exec_byname(fname, &pn, &resolvepn, &vp, &dir);
	}
	if (error)
		goto out;
	...
	if (execfd >= 0)	/* ua.fname is NULL for EXEC_DESCRIPTOR */
		args.fname = resolvepn.pn_path;
	 ...
	 */
	up->u_acflag &= ~AFORK;
	up->u_exflags = (short)flags;
	if (flags & EXEC_RETAINNAME)
		/* EMPTY */;
	else if (flags & (EXEC_DESCRIPTOR | EXEC_ARGVNAME))
		bcopy(argv_name, up->u_comm, MAXCOMLEN+1);
	else
	...
	if (uhrt_enable) {
		/*
		 * Map the kernel's hrt data into the user process.
		 */
		ASSERT(up->u_shared.hrt == NULL);
		up->u_shared.hrt = hrt_map();
	}
#endif	/* __x86 */
	...
	if (PROC_HAS_BRANDOPS(p) && (brand_action == EBA_NATIVE))
		brand_clearbrand(p, B_FALSE);
	...
	/*
	 * Home lgroup is inherited by default and if affinity inheritance
	 * flag is set.
	 */
	if (curthread->t_lgrp_aff != NULL &&
	    curthread->t_lgrp_aff->aff_inherit == LGRP_AFF_INHERIT_NONE)
		lgrp_move_thread(curthread,
		    lgrp_choose(curthread, curthread->t_cpupart), 1);
	else
	...
	/*
	 * lgroup affinities are inherited by default. They are also
	 * inherited if affinity inheritance flag is set.
	 */
	if (curthread->t_lgrp_aff != NULL &&
	    curthread->t_lgrp_aff->aff_inherit == LGRP_AFF_INHERIT_NONE &&
	    curthread->t_lgrp_aff->aff != NULL) {

		for (i = 0; i < nlgrpsmax; i++)
			curthread->t_lgrp_aff->aff[i] = LGRP_AFF_NONE;
	}
...
static int
execsetid(vnode_t *vp, struct vattr *vattrp, uid_t *uidp, uid_t *gidp,
...
int
execpermissions(vnode_t *vp, struct vattr *vattrp, uarg_t *args)
{
	int error;
	proc_t *p = ttoproc(curthread);
	 ...
	 * Must be a regular file or a /proc object file.
	 */
 ...
 */
int
execmap(vnode_t *vp, caddr_t addr, size_t len, size_t zfodlen,
...
noexec(
    ...
    struct cred *cred)
{
	cmn_err(CE_WARN, "missing exec capability for %s", args->pathname);
	return (ENOEXEC);
}

	...
	zoneid_t zoneid)
{
	...
	if (panicstr != NULL && panic_cpu.cpu_id != CPU->cpu_id) {
		/*
		 * Drop spurious high level interrupt messages from
		 * other CPUs during panic. kmem and log_sendmsg are
		 * non-reentrant at this point. Continuing from here
		 * causes heap corruption. Any actual problem causing
		 * a crash would have been logged already.
		 */
		return;
	}

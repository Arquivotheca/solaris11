...
proc_exit(int why, int what)
{
	...
	if ((q = p->p_child) != NULL && p != proc_init) {
		...
		p->p_child = NULL;
		ASSERT(list_is_empty(&p->p_ns_head));
	}
...
#define	_WNOCHLD	0400
	...
	while (pp->p_child != NULL) {
		...
		for (cp = list_head(&pp->p_ns_head); cp != NULL;
		    cp = list_next(&pp->p_ns_head, cp)) {
...
proc_detach(proc_t *p)
{
	...
	if (q->p_child == p) {
		 ...
		 */
		ASSERT(q->p_child ? 1 : list_is_empty(&q->p_ns_head));
	}
...
delete_ns(proc_t *parent, proc_t *child)
{
	...
	if (list_link_active(&child->p_ns_list))
		list_remove(&parent->p_ns_head, child);
}
...
{
	ASSERT(!list_link_active(&child->p_ns_list));
	list_insert_head(&parent->p_ns_head, child);
}

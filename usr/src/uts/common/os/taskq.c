...
taskq_bucket_extend(void *arg)
{
	 ...
	 */
	if (tq->tq_proc != &p0) {
		tqe->tqent_thread = lwp_kernel_create(tq->tq_proc,
		    taskq_d_thread, tqe, TS_STOPPED, tq->tq_pri);
	} else {
	 ...
	 */
	if (tq->tq_proc != &p0) {
		mutex_enter(&tq->tq_proc->p_lock);
		tqe->tqent_thread->t_proc_flag &= ~TP_HOLDLWP;
		tqe->tqent_thread->t_taskq = tq;
		lwp_create_done(tqe->tqent_thread);
		mutex_exit(&tq->tq_proc->p_lock);
		return;
	}

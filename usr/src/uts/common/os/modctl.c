...
iovcfg_modctl_fn_t	iovcfg_modctl_fn_p = NULL;
...
skip:;
	...
	/*
	 * Initialize the socket module list.
	 */
	smod_init();
    ...
    uintptr_t rval)
{
	...
	case MODHPOPS_REMOVE_PORT:
		...
		error = ddihp_modctl(DDI_HPOP_CN_REMOVE_PORT, devpath,
		    cn_name_str, NULL, NULL);
		break;
	case MODHPOPS_INSTALL:
		/* Probe the dependent devices of a hotplug connection */
		error = ddihp_modctl(DDI_HPOP_CN_PROBE, devpath,
		    cn_name_str, NULL, NULL);
		break;
	case MODHPOPS_UNINSTALL:
		/* Unprobe the dependent devices of a hotplug connection */
		error = ddihp_modctl(DDI_HPOP_CN_UNPROBE, devpath,
		    cn_name_str, NULL, NULL);
		break;
...
static int
modctl_iov(int subcmd, uintptr_t arg, uintptr_t arg_size,
    uintptr_t rval, uintptr_t rval_size)
{
	int	error = EINVAL;

	if (iovcfg_modctl_fn_p != NULL) {
		error = (*iovcfg_modctl_fn_p)(subcmd, arg, arg_size,
		    rval, rval_size);
	}
	return (error);
}
    ...
    uintptr_t a5)
{
	...
	case MODIOVOPS:
		error = modctl_iov((int)a1, a2, a3, a4, a5);
		break;
...
/* wait for disables to complete before beginning unload */
void
modunload_begin()
{
	mutex_enter(&modunload_wait_mutex);
 ...
 *
 * Minor perms in the clone:<driver> form are mapped to the driver's
...
/*
 * dynamically reference load a dl module/library, returning handle
 */
/*ARGSUSED*/
ddi_modhandle_t

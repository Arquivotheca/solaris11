...
{
	/*
	 * Given current bioclone(9F) documentation, which does not talk about
	 * needing to call bp_mapout() before a freerbuf() of a bioclone
	 * allocated buf structure, we need to call bp_mapout() here. The
	 * bp_mapout() call here might also catch other missing bp_mapout()
	 * calls too.
	 *
	 * NOTE: For now, to avoid introducting errors when drivers are
	 * backported, it is best if freerbuf callers take care of their
	 * own bp_mapout -- in which case the bp_mapout() below is a noop.
	 */
	bp_mapout(bp);

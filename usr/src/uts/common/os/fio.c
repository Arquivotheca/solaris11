...
static uint32_t afd_wait;	/* count of waits on non-zero uf_refcnt */
static uint32_t afd_busy;	/* count of waits on non-zero uf_afd_busy */
...
flist_grow(int maxfd)
{
	...
	for (src = oldlist, dst = newlist; src < oldend; src++, dst++) {
		...
		dst->uf_afd_busy = src->uf_afd_busy;
	}
 ...
 */
static file_t *
getf_common(int fd, int polling)
{
	uf_info_t *fip = P_FINFO(curproc);
	uf_entry_t *ufp;
	file_t *fp;
...
getf(int fd)
{
	return (getf_common(fd, 0));
}

/*
 * getf_poll() and releasef_poll() are like getf() and releasef()
 * and must be called as matched pair. Together, they arrange
 * that closeandsetf() will not mark the fd as stale (causing
 * the polling thread to draw EBADF).
 *
 * The callers of these functions must guarantee that they do not
 * go to sleep between the calls to getf_poll() and releasef_poll().
 */
file_t *
getf_poll(int fd)
{
	return (getf_common(fd, 1));
}
...
closeandsetf(int fd, file_t *newfp)
{
	...
	if (ufp->uf_refcnt > 0) {
		...
		/*
		 * We first wait until all polling threads have completed
		 * their getf_poll()...releasef_poll() on the given fd.
		 * Such threads should not draw EBADF just because
		 * the fds they are polling on get closed while they
		 * are polling.
		 *
		 * Then we call sprlock_proc(p) to ensure that the thread
		...
		while (ufp->uf_afd_busy) {
			COUNT(afd_busy);
			cv_wait(&ufp->uf_afd_cv, &ufp->uf_lock);
			/*
			 * cv_wait() drops ufp->uf_lock, so the file list
 ...
 */
static void
releasef_common(int fd, int polling)
{
	uf_info_t *fip = P_FINFO(curproc);
	uf_entry_t *ufp;
	...
	if (polling) {
		ASSERT(ufp->uf_afd_busy > 0);
		if (--ufp->uf_afd_busy == 0)
			cv_broadcast(&ufp->uf_afd_cv);
	}
	UF_EXIT(ufp);
}
...
releasef(int fd)
{
	releasef_common(fd, 0);
}

/*
 * See the block comment on getf_poll(), above.
 */
void
releasef_poll(int fd)
{
	releasef_common(fd, 1);
}
...
file_cache_constructor(void *buf, void *cdrarg, int kmflags)
{
	...
	mutex_init(&fp->f_rwlock, NULL, MUTEX_DEFAULT, NULL);
	return (0);
}
 ...
 * Called from utimensat() to set the utime and mtime.
 * The contents of *vap must be set before calling here.
 */
int
fsetattrat(int fd, char *path, int flag, struct vattr *vap, int vaflag)
{
	vnode_t		*startvp;
	vnode_t		*vp;
	int 		error;
	...
	if (vn_is_readonly(vp)) {
		...
		error = VOP_SETATTR(vp, vap, vaflag, CRED(), NULL);
	}

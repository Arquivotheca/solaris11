    ...
    void *arg, int (*callback)(const ldi_usage_t *, void *))
{
	 ...
	 *    when a persistent streams link is formed we can't associate
...
ldi_mlink_fp(struct stdata *stp, file_t *fpdown, int lhlink, int type)
{
	...
	if (type == LINKNORMAL) {
		/*
		 * if the link is not persistent then we can associate
		 ...
		 * persistent once the stream is destroyed the link is
		...
		/*
		 * if the link is persistent we can only associate the
		 ...
		 * persistent link operation is completed, a subsequent

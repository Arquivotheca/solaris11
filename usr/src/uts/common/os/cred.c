...
cred2ucaud(const cred_t *cr, auditinfo64_addr_t *ainfo, const cred_t *rcr)
{
	...
	ainfo->ai_mask.am_success_lo = AU_CLASS_LO(ai->ai_mask.as_success);
	ainfo->ai_mask.am_success_hi = AU_CLASS_HI(ai->ai_mask.as_success);
	ainfo->ai_mask.am_failure_lo = AU_CLASS_LO(ai->ai_mask.as_failure);
	ainfo->ai_mask.am_failure_hi = AU_CLASS_HI(ai->ai_mask.as_failure);
...
/*
 * Functions on the credgrp_t structures: alloc, hold, rele, sort.
 */

/* This routine consumes one ref of credgrp_t */
void
...
crgrpalloc(int n)
{
	...
	ASSERT(n > 0 || n <= ngroups_max);

	cg = kmem_alloc(CREDGRPSZ(n), KM_SLEEP);
	cg->crg_ref = 1;
	cg->crg_ngroups = n;
...
gid_t *
crgrpgetgroups(credgrp_t *cg)
{
	return (cg->crg_groups);
}

int
crgrpgetngroups(credgrp_t *cg)
{
	return (cg->crg_ngroups);
}

void
crgrpsort(credgrp_t *cg)
{
	ASSERT(cg->crg_ref == 1);

	if (cg->crg_ngroups > BIN_GROUP_SEARCH_CUTOFF)
		qsort(cg->crg_groups, cg->crg_ngroups, sizeof (gid_t), gidcmp);
}

 ...
 * CNA Manipulation
 *
 * Routines to generate CNA in CNA_DEV format are available:
 * fm_cna_dev_generate(). Individual fields within a CNA_DEV are extractable
 * via fm_cna_dev_gen_get(), and fm_cna_dev_timebase_get().
 */
 ...
 *	====================================================
 *	version			uint8_t		0
 *	[dev-cna]		uint64_t	<cna_dev>
 *	[auth]			nvlist_t	<auth>
 *	devpath			string		<devpath>
 *	[devid]			string		<devid>
 *	[target-port-l0id]	string		<target-port-lun0-id>
 *
 * Note that dev-cna, auth, devid, and target-port-l0id are optional members.
 */
void
fm_fmri_dev_set(nvlist_t *fmri_dev, int version, uint64_t cna_dev,
    const nvlist_t *auth, const char *devpath, const char *devid,
    const char *tpl0)
{
	int err = 0;
	...
	if (cna_dev)
		err |= nvlist_add_uint64(fmri_dev, EA_CNA_DEV, cna_dev);
...
fm_ena_increment(uint64_t ena)
{
	...
	/*
	 * The ena++ generation bit field should never
	 * overflow to other bits outside the field.
	 * Once it reach the highest value, stay at the
	 * highest value
	 */

	switch (ENA_FORMAT(ena)) {
	case FM_ENA_FMT1:
		if ((ena & ENA_FMT1_GEN_MASK) == ENA_FMT1_GEN_MASK)
			/* stay at highest generation number */
			new_ena = ena;
		else
			new_ena = ena + (1 << ENA_FMT1_GEN_SHFT);
		break;
	case FM_ENA_FMT2:
		if ((ena & ENA_FMT2_GEN_MASK) == ENA_FMT2_GEN_MASK)
			/* stay at highest generation number */
			new_ena = ena;
		else
...
/*
 * NOTE: nodes that are allocated early in boot, before boot_time is
 * established or i_ddi_di_cache_invalidate is called will have a
 * value of zero.
 */
uint64_t
fm_cna_dev_generate()
{
	return ((((uint64_t)boot_time) << CNA_DEV_TIMEBASE_SHFT) |
	    (devtree_gen & CNA_DEV_GEN_MASK));
}

extern uint32_t
fm_cna_dev_gen_get(uint64_t cna_dev)
{
	return ((uint32_t)(cna_dev & CNA_DEV_GEN_MASK));
}

extern time_t
fm_cna_dev_timebase_get(uint64_t cna_dev)
{
	return ((time_t)(cna_dev >> CNA_DEV_TIMEBASE_SHFT));
}

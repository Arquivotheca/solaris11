...
/*
 * Return lgroup with given lgrp id, or NULL if the specified id
 * is not valid or doesn't match a valid lgrp.
 */
lgrp_t *
lgrp_get(lgrp_id_t id)
{
	lgrp_t *lgrp;

	if (id > lgrp_alloc_max)
		return (NULL);
...
lgrp_madv_to_policy(uchar_t advice, size_t size, int type)
{
	...
	case MADV_ACCESS_MANY_PSET:
		return (LGRP_MEM_POLICY_RANDOM_PSET);
	default:
...
/*
 * Return lgroup memory allocation policy given advice from shmadv()
 */
lgrp_mem_policy_t
lgrp_shmadv_to_policy(uint_t advice)
{
	switch (advice) {
	case SHM_ACCESS_LWP:
		return (LGRP_MEM_POLICY_NEXT);
	case SHM_ACCESS_MANY:
		return (LGRP_MEM_POLICY_RANDOM);
	case SHM_ACCESS_MANY_PSET:
		return (LGRP_MEM_POLICY_RANDOM_PSET);
	default:
		return (LGRP_MEM_POLICY_DEFAULT);
	}
}
...
lgrp_mem_choose(struct seg *seg, caddr_t vaddr, size_t pgsz)
{
	...
	/*
	 * If this thread belongs to vmtask, then use the lgroup of the user
	 * thread that initiated the vm operation for placement decisions.
	 */
	if (curthread->t_vmtask_user != NULL) {
		thrd = curthread->t_vmtask_user;
		lgrp = lgrp_table[lgrp_home_id(thrd)];
	} else {
		thrd = curthread;
		/*
	...
	case LGRP_MEM_POLICY_RANDOM_PROC:
		 ...
		 * this process
		 */
		klgrpset_copy(lgrpset, ttoproc(thrd)->p_lgrpset);
	...
	case LGRP_MEM_POLICY_RANDOM_PSET:
		...
		if (klgrpset_isempty(lgrpset)) {
			 ...
			 */
			kpreempt_disable();
			klgrpset_copy(lgrpset, thrd->t_cpupart->cp_lgrpset);
			kpreempt_enable();
		}

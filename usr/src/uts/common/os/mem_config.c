...
static int chk_osize_page(page_t *, pfn_t, pfn_t, struct mem_handle *);
		...
		for (mdsp = mhp->mh_transit.trl_spans; (mdsp != NULL) &&
			...
			for (pfn = mdsp->mds_base; (pfn < p_end) &&
				...
				if (PP_TOXIC(pp)) {
					...
					/*
					 * If the delete range overlaps part of
					 * a page then demote it.
					 */
					if (!chk_osize_page(pp, mdsp->mds_base,
					    p_end, mhp)) {
...
mem_del_stat_print_func(struct mem_handle *mhp)
{
	...
	if (mem_del_stat_print) {
		...
		printf("\t%8u nosizepage\n", mhp->mh_delstat.nosizepage);
...
/*
 * Handles large pages which are greater than the granularity for DR. If
 * such a page partially overlaps the range of memory being deleted then
 * demote it.
 */
/*ARGSUSED*/
static int
chk_osize_page(page_t *pp, pfn_t md_base, pfn_t md_end,
    struct mem_handle *mhp)
{
	pfn_t base_pfn, end_pfn;

	base_pfn = PFN_BASE(pp->p_pagenum, pp->p_szc);
	end_pfn = base_pfn + page_get_pagecnt(pp->p_szc);

	if ((md_base <= base_pfn) && (md_end >= end_pfn)) {
		return (1);
	}

	MDSTAT_INCR(mhp, nosizepage);
	return (page_try_demote_pages(pp));
}

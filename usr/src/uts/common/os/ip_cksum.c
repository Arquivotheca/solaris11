 ...
 *	 checksummed, this routine will handle additional data prefixed within
	...
	if (mp->b_cont == NULL) {
		...
		if (dp->db_struioflag & STRUIO_IP) {
			...
			if (mlen && dp->db_cksumstart != dp->db_cksumstuff &&
				 ...
				 * data has already been checksummed and there
	...
	for (; ; ) {
		...
		if (pmp) {
			...
			if (mlen && dp->db_cksumstart != dp->db_cksumstuff) {
				 ...
				 * uio data has already been checksummed,
			...
			douio:
				...
				if (dp->db_cksumend != dp->db_cksumstuff) {
					 ...
					 * the uio data nees to be checksummed,

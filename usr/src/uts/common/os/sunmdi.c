...
mdi_vhci_unregister(dev_info_t *vdip, int flags)
{
	...
	mdi_devi_set_dma_attr(vdip, NULL);
...
mdi_phci_unregister(dev_info_t *pdip, int flags)
{
	...
	mdi_devi_set_dma_attr(pdip, NULL);
...
/* Invalidate the cached devinfo snapshot and generate an event. */
static void
i_mdi_log_addrem_event(char *subclass, mdi_pathinfo_t *pip)
{
	(void) i_ddi_log_event(subclass, NULL, pip,
 ...
 * 		Utility function to return indication of whether client is
 *		in a failed state.
 *
 * Returns:	0 = Client is not in failed state
 *		1 = Client is in failed state
 */
int
mdi_client_is_failed(dev_info_t *cdip)
{
	mdi_client_t	*ct;
	dev_info_t	*pdip;
	mdi_vhci_t	*vh;
	...
	ASSERT(pdip != NULL);
	vh = i_devi_get_vhci(pdip);
	ASSERT(vh != NULL);

	MDI_VHCI_CLIENT_LOCK(vh);
	/* If the node has no client, it is considered FAILED. */
	if ((ct = i_devi_get_client(cdip)) != NULL) {
		MDI_CLIENT_LOCK(ct);
		is_failed = MDI_CLIENT_IS_FAILED(ct);
		MDI_CLIENT_UNLOCK(ct);
	}
	MDI_VHCI_CLIENT_UNLOCK(vh);
	return (is_failed);
    ...
    int least_desired_pi)
{
	...
	while (pip) {
		MDI_PI_LOCK(pip);
		if (least_desired_pi &&
		    mdi_pi_get_path_instance(pip) == least_desired_pi) {
		    goto lb_next_path;
		}
 ...
 *		appropriate flags.  The meaning of the third argument depends
    ...
    void *arg, mdi_pathinfo_t **ret_pip)
{
	...
	int		cond, cont = 1;
	int		retry = 0;
	mdi_pathinfo_t	*start_pip = NULL; /* request starting pathinfo */
	int		path_instance = 0;
	int		least_desired_pi = 0;
	mdi_pathinfo_t	*least_desired_pip = NULL;

	/* determine type of arg based on flags */
	if (flags & MDI_SELECT_PATH_INSTANCE) {
		path_instance = (int)(intptr_t)arg;
	} else {
		if (flags == MDI_SELECT_PATH_INSTANCE_AVOID_LD) {
		    /*
		     * MDI_SELECT_PATH_INSTANCE_AVOID_LD has arg be specified
		     * as the least desired path instance. This flag is
		     * supposed to be used for standard behavior.
		     */
		    least_desired_pi = (int)(intptr_t)arg;
		} else {
	...
	case LOAD_BALANCE_NONE:
		...
		do {
			...
			if ((MDI_PI(pip)->pi_state  ==
			    ...
			    /*
			     * Try to avoid selecting the least desired path
			     * instance.
			     */
			    if (least_desired_pi &&
				    mdi_pi_get_path_instance(pip) ==
				    least_desired_pi) {
				least_desired_pip = pip;
				goto none_next_path;
			    }
...
none_next_path:
	...
	case LOAD_BALANCE_LBA:
		...
		if ((ct->ct_lb_args != NULL) &&
			...
			if (i_mdi_lba_lb(ct, ret_pip, bp, least_desired_pi)
				    == MDI_SUCCESS) {
				MDI_CLIENT_UNLOCK(ct);
				return (MDI_SUCCESS);
			}
			...
			if (sb) {
				cond = ((MDI_PI(pip)->pi_state ==
					...
					MDI_PI(pip)->pi_preferred ==
						preferred) ? 1 : 0);
				/*
				 * skip the least desired path instance only
				 * for standard behavior.
				 */
				if (least_desired_pi && cond &&
					(mdi_pi_get_path_instance(pip) ==
					 least_desired_pi)) {
				    least_desired_pip = pip;
				    cond = 0;
...
do_again:
	...
	if (retry == 1) {
		MDI_CLIENT_UNLOCK(ct);
		return (MDI_BUSY);
	} else if (least_desired_pip != NULL){
		/*
		 * If entering here, it means that we have no choices but the
		 * least desired path.
		 */
		MDI_PI_LOCK(least_desired_pip);
		MDI_PI_HOLD(least_desired_pip);
		MDI_PI_UNLOCK(least_desired_pip);
		ct->ct_path_last = least_desired_pip;
		*ret_pip = least_desired_pip;
		MDI_CLIENT_UNLOCK(ct);
		return (MDI_SUCCESS);
	} else {
		MDI_CLIENT_UNLOCK(ct);
    ...
    char **compatible, int ncompatible, int flags, mdi_pathinfo_t **ret_pip)
{
	...
	*ret_pip = NULL;
	...
	if (pip == NULL) {
		...
		/*
		 * Check the client to see if it has been marked removed.
		 * If so, reflect that state in this path in order to drive
		 * proper behavior from, for example, scsi_device_config
		 */
		if (DEVI_IS_DEVICE_REMOVED(cdip)) {
			...
			/* Generate EC_DEVFS pathinfo add event. */
			i_mdi_log_addrem_event(ESC_DEVFS_PATHI_ADD, pip);
		}
...
i_mdi_pi_alloc(mdi_phci_t *ph, char *paddr, mdi_client_t *ct)
{
	 ...
	 */
	mdi_pi_set_cna_dev(pip, fm_cna_dev_generate());
...
i_mdi_pi_free(mdi_phci_t *ph, mdi_pathinfo_t *pip, mdi_client_t *ct)
{
	...
	/*
	 * If the path is not already set as removed, generate an
	 * an EC_DEVFS remove event.
	 */
	if (!MDI_PI_FLAGS_IS_DEVICE_REMOVED(pip))
		i_mdi_log_addrem_event(ESC_DEVFS_PATHI_REMOVE, pip);
...
uint64_t
mdi_pi_get_cna_dev(mdi_pathinfo_t *pip)
{
	if (pip)
		return (MDI_PI(pip)->pi_cna_dev);
	else
		return (0);
}

void
mdi_pi_set_cna_dev(mdi_pathinfo_t *pip, uint64_t cna_dev)
{
	if (pip)
		MDI_PI(pip)->pi_cna_dev = cna_dev;
}
...
mdi_pi_set_state(mdi_pathinfo_t *pip, mdi_pathinfo_state_t state)
{
	...
	if (pip) {
		...
		i_ddi_di_cache_invalidate(NULL, pip);
	}
}
 ...
 *		then a leading ! is implied (but can be overridden via
...
/*
 * A vHCI that wants to maintain a combined/merged ddi_dma_attr(9S) must call
 * mdi_devi_set_dma_attr() to register the address of the vHCI ddi_dma_attr_t
 * used for data transfer. This must occur during vHCI attach, after
 * mdi_vhci_register(). For class "scsi", scsi_hba_attach_setup() registers
 * &tran->tran_dma_attr.
 * 
 * For proper operation, all pHCI drivers associated with this vHCI must
 * call mdi_devi_set_dma_attr to register the address of the pHCI
 * ddi_dma_attr_t used for data DMA. This must occur during pHCI attach,
 * after mdi_phci_register(). For class "scsi", the SCSA code takes 
 * care of this: scsi_hba_attach_setup() registers &tran->tran_dma_attr,
 * and scsi_hba_detach() registers NULL.
 *
 * pHCI calls result in updates to the combined/merged vHCI ddi_dma_attr(9S).
 *
 * NOTE: this is being implemented as a new interface instead of new arguments
 * to mdi_vhci_register/mdi_phci_register to minimize any backport complexity.
 */
void
mdi_devi_set_dma_attr(dev_info_t *dip, ddi_dma_attr_t *dma_attr)
{
	mdi_vhci_t	*vh;
	mdi_phci_t	*ph;
	int		first = 1;
	...
	vh = i_devi_get_vhci(dip);
	if (vh) {
		vh->vh_dma_attr = dma_attr;
		return;				/* vHCI pointer update, done */
	}

	ph = i_devi_get_phci(dip);
	if (ph == NULL)
		return;				/* not a pHCI - noop */

	ph->ph_dma_attr = dma_attr;		/* pHCI pointer update */

	vh = ph->ph_vhci;
	if ((vh == NULL) || (vh->vh_dma_attr == NULL))
		return;

	/* pHCI pointer update results in merge and vHCI contents update */
	MDI_VHCI_PHCI_LOCK(vh);
	for (ph = vh->vh_phci_head; ph; ph = ph->ph_next) {
		if (ph->ph_dma_attr == NULL)
			continue;
		if (first) {
			first = 0;
			vd = *(ph->ph_dma_attr);
		} else
			ddi_dma_attr_merge(&vd, ph->ph_dma_attr);
	}
...
mdi_pi_device_remove(mdi_pathinfo_t *pip)
{
	...
	/* Generate an EC_DEVFS event for path removal. */
	i_mdi_log_addrem_event(ESC_DEVFS_PATHI_REMOVE, pip);
	...
	if (ct && ct->ct_dip && i_mdi_client_all_devices_removed(ct))
		rval += ndi_devi_device_remove(ct->ct_dip);
 ...
 * is now accessible, then this interface is used to represent device insertion
 * as well as driving the path online.
 */
int
mdi_pi_device_insert(mdi_pathinfo_t *pip)
{
	...
	/* Generate an EC_DEVFS pathinfo add event. */
	i_mdi_log_addrem_event(ESC_DEVFS_PATHI_ADD, pip);

	/*
	 * Bring the path online.  Also, now that there is at least one
	 * accessible path, ensure the client is no longer marked as removed.
	 */
	(void) mdi_pi_online(pip, 0);

	ct = MDI_PI(pip)->pi_client;
	if (ct && ct->ct_dip)
		rval += ndi_devi_device_insert(ct->ct_dip);
 ...
 *		pHCI.  This decision is made by the vHCI, using cinfo identity

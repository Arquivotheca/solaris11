...
pci_ereport_setup(dev_info_t *dip)
{
	...
	/*
	 * If device is not ereport capable then report an error against the
    ...
    uint8_t version, ...)
{
	...
	fm_fmri_dev_set(detector, FM_DEV_SCHEME_VERSION, ddi_cna_dev(dip),
	    NULL, device_path, NULL, NULL);
...
pci_check_regs(dev_info_t *dip, void *arg)
{
	...
	if (tgt_err->tgt_pci_space == PCI_REG_ADDR_G(PCI_ADDR_CONFIG)) {
		 ...
		 * for any non-relocatable mapping, otherwise check
		 * assigned-addresses.
		 */
		if (ddi_getlongprop(DDI_DEV_T_NONE, dip, DDI_PROP_DONTPASS,
		    "reg", (caddr_t)&drv_regp, &reglen) != DDI_SUCCESS)
...
pci_target_drain(void *private_p, pci_target_err_t *tgt_err)
{
	 ...
	 * are configured as transparent. Find the top-level pci

 ...
 * levels of secpolicy; the policy_override and the va-list functions
 * add a few more levels. Plus one other we may not be interested in, so
 * we may need as many as 10 but no more.
 */
#define	MAXPRIVSTACK		10
 ...
 * to B_FALSE.
 *
 * RESTRICTIONS IN CODE IN THE ERROR PATH
 *
 * When the privilege system determines that required privileges aren't
 * available, the error handling is called.
 *
 * It is important that no routines are called in the error handling which
 * may themselves involve privilege checking.  Recursion and panic will
 * result.
 *
 * When memory is needed, memory should be allocated with KM_NOSLEEP as
 * multiple locks might be held by the thread calling the secpolicy
 * routines.
 */
...
priv_policy_errmsg(const cred_t *cr, int priv, const char *msg)
{
	...
	ASSERT(priv == PRIV_ALL || priv == PRIV_MULTIPLE ||
	    priv == PRIV_ALLZONE || priv == PRIV_GLOBAL || priv == PRIV_MWAC ||
	...
	if (priv_debug) {
		zcmn_err(me->p_zone->zone_id, CE_NOTE, fmt, cmd, me->p_pid,
		    pname, cr->cr_uid, curthread->t_sysnum, msg, sym, off);
	}
}
...
static char *
priv_find_vpath(const cred_t *cr, va_list oap)
{
	va_list ap;
	vnode_t *vp;
	char *nm;
	...
	if ((int)va_arg(ap, int) != KLPDARG_VNODE) {
		va_end(ap);
		return (NULL);
	}

	vp = (vnode_t *)va_arg(ap, vnode_t *);
	nm = (char *)va_arg(ap, char *);
	...
	/*
	 * vnodetopath may again involve the privilege code so we avoid it.
	 */
	if (priv_build_path(cr->cr_zone->zone_rootvp, vp, path, nm, cr,
	    B_TRUE) != 0) {
    ...
    va_list ap)
{
	...
	if (priv_debug || (CR_FLAGS(cr) & PRIV_DEBUG) ||
		...
		/* Only create the pathname if we want to use it. */
		if (str == NULL && (priv_debug || (CR_FLAGS(cr) & PRIV_DEBUG)))
			str = priv_find_vpath(cr, ap);

		if (allzone && !HAS_ALLZONEPRIVS(cr)) {
			priv_policy_errmsg(cr, PRIV_ALLZONE, str);
		} else {
			ASSERT(!HAS_PRIVILEGE(cr, priv));
			priv_policy_errmsg(cr, priv, str);
		}
		if (str != NULL && str != msg)
    ...
    const char *msg, va_list ap)
{
	if ((HAS_PRIVILEGE(cr, priv) && (!allzone || HAS_ALLZONEPRIVS(cr))) ||
		...
		priv_policy_err(cr, priv, allzone, msg, ap);
	}
	return (err);
}
    ...
    const char *msg, ...)
{
	...
	if (req == PRIV_FULLSET || priv_isfullset(req)) {
		va_start(ap, msg);
		priv_policy_err(cr, PRIV_ALL, B_FALSE, msg, ap);
		va_end(ap);
		return (EACCES);
	}
...
secpolicy_fs_mount_clearopts(cred_t *cr, struct vfs *vfsp)
{
	...
	if (vfs_optionisset(vfsp, MNTOPT_NORSTCHOWN, NULL) && !amsuper)
		vfs_setmntopt(vfsp, MNTOPT_RSTCHOWN, NULL, 0);
    ...
    mode_t curmode, mode_t wantmode)
{
	...
	/*
	 * You would expect a check for (wantmode & WRITE) and a check
	 * against the FILE_WRITE privilege.  However, when we want
	 * to check FILE_WRITE we need to know about exactly which file
	 * we write to or want to create, we need more information.
	 * That's why we call secpolicy_basic_file_write() earlier
	 * with an optional additional argument: the basename of the file.
	 * secpolicy_basic_file_write() is called in each relevant fop_*()
	 * instead of being called later here.
	 */
...
int
secpolicy_vnode_chown(const cred_t *cred, vfs_t *vfsp, uid_t owner)
{
	boolean_t is_owner = (owner == crgetuid(cred));
	boolean_t allzone = B_FALSE;
	int priv;
	boolean_t norstchown = vfs_has_feature(vfsp, VFSFT_NORSTCHOWN);
	int error;
	...
	/*
	 * Handle PRIV_FILE_CHOWN_SELF as an implicit privilege if
	 * the file system was mounted without rstchown
	 */

	if (priv == PRIV_FILE_CHOWN_SELF && norstchown && is_owner)
		error = 0;
	else
		error = PRIV_POLICY(cred, priv, allzone, EPERM, NULL);
	return (error);
}
...
secpolicy_xvattr(xvattr_t *xvap, uid_t owner, cred_t *cr, vtype_t vtype)
{
	...
	if (error == 0 && XVA_ISSET_REQ(xvap, XAT_REPARSE))
	...
	void *node)
{
	...
	if (mask & (AT_UID|AT_GID)) {
		 ...
		 */
		if (checkpriv &&
		    (error = secpolicy_vnode_chown(cr, vp->v_vfsp,
		    ovap->va_uid)) != 0) {
			goto out;
		}
...
/*
 * PRIV_SYS_DL_CONFIG is a superset of PRIV_SYS_FLOW_CONFIG.
 */
int
secpolicy_flow_config(const cred_t *cr)
{
	if (PRIV_POLICY_ONLY(cr, PRIV_SYS_NET_CONFIG, B_FALSE))
		return (secpolicy_net_config(cr, B_FALSE));
	if (PRIV_POLICY_ONLY(cr, PRIV_SYS_DL_CONFIG, B_FALSE))
		return (secpolicy_dl_config(cr));
	return (PRIV_POLICY(cr, PRIV_SYS_FLOW_CONFIG, B_FALSE, EPERM, NULL));
}
...
int
secpolicy_share(const cred_t *cr)
{
	return (PRIV_POLICY(cr, PRIV_SYS_SHARE, B_FALSE, EPERM, NULL));
}
...
/*
 * Called from the relevant fop_* functions.
 */
int
secpolicy_basic_file_write(const cred_t *cr, vnode_t *vp, const char *pn)
{
	if ((cr->cr_zone->zone_flags & ZF_LOCKDOWN) &&
	    (vp->v_vfsp->vfs_flag & VFS_MWAC) &&
	    priv_policy_mwac(cr, vp, pn) != 0) {
		return (EROFS);
	}

	/* Sockets are checked with the net_access basic privilege. */
	if (vp->v_type == VSOCK)
		return (0);
...
int
secpolicy_mwac_protected(const cred_t *cr, vnode_t *vp)
{
	if ((cr->cr_zone->zone_flags & ZF_LOCKDOWN) &&
	    (vp->v_vfsp->vfs_flag & VFS_MWAC) &&
	    priv_policy_mwac(cr, vp, NULL) != 0) {
		return (EROFS);
	}
	return (0);
}
...
/*
 * secpolicy_uvfs_server
 *
 * Determine if the subject has permission to interact with UVFS mounts,
 * by providing the file system with a new door.
 */
int
secpolicy_uvfs_server(const cred_t *cr)
{
...
/*
 * secpolicy_procfs_kaddr
 *
 * Determine if the subject has sufficient privileges to see kernel addresses
 * via proc(4) - basically ADDR and WCHAN fields in ps(1).
 */
int
secpolicy_procfs_kaddr(const cred_t *cr)
{
	return (PRIV_POLICY_ONLY(cr, PRIV_ALL, B_TRUE) ? 0 : EPERM);
}

 ...
 * the variables related to adjusted high-resolution time in hrt structure
 * are adjusted under hrt->hres_lock -- and this lock may be held when we
 * enter probe context.  One might think that we could address this by
 * having a single snapshot copy that is stored under a different lock from
 * hres_tick(), using the snapshot iff hrt->hres_lock is locked in probe
 * context.  Unfortunately, this too won't work:  because hrt->hres_lock is
 * grabbed in more than just hres_tick() context, we could enter probe
 * context concurrently on two different CPUs with both locks (hrt->hres_lock
 * and the snapshot lock) held.  As this implies, the fundamental problem is
 * that we need to have access to a snapshot of these variables that
 * we _know_ will not be locked in probe context.  To effect this, we have two
 * snapshots protected by two different locks, and we mandate that these
 * snapshots are recorded in succession by a single thread calling
 * dtrace_hres_tick().  (We assure this by calling it out of the same
 * CY_HIGH_LEVEL cyclic that calls hres_tick().)  A single thread can't be in
 * two places at once:  one of the snapshot locks is guaranteed to be unheld
 * at all times.  The dtrace_gethrestime() algorithm is thus to check first
 * one snapshot and then the other to find the unlocked snapshot.
 */
void
dtrace_hres_tick(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		spl = hr_clock_lock();
		tmp.dthr_hrestime.tv_sec = hrt->hrestime.tv_sec;
		tmp.dthr_hrestime.tv_nsec = hrt->hrestime.tv_nsec;
		tmp.dthr_adj = hrt->hrestime_adj;

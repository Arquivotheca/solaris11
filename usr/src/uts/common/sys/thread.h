...
typedef struct _kthread {
	...
	uint32_t t_flag;	/* modified only by current thread */
	uint32_t t_proc_flag;	/* modified holding ttoproc(t)->p_lock */
	uint32_t t_schedflag;	/* modified holding thread_lock(t) */
...
#define	t_wchan		t_lwpchan.lc_wchan
	...
	void		*t_lgrp_reserv;	/* reserved for future */
	struct _kthread	*t_vmtask_user;	/* user of vmtask thread */
	...
	ushort_t	t_acflag;	/* accounting flags */
	...
	hrtime_t	t_disp_time;	/* last time this thread was running */
...
#define	t_post_sys_ast	_tu._t_post_sys_ast
	...
	struct lgrp_aff	*t_lgrp_aff;	/* lgroup affinity */
...
#define	t_dtrace_reg	_tdu._tds._t_dtrace_reg
	...
	ushort_t	t_wkld_flags;	/* workload characterization flags */
	uint8_t		t_transience;	/* thread transience counter */
	kmutex_t	t_ac_lock;	/* thread CPU time accounting lock */
	void		*t_zone;	/* zone containing this thread */
	void		*t_task;	/* task containing this thread */
} kthread_t;
 ...
 *		T_WAKEABLE
 */
#define	T_INTR_THREAD	0x00001	/* thread is an interrupt thread */
#define	T_WAKEABLE	0x00002	/* thread is blocked, signals enabled */
#define	T_TOMASK	0x00004	/* use lwp_sigoldmask on return from signal */
#define	T_TALLOCSTK	0x00008	/* thread structure allocated from stk */
#define	T_FORKALL	0x00010	/* thread was cloned by forkall() */
#define	T_WOULDBLOCK	0x00020	/* for lockfs */
#define	T_DONTBLOCK	0x00040	/* for lockfs */
#define	T_DONTPEND	0x00080	/* for lockfs */
#define	T_SYS_PROF	0x00100	/* profiling on for duration of system call */
#define	T_WAITCVSEM	0x00200	/* waiting for a lwp_cv or lwp_sema on sleepq */
#define	T_WATCHPT	0x00400	/* thread undergoing a watchpoint emulation */
#define	T_PANIC		0x00800	/* thread initiated a system panic */
#define	T_LWPREUSE	0x01000	/* stack and LWP can be reused */
#define	T_CAPTURING	0x02000	/* thread is in page capture logic */
#define	T_VFPARENT	0x04000	/* thread is vfork parent, must call vfwait */
#define	T_DONTDTRACE	0x08000	/* disable DTrace probes */
#define	T_PUSHPAGE	0x10000	/* thread can tap pageout reserve */
...
/*
 * Thread accounting flags (t_acflag) definitions.
 *
 * TA_NO_PROCESS_LOCK
 *
 * Historically, the process lock used to be held when the scheduling class
 * tick processing function (CL_TICK()) was called for a thread. This was done
 * to protect certain states from changing during LWP accounting.
 *
 * ON scheduling classes (TS, IA, FX, RT, FSS and SYSDC) have been enhanced
 * to not require the process lock to be held while doing CL_TICK(). This has
 * been done to improve scalability for multi-threaded processes.
 *
 * However, there are still a few cases where the process lock does need to
 * be held.
 *
 *	1. Third-party scheduling classes that have not been enhanced in
 *	   the same way would expect the process lock to be held across
 *	   the CL_TICK() function.
 *
 *	2. If the time quantum signal is set for a realtime thread (RT_DOSIG),
 *	   then the the process lock is needed to send a signal to a process
 *	   when the thread's time quantum expires.
 *
 *	3. When FX callbacks are registered for a thread in the FX
 *	   scheduling class, the process lock needs to be held since
 *	   the callback functions are third-party and they would expect the
 *	   lock to be held.
 *
 * TA_NO_PROCESS_LOCK is set in the scheduling class code for a thread to
 * indicate to the LWP accounting code that the process lock is not needed
 * for CL_TICK().
 */
#define	TA_NO_PROCESS_LOCK	0x0001
 ...
 *
 * ttotask(x)
 * 	convert a thread pointer to its task pointer.
 *
 * ttoproj(x)
...
/*
 * Routine for manipulating T_PUSHPAGE thread flag
 */
extern void	thread_setpushpage(void);
...
extern void	clock_tick_init(void);
...
/*
 * thread_transience_{user,kernel} causes user/kernel threads to be marked
 * as non-transient, effectively turning off this optimization for that specific
 * set of threads. Setting both to one will disable it for the entire system.
 */
extern uint_t thread_transience_pct;
extern uint_t thread_transience_thresh;
extern boolean_t thread_transience_user;
extern boolean_t thread_transience_kernel;

extern void thread_transience_update(kthread_t *);

/*
 * Thresholds for workload characterization.
 *
 * - T_TRANSIENCE_PCT is the CPU utilization threshold at/under which
 *   threads are flagged as transient. This is a 32bit scaled integer to
 *   facilitate comparing against t_pctcpu. A simple formula to extract
 *   the percentage is (uint64_t)((t_pctcpu * 100) >> 31). Note that you should
 *   the result by the number of processors in a MP system if you're looking
 *   for the system wide %. The default value is 0.02%.
 *
 * - T_TRANSIENCE_THRESH is the t_transience value at/above which a thread
 *   is considered transient. The t_transience counter is incremented when
 *   the thread is characterized as transient. When the counter reaches
 *   this threshold, the thread is considered transient.
 *
 * - T_CPU_INTENSIVE_PCT is the CPU utilization threshold at/above which
 *   threads are flagged as CPU intensive. The default value is 30%.
 */
#define	T_TRANSIENCE_PCT	(((1U<<31)/100)/50)
#define	T_TRANSIENCE_THRESH	(10)
#define	T_TRANSIENT(t)		(t->t_transience >= thread_transience_thresh)

#define	T_CPU_INTENSIVE_PCT	(((1U<<31)/100) * 30)

/*
 * Flags and macros for workload characterization (t_wkld_flags).
 *
 * - T_WKLD_PERMANENT indicates that the thread has fixed values for its
 *   workload characterization flags, and they should not be further modified.
 *
 * - T_WKLD_CPU_INTENSIVE indicates that the thread stays on proc long enough
 *   such that its t_pctcpu is equal or larger than T_CPU_INTENSIVE_PCT.
 */
#define	T_WKLD_PERMANENT	(0x01)
#define	T_WKLD_CPU_INTENSIVE	(0x02)

#define	T_WKLD_IS_PERMANENT(t)	(t->t_wkld_flags & T_WKLD_PERMANENT)
#define	T_WKLD_IS_CPU_INT(t)	(t->t_wkld_flags & T_WKLD_CPU_INTENSIVE)

...
struct dev_mode_page {
	...
	uchar_t	prmwp:		1,	/* PeRManent Write Protect */
		perswp:		1,	/* persistent write protection */
...
#if defined(_BIT_FIELDS_LTOH)
#define	SEQ_DEVICE_SPECIFIC_PARAMETER \
...
#define	MODE_PARAMETER_HEADER_6 \
	uchar_t	data_len;	/* sense data length, sense only */ \
	uchar_t	media_type;	/* medium type, sense only */ \
	SEQ_DEVICE_SPECIFIC_PARAMETER \
	uchar_t	bd_len		/* block descriptor length in bytes */

typedef struct {
	MODE_PARAMETER_HEADER_6;
}mode_parameter_header6;

#if defined(_BIT_FIELDS_LTOH)
#define	MODE_PARAMETER_HEADER_10 \
	uchar_t	data_len[2];	/* sense data length, sense only */ \
	uchar_t	media_type;	/* medium type, sense only */ \
	SEQ_DEVICE_SPECIFIC_PARAMETER \
	uchar_t		:7, \
		LONGLBA	:1; \
	uchar_t		:8; \
	uchar_t	bd_len[2]	/* block descriptor length in bytes */
#else
#define	MODE_PARAMETER_HEADER_10 \
	uchar_t	data_len[2];	/* sense data length, sense only */ \
	uchar_t	media_type;	/* medium type, sense only */ \
...
#define	TWO_BYTE(in)	(in[1] | (in[0] << 8))
...
typedef union {
	mode_parameter_header6 six;
	mode_parameter_header10 ten;
}mode_parameter_header;

#define	MODE_PARAMETER_BLOCK_DESCRIPTOR \
...
typedef struct {
	MODE_PARAMETER_BLOCK_DESCRIPTOR;
}mode_block_descriptor;

#if defined(_BIT_FIELDS_LTOH)
#define	MODE_PAGE_HEADER_COMMON \
	uchar_t	page_code:	6, \
		subpf:		1, \
...
struct seq_mode {
	MODE_PARAMETER_HEADER_6;
	MODE_PARAMETER_BLOCK_DESCRIPTOR;
	MODE_PAGE_HEADER_COMMON;
	uchar_t page_len[1];
	modepage page;
};

/*
 * Digital Archive Data Protection (DADP) mode page.
 * Notice that it uses subpage format with two byte lenth field.
 */
typedef struct {
#if defined(_BIT_FIELDS_HTOL)
	uchar_t ps:		1,	/* Page Savable sense only */
		subpf:		1,
		page_code:	6;
	uchar_t	subpage_code;
	uchar_t page_len[2];		/* two bytes of length */
	uchar_t method;
	uchar_t	:		2,
		prot_len:	6;
	uchar_t	LBP_W:		1,
...
typedef struct {
	const char *(*dp_name)(void);
	uint32_t (*dp_len)(dp_setable const *set, uint32_t data_len);
	void (*dp_add)(dp_setable const *set, caddr_t datap, uint32_t blk_len);
	boolean_t (*dp_chk)(dp_setable const *set, caddr_t datap,
	    uint32_t blk_len);
	uint32_t (*dp_crc)(dp_setable const *set, const caddr_t datap,
	    uint32_t blk_len);
}dp_funcs;

extern uint32_t no_crc(dp_setable const *, const caddr_t, uint32_t);
...
extern const dp_funcs crc32c_software;
#if defined(__i386) || defined(__amd64)
extern const dp_funcs crc32c_xhardware;
#endif
...
struct scsi_tape {
	...
	mode_parameter_header *un_mode_data;
	ushort_t un_mode_data_len;
	uchar_t un_mode_data_cmd;
	dadp_info un_dadp;
	tape_dadp_state un_dadp_state;
};
...
#define	ST_ENTR(d, fn) if (st_debug & 0x10) { DEBLOCK(d) \
    scsi_log(d, st_label, lev, __func__); DEBUNLOCK(d) }

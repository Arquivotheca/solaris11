...
struct pmcs_phy {
	...
	uint32_t	ncphy;		/* # of contained phys for expander */
	...
	uint16_t	sysevent;	/* Bits indicating SAS event info */
	...
	uint8_t		phy_cc_delta;	/* PHY change count delta (jitter) */
...
#define	PMCS_WORK_SATA_PHY_UP		1
#define	PMCS_WORK_REGISTER_DEV		2
...
#define	PMCS_WORK_ABORT_COMPLETION	13
#define	PMCS_WORK_BC_PROCESS		14

/*
 * The actual values as they appear in work_flags
 */
#define	PMCS_WORK_FLAG_DISCOVER		(1 << 0)
#define	PMCS_WORK_FLAG_SATA_PHY_UP	(1 << 1)
#define	PMCS_WORK_FLAG_REGISTER_DEV	(1 << 2)
...
#define	PMCS_WORK_FLAG_ABORT_COMPLETION	(1 << 13)
#define	PMCS_WORK_FLAG_BC_PROCESS	(1 << 14)
...
#define	PHY_CHANGED(pwp, p) {						\
	...
	if (p->changed == 0) {						\
		p->config_stop = ddi_get_lbolt() +			\
			drv_usectohz(PMCS_MAX_CONFIG_TIME);		\
	}								\
	if (p->iport) {							\
		pmcs_set_iport_changed(p->iport);			\
	}								\
	p->changed = 1;							\
	p->enum_attempts = 0;						\
}
...
#define	PMCS_CQ_RUN_LOCKED(hwp)						\
	if ((!STAILQ_EMPTY(&hwp->cq) || hwp->iocomp_cb_head) &&		\
	    (hwp->cq_info.cq_threads_running == 0)) {			\
 ...
 *
 */
typedef struct pmcs_abort_comp_s {
	uint32_t	iomb[PMCS_QENTRY_SIZE];
	size_t		amt;
 ...
 * Get/Set ACK info
 */
#define	PMCS_ACK_INFO(w1)		(w1 & 0x00ffffff)

#define	PMCS_NEED_ACK(pwp, phynum)	((pwp)->ack_bitmap & (1 << phynum))

#define	PMCS_SET_ACK(pwp, phynum, ackinfo) {			\
	(pwp)->ack_info[(phynum)] = (ackinfo);			\
	(pwp)->ack_bitmap |= (1 << (phynum));			\
	SCHEDULE_WORK((pwp), PMCS_WORK_SAS_HW_ACK);		\
	(void) ddi_taskq_dispatch((pwp)->tq, pmcs_worker,	\
	    (pwp), DDI_NOSLEEP);				\
}

#define	PMCS_CLEAR_ACK(pwp, phynum)	(pwp)->ack_bitmap &= ~(1 << (phynum));

/*
 * Get/Set BROADCAST CHANGE Info
 */
#define	PMCS_NEED_BC_PROCESS(pwp, phynum) ((pwp)->bc_bitmap & (1 << phynum))

#define	PMCS_SET_BC_PROCESS(pwp, phynum) {			\
	(pwp)->bc_bitmap |= (1 << (phynum));			\
	SCHEDULE_WORK((pwp), PMCS_WORK_BC_PROCESS);		\
	(void) ddi_taskq_dispatch((pwp)->tq, pmcs_worker,	\
	    (pwp), DDI_NOSLEEP);				\
}

#define	PMCS_CLEAR_BC_PROCESS(pwp, phynum) (pwp)->bc_bitmap &= ~(1 << (phynum));

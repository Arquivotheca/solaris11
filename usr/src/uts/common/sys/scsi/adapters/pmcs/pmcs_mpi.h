...
/* IOP_EVENT_IT_NEXUS_LOSS */
#define	PMCS_MPI_CUST_ABORT_ITNL	0x2	/* Bit 1 */

/* RETURN_ERROR_ON_OUT_OF_IOST_RESOURCE */
#define	PMCS_IB_FAIRQ_BIT		0x10 	/* Bit 4 */

/* ENABLE_OUTBOUND_PROCESSING_FAIRNESS */
#define	PMCS_OB_FAIRQ_BIT		0x20	/* Bit 5 */

/* ENABLE_FWASSERT_ON_STALE_RESPONSE_FRAME */
#define	PMCS_IOST_CHECK_BIT		0x100	/* Bit 8 */

#define	PMCS_SET_CUSTOM(pwp, bits)	{				\
	uint32_t cust = pmcs_rd_mpi_tbl(pwp, PMCS_MPI_CUSTSET);		\
	cust |= (bits);							\
	pmcs_wr_mpi_tbl(pwp, PMCS_MPI_CUSTSET, cust);			\
}

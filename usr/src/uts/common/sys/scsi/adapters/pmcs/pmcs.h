...
struct pmcs_xscsi {
	uint32_t
		...
		abort_pending	:	1,		/* on behalf of phy */
		draining	:	1,
		new		:	1,
		assigned	:	1,
		dev_gone	:	1,
...
struct pmcs_iport {
	...
	boolean_t	iport_changed;	/* New observation is required */
	kmutex_t	iport_changed_lock;	/* Protects iport_changed */
	/* SMP serialization */
	kmutex_t	smp_lock;
	kcondvar_t	smp_cv;
...
typedef struct pmcs_scratch {
	uint64_t scratch_dma;
	void *scratch_addr;
	volatile uint8_t scratch_lock;
	void *scratch_held_by;
	hrtime_t scratch_lock_time;
} pmcs_scratch_t;
...
struct pmcs_hw {
	...
	ddi_acc_handle_t	scratch_acchdls;
	...
	ddi_dma_attr_t		scratch_dma_attr;
	...
	/*
	 * Producer/Queue Host Memory Pointers and DMA scatter/gather chunk
	 * areas.
	 *
	...
	/*
	 * Scratch area pointer and DMA address for SATA and SMP operations.
	 * scratch_addr = pointer to entire region.
	 * scratch = array of pmcs_scratch_t's, one for each scratch area.
	 */
	void			*scratch_addr;
	pmcs_scratch_t		*scratch;
	uint64_t		scratch_dma;
	...
	/*
	 * Register dump loaded from flash on HBA
	 */
	caddr_t		flash_regdumpp;
	...
	/*
	 * Used for offline (non-intr thread) processing of abort completions
	 */
	kmem_cache_t	*abort_comp_cache;
	kmutex_t	abort_comp_list_lock;	/* Protects abort_comp_list */
	list_t		abort_comp_list;	/* Abort completion list */

	/*
	 * Used for offline (non-intr thread) processing of hardware acks
	 */
	kmutex_t	ack_lock;	/* Protects ack_bitmap & ack_info */
	uint32_t	ack_bitmap;	/* Bitmap of PHYs requiring ACKs */
	uint32_t	*ack_info;	/* SEA/PhyID/PortID ack value */

	/*
	 * Used for offline (non-intr thread) processing of BROADCAST_CHANGE
	 */
	uint32_t	bc_bitmap;	/* PHYs requiring BC processing */

...
/*
 * Limit for the event payload size, for events published from userland.
 * The maximum memory that a channel may use is determined by the maximum
 * event size and the channel depth;  since publishing requires root
 * privilege the exposure is further limited.
 */
#define	MAX_EV_SIZE_LEN		(SHRT_MAX / 2)

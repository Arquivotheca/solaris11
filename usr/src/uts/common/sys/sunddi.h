...
#define	ESC_DEVFS_PATHI_ADD	"ESC_devfs_pathi_add"
#define	ESC_DEVFS_PATHI_REMOVE	"ESC_devfs_pathi_remove"
...
#define	NUM_VF_NVLIST_NAME		"#vfs"
#define	REASON_NVLIST_NAME		"reason"
#define	VF_PARAM_NVLIST_ARRAY_NAME	"VF_PARAM"

typedef void	*pci_plist_t;
...
/*
 * bind multiple IO vectors to DMA handle
 */
...
typedef void	*pci_param_t;

/*
 * The following are used by PF driver call back function.
 */
#define	PCIV_NOTAPPLICABLE	0x08
#define	PCIV_REQRESET		0x10
#define	PCIV_REQREATTACH	0x11


int
pciv_vf_config(dev_info_t *, pciv_config_vf_t *);

int pci_param_get(dev_info_t *dip, pci_param_t *param_p);
int pci_param_free(pci_param_t param);
int pci_plist_get(pci_param_t param, pci_plist_t *plist_p);
int pci_plist_lookup(pci_plist_t plist, ...);
int pci_plist_getvf(pci_param_t param, uint16_t vf_index,
    pci_plist_t *plist_p);
int pci_param_get_ioctl(dev_info_t *dip, intptr_t ioctl_arg, int ioctl_mode,
    pci_param_t *);
...
int pci_plist_lookup_plist(pci_plist_t plist, const char *name,
    pci_plist_t *val);
int pci_plist_lookup_byte_array(pci_plist_t plist, const char *name,
...
typedef void	*pciv_param_handle_t;
...
/*
 * Inter-domain communication interfaces
 */
...
typedef struct pciv_validate_config_list {
	char			*reason_p;
	pciv_param_handle_t	ph;
} pciv_validate_config_list_t;

...
typedef struct proc_ac_ctl {
	struct proc_ac	*pac;		/* per-set array of accounting data */
	size_t		pac_offset;	/* array alignment offset */
	int		pac_multi;	/* multi-threaded process */
	kthread_t	*pac_thread;	/* first thread */
} proc_ac_ctl_t;
...
#define	CLDPEND_LIST_HEAD(cp)       \
	(cp)->p_parent->p_cldpend_head
...
typedef struct	proc {
	...
	struct brand	*p_brand;	/* process's brand module ptr */
	...
	proc_ac_ctl_t	p_ac_ctl;	/* process accounting control block */

	/*
	 * p_cldpend_head:  empty: no outstanding CLDPENDS for the parent.
	 *		   !empty: head of all children with CLDPENDS
	 * p_cldpend_list:  empty: no CLDPEND for this proc.
	 *		   !empty: list of siblings + this proc with CLDPENDs
	 * These list are protected by pidlock
	 */
	list_t p_cldpend_head;
	list_node_t p_cldpend_list;
	/*
	 * p_ns_head:	    empty: no newstate for the proc children.
	 *		   !empty: head of all children with a newstate
	 * p_ns_list:	    empty: no newstate for the proc
	 *		   !empty: list of siblings + this proc with newstate
	 * These lists are protected by the pidlock
	 */
	list_t p_ns_head;
	list_node_t p_ns_list;
} proc_t;
...
extern int profil_poll(proc_t *, klwp_t *, uint64_t);

#define	PROFIL_POLL(pp, lwp, delta)	\
	(((pp)->p_prof.pr_scale) ? profil_poll(pp, lwp, delta) : 0)
...
extern void sigcld_list_create(proc_t *);
extern void sigcld_insert(proc_t *);
extern void sigcld_remove(proc_t *);
...
extern	void	proc_ac_init(void);
extern	void	proc_ac_create(proc_t *);
extern	void	proc_ac_destroy(proc_t *);
extern	int	proc_cpu_time_add(proc_t *, int, uint64_t);
extern	void	proc_cpu_time_update(proc_t *);
extern	void	proc_cpu_time_check(proc_t *);
extern	void	proc_rss_aggregate(proc_t *, uint64_t *, size_t *, ulong_t *);
extern	void	proc_rss_update(proc_t *, int);

...
/*
 * User process page locking support during uiomove()
 */
typedef struct uio_pagelock_seg {
	caddr_t	base;		/* iov_base */
	size_t	len;		/* 0 < len <= iov_len */
	void	**pplist;	/* list of pages mapping above */
} uio_pagelock_seg_t;

typedef struct uio_pagelock {
	void			*as;		/* address space */
	int			rw;		/* access type */
	int			nsegs;		/* length of segs[] */
	uio_pagelock_seg_t	*segs;		/* array of pagelock segments */
	uio_pagelock_seg_t	sseg;		/* used when nsegs = 1 */
} uio_pagelock_t;
...
int	_uio_pagelock(size_t n, enum uio_rw rw, struct uio *uio,
    uio_pagelock_t *pglock);
void	_uio_pageunlock(uio_pagelock_t *pglock);

...
typedef enum {
	...
	PMR_PPM_ENTER_SX,		/* ppm: enter ACPI S[1-4] state	*/
	PMR_PPM_EXIT_SX,		/* ppm: exit ACPI S[1-4] state	*/
...
/*
 * These *should* be defined in sunpm.h along with other ACPI-like defines.
 * But consumers don't always include that file, and other definitions
 * from that file are not needed by most consumers of these defines.
 * However, these are the "System" power states, and are based on the
 * ACPI definitions.  Mostly like ACPI, the "S2" state is more ambiguous
 * than others, but the others track reasonably well.
 */
#define	SYSTEM_POWER_S0		0
#define	SYSTEM_POWER_S1		1
#define	SYSTEM_POWER_S2		2
#define	SYSTEM_POWER_S3		3
#define	SYSTEM_POWER_S4		4
#define	SYSTEM_POWER_S5		5
#define	SYSTEM_POWER_STATE_MAX	SYSTEM_POWER_S5
#define	SYSTEM_POWER_STATE_CNT	(SYSTEM_POWER_STATE_MAX + 1)
#define	SYSTEM_POWER_OVERRIDE	SYSTEM_POWER_STATE_MAX

/*
 * This structure defines a capability for a particular power "state".
 * For each power manageable component (specific device, group of
 * devices, or the entire system), there would be zero or more of
 * these elements that would describe state capabilities.
 *
 *   cp_state is expected to align with equivilent ACPI states.
 *   cp_ttmr is the latency (in usec) in entering this state.
 *   cp_ttfc is the latency to return to maximum capacity state.
 *   cp_capacity is the percent of full capacity in this state.
 *   cp_power is the percent of full power in this state.
 */
typedef struct pm_capability {
	uint8_t		cp_state;
	uint8_t		cp_capacity;
	uint8_t		cp_power;
	uint8_t		cp_flags;
...
/* These are adjusted for single percentage values. */
#define	PM_CAP_CAPACITY_NONE	0
#define	PM_CAP_CAPACITY_MAX	100
#define	PM_CAP_CAPACITY_UNKNOWN	255
#define	PM_CAP_POWER_NONE	0
#define	PM_CAP_POWER_MAX	100
#define	PM_CAP_POWER_UNKNOWN	255

extern pm_capability_t *pm_syscap[SYSTEM_POWER_STATE_CNT];

extern major_t	cpr_device;
extern int	pm_suspend_enabled;
extern int	pm_cpr_save_state;
extern int	pm_cpr_state;
extern int	cpr_platform_enable;
extern int	cpr_supported_override;
extern int	pm_cpr_state_override;

/*
 * Struct passed as arg to appm_ioctl
 * This should either be renamed or rolled into one of the other
 * CPR structures (expecially if we want to remove the ppm linkage).
 */
typedef struct sx_args {
	int		sxa_state;	/* See SYSTEM_POWER_Sx above */
	int		sxa_test_point;	/* test point */
	uint64_t	sxa_wakephys;	/* restart vector physical addr */
	void		*sxa_psr;	/* apic state save buffer */
} sxa_t;
...
#define	PM_PSTATE_LATENCY	70
#define	PM_CPU_DEEP_IDLE_LATENCY	200

extern uint64_t	pm_pstate_latency_val;
extern uint64_t	pm_cpu_deep_idle_latency_val;

/*
 * enable P-states and C-states by default
 */
#define	PM_PLAT_DEFAULT_TTFC	(pm_cpu_deep_idle_latency_val+50)
/*
 * this currently has no effect
 */
#define	PM_PLAT_DEFAULT_TTMR	0

typedef struct pm_kernel_config {
	pm_authority_t	pm_auth;
	boolean_t	pm_suspend_enabled;
	boolean_t	pm_plat_disabled;
	boolean_t	pm_curr_disabled;
	uint64_t	pm_curr_ttfc;
	uint64_t	pm_curr_ttmr;
	uint64_t	pm_plat_ttfc;
	uint64_t	pm_plat_ttmr;
} pm_kernel_config_t;

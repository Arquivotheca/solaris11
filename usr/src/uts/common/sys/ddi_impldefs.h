...
typedef enum {
	...
	DDI_CB_PCIV_CONFIG_VF,		/* used by PF drivers */
	DDI_CB_COMM_RECV,		/* Inter-domain communiation */
	DDI_CB_PARAM_READ,		/* return dev params */
	DDI_CB_PARAM_VALIDATE,		/* validate the param list */
	DDI_CB_LSR_SUSPEND,		/* Live suspend */
	DDI_CB_LSR_RESUME,		/* Live resume */
	DDI_CB_LSR_QUERY_CAPABILITY	/* supported LSR types */
} ddi_cb_action_t;

typedef enum {
	DDI_CB_FLAG_INTR = 0x1,		/* Driver is IRM aware */
	DDI_CB_FLAG_SRIOV = 0x2,	/* Drive is SRIOV aware */
	DDI_CB_FLAG_COMM = 0x4,		/* Driver is IOV aware */
	DDI_CB_FLAG_LSR = 0x8,		/* Driver is LSR aware */
	DDI_CB_FLAG_PARAM = 0x10	/* get param and validate supported */
} ddi_cb_flags_t;

#define	DDI_CB_FLAG_MASK	0xf

#define	DDI_CB_FLAG_VALID(f)	((f) & DDI_CB_FLAG_MASK)
...
#define	DDI_CB_LSR_ACT_DMA			0x1
#define	DDI_CB_LSR_ACT_PIO 			0x2
#define	DDI_CB_LSR_ACT_INTR			0x4

#define	DDI_CB_LSR_IMP_DMA_ADDR_CHANGE		0x01
#define	DDI_CB_LSR_IMP_DMA_PROP_CHANGE		0x02
#define	DDI_CB_LSR_IMP_DEVICE_RESET		0x04
#define	DDI_CB_LSR_IMP_DEVICE_REPLACE		0x08
#define	DDI_CB_LSR_IMP_LOSE_POWER		0x10

/*
 * Definitions for interdomain communication interfaces
 */
typedef enum {
...
#define	PCIV_PF		-1	/* Default func address for PF */
#define	PCIV_FRAMEWORK	-2	/* Reserved address for PCIv framework */
#define	PCIV_INVAL_VF	-3	/* Invalid VF func index number */

typedef uint64_t dom_id_t;	/* Domain handle */

typedef struct ddi_pciv_recv_event {
	pciv_event_type_t	type;		/* event type */
...
extern int pciv_register_notify(dev_info_t *dip);
extern int pciv_unregister_notify(dev_info_t *dip);

#define	PCIV_WAIT	0	/* set to pvp_flag for blocking transfer */
#define	PCIV_NOWAIT	1	/* set to pvp_flag for non-blocking transfer */
...
struct dev_info  {
	...
	kcondvar_t	devi_config_cv;		/* cv for bus config ops */
	...
	/* detach event data */
	char		*devi_ev_path;
	char		*devi_ev_devidstr;
	int		devi_ev_instance;

	/* NUMA IO info */
	void		*devi_numaio;

	/* CNA_DEV "Configuration Numeric Association" of the node */
	uint64_t	devi_cna_dev;

	/*
	 * The delivered properties record the original properties
	 * provided in the driver.conf, which may be supplemented
	 * with additional properties and/or merged values from
	 * the per-site additions.
	 */
	ddi_prop_t *devi_vdr_prop_ptr;		/* delivered prop list */
	ddi_prop_t *devi_adm_prop_ptr;		/* site prop list */
	ddi_prop_list_t *devi_vdr_global_props; /* delivered global prop list */
	ddi_prop_list_t *devi_adm_global_props; /* site global prop list */
};
...
#define	DEVI_DEVICE_LIVE_SUSPENDED	0x00000010
...
#define	DEVI_IS_DEVICE_LIVE_SUSPENDED(dip)				\
	(DEVI(dip)->devi_state & DEVI_DEVICE_LIVE_SUSPENDED)

#define	DEVI_SET_DEVICE_LIVE_SUSPENDED(dip)	{			\
	ASSERT(mutex_owned(&DEVI(dip)->devi_lock));			\
	DEVI(dip)->devi_state |= DEVI_DEVICE_LIVE_SUSPENDED;		\
	}

#define	DEVI_CLR_DEVICE_LIVE_SUSPENDED(dip)	{			\
	ASSERT(mutex_owned(&DEVI(dip)->devi_lock));			\
	DEVI(dip)->devi_state &= ~DEVI_DEVICE_LIVE_SUSPENDED;		\
	}
 ...
 * devi_flags bits
 *
 * NOTE: all devi_flags updates should be protected by devi_lock.
 */
...
#define	DEVI_ANNOTATIONS	0x00004000 /* dip supports zone annotations */
#define	DEVI_ASSIGNED		0x00008000 /* dev assigned to another domain */
#define	DEVI_CONFIG_ALL_CMPLT	0x00010000 /* BUS_CONFIG_ALL is complete */
...
#define	DEVI_IS_ASSIGNED(dip)	(DEVI(dip)->devi_flags & DEVI_ASSIGNED)

char	*i_ddi_devi_class(dev_info_t *);
int	i_ddi_set_devi_class(dev_info_t *, char *, int);

/*
 * x64 regspec/rangespec are widened to contain 64-bit base/len
 */
...
struct regspec {
	...
	uint_t regspec_addr_high;	/* address of reg relative to bus */
	uint_t regspec_addr_low;
	uint_t regspec_size_high;	/* size of this register set */
	uint_t regspec_size_low;
};

#define	REGSPEC_ADDR64(p)	((((uint64_t)(p)->regspec_addr_high) << 32) \
	| (p)->regspec_addr_low)

#define	REGSPEC_SIZE64(p)	((((uint64_t)(p)->regspec_size_high) << 32) \
	| (p)->regspec_size_low)

#define	SPLIT_REGSPEC_ADDR(a, p) \
	(p)->regspec_addr_high = (uint32_t)((uint64_t)(a) >> 32),\
	(p)->regspec_addr_low = (uint32_t)((uint64_t)(a) & 0xFFFFFFFF)

#define	SPLIT_REGSPEC_SIZE(s, p) \
	(p)->regspec_size_high = (uint32_t)((uint64_t)(s) >> 32),\
	(p)->regspec_size_low = (uint32_t)((uint64_t)(s) & 0xFFFFFFFF)
 ...
 */
struct rangespec {
	uint_t rng_caddr_high;		/* Child's address, hi order */
	uint_t rng_caddr_low;		/* Child's address, lo order */
	uint_t rng_addr_high;		/* Parent's address, hi order */
	uint_t rng_addr_low;		/* Parent's address, lo order */
	uint_t rng_size_high;		/* size of space, hi order */
	uint_t rng_size_low;		/* size of space, lo order */
};

#define	RANGESPEC_ADDR64(p)	((((uint64_t)(p)->rng_addr_high) << 32) \
	| (p)->rng_addr_low)

#define	RANGESPEC_CADDR64(p)	((((uint64_t)(p)->rng_caddr_high) << 32) \
	| (p)->rng_caddr_low)

#define	RANGESPEC_SIZE64(p)	((((uint64_t)(p)->rng_size_high) << 32) \
	| (p)->rng_size_low)
 ...
 *
 * For a pure dynamic property to show up in a di_init() devinfo snapshot, the

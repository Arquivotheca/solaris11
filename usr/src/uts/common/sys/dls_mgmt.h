...
/*
 * The slot name associated with the physical device.
 *
 * Set for physical links only.
 */
#define	FPHYLOC		"loc"		/* string */

/*
 * The door file for the dlmgmtd (data-link management) daemon.
 */
#define	DLMGMT_TMPFS_DIR	_PATH_SYSVOL "/dladm"
...
#define	DLMGMT_CMD_VLINK_UPDATE		10
#define	DLMGMT_CMD_GETNEXTPHYSATTR	11
#define	DLMGMT_CMD_GETPHYS_PREFIX	12
...
/*
 * Return datalinks in all zones when caller is in GZ
 */
#define	DLMGMT_ALLZONES		0x04

/*
 * Signifies given datalink is an Auto VNIC datalink
 */
#define	DLMGMT_AUTOVNIC		0x08
...
typedef struct dlmgmt_door_getnextphysattr_s {
	int			ld_cmd;
	uint32_t		ld_flags;
	char			ld_dev[MAXLINKNAMELEN];
} dlmgmt_door_getnextphysattr_t;
...
typedef struct dlmgmt_door_vlink_update {
	int			ld_cmd;
	datalink_id_t		ld_linkid;
	datalink_id_t		ld_vlinkid;
	uint16_t		ld_vid;
	boolean_t		ld_isprim;
	boolean_t		ld_added;
} dlmgmt_door_vlink_update_t;

typedef struct dlmgmt_door_getphys_prefix {
	int			ld_cmd;
} dlmgmt_door_getphys_prefix_t;
...
typedef struct dlmgmt_getnext_retval_s {
	uint_t			lr_err;
	char			lr_zoneprefixlink[MAXLINKNAMESPECIFIER];
...
typedef struct dlmgmt_getnextphysattr_retval_s {
	uint_t			lr_err;
	datalink_id_t		lr_linkid;
	char			lr_dev[MAXLINKNAMELEN];
} dlmgmt_getnextphysattr_retval_t;

typedef struct dlmgmt_getname_retval_s {
	uint_t			lr_err;
	char			lr_link[MAXLINKNAMESPECIFIER];
	datalink_class_t	lr_class;
	uint32_t		lr_media;
	uint32_t		lr_flags;
	zoneid_t		lr_zoneid;
...
typedef struct dlmgmt_getphys_prefix_retval_s {
	uint_t			lr_err;
	char			lr_phys_prefix[MAXLINKNAMELEN];
} dlmgmt_getphys_prefix_retval_t;

...
#define	CARRIER_UNKNOWN(qlge)	mac_link_update((qlge)->mh, LINK_STATE_UNKNOWN)
/*
...
#define	ql_get_next_index(index, length)	\
	((((index) + 1) == length)?0:(index) + 1)

#define	ql_get_index_range(index, length, count)	\
	((((index) + (count)) >= length)? \
...
#define	INIT_MC_LIST_ALLOC		BIT_19
...
#define	MAX_MULTICAST_LIST_SIZE	4096
...
struct bq_desc {
	...
	uint32_t	ref_cnt;
					 ...
					 * 1 created, but not used or recycled;
					 * 2 currently held by OS
					 */
};
...
typedef struct ql_dmah_node {
	struct ql_dmah_node *next;
...
struct tx_ring_desc {
	...
	uint32_t		tx_bytes;
	void			*oal;
	ql_dmah_node_t		*head, *tail;	/* for map mode */
};
...
struct tx_ring {
	...
	uint64_t		tx_too_many_dma_hdl;
	uint64_t		tx_out_of_dma_hdl;
	uint64_t		tx_no_dma_cookie;
	uint64_t		tx_pullup;
	/* dma handles */
	ql_dmah_node_t	tx_dma_hdls[MAX_TX_RING_DMA_HDLS];
	ql_dmah_node_t	*dmah_free_list[MAX_TX_RING_DMA_HDLS];
...
struct rx_data {
	...
	/* free thread that belongs to previous driver instance */
	boolean_t		free_old_thread;

	uint32_t		buf_q_ptr_space;
	/* completion queue */
...
struct rx_ring {
	...
	/* for test purpose */
	uint32_t		tx_comp;
	/* dynamically allocated rx ring resources */
	struct rx_data		*rx_data;
};
...
#define	MAX_POWER_LEVEL		0
	 ...
	 * Multicast list
	 */
#define	MCAST_ALLOC_SIZE	128
	uint32_t		multicast_list_count;
	uint32_t		multicast_list_alloc_count;
	ql_multicast_addr	*multicast_list;
...
extern void ql_get_and_report_link_state(qlge_t *qlge, uint32_t link_state);
extern unsigned int ql_read_sh_reg(qlge_t *qlge, struct rx_data *rx_data);
extern void ql_try_to_clean_outbound_rx_ring(struct rx_ring *ob_ring);

...
#define	DI_PROP_VENDOR_LIST	4
#define	DI_PROP_ADMIN_LIST	5
#define	DI_PROP_VENDOR_GLB_LIST	6
#define	DI_PROP_ADMIN_GLB_LIST	7
#define	DI_PROP_NLISTS		8
...
#define	DI_SNAPSHOT_VERSION_3	3	/* reserved */
#define	DI_SNAPSHOT_VERSION_4	4	/* reserved */
#define	DI_SNAPSHOT_VERSION	DI_SNAPSHOT_VERSION_4	/* current version */
...
/*
 * These data structures defined below get copied out from a 64bit kernel to
 * 32/64 bit libdevinfo code.  Because of this, we define these structues
 * under pack(4) in order to avoid any alignment issues for uint64_t
 * structure members between kernel and userland code.
 */
...
struct di_all {
	...
	uint64_t	cna_dev;	/* CNA_DEV of device tree */
...
struct di_devnm {
	...
	di_off_t	vendor_global_prop;
	di_off_t	admin_global_prop;
...
struct di_link {
	...
	uint64_t	user_private_data; /* userland private data */
};
...
struct di_lnode {
	...
	/*
	 * di_link ptr to links coming into this node
...
struct di_node {	/* useful info to export for each tree node */
	...
	/*
	 * offset to minor nodes, property lists, etc.
	 */
	di_off_t	minor_data;
	di_off_t	drv_prop;
	di_off_t	sys_prop;
	...
	di_off_t	lnodes;		/* lnodes associated with this node */
	di_off_t	tgt_links;
	di_off_t	src_links;

	/*
	 * offsets to link vhci/phci nodes.
	 */
	di_off_t	next_vhci;
	di_off_t	top_phci;
	di_off_t	next_phci;
	...
	uint32_t	flags;		/* devi_flags field */
	di_off_t	hp_data;	/* offset to hotplug information.  */
	uint64_t	node_cna_dev;	/* CNA_DEV of device node */

	uint64_t	user_private_data; /* userland private data */

	di_off_t	vendor_prop;
	di_off_t	admin_prop;
	di_off_t	vendor_global_prop;
	di_off_t	admin_global_prop;
};
...
struct di_path {
	...
	uint64_t	path_cna_dev;	/* CNA_DEV of pathinfo node */
...
struct di_hp {
	...
	uint32_t	hp_state_priv_size;
	di_off_t	hp_state_priv;
...
struct di_priv_format {
	...
	int	bytes;			/* size in bytes of this struct */

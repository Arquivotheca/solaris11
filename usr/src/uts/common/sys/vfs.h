...
struct vfs_shadow;	/* from shadow.h */
...
/* vfs_op should not be used directly.  Accessor functions are provided */
	...
	/* Shadow data migration control */
	struct vfs_shadow *vfs_shadow;		/* in-core shadow data */
	krwlock_t	vfs_shadow_lock;	/* protects shadow data */
	kcondvar_t	vfs_shadow_cv;		/* change of pending state */
	kmutex_t	vfs_shadow_cv_lock;	/* for above cv */

	/* vfs_zone can be accessed without holding any locks */
	struct zone	*vfs_zone;		/* zone containing the mount */
...
#define	vfs_shrzone_lock	vfs_implp->vi_shrzone_lock
#define	vfs_shrzone_id		vfs_implp->vi_shrzone_id
#define	vfs_shrzone_refcnt	vfs_implp->vi_shrzone_refcnt
...
#define	VFS_MWAC	0x8000		/* file system enforced mwac */
#define	VFS_NOZONEMOD	0x10000		/* only modifiable by global zone */
...
#define	VFSFT_NORSTCHOWN	0x100000400	/* NO restricted chown */
/*
...
#define	VSW_MWAC	0x200	/* file system enforced mwac */
...
typedef struct vfs_impl {
	...
	kmutex_t	vi_shrzone_lock;	/* serialize vi_shrzone_* */
	zoneid_t	vi_shrzone_id;		/* zoneid of sharing zone */
	uint_t		vi_shrzone_refcnt;	/* shrzone ref count */
} vfs_impl_t;
...
void	vfs_zone_change(vfs_t *, struct zone *);

int	vfs_shrzid_ref(vfs_t *, zoneid_t);
int	vfs_shrzid_unref(vfs_t *, zoneid_t);
int	vfs_shrzid_set(vfs_t *, zoneid_t);

int	vfs_get_lofi(vfs_t *, vnode_t **);
boolean_t vfs_is_migrated(vfs_t *, cred_t *);

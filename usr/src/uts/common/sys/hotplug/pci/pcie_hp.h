 ...
 * (1 + 3 + 1).
 */
#define	PCIE_HP_DEV_FUNC_NUM_STRING_LEN 8
...
struct pcie_hp_slot {
	...
	ddi_hp_cn_state_code_t hs_state;	/* Slot state */
...
/*
 * arg for getting a port's info
 */
typedef struct pcie_hp_port_info {
	char			*cn_name;
	dev_info_t		*cn_child;
	ddi_hp_cn_type_t	cn_type;
	ddi_hp_cn_state_code_t	cn_state;
	int			rv;
} pcie_hp_port_info_t;
...
extern void pcie_hp_fini_state_priv(dev_info_t *dip);
extern int pcie_hp_register_port(dev_info_t *, dev_info_t *, char *);
extern int pcie_hp_unregister_port(dev_info_t *, dev_info_t *, char *);

 ...
 * FCOE frame version
 */
#define	FCOE_FCOE_FRM_VER	0
#define	FCOE_FIP_FRM_VER	1
...
#define	FCOE_MAX_FRAME_SIZE	2112

/*
 * FCOE offload
 */
#define	FCOE_TYPE_EOF_N	0x41
#define	FCOE_TYPE_EOF_T	0x42
#define	FCOE_TYPE_SOF_I	0x2E
#define	FCOE_TYPE_SOF_N	0x36

#define	FCOE_MAC_TXCRC_ENABLED(mac)	\
	(((mac)->fm_eport.offload_capab.capab_fcoe_flags & \
	MAC_FCOE_FLAG_TXCRC) ? \
	B_TRUE : B_FALSE)

#define	FCOE_MAC_RXCRC_ENABLED(mac)	\
	(((mac)->fm_eport.offload_capab.capab_fcoe_flags & \
	MAC_FCOE_FLAG_RXCRC) ? \
	B_TRUE : B_FALSE)

#define	FCOE_MAC_LSO_ENABLED(mac)	\
	(((mac)->fm_eport.offload_capab.capab_fcoe_flags & \
	MAC_FCOE_FLAG_LSO) ? \
	B_TRUE : B_FALSE)

#define	FCOE_MAC_LRO_ENABLED(mac)	\
	(((mac)->fm_eport.offload_capab.capab_fcoe_flags & \
	MAC_FCOE_FLAG_LRO) ? \
	B_TRUE : B_FALSE)

#define	SS_LSO_ENABLED(ss)	\
	(((ss)->ss_eport->offload_capab.capab_fcoe_flags & \
	MAC_FCOE_FLAG_LSO) ? \
	B_TRUE : B_FALSE)

#define	SS_LRO_ENABLED(ss)	\
	(((ss)->ss_eport->offload_capab.capab_fcoe_flags & \
	MAC_FCOE_FLAG_LRO) ? \
	B_TRUE : B_FALSE)

#define	IS_SCSI_READ(cdb)	\
	(((cdb == SCMD_READ) ||	\
	(cdb == SCMD_READ_G1) ||	\
	(cdb == SCMD_READ_G4) ||	\
	(cdb == SCMD_READ_G5)) ? 	\
...
typedef	struct	fcoe_offload_capab {
	uint16_t	capab_fcoe_flags;
	uint32_t	fcoe_max_lso_size;
	uint32_t	fcoe_max_lro_size;
	uint16_t	fcoe_min_lro_xchgid;
	uint16_t	fcoe_max_lro_xchgid;
} fcoe_offload_capab_t;
...
typedef struct fcoe_port {
	...
	int		 (*eport_fcoe_setup_lro)(struct fcoe_port *eport,
	    uint16_t flags, uint16_t xid, uint32_t buf_size);
	void		 (*eport_fcoe_cancel_lro)(struct fcoe_port *eport,
	    uint16_t flags, uint16_t xid);
	void		 (*eport_mac_fcoe_set)(fcoe_frame_t *frame,
	    uint16_t mss, uint16_t fc_headlen,
...
#define	EPORT_FLAG_FLOGI_DONE		0x10
...
#define	FCOE_VER_NOW FCOE_VER_2
...
extern void fcoe_trace(const char *ident, const char *fmt, ...);

...
/*
 * N2/NIU: Maximum descriptors if we need to call Hypervisor to set up
 * the logical pages and the driver must use contiguous memory.
 */
#if defined(sun4v)
...
typedef struct nxge_hw_pt_cfg {
	...
	int32_t		def_mac_rxdma_grpid; /* default RDC group ID	*/
...
typedef struct nxge_hw_list {
	 ...
	 *
	 * NXGE_MAX_GUEST_FUNCTIONS needs to the product of number
	 * of CPU nodes times 8 (for the number of virtualization regions
	 * per CPU node).
	 */
#define	NXGE_VRS_PER_CPU		8
#define	NXGE_MAX_GUEST_FUNCTIONS	(NXGE_VRS_PER_CPU * 8)

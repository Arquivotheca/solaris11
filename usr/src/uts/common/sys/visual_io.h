...
typedef union {
	...
	unsigned char	sixteen[2];	/* sixteen bit */
	unsigned char	twentyfour[3];	/* twentyfour bit */
} color_t;
...
/*
 * VIS_CONSCLEAR:
 * Clear the entire screen.  Used only in VIS_PIXEL mode, allows the driver to
 * optimize the operation of clearing the screen.
 *
...
/* XXX - The color type really should be text_color_t */
...
struct vis_devinit {
	...
	color_map_fn_t	color_map;	/* Color map function (tem -> fb) */

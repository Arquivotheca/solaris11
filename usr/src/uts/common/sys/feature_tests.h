...
/*
 * linting works incorrectly when __PRAGMA_REDEFINE_EXTNAME is defined.
 * Also, when linting we want to see everything, hence __EXTENSIONS__
 */
#if defined(__lint)
...
#if defined(__XOPEN_OR_POSIX) && defined(__USE_LEGACY_PROTOTYPES__)
#error "Cannot combine X/Open or POSIX feature test macro \
	with __USE_LEGACY_PROTOTYPES__"
#endif

...
typedef struct task_cpu_time_ctl {
	struct task_cpu_time	*tcp;		/* per-set cpu time array */
	size_t			tc_offset;	/* array alignment offset */
} task_cpu_time_ctl_t;
...
extern void task_cpu_time_check(task_t *, struct proc *);
extern int task_cpu_time_add(task_t *, int, uint64_t);

...
/*
 * cmlb_ioctl_secpolicy:
 * Apply policy for disk ioctl()s.
 *
 *   Arguments:
 *	devt		dev_t for device
...
int cmlb_rwcmd_copyin(struct dadkio_rwcmd *rwcmdp, caddr_t inaddr, int flag);
int cmlb_rwcmd_copyout(struct dadkio_rwcmd *rwcmdp, caddr_t outaddr, int flag);

 ...
 * /dev mount options
 */
#define	MNTOPT_SDEV_ATTRDIR	"attrdir"
...
#define	SDEV_NVNAME_ANNOTATE	"prof_annotate"
...
struct sdev_dprof {
	...
	nvlist_t *dev_name_ann;
	nvlist_t *dev_glob_ann;
};
...
#define	SDEV_ANNOTATED		0x0400	/* device annotation should exist */
#define	SDEV_NAME_REMAPPED	0x0800	/* profile node name is remapped */
...
extern int prof_lookup(vnode_t *, char *, vnode_t **, struct cred *, int);
...
extern char *devzvol_make_dsname(const char *path, const char *name,
    struct zone *zone);
extern void *sdev_get_vtor(struct sdev_node *dv);
extern void sdev_apply_devann(struct sdev_node *dv);
...
extern const fs_operation_def_t	sdev_vnodefem_tbl[];

...
/*
 * The following macros are used to determine whether the version
 * number for the passed in superblock are valid for UFS
 * and MTB UFS.
 */

#define	UFS_VALID_VERSION(sblock) \
	((sblock)->fs_version == UFS_EFISTYLE4NONEFI_VERSION_2 || \
	(sblock)->fs_version == UFS_VERSION_MIN)

#define	MTB_UFS_VALID_VERSION(sblock) \

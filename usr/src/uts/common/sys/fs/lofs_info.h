...
struct loinfo {
	...
	krwlock_t	li_unmount_lock; /* lock for force unmount */
	boolean_t	li_unmounted;	/* forcefully unmounted */
...
#define	LOFS_ENTER(li)						\
	rw_enter(&(li)->li_unmount_lock, RW_READER);		\
	if ((li)->li_unmounted) {				\
		LOFS_EXIT(li);					\
		return (EIO);					\
	}

#define	LOFS_ENTER_VOID(li)					\
	rw_enter(&(li)->li_unmount_lock, RW_READER);		\
	if ((li)->li_unmounted) {				\
		LOFS_EXIT(li);					\
		return;						\

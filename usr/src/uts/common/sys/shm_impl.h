...
typedef struct kshmid {
	...
	ulong_t		shm_attach;	/* number of attaches */
	...
	uint_t		shm_advice;	/* advice for shared memory segment */
	size_t		shm_granule_sz;	/* OSM granule size */
	size_t		shm_allocated;	/* mem allocated for OSM */
} kshmid_t;
...
struct shmid_ds32 {
	...
	uint32_t	shm_flags;	/* see SHMDS_* flags */

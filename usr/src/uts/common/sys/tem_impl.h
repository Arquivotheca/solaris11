...
/*
 * For 16-bit we specify upper and lower 8 bits. This does not represent
 * how colors are divided (565 RGB, f.e.), but since we're hardcoding the
 * 0xffff and 0x0000 color_t values (see sys/visual_io.h), this is better
 * than having to or and shift RGB values later.
 */
#define	TEM_TEXT_BLACK16_UPPER	0x00
#define	TEM_TEXT_BLACK16_LOWER	0x00
#define	TEM_TEXT_WHITE16_UPPER	0xff
#define	TEM_TEXT_WHITE16_LOWER	0xff
...
struct tem_vt_state {
	...
	boolean_t	tvs_linewrap;	/* does next char go on a new line */
...
typedef struct tem_state {
	...
	color_map_fn_t	ts_color_map;	/* color map func (tem -> fb) */
...
int	tems_clearscr_layered(struct vis_consclear *, cred_t *);

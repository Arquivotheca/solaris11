...
/*
 * sizeof dblk_t must be a multiple of 8 bytes on all platforms.
 * Padding may be needed, if structure members are added or removed.
 */
typedef struct datab {
	...
	void		*db_cache_data;	/* kmem cache descriptor */
	struct msgb	*db_mblk;
	...
	intptr_t	db_packet;	/* Buffer Management */
} dblk_t;
...
#define	DBLK_DMA_PREMAP		0x08

#define	DBLK_IS_PREMAPPED(db)	((db)->db_flags & DBLK_DMA_PREMAP)
...
#define	MSGCRED		0x08	/* Private: consumer must verify credentials */
...
typedef struct __pm_handle *pm_handle_t;

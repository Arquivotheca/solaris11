...
typedef struct usb_event {
	...
	/* notification that system is about to do LSR */
	void	(*lsr_event_handler)(dev_info_t *dip,
...
/*
 * Post an ereport for a client driver
 * Arguments:
 *	dip		- pointer to client driver's devinfo node
 *	err		- USB ereport subclass
 */
void usb_ereport_post(dev_info_t *dip, char *err);

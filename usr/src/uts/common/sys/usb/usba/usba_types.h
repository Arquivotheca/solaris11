...
typedef struct usba_evdata {
	...
	ddi_callback_id_t	ev_lsr_cb_id;
} usba_evdata_t;
...
typedef struct usba_device {
	...
	int			usb_fm_capable;
	/* fm error counter alloc'ed for normal device */
	uint_t			*usb_fm_err_count;
	/* fm error counter for usb_mid and usb_ia */
	uint_t			usb_fm_err_ext_count;
...
#define	USBA_CLIENT_FLAG_FM_INIT	0x08
...
typedef struct usba_cb_lsr {
	ddi_cb_action_t	usba_lsr_cbaction;
	ddi_cb_lsr_t	usba_lsr_cbarg;
	int		usba_lsr_cbret;
} usba_cb_lsr_t;

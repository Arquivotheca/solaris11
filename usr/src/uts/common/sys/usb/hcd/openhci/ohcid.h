...
typedef struct ohci_state {
	...
	/* Openhci Host Controller LSR State */
	uint_t			ohci_hc_lsr_state;
	uint_t			ohci_hc_lsr_activities;
	uint_t			ohci_hc_lsr_impacts;
...
/*
 * Define OHCI LSR state
 */
#define	OHCI_LSR_SUSPENDED		1

/*
 * Define OHCI LSR capability
 */
#define	OHCI_CB_LSR_ACTIVITIES		(DDI_CB_LSR_ACT_DMA | \
					    DDI_CB_LSR_ACT_PIO | \
					    DDI_CB_LSR_ACT_INTR)
#define	OHCI_CB_LSR_IMPACTS		(DDI_CB_LSR_IMP_DMA_ADDR_CHANGE | \
...
#define	PRINT_MASK_LSR		0x00000200	/* Live Suspend & Resume */
...
/* ohci FMA functions */
void	 ohci_fm_ereport(
			ohci_state_t *ohcip,
			char *detail);
int 	ohci_check_acc_handle(
			ohci_state_t *ohcip,
			ddi_acc_handle_t handle);
int 	ohci_check_dma_handle(
			ohci_state_t *ohcip,

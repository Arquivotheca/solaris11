...
typedef struct uhci_state {
	...
	/* Chip identification */
	uint16_t		uhci_vendor_id;		/* chip vendor */
	uint16_t		uhci_device_id;		/* chip device */
	uint16_t		uhci_subvendor_id;	/* sub vendor id */
 ...
 *
 * UHCI_CTRL_STOPPED_STATE:
 *      The host controller soft state will be set to this after host
 *      controller is stopped successfully when no devices are connected
 *      to the root hub ports.
 *
...
#define	UHCI_CTLR_STOPPED_STATE		4	/* Host controller stopped */

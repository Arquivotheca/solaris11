...
typedef struct vnode {
	...
	struct fw	*v_fw;		/* reserved for VM2 */
} vnode_t;
...
#define	VN_ALIGN	64
/*
 * Define VN_SHIFT so low order vnode address bits with less variation are
 * dropped for hash functions.  The shift should preserve the high bit of the
 * vnode size rounded to the next power of 2.
 */
#define	VN_SHIFT	LOG2UP_CONST(P2ROUNDUP(sizeof (vnode_t), VN_ALIGN))
...
typedef struct xoptattr {
	...
	uint8_t		xoa_offline;
	uint8_t		xoa_sparse;
	char		xoa_mlslabel[MAXLABELSTR];	/* filesystem slabel */
} xoptattr_t;
...
#define	XAT0_MLSLABEL	0x00020000	/* filesystem slabel */
...
#define	XAT_MLSLABEL		((XAT0_INDEX << XVA_SHFT) | XAT0_MLSLABEL)
 ...
 */
enum vn_rm	{ RMFILE, RMDIRECTORY };	/* rm or rmdir (remove) */
...
int	vn_renameat(vnode_t *fdvp, char *fname, vnode_t *tdvp, char *tname,
		enum uio_seg seg);
int	vn_remove(char *fnamep, enum uio_seg seg, enum vn_rm dirflag);
...
#define	VFTOZONE(vfsp)	((vfsp)->vfs_zone)
#define	VTOZONE(vp)	(VFTOZONE((vp)->v_vfsp))

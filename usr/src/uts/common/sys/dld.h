 ...
 */
/* use DLDIOC(0x12) for CREATEKSTAT */
#define	DLDIOC_CREATEKSTAT	DLDIOC(0x12)
typedef struct dld_ioc_createkstat {
	char			ck_flow[MAXFLOWNAMELEN];
	zoneid_t		ck_zoneid;
} dld_ioc_createkstat_t;
...
typedef struct dld_mac_addr {
	uchar_t	dm_addr[MAXMACADDRLEN];
	uint_t	dm_len;
} dld_mac_addr_t;
...
typedef struct dld_ioc_hwgrpget {
	...
	uint_t		dih_ngroups;	/* number of groups included in ioc */
	uint_t		dih_nrxrings;	/* total number of rx rings */
	uint_t		dih_ntxrings;	/* total number of tx rings */
	uint_t		dih_size;
} dld_ioc_hwgrpget_t;
...
#define	DLDIOC_GETRINGIDS	DLDIOC(0x1e)

typedef struct dld_ioc_ringidget {
	datalink_id_t	dir_linkid;
	uint_t		dir_nrxrings;	/* number of rx rings */
	uint_t		dir_rxringids[MAX_RINGS_PER_GROUP];
	boolean_t	dir_rxdedicated;
	uint_t		dir_ntxrings;	/* number of tx rings */
	uint_t		dir_txringids[MAX_RINGS_PER_GROUP];
	boolean_t	dir_txdedicated;
} dld_ioc_ringidget_t;
...
typedef struct dld_capab_direct_s {
	...
	uintptr_t	di_tx_pmh;	/* pm_handle_t */
} dld_capab_direct_t;
...
typedef struct dld_capab_poll_s {
	uintptr_t	poll_ring_add_cf;	/* ip_gld_squeue_add */
	uintptr_t	poll_ring_remove_cf;	/* ip_gld_squeue_remove */
	void		*poll_ring_ch;
} dld_capab_poll_t;
...
int	dld_remove_flow(char *, zoneid_t);
int	dld_modify_flow(char *, mac_resource_props_t *, zoneid_t);
int	dld_walk_flow(dld_ioc_walkflow_t *, intptr_t, cred_t *);
int	dld_create_kstat(char *, zoneid_t);

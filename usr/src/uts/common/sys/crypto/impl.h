...
/*
 * A mechanism entry in one of the kcf_*_mech_tab[] tables. KCF_CPU_PAD needs
...
#define	KCF_PROV_COPYIN_DATA(pd) ( \
	((pd)->pd_ops_vector->co_uio_userspace_ok == B_TRUE) ? \
	CRYPTO_SUCCESS : CRYPTO_NOT_SUPPORTED)

#define	KCF_CTX(ctx) \
	(kcf_context_t *)((ctx)->cc_framework_private)

#define	KCF_CTX_SW_PROV(ctx) \
	(((kcf_context_t *)KCF_CTX(ctx))->kc_sw_prov_desc)

#define	KCF_PROV_PREFER_SW(ctx, datalen, cr) \
	(((ctx)->cc_flags & CRYPTO_USE_OPSTATE) && \
	((cr) == NULL) && \
	((datalen) < \
	    ((kcf_context_t *)KCF_CTX(ctx))->kc_mech->me_threshold) && \
	(KCF_CTX_SW_PROV(ctx) != NULL) && \
	KCF_IS_PROV_USABLE(KCF_CTX_SW_PROV(ctx)))

#define	KCF_DATA_NEED_COPYIN(swpd, data) \
	((KCF_PROV_COPYIN_DATA(swpd) == CRYPTO_NOT_SUPPORTED) && \
	((data)->cd_format == CRYPTO_DATA_UIO) && \
	((data)->cd_uio->uio_segflg == UIO_USERSPACE))
...
/* FIPS 140 functions */
extern void kcf_init();
extern void kcf_activate();
extern int kcf_fips140_integrity_check();
extern int kcf_get_fips140_state(void);

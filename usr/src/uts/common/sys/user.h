...
typedef struct uf_entry {
	...
	int		uf_afd_busy;	/* some thread is in getf_poll [grow] */
	kcondvar_t	uf_afd_cv;	/* uf_afd_busy dance [never copied] */
	short		uf_pad16;
	...
	char		uf_pad[64 - sizeof (kmutex_t) - 2 * sizeof (void*) -
		...
		sizeof (int) - sizeof (kcondvar_t) - sizeof (short)];
} uf_entry_t;
...
/*
 * kernel pages shared with process.
 */
...
typedef	struct	user {
	...
	short	u_exflags;		/* flags used in exec */
	...
	uint64_t u_mem;			/* accumulated memory usage (pages) */
	size_t	u_mem_max;		/* peak RSS (pages) */
	...
	shared_t	u_shared;	/* kernel info shared with process */
} user_t;

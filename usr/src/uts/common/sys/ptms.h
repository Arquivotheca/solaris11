...
extern int ptms_minor_valid(zoneid_t zoneid, minor_t ptmin,
    uid_t *uid, gid_t *gid);
extern int ptms_minor_exists(minor_t ptmin);
extern void ptms_set_owner(zoneid_t zoneid, minor_t ptmin,
    uid_t uid, gid_t gid);
...
#define	_PTSSTTY	(('P'<<8)|6)	/* set tty flag as passed by ldterm */

...
#define	so_rcv_timer_interval so_proto_props.sopp_rcvtimer
	...
	list_node_t		so_filter_drop;		/* drop list */
	pm_handle_t		so_pmh;
	kmutex_t		so_pmh_lock;
};
...
extern int 	sockparams_add_mapping(int, int, int, const char *);
...
extern int 	sockparams_get_num_of_entries();
extern int	sockparams_copyout_entries(struct uio *, int *);
...
extern void smod_set_func(so_create_func_t, so_destroy_func_t);
...
#define	SOCKCONFIG_GET_NUM_ENTRIES	4
#define	SOCKCONFIG_GET_ENTRIES		5
...
typedef struct sock_config {
	...
	int	sc_type;
	int	sc_protocol;
	char 	sc_modname[MAXPATHLEN];
	boolean_t	sc_loaded;  /* True if the module is loaded */
	boolean_t	sc_auto_filters; /* True if filters are present */
	boolean_t	sc_prog_filters; /* True if filters are present */
} sock_config_t;

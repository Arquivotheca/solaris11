 ...
 *
 *    Note: the Little Endian struct/bitfields have been changed to match
 *    the fact that they are being delivered to the device via ddi_put32().
 *    This was needed because of the malaligned Address Path subfields.
 *    Most, if not all, of the other Little Endian struct/bitfields are
 *    defined so that ddi_put64() will work.
 */
...
#define	HERMON_WQE_BUILD_INLINE(ds, sz)				\
	*(uint32_t *)(ds) = htonl(HERMON_WQE_SGL_INLINE_MASK | (sz))

#define	HERMON_WQE_BUILD_INLINE_ICRC(ds, sz, icrc)			\
{									\
	uint32_t		*tmp;					\
									\
	tmp = (uint32_t *)(ds);						\
...
#define	HERMON_WQE_BUILD_MLX_LRH(lrh, qp, udav, drslid, pktlen)		\
{									\
	uint32_t		*tmp;					\
	uint32_t		lrh_tmp;				\
									\

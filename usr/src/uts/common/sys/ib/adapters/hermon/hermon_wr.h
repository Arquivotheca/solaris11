...
#define	HERMON_SPECIAL_QP_DRMAD_GET_METHOD(method, offset, va, len)	\
	if (((method) == NULL) && ((offset) + (len) > 3)) {		\
	    (method) = &((uint8_t *)(uintptr_t)(va))[3 - (offset)];	\
	}
...
#define	HERMON_SPECIAL_QP_DRMAD_GET_DRSLID(DRSLid, offset, va, len)	\
	if (((DRSLid) == NULL) &&					\
	    ((offset) + (len) > 32)) {					\
	    (DRSLid) = &((uint8_t *)(uintptr_t)(va))[32 - (offset)];	\
	}
#define	HERMON_SPECIAL_QP_DRMAD_GET_DRDLID(DRDLid, offset, va, len)	\
	if (((DRDLid) == NULL) &&					\
	    ((offset) + (len) > 34)) {					\
	    (DRDLid) = &((uint8_t *)(uintptr_t)(va))[34 - (offset)];	\
	}

...
typedef enum {
	...
	HERMON_IOCTL_SET_NODEDESC	= HERMON_IOCTL | 0x30,
	HERMON_IOCTL_GET_NODEDESC	= HERMON_IOCTL | 0x31
} hermon_ioctl_enum_t;
#else
typedef enum {
	HERMON_IOCTL_FLASH_READ		= HERMON_IOCTL | 0x00,
...
/*
 * Flags for updating the Node descriptor persistent store.
 */
#define	HERMON_NODEDESC_UPDATE_STRING		0x00000001
#define	HERMON_NODEDESC_UPDATE_HCA_STRING	0x00000002

/*
 * This structure is used to pass the common portion of node
 * descriptor string, to be used for overiding the response of
 * Node Descriptor MAD and a flag which indicates what portion
 * of Node descriptor persistent store should be updated.
 */
typedef struct hermon_nodedesc_ioctl_s {
	char			node_desc_str[64];
	uint32_t		node_desc_update_flag;
} hermon_nodedesc_ioctl_t;

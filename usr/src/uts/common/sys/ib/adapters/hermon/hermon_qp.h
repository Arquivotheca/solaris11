...
typedef struct hermon_qpn_entry_s {
	...
	timeout_id_t		qpn_timeout_id;
	hermon_state_t		*qpn_statep;
} hermon_qpn_entry_t;

#define	HERMON_QPN_RELEASE		0x1
#define	HERMON_QPC_FREE			0x2

#define	HERMON_QPN_UPPER_BITS(state, entry) \
	(((entry)->qpn_counter << (state)->hs_cfg_profile->cp_log_num_qp))

...
/* Node Descriptor update flags */
#define	IBC_NODEDESC_UPDATE_STRING	0x00000001
#define	IBC_NODEDESC_UPDATE_HCA_STRING	0x00000002
...
/*
 * ibc_get_nodedesc(), ibc_write_nodedesc()
 *
 *	Get and put Node descriptor in persistent store. If the persistent store
 *	is not present or empty or corrupt, node descriptor string from the HCA
 *	firmware will be used.
 */
char *ibc_read_nodedesc(dev_info_t *, size_t *);
ibt_status_t ibc_write_nodedesc(dev_info_t *, char *, uint32_t);

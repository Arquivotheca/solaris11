...
#define	ISER_MR_MASK		(ISER_MR_QUANTSIZE - 1)
...
typedef struct iser_wr_s {
	...
	uint32_t		iw_nwrs;	/* num WRs in this xfer */
...
/*
 * iSER data buffer structure for scatter-gather (zero copy) I/O ops
 * In general, we expect the product of PAGESIZE and hca_max_sgl_size
 * and ISER_IB_MAX_WRS to be greater than 1MB, but any number will do.
 */
#define	ISER_IB_MAX_WRS	10

typedef struct iser_sgl_s {
	uint32_t	buf_numpages;
	ibt_mi_hdl_t	buf_mi_hdl;
	ibt_send_wr_t	buf_wr;		/* Output of ibt_map_mem_iov */
	ibt_send_wr_t	buf_xmit_wrs[ISER_IB_MAX_WRS];
} iser_sgl_t;

...
typedef struct sol_ofs_client_s {
	...
	krwlock_t		lock;
	/*
	 * Clients which do not use the Kverbs interfaces
	 * but use the IBTF interfaces. sol_uverbs and
	 * sol_umad are the clients which fit in this
	 * category.
	 */
	boolean_t		client_not_kverbs;

	/*
	 * Async handler registered by the client.
	 */
	struct ib_event_handler	client_ib_event_hdlr;
	void			(*clnt_async_hdlr)(
				    struct ib_event_handler *,
				    struct ib_event *);
	uint16_t		clnt_async_hdlr_cnt;

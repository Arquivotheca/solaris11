...
/*
 * The HCA info structure's state field defines. These are not
 * consecutive numbers but each bit defines a state
 */

typedef enum {
	SOL_UVERBS_HCA_UNINITIALIZED	= 0,
	SOL_UVERBS_HCA_ATTACHED		= 1,
	SOL_UVERBS_HCA_DETACHED		= 2,
} sol_uverbs_hca_state_t;
...
#define	drv_hca_hdl		hca_ib_devp->hca_hdl
...
typedef struct {
	...
	/*
	 * Register with sol_ofs HCA related info
	 */
	struct ib_client	client_info;
	uint32_t		hca_count;
...
extern uverbs_module_context_t *sol_uverbs_mod_ctxt;
...
/*
 * The following functions are used in attach/detach entry points and HCA DR
 * operations.
 */
int sol_uverbs_hca_open(uverbs_module_context_t *, ib_guid_t);
int sol_uverbs_hca_close(sol_uverbs_hca_t *);
void sol_uverbs_uctxt_uobj_free(sol_ofs_uobj_t *);

...
static inline void ib_addr_set_sgid(struct rdma_dev_addr *dev_addr,
    union ib_gid *gid)
{
	union ib_gid	horder_gid;

	horder_gid.global.subnet_prefix = htonll(
	    gid->global.subnet_prefix);
	horder_gid.global.interface_id = htonll(
	    gid->global.interface_id);
	(void) memcpy(dev_addr->src_dev_addr + 4, &horder_gid, sizeof (*gid));
}
    ...
    union ib_gid *gid)
{
	...
	horder_gid.global.interface_id = htonll(
	    gid->global.interface_id);
	(void) memcpy(dev_addr->dst_dev_addr + 4, &horder_gid, sizeof (*gid));
}

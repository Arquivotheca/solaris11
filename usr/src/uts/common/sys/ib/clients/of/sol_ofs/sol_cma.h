...
typedef struct {
	...
	/*
	 * Per-Chan mutex and CV for syncing rdma_destroy and other APIs.
	 */
	kmutex_t		chan_mutex;
	...
	/*
	 * Per Chan states :
	 *	chan_state : Tracks last API called
	 *
	 *	destroy_state : rdma_destroy(), other API &
	 *	    Event notification sync.
	 *	req_state : For CMIDs created on CONNECT_REQ
	 *	    Passive / Server side only.
	 *	connect_flag : Active Side CMIDs only.
	 */
	...
	/*
	 * Active Side only : chan_param
	 *	This is stored on REP and passed to consumer
	 *	on EST.
	 */
	struct rdma_conn_param	chan_param;

	/*
	 * Following Transport independent fields are for
	 * Server / Passive side only
	 */
	 ...
	 * chan_total_req_cnt -
	 *	REQ CMIDs created not destroy_id() not called.
	 */
	...
	/*
	 * chan_listenp - Data for root / endpoint CM ID.
	 * listen_root - root CMID for Endpoint | Req CMID
	 */
...
#define	CMA_PORT_MASK_INITED	0x80000000
...
typedef struct cma_device {
	...
	/*
	 * For special listen : 31 bits for the bit mask
	 * for HCA ports on this HCA and HCA ports that
	 * listen has been initiated for the special
	 * listen. The bit 31 is used indicate if the mask
	 * has been inited or not.
	 */
	uint32_t		cma_port_mask;
	uint32_t		cma_port_listen_mask;
} cma_device_t;
...
static inline cma_device_t *
cma_get_hca_device(uint64_t hca_guid)
{
	llist_head_t	*entry;
	cma_device_t	*cma_devp;

	ASSERT(MUTEX_HELD(&sol_cma_dev_mutex));
	list_for_each(entry, &sol_cma_dev_list) {
		cma_devp = (cma_device_t *)entry->ptr;
		mutex_enter(&cma_devp->cma_mutex);
		if (cma_devp->cma_dev_state !=
		    SOL_CMA_DEV_REM_IN_PROGRESS &&

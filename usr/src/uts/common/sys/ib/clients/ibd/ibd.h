...
#define	IBD_DRV_ASYNC_EVENT_TASKQ_CREATED	0x40000000
...
#define	IBD_SEND			0
#define	IBD_RECV			1
#define	IBD_PATH_RNR_RETRY_COUNT	1
#define	IBD_PATH_RETRY_COUNT		1
#define	IBD_CM_RETRY_COUNT		2
#define	IBD_NUM_ACACHE_TASKQ_THREADS	4
...
#define	IBD_MIN_RC_RX_COPY_THRESH	64
#define	IBD_MAX_RC_RX_COPY_THRESH	65520
#define	IBD_MIN_RC_TX_COPY_THRESH	64
...
#define	IBD_RC_RWQE_MEM_SHORT		64
#define	IBD_RC_RWQE_MEM_VERY_SHORT	16
#define	IBD_RC_RWQE_MEM_EX_SHORT	2
 ...
 * handler not to acquire the id_ac_rwlock to process every completion,
 ...
 *
 * The AH free list insertion/deletion is protected with the id_ac_rwlock,
...
#define	IBD_ACACHE_INSERT_FREE(state, ce) {			\
	ce->ac_state = IBD_AC_LKUP_INFREELIST;			\
...
#define	IBD_ACACHE_PULLOUT_ACTIVE(state, ce) {			\
	...
	(void) mod_hash_remove(state->id_ah_active_hash,	\
	    (mod_hash_key_t)&ce->ac_mac, (mod_hash_val_t)&_ce_);\
}
#define	IBD_ACACHE_GET_ACTIVE(state) \
	list_get_head(&state->id_ah_active)

#define	IBD_ACACHE_INSERT_INPROG(state, ce) {			\
	int _ret_;						\
	_ret_ = mod_hash_insert(state->id_ah_inprog_hash,	\
	    (mod_hash_key_t)&ce->ac_mac, (mod_hash_val_t)ce);	\
	ASSERT(_ret_ == 0);					\
	state->id_ah_num_inprog++;				\
}
#define	IBD_ACACHE_PULLOUT_INPROG(state, ce) {			\
	ibd_ace_t *_ce_;					\
	(void) mod_hash_remove(state->id_ah_inprog_hash,	\
	    (mod_hash_key_t)&ce->ac_mac, (mod_hash_val_t)&_ce_);\
	state->id_ah_num_inprog--;				\
}
...
typedef enum {
	IBD_AC_LKUP_INFREELIST	= 0x0,
	IBD_AC_LKUP_SUCCESS	= 0x1,
	IBD_AC_LKUP_INPROG	= 0x2,
	IBD_AC_LKUP_FAILED	= 0x4
} ibd_ace_state_t;
...
typedef struct ibd_acache_s {
	...
	kmutex_t		ac_mutex;
	ibd_ace_type_t		ac_type;
	ibd_ace_state_t		ac_state;
	mblk_t			*ac_mblk_head;
	mblk_t			*ac_mblk_tail;
	uint32_t		ac_mblk_cnt;
...
typedef struct ibd_state_s {
	...
	ddi_taskq_t		*id_ac_taskq;

	krwlock_t		id_ac_rwlock;
	...
	uint32_t		id_ah_num_inprog;
	ibd_ace_t		*id_ac_list;
	mod_hash_t		*id_ah_active_hash;
	mod_hash_t		*id_ah_inprog_hash;
	...
	/* id_rc_rx_rwqe_thresh_orig: workaround for a memory exhaustion bug */
	uint_t id_rc_rx_rwqe_thresh_orig;
	uint_t id_rc_num_swqe;
	uint_t id_rc_num_rwqe;
	uint_t id_rc_num_srq;
	kmutex_t id_worker_flush_lock;
	kcondvar_t id_worker_flush_cv;
	uint_t id_worker_flushed;

	/*
	 * id_mt_lock, id_mt_state are used to ensure consistency when
	 * ibdpd_async_handler() is invoked around the same time as processing
	 * ibd_port_attach() and ibd_port_unattach().
	 */
	kmutex_t	id_mt_lock;
	uint_t		id_mt_state;

	/* workaround for a memory exhaustion bug */
	uint64_t	rc_mem_short;		/* short memory */
	uint64_t	rc_mem_very_short;	/* very short memory */
	uint64_t	rc_mem_extr_short;	/* extremely short memory */
	uint64_t	rc_rwqe_thresh_recovered;

	ddi_taskq_t	*id_async_event_taskq;

	/*
	 * Link name is cached here for using in some debug messages.
	 * If the link name changes, this field may not always get updated.
	 */
	char		id_link_name[MAXLINKNAMELEN];
	int		id_link_state_change_in_progress;
	int64_t		id_attach_time;
} ibd_state_t;

#define	IBD_MT_PORT_UNATTACH_IN_PROGRESS	1
...
ibd_ace_t *ibd_acache_find(mod_hash_t *, ipoib_mac_t *, boolean_t, int);

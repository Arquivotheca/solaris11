...
#define	RDSV3_FMR_POOL_SIZE		(64 * 1024)
...
#define	RDSV3_IB_DEFAULT_RETRY_COUNT	1
...
struct rdsv3_page_frag {
	...
	struct rdsv3_ib_device	*f_ibdev;
};
...
struct rdsv3_ib_device {
	...
	struct kmem_cache	*ib_send_slab;
	unsigned long		ib_max_recv_alloc;
	atomic_t		ib_recv_alloc;
...
extern void rdsv3_ib_init_delayed_msg(rdsv3_delayed_work_t *dwp, void *dev);
extern void rdsv3_ib_queue_delayed_msg(rdsv3_delayed_work_t *dwp, uint_t delay);
extern void rdsv3_ib_cancel_delayed_msg(rdsv3_delayed_work_t *dwp);
extern void rdsv3_ib_drain_wkrlist(void *data);
void rdsv3_ib_create_wkr_pool(struct rdsv3_ib_device *);
void rdsv3_ib_destroy_wkr_pool(struct rdsv3_ib_device *);
...
uint32_t rdsv3_ib_ring_alloc(struct rdsv3_ib_work_ring *ring, uint32_t val,
    uint32_t *pos);
uint32_t rdsv3_ib_send_ring_alloc(struct rdsv3_ib_work_ring *ring,
    uint32_t val, uint32_t *pos);
void rdsv3_ib_ring_free(struct rdsv3_ib_work_ring *ring, uint32_t val);
void rdsv3_ib_send_ring_free(struct rdsv3_ib_work_ring *ring, uint32_t val);
...
/* ib_send.c */
void rdsv3_ib_message_purge(struct rdsv3_message *rm);
int rdsv3_ib_message_copy_from_user(void *devp, struct rdsv3_message *rm,
...
struct rdsv3_cong_map *rdsv3_ib_alloc_cong_map(struct rdsv3_ip_bucket *bucketp);
void rdsv3_ib_free_cong_map(struct rdsv3_ip_bucket *bucketp);

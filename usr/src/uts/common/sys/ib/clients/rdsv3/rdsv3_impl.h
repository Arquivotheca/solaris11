...
#define	RDSV3_QUEUE_DELAYED_MSG(conn, dwp, delay)		\
	(conn)->c_trans->queue_delayed_msg((dwp), (delay))
#define	RDSV3_CANCEL_DELAYED_MSG(conn, dwp)			\
	(conn)->c_trans->cancel_delayed_msg((dwp))
#define	RDSV3_INIT_DELAYED_WORK(conn, dwp, dev, f)		\
	(dwp)->work.func = f;					\
	(dwp)->timeid = 0;					\
	(conn)->c_trans->init_delayed_msg((dwp), (dev));	\

...
#define	RDSV3_MAX_BUF		1024 * 1024 * 1024
#define	RDSV3_RECV_HIWATER	4194304
#define	RDSV3_XMIT_HIWATER	262144

extern uint_t rdsv3_max_buf;
extern uint_t rdsv3_xmit_hiwat;
extern uint_t rdsv3_recv_hiwat;

#define	RDSV3_CONG_MAP_BYTES	(65536 / 8)
#define	RDSV3_CONG_MAP_LONGS	(RDSV3_CONG_MAP_BYTES / sizeof (unsigned long))
#define	RDSV3_CONG_MAP_PAGES	(RDSV3_CONG_MAP_BYTES / RDSV3_FRAG_SIZE)
#define	RDSV3_CONG_MAP_PAGE_BITS	(RDSV3_FRAG_SIZE * 8)

struct rdsv3_cong_map {
	uint32_be_t		m_addr;
	struct rdsv3_message	*m_rm;
...
struct rdsv3_ip_bucket {
	...
	struct rdsv3_cong_map	*lcong;
	krwlock_t		rwlock;
	struct rdsv3_transport	*trans;
	void			*devp;	/* null for 127.0.0.1 */
	uint_t			nconn;
	avl_tree_t		conn_tree;
	krwlock_t		conn_lock;
...
struct rdsv3_connection {
	...
	struct rdsv3_delayed_work_s	c_down_w;
	...
	rdsv3_af_procspec_t	c_procspec;
};
...
/*
 * inline data immediately follows the rdsv3_header.
 * rdsv3_max_inline can be set to values smaller than
 * RDSV3_MAX_INLINE in /etc/system for experiments.
 *
 * Here are the choices that utilize the full wqe size:
 *
 *	56	 56+48==104	128-byte wqe
 *	176	176+48==224	256-byte wqe
 *	416	416+48==464	512-byte wqe
 *	880	880+48==928	1024-byte wqe (last 16 bytes not used)
 */
#define	RDSV3_MAX_INLINE	416
extern uint_t rdsv3_max_inline;
...
struct rdsv3_message {
	...
	struct rdsv3_transport	*m_trans;
	...
	struct rdsv3_connection *m_conn;
	struct rdsv3_rdma_op	*m_rdma_op;
	rds_rdma_cookie_t	m_rdma_cookie;
	struct rdsv3_mr		*m_rdma_mr;
	unsigned int		m_nents;
	unsigned int		m_count;
	struct rdsv3_header	m_hdr;
	uint8_t			m_inline[RDSV3_MAX_INLINE];
...
struct rdsv3_transport {
	...
	int (*laddr_check)(uint32_be_t addr, void **dev);
	int (*copy_from_user)(void *devp, struct rdsv3_message *rm,
	    uio_t *uiop, size_t payload_len);
	void (*message_purge)(struct rdsv3_message *rm);
	...
	struct rdsv3_cong_map *(*alloc_cong_map)
	...
	void (*free_cong_map)(struct rdsv3_ip_bucket *bucketp);
	...
	void (*flush_mrs)(void);
	void (*init_delayed_msg)(rdsv3_delayed_work_t *dwp, void *devp);
	void (*queue_delayed_msg)(rdsv3_delayed_work_t *dwp, uint_t delay);
	void (*cancel_delayed_msg)(rdsv3_delayed_work_t *dwp);
};
...
struct rdsv3_sock {
	...
	struct rdsv3_ip_bucket	*rs_bucketp;
...
struct rdsv3_connection *rdsv3_conn_create(struct rdsv3_ip_bucket *bucketp,
    uint32_be_t faddr, int gfp);
struct rdsv3_connection *rdsv3_conn_create_outgoing(
    struct rdsv3_ip_bucket *bucketp, uint32_be_t faddr, int gfp);
...
void rdsv3_hdr_info_copy(struct rdsv3_header *hdr,
    struct rdsv3_info_iterator *iter,
...
struct rdsv3_cong_map *rdsv3_alloc_cong_map(struct rdsv3_ip_bucket *bucketp);
void rdsv3_free_cong_map(struct rdsv3_ip_bucket *bucketp);
...
void rdsv3_cong_queue_updates(struct rdsv3_ip_bucket *bucketp);
...
struct rdsv3_transport *rdsv3_trans_get_preferred(uint32_be_t addr, void **dev);
...
struct rdsv3_message *rdsv3_message_copy_from_user(struct rdsv3_ip_bucket *,

...
/*
 * definitions for kernel, cprboot, poweradm
 */
#define	CPR_VERSION		7
#if defined(__sparc)
#define	CPR_CONFIG		"/etc/.cpr_config"
#else
#define	CPR_CONFIG		"/boot/solaris/cpr_config"
#endif
...
/*
 * Configuration info provided by user via poweradm.
 *
 ...
 * XXXX - Note that CFT_UFS is no longer supported.
 *
...
struct cprconfig {
	int	cf_magic;			/* magic word for	*/
						/* booter to verify	*/
	int	cf_type;			/* CFT_ZVOL or CFT_SPEC	*/
	 ...
	 * May need reworking in the face of SMF migration
	 */
 ...
 *	Machdep data: sun4u machine dependent info:
...
struct cpr_dump_desc {
	...
	ushort_t	cdd_machine;	/* sun4u, i86pc, etc. */
...
/*
 * physical memory bitmap descriptor, precedes the actual bitmap.
 */
...
/*
 * Describes saved pages, precedes page data;
 ...
 * for x86, page size is 4k, CPR_MAXCONTIG is 8
 * for sun4u, page size is 8k, CPR_MAXCONTIG is 4
 * Note, that even though there isn't a "prom" read for x86 platforms,
 * we define this value here so that the code is portable to all platforms.
 * Since the values are fine for x86, we just use them.  If this assumption
 * changes, so can these defines.
 */
 ...
 */
/* Suspend, leave hardware on and return success */
#define	AD_LOOPBACK_SUSPEND_TO_RAM_PASS	22

/* Suspend, leave hardware on and return failure */
#define	AD_LOOPBACK_SUSPEND_TO_RAM_FAIL	23

/* never shown to work properly, and it should be removed */
/* Suspend, ignored devices that fail to suspend */
...
/*
 * cpr_test_point values
 * Caution should be taken when adding to these events, as the
 * cpr_test_point can also be set to the AD* fcn's defined above.
 * So make sure that these do not conflict with any *CPR_TEST* fcn
 * values.
 */
#define	LOOP_BACK_NONE	0
#define	DEVICE_SUSPEND	1
#define	LOOP_BACK_FAIL	AD_LOOPBACK_SUSPEND_TO_RAM_FAIL
#define	LOOP_BACK_PASS	AD_LOOPBACK_SUSPEND_TO_RAM_PASS
...
struct cpr_stat {
	...
	size_t	cs_upage2statef;	/* actual # of upages gone to statef */
...
typedef struct cpr {
	...
	size_t		c_size;		/* Size of mapping area		*/
	size_t		c_iosize;	/* Best transfer size for dev	*/
} cpr_t;

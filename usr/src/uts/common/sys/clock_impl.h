...
/*
 * The deadman journal holds debugging information on a per CPU basis to
 * assist diagnosing possible system hangs.
 */
typedef struct {
	hrtime_t dj_hrtime;
	uint_t dj_intr_actv;
	uint32_t dj_nrunnable;
	kthread_t *dj_cpu_thread;
	uint64_t dj_pil_high_start[HIGH_LEVELS];
	uint64_t dj_intrstat[PIL_MAX][2];
} deadman_journal_t;

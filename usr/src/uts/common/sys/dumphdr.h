...
/*
 * Patch records immediately follow the data stream. These records are
 * used to update the dump image for memory that changes while writing
 * the crash file.
 *
 * Each dump_patch_t is followed by csize bytes of lzjb compressed data.
 * csize <= dsize <= pagesize
 */
typedef struct dump_patch {
	uint32_t patch_magic;		/* record identifier */
	uint32_t patch_sum;		/* data checksum */
	pfn_t patch_pfn;		/* pfn to patch */
	long patch_csize;		/* compressed size bytes follow */
	long patch_dsize;		/* original size */
	long patch_off;			/* offset within pfn */
} dump_patch_t;

#define	DUMP_PATCH_MAGIC	('d' << 24 | 'p' << 16 | 'a' << 8 | 't')

...
typedef struct vc_state {
	...
	int	vc_hold_cnt;
	kcondvar_t	vc_state_cv;

	/*
	 * vc_state_lock is currently used to protect vc_flags&vc_hold_cnt,
	 * The existence of this lock is because of wc_modechg_cb().
...
typedef struct vc_waitactive_msg {
	...
	vc_state_t	*wa_msg_vc;	/* state pointer from which msg comes */
	minor_t	wa_wait_minor;	/* which node we are waiting for */
	mblk_t *wa_mp;
} vc_waitactive_msg_t;
...
vc_state_t *vt_minor2vc_locked(minor_t minor, boolean_t polledio);

...
#define	MB_HEADER_FLAGS_64	 0x00010007	/* flags we use */
#define	MB_HEADER_CHECKSUM_64	-0x1BAEB009	/* -(magic + flag) */
...
typedef struct multiboot_header {
	...
	uint32_t	flags;
	uint32_t	checksum;
	caddr32_t	header_addr;	/* use as (multiboot_header_t *) */
	caddr32_t	load_addr;
...
typedef struct multiboot_info {
	...
	caddr32_t	apm_table;
	caddr32_t	vbe_control_info;
	caddr32_t	vbe_mode_info;
	uint16_t	vbe_mode;

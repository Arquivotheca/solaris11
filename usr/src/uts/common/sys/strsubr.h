...
#define	DMAB_NCOOKIES	32
#define	PM_MAX_NAME_LEN	64

#define	PM_HUGE_MEM	(1L << 37)		/* 128 GB */

typedef struct dbsizes_s {
	uint_t  dbs_size;
	uint_t  dbs_align;
	uint_t	dbs_magsize_max;
	uint_t	dbs_reap_all;
} dbsizes_t;
...
typedef struct dbinfo_s {
	ssize_t		dbi_size;
	ssize_t		dbi_align;
	ssize_t		dbi_dblk_size;
	ssize_t		dbi_magsize_max;
	uint_t		dbi_reap_all;
	struct vmem	*dbi_vmp;
	struct kmem_cache    *dbi_cache;
	struct kmem_cache    *dbi_cache_data;
	char		dbi_name[48];
	char		dbi_name_data[48];
} dbinfo_t;

typedef struct pm_contig_mem_descr_s {
	void	*pmd_contig_kaddr;
	size_t	pmd_contig_size;
} pm_contig_mem_descr_t;

extern	size_t dblk_dma_sizes[];
extern	int dma_block_size;
extern	int dma_block_size_max;

extern boolean_t premapped_cache_create(char *, boolean_t, size_t,
    size_t, dev_info_t *, ddi_dma_attr_t *, pm_handle_t *);
extern void premapped_cache_delete(pm_handle_t);
extern void *premapped_contig_mem_alloc(pm_handle_t, size_t);
extern void premapped_contig_mem_free(pm_handle_t, void *, size_t);
extern boolean_t premapped_contig_mem_info(pm_handle_t,
    pm_contig_mem_descr_t *);
 ...
 */
#define	U_TO_K 		0x01		/* User to Kernel */
#define	K_TO_K  	0x02		/* Kernel to Kernel */

#define	STRIOCTL_CRED  	0x04		/* Consumer must verify credentials */
...
extern mblk_t *desballoc_reusable(unsigned char *, size_t, frtn_t *);
...
extern mblk_t *allocb_premapped(pm_handle_t, size_t, boolean_t,
    ddi_dma_handle_t *, ddi_dma_cookie_t *);
extern mblk_t *allocb_premapped_reusable(pm_handle_t, size_t, frtn_t *);
...
extern int dblk_dma_info_get(pm_handle_t, void *, size_t, dblk_t *,
    ddi_dma_cookie_t *, uint_t *, ddi_dma_handle_t *);
...
#define	DB_HASH(mp)		((mp)->b_datap->db_struioun.cksum.pad)
#endif	/* _KERNEL */

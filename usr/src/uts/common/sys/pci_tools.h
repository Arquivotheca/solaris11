...
/* Set Completion Timeout value */
#define	PCITOOL_DEVICE_SET_CTO		(PCITOOL_IOC | 7)

/* Get system interrupt information */
#define	PCITOOL_SYSTEM_INTR_INFO	(PCITOOL_IOC | 8)

/* Link Retrain */
#define	PCITOOL_DEVICE_LINK_RETRAIN	(PCITOOL_IOC | 9)

/* Link Equalization */
#define	PCITOOL_DEVICE_LINK_EQUALIZATION	(PCITOOL_IOC | 10)
...
#define	PCITOOL_INTR_FLAG_BALANCER	0x80
...
typedef struct pcitool_intr_dev {
	...
	uint32_t	msg;		/* msg - from kernel */
	uint8_t		inum;		/* dev intr num - from kernel */
	uint8_t		pri;		/* intr priority - from kernel */
	char		type[MODMAXNAMELEN];	/* intr type - from kernel */
...
typedef struct pcitool_intr_get {
	...
	uint32_t	sysino;		/* apic id or sysino - from kernel */
	uint32_t	flags;		/* to kernel */
	uint8_t		ctlr;		/* controller number - from kernel */
	uint8_t		irq;		/* irq ino - from kernel */
	uint8_t		share;		/* intr share num - from kernel */
...
/* The data structure for Intr info list */
typedef struct pcitool_intr_node {
	pcitool_intr_get_t *node;
	struct pcitool_intr_node *link;
} pcitool_intr_node_t;
...
typedef struct pcitool_intr_info {
	...
	uint32_t num_cpu;		/* Number of cpu on platform */

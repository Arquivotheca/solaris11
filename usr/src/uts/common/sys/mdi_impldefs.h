...
typedef struct mdi_vhci {
	...
	ddi_dma_attr_t		*vh_dma_attr;	/* vHCI merged dma attributes */
...
typedef struct mdi_phci {
	...
	ddi_dma_attr_t		*ph_dma_attr;	/* pHCI dma attributes	*/
...
struct mdi_pathinfo {
	...
	uint64_t		pi_cna_dev;	/* CNA_DEV of path */
};
 ...
 *   MDI_SELECT_PATH_INSTANCE_AVOID_LD:	take the specified path(arg) as the
 *   					least desired choice, exclusive with
 *   					MDI_SELECT_PATH_INSTANCE and will be
...
#define	MDI_SELECT_PATH_INSTANCE_AVOID_LD 0x0020

/*
 * MDI client device utility functions
 */
int		mdi_client_get_path_count(dev_info_t *);
int		mdi_client_is_failed(dev_info_t *);
 ...
 * probe dev_info node, which is decorated with the device identity information

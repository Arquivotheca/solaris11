...
struct vmem {
	...
	void		*vm_private;	/* For use by vm_source_alloc/free */
	/*
	 * vm_qcache must be last; it may have more than VMEM_NQCACHE_MAX
	 * elements
	 */
...
#define	VMEM_SIZE(c) \
	((c <= VMEM_NQCACHE_MAX) ? sizeof (vmem_t) :	\
	offsetof(vmem_t, vm_qcache[c]))

...
/*
 * NOTE: Developers making use of the scheduler class switch mechanism to
 * develop scheduling class modules should be aware that the architecture is
 * not frozen and the kernel interface for scheduling class modules may change
 * in future releases of Solaris. Support for the current interface is not
 * guaranteed and class modules developed to this interface may require changes
 * in order to work with future releases of the system.
 */

/*
 * Three different ops vectors are bundled together, one for each of the
 * fundamental objects acted upon by these operators: procs, threads and the
 * scheduler.
 */
...
#define	CLASS_SYS(cid)		((cid) == syscid)
...
#define	CL_YIELD(t)		(*(t)->t_clfuncs->cl_yield)(t)
...
#define	CL_PRI_UTIL(t)		(*(t)->t_clfuncs->cl_pri_util)(t)

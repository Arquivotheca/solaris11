...
#define	IPC_XSTAT64	15	/* get options */
 ...
 * programs.
 * This structure is Consolidation Private, but the Xorg window system
 * also consumes it (as specified by the PSARC/2007/500 contract).
 */
struct shmid_ds64 {
...
/*
 * Used by IPC_XSTAT64 to obtain data related to Optimized Shared Memory
 * segment. Extra pad fields are reserved for future use, they are filled
 * with with zeros if they are not used.
 */
struct shmid_xds64 {
	ipc_perm64_t	shmx_perm;	/* operation permission struct */
	pid_t		shmx_lpid;	/* pid of last shmop */
	...
	ipc_time_t	shmx_dtime;	/* last shmdt time */
	ipc_time_t	shmx_ctime;	/* last change time */
	uint64_t	shmx_granule_sz; /* granule size */
	uint64_t	shmx_allocated;	/* mem allocated for OSM */
	uintptr_t	shmx_flags;
	uint64_t	shmx_pad[5];	/* reserved for future use */
};
...
int shmxctl64(int, int, struct shmid_xds64 *);

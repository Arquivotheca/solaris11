 ...
 * IDM lock order:
 *
 * idm_taskid_bucket.task_bkt_lock, idm_task_t.idt_mutex
 */
...
typedef struct idm_conn_params_s {
	...
	uint32_t		max_sgl_xfer_len;
	uint32_t		first_sgl_xfer_len;
...
typedef struct idm_svc_s {
	...
	/* Number of portals that are active & using this port */
	int			is_online;
...
/* Values for idb->idb_bufalloc -- where the buffer was allocated */
typedef enum {
	IDB_REG_MEM = 1,	/* caller's flat memory, no buffer */
	IDB_ZCOPY,		/* caller's SGlist, no buffer */
	IDB_IDMALLOC		/* buffer allocated from IDM's cache */
} idm_bufalloc_t;

#define	IDM_TASKIDS_MAX		16384		/* 0x4000h */
#define	IDM_ITT_MASK		0x3FFF		/* IDM_TASKIDS_MAX - 1 */
...
typedef struct idm_buf_s {
	...
	idm_bufalloc_t	idb_bufalloc;	/* where the buf was alloc'd from */
	/*
...
/* idb_flags */
#define	IDM_BUF_PHASECOLLAPSE_REQ	0x00000001 /* request phase collapse */
#define	IDM_BUF_PHASECOLLAPSE_SUCCESS	0x00000002 /* phase collapse success */
...
/*
 * Task "bucket" structure used to provide finer grain locking for
 * idm_taskid_table. Padded to 64 bytes to prevent false cache sharing.
 */

typedef struct {
	kmutex_t task_bkt_lock;
	idm_task_t *task;
	char pad[(64 - (sizeof (kmutex_t) + sizeof (idm_task_t *)))];
} idm_taskid_bucket_t;
...
#define	IDM_PDU_PHASECOLLAPSE	0x00000040
...
typedef struct {
	...
	idm_taskid_bucket_t	*idm_taskid_table;
	id_space_t	*idm_taskid_space;

...
extern int exece(uintptr_t file, const char **argp, const char **envp,
    int flags);
extern int exec_common(int fd, const char *path, const char **argp,
    const char **envp, int brand_action, int flags);
extern int gexec(vnode_t **vp, execa_t *uap, uarg_t *args,
    intpdata_t *idata, int level, long *execsz, caddr_t exec_file,
    struct cred *cred, int brand_action);
...
extern int execpermissions(vnode_t *vp, struct vattr *vattrp, uarg_t *args);
extern int execmap(vnode_t *vp, caddr_t addr, size_t len, size_t zfodlen,
    off_t offset, int prot, int page, uint_t);
extern void setexecenv(execenv_t *ep);
extern int execopen(vnode_t **vpp, int *fdp);
 ...
 * when compiling the 32-bit compatibility elf code in the elfexec module.
 */
extern int elfexec(vnode_t *, execa_t *, uarg_t *, intpdata_t *, int,
    long *, int, caddr_t, cred_t *, int);
extern int mapexec_brand(vnode_t *, uarg_t *, Ehdr *, Addr *,
    intptr_t *, caddr_t, int *, execenv_t *, uintptr_t *);
#endif /* !_ELF32_COMPAT */

...
typedef struct cpu {
	...
	hrtime_t	cpu_last_swtch;	/* last time switched to new thread */
	...
	/*
	 * cpu_steal_from indicates from which target CPU the current CPU
	 * should steal work from. This field can be set by the CPU wakeup
	 * routines when waking up an idle CPU to take work from a busy
	 * CPU's run queue. The purpose of this targeted stealing mechanism
	 * is to reduce dispatch latency and allow threads to be stolen
	 * more quickly.
	 */
	volatile struct cpu	*cpu_steal_from;

	struct numaio_info_s	*cpu_numaio_info;

	/*
	 * Current thread's CMT utilization. Since a thread's priority (and
	 * as a consequence, its utilization) can change while running, we
	 * keep track of what it was when the thread was last switched onto
	 * the CPU so we can increment/decrement accordingly.
	 */
	uint64_t	cpu_initial_util;

	/*
	 * cpu_intrstat[][] is used to keep track of ticks used at a given pil
	 * level. cpu_intrstat[pil][0] is cumulative and exported via kstats.
	 * cpu_intrstat[pil][1] is used in intr_get_time() and is private.
	 * 2-dimensional array improves cache locality.
	 */
	uint64_t	cpu_intrstat[PIL_MAX + 1][2];
	uint64_t	cpu_pil_high_start[HIGH_LEVELS];
 ...
 * case).
 *
 * CPU_POWERIDLE is used only for power capping, it indicates that the system
...
#define	CPU_POWERIDLE	0x400		/* CPU offline for power capping */

#define	FMT_CPU_FLAGS							\
	"\20\13poweridle\12fault\11spare\10frozen"			\
c	"\7poweroff\6offline\5enable\4exist\3quiesced\2ready\1run"
 ...
 */
#define	CPU_DISP_CTX_SWITCH	0x01	/* CPU undergoing context swtch */
#define	CPU_DISP_HALTED		0x02	/* CPU halted waiting for interrupt */
#define	CPU_DISP_REGULAR	0x04    /* current thread is not transient */
#define	CPU_DISP_TRANSIENT	0x08    /* current thread is transient */

/*
 * Dispatcher macros.
 */
#define	CPU_DISP_IS_CTX_SWITCH(cp)					\
	(cp->cpu_disp_flags & CPU_DISP_CTX_SWITCH)

#define	CPU_DISP_IS_HALTED(cp)						\
	(cp->cpu_disp_flags & CPU_DISP_HALTED)

#define	CPU_DISP_IS_REGULAR(cp)						\
	(cp->cpu_disp_flags & CPU_DISP_REGULAR)

#define	CPU_DISP_IS_TRANSIENT(cp)					\
	(cp->cpu_disp_flags & CPU_DISP_TRANSIENT)

#define	CPU_DISP_IDLE(cp)	(cp->cpu_dispatch_pri == -1)
...
extern void cpu_call_nowait(cpu_t *, cpu_call_func_t, uintptr_t, uintptr_t);

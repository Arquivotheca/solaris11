...
/*
 * new_freemem_lock protects freemem, freemem_throttle_wait,
 * freemem_throttle_cv, freemem_wait & freemem_cv.
 */
static kmutex_t	new_freemem_lock;

/* Sleepers in page_create_throttle */
static uint_t	freemem_throttle_wait;
static kcondvar_t freemem_throttle_cv;

/* Sleepers in page_create_wait */
static uint_t	freemem_wait;
static kcondvar_t freemem_cv;

/*
 * Wake up sleepers after adding (count) pages to PCF bucket (p).  For
 * single-page frees, we wake up a single waiter, preferring those
 * in page_create_wait() (which can tap the reserve).  For multi-page frees,
 * we wake all sleepers.
 */
#define	PCF_FREEMEM_WAKEUP(p, count)					\
	do {								\
		ASSERT(MUTEX_HELD(&(p)->pcf_lock));			\
		if (!(p)->pcf_wait) {					\
			break;						\
		}							\
		mutex_enter(&new_freemem_lock);				\
		if ((freemem_wait | freemem_throttle_wait) != 0) {	\
			_NOTE(CONSTCOND)				\
			if ((count) > 1) {				\
				cv_broadcast(&freemem_cv);		\
				cv_broadcast(&freemem_throttle_cv);	\
				(p)->pcf_wait = 0;			\
			} else {					\
				if (CV_HAS_WAITERS(&freemem_cv)) {	\
					cv_signal(&freemem_cv);		\
				} else {				\
					cv_signal(&freemem_throttle_cv); \
				}					\
				(p)->pcf_wait--;			\
			}						\
		} else {						\
 ...
 * PAGE_GET_FREELISTS(), PAGE_GET_CACHELISTS(),
...
/*
 * When a large span is returned to pcf buckets, give at least
 * pcf_chunk_pages to each bucket.  With a chunk of 512 pages, spans
 * larger than a 2MB page on x86 and a 4MB page on sparc are returned
 * to multiple buckets.
 */
uint_t pcf_chunk_pages = 512;
...
/*
 * Evenly spread out the PCF counters for large free pages.  Called
 * only when memory is added to the system, so do not take pcf locks,
 * and do not enforce the pcf_chunk_pages threshold.
 */
static void
 ...
 * clock() on each TICK.
 *
 * In between calls to set_freemem(), freemem is updated, but without
 * any locking so it should only be considered to be a hint.  Even
 * without any locking, it can become very hot when many CPUs all
 * allocate or free memory at the same time.  This can cause
 * performance issues with freemem ping ponging across caches.
 *
 * To mitigate this issue, single page updates to freemem have been
 * removed from this code.  In the worst case, pageout may get
 * confused and not page out when it should because freemem is bigger
 * than it should be due to this lazy accounting.  However, the error
 * caused by ignoring single page updates is considered to be small
 * enough not to matter much (eg. ~0.2-0.4% of total physical memory
 * while the "lotsfree" threshold below which pageout becomes active
 * is 1.56% of memory).  Besides, freemem will get accurately set
 * at the next clock tick, so everything will be perfectly clear to
 * pageout within 10 milliseconds anyway.
 */

#ifdef	DEBUG
/*
 * The most that freemem has been off between calls to set_freemem()
 */
static pgcnt_t	freemem_max_diff = ULONG_MAX;
#endif	/* DEBUG */
...
set_freemem()
{
	...
	ulong_t		t;
	uint_t		i;
	int		pcf_count = pcf_fanout;

	t = 0;
	p = pcf;
	for (i = 0;  i < pcf_count; i++) {
		if ((i + PREFETCH_Q_LEN) < pcf_count) {
			prefetch_read_once(p + PREFETCH_Q_LEN);
		}
		t += p->pcf_count;
		p++;
	}
	...
	/*
	 * Keep track of how far off freemem can get between calls to this
	 * routine (ignore first time since it can be pretty far off during
	 * boot)
	 */
	if (freemem_max_diff == ULONG_MAX)
		freemem_max_diff = 0;
	else if (diff > freemem_max_diff)
		freemem_max_diff = diff;
#endif	/* DEBUG */
...
page_create_throttle(pgcnt_t npages, int flags)
{
	...
	/*
	 * If we have memory pressure in early boot, we can get here.
	 * Just return failure in that case.
	 */
	if (proc_pageout == NULL) {
		if (flags & PG_WAIT) /* this will never succeed */
			panic("Early-boot memory exhausted.");
		return (0);
	}
	...
	for (;;) {
		...
		needfree += npages;
		freemem_throttle_wait++;
		cv_wait(&freemem_throttle_cv, &new_freemem_lock);
		freemem_throttle_wait--;
		needfree -= npages;
		mutex_exit(&new_freemem_lock);
	}
...
checkagain:
	 ...
	 * get the new_freemem_lock (it protects freemem_*wait),
...
/*
 * Return npages to pcf buckets, spreading among multiple buckets if
 * npages is large enough.  Return to pcf_count if the bucket has no
 * waiters, else return to pcf_reserve and wake waiters.
 */
void
page_create_putback(spgcnt_t npages)
{
	...
	/*
	 * When a large chunk is put back, spread the wealth around,
	 * giving lump to each bucket.  Start at a random bucket to
	 * avoid lock contention on pcf[0].  Lump should be at least
	 * pcf_chunk_pages, as there is no sense in locking every
	 * bucket to put just a few pages in each.  The randomness
	 * refills all buckets over time.
	 */
	lump = roundup(npages, pcf_fanout) / pcf_fanout;
	lump = MAX(lump, pcf_chunk_pages);
	if (npages > 1)
		freemem += npages;
	...
	last = &pcf[pcf_fanout];
	while (npages > 0) {
		const pgcnt_t give = MIN(lump, npages);
		npages -= give;

		mutex_enter(&p->pcf_lock);
		if (p->pcf_block) {
			p->pcf_reserve += (uint_t)give;
		} else {
			p->pcf_count += (uint_t)give;
			PCF_FREEMEM_WAKEUP(p, give);
		}
		mutex_exit(&p->pcf_lock);
...
pcgs_unblock(void)
{
	...
	for (i = 0; i < pcf_fanout; i++) {
		...
		p->pcf_block = 0;
		freemem += p->pcf_count;
		PCF_FREEMEM_WAKEUP(p, p->pcf_count);
		mutex_exit(&p->pcf_lock);
		p++;
	}
}
    ...
    caddr_t vaddr, uint_t flags)
{
	...
	if ((flags & (PG_NORELOC|PG_KFLT)) != 0) {
		 ...
		 *
		 * The kernel freelist also uses this throttling mechanism.
		 */
		/* LINTED */
	...
	for (count = 0; kcage_on || kflt_on || count < MAX_PCGS; count++) {
		PAGE_GET_FREELISTS(pp, vp, off, seg, vaddr, PAGESIZE,
		    flags, lgrp);
		if (pp == NULL) {
			PAGE_GET_CACHELISTS(pp, vp, off, seg, vaddr,
			    flags, lgrp);
		}
		if (pp == NULL) {
			/*
    ...
    struct seg *seg, caddr_t vaddr, void *arg)
{
	...
	/*
	 * If cage or kernel freelist is on, dampen draw from cage/freelist
	 * when available space is low.
	 */
	/* LINTED */
...
top:
		...
		if (npp == NULL) {
			...
			if (npp == NULL) {
				PAGE_GET_CACHELISTS(npp, vp, off, seg,
...

	{
		...
		if (overshoot) {
			...
			if (p->pcf_block) {
				...
				PCF_FREEMEM_WAKEUP(p, overshoot);
			}
			mutex_exit(&p->pcf_lock);
			/* freemem is approximate, so this test OK */
			if (!p->pcf_block && overshoot > 1)
				freemem += overshoot;
		}
	}
...
page_destroy_free(page_t *pp)
{
	...
	mutex_enter(&new_freemem_lock);
	if (freemem_wait && CV_HAS_WAITERS(&freemem_cv)) {
		cv_signal(&freemem_cv);
	} else if (freemem_throttle_wait) {
		cv_signal(&freemem_throttle_cv);
	}
	mutex_exit(&new_freemem_lock);
}
 ...
 *
 * Cannot fail; if there is another page hashed to this <vp, off> name,
 * we panic.
 */
void
page_hashin(page_t *pp, vnode_t *vp, u_offset_t offset, kmutex_t *hold)
{
	const ulong_t	index = PAGE_HASH_FUNC(vp, offset);
	kmutex_t *const	phm = (hold != NULL) ? hold : PAGE_HASH_MUTEX(index);
	kmutex_t *const	vphm = page_vnode_mutex(vp);
	page_t		**listp;
	page_t		*tp;

	ASSERT(MUTEX_NOT_HELD(vphm));
	...
	IMPLY(hold != NULL, hold == PAGE_HASH_MUTEX(index));
	if (hold == NULL) {
	...
	/*
	 * If this page is already hashed in, we panic.
	 */
	for (tp = *listp; tp != NULL; tp = tp->p_hash) {
		if (tp->p_vnode == vp && tp->p_offset == offset) {
			panic("page_hashin: pp %p on bucket %ld is already "
			    "named <vnode %p, offset %llx>. Can't hash in "
			    "pp %p.", (void *)tp, index, (void *)vp, offset,
			    (void *)pp);
		}
	}
	pp->p_hash = *listp;
...
{
	vnode_t	*const	vp = pp->p_vnode;
	const ulong_t	index = PAGE_HASH_FUNC(vp, pp->p_offset);
	kmutex_t *const	phm = (hold != NULL) ? hold : PAGE_HASH_MUTEX(index);
	kmutex_t *const	vphm = page_vnode_mutex(vp);
	page_t		**hpp;

	ASSERT(MUTEX_NOT_HELD(vphm));
	IMPLY(hold != NULL, MUTEX_HELD(hold));
	IMPLY(hold != NULL, hold == PAGE_HASH_MUTEX(index));
	ASSERT(vp != NULL);
	 ...
	 */
	for (hpp = &page_hash[index]; *hpp != pp; hpp = &(*hpp)->p_hash) {
		if (*hpp == NULL) {
			panic("page_hashout: page %p not found in chain %ld",
			    (void *)pp, index);
			/*NOTREACHED*/
		}
	}
 ...
 *	off = old->p_offset;
 *	page_hashout(old)
 *	page_hashin(new, vp, off)
 *
 * Doesn't work, since we would drop and re-acquire the vphm:
 *
...
page_do_relocate_hash(page_t *new, page_t *old)
{
	...
	for (;;) {
		if (*hash_list == old)
			break;
		if (*hash_list == NULL) {
			panic("page_do_relocate_hash: page not found");
			/*NOTREACHED*/
		}
...
{
	struct pcf	*p, *first, *last;
	pgcnt_t pcftotal;

	/*
	 * Try to collect pages from several pcf bins.  Start at
	 * a random bucket to avoid lock contention on pcf[0].
	 */
	p = first = &pcf[PCF_INDEX()];
	last = &pcf[pcf_fanout];
	pcftotal = 0;
	for (;;) {
		mutex_enter(&p->pcf_lock);
		...
		if (p >= last) {
			/*
			 * If we started at the beginning we are
			 * done. Otherwise, release the locks we have
			 * taken so far and start over. We can't just
			 * wrap around to pcf[0], else deadlock would
			 * occur vs other threads taking the locks in
			 * the opposite order.
			 */
			if (first == pcf)
				break;

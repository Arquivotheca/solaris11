...
void
as_setlwpchan(struct as *as, caddr_t addr)
{
	struct seg *seg;

	AS_LOCK_ENTER(as, &as->a_lock, RW_READER);
	seg = as_segat(as, addr);
	if (seg != NULL && !(seg->s_flags & S_LWPCHAN))
		atomic_or_32(&seg->s_flags, S_LWPCHAN);
	AS_LOCK_EXIT(as, &as->a_lock);
}
...
top:
	...
	for (seg = as_findseg(as, raddr, 0); seg != NULL; seg = seg_next) {
		...
		/*
		 * If this segment contains lwpchan cache mappings,
		 * delete all such mappings in the full address range.
		 * We have to drop the address space lock to do
		 * this, so start over at the top when done
		 * (and remember that we have done this).
		 */
		if ((seg->s_flags & S_LWPCHAN) && !lwpchan_done &&
		    (p = curproc)->p_as == as && p->p_lcp != NULL) {
			AS_LOCK_EXIT(as, &as->a_lock);
			lwpchan_delete_mapping(p, raddr, eaddr);
			lwpchan_done = 1;
			goto top;
		}
	...
	ulong_t *bitmap, size_t position, size_t npages)
{
	...
	/*
	 * Unlocking OSM pages when MC_LOCK_GRANULE failed.
	 */
	if (bitmap == NULL) {
		(void) SEGOP_LOCKOP(seg, addr, ptob(npages), attr,
		    MC_UNLOCK_GRANULE, (ulong_t *)NULL, (size_t)NULL);
		return;
	}
...
/*
 * When MC_LOCK_GRANULE fails the as_unlockerr() with mlock_map == NULL
 * is called to unlock all pages. It has to unlock all pages because
 * MC_UNLOCK_GRANULE addr and size has to be aligned on granule size.
 * The SEGOP_LOCKOP(,,MC_UNLOCK_GRANULE,,,) skips unlocked granules (pages).
 */
static void
...
retry:
	...
	/*
	 * The MC_LOCK_GRANULE, MC_UNLOCK_GRANULE is only supported by seg_spt
	 */
	if (!SEGOP_CAPABLE(seg, S_CAPABILITY_LOCK_GRANULE)) {
		if (func == MC_LOCK_GRANULE || func == MC_UNLOCK_GRANULE) {
			AS_LOCK_EXIT(as, &as->a_lock);
			return (ENOSYS);
		}
	}

	if (func == MC_LOCK || func == MC_LOCK_GRANULE) {
	...
	while (rsize != 0) {
		...
		if (raddr >= seg->s_base + seg->s_size) {
			...
			/*
			 * The MC_LOCK_GRANULE/MC_UNLOCK_GRANULE is only
			 * supported by seg_spt
			 */
			if ((seg != NULL) &&
			    !SEGOP_CAPABLE(seg, S_CAPABILITY_LOCK_GRANULE)) {
				if (func == MC_LOCK_GRANULE ||
				    func == MC_UNLOCK_GRANULE) {
					if (func == MC_LOCK_GRANULE) {
						as_unlockerr(as, attr, NULL,
						    initraddr, initrsize);
						kmem_free(mlock_map,
						    mlock_size *
						    sizeof (ulong_t));
			...
			if (seg == NULL || raddr != seg->s_base) {
				if (func == MC_LOCK ||
				    func == MC_LOCK_GRANULE) {
					if (func == MC_LOCK) {
						as_unlockerr(as, attr,
						    mlock_map, initraddr,
						    initrsize - rsize);
					} else {
						as_unlockerr(as, attr, NULL,
		...
		case MC_LOCK_GRANULE:
			if (error = SEGOP_LOCKOP(seg, raddr, ssize,
				...
				} else {	/* MC_LOCK_GRANULE */
					as_unlockerr(as, attr, NULL, initraddr,
					    initrsize);
				}

...
/*
 * if this is a zvol being used as swap, try to allocate (write) all
 * blocks ahead of time in order to minimize the amount of memory we
 * need to use when swapping
 */
int
swapify(vnode_t *cvp, boolean_t add)
{
	vnode_t *cdev_vp;
	int error = EINVAL;
	...
	if (VOP_OPEN(&cdev_vp, FREAD | FWRITE, kcred, NULL) == 0) {
		...
		if (VOP_IOCTL(cdev_vp, DKIOCINFO, (intptr_t)&dki,
		    ...
		    strcmp(dki.dki_dname, ZVOL_DRIVER) == 0)
			error = VOP_IOCTL(cdev_vp, add ?
			    DKIOCPREALLOCINIT : DKIOCPREALLOCFINI,
			    NULL, FKIOCTL, kcred, NULL, NULL);

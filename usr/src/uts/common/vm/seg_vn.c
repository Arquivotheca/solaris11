...
segvn_create(struct seg *seg, void *argsp)
{
	...
	if (a->amp == NULL && !use_rgn && !trok) {
		...
		if ((a->vp == NULL) && (a->type == MAP_PRIVATE) &&
			...
			if (lgrp_mem_policy_flags == LGRP_MEM_POLICY_DEFAULT)
				mpolicy = lgrp_mem_policy_default(seg->s_size,
				    a->type);
		} else {
			/*
	...
	/*
	 * Set memory allocation policy for segment
	 *
	...
	if (svd->type == MAP_SHARED) {
		mpolicy = lgrp_shmadv_to_policy(a->advice);
    ...
    caddr_t eaddr, int brkcow)
{
	...
	for (;;) {
		...
		chkszc:
			...
			upgrdfail = 0;
			if (szc == 0 ||
			    (pszc >= szc && !IS_P2ALIGNED(pfn, pages)) ||
			    (pszc < szc &&
...
segvn_setprot(struct seg *seg, caddr_t addr, size_t len, uint_t prot)
{
	...
	if (addr == seg->s_base && len == seg->s_size && svd->vpage == NULL) {
		 ...
		 */
		for (svp = &svd->vpage[seg_page(seg, addr)]; svp < evp; ) {
			pgcnt_t idx, pages_in;
			int success = 1;
			...
			/*
			 * We treat large pages that could be demoted as if they
			 * are small pages to make sure we don't intersect with
			 * a pagesize change and thus wind up holding the
			 * "wrong" lock part of the way through.
			 */
			if (PAGE_MAY_DEMOTE(pp) || pp->p_szc == 0) {
				if (amp != NULL)
					anon_array_exit(&cookie);
				if ((VPP_PROT(svp) ^ prot) & PROT_WRITE) {
					if (prot & PROT_WRITE)
						success = page_addclaim(pp);
					else
						success = page_subclaim(pp);
				}
				page_unlock(pp);
				if (!success)
					break;
			...
			/*
			 * This is a large page. We may have skipped the root
			 * page in the above VPP_ISPPLOCK check but we have a
			 * subpage that needs to be handled so back up to the
			 * root page if we're not already there.
			 * segvn_claim_pages() will do the right thing for each
			 * page.
			 */
			pgcnt = page_get_pagecnt(pp->p_szc);
			pages_in = P2PHASE(anon_idx, pgcnt);
			offset -= pages_in * PAGESIZE;
			svp -= pages_in;
			anon_idx -= pages_in;
			success = segvn_claim_pages(seg, svp, offset, anon_idx,
			    prot, pp, anon_idx + pages_in);
			if (amp != NULL)
				anon_array_exit(&cookie);
			if (!success)
				break;
			...
			/* pp was page_unlock'd inside segvn_claim_pages(). */
		}
		if (amp != NULL)
			ANON_LOCK_EXIT(&amp->a_rwlock);
...
segvn_claim_pages(
	...
	ulong_t locked_idx)
{
	pgcnt_t	pgcnt = page_get_pagecnt(locked_pp->p_szc);
	...
	ASSERT(svd->type == MAP_PRIVATE);
	ASSERT(svd->vpage != NULL);
	ASSERT(locked_pp->p_szc != 0);
	ASSERT(IS_P2ALIGNED(pgcnt, pgcnt));
	ASSERT(amp == NULL || IS_P2ALIGNED(anon_idx, pgcnt));
	ASSERT(sameprot(seg, addr, pgcnt << PAGESHIFT));

	if (VPP_PROT(svp) == prot || !((VPP_PROT(svp) ^ prot) & PROT_WRITE)) {
		page_unlock(locked_pp);
		return (1);
	}
	...
	for (*ppa = NULL, pg_idx = 0; svp < evp; svp++, anon_idx++,
		...
		ASSERT(vp != NULL);
		/* We are passed one locked pp, so don't look it up again. */
		if (anon_idx != locked_idx) {
	...
	/* Adjust the claims for the pages in ppa */
 ...
 *	MADV_ACCESS_MANY_PSET - Many LWPs or processes in pset will
 *				access heavily
 */
static int

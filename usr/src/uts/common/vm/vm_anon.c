 ...
 * starting from the given index and going up to the given limit (end).  The
 * index of found anon pointer is set in '*index', and if no valid anon
 * pointer is found in the interval [index, end) null is returned.
 */
struct anon *
anon_get_next_ptr(struct anon_hdr *ahp, ulong_t *index, ulong_t end)
{
	struct anon *ap;
	struct anon **app;
	ulong_t chunkoff;
			...
			ulong_t new_idx, size_t size)
{
	...
	while (npages > 0) {
		index = old_idx;
		if ((ap = anon_get_next_ptr(old, &index, ULONG_MAX)) == NULL)
			break;
	...
	int noalloc)
{
	...
	while (npages > 0) {
		 ...
		 */
		if (anon_get_next_ptr(old, &index, ULONG_MAX) == NULL)
			break;
	...
	struct cred *cred)
{
	...
	while (npages > 0) {
		 ...
		 * Find the next valid slot.
		 */
		if (anon_get_next_ptr(ahp, &index, ULONG_MAX) == NULL) {
			break;
		}
...
anon_free(struct anon_hdr *ahp, ulong_t index, size_t size)
{
	...
	while (npages > 0) {
		old = index;
		if ((ap = anon_get_next_ptr(ahp, &index, ULONG_MAX)) == NULL)
			break;
...
static int anon_free_pages_task(ulong_t, ulong_t, void *, ulong_t *);
	...
	uint_t szc)
{
	...
	/*
	 * anon_free_pages() uses large page granularity to split the job into
	 * tasks.  Therefore, job_size is the number of large pages (szc) that
	 * will be freed in parallel with minimal recommended chunks equal to:
	 * VMTASK_LPGS_MINJOB.
	 */
	job_size	= npages / pgcnt;

	(void) vmtask_run_job(job_size, VMTASK_LPGS_MINJOB,
	    anon_free_pages_task, (void *)&targ, NULL);
}

static int
anon_free_pages_task(ulong_t idx, ulong_t end, void *arg, ulong_t *ridx)
{
	struct anon_frp	*targp	= (struct anon_frp *)arg;
	struct anon_hdr	*ahp	= targp->ahp;
	uint_t		szc	= targp->szc;
	pgcnt_t		pgcnt	= page_get_pagecnt(szc);
	ulong_t		an_idx 	= targp->an_idx + idx * pgcnt;
	spgcnt_t	npages	= (end - idx) * pgcnt;
	ulong_t		index, off;
	...
	while (npages > 0) {
		 ...
		 * Find the next valid slot.
		 */
		if (anon_get_next_ptr(ahp, &index, index + npages) == NULL)
			break;
	...
	*ridx = end - npages/pgcnt;

	return (0);
}

/*
 * Make anonymous pages discardable. If freeall is set all pages
 * are freed. If freeall is not set skip pages which can not
 * be locked. For freeall set index must be aligned on page size
 * being freed and size must be multiple of page size being freed.
 */
void
anon_disclaim(struct anon_map *amp, ulong_t index, size_t size, int freeall)
{
	...
	for (; npages > 0; index = (pgcnt == 1) ? index + 1 :
		...
		for (idx = 1; idx < pgcnt; idx++) {
			pp++;
			if (freeall) {
				while (page_lock(pp, SE_EXCL, NULL,
				    P_NO_RECLAIM) == 0) {
					continue;
				}
			} else {
	...
	struct cred *cred)
{
	...
	*app = new = anon_alloc(NULL, 0);
	 ...
	 * to the new page by transferring 'cowcnt/lckcnt' of the original
	...
	struct cred *cred)
{
	...
	for (; pg_idx < pgcnt; pg_idx++, an_idx++, vaddr += PAGESIZE) {
		 ...
		 * the lock to the new page by transferring
	...
	struct cred *cred)
{
	...
	/*
	 * anon_map_createpages() splits the job into a number of tasks using
	 * small page granularity.  However, to make sure large pages are used
	 * by threads, the job_chunk_min is set to page count in largest allowed
	 * page.
	 */
	...
	(void) vmtask_run_job(npgs, job_chunk_min, anon_map_createpages_task,
	    (void *)&targ, NULL);
...
static int
anon_map_createpages_task(ulong_t idx, ulong_t end, void *arg, ulong_t *ridx)
{
	struct anon	*ap;
	struct vnode	*ap_vp;
	...
	addr	= targp->addr + PAGESIZE * idx;
	index	= targp->start_index + idx;
	npgs	= end - idx;
	p_index	= idx;
...
anon_shmap_free_pages(struct anon_map *amp, ulong_t sidx, size_t len)
{
	...
	tidx = eidx;
	if (eidx != ahp->size && anon_get_next_ptr(ahp, &tidx, ULONG_MAX)
	    != NULL && tidx - sidx < pages) {

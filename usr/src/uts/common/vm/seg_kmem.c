...
boot_mapin(caddr_t addr, size_t size)
{
	...
	for (eaddr = addr + size; addr < eaddr; addr += PAGESIZE) {
		...
		page_hashin(pp, &kvp, (u_offset_t)(uintptr_t)addr, NULL);
		pp->p_lckcnt = 1;
#if defined(__x86)
		page_downgrade(pp);
#else
...
/*
 * Aligned allocation as requested by the caller. Currently used by
 * network DMA buffer cache framework
 */
void *
segkmem_align_alloc(vmem_t *vmp, size_t *size_ptr, size_t align, int vmflag)
{
	char	*addr;

	addr = vmem_xalloc(vmp, *size_ptr, align, 0, 0, NULL, NULL, vmflag);
	if (addr == NULL)
		return (NULL);

	return (segkmem_xalloc(vmp, addr, *size_ptr, vmflag, 0,
	    segkmem_page_create, &kvp));
}
...
segkmem_alloc_lp(vmem_t *vmp, size_t *sizep, size_t align, int vmflag)
{
	...
	if (lpcb->lp_uselp && !(t->t_flag & T_PANIC) &&
		...
		if (asize < size) {	/* overflow */
			cmn_err(CE_WARN, "segkmem_alloc_lp(): size overflow");
			return (NULL);
		}
...
/*
 * Allocations from heap_lp_arena with permanent va to pa mapped
 * pages.
 */
void *
segkmem_alloc_lpi_permanent(vmem_t *vmp, size_t size, int vmflag)
{
	return (segkmem_alloc_lpi(vmp, size, vmflag | VM_NORELOC));
}
...
segkmem_zio_init(void *zio_mem_base, size_t zio_mem_size)
{
	 ...
	 * smaller sizes;  we chose 256k because that translates to 1M VA
	 * slabs.
	 */
	zio_arena = vmem_create("zfs_file_data", zio_mem_base, zio_mem_size,
	    PAGESIZE, NULL, NULL, NULL, 256 * 1024, VM_SLEEP);

    ...
    uintptr_t arg3, uintptr_t arg4, uintptr_t arg5, uintptr_t arg6)
{
	...
	*rval = 0;
	...
	/*
	 * The sn1 brand has no brand-specific subcodes, so simply fail.
	 */
	return (ENOSYS);
}
...
_init(void)
{
	...
	sn1_brand.b_modname = strdup(mod_modname(&modlinkage));
...
_fini(void)
{
	...
	strfree(sn1_brand.b_modname);

	err = mod_remove(&modlinkage);
	if (err)
		cmn_err(CE_WARN, "Couldn't unload sn1_brand brand module");
	return (err);
}

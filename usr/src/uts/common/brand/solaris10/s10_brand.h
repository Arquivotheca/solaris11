...
/*
 * Brand system call subcodes.  Codes below B_BRANDSPECIFIC_BASE are reserved
 * for generic subcodes.
 */
#define	B_S10_PIDINFO		(B_BRANDSPECIFIC_BASE + 0)
#define	B_S10_NATIVE		(B_BRANDSPECIFIC_BASE + 1)
#define	B_S10_FSREGCORRECTION	(B_BRANDSPECIFIC_BASE + 2)
#define	B_S10_ISFDXATTRDIR	(B_BRANDSPECIFIC_BASE + 3)
...
typedef struct s10_proc_args {
	int	sa_argc;
	uintptr_t sa_argv;
	uintptr_t sa_envp;
} s10_proc_args_t;

typedef struct s10_proc_args_32 {
	int	sa_argc;
	uint32_t sa_argv;
	uint32_t sa_envp;
} s10_proc_args_32_t;

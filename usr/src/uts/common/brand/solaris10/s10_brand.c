 ...
 */
int
s10_native(void *cmd, void *execname, void *argstring, void *proc_args)
{
	...
	s10_proc_args_32_t s10_args32;
	int		i;

	if (copyin(cmd, &cmd_buf, sizeof (cmd_buf)) != 0)
		return (EFAULT);
	if (copyin(argstring, &args_buf, sizeof (args_buf)) != 0)
		return (EFAULT);
	if (get_udatamodel() == DATAMODEL_ILP32) {
		if (copyin((const void *)proc_args, &s10_args32,
		    sizeof (s10_args32)) != 0)
			return (EFAULT);
	} else {
		if (copyin((const void *)proc_args, &s10_args,
		    sizeof (s10_args)) != 0)
			return (EFAULT);
	}
	...
	(void) strlcpy(up->u_psargs, args_buf, sizeof (up->u_psargs));

	if (get_udatamodel() == DATAMODEL_ILP32) {
		up->u_argc =  s10_args32.sa_argc;
		up->u_argv =  (uintptr_t)s10_args32.sa_argv;
		up->u_envp =  (uintptr_t)s10_args32.sa_envp;
	} else {
		up->u_argc =  s10_args.sa_argc;
		up->u_argv =  s10_args.sa_argv;
		up->u_envp =  s10_args.sa_envp;
	}
    ...
    uintptr_t arg3, uintptr_t arg4, uintptr_t arg5, uintptr_t arg6)
{
	...
	*rval = 0;
	...
	/*
	 * See if this a generic command we need to handle.  brand_solaris_cmd()
	 * will return -1 if it doesn't know what to do.
	 */
	...
	/*
	 * Handle native processes in S10 branded zones invoking brandsys()
	 */
	if (!PROC_HAS_BRANDOPS(p)) {
		/* B_S10_NATIVE is only allowed for native processes. */
		if (cmd == B_S10_NATIVE)
			return (s10_native((void *)arg1, (void *)arg2,
			    (void *)arg3, (void *)arg4));

		/* No other subcmds are allowed for native processes. */
		return (ENOSYS);
	}
...
_init(void)
{
	...
	s10_emulation_table[SYS_ucredsys] = 1;			/*  83 */
	...
	s10_emulation_table[S10_SYS_utimesys] = 1;		/* 110 */
#if defined(__x86)
	...
	s10_brand.b_modname = strdup(mod_modname(&modlinkage));
...
_fini(void)
{
	...
	strfree(s10_brand.b_modname);

	err = mod_remove(&modlinkage);
	if (err)
		cmn_err(CE_WARN, "Couldn't unload s10_brand brand module");
	return (err);
}

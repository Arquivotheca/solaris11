	...
	off_t offset, off_t len, caddr_t *vaddrp)
{
	...
	int check, rnum = -1;
	uint32_t base;
	uchar_t regbuf[sizeof (u_regspec_t)];
...
#if defined(__i386) || defined(__amd64)
	case PTYPE_ISA:
		isareg = (isa_regspec_t *)buff;
		isareg->phys_hi = (type == PCMCIA_MAP_IO) ? 1 : 0;
		isareg->phys_lo = ret->ra_addr_lo;
		isareg->size = ret->ra_len;
		break;
#endif
	default:
...
#if defined(__i386) || defined(__amd64)
		defreg->regspec_addr_high = ret->ra_addr_hi;
		defreg->regspec_addr_low = ret->ra_addr_lo;
		SPLIT_REGSPEC_SIZE((uint64_t)ret->ra_len, defreg);
#else

...
top:
	...
	if (vfsp != NULL) {
		...
		if (!error) {
			/*
			 * Prevent unmount of fs on top of dvp for next
			 * "dfnip->mount_to" seconds (at least).
			 */
			mutex_enter(&dfnp->fn_lock);
	...
	case 0:
		/*
		 * Check for any errors on the previous operation on that node.
		 * If the previous operation on this node was not completed
		 * fall through to AUTOFS_LOOKUP.
		 */
		error = fnp->fn_error;
	...
	case AUTOFS_LOOKUP:
		 ...
		 * responsible for triggering the look-up of this node.
		 */
	...
	case AUTOFS_MOUNT:
		/*
		 * Since I got to this point, it means I'm the one
		 * responsible for triggering the mount of this node.
		 */
	...
	if (!error) {
		/*
		 * Prevent unmount of fs on top of vpp for next
		 * "dfnip->mount_to" seconds (at least).
		 */
		mutex_enter(&fnp->fn_lock);
...
auto_inactive(vnode_t *vp, cred_t *cred, caller_context_t *ct)
{
	...
	/*
	 * The rwlock should not be already held by this thread, unless
	 * VOP_INACTIVE is called as result of trigger node unmount.
	 */
	if (rw_write_held(&dfnp->fn_rwlock))
		lockheld = B_TRUE;
	else
		rw_enter(&dfnp->fn_rwlock, RW_WRITER);
	...
	if (--vp->v_count > 0) {
		/*
		 * Someone grabbed reference to this vnode while v_lock
		 * wasn't held.
		 */
		mutex_exit(&vp->v_lock);
		if (!lockheld)
			rw_exit(&dfnp->fn_rwlock);
		AUTOFS_DPRINT((5, "auto_inactive: (exit) vp=%p v_count=%u "
		    "fn_link=%d\n", (void *)vp, vp->v_count, fnp->fn_linkcnt));
		return;
	}
	mutex_exit(&vp->v_lock);
	...
	/*
	 * It's bad when node is supposed to be freed eventhough it has
	 * subdirectories.
	 */
...
retry:
	...
	if (delayed_ind ||
		...
		mutex_exit(&fnp->fn_lock);
		if (delayed_ind)
			(void) strlcpy(name, fnp->fn_name, AUTOFS_MAXPATHLEN);
		else
			(void) strcpy(name, ".");

...
/*ARGSUSED*/
static void
autofs_zone_shutdown(zoneid_t zoneid, void *arg)
{
...
autofs_init(int fstype, char *name)
{
	 ...
	 * constructor.
	 */
	zone_key_create(&autofs_key, NULL, autofs_zone_shutdown,
...
auto_mount(vfs_t *vfsp, vnode_t *vp, struct mounta *uap, cred_t *cr)
{
	...
	if (!(uap->flags & MS_SYSSPACE)) {
		...
		fngp->fng_rootfnnodep->fn_linkcnt++;
		fngp->fng_rootfnnodep->fn_size++;
...
auto_root(vfs_t *vfsp, vnode_t **vpp)
{
	...
	*vpp = (vnode_t *)vfstofni(vfsp)->fi_rootvp;
	...
	/* Update timestamp of fnnode's last use */
	fnp = vntofn(*vpp);

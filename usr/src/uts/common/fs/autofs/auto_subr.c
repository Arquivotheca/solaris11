...
auto_mount_thread(struct autofs_callargs *argsp)
{
	...
	/*
	 * hard flag is set to false, because we want user to be able to
	 * terminate stuck mount thread by restarting autofs service.
	 */
	error = auto_mount_request(fnip, name, &alp, cred, FALSE);
	if (!error)
...
/*
 * Meaning of "hard flag" is to try the operation "forever" until it is
 * successful. We distinguish between hard and soft errors. Soft error
 * is error returned by doors server. Hard error is communication error
 * (doors failure). Soft errors other than EOVERFLOW cause immediate
 * return from auto_calldaemon() no matter what is the value of "hard flag".
 * In case of hard error, the flag isn't honored and auto_calldaemon() doesn't
 * retry doors call if:
 *
 *	thread was canceled by pthread_cancel()
 *	automountd was running but now it isn't running (stale door descriptor)
 *	doors call returns unexpected error
 *	doors call doesn't handle interrupt completely
 */
int
auto_calldaemon(
	zoneid_t 		zoneid,
	...
	bool_t 			hard)
{
	...
	do {
		...
		/*
		 * Handle daemon errors (soft errors)
		 */
		if (error == 0) {
			/*
			...
			switch (error = door_resp->res_status) {
			...
			case EOVERFLOW:
				 ...
				 */
				if (door_resp->xdr_len > 0 &&
				    btopr(door_resp->xdr_len) < freemem / 64) {
					kmem_free(orp, orl);
					orp = kmem_zalloc(door_resp->xdr_len,
					    KM_SLEEP);
					orl = door_resp->xdr_len;
					retry = B_TRUE;
					break;
				}
				/*FALLTHROUGH*/
			...
			default:
				/*
				 * We never retry call in case of soft error
		...
		/*
		 * now process door/comm errors (hard errors)
		 */
		switch (error) {
		case EINTR:
			/*
			 ...
			 * interrupt completely then we need to bail out.
			 */
		...
		/* Back off for a bit in case of hard error */
		if (retry)
			delay(hz);
	} while (error != 0 && retry);

	door_resp = (autofs_door_res_t *)door_args.rbuf;
	if ((void *)door_resp != orp)
		kmem_free(orp, orl);
	...
	if (error == 0) {
		...
		if (!((*xresp_func)(&xdrres, resp))) {
			error = EINVAL;
		} else if (fngp->fng_printed_not_running_msg == 1) {
...
auto_null_request(zoneid_t zoneid, bool_t hard)
{
	...
	error = auto_calldaemon(zoneid, NULLPROC,
	    xdr_void, NULL, xdr_void, NULL, hard);
	...
	bool_t hard)
{
	...
	error = auto_calldaemon(fngp->fng_zoneid, AUTOFS_UNMOUNT,
	    xdr_umntrequest, (void *)ul, xdr_umntres, (void *)&xdrres, hard);
	...
	cred_t *cred)	/* Credentials of the caller */
{
	...
	/*
	 * Lock the base mount point if daemon mounted a filesystem on autofs
	 * root node until we finish mounting of all filesystems. This is done
	 * so that nobody can forcibly unmount base filesystem while subordinate
	 * mounts are in progress.
	 */
	if ((error = vn_vfsrlock_wait(dvp)) != 0) {
		xdr_free(xdr_action_list, (char *)alp);
		crfree(zcred);
		return (error);
	}
	...
	for (p = alp; p != NULL; p = p->next) {
		...
		if (m->dir[0] == '.' && m->dir[1] == '\0') {
			...
			if (dfnp->fn_flags & MF_MOUNTPOINT) {
				auto_log(fngp->fng_verbose, fngp->fng_zoneid,
				    CE_WARN, "autofs: mount of %s failed - "
				    "another filesystem is already mounted.",
				    dvp->v_path);
				continue;
			}
			mvp = dvp;
		...
		if ((dfnp->fn_flags & MF_MOUNTPOINT) == 0) {
			/*
	...
	fnnode_t *fnp)
{
	...
	/*
	 * Disconnect node from parent directory. In following two cases
	 * the node isn't found in parent's dirent list:
	 *
	 * 	Trigger node. Reference from parent to trigger node is
	 *	maintained in parent's fn_trigger list. It is removed from
	 * 	parent's trigger list elsewhere, because all trigger siblings
	 *	need to be removed from parent at once (atomically).
	 *
	 *	Autofs mounted on autofs dirent. Reference from parent to
	 *	autofs node, which is root of autofs, is maintained via
	 *	traditional vnode's v_vfsmountedhere pointer. Is is updated
	 *	when autofs is unmounted.
	 */
	fnpp = &dfnp->fn_dirents;
	for (tmp = dfnp->fn_dirents; tmp != NULL; tmp = tmp->fn_next)  {
		if (tmp == fnp) {
...
try_unmount_node(fnnode_t *fnp, boolean_t force)
{
	...
	if (error != 0) {
		...
		mutex_enter(&fnp->fn_lock);
		/*
		 * If the filesystem is in use, then start counting idleness
		 * from now to prevent frequent unmount attempts.
		 */
		if (error == EBUSY) {
			timestruc_t now;
...
unmount_subtree(fnnode_t *rootfnp, boolean_t force)
{
	...
	while (currfnp != rootfnp || rootfnp->fn_unmount_ref_time < timestamp) {
		ASSERT(MUTEX_HELD(&currfnp->fn_lock));

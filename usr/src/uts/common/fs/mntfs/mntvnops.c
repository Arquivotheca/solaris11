...
/*
 * Determine whether or not a vfs_t's resource is a ZFS dataset that qualifies
 * for aliasing. This occurs in the following situations:
 *
 * What		VFS_NOZONEMOD	Aliased?
 * ------------	---------------	--------
 * /		True		Yes
 * delegated	False		Yes
 * add fs	True		No
 *
 * Note that other fs types (including lofs-mounted zfs) are not aliased.
 */
#define	MNTFS_ALIAS_DATASET(zonep, vfsp, mntpt)				\
	((zonep) != global_zone &&					\
	(!((vfsp)->vfs_flag & VFS_NOZONEMOD) ||				\
	((mntpt) != NULL && (mntpt)[0] != '\0' &&			\
	strcmp(ZONE_PATH_TRANSLATE((mntpt), (zonep)), "/") == 0)) &&	\
	strcmp(vfssw[(vfsp)->vfs_fstype].vsw_name, "zfs") == 0)

/* Constants for identifying the newer of two database elements. */
...
static int
mntfs_newer(timespec_t *a, timespec_t *b)
{
...
/*
 * The following five functions generate the respective fields of a vfs_t's
 * entry in /etc/mnttab. In each case the resulting string is written into a
 * buffer of a given size and the caller is notified of the number of bytes
 * consumed in the process. If the buffer is not large enough then the function
 * returns ENOSPC in the expectation that the caller will resize it and repeat
 * the request.
 */
static int
mntfs_construct_special(vfs_t *vfsp, zone_t *zonep, char *bufp,
    size_t buf_size, size_t *consumedp)
{
	const char	*resource = refstr_value(vfsp->vfs_resource);
	const char	*mntpt = refstr_value(vfsp->vfs_mntpt);
	const char	*special;

	if (resource != NULL && resource[0] != '\0') {
		if (resource[0] != '/') {
			/*
			 * If the resource is not an absolute path then it will
			 * be either a descriptive name, e.g. "proc", or a ZFS
			 * dataset; the latter may be aliased.
			 */
			if (MNTFS_ALIAS_DATASET(zonep, vfsp, mntpt)) {
				int alias_length;

				if ((alias_length = zone_dataset_alias(zonep,
				    resource, bufp, buf_size)) + 2 > buf_size) {
					return (ENOSPC);
				}

				*(bufp + alias_length) = '\t';
				*(bufp + alias_length + 1) = '\0';
				*consumedp = alias_length + 1;
			...
			/* Try to use the mount point as the resource. */
			special = (mntpt != NULL && mntpt[0] != '\0') ?
			    ZONE_PATH_TRANSLATE(mntpt, zonep) : "-";
		} else {
			special = ZONE_PATH_TRANSLATE(resource, zonep);
		}
	} else {
		special = "-";
	}

	*consumedp = snprintf(bufp, buf_size, "%s\t", special);
	return ((*consumedp + 1 > buf_size) ? ENOSPC : 0);
}

/* See the comments for mntfs_construct_special(), above. */
static int
mntfs_construct_mountp(vfs_t *vfsp, zone_t *zonep, char *bufp, size_t buf_size,
    size_t *consumedp)
{
	const char *mntpt = refstr_value(vfsp->vfs_mntpt);
	const char *mountp;
	...
	/*
	 * We know the mount point should be visible from within the zone
	 * because otherwise it wouldn't be on the zone's vfs list.
	 */
	mountp = (mntpt != NULL && mntpt[0] != '\0') ?
	    ZONE_PATH_TRANSLATE(mntpt, zonep) : "-";

	*consumedp = snprintf(bufp, buf_size, "%s\t", mountp);
	return ((*consumedp + 1 > buf_size) ? ENOSPC : 0);
}
...
/* ARGSUSED */
static int
mntfs_construct_fstype(vfs_t *vfsp, zone_t *zonep, char *bufp, size_t buf_size,
    size_t *consumedp)
{
	*consumedp =
	    snprintf(bufp, buf_size, "%s\t", vfssw[vfsp->vfs_fstype].vsw_name);
	return ((*consumedp + 1 > buf_size) ? ENOSPC : 0);
}
...
/* ARGSUSED */
static int
mntfs_construct_mntopts(vfs_t *vfsp, zone_t *unused, char *bufp,
    size_t buf_size, size_t *consumedp)
{
	char		*bufp_start = bufp;
	int		i;
	int		len;
	boolean_t	optinbuf = B_FALSE;
	zone_t		*vfszone;

	for (i = 0; i < vfsp->vfs_mntopts.mo_count; i++) {
		mntopt_t *mop = &vfsp->vfs_mntopts.mo_list[i];

		if (mop->mo_flags & MO_NODISPLAY)
			continue;
		if (mop->mo_flags & MO_SET) {
			if ((len = snprintf(bufp, buf_size, "%s%s%s%s",
			    (optinbuf == B_TRUE) ? "," : "",
			    mop->mo_name,
			    (mop->mo_arg != NULL) ? "=" : "",
			    (mop->mo_arg != NULL) ? mop->mo_arg : "")) + 1 >
			    buf_size)
				return (ENOSPC);
			bufp += len;
			buf_size -= len;
	...
	vfszone = VFTOZONE(vfsp);
	if (vfszone != global_zone) {
		if ((len = snprintf(bufp, buf_size, "%szone=%s",
		    (optinbuf == B_TRUE) ? "," : "",
		    vfszone->zone_name)) + 1 > buf_size)
			return (ENOSPC);
		bufp += len;
		buf_size -= len;
		optinbuf = B_TRUE;
	...
	if (vfsp->vfs_flag & VFS_NOZONEMOD) {
		if ((len = snprintf(bufp, buf_size, "%snozonemod",
		    (optinbuf == B_TRUE) ? "," : "")) + 1 > buf_size)
			return (ENOSPC);
		bufp += len;
		buf_size -= len;
		optinbuf = B_TRUE;
	...
	if (vfsp->vfs_shrzone_refcnt > 0 &&
	    vfsp->vfs_shrzone_id != GLOBAL_ZONEID) {
		if ((len = snprintf(bufp, buf_size, "%s%s=%d",
		    (optinbuf == B_TRUE) ? "," : "",
		    MNTOPT_SHRZONE, vfsp->vfs_shrzone_id)) + 1 > buf_size)
			return (ENOSPC);
		bufp += len;
		buf_size -= len;
		optinbuf = B_TRUE;
	...
	if (mntfs_enabledev) {
		...
		if ((len = snprintf(bufp, buf_size, "%sdev=%x",
		    (optinbuf == B_TRUE) ? "," : "",
		    odev)) + 1 > buf_size)
			return (ENOSPC);
		bufp += len;
	...
	if ((len = snprintf(bufp, buf_size, "%s\t",
	    (optinbuf == B_FALSE) ? "-" : "")) + 1 > buf_size)
		return (ENOSPC);
	bufp += len;

	if (consumedp != NULL)
		*consumedp = bufp - bufp_start;
	return (0);
}
...
/* ARGSUSED */
static int
mntfs_construct_time(vfs_t *vfsp, zone_t *zonep, char *bufp, size_t buf_size,
    size_t *consumedp)
{
	*consumedp = snprintf(bufp, buf_size, "%ld\n", vfsp->vfs_mtime);
	return ((*consumedp + 1 > buf_size) ? ENOSPC : 0);
}

/*
 * mntfs_populate_text() constructs the /etc/mnttab entry for a given vfs_t and
 * stores it in a supplied database element. The text is written into a buffer
 * ready for use by read(2); the offsets of each of the five fields are also
 * recorded in a struct extmnttab ready for use by getmntent/getextmntent(3C).
 *
 * If the buffer within the given database element proves to be too small then
 * the function returns ENOSPC in the expectation that the caller will resize it
    ...
    size_t buf_size)
{
	...
	int	(*func[5])(vfs_t *, zone_t *, char *, size_t, size_t *) = {
		mntfs_construct_special,
		mntfs_construct_mountp,
		mntfs_construct_fstype,
		mntfs_construct_mntopts,
		mntfs_construct_time
	};
	int	f;
	char	*bufp = elemp->mnte_text;
	size_t	remaining = buf_size;
	size_t	consumed;

	for (f = 0; f < 5; f++) {
		*member[f] = (char *)(buf_size - remaining);
		if ((*func[f])(vfsp, zonep, bufp, remaining, &consumed) != 0)
			return (ENOSPC);
	...
	elemp->mnte_text_size = bufp - elemp->mnte_text;
...
mntfs_elem_in_range(mntsnap_t *snapp, mntelem_t *elemp)
{
	 ...
	 */
	if (mntfs_newer(btimep, stimep) == MNTFS_SECOND &&
	    (MNTFS_ELEM_IS_ALIVE(elemp) ||
	    mntfs_newer(stimep, dtimep) == MNTFS_SECOND))
		return (1);
	else
		return (0);
}
...
mntfs_snapshot(mntnode_t *mnp, mntsnap_t *snapp)
{
	 ...
	 */
	if (snapp->mnts_nmnts) {
		if (mntfs_newer(&last_mtime, &snapp->mnts_last_mtime) ==
		    MNTFS_NEITHER) {
			/*
	...
	if (is_global_zone) {
		...
		if (firstvfsp != NULL)
			mntpt = refstr_value(firstvfsp->vfs_mntpt);
		/*
		 * If there isn't already a vfs_t for "/" then we create a
		...
		if (firstvfsp == NULL ||
			 ...
			 * and resource. However, if the zone's root is a zfs
			 * dataset then we use the dataset name as the resource.
			 *
	...
	for (vfsp = firstvfsp;;
		...
		if ((vfsp->vfs_flag & VFS_NOMNTTAB) == 0 || show_hidden) {
			...
			for (; elemp; prevp = elemp, elemp = elemp->mnte_next) {
				...
				order = mntfs_newer(&elemp->mnte_vfs_ctime,
				    &vfsp->vfs_hrctime);
			...
			while (mntfs_populate_text(vfsp, zonep, tempelemp,
			    entry_length) == ENOSPC) {
				kmem_free(tempelemp->mnte_text, entry_length);
				entry_length *= 2;
				tempelemp->mnte_text =
				    kmem_alloc(entry_length, KM_SLEEP);
			}
			if (elemp == NULL) {
				/*
...
/*
 * mntfs_getmntopts() is a public function that provides a buffer, along with
 * its size, containing a string representation of a vfs_t's mntopts. It is the
 * caller's responsibility to free the buffer.
 */
void
mntfs_getmntopts(struct vfs *vfsp, char **bufpp, size_t *buf_sizep)
{
	vfs_list_read_lock();
	if ((*bufpp = kmem_alloc(MAX_MNTOPT_STR, KM_NOSLEEP)) != NULL) {
		(void) mntfs_construct_mntopts(vfsp, NULL, *bufpp,
		    MAX_MNTOPT_STR, NULL);
		*buf_sizep = MAX_MNTOPT_STR;
	}
	vfs_list_unlock();
}
	...
	caller_context_t *ct)
{
	...
	if (mask & AT_SIZE|AT_NBLOCKS) {
		...
		} else if (mntfs_newer(&mtime, &old_mtime) == MNTFS_NEITHER) {
			/*
	...
	caller_context_t *ct)
{
	...
	if (mntfs_newer(&mnp->mnt_ioctl.mnts_last_mtime,

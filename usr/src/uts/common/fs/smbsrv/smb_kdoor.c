...
static boolean_t smb_kdoor_chkhdr(smb_doorarg_t *);
...
smb_kdoor_close(void)
{
	...
	if (smb_kdoor_hd != NULL) {
		while (smb_kdoor_ncall > 0)
			(void) cv_wait_sig(&smb_kdoor_cv, &smb_kdoor_mutex);
 ...
 *
 * Returns the success/failure of the doorcall mechanism:
 *  NT_STATUS_SUCCESS
 *  NT_STATUS_INTERNAL_ERROR
 * The status of the called function is conveyed within its response data.
 */
uint32_t
    ...
    void *rsp_data, xdrproc_t rsp_xdr)
{
	...
	return ((rc == 0) ? 0 : NT_STATUS_INTERNAL_ERROR);
}
...
smb_kdoor_encode(smb_doorarg_t *da)
{
	...
	len = xdr_sizeof(smb_doorhdr_xdr, &da->da_req_hdr);
...
smb_kdoor_decode(smb_doorarg_t *da)
{
	...
	if (!smb_kdoor_chkhdr(da)) {
		xdr_free(smb_doorhdr_xdr, (char *)&da->da_rsp_hdr);
		xdr_destroy(&xdrs);
		return (-1);
	}

	if (da->da_rsp_hdr.dh_datalen != 0 && da->da_rsp_xdr != NULL) {
...
{
	smb_doorhdr_t	*hdr = &da->da_req_hdr;
...
static boolean_t
smb_kdoor_chkhdr(smb_doorarg_t *da)
{

...
static uint32_t smb_check_create_options(smb_request_t *);
...
smb_common_open(smb_request_t *sr)
{
	...
	status = smb_check_create_options(sr);
	...
	if (status == NT_STATUS_SHARING_VIOLATION) {
		smb_errcode_seterror(NT_STATUS_SHARING_VIOLATION,
		    ERRDOS, ERROR_SHARING_VIOLATION);
	}

	if (status == NT_STATUS_NO_SUCH_FILE) {
		smb_errcode_seterror(NT_STATUS_OBJECT_NAME_NOT_FOUND,
...
/*
 * smb_check_create_options
 *
 * We do not check the create_options for STYPE_IPC, since we
 * ignore create_options for pipes.
 *
 * FILE_OPEN_BY_FILE_ID is not supported and, according to
 * observed Windows behavior, should result in a status code
 * of NT_STATUS_NOT_SUPPORTED.
 *
 * According to observed Windows behavior, the following
 * create_options should result in a status code of
 ...
 */
static uint32_t
smb_check_create_options(smb_request_t *sr)
{
	uint32_t create_options = sr->arg.open.create_options;
	uint32_t invalid_mask;

	if (STYPE_ISIPC(sr->tid_tree->t_res_type & STYPE_MASK))
		return (NT_STATUS_SUCCESS);

	if ((create_options & FILE_OPEN_BY_FILE_ID) != 0) {
		smb_errcode_seterror(NT_STATUS_NOT_SUPPORTED,
		    ERRDOS, ERROR_NOT_SUPPORTED);
		return (NT_STATUS_NOT_SUPPORTED);
	}

	invalid_mask = (uint32_t)(~FILE_VALID_OPTION_FLAGS) |
	    FILE_SYNCHRONOUS_IO_ALERT |
	    FILE_SYNCHRONOUS_IO_NONALERT |
	    FILE_OPFILTER;

	if ((create_options & invalid_mask) != 0) {
...
smb_open_subr(smb_request_t *sr)
{
	...
	if (sr->session->s_file_cnt >= SMB_SESSION_OFILE_MAX) {
		...
		smb_errcode_seterror(NT_STATUS_TOO_MANY_OPENED_FILES,
	...
	case STYPE_DISKTREE:
		...
		break;
	case STYPE_PRINTQ:
		op->ftype = SMB_FTYPE_PRINTER;
		break;

	case STYPE_IPC:
		op->ftype = SMB_FTYPE_MESG_PIPE;
		op->create_options = 0;
		of = smb_ofile_open(sr->tid_tree, NULL, sr->sr_pid, op,
		    SMB_UNIQ_FID());
		if (of == NULL) {
			status = smb_errcode_getstatus();
		} else {
			sr->smb_fid = of->f_fid;
			sr->fid_ofile = of;
		}
	...
	if (!smb_pathname_validate(pn))
		return (smb_errcode_getstatus());

	if (strlen(pn->pn_path) >= MAXPATHLEN) {
		smb_errcode_seterror(0, ERRSRV, ERRfilespecs);
		return (NT_STATUS_NAME_TOO_LONG);
	}

	if (is_dir) {
		if (!smb_validate_dirname(pn))
			return (smb_errcode_getstatus());
	} else {
		if (!smb_validate_object_name(pn))
			return (smb_errcode_getstatus());
	}
	...
	/*
	 * If the access mask has only STANDARD_DELETE set
	 * (ignore FILE_READ_ATTRIBUTES), then assume that
	 * this is a request to delete the link (if a link)
	 * and do not follow links.  Otherwise, follow
	 * the link to the target.
	 */
	if ((op->desired_access & ~FILE_READ_ATTRIBUTES) == STANDARD_DELETE)
	...
	if (last_comp_found) {
		...
		if ((sr->sr_flags2 & SMB_FLAGS2_DFS) &&
		    (smb_node_is_dfslink(node))) {
			smb_node_release(node);
			smb_node_release(dnode);
			smb_errcode_seterror(NT_STATUS_PATH_NOT_COVERED,
			    ERRSRV, ERRbadpath);
		...
		if (smb_node_is_dir(node)) {
			...
			if ((op->create_options & FILE_DIRECTORY_FILE) ||
				...
				smb_node_release(node);
				smb_node_release(dnode);
				smb_errcode_seterror(NT_STATUS_NOT_A_DIRECTORY,
		...
		if (op->create_disposition == FILE_CREATE) {
			smb_node_release(node);
			smb_node_release(dnode);
			smb_errcode_seterror(NT_STATUS_OBJECT_NAME_COLLISION,
		...
		if (pn->pn_fname && smb_is_invalid_filename(pn->pn_fname)) {
			smb_node_release(dnode);
			smb_errcode_seterror(NT_STATUS_OBJECT_NAME_INVALID,
	...
	of = smb_ofile_open(sr->tid_tree, node, sr->sr_pid, op,
	    uniq_fid);
	if (of == NULL)
		status = smb_errcode_getstatus();

    ...
    smb_attr_t *attr, smb_node_t **ret_snode, smb_fssd_t *fs_sd)
{
	...
	if (smb_tree_has_feature(sr->tid_tree, SMB_TREE_ACLONCREATE)) {
		if (fs_sd->sd_secinfo & SMB_ACL_SECINFO) {
			...
			rc = smb_fsacl_to_vsa(acl, &vsecattr);
...
smb_fsop_remove_streams(smb_request_t *sr, cred_t *cr, smb_node_t *fnode)
{
	...
	if (SMB_TREE_CONTAINS_NODE(sr, fnode) == 0) {
		smb_errcode_seterrno(EACCES);
		return (-1);
	}

	if (SMB_TREE_IS_READONLY(sr)) {
		smb_errcode_seterrno(EROFS);
		return (-1);
	}
    ...
    char *to_name)
{
	 ...
	 *   rename is permitted - requires STANDARD_DELETE access.
	 ...
	 */
	if (sr && sr->fid_ofile) {
		rc = smb_ofile_access(sr->fid_ofile, cr, STANDARD_DELETE);
 ...
 * Please document any direct call to explain the reason
 * for avoiding this wrapper.
 * Setting attributes requires the following access permissions:
 ...
 *
 * If the requested access (faccess) is 0, NT_STATUS_SUCCESS is
 * returned. This scenario could occur during file open. Windows
 * behavior is that when a client opens a file requesting 0 access
 * the request will succeed. The granted_access mask on the ofile
 * will take care of ensuring that ACCESS_DENIED is returned for
 * subsequent requests on that fid for which insufficient access
    ...
    uint32_t faccess)
{
	...
	if (SMB_TREE_IS_READONLY(sr)) {
		if (faccess & (FILE_WRITE_DATA|FILE_APPEND_DATA|
		    ...
		    STANDARD_DELETE|WRITE_DAC|WRITE_OWNER)) {
			return (NT_STATUS_ACCESS_DENIED);
		}
	}

	if (smb_node_is_reparse(snode) && (faccess & STANDARD_DELETE))
		return (NT_STATUS_ACCESS_DENIED);
 ...
 * or not).
 *
 * Delete-on-close flag SMB_IGNORE_DOC:
 * Path-based client requested operations should fail if the node has
 * delete-on-close set, so by default smb_fsop_lookup will return an error
 * (ETOOMANYREFS => NT_STATUS_DELETE_PENDING) if the looked up node (ret_node)
 * has delete-on-close set. SMB_IGNORE_DOC allows smb_fsop_lookup to be used
 * internally to look up a node regardless of whether or not the node has
 * delete-on-close set.
 */
int
smb_fsop_lookup(
    ...
    smb_node_t	**ret_snode)
{
	...
	if (rc == 0) {
		if (((flags & SMB_IGNORE_DOC) == 0) &&
		    (smb_node_delete_pending(*ret_snode))) {
			smb_node_release(*ret_snode);
			rc = ETOOMANYREFS;
		}
	}
    ...
    cred_t *cr)
{
	 ...
	 *
	 * 2) The lock rolls over (start + length < start). Solaris will assert

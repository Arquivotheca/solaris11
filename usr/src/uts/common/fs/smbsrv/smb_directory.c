...
smb_com_delete_directory(smb_request_t *sr)
{
	...
	if (!smb_pathname_validate(&fqi->fq_path) ||
	    !smb_validate_dirname(&fqi->fq_path)) {
		return (SDRC_ERROR);
	}
	...
	if ((fqi->fq_fattr.sa_dosattr & FILE_ATTRIBUTE_READONLY) ||
	    (smb_fsop_access(sr, sr->user_cr, fqi->fq_fnode, STANDARD_DELETE)
	    != NT_STATUS_SUCCESS)) {
		smb_errcode_seterror(NT_STATUS_CANNOT_DELETE,
		    ERRDOS, ERROR_ACCESS_DENIED);
		smb_node_release(fqi->fq_dnode);
		smb_node_release(fqi->fq_fnode);
		return (SDRC_ERROR);
	...
	if (rc != 0) {
		if (rc == EEXIST)
			smb_errcode_seterror(NT_STATUS_DIRECTORY_NOT_EMPTY,

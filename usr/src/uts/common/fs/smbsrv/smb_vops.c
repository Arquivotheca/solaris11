    ...
    int flags, cred_t *cr, vsecattr_t *vsap)
{
	...
	/* size cannot be set via VOP_CREATE. Use explicit VOP_SETATTR. */
	if ((error == 0) && (size != 0)) {
		attr->sa_vattr.va_mask = AT_SIZE;
		attr->sa_vattr.va_size = size;
		(void) VOP_SETATTR(*vpp, &attr->sa_vattr, option_flags,
		    kcred, &smb_ct);
	}
...
smb_vop_acl_write(vnode_t *vp, acl_t *aclp, int flags, cred_t *cr)
{
	...
	error = smb_fsacl_to_vsa(aclp, &vsecattr);

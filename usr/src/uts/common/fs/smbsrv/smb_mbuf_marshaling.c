...
static int mbc_marshal_put_header(mbuf_chain_t *, smb_hdr_t *);
...
static int mbc_marshal_get_header(mbuf_chain_t *, smb_hdr_t *);
...
smb_mbc_vdecodef(mbuf_chain_t *mbc, char *fmt, va_list ap)
{
	...
	while ((c = *fmt++) != 0) {
		...
		case '%':
			...
			unicode = sr->sr_flags2 & SMB_FLAGS2_UNICODE;
			break;
...
unicode_translation:
		...
		case 'H':
			if (mbc_marshal_get_header(mbc,
			    va_arg(ap, smb_hdr_t *)) != 0)
				return (-1);
			break;
...
static int
mbc_marshal_get_header(mbuf_chain_t *mbc, smb_hdr_t *hh)
{
	int	i;
	int	rc;

	ASSERT(MBC_LENGTH(mbc) >= SMB_HEADER_LEN);

	rc = mbc_marshal_get_long(mbc, &hh->hd_protocol);
	rc |= mbc_marshal_get_char(mbc, &hh->hd_command);
	rc |= mbc_marshal_get_long(mbc, &hh->hd_status);
	rc |= mbc_marshal_get_char(mbc, &hh->hd_flags);
	rc |= mbc_marshal_get_short(mbc, &hh->hd_flags2);
	rc |= mbc_marshal_get_short(mbc, &hh->hd_pidhigh);

	for (i = 0; i < SMB_SIG_SIZE; i++)
		rc |= mbc_marshal_get_char(mbc, &hh->hd_signature[i]);

	rc |= mbc_marshal_get_skip(mbc, 2);
	rc |= mbc_marshal_get_short(mbc, &hh->hd_tid);
	rc |= mbc_marshal_get_short(mbc, &hh->hd_pidlow);
	rc |= mbc_marshal_get_short(mbc, &hh->hd_uid);
	rc |= mbc_marshal_get_short(mbc, &hh->hd_mid);

	if ((rc == 0) && (hh->hd_protocol == SMB_PROTOCOL_MAGIC))
		return (0);
...
static int
mbc_marshal_put_header(mbuf_chain_t *mbc, smb_hdr_t *hh)
{
	int	i;
	int	rc;

	ASSERT(mbc_space(mbc) >= SMB_HEADER_LEN);

	rc = mbc_marshal_put_long(mbc, hh->hd_protocol);
	rc |= mbc_marshal_put_char(mbc, hh->hd_command);
	rc |= mbc_marshal_put_long(mbc, hh->hd_status);
	rc |= mbc_marshal_put_char(mbc, hh->hd_flags);
	rc |= mbc_marshal_put_short(mbc, hh->hd_flags2);
	rc |= mbc_marshal_put_short(mbc, hh->hd_pidhigh);

	for (i = 0; i < SMB_SIG_SIZE; i++)
		rc |= mbc_marshal_put_char(mbc, hh->hd_signature[i]);

	rc |= mbc_marshal_put_short(mbc, 0);
	rc |= mbc_marshal_put_short(mbc, hh->hd_tid);
	rc |= mbc_marshal_put_short(mbc, hh->hd_pidlow);
	rc |= mbc_marshal_put_short(mbc, hh->hd_uid);
	rc |= (mbc_marshal_put_short(mbc, hh->hd_mid));

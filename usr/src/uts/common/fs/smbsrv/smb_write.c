...
smb_com_write(smb_request_t *sr)
{
	...
	if (rc != 0) {
		if (smb_errcode_getstatus() != NT_STATUS_FILE_LOCK_CONFLICT)
 ...
 * closed.
 *
 * If Count is 0, the file is truncated (or extended) to offset but
...
smb_pre_write_andx(smb_request_t *sr)
{
	...
	if (param->rw_dsoff < dsoff_adjust)
		rc = -1;
	else
...
smb_com_write_andx(smb_request_t *sr)
{
	...
	if (SMB_WRMODE_IS_STABLE(param->rw_mode) &&
		...
		smb_errcode_seterror(0, ERRSRV, ERRaccess);
		return (SDRC_ERROR);
	}
...
smb_write_truncate(smb_request_t *sr, smb_rw_param_t *param)
{
	...
	if (!smb_node_is_dir(node)) {
		...
		if (status != NT_STATUS_SUCCESS) {
			...
			smb_errcode_seterror(NT_STATUS_FILE_LOCK_CONFLICT,
			    ERRDOS, ERROR_LOCK_VIOLATION);
			return (EACCES);
		}
	}

...
smb_fsacl_split(acl_t *zacl, acl_t **dacl, acl_t **sacl, int which_acl)
{
	...
	int trivial_dir_acl = ACL_IS_TRIVIAL | ACL_IS_DIR;
	int i;
	...
	*dacl = *sacl = NULL;
	...
	for (i = 0, zace = zacl->acl_aclp; i < zacl->acl_cnt; zace++, i++) {
		if (get_dacl && smb_ace_is_access(zace->a_type)) {
			*access_ace = *zace;
			/*
			 * Overcome the issue that an absolute mode chmod
			 * on a directory will strip the inheritance flags.
			 */
			if ((zacl->acl_flags & trivial_dir_acl) ==
			    trivial_dir_acl) {
				access_ace->a_flags |=
 ...
 *
 * The allocated memory for returned acl_t should be freed by
 * calling acl_free().
 */
acl_t *
smb_fsacl_from_vsa(vsecattr_t *vsecattr, acl_type_t acl_type)
{
	...
	case ACE_T:
		...
		acl_info->acl_aclp = kmem_alloc(vsecattr->vsa_aclentsz,
		    KM_SLEEP);
		(void) memcpy(acl_info->acl_aclp, vsecattr->vsa_aclentp,
		    vsecattr->vsa_aclentsz);
 ...
 * in the field vsa_aclentsz of the structure vsecattr.
 */
int
smb_fsacl_to_vsa(acl_t *acl_info, vsecattr_t *vsecattr)
{
	int		error = 0;
	int		numacls;
	aclent_t	*aclp;
	...
	case ACLENT_T:
		...
		vsecattr->vsa_aclentsz = numacls * sizeof (aclent_t);
		vsecattr->vsa_aclentp = kmem_alloc(vsecattr->vsa_aclentsz,
		    KM_SLEEP);

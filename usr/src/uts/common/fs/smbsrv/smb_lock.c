    ...
    uint32_t		locktype)
{
	...
	/*
	 * Do not insert into the SMB node's lock list unless the
	 * posix lock is successful, and the ofile is still open.
	 * Do not hold the ofile mutex across the lock/unlock.
	 */
	if (result == NT_STATUS_SUCCESS) {
		if (smb_fsop_frlock(node, lock, B_FALSE, sr->user_cr)) {
			...
			if (file->f_state == SMB_OFILE_STATE_OPEN) {
...
void
smb_lock_range_error(uint32_t status32)
{
	uint16_t errcode;
	...
	smb_errcode_seterror(status32, ERRDOS, errcode);
}
    ...
    uint64_t length, boolean_t will_write)
{
	...
	if ((smb_lock_range_access(sr, node, start, length,
	    will_write) == NT_STATUS_SUCCESS) &&
	    (nbl_svmand(node->vp, kcred, &svmand) == 0) &&
	    (nbl_lock_conflict(node->vp, (will_write) ? NBL_WRITE : NBL_READ,
	    start, length, svmand, &smb_ct) == 0))
		return (NT_STATUS_SUCCESS);

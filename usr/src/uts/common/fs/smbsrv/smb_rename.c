...
static void smb_rename_set_error(int);
...
smb_com_rename(smb_request_t *sr)
{
	...
	if (!smb_pathname_validate(src_pn) ||
	    !smb_pathname_validate(dst_pn)) {
		return (SDRC_ERROR);
	}
...
smb_trans2_rename(smb_request_t *sr, smb_node_t *node, char *fname, int flags)
{
	...
	/* construct and validate the dst pathname */
	...
	if (!smb_pathname_validate(dst_pn))
		return (-1);
...
smb_common_rename(smb_request_t *sr, smb_fqi_t *src_fqi, smb_fqi_t *dst_fqi)
{
	...
	if (src_fqi->fq_fnode) {
		if (smb_node_delete_pending(src_fqi->fq_fnode))
			return (EACCES);
	...
	if ((rc == 0) &&
		...
		if (smb_tree_has_feature(sr->tid_tree,
			...
			rc = smb_fsop_lookup(sr, sr->user_cr,
			    SMB_CASE_SENSITIVE | SMB_IGNORE_DOC,
			    tnode, dst_dnode, new_name,
			    &dst_fqi->fq_fnode);
	...
	if (dst_fqi->fq_fnode) {
		...
		if (smb_node_delete_pending(dst_fnode)) {
			smb_rename_release_src(sr);
			smb_node_release(dst_fnode);
			smb_node_release(dst_dnode);
			return (EACCES);
 ...
 */
static void
smb_rename_set_error(int errnum)
{
	static struct {
		int errnum;
		uint16_t errcode;
	...
	for (i = 0; i < sizeof (rc_map)/sizeof (rc_map[0]); ++i) {
		if (rc_map[i].errnum == errnum) {
			smb_errcode_seterror(rc_map[i].status32,
			    ERRDOS, rc_map[i].errcode);
			return;
		}
	}

	smb_errcode_seterrno(errnum);
}

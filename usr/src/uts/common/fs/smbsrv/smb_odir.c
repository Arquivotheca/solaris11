...
smb_odir_openat(smb_request_t *sr, smb_node_t *unode)
{
	...
	VN_RELE(xattr_dvp);
	if (xattr_dnode == NULL) {
		smb_errcode_seterror(NT_STATUS_NO_MEMORY,
    ...
    smb_streaminfo_t *sinfo, boolean_t *eof)
{
	...
	for (;;) {
		...
		rc = smb_fsop_lookup(sr, od->d_cred, SMB_IGNORE_DOC,
    ...
    smb_fileinfo_t *fileinfo)
{
	...
	rc = smb_fsop_lookup(sr, od->d_cred, SMB_IGNORE_DOC,
	    od->d_tree->t_snode, od->d_dnode, od->d_pattern, &fnode);
	if (rc != 0)
		return (rc);
    ...
    smb_odirent_t *odirent, smb_fileinfo_t *fileinfo)
{
	...
	rc = smb_fsop_lookup(sr, od->d_cred,
...
{
	int rc;
	uint32_t flags = SMB_FOLLOW_LINKS | SMB_CASE_SENSITIVE | SMB_IGNORE_DOC;

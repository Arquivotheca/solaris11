...
static void smb_server_disconnect_share(smb_llist_t *, uint64_t);
...
static smb_llist_t	smb_servers;	/* List of smb_server structures. */
static uint_t		smb_thkey = 0;	/* SMB thread specific data key */
...
smb_server_svc_init(void)
{
	...
	tsd_create(&smb_thkey, NULL);

	while (rc == 0) {
		if ((rc = smb_oemcpg_init()) != 0)
			continue;
		if ((rc = smb_shrmgr_init()) != 0)
			continue;
		if ((rc = smb_mbc_init()) != 0)
			continue;
		if ((rc = smb_vop_init()) != 0)
			continue;
		if ((rc = smb_node_init()) != 0)
			continue;
		if ((rc = smb_oplock_init()) != 0)
			continue;
		if ((rc = smb_fem_init()) != 0)
			continue;
		if ((rc = smb_fcn_init()) != 0)
			continue;
		if ((rc = smb_net_init()) != 0)
			continue;
		smb_llist_init();
...
smb_server_enum(smb_ioc_svcenum_t *ioc)
{
	...
	if (svcenum->se_type == SMB_SVCENUM_TYPE_SHARE) {
		smb_shrmgr_share_enumsvc(svcenum);
		smb_server_release(sv);
		return (0);
	}
...
int
smb_server_unshare(uint64_t share_id)
{
	smb_server_t	*sv;
	smb_llist_t	*ll;
	int		rc;
	...
	ll = &sv->sv_nbt_daemon.ld_session_list;
	smb_server_disconnect_share(ll, share_id);
 ...
 */
static void
smb_server_disconnect_share(smb_llist_t *ll, uint64_t share_id)
{
	...
	while (session) {
		...
		case SMB_SESSION_STATE_WRITE_RAW_ACTIVE:
			smb_session_disconnect_share(session, share_id);
			break;
		default:
			break;
		}
...
void
smb_server_settsd(smb_thsd_t *thsd)
{
	if (thsd != NULL) {
		bzero(thsd, sizeof (*thsd));
		thsd->tsd_magic = SMB_THSD_MAGIC;
	}
	VERIFY(tsd_set(smb_thkey, thsd) == 0);
}

smb_thsd_t *
smb_server_gettsd(void)
{
	smb_thsd_t	*thsd;

	thsd = tsd_get(smb_thkey);
	SMB_THSD_VALID(thsd);
	return (thsd);
}
    ...
    const char *client, const char *name)
{
	...
	while (sn != NULL) {
		...
		count += smb_session_logoff(sn, name);
		sn = smb_llist_next(ll, sn);
	}
...
smb_server_store_cfg(smb_server_t *sv, smb_ioc_cfg_t *ioc)
{
	...
	/* limit maxworkers and maxconnections to max taskq threads */
	sv->sv_cfg.skc_maxworkers = ioc->maxworkers;
	if (sv->sv_cfg.skc_maxworkers > INT_MAX)
		sv->sv_cfg.skc_maxworkers = INT_MAX;
	sv->sv_cfg.skc_maxconnections = ioc->maxconnections;
	if (sv->sv_cfg.skc_maxconnections > INT_MAX)
		sv->sv_cfg.skc_maxconnections = INT_MAX;

	sv->sv_cfg.skc_keepalive = ioc->keepalive;
	sv->sv_cfg.skc_restrict_anon = ioc->restrict_anon;
	sv->sv_cfg.skc_enforce_vczero = ioc->enforce_vczero;
	...
	(void) strlcpy(sv->sv_cfg.skc_native_os, ioc->native_os,
	    sizeof (sv->sv_cfg.skc_native_os));
	(void) strlcpy(sv->sv_cfg.skc_native_lm, ioc->native_lm,
	    sizeof (sv->sv_cfg.skc_native_lm));
	sv->sv_cfg.skc_extsec_enable = ioc->extsec_enable;
	bcopy(ioc->machine_guid, sv->sv_cfg.skc_machine_guid,
	    sizeof (sv->sv_cfg.skc_machine_guid));
}
...
void
smb_event_signal(uint32_t txid)
{
	smb_server_t    *sv;

 ...
 * SMB_FILE_ATTRIBUTE_TAG_INFORMATION - not valid for pipes
 *
...
static int smb_query_pipeinfo(smb_request_t *, smb_opipe_t *,
    uint16_t, smb_queryinfo_t *);
static boolean_t smb_query_pipe_valid_infolev(uint16_t);
...
smb_query_by_fid(smb_request_t *sr, smb_xa_t *xa, uint16_t infolev)
{
	...
	if (infolev == SMB_INFO_IS_NAME_VALID) {
		smb_errcode_seterror(0, ERRDOS, ERROR_INVALID_LEVEL);
		smbsr_release_file(sr);
		return (-1);
	}

	if ((SMB_FTYPE_IS_PIPE(sr->fid_ofile->f_ftype)) &&
	    (!smb_query_pipe_valid_infolev(infolev))) {
		smbsr_release_file(sr);
		return (-1);
	}
...
smb_query_by_path(smb_request_t *sr, smb_xa_t *xa, uint16_t infolev)
{
	...
	if ((sr->sr_flags2 & SMB_FLAGS2_DFS) && smb_node_is_dfslink(node)) {
		smb_errcode_seterror(NT_STATUS_PATH_NOT_COVERED, ERRSRV,
		    ERRbadpath);
		kmem_free(qinfo, sizeof (smb_queryinfo_t));
 ...
 * smb_query_fileinfo
 *
 * Populate smb_queryinfo_t structure for SMB_FTYPE_DISK/PRINTER
    ...
    smb_queryinfo_t *qinfo)
{
	...
	qinfo->qi_node = node;
	qinfo->qi_delete_on_close = smb_node_delete_pending(node);
 ...
 */
static boolean_t
smb_query_pipe_valid_infolev(uint16_t infolev)
{
	switch (infolev) {
	case SMB_INFO_QUERY_ALL_EAS:
		smb_errcode_seterror(NT_STATUS_ACCESS_DENIED,

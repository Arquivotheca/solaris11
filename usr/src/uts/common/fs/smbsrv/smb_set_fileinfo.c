...
smb_set_fileinfo(smb_request_t *sr, smb_setinfo_t *sinfo)
{
	...
	smb_errcode_seterror(NT_STATUS_INVALID_INFO_CLASS,
 ...
 * File should have been opened with STANDARD_DELETE access otherwise
...
smb_set_disposition_info(smb_request_t *sr, smb_setinfo_t *sinfo)
{
	...
	if ((sr->fid_ofile == NULL) ||
	    !(smb_ofile_granted_access(sr->fid_ofile) & STANDARD_DELETE)) {
		smb_errcode_seterror(NT_STATUS_ACCESS_DENIED,
		    ERRDOS, ERROR_ACCESS_DENIED);
		return (-1);
	}
	...
	if (mark_delete) {
		...
		rc = smb_node_set_delete_on_close(sinfo->si_node,
		    sr->user_cr, flags);
		if (rc != 0) {
			if (rc == ENOTEMPTY) {

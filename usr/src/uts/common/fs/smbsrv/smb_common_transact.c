...
smb_com_transaction(smb_request_t *sr)
{
	...
	xa = smb_xa_create(sr->session, sr, tpscnt, tdscnt, mprcnt, mdrcnt,
	    msrcnt, suwcnt);
	if (xa == NULL) {
		smb_errcode_seterror(0, ERRSRV, ERRnoroom);
		return (SDRC_ERROR);
	}
	...
	if (MBC_SHADOW_CHAIN(&xa->req_setup_mb, &sr->smb_vwv,
		...
		smb_xa_rele(sr->session, xa);
		smb_errcode_seterror(0, ERRDOS, ERRbadformat);
		return (SDRC_ERROR);
	}
	...
	if (smb_xa_open(xa)) {
		smb_xa_rele(sr->session, xa);
		smb_errcode_seterror(0, ERRDOS, ERRsrverror);
		return (SDRC_ERROR);
	}
	sr->sr_xa = xa;
...
smb_com_transaction_secondary(smb_request_t *sr)
{
	...
	if ((xa = smbsr_lookup_xa(sr)) == 0) {
		smb_errcode_seterror(0, ERRSRV, ERRsrverror);
		return (SDRC_ERROR);
	}
	...
	if (xa->xa_command != SMB_COM_TRANSACTION)
		return (SDRC_DROP_VC);
...
smb_com_transaction2_secondary(smb_request_t *sr)
{
	...
	if (xa->xa_command != SMB_COM_TRANSACTION2) {
		return (SDRC_DROP_VC);
	}
...
smb_nt_trans_dispatch(struct smb_request *sr, struct smb_xa *xa)
{
	...
	default:
		smb_errcode_seterror(0, ERRSRV, ERRsmbcmd);
		return (SDRC_ERROR);
	}
...
smb_trans_net_share_getinfo(smb_request_t *sr, struct smb_xa *xa)
{
	...
	if (smb_mbc_decodef(&xa->req_param_mb, "%sww", sr,
	    &shrname, &level, &max_bytes) != 0)
		return (SDRC_NOT_IMPLEMENTED);

	si = smb_shrmgr_share_lookup(shrname);
 ...
 *   should enumerate (see the SV_TYPE definitions in svrapi.h).
...
smb_trans_net_server_enum2(struct smb_request *sr, struct smb_xa *xa)
{
	...
	/*
	 * according to the protocol the req_fmt must be either "WrLehDO"
	 * or "WrLehDz". If not, ERROR_INVALID_PARAMETER must be returned.
	 * The final char in the req_fmt "z" means a domain is supplied,
	 * 'O' means there is no domain.
	 */

	if (strncmp(xa->req_fmt, "WrLehD", 6) != 0)
		return (ERROR_INVALID_PARAMETER);

	switch (xa->req_fmt[6]) {
	case 'z':
		rc = smb_mbc_decodef(&xa->req_param_mb, "%wwls",
		    sr, &level, &max_bytes, &server_type, &domain);
		break;
	case 'O':
		rc = smb_mbc_decodef(&xa->req_param_mb, "%wwl",
		    sr, &level, &max_bytes, &server_type);
		domain = "";
		break;
	default:
		rc = -1;
	...
	if (xa->smb_suwcnt > 0 && STYPE_ISIPC(sr->tid_tree->t_res_type)) {
		...
		case TRANS_WAIT_NMPIPE:
			if (!is_supported_pipe(xa->xa_pipe_name)) {
				smb_errcode_seterror(0, ERRDOS, ERRbadfile);
				return (SDRC_ERROR);
			}
			rc = SDRC_SUCCESS;
			break;
		...
		if ((rc = smb_mbc_decodef(&xa->req_param_mb, "%wss", sr,
		    &opcode, &xa->req_fmt, &xa->rep_fmt)) != 0)
			goto trans_err_not_supported;
...
smb_trans2_dispatch(smb_request_t *sr, smb_xa_t *xa)
{
	...
	case TRANS2_FIND_FIRST2:
		 ...
		 */
		if (n_data == 0) {
			smb_errcode_seterror(NT_STATUS_INFO_LENGTH_MISMATCH,
    ...
    uint32_t		setup_word_count)
{
	...
	xa->xa_refcnt = 1;
	xa->xa_header = sr->sr_header;
	xa->xa_command = sr->sr_com_current;
	...
	while (nxa) {
		...
		if (nxa->xa_mid == xa->xa_mid &&
		    nxa->xa_pid == xa->xa_pid &&
		    !SMB_XA_CLOSED(nxa) &&

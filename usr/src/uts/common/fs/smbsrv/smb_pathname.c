    ...
    char		*last_component)
{
	...
	*last_component = '\0';
	...
	if (SMB_TREE_IS_DFSROOT(sr) && (sr->sr_flags2 & SMB_FLAGS2_DFS)) {
	...
	if (sr && (sr->sr_flags2 & SMB_FLAGS2_REPARSE_PATH)) {
    ...
    smb_node_t **ret_node, cred_t *cred)
{
	...
	*ret_node = NULL;
	...
	while ((pathleft = pn_pathleft(&upn)) != 0) {
		...
		if (fnode && smb_node_delete_pending(fnode)) {
			err = ETOOMANYREFS; /* NT_STATUS_DELETE_PENDING */
			break;
		}
 ...
 */
boolean_t
smb_pathname_validate(smb_pathname_t *pn)
{
 ...
 */
boolean_t
smb_validate_dirname(smb_pathname_t *pn)
{
	char *name;
	char *path = pn->pn_path;

...
static smb_opipe_t *smb_opipe_alloc(smb_server_t *);
static void smb_opipe_dealloc(smb_opipe_t *);
static int smb_opipe_do_open(smb_opipe_t *, smb_user_t *user);
 ...
 */
smb_opipe_t *
smb_opipe_open(smb_ofile_t *of, smb_arg_open_t *op)
{
	smb_opipe_t *opipe;
	smb_doorhdr_t hdr;
	char *pipe_name;

	pipe_name = smb_opipe_lookup(op->fqi.fq_path.pn_path);

	if ((pipe_name == NULL) || !smb_tree_is_connected(of->f_tree)) {
		smb_errcode_set(NT_STATUS_OBJECT_NAME_NOT_FOUND, ERRDOS,
	...
	opipe = smb_opipe_alloc(of->f_tree->t_server);
	...
	if (smb_opipe_do_open(opipe, of->f_user) != 0) {
		/*
		...
		smb_errcode_set(NT_STATUS_NO_MEMORY, ERRDOS,
 ...
 */
static int
smb_opipe_do_open(smb_opipe_t *opipe, smb_user_t *user)
{
	...
	if (smb_opipe_sethdr(opipe, SMB_OPIPE_OPEN, len) == -1) {
		smb_user_netinfo_fini(userinfo);
		return (-1);
	}
...
void
smb_opipe_delete(smb_opipe_t *opipe)
{
	smb_opipe_dealloc(opipe);
}
...
smb_opipe_door_close(void)
{
	...
	if (smb_opipe_door_hd != NULL) {
		while (smb_opipe_door_ncall > 0)
			(void) cv_wait_sig(&smb_opipe_door_cv,
			    &smb_opipe_door_mutex);

...
static int smb_delete_single_file(smb_request_t *);
static int smb_delete_multiple_files(smb_request_t *);
static int smb_delete_find_fname(smb_request_t *, smb_odir_t *, char *, int);
static int smb_delete_check_dosattr(smb_request_t *);
static int smb_delete_remove_file(smb_request_t *);
...
smb_com_delete(smb_request_t *sr)
{
	...
	if (rc != 0) {
		if (rc == ENOTDIR) {
			...
			smb_errcode_seterror(status, ERRDOS,
	...
	if ((fqi->fq_dnode == sr->tid_tree->t_snode) &&
		...
		smb_node_release(fqi->fq_dnode);
		smb_errcode_seterror(NT_STATUS_OBJECT_PATH_SYNTAX_BAD,
	...
	if (wildcards)
		rc = smb_delete_multiple_files(sr);
	else
		rc = smb_delete_single_file(sr);
 ...
 */
static int
smb_delete_single_file(smb_request_t *sr)
{
	smb_fqi_t *fqi;
	smb_pathname_t *pn;
	int rc = 0;
	...
	rc = smb_fsop_lookup_name(sr, sr->user_cr, 0, sr->tid_tree->t_snode,
	    fqi->fq_dnode, fqi->fq_last_comp, &fqi->fq_fnode);
	if (rc != 0) {
		if (rc == ETOOMANYREFS)
			smb_errcode_set(NT_STATUS_DELETE_PENDING,
			    ERRDOS, ERROR_ACCESS_DENIED);
		else
			smb_errcode_set(NT_STATUS_OBJECT_NAME_NOT_FOUND,
			    ERRDOS, ERROR_FILE_NOT_FOUND);
		return (-1);
	}

	if (smb_delete_check_dosattr(sr) != 0) {
		smb_node_release(fqi->fq_fnode);
		return (-1);
	}

	if (smb_delete_remove_file(sr) != 0) {
		smb_node_release(fqi->fq_fnode);
		return (-1);
	}
 ...
 */
static int
smb_delete_multiple_files(smb_request_t *sr)
{
	int rc, deleted = 0;
	ntstatus_t status;
	smb_fqi_t *fqi;
	...
	for (;;) {
		...
		if (smb_delete_check_dosattr(sr) != 0) {
			...
			if (status == NT_STATUS_CANNOT_DELETE) {
				smb_odir_close(od);
				smb_odir_release(od);
				return (-1);
			}
			if ((status == NT_STATUS_FILE_IS_A_DIRECTORY) &&
	...
	if (deleted == 0) {
		smb_errcode_set(NT_STATUS_NO_SUCH_FILE,
		    ERRDOS, ERROR_FILE_NOT_FOUND);
		return (-1);
	}
 ...
 */
static int
smb_delete_check_dosattr(smb_request_t *sr)
{
	smb_fqi_t *fqi;
	smb_node_t *node;
	smb_attr_t attr;
	...
	if (smb_node_getattr(sr, node, &attr) != 0) {
		smb_errcode_set(NT_STATUS_INTERNAL_ERROR,
		    ERRDOS, ERROR_INTERNAL_ERROR);
		return (-1);
	}

	if (attr.sa_dosattr & FILE_ATTRIBUTE_DIRECTORY) {
		smb_errcode_set(NT_STATUS_FILE_IS_A_DIRECTORY,
		    ERRDOS, ERROR_ACCESS_DENIED);
		return (-1);
	}

	if (SMB_PATHFILE_IS_READONLY(sr, node)) {
		smb_errcode_set(NT_STATUS_CANNOT_DELETE,
		    ERRDOS, ERROR_ACCESS_DENIED);
		return (-1);
	}
 ...
 */
static int
smb_delete_remove_file(smb_request_t *sr)
{
	int rc;
	uint32_t status;
	smb_fqi_t *fqi;
	...
	if (status != NT_STATUS_SUCCESS) {
		smb_errcode_set(NT_STATUS_SHARING_VIOLATION,
	...
	if (status != NT_STATUS_SUCCESS) {
		smb_errcode_set(NT_STATUS_ACCESS_DENIED,
		    ERRDOS, ERROR_ACCESS_DENIED);
		smb_node_end_crit(node);
		return (-1);
	}
...
smb_delete_check_path(smb_request_t *sr)
{
	...
	if (pn->pn_fname == NULL) {
		smb_errcode_seterror(NT_STATUS_FILE_IS_A_DIRECTORY,
		    ERRDOS, ERROR_ACCESS_DENIED);
		return (-1);
	}

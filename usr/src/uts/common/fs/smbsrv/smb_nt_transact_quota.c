...
smb_nt_transact_query_quota(smb_request_t *sr, smb_xa_t *xa)
{
	...
	if (!smb_tree_has_feature(sr->tid_tree, SMB_TREE_QUOTA)) {
		smb_errcode_seterror(NT_STATUS_NOT_SUPPORTED, 0, 0);
		return (SDRC_ERROR);
	}
	...
	if ((ofile->f_node == NULL) || SMB_FTYPE_IS_PIPE(ofile->f_ftype)) {
	...
	if (status != NT_STATUS_SUCCESS) {
		if (status == NT_STATUS_NO_MORE_ENTRIES) {
			...
			smb_errcode_setwarning(status, 0, 0);
 ...
 *
 * Get quota information for the current user for the current file system.
 * Insert the uid in a smb_quota_sid_t, insert the smb_quota_sid_t in the
 * sidlist of a smb_quota_query_t request (query_op SMB_QUOTA_QUERY_USER)
 * and invoke door call smb_quota_query() to obtain the quota information.
 *
 * Returns: NT_STATUS codes.
 */
uint32_t
smb_quota_query_user_quota(smb_request_t *sr, smb_quota_t *quota)
{
	smb_quota_sid_t qsid;
	smb_quota_query_t request;
	smb_quota_response_t reply;
	...
	qsid.qs_id = crgetuid(sr->uid_user->u_cred);
	qsid.qs_idtype = SMB_QUOTA_UID;
	...
	request.qq_query_op = SMB_QUOTA_QUERY_USER;

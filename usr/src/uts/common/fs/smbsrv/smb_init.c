    ...
    int *retval)
{
	...
	case SMB_IOC_FILE_CLOSE:
		...
		break;
	case SMB_IOC_NUMSHARE:
		rc = smb_shrmgr_share_num(&ioc->ioc_sharenum);
		copyout = B_TRUE;
		break;
	case SMB_IOC_GETSHARE:
		rc = smb_shrmgr_share_get(&ioc->ioc_share);
		copyout = B_TRUE;
		break;
	case SMB_IOC_CHKSHARE:
		rc = smb_shrmgr_share_chk(&ioc->ioc_share);
		copyout = B_TRUE;
		break;
	default:
		rc = ENOTTY;

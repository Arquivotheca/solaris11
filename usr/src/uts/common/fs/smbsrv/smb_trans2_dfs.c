...
static uint32_t smb_dfs_encode_refv1(smb_xa_t *, dfs_info_t *);
static uint32_t smb_dfs_encode_refv2(smb_xa_t *, dfs_info_t *);
static uint32_t smb_dfs_encode_refv3_v4(smb_xa_t *, dfs_info_t *, uint16_t);
static void smb_dfs_encode_targets(smb_xa_t *, dfs_info_t *);
static uint32_t smb_dfs_referrals_get(char *, dfs_reftype_t,
...
smb_com_trans2_get_dfs_referral(smb_request_t *sr, smb_xa_t *xa)
{
	...
	case DFS_REFERRAL_INVALID:
		...
		smb_errcode_seterror(NT_STATUS_INVALID_PARAMETER, ERRDOS,
	...
	case DFS_REFERRAL_SYSVOL:
		/* MS-DFSC: this error is returned by non-DC root */
		smb_errcode_seterror(NT_STATUS_NO_SUCH_DEVICE, ERRDOS,
		    ERROR_BAD_DEVICE);
		return (SDRC_ERROR);
	...
	status = smb_dfs_referrals_get(path, reftype, &refrsp);
	...
	case DFS_REFERRAL_V1:
		status = smb_dfs_encode_refv1(xa, referrals);
		break;

	case DFS_REFERRAL_V2:
		status = smb_dfs_encode_refv2(xa, referrals);
		break;

	case DFS_REFERRAL_V3:
		status = smb_dfs_encode_refv3_v4(xa, referrals, maxreflvl);
		break;

	default:
		status = smb_dfs_encode_refv3_v4(xa, referrals,
		    DFS_REFERRAL_V4);
		break;
	}
...
static uint32_t
smb_dfs_encode_refv1(smb_xa_t *xa, dfs_info_t *referrals)
{
	 ...
	 */
	if (r == 0) {
		smb_errcode_setwarning(NT_STATUS_BUFFER_OVERFLOW,
 ...
 */
static uint32_t
smb_dfs_encode_refv2(smb_xa_t *xa, dfs_info_t *referrals)
{
	uint16_t entsize, rep_bufsize;
	uint16_t server_type;
	uint16_t flags = 0;
 ...
 */
static uint32_t
smb_dfs_encode_refv3_v4(smb_xa_t *xa, dfs_info_t *referrals, uint16_t ver)
{
 ...
 */
static uint32_t
smb_dfs_referrals_get(char *dfs_path, dfs_reftype_t reftype,
    dfs_referral_response_t *refrsp)
{

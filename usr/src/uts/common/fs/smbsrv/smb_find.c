...
smb_com_search(smb_request_t *sr)
{
	...
	if ((sr->session->dialect <= LANMAN1_0) ||
	    ((sr->sr_flags2 & SMB_FLAGS2_KNOWS_LONG_NAMES) == 0)) {
		to_upper = B_TRUE;
	}

	/* We only handle 8.3 name here */
	sr->sr_flags2 &= ~SMB_FLAGS2_KNOWS_LONG_NAMES;
	...
	if (!smb_pathname_validate(pn) ||
		...
		smb_errcode_setwarning(NT_STATUS_NO_MORE_FILES,
	...
	if (find_first) {
		...
		if (odid == 0) {
			if (smb_errcode_getstatus() == NT_STATUS_ACCESS_DENIED)
				smb_errcode_setwarning(NT_STATUS_NO_MORE_FILES,
				    ERRDOS, ERROR_NO_MORE_FILES);
			return (SDRC_ERROR);
		}

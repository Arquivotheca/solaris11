...
lookuppnvp(
	struct pathname *pnp,		/* pathname to lookup */
	struct pathname *rpnp,		/* if non-NULL, return resolved path */
	int flags,			/* flags: FOLLOW, NONSYSCALL */
	vnode_t **dirvpp,		/* ptr for parent vnode */
	vnode_t **compvpp,		/* ptr for entry vnode */
	...
	cred_t *cr)			/* user's credential */
{
	...
	boolean_t auditing = (flags & NONSYSCALL) == 0 && AU_AUDITING() != 0;
...
/*
 * Return the lowermost vnode if this is a mountpoint. Returned vnode is
 * VN_HOLD'd. Input vnode can be VN_RELE'd in vn_under(), so you need to
 * do extra VN_HOLD before calling vn_under(), if you want to use input
 * vnode afterwards.
 */
vnode_t *
vn_under(vnode_t *vp)
...
{
	return (vnodetopath_common(vrootp, vp, buf, buflen, cr, NONSYSCALL));
}

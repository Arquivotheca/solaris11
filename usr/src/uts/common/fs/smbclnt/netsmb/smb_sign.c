...
#define	SMBSIGOFF	14	/* SMB security signature offset */
	...
	uint32_t seqno, uchar_t *signature)
{
	...
	union {
		struct {
			...
			uint8_t raw[SMB_HEADER_LEN];  /* header length (32) */
		} r;
		struct {
			uint8_t skip[2]; /* not used - just alignment */
	...
	ASSERT(mp != NULL);
	ASSERT(MBLKL(mp) >= SMB_HEADER_LEN);
	 ...
	 */
	bcopy(mp->b_rptr, smbhdr.r.raw, SMB_HEADER_LEN);
	...
	data.cd_offset = 0;
	data.cd_length = SMB_HEADER_LEN;
	data.cd_miscdata = 0;
	data.cd_raw.iov_base = (char *)smbhdr.r.raw;
	data.cd_raw.iov_len = SMB_HEADER_LEN;
	...
	data.cd_offset = SMB_HEADER_LEN;
	data.cd_length = msgdsize(mp) - SMB_HEADER_LEN;
	data.cd_miscdata = 0;
	data.cd_mp = mp;
	status = crypto_digest_update(crypto_ctx, &data, 0);
	if (status != CRYPTO_SUCCESS)
	 ...
	 */
	if (signature)
		bcopy(mac, signature, SMB_SIG_SIZE);
...
smb_rq_sign(struct smb_rq *rqp)
{
	 ...
	 * but just in case...
	 */
	if (MBLKL(mp) < SMB_HEADER_LEN) {
		if (!pullupmsg(mp, SMB_HEADER_LEN))
			return;
	}
...
smb_rq_verify(struct smb_rq *rqp)
{
	 ...
	 */
	if (bcmp(sigbuf, sigloc, SMB_SIG_SIZE) == 0)
		return (0);

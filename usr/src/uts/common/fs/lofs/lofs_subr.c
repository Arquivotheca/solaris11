...
{
	/*
	 * The reference count starts at 1, which is the hold by the vfs and is
	 * decremented in lo_freevfs().  This allows vnodes in nested
	 * filesystems to exist even after the parent lofs filesystem has been
	 * force unmounted.  The last vnode reference will trigger ldestroy().
	 */
	li->li_refct = 1;
	li->li_lfs = NULL;
	if (htsize == 0)
...
freelfsnode(struct lfsnode *lfs, struct loinfo *li)
{
	...
	ASSERT(li->li_refct > 1 || (li->li_refct == 1 && li->li_lfs == NULL) ||
	    li->li_unmounted);
...
lsave(lnode_t *lp, struct loinfo *li)
{
	...
	if (li->li_refct - 1 > (li->li_htsize << lo_resize_threshold)) {
...
freelonode(lnode_t *lp)
{
	...
	for (lt = TABLE_BUCKET(lp->lo_vp, li); lt != NULL;
		...
		if (lt == lp) {
			uint_t newct = atomic_add_32_nv(&li->li_refct, -1);
			vfsp = vp->v_vfsp;
			vn_invalid(vp);
			mutex_enter(&li->li_lfslock);
			/*
			...
			while (lfs != NULL) {
				...
				if (lfs->lfs_vfs.vfs_count == 1) {
					DTRACE_PROBE(lofs__sly__free__vfs);
					/*
					 * Lfs is idle
					 */
					freelfsnode(lfs, li);
			...
			if (newct == 0) {
				/*
				 * There was a race, and the
				 * VFS_FREEVFS() happened before we
				 * lowered the li_refct.  We need to
				 * clean up the lo_info now so it
				 * isn't leaked, lo_freevfs() will not
				 * be called again.
				 */
				ldestroy(li);
			}

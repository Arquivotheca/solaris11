	...
	struct cred *cr)
{
	...
	/*
	 * Secpolicy_fs_mount may change the mount options; see
	 * secpolicy_fs_common and secpolicy_fs_mount_clearopts. After this,
	 * a call to vfs_optionisset(MNTOPT_NODEVICES) may return true.
	 */
	if (secpolicy_fs_mount(cr, vp, vfsp) != 0)
		return (EPERM);
	...
	if ((li->li_realvfs->vfs_flag & VFS_MWAC) &&
	    !(li->li_realvfs->vfs_flag & VFS_RDONLY) &&
	    priv_mwac_needed(cr, vfsp, vp)) {
...
lo_unmount(struct vfs *vfsp, int flag, struct cred *cr)
{
	...
	/*
	 * We support a rather kludgely form of force unmount that allows us to
	 * return EIO to all future requests, but doesn't actually go through
	 * and clean up the vnodes.  This keeps it simpler, but leaves the
	 * underlying filesystem still referenced.  Since we always force
	 * unmount our filesystems in fishworks, this is not an issue.  A
	 * complete solution would require going through all the lofs vnodes
	 * are releasing the underlying (real) vnodes.
	 */
	if (flag & MS_FORCE) {
		rw_enter(&li->li_unmount_lock, RW_WRITER);
		if (li->li_unmounted) {
			rw_exit(&li->li_unmount_lock);
			return (0);
		}

		li->li_unmounted = B_TRUE;
		rw_exit(&li->li_unmount_lock);
	} else if (li->li_refct > 2 || li->li_rootvp->v_count != 1) {
		/*
		 * The loinfo refcount should have one hold from the vfs_t when
		 * it was created and another from the root vnode.  If it's
		 * greater, then it indicates we have a nested filesystem
		 * active.
		 */
		return (EBUSY);
	}
...
lo_statvfs(register struct vfs *vfsp, struct statvfs64 *sbp)
{
	...
	if (realrootvp != NULL)
		ret = VFS_STATVFS(realrootvp->v_vfsp, sbp);
	else
		ret = EIO;
...
lo_vget(struct vfs *vfsp, struct vnode **vpp, struct fid *fidp)
{
	...
	(void) lo_realvfs(vfsp, &realrootvp);
	if (realrootvp != NULL)
		ret = VFS_VGET(realrootvp->v_vfsp, vpp, fidp);
	else
		ret = EIO;
...
lo_freevfs(struct vfs *vfsp)
{
	...
	/*
	 * Normally, this should be the last reference to the loinfo.  But with
	 * force unmount, it's possible that someone is holding onto a lfsnode,
	 * and we can't destroy it until the last reference is gone.
	 */

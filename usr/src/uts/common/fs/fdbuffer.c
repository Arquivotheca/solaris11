    ...
    int b_flags)
{
	...
	/*
	 * NOTE: in the ~B_ASYNC case, the caller of fdb_iosetup is
	 * responsible for performing the following on the returned buf:
	 *	biowait(bp);
	 *	fdb_iodone(bp);
	 * If this does not occur, we will leak the buf allocated by
	 * bioclone. The bp_mapout is done by fdb_iodone prior to freerbuf.
	 */
...
fdb_iodone(buf_t *bp)
{
	...
	bp_mapout(bp);			/* of bioclone-ed buf */

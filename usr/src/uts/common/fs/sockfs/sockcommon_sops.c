    ...
    const void *optval, socklen_t optlen, struct cred *cr)
{
	...
	if (so->so_filter_active > 0) {
		/*
		 * Deny zero-copy if there is a filter with a data_out
		 * callback.
		 */
		if (option_name == SO_SND_COPYAVOID && level == SOL_SOCKET) {
			sof_instance_t *inst;
		...
		if ((error = sof_filter_setsockopt(so, level, option_name,
...
so_set_prop(sock_upper_handle_t sock_handle, struct sock_proto_props *soppp)
{
	...
	if (soppp->sopp_flags & SOCKOPT_ZCOPY) {
		/* Only allow zero-copy when no filters are active */
	...
	if (soppp->sopp_flags & SOCKOPT_PREMAPPED_MIN)
		so->so_proto_props.sopp_pm_min = soppp->sopp_pm_min;
	if (soppp->sopp_flags & SOCKOPT_PREMAPPED_MAX)
		so->so_proto_props.sopp_pm_max = soppp->sopp_pm_max;
	if (soppp->sopp_flags & SOCKOPT_STRICT_BACKLOG)
	...
	soppp->sopp_flags &= ~(SOCKOPT_MAXBLK | SOCKOPT_WROFF | SOCKOPT_TAIL |
	    ...
	    SOCKOPT_LOOPBACK | SOCKOPT_PREMAPPED_MIN | SOCKOPT_PREMAPPED_MAX);
...
so_close(struct sonode *so, int flag, struct cred *cr)
{
	 ...
	 */
	if (so->so_filter_active > 0) {
		sof_sonode_notify_filters(so, SOF_EV_CLOSING,
		    (uintptr_t)B_TRUE);
	}
	...
	/*
	 * All async filter operations must complete before the downcall
	 * is made.
	 */
	sof_sonode_wait_async_ops(so);
...
void
so_set_pmh(sock_upper_handle_t sock_handle, pm_handle_t pmh)
{
	struct sonode *so = (struct sonode *)sock_handle;
...
retry:
	 ...
	 */
	if ((rval.r_val1 & SO_BUF_MSG_MORE) && (so->so_mode & SM_ATOMIC))
		msg->msg_flags |= MSG_TRUNC;
	if (mctlp == NULL) {

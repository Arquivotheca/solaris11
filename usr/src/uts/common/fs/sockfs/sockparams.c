 ...
 *
 * Returns:
 *   On success, 0. If an entry already exists, and it map to a
 *   different transport provider than `sp', then EEXIST is returned.
 *   However, if the entry map to the same transport provider as `sp',
 *   then EALREADY is returned.
 *
 * Locking:
...
sockparams_add(struct sockparams *sp)
{
	struct sockparams *tmp;
	int error;

	ASSERT(!(sp->sp_flags & SOCKPARAMS_EPHEMERAL));

	rw_enter(&sockconf_lock, RW_WRITER);
	if ((tmp = sockparams_find(&sphead, sp->sp_family, sp->sp_type,
	    sp->sp_protocol, B_TRUE, NULL)) != NULL) {
		int cmp;

		if (sp->sp_sdev_info.sd_devpath != NULL &&
		    tmp->sp_sdev_info.sd_devpath != NULL) {
			cmp = strcmp(sp->sp_sdev_info.sd_devpath,
			    tmp->sp_sdev_info.sd_devpath);
		} else {
			cmp = strcmp(sp->sp_smod_name, tmp->sp_smod_name);
		}
		rw_exit(&sockconf_lock);

		return ((cmp == 0) ? EALREADY : EEXIST);
	} else {
		/*
...
/*
 * Create a mapping from <`family', `type', `protocol'> to `name',
 * where `name' can either be a socket module or a TPI device. Returns
 * 0 on success and an errno otherwise.
 */
int
sockparams_add_mapping(int family, int type, int protocol, const char *name)
{
	int error = 0;
	char *devpath = NULL;
	char *module = NULL;
	...
	if (strncmp(name, "/dev", strlen("/dev")) == 0) {
		...
		if (strcmp(name, NCA_DEV) == 0) {
			...
			namelen = strlen(NCA_INET_DEV) + 1;
			devpath = kmem_alloc(namelen, KM_SLEEP);
			bcopy(NCA_INET_DEV, devpath, namelen);
			devpath[namelen - 1] = '\0';
		} else {
			devpath = kmem_alloc(namelen, KM_SLEEP);
			bcopy(name, devpath, namelen);
			devpath[namelen - 1] = '\0';
		}
	} else {
		/* For socket module */
		module = kmem_alloc(namelen, KM_SLEEP);
		bcopy(name, module, namelen);
		module[namelen - 1] = '\0';
		namelen = 0;
	}
	...
	sp = sockparams_create(family, type, protocol, module,
	    devpath, namelen, 0, KM_SLEEP, &error);
	if (sp != NULL) {
...
int
sockparams_copyout_entries(struct uio *uiop, int *num_of_entries)
{
	STRUCT_DECL(sock_config, so_config);
	struct sockparams *sp = NULL;
	int error;
	int count = 0;
	...
	*num_of_entries = 0;
	...
	STRUCT_INIT(so_config, get_udatamodel());
	/*
	 * There should be space for atleast one entry to be returned
	 */
	if (uiop->uio_resid < STRUCT_SIZE(so_config))
		return (EINVAL);

	rw_enter(&sockconf_lock, RW_READER);
	for (sp = list_head(&sphead); sp != NULL;
	    sp = list_next(&sphead, sp)) {
		STRUCT_FSET(so_config, sc_family, sp->sp_family);
		STRUCT_FSET(so_config, sc_type, sp->sp_type);
		STRUCT_FSET(so_config, sc_protocol, sp->sp_protocol);
		STRUCT_FSET(so_config, sc_loaded, B_FALSE);
		if (strcmp(sp->sp_smod_name, SOTPI_SMOD_NAME) == 0) {
			(void) strncpy(STRUCT_FGETP(so_config, sc_modname),
			    sp->sp_sdev_info.sd_devpath, MAXPATHLEN);
		} else {
			(void) strncpy(STRUCT_FGETP(so_config, sc_modname),
			    sp->sp_smod_name, MAXPATHLEN);
			if (sp->sp_smod_info != NULL)
				STRUCT_FSET(so_config, sc_loaded, B_TRUE);
		}
		STRUCT_FSET(so_config, sc_auto_filters,
		    list_head(&sp->sp_auto_filters) == NULL ? B_FALSE:B_TRUE);
		STRUCT_FSET(so_config, sc_prog_filters,
		    list_head(&sp->sp_prog_filters) == NULL ? B_FALSE:B_TRUE);

		error = uiomove(STRUCT_BUF(so_config), STRUCT_SIZE(so_config),
		    UIO_READ, uiop);
		if (error != 0) {
			break;
		} else {

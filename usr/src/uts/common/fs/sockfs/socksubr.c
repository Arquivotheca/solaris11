...
sockinit(int fstype, char *name)
{
	 ...
	 */
	smod_set_func(socket_sonode_create, socket_sonode_destroy);

	/*
	 * Create and add the TPI socket module.
	 */
		...
		vnode_t **vpp)
{
	...
	/*
	 * Check if the remote socket has been closed, and verify that `vp'
	 * is a VSOCK vnode in the underlying filesystem. A filesystem
	 * vnode should have its v_stream->sd_vnode refer to a vnode other
	 * than itself.
	 *
	...
	if (vp->v_stream == NULL || vp->v_stream->sd_vnode == vp) {
...
fdbuf_create(void *rights, int rightslen, struct fdbuf **fdbufp)
{
	...
	for (i = 0; i < numfd; i++) {
		 ...
		 * we don't need to do anything on the send side (we lose

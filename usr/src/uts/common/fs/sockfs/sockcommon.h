...
extern void	so_set_pmh(sock_upper_handle_t, pm_handle_t);
...
extern void	socket_sendsig(struct sonode *, int);
	...
	/* socket buffer message handling */
...
extern boolean_t	so_check_flow_control(struct sonode *);
	...
	/* Flag returned by so_dequeue_msg() in rval_t.r_val1 */
enum {
	SO_BUF_MSG_MORE = 0x01,	/* more data available in the message */
	SO_BUF_MSG_NEXT = 0x02	/* next message available */
};

extern mblk_t	*socopyinuio(sonode_t *, uio_t *, ssize_t, size_t, ssize_t,
    size_t, ssize_t, ssize_t, int *);
...
extern mblk_t	*so_allocb_premapped(struct sonode *, size_t);

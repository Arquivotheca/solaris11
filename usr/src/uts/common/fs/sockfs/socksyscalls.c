...
sockconf_add_sock(int family, int type, int protocol, char *name)
{
	 ...
	 */
	buf = kmem_alloc(MAXPATHLEN, KM_SLEEP);
	if ((error = copyinstr(name, buf, MAXPATHLEN, NULL)) == 0)
		error = sockparams_add_mapping(family, type, protocol, buf);
	kmem_free(buf, MAXPATHLEN);
...
sockconfig(int cmd, void *arg1, void *arg2, void *arg3, void *arg4)
{
	...
	/*
	 * Allow non priviledged user to see socket configuration
	 */
	if ((secpolicy_net_config(CRED(), B_FALSE) != 0) &&
	    cmd != SOCKCONFIG_GET_NUM_ENTRIES &&
	    cmd != SOCKCONFIG_GET_ENTRIES) {
	...
	case SOCKCONFIG_GET_NUM_ENTRIES:
		if (arg2 != (void *)(sizeof (num_of_entries))) {
			error = EINVAL;
			break;
		}
		num_of_entries = sockparams_get_num_of_entries();
		if (ddi_copyout(&num_of_entries, arg1, sizeof (int), 0) < 0) {
			error = EFAULT;
		}
		break;
	case SOCKCONFIG_GET_ENTRIES:
		if (arg4 != (void *)(sizeof (num_of_entries))) {
			error = EINVAL;
			break;
		}
		iov.iov_base = (char *)arg1;
		iov.iov_len = (size_t)arg2;
		...
		error = sockparams_copyout_entries(&uio, &num_of_entries);
		if ((error == 0) &&
		    (ddi_copyout(&num_of_entries, arg3, sizeof (int), 0) < 0)) {
			error = EFAULT;
		}
		break;
	default:
    ...
    uint_t maxpsz, ssize_t *count)
{
	...
	for (;;) {
		...
		if (is_system_labeled()) {
			...
			mp = so_allocb_premapped(so, iosize + extra);
		}
    ...
    int *errorp)
{
	...
	} else if ((*errorp = SOP_INIT(so, NULL, CRED(), SOCKET_SLEEP)) != 0 ||
	    (*errorp = VOP_OPEN(&SOTOV(so), FREAD|FWRITE, CRED(), NULL)) != 0) {
		socket_destroy(so);
		so = NULL;
	}
	return (so);

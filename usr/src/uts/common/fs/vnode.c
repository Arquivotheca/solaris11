...
top:
	/* reset filemode upon a retry loop */
	filemode = ofilemode;

	/*
	 * The standard, IEEE Std 1003.1, specifies that "O_CREAT" has no
	 * effect if the file exists and O_EXCL is not set.  We first try to
	 * open the file if FCREAT and FEXCL aren't both set because we want
	 * to avoid the secpolicy_() calls in fop_create.  This is quicker too,
	 * anyway.
	 */
	if ((filemode & (FCREAT|FEXCL)) != (FCREAT|FEXCL)) {
		/*
	...
	/* FCREAT is set and FEXCL is set or the file didn't exist */
	if (filemode & FCREAT) {
		enum vcexcl excl;
	...
	/*
	 * Do remaining checks for FNOFOLLOW, FDIRECTORY and FNOLINKS.
	 */
	...
	if (vp->v_type == VREG && !(filemode & FEXEC) && nbl_need_check(vp)) {
		...
		error = VOP_SHRLOCK(vp, F_SHARE_NBMAND, &shr, ofilemode, CRED(),
		    NULL);
		if (error)
			goto out;
		shrlock_done = 1;
	 ...
	 * Do opening protocol.
	 */
	error = VOP_OPEN(&vp, ofilemode, CRED(), NULL);
	if (error)
		goto out;
	open_done = 1;

	/*
	 * Truncate if required.  Before when FCREAT was specified we
	 * always called vn_create() and O_TRUNC was only applied to VREG.
	 * It is unclear why O_TRUNC shouldn't always be ignored for non-VREG.
	 */
	if ((filemode & FTRUNC) && !(filemode & FCREAT)) {
		if ((ofilemode & FCREAT) && vp->v_type != VREG)
			goto out;
		vattr.va_size = 0;
		vattr.va_mask = AT_SIZE;
...
out:
	...
	if (error) {
		if (open_done) {
			(void) VOP_CLOSE(vp, ofilemode, 1, (offset_t)0, CRED(),
			    NULL);
			open_done = 0;
			shrlock_done = 0;
		}
...
out:
	...
	if (auditing) {
		audit_vncreate_finish(*vpp, flag, error);
	}
	if (in_crit) {
		nbl_end_crit(vp);
		in_crit = 0;
...
top:
	...
	/*
	 * Make sure we don't rename active mount points.
	 */
	if ((fvp->v_flag & VROOT) && (fvp->v_type == VDIR)) {
		error = EBUSY;
		goto out;
	}
 ...
 */
int
vn_remove(char *fnamep, enum uio_seg seg, enum vn_rm dirflag)
{
	return (vn_removeat(NULL, fnamep, seg, dirflag));
}

int
vn_removeat(vnode_t *startvp, char *fnamep, enum uio_seg seg,
    enum vn_rm dirflag)
{
...
int
vn_replace_ops(const char *name, vnodeops_t *vnops,
    const fs_operation_def_t *templ)
{
	int unused_ops;
	int error;

	vnops->vnop_name = name;
	error = fs_build_vector(vnops, &unused_ops, vn_ops_table, templ);
	if (error)
		return (error);
	return (error);
}
...
{
#ifdef DEBUG
	/* See comments where VN_SHIFT is defined. */
	int size = P2ROUNDUP(sizeof (struct vnode), VN_ALIGN);
	int low_bits = highbit(size) - 1;

	if (!ISP2(size))
		low_bits++;
	ASSERT(low_bits == VN_SHIFT);
#endif
	vn_cache = kmem_cache_create("vn_cache", sizeof (struct vnode),
	    VN_ALIGN, vn_cache_constructor, vn_cache_destructor, NULL, NULL,
	    NULL, 0);
}
    ...
    const char *path, size_t plen)
{
	...
	/*
	 * If the path added is ".." then we'll strip the last component.
	 * This is somewhat tricky and we really need to verify the action
	 * is correct.
	 */
	if (strcmp(path, "..") == 0) {
		char *up;
		...
		if (rpathlen > 1) {
			up = strrchr(base->v_path, '/');
			if (up != NULL) {
				sublen = up - base->v_path;
				doslash = 0;
				if (sublen == 0)
					sublen++;
				rpathalloc = sublen + 1;
 ...
 * context.  The string must be a complete path from rootdir.
 */
void
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_ATTR(vp);

	/* no open protocol for FEXEC of a regular file */
	if ((mode & FEXEC) &&
	    (vp->v_type == VREG || pr_isobject(vp)))
		return (0);
	...
	if ((mode & (FWRITE|FTRUNC)) == 0 ||
	    (ret = secpolicy_basic_file_write(cr, vp, NULL)) == 0) {
	...
	caller_context_t *ct)
{
	...
	/* no close protocol for FEXEC of a regular file */
	if ((flag & FEXEC) &&
	    (vp->v_type == VREG || pr_isobject(vp)))
		return (0);
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_UIO(vp, uiop, B_TRUE);
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_UIO(vp, uiop, B_FALSE);
	...
	caller_context_t *ct)
{
	...
	/*
	 * We override the SHADOW_IOC_* ioctls to do our own processing, and
	 * fall through to the normal processing if it's anything else.  We
	 * expect vfs_shadow_check_ioctl() to return 0 for sucess, ENOTSUP if
	 * it's not one of our known ioctls, or error otherwise.
	 */
	if ((vp)->v_vfsp != NULL && (vp)->v_vfsp->vfs_shadow != NULL) {
		int shadow_err = vfs_shadow_check_ioctl(vp, cmd, arg, flag, cr);
		if (shadow_err != ENOTSUP)
			return (shadow_err);

		shadow_err = vfs_shadow_check_vp(vp, B_FALSE, NULL);
		if (shadow_err != 0)
			return (shadow_err);
	}
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_SETATTR(vp, vap);
	...
	if ((err = secpolicy_basic_file_write(cr, vp, NULL)) == 0)
	...
	caller_context_t *ct)
{
	...
	if ((mode & VWRITE) == 0 ||
	    (err = secpolicy_basic_file_write(cr, vp, NULL)) == 0) {
	...
	vsecattr_t *vsecp)	/* ACL to set during create */
{
	...
	if ((ret = secpolicy_basic_file_write(cr, dvp, name)) == 0) {
	...
	int flags)
{
	...
	if ((err = secpolicy_basic_file_write(cr, dvp, nm)) == 0)
	...
	int flags)
{
	...
	if ((err = secpolicy_mwac_protected(cr, svp)) == 0 &&
	    (err = secpolicy_basic_file_write(cr, tdvp, tnm)) == 0)
	...
	int flags)
{
	...
	if ((err = secpolicy_basic_file_write(cr, sdvp, snm)) == 0 &&
	    (err = secpolicy_basic_file_write(cr, tdvp, tnm)) == 0) {
	...
	vsecattr_t *vsecp)	/* ACL to set during create */
{
	...
	if ((ret = secpolicy_basic_file_write(cr, dvp, dirname)) == 0) {
		ret = (*(dvp)->v_op->vop_mkdir)
		    (dvp, dirname, vap, vpp, cr, ct, flags, vsecp);
	} else if (ret == EACCES || ret == EROFS) {
		vnode_t *lvp;
		if (VOP_LOOKUP(dvp, dirname, &lvp, NULL, flags, NULL,
		    cr, ct, NULL, NULL) == 0) {
			ret = EEXIST;
			VN_RELE(lvp);
		}
	...
	int flags)
{
	...
	if ((err = secpolicy_basic_file_write(cr, dvp, linkname)) == 0) {
	...
	caller_context_t *ct)
{
	...
	/*
	 * There is no need to check for shadowed vnode, as any pending writes
	 * (if they exist) will have already migrated the necessary data.
	 */
	...
	caller_context_t *ct)
{
	...
	/*
	 * There is no need to check for shadow vnodes here, as there is no way
	 * for the identity of a vnode to be dependent on the contents of the
	 * file.
	 */
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_SPACE(vp, offset, flag);
	...
	caller_context_t *ct)
{
	...
	/*
	 * No need to check for shadow vnodes here.  No filesystems have a
	 * vop_realvp() that depends on the contents of the file or directory.
	 */
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_RANGE(vp, off, len);
	...
	caller_context_t *ct)
{
	...
	/*
	 * We don't check for shadow migration on a putpage.  While this may
	 * seem counterintutive, the only way the user could push plain file
	 * contents is through a write or mmap call, and we already migrate
	 * files on these operations.  More importantly, some filesystems (most
	 * notably ufs) will push metadata via this interface, so we lose the
	 * benefits of shadow migration because we rely on the ability to make
	 * metadata modifications (most notably xattr manipulation) without
	 * bringing in the plain file contents.
	 */
	...
	caller_context_t *ct)
{
	...
	/*
	 * A variant of VFS_SHADOW_CHECK_ATTR(), we only need to check the
	 * shadow attribute if the value is specific to the vnode itself (and
	 * not the filesystem).  This is only the case for _PC_XATTR_EXISTS.
	 */
	if (vp->v_vfsp != NULL && vp->v_vfsp->vfs_shadow != NULL &&
	    cmd == _PC_XATTR_EXISTS) {
	...
	caller_context_t *ct)
{
	...
	VFS_SHADOW_CHECK_RANGE(vp, io_off, io_len);
	...
	caller_context_t *ct)
{
	...
	/*
	 * It is not necessary to check for shadow vnodes here.  The ACLs are
	 * migrated as part of creating the directory entry, and we can get
	 * this information without pulling in the entire file.
	 */
...
vn_is_reparse(vnode_t *vp, cred_t *cr, caller_context_t *ct)
{
	...
	if ((vp->v_type != VLNK && vp->v_type != VDIR) ||

    ...
    caller_context_t *ct, int *direntflags, pathname_t *realpnp)
{
	...
	if (!SDEV_IS_GLOBAL(parent))
		return (prof_lookup(dvp, nm, vpp, cred, 0));
...
/* XXXci - We may need to translate the C-I flags on VOP_LOOKUP */
	...
	if (SDEV_IS_GLOBAL(fromparent))
		atomic_inc_ulong(&fromparent->sdev_gdir_gen);
...
/*
 * When ioctl()ing a non-global special node, we need to re-assert any
 * device annotations, since they may have been removed (via
 * ddi_remove_minor_node()) before this point: for example, disk
 * relabelling, or device reconfiguration.  This vnode came via
 * sdev_to_vp()'s spec_open(), so we can get to the sdev_node via
 * VOP_REALVP().
 */
static int
sdev_fem_ioctl(femarg_t *vf, int cmd, intptr_t arg, int flag, cred_t *cr,
    int *rvalp, caller_context_t *ct)
{
	...
	if (VOP_REALVP(vp, &rvp, ct) == 0 &&
	    (VTOSDEV(rvp)->sdev_flags & SDEV_ANNOTATED))
		sdev_apply_devann(VTOSDEV(rvp));

	return (vnext_ioctl(vf, cmd, arg, flag, cr, rvalp, ct));
}

static void
sdev_fem_inactive(femarg_t *vf, cred_t *cr, caller_context_t *ct)
{
	vnode_t	*vp = (vnode_t *)vf->fa_fnode->fn_available;
	(void) fem_uninstall(vp, sdev_femops, vp);
	vnext_inactive(vf, cr, ct);
}
...
const fs_operation_def_t sdev_vnodefem_tbl[] = {
	VOPNAME_IOCTL,		{ .femop_ioctl = sdev_fem_ioctl },
	VOPNAME_INACTIVE,	{ .femop_inactive = sdev_fem_inactive },
	NULL,	NULL
};

...
devipnet_validate(struct sdev_node *dv)
{
	...
	dev = ipnet_if_getdev(dv->sdev_name, VTOZONE(SDEVTOV(dv))->zone_id);
	if (dev == (dev_t)-1)
    ...
    void **arg, cred_t *cred, void *whatever, char *whichever)
{
	...
	dev = ipnet_if_getdev(nm, VTOZONE(SDEVTOV(ddv))->zone_id);
	if (dev == (dev_t)-1)
		err = ENOENT;
	else
...
devipnet_filldir(struct sdev_node *ddv)
{
	...
	ipnet_walk_if(devipnet_filldir_entry, ddv,
	    VTOZONE(SDEVTOV(ddv))->zone_id);

...
/*
 * /dev mount options table
 */
static mntopt_t mntopts[] = {
	{ MNTOPT_SDEV_ATTRDIR, NULL, NULL, MO_NODISPLAY|MO_HASVALUE, NULL },
};

static mntopts_t sdev_mntopts = {
...
static vfsdef_t sdev_vfssw = {
	...
	VSW_CANREMOUNT|VSW_HASPROTO,	/* flags */
	&sdev_mntopts		/* mount options table prototype */
};
...
devinit(int fstype, char *name)
{
	...
	error = fem_create("sdev_fem", sdev_vnodefem_tbl,
	    (fem_t **)&sdev_femops);

	if (error != 0) {
		(void) vfs_freevfsops_by_type(fstype);
		cmn_err(CE_WARN, "devinit: fem_create failed");
		return (error);
	}

	error = vn_make_ops("dev", sdev_vnodeops_tbl, &sdev_vnodeops);
	if (error != 0) {
		fem_free(sdev_femops);
		(void) vfs_freevfsops_by_type(fstype);
    ...
    struct cred *cr)
{
	 ...
	 */
	if (vfs_optionisset(vfsp, MNTOPT_SDEV_ATTRDIR, &attrdir)) {
		if (attrdir == NULL)
			return (EINVAL);

		/* user supplied an attribute store */
		if (error = lookupname(attrdir, UIO_SYSSPACE, FOLLOW, NULLVPP,
		    &avp)) {
			cmn_err(CE_NOTE, "/dev fs: lookup on attribute "
			    "directory %s failed", attrdir);
			return (error);
		}
	...
	} else if (uap->flags & MS_REMOUNT) {
		/* Must specify attribute directory for a remount */
		return (EINVAL);
	} else {
		/* use the soon-to-be-covered mountp as the attribute store */
		avp = mvp;
		VN_HOLD(avp);
	}
...
/*
 * unmounting the non-global /dev instances, e.g. when halting a zone.
 */
static int

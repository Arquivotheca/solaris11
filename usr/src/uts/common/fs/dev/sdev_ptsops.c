...
devpts_validate(struct sdev_node *dv)
{
	...
	if (ptms_minor_valid(VTOZONE(SDEVTOV(dv))->zone_id, min, &uid,
	    &gid) == 0) {
    ...
    void **arg, cred_t *cred, void *whatever, char *whichever)
{
	 ...
	 */
	if (!ptms_minor_valid(VTOZONE(SDEVTOV(ddv))->zone_id, min, &uid,
	    &gid)) {
...
devpts_set_id(struct sdev_node *dv, struct vattr *vap, int protocol)
{
	...
	ptms_set_owner(VTOZONE(SDEVTOV(dv))->zone_id,
	    getminor(SDEVTOV(dv)->v_rdev), vap->va_uid, vap->va_gid);
	return (0);

...
sdev_prof_free(struct sdev_node *dv)
{
	...
	if (dv->sdev_prof.dev_name_ann)
		nvlist_free(dv->sdev_prof.dev_name_ann);
	...
	if (dv->sdev_prof.dev_glob_ann)
		nvlist_free(dv->sdev_prof.dev_glob_ann);

	if (dv->sdev_origin != NULL && SDEVTOV(dv)->v_type == VDIR)
...
/*
 * Return a pointer to the second component of a node's /dev path
 */
static char *
walk_dev_prefix(struct sdev_node *dv)
{
	char *p = dv->sdev_path;
...
sdev_match(struct sdev_node *dv)
{
	...
	for (i = 0; vtab[i].vt_name; i++) {
		...
		if (vtab[i].vt_flags & SDEV_SUBDIR) {
			char *ptr;
			if ((ptr = walk_dev_prefix(dv)) == NULL)
				continue;
...
/*
 * If vnode is a device, return special vnode instead (though it knows
 * all about -us- via sp->s_realvp).  If this is a non-global mount,
 * interpose on the node, so we can re-assert any device annotations if
 * needed.
 */
int
sdev_to_vp(struct sdev_node *dv, struct vnode **vpp)
{
	...
	*vpp = specvp(vp, vp->v_rdev, vp->v_type, kcred);
	...
	if (!(dv->sdev_flags & SDEV_ANNOTATED))
		return (0);

	/*
	 * Note that we do *not* hold the vnode: we want the inactive
	 * hook to fire, so we can remove ourselves.
	 */
	rv = fem_install(*vpp, sdev_femops, *vpp, OPUNIQ, NULL, NULL);

	/*
	 * EBUSY just implies we've installed before, which is OK.
	 */
	if (rv == EBUSY)
		rv = 0;
...
sdev_stale(struct sdev_node *ddv)
{
	...
	for (dv = SDEV_FIRST_ENTRY(ddv); dv; dv = next) {
		...
		mutex_enter(&vp->v_lock);
		if (vp->v_count != 0) {
			/* cleanup will be done by devname_inactive_func() */
			mutex_exit(&vp->v_lock);
			/*
			 * Add a VN_HOLD() to the parent so it sticks
			 * around as long as we do, since sdev_dotdot needs to
			 * remain valid.
			 *
			 * Even though we dropped v_lock, dv can't disappear,
			 * since we're holding ddv->sdev_contents, which is
			 * grabbed by devname_inactive_func() before freeing
			 * a node.
			 */
			VN_HOLD(SDEVTOV(ddv));
			continue;
		}
		dv->sdev_nlink--; /* remove link to ourselves */
		if (vp->v_type == VDIR)
			dv->sdev_nlink--; /* once again, for '.' */
		mutex_exit(&vp->v_lock);
		sdev_nodedestroy(dv, 0);
	}
    ...
    void (*callback)(struct vnode *))
{
	...
	rw_enter(&ddv->sdev_contents, RW_WRITER);
	stale = dv->sdev_flags & SDEV_STALE;
	...
	/*
	 * last ref count on the ZOMBIE (or STALE) node is released.
	 * clean up the sdev_node, and
	 * release the hold on the backing store node so that
	 * the backing stores also cleaned out.
	 */
	if (vp->v_count == 1 && (dv->sdev_state == SDEV_ZOMBIE || stale)) {
		ASSERT(ddv);
		ASSERT(dv->sdev_nlink > 0);

		if (!stale) {
			/* Already done for SDEV_STALE in sdev_stale() */
			ASSERT(ddv->sdev_nlink > 2);
			ddv->sdev_nlink--;
			avl_remove(&ddv->sdev_entries, dv);
		}
		...
		sdev_nodedestroy(dv, 0);
		/*
		 * If the node was SDEV_STALE, then release the hold placed
		 * in sdev_stale() so ddv can be cleaned up if this was the

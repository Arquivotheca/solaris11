...
xdr_READ3uiores(XDR *xdrs, READ3uiores *objp)
{
	...
	if (xdrs->x_ops == &xdrrdma_ops) {
		struct clist *cl;
		rdma_wlist_conn_info_t	wci;

		XDR_CONTROL(xdrs, XDR_RDMA_GET_WCINFO, &wci);
		...
		if (objp->wlist) {
			...
			objp->wlist_len = clist_len(cl);
			if (ocount != objp->count || (objp->wlist_len !=
			    roundup(ocount, BYTES_PER_XDR_UNIT))) {
				DTRACE_PROBE3(xdr__e__read3uiores_fail,
				    int, ocount, int, objp->count, int,
				    objp->wlist_len);
				objp->wlist = NULL;
				return (FALSE);
			}
			XDR_CONTROL(xdrs, XDR_RDMA_GET_FLAGS, &xdr_flag);
			if (xdr_flag & XDR_RDMA_WLIST_PREREG) {
				ASSERT(cl->rb_longbuf.addr != NULL);
				ASSERT(objp->wlist_len <= cl->rb_longbuf.len);
				ASSERT(cl->rb_longbuf.addr == cl->u.c_daddr3);
				error = uiomove(cl->u.c_daddr3,
				    ocount, UIO_READ, uiop);
				if (error)
					return (FALSE);

...
fsh_entry_t *
rfs4_dbe_getfse(rfs4_dbe_t *entry)
{
	return (entry->dbe_table->dbt_db->db_fse);
}
...
rfs_inst_t *
rfs4_dbe_rip(rfs4_dbe_t *entry)
{
	return (entry->dbe_table->dbt_rip);
}
...
rfs4_database_t *
rfs4_database_create(uint32_t flags, fsh_entry_t *fse)
{
	rfs4_database_t *db;
 ...
 * of the tables are destroyed and then the database itself freed.
 */
void
...
rfs4_table_t *
rfs4_table_create(rfs_inst_t *rip, rfs4_database_t *db, char *tabname,
    ...
    uint32_t maxentries, id_t start)
{
	...
	int		 len;
	char		*fs_name = "";
	char		unique_nm[RFS_UNIQUE_BUFLEN], name[RFS_UNIQUE_BUFLEN];
	...
	/*
	 * If we are using the file system state store, then we will have
	 * multiple tables for this state.  Therefore, we need to give it
	 * a unique name.
	 */
	if (db->db_fse)
		fs_name = db->db_fse->fse_name;
	len = strlen(tabname) + strlen(fs_name);
	table->dbt_name = kmem_alloc(len+1, KM_SLEEP);
	(void) sprintf(table->dbt_name, "%s%s", tabname, fs_name);
	...
	if (start >= 0) {
		...
		(void) snprintf(name, sizeof (name), "%s_id",
		    table->dbt_name);
		rfs_inst_uniqstr(rip, name, unique_nm, sizeof (unique_nm));
		table->dbt_id_space = id_space_create(unique_nm, start,
		    maxentries + start);
	}
	...
	(void) snprintf(name, sizeof (name), "%s_ec", table->dbt_name);
	rfs_inst_uniqstr(rip, name, table->dbt_cache_name,
	    sizeof (table->dbt_cache_name));

	table->dbt_mem_cache = kmem_cache_create(table->dbt_cache_name,
...
rfs4_dbe_reap(rfs4_table_t *table, time_t cache_time, uint32_t desired)
{
	...
	/* if this file system is frozen or in tsm, don't reap entries */
	if (!table->dbt_reaper_shutdown && table->dbt_db->db_fse != NULL &&
	    (table->dbt_db->db_fse->fse_state & FSE_FROZEN))
		return;
...
reaper_thread(caddr_t *arg)
{
	...
	rfs4_database_t *db = table->dbt_db;
	...
	mutex_enter(&table->dbt_reaper_cv_lock);
	while (table->dbt_reaper_shutdown == FALSE) {
		CALLB_CPR_SAFE_BEGIN(&table->dbt_reaper_cpr_info);
		(void) cv_reltimedwait(&table->dbt_reaper_wait,
	...
	if (db->db_shutdown_count == 0)
		cv_signal(&db->db_shutdown_wait);
	mutex_exit(db->db_lock);
...
rfs4_start_reaper(rfs4_table_t *table)
{
	...
	(void) zthread_create(NULL, 0, reaper_thread, table, 0, minclsyspri);
}

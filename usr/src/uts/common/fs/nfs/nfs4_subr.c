    ...
    enum clnt_stat *rpc_statusp, int flags, struct nfs4_clnt *nfscl)
{
	...
	do {
		...
		default:		/* probably RPC_TIMEDOUT */
			 ...
			 */
			if (mi->mi_vers == 4 && nfs4_failover_mount(mi) &&
...
static int
serv_path_cmp(servinfo4_t *o, servinfo4_t *n)
{
	int	oplen = o->sv_pathlen;
	int	nplen = n->sv_pathlen;

	return (oplen != nplen || bcmp(o->sv_path, n->sv_path, oplen));
}

int
netbuf_cmp(struct netbuf *o, struct netbuf *n)
{
	return ((o->len != n->len) || bcmp(o->buf, n->buf, o->len));
}

int
nfs4_src_dest_cmp(servinfo4_t *src, servinfo4_t *dest)
{
	return (netbuf_cmp(&src->sv_addr, &dest->sv_addr) ||
	    serv_path_cmp(src, dest));
}

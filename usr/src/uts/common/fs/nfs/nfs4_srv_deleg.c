 ...
 */
void
rfs4_set_deleg_policy(rfs_inst_t *rip, srv_deleg_policy_t new_policy)
{
	rw_enter(&rip->ri_v4.r4_deleg_policy_lock, RW_WRITER);
	rip->ri_v4.r4_deleg_policy = new_policy;
	rw_exit(&rip->ri_v4.r4_deleg_policy_lock);
}

void
rfs4_hold_deleg_policy(rfs_inst_t *rip)
{
	rw_enter(&rip->ri_v4.r4_deleg_policy_lock, RW_READER);
}

void
rfs4_rele_deleg_policy(rfs_inst_t *rip)
{
	rw_exit(&rip->ri_v4.r4_deleg_policy_lock);
}
...
rfs4_cbinfo_rele(rfs4_cbinfo_t *cbp, rfs4_cbstate_t newstate)
{
	 ...
	 * there ready to do the CB_NULL, then return true to the
...
rfs4_deleg_cb_check(rfs4_client_t *cp)
{
	...
	(void) zthread_create(NULL, 0, rfs4_do_cb_null_thread, cp, 0,
	    minclsyspri);
}
...
static void
do_recall_thread(struct recall_arg *arg)
{
	 ...
	 * recall callback.
	 */
	if ((dsp->rds_dtype != OPEN_DELEGATE_NONE) && !rip->ri_unregistered) {
...
static void
do_recall_file_thread(struct master_recall_args *map)
{
	...
	if (rfs_inst_active_tryhold(rip, TRUE) == 0) {
		rfs4_file_rele(fp);
		goto out;
	}
	...
	for (dsp = list_head(&fp->rf_delegstatelist); dsp != NULL;
		...
		/*
		 * if this file was just migrated and this client has not yet
		 * talked with the server to set up the call back channel,
		 * don't issue the recall and give it more time before revoke.
		 */
		if (dsp->rds_client->rc_cbinfo.cb_state == CB_UNINIT) {
			/* only do this once */
			if (dsp->rds_time_recalled == 0) {
				dsp->rds_time_recalled = 1;
		...
		(void) zthread_create(NULL, 0, do_recall_thread, arg, 0,
		    minclsyspri);
	}
    ...
    bool_t trunc, rfs4_client_t *cp)
{
	...
	(void) zthread_create(NULL, 0, do_recall_file_thread, args, 0,
	    minclsyspri);
}
 ...
 */
static open_delegation_type4
rfs4_delegation_policy(rfs_inst_t *rip, open_delegation_type4 dtype,
    rfs4_dinfo_t *dinfo, clientid4 cid)
{
	time_t elapsed;

	if (rip->ri_v4.r4_deleg_policy != SRV_NORMAL_DELEGATE)
		return (OPEN_DELEGATE_NONE);
 ...
 */
rfs4_deleg_state_t *
rfs4_grant_delegation(struct compound_state *cs, delegreq_t dreq,
    rfs4_state_t *sp, int *recall)
{
	rfs4_file_t *fp = sp->rs_finfo;
	rfs_inst_t *rip = rfs4_dbe_rip(sp->rs_dbe);
	...
	mutex_enter(&rip->ri_v4.r4_deleg_lock);
	no_delegation = rip->ri_v4.r4_deleg_disabled;
	mutex_exit(&rip->ri_v4.r4_deleg_lock);
	...
	/*
	 * Don't grant a delegation if this is an active shadow vnode.
	 * Otherwise, when a client with a read delegation makes a read
	 * request, we're forced to break the delegation when we try to write
	 * to the local file.  But because the client is in the middle of a
	 * read the revoke won't succeed and we end up timing out on every
	 * operation.
	 */
	if (fp->rf_vp != NULL && vn_is_shadow(fp->rf_vp)) {
		return (NULL);
	}
	...
	case DELEG_ANY:
		 ...
		 * actual delegation.
		 */
		dtype = rfs4_delegation_policy(rip, dtype, &fp->rf_dinfo,
...
/*
 * Check if the file is delegated in the case of klm access.
 * Return TRUE if it is delegated.
 */
bool_t
rfs4_check_delegated(int mode, vnode_t *vp, bool_t trunc)
{
	...
	if ((rip = rfs_inst_find(FALSE)) == NULL)
		return (rc);

	if (rip->ri_v4.r4_enabled)
		rc = rfs4_check_delegated_byrip(rip, mode, vp, trunc);
 ...
 */
bool_t
rfs4_check_delegated_byrip(rfs_inst_t *rip, int mode, vnode_t *vp, bool_t trunc)
{
	rfs4_file_t *fp;
	bool_t create = FALSE;
	bool_t rc = FALSE;
	...
	rfs4_hold_deleg_policy(rip);

	fse = fsh_get_ent(rip, vp->v_vfsp->vfs_fsid);
	if (fse == NULL) {
		rfs4_rele_deleg_policy(rip);
		return (FALSE);
	}
...
rfs4_deleg_state(struct compound_state *cs, rfs4_state_t *sp,
...
void
rfs4_disable_delegation(rfs_inst_t *rip)
{
	mutex_enter(&rip->ri_v4.r4_deleg_lock);
	rip->ri_v4.r4_deleg_disabled++;
	mutex_exit(&rip->ri_v4.r4_deleg_lock);
}

void
rfs4_enable_delegation(rfs_inst_t *rip)
{
	mutex_enter(&rip->ri_v4.r4_deleg_lock);
	ASSERT(rip->ri_v4.r4_deleg_disabled > 0);
	rip->ri_v4.r4_deleg_disabled--;
	mutex_exit(&rip->ri_v4.r4_deleg_lock);
}

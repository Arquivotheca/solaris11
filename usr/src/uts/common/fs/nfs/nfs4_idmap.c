...
static void	 nfs_idmap_shutdown_zone(zoneid_t, void *);
...
nfs_idmap_init(void)
{
	 ...
	 */
	zone_key_create(&nfsidmap_zone_key, nfs_idmap_init_zone,
	    nfs_idmap_shutdown_zone, nfs_idmap_fini_zone);
}
...
/*ARGSUSED*/
static void
nfs_idmap_shutdown_zone(zoneid_t zoneid, void *arg)
{
	 ...
	 * Cleanup held vnodes.
	 *
	 * Since the door is established by the daemon in userland, and
	 * since userland is now gone, we know that it won't be re-set
	 * after this point.
	 */
...
nfs_idmap_fini_zone(zoneid_t zoneid, void *arg)
{
	...
	ASSERT(nig->nfsidmap_daemon_dh == NULL);
...
nfs_idmap_str_uid(utf8string *u8s, uid_t *uid, bool_t isserver)
{
	 ...
	 * running, we'll leverage its first flush to let the kernel know
	 * when it's up and available to perform mappings. Also, on client
	 * only, be smarter about when to issue upcalls by checking the
	 * string to see if it's a bare numeric ID.  If it is, then we just
...
retry:
	...
	if (dh == NULL || nig->nfsidmap_pid == curproc->p_pid ||
	    (is_stringified_id(u8s) && !isserver)) {
		if (dh)
			door_ki_rele(dh);

...
rfs4_attr_init(rfs_inst_t *rip)
{
	...
	rip->ri_v4.r4_supported_attrs = 0;
	for (i = 0; i < NFS4_MAXNUM_ATTRS; i++) {
		...
		if ((*nfs4_ntov_map[i].sv_getit)(NFS4ATTR_SUPPORTED,
		    &sarg, NULL) == 0) {
			rip->ri_v4.r4_supported_attrs |= nfs4_ntov_map[i].fbit;
		}
	}
}
 ...
 * The function will return NFS4_ATTR_ERR_RDATTR_OKAY if it found that the
	...
	union nfs4_attr_u *na)
{
	...
	case NFS4ATTR_GETIT:
		na->supported_attrs = rip->ri_v4.r4_supported_attrs;
		break;
	case NFS4ATTR_SETIT:
		/*
		 * read-only attr
	...
	case NFS4ATTR_VERIT:
		 ...
		 */
		if (na->supported_attrs != rip->ri_v4.r4_supported_attrs)
			error = NFS4_ATTR_ERR_RDATTR_OKAY;
		break;
...
fattr4_get_change(struct nfs4_svgetit_arg *sarg, fattr4_change *changep)
{
	...
	if ((vap->va_mask & AT_CTIME) == 0) {
		if (sarg->rdattr_error && (vp == NULL))
			return (NFS4_ATTR_ERR_RDATTR_OKAY);
		ASSERT(vp != NULL);
		vap = vap2;
	...
	union nfs4_attr_u *na)
{
	...
	case NFS4ATTR_GETIT:
		if (sarg->is_referral &&
		    sarg->cs->fse->fse_state == FSE_AVAILABLE) {
			na->fsid.major = 1;
			na->fsid.minor = 0;
	...
	case NFS4ATTR_VERIT:
		if (sarg->is_referral) {
			if (na->fsid.major != 1 || na->fsid.minor != 0)
				error = NFS4_ATTR_ERR_RDATTR_OKAY;
		} else if (sarg->cs->exi->exi_volatile_dev) {
...
rfs4_get_mntdfileid(nfs4_attr_cmd_t cmd, struct nfs4_svgetit_arg *sarg)
{
	int error = 0;
	vattr_t	*vap, va;
	vnode_t *stubvp = NULL, *zonevp, *vp;

	vp = sarg->cs->vp;
	sarg->mntdfid_set = FALSE;
	zonevp = RFS_INST_ROOTVP(sarg->cs->rip);

	/* VROOT object, untraverse if not zone root */
	if ((vp->v_flag & VROOT) && VN_CMP(vp, zonevp) == 0) {
		stubvp = vp;
		VN_HOLD(stubvp);
		stubvp = vn_under(stubvp);
		ASSERT(VN_CMP(vp, stubvp) == 0);
	 ...
	 *	   failure due to previous attr processing error (rddir only).
	...
	union nfs4_attr_u *na)
{
	...
	case NFS4ATTR_GETIT:
		fsl = fetch_referral(sarg->cs->rip, sarg->cs->vp, sarg->cs->cr);
		if (fsl == NULL)
			fsl = fetch_replicas(sarg->cs->rip,
			    sarg->cs->vp->v_vfsp, sarg->cs->cr);
		if (fsl == NULL)
			error = NFS4_ATTR_ERR_SILENT_IGNORE;
		else {
			na->fs_locations = *fsl;
			kmem_free(fsl, sizeof (fs_locations4));
			sarg->cs->got_fs_loca = TRUE;
		}

		nfsstat_inc_svstat(sarg->cs->rip, NFS_V4, NFS_REFERLINKS);
		nfsstat_inc_svstat(sarg->cs->rip, 0, NFS_REFERLINKS);
		break;
	...
	union nfs4_attr_u *na)
{
	...
	case NFS4ATTR_VERIT:
		...
		if (error == ENOTSUP)
			error = 0;
		else if (error || sarg->vap->va_uid != uid)
			error = NFS4_ATTR_ERR_RDATTR_OKAY;
		break;
	case NFS4ATTR_FREEIT:
		if (sarg->op == NFS4ATTR_GETIT) {

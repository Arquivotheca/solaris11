...
#define	FSTYPE_NFS	"nfs"

static int nfs_update_sharetab(sharefs_op_t, struct exp_share *, model_t);
static int	unexport(exportinfo_t *);
static void	exportfree(exportinfo_t *);
static int	loadindex(exportdata_t *);
static exportinfo_t *findexport(rfs_namespc_t *, fsid_t *, fid_t *, vnode_t *);
...
static bool_t rfs_gsscallback(struct svc_req *, gss_cred_id_t, gss_ctx_id_t,
    rpc_gss_lock_t *, void **);
 ...
 * this routine.
 *
 * This routine is used under the protection of
 * rfs_inst_t.ri_nm.rn_export_rwlock (RW_WRITER).
 */
void
...
srv_secinfo_treeclimb(exportinfo_t *exip, secinfo_t *sec, int seccnt, int isadd)
{
	...
	ASSERT(RW_WRITE_HELD(&exip->exi_rip->ri_nm.rn_export_rwlock));
	ASSERT(tnode);
	 ...
	 * also a vfs/zone root, its implicitly allowed flavors are
	 * inherited from from its pseudonode.  For exports which are
	 * vfs/zone roots, implicitly allowed flavors were transferred
	 * from its EX_PSEUDO exi in exportfs().
	 */
	if (isadd && !RFS_VN_ISROOT(exip->exi_rip, exip->exi_vp) &&
...
export_link(exportinfo_t *exi)
{
	...
	fsh_add(exi->exi_rip, exi->exi_fsid, FALSE);

	bckt = &rnp->rn_exptable[exptablehash(&exi->exi_fsid, &exi->exi_fid)];
	exp_hash_link(exi, fid_hash, bckt);

	bckt = &rnp->rn_exptable_path_hash[pkp_tab_hash(exi->exi_export.ex_path,
 ...
 * Should only be called once per server instance.
 */
void
nfs_exportinit(rfs_inst_t *rip)
{
	rfs_namespc_t *rnp = &rip->ri_nm;

	rnp->rn_exptable =
	    kmem_zalloc(EXPTABLESIZE * sizeof (exportinfo_t *), KM_SLEEP);
	rnp->rn_exptable_path_hash =
	    kmem_zalloc(PKP_HASH_SIZE * sizeof (exportinfo_t *), KM_SLEEP);

	rnp->rn_exi_public = NULL;
	rnp->rn_ns_root = NULL;
}
...
/* ARGSUSED */
void
nfs_exportfini(rfs_inst_t *rip)
{
	rfs_namespc_t *rnp = &rip->ri_nm;

	kmem_free(rnp->rn_exptable, EXPTABLESIZE * sizeof (exportinfo_t *));
	kmem_free(rnp->rn_exptable_path_hash,
	    PKP_HASH_SIZE * sizeof (exportinfo_t *));
}
 ...
 *				rpc_gss_lock_t *, void **),
 *
 * If no nfs pseudo number mapping can be found in the instance's
 * root pseudo export, assign the nfsflavor to NFS_FLAVOR_NOMAP.
 * In V4, we may recover the flavor mismatch from NFS layer
 * (NFS4ERR_WRONGSEC).
 *
 * For example:
 ...
 *
 * NFS_FLAVOR_NOMAP is defined in Solaris to work around
 * the implementation issue. This number should not overlap with
 * any new IANA defined pseudo flavor numbers.
 */
/*ARGSUSED*/
bool_t
rfs_gsscallback(struct svc_req *req, gss_cred_id_t deleg,
    gss_ctx_id_t gss_context, rpc_gss_lock_t *lock, void **cookie)
{
	...
	*cookie = NULL;
	...
	/*
	 * The sec flavor list in the instance's root
	 * pseudo export contains all flavors of its shares.
	 */
	rw_enter(&rnp->rn_export_rwlock, RW_READER);
	if (rnp->rn_ns_root != NULL)
		exi = rnp->rn_ns_root->tree_exi;
	if (exi != NULL && exi->exi_export.ex_seccnt > 0) {
		struct secinfo *secp;
		seconfig_t *se;
		int seccnt;
		...
		for (j = 0; j < seccnt; j++) {
			...
			if (! nfs_mech_equal(se->sc_gss_mech_type,
			    raw_cred->mechanism))
				continue;

			if (se->sc_service != raw_cred->service ||
			    raw_cred->qop != se->sc_qop)
				continue;

			/*
			 * Found it.  Grab pseudo flavor number
			 * and bail out.
			 */
			*cookie = (void *)(uintptr_t)se->sc_nfsnum;
			break;
		}
	}
...
static int
nfs_update_sharetab(sharefs_op_t opcode, struct exp_share *sh_in,
    model_t model)
{
	int error = 0;
	size_t name_len;
	...
	case SHAREFS_PUBLISH:
		shr_len = STRUCT_FGET(u_sh, shr_len);
		shr_buf = kmem_zalloc(shr_len, KM_SLEEP);
		if (ddi_copyin(STRUCT_FGETP(u_sh, shr_buf), shr_buf,
		    shr_len, 0)) {
			error = EFAULT;
			goto cleanup;
		}

		if ((error = nvlist_unpack(shr_buf, shr_len, &share,
		    KM_SLEEP)) != 0) {
			goto cleanup;
		}

		opt_len = STRUCT_FGET(u_sh, opt_len) + 1;
		opt_str = kmem_zalloc(opt_len, KM_SLEEP);
		if (ddi_copyin(STRUCT_FGETP(u_sh, opt_str), opt_str,
		    opt_len, 0)) {
			error = EFAULT;
			goto cleanup;
		}

		error = sharetab_add(share, FSTYPE_NFS, opt_str);
		break;
	case SHAREFS_UNPUBLISH:
		name_len = STRUCT_FGET(u_sh, name_len) + 1;
		shr_name = kmem_zalloc(name_len, KM_SLEEP);
		if (ddi_copyin(STRUCT_FGETP(u_sh, name), shr_name,
		    name_len, 0)) {
			error = EFAULT;
			goto cleanup;
		}

		error = sharetab_remove(shr_name, FSTYPE_NFS);
		break;
	default:
		error = EINVAL;
		break;
...
cleanup:
	if (share != NULL)
		nvlist_free(share);
	if (shr_buf != NULL)
		kmem_free(shr_buf, shr_len);
	if (opt_str != NULL)
		kmem_free(opt_str, opt_len);
	if (shr_name != NULL)
		kmem_free(shr_name, name_len);
 ...
 */
static int
exportfs_impl(rfs_inst_t *rip, struct exportfs_args *args, model_t model,
    cred_t *cr, sharefs_op_t shop)
{
	vnode_t *vp;
	vnode_t *dvp;
	...
	if (sharefs_secpolicy_share(lookpn.pn_path) != 0) {
		pn_free(&lookpn);
		return (EPERM);
	}
	...
	rw_enter(&rnp->rn_export_rwlock, RW_READER);
	DTRACE_PROBE(nfss__i__export_rwlock1_start);
	for (ex1 = rnp->rn_exptable_path_hash[pkp_tab_hash(lookpn.pn_path,
	    strlen(lookpn.pn_path))]; ex1; ex1 = ex1->path_hash.next) {
		if (0 == strcmp(ex1->exi_export.ex_path, lookpn.pn_path)) {
			exi_hold(ex1);
			break;
		}
	}
	DTRACE_PROBE(nfss__i__export_rwlock1_stop);
	rw_exit(&rnp->rn_export_rwlock);

	/* Is this an unshare? */
	if (shop == SHAREFS_UNPUBLISH) {
		pn_free(&lookpn);
		if (ex1 == NULL)
			error = EINVAL;
		else
			error = unexport(ex1);
		/*
		 * remove entry from sharetab
		 */
		(void) nfs_update_sharetab(shop,
		    STRUCT_FGETP(uap, shareinfo), model);
		if (ex1 != NULL)
			exi_rele(ex1);
		return (error);
	}
	...
	if (vfs_shrzid_ref(vp->v_vfsp, RFS_INST_ZONEID(rip))) {
		VN_RELE(vp);
		if (dvp != NULL)
			VN_RELE(dvp);
		pn_free(&lookpn);
	...
	if (ex1 && !PSEUDO(ex1) && !VN_CMP(ex1->exi_vp, vp)) {
		(void) vfs_shrzid_unref(vp->v_vfsp, RFS_INST_ZONEID(rip));
		VN_RELE(vp);
		if (dvp != NULL)
			VN_RELE(dvp);
		pn_free(&lookpn);
	 ...
	 */
	rw_enter(&rnp->rn_export_rwlock, RW_READER);
	DTRACE_PROBE(nfss__i__export_rwlock2_start);
	for (ex2 = rnp->rn_exptable[exptablehash(&fsid, &fid)]; ex2;
	    ex2 = ex2->fid_hash.next) {
		if (!PSEUDO(ex2) && VN_CMP(ex2->exi_vp, vp) &&
		    strcmp(ex2->exi_export.ex_path, lookpn.pn_path) != 0) {
			DTRACE_PROBE(nfss__i__export_rwlock2_stop);
			(void) vfs_shrzid_unref(vp->v_vfsp,
			    RFS_INST_ZONEID(rip));
			VN_RELE(vp);
			if (dvp != NULL)
				VN_RELE(dvp);
	...
	/* At least NFSv4 file handle must fit */
	if (exi->exi_fid.fid_len > NFS_FH4MAXDATA) {
		error = EREMOTE;
		goto out1;
	}
	...
	if (RFS_VN_ISROOT(exi->exi_rip, vp))
		kex->ex_flags |= EX_ROOT;
...
#ifndef VOLATILE_FH_TEST
	/*
	 * Reject volatile filehandle options on a non-debug kernel
	 */
	if (kex->ex_flags & (EX_VOLFH | EX_VOLRNM | EX_VOLMIG | EX_NOEXPOPEN)) {
		error = ENOTSUP;
		goto out1;
	}
#endif /* VOLATILE_FH_TEST */
	 ...
	 */
	rw_enter(&rnp->rn_export_rwlock, RW_WRITER);
	DTRACE_PROBE(nfss__i__export_rwlock3_start);
	...
	for (ex = exi->fid_hash.next; ex != NULL; ex = ex->fid_hash.next) {
		if (VN_CMP(ex->exi_vp, vp)) {
			export_unlink(ex);
			/* release the hold put on by export_link() */
			fsh_fsid_rele(ex->exi_rip, ex->exi_fsid);
			break;
		}
	}
	...
	if (ex == NULL) {
		...
		/*
		 * If it's a re-export update namespace tree, and do not count
		 * as a share reference.
		 */
		if (!PSEUDO(ex))
			(void) vfs_shrzid_unref(vp->v_vfsp,
			    RFS_INST_ZONEID(rip));
	...
	/*
	 * If this was shared as public, adjust rn_exi_public.
	 * Else if the old export was public or if this is the first export and
	 * rn_exi_public was not set yet, set it to the root export.
	 */
	if (kex->ex_flags & EX_PUBLIC) {
		rnp->rn_exi_public = exi;
		kex->ex_flags &= ~EX_PUBLIC;
	} else if (ex && ex == rnp->rn_exi_public || !rnp->rn_exi_public) {
		rnp->rn_exi_public = rnp->rn_ns_root->tree_exi;
	}
	...
	DTRACE_PROBE(nfss__i__export_rwlock3_stop);
	rw_exit(&rnp->rn_export_rwlock);

	/* Log share operation to this buffer only. */
	if (rnp->rn_exi_public == exi || kex->ex_flags & EX_LOG)
		nfslog_share_record(exi, cr);

	error = nfs_update_sharetab(shop, STRUCT_FGETP(uap, shareinfo), model);
	if (error) {
		int e = unexport(exi);
		DTRACE_PROBE3(nfss__i__nmspc__shtab, int, error, char *,
		    pathbuf, int, e);
	}
...
out7:
	...
	export_unlink(exi);
	/* release the hold put on by export_link() */
	fsh_fsid_rele(exi->exi_rip, exi->exi_fsid);
	DTRACE_PROBE(nfss__i__export_rwlock3_stop);
	rw_exit(&rnp->rn_export_rwlock);
out6:
	if (kex->ex_flags & EX_INDEX)
...
int
exportfs(struct exportfs_args *args, model_t model,
    cred_t *cr, sharefs_op_t shop)
{
	rfs_inst_t *rip;
	int err = ENOENT;
	...
	if (rip) {
		err = exportfs_impl(rip, args, model, cr, shop);
...
#ifdef DEBUG
	rfs_namespc_t *rnp = &exi->exi_rip->ri_nm;
	ASSERT(RW_WRITE_HELD(&rnp->rn_export_rwlock));
#endif
...
unexport(struct exportinfo *exi)
{
	...
	rfs_namespc_t *rnp = &rip->ri_nm;
	exportinfo_t *new_pub_share = NULL;
	...
	if (vfs_shrzid_unref(exi->exi_vp->v_vfsp, RFS_INST_ZONEID(rip))) {
		rw_exit(&rnp->rn_export_rwlock);
		return (EINVAL);
	}
	export_unlink(exi);
	...
	if (exi->exi_visible) {
		...
		newexi = pseudo_exportfs(exi->exi_rip, exi->exi_vp,
		    &exi->exi_fid, exi->exi_visible, &exi->exi_export);
	...
	/* Adjust rn_exi_public */
	if (rnp->rn_ns_root == NULL)
		rnp->rn_exi_public = NULL;
	else if (exi == rnp->rn_exi_public) {
		new_pub_share = rnp->rn_exi_public = rnp->rn_ns_root->tree_exi;
		exi_hold(new_pub_share);
	}

	mutex_enter(&rip->ri_lock);
	ASSERT(rip->ri_export_cnt > 0);
	rip->ri_export_cnt--;
	mutex_exit(&rip->ri_lock);
	rw_exit(&rnp->rn_export_rwlock);
	...
	/*
	 * Need to decrement the fse_refcnt and possibly release
	 * the fsh entry for this filesystem.
	 */
	fsh_fsid_rele(exi->exi_rip, exi->exi_fsid);

	/* Notify the lock manager that the filesystem is being unexported. */
	lm_unexport(exi);

	/* If there is a new rn_exi_public, log it */
	if (new_pub_share) {
		nfslog_share_record(new_pub_share, CRED());
		exi_rele(new_pub_share);
	}
...
void
rfs_inst_unexport_all(rfs_inst_t *rip)
{
	struct exportinfo *eip;
	int		exp_array_idx = 0;
	rfs_namespc_t	*rnp;
	...
	for (exp_array_idx = 0; exp_array_idx < EXPTABLESIZE; exp_array_idx++) {
		eip = rnp->rn_exptable[ exp_array_idx ];

		while (eip) {
			if (PSEUDO(eip)) {
				eip = eip->fid_hash.next;
				continue;
			}
			exi_hold(eip);
			rw_exit(&rnp->rn_export_rwlock);
...
nfs_getfh(struct nfs_getfh_args *args, model_t model, cred_t *cr)
{
	...
	if (rip != NULL) {
		exi = nfs_vptoexi(rip, dvp, vp, cr, NULL, &error, FALSE);
		rfs_inst_active_rele(rip);
	} else
		error = EINVAL;
 ...
 */
struct   exportinfo *
nfs_vptoexi(rfs_inst_t *rip, vnode_t *dvp, vnode_t *vp, cred_t *cr, int *walk,
	int *err,  bool_t v4srv)
{
	...
	for (;;) {
		...
		if (v4srv)
			exi = checkexport4(rip, &vp->v_vfsp->vfs_fsid,
			    &fid, vp);
		else
			exi = checkexport(rip, &vp->v_vfsp->vfs_fsid, &fid,
			    FALSE);
		...
		/*
		 * Export wasn't found.  If vp is the vfs/zone root, then
		 * it's time to stop (with failure).
		 */
		if (RFS_VN_ISROOT(rip, vp)) {
			error = EINVAL;
			break;
		}
...
makefh(fhandle_t *fh, vnode_t *vp, exportinfo_t *exi)
{
	...
	if (exi->exi_fid.fid_len > NFS_FHMAXDATA)
		return (EREMOTE);

	bzero(&fid, sizeof (fid));
	fid.fid_len = NFS_FHMAXDATA;
	if (VOP_FID(vp, &fid, NULL) != 0)
		return (EREMOTE);

	bzero(fh, sizeof (fh));
	fh->fh_fsid = exi->exi_fsid;
	fh->fh_len = fid.fid_len;
	bcopy(fid.fid_data, fh->fh_data, fh->fh_len);
...
makefh4(nfs_fh4 *fh, vnode_t *vp, struct exportinfo *exi)
{
	...
	fh_fmtp->fh4_i.fhx_fsid = exi->exi_fsid;
	fh_fmtp->fh4_i.fhx_xlen = exi->exi_fid.fid_len;

	bzero(fh_fmtp->fh4_i.fhx_data, sizeof (fh_fmtp->fh4_i.fhx_data));
	bzero(fh_fmtp->fh4_i.fhx_xdata, sizeof (fh_fmtp->fh4_i.fhx_xdata));
	bcopy(exi->exi_fid.fid_data, fh_fmtp->fh4_i.fhx_xdata,
	    exi->exi_fid.fid_len);
...
/*
 * Find the exportinfo_t using fsid and fid. If vp is given, check if vp is the
 * same vnode as the exported node. Since VOP_FID of a lofs node returns the
 * fid of its real node (ufs), the exported node for lofs and (pseudo) ufs may
 * have the same fsid and fid.
 */
static exportinfo_t *
findexport(rfs_namespc_t *rnp, fsid_t *fsid, fid_t *fid, vnode_t *vp)
{
	struct exportinfo *exi = rnp->rn_exptable[exptablehash(fsid, fid)];

	for (; exi != NULL; exi = exi->fid_hash.next)
		if (exportmatch(exi, fsid, fid) &&
		    (vp == NULL || vp == exi->exi_vp))
			return (exi);
	return (NULL);
}
 ...
 */
struct exportinfo *
checkexport(rfs_inst_t *rip, fsid_t *fsid, fid_t *fid, bool_t publicfh)
{
	struct exportinfo *exi;
	rfs_namespc_t *rnp = &rip->ri_nm;

	rw_enter(&rnp->rn_export_rwlock, RW_READER);
	exi = publicfh ? rnp->rn_exi_public : findexport(rnp, fsid, fid, NULL);
	if (exi)
		exi_hold(exi);
	rw_exit(&rnp->rn_export_rwlock);
	return (exi);
 ...
 * rfs4_compound holds rfs_inst_t.ri_nm.rn_export_rwlock for
 * duration of compound processing.  This version doesn't
 * manipulate exi_count since NFS4 breaks fundamental
 * assumptions in the exi_count design.
 */
struct exportinfo *
checkexport4(rfs_inst_t *rip, fsid_t *fsid, fid_t *fid, vnode_t *vp)
{
	ASSERT(RW_LOCK_HELD(&rip->ri_nm.rn_export_rwlock));
	return (findexport(&rip->ri_nm, fsid, fid, vp));
}
...
find_volrnm_fh(struct exportinfo *exi, nfs_fh4 *fh4p)
{
	...
	/* XXX shouldn't we assert rn_export_rwlock held? */

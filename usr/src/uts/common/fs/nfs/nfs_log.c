...
static void log_buffer_rele(struct log_buffer *, rfs_zone_t *);
 ...
 */
void
nfslog_init(rfs_zone_t *rzp)
{
	struct nfslog_mem *mp;
	char	cname[RFS_UNIQUE_BUFLEN];

	rw_init(&rzp->rz_nfslog_buffer_list_lock, NULL, RW_DEFAULT, NULL);
	rzp->rz_nfslog_mem_alloc = kmem_zalloc(sizeof (struct nfslog_mem) * 4,
	    KM_SLEEP);
	mp = rzp->rz_nfslog_mem_alloc;

	rfs_zone_uniqstr(rzp, NFSLOG_SMALL_REC_NAME, cname, sizeof (cname));
	mp->size = NFSLOG_SMALL_RECORD_SIZE - sizeof (struct lr_alloc);
	mp->mem_cache = kmem_cache_create(cname,
	    mp->size + sizeof (struct lr_alloc),
	    0, NULL, NULL, NULL, NULL, NULL, 0);
	mp++;

	rfs_zone_uniqstr(rzp, NFSLOG_MEDIUM_REC_NAME, cname, sizeof (cname));
	mp->size = NFSLOG_MEDIUM_RECORD_SIZE - sizeof (struct lr_alloc);
	mp->mem_cache = kmem_cache_create(cname,
	    mp->size + sizeof (struct lr_alloc),
	    0, NULL, NULL, NULL, NULL, NULL, 0);
	mp++;

	rfs_zone_uniqstr(rzp, NFSLOG_LARGE_REC_NAME, cname, sizeof (cname));
	mp->size = NFSLOG_LARGE_RECORD_SIZE - sizeof (struct lr_alloc);
	mp->mem_cache = kmem_cache_create(cname,
	    mp->size + sizeof (struct lr_alloc),
	    0, NULL, NULL, NULL, NULL, NULL, 0);
	mp++;
...
void
nfslog_fini(rfs_zone_t *rzp)
{
	struct nfslog_mem *mp = rzp->rz_nfslog_mem_alloc;

	kmem_cache_destroy(mp[0].mem_cache);
	kmem_cache_destroy(mp[1].mem_cache);
	kmem_cache_destroy(mp[2].mem_cache);
	kmem_free(mp, sizeof (struct nfslog_mem) * 4);
}
...
nfslog_setup(struct exportinfo *exi)
{
	 ...
	 * one.
	 */
	rw_enter(&rzp->rz_nfslog_buffer_list_lock, RW_READER);
	for (lbp = rzp->rz_nfslog_buffer_list;
	    lbp != NULL;
	    lbp = lbp->lb_next) {
		LOGGING_DPRINT((10,
		...
		if (strcmp(lbp->lb_path, kex->ex_log_buffer) == 0) {
			...
			LOGGING_DPRINT((10,  "\tfound log_buffer for '%s'\n",
			    kex->ex_log_buffer));
			rw_exit(&rzp->rz_nfslog_buffer_list_lock);
			return (0);
		}
	}
	rw_exit(&rzp->rz_nfslog_buffer_list_lock);
	...
	rw_enter(&rzp->rz_nfslog_buffer_list_lock, RW_WRITER);
	for (lbp = rzp->rz_nfslog_buffer_list; lbp != NULL;
	    lbp = lbp->lb_next) {
		if (strcmp(lbp->lb_path, kex->ex_log_buffer) == 0) {
				/*
	...
	nlbp->lb_next = rzp->rz_nfslog_buffer_list;
	rzp->rz_nfslog_buffer_list = nlbp;
...
{
	log_buffer_rele(exi->exi_logbuffer, exi->exi_rip->ri_rzone);
}
 ...
 */
static void
log_buffer_rele(struct log_buffer *lbp, rfs_zone_t *rzp)
{
	int len;
	ASSERT(rzp);
	...
	if (lbp->lb_refcnt == 1) {
		...
		if (lbp == rzp->rz_nfslog_buffer_list) {
			rzp->rz_nfslog_buffer_list = lbp->lb_next;
		} else {
			struct log_buffer *tlbp;

			/* Drop the log_buffer from the master list */
			for (tlbp = rzp->rz_nfslog_buffer_list;
			    tlbp->lb_next != NULL;
			    tlbp = tlbp->lb_next) {
				if (tlbp->lb_next == lbp) {
	...
	int flags)
{
	...
	lrp = (struct lr_alloc *)
	    kmem_cache_alloc(rzp->rz_nfslog_mem_alloc[alloc_indx].mem_cache,
	    KM_NOSLEEP);
	...
	lrp->size = rzp->rz_nfslog_mem_alloc[alloc_indx].size;
	lrp->alloc_cache = rzp->rz_nfslog_mem_alloc[alloc_indx].mem_cache;
	lrp->exi = exi;
...
nfslog_record_append2all(struct lr_alloc *lrp)
{
	...
	rfs_zone_t *rzp = lrp->exi->exi_rip->ri_rzone;

	rw_enter(&rzp->rz_nfslog_buffer_list_lock, RW_READER);
	if ((lbp = rzp->rz_nfslog_buffer_list) != NULL)
		LOG_BUFFER_HOLD(lbp);
...
nfslog_do_flush(struct flush_thread_params *tparams)
{
	...
	rzp = rfs_zone_find(curproc->p_zone->zone_id, FALSE);
	if (rzp == NULL)
		return;
	...
	unsigned int *nfslog_rec_id)
{
	...
	if (exi != exi->exi_rip->ri_nm.rn_exi_public)
		return (NULL);
	...
	if (req->rq_prog == NFS_PROGRAM) {
		...
		case NFS_V3:
			if ((req->rq_proc == NFSPROC3_LOOKUP) &&
				...
				exi_ret = checkexport(exi->exi_rip,
				    &fh3->fh3_fsid, FH3TOXFIDP(fh3), FALSE);
			}
			break;
		...
		case NFS_VERSION:
			if ((req->rq_proc == RFS_LOOKUP) &&
				...
				exi_ret = checkexport(exi->exi_rip,
				    &fh->fh_fsid, (fid_t *)&fh->fh_xlen, FALSE);
			}
			break;
		default:
			break;
	...
	unsigned int record_id, unsigned int which_buffers)
{
	...
	do {
		...
		if (rzp->rz_nfslog_mem_alloc[alloc_indx].size == (-1)) {
			cmn_err(CE_WARN,
		...
		xdrmem_create(&xdrs, buffer,
		    rzp->rz_nfslog_mem_alloc[alloc_indx].size, XDR_ENCODE);

		/*
		 * Encode the header, args and results of the record
		 */
		if (xdr_nfslog_request_record(&xdrs, exi, req, cr, pnb,
		    rzp->rz_nfslog_mem_alloc[alloc_indx].size, record_id) &&
...
static char *
get_publicfh_path(rfs_inst_t *rip, int *alloc_length)
{
	char *pubpath = NULL;
	...
	if (rnp->rn_exi_public != NULL) {
		*alloc_length = rnp->rn_exi_public->exi_export.ex_pathlen + 1;
		pubpath = kmem_alloc(*alloc_length, KM_SLEEP);
		(void) strcpy(pubpath, rnp->rn_exi_public->exi_export.ex_path);
	}
...
log_public_record(struct exportinfo *exi, cred_t *cr)
{
	...
	if ((args.name = get_publicfh_path(exi->exi_rip, &free_length)) == NULL)
		return;
	args.dirp = &args.dir;
...
nfslog_share_record(struct exportinfo *exi, cred_t *cr)
{
	...
	if (rzp->rz_nfslog_buffer_list == NULL)
		return;

...
tryagain:
	...
	if (!error) {
		...
		/*
		 * If changing the size of the file, do not rely
		 * on attr cache update routine to update the file size.
		 * There are situations where the attrcache logic fails
		 * to update the r_size.
		 * For eg:- if r_count is non-zero due to a pending
		 * 		read-ahead/putpage operation.
		 * We do an explicit update of the r_size here
		 * since we have already updated it on the server.
		 */
		if (mask & AT_SIZE) {
			mutex_enter(&rp->r_statelock);

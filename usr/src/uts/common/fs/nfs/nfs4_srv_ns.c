 ...
 */
struct exportinfo *
pseudo_exportfs(rfs_inst_t *rip, vnode_t *vp, fid_t *fid,
    struct exp_visible *vis_head, struct exportdata *exdata)
{
	...
	ASSERT(RW_WRITE_HELD(&rip->ri_nm.rn_export_rwlock));
	...
	kex = &exi->exi_export;
	kex->ex_flags = EX_PSEUDO | EX_ROOT;
 ...
 * Return TRUE if tree becomes epmty.
 */
static bool_t
...
treeclimb_export(struct exportinfo *exip)
{
	...
	rfs_inst_t *rip = exip->exi_rip;
	rfs_namespc_t *rnp = &rip->ri_nm;
	vnode_t *zonevp = RFS_INST_ROOTVP(exip->exi_rip);
	...
	for (;;) {
		...
		if (! exportdir) {
			/*
			 * Check if this exportroot is a vfs/zone root.
			 * If so, then attach the pseudonodes.  If not,
			 * then continue .. traversal until we hit an
			 * export that's also a vfs/zone root.
			 */
			exi = checkexport4(rip, &vp->v_vfsp->vfs_fsid,
			    &fid, vp);
			if (exi != NULL && RFS_VN_ISROOT(exip->exi_rip, vp)) {
		...
		/*
		 * If at the root of a filesystem, then untraverse
		 * to reach the mounted-over stub and continue the
		 * climb.  If vp is also a zone root, then climb
		 * no further.
		 */
		if (RFS_VN_ISROOT(exip->exi_rip, vp)) {

			if (! exportdir) {
				/*
				 * Found the root directory of a vfs/zone
				 * that isn't exported so create an EX_PSEUDO
				 * exportinfo.
				 */
				new_exi = pseudo_exportfs(rip, vp, &fid,
				    vis_head, NULL);
				vis_head = NULL;
			}
			...
			if (VN_CMP(vp, zonevp)) {
				/*
				 * If vp is the zone root, then set
				 * this instance's namespace root.
				 */
				rnp->rn_ns_root =
				    tree_prepend_node(tree_head, 0, new_exi);
				break;
			}
	...
	if (error) {
		...
		while (tree_head) {
			...
			if (e && e != exip) {
				export_unlink(e);
				/* rele the hold put on by export_link() */
				fsh_fsid_rele(e->exi_rip, e->exi_fsid);
				exi_rele(e);
			}
			tree_head = tree_head->tree_child_first;
...
treeclimb_unexport(struct exportinfo *exip)
{
	...
	while (tnode) {
		...
		/* Stop at vfs/zone root node which is exported or has child */
		if (TREE_ROOT(tnode) &&
		...
		if (TREE_ROOT(tnode) && !TREE_EXPORTED(tnode) &&
			...
			/* rele the hold put on by export_link() */
			fsh_fsid_rele(tnode->tree_exi->exi_rip,
			    tnode->tree_exi->exi_fsid);
		...
		if (old_nd->tree_child_first == NULL && !TREE_EXPORTED(old_nd))
			if (TRUE == tree_remove_node(old_nd))
				exip->exi_rip->ri_nm.rn_ns_root = NULL;
	}
}
...
has_visible(struct exportinfo *exi, vnode_t *vp)
{
	 ...
	 */
	if (vp_is_exported && RFS_VN_ISROOT(exi->exi_rip, vp))
...
nfs_visible(struct exportinfo *exi, vnode_t *vp, int *expseudo)
{
	...
	/*
	 * export roots are always visible.
	 */
	...
	/*
	 * vp is not the export root, but it could
	 * be on the vis list.  A vis list is only
	 * attached to an exi which is either a vfs
	 * root or zone root.
	 */
	if (! PSEUDO(exi))
		exi = get_root_export(exi);
...
nfs_exported(struct exportinfo *exi, vnode_t *vp)
{
	 ...
	 * pseudonode is visible.
	 *
	 * Note: VN_CMP() won't match for LOFS shares, but
	 * EQFID/EQFSID below will.
	 */
	if (VN_CMP(vp, exi->exi_vp))
		return (1);
	...
	/*
	 * vp is not the export root, but it might be in the visible list.
	 */
	for (visp = exi->exi_visible; visp; visp = visp->vis_next) {
		if (EQFID(&fid, &visp->vis_fid))

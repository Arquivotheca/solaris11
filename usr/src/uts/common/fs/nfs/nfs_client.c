...
nfs_attrcache_va(vnode_t *vp, struct vattr *va)
{
	 ...
	 * out, and the RMODINPROGRESS flag is not set.  If RMODINPROGRESS
	 * is set we must skip updating the file size here, as we have a writer
	 * in writerp() that might be extending the file, but has not
	 * yet updated r_size.
	 */
...
nfs_mi_shutdown(zoneid_t zoneid, void *data)
{
	...
	for (;;) {
		...
		/* Tell the the async manager to exit. */
		mutex_enter(&mi->mi_lock);
		...
		VFS_RELE(mi->mi_vfsp); /* hold was in nfs_mi_zonelist_add */
	}
}
...
nfs_mi_destroy(zoneid_t zoneid, void *data)
{
	...
	ASSERT(mig != NULL);
	mutex_enter(&mig->mig_lock);
	ASSERT(list_head(&mig->mig_list) == NULL);
...
nfs_mi_zonelist_add(mntinfo_t *mi)
{
	...
	zone_hold_ref(mi->mi_zone, &mi->mi_zone_ref, ZONE_REF_NFS);
...
/*
 * All mntinfo_t kstat's have to be deleted before freeing any
 * of mntinfo_t members.
 */
void
nfs_mnt_kstat_free(struct vfs *vfsp)
{
	mntinfo_t *mi = VFTOMI(vfsp);

...
nfs4_attrcache_va(vnode_t *vp, nfs4_ga_res_t *garp, int set_cache_timeout)
{
	 ...
	 * out, and the R4MODINPROGRESS flag is not set.  If R4MODINPROGRESS
	 * is set we must skip updating the file size here, as we have a writer
	 * in writerp4() that might be extending the file, but has not
	 * yet updated r_size.
	 */
...
/*
 * All mntinfo4_t kstat's have to be deleted before freeing any
 * of mntinfo4_t members.
 */
void
nfs4_mnt_kstat_free(struct vfs *vfsp)
{
	mntinfo4_t *mi = VFTOMI4(vfsp);
	/* Grab the mi_kslock before deleting */
	mutex_enter(&mi->mi_kslock);
...
nfs4_mi_shutdown(zoneid_t zoneid, void *data)
{
	...
	for (;;) {
		...
		/*
		 * Release holds done in nfs4_mi_zonelist_add().
		 */
 ...
 * Fail if the zone is going away, as it may have already executed its
 * _shutdown() callbacks.
 */
int
nfs4_mi_zonelist_add(mntinfo4_t *mi)
{
	...
	mutex_enter(&mig->mig_lock);
	if (zone_status_get(mi->mi_zone) >= ZONE_IS_SHUTTING_DOWN) {
		mutex_exit(&mig->mig_lock);
		return (-1);
	}
	zone_init_ref(&mi->mi_zone_ref);
	zone_hold_ref(mi->mi_zone, &mi->mi_zone_ref, ZONE_REF_NFSV4);
...
nfs4_renew_lease_thread(nfs4_server_t *sp)
{
	...
	l4p = &sp->s_lease;
	/* l4p->l4_time is set via nfs4setclientid_otw */
	l4p->l4_last = gethrestime_sec();
	l4p->l4_flags |= L4_THR_ACTIVE;
	l4p->l4_renew = curthread;
	...
	for (;;) {
		if (!sp->state_ref_count || !(l4p->l4_flags & L4_LSE_VALID)) {
			...
			NFS4_DEBUG(nfs4_client_lease_debug, (CE_NOTE,
			    ...
			    "state_ref_count %d, l4_flags = 0x%x",
			    sp->state_ref_count, l4p->l4_flags));

			mutex_enter(&cpr_lock);
			CALLB_CPR_SAFE_BEGIN(&cpr_info);
			mutex_exit(&cpr_lock);

			time_left = cv_reltimedwait(&l4p->l4_cv, &sp->s_lock,
		...
		tmp_last_renewal_time = l4p->l4_last;
		tmp_time = gethrestime_sec() - l4p->l4_last;

		NFS4_DEBUG(nfs4_client_lease_debug, (CE_NOTE,
		    "nfs4_renew_lease_thread: tmp_time %ld, "
		    "l4p->l4_last %ld", tmp_time, l4p->l4_last));

		kip_secs = MAX((l4p->l4_time >> 1) - tmp_time, 1);
		tick_delay = SEC_TO_TICK(kip_secs);
		...
		NFS4_DEBUG(nfs4_client_lease_debug, (CE_NOTE,
		    ...
		    "l4_last %ld, nfs4_client_resumed %ld, "
		    "tmp_last_renewal_time %ld", time_left,
		    l4p->l4_last, nfs4_client_resumed,
		    tmp_last_renewal_time));

		if (l4p->l4_flags & L4_THR_KILL)
			goto die;

		if (tmp_last_renewal_time == l4p->l4_last ||
		    (nfs4_client_resumed != 0 &&
		    nfs4_client_resumed > l4p->l4_last)) {
			/*
			...
			/*
			 * See if someone changed l4_flags to set the
			 * thread to exit while we gave up s_lock.
			 */
			if (l4p->l4_flags & L4_THR_KILL)
				goto die;
...
void
nfs4renew_otw_with_clid(mntinfo4_t *mi, nfs4_server_t *sp,
    clientid4 clid, nfs4_error_t *ep)
{
	COMPOUND4args_clnt args;
	COMPOUND4res_clnt res;
	nfs_argop4 argop[1];
	...
	argop[0].nfs_argop4_u.oprenew.clientid = clid;
...
static void
nfs4renew_otw_with_tag(mntinfo4_t *mi, nfs4_server_t *sp, nfs4_error_t *ep,
    nfs4_tag_type_t tag)
{
	COMPOUND4args_clnt args;
	COMPOUND4res_clnt res;
...
void
nfs4_probe_clid(mntinfo4_t *mi, nfs4_server_t *sp, nfs4_error_t *ep)
{
	nfs4renew_otw_with_tag(mi, sp, ep, TAG_PROBE_CLID);
}

/*
 * Send a renew OP to the server, let the caller interpret
 * the errors.
 */
void
nfs4renew_otw(mntinfo4_t *mi, nfs4_server_t *sp, nfs4_error_t *ep)
{
	nfs4renew_otw_with_tag(mi, sp, ep, TAG_RENEW);
}
...
recov_retry:
	...
	mutex_enter(&sp->s_lock);
	if (sp->s_lease.l4_flags & L4_THR_KILL) {
		mutex_exit(&sp->s_lock);
	...
	nfs4renew_otw(mi, sp, &e);

	DTRACE_PROBE2(nfs4__renew__end, nfs4_server_t *, sp,
	    mntinfo4_t *, mi);

	if (e.error == 0 && e.stat == NFS4ERR_CB_PATH_DOWN) {
...
nfs4_inc_state_ref_count_nolock(nfs4_server_t *sp, mntinfo4_t *mi)
{
	 ...
	 * on lease renewal.
	 */
	if ((l4p->l4_flags & L4_LSE_VALID) && sp->state_ref_count == 1)
		l4p->l4_last = gethrestime_sec();
...
inlease(nfs4_server_t *sp)
{
	...
	if ((l4p->l4_flags & L4_LSE_VALID) &&
	    gethrestime_sec() < l4p->l4_last + l4p->l4_time)
		result = TRUE;
	else
		result = FALSE;

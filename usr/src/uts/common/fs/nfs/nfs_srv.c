	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	ct.cc_sysid = 0;
	ct.cc_pid = 0;
	ct.cc_caller_id = rfs.rg_v2.rg2_caller_id;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	if (PUBLIC_FH2(fhp)) {
		publicfh_flag = TRUE;
		error = rfs_publicfh_mclookup(rip, name, dvp, cr, &vp, &exi,
		    &sec);
	} else {
		/*
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (is_referral) {
		...
		nfsstat_inc_svstat(exi->exi_rip, NFS_V2, NFS_REFERLINKS);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	vip = &rip->ri_v2;
	mutex_enter(&vip->r2_async_write_lock);
	for (lp = vip->r2_async_write_head; lp != NULL; lp = lp->next) {
	...
	if (lp != NULL) {
		...
		while (nrp->ns->ns_status == RFSWRITE_INITVAL)
			cv_wait(&lp->cv, &vip->r2_async_write_lock);
		mutex_exit(&vip->r2_async_write_lock);
	...
	if (vip->r2_async_write_head == NULL) {
		vip->r2_async_write_head = nlp;
	} else {
		lp = vip->r2_async_write_head;
		while (lp->next != NULL)
			lp = lp->next;
		lp->next = nlp;
	}
	...
	if (vp == NULL) {
		mutex_enter(&vip->r2_async_write_lock);
		if (vip->r2_async_write_head == nlp)
			vip->r2_async_write_head = nlp->next;
		else {
			lp = vip->r2_async_write_head;
			while (lp->next != nlp)
				lp = lp->next;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	name = nfscmd_convname(ca, exi, name, NFSCMD_CONV_INBOUND,
	    MAXPATHLEN);
	if (name == NULL) {
		dr->dr_status = NFSERR_NAMETOOLONG;
		return;
	}
	...
	if (!error && (va.va_type == VREG) && (va.va_mask & AT_SIZE)) {
		...
		if (!lookuperr &&
		    rfs4_check_delegated_byrip(exi->exi_rip, FWRITE, tvp,
		    va.va_size == 0)) {
			VN_RELE(tvp);
			curthread->t_flag |= T_WOULDBLOCK;
			goto out;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	to_exi = checkexport(exi->exi_rip, &fh->fh_fsid, (fid_t *)&fh->fh_xlen,
	    FALSE);
	if (to_exi == NULL) {
		VN_RELE(fromvp);
		*status = NFSERR_ACCES;
	...
	ca = (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf;
	fromname = nfscmd_convname(ca, exi, args->rna_from.da_name,
	    NFSCMD_CONV_INBOUND, MAXPATHLEN);
	if (fromname == NULL) {
		error = ENOENT;
		goto out;
	}
	toname = nfscmd_convname(ca, exi, args->rna_to.da_name,
	    NFSCMD_CONV_INBOUND, MAXPATHLEN);
	if (toname == NULL) {
		error = ENAMETOOLONG;
		goto out;
	 ...
	 * Check for a conflict with a non-blocking mandatory share reservation.
	 */
	error = VOP_LOOKUP(fromvp, fromname, &srcvp, NULL, 0,
	    NULL, cr, NULL, NULL, NULL);
	if (error != 0)
		goto out;
	...
	if (rip->ri_v4.r4_deleg_policy != SRV_NEVER_DELEGATE &&
	    VOP_LOOKUP(tovp, toname, &targvp, NULL, 0, NULL, cr,
	    NULL, NULL, NULL) == 0) {
	...
	error = VOP_RENAME(fromvp, fromname, tovp, toname, cr, NULL, 0);

	if (error == 0)
		vn_renamepath(tovp, srcvp, toname, strlen(toname));
...
out:
	...
	if (targvp != NULL)
		VN_RELE(targvp);
	if (toname != NULL && toname != args->rna_to.da_name)
		kmem_free(toname, MAXPATHLEN);
	if (fromname != NULL && fromname != args->rna_from.da_name)
		kmem_free(fromname, MAXPATHLEN);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	ca = (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf;
	name = nfscmd_convname(ca, exi, args->la_to.da_name,
	    NFSCMD_CONV_INBOUND, MAXPATHLEN);
	if (name == NULL) {
		VN_RELE(tovp);
		VN_RELE(fromvp);
	...
	error = VOP_LINK(tovp, fromvp, name, cr, NULL, 0);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	ca = (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf;
	linkname = nfscmd_convname(ca, exi, args->sla_from.da_name,
	    NFSCMD_CONV_INBOUND, MAXPATHLEN);
	if (linkname == NULL) {
		VN_RELE(vp);
	...
	targname = nfscmd_convname(ca, exi, args->sla_tnm,
	    NFSCMD_CONV_INBOUND, MAXPATHLEN);
	if (targname == NULL) {
		VN_RELE(vp);
		*status = NFSERR_NAMETOOLONG;
		if (linkname != args->sla_from.da_name)
			kmem_free(linkname, MAXPATHLEN);
		return;
	}
	...
	error = VOP_SYMLINK(vp, linkname, &va, targname, cr, NULL, 0);

	/*
	 * Force new data and metadata out to stable storage.
	 */
	lerror = VOP_LOOKUP(vp, linkname, &svp, NULL, 0,
	    NULL, cr, NULL, NULL, NULL);
	...
	if (targname != args->sla_tnm)
		kmem_free(targname, MAXPATHLEN);
	if (linkname != args->sla_from.da_name)
		kmem_free(linkname, MAXPATHLEN);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	ca = (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf;
	name = nfscmd_convname(ca, exi, name, NFSCMD_CONV_INBOUND, MAXPATHLEN);
	if (name == NULL) {
		VN_RELE(vp);
		dr->dr_status = NFSERR_NAMETOOLONG;
		return;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	 ...
	 * remove.
	 */
	error = VOP_RMDIR(vp, name, RFS_INST_ROOTVP(exi->exi_rip),
	    cr, NULL, 0);
...
getnewdents:
	...
	if (uio.uio_resid == rda->rda_count) {
		...
		int nents;
		size_t rsize = rda->rda_count - uio.uio_resid;
		...
		if (charset != NULL && charset->outbound != NULL) {
			if (nfscmd_convdirents(charset->outbound,
			    (char **)&rd->rd_entries, &rsize, rda->rda_count,
			    &nents, NULL) != 0) {
				/*
				 * If data didn't fit in buffer then eof must
				 * be set to false, because another readdir
				 * request from client is needed to read all
				 * dirents.
				 */
				iseof = FALSE;
			} else {
				/*
				 * What to do if all entries are invalid and we
				 * haven't reached an end of directory? Read
				 * new dirents until valid dirent is found or
				 * EOF is encountered.
				 */
				if (nents == 0 && !iseof)
					goto getnewdents;
...
void
rfs_global_init(void)
{
	rfs.rg_v2.rg2_caller_id = fs_new_caller_id();
}

void
rfs_global_fini(void)
{
	rfs.rg_v2.rg2_caller_id = 0;
}

void
rfs_srvrinit(rfs_inst_t *rip)
{
	mutex_init(&rip->ri_v2.r2_async_write_lock, NULL, MUTEX_DEFAULT, NULL);
}

void
rfs_srvrfini(rfs_inst_t *rip)
{
	mutex_destroy(&rip->ri_v2.r2_async_write_lock);
}

	...
	struct svc_req *req, cred_t *cr)
{
	...
	ct.cc_sysid = 0;
	ct.cc_pid = 0;
	ct.cc_caller_id = rfs.rg_v3.rg3_caller_id;
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (PUBLIC_FH3(&args->what.dir)) {
		publicfh_flag = TRUE;
		error = rfs_publicfh_mclookup(rip, name, dvp, cr, &vp,
		    &exi, &sec);
		if (error && exi != NULL)
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (is_referral) {
		...
		s = build_symlink(exi->exi_rip, vp, cr, &strsz);

		nfsstat_inc_svstat(exi->exi_rip, NFS_V3, NFS_REFERLINKS);
		nfsstat_inc_svstat(exi->exi_rip, 0, NFS_REFERLINKS);
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (args->count == 0) {
		...
		resp->resok.verf = rfs.rg_v3.rg3_write3verf;
		goto out;
	}
	...
	if (args->how.mode == EXCLUSIVE) {
		...
		if (args->how.mode == GUARDED)
			...
			if (va.va_mask & AT_SIZE) {
				...
				trunc = va.va_size == 0;
				if (!error &&
				    rfs4_check_delegated_byrip(exi->exi_rip,
				    FWRITE, tvp, trunc)) {
					resp->status = NFS3ERR_JUKEBOX;
					goto out1;
				}
...
tryagain:
	...
	if (error) {
		...
		if (rfs4_check_delegated_byrip(exi->exi_rip, FWRITE, vp,
		    FALSE)) {
			VN_RELE(vp);
			resp->status = NFS3ERR_JUKEBOX;
			goto out1;
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (rfs4_check_delegated_byrip(exi->exi_rip, FWRITE, targvp, TRUE)) {
		resp->status = NFS3ERR_JUKEBOX;
		goto err1;
	}
	...
	struct svc_req *req, cred_t *cr)
{
	...
	error = VOP_RMDIR(vp, name, RFS_INST_ROOTVP(exi->exi_rip), cr, NULL, 0);
	...
	struct svc_req *req, cred_t *cr)
{
	...
	fh3 = &args->to.dir;
	to_exi = checkexport(rip, &fh3->fh3_fsid, FH3TOXFIDP(fh3), FALSE);
	if (to_exi == NULL) {
		resp->status = NFS3ERR_ACCES;
		goto err1;
	}
	 ...
	 * refer to a new file.
	 */
	if (rfs4_check_delegated_byrip(rip, FWRITE, srcvp, FALSE)) {
		resp->status = NFS3ERR_JUKEBOX;
		goto err1;
	}

	/*
	 * Check for renaming over a delegated file.  Check r4_deleg_policy
	 * first to avoid VOP_LOOKUP if possible.
	 */
	if (rip->ri_v4.r4_deleg_policy != SRV_NEVER_DELEGATE &&
	    VOP_LOOKUP(tvp, toname, &targvp, NULL, 0, NULL, cr,
	    NULL, NULL, NULL) == 0) {

		if (rfs4_check_delegated_byrip(rip, FWRITE, targvp, TRUE)) {
			VN_RELE(targvp);
			resp->status = NFS3ERR_JUKEBOX;
			goto err1;
		}
	...
	struct svc_req *req, cred_t *cr)
{
	...
	fh3 = &args->link.dir;
	to_exi = checkexport(exi->exi_rip, &fh3->fh3_fsid, FH3TOXFIDP(fh3),
	    FALSE);
	if (to_exi == NULL) {
		resp->status = NFS3ERR_ACCES;
		goto out1;
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (args->count < DIRENT64_RECLEN(MAXNAMELEN))
		maxsize = DIRENT64_RECLEN(MAXNAMELEN);
	else
		maxsize = args->count;

	data = kmem_alloc(maxsize, KM_SLEEP);
...
getnewdents:
	iov.iov_base = data;
	iov.iov_len = maxsize;
	uio.uio_resid = maxsize;

	error = VOP_READDIR(vp, &uio, cr, &iseof, NULL, 0);
	rsize = maxsize - uio.uio_resid;
	...
	/*
	 * Have a valid readdir buffer for the native character
	 ...
	 * fit. In this case, we need to drop entries until it fits
	 * and patch the counts in order that the next readdir will
	 * get the correct entries.
	 */
	charset = nfscmd_findmap(exi,
	    (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf);
	if (charset != NULL && charset->outbound != NULL) {
		if (nfscmd_convdirents(charset->outbound, &data, &rsize,
		    maxsize, &nents, NULL) != 0) {
			/*
			 * If data didn't fit in buffer then eof must be set to
			 * false, because another readdir request from client
			 * is needed to read all dirents.
			 */
			iseof = FALSE;
		} else {
			/*
			 * What to do if all entries are invalid and we haven't
			 * reached an end of directory? Read new dirents until
			 * valid dirent is found or EOF is encountered.
			 */
			if (nents == 0 && !iseof)
				goto getnewdents;
		}
	...
	resp->resok.size = rsize;
	resp->resok.count = args->count;
	resp->resok.freecount = maxsize;
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	int prev_len;
	size_t rsize;	/* size of dirents in data */
	int i;
	uint_t *namlen = NULL;
	struct charset_cache *charset;
...
getmoredents:
	 ...
	 */
	rd_unit = MIN(rd_unit, args->dircount - rsize);
	...
	if (!iseof &&
	    ...
	    (rsize + DIRENT64_RECLEN(MAXNAMELEN) <= args->dircount))
		goto getmoredents;
...
good:
	...
	infosize = nents * sizeof (struct entryplus3_info);
	infop = kmem_alloc(infosize, KM_SLEEP);
...
#endif
		/*
		 * Save offset of last processed dirent. We might need it
		 * in case we reissue a new VOP_READDIR.
		 */
		uio.uio_loffset = dp->d_off;
	...
	/*
	 * Have a valid readdir buffer for the native character set. Need to
	 * check if a conversion is necessary and potentially rewrite the whole
	 * buffer. Note that if the conversion expands names enough, the
	 * structure may not fit. In this case, we need to drop entries until
	 * it fits and patch the counts in order that the next readdir will get
	 * the correct entries.
	 *
	 * We pass infop to nfscmd_convdirents() so that entries associated
	 * with skipped dirents can be removed from infop array.
	 *
	 * XXX: It is possible that we return answer bigger than args->maxcount
	 * in the unlikely case when size of dirents expands and stays below
	 * args->dircount, but the whole answer (including infop) gets over
	 * args->maxcount.  In this corner case we violate NFS specification.
	 */
	charset = nfscmd_findmap(exi,
	    (struct sockaddr *)svc_getrpccaller(req->rq_xprt)->buf);
	if (charset != NULL && charset->outbound != NULL) {
		if (nfscmd_convdirents(charset->outbound, &data, &rsize,
		    args->dircount, &nents, infop) != 0) {
			/*
			 * If data didn't fit in buffer then eof must be set to
			 * false, because another readdir request from client
			 * is needed to read all dirents.
	...
	resp->resok.reply.eof = iseof;
	resp->resok.freecount = args->dircount;
	resp->resok.nents = nents;
	resp->resok.infop = infop;
	resp->resok.infosize = infosize;
...
rfs3_readdirplus_free(READDIRPLUS3res *resp)
{
	...
	if (resp->status == NFS3_OK) {
		kmem_free(resp->resok.reply.entries, resp->resok.freecount);
		kmem_free(resp->resok.infop, resp->resok.infosize);
	}
}
...
rfs3_global_init(void)
{
	 ...
	 */
	ASSERT(sizeof (*verfp) <= sizeof (rfs.rg_v3.rg3_write3verf));
#endif

	gethrestime(&now);
	verfp = (struct rfs3_verf_overlay *)&rfs.rg_v3.rg3_write3verf;
	...
	rfs.rg_v3.rg3_caller_id = fs_new_caller_id();
}

void
rfs3_global_fini(void)
{
	rfs.rg_v3.rg3_caller_id = 0;
}

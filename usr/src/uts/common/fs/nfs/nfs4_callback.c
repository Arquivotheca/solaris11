...
nfs4_cb_args(nfs4_server_t *np, struct knetconfig *knc, SETCLIENTID4args *args)
{
	...
	if (found == FALSE) {
		...
		mutex_exit(&ncg->nfs4_cb_lock);
		DTRACE_PROBE2(nfsc__i__netconfig, nfs4_server_t *, np,
	...
	if ((pgm = nfs4_getnextprogram(ncg)) == 0) {
		...
		args->callback_ident = 0;
		mutex_exit(&ncg->nfs4_cb_lock);
		DTRACE_PROBE2(nfsc__i__nxtprogno, nfs4_server_t *, np,
		    struct knetconfig *, knc);
		return;
	}
...
nfs4_deleg_discard(mntinfo4_t *mi, nfs4_server_t *sp)
{
	...
	for (rp = list_head(&sp->s_deleg_list); rp != NULL; rp = next) {
		...
		rp->r_deleg_needs_recovery = OPEN_DELEGATE_NONE;
		rp->r_deleg_needs_recall = FALSE;
		bzero(&rp->r_deleg_limit, sizeof (nfs_space_limit4)); /* XXX */
...
static int
nfs4_deleg_migrate(mntinfo4_t *mi, nfs4_server_t *sp, nfs4_server_t *np)
{
	rnode4_t *rp, *next;
	mntinfo4_t *r_mi;
	struct nfs4_callback_globals *ncg;
	...
	for (rp = list_head(&sp->s_deleg_list); rp != NULL; rp = next) {
		...
		/*
		 * Skip if this rnode is in not on the same mount-point
		 */
		if (r_mi != mi)
			continue;
		...
		np->s_refcnt++;		/* added list node gets a reference */
	...
	DTRACE_PROBE2(nfsc__i__deleg_mig, mntinfo4_t *, mi, int, cnt);
	return (cnt);
}

/*
 * s_lock of both new and old server must be held when calling this function.
 */
void
nfs4_migrate_mi(mntinfo4_t *mi, nfs4_server_t *op, nfs4_server_t *np)
{
	int ori_open_files, n_moved, num_open;

	ASSERT(MUTEX_HELD(&op->s_lock));
	ASSERT(MUTEX_HELD(&np->s_lock));

	/*
	 * mi_open_files maintains the number of open files and
	 * delegations for this mntinfo; a open file with delegation
	 * has 2 counts in mi_open_files: 1 for the open file and 1
	 * for the delegation.
	 */
	ori_open_files = mi->mi_open_files;

	/*
	 * Move all delegations to new server. N_moved is the number
	 * of delegation moved from old to new server. After the move,
	 * the state_ref_count of both old and new server are updated
	 * with the number of moved or newly added delegations. The
	 * mi_open_files should have the correct count for open files
	 * with, or without, delegation.
	 */
	n_moved = nfs4_deleg_migrate(mi, op, np);
	np->s_lease.l4_time = op->s_lease.l4_time;	/* inherit lease time */

	/*
	 * Remove mntinfo from the mntinfo list of the old server,
	 * need to temporarily clear mi_open_files for this.
	 */
	num_open = mi->mi_open_files;
	mi->mi_open_files = 0;
	op->state_ref_count -= (ori_open_files - n_moved);
	ASSERT(op->state_ref_count >= 0);
	nfs4_remove_mi_from_server_nolock(mi, op);

	mi->mi_open_files = num_open;	/* restore ori open_files count */
	ASSERT(ori_open_files == mi->mi_open_files);

	/*
	 * Update state_ref_count of new server with the number
	 * of open files (that do not have delegation).
	 */
	np->state_ref_count += (ori_open_files - n_moved);
	...
	nfs4_ga_res_t *garp, cred_t *cr)
{
	 ...
	 *
	 * NB: find_nfs4_srv returns with s_lock held.
	 */

	if ((np = find_nfs4_srv(mi)) == NULL)
		return;

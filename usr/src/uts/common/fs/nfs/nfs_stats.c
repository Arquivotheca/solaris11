...
nfsstat_zone_init_server(zoneid_t zoneid, kstat_named_t *svstatp[])
{
	...
	/*
	 * The second index of these arrays are not used, so initialize
	 * to NULL. The first index is used for instance 0 to gather aggregate
	 * NFS traffic across all versions.
	 */
	svstatp[1] = NULL;
	...
	svstatp[0] = nfsstat_zone_init_common(zoneid, "nfs", 0,
	    "nfs_server", svstat_tmpl, sizeof (svstat_tmpl));
}
...
nfsstat_zone_fini_server(zoneid_t zoneid, kstat_named_t **svstatp)
{
	...
	nfsstat_zone_fini_common(zoneid, "nfs", 0, "nfs_server");
	kmem_free(svstatp[0], sizeof (svstat_tmpl));
}
...
nfsstat_zone_init(zoneid_t zoneid)
{
	 ...
	 */
	nfsstat_zone_init_server(zoneid, nfs_stats_ptr->svstat_ptr);

	/*
	 * Initialize v2 stats
	 */
	nfsstat_zone_init_rfsreq_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	nfsstat_zone_init_rfsproc_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	nfsstat_zone_init_aclreq_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	nfsstat_zone_init_aclproc_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	/*
	 * Initialize v3 stats
	 */
	nfsstat_zone_init_rfsreq_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	nfsstat_zone_init_rfsproc_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	nfsstat_zone_init_aclreq_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	nfsstat_zone_init_aclproc_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	/*
	 * Initialize v4 stats
	 */
	nfsstat_zone_init_rfsreq_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
	nfsstat_zone_init_rfsproc_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
	nfsstat_zone_init_aclreq_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
	nfsstat_zone_init_aclproc_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
...
nfsstat_zone_fini(zoneid_t zoneid, void *data)
{
	 ...
	 */
	nfsstat_zone_fini_server(zoneid, nfs_stats_ptr->svstat_ptr);

	/*
	 * Free v2 stats
	 */
	nfsstat_zone_fini_rfsreq_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	nfsstat_zone_fini_rfsproc_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	nfsstat_zone_fini_aclreq_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	nfsstat_zone_fini_aclproc_v2(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V2]);
	/*
	 * Free v3 stats
	 */
	nfsstat_zone_fini_rfsreq_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	nfsstat_zone_fini_rfsproc_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	nfsstat_zone_fini_aclreq_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	nfsstat_zone_fini_aclproc_v3(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V3]);
	/*
	 * Free v4 stats
	 */
	nfsstat_zone_fini_rfsreq_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
	nfsstat_zone_fini_rfsproc_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
	nfsstat_zone_fini_aclreq_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);
	nfsstat_zone_fini_aclproc_v4(zoneid, &nfs_stats_ptr->nfs_stats[NFS_V4]);

	kmem_free(nfs_stats_ptr, sizeof (*nfs_stats_ptr));
}

void
nfsstat_inc_svstat(rfs_inst_t *rip, int version, int op_stat)
{
	struct nfs_stats *nfs_stats;

	if ((rip == NULL) || (rip->ri_rzone == NULL) ||
	    (rip->ri_rzone->rz_nfsstats == NULL) ||
	    ((version < NFS_V2) && (version != 0)) || (version > NFS_V4)) {
		return;
	}

	nfs_stats = rip->ri_rzone->rz_nfsstats;
	nfs_stats->svstat_ptr[version][op_stat].value.ui64++;
}

void
nfsstat_inc_proc_cnt(rfs_inst_t *rip, int version, int op_stat)
{
	struct nfs_stats *nfs_stats;

	if ((rip == NULL) || (rip->ri_rzone == NULL) ||
	    (rip->ri_rzone->rz_nfsstats == NULL) ||
	    (version < NFS_V2) || (version > NFS_V4)) {
		return;
	}

	nfs_stats = rip->ri_rzone->rz_nfsstats;
	nfs_stats->nfs_stats[version].rfsproccnt_ptr[op_stat].value.ui64++;
}

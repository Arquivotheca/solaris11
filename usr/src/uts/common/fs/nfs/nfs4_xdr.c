	...
	if (aobjp->res_uiop != NULL) {
		...
		if (xdrs->x_ops == &xdrrdma_ops) {
			...
			if (objp->wlist) {
				...
				if (objp->wlist_len != roundup(objp->data_len,
				    BYTES_PER_XDR_UNIT)) {
					DTRACE_PROBE3(
					    xdr__e__read4res_clnt_fail,
					    int, ocount, int, objp->data_len,
					    int, objp->wlist_len);
					objp->wlist = NULL;
					return (FALSE);
				}

				XDR_CONTROL(xdrs, XDR_RDMA_GET_FLAGS,
				    &xdr_flag);
				if (xdr_flag & XDR_RDMA_WLIST_PREREG) {
					ASSERT(cl->rb_longbuf.addr != NULL);
					ASSERT(objp->wlist_len <=
					    cl->rb_longbuf.len);

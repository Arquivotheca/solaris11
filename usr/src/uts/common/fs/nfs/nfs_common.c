...
static vfsdef_t vfw2 = {
	...
	"nfs",
	nfsinit,
	VSW_CANREMOUNT|VSW_NOTZONESAFE|VSW_STATS|VSW_ZMOUNT|VSW_MWAC,
	NULL
};
...
_init(void)
{
	...
	status = sharefs_register(SHAREFS_NFS, nfs_export);
	if (status)
		cmn_err(CE_WARN, "_init: sharefs_register failed %d", status);
	return (status);
}
...
/*
 * This is filled in by nfssrv:_init()
 */
uint32_t (*nfs_get_fsh_status_func)(vnode_t *) = NULL;

/*
 * If the server is not loaded, then return that
 * the filesystem is indeed local.
 */
uint32_t
nfs_get_fsh_status(vnode_t *vp)
{
	if (nfs_get_fsh_status_func == NULL)
		return (FSE_AVAILABLE);

	return (nfs_get_fsh_status_func(vp));
}

		...
		struct compound_state *cs, int expseudo)
{
	...
	*vpp = vp = NULL;
	...
	newexi = checkexport4(cs->rip, &vp->v_vfsp->vfs_fsid, &fid, vp);
	if (newexi == NULL) {
		if (ismntpt == 0) {
			*vpp = vp;
		} else {
	...
	struct svc_req *req, struct compound_state *cs)
{
	...
	if (args->cookieverf != rfs.rg_v4.rg4_readdir4verf) {
	...
	IXDR_PUT_HYPER(ptr, rfs.rg_v4.rg4_readdir4verf);
		...
		if (ae & 0xffffffff00000000) {
			...
			if (ae &
				...
				if (ae & FATTR4_SUPPORTED_ATTRS_MASK) {
					...
					IXDR_PUT_HYPER(ptr,
					    cs->rip->ri_v4.r4_supported_attrs);
				}

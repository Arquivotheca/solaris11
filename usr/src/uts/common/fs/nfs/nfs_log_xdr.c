...
xdr_nfslog_sharefsargs(XDR *xdrs, struct exportinfo *objp)
{
	...
	bzero(&fh, sizeof (fh));
	fh.fh_fsid = objp->exi_fsid;
	/* Truncate the fid_len similary like xdr_nfslog_nfs_fh3() */
	fh.fh_xlen = objp->exi_fid.fid_len > NFS_FHMAXDATA ? NFS_FHMAXDATA
	    : objp->exi_fid.fid_len;
	bcopy(objp->exi_fid.fid_data, fh.fh_xdata, fh.fh_xlen);
	fh.fh_len = sizeof (fh.fh_data);

	return (xdr_fhandle(xdrs, &fh));
}

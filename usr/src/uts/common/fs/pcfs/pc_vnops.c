...
int pcfs_access2(void *vp, int mode, cred_t *cr);
	...
	caller_context_t *ct)
{
	...
	if (PCA_IS_HIDDEN(fsp, attr))
		...
		vap->va_mode = (S_IRUSR|S_IRGRP|S_IROTH) & ~fsp->pcfs_umask;
	else if (attr & PCA_RDONLY)
		vap->va_mode = (S_IRUSR|S_IXUSR|S_IRGRP|S_IXGRP|
		    S_IROTH|S_IXOTH) & ~fsp->pcfs_umask;
	else
		vap->va_mode = (S_IRWXU|S_IRWXG|S_IRWXO) & ~fsp->pcfs_umask;
	...
	vap->va_uid = fsp->pcfs_uid;
	vap->va_gid = fsp->pcfs_gid;
	...
	caller_context_t *ct)
{
	...
	oldva.va_uid = fsp->pcfs_uid;
	oldva.va_gid = fsp->pcfs_gid;
	...
	if (PCA_IS_HIDDEN(fsp, attr))
		...
		oldva.va_mode = (S_IRUSR|S_IRGRP|S_IROTH) & ~fsp->pcfs_umask;
	else if (attr & PCA_RDONLY)
		oldva.va_mode = (S_IRUSR|S_IXUSR|S_IRGRP|S_IXGRP|
		    S_IROTH|S_IXOTH) & ~fsp->pcfs_umask;
	else
		oldva.va_mode = (S_IRWXU|S_IRWXG|S_IRWXO) & ~fsp->pcfs_umask;
...
int
pcfs_access2(void *vp, int mode, cred_t *cr)
{
	return (pcfs_access((struct vnode *)vp, mode, 0, cr,  NULL));
}
	...
	caller_context_t *ct)
{
	...
	if (crgetuid(cr) != fsp->pcfs_uid) {
		shift += 3;
		if (!groupmember((uid_t)fsp->pcfs_gid, cr))
			shift += 3;
	}

	if (secpolicy_vnode_access2(cr, vp, fsp->pcfs_uid,
	    (~fsp->pcfs_umask) << shift, mode) != 0) {
		return (EACCES);
	}
	return (0);
}
	...
	int flags)
{
	...
	error = pcfs_access(vp, VWRITE, 0, cr, ct);
	if (error) {
		pc_unlockfs(fsp);
		return (error);
	}
	...
	int flags)
{
	...
	error = pcfs_access(dvp, VWRITE, 0, cr, ct);
	if (error) {
		pc_unlockfs(fsp);
		return (error);
	}

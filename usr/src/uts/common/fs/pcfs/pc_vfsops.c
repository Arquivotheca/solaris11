...
static void pcfs_parse_mntopts(struct pcfs *fsp, struct cred *cr);
 ...
 */
	...
	{ MNTOPT_PCFS_OWNER, NULL, NULL, MO_HASVALUE, NULL },
	{ MNTOPT_PCFS_GROUP, NULL, NULL, MO_HASVALUE, NULL },
	{ MNTOPT_PCFS_MASK, NULL, NULL, MO_HASVALUE, NULL },
};
 ...
 */
static void
pcfs_parse_mntopts(struct pcfs *fsp, struct cred *cr)
{
	char *c;
	char *endptr;
	long l;
	...
	/* Preinit uid, gid and umask; override below, if specified and valid */
	fsp->pcfs_uid = crgetuid(cr);
	fsp->pcfs_gid = crgetgid(cr);
	fsp->pcfs_umask = PTOU(curproc)->u_cmask;

	if (vfs_optionisset(vfsp, MNTOPT_PCFS_OWNER, &c)) {
		if (ddi_strtoul(c, &endptr, 0, &ulval) == 0 &&
		    *endptr == '\0' && ulval <= UINT_MAX &&
		    VALID_UID((uid_t)ulval, crgetzone(cr))) {
			fsp->pcfs_uid = (uid_t)ulval;
		} else {
			cmn_err(CE_WARN, "!pcfs: invalid value for owner %s -"
			    " using current uid (%u).", c, fsp->pcfs_uid);
		}
	}

	if (vfs_optionisset(vfsp, MNTOPT_PCFS_GROUP, &c)) {
		if (ddi_strtoul(c, &endptr, 0, &ulval) == 0 &&
		    *endptr == '\0' && ulval <= UINT_MAX &&
		    VALID_GID((gid_t)ulval, crgetzone(cr))) {
			fsp->pcfs_gid = (gid_t)ulval;
		} else {
			cmn_err(CE_WARN, "!pcfs: invalid value for group %s -"
			    " using current gid (%u).", c, fsp->pcfs_gid);
		}
	}

	if (vfs_optionisset(vfsp, MNTOPT_PCFS_MASK, &c)) {

		if (ddi_strtoul(c, &endptr, 8, &ulval) == 0 &&
		    *endptr == '\0') {
			fsp->pcfs_umask = (mode_t)(ulval & MODEMASK);
		} else {
			cmn_err(CE_WARN,
			    "!pcfs: invalid value for mask %s -  using current"
			    " umask (0%o).", c, fsp->pcfs_umask);
		}
	}
}
	...
	struct cred *cr)
{
	...
	if (fsp->pcfs_flags & PCFS_BOOTPART) {
		/* special case, override the user setting */
		fsp->pcfs_uid = 0;
		fsp->pcfs_gid = 0;
		fsp->pcfs_umask = fsp->pcfs_umask & ~(S_IRWXU|S_IRGRP|S_IXGRP|
		    S_IROTH|S_IXOTH);
	}

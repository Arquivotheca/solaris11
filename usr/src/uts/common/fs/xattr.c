...
/* ARGSUSED */
static int
xattr_file_setattr(vnode_t *vp, vattr_t *vap, int flags, cred_t *cr,
    caller_context_t *ct)
{
	/*
	 * Allow a truncate request to "succeed". This allows vn_openat to
	 * open a system attribute file with the O_TRUNC flag set.
	 */
...
static const fs_operation_def_t xattr_file_tops[] = {
	...
	{ VOPNAME_SETATTR,	{ .vop_setattr = xattr_file_setattr }	},
	...
	if (dvp->v_xattrdir != NULL) {
		...
		*vpp = gfs_dir_create(
		...
		if (dvp->v_xattrdir != NULL) {
			...
			ASSERT(dvp == dp->gfsd_file.gfs_parent);
			held = 1;

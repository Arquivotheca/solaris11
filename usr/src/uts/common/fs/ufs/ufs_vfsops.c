...
static vfsdef_t vfw = {
	...
	"ufs",
	ufsinit,
	VSW_HASPROTO|VSW_CANREMOUNT|VSW_STATS|VSW_CANLOFI|VSW_MWAC,
	&ufs_mntopts
};
...
#define	BOOT_TIME_LIMIT	(180*hz)
	...
	if (((fspt->fs_magic != FS_MAGIC) &&
	    ...
	    !UFS_VALID_VERSION(fspt)) ||
	    (fspt->fs_magic == MTB_UFS_MAGIC &&
	    !MTB_UFS_VALID_VERSION(fspt)) ||
	...
	char *path, cred_t *cr, int isroot, void *raw_argsp, int args_len)
{
	...
	rw_init(&ufsvfsp->vfs_dqrwlock, NULL, RW_DEFAULT, NULL);
	...
	if ((fsp->fs_magic == FS_MAGIC && !UFS_VALID_VERSION(fsp)) ||
	    (fsp->fs_magic == MTB_UFS_MAGIC && !MTB_UFS_VALID_VERSION(fsp))) {
		cmn_err(CE_NOTE,
...
out:
	...
	if (ufsvfsp) {
		...
		rw_destroy(&ufsvfsp->vfs_dqrwlock);

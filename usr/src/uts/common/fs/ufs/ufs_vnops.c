...
/*
 * Copyright (c) 1984, 2011, Oracle and/or its affiliates. All rights reserved.
 */
 ...
 * it's segmap slot is needed for something else. If we lose the bet,
...
wrip(struct inode *ip, struct uio *uio, int ioflag, struct cred *cr)
{
	...
	uint_t flags;
	int newpage;
	int iupdat_flag, directio_status, rewriteflg;
	...
	rewriteflg = ufs_check_rewrite(ip, uio, ioflag);
	...
	do {
		...
		mapon = uoff & (u_offset_t)MAXBOFFSET;
		on = (u_offset_t)blkoff(fs, uoff);
		n = (u_offset_t)MIN((u_offset_t)(fs->fs_bsize) - on,
	...
	struct cred *cr)
{
	...
	/*
	 * For the mmap VSX conformance test we need to update times
	 * so lets do that only in the case of an mmap write reference.
	 * If there are no dirty pages then we won't call ufs_putapage()
	 * anyway, so always turn off the flag later.
	 */
	if (vp->v_mmap_write > 0) {
		mutex_enter(&ip->i_tlock);
		ip->i_flag |= IFIRSTPAGE;
		mutex_exit(&ip->i_tlock);
	}
	...
	/* Turn off flag in case we didn't call ufs_putapage() */
	if (ip->i_flag & IFIRSTPAGE) {
		mutex_enter(&ip->i_tlock);
		ip->i_flag &= ~IFIRSTPAGE;
		mutex_exit(&ip->i_tlock);
	}
	...
	struct cred *cr)
{
	 ...
	 */
	if (((ip->i_flag & IMODTIME) == 0) || (ip->i_flag & IFIRSTPAGE)) {
		mutex_enter(&ip->i_tlock);
		ip->i_flag |= IUPD;
		if (ip->i_flag & IFIRSTPAGE) {
			ip->i_flag |= ICHG;
...
uint64_t ufs_map_lockfs_retry_cnt = 0;
...
retry_map:
	*addrp = hint;
	 ...
	 * because that is the order in which pagefault works.  It's safe
	 * to request the lock as writer because pending pagefault reads
	 * will get EDEADLK and fail back to as_fault(), where they will
	 * block.
	 */
	AS_LOCK_ENTER(as, &as->a_lock, RW_WRITER);

	/*
	 * Since we hold a_lock as a writer, pass a non-zero dl_chk arg to
	 * ufs_lockfs_being_dlc so that it will return EDEADLK if blocking
	 * would result in a deadlock.
	 */
	if (error = ufs_lockfs_begin_dlc(ufsvfsp, &ulp, ULOCKFS_MAP_MASK,
	    dl_chk)) {
		/*
		 * ufs_lockfs_begin_dlc() did not succeed.  Drop all locks.
		 */
		...
		/*
		 * Don't expect to recover from EIO;  on any other error, take
		 * another shot at it.
		 */
		if (error == EIO)
			goto out;

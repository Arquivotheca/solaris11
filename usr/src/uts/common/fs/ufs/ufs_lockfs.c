...
/*
 * ufs_check_lockfs_dlc
 *	check whether a ufs_vnops conflicts with the file system lock
 *	if dl_chk is non-zero, we cannot get the lock, and ufs_quiesce_pend
 *	is non-zero, then return EDEADLK rather than blocking.
 */
static int
ufs_check_lockfs_dlc(struct ufsvfs *ufsvfsp, struct ulockfs *ulp,
    ulong_t mask, int dl_chk)
{
	k_sigset_t	smask;
	int		sig, slock;
	...
	while (ulp->ul_fs_lock & mask) {
		...
		/*
		 * If deadlock checking was requested and there's a pending
		 * quiesce, then blocking here could result in a deadlock.
		 * Return the deadlock error and allow the caller to deal
		 * with it.
		 */
		if (dl_chk && ufs_quiesce_pend)
			return (EDEADLK);
		...
		if (slock || ufsvfsp->vfs_nointr) {
			(void) cv_timedwait(&ulp->ul_cv, &ulp->ul_lock,
			    ddi_get_lbolt() + hz);
		} else {
			sigintr(&smask, 1);
			sig = cv_timedwait_sig(&ulp->ul_cv, &ulp->ul_lock,
			    ddi_get_lbolt() + hz);
			sigunintr(&smask);
...
/*
 * ufs_check_lockfs - legacy behavior maintained by passing a 0 'check for
 * deadlock' argument
 */
int
ufs_check_lockfs(struct ufsvfs *ufsvfsp, struct ulockfs *ulp, ulong_t mask)
{
	return (ufs_check_lockfs_dlc(ufsvfsp, ulp, mask, 0));
}
...
/*
 * ufs_lockfs_begin_dlc - start the lockfs locking protocol
 * 'dlc' short for deadlock check
 */
int
ufs_lockfs_begin_dlc(struct ufsvfs *ufsvfsp, struct ulockfs **ulpp,
    ulong_t mask, int dl_chk)
{
	int 		error;
	int		rec_vop;
	...
	*ulpp = ulp = &ufsvfsp->vfs_ulockfs;
	...
	if (ULOCKFS_IS_JUSTULOCK(ulp))
		...
		if (error = ufs_check_lockfs_dlc(ufsvfsp, ulp, mask, dl_chk)) {
			mutex_exit(&ulp->ul_lock);
			if (ulockfs_info_free == NULL)
 ...
 * Legacy, blocking behavior maintained by passing 0 as the 'check for
 * deadlock' argument
 */
int
ufs_lockfs_begin(struct ufsvfs *ufsvfsp, struct ulockfs **ulpp, ulong_t mask)
{
	return (ufs_lockfs_begin_dlc(ufsvfsp, ulpp, mask, 0));
}

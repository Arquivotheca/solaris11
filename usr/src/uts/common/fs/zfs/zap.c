...
/*
 * zap_increment_num_entries() must be called without holding a
 * zap_leaf_t l_rwlock in order to avoid a deadlock with callers of
 * zap_create_leaf(), zap_open_leaf(), zap_get_leaf_byblk(),
 * fzap_cursor_retrieve() and zap_cursor_fini().
 */
static void
zap_increment_num_entries(zap_name_t *zn, int delta, dmu_tx_t *tx)
{
	zap_t *zap = zn->zn_zap;
	int err;
	...
	if (zap_tryupgradedir(zap, tx) == 0) {
		...
		zap_unlockdir(zap);
		err = zap_lockdir(os, zapobj, tx, RW_WRITER, FALSE, FALSE,
		    &zn->zn_zap);
		zap = zn->zn_zap;
		if (err)
			return;
...
out:
	...
	if (err == 0)
		zap_increment_num_entries(zn, 1, tx);
		/* zap_increment_num_entries() may change zap */
	return (err);
}
...
zap_join(objset_t *os, uint64_t fromobj, uint64_t intoobj, dmu_tx_t *tx)
{
	...
	for (zap_cursor_init(&zc, os, fromobj);
		...
		if (!ZA_PROP_IS_INT(za) || za.za_num_integers != 1)
			return (EINVAL);
...
fzap_count_write(zap_name_t *zn, int add, uint64_t *towrite,
    uint64_t *tooverwrite, uint64_t *tounref)
{
	zap_t *zap = zn->zn_zap;
	zap_leaf_t *l;
	boolean_t refd;
	...
	if (!add) {
		if (dmu_buf_freeable(l->l_dbuf, &refd))
			*tooverwrite += l->l_dbuf->db_size;
		else
			*towrite += l->l_dbuf->db_size;
		if (refd)
			*tounref += l->l_dbuf->db_size;
	} else {
		/* Account for additional leaf block if we split */
		*towrite += 2 * l->l_dbuf->db_size;
	}

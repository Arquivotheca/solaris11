...
zfs_sa_upgrade(sa_handle_t *hdl, dmu_tx_t *tx)
{
	...
	boolean_t drop_acl_lock = B_FALSE;

	/*
	 * symlinks are never upgraded.
	 */
	...
	/*
	 * If we are going to have to upgrade/convert the ACL then we must
	 * have the ACL lock since zfs_acl_xform() will change
	 * replace the ACL nodes and operation vectors in the cached ACL.
	 */
	if (zp->z_acl_cached->z_version < ZFS_ACL_VERSION_FUID &&
	    mutex_owner(&zp->z_acl_lock) != curthread) {
		if (mutex_tryenter(&zp->z_acl_lock) == 0) {
			goto done;
		} else {
			drop_acl_lock = B_TRUE;
		}
	}
	...
	SA_ADD_BULK_ATTR(sa_attrs, count, SA_ZPL_FLAGS(zfsvfs), NULL,
	    &zp->z_pflags, 8);
	SA_ADD_BULK_AMCTIME(sa_attrs, count, zfsvfs, zp);
...
zfs_sa_upgrade_txholds(dmu_tx_t *tx, znode_t *zp)
{
	...
	if (zfs_external_acl(zp))
		dmu_tx_hold_free(tx, zfs_external_acl(zp), 0, DMU_OBJECT_END);
}

...
/*
 * We'll take the unused errnos, EBADE, EBADR, ENOANO (from the Convergent
 * graveyard) to indicate checksum errors, fragmentation and lack of an
 * encryption key.
 */
#define	ECKSUM	EBADE
#define	EFRAGS	EBADR
...
struct zio {
	...
	/* Multiple IO vectors for aggregated IOs */
	io_mvector_t	*io_mvector;

	/* Stuff for the vdev stack */
	vdev_t		*io_vd;
	vdev_ops_t	*io_vd_ops;
	void		*io_vsd;
...
extern zio_t *zio_vdev_child_io(zio_t *zio, blkptr_t *bp, vdev_t *vd,
    vdev_ops_t *ops, uint64_t offset, void *data, uint64_t size, int type,
    int priority, enum zio_flag flags, zio_done_func_t *done, void *private);

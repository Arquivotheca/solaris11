...
#define	ZFS_ACL_CHMOD		0x200		/* ACL has been chmod'd */
#define	ZFS_ACL_MASK_X		0x400		/* mask 'x' bit */
 ...
 */
#define	SA_ADD_BULK_AMCTIME(b, idx, zfsvfs, zp) do { \
	SA_ADD_BULK_ATTR(b, idx, SA_ZPL_ATIME(zfsvfs), NULL, &zp->z_atime, \
	    sizeof (zp->z_atime)); \
	SA_ADD_BULK_ATTR(b, idx, SA_ZPL_MTIME(zfsvfs), NULL, &zp->z_mtime, \
	    sizeof (zp->z_mtime)); \
	SA_ADD_BULK_ATTR(b, idx, SA_ZPL_CTIME(zfsvfs), NULL, &zp->z_ctime, \
	    sizeof (zp->z_ctime)); \
	_NOTE(CONSTCOND) \
} while (0)
...
#define	ZFS_ACCESSTIME_STAMP(zfsvfs, zp) \
	if ((zfsvfs)->z_atime && !((zfsvfs)->z_vfs->vfs_flag & VFS_RDONLY)) \
		zfs_tstamp_update_setup(zp, ACCESSED, B_FALSE);

extern int	zfs_init_fs(zfsvfs_t *, znode_t **);
extern void	zfs_set_dataprop(objset_t *);
extern void	zfs_create_fs(objset_t *os, void *, cred_t *cr,
    dmu_tx_t *tx);
extern void	zfs_tstamp_update_setup(znode_t *, uint_t, boolean_t);
...
extern vfs_t 	*zfs_get_vfs(const char *resource);
...
struct zfsvfs;			/* forward reference */
struct zfs_stat;		/* forward reference */
extern int zfs_obj_to_path(objset_t *osp, uint64_t obj, char *buf, int len);
extern int zfs_obj_to_stats(struct zfsvfs *zfsvfs, uint64_t obj,
    struct zfs_stat *sb, char *pathbuf, int pathlen);

 ...
 */
#define	DMU_MAX_ACCESS (24<<20) /* 24MB */
#define	DMU_MAX_DELETEBLKCNT (98304ULL) /* ~12MB of indirect blocks */
...
int dmu_objset_create(const char *name, dmu_objset_type_t type, uint64_t flags,
    struct dsl_crypto_ctx *ctx,
    int (*chkfunc)(struct spa *spa, void *arg),
    void (*func)(objset_t *os, void *arg, cred_t *cr, dmu_tx_t *tx), void *arg);
int dmu_objset_clone(const char *name, struct dsl_dataset *clone_origin,
    struct dsl_crypto_ctx *ctx, uint64_t flags);
...
#define	DMU_POOL_CONDENSE_TXG		"condense_txg"
...
uint16_t dmu_get_bonuslen(dmu_buf_t *);
 ...
 */
boolean_t dmu_buf_freeable(dmu_buf_t *, boolean_t *);

/*
 * Inform the DMU that you will be explicitly managing the locking
 * for this dbuf so that it will be safe for the dmu to modify the
 * db_data while holds are active.
 */
void dmu_buf_manage(dmu_buf_t *);
...
struct arc_ref *dmu_request_arcbuf(int size);
void dmu_release_arcbuf(struct arc_ref *buf);
void dmu_assign_arcbuf(dmu_buf_t *handle, uint64_t offset, struct arc_ref *buf,
    dmu_tx_t *tx);
void dmu_xuio_init(struct xuio *uio, int niov);
void dmu_xuio_fini(struct xuio *uio);
int dmu_xuio_add(struct xuio *uio, struct arc_ref *abuf, offset_t off,
    size_t n);
int dmu_xuio_cnt(struct xuio *uio);
struct arc_ref *dmu_xuio_arcbuf(struct xuio *uio, int i);
...
boolean_t dmu_object_is_free(objset_t *os, uint64_t object);
...
extern uint64_t dmu_objset_guid(objset_t *os);

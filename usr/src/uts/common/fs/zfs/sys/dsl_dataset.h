...
#define	DS_IS_INCONSISTENT(_ds)	\
	((_ds)->ds_phys->ds_flags & DS_FLAG_INCONSISTENT)
/*
...
#define	DS_IS_DEFER_DESTROY(_ds)	\
	((_ds)->ds_phys->ds_flags & DS_FLAG_DEFER_DESTROY)
...
typedef struct dsl_dataset {
	...
	boolean_t ds_destroying;	/* identify trimmable dnode frees */
...
#define	dsl_dataset_is_snapshot(_ds) \
	((_ds)->ds_phys->ds_num_children != 0)

#define	DS_UNIQUE_IS_ACCURATE(_ds)	\
	(((_ds)->ds_phys->ds_flags & DS_FLAG_UNIQUE_ACCURATE) != 0)

#define	DS_POOL(_ds)		((_ds)->ds_dir->dd_pool)
#define	DS_SPA(_ds)		(DS_POOL(_ds)->dp_spa)
#define	DS_META_OBJSET(_ds)	(DS_POOL(_ds)->dp_meta_objset)
#define	DS_CONFIG_RWLOCK(_ds)	(DS_POOL(_ds)->dp_config_rwlock)
...
int dsl_dataset_own_exclusive(const char *name, boolean_t inconsistentok,
    void *tag, dsl_dataset_t **dsp);
...
boolean_t dsl_dataset_tryown(dsl_dataset_t *ds, boolean_t inconsistentok,
    void *tag);
int dsl_dataset_make_exclusive(dsl_dataset_t *ds, void *tag);
void dsl_register_onexit_hold_cleanup(dsl_dataset_t *ds, const char *htag,
    minor_t minor);
uint64_t dsl_dataset_create_sync(dsl_dir_t *pds, const char *lastname,
    dsl_dataset_t *origin, struct dsl_crypto_ctx *dcc,
    uint64_t flags, cred_t *, dmu_tx_t *);
...
void dsl_dataset_start_send(dsl_dataset_t *ds);
void dsl_dataset_end_send(dsl_dataset_t *ds);
boolean_t dsl_dataset_is_tmp(const char *dsname);
...
uint64_t dsl_dataset_trim_txg(dsl_dataset_t *ds);
...
int dsl_dataset_check_quota(dsl_dataset_t *ds, uint64_t inflight);
...
void dsl_dataset_adjust_space(dsl_dataset_t *ds, uint64_t asize,
    uint64_t inflight, uint64_t *used, uint64_t *ref_rsrv);

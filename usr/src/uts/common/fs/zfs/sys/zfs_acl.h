...
typedef struct zfs_acl {
	...
	uint32_t	z_group_mask;	/* group@ max allow */
	uint32_t	z_owner_mask;	/* owner@ max allow */
	uint64_t	z_fowner;	/* current file owner */
} zfs_acl_t;
...
#define	ACL_NO_MASK	(ACE_ALL_PERMS)
...
#define	ZFS_ACL_WIDE_FLAGS (V4_ACL_WIDE_FLAGS|ZFS_ACL_TRIVIAL|ZFS_INHERIT_ACE|\
    ZFS_ACL_OBJ_ACE|ZFS_ACL_CHMOD|ZFS_ACL_MASK_X)
...
int zfs_vsec_2_aclp(struct zfsvfs *, vtype_t, uint64_t, vsecattr_t *, cred_t *,
...
zfs_acl_t *zfs_acl_alloc(int, uint64_t);

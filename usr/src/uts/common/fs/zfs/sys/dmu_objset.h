...
struct objset {
	...
	uint64_t os_last_sync;
	boolean_t os_initializing;
	zil_header_t os_zil_header;
...
#define	DMU_OS_IS_MOS(os)	((os)->os_dsl_dataset == NULL)
...
/* called from dsl */
void dmu_objset_sync(objset_t *os, zio_t *zio, blkptr_t *rootbp, dmu_tx_t *tx);
boolean_t dmu_objset_is_dirty_in_txg(objset_t *os, uint64_t txg);
boolean_t dmu_objset_is_dirty(objset_t *os);
objset_t *dmu_objset_create_impl(spa_t *spa, struct dsl_dataset *ds,
    blkptr_t *bp, dmu_objset_type_t type, struct dsl_crypto_ctx *dcc,
    dmu_tx_t *tx);

...
#define	TOC_OFF(x)		BF32_GET(x, 0, 24)
#define	TOC_ATTR_PRESENT(x)	BF32_GET(x, 31, 1)
#define	TOC_LEN_IDX(x)		BF32_GET(x, 24, 7)
...
#define	SA_GET_DB(hdl, type)	\
	(dmu_buf_impl_t *)((type == SA_BONUS) ? \
	(hdl)->sa_bonus : (hdl)->sa_spill)
...
#define	SA_ATTR_INFO(sa, idx, hdr, attr, bulk, hdl) \
	{ \

 ...
 * dnode id flags
 */
#define	DN_ID_NOBEFORE		0x1
#define	DN_ID_BEFORE		0x2
#define	DN_ID_AFTER		0x4
...
#define	DN_DNODE_OFFSET(dn) 	((dn) ->dn_object == DMU_USERUSED_OBJECT ? \
	offsetof(objset_phys_t, os_userused_dnode) : \
	(dn)->dn_object == DMU_GROUPUSED_OBJECT ? \
	offsetof(objset_phys_t, os_groupused_dnode) : \
	((dn)->dn_object & (DNODES_PER_BLOCK - 1)) << DNODE_SHIFT)
#define	DN_BLKPTR_OFFSET(dn, i) 	\
	(DN_DNODE_OFFSET(dn) + DNODE_CORE_SIZE + ((i) << SPA_BLKPTRSHIFT))
#define	DN_SPILL_OFFSET(dn) 	\
	(DN_DNODE_OFFSET(dn) + DNODE_SIZE - (1 << SPA_BLKPTRSHIFT))
...
typedef struct dnode_handle {
	...
	krwlock_t dnh_rwlock;
	dnode_t *dnh_dnode;
} dnode_handle_t;

#define	DN_HOLD_HANDLE(dnh)	\
	rw_enter(&(dnh)->dnh_rwlock, RW_READER)

#define	DN_RELEASE_HANDLE(dnh)	\
	rw_exit(&(dnh)->dnh_rwlock)

typedef struct dnode_list {
	krwlock_t	dl_phys_rwlock;
	size_t		dl_count;
	dnode_handle_t	dl_dnodes[1];
} dnode_list_t;
...
void dnode_buf_will_dirty(dmu_buf_t *db);
...
uint64_t dnode_trim_txg(dnode_t *dn);

...
typedef struct space_map {
	...
	uint8_t		sm_recovered;	/* recovered map? */
	uint8_t		sm_pad;		/* unused */
...
struct space_map_ops {
	...
	boolean_t (*smop_fragmented)(space_map_t *sm, uint64_t size);
};

/*
 * time stamp entry -- these entries are used primarily by zdb to display
 * more information, but are also used to figure out if we need to condense
 * metaslabs.
 *
 *    1      3         10                     50
 *  ,---+--------+------------+---------------------------------.

...
typedef struct dsl_pool {
	...
	hrtime_t dp_sync_write_time;
	hrtime_t dp_read_overhead;
	uint64_t dp_written_in_sync;
	...
	kmutex_t dp_tmp_uref_lock;
	list_t dp_tmp_uref_list;
	boolean_t dp_processed_tmp_urefs;
	kcondvar_t dp_tmp_uref_cv;
...
dsl_pool_t *dsl_pool_create(spa_t *spa, struct dsl_crypto_ctx *dcc,
    nvlist_t *zplprops, uint64_t txg);
...
void dsl_free_sync(zio_t *pio, dsl_pool_t *dp, uint64_t txg,
    const blkptr_t *bpp);
int dsl_read_from_ref(zio_t *pio, spa_t *spa, const int blkoff,
    arc_ref_t *parent, uint64_t size, arc_done_func_t *done, void *private,
    int priority, enum zio_flag flags, arc_options_t options,
    const zbookmark_t *zb);
int dsl_read(zio_t *pio, spa_t *spa, const blkptr_t *bpp, uint64_t size,
...
int dsl_pool_user_release(dsl_pool_t *dp, uint64_t dsobj, const char *tag,
    dmu_tx_t *tx);
void dsl_pool_cleanup(dsl_pool_t *dp);
void dsl_pool_cache_tmp_userrefs(dsl_pool_t *dp);
void dsl_pool_tmp_uref_barrier(dsl_pool_t *dp);

    ...
    const zbookmark_t *zb, const dnode_phys_t *dnp, void *arg)
{
	...
	if (zb->zb_object != DMU_META_DNODE_OBJECT &&
		...
		uint64_t span = BP_SPAN(dnp, zb->zb_level);
		/* make sure our free range stays within the max file range */
		if (zb->zb_blkid > UINT64_MAX / span)
			return (0);
		uint64_t offset = zb->zb_blkid * span;
		if (offset > UINT64_MAX - span)
			span = UINT64_MAX - offset;
		err = dump_free(ba, zb->zb_object, offset, span);
		...
		err = dump_spill(ba, zb->zb_object, blksz, abuf->r_data);
		arc_free_ref(abuf);
    ...
    vnode_t *vp, offset_t *off)
{
	...
	if (dmu_objset_type(tosnap) == DMU_OST_ZFS) {
		...
		err = zfs_get_mlslabel(tosnap, ds_mlslabel,
		    sizeof (ds_mlslabel));
		if ((err == 0) && (strcasecmp(ds_mlslabel,
		    ZFS_MLSLABEL_NONE) != 0))
			featureflags |= DMU_BACKUP_FEATURE_LABELED;
	}
#endif
	...
	if (zone_dataset_alias(curzone, drr->drr_u.drr_begin.drr_toname,
...
recv_new_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if (rbsa->origin == NULL) {
		(void) dmu_objset_create_impl(dd->dd_pool->dp_spa,
		    rbsa->ds, &rbsa->ds->ds_phys->ds_bp, rbsa->type,
		    rbsa->dcc, tx);
	}
...
recv_existing_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	dsobj = dsl_dataset_create_sync(ohds->ds_dir, rbsa->clonelastname,
	    ohds->ds_prev, rbsa->dcc, flags, rbsa->cr, tx);
	...
	if (BP_IS_HOLE(dsl_dataset_get_blkptr(cds))) {
		(void) dmu_objset_create_impl(dp->dp_spa, cds,
		    dsl_dataset_get_blkptr(cds), rbsa->type, rbsa->dcc, tx);
	}
...
dmu_recv_verify_features(dsl_dataset_t *ds, struct drr_begin *drrb)
{
	...
	/*
	 * Verify whether the stream has security labels; if so then it
	 * must only be processed if Trusted Extensions is enabled.
	 */
	if ((featureflags & DMU_BACKUP_FEATURE_LABELED) &&
	    !is_system_labeled()) {
		return (B_FALSE);
	}
...
find_ds_by_guid(const char *name, void *arg)
{
	...
	while (lastobj != firstobj) {
		...
		if (dsl_dataset_hold_obj(dp, lastobj, guid_map, &snapds) != 0) {
			/*
			 * Skip this snapshot and move on. It's not
			 * clear why this would ever happen, but the
			 * remainder of the snapshot stream can be
			 * processed.
			 */
			rw_exit(&dp->dp_config_rwlock);
			dsl_dataset_rele(ds, FTAG);
		...
		gmep = avl_find(guid_map, &gmesrch, &where);
		if (gmep != NULL) {
			ASSERT(gmep->gme_ds == snapds);
			dsl_dataset_rele(snapds, guid_map);
		} else {
			gmep = kmem_alloc(sizeof (guid_map_entry_t), KM_SLEEP);
			gmep->guid = snapds->ds_phys->ds_guid;
			gmep->gme_ds = snapds;
			avl_insert(guid_map, gmep, where);
		}
	}
...
restore_spill(struct restorearg *ra, objset_t *os, struct drr_spill *drrs)
{
	...
	if (dmu_bonus_hold(os, drrs->drr_object, FTAG, &bonus) != 0)
		return (EINVAL);

	err = dmu_spill_hold_by_bonus(bonus, FTAG, &db);
	...
	if (db->db_size < drrs->drr_length)
		dbuf_spill_set_blksz(db, drrs->drr_length, tx);
	bcopy(data, db->db_data, drrs->drr_length);
    ...
    struct drr_free *drrf)
{
	...
	if (drrf->drr_length != -1ULL &&
	    drrf->drr_offset > UINT64_MAX - drrf->drr_length)
		return (EINVAL);
    ...
    int cleanup_fd, uint64_t *action_handlep)
{
	...
	if (featureflags & DMU_BACKUP_FEATURE_DEDUP) {
		...
		(void) dmu_objset_find(drc->drc_top_ds, find_ds_by_guid,
		    (void *)ra.guid_to_ds_map, DS_FIND_CHILDREN);
	}

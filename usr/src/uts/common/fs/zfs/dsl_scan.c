...
int zfs_resilver_delay = 0;		/* number of ticks to delay resilver */
...
/*
 * percentage values relative to zfs_txg_synctime_ms
 */
int zfs_scan_min_synctime_pct = 20;
int zfs_free_min_synctime_pct = 20;
int zfs_resilver_synctime_pct = 60;

/*
 * min millisecs to scrub per txg
 */
#define	ZFS_SCAN_MIN_TIME_MS \
	((zfs_scan_min_synctime_pct * zfs_txg_synctime_ms) / 100)

/*
 * min millisecs to free per txg
 */
#define	ZFS_FREE_MIN_TIME_MS \
	((zfs_free_min_synctime_pct * zfs_txg_synctime_ms) / 100)

/*
 * min millisecs to resilver per txg
 */
#define	ZFS_RESILVER_MIN_TIME_MS \
	((zfs_resilver_synctime_pct * zfs_txg_synctime_ms) / 100)
...
dsl_scan_setup_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if (DSL_SCAN_IS_SCRUB_RESILVER(scn)) {
		...
		if (vdev_resilver_needed(spa->spa_root_vdev,
			...
			if (logs_only) {
				/*
				 * resilvering separate log devices is special
				 * because we can skip most of the work, as
				 * only claimed but not yet replayed log chains
				 * located on the slogs need to be visited
				 */
				scn->scn_phys.scn_flags |= DSF_RESILVER_LOGS;
				scn->scn_phys.scn_to_process =
	...
	spa_history_log_internal(LOG_POOL_SCAN, spa, tx,
	    "func=%u mintxg=%llu maxtxg=%llu logs=%d",
	    *funcp, scn->scn_phys.scn_min_txg, scn->scn_phys.scn_max_txg,
	    dsl_scan_logs_only(dp));
}
...
dsl_scan_done(dsl_scan_t *scn, boolean_t complete, dmu_tx_t *tx)
{
	...
	spa_history_log_internal(LOG_POOL_SCAN_DONE, spa, tx,
	    "complete=%u logs=%d", complete, dsl_scan_logs_only(dp));
	...
	if (DSL_SCAN_IS_SCRUB_RESILVER(scn)) {
		...
		/* Reset spa_scrub_started after vdev_dtl_reassess. */
...
static void dsl_scan_visitbp(blkptr_t *bp,
    const zbookmark_t *zb, dnode_phys_t *dnp, dsl_dataset_t *ds,
    dsl_scan_t *scn, dmu_objset_type_t ostype, dmu_tx_t *tx);
static void dsl_scan_visitdnode(dsl_scan_t *, dsl_dataset_t *ds,
    dmu_objset_type_t ostype, dnode_phys_t *dnp, uint64_t object, dmu_tx_t *tx);
...
int
dsl_read(zio_t *pio, spa_t *spa, const blkptr_t *bpp, uint64_t size,
    arc_done_func_t *done, void *private, int priority, enum zio_flag flags,
    arc_options_t options, const zbookmark_t *zb)
{
	return (arc_read(pio, spa, bpp, size,
	    done, private, priority, flags, options, zb));
}
...
/* ARGSUSED */
static void
dsl_scan_prefetch(dsl_scan_t *scn, blkptr_t *bp,
    uint64_t objset, uint64_t object, uint64_t blkid)
{
	...
	/*
	 * XXX BPRW: need to make sure all of these arc_read() prefetches are
	 * done before setting xlateall (similar to dsl_read())
	 */
	(void) arc_read(scn->scn_zio_root, scn->scn_dp->dp_spa, bp,
	    0, NULL, NULL, ZIO_PRIORITY_ASYNC_READ,
	    ZIO_FLAG_CANFAIL | ZIO_FLAG_SCAN_THREAD, 0, &czb);
}
...
dsl_scan_recurse(dsl_scan_t *scn, dsl_dataset_t *ds, dmu_objset_type_t ostype,
    dnode_phys_t *dnp, const blkptr_t *bp, const zbookmark_t *zb, dmu_tx_t *tx)
{
	dsl_pool_t *dp = scn->scn_dp;
	arc_ref_t *buf = NULL;
	int err;
	...
	if (BP_GET_LEVEL(bp) > 0) {
		...
		err = arc_read(NULL, dp->dp_spa, bp, 0,
		    arc_getref_func, &buf, ZIO_PRIORITY_ASYNC_READ,
		    ZIO_FLAG_CANFAIL | ZIO_FLAG_SCAN_THREAD, 0, zb);
		if (err) {
			scn->scn_phys.scn_errors++;
			return (err);
		}
		for (i = 0, cbp = buf->r_data; i < epb; i++, cbp++) {
			dsl_scan_prefetch(scn, cbp, zb->zb_objset,
		...
		for (i = 0, cbp = buf->r_data; i < epb; i++, cbp++) {
			...
			SET_BOOKMARK(&czb, zb->zb_objset, zb->zb_object,
			    zb->zb_level - 1,
			    zb->zb_blkid * epb + i);
			dsl_scan_visitbp(cbp, &czb, dnp, ds, scn, ostype, tx);
		}
		...
		for (i = 0, cdnp = buf->r_data; i < epb; i++, cdnp++) {
			for (j = 0; j < cdnp->dn_nblkptr; j++) {
				blkptr_t *cbp = &cdnp->dn_blkptr[j];
				dsl_scan_prefetch(scn, cbp,
		...
		if (OBJSET_BUF_HAS_USERUSED(buf)) {
			 ...
			 */
			dsl_scan_visitdnode(scn, ds, osp->os_type,
			    &osp->os_groupused_dnode, DMU_GROUPUSED_OBJECT, tx);
			dsl_scan_visitdnode(scn, ds, osp->os_type,
			    &osp->os_userused_dnode, DMU_USERUSED_OBJECT, tx);
		}
	}
    ...
    dmu_objset_type_t ostype, dnode_phys_t *dnp, uint64_t object, dmu_tx_t *tx)
{
	for (int i = 0; i < dnp->dn_nblkptr; i++) {
		...
		SET_BOOKMARK(&czb, ds ? ds->ds_object : 0, object,
		    dnp->dn_nlevels - 1, i);
		dsl_scan_visitbp(&dnp->dn_blkptr[i],
 ...
 */
static void
dsl_scan_visitbp(blkptr_t *bp, const zbookmark_t *zb, dnode_phys_t *dnp,
    dsl_dataset_t *ds, dsl_scan_t *scn, dmu_objset_type_t ostype, dmu_tx_t *tx)
{
	dsl_pool_t *dp = scn->scn_dp;
	...
	dprintf_bp(bp,
	    ...
	    zb->zb_objset, zb->zb_object, zb->zb_level, zb->zb_blkid, bp);
	 ...
	 * callback again.
	 */
	if (ddt_class_contains(dp->dp_spa, scn->scn_phys.scn_ddt_class_max, bp))
		return;
...
dsl_scan_visitds(dsl_scan_t *scn, uint64_t dsobj, dmu_tx_t *tx)
{
	...
	if (DSL_SCAN_IS_SCRUB_RESILVER(scn) && !dsl_dataset_is_snapshot(ds)) {
		if (dsl_scan_logs_only(dp)) {
			zbookmark_t zb;

			SET_BOOKMARK(&zb, dsobj, 0, 0, 0);
			if (dsl_scan_check_pause(scn, &zb)) {
				zfs_dbgmsg("resilvering logs; pausing at "
				    "ds %llu", (u_longlong_t)dsobj);
				goto out;
			}
		}
	...
	/*
	 * If resilvering everything, iterate over the bps in this ds.
	 */
	if (!dsl_scan_logs_only(dp)) {
		...
		zfs_dbgmsg("scanned dataset %llu (%s) with min=%llu max=%llu; "
		    "pausing=%d",
		    (u_longlong_t)dsobj, dsname,
		    (u_longlong_t)scn->scn_phys.scn_cur_min_txg,
		    (u_longlong_t)scn->scn_phys.scn_cur_max_txg,
		    (int)scn->scn_pausing);
		...
		zfs_dbgmsg("skipping bps in ds %llu", dsobj);
	}
...
dsl_scan_sync(dsl_pool_t *dp, dmu_tx_t *tx)
{
	...
	if (scn->scn_restart_txg != 0 &&
		...
		dsl_scan_done(scn, B_FALSE, tx);
		if (vdev_resilver_needed(spa->spa_root_vdev, NULL, NULL, NULL))
...
/*
 * dp_scan is created after a pool is opened. It only makes sense to
 * check if the pool is being resilvered after it's opened.
 */
boolean_t
...
uint64_t
dsl_scan_errors(dsl_pool_t *dp)
{
	return (dp->dp_scan ? dp->dp_scan->scn_phys.scn_errors : 0);
}

uint64_t
dsl_scan_maxtxg(dsl_pool_t *dp)
{
	return (dp->dp_scan ? dp->dp_scan->scn_phys.scn_max_txg : 0);
}

void
dsl_scan_update_processed(dsl_pool_t *dp, uint64_t psize)
{
...
boolean_t
dsl_scan_logs_only(dsl_pool_t *dp)
{
	if (dp->dp_scan)
		return ((dp->dp_scan->scn_phys.scn_flags & DSF_RESILVER_LOGS)
		    != 0);
	return (B_FALSE);
}
...
dsl_scan(dsl_pool_t *dp, pool_scan_func_t func)
{
	 ...
	 * vdev_reopen() checks if a resilver needs to be started or restarted.
	 * If so, this scrub will be stopped and a resilver will be
	 * started or restarted in dsl_scan_sync().
	 */

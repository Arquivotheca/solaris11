    ...
    const char *tail, void *tag, dsl_dir_t **ddp)
{
	...
	if (dd == NULL) {
		...
		dd->dd_compressed_bytes = dd->dd_phys->dd_compressed_bytes;
		dd->dd_uncompressed_bytes = dd->dd_phys->dd_uncompressed_bytes;
		for (i = 0; i < DD_USED_NUM; i++)
			dd->dd_used_breakdown[i] =
			    dd->dd_phys->dd_used_breakdown[i];
    ...
    dmu_tx_t *tx)
{
	...
	if (spa_version(dp->dp_spa) < SPA_VERSION_OPTIONAL_CHILD_DIR) {
...
dsl_dir_destroy_sync(void *arg1, void *tag, dmu_tx_t *tx)
{
	...
	for (t = 0; t < DD_USED_NUM; t++)
		ASSERT3U(dd->dd_used_breakdown[t], ==, 0);

	if (dd->dd_phys->dd_child_dir_zapobj != 0) {
		VERIFY(0 == zap_destroy(mos,
		    dd->dd_phys->dd_child_dir_zapobj, tx));
	}
	VERIFY(0 == zap_destroy(mos, dd->dd_phys->dd_props_zapobj, tx));
	VERIFY(0 == dsl_deleg_destroy(mos, dd->dd_phys->dd_deleg_zapobj, tx));
	VERIFY3U(0, ==, zap_remove(mos,
	    dd->dd_parent->dd_phys->dd_child_dir_zapobj, dd->dd_myname, tx));
	/*
	 * It would be nice to free the child_dir if it is empty, but
	 * some functions (dmu_dir_list_next, dmu_objset_find_spa)
	 * access it without holding the dp_config_rwlock, so they could
	 * access a freed object (or worse, it could be reallocated as
	 * another zap object).
	 */
...
dsl_dir_stats(dsl_dir_t *dd, nvlist_t *nv)
{
	...
	dsl_prop_nvlist_add_uint64(nv, ZFS_PROP_COMPRESSRATIO,
	    dd->dd_compressed_bytes == 0 ? 100 :
	    (dd->dd_uncompressed_bytes * 100 / dd->dd_compressed_bytes));
	if (dd->dd_phys->dd_flags & DD_FLAG_USED_BREAKDOWN) {
		dsl_prop_nvlist_add_uint64(nv, ZFS_PROP_USEDSNAP,
		    dd->dd_used_breakdown[DD_USED_SNAP]);
		dsl_prop_nvlist_add_uint64(nv, ZFS_PROP_USEDDS,
		    dd->dd_used_breakdown[DD_USED_HEAD]);
		dsl_prop_nvlist_add_uint64(nv, ZFS_PROP_USEDREFRESERV,
		    dd->dd_used_breakdown[DD_USED_REFRSRV]);
		dsl_prop_nvlist_add_uint64(nv, ZFS_PROP_USEDCHILD,
		    dd->dd_used_breakdown[DD_USED_CHILD] +
		    dd->dd_used_breakdown[DD_USED_CHILD_RSRV]);
	}
	mutex_exit(&dd->dd_lock);
	...
	if (dsl_dir_is_clone(dd)) {
		...
		(void) zone_dataset_alias(curzone, buf, buf, sizeof (buf));
...
dsl_dir_sync(dsl_dir_t *dd, dmu_tx_t *tx)
{
	...
	dd->dd_phys->dd_compressed_bytes = dd->dd_compressed_bytes;
	dd->dd_phys->dd_uncompressed_bytes = dd->dd_uncompressed_bytes;
	for (i = 0; i < DD_USED_NUM; i++)
		dd->dd_phys->dd_used_breakdown[i] = dd->dd_used_breakdown[i];
	mutex_exit(&dd->dd_lock);
    ...
    dmu_tx_t *tx, boolean_t first)
{
	...
	if (first && tx->tx_objset) {
		...
		if (checkrefquota &&
		    (error = dsl_dataset_check_quota(ds, est_inflight))) {
			mutex_exit(&dd->dd_lock);
			return (error);
		}
		dsl_dataset_adjust_space(ds, asize, est_inflight,
		    &used_on_disk, &ref_rsrv);
	}
...
static void
dsl_dir_willuse_space_impl(dsl_dir_t *dd, int64_t space, boolean_t use_ds_rsrv,
    dmu_tx_t *tx)
{
	int64_t parent_space;
	uint64_t est_used, used_on_disk, ref_rsrv;
	...
	/*
	 * If use_ds_rsrv is raised, adjust our space_towrite with dataset's
	 * used-on-disk and refreservation values.
	 * Note that use_ds_rsrv can be set only on first iteration.
	 */
	if (space > 0 && use_ds_rsrv && tx->tx_objset) {
		dsl_dataset_t *ds = tx->tx_objset->os_dsl_dataset;

		ASSERT(ds->ds_dir == dd);
		dsl_dataset_adjust_space(ds, space, est_used, &used_on_disk,
		    &ref_rsrv);
	}

	parent_space = parent_delta(dd, used_on_disk + est_used,
	    space - ref_rsrv);
 ...
 */
void
dsl_dir_willuse_space(dsl_dir_t *dd, int64_t space, boolean_t use_ds_rsrv,
    dmu_tx_t *tx)
{
	dsl_pool_willuse_space(dd->dd_pool, space, tx);
	dsl_dir_willuse_space_impl(dd, space, use_ds_rsrv, tx);
}
    ...
    int64_t used, int64_t compressed, int64_t uncompressed, dmu_tx_t *tx)
{
	...
	ASSERT(compressed >= 0 || dd->dd_compressed_bytes >= -compressed);
	dd->dd_compressed_bytes += compressed;
	ASSERT(uncompressed >= 0 || dd->dd_uncompressed_bytes >= -uncompressed);
	dd->dd_uncompressed_bytes += uncompressed;

	if (dd->dd_phys->dd_flags & DD_FLAG_USED_BREAKDOWN) {
		ASSERT(used > 0 || dd->dd_used_breakdown[type] >= -used);
		dd->dd_used_breakdown[type] += used;
#ifdef DEBUG
		dd_used_t t;
		uint64_t u = 0;
		for (t = 0; t < DD_USED_NUM; t++)
			u += dd->dd_used_breakdown[t];
		ASSERT3U(u, ==, dd->dd_used_bytes);
#endif
	}
	if (needlock)
		mutex_exit(&dd->dd_lock);
    ...
    dd_used_t oldtype, dd_used_t newtype, dmu_tx_t *tx)
{
	...
	if (needlock)
		mutex_enter(&dd->dd_lock);
	ASSERT(delta > 0 ?
	    dd->dd_used_breakdown[oldtype] >= delta :
	    dd->dd_used_breakdown[newtype] >= -delta);
	ASSERT(dd->dd_used_bytes >= ABS(delta));
...
dsl_dir_set_quota_check(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if ((dmu_tx_is_syncing(tx) || towrite == 0) &&
	    (effective_value < dd->dd_phys->dd_reserved ||
	    effective_value < dd->dd_used_bytes + towrite)) {
		err = ENOSPC;
	}
	mutex_exit(&dd->dd_lock);
	return (err);
...
int
dsl_dir_set_quota(const char *ddname, zprop_source_t source,
    zprop_setflags_t flags, uint64_t quota)
{
	...
	dsl_prop_setarg_init_uint64(&psa, "quota", source, &quota, flags);
	psa.psa_effective_value = &effective_value;
...
dsl_dir_rename_check(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if (ra->newparent->dd_phys->dd_child_dir_zapobj != 0) {
		err = zap_lookup(mos,
		    ra->newparent->dd_phys->dd_child_dir_zapobj,
...
dsl_dir_rename_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if (ra->newparent != dd->dd_parent) {
		dsl_dir_diduse_space(dd->dd_parent, DD_USED_CHILD,
		    -dd->dd_used_bytes,
		    -dd->dd_compressed_bytes,
		    -dd->dd_uncompressed_bytes, tx);
		dsl_dir_diduse_space(ra->newparent, DD_USED_CHILD,
		    dd->dd_used_bytes,
		    dd->dd_compressed_bytes,
		    dd->dd_uncompressed_bytes, tx);

		if (dd->dd_phys->dd_reserved > dd->dd_used_bytes) {
			uint64_t unused_rsrv =
			    dd->dd_phys->dd_reserved - dd->dd_used_bytes;
	...
	if (ra->newparent->dd_phys->dd_child_dir_zapobj == 0) {
		dmu_buf_will_dirty(ra->newparent->dd_dbuf, tx);
		ra->newparent->dd_phys->dd_child_dir_zapobj = zap_create(mos,
		    DMU_OT_DSL_DIR_CHILD_MAP, DMU_OT_NONE, 0, tx);
	}

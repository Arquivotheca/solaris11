...
zcrypt_key_allocate(void)
{
	...
	key = kmem_zalloc(sizeof (zcrypt_key_t), KM_SLEEP);
	...
	caddr_t *wkeybuf, size_t *wkeylen, uint64_t wcrypt)
{
	...
	if (wmech.cm_type == CRYPTO_MECH_INVALID)
		return (EINVAL);
	wkeyphys = kmem_zalloc(sizeof (zcrypt_key_phys_t), KM_SLEEP);
...
zcrypt_keystore_init(spa_t *spa)
{
	...
	rw_init(&spa->spa_keystore->sk_lock, NULL, NULL, NULL);
    ...
    zcrypt_key_t *wrapkey)
{
	...
	tree = &spa->spa_keystore->sk_dslkeys;
	zk = kmem_zalloc(sizeof (zcrypt_keystore_node_t), KM_SLEEP);
    ...
    zcrypt_key_t *key)
{
	...
	dkn = kmem_alloc(sizeof (zcrypt_keychain_node_t), KM_SLEEP);
...
zcrypt_key_gen(int crypt)
{
	...
	genkeybuf = kmem_alloc(genkeylen, KM_SLEEP);

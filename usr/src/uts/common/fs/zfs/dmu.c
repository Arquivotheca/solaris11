...
/* BEGIN CSTYLED */
    ...
    { zap_byteswap,		TRUE,	FALSE,	"scrub work queue"	},
...
uint16_t
dmu_get_bonuslen(dmu_buf_t *db_fake)
{
	dmu_buf_impl_t *db = (dmu_buf_impl_t *)db_fake;
	dnode_t *dn;
	uint16_t bonuslen;
...
get_next_chunk(dnode_t *dn, uint64_t *start, uint64_t limit)
{
	...
	uint64_t blkcnt = 0;
	uint64_t maxbps = DMU_MAX_ACCESS / 2 / SPA_DDTBLOCKSIZE;
	uint64_t maxiblks = maxbps / EPB(dn->dn_indblkshift, SPA_BLKPTRSHIFT);
	uint64_t iblkrange =
	    dn->dn_datablksz * EPB(dn->dn_indblkshift, SPA_BLKPTRSHIFT);

	if (os->os_dedup_checksum == ZIO_CHECKSUM_OFF)
		maxiblks = DMU_MAX_ACCESS / (1ULL << (dn->dn_indblkshift + 1));

	ASSERT(limit <= *start);

	if (len <= iblkrange * maxiblks) {
		*start = limit;
		return (0);
	}
	ASSERT(ISP2(iblkrange));

	while (*start > limit && blkcnt < maxiblks) {
		int err;

		/* find next allocated L1 indirect */
		err = dnode_next_offset(dn,
		    DNODE_FIND_BACKWARDS, start, 2, 1, dnode_trim_txg(dn));
...
dmu_xuio_init(xuio_t *xuio, int nblk)
{
	...
	priv->cnt = nblk;
	priv->bufs = kmem_zalloc(nblk * sizeof (arc_ref_t *), KM_SLEEP);
...
dmu_xuio_fini(xuio_t *xuio)
{
	...
	kmem_free(priv->bufs, nblk * sizeof (arc_ref_t *));
 ...
 */
int
dmu_xuio_add(xuio_t *xuio, arc_ref_t *abuf, offset_t off, size_t n)
{
	struct iovec *iov;
	uio_t *uio = &xuio->xu_uio;
	dmu_xuio_t *priv = XUIO_XUZC_PRIV(xuio);
	...
	iov = uio->uio_iov + i;
	iov->iov_base = (char *)abuf->r_data + off;
	iov->iov_len = n;
	priv->bufs[i] = abuf;
	return (0);
}
...
dmu_read_uio(objset_t *os, uint64_t object, uio_t *uio, uint64_t size)
{
	...
	for (i = 0; i < numbufs; i++) {
		...
		if (xuio) {
			...
			arc_ref_t *abuf = dbuf_hold_arcbuf(dbi);
			err = dmu_xuio_add(xuio, abuf, bufoff, tocpy);
			if (!err) {
				uio->uio_resid -= tocpy;
				uio->uio_loffset += tocpy;
			}
			if (abuf->r_data == db->db_data)
 ...
 */
arc_ref_t *
dmu_request_arcbuf(int size)
{
	return (arc_loan_buf(size));
}
 ...
 */
void
dmu_release_arcbuf(arc_ref_t *buf)
{
	arc_return_ref(buf);
	arc_free_ref(buf);
}
 ...
 * If this is not possible copy the contents to the dbuf via
 * dmu_write().
 */
void
dmu_assign_arcbuf(dmu_buf_t *handle, uint64_t offset, arc_ref_t *buf,
    dmu_tx_t *tx)
{
	...
	if (offset == db->db.db_offset && blksz == db->db.db_size) {
		...
		dbuf_rele(db, FTAG);
		dmu_write(os, object, offset, blksz, buf->r_data, tx);
		dmu_release_arcbuf(buf);
		XUIOSTAT_BUMP(xuiostat_wbuf_copied);
	}
}
...
dmu_sync_done(zio_t *zio)
{
	...
	if (zio->io_error == 0) {
		dr->dr_overridden_by = *zio->io_bp;
		dr->dr_override_state = DR_OVERRIDDEN;
		dr->dr_copies = zio->io_prop.zp_copies;
		if (BP_IS_HOLE(&dr->dr_overridden_by))
			BP_ZERO(&dr->dr_overridden_by);
	} else {
    ...
    zio_prop_t *zp, zbookmark_t *zb)
{
	...
	zio_nowait(zio_write(pio, os->os_spa, dmu_tx_get_txg(tx), zgd->zgd_bp,
	    ...
	    dmu_sync_ready, dmu_sync_late_arrival_done, dsa,
...
dmu_sync(zio_t *pio, uint64_t txg, dmu_sync_cb_t *done, zgd_t *zgd)
{
	...
	if (dr->dr_override_state == DR_IN_DMU_SYNC ||
	    dr->dr_override_state == DR_OVERRIDDEN) {
		/*
	...
	dr->dr_override_state = DR_IN_DMU_SYNC;
	mutex_exit(&db->db_mtx);
	...
	zio_nowait(arc_write(pio, os->os_spa, txg,
	    bp, dr->dr_ref, DBUF_IS_L2CACHEABLE(db), &zp,
	    dmu_sync_ready, dmu_sync_done, dsa,
	    pio->io_priority, ZIO_FLAG_CANFAIL, &zb));
...
dmu_write_policy(objset_t *os, dnode_t *dn, int level, int wp, zio_prop_t *zp)
{
	...
	/*
	 * If data is compressed, it must also be checksummed to ensure
	 * that we only feed known-good input into decompress routines.
	 */
	if (compress != ZIO_COMPRESS_OFF && checksum == ZIO_CHECKSUM_OFF)
		checksum = ZIO_CHECKSUM_FLETCHER_4;
...
dmu_offset_next(objset_t *os, uint64_t object, boolean_t hole, uint64_t *off)
{
	...
	err = dnode_next_offset(dn, (hole ? DNODE_FIND_HOLE : 0), off, 1, 1,
	    dnode_trim_txg(dn));
	dnode_rele(dn, FTAG);
...
/*
 * Determine if the specified object is is free for allocation.
 */
boolean_t
dmu_object_is_free(objset_t *os, uint64_t object)
{
	dnode_t *dn = NULL;

	(void) dnode_hold_impl(os, object, DNODE_MUST_BE_FREE, FTAG, &dn);
...
boolean_t
dmu_buf_freeable(dmu_buf_t *dbuf, boolean_t *refd)
{
	dmu_buf_impl_t *db = (dmu_buf_impl_t *)dbuf;
	uint64_t birth;
	boolean_t res;
	...
	mutex_enter(&db->db_mtx);
	res = dbuf_block_freeable(db, &birth);
	mutex_exit(&db->db_mtx);
	*refd = birth > 0;
	return (res);
}

...
zfs_znode_move_impl(znode_t *ozp, znode_t *nzp)
{
	...
	bcopy(ozp->z_mtime, nzp->z_mtime, sizeof (uint64_t) * 2);
	bcopy(ozp->z_ctime, nzp->z_ctime, sizeof (uint64_t) * 2);
    ...
    uint_t flag, znode_t **zpp, zfs_acl_ids_t *acl_ids)
{
	...
	pflags |= acl_ids->z_aclp->z_hints &
	    (ZFS_ACL_WIDE_FLAGS|ZFS_NO_EXECS_DENIED);
	...
	sa_handle_get_from_db(zfsvfs->z_os, db, NULL, SA_HDL_SHARED, &sa_hdl);
	...
	if (!(flag & IS_ROOT_NODE)) {
		...
		*zpp = dzp;
		(*zpp)->z_pflags = pflags;
		(*zpp)->z_mode = (mode_t)mode;
		(*zpp)->z_sa_hdl = sa_hdl;
		(*zpp)->z_atime[0] = atime[0];
		(*zpp)->z_atime[1] = atime[1];
		(*zpp)->z_mtime[0] = mtime[0];
		(*zpp)->z_mtime[1] = mtime[1];
		(*zpp)->z_ctime[0] = ctime[0];
		(*zpp)->z_ctime[1] = ctime[1];
	}
...
void
zfs_tstamp_update_setup(znode_t *zp, uint_t flag, boolean_t have_tx)
{
	timestruc_t	now;
...
top:
	...
	if (end > zp->z_blksz &&
		...
		if (zp->z_blksz > zp->z_zfsvfs->z_max_blksz) {
			...
			newblksz = MIN(end, spa_max_block_size(
 ...
 * zfs_create_fs()
 *
 * Called either directly from dsl_pool_create() for the root
 * filesystem of the pool, or as a callback when a new file system
 * (dataset) is created within the pool.  Receives as "arg" a
 * possibly NULL pointer to an nvlist containing any desired
 * create-time ZPL properties.
 */
void
zfs_create_fs(objset_t *os, void *arg, cred_t *cr, dmu_tx_t *tx)
{
	zfsvfs_t	zfsvfs;
	uint64_t	moid, obj, sa_obj, version;
	uint64_t	sense = ZFS_CASE_MIXED;
	uint64_t	norm = 0;
	nvpair_t	*elem;
	int		error;
	int		i;
	...
	/*
	 * Set starting attributes.  Note any changes to default
	 * normalization or case-sensitivity.
	 */
	...
	while ((elem = nvlist_next_nvpair(zplprops, elem)) != NULL) {
		...
		if (strcmp(name, zfs_prop_to_name(ZFS_PROP_VERSION)) == 0) {
		    ...
		    zfs_prop_to_name(ZFS_PROP_CASE)) == 0) {
			sense = val;
		} else if (strcmp(name,
		    zfs_prop_to_name(ZFS_PROP_NORMALIZE)) == 0) {
			norm = val;
		} else {
	...
	/*
	 * Record version, sensitivity and normalization in the master
	 * node.  Default "mixed" sensitivity will not work on older
	 * pools, so fall back to case-sensitive if the pool is immature.
	 */
	ASSERT(version != 0);
	error = zap_update(os, moid, ZPL_VERSION_STR, 8, 1, &version, tx);
	ASSERT(error == 0);

	if (version < ZPL_VERSION_NORMALIZATION)
		sense = ZFS_CASE_SENSITIVE;
	error = zap_update(os, moid, zfs_prop_to_name(ZFS_PROP_CASE),
	    8, 1, &sense, tx);
	ASSERT(error == 0);

	error = zap_update(os, moid, zfs_prop_to_name(ZFS_PROP_NORMALIZE),
	    8, 1, &norm, tx);
	ASSERT(error == 0);
...
zfs_obj_to_pobj(sa_handle_t *hdl, sa_attr_type_t *sa_table, uint64_t *pobjp,
    uint64_t *flagsp, uint64_t *modep)
{
	uint64_t parent, flags, mode, links;
	sa_bulk_attr_t bulk[4];
	int count = 0;
	int error;
	...
	*flagsp = flags;
	...
	return (links == 0 ? ENOENT : 0);
}

/*
 * An XATTR object can still look "live" when the xattr
 * directory has been freed and is in the "unlinked set".
 * If this occurs, either the object or its parent will
 * have a link count of zero.
 */
static int
zfs_xattr_obj_is_live(objset_t *osp, uint64_t obj, sa_handle_t *hdl,
    sa_attr_type_t *sa_table)
{
	uint64_t parent, mode, flags, links;
	sa_handle_t *parent_hdl;
	dmu_buf_t *parent_db = NULL;
	int error;

	/* Is the object an XATTR? */
	if ((error = zfs_obj_to_pobj(hdl, sa_table, &parent,
	    &flags, &mode)) != 0)
		return (error);
	if (! (flags & ZFS_XATTR))
		return (0);
	...
	/* Does the parent have a link count of zero? */
	error = zfs_grab_sa_handle(osp, parent, &parent_hdl,
	    &parent_db, FTAG);
	if (error != 0)
		return (error);
	error = sa_lookup(parent_hdl, sa_table[ZPL_LINKS],
	    &links, sizeof (links));
	if (error == 0 && links == 0)
		error = ENOENT;
	zfs_release_sa_handle(parent_hdl, parent_db, FTAG);
    ...
    sa_attr_type_t *sa_table, char *buf, int len)
{
	...
	*path = '\0';
	...
	for (;;) {
		...
		component[0] = '/';
		if (((flags & ZFS_XATTR) != 0) && S_ISDIR(mode)) {
    ...
    zfs_stat_t *sb, zfsvfs_t *zv)
{
	...
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_PARENT], NULL,
	    &sb->zs_parent, sizeof (sb->zs_parent));
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_SIZE], NULL,
	    &sb->zs_size, sizeof (sb->zs_size));
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_CTIME], NULL,
	    &sb->zs_ctime, sizeof (sb->zs_ctime));
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_MTIME], NULL,
	    &sb->zs_mtime, sizeof (sb->zs_mtime));
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_ATIME], NULL,
	    &sb->zs_atime, sizeof (sb->zs_atime));
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_CRTIME], NULL,
	    &sb->zs_crtime, sizeof (sb->zs_crtime));

	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_UID], NULL,
	    &zuid, sizeof (zuid));
	SA_ADD_BULK_ATTR(bulk, count, sa_table[ZPL_GID], NULL,
	    &zgid, sizeof (zgid));

	error = sa_bulk_lookup(hdl, bulk, count);
	if (error == 0) {
		sb->zs_uid = zfs_fuid_map_id(zv, zuid, kcred, ZFS_OWNER);
		sb->zs_gid = zfs_fuid_map_id(zv, zgid, kcred, ZFS_GROUP);
	}
...
int
zfs_obj_to_stats(zfsvfs_t *zfsvfs, uint64_t obj, zfs_stat_t *sb,
    char *pathbuf, int pathbuflen)
{
	objset_t *osp = zfsvfs->z_os;
	int error;
	dmu_buf_t *db;
	sa_handle_t *hdl;
	sa_attr_type_t *sa_table;
	...
	sb->zs_nameerr = ZFS_NAMEERR_NO_NAME_REQUESTED;
	error = zfs_grab_sa_handle(osp, obj, &hdl, &db, FTAG);
	if (error != 0) {
		return (error);
	}

	error = zfs_obj_to_stats_impl(hdl, sa_table, sb, zfsvfs);

	if (error == 0) {
		if (pathbuflen) {
			if ((sb->zs_nameerr = zfs_obj_to_path_impl(osp, obj,
			    hdl, sa_table, pathbuf, pathbuflen)) != ESRCH) {
				/*
				 * Errors other than "cannot resolve path"
				 * must be returned to the caller.
				 */
				error = sb->zs_nameerr;
			}
		} else {
			error = zfs_xattr_obj_is_live(osp, obj, hdl, sa_table);
		}
	}

...
space_map_add(space_map_t *sm, uint64_t start, uint64_t size)
{
	...
	if (ss != NULL && ss->ss_start <= start && ss->ss_end >= end) {
		if (sm->sm_condense) {
			zfs_dbgmsg("zfs: allocating allocated segment "
			    "(sm=%p offset=%llu size=%llu)",
			    sm, (longlong_t)start, (longlong_t)size);
			sm->sm_recovered = B_TRUE;
			return;
		}
	...
	space_map_obj_t *smo, objset_t *os)
{
	...
	for (offset = 0; offset < end; offset += bufsize) {
		...
		for (entry = entry_map; entry < entry_map_end; entry++) {
			...
			if (SM_DEBUG_DECODE(e)) {
				/*
				 * If we're loading a pool which hasn't been
				 * condensed since the specified txg, mark
				 * it for compaction.
				 */
				if (offset == 0 && maptype == SM_FREE &&
				    SM_DEBUG_TXG_DECODE(e) <=
				    spa_condense_txg(dmu_objset_spa(os)))
	...
	if (error == 0) {
		if (sm->sm_recovered)
			zfs_dbgmsg("sm=%p space recovered %llu",
			    sm, space - sm->sm_space);
		else

...
spa_prop_get_config(spa_t *spa, nvlist_t **nvp)
{
	...
	spa_prop_add_list(*nvp, ZPOOL_PROP_GUID, NULL, spa_guid(spa),
	    ZPROP_SRC_NONE);
...
spa_prop_get(spa_t *spa, nvlist_t **nvp)
{
	...
	for (zap_cursor_init(&zc, mos, spa->spa_pool_props_object);
		...
		if (ZA_PROP_IS_INT(za)) {
			/*
			 * If it's the bootfs property, save it for later. We
			 * mustn't grab the dp_config_rwlock inside the
			 * spa_props_lock or we'll deadlock with the sync task.
			 * Otherwise, we can just add it to the nvlist.
			 */
			if (prop == ZPOOL_PROP_BOOTFS)
				bootfs = za.za_first_integer;
			else
				spa_prop_add_list(*nvp, prop, NULL,
				    za.za_first_integer, ZPROP_SRC_LOCAL);
		} else {
			char *strval = kmem_alloc(za.za_num_integers, KM_SLEEP);

			ASSERT(ZA_PROP_IS_STR(za));
			err = zap_lookup(mos, spa->spa_pool_props_object,
			    za.za_name, 1, za.za_num_integers, strval);
			if (err == 0)
				spa_prop_add_list(*nvp, prop, strval, 0,
				    ZPROP_SRC_LOCAL);
	...
	if (bootfs != 0) {
		...
		if ((err = dsl_dataset_hold_obj(dp, bootfs, FTAG, &ds)) == 0) {
			int size = MAXNAMELEN + strlen(MOS_DIR_NAME) + 1;
			char *strval = kmem_alloc(size, KM_SLEEP);

			dsl_dataset_name(ds, strval);
			dsl_dataset_rele(ds, FTAG);

			spa_prop_add_list(*nvp, ZPOOL_PROP_BOOTFS, strval, 0,
			    ZPROP_SRC_LOCAL);
			kmem_free(strval, size);
		}
	...
	while ((elem = nvlist_next_nvpair(props, elem)) != NULL) {
		...
		case ZPOOL_PROP_BOOTFS:
			...
			if (!error) {
				...
				/*
				 * Must be ZPL not gzip compressed
				 * and not encrypted.
				 */
...
spa_config_valid(spa_t *spa, nvlist_t *config)
{
	...
	for (int c = 0; c < rvd->vdev_children; c++) {
		...
		if (tvd->vdev_ops == &vdev_missing_ops &&
			...
			/*
			 * Check if log vdev is "missing" according to DTL
			 */
			if (!vdev_dtl_empty(tvd, DTL_MISSING) &&
    ...
    char **ereport)
{
	...
	if (spa_version(spa) >= SPA_VERSION_ONE_MEG_BLKSZ) {
		spa->spa_maxblksize = SPA_MAXBLOCKSIZE;
		spa->spa_maxblkshift = SPA_MAXBLOCKSHIFT;
	}
	...
	if (!mosconfig) {
		...
		if (!spa_is_root(spa) && state != SPA_LOAD_TRYIMPORT &&
	...
	/*
	 * Load the condense txg value, if it exists.  Otherwise, we
	 * set it to the last txg we synced out.
	 */
	spa->spa_condense_txg = spa_last_synced_txg(spa);
	error = spa_dir_prop(spa, DMU_POOL_CONDENSE_TXG,
	    &spa->spa_condense_txg);
	if (error != 0 && error != ENOENT)
		return (spa_vdev_err(rvd, VDEV_AUX_CORRUPT_DATA, EIO));
	...
	if (spa_writeable(spa) && (state == SPA_LOAD_RECOVER ||
		...
		VERIFY(zap_update(spa->spa_meta_objset,
		    DMU_POOL_DIRECTORY_OBJECT, DMU_POOL_CONDENSE_TXG,
		    sizeof (uint64_t), 1, &spa->spa_condense_txg, tx) == 0);
		dmu_tx_commit(tx);
		...
		if (!dsl_scan_resilvering(spa->spa_dsl_pool) &&
		    vdev_resilver_needed(rvd, NULL, NULL, NULL))
			spa_async_request(spa, SPA_ASYNC_RESILVER);

		/*
		 * Transfer any stale temporary dataset userrefs to
		 * a dsl_pool list for later processing.
		 */
		dsl_pool_cache_tmp_userrefs(spa->spa_dsl_pool);

		/* Schedule any necessary DSL pool cleanup. */
		spa_async_request(spa, SPA_ASYNC_POOL_CLEANUP);
	}
    ...
    nvlist_t *nvpolicy)
{
	...
	ASSERT(*state != SPA_LOAD_RECOVER);

	zpool_get_load_policy(nvpolicy, &policy);

	if (policy.zrp_request & ZPOOL_DO_REWIND) {
		*state = SPA_LOAD_RECOVER;
	} else if (spa->spa_last_open_failed != 0 &&
	    !(policy.zrp_request & ZPOOL_RETRY_LOAD)) {
	...
	min_txg = (policy.zrp_request & ZPOOL_EXTREME_REWIND) ?
    ...
    nvlist_t **config)
{
	...
	*spapp = NULL;
	...
	if (spa->spa_state == POOL_STATE_UNINITIALIZED) {
		nvlist_t *policy = nvpolicy ? nvpolicy : spa->spa_config;

		spa_activate(spa, spa_mode_global);
		error = spa_load_best(spa, &state, B_FALSE, policy);
...
int
spa_open_policy(const char *name, spa_t **spapp, void *tag, nvlist_t *policy,
    nvlist_t **config)
{
    ...
    const char *history_str, struct dsl_crypto_ctx *dcc, nvlist_t *zplprops)
{
	...
	if (version >= SPA_VERSION_ONE_MEG_BLKSZ) {
		spa->spa_maxblksize = SPA_MAXBLOCKSIZE;
		spa->spa_maxblkshift = SPA_MAXBLOCKSHIFT;
	}
...
static nvlist_t *
spa_generate_rootconf(char *devpath, char *devid)
{
	nvlist_t *config;
 ...
 */
static void
spa_alt_rootvdev(uint64_t pguid, vdev_t *vd, vdev_t **avd, uint64_t txg)
{
	for (int c = 0; c < vd->vdev_children; c++)
		spa_alt_rootvdev(pguid, vd->vdev_child[c], avd, txg);
...
spa_import_rootpool(char *devpath, char *devid)
{
	 ...
	 */
	config = spa_generate_rootconf(devpath, devid);
	...
	VERIFY(nvlist_lookup_uint64(config, ZPOOL_CONFIG_POOL_GUID,
	    &pguid) == 0);
	avd = bvd;
	spa_alt_rootvdev(pguid, rvd, &avd, txg);
	if (avd != bvd) {
		cmn_err(CE_NOTE, "The boot device is out of date. Please "
...
spa_import(const char *pool, nvlist_t *config, nvlist_t *props, uint64_t flags)
{
	 ...
	 * doing an import.
	 */
	error = spa_load_best(spa, &state, B_TRUE, config);
...
nvlist_t *
spa_tryimport(nvlist_t *tryconfig, boolean_t trusted)
{
	nvlist_t *config = NULL;
	char *poolname, *hostname = NULL;
	spa_t *spa;
	uint64_t state, hostid = 0;
	int error;
	...
	if (!trusted)
		/* change poolname to detect if MOS config was loaded */
		VERIFY(nvlist_add_string(tryconfig, ZPOOL_CONFIG_POOL_NAME,
		    TRYIMPORT_NAME) == 0);
	 ...
	 * Pass 'trusted' for mosconfig because untrusted config may not
	 * reflect current pool name/guid recorded in MOS config
	 */
	error = spa_load(spa, SPA_LOAD_TRYIMPORT, SPA_IMPORT_EXISTING, trusted);

	if (!trusted) {
		VERIFY(nvlist_lookup_string(spa->spa_config,
		    ZPOOL_CONFIG_POOL_NAME, &poolname) == 0);
		VERIFY(nvlist_lookup_uint64(spa->spa_config,
		    ZPOOL_CONFIG_POOL_STATE, &state) == 0);

		/* hostname and hostid may not be present for old configs */
		(void) nvlist_lookup_string(spa->spa_config,
		    ZPOOL_CONFIG_HOSTNAME, &hostname);
		(void) nvlist_lookup_uint64(spa->spa_config,
		    ZPOOL_CONFIG_HOSTID, &hostid);
	}
	/*
	 * If 'tryconfig' was at least parsable and in case of untrusted
	 * 'tryconfig' config from MOS was loaded successfully,
	 * return the current config.
	 */
	if (spa->spa_root_vdev != NULL &&
	    strcmp(poolname, TRYIMPORT_NAME) != 0) {
		config = spa_config_generate(spa, NULL, -1ULL, B_TRUE);
    ...
    boolean_t force, boolean_t hardforce)
{
	...
	pool_active = (spa->spa_state != POOL_STATE_UNINITIALIZED) &&
	    spa->spa_sync_on && !spa_refcount_zero(spa);
	/*
	 ...
	 * We only need to call txg_wait_synced() when there are
	 * open refs.
	 */
	if (pool_active)
		txg_wait_synced(spa->spa_dsl_pool, 0);
...
spa_vdev_attach(spa_t *spa, uint64_t guid, nvlist_t *nvroot, int replacing)
{
	...
	/*
	 * Reopen vdevs so that we can correctly assess the state of the newly
	 * attached device. Make sure DTL_MISSING is updated since vdev_reopen()
	 * will start or restart the resilvering based on DTL_MISSING.
	 */
	vdev_reopen(rvd);

	/*
	 * We're about to operate on this spa after dropping the config lock
	 * so hold onto a reference.
	 */
	spa_open_ref(spa, FTAG);
...
spa_vdev_detach(spa_t *spa, uint64_t guid, uint64_t pguid, int replace_done)
{
	...
	/*
	 * If we are resilvering everything check if we still need to resilver
	 * everything and restart if we need to resilver just slogs
	 */
	if (dsl_scan_resilvering(spa->spa_dsl_pool) &&
	    !dsl_scan_logs_only(spa->spa_dsl_pool)) {
		boolean_t logs_only = B_TRUE;

		if (vdev_resilver_needed(spa->spa_root_vdev,
		    NULL, NULL, &logs_only) && logs_only == B_TRUE)
			spa_async_request(spa, SPA_ASYNC_RESILVER);
	}
    ...
    nvlist_t *props, boolean_t exp)
{
	...
	for (c = 0; c < children; c++) {
		...
		VERIFY(nvlist_add_uint64(child[c], ZPOOL_CONFIG_ASIZE,
		    vml[c]->vdev_top->vdev_min_asize) == 0);
...
spa_vdev_remove_evacuate(spa_t *spa, vdev_t *vd)
{
	...
	if (vd->vdev_islog) {
		...
		if (error == 0 && vd->vdev_stat.vs_alloc != 0)
			error = EBUSY;
	} else {
		error = ENOTSUP;
	}
...
spa_vdev_remove_from_namespace(spa_t *spa, vdev_t *vd)
{
	...
	/*
	 * vs_alloc should be zero by now.
	 */
	...
	if (last_vdev) {
		uint64_t c = id - 1;
		while (rvd->vdev_child[c]->vdev_ishole) {
			vdev_free(rvd->vdev_child[c]);
			rvd->vdev_child[c--] = NULL;
		}
...
spa_scan(spa_t *spa, pool_scan_func_t func)
{
	...
	ASSERT(func != POOL_SCAN_RESILVER);
...
spa_async_thread(spa_t *spa)
{
	...
	/*
	 * Kick off any DSL pool cleanup.
	 */
	if (tasks & SPA_ASYNC_POOL_CLEANUP)
		dsl_pool_cleanup(spa_get_dsl(spa));
...
spa_sync_upgrades(spa_t *spa, dmu_tx_t *tx)
{
	...
	if (spa->spa_ubsync.ub_version >= SPA_VERSION_ONE_MEG_BLKSZ) {
		spa->spa_maxblksize = SPA_MAXBLOCKSIZE;
		spa->spa_maxblkshift = SPA_MAXBLOCKSHIFT;
	}
}
...
spa_sync(spa_t *spa, uint64_t txg)
{
	...
	spa->spa_sync_pass = 0;
	spa->spa_sync_start  = ddi_get_lbolt64();
	...
	if (!spa_shutting_down(spa) &&
	    (!txg_list_empty(&dp->dp_dirty_datasets, txg) ||
	...
	} while (dmu_objset_is_dirty_in_txg(mos, txg));

...
/*
 * return true if we are in a non-global zone and
 * the dataset doesn't have zoned set.
 * otherwise return false
 */
static boolean_t
dsl_deleg_skip_non_zoned(dsl_dir_t *dd)
{
	...
	if (crgetzoneid(CRED()) != GLOBAL_ZONEID) {
		...
		if ((dsl_prop_get_dd_zone(dd,
		    zfs_prop_to_name(ZFS_PROP_ZONED),
		    8, 1, &zoned, NULL, B_FALSE,
		    DSL_PROP_GET_EFFECTIVE, crgetzone(CRED())) != 0) ||
		    !zoned)
			error = B_TRUE;
	}
	return (error);
...
dsl_deleg_get(const char *ddname, nvlist_t **nvp)
{
	...
	for (dd = startdd; dd != NULL; dd = dd->dd_parent) {
		 ...
		 */
		if (dsl_deleg_skip_non_zoned(dd))
			break;
    ...
    boolean_t dosets, uint64_t uid, dmu_tx_t *tx)
{
	...
	for (zap_cursor_init(&zc, mos, pjumpobj);
	    ...
	    zap_cursor_advance(&zc)) {
		uint64_t zero = 0;
		ASSERT(ZA_PROP_IS_INT(za) && za.za_num_integers == 1);

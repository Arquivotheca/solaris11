...
dnode_init(void)
{
	...
	dnode_cache = kmem_cache_create("dnode_t",
	    sizeof (dnode_t), 0, dnode_cons, dnode_dest, NULL, NULL, NULL, 0);
    ...
    uint64_t object, dnode_handle_t *dnh)
{
	...
	ASSERT(dnp->dn_type < DMU_OT_NUMTYPES);
...
dnode_destroy(dnode_t *dn)
{
	...
	ASSERT((dn->dn_id_flags & DN_ID_AFTER) == 0);
	...
	if (dn->dn_bonus != NULL) {
		...
		arc_free_ref(dn->dn_bonus->db_ref);
		dn->dn_bonus->db_ref = NULL;
    ...
    dmu_object_type_t bonustype, int bonuslen, dmu_tx_t *tx)
{
	...
	have_spill = (dn->dn_phys->dn_flags & DNODE_FLAG_SPILL_BLKPTR) > 0;
	rw_exit(dn->dn_phys_rwlock);
...
dnode_move_impl(dnode_t *odn, dnode_t *ndn)
{
	...
	ndn->dn_phys_rwlock = odn->dn_phys_rwlock;
...
dnode_move(void *buf, void *newbuf, size_t size, void *arg)
{
	...
	ASSERT(dn->dn_dbuf != NULL); /* only "special" dnodes have no parent */
	 ...
	 * go away, neither can its handle. Once we lock the handle, we can
	 * safely move any dnode referenced only by dbufs.
	 */
	if (!rw_tryenter(&dn->dn_handle->dnh_rwlock, RW_WRITER)) {
		mutex_exit(&os->os_lock);
	 ...
	 * progress.
	 */
	if (!rw_tryenter(&dn->dn_struct_rwlock, RW_WRITER)) {
		rw_exit(&dn->dn_handle->dnh_rwlock);
		mutex_exit(&os->os_lock);
	...
	/*
	 * When evicting a dbuf we may not have an active dnode hold. In that
	 * case, the dbuf count is decremented under the handle lock before the
	 * dbuf's hold is released. This order ensures that if we count the hold
	 * after the dbuf is removed from dn_dbufs but before its hold is
	 * released, we will treat the unmatched hold as active and exit here.
	 */
	mutex_enter(&dn->dn_dbufs_mtx);
	refcount = refcount_count(&dn->dn_holds);
	ASSERT(refcount >= 0);
	dbufs = dn->dn_dbufs_count;
	mutex_exit(&dn->dn_dbufs_mtx);

	/* We can't have more dbufs than dnode holds. */
	ASSERT3U(dbufs, <=, refcount);
	DTRACE_PROBE3(dnode__move, dnode_t *, dn, int64_t, refcount,
	    uint32_t, dbufs);
	 ...
	 */
	dnode_move_impl(dn, newbuf);
	dn = newbuf;

	/* If the dnode was safe to move, the refcount cannot have changed. */
	ASSERT3U(refcount, ==, refcount_count(&dn->dn_holds));
	ASSERT3U(dbufs, ==, dn->dn_dbufs_count);
	rw_exit(&dn->dn_handle->dnh_rwlock);
	mutex_exit(&os->os_lock);
...
dnode_special_close(dnode_handle_t *dnh)
{
	...
	ASSERT(refcount_is_zero(&dn->dn_holds));
	/* Need to clean up special instances of locks */
	rw_destroy(dn->dn_phys_rwlock);
	kmem_free(dn->dn_phys_rwlock, sizeof (krwlock_t));
	DN_HOLD_HANDLE(dnh);
	dnode_destroy(dn); /* implicit handle release */
	rw_destroy(&dnh->dnh_rwlock);
	dnh->dnh_dnode = NULL;
}
    ...
    dnode_handle_t *dnh)
{
	...
	/* Need to create special instances of dnh_rwlock and dn_phys_rwlock */
	dnh->dnh_dnode = dn;
	rw_init(&dnh->dnh_rwlock, NULL, RW_REENTRANT, NULL);
	dn->dn_phys_rwlock = kmem_alloc(sizeof (krwlock_t), KM_SLEEP);
	rw_init(dn->dn_phys_rwlock, NULL, RW_DEFAULT, NULL);
	DNODE_VERIFY(dn);
	return (dn);
}

void
dnode_buf_will_dirty(dmu_buf_t *fake_db)
{
	dmu_buf_impl_t *db = (dmu_buf_impl_t *)fake_db;
	dnode_list_t *children;

	if (arc_try_make_writable(db->db_ref, B_TRUE, B_TRUE)) {
		ASSERT(db->db_ref->r_data == db->db.db_data);
		return;
	}

	/*
	 * Fix up the phys pointers if the data pointer changes when
	 * we make our buffer writable.
	 */
	children = dmu_buf_get_user(fake_db);
	rw_enter(&children->dl_phys_rwlock, RW_WRITER);
	arc_make_writable(db->db_ref);
	if (db->db_ref->r_data != db->db.db_data) {
		int idx;

		db->db.db_data = db->db_ref->r_data;
		for (idx = 0; idx < children->dl_count; idx++) {
			if (children->dl_dnodes[idx].dnh_dnode) {
				dnode_handle_t *dnh = &children->dl_dnodes[idx];
				DN_HOLD_HANDLE(dnh);
				dnh->dnh_dnode->dn_phys =
				    (dnode_phys_t *)db->db.db_data+idx;
				DN_RELEASE_HANDLE(dnh);
			}
		}
	}
	rw_exit(&children->dl_phys_rwlock);
}
...
{
	dnode_list_t *children = arg;

	rw_enter(&children->dl_phys_rwlock, RW_WRITER);
	for (int i = 0; i < children->dl_count; i++) {
		dnode_handle_t *dnh = &children->dl_dnodes[i];
		dnode_t *dn;
	...
	rw_exit(&children->dl_phys_rwlock);
	rw_destroy(&children->dl_phys_rwlock);
	kmem_free(children, sizeof (dnode_list_t) +
	    (children->dl_count - 1) * sizeof (dnode_handle_t));
}
    ...
    void *tag, dnode_t **dnp)
{
	...
	dnodes = dmu_buf_get_user(&db->db);
	if (dnodes == NULL) {
		dnode_list_t *winner;
		int list_size = sizeof (dnode_list_t) +
		    (epb - 1) * sizeof (dnode_handle_t);
		dnodes = kmem_zalloc(list_size, KM_SLEEP);
		rw_init(&dnodes->dl_phys_rwlock, NULL, RW_DEFAULT, NULL);
		rw_enter(&dnodes->dl_phys_rwlock, RW_WRITER);
		if (winner = dmu_buf_set_user(&db->db, dnodes, NULL,
		    dnode_buf_pageout)) {
			rw_exit(&dnodes->dl_phys_rwlock);
			rw_destroy(&dnodes->dl_phys_rwlock);
			kmem_free(dnodes, list_size);
			dnodes = winner;
		} else {
			dnodes->dl_count = epb;
			for (int i = 0; i < epb; i++)
				rw_init(&dnodes->dl_dnodes[i].dnh_rwlock,
				    NULL, RW_REENTRANT, NULL);
			rw_exit(&dnodes->dl_phys_rwlock);
		}
	}
	rw_enter(&dnodes->dl_phys_rwlock, RW_READER);
	ASSERT3U(dnodes->dl_count, ==, epb);
	dnh = &dnodes->dl_dnodes[idx];
	DN_HOLD_HANDLE(dnh);
	if ((dn = dnh->dnh_dnode) == NULL) {
		...
		dn->dn_phys_rwlock = &dnodes->dl_phys_rwlock;
		if (winner = atomic_cas_ptr(&dnh->dnh_dnode, NULL, dn)) {
			/* need extra hold, since the destroy will drop one */
			DN_HOLD_HANDLE(dnh);
			dnode_destroy(dn);
			dn = winner;
		}
		...
		/*
		 * Reset the dnode phys pointer in case db_data has
		 * changed (e.g. if the dbuf goes inactive) since this
		 * dnode was last instantiated.
		 */
		dn->dn_phys = (dnode_phys_t *)db->db.db_data+idx;
	}
	rw_exit(&dnodes->dl_phys_rwlock);
...
{
	uint64_t refs;
	/* Get these values before releasing our hold: */
	...
	/*
	 * If this was the last hold on the dnode, drop its hold on its
	 * parent dbuf (Note: the DNODE_DNODE does not have a parent)
	 */
	if (refs == 0 && db != NULL) {
		/*
		 * Note: callers should not be holding the dnode handle
		 * if there is a chance we could end up here.
		 */
		dbuf_rele(db, dnh);
	}
}
 ...
 */
int
dnode_set_blksz(dnode_t *dn, uint64_t size, int ishift, dmu_tx_t *tx)
{
	dmu_buf_impl_t *db;
	int err = ENOTSUP;
	...
	if (size > spa_max_block_size(dn->dn_objset->os_spa))
		size = spa_max_block_size(dn->dn_objset->os_spa);
	else
		size = P2ROUNDUP(size, SPA_MINBLOCKSIZE);

	if (ishift == dn->dn_indblkshift)
		ishift = 0;

	if (size >> SPA_MINBLOCKSHIFT == dn->dn_datablkszsec && ishift == 0)
		return (0);

	rw_enter(&dn->dn_struct_rwlock, RW_WRITER);

	if (ishift && dn->dn_nlevels != 1)
		goto out;
	...
	mutex_enter(&dn->dn_dbufs_mtx);
	for (db = list_head(&dn->dn_dbufs); db;
	...
	/* is there an existing data block to resize? */
	err = dbuf_hold_impl(dn, 0, 0, TRUE, FTAG, &db);
	if (err == 0) {
		/*
		 * It would be nice to insist (and verify) that there are
		 * no outstanding holds on the dbuf besides our own (so
		 * we can be sure noone will be modifying), but unfortunately
		 * some callers (zap) insist on having outstanding holds on
		 * the 0 block when they call us (*sigh*).
		 * if (refcount_count(&db->db_holds) > db->db_dirtycnt + 1)
		 * 	err = ENOTSUP;
		 */
		err = dbuf_read(db, NULL,
	...
	if (ishift) {
		dn->dn_indblkshift = ishift;
		dn->dn_next_indblkshift[tx->tx_txg&TXG_MASK] = ishift;
	}
	/* XXX - could we drop the struct lock before we resize? */
	/* I think no, because the struct lock prevents new refs */
	if (db) {
	...
	if (new_nlevels > dn->dn_nlevels) {
		...
		for (dr = list_head(list); dr; dr = dr_next) {
			...
			if (dr->dr_dbuf->db_level != new_nlevels-1 &&
				...
				list_insert_tail(&new->dr_children, dr);
				dr->dr_parent = new;
			}
		}
		mutex_exit(&new->dr_mtx);
		mutex_exit(&dn->dn_mtx);
	}
...
/*
 * Account for dedup table updates
 */
static void
dnode_dirty_ddt(blkptr_t *bplist, int start, int len, dmu_tx_t *tx)
{
	for (int i = start; i < len; i++)
		if (BP_GET_DEDUP(&bplist[i]))
...
dnode_free_range(dnode_t *dn, uint64_t off, uint64_t len, dmu_tx_t *tx)
{
	...
	if (head) {
		...
		if (len < head)
			head = len;
		if (dbuf_hold_impl(dn, 0,
		    dbuf_whichblock(dn, off), TRUE, FTAG, &db) == 0) {
			caddr_t data;
	...
	if (tail) {
		if (len < tail)
			tail = len;
		if (dbuf_hold_impl(dn, 0,
		    dbuf_whichblock(dn, off+len), TRUE, FTAG, &db) == 0) {
			rw_exit(&dn->dn_struct_rwlock);
			dbuf_will_dirty(db, tx);
			rw_enter(&dn->dn_struct_rwlock, RW_WRITER);
	...
	if (dn->dn_nlevels > 1) {
		...
		if (last > first && (db = dbuf_hold_level(dn, 1, last, FTAG))) {
			dbuf_will_dirty(db, tx);
			dnode_dirty_ddt(db->db.db_data, 0,
			    trunc ? epb : (blkid + nblks - 1) & (epb - 1), tx);
			dbuf_rele(db, FTAG);
		}
		for (i = first + 1; i < last; i++) {
			...
			if (db) {
				dbuf_will_dirty(db, tx);
				dnode_dirty_ddt(db->db.db_data, 0, epb, tx);
				dbuf_rele(db, FTAG);
			}
		}
	}
...
dnode_willuse_space(dnode_t *dn, int64_t space, dmu_tx_t *tx)
{
	...
	if (ds)
		dsl_dir_willuse_space(ds->ds_dir, space, B_TRUE, tx);
...
uint64_t
dnode_trim_txg(dnode_t *dn)
{
	return (dsl_dataset_trim_txg(dn->dn_objset->os_dsl_dataset));
}

    ...
    krw_t lti, boolean_t fatreader, boolean_t adding, zap_t **zapp)
{
	...
	*zapp = NULL;
	...
	if (zap->zap_ismicro && tx && adding &&
		...
		ASSERT3U(err, ==, 0);
		err = dmu_buf_hold(os, obj, 0, NULL, &db, DMU_READ_PREFETCH);
		ASSERT3U(err, ==, 0);
		dmu_buf_will_dirty(db, tx);
    ...
    dmu_object_type_t bonustype, int bonuslen, dmu_tx_t *tx)
{
	...
	ASSERT(leaf_blockshift >= SPA_MINBLOCKSHIFT &&
	    leaf_blockshift <= SPA_128KBLOCKSHIFT &&
	    indirect_blockshift >= SPA_MINBLOCKSHIFT &&
	    indirect_blockshift <= SPA_128KBLOCKSHIFT);
...
zap_cursor_retrieve(zap_cursor_t *zc, zap_attribute_t *za)
{
	...
	if (!zc->zc_zap->zap_ismicro) {
		...
		if (mze) {
			...
			za->za_integer_length = ZA_INTEGER_SIZE;
...
zap_count_write(objset_t *os, uint64_t zapobj, const char *name, int add,
    uint64_t *towrite, uint64_t *tooverwrite, uint64_t *tounref)
{
	zap_t *zap;
	boolean_t refd;
	int err = 0;
	...
	/*
	 * Lock the zap with 'adding' == FALSE so that we do not
	 * accidentally trigger an upgrade.
	 */
	err = zap_lockdir(os, zapobj, NULL, RW_READER, TRUE, FALSE, &zap);
	if (err)
		return (err);
	...
	/*
	 * If we don't have a name, we cannot figure out which blocks will
	 * be affected in this operation. So, account for the worst case :
	 * - 3 blocks overwritten: target leaf, ptrtbl block, header block
	 * - 4 new blocks written if adding:
	 * 	- 2 blocks for possibly split leaves,
	 * 	- 2 grown ptrtbl blocks
	 * - 3 blocks unreferenced: target leaf, ptrtbl block, header block
	 *
	 * This also accomodates the case where an add operation to a fairly
	 * large microzap results in a promotion to fatzap.
	 */
	if (name == NULL) {
		if (dmu_buf_freeable(zap->zap_dbuf, &refd))
			*tooverwrite += 3 * SPA_128KBLOCKSIZE;
		else
			*towrite += 3 * SPA_128KBLOCKSIZE;
		if (add)
			*towrite += 4 * SPA_128KBLOCKSIZE;
		if (refd)
			*tounref += 3 * SPA_128KBLOCKSIZE;

	} else if (!zap->zap_ismicro) {
		...
		if (zn) {
			...
			/*
			 * As with the (name == NULL) case, account for
			 * the worst case:
			 */
			if (dmu_buf_freeable(zap->zap_dbuf, &refd))
				*tooverwrite += 3 * SPA_128KBLOCKSIZE;
		 ...
		 * Micro-zap:
		 *
		 * Note, it is difficult to determine if an add operation will
		 * promote us to a fat zap, so we must account for the
		 * possibility.
		 * - 1 block overwritten  : header
		 * - 4 new blocks if adding/promoting:
		 *	- 2 blocks for possibly split leaves,
		 *	- 2 grown ptrtbl blocks
		 * - 1 block unreferenced : header
		 */
		if (dmu_buf_freeable(zap->zap_dbuf, &refd))

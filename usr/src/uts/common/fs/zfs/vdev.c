...
static boolean_t vdev_resilver_restart_needed(vdev_t *vd);
...
/*
 * Default asize function: round up psize to the top-level alignment.
 * This is what's used by anything other than RAID-Z.
 */
uint64_t
vdev_default_asize(vdev_t *vd, uint64_t psize, dva_layout_t layout, int copies)
{
	ASSERT(layout == DVA_LAYOUT_STANDARD);
	ASSERT(copies == 1);

	return (P2ROUNDUP(psize, 1ULL << vd->vdev_ashift));
}

/*
 * Default layout function: standard layout, single copy.
 * This is what's used by anything other than RAID-Z.
 */
/*ARGSUSED*/
uint64_t
vdev_default_layout(vdev_t *vd, uint64_t psize, int ndvas, int flag, dva_t *dva)
{
	dva_layout_t layout = DVA_LAYOUT_STANDARD;
	int copies = 1;

	DVA_SET_LAYOUT(dva, layout);
	DVA_SET_COPIES(dva, copies);

	return (vdev_psize_to_asize(vd, psize, layout, copies));
}
...
vdev_get_min_asize(vdev_t *vd)
{
	...
	if (vd == vd->vdev_top) {
		/*
		 * If we already know our min asize, return that.
		 */
		if (vd->vdev_min_asize > 0)
...
static void
vdev_propagate_min_asize(vdev_t *vd)
{
	vd->vdev_min_asize = vdev_get_min_asize(vd);

	for (int c = 0; c < vd->vdev_children; c++)
		vdev_propagate_min_asize(vd->vdev_child[c]);
}
    ...
    int alloctype)
{
	...
	/*
	 * Mirror and RAID-Z can only be top-level vdevs.
	 * (Other vdevs that use mirroring logic -- like spare
	 * and replacing -- have their own ops vectors.)
	 */
	if ((ops == &vdev_mirror_ops || ops == &vdev_raidz_ops) &&
	    (parent == NULL || parent->vdev_parent != NULL))
		return (EINVAL);
	...
	if (parent && !parent->vdev_parent &&
		...
		vd->vdev_asize = vd->vdev_min_asize;
	}
...
vdev_metaslab_init(vdev_t *vd, uint64_t txg)
{
	...
	uint64_t newc = vd->vdev_min_asize >> vd->vdev_ms_shift;
	metaslab_t **mspp;
	int error;
	...
	vd->vdev_deflate_ratio = (1 << 17) /
	    (vdev_psize_to_asize(vd, 1 << 17, DVA_LAYOUT_STANDARD, 1) >>
	    SPA_MINBLOCKSHIFT);
...
/*
 * Number of seconds beyond which a probe is considered to have timed out
 */
hrtime_t zfs_vdev_probe_time_limit = 120;

#define	SEC_TO_NANO(sec)	((sec) * NANOSEC)

typedef struct vdev_probe_stats {
	zio_t		*vps_timed_probe;
	hrtime_t	vps_probe_start;
	boolean_t	vps_async_probe;
	boolean_t	vps_timing_out;
...
vdev_probe_done(zio_t *zio)
{
	...
	if (vps->vps_timed_probe == zio && zio->io_error != 0)
		vps->vps_timing_out =
		    gethrtime() - vps->vps_probe_start >=
		    SEC_TO_NANO(zfs_vdev_probe_time_limit);
	...
	if (zio->io_type == ZIO_TYPE_READ) {
		...
		if (zio->io_error == 0 && spa_writeable(spa)) {
			zio_t *wio = zio_write_phys(vd->vdev_probe_zio, vd,
			    zio->io_offset, zio->io_size, zio->io_data,
			    ZIO_CHECKSUM_OFF, vdev_probe_done, vps,
			    ZIO_PRIORITY_SYNC_WRITE, vps->vps_flags, B_TRUE);

			/*
			 * If this was a timed read probe, time the
			 * write probe too.
			 */
			if (vps->vps_timed_probe == zio) {
				vps->vps_timed_probe = wio;
				vps->vps_probe_start = gethrtime();
			}
		...
		vd->vdev_timing_out |= vps->vps_timing_out;
		...
		/*
		 * If the current probe was not async and the vdev is
		 * not timing out, request an async probe.
		 */
		if (!vps->vps_async_probe && !vd->vdev_timing_out) {
	...
	/*
	 * If the timed probes of the first vdev label have completed,
	 * submit the reads for the remaining labels.
	 */
	if (((zio->io_type == ZIO_TYPE_READ && !spa_writeable(spa)) ||
	    zio->io_type == ZIO_TYPE_WRITE) &&
	    vps->vps_timed_probe == zio && !vps->vps_timing_out) {
		vps->vps_timed_probe = NULL;

		for (int l = 2; l < VDEV_LABELS; l++) {
			zio_nowait(zio_read_phys(zio_unique_parent(zio),
			    vd, vdev_label_offset(vd->vdev_psize, l,
...
vdev_probe(vdev_t *vd, zio_t *zio)
{
	...
	/*
	 * To avoid an extended hang if the device is timing out,
	 * initially only issue a probe I/O for the first label.
	 */
	vps->vps_probe_start = gethrtime();
	vps->vps_timed_probe = zio_read_phys(pio, vd,
	    vdev_label_offset(vd->vdev_psize, 1,
	    offsetof(vdev_label_t, vl_pad2)),
	    VDEV_PAD_SIZE, zio_buf_alloc(VDEV_PAD_SIZE),
	    ZIO_CHECKSUM_OFF, vdev_probe_done, vps,
	    ZIO_PRIORITY_SYNC_READ, vps->vps_flags, B_TRUE);
	zio_nowait(vps->vps_timed_probe);
...
vdev_open(vdev_t *vd)
{
	...
	} else if (ashift > vd->vdev_top->vdev_ashift) {
		/*
		 * The alignment requirement has increased, therefore
	...
	if (vd->vdev_state == VDEV_STATE_HEALTHY &&
	    asize > vd->vdev_min_asize &&
	    (vd->vdev_expanding || spa->spa_autoexpand)) {
		vd->vdev_asize = asize;
		vd->vdev_min_asize = 0;
	}

	vdev_propagate_min_asize(vd);
...
vdev_reopen(vdev_t *vd)
{
	...
	/*
	 * Check if a resilver needs to be started or restarted.
	 */
	vdev_dtl_reassess(vd, 0, 0, B_FALSE);
	if (dsl_scan_resilvering(spa->spa_dsl_pool))
		start_resilver = vdev_resilver_restart_needed(vd);
	else
		start_resilver = vdev_resilver_needed(vd, NULL, NULL, NULL);
	if (start_resilver)
...
static uint64_t
vdev_dtl_space(vdev_t *vd, vdev_dtl_type_t t)
{
	space_map_t *sm = &vd->vdev_dtl[t];
	uint64_t space;
...
vdev_dtl_reassess(vdev_t *vd, uint64_t txg, uint64_t scrub_txg, int scrub_done)
{
	...
	if (vd->vdev_ops->vdev_op_leaf) {
		...
		if (scrub_txg != 0 &&
		    ...
		    dsl_scan_errors(spa->spa_dsl_pool) == 0)) {
			/*
			 * We completed a scrub up to scrub_txg.  If we
			 * did it without rebooting, then the scrub dtl
			 * will be available in core, so excise the old region
			 * and fold in the scrub dtl.  Otherwise, leave the
			 ...
			 * entries in the reference tree with positive
			 * refcnt -- either 1 or 2 can be converted into the
			 * new DTL_MISSING map.
			 */
...
vdev_dtl_required(vdev_t *vd)
{
	...
	/*
	 * Compare the OUTAGE dtl between this vdev being readable (before)
	 * and this vdev being not-readable (after). If there is any increased
	 * OUTAGE dtl from this transition, this vdev is required to stay in
	 * the pool and should not be offlined/detached/removed.
	 */
	vd->vdev_cant_read = B_FALSE;
	vdev_dtl_reassess(tvd, 0, 0, B_FALSE);
	before = vdev_dtl_space(tvd, DTL_OUTAGE);

	vd->vdev_cant_read = B_TRUE;
	vdev_dtl_reassess(tvd, 0, 0, B_FALSE);
	after = vdev_dtl_space(tvd, DTL_OUTAGE);
...
/*
 * Determine if resilver is needed, and if so the txg range and scope.
 * lop, if given, should point to a variable initialized with B_TRUE.
 */
boolean_t
vdev_resilver_needed(vdev_t *vd, uint64_t *minp, uint64_t *maxp, boolean_t *lop)
{
	...
	if (vd->vdev_children == 0) {
		...
		for (int c = 0; c < vd->vdev_children; c++) {
			...
			if (vdev_resilver_needed(cvd, &cmin, &cmax, lop)) {
	...
	if (needed) {
		if (lop && vd->vdev_top && !vd->vdev_top->vdev_islog)
			*lop = B_FALSE;
		if (minp) {
			*minp = thismin;
			*maxp = thismax;
...
/*
 * Determine if a resilver needs to be restarted.
 */
static boolean_t
vdev_resilver_restart_needed(vdev_t *vd)
{
	boolean_t needed = B_FALSE;
	...
	if (vd->vdev_children == 0) {
		...
		if (vdev_resilver_needed(vd, &min, &max, NULL) &&
		    (max > dsl_scan_maxtxg(vd->vdev_spa->spa_dsl_pool)))
			needed = B_TRUE;
	} else {
		for (int c = 0; c < vd->vdev_children; c++) {
			vdev_t *cvd = vd->vdev_child[c];

			if (vdev_resilver_restart_needed(cvd)) {
				needed = B_TRUE;
				break;
			}
		}
...
uint64_t
vdev_psize_to_asize(vdev_t *vd, uint64_t psize, dva_layout_t layout, int copies)
{
	ASSERT(vd == vd->vdev_top);

	return (vd->vdev_ops->vdev_op_asize(vd, psize, layout, copies));
}

uint64_t
vdev_layout(vdev_t *vd, uint64_t psize, int ndvas, int msflag, dva_t *dva)
{
	ASSERT(vd == vd->vdev_top);

	if (spa_version(vd->vdev_spa) < SPA_VERSION_RAIDZ_MIRROR)
		return (vdev_default_layout(vd, psize, ndvas, msflag, dva));

	return (vd->vdev_ops->vdev_op_layout(vd, psize, ndvas, msflag, dva));
}
...
vdev_clear(spa_t *spa, vdev_t *vd)
{
	...
	if (vd->vdev_faulted || vd->vdev_degraded ||
		...
		/* vdev_reopen will start or restart a resilver if needed */
...
boolean_t
vdev_is_preallocatable(vdev_t *vd)
{
	char *type = vd->vdev_ops->vdev_op_type;

	for (int c = 0; c < vd->vdev_children; c++)
		if (!vdev_is_preallocatable(vd->vdev_child[c]))
			return (B_FALSE);
	if (strcmp(type, VDEV_TYPE_ROOT) == 0 ||
	    strcmp(type, VDEV_TYPE_MIRROR) == 0 ||
	    strcmp(type, VDEV_TYPE_REPLACING) == 0 ||
	    strcmp(type, VDEV_TYPE_DISK) == 0 ||
	    strcmp(type, VDEV_TYPE_SPARE) == 0 ||
	    strcmp(type, VDEV_TYPE_L2CACHE) == 0)
		return (!vd->vdev_islog);
	return (B_FALSE);
}
	...
	if (zio->io_error == 0) {
		...
		if (flags & ZIO_FLAG_IO_REPAIR) {
			if ((flags & ZIO_FLAG_SCAN_THREAD) &&
				...
				dsl_scan_update_processed(spa->spa_dsl_pool,
				    psize);

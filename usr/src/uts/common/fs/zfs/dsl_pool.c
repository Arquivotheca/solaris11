...
dsl_pool_open_impl(spa_t *spa, uint64_t txg)
{
	...
	mutex_init(&dp->dp_tmp_uref_lock, NULL, MUTEX_DEFAULT, NULL);
	list_create(&dp->dp_tmp_uref_list, sizeof (dsl_tmp_uref_t),
	    offsetof(dsl_tmp_uref_t, link));
	cv_init(&dp->dp_tmp_uref_cv, NULL, CV_DEFAULT, NULL);
...
dsl_pool_close(dsl_pool_t *dp)
{
	...
	mutex_destroy(&dp->dp_tmp_uref_lock);
	list_destroy(&dp->dp_tmp_uref_list);
	cv_destroy(&dp->dp_tmp_uref_cv);
	kmem_free(dp, sizeof (dsl_pool_t));
}

dsl_pool_t *
dsl_pool_create(spa_t *spa, dsl_crypto_ctx_t *dcc, nvlist_t *zplprops,
    uint64_t txg)
{
	int err;
	dsl_pool_t *dp = dsl_pool_open_impl(spa, txg);
	...
	if (spa_version(spa) >= SPA_VERSION_DEADLISTS) {
		...
		obj = bpobj_alloc(dp->dp_meta_objset, SPA_128KBLOCKSIZE, tx);
	...
	zfs_create_fs(os, zplprops, kcred, tx);
	os->os_initializing = B_FALSE;
#endif
	dsl_dataset_rele(ds, FTAG);

	/* The pool is new, no temp snapshot holds to release. */
	dp->dp_processed_tmp_urefs = B_TRUE;
...
dsl_pool_sync(dsl_pool_t *dp, uint64_t txg)
{
	...
	if (spa_sync_pass(dp->dp_spa) == 1) {
		/*
		 * NOTE: dsl_dataset_snapshot_reserve_space() will increase
		 * dp_space_towrite but not actually write anything.
		 */
		dp->dp_written_in_sync = dp->dp_space_towrite[txg & TXG_MASK];
		dp->dp_sync_write_time = 0;
	...
	dp->dp_sync_write_time += gethrtime() - start;
	ASSERT(err == 0);
	...
	if (list_head(&mos->os_dirty_dnodes[txg & TXG_MASK]) != NULL ||
		...
		zio = zio_root(dp->dp_spa, NULL, NULL, ZIO_FLAG_MUSTSUCCEED);
		dmu_objset_sync(mos, zio, &dp->dp_meta_rootbp, tx);
		err = zio_wait(zio);
		ASSERT(err == 0);
	...
	dp->dp_sync_write_time += gethrtime() - start;
	DTRACE_PROBE2(pool_sync__4io, hrtime_t, dp->dp_sync_write_time,
	    hrtime_t, dp->dp_read_overhead);
	/* XXX - do we really want to factor this out? */
	dp->dp_sync_write_time -= dp->dp_read_overhead;
...
dsl_pool_sync_done(dsl_pool_t *dp, uint64_t txg)
{
	...
	space = dp->dp_space_towrite[txg & TXG_MASK];
	atomic_add_64(&dp->dp_writes_inflight, -space);
	...
	while (ds = list_head(&dp->dp_synced_datasets)) {
		...
		ASSERT(!dmu_objset_is_dirty_in_txg(os, txg));
		dmu_buf_rele(ds->ds_dbuf, ds);
	}
	ASSERT(!dmu_objset_is_dirty_in_txg(dp->dp_meta_objset, txg));
	...
	if (dp->dp_written_in_sync > zfs_write_limit_min / 8 &&
	    dp->dp_sync_write_time > MICROSEC) {
		uint64_t throughput = dp->dp_written_in_sync /
		    (dp->dp_sync_write_time / MICROSEC);
...
dsl_pool_adjustedsize(dsl_pool_t *dp, boolean_t netfree)
{
	...
	resv = SPA_SPACE_RESERVE(space, netfree);
	return (space - resv);
}

/*
 * Make sure that total estimated inflight writes for the whole pool
 * is less then adjusted free space for that pool.
 */
static int
dsl_pool_reserve_inflight(dsl_pool_t *dp, uint64_t *tempreserved,
    uint64_t space)
{
	uint64_t defer, free, towrt;

	do {
		defer = spa_get_deferred(dp->dp_spa);
		free = spa_get_adjfree(dp->dp_spa);
		towrt = dp->dp_writes_inflight;
	...
	} while (atomic_cas_64(&dp->dp_writes_inflight, towrt,
	    towrt + space) != towrt);

	atomic_add_64(tempreserved, space);
	return (0);
}
...
dsl_pool_tempreserve_space(dsl_pool_t *dp, uint64_t space, dmu_tx_t *tx)
{
	...
	if (zfs_no_write_throttle) {
		return (dsl_pool_reserve_inflight(dp,
	...
	if ((err = dsl_pool_reserve_inflight(dp,
	    &dp->dp_tempreserved[tx->tx_txg & TXG_MASK], space)) != 0)
		return (err);
...
{
	uint64_t space_inuse = dp->dp_writes_inflight;
...
dsl_pool_upgrade_dir_clones(dsl_pool_t *dp, dmu_tx_t *tx)
{
	...
	obj = dmu_object_alloc(dp->dp_meta_objset, DMU_OT_BPOBJ,
	    SPA_128KBLOCKSIZE, DMU_OT_BPOBJ_HDR, sizeof (bpobj_phys_t), tx);
...
/*
 * Walk through the pool-wide list of temporary snapshot user holds
 * and release them.
 */
static void
dsl_pool_clean_tmp_userrefs(dsl_pool_t *dp)
{
	...
	mutex_enter(&dp->dp_tmp_uref_lock);
	while (up = list_head(&dp->dp_tmp_uref_list)) {
		(void) dsl_dataset_user_release_tmp(dp, up->dsobj, up->htag,
		    B_FALSE);
		list_remove(&dp->dp_tmp_uref_list, up);
		kmem_free(up, sizeof (dsl_tmp_uref_t));
	}
	dp->dp_processed_tmp_urefs = B_TRUE;
	cv_broadcast(&dp->dp_tmp_uref_cv);
	mutex_exit(&dp->dp_tmp_uref_lock);
}

void
dsl_pool_cache_tmp_userrefs(dsl_pool_t *dp)
{
	zap_attribute_t za;
	zap_cursor_t zc;
	objset_t *mos = dp->dp_meta_objset;
	...
	ASSERT(!dp->dp_processed_tmp_urefs);
	...
	for (zap_cursor_init(&zc, mos, zapobj);
		...
		up = kmem_alloc(sizeof (dsl_tmp_uref_t), KM_SLEEP);
		htag = strchr(za.za_name, '-');
		*htag = '\0';
		++htag;
		up->dsobj = strtonum(za.za_name, NULL);
		(void) strlcpy(up->htag, htag, sizeof (up->htag));
		list_insert_tail(&dp->dp_tmp_uref_list, up);
	}
	zap_cursor_fini(&zc);
}
...
void
dsl_pool_cleanup(dsl_pool_t *dp)
{
	/* Delete any inconsistent datasets. */
	(void) dmu_objset_find(spa_name(dp->dp_spa), dsl_destroy_inconsistent,
	    dp, DS_FIND_CHILDREN);

	/* Release any stale temporary snapshot holds. */
	dsl_pool_clean_tmp_userrefs(dp);
}

/*
 * Following pool import, we need to prevent new temporary snapshot holds
 * from coming in until we've released any stale ones.  Otherwise, a user
 * can get EEXIST by colliding with a tag that shouldn't really be there.
 */
void
dsl_pool_tmp_uref_barrier(dsl_pool_t *dp)
{
	mutex_enter(&dp->dp_tmp_uref_lock);
	if (!dp->dp_processed_tmp_urefs) {
		cv_wait(&dp->dp_tmp_uref_cv, &dp->dp_tmp_uref_lock);
	}
	mutex_exit(&dp->dp_tmp_uref_lock);
}

...
static void sa_build_index(sa_handle_t *hdl, sa_buf_type_t buftype);
...
#define	SA_COPY_DATA(f, s, t, l) \
	{ \
		if (f == NULL) { \
			if (l == 8 && \
			    IS_P2ALIGNED(t, sizeof (uint64_t)) && \
			    IS_P2ALIGNED(s, sizeof (uint64_t))) { \
...
sa_get_spill(sa_handle_t *hdl)
{
	...
	if (hdl->sa_spill == NULL) {
		err = dmu_spill_hold_existing(hdl->sa_bonus, NULL,
		    &hdl->sa_spill);
		if (err == 0) {
			sa_build_index(hdl, SA_SPILL);
			dmu_buf_manage(hdl->sa_spill);
		}
	}
	return (err);
}
...
{
	sa_os_t *sa = hdl->sa_os->os_sa;
	boolean_t bonus_dirty = B_FALSE;
	boolean_t spill_dirty = B_FALSE;
	int error = 0;

	ASSERT(count > 0);
	for (int i = 0; i != count; i++) {
		sa_attr_type_t attr = bulk[i].sa_attr;
		sa_idx_tab_t *table;
		ASSERT(attr <= sa->sa_num_attrs);

		/* First check the bonus buffer */

		if ((table = hdl->sa_bonus_tab) != NULL &&
		    TOC_ATTR_PRESENT(table->sa_idx_tab[attr])) {
			if (tx && !bonus_dirty) {
				dmu_buf_will_dirty(hdl->sa_bonus, tx);
				bonus_dirty = B_TRUE;
			}
			sa_hdr_phys_t *data = hdl->sa_bonus->db_data;
			bulk[i].sa_size = SA_ATTR_LEN(sa, table, attr, data);
			bulk[i].sa_addr =  (void *)((uintptr_t)data +
			    (uintptr_t)TOC_OFF(table->sa_idx_tab[attr]));
		} else if ((error = sa_get_spill(hdl)) == 0 &&
		    (table = hdl->sa_spill_tab) != NULL &&
		    TOC_ATTR_PRESENT(table->sa_idx_tab[attr])) {
			if (tx && !spill_dirty) {
				dmu_buf_will_dirty(hdl->sa_spill, tx);
				spill_dirty = B_TRUE;
			}
			sa_hdr_phys_t *data = hdl->sa_spill->db_data;
			bulk[i].sa_size = SA_ATTR_LEN(sa, table, attr, data);
			bulk[i].sa_addr =  (void *)((uintptr_t)data +
			    (uintptr_t)TOC_OFF(table->sa_idx_tab[attr]));
		} else {
    ...
    uint64_t lot_num, uint64_t hash, boolean_t zapadd, dmu_tx_t *tx)
{
	...
	for (i = 0; i != attr_count; i++) {
		if (SA_REGISTERED_LEN(sa, tb->lot_attrs[i]) == 0)
			tb->lot_var_sizes++;
	}
...
{
	VERIFY(size <= SPA_128KBLOCKSIZE);
	dbuf_spill_set_blksz(hdl->sa_spill, size, tx);
}
...
#define	GROWHDR(hdrsize, length_space) { \
    hdrsize += sizeof (uint64_t); \
    length_space += (sizeof (uint64_t) >> 1); \
}
...
sa_find_sizes(sa_os_t *sa, sa_bulk_attr_t *attr_desc, int attr_count,
    dmu_buf_t *db, sa_buf_type_t buftype, int *spill_point, int *total,
    boolean_t *will_spill)
{
	...
	*total = 0;
	...
	for (i = 0; i != attr_count; i++) {
		...
		/*
		 * Round up to 8 byte boundary for the beginning of
		 * each new attribute.
		 *
		 * The last attribute gets an exception for this.
		 * This is typically a symlink and changing this would
		 * break boot for sparc+intel, which finds the symlink in
		 * a special way.
		 */
		...
		*total += attr_desc[i].sa_length;
		...
		/*
		 * If not last attribute and it has a size of zero
		 * then reserve 8 bytes for NULL attributes
		 */
		if (is_var_sz && attr_desc[i].sa_length == 0 &&
		    (i + 1) != attr_count)
			*total += 8;

		/*
		 * No more hdrsize growth is allowed once we are known to
		 * be spilling
		 */
		if (*will_spill) {
			ASSERT(*spill_point != -1);
			continue;
		}
		...
		if (is_var_sz && length_space < var_size) {
			/*
			 * grow hdrsize by 8 bytes and see if it will
			 * still fit in bonus
			 */
			if (buftype == SA_BONUS &&
			    P2ROUNDUP(hdrsize + sizeof (uint64_t) +
			    *total, 8) <= full_space) {
				GROWHDR(hdrsize, length_space);
			} else { /* won't fit, force spill if bonus */
				/*
				 * If spill block then just continue on.
				 * spill will be resized to fit data in
				 * sa_build_layouts.
				 *
				 * If spilling we now know that this attribute
				 * is larger than space from spill point to
				 * end of buffer
				 */
				if (buftype == SA_BONUS) {
					*will_spill = B_TRUE;
		...
		/*
		 * Determine spill point and header size at spill point
		 */

		if (buftype == SA_BONUS && P2ROUNDUP(*total + hdrsize, 8) <=
		    (full_space - sizeof (blkptr_t))) {
			spill_point_hdrsize = hdrsize;
		} else if (buftype == SA_BONUS && *spill_point == -1) {
			ASSERT(P2ROUNDUP(*total + hdrsize, 8) >
			    (full_space - sizeof (blkptr_t)));
			*spill_point = i;
		}
	...
	/*
	 * This condition shouldn't occur, but we should handle
	 * it in case someday something other than the ZPL uses
	 * system attributes
	 */
	if (spill_point == 0 && *will_spill) {
		*total = 0;
		return (0);
	}
	return (*will_spill ? spill_point_hdrsize : hdrsize);
}
    ...
    dmu_tx_t *tx)
{
	...
	hdrsize = sa_find_sizes(sa, attr_desc, attr_count, hdl->sa_bonus,
	    SA_BONUS, &spill_point, &used, &spilling);

	if (used > SPA_128KBLOCKSIZE)
		return (EFBIG);

	dmu_buf_will_dirty(hdl->sa_bonus, tx);

	/*
	 * buf_space with hdrsize included
	 */
	buf_space = spilling ? MIN(DN_MAX_BONUSLEN - sizeof (blkptr_t),
	    used + hdrsize) : used + hdrsize;
	VERIFY(0 == dmu_set_bonus(hdl->sa_bonus, buf_space, tx));

	bonus_attrs = (buf_space != 0);

	ASSERT((bonustype == DMU_OT_ZNODE && spilling == 0) ||
	    bonustype == DMU_OT_SA);

	ASSERT(hdl->sa_os->os_crypt == ZIO_CRYPT_OFF ||
	    (hdl->sa_os->os_crypt != ZIO_CRYPT_OFF && spilling));

	/* setup and size spill buffer when needed */
	if (spilling) {
		boolean_t dummy;
		int dummy_spill_point;
		dmu_buf_t *spilldb = hdl->sa_spill;

		if (spilldb == NULL) {
			VERIFY(dmu_spill_hold_by_bonus(hdl->sa_bonus, NULL,
			    &spilldb) == 0);
			dmu_buf_manage(spilldb);
		}

		spillhdrsize = sa_find_sizes(sa, &attr_desc[spill_point],
		    attr_count - spill_point, spilldb,
		    SA_SPILL, &dummy_spill_point, &spill_used, &dummy);

		if (spill_used > SPA_128KBLOCKSIZE) {
			if (hdl->sa_spill == NULL)
				dmu_buf_rele(spilldb, NULL);
			return (EFBIG);
		}

		if (hdl->sa_spill == NULL)
			hdl->sa_spill = spilldb;

		dmu_buf_will_dirty(hdl->sa_spill, tx);

		/*
		 * clear out bonus data in case bonus db has some non-zero data
		 * in it.  We don't want sa_build_index() to interpret it
		 * incorrectly.
		 *
		 * Should only be an issue if sa_force_spill is toggled
		 * live and file previously had attributes in bonus buffer.
		 */
		if (!bonus_attrs)
			bzero(hdl->sa_bonus->db_data, DN_MAX_BONUSLEN);
	...
	/*
	 * Drop buf_space by hdrsize since we start out at an address
	 * past the hdr.
	 */
	if (bonus_attrs) {
		buf_space -= hdrsize;
		ASSERT(buf_space > 0 && buf_space <= DN_MAX_BONUSLEN);
	}
	...
	for (i = 0, len_idx = 0, hash = -1ULL; i != attr_count; i++) {
		...
		if (i == spill_point && spilling) {  /* switch to spill */
			VERIFY(bonustype == DMU_OT_SA);
			if (buftype == SA_BONUS && bonus_attrs) {
				sa_find_layout(hdl->sa_os, hash,
				    attrs_start, lot_count, tx, &lot);
		...
		ASSERT(buf_space >= length);
		SA_COPY_DATA(attr_desc[i].sa_data_func, attr_desc[i].sa_data,
		    data_start, length);
		if (SA_REGISTERED_LEN(sa, attrs[i]) == 0) {
			sahdr->sa_lengths[len_idx++] = length;
		}
		data_start = (void *)P2ROUNDUP(((uintptr_t)data_start +
		    MAX(length, sizeof (uint64_t))), 8);
		/*
		 * If not last attribute then make sure we decrement
		 * buf_space by at least 8 bytes.
		 */
		if (i + 1 != attr_count)
			buf_space -=
			    P2ROUNDUP(MAX(length, sizeof (uint64_t)), 8);
		else
    ...
    sa_iterfunc_t func, sa_lot_t *tab, void *userp)
{
	...
	for (i = 0; i != tb->lot_attr_count; i++) {
		...
		reg_length = SA_REGISTERED_LEN(sa, tb->lot_attrs[i]);
		if (reg_length) {
		...
		data_start = (void *)P2ROUNDUP(((uintptr_t)data_start +
		    MAX(attr_length, sizeof (uint64_t))), 8);
	}
}
...
sa_byteswap(sa_handle_t *hdl, sa_buf_type_t buftype)
{
	...
	/*
	 * N.B. this will make *all* spill buffers anonymous if they need
	 * swapping if we could prevent concurrent access (from snapshots, etc),
	 * we could perhaps modify without making them anonymous...
	 */
	if (buftype == SA_SPILL)
		VERIFY(arc_try_make_writable(db->db_ref, B_TRUE, B_FALSE));
	else
		arc_ref_thaw(db->db_ref);

	sa_hdr_phys = SA_GET_HDR(hdl, buftype);
	sa_hdr_phys->sa_magic = BSWAP_32(sa_hdr_phys->sa_magic);
	VERIFY(sa_hdr_phys->sa_magic == SA_MAGIC);
...
{
	sa_hdr_phys_t *sa_hdr_phys;
	dmu_buf_t *db = (dmu_buf_t *)SA_GET_DB(hdl, buftype);
	...
	if (buftype == SA_BONUS && (dmu_get_bonuslen(db) == 0))
		return;
	...
	if (IS_SA_BONUSTYPE(bonustype) &&
	    sa_hdr_phys->sa_magic && sa_hdr_phys->sa_magic != SA_MAGIC)
    ...
    sa_handle_type_t hdl_type, sa_handle_t **handlepp)
{
	...
	if (handle == NULL) {
		...
		sa_build_index(handle, SA_BONUS);
    ...
    sa_handle_type_t hdl_type, sa_handle_t **handlepp)
{
	...
	sa_handle_get_from_db(objset, db, userp, hdl_type, handlepp);
	return (error);
}
...
sa_find_idx_tab(objset_t *os, dmu_object_type_t bonustype, void *data)
{
	...
	ASSERT(tb);
	VERIFY(IS_SA_BONUSTYPE(bonustype) &&
    ...
    uint16_t buflen, dmu_tx_t *tx)
{
	...
	bonuslen = dmu_get_bonuslen(hdl->sa_bonus);
	if (bonuslen != 0) {
	...
	ASSERT((bonuslen > 0 && bonus_attr_count > 0) ||
	    (bonuslen == 0 && spill_attr_count > 0));
...
sa_remove(sa_handle_t *hdl, sa_attr_type_t attr, dmu_tx_t *tx)
{
	...
	mutex_enter(&hdl->sa_lock);
	error = sa_attr_op(hdl, &bulk, 1, SA_REMOVE, tx);
	mutex_exit(&hdl->sa_lock);
	return (error);
}

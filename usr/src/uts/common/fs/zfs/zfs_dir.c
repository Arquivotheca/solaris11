    ...
    int *deflg, pathname_t *rpnp)
{
	...
	if (name[0] == 0 || (name[0] == '.' && name[1] == 0)) {
		*vpp = ZTOV(dzp);
		...
		/*
		 * If we are a snapshot or sharedir mounted under .zfs, return
		...
		if (parent == dzp->z_id && zfsvfs->z_parent != zfsvfs) {
			error = zfsctl_root_lookup(zfsvfs->z_parent->z_ctldir,
			    ...
			    NULL, NULL, NULL);
			return (error);
		} else if (parent == dzp->z_zfsvfs->z_shares_dir) {
			*vpp = zfsctl_root(dzp);
			return (0);
		}
	...
	if (ZTOV(zp)->v_type == VDIR && (zp->z_pflags & ZFS_XATTR)) {
		...
		if (error) {
			...
			(void) sa_update(zp->z_sa_hdl, SA_ZPL_FLAGS(zfsvfs),
			    (void *)&zp->z_pflags, sizeof (zp->z_pflags), tx);
...
zfs_link_create(zfs_dirlock_t *dl, znode_t *zp, dmu_tx_t *tx, int flag)
{
	...
	SA_ADD_BULK_AMCTIME(bulk, count, zfsvfs, dzp);
	SA_ADD_BULK_ATTR(bulk, count, SA_ZPL_FLAGS(zfsvfs), NULL,
	    &dzp->z_pflags, sizeof (dzp->z_pflags));
	zfs_tstamp_update_setup(dzp, CONTENT_MODIFIED, B_TRUE);
	...
	boolean_t *unlinkedp)
{
	...
	if (!(flag & ZRENAMING)) {
		if (vn_vfsrlock(vp))		/* prevent new mounts on zp */
			return (EBUSY);
...
top:
	...
	SA_ADD_BULK_ATTR(bulk, count, SA_ZPL_XATTR(zfsvfs), NULL,
	    &xzp->z_id, sizeof (xzp->z_id));
	SA_ADD_BULK_AMCTIME(bulk, count, zfsvfs, zp);

	VERIFY(0 == sa_bulk_update(zp->z_sa_hdl, bulk, count, tx));

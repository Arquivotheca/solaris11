...
{
	ASSERT(zfs_has_ctldir(zp) || zp->z_id == zp->z_zfsvfs->z_shares_dir);
...
domount:
	...
	err = dsl_crypto_key_inherit(snapname);
	if (err != 0 && err != EEXIST) {
		mutex_exit(&sdp->sd_lock);
		ZFS_EXIT(zfsvfs);
		return (err);
...
retry:
	...
	if (sep != NULL) {
		...
		/*
		 * try and locate/mount the file system
		 *
		 * This should only be needed if the server has
		 * been rebooted.
		 */

		osname = kmem_zalloc(MAXNAMELEN, KM_SLEEP);
		snapname = kmem_zalloc(MAXNAMELEN, KM_SLEEP);
		fullname = kmem_zalloc(MAXNAMELEN, KM_SLEEP);

		dmu_objset_name(zfsvfs->z_os, osname);

		cookie = 0;
		while (dmu_snapshot_list_next(zfsvfs->z_os, 1024, snapname, &id,
		    &cookie, &conflict) == 0) {
			(void) snprintf(fullname, MAXNAMELEN, "%s@%s",
			    osname, snapname);
			if (dmu_objset_hold(fullname, FTAG, &snap) != 0) {
				dmu_objset_rele(snap, FTAG);
				break;
			}
			snapguid = dmu_objset_guid(snap);
			dmu_objset_rele(snap, FTAG);
			error = ENOENT;
			if (snapguid == objsetid) {
				vnode_t *rvp;
				vnode_t *svp = NULL;
				static enum symfollow follow = NO_FOLLOW;
				static enum uio_seg seg = UIO_SYSSPACE;

				(void) snprintf(fullname, MAXNAMELEN,
				    ".zfs/snapshot/%s", snapname);
				VERIFY(VFS_ROOT(zfsvfs->z_vfs, &rvp) == 0);
				error = lookupnameat(fullname, seg, follow,
				    NULL, &svp, rvp);
				VN_RELE(rvp);
				if (svp)
					VN_RELE(svp);

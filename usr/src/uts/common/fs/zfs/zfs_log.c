	...
	znode_t *zp, offset_t off, ssize_t resid, int ioflag)
{
	...
	while (resid) {
		 ...
		 */
		if (write_state != WR_INDIRECT && resid >
		    ZIL_MAX_LOG_DATA(zilog->zl_spa))
			len = spa_max_block_size(zilog->zl_spa) >> 1;
		else
			len = resid;
	...
	znode_t *zp, vattr_t *vap, uint_t mask_applied, zfs_fuid_info_t *fuidp)
{
	...
	if ((mask_applied & AT_UID) && IS_EPHEMERAL((uint32_t)vap->va_uid))
		lr->lr_uid = fuidp->z_fuid_owner;
	else
		lr->lr_uid = (uint64_t)vap->va_uid;

	if ((mask_applied & AT_GID) && IS_EPHEMERAL((uint32_t)vap->va_gid))
		lr->lr_gid = fuidp->z_fuid_group;
	else

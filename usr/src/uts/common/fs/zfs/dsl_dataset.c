...
static boolean_t dsl_dataset_sending(dsl_dataset_t *ds);
...
/*
 * Return a 'trim' txg for clone destruction. This is the birth txg
 * of the clone origin. Any block born before that snapshot is not
 * really part of the clone and so we don't have to remove those
 * blocks when destroying the clone.
 */
uint64_t
dsl_dataset_trim_txg(dsl_dataset_t *ds)
{
	if (ds == NULL || ds->ds_phys == NULL || !DS_IS_INCONSISTENT(ds))
		return (0);

	/* If we're being destroyed, we can't be a snapshot. */
	ASSERT(!dsl_dataset_is_snapshot(ds));

	/* If a snapshot's destruction is deferred, we can't trim */
	if (ds->ds_phys->ds_creation_txg < ds->ds_phys->ds_prev_snap_txg)
		return (0);

	/*
	 * If we're a clone in the process of being destroyed, we can use our
	 * snapshot origin txg to reduce the amount of work in dnode_sync()
	 */
	if (ds->ds_destroying &&
	    ds->ds_prev != ds->ds_dir->dd_pool->dp_origin_snap)
		return (ds->ds_phys->ds_prev_snap_txg);
	...
	if (ds == NULL) {
		...
		if (err == 0 && !dsl_dataset_is_snapshot(ds)) {
			...
			err = dsl_prop_get_ds(ds,
			    ...
			    &ds->ds_reserved, NULL, DSL_PROP_GET_EFFECTIVE);
			if (err == 0) {
				err = dsl_prop_get_ds(ds,
...
int
dsl_dataset_own_exclusive(const char *name, boolean_t inconsistentok,
    void *tag, dsl_dataset_t **dsp)
{
	int err = dsl_dataset_own(name, inconsistentok, tag, dsp);

	if (err)
		return (err);
	err = dsl_dataset_make_exclusive(*dsp, tag);
	if (err)
		dsl_dataset_disown(*dsp, tag);
	return (err);
}
...
/*
 * If this dataset is being sent, it could cause us to block on the
 * ds_rwlock for a long time.  Since we're requesting RW_WRITER, this
 * will also block all RW_READER attempts. To avoid this, check if
 * there's a send in progress, and if so, return EBUSY.  That check
 * is still racy since we're not actually synchronizing with the send
 * thread, so we might need to retry.  Other RW_READERs should only
 * have short-term holds and shouldn't cause us to stay in this loop
 * for long.
 */
clock_t zfs_make_excl_delay = 10;
...
dsl_dataset_make_exclusive(dsl_dataset_t *ds, void *owner)
{
	...
	if (!RW_WRITE_HELD(&ds->ds_rwlock)) {
		while (rw_tryenter(&ds->ds_rwlock, RW_WRITER) == 0) {
			if (dsl_dataset_sending(ds))
				return (EBUSY);
			delay(zfs_make_excl_delay);
		}
	}
	return (0);
}
...
dsl_dataset_create_sync(dsl_dir_t *pdd, const char *lastname,
    dsl_dataset_t *origin, dsl_crypto_ctx_t *dcc,
    uint64_t flags, cred_t *cr, dmu_tx_t *tx)
{
...
dsl_snapshot_destroy_one(const char *name, void *arg)
{
	...
	err = dsl_dataset_own_exclusive(dsname, B_TRUE, da->dstg, &ds);
	strfree(dsname);
	if (err == 0) {
...
#endif
		error = dsl_dataset_own_exclusive(name, B_TRUE, tag, &origin);
		kmem_free(name, namelen);
		if (error)
			return (error);
...
clock_t zfs_namespace_lock_delay = 10;
...
dsl_dataset_destroy(dsl_dataset_t *ds, void *tag, boolean_t defer)
{
	...
	if (dsl_dataset_is_snapshot(ds)) {
		...
		err = dsl_dataset_make_exclusive(ds, tag);
		if (err)
			goto out;
	 ...
	 *
	 * If the decryption key wasn't available the user/group
	 * used accounting won't go to zero because we could't
	 * determine the user/group IDs because they are stored in
	 * encrypted spill blocks.
	 */
	...
	do {
		...
		if (dsl_dir_is_clone(dd)) {
			/*
			 * If we're destroying a clone, and that results in
			 * also destroying its origin, we can have this
			 * situation:
			 *
			 * Thread 1: owns clone, trying to hold origin, stuck
			 * on spa_namespace_lock in spa_open_common()
			 *
			 * Thread 2: holds spa_namespace_lock, doing
			 * spa_vdev_remove_evacuate->..zil_vdev_offline()
			 * which deadlocks trying to hold the clone
			 *
			 * We work around this here by retrying if we fail to
			 * grab the spa_namespace_lock.
			 */
			while (!mutex_tryenter(&spa_namespace_lock))
				delay(zfs_namespace_lock_delay);
...
dsl_dataset_destroy_begin_check(void *arg1, void *arg2, dmu_tx_t *tx)
{
	 ...
	 * nearly destroyed.
	 */
	if (ds->ds_dir->dd_phys->dd_child_dir_zapobj != 0) {
		err = zap_count(mos,
...
dsl_dataset_destroy_sync(void *arg1, void *tag, dmu_tx_t *tx)
{
	...
	if (ds->ds_reserved != 0) {
		...
		uint64_t value = 0;
		uint64_t effective_value = 0; /* predict default value */
...
dsl_dataset_snapshot_reserve_space(dsl_dataset_t *ds, dmu_tx_t *tx)
{
	 ...
	 */
	if (asize > 0)
		dsl_dir_willuse_space(ds->ds_dir, asize, B_FALSE, tx);
...
dsl_dataset_sync(dsl_dataset_t *ds, zio_t *zio, dmu_tx_t *tx)
{
	...
	dmu_objset_sync(ds->ds_objset, zio, &ds->ds_phys->ds_bp, tx);
}
...
dsl_snapshot_rename_one(const char *name, void *arg)
{
	...
	/* As in dmu_objset_snapshot_one(), skip temporary datasets. */
	if (dsl_dataset_is_tmp(name))
		return (0);
...
dsl_dataset_promote_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	delta = pa->cloneusedsnap - dd->dd_used_breakdown[DD_USED_SNAP];
	ASSERT3S(delta, >=, 0);
	...
	delta = pa->originusedsnap - odd->dd_used_breakdown[DD_USED_SNAP];
	ASSERT3S(delta, <=, 0);
	ASSERT3U(pa->used, >=, -delta);
	...
	while (obj != first_obj) {
		...
		if (own) {
			err = dsl_dataset_own_obj(dp, obj,
			    0, snaplist_tag, &ds);
			if (err == 0) {
				err = dsl_dataset_make_exclusive(ds,
				    snaplist_tag);
				if (err)
					dsl_dataset_disown(ds, snaplist_tag);
			}
		} else {
...
dsl_dataset_clone_swap_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	/* set dd_*_bytes */
	{
		...
		ASSERT3U(csa->cds->ds_dir->dd_used_breakdown[DD_USED_SNAP],
		    ==, 0);
...
retry:
	...
	/*
	 * Release the temporary clone's ds_rwlock, reverting this thread's
	 * usage back to owner status.  This will allow other readers to
	 * make progress until we grab exclusive access as part of the
...
void
dsl_dataset_adjust_space(dsl_dataset_t *ds, uint64_t asize, uint64_t inflight,
    uint64_t *used, uint64_t *ref_rsrv)
{
	ASSERT3S(asize, >, 0);
...
dsl_dataset_set_quota_check(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if (!dmu_tx_is_syncing(tx)) {
		/* need to repeat the prediction in syncing context */
	...
	if (effective_value < ds->ds_phys->ds_used_bytes ||
	    effective_value < ds->ds_reserved)
		return (ENOSPC);
...
dsl_dataset_set_quota_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	dsl_dataset_t *ds = arg1;
	dsl_prop_setarg_t *psa = arg2;
	uint64_t effective_value = *(uint64_t *)psa->psa_effective_value;
...
int
dsl_dataset_set_quota(const char *dsname, zprop_source_t source,
    zprop_setflags_t flags, uint64_t quota)
{
	dsl_dataset_t *ds;
	dsl_prop_setarg_t psa;
	uint64_t effective_value = -1ULL;
	int err;

	dsl_prop_setarg_init_uint64(&psa, "refquota", source, &quota, flags);
	psa.psa_effective_value = &effective_value;
...
dsl_dataset_set_reservation(const char *dsname, zprop_source_t source,
    zprop_setflags_t flags, uint64_t reservation)
{
	dsl_dataset_t *ds;
	dsl_prop_setarg_t psa;
	uint64_t effective_value = -1ULL;
    ...
    minor_t minor)
{
	...
	/*
	 * We're stashing aside a pointer to this dsl_pool_t, so we need
	 * to reflect that with a reference on the spa_t.
	 */
	spa_open_ref(ca->dp->dp_spa, ca);
...
dsl_dataset_user_hold_check(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	char *htag = ha->htag;
	objset_t *mos = ds->ds_dir->dd_pool->dp_meta_objset;
	uint64_t zapobj = ds->ds_phys->ds_userrefs_obj;
	...
	if (ha->temphold && strlen(htag) + MAX_TAG_PREFIX_LEN >= MAXNAMELEN)
		return (E2BIG);
	...
	if (zapobj != 0) {
		...
		error = zap_lookup(mos, zapobj, htag, sizeof (tmp), 1, &tmp);
		if (error == 0)
			return (EEXIST);
...
dsl_dataset_user_hold_sync(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	if (zapobj == 0) {
		 ...
		 */
		dmu_buf_will_dirty(ds->ds_dbuf, tx);
		zapobj = zap_create(mos, DMU_OT_USERREFS, DMU_OT_NONE, 0, tx);
	}

	mutex_enter(&ds->ds_lock);
	ASSERT(zapobj == ds->ds_phys->ds_userrefs_obj ||
	    ds->ds_phys->ds_userrefs_obj == 0);
	ds->ds_phys->ds_userrefs_obj = zapobj;
	ds->ds_userrefs++;
	mutex_exit(&ds->ds_lock);
...
dsl_dataset_user_release_one(const char *dsname, void *arg)
{
	...
	if (might_destroy) {
		...
		if (!dsl_dataset_tryown(ds, B_TRUE, dtag)) {
			...
			own = B_TRUE;
			error = dsl_dataset_make_exclusive(ds, dtag);
			if (error) {
				dsl_dataset_disown(ds, dtag);
				return (error);
			}
		}
	}
...
dsl_destroy_inconsistent(const char *dsname, void *arg)
{
	...
	if (dsl_dataset_hold(dsname, FTAG, &ds) != 0)
		return (0);
	...
	if (dsl_dataset_tryown(ds, B_TRUE, FTAG)) {
		/*
		 * We need to check again now that we're the owner, since
		 * the dataset could've been inconsistent due to an in-progress
		 * receive.
		 */
		if (DS_IS_INCONSISTENT(ds))
...
void
dsl_dataset_start_send(dsl_dataset_t *ds)
{
	mutex_enter(&ds->ds_lock);
	ASSERT(ds->ds_send_cnt >= 0);
	ds->ds_send_cnt++;
...
void
dsl_dataset_end_send(dsl_dataset_t *ds)
{
	mutex_enter(&ds->ds_lock);
	ASSERT(ds->ds_send_cnt > 0);
	ds->ds_send_cnt--;
...
static boolean_t
dsl_dataset_sending(dsl_dataset_t *ds)
{
	boolean_t rv;
...
/*
 * We use '%' as the first character of temporary datasets, such as the
 * one used for rollback. We can't just check if there's a '%' anywhere
 * in dsname though; we want to make sure we're not mistakenly looking
 * at a '%' in an ancestor's name.
 */
boolean_t
dsl_dataset_is_tmp(const char *dsname)
{
	char *cp;

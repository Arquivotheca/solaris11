...
{
	spa_t *spa = vd->vdev_spa;
	uint64_t state, device_guid, txg, spare_pool;
	uint64_t pool_guid = 0ULL;
	uint64_t vdtxg = 0;
	nvlist_t *label;
	 ...
	 * read-only.  Instead we look to see if the pool is marked
...
vdev_label_init(vdev_t *vd, uint64_t crtxg, vdev_labeltype_t reason)
{
	...
	/*
	 * At this point we're about to write out new labels.  If we're
	 * doing a read-only import, let the caller know the labels
	 * cannot be written.
	 */

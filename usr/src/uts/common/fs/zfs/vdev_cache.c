 ...
 *
 * ** Note: based on our most recent performance measurements, vdev caching
 * ** no longer appears to help most workloads due to improvements in other
 * ** parts of the code -- in fact, it often hurts performance considerably.
 * ** It is disabled for now, with intent to remove it in the near future.
 *
...
int zfs_vdev_cache_size = 0;			/* by default, no caching */
...
vdev_cache_hit(vdev_cache_t *vc, vdev_cache_entry_t *ve, zio_t *zio)
{
	...
	if (zio->io_flags & ZIO_FLAG_MVECTOR)
		(void) io_mvector_copyin(ve->ve_data + cache_phase,
		    zio->io_mvector, 0, zio->io_size);
	else
...
vdev_cache_write(zio_t *zio)
{
	...
	while (ve != NULL && ve->ve_offset < max_offset) {
		...
		if (ve->ve_fill_io != NULL) {
			...
			if (zio->io_flags & ZIO_FLAG_MVECTOR)
				(void) io_mvector_copyout(zio->io_mvector,
				    ve->ve_data + start - ve->ve_offset,
				    start - io_start,
				    end - start);
			else

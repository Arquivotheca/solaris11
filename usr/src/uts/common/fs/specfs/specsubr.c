...
specfind(dev_t dev, vtype_t type)
{
	...
	while (st != NULL) {
		...
		/*
		 * Make sure it holds the right vnode while opened from
		 * the non global zone.
		 */
		if (ZONE_PATH_VISIBLE(nvp->v_path, curzone)) {
			VN_HOLD(nvp);
			mutex_exit(&stable_lock);
			return (nvp);
		}
 ...
 * All the non global zone device nodes will be inserted to the
 * last in the list to ensure the global zone device node to
 * appear in prior.
 */
static void
sinsert(struct snode *sp)
{
	...
	if (st == NULL || curzone == global_zone) {
		sp->s_next = st;
		stable[STABLEHASH(sp->s_dev)] = sp;
		return;
	}
	while (st->s_next != NULL)
		st = st->s_next;
	st->s_next = sp;
	sp->s_next = NULL;
}

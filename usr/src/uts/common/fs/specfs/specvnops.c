...
static int
spec_can_clone(struct vnode *vp, dev_t newdev)
{
	major_t maj, newmaj;
	...
	newmaj = getmajor(newdev);

	if (maj != newmaj && maj != clone_major) {
		cmn_err(CE_NOTE,
...
not_streams:
	...
	if (error == 0 && dev != newdev) {
		if (!spec_can_clone(vp, newdev)) {
			(void) dev_close(newdev, flag, type, cr);
			return (ENXIO);
		}
		spec_clone(vpp, newdev, vp->v_type, NULL);
		sp = VTOS(*vpp);
...
streams_open:
	...
	if ((error == 0) && (dev != newdev)) {
		...
		UNLOCK_CSP(csp);
		spec_clone(vpp, newdev, vp->v_type, stp);
		sp = VTOS(*vpp);
		csp = VTOS(sp->s_commonvp);
	} else if (error == 0) {
		vp->v_stream = stp;

...
static tsb_replace_rc_t sfmmu_replace_tsb(sfmmu_t *, struct tsb_info *,
    struct tsb_info **, uint_t, hatlock_t *, uint_t);
...
#ifdef sun4v
/* context domains locks for LDom migration */
static kcondvar_t sfmmu_ctxdoms_cv;
static kmutex_t sfmmu_ctxdoms_lk;
#endif
 ...
 */
#define	SPL_SHIFT	MAX(7U, NCPU_LOG2 + 1U)
...
/*
 * Array of mutexes protecting a page's mapping list and p_nrm field.  Lock
 * order is mml_table, hat_lock.
 */
...
#ifdef sun4v
	/*
	 * Initialize context domain locks that are used during LDom migration.
	 */
	cv_init(&sfmmu_ctxdoms_cv, NULL, CV_DEFAULT, NULL);
	mutex_init(&sfmmu_ctxdoms_lk, NULL, MUTEX_SPIN,
	    (void *)ipltospl(DISP_LEVEL));
#endif

	/*
	 * Initialize ism mapping list lock.
	 */
 ...
 * consistent after suspend/resume on system that can resume on a different
 ...
 * on hat_lock trying to wrap.  sfmmu_ctxdom_lock() must be called after all the
 * other necessary locks needed by suspend/resume are acquired. Also, it must be
 * called right before CPUs are paused, else it could deadlock acquiring locks
 * held by paused CPUs.
 *
 ...
 *
 * sfmmu_ctxdoms_unlock(void) releases all hat_locks.  Must be called before
 * CPUs are resumed and call kmem_alloc(), because kmem_alloc() descendants
 * take mml_table[].  This violates lock ordering and will cause deadlock if
 * another thread concurrently expands the kernel cage and invalidates a user
 * page -- kcage_invalidate_page calls hat_page_unload which takes mml_lock, and
 * then calls sfmmu_tlb_demap which takes hat_lock.  It is important for threads
 * to check return value of CPU_MMU_CTXP() after this call, because domains are
 * still not initialized and NULL is returned.
 *
...
sfmmu_ctxdoms_update(void)
{
	...
	mutex_enter(&sfmmu_ctxdoms_lk);
	cv_broadcast(&sfmmu_ctxdoms_cv);
	mutex_exit(&sfmmu_ctxdoms_lk);
}

/*
 * sfmmu_ctxdoms_wait puts a thread to sleep if it needs a context but the
 * context domain has not yet been re-created after a guest migration.  An
 * alternative is to return immediately with no context, retry, and re-trap
 * continuously until the domain is ready.  However, a thread with RT priority
 * in such a loop would starve the migration thread and prevent domains from
 * being created.
 */
struct mmu_ctx *
sfmmu_ctxdoms_wait(void)
{
	mmu_ctx_t *mmu_ctxp;
	klwp_id_t lwp;
	char lwp_save_state;
	...
	mutex_enter(&sfmmu_ctxdoms_lk);
	while ((mmu_ctxp = CPU_MMU_CTXP(CPU)) == NULL) {
		cv_wait(&sfmmu_ctxdoms_cv, &sfmmu_ctxdoms_lk);
	}
	mutex_exit(&sfmmu_ctxdoms_lk);
...
hat_free_end(struct hat *sfmmup)
{
	...
	SFMMU_CHECK_TTECNTS_ARE_ZERO(sfmmup);
	...
	uint_t attr, int flags)
{
	...
	while (len) {
		 ...
		 *  Note that 32M/256M/2G page sizes are not (yet)
		 *  supported.
		 */
...
sfmmu_mod_tsb(sfmmu_t *sfmmup, caddr_t vaddr, tte_t *ttep, int ttesz)
{
	...
	if (sfmmup == ksfmmup) { /* No support for 32/256M/2G ksfmmu pages */
		phys = ktsb_phys;
		if (ttesz >= TTE4M) {
			SFMMU_CHECK_KERNEL_TTESZ(ttesz);
	...
	caddr_t vaddr, page_t **pps, uint_t flags, uint_t rid)
{
	...
	if ((flags & SFMMU_NO_TSBLOAD) == 0) {
		 ...
		 * have a single, unique TSB entry. Ditto for 32M/256M/2G.
		 */
...
sfmmu_shadow_hcreate(sfmmu_t *sfmmup, caddr_t vaddr, int ttesz, uint_t flags)
{
	...
	ASSERT(sfmmup != KHATID);
	SFMMU_GET_LARGEST_TTESZ(max_ttesz);
	SFMMU_CHECK_UNUSED_TTECNTS(sfmmup);
	ASSERT(ttesz < max_ttesz);
	...
	int mode)
{
	...
	sfmmu_hblks_list_purge(&list, 0);
	DEMAP_RANGE_FLUSH(&dmr, DMR_SYNC);
}
	...
	caddr_t endaddr, demap_range_t *dmrp, uint_t attr, int mode)
{
	 ...
	 */
	DEMAP_RANGE_CONTINUE(dmrp, addr, endaddr, ttesz);
...
hat_get_mapped_size(struct hat *hat)
{
	...
	ism = (hat->sfmmu_iblk != NULL);

	for (i = 0; i < mmu_page_sizes; i++) {
		cnt = hat->sfmmu_ttecnt[i] + hat->sfmmu_scdrttecnt[i];
		if (ism) {
			cnt += hat->sfmmu_ismttecnt[i] +
			    hat->sfmmu_scdismttecnt[i];
		}
		assize += cnt << TTE_PAGE_SHIFT(i);
	}
...
/*
 * Safely copy an ism_map_t
 *
 * The trap handlers read ism_map[i] without holding any locks, and test
 * the imap_saddr field to see if the entry is valid. We must guarantee that
 * all fields are modified before imap_saddr is changed.
 */
static void
ism_map_copy(ism_map_t *src, ism_map_t *dst)
{
	dst->imap_saddr = 0;
	membar_stst(); /* make sure change to imap_saddr is globally visible */
	dst->imap_eaddr = src->imap_eaddr;
	dst->imap_vb_shift = src->imap_vb_shift;
	dst->imap_rid = src->imap_rid;
	dst->imap_hatflags = src->imap_hatflags;
	dst->imap_sz_mask = src->imap_sz_mask;
	dst->imap_ismhat = src->imap_ismhat;
	dst->imap_ment = src->imap_ment;
	/*
	 * make sure all members are updated before imap_saddr is
	 */
	membar_stst();
	dst->imap_saddr = src->imap_saddr;
}
	...
	struct hat *ism_hatid, caddr_t sptaddr, size_t len, uint_t ismszc)
{
	 ...
	 */
	if (!IS_P2ALIGNED(addr, ismpgsz) || !IS_P2ALIGNED(sptaddr, ismpgsz))
		return (EINVAL);

	/*
	 * Check the size alignment.
	 */
	if (!IS_P2ALIGNED(len, ism_min_pgsz))
		return (EINVAL);
	...
	} else if (ismszc == TTE2G) {
		ismhatflag = HAT_2G_FLAG;
	}
	...
	while (!added) {
		...
		for (i = 0; i < ISM_MAP_SLOTS; i++)  {
			if (ism_map[i].imap_ismhat == NULL) {
				...
				ism_map[i].imap_eaddr = (uintptr_t)eaddr;

				/*
				 * imap_saddr is checked in ISM_CHECK to see
				 * if non-NULL, then other info assumed valid.
				 */
				membar_stst();
				ism_map[i].imap_saddr = (uintptr_t)addr;
	 ...
	 *	Number of Page Sizes		Hashes Searched
	 *	    4			 4M and 8K/64K
	 *	    6			256M or 32M, and 4M and 8K/64K
	 *	    7			2G and 256M and 4M and 8K/64K
	 *
	...
	switch (ismszc) {
	case TTE2G:
		if (!SFMMU_FLAGS_ISSET(sfmmup, HAT_2G_ISM)) {
			hatlockp = sfmmu_hat_enter(sfmmup);
			SFMMU_FLAGS_SET(sfmmup, HAT_2G_ISM);
			sfmmu_hat_exit(hatlockp);
		}
		break;
	case TTE256M:
...
hat_unshare(struct hat *sfmmup, caddr_t addr, size_t len, uint_t ismszc)
{
	...
	ASSERT(IS_P2ALIGNED(len, ism_min_pgsz));
	ASSERT(sfmmup != NULL);
	ASSERT(sfmmup != ksfmmup);
	ASSERT(sfmmup->sfmmu_as != NULL);
	...
	if (found) {
		...
		while (ism_blkp != NULL) {
			...
			while (i < (ISM_MAP_SLOTS - 1)) {
				ism_map_copy(&ism_map[i + 1], &ism_map[i]);
				i++;
			}
			/* i == (ISM_MAP_SLOTS - 1) */
			ism_blkp = ism_blkp->iblk_next;
			if (ism_blkp != NULL) {
				ism_map_copy(&(ism_blkp->iblk_maps[0]),
				    &ism_map[i]);
				i = 0;
			} else {
				ism_map[i].imap_saddr = 0;
				membar_stst();
				ism_map[i].imap_eaddr = 0;
...
#ifdef sun4v
	/* It is possible that domain is not yet initialized after migration */
	if (mmu_ctxp == NULL)
		mmu_ctxp = sfmmu_ctxdoms_wait();
#endif
	ASSERT(mmu_ctxp);
 ...
 *
 * Upon success and when a new tsb_info is allocated, this function can pass
 * the address of the new tsb_info back to the caller, provided that the caller
 * is interested.
 *
 * Return values:
...
sfmmu_replace_tsb(sfmmu_t *sfmmup, struct tsb_info *old_tsbinfop,
    struct tsb_info **new_tsbinfopp, uint_t szc, hatlock_t *hatlockp,
    uint_t flags)
{
	struct tsb_info *newtsb = NULL;
	...
	for (prevtsb = NULL, curtsb = sfmmup->sfmmu_tsb;
	    curtsb != old_tsbinfop && curtsb != NULL;
	...
	tte_sz_mask = old_tsbinfop->tsb_ttesz_mask;
	 ...
	 */
	sfmmu_hat_exit(hatlockp);
	if (sfmmu_tsbinfo_alloc(&newtsb, szc, tte_sz_mask, flags, sfmmup) &&
	    (!(flags & (TSB_GROW | TSB_SWAPIN)) || (szc <= TSB_4M_SZCODE) ||
	    (!(flags & TSB_SWAPIN) &&
	    (old_tsbinfop->tsb_szc >= TSB_4M_SZCODE)) ||
	    sfmmu_tsbinfo_alloc(&newtsb, TSB_4M_SZCODE, tte_sz_mask, flags,
	    sfmmup))) {
	...
	newtsb->tsb_next = old_tsbinfop->tsb_next;
	membar_stst();	/* strict ordering required */
	if (prevtsb)
		prevtsb->tsb_next = newtsb;
	else
		sfmmup->sfmmu_tsb = newtsb;
	...
	if (tsb_remap_ttes && ((flags & TSB_GROW) == TSB_GROW))
		sfmmu_copy_tsb(old_tsbinfop, newtsb);
	...
	sfmmu_tsbinfo_free(old_tsbinfop);

	(void) sfmmu_hat_enter(sfmmup);

	if (new_tsbinfopp)
		*new_tsbinfopp = newtsb;
...
sfmmu_check_page_sizes(sfmmu_t *sfmmup, int growing)
{
	...
	SFMMU_GET_LARGEST_TTESZ(max_ttesz);
	if (max_ttesz > TTE4M) {
		for (i = TTE4M + 1; i <= max_ttesz; i++) {
			tte4m_cnt += (ttecnt[i] <<
			    (hw_page_array[i].hp_shift - MMU_PAGESHIFT4M));
		}
	}
	...
	uint64_t tte4m_cnt, int sectsb_thresh)
{
	...
	if (!growing && (tsb_szc < tsbinfop->tsb_szc) &&
		...
		(void) sfmmu_replace_tsb(sfmmup, tsbinfop, NULL, tsb_szc,
		    hatlockp, TSB_SHRINK);
	 ...
	 * after it's freed). Note: second tsb is required for 32M/256M/2G
	 * page sizes.
	 */
...
retry:
		if (tsbinfop->tsb_next == NULL) {
			...
			boolean_t swapped = SFMMU_FLAGS_ISSET(sfmmup,
			    HAT_SWAPPED);
			int allocflags = swapped ? 0 : TSB_ALLOC;
			...
			if (swapped != SFMMU_FLAGS_ISSET(sfmmup, HAT_SWAPPED)) {
				/*
				 * The HAT_SWAPPED bit changed while we were not
				 * holding the HAT lock, which means that the
				 * second tsb_info was allocated with wrong
				 * allocflags.
				 */
				sfmmu_hat_exit(hatlockp);
				sfmmu_tsbinfo_free(newtsb);
		...
		if (!growing && (tsb_szc < tsbinfop->tsb_szc) &&
		    (tsb_szc >= default_tsb_size) && TSB_OK_SHRINK()) {
			(void) sfmmu_replace_tsb(sfmmup, tsbinfop, NULL,
	...
	uint64_t hblkpa, struct hme_blk *pr_hblk)
{
	...
	if (hmeblkp->hblk_vcnt || hmeblkp->hblk_hmecnt) {
		...
		DEMAP_RANGE_FLUSH(&dmr, DMR_NOSYNC);
		if (hmeblkp->hblk_vcnt || hmeblkp->hblk_hmecnt) {
			/*
...
{
	struct tsb_info *tsbinfop, *new_tsbinfop, *next;
	tsb_replace_rc_t rc;
	 ...
	 *
	 * Note that for walking the tsbinfo's list we always use the next
	 * pointer read from the already replaced tsbinfo. This guarantees that
	 * we visit and replace all items on the list, including items silently
	 * added by e.g. sfmmu_size_tsb() when sfmmu_replace_tsb() does not
	 * hold the HAT lock.
	 */
	for (tsbinfop = sfmmup->sfmmu_tsb; tsbinfop != NULL;
	    tsbinfop = new_tsbinfop->tsb_next) {
		ASSERT(tsbinfop->tsb_flags & TSB_SWAPPED);
		rc = sfmmu_replace_tsb(sfmmup, tsbinfop, &new_tsbinfop,
		    tsbinfop->tsb_szc, hatlockp, TSB_SWAPIN);
		if (rc != TSB_SUCCESS)
			break;
		gotfirst = B_TRUE;
	}
...
retry:
	...
	if (!gotfirst) {
		tsbinfop = sfmmup->sfmmu_tsb;
		rc = sfmmu_replace_tsb(sfmmup, tsbinfop, NULL, TSB_MIN_SZCODE,
	...
	/*
	 * Recheck after reacquiring the mutex.
	 */
	if (sfmmup->sfmmu_tsb->tsb_next) {
		/* Someone raced with us and added additional tsbinfos. */
		goto retry;
	}
...
sfmmu_tlb_range_demap(demap_range_t *dmrp)
{
	...
	hatlock_t *hatlockp;
	cpuset_t cpuset;
	uint64_t addr_pgcnt_pshift;
	pgcnt_t pgcnt = 0;
	pgcnt_t pgunload = 0;
	pgcnt_t dirtypg = 0;
	int ttesz = dmrp->dmr_ttesz;
	size_t pagesize = TTEBYTES(ttesz);
	int pshift = page_get_shift(ttesz);
	...
	while (bitvec != 0) {
		...
		eaddr = addr + (dirtypg << pshift);
		hatlockp = sfmmu_hat_enter(sfmmup);
		sfmmu_unload_tsb_range(sfmmup, addr, eaddr, ttesz);
		sfmmu_hat_exit(hatlockp);
		pgunload += dirtypg;
		addr = eaddr;
		pgcnt += dirtypg;
	...
	ASSERT((pgcnt << pshift) <= dmrp->dmr_endaddr - dmrp->dmr_addr);

	addr = dmrp->dmr_addr;
	bitvec = dmrp->dmr_bitvec;

	/*
	 * Pass 4 values in 2 args by reusing low order zero bits:
	 *	arg0: addr:51, pgcnt:7, pshift:6
	 *	arg1: sfmmup:64
	 *
	 * Page size is encoded as log2 page size.
	 */
	ASSERT(pgcnt < (1 << SFMMU_PGCNT_SHIFT));
	ASSERT(pshift < (1 << SFMMU_PSHIFT_SHIFT));
	ASSERT(((uint64_t)addr & ((1 << (SFMMU_PGCNT_SHIFT +
	    SFMMU_PSHIFT_SHIFT)) - 1)) == 0);
	addr_pgcnt_pshift = (uint64_t)addr |
	    (pgcnt << SFMMU_PSHIFT_SHIFT) | pshift;
	...
	SFMMU_XCALL_STATS(sfmmup);
	xt_some(cpuset, vtag_flush_pgcnt_tl1, addr_pgcnt_pshift,
	    (uint64_t)sfmmup);
	...
	/*
	 * Setting dmr_xt_sync indicates that there has been some
	 * demap activity in the range. When DEMAP_RANGE_FLUSH is
	 * finally called an xt_sync will be performed. However, if
	 * dmr_xt_sync is never set then the final xt_sync is
	 * eliminated, thereby reducing cross call overhead.
	 */
	dmrp->dmr_xt_sync = 1;

	/*
	 * Clear dmr_bitvec in order to start the next sub-range of up
	 * to 64 pages.
	 */
	dmrp->dmr_bitvec = 0;
}
...
sfmmu_invalidate_ctx(sfmmu_t *sfmmup)
{
	...
	/*
	 * It is possible that domain is not yet initialized after migration,
	 * hence we are done since we do not have a valid context.
	 */
	if (mmu_ctxp == NULL) {
		kpreempt_enable();
	...
	if (old_bytes == new_bytes) {
		...
		for (i = 0; i < old_entries; i++, old++) {
			...
			if (!old->tte_tag.tag_invalid) {
				 ...
				 * Ditto for 32M, 256M and 2G TTEs.
				 */
				if (TTE_CSZ(&old->tte_data) != TTE8K &&
				    TTE_CSZ(&old->tte_data) != TTE4M) {
					continue;
				}
	...
	uint_t flags)
{
	...
	if (!IS_P2ALIGNED(r_saddr, TTEBYTES(r_pgszc)) ||
	    (((r_type == HAT_REGION_TEXT) &&
	    !IS_P2ALIGNED(r_size, TTEBYTES(r_pgszc))) ||
	    ((r_type == HAT_REGION_ISM) &&
	    !IS_P2ALIGNED(r_size, ism_min_pgsz)))) {

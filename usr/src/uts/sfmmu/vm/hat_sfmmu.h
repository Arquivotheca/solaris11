...
#define	ISM_PG_SIZE(ism_vbshift)	(1L << ism_vbshift)
 ...
 * will be constrained to a 4M, 32M, 256M or 2G alignment. Also since every
 * newly-created ISM segment is created out of a new address space at base
 * va of 0 we don't need to store it.
 */
...
/*
 * All segments mapped with ISM are guaranteed to be 4M, 32M, 256M or 2G
 * aligned.
 *
...
typedef struct ism_map {
	uintptr_t	imap_saddr;  	/* start addr of this ISM segment */
	uintptr_t	imap_eaddr;  	/* end addr of this ISM segment */
	uchar_t		imap_vb_shift;	/* mmu_pageshift for ism page size */
	uchar_t		imap_rid;	/* region id for ism */
	ushort_t	imap_hatflags;	/* hatflags */
...
#define	ism_start(map)	((caddr_t)((map).imap_saddr))
#define	ism_end(map)	((caddr_t)((map).imap_eaddr))
#define	ism_size(map)	(ism_end(map) - ism_start(map))
 ...
 */
#define	CPU_HME_PEND_THRESH 4000
...
#define	HAT_2G_ISM		0x08
#define	HAT_SWAPPED		0x10 /* swapped out */
#define	HAT_SWAPIN		0x20 /* swapping in */
#define	HAT_BUSY		0x40 /* replacing TSB(s) */
#define	HAT_ISMBUSY		0x80 /* adding/removing/traversing ISM maps */
...
#define	SFMMU_PGCNT_MASK	0x7f
#define	SFMMU_PGCNT_SHIFT	7
#define	SFMMU_PSHIFT_MASK	0x3f
#define	SFMMU_PSHIFT_SHIFT	6
 ...
 * in the dmr_bitvec field times the pagesize encoded in dmr_ttesz.
 *
 * It is not necessary to do flushes when the process is not runnable
 * because it's swapping out. Set dmr_sfmmup to NULL and the macros do
 * nothing in that case.  The macros also do nothing if dmr_sfmmup is
 * defined but the address space is being destroyed, indicated by
 * sfmmu_free=1.
 */
typedef struct {
	...
	int		dmr_ttesz;	/* page size code */
	int		dmr_xt_sync;	/* flag to trigger xt_sync */
} demap_range_t;
...
#define	DEMAP_RANGE_INIT(sfmmup, dmrp) \
	...
	(dmrp)->dmr_bitvec = 0; \
	(dmrp)->dmr_ttesz = TTE8K; \
	(dmrp)->dmr_xt_sync = 0; \
	}

/*
 * Extend the demap range from addr to endaddr, using pagesize ttesz.
 * Flush if not contiguous with existing range or the pagesize
 * changed.
 */
#define	DEMAP_RANGE_CONTINUE(dmrp, addr, endaddr, ttesz) \
	{ \
	if ((dmrp)->dmr_bitvec != 0 && ((dmrp)->dmr_endaddr != (addr) || \
	    (dmrp)->dmr_ttesz != (ttesz))) \
...
/*
 * Generate demap xcall for previously MARKed pages
 */
#define	DMR_NOSYNC	0	/* do not xt_sync() */
#define	DMR_SYNC	1	/* xt_sync() if necessary */

#define	DEMAP_RANGE_FLUSH(dmrp, sync) \
	{ \
	if ((dmrp)->dmr_bitvec != 0) \
		sfmmu_tlb_range_demap(dmrp); \
	_NOTE(CONSTCOND) \
	if ((sync) == DMR_SYNC && (dmrp)->dmr_xt_sync != 0) { \
		xt_sync((dmrp)->dmr_sfmmup->sfmmu_cpusran); \
		(dmrp)->dmr_xt_sync = 0; \
	} \
	}

/*
 * Mark that the page at addr must be demapped
 */
#define	DEMAP_RANGE_MARKPG(dmrp, addr) \
	if ((dmrp)->dmr_sfmmup != NULL && \
	    (dmrp)->dmr_sfmmup->sfmmu_free == 0) { \
		if ((dmrp)->dmr_bitvec == 0) { \
			(dmrp)->dmr_addr = (addr); \
			(dmrp)->dmr_bit = 1; \
		} \
...
/*
 * Advance to next page in range. Should be called after examining
 * the current page and either MARKing or skipping it.
 */
#define	DMR_MAXBIT ((ulong_t)1<<63)	/* max dmr_bitvec */

#define	DEMAP_RANGE_NEXTPG(dmrp) \
	if ((dmrp)->dmr_bitvec != 0) { \
		if ((dmrp)->dmr_bit & DMR_MAXBIT) { \
			sfmmu_tlb_range_demap(dmrp); \
		} else { \
			(dmrp)->dmr_bit <<= 1; \
		} \
...
/*
 * The maximum number of page sizes supported by hardware is SFMMU_INDEX_SHIFT
 */
#define	SFMMU_INDEX_SHIFT		7
...
/*
 * Functions exported to hat_sfmmu.c and mach_kpm.c
 */
...
struct sfmmu_global_stat {
	...
	int		sf_tteload2g;		/* calls to sfmmu_tteload */

...
#define	TSB_UPDATE_TL(tsbep, tte, tagtarget, ttepa, tmp1, tmp2, label, suspendlabel) \
	...
	/*								;\
	 * Check the TTE_SUSPEND flag					;\
	 */								;\
	set	TTE_SUSPEND, tmp1					;\
	TTE_SUSPEND_INT_SHIFT(tmp1)					;\
	btst	tte, tmp1						;\
	bz,pt	%xcc, label/**/_1		/* not suspended */	;\
	  sethi	%hi(TSBTAG_INVALID), tmp2				;\
	/*								;\
	 * Mapping is suspended,  so unlock the entry			;\
	 * and goto suspendlabel.					;\
	 */								;\
	add	tsbep, TSBE_TAG, tmp1					;\
...
#define	TSB_UPDATE_TL_PN(tsbep, tte, tagtarget, ttepa, tmp1, tmp2, label, suspendlabel) \
	TSB_LOCK_ENTRY(tsbep, tmp1, tmp2, label)			;\
	/*								;\
	 * I don't need to update the TSB then check for the valid tte.	;\
	 * TSB invalidate will spin till the entry is unlocked.	Note,	;\
	 ...
	 * Check the TTE_SUSPEND flag					;\
	 */								;\
	set	TTE_SUSPEND, tmp2					;\
	TTE_SUSPEND_INT_SHIFT(tmp2)					;\
	btst	tte, tmp2						;\
	bz,pt	%xcc, label/**/_1		/* not suspended */	;\
	  sethi	%hi(TSBTAG_INVALID), tmp2				;\
	/*								;\
...
#if (IMAP_SADDR != 0)
#error - ism_map->imap_saddr offset is not zero
#endif
 ...
 * tsbmiss	= address of tsb miss area (struct tsbmiss) (in)
 * ismbase	= base virtual address of this ISM segment (out)
 ...
 */
#define ISM_CHECK(tagacc, tsbmiss, ismbase, ismhat, tmp1, tmp2, tmp3, 	    \
	label, ismhit)							    \
	ldx	[tsbmiss + TSBMISS_ISMBLKPA], tmp1 /* tmp1 = &ism_blk */   ;\
	brlz,pt	tmp1, label/**/4		   /* exit if -1 */	   ;\
	  add	tmp1, IBLK_MAPS, ismhat /* ismhat=&ism_blk.iblk_maps[0] */ ;\
label/**/1:								   ;\
	ldxa	[ismhat]ASI_MEM, ismbase     /* ismbase = imap_saddr */	   ;\
	mov	tmp1, tmp3		     /* update ism_blk pa head */  ;\
label/**/2:								   ;\
	brz,pt	ismbase, label/**/4	     /* no mapping */		   ;\
	   srlx	tagacc, TAGACC_SHIFT, tmp1				   ;\
	sllx	tmp1, TAGACC_SHIFT, tmp1     /* tmp1 = vaddr */		   ;\
	cmp	tmp1, ismbase		     /* check vaddr < vbase */	   ;\
	blu,pt	%xcc, label/**/3	     /* va not in this ism slot */ ;\
	   add	ismhat, IMAP_EADDR, tmp2     /* tmp2 = &imap_eaddr */      ;\
	ldxa	[tmp2]ASI_MEM, tmp2	     /* tmp2 = vend */		   ;\
	cmp	tmp1, tmp2		     /* check vaddr < vend */	   ;\
	blu,a,pt	%xcc, ismhit	     /* ism hit */		   ;\
	    add	ismhat, IMAP_ISMHAT, ismhat  /* ismhat = &imap_ismhat */   ;\
									   ;\
label/**/3:								   ;\
	/* check next ISM_MAP_SLOT */					   ;\
	...
	bl,pt	%xcc, label/**/2	      /* keep looking */	   ;\
	    ldxa [ismhat]ASI_MEM, ismbase     /* ismbase = imap_saddr*/	   ;\
									   ;\
	add	tmp3, IBLK_NEXTPA, tmp1	      /* tmp1 = &iblk_nextpa */	   ;\
...
#define	DSHARED_CTX_V_SHIFT     (63-15)

#define	DTLB_DEMAP_ALL_SHCTX_ON(scr0, scr1, exit_label)			\
	GET_CPU_IMPL(scr0)						;\
	cmp	scr0, JUPITER_IMPL					;\
	bne,pn	%xcc, exit_label		/* not Jupiter, exit */ ;\
	  mov	MMU_SHARED_CONTEXT, scr1				;\
	ldxa	[scr1]ASI_MMU_CTX, scr0		/* scr0 = shctx reg. */ ;\
	sllx	scr0, DSHARED_CTX_V_SHIFT, scr1				;\
	brgez	scr1, exit_label		/* dshctx invalid ? */	;\
	  mov	DEMAP_ALL_TYPE, scr0					;\
	stxa	%g0, [scr0]ASI_DTLB_DEMAP	/* Demap all */		;\
	membar	#Sync							;\
exit_label:
...
#ifdef UTSB_PHYS
	/*
	 * If shared context is enabled, demap all TLB entries.
	 *
	 * g5, g6 = scratch registers
	 */
	DTLB_DEMAP_ALL_SHCTX_ON(%g5, %g6, uprot_shctx)
...
#endif
tsb_2G:
	/* Avoid the probe for 2G pages for kernel */
	sllx	%g2, TAGACC_CTX_LSHIFT, %g5
	brz,pn %g5, tsb_pagefault
	  ldub	[%g6 + TSBMISS_UTTEFLAGS], %g4
	and	%g4, HAT_2G_FLAG, %g5
	brz,pn	%g5, tsb_shme
	  nop
	 ...
	 */
	GET_TTE(%g2, %g7, %g3, %g4, %g6, %g1,
	    MMU_PAGESHIFT2G, TTE2G, %g5, tsb_l2G, tsb_checktte,
...
tsb_shme_2G:
	...
	GET_SHME_TTE(%g2, %g7, %g3, %g4, %g6, %g1,
	    MMU_PAGESHIFT2G, TTE2G, %g5, tsb_shme_l2G, tsb_shme_checktte,
...
1:
	...
	TSB_UPDATE_TL(%g1, %g3, %g2, %g4, %g7, %g6, locked_tsb_l3, sfmmu_suspend_tl)
...
4:
	...
	mov	%g7, %asi
        ...
        TSB_UPDATE_TL(%g1, %g3, %g2, %g4, %g7, %g6, locked_tsb_l4, sfmmu_suspend_tl)
...
tsb_user_pn_synth:
	...
	mov	%g7, %asi
	TSB_UPDATE_TL_PN(%g1, %g5, %g2, %g4, %g7, %g3, locked_tsb_l5, sfmmu_suspend_tl) /* update TSB */
5:
        DTLB_STUFF(%g5, %g1, %g2, %g3, %g4)
        retry
...
tsb_user_itlb_synth:
	...
	mov	ASI_N, %g7	/* user TSBs always accessed by VA */
	mov	%g7, %asi
	TSB_UPDATE_TL_PN(%g1, %g5, %g2, %g4, %g7, %g3, locked_tsb_l6, sfmmu_suspend_tl) /* update TSB */
7:
...
#endif
	TSB_UPDATE_TL(%g1, %g3, %g2, %g4, %g7, %g6, locked_tsb_l7, sfmmu_suspend_tl)
3:
#ifdef sun4v
	cmp	%g5, T_INSTR_MMU_MISS
	be,a,pn	%icc, 1f
...
tsb_ism:
	/*
	 * This is an ISM [i|d]tlb miss. The hat flag indicates the
	 * largest page size used in this ISM segment. One of the flags
	 * for 4M or larger must be set and that is the size we will
	 * start searching at. If we miss on the largest page size, then
	 * we will try the smaller page sizes from largest to smallest,
	 * except for for page sizes which are not supported or disabled
	 * for particular platforms.
	 *
	 * g2 = vaddr + ctx(or ctxtype (sun4v)) aka (pseudo-)tag access
	 *	register
	 * g3 = ism_map->imap_saddr (ism vbase)
	...
	brz,a,pn %g7, ptl1_panic		/* if zero jmp ahead */
	  mov	PTL1_BAD_ISM, %g1
        ...
        sub     %g4, (IMAP_ISMHAT - IMAP_HATFLAGS), %g5
        					/* g5 = pa of imap_hatflags */
        ...
        lduha   [%g5]ASI_MEM, %g4               /* g4 = imap_hatflags */
#if defined(sun4v) || defined(UTSB_PHYS)
...
tsb_ism_2G:
	and	%g4, HAT_2G_FLAG, %g5		/* check 2G */
	brz,pn  %g5, tsb_chkism_256M
	  nop

	/*
	 * 2G hash.
	 */
	GET_TTE(%g2, %g7, %g3, %g4, %g6, %g1, MMU_PAGESHIFT2G,
	    TTE2G, %g5, tsb_ism_l2G, tsb_ism_2G_found, sfmmu_suspend_tl,
	    tsb_ism_256M)

tsb_ism_2G_found:
	brlz,a,pt %g3, tsb_validtte
	  rdpr	%tt, %g7
	ba,pt	%xcc, tsb_ism_256M
	  nop

tsb_chkism_256M:
	and	%g4, HAT_256M_FLAG, %g5		/* check 256M */
...
kvtop_nohblk:
	...
	cmp	%g5, DEFAULT_MAX_HASHCNT       /* no 32/256M/2G kernel pages */
	be,a,pn	%icc, 6f
	  mov	-1, %o0				/* output = -1 (PFN_INVALID) */

 ...
 */
	...
	NO_UNLOAD_STUB(sockfs, so_allocb_premapped,  	nomod_einval);
 ...
 * Stubs for sharefs
 */
#ifndef SHAREFS_MODULE
	MODULE(sharefs,fs);
	WSTUB(sharefs,	sharefs_zone_lookup,	nomod_zero);
	WSTUB(sharefs,	sharefs_zone_rele,	nomod_zero);
	END_MODULE(sharefs);
#endif
 ...
 */
	...
	NO_UNLOAD_STUB(rpcmod, stp_queuereq,		nomod_minus_one);
 ...
 */
	...
	NO_UNLOAD_STUB(c2audit, audit_filepath,		nomod_zero);
	END_MODULE(c2audit);
#endif
 ...
 */
	...
	STUB(pcicfg, pcicfg_config_vf, 0);
	STUB(pcicfg, pcicfg_unconfig_vf, 0);
	END_MODULE(pcicfg);
#endif

/*
 * Stubs for PCIE misc module (misc/pcie).
 */
#ifndef	PCIE_MODULE
	MODULE(pcie,misc);
	STUB(pcie, pciv_send, 0);
	STUB(pcie, pci_param_get, 0);
	STUB(pcie, pciv_vf_config, 0);
	STUB(pcie, pci_param_free, 0);
	STUB(pcie, pci_plist_get, 0);
	STUB(pcie, pci_plist_getvf, 0);
	STUB(pcie, pci_param_get_ioctl, 0);
	STUB(pcie, pci_plist_lookup, 0);
	STUB(pcie, pci_plist_lookup_int8, 0);
	STUB(pcie, pci_plist_lookup_uint8, 0);
	STUB(pcie, pci_plist_lookup_int16, 0);
	STUB(pcie, pci_plist_lookup_uint16, 0);
	STUB(pcie, pci_plist_lookup_int32, 0);
	STUB(pcie, pci_plist_lookup_uint32, 0);
	STUB(pcie, pci_plist_lookup_int64, 0);
	STUB(pcie, pci_plist_lookup_uint64, 0);
	STUB(pcie, pci_plist_lookup_string, 0);
	STUB(pcie, pci_plist_lookup_plist, 0);
	STUB(pcie, pci_plist_lookup_byte_array, 0);
	STUB(pcie, pci_plist_lookup_int8_array, 0);
	STUB(pcie, pci_plist_lookup_uint8_array, 0);
	STUB(pcie, pci_plist_lookup_int16_array, 0);
	STUB(pcie, pci_plist_lookup_uint16_array, 0);
	STUB(pcie, pci_plist_lookup_int32_array, 0);
	STUB(pcie, pci_plist_lookup_uint32_array, 0);
	STUB(pcie, pci_plist_lookup_int64_array, 0);
	STUB(pcie, pci_plist_lookup_uint64_array, 0);
	STUB(pcie, pci_plist_lookup_string_array, 0);
	STUB(pcie, pciv_register_notify, 0);
	STUB(pcie, pciv_unregister_notify, 0);
	END_MODULE(pcie);
#endif
 ...
 */
	...
	NO_UNLOAD_STUB(kcf, kcf_fips140_integrity_check, nomod_minus_one);
	NO_UNLOAD_STUB(kcf, kcf_init, nomod_minus_one);
 ...
 */
	...
	STUB(dls, dls_mgmt_update_vlink, nomod_einval);
        END_MODULE(dls);
#endif
 ...
 */
	...
	NO_UNLOAD_STUB(acpica, AcpiGetDevices, nomod_minus_one) ;
...
#ifndef IOMMU_MODULE
	MODULE(iommu,misc);
	STUB(iommu, iommu_setup, nomod_void);
	STUB(iommu, iommu_teardown, nomod_void);
	END_MODULE(iommu);
#endif

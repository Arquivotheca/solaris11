...
#error "sendsig() amd64 did not find the expected stack alignments"
	...
	/*
	 * Since sizeof ucontext_t is multiple of 16-byte, and ucontext_t is
	 * 16-byte aligned, xregs will also be 16-byte aligned.
	 */
	/* LINTED: logical expression always true: op "||" */
	ASSERT((sizeof (*uc) % 16) == 0);
	xregs_size = xregs_getsize(ttoproc(curthread));
	...
	if ((lwp->lwp_pcb.pcb_fpu.fpu_flags & FPU_EN) &&
	    xregs_size != 0) {
		fp -= SA(xregs_size);
		xregs_setptr(lwp, tuc, fp);
		xregs = kmem_alloc(xregs_size, KM_SLEEP);
		xregs_get(lwp, xregs);
		copyout_noerr(xregs, fp, xregs_size);
...
sendsig32(int sig, k_siginfo_t *sip, void (*hdlr)())
{
	...
	if ((lwp->lwp_pcb.pcb_fpu.fpu_flags & FPU_EN) &&
	    xregs_size != 0) {
		fp -= SA32(xregs_size);
		xregs_setptr32(lwp, tuc, (caddr32_t)(uintptr_t)fp);
		xregs = kmem_alloc(xregs_size, KM_SLEEP);
		xregs_get(lwp, xregs);
		copyout_noerr(xregs, fp, xregs_size);
		kmem_free(xregs, xregs_size);

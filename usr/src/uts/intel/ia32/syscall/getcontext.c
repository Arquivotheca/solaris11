...
restorecontext(ucontext_t *ucp)
{
	...
	if (ucp->uc_flags & UC_FPU) {
		xregs_addr = xregs_getptr(lwp, ucp);
		if (xregs_getsize(ttoproc(t)) > 0 && xregs_addr &&
		    xregs_isvalid(xregs_addr)) {
			xregs_setfpregs(lwp, xregs_addr);
		} else {
...
getsetcontext(int flag, void *arg)
{
	...
	case SETCONTEXT:
		 ...
		 */
		if (copyin(ucp, &uc, offsetof(ucontext_t,
		    uc_mcontext.fpregs))) {
			return (set_errno(EFAULT));
		}
		if (uc.uc_flags & UC_SIGMASK)
		...
		if (uc.uc_flags & UC_XREGS) {
			if (copyin(&ucp->uc_xrs, &uc.uc_xrs,
			    sizeof (uc.uc_xrs))) {
				return (set_errno(EFAULT));
			}
		} else {
		 ...
		 */
		xregs_size = xregs_getsize(curproc);
		if (xregs_hasptr(lwp, &uc) && xregs_size > 0) {
			xregs = kmem_zalloc(xregs_size, KM_SLEEP);
			if (copyin(xregs_getptr(lwp, &uc), xregs,
			    xregs_size)) {
				kmem_free(xregs, xregs_size);
				return (set_errno(EFAULT));
			}
			xregs_setptr(lwp, &uc, xregs);
		} else {
			xregs_clrptr(lwp, &uc);
		}
...
getsetcontext32(int flag, void *arg)
{
	...
	case SETCONTEXT:
		...
		if (ucp == NULL)
			exit(CLD_EXITED, 0);
		if (copyin(ucp, &uc, offsetof(ucontext32_t,
		    uc_mcontext.fpregs))) {
			return (set_errno(EFAULT));
		}
		if (uc.uc_flags & UC_SIGMASK)
		 ...
		 */
		xregs_size = xregs_getsize(curproc);
		if (xregs_hasptr32(lwp, &uc) && xregs_size > 0) {
			xregs = kmem_zalloc(xregs_size, KM_SLEEP);

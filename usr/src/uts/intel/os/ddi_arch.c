	...
	off_t offset, off_t len, caddr_t *vaddrp)
{
...
#ifdef	DDI_MAP_DEBUG
	cmn_err(CE_CONT,
	    "i_ddi_bus_map: <%s,%s> <0x%x, 0x%"PRIx64", 0x%"PRIx64"> "
	...
	if (rp->regspec_bustype > 1 && rp_addr != 0) {
		cmn_err(CE_WARN, "<%s,%s>: invalid register spec"
		    ...
		    ddi_get_name(rdip), rp->regspec_bustype, rp_addr, rp_size);
		return (DDI_ME_INVAL);
	}
...
#ifdef	DDI_MAP_DEBUG
	cmn_err(CE_CONT,
	    "               <%s,%s> <0x%x, 0x%"PRIx64", 0x%"PRIx64"> "
	    "offset %d len %d\n",
	    ddi_get_name(dip), ddi_get_name(rdip),
	    rp->regspec_bustype, rp_addr, rp_size, offset, len);
#endif	/* DDI_MAP_DEBUG */
 ...
 * a given range spec.  (For readability: only used by i_ddi_apply_range.).
 */
static int
reg_is_enclosed_in_range(struct regspec *rp, struct rangespec *rangep)
{
	uint64_t reg_addr, range_caddr;

	reg_addr = REGSPEC_ADDR64(rp);
	range_caddr = RANGESPEC_CADDR64(rangep);

	if (reg_addr < range_caddr)
		return (0);

	if (RANGESPEC_SIZE64(rangep) == 0)
		return (1);	/* size is really 2**(bits_per_word) */

	if ((reg_addr + REGSPEC_SIZE64(rp) - 1) <=
	    (range_caddr + RANGESPEC_SIZE64(rangep) - 1))
		return (1);
...
i_ddi_apply_range(dev_info_t *dp, dev_info_t *rdip, struct regspec *rp)
{
	...
	uint64_t rp_addr;
	uint64_t range_addr, range_caddr;
	...
	range_addr = RANGESPEC_ADDR64(rangep);
	range_caddr = RANGESPEC_CADDR64(rangep);
#ifdef	DDI_MAP_DEBUG
	ddi_map_debug("    Input:  %x.%"PRIx64".%"PRIx64"\n",
	    rp->regspec_bustype, rp_addr, rp_size);
	ddi_map_debug("    Range:  %x.%x %x.%x %x.%x\n",
	    rangep->rng_caddr_high, rangep->rng_caddr_low,
	    rangep->rng_addr_high, rangep->rng_addr_low,
	    rangep->rng_size_high, rangep->rng_size_low);
#endif	/* DDI_MAP_DEBUG */

	/*
	 * addr = range_parent_addr + range_child_offset
	 *	= range_parent_addr + (addr - range_child_addr)
	 *	You can reorder this to
	 * addr = addr + (range_parent_addr - range_child_addr)
	 * 	and reduce it to
	 * addr += range_parent_addr - range_child_addr
	 */
	rp_addr += range_addr - range_caddr;
	SPLIT_REGSPEC_ADDR(rp_addr, rp);

#ifdef	DDI_MAP_DEBUG
	ddi_map_debug("    Return: %x.%x.%"PRIx64"\n", rp->regspec_addr_high,
	    rp->regspec_addr_low, rp_size);
#endif	/* DDI_MAP_DEBUG */

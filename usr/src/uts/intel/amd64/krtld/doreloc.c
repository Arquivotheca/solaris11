 ...
 */
#if !defined(DO_RELOC_LIBLD) || defined(__amd64)
...
#define	HIBITS_32	0xffffffff00000000ULL
#define	HIBITS_33	0xffffffff80000000ULL
...
#define	sym (* rel_desc_sname_func)(rdesc)
	...
	case 4:
		...
		if (rtype == R_AMD64_32) {
			 ...
			 * requires that the upper 32 bits all be 0.
			 */
			if ((*value & HIBITS_32) != 0) {
				/*
			 ...
			 * This is true of the upper 33 bits are all either
			 * 0 or all 1.
			 */
			if (((*value & HIBITS_33) != HIBITS_33) &&
			    ((*value & HIBITS_33) != 0)) {
				/*
				 * To keep chkmsg() happy:
				 *  MSG_INTL(MSG_REL_NOFIT)

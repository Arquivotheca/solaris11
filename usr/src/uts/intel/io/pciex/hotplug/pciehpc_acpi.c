...
/*ARGSUSED*/
static int
pciehpc_acpi_slot_poweron(pcie_hp_slot_t *slot_p,
    ddi_hp_cn_state_code_t *result)
{
...
/*ARGSUSED*/
static int
pciehpc_acpi_slot_poweroff(pcie_hp_slot_t *slot_p,
    ddi_hp_cn_state_code_t *result)
{
	pcie_hp_ctrl_t	*ctrl_p = slot_p->hs_ctrl;
	pcie_bus_t	*bus_p = PCIE_DIP2BUS(ctrl_p->hc_dip);
...
pciehpc_acpi_notify_handler(ACPI_HANDLE device, uint32_t val, void *context)
{
	...
	case 3: /* Request Eject */
	{
		...
		if (curr_state < DDI_HP_CN_STATE_POWERED) {
			...
			target_state.state_code = DDI_HP_CN_STATE_ENABLED;
			...
			target_state.state_code = DDI_HP_CN_STATE_EMPTY;
		}
		target_state.state_priv = NULL;

...
struct	buf *bp1, *bp2;
{
	...
	const int c_flags = B_REMAPPED | B_MVECTOR | B_MVEC_SHADOW;

	if ((bp1->b_un.b_addr != 0) || (bp2->b_un.b_addr != 0) ||
	    ((bp1->b_flags & (paw_flags | c_flags)) != paw_flags) ||
	    ((bp2->b_flags & (paw_flags | c_flags)) != paw_flags) ||

...
extern void pci_cfgacc_add_bad_bridge(uint16_t, uchar_t, uchar_t);
	...
	for (i = 0; i <= pci_bios_maxbus; i++) {
		...
		if (pci_bus_res[i].root_addr != root_addr) {
			...
			(void) ndi_prop_update_int_array(DDI_DEV_T_NONE,
			    pci_bus_res[i].dip, "reg", (int *)pci_regs,
...
pci_reprogram(void)
{
	...
	/*
	 * Reserve enough resources for IOV devices, if BIOS didn't
	 * do this.
	 */
	for (bus = 0; bus <= pci_bios_maxbus; bus++) {
		/* skip non-root (peer) PCI busses */
		...
		ASSERT(ddi_get_parent(pci_bus_res[bus].dip) == ddi_root_node());
		if (PCIE_DIP2BUS(pci_bus_res[bus].dip) != NULL)
			(void) pcie_boot_rebalance(pci_bus_res[bus].dip);
	}
}
...
populate_bus_res(uchar_t bus)
{
	...
	if (pci_bus_res[bus].bus_avail != NULL) {
		...
		/*
		 * In the fast reboot case, resource rebalance in the previous
		 * boot could have enlarged bus range of root complex node,
		 * compared to what was reported via ACPI. In this case we
		 * should use the current bus range settings in the system
		 * to update bus_avail.
		 */
		if (pci_bus_res[bus].sub_bus > max_bus)
			memlist_insert(&pci_bus_res[bus].bus_avail,
			    max_bus + 1, pci_bus_res[bus].sub_bus - max_bus);
	}
...
create_root_bus_dip(uchar_t bus)
{
	...
	(void) ndi_prop_update_int_array(DDI_DEV_T_NONE, dip,
	    "reg", (int *)pci_regs, sizeof (struct regspec) / sizeof (int));
...
/*
 * For any fixed configuration (often compatibility) pci devices
...
undo_amd8111_pci_fix(uint8_t bus, uint8_t dev, uint8_t fn)
{
	...
	/*
	 * AMD 8111 chipset is only used in PCI/PCI-X systems,
	 * on which boot rebalance will not be enabled thus
	 * there's no need to update bus number here.
	 */
    ...
    ushort_t vendorid, int config_op)
{
	...
	if (IS_BAD_AMD_NTBRIDGE(vendorid, deviceid) ||
		...
		pci_cfgacc_add_bad_bridge(bdf, secbus, subbus);
	}
...
#ifdef DEBUG
	if (pci_bus_res[secbus].dip != NULL) {
		cmn_err(CE_WARN,
		    " overwriting the existing dip %p with %p\n",
		    (void *)pci_bus_res[secbus].dip, (void *)dip);
	}
#endif
...
add_bus_slot_names_prop(int bus)
{
	...
	/*
	 * Otherwise overwrite slot-names we already have with the irq
	 * routing table values as returned by pci_slot_names_prop(),

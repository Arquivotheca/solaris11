...
static gms_mode_t gms_modes[] = {
	...
	{INTEL_BR_B43, I8XX_CONF_GC, I8XX_GC_MODE_MASK,
		GMS_SIZE(gms_G4X), gms_G4X},
	{INTEL_BR_SANDYBRIDGE, I8XX_CONF_GC, I8XX_GC_MODE_MASK,
		GMS_SIZE(gms_G4X), gms_G4X},
	{INTEL_BR_SANDYBRIDGE_M, I8XX_CONF_GC, I8XX_GC_MODE_MASK,
		GMS_SIZE(gms_G4X), gms_G4X},
	{INTEL_BR_SANDYBRIDGE_S, I8XX_CONF_GC, I8XX_GC_MODE_MASK,
		GMS_SIZE(gms_G4X), gms_G4X}
};
static int
    ...
    cred_t *cred, int *rval)
{
	...
	case AGP_TARGET_PCICONFIG_SET16:
	{
		...
		if (ddi_copyin((void *)data, &arg, sizeof (uint32_t), mode)) {
			mutex_exit(&st->tsoft_lock);
			return (EFAULT);
		}
		offset = (arg >> 16) & 0xffff;
		value = arg & 0xffff;

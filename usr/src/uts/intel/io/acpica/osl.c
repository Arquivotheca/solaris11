...
{
	ACPI_PHYSICAL_ADDRESS Address = NULL;
	uchar_t *dp = 0;
	unsigned dpsz;

	/*
	 * For EFI firmware, the root pointer is defined in the EFI systab.
	 * The loader passes the RSDP pointer either directly or indirectly
	 * and early kernel startup places it in the acpi-root-tab property.
	 */
...
AcpiOsGetThreadId(void)
{
	 ...
	 */
	return ((ACPI_THREAD_ID)((uintptr_t)curthread));
...
acpica_find_pcibus_walker(ACPI_HANDLE hdl, UINT32 lvl, void *ctxp, void **rvpp)
{
	...
	if (acpi_has_broken_bbn) {
		...
		(void) acpica_eval_int(hdl, "_SEG", &seg);

		if ((acpica_allow_multiseg || seg == 0) && bbn == busno) {
			*hdlp = hdl;
			return (AE_CTRL_TERMINATE);
		}
	} else if (busno == 0) {
...
/*
 * In order for there to be a problem, we must encounter multiple SEGMENT/BBN
 * tuples that are identical.
 */
static ACPI_STATUS
acpica_query_bbn_walker(ACPI_HANDLE hdl, UINT32 lvl, void *ctxp, void **rvpp)
{
	...
	if (ACPI_FAILURE(acpica_eval_hid(hdl, "_HID", &hid)) ||
		...
		/*
		 * XXX: This is not good enough -- need to remember
		 * all _SEG/_BBN tuples and test this tuple against
		 * that memorized set
		 */
		if (seg == 0 && bbn == 0 && ++(*cntp) > 1) {
			/*
	...
	/* Otherwise, skip this node (but visit siblings) */
	return (AE_OK);
}
...
int
acpica_get_initial_bdf(dev_info_t *dip, int *bus, int *device, int *func)
{
	pcie_bus_t *bus_p;
	int rval;

	rval = acpica_get_bdf(dip, bus, device, func);
	if (rval != 0)
		return (rval);

	if ((bus_p = PCIE_DIP2BUS(dip)) != NULL)
		*bus = PCIE_GET_INITIAL_BUS(bus_p);
	return (rval);
}

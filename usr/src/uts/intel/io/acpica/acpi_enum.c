...
static void
add_used_io_mem(isa_regspec_t *io, int io_count)
{
	int i;
	used_io_mem_t *used;
	...
	for (i = 0; i < io_count; i++) {
		used = (used_io_mem_t *)kmem_zalloc(sizeof (used_io_mem_t),
		    KM_SLEEP);
		used->start_addr = io[i].phys_lo;
		used->length = io[i].size;
		if (io[i].phys_hi == 1) {
...
static void
parse_resources_io(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	io[*io_count].phys_hi = 1; /* io */
	io[*io_count].size = acpi_io.AddressLength;
	io[*io_count].phys_lo = acpi_io.Minimum;
...
static void
parse_resources_fixed_io(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	io[*io_count].phys_hi = 1; /* io */
	io[*io_count].phys_lo = fixed_io.Address;
	io[*io_count].size = fixed_io.AddressLength;
	if (acpi_enum_debug & PARSE_RES_IO) {
		cmn_err(CE_NOTE, "parse_resources() "\
...
static void
parse_resources_fixed_mem32(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	io[*io_count].phys_hi = 0; /* memory */
	io[*io_count].phys_lo = fixed_mem32.Address;
	io[*io_count].size = fixed_mem32.AddressLength;
...
static void
parse_resources_mem32(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	if (resource_ptr->Data.Memory32.Minimum ==
		...
		io[*io_count].phys_hi = 0; /* memory */
		io[*io_count].phys_lo = mem32.Minimum;
		io[*io_count].size = mem32.AddressLength;
		(*io_count)++;
		if (acpi_enum_debug & PARSE_RES_MEMORY) {
			cmn_err(CE_NOTE, "parse_resources() "\
...
static void
parse_resources_addr16(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	if (addr16.AddressLength > 0) {
		if (addr16.ResourceType == ACPI_MEMORY_RANGE) {
			/* memory */
			io[*io_count].phys_hi = 0;
		} else {
			/* io */
			io[*io_count].phys_hi = 1;
		}
		io[*io_count].phys_lo = addr16.Minimum;
		io[*io_count].size = addr16.AddressLength;
		(*io_count)++;
	}
}

static void
parse_resources_addr32(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	if (addr32.AddressLength > 0) {
		...
		io[*io_count].phys_lo = addr32.Minimum;
		io[*io_count].size = addr32.AddressLength;
		(*io_count)++;
	}
}

static void
parse_resources_addr64(ACPI_RESOURCE *resource_ptr, isa_regspec_t *io,
    int *io_count)
{
	...
	if (addr64.AddressLength > 0) {
		...
		io[*io_count].phys_lo = addr64.Minimum;
		io[*io_count].size = addr64.AddressLength;
		(*io_count)++;
	}
}
...
parse_resources(ACPI_HANDLE handle, dev_info_t *xdip)
{
	...
	io = (isa_regspec_t *)kmem_zalloc(sizeof (isa_regspec_t) *
	...
	if (io_count) {
		...
		if (xdip != NULL) {
			(void) ndi_prop_update_int_array(DDI_DEV_T_NONE, xdip,
			    "reg", (int *)io,
			    io_count * sizeof (isa_regspec_t) / sizeof (int));
		}
	}
	...
	kmem_free(io, sizeof (isa_regspec_t) * MAX_PARSED_ACPI_RESOURCES);
	return (status);
}
...
get_bus_dip(char *nodename, dev_info_t *isa_dip)
{
	...
	(void) ndi_prop_update_int_array(DDI_DEV_T_NONE, i8042_dip,
	    "reg", (int *)i8042_regs,
	    2 * sizeof (isa_regspec_t) / sizeof (int));
	(void) ndi_prop_update_int_array(DDI_DEV_T_NONE, i8042_dip,
    ...
    void **b)
{
	...
	if (m != NULL) {
		/*
		 * Special processing for mouse and keyboard devices per IEEE
		 * 1275.
		 * If the master entry doesn't contain "compatible" then we add
		 * a default entry
		 */
		if (strcmp(m->name, keyboard_alias) == 0) {
			(void) ndi_prop_update_int(DDI_DEV_T_NONE, xdip, "reg",
			    0);
			(void) ndi_prop_update_string(DDI_DEV_T_NONE, xdip,
			    "device-type", keyboard_alias);
			if (!compatible_present)
				(void) ndi_prop_update_string(DDI_DEV_T_NONE,
				    xdip, "compatible", "pnpPNP,303");

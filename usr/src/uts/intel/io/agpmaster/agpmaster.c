...
detect_i8xx_device(agp_master_softc_t *master_softc)
{
	...
	case INTEL_IGD_B43:
	case INTEL_IGD_SANDYBRIDGE_GT1:
	case INTEL_IGD_SANDYBRIDGE_GT2:
	case INTEL_IGD_SANDYBRIDGE_GT2_PLUS:
	case INTEL_IGD_SANDYBRIDGE_M_GT1:
	case INTEL_IGD_SANDYBRIDGE_M_GT2:
	case INTEL_IGD_SANDYBRIDGE_M_GT2_PLUS:
	case INTEL_IGD_SANDYBRIDGE_S:
...
i8xx_add_to_gtt(gtt_impl_t *gtt, igd_gtt_seg_t seg)
{
	...
	for (i = seg.igs_pgstart; i < (seg.igs_pgstart + seg.igs_npage);
		...
		if (phys2entry(seg.igs_type, *paddr, &entry)) {
			AGPM_DEBUG((CE_WARN, "phys2entry error"));
			return (-1);
		}

...
dtrace_getarg(int arg, int aframes)
{
	...
	for (i = 1; i <= aframes; i++) {
		...
		if (fp->fr_savpc == (pc_t)dtrace_invop_callsite) {
			/*
			 * Use the pointer to the regs structure that was
			 * pushed when we took the trap. To get this structure,
			 * we must increment beyond the frame structure, and
			 * then again beyond the calling RIP stored in
			 * dtrace_invop().  If the argument that we're seeking
			 * is passed on the stack, we'll pull the true stack

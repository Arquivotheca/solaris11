...
sdt_invop(uintptr_t addr, uintptr_t *stack, uintptr_t eax)
{
	...
	/*
	 * stack[0] contains the dereferenced stack pointer, stack[1] contains
	 * savfp, stack[2] contains savpc.  We want to step over these entries.
	 */
	i += 3;
...
sdt_getarg(void *arg, dtrace_id_t id, void *parg, int argno, int aframes)
{
	...
	for (i = 1; i <= aframes; i++) {
		...
		if (fp->fr_savpc == (pc_t)dtrace_invop_callsite) {
			/*
			 * We will use the pointer to the regs structure that
			 * was pushed when we took the trap.  To get this
			 * structure, we must increment beyond the frame
			 * structure.  If the argument that we're seeking is
			 * passed on the stack, we'll pull the true stack
			 * pointer out of the saved registers and decrement our
			 * argument by the number of arguments passed in
			 * registers; if the argument we're seeking is passed
			 * in regsiters, we can just load it directly.
			 */

	...
	for (; fbt != NULL; fbt = fbt->fbtp_hashnext) {
		if ((uintptr_t)fbt->fbtp_patchpoint == addr) {
			if (fbt->fbtp_roffset == 0) {
				...
				/*
				 * stack[0] contains the dereferenced
				...
				/*
				 * We instrument the ret, not the leave.  We
				 * therefore need to set the caller to assure
				 * that the top frame of a stack() action is
				 * correct.
				 */
				DTRACE_CPUFLAG_SET(CPU_DTRACE_NOFAULT);
...
again:
		...
		/*
		 * We only instrument "ret" -- we don't yet instrument
		 ...
		 * jump table, especially given that our marker is just one
		 * byte.  To determine if we're looking at a true instruction
		 * sequence or an inline jump table that happens to contain the
		 * same byte sequences, we resort to some heuristic sleeze:  we

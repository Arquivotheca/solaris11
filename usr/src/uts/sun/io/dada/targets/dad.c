...
static void dcd_translate(struct dadkio_status *statp, struct udcd_cmd *cmdp);
...
dcd_doattach(dev_info_t *devi, int (*canwait)())
{
	...
	un->un_flush_not_supported =
	    (devp->dcd_ident->dcd_cmdset83 & FLUSH_CACHE_SUPPORTED)? 0:1;
 ...
 */
	...
	case DIOCTL_RWCMD:
		...
		if (cmlb_rwcmd_copyin(&rwcmd, (caddr_t)arg, flag))
			return (EFAULT);
		 ...
		 */
		if (rwcmd.cmd == DADKIO_RWCMD_READ) {
			if ((un->un_dp->options & DMA_SUPPORTTED) ==
			    DMA_SUPPORTTED)
		...
		} else if (rwcmd.cmd == DADKIO_RWCMD_WRITE) {
			if ((un->un_dp->options & DMA_SUPPORTTED) ==
			    DMA_SUPPORTTED)
		...
		scmd->udcd_cmd->size = rwcmd.buflen;
		scmd->udcd_cmd->sector_num.lba_num = rwcmd.blkaddr;
		scmd->udcd_bufaddr = (caddr_t)(uintptr_t)rwcmd.bufaddr;
		scmd->udcd_buflen = rwcmd.buflen;
		 ...
		 * dadkio_status
		 */
		(void) dcd_translate(&(rwcmd.status), scmd);
		rwcmd.status.resid = scmd->udcd_resid;
		mutex_exit(DCD_MUTEX);

		if (cmlb_rwcmd_copyout(&rwcmd, (caddr_t)arg, flag)) {
			if (i != 0)
				i = EFAULT;
		}
...
void
dcd_translate(struct dadkio_status *statp, struct udcd_cmd *cmdp)
{

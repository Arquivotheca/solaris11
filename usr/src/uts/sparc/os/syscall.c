...
pre_syscall(int arg0)
{
	 ...
	 * and auditing is enabled for this process
	 */
	if (audit_getpflags()) {
		uint32_t auditing = au_zone_getstate(NULL);

		if ((auditing & AU_AUDIT_MASK) &&
		    (audit_start(T_SYSCALL, code, auditing, lwp) == 0)) {
			/* kernel audit events still in place */
			repost = 1;
		}
	}
...
post_syscall(long rval1, long rval2)
{
	...
	if (AU_AUDITING()) {
		...
		if (audit_finish(T_SYSCALL, code, error, &rval) == 0) {
			/* kernel audit events still in place */
			repost = 1;
		}
	}
...
set_all_zone_usr_proc_sys(zoneid_t zoneid)
{
	...
	for (p = practive; p != NULL; p = p->p_next) {
		...
		if (zoneid == ALL_ZONES || p->p_zone->zone_id == zoneid) {
			...
			ai = crgetauinfo(p->p_cred);
			if ((ai == NULL) ||
			    (((ai->ai_mask.am_success |
			    ai->ai_mask.am_failure) &
			    au_zone_getkclasses(p)) == 0)) {
				mutex_exit(&p->p_lock);
				continue;
			}
			audit_setpflags(p);	/* enable audit_start */
			/*

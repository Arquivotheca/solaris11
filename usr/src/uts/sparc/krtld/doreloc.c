...
/* R_SPARC_WDISP10 */	{0x0, FLG_RE_PCREL | FLG_RE_WDISP10 |
				FLG_RE_VERIFY | FLG_RE_SIGN, 4, 2, 10},
};
#if R_SPARC_NUM != (R_SPARC_WDISP10 + 1)
#error "R_SPARC_NUM has changed"
#endif
...
#define	sym (* rel_desc_sname_func)(rdesc)
	...
	/*
	 * Sparc has different relocation types for aligned and unaligned data,
	 * suported here by dual fetch/store code paths. We have seen cases
	 * where the Studio and GNU assemblers issue aligned relocations for
	 * unaligned data. It is best for such bugs to be fixed, but rather
	 * than punish the end user for something that is out of their control,
	 * we use the unaligned code path for this case.
	 */
	if ((re_flags & FLG_RE_UNALIGN) == 0) {
		if (((field_size == 2) && ((uintptr_t)off & 0x1)) ||
			...
			re_flags |= FLG_RE_UNALIGN;
			REL_ERR_NONALIGN(lml, file, sym, rtype,
			    (uintptr_t)off, field_size);
		}
	}
	...
	if (sigbit_mask) {
		if (re_flags & FLG_RE_WDISP16) {
			/*
			 * The WDISP16 relocation is an unusual one in that
			 * its bits are not all contiguous.  We have to
			 * selectivly pull them out.
			 */
			uvalue = ((basevalue & 0x300000) >> 6) |
			    (basevalue & 0x3fff);
			basevalue &= ~0x303fff;
		} else if (re_flags & FLG_RE_WDISP10) {
			/* BEGIN CSTYLED */
			/*
			 * WDISP10 also has non-contiguous bits, plus the
			 * bottom of the displacement value is not at the
			 * bottom of the word.
			 * ---------------------------------------
			 * |              | d2|  |  disp8 |       |
			 * |              |   |  |        |       |
	...
	if (sigbit_mask) {
		/*
		 * Again the R_SPARC_WDISP16 and R_SPARC_WDISP10 relocations
		 * take special processing because of their non-continguous
		 * bits.
		 */
		if (re_flags & FLG_RE_WDISP16) {

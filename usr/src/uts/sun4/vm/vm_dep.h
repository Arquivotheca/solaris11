...
extern page_freelist_type_t *ucltp;
 ...
 * is only a single user freelist and cachelist as the kernel cage provides
 * a similar function to kernel freelist in that it prevents memory
 * fragmentation.
 *
...
struct page_freelist_type {
	...
	page_t ***pflt_cachelists[MAX_MEM_TYPES];
};
...
#define	PAGE_CACHELISTP(is_kflt, mnode, color, mtype) 			\
	(ucltp->pflt_cachelists[(mtype)][(mnode)] + (color))

...
static void page_flt_init(page_freelist_type_t *, page_freelist_type_t *);
...
alloc_page_freelists(caddr_t alloc_base)
{
	...
	for (mtype = 0; mtype < MAX_MEM_TYPES; mtype++) {
		ucltp->pflt_cachelists[mtype] = (page_t ***)alloc_base;
		alloc_base += (max_mem_nodes * sizeof (page_t **));
		for (mnode = 0; mnode < max_mem_nodes; mnode++) {
			ucltp->pflt_cachelists[mtype][mnode] =
			    (page_t **)alloc_base;
			alloc_base +=
 ...
 */
static void
page_flt_init(page_freelist_type_t *ufp, page_freelist_type_t *ucp)
{

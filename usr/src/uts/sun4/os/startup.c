...
extern void load_iov_modules(void);
#pragma weak load_iov_modules
...
#pragma weak mblock_boundary_check
...
extern int kcf_fips140_integrity_check();
...
int uhrt_enable = 0;	/* not supported */
...
calc_pagehash_sz(pgcnt_t npages)
{
	...
	page_hashsz_shift = MAX((AN_VPSHIFT + VN_SHIFT + 1),
	    highbit(page_hashsz));
...
alloc_kmem64(caddr_t base, caddr_t end)
{
	...
	for (i = max_bootlp_tteszc; i >= TTE8K; i--) {
		...
		if (prom_allocate_phys(alloc_size, alignsize, &pa) == 0) {
			/*
			 * make sure the physically contiguous chunk of memory
			 * is not crossing an mblock boundary.
			 */
			if (&mblock_boundary_check) {
				ret_val = mblock_boundary_check(pa, alignsize);
				if (ret_val) {
...
startup_modules(void)
{
	...
	/*
	 * Load vpci and its parent drivers which might
	...
	/*
	 * Need KCF running before mounting root since ZFS needs KCF
	 * as may other methods of getting to the root filesystem.
	 * Integrity check needs to be done at startup, as it opens
	 * a file (Access to the filesystem is disabled after startup)
	 */
	if (kcf_fips140_integrity_check() != 0) {
		cmn_err(CE_NOTE, "fips140_integrity_check failed.");
	}
...
kphysm_erase(uint64_t addr, uint64_t len)
{
	...
	while (num != 0) {
		...
		while (num != left) {
			...
			page_hashin(pp, &promvp, ptob((offset_t)base), NULL);

...
#ifdef	sun4v
	/*
	 * setcpudelay() must be called early on sun4v to support
	 * enabling stick normalization at start-of-day.
	 */
	setcpudelay();
#endif /* sun4v */
...
/*
 * Check all sparc implementations for EOL.
 */
static int
check_eol()
{
...
kobj_start(void *cif)
{
	...
	/*
	 * Check for hardware EOL. Do this before attempting to load
	 * CPU modules which may no longer exist.
	 * Allow the definition of a boot property to override EOL, for now.
	 */
	if (check_eol()) {
		if (prom_getproplen(prom_rootnode(),
		    "boot-s11-unsupported") == -1) {
			prom_printf("Error: '%s' is not supported by this "
			    "release of Solaris.\n",
			    (const char *)bootaux[BA_CPU].ba_ptr);
			prom_exit_to_mon();
		}
		prom_printf("Warning: '%s' support will be removed in a "

...
pxtool_intr_info(dev_info_t *dip, void *arg, int mode)
{
	...
	if (intr_info.flags & PCITOOL_INTR_FLAG_GET_MSI)
		intr_info.num_intr = msi_state_p->msi_total_cnt;
	else
...
static boolean_t
pxtool_is_valid_msinum(px_msi_state_t *msi_state_p, uint_t msinum)
{
	int i;

	for (i = 0; i < msi_state_p->msi_num_ranges; ++i) {
		px_msi_ranges_t *range = &(msi_state_p->msi_ranges[i]);
		if (msinum >= range->msi_1st_msinum &&
		    msinum < (range->msi_1st_msinum + range->msi_cnt))
			return (B_TRUE);
	}
	return (B_FALSE);
}
...
pxtool_get_intr(dev_info_t *dip, void *arg, int mode)
{
	...
	if (iget->flags & PCITOOL_INTR_FLAG_GET_MSI) {
		...
		if (!pxtool_is_valid_msinum(msi_state_p, iget->msi)) {
		...
		if (pxtool_ib_get_msi_info(px_p, iget->ino, iget->msi,
		    &hdle) != DDI_SUCCESS) {
			iget->status = PCITOOL_INVALID_MSI;
			rval = EINVAL;
			goto done_get_intr;
	...
	if (intr_valid_state == INTR_VALID) {
		 ...
		 */
		if ((iget->flags & PCITOOL_INTR_FLAG_BALANCER) != 0 &&
		    (mode & FKIOCTL) != 0) {
			/*
			 * Several hypervisor releases were buggy and
			 * return incorrect info for interrupt target.
			 * Just return kernel software recorded target.
			 * rather than querying hypervisor/hardware.
			 */
			mutex_enter(&ib_p->ib_ino_lst_mutex);
			ino_p = px_ib_locate_ino(ib_p, iget->ino);
			if (ino_p != NULL) {
				old_cpu_id = ino_p->ino_cpuid;
			} else {
		...
		bzero(&hdle, sizeof (ddi_intr_handle_impl_t));
		if (pxtool_ib_get_ino_info(px_p, iget->ino, &hdle)
		    != DDI_SUCCESS) {
			goto done_get_intr;
		}

		iget->sysino = sysino;
		iget->share = iget->num_devs;
	}
...
pxtool_set_intr(dev_info_t *dip, void *arg, int mode)
{
	...
	if (iset.flags & PCITOOL_INTR_FLAG_SET_MSI) {
		...
		if (!pxtool_is_valid_msinum(msi_state_p, iset.msi)) {
...
static int
pxtool_set_cto(dev_info_t *dip, pcitool_reg_t *prg_p)
{
	dev_info_t	*cdip;
	int		rval, size;
	...
	local_data = big_endian ? pxtool_swap_endian(prg_p->data, size) :
	    prg_p->data;
...
static int
pxtool_retrain_link(dev_info_t *dip, pcitool_reg_t *prg_p,
...
pxtool_dev_reg_ops(dev_info_t *dip, void *arg, int cmd, int mode)
{
	...
	if (cmd == PCITOOL_DEVICE_LINK_RETRAIN) {
		rval = pxtool_retrain_link(dip, &prg, B_FALSE);
		goto done_reg;
	} else if (cmd == PCITOOL_DEVICE_LINK_EQUALIZATION) {
		rval = pxtool_retrain_link(dip, &prg, B_TRUE);
		goto done_reg;
	} else if (cmd == PCITOOL_DEVICE_SET_CTO) {
		rval = pxtool_set_cto(dip, &prg);
		goto done_reg;
	}

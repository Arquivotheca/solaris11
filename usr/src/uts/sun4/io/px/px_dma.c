	...
	caddr_t arg)
{
	...
	if (dip != rdip) {
		...
		/*
		 * The rdip may equal to dip as the rdip may be the root
		 * complex itself. This happens when a removable device
		 * plugged directly under root complex is enabled by
		 * hotplug command, it's fcode may try to setup DMA mapping,
		 * and in this case the dip == rdip happens.
		 */
		mp->dmai_bdf = PCIE_INVALID_BDF;
	}
	...
	ddi_dma_req_t *dmareq)
{
	...
	if (PX_DEV_NOSYSLIMIT(lo, hi, syslo, syshi, 1))
...
px_dma_attach(px_t *px_p)
{
	...
	/*
	 * Need to pass a valid address to reliablely determine bypass DMA
	 * support.  For convenience, here px_p is used.
	 */
	if (px_lib_iommu_getbypass(px_p->px_dip, va_to_pa(px_p),
...
px_dma_attr2hdl(px_t *px_p, ddi_dma_impl_t *mp)
{
	...
	/*
	 * mpt driver has dma_attr_seg = 0xffffefff, and when attaching
	 * we shrink last 1M from DVMA space, so syshi is smaller than
	 * dma_attr_seg.
	 */
	if (nocross > syshi)
		nocross = (uint64_t)-1;
...
px_dvma_unmap(px_mmu_t *mmu_p, ddi_dma_impl_t *mp)
{
	...
	switch (mp->dmai_flags & PX_DMAI_FLAGS_VMEM) {
	case PX_DMAI_FLAGS_KMEM1:
		kmem_cache_free(mmu_p->mmu_dvma_cache_1, (void *)dvma_addr);
		break;
	case PX_DMAI_FLAGS_KMEM2:
		kmem_cache_free(mmu_p->mmu_dvma_cache_2, (void *)dvma_addr);
		break;
	case PX_DMAI_FLAGS_KMEM8:
		kmem_cache_free(mmu_p->mmu_dvma_cache_8, (void *)dvma_addr);
		break;
	case PX_DMAI_FLAGS_VMEM16:
		npages = PX_DMA_WINNPGS(mp) + PX_HAS_REDZONE(mp);
		vmem_free(mmu_p->mmu_dvma_vmem_16, (void *)dvma_addr,
		    P2ROUNDUP(MMU_PTOB(npages), px_dvma_cache_quantum));
		break;
	case PX_DMAI_FLAGS_VMEM1M:
		npages = PX_DMA_WINNPGS(mp) + PX_HAS_REDZONE(mp);
		vmem_xfree(mmu_p->mmu_dvma_map, (void *)dvma_addr,
		    P2ROUNDUP(MMU_PTOB(npages), px_dvma_top_cache_quantum));
		break;
	case PX_DMAI_FLAGS_FAILSAFE:
		npages = PX_DMA_WINNPGS(mp) + PX_HAS_REDZONE(mp);
		vmem_xfree(mmu_p->mmu_dvma_mapc, (void *)dvma_addr,
		    MMU_PTOB(npages));
		break;
	default:
		panic("px_dvma_unmap: unknown DMA vmem flag");
	}
}
...
{
	uint_t npages = PX_DMA_WINNPGS(mp), pages_to_allocate;
	...
	pages_to_allocate = npages + PX_HAS_REDZONE(mp);
	n = MMU_PTOB(pages_to_allocate);
	nocross = mp->dmai_attr.dma_attr_seg + 1;

	ASSERT(nocross == 0 || n <= nocross);
	ASSERT(ISP2(px_dvma_cache_quantum));
	ASSERT(ISP2(px_dvma_top_cache_quantum));

	if (PX_HAS_NOSYSLIMIT(mp)) {
		if (n <= MMU_PAGESIZE) {
			ASSERT(nocross == 0 || nocross >= MMU_PAGESIZE);
			dvma_addr = kmem_cache_alloc(mmu_p->mmu_dvma_cache_1,
			    sleep);
			if (dvma_addr) {
				mp->dmai_flags |= PX_DMAI_FLAGS_KMEM1;
				goto allocated;
			}
		} else if (n <= 2*MMU_PAGESIZE) {
			if (P2PHASE(nocross, 2*MMU_PAGESIZE) == 0) {
				dvma_addr =
				    kmem_cache_alloc(mmu_p->mmu_dvma_cache_2,
				    sleep);
				if (dvma_addr) {
					mp->dmai_flags |= PX_DMAI_FLAGS_KMEM2;
					goto allocated;
				}
			}
		} else if (n <= 8*MMU_PAGESIZE) {
			if (P2PHASE(nocross, 8*MMU_PAGESIZE) == 0) {
				dvma_addr =
				    kmem_cache_alloc(mmu_p->mmu_dvma_cache_8,
				    sleep);
				if (dvma_addr) {
					mp->dmai_flags |= PX_DMAI_FLAGS_KMEM8;
					goto allocated;
				}
			}
		} else if (n <= px_dvma_cache_quantum) {
			if (P2PHASE(nocross, px_dvma_cache_quantum) == 0) {
				dvma_addr = vmem_alloc(mmu_p->mmu_dvma_vmem_16,
				    P2ROUNDUP(n, px_dvma_cache_quantum), sleep);
				if (dvma_addr) {
					mp->dmai_flags |= PX_DMAI_FLAGS_VMEM16;
					goto allocated;
				}
			}
		} else {
			/* Requesting more than 16 pages */
			dvma_addr = vmem_xalloc(mmu_p->mmu_dvma_map,
			    P2ROUNDUP(n, px_dvma_top_cache_quantum),
			    P2ROUNDUP(mp->dmai_attr.dma_attr_align,
			    px_dvma_top_cache_quantum), 0,
			    P2ROUNDUP(nocross, px_dvma_top_cache_quantum),
			    0, 0, sleep);
			if (dvma_addr) {
				mp->dmai_flags |= PX_DMAI_FLAGS_VMEM1M;
				goto allocated;
			}
		}
		/*
		 * use reserved space as failsafe when:
		 *  - nocross restriction not fulfilled
		 *  - constrained allocations
		 */
		dvma_addr = vmem_xalloc(mmu_p->mmu_dvma_mapc, n,
		    P2ROUNDUP(mp->dmai_attr.dma_attr_align, MMU_PAGESIZE),
		    0, P2ROUNDUP(nocross, MMU_PAGESIZE), 0, 0, sleep);
		if (dvma_addr) {
			mp->dmai_flags |= PX_DMAI_FLAGS_FAILSAFE;
			goto allocated;
		}
	} else {
		/*
		 * Allocation has constraints
		 */
		ASSERT(ISP2(nocross));
		dvma_addr = vmem_xalloc(mmu_p->mmu_dvma_mapc, n,
		    P2ROUNDUP(mp->dmai_attr.dma_attr_align,
		    MMU_PAGE_SIZE), 0,
		    P2ROUNDUP(nocross, MMU_PAGE_SIZE),
		    (void *)mp->dmai_attr.dma_attr_addr_lo,
		    (void *)(mp->dmai_attr.dma_attr_addr_hi + 1), sleep);
		if (dvma_addr) {
			mp->dmai_flags |= PX_DMAI_FLAGS_FAILSAFE;
			goto allocated;
		}
...
int
px_dvma_space(dev_info_t *dip, dev_info_t *rdip, int *nrec, dvma_space_t *rec)
{
	const int nrecords = 1;		/* currently has only one DVMA space */
	px_t *px_p = DIP_TO_STATE(dip);
	px_mmu_t *mmu_p = px_p->px_mmu_p;

	if (*nrec < nrecords) {
		/*
		 * return nrec to caller so it can reallocate buffer and
		 * start over again
		 */
		*nrec = nrecords;
		return (DDI_EAGAIN);
	...
	ASSERT(mmu_p != NULL);
	ASSERT(mmu_p->mmu_dvma_end >= mmu_p->mmu_dvma_base);

	rec->dvs_hdl = mmu_p;
	rec->dvs_base = mmu_p->mmu_dvma_base;
	rec->dvs_size = mmu_p->mmu_dvma_end - mmu_p->mmu_dvma_base + 1;

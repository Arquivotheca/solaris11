...
static void px_fill_in_intr_devs(pcitool_intr_dev_t *dev, px_ih_t *ih_p);
...
static char *
pxtool_get_intr_type(uint16_t type)
{
	switch (type) {
...
static void
px_fill_in_intr_devs(pcitool_intr_dev_t *dev, px_ih_t *ih_p)
{
	uint16_t	intr_type;
	char	*driver_name = (char *)ddi_driver_name(ih_p->ih_dip);
	char	pathname[MAXPATHLEN];

	(void) ddi_pathname(ih_p->ih_dip, pathname);
	(void) strlcpy(dev->driver_name, driver_name, MAXMODCONFNAME);
	(void) strlcpy(dev->path, pathname, MAXPATHLEN);
	dev->dev_inst = ddi_get_instance(ih_p->ih_dip);

	if (ih_p->ih_rec_type != MSG_REC)
		intr_type =
		    DEVI(ih_p->ih_dip)->devi_intr_p->devi_intr_curr_type;

	(void) strlcpy(dev->type, pxtool_get_intr_type(intr_type),
	    MODMAXNAMELEN);
	dev->pri = ih_p->ih_ipil_p->ipil_pil;
	dev->inum = ih_p->ih_inum;
	dev->msg = ih_p->ih_msg_code;
}
    ...
    uint8_t *devs_ret, pcitool_intr_dev_t *devs)
{
	...
	if (ino_p != NULL) {
		for (j = 0, ipil_p = ino_p->ino_ipil_p; ipil_p;
			...
			for (i = 0, ih_p = ipil_p->ipil_ih_head;
				...
				if (ih_p->ih_msg_code == msi_num) {
					num_devs = *devs_ret = 1;
					px_fill_in_intr_devs(&devs[0], ih_p);

					goto done;
				}
				px_fill_in_intr_devs(&devs[j], ih_p);
			}
		}
		...
		*devs_ret = j;
		...
		(void) strlcpy(devs[0].driver_name,
		    (char *)ddi_driver_name(px_p->px_dip), MAXMODCONFNAME);
		(void) strlcpy(devs[0].path, pathname, MAXPATHLEN);
		devs[0].dev_inst = ddi_get_instance(px_p->px_dip);
		num_devs = *devs_ret = 1;
	}
...
int
pxtool_ib_get_ino_info(px_t *px_p, devino_t ino,
    ddi_intr_handle_impl_t *hdlp)
{
	px_ib_t		*ib_p = px_p->px_ib_p;
	px_ino_t	*ino_p;
	...
	for (i = 0, ipil_p = ino_p->ino_ipil_p; ipil_p;
	    i++, ipil_p = ipil_p->ipil_next_p) {

		hdlp->ih_pri = ipil_p->ipil_pil;

		if ((ih_p = ino_p->ino_ipil_p->ipil_ih_head) == NULL)
			continue;

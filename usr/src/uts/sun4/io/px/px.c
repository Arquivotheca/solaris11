...
extern void pcie_fab_copy_fix_addr_prop(dev_info_t *);
...
_init(void)
{
	...
	/*
	 * Check if "pcie-acs-supported" property exists to
	 * determine if disable ACS support
	 */
	extern int pcie_disable_acs;
	if (pcie_disable_acs == 0)
		pcie_disable_acs = ddi_prop_exists(DDI_DEV_T_ANY,
		    ddi_root_node(), DDI_PROP_DONTPASS,
		    "pcie-acs-supported") ? 0 : 1;
...
px_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		/* Finish initialization of pcie_bus_t */
		pcie_rc_init_bus(dip, PCIE_BUS_ALL);
		...
		/*
		 * For virtual farbic, the vpci driver should be loaded
		 * before the first PCI config access and err handling
		 * code initiaization.
		 */
		if (!pcie_is_physical_fabric(dip))
			(void) px_lib_vpci_driver_attach(dip);
		...
		/* Only trigger rebalance for root domain */
		if (pcie_is_physical_fabric(dip)) {
			/* copy address properties from PROM */
			pcie_fab_copy_fix_addr_prop(dip);

			/* reserve resources for IOV devices */
			pcie_boot_rebalance(dip);

			/* update px's copy of properties after rebalance */
			(void) px_update_props(px_p, dip);
		}

		/*
		 * For physical farbic, the vpci driver should be loaded
		 * after the pcie_bus_init but before the fabric sync code
		 * is called.
		 */
		if (pcie_is_physical_fabric(dip))
			(void) px_lib_vpci_driver_attach(dip);
		 ...
		 * any bridge driver to be ready. do this after
		 * boot rebalance since rebalance can only happen
		 * before guest domain starts.
		 */
...
px_enable_err_intr(px_t *px_p)
{
	...
	if (pcie_is_physical_fabric(px_p->px_dip) &&
	    px_pec_msg_add_intr(px_p) != DDI_SUCCESS)
		goto msg_bad;
...
px_pwr_setup(dev_info_t *dip)
{
	...
	/* No PM messages for I/O domains */
	if (!pcie_is_physical_fabric(dip))
		return (DDI_SUCCESS);
	...
	ddi_ctl_enum_t op, void *arg, void *result)
{
	...
	case DDI_CTLOPS_ATTACH:
		...
		case DDI_PRE:
			...
			(void) pcie_preattach_child(rdip, as);
		...
		case DDI_POST:
			DBG(DBG_PWR, dip, "POST_ATTACH for %s@%d\n",
			    ddi_driver_name(rdip), ddi_get_instance(rdip));
			/*
			 * call pice_postattach_child even if the driver
			 * failed the ATTACH/RESUME.
			 */
	...
	case DDI_CTLOPS_DVMASPACE:
		return (px_dvma_space(dip, rdip, arg, result));
...
px_set_mps(px_t *px_p)
{
	...
	if (pcie_root_port(dip) == DDI_FAILURE) {
		...
		(void) pcie_get_fabric_mps(dip, dip,
		    &max_supported);

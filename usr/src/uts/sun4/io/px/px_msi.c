...
px_msi_attach(px_t *px_p)
{
	...
	msi_state_p->msi_p = kmem_zalloc(msi_state_p->msi_total_cnt *
	    sizeof (px_msi_t), KM_SLEEP);

	for (range = 0, j = 0; range < msi_state_p->msi_num_ranges; ++range) {
		for (i = 0,
		    msi_num = msi_state_p->msi_ranges[range].msi_1st_msinum;
		    i < msi_state_p->msi_ranges[range].msi_cnt;
		    i++, msi_num++, j++) {
			ASSERT(j < msi_state_p->msi_total_cnt);
			msi_state_p->msi_p[j].msi_msinum = msi_num;
	...
	irm_params.iparams_total = msi_state_p->msi_total_cnt;
...
px_msi_detach(px_t *px_p)
{
	...
	if (msi_state_p->msi_p) {
		kmem_free(msi_state_p->msi_p,
		    msi_state_p->msi_total_cnt * sizeof (px_msi_t));
	}

	mutex_destroy(&msi_state_p->msi_mutex);

	if (msi_state_p->msi_ranges) {
		kmem_free(msi_state_p->msi_ranges,
		    msi_state_p->msi_num_ranges * sizeof (px_msi_ranges_t));
	}
...
/*
 * Find a set of count free contiguous MSI vectors
 */
static boolean_t
px_msi_find_contig(px_msi_state_t *msi_state_p, int count, int *firstp)
{
	px_msi_ranges_t *msi_range;
	int range, first, start_range = 0, i;
	boolean_t all_free;

	DBG(DBG_MSIQ, NULL, "px_msi_find_contig: count %d nranges %d\n", count,
	    msi_state_p->msi_num_ranges);
	for (range = 0, i = 0; range < msi_state_p->msi_num_ranges; ++range) {
		msi_range = &(msi_state_p->msi_ranges[range]);
		for (first = 0; (first+count) <= msi_range->msi_cnt;
		    first += count) {
			all_free = B_TRUE;
			for (i = first; i < (first + count) && all_free; i++) {
				if (msi_state_p->msi_p[start_range + i].
				    msi_state != MSI_STATE_FREE) {
					all_free = B_FALSE;
				} else if (i == (first + count - 1)) {
					*firstp = start_range + first;
					DBG(DBG_MSIQ, NULL,
					    "px_msi_find_contig: success "
					    "first=%d\n", *firstp);
					return (B_TRUE);
				}
			}
		}
		start_range += msi_range->msi_cnt;
	}
	return (B_FALSE);
}
	...
	*actual_msi_count_p = 0;
	...
	if (type == DDI_INTR_TYPE_MSI) {
		...
		for (count = msi_count; count; count >>= 1) {
			if (px_msi_find_contig(msi_state_p, count, &first))
				break;
	...
	if (type == DDI_INTR_TYPE_MSIX) {
		...
		for (count = 0, i = (msi_state_p->msi_total_cnt - 1);
		    i >= 0; i--) {
...
px_msi_free(px_t *px_p, dev_info_t *rdip, int inum, int msi_count)
{
	 ...
	 */
	for (i = 0, n = 0; (i < msi_state_p->msi_total_cnt) && (n < msi_count);
	    i++) {
...
px_msi_get_msinum(px_t *px_p, dev_info_t *rdip, int inum, msinum_t *msi_num_p)
{
	...
	for (i = 0; i < msi_state_p->msi_total_cnt; i++) {
	...
	if (i >= msi_state_p->msi_total_cnt)
		DBG(DBG_A_MSIX, px_p->px_dip, "px_msi_get_msinum: "
...
px_msi_get_props(px_t *px_p)
{
	...
	/* #msi */
	msi_state_p->msi_total_cnt = ddi_getprop(DDI_DEV_T_ANY, dip,
	    DDI_PROP_DONTPASS, "#msi", 0);

	DBG(DBG_MSIQ, dip, "#msi=%d\n", msi_state_p->msi_total_cnt);
	if (msi_state_p->msi_total_cnt == 0)
		return (DDI_FAILURE);
	...
	msi_state_p->msi_num_ranges = length/(2 * sizeof (int));
	msi_state_p->msi_ranges = (px_msi_ranges_t *)valuep;

	DBG(DBG_MSIQ, dip, "msi_num_ranges=%d\n", msi_state_p->msi_num_ranges);
#ifdef DEBUG
	for (int i = 0; i < msi_state_p->msi_num_ranges; ++i) {
		DBG(DBG_MSIQ, dip, "msi_ranges[%d] msi_1st_msinum %d "
		    "num_msis %d\n", i,
		    msi_state_p->msi_ranges[i].msi_1st_msinum,
		    msi_state_p->msi_ranges[i].msi_cnt);
	}
#endif /* DEBUG */

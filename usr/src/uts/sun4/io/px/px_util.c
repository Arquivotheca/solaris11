 ...
 */
int
px_update_props(px_t *px_p, dev_info_t *dip)
{
	int i;
	...
	pci_regspec_t *rp)
{
	...
	for (i = 0; i < assign_entries; i++, assign_p++) {
		...
		if (assign_type == PCI_ADDR_MEM64)
			rp->pci_phys_mid += assign_p->pci_phys_mid;
		if (space_type != assign_type) {
			DBG(DBG_MAP | DBG_CONT, dip,
			    "reg and assigned-addresses property"
			    " mismatch in address space"
			    " reg indicates 0x%x assigned indicates 0x%x\n"
			    "assigned overrides reg property\n",
			    rp->pci_phys_hi, assign_p->pci_phys_hi);
			if (space_type == PCI_ADDR_MEM32 &&
			    assign_type == PCI_ADDR_MEM64) {
				rp->pci_phys_hi ^=

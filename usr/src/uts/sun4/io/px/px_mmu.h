...
typedef struct px_mmu {
	...
	/*
	 * virtual memory map for dvma space: one for unconstrained allocations,
	 * the other for constrained allocations
	 */
	vmem_t *mmu_dvma_map;
	vmem_t *mmu_dvma_mapc;

	/*
	 * mmu_dvma_map breakdown
	 */
	vmem_t *mmu_dvma_vmem_16;
	kmem_cache_t *mmu_dvma_cache_1;
	kmem_cache_t *mmu_dvma_cache_2;
	kmem_cache_t *mmu_dvma_cache_8;

	/*
	 * callback id for dvma space
	 */

...
unscalehrtime(hrtime_t ts)
{
	...
	/*
	 * If sys_tick_freq is already a 1Ghz clock
	 * No unscaling is needed.
	 */
...
{
#ifdef sun4v
	extern uint64_t unscale2rawstick(uint64_t);
#endif
	uint64_t unscale;
	...
	unscale = unscalehrtime(time);

#ifdef sun4v
	/*
	 * For migration support.
	 * May need to re-unscale to raw stick value
	 */
	unscale = unscale2rawstick(unscale);
#endif

	tickcmpr_set(unscale);
}
...
cbe_init(void)
{
	...
	/*
	 * Initialize hrt->hrtime_base and hrt->hres_last_tick to reasonable
	 * starting values.
	 */
	hrt->hrtime_base = gethrtime();
	hrt->hres_last_tick = gethrtime_unscaled();

...
/*
 * Name of the ACPI PXM property.
 */
#define	LGRP_PLAT_ACPI_PXM	"acpi-pxm"

/*
 * Maximum number of proximity domains per device.
 */
#define	LGRP_PLAT_MAX_PXM	2
...
static lgrp_handle_t lgrp_plat_domain_to_lgrphand(int domain);
...
static boolean_t lgrp_plat_is_G5(void);
...
{
	int pxms[LGRP_PLAT_MAX_PXM];
	lgrp_handle_t lh, tmp_lh[LGRP_PLAT_MAX_PXM];
	uint_t i, nhandles = 1, valid_nhandles;

	/* get the proximity domain for the device from the ACPI */
	if ((pxms[0] = ddi_prop_get_int(DDI_DEV_T_ANY, dip, 0,
	    LGRP_PLAT_ACPI_PXM, -1)) == -1) {
		/* no proximity domain for this device */
		return (0);
	}

	if (lgrp_plat_is_G5()) {
		/*
		 * On the G5 platform, there are two sockets per CPU
		 * board with their own memory (i.e. NUMA nodes)
		 * sharing the same PCI I/O complex. All devices on
		 * that PCI I/O complex are therefore closest to two
		 * NUMA nodes. The proximity domain obtained from the
		 * ACPI gives us the index of the first socket (if it
		 * has memory, see below). We compute the index of the
		 * second socket, which always follows the first.
		 */
		nhandles = 2;

		/*
		 * The ACPI PXM may contain the index of the second
		 * socket if the first socket doesn't have physical
		 * memory attached.  We still double check the
		 * existence of the lgrp below.
		 */
		if ((pxms[0] % 2) != 0)
			pxms[0]--;
		pxms[1] = pxms[0] + 1;
	...
	/*
	 * Walk through the PXMs and find their corresponding
	 * platform lgrp handles, validating them in the process.
	 */
	for (i = 0, valid_nhandles = 0; i < nhandles; i++) {
		lh = lgrp_plat_domain_to_lgrphand(pxms[i]);
		if (lh == -1)
			continue;
		tmp_lh[valid_nhandles++] = lh;
	}

	if (valid_nhandles > 0) {
		*lhp = kmem_alloc(valid_nhandles * sizeof (lgrp_handle_t),
		    KM_SLEEP);
		bcopy(tmp_lh, *lhp, valid_nhandles * sizeof (lgrp_handle_t));
	}

	return (valid_nhandles);
}
...
/* by default do not use the HT threads on the G5 */
static boolean_t lgrp_plat_use_hw_threads_g5 = B_FALSE;

boolean_t
lgrp_plat_use_all_hw_threads(void)
{
	return (lgrp_plat_is_G5() ? lgrp_plat_use_hw_threads_g5 :
	    lgrp_plat_use_hw_threads_default);
}
...
/*
 * Translate platform "proximity domain/locality number" to lgrp_handle_t.
 */
static lgrp_handle_t
lgrp_plat_domain_to_lgrphand(int domain)
{
	ASSERT(0 <= domain && domain < MAX_NODES);
	return ((lgrp_handle_t)lgrp_plat_domain_to_node(lgrp_plat_node_domain,
	    lgrp_plat_node_cnt, domain));
}
...
/*
 * Find out if platform is G5, to understand _PXM ACPI object
 */

static boolean_t lgrp_force_G5 = B_FALSE;

static boolean_t
lgrp_plat_is_G5(void)
{
	static int is_G5 = -1;
	id_t smid;
...
#define	STRMATCH(s1, s2)	(strcasestr(s1, s2) != NULL)

	is_G5 = ((STRMATCH(mfg, "SUN MICROSYSTEMS") ||
	    STRMATCH(mfg, "Oracle")) && STRMATCH(product, "X4800"));

	return (is_G5 ? B_TRUE : B_FALSE);
}

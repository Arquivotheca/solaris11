...
cmi_mce_response(struct regs *rp, uint64_t disp)
{
	...
	/*
	 * Async errors are irrelevant to the currently thread,
	 * we need to let the trapped thread continue after
	 * we exit the handler.
	 */
	if (!(disp & CMI_ERRDISP_ASYNC)) {
		/*
		 * ontrap OT_DATA_EC and lofault protection forgive any
		 * disposition other than unconstrained, even those normally
		 * forced fatal.
		 */
		if ((otp = curthread->t_ontrap) != NULL &&
		    otp->ot_prot & OT_DATA_EC)
...
/*
 * These are required for /dev/fm and intel_nhmex. They are dummy routines
 */
...
#define	CMI_HDL_OPFUNC_STR(what, type)                          \
	type                                                    \
	cmi_hdl_##what(cmi_hdl_t ophdl)                         \
	{                                                       \
		return ((type)NULL);				\
...
/*ARGSUSED*/
CMI_HDL_OPFUNC_STR(vendorstr, const char *)
/*ARGSUSED*/
...
/*ARGSUSED*/
CMI_HDL_OPFUNC_STR(chiprevstr, const char *)
/*ARGSUSED*/
CMI_HDL_OPFUNC(getsockettype, uint32_t)
/*ARGSUSED*/
CMI_HDL_OPFUNC_STR(getsocketstr, const char *)
/*ARGSUSED*/
...
/*ARGSUSED*/
CMI_HDL_OPFUNC_STR(smb_bboard, nvlist_t *)

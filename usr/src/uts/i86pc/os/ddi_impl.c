...
/*
 * x86-specific DDI implementation
 */
#include <sys/types.h>
...
/*
 * Hash table mapping virtual addresses to the allocation size for oversized
 * kalloca allocations.
 */
static mod_hash_t *ka_addr2size_hash;

/*
 * Number of buckets in ka_addr2size_hash
 */
int ka_addr2size_hash_size = 127;

int	disable_numa_alloc = 0;
...
impl_sunbus_name_child(dev_info_t *child, char *name, int namelen)
{
	...
	if (sparc_pd_getnreg(child) > 0) {
		(void) snprintf(name, namelen, "%x,%x",
		    ...
		    (uint_t)sparc_pd_getreg(child, 0)->regspec_addr_low);
	}
...
} kmem_io[MAX_MEM_RANGES + MAX_MEM_NODES];
...
static void *
segkmem_alloc_io_4P(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[IO_4P].kmem_io_attr));
}

static void *
segkmem_alloc_io_64G(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_4G(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_2G(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_1G(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_512M(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_256M(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_128M(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_64M(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_32M(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_io_16M(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);
...
static void *
segkmem_alloc_mn0(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES].kmem_io_attr));
}

static void *
segkmem_alloc_mn1(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 1].kmem_io_attr));
}

static void *
segkmem_alloc_mn2(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 2].kmem_io_attr));
}

static void *
segkmem_alloc_mn3(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 3].kmem_io_attr));
}

static void *
segkmem_alloc_mn4(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 4].kmem_io_attr));
}

static void *
segkmem_alloc_mn5(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 5].kmem_io_attr));
}

static void *
segkmem_alloc_mn6(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 6].kmem_io_attr));
}

static void *
segkmem_alloc_mn7(vmem_t *vmp, size_t *size, size_t align, int vmflag)
{
	void	*addr;
	addr = vmem_xalloc(vmp, *size, align, 0, 0, NULL, NULL, vmflag);

	return (segkmem_xalloc(vmp, addr, *size, vmflag, 0,
	    page_create_io_wrapper, &kmem_io[MAX_MEM_RANGES + 7].kmem_io_attr));
}

struct {
	uint64_t	io_limit;
	char		*io_name;
	vmem_ximport_t	*io_alloc;
	int		io_initial;	/* kmem_io_init during startup */
} io_arena_params[MAX_MEM_RANGES + MAX_MEM_NODES] = {
	...
	{0x0000000000000000ULL, "kmem_mnode_0", segkmem_alloc_mn0,	1},
	{0x0000000000000000ULL, "kmem_mnode_1", segkmem_alloc_mn1,	1},
	{0x0000000000000000ULL, "kmem_mnode_2", segkmem_alloc_mn2,	1},
	{0x0000000000000000ULL, "kmem_mnode_3", segkmem_alloc_mn3,	1},
	{0x0000000000000000ULL, "kmem_mnode_4", segkmem_alloc_mn4,	1},
	{0x0000000000000000ULL, "kmem_mnode_5", segkmem_alloc_mn5,	1},
	{0x0000000000000000ULL, "kmem_mnode_6", segkmem_alloc_mn6,	1},
	{0x0000000000000000ULL, "kmem_mnode_7", segkmem_alloc_mn7,	1}
};

/*
 * Return pointer to a vmem_source routine for a given lgrp
 */
vmem_ximport_t *
get_lgrp_alloc(lgrp_t *lgrp)
{
	lgrp_mnode_cookie_t	lgrp_cookie;
	int			mnode;

	if (disable_numa_alloc)
		return (segkmem_align_alloc);

	if (!LGRP_EXISTS(lgrp) || max_mem_nodes == 1)
		return (NULL);
	LGRP_MNODE_COOKIE_INIT(lgrp_cookie, lgrp, LGRP_SRCH_LOCAL);

	mnode = lgrp_memnode_choose(&lgrp_cookie);

	return (io_arena_params[MAX_MEM_RANGES + mnode].io_alloc);
}
...
kmem_io_init(int a)
{
	...
	kmem_io[a].kmem_io_arena = vmem_xcreate(io_arena_params[a].io_name,
 ...
 * after curindex.
 *
 * -1 always returned for the mnode slots > MAX_MEM_RANGES.
 */
static int
...
ka_init(void)
{
	...
	/*
	 * Create ptr hash with PAGESHIFT as the value used by mod_hash_byptr.
	 * The address from contig_alloc is likely to be page aligned, so we
	 * want to get rid of all the zeroes for the hash.
	 */
	ka_addr2size_hash = mod_hash_create_extended("ka_addr2size_hash",
	    (size_t)ka_addr2size_hash_size,
	...
	/* initalize mnode ranges */
	for (a = 0; a < max_mem_nodes; a++) {

		kmem_io[a + MAX_MEM_RANGES].kmem_io_attr = kmem_io_attr;

		/* ### revisit this on memory DR */
		if (mem_node_config[a].exists == 0)
			continue;
		kmem_io[a + MAX_MEM_RANGES].kmem_io_attr.dma_attr_addr_lo =
		    ptob(mem_node_config[a].physbase);
		kmem_io[a + MAX_MEM_RANGES].kmem_io_attr.dma_attr_addr_hi =
		    ptob(mem_node_config[a].physmax + 1) - 1;

		kmem_io_init(a + MAX_MEM_RANGES);
	}
}
 ...
 */
static int
putctgas(void *addr, size_t size, int cansleep)
{
	if (cansleep) {
		return (mod_hash_insert(ka_addr2size_hash, (mod_hash_key_t)addr,
		    (mod_hash_val_t)size));
	} else {
		mod_hash_hndl_t hdl;

		/*
		 * Need to do no-sleep allocation for the key and value.
		 */
		if (mod_hash_reserve_nosleep(ka_addr2size_hash, &hdl) != 0)
			return (1);

		return (mod_hash_insert_reserve(ka_addr2size_hash,
		    (mod_hash_key_t)addr, (mod_hash_val_t)size, hdl));
	}
}
...
getctgsz(void *addr)
{
	...
	(void) mod_hash_remove(ka_addr2size_hash, (mod_hash_key_t)addr,
	    (mod_hash_val_t *)&size);
 ...
 *
 * If mnode < 0, allocate from the dma_attr range otherwise allocate from
 * the mnode range.
 */
static void *
kalloca_internal(size_t size, size_t align, int cansleep, int physcontig,
	ddi_dma_attr_t *attr, int mnode)
{
	...
	if (physcontig && rsize > PAGESIZE) {
		if (addr = contig_alloc(size, attr, align, cansleep)) {
			if (putctgas(addr, size, cansleep) == 0)
				return (addr);
			else
	...
	if (rsize > PAGESIZE) {
		...
		c = highbit((rsize - 1) >> KA_ALIGN_SHIFT);
...
kalloca(size_t size, size_t align, int cansleep, int physcontig,
	ddi_dma_attr_t *attr, lgrp_t *lgrp)
{
	ddi_dma_attr_t		tmpattr;
	lgrp_mnode_cookie_t	lgrp_cookie;
	...
	/*
	 * allocate from the specified lgrp if it exists and its memory
	 * range overlaps the dma range specified in attr. If the lgrp
	 * allocation fails, allocate from the original attr range.
	 */
	if ((disable_numa_alloc == 0) &&
	    (max_mem_nodes > 1 && LGRP_EXISTS(lgrp))) {
		int	mnode;
		...
		while ((mnode = lgrp_memnode_choose(&lgrp_cookie)) >= 0) {
			...
			mlo = ptob(mem_node_config[mnode].physbase);
			mhi = ptob(mem_node_config[mnode].physmax + 1) - 1;
			ASSERT(mhi > mlo);

			tmpattr = *attr;

			if (mhi <= tmpattr.dma_attr_addr_lo ||
			    mlo > tmpattr.dma_attr_addr_hi) {
				/* no overlap of lgrp and dma range */
				mnode = lgrp_memnode_choose(&lgrp_cookie);
				continue;
			}

			if (mlo > tmpattr.dma_attr_addr_lo)
				tmpattr.dma_attr_addr_lo = mlo;

			if (mhi < tmpattr.dma_attr_addr_hi)
				tmpattr.dma_attr_addr_hi = mhi;

			/*
			 * account for cases where parts of the mnode memory
			 * can be outside of the dma range - set mnode to -1
			 * so to use dma ranges for correctness.
			 */
			if (mhi > tmpattr.dma_attr_addr_hi ||
			    mlo < tmpattr.dma_attr_addr_lo) {
				mnode = -1;
			}

			ptr = kalloca_internal(size, align, cansleep,
			    physcontig, &tmpattr, mnode);
	...
	/* allocate from the dma attribute */
	return (kalloca_internal(size, align, cansleep, physcontig,
	    attr, (int)-1));
}
	...
	size_t *real_length, ddi_acc_hdl_t *ap)
{
	...
	/*
	 * determine preferred lgrp through driver info. The dma
	 * memory returned will be from the preferred lgrp if the lgrp
	 * memory range overlaps the specified dma range.
	 */
	...
	for (;;) {
		 ...
		 */
		*kaddrp = kalloca(length, iomin, cansleep, physcontig,
		    attr, lgrp);
		...
		/* retry if there are additional lgrps */
		li = (li + 1) % nlgrps;
		if (li != sli)
			lgrp = lgrps[li];
		else
	...
	if (OVERRIDE_CACHE_ATTR(flags)) {
		...
		e = kmem_override_cache_attrs(*kaddrp, length, order);
		if (e != 0) {
			kfreea(*kaddrp);
			return (DDI_FAILURE);
		}
...
get_boot_properties(void)
{
	...
	for (name = BOP_NEXTPROP(bootops, "");		/* get first */
		...
		} else if (strcmp(name, "bios-free") == 0) {
			/* acpica's osl.c consumes this */
...
/*ARGSUSED*/
boolean_t
dev_path_match_assigned(char *devpath)
{
	return (B_FALSE);
}
...
/*ARGSUSED*/
int
i_ddi_dvma_space(dev_info_t *dip, int *nrec, dvma_space_t *rec)
{
	return (DDI_ENOTSUP);
}

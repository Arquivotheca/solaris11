 ...
 * Pass 1 includes:
 *
 *	o Determining vendor/model/family/stepping and setting x86_vendor.
...
/*
 * This is only used in kernel code. Use cpuid_feature_ecx_exclude in EEPROM
 * parameter to manually disable XSAVE.
 */
static boolean_t xsave_force_disable = B_FALSE;
...
struct cpuid_info {
	...
	uint8_t cpi_pabits;		/* fn 0x80000008: %eax[07-00] */
	uint8_t	cpi_vabits;		/* fn 0x80000008: %eax[15-08] */
 ...
 */
#define	CPUID_LEAFD_1_EAX_XSAVEOPT	0x1
...
/*
 * Check CPUID for XSAVEOPT support.
 * Must be called after cpi->cpi_maxeax is set.
 */
static int
detect_xsaveopt(struct cpuid_info *cpi)
{
	...
	cpr.cp_eax = 0xD;
	cpr.cp_ecx = 1;
	cpr.cp_edx = cpr.cp_ebx = 0;
	(void) __cpuid_insn(&cpr);
	return (cpr.cp_eax & CPUID_LEAFD_1_EAX_XSAVEOPT);
}
...
cpuid_pass1(cpu_t *cpu, uchar_t *featureset)
{
	...
	case X86_VENDOR_AMD:
		 ...
		 * This is still true for Family 15h processors.
	...
	case X86_VENDOR_Intel:
		/*
		 * For all processors supporting 64bit, SSE3 and CX16, at least,
		 * are valid; enable all
		 */
		mask_ecx = 0xffffffff;
	...
	if (cp->cp_edx & CPUID_INTC_EDX_FXSR) {
		...
		if (cpi->cpi_vendor == X86_VENDOR_Intel) {
			...
			if (cp->cp_ecx & CPUID_INTC_ECX_XSAVE) {
				...
				/*
				 * We only test XSAVEOPT and AVX when XSAVE
				 * exists.
				 */
	...
	if (cpi->cpi_xmaxeax & 0x80000000) {
		...
		case X86_VENDOR_AMD:
			...
			if (cp->cp_edx & CPUID_AMD_EDX_TSCP) {
				/*
				 * If TSC is disabled also disable TSCP.
				 */
				if (tsc_gethrtime_enable == 0)	{
					uhrt_enable = 0;
				} else {
					add_x86_feature(featureset,
					    X86FSET_TSCP);
				...
				/*
				 * Disable userland high resolution time if
				 * hardware does not support tscp.
				 */
				uhrt_enable = 0;
			}
			break;
	...
	if ((cpi->cpi_xmaxeax & 0x80000000) == 0) {
		...
		if (cpi->cpi_brandstr[0]) {
			...
			*dst = '\0';
			...
			/*
			 * Create brandstr by concatenating vendorstr,
			 * family, mode, and step info.
			 */
			fabricate_brandstr(cpi);
		}
	}
...
cpuid_pass4(cpu_t *cpu)
{
	...
	if (cpi->cpi_vendor == X86_VENDOR_Intel ||
		...
		*ecx = CPI_FEATURES_XTD_ECX(cpi);
		...
		/*
		 * Both AMD and Intel use CPUID_AMD_ECX_AHF64 and
		 * CPUID_AMD_EDX_TSCP for LAHF/SAHF support in 64-bit mode
		 * and RDTSCP support, respectively.
		 */
...
cpuid_opteron_erratum(cpu_t *cpu, uint_t erratum)
{
	...
	/*
	 * Bail out if this CPU isn't an AMD CPU.
	 */
	if (cpi->cpi_vendor != X86_VENDOR_AMD)
		return (0);
...
add_amd_l2_assoc(dev_info_t *devi, const char *label, uint_t assoc)
{
	...
	afd = (assoc < (sizeof (amd_afd) / sizeof (amd_afd[0]))) ?
	    amd_afd[assoc] : -1;
	...
	if (afd == 0) {
		...
		add_cache_prop(devi, label, assoc_str, afd);
	}
}
...
amd_cache_info(struct cpuid_info *cpi, dev_info_t *devi)
{
	...
	if (BITX(cp->cp_ebx, 31, 16) == 0) {
		add_amd_l2_tlb(devi, "l2-tlb-4K",
		    BITX(cp->cp_ebx, 15, 12), BITX(cp->cp_ebx, 11, 0));
	} else {
		add_amd_l2_tlb(devi, "l2-dtlb-4K",
		    BITX(cp->cp_ebx, 31, 28), BITX(cp->cp_ebx, 27, 16));
...
x86_which_cacheinfo(struct cpuid_info *cpi)
{
	...
	default:
		 ...
		 * information.
		 *
		 * Else assume we're using Intel's table-driven format instead.
		 */
		if (cpi->cpi_xmaxeax >= 0x80000005)
			return (X86_VENDOR_AMD);
...
cpuid_x2apic_supported(void)
{
	...
	return ((cp.cp_ecx & CPUID_INTC_ECX_X2APIC) ? 1 : 0);
}

...
mp_cpu_configure_common(int cpun, boolean_t boot)
{
	...
	/*
	 * The startup thread isn't transient.
	 */
...
mp_startup_common(boolean_t boot)
{
	...
	if (compare_x86_featureset(x86_featureset, new_x86_featureset) ==
		...
		for (idx = 0; idx < NUM_X86_FEATURES; idx++) {
			if (is_x86_feature(x86_featureset, idx) &&
			    !(is_x86_feature(new_x86_featureset, idx))) {
				cmn_err(CE_CONT, "?cpu%d: featureset\n",
				    cp->cpu_id);
				print_x86_featureset(new_x86_featureset);
				cmn_err(CE_WARN, "cpu%d feature mismatch",
				    cp->cpu_id);
				break;
			}
		}
	 ...
	 * it sees this bit set for all active CPUs.  Any code after this
	 * point until thread_exit() must not block, as it's easy to cause
	 * deadlock with the master CPU or the next slave CPU to start.
	 */

...
/*
 * The ACPI _CSD object provides the C-state dependencies.
 * Unfortunately _CSD objects are not commonly used by other operating systems
 * because the C-state dependency is trivial (i.e. the core).  As a result of
 * this lack of use, _CSD objects are often incorrect.
 * For now we also use the core-id to determine C-state dependency.
 */
boolean_t cpupm_csd_untrusted = B_TRUE;
    ...
    cpu_acpi_obj_t objtype, cpu_acpi_state_dependency_t *sd)
{
	...
	if (objtype == CSD_OBJ) {
		if (cpupm_csd_untrusted) {
			sd->sd_domain = cpuid_get_coreid(cpu[handle->cs_id]);
			sd->sd_entries = 6;
			sd->sd_revision = 0x0;
			sd->sd_type = CPU_ACPI_HW_ALL;
			sd->sd_num = 0;
			sd->sd_index = 0;
			return (0);
		}
...
/*
 * Get processor Aggregator Device handle thru the Plug and Play ID
 */
static ACPI_STATUS
cpu_probe_prad(ACPI_HANDLE obj, UINT32 level, void *ctx, void **rv)
{
	...
	/*
	 * The desired PRAD object is in the ACPI_HANDLE obj. Store it in
	 * the context (ctx) passed in by the caller of the AcpiGetDevices
	 * itterator.
	 */
	*(ACPI_HANDLE *)ctx = obj;

	/*
	 * We have found the desired object.  Terminate the search that
	 * called this function (this will end the search and return AE_OK).
	 */
...
cpu_acpi_pur(cpu_acpi_handle_t handle)
{
	...
	/*
	 * Fetch the PRAD object (if present)
	 */
	if (AcpiGetDevices(ACPI_DEVNAME_PRAD, cpu_probe_prad,
	    &pur_handle, NULL) != AE_OK || pur_handle == NULL) {
		goto out;
	}

	astatus = AcpiEvaluateObjectTyped(pur_handle, "_PUR",
	    NULL, &abuf, ACPI_TYPE_PACKAGE);
	if (ACPI_FAILURE(astatus)) {
		if (astatus == AE_NOT_FOUND) {
			DTRACE_PROBE3(cpu_acpi__eval__err, int, handle->cs_id,
 ...
 */
void
cpu_acpi_ost(uint32_t status, uint32_t num_idle_cpus)
{
	ACPI_HANDLE ost_handle = NULL;
	ACPI_OBJECT obj[3] = {
	...
	if (AcpiGetDevices(ACPI_DEVNAME_PRAD, cpu_probe_prad,
	    &ost_handle, NULL) == AE_OK && ost_handle != NULL) {
		(void) AcpiEvaluateObject(ost_handle, "_OST", &list, NULL);
	}
}

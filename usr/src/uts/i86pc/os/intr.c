...
hilevel_intr_prolog(struct cpu *cpu, uint_t pil, uint_t oldpil, struct regs *rp)
{
	...
	if (mask != 0) {
		...
		ASSERT(nestpil < pil);
		intrtime = now -
		    cpu->cpu_pil_high_start[nestpil - (LOCK_LEVEL + 1)];
		cpu->cpu_intrstat[nestpil][0] += intrtime;
		...
		if ((t->t_flag & T_INTR_THREAD) != 0 && t->t_intr_start != 0) {
			...
			cpu->cpu_intrstat[t->t_pil][0] += intrtime;
			cpu->cpu_intracct[cpu->cpu_mstate] += intrtime;
			t->t_intr_start = 0;
		}
	}
	 ...
	 */
	cpu->cpu_pil_high_start[pil - (LOCK_LEVEL + 1)] = now;
...
hilevel_intr_epilog(struct cpu *cpu, uint_t pil, uint_t oldpil, uint_t vecnum)
{
	...
	intrtime = now - cpu->cpu_pil_high_start[pil - (LOCK_LEVEL + 1)];
	cpu->cpu_intrstat[pil][0] += intrtime;
	cpu->cpu_intracct[cpu->cpu_mstate] += intrtime;

	/*
	 * Check for lower-pil nested high-level interrupt beneath
	 * current one.  If so, place a starting timestamp in its
	 * cpu_pil_high_start entry.
	 */
	mask = cpu->cpu_intr_actv & CPU_INTR_ACTV_HIGH_LEVEL_MASK;
	if (mask != 0) {
		int nestpil;
		...
		nestpil = bsrw_insn((uint16_t)mask);
		ASSERT(nestpil < pil);
		cpu->cpu_pil_high_start[nestpil - (LOCK_LEVEL + 1)] = now;
		/*
 ...
 * and updates cpu_intrstat[pil][1] to be the same as the new value of
 * cpu_intrstat[pil][0].
 *
 ...
 * the higher-pil interrupt's time is accumulated in cpu_intrstat[higherpil][].
 */
uint64_t
intr_get_time(void)
{
	...
	time = cpu->cpu_intrstat[pil][0] + delta;
	ret = time - cpu->cpu_intrstat[pil][1];
	cpu->cpu_intrstat[pil][0] = time;
	cpu->cpu_intrstat[pil][1] = time;
...
cpu_kstat_intrstat_update(kstat_t *ksp, int rw)
{
	...
	for (i = 0; i < PIL_MAX; i++) {
		hrt = (hrtime_t)cpup->cpu_intrstat[i + 1][0];
		scalehrtimef(&hrt);
...
cpu_intr_swtch_enter(kthread_id_t t)
{
	...
	if (t->t_intr_start) {
		...
		cpu = CPU;
		cpu->cpu_intrstat[t->t_pil][0] += interval;
...
do_splx(int newpri)
{
	...
	if (IS_FAKE_SOFTINT(flag, newpri)) {
		/*
		 * NOTE: If interrupts are enabled at the beginning of do_splx,
		 * fakesoftint() will return with interrupts enabled.
		 */
		fakesoftint(flag & PS_IE);
	}
...
splr(int newpri)
{
	...
	if (newpri > curpri) {
		...
		if (IS_FAKE_SOFTINT(flag, newpri)) {
			/*
			 * NOTE: If interrupts are enabled at the beginning of
			 * splr(), fakesoftint will return with interrupts
			 * enabled.
			 */
			fakesoftint(flag & PS_IE);
		}
...
typedef struct softint_hdl {
	ddi_softint_hdl_impl_t	sh_hdl;
	int			sh_pil;
	avfunc			sh_func;
	caddr_t			sh_arg;
} softint_hdl_t;
...
ulong_t
create_softint(uint_t pil, uint_t (*func)(caddr_t, caddr_t), caddr_t arg,
    char *name)
{
	softint_hdl_t	*hdlp;

	hdlp = kmem_zalloc(sizeof (softint_hdl_t), KM_SLEEP);
	hdlp->sh_pil = pil;
	hdlp->sh_func = (avfunc)func;
	hdlp->sh_arg = arg;

	(void) add_avsoftintr(&hdlp->sh_hdl, pil, (avfunc)func, name, arg, 0);
	return ((ulong_t)hdlp);
}

/*ARGSUSED*/
static void
invoke_softint_helper(uintptr_t arg1, uintptr_t arg2)
{
	softint_hdl_t	*hdlp;

	hdlp = (softint_hdl_t *)arg1;
	(*setsoftint)(hdlp->sh_pil, hdlp->sh_hdl.ih_pending);
}

void
invoke_softint(processorid_t cpuid, ulong_t hdl)
{
	cpu_call_nowait(cpu[cpuid], invoke_softint_helper, (uintptr_t)hdl, 0);
}

void
remove_softint(ulong_t hdl)
{
	softint_hdl_t	*hdlp = (softint_hdl_t *)hdl;

	(void) rem_avsoftintr(&hdlp->sh_hdl, hdlp->sh_pil, hdlp->sh_func);
	kmem_free(hdlp, sizeof (softint_hdl_t));
}

/*ARGSUSED*/
static void
sync_softint_helper(uintptr_t arg1, uintptr_t arg2)
{
}
...
{
	cpu_call(cpu[cpuid], sync_softint_helper, NULL, NULL);
}

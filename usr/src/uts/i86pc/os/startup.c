...
#define	X86_CLOCK_TICK_THRESHOLD	64
#define	X86_CLOCK_TICK_NCPUS		64
...
/*
 * IOMMU operations vector (Intel or AMD)
 * Initialized in startup_end by one of the IOMMU modules. Remains
 * NULL if unused / not present.
 */
struct iommu_ops *iommu_ops = NULL;
...
startup_init()
{
	...
	if (clock_tick_threshold == 0)
		clock_tick_threshold = X86_CLOCK_TICK_THRESHOLD;
	if (clock_tick_ncpus == 0)
		clock_tick_ncpus = X86_CLOCK_TICK_NCPUS;
...
startup_memlist(void)
{
	...
	extern void page_coloring_setup(caddr_t, int);
...
#ifdef DEBUG
	{
		/* See comments where PP_SHIFT is defined. */
		int size = sizeof (struct page);
		int low_bits = highbit(size) - 1;

		if (!ISP2(size))
			low_bits++;
		ASSERT(low_bits == PP_SHIFT);
	}
#endif
	/*
	 ...
	 * setup page coloring
	 */
	page_coloring_setup(pagecolor_mem, l2cache_linesz);
...
startup_vm(void)
{
	...
	/*
	 * Set up the kernel page freelist for x64 systems, with more than
	 * 7G of memory.
	 */
	if (physmem <= PHYSMEM_7G) {
		kflt_disable = 1;
	}
	if (!kflt_disable) {
		kflt_init();

		/* Enable large user text pages if kernel freelist is on */
		max_utext_lpsize = LEVEL_SIZE(1);
	}
...
#if !defined(__xpv)
	for (i = 0; i < (sizeof (iommu_drivers) / sizeof (char *)); i++) {
		if (modload("drv", iommu_drivers[i]) >= 0) {
			iommu_driver_name = iommu_drivers[i];
			break;
		}
	}

	if (iommu_driver_name != NULL)
		(void) i_ddi_attach_hw_nodes(iommu_driver_name);

	if (iommu_ops != NULL)
		iommu_ops->ioo_start_units();
#endif
...
post_startup(void)
{
	...
	/*
	 * Initialize high resolution time state for libc.
	 */
	hrt_init();
...
release_bootstrap(void)
{
	...
	/* If a framebuffer console was specified, clear the 1:1 fb mapping. */
	start = 0;
	end = _userlimit;

	if (console == CONS_SCREEN_FB || console == CONS_SCREEN_GRAPHICS) {
		uintptr_t start_fb, end_fb;

		start_fb = fb_info.phys_addr;
		end_fb = start_fb + fb_info.used_mem;

		if (end_fb > end) {
			if (start_fb <= end) {
				end = end_fb;
			} else {
				clear_boot_mappings(start_fb, end_fb);
			}
		}
	}
	clear_boot_mappings(start, end);

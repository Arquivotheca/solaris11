...
/* for legacy IO config space access workaround */
kmutex_t pcicfg_bad_bridge_mutex;
...
pci_cfgspace_init(void)
{
	...
	mutex_init(&pcicfg_bad_bridge_mutex, NULL, MUTEX_SPIN,
	    (ddi_iblock_cookie_t)ipltospl(DISP_LEVEL));
	mutex_init(&pcicfg_chipset_mutex, NULL, MUTEX_SPIN,
	    (ddi_iblock_cookie_t)ipltospl(15));
	if (!pci_check()) {
		mutex_destroy(&pcicfg_mutex);
		mutex_destroy(&pcicfg_bad_bridge_mutex);
...
pci_check_bios(void)
{
	...
	/* On non-BIOS systems, use mechanism 1 */
	if (_sys_fw_type != FWT_BIOS) {
		pci_bios_mech = 1;
		pci_bios_vers = 0;
		pci_bios_maxbus = pci_max_nbus;	/* XXX */

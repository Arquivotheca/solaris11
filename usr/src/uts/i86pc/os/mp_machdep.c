...
int (*psm_check_isr_pending)(void) = NULL;
...
cpu_idle_mwait(void)
{
	...
	*mcpu_mwait = MWAIT_HALTED;
	...
	/*
	 * If we have anything local that is runnable we do not have to wait
	 * for the write to mcpu_mwait. We also check if this CPU is being
	 * selected to steal work from another CPU as indicated by the
	 * cpu_steal_from field.
	 */
	if (cpup->cpu_disp->disp_nrunnable > 0 ||
	    cpup->cpu_steal_from != NULL) {
		if (hset_update) {
...
cpu_wakeup_mwait(cpu_t *cp, int bound)
{
	...
	if (cpu_found != CPU->cpu_seqid) {
		...
		/*
		 * A monitor/mwait wakeup is cheap enough that it can be
		 * done even if we're trying to awake the current CPU. But
		 * since we've checked anyway..
		 */
...
/*
 * Allow threads at priority 60 and higher to take up 25% of a chip PG,
 * decaying that amount also by 50% at each priority level below 60.
 */
pg_pri_util_spec_t pg_plat_pri_util_chip = {.pct = 25, .pri = 60, .decay = 50};
...
pg_plat_pri_util(pghw_type_t hw, pg_pri_util_spec_t *spec)
{
	switch (hw) {
	case PGHW_CHIP:
		spec->pct = pg_plat_pri_util_chip.pct;
		spec->pri = pg_plat_pri_util_chip.pri;
		spec->decay = pg_plat_pri_util_chip.decay;

 ...
 * multiboot_info.
 */
extern multiboot_info_t *saved_mbi1;
extern mb_memory_map_t *saved_mmap;
extern size_t saved_mmap_size;
extern uint8_t *saved_drives;
extern char saved_cmdline[FASTBOOT_SAVED_CMDLINE_LEN];
extern int saved_cmdline_len;
extern size_t saved_file_size[];
extern int need_restore_vbe_info;
extern int saved_mbi_version;

extern char *saved_mbi2;
...
static boolean_t any_bios_routines_in_ram(void);
...
void
fastboot_dboot_dprintf(char *fmt, ...)
{
	va_list adx;
...
do1_fastboot_build_mbi1(char *mdep, fastboot_info_t *nk)
{
	...
	struct VbeInfoBlock	*vbe_controlp = NULL;
	struct ModeInfoBlock	*vbe_modep = NULL;
	struct multiboot_tag_vbe *vesap = NULL;
	extern struct VbeInfoBlock	*fastboot_saved_vbe_control;
	extern struct ModeInfoBlock	*fastboot_saved_vbe_mode;
	struct multiboot2_info_header *mb2_info;
	struct multiboot_tag_mmap *mb2_mmap_tagp;
	int mmap_num_entries;
	 ...
	 */
	if (saved_mbi_version == 1) {
		size = PAGESIZE + P2ROUNDUP(arglen +
		    saved_mbi1->drives_length + saved_mbi1->mmap_length +
		    sizeof (struct VbeInfoBlock) +
		    sizeof (struct ModeInfoBlock), PAGESIZE);
	} else {
		mb2_info = (struct multiboot2_info_header *)saved_mbi2;
		mb2_mmap_tagp = dboot_multiboot2_get_mmap_tagp(mb2_info);
		mmap_num_entries = dboot_multiboot2_mmap_entries(mb2_info,
		    mb2_mmap_tagp);

		size = PAGESIZE + P2ROUNDUP(arglen +
		    (mmap_num_entries * sizeof (mb_memory_map_t)) +
		    sizeof (struct VbeInfoBlock) +
		    sizeof (struct ModeInfoBlock), PAGESIZE);
	}
	...
	if (saved_mbi_version == 1) {
		/*
		 * Copy from the saved mbi to preserve all non-pointer type
		 * fields.
		 */
		bcopy(saved_mbi1, mbi, sizeof (*mbi));
	} else {
		struct multiboot_tag_bootdev *bootdev;
		uint32_t lower, upper;

		/* fill out non-pointer fields */
		if (dboot_multiboot2_basicmeminfo(mb2_info, &lower, &upper) ==
		    1) {
			mbi->mem_lower = lower;
			mbi->mem_upper = upper;
		...
		/* The equivalent in mb2 is not reliable per Seth XXX */
		...
		if (bootdev != NULL) {
			mbi->boot_device = ((bootdev->biosdev & 0xFF) << 24) |
			    ((bootdev->slice & 0xFF) << 16) |
			    (bootdev->part & 0xFF);
			dprintf("bootdev: %x %x %x %x %x\n",
			    bootdev->type, bootdev->size, bootdev->biosdev,
			    bootdev->slice, bootdev->part);
			mbi->flags |= MB_INFO_BOOTDEV;
		} else {
			/* should we assume we netbooted? XXX */
			dprintf("No bootdev info \n");
		}
	}
	 ...
	 * to the name of the new boot archive.
	 */
	...
	/* Recalculate mmap addr */
	mbi->mmap_addr = start_addr_pa + offs;
	if (saved_mbi_version == 1) {
		bcopy((void *)(uintptr_t)saved_mmap,
		    (void *)(start_addr_va + offs), saved_mbi1->mmap_length);
		offs += saved_mbi1->mmap_length;
	} else {
		uint64_t start, length;
		mb_memory_map_t *mmap;
		int i;

		mmap = (mb_memory_map_t *)(start_addr_va + offs);

		for (i = 0; i < mmap_num_entries; i++) {

			mmap->type = dboot_multiboot2_mmap_get_type(mb2_info,
			    mb2_mmap_tagp, i);

			start = dboot_multiboot2_mmap_get_base(mb2_info,
			    mb2_mmap_tagp, i);
			mmap->base_addr_low = (uint32_t)(start & 0xFFFFFFFFULL);
			mmap->base_addr_high = (uint32_t)
			    ((start >> 32) & 0xFFFFFFFFULL);

			length = dboot_multiboot2_mmap_get_length(mb2_info,
			    mb2_mmap_tagp, i);
			mmap->length_low = (uint32_t)(length & 0xFFFFFFFFULL);
			mmap->length_high = (uint32_t)
			    ((length >> 32) & 0xFFFFFFFFULL);

			mmap->size = sizeof (mb_memory_map_t) -
			    sizeof (mmap->size);

			mmap++;
		}

		mbi->mmap_length =  (uintptr_t)mmap - (start_addr_va + offs);
		mbi->flags |= MB_INFO_MEM_MAP;
		offs += mbi->mmap_length;
	}

	/*
	 * Recalculate drives_addr
	 * multiboot2 does not provide the equivalent of saved_drives.
	 * For Solaris hack, if the field contain the dhcp ack for first
	 * netboot XX, we will not be copying the data anyway. drives_addr
	 * and drives_length should already be set to 0. flag should not
	 * have MB_INFO_DRIVE_INFO set.
	 */
	if (saved_mbi_version == 1) {
		mbi->drives_addr = start_addr_pa + offs;
		bcopy((void *)(uintptr_t)saved_drives,
		    (void *)(start_addr_va + offs), saved_mbi1->drives_length);
		offs += saved_mbi1->drives_length;
	}

	if (saved_mbi_version == 1) {
		if (saved_mbi1->flags & MB_INFO_VIDEO_INFO &&
		    need_restore_vbe_info != 0)  {
			vbe_controlp = fastboot_saved_vbe_control;
			vbe_modep = fastboot_saved_vbe_mode;
		}
	} else {
		...
		if (fbip != NULL) {
			...
			dboot_fb_info_to_vesa_info(fbip,
			    fastboot_saved_vbe_control,
			    fastboot_saved_vbe_mode);
			vbe_controlp = fastboot_saved_vbe_control;
			vbe_modep = fastboot_saved_vbe_mode;

		} else  if ((vesap = (struct multiboot_tag_vbe *)
		    dboot_multiboot2_find_tag(mb2_info,
		    MULTIBOOT_TAG_TYPE_VBE)) != NULL) {

			vbe_controlp = (struct VbeInfoBlock *)
			    &vesap->vbe_control_info;
			vbe_modep = (struct ModeInfoBlock *)
			    &vesap->vbe_mode_info;
		}
	}

	if (vbe_controlp != NULL && vbe_modep != NULL) {
		if (nk->fi_mbi_size <
		    (offs + sizeof (struct VbeInfoBlock) +
		    sizeof (struct ModeInfoBlock))) {
			dprintf("VBE OVERFLOW\n");
			return (-1);
		}

		mbi->vbe_control_info = start_addr_pa + offs;
		bcopy((void *)(uintptr_t)vbe_controlp,
		    (void *)(start_addr_va + offs),
		    sizeof (struct VbeInfoBlock));
		offs += sizeof (struct VbeInfoBlock);

		mbi->vbe_mode_info = start_addr_pa + offs;
		bcopy((void *)(uintptr_t)vbe_modep,
		    (void *)(start_addr_va + offs),
		    sizeof (struct ModeInfoBlock));
		offs += sizeof (struct ModeInfoBlock);

		mbi->flags |= MB_INFO_VIDEO_INFO;
	} else {
		/* clean flag relative to video information. */
		mbi->flags &= ~MB_INFO_VIDEO_INFO;
		/* zero out the vbe_* members of the multiboot structure. */
		bzero(&mbi->vbe_control_info, sizeof (*mbi) -
		    offsetof(multiboot_info_t, vbe_control_info));
	}
	...
	if (nk->fi_mbi_size < (offs + arglen)) {
		dprintf("ARGLEN OVERFLOW\n");
		return (-1);
	}
	...
	bzero(&mbi->config_table, offsetof(multiboot_info_t, apm_table) -
	    offsetof(multiboot_info_t, config_table));
	mbi->flags &= ~(MB_INFO_CONFIG_TABLE | MB_INFO_BOOT_LOADER_NAME |
	    MB_INFO_APM_TABLE);

	nk->fi_mbi_bootloader_magic = MB_BOOTLOADER_MAGIC;

	return (0);
}


#define	MULTIBOOT2_PAD	256	/* padding in case we need to increase length */

static int
do2_fastboot_build_mbi2(char *mdep, fastboot_info_t *nk)
{
	...
	bcopy(saved_mbi2, &hdr, sizeof (struct multiboot2_info_header));

	/*
	 * Allocate memory for the new multiboot2 info structure
	 * If we have reserved memory but it's not enough, free it
	 * and reallocate.
	 */
	size = P2ROUNDUP((size_t)hdr.total_size + MULTIBOOT2_PAD, PAGESIZE);
	...
	/* Copy from the saved mb2 and adjust if necessary */

	mb2_info = (struct multiboot2_info_header *)start_addr_va;
	bcopy(saved_mbi2, mb2_info, hdr.total_size);

	/* Populate the rest of the fields in the data structure */

	if (dboot_multiboot2_set_modstart(mb2_info, 0,
	    nk->fi_files[FASTBOOT_BOOTARCHIVE].fb_dest_pa) != 0) {
		dprintf("set modstart failed\n");
		return (-1);
	}

	if (dboot_multiboot2_set_modend(mb2_info, 0,
	    nk->fi_files[FASTBOOT_BOOTARCHIVE].fb_next_pa) != 0) {
		dprintf("set modend failed\n");
		return (-1);
	}

	filename = (char *)fastboot_filename[FASTBOOT_NAME_BOOTARCHIVE];

	if (*fastboot_filename[FASTBOOT_NAME_BOOTARCHIVE] == '/' &&
	    *((char *)fastboot_filename[FASTBOOT_NAME_BOOTARCHIVE] + 1) ==
	    '/') {

		/*
		 * If there name starts with //, ignore the first /
		 * otherwise we would unnecessarily resize mb2_info
		 */
		filename =
		    (char *)fastboot_filename[FASTBOOT_NAME_BOOTARCHIVE] + 1;
	}

	/* using index 0 for boot archive */
	if (dboot_multiboot2_set_modcmdline(mb2_info, 0, filename) != 0) {
		dprintf("setting modcmdline failed\n");
		return (-1);
	}

	if (mdep != NULL && strlen(mdep) != 0) {
		if (dboot_multiboot2_set_cmdline(mb2_info, (char *)mdep) != 0) {
			dprintf("set_cmdline mdep failed\n");
			return (-1);
		}
	} else {
		if (dboot_multiboot2_set_cmdline(mb2_info,
		    (char *)saved_cmdline) != 0) {
			dprintf("set_cmdline saved cmdline failed\n");
			return (-1);
		}
	}

	nk->fi_mbi_bootloader_magic = MULTIBOOT2_BOOTLOADER_MAGIC;
...
fastboot_build_mbi(char *mdep, fastboot_info_t *nk)
{
	...
	if (saved_mbi_version == 1) {
		/*
		 * we are not inspecting kernel here since
		 * a multiboot2 capable kernel is also backwards
		 * compatible and can boot with multiboot1
		 */

		return (do1_fastboot_build_mbi1(mdep, nk));

	} else if (saved_mbi_version == 2) {
		struct multiboot2_header *header;
		uintptr_t  va;

		if ((va = (&nk->fi_files[FASTBOOT_NAME_UNIX])->fb_va) == NULL) {
			dprintf("NO VA \n");
			return (-1);
		}

		/* inspect the new kernel to see if it is mb2 capable */
		for (header = (struct multiboot2_header *)va;
		    ((uintptr_t)header <= va + MULTIBOOT2_SEARCH - 12) ||
		    (header = NULL); header = (struct multiboot2_header *)
		    ((char *)header + MULTIBOOT2_HEADER_ALIGN)) {
		...
		if (header == NULL) {
			/* New kernel is not multiboot2 capable */
			return (do1_fastboot_build_mbi1(mdep, nk));
		} else {
			return (do2_fastboot_build_mbi2(mdep, nk));
		}
	} else {
		dprintf("unrecognized multiboot version\n");
		return (-1);
	}
}
...
fastboot_post_startup()
{
	...
	/*
	 * Booting currently uses three BIOS routines: INT 0x10, 0x13 and 0x1A.
	 * If any of these vectors have been hooked to routines in RAM,
	 * they may have been overwritten by Solaris and may no longer valid,
	 * so don't allow fast reboot.
	 * Of these 3 vectors, I have only seen 0x1A hooked to a routine
	 * in RAM and it is only when the machine is net booted.
	 * RAM addresses are in the range 0000:0000 to BFFF:FFFF
	 */
	if (any_bios_routines_in_ram())
		fastreboot_disable(FBNS_BIOSERR);
...
/*
 * Map in the BIOS Interrupt Descriptor Table and examine the vectors for
 * INT 10, 13 and 1A.
 * Return B_TRUE if any of them are in RAM.
 */

#define	BIOS_IDT_ADDR		((paddr_t)0x0)
#define	BIOS_IDT_LEN		1024
#define	STARTING_ROM_ADDRESS	0xc0000000

static boolean_t
any_bios_routines_in_ram(void)
{
	uint_t *bios_idt;
	int ret = B_FALSE;

	bios_idt = (uint_t *)psm_map_new(BIOS_IDT_ADDR, BIOS_IDT_LEN,
	    PSM_PROT_READ);
	if (bios_idt != NULL) {
		if (bios_idt[0x10] < STARTING_ROM_ADDRESS ||
		    bios_idt[0x13] < STARTING_ROM_ADDRESS ||
		    bios_idt[0x1a] < STARTING_ROM_ADDRESS) {
			ret = B_TRUE;
		}
		psm_unmap((caddr_t)bios_idt, BIOS_IDT_LEN);
	}

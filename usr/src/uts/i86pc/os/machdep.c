...
/*
 * Global flag to indicate if x2apic mode is supported by the platform
 * when the processor supports it.
 */
int	x2apic_enable = 1;
...
mdboot(int cmd, int fcn, char *mdep, boolean_t invoke_cb)
{
	 ...
	 *	with panicking before it's really safe to panic.
...
reset(void)
{
	...
	if (khat_running) {
		 ...
		 *
		 * XXX - The below test may be insufficient on UEFI systems
		 * XXX (the options_dip check may interfere with the UEFI check)
		 */
...
loop:
	...
	now.tv_sec = hrt->hrestime.tv_sec;
	now.tv_nsec = hrt->hrestime.tv_nsec;
	nslt = (int)(gethrtime() - hrt->hres_last_tick);
	if (nslt < 0) {
		/*
...
gethrestime_lasttick(timespec_t *tp)
{
	...
	s = hr_clock_lock();
	tp->tv_sec = hrt->hrestime.tv_sec;
	tp->tv_nsec = hrt->hrestime.tv_nsec;
	hr_clock_unlock(s);
}
...
{
	/*
	 * Notify the fbconsole subsystem that we want to write on the screen no
	 * matter what.
	 */
	force_screen_output = 1;

	/*
	 * Stop performance counters so that we do not get any more overflow
	 * interrupts.
	 */
	kcpc_cpu_stop(CPU, B_FALSE);

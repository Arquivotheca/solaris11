...
#ifdef DEBUG
	/*
	 * all the htables should be gone
	 */
	for (h = 0; h < hat->hat_num_hash; ++h)
		ASSERT(hat->hat_ht_hash[h] == NULL);
#endif
}
...
htable_release(htable_t *ht)
{
	...
	while (ht != NULL) {
		...
		for (;;) {
			...
			HTABLE_EXIT(hashval);
			if (higher == NULL) {
				ASSERT(ht == hat->hat_htable);
#if defined(__xpv)
				/*
				 * unpin top Xen level page table(s)
				 */
				xen_unpin(ht->ht_pfn);
#if defined(__amd64)
	...
	pfn_t pfn)
{
	...
	if (pp->p_vnode == NULL) {
		...
		page_hashin(pp, &kvp, offset, NULL);
	}
	page_downgrade(pp);
#if defined(__xpv)
	/*

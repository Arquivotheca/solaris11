...
/*
 * VM - Hardware Address Translation management for amd64
 *
...
/*
 * Allocate a hat structure for as. We also initialize it to contain
 * the shared kernel hat entries.
 */
hat_t *
hat_alloc(struct as *as)
{
	...
	hat->hat_htable =
	    htable_create(hat, (uintptr_t)0, TOP_LEVEL(hat), NULL);
	...
	for (r = 0; r < num_kernel_ranges; ++r) {
		...
		ASSERT(rp->hkr_end_va <= va + cnt * LEVEL_SIZE(ht->ht_level));
		if (rp->hkr_end_va != va + cnt * LEVEL_SIZE(ht->ht_level))
...
hat_free_end(hat_t *hat)
{
	...
	/*
	 * in 64 bit kernel, 32 bit process don't share anything
	 */
#if defined(__amd64)
	if (hat->hat_flags & HAT_VLP)
		goto skip_ranges;
	...
	for (r = 0; r < num_kernel_ranges; ++r) {
		...
		if (rp->hkr_level == TOP_LEVEL(hat))
			...
			ht = htable_lookup(hat, va, rp->hkr_level);
...
skip_ranges:
	/*
	 * Release of top level table, this may trigger hat_destroy(), unless
	 * a hat_pageunload() is still in progress. In which case, it happens
	 * from there.
	 */
	htable_release(hat->hat_htable);
}
...
hat_destroy(hat_t *hat)
{
	...
	/*
	 * Make a pass through the htable cache freeing it all up.
	 */
...
hat_init_finish(void)
{
	...
	/*
	 * User HAT page tables that cross kernelbase are initialized with
	 * copies of kernel mappings in higher level page tables. Ensure
	 * that those kernel entries exist.
	 */
	...
	/*
	 * Fill in all kernel pagetables that will have entries shared to
	 * user HATs. Note we don't ever htable_release() these mappings,
	 * since they are used to initialize user HATs.
	 */
	for (r = 0; r < num_kernel_ranges; ++r) {
		rp = &kernel_ranges[r];
	 ...
	 * we map from segmap_start .. segmap_start + segmapsize;
	 */
	size = segmapsize;
	...
	uint_t		flags)
{
	...
	while (va < eaddr) {
		...
		/*
		 * Prevent 32-bit (VLP) processes from using pagesizes
		 * greater than 2M (level == 1).
		 * XXX: 32-bit kernel EOL, need to rip-out VLP
		 */
		level = mmu.max_page_level;
		if (hat->hat_flags & HAT_VLP)
			level = MIN(level, 1);
...
handle_ranges(hat_callback_t *cb, uint_t cnt, range_info_t *range)
{
	...
	while (cb != NULL && cnt > 0) {
		...
		cb->hcb_end_addr +=
		    range[cnt].rng_cnt << LEVEL_SHIFT(range[cnt].rng_level);
		cb->hcb_function(cb);
	}
}

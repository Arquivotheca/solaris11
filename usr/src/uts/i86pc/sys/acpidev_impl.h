...
struct acpidev_data_impl {
	...
	/* Fields below are used for DR operations only. */
	acpidev_class_id_t		aod_class_id;	/* Dev type for DR. */
	uint32_t			aod_bduid;	/* _UID of board */
...
#define	ACPIDEV_ODF_DR_CAPABLE		0x100
#define	ACPIDEV_ODF_DR_READY		0x200
#define	ACPIDEV_ODF_DR_FAILED		0x400
#define	ACPIDEV_ODF_OxLINE_CAPABLE	0x800
#define	ACPIDEV_ODF_POWER_CAPABLE	0x1000
#define	ACPIDEV_ODF_HOTPLUG_CAPABLE	0x2000
#define	ACPIDEV_ODF_SPARE_CAPABLE	0x4000
#define	ACPIDEV_ODF_MIGRATION_CAPABLE	0x8000
#define	ACPIDEV_ODF_MIGRATION_MASK	0x30000
#define	ACPIDEV_ODF_MIGRATION_NONE	0x00000
#define	ACPIDEV_ODF_MIGRATION_NONBLOCK	0x10000
#define	ACPIDEV_ODF_MIGRATION_BLOCK	0x20000
#define	ACPIDEV_ODF_MIGRATION_CHILD	0x30000

#define	ACPIDEV_DR_IS_CAPABLE(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_DR_CAPABLE)

#define	ACPIDEV_DR_SET_CAPABLE(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_DR_CAPABLE

#define	ACPIDEV_DR_IS_READY(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_DR_READY)

#define	ACPIDEV_DR_SET_READY(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_DR_READY

#define	ACPIDEV_DR_IS_FAILED(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_DR_FAILED)
...
#define	ACPIDEV_DR_IS_WORKING(hdl)	\
	(((hdl)->aod_iflag & (ACPIDEV_ODF_DR_READY | \
	ACPIDEV_ODF_DR_FAILED)) == ACPIDEV_ODF_DR_READY)

#define	ACPIDEV_DR_IS_PROCESSED(hdl)	\
	((hdl)->aod_iflag & (ACPIDEV_ODF_DR_READY | \
	ACPIDEV_ODF_DR_FAILED | ACPIDEV_ODF_DR_CAPABLE))

#define	ACPIDEV_DR_BOARD_READY(hdl)	\
	(((hdl)->aod_iflag & \
	(ACPIDEV_ODF_DR_READY | ACPIDEV_ODF_DR_CAPABLE)) == \
	(ACPIDEV_ODF_DR_READY | ACPIDEV_ODF_DR_CAPABLE))

#define	ACPIDEV_DR_IS_OxLINE_CAPABLE(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_OxLINE_CAPABLE)

#define	ACPIDEV_DR_SET_OxLINE_CAPABLE(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_OxLINE_CAPABLE

#define	ACPIDEV_DR_IS_POWER_CAPABLE(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_POWER_CAPABLE)

#define	ACPIDEV_DR_SET_POWER_CAPABLE(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_POWER_CAPABLE

#define	ACPIDEV_DR_IS_HOTPLUG_CAPABLE(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_HOTPLUG_CAPABLE)

#define	ACPIDEV_DR_SET_HOTPLUG_CAPABLE(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_HOTPLUG_CAPABLE

#define	ACPIDEV_DR_IS_SPARE_CAPABLE(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_SPARE_CAPABLE)

#define	ACPIDEV_DR_SET_SPARE_CAPABLE(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_SPARE_CAPABLE

#define	ACPIDEV_DR_IS_MIGRATION_CAPABLE(hdl)	\
	((hdl)->aod_iflag & ACPIDEV_ODF_MIGRATION_CAPABLE)

#define	ACPIDEV_DR_SET_MIGRATION_CAPABLE(hdl)	\
	(hdl)->aod_iflag |= ACPIDEV_ODF_MIGRATION_CAPABLE

#define	ACPIDEV_DR_GET_MIGRATION_MODE(hdl) \
	((hdl)->aod_iflag & ACPIDEV_ODF_MIGRATION_MASK)

#define	ACPIDEV_SET_MIGRATION_MODE(hdl, mode) \
	(hdl)->aod_iflag &= ~ACPIDEV_ODF_MIGRATION_MASK; \
	(hdl)->aod_iflag |= ((mode) & ACPIDEV_ODF_MIGRATION_MASK)

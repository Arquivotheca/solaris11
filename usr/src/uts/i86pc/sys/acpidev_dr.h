 ...
 * Note: Lock is held during board level operations and the event handler.
 */
...
/* Initialize the Platform RAS Capabilities related interfaces. */
extern ACPI_STATUS acpidev_ras_init(void);

/* Check the RAS features supported by the device. */
extern ACPI_STATUS acpidev_ras_check_capabilities(ACPI_HANDLE hdl,
    acpidev_data_handle_t dhdl);

/*
 * Trigger a hot-add event for the device.
 * Must be called with the acpidev_dr_lock held.
 * The acpidev_dr_lock may be temporarily released and then reacquired
 * to work around a deadlock case.
 */
extern ACPI_STATUS acpidev_ras_trigger_online_event(ACPI_HANDLE hdl);

/*
 * Trigger a hot-removal event for the device.
 */
extern ACPI_STATUS acpidev_ras_trigger_offline_event(ACPI_HANDLE hdl);

/*
 * Check whether the hotplug event is triggered by software and
 * clear it if so.
 * Must be called with the acpidev_dr_lock held.
 */
extern ACPI_STATUS acpidev_ras_clear_event(ACPI_HANDLE hdl,
    UINT32 event_type);

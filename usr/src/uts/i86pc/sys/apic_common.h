...
extern void apic_intrmap_init(void);
...
extern int apic_check_isr_pending(void);

/* vector type */
#define	APIC_TYPE_FIXED		DDI_INTR_TYPE_FIXED	/* 1 */
#define	APIC_TYPE_MSI		DDI_INTR_TYPE_MSI	/* 2 */
#define	APIC_TYPE_MSIX		DDI_INTR_TYPE_MSIX	/* 4 */
#define	APIC_TYPE_IPI		8

#define	GET_INTR_INUM(ihdlp)		\
	(((ihdlp) != NULL) ? ((ddi_intr_handle_impl_t *)(ihdlp))->ih_inum : 0)

extern char *apic_get_intr_type(short index);
extern char *apic_get_intr_bustype(short bustype);
extern char *apic_get_intr_el(short el);
#ifdef	__cplusplus
}
#endif

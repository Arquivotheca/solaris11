...
typedef struct	isr_info {
	uchar_t		isr_irqno;	/* isr irq */
	uint32_t	isr_cpu;	/* isr cpu id */
	uchar_t		isr_vector;	/* isr vector */
	char		*isr_name;	/* isr name */
	struct isr_info *isr_link;
} isr_info_t;

extern isr_info_t *isr_info_list;
...
typedef struct {
	...
	uint8_t		avgi_apic;	/* apic id - from kernel */
	uint8_t		avgi_index;	/* int no on apic */
	uint8_t		avgi_inum;	/* irq - from kernel */
	uint8_t		avgi_devinum;	/* device intr number - from kernel */
	uchar_t		avgi_share;	/* share num - from kernel */
	uint_t		avgi_pri; 	/* priority - from kernel */
	char		*avgi_state;	/* state - from kernel */
	char		*avgi_type;	/* type - from kernel */
	char		*avgi_bus;	/* bus - from kernel */
	char		*avgi_trg;	/* trg - from kernel */
	char		*avgi_isr;	/* isr name - from kernel */
...
#define	APIC_DEBUG_MSGBUFSIZE	4096
extern int	apic_debug_msgbuf[];
extern int	apic_debug_msgbufindex;
extern int	apic_debug_msgbufpad;
extern int	apic_debug_buf_enable;

/*
 * Put "int" info into debug buffer. No MP consistency, but light weight.
 * Good enough for most debugging.
 * Leave several bytes un-used at the end of the buffer to tolerate
 * buffer overflow.
 */
#define	APIC_DEBUG_BUF_PUT(x) \
	if (apic_debug_buf_enable) { \
		apic_debug_msgbuf[apic_debug_msgbufindex++] = x; \
		if (apic_debug_msgbufindex >= \
		    (APIC_DEBUG_MSGBUFSIZE - apic_debug_msgbufpad)) \

...
static acpidev_dr_capacity_t acpidev_dr_capacities[] = {
	...
	{   /* Westmere-EX */
	    X86_VENDOR_Intel, 0x6, 0x2f, 0x2f, 0, UINT_MAX,
	    B_TRUE,		/* Hotplug capable */
	    1ULL << 30,		/* Align on 1GB boundary */
	},
...
acpidev_dr_init(void)
{
	...
	if (ACPI_FAILURE(rc) && rc != AE_SUPPORT) {
		cmn_err(CE_WARN, "!acpidev: failed to initialize ACPI "
		    "RAS interfaces, disable DR operations.");
		acpidev_dr_failed = 1;
	}
}
...
acpidev_dr_check(acpidev_walk_info_t *infop)
{
	...
	/*
	 * Check the device RAS capabilities.
	 */
	if (!ACPIDEV_DR_IS_CAPABLE(dhdl)) {
		rc = acpidev_ras_check_capabilities(infop->awi_hdl, dhdl);
		if (ACPI_FAILURE(rc) && rc != AE_SUPPORT) {
			ACPIDEV_DEBUG(CE_WARN,
			    "!acpidev: failed to check device RAS "
			    "capabilities, disable DR operations.");
	...
	while (ACPI_SUCCESS(AcpiGetParent(phdl, &phdl)) &&
		...
		if (pdhdl != NULL && ACPIDEV_DR_IS_CAPABLE(pdhdl)) {
...
acpidev_dr_create_node(dev_info_t *pdip)
{
	...
	cmn_err(CE_NOTE,
	    "?acpidev: the device node for Dynamic Reconfiguration is ready.");
...
acpidev_dr_find_board(ACPI_HANDLE hdl, uint_t lvl, void *ctx, void **retval)
{
	...
	} else if (ACPIDEV_DR_IS_CAPABLE(dhdl) && ACPIDEV_DR_IS_WORKING(dhdl) &&
...
/*
 * On Intel Stoutland and Quanta QSSC-S4R platforms, the platform can't
 * detect the presence of CPU and memory boards. To satisify the status
 * check in the DR driver, assume all boards are always present.
 */
static int
acpidev_dr_device_is_present_workaround(void)
{
	ACPI_TABLE_HEADER *hp;
	...
	/*
	 * Detect affected platforms by checking the ACPI MSCT table
	 * signature.
	 */
	if (ACPI_FAILURE(AcpiGetTable(ACPI_SIG_MSCT, 1, &hp))) {
		fake_present = 0;
	} else if (strncmp(hp->OemId, "INTEL ", ACPI_OEM_ID_SIZE) == 0 &&
	    strncmp(hp->OemTableId, "STOUTLAN", ACPI_OEM_TABLE_ID_SIZE) == 0) {
		fake_present = 1;
	} else if (strncmp(hp->OemId, "QUANTA", ACPI_OEM_ID_SIZE) == 0 &&
	    strncmp(hp->OemTableId, "QSSC-S4R", ACPI_OEM_TABLE_ID_SIZE) == 0) {
		fake_present = 1;
	} else {
		fake_present = 0;
	}
...
acpidev_dr_device_is_present(ACPI_HANDLE hdl)
{
	...
	if (acpidev_dr_device_is_present_workaround()) {
		return (1);
	}
...
acpidev_dr_device_get_memory_index(ACPI_HANDLE hdl, uint32_t *idxp)
{
	...
	if (dhdl == NULL) {
		...
		ASSERT(ACPIDEV_DR_IS_CAPABLE(dhdl));
		*idxp = dhdl->aod_bduid;
	}
...
acpidev_dr_device_is_board(ACPI_HANDLE hdl)
{
	...
	if (dhdl == NULL) {
		return (0);
	} else if (!ACPIDEV_DR_IS_CAPABLE(dhdl)) {
		return (0);
	}
    ...
    ACPI_WALK_CALLBACK cb, void *arg, void **retval)
{
	...
	for (i = 0; i < obj->Package.Count; i++) {
		if (obj->Package.Elements[i].Type !=
			...
			ACPIDEV_DEBUG(CE_WARN,
			    "!acpidev: element %d in package returned by %s "
			    "of %s is not local reference.",
			    i, method, objname);
	...
	for (i = 0; i < obj->Package.Count; i++) {
		if (obj->Package.Elements[i].Reference.Handle == NULL) {
			ACPIDEV_DEBUG(CE_WARN,
			    "!acpidev: handle of element %d in package "
			    "returned by %s of %s is NULL.",
			    i, method, objname);
			continue;
		}
    ...
    ACPI_WALK_CALLBACK cb, void *arg, void **retval)
{
	...
	if (ACPI_FAILURE(AcpiGetHandle(NULL, obj->String.Pointer, &chdl))) {
		ACPIDEV_DEBUG(CE_WARN, "!acpidev: failed to get handle for %s.",
		    obj->String.Pointer);
		rc = AE_ERROR;
	} else {
...
acpidev_dr_scan_topo(ACPI_HANDLE hdl, UINT32 lvl, void *arg, void **retval)
{
	...
	if (lvl != 0) {
		 ...
		 */
		if (ACPI_SUCCESS(acpidev_ras_check_capabilities(hdl, NULL))) {
			acpidev_free_object_name(objname);
			return (AE_CTRL_DEPTH);
		}
		...
		if (acpidev_dr_device_has_edl(hdl)) {
			ACPIDEV_DEBUG(CE_NOTE,
			    "!acpidev: non-hotplug-capable object %s has _EDL "
			    "method, disable DR operations.", objname);
			acpidev_free_object_name(objname);
			acpidev_dr_failed = 1;
			return (AE_SUPPORT);
		}
...
acpidev_dr_device_poweron(ACPI_HANDLE hdl)
{
	...
	/* Power on the device if it hasn't been powered on yet. */
	if (!acpidev_check_device_enabled(acpidev_query_device_status(hdl))) {
		rc = acpidev_ras_trigger_online_event(hdl);
		if (ACPI_FAILURE(rc)) {
			return (rc);
		}
		if (!acpidev_check_device_enabled(
		    acpidev_query_device_status(hdl))) {
			cmn_err(CE_WARN,
			    "!acpidev: failed to online device %p.", hdl);
			return (AE_ERROR);
		}
	}

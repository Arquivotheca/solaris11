 ...
 *
 * Despite what the ACPI spec says, there are systems whose X2APICs have been
 * configured with X2APIC IDs lower than 256.  To support such systems, we DO
 * allow X2APIC IDs lower than 256.
 */
...
acpidev_cpu_init(acpidev_walk_info_t *infop)
{
	...
	if (infop->awi_op_type == ACPIDEV_OP_BOOT_PROBE ||
		...
		if (ACPI_FAILURE(acpica_add_processor_to_map(
			...
			ACPIDEV_DEBUG(CE_WARN, "!acpidev: failed to bind "
			    "processor id/object handle for %s.",
			    infop->awi_name);
			return (AE_ERROR);
		}
	} else {

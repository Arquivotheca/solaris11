    ...
    void **retval)
{
	...
	if (ACPI_FAILURE(rc)) {
		ACPIDEV_DEBUG(CE_WARN, "!acpidev: failed to get ACPI "
		    "object info in acpidev_get_device_callback().");
    ...
    acpidev_class_list_t **listpp, acpidev_walk_info_t *pinfop)
{
	...
	if (ACPI_FAILURE(AcpiGetObjectInfo(hdl, &infop->awi_info))) {
		ACPIDEV_DEBUG(CE_WARN, "!acpidev: failed to get object info "
		    "for %s in acpidev_alloc_walk_info().", infop->awi_name);
...
{
	ASSERT(hdl != NULL);
	return (ACPIDEV_DR_IS_CAPABLE(hdl));
}
...
{
	ASSERT(hdl != NULL);
	return (ACPIDEV_DR_IS_READY(hdl));
}
...
{
	ASSERT(hdl != NULL);
	return (ACPIDEV_DR_IS_FAILED(hdl));
}
    ...
    char *unitaddr)
{
	...
	if (unitaddr == NULL && (infop->awi_info->Valid & ACPI_VALID_UID)) {
		...
		if (unitaddr == NULL) {
			ACPIDEV_DEBUG(CE_WARN, "!acpidev: failed to generate "
			    "unit address from %s.",
			    infop->awi_info->UniqueId.String);
			return (AE_ERROR);
		}

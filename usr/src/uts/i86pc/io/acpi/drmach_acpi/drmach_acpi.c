...
drmach_board_status(drmachid_t id, drmach_status_t *stat)
{
	...
	switch (acpidev_dr_get_board_type(hdl)) {
...
drmach_init(void)
{
	...
	if (!acpidev_dr_capable()) {
		return (ENOTSUP);
	} else if (MAX_BOARDS > SHRT_MAX) {
...
drmach_board_poweron(drmachid_t id)
{
	...
	if (err == NULL) {
		...
		acpidev_dr_lock_all();
		rc = acpidev_dr_device_poweron(hdl);
		if (rc == AE_SUPPORT) {
			err = drerr_new(0, EX86_SUPPORT, NULL);
	...
	*ip = X86_CPU_IMPL_UNKNOWN;
	...
	if (cpuid_getvendor(CPU) == X86_VENDOR_Intel) {
		...
		if (cpuid_getfamily(CPU) == 0x6) {
			if (cpuid_getmodel(CPU) == 0x2e) {
				*ip = X86_CPU_IMPL_NEHALEM_EX;
			} else if (cpuid_getmodel(CPU) == 0x2f) {
				*ip = X86_CPU_IMPL_WESTMERE_EX;
			}
		}
	}
	kpreempt_enable();

...
_init(void)
{
	...
	if ((error = mod_install(&modlinkage)) != 0) {
		ACPINEX_DEBUG(CE_WARN, "!acpinex: failed to install module.");
...
acpinex_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	if (instance >= ACPINEX_INSTANCE_MAX) {
		ACPINEX_DEBUG(CE_WARN, "!acpinex: instance number %d is out "
		    "of range in acpinex_attach(), max %d.",
	...
	if (ddi_soft_state_zalloc(acpinex_softstates, instance)
	    != DDI_SUCCESS) {
		ACPINEX_DEBUG(CE_WARN, "!acpinex: failed to allocate soft "
		    "state object in acpinex_attach().");
		return (DDI_FAILURE);
	}
	...
	/*
	 * Install ACPI event handler for all hot plug capable devices
	 * if the current device (devi) is the ACPI root node.
	 */
	if (devi == acpidev_root_node() &&
	    acpinex_event_install_handler(softsp) != DDI_SUCCESS) {
...
acpinex_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	...
	if (instance >= ACPINEX_INSTANCE_MAX) {
		ACPINEX_DEBUG(CE_WARN, "!acpinex: instance number %d is out "
		    "of range in acpinex_detach(), max %d.",
		    instance, ACPINEX_INSTANCE_MAX - 1);
		return (DDI_FAILURE);
	}
	...
	case DDI_DETACH:
		/*
		 * Uninstall ACPI event handler for all hot plug capable
		 * devices if the current device (devi) is the ACPI root node.
		 */
		if (devi == acpidev_root_node() &&
		    acpinex_event_uninstall_handler(softsp) != DDI_SUCCESS) {
			cmn_err(CE_WARN, "!acpinex: failed to uninstall event "
			    "handler for hot plug capable devices");
			return (DDI_FAILURE);
		}
...
{
	ACPINEX_DEBUG(CE_WARN,
	    "!acpinex: calling unimplemented function acpinex_bus_map().");

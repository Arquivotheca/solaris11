...
/*
 * FIPE:  FBDIMM Idle Power Enhancement
 * This driver provides a way for Solaris to put FBDIMM2 controllers that
 * that support the FIPE interface (currently Intel 5000 and 7300 chipsets)
 * into a lower power state when the CPU is in an idle mode (See CPU Idle
 * Notification - PSARC/2009/115).
 * This driver communicates with the controllers via IOAT (either by
 * common driver or inline IOAT code), so requires that IOAT be available
 * and enabled by the hardware.
 */

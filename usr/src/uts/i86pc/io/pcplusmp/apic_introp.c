...
extern char *apic_get_intr_type(short);
...
apic_get_vector_intr_info(int vecirq, apic_get_intr_t *intr_params_p)
{
	...
	if (intr_params_p->avgi_num_devs == 0 && irq_p->airq_ipl == 11)
		intr_params_p->avgi_num_devs = 1;

	intr_params_p->avgi_type =
	    apic_get_intr_type(irq_p->airq_mps_intr_index);
	intr_params_p->avgi_share = intr_params_p->avgi_num_devs;
	intr_params_p->avgi_pri = irq_p->airq_ipl;

	/* These 2 fields aren't supported by pcplusmp platform */
	intr_params_p->avgi_isr = NULL;
	intr_params_p->avgi_state = NULL;

	if (irq_p != NULL) {
		intr_params_p->avgi_bus =
		    apic_get_intr_bustype(irq_p->airq_iflag.bustype);
		intr_params_p->avgi_trg =
		    apic_get_intr_el(irq_p->airq_rdt_entry);
		intr_params_p->avgi_apic = irq_p->airq_ioapicindex;
		intr_params_p->avgi_index = irq_p->airq_intin_no;
		intr_params_p->avgi_inum = irq_p->airq_intin_no;
	} else {
		/* assign statically value to IPI and MSI/MSI-X */
		if (irq_p->airq_mps_intr_index == RESERVE_INDEX)
			intr_params_p->avgi_bus = "none";
		else
			...
			for (i = 0, av_dev = autovect[irqno].avh_link;
				...
				if (av_dev->av_vector && av_dev->av_dip) {
					...
					intr_params_p->avgi_devinum =
					    GET_INTR_INUM(av_dev->av_intr_id);
				}
		}
	}

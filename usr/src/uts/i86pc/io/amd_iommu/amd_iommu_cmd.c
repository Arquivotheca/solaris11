...
static int
create_compl_wait_cmd(amd_iommu_t *iommu, amd_iommu_cmd_flags_t flags,
    uint32_t *cmdptr)
{
	if (flags & AMD_IOMMU_CMD_FLAGS_COMPL_WAIT_S) {
		cmn_err(CE_WARN, "%s: %s%d: idx=%d: 'store' completion "
		    ...
		    __func__, ddi_driver_name(iommu->aiomt_iommu.iommu_dip),
		    ddi_get_instance(iommu->aiomt_iommu.iommu_dip),
		    iommu->aiomt_idx);
		return (DDI_FAILURE);
	}
    ...
    amd_iommu_cmd_flags_t flags, uint32_t *cmdptr)
{
	...
	if (flags != AMD_IOMMU_CMD_FLAGS_NONE) {
		cmn_err(CE_WARN, "%s: %s%d: idx=%d: invalidate devtab entry "
		    "no flags supported", __func__,
		    ddi_driver_name(iommu->aiomt_iommu.iommu_dip),
		    ddi_get_instance(iommu->aiomt_iommu.iommu_dip),
		    iommu->aiomt_idx);
		return (DDI_FAILURE);
	}
    ...
    amd_iommu_cmdargs_t *cmdargs, amd_iommu_cmd_flags_t flags, int lock_held)
{
	...
	case AMD_IOMMU_CMD_COMPL_WAIT:
		...
		error = create_compl_wait_cmd(iommu, flags, cmdptr);
		break;

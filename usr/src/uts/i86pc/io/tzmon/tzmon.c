...
/*
 * Maximum number of retries before a thermal zone monitor will stop trying to
 * evaluate the ACPI _TMP object.
 */
#define	TZ_MAX_RETRIES		3
...
tzmon_eval_zone(thermal_zone_t *tzp)
{
	...
	if (tmp < 0) {
		/* on failure, check if retry count is exceeded */
		tzp->retry_count++;
		if (tzp->retry_count >= TZ_MAX_RETRIES) {
			cmn_err(CE_WARN,
			    "tzmon: Thermal zone (%s) _TMP retry count "
			    "(%d) exceeded\n",
			    (char *)tzp->zone_name, TZ_MAX_RETRIES);

			/* Cause monitor for this thermal zone to exit */
			tzp->polling_period = 0;
			mutex_exit(&tzp->lock);

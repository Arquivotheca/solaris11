...
extern int acpi_enter_sleepstate(sxa_t *);
extern int acpi_exit_sleepstate(sxa_t *);
...
static void
appm_init_syscap(void) {
	uint8_t		state;
	pm_capability_t	*pmcp;

	for (state = 1; state < SYSTEM_POWER_STATE_CNT; state++) {
		uint8_t	sa, sb;
		if (AcpiGetSleepTypeData(state, &sa, &sb) == AE_OK) {
			/*
			 * Special consideration for S4 state.  Since
			 * it is currently unsupported due to various
			 * hardware bugs, we don't want to allow this
			 * state for general consumption.  However,
			 * for development debugging, we want this to
			 * actually work.  So if if pm_cpr_state_override
			 * is set, then apply this configuration to *all*
			 * states, otherwise, dissallow S4.
			 */
			if ((state == SYSTEM_POWER_S4) &&
			    !pm_cpr_state_override)
				continue;

			/*
			 * If there is already an element for this state,
			 * use it, otherwise alloc a new one.
			 */
			if (pm_syscap[state] == NULL) {
				pm_syscap[state] =
				    (pm_capability_t *)kmem_zalloc(
				    sizeof (pm_capability_t), KM_SLEEP);
			} else {
				ASSERT(pm_syscap[state]->cp_state == state);
			}
			pmcp = pm_syscap[state];
			pmcp->cp_state = state;
			pmcp->cp_flags = pmcp->cp_capacity = 0;
			pmcp->cp_ttmr = pmcp->cp_ttfc = (uint32_t)-1;
			pmcp->cp_power = (uint8_t)-1;
		}
	}
	/*
	 * Again, special consideration for "S4".  Unless there is
	 * an override, make sure that we disallow saving the state.
	 */
	if (!pm_cpr_state_override)
		pm_cpr_save_state = 0;
...
appm_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	/*
	 * Init pm_syscap array
	 * Note that this driver will fail detach, so there is no
	 * "free" routine.  If the assumption for this driver changes,
	 * so should the need to remove the elements this driver added.
	 */
	appm_init_syscap();
 ...
 */
#define	APPMIOC			('A' << 8)
#define	APPMIOC_ENTER_SUSPEND	(APPMIOC | 1)	/* arg *sxa_t */
#define	APPMIOC_EXIT_SUSPEND	(APPMIOC | 2)	/* arg *sxa_t */
    ...
    cred_t *cred_p, int *rval_p)
{
	...
	switch (cmd) {
	case APPMIOC_ENTER_SUSPEND:
		/*
		 * Suspend
		 */
		PMD(PMD_SX, ("%s: cmd %x, arg %p\n", str, cmd, (void *)arg))
		acpica_use_safe_delay = 1;
		ret = acpi_enter_sleepstate(sxap);
		break;

	case APPMIOC_EXIT_SUSPEND:
		/*
		 * return from suspend
		 */
		PMD(PMD_SX, ("%s: cmd %x, arg %p\n", str, cmd, (void *)arg))
		ret = acpi_exit_sleepstate(sxap);

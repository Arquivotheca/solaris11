...
static void
pcitool_get_intr_dev_info(dev_info_t *dip, pcitool_intr_dev_t *dev,
    apic_get_intr_t intr_info)
{

	(void) strlcpy(dev->driver_name, ddi_driver_name(dip), MAXMODCONFNAME);
	(void) ddi_pathname(dip, dev->path);
	(void) strlcpy(dev->type, intr_info.avgi_type, MODMAXNAMELEN);
	dev->dev_inst = ddi_get_instance(dip);
	dev->inum = intr_info.avgi_devinum;
	dev->pri = intr_info.avgi_pri;
}
...
pcitool_get_intr(dev_info_t *dip, void *arg, int mode)
{
	...
	/*
	 * Check for special request for I/O balancer
	 */
	if ((mode & FKIOCTL) != 0 &&
	    (partial_iget.flags & PCITOOL_INTR_FLAG_BALANCER) != 0 &&
	    strcmp(type_info.avgi_type, APIC_APIX_NAME) == 0) {
		intr_info.avgi_dip_list = NULL;
		intr_info.avgi_req_flags =
		    PSMGI_REQ_CPUID | PSMGI_REQ_VECTOR | PSMGI_INTRBY_DEFAULT;
		thdl = ((uint64_t)partial_iget.msi << 32) |
		    (uint64_t)partial_iget.sysino;
		hdlp = (ddi_intr_handle_impl_t *)thdl;
		savep = hdlp->ih_private;
		hdlp->ih_private = &intr_info;
		(*psm_intr_ops)(NULL, hdlp, PSM_INTR_OP_GET_INTR, NULL);
		rval = (*psm_intr_ops)(NULL, hdlp, PSM_INTR_OP_GET_INTR, NULL);
		...
		iget->ino = intr_info.avgi_vector;
		goto done_get_intr;
	}

	bzero(iget, PCITOOL_IGET_SIZE(num_devs_ret));
	iget->ino = info_hdl.ih_vector & APIX_VIRTVEC_VECMASK;
	...
	if (intr_info.avgi_req_flags & PSMGI_REQ_GET_DEVS) {
		...
		iget->share = intr_info.avgi_share;
		iget->irq = intr_info.avgi_inum;
		iget->sysino = intr_info.avgi_apic;

		/*
		 * For intr which has devices, walk thru list of dips and fill
		 * in the pcitool_intr_dev_t's with extracted dev info.
		 * For intr which has no device, fill in the
		 * pcitool_intr_dev_t's  with isr name.
		 */
		if (intr_info.avgi_dip_list) {
			for (i = 0; i < iget->num_devs_ret; i++) {
				pcitool_get_intr_dev_info(
				    intr_info.avgi_dip_list[i],
				    &iget->dev[i], intr_info);
			}
			kmem_free(intr_info.avgi_dip_list,
			    intr_info.avgi_num_devs * sizeof (dev_info_t *));
		} else if (intr_info.avgi_isr) {
			(void) strlcpy(iget->dev[0].driver_name,
			    intr_info.avgi_isr, MAXMODCONFNAME);
			iget->dev[0].path[0] = '\0';
			iget->dev[0].dev_inst = 0;
			iget->dev[0].inum = intr_info.avgi_devinum;
			iget->dev[0].pri = intr_info.avgi_pri;

			if (intr_info.avgi_type) {
				(void) strlcpy(iget->dev[0].type,
				    intr_info.avgi_type, MODMAXNAMELEN);
			}
		}
	}
...
#ifdef PCIE
static int
pcitool_set_cto(dev_info_t *dip, pcitool_reg_t *prg_p)
{
	dev_info_t	*cdip;
	int		rval, size;
	...
	if (!(cdip = pcie_find_dip_by_bdf(dip,
	    PCI_GETBDF(prg_p->bus_no, prg_p->dev_no, prg_p->func_no)))) {
		prg_p->status = PCITOOL_INVALID_ARG;
		return (EINVAL);
	}

	size = PCITOOL_ACC_ATTR_SIZE(prg_p->acc_attr);
	big_endian = PCITOOL_ACC_IS_BIG_ENDIAN(prg_p->acc_attr);

	local_data = big_endian ? pcitool_swap_endian(prg_p->data, size) :
	    prg_p->data;

	rval = pcie_set_completion_timeout(cdip, (uint16_t)local_data);

	switch (rval) {
	case PCIE_ERR_INVALID_ARG:
		prg_p->status = PCITOOL_INVALID_ARG;
		rval = EINVAL;
		break;
	case PCIE_ERR_OPERATION_NOTSUP:
		prg_p->status = PCITOOL_OPERATION_NOTSUP;
		rval = ENOTSUP;
		break;
	case SUCCESS:
...
static int
pcitool_retrain_link(dev_info_t *dip, pcitool_reg_t *prg_p,
    boolean_t equalization)
{
	dev_info_t	*cdip;
	pcie_req_id_t   bdf = PCI_GETBDF(prg_p->bus_no, prg_p->dev_no,
	    prg_p->func_no);
	int	rval;

	/* Check if bdf is correct */
	if (pci_cfgacc_get32(dip, bdf, PCI_CONF_VENID) == (uint32_t)-1) {
		prg_p->status = PCITOOL_INVALID_ARG;
		return (EINVAL);
	}

	/* dip has been removed by cfgadm/hotplug command */
	if (!(cdip = pcie_find_dip_by_bdf(dip, bdf))) {
		/* Get parent's dip */
		if (!(cdip = pcie_find_parent_by_busno(dip, prg_p->bus_no))) {
			prg_p->status = PCITOOL_INVALID_ARG;
			return (EINVAL);
		}
	}

	rval = pcie_link_retrain(cdip, (uint16_t)prg_p->data, equalization);
	...
	switch (rval) {
		case PCIE_ERR_INVALID_ARG:
			...
			rval = EINVAL;
			break;
		case PCIE_ERR_DEV_BUSY:
			prg_p->status = PCITOOL_DEVICE_BUSY;
			rval = EAGAIN;
			break;
		case PCIE_ERR_OPERATION_NOTSUP:
			prg_p->status = PCITOOL_OPERATION_NOTSUP;
			rval = ENOTSUP;
			break;
		case PCIE_ERR_LINK_OPERATION_FAIL:
			prg_p->status = PCITOOL_LINK_OPERATION_FAIL;
			rval = EIO;
			break;
		case PCIE_ERR_LINK_DOWN:
			prg_p->status = PCITOOL_LINK_DOWN;
			rval = EIO;
			break;
		case SUCCESS:
			prg_p->status = PCITOOL_SUCCESS;
...
#ifdef PCIE
		if (cmd == PCITOOL_DEVICE_LINK_RETRAIN) {
			rval = pcitool_retrain_link(dip, &prg, B_FALSE);
			goto done_reg;
		} else if (cmd == PCITOOL_DEVICE_LINK_EQUALIZATION) {
			rval = pcitool_retrain_link(dip, &prg, B_TRUE);
			goto done_reg;
		} else if (cmd == PCITOOL_DEVICE_SET_CTO) {
			rval = pcitool_set_cto(dip, &prg);
			goto done_reg;
		}
#endif /* PCIE */

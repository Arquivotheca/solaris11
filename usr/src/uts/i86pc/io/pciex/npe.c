...
npe_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	pcie_rc_init_bus(devi, PCIE_BUS_FINAL);
    ...
    off_t offset, off_t len, caddr_t *vaddrp)
{
	...
	if ((mp->map_op == DDI_MO_UNMAP) || (mp->map_op == DDI_MO_UNLOCK)) {
		...
		reg.regspec_addr_high = pci_rp->pci_phys_mid;
		reg.regspec_addr_low = pci_rp->pci_phys_low;
		reg.regspec_size_high = pci_rp->pci_size_hi;
		reg.regspec_size_low = pci_rp->pci_size_low;
	...
	ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	case DDI_CTLOPS_ATTACH:
		...
		/*
		 * Always call postattach even if result is NOT DDI_SUCCESS.
		 * The pcie_postattach_child will check the result and do the
		 * needful.
		 */
		if (asp->when == DDI_POST) {
			if (asp->result == DDI_SUCCESS)
				pf_init(rdip, (void *)pci_p->pci_fm_ibc,
				    asp->cmd);
			(void) pcie_postattach_child(rdip, asp);
		}
		...
		if (asp->when == DDI_PRE) {
			return (pcie_preattach_child(rdip, asp));
		}
	...
	case DDI_CTLOPS_DETACH:
		...
		if (dsp->when == DDI_PRE) {
			if (pcie_predetach_child(rdip, dsp) != DDI_SUCCESS)
				return (DDI_FAILURE);
			pf_fini(rdip, dsp->cmd);
		}
		if (dsp->when == DDI_POST)
			(void) pcie_postdetach_child(rdip, dsp);

		/* only do this for immediate children */
		if (dsp->cmd == DDI_SUSPEND && dsp->when == DDI_POST &&
		    dsp->result == DDI_SUCCESS &&
		    ddi_get_parent(rdip) == dip)
...
npe_initchild(dev_info_t *child)
{
	...
	iommu_setup(child, IOMMU_SETUP_PCIEX_ROOT);

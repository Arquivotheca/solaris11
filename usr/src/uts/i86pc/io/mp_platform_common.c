...
apic_probe_common(char *modname)
{
	...
	/*
	 * If system firmware handed off the system with X2APIC enabled,
	 * switch the PSM's state over to use X2APIC ops.
	 */
	if (apic_local_mode() == LOCAL_X2APIC) {
		/* FW apparently has enabled X2APIC */
	...
	/*
	 * Avoid the BDA inspection on UEFI systems.
	 */
...
acpi_probe(char *modname)
{
	...
	while (madt_seen < madt_size) {
		...
		case ACPI_MADT_TYPE_LOCAL_X2APIC:
			...
			/*
			 * Though the ACPI spec states that all logical
			 * processors with APIC ID values of 255 and greater
			 * will have their APIC reported through Processor
			 * X2APIC structure, there are systems that do not
			 * abide by this limitation.  To accommodate them,
			 * we allow X2APIC entries whose X2APIC IDs are <256.
			 */
	...
	if (apic_acpi_enter_apicmode() != PSM_FAILURE) {
		 ...
		 * in apic_picinit(), if cpu supports deep C-states and
		 * the local apic timer is not C-state invariant.
		 */
		if (cpuid_deep_cstates_supported() && !cpuid_arat_supported()) {
			if (hpet_acpi_init(&apic_hpet_vect,
			    &apic_hpet_flags) < 0) {
				cmn_err(CE_NOTE,
			...
			struct apic_io_intr **intrp)
{
	...
	while (1) {
		...
		if ((pci_config_get8(cfg_handle, PCI_CONF_BASCLASS) ==
			...
			if (acpica_get_initial_bdf(dipp, &bridge_bus,
			    &bridge_devno, NULL) != 0)
				return (-1);
			/*

...
static char *apix_get_known_vector(int cpuid, int vector);
...
extern void apix_insert_isr(uchar_t, uint32_t, uchar_t, char *);
extern char *apic_get_intr_bustype(short);
extern char *apic_get_intr_el(short);
static char *apix_get_intr_type(short type);
 ...
 */
#define	APIX_SUPPORT_X2APIC		0x00000001 /* x2APIC */
#define	APIX_SUPPORT_WESTMERE_EP	0x00000002 /* Westmer-EP processor */

uint_t apix_supported_hw = APIX_SUPPORT_X2APIC | APIX_SUPPORT_WESTMERE_EP;
...
#ifdef DEBUG
uint32_t apix_wait_rirr_timeout = 0;
#endif
...
apix_probe()
{
	...
	/* Enable apix only on native hardware */
	...
	if (apix_hw_chk_enable) {
		...
		if (apix_supported_hw & APIX_SUPPORT_X2APIC) {
			if ((apic_local_mode() == LOCAL_X2APIC) ||
			    cpuid_x2apic_supported()) {
				/* x2APIC mode is supported in the hw */
				apix_enable = 1;
			}
		}
		if (apix_supported_hw & APIX_SUPPORT_WESTMERE_EP) {
			if (cpuid_getvendor(CPU) == X86_VENDOR_Intel &&
			    cpuid_getfamily(CPU) == 0x6 &&
			    cpuid_getmodel(CPU) == 0x2c) {
				/* Westmere-EP */
				apix_enable = 1;
			}
		}
...
apix_init()
{
	...
	psm_check_isr_pending = apic_check_isr_pending;
...
static char *
apix_get_intr_state(short state)
{
	char *intr_state = NULL;
	switch (state) {
	case APIX_STATE_FREED:
		intr_state = "free";
		break;
	case APIX_STATE_OBSOLETED:
		intr_state = "obso";
		break;
	case APIX_STATE_ALLOCED:
		intr_state = "alloc";
		break;
	case APIX_STATE_ENABLED:
		intr_state = "enbl";
		break;
	case APIX_STATE_DISABLED:
		intr_state = "disbl";
	}
	return (intr_state);
}
...
static char *
apix_get_intr_type(short type)
{
	char *intr_type = NULL;
	switch (type) {
	case APIC_TYPE_FIXED:
		intr_type = "Fixed";
		break;
	case APIC_TYPE_MSI:
		intr_type = "MSI";
...
static char *
apix_get_known_vector(int cpuid, int vector)
{
	isr_info_t *p;

	/* find where it goes in list */
	for (p = isr_info_list; p != NULL; p = p->isr_link) {
		if ((cpuid == p->isr_cpu && vector == p->isr_vector) ||
		    (0xff == p->isr_cpu && vector == p->isr_vector))
			break;
	}

	return ((p == NULL) ? NULL : p->isr_name);
}
...
apix_get_ipivect(int ipl, int type)
{
	...
	if ((vector = apix_alloc_ipi(ipl)) > 0) {
		...
		if (ipl == XC_CPUPOKE_PIL)
			apix_insert_isr(0xff, 0xff, vector, "poke_cpu");
		return (vector);
	}
    ...
    psm_intr_op_t intr_op, int *result)
{
	...
	case PSM_INTR_OP_GET_SHARED:
		...
		*result = (apix_intx_get_shared(vecp->v_inum) > 1) ?
		    B_TRUE : B_FALSE;
		break;
...
apix_get_pending(apix_vector_t *vecp)
{
	...
	irr = apic_reg_ops->apic_read(APIC_IRR_REG + (index * 4));
    ...
    apic_get_intr_t *intr_params_p)
{
	...
	/* assign avgi_num_devs to 1 for special isr "poke_cpu" */
	if (intr_params_p->avgi_num_devs == 0 && vecp->v_pri == 11)
		intr_params_p->avgi_num_devs = 1;

	intr_params_p->avgi_type = apix_get_intr_type(vecp->v_type);
	intr_params_p->avgi_state = apix_get_intr_state(vecp->v_state);
	intr_params_p->avgi_inum = vecp->v_inum;
	intr_params_p->avgi_share = vecp->v_share;
	intr_params_p->avgi_pri = vecp->v_pri;
	intr_params_p->avgi_isr =
	    apix_get_known_vector(vecp->v_cpuid, vecp->v_vector);

	if (vecp->v_type == APIC_TYPE_FIXED) {
		if ((irqp = apic_irq_table[vecp->v_inum]) != NULL) {
			intr_params_p->avgi_bus =
			    apic_get_intr_bustype(irqp->airq_iflag.bustype);
			intr_params_p->avgi_trg =
			    apic_get_intr_el(irqp->airq_iflag.intr_el);
			intr_params_p->avgi_apic = irqp->airq_ioapicindex;
			intr_params_p->avgi_index = irqp->airq_intin_no;
		}
	} else {
		/* assign statically value to IPI and MSI/MSI-X */
		if (vecp->v_type == APIX_TYPE_IPI)
			intr_params_p->avgi_bus = NULL;
		else
			intr_params_p->avgi_bus = "PCI";

		intr_params_p->avgi_trg = "Edg";
		intr_params_p->avgi_apic = 0;
		intr_params_p->avgi_index = 0;
	}
...
void
apix_intx_set_vector(int irqno, uint32_t cpuid, uchar_t vector, char *name)
{
	apic_irq_t *irqp;
	...
	apix_insert_isr(irqno, cpuid, vector, name);
...
apix_intx_rebind(int irqno, processorid_t cpuid, uchar_t vector)
{
	...
	lock_clear(&apic_ioapic_lock);
	intr_restore(iflag);
	apix_intx_set_vector(irqno, cpuid, vector, NULL);
	iflag = intr_clear();
	lock_set(&apic_ioapic_lock);
...
apix_level_intr_pre_eoi(int irq)
{
	...
	if (apix_mul_ioapic_method == APIC_MUL_IOAPIC_IOXAPIC) {
		...
		/*
		 * Wait for Remote IRR to be set
		 */
		for (waited = 0; waited < apic_max_reps_clear_pending;
		    waited++) {
			if (GET_RDT_BITS(apic_ix, intin_ix, AV_REMOTE_IRR) == 1)
				break;
		}
#ifdef DEBUG
		if (waited == apic_max_reps_clear_pending)
			apix_wait_rirr_timeout++;
#endif
    ...
    struct intrspec *ispec)
{
	...
	if (acpica_get_initial_bdf(dip, &busid, &devid, NULL) != 0)
		return (-1);
...
apix_intx_alloc_vector(dev_info_t *dip, int inum, struct intrspec *ispec)
{
	...
	if ((vecp = apix_alloc_intx(dip, inum, irqno, NULL)) == NULL)
		return (0);

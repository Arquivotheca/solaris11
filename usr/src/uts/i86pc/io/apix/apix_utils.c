...
extern void apix_insert_isr(uchar_t irqno, uint32_t cpuid, uchar_t vector,
    char *name);
...
char		*apic_isr_table[APIC_MAX_VECTOR+1];
    ...
    caddr_t arg1, caddr_t arg2)
{
	...
	apix_insert_isr(0, 0xff, vector, name);
    ...
    int inum, int count, uchar_t vector, int target_apic_id)
{
	...
	if (type == APIX_TYPE_MSI) {
		...
		/*
		 * If MSI vector is not maskable and interrupt remapping
		 * is not enabled then use APIX_RESV_VECTOR to avoid
		 * missing the device interrupt if it occurs during the
		 * small window between writing to the MSI address
		 * register and MSI data register below.
		 *
		 * NOTE: When interrupt remapping is enabled (at least
		 * with Intel vt-d case), the values programmed into
		 * MSI registers (address and data) do not change during
...
/* ARGSUSED */
static int
apix_check_irr(apix_vector_t *vecp, xc_arg_t unused2, xc_arg_t unused3)
{
	int bit, index, irr;
	...
	vecp->v_irr_pending = (irr & (1 << bit)) ? 1 : 0;

	return (vecp->v_irr_pending);
}
 ...
 * being serviced.
 * This function is called when a new vector is obsoleted or
 * to process a previously obsoleted vector.
 *
...
apix_obsolete_vector(apix_vector_t *vecp)
{
	...
	for (avp = vecp->v_autovect; avp != NULL; avp = avp->av_link) {
		...
		/*
		 * If the source vector is on the local CPU with interrupt
		 * disabled, check local APIC IRR register for cached
		 * interrupt request.
		 * Otherwise, use cross calls to check the IRR register
		 * on source CPU.
		 */

		if (LOCAL_WITH_INTR_DISABLED(cpuid)) {
			(void) apix_check_irr(vecp, 0, 0);
		} else {
			CPUSET_ONLY(cpu_set, cpuid);
			xc_call((xc_arg_t)vecp, 0, 0, CPUSET2BV(cpu_set),
			    (xc_func_t)apix_check_irr);
		}

		if (vecp->v_irr_pending) {
			busy++; /* pending in source CPU's IRR reg */
		} else if (INTR_BUSY(avp)) {
			/*
			 * The interrupt handler is in-pending-queue
			 * or in-service
			 */
			busy++;
		}
...
extern void
apix_insert_isr(uchar_t irqno, uint32_t cpuid, uchar_t vector, char *name)
{
	isr_info_t *isr_info;
	isr_info_t *prep, *p;

	apic_isr_table[irqno] = name;
	APIC_VERBOSE(INTR, (CE_CONT, "apix_insert_isr: irqno %d,"
	    "cpuid %d, vector 0x%x, name %s\n",
	    irqno, cpuid, vector, name));
	for (p = isr_info_list; p != NULL; p = p->isr_link) {
		if (irqno == p->isr_irqno && 0xff != p->isr_cpu) {
			p->isr_cpu = cpuid;
			p->isr_vector = vector;
			return;
		}
	...
	isr_info = kmem_zalloc(sizeof (isr_info_t), KM_SLEEP);
	isr_info->isr_irqno = irqno;
	isr_info->isr_cpu = cpuid;
	isr_info->isr_vector = vector;
	isr_info->isr_name = ddi_strdup(name, KM_SLEEP);
	isr_info->isr_link = NULL;

	if (isr_info_list == NULL) {
		isr_info_list = isr_info;
		return;
	}
	...
	prep = NULL;
	for (p = isr_info_list; p != NULL; p = p->isr_link) {
		if (isr_info->isr_vector > p->isr_vector)
			break;
		prep = p;
	}
    ...
    dev_info_t *dip)
{
	...
	if (!APIX_IS_VIRTVEC(virt_vect)) {	/* got irq */
		 ...
		 */
		if ((vecp = apix_alloc_intx(dip, inum, irqno, name)) == NULL)
			return (0);
 ...
 */
apix_vector_t *
apix_alloc_intx(dev_info_t *dip, int inum, int irqno, char *name)
{
	apic_irq_t *irqp;
	apix_vector_t *vecp;
 ...
 */
static void
ioapix_setup_intr(int irqno, iflag_t *flagp, char *name)
{
	...
	if ((irqp != NULL) && (irqp->airq_mps_intr_index == ACPI_INDEX)) {
		...
		vecp = apix_alloc_intx(NULL, 0, irqno, name);

...
/*
 * Initialize IRM pool for APIC interrupts if the PSM module
...
apix_irm_init(void)
{
	 ...
	 * Unless the value was tuned, in which case retain
	 * the tuned value.
	 *
	 ...
	 */
	ddi_limit = ddi_msix_alloc_limit;
	if (!ddi_msix_alloc_tuned)
	...
	if (ndi_irm_create(dip, &apix_irm_params,
		...
		ddi_msix_alloc_limit = ddi_limit;
	}
}
	...
	int *result)
{
	...
	ret = (*psm_intr_ops_saved)(dip, handle,
	    PSM_INTR_OP_GET_SHARED, &is_shared);

	if ((ret == PSM_SUCCESS) && (is_shared == B_TRUE)) {

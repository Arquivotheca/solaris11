...
_init(void)
{
	if (strcmp(tod_module_name, "todstarcat") == 0) {
		...
		hrt->hrestime.tv_sec = (time_t)ssc_time32;
...
/*
 * Starcat tod_get is simplified to return hrt->hrestime and to
...
todsc_set(timestruc_t ts)
{
	...
	if (abs(hrt->hrestime.tv_sec - ts.tv_sec) > TODSC_SET_THRESHOLD) {
		/*

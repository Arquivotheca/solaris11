...
static int pcitool_get_intr_info(pci_t *pci_p, ib_ino_t ino,
...
static int
pcitool_get_intr_info(pci_t *pci_p, ib_ino_t ino, pcitool_intr_get_t *iget)
{
	...
	for (; ino_p && ino_p->ino_ino != ino; ino_p = ino_p->ino_next_p)
		;
...
pcitool_get_intr(dev_info_t *dip, void *arg, int mode, pci_t *pci_p)
{
	...
	if (imregval & COMMON_INTR_MAP_REG_VALID) {
		...
		if (pcitool_get_intr_info(pci_p, ino, iget) != DDI_SUCCESS) {
			iget->status = PCITOOL_IO_ERROR;
			rval = EIO;
			goto done_get_intr;
		}

...
_init(void)
{
	...
	if (strcmp(tod_module_name, "todsg") == 0) {
		 ...
		 */
		if (TIMESPEC_OVERFLOW(&hrt->hrestime)) {
			...
			/*
			 * By setting hrt->hrestime.tv_sec to zero
			 * we force the vfs_mountroot() to set
			 * the date from the last shutdown.
			 */
			hrt->hrestime.tv_sec = (time_t)0;
			/*
...
todsg_get(void)
{
	 ...
	 * to sync the IOSRAM to hrt->hrestime when the SC comes back.  We
	 * can determine that either NTP slew (or date -a) was called if
	 * the global hrt->timedelta was non-zero at any point while the SC
	 * was away.  If hrt->timedelta remains zero throughout, then the
	 * default action will be to sync hrt->hrestime to IOSRAM
	 */
	if (seconds != pre_seconds) {	/* SC still alive */
		pre_seconds = seconds;
		if (is_sc_down >= SC_DOWN_COUNT_THRESHOLD && adjust_sc_down) {
			skew_adjust = hrt->hrestime.tv_sec -
			    (seconds + domain_skew);
			complained = update_tod_skew(0);
			if (!complained && (iosram_read(SBBC_TOD_KEY,
		 ...
		 * Hand back hrt->hrestime instead.
		 */
		if (!complained) {
			/*
...
return_hrestime:
	 ...
	 * the SC comes back up again.  Note we will return hrt->hrestime below
...
todsg_set(timestruc_t ts)
{
	...
	/*
	 * We are passed hrt->hrestime from clock.c so we need to read the

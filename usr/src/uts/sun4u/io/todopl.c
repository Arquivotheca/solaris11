 ...
 *
 * diff_delta = new time -  current domain time (hrt->hrestime)
 *
 *
...
todopl_set(timestruc_t ts)
{
	...
	if (abs(ts.tv_sec - hrt->hrestime.tv_sec) > TODOPL_SET_THRESHOLD) {
		/*
		 * Send time difference to SP
		 */
		if (enable_time_sync)
			prom_opl_set_diff(ts.tv_sec - hrt->hrestime.tv_sec);
		else {
			/*

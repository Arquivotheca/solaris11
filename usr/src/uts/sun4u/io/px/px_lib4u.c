...
px_lib_dev_init(dev_info_t *dip, devhandle_t *dev_hdl)
{
	...
	*dev_hdl = (devhandle_t)csr_base;
	 ...
	 * depends on px structure being properly initialized.
	 */
...
px_lib_msiq_init(dev_info_t *dip)
{
	...
	pxu_p->msiq_mapped_p = vmem_xalloc(px_p->px_mmu_p->mmu_dvma_map,
	    P2ROUNDUP(size, px_dvma_top_cache_quantum),
	    P2ROUNDUP(px_dvma_top_cache_quantum, 512*1024), 0, 0, NULL,
	    NULL, VM_NOSLEEP | VM_BESTFIT);

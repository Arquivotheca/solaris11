...
i_cpr_find_ppages(void)
{
	...
	for (pp = promvp.v_pages; pp; ) {
		if (cpr_setbit(btop(pp->p_offset), mapflag) == 0)
			vcnt++;
		pp = pp->p_vpnext;
...
i_cpr_is_supported(int sleeptype)
{
	...
	/*
	 * Sparc only supports S4 (i.e. Suspend to Disk).
	 */
	if ((sleeptype != SYSTEM_POWER_S4) && !pm_cpr_state_override)
		return (0);

	/*
	 * Verify that we have a valid capability for this sleeptype.
	 */
	...
	/*
	 * Check if the configuration has specifically turned on cpr support
	 */
	if (pm_suspend_enabled)
		return (1);

	/*
	 * Otherwise, check the energystar property and set appropriately
	 */
	node = prom_rootnode();
...
i_cpr_power_down(void)
{
	...
	/*
	 * On a Sparc machine, only S4 is supported.
	 */
	ASSERT(pm_cpr_state == SYSTEM_POWER_S4);
	...
	if (!setjmp(&tlwp->lwp_qsav)) {
		...
		if (cpr_reusable_mode) {
			/*
			 * If an error occured, reset the properties
			 * and return, otherwise pretend this is a resume.
			 */
			if (ret) {
		...
		/* If a dump error, reset properties and return the error */
		if (ret) {
			cpr_set_substate(C_ST_SETPROPS_0);
			(void) cpr_set_properties(0);
			return (ret);
		} else
			if ((ret = cpr_set_properties(1)) != 0)
				return (ret);

		/*
		 * Now to get the machine properly "halted".
		 *
		 * If the one of the restart cases, call mdboot,
		 * it will handle the cpu's, devtree, etc.
		 */
		if (CPR->c_fcn == AD_CPR_TESTZ ||
		    CPR->c_fcn == AD_CPR_TESTNOZ) {
			mdboot(0, AD_BOOT, "", B_FALSE);
			/* NOTREACHED */
		}

		/*
		 * Make sure there are no more changes to the
		 * device tree.  Note that this is after we have
		 * saved the statefile, so this state won't be
		 * retained if the resume was from a re-loaded
		 * statefile.  However, if Sparc is ever configured
		 * to resume from a RAM image, this will need to
		 * either be changed, or the resume code will
		 * need to make sure that the devtree is unfrozen.
		 */
		PMD(PMD_SX, ("cpr: dev tree freeze\n"))
		devtree_freeze();

		/*
		 * stop other cpus and raise our priority.  since
		 * there is only one active cpu after this, and
		 * our priority will be too high for us to be
		 * preempted, we're essentially single threaded
		 * from here on out.
		 */
		...
		/*
		 * try and reset leaf devices.  reset_leaves()
		 * should only be called when there are no
		 * other threads that could be accessing devices
		 */
		PMD(PMD_SX, ("cpr: reset leaves\n"))
		reset_leaves();

		/*
		 * The best method is to let the prom 'power-off',
		 * unless the caller specifically requested the halt.
		 * So use the OBP, and let halt() happen if it fails.
		 */
		if (CPR->c_fcn != AD_CPR_TESTHALT) {
			/*
			 * is_defined has value -1 when defined
			 */
			prom_interpret(wordexists,
			    (uintptr_t)&is_defined, 0, 0, 0, 0);
			if (is_defined) {
				CPR_DEBUG(CPR_DEBUG1,
				    "\ncpr: %s...\n", req);
		...
		/*
		 * Halt the machine if we get to this point.
		 */
		...
		PMD(PMD_SX, ("i_cpr_power_down: return from longjump\n"))
	}
	return (0);
}
...
i_cpr_checkargs(int fcn, void *mdep)
{
	...
	/*
	 * sun4u *only* supports an S4 state AND must save
	 * a statefile, so fail if it is something different.
	 */
	if (pm_cpr_state != SYSTEM_POWER_S4 || pm_cpr_save_state == 0)
		return (ENOTSUP);
...
i_cpr_get_config_path(char *path) {
	(void) strncpy(path, CPR_CONFIG, MAXPATHLEN - 1);
	return (0);
}

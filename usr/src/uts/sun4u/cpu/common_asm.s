...
0:
	...
	ldn	[%o1 + %lo(boot_hrt + HRT_HRESTIME)], %g1	! Seconds.
	ldn	[%o1 + %lo(boot_hrt + HRT_HRESTIME_NSEC)], %g2	! Nanoseconds.
...
int traptrace_use_stick;
	 ...
	 *  -- WARNING --
	 *
	 * boot_hrt MUST be on a 64-byte boundary.
	 * In addition to the primary performance motivation (having boot_hrt
	 * all on the same 64-byte cache line(s)), code here and in the
	 * GET*TIME() macros assumes that all boot_hrt members have the same
	 * high 22 address bits (so there's only one sethi).
	 */
	.seg	".data"

...
#ifndef _ASM
/*
 * Buffers will be allocated for dtlb itlb for diagnostic purpose if
 * tlb_dump_enable is set to 1. Because we need to use the physical
 * addresses of the buffers to store the tlb, they must be allocated
 * from static arena and must not straddle page boundary.
 *
 * The contents of dtlb itlb will be saved automatically to these
 * buffers by tlb_dump_debug() when the system panics.
 */
typedef struct tlb_dump_buf {
	/* buf for dtlb */
	size_t	dtlb_sz;
	void	*dtlb_va;
	uint64_t dtlb_pa;

...
/*
 * The oplmsu MUX driver provides support for multiple XSCF units on M8000/M9000
 * servers. Each XSCF unit is presented on a serial port of the domain.
 *
 * The MUX maintains a lower stream for each of these serial devices.
 * On the upper side of the stream is /dev/console. At any given time only one
 * serial device is active, the remainder being in standby state.
 * Messages are routed to the active lower_queue indicated by oplmsu_uinst.
 *
 * An XSCF unit may change status as a result of a hardware failure,
 * switchscf(8) etc. When this happens the XSCF unit signals its wish to become
 * active by sending the sequence XOFF, XON (0x13, 0x11) upstream.
 */

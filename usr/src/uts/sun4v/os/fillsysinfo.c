...
static int get_lastlevel_cache_info(md_t *, mde_cookie_t, uint64_t *,
	uint64_t *, uint64_t *);
...
static int get_lastlevel_cache_node_count(md_t *);
...
static int n_lastlevel_caches = 0;
 ...
 */
int cpr_supported_override = 0;

/*
 * Some platforms may need to support CPR.  If this variable is non-zero,
 * cpr should be enabled.
 */
...
fill_cpu(md_t *mdp, mde_cookie_t cpuc)
{
	...
	/*
	 * Obtain the LASTLEVEL cache information from MD.
	 * If "Cache" node exists, then set LASTLEVEL cache properties
	 * as read from MD.
	 * If node does not exists, then set the LASTLEVEL cache properties
	 * in individual CPU module.
	 */
	/*
	 * For YF implementation, the L3 cache is the last level in the
	 * hierarchy, instead of L2 for the prior implementations.
	 */
	if ((!get_lastlevel_cache_info(mdp, cpuc,
	    &associativity, &size, &linesize)) ||
	    associativity == 0 || size == 0 || linesize == 0) {
		if (&cpu_fiximp) {
			/*
			 * We have cpu_fiximp() for legacy N1 systems.
			 */
			cpu_fiximp(cpunode);
		} else {
			/*
			 * We are on a T2 or better system and
			 * get_lastlevel_cache_info() failed, we will print
			 * an error and panic
			 */
			cmn_err(CE_PANIC,
			    "Failed to get last level cache info!\n");
		}
	} else {
		/*
		 * Do not expect LASTLEVEL cache properties to be bigger
	...
	cpunode->lastlevel_cache_mapping = NO_MAPPING_FOUND;
...
static int
get_lastlevel_cache(md_t *mdp)
{
	mde_cookie_t	*cachenodes;
	int	n_cachenodes;
	int	i;
	uint64_t	cache_level;
	uint64_t	lastlevel_cache = 0;

	n_cachenodes = md_alloc_scan_dag(mdp, md_root_node(mdp), "cache",
	    "fwd", &cachenodes);

	for (i = 0; i < n_cachenodes; i++) {
		if (md_get_prop_val(mdp, cachenodes[i], "level", &cache_level))
			continue;

		if (cache_level > lastlevel_cache)
			lastlevel_cache = cache_level;
	}

	md_free_scan_dag(mdp, &cachenodes);

	return (lastlevel_cache);
}

/*
 * Use LASTLEVEL cache node to derive the chip mapping.
 */
void
setup_chip_mappings(md_t *mdp)
{
	...
	processorid_t 	cpuid;
	int 		idx = 0;
	uint64_t	lastlevel_cache = get_lastlevel_cache(mdp);

	ncache = md_alloc_scan_dag(mdp, md_root_node(mdp), "cache",
	    "fwd", &cachelist);

	/*
	 * The "cache" node is optional in MD, therefore ncaches can
	 * be 0.
	 */
	if (ncache < 1) {
		return;
	...
	for (i = 0; i < ncache; i++) {
		...
		/*
		 * Found a lastlevel cache node. Find out the cpu nodes it
		 * points to.
		 */
		...
		for (j = 0; j < ncpu; j++) {
			...
			cpunodes[cpuid].lastlevel_cache_mapping = idx;
		}
...
cpu_setup_common(char **cpu_module_isa_set)
{
	...
	if (broken_md_flag) {
		/*
		 * Turn on the missing bits supported by sun4v architecture
		 * in MMU pagesize mask returned by MD.
		 */
	...
	/* setup lastlevel cache count. */
	n_lastlevel_caches = get_lastlevel_cache_node_count(mdp);
...
get_q_sizes(md_t *mdp, mde_cookie_t cpu_node_cookie)
{
	...
	cpu_rq_entries = get_single_q_size(mdp, cpu_node_cookie,
	    "q-resumable-#bits", DEFAULT_CPU_RQ_ENTRIES, MAX_CPU_RQ_ENTRIES);

	cpu_nrq_entries = get_single_q_size(mdp, cpu_node_cookie,
	    "q-nonresumable-#bits", DEFAULT_CPU_NRQ_ENTRIES,
	    MAX_CPU_NRQ_ENTRIES);
}
...
int
lastlevel_cache_node_count(void)
{
	return (n_lastlevel_caches);
}

/*
 * count the number of lastlevel caches.
 */
int
get_lastlevel_cache_node_count(md_t *mdp)
{
	mde_cookie_t 	*cachenodes;
	int 		n_cachenodes;
	int 		lastlevel_caches = 0;
	int 		i;
	uint64_t 	cache_level;
	uint64_t	lastlevel_cache = get_lastlevel_cache(mdp);

	n_cachenodes = md_alloc_scan_dag(mdp, md_root_node(mdp),
	...
	return (lastlevel_caches);
}


/*
 * This routine returns the LASTLEVEL cache information such as --
 * associativity, size and linesize.
 */
static int
get_lastlevel_cache_info(md_t *mdp, mde_cookie_t cpu_node_cookie,

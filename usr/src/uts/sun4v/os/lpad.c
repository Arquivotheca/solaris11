...
lpad_setup(int cpuid, uint64_t pc, uint64_t arg)
{
	...
	if (lpad_dbg)
		(void) printf("copied %ld bytes of data into lpad\n", datasz);
...
lpad_alloc(void)
{
	...
	for (;;) {
		if (lpad_dbg)
			(void) printf("checking lpad pool:\n");

		/* walk the lpad buffer array */
		for (idx = 0; idx < LPAD_POOL_SIZE; idx++) {

			if (lpad_dbg)
				(void) printf("\tchecking lpad_pool[%d]\n",
				    idx);

			if (lpad_pool[idx].inuse == 0) {
				if (lpad_dbg)
					(void) printf("found empty lpad (%d)\n",
					    idx);

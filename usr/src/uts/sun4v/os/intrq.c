...
cpu_intrq_setup(struct cpu *cpu)
{
	 ...
	 */
	mcpup->mondo_data = contig_mem_alloc(sizeof (mondo_data_t));
	...
	/*
	 * Allocate the mondo sync cpuset.
	 */
	mcpup->mondo_sync_set = kmem_zalloc(sizeof (cpuset_t), KM_SLEEP);
...
cpu_intrq_cleanup(struct cpu *cpu)
{
	 ...
	 */
	if (mcpup->mondo_data) {
		contig_mem_free(mcpup->mondo_data, sizeof (mondo_data_t));
		mcpup->mondo_data = NULL;
		mcpup->mondo_data_ra = NULL;
	}

	if (mcpup->mondo_sync_set) {
		kmem_free(mcpup->mondo_sync_set, sizeof (cpuset_t));
		mcpup->mondo_sync_set = NULL;
	}

...
/*
 * Cap the current maximum number of API groups in the hv-api-groups
 * property to 16 which is more than enough to support current
 * platforms.  Hopefully if this ever needs to increase this will
 * all be replaced by a more comprehensive solution for any OBP
 * cleanup that needs to happen before it goes away.
 */
#define	MAX_OBP_HV_GROUPS 16

/*
 * Unregister any HV APIs OBP has registered that have not also
 * been registered by Solaris.  This is done when Solaris takes
 * over from OBP to ensure that OBP does not inadvertently block
 * a migration of a guest domain to another platform by leaving
 * behind a registration of an API not available on the other
 * platform.
 */
void
hsvc_boot_cleanup()
{
	...
	size_t		size;
	hsvc_t		*hsvcp;
	uint64_t	obp_api_group, supported_minor;
	int		obp_hv_groups[MAX_OBP_HV_GROUPS];
	int		nentries, i;

	node = prom_rootnode();
	if ((node == OBP_NONODE) || (node == OBP_BADNODE)) {
		cmn_err(CE_WARN, "?hsvc_boot_cleanup: cannot get OBP root "
		    "node");
		return;
	}

	/*
	 * If the hv-api-groups property is not present or there is a
	 * problem getting its value then just return.
	 */
	size = prom_getproplen(node, OBP_HV_API_GROUPS);
	if (size == -1) {
		HSVC_DEBUG(DBG_HSVC_OBP_CIF,
		    ("hsvc_boot_cleanup: prom_getproplen returned -1\n"));
		return;
	}

	if (size > sizeof (obp_hv_groups)) {
		cmn_err(CE_WARN, "hsvc_boot_cleanup: property size too big");
		return;
	}

	if (prom_getprop(node, OBP_HV_API_GROUPS, (caddr_t)obp_hv_groups) < 0) {
		HSVC_DEBUG(DBG_HSVC_OBP_CIF,
		    ("hsvc_boot_cleanup: prom_getprop failed\n"));
		return;
	}

	nentries = size / sizeof (int);

	mutex_enter(&hsvc_lock);

	for (i = 0; i < nentries; i++) {
		obp_api_group = obp_hv_groups[i];

		for (hsvcp = hsvc_groups; hsvcp != NULL; hsvcp = hsvcp->next) {
			if (obp_api_group == hsvcp->group)
				break;
		}
		...
		if (hsvcp == NULL) {
			...
			hvstat = prom_set_sun4v_api_version(obp_api_group,
			    0, 0, &supported_minor);

			HSVC_DEBUG(DBG_HSVC_OBP_CIF,
			    ("hsvc_boot_cleanup prom unreg group: 0x%lx "
			    "hvstat: 0x%lx\n", obp_api_group, hvstat));
		}
	}

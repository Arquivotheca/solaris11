 ...
 */
extern void init_mondo(xcfunc_t *func, uint64_t arg1, uint64_t arg2);
extern void send_mondo_set(cpuset_t set);
...
cpu_init_tick_freq(void)
{
	...
	extern uint_t enable_1ghz_stick;

	if (broken_md_flag) {
		/*
		 * To support some older platforms, set
		 * sys_tick_freq based on the CPU clock_freq value
		 * and return if broken_md_flag is set.
		 * There's nothing more to do because 1Ghz stick
		 * normalization is not supported on these platforms.
		 */
	...
	if (md_get_prop_val(mdp, *listp, "stick-frequency", &stick_prop) != 0) {
		if (firsttime) {
			 ...
			 * during boot.  Because the first time it's
			 * called is very early during boot before
			 * cpunodes and broken_md_flag have been been
			 * initialized, we simply return if the
			 * "stick-frequency" property cannot be
			 * retrieved from the MD.  When called a
			 * second time, we panic if the broken_md_flag
			 * is not set and the "stick-frequency"
			 * property still cannot be retrieved.
			 *
			 * When Niagara1-based systems are eol'd,
			 * this "broken" MD handling can be removed.
			 */
			firsttime = 0;
	...
	/*
	 * See if the sun4v generic cpu module is in use.
	 * If so, we treat the system as if it is booted with
	 * 1Ghz stick freq and make it invariant i.e. we always
	 * normalize the underlying HW stick to 1Ghz.
	 */
	if (enable_1ghz_stick == 0 && &cpu_gen_status)
		enable_1ghz_stick = 1;
...
send_one_mondo(int cpuid)
{
	...
	while (stat != H_EOK) {
		...
		if ((retries == 0) &&
		    (mcpup->mondo_data->md_func != (caddr_t)xt_sync_tl1)) {
			/*
			 * Add the failed CPU to a sync set. sync_mondo()
			 * will later sync to this CPU. This is one way
			 * to throttle the rate at which the current CPU
			 * sends cross traps to the failed CPU. In the
			 * absence of a throttle, mondo queues may fill up
			 * often. Under a heavy cross call load, the system
			 * may experience send mondo timeouts or other serious
			 * conditions.
			 *
			 * Note that if the caller is xt_sync(), we do not
			 * do this. This is because xt_sync() will do a sync
			 * anyway.
			 */
			if (mcpup->mondo_sync == 0)
				mcpup->mondo_sync = 1;
			sync_set = mcpup->mondo_sync_set;
			CPUSET_ADD(sync_set[0], cpuid);
		}
...
sync_mondo(cpuset_t set)
{
	...
	/*
	 * Compute the complement of set. This is used to remove this
	 * set from the mondo sync set.
	 */
	CPUSET_ALL(tmp);
	CPUSET_XOR(tmp, set);
	sync_set = mcpup->mondo_sync_set;
	...
	/*
	 * Compute the set to be synced. Then, remove that set from the sync
	 * set. This must be done with the PIL raised to XCALL_PIL so that
	 * cross trap functions executing at different PILs do not interfere
	 * with each other.
	 */
	CPUSET_AND(sync_set[0], cpu_ready_set);
	CPUSET_AND(set, sync_set[0]);
	CPUSET_AND(sync_set[0], tmp);
	if (CPUSET_ISNULL(sync_set[0])) {
		mcpup->mondo_sync = 0;
	}
	...
	/*
	 * Now sync to the specified CPUs.
	 */
...
xt_sync(cpuset_t cpuset)
{
	...
	func = (xcfunc_t *)xt_sync_tl1;
	arg1 = (uint64_t)cpu_sync.byte;
	arg2 = traptrace_id;

	XC_SPL_ENTER(lcx, opl); /* lcx set by the macro */

	XC_TRACE(XT_SOME_OTHER, &cpuset, func, arg1, arg2);
	init_mondo(func, arg1, arg2);
	send_mondo_set(cpuset);
	XC_STAT_INC(x_dstat[CPU->cpu_id][XT_SOME_OTHER]);

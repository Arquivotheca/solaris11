...
/*
 * enable_2g_pages can be set in at system startup to enable the use of the
 * 2G page size.
 */
int enable_2g_pages = 1;
...
hw_pagesize_t hw_page_array[] = {
	...
	{MMU_PAGESIZE2G, MMU_PAGESHIFT2G, 0,
	    MMU_PAGESIZE2G >> MMU_PAGESHIFT},
	{0, 0, 0, 0}
};
...
static size_t contig_mem_import_size_max	= MMU_PAGESIZE256M;
    ...
    caddr_t userlimit, struct proc *p, uint_t flags)
{
	...
	for (i = 0; i < mmu_page_sizes; i++) {
		if (disable_large_pages & (1 << i))
			continue;

		if (len >= hw_page_array[i].hp_size) {
			align_amount = hw_page_array[i].hp_size;
		}

		if (!allow_largepage_alignment &&
	...
	if (align_amount < ELF_SPARC_MAXPGSZ)  {
		/*
...
mmu_init_mmu_page_sizes(int32_t not_used)
{
	if (!init_mmu_page_sizes &&
	    (mmu_exported_pagesize_mask & (1 << TTE2G))) {
		if (enable_2g_pages) {

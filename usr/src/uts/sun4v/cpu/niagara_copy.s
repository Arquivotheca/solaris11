 ...
 *	restore t_lofault handler if non-zero
 *	exit
 *
 * do_copy:
 ...
 *       block load, fsrc1, block-init-store, block-store, fsrc1 operations
 ...
 *       block load, falign, fsrc1, block-store loop
 ...
 *
 * Programming Note: 
 *   Do NOT use fmovd instead of fsrc1.
 *   fmovd has the side effect of modifying %fsr
 *   bcopy can be called from interrupt code and must preserve %fsr
 *   See CR7007098 for example of effects of using fmovd
 *
 *
 ...
 *
 * If FP registers are used during a kcopy/bcopy called by a kernel thread,
 * kernel preemption is disabled as kernel threads do not have pcb's
 * to store the floating point state.
 */
 ...
 * LOFAULT_SET: Set for kcopy calls, clear for bcopy calls
...
/*
 * 	membar	#Sync
 *	is needed after BLD_FP_FROMSTACK before return from copy
 */
...
.kcopy_small:					! setup error handler
	...
	! We need to mark ourselves as being from kcopy since both
	! kcopy and bcopy use the same code path.
...
.copyerr:
	...
	btst	FPUSED_FLAG, %o5
	bz,pt	%xcc, 1f
	and	%o5, LOFAULT_SET, %l1		! kcopy flag to %l1

	membar	#Sync				! sync error barrier
	ldx	[%fp + STACK_BIAS - SAVED_GSR_OFFSET], %o2
	wr	%o2, 0, %gsr

	ld	[%fp + STACK_BIAS - SAVED_FPRS_OFFSET], %g5
	btst	FPRS_FEF, %g5
	bz,pt	%icc, 4f
	nop
...
4:
	...
	wr	%g5, 0, %fprs		! restore fprs
3:
	! Restore T_LOFAULT before enabling kernel preemption
	! The kcopy will always set a t_lofault handler. If it fires,
	! we're expected to just return the error code and not to
	! invoke any existing error handler. As far as bcopy is concerned,
	! we only set t_lofault if there was an existing lofault handler.
	...
	brz,pt	%l0, 1f		! Need to call kpreempt?
	nop
	call	kpreempt
	...
	nop
1:
	! FPUSED_FLAG not set, so don't modify kpreempt status
	! The kcopy will always set a t_lofault handler. If it fires,
	! we're expected to just return the error code and not to
	! invoke any existing error handler. As far as bcopy is concerned,
	! we only set t_lofault if there was an existing lofault handler.
...
.bcopy_small:					! setup error handler
	ldn	[THREAD_REG + T_LOFAULT], %o5	! save existing handler
	tst	%o5
	bz,pt	%ncc, .sm_do_copy
	! We do not set LOFAULT_SET. If no existing LOFAULT handler on
	! entry to bcopy, then on exit we don't need to restore T_LOFAULT
	sethi	%hi(.sm_copyerr), %o4
	or	%o4, %lo(.sm_copyerr), %o4	! .sm_copyerr is lofault value
	membar	#Sync				! sync error barrier
	stn	%o4, [THREAD_REG + T_LOFAULT]	! set t_lofault
...
.bcopy_more:
	...
	brz,pt	%o5, .do_copy
	nop
	! We do not set LOFAULT_SET. If no existing LOFAULT handler
	! on entry to bcopy, then on exit we don't need to restore T_LOFAULT
	sethi	%hi(.copyerr), %l7		! copyerr is lofault value
	or	%l7, %lo(.copyerr), %l7
	membar	#Sync				! sync error barrier
	stn	%l7, [THREAD_REG + T_LOFAULT]	! set t_lofault
 ...
 */
	...
	st	%g5, [%fp + STACK_BIAS - SAVED_FPRS_OFFSET]
	! if fprs.fef == 0, set it.
	! Setting it when already set costs more than checking
	btst	FPRS_FEF, %g5		! test FEF, fprs.du = fprs.dl = 0
...
.bc_fp_ready:
	...
	stx	%l5, [%fp + STACK_BIAS - SAVED_GSR_OFFSET]
...
.bc_fp_clear:
	...
	wr	%g5, 0, %fprs
.bc_ex2:
	! Whether entered from kcopy or bcopy, if %o5 is non-zero
	! then T_LOFAULT needs to be restored unless it is zero
	! if entered from kcopy (LOFAULT_SET non-zero) then
	! T_LOFAULT needs to be restored even if it was zero
	membar	#Sync			! sync error barrier
	tst	%o5
	bz,pt	%ncc, 1f
	andn	%o5, COPY_FLAGS, %o5	! remove flags from lofault address
...
1:
	...
	ldn	[THREAD_REG + T_CPU], %l0
	ldub	[%l0 + CPU_KPRUNRUN], %l0
	brz,pt	%l0, 2f	! Need to call kpreempt?
	nop
	call	kpreempt
	rdpr	%pil, %o0		! pass %pil
2:
 ...
 */
	...
	ld	[%fp + STACK_BIAS - SAVED_GSR_OFFSET], %l5
	wr	%l5, 0, %gsr		! restore gsr

	ld	[%fp + STACK_BIAS - SAVED_FPRS_OFFSET], %o5
	btst	FPRS_FEF, %o5
	bz	%icc, 4f
	nop
 ...
 */
	...
	st	%o5, [%fp + STACK_BIAS - SAVED_FPRS_OFFSET]
	! if fprs.fef == 0, set it.
	! Setting it when already set costs more than checking
	btst	FPRS_FEF, %o5		! test FEF, fprs.du = fprs.dl = 0

...
#define	CPU_LASTLEVEL_CACHEID_INVALID	-1

#define	INTR_REPORT_SIZE	64

typedef struct mondo_data {
	caddr_t		md_func;
	caddr_t		md_arg1;
	caddr_t		md_arg2;
	char		md_pad[INTR_REPORT_SIZE - (3 * sizeof (caddr_t))];
} mondo_data_t;
...
struct	machcpu {
	...
	mondo_data_t	*mondo_data;		/* send mondo data */
	uint64_t	mondo_data_ra;		/* mono data pa */
	int		mondo_sync;		/* sync flag */
	void		*mondo_sync_set;	/* CPUs that need sync */

...
#define	VGEN_LDC_UNREG_CB_DELAY	100	/* usec delay between unreg retries */
...
#define	VGEN_TX_REFHOLD(ldcp)						\
{									\
	atomic_inc_32(&(ldcp)->tx_refcnt);				\
	ASSERT((ldcp)->tx_refcnt != 0);					\
}

#define	VGEN_TX_REFRELE(ldcp)						\
{									\
	ASSERT((ldcp)->tx_refcnt != 0);					\
	atomic_dec_32(&(ldcp)->tx_refcnt);				\
}
...
typedef struct vgen_ldc {
	...
	uint32_t		tx_refcnt;	   /* # of threads in tx */
...
/* Argument for task to process channel resets */
typedef struct vgen_reset_evt {
	int		flags;	/* Additional info about reset processing */
	vgen_ldc_t	*ldcp;	/* Channel being reset */
} vgen_reset_evt_t;
...
typedef struct vgen {
	...
	ddi_taskq_t		*rst_taskq;	/* LDC reset taskq */

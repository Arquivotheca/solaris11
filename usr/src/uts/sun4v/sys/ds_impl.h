...
typedef struct ds_port {
	...
	taskq_t		*taskq;		/* per-port taskq */
} ds_port_t;

#define	IS_DS_PORT(port)	1	/* VBSC code compatibility */
...
typedef struct ds_svc {
	...
	int		drvi;		/* driver instance */
	void		*drv_psp;	/* driver per svc ptr */
	ds_port_t	*rport;		/* port with pending registration */
} ds_svc_t;
...
/* VBSC code compatibility related defines */
...
extern int ds_sys_dispatch_func(ds_port_t *port, void (func)(void *),
    void *arg);
...
int ds_unreg_hdl_locked(ds_svc_hdl_t hdl);

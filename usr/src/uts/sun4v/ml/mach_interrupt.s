...
sfmmu_shctx_cpu_mondo_patch:
	...
	ble,pn %xcc, 1f			! or the primary context is invalid
	  nop
	set	INVALID_CONTEXT, %g4	! Invalidate contexts - compatibility
 ...
 */
	...
	set	CPU_RQ_SIZE_OFF, %g7
	ldx	[%g1 + %g7], %g7		! %g7 = queue size

2:
	set	CPU_RQ_BASE_OFF, %g4
	...
	CPU_ADDR(%g1, %g4)			! reacquire cpu struct addr

	set	CPU_RQ_SIZE_OFF, %g7
	ldx	[%g1 + %g7], %g7		! %g7 = queue size
	sub	%g7, 1, %g5			! %g5 = queue size mask
	...
	/*
	 * Call sys_trap at PIL 14 unless we're already at PIL 15. %g2 is
 ...
 */
	...
	set	CPU_NRQ_SIZE_OFF, %g7
	ldxa	[%g1 + %g7]ASI_MEM, %g7		! %g7 = NR queue size

2:
	set	CPU_NRQ_BASE_OFF, %g4
	...
	CPU_PADDR(%g1, %g4)			! reacquire cpu struct addr

	set	CPU_NRQ_SIZE_OFF, %g7
	ldxa	[%g1 + %g7]ASI_MEM, %g7		! %g7 = queue size
	sub	%g7, 1, %g5			! %g5 = queue size mask

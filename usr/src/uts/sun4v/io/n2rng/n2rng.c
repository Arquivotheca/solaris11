...
static int	n2rng_init_rng_api(n2rng_t *);
static int	n2rng_register_rng_api(void);
static void	n2rng_unregister_rng_api(void);

static const char *n2rng_error_decode_cb(int);
static int	n2rng_pre_suspend_cb(void);
static int	n2rng_post_suspend_cb(void);
static void	n2rng_set_suspend_callbacks(void);
...
errorexit:
	...
	if (rng_hsvc_available == B_TRUE) {
		(void) hsvc_unregister(&rng_hsvc[rng_version_idx]);
...
n2rng_init_ctl(n2rng_t *n2rng)
{
	...
	if (n2rng->n_is_fips == B_TRUE) {
		...
		if (fips_check_module("drv/n2rng",
		    B_FALSE /* is_optional? */) != 0) {
...
static const char *
n2rng_error_decode_cb(int error)
{
	switch (error) {
	case EBUSY:
		return ("Unable to unregister n2rng crypto provider");

	case ENOTSUP:
		return ("RNG HV API version mismatch");

	case EIO:
		return ("Unable to register n2rng crypto provider");
...
/*
 * The pre- and post- suspend callbacks are called as part of process to
 * suspend a guest domain.  They are not used by and do not apply to a
 * control domain.
 */
/*ARGSUSED*/
static int
n2rng_pre_suspend_cb(void)
{
	n2rng_t *n2rng;

	n2rng = (n2rng_t *)ddi_get_soft_state(n2rng_softstate, 0);
	...
	/* unregister with KCF---also tears down FIPS state */
	if (n2rng_uninit(n2rng) != DDI_SUCCESS) {
		cmn_err(CE_WARN, "n2rng_pre_suspend_cb: unable to unregister "
		    "from KCF");
		return (EBUSY);
	}
...
n2rng_post_suspend_cb(void)
{
	...
	case EINVAL:
		/*
		 * The rng API is not available on this platform
		 * so don't try to reconfigure the RNG.
		 */
		return (0);

	case ENOTSUP:
		/*
		 * The rng API is available but there is a major number
		 * mismatch or the API has already been registered with a
		 * minor number greater than the one we requested.
	...
	/* Re-configure the RNG hardware and register with KCF */
	rv = n2rng_config(n2rng);

	return (rv == DDI_SUCCESS ? 0 : EIO);
}

static void
n2rng_set_suspend_callbacks(void)
{
	extern const char *(*n2rng_suspend_error_decode)(int);
	extern int (*n2rng_suspend_pre_callback)(void);
	extern int (*n2rng_suspend_post_callback)(void);

	n2rng_suspend_error_decode = &n2rng_error_decode_cb;
	n2rng_suspend_pre_callback = &n2rng_pre_suspend_cb;
	n2rng_suspend_post_callback = &n2rng_post_suspend_cb;
}

static int
n2rng_init_rng_api(n2rng_t *n2rng)
{
	uint64_t	rng_minor_ver;
	int		v, rv;

	for (v = 0; v < NVERSIONS; v++) {
		rv = hsvc_register(&rng_hsvc[v], &rng_minor_ver);
		if (rv == 0)
			break;

		DBG4(n2rng, DCHATTY,
		    "n2rng_init_rng_api: grp: 0x%lx, maj: %ld, min: %ld, "
		    "errno: %d",
		    rng_hsvc[v].hsvc_group, rng_hsvc[v].hsvc_major,
		    rng_hsvc[v].hsvc_minor, rv);
	}
	if (v == NVERSIONS) {
		for (v = 0; v < NVERSIONS; v++) {
			cmn_err(CE_WARN,
			    ...
			    rng_hsvc[v].hsvc_modname,
			    rng_hsvc[v].hsvc_group,
			    rng_hsvc[v].hsvc_major,
			    rng_hsvc[v].hsvc_minor, rv);
		}
		return (-1);
	}
	...
	DBG2(n2rng, DATTACH,
	    "n2rng_init_rng_api: RNG API VERSION (%ld.%ld)",
	    rng_hsvc[rng_version_idx].hsvc_major, rng_minor_ver);
	n2rng->n_hvapi_major_version  = rng_hsvc[rng_version_idx].hsvc_major;
	n2rng->n_hvapi_minor_version = (uint_t)rng_minor_ver;

	return (0);
}

int
n2rng_register_rng_api(void)
{
	int			rv;
	uint64_t		rng_minor_ver;
	...
	rv = hsvc_register(&rng_hsvc[rng_version_idx], &rng_minor_ver);
	if (rv == 0) {
		rng_hsvc_available = B_TRUE;
	}
...
void
n2rng_unregister_rng_api(void)
{
	int	rv;

	if (rng_hsvc_available == B_TRUE) {
		rv =  hsvc_unregister(&rng_hsvc[rng_version_idx]);
		if (rv == EINVAL) {
			cmn_err(CE_WARN, "n2rng_unregister_rng_api: rng api "
			    "not registered");
		}
		rng_hsvc_available = B_FALSE;
	}

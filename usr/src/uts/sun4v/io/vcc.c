...
#define	VCC_LDC_RETRIES		100
#define	VCC_LDC_DELAY		1000 /* usec */

static int vcc_ldc_retries = VCC_LDC_RETRIES;
...
i_vcc_ldc_init(vcc_t *vccp, vcc_port_t *vport)
{
	...
	if ((rv = ldc_init(vport->ldc_id, &attr, &(vport->ldc_handle))) != 0) {
		cmn_err(CE_CONT, "i_vcc_ldc_init: port %u ldc channel %lu"
	...
	if ((rv = ldc_reg_callback(vport->ldc_handle, vcc_ldc_cb,
	    (caddr_t)vport)) != 0) {
		cmn_err(CE_CONT, "i_vcc_ldc_init: port@%u ldc_register_cb"
		    "failed\n", vport->number);
	...
	if ((rv = ldc_open(vport->ldc_handle)) != 0) {
		cmn_err(CE_CONT, "i_vcc_ldc_init: port@%u inv channel 0x%lu\n",
	...
	if ((rv = ldc_status(vport->ldc_handle, &vport->ldc_status)) != 0) {
		cmn_err(CE_CONT, "i_vcc_ldc_init: port@%u ldc_status failed\n",
		    vport->number);
...
i_vcc_ldc_fini(vcc_port_t *vport)
{
	...
	while ((rv = ldc_close(vport->ldc_handle)) == EAGAIN) {
		...
		if (++retry > vcc_ldc_retries) {
			/*
			 * Closing the LDC channel has failed. Ideally we
			 * should return an error to the caller, but there
			 * is no Zeus level infrastructure to handle this.
			 * The MD has already been changed and we have to
			 * close the channel.
			 */
...
i_vcc_config_port(vcc_t *vccp, uint_t portno, uint64_t ldc_id)
{
	...
	if ((portno >= VCC_MAX_PORTS) || (portno == VCC_CONTROL_PORT)) {
		cmn_err(CE_CONT, "i_vcc_config_port: invalid port number %u\n",
		    portno);
		return (EINVAL);
	}
	...
	if ((vport->status & VCC_PORT_AVAIL) == 0) {
		cmn_err(CE_CONT, "i_vcc_config_port: port@%u does not exist\n",
		    portno);
		return (EINVAL);
	}
	...
	if (vport->ldc_id != VCC_INVALID_CHANNEL) {
		cmn_err(CE_CONT, "i_vcc_config_port: port@%u channel already"
		    "configured\n", portno);
		return (EINVAL);
	}
    ...
    uint_t portno, char *domain_name)
{
	...
	if (vport->status & VCC_PORT_AVAIL) {
		...
		cmn_err(CE_CONT, "i_vcc_add_port: invalid port - port@%u "
		    "exists\n", portno);
		return (MDEG_FAILURE);
	}
...
vcc_open(dev_t *devp, int flag, int otyp, cred_t *cred)
{
	...
	if (vport->status & VCC_PORT_OPEN) {
		...
		cmn_err(CE_CONT, "vcc_open: virtual-console-concentrator@%d:%u "
		    "is already open\n", instance, portno);
		mutex_exit(&vport->lock);
		return (EAGAIN);
	}
...
i_vcc_close_port(vcc_port_t *vport)
{
	...
	while (vport->status & VCC_PORT_LDC_CHANNEL_READY) {
		if (ldc_unreg_callback(vport->ldc_handle) == EWOULDBLOCK) {
			if (++retry > vcc_ldc_retries) {
				cmn_err(CE_CONT, "i_vcc_close_port: cannot "
				    "unregister callback %ld\n", vport->ldc_id);
				break;
			}

			/*
			 * There is an ongoing callback preventing
			 * the callback unregistration. Temporarily
			 * drop the port lock to allow the callback
			 * to finish.
			 */
			mutex_exit(&vport->lock);
			delay(USEC_TO_TICK(VCC_LDC_DELAY));
			mutex_enter(&vport->lock);
			...
			/*
			 * Either callback unregisration succeeded
			 * or no callback was configured.
			 */
			DTRACE_PROBE1(ldc__unreg__retries, uint32_t, retry);
...
i_vcc_md_remove_port(md_t *mdp, mde_cookie_t mdep, vcc_t *vccp)
{
	...
	if ((portno >= VCC_MAX_PORTS) || (portno < 0)) {
		cmn_err(CE_CONT, "i_vcc_md_remove_port@%lu invalid port no\n",
		    portno);
		return (MDEG_FAILURE);
	}

	if (portno == VCC_CONTROL_PORT) {
		cmn_err(CE_CONT, "i_vcc_md_remove_port@%lu can not remove"
		    "control port\n",
		    portno);
		return (MDEG_FAILURE);
	}
...
i_vcc_md_add_port(md_t *mdp, mde_cookie_t mdep, vcc_t *vccp)
{
	...
	if (md_get_prop_str(mdp, mdep, "vcc-domain-name", &domain_name)) {
		cmn_err(CE_CONT, "i_vcc_md_add_port: port%lu has "
	...
	if (md_get_prop_str(mdp, mdep, "vcc-group-name", &group_name)) {
		cmn_err(CE_CONT, "i_vcc_md_add_port: port%lu has no "

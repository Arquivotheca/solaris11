...
/*
 * Create a pool of mblks from which future vio_allocb() requests will be
 * serviced.  The num_mblks argument must be non-zero and a power-of-2. The
 * size of mblks to be allocated must also be specified. The client can
 * optionally provide the data buffer area(mblk_datap) for the mblks (allocated
 * by kmem_alloc()). If mblk_datap is provided, its size must be >=
 * (num_mblks * mblk_size).
 *
 * Returns
 *	0 on success
...
/*
 * Destroy the pool of mblks. This can only succeed when all allocated mblks
 * have been returned to the pool. It is up to the caller to ensure that no
 * further mblks are requested from the pool after destroy has been invoked.
 * We also free the pool data buffer area (vmplp->datap); the client should
 * not free this even if it was provided by the client during the creation of
 * the pool (VMPL_FLAG_CLIENT_DATA).
 *
 ...
 *
 * Assumption:
 *	This function assumes that backward compatibility is not broken in

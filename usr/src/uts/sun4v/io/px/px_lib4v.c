...
#define	PF_EMULATION_NODE_NAME	80
...
hv_negotiation_complete:
	...
	/* Virtual fabric doesn't support fixed interrupt. */
	if (!bus_p->virtual_fabric)
...
px_lib_do_count_waiting_dev(dev_info_t *dip, void *arg)
{
	...
	char		*dev_path = NULL;
	boolean_t	assigned;
	int		num_vf_assigned = 0;
	uint16_t	sriov_cap_ptr;
	char		name[PF_EMULATION_NODE_NAME];
	pcie_req_id_t	pf_bdf;
	int		first_vf_offset, vf_stride;
	emulated_node_t	*enodep;
	pcie_bus_t	*bus_p;
	dev_info_t	*rcdip;

	/*
	 * Look for emulated PF devices.
	 * These will be expunged from the device tree later
	 */
	len = sizeof (name);
	rval = ddi_getlongprop_buf(DDI_DEV_T_ANY, dip, DDI_PROP_DONTPASS,
	    OBP_NAME, name, &len);
	if (strcmp(name, "SUNW,pf-emulation") == 0) {
		...
		if (bus_p == NULL)
			return (DDI_WALK_CONTINUE);
		rval = pcie_get_bdf_from_dip(dip, &pf_bdf);
		/*
		 * VFs BDF wil be in the range of:
		 * PF_BDF + (first_vf_offset) to PF_BDF + first_vf_offset +
		 * (num_vfs - 1) * vf_stride.
		 */
		num_vfs = ddi_getprop(DDI_DEV_T_ANY, dip,
		    DDI_PROP_DONTPASS, "#vfs", -1);
		if ((num_vfs == 0) || (num_vfs == -1))
			return (DDI_WALK_CONTINUE);
		first_vf_offset = ddi_getprop(DDI_DEV_T_ANY, dip,
		    DDI_PROP_DONTPASS, "first-vf-offset", -1);
		if ((first_vf_offset == 0) || (first_vf_offset == -1))
			return (DDI_WALK_CONTINUE);
		vf_stride = ddi_getprop(DDI_DEV_T_ANY, dip,
		    DDI_PROP_DONTPASS, "vf-stride", -1);
		if ((vf_stride == 0) || (vf_stride == -1))
			return (DDI_WALK_CONTINUE);
		enodep = kmem_alloc(sizeof (struct emulated_node),
		    KM_SLEEP);
		enodep->en_dip = dip;
		enodep->en_bdf = pf_bdf;
		enodep->en_vf_stride = vf_stride;
		enodep->en_start_bdf = pf_bdf + first_vf_offset;
		enodep->en_end_bdf = pf_bdf + first_vf_offset +
		    (num_vfs - 1) * vf_stride;
		/*
		 * Insert the new emulated_node
		 */
		enodep->en_next = bus_p->enodep;
		bus_p->enodep = enodep;
	}
	dev_path = kmem_alloc(MAXPATHLEN, KM_SLEEP);
	(void) pcie_pathname(dip, dev_path);
	/* check if this is an assigned device */
	if (DEVI_IS_ASSIGNED(dip)) {
		DBG(DBG_ATTACH, dip, "px_lib_do_count_waiting_dev: "
		    "Found an assigned dev %p", dip);
		(*count)++;
	} else if (pcie_cap_locate(dip, PCI_CAP_XCFG_SPC(PCIE_EXT_CAP_ID_SRIOV),
	    &sriov_cap_ptr) == DDI_SUCCESS) {
		/*
		 * found a SRIOV device.
		 * Now check if any VF for this PF is ASSIGNED
		 * We increment the count by the number of VFs assigned.
		 */
		rval = iovcfg_get_numvfs(dev_path, &num_vfs);
		if ((rval != 0) || (num_vfs <= 0)) {
			kmem_free(dev_path, MAXPATHLEN);
			return (DDI_WALK_CONTINUE);
		}
		for (i = 0; i < num_vfs; i++) {
			rval = iovcfg_is_vf_assigned(dev_path, i,
			    &assigned);
			if ((rval == 0) && assigned)
				num_vf_assigned += 1;
		}
		*count += num_vf_assigned;
	}
	kmem_free(dev_path, MAXPATHLEN);
	return (DDI_WALK_CONTINUE);
}
...
px_lib_count_waiting_dev(dev_info_t *dip)
{
	...
	/*
	 * expunge the emulated PF nodes from device tree.
	 */
	bus_p = PCIE_DIP2BUS(dip);
	if (bus_p == NULL)
		return (count);
	for (enodep = bus_p->enodep;
	    enodep && enodep->en_dip;
	    enodep = enodep->en_next) {
		if (ddi_remove_child(enodep->en_dip, 0) != DDI_SUCCESS)
			cmn_err(CE_NOTE, "Failed to remove PF emulated node\n");
		else
			enodep->en_dip = NULL;
	}
	return (count);
...
/*
 * Attach vpci driver and its parent drivers.
 *
 * The vpci driver and its parent drivers are already loaded during
 * early boot stage before bopops is disabled.
 *
 * For IO domain, it requires to attach virtual proxy driver which
 * will start inter-domain communication channel handshake between
 * IO Domain and Root Domain. The communication channel will be used
 * by px driver itself.
 *
 * Under Root Domain, if there is a device assigned to IO Domain,
 * it requires to force attach virtual proxy driver which will get
 * device assigment information by parsing the MDs watched by the
 * driver.
 *
 * It is possible that we are running on a non-LDOM firmware or the
 * domaining is not enabled. In these cases, we should not attach
 * vpci driver.
 */
/*ARGSUSED*/
void
px_lib_vpci_driver_attach(dev_info_t *dip)
{
	if (domaining_supported()) {
		if (ddi_hold_installed_driver(ddi_name_to_major("vpci"))
		    == NULL)
			DBG(DBG_ATTACH,
			    dip, "Can't load vpci driver\n");
		pcie_scan_assigned_devs(dip);
	}
}
...
px_lib_fabric_sync(dev_info_t *dip)
{
	...
	if ((rcdip == NULL) || ((px = (px_t *)DIP_TO_STATE(rcdip)) == NULL))
		return (DDI_FAILURE);
	/*
	 * We expect to come here exactly once for each of the ASSIGNED device.
	 * Also it is expected that we will be called at the time of
	 * pcie_initchild()
	 * The device should not be in attached state when we are invoked.
	 * A Check for device's attached state is redundant now since
	 * ASSIGNED devices are NOT attached in primary domain, but it will
	 * guard against any future changes that might permit attach of
	 * ASSIGNED devices.
	 */
	...
	if (waitdev <= 0) {
		DBG(DBG_CTLOPS, dip,
		    "HV has already been Notified that RC %p is ready\n",
		    rcdip);
		return (DDI_FAILURE);
	}

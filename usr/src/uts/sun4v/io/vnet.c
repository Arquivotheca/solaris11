...
static mblk_t *vnet_rx_poll(void *arg, int bytes_to_pickup, int pkts_to_pickup);
static int vnet_addmac(void *arg, const uint8_t *mac_addr, uint64_t);
...
int vnet_hio_mac_init(vnet_t *vnetp, char *macname);
...
extern mblk_t *vgen_rx_poll(void *arg, int bytes_to_pickup, int pkts_to_pickup);
...
vnet_m_capab(void *arg, mac_capab_t cap, void *cap_data)
{
	...
	case MAC_CAPAB_RINGS: {
		...
		/*
		 * Set the version for the rings capability in use.
		 */
 ...
 */
static mblk_t *
vnet_rx_poll(void *arg, int bytes_to_pickup, int pkts_to_pickup)
{
	vnet_pseudo_rx_ring_t	*rx_ringp = (vnet_pseudo_rx_ring_t *)arg;
	mblk_t			*mp = NULL;
	vnet_res_t		*vresp;
...
static int
vnet_addmac(void *arg, const uint8_t *mac_addr, uint64_t flags)
{
...
int
vnet_hio_mac_init(vnet_t *vnetp, char *macname)
{
	mac_handle_t		mh;
	...
	do {
		rv = mac_open(macname, &mh);
		if (rv == 0) {
			break;
		}
		if ((rv != ENOENT && rv != EBUSY) ||
		    (retries++ >= vnet_mac_open_retries)) {
			mac_free(macp);
			return (rv);
		}
...
vnet_bind_hwrings(vnet_t *vnetp)
{
	...
	for (i = 0; i < VNET_NUM_HYBRID_RINGS; i++) {
		...
		mac_hwring_setup(rx_ringp->hw_rh,
		    (mac_resource_handle_t)rx_ringp, rx_ringp->handle);
	...
	for (i = 0; i < tx_grp->ring_cnt; i++) {
		...
		mac_hwring_setup(tx_ringp->hw_rh,
		    (mac_resource_handle_t)tx_ringp, tx_ringp->handle);

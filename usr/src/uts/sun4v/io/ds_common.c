...
ds_ldc_cb(uint64_t event, caddr_t arg)
{
	...
	if (event & LDC_EVT_READ) {
		if (port->ldc.state != LDC_UP) {
			DS_DBG_LDC(CE_NOTE, "ds@%lx: %s: LDC READ event while "
			    "port not up" DS_EOL, PORTID(port), __func__);
			goto done;
		}

		if (ds_sys_dispatch_func(port, ds_handle_recv, port)) {
...
ds_handle_recv(void *arg)
{
	...
	for (;;) {
		...
		if (ds_sys_dispatch_func(port, ds_dispatch_event, devent)) {
	...
	do {
		...
		if (rv != 0) {
			if (rv == ECONNRESET) {
				...
				(void) ds_sys_dispatch_func(port,
				    (void (*)(void *))
				    ds_handle_down_reset_events, port);
				return (rv);
...
ds_send_reg_req(ds_svc_t *svc, ds_port_t *port)
{
	...
	if ((rv = ds_send_msg(port, msg, msglen)) != 0) {
		...
		/*
		 * svc->port is not set until the registration is completed.
		 * Until then rport is set so that a port reset can clear the
		 * service state.
		 */
...
ds_svc_unregister(ds_svc_t *svc, void *arg)
{
	...
	if (svc->port != port && svc->rport != port) {
		return (0);
	}
...
/*
 * Unreg handle already holding ds_svcs lock.
 */
int
ds_unreg_hdl_locked(ds_svc_hdl_t hdl)
{
	ds_svc_t	*svc;
	int		is_loopback;
	ds_svc_hdl_t	lb_hdl;
...
/*
 * Interface for ds_unreg_hdl in vlds driver.
 */
int

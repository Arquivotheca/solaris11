...
vsw_port_delete(vsw_port_t *port)
{
	...
	/*
	 * Now that all tasks and callbacks have completed, destroy
	 * any resources they might have accessed.
	 */
...
vsw_conn_task(void *arg)
{
	...
	/* Stop transmits as the channel is being reset */
	mutex_enter(&portp->tx_lock);
	portp->transmit = NULL;
	mutex_exit(&portp->tx_lock);
...
vsw_next_milestone(vsw_ldc_t *ldcp)
{
	...
	case VSW_MILESTONE3:
		/*
		 * Pass this milestone when all parameters have been
...
/*
 * Break up frames into 2 separate chains: normal and
...
vsw_ldcsend_pkt(vsw_ldc_t *ldcp, mblk_t *mp)
{
	...
	/*
	 * We need to check the relevant lstate here. In RxDring mode, check
	 * if lane_in lstate is active, in TxDring mode, check lane_out.
	 */
	if ((ldcp->lane_out.dring_mode == VIO_TX_DRING &&
	    !(ldcp->lane_out.lstate & VSW_LANE_ACTIVE)) ||
	    (ldcp->lane_out.dring_mode == VIO_RX_DRING_DATA &&
	    !(ldcp->lane_in.lstate & VSW_LANE_ACTIVE)) ||
...
vsw_descrsend(vsw_ldc_t *ldcp, mblk_t *mp)
{
	...
	if (size < (size_t)ETHERMIN) {
		/* bzero the uninitialized portion of buffer */
		bzero(bufp, (size_t)ETHERMIN - size);
		priv_desc->datalen = ETHERMIN;
	} else {
		priv_desc->datalen = size;
	}

...
static int dr_cpu_check_cpu_config_limit(dr_cpu_hdr_t *req);
...
dr_cpu_list_wrk(dr_cpu_hdr_t *req, dr_cpu_hdr_t **resp, int *resp_len)
{
	...
	/*
	 * Sanity-check a request to configure CPUs
	 */
	if ((req->msg_type == DR_CPU_CONFIGURE) &&
	    (dr_cpu_check_cpu_config_limit(req) != 0)) {
		return (-1);
	}
...
/*
 * Verify that a request to configure CPUs will not cause the total
 * number of CPUs to exceed the maximum number supported by the OS.
 */
static int
dr_cpu_check_cpu_config_limit(dr_cpu_hdr_t *req)
{
	int		idx;
	int		addcount;
	uint32_t	*cpuids;

	ASSERT(req->msg_type == DR_CPU_CONFIGURE);

	DR_DBG_CPU("dr_cpu_check_cpu_config_limit...\n");
	...
	/*
	 * Count the number of CPUs in the request that
	...
	DR_DBG_CPU("ncpu_guest_max: %ld  ncpus: %d  addcount: %d\n",
	    ncpu_guest_max, ncpus, addcount);

	if ((ncpus + addcount) > ncpu_guest_max) {
		DR_DBG_CPU("request to configure CPUs exceeds "
		    "ncpu_guest_max\n");
		return (-1);
	}

...
vsw_mac_open(vsw_t *vswp)
{
	...
	rv = mac_open_by_linkname(vswp->physname, &vswp->mh);
	...
	case EBADF:
		/*
		 * The door is not available yet - retry.  This can happen
		 * during boot up.
		 */
		return (EAGAIN);
	case ENOENT:
		/*
		 * If mac_open_by_linkname() failed and the error indicates
		 * that the device is not available yet, we need to check the
		 * case that the physname matches a device name rather than a
		 * a current link name (this can happen if the LDOM config was
		 * created prior to setting a vanity name on the physical link
		 * used for the vsw).  If mac_open() also fails, return EAGAIN
		 * to indicate that mac_open*() needs to be retried.
		 * For example, this may happen during boot if the required
		 * link aggregation groups(devices) have not been created yet.
		 */
	...
	if (mip->mi_media != DL_ETHER) {
		/*
		 * vsw does not support mac providers
		 * other than DL_ETHER as its net-dev.
		 */
		cmn_err(CE_WARN,
		    "!vsw%d: Device (%s) of media (%d) is not supported\n",
		    vswp->instance, vswp->physname, mip->mi_media);
		mac_close(vswp->mh);
		vswp->mh = NULL;
		return (EINVAL);
	}
...
vsw_tx_msg(vsw_t *vswp, mblk_t *mp, int caller, vsw_port_t *port)
{
	...
	(void) mac_tx_chain(mch, mp, 0, MAC_DROP_ON_NO_DESC, NULL);
	RW_MACCL_EXIT(vswp, port, caller);
	return (NULL);
}
...
vsw_maccl_set_bandwidth(vsw_t *vswp, vsw_port_t *port, int type, uint64_t maxbw)
{
	...
	if (maxbw == 0) {
		rv = mac_client_reset_maxbw(mch);
	} else {
		rv = mac_client_set_maxbw(mch, maxbw);
	}

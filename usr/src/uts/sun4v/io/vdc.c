...
{
	kt_did_t eio_tid, ownership_tid, msg_thr_tid;
	int	instance;
	int	rv;
	vdc_server_t *srvr;
	vdc_t	*vdc = NULL;
	...
	if (vdc->initialized & VDC_THREAD)
		msg_thr_tid = vdc->msg_proc_thr->t_did;
	mutex_exit(&vdc->lock);
...
/*
 * Function:
 *	vdc_free_empty_backup_dring
 *
 * Description:
 *	Free the backup dring of the given vDisk.
 *	Check also that this dring is empty - that there is no
 *	outstanding transaction, and give a message or panic (DEBUG mode)
 ...
 */
static void
vdc_free_empty_backup_dring(vdc_t *vdcp)
{
	vdc_local_desc_t *ldep;
	int		count;
	int		b_idx;
	...
	ASSERT(MUTEX_HELD(&vdcp->lock));
	ASSERT(vdcp->state == VDC_STATE_DETACH);

	if (vdcp->local_dring_backup == NULL) {
		/* no backup dring to release */
		return;
	}

	DMSG(vdcp, 1, "freeing backup dring (len=%d, tail=%d)\n",
	    vdcp->local_dring_backup_len, vdcp->local_dring_backup_tail);

	/*
	 * Walk the backup copy of the local descriptor ring and
	 * verify that there is no outstanding transaction.
	 */
	b_idx = vdcp->local_dring_backup_tail;
	for (count = 0; count < vdcp->local_dring_backup_len; count++) {
		ldep = &(vdcp->local_dring_backup[b_idx]);
		...
		if (!ldep->is_free) {
			io_lost++;
			DTRACE_PROBE2(lost_io, int, vdcp->instance,
			    vdc_local_desc_t *, ldep);
		}
	...
	if (io_lost > 0) {
		cmn_err(CE_WARN, "vdisk@%d: %d lost IO request%s",
		    vdcp->instance, io_lost, io_lost == 1 ? "" : "s");
	}

	/* clear up pending dring copy */
	dring_size = vdcp->local_dring_backup_len *
	    sizeof (vdcp->local_dring_backup[0]);
...
reset:
		...
		case VDC_STATE_DETACH:
			...
			/*
			 * An empty backup dring might exist. Free it now.
			 */
			vdc_free_empty_backup_dring(vdcp);
...
vdc_dioctl_rwcmd(vdc_t *vdc, caddr_t arg, int flag)
{
	...
	if (cmlb_rwcmd_copyin(&rwcmd, arg, flag))
		return (EFAULT);

...
static ds_ver_t		ps_suspend_vers[] = { { 1, 1 }, { 1, 0 } };
#define	PS_SUSPEND_NVERS	(sizeof (ps_suspend_vers) / \
				    sizeof (ps_suspend_vers[0]))
static ds_ver_t		ps_suspend_ver;
...
static ds_capability_t ps_suspend_cap = {
	...
	ps_suspend_vers,	/* vers */
	PS_SUSPEND_NVERS	/* nvers */
};
...
static void ps_suspend_caps(platsvc_suspend_req_t *msg);
...
ps_suspend_data_handler(ds_cb_arg_t arg, void *buf, size_t buflen)
{
	...
	switch (msg->type) {
	case DOMAIN_SUSPEND_SUSPEND:
		break;
	case DOMAIN_SUSPEND_CAPS:
		if (DS_VER_GTEQ(&ps_suspend_ver, 1, 1)) {
			ps_suspend_caps(msg);
			return;
		}
	default:
...
static void
ps_suspend_caps(platsvc_suspend_req_t *msg)
{
	ds_svc_hdl_t		ds_handle = ds_suspend_handle;
	platsvc_suspend_caps_t	resp_msg;
	uint_t			rv;

	resp_msg.req_num = msg->req_num;
	resp_msg.result = DOMAIN_SUSPEND_CAP_SUCCESS;
	if (&cpu_gen_status) {
		cpu_gen_status(&resp_msg.gen_major, &resp_msg.gen_minor,
		    &resp_msg.caps);
	} else {
		resp_msg.gen_major = (uint16_t)-1;
		resp_msg.gen_minor = (uint16_t)-1;
		resp_msg.caps = DOMAIN_SUSPEND_CAP_GEN_SPEED;
	}

	if ((rv = ds_cap_send(ds_handle, &resp_msg, sizeof (resp_msg))) != 0) {
		cmn_err(CE_NOTE, "suspend ds_cap_send resp failed (%d)", rv);
	}
}

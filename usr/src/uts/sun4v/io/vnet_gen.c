...
mblk_t *vgen_rx_poll(void *arg, int bytes_to_pickup, int pkts_to_pickup);
...
/* I/O Processing */
static mblk_t *vgen_portsend(vgen_port_t *portp, mblk_t *mp);
...
static int vgen_dispatch_reset_task(vgen_ldc_t *ldcp, int flags);
static void vgen_process_reset(void *arg);
static int vgen_add_ldc_cb(vgen_ldc_t *ldcp);
static int vgen_remove_ldc_cb(vgen_ldc_t *ldcp);
static int vgen_enable_ldc_cb(vgen_ldc_t *ldcp);
static int vgen_disable_ldc_cb(vgen_ldc_t *ldcp);
...
static void vgen_tx_drain_dring(vgen_ldc_t *ldcp);
...
uint32_t vgen_unreg_cb_retries = 5;	/* max # ldc_unreg_callback() retries */
...
uint32_t vgen_tx_drain_delay = 1000;	/* dealy in microsec for tx drain */
    ...
    const uint8_t *macaddr, void **vgenhdl)
{
	...
	(void) snprintf(qname, TASKQ_NAMELEN, "reset_taskq%d",
	    instance);
	if ((vgenp->rst_taskq = ddi_taskq_create(vnetdip, qname, 1,
	    TASKQ_DEFAULTPRI, 0)) == NULL) {
		cmn_err(CE_WARN, "!vnet%d: Unable to create reset task queue",
		    instance);
		goto vgen_init_fail;
	}
...
vgen_init_fail:
	...
	if (vgenp->rst_taskq != NULL) {
		ddi_taskq_destroy(vgenp->rst_taskq);
		vgenp->rst_taskq = NULL;
	}
	KMEM_FREE(vgenp);
	return (DDI_FAILURE);
}
...
{
	vgen_t			*vgenp = LDC_TO_VGEN(ldcp);
	vgen_reset_evt_t	*evtp;

	evtp = kmem_zalloc(sizeof (*evtp), KM_SLEEP);
	if (evtp == NULL) {
		cmn_err(CE_NOTE,
		    "!vnet%d: unable to allocate memory for reset event:"
		    " channel(0x%lx)\n",
		    vgenp->instance, ldcp->ldc_id);
		return;
	}

	evtp->flags = VGEN_FLAG_UNINIT;
	evtp->ldcp = ldcp;
...
vgen_handle_evt_reset(vgen_ldc_t *ldcp, vgen_caller_t caller)
{
	...
	(void) vgen_dispatch_reset_task(ldcp, VGEN_FLAG_EVT_RESET);
...
/*
 * Dispatch a task to initiate a reset or handle a reset event based on flags.
 *
 * Arguments:
 ...
 *		after resetting.
 *
 *		Note we don't dispatch task in this case but call
 *		vgen_process_reset() directly.
 */
static int
vgen_dispatch_reset_task(vgen_ldc_t *ldcp, int flags)
{
	int			rv;
	vgen_t			*vgenp = LDC_TO_VGEN(ldcp);
	vgen_reset_evt_t	*evtp;
	...
	evtp = kmem_zalloc(sizeof (*evtp), KM_NOSLEEP);
	if (evtp == NULL) {
		cmn_err(CE_NOTE,
		    "!vnet%d: unable to allocate memory for reset event:"
		    " channel(0x%lx)\n",
	...
	rv = ddi_taskq_dispatch(vgenp->rst_taskq, vgen_process_reset,
	    (void *)evtp, DDI_NOSLEEP);
	if (rv != DDI_SUCCESS) {
		cmn_err(CE_WARN, "!vnet%d: Can't dispatch reset task"
		    " for channel(0x%lx)\n",
		    vgenp->instance, ldcp->ldc_id);
		kmem_free(evtp, sizeof (*evtp));
		goto fail;
 ...
 */
static void
vgen_process_reset(void *arg)
{
	vgen_reset_evt_t	*evtp = (vgen_reset_evt_t *)arg;
	vgen_ldc_t		*ldcp = evtp->ldcp;
	int			flags = evtp->flags;
	vgen_t			*vgenp = LDC_TO_VGEN(ldcp);
	vgen_port_t		*portp = ldcp->portp;
	vgen_hparams_t  	*lp = &ldcp->local_hparams;
...
again:
	...
	/*
	 * Drain any pending callback and stop further callbacks
	 * by unregistering the channel callback handler.
	 */
	rv = vgen_remove_ldc_cb(ldcp);
	if (rv != 0) {
		/*
		 * Failed to unregister callback. This shouldn't happen; but we
		 * can't go further, so bail out. This will keep the channel in
		 * reset state.
		 */
		goto done;
	}
	...
	/*
	 * Drain any threads that may be transmitting on the dring. Note that
	 * we have already cleared the hphase above, which should prevent new
	 * transmits. We now wait for any transmits that are already in
	 * progress.
	 */
	vgen_tx_drain_dring(ldcp);
	...
	if (flags != VGEN_FLAG_UNINIT) {
		...
		/* Re-register ldc callback before bringing the channel up */
		rv = vgen_add_ldc_cb(ldcp);
		if (rv != 0) {
			LDC_UNLOCK(ldcp);
			goto done;
		}
		...
		while ((rv = ldc_close(ldcp->ldc_handle)) == EAGAIN) {
			...
			LDC_UNLOCK(ldcp);
			delay(drv_usectohz(VGEN_LDC_CLOSE_DELAY));
			LDC_LOCK(ldcp);
		}
		if (rv != 0) {
			cmn_err(CE_NOTE,
		...
		/*
		 * In the case of UNINIT, We re-register ldc callback after
		 * closing the channel. Also, we need to have the callback
		 * disabled. It will be enabled whenever vgen_ldc_init() is
		 * invoked (e.g replumb).
		 */
		(void) vgen_add_ldc_cb(ldcp);
		(void) vgen_disable_ldc_cb(ldcp);
...
/*
 * Unregister the callback function with LDC. If a callback(interrupt) is
 * pending, the LDC module sets an internal flag and fails any attempt to
 * unregister the callback with EWOULDBLOCK. We retry until unregister
 * succeeds, thus draining any pending callback during that process. This
 * function is called only while the channel reset is being handled from
 * vgen_process_reset().
 */
static int
vgen_remove_ldc_cb(vgen_ldc_t *ldcp)
{
	int	rv;
	uint_t	retries = 0;
	vgen_t	*vgenp = LDC_TO_VGEN(ldcp);
	...
	/*
	 * Must never be invoked in interrupt context.
	 */
	ASSERT(!servicing_interrupt());

	while ((rv = ldc_unreg_callback(ldcp->ldc_handle)) == EWOULDBLOCK) {
		if (++retries > vgen_unreg_cb_retries) {
			break;
		}
		delay(drv_usectohz(VGEN_LDC_UNREG_CB_DELAY));
	}
	if (rv != 0) {
		cmn_err(CE_NOTE,
		    "!vnet%d: Error(%d) unregistering callback: "
		    "channel(0x%lx)\n",
		    vgenp->instance, rv, ldcp->ldc_id);
	}
	return (rv);
...
/*
 * Register the LDC callback function.
 */
static int
vgen_add_ldc_cb(vgen_ldc_t *ldcp)
{
	int	rv;
	vgen_t	*vgenp = LDC_TO_VGEN(ldcp);

	/* Re-register ldc callback */
	rv = ldc_reg_callback(ldcp->ldc_handle, vgen_ldc_cb, (caddr_t)ldcp);
	if (rv != 0) {
		cmn_err(CE_NOTE,
		    "!vnet%d: Error(%d) registering callback: "
		    "channel(0x%lx)\n",
		    vgenp->instance, rv, ldcp->ldc_id);
	}
	return (rv);
...
/*
 * Enable LDC callbacks.
 */
static int
vgen_enable_ldc_cb(vgen_ldc_t *ldcp)
{
	int	rv;
	vgen_t	*vgenp = LDC_TO_VGEN(ldcp);
	...
	if (rv != 0) {
		cmn_err(CE_NOTE,
		    "!vnet%d: Error(%d) enabling callback: "
		    "channel(0x%lx)\n",
		    vgenp->instance, rv, ldcp->ldc_id);
	}
	return (rv);
...
/*
 * Disable LDC callbacks.
 */
static int
vgen_disable_ldc_cb(vgen_ldc_t *ldcp)
{
	int	rv;
	vgen_t	*vgenp = LDC_TO_VGEN(ldcp);
	...
	if (rv != 0) {
		cmn_err(CE_NOTE,
		    "!vnet%d: Error(%d) disabling callback: "
		    "channel(0x%lx)\n",
		    vgenp->instance, rv, ldcp->ldc_id);
	}
	return (rv);
...
vgen_ldc_reset(vgen_ldc_t *ldcp, vgen_caller_t caller)
{
	...
	rv = vgen_dispatch_reset_task(ldcp, VGEN_FLAG_NEED_LDCRESET);
...
mblk_t *
vgen_rx_poll(void *arg, int bytes_to_pickup, int pkts_to_pickup)
{
	vgen_port_t		*portp = (vgen_port_t *)arg;
	vgen_ldc_t		*ldcp = portp->ldcp;
	vgen_hparams_t		*lp = &ldcp->local_hparams;
...
vgen_tx_watchdog(void *arg)
{
	...
	if (vgen_txwd_timeout &&
		...
		/*
		 * Clear tid before invoking vgen_ldc_reset(). We don't need
		 * vgen_process_reset() to untimeout, as we will not be
		 * restarting the timer. When handshake completes and
		 * the channel is ready for data transmit/receive we start a
		 * new watchdog timer. If we are in tx flow controlled state,
		 * we also restart transmits at that time.
		 */
		mutex_enter(&ldcp->cblock);
		if (ldcp->wd_tid == 0) {
...
static void
vgen_tx_drain_dring(vgen_ldc_t *ldcp)
{
	vgen_t *vgenp = LDC_TO_VGEN(ldcp);

	while (ldcp->tx_refcnt > 0) {
		DWARN(vgenp, ldcp, "tx_refcnt: %d\n", ldcp->tx_refcnt);
		delay(drv_usectohz(vgen_tx_drain_delay));
	}
}

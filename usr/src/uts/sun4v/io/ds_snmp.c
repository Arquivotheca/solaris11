 ...
 * or not.
 *
 * The ds_snmp_lock_cv condition variable is used to resolve a potential race
 * between the interrupt handlers and the minor device close function.
 *
 * The ds_snmp_minor_pool[] is a bitmask to allocate and keep track of
 * minor numbers dynamically.
 *
 * The ds_snmp_minor_locks[] is a bitmask to lock minor device.
 */
static kmutex_t		ds_snmp_lock;
static kcondvar_t	ds_snmp_service_cv;
static kcondvar_t	ds_snmp_minor_cv;
...
static uint64_t		ds_snmp_minor_locks[DS_MINOR_POOL_SZ];  /* bitmask */
...
ds_snmp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	bzero(ds_snmp_minor_locks, DS_MINOR_POOL_SZ * sizeof (uint64_t));
...
static void
ds_snmp_lock_minor(minor_t minor)
{
	int		i, ndx;
	uint64_t	val;
	...
	while (ds_snmp_minor_locks[ndx] & val) {
		cv_wait(&ds_snmp_minor_cv, &ds_snmp_lock);
	}
	ds_snmp_minor_locks[ndx] |= val;

	mutex_exit(&ds_snmp_lock);
}

static void
ds_snmp_unlock_minor(minor_t minor)
{
	int	i, ndx;
	...
	ASSERT(ndx < DS_MINOR_POOL_SZ);
	mutex_enter(&ds_snmp_lock);
	ds_snmp_minor_locks[ndx] &= ~((uint64_t)1 << i);
	cv_broadcast(&ds_snmp_minor_cv);
	mutex_exit(&ds_snmp_lock);
}
...
ds_snmp_write(dev_t dev, struct uio *uiop, cred_t *credp)
{
	...
	DS_SNMP_DBG("ds_snmp_write: ds_cap_send(0x%lx, %lu) called.\n",
	    ds_snmp_handle, len);
	/*
	 * Calling ds_cap_send with ds_snmp_lock can result in deadlock if
	 * ds_snmp_unreg_handler is called during reset processing. ds_cap_send
	 * will fail if the channel is reset.
	 */
	handle = ds_snmp_handle;
	mutex_exit(&ds_snmp_lock);
	rv = ds_cap_send(handle, tmpbufp, len);
	if (rv != 0)
		rv = ECANCELED;
...
ds_snmp_data_handler(ds_cb_arg_t arg, void *buf, size_t buflen)
{
	...
	if (hdr.seq_num != sp->last_req_id) {
		cmn_err(CE_NOTE, "!Received DS snmp data out of sequence with "
		    "request");
		mutex_exit(&sp->lock);
		ds_snmp_unlock_minor(minor);
		return;

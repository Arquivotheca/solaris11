...
static int vdds_register_niu_api(void);
static void vdds_unregister_niu_api(void);
...
extern int vnet_hio_mac_init(vnet_t *vnetp, char *macname);
 ...
 */
static kmutex_t vdds_dev_lock;

static boolean_t vdds_hv_niu_api_registered = B_FALSE;
...
vdds_mod_fini(void)
{
	if (vdds_hv_niu_api_registered == B_TRUE) {
...
/*
 * Register HV API when NIU node is created.
 */
static int
vdds_register_niu_api(void)
{
	int i;
	int rv;
	uint64_t minor = 0;
	...
	if (!vdds_hv_niu_api_registered) {
		DBG2(NULL, "Could not register the Hypervisor NIU API.");
		return (-1);
	}

	DBG2(NULL, "HV NIU API registered: ver(%ld.%ld)",
	    niu_hsvc[niu_hsvc_index].hsvc_major, minor);

	return (0);
}

/*
 * Unregister HV API when NIU node is destroyed.
 */
static void
vdds_unregister_niu_api(void)
{
	if (vdds_hv_niu_api_registered == B_TRUE) {
		(void) hsvc_unregister(&niu_hsvc[niu_hsvc_index]);
		vdds_hv_niu_api_registered = B_FALSE;
	}
	DBG2(NULL, "HV NIU API unregistered ver(%ld.%ld)",
	    niu_hsvc[niu_hsvc_index].hsvc_major,
	    niu_hsvc[niu_hsvc_index].hsvc_minor);
}
...
vdds_process_dds_msg_task(void *arg)
{
	...
	case VNET_DDS_TASK_ADD_SHARE:
		...
		if (dip == NULL) {
			...
			(void) snprintf(vdds->hio_macname,
			    sizeof (vdds->hio_macname), "%s%d",
			    ddi_driver_name(dip), ddi_get_instance(dip));

			rv = vnet_hio_mac_init(vnetp, vdds->hio_macname);
			if (rv != 0) {

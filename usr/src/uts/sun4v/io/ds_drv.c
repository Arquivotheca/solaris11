...
/*
 * Taskq for ds_log task processing
 */
static taskq_t *ds_log_taskq;

/*
 * Per-port taskq thread max alloc limit.  The number of parallel
 * threads can be large on the control domain (essentially on the
 * order of the number of registered services times the number of
 * guest domains).  We allocate a taskq per port with the following
 * limit which needs to be at least twice the number of services
 * registered within the kernel at start of day (currently around 15).
 */
uint_t ds_taskq_max_alloc	= 256;
...
ds_init()
{
	...
	/*
	 * Create taskq for ds_log processing.
	 */
	ds_log_taskq = taskq_create("ds_log_taskq", 1, minclsyspri, 1,
	    ds_taskq_max_alloc, TASKQ_PREPOPULATE | TASKQ_DYNAMIC);
...
int
ds_sys_dispatch_func(ds_port_t *port, void (func)(void *), void *arg)
{
	ASSERT(port != NULL);
	ASSERT(port->taskq != NULL);
	return (taskq_dispatch(port->taskq, func, arg, TQ_SLEEP) == NULL);
}
...
static struct log {
	...
	boolean_t		purge_sched;	/* purge scheduled? */
} ds_log;

/* enable logging of messages */
int ds_log_enabled = 1;
...
ds_log_add_msg(int32_t dest, uint8_t *msg, size_t sz)
{
	...
	if ((ds_log.nentry > 1) && (ds_log.size >= DS_LOG_LIMIT) &&
		...
		if (taskq_dispatch(ds_log_taskq, ds_log_purge, NULL,
		    TQ_SLEEP) == NULL) {
			cmn_err(CE_NOTE, "%s: purge thread failed to start",
			    __func__);
			/*
			 * If ds_log_purge can't be dispatched, call
			 * it directly.
			 */
			ds_log_purge(NULL);
		} else {
			ds_log.purge_sched = B_TRUE;
		}
	}
    ...
    char *dom_name, int verbose)
{
	 ...
	 */
	(void) snprintf(taskq_name, sizeof (taskq_name), "ds_taskq.%ld",
	    PORTID(newport));
	newport->taskq = taskq_create(taskq_name, 1, minclsyspri, 1,
	    ds_taskq_max_alloc, TASKQ_PREPOPULATE | TASKQ_DYNAMIC);
...
ds_remove_port(uint64_t port_id, int is_fini)
{
	...
	/*
	 * Need to destroy taskq before calling ds_port_common_fini (where
	 * port lock is destroyed).
	 */
	if (port->taskq) {
		taskq_destroy(port->taskq);
		port->taskq = NULL;
	}
...
ds_unreg_all(int instance)
{
	...
	for (idx = 0; idx < ds_svcs.maxsvcs; idx++) {
		...
		if ((svc->flags & DSSF_ISUSER) != 0 && svc->drvi == instance) {
			hdl = svc->hdl;
			svc->drvi = DS_INVALID_INSTANCE;
			(void) ds_unreg_hdl_locked(hdl);

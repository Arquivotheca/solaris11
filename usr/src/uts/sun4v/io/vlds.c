...
/*
 * Lock table indexed by hashed minor number.  These locks protect
 * the sysevent channel for their respective minor device, since
 * there is a window between the sysevent channel being unbound
 * and an asynchronous callback event happening.  There may be
 * a very large number of minor devices and one global lock would
 * possibly affect performance, so we compromise by locking on
 * a hashed lock.
 *
 * vlds_sysev_lock_num will be forced to be a power of two at
 * start of day and vlds_sysev_lock_mask will be computed then.
 */
#define	VLDS_SYSEV_LOCK_NUM_DEFAULT	32
uint_t vlds_sysev_lock_num		= VLDS_SYSEV_LOCK_NUM_DEFAULT;
static uint_t vlds_sysev_lock_mask;
static kmutex_t *vlds_sysev_lock_tbl;

#define	VLDS_SYSEV_LOCK_HASH(minor)	((minor) & vlds_sysev_lock_mask)
#define	VLDS_SYSEV_LOCK_ENTER(minor)	\
	mutex_enter(&vlds_sysev_lock_tbl[VLDS_SYSEV_LOCK_HASH(minor)])
#define	VLDS_SYSEV_LOCK_EXIT(minor)	\
	mutex_exit(&vlds_sysev_lock_tbl[VLDS_SYSEV_LOCK_HASH(minor)])
...
vlds_minor_init(void)
{
	...
	/*
	 * If vlds_sysev_lock_num is zero or not a power of two,
	 * reset it to the default value.
	 */
	if (vlds_sysev_lock_num == 0 || !ISP2(vlds_sysev_lock_num))
		vlds_sysev_lock_num = VLDS_SYSEV_LOCK_NUM_DEFAULT;
	vlds_sysev_lock_mask = vlds_sysev_lock_num - 1;
	vlds_sysev_lock_tbl = kmem_zalloc(
	    vlds_sysev_lock_num * sizeof (kmutex_t), KM_SLEEP);
	for (i = 0; i < vlds_sysev_lock_num; i++)
		mutex_init(&vlds_sysev_lock_tbl[i], NULL, MUTEX_DEFAULT, NULL);
}
...
_fini(void)
{
	...
	kmem_free(vlds_sysev_lock_tbl, vlds_sysev_lock_num * sizeof (kmutex_t));
...
vlds_close(dev_t dev, int flag, int otyp, cred_t *credp)
{
	...
	VLDS_SYSEV_LOCK_ENTER(minor);
	sp = ddi_get_soft_state(vlds_statep, minor);
	if (sp == NULL) {
		VLDS_SYSEV_LOCK_EXIT(minor);
		return (ENXIO);
	}
...
int
vlds_init_sysevent(int minor, vlds_state_t *sp, uint32_t flags, int *did_initp)
{
	...
	*did_initp = 0;
	if (flags & DSSF_ANYCB_VALID) {
		...
		if ((rv = sysevent_evc_bind(evchan_name, &sp->evchan,
			...
			VLDS_SYSEV_LOCK_EXIT(minor);
			DS_DBG_VLDS(CE_NOTE, "%s: can't bind to '%s' (%d)",
    ...
    int *rvalp)
{
	...
	case VLDS_SVC_REG:
	{
		...
		if ((rv = vlds_init_sysevent(minor, sp, flags,
		    &did_init)) != 0) {
			vlds_free_ucap(&ucap);
			return (rv);
		}

		rv = ds_ucap_init(&ucap, &ds_user_ops, flags | DSSF_ISUSER,
		    minor, &hdl);
...
vlds_user_reg_cb(ds_cb_arg_t arg, ds_ver_t *ver, ds_svc_hdl_t hdl)
{
	...
	if ((flags & DSSF_REGCB_VALID) != 0) {
		...
		/*
		 * Get the service minor number and check if valid
		 */
		ds_cbarg_get_drv_info(arg, &minor);
		if (minor == DS_INVALID_INSTANCE)
			return;

		VLDS_SYSEV_LOCK_ENTER(minor);

		/*
		 * Get the softstate and check if it is valid and
		 * the sysevent channel is valid.
		 */
		sp = ddi_get_soft_state(vlds_statep, minor);
		if (sp == NULL || sp->evchan == NULL) {
			VLDS_SYSEV_LOCK_EXIT(minor);
			return;
		}
		...
		if (nvlist_alloc(&nvl, NV_UNIQUE_NAME, KM_SLEEP) ||
			...
			cmn_err(CE_WARN, "%s: failed to publish REG event",
			    __func__);
		}
		nvlist_free(nvl);
		VLDS_SYSEV_LOCK_EXIT(minor);
...
vlds_user_unreg_cb(ds_cb_arg_t arg)
{
	...
	if ((flags & DSSF_UNREGCB_VALID) != 0) {
		...
		if (nvlist_alloc(&nvl, NV_UNIQUE_NAME, KM_SLEEP) ||
		    ...
		    EVCH_SLEEP)) {
			cmn_err(CE_WARN, "%s: failed to publish UNREG callback",
			    __func__);
		}
		nvlist_free(nvl);
		VLDS_SYSEV_LOCK_EXIT(minor);
...
vlds_user_data_cb(ds_cb_arg_t arg, void *buf, size_t buflen)
{
	...
	if ((flags & DSSF_DATACB_VALID) == 0) {
		...
		if (nvlist_alloc(&nvl, NV_UNIQUE_NAME, KM_SLEEP) ||
			...
			cmn_err(CE_WARN, "%s: failed to publish DATA callback",
			    __func__);
		}
		nvlist_free(nvl);
		VLDS_SYSEV_LOCK_EXIT(minor);

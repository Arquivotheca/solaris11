...
/*
 * SCSI "Additional Sense Code" (ASC) value.
 */
...
/* Read VTOC disk label from specified LBA of a disk image */
#define	VD_DSKIMG_LABEL_READ(vd, labelp, lba) \
	vd_dskimg_rw(vd, VD_SLICE_NONE, VD_OP_BREAD, (caddr_t)labelp, \
	    (lba), sizeof (struct dk_label))

/* Write VTOC disk label to specified LBA of a disk image */
#define	VD_DSKIMG_LABEL_WRITE(vd, labelp, lba)	\
	vd_dskimg_rw(vd, VD_SLICE_NONE, VD_OP_BWRITE, (caddr_t)labelp, \
	    (lba), sizeof (struct dk_label))

/* Read EFI disk label from specified LBA of a disk image */
#define	VD_DSKIMG_EFI_LABEL_READ(vd, labelp, lba) \
	vd_dskimg_rw(vd, VD_SLICE_NONE, VD_OP_BREAD, (caddr_t)labelp, \
	    (lba), sizeof (efi_gpt_t))

/* Write EFI disk label to specified LBA of a disk image */
#define	VD_DSKIMG_EFI_LABEL_WRITE(vd, labelp, lba)	\
	vd_dskimg_rw(vd, VD_SLICE_NONE, VD_OP_BWRITE, (caddr_t)labelp, \
	    (lba), sizeof (efi_gpt_t))
...
/*
 * CDB Table that is ordered from largest Reach to smallest Reach.
 *
 * "Reach" is an informal term that is a function of the device's
 * maximum (SCSI) LBA and maximum Transfer Length.
 *
 * When issuing raw IO, vds builds a CDB that has the largest Reach
 * each vdisk can support.
 */
static struct cdbinfo {
	uint_t	sc_grpid;	/* CDB group ID */
	uint_t	sc_grpcode;	/* CDB group code */
	uint_t	sc_grpmask;	/* CDB group code mask */
} vd_cdbtbl[] = {
	{ CDB_GROUPID_4, CDB_GROUP4, SCMD_GROUP4 },
	{ CDB_GROUPID_5, CDB_GROUP5, SCMD_GROUP5 },
	{ CDB_GROUPID_1, CDB_GROUP1, SCMD_GROUP1 },
	{ CDB_GROUPID_0, CDB_GROUP0, SCMD_GROUP0}
};
...
typedef struct vd {
	...
	size_t			backend_size;	/* number of blks in backend */
	...
	uint16_t		scsi_grp_idx;	/* ref to preferred SCSI CDB */
...
/*
 * Macros to manipulate the scsi_grp_idx field.
 *
 * This field holds the index into a table (vd_cdbtbl) that identifies the
 * preferred SCSI Command Group to use when issuing raw reads or writes
 * to the vdisk backend device.
 *
 * Since zero is a valid SCSI Group ID, bias all values SET in this field
 * so that a simple test for non-zero confirms that the field has been
 * initialized.  For the GET operation, the bias is removed.
 */
#define	VD_GET_SCSI_GRP_IDX(vd)\
	((vd)->scsi_grp_idx & 0x7)

#define	VD_SET_SCSI_GRP_IDX(vd, idx)\
	((vd)->scsi_grp_idx = (idx) | 0x8)
    ...
    size_t len)
{
	...
	if (vd->volume) {
		 ...
		 * committed and not cached as this may be the case with
 ...
 *
 *	Note that caller can specify a boolean, 'clear', that determines
 *	whether this function should zero-fill the buffer ('label') before
 *	writing it.  (Note that when a zero-filled buffer is to be written,
 *	the caller must still specify CHS values (in the buffer) that allow
 *	this function to compute the backup labels' location; without an
 *	option to clear the buffer, these CHS values would be written to disk).
 *
 * Parameters:
 *	vd		- disk on which the operation is performed.
 *	label		- the data to be written.
 *	clear		- determination of whether 'label' buffer should be
 *			  zero-filled by this function before performing
 *			  the Writes.
 * Return Code:
 *	0		- success.
 ...
 */
static int
vd_dskimg_set_vtoc(vd_t *vd, struct dk_label *label, boolean_t clear)
{
	size_t blk, sec, cyl, head, cnt, nsect;

	ASSERT(VD_DSKIMG(vd));

	/* Write disk label to LBA 0. */
	if (VD_DSKIMG_LABEL_WRITE(vd, label, 0) < 0) {
	...
	/*
	 * Zero-fill the buffer containing the disk label contents if
	...
	for (cnt = 0; cnt < VD_DSKIMG_NUM_BACKUP; cnt++) {
		...
		if (VD_DSKIMG_LABEL_WRITE(vd, label, blk + sec) < 0) {
 ...
 *
 *      On the first time this function is called for this vd_t, use retry-logic
 *      that computes the preferred CDB size for the device.  The preferred
 *      size is that which has the largest "reach".  "Reach" is an informal
 *      term that is a function of the device's maximum (SCSI) LBA and maximum
 *      Transfer Length.
 *
 *      Issue an IO using a CDB with the largest Reach, and retry with a CDB
 *      having the next lower Reach, if needed.  If the device fails all CDB
 *      sizes, then return EIO.
 *
 *      Save a reference to the preferred CDB size in the vd_t and use it
 *      on all future calls to this function for the associated vd_t.
 *
 * Parameters:
 *	vd		- disk on which the operation is performed.
 *	operation	- operation to execute: read (VD_OP_BREAD) or
...
vd_do_scsi_rdwr(vd_t *vd, int operation, caddr_t data, size_t blk, size_t len)
{
	...
	int max_sectors;
	int status, rval;
	struct scsi_extended_sense  req_sense;
	struct cdbinfo *cdbp;
	uint_t max_index = sizeof (vd_cdbtbl) / sizeof (struct cdbinfo) - 1;
	uint_t cur_index;
	...
	/* Index into vd_cdbtbl[] that references the CDB with largest Reach */
	cur_index = 0;

	/*
	 * Build and execute the uscsi ioctl.
	 *
	 * If this vd_t already has a preferred CDB size, use it, else do
	 * retry-logic in order to compute and save the device's preferred
	 * CDB size.
	 */
	while (nblk) {
		...
		/* Does vd_t already have a preferred CDB size ? */
		if (vd->scsi_grp_idx != 0) {
			cur_index = VD_GET_SCSI_GRP_IDX(vd);
		}

		cdbp = &vd_cdbtbl[cur_index];

		switch (cdbp->sc_grpid) {
		case CDB_GROUPID_0:
			FORMG0ADDR(&cdb, blk);
		...
		case CDB_GROUPID_4:
			...
			FORMG4COUNT(&cdb, (uchar_t)nsectors);
			break;

		case CDB_GROUPID_5:
			FORMG5ADDR(&cdb, blk);
			FORMG5COUNT(&cdb, (uchar_t)nsectors);
			break;

		default:
			PR0("SCSI Group Id is unexpected: %d.", cdbp->sc_grpid);
			return (EIO);
		}

		ucmd.uscsi_cdblen = cdbp->sc_grpcode;
		cdb.scc_cmd |= cdbp->sc_grpmask;

		ucmd.uscsi_cdb = (caddr_t)&cdb;
		ucmd.uscsi_bufaddr = data;
		ucmd.uscsi_buflen = nsectors * vd->backend_bsize;
		ucmd.uscsi_timeout = vd_scsi_rdwr_timeout;
		ucmd.uscsi_rqbuf = (caddr_t)&req_sense;
		ucmd.uscsi_rqlen = sizeof (req_sense);
		/*
		 * Set flags so that the command is isolated from normal
		 * commands, no error message is printed, and RequestSense
		 * data is obtained from device, if needed.
		 */
		...
		if (status != 0) {
			/*
			 * If the error is Invalid Command, then this is most
			 * likely due to the device not supporting the chosen
			 * SCSI (cmd) Group.  Retry the IO using a CDB formed
			 * from a different SCSI Group (if a preferred CDB was
			 * not already computed for this vdisk).
			 */
			if (vd->scsi_grp_idx == 0 &&
			    cur_index < max_index &&
			    (req_sense.es_key == KEY_ILLEGAL_REQUEST &&
			    req_sense.es_add_code ==
			    SD_SCSI_ASC_INVALID_COMMAND_CODE)) {
				cur_index++;
				continue;
			}
			break;
		...
		/*
		 * Store table index of preferred SCSI Group ID in vd_t.
		 * Use on all future Reads/Writes to this device.
		 */
		if (vd->scsi_grp_idx == 0) {
			VD_SET_SCSI_GRP_IDX(vd, cur_index);
		}
...
vd_dskimg_validate_geometry(vd_t *vd)
{
	...
	/* Read disk label from LBA 0. */
	if (VD_DSKIMG_LABEL_READ(vd, &label, 0) < 0)
		return (EIO);
 ...
 * Description:
 *
 *	Write a zero-filled buffer to the Primary Label and all Backup Labels
 *	for a VTOC-based disk.
 *
 *	Allocate and clear a memory buffer.  Pass that buffer to
 *	vd_dskimg_set_vtoc, because it knows where to write labels to a
 *	VTOC-based disk.
 *
 * Parameters:
 *	vd		- disk to be operated on
 *
 * Return Code:
 *	0		- success.
 *	n > 0		- error, n indicates the errno code.
...
vd_dskimg_clear_vtoc(vd_t *vd)
{
	...
	/*
	 * Allocate and clear a buffer to be used to clear disk sectors
	 * defining the disk's labels.
	 *
	 * The memory is heap-allocated because our IO implementation might
	 * need to lock the underlying memory pages (see vd_dskimg_rw).
	 */
	dkl = kmem_zalloc(sizeof (*dkl), KM_SLEEP);

	/*
	 * Fill in the minimum, necessary fields in the disk label, as
	 * vd_dskimg_set_vtoc uses this info to compute the location of
	 * the backup labels.
	 */
	dkl->dkl_apc = vd->dk_geom.dkg_apc;
	dkl->dkl_ncyl = vd->dk_geom.dkg_ncyl;
	dkl->dkl_acyl = vd->dk_geom.dkg_acyl;
	dkl->dkl_nhead = vd->dk_geom.dkg_nhead;
	dkl->dkl_nsect = vd->dk_geom.dkg_nsect;

	/* write the empty buffer to Primary Label and all Backup Labels */
	rv = vd_dskimg_set_vtoc(vd, dkl, B_TRUE);
 ...
 * Description:
 *
 *	If disk sector at 'lba' is an (EFI) GPT data structure, write
 *	buffer 'gpt' to disk at 'lba'.
 *
 *	Read a sector specified by 'lba' and call vd_efi_check_gpt to
 *	confirm whether this is a GPT.  If it is, call
 *	VD_DSKIMG_EFI_LABEL_WRITE to write the GPT-sized buffer
 *	specified by 'gpt'.
 *
 * Parameters:
 *	vd 	- disk to be operated on
 ...
 */
static int
vd_dskimg_clear_efi_gpt(vd_t *vd, efi_gpt_t *gpt, diskaddr_t lba)
{
	int		rv;
	efi_gpt_t	efi_label;
	...
	if (VD_DSKIMG_EFI_LABEL_READ(vd, &efi_label, lba) < 0) {
		PR1("Read of lba %llx had err.", lba);
		return (EIO);
	}

	rv = vd_efi_check_gpt(&efi_label, vd->vdisk_bsize);
	if (rv == 0) {
		/* then sector contains a GPT */

		if (VD_DSKIMG_EFI_LABEL_WRITE(vd, gpt, lba) < 0) {
			PR1("Write at lba %llx had err.", lba);
			rv = EIO;
		}
	} else {
		/*
		 * force return-value to Success, as the specified block
		 * not having the syntax of a GPT is not a reason to
		 * tell caller that there was a fatal error.
		 */
		rv = 0;
	}
 ...
 * Description:
 *
 *	Clear all disk labels from an EFI-labeled disk.
 *
 *	For EFI, a disk label is a GPT ("GUID Partition Table Header").
 *	GPTs are found in two locations on the logical device: LBA 1 and
 *	the last LBA (eg, Capacity - 1).
 *
 *	Allocate and clear a GPT-sized heap-memory buffer.  Pass the buffer
 *	to vd_dskimg_clear_efi_gpt to both check whether the specified LBA
 *	contains a GPT, and to write the zero-filled buffer.  Call
 *	vd_dskimg_clear_efi_gpt for both LBA 1 and the Last LBA.
 *
 * Parameters:
 *	vd 	- disk on which operation is performed
 * Return Values:
 *	0	- success
 *	!0	- error code
 */
static int
vd_dskimg_clear_efi(vd_t *vd)
{
	efi_gpt_t	*gpt;
	diskaddr_t	capacity;
	diskaddr_t	lba;
	...
	/*
	 * Ensure the disk size as known to vds is up to date.  We will
	 * use vd.vdisk_size as the Capacity of the disk.  "Last LBA" will
	 * be computed relative to vdisk_size.
	 */
	if ((rv = vd_backend_check_size(vd)) != 0) {
		PR1("vd_backend_check_size err: %d", rv);
		return (rv);
	}

	/*
	 * Allocate and clear a buffer to be used to clear disk sectors
	 * defining an EFI disk's GPT data structures.
	 *
	 * The memory is heap-allocated because our IO implementation might
	 * need to lock the underlying memory pages (see vd_dskimg_rw).
	 */
	gpt = kmem_zalloc(vd->vdisk_bsize, KM_SLEEP);

	/* Clear Primary GPT. */
	lba = 1;
	if ((rv = vd_dskimg_clear_efi_gpt(vd, gpt, lba)) == 0) {

		/* Clear Backup GPT. */
		capacity = vd->vdisk_size;
		lba = capacity - 1;
		rv = vd_dskimg_clear_efi_gpt(vd, gpt, lba);
	}

	kmem_free(gpt, vd->vdisk_bsize);
...
vd_do_dskimg_ioctl(vd_t *vd, int cmd, void *ioctl_arg)
{
	...
	case DKIOCSEXTVTOC:
		...
		if (vtoc->v_sanity != VTOC_SANE ||
			...
			return (EINVAL);
		/*
		 * If the specified disk has an EFI label, erase the
		 * associated Primary and Backup labels.
		 */
		if (vd->vdisk_label == VD_DISK_LABEL_EFI) {
			if ((rc = vd_dskimg_clear_efi(vd)) != 0) {
				return (rc);
			}
		}
		...
		if ((rc = vd_dskimg_set_vtoc(vd, &label, B_FALSE)) != 0)
			return (rc);
	...
	case DKIOCSETEFI:
		...
		/*
		 * If the specified disk has a VTOC label, erase the
		 * associated Primary and Backup labels.
		 */
		if (vd->vdisk_label == VD_DISK_LABEL_VTOC) {
			if ((rc = vd_dskimg_clear_vtoc(vd)) != 0) {
				return (rc);
			}
		}
...
vd_setup_full_disk(vd_t *vd)
{
	...
	if (status != 0) {
		...
		vd->vdisk_bsize = 0;
		vd->backend_size = VD_SIZE_UNKNOWN;
		vd->backend_bsize = 0;
...
{
	size_t backend_bytes, backend_bsize, vdisk_bsize;
	...
	if (vd->file) {
		...
		rv = ldi_get_size(vd->ldi_handle[0], &backend_bytes);
		if (rv != DDI_SUCCESS) {
		...
		if (vd->vdisk_type == VD_DISK_TYPE_SLICE) {
			...
			ASSERT(vd->vdisk_type == VD_DISK_TYPE_DISK);
			backend_bytes = minfo.dki_capacity * minfo.dki_lbsize;
		}
		 ...
		 * be different from backend_bsize.
		 */
	...
	old_size = vd->backend_size;
	new_size = backend_bytes / backend_bsize;
	...
	vd->backend_size = new_size;
	vd->backend_bsize = backend_bsize;

	if (vd->file || vd->volume)
		vd->dskimg_size = backend_bytes;
	...
	if (vd->vdisk_type == VD_DISK_TYPE_SLICE) {
		if (!(vd->initialized & VD_DISK_READY)) {
			/*
			 * The vdisk is not ready yet. The partition
			 * will be setup by the init function.
			 */
			return (0);
		}
